...
3. **E2E body check** ([.repo/🎫/26/07/04/WGPU-PLAYGROUND-E2E/verify-wgpu-playgrounds-e2e.ts](.repo/🎫/26/07/04/WGPU-PLAYGROUND-E2E/verify-wgpu-playgrounds-e2e.ts)) - call the existing `assertBodyContent(page, programId)` inside `smokePlugin` so an empty 3D body fails the test instead of passing on chrome pixels.
...
3. **Camera persistence** - on nav gesture end (right/middle button up), emit `setCamera` with `{camera: {position, target, zoom/fov}}` from the orbit state so the camera survives re-renders and reloads. Add `setCamera` handlers to plugins missing them: [procedural/3d/program/rs/lib.rs](procedural/3d/program/rs/lib.rs), [lowpoly/program/rs/lib.rs](lowpoly/program/rs/lib.rs), [puzzle/3d/program/rs/lib.rs](puzzle/3d/program/rs/lib.rs) (has it), [puzzle/5d/program/rs/lib.rs](puzzle/5d/program/rs/lib.rs) (`setCamera3d`); cad/shooting already handle it. Plugins that persist camera must send position+target (shooting/cad/puzzle5d currently send only x/y/z without target - normalize to full `{position, target, fov}` via `world3d_camera_json`).
...
4. **Plugin handlers** - add `translateSelection`/`rotateSelection`/`scaleSelection` to: [procedural/3d/program/rs/lib.rs](procedural/3d/program/rs/lib.rs), [puzzle/3d/program/rs/lib.rs](puzzle/3d/program/rs/lib.rs) (translate updates object origins + proximity connect, rotate updates orientation), [cad/program/rs/lib.rs](cad/program/rs/lib.rs), [shooting/program/rs/lib.rs](shooting/program/rs/lib.rs), [puzzle/5d/program/rs/lib.rs](puzzle/5d/program/rs/lib.rs). [lowpoly/program/rs/lib.rs](lowpoly/program/rs/lib.rs) already handles `translateSelection`; add rotate/scale.
