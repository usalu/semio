...
- Puzzle 2d (`Puzzle2dPlayHierarchyPanel` ~1959): pass `highlightedIds` derived from the shell `hoveredId` (already exists at ~3176 but unused for the tree), and add `onPointerEnter`/`onPointerLeave` in `buildPuzzle2dPlayHierarchySections` to set `hoveredId`. This adds cross-surface hover at near-zero cost via the store.
...
- Make `buildCadPlayHierarchySections` build **neutral** items (drop the `hoveredKey`/`hoverTarget` highlight baking; keep `onClick` + hover handlers). Memoize the sections so they stay referentially stable when only hover changes (keyed by models digest + selection, not hover), and also produce a `highlightKey -> treeItemIds` map.
- Feed hover as `highlightedIds` on the hierarchy `TreePanelConfig`: subscribe to `pointerFocus.hover`, map it through the `highlightKey -> treeItemIds` map, and pass the resulting ids. The hierarchy tab rebuild on hover then only creates new (cheap) tab objects while `sections` stays referentially stable, so row memos skip and only the hovered row(s) re-render via the highlight store.
- Keep existing `onPointerEnter`/`onPointerLeave` (`cadPlayHierarchyHoverHandlers`) and `AppPointerFocusStore` cross-surface sync intact.

## Validation

Per repo rules, open a repo MCP ticket (reuse goal `🎯semio`) and track all touched files. Extend existing vitest files only (no new test files):
...
- puzzle 2d test: canvas hover highlights the matching hierarchy row.
Run affected package tests (`ui/react`, cad play, puzzle 2d) and runtime-verify with `[DEBUG]` logs that hovering a large hierarchy (e.g. cad play loaded model) is smooth. Close the ticket with a summary + file list.
