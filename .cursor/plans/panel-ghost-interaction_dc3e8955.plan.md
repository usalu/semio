...
- There is an inert interaction system: `InteractionContext` / `ActiveInteractionContext` + `setActiveInteraction(elementId, interactionId)` (~2271), consumed by `Slider`/`Input`/`Combobox` to fade *sibling controls to opacity 0* via `shouldFade`. `InteractionProvider` is never mounted anywhere, so nothing currently fades.
