...
- New WASM export `setStepCollapsed(stepId, collapsed)`. When collapsed, slot-member nodes are omitted from the DAG fixture built in `build_dag_fixture`/`step_to_dag_node` (hidden, not deleted) and the control step's node shows a small badge (e.g. "▸️ 3 steps"); when expanded, the `reorganize` auto-layout is extended to fan out each slot's chain near/below its owning step (like a flowchart), and a distinct dashed "slot" edge style connects the control step to each slot's first node.
