...
3. **Rewire discovery (no dual-name shims):**
   - [🟨️nx-plugin.mjs](🧰️framework/🛍️products/🦑️repo/🔨️modules/📚️library/⚡️implementations/🟦️typescript/🟨️nx-plugin.mjs): `createNodesV2: ["**/📜️script.ts", …]`.
   - [📦️index.ts](🧰️framework/🛍️products/🦑️repo/🔨️modules/📚️library/⚡️implementations/🟦️typescript/📦️index.ts): every `base !== "script.ts"`, `**/script.ts` walker, usage strings → `📜️script.ts`.
