...
- Extend `lowpoly/plugin/rs`'s existing `#[cfg(test)]` module with real coverage for each edit operation, the Rock default fixture, picking/selection resolution, and paint stroke round-trips.
- Update the ticket `.repo/🎫️/26/07/05/SUPPORT-REACT-AND-WGPU-RENDERERS-IN-PLAYGROUNDS` (reopen), run the existing React E2E sweep, and manually verify in a live browser: Rock renders by default, toolbar tools reshape the mesh, vertex/edge/face picking + highlighting works, the transform gizmo moves the mesh, paint mode brush/eraser/fill/UV-unwrap work, and undo/redo works for paint strokes.
