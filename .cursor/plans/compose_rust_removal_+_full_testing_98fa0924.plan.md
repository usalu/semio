...
- Fix the `forms.dictionary/v1` vs `forms.form/v1` mismatch found during the audit: the `forms` program's registry entry declares `sourceFormat: "forms.dictionary/v1"` ([semios/core/index.ts:275](semios/core/index.ts)) but the typed handler `createFormsAppVcsHandler` is registered for `forms.form/v1` ([forms/core/index.ts:1092](forms/core/index.ts)) — align these so `forms` actually exercises its typed handler end-to-end instead of silently falling back to a JSON-replace stub.
