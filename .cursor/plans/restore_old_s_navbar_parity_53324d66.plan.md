...
| *(theme/compact/expertise lived in a "Settings" side-panel tab, not the navbar)* | fill, search toggle (new), find toggle (new) |
...
| | *(no mode switcher in navbar -- `App` component renders an internal `Select` dropdown instead when `chrome={modes.length>1}`)* |
