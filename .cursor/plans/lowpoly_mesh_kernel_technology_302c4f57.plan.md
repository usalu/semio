...
- `lowpoly/core/index.ts` (`@semio-tech/lowpoly-core`): `LOWPOLY_FIXTURE_SCHEMA = "lowpoly.fixture"`, `LowpolyObject` (`id`, `name`, `transform`, `smoothShading`, `meshJson`), `LowpolyFixture` (`objects`, `activeObjectId`, `selection: { mode: "object"|"vertex"|"edge"|"face"; ids: number[] }`), `DEFAULT_LOWPOLY_FIXTURE` (one cube), `lowpolyFixtureToJson` / `parseLowpolyFixtureJson`.
