...
overview: Refactor `semio/rs` GraphQL generation so `semio/graphql/schema.graphql` contains no `JSON` scalar or opaque JSON fields, and expose the kit data as explicit graph nodes, edges, typed inputs, and typed events.
todos:
 - id: ticket
...
- Reopen the existing ticket `2026/04/25/UNIFY-GRAPH-QL-MUTATIONS-AND-REMOVE-JSON-SCALARS`; it is the exact prior work item for removing JSON scalars from `semio/graphql`.
- The generated schema is built from [`semio/rs/lib.rs`](semio/rs/lib.rs) through `kit_graphql::schema_sdl()` and checked against [`semio/graphql/schema.graphql`](semio/graphql/schema.graphql).
...
2. Replace schema-visible `Json` usage in [`semio/rs/lib.rs`](semio/rs/lib.rs):
...
6. Update the existing GraphQL smoke test in [`semio/rs/lib.rs`](semio/rs/lib.rs) to assert the generated SDL contains no `scalar JSON`, no `JSON` field references, and still matches [`semio/graphql/schema.graphql`](semio/graphql/schema.graphql).
7. Regenerate [`semio/graphql/schema.graphql`](semio/graphql/schema.graphql) through the existing [`semio/graphql/project.json`](semio/graphql/project.json) build target, then run the relevant Rust/GraphQL tests and inspect the regenerated schema for `JSON`.
8. Close the reopened ticket with a concise summary and the changed files once verification is complete.

## Validation

- Run the schema export/build target for `semio/graphql`.
- Run the existing `kit_graphql_smoke` tests in `semio/rs`.
