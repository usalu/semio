...
overview: Eliminate the ~6s UI freeze when selecting an object in the puzzle 3D play harness by routing selection through the Tree's existing per-id selectedIds store instead of rebuilding and fully re-rendering the entire 538-node hierarchy tree on every click.
todos:
  - id: ticket
  ...
  - id: renderer
    ...
    status: completed
  - id: play-tree
    content: Build stable structural hierarchy tree (no per-item selected), derive selectedIds, and cache sections by fixtureRevision in Puzzle3dPlayShellController
    status: completed
  - id: tests
...
## Root cause
Each object click bumps the full shell generation (`emit()`), which rebuilds the entire hierarchy tree (~538 nodes: 180 objects + 358 vortices) with selection baked into each row, hands `<Tree>` a brand-new `sections` array, and forces all `TreeDataItemView` rows to re-render (memo fails on new item/path objects). In dev/StrictMode this is the ~6s freeze.
...
1. The hierarchy `sections` array identity must stay STABLE across selection-only changes (only change when fixture structure changes).
...
- Keep per-item `isSelected` working for other callers, but the 3D hierarchy will stop setting it (selection comes from `selectedIds`).

### 3. Puzzle 3D play: stable structural tree + selectedIds
[puzzle/3d/play/index.ts](puzzle/3d/play/index.ts)
- Split `buildPuzzle3dPlayHierarchyTree` (L433-491): build the structural tree WITHOUT `selected` flags on rows; add a helper `puzzle3dPlayHierarchySelectedIds(selection)` that maps selection to row ids (`puzzle-3d-play-hierarchy.object.<id>`, `...vortex.<fullId>`, `...attraction.<id>`).
- Cache the structural tree in `Puzzle3dPlayShellController` keyed by `fixtureRevision` (and fixture identity), so `buildPuzzle3dPlayHierarchyPanelBody` (L1929-1933) returns the SAME `sections` object across selection-only generations, attaching the freshly derived `selectedIds`.
...
- [puzzle/3d/play/index.ts](puzzle/3d/play/index.ts) inline tests (around L2279-2505): assert the hierarchy `sections` identity is stable across two selections of the same fixture, and that `selectedIds` reflects the picked row.
