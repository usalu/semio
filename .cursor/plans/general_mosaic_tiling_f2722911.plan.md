...
The algorithm guarantees correct *relative* tiling. If the 3x5 grid still doesn't land on the PNG's product rows, adjust `CATALOGUE_FRAME` in [spec.ts:105](mit-bestand/präsentation/33.projektetage/spec.ts) — this is content calibration, independent of the tiling math.
