...
todos:
  ...
  - id: channel-operators-modules
    content: Update flow/modules/wasm glue and all in-repo modules (core/math/text/logic/dictionary/list) registrations + tests to operator-typed channels.
    status: completed
  ...
  - id: brep-rust-brepkit
    ...
    status: completed
  - id: brep-flow-module
    content: "Create flow/modules/brep WASM module: register geometry schema + operator-typed brep operators (reusing math point/vector + core schemas), stateful kernel, and wasm-bindgen manifest/evaluate/tessellate/dispose; tests."
    status: completed
...
2. Replace the OpenCascade JS brep kernel with a pure-Rust kernel built on [brepkit](https://github.com/andymai/brepkit), exposed as a real flow WASM module (`flow/modules/brep`).
...
graph TB
  subgraph rust [Rust]
    ...
    wasmglue["flow/modules/wasm<br/>manifest + dispatch glue"]
    ...
    brepmod["flow/modules/brep<br/>schemas+operators, stateful Topology, tessellate export"]
...
Files: [neural/engine/lib.rs](neural/engine/lib.rs), [flow/modules/wasm/lib.rs](flow/modules/wasm/lib.rs), all `flow/modules/*/lib.rs`, [flow/core/lib.rs](flow/core/lib.rs), [flow/react/index.tsx](flow/react/index.tsx).
...
## Phase 3 — `flow/modules/brep` WASM module + node catalogue

- New crate `flow/modules/brep` (mirror `flow/modules/math`): `Cargo.toml`, `lib.rs`, `project.json`, `package.json`, `script.ts`; uses `flow/modules/wasm` glue and depends on `geometry/brep/engine`.
...
- Update `launch.json`, root `package.json` workspaces (`flow/modules/*/pkg`), and `procedural/play/index.ts` extension-tree counts.
