...
todos:
 ...
 - id: build-registry-wiring
   content: Add new crate to workspace Cargo.toml, regenerate plugin registry, wire into dev/js and wgpu boot.ts program target lists
   status: completed
 - id: verify
...
- Re-run/extend the WGPU playground E2E harness ([.repo/🎫️/26/07/04/WGPU-PLAYGROUND-E2E/verify-wgpu-playgrounds-e2e.ts](.repo/🎫️/26/07/04/WGPU-PLAYGROUND-E2E/verify-wgpu-playgrounds-e2e.ts)) for `forms`, screenshot-diffing wgpu vs React for Blueprint/Try with and without `forms-module-procedural` loaded.
