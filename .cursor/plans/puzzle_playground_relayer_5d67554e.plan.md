...
- `@semio-tech/framework-playground-core-renderer-react` aggregates all three puzzle renderers (2d + 3d + 5d).
- Two playground packages (`entry_two`): `@semio-tech/framework-playground-core` is the renderer-neutral runtime; the single renderer wiring line lives in each play's `main.ts` entry (renderer is entry-only/dev, not a play-logic dependency).

## Target dependency graph

```mermaid
flowchart TB
  styling["@semio-tech/ui-styling"]
  uireact["@semio-tech/ui-react"]
  pg["@semio-tech/framework-playground-core (runtime)"]
  pgr["@semio-tech/framework-playground-core-renderer-react"]
  r2["@puzzle/2d-react"]
  r3["@puzzle/3d-react"]
  r5["@puzzle/5d-react"]
  p2["@puzzle/2d-play"]
...
- `@semio-tech/framework-playground-core-react` -> `@semio-tech/framework-playground-core-renderer-react` (`framework/playground/renderer/react`)
- `@semio-tech/framework-playground-core` keeps its name (now the renderer-neutral runtime)

## 1. Pure puzzle renderers (depend only on @semio-tech/ui-react)

- [puzzle/2d/react/package.json](puzzle/2d/react/package.json): name `@puzzle/2d-react`, drop `@semio-tech/framework-playground-core` dep, `repository.directory` -> `puzzle/2d/react`, script ids `@puzzle/2d-react:*`.
- [puzzle/2d/react/index.tsx](puzzle/2d/react/index.tsx): remove the `@semio-tech/framework-playground-core` (line 26) and `@semio-tech/framework-playground-core-react` (lines 27-37) imports; the board canvas becomes prop-driven, importing only from `@semio-tech/ui-react`. Any `ProductRuntime`/`FooterItem`/`Expertise` usage moves to the play package or is passed in via props.
- [puzzle/3d/react/package.json](puzzle/3d/react/package.json) + [index.tsx](puzzle/3d/react/index.tsx): name `@puzzle/3d-react`, drop `@semio-tech/framework-playground-core`, keep `@semio-tech/ui-react` + three/r3f.
- [puzzle/5d/react/package.json](puzzle/5d/react/package.json) + [index.tsx](puzzle/5d/react/index.tsx): name `@puzzle/5d-react`, deps `@puzzle/2d-react` + `@puzzle/3d-react` + `@semio-tech/ui-react`, drop `@semio-tech/framework-playground-core`.
...
- [framework/playground/renderer/react/package.json](framework/playground/renderer/react/package.json): name `@semio-tech/framework-playground-core-renderer-react`; add deps `@puzzle/2d-react`, `@puzzle/3d-react`, `@puzzle/5d-react`.
...
- `framework/playground/renderer/react/{project.json,vitest.config.ts}`: rename project to `@semio-tech/framework-playground-core-renderer-react`, add the puzzle-react aliases.

## 3. framework/playground stays the neutral runtime

- [framework/playground/core/index.ts](framework/playground/core/index.ts): absorb the react-free helpers that play currently pulls from the renderer (notably `playgroundTreePanelRootItems`, used in [puzzle/2d/play/index.ts](puzzle/2d/play/index.ts) line 24) so play imports them from `@semio-tech/framework-playground-core`. No React/puzzle imports added here.

## 4. New standalone play packages (logic depends only on @semio-tech/framework-playground-core)

For each of `puzzle/{2d,3d,5d}/play`:

- New `package.json`: `@puzzle/<dim>-play`, dependency `@semio-tech/framework-playground-core` only; `@semio-tech/framework-playground-core-renderer-react` + react/vite as devDependencies (entry-only). 2d also depends on `@puzzle/2d-wasm` build.
...
- `play/index.ts`: imports only from `@semio-tech/framework-playground-core` (declarative bodies, tools, measures - no React, no `@semio-tech/ui-react`, no renderer).
- `play/main.ts`: the single renderer line, e.g. `import { renderPlayground } from "@semio-tech/framework-playground-core-renderer-react"; renderPlayground(boardPlay);`.
...
- [.storybook/main.ts](.storybook/main.ts): alias `@puzzle/board` -> `@puzzle/2d-react`, `@semio-tech/framework-playground-core-react` -> `@semio-tech/framework-playground-core-renderer-react`; update optimizeDeps excludes.
- `bun install` to regenerate `bun.lock`; sanity-check `nx.json` / `eslint.config.mjs` / `Monorepo.sln`.

## 6. Verify

- Grep shows no remaining `@puzzle/board|@puzzle/scene|@puzzle/topology|@semio-tech/framework-playground-core-react` in `ui/`, `framework/`, `puzzle/`.
- Confirm no play `index.ts` imports `@semio-tech/ui-react` or any renderer package (layering check).
- `bun nx test` for `@semio-tech/ui-react`, `@puzzle/2d-react`, `@puzzle/3d-react`, `@puzzle/5d-react`, `@semio-tech/framework-playground-core`, `@semio-tech/framework-playground-core-renderer-react`, and each `@puzzle/*-play`.
