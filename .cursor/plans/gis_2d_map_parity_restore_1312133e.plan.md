...
2. `**gis/plugin/rs/../../2d/rs` dependency**: add `"@semio-tech/gis-2d-rs": "workspace:*"` to `framework/renderer/react/package.json` dependencies (pkg already exports `.` → `./pkg/gis_2d.js`, per `gis/2d/rs/package.json`).
