...
todos:
  - id: host-metadata
    content: Add host metadata to s program Cargo.toml; generate into program registry; delete hardcoded PLUGIN_HOST_CONFIGS in JS and Rust
    status: completed
  - id: session-artifact
  ...
  - id: boot-decouple
    ...
    status: completed
  - id: rust-align
    content: Remove is_space_mode(\"s\") / filter_programs identity hardcoding; use pre-filtered handles + generated host config
    status: completed
  - id: tests
...
// ... maps PROGRAM_BUILD_TARGETS / PROGRAM_TARGETS, then expandProgramRegistry(...)
```

That violates SOLID in two ways:

1. **Runtime identity branching** — `studioMode === "s"`, `?? "aggregator"` / `?? "s"`, and `expandProgramRegistry` over the full catalog make boot behavior (and often the compiled `boot.js` via `--define DEFAULT_PLUGIN_FILTER`) depend on which app you start.
2. **Hardwired plugins** — `PLUGIN_HOST_CONFIGS = [{ programId: "s", ... }]` in `[framework/core/js/index.ts](framework/core/js/index.ts)` and mirrored in `[framework/renderer/wgpu/rs/lib.rs](framework/renderer/wgpu/rs/lib.rs)`; `isStudioPluginFilter` in `[framework/program/registry/script.ts](framework/program/registry/script.ts)` hardcodes `"s"`.

Registry codegen already supports build-time filtering (`generateProgramRegistry({ filterPlaygroundPlugin })`) but the browser still embeds the full catalog and re-filters.

```mermaid
flowchart LR
  subgraph today [Today]
    FullCatalog[PROGRAM_BUILD_TARGETS full]
...
Composition root = `[framework/product/os/dev](framework/product/os/dev)` + `[framework/program/registry](framework/program/registry)`. Framework renderer/core only consume opaque session config + a pre-expanded program list.


| Concern                          | Today                                                          | After                                                                                                        |
| -------------------------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| Which plugins load               | Runtime `expandProgramRegistry` on full catalog                 | Build/dev generates filtered `PROGRAM_TARGETS` (already possible); boot loads that list as-is                 |
...
| Studio / host                    | `pluginFilter === "s"` + hand `PLUGIN_HOST_CONFIGS`            | Host roles declared on the program crate metadata; generated into registry; `studioMode = hostConfig != null` |
| Variant → programId / default app | Split across boot vs React                                     | Single generated session fields: `registryPluginId`, `defaultAppId`, optional `host`                         |
...
In `[s/program/rs/Cargo.toml](s/program/rs/Cargo.toml)` under `[package.metadata.semio]`:

```toml
host = { landing = "home", studio = "studio" }
```

Extend registry parse/codegen in `[framework/program/registry/script.ts](framework/program/registry/script.ts)` so each `ProgramBuildTarget` may carry optional `host: { landingAppId, hostAppId }`. Emit a generated host table (or embed on the entry) consumed by core — **delete** the hand-written:

```typescript
const PLUGIN_HOST_CONFIGS = [{ programId: "s", landingAppId: "home", hostAppId: "studio" }];
```
...
- Regenerate or write a **session** module (under os/dev generated output or filtered registry already used for that launch) containing at least:
  - `plugins: { programId, moduleUrl, contributes?, consumes? }[]` — **already expanded** via existing `resolveRegistryPluginIdsForFilter`
  - `defaultAppId?`, `host?` from playground row + program host metadata
  - no need for runtime `expandProgramRegistry` in boot

Change `[framework/renderer/wgpu/js/boot.ts](framework/renderer/wgpu/js/boot.ts)`:

- Remove `DEFAULT_PLUGIN_FILTER`, `pluginFilter === "s"`, and the `PROGRAM_BUILD_TARGETS.map` / `expandProgramRegistry` block.
- Load the fixed session program list; pass opaque session id / default app into `semioRendererBoot` as needed.
...
React shell (`[framework/renderer/react/index.tsx](framework/renderer/react/index.tsx)`): keep `resolveProgramHostConfig` / `expandProgramRegistry` only as fallbacks for callers that still pass a full list; preferred path is “plugins already filtered + host from props/session”. Align wgpu with React: never pass raw variant (`aggregator`) into `expandProgramRegistry` without resolving registry id first — ideally neither path expands at runtime for os/dev.
...
- Prefer: Rust receives the same pre-filtered list JS already loaded (no `filter_programs` by raw id / `"s"`).
...
- Changing WIT program contracts or per-app `studioMode` UI behavior beyond how studio is *detected*.
