...
overview: Remove runtime plugin-filter branching and hardcoded plugin IDs from framework boot/core so starting any playground only changes generated session artifacts — never framework source or baked-in `--define` defaults. Host/studio semantics move into plugin metadata.
todos:
  - id: host-metadata
    content: Add host metadata to s plugin Cargo.toml; generate into plugin registry; delete hardcoded PLUGIN_HOST_CONFIGS in JS and Rust
    status: completed
  - id: session-artifact
    content: Emit per-launch pre-filtered plugin session from os/dev + registry; stop full-catalog runtime expand in wgpu boot
    status: completed
  - id: boot-decouple
    ...
    status: completed
  - id: rust-align
    content: Remove is_studio_mode(\"s\") / filter_plugins identity hardcoding; use pre-filtered handles + generated host config
    status: completed
  - id: tests
...
// ... maps PLUGIN_BUILD_TARGETS / PLUGIN_TARGETS, then expandPluginRegistry(...)
```
...
**In scope:** plugin selection / host config / studio detection in framework boot, core playground resolution, os/dev entry, registry codegen, and matching Rust wgpu bridge helpers.
...
| Concern                          | Today                                                          | After                                                                                                        |
| -------------------------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| Which plugins load               | Runtime `expandPluginRegistry` on full catalog                 | Build/dev generates filtered `PLUGIN_TARGETS` (already possible); boot loads that list as-is                 |
| Default plugin                   | `DEFAULT_PLUGIN_FILTER` / `"s"` / stale `"aggregator"` in boot | Session artifact / Vite env from os/dev only; framework boot has **no** plugin-id default                    |
...
### 1. Host config from plugin metadata (not framework literals)
...
Mirror: generate the same for Rust (or stop hardcoding in wgpu `PLUGIN_HOST_CONFIGS` / `is_studio_mode` that compares to `"s"`). Studio detection = “resolved entry has `host`”, never `=== "s"`. Replace `isStudioPluginFilter` accordingly (host metadata present on the resolved crate, not a magic id).
...
- Stop defaulting `?? "s"` inside a path that looks like framework identity; default only when the **dev runner** has no session (product layer). Prefer Vite-injected session: pre-filtered `plugins` + `plugin` / `appId` / host already resolved so React shell does not need to expand the full fleet.
