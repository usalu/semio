...
## 1. Target shape (mirror `Semio.cs`)
...
- `XsDiff { removed: Vec<XIdDto>, updated: Vec<XDiffUpdate>, added: Vec<XFullDto> }` (matches `Semio.cs` `AttributesDiff`, `PortsDiff`, `TypesDiff`, …).
- `XDiffUpdate { id: XIdDto, diff: XDiff }`.

Reference lines in `Semio.cs` (authoritative shape): `AttributeDiff` 2196, `AttributesDiff.Apply` 2247, `PortDiff` 2889, `PortsDiff` 2920, `TypeDiff` 3659, `TypesDiff` 3750, `PieceDiff` 4125, `PiecesDiff` 4106, `DesignDiff` 4777, `DesignsDiff` 4768, `KitDiff` 7266, `Kit.ApplyDiff` 7435.

`Kit.ApplyDiff` (Semio.cs 7435) is the template: it applies collection diffs by calling `ApplyTypesDiff`/`ApplyDesignsDiff`, each of which does `removed → updated → added`.

## 2. Central `apply_diff` on every store

On every `pub mod <entity>` in [semio/rs/lib.rs](semio/rs/lib.rs), add one method:
...
5. Parent stores bubble invalidation (kit-level hash/validation) after their recursive call — see existing `KitStore::apply_design_diff` pattern at [lib.rs:10128](semio/rs/lib.rs).

This replaces the scattered `emit_ev` + `invalidate_hash` + `wire_graph_bus` calls currently sprinkled through every arm of `ChangeKitCommand::apply` at [lib.rs:1645-1994](semio/rs/lib.rs).
...
Consequence: the `FromKitDiff` variant at [lib.rs:492](semio/rs/lib.rs) becomes unnecessary because every command already produces a diff — delete it. `KitStore::replace_from_full_dto` stays only for initial kit load (not change flow).
...
`apply_many` folds per-command `XDiff`s via `merge` into a single kit-scoped `KitDiff` and concatenates inverses in undo order — same reverse-order convention as the current `apply_many` at [lib.rs:1999-2014](semio/rs/lib.rs).

## 5. `KitChange` and `KitChangeKind`

`KitChange` stays as it is today at [lib.rs:7537-7549](semio/rs/lib.rs):
...
- `pub mod diff` [lib.rs:6834](semio/rs/lib.rs): new sparse `DesignDiff` + `PiecesDiff`/`ConnectionsDiff`/`LayersDiff`/`GroupsDiff`/`StatsDiff` + helpers (`is_empty`, `merge`).
- `pub mod kit_diff` [lib.rs:6993](semio/rs/lib.rs): new sparse `KitDiff` + `TypesDiff`/`DesignsDiff`/`FilesDiff`/`FoldersDiff`/`AuthorsDiff`/`ConceptsDiff`/`TagsDiff`/`QualitiesDiff`/`PropsDiff`/`AttributesDiff` + helpers.
- Every entity module (`attribute`, `author`, `benchmark`, `concept`, `connection`, `connector`, `design`, `file`, `folder`, `group`, `kit`, `layer`, `piece`, `port`, `prop`, `quality`, `representation`, `side`, `stat`, `tag`, `typ`): add `pub fn apply_diff(&mut self, &XDiff) -> Result<()>`. Migrate the existing dto-apply helpers (`apply_full_dto_fields`, `apply_metadata_fields`, `apply_metadata_dto` — see [lib.rs:4226, 4450, 4635, 4955, 5795, 12202, 12218, 13327, 13567, 14133, 14310, 14498](semio/rs/lib.rs)) to be internal to the new `apply_diff`.
- `pub mod change_command` [lib.rs:346](semio/rs/lib.rs): rewrite every `impl`: return `(XDiff, Vec<Self>)`; call only `apply_diff` on stores (not `set_*` directly).
- `pub mod kit_change` [lib.rs:7507](semio/rs/lib.rs): trim to the new `apply_forward`/`apply_backward` wrappers over `apply_many`.
...
- `pub mod io` [lib.rs:15446](semio/rs/lib.rs): update serde shape for `KitChange` persistence (command-only, no pre-state diff snapshots).
- `pub mod wasm` [lib.rs:16876](semio/rs/lib.rs): delete `apply_design_diff` / `apply_kit_diff` handles; keep only `execute_change_kit_commands` (already added in plan [kit_store_story_0a521ca8](.cursor/plans/kit_store_story_0a521ca8.plan.md)).

## 8. Test coverage in `mod tests` [lib.rs:17436](semio/rs/lib.rs)
...
- `round_trip_forward_inverse` — `apply_many(forward)` then `apply_many(inverse)` returns initial DTO (already partially at [lib.rs:17552](semio/rs/lib.rs), extend).
- Retire `apply_design_diff_add_piece_emits_child_added_and_hashes` [lib.rs:18944] and rewrite as command-based equivalent.

## 9. [semio/rs/AGENTS.md](semio/rs/AGENTS.md)
