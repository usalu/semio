...
Additionally, `tool_button`/`tool_toggle`/`tool_collection` in [framework/core/rs/tools.rs](framework/core/rs/tools.rs) are called without labels, so every tool loses its tooltip (`title`/`label` default to `None`); and the 11 edit ops were flattened into loose top-level buttons instead of the old `edit` collection, and paint's UV/history ops were similarly flattened — both structural regressions versus `git show 32693795d:lowpoly/core/js/index.ts` (`buildLowpolyPlayToolbarTools`/`buildLowpolyPlayPaintToolbarTools`, lines 131-213).
...
- Fix the hierarchy tree's face hover-reveal "Flip normal" action icon (line ~619, currently `flip-horizontal`) to `flip-vertical` so it matches the Edit Tools "Flip Normals" button that dispatches the same `flipFaces` command.
