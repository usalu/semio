---
name: Ephemeral Studio Create Open
...
todos:
  - id: ephemeral-api
    content: Add create_ephemeral_os_studio in os-core (empty doc, backbone None, no port write)
    status: completed
  - id: default-create
...
Source already removed `DownloadMediaExport` from `createStudio`, but default create still calls [`create_os_studio`](framework/product/os/core/rs/lib.rs) which:

- writes through `LocalStorageBackbonePort` (`CATALOG_PORT` in [`s/plugin/rs/lib.rs`](s/plugin/rs/lib.rs))
- sets `document.backbone = studio://{id}` via `sync_os_studio_document`
...
sequenceDiagram
  ...
  Home->>Shell: Navigate /studios/studio-id
  Shell->>Studio: openStudio studioId
...
- Add `create_ephemeral_os_studio(name) -> OsDocument` that only calls `create_empty_os_document` (already has `backbone: None`).
- Do **not** call `sync_os_studio_document` / `track_os_studio_backbone_uri`.

Keep existing `create_os_studio(port)` for explicit later persistence (file/folder/catalog save).
...
- Default Meta+N / `createStudio` (no kind / `"catalog"` / `"file"`):
  1. `create_ephemeral_os_studio(name)`
...
- Ensure create never navigates to `/studios/default` or `/studios/demo`.
...
Do not call shell `openDocument` or plugin `attachBackbone` from create/open path. Persistence remains future opt-in (`bindStudioFile` / folder / sync card).
