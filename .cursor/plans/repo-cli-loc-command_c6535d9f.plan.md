...
overview: Add a new `loc` subcommand to the repo CLI that reports current LOC (via `cloc`) plus cumulative edited/added/removed LOC (via `git log --numstat`) for TypeScript, Go, C#, Python, and Rust, with optional `--history` (per-commit time series along `⛳wip`) and `--byContributors` flags.
todos:
 - id: wire-cmd
...
repo loc --history                    # per-commit time series along ⛳wip
repo loc --byContributors             # per-contributor breakdown
repo loc --history --byContributors   # per-contributor time series along ⛳wip
```
...
- `--branch` (string, default `⛳wip`) — branch to walk for `--history` (escape hatch; not advertised).
...
- with `--history`: commits reachable from `--branch` (default `⛳wip`).
...
- Default range is `HEAD`'s reachable history. `--history` switches the range to the dev branch `⛳wip` and adds per-commit rows.
