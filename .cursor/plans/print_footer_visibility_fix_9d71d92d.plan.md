...
- **Footer invisible**: without `includefoot`, the footer floats *below* the last text line by `\footskip` (~24.8pt) *inside* the `bottom` margin band, but that band is only ~2.2pt — the footer bar's bottom edge lands ~22.6pt past the physical page edge and is clipped/off-page.
