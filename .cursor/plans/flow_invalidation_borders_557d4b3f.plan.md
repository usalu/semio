...
overview: Wire end-to-end input invalidation so slider changes immediately mark downstream nodes with active/stale computing borders, keep plugin ticks authoritative, and fix the gesture-resync hole that currently prevents sphere-cut-with-torus borders from updating at all.
todos:
  - id: ticket-open
  ...
  - id: dirty-remaining
    ...
    status: completed
  - id: flow-play-arm
    content: Always arm flowEvalTick when pending work exists in flow plugin (parity with procedural)
    status: completed
  - id: verify
...
flowchart TD
  ...
  plugin --> scene["Scene computingJson"]
