...
todos:
  ...
  - id: local-chrome
    ...
    status: completed
  - id: driver-baseline
    content: Persist previous_snapshot/previous_channels on FlowEvalDriver and restore into ephemeral program hosts
    status: completed
  - id: dirty-remaining
...
4. **Ephemeral program hosts drop eval baseline** — `host_from_fixture` always builds a host with `previous_snapshot: None`, so dirty tracking relies on cache luck instead of a durable baseline on `FlowEvalDriver`.
...
Make chrome authoritative from the program scene **and** instantly optimistic on the local canvas; keep fixture reload suppressed only while dragging.
...
In `[flow/program/rs/lib.rs](flow/program/rs/lib.rs)`:
...
- `[procedural/program/rs/lib.rs](procedural/program/rs/lib.rs)`: extend `sphere_cut_example_computing_chrome_clears_once_ticks_converge` with a mid-chain slider mutate asserting recomputed `computing_json` targets the cut branch.
