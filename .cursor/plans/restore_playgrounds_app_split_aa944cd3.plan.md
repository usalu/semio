...
**Rule of thumb**: if the code would need to change when swapping in a different fixture, it's a playground concern → stays in `playground.ts`. If it defines *how the app behaves* regardless of which fixture is loaded, it's app logic → moves to `index.ts`.
