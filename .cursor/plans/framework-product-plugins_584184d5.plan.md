...
Turn `elements/lib/framework` into a generic, product-agnostic framework so that any product (Sketchpad, future products) can be built on top of it and grow its own program ecosystem. No business logic in the framework, no Sketchpad-specific names, no legacy.
...
- No marketplace, no program discovery, no permissions enforcement (manifests carry the fields but the host doesn't yet gate on them).
...
- No Sketchpad migration, no Energy/Structure program implementations.
