...
todos:
 ...
 - id: operation-history
   ...
   status: completed
 - id: operations-macro
   content: Author operations!{} macro producing Operation struct + Modification + Diff + Diffs + edges + connections + interface registration
   status: completed
 - id: operations-rows
   content: Emit ~100 operation rows covering Kit/Tag/Concept/Port/Quality/Type/Connector/Design/Piece
   status: completed
 - id: query-root
   content: Static Query root with session/wip/authoritative/conflicts/node/entity/pieceInDesign/alternativePieceKind
   status: completed
 - id: mutation-root
   content: "Static Mutation root: ~100 async fns calling operation apply functions, snapshotting before/after, emitting events, returning Id"
   status: pending
...
- Generated SDL byte-for-byte covers `target.schema.graphql` (689 types, 100 unions, 11 interfaces, 17 inputs, 95 operations, ~95 subscription fields).
...
flowchart TD
  ...
  Per --> Hist["Operation history: Arc<Snapshot> per operation record"]
  Hist --> Operations["macro operations!{}  ~100 operation types"]
  Operations --> Roots["Query / Mutation / Subscription roots"]
...
- Add per-entity unit tests for one mutation per operation family (Tag, Concept, Port, Quality, Type, Connector, Design, Piece, Kit) verifying before/after snapshots are distinct `Arc`s with the expected fields.
- `cargo test -p compose` (native) and `cargo check -p compose --target wasm32-unknown-unknown --no-default-features` must pass.

## Risk and size

- Final `lib.rs` lands at roughly **35 000–45 000 lines**, almost entirely macro-generated (the three relay macros + the `operations!` macro keep authored lines below ~10 000).
...
- Phasing inside the single file is enforced by Rust's top-down resolution: foundation → entities → interfaces → relay macros → expansions → operations → roots → schema build. Out-of-order edits fail to compile, which is the type-safety win the user asked for.
