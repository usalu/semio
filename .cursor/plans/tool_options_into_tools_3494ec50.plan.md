...
Prior ticket [Brush Kind Suggestion Percentages](.repo/🎫/26/06/02/BRUSH-KIND-SUGGESTION-PERCENTAGES/ticket.json) claimed renorm helpers; handlers today only clamp/insert and do **not** keep sum = 1.
...
```
Distribution
├── Objects | Nodes | Parts   (sliders, sum = 1)
└── Vortices | Handles | Grips (sliders, sum = 1)
```
...
On execution: reopen or open a ticket via repo MCP; associate with `🎯r2603` (current playground/UI release cycle). Temp artifacts only under the ticket folder. Do not mix compose/mit-bestand.
