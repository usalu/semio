...
# Play Document Tabs on Left Workbench

## Goal

Every play host under `@elements/playground` gets a **left workbench tab** (icon: `ListTree`) labeled **Document**, showing a navigable composition tree. Clicking a leaf syncs canvas selection (same as existing inspectors). Right **details** panels stay as-is (inspector/setting/status).
...
flowchart LR
  ...
  subgraph builders [Pure tree builders]
    BBuild[buildBoardPlayDocumentSections]
    SBuild[buildScenePlayDocumentSections]
    SpBuild[buildSpatialPlayDocumentSections]
  end
  subgraph shell [PlaygroundView workbench]
    Tab[Document tab StaticTreePanelDefinition]
...
- `order: 0` on document tab; bump existing left tabs (board Library → `order: 1`)
- `initialPanelVisibility.leftSidePanel: true` for scene, topology, spatial (board already true)
- Builders return `TreeDataSection[]` with one section `Document` and a single expanded root item
...
- Manual: launch each play from `launch.json`, open workbench, expand Document, click object/vortex/node/primitive and confirm viewport selection matches

## Ticket workflow (implementation phase)

- Read `repo://goals`, open ticket e.g. **Play Document Workbench Tab**
- Close ticket with touched files when done

## Out of scope (v1)

- Drag-reorder in document
