...
1. New declarative, render-independent framework product: `@semio-tech/framework-presentation-core` (pure TypeScript, no React/reveal).
...
flowchart LR
  ...
  core["@semio-tech/framework-presentation-core (Presentation, Sequence, Thought, Participant, Embodiment, Arrangement, Transition, intro template)"]
  renderer["@semio-tech/framework-presentation-renderer-react (React + reveal.js adapter)"]
  fcore["@semio-tech/framework-core"]
  ui["@semio-tech/ui-react (chrome + tokens)"]
  app --> core
  app --> renderer
  renderer --> core
  core --> fcore
...
## Domain model (`@semio-tech/framework-presentation-core/index.ts`)
...
- `framework/product/presentation/package.json`: `{ "name": "@semio-tech/framework-presentation", "private": true, "workspaces": ["core", "renderer/react"] }`.
- `core/`: `package.json` (dep `@semio-tech/framework-core`), `project.json` (test target), `script.ts` (TestScript via shared `runVitest`), `vitest.config.ts`, `index.ts` — copy shapes from [`framework/product/playground/core`](framework/product/playground/core/package.json).
- `renderer/react/`: `package.json` (deps `@semio-tech/framework-presentation-core`, `@semio-tech/framework-core`, `@semio-tech/ui-react`, `react`, `react-dom`, `reveal.js`; devDeps `@types/reveal.js`, `@types/react`, `@types/react-dom`, `typescript`), `project.json`, `script.ts` (with `policy` lint export), `vitest.config.ts` (react() + jsdom + aliases), `index.tsx`, `globals.css` — copy shapes from [`framework/product/playground/renderer/react`](framework/product/playground/renderer/react/package.json).
...
- [`package.json`](mit-bestand/präsentation/33.projektetage/package.json): drop `react`, `react-dom`, `reveal.js`, `@types/reveal.js`, `@semio-tech/ui-react`; add `@semio-tech/framework-presentation-core` + `@semio-tech/framework-presentation-renderer-react`. Keep vite/tailwind/vitest + `@vitejs/plugin-react` (vite still compiles the renderer's tsx).
- [`vite.config.ts`](mit-bestand/präsentation/33.projektetage/vite.config.ts): add aliases for `@semio-tech/framework-presentation-core`, `@semio-tech/framework-presentation-renderer-react`, `@semio-tech/framework-core`, keep `@semio-tech/ui-react`.
- [`globals.css`](mit-bestand/präsentation/33.projektetage/globals.css): import `@semio-tech/framework-presentation-renderer-react/globals.css` + keep `@source` lines; remove the now-duplicated reveal overrides (they live in the renderer).
...
- Validate by running the new tests (`bun nx run @semio-tech/framework-presentation-core:test`, `:renderer/react:test`) and the projektetage dev server (port 6050), confirming the 5-slide auto-animate intro renders.
