...
overview: Restore the real FlowHost-backed undo/redo and operator catalogue that regressed/were never wired in flow/program/rs/lib.rs, matching what commit 465eed3c1 already achieved and what the pre-migration flow/react implementation had.
todos:
 - id: core-history-api
 ...
 - id: plugin-host-restore
   content: "flow/program/rs/lib.rs: restore persistent FlowPlayApp{host} + host_for caching; wire set_host_catalogue_json on construction"
   status: completed
 - id: plugin-undo-redo
...
Comparing the uncommitted working tree of [flow/program/rs/lib.rs](flow/program/rs/lib.rs) against `HEAD` (`465eed3c1`, whose message is literally "audit full 25-app premigration parity gaps... restoration"):

- `git diff HEAD -- flow/program/rs/lib.rs` shows the working tree **reverted** the persistent `FlowPlayApp { host: Option<FlowHost> }` + `host_for()` caching pattern back to a stateless `struct FlowPlayApp;` that rebuilds a throwaway `FlowHost` on every command. This silently switched `"undo"`/`"redo"` from real `FlowHost::undo()/redo()` (backed by `flow/core/rs/lib.rs`'s `FlowHistory` stack, added by the completed [`.cursor/plans/flow_undo_redo_6e12141c.plan.md`](.cursor/plans/flow_undo_redo_6e12141c.plan.md)) back to an ad-hoc `envelope.runtime.undo_fixtures`/`redo_fixtures` `Vec<FlowFixture>` snapshot list — a duplicate, weaker mechanism that also bloats the persisted document JSON with full fixture clones.
...
The same audit plan also flags an "async eval-bridge/worker path" gap (`set_eval_bridge`, `apply_eval_outputs_json`, `set_computing_progress` unused). Investigation shows `neural_engine`'s `Registry::dispatch` already evaluates all current operators natively and synchronously in Rust, and the entire program runtime ([framework/program/rs/program_runtime.rs](framework/program/rs/program_runtime.rs)) is synchronous end-to-end for all 25 apps — there is no worker-thread invocation path anywhere in the new architecture to wire into. Re-introducing genuine off-main-thread evaluation would be a cross-cutting plugin-runtime change, not a flow-specific fix, so it is **not** part of this plan.
...
- Make `begin_change` `pub fn begin_change(&mut self)` (currently private) so the program can snapshot single-shot mutations that don't already call it internally (`move_widget`).
...
### [flow/program/rs/lib.rs](flow/program/rs/lib.rs)
...
- `cargo test -p flow_core -p flow-program`.
- Rebuild the flow program WASM bundle (existing `bun ./script.ts wasm` flow used earlier in this session).
