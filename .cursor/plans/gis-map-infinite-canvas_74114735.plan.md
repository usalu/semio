...
overview: Extract the shared infinite-canvas engine (camera, Vello/WebGPU GPU session, raster tiles, pan/zoom, LOD) out of puzzle_2d into infinite_canvas, then build a new gis/map WASM crate + React renderer + playground on top of it, rendering OpenStreetMap raster tiles (via a dev tile proxy) with Position / Route / Region overlays and a default whole-world view.
todos:
 - id: extract-engine
   content: Extract shared engine (Camera/transforms, LOD, raster cache, CanvasGpuSession, CanvasContent trait) from puzzle_2d into infinite/canvas/vello/lib.rs + Cargo.toml; extend its tests.
   status: completed
...
A `gis/map` Rust WASM crate that renders OpenStreetMap raster tiles on the infinite canvas with `Position`, `Route`, and `Region` overlays, plus a React renderer and an end-to-end playground. Default camera fits the whole world. The shared canvas engine is first extracted from `puzzle_2d` into `infinite_canvas` so the map builds on it cleanly.
...
- The real engine lives in `[puzzle/2d/rs/lib.rs](puzzle/2d/rs/lib.rs)`: `Camera`/transforms (`screen_to_world`/`world_to_screen`), LOD, pan/zoom, `build_vector_scene`, raster `scene.draw_image(...)`, and the WASM `BoardSession` + `BoardSessionInner` GPU plumbing (`render_frame_gpu`, `attach_canvas`) wrapping `canvas::vello::util::{RenderContext, RenderSurface}` + `Renderer`.
- `[infinite/canvas/vello/lib.rs](infinite/canvas/vello/lib.rs)` is only geometry/scene-JSON utils + `CanvasExtension` today.
...
flowchart TB
  subgraph rust [Rust]
    IC["infinite_canvas (rlib)<br/>CanvasEngine + CanvasContent + GPU session + raster cache"]
  ...
  subgraph ts [TypeScript]
    RR["@semio-tech/infinite-canvas-react-renderer"]
...
## Stage 1 — Extract shared canvas engine into `infinite_canvas`

In `[infinite/canvas/vello/lib.rs](infinite/canvas/vello/lib.rs)`, add new regions (no wasm-bindgen here; it stays rlib):
...
- Update `[infinite/canvas/vello/Cargo.toml](infinite/canvas/vello/Cargo.toml)` to add `vello` wgpu features and, under `cfg(wasm32)`, `web-sys`/`wasm-bindgen-futures`/`js-sys` as needed for the surface (no `#[wasm_bindgen]` exports).
...
- Replace local `Camera`/transforms/LOD/raster helpers with `infinite_canvas` re-exports; `BoardHost` implements `canvas::CanvasContent` (`build_scene` = existing `build_vector_scene`, `clear_color` = `vello_theme.raster_clear`).
- Reduce `BoardSessionInner` to wrap `canvas::CanvasGpuSession` + `BoardHost`; keep the public `BoardSession` wasm API identical so `[puzzle/2d/react/index.tsx](puzzle/2d/react/index.tsx)` is unchanged.
- Validate: `cargo test -p puzzle_2d -p infinite_canvas` and a `@semio-tech/puzzle-2d-play` dev smoke (confirm tiles/edges still render).
...
- `#region MapContent`: `MapHost` holding camera, tiles, and overlays:
  ...
  - implements `canvas::CanvasContent::build_scene` (tiles first, then regions, routes, positions) + pan/zoom/hit-test.
...
- Cargo: `crate-type = ["rlib","cdylib"]`, deps `infinite_canvas`, `serde`, `serde_json`, `vello`, `image` (PNG decode), `cfg(wasm32)` wasm-bindgen stack — mirror `[puzzle/2d/rs/Cargo.toml](puzzle/2d/rs/Cargo.toml)`.
...
- Declarative `@semio-tech/infinite-canvas-react-renderer` reconciler host kinds `gis.map/position|route|region` + components `<MapCanvas>`, `<Position>`, `<Route>`, `<Region>`; default camera = whole world.
- Depends on `@semio-tech/infinite-canvas-react-renderer`, `@semio-tech/gis-map-rs`, `@semio-tech/ui-react`, react stack.
...
- Validate end-to-end: `cargo test` for `infinite_canvas`/`gis_map`/`puzzle_2d`; `@semio-tech/gis-map-react` vitest; `@semio-tech/gis-map-play` dev → confirm (with console logs) tiles load through the proxy, the world fits by default, and Position/Route/Region render and pan/zoom.
