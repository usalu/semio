...
- Run the existing WGPU E2E suite ([.repo/🎫️/26/07/04/WGPU-PLAYGROUND-E2E/verify-wgpu-playgrounds-e2e.ts](.repo/🎫️/26/07/04/WGPU-PLAYGROUND-E2E/verify-wgpu-playgrounds-e2e.ts)) against `s` and `draw` (the current producers of `measures`/`engagement` data), asserting: folded/collapsed chips are visible and clickable on load, unfold/expand shows the rail contents on the correct (React-matching) side, and window content underneath no longer paints over the chip.
- Manual screenshot diff of `s` and `draw` in WGPU vs React to confirm side/placement now match.
- Per workspace rules, this work happens inside a ticket (`.repo/🎫️/26/07/06/...`) — open a new ticket for this fix (no existing open ticket covers it; `WGPU-THEME-CURSOR-PARITY` is cursor-icon-only) and close it with a summary listing the files touched.
