...
Every artifact becomes importable and exportable to the repo's file-type catalog through a new required taxonomy facet, replacing today's scattered `register_2d_export_handlers` / `register_mesh_*` / `register_solid_*` / `register_dwg_*` calls buried in each `⚙️engine/🦀️component.rs`.
...
✏️s/🔌️plugins/<plugin>/🗿️artifacts/<artifact>/
  🚪️io/
    🦀️component.rs          ArtifactIo impl: declared format table + register()
    🟦️component.ts
    🧊️glb/
      📥️import/{🦀️component.rs, 🟦️component.ts}
      📤️export/{🦀️component.rs, 🟦️component.ts}
    🎨️svg/  📷️png/  📄️pdf/  📊️csv/  …
```
...
`MeshData` and `DwgDrawing` already exist in [🧰️framework/🔨️modules/🔺️mesh/🦀️component.rs](🧰️framework/🔨️modules/🔺️mesh/🦀️component.rs). The other five are new.
...
`MediaFormat` in [🧰️framework/🔨️modules/🔺️mesh/🦀️component.rs](🧰️framework/🔨️modules/🔺️mesh/🦀️component.rs) is renamed `MediaFormat` and grows to all 26 variants. A policy rule asserts CSV-to-enum parity, following the taxonomy-twin pattern.
...
- `artifactFacetChildDirs()` and `validateTaxonomy()` in [🔍️discovery/🟦️component.ts](🧰️framework/🛍️products/🦑️repo/🔨️modules/📚️library/🔍️discovery/🟦️component.ts) — needs a wildcard level for the format dir
...
- `assert_taxonomy_components` in the plugin SDK `🦀️component.rs`
...
- W7 — 1 agent. TS/WASM bridge so the `🟦️component.ts` leaves stop being throwing stubs, plus file-picker `accept` and export-menu wiring.
