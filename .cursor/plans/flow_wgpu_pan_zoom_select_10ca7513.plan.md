...
- Fix `graph_interaction_commands` to emit `nodeGraphSelect` as `{"nodeIds": [...]}` — parse `host.selected_widget_ids_json()` (a JSON array string) into `Vec<String>` and embed as a real array, matching `flow/plugin/rs/lib.rs:707-714` and React's shape exactly. Keep `nodeGraphHover`/`nodeGraphViewport` argument shapes as-is (`hoverJson`, `viewportJson`), which already match the program.
