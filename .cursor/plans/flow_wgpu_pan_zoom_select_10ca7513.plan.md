...
- Fix `graph_interaction_commands` to emit `nodeGraphSelect` as `{"nodeIds": [...]}` — parse `host.selected_widget_ids_json()` (a JSON array string) into `Vec<String>` and embed as a real array, matching `flow/program/rs/lib.rs:707-714` and React's shape exactly. Keep `nodeGraphHover`/`nodeGraphViewport` argument shapes as-is (`hoverJson`, `viewportJson`), which already match the program.
...
- Re-check whether `bun nx run @semio-tech/framework-os-dev:dev` still fails to compile for unrelated reasons (previous session noted `parse_program_entries`/borrow/`UiToggleNode.checked` errors); fix if still blocking, since it's required to actually observe the fix.
