...
| Layer                                                                                  | Status                                                                                    |
| -------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Catalogue rows (`draggable` + `application/x-semio-catalogue-item`)                    | Wired in `[puzzle/program/rs/lib.rs](puzzle/program/rs/lib.rs)` `puzzle3d_object_kind_item` |
...
1. **Host owns preview + raycast** (no program round-trip per `dragover` frame — matches premigration R3F ghost).
...
In `[puzzle/program/rs/lib.rs](puzzle/program/rs/lib.rs)` `puzzle3d_object_kind_item`:
...
- `[puzzle/program/rs/lib.rs](puzzle/program/rs/lib.rs)` d3 tests: kinds tree drag_data includes `objectKind` + `meshUrl`; `addObjectKind` with `origin` places at that point.
...
- `[puzzle/program/rs/lib.rs](puzzle/program/rs/lib.rs)` — enrich `drag_data`, tests
