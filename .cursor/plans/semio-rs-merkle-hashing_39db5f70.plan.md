---
name: semio-rs-merkle-hashing
overview: "Replace the partial per-entity hash logic in `semio/rs/lib.rs` with a uniform Merkle scheme: every entity hashes all its non-computed scalar fields plus the sorted hashes of its child entities; every collection (`gql_relay::*Connection`) hashes the sorted hashes of its child nodes (not their ids)."
todos:
  - id: hash-helpers
...
All work happens inside the single crate file [`semio/rs/lib.rs`](semio/rs/lib.rs) (plus the existing ticket folder for any temporary logs). Exit conditions: every entity exposes a `hash` GraphQL field that is a Merkle hash, every `XxxConnection` exposes a `hash` that combines sorted child hashes, and `cargo test -p semio` passes (extending the existing `mod tests` block, no new test files).
...
Because `compute_hash` is async, every `from_*` constructor becomes async. Update all ~37 call sites (`PieceConnection::from_pieces(...).await`, etc.); they're already inside `async fn` GraphQL resolvers in [`semio/rs/lib.rs`](semio/rs/lib.rs) so adding `.await` is mechanical. The `simple_conn!` macro and `entity_relay!` macro both expand to async `from_rows` returning `Self`. SimpleObject value rows (`File`, `Layer`, …) call their new sync `compute_hash()`; Arc'd entity rows call `compute_hash().await`.
...
6. Extend `mod tests` with the five new tests above; run `cargo test -p semio --lib` until green; update `kit_store_bundle_serialize_hydrate_round_trip_via_graphql` if it asserts on specific hash strings.
7. Verify `npx nx build semio/graphql` regenerates the SDL with the new `contentHash` field on `File` (no consumer ticket needed because the bundle is greenfield per workspace rules).
