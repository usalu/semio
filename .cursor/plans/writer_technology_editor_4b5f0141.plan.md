...
flowchart TB
  subgraph main [Main thread]
    ...
    WASM["writer/rs: WriterSession (Vello text engine on infinite_canvas)"]
...
- `WriterDocument`: schema `"writer.document"` = `{ schema, id, languageId, uri, text, camera: {x,y,zoom} }` (camera makes it a true infinite-canvas document). Parser + validation.
...
### Layer 3 - `writer/rs` (Rust/WASM editor engine) - on `infinite_canvas`

`writer/rs/lib.rs` (`pub use infinite_canvas`): `WriterSession` (`wasm_bindgen`) mirroring the raster/flow session pattern:
...
- Text buffer + edit operations: `insert`, `delete`, `newline`, arrows/selection, `pointer_down/move/up_screen` (caret/selection hit-testing through the camera).
