...
- Add `face_centroid_sync`-equivalent helper (vertex-average via existing `deconstruct`/point-query APIs) and a `face_plane_group_key` helper (port of `facePlaneGroupKey`, `cad/kernel/brepjs/js/index.ts:1337-1352`) — implemented CAD-side (no kernel crate changes needed) since the kernel already exposes normals + boolean ops.
...
- Keep `dispatch_cad_ops`/VCS routing (`CadOp::SetPaneObjects`) as the mutation path — only the objects computed for the target pane change.
...
**wasm32:** no change needed beyond wiring the new `requestFileOpen` op — `rfd` supports wasm32 via the browser's file picker, and `download_media_export`'s wasm32 anchor-download branch is already correct.
