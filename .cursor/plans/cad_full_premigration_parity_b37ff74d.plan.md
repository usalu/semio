...
todos:
 - id: typology-alignment
   content: Rename TYPOLOGY_CATALOG/fixtures to premigration-qualified typology ids scoped per pane (cad/program/rs/lib.rs)
   status: completed
 ...
 - id: load-envelope-unwrap
   ...
   status: completed
 - id: tests-extend
   content: Extend existing cad/rs and cad/program/rs test modules to cover derive classification, statechart commits, and save/load round-trips
   status: completed
isProject: false
---
...
1. **`applyTransformation`** (`cad/program/rs/lib.rs:623-665`) clones objects and relabels typology to `"energy.energy.hull"` — it never fuses BREP solids or classifies faces like premigration's `runDeriveTransformation` (`cad/core/js/index.ts:3531-3627`).
2. **Interaction engagement** (`cad_window_engagement`, `engagementSubmit`, `cad/program/rs/lib.rs:1488-1567,2229-2256`) is a flat typology-string matcher that instantly creates a placeholder object — not premigration's per-typology `InteractionSpec` statechart (states/transitions/guards/effects/commit, `cad/core/js/index.ts:5173-6665`) with REPL keyed-transition parsing (`cad/renderer/js/index.tsx:5493-5525`).
3. **Save/load** (`export_spatial_json`, `pending_export`, `cadLoadRequest`, `cad/program/rs/lib.rs:667-723,2124-2149`) only stashes/reads JSON in-memory — `pending_export` has no consumer and `cadLoadRequest` is unhandled by the wgpu renderer, unlike premigration's real file-picker + STEP/JSON export (`cad/renderer/react/index.tsx:1162-1433`).
...
## Phase 1 — Typology/model-definition alignment (`cad/rs/lib.rs`, `cad/program/rs/lib.rs`)
...
## Phase 2 — Real derive-transformation engine (`cad/program/rs/lib.rs`)
...
- Build each source-pane `CadObject` as a real kernel solid (extending `typology_brep_mesh`'s existing `box_prim`/`cylinder_prim` construction pattern, `cad/program/rs/lib.rs:133-146`, to keep the resulting `GeometryHandle`s instead of discarding them after tessellation).
...
## Phase 3 — Real per-typology Interaction state machine (`cad/rs/lib.rs`, `cad/program/rs/lib.rs`)
...
- Add the `rfd` crate (cross-platform native file dialogs, also supports wasm32) to `cad/program/rs/Cargo.toml`/`framework/renderer/wgpu/rs/Cargo.toml` as appropriate.
...
- Extend the existing test modules in `cad/rs/lib.rs` and `cad/program/rs/lib.rs` (no new test files) to cover: derive-transformation face classification on a known box fixture, interaction statechart commit for each of the 4 ported specs, and save/load round-trips (JSON for selected, STEP for model-space/current).
- No React/JS renderer restoration — everything above is implemented in Rust against the existing wgpu program/kernel stack.
