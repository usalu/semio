...
- Package/nx name (ASCII to keep `bun install` safe): `@mit-bestand/praesentation/projektetage`. Folder: `mit-bestand/präsentation/33.projektetage`. Dev port: `6050`.
...
- `package.json` — name `@mit-bestand/praesentation/projektetage`, `bundleKind: "application"`, scripts call `bun nx run …:{dev,build}`, deps `reveal.js` + `@types/reveal.js`, workspace deps `@ui/react`, `@ui/styling`, `vite`. Mirrors [`puzzle/2d/play/package.json`](puzzle/2d/play/package.json).
- `project.json` — nx `dev`/`build` targets via `bun ./script.ts …`, `PRAESENTATION_PROJEKTETAGE_PORT=6050`. Mirrors [`puzzle/2d/play/project.json`](puzzle/2d/play/project.json).
- `script.ts` — `ScriptRouter` with `DevScript` (`runViteBunxDev`, portEnv `PRAESENTATION_PROJEKTETAGE_PORT`, default `6050`) and `BuildScript` (vite build), using helpers from [`repo/lib/js/src/index.ts`](repo/lib/js/src/index.ts). Mirrors [`puzzle/2d/play/script.ts`](puzzle/2d/play/script.ts).
- `vite.config.ts` — `defineConfig` reusing exports from [`ui/styling/vite-elements-assets.ts`](ui/styling/vite-elements-assets.ts): `base: "./"`, `elementsAssetsVitePlugin(ui/asset)`, `tailwindcss()`, `react()`, `playgroundIframeEmbedHeadersPlugin()`, `playgroundStaticSiteBuildOptions()`, alias `@ui/react` → `ui/react/index.tsx`. (Lighter than `createPlaygroundPlayViteConfig`, which carries puzzle/renderer-only aliases.)
...
- [`ui/styling/vite-elements-assets.ts`](ui/styling/vite-elements-assets.ts): inside `🔖ViteElementsAssets`, add `PLAYGROUND_SITE_DEV_PORTS` (`{ compose:4000, cad:6020, "2d":6012, "3d":6013, "5d":6014 }`) and `playgroundEmbedUrl(kind, isDev)` returning the dev `http://localhost:<port>` or `https://<PLAYGROUND_SITE_HOSTS[kind]>`. Keeps hosts + ports + URL logic in one place.
- [`package.json`](package.json) (root `workspaces`): add `"mit-bestand/präsentation/*"` so `bun install` and `@ui/*` resolution work.
- [`.github/workflows/play-sites.yml`](.github/workflows/play-sites.yml): add matrix entry `{ project: "@mit-bestand/praesentation/projektetage", dist: "mit-bestand/präsentation/33.projektetage/dist" }` (same `index.html`/`.nojekyll`/`CNAME` verification step applies).
- [`.vscode/launch.json`](.vscode/launch.json): add a `3_dev` entry `🛠️dev📽️projektetage` (`bun nx run @mit-bestand/praesentation/projektetage:dev`, `PRAESENTATION_PROJEKTETAGE_PORT=6050`, `serverReadyAction` opening `http://localhost:6050`) and a `4_build` entry, following existing grouping/order.
...
- `bun nx run @mit-bestand/praesentation/projektetage:dev` serves on `:6050`; deck renders with `@ui/react` styling; the 4 playground iframes load from `localhost` ports (confirm via console/network, not assumption).
- `bun nx run @mit-bestand/praesentation/projektetage:build` produces `dist/` with `index.html`, `.nojekyll`, `CNAME`; built HTML references the public `play.*.semio-tech.com` iframe URLs.
