...
Finish [framework/product/os/core/rs](framework/product/os/core/rs) to full `os/core/js/index.ts` (3,095) coverage: every backbone, media export handler with coverage assertion, plugin/resource descriptor, parameter compatibility rule, studio catalog operation, OsStore alternative/checkpoint materialization path.
