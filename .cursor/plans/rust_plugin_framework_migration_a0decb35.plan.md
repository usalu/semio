...
overview: "Rebuild the framework technology as a Rust kernel with a WASM-Component program OS: apps become fully declarative Rust plugins with no DOM access, rendered by a single thin React renderer that implements a compile-time set of general-purpose UI components."
todos:
 - id: ticket
   content: Read repo://goals, open ticket for the migration
   status: completed
 - id: wit
   content: Define semio:framework WIT package (ui.wit, program.wit, component scenes)
   status: completed
 - id: kernel
 ...
 - id: devhost
   ...
   status: completed
 - id: pilot-draw
   content: Migrate draw as pilot program (document, controller, canvas-2d scene), verify boot + hot-swap in browser
   status: completed
 - id: apps-2d
...
flowchart TB
  ...
  plugins -->|"WIT: manifest, render(state) -> UiTree, handle(event) -> operations"| kernel
...
Then delete per-app TS: all `<tech>/core/js` app-shell/controller/play-id code, all `<tech>/react` packages, all `semio.app` manifests in `package.json` (replaced by WIT `manifest()`), `repo/lib/js/playground-manifest.ts`, and the `virtual:semio-playground-apps` program in `ui/styling/vite-elements-assets.ts`.
...
- Extend existing test files: kernel unit tests in Rust (`framework/core/rs`), program conformance tests (every plugin's `manifest()`/`render()` golden-checked), renderer interpretation tests in the existing vitest setup.
...
- `repo/lib/js/playground-manifest.ts`, playground virtual-module program in `ui/styling/vite-elements-assets.ts`, all `semio.app` blocks
...
- The generic `component-scene` types must cover today's bespoke canvases (e.g. draw's SVG features, note's math rendering, flow's WASM modules). Where a canvas already runs Rust/WASM (infinite/canvas, flow), the scene protocol routes data Rust-to-Rust and the React wrapper stays thin.
