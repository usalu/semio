...
- `framework/wit/` — WIT package `semio:framework`
  ...
  - `plugin.wit`: world `plugin` — exports `manifest() -> plugin-manifest` (apps, modes, window kinds, keybindings, programs, examples), `create-app(app-id) -> instance`, `handle-command(instance, command, doc-snapshot) -> list<edit-operation>`, `render(instance, doc-snapshot, view-state) -> ui-tree`; imports `host` (log, clock, asset fetch)
