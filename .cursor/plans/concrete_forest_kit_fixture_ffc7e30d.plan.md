...
- Binary geometry already exists in `compose/fixture/kit/folder/abbau-aufbau/`: `hexagonal-cut-concrete-forest-{left,right}.glb`, `…-{left,right}.3dm`, `stockwerk.3dm`, the `.gh`. These cannot be edited and are the source meshes.
- The dev kit placeholders are **empty**: [kit.compose.json](compose/fixture/kit/dev/abbau-aufbau/wip/initialKit/kit.compose.json), [types/hexagonal-cut-concrete-forest-left.json](compose/fixture/kit/dev/abbau-aufbau/wip/initialKit/type/hexagonal-cut-concrete-forest-left.json), [types/hexagonal-cut-concrete-forest-right.json](compose/fixture/kit/dev/abbau-aufbau/wip/initialKit/type/hexagonal-cut-concrete-forest-right.json). There is no `index.compose.json`.
- Reference formats: metabolism dev kit ([kit.compose.json](compose/fixture/kit/dev/metabolism/wip/initialKit/kit.compose.json) + `types/*.type.compose.json` + [index.compose.json](compose/fixture/kit/dev/metabolism/wip/initialKit/index.compose.json)); puzzle 3d fixture [nakagin-capsule-tower.3d.json](puzzle/3d/fixture/nakagin-capsule-tower.3d.json).
...
- Meshes are served at `/mesh/<name>.glb` only from the metabolism representations dir: `puzzle3dMetabolismMeshRoots` / `createPuzzle3dMeshesMiddleware` in [ui/styling/vite-elements-assets.ts](ui/styling/vite-elements-assets.ts) (108-180).

## Data mapping (given connectors → both formats)

The 4 ports are `b-l`, `b-l-m`, `b-s`, `b-s-m`, and per the user **all ports are mutually compatible**. Connector → compose uses `point`/`direction`/`t` + `port` ref; connector → puzzle3d uses a vortex (`vortexKind` = port id, `position` = point, `direction` = direction, `radius` 0.36).

```mermaid
flowchart LR
  conn["Connector(Id, Point, Direction, Port)"] --> compose["compose Type.connectors[] + family ports"]
  conn --> p3d["puzzle3d objectKind.vortices[] + kindCompatibility"]
  glb["folder/abbau-aufbau/*.glb"] --> mesh["/mesh/*.glb middleware"]
...
2. Replace the two empty `.json` placeholders with split-format files named `…-left.type.compose.json` and `…-right.type.compose.json` (the assembler in [compose/fixture/script.ts](compose/fixture/script.ts) only merges `*.type.compose.json`). Each type: 2 `representations` (the `.glb` tagged gltf-binary + the `.3dm` tagged vnd.3dm), and 7 `connectors` from the user's data (id/name, `point`, `direction`, `t` default 0, `port` ref by id).
3. Add `index.compose.json` listing the two types (mirrors metabolism's index).
...
2. Generalize mesh serving in [ui/styling/vite-elements-assets.ts](ui/styling/vite-elements-assets.ts): make the `/mesh/` middleware resolve from multiple kit representation roots (add `compose/fixture/kit/folder/abbau-aufbau`) with fallback lookup, and copy that root in the build step. Rename `puzzle3dMetabolismMeshRoots` to a kit-neutral name and update its callers/tests.
