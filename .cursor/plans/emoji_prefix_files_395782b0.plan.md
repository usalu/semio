...
4. **Rewrite references** (extend [rewrite-refs-v2.py](.repo/🎫️/26/07/29/MOVE-ALL-APPS-INTO-THE-S-PRODUCT-TREE-WITH-CONSTITUTIONAL-CRATES-EMOJI-LAYOUT/rewrite-refs-v2.py)):
   ...
   - `runVitest(..., "vitest.config.ts")` → new default in [`📦️index.ts` `runVitest`](🧰️framework/🛍️products/🦑️repo/🔨️modules/📚️library/⚡️implementations/🟦️typescript/📦️index.ts) and all call sites
