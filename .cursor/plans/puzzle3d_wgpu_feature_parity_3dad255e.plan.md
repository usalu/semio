...
The current Rust engine (`puzzle/3d/rs/lib.rs`) and program (`puzzle/3d/plugin/rs/lib.rs`) already model attractions, vortices, target volumes, and brush/fill logic — none of it reaches the renderer today. `world3d_scene()` only carries camera/meshes/instances/selection.
