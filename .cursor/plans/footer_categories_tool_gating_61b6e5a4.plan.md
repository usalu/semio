...
- For nodes in the `Commands` section: if `active_tool_id.is_some()` and not allowed by the list above, render the item disabled (muted icon/label via `theme.text_muted`, no hit registered) instead of skipping/hiding it, so users see _why_ it's unavailable. `Selection` and `History` sections are never gated.
