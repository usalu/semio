...
impl DisplayCase {
    ...
    pub fn simulate(&self, case_temperature_c: f64, ambient_c: f64, load_factor: f64) -> RefrigerationOutput {
        ...
        let anti_sweat = if ambient_c > 18.0 {
            self.anti_sweat_heater_w
        } else {
            0.0
        };
        ...
        RefrigerationOutput {
            ...
            compressor_power_w: compressor_w,
            condenser_heat_w: cooling_w + compressor_w * 0.85,
            mass_flow_kg_s: cooling_w / 150_000.0,
        }
    }
}
...
impl WalkIn {
    ...
    pub fn simulate(&self, ambient_c: f64, humidity_factor: f64) -> RefrigerationOutput {
        ...
        RefrigerationOutput {
            cooling_power_w: cooling_w,
            compressor_power_w: compressor_w,
            condenser_heat_w: cooling_w + compressor_w,
            mass_flow_kg_s: cooling_w / 140_000.0,
        }
    }
}
// #endregion 🔖WalkIn
...
impl CompressorRack {
    ...
    pub fn simulate(
        ...
        let eir = self.eir_curve.evaluate(plr)
            * self.eir_f_t_curve.evaluate_2d(evaporating_c, condensing_c).max(0.1);
        let compressor_w = load * eir;
        RefrigerationOutput {
            cooling_power_w: load,
            compressor_power_w: compressor_w,
            condenser_heat_w: load + compressor_w,
            mass_flow_kg_s: load / (150_000.0 * plr.max(0.2)),
        }
    }
...
pub fn refrigeration_state_from_pressures(
    suction_pa: f64,
    discharge_pa: f64,
) -> RefrigerationState {
    RefrigerationState {
        evaporating_temperature_c: r410a_saturation_temp_c(suction_pa.max(P_STD * 0.3)),
        condensing_temperature_c: r410a_saturation_temp_c(discharge_pa.max(P_STD)),
        suction_superheat_k: 5.0,
        liquid_subcool_k: 3.0,
    }
}
...
mod tests {
    ...
    fn display_case_cooling_positive() {
        let case = DisplayCase {
            length_m: 3.0,
            design_cooling_w: 2000.0,
            fan_power_w: 150.0,
            lighting_power_w: 200.0,
            anti_sweat_heater_w: 100.0,
            defrost_power_w: 500.0,
            defrost_fraction: 0.05,
            evaporating_temperature_c: -8.0,
        };
    ...
    fn walk_in_freezer_higher_compressor_ratio() {
        let cooler = WalkIn {
            ...
            wall_area_m2: 60.0,
            ua_w_per_k: 30.0,
            internal_gain_w: 500.0,
            design_box_temperature_c: 2.0,
            evaporating_temperature_c: -5.0,
            door_opening_fraction: 0.1,
        };
        let freezer = WalkIn {
            design_box_temperature_c: -20.0,
            evaporating_temperature_c: -28.0,
            ..cooler
        };
        let cool_out = cooler.simulate(25.0, 1.0);
        let freeze_out = freezer.simulate(25.0, 1.0);
        assert!(freeze_out.compressor_power_w / freeze_out.cooling_power_w
            > cool_out.compressor_power_w / cool_out.cooling_power_w);
    }

    #[test]
    fn compressor_rack_part_load() {
        let rack = CompressorRack {
            rated_capacity_w: 100_000.0,
            compressor_count: 2,
            eir_curve: PerformanceCurve::Constant(0.35),
            eir_f_t_curve: PerformanceCurve::Constant(1.0),
            min_evaporating_c: -15.0,
            max_condensing_c: 45.0,
        };
    ...
    fn condenser_approach_temperature() {
        let cond = RefrigerationCondenser {
            ua_w_per_k: 5000.0,
            fan_power_w: 3000.0,
            design_approach_k: 8.0,
            evaporative: false,
        };
    ...
    fn secondary_loop_return_rises_with_load() {
        let loop_sys = SecondaryLoop {
            pump_power_w: 800.0,
            pipe_ua_w_per_k: 20.0,
            supply_temperature_c: -5.0,
            return_temperature_c: -3.0,
            mass_flow_kg_s: 5.0,
            fluid_cp: 3500.0,
        };
