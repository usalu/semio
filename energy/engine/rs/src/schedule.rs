...
mod tests {
    ...
    fn constant_schedule_lookup() {
        let set = ScheduleSet {
            constants: vec![ConstantSchedule { id: ScheduleId(1), value: 0.5 }],
            ..Default::default()
        };
    ...
    fn daily_schedule_respects_limits() {
        let set = ScheduleSet {
            daily: vec![DailySchedule {
                id: ScheduleId(2),
                hourly_values: [2.0; 24],
                interpolation: ScheduleInterpolation::Discrete,
                limits: Some(ScheduleLimits { min: 0.0, max: 1.0 }),
            }],
            ..Default::default()
        };
