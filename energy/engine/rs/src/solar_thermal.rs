...
impl FlatPlateCollector {
    ...
    pub fn useful_gain_w(
        ...
        collector_thermal_output_w(
            CollectorKind::FlatPlate,
            self.area_m2,
            irradiance_w_m2,
            ambient_c,
            wind_m_s,
...
impl IntegralCollectorStorage {
    ...
    pub fn simulate(
        &self,
        storage_temperature_c: f64,
        irradiance_w_m2: f64,
        ambient_c: f64,
        dt_s: f64,
    ) -> (f64, f64) {
        let gain = collector_thermal_output_w(
            CollectorKind::IntegralCollectorStorage,
            self.area_m2,
            irradiance_w_m2,
            ambient_c,
            1.0,
...
impl UnglazedTranspiredCollector {
    ...
    pub fn preheat_air_w(
        ...
        let outlet_t = if m_dot > 1e-6 {
            ambient_c + gain / (m_dot * CP_DRY_AIR)
        } else {
            ambient_c
        };
        (gain, outlet_t)
...
impl PvtCollector {
    ...
    pub fn simulate(
        ...
        let thermal_w = collector_thermal_output_w(
            CollectorKind::PhotovoltaicThermal,
            self.area_m2,
            thermal_irrad,
            ambient_c,
            wind_m_s,
...
mod tests {
    ...
    fn flat_plate_gain_positive_at_noon() {
        let collector = FlatPlateCollector {
            area_m2: 10.0,
            tau_alpha: 0.75,
            ul_w_m2k: 3.5,
            iam_factor: 0.95,
    ...
    fn zero_irradiance_zero_gain() {
        let q = collector_thermal_output_w(
            CollectorKind::FlatPlate,
            5.0,
            0.0,
            15.0,
    ...
    fn ics_raises_storage_temperature() {
        let ics = IntegralCollectorStorage {
            area_m2: 3.0,
    ...
    fn unglazed_preheats_air() {
        let utc = UnglazedTranspiredCollector {
            area_m2: 50.0,
            porosity: 0.6,
            h_conv_w_m2k: 15.0,
            suction_velocity_m_s: 0.04,
        };
