//! 🌊 Breadth-first and depth-first traversal families over the generic `GraphView` traits.
...
// #region 🔖Shared
/// 🔗 Picks the first (lowest `EdgeId`) edge connecting `u` to `v`; safe to `expect` because every caller only invokes this for a pair already reported by `out_neighbors`/`neighbors`, which guarantees at least one connecting edge exists.
...
/// 🔌 Every edge incident to `node`: out-edges only for directed views (matching successor-direction traversal), all touching edges for undirected views. Parallel edges (multigraphs) each appear once per their `EdgeId`.
...
// #region 🔖Bfs
/// 🧭 Generic breadth-first edge traversal driven by a caller-supplied neighbor-ordering hook (NetworkX `generic_bfs_edges`); every other BFS function in this crate is built on top of this one.
...
/// 🔁 Breadth-first tree edges from `source`, in discovery order (NetworkX `bfs_edges`).
...
/// 🌳 Breadth-first spanning tree edges from `source` (NetworkX `bfs_tree`); NetworkX returns a tree graph, this crate returns the same edge list as `bfs_edges` — callers build a graph from it if a materialized tree is needed.
pub fn bfs_tree<G: GraphView>(graph: &G, source: NodeId) -> Vec<EdgeRef> {
    bfs_edges(graph, source)
}

/// 🧱 Multi-source breadth-first layering: `layers[0]` is the (deduplicated, existing) `sources`, `layers[k]` is every node first reached at distance `k` (NetworkX `bfs_layers`).
...
/// 📏 Every node whose shortest-path distance from `source` is exactly `distance` (NetworkX `descendants_at_distance`); `distance == 0` yields `{source}`, and an unreachable `distance` yields the empty set.
...
// #region 🔖Dfs
/// 🪆 Depth-first tree edges from `source`, iterative (explicit stack, no recursion) so traversal depth is bounded only by heap memory (NetworkX `dfs_edges`).
...
/// 🌲 Depth-first spanning tree edges from `source` (NetworkX `dfs_tree`); same simplification as `bfs_tree` — returns the edge list, not a materialized tree graph.
pub fn dfs_tree<G: GraphView>(graph: &G, source: NodeId) -> Vec<EdgeRef> {
    dfs_edges(graph, source)
}

/// 🔼 Nodes in depth-first preorder (parent emitted before its subtree) starting at `source` (NetworkX `dfs_preorder_nodes`).
...
/// 🔽 Nodes in depth-first postorder (a node emitted only after its whole subtree finishes) starting at `source` (NetworkX `dfs_postorder_nodes`).
...
/// ⏪ Maps every non-source node reached from `source` to its depth-first parent (NetworkX `dfs_predecessors`).
...
/// ⏩ Maps every node with depth-first tree children to the list of those children, in discovery order (NetworkX `dfs_successors`).
...
// #endregion 🔖EdgeTraversal

// #region 🔖Beam
/// 📡 Breadth-first traversal that keeps only the `width` highest-`value` nodes of each newly discovered layer before expanding further (NetworkX `bfs_beam_edges` instead prunes per-node successor lists to the top `width` by value; this crate prunes per-layer across the whole frontier, which is the semantics specified for this crate — documented deviation).
