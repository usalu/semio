//! 🚰 Resource-flow constraint: requires at least `min_flow` edge-disjoint paths (uniform
...
// #region 🔖Network
/// 🚰 A minimal adjacency-list max-flow network: paired forward/reverse edges at consecutive
...
impl FlowNetwork {
    ...
    /// 🚰 One BFS augmenting-path step: finds the shortest (fewest-edges) path from `s` to `t`
...
// #region 🔖Constraint
/// 🚰 Requires at least `min_flow` edge-disjoint paths from `sources` to `sinks`, through nodes
