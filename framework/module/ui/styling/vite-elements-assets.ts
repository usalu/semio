...
import type { PlaygroundAssetSpec } from "../../../product/os/module/plugin/registry/generated/playgrounds.ts";
// #endregion 🔌Adapters
...
export function semioFaviconSources(repoRoot: string): { readonly svg: string; readonly ico: string } {
  const logoRoot = resolve(repoRoot, "framework/module/asset/logo");
  return {
...
export const PLAYGROUND_PLAY_STATIC_ASSETS: readonly Extract<PlaygroundAssetSpec, { kind: "static-dir" }>[] = [
  ...
  { kind: "static-dir", route: "/infinite-fixture", root: "framework/product/os/module/infinite/fixture" },
];
...
export function createPlaygroundPlayViteConfig(options: PlaygroundPlayViteOptions) {
  ...
  const uiAssetsRoot = resolve(repoRoot, "framework/module/ui/asset");
  const osHubAliases =
    playEntryKind === "s"
      ? [
          {
...
if (import.meta.vitest) {
  ...
  describe("semioFaviconVitePlugin", () => {
    it("points at round dark emblem svg and ico under asset/logo", () => {
      ...
      expect(svg).toBe(resolve(repoRoot, "framework/module/asset/logo/emblem_dark_round.svg"));
      expect(ico).toBe(resolve(repoRoot, "framework/module/asset/logo/favicon_dark_round_32x32.ico"));
