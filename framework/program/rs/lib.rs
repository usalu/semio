//! 🔌 Declarative app program SDK — build fully declarative Rust apps bundled into hot-swappable WASM programs.

#[cfg(all(target_arch = "wasm32", target_env = "p2"))]
pub mod component {
    //! 🧩 WASI P2 component exports for the program world contract.

    use crate::program_runtime::{
        ensure_program_initialized, plugin_attach_backbone, plugin_clear_instance_guard, plugin_consume_media, plugin_create_app,
        ...
        program_manifest, plugin_produce_media, plugin_refresh_ui, plugin_render_with_document,
    };
    ...
    impl Guest for ComponentGuest {
        fn manifest() -> ProgramManifestJson {
            ensure_program_initialized();
            ProgramManifestJson {
                json: serde_json::to_string(&program_manifest()).unwrap_or_else(|_| "{}".into()),
            }
        }

        fn instantiate_app(app_id: String, _instance_id: String) -> Result<u32, ProgramError> {
            ensure_program_initialized();
            plugin_create_app(&app_id).map_err(ProgramError::Message)
        }
        ...
        ) -> Result<InvocationResponseJson, ProgramError> {
            ensure_program_initialized();
            let result = plugin_handle_action(instance_id, &action.json, &context.json)
                .map_err(ProgramError::Message)?;
        ...
        ) -> Result<WindowOutputJson, ProgramError> {
            ensure_program_initialized();
        ...
        ) -> Result<UiRefreshResponseJson, ProgramError> {
            ensure_program_initialized();
            let json = plugin_refresh_ui(instance_id, &request.json).map_err(ProgramError::Message)?;
            Ok(UiRefreshResponseJson { json })
        }

        fn migrate_document(_input: MigrateDocumentInput) -> Result<MigrateDocumentOutput, ProgramError> {
            Err(ProgramError::Message("migrate-document not implemented".into()))
        }

        fn apply_operations(instance_id: u32, operations: Vec<u8>) -> Result<(), ProgramError> {
            ensure_program_initialized();
            plugin_ingest_operations(instance_id, &operations).map_err(ProgramError::Message)
        }

        fn apply_operations_text(instance_id: u32, operations_text: String) -> Result<(), ProgramError> {
            ensure_program_initialized();
            plugin_ingest_operations_text(instance_id, &operations_text).map_err(ProgramError::Message)
        }

        fn read_app_document_text(instance_id: u32) -> Result<DocumentTextFiles, ProgramError> {
            ensure_program_initialized();
            let files = plugin_document_text(instance_id).map_err(ProgramError::Message)?;
            Ok(DocumentTextFiles { dsl: files.dsl, ops: files.ops })
        }

        fn load_app_document_text(instance_id: u32, document_text: DocumentTextFiles) -> Result<(), ProgramError> {
            ensure_program_initialized();
            let files = store::DocumentTextFiles { dsl: document_text.dsl, ops: document_text.ops };
            plugin_load_document_text(instance_id, &files).map_err(ProgramError::Message)
        }

        fn read_app_document_pack(instance_id: u32) -> Result<DocumentPackFiles, ProgramError> {
            ensure_program_initialized();
            let files = plugin_document_pack(instance_id).map_err(ProgramError::Message)?;
            Ok(DocumentPackFiles { pack: files.pack, spr: files.spr, ops: files.ops })
        }

        fn load_app_document_pack(instance_id: u32, document_pack: DocumentPackFiles) -> Result<(), ProgramError> {
            ensure_program_initialized();
            let files = store::DocumentPackFiles { pack: document_pack.pack, spr: document_pack.spr, ops: document_pack.ops };
            plugin_load_document_pack(instance_id, &files).map_err(ProgramError::Message)
        }

        fn attach_backbone(instance_id: u32, uri: String) -> Result<(), ProgramError> {
            ensure_program_initialized();
            plugin_attach_backbone(instance_id, &uri).map_err(ProgramError::Message)
        }

        fn detach_backbone(instance_id: u32) -> Result<(), ProgramError> {
            ensure_program_initialized();
            plugin_detach_backbone(instance_id).map_err(ProgramError::Message)
        }

        fn consume_media(instance_id: u32, port_id: String, artifact: MediaArtifact) -> Result<(), ProgramError> {
            ensure_program_initialized();
            plugin_consume_media(instance_id, &port_id, &artifact.descriptor_json, artifact.data).map_err(ProgramError::Message)
        }

        fn produce_media(instance_id: u32, port_id: String, request_json: String) -> Result<MediaArtifact, ProgramError> {
            ensure_program_initialized();
            let (descriptor_json, data) = plugin_produce_media(instance_id, &port_id, &request_json).map_err(ProgramError::Message)?;
...
pub mod app {
// #region app
//! 🧩 Declarative app builder and program trait.
...
use semio_framework_core::{
    ...
    ModeDefinition, Modes, PanelGroup, PanelTabDefinition, PanelTabKind, ProgramManifest, WorkflowDefinition, ToolDefinition, ToolRef, TutorialDefinition,
...
/// below) — the program ABI's `consume-media`/`produce-media` payload framing, separate from
...
impl AppBuilder {
    ...
    /// supplied dynamically via `DocumentApp::tool_measures`/`ProgramApp::tool_measures`.
...
// ~15 program crates (flow, procedural, layout, gis, puzzle, sequence, trinity, dag, …) into the SDK.
...
/// skeleton duplicated across program crates: namespaced item ids, sections (optionally substituting a
...
// (and the sibling `entity_detail` read-only `KeyValue` summary block) duplicated across program crates
...
use super::{ActionMeta, App, AppActionRegistry, DocumentApp, ProgramApp, VcsDocumentApp};
...
/// two-instance-convergence test body (see `playbook-program`'s
...
mod app_builder_tests {
    ...
    #[test]
    fn build_definition_rejects_os_or_program_scope_command() {
        ...
        assert!(result.is_err(), "AppBuilder must reject Os/Plugin-scope commands — those are declared by the shell/ProgramBundle, not an app");
    }
...
impl App {
    ...
    /// (round-trips through the program manifest; `semio_framework_os`'s artifact catalog registry
    /// consumes it from there at program registration time).
...
/// @emoji 🪪 Per-invocation runtime metadata handed to the object-safe {@link ProgramApp} — the local
...
pub trait DocumentApp: Send + 'static {
    ...
    /// 🎛️ Handles a dispatched `CommandDefinition` (os/program/app/mode-scoped — never window-level).
    ...
    /// (Document/Catalogue/Inspection/Parameters) are merged in automatically by `program_runtime::program_app_labels`
...
/// level, not a specific `MediaPortSpec`, so `ProgramApp::{consume_media, produce_media}` below default
...
/// 🚧 Failure producing/consuming a `MediaArtifact` through `ProgramApp::{produce_media, consume_media}`.
...
pub trait ProgramApp: Send {
    ...
    /// `ProgramApp` gets for free without declaring any `media_ports()`. Apps whose media output isn't
...
/// (default materialization, required-arg validation, kind discipline) without the program re-checking.
...
/// {@link ProgramApp}. Owns a persistent `DocumentStore<Projection, Operation>` — the single source of
...
impl<A: DocumentApp> VcsDocumentApp<A> {
    ...
    /// `render`'s `UiNode` output, for callers (host code, downstream program crates' own tests) that
...
impl<A: DocumentApp> ProgramApp for VcsDocumentApp<A> {
    fn app_id(&self) -> &str {
        self.app.app_id()
    }
...
pub struct AppInstance {
    pub id: u32,
    pub app: Box<dyn ProgramApp>,
}
//#endregion 🔖DocumentContract

pub trait Program: Send {
    fn manifest(&self) -> ProgramManifest;
    fn create_app(&self, app_id: &str) -> Option<Box<dyn ProgramApp>>;
}

pub struct ProgramBundle {
    pub manifest: ProgramManifest,
    apps: HashMap<String, Box<dyn Fn() -> Box<dyn ProgramApp> + Send + 'static>>,
}

impl ProgramBundle {
    pub fn new(program_id: impl Into<String>, label: impl Into<String>, version: impl Into<String>) -> Self {
        Self {
    ...
    /// focused). Panics if `command.scope != CommandScope::Program`.
    pub fn plugin_command(mut self, command: CommandDefinition) -> Self {
        assert!(
            command.scope == CommandScope::Program,
            "plugin {} command {} must be declared CommandScope::Program",
    ...
    fn register_app(
        mut self,
        app: App,
        factory: impl Fn() -> Box<dyn ProgramApp> + Send + 'static,
    ) -> Self {
    ...
    /// {@link VcsDocumentApp} so it satisfies the object-safe runtime {@link ProgramApp} contract with
    ...
    pub fn create_app(&self, app_id: &str) -> Option<Box<dyn ProgramApp>> {
        self.apps.get(app_id).map(|factory| factory())
    }
}

impl Program for ProgramBundle {
    fn manifest(&self) -> ProgramManifest {
        self.manifest.clone()
    }

    fn create_app(&self, app_id: &str) -> Option<Box<dyn ProgramApp>> {
        ProgramBundle::create_app(self, app_id)
    }
}
// #endregion app
}
...
// #region program_runtime
//! 📤 WASM component export glue for program bundles.

use crate::app::{ActionMeta, AppInstance, MediaArtifact, MediaArtifactDescriptor, Program, ProgramBundle};
use crate::DocumentApp;
use semio_framework_core::{kernel::{HostEffect, InvocationResult}, ProgramManifest, ViewState};
...
thread_local! {
    static PLUGIN: RefCell<Option<ProgramBundle>> = const { RefCell::new(None) };
    ...
    // serialized program bridge instead.
...
impl InstanceGuard {
    fn enter() -> Result<Self, String> {
        if INSTANCE_GUARD.get() > 0 {
            return Err("program instance busy".to_string());
        }
...
fn with_instances_mut<R, F: FnOnce(&mut Vec<AppInstance>) -> Result<R, String>>(f: F) -> Result<R, String> {
    ...
    // SAFETY: `InstanceGuard` + the JS/host serialized program bridge ensure exclusive access.
...
/// `program instance busy`. No-operation when the guard is already clear.
...
pub fn install_program_bundle(bundle: ProgramBundle) {
    PLUGIN.with(|slot| {
...
extern "C" {
    fn semio_program_install_bundle();
}

/// Ensures the embedding program crate's bundle installer ran before any WIT export is served.
pub fn ensure_program_initialized() {
    PLUGIN_INIT_ONCE.call_once(|| {
        #[cfg(all(target_arch = "wasm32", target_env = "p2"))]
        crate::host_port::register_host_backbone_channel();
        unsafe {
            semio_program_install_bundle();
        }
    });
}

pub fn program_manifest() -> ProgramManifest {
    ensure_program_initialized();
    PLUGIN.with(|slot| {
        slot.borrow()
            .as_ref()
            .map(|program| program.manifest())
            .unwrap_or_else(|| ProgramManifest {
...
pub fn plugin_create_app(app_id: &str) -> Result<u32, String> {
    PLUGIN.with(|slot| {
        ...
        let program = program.as_ref().ok_or_else(|| "program not initialized".to_string())?;
        let app = program
            .create_app(app_id)
...
/// @emoji 🎛️ Dispatches a scoped command (os/program/app/mode) through the same instance/context
...
/// and dispatches to `ProgramApp::consume_media`.
...
/// 🎞️ WIT `produce-media` glue — dispatches to `ProgramApp::produce_media` and encodes the result back
/// into `(descriptor-json, data)` for the `media-artifact` WIT record. `_request_json` is reserved for
/// future parameterized requests; unused by the SDK default (see `ProgramApp::produce_media`).
...
/// tradeoff (mirrors the identical pattern already used for `cached_fixture_json` in puzzle's program).
...
/// program section — the renderer derives them from the utility registry via `derive_utility_nodes`.
pub fn plugin_refresh_ui(instance_id: u32, request_json: &str) -> Result<String, String> {
    ...
    with_instances_mut(|list| {
        ...
        // 🚧 `utilities` intentionally unhandled here: `ProgramApp`/`DocumentApp` currently expose no
...
#[macro_export]
macro_rules! program_exports {
    ($bundle_fn:expr) => {
        #[doc(hidden)]
        #[unsafe(no_mangle)]
        pub extern "C" fn semio_program_install_bundle() {
            $crate::program_runtime::install_program_bundle(($bundle_fn)());
        }
...
/// 🧵 Collapses a program crate's hand-written `bundle()` fn + `program_exports!` call into one
/// declarative block: `semio_framework_program::semio_program! { id: "note", label: "Note", version:
...
/// satisfies the object-safe runtime [`ProgramApp`](crate::ProgramApp) contract with a persistent operation
/// store. Expands to the equivalent `bundle()` fn plus a `program_exports!(bundle)` call, and a
/// `#[cfg(test)]` regression check asserting every declared app id actually lands in the built
/// `ProgramBundle`'s manifest.
#[macro_export]
macro_rules! semio_program {
    (
        id: $id:literal,
        label: $label:literal,
        ...
        fn __semio_program_bundle() -> $crate::ProgramBundle {
            ($setup)();
            $crate::ProgramBundle::new($id, $label, $version)
                $( .register_document_app(($app_fn)(), || <$app_ty as ::std::default::Default>::default()) )+
        }

        $crate::program_exports!(__semio_program_bundle);

        #[cfg(test)]
        #[test]
        fn __semio_program_sanity_declared_apps_appear_in_bundle_manifest() {
            let manifest = __semio_program_bundle().manifest;
            $(
                let expected_id = ($app_fn)().definition.id;
                assert!(
                    manifest.apps.iter().any(|app| app.id == expected_id),
                    "semio_program({}): app `{}` (from `{}`) missing from bundle manifest",
                    $id,
                    expected_id,
                    stringify!($app_fn),
                );
...
#[cfg(test)]
mod semio_program_macro_tests {
    //! 🧪 The program contract's own unit test: a `TestApp` implementing the typed `DocumentApp`
    ...
    use crate::app::{
        ActionEmit, ActionMeta, App, AppActionRegistry, DocumentApp, DocumentView, ProgramApp, VcsDocumentApp,
    };
    ...
    #[test]
    fn semio_program_macro_builds_bundle_from_declarative_spec() {
        let bundle = __semio_program_bundle();
        assert_eq!(bundle.manifest.program_id, "synthetic");
    ...
    #[test]
    fn semio_program_macro_wires_app_factory_for_create_app() {
        let bundle = __semio_program_bundle();
...
// #endregion program_runtime
}

pub mod world3d_host {
// #region world3d_host
//! 🌐 Shared world-3d scene payload builders for program apps.
...
pub use app::{
    ...
    KeybindingSpec, MediaClass, MediaType, ModeSpec, OsMediaCapability, PanelTabSpec, PanelTreeBuilder, Program, ProgramApp,
    ProgramBundle, ArtifactKindSpec, VcsDocumentApp, WindowKindSpec,
};
...
pub use program_runtime::{
    install_program_bundle, plugin_attach_backbone, plugin_detach_backbone, plugin_document_pack,
...
// apps keep the flat `semio_framework_program::*` import surface with zero Cargo.toml churn.
pub use ui_wgpu::*;

#[macro_export]
macro_rules! register_program {
    ($bundle:expr) => {
        $crate::program_runtime::install_program_bundle($bundle);
    };
}
