...
//#region ContextMenu
function buildGisMapContextMenuItems(
	scene: GisMapScene,
	...
	if (feature) {
		const selected =
			feature.kind === "position" ? selection.positions.includes(feature.id) : selection.routes.includes(feature.id);
...
export function GisMapHost({
	...
	return (
		<div
			ref={containerRef}
			className="semio-gis-map-host absolute inset-0 box-border min-h-0 min-w-0 overflow-hidden select-none"
