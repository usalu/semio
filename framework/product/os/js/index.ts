 ...
 * plain (not renamed), so it stays snake_case like the Rust source. 🎯 W5: `diff`/`inverse` payloads
 ...
 * `protocol_core`'s `write_varint_u64` (`protocol/core/rs/lib.rs` `🔖WireCodec`). */
...
/** 🎯 `operation_id str | document_id str | actor str | dependencies vec<str> | diff.schema str |
...
/** 🎯 Inverse of {@link encodeEnvelope} — the TS twin of Rust `protocol_causal::decode_envelope`. */
...
/** 🎯 `document_id str | head_edit_ordinal varint | head_edit_id str | last_commit_seq varint |
...
/** 🎯 Inverse of {@link encodeFrontier} — the TS twin of Rust `protocol_causal::decode_frontier`. */
...
/** 📤 Encodes one `ClientFrame` on the given lane: `lane u8 | tag u8 | fields` — the TS twin of
...
/** 📤 Encodes one `ServerFrame` on the given lane: `lane u8 | tag u8 | fields` — the TS twin of
...
export type DocumentActorConfig = {
  ...
  readonly actor: string;
  /** 🧬 W5.7: this document kind's `store::DocumentCodec.pack_schema_hash`, for hub schema-hash
...
/** 🤝 TS twin of Rust `MediaContract` (`workflow::MediaContract`, hand-written `dsl::DslField`). */
...
/** 🔬 TS twin of Rust `WorkflowFixture` — decoded from the shared `.dsl`/`.spk` fixture pairs via wasm, never JSON. */
...
if (import.meta.vitest) {
  ...
  describe("@semio-tech/framework-os-core workflow", () => {
    ...
    // 🔬 Shared fixtures replay (`framework/os/core/fixtures/*.dsl`+`*.spk`) — the same files
    ...
    it("matches the Rust plan_workflow across shared fixtures decoded via wasm", async () => {
      ...
      const rsPkgDir = join(here, "..", "rs", "pkg");
