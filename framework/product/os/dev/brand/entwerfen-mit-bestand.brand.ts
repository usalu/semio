...
/** 🏷️ The Aggregator ships puzzle3d with locked German locale, locked reuse terminology (window "Aggregator", document "Entwerfen mit Bestand", example "Abbau Aufbau"), locked semio theme, switchable appearance, a brand-owned German introduction, and Abbau Aufbau (`concrete-forest`) seeded as the default-but-switchable example. Ephemeral: nothing survives a window refresh — dock, panes, chrome prefs, and the introduction all reset to brand defaults. Anchors reference `puzzle/plugin/rs/lib.rs`'s puzzle3d app (`puzzle3d-main`, `move`, `addObjectKind`) and `framework/core/js`'s `FRAMEWORK_PANEL_TAB_CATALOGUE_ID`. Tour order: viewport → catalogue (uncollapsed panel cutout) → drag-and-drop the first catalogue component (with all world-3d windows unveiled as drop targets) → move. */
