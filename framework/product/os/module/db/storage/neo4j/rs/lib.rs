...
//!
//! ⏳ Sync boundary: `neo4rs` is fully async (`tokio`); every `DbStorage` sub-trait method here is
...
//!
//! 💾 Durability: every write here is its own committed (or txn-committed) Cypher statement — by
...
//!
//! 🚧 Extension seam (documented, not a TODO): read-modify-write operations (`append`,
...
/// @emoji 📖 Inverse of `encode_bytes`. Never panics on malformed input — a corrupt/hand-edited
...
/// @emoji 🔢 Neo4j's `Integer` bolt type is a signed 64-bit value; the family's identity/sequence
...
/// @emoji 🔢 Inverse of `u64_to_i64` for values read back from a Neo4j `Integer` property. A
...
//#region 🔖WalLaws
/// @emoji ➕ The pure decision behind `WalStorage::append`: reject a sealed segment, otherwise
...
//#region 🔖LeaseLaws
/// @emoji ⏳ One lease row as read back from Neo4j: `(fence, expires_at_ms, holder)`.
type LeaseRow = (EpochFence, u64, String);

/// @emoji 🤝 The pure decision behind `LeaseStorage::acquire` — see `MemoryStorage::acquire`'s
...
//#region 🔖ErrorMapping
/// @emoji 🚨 Maps a `neo4rs::Error` into the family's single `DbError` — never lets a foreign
...
fn map_neo4rs_error(err: neo4rs::Error) -> DbError {
    match &err {
        ...
        // 🎯 Everything else (protocol/serialization/config-shape errors) is this driver/process's
...
/// @emoji 🚨 Maps a row-decoding error (a property missing or of the wrong bolt type) into
...
//#endregion 🔖ErrorMapping

//#region 🔖Cypher
/// @emoji 🧱 Uniqueness constraints (single-property, Community-Edition-compatible) and
...
const CYPHER_SNAPSHOT_DELETE: &str = "
    MATCH (n:SnapshotGeneration {document: $document, generation: $generation})
    DETACH DELETE n";
//#endregion 🔖SnapshotCypher

//#region 🔖PayloadCypher
...
const CYPHER_PAYLOAD_DELETE: &str = "
    MATCH (p:Payload {hash: $hash})
    DETACH DELETE p";
//#endregion 🔖PayloadCypher

//#region 🔖CatalogCypher
const CYPHER_CATALOG_READ: &str = "
    MATCH (c:CatalogRoot {id: 'root'})
    RETURN c.epoch AS epoch, c.bytes AS bytes, c.len AS len";

/// @emoji ✅ Single-statement compare-and-swap: `OPTIONAL MATCH` never creates a node, so a failed
...
impl Neo4jStorage {
    /// @emoji 🔌 Connects to `uri` with `user`/`password` (default database) and bootstraps
    /// `SCHEMA_STATEMENTS`.
    ...
    /// @emoji ⏳ Drives `fut` to completion on this backend's dedicated runtime — see module doc
    /// for why this is never a nested-runtime `block_on`.
    fn block_on<F: std::future::Future>(&self, fut: F) -> F::Output {
        self.runtime.block_on(fut)
    }

    /// @emoji 1️⃣ Runs `q` (autocommit) and returns its first row, if any.
...
impl WalStorage for Neo4jStorage {
    fn create_segment(&self, document: &DocumentId, index: u64) -> Result<(), DbError> {
        ...
        self.block_on(async {
            ...
            // 🎯 `MERGE` always yields exactly one row; an empty stream here means the driver
    ...
    fn sync(&self, _document: &DocumentId, _index: u64, _class: DurabilityClass) -> Result<(), DbError> {
        // 🎯 See module doc's "Durability" section: every prior `append`/`seal` already committed
...
impl CatalogStorage for Neo4jStorage {
    ...
    fn cas_root(&self, expected: EpochFence, new_bytes: &[u8]) -> Result<EpochFence, DbError> {
        ...
        // 🎯 The CAS attempt itself was atomic (see `CYPHER_CATALOG_CAS`'s doc); this follow-up
...
//#region 🔖LeaseStorage
impl Neo4jStorage {
    /// @emoji 📖 Reads `resource`'s current lease row (regardless of expiry — callers decide what
...
impl DbStorage for Neo4jStorage {
    ...
    fn capabilities(&self) -> StorageCapabilities {
        // 🎯 See module doc's "Durability" section: every write already committed server-side by
...
mod tests {
    ...
    fn map_de_error_maps_to_corrupt() {
        // 🎯 `neo4rs::DeError` has no public constructor reachable without a live row (its variants
    ...
    fn capabilities_report_durable_cas_and_fsync_backed_storage() {
        // 🎯 Exercises the `capabilities()` shape without a live connection (constructing a full
