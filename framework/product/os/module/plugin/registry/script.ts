...
function findPluginCargoFiles(root: string): string[] {
  ...
  function walk(dir: string) {
    for (const name of readdirSync(dir)) {
      ...
      if (st.isDirectory()) {
        walk(path);
      } else if (name === "Cargo.toml" && !path.includes("/framework/product/os/module/plugin/rs/")) {
        const isModuleCrate = /\/module\/[^/]+\/rs\/Cargo\.toml$/.test(path);
        // 🏛️ Contribution bundle crate: `s/plugin/<p>/extension/<e>/rs/Cargo.toml` (new node/document
        // types the plugin contributes, e.g. imperative's control/logic/math/text).
        const isExtensionCrate = /\/s\/plugin\/[^/]+\/extension\/[^/]+\/rs\/Cargo\.toml$/.test(path);
        // 🏛️ Plugin bundle crate lives at `s/plugin/<p>/manifest/artifact/rs/Cargo.toml`.
        const isPluginBundleCrate = /\/s\/plugin\/[^/]+\/manifest\/artifact\/rs\/Cargo\.toml$/.test(path);
...
function discoverExamplesForPlayground(repoRoot: string, cratePath: string, pluginId: string, variant: string): string[] {
  ...
  if (variant.startsWith(pluginId) && variant.length > pluginId.length) {
    ...
    const dir = join(repoRoot, techRoot, "app", suffix, "example");
