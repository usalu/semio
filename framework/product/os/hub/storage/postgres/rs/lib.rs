...
impl PostgresStorage {
    ...
    pub async fn seed(&self) -> StorageResult<()> {
        ...
        let exists: (i64,) = sqlx_core::query_as::query_as("SELECT COUNT(*) FROM hub_studio WHERE id = 'default'").fetch_one(&self.pool).await.map_err(backend)?;
        if exists.0 == 0 {
            sqlx_core::query::query("INSERT INTO hub_studio (id, name, owner_user_id, created_at) VALUES ('default', 'Studio', 'seed', $1)").bind(now_ms()).execute(&self.pool).await.map_err(backend)?;
        }
...
impl HubStorage for PostgresStorage {
    ...
    async fn ensure_document(&self, studio_id: &str, id: &str) -> StorageResult<DocumentRecord> {
        let existing: Option<(String, String, serde_json::Value, i64)> = sqlx_core::query_as::query_as("SELECT studio_id, schema, snapshot, version FROM hub_document WHERE id = $1").bind(id).fetch_optional(&self.pool).await.map_err(backend)?;
        ...
        sqlx_core::query::query("INSERT INTO hub_document (id, studio_id, schema, snapshot, version) VALUES ($1, $2, $3, $4, 0)").bind(id).bind(studio_id).bind(&schema).bind(&snapshot).execute(&self.pool).await.map_err(backend)?;
    ...
    async fn list_nodes(&self, studio_id: &str, parent: Option<&str>) -> StorageResult<Vec<NodeRecord>> {
        let rows: Vec<(String, Option<String>, String, String)> = match parent {
            Some(parent) => sqlx_core::query_as::query_as("SELECT id, parent_id, name, kind FROM hub_node WHERE studio_id = $1 AND parent_id = $2 ORDER BY name").bind(studio_id).bind(parent).fetch_all(&self.pool).await.map_err(backend)?,
            None => sqlx_core::query_as::query_as("SELECT id, parent_id, name, kind FROM hub_node WHERE studio_id = $1 AND parent_id IS NULL ORDER BY name").bind(studio_id).fetch_all(&self.pool).await.map_err(backend)?,
        };
        Ok(rows.into_iter().map(|(id, parent_id, name, kind)| NodeRecord { id, studio_id: studio_id.to_string(), parent_id, name, kind }).collect())
    }

    async fn create_node(&self, studio_id: &str, parent_id: Option<&str>, name: &str, kind: &str) -> StorageResult<NodeRecord> {
        let id = Uuid::now_v7().to_string();
        sqlx_core::query::query("INSERT INTO hub_node (id, studio_id, parent_id, name, kind) VALUES ($1, $2, $3, $4, $5)").bind(&id).bind(studio_id).bind(parent_id).bind(name).bind(kind).execute(&self.pool).await.map_err(backend)?;
    ...
    async fn create_studio(&self, name: &str, owner_user_id: &str) -> StorageResult<StudioRecord> {
        let id = Uuid::now_v7().to_string();
        let created_at = now_ms();
        sqlx_core::query::query("INSERT INTO hub_studio (id, name, owner_user_id, created_at) VALUES ($1, $2, $3, $4)").bind(&id).bind(name).bind(owner_user_id).bind(created_at).execute(&self.pool).await.map_err(backend)?;
    ...
    async fn list_studios_for_user(&self, user_id: &str) -> StorageResult<Vec<(StudioRecord, StudioRole)>> {
        ...
        .await
        .map_err(backend)?;
        Ok(rows.into_iter().filter_map(|(id, name, owner_user_id, created_at, role)| StudioRole::parse(&role).map(|role| (StudioRecord { id, name, owner_user_id, created_at }, role))).collect())
    }

    async fn list_studios(&self, limit: i64, offset: i64) -> StorageResult<Vec<StudioRecord>> {
        let rows: Vec<(String, String, String, i64)> =
            sqlx_core::query_as::query_as("SELECT id, name, owner_user_id, created_at FROM hub_studio ORDER BY created_at LIMIT $1 OFFSET $2").bind(limit).bind(offset).fetch_all(&self.pool).await.map_err(backend)?;
    ...
    async fn list_documents_for_studio(&self, studio_id: &str) -> StorageResult<Vec<DocumentRecord>> {
        let rows: Vec<(String, String, String, serde_json::Value, i64)> =
            sqlx_core::query_as::query_as("SELECT id, studio_id, schema, snapshot, version FROM hub_document WHERE studio_id = $1").bind(studio_id).fetch_all(&self.pool).await.map_err(backend)?;
        Ok(rows.into_iter().map(|(id, studio_id, schema, snapshot, version)| DocumentRecord { id, studio_id, schema, snapshot, version }).collect())
    }
    ...
    async fn remove_membership(&self, studio_id: &str, user_id: &str) -> StorageResult<()> {
        sqlx_core::query::query("DELETE FROM hub_studio_membership WHERE studio_id = $1 AND user_id = $2").bind(studio_id).bind(user_id).execute(&self.pool).await.map_err(backend)?;
        Ok(())
    }

    async fn get_role(&self, studio_id: &str, user_id: &str) -> StorageResult<Option<StudioRole>> {
        let row: Option<(String,)> = sqlx_core::query_as::query_as("SELECT role FROM hub_studio_membership WHERE studio_id = $1 AND user_id = $2").bind(studio_id).bind(user_id).fetch_optional(&self.pool).await.map_err(backend)?;
    ...
    async fn record_sync_session_open(&self, document_id: &str, user_id: Option<&str>, studio_role: Option<StudioRole>, client_label: &str) -> StorageResult<SyncSessionRecord> {
        ...
        sqlx_core::query::query("INSERT INTO hub_sync_session (id, document_id, user_id, studio_role, client_label, connected_at) VALUES ($1, $2, $3, $4, $5, $6)")
            .bind(&id)
            .bind(document_id)
            .bind(user_id)
            .bind(role_str)
    ...
    async fn get_blob(&self, hash: &str) -> StorageResult<Option<Vec<u8>>> {
        let row: Option<(Vec<u8>,)> = sqlx_core::query_as::query_as("SELECT bytes FROM hub_blob WHERE hash = $1").bind(hash).fetch_optional(&self.pool).await.map_err(backend)?;
        Ok(row.map(|(bytes,)| bytes))
    }

    async fn has_blob(&self, hash: &str) -> StorageResult<bool> {
        let row: (i64,) = sqlx_core::query_as::query_as("SELECT COUNT(*) FROM hub_blob WHERE hash = $1").bind(hash).fetch_one(&self.pool).await.map_err(backend)?;
        Ok(row.0 > 0)
    }
    //#endregion
}
