...
impl HubDirectory for PostgresDirectory {
    ...
    //#region SyncSessions
    async fn record_sync_session_open(&self, document_id: &str, user_id: Option<&str>, studio_role: Option<StudioRole>, client_label: &str) -> DirectoryResult<SyncSessionRecord> {
        let id = Uuid::now_v7().to_string();
...
mod tests {
    ...
    #[tokio::test]
    async fn seed_creates_studio_and_node_tree() {
        let (directory, _container) = test_directory().await;
