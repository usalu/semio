mod header {
    ...
    //
    // 🎯 Design choice (split from the pre-CW6 `HubStorage`): document persistence (snapshots,
...
//#region 🔖Error
pub mod error {
    /// @emoji 🧯 Opaque directory error — never wraps a backend driver's error type, so no `sqlx`/
