...
import { oklabMix } from "../../module/ui/styling/script.ts";

const printRoot = import.meta.dir;
const repoRoot = getWorkspaceRoot();
const tokensPath = join(repoRoot, "framework/ui/styling/tokens.json");
...
/** @emoji 🎨 Writes `tex/semio-tokens.sty` from {@link framework/ui/styling/tokens.json}. */
export function emitSemioTokensSty(): void {
  const tokens = JSON.parse(readFileSync(tokensPath, "utf8")) as Tokens;
  const lines: string[] = ["% Generated from framework/ui/styling/tokens.json — run `bun ./script.ts generate`.", "\\NeedsTeXFormat{LaTeX2e}", "\\ProvidesPackage{semio-tokens}[2026/07/06 v0.1.0 semio design tokens]", "\\RequirePackage{xcolor}", ""];
