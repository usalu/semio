...
export const PRESENTATION_PLAY_SURFACE_ID = "presentation.tile.play";
...
export class PresentationPlayController extends Controller implements PlaygroundExampleHost {
	...
	private readonly docStore = new DocumentVcsStore<PresentationDeck, PresentationEditOp>({
		envelope: createDocumentVcsEnvelope("presentation.deck", "presentation-tile-play", {
	...
	private projection(): PresentationDeck {
	...
	private commitDeck(next: PresentationDeck): void {
	...
	getDocumentVcsStore(): DocumentVcsStore<PresentationDeck, PresentationEditOp> {
...
//#region 🔖MediaExport
async function presentationDeckToPngDataUrl(deck: PresentationDeck): Promise<{ dataUrl: string; width: number; height: number }> {
...
function presentationDeckToSvg(deck: PresentationDeck, width: number, height: number, pngDataUrl: string): string {
...
export function registerPresentationMediaExportHandlers(): void {
	registerOsMediaExportHandler("presentation.deck", "png", async (doc) => {
		const deck = doc as PresentationDeck;
