...
When auto-animating many-to-one, then you MUST use ghosts to fix the problem. The source dispositions (called source ghost) are morphing into a ghost with the same target position and size. The target disposition (target ghost) already exists "too early" in the previous slide as ghost on the target position and size waiting to be faded inplace turning the opactiy from 0 to 100% in by the transition.
