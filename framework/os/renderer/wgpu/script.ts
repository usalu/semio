...
const outDir = join(repoRoot, "framework/os/dev/renderer-modules/wgpu");
...
function buildBootScript(bundleRoot: string): void {
  ...
  const sessionPath = join(repoRoot, "framework/os/dev/generated/session.ts");
...
class NativeBuildScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    ...
    const osDevScript = join(repoRoot, "framework/os/dev/script.ts");
