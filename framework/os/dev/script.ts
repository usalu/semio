...
const pluginOutRoot = join(repoRoot, "framework/os/dev/plugin-modules");
const playgroundSessionPath = join(repoRoot, "framework/os/dev/generated/session.ts");
...
async function buildPlugins(filterPlugin?: string): Promise<void> {
  ...
  const stalePublicPlugins = join(repoRoot, "framework/os/dev/public/plugin-modules");
...
class DevScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    ...
    const defaultPort = String(frameworkOsPlaygroundDefaultPort(playgroundCatalog, program, renderer));
    if (renderer === "wgpu") {
      const host = process.env.DEVCONTAINER === "true" ? "0.0.0.0" : "127.0.0.1";
      const port = Number(process.env.S_OS_PORT ?? defaultPort);
      const playUrl = wgpuDevPlayUrl(host, port, program);
      if (isDevPortInUse(host, port)) {
        ...
        if (occupant?.startsWith("trunk")) {
          ...
          console.log(`[dev] Port ${port} already serving legacy wgpu trunk at ${wgpuDevPlayUrl(host, port, program, entry.entryPath)}`);
          return;
        } else {
      ...
      const wgpuScript = join(repoRoot, "framework/os/renderer/wgpu/script.ts");
      const serveStatus = runCmdStatus("bun", [wgpuScript, "serve"], {
        cwd: join(repoRoot, "framework/os/renderer/wgpu"),
        env: {
          ...process.env,
...
class BuildScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    ...
    const program = variantSegment ?? process.env.SEMIO_PLUGIN ?? process.env.PLAYGROUND_APP_KIND ?? "s";
...
class VerifyScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    ...
    if (runCmdStatus("bunx", ["vitest", "run"], { cwd: join(repoRoot, "framework/os/renderer/js/react") }) !== 0) throw new Error("framework-renderer-react tests failed");
    await runStudioE2eVerify(studioUrl, timeoutMs);
