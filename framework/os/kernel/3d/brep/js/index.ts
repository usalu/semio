...
export async function ensureBrepWasmLoaded(): Promise<BrepWasmModule> {
  ...
  if (import.meta.env.VITEST) {
    ...
    const mod = (await import("../../../flow/module/brep/rs/pkg/flow_module_brep.js")) as BrepWasmModule & {
      initSync?: (input: { module: BufferSource }) => void;
    };
    mod.initSync?.({ module: readFileSync(join(here, "../../../flow/module/brep/rs/pkg/flow_module_brep_bg.wasm")) });
    brepWasm = mod;
    return mod;
  }
  const [{ default: initFlow, tessellate, dispose }, { default: wasmUrl }] = await Promise.all([import("../../../flow/core/rs/pkg/flow_core.js"), import("../../../flow/core/rs/pkg/flow_core_bg.wasm?url")]);
...
export async function ensureBrepModuleWasmLoaded(): Promise<BrepModuleWasm> {
  ...
  if (import.meta.env.VITEST) {
    ...
    const { fileURLToPath } = await import("node:url");
    const here = dirname(fileURLToPath(import.meta.url));
    const mod = (await import("../../../flow/module/brep/rs/pkg/flow_module_brep.js")) as BrepModuleWasm;
    mod.initSync?.({ module: readFileSync(join(here, "../../../flow/module/brep/rs/pkg/flow_module_brep_bg.wasm")) });
    mod.activate();
    brepModuleWasm = mod;
    return mod;
  }
  const [{ default: initBrep, evaluate, activate }, { default: wasmUrl }] = await Promise.all([import("../../../flow/module/brep/rs/pkg/flow_module_brep.js"), import("../../../flow/module/brep/rs/pkg/flow_module_brep_bg.wasm?url")]);
