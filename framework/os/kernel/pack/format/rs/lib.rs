//! 📦 `pack_format` — the `SPK` binary document container: exact byte-level header/footer/
...
//#region 🔖Header
/// @emoji 🧲 The 8-byte magic every `.spk` pack file begins with.
...
/// @emoji 🧱 Required flag bit: this file contains a chunk table.
pub const REQUIRED_CHUNKED: u32 = 1 << 1;
/// @emoji 🔒 Required flag bit: reserved for encryption, never set by this crate.
...
/// @emoji 🌊 Optional flag bit: the file was produced by a streaming writer.
pub const OPTIONAL_STREAMED: u32 = 1 << 1;
/// @emoji 🧬 Optional flag bit: a schema segment is present.
pub const OPTIONAL_HAS_SCHEMA: u32 = 1 << 2;

/// @emoji 🪪 The fixed 32-byte superblock header: magic, version, feature flags, self-CRC.
...
impl Header {
    ...
    /// @emoji 📖 Parses and validates a 32-byte header: magic, self-CRC, and that
...
//#region 🔖Footer
/// @emoji 🧲 The 8-byte magic the footer begins with.
pub const FOOTER_MAGIC: [u8; 8] = *b"SPKFOOT1";
/// @emoji 📏 Fixed wire size of the footer, in bytes, at the end of the file.
...
/// @emoji 🪶 The fixed-size trailer every pack file ends with — the single root of trust a
...
impl Footer {
    ...
    /// @emoji 📖 Parses and validates an 84-byte footer: magic and self-CRC over the first 80
...
//#region 🔖Segment
/// @emoji 📦 The fully-encoded wire bytes of one framed segment, plus the byte offsets within
...
/// @emoji 🧵 Resolves `CodecId` to this crate's codec implementations for compression.
...
/// @emoji 🧵 Resolves `CodecId` to this crate's codec implementations for decompression.
...
/// @emoji 👓 A decoded, CRC-checked, decompressed segment plus enough position bookkeeping for
...
/// @emoji 1️⃣ Bounds-checked single-byte read at an absolute file offset.
...
/// @emoji 🔢 Reads one LEB128 varint starting at an absolute file offset, one byte at a time so
...
/// @emoji 🚪 Decodes one framed segment starting at an absolute file offset: reads and
...
/// @emoji 📖 Parses a symbol table, rejecting a count over `limits.max_symbols` before
...
//#region 🔖ChunkTable
/// @emoji 🧱 One row of the chunk table: where a chunk's (possibly compressed) payload lives
...
/// @emoji 📖 Parses the chunk table, rejecting a count over `limits.max_items` or an entry
...
/// @emoji 📖 Parses the manifest segment payload. Trailing bytes beyond the known fields are
...
impl<S: PackSink> PackWriter<S> {
    /// @emoji 🚀 Writes the 32-byte header and returns a writer positioned right after it.
    ...
    /// @emoji 📍 Current absolute write position — the offset the next segment/chunk will start
    ...
    /// @emoji 🧱 Writes a `KIND_CHUNK` segment and records its offset/lengths/hashes for the
    ...
    /// @emoji 🏁 Writes the chunk table (if any chunks were written), the manifest, an `End`
...
//#region 🔖Reader
/// @emoji 🪪 The two fixed-size, always-present anchors of a pack file.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// @emoji 📂 Random-access pack file reader with three progressively-deeper open levels:
...
impl<S: PackSource> PackFile<S> {
    /// @emoji 1️⃣ Level 1: parses and CRC-validates the header and footer only, and cross-checks
    ...
    /// @emoji 2️⃣ Level 2: `open_superblock` plus decoding the manifest, its symbol table (used
    ...
    /// @emoji 🔤 Resolves a symref (index into the symbol table loaded by `open_manifest`).
    ...
    /// @emoji 📏 The `(offset, stored_len)` range of a chunk's on-disk (possibly compressed)
    ...
    /// @emoji 3️⃣ Level 3: reads, optionally CRC-verifies (`Standard`+) and decompresses one
    ...
    /// @emoji 📄 Level 3: reads and concatenates the `doc_frame_count` `KIND_DOCUMENT` segments
    ...
    /// @emoji #️⃣ The footer's content hash — no decode needed.
    pub fn content_hash(&self) -> ContentHash {
        self.superblock.footer.content_hash
    }
}

/// @emoji 🔎 Standalone helper (used by `pack::content_hash`) that reads and parses only the
...
//#region 🔖Recover
/// @emoji 🩹 What a forward-scan recovery pass managed to salvage.
#[derive(Clone, Debug)]
...
/// @emoji 🩺 Forward-scans from byte `HEADER_SIZE` (right after the header), CRC-validating and
