//! 🎲 Probability distributions and special functions: erf, gamma, beta, and pdf/cdf/quantile/sampling for the standard statistical laws.

// #region 🔖Error
/// ⚠️ Fallible-construction and fallible-inversion error type shared by every distribution in this crate.
#[derive(Debug, thiserror::Error)]
pub enum ProbabilityError {
    /// 🚫 A constructor argument (e.g. `std_dev`, `dof`, `n`) fell outside its valid range.
    #[error("invalid parameter {name} = {value}")]
    InvalidParameter { name: &'static str, value: f64 },
    /// 🚫 A call-site argument (e.g. a `quantile` probability) fell outside its valid domain.
    #[error("{name} = {value} outside domain")]
    OutOfDomain { name: &'static str, value: f64 },
    /// 🚫 An iterative numerical method (series, continued fraction, Newton/bisection) ran out of iterations.
...
// #region 🔖Special
/// 🧮 Natural log of the gamma function via the Lanczos approximation (g=7, 9-term coefficient
...
/// 🧮 Regularized lower incomplete gamma `P(a, x) = γ(a, x) / Γ(a)`: a power series for `x < a+1`,
...
/// 🧮 Regularized upper incomplete gamma `Q(a, x) = 1 - P(a, x)`, computed directly via the
...
/// 🧮 Complementary error function `1 - erf(x)`, computed via the `gamma_q` continued-fraction
...
/// 🧮 Regularized incomplete beta `I_x(a, b)` via a modified-Lentz continued fraction, with the
...
/// 🧮 Standard normal inverse CDF via Acklam's rational approximation (~1.15e-9 relative
...
// #region 🔖Traits
/// 🔔 A continuous univariate distribution over `f64` support.
...
/// 🎯 A discrete univariate distribution over `u64` support.
pub trait Discrete {
...
// #region 🔖Invert
/// 🧭 Inverts a monotone `f` at `target` by Newton's method seeded at `x0`, falling back to
...
/// 🎲 Marsaglia–Tsang (2000) gamma-variate sampler: squeeze method for `shape >= 1`, boosted by
...
// #region 🔖Normal
/// 🔔 Gaussian distribution with mean `mean` and standard deviation `std_dev`.
...
impl Continuous for Normal {
    ...
    /// 🎲 Marsaglia polar method: rejects points outside the unit disk, transforms the accepted
...
impl Continuous for ChiSquared {
    ...
    /// 📐 Newton's method seeded with the Wilson–Hilferty cube-root approximation, safeguarded
    ...
    /// 🎲 `2 * gamma_sample(dof/2)`.
    fn sample(&self, rng: &mut mathematical_random::Rng) -> f64 {
        2.0 * gamma_sample(self.dof / 2.0, rng)
    }
}
// #endregion 🔖ChiSquared
...
impl Continuous for StudentT {
    ...
    /// 🎲 `normal_sample / sqrt(chi2_sample(dof) / dof)`.
    fn sample(&self, rng: &mut mathematical_random::Rng) -> f64 {
...
// #region 🔖FisherF
/// 📐 Fisher–Snedecor F-distribution with `dof1`, `dof2` degrees of freedom.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct FisherF {
    pub dof1: f64,
    pub dof2: f64,
...
impl Continuous for FisherF {
    ...
    /// 🎲 `(chi2_sample(dof1)/dof1) / (chi2_sample(dof2)/dof2)`.
    fn sample(&self, rng: &mut mathematical_random::Rng) -> f64 {
...
// #region 🔖Binomial
/// 🎯 Binomial distribution: number of successes in `n` independent Bernoulli(`p`) trials.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct Binomial {
    pub n: u64,
    pub p: f64,
...
impl Discrete for Binomial {
    ...
    /// 🎯 Exact O(1) CDF via the regularized-beta/binomial-CDF identity
    ...
    /// 🎯 Walks the CDF from `0` upward — O(n), acceptable since `n` is small in the intended
    ...
    /// 🎯 `n` independent Bernoulli draws — O(n), no BTPE, not needed at this scale.
    fn sample(&self, rng: &mut mathematical_random::Rng) -> u64 {
...
// #region 🔖Multinomial
/// 🎲 Multinomial distribution: `n` draws over `probs.len()` categories. Multivariate, so it
...
impl Multinomial {
    ...
    /// 🎲 Sequential conditional binomial draws: category `i`'s count is `Binomial(remaining,
...
mod tests {
    ...
    // #endregion 🔖UniformTests

    // #region 🔖ChiSquaredTests
    #[test]
    ...
    // #endregion 🔖ChiSquaredTests

    // #region 🔖StudentTTests
    #[test]
    ...
    // #endregion 🔖FisherFTests

    // #region 🔖BernoulliTests
    #[test]
    ...
    // #endregion 🔖BernoulliTests

    // #region 🔖BinomialTests
    #[test]
    ...
    // #endregion 🔖BinomialTests

    // #region 🔖MultinomialTests
    #[test]
