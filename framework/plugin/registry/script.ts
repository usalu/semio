  ...
  function walk(dir: string) {
    for (const name of readdirSync(dir)) {
      ...
      if (st.isDirectory()) {
        ...
        if (isPluginCrate || isModuleCrate || isPluginBundleCrate) {
          out.push(path);
        }
      }
    }
...
export function isSpacePluginFilter(pluginFilter?: string, repoRoot = getWorkspaceRoot()): boolean {
...
export function generatePluginRegistry(repoRoot = getWorkspaceRoot(), options: GeneratePluginRegistryOptions = {}): PluginRegistryEntry[] {
  ...
  const filterIds = filterPlaygroundPlugin && !isSpacePluginFilter(filterPlaygroundPlugin) ? resolveRegistryPluginIdsForFilter(filterPlaygroundPlugin) : undefined;
...
export function buildPlaygroundSession(variant: string, repoRoot = getWorkspaceRoot()): PlaygroundSession {
  const studioMode = isSpacePluginFilter(variant, repoRoot);
...
function validatePlaygroundSessions(repoRoot: string): string[] {
  ...
  if (!isSpacePluginFilter("s", repoRoot) || isSpacePluginFilter("puzzle3d", repoRoot)) {
