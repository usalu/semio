"""🔊 Subtitle voiceover for Cooling Part 5 (Systemauslegung) — synth, align, verify.

The spoken text is the German subtitle already on screen, so audio and captions
come from one source (``scene_5.py`` NARRATION). Workflow:
...
Voice model is chosen by ``tutorial/.env`` (see repo README audio notes):
gpt-4o-mini-tts on OpenAI, German, one delivery brief for every clause.
"""
