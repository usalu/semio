"""Render, mux, and compose Scene 1 (Heizlast vs. Kühllast) into a single video."""
...
def main():
    ...
    scenes = [
        ("Beat1_WinterGains", base_dir / "beat_1_audio.mp3"),
        ("Beat2_SummerOverheat", base_dir / "beat_2_audio.mp3"),
        ("Beat3_CoolingSystem", base_dir / "beat_3_audio.mp3"),
    ]
    ...
    for idx, (scene_name, audio_path) in enumerate(scenes, start=1):
        print(f"\n{'=' * 60}")
        print(f"--- Rendering {scene_name} (Beat {idx}/3) ---")
        print(f"{'=' * 60}")
    ...
    final_output = base_dir / "Full_Scene1_Heating_Vs_Cooling.mp4"
