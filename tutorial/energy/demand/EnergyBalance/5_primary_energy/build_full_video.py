"""Render, mux, and compose Chapter 5 (Primärenergie & das GEG) into a single video."""
...
def main():
    ...
    script_path = base_dir / "scene_5.py"
    output_dir = base_dir / "rendered"
    output_dir.mkdir(exist_ok=True)

    # Each tuple: (SceneClassName, audio_file_stem)
    scenes = [
        ("Beat1_DreiBilanzgrenzen", base_dir / "beat_1_audio.mp3"),
        ("Beat2_Primaerenergiefaktor",  base_dir / "beat_2_audio.mp3"),
        ("Beat3_Rechenbeispiel",         base_dir / "beat_3_audio.mp3"),
        ("Beat4_Referenzgebaeude",             base_dir / "beat_4_audio.mp3"),
    ]

    # Also check for .wav variants (Gemini TTS returns WAV)
    ...
    for idx, (scene_name, audio_path) in enumerate(scenes, start=1):
        ...
        rendered_mp4 = output_dir / "media" / "videos" / "scene_5" / "1080p60" / f"{scene_name}.mp4"
        ...
        if muxed_path:
            ...
            print(f"  ✗ Mux failed, using raw video")
            muxed_clips.append(str(rendered_mp4))

    print(f"\n{'='*60}")
    print("--- Composing Final Chapter Video ---")
    print(f"{'='*60}")

    final_output = base_dir / "Full_Chapter5_Primary_Energy.mp4"
