"""🔊 Subtitle voiceover for EnergyBalance Chapter 3 (Bedarf vs. Leistung) — synth, align, verify.
...
from scene_3 import (  # noqa: E402
    Beat1_LeistungUndEnergie,
    Beat2_Normheizlast,
    Beat3_Jahresdauerlinie,
    Beat4_RichtigAuslegen,
)
...
SCENE_FILE = BASE_DIR / "scene_3.py"
TIMING_MANIFEST = BASE_DIR / "vo_timing.json"
TRACE_MANIFEST = BASE_DIR / "vo_trace.json"
CLAUSE_GAP = 0.25


def _beat_index(cls) -> int:
    match = re.search(r"Beat(\d+)", cls.__name__)
    if not match:
...
def _rendered_clip(beat_cls) -> Path | None:
    ...
    candidates += [
        p for p in (BASE_DIR / "rendered").rglob(f"{beat_cls.__name__}.mp4")
...
def _selected(beat_arg: int | None):
    if beat_arg is None:
        return [(_beat_index(cls), cls) for cls in BEATS]
    for cls in BEATS:
        if _beat_index(cls) == beat_arg:
            return [(beat_arg, cls)]
    raise SystemExit(f"Beat {beat_arg} not in this module")
...
def cmd_trace(beat_arg: int | None) -> int:
    ...
    for index, cls in _selected(beat_arg):
        ...
        proc = subprocess.run(
            [str(manim), "-qh", str(SCENE_FILE), cls.__name__],
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--beat", type=int, help="Only this beat number")
    mode = parser.add_mutually_exclusive_group()
    mode.add_argument("--trace", action="store_true", help="Re-render with VO_TRACE=1")
    mode.add_argument("--align", action="store_true", help="Rebuild tracks on traced clock")
