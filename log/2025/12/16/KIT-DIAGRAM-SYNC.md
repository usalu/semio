...
- **KitDiagramInner**: Added `visibleGuids` memoized computation that filters types/designs based on `expandedRows` and `filterSearch` from KitApp state
- **KitDiagramInner**: Modified node/edge building to filter based on visible GUIDs from table state
