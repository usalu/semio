#!/usr/bin/env bun
/**
 * 🧩 Runs `migrations.cypher` via cypher-shell; copies batch to `.repo/cache` for Windows `-f` paths. `NEO4J_DATABASE` defaults to `semio`; live schema export is `bun ./script.ts generate neo4j`.
 */
import { existsSync, mkdirSync, readFileSync, unlinkSync, writeFileSync } from "node:fs";
import { join } from "node:path";
import { spawnSync } from "node:child_process";
import { findRepoRoot } from "../js/src/bundle-script.ts";
...
const MIGRATIONS_CYPHER = join(MODULE_DIR, "migrations.cypher");
