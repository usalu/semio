...
- You MUST use repo mcp (or the cli `go run ./repo/cli`) for repo-specific infrastructure.
  - You MUST work inside a ticket.
  - You MUST start by gathering information about the repo with mcp tool `search` (or `go run ./repo/cli search <query>`). `query` is list of keywords that are relevant to ticket. This includes all prefiltered information about relevant projects, bundles, folders, files, sections, definitions, goals, tickets, drafts, policies, statutes.
  - You MUST reopen a ticket with `ticket_reopen` (or `go run ./repo/cli ticket reopen <ticket-id> <prompt> <client> <llm> --draft <draft-id>? --title <new-title?> --goal <new-goal-id>`) if an existing ticket is already covering the same task.
  - If no existing ticket is covering the same task then you MUST create a new ticket with mcp tool `ticket_open` (or `go run ./repo/cli ticket open <goal-id> <title> <prompt> <client> <llm> --draft <draft-id>?`). This creates a ticket folder `.repo/🎫/YY/MM/DD/TICKETSLUG`.
  ...
  - You MUST close the ticket once you are done with the mcp tool `ticket_close` (or `go run ./repo/cli ticket close <ticket-id> <summary> <files...>`) to finish the ticket along with the summary and at all the files you worked on (created, updated or removed). When a dev sends a new message to the chat most likely it is related to the old task and you MAY reopen the same ticket with mcp tool `ticket_reopen` (or `go run ./repo/cli ticket reopen <ticket-id> <prompt> <client> <llm> --draft <draft-id>? --title <new-title?> --goal <new-goal-id>?`).
  - You MUST NOT open, close or reopen goals without the explicit instructions from the dev.
  - Open a goal with mcp tool `goal_open` (or `go run ./repo/cli goal open <title> <description> <prompt> <client> <llm> --due <due-date>? --parent <parent-goal>?`).
  - Close a goal with mcp tool `goal_close` (or `go run ./repo/cli goal close <GOALSLUG/SUBGOALSLUG> <summary>`).
  - Reopen a goal with mcp tool `goal_reopen` (or `go run ./repo/cli goal reopen <GOALSLUG/SUBGOALSLUG> <prompt> <client> <llm> --title <new-title>? --description <new-description>? --due <new-due-date>? --parent <new-parent-goal>?`).
