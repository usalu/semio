...
- **🧱 Geometry (Representation)** → The 3D shape attached to the Type. It shows what the piece looks like.

- **🧲 Connection Points (Ports)** → The snap points where this piece connects to others in the system.
...
Place Ports in **consistent, logical locations** — not necessarily where pieces touch.

In Semio, Pieces can be **moved or rotated after snapping**, so Ports don’t have to match exact geometry edges.  
Instead, place them in places that make sense across all Variants — like the **center of a face** or **midpoint of a side**.

This helps you:

- Keep your Port layout clean and reusable ♻️  
- Easily swap between Variants of a Type without breaking connections 🔁  
- Focus on logic, not micromanaging alignment 🎯

---

##### 💡 Tip: How Many Ports Should You Add?

Add as many Ports as your design needs — no more, no less.

Just like LEGO bricks have **extra studs** for flexibility, you can place **multiple Ports** to offer different connection options.

- Use **fewer Ports** if you want simple, clear snapping 🔒  
- Use **more Ports** if your design benefits from flexible connections 🔓  

There’s no perfect number — choose what supports your system’s logic best.
