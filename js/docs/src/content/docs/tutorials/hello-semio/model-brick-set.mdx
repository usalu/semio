...
Although modeling happens inside a **Cluster** at the very left of your definition, we’ll focus here on the **export logic**:
...
This version includes the full profile of the piece —  
a rectangle cut at an angle and extruded into a wooden section 🪵.  
It reflects real-world geometry and is ideal for:
...
This version is just a **bounding box** — a minimal block that matches the brick’s size 📦, but skips inner detail.  
It’s perfect for:

- Early-stage layouts  
- Fast performance  
- Projects where shape is still evolving
