...
Let’s try to model our shape — a rectangle, cut at an angle.

As you can see in the sketch, there are two key design parameters:

- `W` = single unit dimension  
...
We’ll use a parametric setup to generate the shape based on how many units it contains.  
A Grasshopper definition creates the cut rectangle and then extrudes it into a wood profile.

</TabItem>

<TabItem label="File-Based Reference (🔁 Recommended)">

We can model the geometry externally and reference a specific layer at the end.
...
A high-detail Representation of the model —  
includes elements like profiles, joints, or textures.
</TabItem>

<TabItem label="⚡ Simplified Version">
A simplified Representation of the same model —  
