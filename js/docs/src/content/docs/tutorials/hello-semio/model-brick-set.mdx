...
As mentioned earlier in **[Thinking in Semio](/think-in-semio/)**. A **Type** is like a **brick mold** 🧱 A reusable shape template that defines how multiple Pieces are made

You're not just drawing geometry 🎨  
You're designing a **template** that combines shape with meaningful design information 🧠🔧
...
But in Semio, it becomes part of your **design language** 🗣️🎨 —  
a way to represent **intent**, not just surfaces 🎯
...
🔗 The geometry still exists — but now it stands in for something more:  
**a role**, **a relationship**, **a meaning** 🌐
...
Giving clear and consistent names to elements in Semio **really pays off later** ✅  
Especially when designing and making connections between many components 🔗

As your project grows and you're working with lots of **Types**, **Variants**, and **Pieces**,  
meaningful names make everything easier to **navigate** and **understand** 🧭
...
We’re modeling Pieces based on custom wood shapes with a defined profile 🪵✏️
...
But that form isn’t fixed. 

A Representation is more like a **costume** — it can be swapped at any time, as long as it fits the logic 🎨
...
You’ll notice that it’s always the same shape 🧱  
A rectangle cut at an angle ✂️ just stretched into different sizes 📏
...
In Semio, you can attach geometry to a Type in different ways — depending on what fits your workflow best. Let’s take a look at how that works in practice 👀 
...
— even when generated inside Grasshopper  

This approach helps maintain **clean, modular workflows** 🧼  
and supports flexible updates

While the modeling process itself isn’t the focus of this tutorial, the key takeaway is this:  
you’ll need to go through three simple steps to attach this geometry as a Representation

---

### 🛠️ Step 1: Geometry Modeling
...
It can be placed directly after your geometry definition in Grasshopper 🦗

The modeling of the geometry happens earlier in the Grasshopper definition 🛠️  
Here, we’ll focus mainly on the **export logic**  
how to turn that geometry into a file Semio can reference 💾
...
But this geometry isnt fixed and final: it acts as more like a ** placeholder** 🧠  
...
Just like architects might use blocky volumes for urban models 🏙️ or detailed profiles for close-ups 🪟  
