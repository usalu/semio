...
semio doesn’t change how Grasshopper works — it just changes **what** you’re building.

Instead of only passing geometry between components, you’re now passing structured data like:

- 🧱 **Types** (brick molds)  
- 🧩 **Pieces** (placed bricks)  
- 🔌 **Ports** and 🔗 **Connections** between them
...
You can plug these outputs back into the **same component** — usually the first input — to modify or rebuild them.  
Only the parameters you redefine will be changed; all other values stay untouched.  
...
#### 🔗 Adjust a Connection  
→ Plug the `Cn` output from `Model Connection` back into a new `Model Connection` component  
to change the **transformations** or **rotation adjustments**
