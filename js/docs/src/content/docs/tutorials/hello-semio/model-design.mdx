...
### 🔄 Reuse & Modify with the Same Block

In Semio, each **Model** component (like `Model Type`, `Model Piece`, or `Model Design`) outputs a special Semio parameter:

- `Ty` → a **Type** (your brick mold)  
- `Pc` → a **Piece** (a placed brick)  
- `Cn` → a **Connection** (how two bricks snap together)  
- `Ds` → a **Design** (the full assembled model)

You can plug these outputs **back into the same component** — usually into the **first input** — to reconstruct or update them without rebuilding from scratch.
...
- **Update a Piece’s position or ID**  
  → Take one `Pc` from the output of `Model Piece`, plug it back into the same component, and change the plane or name.  
  _E.g. flipping the first brick in the Hello Semio logo by changing its base plane._
  By feeding a modified Piece (with a rotated Plane) into the Design input, all connected Pieces will follow — since they’re linked by logic.

  ![Reconstruct Piece](/reconruct-piece-2.png)

  ![Reconstruct Piece](/reconruct-piece.png)

- **Modify a Type’s geometry or metadata**  
  → Take the `Ty` output and reconnect it to `Model Type` with updated geometry or new properties.

- **Adjust a snap or add a gap**  
  → Reuse the `Cn` from `Model Connection` and input it again to tweak transformation or rotation values.
...
- **Rename or reorganize a Design**  
  → Inject the existing `Ds` into `Model Design` and update its name or structure.
