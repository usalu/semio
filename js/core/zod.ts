...
const Kit = z
  .object({
    ...
    icon: z
      .string()
      .max(1024)
      .describe("🪙 The optional icon [ emoji | logogram | url ] of the kit. The url must point to a quadratic image [ png | jpg | svg ] which will be cropped by a circle. The image must be at least 256x256 pixels and smaller than 1 MB. kit.")
      .optional(),
    image: z
      .string()
      .max(1024)
      .describe("🖼️ The optional url to the image of the kit. The url must point to a quadratic image [ png | jpg | svg ] which will be cropped by a circle. The image must be at least 720x720 pixels and smaller than 5 MB.")
      .optional(),
    preview: z
      .string()
      .max(1024)
      .describe("🔮 The optional url of the preview image of the kit. The url must point to a landscape image [ png | jpg | svg ] which will be cropped by a 2x1 rectangle. The image must be at least 1920x960 pixels and smaller than 15 MB.")
      .optional(),
    version: z.string().max(64).describe("🔀 The optional version of the kit. No version means the latest version.").optional(),
    remote: z.string().max(1024).describe("☁️ The optional Unique Resource Locator (URL) where to fetch the kit remotely.").optional(),
