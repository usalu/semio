...
interface ToggleCycleProps<T extends string> extends Omit<React.ButtonHTMLAttributes<HTMLButtonElement>, "value" | "onValueChange">, VariantProps<typeof toggleVariants> {
  value?: T;
...
export function ToggleCycle<T extends string>({ className, variant = "outline", size = "default", value, onValueChange, items, tooltip, hotkey, ...props }: ToggleCycleProps<T>) {
  ...
  const buttonElement = (
    <button type="button" data-state={value ? "on" : "off"} className={cn(toggleVariants({ variant, size }), className)} onClick={handleClick} {...props}>
      {currentItem.label}
    </button>
  );
