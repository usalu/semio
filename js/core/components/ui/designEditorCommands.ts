...
import { addConnectionToDesign, addDesignToKit, addTypeToKit, Design, findDesignInKit, flattenDesign, orientDesign, removeDesignFromKit, removePieceFromDesign, removePiecesAndConnectionsFromDesign, removeTypeFromKit, setConnectionInDesign, setDesignInKit, setPieceInDesign, setTypeInKit } from "../../semio"
...
export const designEditorCommands: Command[] = [
    ...
    // Type CRUD
    {
        ...
        parameters: [
            { name: 'name', type: 'string', description: 'Name of the type', required: true },
            { name: 'variant', type: 'string', description: 'Variant of the type' },
            { name: 'description', type: 'string', description: 'Description of the type' }
        ],
        execute: async (context, payload) => {
            const newType = {
                name: payload.name,
...

    {
        ...
        parameters: [
            { name: 'name', type: 'string', description: 'Name of the type', required: true },
            { name: 'variant', type: 'string', description: 'Variant of the type' },
            { name: 'description', type: 'string', description: 'New description of the type' }
        ],
        execute: async (context, payload) => {
            const existingType = context.kit.types?.find(t =>
                t.name === payload.name && (t.variant || '') === (payload.variant || ''))

            if (!existingType) {
                return { content: `❌ Type not found: ${payload.name}${payload.variant ? ` (${payload.variant})` : ''}` }
            }
...

    {
        ...
        execute: async (context, payload) => {
            const typeId = { name: payload.name, variant: payload.variant || undefined }
            return {
    ...
    // Design CRUD
    {
        ...
        parameters: [
            { name: 'name', type: 'string', description: 'Name of the design', required: true },
            { name: 'variant', type: 'string', description: 'Variant of the design' },
            { name: 'view', type: 'string', description: 'View of the design' },
            { name: 'description', type: 'string', description: 'Description of the design' }
        ],
        execute: async (context, payload) => {
            const newDesign = {
                name: payload.name,
...

    {
        ...
        parameters: [
            ...
            { name: 'variant', type: 'string', description: 'Variant of the design' },
            { name: 'view', type: 'string', description: 'View of the design' },
            { name: 'description', type: 'string', description: 'New description of the design' }
        ],
        execute: async (context, payload) => {
            const existingDesign = context.kit.designs?.find(d =>
                d.name === payload.name &&
                (d.variant || '') === (payload.variant || '') &&
                (d.view || '') === (payload.view || ''))

            if (!existingDesign) {
                return { content: `❌ Design not found: ${payload.name}${payload.variant ? ` (${payload.variant})` : ''}${payload.view ? ` [${payload.view}]` : ''}` }
            }
    ...
    // Piece CRUD (add already exists)
    {
        ...
        parameters: [
            { name: 'id', type: 'string', description: 'ID of the piece to update', required: true },
...

    {
        ...
        parameters: [
            { name: 'id', type: 'string', description: 'ID of the piece to remove', required: true }
        ],
        execute: async (context, payload) => {
            const { kit, designId } = context
    ...
    // Connection CRUD (add already exists)  
    {
        ...
        parameters: [
            { name: 'connectingPiece', type: 'string', description: 'Connecting piece ID', required: true },
            { name: 'connectedPiece', type: 'string', description: 'Connected piece ID', required: true },
        ...
        execute: async (context, payload) => {
            ...
            const connectionToUpdate = design.connections?.find(c =>
                c.connecting.piece.id_ === payload.connectingPiece &&
                c.connected.piece.id_ === payload.connectedPiece
            )

            if (!connectionToUpdate) {
                return { content: `❌ Connection not found between ${payload.connectingPiece} and ${payload.connectedPiece}` }
            }
    ...
    // Selection Commands (new ones)
    {
        ...
        parameters: [
            { name: 'pieceIds', type: 'string', description: 'Comma-separated piece IDs', required: true }
        ],
        editorOnly: true,
        execute: async (context, payload) => {
...

    {
        ...
        parameters: [
            { name: 'connections', type: 'string', description: 'Comma-separated connections (format: "piece1->piece2")', required: true }
        ],
        editorOnly: true,
        execute: async (context, payload) => {
...
${typesList}`
...

    {
        ...
        execute: async (context) => {
            ...
            const piecesToDelete = selection.selectedPieceIds.map((id: string) => ({ id_: id }))
...

    {
        ...
        execute: async (context) => {
            ...
            try {
                ...
                return {
                    design: newDesign,
                    selection: { selectedPieceIds: newPieceIds, selectedConnections: [], selectedPiecePortId: undefined },
                    content: `✅ Pasted ${data.pieces.length} pieces`
                }
            } catch (error) {
                return { content: `❌ Failed to paste from clipboard: Invalid data format` }
            }
        }
    },
