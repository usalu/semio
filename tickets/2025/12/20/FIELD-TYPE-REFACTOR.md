---
slug: FIELD-TYPE-REFACTOR
prompt: Introduce Field<T> type and useDesignAppField helper to reduce hook boilerplate
summary: Introduce Field<T> type and useDesignAppField helper to reduce hook boilerplate (Smell 1/2)
status: finished
author: Ueli Saluz <ueli.saluz@iek.uni-hannover.de>
date:
  created: 2025-12-20T13:03:02.551Z
  finished: 2025-12-20T13:38:37.874Z
commit: 4ff6fd77dee713af972c27bd3761939be4302c80
model: claude-opus-4-5
iterations:
  - prompt: Introduce Field<T> type and useDesignAppField helper to reduce hook boilerplate
    model: claude-opus-4-5
    date:
      started: 2025-12-20T13:03:20.592Z
      ended: 2025-12-20T13:03:26.092Z
    author: Ueli Saluz <ueli.saluz@iek.uni-hannover.de>
    commit: 4ff6fd77dee713af972c27bd3761939be4302c80
    bundles:
      "@semio":
        files:
          "":
            sections:
              "": {}
---


# Previously

# Plan

# Changes
