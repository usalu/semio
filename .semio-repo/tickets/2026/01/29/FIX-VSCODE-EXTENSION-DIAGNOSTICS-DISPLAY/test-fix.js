...
const testOutput = '{"kind":"result","result":{"data":{"violations":[{"id":"v1","summary":"Test violation"}]}}}';
const events = parseRepoEvents(testOutput);
const result = extractRepoResult(events);

assert.ok(result.data, "Should have data field");
assert.ok(result.data.violations, "Should have violations");
assert.strictEqual(result.data.violations.length, 1, "Should have 1 violation");
assert.strictEqual(result.data.violations[0].id, "v1", "Should have correct violation id");
