...
- [x] Update go/repo/main.go goal open command signature and extraction logic.
...
- Updated `go/repo/main.go` `goalCommand` to swap `extractUIFromArgs` and `extractLLMFromArgs` call order and make LLM optional.
- Rebuilt `repo` binary.
