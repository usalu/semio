...
## Log
# Log: Standardize Semio Tests Across All Implementations
...
#### Go Fixes
- Removed dead `loadLlms` code from `go/semio/kit_sqlite.go` (lines 87-89 and entire function at 248-267)
- Go now compiles successfully
- Go tests: 10/10 pass

#### TypeScript Fixes
- Regenerated `assets/semio/validation.json` from TypeScript to use `"Interface"` instead of `"Port"` for entityKind
...
Fixed multiple duplicate function declarations in `js/semio/semio.ts`:
...
Also updated:
- `semioPortNameUniquenessConstraint` (line 7464) renamed to `semioConnectorNameUniquenessConstraint`
...
**TypeScript tests: 10/10 ✅**

#### C# Compile Error Fix
Fixed parameter naming issue in `net/Semio/Semio.cs`:
...
|---------------|--------|-----------------|--------|
| TypeScript    | 10/10  | 0               | ✅ Complete |
| Python        | 10/10  | 0               | ✅ Complete |
| Go            | 10/10  | 0               | ✅ Complete |
