...
- [ ] Analyze codebase to locate relevant logic in `go/repo` and `js/vscode`.
- [ ] Refactor Ticket Title Validation in `go/repo`.
- [ ] Update Author Resolution logic in `go/repo` (contributors file).
- [ ] Refactor Plan Handling in `go/repo` (move original plan, store in ticket.json, no plan_ITERATION.md).
- [ ] Update Ignore Logic in `go/repo`:
...
- [ ] Refactor Hierarchy/IDs in `go/repo`:
