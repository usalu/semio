#!/usr/bin/env python3
"""Script to fix all policy violations in semio/py/semio.py."""
import re
import sys
...
if orphan_idx is not None and region_imports_idx is not None and orphan_idx < region_imports_idx:
    ...
    if m:
        ...
        if section_name in section_summaries:
            ...
            if next_idx < len(lines):
                ...
                is_summary = next_stripped.startswith('#') and not next_stripped.startswith('# region') and not next_stripped.startswith('# #region') and not next_stripped.startswith('# endregion') and not next_stripped.startswith('# #endregion')
                if not is_summary:
...
# Parse violations from files to know exact names and current line numbers
...
def make_summary(name, kind, parent_classes="", body_hint=""):
    ...
    # Exception classes
    if "Error" in name or "NotFound" in name or "NotYetSupported" in name or "NotAssigned" in name or "AlreadyExists" in name or "Unreachable" in name:
        if name == "Error":
        ...
        if name == "SpecificationError":
            return "# Base exception for specification constraint violations."
        ...
        if name == "TypeHasNotAllUsedConnectors":
            return "# Exception for a type missing connectors referenced by connections."
    ...
    if name.endswith("Field"):
        ...
        for prefix in ["Attribute", "Tag", "Concept", "Coord", "Point", "Vector",
                       "Plane", "Location", "Author", "ArtifactAuthor", "File", "Folder",
                       "Benchmark", "Quality", "Prop", "Model", "Port", "CompatiblePort",
                       "Connector", "Type", "Layer", "Piece", "Group", "Side",
                       "Connection", "Stat", "Design", "Kit"]:
    ...
    if name.endswith("IdInputNode"):
        entity = name[:-11]
        return f"# GraphQL input node for {camel_to_words(entity).lower()} identification."

    # Node classes (but not InputNode/IdInputNode)
    if name.endswith("Node") and not name.endswith("InputNode") and not name.endswith("IdInputNode"):
        entity = name[:-4]
...
def make_spec(name, kind):
    ...
    # Entity classes
    entity_names = ["Attribute", "Tag", "Concept", "Location", "Author", "ArtifactAuthor",
                    "File", "Folder", "Benchmark", "Quality", "Prop", "Model", "Port",
                    "CompatiblePort", "Connector", "Type", "Layer", "Piece", "Group",
                    "Connection", "Stat", "Design", "Kit", "Semio"]
    if name in entity_names:
        return f"# {name} MUST implement idMembers and inherit from the appropriate field mixins."

    # Abstract bases
    abstract_names = {"SModel", "Field", "RealField", "MaskedField", "Base", "Id", "Props",
                      "Input", "Context", "Output", "Prediction", "Entity", "Table", "TableEntity",
                      "Node", "InputNode", "RelayNode", "TableNode", "TableEntityNode",
                      "Error", "ServerError", "ClientError", "NotFound", "SpecificationError",
                      "NoParentAssigned", "AlreadyExists", "KitUriNotValid", "LocalKitUriNotValid"}
...
# We need to find all class/def/async def lines that are in the violation list
...
for def_line_idx, name, kind, insert_idx in reversed(definitions):
    ...
    while above_idx >= 0:
        ...
        if stripped.startswith('#') and not stripped.startswith('# region') and not stripped.startswith('# #region') and not stripped.startswith('# endregion') and not stripped.startswith('# #endregion'):
