#!/usr/bin/env python3
"""Fix all policy violations in semio/js/semio.ts.
...
export_pattern = re.compile(r'^export\s+(?:(?:async|abstract|declare|default)\s+)*(?:const|let|var|function|class|interface|type|enum)\s+(\w+)')
...
def gen_summary(name, lt):
    ...
    if name.endswith('Schema') and ('z.' in lt or 'z.object' in lt or 'z.enum' in lt or 'z.string' in lt or 'z.array' in lt):
        base = name[:-6]
...
def gen_spec(name, lt):
    ...
    if name.startswith('can') or (name.startswith('are') and not name.startswith('areSame')):
        return "MUST return a boolean result."
    if 'Constraint' in name:
        return "MUST detect and report constraint violations."
    if is_cls:
...
while i < len(lines):
    ...
    if stripped.startswith('import ') and i > 0 and lines[i-1].strip().startswith('// #endregion'):
        result.append('')
...
for i in range(len(lines2)):
    ...
    if sm:
        ...
        if not has_summary and section_name not in ('Header', 'License', 'Specs'):
            summary_text = section_summary_map.get(section_name,
                f'{section_name} definitions MUST be defined here.')
    ...
    if em:
        ...
        while j >= 0:
            ...
            if prev == '':
                j -= 1
                continue
            if prev.startswith('//') and not prev.startswith('// #region') and not prev.startswith('// #endregion'):
