...
section_summaries = {
    ...
    'Types': 'Scope, todo, violation, and ticket metric types for the repository model.',
    ...
    'Codebase Types': 'Internal metric, contributor, ticket, policy, violation, and tree node types for codebase analysis.',
    ...
    'Codebase': 'Codebase builder that assembles bundles, folders, files, sections, definitions, contributors, tickets, policies, and violations.',
...
def gen_summary(name, line, section_name):
    ...
    if type_alias_re.match(line) and not type_struct_re.match(line) and not type_interface_re.match(line) and not type_func_re.match(line):
...
def gen_func_summary(name, recv, line, section_name):
    ...
    if name.startswith('Check'):
        what = camel_to_words(name[5:])
        return f'{name} validates the {what} and returns any violations.'
    ...
    if name.startswith('Match') and name[5:6].isupper():
        what = camel_to_words(name[7:]) if name.startswith('Matches') else camel_to_words(name[5:])
    ...
    if name == 'Info':
        return f'Info returns the metadata for the violation kind.'
    if name == 'AllKinds':
        return f'AllKinds returns all violation kinds associated with the group.'
    if name == 'Priority':
        return f'Priority returns the priority of the violation from its kind metadata.'
    if name == 'Autofixable':
        return f'Autofixable reports whether the violation kind supports automatic fixing.'
...
def gen_spec(name, line, section_name):
    ...
    if name.startswith('Check'):
        return f'{name} MUST run all applicable policies and aggregate violations.'
    if name.startswith('Derive'):
    ...
    if name.startswith('Ensure'):
        return f'{name} MUST be idempotent and MUST NOT fail if the target already exists.'
    if name.startswith('Walk'):
    ...
    if name == 'Render':
        return 'Render MUST consume the full event stream and return the final exit code.'
    if name == 'Info':
        return 'Info MUST return the metadata entry for the violation kind.'
    if name == 'AllKinds':
        return 'AllKinds MUST include all violation kinds from the group and its children.'
    if name == 'Priority':
        return 'Priority MUST derive the value from the violation kind metadata.'
    if name == 'Autofixable':
        return 'Autofixable MUST return true only for violation kinds that support auto-fix.'
    if name == 'ToStreamOptions':
    ...
    if recv:
        return f'{name} MUST operate on the {recv} receiver and return consistent results.'
...
def has_summary_comment(lines, line_idx):
    ...
    if idx >= 0:
        ...
        if stripped.startswith('//') and not stripped.startswith('// #region') and not stripped.startswith('// #endregion'):
            return True
    return False
...
def is_comment_line(line):
    s = line.strip()
    return s.startswith('//') and not s.startswith('// #region') and not s.startswith('// #endregion')
    ...
    if m:
        ...
        if section_name in section_summaries:
            ...
            if next_idx < len(lines):
                ...
                if not (next_line.startswith('//') and not next_line.startswith('// #region') and not next_line.startswith('// #endregion')):
                    insertions[next_idx] = insertions.get(next_idx, [])
                    insertions[next_idx].insert(0, ('section_summary', f'// {section_summaries[section_name]}\n'))
