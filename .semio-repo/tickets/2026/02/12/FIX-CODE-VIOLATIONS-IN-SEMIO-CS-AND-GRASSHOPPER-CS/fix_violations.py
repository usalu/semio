...
SECTION_SUMMARIES = {
    "Constants": "Shared constants, enums, and configuration values used across semio.",
    ...
    "SemioValidation": "Validation result types and kit-level validation logic.",
    ...
    "Folder": "Folder reference with name and optional parent hierarchy.",
    ...
    "Piece": "Instantiated type placed within a design hierarchy.",
    ...
    "Api": "REST API client for communicating with the semio engine.",
    ...
    "SemioDiff": "Diff computation, application, and comparison for kits.",
    "Imports": "External namespace imports required by the library.",
    "Namespace": "Root namespace declaration for the Semio library.",
    "Converters": "Grasshopper data type converters between semio and GH types.",
...
DEF_SUMMARIES = {
    ...
    "Goo": "Generic Grasshopper data wrapper for semio entity types.",
    "Param": "Generic Grasshopper parameter for semio entity types.",
...
def process_file(filepath):
    ...
    for i, line in enumerate(lines):
        ...
        m = re.match(r'\s*#region 🔖(.+)', stripped)
        if m:
...
def process_orphans(filepath):
    ...
    if filepath.endswith('Semio.cs'):
        ...
        content = content.replace(
            '\nnamespace Semio;\n\n#region 🔖Constants',
            '\n#endregion 🔖Imports\n\n#region 🔖Namespace\n// Root namespace declaration for the Semio library.\nnamespace Semio;\n#endregion 🔖Namespace\n\n#region 🔖Constants',
            1
        )
...
def process_definitions(filepath):
    ...
    # Get violations from analyze
    import subprocess, json
    result = subprocess.run(
        ['./semio-repo/cli/cli', 'analyze', filepath, '--json'],
        capture_output=True, text=True
    )
    data = json.loads(result.stdout)
    violations = data['analyze']['violations']
    ...
    for v in violations:
        if v['kind']['id'] == '🚫Code#Definition#Missing Summary':
            missing_summary[v['line']] = v['excerpt'] if v.get('excerpt') else v['summary'].split('"')[1]
        if v['kind']['id'] == '🚫Code#Definition#Missing Specs':
            missing_specs[v['line']] = v['excerpt'] if v.get('excerpt') else v['summary'].split('"')[1]

    if not missing_summary and not missing_specs:
        print(f"No definition violations found in {filepath}")
        return

    insertions = []
    for lineno in sorted(set(list(missing_summary.keys()) + list(missing_specs.keys()))):
        idx = lineno - 1
        if idx >= len(lines):
            continue
        line = lines[idx]
        ...
        m = re.search(r'(?:public|internal)\s+(?:abstract\s+|static\s+|sealed\s+)*(?:class|struct|interface|enum|record)\s+(\w+)', line)
        if m:
            name = m.group(1)
        else:
        ...
        if lineno in missing_specs and is_class and not is_interface and not is_enum:
            spec = DEF_SPECS.get(name, f"Implementations MUST conform to the {name} contract.")
