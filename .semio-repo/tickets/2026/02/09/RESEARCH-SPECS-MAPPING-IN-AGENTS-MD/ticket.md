...
Mapped all 33 Business Logic + 8 UI/UX spec subsections in AGENTS.md to their owning bundles/files with implementation-specificity analysis. Domain model specs (Design through Stat, 16 sections) are clean. Repo CLI specs (Code Hygiene, Tree, Tooling, Repo Tooling, Artifact Kind Derivation) are heavily implementation-specific. Detailed line ranges, primary files, and cleaning needs documented in ticket.
## Changes
...
- **Implementation-specific language:** YES — heavily references: `YYYY/MM/DD/SLUG` path identifiers, `--all` flag, GraphQL field names (`TicketClient`, `TicketDate`, `Interaction`, `Position`), `semio-repo/cli/cli.go` file path, `.semio-repo/reports/codebase.json`, `.semio-repo/` storage, `--md` flag, `--text` flag, JSONL event stream, usalu project 2. Mix of behavioral specs and implementation details.
...
| **semio/go/**, **semio/net/**, **semio/py/** | (consumers of domain model specs) | NONE |

## Implementation-Specificity Classification

### CLEAN (implementation-agnostic, pure domain specs)
