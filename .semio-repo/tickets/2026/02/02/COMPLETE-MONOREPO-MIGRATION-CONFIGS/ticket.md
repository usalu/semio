...
- [ ] Fix `.gitignore` — old Go binary paths (`@semio-repo/go/repo`, `@semio-repo/go/main`, `@semio-repo/server/server`, `@semio-repo/server/main`) need verification; old `js/**/public` and `js/semio/lib` exception paths are stale; `!rb/lib` references old `rb/` location
...
- [ ] Fix `.github/dependabot.yml` — ALL directories use old paths: `js/semio`, `js/desktop`, `js/docs`, `py/semio`, `py/engine`, `net/Semio`, `net/Semio.Grasshopper`, `net/Semio.Tests`, `net/Semio.Grasshopper.Tests`, `go/cli`, `go/mcp`, `go/repo`, `go/semio`, `rs/semio`
