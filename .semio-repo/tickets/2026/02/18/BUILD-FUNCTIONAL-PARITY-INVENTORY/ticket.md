...
- `.semio-repo/tickets/2026/02/18/BUILD-FUNCTIONAL-PARITY-INVENTORY/ticket.md` — created inventory document
...
- Analyzed semio-repo CLI preflight orchestration logic in main.go
...
### 1.1 Root Orchestrator: `semio-repo/cli` (Go binary)

The root `package.json` scripts delegate to the semio-repo CLI binary (`./semio-repo/cli/cli`), which then dispatches to `npx nx run-many -t <target>`:

| Root Script | CLI Command | Pipeline |
|---|---|---|
| `dev` | `npx nx run-many -t dev` | Direct Nx |
| `analyze` | `semio-repo/cli/cli analyze` | CLI analyze |
| `fix` | `semio-repo/cli/cli fix` | CLI fix |
| `preflight` | `semio-repo/cli/cli preflight` | fix → analyze |
| `test` | `semio-repo/cli/cli preflight test` | fix → analyze → `nx test` |
| `build` | `semio-repo/cli/cli preflight build` | `nx test` → `nx build` |
| `update` | `semio-repo/cli/cli update` | npm+python+rust+go+dotnet parallel |
| `publish:test` | `semio-repo/cli/cli preflight publish:test` | `nx build` → `nx publish:test` |
| `publish` | `semio-repo/cli/cli preflight publish` | `nx build` → `nx publish` |
| `pre-commit` | `npm run preflight` | fix → analyze |
| `benchmark` | `semio-repo/cli/cli benchmark` | CLI benchmark |
...
### 1.3 Preflight Pipeline (in semio-repo CLI)
...
#### `semio-repo` (semio-repo/vscode) — VS Code Extension
| Script | Command | Behavior |
|---|---|---|
...
| `package` | `vsce package --no-dependencies --out semio-repo.vsix` | Package VSIX |
...
- Go 1.24.0
- Members: `semio-repo/cli`, `semio-repo/server`, `semio/go`
...
**Module:** `github.com/usalu/semio/semio-repo/cli`
**Dep:** `github.com/usalu/semio/semio-repo/go` (internal)

#### `@semio-repo/server` (semio-repo/server)
| Script | Command | Behavior |
|---|---|---|
...
| `semio-repo/sqlite` | `@semio-repo/sqlite` | schema |
...
@semio/js ← semio-repo (vscode ext, devDep)
```
...
semio-repo/go → semio-repo/cli (Go module require)
```

### 4.3 Build-to-Build Dependencies (Nx `^build`)

Since `build.dependsOn = ["^build"]`, build order is topologically sorted:
1. `@semio/logo` → 2. `@semio/icons` → 3. `@semio/assets` → 4. `@semio/js` → 5. `@semio/docs`, `@semio/play`, `@semio/sketchpad`, `@semio/desktop`, `semio-repo`

Other chains build independently in parallel:
- `@semio/py` → `@semio/engine`
- `@semio/go`, `@semio-repo/cli`, `@semio-repo/server` (Go)
- `@semio/rs` (Rust)
- `@semio/net` (C#/.NET)
...
### 5.4 Go tests
- `go test -v ./...` in `semio/go`, `semio-repo/cli`, `semio-repo/server`
...
| `semio-repo/cli dev` | `go run` | No | — |
| `semio-repo/server dev` | `go run main.go` | No | — |
| `semio-repo/vscode dev` | `vite build --watch` | Yes | — |
...
| `semio-repo` (vscode) | `tsc --noEmit` | TypeScript type-check |
| `@semio/net` | `dotnet build` | .NET build |
| `@semio/go` | `go vet ./...` | Go vet |
| `@semio-repo/cli` | `go vet ./...` | Go vet |
| `@semio-repo/server` | `go vet ./...` | Go vet |
| `@semio/rs` | `cargo fmt --check && cargo clippy` | Rust format check + lint |
| `@coda/engine` | `ruff format . && ruff check --fix .` | Python format + lint |

## 8. Update Mechanism

The `semio-repo/cli update` command manages dependency updates across all ecosystems in parallel:
...
15. **Hierarchical VS Code task naming** follows pattern `project/bundle target` (e.g., `semio/js build`, `semio-repo/cli test`)
