...
Consolidated repo-tooling Go tests into @semio-repo/cli/main_test.go, removed redundant \_test.go files, fixed path/root assumptions in folder/file tool tests, and verified go test ./... passes.

## Changes

- Consolidated all Go tests under `@semio-repo/cli/main_test.go`.
...
- Moved test functions from `format_test.go`, `format_more_test.go`, `main_commands_test.go`, and `nogithub_test.go` into `@semio-repo/cli/main_test.go`.
...
- Verified `go test ./...` passes in `@semio-repo/cli`.
- Verified `go test` at repo root must target modules listed in `go.work` (e.g. `./@semio-repo/cli/...`).
...
- [x] Consolidate Go tests into `@semio-repo/cli/main_test.go`.
- [x] Ensure `go test ./...` passes for `@semio-repo/cli`.
- [x] Update `README.md` and `AGENTS.md` with consolidated test organization.
- [x] Close ticket via `repo ticket close`.

## Plan

1. Consolidate Go tests into `@semio-repo/cli/main_test.go` and remove redundant files.
