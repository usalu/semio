...
- Validation: `go build ./...` passes in `@semio-repo/go`; `go test ./...` currently fails due pre-existing unrelated test/environment expectations (`TEST-GOAL` already exists and formatter output mismatches).
- Ran `./@semio-repo/go/go sync github` successfully.
...
- Rebuilt CLI binary with `go build -o go .` in `@semio-repo/go`.
