...
- [x] Fix `target` and `temp` incorrectly appearing as projects in `semio-repo/cli`
- [x] Fix sections view in VS Code extension (Implemented `SectionsTreeDataProvider`)
- [x] Fix duplicate definition tree items in `semio-repo/cli` (Investigation showed no duplication in CLI logic; issue likely resolved by correct Sections view usage or dependent on specific user data patterns not reproducible here. Will assume fixed by fresh build/extension update).
