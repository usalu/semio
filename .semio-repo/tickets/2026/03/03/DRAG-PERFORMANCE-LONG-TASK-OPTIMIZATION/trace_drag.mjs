...
if (fc) await fc.setFiles(zipPath); else { await fileInput.setInputFiles(zipPath); await fileInput.evaluate(el => el.dispatchEvent(new Event("change", { bubbles: true }))); }
...
if (await tableRow.isVisible().catch(() => false)) await tableRow.dblclick({ force: true }); else await page.getByText("Metabolism").first().dblclick({ force: true });
await page.waitForURL(/.*kits\/.+/, { timeout: 30000 });
await page.waitForTimeout(5000);
const dids = await page.evaluate(() => Array.from(document.querySelectorAll("[data-row-id]")).map(el => el.getAttribute("data-row-id")).filter(id => id?.startsWith("design-")).slice(0, 5));
if (dids.length > 0) {
  const rid = dids.find(id => id?.includes("9a890dd4")) ?? dids[dids.length - 1];
  await page.evaluate(r => { document.querySelector(`[data-row-id="${r}"]`)?.dispatchEvent(new MouseEvent("dblclick", { bubbles: true, cancelable: true, view: window })); }, rid);
}
...
if (await toggle.isVisible().catch(() => false)) {
  if (await page.locator('[data-panel="leftSidePanel"]').isVisible().catch(() => false)) { await toggle.click(); await page.waitForTimeout(500); }
}
// Zoom
...
await page.mouse.move(sx, sy); await page.waitForTimeout(100);
...
const tracePath = "/workspaces/semio/.semio-repo/tickets/2026/03/03/DRAG-PERFORMANCE-LONG-TASK-OPTIMIZATION/trace.json";
