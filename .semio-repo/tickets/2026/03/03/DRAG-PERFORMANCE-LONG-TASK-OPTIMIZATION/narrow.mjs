...
(async () => {
  ...
  await page.evaluate(() => { window.__PHASES__.push({ name: 'mousedown', mutations: 0, added: 0, removed: 0, attrs: 0 }); window.__MUT_OBS__.observe(document.body, { childList: true, attributes: true, subtree: true }); });
  await page.mouse.move(sx, sy); await sleep(50);
  await page.mouse.down(); await sleep(200);
  await page.evaluate(() => { window.__MUT_OBS__.disconnect(); });
  
  // Phase 2: mouse.move (drag)
  await page.evaluate(() => { window.__PHASES__.push({ name: 'mousemove', mutations: 0, added: 0, removed: 0, attrs: 0 }); window.__MUT_OBS__.observe(document.body, { childList: true, attributes: true, subtree: true }); });
  await page.mouse.move(sx + 100, sy, { steps: 1 }); await sleep(200);
  await page.evaluate(() => { window.__MUT_OBS__.disconnect(); });
  
  // Phase 3: mouse.up (drag end)  
  await page.evaluate(() => { window.__PHASES__.push({ name: 'mouseup', mutations: 0, added: 0, removed: 0, attrs: 0 }); window.__MUT_OBS__.observe(document.body, { childList: true, attributes: true, subtree: true }); });
  await page.mouse.up(); await sleep(200);
  await page.evaluate(() => { window.__MUT_OBS__.disconnect(); });

  // Phase 4: settle (5 seconds)
  await page.evaluate(() => { window.__PHASES__.push({ name: 'settle', mutations: 0, added: 0, removed: 0, attrs: 0 }); window.__MUT_OBS__.observe(document.body, { childList: true, attributes: true, subtree: true }); });
  await sleep(5000);
  await page.evaluate(() => { window.__MUT_OBS__.disconnect(); });
