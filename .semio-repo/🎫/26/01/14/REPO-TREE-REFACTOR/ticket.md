...
- Policy: `"policy:" + name`
- Statute: `"statute:" + kind` (not linked to policy!)
- Breach: timestamp-based (not deterministic!)
...
- `StatuteMeta.PolicyID` to link breachs to policies
...
- `buildBreachID(scope, line, col)` - breach IDs
...
- `CreateBreach()` - uses `buildBreachID()`
...
5. **Set PolicyID** on all StatuteMeta instances in GetPolicies() and GetStatutes()
