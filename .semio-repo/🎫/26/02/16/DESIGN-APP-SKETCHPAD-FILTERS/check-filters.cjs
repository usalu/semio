...
(async () => {
  ...
  const filterToggle = await page.locator('[id="semio.sketchpad.toolbar.group.filter"]').count();
  ...
  const filterToggle2 = await page.locator('[id="semio.sketchpad.toolbar.group.filter"]').count();
  console.log("[DEBUG] Filter toggle present on design:", filterToggle2);
  
  // Take a screenshot to see what we have
  await page.screenshot({ path: "/workspaces/semio/.semio-repo/🎫/26/02/16/DESIGN-APP-SKETCHPAD-FILTERS/state.png" });
  
  // Check what toolbar buttons exist
  const toolbarButtons = await page.locator('[id^="semio.sketchpad.toolbar.group."]').allInnerTexts();
  console.log("[DEBUG] Toolbar buttons:", toolbarButtons);
  const toolbarIds = await page.locator('[id^="semio.sketchpad.toolbar.group."]').evaluateAll(els => els.map(e => e.id));
