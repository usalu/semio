#!/usr/bin/env python3
"""Migrate all .semio-repo JSON files to use the semio repo ID system for resource references.
...
def goal_path_to_semio_id(path: str) -> str:
    """Convert a goal filesystem path to a semio repo ID.
...
def contributor_to_semio_id(github_handle: str) -> str:
    """Convert a contributor reference to a semio repo ID.
...
def is_already_semio_id(value: str, emoji: str) -> bool:
    """Check if a value is already in semio repo ID format."""
...
def convert_goal_ref(value: str) -> str:
    """Convert a goal reference to semio repo ID format."""
    if not value:
        return value
...
def convert_contributor_ref(value: str, lookup: dict) -> str:
    """Convert a contributor reference to semio repo ID format."""
    if not value or value in ("unknown", "", "Migration Script", "GitHub Copilot"):
        return value
...
def main():
    ...
    semio_repo_dir = "/workspaces/semio/.semio-repo"
    ...
    ticket_files = glob.glob(os.path.join(semio_repo_dir, "🎫", "**", "ticket.json"), recursive=True)
    ...
    goal_files = glob.glob(os.path.join(semio_repo_dir, "🎯", "**", "goal.json"), recursive=True)
