...
- `semio-repo/vscode/extension.ts`: Fixed `analyzeFile()` to unwrap the `analyze` envelope from CLI JSON output (`result.data.analyze.breachs` instead of `result.data.breachs`). Also cleans up `fileBreachsMap` when breachs are empty.
...
- Ran `./semio-repo/cli/cli --json analyze semio` to see actual CLI output format: `{"analyze":{"metrics":{...},"breachs":[...]}}`
