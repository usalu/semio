...
- `semio-repo/cli/main.go`: Added `InteractionResource` struct, `ListInteractions`/`StreamInteractions` functions, `interactionCommand` (list/tree), `Interactions` resolver, `interactionResourceType` GraphQL type, `interactions` query field, `interactions` field on Goal type
- `semio-repo/graphql/schema.graphql`: Added `InteractionResource` type, `interactions` query, `interactions` field on Goal
...
1. ~~Add `InteractionResource` struct to main.go~~ ✅
2. ~~Add `ListInteractions`/`StreamInteractions` functions~~ ✅
