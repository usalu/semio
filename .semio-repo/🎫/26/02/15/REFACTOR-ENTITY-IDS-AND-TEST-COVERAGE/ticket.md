...
## Changes
- Updated project URI generation in `semio-repo/cli/main.go`:
...
- Updated URI expectations in `semio-repo/cli/main_test.go` for:
...
## Log
- Checked repository metadata and tree output with `./semio-repo/cli/cli tree`.
- Located all project URI builders and assertions via `git grep`.
- Applied source and test updates.
- Verified tests:
- `go test ./semio-repo/cli -run 'TestArtifactIDAndURI|TestIdToUri|TestUriToId|TestIdUriRoundTrip'`
- Verified runtime behavior from updated source:
- `go run ./semio-repo/cli tree semio --only-project --json | grep -o 'semiorepo://project[^\" ]*' | head -n 10`
