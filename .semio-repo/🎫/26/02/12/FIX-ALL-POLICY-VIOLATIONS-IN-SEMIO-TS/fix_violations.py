#!/usr/bin/env python3
"""Fix all policy breachs in semio/js/semio.ts.
...
section_start_pattern = re.compile(r"^\s*//\s*#region\s+\u0001?\u0002?🔖(.+?)\s*$")
section_end_pattern = re.compile(r"^\s*//\s*#endregion\s+\u0001?\u0002?🔖(.+?)\s*$")
...
def gen_summary(name, lt):
    ...
    if "Constraint" in name and name.startswith("semio"):
        base = name.replace("semio", "").replace("Constraint", "")
...
def gen_spec(name, lt):
    ...
    special = {
        ...
        "vectorToThree": "MUST convert semio vector to Three.js vector.",
        "semioMakeFix": "MUST produce a Fix that regenerates the GUID.",
...
while i < len(lines):
    ...
    if stripped == "// #endregion 🔖Constants":
        result.append(line)
        i += 1
        result.append("")
        result.append("// #region 🔖Utilities")
    ...
    if stripped == "// #region 🔖Entity IDs":
        result.append("// #endregion 🔖Utilities")
        result.append("")
        result.append(line)
        i += 1
        continue
...
for i in range(len(lines2)):
    ...
    sm = re.match(r"^\s*//\s*#region\s+🔖(.+?)\s*$", stripped)
    if sm:
