#!/usr/bin/env python3
"""Script to fix all policy breachs in semio/py/semio.py."""

import re
import sys

FILE = "/workspaces/semio/semio/py/semio.py"
...
def make_summary(name, kind, parent_classes="", body_hint=""):
    ...
    if (
        ...
        if name == "Error":
            return "# Base exception for all semio errors."
        if name == "ServerError":
        ...
        if name == "KitZipDoesNotContainSemioFolder":
            return "# Exception for a remote zip kit missing the semio folder."
    ...
    class_summaries = {
        "Semio": "# Metadata table recording the database release and engine version.",
        "SModel": "# Abstract base model for all semio pydantic models.",
