...
Implemented project generate specs/docs/todos CLI commands with content-based extraction, 17 tests, zero license leaks
## Changes

- semio-repo/cli/main.go: Added `isLicenseText`, `isHeaderMetaLine` helpers, `ExtractMarkdownSection`, `ExtractFileHeaderSummary` (content-based), `ExtractFileHeaderSpecs` (content-based), `ExtractSectionLeadComments` (filters license/region markers), `ExtractDefinitionDocstring`, `findProjectByName`, `walkProjectFiles` (gitignore+generated filtering), `findFolderReadmes`, `EntityEntry` struct, `GenerateProjectSpecs`, `GenerateProjectDocs`, `GenerateProjectTodos`, `projectCommand` with generate subcommand registered in root
- semio-repo/cli/main_test.go: Added 17 tests covering isLicenseText, isHeaderMetaLine, ExtractMarkdownSection, ExtractFileHeaderSummary, ExtractFileHeaderSpecs, ExtractSectionLeadComments, GenerateProjectSpecs/Docs/Todos for coda+semio+semio-repo, invalid project error handling
...
- Analyzed existing CLI structure, entity types, specs/docs/todos extraction patterns
- Implemented generate command with specs, docs, todos subcommands
...
- [x] Implement GenerateProjectSpecs
...
1. Add helper functions to extract markdown sections (Summary, Specs, Docs) from README.md
2. Add helper functions to extract specs/summary from code file headers and sections
3. Implement GenerateProjectSpecs/Docs/Todos that walk all entities in a project
4. Add `project <name> generate specs|docs|todos` CLI commands
