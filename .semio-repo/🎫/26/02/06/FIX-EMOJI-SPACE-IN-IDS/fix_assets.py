...
def fix_paths():
    ...
    for subdir in ["semio", "semio-repo", "coda"]:
        ...
        for root, dirs, files in os.walk(base_path):
            if "node_modules" in root:
                continue
            for file in files:
                if file.endswith(('.ts', '.tsx', '.json', '.go', '.py', '.css', '.mdx', '.md', '.rs', '.cs', '.d')):
                    ...
                    try:
                        ...
                        if subdir == "semio":
                            ...
                            new_content = re.sub(r'(\.\./){2,}assets', sub_assets, new_content)
                        ...
                        if subdir == "semio":
                            ...
                            new_content = re.sub(r'(\.\./)+sql/sqlite/semio/', sub_sql_semio, new_content)
                            
                            # Handle cases without trailing slash
                            new_content = re.sub(r'(\.\./)+sql/sqlite/semio', lambda m: '../' * (m.group(0).count('../') - 1) + 'sqlite', new_content)

                        # Fix sql patterns for semio-repo/
                        if subdir == "semio-repo":
                            # ../../sql/sqlite/repo/ -> ../sqlite/
                            def sub_sql_repo(m):
                                count = m.group(0).count('../')
                                return '../' * (count - 1) + 'sqlite/'
                            new_content = re.sub(r'(\.\./)+sql/sqlite/repo/', sub_sql_repo, new_content)
                            new_content = re.sub(r'(\.\./)+sql/sqlite/repo', lambda m: '../' * (m.group(0).count('../') - 1) + 'sqlite', new_content)
    ...
    if os.path.exists(agents_path):
        with open(agents_path, 'r', encoding='utf-8') as f:
            content = f.read()
        new_content = content.replace("sql/sqlite/repo/", "semio-repo/sqlite/")
