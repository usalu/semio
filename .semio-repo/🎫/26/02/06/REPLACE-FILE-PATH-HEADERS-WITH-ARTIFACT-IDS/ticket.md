...
- `semio-repo/cli/cli.go`: Added `FileHeaderId()` helper, renamed statute, updated `headerPolicy` detection, updated `generateFileHeader`, added autofix case in `applyAutofixes`, fixed pre-existing `BundleKindClient` → `BundleKindUI` bug
- `semio-repo/cli/cli_test.go`: Added `TestFileHeaderId`, `TestFixHeaderWrongFileId`, `TestFixHeaderWrongFileIdIdempotent`, `TestFixHeaderWrongFileIdDetection`, `TestFixHeaderWrongFileIdEndToEnd`; updated existing tests for new statute; fixed inline comment count expectation
