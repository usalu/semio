...
- [x] Search all references in semio-repo/cli/main.go
- [x] Search all references in semio-repo/cli/main_test.go
- [x] Search all references in semio-repo/vscode/extension.ts
- [x] Search all references in semio-repo/vscode/extension.test.ts
- [x] Search all references in semio-repo/go/repo/main.go (does not exist)
...
// L18448:   ticketPath = fmt.Sprintf(".semio-repo/🎫/%02d/%02d/%02d/%s/ticket.md", ticket.Year, ticket.Month, ticket.Day, ticket.Slug)
// L18449: }
```
...
### FILE 2: semio-repo/cli/main_test.go
...
```go
// L567: files := []string{
// L568:   ".semio-repo/🎫/26/01/20/SAMPLE/plan.md",
// L569:   "./.semio-repo/🎫/26/01/20/SAMPLE/ticket.md",
// L570:   filepath.Join(rootDir, ".semio-repo", "🎫", "26", "01", "20", "SAMPLE", "extra.txt"),
```
...
### FILE 3: semio-repo/vscode/extension.ts
...
// L814: ticketPath = path.join(wsRoot, ".semio-repo", "🎫", year, month, day, slug, "ticket.md");
```
...
// L818:   ticketPath = path.join(wsRoot, ".semio-repo", "🎫", parsed.path, "ticket.md");
```
...
// L1066:   return path.join(root, ".semio-repo", "🎫", relPath);
// L1067: }
```
...
### FILE 4: semio-repo/vscode/extension.test.ts
...
### FILE 5: semio-repo/go/repo/main.go

**File does not exist.** The directory `semio-repo/go/` contains only: `README.md`, `emit.go`, `events.go`, `go.mod`. There are NO references to `ToolTicketProgress` in any Go file outside main.go.
