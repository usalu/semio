...
Applied autofix to resolve all 1429 wrong identification ID breaches in semio-repo/cli/main.go. 0 breaches remaining.
## Changes

- `semio-repo/cli/main.go`: Fixed 1429 wrong section identification IDs across all sections (Preamble, Engine Events, Engine Errors, Engine Requests, Engine, Cli Adapter and all subsections including Utilities, Models, Monorepo Tree Types, Tree Logic, Monorepo Tree, Query Cache, Tree Cache, CLI Renderers, ANSI, Mermaid, GraphQL Types, Drafts, GraphQL Input Types, Providers, Provider Interfaces, GitHub Management Provider, GitHub Source Control Provider, Devcontainer Sandbox Provider, Editor Providers, Provider Registry, Types, Languages, TypeScript, Go, C#, and more).

## Log

- Ran `mcp__semio-repo__analyze` on `semio-repo/cli/main.go` → identified 1429 breaches of kind `code/section/wrong-identification/id` and `code/section/wrong-identification/uri`
- Ran `mcp__semio-repo__fix` on `semio-repo/cli/main.go` → fixed 1429, 0 remaining

## Todos

- [x] Analyze breaches in `semio-repo/cli/main.go`
- [x] Apply autofix
