...
- Updated `semio-repo/vscode/extension.ts`:
...
- Updated `AGENTS.md`:
  ...
  - Added `# Codebase` entry detail for `semio-repo/vscode/extension.ts`.

## Log

- Listed goal tree via `./semio-repo/cli/cli goal tree`.
- Opened ticket via `./semio-repo/cli/cli ticket open`.
...
- Attempted validation:
  - `npm --prefix semio-repo/vscode run preflight` failed due pre-existing `extension.test.ts` `RepoEvent` type errors.
