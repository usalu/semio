...
function createGLTFStructure(design: any): any {
  return {
    ...
    nodes: design.pieces?.map((piece: any, index: number) => ({
    ...
    meshes: design.pieces?.map((piece: any, index: number) => ({
...
function exportGLBCompatible(model: any, outputPath: string): void {
  try {
    ...
    console.log(`✅ GLTF (GLB-compatible) exported to: ${outputPath}`);
    console.log(`📊 Model contains ${model.nodes?.length || 0} nodes and ${model.meshes?.length || 0} meshes`);
  } catch (error) {
 ...
 */
function main(): void {
  console.log('🚀 Starting Nakagin Capsule Tower design model export to GLB format...');
  ...
  console.log(`🧩 Pieces: ${design.pieces?.length || 0}`);
  ...
  console.log('💡 To convert to actual GLB, use: gltf-pipeline -i nakagin-capsule-tower.gltf -o nakagin-capsule-tower.glb');
}
