...
- Removed the design app registrations that injected `semio.sketchpad.app.design.settings` and `semio.sketchpad.app.design.chat` into the right-side panel.
...
- `./semio-repo/cli/cli tree sketchpad detail panel tabs chat settings tests` timed out without returning output.
- The semio CLI binary did not produce usable non-interactive output in this environment, so the task is being tracked directly in this ticket file.
