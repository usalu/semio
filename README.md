...
- Create a ticket: `npx tsx scripts/log.ts ticket create SLUG "Summary"`.
...
- Finish the ticket (aggregates files and recomputes total line stats): `npx tsx scripts/log.ts ticket finish SLUG`.
...
- The codebase report `reports/code.json` flags inline comments (excluding license headers, region markers, TODOs, and header-region blocks), temporary `[DEBUG]` logs, missing SPDX license headers, and mismatched `#region`/`#endregion` pairs; config files and TypeScript `/// <reference ... />` directives are exempt from comment enforcement.
