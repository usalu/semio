...
Code hygiene diagnostics flag orphan lines that live outside named sections so the file structure stays consistent.
...
- The codebase report `reports/code.json` runs the `code` policy with tree-structured violation kinds: `code:header:*` (validates header region with filename, contributors, AGPL-3.0 license), `code:section:*` (validates region naming, content, and orphan lines outside sections), `code:comment:*` (flags inline, block, and JSDoc comments); license headers, region markers, TODOs, and `[DEBUG]` logs are exempt from comment enforcement.
...
All code must sit inside named regions; orphan lines outside any section are reported as code:section:orphan so you can relocate them.
