...
- Development tickets live under `tickets/YEAR/MONTH/DAY/SLUG/ticket.md` with YAML frontmatter tracking task progress and contribution summaries.
- Create a ticket: `npx tsx scripts/log.ts ticket open SLUG --prompt="User prompt..."`.
- Finish the ticket (calculates affected sections, definitions, and line stats): `npx tsx scripts/log.ts ticket close SLUG --summary="..." --files="path1.ts,path2.ts"`.
- Reopen a finished ticket (sets status to open): `npx tsx scripts/log.ts ticket reopen SLUG`.
- List tickets: `npx tsx scripts/log.ts ticket list` (supports year/month/day filters).
- Search tickets: `npx tsx scripts/log.ts ticket search "query"`.
