...
Each reported issue includes a short reason and solution so contributors can resolve violations quickly.
...
- The codebase report `reports/code.json` flags inline and multi-line comments (excluding license headers, region markers, TODOs, and header-region blocks), temporary `[DEBUG]` logs, missing SPDX license headers, and mismatched `#region`/`#endregion` pairs; config files and TypeScript `/// <reference ... />` directives are exempt from comment enforcement.
- Each code report issue includes a reason and a solution to document rule intent and remediation steps.
...
The code hygiene hook enforces comment, license, and region rules before changes are shared.
