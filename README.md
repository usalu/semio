...
- Update per-task git stats in the log frontmatter via `npx tsx scripts/log.ts stats SLUG` (uses tracked affected files; writes `lines.added`/`lines.removed`).
...
- The codebase report `reports/code.json` flags inline comments (excluding license headers and region markers), missing SPDX license headers, and mismatched `#region`/`#endregion` pairs.
...
- `analyze`: `hooks/i18n.ts`, `hooks/typescript.ts` (also emits `reports/code.json`), `hooks/eslint.ts`
...
npx tsx hooks/ruff.ts
 ...
 npx tsx hooks/typescript.ts  # Uses root tsconfig.json (also emits `reports/code.json`)
  npx tsx hooks/eslint.ts
```

Required post-change checks are `npx tsx hooks/typescript.ts` and `npx tsx hooks/eslint.ts` (plus `npx tsx hooks/i18n.ts` when changing UI IDs/translations).
...
## 🟪 [.NET](https://github.com/usalu/semio/tree/main/net) [↑](#%EF%B8%8F-ecosystems-)
...
The repository emits a machine-readable report (`reports/code.json`) that enforces a comment-free codebase (excluding license headers and region markers), mandatory SPDX license headers, and properly nested named regions.
