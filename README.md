...
- The codebase report `reports/code.json` runs generalized policies: `header` (validates header region with filename, contributors, AGPL-3.0 license), `region` (validates region naming and content), `comment` (flags inline, block, and JSDoc comments); license headers, region markers, TODOs, and `[DEBUG]` logs are exempt from comment enforcement.
