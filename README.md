...
- The codebase report `reports/code.json` runs the `code` policy with tree-structured violation kinds: `code:header:*` (validates header region with filename, contributors, AGPL-3.0 license), `code:section:*` (validates region naming and content), `code:comment:*` (flags inline, block, and JSDoc comments); license headers, region markers, TODOs, and `[DEBUG]` logs are exempt from comment enforcement.
