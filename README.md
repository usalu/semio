...
- The codebase report `reports/code.json` flags inline comments (excluding license headers and region markers), temporary `[DEBUG]` logs, missing SPDX license headers, and mismatched `#region`/`#endregion` pairs.
- Documentation is centralized in the root `README.md` and `AGENTS.md`; any other `README.md`/`AGENTS.md` files are flagged by `reports/code.json` and deleted by `npm run fix`.
...
The repository emits a machine-readable report (`reports/code.json`) that enforces a comment-free codebase (excluding license headers and region markers), flags temporary `[DEBUG]` logs, requires SPDX license headers, and validates properly nested named regions.
