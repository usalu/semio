...
- Development logs live under `log/YEAR/MONTH/DAY/SLUG.md` with YAML frontmatter; create entries via `npx tsx scripts/log.ts create SLUG "Summary" --model=MODEL --prompt="User prompt..."`.
- Append each follow-up user message to the log frontmatter via `npx tsx scripts/log.ts prompt SLUG "User prompt..."`.
- Track task-scoped affected files via `npx tsx scripts/log.ts files SLUG <paths...>` (or `--detect --reset`).
- Update per-task git stats in the log frontmatter via `npx tsx scripts/log.ts stats SLUG` (uses tracked affected files).
- Validation reports live in `reports/` and are generated by husky pre-commit hooks in JSON format; all reports are gitignored except `reports/README.md`.
- Runtime Sketchpad components stay console-clean: keep only warnings/errors for real issues and gate temporary diagnostics behind `[DEBUG]` strings that get removed after use.
...
Sketchpad currently keeps Y.js stores as the persisted source of truth while XState v5 machines define the allowed events and selectors. The full transition flips that relationship: the `sketchpadMachine` becomes the canonical state container, carries each app slice (home, kit, type, design, quality, tutorial, transactions), and emits events that keep the Y.js stores synchronized.

Key mechanics for that transition:

- XState actors observe Y.js changes and translate them into sync events (`DESIGN.INIT`, `DESIGN.SYNC`, `TYPE.INIT`, `TYPE.SYNC`, etc.) so the machine context mirrors the live store data.
- React hooks and selectors read from the machine context via `useSketchpadSelector()`/`useDesignAppPanelVisibility()` instead of registering their own Y.js observers.
- Commands route through XState so the machine drives both Y.js mutations and the exposed selectors, allowing stores to drop their direct observation logic and simply surface machine-provided state.
- Kit table row expansion dispatches `useKitAppToggleRow` for immediate machine updates before issuing `semio.kitApp.toggleExpandedRow`, keeping the UI responsive while the Y.js store remains the source of truth.
...
```bash
cd js/js && npm run preflight      # JavaScript/TypeScript workspace
cd py/engine && npm run preflight  # Python workspace
```
...
- **js/js workspace**: Prettier formatting, ESLint auto-fixes, TypeScript type checking
- **py/engine workspace**: Ruff formatting and linting
- **Other workspaces**: Workspace-specific checks or placeholders for future tooling
...
- **Large**: 9 units (e.g. `h-9`) - height of navbar, footer, table row, table header
