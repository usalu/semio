...
} from "../../playground/renderer/react/index.js";
...
function ScenePlayVec3Row(props: { readonly id: string; readonly label: string; readonly values: readonly Vec3[]; readonly uniform: boolean; readonly onChange: (next: Vec3) => void }): React.ReactElement {
...
function ScenePlayInspectorObjects(props: { readonly fixture: FixtureV1; readonly objectIds: readonly string[] }): React.ReactElement {
  ...
  const objects = useMemo(() => props.fixture.objects.filter((object) => idSet.has(object.id)), [props.fixture.objects, idSet]);
...
function ScenePlayInspectorVortices(props: { readonly fixture: FixtureV1; readonly vortexFullIds: readonly string[] }): React.ReactElement {
    ...
    <div className="space-y-3 border-l border-element/60 pl-2">
      {rows.map((row) => (
        <div key={row.fullId} className="space-y-2 rounded border border-element/40 p-2">
          <div className="text-[11px] text-muted-foreground">{scenePlayFixtureRowLabel(row.vortex.label, row.fullId)}</div>
...
function ScenePlayInspectorAttractions(props: { readonly fixture: FixtureV1; readonly attractionIds: readonly string[] }): React.ReactElement {
  const { patchFixture, kindCatalogs } = useScenePlayShell();
  const wireKinds = kindCatalogs?.wires ?? [];
  const attractions = useMemo(() => props.fixture.attractions.filter((attraction) => props.attractionIds.includes(attraction.id)), [props.attractionIds, props.fixture.attractions]);
  return (
    <div className="space-y-3 border-l border-element/60 pl-2">
      ...
      items: [
        {
          ...
          description: (
            <div className="flex flex-col gap-2 pt-1">
              {!hasSelection ? <p className="text-muted-foreground text-xs">Select objects, vortices, or attractions in the canvas or workbench hierarchy.</p> : null}
              <Button className="h-7 gap-1 self-start px-2" disabled={!hasSelection} onClick={() => bus.dispatch(SCENE_PLAY_CONTROLLER_ID, "deleteSelection")} size="sm" type="button" variant="destructive">
  ...
  return (
    <>
      ...
      <Canvas3D
        ...
        selection={props.selection}
      >
        <PlaySceneCanvasContent relocateMode={props.relocateMode} selection={props.selection} selectedId={props.selectedId} selectedVortexFullIds={props.selectedVortexFullIds} setSelectedId={props.setSelectedId} />
      </Canvas3D>
    </>
  );
}
...
function PlayInner(): React.ReactElement {
  ...
  return <ScenePlayProductShell extraFooterItems={surfaceFooterItems} mobile={device === "mobile"} runtime={sceneShellRef.current} />;
}

function ScenePlayProductShell(props: { readonly runtime: ProductRuntime; readonly extraFooterItems: FooterItem[]; readonly mobile: boolean }): React.ReactElement {
  ...
  const selectionKey = shellValue ? `${shellValue.snap.selection.objectIds.join("\0")}\u0001${shellValue.snap.selection.vortexIds.join("\0")}\u0001${shellValue.snap.selection.attractionIds.join("\0")}\u0001${shellValue.snap.fixtureRevision}` : "";
        ...
        ? [
            new ScenePlayHierarchyPanelDefinition(() =>
              buildScenePlayHierarchySections(shellValue.snap.fixture, shellValue.snap.selection, {
                onSelectObject: (objectId) => shellValue.setSelection({ objectIds: [objectId], vortexIds: [], attractionIds: [] }),
                onSelectVortex: (vortexFullId) => shellValue.setSelection({ objectIds: [], vortexIds: [vortexFullId], attractionIds: [] }),
                onSelectAttraction: (attractionId) => shellValue.setSelection({ objectIds: [], vortexIds: [], attractionIds: [attractionId] }),
              }),
            ).resolveTab(),
  ...
  const detailTabs = useMemo(
    () => (shellValue ? [new ScenePlayInspectorPanelDefinition(() => buildScenePlayInspectorSections(shellValue, bus)).resolveTab(), new ScenePlaySettingsPanelDefinition(() => buildScenePlaySettingsSections(shellValue.snap, bus)).resolveTab()] : []),
    [shellValue, bus],
  );
  const product = (
    <PlaygroundView
