  ...
  return (
    <group ref={groupRef} position={position} quaternion={quaternion} scale={scale}>
      {fill ? (
        <mesh onPointerDown={select}>
          <bufferGeometry>
            ...
            {fill.normal.length > 0 ? <bufferAttribute attach="attributes-normal" array={fill.normal} itemSize={3} count={fill.normal.length / 3} /> : null}
            <bufferAttribute attach="index" array={fill.index} itemSize={1} count={fill.index.length} />
          </bufferGeometry>
          <meshStandardMaterial color={selectedColor(props.renderable.style?.color, selected, "#7dd3fc")} opacity={props.renderable.style?.opacity ?? 0.65} transparent polygonOffset polygonOffsetFactor={1} polygonOffsetUnits={1} side={2} />
        </mesh>
      ) : null}
      {edges ? (
      ...
      {props.renderable.children?.map((child) => (
        <SpatialRenderableNode key={child.id} renderable={child} draggingId={props.draggingId} selectedId={props.selectedId} selectableKinds={props.selectableKinds} onSelect={props.onSelect} onDragStart={props.onDragStart} />
      ))}
    </group>
  );
}

function SpatialViewportScene(props: { readonly snapshot: SpatialSurfaceSnapshot; readonly renderables: readonly SpatialRenderable[]; readonly onSelect: (id: string | null) => void }): React.ReactElement {
  ...
  return (
    <>
      ...
      {props.renderables.map((renderable) => (
        <SpatialRenderableNode key={renderable.id} renderable={renderable} draggingId={draggingId} selectedId={props.snapshot.selectedId} selectableKinds={props.snapshot.selectableKinds} onSelect={props.onSelect} onDragStart={beginDrag} />
      ))}
...
export function SpatialViewport(props: { readonly snapshot: SpatialSurfaceSnapshot; readonly onSelect: (id: string | null) => void }): React.ReactElement {
  const renderables = React.useMemo(() => listSpatialViewportRenderables({ model: props.snapshot.model, focusedKind: props.snapshot.focusedKind, visibleKinds: props.snapshot.visibleKinds }), [props.snapshot]);
  return (
...
//#region 🔖Surface
export function SpatialSurface(props: { readonly snapshot: SpatialSurfaceSnapshot; readonly disableViewport?: boolean }): React.ReactElement {
  return (
    <div className={`absolute inset-0 min-h-0 min-w-0 overflow-hidden ${getLevelBgClass("window")} bg-[radial-gradient(circle_at_top,rgba(148,163,184,0.12),transparent_28%),linear-gradient(180deg,rgba(15,23,42,0.98),rgba(2,6,23,1))]`}>
      {props.snapshot.model && props.snapshot.status === "ready" && !props.disableViewport ? (
        ...
        <div className="flex h-full items-center justify-center text-sm text-muted-foreground">{props.snapshot.status === "ready" ? "Viewport disabled for this surface." : "Preparing scene…"}</div>
      )}
    </div>
  );
}
    ...
    <div className="flex h-full min-h-0 flex-col gap-3 p-3" data-e2e-spatial-workbench-panel>
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        <div className="mb-3 flex items-center justify-between text-xs text-muted-foreground">
          ...
          <span className="rounded-full border border-border bg-muted px-2 py-0.5" data-e2e-spatial-entity-count>
            {panel.entityCount}
          </span>
        </div>
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-3 p-3" data-e2e-spatial-details-panel>
      <div className="rounded-lg border border-border bg-background px-3 py-3 text-sm">
        ...
        <div className="mt-2 font-medium text-foreground" data-e2e-spatial-selection-label>
          {details.selectedLabel}
        </div>
        <div className="mt-1 text-xs text-muted-foreground" data-e2e-spatial-selection-kind>
          {details.selectedKindLabel}
        </div>
      ...
      <div className="rounded-lg border border-border bg-background px-3 py-3 text-xs text-muted-foreground">
        <div>
          <span className="font-medium text-foreground">Status</span>: {details.status}
        </div>
        <div className="mt-1">
          <span className="font-medium text-foreground">Focus</span>: {details.focusedKindLabel}
        </div>
        <div className="mt-1">
          <span className="font-medium text-foreground">Query</span>: {details.query}
        </div>
      </div>
    </div>
  );
