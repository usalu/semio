...
mod board_host {
    ...
    impl BoardHost {
        ...
        fn lod_allows_individual_picks(&self, lod: BoardDrawLod) -> bool {
            lod != BoardDrawLod::Minimap
        }

        fn lod_allows_node_drag(&self, lod: BoardDrawLod) -> bool {
            lod != BoardDrawLod::Minimap
        }
            ...
            if draw_icon {
                if let Some(k) = h.icon_kind.as_ref().map(|s| s.trim()).filter(|s| !s.is_empty()) {
                    if let Some((bx, by, bw, bh, body)) = self.get_or_build_icon_paint(k, stroke_c, fill) {
                        let fit_inset = HANDLE_MARKER_ICON_FIT_INSET;
                        ...
                        let r_clip = (radius_world * self.camera.zoom * HANDLE_MARKER_ICON_CLIP_FRAC).max(1.0);
            ...
            for n in self.nodes.values() {
                ...
                if draw_node_icons {
                    if let Some(k) = n.icon_kind.as_ref().map(|s| s.trim()).filter(|s| !s.is_empty()) {
                        if let Some((bx, by, bw, bh, body)) = self.get_or_build_icon_paint(k, stroke_c, fill) {
                            let clip_inset = NODE_ICON_CLIP_INSET;
                            let fit_inset = NODE_ICON_FIT_INSET;
...
mod host_tests {
    ...
    fn board_host_proximity_connect_sets_preview_while_overlapping() {
        ...
        assert!(matches!(
            &h.interaction,
            Interaction::DragNodes { proximity_connect: Some((s, t)), .. } if s == "a:h0" && t == "b:h0"
        ));
        h.pointer_up_screen(sm.x, sm.y);
    }
