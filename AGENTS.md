...
- You MUST use `bun` as package manager and `nx` as task runner.
  - You MUST use only conventional script names in `package.json` such as `setup`, `start`, `dev`, `generate`, `lint`, `format`, `test`, `build`, `publish`, `purge` and when a bundle has more than one script then use `:` for the additional scripts e.g. `dev:mcp`
