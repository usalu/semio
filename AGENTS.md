...
The `reports/` folder stores all autogenerated JSON outputs, so the node files stay gitignored while `reports/README.md` documents how to regenerate and interpret them (keep the README current whenever a new hook writes a report).
