...
- You MUST use `bun` as package manager and `nx` as task runner.
  - You MUST use only conventional script names in `package.json` such as `dev`, `build`, `lint`, `publish` and when a bundle has more than one script then use `:` for the additional scripts e.g. `dev:mcp`
