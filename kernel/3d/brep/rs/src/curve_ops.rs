...
const GL5_NODES: [f64; 5] = [-0.9061798459386640, -0.5384693101056831, 0.0, 0.5384693101056831, 0.9061798459386640];
...
pub fn closest_point(curve: &Curve3, domain: (f64, f64), target: Pnt3, samples: usize) -> (f64, f64) {
    ...
    // Clamp to the full domain, not a tight neighborhood: a too-tight window can trap Newton at
    // its boundary and silently return a biased (locally-clamped, not truly optimal) result.
    let refined = newton_closest_point(curve, target, best_t, domain.0, domain.1);
...
fn newton_closest_point(curve: &Curve3, target: Pnt3, mut t: f64, lo: f64, hi: f64) -> f64 {
    for _ in 0..30 {
        let c = curve.eval(t);
        let d1 = curve.d1(t);
        let d2 = curve.d2(t);
...
pub fn all_extrema(curve: &Curve3, domain: (f64, f64), target: Pnt3, samples: usize) -> Vec<f64> {
    ...
    for i in 1..=samples {
        ...
        if prev_f == 0.0 {
            ...
            roots.push(newton_closest_point(curve, target, 0.5 * (prev_t + t), prev_t, t));
        }
        prev_t = t;
        prev_f = ft;
    }
...
pub fn split_nurbs(curve: &NurbsCurve3, t: f64) -> (NurbsCurve3, NurbsCurve3) {
    ...
    let split_index = knots.find_span(t) - degree + 1;
    let dehomogenize = |i: usize| Pnt3::new(hx[i] / hw[i], hy[i] / hw[i], hz[i] / hw[i]);
    let left_controls: Vec<Pnt3> = (0..split_index + degree).map(dehomogenize).collect();
    let left_weights: Vec<f64> = hw[0..split_index + degree].to_vec();
    let right_controls: Vec<Pnt3> = (split_index..hx.len()).map(dehomogenize).collect();
    let right_weights: Vec<f64> = hw[split_index..].to_vec();
...
mod tests {
    ...
    fn interpolate_centripetal_passes_through_all_points() {
        ...
        let (lo, hi) = curve.knots.domain();
        assert!((de_boor_pnt(&curve, lo)).distance(points[0]) < 1e-6);
        assert!((de_boor_pnt(&curve, hi)).distance(*points.last().unwrap()) < 1e-6);
        // Interior points must also be interpolated (not just endpoints).
        for p in &points[1..points.len() - 1] {
            let closest = (0..=200).map(|i| lo + (hi - lo) * i as f64 / 200.0).map(|t| de_boor_pnt(&curve, t).distance(*p)).fold(f64::INFINITY, f64::min);
            assert!(closest < 1e-3, "point {p:?} not interpolated, closest sampled distance {closest}");
        }
    }
