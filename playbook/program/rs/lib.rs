//! 🧩 Playbook program — standalone strict-list, Blockly-like builder app bundled as a hot-swappable
...
use semio_framework_program::{
    ...
    DocumentView, ProgramBundle, SurfaceKind, UiNode, ViewState,
};
...
fn register_playbook_play_exports() {
    semio_framework_program::program_runtime::register_document_codec_for_app::<PlaybookPlayApp>(PLAYBOOK_DOCUMENT_SCHEMA);
}

fn playbook_play_bundle() -> ProgramBundle {
    register_playbook_play_exports();
    ProgramBundle::new(PLAYBOOK_PLAY_PLUGIN_ID, "Playbook", "0.1.0")
        .register_document_app(create_playbook_play_app(), PlaybookPlayApp::default)
}

semio_framework_program::program_exports!(playbook_play_bundle);
//#endregion 🔖Manifest
