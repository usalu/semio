//! 🗺️ Mindmap program — WIRES app in a hot-swappable WASM program.
//! 🧠 Mindmap Wires program — declarative WIRES play app bundled as a hot-swappable WASM program.
...
/// 🎯 `ids` array first via the shared SDK core, falling back to a singular `id` key — this app's
/// selection actions accept either shape.
fn selection_ids(args: Option<&Value>) -> Vec<String> {
    let ids = semio_framework_program::selection_ids(args);
    if !ids.is_empty() {
        return ids;
    }
...
fn force_layout_board(board: &mut Value) {
    let Ok(layout_json) = infinite_board_port_directed::apply_force_graph_layout_to_fixture_v1_json(&board.to_string(), r#"{"mode":"force-graph"}"#) else {
        return;
    };
...
fn kind_catalog_items(namespace: &PanelTreeBuilder, kind: &str, entries: &[Value]) -> Vec<UiTreeItemNode> {
    entries
        .iter()
        .enumerate()
        .map(|(index, entry)| {
...
//#endregion 🔖ReasoningWiresPlayApp
...
fn register_reasoning_mindmap_exports() {
    semio_framework_program::program_runtime::register_document_codec_for_app::<ReasoningWiresPlayApp>(WIRES_FIXTURE_SCHEMA);
}
...
semio_framework_program::semio_program! {
    ...
    apps: [ create_wires_app => ReasoningWiresPlayApp ],
}
//#endregion 🔖Manifest
...
mod tests {
    use super::*;
    use semio_framework_program::testkit;
    use semio_framework_program::{ProgramApp, VcsDocumentApp};
