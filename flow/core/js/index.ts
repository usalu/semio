...
export const FLOW_PLAY_SURFACE_ID = "flow.play";
...
export const FLOW_PLAY_DEFAULT_FIXTURE: FlowFixture = FLOW_DEFAULT_FIXTURE;
...
export function parseFlowPlayFixtureJson(json: string): FlowFixture | null {
  try {
    const parsed = JSON.parse(json) as FlowFixture;
    if (parsed.schema !== "flow.fixture" || !Array.isArray(parsed.widgets) || !Array.isArray(parsed.synapses)) {
      return null;
    }
    return parsed;
  } catch {
...
function flowPlayWidgetTreeLabel(widget: FlowWidget): string {
  switch (widget.kind) {
    case "neuron":
...
function flowPlayInspectorKindGroup(kind: FlowWidget["kind"], widgets: readonly FlowWidget[], controllerId: string): UiInspectorFieldGroup | null {
...
function flowPlayInspectorBaseGroup(widgets: readonly FlowWidget[], controllerId: string): UiInspectorFieldGroup {
  const widgetIds = widgets.map((entry) => entry.id);
...
export function buildFlowPlayInspectorTree(
  ...
  const widgets = selectedNodeIds
    ...
    .filter((widget): widget is FlowWidget => Boolean(widget));
  if (!widgets.length) {
    return uiDeclarativeSectionsToTree([
...
export class FlowPlayController extends Controller {
  ...
  private readonly docStore = new DocumentVcsStore<FlowFixture, FlowFixtureEditOp>({
    envelope: createDocumentVcsEnvelope("flow.fixture", "flow-play", FLOW_PLAY_DEFAULT_FIXTURE),
...
if (import.meta.vitest) {
  ...
  describe("flow play shell", () => {
    ...
    it("batch-patches shared fields across multiple widgets", () => {
      const bus = new CommandBus();
      const ctrl = new FlowPlayController(bus, () => {});
      const json = JSON.parse(FLOW_PLAY_DEFAULT_FIXTURE_JSON) as FlowFixture;
      ...
      const fixture: FlowFixture = { ...json, widgets: [...json.widgets, secondSlider] };
      ...
      const updated = JSON.parse(ctrl.getFixtureJson()) as FlowFixture;
    ...
    it("registers generate mode and maps fixture widgets to form spec", () => {
      ...
      expect(spec.schema).toBe("forms.form");
