...
if (import.meta.env.VITEST) {
  ...
  initSync({ module: readFileSync(join(reactDir, "../core/rs/pkg/flow_core_bg.wasm")) });
  const [
    ...
    import("../module/core/rs/pkg/flow_module_core.js"),
    import("../module/math/rs/pkg/flow_module_math.js"),
    import("../module/text/rs/pkg/flow_module_text.js"),
    import("../module/logic/rs/pkg/flow_module_logic.js"),
    import("../module/dictionary/rs/pkg/flow_module_dictionary.js"),
    import("../module/list/rs/pkg/flow_module_list.js"),
    import("../module/brep/rs/pkg/flow_module_brep.js"),
    import("../module/bim/rs/pkg/flow_module_bim.js"),
    import("../module/draw/rs/pkg/flow_module_draw.js"),
  ]);
  initCoreSync({ module: readFileSync(join(reactDir, "../module/core/rs/pkg/flow_module_core_bg.wasm")) });
  initMathSync({ module: readFileSync(join(reactDir, "../module/math/rs/pkg/flow_module_math_bg.wasm")) });
  initTextSync({ module: readFileSync(join(reactDir, "../module/text/rs/pkg/flow_module_text_bg.wasm")) });
  initLogicSync({ module: readFileSync(join(reactDir, "../module/logic/rs/pkg/flow_module_logic_bg.wasm")) });
  initDictionarySync({ module: readFileSync(join(reactDir, "../module/dictionary/rs/pkg/flow_module_dictionary_bg.wasm")) });
  initListSync({ module: readFileSync(join(reactDir, "../module/list/rs/pkg/flow_module_list_bg.wasm")) });
  initBrepSync({ module: readFileSync(join(reactDir, "../module/brep/rs/pkg/flow_module_brep_bg.wasm")) });
...
export {
  ...
  dagLodCanvasProps,
  isDagDrawLodKind,
  parseDagLodScaleJson,
  type DagDrawLodKind,
  type DagLodEntry,
  type DagLodModeKind,
};

let flowDagLodScaleCache: readonly DagLodEntry[] | null = null;

/** @emoji 📶 Flow play LOD table from the already-booted {@link FlowSession} WASM host. */
export function getDagLodScale(): readonly DagLodEntry[] {
  if (!flowDagLodScaleCache) {
    flowDagLodScaleCache = parseDagLodScaleJson(new FlowSession().lodScaleJson());
  }
  return flowDagLodScaleCache;
}

/** @emoji 📶 Ordered LOD tier ids for flow play window selects. */
...
/** @emoji 📶 Menu label for a pinned flow play LOD tier. */
...
const FLOW_MODULE_LOADERS: Record<string, FlowModuleLoader> = {
  core: () => loadFlowWasmModule(import("../module/core/rs/pkg/flow_module_core.js"), import("../module/core/rs/pkg/flow_module_core_bg.wasm?url")),
  math: () => loadFlowWasmModule(import("../module/math/rs/pkg/flow_module_math.js"), import("../module/math/rs/pkg/flow_module_math_bg.wasm?url")),
  text: () => loadFlowWasmModule(import("../module/text/rs/pkg/flow_module_text.js"), import("../module/text/rs/pkg/flow_module_text_bg.wasm?url")),
  logic: () => loadFlowWasmModule(import("../module/logic/rs/pkg/flow_module_logic.js"), import("../module/logic/rs/pkg/flow_module_logic_bg.wasm?url")),
  dictionary: () =>
    loadFlowWasmModule(import("../module/dictionary/rs/pkg/flow_module_dictionary.js"), import("../module/dictionary/rs/pkg/flow_module_dictionary_bg.wasm?url")),
  list: () => loadFlowWasmModule(import("../module/list/rs/pkg/flow_module_list.js"), import("../module/list/rs/pkg/flow_module_list_bg.wasm?url")),
  brep: () => loadFlowWasmModule(import("../module/brep/rs/pkg/flow_module_brep.js"), import("../module/brep/rs/pkg/flow_module_brep_bg.wasm?url")),
  draw: () => loadFlowWasmModule(import("../module/draw/rs/pkg/flow_module_draw.js"), import("../module/draw/rs/pkg/flow_module_draw_bg.wasm?url")),
  bim: () => loadFlowWasmModule(import("../module/bim/rs/pkg/flow_module_bim.js"), import("../module/bim/rs/pkg/flow_module_bim_bg.wasm?url")),
};
...
export type FlowNoteEditKeyAction =
  | { readonly kind: "commit" }
  | { readonly kind: "backspace" }
  | { readonly kind: "deleteForward" }
  | { readonly kind: "moveCaret"; readonly direction: "left" | "right" | "home" | "end"; readonly extend: boolean }
  | { readonly kind: "insertText"; readonly text: string };

/** @emoji ✏️ Maps a keydown event to a native note-edit action while a note is being edited. */
export function flowNoteEditKeyAction(key: string, mod: boolean, shift: boolean): FlowNoteEditKeyAction | null {
  if (key === "Escape" || key === "Enter") return { kind: "commit" };
  if (key === "Backspace") return { kind: "backspace" };
  if (key === "Delete") return { kind: "deleteForward" };
  if (key === "ArrowLeft") return { kind: "moveCaret", direction: "left", extend: shift };
  if (key === "ArrowRight") return { kind: "moveCaret", direction: "right", extend: shift };
  if (key === "Home") return { kind: "moveCaret", direction: "home", extend: shift };
  if (key === "End") return { kind: "moveCaret", direction: "end", extend: shift };
  if (key.length === 1 && !mod) return { kind: "insertText", text: key };
  return null;
}
...
type FlowSessionNoteEditApi = FlowSession & {
  beginNoteEdit(widgetId: string, worldX: number, worldY: number): void;
  noteInsertText(chunk: string): void;
  noteBackspace(): void;
  noteDeleteForward(): void;
  noteMoveCaret(direction: string, extend: boolean): void;
  noteCommitEdit(): void;
  setNoteCaretVisible(visible: boolean): void;
};
...
export function FlowCanvas({
  ...
  const commitNoteEdit = useCallback(() => {
    const session = sessionRef.current as FlowSessionNoteEditApi | null;
    if (!session || !editingNoteIdRef.current) return;
    session.noteCommitEdit();
  ...
  const startNoteEdit = useCallback(
    (widgetId: string, worldX: number, worldY: number) => {
      const session = sessionRef.current as FlowSessionNoteEditApi | null;
      if (!session) return;
      session.beginNoteEdit(widgetId, worldX, worldY);
      editingNoteIdRef.current = widgetId;
      setEditingNoteId(widgetId);
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (editingNoteIdRef.current) {
        ...
        const action = flowNoteEditKeyAction(event.key, mod, event.shiftKey);
        if (!action) return;
        event.preventDefault();
        ...
        if (action.kind === "deleteForward") {
          session.noteDeleteForward();
          evaluate();
          persistFixture();
          renderFrame();
          return;
        }
        if (action.kind === "moveCaret") {
          session.noteMoveCaret(action.direction, action.extend);
          renderFrame();
          return;
        }
        session.noteInsertText(action.text);
        evaluate();
        persistFixture();
        renderFrame();
        return;
  ...
  }, [commitNoteEdit, emitInteractionState, evaluate, persistFixture, renderFrame]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const tick = () => {
        const session = sessionRef.current as FlowSessionNoteEditApi | null;
        if (session && editingNoteIdRef.current) {
          session.setNoteCaretVisible(Math.floor(performance.now() / 530) % 2 === 0);
        }
        renderFrame();
  ...
  const onPointerDown = useCallback(
    (e: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      if (editingNoteIdRef.current) {
        ...
        if (hoveredId !== editingNoteIdRef.current) {
          commitNoteEdit();
        } else {
          (session as FlowSessionNoteEditApi).beginNoteEdit(hoveredId, world.x, world.y);
          renderFrame();
          return;
        }
      }
    ...
    [canvasPick, clientToCanvas, commitNoteEdit, emitInteractionState, renderFrame],
  );
  ...
  return (
    <div
      ref={containerRef}
      tabIndex={0}
      role={editingNoteId ? "textbox" : undefined}
      aria-label={editingNoteId ? "Note editor" : undefined}
...
import type { AppRendererContribution, UiFlowHostSurfaceNode, UiFormsHostSurfaceNode, UiWriterHostSurfaceNode } from "@semio-tech/framework-platform-core";
...
import { useApp, usePlayController, collectUiTreeItemDragData } from "@semio-tech/framework-playground-renderer-react";
...
import type { FlowPlayController } from "@semio-tech/flow-core";
import { parseFormSpec } from "@semio-tech/forms-core";

let flowPlayDefaultFixtureJsonRef = "{}";
let buildFlowPlayCanvasContextMenuRef:
	| typeof import("@semio-tech/flow-core").buildFlowPlayCanvasContextMenu
	| undefined;
...
export async function downloadFlowOutputExport(format: string, resolvedValueJson: string, widgetId: string): Promise<void> {
  ...
  if (normalized === "png") {
    ...
    const { canvasDrawingPngExportPort } = await import("@semio-tech/procedural-2d-react");
...
function FlowPlayPaneSurfaceHost({ node }: { readonly node: UiFlowHostSurfaceNode }): ReactElement {
  ...
  const ctrl = usePlayController<FlowPlayController>();
  ...
  const scopeId = node.paneId ?? "flow-main";
  ...
  return (
    <FlowCanvas
      fixtureJson={ctrl?.getFixtureJson() ?? flowPlayDefaultFixtureJsonRef}
      fixtureDragDrop
      ...
      contextMenu={(ctx) => buildFlowPlayCanvasContextMenuRef?.(ctx, onCanvasCommand) ?? []}
...
function FlowPlayCompiledDagSurfaceHost({ node: _node }: { readonly node: UiWriterHostSurfaceNode }): ReactElement {
  const { runtime } = useApp();
  const ctrl = usePlayController<FlowPlayController>();
...
function FlowPlayGenerateSurfaceHost({ node }: { readonly node: UiFormsHostSurfaceNode }): ReactElement {
  const ctrl = usePlayController<FlowPlayController>();
  const [FlowGenerateSurface, setFlowGenerateSurface] = reactHostPort.useState<typeof import("@semio-tech/forms-react").FlowGenerateSurface | null>(null);
  reactHostPort.useEffect(() => {
    void import("@semio-tech/forms-react").then((mod) => setFlowGenerateSurface(() => mod.FlowGenerateSurface));
  }, []);
  ...
  if (!FlowGenerateSurface) return <div className="h-full" />;
  return (
    <FlowGenerateSurface
      formSpec={spec}
...
export async function flowAppRenderer(): Promise<AppRendererContribution> {
  const core = await import("@semio-tech/flow-core");
  flowPlayDefaultFixtureJsonRef = core.FLOW_PLAY_DEFAULT_FIXTURE_JSON;
  buildFlowPlayCanvasContextMenuRef = core.buildFlowPlayCanvasContextMenu;
  return {
    windowBodies: core.flowPlayWindowBodies,
    sidePanelBodies: core.flowPlaySidePanelBodies,
    surfaceHosts: {
      [core.FLOW_PLAY_SURFACE_ID]: FlowPlayPaneSurfaceHost,
      [core.FLOW_PLAY_SURFACE_ID_GENERATE]: FlowPlayGenerateSurfaceHost,
      [core.FLOW_PLAY_SURFACE_ID_COMPILED_DAG]: FlowPlayCompiledDagSurfaceHost,
    },
...
if (import.meta.vitest) {
  ...
  describe("flowNoteEditKeyAction", () => {
    it("maps editing keys and ignores canvas shortcuts", () => {
      expect(flowNoteEditKeyAction("Enter", false, false)).toEqual({ kind: "commit" });
      expect(flowNoteEditKeyAction("a", false, false)).toEqual({ kind: "insertText", text: "a" });
      expect(flowNoteEditKeyAction("z", true, false)).toBeNull();
      expect(flowNoteEditKeyAction("ArrowLeft", false, true)).toEqual({ kind: "moveCaret", direction: "left", extend: true });
    });
  });

  describe("flow note edit engagement routing", () => {
    it("treats the flow canvas container as a typing target while editing a note", () => {
      const host = document.createElement("div");
      host.tabIndex = 0;
      host.setAttribute("role", "textbox");
      document.body.appendChild(host);
      host.focus();
      expect(isUiTypingTarget(host)).toBe(true);
      expect(shouldRouteKeysToWindowEngagement(host)).toBe(false);
      host.remove();
    });
  });
