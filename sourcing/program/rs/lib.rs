//! 🛒 Sourcing program — curate app: handpick and curate 3D object kinds out of a modular catalogue.
...
/// 🧩 One module's typology + catalogue kinds, resolved either from a contributed program or, as a
...
/// 🧩 Contributed module catalogues if any program has contributed, else the built-in modules — so the
...
fn sourcing_setup() {
    semio_framework_program::program_runtime::register_document_codec_for_app::<SourcingCurateApp>(SOURCING_CURATE_SCHEMA);
}
...
mod tests {
    use super::*;
    use semio_framework_program::{testkit, ProgramApp};
    ...
    fn contributions_parse_into_module_catalogues() {
        ...
        let contributions_json = serde_json::to_string(&vec![ProgramContributionEntry { program_id: "sourcing-module-beams".into(), contribution }]).unwrap();
