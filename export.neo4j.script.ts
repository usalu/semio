#!/usr/bin/env bun
/**
 * 🛂 Dumps the **handcrafted** Neo4j graph for one technology/database to a replayable Cypher file under `.repo/🛂/`.
 * Uses APOC only (`apoc.export.cypher.all`); **no** YAML, GraphQL, or other schema sources.
 *
 * Usage:
 *   `bun ./export.neo4j.script.ts [technology]`
 *   `technology` defaults to `NEO4J_DATABASE` or `semio`. Allowed: `semio` | `elements` | `coda` | `reuse`.
 *
 * Multi-database (Desktop): export each graph, then commit the four files:
 *   `NEO4J_DATABASE=semio bun ./export.neo4j.script.ts semio`
 *   `NEO4J_DATABASE=elements bun ./export.neo4j.script.ts elements`
 *   …
 *
 * Requires: APOC `apoc.export.cypher.all`. Prefer `apoc.import.file.use_neo4j_config=false` so exports can write an **absolute** path under this repo (matches native/Desktop setup scripts).
 */
...
function runCypher(database: string, cypher: string): { ok: boolean; stdout: string; stderr: string } {
  ...
  const queryPath = join(queryDir, `neo4j-export-query-${process.pid}-${Date.now()}.cypher`);
...
function writeHandcraftedBundle(technology: string, database: string, body: string, finalPath: string): void {
  ...
  const header = [
    ...
    `// technology: ${technology} | database: ${database} | exported: ${stamp}`,
    "//",
    "",
  ].join("\n");
...
function main(): void {
  const arg = process.argv.slice(2).find((a) => ALLOWED.has(a));
  const technology = arg ?? process.env.NEO4J_DATABASE ?? "semio";
  if (!ALLOWED.has(technology)) {
    console.error(`[export:neo4j] technology must be one of: ${[...ALLOWED].join(", ")} (got ${JSON.stringify(technology)})`);
    process.exit(1);
  }
  ...
  const tmpFileName = `.export-${technology}-${process.pid}.tmp.cypher`;
  const tmpAbs = join(cacheDir, tmpFileName);

  const probe = runCypher(database, "RETURN 1 AS ok;");
  if (!probe.ok) {
    console.error(`[export:neo4j] cannot reach database ${JSON.stringify(database)}:\n${probe.stderr || probe.stdout}`);
    process.exit(1);
  }
  ...
  if (!result.ok) {
    console.error(`[export:neo4j] apoc.export.cypher.all failed:\n${result.message}`);
    process.exit(1);
  }

  if (!existsSync(tmpAbs)) {
    console.error(`[export:neo4j] expected export file missing at ${tmpAbs} after APOC call.`);
    process.exit(1);
  }

  const body = readFileSync(tmpAbs, "utf8");
  unlinkSync(tmpAbs);
  writeHandcraftedBundle(technology, database, body, finalAbs);

  console.log(`[export:neo4j] wrote ${finalAbs} (database ${database}).`);
  if (result.message) {
