...
// Specs: Re-exports AlgorithmApp from @semio/ui and native algorithm helpers that route TS work through `@semio/js` `KitStore` / REST for other languages.
// Summary: Single entry; native runners live in this file (no separate adapter module).
...
// #region 🧮NativeKitStoreRunners
// @emoji 🧮 TS path: `KitStore` + GraphQL flatten rows; non-`ts` languages POST to engine REST. Design diff assembly for stories stays here until rs exposes preview-only commands.
...
// #endregion 🧮NativeKitStoreRunners
