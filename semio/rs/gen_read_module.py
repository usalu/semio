#!/usr/bin/env python3
"""One-shot generator for read_module.rs (run from semio/rs)."""
from __future__ import annotations

OUT = "read_module.rs"

HEADER = r'''//! Machine-generated read command surface: exhaustive commands + outputs, live `KitGraph` execution.
...
def main() -> None:
    parts = [HEADER]
    # Kit command enum - explicit list
    parts.append(emit_read_kit())
    parts.append(emit_read_type())
    parts.append(emit_read_design())
    parts.append(emit_read_piece())
    parts.append(emit_read_connection())
    parts.append(emit_read_side())
    parts.append(emit_read_port())
    parts.append(emit_read_connector())
    parts.append(emit_read_representation())
    parts.append(emit_read_family())
    parts.append(emit_read_file())
    parts.append(emit_read_folder())
    parts.append(emit_read_location())
    parts.append(emit_read_layer())
    parts.append(emit_read_group())
    parts.append(emit_read_author())
    parts.append(emit_read_concept())
    parts.append(emit_read_tag())
    parts.append(emit_read_quality())
    parts.append(emit_read_benchmark())
    parts.append(emit_read_prop())
    parts.append(emit_read_attribute())
    parts.append(emit_read_stat())
    parts.append(emit_impls())
    with open(OUT, "w", encoding="utf-8") as f:
        f.write("".join(parts))
    print("wrote", OUT)


def emit_read_kit() -> str:
    return r'''
// --- Kit ---
...
def emit_read_representation() -> str:
    return r'''
// --- Representation ---
