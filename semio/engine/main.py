...
# [👤semio📚engine💻engine🔖rest🔖appendpoints](repo://p/u/semio/b/l/engine/f/engine.py/s/Rest/s/App%20Endpoints)
# App Endpoints MUST serve sandboxed iframe-compatible HTML for MCP app tools.
...
html,body,#root{width:100%;height:100%;overflow:hidden;font-family:system-ui,sans-serif}
body{background:var(--background,#09090b);color:var(--foreground,#fafafa)}
:root{--background:#09090b;--foreground:#fafafa;--border:#27272a;--muted:#a1a1aa;--accent:#3b82f6;--destructive:#ef4444;--success:#22c55e;--warning:#f59e0b}
.loading{display:flex;align-items:center;justify-content:center;height:100%;color:var(--muted);font-size:14px}
.error{color:var(--destructive);padding:16px;font-size:14px}
</style>
</head>
<body>
<div id="root"><div class="loading">Loading semio design viewer&hellip;</div></div>
<script type="module">
// Sandboxed iframe app: receives kit, design, mode, selection via postMessage.
// Communicates selection changes back to parent via postMessage.
// No cross-origin assumptions, no window.top access.
...
const state = {
  ...
  designGuid: null,
  designDiff: null,
  selection: { pieceGuids: [], connectionGuids: [] },
  camera: null,
  capabilities: { pieceSelection: false, connectionSelection: false, diff: false },
};

function render() {
  const root = document.getElementById("root");
  if (!state.kit || !state.designGuid) {
    root.innerHTML = '<div class="loading">Waiting for design data&hellip;</div>';
    return;
  }

  const modeLabel = (state.mode || "show-design").replace(/-/g, " ");
  const pieces = [];
  const designs = state.kit.designs || [];
  const design = designs.find(d => d.guid === state.designGuid);
  ...
  const connections = design && design.connections ? design.connections : [];
  const selPieces = new Set(state.selection.pieceGuids || []);
  const selConns = new Set(state.selection.connectionGuids || []);

  let html = '<div style="padding:8px;height:100%;overflow:auto">';
  html += '<div style="font-size:12px;color:var(--muted);margin-bottom:8px">';
  html += '<strong>' + escapeHtml(modeLabel) + '</strong>';
  html += ' &middot; ' + escapeHtml(design ? design.name || state.designGuid : state.designGuid);
  html += ' &middot; ' + pieces.length + ' pieces';
  html += ' &middot; ' + connections.length + ' connections';
  html += '</div>';

  if (state.capabilities.pieceSelection || state.capabilities.connectionSelection) {
    html += '<div style="font-size:11px;color:var(--accent);margin-bottom:8px">';
    html += 'Selected: ' + (state.selection.pieceGuids || []).length + ' pieces, ';
    html += (state.selection.connectionGuids || []).length + ' connections';
    html += '</div>';
  }

  if (state.capabilities.diff && state.designDiff) {
    const dp = state.designDiff.pieces || {};
    html += '<div style="font-size:11px;margin-bottom:8px">';
    html += '<span style="color:var(--success)">+' + (dp.added ? dp.added.length : 0) + '</span> ';
    html += '<span style="color:var(--destructive)">-' + (dp.removed ? dp.removed.length : 0) + '</span> ';
    html += '<span style="color:var(--warning)">~' + (dp.updated ? dp.updated.length : 0) + '</span>';
    html += '</div>';
  }

  // Piece list
  if (pieces.length > 0 && (state.capabilities.pieceSelection || state.mode === "show-design" || state.mode === "show-diagram")) {
    html += '<div style="margin-bottom:4px;font-size:11px;color:var(--muted)">Pieces</div>';
    for (const p of pieces) {
      const selected = selPieces.has(p.guid);
      const cursor = state.capabilities.pieceSelection ? "pointer" : "default";
      const bg = selected ? "rgba(59,130,246,0.2)" : "transparent";
      const border = selected ? "1px solid var(--accent)" : "1px solid var(--border)";
      html += '<div data-piece-guid="' + escapeHtml(p.guid) + '" ';
      html += 'style="padding:4px 8px;margin:2px 0;border-radius:4px;font-size:12px;';
      html += 'cursor:' + cursor + ';background:' + bg + ';border:' + border + '">';
      html += escapeHtml(p.name || p.guid.slice(0, 8));
      html += '</div>';
    }
  }

  // Connection list
  if (connections.length > 0 && (state.capabilities.connectionSelection || state.mode === "show-design" || state.mode === "show-diagram")) {
    html += '<div style="margin-top:8px;margin-bottom:4px;font-size:11px;color:var(--muted)">Connections</div>';
    for (const c of connections) {
      const selected = selConns.has(c.guid);
      const cursor = state.capabilities.connectionSelection ? "pointer" : "default";
      const bg = selected ? "rgba(59,130,246,0.2)" : "transparent";
      const border = selected ? "1px solid var(--accent)" : "1px solid var(--border)";
      html += '<div data-connection-guid="' + escapeHtml(c.guid) + '" ';
      html += 'style="padding:4px 8px;margin:2px 0;border-radius:4px;font-size:12px;';
      html += 'cursor:' + cursor + ';background:' + bg + ';border:' + border + '">';
      const connDesc = (c.connected && c.connected.piece ? c.connected.piece.guid.slice(0, 6) : "?");
      const conngDesc = (c.connecting && c.connecting.piece ? c.connecting.piece.guid.slice(0, 6) : "?");
      html += escapeHtml(connDesc + " ↔ " + conngDesc);
      html += '</div>';
    }
  }
  ...
  if (state.capabilities.pieceSelection) {
    root.querySelectorAll("[data-piece-guid]").forEach(el => {
      el.addEventListener("click", () => {
        const guid = el.getAttribute("data-piece-guid");
        const current = new Set(state.selection.pieceGuids || []);
        if (current.has(guid)) current.delete(guid);
        else current.add(guid);
        state.selection.pieceGuids = [...current];
        notifySelectionChange();
        render();
      });
    });
  }
  if (state.capabilities.connectionSelection) {
    root.querySelectorAll("[data-connection-guid]").forEach(el => {
      el.addEventListener("click", () => {
        const guid = el.getAttribute("data-connection-guid");
        const current = new Set(state.selection.connectionGuids || []);
        if (current.has(guid)) current.delete(guid);
        else current.add(guid);
        state.selection.connectionGuids = [...current];
        notifySelectionChange();
        render();
      });
    });
...
window.addEventListener("message", (event) => {
  ...
  if (data.kind === "semio:init" || data.kind === "semio:update") {
    if (data.mode) state.mode = data.mode;
    if (data.kit) state.kit = data.kit;
    if (data.designGuid) state.designGuid = data.designGuid;
    if (data.designDiff !== undefined) state.designDiff = data.designDiff;
    if (data.selection) state.selection = data.selection;
    if (data.camera !== undefined) state.camera = data.camera;
    if (data.capabilities) state.capabilities = data.capabilities;
    render();
  }
});

// Signal readiness to parent
window.parent.postMessage({ kind: "semio:ready" }, "*");
</script>
</body>
</html>"""
...
async def app_design_viewer():
    """Serve the sandboxed design viewer app as an iframe-compatible HTML page.
    [👤semio📚engine💻engine🔖rest🔖appendpoints🛠️appdesignviewer](repo://p/u/semio/b/l/engine/f/engine.py/s/Rest/s/App%20Endpoints/d/i/app_design_viewer)
    """
    return fastapi.responses.HTMLResponse(
        content=_APP_HTML_TEMPLATE,
        headers={
            "Content-Security-Policy": "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; frame-ancestors *",
            "X-Frame-Options": "ALLOWALL",
        },
    )


# endregion App Endpoints
