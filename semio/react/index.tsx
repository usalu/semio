...
type DtoKit = InstanceType<typeof KitModel>;
type DtoDesign = InstanceType<typeof DesignModel>;
type DtoPiece = InstanceType<typeof PieceModel>;
type DtoType = InstanceType<typeof TypeModel>;
type DtoConnection = InstanceType<typeof ConnectionModel>;
type DtoQuality = InstanceType<typeof QualityModel>;
type DtoTag = InstanceType<typeof TagModel>;
type DtoConcept = InstanceType<typeof ConceptModel>;
type DtoAuthor = InstanceType<typeof AuthorModel>;
type DtoRepresentation = InstanceType<typeof RepresentationModel>;
...
function __kitHostPlainDtoFromStore(store: KitHostStore): KitFullDto {
  ...
  if (typeof (snapKit as { toJSON?: unknown }).toJSON === "function") return (snapKit as DtoKit).toJSON();
...
export async function executeSemioKitCommand(store: KitHostStore, command: string, _origin: string, ...args: unknown[]): Promise<unknown> {
  ...
  if (command === "semio.kit.addFile" && args[0]) {
    ...
    store.replace(KitModel.fromDto(merged));
