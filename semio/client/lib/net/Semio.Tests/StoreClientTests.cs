...
public sealed class StoreClientTests
{
    public static bool StoreBinaryPresent() =>
        ...
        System.IO.File.Exists(Path.Combine("target", "release", "semio-store.exe")) ||
        System.IO.File.Exists(Path.Combine("target", "release", "semio-store"));

    [Fact]
    public void Semio_Store_Graphql_Entry_And_Wip_Read()
    {
    ...
    [Fact]
    public void Semio_Store_Graphql_Rename_Materialization_Roundtrip()
    {
        if (!StoreBinaryPresent()) return;
    ...
    [Fact]
    public void Semio_Store_Graphql_CreateDesign_And_CreateType_Materialize()
    {
        if (!StoreBinaryPresent()) return;
...
public sealed class StoreKitIOTests
{
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), $"semio-kitio-{Guid.NewGuid():N}");
        Directory.CreateDirectory(dir);
        try
        {
