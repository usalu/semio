...
// #endregion ÔÜø´©ÅImports

// #region ­ƒ¬ØFieldBind
type FieldBindOptions<E, T> = Readonly<{
  /** @emoji ­ƒº▓ Single async read (one GraphQL selection / entity method). */
  read: (entity: E) => Promise<T>;
  /** @emoji ­ƒôí Entity field name ÔåÆ {@code on{Name}Changed} on the anchor (preferred over bus). */
  field?: string;
  /** @emoji ­ƒ¬Ø  source; re-invoked each render ÔÇö keep stable via {@link React#useCallback}. */
  get: () => E | null;
}>;
...
/**
 * @emoji ­ƒ¬Ø Binds one async entity read to React state; optional bus kind narrows refresh fan-in (no `useSyncExternalStore`).
...
// #endregion ­ƒ¬ØFieldBind

// #region ­ƒ¬ØOperationBind
/** @emoji ­ƒÄø´©Å UI-facing operation lifecycle for entity command hooks (idle ÔåÆ pending ÔåÆ settled). */
export type OperationStatus = { readonly kind: "idle" } | { readonly kind: "pending" } | { readonly kind: "settled"; readonly result: SetResult };

/**
 * @emoji ­ƒù║´©Å Maps {@link SetErrorKind#NameTooLong} to a fixed max-length message; otherwise returns {@link SetError#message}.
 * @param maxChars ÔÇö Upper bound communicated to the user (schema limit or UI policy).
 */
...
/** @emoji ­ƒÄø´©Å Idle {@link OperationStatus} for read-only sketchpad rows. */
export const OPERATION_STATUS_IDLE: OperationStatus = Object.freeze({ kind: "idle" });

/** @emoji ­ƒÄø´©Å Shared {@link OperationStatus} plus a map of async command runners for one scoped anchor (entity, store, session, or batch). */
...
/**
 * @emoji ­ƒ¬Ø Builds one React hook per anchor: every command in {@code spec} shares a single {@link OperationStatus}.
 */
...
// #endregion ­ƒ¬ØOperationBind

// #region ­ƒ¬ØKitFieldBind
/** @emoji ­ƒ¬Ø Kit-scoped field bind (uses {@link Store#bus} like {@link semioInternalFieldBind}). */
...
/** @emoji ­ƒ¬Ø Store-root field bind for session/backbone/root fields. */
...
/** @emoji ­ƒ¬ó Batch piece ids for {@link usePiecesCommand} under {@link DesignContext}. */
...
/** @emoji ­ƒùé´©Å Publishes the semio/js session handle (opaque to public hooks). */
...
//#region ­ƒöîKitWasmHostBridge
/** @emoji ­ƒöî Sketchpad registry row: opaque {@link KitHostStore} for the active kit tab. */
...
/** @emoji ­ƒöî Reads {@link KitWasmMountProvider} host bindings (sketchpad registry bridge). */
...
/** @emoji ­ƒöî Publishes registry {@link KitHostStore} for sketchpad kit tabs. */
...
//#endregion ­ƒöîKitWasmHostBridge

//#region ­ƒîÉSemioStoreKitLineHost
/** @emoji ­ƒîÉ Provider stack for one {@link Store} branch under an active {@link Session}. */
...
/** @emoji ­ƒîÉ WASM {@link Session} line: same contexts as {@link SemioStoreKitLineHost} without HTTP bootstrap. */
...
/** @emoji ­ƒîÉ Composes `SessionContextProvider` ÔåÆ `StoreContextProvider` ÔåÆ WIP graph ÔåÆ `TheKit` ÔåÆ `KitContextProvider` for native `semio-store` + {@link openSessionHttp}. */
export type SemioStoreKitLineHostProps = Readonly<
  {
    baseUrl: string;
    children: ReactNode;
    /** @emoji ÔÅ│ Shown until the HTTP session and first store/kit ids resolve. */
    fallback?: ReactNode;
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (stores.length === 0) {
          throw new Error("semio/react: SemioStoreKitLineHost found no stores ÔÇö POST /install first or pass installCreateDto.");
        }
        const sid = stores[0]!.id;
...
//#endregion ­ƒîÉSemioStoreKitLineHost
...
/** @emoji ­ƒÅ¬ Resolves {@link Store} from {@link SessionContextProvider} + id. */
...
/** @emoji ­ƒÅ¬ Active {@link Store} branch (requires {@link StoreContextProvider}). */
export function useJsStore(): Store {
  const s = React.useContext(StoreHandleContext);
  if (s == null) throw new Error("semio/react: useJsStore requires StoreContextProvider id={ÔÇª}.");
  return s;
}
...
/** @emoji ­ƒª┤ Active backbone id from whichever of file / folder / websocket backbone contexts is mounted. */
...
/** @emoji ­ƒº¡ Optional {@link Store} when {@link StoreContextProvider} is mounted above. */
export function useStoreOptional(): Store | null {
  return React.useContext(StoreHandleContext);
}

/** @emoji ­ƒîÉ WIP {@link Graph} from the active graph tier. */
...
/** @emoji ­ƒÅø {@link TheKit} under the active graph tier. */
...
/** @emoji ­ƒôª {@link Kit} from {@link KitContextProvider}. */
...
/** @emoji ­ƒôª Optional {@link Kit} from {@link KitHandleContext} (no throw). */
export function useWipKitOptional(): Kit | null {
  return React.useContext(KitHandleContext);
}

/** @emoji ­ƒîÉ Authoritative graph (bypasses tier marker; prefer {@link AuthoritativeContextProvider}). */
export function useAuthoritativeGraph(): Graph {
  return useJsStore().authoritative();
}

/** @emoji ­ƒîÉ {@link Graph} for {@link GraphContextProvider} (compat). */
...
// #endregion ­ƒ¬¬IdsAndProviders

// #region ­ƒîÉGraphContext
...
/** @emoji ­ƒîÉ Binds {@link GraphRootKind} for {@link useGraph}. */
...
/** @emoji ­ƒºÀ {@link ConnectionContextProvider} using enclosing design scope. */
...
// #endregion ­ƒöûEntityContextHelpers

/** @emoji ­ƒº® Active {@link Design} entity from {@link DesignContext} + {@link StoreHandleContext}. */
...
// #region ­ƒ¬ØIdStableEntityLists
/** @emoji ­ƒôÜ Kit-level designs via {@link Kit#designs} (stable entity handles). */
...
/** @emoji ­ƒôÜ Kit-level kinds via {@link Kit#types}. */
...
/** @emoji ­ƒôÜ Kit-level authors via {@link Kit#authors}. */
...
/** @emoji ­ƒôÜ Kit-level qualities via {@link Kit#qualities}. */
...
/** @emoji ­ƒôÜ Kit-level tags via {@link Kit#tags}. */
...
/** @emoji ­ƒôÜ Kit-level concepts via {@link Kit#concepts}. */
...
/** @emoji ­ƒôÜ Pieces in the active {@link DesignContext} design. */
...
/** @emoji ­ƒôÜ Connections in the active {@link DesignContext} design. */
...
/** @emoji Ô£ì´©Å All {@link Kit} mutations for the active WIP kit ({@link useWipKit}). */
...
/** @emoji Ô£ì´©Å Store-scoped mutations ({@link useJsStore}). */
...
/** @emoji Ô£ì´©Å {@link Session} mutations for the active JS session. */
export function useSessionCommand(): EntityCommand<SemioCommandRunners> {
  const session = useJsSession();
  return useSessionCommandBound(() => session);
}

// #region ­ƒ¬ØBackboneOps

/** @emoji ­ƒø£ Live {@link Session#backboneStatus} for the active store (refreshes on {@code commandSucceeded} bus events). */
...
// #endregion ­ƒ¬ØBackboneOps

// #endregion ­ƒ¬ØHooksKit

// #region ­ƒ¬ØHooksDesign
// #region ­ƒôûDesignReads
/** @emoji ­ƒôû Live {@link Design#name}. */
...
/** @emoji Ô£ì´©Å All {@link Design} mutations; optional {@code id} overrides {@link DesignContext}. */
export function useDesignCommand(id?: ID): EntityCommand<SemioCommandRunners> {
  return useDesignCommandBound(() => resolveDesign(id));
}
// #endregion Ô£ì´©ÅDesignWrites
// #endregion ­ƒ¬ØHooksDesign
...
/** @emoji ­ƒôû Live {@link Type#description}. */
...
/** @emoji ­ƒôû Representation ids for {@link Type#representations}. */
...
/** @emoji ­ƒôû Live {@link Type#attributes}. */
...
/** @emoji ­ƒôû Author ids for {@link Type#authors}. */
...
/** @emoji Ô£ì´©Å All {@link Type} mutations for {@link TypeContext} (optional {@code id} overrides). */
...
/** @emoji Ô£ì´©Å All {@link Port} mutations for {@link PortContext}. */
...
/** @emoji ­ƒôû Live {@link Connector#code}. */
...
/** @emoji ­ƒôû Live {@link Connector#description}. */
...
/** @emoji ­ƒôû Bulky {@link Connector#attributes}. */
...
/** @emoji Ô£ì´©Å All {@link Connector} mutations (optional {@code id} overrides {@link ConnectorContext}). */
...
// #region Ô£ì´©ÅAuthor
/** @emoji ­ƒôû Live {@link Author#name}. */
...
/** @emoji ­ƒôû Live {@link Quality#benchmarks} as ids. */
...
/** @emoji Ô£ì´©Å All {@link Quality} mutations for {@link QualityContext}. */
...
/** @emoji ­ƒôû Live {@link Concept#name}. */
...
/** @emoji ­ƒôû Live {@link Representation#url}. */
...
/** @emoji ­ƒôû Live {@link Representation#description}. */
...
/** @emoji ­ƒôû Tag ids for {@link Representation#tags}. */
...
/** @emoji ­ƒôû Quality ids for {@link Representation#qualities}. */
...
/** @emoji ­ƒôû Live {@link Representation#attributes}. */
...
/** @emoji ­ƒôû Linked file id for {@link Representation#file}. */
...
/** @emoji ­ƒôû Live {@link Piece#flatPosition}. */
...
/** @emoji ­ƒôû Live {@link Piece#plane}. */
...
/** @emoji ­ƒôû Live {@link Piece#center}. */
...
/** @emoji ­ƒôû Live {@link Piece#flatPlane}. */
...
/** @emoji ­ƒôû Live {@link Piece#flatCenter}. */
...
/** @emoji ­ƒôû Live {@link Piece#blueprint}. */
...
/** @emoji ­ƒôû Live {@link Piece#attributes}. */
export function usePieceAttributes(id?: string): readonly Attribute[] | undefined {
  const entity = resolvePiece(id);
  return useCurrentEntityField(entity, (p) => p.attributes(), "attributes");
}

/** @emoji ­ƒôû Live {@link Piece#connectionKind}. */
...
/** @emoji ­ƒôû Parent piece id for {@link Piece#parentPiece}. */
...
/** @emoji ­ƒôû Parent connection id for {@link Piece#parentConnection}. */
...
/** @emoji ­ƒôû Child piece ids for {@link Piece#childPieces}. */
...
/** @emoji ­ƒôû Child connection ids for {@link Piece#childConnections}. */
...
/** @emoji Ô£ì´©Å All {@link Piece} mutations (optional {@code id} overrides {@link PieceContext}). */
...
/** @emoji Ô£ì´©Å {@link PiecesOperation} batch mutations via {@link PiecesBatchContext} + {@link DesignContext}. */
...
/** @emoji ­ƒôû Live {@link Connection#gap}. */
...
/** @emoji ­ƒôè Transaction/diagram diff status labels for sketchpad chrome. */
...
/** @emoji ­ƒôÅ Sketchpad geometric tolerance constant. */
...
/** @emoji ­ƒôî Tab-scoped kit id for sketchpad shell / scene bridge. */
...
/** @emoji ­ƒôî Active sketchpad kit tab id (`{ id }`) or `null` outside {@link ActiveKitTabContextProvider}. */
export function useActiveKitTab(): ActiveKitTabScope | null {
  const id = React.useContext(ActiveKitTabContext);
  return id == null || id === "" ? null : { id };
}

/** @emoji ­ƒôî Whether {@link ActiveKitTabContextProvider} is mounted. */
...
/** @emoji ­ƒî▒ Sketchpad alternative picker; lists {@link Graph#alternatives} when {@link StoreContextProvider} is active. */
...
/** @emoji ­ƒî▒ Selected alternative id + setter for host dropdowns. */
...
/** @emoji ­ƒî▒ Alternatives list for host dropdowns. */
...
/** @emoji ­ƒºá In-memory kit host for sketchpad temporary kits and imports. */
...
/** @emoji ­ƒº¥ DTO host-store command bridge for folder/file UI tests and {@link InMemoryKitStore}; GraphQL kits use {@link useKitCommand}. */
...
//#endregion ­ƒº¥KitHostStore

//#region ­ƒôªKitRegistry
...
/** @emoji ­ƒôú Subscribe to registry open/close list changes. */
...
/** @emoji ­ƒº¥ Registry bridge for {@link SketchpadStore} and other non-hook callers. */
export function getKitRegistryBridge(): KitRegistryValue | null {
  return _semioKitRegistryBridge;
}

/** @emoji ­ƒôª Host registry for open kit tabs (sketchpad). */
...
/** @emoji ­ƒôª Sketchpad host root: {@link KitRegistryProvider}. */
...
/** @emoji ­ƒôª Like {@link useKitRegistry} but returns null outside provider. */
export function useKitRegistrySafe(): KitRegistryValue | null {
  return React.useContext(KitRegistryContext);
}
//#endregion ­ƒôªKitRegistry

//#region ­ƒöîKitRuntimeBridge
function useKitRuntimeSafe(): KitRuntimeContextValue | null {
  return React.useContext(KitRuntimeContext);
}

/** @emoji ­ƒôî Live kit host snapshot for registry tab or {@link KitRuntimeContext}. */
...
/** @emoji ­ƒôï Plain kit JSON from host snapshot or GraphQL bridge (tree, ports, folders). */
export function useKitPlainOptional(): unknown | null {
  return useKitStoreSnapshot()?.kit ?? null;
}

/** @emoji ­ƒöî Flattened kit ports from {@link useKitPlainOptional} + {@link getKitPorts}. */
...
/** @emoji ­ƒöä Active kit host sync lane from {@link useKitStoreSnapshot}. */
export function useKitSyncSnapshot(): KitHostSnap["sync"] {
  return useKitStoreSnapshot()?.sync ?? DEFAULT_KIT_SYNC;
}

/** @emoji ­ƒôÑ Active kit host store for the resolved tab / runtime scope. */
...
/** @emoji ­ƒôé Open kit ids from {@link KitRegistryProvider}. */
...
/** @emoji ­ƒôÜ Kit-level files (WIP: empty until {@link Kit} exposes files). */
export function useKitFiles(): readonly File[] {
  return EMPTY_FILES;
}

/** @emoji ­ƒôî Included design ids for explode / replace UI (WIP: empty until design graph exposes included nodes). */
...
/** @emoji ­ƒº¡ Maps entity id prefix to schema scope kind for sketchpad property panels. */
...
/** @emoji ­ƒöù Attach backbone via {@link useStoreCommand} (alias for sketchpad docs). */
...
//#endregion ­ƒöîKitRuntimeBridge
// #endregion ­ƒÄ¿SketchpadFacade

// #region ­ƒÄø´©ÅWriteIndicator
/** @emoji ­ƒÄø´©Å Maps {@link OperationStatus} into sketchpad row affordances. */
...
if (import.meta.vitest) {
  ...
  const vitestRegion = reactSrc.indexOf("// #region ­ƒº¬Vitest");
  const sketchpadFacadeRegion = reactSrc.indexOf("// #region ­ƒÄ¿SketchpadFacade");
  const scanEnd =
