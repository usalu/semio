...
fn install_k(graph: KitGraphRef) {
    ...
    EVENTS.call_once(move || {
        let _ = thread::Builder::new()
            .name("semio-store-event-log".to_string())
            .spawn(move || {
...
impl InstallBody {
    fn into_graph(self) -> std::result::Result<KitGraphRef, String> {
        ...
        if let Some(p) = self.import_file {
            return KitGraph::load_json_file(Path::new(&p.path))
                .map_err(|e: SemioError| e.to_string());
        }
        if let Some(p) = self.import_from_folder {
            return KitGraph::load_local_folder(Path::new(&p.path))
                .map_err(|e: SemioError| e.to_string());
        }
        ...
        if let Some(r) = self.import_from_remote {
            return KitGraph::load_remote_session(&r.hub_url, &r.session_id)
                .map_err(|e: SemioError| e.to_string());
        }
...
async fn post_graphql(
        ...
        match l.as_ref() {
            None => {
                return (
                    ...
                    "no kit: send POST /install with { \"create\": { \"dto\": { ... } } } first"
                        .to_string(),
                )
                    .into_response();
            }
    ...
    let vcs = GraphQlVcsOverride {
        native: Some(rt.store.clone()),
    };
    let resp = match semio::kit_graphql::execute_with_control_plane(
        &body,
        rt.store.graph().clone(),
        rt.work_tx,
        vcs,
    )
    .await
...
pub async fn run() {
    ...
    let port: u16 = std::env::var("SEMIO_STORE_PORT")
        .ok()
    ...
    let listener: TcpListener = TcpListener::bind(&addr)
        .await
        .unwrap_or_else(|e| panic!("semio-store bind {addr}: {e}"));
