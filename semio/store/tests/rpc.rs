...
fn spawn_server() -> Result<(std::process::Child, u16, String), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let mut child = Command::new(exe).env("SEMIO_STORE_PORT", "0").env("RUST_LOG", "off").stdout(Stdio::piped()).stderr(Stdio::null()).spawn()?;
    let stdout = child.stdout.take().ok_or("no stdout")?;
    let mut br = BufReader::new(stdout);
    let mut line = String::new();
    br.read_line(&mut line)?;
    let j: Value = serde_json::from_str(line.trim())?;
    let port: u16 = j.get("port").and_then(|p| p.as_u64().and_then(|n| u16::try_from(n).ok())).ok_or("port in ready line")?;
