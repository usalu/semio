...
- **Method catalog (high level)**:
  ...
  - **Store**: `kit.snapshot`, `kit.theKitDto`, `kit.execute`, `kit.executeChangeKitCommands` `{ cmds }`, `kit.executeReadKitCommands` `{ cmds }`, `kit.materializeAt`, `kit.vcsState`, `kit.getField` / `kit.setField` / `kit.addChild` / `kit.removeChild`.
