// #region 🔖Header
// [👤semio📚go🥼semiobenchmark](semiorepo://p/u/semio/b/l/go/f/semio_benchmark.go)
...
func loadKit(filename string) Kit {
...
func loadKitDiff(filename string) KitDiff {
	data, err := os.ReadFile(AssetsPath + "/" + filename)
	if err != nil {
		panic(err)
	}
...
func findDesign(kit Kit, name string, parentName string) Design {
...
func main() {
	...
	bench("Roundtrip/Metabolism", func() {
		...
		err = KitToZip(kit, files, "temp_benchmark_metabolism.zip", string(schemaData))
		if err != nil {
			panic(err)
		}
