...
- NEVER use inline styling. Use tailwindcss (v4). v4 uses `elements/ui/theme.css` for theming and not `{theme:{…}}` in `tailwindconfig`.
- ALWAYS use colors defined in `@theme inline {…}` from `elements/ui/globals.css`. NEVER use direct colors such as light, gray, …, dark, primary, secondary, tertiary outside of `elements/ui/globals.css` and ALWAYS use semantic colors instead such as active, disabled, hover, …
...
- NEVER add semantic values and ALWAYS use hardcoded values in `elements/ui/theme.css`. NEVER use `elements/ui/theme.css` outside of `global.css`.
...
- `elements/ui/elements.tsx` defines `TransactionProvider` and `useTransaction()`.
- `elements/ui/elements.tsx` `Geometry` treats `color` as the base (non-interactive) color and uses selection/hover theme colors for the rendered material/edges when `selected`/`hovered` are true.
...
- `elements/ui/elements.tsx` provides `TransactionProvider` and `useTransaction()` for UI-scoped transactions.
...
- `elements/ui/elements.tsx` `Geometry` renders selection/hover colors even when a base `color` is provided (it is treated as the non-interactive default).
...
- `elements/ui/elements.tsx` `Table` exposes row hover callbacks so apps can forward row enter/leave events into their state machine commands.
...
- `layout` naming is reserved for window layout configs (GoldenLayout) and the `Layout` component in `elements/ui/elements.tsx`.
...
All size constants are defined in `elements/ui/globals.css` and derived from `--spacing`:
