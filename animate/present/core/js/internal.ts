...
export function clampTileCrop(crop: FigureTileDraft["crop"]): FigureTileDraft["crop"] {
	const width = Math.max(NORMALIZED_RECT_MIN_FRACTION, crop.width);
	const height = Math.max(NORMALIZED_RECT_MIN_FRACTION, crop.height);
