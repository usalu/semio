...
// Summary: `glassClass` is a generic Tailwind `@utility` class defined in `ui/styling/js/ui.css` (backdrop-filter blur/saturate + a `color-mix` fill); its alpha/blur are derived per the `[data-level="…"]` ancestor the swatch is stamped with — not per a hand-picked tier. A level's attached chrome (title caps, ribbons, tab bars, rails) renders this exact same fill as its body, so one level always shows one appearance. Each swatch renders over a colorful backdrop so the blur/alpha differences across levels are visible without needing WebGL or a running plugin.
