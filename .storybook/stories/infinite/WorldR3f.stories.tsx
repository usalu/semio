...
// Summary: `WorldChunks`/`ViewRadiusLayer` bucket-and-cull their children by an `origin` prop read straight off each child element (see `chunkKey`/`useVisibleChunkKeys` in `infinite/world/r3f/index.tsx`), so a grid of plain `<mesh>` boxes tagged with `origin` is enough to exercise real chunk visibility culling with zero fixtures/WASM. `WorldLodGridHelper`/`GridLayer` need the `useLod()` context that only `WorldLodBridge` provides — wired here exactly as `World3dHost` wires it.
...
import {
  ...
  type Vec3,
} from "../../../infinite/world/r3f/index.tsx";
