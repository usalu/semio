...
// #region 🧲️Header
// 💻️ .storybook/stories/fem/scene.ts
...
import { EMPTY_FEM2D_SNAPSHOT, EMPTY_FEM3D_SNAPSHOT, parseFem2dDsl, parseFem3dDsl, type Fem2dSnapshot, type Fem3dSnapshot, type FemVec2 } from "./dsl";

import fem2dDemoDsl from "../../../✏️s/🔌️plugins/🏗️fem/🗿️artifacts/◻️2d/🏅️standards/🔖️1/🪆️subsets/🌐️any/📚️examples/🎬️demo/🖼️assets/🗣️.dsl.semio?raw";
import fem3dDemoDsl from "../../../✏️s/🔌️plugins/🏗️fem/🗿️artifacts/🧊️3d/🏅️standards/🔖️1/🪆️subsets/🌐️any/📚️examples/🎬️demo/🖼️assets/🗣️.dsl.semio?raw";

//#region 🔖️Locale
/** @emoji 🗣️ The two BCP-47 tags these stories carry labels for. There is no default: every story sets `locale` explicitly, and fem2d's own `setLocale` tool (`✏️editor/🎮️commands/🗣️set-locale`) writes the very same tag into `Fem2dConfig::locale`. */
...
/** @emoji 🎬️ fem3d ships the same `"demo"` `ExampleSource` id, but its `setActiveExample` handler only recognizes the literal `"default"` (`✏️editor/🎮️commands/📚️set-active-example/🦀️.rs`) — every other id resets to an empty document. Both ids are exposed so a story can show either branch. */
export const FEM3D_STORY_EXAMPLE_ID = "default";
export const FEM3D_STORY_SHIPPED_EXAMPLE_ID = "demo";
...
export type FemStoryCamera2d = { readonly x: number; readonly y: number; readonly zoom: number };

/** @emoji 🎥️ `FemCamera::default()` for fem2d (`🗿️artifacts/◻️2d/🦀️.rs`). */
export const FEM2D_DEFAULT_CAMERA: FemStoryCamera2d = { x: 0, y: 0, zoom: 1 };
...
export function buildFem2dSceneNode(layers: readonly FemLayer[], camera: FemStoryCamera2d, bodyKey: string, controllerId: string): UiComponentSceneNode {
  return {
    type: "componentScene",
    surfaceId: bodyKey,
    controllerId,
...
/** @emoji 🎥️ `world3d_default_camera()` — what `fem3d_camera_json` falls back to while `FemCamera::json` is still the `"{}"` sentinel. */
export const FEM3D_DEFAULT_CAMERA_JSON = JSON.stringify({ position: [4, -4, 3], target: [0, 0, 0], up: [0, 0, 1], fov: 45 });
...
export function buildFem3dSceneNode(instances: readonly Record<string, unknown>[], cameraJson: string, bodyKey: string, controllerId: string): UiComponentSceneNode {
  return {
    type: "componentScene",
    surfaceId: bodyKey,
    controllerId,
...
/** @emoji 👁️ `Fem2dConfig`/`Fem3dConfig`'s flat result-display triple, translated to `crate::app_surface::ResultDisplay` at the render boundary. */
...
export type Fem2dStoryState = {
  ...
  readonly camera: FemStoryCamera2d;
...
/**
 * @emoji 🧩️ Story-local mirror of fem2d's `Fem2dCommand` dispatch for the five tools these stories exercise
 * (`setActiveExample`, `addNode`, `setCamera`, `setResultDisplay`, `setLocale` — see the
 * `bounded_first_step_tool_proofs!` roster in `✏️editor/🦀️.rs`). An unknown action is ignored, exactly as
 * the real dispatcher rejects a tool id outside `FEM2D_RETAINED_TOOL_IDS`.
 *
 * 🕳️ `setActiveExample` resets the WHOLE config — camera, result display AND locale back to `"en-US"` —
 * because its handler emits `Fem2dConfigMutation::Snapshot { config: Fem2dConfig::default() }`; switching
 * example therefore drops a German session back to English, which this reducer reproduces rather than hides.
 */
export function reduceFem2dStoryAction(state: Fem2dStoryState, action: string, rawArgs: unknown): Fem2dStoryState {
  ...
  switch (action) {
    case "setActiveExample": {
      ...
      return exampleId === undefined ? state : fem2dStoryStateFor(exampleId, "en-US");
    }
    case "addNode": {
    ...
    case "setCamera": {
      const camera = args.camera as FemStoryCamera2d | undefined;
      return camera === undefined ? state : { ...state, camera };
    }
    ...
    case "setLocale": {
      ...
      return value === undefined || !FEM_STORY_LOCALES.includes(value as FemStoryLocale) ? state : { ...state, locale: value as FemStoryLocale };
    }
    default:
      return state;
  }
...
/** @emoji 🎬️ The state a fem3d session starts in for one example id — only the literal `"default"` loads the bundled fixture (`✏️editor/🎮️commands/📚️set-active-example/🦀️.rs`). */
export function fem3dStoryStateFor(exampleId: string, locale: FemStoryLocale): Fem3dStoryState {
  return {
    ...
    cameraJson: FEM3D_DEFAULT_CAMERA_JSON,
    resultDisplay: FEM_DEFAULT_RESULT_DISPLAY,
  };
}

/**
 * @emoji 🧩️ Story-local mirror of fem3d's `Fem3dCommand` dispatch for the four tools these stories exercise
 * (`setActiveExample`, `addNode`, `setCamera`, `setResultDisplay`). fem3d's roster carries NO `setLocale`
 * (fem2d's does), so the 3D stories' language switch is story-level only — it dispatches nothing.
 *
 * 🕳️ `setActiveExample` resets only the two granular config fields the handler emits (`SetResultDisplay`,
 * `SetCamera`); fem3d's `Fem3dConfigPreparationFactory` refuses a whole-config `Snapshot` row.
 */
export function reduceFem3dStoryAction(state: Fem3dStoryState, action: string, rawArgs: unknown): Fem3dStoryState {
  ...
  switch (action) {
    case "setActiveExample": {
      const exampleId = typeof args.exampleId === "string" ? args.exampleId : undefined;
      return exampleId === undefined ? state : fem3dStoryStateFor(exampleId, state.locale);
    }
    case "addNode": {
      const x = typeof args.x === "number" ? args.x : 0;
      const y = typeof args.y === "number" ? args.y : 0;
    ...
    case "setCamera": {
      ...
      return camera === undefined ? state : { ...state, cameraJson: typeof camera === "string" ? camera : JSON.stringify(camera) };
    }
    case "setResultDisplay": {
      return {
        ...state,
