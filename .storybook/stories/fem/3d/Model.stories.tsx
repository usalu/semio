// #region 🧲️Header
// 💻️ .storybook/stories/fem/3d/Model.stories.tsx
...
function Fem3dModelStoryHost({ initialExampleId, locale }: { readonly initialExampleId: string; readonly locale: FemStoryLocale }): ReactElement {
  ...
  const node = useMemo(() => buildFem3dSceneNode(instances, state.cameraJson, FEM3D_MODEL_BODY_KEY, FEM3D_EDITOR_CONTROLLER_ID), [instances, state.cameraJson]);
  const debug = useMemo(
  ...
  return (
    <div style={{ display: "flex", height: "100%", width: "100%", minHeight: "24rem", flexDirection: "column" }}>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 4, padding: 4 }}>
        ...
        <button type="button" data-testid="fem3d-load-shipped-id" onClick={() => dispatch("setActiveExample", { exampleId: FEM3D_STORY_SHIPPED_EXAMPLE_ID })}>
          {femStoryLabel("example", state.locale)}: {FEM3D_STORY_SHIPPED_EXAMPLE_ID}
        </button>
...
/** 🧊️ The bundled fixture under the id `setActiveExample` actually recognizes (`"default"`): 16 nodes plus 16 oriented HEA200 frame prisms. */
export const DefaultExampleEnglish: Story = {
  args: { initialExampleId: FEM3D_STORY_EXAMPLE_ID, locale: "en-US" },
};

/** 🇩🇪️ The same scene with German panel labels. fem3d's tool roster has no `setLocale` (fem2d's does), so this is a story-level selection only. */
export const DefaultExampleGerman: Story = {
  args: { initialExampleId: FEM3D_STORY_EXAMPLE_ID, locale: "de-DE" },
};

/** 🐞️ The id the subset's own `ExampleSource` ships (`"demo"`) — `setActiveExample` only recognizes `"default"`, so this branch resets to an EMPTY document and the world paints nothing. */
