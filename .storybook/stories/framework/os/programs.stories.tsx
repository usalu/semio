// #region 🧲Header
// 💻 .storybook/stories/framework/os/programs.stories.tsx
// Specs: One story per `PROGRAM_BUILD_TARGETS` entry (`framework/program/registry/generated/programs.ts`) — full plugin-matrix coverage for the `framework/os` scope.
// Summary: Reuses `OsBootHost` (`.storybook/framework/os/index.tsx`) per program id. Storybook's Vite-based CSF indexer statically parses export *declarations* only — verified empirically (boot the scope, diff `/index.json` story ids against `PROGRAM_BUILD_TARGETS.length`) that a generated `export const { A, B, ... } = Object.fromEntries(...)` destructuring produces zero indexed stories (the indexer never evaluates the module, so a destructuring target list isn't recognized as CSF story exports the way literal `export const X = {...}` declarators are) — so each program gets its own literal `export const <PascalId>: Story = { args: { plugin: "<programId>" } }` below, generated once from the registry and pasted as static source rather than computed at module-eval time. `assertPluginMatrixCoverage` (dev-time only) still iterates `PROGRAM_BUILD_TARGETS` to catch drift between the registry and this literal list.
...
// #region 🔖PluginMatrix
// 🧩 One literal export per `PROGRAM_BUILD_TARGETS` row — CSF3's Vite indexer needs a statically
// visible `export const X = {...}` declarator per story, so these can't be generated by a runtime
// loop (see header note); `toPascalCase(programId)` names each export.
...
/** @emoji 🔤 `programId` (kebab-case, e.g. `"playbook-module-procedural"`) → the PascalCase export name used above. */
function toPascalCase(programId: string): string {
  return programId
    .split("-")
...
function assertPluginMatrixCoverage(): void {
  for (const target of PROGRAM_BUILD_TARGETS) {
    const exportName = toPascalCase(target.programId);
    const story = EXPORTED_STORIES[exportName];
    if (!story) throw new Error(`programs.stories.tsx: missing literal export "${exportName}" for registry programId ${JSON.stringify(target.programId)}`);
    if (story.args?.plugin !== target.programId) {
      throw new Error(`programs.stories.tsx: export "${exportName}" args.plugin ${JSON.stringify(story.args?.plugin)} !== registry programId ${JSON.stringify(target.programId)}`);
    }
  }
}
