...
// Summary: Args-driven `plugin` select (populated from `PLUGIN_BUILD_TARGETS`) + `appId` text control via `OsBootHost` (`.storybook/framework/os/index.tsx`). Serves prebuilt WASM from `/plugin-modules`; a missing artifact renders an instruction panel instead of blocking on cargo.
...
/** @emoji 🖥️ Studio host plugin — `PLUGIN_HOST_CONFIGS` (`framework/core/js/index.ts`) resolves `"s"` to studio mode. */
export const Studio: Story = {
  args: { plugin: "s" } satisfies OsBootHostProps,
};

/** @emoji 🧩 Puzzle plugin, no `appId` override — lands on its manifest's first app. */
