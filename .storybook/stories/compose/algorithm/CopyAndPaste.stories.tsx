...
const TargetDesignWindow: React.FC = () => {
  ...
  return <ComposeDiagram design={(flatPasteTargetDesign ?? pasteTargetDesign) as Design} diffEnabled={false} zoomTarget="design" selectionEnabled={false} panEnabled={true} zoomEnabled={true} />;
};
...
const PasteAnchoringWindow: React.FC = () => {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-2 overflow-auto border-border/40 border-t p-3">
      <label className="text-xs font-medium text-muted-foreground" htmlFor="compose-algorithms-paste-anchoring">
        Paste anchoring
      </label>
      <select
        id="compose-algorithms-paste-anchoring"
