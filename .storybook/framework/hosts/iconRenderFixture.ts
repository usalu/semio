...
// Summary: `IconRenderRequest.assetUrl` is normally a served GLB path; `framework/hosts` registers no static-dir asset route (unlike `framework/os`'s `/program-modules`), so this hand-builds a minimal valid glTF 2.0 JSON document (a two-triangle quad, embedded `POSITION`/indices buffer as its own `data:` URI) and base64-encodes the whole document as a second `data:` URI at call time — `GLTFLoader`'s `FileLoader` fetches both via the browser's native `data:` URL support (see `LoaderUtils.resolveURL`, which passes `data:` URIs through unresolved), so the real default `iconRenderPort` (three.js GLTFLoader + WebGL/SVG renderer, `ui/js/react/index.tsx`) renders it with zero network/backend dependency.
// 2026 Ueli Saluz <ueli@semio-tech.com>
// #endregion 🧲Header

//#region QuadBuffer
/** 🔺 Two-triangle unit quad: 4×vec3 float32 positions (48 bytes) + 6×uint16 indices (12 bytes), precomputed once (`node -e` Float32Array/Uint16Array → base64) since the bytes never change. */
...
//#region GltfDocument
/** 🧩 Minimal valid glTF 2.0 document three.js's `GLTFLoader` can parse standalone (no `.bin` sibling — the buffer is itself a `data:` URI). */
