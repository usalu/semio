...
/** @emoji 🧪 Navigates to one story's iframe and asserts it actually rendered (no missing-story/preview-failure text, no page/console errors) — the shared "loads cleanly" assertion every story id below gets. */
...
test("TableHost empty-scene: renders the empty-scene fallback with zero console errors", async ({ page }) => {
  await expectStoryLoads(page, "🛠️framework🔌hosts-tablehost--empty-scene");
...
test("BlockListHost editable: loads with two steps, Add Step round-trips a third", async ({ page }) => {
  await expectStoryLoads(page, "🛠️framework🔌hosts-blocklisthost--editable");
...
test("BlockListHost empty-scene: renders the empty-scene fallback with zero console errors", async ({ page }) => {
  await expectStoryLoads(page, "🛠️framework🔌hosts-blocklisthost--empty-scene");
...
test("GraphTimelineHost branching: clicking a row dispatches checkoutCheckpoint", async ({ page }) => {
  await expectStoryLoads(page, "🛠️framework🔌hosts-graphtimelinehost--branching");
...
test("GraphTimelineHost linear/empty: both load with zero console errors", async ({ page }) => {
  await expectStoryLoads(page, "🛠️framework🔌hosts-graphtimelinehost--linear");
  await page.goto(`iframe.html?id=🛠️framework🔌hosts-graphtimelinehost--empty&viewMode=story`);
...
test("IconRenderHost toolbar-format/svg-fixed/png-fixed: all load and render a preview with zero console errors", async ({ page }) => {
  await expectStoryLoads(page, "🛠️framework🔌hosts-iconrenderhost--toolbar-format");
  await expect(page.getByText(/^format: (png|svg)$/)).toBeVisible();

  await expectStoryLoads(page, "🛠️framework🔌hosts-iconrenderhost--svg-fixed");
  await expect(page.getByText("format: svg")).toBeVisible();

  await expectStoryLoads(page, "🛠️framework🔌hosts-iconrenderhost--png-fixed");
...
test("InkCanvasHost editable: clicking the text block selects it", async ({ page }) => {
  await expectStoryLoads(page, "🛠️framework🔌hosts-inkcanvashost--editable");
...
test("InkCanvasHost navigator-preview: loads read-only with zero console errors", async ({ page }) => {
  await expectStoryLoads(page, "🛠️framework🔌hosts-inkcanvashost--navigator-preview");
});
//#endregion InkCanvasHost

//#region Canvas2dHost
test("Canvas2dHost scene: wheel-zoom round-trips the camera into the debug readout", async ({ page }) => {
  await expectStoryLoads(page, "🛠️framework🔌hosts-canvas2dhost--scene");
...
test("Canvas2dHost empty-canvas: loads with zero layers and zero console errors", async ({ page }) => {
  await expectStoryLoads(page, "🛠️framework🔌hosts-canvas2dhost--empty-canvas");
...
test("UiInterpreter button: clicking the button dispatches addItem", async ({ page }) => {
  await expectStoryLoads(page, "🛠️framework🔌hosts-uiinterpreter--button");
...
test("UiInterpreter tree: toggling the Visible control dispatches toggleVisible", async ({ page }) => {
  await expectStoryLoads(page, "🛠️framework🔌hosts-uiinterpreter--tree");
...
test("UiInterpreter panel: renders the field/section/slider/keyValue shape with zero console errors", async ({ page }) => {
  await expectStoryLoads(page, "🛠️framework🔌hosts-uiinterpreter--panel");
