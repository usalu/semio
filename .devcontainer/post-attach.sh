...
#endregion 🔖️GitKrakenCliHelpers
...
configure_gitkraken_workspace() {
  ...
  if [ -n "$has_workspace" ]; then
    ...
    if [ "${#missing_repos[@]}" -gt 0 ]; then
      ...
      echo "✅️ GitKraken workspace updated: $GITKRAKEN_WORKSPACE_NAME"
    else
      echo "✅️ GitKraken workspace already current: $GITKRAKEN_WORKSPACE_NAME"
    fi
  else
    repo_csv="$(IFS=,; printf '%s' "${gitkraken_repos[*]}")"
    gk ws create "$GITKRAKEN_WORKSPACE_NAME" --add-repos "$repo_csv" >/dev/null 2>&1 || true
    created="1"
    echo "✅️ GitKraken workspace created: $GITKRAKEN_WORKSPACE_NAME"
  fi
  ...
  echo "✅️ GitKraken default workspace set: $GITKRAKEN_WORKSPACE_NAME"
}
#endregion 🔖️GitKrakenWorkspace

if [ -n "$SKIP_TOOL_INSTALL" ]; then
  echo "ℹ  Tool installation skipped."
else
  install_gitkraken_desktop
  install_gitkraken_cli
fi
...
#endregion 🔖️RepoConfigure

#region 🔖️InstallExtension
if [ "${#IDE_CLIS[@]}" -gt 0 ]; then
  ...
  echo "ℹ  No IDE CLIs detected, skipping extension installation."
fi
#endregion 🔖️InstallExtension


echo "✅️ Post-attach setup complete."
#endregion 🔖️PostAttach
