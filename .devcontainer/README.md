...
**Native (Windows / macOS / Linux):** Create a Neo4j Desktop **Local Instance** named **`semio`**, set password **`password`**, and start it on Bolt port **`7687`**. On DBMS editions that support multiple user databases, also create **`elements`**, **`coda`**, **`reuse`**, and **`metabolism`** so the matching MCP servers (`neo4j-*` in `.mcp.json`; graph name argv is one or more tokens joined with `-`, e.g. `… neo4j metabolism` → Bolt database `metabolism`) and `bun run generate` APOC exports can run against each graph. On **Neo4j Community**, only **one** standard user graph exists per DBMS, so names like `metabolism` do **not** appear as extra databases; use **Enterprise** (or equivalent) for multiple isolated graphs, or point every MCP entry at your single graph name. Native setup enables APOC on the local DBMS when needed, uses the native Neo4j Desktop/DBMS only, does not depend on the devcontainer, and does not edit Desktop internals.
