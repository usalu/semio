//! 🔧 Procedural program — 2D and 3D flow apps in one hot-swappable WASM program.

pub mod app_2d {
    //! 🎲 Procedural 2D program — procedural flow play app bundled as a hot-swappable WASM program.
    ...
    /// 🎯 `semio_framework_program::selection_ids`'s "ids" array plus a singular "nodeIds" fallback —
    ...
    fn procedural2d_labels(view_state: &ViewState) -> &'static Procedural2dLabels {
        semio_framework_program::resolve_labels::<Procedural2dLabels>(view_state)
    }
    ...
    fn procedural2d_action_labels(is_de: bool) -> std::collections::HashMap<String, String> {
        ...
        semio_framework_program::localized_label_map(is_de, ENTRIES)
    }
    ...
    impl DocumentApp for Procedural2dPlayApp {
        ...
        fn app_labels(&self, view_state: &ViewState) -> semio_framework_program::AppLabelsOverlay {
            let labels = procedural2d_labels(view_state);
            let is_de = semio_framework_program::is_de_locale(view_state);
            semio_framework_program::AppLabelsOverlay::default()
                ...
                .example_labels(semio_framework_program::localized_label_map(is_de, &[("default", "Default", "Standard")]))
        }
    }
    ...
    pub fn register_procedural2d_exports() {
        ...
        semio_framework_program::program_runtime::register_document_codec_for_app::<Procedural2dPlayApp>(PROCEDURAL_2D_SCHEMA);
    }
    //#endregion 🔖Manifest
    ...
    mod tests {
        ...
        use semio_framework_program::{ActionMeta, ProgramApp, VcsDocumentApp};
...
pub mod app_3d {
    //! 🧱 Procedural 3D program — flow-based procedural brep editor bundled as a hot-swappable WASM program.
    ...
    /// program dispatch, so a fresh `FlowHost::from_fixture` would otherwise discard per-node
    ...
    fn export_mesh_from_document(projection: &Procedural3dDocument) -> semio_framework_program::MeshData {
    ...
    fn procedural3d_labels(view_state: &ViewState) -> &'static Procedural3dLabels {
        semio_framework_program::resolve_labels::<Procedural3dLabels>(view_state)
    }
    ...
    impl DocumentApp for Procedural3dPlayApp {
        ...
        fn handle_action(
            ...
            match action {
                ...
                // the program worker runs this, never the renderer. Chains itself via `DispatchAction`
        ...
        fn app_labels(&self, view_state: &ViewState) -> semio_framework_program::AppLabelsOverlay {
            ...
            semio_framework_program::AppLabelsOverlay::default()
                ...
                .example_labels(semio_framework_program::localized_label_map(is_de, &[
    ...
    /// 🎯 `semio_framework_program::selection_ids`'s "ids" array plus a singular "nodeId" fallback —
    /// this app's actions accept either shape depending on the caller.
    fn selection_ids(args: Option<&Value>) -> Vec<String> {
        let ids = semio_framework_program::selection_ids(args);
        if !ids.is_empty() {
    ...
    fn procedural3d_mesh_from_document(doc: &serde_json::Value) -> Result<semio_framework_program::MeshData, String> {
    ...
    fn procedural3d_document_from_mesh(_mesh: &semio_framework_program::MeshData) -> Result<Value, String> {
        serde_json::to_value(default_projection()).map_err(|err| err.to_string())
    }

    pub fn register_procedural3d_exports() {
        semio_framework_os::register_mesh_exporter("3d.procedural", "procedural", procedural3d_mesh_from_document, Box::new(semio_framework_program::ObjExporter));
        semio_framework_os::register_mesh_exporter("3d.procedural", "procedural", procedural3d_mesh_from_document, Box::new(semio_framework_program::GlbExporter));
        semio_framework_os::register_mesh_exporter("3d.procedural", "procedural", procedural3d_mesh_from_document, Box::new(semio_framework_program::StlExporter));
        semio_framework_os::register_mesh_dwg_export_handler("3d.procedural", "procedural", procedural3d_mesh_from_document);
        semio_framework_os::register_mesh_importer("3d.procedural", procedural3d_document_from_mesh, Box::new(semio_framework_program::ObjImporter));
        semio_framework_os::register_mesh_importer("3d.procedural", procedural3d_document_from_mesh, Box::new(semio_framework_program::GlbImporter));
        semio_framework_os::register_mesh_importer("3d.procedural", procedural3d_document_from_mesh, Box::new(semio_framework_program::StlImporter));
        ...
        semio_framework_program::program_runtime::register_document_codec_for_app::<Procedural3dPlayApp>(PROCEDURAL_3D_SCHEMA);
    }
    //#endregion 🔖Manifest
    ...
    mod tests {
        ...
        fn document_from_mesh_returns_valid_default_projection() {
            let mesh = semio_framework_program::MeshData::default();
