#!/usr/bin/env bun
/** 🧭 `@semio-tech/procedural-3d-play` task router: `bun ./script.ts <dev|build|test> [fixture <id>] [args…]`. */
...
class DevScript extends BundleScript {
	run(segments: string[]): void {
		...
		runViteBunxDev(this.root, viteSegments, {
			portEnv: playgroundPortEnv("procedural-3d"),
			defaultPort: playgroundDevPortString("procedural-3d"),
			fixedPort: true,
		});
	}
}
