...
- **Multiple attachable/detachable backbones at runtime**, notably:
  ...
  - **Local backbone**: a folder containing `.semio/` with `wip.db`, `staged.db`, `authoritative.db`, `conflicts.db`, and `blobs/BLOBHASH.EXT` content-addressed blobs.
- **`semio/rs` is the in-memory source of truth** for kit graphs while attached; UI reads/writes through **`semio/js` → `semio/react`**, with **`useSyncExternalStore` only for kit state** and **granular invalidation** aligned to RS events (no React-wide blind refetch when a narrower projection updates).
- IPC/schema boundary remains **`semio/graphql/schema.graphql`** (Relay-style extension with owners/children/hashes as already established in the schema direction).
...
- Ensure **`semio/graphql` covers snapshots, diffs, connections, operations, and backbone lifecycle** end-to-end as the primary “done” signal.
- Ensure **`semio/js` exposes RS capabilities as conventional OO APIs** using RxJS internally, **without duplicating kit caching/state ownership** in JS.
- Ensure **`semio/react` composes only `semio/js`**, exporting **`[STATE, SETSTATE, STATUS] = use*(…)`** hooks for settable surfaces, and **`useSyncExternalStore`** kit subscriptions that update **only when the subscribed projection changes**.
- Deliver **`semio/sketchpad` as the integration acceptance surface** demonstrating checkpoints/drafts/transactions + backbone attach/detach.
...
**Rust (`semio/rs`) when Rust code or GraphQL export wiring changes:**
- ```bash
cargo test --manifest-path semio/rs/Cargo.toml
cargo clippy --manifest-path semio/rs/Cargo.toml --all-targets -- -D warnings
```

**GraphQL schema drift gate when the GraphQL contract changes:**
- ```bash
pnpm exec nx build semio/graphql
```

**Embedded/unit tests when behavior changes in TS layers (run when the story touches that package):**
- ```bash
pnpm --filter @semio/js test
pnpm --filter @semio/react test
```

**Sketchpad build gate when sketchpad changes:**
- ```bash
pnpm --filter @semio/sketchpad build
```
...
As an integrator, I want `semio/graphql/schema.graphql` to fully specify the kit store surface (Relay-style patterns already present: owners/children/hashes) so `semio/rs` can implement a single GraphQL target without ad hoc gaps.
...
- [ ] Schema includes backbone operations needed for this epic: **attach**, **detach**, and **capability discovery** sufficient for dev JSON vs local `.semio/` backends (without exposing backbone-internal SQL details to clients).

### US-003: Implement `semio/rs` Core Kit Graph Engine (Async, Pointer-Internal, Multi-State)

As a library author, I want `semio/rs` to represent kit graphs internally with **stable pointers/handles** (not ID-string resolution as the primary internal mechanism) and to allow **reads on any readable state** and **writes on any writable state**, so hosts can implement complex parallel editing workflows safely.
...
### US-004: Implement Attachable Backbones — Dev JSON And Local `.semio/` Multi-DB + Blobs
...
- [ ] **Local backbone** persists under `.semio/` using separate **SQLite** databases: `wip.db`, `staged.db`, `authoritative.db`, `conflicts.db`, plus `blobs/BLOBHASH.EXT` for referenced payloads.
...
### US-005: Implement GraphQL Target In `semio/rs` Matching `semio/graphql/schema.graphql`

As a host integrator, I want `semio/rs` to implement the GraphQL target directly so all IPC semantics remain in schema, not bespoke channels.

**Acceptance Criteria:**
- [ ] `async-graphql` schema/types implement the updated `semio/graphql/schema.graphql` surface required by US-002 for this epic’s scope.
...
- [ ] GraphQL export/build wiring remains consistent with `pnpm exec nx build semio/graphql`.

### US-006: Rebuild `semio/js` Facade Over `semio/rs` Using RxJS Without JS-Layer Kit Caching

As an app developer, I want `semio/js` to expose clean OO APIs that hide RS’s internal architecture while preserving correct async behavior.
...
- [ ] Extend existing embedded tests in `semio/js` (do not add new test files) to cover new facade behaviors related to subscriptions/progress/error channels as applicable.

### US-007: Rebuild `semio/react` Hooks Over `semio/js` With `useSyncExternalStore` Kit Subscriptions And Tuple Hooks

As a UI developer, I want React bindings that avoid over-fetching and match the repo’s hook conventions.

**Acceptance Criteria:**
- [ ] Hooks consume **only** `semio/js` (no direct RS imports from React code).
- [ ] Settable hooks follow **`[STATE, SETSTATE, STATUS] = use*(…)`** naming/shape where applicable.
- [ ] Kit state uses **`useSyncExternalStore`** wired to **subscribe + snapshot** callbacks provided by `semio/js`.
...
- [ ] Extend existing embedded tests in `semio/react` (do not add new test files).

### US-008: Sketchpad Integration Demonstrating End-To-End Store Workflows

As a reviewer, I want `semio/sketchpad` to prove checkpoints/drafts/transactions workflows plus backbone attach/detach against real GraphQL/Relay-shaped usage patterns.
...
- [ ] Sketchpad demonstrates switching between **dev JSON backbone** and **local `.semio/` backbone** (attach/detach), without violating layering rules.
...
- **FR-4:** `semio/rs` must support **multiple readable states** concurrently and **writes targeting explicitly selected writable states**.
...
- **FR-7:** Local backbone must create/use `.semio/` containing **`wip.db`**, **`staged.db`**, **`authoritative.db`**, **`conflicts.db`**, and **`blobs/BLOBHASH.EXT`**.
- **FR-8:** IPC must remain **`semio/graphql/schema.graphql`**-based; no parallel IPC channel is introduced for kit store functionality.
- **FR-9:** `semio/js` must not cache kit graphs independently of `semio/rs` ownership rules; caching belongs in RS’s computed/cache layers only.
- **FR-10:** `semio/react` must use **`useSyncExternalStore`** for kit-derived UI state subscription boundaries aligned with RS granular invalidation signals.
...
- Supporting multiple technologies mixing (**elements**, **coda**, **semio**) beyond existing repo separation rules.
...
- **`async-graphql` dependency already exists in `semio/rs`**; GraphQL implementation should align with its async execution model.
