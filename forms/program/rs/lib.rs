//! 📋 Forms program — declarative forms play app bundled as a hot-swappable WASM program.
...
fn find_question_kind_contribution<'a>(
    ...
    contributions.iter().find_map(|entry| {
        if let Contribution::PlaybookBlockKind { block_kind, .. } = &entry.contribution {
            if block_kind == kind {
                return Some((entry.program_id.as_str(), &entry.contribution));
            }
        }
        None
    })
...
fn render_extension_question(
    ...
    let Some((program_id, contribution)) = find_question_kind_contribution(contributions, &question.kind) else {
    ...
    ui_stack_vertical(vec![
        ui_external_slot(program_id, app_id, params_body_key, &payload),
        ui_external_slot(program_id, app_id, preview_body_key, &payload),
    ])
}
...
fn register_forms_exports() {
    semio_framework_program::program_runtime::register_document_codec_for_app::<FormsPlayApp>(FORMS_DOCUMENT_SCHEMA);
}
...
mod tests {
    ...
    fn building_component_contributions() -> Vec<ProgramContributionEntry> {
        vec![ProgramContributionEntry {
            program_id: "forms-module-procedural".into(),
