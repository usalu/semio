...
## Changes
- Refactored [dev.ts](/workspaces/semio/semio/js/dev.ts) into a real launcher with explicit `storybook` and workspace modes instead of top-level side effects.
...
- Routed [package.json](/workspaces/semio/semio/js/package.json) `dev` and `dev:storybook` through the new launcher while preserving the actual Storybook CLI command as `dev:storybook:inner`.
- Extended [semio.test.ts](/workspaces/semio/semio/js/semio.test.ts) to cover the new Storybook launch classification logic and wrapper argument parsing.
...
- Confirmed port 6006 was already listening and found active `storybook` and `nx dev:storybook semio/js` processes in the workspace, which explained the silent exit.
...
- Verified `npm run test --workspace @semio/js -- semio.test.ts` passed with 14 tests after the launcher refactor.
