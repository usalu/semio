...
(async () => {
  ...
  await page.evaluate(() => {
    window.__PHASES__ = [];
    window.__MUT_OBS__ = new MutationObserver(ms => {
      const p = window.__PHASES__[window.__PHASES__.length - 1];
      for (const m of ms) {
        if (m.type === 'childList') {
          p.added += m.addedNodes.length;
          p.removed += m.removedNodes.length;
          for (const n of m.addedNodes) {
            if (n.nodeType === 1) {
              const tag = n.tagName?.toLowerCase();
              if (!p.addedTags[tag]) p.addedTags[tag] = 0;
              p.addedTags[tag]++;
            }
          }
          for (const n of m.removedNodes) {
            if (n.nodeType === 1) {
              const tag = n.tagName?.toLowerCase();
              if (!p.removedTags[tag]) p.removedTags[tag] = 0;
              p.removedTags[tag]++;
            }
          }
        } else {
  ...
  function startPhase(name) {
    return page.evaluate(n => {
      window.__PHASES__.push({ name: n, added: 0, removed: 0, attrs: 0, addedTags: {}, removedTags: {} });
      window.__MUT_OBS__.observe(document.body, { childList: true, attributes: true, subtree: true });
    }, name);
  }
  function endPhase() {
    return page.evaluate(() => { window.__MUT_OBS__.disconnect(); });
  }

  // FRESH DRAG (no prior selection) - this is the FIRST interaction
  await startPhase('mousedown');
  await page.mouse.move(sx, sy); await sleep(50);
  await page.mouse.down(); await sleep(300);
  await endPhase();
  
  await startPhase('mousemove');
  await page.mouse.move(sx + 100, sy, { steps: 1 }); await sleep(300);
  await endPhase();
  
  await startPhase('mouseup');
  await page.mouse.up(); await sleep(5000);
  await endPhase();

  const phases = await page.evaluate(() => window.__PHASES__);
  console.log('\n=== MUTATION PHASES ===');
  for (const p of phases) {
    console.log(`  ${p.name}: added=${p.added}(${JSON.stringify(p.addedTags)}), removed=${p.removed}(${JSON.stringify(p.removedTags)}), attrs=${p.attrs}`);
  }
