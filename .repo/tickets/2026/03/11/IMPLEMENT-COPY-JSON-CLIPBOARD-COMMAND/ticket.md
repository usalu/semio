...
Adjusted clipboard behavior to copy active kit JSON by default, removed design payload override from diagram Cmd/Ctrl+C, and updated existing clipboard e2e assertions to validate kit payload shape. TSC passes; focused Playwright remains blocked by existing @semio/assets named-export runtime mismatch.

## Changes

- Reopened ticket to align clipboard behavior with requirement: copy kit JSON.
- Updated `semio/js/sketchpad/Sketchpad.tsx`:
- Changed `semio.sketchpad.copyJsonToClipboard` default serialization from `dumpState()` to active kit snapshot resolution from current navigation (`/kits/:guid`), with first-kit fallback when no active kit path exists.
...
- Updated `semio/js/sketchpad/Sketchpad.tsx`:
- Added `semio.sketchpad.copyJsonToClipboard` execution handling in `executeCommand`.
...
- Added a global hotkey binding using `useHotkeys("semio.sketchpad.navbar.copyJsonToClipboard", ...)`.
- Registered `semio.sketchpad.copyJsonToClipboard` in exported `commands`.
- Updated `semio/js/sketchpad/locales/en.json`:
- Added `semio.sketchpad.navbar.copyJsonToClipboard` label + hotkey (`Ctrl+Shift+J`).
- Updated `semio/js/sketchpad/locales/de.json`:
- Added German translation for `semio.sketchpad.navbar.copyJsonToClipboard` label + hotkey.
- Updated `semio/js/sketchpad.test.ts`:
...
- Reopened ticket and updated `semio/js/sketchpad/Design.tsx`:
...
- Updated `semio/js/sketchpad.test.ts` clipboard test:
...
- `cd semio/js && npm run test:e2e -- --grep "Copy Json To Clipboard Command"` failed before test execution with existing `@semio/assets` named export interop error (`CodeIcon`).
...
- `SyntaxError: Named export 'CodeIcon' not found` from `@semio/assets` CommonJS interop in Playwright runtime.

## Todos

- Re-run `npm run test:e2e -- --grep "Copy Json To Clipboard Command"` after resolving the pre-existing `@semio/assets` named export runtime mismatch.
- Resolve the existing `@semio/assets` ESM/CJS named export mismatch in the Playwright test runtime.
- Re-run `npm run test:e2e -- --grep "Copy Json To Clipboard Command"` after the import issue is fixed.

## Plan

- Make `semio.sketchpad.copyJsonToClipboard` default to active kit JSON.
