...
## Changes
- Removed `intersect` and freeform `lasso` toggles from Design selection settings in `semio/js/sketchpad/Design.tsx`.
- Removed `intersect` and freeform `lasso` toggles from Kit selection settings in `semio/js/sketchpad/Kit.tsx`.
- Removed `intersect` toggle from Type selection settings and removed Type lasso toolbar group registration in `semio/js/sketchpad/Type.tsx`.
- Removed `intersect` toggle from Quality selection settings in `semio/js/sketchpad/Quality.tsx`.
- Updated Playwright assertions in `semio/js/sketchpad.test.ts` to verify removed controls are absent and retained controls still render.
...
- Executed `npm run test:e2e -- sketchpad.test.ts` in `semio/js`; result: `7 passed`.
...
- [x] Update Playwright coverage in `semio/js/sketchpad.test.ts`.
...
3. Update `semio/js/sketchpad.test.ts` to assert these controls are absent and preserve existing checks for retained controls.
