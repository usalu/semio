"""🎬 Cooling full series with synced German subtitle VO (1080p60).

Pipeline:
  1. synth   — per-clause TTS + vo_timing.json in every Cooling part
  2. render  — HQ section re-render with VO_TRACE=1 (holds use measured TTS)
  3. align   — place each clause on its traced subtitle start inside the section
  4. mux     — embed section audio, then concat intro + sections

Temporary orchestrator for ticket MUX-COOLING-VO-SYNC.
"""
...
def step_render_trace() -> None:
    ...
    # Clear stale series trace so this run owns the marks.
    series_trace = COOLING / "vo_trace.json"
    if series_trace.is_file():
        series_trace.unlink()
    code = _run(
        [str(ROOT / ".venv" / "bin" / "python"), str(COOLING / "full_cooling_video.py"), "-q", "h", "--no-play", "--force"],
        env=env,
    )
    if code != 0:
...
def _merge_traces() -> dict:
    """📍 Prefer series dump next to full_cooling_video; also merge per-module traces."""
    merged: dict = {}
...
def step_align_mux_concat() -> Path:
    ...
    (TICKET / "vo_trace_merged.json").write_text(
        __import__("json").dumps({"beats": trace}, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    ...
    final = COOLING / "rendered" / "Full_Cooling_Demand_1080p60.mp4"
    final_audio = COOLING / "rendered" / "Full_Cooling_Demand_WithAudio_1080p60.mp4"
    cmd = [
        "ffmpeg", "-y", "-f", "concat", "-safe", "0",
        "-i", str(list_path), "-c", "copy", str(final_audio),
    ]
    proc = subprocess.run(cmd, capture_output=True, text=True, check=False)
    if proc.returncode != 0:
        raise RuntimeError(proc.stderr[-800:])
    shutil.copy2(final_audio, final)
    _log(f"\n✅ Final → {final}")
    _log(f"   copy → {final_audio}")
    _log(f"   duration {probe_duration(final):.1f}s")
    return final
