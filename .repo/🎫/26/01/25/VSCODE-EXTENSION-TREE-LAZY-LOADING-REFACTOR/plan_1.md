...
└── Bundles (fetch bundles query)
    └── Bundle expanded → fetch folder(path: bundle.root)
        └── Folder expanded → fetch folder(path: folder.path)
            └── File expanded → fetch file(path: file.path) for sections/definitions
