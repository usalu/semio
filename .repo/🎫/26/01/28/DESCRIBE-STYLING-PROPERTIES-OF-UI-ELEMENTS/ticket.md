...
This element matches the Radix Avatar fallback in [js/compose/sketchpad/elements.tsx](js/compose/sketchpad/elements.tsx#L790) (it renders `AvatarPrimitive.Fallback`, which is a `span`), composed as:

- Base classes from `AvatarFallback`: `bg-muted flex size-full items-center justify-center rounded-full`
- Additional classes passed by callers (example: `TableAvatar` adds `text-xs` and may add the active colors) in [js/compose/sketchpad/elements.tsx](js/compose/sketchpad/elements.tsx#L858-L875)
...
- Kit diagram edge component: [js/compose/sketchpad/Kit.tsx](js/compose/sketchpad/Kit.tsx#L5085-L5130)
- Design diagram edge component: [js/compose/sketchpad/Design.tsx](js/compose/sketchpad/Design.tsx#L4915-L4963)

Edge color is primarily driven by inline style props passed to `BaseEdge`:

- Kit: `stroke` is set to `var(--foreground)` / `var(--accent-secondary)` / `var(--active-base)` etc. in [js/compose/sketchpad/Kit.tsx](js/compose/sketchpad/Kit.tsx#L5085-L5125)
- Design: `stroke` is set similarly based on diff/selection in [js/compose/sketchpad/Design.tsx](js/compose/sketchpad/Design.tsx#L4915-L4960)
