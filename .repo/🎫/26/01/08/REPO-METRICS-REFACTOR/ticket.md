...
2. **Go Repository** (`./repo/cli/cli.go`):
...
3. **SQLite Schema** (`sql/sqlite/repo/schema.sql`):
   - Already has views for metrics: `repo_metrics`, `bundle_metrics_view`, `folder_metrics_view`, `file_metrics_view`, `section_metrics_view`, `contributor_metrics_view`, `ticket_metrics_view`, `breach_priority_counts`

4. **Tests** (`./repo/cli/cli_test.go`):
...
func TestNodesAndEdges(t *testing.T) {
    ...
    // Query degree-one edges (e.g., file.folder, section.file, breach.kind)
}
```
...
2. **Removed metrics from repo.go**
   ...
   - Added `Breach.kind` resolver to fix type mismatch (was returning Statute enum, now returns \*StatuteMeta)

3. **Updated gqlgen.yml**
   - Removed metrics type bindings

4. **Added/Fixed GraphQL Tests** (`./repo/cli/cli_test.go`)
...
3. **Missing resolvers identified:**
   ...
   - `Statute.policy`, `Statute.breachs` - no resolvers
   - `Contributor.commits` - schema has it but resolver may be incomplete

**Test Results (short mode):**

- PASS: TestTicketsNonEmpty, TestPoliciesNonEmpty, TestStatutesNonEmpty
- PASS: TestFoldersNonEmpty, TestFilesNonEmpty, TestBreachsNonEmpty
...
1. **Fixed codegen.ts schema path**
   - Changed from `../../repo/cli/schema.graphql` to `../../graphql/repo/schema.graphql`
...
2. **Fixed Go Backend (`./repo/cli/cli.go`):**
...
4. **Verification:**
   ...
   - **Tests**: `go test ./...` in `./repo/cli` passes.
...
1. **Metric Removal**: Removed all \*Metrics types and metrics fields from graphql/repo/schema.graphql and ./repo/cli/cli.go to simplify the schema and resolve query errors in the CLI. SQLite views remain as the source of truth for metrics.
2. **Schema Fixes**: Updated Range and Position types in graphql/repo/schema.graphql and ./repo/cli/cli.go:
...
- **Tests**: go test ./... in ./repo/cli passes all tests, encompassing the schema changes.
