...
Consolidate repo binary into ./repo/cli/main.go with clean engine-only architecture.
...
- Integrated benchmark, preflight, update, and github sources into ./repo/cli/main.go; repaired update command block, added missing imports, removed duplicate GitHub helpers, and cleaned inline comments.
...
1. Integrate remaining ./repo/cli/\*.go sources into ./repo/cli/main.go using the integrate command. (done)
2. Reconcile duplicated logic and delete legacy ./repo/cli sources and internal packages. (in progress)
