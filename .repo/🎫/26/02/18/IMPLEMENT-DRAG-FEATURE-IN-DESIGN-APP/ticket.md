...
## Changes
- `compose/js/sketchpad/shared.ts`: Fixed `createPathObserver` to use serialized JSON comparison; added `DerivedNode.jsonReplacer` for Map/Set-aware serialization
- `compose/js/sketchpad/Sketchpad.tsx`: Added `jsonReplacerMapSet` helper; fixed `useDerived` getSnapshot; removed debug logs
- `compose/js/sketchpad/Design.tsx`: Removed all [DEBUG] console.warn logs from drag handlers, command execution, and node computation
- `compose/js/sketchpad.test.ts`: Refactored drag test to use direct store assertions instead of console warning parsing; updated Kit test to expect intersect/lasso toggles (intentionally added by prior ticket)
...
4. Extend `compose/js/sketchpad.test.ts` Design test to validate actual workbench-to-diagram drag creates a piece.
5. Run Playwright sketchpad test file and record results.

## Reopen TODOs
- [x] Analyze old and current drag/drop architecture.
- [x] Implement drag-end client point handling in `compose/js/sketchpad/Design.tsx` to support pointer and mouse activator events for diagram/scene drops.
- [x] Extend Design test block in `compose/js/sketchpad.test.ts` to verify workbench-to-diagram piece creation (native drag attempt + deterministic drag-end fallback).
...
## Reopen Validation
- Command: `cd compose/js && npx playwright test sketchpad.test.ts --grep "sketchpad.*Design" --timeout 240000 --workers=1 --max-failures=1 --reporter=list`
...
- [x] Extend/refactor existing Design Playwright test in `compose/js/sketchpad.test.ts`.
...
- `compose/js/sketchpad/Design.tsx`
  ...
  - Added drag source marker propagation from workbench type/design avatars and global marker fallback (`__COMPOSE_MANUAL_DRAGGED__`).
...
- [x] Implement runtime fix in `compose/js/sketchpad/Design.tsx` (payload fallback from draggable id and activator position fallback).
- [x] Adjust existing Design test selector to explicit type drag avatars in `compose/js/sketchpad.test.ts`.
- [x] Run verification command and capture result.
- [x] Finalize and close ticket.
