...
- Refactored `compose.designApp.updatePiece` handler to normalize both legacy and object payload forms into `{ piece: { guid }, diff }`.
- Refactored `compose.designApp.updatePieces` handler to normalize mixed update entries (`piece`, `id`, `guid`) into `pieces.updated` entries with canonical `piece.guid`.
...
- `cd compose/js && npx playwright test sketchpad.test.ts --grep "Design" --timeout 240000 --workers=1 --max-failures=1 --reporter=line`
