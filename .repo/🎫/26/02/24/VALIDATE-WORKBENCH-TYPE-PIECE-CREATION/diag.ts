...
async function main() {
  ...
  const allIds = await page.evaluate(() =>
    Array.from(document.querySelectorAll("[id]"))
      .map((el) => el.id)
      .filter((id) => id.includes("diagram") || id.includes("workbench") || id.includes("scene"))
      .slice(0, 20),
  );
  ...
  const allDesignIds = await page.evaluate(() =>
    Array.from(document.querySelectorAll("[id]"))
      .map((el) => el.id)
      .filter((id) => id.includes("diagram") || id.includes("workbench") || id.includes("scene") || id === "root")
      .slice(0, 30),
  );
