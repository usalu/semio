...
Added two new policies to repo CLI: (1) Folder policy with BreachFolderIllegalEmpty (autofixable, removes empty folders) and (2) File policy with BreachFileIllegalUseGodfile (checks files against .repo/files.json). Both policies only run for ScopeRepo, skip excluded dirs (.git, .repo, node_modules, .venv, .nx), and have 8 passing tests. Also created .repo/files.json godfile with 4670 tracked files, added autofix routing in Fix function for folder-level breachs, and fixed a pre-existing build error with interactionResourceType.

## Changes

- repo/cli/main.go: Add BreachFolderIllegalEmpty and BreachFileIllegalUseGodfile statutes
- repo/cli/main.go: Add folder and file policy definitions
- repo/cli/main.go: Implement folderPolicy and filePolicy checker functions
- repo/cli/main.go: Add autofix for empty folders in applySystemAutofixes
- .repo/files.json: Create godfile listing all allowed files
- repo/cli/main_test.go: Add tests for both new policies
...
- [x] Create .repo/files.json godfile with 4670 tracked files
...
4. Implement filePolicy (reads .repo/files.json, checks all tracked files against it)
5. Add folder empty autofix in applySystemAutofixes (os.Remove empty folder)
6. Create .repo/files.json with all current repo files
