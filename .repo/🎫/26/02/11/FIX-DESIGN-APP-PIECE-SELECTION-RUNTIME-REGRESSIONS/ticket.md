...
- [x] Extend `compose/js/sketchpad.test.ts` with additive multi-piece selection coverage in the existing Design flow
...
## Validation
- `cd compose/js && npx playwright test sketchpad.test.ts --grep "Design" --timeout 240000 --workers=1 --max-failures=1 --reporter=list` ⚠️
