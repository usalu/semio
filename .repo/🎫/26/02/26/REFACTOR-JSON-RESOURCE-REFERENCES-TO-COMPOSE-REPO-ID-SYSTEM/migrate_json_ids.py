...
def build_contributor_lookup(compose_repo_dir: str) -> dict:
    ...
    lookup = {}
    contrib_dir = os.path.join(compose_repo_dir, "🧑‍💻")
...
def main():
    ...
    ticket_files = glob.glob(
        os.path.join(compose_repo_dir, "🎫", "**", "ticket.json"), recursive=True
    )
    ...
    goal_files = glob.glob(
        os.path.join(compose_repo_dir, "🎯", "**", "goal.json"), recursive=True
    )
