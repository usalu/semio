...
- **Primary bundle/file:** `repo/cli/` — specifically `repo/cli/main.go` (policies, `headerPolicy`, `sectionPolicy`, `ScanComments`, `BaseLanguage`, language plugins, autofix logic)
...
- **Secondary:** `repo/vscode/` (extension install), `~/.codeium/windsurf/mcp_config.json`
...
- **Primary bundle/file:** `repo/cli/main.go` (section parsing, `LanguagePlugin`)
...
- **Primary bundle/file:** `repo/cli/main.go` (`moveCommand`, `integrateCommand`, `extractCommand`, `ToolFileMove`, `ToolFolderMove`, `ToolSectionMove`, `ToolIntegrate`, `ToolExtract`)
...
- **Primary bundle/file:** `repo/cli/main.go` (`BuildMonorepoTree`, `FilterMonorepoTree`, `SearchMonorepoTree`, `RenderMonorepoTree`, `treeCommand`)
...
- **Primary bundle/file:** `repo/cli/main.go` (ticket lifecycle), `.repo/tickets/` (storage)
- **Secondary:** `repo/vscode/extension.ts` (ticket UI), `repo/server/main.go` (server persistence)
- **Implementation-specific language:** MODERATE — references storage format (`.repo/tickets`), `ticket.md`, `important.md`, `CONTINUE`/`NOTICKET` keywords. These are spec-level naming conventions, not code identifiers.
...
- **Primary bundle/file:** `repo/server/main.go`
...
- **Primary bundle/file:** `repo/cli/main.go` and `repo/cli/cli.go` (CLI binary consolidation), `repo/vscode/extension.ts` (VS Code tooling)
- **Secondary:** `graphql/repo/schema.graphql` (GraphQL types), `sql/sqlite/repo/schema.sql` (export)
- **Implementation-specific language:** YES — heavily references: `YYYY/MM/DD/SLUG` path identifiers, `--all` flag, GraphQL field names (`TicketClient`, `TicketDate`, `Interaction`, `Position`), `repo/cli/cli.go` file path, `.repo/reports/codebase.json`, `.repo/` storage, `--md` flag, `--text` flag, JSONL event stream, usalu project 2. Mix of behavioral requirements and implementation details.

### Artifact Kind Derivation (lines 303–316)

- **Primary bundle/file:** `repo/cli/main.go` (`DeriveBundleKind`, `DeriveFolderKind`, `DeriveFileKind`, `DeriveDefinitionKind`, `FileHeaderId`)
...
- **Primary bundle/file:** `repo/cli/main.go` (MCP handler functions)
- **Implementation-specific language:** MINIMAL — generic validation requirements. No code identifiers.

### Contributor (lines 322–328)

- **Primary bundle/file:** `repo/cli/main.go` (contributor derivation), `.repo/contributors/` (storage)
...
- **Primary bundle/file:** `repo/vscode/extension.ts` (ticket close output), `repo/cli/main.go` (CLI rendering)
- **Implementation-specific language:** MINIMAL

### CLI (lines 540–546)

- **Primary bundle/file:** `repo/cli/main.go` (`sanitizeProp`, `sanitizeSingleLine`, `renderEntityMarkdownLink`, `renderEntityHuman`)
...
- **Primary bundle/file:** `repo/vscode/extension.ts`, `repo/vscode/extension.test.ts`
...
| -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| **repo/cli/** (Go binary)                          | Code Hygiene, Sections, Move/Integrate/Extract, Tree, Tooling (partial), Ticket (partial), Goal (partial), Repo Tooling, Artifact Kind Derivation, MCP Tools, Contributor, CLI (UI/UX) | HIGH           |
...
- **Goal** (lines 228–242): References `.repo/goals/SLUG/goal.json` — storage format
