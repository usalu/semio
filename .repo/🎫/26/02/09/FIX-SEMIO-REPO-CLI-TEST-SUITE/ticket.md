...
- `repo/cli/main.go`: Implemented `Node` resolver to correctly identify and route entities based on Emoji prefixes (stripping variation selectors) and fallback to legacy `type:` prefixes.
- `repo/cli/main_test.go`: Removed invalid `foobar123` field from `TestNodesAndEdgesQuick` query.
