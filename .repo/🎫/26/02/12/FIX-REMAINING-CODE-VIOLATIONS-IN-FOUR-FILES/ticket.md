...
- **compose/assets/index.ts**: Added MUST keyword to Exports section summary; removed 8 comment lines before non-definition imports/re-exports
- **compose/assets/grasshopper/build.py**: Removed shebang; wrapped all code after header in `# region 🔖Build` section with MUST summary; removed 4 comment lines before non-definition variable assignments; fixed file ID emoji (📜→💻)
...
1. **compose/assets/index.ts** (9 inline comments):
...
2. **compose/assets/grasshopper/build.py** (8 orphan defs + 4 inline comments):
