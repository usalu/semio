...
async function openDetails(page) {
  ...
  const kitGuid = await page.evaluate(async () => {
    ...
    const kitModule = await import('/@fs/workspaces/semio/semio/assets/semio/kit_metabolism.json');
    await store.execute('semio.sketchpad.createKit', 'semio.sketchpad.test.ensureMetabolismKitLoaded', kitModule.default, false, false);
  ...
  await page.evaluate((nextKitGuid) => {
    window.__SEMIO_NAVIGATE__(`/kits/${nextKitGuid}`);
  }, kitGuid);
  ...
  await page.evaluate(({ nextKitGuid, nextDesignGuid }) => {
    window.__SEMIO_NAVIGATE__(`/kits/${nextKitGuid}/designs/${nextDesignGuid}`);
