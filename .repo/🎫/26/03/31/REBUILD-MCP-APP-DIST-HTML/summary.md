...
Rebuilt `compose/engine/dist/mcp-app.html` by running `npm run build:mcp-app` in `compose/engine/`.

The previous build was from 2026-03-30 and lacked all recent McpDesignViewer routing logic:
- `show-design` → `ComposeDesign` split view (3D scene + 2D diagram)
- `show-scene` → `ComposeScene` 3D only
- `mcpFlattenDesignForComposeSurface`
...
## Files
- `compose/engine/dist/mcp-app.html` (rebuilt)
