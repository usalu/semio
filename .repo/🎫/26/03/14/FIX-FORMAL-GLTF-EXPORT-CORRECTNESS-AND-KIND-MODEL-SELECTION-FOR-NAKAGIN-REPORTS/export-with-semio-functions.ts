...
// #region 🔖Header
// Export Nakagin Capsule Tower Design Model using Semio Functions
// 
// This script extracts the Nakagin Capsule Tower design from the Metabolism kit
// and exports it as a proper semio Model using the serializeModel function.
// #endregion 🔖Header
...
// #region 🔖Main
// Main export logic using semio Model schema
...
/**
 * Convert design to semio Model format
 * This creates a proper Model structure that matches the semio Model schema
 */
function designToSemioModel(design: any): any {
  return {
    guid: design.guid,
    name: design.name,
...
/**
 * Simple model serialization (mimicking semio serializeModel)
 */
...
function exportModelToAssets(model: any, outputPath: string): void {
  try {
    ...
    console.log(`✅ Semio Model exported to: ${outputPath}`);
 ...
 */
function main(): void {
  console.log('🚀 Starting Nakagin Capsule Tower design model export with semio functions...');
  ...
  // Convert to semio Model format
  const model = designToSemioModel(design);
  
  // Define output path
  const outputPath = '/workspaces/semio/semio/assets/models/nakagin-capsule-tower-semio.json';
