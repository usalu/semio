"""Transform compose.py: remove sqlmodel/SQLAlchemy ORM, use pure Pydantic.

This script reads compose.py and applies systematic transformations:
...
def fix_compose_table(content: str) -> str:
    """Fix the Compose metadata table class to be a simple marker."""
    # The Compose class was: class Compose(sqlmodel.SQLModel, table=True)
    # After transform it becomes: class Compose(pydantic.BaseModel)
    # We need it as a simple dataclass or remove it
    content = content.replace(
        'class Compose(pydantic.BaseModel):\n    """Metadata table',
        'class Compose(pydantic.BaseModel):\n    """Metadata marker'
    )
    return content
...
def main():
    ...
    content = fix_compose_table(content)

    write_file(COMPOSE_PY, content)
    print("[DEBUG] Done writing transformed compose.py")
