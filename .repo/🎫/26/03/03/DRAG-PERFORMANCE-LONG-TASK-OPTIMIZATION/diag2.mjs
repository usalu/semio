...
if (isVis) { await tableRow.dblclick({ force: true }); } else { await metText.dblclick({ force: true }); }
await page.waitForURL(/.*kits\/.+/, { timeout: 30000 });
await page.waitForLoadState("networkidle");
await page.waitForTimeout(5000);
// Now navigate to design
const designRowIds = await page.evaluate(() => Array.from(document.querySelectorAll("[data-row-id]")).map(el => el.getAttribute("data-row-id")).filter(id => id?.startsWith("design-")).slice(0, 5));
...
if (designRowIds.length > 0) {
  ...
  await page.evaluate(rid => { const row = document.querySelector(`[data-row-id="${rid}"]`); if (row) { row.dispatchEvent(new MouseEvent("dblclick", { bubbles: true, cancelable: true, view: window })); } }, rowId);
}
...
const cx = paneBox.x + paneBox.width / 2, cy = paneBox.y + paneBox.height / 2;
...
for (const lt of sortedByDur) { console.log(`  start: ${lt.startTime.toFixed(1)}ms, dur: ${lt.duration.toFixed(1)}ms`); }
await browser.close();
