...
(async () => {
  ...
  // Test 1: Click on empty pane (no node)
  await page.evaluate(() => {
    window.__MUT_LOG__ = { added: 0, removed: 0, attrs: 0 };
    window.__MO__ = new MutationObserver(ms => {
      for (const m of ms) {
        if (m.type === 'childList') { window.__MUT_LOG__.added += m.addedNodes.length; window.__MUT_LOG__.removed += m.removedNodes.length; }
        else window.__MUT_LOG__.attrs++;
      }
    });
  });

  // Click on pane (not a node)
  console.log('\n--- Test 1: Click on empty pane ---');
  await page.evaluate(() => { window.__MUT_LOG__ = { added: 0, removed: 0, attrs: 0 }; window.__MO__.observe(document.body, { childList: true, attributes: true, subtree: true }); });
  await page.mouse.click(10, 10);
  await sleep(5000);
  await page.evaluate(() => { window.__MO__.disconnect(); });
  const paneResult = await page.evaluate(() => window.__MUT_LOG__);
  console.log(`  added=${paneResult.added}, removed=${paneResult.removed}, attrs=${paneResult.attrs}`);
  
  let lt = await page.evaluate(() => {
    const s = window.__COMPOSE_PERFORMANCE__;
    const tasks = s.longTasks.sort((a,b) => b.duration - a.duration).slice(0,3);
    s.longTasks = [];
    return { count: tasks.length, max: tasks[0]?.duration || 0 };
  });
  console.log(`  long tasks: ${lt.count}, max=${lt.max.toFixed(0)}ms`);

  // Test 2: Click on a node (no drag)
  console.log('\n--- Test 2: Click on node (no drag) ---');
  await page.evaluate(() => { window.__MUT_LOG__ = { added: 0, removed: 0, attrs: 0 }; window.__MO__.observe(document.body, { childList: true, attributes: true, subtree: true }); });
  await page.mouse.click(sx, sy);
  await sleep(5000);
  await page.evaluate(() => { window.__MO__.disconnect(); });
  const clickResult = await page.evaluate(() => window.__MUT_LOG__);
  console.log(`  added=${clickResult.added}, removed=${clickResult.removed}, attrs=${clickResult.attrs}`);

  lt = await page.evaluate(() => {
    const s = window.__COMPOSE_PERFORMANCE__;
    const tasks = s.longTasks.sort((a,b) => b.duration - a.duration).slice(0,3);
    s.longTasks = [];
    return { count: tasks.length, max: tasks[0]?.duration || 0 };
  ...
  // Test 3: mousedown + mouseup on node (no move)
  console.log('\n--- Test 3: mousedown + mouseup on node (no move) ---');
  await page.evaluate(() => { window.__MUT_LOG__ = { added: 0, removed: 0, attrs: 0 }; window.__MO__.observe(document.body, { childList: true, attributes: true, subtree: true }); });
  ...
  await sleep(5000);
  await page.evaluate(() => { window.__MO__.disconnect(); });
  const downupResult = await page.evaluate(() => window.__MUT_LOG__);
  console.log(`  added=${downupResult.added}, removed=${downupResult.removed}, attrs=${downupResult.attrs}`);
  ...
  // Test 4: drag (mousedown + move + mouseup)
  console.log('\n--- Test 4: drag (mousedown + move + mouseup) ---');
  await page.evaluate(() => { window.__MUT_LOG__ = { added: 0, removed: 0, attrs: 0 }; window.__MO__.observe(document.body, { childList: true, attributes: true, subtree: true }); });
  await page.mouse.move(sx, sy);
  await sleep(50);
  ...
  await sleep(5000);
  await page.evaluate(() => { window.__MO__.disconnect(); });
  const dragResult = await page.evaluate(() => window.__MUT_LOG__);
  console.log(`  added=${dragResult.added}, removed=${dragResult.removed}, attrs=${dragResult.attrs}`);
