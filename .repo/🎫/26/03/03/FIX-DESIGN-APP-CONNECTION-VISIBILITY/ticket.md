...
- Strengthened the existing Design filter assertions in `compose/js/sketchpad.test.ts` to verify real React Flow edge paths via `.react-flow__edge-path` path data before filter toggles hide connections.
- Removed the regressed `CustomDesignEdgeLayer` override from `compose/js/sketchpad/Design.tsx` after it reappeared and restored built-in edge rendering with `edges={edges}`.
- Tightened `compose/js/sketchpad.test.ts` filter coverage to inspect actual rendered edge path nodes with non-transparent strokes and log their path metrics.

## Log

- Inspected repo context with `repo tree`, `ticket list`, and the existing sketchpad source/tests.
- Isolated the rendering path to `compose/js/sketchpad/Design.tsx` `ConnectorHandle`.
...
- The same grep run still stops on an unrelated pre-existing failure in `sketchpad › Design Utility Tabs Stay Removed` (`compose/js/sketchpad.test.ts:5996` expecting navbar utility toggles that are absent).
...
- Traced the line rendering to `CustomDesignEdgeLayer` in `compose/js/sketchpad/Design.tsx`.
...
- The same focused run still ends on an unrelated existing failure in `sketchpad › Design Drag Performance` (`compose/js/sketchpad.test.ts:6036`) because the long-task budget remains far above target.
...
- [x] Extend `compose/js/sketchpad.test.ts` to verify actual custom connector handles are visible, not just generic handle count.
