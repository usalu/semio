...
`/Users/ueli/Documents/semio/.repo/🎫/26/07/27/PACK-BINARY-DOCUMENT-LAYER-ACROSS-ALL-APPS/contract.md`
...
`rust-version = "1.88"`; `[lints] workspace = true`; code in `//#region 🔖Name` /
`//#endregion 🔖Name` blocks; every doc comment starts with an emoji (`/// 🎞️ ...` or
`//! 🎞️ ...` — use 🎞️ film-strip as this family's emoji, distinct from pack's 📦); tests inline
in `//#region 🧪Tests` at the bottom of `lib.rs`, `mod tests { ... mod quick { } mod long { }
...
`🔖TextFormat` + `🔖OpsHeaderGrammar`) FIRST for the exact `OpsHeaderLine` grammar you are
...
- `protocol_command`/`protocol_causal`/`protocol_crdt`/`protocol_wire` must each add inline
  `//#region 🧪Tests` covering: OpText round-trip on a hand-built dummy `(P, Op)` pair;
...
- `protocol_testkit` (this ticket's original scope) additionally gets law fns:
  ...
  `assert_crdt_idempotent`, `assert_wire_frame_round_trip` — same file, new `//#region 🔖Laws`
...
- Write scratch/progress notes only inside this ticket folder
  (`.repo/🎫/26/07/27/PROTOCOL-BINARY-OP-LOG-LAYER/`), as `.txt` files (gitignored `*.log` is
