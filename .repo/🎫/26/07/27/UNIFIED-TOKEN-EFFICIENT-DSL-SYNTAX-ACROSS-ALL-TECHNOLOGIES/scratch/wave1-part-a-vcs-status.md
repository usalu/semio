...
- `vcs/rs/lib.rs`:
  ...
  - `🔖TextFormat` region: replaced the hand-rolled `StructuralLine`/`parse_structural_line`/
    ...
    `optional_field` machinery with a new `🔖OpsHeaderGrammar` sub-region: `struct OpsAuthor`
  ...
  - `🧪Tests` region (`🔖TextFormatHelpers` subregion): rewrote every test that exercised a deleted
