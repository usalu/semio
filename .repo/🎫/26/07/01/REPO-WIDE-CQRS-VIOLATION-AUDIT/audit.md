...
The Trinity Jack CQRS gate plan claimed *"every other CQRS-migrated technology already follows the rule correctly."* That claim is **false**. Trinity was the worst offender (Jack + host direct graph writes with dead VCS stubs), but many technologies still violate the rule in different ways.
...
3. **Play TS** may mirror projection into a ledger, but only **after** semantic ops are computed — not as a blind `setDocument` blob that *is* the mutation.
