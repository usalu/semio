...
def _(m):
    return f"🌎/🔨/directory/⚡️/🦀/{m.group('sub')}"
...
def _(m):
    return f"✏️/🔌/{m.group('p')}/🛂/🗿/⚡️/🦀"

@rule(r'^s/plugin/(?P<p>[^/]+)/extension/(?P<e>[^/]+)/rs$')
def _(m):
    return f"✏️/🔌/{m.group('p')}/🧩/{m.group('e')}/⚡️/🦀"

@rule(r'^s/plugin/(?P<p>[^/]+)/module/(?P<sub>.+)/rs$')
def _(m):
    return f"✏️/🔌/{m.group('p')}/🔨/{m.group('sub')}/⚡️/🦀"

@rule(r'^s/plugin/(?P<p>[^/]+)/app/(?P<a>[^/]+)/(?P<slot>engine|dsl|op|pack|protocol|ui)/rs$')
def _(m):
    return f"✏️/🔌/{m.group('p')}/🎛️/{m.group('a')}/🔨/{m.group('slot')}/⚡️/🦀"

@rule(r'^s/plugin/(?P<p>[^/]+)/app/(?P<a>[^/]+)/rs$')
def _(m):
    return f"✏️/🔌/{m.group('p')}/🎛️/{m.group('a')}/⚡️/🦀"
...
with open('.repo/🎫/26/07/29/MOVE-ALL-APPS-INTO-THE-S-PRODUCT-TREE-WITH-CONSTITUTIONAL-CRATES-EMOJI-LAYOUT/emoji-moves.json', 'w') as f:
