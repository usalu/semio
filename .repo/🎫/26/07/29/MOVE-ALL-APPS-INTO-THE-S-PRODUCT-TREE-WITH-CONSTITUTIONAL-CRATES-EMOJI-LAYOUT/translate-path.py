...
def _(m):
    return f"🧰/🛍️/🦑/🔨/client/mcp/⚡️/🐹/{m.group(1)}"

@rule(r'^repo/client/(cli|sqlite|vscode)/js/(.+)$')
def _(m):
    return f"🧰/🛍️/🦑/🔨/client/{m.group(1)}/⚡️/🟦/{m.group(2)}"
...
def _(m):
    return f"🧰/🛍️/🦑/🔨/server/coordinator/⚡️/🟦/{m.group(1)}"
...
def _(m):
    return f"🧰/🛍️/🦑/🔨/server/lib/⚡️/🟦/{m.group(1)}"
...
def _(m):
    return f"🧰/🛍️/💻/🔨/renderer/⚡️/🦀/🧑‍🎨/🧊/{m.group(1)}"

@rule(r'^framework/os/renderer/js/react/(.+)$')
def _(m):
    return f"🧰/🛍️/💻/🔨/renderer/⚡️/🟦/🧑‍🎨/⚛️/{m.group(1)}"
...
def _(m):
    return f"🧰/🛍️/💻/🔨/plugin/⚡️/🟦/registry/{m.group(1)}"
...
def _(m):
    return f"🧰/🛍️/💻/🔨/infinite/⚡️/🟦/canvas/react-renderer/{m.group(1)}"

@rule(r'^framework/os/kernel/infinite/world/r3f/(.+)$')
def _(m):
    return f"🧰/🛍️/💻/🔨/infinite/⚡️/🟦/world/r3f/{m.group(1)}"
...
def _(m):
    return f"🧰/🛍️/💻/🔨/flow/⚡️/🦀/core/pkg/{m.group(1)}"
...
def _(m):
    return f"🧰/🛍️/📓/⚡️/🟦/{m.group(1)}"

# ---- framework/module ----
@rule(r'^framework/module/ui/js/react/(.+)$')
def _(m):
    return f"🧰/🔨/ui/⚡️/🟦/react/{m.group(1)}"
...
def _(m):
    return f"🧰/🔨/ui/⚡️/🟦/styling/{m.group(1)}"
...
def _(m):
    return f"🧰/🔨/math/⚡️/🟦/graph/dsl/core/{m.group(1)}"

@rule(r'^framework/module/math/(.+)$')
def _(m):
    return f"🧰/🔨/math/⚡️/🦀/{m.group(1)}"

@rule(r'^framework/module/surface/(.+)$')
def _(m):
    return f"🧰/🔨/surface/⚡️/🦀/{m.group(1)}"

@rule(r'^framework/module/asset/(.+)$')
def _(m):
    return f"🧰/🔨/asset/⚡️/🟦/{m.group(1)}"
...
def _(m):
    return f"🧰/🔨/{m.group(1)}/⚡️/🦀/{m.group(2)}"

@rule(r'^framework/(rs|js)/(.+)$')
def _(m):
    lang = '🦀' if m.group(1) == 'rs' else '🟦'
    return f"🧰/⚡️/{lang}/{m.group(2)}"
...
def _(m):
    return f"✏️/🔨/2d/⚡️/🟦/{m.group(1)}"

@rule(r'^s/module/3d/brep/js/(.+)$')
def _(m):
    return f"✏️/🔨/3d/⚡️/🟦/brep/{m.group(1)}"
...
def _(m):
    return f"✏️/🔌/{m.group(1)}/🧩/{m.group(2)}/⚡️/🦀/{m.group(3)}"

@rule(r'^s/plugin/puzzle/module/asset/(.+)$')
def _(m):
    return f"✏️/🔌/puzzle/🔨/asset/⚡️/🟦/{m.group(1)}"
...
def _(m):
    return f"✏️/🔌/animate/🎛️/present/⚡️/🟦/{m.group(1)}"

@rule(r'^s/plugin/([^/]+)/app/([^/]+)/(engine|dsl|op|pack|protocol|ui)/rs/(.+)$')
def _(m):
    return f"✏️/🔌/{m.group(1)}/🎛️/{m.group(2)}/🔨/{m.group(3)}/⚡️/🦀/{m.group(4)}"

@rule(r'^s/plugin/([^/]+)/app/([^/]+)/rs/(.+)$')
def _(m):
    return f"✏️/🔌/{m.group(1)}/🎛️/{m.group(2)}/⚡️/🦀/{m.group(3)}"

@rule(r'^s/plugin/([^/]+)/app/([^/]+)/(.+)$')
def _(m):
    return f"✏️/🔌/{m.group(1)}/🎛️/{m.group(2)}/{m.group(3)}"

@rule(r'^s/plugin/cad/(core|runtime|query|renderer|stately|brepjs)/(.+)$')
def _(m):
    return f"✏️/🔌/cad/🔨/{m.group(1)}/⚡️/🟦/{m.group(2)}"
...
def _rename_entry(path):
    if path.endswith('/index.ts') or path == 'index.ts':
        return path[: -len('index.ts')] + '📦.ts'
    if path.endswith('/index.tsx') or path == 'index.tsx':
        return path[: -len('index.tsx')] + '📦.tsx'
    return path
