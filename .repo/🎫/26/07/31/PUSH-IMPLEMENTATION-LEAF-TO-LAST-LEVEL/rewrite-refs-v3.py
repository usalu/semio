...
def do_cargo_metadata_string_fields():
    ...
    pattern = re.compile(r'"((?:🧰framework|✏️s|🌎hub|♻️mit-bestand)/[^"]+)"')
    for f in cargo_files():
        if not os.path.exists(f):
            continue
...
RESOLVE_ABS_RE = re.compile(r'"((?:🧰framework|✏️s|🌎hub|♻️mit-bestand)/[^"]+)"')
DOTSLASH_ABS_RE = re.compile(r'"(\./(?:🧰framework|✏️s|🌎hub|♻️mit-bestand)/[^"]+)"')
...
def do_dependency_cruiser():
    ...
    new_content = re.sub(r'"((?:🧰framework|✏️s|🌎hub|♻️mit-bestand)/[^"]*)"', repl, content)
    if new_content != content:
