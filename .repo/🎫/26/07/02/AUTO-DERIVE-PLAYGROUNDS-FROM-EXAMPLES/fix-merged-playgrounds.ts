...
for (const rel of files) {
  ...
  if (appId) content = content.replace(/createProductPlaygroundPlatform\(this\.id(?:, "([^"]*)")?\)/g, (_, name) => (name ? `createProductPlaygroundPlatform(${appId}, ${JSON.stringify(name)})` : `createProductPlaygroundPlatform(${appId})`));
...
for (const rel of [
  "draw/core/example-slugs.ts",
  "note/core/example-slugs.ts",
  "writer/core/example-slugs.ts",
  "forms/core/example-slugs.ts",
  "s/core/example-slugs.ts",
  "shooting/core/example-slugs.ts",
  "procedural/2d/core/example-slugs.ts",
  "procedural/3d/core/example-slugs.ts",
  "raster/core/example-slugs.ts",
  "cad/js/renderer/core/example-slugs.ts",
  "trinity/jack/host-core/example-slugs.ts",
]) {
  const path = join(repoRoot, rel);
  if (!existsSync(path)) continue;
  let content = readFileSync(path, "utf8");
