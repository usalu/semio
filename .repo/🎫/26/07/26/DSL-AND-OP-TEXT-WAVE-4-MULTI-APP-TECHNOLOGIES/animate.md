...
- `animate/present/core/js/index.ts` region `//#region 🔖DocumentVcs` (was lines 1987-2175) declared
...
- Grepped for every exported symbol from the dead region (`PresentationEditOperation`,
  ...
  one-off migration scripts under `.repo/🎫/26/07/03/APP-ISOLATION-ENFORCED-BOUNDARIES/*.ts` that only
...
- `animate/present/core/js/index.ts` — deleted dead `🔖DocumentVcs` region + its dead test block.
- `animate/present/rs/lib.rs` — added `mod present_text` (`🔖Dsl`/`🔖OpText` regions), extended
  `🧪Tests` with `🔖DslTests`/`🔖OpTextTests`/`🔖DocumentTextTests`.
