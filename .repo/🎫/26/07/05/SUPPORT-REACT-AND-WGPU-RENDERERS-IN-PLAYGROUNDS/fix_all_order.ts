...
// Rename the second "🛠️dev🖥️s🧊wgpu" to "🛠️dev🖥️s🧊wgpu-alt"
let wgpuCount = 0;
for (const config of configs) {
	if (config.name === "🛠️dev🖥️s🧊wgpu") {
		wgpuCount++;
		if (wgpuCount === 2) {
			console.log("Renaming second 🛠️dev🖥️s🧊wgpu to 🛠️dev🖥️s🧊wgpu-alt");
			config.name = "🛠️dev🖥️s🧊wgpu-alt";
			config.obj.name = "🛠️dev🖥️s🧊wgpu-alt";
...
for (let j = 0; j < configs.length; j++) {
	...
	const isWgpu = config.name.endsWith("🧊wgpu");
	const isWgpuAlt = config.name.endsWith("🧊wgpu-alt");
	if (isWgpu || isWgpuAlt) {
		const base = isWgpu ? config.name.replace("🧊wgpu", "") : config.name.replace("🧊wgpu-alt", "");
	...
	if (config.name.endsWith("⚛️react")) {
		...
		const wgpuName = base + "🧊wgpu";
		const wgpuAltName = base + "🧊wgpu-alt";
