...
- **Required side-fix for correct height math**: `render_window_measures_rail`'s top-level loop (line ~9760) and `render_window_measure`'s `Group` children loop (line ~9833) currently render every sibling at the *same* `y` (no advancement between iterations) — a pre-existing stacking bug that must be fixed to thread a running `y` cursor (advanced by each measure's height from the new helper) so siblings actually stack vertically like React's flex column, and so the auto-height sum is accurate. Also add the missing `y += theme.control_height` after the `control` block in the engagement renderer (~10066), which today isn't followed by an advance before `status`/`possible_engagements` render.
