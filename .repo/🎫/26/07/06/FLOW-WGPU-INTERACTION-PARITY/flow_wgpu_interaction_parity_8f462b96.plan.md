...
Per repo workflow, reopen `.repo/🎫/26/07/06/FLOW-WGPU-RICH-RENDERING-PARITY` (closely related follow-on) or open a new ticket if the scope is judged distinct enough during execution; update `important.md` with root causes/fixes per item above, and close with a full file list when done.
