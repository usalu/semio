...
todos:
  - id: reopen-ticket
    content: Reopen .repo/🎫/26/07/06/FLOW-WGPU-RICH-RENDERING-PARITY ticket
    status: completed
  - id: clean-rebuild
...
Despite this, the user's screenshot shows only bare outlined rectangles — **no edges, no port handles/channels, no LOD content, no text labels at all**. Static tracing of the pipeline shows it *should* work:
...
1. **Reopen** ticket `.repo/🎫/26/07/06/FLOW-WGPU-RICH-RENDERING-PARITY` (covers this exact regression; do not open a new one).
