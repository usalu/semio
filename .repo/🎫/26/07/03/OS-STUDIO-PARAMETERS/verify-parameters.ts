import { OsStore, createEmptyOsDocument, mergeOsProgramDefinition, osOutPort, isParameterPortId, resolveParameterValuesForInstance, applyParameterValuesToProjection } from "@semio-tech/framework-os-core";
...
console.log(
  "[DEBUG] parameter port present",
  node.inputs.some((port) => isParameterPortId(port.id)),
);
...
console.log("[DEBUG] json pointer override", applyParameterValuesToProjection({ brushSize: 1 }, store.projection().parameterBindings, store.projection().parameters, instance.id));
console.log("[DEBUG] demo fixture binding", demoRasterBinding);
console.log(
  "[DEBUG] demo raster parameter port",
  demoRasterNode?.inputs.some((port) => isParameterPortId(port.id)),
);
