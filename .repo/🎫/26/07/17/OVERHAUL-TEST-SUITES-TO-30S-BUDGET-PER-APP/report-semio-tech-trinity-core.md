...
- `project.json` already only called `bun ./script.ts test`; unchanged.

## After

Re-ran through the real entry point (`bun ./script.ts test`, same isolated target dir / no-sccache override used only to avoid an unrelated environment-wide sccache-server queueing stall other concurrent sessions were causing — not a code change):
- `Finished test profile ... in 2.84s` (build check, uncounted), then `running 11 tests ... test result: ok. 11 passed; ... finished in 0.02s`.
- Total wall time for the whole `bun ./script.ts test` invocation: ~6.5s, dominated by bun/cargo startup, not test execution.
