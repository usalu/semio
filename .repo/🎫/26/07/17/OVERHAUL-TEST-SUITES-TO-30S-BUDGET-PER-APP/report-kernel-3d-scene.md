...
- `kernel/3d/scene/project.json` `test` target already just called `bun ./script.ts test` (no change needed there).
...
- `kernel/3d/scene/project.json` — reviewed only, not modified (already `bun ./script.ts test`, no `test-e2e` needed).
- `.repo/🎫/26/07/17/OVERHAUL-TEST-SUITES-TO-30S-BUDGET-PER-APP/report-kernel-3d-scene.md` — this report.
