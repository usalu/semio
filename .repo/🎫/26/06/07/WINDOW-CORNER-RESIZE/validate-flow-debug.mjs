...
writeFileSync(
  ".repo/🎫/26/06/07/WINDOW-CORNER-RESIZE/validate-flow-console.txt",
  [...logs, ...errors.map((e) => `[pageerror] ${e}`)].join("\n"),
);

console.log("[validate-flow] body:", bodyText.slice(0, 500));
console.log("[validate-flow] errors:", errors);
console.log("[validate-flow] debug logs:", logs.filter((l) => l.includes("DEBUG")));
