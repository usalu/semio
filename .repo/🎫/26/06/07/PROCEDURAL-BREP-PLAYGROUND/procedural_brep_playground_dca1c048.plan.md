...
overview: Extract the brepjs kernel into a reusable cad-free `@geometry/brep/js` bundle, then build the `@procedural/react` flow-based brep editor (FlowCanvas node graph + brep nodes + R3F preview), and a `@procedural/play` playground mirroring the flow/dag split pattern.
todos:
  - id: ticket
    content: Read repo://goals and open a ticket for the procedural brep playground work
    status: completed
  - id: geo-scaffold
    content: Scaffold @geometry/brep/js bundle (package.json, project.json, script.ts, tsconfig, vitest.config) mirroring cad/js/kernel/brepjs
    status: completed
  - id: geo-move-impl
  ...
  - id: geo-move-contracts
    ...
    status: completed
  - id: cad-slim-core
    content: "Slim @cad/js/core: import moved contracts from @geometry/brep/js, redefine SpatialKernel extends BrepKernel"
    status: completed
  - id: cad-slim-kernel
    content: Slim @cad/js/kernel/brepjs into a cad SpatialKernel adapter wrapping geometry BrepjsKernel + Model sync + STEP-UDA/typology; fix embedded tests
    status: completed
...
```mermaid
flowchart TB
  subgraph geo ["@geometry/brep/js (new, cad-free)"]
  ...
  subgraph cad ["cad (slimmed)"]
    Core["@cad/js/core: SpatialKernel extends BrepKernel + Model/ModelDiff/STEP-UDA/typology"]
    CadKernel["@cad/js/kernel/brepjs: CadSpatialKernel wraps geometry BrepjsKernel + Model sync"]
    Renderer["@cad/js/renderer"]
  end
  subgraph proc ["procedural (new)"]
    ProcReact["@procedural/react: ProceduralEditor = FlowCanvas + brep JS module + BrepViewport"]
    ProcPlay["@procedural/play: PlaygroundProcedural (PUZZLE_PLAY_ENTRY=procedural)"]
  end
  Flow["@flow/react: FlowCanvas + eval bridge (sync, JSON ports)"]
  Infinite["@infinite/world/r3f: WorldCanvas + WorldOrbitGated"]
...
## Phase 1 - Extract reusable brep kernel `@geometry/brep/js`
...
- Create `geometry/brep/js/index.ts` (`@geometry/brep/js`, `bundleKind: "library"`) plus `package.json`, `project.json`, `script.ts`, `tsconfig.json`, `vitest.config.ts` mirroring [cad/js/kernel/brepjs](cad/js/kernel/brepjs/package.json). Deps: `brepjs`, `brepjs-opencascade` only.
...
- Move the GENERIC contracts out of [cad/js/core/index.ts](cad/js/core/index.ts) into `@geometry/brep/js`: `kernelGeometry` namespace (records/refs ~1161-1260), `Vec3`/`EdgeCurve`/`ArcPlaneFrame` (~172-208), `MeshTransfer`/`FaceGroup`/`EdgeGroup`/`FaceInfo`/`EdgeInfo`/`emptyMeshTransfer` (~4124-4167). Define new interfaces `BrepPreviewKernel` (generic preview math) and `BrepKernel` (model-free brep ops); these are the cad-free split of the current `SpatialPreviewKernel`/`SpatialKernel`.
- Add `buildBufferGeometryFromMeshTransfer` + `isRenderableMeshTransfer` (pure, three-free: return plain typed-array geometry data, or keep three behind the existing `sceneHostPort` boundary) so the viewport can convert tessellation without depending on `@cad/js/renderer`.
- Slim [cad/js/core/index.ts](cad/js/core/index.ts): re-export/import the moved contracts from `@geometry/brep/js`; redefine cad `SpatialKernel` to `extends BrepKernel` adding the `Model`/`ModelDiff` methods (`syncSolidsFromModel`, `tessellate(...model)`, `executeCommandDiff`, `*Diff`). Keep all STEP-UDA/typology helpers in core.
- Slim [cad/js/kernel/brepjs/index.ts](cad/js/kernel/brepjs/index.ts): becomes the cad `SpatialKernel` adapter composing `@geometry/brep/js`'s `BrepjsKernel` + `Model` sync + cad STEP import/export (`importStepBimToModelSpace`, typology mapping, UDA). Re-point its embedded tests; keep `@cad/js/runtime`/module test imports.
- Update consumers: [cad/js/renderer/index.tsx](cad/js/renderer/index.tsx) and [cad/js/renderer/play/index.tsx](cad/js/renderer/play/index.tsx) import `PreciseSpatialKernelMath`/`preciseSpatialKernelMath`/`Vec3`/`MeshTransfer` from `@geometry/brep/js`, and `BrepjsKernel` (cad adapter) from `@cad/js/kernel/brepjs`.
- Wire config: add `@geometry/brep/js` to root `package.json` workspaces; add alias in [ui/styling/vite-elements-assets.ts](ui/styling/vite-elements-assets.ts) and in [cad/js/renderer/play/vite.config.ts](cad/js/renderer/play/vite.config.ts) `extraAliases`/`optimizeDeps` (move `brepjs`/`brepjs-opencascade` prebundling target); update tsconfig `paths` and vitest aliases in the affected bundles; add `launch.json` entry for `@geometry/brep/js:fixture`/`:test` near the existing `🛠️dev📐cad🧪kernel🪜fixture`.
- Validate: run `@geometry/brep/js:test`, `@cad/js/kernel/brepjs:test`, `@cad/js/renderer` build/dev, confirming the cad play viewport still renders.

## Phase 2 - Procedural react bundle `@procedural/react`
...
- Scaffold `procedural/react`: fill `index.tsx`, add `script.ts`, `package.json` (`@procedural/react`, library), `project.json`, `vitest.config.ts` mirroring [flow/react](flow/react/package.json). Deps: `@flow/react`, `@geometry/brep/js`, `@infinite/world/r3f`, `react`, `react-dom`, `three`, `@react-three/fiber`, `@react-three/drei`, `@ui/styling`.
- Brep flow module (JS-evaluated virtual module, no Rust): register a `FlowModuleManifestV1` in TS with `NeuronKindInfo`s like `brep.box`, `brep.sphere`, `brep.cylinder`, `brep.extrude`, `brep.union`, `brep.difference`, `brep.intersect`, `brep.translate`. Provide a synchronous `evaluate(kindId, inputJson)` that calls a pre-initialized `@geometry/brep/js` brep engine (local sync engine, WASM initialized eagerly via `ensureBrepWasmLoaded()` before evaluation is enabled). Solids are stored in an in-memory `Map<id, SolidRef>`; node outputs carry `{ "brep": "<solidId>" }` dictionaries (ids cross the JSON port boundary; geometry stays in the engine).
...
## Phase 3 - Procedural playground `@procedural/play`
...
- Vite config: `createPlaygroundPlayViteConfig({ playEntryKind: "procedural", extraAliases: [@procedural/react, @geometry/brep/js, @flow/react, three], resolveDedupe: ["react","react-dom","three","scheduler"], optimizeDeps: ["brepjs","brepjs-opencascade", R3F, @infinite/world/r3f] })`.
- Repo registration: add `procedural/react` + `procedural/play` to root `package.json` workspaces; add `dev:procedural` script + a `dev procedural` route in root [script.ts](script.ts); add `@procedural/react`/`@procedural/play` aliases in [ui/styling/vite-elements-assets.ts](ui/styling/vite-elements-assets.ts); add `launch.json` entries (`🛠️dev🔧procedural`, optional `:validate`) following the flow grouping/order with `PROCEDURAL_PLAY_PORT=6018`.
- Validate: `bun run dev:procedural` serves on 6018; drop brep nodes (box -> extrude -> union), confirm the R3F viewport renders the tessellated solid (check `[DEBUG]` eval/preview logs); run `@procedural/react:test` and `@procedural/play:test`.
...
- Avoid cad coupling in procedural: `buildBufferGeometryFromMeshTransfer` moves to `@geometry/brep/js` so procedural does not import `@cad/js/renderer`.
- Circular-dep avoidance: all `Model`/`ModelDiff`/STEP-UDA/typology logic stays in cad; `@geometry/brep/js` never imports `@cad/*`.