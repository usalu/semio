...
todos:
  ...
  - id: fixture
    ...
    status: completed
  - id: renderer-host
    content: Add 🔖ShootingPlayHost region + bootShootingPlay + surface host in playground renderer/react and ./shooting export
    status: completed
  - id: vite-ports
...
- `.vscode/launch.json`: add `🛠️dev🔧shooting` (port 6019) and `📦build🔧shooting` entries in each duplicated block, following procedural's group/order.
