...
The `SolidSource` enum, `Entry.source`, `entry_source`, `solid_bounds_overlap`, the `source` param on `register_solid`, and the `translate_sync` source branch exist *only* to feed these guards.
