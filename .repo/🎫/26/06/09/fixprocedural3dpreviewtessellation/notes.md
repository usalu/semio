...
## Root cause
Flow graph evaluation moved to in-WASM `flow_core` registry. Geometry handles were created in `flow_core`'s linked brep kernel, but 3D preview tessellation still used standalone `@semio-tech/flow-module-brep` WASM — a separate kernel instance where those handles do not exist.
