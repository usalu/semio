...
todos:
  ...
  - id: launch
    ...
    status: completed
  - id: verify
    content: Run @framework/presentation/play:dev and confirm seed→drag→resize→rename→copy-prompt at runtime; run :test for new core tests; ticket_close with summary.
    status: completed
isProject: false
---
...
A standalone Vite sandbox at `framework/product/presentation/play/`, on the `@framework/playground` shell, modeled on [puzzle/2d/play](puzzle/2d/play/index.ts). First (and only initially shipped) feature: the **one-to-many morph tile selector**. Architected to host more presentation-feature pickers later (one Playground app, feature-per-mode/window).

Greenfield: no compat layers. Pure geometry/prompt logic lives in `@framework/presentation/core` (testable, DOM-free); interaction lives in the playground React renderer; clipboard write is the only browser side-effect.

### 1. Where it lives & how it launches
- New package `@framework/presentation/play` in `framework/product/presentation/play/` with: `index.ts`, `index.html`, `globals.css`, `vite.config.ts`, `project.json`, `package.json`, `script.ts`, `vitest.config.ts` — mirroring [puzzle/2d/play](puzzle/2d/play/project.json) and [33.projektetage](mit-bestand/präsentation/33.projektetage/index.html).
...
- Register in [.vscode/launch.json](.vscode/launch.json): a `🛠️dev📽️presentationplay` entry in the `🛠️ Dev` region right after `🛠️dev📽️projektetage`, and a `📦build📽️presentationplay` in the `📦 Build` region, both calling `bun nx run @framework/presentation/play:dev|build` with `serverReadyAction` opening `http://localhost:6051`.
...
- Register via the generic `registerSurfaceBinding(PRESENTATION_PLAY_SURFACE_ID, FigureTilesSurfaceHost)` (the `panel` host path) — no new `componentKind` in `@framework/platform/core`.
...
- Add the `@framework/presentation/play` export path to [framework/product/playground/renderer/react/package.json](framework/product/playground/renderer/react/package.json) deps + `exports["./presentation"]`.
...
- `framework/product/presentation/play/vite.config.ts`: `createPlaygroundPlayViteConfig({ playDir, repoRoot, playEntryKind: "presentation", extraAliases: [@framework/presentation/core, /react renderer] })`.
