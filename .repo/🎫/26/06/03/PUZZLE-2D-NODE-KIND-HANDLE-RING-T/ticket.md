...
Fix node-kind handle template angles to use semio connector normalized ring `t` instead of CAD point → angle conversion.
...
- One-off migration rebakes `puzzle/2d/fixture/nakagin-capsule-tower.2d.json` catalog + instance handle angles from `nakagin-capsule-tower.filtered.kit.semio.json`.
