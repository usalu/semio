...
- `CATALOGUE_FRAME` restored to hand-tuned `{ x: 0.127, y: 0.1, width: 0.746, height: 0.75 }`; full catalogue embodiment uses the same crop.
...
- Split tiles: `FigureEmbodiment.mosaic` includes `frame`; edge-aligned `background-position`; **no cover zoom** on mosaic rest (exact stretch per axis).
