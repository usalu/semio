...
- Split tiles: `FigureEmbodiment.mosaic` includes `frame`; edge-aligned `background-position`.
- **CSS `background-size: cover`** on all figure crop slots (never dual-axis `%` — that always distorts). Morph keyframes animate **position only**.
