...
console.log(
  "[DEBUG] preview-check labels",
  paramSpec.steps[0]?.questions.map((question) => `${question.id}:${question.label}`),
);
...
console.log(
  "[DEBUG] preview-check items",
  items.map((item) => `${item.widgetId}:${item.port}:${item.kind}`),
);
