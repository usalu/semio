...
- `compose/graphql/target.schema.graphql` — refactored `#region MutationInputs` and `type Mutation`:
...
- `compose/graphql/target.schema.graphql` — stripped narrow-typed owner duplicates from every entity, leaving only the general interface fields:
...
- `compose/graphql/target.schema.graphql` — removed every `union` declaration and retyped every union-typed field:
...
- `compose/graphql/target.schema.graphql` — leaned every command argument:
...
- `node -e "buildSchema(read('compose/graphql/target.schema.graphql'))"`: `parse OK`. Only remaining `BUILD ERR` is the pre-existing duplicate `Modification` (interface @ L109 + union @ L12741), unrelated to this ticket.
...
- `rg "ScopeInput|^.*draftId" compose/graphql/target.schema.graphql` → 1 match: the `draftId: ID!` field inside `TransactionScopeInput`.
- `rg transactionId compose/graphql/target.schema.graphql` → 1 match: the `transactionId: ID!` field inside `TransactionScopeInput`.
- `rg transactionScope compose/graphql/target.schema.graphql` → 1 match: the `change(transactionScope: TransactionScopeInput!): KitChange!` field on `Mutation`.
...
- `rg "^  owner: |^  \w+Owner: |^  ownerModifications: |^  ownerDiffs: " compose/graphql/target.schema.graphql` → 0 matches (every narrow-typed owner duplicate is gone).
- `rg "^  ownerEntity: OwnerEntity" compose/graphql/target.schema.graphql` → 265 matches (the general interface field is intact on every type and interface).
...
- `rg "^union\s" compose/graphql/target.schema.graphql` → 0 matches.
- `rg "\bOwnerEntity\b|\bOwnedEntityConnection\b|\bEntityConnectionInterface\b" compose/graphql/target.schema.graphql` → 0 matches.
