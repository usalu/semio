#!/usr/bin/env python3
"""Replace schema_gap_surfaces roster macros (single __idents list)."""
...
block_start = next(
    i for i, ln in enumerate(lines) if i > mod_start and "macro_rules! gap_surface_families" in ln
)
endregion_idx = next(
...
def extract_idents(marker: str) -> list[str]:
...
family_idents = extract_idents("macro_rules! gap_surface_family_name_list")
relay_idents = extract_idents("macro_rules! gap_surface_existing_relay_name_list")
...
tmp = p.with_suffix(".rs.tmp")
tmp.write_text(out, encoding="utf-8")
tmp.replace(p)
print(f"rewrote {block_start}..{endregion_idx}: {len(family_idents)} family, {len(relay_idents)} relay")
