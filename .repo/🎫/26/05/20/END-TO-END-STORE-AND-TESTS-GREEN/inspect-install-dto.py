...
'@
$tmp = Join-Path $env:TEMP ("compose-dump-" + [guid]::NewGuid().ToString("N") + ".cs")
Set-Content -Path $tmp -Value $code -Encoding UTF8
dotnet run --project ''' + str(repo / "compose/client/lib/net/Compose.Tests/Compose.Tests.csproj").replace('\\', '\\\\') + r''' -f net8.0 --no-build 2>&1 | Out-Null
"""
