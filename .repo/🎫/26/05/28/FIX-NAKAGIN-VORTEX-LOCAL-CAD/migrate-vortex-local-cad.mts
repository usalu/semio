#!/usr/bin/env bun
/** @emoji 🧾 One-off: rewrite nakagin scene vortex positions as type-local CAD connector points (kit port → connector.point). */
import { readFileSync, writeFileSync } from "node:fs";
import { join } from "node:path";

const repoRoot = join(import.meta.dir, "..", "..", "..", "..", "..", "..");
const kitPath = join(repoRoot, "semio/fixtures/stores/metabolism/wip/initialKit/kit.semio.json");
...
function main(): void {
	...
	const objects = scene.objects.map((obj) => {
		...
		for (const h of node.handles) {
			...
			const direction =
				connector.direction != null
					? cadPoint(connector.direction as { x: number; y: number; z: number })
					: prev?.direction;
