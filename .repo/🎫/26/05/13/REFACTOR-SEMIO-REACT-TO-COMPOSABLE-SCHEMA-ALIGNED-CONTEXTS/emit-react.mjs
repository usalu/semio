...
const outPath = path.resolve(__dirname, "../../../..", "semio/client/lib/react/index.tsx");

const header = `// #region ⚛️Header
/** @emoji ⚛️ Sealed React surface for semio: composable id-only contexts over semio/js (implementation detail). */
// #endregion ⚛️Header
...
/** @emoji 🪪 Entity hooks return this shape with {@code value: { id }} (no semio/js class in {@code value}). */
...
/** @emoji 📇 Plain list row for id-stable kit lists (consumers wrap with matching {@code XContextProvider}). */
...
function useJsSession(): Session {
  ...
  if (s == null) throw new Error("semio/react: SessionHandleContext missing; wrap with <SessionContextProvider>.");
  return s;
}

function useJsStore(): Store {
  const s = React.useContext(StoreHandleContext);
  if (s == null) throw new Error("semio/react: StoreHandleContext missing; wrap with <StoreContextProvider id={…}>.");
  return s;
}

/** @emoji 🪝 Private field bind: async read + optional bus kind (no useSyncExternalStore). */
...
/** @emoji 🪝 Private operation bind returning {@link OperationStatus}. */
...
/** @emoji 🪢 Batch piece ids for {@link useDragPieces} / {@link useMovePieces} (design from {@link DesignIdContext}). */
...
${idCtx("PortId", "🔘")}
${idCtx("ConnectorId", "🔗")}
${idCtx("RepresentationId", "🎨")}
${idCtx("RemoteProviderUrl", "🛜")}
${idCtx("FileBackboneId", "📁")}
${idCtx("FolderBackboneId", "📂")}
...
/** @emoji 🏪 Resolves {@link Store} from session + id; applies optional {@link Store#setReadPoint}. */
...
export function TheKitContextProvider(props: Readonly<{ children: ReactNode }>): React.ReactElement {
  ...
  if (graph == null) throw new Error("semio/react: TheKitContextProvider requires a graph tier (Wip|Stage|Authoritative) above.");
...
export function AlternativeContextProvider(props: Readonly<{ id: string; children: ReactNode }>): React.ReactElement {
  const graph = React.useContext(GraphHandleContext);
  if (graph == null) throw new Error("semio/react: AlternativeContextProvider requires a graph tier above.");
...
export function KitContextProvider(props: Readonly<{ id: string; children: ReactNode }>): React.ReactElement {
  ...
  const kit = React.useMemo(() => {
    ...
    throw new Error("semio/react: KitContextProvider requires TheKitContextProvider or AlternativeContextProvider.");
...
export function FileBackboneContextProvider(props: Readonly<{ id: string; children: ReactNode }>): React.ReactElement {
  ...
  if (lp == null) throw new Error("semio/react: FileBackboneContextProvider requires LocalProviderContextProvider.");
...
export function FolderBackboneContextProvider(props: Readonly<{ id: string; children: ReactNode }>): React.ReactElement {
  const lp = React.useContext(LocalProviderHandleContext);
  if (lp == null) throw new Error("semio/react: FolderBackboneContextProvider requires LocalProviderContextProvider.");
  const bb = React.useMemo(() => new Backbone(useJsSession(), props.id, lp), [props.id, lp]);
  const idRow = React.useMemo(() => ({ id: props.id }), [props.id]);
  return React.createElement(
    BackboneHandleContext.Provider,
...
export function WebsocketBackboneContextProvider(props: Readonly<{ id: string; children: ReactNode }>): React.ReactElement {
  ...
  if (rp == null) throw new Error("semio/react: WebsocketBackboneContextProvider requires RemoteProviderContextProvider.");
...
// #endregion 🪝EntityHooks
`;

const weakHooks = `
// #region 🪶WeakGeometryHooks
...
const body = [
  ...
  weakHooks,
  listHooks,
  "// #region 🪝FieldHooks\n// (generated below via patch)\n// #endregion 🪝FieldHooks\n",
  "// #region 🪝OperationHooks\n// #endregion 🪝OperationHooks\n",
  vitest,
].join("\n");
