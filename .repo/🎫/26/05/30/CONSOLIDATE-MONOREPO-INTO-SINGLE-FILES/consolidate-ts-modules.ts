...
function mergeSatellites(targetPath: string, satellitePaths: string[], regionPrefix: string, stripPatterns: RegExp[] = []): void {
	...
	for (const satellitePath of satellitePaths) {
		...
		body = stripLocalImports(body, satelliteNames.filter((n) => n !== name));
...
mergeSatellites(
	...
	[
		join(root, "framework/renderer/react/components/graph-canvas-overlays.tsx"),
		join(root, "framework/renderer/react/components/flow-graph-canvas-host.tsx"),
	],
	"🔖",
	[
...
const shellSatellites = [
	"types.ts",
	"plugin-runtime.ts",
	"wasm-session-loader.ts",
	"ui-search-find.tsx",
	"tool-tree.tsx",
	"os-chrome-panels.tsx",
...
for (const satellitePath of shellSatellites) {
	const name = basename(satellitePath).replace(/\.(tsx?)$/, "");
	let body = stripHeader(readFileSync(satellitePath, "utf8"));
	body = stripLocalImports(body, shellSatelliteNames.filter((n) => n !== name));
...
for (const file of kitStoreFiles) {
	...
	let body = stripHeader(readFileSync(satellitePath, "utf8"));
	body = stripLocalImports(body, kitStoreNames.filter((n) => n !== name));
...
if (existsSync(join(osDevDir, "plugin-registry.ts"))) {
	...
	if (!index.includes("const plugins = PLUGIN_BUILD_TARGETS.map")) {
		index = index.replace(
			/void bootFrameworkOsWgpu\(\{\s*plugin: pluginFilter,\s*plugins: PLUGIN_BUILD_TARGETS\.map\(\(target\) => \(\{[\s\S]*?\}\)\),\s*\}\)/,
			"void bootFrameworkOsWgpu({ plugin: pluginFilter, plugins })",
		);
		index = index.replace(
			/void bootFrameworkOs\(\{\s*plugin: pluginFilter,\s*plugins: PLUGIN_BUILD_TARGETS\.map\(\(target\) => \(\{[\s\S]*?\}\)\),\s*\}\)/,
			"void bootFrameworkOs({ plugin: pluginFilter, plugins })",
		);
		index = index.replace(
...
docsMdx = docsMdx.replace(
	"export function sketchpadBuildDocsRegistry()",
	"function sketchpadBuildDocsRegistryFromGlob()",
);
sketchpad = sketchpad.replace(
