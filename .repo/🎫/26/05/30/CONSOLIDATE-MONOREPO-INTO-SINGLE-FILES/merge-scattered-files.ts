...
function mergeTsSatellites(indexPath: string, satellites: string[]): void {
  ...
  for (const sat of satellites) {
    ...
    if (!index.includes(`//#region 🔖${name}`)) index += `\n${body}`;
    unlinkSync(satPath);
  }
...
writeFileSync(join(root, "mathematical/graph/manifest/core/index.ts"), `/** @emoji 📜 \`@semio-tech/graph-manifest\` — compile-time graph manifest types and catalog projection. */\nexport * from "../generated/index.ts";\n${validateBody}`);
...
if (!specIndexContent.includes("//#region 🔖spec")) specIndexContent += `\n${specBody}`;
