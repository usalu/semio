...
function inlineMod(lib: string, libPath: string, name: string, pub: boolean, cfg?: string): string {
  ...
  const decl = cfg ? `#[cfg(${cfg})]\n${pub ? "pub " : ""}mod ${name};` : `${pub ? "pub " : ""}mod ${name};`;
  if (!lib.includes(decl)) throw new Error(`${libPath}: missing declaration:\n${decl}`);
  const inline = cfg ? `#[cfg(${cfg})]\n${pub ? "pub " : ""}mod ${name} {\n// #region ${name}\n${body}\n// #endregion ${name}\n}\n` : `${pub ? "pub " : ""}mod ${name} {\n// #region ${name}\n${body}\n// #endregion ${name}\n}\n`;
...
const specs: ModSpec[] = [
  ...
  },
  {
    ...
    mods: [{ name: "document" }, { name: "display" }, { name: "engine" }, { name: "export" }, { name: "wasm_session", cfg: 'target_arch = "wasm32"' }],
  },
  {
