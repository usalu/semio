...
- `Component<TModel>` (lines ~288-320) → `Component<TSnapshot> extends Store<TSnapshot>`. Rename `getModel`→`getSnapshot`, `buildModel`→`buildSnapshot`, `TModel`→`TSnapshot`, drop the internal `modelCell` (the Component is the store), and update `refresh()`/`setModel` accordingly. This also removes the semio-forbidden term `model`.
...
- **sketchpad / semio-js (WASM worker)** in [semio/client/lib/sketchpad/js/index.ts](semio/client/lib/sketchpad/js/index.ts): replace the ad-hoc `KitHostStore` type + `setSketchpadKitRegistryBridge` (lines ~40-72) with a concrete `class SemioKitStore extends Store<{ kit: Kit }>` that adapts the `@semio/js` `Session`/`Store` (the WASM web-worker backend). `@semio/js` stays below the framework, so this adapter lives in sketchpad and is `provideStore("kit", …)`-ed by `SketchpadShellController`. The local-UI state (the "stately"-style store in the example) is exposed as a second owned store (an `ObservableCell`/actor-backed `Store`), proving the "two stores, different backends, one product" case.
