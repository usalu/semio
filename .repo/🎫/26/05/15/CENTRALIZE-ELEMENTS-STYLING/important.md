...
- Semio tailwind presets now import the shared preset from `elements/client/lib/styling/tailwind.config.ts` (replacing the broken `../react/ui` sketchpad import).
...
- Root `bun install` failed here (`@semio/sketchpad` 404 from registry); relative CSS paths avoid requiring a linked `@elements/styling` package for Vite. Workspace `package.json` still declares `@elements/styling` for explicit dependency graph.
