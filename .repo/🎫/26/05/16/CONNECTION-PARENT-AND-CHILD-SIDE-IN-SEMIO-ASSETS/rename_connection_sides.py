"""One-shot rename Connected/Connecting -> Parent/Child on Connection* types in Semio.cs."""
...
def _repo_root() -> pathlib.Path:
    for p in pathlib.Path(__file__).resolve().parents:
        if (p / "semio" / "client").is_dir():
            return p
    raise RuntimeError("repo root not found")


ROOT = _repo_root()
TARGET = ROOT / "semio" / "client" / "lib" / "net" / "Semio" / "Semio.cs"
