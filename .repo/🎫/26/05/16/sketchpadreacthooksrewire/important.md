...
- `compose/client/lib/react/index.tsx` is now a **barrel** that re-exports `./logic/index` so sketchpad’s `@semio-tech/compose-react` → `../react` resolves to the CQRS hook surface (`ComposeStoreKitLineHost`, field reads, operation binds).
...
- **Reverted**: importing a copied legacy `render-kit-wasm` bundle — current `@semio-tech/compose-js` no longer exports `KitStoreClient`, `createKitStoreClient`, `InMemoryKitStore`, etc., so that layer does not typecheck.
