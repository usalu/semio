...
- SDL `Diff` lives in `semio/graphql/target.schema.graphql` as `interface Diff` + concrete `*Diff` types; `operation::Diff` is a separate in-memory operation payload — doc on `operation::Diff` updated to state that explicitly.
