...
- **`serde_json::Value` for `initialKit` projection**: moved off **`Kit`** / **`Design`** into **`//#region 🔖 dev_backbone_initial_kit_projection`** inside `kit_backbone`: `initial_kit_projection_value`, `hydrate_kit_from_initial_projection_value`, `hydrate_design_pieces_from_snapshot_value`, `graph_new_overlay_from_initial_projection_json`.
...
- **`//#region 🔖canonical_kit_types_designs_wire`** wraps the new wire structs.
