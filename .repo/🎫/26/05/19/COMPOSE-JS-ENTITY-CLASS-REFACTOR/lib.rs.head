...
//#region ­ƒº¼ entity_dsl

/// @emoji ­ƒº® Chains `SchemaBuilder::register_output_type` so macro-emitted shapes stay reachable in `Schema::sdl()`.
#[macro_export]
...
/// @emoji ­ƒº® Chains `SchemaBuilder::register_input_type` for `InputObject` shapes not yet referenced by `Query`/`Mutation` fields (keeps golden `input` lines in `gql::sdl()`).
#[macro_export]
...
/// @emoji ­ƒ¬ó `_entity_relay_shell!` ÔÇö shared relay Edge/Connection structs; `from_entities` body is supplied by [`entity_relay!`] or [`entity_relay_sync!`].
#[macro_export]
...
/// @emoji ­ƒ¬ó `entity_relay_sync!` ÔÇö relay Edge/Connection for `SimpleObject` entities with sync child digests (`compute_entity_hash`, ÔÇª).
#[macro_export]
...
/// @emoji ­ƒ¬ó `entity_full_family!` ÔÇö relay Edge/Connection for geometry (`VectorEdge`ÔÇª`LocationEdge`).
#[macro_export]
...
/// @emoji ­ƒ¬ó `entity_relay!` ÔÇö relay Edge/Connection for `Arc` graph nodes with async `compute_hash` digests.
#[macro_export]
...
/// @emoji ­ƒ¬£ `_ladder_relay_lite!` ÔÇö golden **3-shell** relay (`NameEdge` + `NameConnection`); delegates to [`entity_relay!`].
#[macro_export]
...
/// @emoji ­ƒ¬£ `_ladder_relay_full!` ÔÇö golden **12-shell** tail: `*Diff*` + `*Modification*` + `*Modifications*` relay triple (after main `entity_relay!`).
#[macro_export]
...
/// @emoji ­ƒº▒ `entity_bare!` ÔÇö splices `item` tokens (structs, impls, consts) with **no** relay shells; use for roots and `LocalProvider`-class bare nodes.
#[macro_export]
...
/// @emoji ­ƒ¬ó `entity_lite!` ÔÇö golden **3-ladder** (`Name` + `NameEdge` + `NameConnection`); expands relay via [`_ladder_relay_lite!`].
#[macro_export]
...
/// @emoji ­ƒÅù´©Å `entity_full!` ÔÇö golden **12-ladder** contract: main `entity_relay!` plus optional [`_ladder_relay_full!`] when diff/mod families are wired.
#[macro_export]
...
/// @emoji ÔÜÖ´©Å `operation_with_input!` ÔÇö splices `item` tokens for an operation plus its `*Input` surface; combine with [`entity_lite!`] for `*Edge`/`*Connection` where those Rust types exist.
#[macro_export]
...
/// @emoji ÔÜÖ´©Å `operation_no_input!` ÔÇö splices `item` tokens for inputless operations; combine with [`entity_lite!`] for relay shells.
#[macro_export]
...
/// @emoji ­ƒº¥ `entity_input!` ÔÇö GraphQL `InputObject` with explicit SDL `name` (no serde; control plane is GraphQL-native).
#[macro_export]
...
/// @emoji ­ƒÅÀ´©Å `entity_family!` ÔÇö `SimpleObject` + sync `compute_entity_hash` + `ComplexObject::hash` resolver shell.
#[macro_export]
...
/// @emoji ­ƒÅÀ´©Å `meta_arc_titled_entity!` ÔÇö shared Arc/RwLock tag/concept entity (`new`, `new_with_id`, `compute_hash`, `Default`); owner is always [`crate::gql::interfaces::KitGraphParentWeak`] per golden `Entity.owner`.
#[macro_export]
...
/// @emoji ­ƒÅÀ´©Å `meta_quality_entity!` ÔÇö Arc/RwLock quality entity (`new`, `new_with_id`, `compute_hash`, `Default`).
#[macro_export]
macro_rules! meta_quality_entity {
    () => {
        /// @emoji ­ƒÅÀ´©Å SDL `Quality` entity (benchmarks stay value-typed [`Benchmark`] entities).
        #[derive(Debug)]
        pub struct Quality {
            pub id: $crate::id::Id,
            pub owner: async_lock::RwLock<$crate::gql::interfaces::KitGraphParentWeak>,
...
//#endregion ­ƒº¼ entity_dsl

//#region ­ƒåö id

pub mod id {
    //! ­ƒåö Immutable uuid-v7 wrapper used by every entity.
    use async_graphql::{InputValueError, InputValueResult, Scalar, ScalarType, Value};
    use std::fmt;

    /// @emoji ­ƒåö Opaque node identifier (uuidv7 string); GraphQL wire name `ID` per target schema.
    #[derive(Clone, Debug, Default, Eq, Hash, PartialEq)]
    pub struct Id(pub String);

    impl Id {
        /// ­ƒåò Mint a fresh uuid-v7 (timestamped, monotonic).
...
//#region ÔÅ▒´©Å timestamp

pub mod timestamp {
    //! ÔÅ▒´©Å ISO-8601 millisecond-precision timestamp scalar.
    use async_graphql::{InputValueError, InputValueResult, Scalar, ScalarType, Value};
...
//#endregion ÔÅ▒´©Å timestamp

//#region ­ƒÄ¿ color

pub mod color {
    //! ­ƒÄ¿ Hex/CSS color token scalar matching golden `scalar Color`.
    use async_graphql::{InputValueError, InputValueResult, Scalar, ScalarType, Value};
...
pub mod error {
    //! ­ƒÜ¿ Crate-wide error type wired through the event bus as `OperationFailed`.
...
pub mod geom {
    //! ­ƒôÉ Geometry: wire [`VectorInput`], [`PositionInput`], ÔÇª for GraphQL kit inputs; canonical live weak entities live in [`entity`] as `Arc` graph nodes with one Rust kind per SDL weak entity.
    ...
    impl Default for PlaneInput {
        /// @emoji Ôù¡ World XY plane through origin; hydrates kit JSON that omits plane axes.
        fn default() -> Self {
    ...
    /// @emoji ­ƒîì Wire `LocationInput` (lon/lat/alt) for [`entity::Location`].
    #[derive(Clone, Copy, Debug, Default, PartialEq, InputObject)]
    ...
    //#region ­ƒôÉ entity
    pub mod entity {
        //! ­ƒôÉ `Arc` geometry nodes (target WeakEntity / Entity graph shapes); `#[Object]` impls live after [`crate::interface`].
        use std::sync::Arc;
        ...
        /// @emoji ­ƒôì Canonical weak `Coordinate` (live u/v under `RwLock`).
        #[derive(Debug)]
        ...
        impl Coordinate {
            ...
            /// @emoji ­ƒ¬¬ Merkle leaf: id + live u/v (matches [`super::CoordinateInput`] payload).
            pub async fn compute_hash(&self) -> String {
        ...
        /// @emoji Ôåù Canonical weak `Vector`.
        #[derive(Debug)]
        pub struct Vector {
            pub id: Id,
            pub x: RwLock<f64>,
        ...
        impl Vector {
            ...
            /// @emoji ­ƒ¬¬ Merkle leaf: id + live x/y/z.
            pub async fn compute_hash(&self) -> String {
        ...
        /// @emoji Ôùå Canonical weak `Point`.
        #[derive(Debug)]
        pub struct Point {
            pub id: Id,
            pub x: RwLock<f64>,
        ...
        /// @emoji Ôû¡ Canonical weak `Plane` (owns origin + axes).
        #[derive(Debug)]
        pub struct Plane {
            pub id: Id,
        ...
        impl Plane {
            ...
            /// @emoji ­ƒ¬¬ Merkle node: sorted child digests of origin + axes.
            pub async fn compute_hash(&self) -> String {
        ...
        /// @emoji Ôåû Canonical weak `Offset` (piece drag input echo).
        #[derive(Debug)]
        pub struct Offset {
            pub id: Id,
            pub u: RwLock<f64>,
        ...
        impl Offset {
            ...
            /// @emoji ­ƒ¬¬ Merkle leaf: id + live u/v.
            pub async fn compute_hash(&self) -> String {
                let u = *self.u.read().await;
                let v = *self.v.read().await;
        ...
        /// @emoji Ôîû Canonical weak `Position` (center + plane); live state only in child locks.
        #[derive(Debug)]
        pub struct Position {
            pub id: Id,
        ...
        impl Position {
            ...
            /// @emoji ­ƒô© Wire [`super::PositionInput`] from live center + plane child locks (single source of truth).
            ...
            /// @emoji ­ƒ¬¬ Merkle node: live position scalars plus sorted digests of center + plane arcs.
            pub async fn compute_hash(&self) -> String {
        ...
        /// @emoji ­ƒîì Canonical weak `Location` (lon/lat/alt).
        #[derive(Debug)]
        pub struct Location {
            pub id: Id,
        ...
        impl Location {
            ...
            /// @emoji ­ƒ¬¬ Merkle leaf over lon/lat/alt fields.
            pub async fn compute_hash(&self) -> String {
        ...
        /// @emoji ­ƒº¡ Placeholder shell for `Place` (full meta wiring lands with meta lift).
        #[derive(Debug)]
        pub struct Place {
            pub id: Id,
        ...
        impl Place {
            ...
            /// @emoji ­ƒ¬¬ Merkle leaf: id + optional label.
            pub async fn compute_hash(&self) -> String {
    ...
    //#endregion ­ƒôÉ entity
}

//#endregion ­ƒôÉ geom

//#region ­ƒ¬ó gql_relay

/// ­ƒ¬ó Relay `PageInfo` + connection shells for static GraphQL (edges, pageInfo, hash).
#[allow(unused_macros)]
pub mod gql_relay {
    use std::sync::Arc;
    ...
    /// @emoji ­ƒ¬ó Golden `PageInfoEdge` / `PageInfoConnection` (relay shells around [`PageInfo`]).
    #[derive(Clone, Debug, Default, SimpleObject)]
    ...
    impl PageInfoConnection {
        /// @emoji ­ƒ¬ó Golden `PageInfoConnection` with no edges ÔÇö valid `EntityConnection` implementor for empty `owns` shells.
    ...
    crate::entity_family! {
        /// @emoji ­ƒºÀ Kit [`Family`] SDL shell ÔÇö Artifact [`name`]/[`description`]/[`icon`] are persisted kit fields.
        pub struct Family {
            pub id: Id,
            pub name: String,
...
//#endregion ­ƒ¬ó gql_relay

//#region ­ƒ®╣ schema_gap_surfaces

pub mod schema_gap_surfaces {
    //! ­ƒ®╣ SDL-only synthetic relay surfaces for long-tail golden declarations; registered into `Schema::sdl()` so the exported schema reaches the current target declaration set.
...
//#endregion ­ƒ®╣ schema_gap_surfaces

//#region ­ƒÅÀ´©Å meta

pub mod meta {
    //! ­ƒÅÀ´©Å Metadata: DTO [`SimpleObject`] shells plus Arc-backed [`Tag`]/[`Concept`]/[`Quality`] entities (SDL `Entity`).
    use async_graphql::Object;

    use crate::id::Id;
    use crate::timestamp::Timestamp;

    //#region ­ƒº¥ graphql inputs
    crate::entity_input! {
        /// @emoji ­ƒº¥ SDL `AttributeInput` ÔÇö instantiates [`Attribute`] entities on entity create/update paths.
    ...
    crate::entity_input! {
        /// @emoji ­ƒº¥ SDL `QualityInput` (subset aligned to persisted kit fields).
    ...
    //#endregion ­ƒº¥ graphql inputs

    impl AttributeInput {
        /// @emoji Ô×ò Mint a persisted [`Attribute`] from GraphQL input (fresh [`Id`]).
        ...
        /// @emoji ­ƒ¬¬ Rebuild a persisted [`Attribute`] using a caller-supplied id from a normalized operation scope.
    ...
    /// @emoji Ô×ò Expand optional GraphQL attribute entities into minted [`Attribute`] entities.
    ...
    /// @emoji ­ƒ¬¬ Rebuild optional GraphQL attribute entities using the ids already recorded in operation scope.
    ...
    crate::entity_family! {
        pub struct File {
            ...
            pub size: Option<i32>,
            /// @emoji ­ƒôÄ Blob/content digest on the wire (`hash` in JSON); omitted from GraphQL in favor of entity [`File::hash`] resolver.
            #[graphql(skip)]
            pub hash: String,
            pub description: Option<String>,
    ...
    impl Attribute {
        /// @emoji ­ƒî┐ Blake3 leaf over persisted attribute columns (no owner weak refs).
        pub fn compute_entity_hash(&self) -> String {
    ...
    crate::entity_family! {
        ...
        , extra = (
            /// @emoji ­ƒöÄ SDL `Prop.attribute(id)` ÔÇö props carry no attribute bag yet; reserved for kit snapshots.
            pub async fn attribute(&self, #[graphql(name = "id")] _id: Id) -> Option<Attribute> {
                None
            }
        )
    ...
    crate::entity_family! {
        ...
        , extra = (
            /// @emoji ­ƒöÄ SDL `Stat.attribute(id)` ÔÇö stats carry no attribute bag yet; reserved for kit snapshots.
            pub async fn attribute(&self, #[graphql(name = "id")] _id: Id) -> Option<Attribute> {
                None
            }
        )
    ...
    crate::entity_family! {
        pub struct Layer {
            ...
            pub name: String,
            pub description: Option<String>,
            /// @emoji ­ƒÅÀ´©Å Artifact `icon` on SDL `Layer`.
            pub icon: String,
...
//#endregion ­ƒÅÀ´©Å meta

//#region ­ƒ¬¬ hash

pub mod hash {
    //! ­ƒ¬¬ Blake3 Merkle helpers: [`h`] for delimiter-joined parts; [`merkle_node_str`] for ordered own fields plus sorted child digests; [`merkle_collection`] for relay connection hashes.
    use blake3::Hasher;
    ...
    /// @emoji ­ƒöó Canonical `f64` text for hash joins (integral-ish values without trailing `.0`; trims fractional noise).
    ...
    /// @emoji ­ƒî│ Merkle fold: concatenates `own` in order, then **sorted** `children` hex digests (order-independent set hashing).
    ...
    /// @emoji ­ƒ¬ó Relay collection hash: sorted child entity hashes under a stable collection tag.
...
pub mod kit {
    //! ­ƒôª Kit Ôåö Type Ôåö Design entity tree (Arc + interior RwLock per mutable field).

    //#region ­ƒÅá type
    pub mod r#type {
        //! ­ƒÅá Types, their connectors and representations.
        ...
        //#region ­ƒøƒ port
        /// ­ƒöî Kit-level named attachment point; referenced by [`Connector`] and [`super::connection::Side`].
        #[derive(Debug)]
        pub struct Port {
            pub id: Id,
        ...
        //#region ÔÜô connector
        pub struct Connector {
            pub id: Id,
            pub owner_type: Weak<Type>,
            /// @emoji ­ƒÅÀ´©Å SDL `Connector.name` (Artifact).
            pub name: RwLock<String>,
            pub code: RwLock<String>,
            pub description: RwLock<String>,
            /// @emoji ­ƒÅÀ´©Å SDL `Connector.icon` (Artifact).
            pub icon: RwLock<String>,
            /// @emoji ­ƒöù Resolved port pointer (`# data` on the wire).
        ...
        //#endregion ÔÜô connector

        //#region ­ƒÆ¥ representation
        pub struct Representation {
            pub id: Id,
            pub owner_type: Weak<Type>,
            /// @emoji ­ƒÅÀ´©Å SDL `Representation.name` (Artifact).
            pub name: RwLock<String>,
            pub url: RwLock<String>,
            pub description: RwLock<String>,
            /// @emoji ­ƒÅÀ´©Å SDL `Representation.icon` (Artifact).
            pub icon: RwLock<String>,
        ...
        pub struct Type {
            ...
            /// ­ƒºÀ Refreshed from `connectors` before single-id GraphQL lookups (no stale `Id` scans in resolvers).
        ...
        impl Type {
            ...
            /// ­ƒº¥ Insert a workspace kind with caller-controlled external [`Id`] (wasm / JSON snapshot hydration).
            ...
            /// ­ƒºÀ Rebuild weak maps from the live vecs (call before `connector` / `representation` field resolution).
        ...
        //#endregion ­ƒº® blueprint
    }
    //#endregion ­ƒÅá type

    //#region ­ƒÅÿ design
    pub mod design {
        //! ­ƒÅÿ Designs and their pieces, connections, layers, groups.

        //#region Ô¡ò piece
        pub mod piece {
            //! Ô¡ò Piece (instance of a Type or Design within a Design).
            use std::sync::{Arc, Weak};
            ...
            impl Piece {
                ...
                /// ­ƒº¥ Hydrated workspace piece aligned to external JSON id (facade snapshot hydration).
        ...
        //#region ­ƒöù connection
        pub mod connection {
            //! ­ƒöù Connection between two piece sides + the Side value.
            use std::sync::{Arc, Weak};
            ...
            pub struct Side {
                pub id: Id,
                /// @emoji ­ƒöù Owning connection when sides are wired into a [`Connection`].
            ...
            impl Side {
                ...
                /// @emoji ­ƒ¬¬ Blake3 digest over `Side` wire shape: optional `connector` + `designPiece`, then `piece` (matches kit JSON `parent` / `child` objects).
                pub async fn compute_hash(&self) -> String {
            ...
            //#endregion Ôøô´©Å side

            //#region ­ƒöù connection
            pub struct Connection {
                pub id: Id,
                pub owner_design: Weak<super::Design>,
                /// @emoji ­ƒÅÀ´©Å SDL Artifact `name` / `description` / `icon`.
                pub name: RwLock<String>,
                pub description: RwLock<String>,
                pub icon: RwLock<String>,
            ...
            impl Connection {
                /// @emoji ­ƒ¬¬ Blake3 digest over `Connection` wire shape: sorted `attributes`, `parent` / `child` [`Side`] digests, optional `description`, then scalar join fields.
                pub async fn compute_hash(&self) -> String {
        ...
        //#region ­ƒº▒ clump
        /// @emoji ­ƒº▒ SDL `Clump` ÔÇö connected-component bucket for layout (`WeakEntity` projection hook).
        pub struct Clump {
            pub id: Id,
        ...
        pub struct Design {
            ...
            /// ­ƒºÀ Write-side only: external piece [`Id`] ÔåÆ `Weak` (GraphQL `piece(id:)` upgrades here; no vec index table).
        ...
        impl Design {
            ...
            /// ­ƒåò Push a piece into this design's pieces; returns the same Arc (refcount + 1) for the caller.
            ...
            /// @emoji ­ƒùæ Remove a piece from this design's ordered list and external-id index.
            ...
            /// @emoji ­ƒ¬ó Command / GraphQL boundary: resolve a piece by external [`Id`] via the write-side weak map.
        ...
        //#endregion ­ƒÅÿ design
    }
    //#endregion ­ƒÅÿ design

    //#region ­ƒôÜ kit_target_operations
    /// ­ƒº¥ Arc-backed operation `*Input` shells for Quality / Tag / Concept / Port (`schema.golden.graphql` nested Operations block).
    pub mod target_operations {
        ...
        //#endregion ­ƒöû Quality inputs

        //#region ­ƒÅÀ´©Å Tag inputs
        #[derive(Clone, SimpleObject)]
        ...
        //#endregion ­ƒÅÀ´©Å Tag inputs

        //#region ­ƒÆí Concept inputs
        #[derive(Clone, SimpleObject)]
        ...
        //#endregion ­ƒÆí Concept inputs

        //#region ­ƒöî Port inputs
        #[derive(Clone, SimpleObject)]
        ...
        //#endregion ­ƒöî Port inputs
    }
    //#endregion ­ƒôÜ kit_target_operations
    ...
    pub struct Kit {
        ...
        /// ­ƒºÀ Write-side only: external design [`Id`] ÔåÆ `Weak` (GraphQL `design(id:)` upgrades here).
        pub design_weak_by_id: RwLock<HashMap<Id, Weak<design::Design>>>,
        pub types: RwLock<Vec<Arc<r#type::Type>>>,
        /// ­ƒºÀ Write-side: type [`Id`] ÔåÆ `Weak` for GraphQL `type(id:)` (filled on snapshot hydration).
        ...
        pub attributes: RwLock<Vec<Attribute>>,
        pub stats: RwLock<Vec<Stat>>,
        /// ­ƒºÀ Kit-wide tag identity map (all tag owners).
    ...
    impl Kit {
        ...
        /// @emoji ­ƒº¼ Deep-clone this kit graph (dev-backbone `initialKit` projection round-trip) for immutable graph `initialKit` baselines / operation replay.
        ...
        /// @emoji ­ƒôª Single mutation entry: walks canonical [`crate::operation::CanonicalKitDiff`] from [`crate::operation::Operation::to_diff`].
        pub async fn apply_diff(self: &Arc<Self>, diff: &crate::operation::KitDiff) -> Result<(), crate::error::ComposeError> {
            ...
            if let Some(v) = &d.files {
                if *v {
                    return Err(crate::error::ComposeError::invalid("kit diff `files` subtree apply not implemented"));
                }
            }
            if let Some(v) = &d.folders {
                if *v {
                    return Err(crate::error::ComposeError::invalid("kit diff `folders` subtree apply not implemented"));
                }
            }
        ...
        async fn apply_types_collection_diff(self: &Arc<Self>, t: &crate::operation::TypesCollectionDiff) -> Result<(), crate::error::ComposeError> {
            ...
            if !t.added.is_empty() {
                return Err(crate::error::ComposeError::invalid("kit diff `types.added` apply not implemented"));
            }
            Ok(())
        }

        async fn apply_designs_collection_diff(self: &Arc<Self>, d: &crate::operation::DesignsCollectionDiff) -> Result<(), crate::error::ComposeError> {
            for r in &d.removed {
                let id = r.id.clone();
            ...
            for entity in &d.modified {
                ...
                if let Some(pc) = &diff.pieces {
                    for pr in &pc.removed {
                        ...
                        let design = self.design_by_external_id(&design_id).await.ok_or_else(|| crate::error::ComposeError::not_found("Design", design_id.as_str()))?;
            ...
            if !d.added.is_empty() {
                return Err(crate::error::ComposeError::invalid("kit diff `designs.added` apply not implemented"));
            }
            Ok(())
        }

        async fn apply_design_piece_added(self: &Arc<Self>, design_id: &Id, entity: &crate::operation::PieceAdded) -> Result<(), crate::error::ComposeError> {
        ...
        async fn apply_design_piece_patch(self: &Arc<Self>, design_id: &Id, piece_id: &Id, pdiff: &crate::operation::PiecePatch) -> Result<(), crate::error::ComposeError> {
            use crate::geom::entity::Position as GeomPosition;
            let design = self.design_by_external_id(design_id).await.ok_or_else(|| crate::error::ComposeError::not_found("Design", design_id.as_str()))?;
            let piece = design.piece_by_external_id(piece_id).await.ok_or_else(|| crate::error::ComposeError::not_found("Piece", piece_id.as_str()))?;
            if pdiff.fix_piece {
        ...
        async fn apply_tags_collection_diff(self: &Arc<Self>, t: &crate::operation::TagsCollectionDiff) -> Result<(), crate::error::ComposeError> {
            for r in &t.removed {
                self.delete_tag_by_id(&r.id).await?;
            }
            for m in &t.modified {
                let tag = self.find_tag(&m.tag.id).await.ok_or_else(|| crate::error::ComposeError::not_found("Tag", m.tag.id.as_str()))?;
        ...
        async fn apply_concepts_collection_diff(self: &Arc<Self>, c: &crate::operation::ConceptsCollectionDiff) -> Result<(), crate::error::ComposeError> {
            for r in &c.removed {
                self.delete_concept_by_id(&r.id).await?;
            }
            for m in &c.modified {
                let concept = self.find_concept(&m.concept.id).await.ok_or_else(|| crate::error::ComposeError::not_found("Concept", m.concept.id.as_str()))?;
                if let Some(s) = &m.diff.name {
        ...
        async fn apply_qualities_collection_diff(self: &Arc<Self>, q: &crate::operation::QualitiesCollectionDiff) -> Result<(), crate::error::ComposeError> {
            for r in &q.removed {
                self.delete_quality_by_id(&r.id).await?;
            }
            for m in &q.modified {
                let quality = self.find_quality(&m.quality.id).await.ok_or_else(|| crate::error::ComposeError::not_found("Quality", m.quality.id.as_str()))?;
                if m.diff.description.is_some() {
        ...
        /// @emoji ­ƒ¬¬ Inlined tag creation for [`Kit::apply_diff`] (no public mutator surface).
        ...
        async fn apply_create_concept_scoped(self: &Arc<Self>, owner_id: &Id, concept_id: &Id, attribute_ids: &[Id], input: crate::meta::ConceptInput) -> Result<(), crate::error::ComposeError> {
        ...
        async fn apply_create_quality_scoped(self: &Arc<Self>, owner_id: &Id, quality_id: &Id, attribute_ids: &[Id], benchmark_ids: &[Id], input: crate::meta::QualityInput) -> Result<(), crate::error::ComposeError> {
            let slot = self.resolve_quality_owner_slot(owner_id).await?;
            if !benchmark_ids.is_empty() {
                return Err(crate::error::ComposeError::invalid("quality benchmark ids are not supported yet"));
            }
            let attrs = crate::meta::attributes_from_inputs_with_ids(input.attributes.clone(), attribute_ids)?;
        ...
        /// @emoji ­ƒöÄ Locate a representation by id across all kit types.
        ...
        /// @emoji ­ƒöÄ Locate a connector by id across all kit types.
        ...
        /// @emoji ­ƒ¬ó Resolve SDL `TagInput` owner (`Kit` root id, `Type` id, or `Representation` id).
        ...
        /// @emoji ­ƒ¬ó Resolve SDL `ConceptInput` owner (`Kit` or `Type`).
        ...
        /// @emoji ­ƒ¬ó Resolve SDL `QualityInput` owner (kit/type/representation/connector/design).
        ...
        /// @emoji Ô×ò Create [`Tag`], register globally, append to owner collection.
        ...
        /// @emoji ­ƒ¬¬ Create [`Tag`] from a normalized operation scope carrying every referenced id.
        ...
        /// @emoji ­ƒùæ Remove a tag from its owner vec + id map.
        ...
        /// @emoji Ô×ò Create [`Concept`] under kit or type.
        ...
        /// @emoji ­ƒ¬¬ Create [`Concept`] from a normalized operation scope carrying every referenced id.
        ...
        /// @emoji ­ƒùæ Remove a concept from its owner vec + id map.
        ...
        /// @emoji Ô×ò Create [`Quality`] under resolved owner (kit/type/representation/connector/design).
        ...
        /// @emoji ­ƒ¬¬ Create [`Quality`] from a normalized operation scope carrying every referenced id.
        ...
        /// @emoji ­ƒùæ Remove a quality from its owner vec + id map.
        ...
        /// @emoji ­ƒåò Insert (or look up) a design by id, returning the shared Arc (maintains [`Kit::design_weak_by_id`]).
        ...
        /// ­ƒºÀ Single external-id translation ÔåÆ opaque handle + shared [`Arc`] for all subsequent hot-path graph work.
        ...
        /// @emoji ­ƒöü Clears every **layout** nodeÔÇÖs placed pieces and piece slot maps so [`crate::kit_backbone`] can replay without duplicating projections; kit metadata and empty layout shells stay resident (detach leaves this graph materialized in memory).
...
pub mod vcs {
    ...
    //#region ­ƒÅÀ´©Å golden_version_kind
    /// @emoji ­ƒÅÀ´©Å SDL `VersionKind` (`INITIAL_KIT` | `MATERIALIZED`) on golden `Version`.
    ...
    //#endregion ­ƒÅÀ´©Å golden_version_kind
    ...
    pub struct Change {
        ...
        /// @emoji ­ƒô£ Forward [`operation::Operation`] steps (materialized via `Kit::apply_diff`; persisted `kitDiff` in bundles is derived from each op's `to_diff` at projection time).
        pub forwards: RwLock<Vec<operation::Operation>>,
        /// @emoji ­ƒô£ Backward companion operations for explicit undo/redo (same pipeline).
        pub backwards: RwLock<Vec<operation::Operation>>,
        /// @emoji ­ƒåö Stable operation-log ids aligned with bundle `OperationStep.id` (see `forwardOperationRecordIds`).
        pub forward_record_ids: RwLock<Vec<Id>>,
        pub backward_record_ids: RwLock<Vec<Id>>,
    }

    /// ­ƒöù Weak owner matching SDL `Alternative | Checkpoint` for persisted [`Change`] entities.
    #[derive(Clone)]
    ...
    impl Change {
        ...
        /// @emoji ­ƒöù Ordered operation record ids constituting the forwards side (bundle `OperationLog` ids) when persisted.
        ...
        /// @emoji ­ƒöù Ordered operation record ids for backwards / inverse application when persisted separately from `OperationKind`.
    ...
    //#region ­ƒÆ╝ edit
    /// @emoji ­ƒöù [`Edit`] owner: [`TheKit`] lives on [`Graph`]; [`Alternative`] is its own [`Workspace`](../../../schema/graphql/schema.golden.graphql) (SDL `Workspace`).
    pub enum EditOwner {
        TheKit(Weak<Graph>),
        Alternative(Weak<Alternative>),
    }
    ...
    /// @emoji ­ƒº¥ Flatten [`Edit`] entities into target-schema [`Change`](../../../schema/graphql/schema.golden.graphql) entities for a [`Workspace`](../../../schema/graphql/schema.golden.graphql).
    ...
    //#region ­ƒôì kit read point
    /// @emoji ­ƒôì GraphQL input for [`Graph::materialized_kit_at_point`] (wip anchors).
    ...
    //#endregion ­ƒôì kit read point

    //#region ­ƒôû read write version
    /// @emoji ­ƒôû Placeholder read-side `ReadVersion` shell (golden schema).
    ...
    /// @emoji ­ƒôû Placeholder write-side `WriteVersion` shell (golden schema).
    ...
    //#endregion ­ƒôû read write version

    //#region ­ƒ¬º checkpoint
    pub struct Checkpoint {
        pub id: Id,
        pub timestamp: RwLock<Option<Timestamp>>,
        pub authors: RwLock<Vec<Author>>,
        /// @emoji ­ƒöù Owning graph for [`Checkpoint::initial`] / [`Checkpoint::kit`] (single [`Graph::initial_kit`] baseline, not per-checkpoint kit storage).
    ...
    impl Checkpoint {
        ...
        /// @emoji ­ƒöù SDL `Checkpoint.parent` ÔÇö prior checkpoint in the spine (none for the seed checkpoint).
        pub async fn parent(&self) -> Option<Arc<Checkpoint>> {
            self.parent_checkpoint.read().await.upgrade()
        }

        /// @emoji ­ƒöù SDL `Checkpoint.ancestors` ÔÇö ordered walk up `parent` (immediate parent first).
        ...
        /// @emoji ­ƒî▒ SDL `Checkpoint.initial` ÔÇö graph-level [`Graph::initial_kit`] baseline (saved once per graph, not duplicated on each checkpoint).
        ...
        /// @emoji ­ƒôª SDL `Checkpoint.kit` ÔÇö materialized kit at this anchor (`TheKit` wip parent tip or alternative spine tip); other checkpoints fall back to `initial` until checkpoint-scoped replay exists.
    ...
    //#endregion ­ƒ¬º checkpoint

    //#region ­ƒº¡ the kit version
    pub struct TheKit {
        pub owner_graph: Weak<Graph>,
    }

    impl TheKit {
        /// @emoji ­ƒº¡ SDL [`TheKit`](../../../schema/graphql/schema.golden.graphql) ÔÇö [`Workspace`] on [`Graph`] with `savedChanges` / `unsavedChanges` / `kit`.
    ...
    //#endregion ­ƒº¡ the kit version

    //#region ­ƒî▒ alternative
    ...
    impl Alternative {
        ...
        /// @emoji Ô£Å´©Å Ensure an unsaved [`Edit`] exists for `edit_id` on this [`Alternative`](../../../schema/graphql/schema.golden.graphql) (SDL `unsavedChanges`).
    ...
    //#endregion ­ƒî▒ alternative

    /// @emoji ­ƒôª Cached materialized [`Kit`] for a [`Workspace`](../../../schema/graphql/schema.golden.graphql) (`workspace_id` + `change_seq`).
    ...
    pub struct Graph {
        ...
        /// @emoji ­ƒÅù´©Å Mutable working [`Kit`] used while replaying [`Operation`]s for materialized [`TheKit.kit`](../../../schema/graphql/schema.golden.graphql) / [`Alternative.kit`](../../../schema/graphql/schema.golden.graphql).
    ...
    impl Graph {
        /// ­ƒåò Build a brand-new Graph; seeds [`Graph::mutable_kit`] from a deep-cloned empty [`Kit`] so [`Graph::initial_kit`] baselines never alias live mutation.
        pub async fn new() -> Arc<Self> {
            let id = Id::new().await;
        ...
        /// @emoji ­ƒöù Upgrade `&Graph` to [`Arc`] via the cyclic weak slot (panics if weak is unset).
        pub fn arc_here(&self) -> Arc<Graph> {
            self.self_weak.lock().ok().and_then(|slot| slot.upgrade()).expect("Graph.self_weak upgrade")
        }

        /// @emoji ­ƒ¬¬ Map persisted bundle anchor `the-kit` onto this graph's [`TheKit`](../../../schema/graphql/schema.golden.graphql) [`Workspace`](../../../schema/graphql/schema.golden.graphql) id ([`Graph::id`]).
        ...
        /// @emoji ­ƒôª [`TheKit.kit`](../../../schema/graphql/schema.golden.graphql) ÔÇö materialized [`Kit`] for the graph's [`TheKit`](../../../schema/graphql/schema.golden.graphql) [`Workspace`](../../../schema/graphql/schema.golden.graphql).
        pub async fn materialized_head_kit(self: &Arc<Self>) -> Arc<Kit> {
            self.ensure_default_checkpoint_for_the_kit().await;
            self.materialized_kit_for_workspace(&self.id).await
        }

        /// @emoji ­ƒôª Same as [`Graph::materialized_head_kit`] but callable from `&Graph` resolvers.
        pub async fn materialized_head_kit_from_ref(&self) -> Arc<Kit> {
            self.arc_here().materialized_head_kit().await
        }

        /// @emoji ­ƒºè Invalidate lazily materialized kit cache (abort / record operation).
        pub async fn invalidate_materialized_cache(self: &Arc<Self>) {
            *self.materialized_cache.write().await = None;
        }

        /// @emoji ­ƒº¥ SDL `TheKit.savedChanges` ÔÇö [`ChangeConnection`](../../../schema/graphql/schema.golden.graphql) for this graph's [`TheKit`](../../../schema/graphql/schema.golden.graphql).
        ...
        /// @emoji ­ƒº¥ SDL `TheKit.unsavedChanges` ÔÇö [`ChangeConnection`](../../../schema/graphql/schema.golden.graphql) for this graph's [`TheKit`](../../../schema/graphql/schema.golden.graphql).
        ...
        /// @emoji ­ƒôÄ Ordered saved then unsaved [`Edit`] entities for a [`Workspace`](../../../schema/graphql/schema.golden.graphql) id ([`TheKit`](../../../schema/graphql/schema.golden.graphql) = [`Graph::id`], [`Alternative`](../../../schema/graphql/schema.golden.graphql) = [`Alternative::id`]).
        ...
        /// @emoji Ô£Å´©Å Ensure an unsaved [`Edit`] exists on [`TheKit`](../../../schema/graphql/schema.golden.graphql) (SDL `unsavedChanges`).
        ...
        /// @emoji ­ƒôª Deterministic materialized [`Kit`] for a [`Workspace`](../../../schema/graphql/schema.golden.graphql): clone [`Graph::mutable_kit`] and replay recorded [`Operation`] forwards (matches SDL `Workspace.kit` computation).
        ...
        /// @emoji ­ƒôª [`Kit`] materialized for `workspace_id` through `target_edit` / `change_idx` / `forward_idx` (used when persisting each operation's `kitDiff` beside its input).
        ...
        /// @emoji ­ƒôØ Append one forward operation plus backward operations onto the open [`Edit`]'s tail [`Change`], bumping the workspace cache epoch.
        pub async fn record_operation_in_open_transaction(self: &Arc<Self>, workspace_id: &Id, edit_id: &Id, forward: crate::operation::Operation, backwards: Vec<crate::operation::Operation>) -> Result<(), ComposeError> {
            ...
            let tx = if ws == self.id {
                ...
                self.the_kit_unsaved_edits.read().await.iter().find(|t| &t.id == edit_id).cloned().ok_or_else(|| ComposeError::not_found("Edit", edit_id.as_str()))?
            } else if let Some(alt) = self.workspace_alternative(&ws).await {
                let _ = alt.ensure_unsaved_edit(edit_id).await;
                alt.unsaved_edits.read().await.iter().find(|t| &t.id == edit_id).cloned().ok_or_else(|| ComposeError::not_found("Edit", edit_id.as_str()))?
            } else {
                return Err(ComposeError::not_found("Workspace", workspace_id.as_str()));
            };
            let change = {
        ...
        /// @emoji ­ƒî▒ Ensure a seed [`Checkpoint`] exists and [`TheKit`](../../../schema/graphql/schema.golden.graphql) is anchored (idempotent).
        ...
        /// @emoji ­ƒî▒ Fork a new named [`Alternative`](../../../schema/graphql/schema.golden.graphql) from [`TheKit`](../../../schema/graphql/schema.golden.graphql) or another alternative's tip [`Checkpoint`](../../../schema/graphql/schema.golden.graphql).
        pub async fn create_alternative_from_tip(self: &Arc<Self>, name: String, source_alternative_id: Option<&Id>) -> Result<Id, ComposeError> {
            ...
            if name.is_empty() {
                return Err(ComposeError::invalid("alternative name required"));
            }

            self.ensure_default_checkpoint_for_the_kit().await;

            let parent_cp = match source_alternative_id {
                None => self.the_kit_parent_checkpoint.read().await.upgrade().ok_or_else(|| ComposeError::invalid("theKit workspace has no parent checkpoint"))?,
                Some(aid) => {
                    let alt = {
                        let alts = self.alternatives.read().await;
                        alts.iter().find(|a| &a.id == aid).ok_or_else(|| ComposeError::not_found("Alternative", aid.as_str()))?.clone()
                    };
                    let parent_from_alt = {
                        let start_guard = alt.start.read().await;
                        start_guard.upgrade().ok_or_else(|| ComposeError::invalid("alternative has no start checkpoint"))?
                    };
                    parent_from_alt
                }
            };
        ...
        /// @emoji ­ƒƒó Open a new unsaved [`Edit`] on `workspace_id` (SDL `unsavedChanges`).
        ...
        /// @emoji Ô£à Commit an unsaved [`Edit`]: move it from `unsavedChanges` to `savedChanges` on that [`Workspace`](../../../schema/graphql/schema.golden.graphql).
        pub async fn commit_transaction(self: &Arc<Self>, workspace_id: &Id, edit_id: &Id) -> Result<(), ComposeError> {
            if self.workspace_is_the_kit(workspace_id).await {
                let tx = {
                    ...
                    let pos = txs.iter().position(|t| &t.id == edit_id).ok_or_else(|| ComposeError::not_found("Edit", edit_id.as_str()))?;
                    txs.remove(pos)
                };
        ...
        /// @emoji Ôøö Drop an unsaved [`Edit`] from `unsavedChanges` on that [`Workspace`](../../../schema/graphql/schema.golden.graphql).
        ...
        /// @emoji ­ƒôª Golden `Checkpoint.kit` / `KitReadPointInput.checkpointId` ÔÇö WIP parent checkpoint resolves like `TheKit.kit`; alternative tip like `Alternative.kit`; otherwise graph `initialKit` until per-checkpoint edit slices exist.
        pub async fn materialized_kit_for_checkpoint_id(self: &Arc<Self>, checkpoint_id: &Id) -> Result<Arc<Kit>, ComposeError> {
            ...
            if !found {
                return Err(ComposeError::not_found("Checkpoint", checkpoint_id.as_str()));
            }
        ...
        /// @emoji ­ƒôì Materialized [`Kit`] at a [`KitReadPointInput`] anchor ([`TheKit`](../../../schema/graphql/schema.golden.graphql), [`Checkpoint`](../../../schema/graphql/schema.golden.graphql), [`Alternative`](../../../schema/graphql/schema.golden.graphql)).
        pub async fn materialized_kit_at_point(self: &Arc<Self>, p: KitReadPointInput) -> Result<Arc<Kit>, ComposeError> {
            ...
            if let Some(aid) = p.alternative_id.clone() {
                let alts = self.alternatives.read().await;
                let alt = alts.iter().find(|a| a.id == aid).cloned().ok_or_else(|| ComposeError::not_found("Alternative", aid.as_str()))?;
        ...
        /// @emoji ­ƒöº Apply `createFixedPiece` via [`Graph::record_operation_in_open_transaction`] (tests / golden replay).
        pub async fn apply_create_fixed_piece(
            ...
            name: Option<String>,
            description: Option<String>,
        ) -> Result<(Arc<crate::kit::design::piece::Piece>,), ComposeError> {
            ...
            let piece = after.design_by_external_id(&design_id).await.ok_or_else(|| ComposeError::not_found("Design", design_id.as_str()))?.piece_by_external_id(&piece_id).await.ok_or_else(|| ComposeError::not_found("Piece", piece_id.as_str()))?;
            Ok((piece,))
        }

        /// ­ƒø░´©Å WIP bootstrap: hydrate [`Graph::mutable_kit`] from initial kit projection JSON via [`crate::kit_backbone::graph_new_overlay_from_initial_projection_json`].
        pub async fn new_overlay_from_initial_kit_projection_json(json: serde_json::Value) -> Result<Arc<Self>, ComposeError> {
    ...
    impl Session {
        ...
        /// @emoji ­ƒîÉ Same navigation as WIP [`Graph`] ÔÇö resolved via [`crate::worker::ParentStore::wip_graph`] for the active runtime.
    ...
    //#endregion ­ƒæñ session

    //#region ÔÜá´©Å conflict
    pub struct Conflict {
        pub id: Id,
    ...
    impl Conflict {
        /// @emoji ­ƒ¬¬ Merkle leaf: id, optional backbone tip, reason, created-at (relay + GraphQL `Conflict.hash`).
        pub async fn compute_hash(&self) -> String {
    ...
    //#endregion ÔÜá´©Å conflict
}
//#endregion ­ƒî┐ vcs

//#region ­ƒºÀ interface
...
pub mod interface {
    ...
    /// @emoji ­ƒöÄ Resolve a global id against WIP + authoritative graphs, sessions, and conflicts.
    ...
    /// @emoji ­ƒôì Resolve `pieceInDesign` on the WIP graph line.
    ...
    /// @emoji ­ƒôì Resolve `alternativePieceKind` from the WIP kit: find the piece in any design and return the GraphQL `Blueprint` union tag (`Type` or `Design`).
    ...
    /// @emoji ­ƒôì WeakEntity + entity shell for [`Coordinate`] (SDL `Coordinate`).
    ...
    /// @emoji ­ƒîì WeakEntity shell for [`Location`] (SDL `Location`).
...
//#endregion ­ƒºÀ interface

//#region ÔÜÖ´©Å operation

pub mod operation {
    //! ÔÜÖ´©Å Operation entities and their inputs. Operations carry `Arc<Entity>` payloads so the
    ...
    /// @emoji ­ƒôª `Id` reference wrapper for kit diff collection entities.
    ...
    /// @emoji ­ƒôª One `tags.added[]` entry (owner + ids + GraphQL [`TagInput`]).
    #[derive(Clone, Debug, PartialEq)]
    pub struct TagAdded {
        pub owner_id: Id,
        pub id: Id,
    ...
    /// @emoji ­ƒôª Sparse `concepts` triple.
    #[derive(Clone, Debug, Default, PartialEq)]
    ...
    /// @emoji ­ƒôª Sparse `qualities` triple (kit-level).
    #[derive(Clone, Debug, Default, PartialEq)]
    ...
    //#region ­ƒöûcanonical_kit_types_designs_mod
    /// @emoji ­ƒôª Sparse `types` triple.
    #[derive(Clone, Debug, Default, PartialEq)]
    ...
    /// @emoji ­ƒôª One `types.modified[]` entity.
    #[derive(Clone, Debug, PartialEq)]
    ...
    /// @emoji ­ƒôª Sparse `designs` triple.
    #[derive(Clone, Debug, Default, PartialEq)]
    ...
    /// @emoji ­ƒôª One `designs.modified[]` entity.
    #[derive(Clone, Debug, PartialEq)]
    ...
    //#endregion ­ƒöûcanonical_kit_types_designs_mod

    /// @emoji ­ƒôª `files` / `folders` / `authors`: `None` omitted; `Some(false)` trivial; `Some(true)` means unsupported non-empty subtree.
    pub type KitAuxSubtree = Option<bool>;

    /// @emoji ­ƒôª Canonical sparse kit diff aligned with metabolism fixtures (typed collections; aux subtrees use tri-state `KitAuxSubtree` flags).
    #[derive(Clone, Debug, Default, PartialEq)]
    ...
    impl KitDiff {
        /// @emoji ­ƒöù Shallow-merge `other` scalar/collection fields into `self.0` (used when coalescing multi-target diffs).
    ...
    //#endregion ­ƒöû canonical_kit_diff

    /// @emoji ­ƒÜ½ Empty payload marker for scope-only operations.
    #[derive(Clone, Debug, Default, PartialEq)]
    pub struct NoInput;

    /// @emoji ­ƒ¬¬ Rename-kit scope; the kit id is implicit from the target graph line.
    #[derive(Clone, Debug, Default, PartialEq)]
    pub struct RenameKitScope;

    /// @emoji ­ƒ¬¬ Single entity scope.
    #[derive(Clone, Debug, Default, PartialEq)]
    pub struct EntityScope {
        pub entity_id: Id,
    }

    /// @emoji ­ƒ¬¬ Single tag scope.
    #[derive(Clone, Debug, Default, PartialEq)]
    pub struct TagScope {
        pub tag_id: Id,
    }

    /// @emoji ­ƒ¬¬ Multi-tag scope.
    #[derive(Clone, Debug, Default, PartialEq)]
    pub struct TagsScope {
        pub tag_ids: Vec<Id>,
    }

    /// @emoji ­ƒ¬¬ Single concept scope.
    #[derive(Clone, Debug, Default, PartialEq)]
    pub struct ConceptScope {
        pub concept_id: Id,
    }

    /// @emoji ­ƒ¬¬ Single quality scope.
    #[derive(Clone, Debug, Default, PartialEq)]
    pub struct QualityScope {
        pub quality_id: Id,
    }

    /// @emoji ­ƒ¬¬ Create-tag scope with owner id plus all pre-minted ids.
    #[derive(Clone, Debug, Default, PartialEq)]
    ...
    /// @emoji ­ƒ¬¬ Batch create-tag scope with owner id plus all pre-minted ids.
    #[derive(Clone, Debug, Default, PartialEq)]
    ...
    /// @emoji ­ƒ¬¬ Create-concept scope with owner id plus all pre-minted ids.
    #[derive(Clone, Debug, Default, PartialEq)]
    ...
    /// @emoji ­ƒ¬¬ Create-quality scope with owner id plus all pre-minted ids.
    #[derive(Clone, Debug, Default, PartialEq)]
    ...
    /// @emoji ­ƒ¬¬ Create-piece scope with the parent design id and the pre-minted piece id.
    #[derive(Clone, Debug, Default, PartialEq)]
    ...
    /// @emoji ­ƒ¬¬ Single piece-in-design scope.
    #[derive(Clone, Debug, Default, PartialEq)]
    ...
    /// @emoji ­ƒ¬¬ Multi-piece-in-design scope.
    #[derive(Clone, Debug, Default, PartialEq)]
    ...
    /// @emoji Ô£Å´©Å Rename-kit payload.
    #[derive(Clone, Debug, Default, PartialEq)]
    pub struct RenameKitInput {
        pub name: String,
    }

    /// @emoji Ô£Å´©Å Rename-tag payload.
    #[derive(Clone, Debug, Default, PartialEq)]
    pub struct RenameTagInput {
        pub name: String,
    }

    /// @emoji Ô£Å´©Å Generic description payload.
    #[derive(Clone, Debug, Default, PartialEq)]
    pub struct ChangeDescriptionInput {
        pub description: Option<String>,
    }

    /// @emoji Ô£Å´©Å Generic icon payload.
    #[derive(Clone, Debug, Default, PartialEq)]
    pub struct ChangeIconInput {
        pub icon: Option<String>,
    }

    /// @emoji Ô£Å´©Å Generic image payload.
    #[derive(Clone, Debug, Default, PartialEq)]
    pub struct ChangeImageInput {
        pub image: Option<String>,
    }

    /// @emoji Ô£Å´©Å Batch tag payload.
    #[derive(Clone, Debug, Default, PartialEq)]
    pub struct CreateTagsInput {
        pub tags: Vec<TagInput>,
    }

    /// @emoji Ô£Å´©Å Fixed-piece creation payload.
    #[derive(Clone, Debug, Default, PartialEq)]
    ...
    /// @emoji Ô£Å´©Å Piece drag payload.
    #[derive(Clone, Debug, Default, PartialEq)]
    pub struct DragPieceInput {
        pub offset: OffsetInput,
    }

    /// @emoji ­ƒº¡ Shared scope payload: every distinct id-shape used across [`Operation`] commands.
    #[derive(Clone, Debug, PartialEq)]
    pub enum Scope {
        Kit,
        Entity { entity_id: Id },
    ...
    /// @emoji ­ƒº¡ Shared non-id input payload reused across commands with the same shape.
    #[derive(Clone, Debug, PartialEq)]
    pub enum Input {
        None,
        Name { name: String },
    ...
    /// @emoji ­ƒº® Normalized  operation surface: every variant is `{ scope: Scope, input: Input }`.
    #[derive(Clone, Debug, PartialEq)]
    pub enum Operation {
    ...
    impl Operation {
        ...
        /// @emoji ­ƒöó Stable digest for deterministic diff ids (no JSON serde).
        ...
        pub async fn to_diff(&self, kit: &Arc<crate::kit::Kit>) -> Result<KitDiff, ComposeError> {
            match self {
                Operation::RenameKit { scope, input } => {
                    let Scope::Kit = scope else {
                        return Err(ComposeError::invalid("renameKit expects Scope::Kit"));
                    };
                    let Input::Name { name } = input else {
                        return Err(ComposeError::invalid("renameKit expects Input::Name"));
                    };
                ...
                Operation::ChangeDescription { scope, input } => {
                    let Scope::Entity { entity_id } = scope else {
                        return Err(ComposeError::invalid("changeDescription expects Scope::Entity"));
                    };
                    let Input::Description { description } = input else {
                        return Err(ComposeError::invalid("changeDescription expects Input::Description"));
                    };
                    ...
                    Err(ComposeError::not_found("DescriptionEntity", entity_id.as_str()))
                }
                Operation::ChangeIcon { scope, input } => {
                    let Scope::Entity { entity_id } = scope else {
                        return Err(ComposeError::invalid("changeIcon expects Scope::Entity"));
                    };
                    let Input::Icon { icon } = input else {
                        return Err(ComposeError::invalid("changeIcon expects Input::Icon"));
                    };
                ...
                Operation::ChangeImage { scope, input } => {
                    let Scope::Entity { entity_id } = scope else {
                        return Err(ComposeError::invalid("changeImage expects Scope::Entity"));
                    };
                    let Input::Image { image } = input else {
                        return Err(ComposeError::invalid("changeImage expects Input::Image"));
                    };
                    ...
                    Err(ComposeError::not_found("ImageEntity", entity_id.as_str()))
                }
                Operation::CreateTag { scope, input } => {
                    let Scope::CreateTag { owner_id, tag_id, attribute_ids } = scope else {
                        return Err(ComposeError::invalid("createTag expects Scope::CreateTag"));
                    };
                    let Input::Tag { tag } = input else {
                        return Err(ComposeError::invalid("createTag expects Input::Tag"));
                    };
                    kit.resolve_tag_owner_slot(owner_id).await?;
                    if kit.find_tag(tag_id).await.is_some() {
                        return Err(ComposeError::invalid(format!("Tag already exists: {}", tag_id.as_str())));
                    }
                ...
                Operation::CreateTags { scope, input } => {
                    let Scope::CreateTags { owner_id, tag_ids, attribute_ids } = scope else {
                        return Err(ComposeError::invalid("createTags expects Scope::CreateTags"));
                    };
                    let Input::Tags { tags } = input else {
                        return Err(ComposeError::invalid("createTags expects Input::Tags"));
                    };
                    if tag_ids.len() != tags.len() || attribute_ids.len() != tags.len() {
                        return Err(ComposeError::invalid("createTags scope length mismatch"));
                    }
                    kit.resolve_tag_owner_slot(owner_id).await?;
                    let mut added = Vec::new();
                    for (index, tag) in tags.iter().enumerate() {
                        if kit.find_tag(&tag_ids[index]).await.is_some() {
                            return Err(ComposeError::invalid(format!("Tag already exists: {}", tag_ids[index].as_str())));
                        }
                ...
                Operation::DeleteTag { scope, .. } => {
                    let Scope::Tag { tag_id } = scope else {
                        return Err(ComposeError::invalid("deleteTag expects Scope::Tag"));
                    };
                ...
                Operation::DeleteTags { scope, .. } => {
                    let Scope::Tags { tag_ids } = scope else {
                        return Err(ComposeError::invalid("deleteTags expects Scope::Tags"));
                    };
                ...
                Operation::RenameTag { scope, input } => {
                    let Scope::Tag { tag_id } = scope else {
                        return Err(ComposeError::invalid("renameTag expects Scope::Tag"));
                    };
                    let Input::Name { name } = input else {
                        return Err(ComposeError::invalid("renameTag expects Input::Name"));
                    };
                    ensure_tag(kit, tag_id).await?;
                    Ok(KitDiff(CanonicalKitDiff {
                ...
                Operation::CreateConcept { scope, input } => {
                    let Scope::CreateConcept { owner_id, concept_id, attribute_ids } = scope else {
                        return Err(ComposeError::invalid("createConcept expects Scope::CreateConcept"));
                    };
                    let Input::Concept { concept } = input else {
                        return Err(ComposeError::invalid("createConcept expects Input::Concept"));
                    };
                    kit.resolve_concept_owner_slot(owner_id).await?;
                    if kit.find_concept(concept_id).await.is_some() {
                        return Err(ComposeError::invalid(format!("Concept already exists: {}", concept_id.as_str())));
                    }
                ...
                Operation::DeleteConcept { scope, .. } => {
                    let Scope::Concept { concept_id } = scope else {
                        return Err(ComposeError::invalid("deleteConcept expects Scope::Concept"));
                    };
                ...
                Operation::CreateQuality { scope, input } => {
                    let Scope::CreateQuality { owner_id, quality_id, attribute_ids, benchmark_ids } = scope else {
                        return Err(ComposeError::invalid("createQuality expects Scope::CreateQuality"));
                    };
                    let Input::Quality { quality } = input else {
                        return Err(ComposeError::invalid("createQuality expects Input::Quality"));
                    };
                    kit.resolve_quality_owner_slot(owner_id).await?;
                    if kit.find_quality(quality_id).await.is_some() {
                        return Err(ComposeError::invalid(format!("Quality already exists: {}", quality_id.as_str())));
                    }
                    validate_attribute_ids(quality.attributes.as_ref().map(|items| items.len()).unwrap_or_default(), attribute_ids)?;
                    if !benchmark_ids.is_empty() {
                        return Err(ComposeError::invalid("quality benchmark ids are not supported yet"));
                    }
                    Ok(KitDiff(CanonicalKitDiff {
                        qualities: Some(QualitiesCollectionDiff {
                ...
                Operation::DeleteQuality { scope, .. } => {
                    let Scope::Quality { quality_id } = scope else {
                        return Err(ComposeError::invalid("deleteQuality expects Scope::Quality"));
                    };
                ...
                Operation::CreateFixedPiece { scope, input } => {
                    ...
                    if !attribute_ids.is_empty() {
                        return Err(ComposeError::invalid("piece attribute ids are not supported yet"));
                    }
                    Ok(KitDiff(CanonicalKitDiff {
                        designs: Some(DesignsCollectionDiff {
                ...
                Operation::DeletePieceInDesign { scope, .. } => {
                    let Scope::PieceInDesign { design_id, piece_id } = scope else {
                        return Err(ComposeError::invalid("deletePieceInDesign expects Scope::PieceInDesign"));
                    };
                ...
                Operation::DragPieceInDesign { scope, input } => {
                    let Scope::PieceInDesign { design_id, piece_id } = scope else {
                        return Err(ComposeError::invalid("dragPieceInDesign expects Scope::PieceInDesign"));
                    };
                    let Input::Offset { offset } = input else {
                        return Err(ComposeError::invalid("dragPieceInDesign expects Input::Offset"));
                    };
                    ensure_piece(kit, design_id, piece_id).await?;
                    Ok(KitDiff(CanonicalKitDiff {
                        designs: Some(DesignsCollectionDiff {
                ...
                Operation::DragPiecesInDesign { scope, input } => {
                    let Scope::PiecesInDesign { design_id, piece_ids } = scope else {
                        return Err(ComposeError::invalid("dragPiecesInDesign expects Scope::PiecesInDesign"));
                    };
                    let Input::Offset { offset } = input else {
                        return Err(ComposeError::invalid("dragPiecesInDesign expects Input::Offset"));
                    };
        ...
        pub async fn to_backwards(&self, kit: &Arc<crate::kit::Kit>) -> Result<Vec<Operation>, ComposeError> {
            match self {
                    ...
                    for tag_id in tag_ids {
                        ...
                        if let Some(existing_owner_id) = &owner_id {
                            if existing_owner_id != &current_owner_id {
                                return Err(ComposeError::invalid("deleteTags backwards requires a single owner id"));
                            }
                        } else {
                ...
                Operation::DeleteQuality { scope, .. } => {
                    ...
                    if !benchmarks.is_empty() {
                        return Err(ComposeError::invalid("deleteQuality backwards does not support benchmarks yet"));
                    }
    ...
    fn validate_attribute_ids(expected: usize, actual: &[Id]) -> Result<(), ComposeError> {
        if expected != actual.len() {
            return Err(ComposeError::invalid(format!("attribute id count mismatch: expected {}, got {}", expected, actual.len())));
        }
        Ok(())
    }

    async fn ensure_tag(kit: &Arc<crate::kit::Kit>, tag_id: &Id) -> Result<Arc<crate::meta::Tag>, ComposeError> {
        kit.find_tag(tag_id).await.ok_or_else(|| ComposeError::not_found("Tag", tag_id.as_str()))
    }

    async fn ensure_concept(kit: &Arc<crate::kit::Kit>, concept_id: &Id) -> Result<Arc<crate::meta::Concept>, ComposeError> {
        kit.find_concept(concept_id).await.ok_or_else(|| ComposeError::not_found("Concept", concept_id.as_str()))
    }

    async fn ensure_quality(kit: &Arc<crate::kit::Kit>, quality_id: &Id) -> Result<Arc<crate::meta::Quality>, ComposeError> {
        kit.find_quality(quality_id).await.ok_or_else(|| ComposeError::not_found("Quality", quality_id.as_str()))
    }

    async fn ensure_piece(kit: &Arc<crate::kit::Kit>, design_id: &Id, piece_id: &Id) -> Result<Arc<crate::kit::design::piece::Piece>, ComposeError> {
        let design = kit.design_by_external_id(design_id).await.ok_or_else(|| ComposeError::not_found("Design", design_id.as_str()))?;
        design.piece_by_external_id(piece_id).await.ok_or_else(|| ComposeError::not_found("Piece", piece_id.as_str()))
    }

    async fn entity_description(kit: &Arc<crate::kit::Kit>, entity_id: &Id) -> Result<Option<String>, ComposeError> {
        let kid = kit.workspace_kit_id().await;
        if entity_id == &kid || entity_id == &kit.id {
    ...
    async fn entity_image(kit: &Arc<crate::kit::Kit>, entity_id: &Id) -> Result<Option<String>, ComposeError> {
        let kid = kit.workspace_kit_id().await;
        if entity_id == &kid || entity_id == &kit.id {
    ...
    pub(crate) async fn tag_owner_id(kit: &Arc<crate::kit::Kit>, tag: &Arc<crate::meta::Tag>) -> Result<Id, ComposeError> {
        tag.owner.read().await.resolve_workspace_owner_id(kit).await
    }

    pub(crate) async fn concept_owner_id(kit: &Arc<crate::kit::Kit>, concept: &Arc<crate::meta::Concept>) -> Result<Id, ComposeError> {
        concept.owner.read().await.resolve_workspace_owner_id(kit).await
    }

    pub(crate) async fn quality_owner_id(kit: &Arc<crate::kit::Kit>, quality: &Arc<crate::meta::Quality>) -> Result<Id, ComposeError> {
    ...
    //#endregion ­ƒº¡ normalized operation contract
    ...
    impl BackboneKind {
        /// @emoji ­ƒù║´©Å Parse `dev://`, `local://`, `remote://`, or legacy `file://` (dev file path).
        pub fn from_uri(raw: &str) -> Result<(Self, String), crate::error::ComposeError> {
            let u = raw.trim();
    ...
    //#endregion ­ƒº¡ graph workspace + backbone store kind (readable/writable selectors)

    //#region ­ƒô£  operation record (kit bundle / operation log contract)
    /// @emoji ­ƒô£ One persisted  operation: stable id, kind string, JSON payload, monotonic sequence index.
    #[derive(Clone, Debug, Default, async_graphql::SimpleObject)]
    ...
    /// ­ƒîù Sum-type carrying any operation through the event bus / change log (Arc-shared).
    #[derive(Clone, Union)]
    #[graphql(name = "OperationKind")]
    ...
    /// ­ƒ¬ä GraphQL `interface Operation` (each variant is an Arc-shared operation entity).
    ...
    impl OperationInterface {
        /// @emoji ­ƒ¬¬ Stable entity id for relay operation edges / merkle shells.
    ...
    //#endregion ­ƒ¬ä operations

    //#region ­ƒôí commands
    /// ­ƒôí Internal command envelope passed parent ÔåÆ child runtime over the work queue.
    #[derive(Clone, Debug)]
    ...
    /// Ô£à Lightweight signal that a command was accepted (used by `commandSucceeded`).
    #[derive(Clone, Debug, Default, async_graphql::SimpleObject)]
    ...
    /// @emoji ­ƒåö GraphQL `IdResult` ÔÇö command payload id surfaced through `Response.result`.
    #[derive(Clone, Debug)]
    pub struct IdResult {
        pub id: Id,
        pub value: Id,
    ...
    /// @emoji ­ƒôª GraphQL `interface Result` ÔÇö success payloads for `Response.result`.
    ...
    /// @emoji ­ƒô¼ GraphQL command outcome ÔÇö `ok` / `errors` / `result` per golden `Response`.
    #[derive(Clone, Debug)]
    pub struct CommandResponse {
        pub id: Id,
        pub ok: bool,
        pub errors: Option<crate::error::ComposeError>,
    ...
    impl CommandResponse {
        ...
        pub async fn fail(err: crate::error::ComposeError) -> Self {
            let id = Id::new().await;
            Self { id, ok: false, errors: Some(err), result: None }
        }

        pub async fn fail_msg(message: impl Into<String>) -> Self {
            Self::fail(crate::error::ComposeError::invalid(message)).await
        }
    ...
    impl CommandResponse {
        ...
        async fn errors(&self) -> Option<crate::error::ComposeError> {
            self.errors.clone()
        }
    ...
    /// @emoji ­ƒô¼ GraphQL `interface Response` ÔÇö mutation return surface (`CommandResponse` today; operations later).
    #[derive(Clone, Debug, Interface)]
    #[graphql(
        name = "Response",
        field(name = "id", ty = "crate::id::Id"),
        ...
        field(name = "errors", ty = "Option<crate::error::ComposeError>"),
        field(name = "result", ty = "Option<crate::operation::ResultInterface>")
    )]
    pub enum ResponseInterface {
        Command(CommandResponse),
    }
    //#endregion ­ƒôí commands

    /// @emoji ­ƒº® Declarative operation entity registration hook (`operations! { CreatedFixedPiece, ÔÇª }`) ÔÇö expand to typed operation structs + history wiring.
    macro_rules! operations {
        ($($entity:ident),* $(,)?) => {
            /// @emoji ­ƒöó Operation count listed in `operations! { ÔÇª }` (static registry grows toward ~100 SDL operations).
...
//#endregion ÔÜÖ´©Å operation

//#region ­ƒº« kit graph engine

pub mod kit_graph_engine {
    //! @emoji ­ƒº« Core kit graph engine: deterministic projection fingerprints, ephemeral operation diffs, and typed [`crate::operation::Operation`] apply for replay (no serde on the control-plane surface).
    ...
    //#endregion ­ƒöû design_slot

    //#region ­ƒöû projection_fingerprint
    /// @emoji ­ƒöó Stable `projectionFingerprint`: [`h`] over sorted piece centers keyed by design id (matches golden `kit-store.golden.expected`).
    ...
    //#endregion ­ƒöû projection_fingerprint

    //#region ­ƒöû deterministic_diff
    /// @emoji ­ƒôª Deterministic non-persisted diff from operation kind + stable payload digest + projection fingerprint transition.
    ...
    //#endregion ­ƒöû deterministic_diff

    //#region ­ƒöû apply_kit_operation
    /// @emoji ­ƒº¥ Output of [`apply_kit_operation`]: ephemeral diff + optional created entities.
    pub struct AppliedOperation {
        pub diff: operation::Diff,
        pub created_piece: Option<Arc<kit::design::piece::Piece>>,
    }

    /// @emoji ­ƒº® Record one forward [`Operation`] (plus backwards) on `graph` and return deterministic projection metadata.
    ...
    //#endregion ­ƒöû apply_kit_operation
}

//#endregion ­ƒº« kit graph engine
...
pub mod kit_backbone {
    ...
    /// @emoji ­ƒôÄ Mutable slice for hydrate / blob merge paths that must accept block lists or legacy arrays.
    ...
    //#region ­ƒöû dev_backbone_kit_operation_json
    ...
    //#endregion ­ƒöû dev_backbone_canonical_kit_diff_wire_json
    ...
    pub(crate) fn position_input_from_json(v: &serde_json::Value) -> Result<crate::geom::PositionInput, ComposeError> {
        let f2 = |o: &serde_json::Map<String, serde_json::Value>, a: &str, b: &str| -> Result<(f64, f64), ComposeError> {
            let u = o.get(a).and_then(|x| x.as_f64()).ok_or_else(|| ComposeError::invalid("position field"))?;
            let v = o.get(b).and_then(|x| x.as_f64()).ok_or_else(|| ComposeError::invalid("position field"))?;
            Ok((u, v))
        };
        let f3 = |o: &serde_json::Map<String, serde_json::Value>| -> Result<(f64, f64, f64), ComposeError> {
            let x = o.get("x").and_then(|x| x.as_f64()).ok_or_else(|| ComposeError::invalid("position field"))?;
            let y = o.get("y").and_then(|x| x.as_f64()).ok_or_else(|| ComposeError::invalid("position field"))?;
            let z = o.get("z").and_then(|x| x.as_f64()).ok_or_else(|| ComposeError::invalid("position field"))?;
            Ok((x, y, z))
        };
        let center_o = v.get("center").and_then(|x| x.as_object()).ok_or_else(|| ComposeError::invalid("position.center"))?;
        let (u, vv) = f2(center_o, "u", "v")?;
        let plane_o = v.get("plane").and_then(|x| x.as_object()).ok_or_else(|| ComposeError::invalid("position.plane"))?;
        let origin = plane_o.get("origin").and_then(|x| x.as_object()).ok_or_else(|| ComposeError::invalid("plane.origin"))?;
        let (ox, oy, oz) = f3(origin)?;
        let xa = plane_o.get("xAxis").or_else(|| plane_o.get("x_axis")).and_then(|x| x.as_object()).ok_or_else(|| ComposeError::invalid("plane.xAxis"))?;
        let (xx, xy, xz) = f3(xa)?;
        let ya = plane_o.get("yAxis").or_else(|| plane_o.get("y_axis")).and_then(|x| x.as_object()).ok_or_else(|| ComposeError::invalid("plane.yAxis"))?;
        let (yx, yy, yz) = f3(ya)?;
    ...
    fn kit_scope_from_json(v: &serde_json::Value) -> Result<crate::operation::Scope, ComposeError> {
        use crate::operation::Scope;
        ...
        Ok(match k.as_str() {
            ...
            "Tags" => {
                ...
                for x in arr {
                    tag_ids.push(id_from_str(x.as_str().ok_or_else(|| ComposeError::invalid("tag id"))?));
                }
                Scope::Tags { tag_ids }
            }
    ...
    fn attribute_input_from_json(v: &serde_json::Value) -> Result<crate::meta::AttributeInput, ComposeError> {
        let m = v.as_object().ok_or_else(|| ComposeError::invalid("AttributeInput"))?;
    ...
    fn kit_input_from_json(v: &serde_json::Value) -> Result<crate::operation::Input, ComposeError> {
        use crate::operation::Input;
        if let Some(s) = v.as_str() {
            if s == "None" {
        ...
        Ok(match k.as_str() {
            ...
            "Tags" => {
                ...
                let mut tags = Vec::new();
                for t in arr {
                    let m = t.as_object().ok_or_else(|| ComposeError::invalid("tag entity"))?;
                    let attrs = m.get("attributes").and_then(|x| x.as_array()).map(|a| a.iter().map(attribute_input_from_json).collect::<Result<Vec<_>, _>>()).transpose()?;
    ...
    pub(crate) fn kit_operation_from_step_json(v: &serde_json::Value) -> Result<crate::operation::Operation, ComposeError> {
        use crate::operation::Operation;
        let o = v.as_object().ok_or_else(|| ComposeError::invalid("kit operation"))?;
        let (k, inner) = o.iter().next().ok_or_else(|| ComposeError::invalid("empty kit operation"))?;
    ...
    async fn legacy_created_fixed_piece_to_kit_op(input: &serde_json::Value) -> Result<crate::operation::Operation, ComposeError> {
    ...
    pub(crate) async fn kit_operation_from_stored(kind: &str, input: &serde_json::Value) -> Result<crate::operation::Operation, ComposeError> {
        if kind == "createdFixedPiece" {
            return legacy_created_fixed_piece_to_kit_op(input).await;
        }
        kit_operation_from_step_json(input)
    }
    //#endregion ­ƒöû dev_backbone_kit_operation_json

    //#region ­ƒöû dev_backbone_initial_kit_projection
    ...
    pub(crate) async fn hydrate_kit_from_initial_projection_value(kit: &std::sync::Arc<crate::kit::Kit>, json: &serde_json::Value) -> Result<(), crate::error::ComposeError> {
    ...
    /// @emoji ­ƒ¬ó Hydrates [`crate::kit::design::Design`] pieces from one `designs[]` entity (`pieces` block or array).
    pub(crate) async fn hydrate_design_pieces_from_snapshot_value(des: &std::sync::Arc<crate::kit::design::Design>, kit: &std::sync::Arc<crate::kit::Kit>, d_json: &serde_json::Value) -> Result<(), crate::error::ComposeError> {
        use std::collections::HashMap;
        {
        ...
        *des.piece_weak_by_external_id.write().await = HashMap::new();
        ...
        for pj in plist {
            ...
            let type_id_raw = match pj.get("type") {
                ...
                Some(serde_json::Value::Object(map)) => map.get("id").and_then(|x| x.as_str()).ok_or_else(|| crate::error::ComposeError::invalid("design piece type object missing id"))?,
                _ => {
                    return Err(crate::error::ComposeError::invalid("design piece missing type (string id or { id })"));
                }
            };
            let ty = kit.types.read().await.iter().find(|t| t.id.as_str() == type_id_raw).cloned().ok_or_else(|| crate::error::ComposeError::not_found("Type", type_id_raw))?;
    ...
    //#endregion ­ƒöû dev_backbone_initial_kit_projection

    /// @emoji ­ƒô£ `{hash, items: [T]}` envelope ÔÇö the universal "block-hashed list" reused in every nested collection of the bundle.
    ...
    /// @emoji ­ƒöù `{id, hash}` typed reference to another node in the bundle (authors, qualities, ports, families, ÔÇª).
    #[derive(Clone, Debug, serde::Deserialize, serde::Serialize)]
    pub struct HashRef {
        pub id: String,
        pub hash: String,
    ...
    /// @emoji ­ƒîÉ One graph snapshot (head pointer used as `wip` / `authoritative` / `stage` heads in the bundle).
    #[derive(Clone, Debug, serde::Deserialize, serde::Serialize)]
    pub struct DevBackboneGraphHead {
        ...
        /// @emoji ­ƒôª Wire key `initialKit` ÔÇö persisted kit seed for this snapshot (GraphQL `Graph.theKit` is the live materialization, not this JSON name).
    ...
    /// @emoji ­ƒº¡ Main kit version entity; version-scoped changes live here, not on the graph snapshot.
    #[derive(Clone, Debug, serde::Deserialize, serde::Serialize)]
    ...
    /// @emoji ­ƒî┐ Alternative version entity; each alternative owns its own version-scoped changes.
    #[derive(Clone, Debug, serde::Deserialize, serde::Serialize)]
    ...
    /// @emoji ­ƒº¥ Version change record containing ordered edits directly on `the kit` or an alternative.
    #[derive(Clone, Debug, serde::Deserialize, serde::Serialize)]
    ...
    /// @emoji Ô£Å´©Å Version edit record with forward and backward  operation steps.
    #[derive(Clone, Debug, serde::Deserialize, serde::Serialize)]
    ...
    /// @emoji ­ƒ¬í One  operation step inside an edit.
    #[derive(Clone, Debug, serde::Deserialize, serde::Serialize)]
    pub struct OperationStep {
        ...
        /// @emoji ­ƒôª Canonical [`crate::operation::CanonicalKitDiff`] JSON persisted beside `input` for audit and tooling (replay still uses `input`).
        #[serde(rename = "kitDiff", default, skip_serializing_if = "Option::is_none")]
        pub kit_diff: Option<serde_json::Value>,
    }

    /// @emoji ­ƒî▒ Empty `initialKit` JSON snapshot for fresh dev backbones until a full metabolism-shaped [`Kit`] snapshot is persisted.
    ...
    impl DevBackboneGraphHead {
        /// @emoji ­ƒî▒ Empty graph snapshot stamped with `kit_id` (used for fresh `wip` / `authoritative` / `stage` heads).
    ...
    impl DevBackboneBundleDoc {
        /// @emoji ­ƒî▒ Fresh empty bundle stamped with [`KIT_STORE_BUNDLE_SCHEMA`]; kit ids fill in once the live kit projects into `initialKit`.
        ...
        /// @emoji ­ƒöü Flatten every recorded `wip` version edit into ordered [`StoredOperation`] records ready for replay.
        ...
        pub async fn from_graph(graph: &crate::vcs::Graph) -> Self {
            ...
            // ­ƒ¬º Project checkpoints (metadata only; kit baselines live on `Graph.initialKit`).
        ...
        //#region ­ƒôª bundle file blobs (content-addressed outside kit projection JSON)

        /// @emoji ­ƒöó Blake3 hex digest of the UTF-8 blob wire (`data:` URL or raw); identical bytes ÔçÆ identical digest ÔçÆ one entity in [`DevBackboneBundleDoc::blobs`].
        pub(crate) fn digest_kit_blob_wire(wire: &str) -> String {
            blake3::hash(wire.as_bytes()).to_hex().to_string()
        }

        /// @emoji ­ƒôª Hoist each `files[].blob` into [`DevBackboneBundleDoc::blobs`] keyed by [`digest_kit_blob_wire`], set `files[].blobHash`, strip inline payload (shared digest dedupes across graph `initialKit` projections).
        ...
        /// @emoji ­ƒº╣ Drop [`blobs`] entities whose digest is not referenced by any `files[].blobHash` on `wip` / `authoritative` / `stage` `initialKit` snapshots.
        ...
        /// @emoji ­ƒôÄ Merge [`blobs`] into a kit projection clone for hydrate (`files[].blob` restored from `files[].blobHash`); does not mutate the persisted bundle JSON shape.
        ...
        /// @emoji ­ƒî▒ Initialize an empty bundle with a non-empty `wip` head: empty `initialKit` projection stamped with `kit_id`,
        ...
        pub fn initialize_with_unsaved_change(kit_id: &str, change_id: &str, checkpoint_id: &str) -> Self {
            ...
            // ­ƒ¬º First checkpoint anchors the kit at its initial empty projection (no changes yet).
            bundle.wip.checkpoints.items.push(serde_json::json!({
                "id": checkpoint_id,
                "hash": KIT_BUNDLE_HASH_STUB,
            ...
            // ­ƒº¥ Initial unsaved change on the version; edits are added when user actions run.
        ...
        /// @emoji Ô×ò Append a single forward operation to an unsaved version change (creating the change/edit if absent).
        ...
        /// @emoji Ô×ò Append a forward operation to an unsaved version change and keep an optional replay origin anchor plus optional persisted [`crate::operation::CanonicalKitDiff`] JSON.
        ...
        /// @emoji ­ƒ¬¬ Build a metabolism-shaped bundle from a flat ordered  operation log (used by golden test fixtures and import paths).
    ...
    //#endregion ­ƒº¥ wire format

    //#region ­ƒº¡ paths + uri (native only)
    #[cfg(not(target_arch = "wasm32"))]
    ...
    #[cfg(not(target_arch = "wasm32"))]
    pub fn filesystem_path_from_uri(uri: &str) -> Result<PathBuf, ComposeError> {
        let u = uri.trim();
        let p = if let Some(r) = u.strip_prefix("file://") { r } else { u };
        if p.is_empty() {
            return Err(ComposeError::invalid("empty backbone connectionUri"));
        }
        Ok(PathBuf::from(p))
    }
...
"#;
        for name in ["wip.db", "staged.db", "authoritative.db", "conflicts.db"] {
            let db = compose_root.join(name);
            let conn = Connection::open(&db).map_err(|e| ComposeError::invalid(format!("open {name}: {e}")))?;
            conn.execute_batch(ddl).map_err(|e| ComposeError::invalid(format!("init {name}: {e}")))?;
            ensure_operation_log_kit_diff_json_column(&conn).map_err(|e| ComposeError::invalid(format!("migrate {name}: {e}")))?;
        }
        Ok(())
    }

    #[cfg(not(target_arch = "wasm32"))]
    fn ensure_operation_log_kit_diff_json_column(conn: &Connection) -> Result<(), ComposeError> {
        let mut stmt = conn.prepare("PRAGMA table_info(_operation_log)").map_err(|e| ComposeError::invalid(format!("pragma: {e}")))?;
        let mut has = false;
        let mut entities = stmt.query([]).map_err(|e| ComposeError::invalid(format!("pragma query: {e}")))?;
        while let Some(entity) = entities.next().map_err(|e| ComposeError::invalid(format!("pragma entity: {e}")))? {
            let name: String = entity.get(1).map_err(|e| ComposeError::invalid(format!("pragma name: {e}")))?;
        ...
        if !has {
            conn.execute("ALTER TABLE _operation_log ADD COLUMN kit_diff_json TEXT", []).map_err(|e| ComposeError::invalid(format!("alter add kit_diff_json: {e}")))?;
        }
        Ok(())
    }
    ...
    //#endregion ­ƒº¡ paths + uri

    //#region Ô£ì´©Å atomic json (native only)
    #[cfg(not(target_arch = "wasm32"))]
    ...
    #[cfg(not(target_arch = "wasm32"))]
    fn read_or_init_bundle(path: &Path) -> Result<DevBackboneBundleDoc, ComposeError> {
        if !path.exists() {
            return Ok(DevBackboneBundleDoc::template());
        }
        let s = std::fs::read_to_string(path).map_err(|e| ComposeError::invalid(format!("read kit-store bundle: {e}")))?;
        serde_json::from_str(&s).map_err(|e| ComposeError::invalid(format!("parse kit-store bundle: {e}")))
    }
    //#endregion Ô£ì´©Å atomic json
    ...
    //#endregion ­ƒöü replay

    //#region ­ƒº® attached variants (native only)
    #[cfg(not(target_arch = "wasm32"))]
    ...
    impl DevBackboneAttached {
        /// @emoji ­ƒôÑ Read the on-disk bundle (or fresh template if file is absent).
        pub fn read_bundle(&self) -> Result<DevBackboneBundleDoc, ComposeError> {
            read_or_init_bundle(&self.path)
        }

        /// @emoji Ô×ò Append a forward  operation step into the targeted unsaved version change and atomically rewrite the bundle.
        pub fn append_operation(&mut self, workspace_id: &Id, transaction_id: &Id, kind: &str, input: &serde_json::Value, kit_diff: Option<&serde_json::Value>) -> Result<(), ComposeError> {
    ...
    impl LocalBackboneAttached {
        pub fn append_operation(&mut self, workspace_id: &Id, transaction_id: &Id, kind: &str, input: &serde_json::Value, kit_diff: Option<&serde_json::Value>) -> Result<(), ComposeError> {
            let conn = Connection::open(&self.db_path).map_err(|e| ComposeError::invalid(format!("sqlite append: {e}")))?;
            ensure_operation_log_kit_diff_json_column(&conn)?;
            let input_json = serde_json::to_string(input).map_err(|e| ComposeError::invalid(e.to_string()))?;
            let kit_json = match kit_diff {
                Some(v) => Some(serde_json::to_string(v).map_err(|e| ComposeError::invalid(e.to_string()))?),
                None => None,
            };
            conn.execute("INSERT INTO _operation_log (draft_id, transaction_id, kind, input_json, kit_diff_json) VALUES (?1, ?2, ?3, ?4, ?5)", rusqlite::params![workspace_id.as_str(), transaction_id.as_str(), kind, input_json, kit_json])
                .map_err(|e| ComposeError::invalid(format!("sqlite insert: {e}")))?;
            Ok(())
        }

        fn load_operations(&self) -> Result<Vec<StoredOperation>, ComposeError> {
            let conn = Connection::open(&self.db_path).map_err(|e| ComposeError::invalid(format!("sqlite read: {e}")))?;
            ensure_operation_log_kit_diff_json_column(&conn)?;
            let mut stmt = conn.prepare("SELECT draft_id, transaction_id, kind, input_json, kit_diff_json FROM _operation_log ORDER BY seq ASC").map_err(|e| ComposeError::invalid(format!("sqlite prepare: {e}")))?;
            let mut entities = stmt.query([]).map_err(|e| ComposeError::invalid(format!("sqlite query: {e}")))?;
            let mut out = Vec::new();
            while let Some(entity) = entities.next().map_err(|e| ComposeError::invalid(format!("sqlite entity: {e}")))? {
                let draft_id_col: String = entity.get(0).map_err(|e| ComposeError::invalid(format!("sqlite col: {e}")))?;
                let transaction_id: String = entity.get(1).map_err(|e| ComposeError::invalid(format!("sqlite col: {e}")))?;
                let kind: String = entity.get(2).map_err(|e| ComposeError::invalid(format!("sqlite col: {e}")))?;
                let input_json: String = entity.get(3).map_err(|e| ComposeError::invalid(format!("sqlite col: {e}")))?;
                let input: serde_json::Value = serde_json::from_str(&input_json).map_err(|e| ComposeError::invalid(e.to_string()))?;
                let kit_diff: Option<serde_json::Value> = match entity.get::<_, Option<String>>(4) {
                    Ok(Some(s)) if !s.is_empty() => Some(serde_json::from_str(&s).map_err(|e| ComposeError::invalid(e.to_string()))?),
                    _ => None,
                };
    ...
    impl AttachedBackbone {
        pub async fn mount_and_replay(connection_uri: &str, child_label: &'static str, graph: &Arc<Graph>) -> Result<Self, ComposeError> {
            ...
            let mut this = match bone_kind {
                ...
                crate::operation::BackboneKind::Local => {
                    let compose_root = resolve_local_compose_root(&path);
                    init_local_dot_compose_layout(&compose_root)?;
                    let db_path = db_file_for_child(&compose_root, child_label)?;
                    Self::Local(LocalBackboneAttached { _compose_root: compose_root, db_path, connection_uri_normalized: norm })
                }
                crate::operation::BackboneKind::Remote => {
                    return Err(ComposeError::invalid("remote backbone attach is not implemented yet"));
                }
            };
        ...
        pub async fn replay_into_graph(&mut self, graph: &Arc<Graph>) -> Result<(), ComposeError> {
    ...
    pub fn stored_operations_from_golden_operations_json(src: &serde_json::Value) -> Result<Vec<StoredOperation>, ComposeError> {
        ...
        let mut out = Vec::new();
        for rec in arr {
            let kind = rec["kind"].as_str().ok_or_else(|| ComposeError::invalid("operation.kind"))?;
            let input = rec.get("input").cloned().ok_or_else(|| ComposeError::invalid("operation.input"))?;
...
//#endregion ­ƒùä´©Å kit backbone persistence (native)

//#region ­ƒôú event

pub mod event {
    //! ­ƒôú The single emit point of the entire crate. Variants carry Arc-shared payloads.
    ...
    /// ­ƒîÉ Broadcast envelope for every observable thing the control plane emits.
    #[derive(Clone)]
    pub enum Event {
        CommandSucceeded(operation::CommandReceipt),
        OperationSucceeded(operation::OperationKind),
        OperationFailed(ComposeError),
    ...
    impl Event {
        /// @emoji ­ƒº¡ Command / aggregate outcome events re-emit every live subscription root.
        ...
        /// @emoji ­ƒÄ» `watched` comes from subscription lookahead; empty ÔçÆ match all paths.
        ...
        /// @emoji ­ƒôì Canonical dotted paths this event may invalidate (wip / authoritative mirrors).
    ...
    /// ­ƒôú The bus. Holds the only `emit_event` function in the crate.
    pub struct EventBus {
        tx: AsyncMutex<Sender<Event>>,
        keep_alive: InactiveReceiver<Event>,
        /// @emoji ­ƒºÀ Per-subscription [`Event`] fan-out keyed by watched canonical paths.
    ...
    impl EventBus {
        ...
        /// ­ƒôú The **only** `emit_event` in the entire crate. All other code paths must call this.
        ...
        /// ­ƒöö New subscriber receiver (unfiltered broadcast).
        pub fn subscribe(&self) -> Receiver<Event> {
            self.keep_alive.activate_cloned()
        }

        /// @emoji ­ƒöö Path-filtered channel: emits only [`Event`] values matching `watched` canonical paths.
...
pub mod worker {
    //! ­ƒºÁ Parent router + two child runtimes (wip + authoritative).
    //!
    ...
    //#region ­ƒùä´©Å backbone slot
    /// @emoji ­ƒùä´©Å Per-child optional persistence tail: native disk backbones; wasm build returns `NotSupported` on attach.
    ...
    impl BackboneNativeCell {
        ...
        pub async fn mount(&self, graph: &Arc<Graph>, child_label: &'static str, uri: &str) -> Result<(), ComposeError> {
            #[cfg(not(target_arch = "wasm32"))]
            {
            ...
            #[cfg(target_arch = "wasm32")]
            {
                ...
                Err(ComposeError::invalid("Attachable kit backbones use native disk (atomic JSON / SQLite); drive them from native hosts over GraphQL IPC instead of WASM."))
            }
        }

        pub async fn detach_matching(&self, uri: &str) -> Result<(), ComposeError> {
            #[cfg(not(target_arch = "wasm32"))]
            {
                ...
                match &*guard {
                    Some(current) if current.normalized_connection_uri() != norm => {
                        return Err(ComposeError::invalid("`connectionUri` did not match the attached backbone; detach aborted to avoid confusing persistence drift."));
                    }
                    _ => {}
                }
                guard.take();
        ...
        pub async fn record_kit_operation_if_attached(&self, workspace_id: &Id, transaction_id: &Id, operation: &crate::operation::Operation, kit_diff_wire: Option<serde_json::Value>) -> Result<(), ComposeError> {
            #[cfg(not(target_arch = "wasm32"))]
            {
                let mut guard = self.slot.write().await;
    ...
    //#endregion ­ƒùä´©Å backbone slot

    /// ­ƒÜ¬ Per-child handle held by the parent: send commands in (events flow through the shared bus).
    ...
    /// ­ƒø░´©Å Parent runtime hosting the GraphQL schema + routing logic.
    ...
    impl ParentStore {
        /// ­ƒø░´©Å Spawn parent + two child runtimes (in-process on native).
        ...
        /// ­ƒø░´©Å WASM/host bootstrap: hydrate WIP [`Graph`] from `@compose/js` kit JSON snapshot; authoritative line stays mint-empty.
        ...
        /// @emoji ­ƒø░´©Å `POST /install` bootstrap: full `DevBackboneBundleDoc` JSON (schema `KIT_STORE_BUNDLE_SCHEMA`) or bare `initialKit` projection (same contract as wasm `bootstrap_runtime_from_json_value`).
        pub async fn spawn_from_install_json_value(json: serde_json::Value) -> Result<Arc<Self>, crate::error::ComposeError> {
            if json.get("schema").and_then(|s| s.as_str()) == Some(crate::kit_backbone::KIT_STORE_BUNDLE_SCHEMA) {
                let rt = ParentStore::spawn().await;
                let s = serde_json::to_string(&json).map_err(|e| crate::error::ComposeError::invalid(e.to_string()))?;
        ...
        /// @emoji ÔÅ│ Enqueues on wip and waits for `CommandSucceeded` / `OperationFailed` on the bus.
    ...
    /// ­ƒºÁ In-worker actor: drains the inbox, applies, emits.
    ...
    impl ChildStore {
        ...
        async fn apply_kit_operation(&self, request_id: Id, workspace_id: Id, transaction_id: Id, operation: Operation) -> Result<(), ComposeError> {
            ...
            let tx_edit = graph.workspace_saved_and_unsaved_edits(&ws).await.and_then(|(s, u)| s.into_iter().chain(u).find(|t| t.id == transaction_id)).ok_or_else(|| ComposeError::not_found("Edit", transaction_id.as_str()))?;
            ...
            match &operation {
                ...
                Operation::CreateFixedPiece { scope, input } => {
                    ...
                    let design = after_kit.design_by_external_id(design_id).await.ok_or_else(|| ComposeError::not_found("Design", design_id.as_str()))?;
                    let piece = design.piece_by_external_id(piece_id).await.ok_or_else(|| ComposeError::not_found("Piece", piece_id.as_str()))?;
...
//#endregion ­ƒºÁ worker

//#region ­ƒîÉ gql

pub mod gql {
    //! ­ƒîÉ Type-safe static GraphQL schema via `Schema::build` (embedded target SDL string for tooling).
    ...
    /// @emoji ­ƒôí Flattens subscription selection into canonical `root:field:...` strings for [`crate::event::Event::matches_watched_paths`].
    ...
    /// @emoji ­ƒºÀ Maps [`crate::interface::KitGraphNavNode`] into golden `Node` interface for [`Query::node`].
    ...
    //#endregion ­ƒôí subscription_paths

    //#region ­ƒîÉ interfaces
    /// @emoji ­ƒîÉ SDL `Node`, `Entity`, `EntityConnection`, `EntityEdge`, and `Workspace` interface shells (code-first; mirrors `schema.golden.graphql`).
    pub mod interfaces {
        ...
        /// @emoji ­ƒîÉ SDL `interface EntityConnection` ÔÇö shared relay tail (`StoreConnection`, golden `PageInfoConnection` for empty owns).
        #[derive(Interface)]
        #[graphql(
        ...
        /// @emoji ­ƒ¬ó Canonical empty `EntityConnection` for shells without materialized child entities (golden `PageInfoConnection` implementor).
        ...
        /// @emoji ­ƒ¬ó Weak back-reference for golden `Entity.owner` on in-memory kit graph entities.
        ...
        /// @emoji ­ƒîÉ SDL `interface Entity` ÔÇö VCS + conflict shells participating in the global owner graph.
        #[derive(Clone, Interface)]
        #[graphql(
            name = "Entity",
            field(name = "id", ty = "crate::id::Id"),
        ...
        impl KitGraphParentWeak {
            /// @emoji ­ƒîÉ Upgrade to live [`EntityInterface`] for GraphQL `Entity.owner` resolution.
            ...
            /// @emoji ­ƒ¬¬ Resolve owning [`crate::id::Id`] for normalized kit operations (`Kit` uses workspace kit id).
            pub async fn resolve_workspace_owner_id(&self, kit: &Arc<crate::kit::Kit>) -> Result<crate::id::Id, crate::error::ComposeError> {
                use crate::error::ComposeError;
                match self {
                    Self::Unset => Err(ComposeError::invalid("Entity owner unset")),
                    Self::Kit(_) => Ok(kit.workspace_kit_id().await),
                    Self::Type(w) => w.upgrade().map(|v| v.id.clone()).ok_or_else(|| ComposeError::invalid("Entity owner dropped")),
                    Self::Representation(w) => w.upgrade().map(|v| v.id.clone()).ok_or_else(|| ComposeError::invalid("Entity owner dropped")),
                    Self::Connector(w) => w.upgrade().map(|v| v.id.clone()).ok_or_else(|| ComposeError::invalid("Entity owner dropped")),
                    Self::Design(w) => w.upgrade().map(|v| v.id.clone()).ok_or_else(|| ComposeError::invalid("Entity owner dropped")),
                }
            }
        }
        ...
        /// @emoji ­ƒîÉ SDL `interface WeakEntity` ÔÇö hash-backed graph nodes sharing the golden `Entity` tail (`id`, `hash`, `owner`, `owns`).
        #[derive(Clone, Interface)]
        #[graphql(
            name = "WeakEntity",
            field(name = "id", ty = "crate::id::Id"),
        ...
        //#region ­ƒ¬ó golden_interface_relay
        /// @emoji ­ƒ¬ó SDL `InputEdge` / `InputConnection` ÔÇö relay over [`PubInputInterface`].
        #[derive(Clone, SimpleObject)]
        ...
        /// @emoji ­ƒ¬ó SDL `DiffEdge` / `DiffConnection` ÔÇö relay over [`GqlDiffInterface`].
        #[derive(Clone, SimpleObject)]
        ...
        /// @emoji ­ƒ¬ó SDL `ModificationEdge` / `ModificationConnection` ÔÇö relay over [`GqlModificationInterface`].
        #[derive(Clone, SimpleObject)]
        ...
        impl ModificationConnection {
            /// @emoji ­ƒ¬ó Empty `ModificationConnection` for aggregate `Modifications.modifications`.
        ...
        /// @emoji ­ƒº▒ SDL `Modifications` ÔÇö aggregate removed / live / added modifications (minimal projection from live graph where wired).
        #[derive(Clone, Debug, Default)]
        ...
        /// @emoji ­ƒ¬ó SDL `BackboneCommandEdge` / `BackboneCommandConnection`.
        #[derive(Clone, SimpleObject)]
        ...
        /// @emoji ­ƒ¬ó SDL `ProviderEdge` / `ProviderConnection`.
        #[derive(Clone, SimpleObject)]
        ...
        /// @emoji ­ƒ¬ó SDL `ProviderCommandEdge` / `ProviderCommandConnection`.
        #[derive(Clone, SimpleObject)]
        ...
        /// @emoji ­ƒôÉ SDL `VectorDiff` ÔÇö golden `Diff` implementor for [`geom::entity::Vector`] scalars.
        #[derive(Clone, Debug, Default)]
        ...
        /// @emoji ­ƒôÉ SDL `VectorModification` ÔÇö golden `Modification` implementor for vector before/after.
        #[derive(Clone, Debug)]
        ...
        /// @emoji ­ƒôÉ SDL `VectorModifications` ÔÇö aggregate vector modifications.
        #[derive(Clone, Debug, Default)]
        ...
        /// @emoji ­ƒôÉ SDL `PointDiff` ÔÇö golden `Diff` implementor for [`geom::entity::Point`].
        #[derive(Clone, Debug, Default)]
        ...
        /// @emoji ­ƒôÉ SDL `PlaneDiff` ÔÇö nullable nested geometry per golden.
        #[derive(Clone, Debug, Default)]
        ...
        //#endregion ­ƒ¬ó golden_interface_relay

        //#region ­ƒº▒ golden_interface_minimals
        /// @emoji ­ƒºÀ SDL `EmptyDiff` ÔÇö minimal [`Diff`] implementor until geometry `*Diff` kinds wire into this enum.
        #[derive(Clone, Debug, Default)]
        ...
        /// @emoji ­ƒºÀ SDL `EmptyModification` ÔÇö minimal [`Modification`] shell for golden `Operation.modification`.
        #[derive(Clone, Debug, Default)]
        ...
        /// @emoji ­ƒºÀ Shared [`EmptyModification`] for [`crate::operation::OperationInterface::modification`] when no concrete modification is loaded.
        ...
        /// @emoji ­ƒºÀ SDL `EmptyPubInput` ÔÇö minimal [`Input`] implementor until `*Input` GraphQL types implement the interface.
        #[derive(Clone, Debug, Default)]
        ...
        /// @emoji ­ƒºÀ SDL `EmptyDocument` ÔÇö minimal [`Document`] implementor (golden has no concrete `Document` yet).
        #[derive(Clone, Debug, Default)]
        ...
        /// @emoji ­ƒºÀ SDL `EmptyEvent` ÔÇö minimal [`Event`] implementor until concrete event kinds implement the interface.
        #[derive(Clone, Debug, Default)]
        ...
        /// @emoji ­ƒºÀ SDL `EmptyRichStrong` ÔÇö minimal [`RichStrongEntity`] until every artifact exposes rich fields.
        #[derive(Clone, Debug, Default)]
        ...
        /// @emoji ­ƒºÀ SDL `EmptyArtifact` ÔÇö minimal [`Artifact`] until concrete artifact fields are unified on entities.
        #[derive(Clone, Debug, Default)]
        ...
        /// @emoji ­ƒîÉ SDL `interface Diff` ÔÇö hash-backed diff projection (`EmptyDiff` shell plus future `VectorDiff`, ÔÇª).
        #[derive(Clone, Interface)]
        #[graphql(
            name = "Diff",
            field(name = "id", ty = "crate::id::Id"),
        ...
        /// @emoji ­ƒîÉ SDL `interface Modification` ÔÇö `before` / `diff` / `after` triple shell.
        #[derive(Clone, Interface)]
        #[graphql(
            name = "Modification",
            field(name = "id", ty = "crate::id::Id"),
        ...
        /// @emoji ­ƒîÉ SDL `interface Input` ÔÇö operation-side argument projection (`*Input` types implement this in golden).
        #[derive(Clone, Interface)]
        #[graphql(
            name = "Input",
            field(name = "id", ty = "crate::id::Id"),
        ...
        /// @emoji ­ƒîÉ SDL `interface Document` ÔÇö preview-capable artifact (`EmptyDocument` minimal variant).
        #[derive(Clone, Interface)]
        #[graphql(
            name = "Document",
            field(name = "id", ty = "crate::id::Id"),
        ...
        /// @emoji ­ƒîÉ SDL `interface Event` ÔÇö timestamped weak entity (`EmptyEvent` minimal variant).
        #[derive(Clone, Interface)]
        #[graphql(
            name = "Event",
            field(name = "id", ty = "crate::id::Id"),
        ...
        /// @emoji ­ƒîÉ SDL `interface RichStrongEntity` ÔÇö titled uuid entities (`EmptyRichStrong` minimal variant plus future concrete kinds).
        #[derive(Clone, Interface)]
        #[graphql(
        ...
        /// @emoji ­ƒîÉ SDL `interface Artifact` ÔÇö authored change graph on rich entities (`EmptyArtifact` minimal variant).
        #[derive(Clone, Interface)]
        #[graphql(
            name = "Artifact",
            field(name = "id", ty = "crate::id::Id"),
        ...
        /// @emoji ­ƒîÉ SDL `interface StrongEntity` ÔÇö uuidv7-backed [`Entity`] projection (mirrors golden implementors reachable today).
        #[derive(Clone, Interface)]
        #[graphql(
            name = "StrongEntity",
            field(name = "id", ty = "crate::id::Id"),
        ...
        /// @emoji ­ƒîÉ SDL `interface BackboneCommand` ÔÇö `detach` / `sync` command handles on backbones.
        #[derive(Clone, Interface)]
        #[graphql(
        ...
        /// @emoji ­ƒÄø´©Å Golden `FileBackboneCommand` (`detach` / `sync`).
        #[derive(Clone, Debug, Default)]
        ...
        /// @emoji ­ƒÄø´©Å Golden `WebsocketBackboneCommand` (`detach` / `sync`).
        #[derive(Clone, Debug, Default)]
        ...
        //#endregion ­ƒº▒ golden_interface_minimals
    ...
    //#endregion ­ƒîÉ interfaces

    //#region ­ƒîÉ runtime_hosting
    /// @emoji ­ƒø£ Golden `BackboneStatus` enum (`OFFLINE` / `RECONNECTING` / `ONLINE`).
    #[derive(Clone, Copy, PartialEq, Eq, async_graphql::Enum)]
    ...
    /// @emoji ­ƒùä´©Å Golden `FileBackbone` ÔÇö disk-backed backbone shell (fields fill in once native attach completes).
    ...
    /// @emoji ­ƒîÉ Golden `WebsocketBackbone` ÔÇö websocket backbone shell (status surfaces once transport is wired).
    ...
    /// @emoji ­ƒºÀ Golden `interface Backbone` ÔÇö concrete [`FileBackbone`] / [`WebsocketBackbone`].
    ...
    /// @emoji ­ƒ¬ó Golden `BackboneEdge` / `BackboneConnection` relay shells.
    #[derive(Clone, async_graphql::SimpleObject)]
    ...
    impl BackboneConnection {
        /// @emoji ­ƒ¬ó Empty backbone connection (no entities materialised yet).
        pub fn empty() -> Self {
            Self { edges: Vec::new(), page_info: Arc::new(crate::gql_relay::PageInfo::default()), hash: crate::hash::h(&["backbone-connection", "empty"]) }
        }
    }

    /// @emoji ­ƒÅá Golden `LocalProvider` ÔÇö dev host provider (`uri` + active [`StoreConnection`]).
    ...
    impl LocalProvider {
        /// @emoji ­ƒÅá Builds a deterministic shell for the active session provider entity.
    ...
    /// @emoji ­ƒø░´©Å Golden `RemoteProvider` ÔÇö hub-linked provider (`uri` + `url` + backbone catalog).
    #[derive(Clone)]
    ...
    impl RemoteProviderConnection {
        /// @emoji ­ƒ¬ó Builds the golden `RemoteProviderConnection` from runtime entities.
        ...
        /// @emoji ­ƒ¬ó Empty remote-provider catalog.
        pub fn empty() -> Self {
            Self::from_providers(Vec::new())
        }
    }

    /// @emoji ­ƒø░´©Å Golden `interface Provider` ÔÇö concrete [`LocalProvider`] / [`RemoteProvider`].
    #[derive(Clone, async_graphql::Interface)]
    #[graphql(
        name = "Provider",
        field(name = "id", ty = "crate::id::Id"),
    ...
    /// @emoji ­ƒÄø´©Å Golden `LocalProviderCommand` (`createBackbone` / `attachBackbone`).
    ...
    /// @emoji ­ƒÄø´©Å Golden `RemoteProviderCommand` ÔÇö extends provider commands with `login` / `logout`.
    #[derive(Clone, Debug, Default)]
    pub struct RemoteProviderCommand {
        pub url: String,
    }

    /// @emoji ­ƒÄø´©Å Golden `interface ProviderCommand` ÔÇö shared `createBackbone` / `attachBackbone` surface.
    #[derive(Clone, async_graphql::Interface)]
    #[graphql(
    ...
    //#endregion ­ƒîÉ runtime_hosting

    /// @emoji ­ƒº® Executable schema (`Query`, `Mutation`, `Subscription`).
    pub type AppSchema = Schema<Query, Mutation, Subscription>;

    /// @emoji ­ƒÅ¬ Explicit target-schema Store root for graph heads and conflict reads.
    #[derive(Clone)]
    pub struct Store;
    ...
    impl StoreConnection {
        /// @emoji ­ƒ¬ó Builds the active-session store connection from the runtime store roots.
        pub fn active() -> Self {
    ...
    impl Store {
        ...
        /// @emoji ÔÜö´©Å Current conflict registry as a relay connection.
    ...
    #[Object]
    impl Query {
        /// @emoji ­ƒöÄ Golden `Query.node(id)` ÔÇö returns the `Node` interface for resolvable globals on this runtime.
        ...
        /// @emoji ­ƒöÄ Golden `Query.entity(hash)` ÔÇö returns the `Entity` interface for VCS shells resolvable on this runtime.
        ...
        /// @emoji ­ƒÅ¬ Active [`Store`] for bundle tests and relay paths (same surface as `LocalProvider.stores` / `Session.stores`).
        pub async fn store(&self) -> Store {
            Store
        }
    }

    //#region ­ƒÄø´©Åcommands
    /// @emoji ­ƒÄø´©Å `Mutation.session` scope ÔÇö holds kit command context on [`ParentStore`].
    ...
    /// @emoji ­ƒÄø´©Å `Mutation.session.store(id)` scope ÔÇö commands scoped to one active store.
    ...
    //#endregion ­ƒÄø´©Åcommands

    pub struct Mutation;

    #[Object]
    impl Mutation {
        /// @emoji ­ƒÄø´©Å Kit-changing commands ÔÇö navigate via nested fields per `schema.golden.graphql` Commands block.
    ...
    #[Subscription]
    impl Subscription {
        /// @emoji ­ƒôí Golden `Subscription.session` ÔÇö live mirror of [`Query::session`].
        ...
        /// @emoji ­ƒôí Golden `Subscription.operation` ÔÇö emits the latest WIP [`OperationInterface`] (tracks `op_history` tail until finer-grained cursor wiring lands).
    ...
    fn build_schema_sync_for(rt: Arc<ParentStore>) -> AppSchema {
        let builder = Schema::build(Query, Mutation, Subscription)
            ...
            .register_output_type::<crate::error::ComposeError>()
    ...
    /// ­ƒô£ Canonical SDL emitted by the executable async-graphql schema.
    pub async fn sdl() -> String {
        prune_non_golden_sdl(build_schema().await.sdl())
    }

    /// ­ƒº« Normalize SDL text for stable comparisons (trim ends, collapse blank-line runs).
    ...
    /// ­ƒº▒ Build schema with parent runtime + bus.
    pub fn build_schema_for(rt: Arc<ParentStore>) -> AppSchema {
        build_schema_sync_for(rt)
    }

    /// ­ƒº▒ Default schema (fresh runtime).
    pub async fn build_schema() -> AppSchema {
        build_schema_sync_for(ParentStore::spawn().await)
    }

    //#region ­ƒîÉnative_http_graphql
    /// @emoji ­ƒîÉ Parses a GraphQL-over-HTTP JSON body (`query`, optional `variables`, optional `operationName`) into an async-graphql [`Request`].
    pub fn graphql_request_from_json_str(body: &str) -> Result<async_graphql::Request, crate::error::ComposeError> {
        use async_graphql::Variables;
        let v: serde_json::Value = serde_json::from_str(body).map_err(|e| crate::error::ComposeError::invalid(format!("graphql json: {e}")))?;
        let query = v
            .get("query")
            .and_then(|x| x.as_str())
            .ok_or_else(|| crate::error::ComposeError::invalid("graphql json: missing query"))?
            .to_string();
    ...
    //#endregion ­ƒîÉnative_http_graphql
}

//#endregion ­ƒîÉ gql

//#region ­ƒöî wasm_bridge

#[cfg(target_arch = "wasm32")]
pub mod wasm_bridge {
    //! ­ƒîÉ `KitStoreHandle`: GraphQL executor + subscriptions over seeded [`crate::worker::ParentStore`] (WASM build).
    use std::sync::Arc;
    use std::sync::Mutex;
    ...
    async fn fetch_text_url(url: &str) -> Result<String, crate::error::ComposeError> {
        let win = web_sys::window().ok_or_else(|| crate::error::ComposeError::invalid("no window"))?;
        let resp_val = JsFuture::from(win.fetch_with_str(url)).await.map_err(|e| crate::error::ComposeError::invalid(format!("fetch: {e:?}")))?;
        let resp: Response = resp_val.dyn_into().map_err(|_| crate::error::ComposeError::invalid("bad response"))?;
        if !resp.ok() {
            return Err(crate::error::ComposeError::invalid(format!("http {}", resp.status())));
        }
        let text = JsFuture::from(resp.text().map_err(|e| crate::error::ComposeError::invalid(format!("text(): {e:?}")))?).await.map_err(|e| crate::error::ComposeError::invalid(format!("text await: {e:?}")))?;
        Ok(text.as_string().unwrap_or_default())
    }

    async fn bootstrap_runtime_from_json_value(v: serde_json::Value) -> Result<Arc<ParentStore>, crate::error::ComposeError> {
        if v.get("schema").and_then(|s| s.as_str()) == Some(crate::kit_backbone::KIT_STORE_BUNDLE_SCHEMA) {
            let rt = ParentStore::spawn().await;
            let s = serde_json::to_string(&v).map_err(|e| crate::error::ComposeError::invalid(e.to_string()))?;
            crate::kit_backbone::DevBackboneBundleDoc::hydrate_into_graph(&rt.wip_graph, &s).await?;
            Ok(rt)
        } else {
    ...
    async fn bootstrap_runtime_from_open_uri(uri: &str) -> Result<Arc<ParentStore>, crate::error::ComposeError> {
        let u = uri.trim();
        if u.is_empty() || u == "dev://empty" {
            return Ok(ParentStore::spawn().await);
        }
        if let Some(b64) = u.strip_prefix("dev+json:") {
            let bytes = base64::engine::general_purpose::STANDARD.decode(b64.trim()).map_err(|e| crate::error::ComposeError::invalid(format!("base64: {e}")))?;
            let v: serde_json::Value = serde_json::from_slice(&bytes).map_err(|e| crate::error::ComposeError::invalid(e.to_string()))?;
        ...
        if tail.starts_with("http://") || tail.starts_with("https://") || tail.starts_with("blob:") {
            ...
            let v: serde_json::Value = serde_json::from_str(&txt).map_err(|e| crate::error::ComposeError::invalid(e.to_string()))?;
            return bootstrap_runtime_from_json_value(v).await;
        }
        Err(crate::error::ComposeError::invalid("KitStoreHandle.create: use dev://empty, dev+json:<standard-base64>, or dev://<http(s)|blob-url> returning kit or bundle JSON"))
    }
    ...
    /// ­ƒø░´©Å Wasm entry shim (workers may call explicitly); runtime is rooted by [`KitStoreHandle`].
    #[wasm_bindgen(js_name = boot)]
    pub fn boot() {}

    /// ­ƒô£ Schema SDL for tooling (`schema_sdl` export name matches existing pkg consumers).
    #[wasm_bindgen]
    ...
    /// ­ƒø░´©Å Boot the parent runtime inside the current (parent) web worker ÔÇö no-op host hook until iframe wiring lands.
    ...
    /// ­ƒîÉ Stateful GraphQL fa├ºade for `@compose/js` embedded worker + inline WASM.
    #[wasm_bindgen]
    ...
    #[wasm_bindgen]
    impl KitStoreHandle {
        /// ­ƒº¥ `KitStoreHandle.create(uri)` ÔÇö `dev://empty`, `dev+json:<base64>`, or `dev://<fetchable-url>` returning kit/bundle JSON.
...
mod tests {
    ...
    /// @emoji ­ƒô£ Collects `(kind, name)` for top-level GraphQL declarations (first line of each declaration block).
    ...
    /// @emoji ­ƒô£ `gql::sdl()` is code-first; set `COMPOSE_GOLDEN_STRICT=1` to assert every golden `type`/`interface`/`union`/`input`/`enum`/`scalar` exists in generated SDL (structural superset).
    #[test]
    fn schema_matches_target_graphql_file() {
        ...
        if std::env::var("COMPOSE_GOLDEN_STRICT").ok().as_deref() == Some("1") {
            assert!(
                missing.is_empty(),
            ...
            eprintln!(
                "[DEBUG] golden structural gap: {} top-level declarations missing from gql::sdl() (export COMPOSE_GOLDEN_STRICT=1 to fail tests on this)",
                missing.len()
            );
        }
    }

    /// @emoji ­ƒº▒ `entity_bare!` / `operation_with_input!` / `operation_no_input!` expand to real `item` tokens (compile-time splice, not `$( )* => {}` stubs).
    #[test]
    ...
    /// @emoji ­ƒî▒ Opens an unsaved kit change via golden `Mutation.session.store.theKit.startNewChange` (see `schema.golden.graphql` Commands block).
    ...
    /// @emoji ­ƒî▒ GraphQL tests open a target-schema unsaved change; the internal draft anchor stays hidden.
    ...
    fn export_compose_graphql_schema_file() {
        let out = std::env::var("COMPOSE_GRAPHQL_SCHEMA_OUT").expect("COMPOSE_GRAPHQL_SCHEMA_OUT");
    ...
    /// ­ƒøí´©Å Guard test: the crate must contain exactly **one** `pub async fn emit_event` definition.
    #[test]
    ...
    /// ­ƒøí´©Å [`crate::worker::ChildStore`] must record operations and rely on materialization replay (`Kit::apply_diff` inside `Graph::materialized_kit_for_workspace`); it must not replace `mutable_kit` or call `apply_diff` directly.
    #[test]
    fn worker_child_runtime_guard_no_direct_mutable_kit_write_or_apply_diff() {
        ...
        let j = src[i..].find("//#endregion ­ƒºÁ worker").expect("worker end marker") + i;
        let worker = &src[i..j];
    ...
    fn kit_store_bundle_serialize_hydrate_round_trip_via_graphql() {
        // ­ƒô© Renames then hydrates via [`crate::kit_backbone::DevBackboneBundleDoc`] (bundle GraphQL entry points were dropped from the target schema).
        block_on(async {
    ...
    /// ­ƒøí´©Å Traversal must share Arcs, not deep-copy entities. Resolves a deep path and asserts
    ...
    fn no_deep_clone_on_traversal() {
        block_on(async {
            ...
            // if any local Arc clone hasn't been dropped yet ÔÇö but never +2 per piece in the design).
    ...
    /// ­ƒøí´©Å Mutation visibility without re-snapshotting: read wip, mutate, read wip again, second
    ...
    /// ­ƒùâ´©Å Delegates to [`crate::kit_graph_engine::projection_fingerprint_for_kit`] (single implementation).
    ...
    /// ­ƒ¬í `kit_graph_engine::apply_kit_operation` must replay the same golden operations as manual apply.
    #[test]
    ...
    fn kit_store_bundle_template_round_trips_metabolism_top_level_keys() {
        // ­ƒº¥ The empty bundle template is byte-stable in its top-level shape: serialise ÔåÆ deserialise ÔåÆ keys match.
    ...
    fn kit_store_bundle_initialize_with_unsaved_change_seeds_checkpoint_and_change() {
        // ­ƒî▒ Bundle bootstrap matches sketchpad "create dev kit (json)": one `initialKit`, one checkpoint, one unsaved change on the version.
    ...
    fn dev_json_backbone_round_trip_persists_metabolism_shape_on_disk() {
        // ­ƒöü Mount (no file) ÔåÆ append operation via attached backbone ÔåÆ re-read on-disk JSON ÔåÆ confirm metabolism top-level + version change path.
        block_on(async {
            let dir = tempfile::tempdir().expect("temp dir");
            let path = dir.path().join("dev-kit.json");
            let uri_full = format!("file://{}", path.display());
    ...
    fn kit_store_bundle_append_unsaved_edit_creates_change_and_edit_paths() {
        // Ô×ò Appending to a fresh template materialises wip.theKit.unsavedChanges[0].edits[0].forwards[0] without touching authoritative/stage.
        let mut bundle = crate::kit_backbone::DevBackboneBundleDoc::template();
    ...
    /// @emoji ­ƒøí´©Å Relay connection hashes must fold sorted child digests, not legacy id-join helpers.
    #[test]
    fn guard_gql_relay_has_no_legacy_id_join_hash() {
        let src = include_str!("lib.rs");
        let i = src.find("pub mod gql_relay").expect("gql_relay module");
        let j = src[i..].find("//#endregion ­ƒ¬ó gql_relay").expect("gql_relay end") + i;
        let relay = &src[i..j];
