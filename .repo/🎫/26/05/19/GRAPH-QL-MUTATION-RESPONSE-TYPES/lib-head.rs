//! ­ƒªÇ semio rust control plane ÔÇö in-memory Arc-reference architecture (code-first GraphQL).
...
pub mod id {
    ...
    /// @emoji ­ƒåö Wire name `ID` matches relay + [`semio/client/schema/graphql/schema.golden.graphql`](../../../schema/graphql/schema.golden.graphql) `scalar`/Node ids.
    #[Scalar(name = "ID")]
...
pub mod kit {
    ...
    pub struct Kit {
        ...
        /// ­ƒº¡ Optional client-facing kit id from WASM/JSON hydration (`@semio/js` DTO `id`); when None, fall back to internally minted [`Kit::id`].
...
pub mod operation {
    ...
    /// @emoji ­ƒôª Sparse `tags` triple (`metabolism.kit.diff.semio.json`).
    ...
    /// @emoji ­ƒùä´©Å Backbone materialization: dev JSON file, local `.semio/` SQLite+blobs, or remote hub (reserved).
    ...
    //#region ­ƒôª diff
    /// ­ƒô£ Ephemeral operation-side payload (id + summary) for [`CreatedFixedPiece`] / siblings ÔÇö **not** the geometric `interface Diff` from `semio/client/schema/graphql/schema.golden.graphql` (`VectorDiff`, `PositionDiff`, ÔÇª).
    #[derive(Clone, Debug, Default)]
    pub struct Diff {
        pub id: Id,
...
pub mod kit_backbone {
    //! @emoji ­ƒùä´©Å Dev JSON + local `.semio/` kit backbones: atomic single-file writes, multi-db SQLite + blobs dir, replay via [`kit_graph_engine::apply_kit_operation`].
    ...
    /// @emoji ­ƒ¬¬ On-disk schema marker stamped at the bundle root; matches `semio/assets/semio/metabolism.new.kit.semio.json`.
    ...
    /// @emoji ­ƒôÄ Resolve kit snapshot collection slices whether serialized as a legacy JSON array or a `{ hash, items }` block (`metabolism.new.kit.semio.json`).
    ...
    /// @emoji ­ƒôª Serializes [`crate::operation::CanonicalKitDiff`] for `kitDiff` on persisted operation steps (sparse object; aligns with `metabolism.kit.diff.semio.json` collection keys).
    ...
    /// @emoji ­ƒôª Top-level on-disk kit store bundle (mirrors `metabolism.new.kit.semio.json`: `schema / wip / authoritative / stage / conflicts / blobs`; each graph snapshot holds kit seed JSON under `initialKit`).
    #[derive(Clone, Debug, serde::Deserialize, serde::Serialize)]
    ...
    /// @emoji ­ƒô£ Internal value type used by replay + the SQLite local-`.semio/` path; not part of the on-disk dev-json wire format.
    #[derive(Clone, Debug)]
...
"#;
    ...
    /// @emoji ­ƒôæ US-001 golden JSON: top-level `operations` array, or legacy key `ops` (see `kit-store.golden.ops.semio.json`).
    ...
    /// @emoji ­ƒº¬ Build [`StoredOperation`] entities from `kit-store.golden.ops.semio.json` (US-001 fixture) for persistence tests.
...
pub mod worker {
    ...
    impl ParentStore {
        ...
        /// ­ƒø░´©Å WASM/host bootstrap: hydrate WIP [`Graph`] from `@semio/js` kit JSON snapshot; authoritative line stays mint-empty.
...
pub mod wasm_bridge {
    ...
    /// ­ƒîÉ Stateful GraphQL fa├ºade for `@semio/js` embedded worker + inline WASM.
    #[wasm_bindgen]
...
mod tests {
    ...
    /// @emoji ­ƒôÄ US-001 replay fixtures (`kit-store.golden.*`) live under `semio/assets/semio/` when present in the checkout.
    ...
    /// @emoji ­ƒô£ `gql::sdl()` is code-first; set `SEMIO_GOLDEN_STRICT=1` to assert every golden `type`/`interface`/`union`/`input`/`enum`/`scalar` exists in generated SDL (structural superset).
    #[test]
    ...
    /// ­ƒôñ Writes the executable schema's SDL to `SEMIO_GRAPHQL_SCHEMA_OUT`; run via `npx nx build semio/graphql`.
    #[test]
    ...
    /// @emoji ­ƒôª `metabolism.kit.diff.semio.json` parses as JSON and exposes expected top-level contract keys (typed [`crate::operation::CanonicalKitDiff`] lives on the GraphQL control plane only).
    #[test]
