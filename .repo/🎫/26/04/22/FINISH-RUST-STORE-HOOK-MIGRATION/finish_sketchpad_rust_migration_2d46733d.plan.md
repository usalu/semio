...
- **Rust:** all behaviour lives on `impl KitStore` (private + algorithmic) and `impl KitStoreHandle` (`#[wasm_bindgen]` JS boundary that locks `inner` and calls the `KitStore` method). Both files [compose/rs/src/kit_domain.rs](compose/rs/src/kit_domain.rs) and [compose/rs/src/kit_queries.rs](compose/rs/src/kit_queries.rs) get **deleted**; their bodies are merged into the existing `impl KitStore` (~6054–7266) and `impl KitStoreHandle` (~12244–12685) blocks inside [compose/rs/src/lib.rs](compose/rs/src/lib.rs).
- **JS:** no new helper functions in [compose/js/index.ts](compose/js/index.ts). All ex-helpers become methods on `KitStoreClient` (interface + `WorkerKitStoreClient` + `FallbackKitStoreClient`). Removed-helper logic in `FallbackKitStoreClient` is implemented directly on `class KitImpl` as instance methods, so consumers without a worker still go through the kit object.
- **React:** every new query hook reuses the existing `useKitRuntime()` value (`store`, `state`, `recentEvents`, `kitClient`, `subscribe`). No new context, no new emitter, no new cache layer. Pure-data derivations sit inside `React.useMemo` keyed off `runtime.state`. Async/RPC hooks reuse the established `runtime.kitClient.subscribe(load)` pattern from `usePiecesMetadataMap` ([compose/react/index.tsx](compose/react/index.tsx) ~1801–1838).
...
```mermaid
graph LR
  Sketchpad -->|hook calls| ReactPkg[compose/react useKit*]
...
1. In [compose/rs/src/lib.rs](compose/rs/src/lib.rs), inside the existing `impl KitStore` block at ~6054, add the algorithm methods previously in `kit_domain.rs`:
...
4. Update `impl KitStoreHandle` in [compose/rs/src/lib.rs](compose/rs/src/lib.rs) (~12244–12685): every `#[wasm_bindgen(js_name=…)]` method now `self.inner.read()`/`write()` and calls the corresponding `KitStore::*` method directly. Drop all `crate::kit_domain::*` / `crate::kit_queries::*` paths.

5. **Delete** [compose/rs/src/kit_domain.rs](compose/rs/src/kit_domain.rs) and [compose/rs/src/kit_queries.rs](compose/rs/src/kit_queries.rs). Remove their `mod` declarations from `lib.rs`.
...
Goal: every removed helper either (a) becomes a `KitStoreClient` method backed by Rust, or (b) becomes an **instance method on `class KitImpl`** ([compose/js/index.ts](compose/js/index.ts)) so the fallback path stays OO.
...
All new hooks live in [compose/react/index.tsx](compose/react/index.tsx). They follow exactly **one** of two patterns already in use:
...
Done strictly inside [compose/sketchpad/index.tsx](compose/sketchpad/index.tsx). No new files, no new providers.
...
No new module — extend the **existing** `sketchpadMachine` already in [compose/sketchpad/index.tsx](compose/sketchpad/index.tsx).
...
2. For each slice, add explicit XState events (`{ type: "ui.tutorial.advance" }`, `{ type: "ui.panel.toggle", … }`, etc.). The 37 `store.execute("compose.designApp.*", …)` calls that survive Phase D as **UI intents** become `actor.send(...)` here.
...
1. Extend the existing Playwright spec under [compose/sketchpad](compose/sketchpad) (no new file) with three scenarios:
