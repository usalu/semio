...
overview: Delete every kit data store, kit hook definition, and kit lifecycle/state from `compose/sketchpad`. All kit data (read + write + files) is consumed exclusively via hooks from `@compose/react`. Domain logic stays in `compose/rs` (called through `KitStoreClient`).
todos:
 - id: react_file_hooks
   content: Add file/binary/persistence hooks to @compose/react (useKitFileUrl, useKitFileBlobUrl, useEmbedKitFile, useKitBinary, useKitFileState, useKitPersistenceKind, useKitPersistenceSource, *ById aliases, registry openTemporary/openFile/openFolder/openRemote, useActiveKitGuid, useOpenKitGuids).
   status: completed
 ...
 - id: delete_sync_helpers
   ...
   status: completed
 - id: purge_imports
   content: Remove every kit-data import from @compose/js in sketchpad's import block; keep only pure value types (Kit, Design, Piece, Type, Connection, Author, Quality, etc.).
   status: completed
 - id: rewire_callsites
   content: "Rewire ~540 call sites: useKitStoreFromProvider→useKitStoreClient, new CollaborativeKitStore→file/blob hooks directly, commands.updatePiece/updateConnection/addConnection→@compose/react write hooks with useDraft, store.execute('compose.designApp.*')→actor.send or @compose/react hook."
   status: pending
 - id: machine_kit_lifecycle
   ...
   status: completed
 - id: package_cleanup
   content: Verify sketchpad/package.json deps (@compose/react/ui/js only); drop sql.js if unused; confirm @compose/react has no @compose/sketchpad dep.
   status: completed
 - id: tests_and_verify
   content: Extend existing Playwright (sketchpad) and vitest (react) regions for new hooks, registry kinds, useDraft rollback; run cargo test, pnpm -F @compose/js|react|sketchpad test, desktop smoke on metabolism.zip.
   status: pending
isProject: false
---
...
2. Every kit read/write goes through `@compose/react` `HookTriad` hooks; every domain operation is called through `useKitStoreClient()` (which forwards to wasm `KitStoreHandle` in `compose/rs`).
3. App store classes (`KitAppStoreImpl`, `DesignStore`, `QualityAppStore`, `DocsAppStore`) are split: kept-but-pure UI selection/tools/hover/panel state only (no kit reads, no `KitDiffAppStore` base), kit reads moved to consumers via `@compose/react` hooks.
4. SketchpadStore retains only UI state (navigation, theme, language, expertise, mode, panels, fullscreen, settings, hotkeys); kit lifecycle moves into `KitRegistryProvider` from `@compose/react`. The existing `sketchpadMachine` owns the active/open kit ids list.
5. Add the missing kit-file hooks to `@compose/react` so sketchpad has zero need for `CollaborativeKitStore`.
...
## Phase R — extend `@compose/react` (no domain logic, only thin client wrappers)
...
5. Extend `useKitRegistry` to expose persistence-aware open helpers used by the sketchpad machine: `openTemporary(initialKit)`, `openFile(adapter)`, `openFolder(adapter)`, `openRemote(serverConfig)`. Each returns a `kitId` and bumps `refs`. (No new domain logic — these wrap the already-exposed factories from `@compose/js`.)
...
- Delete region `🔑Entity Store Wrappers` (~7002–7240): `DesignEntityStore`, `QualityEntityStore`, `CollaborativeKitStore`. Every consumer that previously did `new CollaborativeKitStore(rawStore)` (lines 36185, 39294, 55985) is rewritten to use `@compose/react` hooks (`useKitFileBlobUrl`, `useEmbedKitFile`, `useTypeById`, etc.).
...
- Migrate command IDs that wrote kit data (e.g. `compose.designApp.updatePiece`, `compose.designApp.addConnection`, every `compose.kit.*` re-emission) into thin shims that **delegate to the `@compose/react` write hook** at the component layer (the store no longer accepts those commands; consumers call `useUpdatePiece().run(...)` etc.).
...
3. Component rewires (call sites count: ~212 kit-store, ~89 entity-hook, ~239 commands/exec):
   ...
   - `store.execute("compose.designApp.*", ...)` (~37 sites) → either `actor.send({ type: "designApp.*", ... })` for UI, or matching `useX` hook from `@compose/react`.
   - Any remaining domain helper (`findTypeInKit`, `findPieceInDesign`, etc.) — replaced by `useTypeById(id)` / `usePieceById(designId, pieceId)`. If a derived shape is needed (e.g. flat piece plane), use `useFlatPiecePlane` from `@compose/react`.
...
- [compose/sketchpad/package.json](compose/sketchpad/package.json) keeps `@compose/react`, `@compose/ui`, `@compose/js` (still needed for value types). Drop `sql.js` if no consumer remains after the strip. Confirm `@compose/sketchpad` is NOT a dependency of `@compose/react`.
...
- No new test files.
- Run, in order: `cargo test` in [compose/rs](compose/rs), `pnpm -F @compose/js test`, `pnpm -F @compose/react test`, `pnpm -F @compose/sketchpad test`. Smoke: open `metabolism.zip` in desktop, drag piece, edit name, undo/redo, export.

## Out of scope

- Moving `applyKitDiff`/`getDesignDiff`/`findRepresentation`/`selectBestRepresentation`/etc. from `@compose/js` into `compose/rs`. (The repo invariant requires it eventually, but this ticket only ensures sketchpad does not depend on those helpers — they remain available in `@compose/js` for `@compose/react` hook implementations until a separate ticket migrates them to wasm methods on `KitStoreHandle`.)
