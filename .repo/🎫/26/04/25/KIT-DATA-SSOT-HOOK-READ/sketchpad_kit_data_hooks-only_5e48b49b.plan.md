...
- Delete region `🔑Entity Store Wrappers` (~7002–7240): `DesignEntityStore`, `QualityEntityStore`, `CollaborativeKitStore`. Every consumer that previously did `new CollaborativeKitStore(rawStore)` (lines 36185, 39294, 55985) is rewritten to use `@semio-tech/compose-react` hooks (`useKitFileBlobUrl`, `useEmbedKitFile`, `useTypeById`, etc.).
