...
overview: Phased migration of `compose/rs` toward object-oriented APIs and pointer-first usage, starting with an executable batch that moves `KitGraphChange`-related free functions onto types while keeping backward-compatible entry points and full test coverage.
todos:
 - id: kg-from-diffs
 ...
 - id: commit-session
   ...
   status: completed
 - id: verify-tests
   content: Run cargo test --lib -p compose and fix any compile/test regressions
   status: completed
isProject: false
---

# compose/rs object-oriented refactor plan

## Current state

- Domain logic still lives primarily in a single file: [`compose/rs/lib.rs`](c:\git\compose\compose\rs\lib.rs) (~27k lines), with nested `mod` regions rather than separate `.rs` files.
- Recent work already moved delete-report logic onto [`Design`](c:\git\compose\compose\rs\lib.rs) (`delete_pieces_and_connections_report`, pointer-aware `delete_pieces_and_connections`), with [`Kit::delete_pieces_and_connections_in_design`](c:\git\compose\compose\rs\lib.rs) delegating for compatibility.
...
1. **Physical modules**: Split `lib.rs` into `compose/rs/src/` (or `compose/rs/`) modules by domain: `piece`, `design`, `kit`, `flatten`, `diff`, `hash`, `session`, backbone — each `mod foo;` + focused files. Keeps binary layout stable via crate-root `lib.rs` re-exports.
...
**Goal:** Move graph-change helpers from standalone `pub fn` to inherent methods on [`KitGraphChange`](c:\git\compose\compose\rs\lib.rs), align call sites with OO style, preserve public API.
...
- **Public API:** Rust downstream crates using `compose::extract_granular_events` / `kit_graph_change_from_diffs` keep working via wrappers; new code should prefer methods.
