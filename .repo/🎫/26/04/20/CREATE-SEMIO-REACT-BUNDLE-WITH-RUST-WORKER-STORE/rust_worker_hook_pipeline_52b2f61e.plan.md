...
overview: End-to-end refactor so the single source of truth is the Rust `KitStore` running in a Web Worker via WASM; `@semio/js` exposes a typed Comlink client; `@semio/react` exports `[value, setValue, status]` hooks driven by `KitEvent`; `@semio/sketchpad` consumes those hooks. Setters are async, return `Promise<Result<void, SetError>>`, and surface domain rejections (`IllegalName`, `Timeout`, `DuplicateGuid`, …).
todos:
  - id: rs_errors
    content: Introduce SetError + KitEvent::SetRejected; refactor every set_* method in semio/rs to return SetResult with domain rejections and update existing events tests.
    status: completed
  - id: rs_rpc
    ...
    status: completed
  - id: rs_wasm
    content: Replace semio/rs/src/wasm.rs with stateful KitStoreHandle exposing snapshot, setField, addChild, removeChild, subscribe, applyDesignDiff returning structured SetResult promises.
    status: completed
  - id: js_client
    content: Delete JS KitImpl mutation path from semio/js/index.ts; add KitStoreClient (Comlink) + semio/js/worker.ts + InlineKitStoreClient fallback + SetError types.
    status: completed
  - id: react_hooks
    content: Rewrite semio/react/index.tsx KitProvider + useField core on top of KitStoreClient event stream; regenerate per-field hooks to return [value, setValue, status].
    status: completed
  - id: sketchpad_migration
    content: In semio/sketchpad/index.tsx replace local usePieceName/etc. with @semio/react re-exports and migrate all call sites from canSet to status.kind.
    status: cancelled
  - id: tests_pipeline
    ...
    status: completed
  - id: verify_pipeline
    content: cargo test --lib + cargo test --target wasm32-unknown-unknown, pnpm -F @semio/js test, pnpm -F @semio/react test, pnpm -F @semio/sketchpad test; confirm pipeline end-to-end.
    status: completed
isProject: false
---
...
```mermaid
flowchart LR
  UI["semio/sketchpad (React components)"] --> Hooks["semio/react (hooks + context)"]
  Hooks --> Client["semio/js (KitStoreClient, Comlink proxy)"]
  Client <-->|"postMessage RPC + MessagePort events"| Worker["Web Worker"]
  Worker --> Wasm["semio/rs wasm-bindgen glue"]
...
### 3.1 Structured errors ([semio/rs/src/error.rs](semio/rs/src/error.rs))

Replace `SemioError::Validation(String)` and `InvalidOperation(String)` with typed variants matching `SetErrorKind` above. Add:
...
Keep existing `SemioError` for I/O but convert at the WASM boundary.

### 3.2 Setter signature sweep

Every `set_<field>(&mut self, v) -> ()` in [piece.rs](semio/rs/src/piece.rs), [design.rs](semio/rs/src/design.rs), [kit.rs](semio/rs/src/kit.rs), [typ.rs](semio/rs/src/typ.rs), [port.rs](semio/rs/src/port.rs), [connector.rs](semio/rs/src/connector.rs), [connection.rs](semio/rs/src/connection.rs), [representation.rs](semio/rs/src/representation.rs), [file.rs](semio/rs/src/file.rs), [folder.rs](semio/rs/src/folder.rs), [layer.rs](semio/rs/src/layer.rs), [group.rs](semio/rs/src/group.rs), [quality.rs](semio/rs/src/quality.rs), [benchmark.rs](semio/rs/src/benchmark.rs), [author.rs](semio/rs/src/author.rs), [concept.rs](semio/rs/src/concept.rs), [tag.rs](semio/rs/src/tag.rs), [prop.rs](semio/rs/src/prop.rs), [stat.rs](semio/rs/src/stat.rs), [attribute.rs](semio/rs/src/attribute.rs), [side.rs](semio/rs/src/side.rs) becomes:
...
Add `crate::validate::{validate_name, validate_url, validate_email, ...}` helpers producing `SetError`. On `Err`, emit `KitEvent::SetRejected { entity, field, error }` (new variant added to [events.rs](semio/rs/src/events.rs)).
...
### 3.4 Event additions ([events.rs](semio/rs/src/events.rs))
...
### 3.5 WASM surface ([wasm.rs](semio/rs/src/wasm.rs))
...
- `#region 🧵 Worker`: [new file loaded as blob URL in browser; Node worker thread in tests] entry that imports the wasm-bindgen module, instantiates `KitStoreHandle`, exposes methods via Comlink, and forwards `subscribe` callbacks over a dedicated `MessagePort`.
- `#region 🔢 Types`: Export `SetError`, `SetResult`, `WriteStatus`, `EntityKind`, and regenerated DTO types (mirror Rust DTO shape).
...
## 5. `semio/react` changes ([semio/react/index.tsx](semio/react/index.tsx))
...
## 6. `semio/sketchpad` changes ([semio/sketchpad/index.tsx](semio/sketchpad/index.tsx))

- Replace the ~300 local `useX` implementations and the `HookResult` / `conditionalHookResult` / `writableHookResult` helpers with re-exports / thin wrappers from `@semio/react`:
  ```ts
  export {
    usePieceName,
    usePieceColor,
  ...
  export type HookResult<T> = import("@semio/react").HookTriad<T>;
  ```
...
- [semio/rs/src/tests/events/\*.rs](semio/rs/src/tests/events/piece.rs): `#region 🚫SetRejected` — every entity has at least one invalid-input test asserting `Err(SetError::IllegalName|InvalidUrl|InvalidValue)` plus `KitEvent::SetRejected` emitted and value unchanged.
- [semio/rs/src/tests/events/diff_apply.rs](semio/rs/src/tests/events/diff_apply.rs): add `set_field_dispatch` tests via the new uniform `set_field`.
- Add `#[cfg(target_arch = "wasm32")] mod wasm;` in [src/tests/mod.rs](semio/rs/src/tests/mod.rs) using `wasm-bindgen-test`: subscribe round-trip, setField success + rejection.
- [semio/js/index.ts](semio/js/index.ts) — extend the `describe("InMemoryKitStore", ...)` region, rename to `describe("KitStoreClient", ...)` with subregions:
...
- [semio/react/index.tsx](semio/react/index.tsx) — add `describe("pipeline hooks", ...)` using `@testing-library/react` + `vitest`:
...
- [semio/sketchpad/index.tsx](semio/sketchpad/index.tsx) — Playwright test already wired; add a spec that: opens design, edits piece name to empty, asserts error toast renders with `IllegalName`; edits to valid value, asserts value commits.

## 8. Dependencies

- Add `comlink` to [semio/js/package.json](semio/js/package.json).
- Add `@testing-library/react` + `@testing-library/dom` to [semio/react/package.json](semio/react/package.json) devDependencies.
- Add `wasm-bindgen-test` to [semio/rs/Cargo.toml](semio/rs/Cargo.toml) target-wasm dev-deps.
- [semio/js/vite.config.ts](semio/js/vite.config.ts) and [semio/sketchpad/vite.config.ts](semio/sketchpad/vite.config.ts) already use `vite-plugin-wasm` — confirm `worker.format = "es"` and worker import uses `new Worker(new URL("./worker.ts", import.meta.url), { type: "module" })`.

## 9. Workflow per repo AGENTS

All edits happen under one ticket (`ticket_open` title "Rust Worker Hook Pipeline"); use `#region`/`#endregion` subregions in every file (no new files except the three unavoidable ones: `semio/rs/src/rpc.rs`, `semio/rs/src/validate.rs`, `semio/js/worker.ts`); no new test files; delegate independent slices (Rust errors+setters, WASM+worker, React hooks, Sketchpad migration, tests) to sub-agents in parallel, driven from one parent ticket.
