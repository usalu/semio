...
- [compose/rs/src/tests/events/\*.rs](compose/rs/src/tests/events/piece.rs): `#region 🚫SetRejected` — every entity has at least one invalid-input test asserting `Err(SetError::IllegalName|InvalidUrl|InvalidValue)` plus `KitEvent::SetRejected` emitted and value unchanged.
