...
- Refactor `compose/react/index.tsx` so hooks delegate to `@semio-tech/compose-js` entity stores via `useSyncExternalStore`. Remove parallel exposed field sources from `IndexedSchemaState`, `ComposeKitViewStore`, `ComposeKitDesignReadStore`, `ComposeKitShallowListReadStore`, and per-field ad hoc live-read hooks where they duplicate entity-store fields.
