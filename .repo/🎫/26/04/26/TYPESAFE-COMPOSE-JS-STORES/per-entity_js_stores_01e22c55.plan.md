...
8. **Repo process (when executing, not in plan-only mode)**
   - Per [AGENTS.md](c:\git\compose\AGENTS.md): `repo` MCP `search`, read `repo://goals`, `ticket_open` for e.g. `refactor-js-per-entity-stores`, work only under `.repo/🎫/...` for scratch, `ticket_close` with file list.
