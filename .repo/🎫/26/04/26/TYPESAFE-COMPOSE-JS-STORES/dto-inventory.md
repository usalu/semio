# DTO inventory (typesafe compose/js)

Grouped loose surfaces addressed in this ticket:

- **Wire tree**: `ComposeJson` → `ComposeKitWireTreeDto` / `ComposeKitWireStructDto` (no `Json` / `Record` alias names on boundary).
...
- **Writes / batch**: session/live/backbone shells → explicit `ComposeShellVariablesPlainDto`, batch row DTOs, `changeKitWithInverse` result DTO.
- **Patches**: `piecePatchToWireCommands`, `connectionPatchToWireCommands`, `buildSchemaEntityChangeCommands` field values.
- **Client**: `KitStoreClient`, `WasmKitStoreClient`, `FallbackKitClient`, `ComposeKitBridge`, `KitStoreReadSnap`, `LiveKitRoot`, stores, event filters.
...
- **React graph ops**: `KitHostGraphOp` uses `TypeId` / `DesignId` / `PieceId` / `ConnectionId` and Plain/Diff DTOs; `kitHostUndo` / `kitHostRedo` replace sketchpad undo/redo `executeComposeKitCommand` calls.
