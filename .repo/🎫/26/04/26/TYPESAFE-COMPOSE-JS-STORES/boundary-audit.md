# Boundary audit (Typesafe Compose)

## Rust (`compose/rs/lib.rs`)
- `KitEvent::ComposeKitCommand` / lifecycle: `command_kind`, `phase` as strings → replace with explicit enums; `result`/`error` as `serde_json::Value` → typed DTOs.
