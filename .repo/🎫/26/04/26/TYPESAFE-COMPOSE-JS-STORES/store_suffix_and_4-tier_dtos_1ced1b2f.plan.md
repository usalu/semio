...
todos:
 ...
 - id: io_wasm
   ...
   status: completed
 - id: consumer
   content: Update `compose/algorithms/native-bridges/rs/src/main.rs` to accept `KitFullDto`, hydrate via `KitStore::from_full_dto`, and call `DesignStore::flatten_change` / `DesignStore::delete_change`.
   status: completed
 - id: verify
   content: cargo build -p compose, cargo build for the native bridge, cargo clippy (no warnings) -- confirm everything compiles cleanly.
   status: completed
isProject: false
---
...
- `FlattenedDesign` and `FlattenedPiece` in [compose/rs/src/design.rs](compose/rs/src/design.rs) and [compose/rs/src/piece.rs](compose/rs/src/piece.rs). `Design::flatten(&self) -> FlattenedDesign` becomes per-piece `PieceStore::flat_plane()` / `flat_center()`.
...
- [compose/rs/src/diff.rs](compose/rs/src/diff.rs) — `DesignDiff` fields become `Vec<Piece{Full|Id}Dto>` / `Vec<Connection{Full|Id}Dto>`; `DesignChange.before / after` become `DesignFullDto`. `Design::flatten_change` / `Design::delete_change` become `DesignStore::flatten_change` / `delete_change` and return `ComposeReport<DesignChange>` built from the new DTOs.
- [compose/rs/src/session.rs](compose/rs/src/session.rs) — `Kit` → `KitStore`, `KitRef = Arc<RwLock<KitStore>>`; API preserved.
- [compose/rs/src/io/json.rs](compose/rs/src/io/json.rs) — `KitStore::from_json_str(s)` parses `KitFullDto` then hydrates; `KitStore::to_json_pretty()` emits `KitFullDto`.
- [compose/rs/src/wasm.rs](compose/rs/src/wasm.rs) — bindings keep JS-visible names; bodies call the new `from_full_dto` / `to_full_dto` pair.
- [compose/rs/src/lib.rs](compose/rs/src/lib.rs) — re-exports switch from `Piece` → `PieceStore` and expose the four DTO names per entity.
- [compose/algorithms/native-bridges/rs/src/main.rs](compose/algorithms/native-bridges/rs/src/main.rs) — `KitDto` → `KitFullDto`; `Kit::from_dto` → `KitStore::from_full_dto`; `kit.design(g)` → `kit.design(g)` (still a lookup on `KitStore`); `design.flatten_change` / `Design::delete_pieces_and_connections_ref` → `DesignStore::flatten_change` / `DesignStore::delete_change`.

## What stays (infrastructure, non-entity)

- `Guid`, `HashWriter`, `ComposeError` / `Result`, `OperationNote`, `NoteSeverity`, `ComposeReport`, `ValidationResult`.
...
- `cargo build --manifest-path compose\algorithms\native-bridges\rs\Cargo.toml` — clean.
- `cargo build -p compose --target wasm32-unknown-unknown` — compiles (target already supported in Cargo.toml).
- `cargo clippy -p compose -- -D warnings` — no warnings on the rewritten crate.
...
- SQLite / ZIP I/O remain stubs returning `ComposeError::InvalidOperation` until the JSON tier is stable.
