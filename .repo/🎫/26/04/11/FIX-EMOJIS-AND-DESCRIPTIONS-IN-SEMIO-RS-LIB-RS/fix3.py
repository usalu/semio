"""Fix remaining broken doc comments in semio/rs/lib.rs.
Specifically targets lines with 🔧 (wrench) that were incorrectly generated
by the first fix script, and fixes 'represents identifies' → 'identifies' for Id types.
"""

import re

file_path = r"c:\git\semio\semio\rs\lib.rs"
...
# ================================================================
# FIX 2: Read all lines and fix broken 🔧 patterns using context
# ================================================================
lines = content.split("\n")
new_lines = []
...
for i, line in enumerate(lines):
    ...
    # Only process lines with 🔧 in a <summary> tag
    if "🔧" not in stripped or "<summary>" not in stripped:
    ...
    if next_def:
        ...
        # Handle struct/enum/type
            ...
            if name in ENTITY_DESC:
                ...
                new_line = f"{indent}/// <summary>🔄DiffUpdate represents a before-after pair for entity updates.</summary>"
            elif name == "Change":
                new_line = f"{indent}/// <summary>🔄Change represents a tracked modification with timestamp and author.</summary>"
            elif name == "CollectionDiff":
                new_line = f"{indent}/// <summary>🔄CollectionDiff represents batched entity additions, removals and updates.</summary>"
                ...
                new_line = f"{indent}/// <summary>🔧ValidationFix represents a suggested fix for a validation problem.</summary>"
            elif name == "ValidationResult":
                new_line = f"{indent}/// <summary>✅ValidationResult represents the outcome of a kit validation.</summary>"
            elif name == "SemioError":
                new_line = f"{indent}/// <summary>❌SemioError represents a domain error with context message.</summary>"
            elif name == "FlattenedPiece":
                new_line = f"{indent}/// <summary>🧩FlattenedPiece represents a piece with fully resolved world-space transform.</summary>"
            elif name == "WasmResult":
                new_line = f"{indent}/// <summary>🌐WasmResult represents a WebAssembly-compatible result wrapper.</summary>"
            else:
                new_line = f"{indent}/// <summary>🔧{name}.</summary>"
        ...
        # Handle fn
            ...
            if name.startswith("find_"):
                ...
                if fm:
                    ...
                    new_line = f"{indent}/// <summary>🔍finds a {entity} in a {container}{mut_str} by GUID.</summary>"
                else:
                    new_line = f"{indent}/// <summary>🔍{readable}.</summary>"
            ...
            # Apply diff
                ...
                new_line = f"{indent}/// <summary>🔄applies a diff to update {target}.</summary>"
            ...
            # Equality
                ...
                new_line = f"{indent}/// <summary>🔄compares two {target} entities for deep equality.</summary>"
            ...
            # Validate
                ...
                new_line = f"{indent}/// <summary>✅validates a kit for structural and referential integrity.</summary>"

            # Flatten
            elif name == "flatten_design":
                new_line = f"{indent}/// <summary>📐flattens nested design references into a single design.</summary>"

            # Planes equal
            elif name == "planes_equal_approx":
                new_line = f"{indent}/// <summary>◻️compares two planes for approximate equality.</summary>"

            # Quat
            elif name == "quat_to_matrix4":
                new_line = f"{indent}/// <summary>🔄converts quaternion to a 4x4 rotation matrix.</summary>"

            # Apply matrix
            elif name.startswith("apply_matrix"):
                new_line = f"{indent}/// <summary>💻applies a 4x4 matrix to a 3D vector.</summary>"

            # Connector to plane
            elif name == "connector_to_plane":
                new_line = f"{indent}/// <summary>🔌converts a connector to a plane representation.</summary>"
        ...
        # Handle const
            ...
            if name == "SUPPORTED_MODEL_EXTENSIONS":
                new_line = f"{indent}/// <summary>🗿the list of supported 3D model file extensions.</summary>"
            else:
                new_line = f"{indent}/// <summary>🔧{name}.</summary>"
    ...
    if new_line is None:
        if (
            ...
            new_line = f"{indent}/// <summary>🔧{broken_content}.</summary>"
        else:
