...
- Tests: `tests::control_plane` (pre-batch vs inverse, apply DTO parity, kit event JSON); `transaction_change_kit_with_inverse_row_includes_change_kind_and_inverse`; SDL line for `KitStoreMutation` doc in `semio/graphql/schema.graphql`.

Verified: `cargo fmt`, `cargo test --lib` (semio/rs, 140 passed, 1 ignored).
