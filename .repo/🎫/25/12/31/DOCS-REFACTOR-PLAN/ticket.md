...
2. Scripts folder references
   - Remove/replace references to `scripts/log.ts` and other `scripts/*.ts` calls in `AGENTS.md` with the `repo` CLI equivalents implemented in `./repo/cli`.

## Phase 2: Repo CLI Documentation Alignment

1. Remove non-existent `tool` command from docs and reconcile with `go/mcp`:
   - Either implement `repo tool` in `./repo/cli` or remove `tool_run` from MCP and the `tool <name>` row in `AGENTS.md`.

2. Document implemented commands in `./repo/cli/main.go`:
...
3. Ticket status terminology
   - Normalize the ticket status to `finished` in code or update `AGENTS.md` to accept `closed`. Prefer changing `./repo/cli` `TicketClosed` to `finished` for consistency.

4. Ticket interaction file flags
   - Either update `./repo/cli` to accept `--file`, `--file-created`, `--file-removed` flags, or remove these examples from `AGENTS.md`.
...
- file caching system (`.repo/cache`)
...
- Update MCP tools list to match `go/mcp` and `./repo/cli` capabilities.
...
2. P0: Fix ticket status mismatch in `./repo/cli` or docs.
