...
Scripts were executed using `node --import tsx` in `preflight.ts`. The `scripts/log.tsx` file used CommonJS `require()` syntax which doesn't work in ES modules. The `scripts/generate-validation.tsx` file imported from `assets` package which had module resolution issues. Help text in scripts referenced the old execution method.
...
- Fixed `scripts/generate-validation.tsx`:
  - Changed import from `assets` to relative path: `import InvalidKit from "../assets/compose/kit_invalid.json"`
