...
// package `vitest.config.ts` in the tree, loaded each one through Vite's own config loader, carried a
...
//
// What remains is the host-local config for the root itself: nothing to collect. Each test concern owns
// an explicitly selected semantic configuration and runs it through its own `📜️script.ts test` target. There is
