//! 🔺 Trinity program — Jack and Rewrite apps in one hot-swappable WASM program.

pub mod app_jack {
    //! 🔱 Trinity Jack program — jack query play app bundled as a hot-swappable WASM program.
    ...
    fn selection_ids(args: Option<&Value>) -> Vec<String> {
        args.and_then(|value| value.get("nodeIds"))
            ...
            .or_else(|| Some(semio_framework_program::selection_ids(args)).filter(|ids: &Vec<String>| !ids.is_empty()))
            .or_else(|| {
    ...
    fn build_inspector_tree(fixture: &GraphFixture, runtime: &TrinityJackRuntime, term_labels: &TrinityJackLabels) -> UiNode {
        ...
        ui_inspector_groups_to_tree(&[
            ...
            UiInspectorFieldGroup {
                ...
                default_open: None,
                fields: vec![
                    semio_framework_program::UiNode::Field(UiFieldNode {presence: UiPresence::default(), 
                        id: "trinity-inspector.name".into(),
                        label: "Name".into(),
                        child: Box::new(semio_framework_program::UiNode::Input(semio_framework_program::UiInputNode {presence: UiPresence::default(), 
    ...
    mod tests {
        use super::*;
        use semio_framework_program::{testkit, ActionMeta, ProgramApp, VcsDocumentApp};
...
pub mod app_rewrite {
    //! ♻️ Trinity Rewrite program — parametric rewrite play app bundled as a hot-swappable WASM program.
    ...
    fn build_parameters_panel(state: &RewriteRuleState, labels: &TrinityRewriteLabels) -> UiNode {
        ...
        for param in &rhs.parameters {
            ...
            children.push(semio_framework_program::UiNode::Field(UiFieldNode {presence: UiPresence::default(), 
    ...
    mod tests {
        ...
        fn dispatch(app: &mut VcsDocumentApp<TrinityRewritePlayApp>, action: &str, args: Option<&Value>) -> semio_framework_program::kernel::InvocationResult {
...
fn register_trinity_exports() {
    semio_framework_program::program_runtime::register_document_codec_for_app::<app_jack::TrinityJackPlayApp>(trinity_ram::TRINITY_GRAPH_SCHEMA);
    semio_framework_program::program_runtime::register_document_codec_for_app::<app_rewrite::TrinityRewritePlayApp>(trinity_rewrite::REWRITE_RULE_SCHEMA);
}
