...
use trinity_ram::{
    apply_trinity_graph_ops, Camera, Edge, EntityRef, Graph, GraphFixture, Manifest, Node, Port, PortDirection, PropertyBag,
    PropertyValue, TrinityGraphOp, port_key,
};
...
/// 🔤 Fine-grained jack token kind, shared with editor consumers (e.g. writer's outline/hover heuristics) that
...
/// 🔤 Byte-span-tracked jack lexer. `forgiving = true` never fails (used by live editors mid-keystroke);
...
/// 🧩 Demo `Piece`/`Connection` fixture shared by the jack language server default session
...
/// 🧩 [`example_graph_fixture`] as a resolved in-memory [`Graph`].
pub fn example_graph() -> Graph {
    Graph::from_fixture(example_graph_fixture()).expect("jack example fixture")
}

/// 🧩 [`example_graph_fixture`] serialized as fixture JSON.
...
// #region 🔖SpannedAst
/// 🌳 Span-tracked jack AST node for editor tree/hover/outline surfaces (e.g. writer's document panel),
...
/// 🌊 Collapses runs of whitespace to a single space, for deriving a node's default display label.
...
impl<'a> SpannedParser<'a> {
    ...
    fn parse_cmp_expr(&mut self) -> Result<SpannedNode, String> {
        ...
        let (prop_text, prop_start, prop_end) = self.expect_ident()?;
        let op = self.bump();
        if !matches!(op.token, Token::Eq | Token::Ne) {
            return Err(format!("expected comparison at {}", op.start));
        }
        let value = self.parse_value()?;
        let var_node = spanned_node("var", var_start, var_end, self.source, Vec::new(), Some(var_text.as_str()));
        let prop_node = spanned_node("property", prop_start, prop_end, self.source, Vec::new(), Some(prop_text.as_str()));
        let kind = if op.token == Token::Eq { "eq" } else { "ne" };
        let end = value.end;
...
/// 🌳 Parse jack source into a span-tracked AST for hierarchy/outline panels (moved from writer's premigration
    ...
    for clause in &query.clauses {
        match clause {
            ...
            Clause::Create(pattern) => {
                ...
                fixture = apply_trinity_graph_ops(fixture, &batch).map_err(|e| e.to_string())?;
            ...
            Clause::Delete(vars) => {
                for var in vars {
                    if let Some(id) = bindings.first().and_then(|b| b.nodes.get(var).cloned()) {
                        ...
                        ops.push(op.clone());
                        fixture = apply_trinity_graph_ops(fixture, std::slice::from_ref(&op)).map_err(|e| e.to_string())?;
                        view = Graph::from_fixture(fixture.clone()).map_err(|e| e.to_string())?;
                    }
                }
            }
...
pub fn run(graph: &mut Graph, source: &str) -> Result<QueryResult, String> {
    ...
    if !ops.is_empty() {
        let fixture = apply_trinity_graph_ops(graph.to_fixture(), &ops).map_err(|e| e.to_string())?;
