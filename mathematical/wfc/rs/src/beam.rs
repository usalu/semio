//! 🌊🔦 Beam search: an incomplete alternative to full backtracking. Maintains up to `width`
...
pub struct BeamConfig {
    /// 🌊🔦 Maximum number of beams kept after each round's prune.
    pub width: usize,
    /// 🌊🔦 Hard cap on rounds (roughly one decision per surviving beam per round).
    pub max_steps: usize,
...
/// 🌊🔦 Higher is better: fewer total remaining candidates across every domain, i.e. closer to a
...
/// 🌊🔦 Runs beam search to either a solution or exhaustion of every beam / `max_steps`. See this
