//! 🧠 Nogood learning: on every contradiction, the current decision prefix (every still-active
...
//! `crate::search::decide_and_propagate`), never to ordinary propagation-caused domain reductions.
...
// #region 🔖Config
/// 🧠 Opt-in — disabled by default so a solve that never asks for this pays no cost (no watcher
/// table allocated, no per-decision lookup).
#[derive(Clone, Copy, PartialEq, Debug)]
pub struct NogoodConfig {
    pub enabled: bool,
    /// 🧠 Nogoods longer than this are discarded unrecorded (a long clause rarely prunes early
    /// enough to be worth its upkeep).
    pub max_len: usize,
    /// 🧠 Above this many stored nogoods, the lowest-activity one is evicted to make room.
    pub max_count: usize,
}
...
/// 🧠 Learned-nogood storage plus its two-watched-literal index. Persists across restarts within
...
pub(crate) struct NogoodStore {
    config: NogoodConfig,
    nogoods: Vec<Nogood>,
    /// 🧠 `(node, pattern) -> indices into `nogoods` currently watching that exact literal.
...
impl NogoodStore {
    ...
    /// 🧠 Records the given decision prefix as a nogood (skipped if disabled, empty, or over
    ...
    /// 🧠 Re-establishes every stored nogood's watched literals against a fresh attempt's domain
    ...
    pub fn rewatch_for_new_attempt<T: Topology>(&mut self, model: &CompiledModel, topo: &T, domains: &mut DomainStore, queue: &mut PropQueue, trail: &mut Trail, metrics: &mut crate::diag::Metrics) -> Option<NodeId> {
    ...
    /// 🧠 Reacts to `node` just having been decided to exactly `pattern` — every nogood watching
    ...
    pub fn on_decision<T: Topology>(&mut self, model: &CompiledModel, topo: &T, node: NodeId, pattern: PatternId, domains: &mut DomainStore, queue: &mut PropQueue, trail: &mut Trail, metrics: &mut crate::diag::Metrics) -> Option<NodeId> {
        if !self.config.enabled {
            return None;
        }
...
/// 🧠 Whether `(node, pattern)`'s corresponding search-decision literal is definitively false —
...
/// 🧠 Removes `pattern` from `node`'s domain (which must still contain it) and, if that alone
...
mod tests {
    ...
    /// 🧪 A trivial arc-less topology sized to `node_count` — every test here only needs the
    /// AC-3 re-propagation hook to be a safe no-op (no neighbors to cascade to), not to exercise
    /// AC-3 itself (that's `prop_ac3.rs`'s own job).
    fn no_arcs_topology(node_count: usize) -> crate::topology::GraphTopology {
    ...
    fn on_decision_detects_conflict_when_the_other_watch_was_already_resolved_true() {
        ...
        let mut singleton_p1 = crate::bitset::PatternSet::new_empty(3);
