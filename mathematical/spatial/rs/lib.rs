...
/// 🌲 Balanced k-d tree over `[f64; D]` points with `u32` payload indices, built by iterative median splits (quickselect on `axis = depth % D`) into a flat node vector with children linked by index; all queries run iteratively with an explicit stack and bounding-plane pruning. <https://en.wikipedia.org/wiki/K-d_tree>
#[derive(Clone, Debug)]
...
impl<const D: usize> KdTree<D> {
    ...
    /// 🎯 Nearest neighbour of `q` as `(payload index, squared distance)`; ties on distance resolve to the smallest index. `None` on an empty tree.
    ...
    /// 🎯 The `k` nearest neighbours of `q`, ascending by `(squared distance, payload index)`, found with a bounded max-heap; returns fewer than `k` entries only when the tree holds fewer points.
    ...
    /// ⭕ All points within euclidean distance `r` of `q` as `(payload index, squared distance)`, sorted by payload index for determinism; negative `r` yields no hits.
    ...
    /// 📦 Calls `f` with the payload index of every point inside the closed axis-aligned box `[min, max]`, pruning subtrees by the splitting plane.
...
// #region 🔖VoxelGrid
/// 🧊 Uniform hashed voxel grid over 3D points: buckets `u32` ids by integer cell, with 27-cell neighbourhood gathering for broad-phase proximity. <https://en.wikipedia.org/wiki/Bin_(computational_geometry)>
#[derive(Clone, Debug)]
...
impl VoxelGrid3 {
    /// 🧊 Creates an empty grid with the given positive cell edge length.
    pub fn new(cell: f64) -> Self {
        assert!(cell > 0.0, "voxel cell size must be positive");
        Self { cell, map: HashMap::new() }
    }

    /// 🧭 Integer cell containing `p`, via floor division that stays correct for negative coordinates.
    pub fn cell_of(&self, p: [f64; 3]) -> (i32, i32, i32) {
        ((p[0] / self.cell).floor() as i32, (p[1] / self.cell).floor() as i32, (p[2] / self.cell).floor() as i32)
    }

    /// ➕ Buckets `id` into the cell containing `p`.
    pub fn insert(&mut self, p: [f64; 3], id: u32) {
        let key = self.cell_of(p);
        self.map.entry(key).or_default().push(id);
    }

    /// 🔍 All ids bucketed in the 27 cells surrounding (and including) the cell of `p`, sorted ascending for determinism.
...
impl Grid2 {
    ...
    /// 🔍 All ids bucketed in the 9 cells surrounding (and including) the cell of `p`, sorted ascending for determinism.
...
/// 🧬 Interleaves the low 21 bits of `x`, `y`, `z` into a 63-bit Morton code (`x` in bit 0, `y` in bit 1, `z` in bit 2 of each triple). <https://en.wikipedia.org/wiki/Z-order_curve>
pub fn morton3_encode(x: u32, y: u32, z: u32) -> u64 {
    morton3_spread(x) | (morton3_spread(y) << 1) | (morton3_spread(z) << 2)
}

/// 🧬 Recovers the 21-bit `(x, y, z)` coordinates interleaved by [`morton3_encode`].
...
/// 🐙 Linear point octree: points hashed to 63-bit Morton codes on a `2^max_depth` grid over the computed bounding cube, stored as a code-sorted array for iterative octant range queries and voxel downsampling. <https://en.wikipedia.org/wiki/Octree>
#[derive(Clone, Debug)]
...
impl PointOctree {
    ...
    /// 🧊 Voxel-downsample primitive: per occupied cell of edge length `cell` (anchored at the cube origin), the point count and centroid, sorted by the cell's Morton code for determinism.
    ...
    /// 📦 Ids of all points inside the closed box `[min, max]`, sorted ascending; iterative octant descent over Morton-code intervals with exact per-point filtering at the leaves.
