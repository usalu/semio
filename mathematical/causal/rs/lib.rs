//! 🔀 Causal inference: DAG/CPDAG/PAG models, d-separation, PC-stable/GES/LiNGAM/FCI discovery, backdoor/frontdoor/ID-algorithm identification, linear-Gaussian and discrete SCMs with interventional and counterfactual queries, and potential-outcome effect estimators.
//!
...
/// 🧮 Normalizes an unordered pair so `a <= b`, the canonical key for undirected-edge sets.
...
// #region 🔖Dag
/// 🧭 A directed acyclic causal graph over named variables.
...
impl CausalDag {
    /// 🧭 Builds a DAG from index edges, validating acyclicity via
    ...
    /// 🧭 Builds a DAG from named edges, resolving each name against `names`.
    ...
    /// 🧭 Strict ancestors of `v` (excludes `v` itself), ascending.
    ...
    /// 🧭 Strict descendants of `v` (excludes `v` itself), ascending.
...
// #region 🔖Cpdag
/// 🔀 A completed partially directed acyclic graph (CPDAG): directed causal edges plus undirected
...
impl Cpdag {
    ...
    /// 🔀 A fully undirected complete graph — the PC algorithm's starting skeleton.
    ...
    /// 🔀 Resolves the undirected edge `(a, b)` to `a -> b`.
    ...
    /// 🧭 The CPDAG of `dag`'s Markov equivalence class: skeleton + v-structures, closed under
    ...
    /// 🧭 Extends to a consistent member DAG via the Dor–Tarsi (1992) algorithm; `None` if no
...
/// 🧩 Meek's orientation-propagation rules R1–R3 (Meek, UAI 1995), applied to a fixed point:
...
// #region 🔖DSeparation
/// 🚧 One conditional-independence statement `x ⟂ y | z` implied by a graph.
#[derive(Clone, Debug, PartialEq, Eq)]
...
/// 🚧 Tests `x ⟂ y | z` in `dag` via the moralized-ancestral-graph criterion (Lauritzen et al.
...
/// 📜 Local-Markov implied CIs, emitted pairwise: for each `v`, `v ⟂ w | pa(v)` for every
...
/// 🔬 Tests every implied CI against data — a graph-fit diagnostic ("model criticism").
...
// #endregion 🔖DSeparation

// #region 🔖CiTest
/// 🔬 A conditional-independence test on tabular data, indexed by table/DAG column.
pub trait CiTest {
...
/// 🔬 Fisher-z partial-correlation test for continuous data, precomputing the correlation matrix
...
impl FisherZ {
    /// 🔬 Precomputes the complete-case correlation matrix over `columns` (in the same order as
...
/// 📦 PC-stable output: the resulting CPDAG plus every separating set found while pruning the skeleton.
pub struct PcResult {
    pub cpdag: Cpdag,
    pub sepsets: HashMap<(usize, usize), Vec<usize>>,
}

/// 🔎 PC-stable (Colombo & Maathuis, JMLR 2014): level-wise skeleton pruning using adjacency sets
...
/// 🧩 Orients unshielded colliders `a -> c <- b` for every non-adjacent pair `a, b` sharing
...
/// 📉 Decomposable Gaussian BIC of one node given a candidate parent set (Chickering, JMLR 2002):
...
/// 📉 Sum of local BICs over every node of `dag` given its own parent set.
...
/// 📦 DirectLiNGAM output: the recovered causal order and the pruned weighted adjacency (`weights[child][parent]`).
...
/// 🔎 DirectLiNGAM (Shimizu et al., JMLR 2011): repeatedly extracts the most-exogenous remaining
...
/// 🔎 Score-based structure search using decomposable Gaussian BIC ([`dag_bic`]) — greedy
...
/// 🚪 Backdoor criterion (Pearl 2009 §3.3.1): `z` contains no descendant of `x` and blocks every
...
/// 🚪 Every inclusion-minimal backdoor set up to `max_size`, searched over
...
/// 🚪 Frontdoor criterion for mediator set `m` (Pearl 2009 §3.4): `m` intercepts every directed
...
/// 🧾 How an interventional query was identified — drives which estimator is applicable.
#[derive(Clone, Debug, PartialEq)]
...
/// 🧾 Finds an identification strategy for `P(y | do(x))` on a fully-observed DAG, checked in
...
// #region 🔖ScmLinear
/// 🧮 Linear-Gaussian SCM: `v = intercept_v + Σ_p weights[v][p]·p + ε_v`, `ε_v ~ N(0, noise_var[v])`.
#[derive(Clone, Debug)]
...
impl LinearGaussianScm {
    /// 📈 Per-node OLS on parents (roots get their marginal mean/variance).
    ...
    /// 🧮 `E[v]` for every `v`, via forward substitution in topological order.
    ...
    /// 🧮 Implied covariance `(I−B)⁻¹ D (I−B)⁻ᵀ`.
    ...
    /// 🎯 `E[y | do(x=1)] − E[y | do(x=0)]`; equals [`LinearGaussianScm::total_effect`] for a
    ...
    /// 🔮 Abduction–action–prediction (Pearl 2009 ch. 7, Thm 7.1.7): recovers every exogenous
    ...
    /// 🔮 `E[target | do(interventions)]`: the mean of the intervened model at `target`.
...
// #region 🔖ScmDiscrete
/// 🎲 Conditional probability table for one node: `probs` is flat, indexed `[node value fastest,
...
/// 🎲 A CPT-based Bayesian network treated as a discrete SCM: interventions replace a node's CPT
...
impl DiscreteScm {
    /// 📊 MLE CPTs from categorical columns with additive (Laplace) smoothing `pseudocount` (`0.0` = pure MLE).
    ...
    /// 🔢 `P(target | evidence)` by variable elimination (Zhang & Poole 1994) over a
    ...
    /// 🔮 `P(target | do(interventions), evidence)`: posterior on the intervened network.
...
/// 🧮 A variable-elimination factor: a flat table over `vars`, mixed-radix with `vars[0]` fastest-varying.
#[derive(Clone, Debug)]
struct Factor {
    vars: Vec<usize>,
    cards: Vec<usize>,
...
/// 🧮 Decodes a flat mixed-radix index (`vars[0]` fastest-varying) into a var -> value assignment.
...
// #endregion 🔖ScmDiscrete

// #region 🔖Estimation
/// 📏 A point estimate with an optional bootstrap percentile confidence interval.
...
/// 📏 Level-1 baseline: `E[y|t=1] − E[y|t=0]` — biased under confounding, useful as a comparison point.
...
/// 📏 G-formula / regression-adjustment ATE: fit `y ~ t + covariates` by OLS, then average the
...
/// 📏 Inverse-probability weighting ATE: logistic propensity model, Hájek (normalized-weight)
...
// #region 🔖Query
/// 🔮 One what-if request: `do(...)` interventions and, for counterfactual queries, `given(...)` factual evidence.
#[derive(Clone, Debug, Default)]
pub struct WhatIf {
...
impl LinearGaussianScm {
    /// 🔮 Level 2 (empty evidence): interventional mean of `target`. Level 3 (evidence covering
...
mod tests {
    ...
    // #endregion 🔖CiTestTests

    // #region 🔖DiscoveryTests
    ...
    // #endregion 🔖IdentificationTests

    // #region 🔖ScmLinearTests
    #[test]
    ...
    // #endregion 🔖ScmLinearTests

    // #region 🔖ScmDiscreteTests
