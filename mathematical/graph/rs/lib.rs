...
// #region 🔖Storage
/// 📦 Per-node record: attribute bag plus, for ported storages, the handles anchored on it (stays empty for `Normal` storages).
...
/// 📦 Per-edge record: typed endpoints (node ids for `Normal`, handle ids for `Ported`) plus attribute bag.
#[derive(Clone, Debug)]
...
impl<P: PortModel, D: Directedness> Storage<P, D> {
    /// 🆕 Empty storage; every id allocator starts at `0` and is monotone — an id is never reused, even after removal.
    pub fn new() -> Self {
        Self {
            nodes: BTreeMap::new(),
            edges: BTreeMap::new(),
    ...
    /// 🔗 Resolves an edge endpoint down to the node it lives on: identity for `Normal` (`Endpoint == NodeId`), a `handle_owner` lookup for `Ported`.
    ...
    /// 🆔 Inserts a node at a caller-supplied id, or merges `attrs` into it if already present (NetworkX `add_node(id, **attrs)` semantics); bumps the allocator past `id` so future auto-ids never collide with it.
    ...
    /// 🔀 `Normal` storages upsert: an edge already connecting this pair gets `attrs` merged into it and its existing id returned (NetworkX `Graph`/`DiGraph`). `Ported` storages always create a fresh parallel edge with a new `EdgeId` (NetworkX `MultiGraph`/`MultiDiGraph`).
    ...
    // #subregion Handles
    /// 🪝 Allocates a new handle anchored on `node`; only meaningful when `P::HAS_PORTS` — returns `None` otherwise (or if `node` doesn't exist), never panics.
    ...
    /// 🧹 Removes every node, edge, and handle; graph-level attrs are cleared too. Id allocators are NOT reset — ids are never reused, even across a clear.
    ...
    /// 🧹 Removes every edge but keeps nodes (and their handles) and graph-level attrs.
...
// #region 🔖View traits
/// 🪢 Node-level edge reference; carries its own id plus both endpoint node ids. Port/handle detail is already resolved away — algorithms never see a `HandleId`.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct EdgeRef {
    pub id: EdgeId,
    pub u: NodeId,
...
/// 🪟 Structural read-only view every future algorithm crate is written against — the single most important contract in this campaign; keep it minimal and stable.
pub trait GraphView {
...
/// 1️⃣ Unweighted default: every edge costs `1.0` (NetworkX's unweighted-graph convention).
#[derive(Clone, Copy, Debug, Default)]
...
impl<P: PortModel, D: Directedness> GraphView for Storage<P, D> {
    ...
    /// 📇 One `EdgeRef` per stored edge, in `EdgeId` order — a self-loop appears once here even though it counts twice towards `degree`.
    fn edges(&self) -> impl Iterator<Item = EdgeRef> {
...
// #endregion 🔖View traits

// #region 🔖Csr
/// 🧊 Frozen, index-based CSR adjacency snapshot for hot algorithms; supersedes the ad-hoc `algorithms::Adjacency` for NEW code (that type is left untouched — old call sites keep using it). Node index assignment is `0..n` in sorted `NodeId` order, so two snapshots of the same graph always assign the same indices.
#[derive(Clone, Debug)]
pub struct Csr {
    node_ids: Vec<NodeId>,
...
/// 🔎 Restricts a graph to an edge subset; nodes are exactly the endpoints of the included edges.
...
// #region 🔖Interner
/// 🔤 Generalized, bidirectional label<->`NodeId` map — the generic successor to the string-only `algorithms::IdIndex` (which stays untouched for old call sites). `intern` is idempotent: the same label always maps to the same id.
#[derive(Clone, Debug, Default)]
...
impl<L: Ord + Clone + std::hash::Hash> Interner<L> {
    ...
    /// ➕ Returns the existing id for `label` if already interned, otherwise allocates the next sequential id.
...
// #region 🔖GraphError
/// 🚨 Flat, non-generic error enum mirroring the NetworkX exception hierarchy; every downstream algorithm crate returns `Result<_, GraphError>`. Nothing here is generic over node/edge label types — everything is `NodeId`/`EdgeId`/`u64`/`String` — so this shape stays stable across the whole family.
...
/// 🔗 Consecutive-pair iterator: `[a, b, c] -> [(a, b), (b, c)]`.
pub fn pairwise<T: Copy>(items: &[T]) -> impl Iterator<Item = (T, T)> + '_ {
    items.windows(2).map(|w| (w[0], w[1]))
}

/// 🎯 Deterministic representative element (the first one) from a slice.
...
mod tests {
    ...
    mod quick {
        ...
        /// 🎲 Tiny deterministic xorshift so this crate doesn't need `mathematical_random` as a dependency just for one fuzz test.
