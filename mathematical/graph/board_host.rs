...
#![allow(clippy::missing_errors_doc, reason = "Graph board host is internal to mathematical graph.")]

use infinite_cavas::board_json_visible_option;
use infinite_cavas::*;
...
use infinite_cavas::vcompute::{circle_handle_angle_toward, compute_edge_bezier_points, distance_between, distance_point_to_cubic_bezier, handle_position_on_circle, handle_position_on_rectangle, rectangle_handle_angle_toward};
...
impl BoardHost {
    ...
    fn get_or_build_icon_paint(&self, encoded: &str, fg: Color, bg: Color, preserve_original_style: bool) -> Option<(f64, f64, f64, f64, CachedIconBody)> {
        ...
        let (bx, by, bw, bh, body) = match resolved {
            ...
            infinite_cavas::icon_codec::BoardResolvedIcon::SvgThemed(s) => {
                let tree = usvg::Tree::from_str(s.trim(), infinite_cavas::svg_icon_vello09::usvg_options_board_icons()).ok()?;
            ...
            infinite_cavas::icon_codec::BoardResolvedIcon::SvgPlain(s) => {
                let svg_t = s.trim();
                let tree = usvg::Tree::from_str(svg_t, infinite_cavas::svg_icon_vello09::usvg_options_board_icons()).ok()?;
                let (bx, by, bw, bh) = infinite_cavas::svg_icon_vello09::svg_icon_content_bounds(&tree);
                if !(bw > 0.0 && bh > 0.0 && bw.is_finite() && bh.is_finite()) {
                    return None;
                }
    ...
    fn handle_slot_center_world(&self, node_id: &str, hw: Point, flush_distance: f64) -> Option<Point> {
        ...
        let normal = infinite_cavas::vcompute::normalize_or_zero(hw - nc);
