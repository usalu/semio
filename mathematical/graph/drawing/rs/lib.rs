...
pub mod force {
    ...
    /// ⭕ Deterministic circular layout: `n` points evenly spaced on a ring of `radius` around `center`.
    ...
    /// 🔲 Deterministic grid layout: `n` points in row-major order, `cols` per row, spaced by `gap`.
