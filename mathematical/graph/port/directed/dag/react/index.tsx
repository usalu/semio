...
const DEFAULT_DAG_LOD_SCALE_JSON = JSON.stringify([
...
/** @emoji 📶 Parses WASM {@link DagSession.lodScaleJson} rows into typed LOD entries. */
export function parseDagLodScaleJson(raw: string): readonly DagLodEntry[] {
...
function readDagLodScaleJson(): string {
  const session = new DagSession() as { lodScaleJson?: () => string };
...
const DAG_WHEEL_ZOOM_IN_FACTOR = 1.1;
const DAG_WHEEL_ZOOM_OUT_FACTOR = 0.9;

function dagWheelDeltaScale(deltaMode: number): number {
  if (deltaMode === WheelEvent.DOM_DELTA_LINE) return 16;
  if (deltaMode === WheelEvent.DOM_DELTA_PAGE) return 400;
  return 1;
}

function dagScreenToWorld(session: DagSession, sx: number, sy: number): { readonly x: number; readonly y: number } {
  const world = session.screenToWorld(sx, sy);
  return { x: world[0] ?? 0, y: world[1] ?? 0 };
}

function dagCameraFromFixtureJson(json: string): { readonly x: number; readonly y: number; readonly zoom: number } {
  const fixture = JSON.parse(json) as { readonly camera?: { readonly x: number; readonly y: number; readonly zoom: number } };
  return fixture.camera ?? { x: 0, y: 0, zoom: 1 };
}

function dagCameraFromSession(session: DagSession): { readonly x: number; readonly y: number; readonly zoom: number } {
  try {
    return dagCameraFromFixtureJson(session.fixtureJson());
  } catch {
    return { x: 0, y: 0, zoom: 1 };
  }
}

/** @emoji 🔍 Zooms the DAG canvas toward the pointer like flow play. */
export function dagWheelZoomScreen(session: DagSession, sx: number, sy: number, deltaY: number): void {
  const before = dagScreenToWorld(session, sx, sy);
  const camera = dagCameraFromSession(session);
  const factor = deltaY < 0 ? DAG_WHEEL_ZOOM_IN_FACTOR : DAG_WHEEL_ZOOM_OUT_FACTOR;
  const zoom = Math.min(DAG_CAMERA_ZOOM_MAX, Math.max(DAG_CAMERA_ZOOM_MIN, camera.zoom * factor));
  session.setCamera(camera.x, camera.y, zoom);
  const after = dagScreenToWorld(session, sx, sy);
  session.setCamera(camera.x + before.x - after.x, camera.y + before.y - after.y, zoom);
}

function dagPanGestureActive(button: number, altKey: boolean, spacePressed: boolean): boolean {
  return button === 1 || (button === 0 && (altKey || spacePressed));
}

/** @emoji 🧪 Whether a pointer should pan the DAG canvas instead of editing the graph. */
export function dagCanvasPanGestureActive(button: number, altKey: boolean, spacePressed: boolean): boolean {
  return dagPanGestureActive(button, altKey, spacePressed);
}
...
export function DagCanvas({ fixtureJson, className, reorganize, onFixtureChange, onSessionReady, onAfterPointerUp, automaticLod = true, lod, onLodChange }: DagCanvasProps): React.JSX.Element {
  ...
  const fixtureJsonRef = useRef(fixtureJson);
  const panAnchorRef = useRef<{ readonly startSx: number; readonly startSy: number; readonly camX: number; readonly camY: number; readonly zoom: number } | null>(null);
  ...
  const persistFixture = useCallback(() => {
    const session = sessionRef.current;
    if (!session || !onFixtureChangeRef.current) return;
    try {
  ...
  const loadFixture = useCallback(() => {
    const session = sessionRef.current;
    if (!session) return;
    const json = fixtureJsonRef.current ?? dagFixtureToJson(DAG_DEFAULT_FIXTURE);
    try {
      session.loadFixtureJson(json);
      renderFrame();
    } catch (err) {
      console.log(`[DEBUG] dag canvas fixture load failed: ${String(err)}`);
    }
  }, [renderFrame]);

  useEffect(() => {
    fixtureJsonRef.current = fixtureJson;
  }, [fixtureJson]);
  ...
  useEffect(() => {
    ...
    void session.attachCanvas(canvas, initW, initH, dpr).then(() => {
      ...
      const onPointerDown = (ev: PointerEvent) => {
        ...
        if (dagPanGestureActive(ev.button, ev.altKey, spacePressedRef.current)) {
          const camera = dagCameraFromSession(session);
          panAnchorRef.current = { startSx: x, startSy: y, camX: camera.x, camY: camera.y, zoom: camera.zoom };
          canvas.setPointerCapture(ev.pointerId);
          renderFrame();
          return;
        }
        session.pointerDown(x, y, ev.shiftKey);
        renderFrame();
      };
      const onPointerMove = (ev: PointerEvent) => {
        ...
        if (panAnchor) {
          const dx = (x - panAnchor.startSx) / panAnchor.zoom;
          const dy = (y - panAnchor.startSy) / panAnchor.zoom;
          session.setCamera(panAnchor.camX - dx, panAnchor.camY - dy, panAnchor.zoom);
          renderFrame();
          return;
        }
      ...
      const onPointerUp = (ev: PointerEvent) => {
        const { x, y } = clientToCanvas(ev.clientX, ev.clientY);
        const panned = panAnchorRef.current != null;
      ...
      const onWheel = (ev: WheelEvent) => {
        ev.preventDefault();
        const scale = dagWheelDeltaScale(ev.deltaMode);
        const deltaY = ev.deltaY * scale;
        syncWheelZoomActive(true);
        if (wheelZoomEndRef.current != null) clearTimeout(wheelZoomEndRef.current);
        wheelZoomEndRef.current = setTimeout(() => {
          wheelZoomEndRef.current = null;
          syncWheelZoomActive(false);
        }, 120);
        const { x, y } = clientToCanvas(ev.clientX, ev.clientY);
        dagWheelZoomScreen(session, x, y, deltaY);
        persistFixture();
        renderFrame();
      };
      const onKeyDown = (ev: KeyboardEvent) => {
        if (ev.code === "Space" && !ev.repeat) spacePressedRef.current = true;
      };
      const onKeyUp = (ev: KeyboardEvent) => {
        if (ev.code === "Space") spacePressedRef.current = false;
      };
  ...
  }, [loadFixture, persistFixture, renderFrame, syncWheelZoomActive]);

  useEffect(() => {
    loadFixture();
  }, [fixtureJson, loadFixture]);
...
if (import.meta.vitest) {
  ...
  describe("dag camera", () => {
    it("zooms toward the pointer with wheel factors", () => {
      const session = {
        ...
        fixtureJson() {
          return JSON.stringify({ camera: this.camera });
        },
      } as unknown as DagSession;
      dagWheelZoomScreen(session, 100, 100, -120);
      expect(session.fixtureJson()).toContain('"zoom":1.1');
    });

    it("treats middle mouse as a pan gesture", () => {
      expect(dagCanvasPanGestureActive(1, false, false)).toBe(true);
      expect(dagCanvasPanGestureActive(0, true, false)).toBe(true);
      expect(dagCanvasPanGestureActive(0, false, true)).toBe(true);
      expect(dagCanvasPanGestureActive(0, false, false)).toBe(false);
    });
  });
