//! ➕ Graph combination and transformation operators: union, products, complement, contractions, line graphs — NetworkX `operators` module parity, built generically over `mathematical_graph::Storage<P, D>`.

use mathematical_graph::{AttrView, Directed, Directedness, EdgeId, GraphError, GraphView, HandleId, NodeId, Normal, PortModel, PropertyBag, Storage};
...
/// 🔁 Resolves an old typed endpoint to its counterpart in a freshly built storage, using `node_map`/`handle_map` recorded while copying nodes/handles. Panics if the endpoint's underlying node or handle wasn't copied first — every call site copies nodes (and their handles) before touching edges.
...
/// 🆕 Copies the whole of `src` into `dst` under a **fresh** id space (`add_node_with` instead of `add_node_with_id`) — the `disjoint_union` building block.
...
// #region 🔖SetOperators
/// 🤝 NetworkX `union`: `g` and `h` must have disjoint node id sets — returns `GraphError::AmbiguousSolution` otherwise (a runtime check standing in for what NetworkX enforces at call time). On success, every node/edge of both graphs is copied into a fresh storage, ids unchanged.
...
/// 🆕 NetworkX `disjoint_union`: always succeeds by relabelling both inputs into a fresh, non-overlapping id space — `g`'s nodes first (in `NodeId` order), then `h`'s.
...
/// ➖ NetworkX `difference`: every node of `g`, but only the edges of `g` that have no counterpart `(u, v)` in `h`.
...
// #region 🔖Products
/// 🔗 True iff `g` has an edge between `a` and `b` (order-sensitive for directed `g`; `Storage`'s undirected adjacency is already symmetric, so order doesn't matter there).
...
/// 🔗 Shared quadruple-loop edge builder for the four products below: wires `(u1, v1)-(u2, v2)` whenever `include` says so. Deliberately not optimized past `O(|Vg|² · |Vh|²)` — products are meant for small graphs here (see module docs).
...
/// 📖 NetworkX `lexicographic_product`: `(u1,v1)-(u2,v2)` iff `u1~u2` in `g`, OR (`u1==u2` and `v1~v2` in `h`).
...
// #region 🔖Power
/// 🧭 Tiny local BFS (not a public API): nodes reachable from `src` within `k` hops via `out_neighbors`, keyed by `NodeId` for determinism.
...
/// 🔋 NetworkX `power`: edge `u-v` (`u ≠ v`) iff the shortest-path distance from `u` to `v` is at most `k` hops. Requires `k ≥ 1`, matching NetworkX's own precondition.
...
// #region 🔖Contraction
/// 🫂 NetworkX `contracted_nodes`: merges `v` into `u` — `v` is removed, its attrs merged onto `u` (`u`'s keys win on conflict, matching `PropertyBag::extend` order), and every edge that touched `v` is redirected to `u`. An edge that becomes a `u-u` self-loop *because of* the merge (i.e. it originally touched `v`) is dropped unless `self_loops` is true; a self-loop that already existed on `u` before the merge is always kept. Always returns a fresh copy — mirrors NetworkX's non-mutating default (`copy=True`).
...
/// 🔗 NetworkX `contracted_edge`: contracts the two endpoints of `edge`; `GraphError::EdgeNotFound` if it doesn't exist.
...
mod tests {
    use super::*;
    use mathematical_graph::Undirected;
    ...
    fn attrs_of(pairs: &[(&str, &str)]) -> PropertyBag {
        ...
        for (k, v) in pairs {
            bag.insert(k.to_string(), mathematical_graph::PropertyValue::String(v.to_string()));
        }
        bag
    }
    ...
    fn complement_of_a_path_matches_a_hand_count() {
        // 🔺 Path 0-1-2 (undirected, 3 nodes, 2 edges) has C(3,2)-2 = 1 missing pair: 0-2.
        let mut g: Storage<Normal, Undirected> = Storage::new();
        for id in [0, 1, 2] {
            g.add_node_with_id(id, PropertyBag::new());
        }
    ...
    fn tensor_product_requires_adjacency_on_both_sides() {
        ...
        assert_eq!(prod.node_count(), 4);
        // 🔗 (0,0)-(1,1) and (0,1)-(1,0): exactly 2 edges.
    ...
    fn lexicographic_product_includes_cross_block_edges() {
        ...
        // 🔗 g-adjacent pairs pull in all 4 combinations of (v1, v2) for u1~u2, plus the within-block h edges.
