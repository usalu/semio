//! 🔌 Declarative app plugin SDK — build fully declarative Rust apps bundled into hot-swappable WASM plugins.
...
impl FormPanelBuilder {
    ...
    /// 📋 Routes the OS `form.dictionary` resource shape (see the `ArtifactKindSpec { id:
...
pub fn assert_constitutional_crates(manifest_dir: &str) {
    ...
    // 🏛️ `../../../app` covers the constitutional-split bundle layout `s/plugin/<p>/manifest/artifact/rs`,
    // whose app tree lives three levels up at `s/plugin/<p>/app`.
    let app_root = [base.join("../app"), base.join("../../app"), base.join("../../../app")]
        .into_iter()
        .find(|candidate| candidate.is_dir());
    let Some(app_root) = app_root else {
        panic!("constitutional-crate gate: no `app/` directory found next to {manifest_dir} (tried ../app, ../../app, ../../../app)");
    };
