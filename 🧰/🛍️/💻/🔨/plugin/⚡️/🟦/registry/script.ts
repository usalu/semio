...
import { BundleScript, getWorkspaceRoot, ScriptRouter, runBundleScriptMain } from "../../../../../../🦑/🔨/lib/⚡️/🟦/📦.ts";
...
function findPluginCargoFiles(root: string): string[] {
  ...
  function walk(dir: string) {
    for (const name of readdirSync(dir)) {
      ...
      if (st.isDirectory()) {
        walk(path);
      } else if (name === "Cargo.toml" && !path.includes("/🧰/🛍️/💻/🔨/plugin/⚡️/🦀/")) {
        const isModuleCrate = /\/🔨\/[^/]+\/⚡️\/🦀\/Cargo\.toml$/.test(path);
        // 🏛️ Contribution bundle crate: `✏️/🔌/<p>/🧩/<e>/⚡️/🦀/Cargo.toml` (new node/document
        // types the plugin contributes, e.g. imperative's control/logic/math/text).
        const isExtensionCrate = /\/✏️\/🔌\/[^/]+\/🧩\/[^/]+\/⚡️\/🦀\/Cargo\.toml$/.test(path);
        // 🏛️ Plugin bundle crate lives at `✏️/🔌/<p>/🛂/🗿/⚡️/🦀/Cargo.toml`.
        const isPluginBundleCrate = /\/✏️\/🔌\/[^/]+\/🛂\/🗿\/⚡️\/🦀\/Cargo\.toml$/.test(path);
...
function discoverExamplesForPlayground(repoRoot: string, cratePath: string, pluginId: string, variant: string): string[] {
  ...
  // 🏛️ Under `✏️/🔌/<p>/...` the tech root is the plugin folder (3 segments), not `✏️` itself.
  const techRoot = segments[0] === "✏️" && segments[1] === "🔌" ? segments.slice(0, 3).join("/") : segments[0];
  const rootDir = join(repoRoot, techRoot, "example");
  ...
  if (variant.startsWith(pluginId) && variant.length > pluginId.length) {
    ...
    const dir = join(repoRoot, techRoot, "🎛️", suffix, "example");
...
function validateConstitutionalCrates(repoRoot: string): string[] {
  ...
  for (const pluginName of readdirSync(pluginRoot).sort()) {
    ...
    for (const { label, dir } of appDirs) {
      ...
      if (!existsSync(join(dir, "⚡️", "🦀", "Cargo.toml"))) missing.push("rs");
      for (const slot of CONSTITUTIONAL_SLOTS) {
        if (!existsSync(join(dir, "🔨", slot, "⚡️", "🦀", "Cargo.toml"))) missing.push(slot);
      }
      if (missing.length > 0) {
        errors.push(`✏️/🔌/${label} is missing constitutional crate slot(s): ${missing.join(", ")} (expected 7: rs, ${CONSTITUTIONAL_SLOTS.join(", ")})`);
      }
    }
  }
  return errors;
