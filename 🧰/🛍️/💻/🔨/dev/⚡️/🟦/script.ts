...
import { generatePluginRegistry, isStudioPluginFilter, writePlaygroundSession, type PluginRegistryEntry } from "../../../plugin/⚡️/🟦/registry/script.ts";
import { PNG } from "pngjs";
import pixelmatch from "pixelmatch";

const repoRoot = getWorkspaceRoot();
const pluginOutRoot = join(repoRoot, "framework/product/os/module/dev/js/plugin-modules");
const playgroundSessionPath = join(repoRoot, "framework/product/os/module/dev/js/generated/session.ts");
...
// 📦 Must match \`framework/os/core/js/index.ts\`'s \`BLOB_ENDPOINT_PATH\`.
...
async function ensurePluginRegistry(filterPlugin?: string): Promise<void> {
  const registryScript = join(repoRoot, "framework/product/os/module/plugin/registry/script.ts");
...
async function buildPlugins(filterPlugin?: string): Promise<void> {
  ...
  const stalePublicPlugins = join(repoRoot, "framework/product/os/module/dev/js/public/plugin-modules");
...
async function buildEngineWasm(variant: string, renderer: string): Promise<void> {
  ...
  const graphScript = join(repoRoot, "framework/module/surface/node-graph/rs/script.ts");
  ...
  const editorScript = join(repoRoot, "framework/module/editor/rs/script.ts");
  ...
  const flowCoreScript = join(repoRoot, "framework/product/os/module/flow/core/script.ts");
...
class DevScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    ...
    if (renderer === "wgpu") {
      ...
      const wgpuScript = join(repoRoot, "framework/product/os/module/renderer/wgpu/script.ts");
      const serveStatus = runCmdStatus("bun", [wgpuScript, "serve"], {
        cwd: join(repoRoot, "framework/product/os/module/renderer/wgpu"),
        env: {
          ...process.env,
          SEMIO_PLUGIN: plugin,
...
class VerifyScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    ...
    if (runCmdStatus("bunx", ["vitest", "run"], { cwd: join(repoRoot, "framework/product/os/module/renderer/js/react") }) !== 0) throw new Error("framework-renderer-react tests failed");
    await runStudioE2eVerify(studioUrl, timeoutMs);
...
/** 🌳DOM-side structural walk — every element carrying `data-ui-path` (see `framework/os/renderer/js/react/index.tsx`
 ...
 * 0–1 floats (see `framework/os/renderer/wgpu/rs/lib.rs`'s `🔬IntrospectionVisualFields` doc comment) —
 ...
 * `framework/os/renderer/wgpu/rs/lib.rs`'s `🔬Introspection` walk, which is a different file, out of
...
async function startParityDevServer(renderer: ParityRenderer, variant: string, port: number): Promise<ParityServerHandle> {
  const devScript = join(repoRoot, "framework/product/os/module/dev/js/script.ts");
  ...
  .register(
    ...
    class extends BundleScript {
      async run(segments: string[]): Promise<void> {
        ...
        // catalog for every `bun ./script.ts program <variant>` invocation.
