...
// #region 🔖Categorical
/// 🚫 Reserved categorical code marking a missing value (mirrors `f64::NAN` for continuous columns).
...
impl CategoricalColumn {
    ...
    /// 🔢 Per-level occurrence counts, missing values excluded.
    ...
    /// 🔢 One-hot encoding, one `Vec<f64>` per row; a missing row is filled with `NaN` indicators.
...
// #region 🔖Column
/// 📦 One table column: either a continuous `f64` series (missing = `NaN`) or a [`CategoricalColumn`].
#[derive(Clone, Debug, PartialEq, serde::Serialize, serde::Deserialize)]
pub enum Column {
    Continuous(Vec<f64>),
...
impl Table {
    ...
    /// 🔎 Infallible variant of [`Table::column_index`].
    ...
    /// 🔀 Projects the table to the given columns (order preserved, repeats allowed).
    ...
    /// 🔀 Gathers rows by index (repeats allowed — needed for bootstrap resampling).
    ...
    /// ✅ Row indices, ascending, where none of the given columns is missing.
    ...
    /// ✅ `select_rows(complete_rows(columns))` — the complete-case sub-table.
...
/// 📄 Splits `text` into rows of raw string fields via a single-pass RFC-4180-subset state machine:
...
impl Table {
    /// 📄 Parses CSV text, inferring each column's type: `Continuous` if every non-missing field
    ...
    /// 📄 Serializes to CSV text: missing values become empty fields, floats use Rust's shortest
