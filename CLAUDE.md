...
- You MUST use semio-repo mcp (or the cli `./semio-repo/cli/cli`) for repo-specific infrastructure.
  ...
  - If no existing ticket is covering the same task, you MUST create a new ticket with mcp tool `ticket_open` (or `./semio-repo/cli/cli ticket open <goal-id> <title> <prompt> <client> <llm> --draft <draft-id>? --parent <parent-ticket-id>?`). This creates a ticket folder `.semio-repo/🎫/YY/MM/DD/TICKETSLUG` along with a ticket file `ticket.md` in it.
