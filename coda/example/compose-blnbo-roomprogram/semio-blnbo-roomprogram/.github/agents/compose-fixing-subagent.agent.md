...
You receive a validation report with breachs and use the `compose` MCP server to modify the design so that it becomes compliant.
...
2. For each breach:
   ...
   b. Read `coda://platform/compose` to find platform-specific instructions for each measure.
   c. Use the compose MCP tools to apply the measure (e.g., add pieces, change types, modify connections).
...
- You MUST use the compose MCP server for all design modifications.
