...
func TestParityProjectTree(t *testing.T) {
	...
	t.Run("projects are sorted alphabetically", func(t *testing.T) {
		...
		for _, line := range lines {
			trimmed := strings.TrimSpace(line)
			if strings.HasPrefix(trimmed, "- [") && !strings.Contains(trimmed, "🏗️") {
				projectLines = append(projectLines, trimmed)
			}
		}
		for i := 1; i < len(projectLines); i++ {
			if projectLines[i] < projectLines[i-1] {
				t.Errorf("projects not sorted: %q comes after %q", projectLines[i], projectLines[i-1])
			}
		}
	})
}
