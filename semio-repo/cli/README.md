...
Ticket and goal interaction decoding accepts both legacy object-form and current string-form `author` payloads. Interaction format uses system (string), client (top-level), and dates.created/finished.
...
The CLI exposes an export command that emits a SQLite snapshot of bundles, folders, files, sections, contributors, tickets, policies, and breachs.
...
Ticket close derives semantic diffs from unstaged git diffs only (index vs working tree) after applying repo exclusions and `.gitignore` filters, so tooling and tests must pass at least one considered (non-gitignored) path.
...
Inline comment breachs are grouped per contiguous inline-comment block while comment detection skips markers inside string literals and template literal text.
...
Query search uses a persistent Bleve index under `.semio-repo/cache/<repoID>/` (repoID = sha256 of absolute repo path). Cache invalidation uses a composite git fingerprint: superproject HEAD, superproject dirty status hash, submodule pointers hash, and per-submodule working-state hash. When fingerprint matches meta.json, the disk index is opened; otherwise incremental or full rebuild. Granular updates: when a file changes, only the file, its folder, bundle, and project docs are re-indexed (not the whole repo). Parent docs include child names for searchability. A `.lock` file prevents concurrent builds; locks older than 5 minutes are treated as stale.
The `policy tree` command renders policies with their statutes as a nested tree, where each statute path (e.g., `code/header/missing-region`) is split on `/` and rendered as a multi-level tree. Redundant root nodes matching the policy ID are stripped. Statute IDs use `#`-separated titleized values (e.g., `🚫Code#Header#Missing Region`), statute paths use `/`-separated slugs (e.g., `code/header/missing-region`), and statute URIs use `/`-separated UPPERCASE slugs (e.g., `semiorepo://statute/code/header/missing-region`). `TitleizeSlug` converts hyphenated slugs to title case words. `StatutePathToIdValue` and `StatuteIdValueToPath` convert between path and ID value formats. `StatuteIdToUriPath` and `StatuteUriPathToId` convert between path and URI path formats.
...
Development work is tracked as tickets composed of interactions. Ticket creation does not create an interaction; interactions are explicitly started and finished, require file lists (`updated`, `created`, `removed`), and ticket close derives semantic diffs from unstaged git diffs only (index vs working tree).
...
File header artifact ID breachs MUST be autofixable by replacing the identified line with the correct artifact ID.
...
Section policy MUST exempt License and Specs children of Header from empty-section breachs.
...
Language-specific skip directives MUST be excluded from inline comment breachs alongside built-in directives (TODO, semio-ignore-).

Inline comment breachs MUST be grouped per contiguous inline-comment block.
...
Autofix MUST detect breachs, filter to autofixable kinds, group by file, and apply fixes bottom-up per file.

Autofixable statutes: file header artifact ID replacement, empty section removal, missing section end name, section name mismatch, inline comment removal, block comment removal, JSDoc comment removal.
...
Spec comments after section start markers MUST be exempt from inline comment breachs when they contain specification keywords (MUST, SHOULD, SHALL, MAY, REQUIRED, RECOMMENDED, OPTIONAL).

JSDoc and block comments that contain specification keywords MUST be exempt from JSDoc and block comment breachs.
...
The policy tree command MUST render policies with their statutes as a nested tree derived from splitting statute paths on the slash separator, stripping redundant root nodes that match the policy ID.

Statute paths MUST use slash-separated slugs. Statute IDs MUST use hash-separated titleized values prefixed with the statute emoji. Statute URIs MUST use slash-separated uppercase slugs.
...
Collection artifact types (projects, bundles, folders, files, sections, definitions, tickets, goals, drafts, todos, policies, statutes, contributors, commits) MUST have dedicated ID and URI formats.
...
A ticket interaction MUST store created and optional finished timestamps under dates.
...
Ticket finish MUST derive semantic diffs across projects, bundles, packages, folders, files, sections, and definitions via unstaged git diff only (index vs working tree), scoped to the files declared on the ticket.

Ticket line metrics MUST map added lines to current scopes and removed lines to base (index) scopes for semantic diffs.
...
The repo dev server MUST persist ticket state, scopes, claims, warnings, breachs, and event history in a local database.
...
Repo CLI MUST expose an export command that emits a SQLite snapshot of projects, bundles, packages, folders, files, sections, contributors, tickets, policies, and breachs.
