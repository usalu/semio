...
const TECHNOLOGIES = ["compose", "🧰framework", "✏️s", "🌎hub", "♻️mit-bestand"];

const PLUGINS = ["🎞️animate", "🏛️architect", "🧱block", "📐cad", "🕸️dag", "🖍️draw", "🔋energy", "🏗️fem", "🌊flow", "📋forms", "🌍gis", "📜imperative", "📏layout", "💠lowpoly", "➗mathematical", "📕norm", "🗒️note", "📖playbook", "🌀procedural", "🏭process", "🧩puzzle", "🖨️raster", "💡reasoning", "📸remodel", "🎬sequence", "🎥shooting", "🪵sourcing", "🪐space", "🔱trinity", "🌿vcs", "✒️writer"];
...
/** 🔌 Plugins must not relative-import a sibling plugin's implementation — cross-plugin sharing goes
...
module.exports = {
  forbidden: [
    ...
    },
    {
      ...
      severity: "error",
      comment: "🧰framework must not import plugin app packages — shells derive from app contributions",
      from: { path: "^🧰framework/" },
      to: {
        path: PLUGINS.map((p) => `^✏️s/🔌plugin/${p}/`).concat(PLUGINS.map((p) => `^@semio-tech/${p.replace(/^[^a-zA-Z]+/, "")}-`)),
      },
    },
    {
      name: "ui-no-framework-packages",
      severity: "error",
      comment: "🧰framework/🔨module/🖱️ui must stay presentational and business-logic free — no OS/plugin/framework coupling",
      from: { path: "^🧰framework/🔨module/🖱️ui/" },
      to: {
        path: ["^🧰framework/", "^@semio-tech/framework-"],
        pathNot: ["^🧰framework/🔨module/🖱️ui/", "^@semio-tech/ui-"],
      },
    },
    {
      ...
      from: { path: "^🧰framework/🛍️product/💻os/🔨module/📺renderer/⚡️implementation/🟦typescript/🧑‍🎨engine/⚛️react/" },
      to: {
        pathNot: ["^🧰framework/🛍️product/💻os/🔨module/📺renderer/⚡️implementation/🟦typescript/🧑‍🎨engine/⚛️react/", "^@semio-tech/ui-", "^@semio-tech/framework-core", "^react$", "^react/", "^react-dom$", "^react-dom/", "^node:"],
      },
    },
    {
