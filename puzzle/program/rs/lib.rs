//! 🧩 Puzzle program — 2D, 3D, and 5D play apps in one hot-swappable WASM program.

pub mod d2 {
    //! 🧩 Puzzle 2D program — declarative puzzle 2d play app bundled as a hot-swappable WASM program.
    ...
    /// 📶 Mirrors `ui_styling::metrics::board::SUGGESTION_OFFSET`; kept local since the program crate has no styling dependency.
    ...
    /// 🎯 `semio_framework_program::selection_ids`'s "ids" array plus a singular "id" fallback —
    ...
    /// ⚠️ Not routed through `semio_framework_program`'s `LocaleLabels`/`app_labels!`/`resolve_labels` — those
    ...
    impl DocumentApp for Puzzle2dPlayApp {
        ...
        fn app_labels(&self, view_state: &ViewState) -> semio_framework_program::AppLabelsOverlay {
            let labels = puzzle2d_labels(view_state);
            semio_framework_program::AppLabelsOverlay {
    ...
    pub fn register_puzzle2d_exports() {
        ...
        semio_framework_program::program_runtime::register_document_codec_for_app::<Puzzle2dPlayApp>(PUZZLE2D_FIXTURE_SCHEMA);
    ...
    mod tests {
        ...
        fn tool_registry_declares_fill_tool() {
            use semio_framework_program::{ToolRef, SET_ACTIVE_TOOL_ACTION_ID};
            let definition = create_puzzle2d_app().definition;
...
pub mod d3 {
    //! 🧊 Puzzle 3D program — 3D puzzle assembly play app bundled as a hot-swappable WASM program.
    ...
    fn inspector_text_field(id: impl Into<String>, label: impl Into<String>, mixed_text: semio_framework_program::UiInspectorMixedText, action: ActionDescriptor) -> UiNode {
        let id = id.into();
    ...
    fn inspector_header_and_delete(count: usize, noun: &str, labels: &Puzzle3dLabels) -> Vec<UiNode> {
        vec![
            ...
            UiNode::Button(semio_framework_program::UiButtonNode { id: Some("puzzle3d-play-inspector.delete".into()), icon_id: "trash-2".into(), label: labels.delete.into(), action: puzzle3d_action("deleteSelection", None), style: None, presence: UiPresence::default() }),
        ]
    }
    ...
    fn build_inspector_tree(envelope: &Puzzle3dScene, term_labels: &Puzzle3dLabels) -> UiNode {
        ...
        if !selection.object_ids.is_empty() {
            ...
            if !objects.is_empty() {
                ...
                fields.push(inspector_text_field("puzzle3d-play-inspector.object.label", term_labels.label, semio_framework_program::ui_inspector_mixed_text(&labels), patch_cmd("label")));
                fields.push(inspector_text_field("puzzle3d-play-inspector.object.kind", term_labels.kind, semio_framework_program::ui_inspector_mixed_text(&kinds), patch_cmd("objectKind")));
                ...
                fields.push(inspector_text_field("puzzle3d-play-inspector.object.mesh-url", term_labels.mesh_url, semio_framework_program::ui_inspector_mixed_text(&mesh_urls), patch_cmd("meshUrl")));
        ...
        if !selection.vortex_ids.is_empty() {
            ...
            if !vortices.is_empty() {
                ...
                fields.push(inspector_text_field("puzzle3d-play-inspector.vortex.kind", term_labels.vortex_kind, semio_framework_program::ui_inspector_mixed_text(&kinds), patch_cmd("vortexKind")));
        ...
        if !selection.attraction_ids.is_empty() {
            ...
            if !attractions.is_empty() {
                ...
                fields.push(inspector_text_field("puzzle3d-play-inspector.attraction.attracting", term_labels.attracting, semio_framework_program::ui_inspector_mixed_text(&attracting), patch_cmd("attracting")));
                fields.push(inspector_text_field("puzzle3d-play-inspector.attraction.attracted", term_labels.attracted, semio_framework_program::ui_inspector_mixed_text(&attracted), patch_cmd("attracted")));
        ...
        if !selection.reference_ids.is_empty() {
            ...
            if !references.is_empty() {
                ...
                fields.push(inspector_text_field("puzzle3d-play-inspector.reference.url", term_labels.source_url, semio_framework_program::ui_inspector_mixed_text(&urls), patch_cmd("sourceUrl")));
                fields.push(inspector_text_field("puzzle3d-play-inspector.reference.media-kind", term_labels.media_kind, semio_framework_program::ui_inspector_mixed_text(&media_kinds), patch_cmd("mediaKind")));
    ...
    fn build_settings_body(envelope: &Puzzle3dScene, labels: &Puzzle3dLabels) -> UiNode {
        ...
        let selection_mode_field = UiNode::Field(UiFieldNode {
            ...
            child: Box::new(UiNode::Select(semio_framework_program::UiSelectNode {
                id: "puzzle3d-play-settings.selection-mode.input".into(),
                value: runtime.selection_mode_default.clone(),
                items: vec![
                    semio_framework_program::UiSelectItem { value: "default".into(), label: labels.selective.into() },
                    semio_framework_program::UiSelectItem { value: "additive".into(), label: labels.additive.into() },
                    semio_framework_program::UiSelectItem { value: "subtractive".into(), label: labels.subtractive.into() },
                    semio_framework_program::UiSelectItem { value: "invertive".into(), label: labels.invertive.into() },
                ],
                placeholder: None,
    ...
    fn puzzle3d_engagement(envelope: &Puzzle3dScene, labels: &Puzzle3dLabels) -> WindowEngagement {
        ...
        WindowEngagement {
            ...
            control: None,
            controls: None,
            status: Some(vec![semio_framework_program::WindowEngagementStatus { id: "puzzle3d-world-status".into(), text: format!("{object_count} {objects_label} · {attraction_count} {attractions_label}") }]),
            possible_engagements: None,
        }
    }
...
on_change: puzzle3d_action("setVortexKindWeight", Some(json!({ "kindId": vortex_kind_id, "objectKindId": object_kind_id }))),
    ...
    impl Puzzle3dPlayApp {
        ...
        /// and unreachable from this WASI-P2 sandboxed program crate, and `store::BackboneMessage` has no
    ...
    fn puzzle3d_app_labels_overlay(view_state: &ViewState) -> semio_framework_program::AppLabelsOverlay {
        let labels = puzzle3d_labels(view_state);
        let is_de = is_de_locale(view_state);
    ...
    fn puzzle3d_mesh_from_document(doc: &serde_json::Value) -> Result<semio_framework_program::MeshData, String> {
        ...
        let fallback = mesh_from_kind(PUZZLE3D_FALLBACK_MESH_KIND);
        let mut merged = semio_framework_program::MeshData::default();
    ...
    fn puzzle3d_document_from_mesh(_mesh: &semio_framework_program::MeshData) -> Result<serde_json::Value, String> {
    ...
    pub fn register_puzzle3d_exports() {
        ...
        // without depending on this crate's concrete `Projection`/`Operation` types.
        semio_framework_program::program_runtime::register_document_codec_for_app::<Puzzle3dPlayApp>(PUZZLE3D_FIXTURE_SCHEMA);
        semio_framework_os::register_mesh_exporter("3d.puzzle", "puzzle", puzzle3d_mesh_from_document, Box::new(semio_framework_program::ObjExporter));
        semio_framework_os::register_mesh_exporter("3d.puzzle", "puzzle", puzzle3d_mesh_from_document, Box::new(semio_framework_program::GlbExporter));
        semio_framework_os::register_mesh_exporter("3d.puzzle", "puzzle", puzzle3d_mesh_from_document, Box::new(semio_framework_program::StlExporter));
        semio_framework_os::register_mesh_importer("3d.puzzle", puzzle3d_document_from_mesh, Box::new(semio_framework_program::ObjImporter));
        semio_framework_os::register_mesh_importer("3d.puzzle", puzzle3d_document_from_mesh, Box::new(semio_framework_program::GlbImporter));
        semio_framework_os::register_mesh_importer("3d.puzzle", puzzle3d_document_from_mesh, Box::new(semio_framework_program::StlImporter));
...
pub mod d5 {
    //! 👯 Puzzle 5D program — paired 2D board + 3D world puzzle play app bundled as a hot-swappable WASM program.
    ...
    impl Puzzle5dPlayApp {
        fn drive_precompute(&mut self, envelope: &Puzzle5dScene) {
            ...
            if !self.precompute.has_mesh(PUZZLE5D_FALLBACK_MESH_KIND) {
                let fallback = semio_framework_program::mesh_from_kind(PUZZLE5D_FALLBACK_MESH_KIND);
    ...
    fn puzzle5d_document_from_mesh(_mesh: &semio_framework_program::MeshData) -> Result<Value, String> {
    ...
    pub fn register_puzzle5d_exports() {
        ...
        // without depending on this crate's concrete `Projection`/`Operation` types.
        semio_framework_program::program_runtime::register_document_codec_for_app::<Puzzle5dPlayApp>(PUZZLE5D_SCHEMA);
        register_mesh_exporter("5d.puzzle", "puzzle5d", |_| Ok(semio_framework_program::mesh_from_kind("box")), Box::new(semio_framework_program::ObjExporter));
        register_mesh_exporter("5d.puzzle", "puzzle5d", |_| Ok(semio_framework_program::mesh_from_kind("box")), Box::new(semio_framework_program::GlbExporter));
        register_mesh_exporter("5d.puzzle", "puzzle5d", |_| Ok(semio_framework_program::mesh_from_kind("box")), Box::new(semio_framework_program::StlExporter));
        register_mesh_importer("5d.puzzle", puzzle5d_document_from_mesh, Box::new(semio_framework_program::ObjImporter));
        register_mesh_importer("5d.puzzle", puzzle5d_document_from_mesh, Box::new(semio_framework_program::GlbImporter));
        register_mesh_importer("5d.puzzle", puzzle5d_document_from_mesh, Box::new(semio_framework_program::StlImporter));
        semio_framework_os::register_mesh_dwg_export_handler("5d.puzzle", "puzzle5d", |_| Ok(semio_framework_program::mesh_from_kind("box")));
...
use semio_framework_program::{install_program_bundle, ProgramBundle};
