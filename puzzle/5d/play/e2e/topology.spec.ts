...
test("topology play exposes paired FiveD surfaces with shared connect state", async ({ page }) => {
  await page.goto("/");
  await expect(page.locator('[data-five-d-mode="2d"][data-five-d-instance="play-board"]')).toBeVisible({ timeout: 120_000 });
  await expect(page.locator('[data-five-d-mode="3d"][data-five-d-instance="play-volume"]')).toBeVisible({ timeout: 120_000 });
  await expect(page.locator('[data-five-d-mode="2d"][data-five-d-indirect-active="false"]')).toBeVisible();
  await expect(page.locator('[data-five-d-mode="3d"][data-five-d-indirect-active="false"]')).toBeVisible();
});
