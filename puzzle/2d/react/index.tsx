...
/** @emoji 📄 Parsed `puzzle.2d.fixture` JSON for declarative puzzle 2d scenes. */
...
import { PlaygroundView, usePlayController } from "@semio-tech/framework-playground-renderer-react";
import {
...
import { useShellWindowInstance } from "@semio-tech/framework-platform-renderer-react";
import { Icon } from "@semio-tech/ui-react";
import {
...
/** @emoji 🌳 Builds hierarchy panel tree from live play shell state for declarative side tabs. */
function puzzle2dPlayBuildHierarchyPanelTree(bus: CommandBus): UiTreeNode {
  ...
  if (!shell || !selection) {
    ...
    return { type: "tree", sections: [{ id: loadingId, label: FRAMEWORK_PANEL_TAB_HIERARCHY_LABEL, items: [{ id: "loading", label: "…" }] }] };
  }
...
/** @emoji 🏷️ Builds kinds panel tree from live play shell state for declarative side tabs. */
function puzzle2dPlayBuildKindsPanelTree(bus: CommandBus): UiTreeNode {
  void bus;
  const shell = puzzle2dPlayShellRef.current;
  if (!shell) {
    const loadingId = PUZZLE_2D_PLAY_IS_WIRES ? "wires-play-kinds.loading" : "puzzle-2d-play-kinds.loading";
    return { type: "tree", sections: [{ id: loadingId, label: FRAMEWORK_PANEL_TAB_CATALOGUE_LABEL, items: [{ id: "loading", label: "…" }] }] };
  }
  return PUZZLE_2D_PLAY_IS_WIRES
...
/** @emoji 🔎 Builds inspector panel tree from live play shell state for declarative side tabs. */
function puzzle2dPlayBuildInspectorPanelTree(bus: CommandBus): UiTreeNode {
  void bus;
  const shell = puzzle2dPlayShellRef.current;
  const selection = puzzle2dPlaySelectionRef.current;
  if (!shell || !selection) {
    return uiDeclarativeSectionsToTree([
      { type: "section", id: "puzzle-2d-play-inspector.loading", label: "Detail", children: [{ type: "text", value: "…" }] },
    ]);
  }
  return buildPuzzle2dPlayInspectorTree(shell.fixture, selection.selectionIds);
}
...
function Puzzle2dPlayInner({
  ...
  const puzzle2dShellController = usePlayController<Puzzle2dPlayShellController>(puzzle2dRuntime);
  ...
  reactHostPort.useEffect(() => {
    ...
    const bridge: Puzzle2dPlayHostBridge = {
      ...
      buildHierarchyPanelTree: (bus) => puzzle2dPlayBuildHierarchyPanelTree(bus),
      buildKindsPanelTree: (bus) => puzzle2dPlayBuildKindsPanelTree(bus),
      buildInspectorPanelTree: (bus) => puzzle2dPlayBuildInspectorPanelTree(bus),
      buildSettingsPanelTree: () => {
        const shell = puzzle2dPlayShellRef.current;
        if (!shell) {
          return uiDeclarativeSectionsToTree([
        ...
        <Puzzle2dPlayCanvasSelectionContext.Provider value={canvasSelectionValue}>
          <Puzzle2dPlayCamerasContext.Provider value={camerasValue}>
            <Puzzle2dPlayLodRuntimeContext.Provider value={setPuzzle2dEffectiveLodForPane}>
              <PlaygroundView runtime={puzzle2dRuntime} defaultAppId={PUZZLE_2D_PLAY_APP_ID} playgroundKeybindings={playgroundKeybindings} onActiveWindowChange={onPuzzle2dPlayActiveWindowChange} exampleContribution={exampleContribution} />
...
export const puzzle2dAppRenderer: AppRendererContribution = {
  ...
  sidePanelBodies: puzzle2dPlaySidePanelBodies,
  surfaceHosts: {
