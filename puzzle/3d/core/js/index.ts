// #region 🧲Header
/** @emoji 🧩 Puzzle 3D play app — controller, tools, panels, and window builders. */
// #endregion 🧲Header
...
export const PUZZLE_3D_PLAY_WINDOW_KIND_JACK = "puzzle-3d-jack";
export const PUZZLE_3D_PLAY_WINDOW_LABEL = "Puzzle 3D";
export const PUZZLE_3D_PLAY_BODY_KEY = "puzzle.3d.play.window";
export const PUZZLE_3D_PLAY_BODY_KEY_JACK = "puzzle.3d.play.jack";
export const PUZZLE_3D_PLAY_SURFACE_ID_JACK = "puzzle.3d.play.jack";
export const PUZZLE_3D_PLAY_DEFAULT_JACK_QUERY = "MATCH (n:Object) RETURN n.name";
...
export const PUZZLE_3D_PLAY_VIEWPORT_SURFACE_ID = "puzzle.3d.play.viewport";
...
export class Puzzle3dPlayShellController extends Controller implements PlaygroundExampleHost {
  ...
  private readonly docStore = new DocumentVcsStore<Fixture | null, Puzzle3dFixtureEditOp>({
    envelope: createDocumentVcsEnvelope("puzzle.3d.fixture", "puzzle3d-play", null),
  ...
  setHoverFocus(payload: Puzzle3dHoverPayload): void {
    ...
    const graphId =
      next.hoverTarget?.kind === "object"
        ? next.hoverTarget.objectId
        : next.hoverTarget?.kind === "vortex"
          ? next.hoverTarget.vortexFullId
          : null;
    this.jackBridge.setGraphHover(graphId);
    this.notifyJackSnapshot();
    this.rebuildSnapshotCache();
  ...
  private syncJackFixtureJson(): void {
    ...
    this.jackBridge.setFixtureJson(puzzle3dFixtureToJackBoardJson(this.fixture));
  }

  private notifyJackSnapshot(): void {
    for (const listener of this.jackSnapshotListeners) {
      listener();
    }
  }

  subscribeJackSnapshot(listener: () => void): () => void {
    this.jackSnapshotListeners.add(listener);
    const unsubJack = this.jackBridge.subscribe(listener);
    return () => {
      this.jackSnapshotListeners.delete(listener);
      unsubJack();
    };
  }
  ...
  getWriterDocumentJack(): WriterDocument {
    return createWriterDocument({ id: "puzzle-3d-jack", languageId: "jack", text: this.jackBridge.getJackQueryText() });
  }
  ...
  private syncJackGraphSelect(): void {
    const ids = [
      ...
      ...this.selection.vortexIds,
      ...this.selection.attractionIds,
      ...(this.selection.referenceIds ?? []),
      ...(this.selection.targetVolumeIds ?? []),
    ];
  ...
  private rebuildShellMode(): void {
    this.mainMode.windowKinds = [
      ...
      new WindowKindRuntime(PUZZLE_3D_PLAY_WINDOW_KIND_JACK, "Jack", PUZZLE_3D_PLAY_BODY_KEY_JACK, undefined, undefined, createJackPlayWindowEngagement(PUZZLE_3D_PLAY_WINDOW_KIND_JACK, PUZZLE_3D_PLAY_CONTROLLER_ID, this.jackEngagementInput)),
    ];
  ...
  override run(command: string, args?: unknown): void {
    ...
    if (command === "runJackQuery") {
      if (this.fixture) {
        runJackOnPuzzle3dFixture(puzzle3dFixtureToJackBoardJson(this.fixture), this.jackBridge.getJackQueryText());
      }
      this.notifyJackSnapshot();
      this.emit();
      return;
    ...
    switch (command) {
      ...
      case "patchPuzzle3dVortices": {
        const { vortexFullIds, field, value } = args as {
          vortexFullIds: readonly string[];
          field: "label" | "vortexKind" | "position" | "direction" | "radius";
          value?: unknown;
        };
      ...
      case "patchPuzzle3dAttractions": {
        const { attractionIds, field, value } = args as {
...
export function buildPuzzle3dPlayAppRuntime(controller: Puzzle3dPlayShellController): AppRuntime {
  const app = new AppRuntime(
    ...
    undefined,
    controller,
    createStackLayout([PUZZLE_3D_PLAY_WINDOW_ID, PUZZLE_3D_PLAY_WINDOW_KIND_JACK], [PUZZLE_3D_PLAY_WINDOW_LABEL, "Jack"]) as never,
    [
...
function buildPuzzle3dPlayJackDeclarativeBody(_ctx: WindowBodyViewContext): UiNode {
  return buildWriterWindowBody(PUZZLE_3D_PLAY_SURFACE_ID_JACK, PUZZLE_3D_PLAY_CONTROLLER_ID, PUZZLE_3D_PLAY_WINDOW_KIND_JACK);
}
...
export function buildPuzzle3dPlayInspectorBody(ctx: WindowBodyViewContext): UiTreeNode {
  ...
  if (selection.objectIds.length > 0) {
    ...
    const kindUniform = uiInspectorAllEqual(kinds);
    const origins = objects.map((object) => object.origin);
    const originUniform = uiInspectorAllEqual(origins);
  ...
  if (selectedVortexRows.length === 1) {
    ...
    const vortexFullIds = selectedVortexRows.map((row) => row.fullId);
    ...
    const labelUniform = uiInspectorAllEqual(labels);
    const kindUniform = uiInspectorAllEqual(kinds);
    const positionUniform = uiInspectorMixedVec3(positions);
      ...
      children: [
        {
          ...
          child: {
            type: "input",
            id: "puzzle-3d-play-inspector.vortices.label.input",
            inputKind: "text",
            value: labelUniform ? (labels[0] ?? "") : "",
            placeholder: labelUniform ? undefined : "Mixed",
            onChange: puzzle3dPlayCmd("patchPuzzle3dVortices", { vortexFullIds, field: "label" }),
          },
        },
        {
          type: "field",
          ...
          child: {
            type: "select",
            id: "puzzle-3d-play-inspector.vortices.kind.select",
            value: kindUniform ? (kinds[0] ?? "") : "",
            placeholder: kindUniform ? "kind" : "Mixed",
            items: puzzle3dPlayKindCatalogSelectItems(vortexKinds),
            onChange: puzzle3dPlayCmd("patchPuzzle3dVortices", { vortexFullIds, field: "vortexKind" }),
          },
        },
        {
          type: "field",
          ...
          child: {
            type: "vec3",
            id: "puzzle-3d-play-inspector.vortices.position.vec3",
            value: positionUniform,
            onChange: puzzle3dPlayCmd("patchPuzzle3dVortices", { vortexFullIds, field: "position" }),
          },
        },
        {
          type: "field",
          ...
          child: {
            type: "input",
            id: "puzzle-3d-play-inspector.vortices.radius.input",
            inputKind: "number",
            value: radiusUniform ? String(radii[0]) : "",
            placeholder: radiusUniform ? undefined : "Mixed",
            onChange: puzzle3dPlayCmd("patchPuzzle3dVortices", { vortexFullIds, field: "radius" }),
          },
        },
      ],
    });
  ...
  if (selectedAttractions.length === 1) {
    ...
    const attractionIds = selection.attractionIds;
    ...
    const kindUniform = uiInspectorAllEqual(kinds);
    const attractingUniform = uiInspectorAllEqual(attracting);
    const attractedUniform = uiInspectorAllEqual(attracted);
    children.push({
      type: "section",
      ...
      children: [
        {
          ...
          child: {
            type: "select",
            id: "puzzle-3d-play-inspector.attractions.kind.select",
            value: kindUniform ? (kinds[0] ?? "") : "",
            placeholder: kindUniform ? "kind" : "Mixed",
            items: puzzle3dPlayKindCatalogSelectItems(attractionKinds),
            onChange: puzzle3dPlayCmd("patchPuzzle3dAttractions", { attractionIds, field: "attractionKind" }),
          },
        },
        {
          type: "field",
          ...
          child: {
            type: "input",
            id: "puzzle-3d-play-inspector.attractions.attracting.input",
            inputKind: "text",
            value: attractingUniform ? attracting[0]! : "",
            placeholder: attractingUniform ? undefined : "Mixed",
            onChange: puzzle3dPlayCmd("patchPuzzle3dAttractions", { attractionIds, field: "attracting" }),
          },
        },
        {
          type: "field",
          ...
          child: {
            type: "input",
            id: "puzzle-3d-play-inspector.attractions.attracted.input",
            inputKind: "text",
            value: attractedUniform ? attracted[0]! : "",
            placeholder: attractedUniform ? undefined : "Mixed",
            onChange: puzzle3dPlayCmd("patchPuzzle3dAttractions", { attractionIds, field: "attracted" }),
          },
        },
      ],
    });
  ...
  if (selectedReferences.length === 1) {
    ...
    const widthUniform = uiInspectorAllEqual(widthWorlds);
    const opacityUniform = uiInspectorAllEqual(opacities);
    children.push({
      type: "section",
...
if (import.meta.vitest) {
  ...
  describe("puzzle 3D play fixture", () => {
    ...
    it("setSelection drops hidden references", () => {
      const bus = new CommandBus();
      const wb = new Platform();
      const ctrl = new Puzzle3dPlayShellController(bus, () => wb.notify());
      const fixture = parseFixture({
      ...
      expect(ctrl.getSnapshot().selection.referenceIds).toEqual([]);
    });
    ...
    it("buildPuzzle3dPlayInspectorBody aggregates multi vortex and attraction selection", () => {
      ...
      const vortexSection = tree.sections.find((section) => section.id === "puzzle-3d-play-inspector.vortices");
      const labelField = vortexSection?.items.find((item) => item.id === "puzzle-3d-play-inspector.vortices.label");
      expect(labelField?.control?.type).toBe("input");
      expect(labelField?.control?.onChange?.command).toBe("patchPuzzle3dVortices");
      const attractionSection = tree.sections.find((section) => section.id === "puzzle-3d-play-inspector.attractions");
      const kindField = attractionSection?.items.find((item) => item.id === "puzzle-3d-play-inspector.attractions.kind");
      expect(kindField?.control?.type).toBe("select");
      expect(kindField?.control?.onChange?.command).toBe("patchPuzzle3dAttractions");
    });

    it("patchPuzzle3dVortices updates labels in batch", () => {
      const bus = new CommandBus();
      const wb = new Platform();
      const ctrl = new Puzzle3dPlayShellController(bus, () => wb.notify());
      const fixture = parseFixture({
