...
import { useApp, engagementSpecControlMirror, controllerBackedExampleContribution } from "@semio-tech/framework-playground-renderer-react";
import {
  CommandBus,
...
import { useControllerStore, useShellWindowInstance } from "@semio-tech/framework-platform-renderer-react";
import type { Puzzle3dPlayShellController, Puzzle3dPlayHostBridge, Puzzle3dPlaySnapshot } from "@semio-tech/puzzle-3d-core";
import { createWriterDocument } from "@semio-tech/writer-core";
import { WriterCanvas } from "@semio-tech/writer-react";

let p3d!: typeof import("@semio-tech/puzzle-3d-core");
...
function Puzzle3dPlayEngagementPublisher(props: {
  ...
  const kindCatalogs = reactHostPort.useMemo(() => p3d.parseKindCatalogs(snap.fixture.meta), [snap.fixture.meta]);
  ...
  const fillSessionReadyEpoch = reactHostPort.useSyncExternalStore(
    p3d.subscribePuzzle3dFillSessionReady,
    p3d.getPuzzle3dFillSessionReadyEpoch,
    p3d.getPuzzle3dFillSessionReadyEpoch,
  );
  ...
  const onSelectTool = reactHostPort.useCallback(() => {
    if (snap.activeTool === "fill") {
      const base = p3d.clearPuzzle3dFillSession();
      if (base && ctrl) {
  ...
  const onFillCount = reactHostPort.useCallback(
    (count: number) => {
      ...
      p3d.puzzle3dFillPendingCountRef.current = n;
      setFillCount(n);
      bus.dispatch(PUZZLE_3D_PLAY_CONTROLLER_ID, "setFillCount", { count: n });
      if (n < prev) {
        const catalogs = p3d.parseKindCatalogs(snap.fixture.meta);
        const compatibility = p3d.parseKindCompatibility(snap.fixture.meta);
        p3d.rerollPuzzle3dFillTail(n, catalogs, compatibility, snap.brushPlacementOverlapBudget);
      }
    },
  ...
  reactHostPort.useEffect(() => {
    ...
    const sequenceLength = p3d.puzzle3dFillSessionRef.current.sequence.length;
    if (sequenceLength === 0) {
      return;
    }
    fillAutoStartedRef.current = true;
    const pending = p3d.puzzle3dFillPendingCountRef.current;
    const nextCount = pending > 0 ? Math.min(pending, sequenceLength) : 1;
    p3d.puzzle3dFillPendingCountRef.current = nextCount;
...
const Puzzle3dPlayViewportHost = reactHostPort.memo(function Puzzle3dPlayViewportHost({ node }: { readonly node: UiPuzzle3dHostSurfaceNode }): React.ReactElement {
  ...
  const kindCompatibility = reactHostPort.useMemo(() => p3d.parseKindCompatibility(snap.fixture.meta), [snap.fixture]);
  const kindCatalogs = reactHostPort.useMemo(() => p3d.parseKindCatalogs(snap.fixture.meta), [snap.fixture]);
  reactHostPort.useEffect(() => {
    p3d.installPuzzle3dPlayBrushHost(snap.fixture.meta as Record<string, unknown> | undefined);
  }, [snap.fixture.meta]);
  ...
  reactHostPort.useEffect(
    () =>
      p3d.subscribePuzzle3dFillDistributionInvalidated(() => {
        fillSessionPreparedRef.current = false;
  ...
  reactHostPort.useEffect(
    () =>
      p3d.subscribePuzzle3dFillTargetVolumesInvalidated(() => {
        fillSessionPreparedRef.current = false;
  ...
  const onFillMeshesReady = reactHostPort.useCallback(() => {
    ...
    fillPrepareTimerRef.current = setTimeout(() => {
      ...
      if (!fillSessionPreparedRef.current) {
        p3d.preparePuzzle3dFillSession(base, kindCatalogs, kindCompatibility, snap.brushPlacementOverlapBudget, base.targetVolumes ?? []);
...
async function preloadPuzzle3dPlayMeshes(): Promise<void> {
  const fixture = parseFixture(p3d.puzzle3dPlayFixtureJson(playgroundResolvedExampleId(PUZZLE_3D_PLAY_EXAMPLE_CONCRETE_FOREST_ID)) as unknown);
  if (!fixture) return;
  const catalogs = p3d.parseKindCatalogs(fixture.meta as Record<string, unknown> | undefined);
  const compatibility = p3d.parseKindCompatibility(fixture.meta as Record<string, unknown> | undefined);
...
export async function puzzle3dAppRenderer(): Promise<AppRendererContribution> {
  p3d = await import("@semio-tech/puzzle-3d-core");
  return {
    windowBodies: p3d.puzzle3dPlayWindowBodies,
    sidePanelBodies: p3d.puzzle3dPlaySidePanelBodies,
    surfaceHosts: {
      [p3d.PUZZLE_3D_PLAY_VIEWPORT_SURFACE_ID]: Puzzle3dPlayViewportHost,
      [p3d.PUZZLE_3D_PLAY_SURFACE_ID_JACK]: Puzzle3dPlayJackSurfaceHost,
    },
    tabIcons: {
      [p3d.PUZZLE_3D_PLAY_ICON_INSPECTOR]: "clipboard-list",
      [p3d.PUZZLE_3D_PLAY_ICON_KINDS]: "tags",
      [p3d.PUZZLE_3D_PLAY_ICON_HIERARCHY]: "list-tree",
      [p3d.PUZZLE_3D_PLAY_ICON_SETTINGS]: "settings",
    },
    preload: preloadPuzzle3dPlayMeshes,
    examples: controllerBackedExampleContribution(p3d.PUZZLE_3D_PLAY_CONTROLLER_ID, p3d.PUZZLE_3D_PLAY_EXAMPLE_OPTIONS),
