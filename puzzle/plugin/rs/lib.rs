...
pub mod d2 {
    ...
    /// so it surfaces in the Utility Options rail only while the fill utility is active.
    fn puzzle2d_fill_utility_options(envelope: &Puzzle2dScene, labels: &Puzzle2dLabels) -> WindowMeasure {
        WindowMeasure::Group {
            id: "puzzle2d-utility-options-fill".into(),
    ...
    mod tests {
        ...
        fn utility_registry_declares_utilities() {
            ...
            assert_eq!(ids, vec![PUZZLE2D_UTILITY_SELECT, PUZZLE2D_UTILITY_BRUSH, PUZZLE2D_UTILITY_FILL]);
        ...
        fn fill_and_brush_params_are_tagged_utility_options_not_engagement_controls() {
            ...
            // 🪣 Fill-count slider now lives in a general measure tagged for the fill utility (surfaced by
            ...
            let fill_scene = Puzzle2dScene { fixture: default_empty_fixture(), runtime: fill_runtime, active_utility: PUZZLE2D_UTILITY_FILL.into() };
            let fill_measures = puzzle2d_window_measures(PUZZLE2D_PANE_OVERVIEW, &fill_scene, labels);
            assert_eq!(group_tag(&fill_measures, "puzzle2d-utility-options-fill"), Some(Some(PUZZLE2D_UTILITY_FILL.into())));
...
pub mod d3 {
    ...
    fn build_inspector_tree(envelope: &Puzzle3dScene, term_labels: &Puzzle3dLabels) -> UiNode {
        ...
        if !selection.object_ids.is_empty() {
            ...
            if !objects.is_empty() {
                ...
                fields.push(inspector_text_field("puzzle3d-play-inspector.object.kind", "Kind", semio_framework_plugin::ui_inspector_mixed_text(&kinds), patch_cmd("objectKind")));
                fields.push(ui_inspector_vec3_group("puzzle3d-play-inspector.object.origin", "Origin", &origins, 0.1, |axis| patch_cmd(&format!("origin.{axis}"))));
                fields.push(inspector_quat_group("puzzle3d-play-inspector.object.orientation", "Orientation", &orientations, 0.01, |axis| patch_cmd(&format!("orientation.{axis}"))));
                fields.push(ui_inspector_vec3_group("puzzle3d-play-inspector.object.scale", "Scale", &scales, 0.1, |axis| patch_cmd(&format!("scale.{axis}"))));
        ...
        if !selection.vortex_ids.is_empty() {
            ...
            if !vortices.is_empty() {
                ...
                fields.push(ui_inspector_vec3_group("puzzle3d-play-inspector.vortex.position", "Position", &positions, 0.1, |axis| patch_cmd(&format!("position.{axis}"))));
                fields.push(ui_inspector_vec3_group("puzzle3d-play-inspector.vortex.direction", "Direction", &directions, 0.1, |axis| patch_cmd(&format!("direction.{axis}"))));
        ...
        if !selection.attraction_ids.is_empty() {
            ...
            if !attractions.is_empty() {
                ...
                fields.push(ui_inspector_stepper_field("puzzle3d-play-inspector.attraction.gap", "Gap", &gaps, 0.1, patch_cmd("gap")));
                fields.push(ui_inspector_stepper_field("puzzle3d-play-inspector.attraction.shift", "Shift", &shifts, 0.1, patch_cmd("shift")));
                fields.push(ui_inspector_stepper_field("puzzle3d-play-inspector.attraction.rise", "Rise", &rises, 0.1, patch_cmd("rise")));
        ...
        if !selection.reference_ids.is_empty() {
            ...
            if !references.is_empty() {
                ...
                fields.push(inspector_text_field("puzzle3d-play-inspector.reference.media-kind", "Media Kind", semio_framework_plugin::ui_inspector_mixed_text(&media_kinds), patch_cmd("mediaKind")));
                fields.push(ui_inspector_vec3_group("puzzle3d-play-inspector.reference.origin", "Origin", &origins, 0.1, |axis| patch_cmd(&format!("origin.{axis}"))));
        ...
        if !selection.target_volume_ids.is_empty() {
            ...
            if !volumes.is_empty() {
                ...
                fields.push(ui_inspector_vec3_group("puzzle3d-play-inspector.target-volume.origin", "Origin", &origins, 0.1, |axis| patch_cmd(&format!("origin.{axis}"))));
                fields.push(inspector_quat_group("puzzle3d-play-inspector.target-volume.orientation", "Orientation", &orientations, 0.01, |axis| patch_cmd(&format!("orientation.{axis}"))));
    ...
    fn build_settings_body(envelope: &Puzzle3dScene) -> UiNode {
        ...
        ui_inspector_groups_to_tree(&[UiInspectorFieldGroup {
            ...
            fields: vec![
                ...
                ui_inspector_stepper_field("puzzle3d-play-settings.proximity-radius", "Proximity Radius", &[runtime.proximity_radius], 0.1, puzzle3d_action("setProximityRadius", None)),
                ui_inspector_stepper_field("puzzle3d-play-settings.chunk-size", "Chunk Size", &[runtime.chunk_size], 1.0, puzzle3d_action("setChunkSize", None)),
                ui_inspector_stepper_field("puzzle3d-play-settings.grid-spacing", "Grid Spacing", &[runtime.grid_spacing], 0.5, puzzle3d_action("setGridSpacing", None)),
            ],
        }])
    }
    //#endregion 🔖Panels
    ...
    /// 🪣 Utility Options group for the Fill utility — the fill-count slider (hidden while editing target volumes,
    ...
    fn puzzle3d_fill_utility_options(envelope: &Puzzle3dScene, precompute: &Puzzle3dPrecomputeSession, labels: &Puzzle3dLabels) -> WindowMeasure {
        ...
        WindowMeasure::Group { id: format!("{PUZZLE3D_PLAY_CONTROLLER_ID}-utility-options-fill"), label: labels.fill.into(), default_open: Some(true), active_utility_id: Some("fill".into()), children }
    }

    /// 🧊 Utility Options group for the Fill utility's voxel-edit mode — the width/depth/height dimension sliders,
    ...
    fn puzzle3d_voxel_utility_options(runtime: &Puzzle3dRuntime, labels: &Puzzle3dLabels) -> Option<WindowMeasure> {
    ...
    impl DocumentApp for Puzzle3dPlayApp {
        ...
        fn handle_action(&mut self, action: &str, args: Option<&Value>, doc: &DocumentView<'_, Value>, view_state: &ViewState) -> ActionEmit<Puzzle3dOp> {
            ...
            match action {
                ...
                SET_ACTIVE_UTILITY_ACTION_ID => {
                    // 🧰 Host already applied `view_state.active_utility_id`; clear in-progress scratch and refresh
                    // the placement engine for the new utility. Emits no ops (View-kind) and no utility-switch effect.
    ...
    fn puzzle3d_action_labels(is_de: bool) -> std::collections::HashMap<String, String> {
        const ENTRIES: &[(&str, &str, &str)] = &[
            ...
            ("setGridVisible", "Set Grid Visible", "Raster anzeigen"),
    ...
    mod tests {
        ...
        /// `partition_window_measures` only for their active utility), never [`WindowEngagementControl`]s on the HUD.
        #[test]
        fn fill_and_brush_params_are_tagged_utility_options_not_engagement_controls() {
            let labels = puzzle3d_labels(&ViewState::default());
            let session = Puzzle3dPrecomputeSession::new();
            // 🪣 Fill utility: the fill Utility Options group is tagged for "fill"; the voxel-dimension group only
            ...
            assert_eq!(measure_group_tag(&fill_measures, "puzzle3d-play-utility-options-fill"), Some(Some("fill".into())));
            assert_eq!(measure_group_tag(&fill_measures, "puzzle3d-play-utility-options-voxel"), None, "voxel-dimension group is absent until edit-volumes mode is on");
            assert!(find_measure_slider(&fill_measures, "puzzle3d-fill-count").is_some(), "fill-count slider lives in the fill Utility Options while not editing volumes");
            ...
            assert_eq!(measure_group_tag(&edit_measures, "puzzle3d-play-utility-options-voxel"), Some(Some("fill".into())), "voxel-dimension steppers surface (tagged for fill) in edit-volumes mode");
