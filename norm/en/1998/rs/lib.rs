...
pub mod part_1 {
    ...
    /// 🌍 EN 1998-1 elastic response spectrum shape per §3.2.2.2: Type 1 (M_s ≥ 5.5) vs Type 2 (M_s < 5.5).
    #[derive(Clone, Copy, Debug, PartialEq, Eq)]
    ...
    /// 🪨 Generic EN 1998-1 ground type per Table 3.1, independent of any national annex table.
    #[derive(Clone, Copy, Debug, PartialEq, Eq)]
    ...
    impl EnGroundType {
        /// 📊 (S, T_B, T_C, T_D) per EN 1998-1 Table 3.2 (Type 1) / Table 3.3 (Type 2).
    ...
    /// 📈 Elastic response spectrum Type 1/2 shape horizontal [g] per EN 1998-1 §3.2.2.2, given resolved (a_g, S, T_B, T_C, T_D).
...
// #region 🔖AnnexParams
/// 🇪🇺 Resolved seismic-action model per NDP: DE zone table vs EN Type-1/2 spectrum (EN 1998-1 §3.2 NDP).
...
impl AnnexParams {
    ...
    /// 📐 Resolved (a_g, S, T_B, T_C, T_D) feeding `part_1::elastic_response_spectrum_type1`.
    ...
    /// 📈 Elastic response spectrum S_e(T) [g] resolved for this annex selection.
...
pub mod part_3 {
    ...
    /// 🔍 Knowledge level per EN 1998-3 §3.4, driving the confidence factor CF.
    #[derive(Clone, Copy, Debug, PartialEq, Eq)]
    ...
    impl KnowledgeLevel {
        /// 🎯 Confidence factor CF per EN 1998-3 Table 3.1.
    ...
    impl RetrofitLimitState {
        /// 📑 EN 1998-3 §2.3.4 clause subsection per limit state.
...
pub mod part_4 {
    ...
    /// 🏺 Impulsive period T_i [s] for circular silo/tank per EN 1998-4 Annex.
    pub fn impulsive_period_s(height_m: f64, radius_m: f64) -> f64 {
        0.1 * (height_m / radius_m).sqrt()
    }

    /// 🏺 Convective (sloshing) period T_c [s] for circular silo/tank.
    ...
    /// 🏺 Combined silo base shear via SRSS of impulsive and convective components [kN].
    ...
    /// 🏺 Behaviour factor q capped at 1.5 for silos per EN 1998-4 Table 2.1.
...
pub mod part_5 {
    ...
    /// 🧱 Foundation stiffness ratio r = K_f / K_s per EN 1998-5 §7.
    ...
    /// 🧱 Bearing capacity reduction factor under seismic loading per EN 1998-5 §7.
    pub fn bearing_reduction_factor(a_g: f64) -> f64 {
        (1.0 - 1.5 * a_g).max(0.5)
    }

    /// 🧱 Seismic bearing pressure [kPa].
    ...
    /// 🌍 Horizontal seismic coefficient k_h = α·S/r per EN 1998-5 §7.3.2.2 (r: wall-displacement class).
    pub fn horizontal_seismic_coefficient(alpha: f64, s: f64, r: f64) -> f64 {
        alpha * s / r
    }

    /// 🧱 Mononobe-Okabe dynamic active earth-pressure coefficient K_AE per EN 1998-5 Annex E (vertical wall, horizontal backfill, no wall friction). Reduces to the classic Rankine K_a at k_h = 0.
    ...
    /// 🧱 Dynamic active thrust increment on a retaining wall [kN/m] from K_AE.
...
pub mod part_6 {
    ...
    /// 🗼 Along-wind base overturning moment [kNm] per EN 1998-6 §4 (wind-induced dynamic response of slender towers).
    ...
    /// 🗼 First-mode natural frequency [Hz] for a cantilever tower.
    ...
    /// 🗼 Behaviour factor q capped per EN 1998-6 Table 4.1: 1.5 for chimneys, 2.0 for other towers/masts.
    pub fn tower_behaviour_factor(q_nominal: f64, is_chimney: bool) -> f64 {
        let cap = if is_chimney { 1.5 } else { 2.0 };
        q_nominal.min(cap)
    }

    /// 🗼 First-mode participation factor Γ for a uniform cantilever with mode shape φ(x) = 1 − cos(πx/2H) per EN 1998-6 Annex B (simplified modal analysis).
    ...
    /// 🗼 Modal base shear V_b1 = Γ · S_d(T1) · m · g [kN] for the cantilever first mode.
...
/// 📋 Building seismic check generalized over DE zone-based or EN Type-1/2-spectrum annex selection.
...
// type definition above (see `#[dsl(extension = "en1998", layout = "lines")]`) — the engine's
// `dsl_schema` grammar replaces this crate's own hand-rolled `norm_core::dsl_kv` printer/parser.
// #endregion 🔖Dsl
