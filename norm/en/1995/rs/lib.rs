...
// #region 🔖AnnexParams
/// 🪵 Timber material category for γ_M selection per EN 1995-1-1 Table 2.3.
...
/// 🇪🇺 National-annex NDPs for EN 1995: γ_M per material (unchanged by the German NA) and the shear-effective-width factor k_cr, which genuinely diverges between EN and DIN EN 1995-1-1/NA.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
impl AnnexParams {
    ...
    /// 📖 γ_M per material per EN 1995-1-1 Table 2.3 — EN-recommended, not amended by DIN EN 1995-1-1/NA.
    ...
    /// 🇩🇪 k_cr shear-effective-width factor: EN 1995-1-1 §6.1.7(2) recommends the constant 0.67; DIN EN 1995-1-1/NA instead specifies min(1.0, 2.5/f_v,k) with f_v,k in MPa.
...
pub mod part_1_1 {
    ...
    /// 📐 Shear stress τ [MPa] per EN 1995-1-1 §6.1.7 Eq. 6.13a, using the k_cr-reduced effective width b_ef = k_cr·b.
    pub fn shear_stress_mpa(v_ed_kn: f64, b_mm: f64, h_mm: f64, k_cr: f64) -> f64 {
        let b_ef_mm = k_cr * b_mm;
        1.5 * v_ed_kn * 1000.0 / (b_ef_mm * h_mm)
    }

    /// 📐 Shear design resistance f_v,d [MPa] per EN 1995-1-1 §2.4.1, Eq. 2.14.
    ...
    /// 📐 Timber shear ULS check per EN 1995-1-1 §6.1.7 — utilization diverges between EN and DE via k_cr.
...
pub mod part_2 {
    ...
    /// 🌉 Pedestrian comfort limit for vertical acceleration [m/s²] per EN 1995-2 §7 (serviceability, footbridges).
    pub const A_VERT_LIMIT_M_S2: f64 = 0.7;

    /// 🌉 Reference cycle count below which no fatigue reduction applies, per EN 1995-2 Annex A style guidance.
    ...
    /// 🌉 Pedestrian-induced vertical vibration serviceability check per EN 1995-2 §7.
    ...
    /// 📐 Simplified fatigue/damage reduction factor k_fat for repeated traffic loading, per EN 1995-2 Annex A style S-N degradation (reference N0 = 1e6 cycles, 10% reduction per decade, floored at 0.5).
    ...
    /// 🌉 Bridge bending check under repeated traffic loading, with the base resistance reduced by k_fat per EN 1995-2 Annex A.
    pub fn check_bridge_fatigue(m_ed_knm: f64, m_rd_knm: f64, n_cycles: f64) -> CheckResult {
        ...
        CheckResult::from_utilization(
            ...
            Quantity::new(norm_core::QuantityKind::Moment, m_rd_fatigue_knm * 1_000_000.0),
...
/// 📋 Glulam beam check with LTB, compression, and k_cr-dependent shear.
...
/// 📋 Full EN 1995 check across bending, compression, shear, connections, fire, and bridge parts.
#[allow(clippy::too_many_arguments)]
...
// #region 🔖Dsl
// `Document`'s `store::DocumentDsl` impl is now generated by `#[derive(dsl::DslDocument)]` on the type
// definition itself (see `Document` above) — the engine's `dsl_schema` grammar replaces this crate's
// own hand-rolled `dsl_kv` printer.
// #endregion 🔖Dsl
...
mod tests {
    ...
    fn full_timber_worked_example() {
        let report = check_full_timber(
            25.0, 50.0, 15.0, 1_800_000.0, 20_000.0, 200.0, 300.0, 24.0, 21.0, 4.0, ServiceClass::Sc1, LoadDuration::Medium, 80.0, 18.0, 12_000.0, 30.0, 300.0, AnnexChoice::De, 0.3, 500_000.0,
        );
    ...
    fn document_text_round_trips_through_store() {
        let envelope = store::create_document_envelope("norm.en1995/v1", "en1995", Document::default(), None);
