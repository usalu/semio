...
//! 📚 Models the classic (pre-2024) Eurocode 7 generation only: EN 1997-1 (general design rules, including piles) + EN 1997-2 (ground investigation and testing); the second-generation EN 1997-3 does not apply here.
...
/// 🇩🇪🇪🇺 Annex-resolved partial factor set for a design approach; DE selects DA2* (combined STR/GEO verification) where EN keeps standard DA2.
...
pub mod part_1 {
    ...
    /// 📐 Sliding resistance [kPa]: (c_d + σ·tan(φ_d)) / γ_R;h.
    ...
    // #region 🔖Piles
    /// 📐 Shaft resistance R_s = α_s · π · D · q_s · L [kN] per EN 1997-1 §7.6.2 (ground-test-derived α_s and q_s from EN 1997-2 investigation).
    pub fn shaft_resistance_kn(alpha_s: f64, d_m: f64, q_s_kpa: f64, l_m: f64) -> f64 {
        alpha_s * std::f64::consts::PI * d_m * q_s_kpa * l_m
    }

    /// 📐 Base resistance R_b = q_b · A_b [kN] per EN 1997-1 §7.6.2.
    pub fn base_resistance_kn(q_b_kpa: f64, a_base_m2: f64) -> f64 {
        q_b_kpa * a_base_m2
    }

    /// 📊 Correlation factors ξ₃ (on mean) / ξ₄ (on min) per EN 1997-1 Annex A Table A.10, by number of test profiles n.
    ...
    /// 📐 Characteristic pile resistance R_k = min(mean(R_cal)/ξ₃, min(R_cal)/ξ₄) per EN 1997-1 §7.6.2.2.
    ...
    /// 📐 Design pile compressive resistance R_c,d = R_b,k/γ_b + R_s,k/γ_s per EN 1997-1 §7.6.2.
    ...
    /// ✅ Pile axial compressive resistance check per EN 1997-1 §7.6.2.
...
pub mod part_2 {
    ...
    /// 📐 Effective friction angle φ′ [deg] from CPT cone resistance q_c and effective overburden σ′_v0, per EN 1997-2 Annex D (Robertson & Campanella 1983).
    ...
    /// 📐 Effective friction angle φ′ [deg] from SPT blow count N, per EN 1997-2 Annex D (Peck–Hanson–Thornburn correlation).
    pub fn phi_from_spt_deg(n_spt: f64) -> f64 {
        27.1 + 0.3 * n_spt - 0.00054 * n_spt * n_spt
    }

    /// 📐 Minimum ground investigation depth [m] per EN 1997-2 §2.4.2: z ≥ 3·b.
    pub fn min_investigation_depth_m(b_m: f64) -> f64 {
        3.0 * b_m
    }

    /// ✅ Ground investigation depth adequacy check per EN 1997-2 §2.4.2.
...
/// 📋 Full EN 1997 check across bearing, sliding, settlement, pile axial (part 1), and ground investigation adequacy (part 2).
...
// type definition itself (see `Document` above) — the engine's `dsl_schema` grammar replaces this
