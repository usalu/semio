...
// #region 🔖Dsl
// `Document`'s `store::DocumentDsl` impl and `FireRating`/`TightnessClass`'s scalar-tag mappings are
...
mod tests {
    ...
    fn document_dsl_parse_error_reports_the_real_line_of_the_bad_field() {
        ...
        let printed = <Document as store::DocumentDsl>::print_dsl(&Document::default());
        ...
        let error = <Document as store::DocumentDsl>::parse_dsl(&bad).expect_err("an unknown fire_rating tag must fail to parse");
