...
// #region 🔖AnnexParams
/// 🇪🇺🇩🇪 Resolved partial safety factors for one national annex choice, threaded through every EN 1993 formula.
...
impl AnnexParams {
    /// 🇪🇺 EN 1993-1-1 §6.1 base recommended values (no DIN EN NA increase on γ_M1).
    pub fn en() -> Self {
        Self { choice: AnnexChoice::En, gamma_m0: 1.0, gamma_m1: 1.0, gamma_m2: 1.25, gamma_mf: 1.15 }
    }

    /// 🇩🇪 DIN EN 1993-1-1/NA: the genuine EN-vs-DE divergence raises γ_M1 to 1.1 for member buckling.
    pub fn de() -> Self {
...
pub mod part_1_1 {
    ...
    /// ⭕ CHS (circular hollow section) class 1–3 per EN 1993-1-1 Table 5.2 (d/t vs 50ε², 70ε², 90ε²).
    ...
    /// 🔗 Net-section tension resistance N_t,Rd [kN] = min(gross yield, net rupture) per EN 1993-1-1 §6.2.3 — rupture MUST use γ_M2, not γ_M0.
...
pub mod part_1_2 {
    ...
    /// 🔥 Critical steel temperature θ_a,cr [°C] per EN 1993-1-2 Eq. 4.22, μ₀ = degree of utilization.
...
pub mod part_1_3 {
    ...
    /// 📏 Cold-formed plate slenderness λ_p = (b̄/t)/(28.4·ε·√k_σ) per EN 1993-1-3 §5.5.
    ...
    /// 📉 Effective-width reduction factor ρ = (λ_p − 0.055·(3+ψ))/λ_p² (capped at 1.0) per EN 1993-1-3 §5.5.2.
    ...
    /// 📐 Effective cross-section resistance [kN] = ρ · gross resistance per EN 1993-1-3 §5.5.
...
pub mod part_1_4 {
    ...
    /// 🪙 Stainless steel γ_M0 = γ_M1 = 1.1 per EN 1993-1-4 §2.2 — a material safety factor fixed in the base EN document, not a DE/EN annex divergence.
    pub const GAMMA_M_STAINLESS: f64 = 1.1;

    /// 📏 Stainless steel material factor ε = √(235/f_y · E/210000) per EN 1993-1-4 Table 2.2.
...
pub mod part_1_6 {
    ...
    /// 🐚 Fabrication quality parameter Q for tolerance quality class B per EN 1993-1-6 Table D.2.
    pub const FABRICATION_QUALITY_Q_CLASS_B: f64 = 25.0;

    /// 🐚 Elastic critical meridional buckling stress σ_x,Rcr [MPa] per EN 1993-1-6 Annex D.1.2.1.
    pub fn sigma_x_rcr_mpa(t_mm: f64, r_mm: f64, e_mpa: f64) -> f64 {
        0.605 * e_mpa * t_mm / r_mm
    }

    /// 📉 Shell meridional slenderness λ̄ = √(f_y/σ_x,Rcr) per EN 1993-1-6 §D.1.2.1.
    pub fn lambda_bar(f_y_mpa: f64, sigma_x_rcr_mpa: f64) -> f64 {
        (f_y_mpa / sigma_x_rcr_mpa).sqrt()
    }

    /// 🐚 Elastic imperfection reduction factor α from fabrication quality class B per EN 1993-1-6 §D.1.2.2.
    ...
    /// 📉 Shell buckling reduction factor χ, λ̄₀=0.2, β=0.6, η=1.0 per EN 1993-1-6 §8.5.2.
    ...
    /// 📐 Shell design buckling resistance σ_x,Rd [MPa] = χ·f_y/γ_M1 per EN 1993-1-6 §8.5.2.
...
pub mod part_1_8 {
    ...
    /// 📐 Bearing coefficient α_b = min(e₁/(3·d₀), f_ub/f_u, 1.0) per EN 1993-1-8 Table 3.4.
    pub fn bearing_alpha_b(e1_mm: f64, d0_mm: f64, f_ub_mpa: f64, f_u_mpa: f64) -> f64 {
        (e1_mm / (3.0 * d0_mm)).min(f_ub_mpa / f_u_mpa).min(1.0)
    }

    /// 📐 Bearing coefficient k₁ = min(2.8·e₂/d₀ − 1.7, 2.5) per EN 1993-1-8 Table 3.4.
    pub fn bearing_k1(e2_mm: f64, d0_mm: f64) -> f64 {
        (2.8 * e2_mm / d0_mm - 1.7).min(2.5)
    }

    /// 🔩 Bolt bearing resistance F_b,Rd [kN] = k₁·α_b·f_u·d·t/γ_M2 per EN 1993-1-8 Table 3.4.
    ...
    /// 🪛 Correlation factor β_w by steel grade per EN 1993-1-8 Table 4.1.
    ...
    /// 🪛 Fillet weld resistance F_w,Rd [kN] = a·l·f_u/(√3·β_w·γ_M2) per EN 1993-1-8 §4.5.3.3 simplified method.
...
pub mod part_1_9 {
    ...
    /// 🔄 Fatigue detail category Δσ_C [MPa] per EN 1993-1-9 Table 8.1.
    ...
    /// 🔄 Fatigue assessment method selecting γ_Mf per EN 1993-1-9 Table 3.1.
    #[derive(Clone, Copy, Debug, PartialEq, Eq)]
    ...
    /// 🔄 Cycles to failure N per EN 1993-1-9 §7 constant-amplitude S-N curve, slope m=3.
    pub fn cycles_to_failure(delta_sigma_mpa: f64, category: u8) -> f64 {
        const M: f64 = 3.0;
        2.0e6 * (detail_category_mpa(category) / delta_sigma_mpa).powf(M)
    }

    /// 🔄 Fatigue verification γ_Ff·Δσ_E2 ≤ Δσ_C/γ_Mf per EN 1993-1-9 §8.
...
pub mod part_1_10 {
    ...
    /// 📊 Max permissible element thickness [mm] at σ_Ed=0.75·f_y(t), EN 1993-1-10 Table 2.1 grid (row-major by subgrade × T_Ed).
    ...
    /// 📊 Max permissible thickness [mm] per EN 1993-1-10 Table 2.1 (bilinear lookup, not an ad-hoc offset).
...
pub mod part_1_11 {
    ...
    /// 🎣 Tension component (cable/rod) resistance F_Rd = min(F_uk/(1.5·γ_R), F_k/γ_R), γ_R = 1.0, per EN 1993-1-11 §6.2.
...
pub mod part_1_12 {
    ...
    /// 💪 EN 1993-1-12 restricts S460–S700 to elastic verification only — no plastic moment redistribution.
    pub fn is_high_strength_restricted(f_y_mpa: f64) -> bool {
        (460.0..=700.0).contains(&f_y_mpa)
    }

    /// 📐 Elastic bending resistance M_Rd = W_el·f_y/γ_M0 [kNm], 0.0 if section class 4 (outside the elastic-only scope) per EN 1993-1-12 §4.
...
pub mod part_2 {
    ...
    /// 🌉 Damage-equivalent fatigue stress range Δσ_E2 = λ·Φ₂·Δσ_p per EN 1993-2 §9.3.
...
pub mod part_3 {
    ...
    /// 🗼 Tower leg buckling with wind gust amplification factor per EN 1993-3-1 §5.
...
pub mod part_4 {
    ...
    /// 🌾 Janssen-type horizontal silo pressure p_h = k·γ·z [kPa] per EN 1993-4-1 Annex C basis.
...
pub mod part_5 {
    ...
    /// 🔨 Pile driving stress limit σ_lim = 0.9·f_y per EN 1993-5 §12 (driving stresses).
...
/// 📋 Full steel member check across all sixteen EN 1993 parts (1-1 through 6), each reached from `evaluate`.
...
// #region 🔖Dsl
/// 📜 Handcrafted `key value`-per-line DSL for the EN 1993 steel `Document` — every field is a
...
impl vcs::DocumentDsl for Document {
    ...
    fn parse_dsl(text: &str) -> Result<Self, vcs::TextError> {
        ...
        Ok(Document {
            ...
            a_v_mm2: norm_core::dsl_kv::scalar(&fields, "a_v_mm2")?,
            w_pl_mm3: norm_core::dsl_kv::scalar(&fields, "w_pl_mm3")?,
            ...
            a_net_mm2: norm_core::dsl_kv::scalar(&fields, "a_net_mm2")?,
            tension_n_ed_kn: norm_core::dsl_kv::scalar(&fields, "tension_n_ed_kn")?,
            fire_thickness_mm: norm_core::dsl_kv::scalar(&fields, "fire_thickness_mm")?,
            fire_rating: norm_core::dsl_kv::scalar(&fields, "fire_rating")?,
            fire_massivity: norm_core::dsl_kv::scalar(&fields, "fire_massivity")?,
            fire_mu_0: norm_core::dsl_kv::scalar(&fields, "fire_mu_0")?,
            fire_design_temperature_c: norm_core::dsl_kv::scalar(&fields, "fire_design_temperature_c")?,
            cf_b_bar_mm: norm_core::dsl_kv::scalar(&fields, "cf_b_bar_mm")?,
            cf_t_mm: norm_core::dsl_kv::scalar(&fields, "cf_t_mm")?,
            cf_k_sigma: norm_core::dsl_kv::scalar(&fields, "cf_k_sigma")?,
            cf_psi: norm_core::dsl_kv::scalar(&fields, "cf_psi")?,
            cf_n_ed_kn: norm_core::dsl_kv::scalar(&fields, "cf_n_ed_kn")?,
            cf_gross_resistance_kn: norm_core::dsl_kv::scalar(&fields, "cf_gross_resistance_kn")?,
            stainless_m_ed_knm: norm_core::dsl_kv::scalar(&fields, "stainless_m_ed_knm")?,
            stainless_w_pl_mm3: norm_core::dsl_kv::scalar(&fields, "stainless_w_pl_mm3")?,
            stainless_f_y_mpa: norm_core::dsl_kv::scalar(&fields, "stainless_f_y_mpa")?,
            plated_lambda_p: norm_core::dsl_kv::scalar(&fields, "plated_lambda_p")?,
            plated_sigma_ed_mpa: norm_core::dsl_kv::scalar(&fields, "plated_sigma_ed_mpa")?,
            silo_t_mm: norm_core::dsl_kv::scalar(&fields, "silo_t_mm")?,
            silo_r_mm: norm_core::dsl_kv::scalar(&fields, "silo_r_mm")?,
            shell_sigma_x_ed_mpa: norm_core::dsl_kv::scalar(&fields, "shell_sigma_x_ed_mpa")?,
            silo_k: norm_core::dsl_kv::scalar(&fields, "silo_k")?,
            silo_gamma_kn_m3: norm_core::dsl_kv::scalar(&fields, "silo_gamma_kn_m3")?,
            silo_depth_m: norm_core::dsl_kv::scalar(&fields, "silo_depth_m")?,
            bolt_f_ed_kn: norm_core::dsl_kv::scalar(&fields, "bolt_f_ed_kn")?,
            bolt_n_bolts: norm_core::dsl_kv::scalar(&fields, "bolt_n_bolts")?,
            bolt_a_s_mm2: norm_core::dsl_kv::scalar(&fields, "bolt_a_s_mm2")?,
            bolt_e1_mm: norm_core::dsl_kv::scalar(&fields, "bolt_e1_mm")?,
            bolt_e2_mm: norm_core::dsl_kv::scalar(&fields, "bolt_e2_mm")?,
            bolt_d0_mm: norm_core::dsl_kv::scalar(&fields, "bolt_d0_mm")?,
            bolt_d_mm: norm_core::dsl_kv::scalar(&fields, "bolt_d_mm")?,
            bolt_t_mm: norm_core::dsl_kv::scalar(&fields, "bolt_t_mm")?,
            bolt_f_u_mpa: norm_core::dsl_kv::scalar(&fields, "bolt_f_u_mpa")?,
            bolt_f_ub_mpa: norm_core::dsl_kv::scalar(&fields, "bolt_f_ub_mpa")?,
            weld_a_mm: norm_core::dsl_kv::scalar(&fields, "weld_a_mm")?,
            weld_l_mm: norm_core::dsl_kv::scalar(&fields, "weld_l_mm")?,
            weld_f_u_mpa: norm_core::dsl_kv::scalar(&fields, "weld_f_u_mpa")?,
            weld_steel_grade: norm_core::dsl_kv::scalar(&fields, "weld_steel_grade")?,
            weld_f_ed_kn: norm_core::dsl_kv::scalar(&fields, "weld_f_ed_kn")?,
            delta_sigma_mpa: norm_core::dsl_kv::scalar(&fields, "delta_sigma_mpa")?,
            fatigue_category: norm_core::dsl_kv::scalar(&fields, "fatigue_category")?,
            fatigue_method: norm_core::dsl_kv::scalar(&fields, "fatigue_method")?,
            t10_steel_subgrade: norm_core::dsl_kv::scalar(&fields, "t10_steel_subgrade")?,
            t10_actual_thickness_mm: norm_core::dsl_kv::scalar(&fields, "t10_actual_thickness_mm")?,
            t10_t_ed_c: norm_core::dsl_kv::scalar(&fields, "t10_t_ed_c")?,
            tension_component_f_uk_kn: norm_core::dsl_kv::scalar(&fields, "tension_component_f_uk_kn")?,
            tension_component_f_k_kn: norm_core::dsl_kv::scalar(&fields, "tension_component_f_k_kn")?,
            tension_component_n_ed_kn: norm_core::dsl_kv::scalar(&fields, "tension_component_n_ed_kn")?,
            hss_w_el_mm3: norm_core::dsl_kv::scalar(&fields, "hss_w_el_mm3")?,
            hss_f_y_mpa: norm_core::dsl_kv::scalar(&fields, "hss_f_y_mpa")?,
            hss_section_class: norm_core::dsl_kv::scalar(&fields, "hss_section_class")?,
            hss_m_ed_knm: norm_core::dsl_kv::scalar(&fields, "hss_m_ed_knm")?,
            bridge_lambda: norm_core::dsl_kv::scalar(&fields, "bridge_lambda")?,
            bridge_phi_2: norm_core::dsl_kv::scalar(&fields, "bridge_phi_2")?,
            bridge_delta_sigma_p_mpa: norm_core::dsl_kv::scalar(&fields, "bridge_delta_sigma_p_mpa")?,
            tower_wind_factor: norm_core::dsl_kv::scalar(&fields, "tower_wind_factor")?,
            tower_n_ed_kn: norm_core::dsl_kv::scalar(&fields, "tower_n_ed_kn")?,
            pile_sigma_mpa: norm_core::dsl_kv::scalar(&fields, "pile_sigma_mpa")?,
            pile_k_red: norm_core::dsl_kv::scalar(&fields, "pile_k_red")?,
            pile_n_ed_kn: norm_core::dsl_kv::scalar(&fields, "pile_n_ed_kn")?,
            crane_f_z_ed_kn: norm_core::dsl_kv::scalar(&fields, "crane_f_z_ed_kn")?,
            crane_wheel_contact_length_mm: norm_core::dsl_kv::scalar(&fields, "crane_wheel_contact_length_mm")?,
            crane_dispersion_mm: norm_core::dsl_kv::scalar(&fields, "crane_dispersion_mm")?,
            crane_t_w_mm: norm_core::dsl_kv::scalar(&fields, "crane_t_w_mm")?,
        })
    }
    ...
    fn print_dsl(&self) -> String {
        [
            ...
            norm_core::dsl_kv::line("a_v_mm2", &self.a_v_mm2),
            norm_core::dsl_kv::line("w_pl_mm3", &self.w_pl_mm3),
            norm_core::dsl_kv::line("f_y_mpa", &self.f_y_mpa),
            norm_core::dsl_kv::line("f_u_mpa", &self.f_u_mpa),
            norm_core::dsl_kv::line("chi", &self.chi),
            norm_core::dsl_kv::line("a_net_mm2", &self.a_net_mm2),
            norm_core::dsl_kv::line("tension_n_ed_kn", &self.tension_n_ed_kn),
            norm_core::dsl_kv::line("fire_thickness_mm", &self.fire_thickness_mm),
            norm_core::dsl_kv::line("fire_rating", &self.fire_rating),
            norm_core::dsl_kv::line("fire_massivity", &self.fire_massivity),
            norm_core::dsl_kv::line("fire_mu_0", &self.fire_mu_0),
            norm_core::dsl_kv::line("fire_design_temperature_c", &self.fire_design_temperature_c),
            norm_core::dsl_kv::line("cf_b_bar_mm", &self.cf_b_bar_mm),
            norm_core::dsl_kv::line("cf_t_mm", &self.cf_t_mm),
            norm_core::dsl_kv::line("cf_k_sigma", &self.cf_k_sigma),
            norm_core::dsl_kv::line("cf_psi", &self.cf_psi),
            norm_core::dsl_kv::line("cf_n_ed_kn", &self.cf_n_ed_kn),
            norm_core::dsl_kv::line("cf_gross_resistance_kn", &self.cf_gross_resistance_kn),
            norm_core::dsl_kv::line("stainless_m_ed_knm", &self.stainless_m_ed_knm),
            norm_core::dsl_kv::line("stainless_w_pl_mm3", &self.stainless_w_pl_mm3),
            norm_core::dsl_kv::line("stainless_f_y_mpa", &self.stainless_f_y_mpa),
            norm_core::dsl_kv::line("plated_lambda_p", &self.plated_lambda_p),
            norm_core::dsl_kv::line("plated_sigma_ed_mpa", &self.plated_sigma_ed_mpa),
            norm_core::dsl_kv::line("silo_t_mm", &self.silo_t_mm),
            norm_core::dsl_kv::line("silo_r_mm", &self.silo_r_mm),
            norm_core::dsl_kv::line("shell_sigma_x_ed_mpa", &self.shell_sigma_x_ed_mpa),
            norm_core::dsl_kv::line("silo_k", &self.silo_k),
            norm_core::dsl_kv::line("silo_gamma_kn_m3", &self.silo_gamma_kn_m3),
            norm_core::dsl_kv::line("silo_depth_m", &self.silo_depth_m),
            norm_core::dsl_kv::line("bolt_f_ed_kn", &self.bolt_f_ed_kn),
            norm_core::dsl_kv::line("bolt_n_bolts", &self.bolt_n_bolts),
            norm_core::dsl_kv::line("bolt_a_s_mm2", &self.bolt_a_s_mm2),
            norm_core::dsl_kv::line("bolt_e1_mm", &self.bolt_e1_mm),
            norm_core::dsl_kv::line("bolt_e2_mm", &self.bolt_e2_mm),
            norm_core::dsl_kv::line("bolt_d0_mm", &self.bolt_d0_mm),
            norm_core::dsl_kv::line("bolt_d_mm", &self.bolt_d_mm),
            norm_core::dsl_kv::line("bolt_t_mm", &self.bolt_t_mm),
            norm_core::dsl_kv::line("bolt_f_u_mpa", &self.bolt_f_u_mpa),
            norm_core::dsl_kv::line("bolt_f_ub_mpa", &self.bolt_f_ub_mpa),
            norm_core::dsl_kv::line("weld_a_mm", &self.weld_a_mm),
            norm_core::dsl_kv::line("weld_l_mm", &self.weld_l_mm),
            norm_core::dsl_kv::line("weld_f_u_mpa", &self.weld_f_u_mpa),
            norm_core::dsl_kv::line("weld_steel_grade", &self.weld_steel_grade),
            norm_core::dsl_kv::line("weld_f_ed_kn", &self.weld_f_ed_kn),
            norm_core::dsl_kv::line("delta_sigma_mpa", &self.delta_sigma_mpa),
            norm_core::dsl_kv::line("fatigue_category", &self.fatigue_category),
            norm_core::dsl_kv::line("fatigue_method", &self.fatigue_method),
            norm_core::dsl_kv::line("t10_steel_subgrade", &self.t10_steel_subgrade),
            norm_core::dsl_kv::line("t10_actual_thickness_mm", &self.t10_actual_thickness_mm),
            norm_core::dsl_kv::line("t10_t_ed_c", &self.t10_t_ed_c),
            norm_core::dsl_kv::line("tension_component_f_uk_kn", &self.tension_component_f_uk_kn),
            norm_core::dsl_kv::line("tension_component_f_k_kn", &self.tension_component_f_k_kn),
            norm_core::dsl_kv::line("tension_component_n_ed_kn", &self.tension_component_n_ed_kn),
            norm_core::dsl_kv::line("hss_w_el_mm3", &self.hss_w_el_mm3),
            norm_core::dsl_kv::line("hss_f_y_mpa", &self.hss_f_y_mpa),
            norm_core::dsl_kv::line("hss_section_class", &self.hss_section_class),
            norm_core::dsl_kv::line("hss_m_ed_knm", &self.hss_m_ed_knm),
            norm_core::dsl_kv::line("bridge_lambda", &self.bridge_lambda),
            norm_core::dsl_kv::line("bridge_phi_2", &self.bridge_phi_2),
            norm_core::dsl_kv::line("bridge_delta_sigma_p_mpa", &self.bridge_delta_sigma_p_mpa),
            norm_core::dsl_kv::line("tower_wind_factor", &self.tower_wind_factor),
            norm_core::dsl_kv::line("tower_n_ed_kn", &self.tower_n_ed_kn),
            norm_core::dsl_kv::line("pile_sigma_mpa", &self.pile_sigma_mpa),
            norm_core::dsl_kv::line("pile_k_red", &self.pile_k_red),
            norm_core::dsl_kv::line("pile_n_ed_kn", &self.pile_n_ed_kn),
            norm_core::dsl_kv::line("crane_f_z_ed_kn", &self.crane_f_z_ed_kn),
            norm_core::dsl_kv::line("crane_wheel_contact_length_mm", &self.crane_wheel_contact_length_mm),
            norm_core::dsl_kv::line("crane_dispersion_mm", &self.crane_dispersion_mm),
            norm_core::dsl_kv::line("crane_t_w_mm", &self.crane_t_w_mm),
        ]
        .join("\n")
            + "\n"
    }
...
mod tests {
    ...
    /// 🔩 M20 bolt bearing worked example: e₁=e₂=40mm, d₀=22mm, t=10mm, f_u=510MPa, f_ub=800MPa → F_b,Rd ≈ 123.6 kN.
    #[test]
    ...
    /// 🪛 Fillet weld resistance sanity check for an S355 weld, β_w=0.9.
    #[test]
    fn fillet_weld_s355() {
    ...
    /// 🔥 Critical steel temperature worked example, EN 1993-1-2 Eq. 4.22 at μ₀=0.5.
    #[test]
    ...
    /// 🐚 Shell meridional critical buckling stress worked example: t=8mm, r=3000mm, E=210000MPa.
    #[test]
    ...
    /// 📐 Cold-formed reduction factor worked example: λ_p=1.0, ψ=1 → ρ=(1−0.055·4)/1=0.78.
    #[test]
