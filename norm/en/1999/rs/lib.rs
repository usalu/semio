...
pub mod na_de {
    ...
    /// 🇪🇺 EN 1999-1-1 §6.1.3 material partial factors, resolved per national annex choice.
    ...
    impl AnnexParams {
        ...
        /// 🇩🇪 DIN EN 1999-1-1/NA sets no NDP override for γ_M1/γ_M2 — equal to EN by design, not an oversight.
        pub fn de() -> Self {
...
pub mod part_1_1 {
    ...
    impl Alloy {
        ...
        /// 🔥 HAZ softened strength f_o,haz [MPa], representative GMAW-welded value per EN 1999-1-1 Table 6.2.
    ...
    /// 🔥 HAZ reduced strength f_o,haz [MPa] in the HAZ zone near welds.
    pub fn haz_strength_mpa(alloy: Alloy) -> f64 {
        alloy.f_o_haz_mpa()
    }

    /// 🔥 HAZ strength reduction factor ρ_o,haz = f_o,haz / f_o per EN 1999-1-1 Table 6.2, applied within the HAZ zone near welds.
    ...
    /// 🔗 Directional weld throat resistance F_w,Rd [kN] per EN 1999-1-1 §8.5 (absorbed here — welds are not a dedicated EN 1999 part).
...
pub mod part_1_4 {
    ...
    /// 📄 Plate slenderness λ_p for a cold-formed compression element per EN 1999-1-4 §5 (effective width method).
    pub fn plate_slenderness(b_mm: f64, t_mm: f64, k_sigma: f64, alloy: Alloy) -> f64 {
        let eps = alloy.epsilon();
        (b_mm / t_mm) / (28.4 * eps * k_sigma.sqrt())
    }

    /// 📄 Effective width reduction factor ρ per EN 1999-1-4 §5.4.
    ...
    /// 📄 Effective elastic section modulus W_eff [mm³] of a class 4 cold-formed cross-section.
...
/// 📋 Full EN 1999 check spanning every remaining part: 1-1 (cross-section, buckling, bending, welds), 1-2 (fire), 1-3 (fatigue), 1-4 (cold-formed sheeting), 1-5 (shell buckling).
...
mod tests {
    ...
    fn haz_reduction_factor_6082_t6_table_6_2() {
        // 📖 EN 1999-1-1 Table 6.2 representative GMAW HAZ value for 6082-T6: f_o,haz ≈ 185 MPa vs f_o = 260 MPa ⇒ ρ_o,haz ≈ 0.71.
    ...
    fn cold_formed_effective_width_factor_worked() {
        // 📖 EN 1999-1-4 §5.4: ρ(λ_p=1.0) = (1 − 0.22/1.0)/1.0 = 0.78.
    ...
    fn shell_critical_axial_stress_worked() {
        // 📖 EN 1999-1-5 §5: σ_cr = 0.605·E·t/r = 0.605·70000·4/500 = 338.8 MPa.
    ...
    fn annex_en_de_documented_equality() {
        // 📖 DIN EN 1999-1-1/NA does not override γ_M1/γ_M2, so EN and DE-NA must yield identical utilization.
    ...
    fn document_text_round_trips_through_store() {
        let envelope = vcs::create_document_vcs_envelope("norm.en1999/v1", "en1999", Document::default(), None);
