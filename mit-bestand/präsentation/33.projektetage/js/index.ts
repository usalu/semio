...
export function catalogueFocusDispositions(): readonly Disposition[] {
	...
	return remapSplitDispositions(
		CATALOGUE_SPLIT.dispositions.filter((disposition) =>
			focusTiles.some((tile) => tile.participantId === disposition.participantId),
		),
		positions,
	);
}
...
const slideModules = Object.fromEntries(
	await Promise.all(
		Object.entries(slideModuleLoaders).map(async ([path, loadModule]) => [path, await loadModule()] as const),
	),
) as Record<string, { readonly default: SlideFile }>;
