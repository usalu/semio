...
//#region 🔖Entwurfswerkzeug
export const ENTWERFSWERKZEUG_KATALOG_FRAME = { x: 0.04, y: 0.06, width: 0.92, height: 0.88 };
export const ENTWERFSWERKZEUG_KATALOG_GAP = 0.02;

export const ENTWERFSWERKZEUG_KATALOG_ITEMS = [
	{
...
/** @emoji 🧩 Participants, embodiments, and 1×3 grid dispositions for the Katalog slide. */
export function entwurfswerkzeugKatalogArtifacts(): {
	readonly participants: Participant[];
	readonly embodiments: Embodiment[];
	readonly dispositions: Disposition[];
} {
	const cells = splitFigureGrid({
		rows: 1,
		columns: 3,
		frame: ENTWERFSWERKZEUG_KATALOG_FRAME,
		gap: ENTWERFSWERKZEUG_KATALOG_GAP,
	});
	const participants: Participant[] = [];
	const embodiments: Embodiment[] = [];
	const dispositions: Disposition[] = [];
	for (const [index, item] of ENTWERFSWERKZEUG_KATALOG_ITEMS.entries()) {
		const position = cells[index]?.position;
		if (!position) {
			throw new Error(`entwurfswerkzeugKatalogArtifacts: missing grid cell for index ${index}.`);
		}
		const embodimentId = `${item.id}--figure`;
		participants.push({ id: item.id });
		embodiments.push({
