//! 📦 `pack_io` — native file I/O for the `pack` binary document container family:
...
mod native {
    ...
    /// @emoji 🚨 Wraps a `std::io::Error` into the crate-wide `PackError::Io` variant — the only
    ...
    //#region 🔖File
    /// @emoji 📂 A read-only, random-access file source. Positional reads go through
    ...
    impl FilePackSource {
        /// @emoji 📖 Opens `path` for reading and stat's its length up front.
    ...
    /// @emoji 📤 A write-only file sink opened truncate-on-create; tracks its own write position
    ...
    impl FilePackSink {
        /// @emoji 🆕 Creates (truncating any existing file) `path` for writing.
    ...
    //#region 🔖Atomic
    /// @emoji 🔢 Per-process monotonic counter mixed into temp-file names so concurrent
    /// `write_atomic` calls (even to the same `path`, even on the same PID) never collide.
    static TMP_COUNTER: AtomicU64 = AtomicU64::new(0);

    /// @emoji 🛟 Writes `bytes` to `path` atomically: writes to a sibling temp file, `fsync`s it,
    ...
    //#region 🔖Stream
    /// @emoji 🌊 An incremental pack file writer: segments/chunks are framed and flushed straight
    ...
    impl StreamingPackWriter {
        /// @emoji 🚀 Creates `path` and writes the 32-byte header.
        ...
        /// @emoji 🧱 Writes and flushes one chunk segment, returning its `ChunkId`.
        pub fn write_chunk(&mut self, payload: &[u8]) -> Result<ChunkId, PackError> {
            self.inner.write_chunk(payload)
        }

        /// @emoji 🏁 Writes the chunk table, manifest, end marker, and footer, `fsync`s (via
    ...
    //#region 🔖Recover
    /// @emoji 🩺 Opens `path` and forward-scans it via `pack_format::recover` — for use when a
    ...
    mod tests {
        ...
        //#region 🔖Stream
        /// @emoji 📏 Wire length of a non-compressed segment frame (`kind, flags, seg_len varint,
