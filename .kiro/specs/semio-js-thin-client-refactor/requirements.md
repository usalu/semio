...
Refactor `semio/js` (`index.ts`, ~31k lines) from a monolithic TypeScript library containing domain logic, caching, and free functions into a pure object-oriented thin client that exclusively delegates to `semio/rs` via WASM. All domain logic (flatten geometry, piece placement, representation selection, diff computation, hashing, etc.) and all caching (flatten merkle cache, SQL.js cache) must be removed from `semio/js` and either already exist or be migrated to `semio/rs`. Every exported free function must become a method on an appropriate class. This is a greenfield repo with no backwards compatibility requirements.

## Glossary

- **Thin_Client**: The refactored `semio/js` library that contains zero domain logic and zero caching, delegating all computation to `semio/rs` via WASM.
- **WASM_Bridge**: The Worker + Comlink communication layer between `semio/js` and the `KitStoreHandle` class exposed by `@semio/rs-wasm`.
- **KitStoreHandle**: The Rust-side WASM class (`semio/rs/pkg`) that owns the live `KitGraph`, exposes GraphQL, read commands, change commands, VCS state, and domain operations.
- **KitStoreClient**: The TypeScript interface in `semio/js` that defines the boundary contract consumed by `@semio/react` and the sketchpad.
...
### Requirement 1: Remove All Domain Logic from semio/js

**User Story:** As a developer, I want semio/js to contain zero domain logic, so that all computation is centralized in semio/rs and semio/js is a pure delegation layer.
...
14. THE Thin_Client SHALL NOT contain any geometry conversion functions that perform 3D math (`planeToMatrix`, `matrixToPlane`, `averagePlane`, `toThreeRotation`, `toSemioRotation`, `toThreeQuaternion`, `toSemioQuaternion`, `vectorToThree`, or equivalent functions).
...
### Requirement 2: Remove All Caching from semio/js

**User Story:** As a developer, I want semio/js to contain zero caching, so that cache management is centralized in semio/rs and semio/js has no stale-data risks.
...
5. THE Thin_Client SHALL NOT contain any undo/redo history stacks (`#historyDone`, `#historyUndone`, or equivalent) — undo/redo state is owned by `KitStoreHandle` in semio/rs.

### Requirement 3: Eliminate All Free Functions

**User Story:** As a developer, I want every exported function in semio/js to be a method on a class, so that the API is fully object-oriented and discoverable.
...
3. THE Thin_Client SHALL NOT export any free Diff_Function. WHEN diff operations are needed, THE Entity_Class SHALL provide them as instance or static methods, or THE Thin_Client SHALL delegate to semio/rs.
4. THE Thin_Client SHALL NOT export any free Hash_Function. WHEN hashing is needed, THE Entity_Class SHALL provide it as an instance method, or THE Thin_Client SHALL delegate to semio/rs.
...
7. THE Thin_Client SHALL NOT export any free design-diff builder functions (`addPieceToDesignDiff`, `removePieceFromDesignDiff`, `addConnectionToDesignDiff`, `removeConnectionFromDesignDiff`, `setPieceInDesignDiff`, and all variants). WHEN design diff building is needed, THE Thin_Client SHALL delegate to semio/rs.
8. THE Thin_Client SHALL NOT export any free collection-diff functions (`getCollectionDiff`, `inverseCollectionDiff`, `applyCollectionDiff`, `mergeCollectionDiff`). WHEN collection diff operations are needed, THE Thin_Client SHALL delegate to semio/rs.
9. THE Thin_Client SHALL NOT export any free backbone factory functions (`createLocalBackbone`, `createDevBackbone`, `createRemoteBackbone`). WHEN backbone creation is needed, THE Thin_Client SHALL provide it as a static method on a `Backbone` class or delegate to semio/rs.
...
### Requirement 4: semio/js Exclusively Uses semio/rs for All Operations

**User Story:** As a developer, I want semio/js to delegate every operation to semio/rs via WASM, so that there is a single source of truth for all kit logic.

#### Acceptance Criteria

1. WHEN the KitStoreClient performs a flatten operation, THE Thin_Client SHALL delegate to `KitStoreHandle.flattenDesign` in semio/rs and SHALL NOT perform any local flatten computation.
2. WHEN the KitStoreClient retrieves piece metadata (planes, centers), THE Thin_Client SHALL delegate to `KitStoreHandle.getPiecesMetadata` in semio/rs and SHALL NOT compute placement locally.
3. WHEN the KitStoreClient performs undo or redo, THE Thin_Client SHALL delegate to `KitStoreHandle.undo` / `KitStoreHandle.redo` in semio/rs and SHALL NOT maintain local history stacks.
4. WHEN the KitStoreClient performs a drag, move, cluster, fix, expand, or paste operation, THE Thin_Client SHALL delegate to the corresponding `KitStoreHandle` method in semio/rs.
5. WHEN the KitStoreClient creates a connected piece, fixed piece, or hanging pieces, THE Thin_Client SHALL delegate to the corresponding `KitStoreHandle` method in semio/rs.
6. WHEN the KitStoreClient reads kit state (types, designs, pieces, connections, authors, metadata), THE Thin_Client SHALL delegate to the corresponding `KitStoreHandle` getter or `executeRead` in semio/rs.
7. WHEN the KitStoreClient performs VCS operations (vcsState, materializeAt, attachBackbone, detachBackbone, syncNow, listConflicts, resolveConflict), THE Thin_Client SHALL delegate to `KitStoreHandle.execute` in semio/rs.
8. WHEN the KitStoreClient validates a kit or computes a diff, THE Thin_Client SHALL delegate to `kitValidate` or equivalent semio/rs WASM functions.
9. WHEN the KitStoreClient needs to serialize or deserialize a kit, THE Thin_Client SHALL delegate to `kitToJson` / `kitFromJson` in semio/rs.
10. WHEN the KitStoreClient needs to compare kits for equality, THE Thin_Client SHALL delegate to `kitsAreEqual` in semio/rs.
11. WHEN the KitStoreClient needs to normalize a name, THE Thin_Client SHALL delegate to `semioNormalizeName` in semio/rs.
12. WHEN the KitStoreClient needs to generate an ID, THE Thin_Client SHALL delegate to `generateId` in semio/rs.
13. WHEN the KitStoreClient needs to round a numeric value, THE Thin_Client SHALL delegate to `semioRound` in semio/rs.
...
3. THE Thin_Client SHALL retain the `KitStoreClient` interface as the boundary contract consumed by `@semio/react` and the sketchpad.
...
**User Story:** As a developer, I want semio/js to still export the TypeScript kind definitions and Zod schemas for kit entities, so that consumers can validate and kind-check data received from semio/rs.
...
1. THE Thin_Client SHALL retain the embedded test mechanism triggered by `SEMIO_JS_RUN_EMBEDDED_TESTS=1`.
2. WHEN embedded tests exercise domain logic that has moved to semio/rs, THE Thin_Client SHALL update those tests to call through the WASM_Bridge instead of local JS functions.
...
3. THE Thin_Client SHALL NOT retain the `KitImpl` class with its embedded domain logic. WHEN `KitImpl` functionality is needed, THE Thin_Client SHALL delegate to `KitStoreHandle` in semio/rs.
...
5. THE Thin_Client SHALL NOT retain the `KitEntity`, `KitEntityIndexes`, `KitEntityCaches`, `KitInteractionsApi`, `KitInteractionEntity`, `KitEntityType`, `KitEntityPiece`, `KitEntityDesign`, `KitDocument`, or `KitBackboneBridge` classes — these contain domain logic that belongs in semio/rs.
6. THE Thin_Client SHALL NOT retain the `KitTypesOps`, `KitDesignsOps`, `KitFamiliesOps`, `KitFilesOps`, `KitTagsOps`, `KitConceptsOps`, `KitAttributesOps`, or `KitOps` classes — these contain domain logic that belongs in semio/rs.
