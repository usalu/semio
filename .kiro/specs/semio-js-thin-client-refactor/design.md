# Design Document: semio-js-thin-client-refactor

## Overview

This design transforms `semio/js/index.ts` (~31k lines) from a monolithic TypeScript library containing domain logic, caching, diff/hash computation, transaction management, and free functions into a pure OO thin client that exclusively delegates to `semio/rs` via WASM. The refactored file retains only:
...
flowchart TB
    subgraph "semio/js (Thin Client)"
        ...
        RC[ReadCommandBatch Types<br/>generated from semio/rs]
        WT[Wire Kinds<br/>SetResult, WriteStatus, etc.]
    end

    subgraph "Web Worker"
        WASM[semio/rs WASM<br/>KitStoreHandle]
    end
...
| Geometry 3D math | `planeToMatrix`, `matrixToPlane`, `averagePlane`, `toThreeRotation`, `toSemioRotation`, `toThreeQuaternion`, `toSemioQuaternion`, `vectorToThree` |
...
The `KitStoreClient` interface remains the boundary contract consumed by `@semio/react` and the sketchpad. No methods are added or removed — every method delegates to `KitStoreHandle` via WASM.
...
Free utility functions become static methods on a `Semio` class that delegates to WASM:

```typescript
export class Semio {
  /** Delegates to semioNormalizeName in semio/rs. */
  static normalizeName(s: string): string { return wasmModule.semioNormalizeName(s); }
  /** Delegates to semioRound in semio/rs. */
  static round(value: number, decimals: number): number { return wasmModule.semioRound(value, decimals); }
  /** Delegates to generateId in semio/rs. */
  static generateId(): string { return wasmModule.generateId(); }
  /** Delegates to kitFromJson in semio/rs. */
  static async kitFromJson(s: string): Promise<any> { return wasmModule.kitFromJson(s); }
  /** Delegates to kitToJson in semio/rs. */
  static async kitToJson(value: any): Promise<any> { return wasmModule.kitToJson(value); }
  /** Delegates to kitValidate in semio/rs. */
  static async kitValidate(value: any): Promise<any> { return wasmModule.kitValidate(value); }
  /** Delegates to kitsAreEqual in semio/rs. */
  static async kitsAreEqual(a: any, b: any): Promise<any> { return wasmModule.kitsAreEqual(a, b); }
  /** Delegates to flattenDesign in semio/rs. */
...
All Zod schemas and their inferred TypeScript kind aliases are retained unchanged. They define the wire format between `semio/js` and `semio/rs`:
...
The `ReadCommandBatch` and `ReadCommandBatchResult` kinds are generated from `semio/rs/read_module.rs` via `gen_read_command_types.py`. They remain unchanged:
...
```mermaid
sequenceDiagram
    participant Consumer as @semio/react
...
5. **Embedded tests**: The existing embedded test suite (triggered by `SEMIO_JS_RUN_EMBEDDED_TESTS=1`) is updated to call through the WASM bridge. Tests that exercised deleted domain logic are either rewritten to use WASM or removed with a comment explaining why.
...
- Tag format: `Feature: semio-js-thin-client-refactor, Property {number}: {property_text}`
...
- Embedded tests: `cross-env SEMIO_JS_RUN_EMBEDDED_TESTS=1 vitest run`
- All tests remain in `semio/js/index.ts` (embedded test pattern, no separate test files per workspace rules)
