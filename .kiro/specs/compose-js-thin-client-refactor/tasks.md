# Implementation Plan: compose-js-thin-client-refactor

## Overview

Transform `compose/js/index.ts` (~31k lines) into a pure OO thin client by: (1) deleting all domain logic classes, caching, free functions, and legacy stores, (2) retaining Zod schemas, type aliases, entity classes (with OO methods only), WASM bridge, wire types, read command types, GraphQL wire layer, live read facades, constants, and Generator, (3) adding the `Compose` utility class and thin `Kit` class, (4) updating embedded tests to use the WASM bridge, and (5) adding `fast-check` for property-based tests. All code stays in `compose/js/index.ts` using regions.

## Tasks

- [x] 1. Add fast-check dev dependency and prepare test infrastructure
  - [x] 1.1 Add `fast-check` as a dev dependency in `compose/js/package.json`
  ...
  - [x] 1.2 Verify existing test runner configuration works with embedded tests
    - Confirm `cross-env COMPOSE_JS_RUN_EMBEDDED_TESTS=1 vitest run` executes successfully before any deletions
    - _Requirements: 8.1_
...
- [x] 6. Delete all domain logic functions
  ...
  - [x] 6.9 Delete all geometry 3D math functions (`planeToMatrix`, `matrixToPlane`, `averagePlane`, `toThreeRotation`, `toComposeRotation`, `toThreeQuaternion`, `toComposeQuaternion`, `vectorToThree`)
    - _Requirements: 1.14_
...
- [x] 9. Create the `Compose` utility class
  - [x] 9.1 Implement the `Compose` class with static methods delegating to WASM
    - `Compose.normalizeName(s)` → `wasmModule.composeNormalizeName(s)`
    - `Compose.round(value, decimals)` → `wasmModule.composeRound(value, decimals)`
    - `Compose.generateId()` → `wasmModule.generateId()`
    - `Compose.kitFromJson(s)` → `wasmModule.kitFromJson(s)`
    - `Compose.kitToJson(value)` → `wasmModule.kitToJson(value)`
    - `Compose.kitValidate(value)` → `wasmModule.kitValidate(value)`
    - `Compose.kitsAreEqual(a, b)` → `wasmModule.kitsAreEqual(a, b)`
    - `Compose.flattenDesign(kit, designId)` → `wasmModule.flattenDesign(kit, designId)`
...
- [x] 14. Update embedded tests to use WASM bridge
  - [x] 14.1 Rewrite embedded tests that exercised deleted domain logic to call through `FallbackKitStoreClient` or `Compose` utility class
...
- All code stays in `compose/js/index.ts` using regions — no separate test files per workspace rules
...
- The `Compose` utility class replaces all free utility functions with WASM-delegating static methods
