...
import { BundleScript, ScriptRouter, buildBudgetMs, runBundleScriptMain, runCargoLint, runCargoTestBudgeted, runCmdStatus, runVitest, resolveTestLevel } from "../../../🧰️framework/🛍️product/🦑️repo/🔨️module/📚️lib/⚡️implementation/🟦️typescript/📦️index.ts";
...
//#region 🔖️Typegen
/** 🧬️ Name of the `#[cfg(test, feature = "typegen")]` test in `rs/lib.rs` that calls `TS::export()` for every mirrored type. */
...
/** 🧬️ Runs the ts-rs export test into a scratch `rs/bindings/` dir, consolidates it in memory, then removes the scratch dir — the caller decides whether to write the result. */
...
/** 🔎️ Rebuilds the manifest in memory and byte-compares it against `js/generated/manifest.ts` — never writes that file (a lint/verify step must never let the auto-commit daemon land regenerated files). */
