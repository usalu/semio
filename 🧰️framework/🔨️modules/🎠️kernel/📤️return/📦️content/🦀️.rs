...
pub struct ReturnContentWriteProgress {
    pub written_bytes: usize,
    pub complete: bool,
}

#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub struct ReturnContentReadProgress {
    pub consumed_bytes: usize,
    pub complete: bool,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct ReturnContentReadError {
    pub consumed_bytes: usize,
    pub reason: &'static str,
}

pub struct ReturnContentHeader {
    bytes: [u8; RETURN_CONTENT_HEADER_MAXIMUM_BYTES],
    length: u8,
    offset: u8,
}
...
impl ReturnContentHeader {
    ...
    pub fn new(tag: u8, mut body_length: u64) -> Result<Self, &'static str> {
        if tag > 9 {
            return Err("return-content.header-tag");
        }
    ...
    pub fn write(&mut self, output: &mut [u8], maximum_items: usize, maximum_bytes: usize) -> ReturnContentWriteProgress {
        if maximum_items == 0 || maximum_bytes == 0 {
            return ReturnContentWriteProgress { written_bytes: 0, complete: self.is_complete() };
        }
...
pub struct ReturnContentHeaderReader {
    tag: u8,
    length: u64,
    index: u8,
    state: ReadState,
...
impl ReturnContentHeaderReader {
    pub fn new() -> Self {
        Self { tag: 0, length: 0, index: 0, state: ReadState::Tag }
    }
    ...
    pub fn consume(&mut self, input: &[u8], maximum_items: usize, maximum_bytes: usize) -> Result<ReturnContentReadProgress, ReturnContentReadError> {
        if let ReadState::Fault(reason) = self.state {
            return Err(ReturnContentReadError { consumed_bytes: 0, reason });
        }
        if maximum_items == 0 || maximum_bytes == 0 || self.value().is_some() {
            return Ok(ReturnContentReadProgress { consumed_bytes: 0, complete: self.value().is_some() });
        }
        ...
        for (index, byte) in input[..limit].iter().copied().enumerate() {
            ...
            if self.value().is_some() {
                return Ok(ReturnContentReadProgress { consumed_bytes: index + 1, complete: true });
            }
        }
    ...
    fn consume_byte(&mut self, byte: u8) -> Result<(), &'static str> {
        match self.state {
            ...
            ReadState::Length => {
                if self.index == 9 && byte & 126 != 0 {
                    return Err("return-content.header-overflow");
                }
                self.length |= u64::from(byte & 127) << (u32::from(self.index) * 7);
                if byte & 128 == 0 {
                    if self.index != 0 && byte == 0 {
                        return Err("return-content.header-noncanonical");
                    }
                    self.state = ReadState::Complete;
                } else {
                    if self.index == 9 {
                        return Err("return-content.header-overlong");
                    }
                    self.index += 1;
                }
            }
