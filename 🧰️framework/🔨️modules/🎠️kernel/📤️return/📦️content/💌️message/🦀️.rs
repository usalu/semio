...
pub struct ReturnMessageProgress {
...
impl<'a> ReturnMessageCursor<'a> {
    ...
    pub fn new(effect: &'a Effect) -> Result<Self, &'static str> {
        let Effect::SendMessage { target, payload } = effect else {
            return Err("return-content.message-effect");
        };
...
fn instance_number(value: &str) -> Result<u32, &'static str> {
    if value.is_empty() || value.len() > 10 || (value.len() > 1 && value.as_bytes()[0] == b'0') {
        return Err("return-content.message-instance");
    }
    let mut result = 0u32;
