...
fn validate_schema_node(scope: Scope<'_>, schema: &Value, path: &str, traversal: &mut Traversal<'_>) -> Result<(), SchemaError> {
    ...
    for (keyword, value) in object {
        ...
        match keyword {
            ...
            "additionalProperties" | "additionalItems" | "not" => validate_schema_node(scope, value, &keyword_path, traversal)?,
...
/// 🔗 Resolves `$ref` to `(document it lives in, referenced node)` — `#`/`#/…` inside the current
/// document, `<$id>`/`<$id>#/…` into a registered sibling document.
fn resolve_reference<'a>(scope: Scope<'a>, reference: &str) -> Result<(&'a Value, &'a Value), String> {
    ...
    let base = if document_id.is_empty() {
        scope.base
    } else {
...
fn validate_string(scope: Scope<'_>, schema: &Object, value: &Value, path: &str) -> Result<(), SchemaError> {
    ...
    if let Some(pattern) = schema.get("pattern").and_then(Value::as_str) {
        let compiled;
        let matcher = match scope.patterns.get(pattern) {
            Some(matcher) => matcher,
            None => {
                compiled = PatternMatcher::compile(pattern).map_err(|reason| SchemaError::Validation(format!("{path}.pattern: {reason}")))?;
                &compiled
            }
        };
...
/// classes, groups, alternation and quantifiers over `char`s.
///
            ...
            '(' => {
                if self.peek() == Some('?') {
                    if self.chars.get(self.position + 1) != Some(&':') {
                        return Err("lookaround and named groups are outside the supported subset".to_string());
                    }
                    self.position += 2;
                }
