...
`);
  ...
  const femMountedJobs = "pub trait BoundedJob {} JobBody::Bounded(owner);";
...
async fn publish_mounted_typed_operation_unit() {
  ...
  const snapshotMergingStore = "pub fn fold_event_log<P, Mutation>( async fn admit_remote_transitions( fn refuse_with_candidate_dag<T>(&mut self, candidate_dag: crate::os_spr::MutationDag, error: VcsError) async fn accept_quarantined_events( Genesis { async fn merge_remote_snapshot(&mut self) {}";
  if (toolJobArtifactEventSourcedMergeExact(snapshotMergingStore)) throw new Error("[verify interactivity tool-jobs] self-test store-merging-remote-snapshots was falsely accepted.");
