//! 🧮️ `compiler_math` — lays out a `compiler_syntax::MathNode` into a backend-neutral tree of
...
use compiler_syntax::{BinOp, MathNode};
...
fn text_box(font: &Font<'_>, font_kind: FontKind, text: &str) -> MathBox {
    let run = compiler_text::shape(font, text);
...
fn raster_glyph_box(raster: compiler_text::RasterGlyph) -> MathBox {
...
fn emoji_box(fonts: &FontContext<'_>, name: &str) -> MathBox {
    ...
    let Some(raster) = compiler_text::glyph_raster_image(fonts.emoji, glyph_id, EMOJI_PIXELS_PER_EM) else {
        return text_box(fonts.serif, FontKind::Serif, &format!(":{name}:"));
    };
...
/// [`compiler_syntax::parse_formula`]-driven layout, this never fails and never interprets `text`'s
...
/// sequence — HarfBuzz shaping via [`compiler_text::shape`] already collapses those into the
...
pub fn layout_raw_emoji(fonts: &FontContext<'_>, text: &str) -> MathBox {
    let run = compiler_text::shape(fonts.emoji, text);
    let mut items = Vec::with_capacity(run.glyphs.len());
    let mut x = 0.0f32;
    let mut height = 0.0f32;
    let mut depth = 0.0f32;
    for glyph in &run.glyphs {
        if let Some(raster) = compiler_text::glyph_raster_image(fonts.emoji, glyph.glyph_id, EMOJI_PIXELS_PER_EM) {
...
fn layout_scripted(fonts: &FontContext<'_>, base: MathBox, script: MathBox, superscript: bool) -> MathBox {
    let Some(constants) = compiler_text::math_constants(fonts.math) else {
...
fn pick_vertical_stretch(font: &Font<'_>, base_glyph_id: u16, target_extent: f32) -> (u16, f32) {
    ...
    let variants = compiler_text::math_stretch_variants(font, base_glyph_id, true);
...
fn delimited(fonts: &FontContext<'_>, open: char, close: char, inner: MathBox) -> MathBox {
    let axis = compiler_text::math_constants(fonts.math).map_or(0.25, |c| mc_em(fonts.math, c.axis_height));
...
fn layout_accent(fonts: &FontContext<'_>, kind: &str, base_node: Option<&MathNode>) -> MathBox {
    ...
    let base_center = base_node.and_then(|n| base_glyph_id_for_top_accent(fonts, n)).and_then(|gid| compiler_text::math_top_accent_attachment(fonts.math, gid)).map_or(base.width / 2.0, |v| em(fonts.math, v as i32));
    let gap = compiler_text::math_constants(fonts.math).map_or(0.05, |c| mc_em(fonts.math, c.overbar_vertical_gap));
...
fn layout_sequence(fonts: &FontContext<'_>, items: &[compiler_syntax::SeqItem]) -> MathBox {
...
mod tests {
    ...
    fn with_fonts<R>(f: impl FnOnce(&FontContext<'_>) -> R) -> R {
        let fonts = compiler_world::embedded_fonts();
