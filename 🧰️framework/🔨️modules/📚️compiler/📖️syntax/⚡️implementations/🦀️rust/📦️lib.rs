...
/// whole to `dsl_core::lex`, exactly like `dsl_grammar::lex` does for its own `? |` extras.
...
mod tests {
    ...
    /// check against real math text, which `dsl_grammar::Recognizer` cannot do here since it
    ...
    fn math_grammar_parses_under_dsl_grammar() {
        let source = include_str!("../../📖️math.grammar");
        let parsed = dsl_grammar::parse_grammar(source).expect("📖️math.grammar must parse under dsl_grammar's own parser");
        assert_eq!(parsed.id, "math");
        assert_eq!(parsed.start, "formula");
        let printed = dsl_grammar::print_grammar(&parsed);
        let reparsed = dsl_grammar::parse_grammar(&printed).expect("canonical print of 📖️math.grammar must reparse");
