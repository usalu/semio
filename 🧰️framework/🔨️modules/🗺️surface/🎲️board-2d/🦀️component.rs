...
//! headless `BoardHost`/`BoardEngine` compute (constitutional: framework surface, not engine —
//! keeps the puzzle 2d app's `⚙️engine` slot free of wasm-bindgen/web-sys/wgpu so a workflow runner
//! can drive it headlessly). Mirrors `framework/surface/node-graph`'s `GraphHost`/`GraphSession` split,
...
#[cfg(target_arch = "wasm32")]
use mathematical_geometry::geometry::ray_from_origin_to_axis_aligned_rectangle_edge;
