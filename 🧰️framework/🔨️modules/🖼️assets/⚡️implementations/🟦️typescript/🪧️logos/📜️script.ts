#!/usr/bin/env bun
/**
 * 🧭️ Logo workspace router: `bun ./📜️script.ts generate` — builds `🔣️logo_generated.svg` from keyframe SVGs.
 */
// #region 🧲️Header
...
//#endregion 🎈️SVG Parsing

//#region 📮️Sequence Generation
...
//#endregion 📮️Sequence Generation

//#region 📻️Animated SVG Output
...
//#endregion 📻️Animated SVG Output
...
function runLogoGenerate(): void {
  ...
  createAnimatedSVG(keyframes, path.join(logoDir, "🔣️logo_generated.svg"));
}
...
class ExportScript extends BundleScript {
  async run(): Promise<void> {
    const logoDir = import.meta.dir;
    const inputPath = path.join(logoDir, "🔣️logo_generated.svg");
    const outputPath = path.join(logoDir, "🎥️logo.mp4");
