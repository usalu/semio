...
import { existsSync, lstatSync, mkdirSync, readdirSync, readFileSync, rmdirSync, unlinkSync } from "node:fs";
...
export function findManifestFiles(root: string, pluginAreas: readonly string[] = loadTaxonomy().pluginAreas): string[] {
  ...
  function walk(dir: string) {
    ...
    for (const name of names) {
      ...
      if (st.isSymbolicLink()) throw new Error(`admitted graph manifest input is a symbolic link: ${relative(root, path)}`);
      if (st.isDirectory()) {
        walk(path);
  ...
  for (const area of pluginAreas) {
    ...
    if (!pathIsExcluded(root, scanRoot, taxonomy) && existsSync(scanRoot)) {
      const rootStatus = lstatSync(scanRoot);
      if (rootStatus.isSymbolicLink()) throw new Error(`admitted graph manifest root is a symbolic link: ${relative(root, scanRoot)}`);
      if (!rootStatus.isDirectory()) throw new Error(`admitted graph manifest root is not a directory: ${relative(root, scanRoot)}`);
      walk(scanRoot);
    }
  }
  return out.sort();
...
export function renderGraphArtifacts(root: string, outDir: string, log = true): { artifacts: readonly GraphArtifact[]; manifestCount: number } {
    ...
    for (const path of files) {
      ...
      if (doc.schema !== "manifest") throw new Error(`admitted graph manifest has schema ${JSON.stringify(doc.schema)}: ${relative(root, path)}`);
    ...
    const outputs = parseGraphOutputCatalog(JSON.parse(readFileSync(resolve(import.meta.dir, "../📇️outputs.json"), "utf8")), docs.map((doc) => doc.id));
