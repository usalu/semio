...
//! {}`) case, kept in a SEPARATE file from `🕳️uninhabited.rs` because `#[dyn_enum]` rejects mixing
//! `&mut self` with `self: Arc<Self>` on the same trait (see `🕳️uninhabited.rs`'s comment).
