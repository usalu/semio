...
impl FromValue for ExpectedSet {
    fn from_value(value: DslValue) -> Result<Self, ValueError> {
        let entries = match value {
            ...
            other => return Err(ValueError::new(format!("expected an object for ExpectedSet, found {other:?}"))),
        };
            ...
            match entries.iter().find(|(name, _)| name == key).map(|(_, slot)| slot) {
                ...
                Some(DslValue::Array(items)) => items
                    ...
                    .map(|item| match item {
                        ...
                        other => Err(ValueError::new(format!("expected a string in ExpectedSet.{key}, found {other:?}"))),
                    })
                    .collect(),
                Some(other) => Err(ValueError::new(format!("expected an array for ExpectedSet.{key}, found {other:?}"))),
            }
        };
...
impl FromValue for Diagnostic {
    fn from_value(value: DslValue) -> Result<Self, ValueError> {
        let entries = match value {
            DslValue::Object(entries) => entries,
            other => return Err(ValueError::new(format!("expected an object for Diagnostic, found {other:?}"))),
        };
        let find = |key: &str| entries.iter().find(|(name, _)| name == key).map(|(_, slot)| slot.clone());
        let required = |key: &str| -> Result<DslValue, ValueError> {
            find(key).ok_or_else(|| ValueError::new(format!("missing Diagnostic.{key}")))
        };
        let message = match required("message")? {
            DslValue::String(text) => text,
            other => return Err(ValueError::new(format!("expected a string for Diagnostic.message, found {other:?}"))),
        };
        Ok(Diagnostic {
