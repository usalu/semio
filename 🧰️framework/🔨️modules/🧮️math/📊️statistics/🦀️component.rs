...
use crate::probability::{ChiSquared, Continuous, Normal, StudentT};
...
pub enum StatisticsError {
    ...
    #[error(transparent)]
    Tabular(#[from] crate::tabular::TabularError),
    #[error(transparent)]
    Probability(#[from] crate::probability::ProbabilityError),
}
// #endregion 🔖️Error
...
/// `crate::algebra::MatD::lu_solve` — adequate at causal-discovery scale (small `p`, modest
...
fn build_strata(x: &[u32], y: &[u32], given: &[&[u32]], nx: usize, ny: usize, given_levels: &[usize]) -> HashMap<usize, MatD> {
    ...
    for row in 0..x.len() {
        if x[row] == crate::tabular::MISSING_CODE || y[row] == crate::tabular::MISSING_CODE {
            continue;
        }
        ...
        for (k, g) in given.iter().enumerate() {
            if g[row] == crate::tabular::MISSING_CODE {
                missing = true;
                break;
            }
...
pub fn crosstab(x: &[u32], y: &[u32], nx: usize, ny: usize) -> Result<MatD, StatisticsError> {
    ...
    for (&xi, &yi) in x.iter().zip(y) {
        if xi == crate::tabular::MISSING_CODE || yi == crate::tabular::MISSING_CODE {
            continue;
        }
...
pub fn entropy(codes: &[u32], n_levels: usize) -> Result<f64, StatisticsError> {
    ...
    for &c in codes {
        if c == crate::tabular::MISSING_CODE {
            continue;
        }
        counts[c as usize] += 1;
        total += 1;
