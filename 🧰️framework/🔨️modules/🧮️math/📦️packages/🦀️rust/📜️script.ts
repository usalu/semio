#!/usr/bin/env bun
/** 📜️ `@semio-tech/framework-math` — the one semio math crate: manifest codegen, cargo test and clippy gates. */
import { existsSync, mkdirSync, readdirSync, readFileSync, statSync, writeFileSync } from "node:fs";
import { basename, dirname, join, relative, resolve } from "node:path";
import { BundleScript, getWorkspaceRoot, ScriptRouter, runBundleScriptMain, runCargoLint, runCargoTestBudgeted, resolveTestLevel } from "../../../../🛍️products/🦑️repo/🔨️modules/📚️lib/⚡️implementations/🟦️typescript/📦️index.ts";
...
function emitRustManifest(doc: ManifestDocument): string {
  ...
  let out = `// Generated from ${doc.id}.manifest.json\n\n${serdeImport}use crate::graph::manifest::Manifest;\n\n`;
  out += families;
  ...
  run(): void {
    ...
    for (const path of files) {
      ...
      if (doc.schema !== "manifest") {
        console.log(`[framework-math] skip ${relative(root, path)} (${doc.schema})`);
        continue;
      }
      console.log(`[framework-math] ${relative(root, path)}`);
      docs.push(doc);
    }
    if (docs.length === 0) {
    ...
    console.log(`[framework-math] wrote ${docs.length} manifests to ${relative(root, outDir)}`);
  }
}
