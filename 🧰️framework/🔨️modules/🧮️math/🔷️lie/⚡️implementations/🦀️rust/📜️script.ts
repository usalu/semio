#!/usr/bin/env bun
/** 🌀️ `@semio-tech/math-lie` — Lie groups for rigid motion: quaternions, SO(3)/SE(3)/Sim(3) exp-log maps, Jacobians, Umeyama alignment and pose interpolation. */
...
class TestScript extends BundleScript {
  run(segments: string[]): void {
    ...
    runCargoTestBudgeted(["mathematical_lie"], join(this.root, "rs"), rest);
  }
}

class LintScript extends BundleScript {
  run(): void {
    runCargoLint(["mathematical_lie"], join(this.root, "rs"));
  }
}
