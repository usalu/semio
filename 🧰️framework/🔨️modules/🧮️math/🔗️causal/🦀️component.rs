...
//!
//! Variable indices are shared across [`CausalDag`]/[`Cpdag`] and `crate::tabular::Table`
...
use crate::probability::Continuous;
...
pub enum CausalError {
    ...
    #[error(transparent)]
    Stats(#[from] crate::statistics::StatisticsError),
    #[error(transparent)]
...
pub fn test_implied_independencies(dag: &CausalDag, data: &crate::tabular::Table, test: &dyn CiTest) -> Result<Vec<(CiStatement, crate::statistics::TestResult)>, CausalError> {
...
pub trait CiTest {
    fn test(&self, data: &crate::tabular::Table, x: usize, y: usize, z: &[usize]) -> Result<crate::statistics::TestResult, CausalError>;
}
...
impl FisherZ {
    ...
    pub fn for_table(data: &crate::tabular::Table, columns: &[usize]) -> Result<Self, CausalError> {
        let (corr, n) = crate::statistics::correlation_from_table(data, columns)?;
        Ok(Self { corr, n })
    }
}

impl CiTest for FisherZ {
    fn test(&self, _data: &crate::tabular::Table, x: usize, y: usize, z: &[usize]) -> Result<crate::statistics::TestResult, CausalError> {
        Ok(crate::statistics::fisher_z_test(&self.corr, x, y, z, self.n)?)
    }
}
...
impl CiTest for GSquared {
    fn test(&self, data: &crate::tabular::Table, x: usize, y: usize, z: &[usize]) -> Result<crate::statistics::TestResult, CausalError> {
        ...
        let given_cols: Vec<&crate::tabular::CategoricalColumn> = z.iter().map(|&zi| data.categorical(zi)).collect::<Result<_, _>>()?;
        ...
        Ok(crate::statistics::g2_ci_test(cat_x.codes(), cat_y.codes(), &given_codes, (cat_x.n_levels(), cat_y.n_levels(), &given_levels))?)
    }
}
// #endregion 🔖️CiTest
...
pub fn pc_stable(data: &crate::tabular::Table, test: &dyn CiTest, opts: PcOptions) -> Result<PcResult, CausalError> {
    let n = data.n_cols();
...
pub fn local_bic(data: &crate::tabular::Table, node: usize, parents: &[usize]) -> Result<f64, CausalError> {
    let y = data.continuous(node)?;
    let n = y.len();
    let p = parents.len();
    if parents.is_empty() {
        let m = crate::statistics::mean(y)?;
        let ss: f64 = y.iter().map(|v| (v - m).powi(2)).sum();
        let sigma2 = (ss / n as f64).max(1e-12);
        return Ok(-0.5 * n as f64 * sigma2.ln() - 0.5 * (n as f64).ln());
    }
    let mut design = crate::algebra::MatD::zeros(n, p);
    ...
    let fit = crate::statistics::ols(&design, y, true)?;
...
pub fn dag_bic(data: &crate::tabular::Table, dag: &CausalDag) -> Result<f64, CausalError> {
...
pub struct LingamResult {
    pub order: Vec<usize>,
    pub weights: crate::algebra::MatD,
    pub dag: CausalDag,
}
...
pub fn direct_lingam(data: &crate::tabular::Table, prune_alpha: f64) -> Result<LingamResult, CausalError> {
    let n = data.n_cols();
    ...
    let mut weights = crate::algebra::MatD::zeros(n, n);
    ...
    for (pos, &child) in order.iter().enumerate() {
        ...
        let mut design = crate::algebra::MatD::zeros(n_rows, candidate_parents.len());
        for (col, &parent) in candidate_parents.iter().enumerate() {
            for row in 0..n_rows {
                design.set(row, col, columns[parent][row]);
            }
        }
        let fit = crate::statistics::ols(&design, &columns[child], true)?;
        for (col, &parent) in candidate_parents.iter().enumerate() {
            let coeff = fit.coefficients[col + 1];
            let se = fit.std_errors[col + 1];
            if se > 1e-12 {
                let t_stat = coeff / se;
                let p_value = 2.0 * (1.0 - crate::probability::StudentT::new(fit.dof as f64)?.cdf(t_stat.abs()));
...
pub fn ges(data: &crate::tabular::Table) -> Result<Cpdag, CausalError> {
    let n = data.n_cols();
...
pub struct LinearGaussianScm {
    pub dag: CausalDag,
    pub weights: crate::algebra::MatD,
    pub intercepts: crate::algebra::VecD,
    pub noise_var: crate::algebra::VecD,
}
...
impl LinearGaussianScm {
    ...
    pub fn fit(dag: &CausalDag, data: &crate::tabular::Table) -> Result<Self, CausalError> {
        let n = dag.n();
        let mut weights = crate::algebra::MatD::zeros(n, n);
        let mut intercepts = crate::algebra::VecD::zeros(n);
        let mut noise_var = crate::algebra::VecD::zeros(n);
        for v in 0..n {
            let y = data.continuous(v)?;
            let parents = dag.parents(v);
            if parents.is_empty() {
                intercepts.set(v, crate::statistics::mean(y)?);
                noise_var.set(v, crate::statistics::variance(y).unwrap_or(1e-12).max(1e-12));
                continue;
            }
            let n_rows = y.len();
            let mut design = crate::algebra::MatD::zeros(n_rows, parents.len());
    ...
    pub fn simulate(&self, n_samples: usize, rng: &mut geometry::random::Rng) -> Result<crate::tabular::Table, CausalError> {
        ...
        for row in 0..n_samples {
            for &v in self.dag.topological_order() {
                ...
                val += crate::probability::Normal::new(0.0, sd)?.sample(rng);
                columns[v][row] = val;
            }
        }
        Ok(crate::tabular::Table::from_f64_columns(self.dag.names().to_vec(), columns)?)
    }
    ...
    pub fn mean(&self) -> crate::algebra::VecD {
        let mut mu = crate::algebra::VecD::zeros(self.dag.n());
        for &v in self.dag.topological_order() {
            let mut val = self.intercepts.get(v);
            for &p in self.dag.parents(v) {
    ...
    pub fn implied_covariance(&self) -> Result<crate::algebra::MatD, CausalError> {
        let n = self.dag.n();
        let mut i_minus_b = crate::algebra::MatD::identity(n);
        for v in 0..n {
            for &p in self.dag.parents(v) {
                i_minus_b.set(v, p, -self.weights.get(v, p));
            }
        }
        let inv = crate::statistics::invert(&i_minus_b)?;
        let mut d = crate::algebra::MatD::zeros(n, n);
        for v in 0..n {
    ...
    pub fn counterfactual(&self, observed: &[f64], interventions: &[(usize, f64)]) -> Result<crate::algebra::VecD, CausalError> {
        let n = self.dag.n();
        if observed.len() != n {
        ...
        let mut out = crate::algebra::VecD::zeros(n);
        for &v in self.dag.topological_order() {
...
impl DiscreteScm {
    ...
    pub fn fit(dag: &CausalDag, data: &crate::tabular::Table, pseudocount: f64) -> Result<Self, CausalError> {
        let n = dag.n();
        let columns: Vec<&crate::tabular::CategoricalColumn> = (0..n).map(|i| data.categorical(i)).collect::<Result<_, _>>()?;
        ...
        for v in 0..n {
            ...
            for row in 0..data.n_rows() {
                ...
                if own == crate::tabular::MISSING_CODE {
                    continue;
                }
                ...
                for &p in &parents {
                    ...
                    if code == crate::tabular::MISSING_CODE {
                        missing_parent = true;
                        break;
                    }
    ...
    pub fn simulate(&self, n_samples: usize, rng: &mut geometry::random::Rng) -> Result<crate::tabular::Table, CausalError> {
        ...
        Ok(crate::tabular::Table::from_categorical_columns(names, columns)?)
    }
...
fn wrap_estimate(point: f64, data: &crate::tabular::Table, opts: &EstimationOptions, point_fn: &dyn Fn(&crate::tabular::Table) -> Result<f64, CausalError>) -> EffectEstimate {
...
fn naive_difference_point(data: &crate::tabular::Table, treatment: usize, outcome: usize) -> Result<f64, CausalError> {
...
pub fn naive_difference(data: &crate::tabular::Table, treatment: usize, outcome: usize, opts: &EstimationOptions) -> Result<EffectEstimate, CausalError> {
...
fn design_with_treatment(data: &crate::tabular::Table, treatment: usize, covariates: &[usize], treatment_value: Option<f64>) -> Result<crate::algebra::MatD, CausalError> {
    let n = data.n_rows();
    let mut design = crate::algebra::MatD::zeros(n, covariates.len() + 1);
    let t = data.continuous(treatment)?;
    for row in 0..n {
...
fn g_formula_point(data: &crate::tabular::Table, treatment: usize, outcome: usize, covariates: &[usize]) -> Result<f64, CausalError> {
    let y = data.continuous(outcome)?;
    let design = design_with_treatment(data, treatment, covariates, None)?;
    let fit = crate::statistics::ols(&design, y, true)?;
    let predict_mean = |design: &crate::algebra::MatD| -> f64 {
        let n = design.rows;
        let p = design.cols;
...
pub fn g_formula_ate(data: &crate::tabular::Table, treatment: usize, outcome: usize, covariates: &[usize], opts: &EstimationOptions) -> Result<EffectEstimate, CausalError> {
...
#[allow(clippy::needless_range_loop, reason = "row indexes both the MatD design matrix by (row, col) and the values slice — enumerate() would only remove the values index")]
fn ipw_point(data: &crate::tabular::Table, treatment: usize, outcome: usize, covariates: &[usize]) -> Result<f64, CausalError> {
    const EPS: f64 = 1e-6;
    let n = data.n_rows();
    let mut design = crate::algebra::MatD::zeros(n, covariates.len());
    for (col, &c) in covariates.iter().enumerate() {
        let values = data.continuous(c)?;
        for row in 0..n {
            design.set(row, col, values[row]);
    ...
    let t = data.continuous(treatment)?;
    let fit = crate::statistics::logistic(&design, t, true)?;
    let propensity = crate::statistics::logistic_predict(&fit, &design, true)?;
    let y = data.continuous(outcome)?;
...
pub fn ipw_ate(data: &crate::tabular::Table, treatment: usize, outcome: usize, covariates: &[usize], opts: &EstimationOptions) -> Result<EffectEstimate, CausalError> {
...
mod tests {
    ...
    fn linear_chain_scm() -> LinearGaussianScm {
        ...
        let mut weights = crate::algebra::MatD::zeros(3, 3);
        weights.set(1, 0, 2.0);
        weights.set(2, 1, 1.5);
        LinearGaussianScm { dag, weights, intercepts: crate::algebra::VecD::from_vec(vec![0.0, 0.0, 0.0]), noise_var: crate::algebra::VecD::from_vec(vec![1.0, 0.25, 0.25]) }
    }
    ...
    fn local_bic_prefers_the_true_parent_set() {
        ...
        let unrelated: Vec<f64> = (0..data.n_rows()).map(|_| crate::probability::Normal::STANDARD.sample(&mut rng)).collect();
    ...
    fn confounded_dataset(true_ate: f64, n: usize, seed: u64) -> crate::tabular::Table {
        ...
        crate::tabular::Table::from_f64_columns(vec!["z".into(), "t".into(), "y".into()], vec![z, t, y]).unwrap()
    }
