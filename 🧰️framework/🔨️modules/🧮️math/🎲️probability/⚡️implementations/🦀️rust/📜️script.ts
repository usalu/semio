#!/usr/bin/env bun
/** 🎲️ `@semio-tech/math-probability` — special functions and probability distributions with cdf/quantile/sampling. `bun ./📜️script.ts <test|lint>`. */
...
class TestScript extends BundleScript {
  run(segments: string[]): void {
    ...
    runCargoTestBudgeted(["mathematical_probability"], join(this.root, "rs"), rest);
  }
}

class LintScript extends BundleScript {
  run(): void {
    runCargoLint(["mathematical_probability"], join(this.root, "rs"));
  }
}
