#!/usr/bin/env bun
/** 🌫️ `@semio-tech/math-fuzzy` — advanced fuzzy logic: sets, inference, learning, clustering, and multicriteria decision-making. `bun ./📜️script.ts <test|lint>`. */
...
class TestScript extends BundleScript {
  run(segments: string[]): void {
    ...
    runCargoTestBudgeted(["mathematical_fuzzy"], join(this.root, "rs"), rest);
  }
}

class LintScript extends BundleScript {
  run(): void {
    runCargoLint(["mathematical_fuzzy"], join(this.root, "rs"));
  }
}
