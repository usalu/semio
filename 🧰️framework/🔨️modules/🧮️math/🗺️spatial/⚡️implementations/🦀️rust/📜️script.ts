#!/usr/bin/env bun
/** 🗺️ `@semio-tech/math-spatial` — Spatial acceleration structures: const-generic k-d trees, hashed voxel grids and point octrees for nearest-neighbour and range queries. */
...
class TestScript extends BundleScript {
  run(segments: string[]): void {
    ...
    runCargoTestBudgeted(["mathematical_spatial"], join(this.root, "rs"), rest);
  }
}

class LintScript extends BundleScript {
  run(): void {
    runCargoLint(["mathematical_spatial"], join(this.root, "rs"));
  }
}
