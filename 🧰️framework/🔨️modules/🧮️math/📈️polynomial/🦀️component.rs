//! 📈️ Generic univariate and multivariate polynomials over the `crate::number` algebraic trait
...
// #region 🔖️Univariate
pub mod univariate {
    //! 📈️ Dense univariate polynomials, generic over the `crate::number` ring hierarchy: arithmetic,
    //! pseudo-division, resultants/discriminants, GCD, and (over a field) squarefree decomposition and
    //! Newton interpolation.

    use crate::number::{CommutativeRing, Field, GcdDomain, IntegralDomain, Ring};
    ...
    impl<C: IntegralDomain> PolyU<C> {
        ...
        /// (rather than depending on `crate::algebra`) to keep `crate::polynomial` free of a
...
pub mod multivariate {
    //! 🕸️ Sparse multivariate polynomials over the `crate::number` ring hierarchy: monomial orders,
    //! multivariate division, and Buchberger's Groebner-basis algorithm.

    use crate::number::{Field, Ring};
...
pub mod roots {
    ...
    fn to_squarefree_integer(f: &PolyU<Integer>) -> PolyU<Integer> {
        ...
        let denom_lcm = result.coeffs().iter().fold(crate::number::Natural::one(), |acc, c| acc.mul(c.denom()).div_rem(&acc.gcd(c.denom())).0);
...
pub mod algebraic {
    ...
    impl AlgebraicReal {
        ...
        pub fn scale_rational(&self, r: &Rational) -> Option<Self> {
            ...
            let denom_lcm = rational_poly.coeffs().iter().fold(crate::number::Natural::one(), |acc, c| {
