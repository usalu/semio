...
pub mod expr {
    ...
    use crate::number::{Integer, Rational};
    use std::rc::Rc;
...
mod canon {
    ...
    use crate::cas::fnkind::FnKind;
    use crate::number::{primes, Integer, Natural, Rational};
...
pub mod assume {
    ...
    pub struct Assumptions {
        facts: Vec<(String, RelationalOperator, crate::number::Rational)>,
    }
    ...
    impl Assumptions {
        ...
        pub fn assume_bound(&mut self, symbol: &str, operator: RelationalOperator, bound: crate::number::Rational) {
            ...
            Kind::Mul(factors) => {
                if let Some(Kind::Integer(n)) = factors.first().map(|f| f.kind()) {
                    if n.is_negative() {
                        ...
                        if *n != crate::number::Integer::from_i64(-1) {
    ...
    fn write_mul(factors: &[Expr], out: &mut String) {
        ...
        for f in factors {
            ...
            if let Kind::Rational(r) = f.kind() {
                if r.numer().abs_integer() != crate::number::Integer::one() {
                    numer.push(Expr::from(r.numer().abs_integer()));
                }
                if r.numer().is_negative() {
                    numer.insert(0, Expr::integer(-1));
                }
                denom.push(Expr::from(crate::number::Integer::from_natural(r.denom().clone())));
                continue;
            }
            numer.push(f.clone());
        }
...
pub mod simplify {
    ...
    /// linear system (via `crate::algebra`'s exact `MatG::solve`) for each factor's numerator
    ...
    fn apart_univariate(num: &PolyU<Rational>, den: &PolyU<Rational>, x: &Expr) -> Expr {
        ...
        let b = crate::algebra::VecG::from_vec(b_data);
...
pub mod series {
    ...
    mod tests {
        ...
        fn taylor_series_of_exp_matches_known_coefficients() {
            ...
            assert_eq!(s.coeffs[2], Expr::from(crate::number::Rational::from_i64(1, 2).unwrap()));
            assert_eq!(s.coeffs[3], Expr::from(crate::number::Rational::from_i64(1, 6).unwrap()));
        }
...
pub mod limits {
    ...
    mod tests {
        ...
        fn one_plus_one_over_n_to_the_n_via_lhopital_on_log_form() {
            ...
            let e = num * Expr::pow(den, Expr::integer(-1));
            assert_eq!(limit(&e, &x, &Expr::integer(0), Direction::Both), Some(Expr::from(crate::number::Rational::from_i64(1, 2).unwrap())));
        }
    }
    // #endregion 🔖️Tests
}
...
pub mod matrix {
    ...
    //! `crate::algebra`'s exact `MatG<Rational>` for rank/nullspace/RREF, which do need a real field
    ...
    impl SymMatrix {
        ...
        /// `crate::algebra`-backed numeric paths below.
        fn is_numeric(&self) -> bool {
            self.data.iter().all(|e| matches!(e.kind(), Kind::Integer(_) | Kind::Rational(_)))
        }

        fn to_numeric(&self) -> Option<crate::algebra::MatG<Rational>> {
            if !self.is_numeric() {
                return None;
            }
            let rows: Vec<Vec<Rational>> = self
                ...
                .collect();
            Some(crate::algebra::MatG::from_rows(rows))
        }

        fn from_numeric(m: &crate::algebra::MatG<Rational>) -> Self {
        ...
        pub fn solve_numeric(&self, b: &[Expr]) -> Option<Vec<Expr>> {
            ...
            let b_rat: Vec<Rational> = b
                ...
                .collect();
            let v = crate::algebra::VecG::from_vec(b_rat);
            let x = m.solve(&v)?;
