#!/usr/bin/env bun
/** 🪞️ `@semio-tech/math-graph-similarity` — Graph and node similarity: graph edit distance, SimRank, and link-prediction indices. */
...
class TestScript extends BundleScript {
  run(segments: string[]): void {
    ...
    runCargoTestBudgeted(["mathematical_graph_similarity"], join(this.root, "rs"), rest);
  }
}

class LintScript extends BundleScript {
  run(): void {
    runCargoLint(["mathematical_graph_similarity"], join(this.root, "rs"));
  }
}
