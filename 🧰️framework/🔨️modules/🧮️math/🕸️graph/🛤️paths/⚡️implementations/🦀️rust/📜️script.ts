#!/usr/bin/env bun
/** 🛤️ `@semio-tech/math-graph-paths` — Shortest-path algorithms (Dijkstra, Bellman-Ford, Johnson, Floyd-Warshall, A*), simple paths, and walks. */
...
class TestScript extends BundleScript {
  run(segments: string[]): void {
    ...
    runCargoTestBudgeted(["mathematical_graph_paths"], join(this.root, "rs"), rest);
  }
}

class LintScript extends BundleScript {
  run(): void {
    runCargoLint(["mathematical_graph_paths"], join(this.root, "rs"));
  }
}
