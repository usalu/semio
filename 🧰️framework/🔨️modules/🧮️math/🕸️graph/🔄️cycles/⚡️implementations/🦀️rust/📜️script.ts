#!/usr/bin/env bun
/** 🔄️ `@semio-tech/math-graph-cycles` — Cycle detection and enumeration, cycle bases, girth, and Eulerian circuits/paths. */
...
class TestScript extends BundleScript {
  run(segments: string[]): void {
    ...
    runCargoTestBudgeted(["mathematical_graph_cycles"], join(this.root, "rs"), rest);
  }
}

class LintScript extends BundleScript {
  run(): void {
    runCargoLint(["mathematical_graph_cycles"], join(this.root, "rs"));
  }
}
