#!/usr/bin/env bun
/** 🌊️ `@semio-tech/math-graph-flow` — Maximum-flow, minimum-cut, and minimum-cost-flow algorithms over a shared residual-network representation. */
...
class TestScript extends BundleScript {
  run(segments: string[]): void {
    ...
    runCargoTestBudgeted(["mathematical_graph_flow"], join(this.root, "rs"), rest);
  }
}

class LintScript extends BundleScript {
  run(): void {
    runCargoLint(["mathematical_graph_flow"], join(this.root, "rs"));
  }
}
