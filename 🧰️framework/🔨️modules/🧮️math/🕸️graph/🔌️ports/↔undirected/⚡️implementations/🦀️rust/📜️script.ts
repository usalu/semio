#!/usr/bin/env bun
/** 🔀️ `@semio-tech/math-graph-port-undirected` — The undirected port (multi-edge) graph family — NetworkX `MultiGraph` parity facade. */
...
class TestScript extends BundleScript {
  run(segments: string[]): void {
    ...
    runCargoTestBudgeted(["mathematical_graph_port_undirected"], join(this.root, "rs"), rest);
  }
}

class LintScript extends BundleScript {
  run(): void {
    runCargoLint(["mathematical_graph_port_undirected"], join(this.root, "rs"));
  }
}
