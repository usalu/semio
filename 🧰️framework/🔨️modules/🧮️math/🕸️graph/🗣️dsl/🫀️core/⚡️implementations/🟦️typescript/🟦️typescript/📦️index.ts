/** 🃏️ Jack query execution for board-style graph fixtures in TypeScript. */
...
/** @emoji 🩹️ Unified wire syntax: `key=value` (never `key: value`), double-quoted strings with
...
// 🩹️ Priority order mirrors the Rust reference (`mathematical_graph_dsl::wire::split_endpoint`,
...
/** @emoji 🔌️ Render a DAG fixture JSON document as wire-literal compiled text. */
...
/** @emoji 📍️ Highlight spans for a node id across wire-literal node declarations and edge endpoints. */
...
/** @emoji 🎯️ Resolve the wire node id at a text offset (cursor must sit on the id token before `:`). */
...
/** @emoji 📍️ Highlight spans for a port token on wire edge endpoints. */
...
/** @emoji 📍️ Highlight spans for a wire hover target (node id or channel port). */
...
/** @emoji 🎯️ Resolve node or port endpoint at a wire text offset. */
