#!/usr/bin/env bun
/** 🕸️ `@semio-tech/math-graph-core` — pure graph foundation: storage, view traits, and shared algorithm infra. */
...
class TestScript extends BundleScript {
  run(segments: string[]): void {
    ...
    runCargoTestBudgeted(["mathematical_graph"], join(this.root, "rs"), rest);
  }
}

class LintScript extends BundleScript {
  run(): void {
    runCargoLint(["mathematical_graph"], join(this.root, "rs"));
  }
}
