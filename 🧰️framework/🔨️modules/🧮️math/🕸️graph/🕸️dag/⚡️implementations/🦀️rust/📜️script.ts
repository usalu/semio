#!/usr/bin/env bun
/** 🔺️ `@semio-tech/math-graph-dag` — Directed acyclic graph algorithms: topological sorts, transitive closure/reduction, dominance, lowest common ancestors, d-separation. */
...
class TestScript extends BundleScript {
  run(segments: string[]): void {
    ...
    runCargoTestBudgeted(["mathematical_graph_dag"], join(this.root, "rs"), rest);
  }
}

class LintScript extends BundleScript {
  run(): void {
    runCargoLint(["mathematical_graph_dag"], join(this.root, "rs"));
  }
}
