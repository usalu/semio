//! 🔧️ Includes codegen output from `bun ./📜️script.ts generate`.
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let generated = manifest_dir.join("../../../../🕸️graph/🛂️manifest/⚡️implementations/🦀️rust/🤖️generated/🦀️registry.rs");
    println!("cargo:rerun-if-changed={}", generated.display());
    println!("cargo:rerun-if-changed={}", manifest_dir.join("../../../../🕸️graph/🛂️manifest/⚡️implementations/🦀️rust/📜️script.ts").display());
    for entry in fs::read_dir(manifest_dir.join("../../..")).into_iter().flatten().flatten() {
        let path = entry.path();
        if path.file_name().and_then(|n| n.to_str()) == Some("🛂️manifest") {
