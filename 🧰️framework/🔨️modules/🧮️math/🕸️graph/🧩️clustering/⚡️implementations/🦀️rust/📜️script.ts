#!/usr/bin/env bun
/** 🔗️ `@semio-tech/math-graph-clustering` — Local clustering structure: triangles, transitivity, square clustering, degree assortativity and mixing. */
...
class TestScript extends BundleScript {
  run(segments: string[]): void {
    ...
    runCargoTestBudgeted(["mathematical_graph_clustering"], join(this.root, "rs"), rest);
  }
}

class LintScript extends BundleScript {
  run(): void {
    runCargoLint(["mathematical_graph_clustering"], join(this.root, "rs"));
  }
}
