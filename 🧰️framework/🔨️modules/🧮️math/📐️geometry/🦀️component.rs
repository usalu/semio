...
macro_rules! with_shape_ref {
    ($shape:expr, |$s:ident| $body:expr) => {
        match $shape {
            $crate::geometry::ShapeRef::Rect($s) => $body,
            $crate::geometry::ShapeRef::RoundedRect($s) => $body,
            $crate::geometry::ShapeRef::Circle($s) => $body,
            $crate::geometry::ShapeRef::Line($s) => $body,
            $crate::geometry::ShapeRef::Arc($s) => $body,
            $crate::geometry::ShapeRef::CubicBez($s) => $body,
            $crate::geometry::ShapeRef::BezPath($s) => $body,
        }
    };
}
...
// #region 🔖️GeomSel
pub mod geom_sel {
    use crate::geometry::{CubicBez, Point};
