...
//! `crate::search`: a seed that depends only on *where* a chunk is and *what* model it's made
//! from, never on solve order — so re-solving an already-committed chunk (e.g. after evicting it
//! from a cache) reproduces byte-identical content, exactly like `crate::repair`'s halo re-solve
...
/// model — the caller (e.g. via `crate::repair`) may need to widen the halo, regenerate the
