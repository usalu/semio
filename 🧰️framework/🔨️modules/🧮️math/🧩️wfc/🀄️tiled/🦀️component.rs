//! 🧱️ Explicit tiled model construction: one authored tile ↔ one pattern (until [`crate::wfc::symmetry`]
//! expands orbits in a later phase), with allow/deny pair lists and eagerly-compiled predicates.
//! A thin `TileId`-facing wrapper over [`crate::wfc::model::ModelBuilder`]'s `PatternId`-facing API.

use crate::wfc::error::ModelError;
use crate::wfc::ids::{RelationId, TileId};
use crate::wfc::model::{CompiledModel, ModelBuilder};
...
pub struct TiledModelBuilder {
    ...
    tile_pattern: Vec<crate::wfc::ids::PatternId>,
}
...
impl TiledModelBuilder {
    ...
    pub fn pattern_of(&self, tile: TileId) -> crate::wfc::ids::PatternId {
