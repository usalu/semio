...
//! [`crate::wfc::grid2d::declare_stencil_relations`] so an extracted model's relations line up exactly
//! with a [`crate::wfc::grid2d::Grid2dTopology`] built with the same (von Neumann) stencil.

use crate::wfc::grid2d::{declare_stencil_relations, Stencil2d};
use crate::wfc::ids::{PatternId, TileId};
use crate::wfc::model::{CompiledModel, ModelBuilder};
use crate::wfc::symmetry::SymmetryGroup2d;
...
pub fn extract_2d(samples: &[Sample2d], cfg: &Extract2dConfig) -> Result<ExtractedModel2d, crate::wfc::error::ModelError> {
...
mod tests {
    ...
    fn periodic_sample_solves_on_a_same_size_wrapped_grid() {
        ...
        // unsolvable.
        use crate::wfc::grid2d::{Boundary, Grid2dTopology};
        use crate::wfc::solver_grid2d::Grid2dSolverBuilder;
        ...
        let relations = Stencil2d::VonNeumann.offsets().iter().enumerate().map(|(i, _)| crate::wfc::ids::RelationId(i as u32)).collect::<Vec<_>>();
        ...
        assert!(matches!(outcome, crate::wfc::outcome::SolveOutcome::Solved(_)), "extracted model must remain solvable on a same-size wrapped grid");
    }
