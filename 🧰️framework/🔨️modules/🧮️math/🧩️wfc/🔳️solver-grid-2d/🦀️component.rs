//! 🧱️ `Grid2dSolver`: dense 2D grid solving on top of [`crate::wfc::grid2d`] and the shared kernel.
//! Masked-out cells and [`Boundary::FixedOutside`] edges are folded into ordinary domain overrides
//! and fixed pins before delegating to the same generic [`crate::wfc::search::solve`] every solver uses.
...
use crate::wfc::grid2d::Grid2dTopology;
...
impl Grid2dSolver {
    ...
    /// derived from `world_seed` plus `(chunk_x, chunk_y)` — see [`crate::wfc::chunk`]'s docs for the
    /// exact contract. Re-solving the same chunk coordinate later reproduces identical content.
    pub fn solve_chunk(&self, world_seed: u64, chunk_x: i64, chunk_y: i64, seam_fixed: &[(crate::wfc::ids::NodeId, PatternId)]) -> SolveOutcome {
    ...
    /// Patterns with no tile provenance (e.g. built directly via [`crate::wfc::model::ModelBuilder`])
...
mod tests {
    use super::*;
    use crate::wfc::grid2d::{declare_stencil_relations_tiled, Boundary, Stencil2d};
    ...
    fn graph_vs_grid2d_strict_equivalence_von_neumann_open() {
        ...
        for y in 0..height {
            for x in 0..width {
                let idx = |x: usize, y: usize| crate::wfc::ids::NodeId::from_index(y * width + x);
                if x + 1 < width {
        ...
        let mut gb = crate::wfc::topology::GraphTopologyBuilder::new(width * height);
