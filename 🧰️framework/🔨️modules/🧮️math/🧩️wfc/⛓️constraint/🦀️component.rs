...
//! twice, once in `crate::wfc::search`'s trail and once in `crate::wfc::prop_ac4`'s counters): a constraint
//! only ever (1) restricts initial domains once, before search starts (no rollback needed — the
//! same treatment `crate::wfc::search::solve` already gives fixed pins and per-node domain overrides),
//! and (2) validates a *complete* (all-singleton) candidate assignment, rejecting it — which
//! `crate::wfc::search`'s existing, proven `backtrack_and_repair` machinery turns into an ordinary
...
use crate::wfc::error::ConstraintError;
use crate::wfc::ids::{NodeId, PatternId, RegionId};
...
pub(crate) fn build_adjacency_view<T: crate::wfc::topology::Topology>(topo: &T) -> AdjacencyView {
...
/// 🧷️ A solver's constraints plus the adjacency view they read — bundled so `crate::wfc::search`'s
