//! 🧊️ `Grid3dSolver`: dense 3D grid solving. Exactly [`crate::wfc::solver_grid2d`]'s design extended to
//! a third axis — masked-out voxels and [`crate::wfc::grid2d::Boundary::FixedOutside`] faces fold into
//! ordinary domain overrides and fixed pins before delegating to the same generic kernel.

use crate::wfc::bitset::PatternSet;
use crate::wfc::constraint::{build_adjacency_view, AdjacencyView, Constraint, ConstraintSet};
use crate::wfc::error::SolveError;
use crate::wfc::grid3d::Grid3dTopology;
use crate::wfc::ids::PatternId;
use crate::wfc::model::CompiledModel;
use crate::wfc::outcome::{Solution, SolveOutcome};
use crate::wfc::search::{self, CancelToken, SearchConfig};
...
pub struct Grid3dSolverBuilder {
    ...
    fixed: Vec<(crate::wfc::ids::NodeId, PatternId)>,
    config: SearchConfig,
...
impl Grid3dSolverBuilder {
    ...
    /// 🏗️ Adds a global constraint. See [`crate::wfc::constraint::Constraint`]'s docs for exactly when
...
impl Grid3dSolver {
    ...
    pub fn decode_tiles(&self, solution: &Solution) -> Vec<Option<crate::wfc::ids::TileId>> {
...
mod tests {
    use super::*;
    use crate::wfc::grid2d::Boundary;
    use crate::wfc::grid3d::{declare_stencil_relations_3d_tiled, Stencil3d};
    ...
    fn graph_vs_grid3d_strict_equivalence_face6_open() {
        ...
        let idx = |x: usize, y: usize, z: usize| crate::wfc::ids::NodeId::from_index(z * width * height + y * width + x);
        ...
        let mut gb = crate::wfc::topology::GraphTopologyBuilder::new(width * height * depth);
        ...
        for seed in 0..10u64 {
            let mut graph_solver = crate::wfc::solver_graph::GraphSolverBuilder::new(model.clone(), graph_topo.clone()).config(config).build().unwrap();
