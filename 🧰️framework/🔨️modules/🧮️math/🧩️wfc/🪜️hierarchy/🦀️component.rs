...
//! with `crate::wfc::search`'s trail (essentially a `crate::wfc::repair`-style halo re-solve scoped to one
...
/// a single splitmix64-style mixing step (matching `crate::wfc::chunk::chunk_seed`'s approach), so the
