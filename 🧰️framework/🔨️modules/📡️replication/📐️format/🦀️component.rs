...
impl<'a> RecordFrame<'a> {
    ...
    /// keyed by `crate::frame_codec_id(self.flags)`, targeting `self.raw_len` bytes.
...
fn prepare_payload(codec: crate::codec::ids::CodecId, payload: &[u8]) -> Result<PreparedPayload<'_>, ProtocolError> {
    match codec.0 {
...
impl<S: PackSink> SprWriter<S> {
    ...
    pub fn write_record(&mut self, kind: u8, critical: bool, payload: &[u8], codec: crate::codec::ids::CodecId) -> Result<u64, ProtocolError> {
    ...
    pub fn commit(&mut self) -> Result<u64, ProtocolError> {
        ...
        encode_frame_into(&mut self.scratch, crate::REC_COMMIT, flags, None, &payload)?;
        self.sink.write_all(&self.scratch)?;
...
mod tests {
    ...
    fn build_small_file() -> Vec<u8> {
        ...
        writer.write_record(crate::REC_DOC, true, b"doc-payload", crate::codec::ids::CodecId(0)).unwrap();
        writer.write_record(crate::REC_EDIT, true, b"edit-payload", crate::codec::ids::CodecId(0)).unwrap();
    ...
    fn cursor_surfaces_unrecognized_extension_kind_transparently() {
        ...
        let options = WriteOptions::default();
        let mut writer = SprWriter::begin(Vec::new(), &options).unwrap();
        writer.write_record(0x50, false, b"extension-payload", crate::codec::ids::CodecId(0)).unwrap();
        writer.write_record(crate::REC_DOC, true, b"doc-payload", crate::codec::ids::CodecId(0)).unwrap();
        let bytes = writer.into_sink();
    ...
    fn compressed_frame_round_trips_stored_and_raw_len() {
        ...
        writer.write_record(crate::REC_DOC, true, payload, crate::codec::ids::CodecId(1)).unwrap();
        let bytes = writer.into_sink();
    ...
    fn commit_chain_links_prev_commit_offset_across_multiple_commits() {
        let options = WriteOptions::default();
        let mut writer = SprWriter::begin(Vec::new(), &options).unwrap();
        writer.write_record(crate::REC_DOC, true, b"a", crate::codec::ids::CodecId(0)).unwrap();
        let commit_1_offset = writer.commit().unwrap();
        writer.write_record(crate::REC_EDIT, true, b"b", crate::codec::ids::CodecId(0)).unwrap();
    ...
    fn parse_commit_payload_is_public_for_downstream_frontier_summaries() {
        ...
        let bytes = build_small_file();
        let mut cursor = FrameCursor::new(&bytes, HEADER_SIZE as u64);
        let commit_frame = std::iter::from_fn(|| cursor.next_frame().transpose()).map(Result::unwrap).find(|frame| frame.kind == crate::REC_COMMIT).unwrap();
    ...
    fn recover_fast_path_walks_multi_commit_chain_to_genesis() {
        ...
        let mut writer = SprWriter::begin(Vec::new(), &options).unwrap();
        for i in 0..5u8 {
            writer.write_record(crate::REC_EDIT, true, &[i], crate::codec::ids::CodecId(0)).unwrap();
            writer.commit().unwrap();
        }
        let bytes = writer.into_sink();
        let report = recover(&bytes, &ProtocolLimits::default(), RecoveryMode::LastCommit).unwrap();
    ...
    fn recover_forward_scan_truncates_to_last_commit_on_torn_tail() {
        ...
        writer.write_record(crate::REC_EDIT, true, b"uncommitted", crate::codec::ids::CodecId(0)).unwrap();
    ...
    fn recover_last_valid_record_mode_trusts_past_the_last_commit() {
        ...
        writer.write_record(crate::REC_DOC, true, b"a", crate::codec::ids::CodecId(0)).unwrap();
        writer.commit().unwrap();
        writer.write_record(crate::REC_EDIT, true, b"uncommitted-but-well-formed", crate::codec::ids::CodecId(0)).unwrap();
        let bytes = writer.into_sink();
    ...
    fn recover_never_panics_on_truncation_at_every_byte() {
        ...
        let mut writer = SprWriter::begin(Vec::new(), &options).unwrap();
        for i in 0..4u8 {
            writer.write_record(crate::REC_EDIT, true, &[i; 5], crate::codec::ids::CodecId(0)).unwrap();
        }
        writer.commit().unwrap();
        writer.write_record(crate::REC_EDIT, true, b"tail-record-after-commit", crate::codec::ids::CodecId(0)).unwrap();
        let bytes = writer.into_sink();
    ...
    fn position_tracks_sink_length_across_writes() {
        ...
        writer.write_record(crate::REC_DOC, true, b"x", crate::codec::ids::CodecId(0)).unwrap();
