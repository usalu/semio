    ...
    match tag {
        ...
        2 => {
            let prev = match prev_epoch_ms {
                Some(v) => v,
                None => return Err(malformed("timestamp", input.position() as u64, "tag 2 delta with no previous timestamp in scope")),
            };
            let delta = input.read_varint_i64()?;
            let epoch_ms = match prev.checked_add(delta) {
                Some(v) => v,
                None => return Err(malformed("timestamp", input.position() as u64, "epoch_ms overflow")),
            };
            Ok((format_rfc3339_ms(epoch_ms), Some(epoch_ms)))
        }
