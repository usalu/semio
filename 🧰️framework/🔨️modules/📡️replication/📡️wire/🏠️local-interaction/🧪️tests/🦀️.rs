...
fn local_interaction_language_neutral_restore_parity() {
    ...
    for row in fixture["cases"].as_array().unwrap() {
        let before: LocalInteractionState = from_json(row["before"].clone());
        let current: LocalInteractionIdentity = from_json(row["current"].clone());
        let restore: LocalInteractionRestore = from_json(row["restore"].clone());
        let actual = restore.apply_cold(&before, &current);
        if row.get("error").is_some() { assert_eq!(actual, Err("stale-authority")); }
        else { assert_eq!(dsl_to_json(&crate::value::ToValue::to_value(&actual.unwrap())), row["expected"]); }
        assert_eq!(dsl_to_json(&crate::value::ToValue::to_value(&before)), row["before"]);
        assert_eq!(dsl_to_json(&crate::value::ToValue::to_value(&restore)), row["restore"]);
    }
}
...
fn local_interaction_wire_preserves_full_identity_and_u64() {
    ...
    let encoded = dsl_to_json(&crate::value::ToValue::to_value(&identity));
    assert_eq!(encoded["generation"], "18446744073709551615");
    assert_eq!(encoded["revision"].as_str().unwrap().len(), 64);
    assert_eq!(from_json::<LocalInteractionIdentity>(encoded.clone()), identity);
    for invalid in [serde_json::json!(9007199254740993u64), serde_json::json!("18446744073709551616"), serde_json::json!("01"), serde_json::json!("+1")] {
        ...
        assert!(<LocalInteractionIdentity as crate::value::FromValue>::from_value(json_to_dsl(value)).is_err());
    }
    let mut truncated = encoded; truncated["documentRevision"] = serde_json::json!("0000000000000000");
    assert!(<LocalInteractionIdentity as crate::value::FromValue>::from_value(json_to_dsl(truncated)).is_err());
}
...
fn local_interaction_nullable_fields_are_explicit_and_unknown_fields_fail() {
    ...
    assert!(<LocalInteractionDomainPatch as crate::value::FromValue>::from_value(json_to_dsl(complete.clone())).is_ok());
    for field in ["selection", "activeMode", "activeGranularity"] {
        let mut value = complete.clone(); value.as_object_mut().unwrap().remove(field);
        assert!(<LocalInteractionDomainPatch as crate::value::FromValue>::from_value(json_to_dsl(value)).is_err());
    }
    let mut extra = complete; extra["selectionJson"] = serde_json::json!("{}");
    assert!(<LocalInteractionDomainPatch as crate::value::FromValue>::from_value(json_to_dsl(extra)).is_err());
    assert!(<LocalInteractionState as crate::value::FromValue>::from_value(json_to_dsl(serde_json::json!({"selection": {}, "activeMode": {}, "activeGranularity": {}, "hover": {}}))).is_err());
}
