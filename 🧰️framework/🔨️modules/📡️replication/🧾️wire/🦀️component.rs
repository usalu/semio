...
pub async fn is_critical_kind(kind: u8) -> bool {
...
pub async fn frame_codec_id(flags: u8) -> u8 {
    (flags >> 2) & 0b111
}

/// @emoji 🏗️ Assembles a frame flags byte from its three logical fields.
pub async fn frame_flags(compressed: bool, critical: bool, codec: u8) -> u8 {
...
impl MergePolicy {
    ...
    pub async fn rejects(self, level: crate::diagnostic::Severity) -> bool {
    ...
    pub async fn from_u8(value: u8) -> Option<Self> {
        match value {
...
async fn kernel_artifact_schema_catalog() -> &'static Mutex<KernelArtifactSchemaCatalog> {
...
pub async fn register_kernel_artifact_schema_descriptor(descriptor: KernelArtifactSchemaDescriptor) {
    kernel_artifact_schema_catalog().await
        .lock()
...
pub async fn kernel_artifact_schema_descriptor_registered(id: &str) -> bool {
    kernel_artifact_schema_catalog().await
        .lock()
        .expect("kernel artifact schema catalog lock")
        .by_id
...
pub async fn kernel_artifact_schema_catalog_len() -> usize {
    kernel_artifact_schema_catalog().await
        .lock()
        .expect("kernel artifact schema catalog lock")
        .by_id
...
pub async fn with_kernel_artifact_schema_catalog<R>(visit: impl FnOnce(&[KernelArtifactSchemaDescriptor]) -> R) -> R {
    let guard = kernel_artifact_schema_catalog().await.lock().expect("kernel artifact schema catalog lock");
...
async fn kernel_artifact_inference_catalog() -> &'static Mutex<KernelArtifactInferenceCatalog> {
...
pub async fn register_kernel_artifact_inference_descriptor(descriptor: KernelArtifactInferenceDescriptor) {
    kernel_artifact_inference_catalog().await
        .lock()
...
pub async fn kernel_artifact_inference_descriptor_registered(id: &str) -> bool {
    kernel_artifact_inference_catalog().await
        .lock()
        .expect("kernel artifact inference catalog lock")
        .by_id
...
pub async fn kernel_artifact_inference_catalog_len() -> usize {
    kernel_artifact_inference_catalog().await
        .lock()
        .expect("kernel artifact inference catalog lock")
        .by_id
...
pub async fn with_kernel_artifact_inference_catalog<R>(visit: impl FnOnce(&[KernelArtifactInferenceDescriptor]) -> R) -> R {
    let guard = kernel_artifact_inference_catalog().await.lock().expect("kernel artifact inference catalog lock");
...
async fn kernel_app_schema_catalog() -> &'static Mutex<KernelAppSchemaCatalog> {
...
pub async fn register_kernel_app_schema_descriptor(descriptor: KernelAppSchemaDescriptor) {
    kernel_app_schema_catalog().await
        .lock()
...
pub async fn kernel_app_schema_descriptor_registered(id: &str) -> bool {
    kernel_app_schema_catalog().await
        .lock()
        .expect("kernel app schema catalog lock")
        .by_id
...
pub async fn kernel_app_schema_catalog_len() -> usize {
    kernel_app_schema_catalog().await
        .lock()
        .expect("kernel app schema catalog lock")
        .by_id
...
pub async fn with_kernel_app_schema_catalog<R>(visit: impl FnOnce(&[KernelAppSchemaDescriptor]) -> R) -> R {
    let guard = kernel_app_schema_catalog().await.lock().expect("kernel app schema catalog lock");
...
pub async fn write_varint_u64(out: &mut Vec<u8>, value: u64) {
    crate::codec::write_varint_u64(out, value).await;
}

pub async fn read_varint_u64(bytes: &[u8], pos: &mut usize) -> Result<u64, ProtocolError> {
    Ok(crate::codec::read_varint_u64(bytes, pos).await?)
}
...
pub async fn read_u8(bytes: &[u8], pos: &mut usize) -> Result<u8, ProtocolError> {
...
pub async fn write_str(out: &mut Vec<u8>, s: &str) {
    write_varint_u64(out, s.len() as u64).await;
    out.extend_from_slice(s.as_bytes());
}

pub async fn read_str(bytes: &[u8], pos: &mut usize) -> Result<String, ProtocolError> {
    let len = read_varint_u64(bytes, pos).await? as usize;
...
pub async fn write_bytes(out: &mut Vec<u8>, b: &[u8]) {
    write_varint_u64(out, b.len() as u64).await;
    out.extend_from_slice(b);
}

pub async fn read_bytes(bytes: &[u8], pos: &mut usize) -> Result<Vec<u8>, ProtocolError> {
    let len = read_varint_u64(bytes, pos).await? as usize;
...
pub async fn write_hash32(out: &mut Vec<u8>, h: &[u8; 32]) {
    out.extend_from_slice(h);
}

pub async fn read_hash32(bytes: &[u8], pos: &mut usize) -> Result<[u8; 32], ProtocolError> {
...
pub async fn write_bool(out: &mut Vec<u8>, value: bool) {
    out.push(if value { 1 } else { 0 });
}

pub async fn read_bool(bytes: &[u8], pos: &mut usize) -> Result<bool, ProtocolError> {
...
pub async fn write_f64(out: &mut Vec<u8>, value: f64) {
    out.extend_from_slice(&value.to_le_bytes());
}

pub async fn read_f64(bytes: &[u8], pos: &mut usize) -> Result<f64, ProtocolError> {
    let end = *pos + 8;
...
mod tests {
    ...
    async fn pack_error_converts_into_protocol_error_via_from() {
    ...
    //#region 🔖️Limits
    #[semio_framework_async_macros::async_test]
    async fn protocol_limits_default_matches_contract() {
    ...
    //#region 🔖️RecordKinds
    #[semio_framework_async_macros::async_test]
    async fn record_kind_constants_match_contract() {
    ...
    async fn is_critical_kind_matches_contract_set() {
        ...
        for kind in [REC_END, REC_FRONTIER, REC_PROJECTION, REC_INDEX, REC_SIGNATURE, REC_REDACTION, REC_UPCAST, REC_EPHEMERAL, REC_SEALED, REC_COMPACTION, REC_PADDING, 0x50] {
            assert!(!is_critical_kind(kind).await, "{kind:#x} should not be critical");
        }
    }
    //#endregion 🔖️RecordKinds
    ...
    //#region 🔖️Flags
    #[semio_framework_async_macros::async_test]
    async fn frame_flags_round_trips_codec_id() {
        for codec in 0u8..=7 {
            for compressed in [false, true] {
                for critical in [false, true] {
                    let flags = frame_flags(compressed, critical, codec).await;
                    assert_eq!(flags & FRAME_FLAG_COMPRESSED != 0, compressed);
                    assert_eq!(flags & FRAME_FLAG_CRITICAL != 0, critical);
                    assert_eq!(frame_codec_id(flags).await, codec);
                }
            }
        }
    }
    ...
    #[semio_framework_async_macros::async_test]
    async fn frame_codec_id_masks_to_three_bits() {
        assert_eq!(frame_codec_id(0b1111_1100).await, 0b111);
    }
    //#endregion 🔖️Flags
    ...
    mod scalars {
        ...
        #[semio_framework_async_macros::async_test]
        async fn timestamp_round_trips_canonical_utc_no_fraction() {
            let raw = "2024-01-15T10:30:00Z";
            let mut out = ByteWriter::new().await;
            let epoch = write_timestamp(&mut out, raw, None).await;
            assert!(epoch.is_some());
            let bytes = out.into_bytes().await;
            let mut reader = ByteReader::new(&bytes).await;
            let (decoded, epoch_back) = read_timestamp(&mut reader, None).await.unwrap();
            assert_eq!(decoded, raw);
            assert_eq!(epoch_back, epoch);
        }

        #[semio_framework_async_macros::async_test]
        async fn timestamp_round_trips_canonical_utc_with_fraction() {
            let raw = "2024-01-15T10:30:00.123Z";
            let mut out = ByteWriter::new().await;
            write_timestamp(&mut out, raw, None).await;
            let bytes = out.into_bytes().await;
            let mut reader = ByteReader::new(&bytes).await;
            let (decoded, _) = read_timestamp(&mut reader, None).await.unwrap();
            assert_eq!(decoded, raw);
        }

        #[semio_framework_async_macros::async_test]
        async fn timestamp_falls_back_to_raw_for_non_canonical_text() {
        ...
        #[semio_framework_async_macros::async_test]
        async fn timestamp_falls_back_to_raw_for_non_utc_offset() {
        ...
        #[semio_framework_async_macros::async_test]
        async fn timestamp_chain_uses_delta_tag_after_first_absolute() {
            let mut out = ByteWriter::new().await;
            let e1 = write_timestamp(&mut out, "2024-01-15T10:30:00Z", None).await.unwrap();
            let e2 = write_timestamp(&mut out, "2024-01-15T10:30:05Z", Some(e1)).await.unwrap();
            assert_eq!(e2 - e1, 5_000);
            let bytes = out.into_bytes().await;
            let mut reader = ByteReader::new(&bytes).await;
            let (d1, p1) = read_timestamp(&mut reader, None).await.unwrap();
            let (d2, p2) = read_timestamp(&mut reader, p1).await.unwrap();
        ...
        #[semio_framework_async_macros::async_test]
        async fn timestamp_epoch_zero_round_trips() {
        ...
        #[semio_framework_async_macros::async_test]
        async fn id_round_trips_via_prefix_uuid_tag() {
        ...
        #[semio_framework_async_macros::async_test]
        async fn id_falls_back_to_dictref_tag_for_plain_strings() {
            let mut out = ByteWriter::new().await;
            write_id(
                &mut out,
                "hello-world",
        ...
        #[semio_framework_async_macros::async_test]
        async fn id_raw_tag_is_readable_even_though_writer_never_emits_it() {
            let mut out = ByteWriter::new().await;
            out.write_u8(0).await;
            out.write_varint_u64(5).await;
        ...
        #[semio_framework_async_macros::async_test]
        async fn id_dictref_dedupes_repeated_ids_through_intern_closure() {
            ...
            let mut out = ByteWriter::new().await;
            {
                ...
                write_id(&mut out, "same-id", &mut intern, |_| None).await.unwrap();
                write_id(&mut out, "same-id", &mut intern, |_| None).await.unwrap();
            }
    ...
    //#region 🔖️Dictionary
    #[semio_framework_async_macros::async_test]
    async fn dict_builder_interns_deterministically_and_dedupes() {
        let mut builder = DictBuilder::new().await;
        assert!(builder.is_empty().await);
        assert_eq!(builder.intern("a").await, 0);
        assert_eq!(builder.intern("b").await, 1);
        assert_eq!(builder.intern("a").await, 0);
        assert_eq!(builder.len().await, 2);
        assert_eq!(builder.entries_since(0).await, &["a".to_string(), "b".to_string()]);
        assert_eq!(builder.entries_since(1).await, &["b".to_string()]);
    }

    #[semio_framework_async_macros::async_test]
    async fn dict_reader_round_trips_builder_deltas_in_order() {
        let mut builder = DictBuilder::new().await;
        builder.intern("x").await;
        builder.intern("y").await;
        let mut reader = DictReader::new().await;
        reader.extend(0, builder.entries_since(0).await.to_vec()).await.unwrap();
        assert_eq!(reader.resolve(0).await.unwrap(), "x");
        assert_eq!(reader.resolve(1).await.unwrap(), "y");
        assert_eq!(reader.len().await, 2);

        builder.intern("z").await;
        reader.extend(2, builder.entries_since(2).await.to_vec()).await.unwrap();
        assert_eq!(reader.resolve(2).await.unwrap(), "z");
    }

    #[semio_framework_async_macros::async_test]
    async fn dict_reader_rejects_out_of_order_deltas() {
        let mut reader = DictReader::new().await;
        let err = reader.extend(5, vec!["late".to_string()]).await.unwrap_err();
        assert_eq!(err, ProtocolError::DictOutOfOrder { expected: 0, actual: 5 });
    }

    #[semio_framework_async_macros::async_test]
    async fn dict_reader_reports_miss_past_the_end() {
        let reader = DictReader::new().await;
        assert_eq!(reader.resolve(0).await.unwrap_err(), ProtocolError::DictMiss(0));
    }
    ...
    #[semio_framework_async_macros::async_test]
    async fn record_hasher_trait_is_object_usable() {
        let hasher = FixedHasher;
        assert_eq!(hasher.hash(b"abc").await[0], 3);
    }
    //#endregion 🔖️Crypto

    //#region 🔖️HybridLogicalTimestamp
    #[semio_framework_async_macros::async_test]
    async fn hlc_tick_advances_on_newer_physical_time() {
        let mut hlc = HybridLogicalTimestamp::new(1, 100).await;
        hlc.tick(200).await;
    ...
    #[semio_framework_async_macros::async_test]
    async fn hlc_tick_bumps_logical_on_equal_or_older_physical_time() {
        let mut hlc = HybridLogicalTimestamp::new(1, 100).await;
        hlc.tick(100).await;
    ...
    #[semio_framework_async_macros::async_test]
    async fn hlc_merge_adopts_the_greater_remote_tick_then_bumps() {
        let mut local = HybridLogicalTimestamp::new(1, 100).await;
        let remote = HybridLogicalTimestamp { actor: 2, physical_ms: 150, logical: 3 };
        local.merge(&remote).await;
        assert_eq!(local.physical_ms, 150);
        assert_eq!(local.logical, 4);
    }

    #[semio_framework_async_macros::async_test]
    async fn hlc_ordering_uses_actor_as_final_tiebreak() {
        ...
        assert_ne!(a.cmp_key().await, b.cmp_key().await);
    }

    #[semio_framework_async_macros::async_test]
    async fn hlc_ordering_prioritizes_physical_then_logical_then_actor() {
    ...
    //#region 🔖️Identifiers
    #[semio_framework_async_macros::async_test]
    async fn identifier_newtypes_serde_round_trip_transparently() {
    ...
    #[semio_framework_async_macros::async_test]
    async fn document_version_orders_numerically() {
    ...
    //#region 🔖️Policies
    #[semio_framework_async_macros::async_test]
    async fn merge_policy_default_is_normal() {
        assert_eq!(MergePolicy::default(), MergePolicy::Normal);
    }

    #[semio_framework_async_macros::async_test]
    async fn merge_policy_rejects_matches_the_frozen_matrix() {
        use crate::diagnostic::Severity;
        assert!(!MergePolicy::LaissezFaire.rejects(Severity::Info).await);
        assert!(!MergePolicy::LaissezFaire.rejects(Severity::Warning).await);
        assert!(!MergePolicy::LaissezFaire.rejects(Severity::Error).await);
        assert!(MergePolicy::LaissezFaire.rejects(Severity::Fatal).await);

        assert!(!MergePolicy::Normal.rejects(Severity::Info).await);
        assert!(!MergePolicy::Normal.rejects(Severity::Warning).await);
        assert!(MergePolicy::Normal.rejects(Severity::Error).await);
        assert!(MergePolicy::Normal.rejects(Severity::Fatal).await);

        assert!(!MergePolicy::Vigilant.rejects(Severity::Info).await);
        assert!(MergePolicy::Vigilant.rejects(Severity::Warning).await);
        assert!(MergePolicy::Vigilant.rejects(Severity::Error).await);
        assert!(MergePolicy::Vigilant.rejects(Severity::Fatal).await);
    }

    #[semio_framework_async_macros::async_test]
    async fn merge_policy_as_u8_from_u8_round_trips() {
        for policy in [MergePolicy::LaissezFaire, MergePolicy::Normal, MergePolicy::Vigilant] {
            assert_eq!(MergePolicy::from_u8(policy.as_u8().await).await, Some(policy));
        }
        assert_eq!(MergePolicy::from_u8(3).await, None);
    }

    #[semio_framework_async_macros::async_test]
    async fn undo_policy_and_state_class_serde_round_trip() {
    ...
    //#region 🔖️ArtifactInferenceCatalog
    async fn empty_kernel_facet_leaves() -> KernelFacetLeaves {
        KernelFacetLeaves { rust: "", typescript: "", graphql: "", json_schema: "", proto: "" }
    }

    #[semio_framework_async_macros::async_test]
    async fn kernel_artifact_inference_catalog_registers_independently_of_the_four_facet_descriptor() {
        let before = kernel_artifact_inference_catalog_len().await;
        register_kernel_artifact_inference_descriptor(KernelArtifactInferenceDescriptor {
            id: "s.wave3.synthetic.inference",
            inference: empty_kernel_facet_leaves().await,
        }).await;
        assert!(kernel_artifact_inference_descriptor_registered("s.wave3.synthetic.inference").await);
        assert_eq!(kernel_artifact_inference_catalog_len().await, before.max(1));
        let mut found = false;
    ...
    //#region 🔖️WireCodec
    #[semio_framework_async_macros::async_test]
    async fn wire_str_round_trips_including_multibyte_utf8() {
        let mut out = Vec::new();
        write_str(&mut out, "héllo wörld 🎞️").await;
        let mut pos = 0;
        assert_eq!(read_str(&out, &mut pos).await.unwrap(), "héllo wörld 🎞️");
        assert_eq!(pos, out.len());
    }

    #[semio_framework_async_macros::async_test]
    async fn wire_str_empty_round_trips() {
        let mut out = Vec::new();
        write_str(&mut out, "").await;
        let mut pos = 0;
        assert_eq!(read_str(&out, &mut pos).await.unwrap(), "");
        assert_eq!(pos, out.len());
    }

    #[semio_framework_async_macros::async_test]
    async fn wire_bytes_round_trips_and_consumes_exact_length() {
        let mut out = Vec::new();
        write_bytes(&mut out, &[1, 2, 3, 4, 5]).await;
        write_bytes(&mut out, &[9]).await;
        let mut pos = 0;
        assert_eq!(read_bytes(&out, &mut pos).await.unwrap(), vec![1, 2, 3, 4, 5]);
        assert_eq!(read_bytes(&out, &mut pos).await.unwrap(), vec![9]);
        assert_eq!(pos, out.len());
    }

    #[semio_framework_async_macros::async_test]
    async fn wire_hash32_round_trips_fixed_width_no_length_prefix() {
        let hash = [7u8; 32];
        let mut out = Vec::new();
        write_hash32(&mut out, &hash).await;
        assert_eq!(out.len(), 32, "hash32 must be fixed-width with no length prefix");
        let mut pos = 0;
        assert_eq!(read_hash32(&out, &mut pos).await.unwrap(), hash);
        assert_eq!(pos, 32);
    }

    #[semio_framework_async_macros::async_test]
    async fn wire_bool_round_trips_as_a_single_byte() {
        let mut out = Vec::new();
        write_bool(&mut out, true).await;
        write_bool(&mut out, false).await;
        assert_eq!(out, vec![1, 0]);
        let mut pos = 0;
        assert!(read_bool(&out, &mut pos).await.unwrap());
        assert!(!read_bool(&out, &mut pos).await.unwrap());
    }

    #[semio_framework_async_macros::async_test]
    async fn wire_varint_u64_round_trips_via_pack_core() {
        let mut out = Vec::new();
        write_varint_u64(&mut out, 300).await;
        let mut pos = 0;
        assert_eq!(read_varint_u64(&out, &mut pos).await.unwrap(), 300);
    }

    #[semio_framework_async_macros::async_test]
    async fn wire_str_rejects_truncated_input() {
        let mut out = Vec::new();
        write_str(&mut out, "hello").await;
        out.truncate(out.len() - 1);
        let mut pos = 0;
        assert!(matches!(read_str(&out, &mut pos).await, Err(ProtocolError::Malformed { .. })));
    }

    #[semio_framework_async_macros::async_test]
    async fn wire_bytes_rejects_truncated_input() {
        let mut out = Vec::new();
        write_bytes(&mut out, &[1, 2, 3]).await;
        out.truncate(out.len() - 1);
        let mut pos = 0;
        assert!(matches!(read_bytes(&out, &mut pos).await, Err(ProtocolError::Malformed { .. })));
    }

    #[semio_framework_async_macros::async_test]
    async fn wire_hash32_rejects_truncated_input() {
        let bytes = [0u8; 10];
        let mut pos = 0;
        assert!(matches!(read_hash32(&bytes, &mut pos).await, Err(ProtocolError::Malformed { .. })));
    }
    //#endregion 🔖️WireCodec
}
//#endregion 🧪️Tests
