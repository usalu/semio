...
import { NumericIndex, type NumericIndexEdit, type NumericIndexRetirement } from "./🟦️component.ts";
...
class TestScript extends BundleScript {
  async run(): Promise<void> {
    ...
    const source = await Bun.file(`${import.meta.dir}/🟦️component.ts`).text();
    ...
    const program = ts.createProgram([`${import.meta.dir}/🟦️component.ts`], { strict: true, noEmit: true, target: ts.ScriptTarget.ES2022, module: ts.ModuleKind.ESNext, moduleResolution: ts.ModuleResolutionKind.Bundler, types: [], lib: ["lib.es2022.d.ts"] });
