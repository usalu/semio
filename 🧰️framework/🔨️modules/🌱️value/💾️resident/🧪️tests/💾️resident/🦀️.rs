        ...
        COUNT_ALLOCATIONS.with(|enabled| {
            if enabled.get() {
                ALLOCATIONS.with(|count| {
                    let index = count.get();
                    if index < 8 {
                        ALLOCATION_LAYOUTS.with(|events| {
                            let mut values = events.get();
                            values[index] = (ALLOCATION_PHASE.with(std::cell::Cell::get), layout.size(), layout.align());
                            events.set(values);
                        });
                    }
                    count.set(index + 1);
        ...
        let _ = COUNT_ALLOCATIONS.try_with(|enabled| {
            if enabled.get() {
                DEALLOCATIONS.with(|count| {
                    let index = count.get();
                    if index < 8 {
                        DEALLOCATION_LAYOUTS.with(|events| {
                            let mut values = events.get();
                            values[index] = (layout.size(), layout.align());
                            events.set(values);
                        });
                    }
                    count.set(index + 1);
...
fn resident_capacity_consumes_the_shared_capacity_and_invalid_vectors() {
    ...
    for vector in fixture["invalidCapacities"].as_array().unwrap() {
        assert!(capacity(&vector["value"]).is_err(), "{}", vector["name"]);
    }
...
impl ResidentDropProbe {
    fn new(drops: &Arc<AtomicUsize>) -> Self {
        Self { payload: Some(Box::new([73; 32])), drops: drops.clone() }
    }
    fn pointer(&self) -> *const u8 {
        self.payload.as_ref().unwrap().as_ptr()
    }
    fn retire_payload(&mut self, grant: ResidentGrant) -> bool {
        if grant.max_items() == 0 || grant.max_bytes() < 32 {
            return false;
        }
...
fn full_admission_grant() -> ResidentGrant {
    admission_grant(admission_fixture()["grants"][2]["maxBytes"].as_u64().unwrap())
}
fn admission_capacity() -> ResidentCapacity {
    capacity(&fixture()["capacity"]).unwrap()
}
fn assert_admission_step(step: ResidentStep, grant: ResidentGrant) {
    assert!(step.items <= grant.max_items());
    assert!(step.bytes <= grant.max_bytes());
}
...
fn admitted_consumer<'root, C: Send + 'static>(root: &'root ResidentLedgerRoot, source: &mut Option<C>) -> ResidentConsumer<'root, C> {
    ...
    for _ in 0..8 {
        if root.prepare_consumer::<C>(ResidentPartition::Data, grant).unwrap().kind == ResidentStepKind::Ready {
            break;
        }
    }
    let consumer = root.prepared_consumer::<C>().unwrap().unwrap();
    if source.is_some() {
        assert_eq!(consumer.install(source, grant).unwrap().kind, ResidentStepKind::Ready);
    }
    consumer
}
...
fn resident_admission_native_layout_has_one_fixed_root_and_separate_move_costs() {
    ...
    for bytes in [layout.admission_page_bytes, layout.consumer_page_bytes, layout.record_page_bytes, layout.consumer_move_bytes, layout.shell_move_bytes, layout.descriptor_move_bytes, layout.final_root_bytes] {
        assert!(bytes <= full_admission_grant().max_bytes());
    }
...
fn resident_admission_short_and_foreign_refusals_preserve_live_consumer() {
    ...
    for (index, grant) in [ResidentGrant::new(0, 4096).unwrap(), admission_grant(layout.admission_page_bytes - 1)].into_iter().enumerate() {
        ...
        assert_eq!(root.allocated_bytes().unwrap(), original_backing);
        assert!(ledger.prepared_admission(&consumer).unwrap().is_none());
    }
    let grant = full_admission_grant();
    for _ in 0..8 {
        if ledger.prepared_admission(&consumer).unwrap().is_some() {
            break;
        }
        assert_admission_step(ledger.prepare_admission(&consumer, ResidentPartition::Data, grant).unwrap(), grant);
    }
    ...
    assert_eq!(parent.as_ref().unwrap().pointer(), original);
    assert!(consumer.read_for_close().unwrap().is_none());
    retire_parent_probe(&mut parent);
...
fn resident_admission_caller_loss_and_parent_move_preserve_original_page_and_consumer() {
    ...
    drop(result);
    let retained = root.allocated_bytes().unwrap();
    assert!(retained > 0);
    assert_eq!(drops.load(Ordering::SeqCst), 0);
    drop(consumer);
    ...
    drop(ledger);
    drop(consumer);
    close_admission_root(&moved_parent.1);
    assert_eq!(drops.load(Ordering::SeqCst), 1);
}

#[test]
fn resident_admission_record_mutation_unwind_and_exact_parent_handoffs_never_cold_drop() {
    let consumer_drops = Arc::new(AtomicUsize::new(0));
    let shell_drops = Arc::new(AtomicUsize::new(0));
    let root = ResidentLedgerRoot::new(admission_capacity());
    ...
    let cell = ledger.prepared_admission(&consumer).unwrap().unwrap();
    ledger.claim_admission(&consumer, &cell, grant).unwrap();
    let envelope = ResidentResources::new(32, 1, 1).unwrap();
    for _ in 0..8 {
        if cell.record::<ResidentDropProbe>().unwrap().is_some() {
            break;
        }
        assert_admission_step(ledger.reserve_record::<ResidentDropProbe, ResidentDropProbe>(&cell, envelope, grant).unwrap(), grant);
    }
    let record = cell.record::<ResidentDropProbe>().unwrap().unwrap();
    let mut source = Some(ResidentDropProbe::new(&shell_drops));
    let shell_pointer = source.as_ref().unwrap().pointer();
    let layout = root.native_layout::<ResidentDropProbe, ResidentDropProbe>();
    ...
    assert_eq!(shell_drops.load(Ordering::SeqCst), 0);
    assert_eq!(consumer_drops.load(Ordering::SeqCst), 0);
    let mut shell_parent = None;
    let mut consumer_parent = None;
    assert!(root.begin_close().unwrap());
    assert_admission_step(root.close_step(grant).unwrap(), grant);
    assert!(!root.terminal_is_empty());
    ...
    drop(transferred);
    assert_eq!(shell_parent.as_ref().unwrap().pointer(), shell_pointer);
    for _ in 0..4 {
        assert_admission_step(root.close_step(grant).unwrap(), grant);
    }
    ...
    retire_parent_probe(&mut shell_parent);
    retire_parent_probe(&mut consumer_parent);
    drop(record);
    drop(cell);
    drop(ledger);
    drop(consumer);
...
fn resident_admission_first_access_refusal_allocation_boundary() {
    ...
    ALLOCATION_LAYOUTS.with(|value| value.set([(0, 0, 0); 8]));
    COUNT_ALLOCATIONS.with(|value| value.set(true));
    ALLOCATION_PHASE.with(|value| value.set(1));
    let root = ResidentLedgerRoot::new(capacity);
...
fn resident_admission_busy_access_preserves_original_root_without_allocation() {
    ...
    let close = root.begin_close();
    let observed = root.allocated_bytes();
    COUNT_ALLOCATIONS.with(|value| value.set(false));
    let allocations = ALLOCATIONS.with(std::cell::Cell::get);
    drop(held);
    close_admission_root(&root);
    assert_eq!(prepare.unwrap().kind, ResidentStepKind::Blocked);
    assert_eq!(close.unwrap(), false);
    assert_eq!(observed, Err(ResidentFault::Busy));
...
fn resident_admission_foreign_repopulation_cannot_trigger_last_consumer_drop() {
    ...
    let (during, after, source_pointer) = std::thread::scope(|scope| {
        ...
        for _ in 0..16 {
            assert_admission_step(root.close_step(grant).unwrap(), grant);
            if root.state.try_lock().ok().unwrap().consumer_release_interlock.is_none() {
                break;
            }
        }
    ...
    assert_eq!(during, ResidentStepKind::Blocked);
    assert_eq!(after, ResidentStepKind::Rejected);
    assert_eq!(source_pointer, Some(original));
    assert!(forward_alias_refused && forward_read_refused && close_read_empty);
    assert_eq!(drops.load(Ordering::SeqCst), 1);
}
...
fn resident_admission_exact_layout_and_partial_consumer_cancel_release_once() {
    ...
    for (frontier, name) in fixture["nativeOwnership"]["exactAllocation"]["cancelFrontiers"].as_array().unwrap().iter().enumerate() {
        let root = ResidentLedgerRoot::new(admission_capacity());
        ALLOCATIONS.with(|value| value.set(0));
        DEALLOCATIONS.with(|value| value.set(0));
        ALLOCATION_LAYOUTS.with(|value| value.set([(0, 0, 0); 8]));
        DEALLOCATION_LAYOUTS.with(|value| value.set([(0, 0); 8]));
        COUNT_ALLOCATIONS.with(|value| value.set(true));
        for _ in 0..=frontier {
            root.prepare_consumer::<AlignedResident>(ResidentPartition::Data, grant).unwrap();
        }
        let allocated = root.allocated_bytes().unwrap();
        let used = root.usage(ResidentPartition::Data).unwrap();
        let aligned = root.state.try_lock().ok().map(|state| state.pending_consumer.as_ref().or(state.consumers.as_ref()).unwrap().pointer.is_none_or(|pointer| pointer.as_ptr() as usize % layout.align() == 0)).unwrap();
        root.begin_close().unwrap();
        for _ in 0..16 {
            if root.terminal_is_empty() {
                break;
            }
            root.close_step(grant).unwrap();
        }
        COUNT_ALLOCATIONS.with(|value| value.set(false));
        let allocations = ALLOCATIONS.with(std::cell::Cell::get);
        let deallocations = DEALLOCATIONS.with(std::cell::Cell::get);
        let allocated_layouts = ALLOCATION_LAYOUTS.with(std::cell::Cell::get);
        let freed_layouts = DEALLOCATION_LAYOUTS.with(std::cell::Cell::get);
        close_admission_root(&root);
        assert!(aligned, "{name}");
        assert_eq!(used, ResidentResources::new(layout.size() as u64, 1, 1).unwrap());
        ...
        if expected == 1 {
            assert_eq!((allocated_layouts[0].1, allocated_layouts[0].2), (layout.size(), layout.align()));
            assert_eq!(freed_layouts[0], (layout.size(), layout.align()));
        }
    }
}
...
fn resident_admission_injected_allocation_failure_preserves_original_reservations() {
    ...
    close_admission_root(&root);
    assert_eq!(consumer_failed, Err(ResidentFault::Allocation));
    assert_eq!(consumer_retained, (consumer_charge, 0));
    for record_failure in [false, true] {
        ...
        if record_failure {
            ...
            cell = ledger.prepared_admission(&consumer).unwrap();
            ledger.claim_admission(&consumer, cell.as_ref().unwrap(), grant).unwrap();
        }
        let before = (root.usage(ResidentPartition::Data).unwrap(), root.allocated_bytes().unwrap());
        FAIL_NEXT_ALLOCATION.with(|value| value.set(true));
        let failure =
            if record_failure { ledger.reserve_record::<ResidentDropProbe, AlignedResident>(cell.as_ref().unwrap(), ResidentResources::new(64, 1, 1).unwrap(), grant) } else { ledger.prepare_admission(&consumer, ResidentPartition::Data, grant) };
        ...
        consumer.handoff_for_close_into(&mut recovered, grant).unwrap();
        let recovered_original = recovered.as_ref().unwrap().pointer() == original;
        retire_parent_probe(&mut recovered);
        drop(cell);
        drop(ledger);
        drop(consumer);
        close_admission_root(&root);
        assert_eq!(failure, Err(ResidentFault::Allocation));
        assert_eq!(after, before);
        assert!(same_source && recovered_original);
        assert_eq!(drops_before_recovery, 0);
        assert_eq!(drops.load(Ordering::SeqCst), 1);
    }
}
...
fn resident_admission_private_consumer_phase_qualifies_access_and_record_aliases() {
    ...
    for vector in fixture["nativeOwnership"]["phaseAccess"].as_array().unwrap() {
        let root = ResidentLedgerRoot::new(admission_capacity());
        let consumer = admitted_consumer(&root, &mut Some(AlignedResident(73)));
        let ledger = root.ledger();
        for _ in 0..8 {
            if ledger.prepared_admission(&consumer).unwrap().is_some() {
                break;
        ...
        for _ in 0..8 {
            if cell.record::<AlignedResident>().unwrap().is_some() {
                break;
            }
            ledger.reserve_record::<AlignedResident, AlignedResident>(&cell, envelope, grant).unwrap();
        }
        let record = cell.record::<AlignedResident>().unwrap().unwrap();
        match vector["phase"].as_str().unwrap() {
            "consumer-closing" => {
                consumer.begin_close(grant).unwrap();
            }
            "root-closing" => {
                root.begin_close().unwrap();
            }
        ...
        let expected = vector["forward"].as_bool().unwrap();
        let recovery = vector["recovery"].as_bool().unwrap();
        ...
        let mut attempted = Some(AlignedResident(91));
        let install = record.install(&mut attempted, grant).unwrap().kind;
        root.begin_close().unwrap();
        let mut shell = None;
        if attempted.is_none() {
            record.handoff_into(&mut shell, grant).unwrap();
        }
        let mut parent = None;
        consumer.handoff_for_close_into(&mut parent, grant).unwrap();
        drop(record);
        drop(cell);
        drop(ledger);
        drop(consumer);
        close_admission_root(&root);
        assert_eq!(forwards, (expected, expected, expected));
        assert_eq!(recoveries, (recovery, recovery, recovery));
        assert_eq!(install, if expected { ResidentStepKind::Ready } else { ResidentStepKind::Rejected });
        assert_eq!(parent.unwrap().0, 73);
        assert_eq!(attempted.or(shell).unwrap().0, 91);
    }
}
...
fn resident_admission_all_three_page_backings_use_exact_layout_and_short_grants() {
    ...
    let envelope = ResidentResources::new(64, 1, 1).unwrap();
    let reserve_work = [std::mem::size_of::<Option<super::ConsumerPage>>(), std::mem::size_of::<u64>(), std::mem::size_of::<ResidentResources>(), std::mem::size_of::<Option<std::ptr::NonNull<super::ConsumerHeader>>>()]
        .into_iter()
    ...
    let consumer = root.prepared_consumer::<AlignedResident>().unwrap().unwrap();
    let ledger = root.ledger();
    for _ in 0..3 {
        ledger.prepare_admission(&consumer, ResidentPartition::Data, grant).unwrap();
    }
    ...
    for (index, short) in [ResidentGrant::new(0, 4096).unwrap(), admission_grant(layouts[2].size() as u64 - 1)].into_iter().enumerate() {
        refused[index + 6] = ledger.reserve_record::<AlignedResident, AlignedResident>(&cell, envelope, short).unwrap().kind;
    }
    ...
    let usage = root.usage(ResidentPartition::Data).unwrap();
    let allocated_bytes = root.allocated_bytes().unwrap();
    drop(record);
    drop(cell);
    drop(ledger);
    drop(consumer);
    ...
    let allocations = ALLOCATIONS.with(std::cell::Cell::get);
    let deallocations = DEALLOCATIONS.with(std::cell::Cell::get);
    let allocation_layouts = ALLOCATION_LAYOUTS.with(std::cell::Cell::get);
    let deallocation_layouts = DEALLOCATION_LAYOUTS.with(std::cell::Cell::get);
    close_admission_root(&root);
    assert_eq!(refused, [ResidentStepKind::Blocked; 8]);
    assert_eq!(short_consumer_allocations, 0);
    assert!(consumer_unchanged && exact_consumer_work);
    assert_eq!(before_record, after_short_record);
    assert!(record_aligned);
    assert_eq!((allocations, deallocations), (3, 3));
    for index in 0..3 {
        assert_eq!((allocation_layouts[index].1, allocation_layouts[index].2), (layouts[index].size(), layouts[index].align()));
    }
    for (index, layout) in [layouts[2], layouts[1], layouts[0]].into_iter().enumerate() {
        assert_eq!(deallocation_layouts[index], (layout.size(), layout.align()));
    }
    let expected_bytes = layouts.iter().map(|layout| layout.size() as u64).sum::<u64>();
    assert_eq!(allocated_bytes, expected_bytes);
    assert_eq!(usage, ResidentResources::new(expected_bytes, 3, 3).unwrap().checked_add(envelope).unwrap());
}
