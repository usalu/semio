...
impl ResidentResources {
    pub fn new(bytes: u64, slots: u64, owners: u64) -> Result<Self, ResidentFault> {
        if bytes > RESIDENT_MAXIMUM_COUNT || slots > RESIDENT_MAXIMUM_COUNT || owners > RESIDENT_MAXIMUM_COUNT {
            return Err(ResidentFault::Count);
        }
...
impl ResidentCapacity {
    pub fn new(total: ResidentResources, control: ResidentResources) -> Result<Self, ResidentFault> {
        if !control.fits_within(total) {
            return Err(ResidentFault::Capacity);
        }
        Ok(Self { total, control })
    }

    pub fn total(self) -> ResidentResources {
        self.total
    }
    pub fn control(self) -> ResidentResources {
        self.control
    }
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ResidentPartition {
    Data,
    Control,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ResidentStepKind {
    Blocked,
    Pending,
    Ready,
    Complete,
...
impl ResidentGrant {
    ...
    fn admits(self, bytes: u64) -> bool {
        self.items > 0 && self.bytes >= bytes
    }
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct ResidentStep {
    pub kind: ResidentStepKind,
    pub items: u64,
    pub bytes: u64,
}
...
impl ResidentStep {
    fn blocked() -> Self {
        Self { kind: ResidentStepKind::Blocked, items: 0, bytes: 0 }
    }
    fn rejected() -> Self {
        Self { kind: ResidentStepKind::Rejected, items: 0, bytes: 0 }
    }
    fn done(kind: ResidentStepKind, bytes: u64) -> Self {
        Self { kind, items: 1, bytes }
    }
}
...
struct ResidentAccess<T> {
    held: AtomicBool,
    poisoned: AtomicBool,
    value: UnsafeCell<T>,
}
struct ResidentAccessGuard<'a, T> {
    access: &'a ResidentAccess<T>,
    thread: PhantomData<*mut ()>,
}
enum ResidentAccessError {
    Busy,
    Poisoned,
}
...
impl<T> ResidentAccess<T> {
    const fn new(value: T) -> Self {
        Self { held: AtomicBool::new(false), poisoned: AtomicBool::new(false), value: UnsafeCell::new(value) }
    }
    fn try_lock(&self) -> Result<ResidentAccessGuard<'_, T>, ResidentAccessError> {
        if self.held.compare_exchange(false, true, Ordering::Acquire, Ordering::Relaxed).is_err() {
            return Err(ResidentAccessError::Busy);
        }
...
impl<T> Deref for ResidentAccessGuard<'_, T> {
    type Target = T;
    fn deref(&self) -> &T {
        unsafe { &*self.access.value.get() }
    }
}

impl<T> DerefMut for ResidentAccessGuard<'_, T> {
    fn deref_mut(&mut self) -> &mut T {
        unsafe { &mut *self.access.value.get() }
    }
}
...
impl<T> Drop for ResidentAccessGuard<'_, T> {
    fn drop(&mut self) {
        if std::thread::panicking() {
            self.access.poisoned.store(true, Ordering::Relaxed);
        }
...
pub struct ResidentLedgerRoot {
    state: ResidentAccess<LedgerState>,
}
...
enum ResidentReleaseOrigin {
    ...
    PendingConsumer,
    Consumer,
    PrimaryPending { registration: NonZeroU64 },
    PrimaryConsumer { registration: NonZeroU64 },
}
...
impl ResidentReleaseStage {
    fn allocated(pointer: NonNull<u8>, layout: Layout, initialized: bool, destroy_empty: unsafe fn(NonNull<u8>)) -> Self {
        ...
        if initialized {
            Self::Destroy { allocation, destroy_empty }
        } else {
            Self::Free { allocation }
        }
    }
    fn pointerless(&self) -> bool {
        matches!(self, Self::Refund { .. } | Self::Clear { .. })
    }
}
...
struct ResidentRecoveryPin {
    pointer: NonNull<ConsumerHeader>,
    registration: NonZeroU64,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ResidentRecoveryMode {
    Forward,
    Closing,
}
...
#[cfg(test)]
struct ConsumerReleaseInterlock {
    observed: std::sync::mpsc::SyncSender<()>,
    resume: std::sync::mpsc::Receiver<()>,
}

pub struct ResidentLedger<'root> {
    root: &'root ResidentLedgerRoot,
}
...
impl ResidentLedgerRoot {
    pub fn new(capacity: ResidentCapacity) -> Self {
        ...
        Self {
            state: ResidentAccess::new(LedgerState {
                capacity,
                data: zero,
                control: zero,
                allocated_bytes: 0,
                ...
                prepared_consumer: None,
                last_consumer_registration: 0,
                primary: None,
                recovery: None,
                closing: false,
                closed: false,
                #[cfg(test)]
                consumer_release_interlock: None,
            }),
        }
    }

    pub fn ledger(&self) -> ResidentLedger<'_> {
        ResidentLedger { root: self }
    }

    pub fn native_layout<C, S>(&self) -> ResidentNativeLayout {
        ResidentNativeLayout {
            root_bytes: size_of::<Self>() as u64,
            admission_page_bytes: size_of::<AdmissionNode>() as u64,
            consumer_page_bytes: size_of::<ConsumerNode<C>>() as u64,
            record_page_bytes: size_of::<RecordNode<S>>() as u64,
            consumer_move_bytes: size_of::<Option<C>>() as u64,
            shell_move_bytes: size_of::<Option<S>>() as u64,
            descriptor_move_bytes: size_of::<ErasedRecord>()
                .max(size_of::<Option<AdmissionPage>>())
                .max(size_of::<ConsumerPage>())
                .max(size_of::<Option<ResidentRelease>>())
                .max(size_of::<Option<ResidentPrimaryAnchor>>())
                .max(size_of::<Option<ResidentRecoveryCursor>>()) as u64,
            final_root_bytes: size_of::<Self>() as u64,
            release_slot_bytes: size_of::<Option<ResidentRelease>>() as u64,
            pending_consumer_bytes: size_of::<Option<ConsumerPage>>() as u64,
        }
    }

    pub fn allocated_bytes(&self) -> Result<u64, ResidentFault> {
        self.access()?.map(|state| state.allocated_bytes).ok_or(ResidentFault::Busy)
    }
    pub fn usage(&self, partition: ResidentPartition) -> Result<ResidentResources, ResidentFault> {
        self.access()?.map(|state| state.used(partition)).ok_or(ResidentFault::Busy)
    }
    pub fn begin_close(&self) -> Result<bool, ResidentFault> {
        let Some(mut state) = self.access()? else {
            return Ok(false);
        };
        state.closing = true;
        Ok(true)
    ...
    pub fn terminal_is_empty(&self) -> bool {
        self.state.try_lock_close().map(|state| state.closed && state.structurally_empty()).unwrap_or(false)
    }

    fn access(&self) -> Result<Option<ResidentAccessGuard<'_, LedgerState>>, ResidentFault> {
        match self.state.try_lock() {
            Ok(state) => Ok(Some(state)),
            Err(ResidentAccessError::Busy) => Ok(None),
            Err(ResidentAccessError::Poisoned) => Err(ResidentFault::Poisoned),
        }
    }
    ...
    pub fn close_step(&self, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        ...
        if !state.closing {
            ...
            if !grant.admits(bytes) {
                return Ok(ResidentStep::blocked());
            }
            state.closing = true;
            return Ok(ResidentStep::done(ResidentStepKind::Pending, bytes));
        }
        if state.release.is_some() {
            return state.advance_release(grant);
        }
        if state.recovery.is_some() {
            return state.close_recovery(grant);
        }
        ...
        if let Some(page) = state.head.as_ref() {
            ...
            if let Some(record) = node.record.as_ref() {
                if !record.empty()? || record.aliases() != 0 {
                    return Ok(ResidentStep::blocked());
                }
                if record.allocated_bytes != record.layout.size() as u64 {
                    return Err(ResidentFault::Identity);
                }
            ...
            if owner.aliases.load(Ordering::Acquire) != 0 {
                return Ok(ResidentStep::blocked());
            }
        ...
        if let Some(page) = state.pending_consumer.as_ref() {
            if page.initialized && page.pointer.is_none_or(|pointer| !unsafe { (page.empty)(pointer) }) {
                return Ok(ResidentStep::blocked());
            }
        ...
        if let Some(page) = state.consumers.as_ref() {
            ...
            if !unsafe { *header.closing.get() } {
                if !grant.admits(size_of::<bool>() as u64) {
                    return Ok(ResidentStep::blocked());
                }
                unsafe {
                    *header.closing.get() = true;
                }
                return Ok(ResidentStep::done(ResidentStepKind::Pending, size_of::<bool>() as u64));
            }
            if !unsafe { (page.empty)(pointer) } {
                return Ok(ResidentStep::blocked());
            }
            ...
            if header.aliases.load(Ordering::Acquire) != 0 || header.admissions.load(Ordering::Acquire) != 0 || header.recovery_pins.load(Ordering::Acquire) != 0 {
                return Ok(ResidentStep::blocked());
            }
            let clear_prepared = state.prepared_consumer == Some(pointer);
            let bytes = resident_release_work(&[
                size_of::<Option<ConsumerPage>>(),
                size_of::<Option<ConsumerPage>>(),
                size_of::<Option<ResidentRelease>>(),
                if clear_prepared { size_of::<Option<NonNull<ConsumerHeader>>>() } else { 0 },
                if primary.is_some() { size_of::<ResidentPrimaryBacking>() } else { 0 },
            ])?;
            if !grant.admits(bytes) {
                return Ok(ResidentStep::blocked());
            }
            if primary.is_some() {
                state.primary.as_mut().unwrap().backing = ResidentPrimaryBacking::Releasing;
            }
...
impl LedgerState {
    fn primary_for_page(&self, page: &ConsumerPage) -> Result<Option<NonZeroU64>, ResidentFault> {
        let Some(anchor) = self.primary.as_ref() else {
            return Ok(None);
        };
        let ResidentPrimaryBacking::Published(pointer) = &anchor.backing else {
            return Ok(None);
        };
        let same_pointer = page.pointer == Some(*pointer);
        let same_registration = page.registration == anchor.stamp.generation;
        if same_pointer != same_registration {
            return Err(ResidentFault::Identity);
        }
        if !same_pointer {
            return Ok(None);
        }
        if page.type_id != anchor.stamp.type_id || page.partition != anchor.partition {
            return Err(ResidentFault::Identity);
        }
    ...
    fn detach_pending_primary(&mut self, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        ...
        let ResidentPrimaryBacking::Pending(page) = &anchor.backing else {
            return Ok(ResidentStep::rejected());
        };
        if page.registration != anchor.stamp.generation || page.type_id != anchor.stamp.type_id || page.partition != anchor.partition {
            return Err(ResidentFault::Identity);
        }
    ...
    fn primary_recovery_target<C: Send + 'static>(&self, mode: ResidentRecoveryMode) -> Result<Option<(ResidentRegistrationStamp, NonNull<ConsumerHeader>)>, ResidentFault> {
        if self.closed || self.closing != (mode == ResidentRecoveryMode::Closing) {
            return Err(ResidentFault::Closed);
        }
        let Some(anchor) = self.primary.as_ref() else {
            return Ok(None);
        };
        if anchor.stamp.type_id != TypeId::of::<C>() {
            return Err(ResidentFault::Identity);
        }
        match &anchor.backing {
            ResidentPrimaryBacking::Published(pointer) => Ok(Some((anchor.stamp, *pointer))),
            _ => Ok(None),
        }
    }
    ...
    fn structurally_empty(&self) -> bool {
        #[cfg(test)]
        if self.consumer_release_interlock.is_some() {
            return false;
        }
        let zero = ResidentResources { bytes: 0, slots: 0, owners: 0 };
        self.head.is_none()
            ...
            && self.consumers.is_none()
            && self.pending_consumer.is_none()
            && self.prepared_consumer.is_none()
    ...
    fn advance_release(&mut self, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        ...
        let primary = match &release.origin {
            ResidentReleaseOrigin::PrimaryPending { registration } | ResidentReleaseOrigin::PrimaryConsumer { registration } => Some(*registration),
            _ => None,
        };
        if let Some(registration) = primary {
            if self.primary.as_ref().is_none_or(|anchor| anchor.stamp.generation != registration || anchor.partition != release.partition || !matches!(&anchor.backing, ResidentPrimaryBacking::Releasing)) {
                return Err(ResidentFault::Identity);
            }
        }
        match &release.stage {
            ResidentReleaseStage::Destroy { allocation, destroy_empty } => {
                ...
                let destroy_empty = *destroy_empty;
                unsafe {
                    destroy_empty(pointer);
                }
            ...
            ResidentReleaseStage::Refund { released_layout } => {
                ...
                let used = match release.partition {
                    ResidentPartition::Data => self.data,
                    ResidentPartition::Control => self.control,
                };
                let next = used.checked_sub(release.charge)?;
                let released_layout = *released_layout;
                let partition = release.partition;
                match partition {
                    ResidentPartition::Data => self.data = next,
                    ResidentPartition::Control => self.control = next,
                }
    ...
    fn used(&self, partition: ResidentPartition) -> ResidentResources {
        match partition {
            ResidentPartition::Data => self.data,
            ResidentPartition::Control => self.control,
        }
    ...
    fn reserve(&mut self, partition: ResidentPartition, charge: ResidentResources) -> Result<(), ResidentFault> {
        ...
        let capacity = match partition {
            ResidentPartition::Data => self.capacity.data(),
            ResidentPartition::Control => self.capacity.control(),
        };
...
#[repr(C)]
struct ConsumerNode<C> {
    header: ConsumerHeader,
    source: UnsafeCell<Option<C>>,
}
...
impl ConsumerPage {
    ...
    fn initialize(&mut self, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        ...
        unsafe {
            (self.initialize)(pointer, self.registration);
        }
...
unsafe fn initialize_consumer<C: Send + 'static>(pointer: NonNull<ConsumerHeader>, registration: NonZeroU64) {
    unsafe {
        pointer.cast::<ConsumerNode<C>>().as_ptr().write(ConsumerNode {
            header: ConsumerHeader { aliases: AtomicUsize::new(0), admissions: AtomicUsize::new(0), recovery_pins: AtomicUsize::new(0), closing: UnsafeCell::new(false), next: UnsafeCell::new(None), type_id: TypeId::of::<C>(), registration },
            source: UnsafeCell::new(None),
        });
    }
}

unsafe fn empty_consumer<C>(pointer: NonNull<ConsumerHeader>) -> bool {
    unsafe { (&*pointer.cast::<ConsumerNode<C>>().as_ref().source.get()).is_none() }
}
unsafe fn destroy_consumer<C>(pointer: NonNull<u8>) {
    unsafe {
        pointer.cast::<ConsumerNode<C>>().as_ptr().drop_in_place();
    }
}

pub struct ResidentConsumer<'root, C> {
    root: &'root ResidentLedgerRoot,
    pointer: NonNull<ConsumerNode<C>>,
}
pub struct ResidentConsumerRead<'root, C> {
    _access: ResidentAccessGuard<'root, LedgerState>,
    pointer: NonNull<C>,
}
...
impl<C> Drop for ResidentConsumer<'_, C> {
    fn drop(&mut self) {
        unsafe {
            self.pointer.as_ref().header.aliases.fetch_sub(1, Ordering::AcqRel);
        }
    }
}
...
impl<'root, C: Send + 'static> ResidentConsumer<'root, C> {
    pub fn install(&self, source: &mut Option<C>, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        let Some(state) = self.root.access()? else {
            return Ok(ResidentStep::blocked());
        };
        let node = unsafe { self.pointer.as_ref() };
        if state.closing || unsafe { *node.header.closing.get() } {
            return Ok(ResidentStep::rejected());
        }
    ...
    pub fn read(&self) -> Result<Option<ResidentConsumerRead<'root, C>>, ResidentFault> {
        let Some(access) = self.root.access()? else {
            return Err(ResidentFault::Busy);
        };
        if access.closing || unsafe { *self.pointer.as_ref().header.closing.get() } {
            return Err(ResidentFault::Closed);
        }
    ...
    pub fn begin_close(&self, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        let Some(_state) = self.root.access()? else {
            return Ok(ResidentStep::blocked());
        };
        if !grant.admits(size_of::<bool>() as u64) {
            return Ok(ResidentStep::blocked());
        ...
        unsafe {
            *self.pointer.as_ref().header.closing.get() = true;
        }
...
impl ResidentLedgerRoot {
    pub fn prepare_consumer<C: Send + 'static>(&self, partition: ResidentPartition, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        ...
        if let Some(page) = state.pending_consumer.as_mut() {
            if page.type_id != TypeId::of::<C>() || page.partition != partition {
                return Ok(ResidentStep::rejected());
            }
            if page.pointer.is_none() {
                return page.allocate(&mut state.allocated_bytes, grant);
            }
            if !page.initialized {
                return page.initialize(grant);
            }
            ...
            let pointer = page.pointer.unwrap();
            unsafe {
                *pointer.as_ref().next.get() = state.consumers.take();
            }
        ...
        if state.reserve(partition, page.charge).is_err() {
            return Ok(ResidentStep::blocked());
        }
    ...
    pub fn reserve_primary_consumer<C: Send + 'static>(&self, partition: ResidentPartition, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        ...
        if state.closing || state.closed || state.primary.is_some() {
            return Ok(ResidentStep::rejected());
        }
    ...
    pub fn prepare_primary_consumer<C: Send + 'static>(&self, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        ...
        let Some(anchor) = state.primary.as_mut() else {
            return Ok(ResidentStep::rejected());
        };
        if anchor.stamp.type_id != TypeId::of::<C>() {
            return Err(ResidentFault::Identity);
        }
        let ResidentPrimaryBacking::Pending(page) = &mut anchor.backing else {
            return Ok(ResidentStep::rejected());
        };
        if page.registration != anchor.stamp.generation || page.type_id != anchor.stamp.type_id || page.partition != anchor.partition {
            return Err(ResidentFault::Identity);
    ...
    pub fn begin_primary_recovery<C: Send + 'static>(&self, mode: ResidentRecoveryMode, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        ...
        let Some((stamp, _)) = state.primary_recovery_target::<C>(mode)? else {
            return Ok(ResidentStep::rejected());
        };
        if state.recovery.is_some() {
            return Ok(ResidentStep::rejected());
    ...
    pub fn advance_primary_recovery<C: Send + 'static>(&self, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        ...
        let Some(cursor) = state.recovery.as_ref() else {
            return Ok(ResidentStep::rejected());
        };
        if cursor.revoked {
            return Err(ResidentFault::Closed);
        }
        let Some((stamp, original)) = state.primary_recovery_target::<C>(cursor.mode)? else {
            return Ok(ResidentStep::rejected());
        };
        if cursor.stamp != stamp || cursor.found.is_some() {
            return Ok(ResidentStep::rejected());
        }
        let Some(pin) = cursor.next.as_ref() else {
            return Ok(ResidentStep::rejected());
        };
        let matching = pin.registration == stamp.generation;
        if matching && pin.pointer != original {
            return Err(ResidentFault::Identity);
        }
        ...
        let header = unsafe { pin.pointer.as_ref() };
        if header.registration != pin.registration {
            return Err(ResidentFault::Identity);
        }
        let remaining = header.recovery_pins.load(Ordering::Acquire).checked_sub(1).ok_or(ResidentFault::Count)?;
        if matching {
            if header.type_id != stamp.type_id || (cursor.mode == ResidentRecoveryMode::Forward && unsafe { *header.closing.get() }) {
                return Err(ResidentFault::Identity);
            }
        ...
        if !successor.initialized || pointer == pin.pointer || registration == pin.registration {
            return Err(ResidentFault::Identity);
        }
    ...
    pub fn capture_primary_consumer<C: Send + 'static>(&self, mode: ResidentRecoveryMode, grant: ResidentGrant) -> Result<(ResidentStep, Option<ResidentConsumer<'_, C>>), ResidentFault> {
        let Some(mut state) = self.access()? else {
            return Ok((ResidentStep::blocked(), None));
        };
        let Some((stamp, original)) = state.primary_recovery_target::<C>(mode)? else {
            return Ok((ResidentStep::rejected(), None));
        };
        let Some(cursor) = state.recovery.as_ref() else {
            return Ok((ResidentStep::rejected(), None));
        };
        ...
        if cursor.mode != mode || cursor.stamp != stamp || cursor.next.is_some() {
            return Ok((ResidentStep::rejected(), None));
        }
        let Some(pin) = cursor.found.as_ref() else {
            return Ok((ResidentStep::rejected(), None));
        };
        if pin.registration != stamp.generation || pin.pointer != original {
            return Err(ResidentFault::Identity);
        }
        ...
        let header = unsafe { pin.pointer.as_ref() };
        if header.registration != stamp.generation || header.type_id != stamp.type_id {
            return Err(ResidentFault::Identity);
        }
        if mode == ResidentRecoveryMode::Forward && unsafe { *header.closing.get() } {
            return Err(ResidentFault::Closed);
        }
        let remaining = header.recovery_pins.load(Ordering::Acquire).checked_sub(1).ok_or(ResidentFault::Count)?;
    ...
    pub fn begin_primary_consumer_close(&self, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        ...
        if state.closed || state.primary.is_none() {
            return Ok(ResidentStep::rejected());
        }
        ...
        if bytes == 0 {
            return Ok(ResidentStep { kind: ResidentStepKind::Pending, items: 0, bytes: 0 });
        }
        if !grant.admits(bytes) {
            return Ok(ResidentStep::blocked());
        }
        ...
        if revoke {
            state.recovery.as_mut().unwrap().revoked = true;
        }
        Ok(ResidentStep::done(ResidentStepKind::Pending, bytes))
    }
    ...
    fn consumer_access<C: Send + 'static>(&self, closing: bool) -> Result<Option<ResidentConsumer<'_, C>>, ResidentFault> {
        let Some(state) = self.access()? else {
            return Ok(None);
        };
        let Some(pointer) = state.prepared_consumer else {
            return Ok(None);
        };
        let header = unsafe { pointer.as_ref() };
        if header.type_id != TypeId::of::<C>() {
            return Err(ResidentFault::Identity);
        }
        if (state.closing || unsafe { *header.closing.get() }) != closing {
            return Err(ResidentFault::Closed);
        }
...
impl ErasedConsumer {
    fn new<C: Send + 'static>(source: &ResidentConsumer<'_, C>) -> Result<Self, ResidentFault> {
        unsafe {
            source.pointer.as_ref().header.admissions.try_update(Ordering::AcqRel, Ordering::Acquire, |count| count.checked_add(1)).map_err(|_| ResidentFault::Count)?;
        }
        Ok(Self { pointer: source.pointer.cast(), empty: empty_consumer::<C> })
    }
    fn matches<C>(&self, source: &ResidentConsumer<'_, C>) -> bool {
        self.pointer == source.pointer.cast()
    }
    fn is_empty(&self) -> bool {
        unsafe { (self.empty)(self.pointer) }
    }
}
...
impl Drop for ErasedConsumer {
    fn drop(&mut self) {
        unsafe {
            self.pointer.as_ref().admissions.fetch_sub(1, Ordering::AcqRel);
        }
    }
}
...
struct AdmissionPage {
    pointer: NonNull<AdmissionNode>,
    initialized: bool,
}

unsafe fn destroy_admission(pointer: NonNull<u8>) {
    unsafe {
        pointer.cast::<AdmissionNode>().as_ptr().drop_in_place();
    }
}
...
pub struct ResidentAdmission<'root, C> {
    root: &'root ResidentLedgerRoot,
    node: NonNull<AdmissionNode>,
...
impl<'root> ResidentLedger<'root> {
    pub fn prepare_admission<C: Send + 'static>(&self, consumer: &ResidentConsumer<'_, C>, partition: ResidentPartition, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        if !std::ptr::eq(self.root, consumer.root) {
            return Ok(ResidentStep::rejected());
        }
        let Some(mut state) = self.root.access()? else {
            return Ok(ResidentStep::blocked());
        };
        if state.closing || state.closed || unsafe { *consumer.pointer.as_ref().header.closing.get() } {
            return Ok(ResidentStep::rejected());
        }
        ...
        if let Some(pending) = state.pending.as_mut() {
            if pending.partition != partition || !pending.consumer.as_ref().is_some_and(|held| held.matches(consumer)) {
                return Ok(ResidentStep::blocked());
            }
            ...
            unsafe {
                page.pointer
                    .as_ptr()
                    .write(AdmissionNode { aliases: AtomicUsize::new(0), fields: UnsafeCell::new(AdmissionFields { next: state.head.take(), consumer: pending.consumer.take(), record: None, partition, charge: pending.charge, claimed: false }) });
            }
        ...
        if state.reserve(partition, charge).is_err() {
            return Ok(ResidentStep::blocked());
        }
        let held = match ErasedConsumer::new(consumer) {
            Ok(held) => held,
            Err(error) => {
                state.release(partition, charge)?;
                return Err(error);
            }
        };
    ...
    fn admission_access<C: Send + 'static>(&self, consumer: &ResidentConsumer<'_, C>, closing: bool) -> Result<Option<ResidentAdmission<'root, C>>, ResidentFault> {
        ...
        if (state.closing || unsafe { *consumer.pointer.as_ref().header.closing.get() }) != closing {
            return Err(ResidentFault::Closed);
        }
        let Some(pointer) = state.prepared else {
            return Ok(None);
        };
        let node = unsafe { pointer.as_ref() };
        let fields = unsafe { &*node.fields.get() };
        if fields.claimed || !fields.consumer.as_ref().is_some_and(|held| held.matches(consumer)) {
            return Ok(None);
        }
    ...
    pub fn claim_admission<C: Send + 'static>(&self, consumer: &ResidentConsumer<'_, C>, cell: &ResidentAdmission<'_, C>, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        if !std::ptr::eq(self.root, cell.root) || !std::ptr::eq(self.root, consumer.root) {
            return Ok(ResidentStep::rejected());
        }
        let Some(mut state) = self.root.access()? else {
            return Ok(ResidentStep::blocked());
        };
        if state.closing || unsafe { *consumer.pointer.as_ref().header.closing.get() } {
            return Ok(ResidentStep::rejected());
        }
        let node = unsafe { &mut *cell.node.as_ref().fields.get() };
        if node.claimed || !node.consumer.as_ref().is_some_and(|held| held.matches(consumer)) {
            return Ok(ResidentStep::rejected());
        }
        if !grant.admits(size_of::<bool>() as u64) {
            return Ok(ResidentStep::blocked());
    ...
    pub fn reserve_record<C: Send + 'static, S: Send + 'static>(&self, cell: &ResidentAdmission<'_, C>, envelope: ResidentResources, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        if !std::ptr::eq(self.root, cell.root) {
            return Ok(ResidentStep::rejected());
        }
        let Some(mut state) = self.root.access()? else {
            return Ok(ResidentStep::blocked());
        ...
        let node = unsafe { &mut *cell.node.as_ref().fields.get() };
        if node.consumer.as_ref().is_none_or(|consumer| unsafe { *consumer.pointer.as_ref().closing.get() }) {
            return Ok(ResidentStep::rejected());
        }
        if !node.claimed {
            return Ok(ResidentStep::rejected());
        ...
        if let Some(record) = node.record.as_mut() {
            if record.type_id != TypeId::of::<S>() || record.initialized {
                return Ok(ResidentStep::rejected());
            }
            ...
            unsafe {
                record.pointer.cast::<RecordNode<S>>().as_ptr().write(RecordNode { source: UnsafeCell::new(None), aliases: AtomicUsize::new(0) });
            }
        ...
        if !grant.admits(work) {
            return Ok(ResidentStep::blocked());
        }
        let charge = envelope.checked_add(ResidentResources { bytes, slots: 1, owners: 1 })?;
        let allocated = state.allocated_bytes.checked_add(bytes).ok_or(ResidentFault::Count)?;
        if state.reserve(node.partition, charge).is_err() {
            return Ok(ResidentStep::blocked());
        }
        let layout = Layout::new::<RecordNode<S>>();
        let Some(pointer) = NonNull::new(unsafe { std::alloc::alloc(layout) }) else {
            state.release(node.partition, charge)?;
            return Err(ResidentFault::Allocation);
        };
        node.record =
            Some(ErasedRecord { pointer, layout, allocated_bytes: bytes, charge, partition: node.partition, initialized: false, type_id: TypeId::of::<S>(), empty: record_empty::<S>, aliases: record_aliases::<S>, destroy_empty: destroy_record::<S> });
        state.allocated_bytes = allocated;
...
impl<C> Drop for ResidentAdmission<'_, C> {
    fn drop(&mut self) {
        unsafe {
            self.node.as_ref().aliases.fetch_sub(1, Ordering::AcqRel);
        }
    }
}
...
impl<'root, C: Send + 'static> ResidentAdmission<'root, C> {
    pub fn handoff_consumer_into(&self, consumer: &ResidentConsumer<'_, C>, target: &mut Option<C>, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        ...
        if target.is_some() || !node.consumer.as_ref().is_some_and(|held| held.matches(consumer)) {
            return Ok(ResidentStep::rejected());
        }
        let bytes = size_of::<Option<C>>() as u64;
        if !grant.admits(bytes) {
    ...
    fn record_access<S: Send + 'static>(&self, closing: bool) -> Result<Option<ResidentRecord<'root, S>>, ResidentFault> {
        ...
        let node = unsafe { &*self.node.as_ref().fields.get() };
        let Some(consumer) = node.consumer.as_ref() else {
            return Ok(None);
        };
        if (state.closing || unsafe { *consumer.pointer.as_ref().closing.get() }) != closing {
            return Err(ResidentFault::Closed);
        }
        let Some(record) = node.record.as_ref() else {
            return Ok(None);
        };
        ...
        unsafe {
            pointer.as_ref().aliases.try_update(Ordering::AcqRel, Ordering::Acquire, |count| count.checked_add(1)).map_err(|_| ResidentFault::Count)?;
        }
...
impl ErasedRecord {
    fn empty(&self) -> Result<bool, ResidentFault> {
        Ok(!self.initialized || unsafe { (self.empty)(self.pointer) })
    }
    fn aliases(&self) -> usize {
        if self.initialized {
            unsafe { (self.aliases)(self.pointer) }
        } else {
            0
        }
    }
...
unsafe fn record_empty<S>(pointer: NonNull<u8>) -> bool {
    unsafe { (&*pointer.cast::<RecordNode<S>>().as_ref().source.get()).is_none() }
}
unsafe fn record_aliases<S>(pointer: NonNull<u8>) -> usize {
    unsafe { pointer.cast::<RecordNode<S>>().as_ref().aliases.load(Ordering::Acquire) }
}
unsafe fn destroy_record<S>(pointer: NonNull<u8>) {
    unsafe {
        pointer.cast::<RecordNode<S>>().as_ptr().drop_in_place();
    }
}

pub struct ResidentRecord<'root, S> {
    root: &'root ResidentLedgerRoot,
    pointer: NonNull<RecordNode<S>>,
    consumer: NonNull<ConsumerHeader>,
}

impl<S> Drop for ResidentRecord<'_, S> {
    fn drop(&mut self) {
        unsafe {
            self.pointer.as_ref().aliases.fetch_sub(1, Ordering::AcqRel);
        }
    }
}
...
impl<S: Send + 'static> ResidentRecord<'_, S> {
    pub fn install(&self, source: &mut Option<S>, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        ...
        if state.closing || unsafe { *self.consumer.as_ref().closing.get() } {
            return Ok(ResidentStep::rejected());
        }
        let slot = unsafe { &mut *self.pointer.as_ref().source.get() };
        if slot.is_some() || source.is_none() {
            return Ok(ResidentStep::rejected());
        }
    ...
    pub fn handoff_into(&self, target: &mut Option<S>, grant: ResidentGrant) -> Result<ResidentStep, ResidentFault> {
        ...
        let slot = unsafe { &mut *self.pointer.as_ref().source.get() };
        if target.is_some() || slot.is_none() {
            return Ok(ResidentStep::rejected());
        }
        let bytes = size_of::<Option<S>>() as u64;
        if !grant.admits(bytes) {
