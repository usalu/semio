//! 🪗️ `#[value(flatten)]`, `#[value(with = "…")]`, and `#[value(skip)]` field attributes — a
//! genuine Cargo-registered integration crate, so `#[derive(ToValue, FromValue)]` runs exactly as any
...
#[test]
fn missing_option_field_defaults_to_none_like_serde() {
    let source = r#"{"required":"kept"}"#;
    let oracle: OptionalField = serde_json::from_str(source).expect("serde option default");
    let ours = OptionalField::from_value(DslValue::object([("required".to_string(), DslValue::String("kept".to_string()))])).expect("value option default");
