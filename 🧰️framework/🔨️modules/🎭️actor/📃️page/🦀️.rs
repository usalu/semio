...
impl ActorBytePage {
    ...
    pub fn try_from_array(bytes: [u8; ACTOR_BYTE_PAGE_BYTES], len: u32) -> Result<Self, &'static str> {
        if len > ACTOR_BYTE_PAGE_BYTES as u32 {
            return Err("actor-page.length");
        }
        if bytes[len as usize..].iter().any(|byte| *byte != 0) {
            return Err("actor-page.padding");
        }
    ...
    pub fn try_copy_from(bytes: &[u8]) -> Result<Self, &'static str> {
        if bytes.len() > ACTOR_BYTE_PAGE_BYTES {
            return Err("actor-page.length");
        }
    ...
    pub fn storage(&self) -> &[u8; ACTOR_BYTE_PAGE_BYTES] {
        &self.bytes
    }
