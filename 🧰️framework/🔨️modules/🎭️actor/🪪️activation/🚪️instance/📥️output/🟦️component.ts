import { OwnedResidentLedger, type OwnedResidentAdmission, type OwnedResidentRecord, type ResidentGrant, type ResidentStep } from "../../../../🌱️value/💾️resident/🟦️component.ts";
...
if (import.meta.vitest) {
  ...
  describe("OwnedActorTurnOutput", () => {
    ...
    it("ActorResponseAdmission binds its declared fields to the actual output source", async () => {
      const { default: contract } = await import("./🏘️admission/🧬️contract.json"); const ts = await import("typescript"); const { readFile } = await import("node:fs/promises"); const source = ts.createSourceFile("output.ts", await readFile(new URL("./🟦️component.ts", import.meta.url), "utf8"), ts.ScriptTarget.Latest, true, ts.ScriptKind.TS);
