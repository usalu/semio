...
impl ActorUiPatchReceipt {
    pub fn is_valid(self) -> bool {
        self.lifetime.is_valid() && self.patch_sequence != 0
    }
    ...
    pub fn encode(self, output: &mut [u8; ACTOR_UI_PATCH_RECEIPT_MAXIMUM_BYTES]) -> Result<usize, &'static str> {
        if !self.is_valid() {
            return Err("actor-patch.invalid-authority");
        }
        let mut offset = 0;
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, &'static str> {
        if bytes.is_empty() || bytes.len() > ACTOR_UI_PATCH_RECEIPT_MAXIMUM_BYTES {
            return Err("actor-patch.envelope");
        }
        let mut offset = 0;
        ...
        if offset != bytes.len() {
            return Err("actor-patch.trailing");
        }
