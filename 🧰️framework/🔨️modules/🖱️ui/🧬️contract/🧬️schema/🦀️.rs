 ...
 * the one camelCase holdout (`"virtualFileSystem"`); the rename reached the contract, the wgpu
 * target's own `SurfaceKind` and this projection, so there is exactly ONE spelling repo-wide.
 */
