...
impl TypedCopy for UiFixedBytes {
    ...
    fn copy_one(&self, candidate: &mut Self, _: &mut [usize], byte_candidate: &mut Vec<u8>, allocation: usize, work: usize) -> Result<UiComponentCopyProgress, PagedListAllocationError> {
        ...
        if byte_candidate.capacity() != UI_FIXED_BYTES {
            return Err(error("component byte candidate requires retirement after capacity rejection"));
        }
        ...
        if start < UI_FIXED_BYTES {
            ...
            if start < self.len() {
                byte_candidate.extend_from_slice(&self.bytes[start..start + bytes]);
            } else {
                byte_candidate.resize(start + bytes, 0);
            }
        ...
        if byte_candidate.len() != byte_candidate.capacity() {
            return Err(error("component byte candidate transfer would reallocate"));
        }
...
fn reserve_byte_candidate(candidate: &mut Vec<u8>, grant: usize, allocate: impl FnOnce(&mut Vec<u8>, usize) -> Result<(), ()>) -> Result<UiComponentCopyProgress, PagedListAllocationError> {
    ...
    if candidate.capacity() != 0 || !candidate.is_empty() {
        return Err(error("component byte candidate allocation requires empty ownership"));
    }
...
impl TypedCopy for UiValue {
    ...
    fn copy_one(&self, candidate: &mut Self, path: &mut [usize], byte_candidate: &mut Vec<u8>, _: usize, work: usize) -> Result<UiComponentCopyProgress, PagedListAllocationError> {
        ...
        if let Self::Text(source) = self {
            ...
            let Self::Text(target) = candidate else {
                return Err(error("typed text candidate changed variant"));
            };
...
impl<A: TypedCopy, B: TypedCopy> TypedCopy for (A, B) {
    ...
    fn allocation(&self, candidate: &Self, path: &[usize]) -> Result<usize, &'static str> {
        ...
        match index {
            ...
            1 => self.1.allocation(&candidate.1, path),
            _ => Ok(0),
        }
    }
    fn copy_one(&self, candidate: &mut Self, path: &mut [usize], byte_candidate: &mut Vec<u8>, allocation: usize, work: usize) -> Result<UiComponentCopyProgress, PagedListAllocationError> {
        ...
        match *index {
            ...
            1 => field(&self.1, &mut candidate.1, index, path, 2, byte_candidate, CopyGrant { allocation, work }),
            _ => Ok(done()),
        }
    }
}
...
impl<T: TypedCopy, const N: usize> TypedCopy for UiFixedList<T, N> {
    ...
    fn allocation(&self, candidate: &Self, path: &[usize]) -> Result<usize, &'static str> {
        ...
        match candidate.get(index) {
            Some(target) => source.allocation(target, path),
...
impl UiComponentCopy {
    pub fn new(source: Component) -> Self {
        Self { owned: ManuallyDrop::new(OwnedComponent { source: Some(source), candidate: None, byte_candidate: Vec::new() }), path: [0; COPY_DEPTH], retirement: Default::default(), complete: false, closing: false }
    }
    pub fn source(&self) -> Option<&Component> {
        if self.closing {
            None
        } else {
            self.owned.source.as_ref()
        }
    }
    pub fn candidate(&self) -> Option<&Component> {
    ...
    pub fn next_allocation_bytes(&self) -> Result<usize, &'static str> {
        ...
        match (&self.owned.source, &self.owned.candidate) {
            (Some(source), Some(target)) if !self.complete => source.allocation(target, &self.path),
            _ => Ok(0),
        }
    }
    ...
    pub fn take_completed(&mut self) -> Option<(Component, Component)> {
        if !self.complete || self.closing || self.owned.source.is_none() || self.owned.candidate.is_none() {
            return None;
        }
        let owned = &mut *self.owned;
    ...
    pub fn take_completed_source_with_grant(&mut self, bytes: usize) -> Option<Component> {
        if !self.complete || self.closing || bytes < size_of::<Component>() {
            return None;
        }
    ...
    pub fn terminal_is_empty(&self) -> bool {
        self.owned.source.is_none() && self.owned.candidate.is_none() && self.owned.byte_candidate.capacity() == 0 && (!self.closing || self.retirement.terminal_is_empty())
    }
}
...
impl UiTypedRetire for OwnedComponent {
    ...
    fn retire_typed(&mut self, path: &mut [u8], value: &mut Option<UiValueRetirement>, bytes: usize) -> Result<UiValueRetirementStep, &'static str> {
        ...
        let mut step = match *field {
            1 => self.candidate.retire_typed(path, value, bytes)?,
...
impl Drop for UiComponentCopy {
    fn drop(&mut self) {
        if !self.terminal_is_empty() && !std::thread::panicking() {
            panic!("component copy requires exact source and candidate retirement");
        }
    }
}
