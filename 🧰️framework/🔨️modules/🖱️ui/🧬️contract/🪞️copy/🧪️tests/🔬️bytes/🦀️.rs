...
fn source() -> Component {
    serde_json::from_value(serde_json::json!({"type":"surface","kind":"canvas-2d","docSchema":"wire","doc":{"bytes":[1,2,3]},"bindings":[]})).unwrap()
}
...
fn semantic(component: &Option<Component>) -> usize {
    match component {
        Some(Component::Surface(props)) => props.doc_schema.len() + props.doc.bytes.len(),
        None => 0,
        _ => unreachable!(),
    }
}
