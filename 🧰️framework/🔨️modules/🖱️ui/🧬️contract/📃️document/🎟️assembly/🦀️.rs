...
pub enum UiDocumentAssemblyErrorKind {
    Occupied,
    MissingSurface,
    InvalidGeneration,
    ArenaFull,
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct UiDocumentAssemblyError {
    pub kind: UiDocumentAssemblyErrorKind,
    pub allocated_bytes: usize,
    pub initialized_bytes: usize,
    pub compared_bytes: usize,
}

fn error(kind: UiDocumentAssemblyErrorKind) -> UiDocumentAssemblyError {
    UiDocumentAssemblyError { kind, allocated_bytes: 0, initialized_bytes: 0, compared_bytes: 0 }
}
fn resident_error(cause: UiResidentFault) -> UiDocumentAssemblyError {
    error(match cause {
        UiResidentFault::Contended => UiDocumentAssemblyErrorKind::Contended,
        UiResidentFault::Poisoned => UiDocumentAssemblyErrorKind::Poisoned,
        UiResidentFault::Owner => UiDocumentAssemblyErrorKind::Stale,
        _ => UiDocumentAssemblyErrorKind::Allocation,
    })
}
fn arena() -> Result<MutexGuard<'static, UiDocumentArena>, UiDocumentAssemblyError> {
    UI_DOCUMENT_ARENA.try_lock().map_err(|cause| {
        error(match cause {
            TryLockError::WouldBlock => UiDocumentAssemblyErrorKind::Contended,
            TryLockError::Poisoned(_) => UiDocumentAssemblyErrorKind::Poisoned,
        })
    })
}
...
impl UiDocumentAssembly {
    pub fn root_identity(&self) -> Option<UiDocumentRootIdentity> {
        self.builder.as_ref().and_then(|builder| builder.handle).map(UiDocumentRootIdentity)
    }
    pub const fn required_open_bytes() -> usize {
    ...
    pub fn open_with_permit(&mut self, permit: &mut Option<UiResidentPermit>, surface: &mut Option<SurfaceId>, identity: UiDocumentAssemblyIdentity, items: usize, bytes: usize) -> Result<UiDocumentAssemblyProgress, UiDocumentAssemblyError> {
        ...
        if self.builder.is_some() || self.closing {
            return Err(error(UiDocumentAssemblyErrorKind::Occupied));
        }
        if generation == 0 {
            return Err(error(UiDocumentAssemblyErrorKind::InvalidGeneration));
        }
        if surface.is_none() {
            return Err(error(UiDocumentAssemblyErrorKind::MissingSurface));
        }
        if items == 0 || bytes < Self::required_open_bytes() {
            return Ok(Default::default());
        }
        let key = permit.as_ref().and_then(UiResidentPermit::root_key).ok_or_else(|| error(UiDocumentAssemblyErrorKind::Stale))?;
        if permit.as_ref().unwrap().limits().bytes < Self::required_open_bytes() {
            return Err(error(UiDocumentAssemblyErrorKind::Allocation));
        }
        let mut arena = arena()?;
        let slot = &mut arena.slots[key.slot];
        if slot.occupied || slot.resident.is_some() || !slot.nodes.entries.terminal_is_empty() || slot.surface.is_some() {
            return Err(error(UiDocumentAssemblyErrorKind::Stale));
        }
    ...
    fn handle(&self) -> Result<UiDocumentHandle, UiDocumentAssemblyError> {
        if self.closing {
            return Err(error(UiDocumentAssemblyErrorKind::Closing));
        }
    ...
    pub fn place_one(&mut self, source: &mut Option<UiNodeRecord>, items: usize, bytes: usize) -> Result<UiDocumentAssemblyProgress, UiDocumentAssemblyError> {
        ...
        let Some(id) = source.as_ref().map(|record| record.id) else {
            return Ok(UiDocumentAssemblyProgress { complete: true, ..Default::default() });
        };
        let mut arena = arena()?;
        ...
        if self.compared < slot.nodes.len() {
            if bytes < 2 * size_of::<UiNodeId>() {
                return Ok(Default::default());
            }
            if slot.nodes.get_index(self.compared).is_some_and(|record| record.id == id) {
                return Err(UiDocumentAssemblyError { compared_bytes: 2 * size_of::<UiNodeId>(), ..error(UiDocumentAssemblyErrorKind::DuplicateNode) });
            }
            self.compared += 1;
        ...
        if !slot.nodes.entries.has_reserved_slot() {
            ...
            return result
                .map(|step| UiDocumentAssemblyProgress {
                    progressed: step.progressed,
                    metadata_items: usize::from(step.progressed),
                    allocated_bytes: step.allocated_bytes,
                    initialized_bytes: if step.progressed { initialized } else { 0 },
                    ..Default::default()
                })
                .map_err(|cause| UiDocumentAssemblyError { allocated_bytes: cause.allocated_bytes, initialized_bytes: if cause.allocated_bytes == 0 { 0 } else { initialized }, ..error(UiDocumentAssemblyErrorKind::Allocation) });
        }
        ...
        if source.is_none() {
            if slot.root == Some(id) {
                self.root_ordinal = Some(ordinal);
            }
            self.checking = None;
            self.compared = 0;
        }
    ...
    pub fn reprice_resident(&mut self, limits: UiResidentLimits, items: usize, bytes: usize) -> Result<UiDocumentAssemblyProgress, UiDocumentAssemblyError> {
        ...
        if limits.bytes < minimum || limits.items < slot.nodes.len() {
            return Err(error(UiDocumentAssemblyErrorKind::Allocation));
        }
    ...
    pub fn finish_into(&mut self, target: &mut Option<UiDocumentLease>, revision: UiRevision, items: usize, bytes: usize) -> Result<UiDocumentAssemblyProgress, UiDocumentAssemblyError> {
        ...
        let handle = self.handle()?;
        if items == 0 || bytes < size_of::<UiDocumentLease>() + size_of::<Self>() {
            return Ok(Default::default());
        }
        let mut arena = arena()?;
        let slot = arena.slot_mut(handle).ok_or_else(|| error(UiDocumentAssemblyErrorKind::Stale))?;
        if slot.retiring || slot.complete || self.checking.is_some() {
            return Err(error(UiDocumentAssemblyErrorKind::Closing));
        }
        let root = self.root_ordinal.and_then(|index| slot.nodes.get_index(index)).map(|record| record.id);
        if root.is_none() || root != slot.root {
            return Err(error(UiDocumentAssemblyErrorKind::MissingRoot));
        }
    ...
    pub fn close_step(&mut self, items: usize, bytes: usize) -> Result<crate::UiValueRetirementStep, &'static str> {
        if self.terminal_is_empty() {
            return Ok(crate::UiValueRetirementStep { complete: true, ..Default::default() });
        }
        if items == 0 || bytes == 0 {
            return Ok(Default::default());
        }
...
//#region 📖️ExactRead
pub struct UiDocumentRead<'a> {
    guard: MutexGuard<'a, UiDocumentArena>,
    handle: UiDocumentHandle,
}
impl UiDocumentRead<'_> {
    pub fn resident_limits(&self) -> UiResidentLimits {
        self.guard.slot(self.handle).unwrap().resident.as_ref().expect("readable canonical root retains its permit").limits()
    }
    pub fn len(&self) -> usize {
        self.guard.slot(self.handle).unwrap().nodes.len()
    }
    pub fn is_empty(&self) -> bool {
        self.len() == 0
    }
    pub fn allocated_bytes(&self) -> usize {
        self.guard.slot(self.handle).unwrap().nodes.entries.allocated_bytes()
    }
    pub fn node_at(&self, ordinal: usize) -> Option<&UiNodeRecord> {
        self.guard.slot(self.handle)?.nodes.get_index(ordinal)
    }
...
impl UiDocumentLease {
    pub fn root_identity(&self) -> Option<UiDocumentRootIdentity> {
        if self.released || self.claimed {
            None
        } else {
            self.handle.map(UiDocumentRootIdentity)
        }
    }
    ...
    pub fn try_read(&self) -> Result<UiDocumentRead<'_>, UiDocumentLeaseError> {
        ...
        let guard = UI_DOCUMENT_ARENA.try_lock().map_err(|cause| match cause {
            TryLockError::WouldBlock => UiDocumentLeaseError::Contended,
            TryLockError::Poisoned(_) => UiDocumentLeaseError::Poisoned,
        })?;
    ...
    pub fn try_alias_into(&self, target: &mut Option<Self>, bytes: usize) -> Result<bool, UiDocumentLeaseError> {
        if target.is_some() || bytes < size_of::<Self>() {
            return Ok(false);
        }
        if self.released || self.claimed {
            return Err(UiDocumentLeaseError::Closing);
        ...
        let handle = self.handle.ok_or(UiDocumentLeaseError::StaleHandle)?;
        let mut arena = UI_DOCUMENT_ARENA.try_lock().map_err(|cause| match cause {
            TryLockError::WouldBlock => UiDocumentLeaseError::Contended,
            TryLockError::Poisoned(_) => UiDocumentLeaseError::Poisoned,
        })?;
    ...
    pub fn same_root(&self, other: &Self) -> bool {
        self.handle.is_some() && self.handle == other.handle && !self.released && !other.released && !self.claimed && !other.claimed
    }
