...
pub struct UiResidentLimits {
    pub items: usize,
    pub bytes: usize,
}

#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub struct UiResidentSnapshot {
    pub items: usize,
    pub bytes: usize,
...
impl UiResidentFault {
    pub const fn reason(self) -> &'static str {
        match self {
            Self::Capacity => "resident capacity exhausted",
            Self::InvalidLimits => "resident limits exceed the fixed contract",
            Self::Contended => "resident ledger is busy",
            Self::Poisoned => "resident ledger is poisoned",
            Self::Owner => "resident permit is not the exact active owner",
            Self::CounterOverflow => "resident ledger counter overflow",
            Self::StaticBacking => "runtime resident backing differs from its admitted domain",
        }
    }
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) struct UiResidentKey {
...
struct ResidentSlot {
    epoch: u64,
    limits: UiResidentLimits,
    owners: u8,
}
impl ResidentSlot {
    const EMPTY: Self = Self { epoch: 0, limits: UiResidentLimits { items: 0, bytes: 0 }, owners: 0 };
}

struct ResidentLedger {
    slots: [ResidentSlot; UI_RESIDENT_SLOTS],
    snapshot: UiResidentSnapshot,
    cursor: usize,
    runtime_backing: usize,
}
...
fn ledger() -> Result<MutexGuard<'static, ResidentLedger>, UiResidentFault> {
    RESIDENT_LEDGER.try_lock().map_err(|error| match error {
        TryLockError::WouldBlock => UiResidentFault::Contended,
        TryLockError::Poisoned(_) => UiResidentFault::Poisoned,
    })
}
...
impl ResidentLedger {
    fn exact(&self, key: UiResidentKey, limits: UiResidentLimits, owner: u8) -> Result<&ResidentSlot, UiResidentFault> {
        ...
        if !matches!(owner, 1 | 2) || slot.epoch != key.epoch || slot.limits != limits || slot.owners & owner != owner {
    ...
    fn release(&mut self, key: UiResidentKey, limits: UiResidentLimits, owner: u8) -> Result<UiResidentProgress, UiResidentFault> {
        ...
        if remaining == 0 {
            ...
            step.returned_items = limits.items;
            step.returned_bytes = limits.bytes;
        }
...
//#region 🪪️Permit
#[derive(Debug)]
pub struct UiResidentPermit {
    pub(crate) key: Option<UiResidentKey>,
    limits: UiResidentLimits,
    owner: u8,
}

/// 🔎️ Read-only ledger observation neither constructs permits nor grants publication authority.
pub struct UiResidentObservation {
    guard: MutexGuard<'static, ResidentLedger>,
}
impl UiResidentObservation {
    pub fn snapshot(&self) -> UiResidentSnapshot {
        self.guard.snapshot
    }
    pub fn owns(&self, permit: &UiResidentPermit) -> bool {
        permit.key.is_some_and(|key| self.guard.exact(key, permit.limits, permit.owner).is_ok())
    }
}

impl UiResidentPermit {
    pub const fn required_reservation_bytes() -> usize {
        size_of::<Self>() + size_of::<ResidentSlot>()
    }
    pub const fn contract_backing_bytes() -> usize {
        CONTRACT_BACKING_BYTES
    }
    pub fn fixed_backing_bytes() -> Result<usize, UiResidentFault> {
        Ok(CONTRACT_BACKING_BYTES + ledger()?.runtime_backing)
    }
    ...
    pub fn try_register_runtime_backing(bytes: usize, admitted_bytes: usize) -> Result<bool, UiResidentFault> {
        if bytes == 0 || admitted_bytes < size_of::<usize>() * 2 {
            return Ok(false);
        }
    ...
    pub fn limits(&self) -> UiResidentLimits {
        self.limits
    }
    pub(crate) fn root_key(&self) -> Option<UiResidentKey> {
        if self.owner == 1 {
            self.key
        } else {
            None
    ...
    pub fn snapshot() -> Result<UiResidentSnapshot, UiResidentFault> {
        Ok(ledger()?.snapshot)
    }
    pub fn try_observe() -> Result<UiResidentObservation, UiResidentFault> {
        Ok(UiResidentObservation { guard: ledger()? })
    }
    pub fn has_pending_returns() -> bool {
        RETURNS.iter().any(|pending| pending.load(Ordering::Acquire) != 0)
    }
    ...
    pub fn try_reserve(limits: UiResidentLimits, target: &mut Option<Self>, admitted_bytes: usize) -> Result<bool, UiResidentFault> {
        if target.is_some() || admitted_bytes < Self::required_reservation_bytes() {
            return Ok(false);
        }
    ...
    pub fn split_output_into(&mut self, target: &mut Option<Self>, admitted_bytes: usize) -> Result<bool, UiResidentFault> {
        if target.is_some() || admitted_bytes < size_of::<Self>() {
            return Ok(false);
        }
        let key = self.key.ok_or(UiResidentFault::Owner)?;
        let mut ledger = ledger()?;
    ...
    pub fn close_step(&mut self, maximum_items: usize) -> Result<UiResidentProgress, UiResidentFault> {
        let Some(key) = self.key else {
            return Ok(UiResidentProgress { complete: true, ..Default::default() });
        };
        if maximum_items == 0 {
        ...
        let mut ledger = match ledger() {
            Ok(ledger) => ledger,
            Err(UiResidentFault::Contended) => return Ok(Default::default()),
    ...
    pub fn drain_one() -> Result<UiResidentProgress, UiResidentFault> {
        ...
        if owner == 0 {
            ...
            return Ok(UiResidentProgress { progressed: true, complete: ledger.snapshot.used_slots == 0, ..Default::default() });
        }
...
impl Drop for UiResidentPermit {
    fn drop(&mut self) {
        if let Some(key) = self.key.take() {
            ...
            if previous & self.owner != 0 && !std::thread::panicking() {
                panic!("resident affine permit returned twice");
            }
        }
    }
}
