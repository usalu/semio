...
fn assembled(generation: u64, bytes: usize) -> UiDocumentAssembly {
    ...
    assert!(!owner.open_with_permit(&mut permit, &mut surface, crate::UiDocumentAssemblyIdentity { generation, revision: UiRevision(1), root: Some(UiNodeId(41)), layout_epoch: 0 }, 1, 0).unwrap().progressed);
    assert!(permit.is_some() && surface.is_some());
    assert!(owner.open_with_permit(&mut permit, &mut surface, crate::UiDocumentAssemblyIdentity { generation, revision: UiRevision(1), root: Some(UiNodeId(41)), layout_epoch: 0 }, 1, 32768).unwrap().progressed);
    ...
    for _ in 0..100 {
        owner.place_one(&mut record, 1, 32768).unwrap();
        if record.is_none() {
            return owner;
        }
    }
...
fn retained_document_root_permit_nine_surfaces_share_one_aggregate() {
    ...
    for index in 0..count {
        roots.push(finish(&mut assembled(index as u64 + 1, bytes)));
    }
...
fn retained_document_root_permit_cancel_and_contended_final_return_keep_exact_owner() {
    ...
    drop(observation);
    for _ in 0..10000 {
        if candidate.close_step(1, 64).unwrap().complete {
            break;
        }
    }
    ...
    drop(candidate);
    for _ in 0..128 {
        close_ui_document_page_with_grant(1, 64).unwrap();
    }
...
fn retained_document_root_permit_reader_pressure_refuses_then_retries_exact_slot() {
    ...
    let mut roots: Vec<_> = (0..data["pressureRoots"].as_u64().unwrap())
        .map(|index| {
    ...
    for _ in 0..100000 {
        ...
        if UiResidentPermit::snapshot().unwrap().bytes < UI_RESIDENT_AGGREGATE_BYTES {
            break;
        }
    }
...
fn retained_document_root_permit_seal_transfers_output_without_detaching_root_credit() {
    ...
    for order in data["outputOrders"].as_array().unwrap() {
        ...
        if output_first {
            assert!(output.as_mut().unwrap().close_step(1).unwrap().complete);
        } else {
            close(&mut lease, 64);
        }
