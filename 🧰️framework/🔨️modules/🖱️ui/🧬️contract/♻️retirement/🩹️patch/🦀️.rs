...
impl UiPatchOps {
    ...
    pub fn capacity(&self) -> usize {
        self.storage.pages.len()
    }
    pub fn allocated_bytes(&self) -> usize {
        self.storage.allocated_bytes
    }
    pub fn has_reserved_slot(&self) -> bool {
        !self.closing && self.storage.pages.len() > self.storage.length
    }
    ...
    pub fn next_allocation_bytes(&self) -> Result<usize, &'static str> {
        if self.closing {
            return Err("patch storage is closing");
        }
        if self.len() == UI_DOCUMENT_PATCH_OPS {
            return Err("patch logical capacity exhausted");
        }
    ...
    pub fn try_reserve_one(&mut self, admitted_bytes: usize) -> Result<usize, UiPatchAllocationError> {
        ...
        if requested == 0 || admitted_bytes < requested {
            return Ok(0);
        }
        ...
        if self.storage.pages.capacity() == 0 {
            ...
            if actual > admitted_bytes {
                return Err(UiPatchAllocationError { allocated_bytes: actual, reason: "patch directory allocator exceeded admission; backing retained" });
            }
            return Ok(actual);
        }
        ...
        if actual > admitted_bytes {
            return Err(UiPatchAllocationError { allocated_bytes: actual, reason: "patch payload allocator exceeded admission; backing retained" });
        }
        Ok(actual)
    }
    ...
    pub fn try_push_reserved(&mut self, source: &mut Option<UiPatchOp>, physical_bytes: usize) -> Result<usize, &'static str> {
        ...
        if source.is_none() || physical_bytes < size_of::<UiPatchOp>() || !self.has_reserved_slot() {
            return Ok(0);
        }
    ...
    pub fn try_push(&mut self, value: UiPatchOp) -> Result<(), UiPatchOp> {
        ...
        while !self.has_reserved_slot() {
            ...
            if self.try_reserve_one(bytes).is_err() {
                return Err(source.take().unwrap());
            }
        }
        match self.try_push_reserved(&mut source, size_of::<UiPatchOp>()) {
            Ok(bytes) if bytes != 0 => Ok(()),
            _ => Err(source.take().unwrap()),
        }
    }

    pub fn get(&self, index: usize) -> Option<&UiPatchOp> {
        if self.closing || index >= self.len() {
            return None;
        }
    ...
    pub fn last_mut(&mut self) -> Option<&mut UiPatchOp> {
    ...
    pub fn release_empty_allocation(&mut self) -> Result<bool, &'static str> {
        if !self.is_empty() {
            return Err("patch payload must retire before backing release");
        }
...
impl PartialEq for UiPatchOps {
    fn eq(&self, other: &Self) -> bool {
        !self.closing && !other.closing && self.iter().eq(other.iter())
    }
}
...
impl std::ops::Index<usize> for UiPatchOps {
    ...
    fn index(&self, index: usize) -> &Self::Output {
        self.get(index).expect("patch index must identify a readable initialized operation")
    }
}
...
impl IntoIterator for UiPatchOps {
    ...
    fn into_iter(self) -> Self::IntoIter {
        assert!(!self.closing, "closing patch payload cannot be extracted");
        self.storage.pages.into_iter().flatten()
    }
}
...
impl Serialize for UiPatchOps {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        if self.closing {
            return Err(serde::ser::Error::custom("closing patch payload cannot be serialized"));
        }
    ...
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        impl<'de> serde::de::Visitor<'de> for Visitor {
            ...
            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("a bounded patch operation sequence")
            }
            fn visit_seq<A: serde::de::SeqAccess<'de>>(self, mut access: A) -> Result<Self::Value, A::Error> {
                ...
                while let Some(operation) = access.next_element()? {
                    if operations.try_push(operation).is_err() {
                        return Err(serde::de::Error::custom("patch allocation or logical capacity exhausted"));
                    }
                }
                Ok(operations)
            }
...
impl UiTypedRetire for UiPatchOps {
    ...
    fn retire_typed(&mut self, _: &mut [u8], _: &mut Option<UiValueRetirement>, bytes: usize) -> Result<UiValueRetirementStep, &'static str> {
        self.close_step(1, bytes)
    }
}
