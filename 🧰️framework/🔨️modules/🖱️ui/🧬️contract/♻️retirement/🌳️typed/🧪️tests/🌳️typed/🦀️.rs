...
fn node(component: crate::Component) -> crate::UiNodeRecord {
    crate::UiNodeRecord {
        id: crate::UiNodeId(1),
        key: UiText::try_from_str("node").unwrap(),
        component,
...
fn instance_lifetime_ui_document_terminal_includes_typed_value_descendants() {
    ...
    let record = crate::UiNodeRecord {
        id: crate::UiNodeId(row["nodeId"].as_u64().unwrap()),
        key: UiText::try_from_str("extension").unwrap(),
...
fn instance_lifetime_ui_typed_all_patch_variants_close_and_max_surface_progresses() {
    ...
    for grant in [1, 64, 4096] {
        ...
        let mut patches = [
            (UiPatchOp::Upsert(node(component())), 10),
            (UiPatchOp::SetComponent { id: UiNodeId(1), component: component() }, 6),
            ...
            (UiPatchOp::SetChildren { id: UiNodeId(1), children }, 0),
            (UiPatchOp::SetStyle { id: UiNodeId(1), style: Default::default() }, 0),
            ...
            (UiPatchOp::Remove { id: UiNodeId(1) }, 0),
            (UiPatchOp::SetRoot { id: UiNodeId(1) }, 0),
        ];
        ...
        for (patch, bytes) in &mut patches {
            assert_eq!(close_typed(patch, grant), *bytes);
        }
