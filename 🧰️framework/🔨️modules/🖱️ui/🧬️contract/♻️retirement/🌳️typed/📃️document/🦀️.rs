...
use super::*;
use crate::{UiArenaHandback, UiArenaHandbacks, UiTypedRetirementCursor, UiValueRetirementStep};
...
impl UiDocumentArena {
    fn consume_handback(&mut self, index: usize) -> Result<UiValueRetirementStep, &'static str> {
        ...
        let result = match obligation {
            UiArenaHandback::ReleaseAlias => {
                if !slot.occupied || slot.retiring || slot.aliases == 0 {
                    Err("returned document alias is not retained")
                } else {
                    slot.aliases -= 1;
                    if slot.aliases == 0 {
                        slot.retiring = true;
            ...
            UiArenaHandback::ReturnClaim => {
                if !slot.occupied || !slot.retiring || !slot.retirement_claimed {
                    Err("returned document claim is not retained")
                } else {
        ...
        if result.is_err() {
            DOCUMENT_HANDBACKS.record(index, obligation);
        }
        result
    }
    ...
    fn retire_exact(&mut self, handle: UiDocumentHandle, maximum_bytes: usize) -> Result<UiValueRetirementStep, &'static str> {
        ...
        let mut step = match slot.retire_scalar {
            ...
            5 => {
                if !slot.nodes.entries.terminal_is_empty() || slot.surface.is_some() {
                    return Err("document credit still retains typed roots");
                }
            ...
            6 => {
                if !slot.nodes.entries.terminal_is_empty() || slot.surface.is_some() || slot.resident.is_some() {
                    return Err("document terminal retains typed roots or credit");
                }
...
pub(super) fn close_document_owner(handle: &mut Option<UiDocumentHandle>, released: &mut bool, claimed: &mut bool, maximum_items: usize, maximum_bytes: usize) -> Result<UiValueRetirementStep, &'static str> {
    ...
    if !arena.slot(exact).unwrap().retirement_claimed {
...
pub(super) fn close_document_read_owner(handle: &mut Option<UiDocumentHandle>, released: &mut bool, claimed: &mut bool, maximum_items: usize, maximum_bytes: usize) -> Result<UiValueRetirementStep, &'static str> {
    ...
    if *released {
        return close_document_owner(handle, released, claimed, maximum_items, maximum_bytes);
    }
    let mut arena = match UI_DOCUMENT_ARENA.try_lock() {
        Ok(arena) => arena,
    ...
    if slot.retiring || slot.aliases == 0 {
        return Err("document read is not an exact live alias");
    }
    arena.release(exact);
    *released = true;
    let slot = arena.slot_mut(exact).unwrap();
    ...
    if slot.retirement_claimed {
        return Err("final document read encountered a foreign claim");
    }
    slot.retirement_claimed = true;
    *claimed = true;
    Ok(UiValueRetirementStep { progressed: true, released_items: 1, ..Default::default() })
...
pub(super) fn hand_back_document_owner(handle: Option<UiDocumentHandle>, released: bool, claimed: bool) {
    ...
    if !released {
        DOCUMENT_HANDBACKS.record(handle.slot, UiArenaHandback::ReleaseAlias);
    }
    if claimed {
        DOCUMENT_HANDBACKS.record(handle.slot, UiArenaHandback::ReturnClaim);
    }
}
...
pub fn close_ui_document_page_with_grant(maximum_items: usize, maximum_bytes: usize) -> Result<UiValueRetirementStep, &'static str> {
    ...
    let mut step = if slot.occupied && slot.retiring && !slot.retirement_claimed {
        ...
        UiValueRetirementStep { progressed: true, ..Default::default() }
    };
