...
pub(crate) enum UiArenaHandback {
    ReleaseAlias,
    ReturnClaim,
}
...
impl<const SLOTS: usize, const WORDS: usize> UiArenaHandbacks<SLOTS, WORDS> {
    ...
    pub(crate) fn record(&self, slot: usize, obligation: UiArenaHandback) {
        match obligation {
            UiArenaHandback::ReleaseAlias => {
                let previous = self.releases[slot].fetch_add(1, Ordering::AcqRel);
                assert!(previous != u64::MAX, "pending releases exceed admitted aliases");
            }
            UiArenaHandback::ReturnClaim => {
                assert!(!self.claims[slot].swap(true, Ordering::AcqRel), "one exact claim returned twice");
            }
        }
    ...
    pub(crate) fn next_slot(&self, start: usize) -> Option<usize> {
        ...
        if suffix != 0 {
            return Some(first_word * 64 + suffix.trailing_zeros() as usize);
        }
        for offset in 1..WORDS {
            let word = (first_word + offset) % WORDS;
            let ready = self.ready[word].load(Ordering::Acquire);
            if ready != 0 {
                return Some(word * 64 + ready.trailing_zeros() as usize);
            }
        }
    ...
    pub(crate) fn has_pending(&self) -> bool {
        self.ready.iter().any(|word| word.load(Ordering::Acquire) != 0)
    }

    pub(crate) fn has_slot_pending(&self, slot: usize) -> bool {
        self.claims[slot].load(Ordering::Acquire) || self.releases[slot].load(Ordering::Acquire) != 0
    }
