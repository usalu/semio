...
impl UiValueRetirement {
    ...
    pub fn close_step(&mut self, maximum_items: usize, maximum_bytes: usize) -> Result<UiValueRetirementStep, &'static str> {
        if self.terminal_is_empty() {
            return Ok(UiValueRetirementStep { complete: true, ..UiValueRetirementStep::default() });
        }
        ...
        if let Some(handle) = handle {
            self.root = {
                ...
                if UI_VALUE_HANDBACKS.has_slot_pending(handle.slot) {
                    return arena.consume_handback(handle.slot, &UI_VALUE_HANDBACKS);
                }
        ...
        if matches!(&*self.value, UiValue::Text(text) if !text.is_empty()) {
...
impl UiValueArena {
    pub(super) fn release_exact_handle(&mut self, handle: UiCollectionHandle) -> Result<Option<UiCollectionHandle>, &'static str> {
        ...
        if collection.retiring || collection.aliases == 0 {
            return Err("UI value owner already released");
        }
        if collection.aliases > 1 {
    ...
    fn link_retiring_root(&mut self, slot: usize) {
        ...
        if tail == UI_VALUE_NONE {
            self.retirement_head = slot;
        } else {
            self.collections[tail].retirement_next = slot;
        }
    ...
    pub(super) fn unlink_retiring_root(&mut self, slot: usize) {
        if !self.collections[slot].retirement_queued {
            return;
        }
        let previous = self.collections[slot].retirement_previous;
        let next = self.collections[slot].retirement_next;
        if previous == UI_VALUE_NONE {
            self.retirement_head = next;
        } else {
            self.collections[previous].retirement_next = next;
        }
        if next == UI_VALUE_NONE {
            self.retirement_tail = previous;
        } else {
            self.collections[next].retirement_previous = previous;
        }
    ...
    fn advance_exact_root(&mut self, root: UiCollectionHandle, maximum_bytes: usize, handbacks: &UiArenaHandbacks<UI_VALUE_ADMISSION_SLOTS, UI_VALUE_HANDBACK_WORDS>) -> Result<UiValueRetirementStep, &'static str> {
        ...
        if head == UI_VALUE_NONE {
            ...
            if !self.release_collection(cursor.slot) {
                return Err("UI value descendant release invariant failed");
            }
            if let Some(parent) = parent {
                self.collection_mut(root).ok_or("UI value root retired before its descendant")?.retirement_cursor = Some(parent);
            }
        ...
        if let Some(nested) = nested {
            if handbacks.has_slot_pending(nested.slot) {
                return self.consume_handback(nested.slot, handbacks);
            }
            let child = self.collection_mut(nested).ok_or("nested UI value owner is stale")?;
            if child.retiring || child.aliases == 0 {
                return Err("nested UI value owner already released");
            }
            child.aliases -= 1;
            ...
            let value = match self.pages[head].value.as_mut().unwrap() {
        ...
        if next == UI_VALUE_NONE {
            self.collections[cursor.slot].tail = UI_VALUE_NONE;
        }
        drop(value);
    ...
    fn consume_handback(&mut self, slot: usize, handbacks: &UiArenaHandbacks<UI_VALUE_ADMISSION_SLOTS, UI_VALUE_HANDBACK_WORDS>) -> Result<UiValueRetirementStep, &'static str> {
        ...
        let result = match obligation {
            ...
            UiArenaHandback::ReturnClaim => {
                ...
                if !collection.occupied || !collection.retirement_claimed || !collection.retiring {
                    Err("returned value claim is not retained")
                } else {
        ...
        if result.is_err() {
            handbacks.record(slot, obligation);
        }
        result
    }
    ...
    pub(super) fn advance_retirement_queue(&mut self, maximum_bytes: usize) -> Result<UiValueRetirementStep, &'static str> {
        ...
        if self.collection(root).is_some() {
...
mod tests;
#[cfg(test)]
#[path = "🌳️typed/🧪️tests/🌳️typed/🦀️.rs"]
mod typed_tests;
