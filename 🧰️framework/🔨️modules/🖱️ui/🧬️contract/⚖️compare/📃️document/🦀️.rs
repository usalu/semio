...
pub enum UiDocumentCompareError {
    Admission,
    Contended,
    Poisoned,
    Closing,
...
fn record_for_comparison<'a>(arena: &'a UiDocumentArena, lease: &UiDocumentLease, ordinal: usize, id: UiNodeId) -> Result<&'a UiNodeRecord, UiDocumentCompareError> {
    if lease.released || lease.claimed {
        return Err(UiDocumentCompareError::Closing);
    }
    ...
    if record.id != id {
        return Err(UiDocumentCompareError::NodeIdentity);
    }
    Ok(record)
}

struct OwnedDocumentComparison {
    lease: Option<UiDocumentLease>,
    incoming: Option<Component>,
}
...
impl UiDocumentComponentCompare {
    ...
    pub const fn required_admission_bytes() -> usize {
        size_of::<Self>() + size_of::<UiDocumentLease>() + size_of::<Component>()
    }
    ...
    pub fn try_new(lease: UiDocumentLease, ordinal: usize, id: UiNodeId, incoming: Component, admitted_bytes: usize) -> Result<(Self, UiDocumentCompareAdmission), (UiDocumentCompareError, UiDocumentLease, Component)> {
        if admitted_bytes < Self::required_admission_bytes() {
            return Err((UiDocumentCompareError::Admission, lease, incoming));
        }
        ...
        if let Err(error) = record_for_comparison(&arena, &lease, ordinal, id) {
            return Err((error, lease, incoming));
        }
    ...
    pub fn incoming(&self) -> Option<&Component> {
        if self.closing {
            None
        } else {
            self.owned.incoming.as_ref()
        }
    }

    pub fn advance(&mut self, items: usize, bytes: usize) -> Result<UiComponentCompareProgress, &'static str> {
        if self.closing {
            return Err("document component comparison is closing");
        }
        if items == 0 || bytes == 0 {
    ...
    pub fn take_completed(&mut self) -> Option<(UiDocumentLease, Component)> {
        if self.closing || self.cursor.result().is_none() {
            return None;
        }
    ...
    pub fn terminal_is_empty(&self) -> bool {
        self.owned.lease.is_none() && self.owned.incoming.is_none() && self.cursor.reads_empty() && (!self.closing || self.retirement.terminal_is_empty())
    }
}
...
impl Drop for UiDocumentComponentCompare {
    fn drop(&mut self) {
        if !self.terminal_is_empty() && !std::thread::panicking() {
            panic!("document comparison requires exact read and incoming retirement");
        }
    }
}
