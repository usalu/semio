...
fn retained_component_compare_frame_storage_matches_exact_bounded_domains() {
    ...
    assert_eq!(UI_VALUE_NESTING_DEPTH, frame["logicalDepth"].as_u64().unwrap() as usize);
...
fn retained_component_compare_cancellation_and_arena_contention_keep_both_roots() {
    ...
    let holder = std::thread::spawn(move || {
        let _arena = UI_VALUE_ARENA.lock().unwrap();
    ...
    for _ in 0..100 {
        if !owner.advance(1, 4096).unwrap().progressed {
            blocked = true;
            break;
        }
    }
