...
export class Builder {
  ...
  *component(value: unknown): Program<RetainedUiComponent> {
    ...
    switch (value.type) {
      ...
      case "treeItem": { const v = yield* this.record(value, ["type", "label", "description", "icon", "defaultOpen", "draggable", "dragData", "dimmed", "rowActions", "window", "granularity"]); const dragData = v.dragData == null ? null : yield* this.stringMap(v.dragData); const rowActions = yield* this.list(v.rowActions === undefined ? [] : v.rowActions, item => this.rowAction(item)); const window = yield* this.treeWindow(v.window); yield 128; return this.fixed({ type: "treeItem", label: text(v.label), description: optional(v.description, text), icon: optional(v.icon, text), defaultOpen: optional(v.defaultOpen, boolean), draggable: optional(v.draggable, boolean), dragData, dimmed: optional(v.dimmed, boolean), rowActions, window, granularity: optional(v.granularity, text) }); }
