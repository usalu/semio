...
export class Builder {
  ...
  *component(value: unknown): Program<RetainedUiComponent> {
    ...
    switch (value.type) {
      ...
      case "toggle": { const v = yield* this.record(value, ["type", "on", "icon", "text", "appearance"]); return this.fixed({ type: "toggle", on: boolean(v.on), icon: text(v.icon), text: optional(v.text, text), appearance: defaulted(v.appearance, "button", value => choice(value, ["button", "checkbox"])) }); }
      ...
      case "progress": { const v = yield* this.record(value, ["type", "completed", "total", "valueText"]); return this.fixed({ type: "progress", completed: number(v.completed), total: optional(v.total, number), valueText: text(v.valueText) }); }
      case "tree": { const v = yield* this.record(value, ["type", "interactionDomain", "presentation"]); return this.fixed({ type: "tree", interactionDomain: optional(v.interactionDomain, text), presentation: defaulted(v.presentation, "standard", value => choice(value, ["standard", "compact"])) }); }
      case "treeSection": { const v = yield* this.record(value, ["type", "label", "defaultOpen", "window"]); const window = yield* this.treeWindow(v.window); return this.fixed({ type: "treeSection", label: optional(v.label, text), defaultOpen: optional(v.defaultOpen, boolean), window }); }
      case "treeItem": { const v = yield* this.record(value, ["type", "label", "description", "icon", "defaultOpen", "draggable", "dragData", "dimmed", "rowActions", "window", "granularity", "inlineToolbar", "detail"]); const dragData = v.dragData == null ? null : yield* this.stringMap(v.dragData); const rowActions = yield* this.list(v.rowActions === undefined ? [] : v.rowActions, item => this.rowAction(item)); const window = yield* this.treeWindow(v.window); yield 128; return this.fixed({ type: "treeItem", label: text(v.label), description: optional(v.description, text), icon: optional(v.icon, text), defaultOpen: optional(v.defaultOpen, boolean), draggable: optional(v.draggable, boolean), dragData, dimmed: optional(v.dimmed, boolean), rowActions, window, granularity: optional(v.granularity, text), inlineToolbar: optional(v.inlineToolbar, natural), detail: optional(v.detail, natural) }); }
