...
export function interactivityMountedPreparedRenderSelfTests(repoRoot: string): void {
  ...
  const mutations: readonly [string, number, string, string][] = [
    ...
    ["whole-gpu-render", 2, "self.encode_prepared_draw_scalar(packet, draw_cursor, command.packet_overlay(), None)?", "self.render_prepared(packet)?"],
    ["whole-clipped-gpu-render", 2, "self.encode_prepared_draw_scalar(packet, draw_cursor, command.packet_overlay(), Some(scissor))?", "self.render_prepared(packet)?"],
    ["bypassed-gpu-timing", 2, "measure_prepared_gpu_opportunity(cursor, command, semio_framework_job::default_now_us", "untimed_gpu_opportunity(cursor, command, semio_framework_job::default_now_us"],
    ["early-gpu-timing-return", 2, "let complete = advance(cursor)?;", "return advance(cursor);"],
