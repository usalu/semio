// #region 🧲️Header
// 💻️ 🧰️framework/🔨️modules/🖱️ui/🎨️styling/📖️stories/🧭️coordination/🟦️.tsx
...
//#region 🔖️Color
/** @emoji 🎨️ CSS `rgba()` for an sRGB8888 tuple as resolved by `resolveThemePaint`/`resolveThemeAppearancePalettes`. */
export function rgba8ToCss([r, g, b, a]: Rgba8): string {
  return `rgba(${r}, ${g}, ${b}, ${(a / 255).toFixed(3)})`;
}

/** @emoji 🎨️ 6-digit hex for an sRGB8888 tuple's RGB channels (alpha is shown separately — swatch labels need both). */
...
//#region 🔖️Layout
/** @emoji 📄️ Shared page chrome for a docs-style gallery story (fullscreen layout, scrollable, theme-aware text color). */
...
//#region 🔖️SwatchGrid
/** @emoji 🧩️ One resolved paint name → sRGB8888 pair, as produced by `Object.entries(resolveThemeAppearancePalettes(theme, appearance)[group])`. */
export type SwatchEntry = readonly [string, Rgba8];

/** @emoji 🟪️ A titled grid of color swatches — the shared primitive for every `ThemePaletteGroup` rendered across the `styling` scope's stories. */
...
//#endregion 🔖️SwatchGrid

//#region 🔖️DataTable
/** @emoji 📊️ A minimal readable table for a generated-tokens record — used by `Tokens.stories.tsx` for colors/spacing/radii/strokes/opacities. */
