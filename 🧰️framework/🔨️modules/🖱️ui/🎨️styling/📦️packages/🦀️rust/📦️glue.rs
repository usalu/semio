...
pub mod color {
    ...
    pub async fn rgba8_to_linear(r: u8, g: u8, b: u8, a: u8) -> [f32; 4] {
        async fn ch(c: u8) -> f32 {
            let x = f64::from(c) / 255.0;
            let lin = if x <= 0.04045 { x / 12.92 } else { ((x + 0.055) / 1.055).powf(2.4) };
            lin as f32
        }
        [ch(r).await, ch(g).await, ch(b).await, f32::from(a) / 255.0]
    }

    /// @emoji 🌈️ Converts linear-sRGB `f32` components to sRGB8888 bytes.
    pub async fn linear_to_rgba8(lr: f32, lg: f32, lb: f32, la: f32) -> [u8; 4] {
        async fn ch(l: f32) -> u8 {
            let x = l as f64;
            let s = if x <= 0.0031308 { x * 12.92 } else { 1.055 * x.powf(1.0 / 2.4) - 0.055 };
            (s * 255.0).round().clamp(0.0, 255.0) as u8
        }
        [ch(lr).await, ch(lg).await, ch(lb).await, (f64::from(la) * 255.0).round().clamp(0.0, 255.0) as u8]
    }
}
// #endregion 🔖️Color
...
pub mod appearance {
    ...
    impl AppearanceName {
        pub async fn board(self) -> &'static BoardPalette {
            match self {
        ...
        pub async fn map(self) -> &'static MapPalette {
            match self {
        ...
        pub async fn canvas(self) -> &'static CanvasPalette {
            match self {
        ...
        pub async fn chrome(self) -> &'static ChromePalette {
            match self {
...
mod tests {
    ...
    #[test]
    async fn appearance_name_parse() {
