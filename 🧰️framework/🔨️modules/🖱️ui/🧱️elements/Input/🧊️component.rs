...
//! `crate::widgets` (declared `#[cfg(feature = "engine")] #[path = "..."] mod input_element;` right
...
//! `use crate::input_element::render_input;` so its own unqualified call sites keep working.
//! (Module named `input_element`, not `input`, to avoid shadowing the sibling `crate::input` mod.)
//! `crate::widgets::{...}` reaches the sibling items this needs (`WidgetContext`, `draw_text`,
//! `measure_text_width`); `crate::chrome`/`crate::input` are the other top-level engine mods
//! `widgets` itself also depends on.

use crate::widgets::{draw_text, measure_text_width, WidgetContext};
use crate::chrome::push_control_border;
use crate::input::{HitKind, HitTarget};

pub(crate) fn render_input<E: Clone>(id: &str, value: &str, placeholder: Option<&str>, bounds: crate::geometry::Rect, ctx: &mut WidgetContext<'_, E>) {
