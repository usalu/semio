...
//! `crate::widgets` (declared `#[cfg(feature = "engine")] #[path = "..."] mod button;` right before
...
//! `use crate::button::render_button;` so its own unqualified call sites keep working.
//! `crate::widgets::{...}` reaches the sibling items this needs (`WidgetContext`, `draw_text`);
//! `crate::chrome`/`crate::input` are the other top-level engine mods `widgets` itself also depends
//! on; `crate::IconName` is the crate-root re-export of the generated icon enum.
...
pub(crate) fn render_button<E: Clone>(id: Option<&String>, icon_id: Option<IconName>, label: &str, event: Option<E>, bounds: crate::geometry::Rect, ctx: &mut WidgetContext<'_, E>) {
