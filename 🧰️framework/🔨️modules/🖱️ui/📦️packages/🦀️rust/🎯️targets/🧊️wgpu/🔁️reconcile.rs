...
/// children of its own.
///
/// Two components own their subtree outright:
/// * `Surface` — "a surface's children are NOT rendered chrome; a renderer draws `props` and nothing
///   else" (`ui_contract`'s `🏗️builder.rs` `SurfaceBuilder`). Its children are the out-of-doc payload
///   lane carriers, which [`surface_scene_node`] folds back into the scene itself.
/// * `Tree` — `UiTreeNode` carries its sections and items INLINE, so the whole rows subtree is
///   assembled into the one spec `paint::retained_tree_node_step` paints. This is the same choice
///   React's `Interpreter` makes with `collectTreeItems`, which walks the store rather than
///   rendering the section/item records as their own elements.
fn record_consumes_subtree(record: &UiNodeRecord) -> bool {
    matches!(record.component, ui_contract::Component::Surface(_) | ui_contract::Component::Tree(_))
}
...
pub fn ui_node_from_record(document: &UiDocumentTree, record: &UiNodeRecord, surface: &str, controller: &str) -> UiNode {
    ...
    match &record.component {
        ...
        // 🌳️ A section/item record reached on its own (its `Tree` ancestor was not in this document)
        // still renders as a one-section tree rather than vanishing.
        ui_contract::Component::TreeSection(props) => UiNode::Tree(UiTreeNode {
            sections: vec![UiTreeSectionNode {
                id: record.key.as_str().to_string(),
                label: optional_contract_label(props.label.as_ref()),
                default_open: props.default_open,
                presence: presence.clone(),
                items: record.children.iter().filter_map(|item_id| document.record(*item_id)).map(|item| tree_item(document, item, surface, controller, 1)).collect(),
            }],
            presence,
            drop_action: None,
            menu,
        ...
        ui_contract::Component::TreeItem(_) => UiNode::Tree(UiTreeNode {
            sections: vec![UiTreeSectionNode { id: record.key.as_str().to_string(), label: None, default_open: None, presence: presence.clone(), items: vec![tree_item(document, record, surface, controller, 1)] }],
            presence,
            drop_action: None,
            menu,
            interaction_domain: None,
