//! 🔧️ Runs UI-axes codegen when the `wgpu` target's `🤖️generated.rs` is missing.
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if !generated.is_file() {
        ...
        if !status.success() || !generated.is_file() {
            return Err(format!("missing {} — run `bun nx run @semio-tech/ui-rs:generate` first", generated.display()).into());
        }
    }
    Ok(())
}
