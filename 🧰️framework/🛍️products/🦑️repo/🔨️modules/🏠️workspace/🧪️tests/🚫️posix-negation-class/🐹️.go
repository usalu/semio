...
func thePosixNegationSpellingIsAccepted(ctx *host.Context) (host.Outcome, error) {
	file, err := loadGlobVectors(ctx)
	if err != nil {
		return host.Outcome{}, err
	}
	return host.Outcome{Projection: map[string]any{"verdicts": globVerdictsFor(file.DivergentVectors)}}, nil
}
...
func Adapter() *host.Adapter {
	return host.NewAdapter("go").
		Subject("the-posix-negation-spelling-is-accepted", thePosixNegationSpellingIsAccepted)
}
