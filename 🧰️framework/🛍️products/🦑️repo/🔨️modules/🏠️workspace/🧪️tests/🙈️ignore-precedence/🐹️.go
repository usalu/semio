...
func vectorsAreIgnoredTheSameWay(ctx *host.Context) (host.Outcome, error) {
	file, err := loadIgnoreVectors(ctx)
	if err != nil {
		return host.Outcome{}, err
	}
	return host.Outcome{Projection: map[string]any{"verdicts": verdictsFor(file.Vectors)}}, nil
}
...
func Adapter() *host.Adapter {
	return host.NewAdapter("go").
		Subject("vectors-are-ignored-the-same-way", vectorsAreIgnoredTheSameWay)
}
