...
function runCorpus(ctx: any, keep: (entry: Corpus["queries"][number]) => boolean): { projection: unknown } {
  ...
  const records = JSON.parse(ctx.fixtureBytes("shared://🔣️repo-records.json").toString("utf8")) as Row;
  const rootValue = sources(records);
  const corpus = JSON.parse(ctx.fixtureBytes("local://🔣️queries.json").toString("utf8")) as Corpus;
