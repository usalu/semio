...
async function testContinuousServiceScenario(workspace: string, generated: string, mode: string, candidate?: string): Promise<void> {
  ...
  put("state/.keep", "");
  put("📜️script.ts", String.raw`import { appendFileSync, existsSync, writeFileSync, readFileSync } from "node:fs";
  ...
  const until = async test => { const deadline = Date.now() + 15000; while (!test()) { if (Date.now() > deadline) throw new Error("Fixture gate timeout: " + id); await Bun.sleep(20); } };
  if (process.argv[2] === "serve") {
    const server = Bun.serve({ port: 0, hostname: "127.0.0.1", fetch: () => new Response(id) });
    event("serve"); writeFileSync(path("ready"), String(server.port));
    const stop = () => { server.stop(true); event("closed"); process.exit(0); };
    process.once("SIGTERM", stop); process.once("SIGINT", stop); setTimeout(stop, 60000);
  } else {
...
const event = phase => appendFileSync("state/events.jsonl", JSON.stringify({ phase, id, pid: process.pid }) + "\n");
if (process.argv[2] === "prepare") openServiceSession(sessions, owner, pid);
else {
  ...
  try {
    if (process.argv[2] === "serve") {
      await serveVite({ root: resolve("app"), config: resolve("app/⚙️vite.config.ts"), host: "127.0.0.1", port: 0, signal: controller.signal, session, ready: url => {
        publishServiceReady(sessions, session, url); writeFileSync("state/ready-" + id, new URL(url).port); writeFileSync("state/session-" + id, JSON.stringify(session)); event("serve");
      } });
    } else {
      await waitForServiceReady(sessions, session, controller.signal, 20000); event("consumer");
      ...
      await waitForServiceReady(sessions, session, controller.signal, 1000); event("done");
    }
  } finally {
    clearTimeout(timer); process.removeListener("SIGTERM", cancel); process.removeListener("SIGINT", cancel);
    if (process.argv[2] === "serve") { closeServiceSession(sessions, session); event("closed"); }
  }
}
`);
  }
  ...
  const children: ReturnType<typeof Bun.spawn>[] = [], runs: Promise<number>[] = [];
  ...
  const run = (id: string) => {
    ...
    const completion = Promise.all([drain(child.stdout), drain(child.stderr), child.exited]).then(results => results[2]);
    runs.push(completion); return completion;
  };
  const until = async (test: () => boolean) => { const deadline = Date.now() + 25000; while (!test()) { assert.ok(Date.now() < deadline, "Continuous task was reused across invocation identities"); await Bun.sleep(50); } };
  let completed = false;
  console.log("[DEBUG] Native continuous task ownership fixture: " + root);
  try {
    const first = run("first"); await until(() => events().some(row => row.phase === "consumer" && row.id === "first"));
    const second = run("second"); await until(() => events().some(row => row.phase === "consumer" && row.id === "second"));
