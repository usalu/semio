...
export async function testNativeInventory(workspace: string, inventory: (root: string) => CacheInventory): Promise<CacheInventory> {
  ...
  assert.deepEqual(result.projects.map((project) => project.name).sort(), names, "Inventory must include every native Nx provider");
  for (const name of fixture.projects) assert.ok(names.includes(name), `Missing native project ${name}`);
  for (const tag of fixture.tags) assert.ok(result.projects.some((project) => project.tags?.includes(tag)), `Missing inferred ${tag} projects`);
