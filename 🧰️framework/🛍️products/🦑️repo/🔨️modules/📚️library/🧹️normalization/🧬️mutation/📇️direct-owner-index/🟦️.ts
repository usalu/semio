...
export function policyListMutationDirs(repoRoot: string, mutationsRel: string, view: MutationDirectoryView = MUTATION_DIRECTORY_VIEW): string[] {
  const taxonomy = loadCatalogTaxonomy(), domains = taxonomy.mutationDomainOwners[mutationsRel];
  ...
  if (domains) return Object.entries(domains).flatMap(([domain, operations]) => children(`${mutationsRel}/${domain}`).filter((entry) => Object.hasOwn(operations, entry.name)).map((entry) => `${domain}/${entry.name}`)).sort();
  const reserved = new Set<string>(["📚️examples", "💾️binary", "📝️text"]);
  return children(mutationsRel).filter((entry) => !reserved.has(entry.name) && !entry.name.startsWith(".")).map((entry) => entry.name).sort();
}
