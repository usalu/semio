...
export function runTaxonomyCliWorkflow(root: string, args: string[]): void {
  ...
  if (operation === "inventory") {
    ...
    if (ticketDir)
      publishTaxonomyInventoryArtifactShards(dirname(taxonomyCliArtifactPath(ticketDir, "inventory", "json")), inventory, (event) => {
  ...
  if (operation === "plan") {
    ...
    taxonomyCliWriteSummary(ticketDir, "plan", "Taxonomy Plan", [`- Baseline: \`${plan.baselineCommit}\``, `- Plan digest: \`${plan.planDigest}\``, ...taxonomyCliPlanOperationSummaryRows(counts), `- Unresolved: ${plan.unresolved.length}`]);
  ...
  if (operation === "apply") {
    ...
    if (options.format === "json") taxonomyCliPrintJson(result);
      ...
      console.log(
        `[clean taxonomy apply] state=${result.state} moves=${result.appliedMoves} relocations=${result.appliedEmbeddedTicketRootRelocations} symlinks=${result.appliedSymlinkTargetEdits} removals=${result.appliedEvidenceRemovals} edits=${result.appliedEdits} regenerations=${result.appliedRegenerations} journal=${result.journalPath}`,
      );
  ...
  taxonomyCliWriteSummary(ticketDir, "verify", "Taxonomy Verification", [`- Clean: ${verification.clean}`, `- Errors: ${errors}`, `- Warnings: ${warnings}`]);
