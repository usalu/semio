...
const vector = JSON.parse(readFileSync(join(import.meta.dir, "../🔎️json-reference-owner-lookup/🔣️.json"), "utf8"));
...
function typescriptOracle(content: string, adapter: string): Token[] {
  const document = ts.parseJsonText("../🔎️json-reference-owner-lookup/🔣️.json", content), rows: Token[] = [];
