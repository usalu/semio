...
const vectorText = readFileSync(join(import.meta.dir, "../🎯️draw-destination-observation/🔣️.json"), "utf8"), vector = JSON.parse(vectorText) as ObservationVector;
const schemaText = readFileSync(join(import.meta.dir, "../🎯️draw-destination-observation/🧬️schema/🔣️.json"), "utf8"), schema = JSON.parse(schemaText);
...
const inputPaths = [import.meta.filename, join(import.meta.dir, "../🎯️draw-destination-observation/🔣️.json"), join(import.meta.dir, "../🎯️draw-destination-observation/🧬️schema/🔣️.json"), catalogPath, authoredPath, authoredSchemaPath, join(libraryRoot, "🔣️taxonomy.json"), join(libraryRoot, "🔍️discovery/🟦️.ts"), join(import.meta.dir, "../🎯️draw-destination-observation/🧪️registration/🔣️.json"), join(import.meta.dir, "../🎯️draw-destination-observation/🧪️registration/🧬️schema/🔣️.json")];
...
test("Draw destination observation has the closed default-budget canonical registration", async () => {
  const directory = join(import.meta.dir, "../🎯️draw-destination-observation/🧪️registration"), bytes = readFileSync(join(directory, "../🎯️draw-destination-observation/🔣️.json"), "utf8"), registration = JSON.parse(bytes);
  const validate = new Ajv({ strict: true, allErrors: true }).compile(JSON.parse(readFileSync(join(directory, "../🎯️draw-destination-observation/🧬️schema/🔣️.json"), "utf8")));
