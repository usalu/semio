...
const vector = JSON.parse(readFileSync(join(repoRoot, library, "📦️packages/🟦️typescript/🧫️fixtures/🔣️artifact-support-leaf-authority.json"), "utf8")) as Vector;
...
function fixture(negative?: Negative): string {
  ...
  if (!negative?.omitDescriptor) {
    ...
    if (negative?.extraDescriptor) put(`${subset}/${dirname(vector.payloadDescriptor)}/🔣️.json`, descriptor);
  }
...
describe("artifact support leaf authority", () => {
  ...
  test("matches the language-neutral owner contracts and exact production inputs", () => {
    ...
    expect(taxonomy.mutationPayloadSchemaProjection).toEqual(vector.payloadAuthority);
  ...
  for (const negative of vector.negativeCases) test(`rejects unproven payload ownership: ${negative.id}`, () => {
    ...
    expect(actual.violations.some((problem) => problem.severity === "error")).toBe(true);
    expect(actual.normalizedPath).not.toBe(`${subset}/${row.destination}`);
  });
