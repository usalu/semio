...
const inputBytes = readFileSync(join(import.meta.dir, "../../🧫️fixtures/🪶️artifact-empty-facet-authoring/🔣️.json"), "utf8"), vector = JSON.parse(inputBytes) as Readonly<{ schemaVersion: number; contractId: string; sourceContractId: string; subsetSegments: string[]; leaf: string; customContent: string; customMode: number; directoryMode: number; surfaceLayout: string[]; subsetLayout: string[]; cases: Case[] }>;
...
const requestInput = JSON.parse(readFileSync(join(import.meta.dir, "../../🧫️fixtures/🪶️artifact-empty-facet-authoring/📨️request/🔣️.json"), "utf8")) as { schemaVersion: number; contractId: string; readBytes: number; readChunkBytes: number; cases: { id: string; leaves: string[]; action: string; error: string; ownedFiles: number; dryRun: boolean }[] };
...
test("registers empty-facet authoring through its closed canonical route", async () => {
  const directory = join(import.meta.dir, "../../🧫️fixtures/🪶️artifact-empty-facet-authoring/📋️registration"), bytes = readFileSync(join(directory, "🔣️.json"), "utf8"), registration = JSON.parse(bytes);
