...
import { taxonomyRelativePathIsExcluded } from "../../🔍️discovery/🟦️component.ts";
...
const source = ts.createSourceFile("📦️index.ts", readFileSync(join(repoRoot, library, "📦️packages/🟦️typescript/📦️index.ts"), "utf8"), ts.ScriptTarget.Latest, true);
...
test("registry catalog filesystem excludes opaque names before metadata enumeration", () => {
  const discovery = ts.createSourceFile("discovery.ts", readFileSync(join(repoRoot, library, "🔍️discovery/🟦️component.ts"), "utf8"), ts.ScriptTarget.Latest, true);
