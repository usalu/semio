...
async function stress(compiler: typeof compilers[number]) {
  ...
  writeNew(join(run, "../../🧫️fixtures/🔗️markdown-inline-references/🔣️.json"), JSON.stringify({ compiler: compiler.id, declarationSha256: sha(closure), sourceDeclarations: closure, limits: vector.limits, stress: vector.stress }, null, 2) + "\n");
