...
const vector = JSON.parse(readFileSync(join(import.meta.dir, "../🥤️rust-finite-target-consumption/🔣️.json"), "utf8"));
...
test("actual rustc proves cancelled symlink steps target different bytes from normalized lexical paths", () => {
  ...
  const binary = join(f.directory, "🧾️native", process.platform === "win32" ? "🔣️.exe" : "../🥤️rust-finite-target-consumption/🔣️.json");
