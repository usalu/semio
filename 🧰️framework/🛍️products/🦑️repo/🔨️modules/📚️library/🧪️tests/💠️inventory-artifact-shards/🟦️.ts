...
const vector = JSON.parse(readFileSync(join(import.meta.dir, "../💠️inventory-artifact-shards/🔣️.json"), "utf8"));
...
test("publication verifies canonical violations and rejects inconsistent replacement before writing", async () => {
  ...
  const source = inventory(vector.cases[0].rows), inputPath = join(root, "../💠️inventory-artifact-shards/🔣️.json");
