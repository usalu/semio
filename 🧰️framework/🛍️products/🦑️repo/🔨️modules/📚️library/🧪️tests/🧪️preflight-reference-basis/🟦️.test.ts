...
function environment(compiler: typeof compilers[number]) {
  ...
  const taxonomy = { exclusions: vector.semantics.opaqueRoots.map((name: string) => ({ path: name })) }, plan = { id: "frozen-plan" };
...
function physicalFixture(name: string) {
  const library = "🧰️framework/🛍️products/🦑️repo/🔨️modules/📚️library", schemaRelative = library + "/🔣️taxonomy.json", suitePath = join(root, library, "🧪️tests/🧪️transaction-v2/🟦️.test.ts");
...
test("the dedicated preflight gate is registered through Nx and both ordered launch catalogs", () => {
  ...
  expect(router).toContain("🧪️tests/🧪️preflight-reference-basis/🟦️.test.ts");
