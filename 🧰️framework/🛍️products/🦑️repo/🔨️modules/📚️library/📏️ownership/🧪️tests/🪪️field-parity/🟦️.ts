...
import { policyDiscoverArtifactSchemaOwners } from "../../../🧬️schema/🗿️artifact/🔍️owner-discovery/🟦️.ts";
import { policyArtifactOwnershipFieldParity } from "../../../🧬️schema/🗿️artifact/⚖️laws/🪪️ownership-field-parity/🟦️.ts";
import { policyArtifactSchemaBreaches } from "../../../🧬️schema/🗿️artifact/⚖️laws/📋️aggregate/🟦️.ts";
...
export function testArtifactFieldParityOracle(): void {
  for (const row of fixture.graphql) {
    ...
    assert.deepEqual(
      actual.fields.map((field) => field.name),
      names,
      row.name,
    );
    assert.deepEqual(
      actual.fields.map((field) => field.state),
      row.state,
      row.name,
    );
  }
  for (const row of fixture.typescriptModules) {
    ...
    const actual = policyExtractTypescriptSchemaFields(
      files["/root.ts"]!,
      "Fixture",
      (specifier, from) => {
    ...
    if ("states" in row && row.states) {
      const states = checker.getPropertiesOfType(checker.getDeclaredTypeOfSymbol(symbol!)).map(
        (field) =>
          field
            .getJsDocTags(checker)
            .find((tag) => tag.name === "state")
  ...
  for (const row of fixture.cases) {
    ...
    assert.deepEqual(
      policyExtractTypescriptSchemaFields(row.typescript, row.typeName).fields.map((field) => field.name),
      expected,
      row.name,
    );
  }
