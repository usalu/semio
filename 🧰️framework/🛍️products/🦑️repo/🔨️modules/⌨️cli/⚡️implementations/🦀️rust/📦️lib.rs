...
pub mod catalog {
    ...
    //#region 🔖️Discovery
    /// 🚫️ Directory names skipped while walking for plugin crates (build/vendor noise and any
    ...
    /// 🔍️ Finds every plugin/module crate `Cargo.toml` under `root` (excluding the plugin SDK itself).
    ...
    /// 🧾️ Parses one plugin/module crate manifest's registry-relevant fields.
    ...
    /// 🎮️ Parses every `[[package.metadata.semio.playground]]` row for one crate (examples unset;
    ...
    /// 📚️ Scans the whole workspace for plugin/module crates, sorted by `pluginId`.
    ...
    //#region 🔖️Validate
    /// 🚦️ Cross-checks the catalog for duplicate variants/aliases/ports and multi-app crate discipline.
    ...
    pub fn write_registry(root: &Path) -> std::io::Result<(usize, usize)> {
        ...
        fs::write(out_dir.join("🔣️plugins.json"), emit_plugins_json(&entries))?;
        fs::write(out_dir.join("🟦️plugins.ts"), emit_plugins_ts(&entries))?;
        fs::write(out_dir.join("🔣️playgrounds.json"), emit_playgrounds_json(&playgrounds))?;
        fs::write(out_dir.join("🟦️playgrounds.ts"), emit_playgrounds_ts(&playgrounds))?;
        Ok((entries.len(), playgrounds.len()))
    }

    /// 🔎️ Renders the catalog in memory and byte-compares it against `generated/*`; never writes.
...
"#;
        ...
        assert_eq!(assets[2].placeholder.as_deref(), Some("🧰️framework/🔨️modules/🖼️assets/⚡️implementations/🟦️typescript/🥽️mesh/🧊️placeholder.glb"));
