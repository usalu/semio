...
Feature: A read verb builds a GraphQL document, the engine executes it and the renderers print it
  ...
  local://🗄️repo-records.json, so a drift in the document a verb builds, in the event order
  ...
  Scenario: Every read verb's document executes and renders in all three formats
    Given the frozen repository local://🗄️repo-records.json and the vectors local://🔁️verb-queries.json
