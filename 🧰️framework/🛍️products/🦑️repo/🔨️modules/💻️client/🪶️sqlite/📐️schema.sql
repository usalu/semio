...
CREATE TABLE IF NOT EXISTS folder_kind (
    id INTEGER PRIMARY KEY, -- enum[🗃️organizational🛅️required]
    name TEXT NOT NULL UNIQUE CHECK (length (trim(name)) > 0), -- nn_text
...
CREATE TABLE IF NOT EXISTS file_kind (
    id INTEGER PRIMARY KEY, -- enum[💻️code🥼️lab📜️script📝️docs⚙️config💾️binary📋️template⚖️license]
    emoji TEXT NOT NULL UNIQUE CHECK (length (trim(emoji)) > 0), -- nn_text
    name TEXT NOT NULL UNIQUE CHECK (length (trim(name)) > 0), -- nn_text
    description TEXT NOT NULL CHECK (length (trim(description)) > 0) -- nn_text
);
...
CREATE TABLE IF NOT EXISTS technology_kind (
    id INTEGER PRIMARY KEY, -- enum[👤️user🧰️infrastructure🔬️research]
    emoji TEXT NOT NULL UNIQUE CHECK (length (trim(emoji)) > 0), -- nn_text
    name TEXT NOT NULL UNIQUE CHECK (length (trim(name)) > 0), -- nn_text
    description TEXT NOT NULL CHECK (length (trim(description)) > 0) -- nn_text
);
...
CREATE TABLE IF NOT EXISTS bundle_kind (
    id INTEGER PRIMARY KEY, -- enum[📚️library🛂️schema⌨️binary🖱️ui📔️examples🌐️site🏪️assets]
    name TEXT NOT NULL UNIQUE CHECK (length (trim(name)) > 0) -- nn_text
);
...
CREATE TABLE IF NOT EXISTS definition_kind (
    id INTEGER PRIMARY KEY, -- enum[🛠️implementation🔌️interface🪨️constant🧪️test]
    emoji TEXT NOT NULL UNIQUE CHECK (length (trim(emoji)) > 0), -- nn_text
    name TEXT NOT NULL UNIQUE CHECK (length (trim(name)) > 0), -- nn_text
    description TEXT NOT NULL CHECK (length (trim(description)) > 0) -- nn_text
);
...
CREATE TABLE IF NOT EXISTS client_kind (
    id INTEGER PRIMARY KEY, -- enum[🎗️vscode🖱️cursor🌊️cascade⚙️codex❄️claude-code🦾️droid]
    emoji TEXT NOT NULL UNIQUE CHECK (length (trim(emoji)) > 0), -- nn_text
    name TEXT NOT NULL UNIQUE CHECK (length (trim(name)) > 0), -- nn_text
    description TEXT NOT NULL CHECK (length (trim(description)) > 0) -- nn_text
);
