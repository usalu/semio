...
-- Persisted local-only: every row is derived from a local working-tree scan of one checkpoint and
-- never crosses the coordinator wire. Shared repo state lives in the repo server's PostgreSQL schema.
