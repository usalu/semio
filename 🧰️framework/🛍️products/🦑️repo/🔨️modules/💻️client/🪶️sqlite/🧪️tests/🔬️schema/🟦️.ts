...
const sql = readFileSync(join(moduleRoot, "🗄️.sql"), "utf-8");
const document = JSON.parse(readFileSync(join(moduleRoot, "🔣️.json"), "utf-8")) as {
  $schema: string;
  $id: string;
  $defs: Record<string, { type?: string; required?: string[]; properties?: Record<string, Record<string, unknown>>; "x-semio-persistence"?: string }>;
};

/** 🚧️ Leading keywords of a table constraint clause, anchored on a word boundary so the `checkpoint` column is not read as `CHECK`. */
const TABLE_CONSTRAINT = /^(?:PRIMARY KEY|UNIQUE|FOREIGN KEY|CHECK|CONSTRAINT)\b/i;
...
/** 🪶️ Reads every table of the native SQLite implementation with its column nullability. */
...
describe("repo client sqlite persistence module", () => {
  ...
  it("implements the entity set the client exports", () => {
    expect([...tables.keys()].sort()).toEqual(["bundle", "definition", "file", "folder", "repo", "section", "technology"]);
  });
  ...
  it("compiles under an independent draft-07 validator", () => {
    ...
    const validate = new AjvConstructor({ strict: false }).compile({ $ref: `${document.$id}#/$defs/FolderRow`, definitions: {}, ...document });
    expect(validate({ id: 1, parent_folder_id: null, kind: 0, name: "🔨️modules", summary: null })).toBe(true);
    expect(validate({ id: 1, parent_folder_id: null, kind: 0, name: null, summary: null })).toBe(false);
    expect(validate({ id: 1, parent_folder_id: null, kind: 0, name: "🔨️modules", summary: null, emoji: "🔨️" })).toBe(false);
  });
  ...
  it("declares the persistence class of every row", () => {
    for (const table of tables.keys()) expect(document.$defs[exportId(table)]["x-semio-persistence"], exportId(table)).toBe("local-only");
  });
});
//#endregion 🧪️Module

//#region 🧪️Parity
describe("column parity between 🗄️.sql and 🔣️.json", () => {
