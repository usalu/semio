...
//   bun 🖥️host/🟦️.ts --plan <plan.json> --out <results.jsonl> [--adapter <abs path>]
...
export function parseHostArgs(argv: readonly string[]): HostArgs {
  ...
  if (planPath === null || outPath === null) throw new Error("usage: bun 🖥️host/🟦️.ts --plan <plan.json> --out <results.jsonl> [--adapter <path>]");
