//! 🖥️ The interactive terminal dashboard: wizard-built commands in tiled pseudo-terminal windows.

use crate::command_tree::{action_key, CommandLeaf, CommandNode, CommandSpec, RepoAction};
...
impl Dashboard {
    ...
    fn spawn_output(&mut self, tui: &mut Tui, win_id: &str, spec: CommandSpec) {
        let (term_id, term_size) = self.open_output(tui, win_id, format!("{} {}", spec.cmd, spec.args.join(" ")));
        ...
        let session = match Pty::spawn(&spec.cmd, &args, &env_refs, Some(spec.cwd.as_path()), pty_size) {
            Ok(pty) => Some(PtySession { pty }),
            Err(e) => {
                Dashboard::feed(tui, term_id, &format!("[semio] spawn failed: {e}\n"));
                None
            }
        };
    ...
    fn show_repo_output(&mut self, tui: &mut Tui, win_id: &str, action: &RepoAction) {
        let (term_id, _) = self.open_output(tui, win_id, action_key(action));
        let answer = action.execute(&self.root);
        Dashboard::feed(tui, term_id, &answer);
    }
            ...
            WidgetSignal::Activated(i) => {
                ...
                if let Some(child) = parent.children.get(i) {
                    if child.children.is_empty() {
                        match child.leaf.clone() {
                            Some(CommandLeaf::Process(spec)) => self.spawn_output(tui, win_id, spec),
                            Some(CommandLeaf::Repo(action)) => self.show_repo_output(tui, win_id, &action),
                            None => {}
                        }
...
pub fn run(root: &Path) -> i32 {
    ...
    let mut dash = Dashboard { root: root.to_path_buf(), tree, layout, shell, windows: Vec::new(), next_serial: 2, focused: "w1".into(), leader: LeaderMode::Idle, terminal_input: false };
