...
Feature: One coordinator event has exactly one canonical line and one checksum
  ...
  as text, in `🧫️fixtures/🧬️g3-event-schema.json`, and the one golden record that satisfies them is
  `🧫️fixtures/📜️g3-event-log.jsonl`. Node's crypto reaches OpenSSL and decides the checksum.

  @id-golden-line-is-canonical
  @level-fundamental
  @mode-conformance
  Scenario: The golden record re-encodes to exactly the committed line
    Given the golden log shared://📜️g3-event-log.jsonl and the schema shared://🧬️g3-event-schema.json
