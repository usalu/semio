...
} from "../../🎛️coordinator/🔌️ports/🟦️.ts";
// #endregion 🔌️Adapters

// #region 🔖️db
// #region ⏱️Config
// 🗄️Database configuration from environment variables.
export const DATABASE_URL = process.env.DATABASE_URL || "postgresql://compose:compose@localhost:5432/compose_repo";
...
export async function listConflicts(): Promise<{ scope_id: string; tickets: string[] }[]> {
  ...
  return (result.rows as { scope_id: string; tickets: string[] }[]).map((r) => ({
    scope_id: r.scope_id,
    tickets: r.tickets,
  }));
}
...
export async function listContributorsOnItem(kind: string, itemId: string): Promise<string[]> {
  ...
  return (result.rows as { github: string }[]).map((r) => r.github);
}
