...
export type AutoAnimateMatcherHost = {
...
export function elementIsTargetGhostAnchor(element: HTMLElement): boolean {
  return element.classList.contains("presentation-target-ghost") || element.closest(".presentation-target-ghost") !== null;
}

export function elementIsSourceGhostAnchor(element: HTMLElement): boolean {
...
export function elementIsInteractiveFigureDisposition(element: HTMLElement): boolean {
...
function emphasisClass(emphasis: ParticipantEmphasis | undefined): string | undefined {
...
const pdfWorkerReady = ephemeralBox<Promise<void> | null>("framework.products.presentation.targets.react.pdfWorkerReady", null);
...
function usePdfContainerSize(anchorRef: RefObject<HTMLDivElement | null>, position: DispositionPosition | undefined, slideEpoch: number, enlarged: boolean): { readonly width?: number; readonly height?: number } {
  ...
  useEffect(() => {
    ...
    const measureTarget = (): HTMLElement | null => {
      if (enlarged) {
        return el.closest(".presentation-interactive-disposition--enlarged")?.querySelector<HTMLElement>(".presentation-interactive-disposition__content") ?? null;
      }
      const frame = el.closest<HTMLElement>(".presentation-disposition-frame");
  ...
  for (const ghost of toSlide.querySelectorAll<HTMLElement>(".presentation-target-ghost.presentation-interactive-disposition--canvas-framed")) {
    ...
    if (slot && cropRaw) {
      try {
        ...
        applyFigureCropCssVars(slot, figureCropBackgroundVarsTargetGhost({ kind: "figure", id: "", src: "", crop }, crop, sourceFrame, labelFrame));
      } catch {
...
export function figureCropBackgroundVars(embodiment: FigureEmbodiment, crop: DispositionPosition, frame?: DispositionPosition, morphToFrame?: DispositionPosition, fromMorphToFrame?: DispositionPosition, slideAspect?: number): CSSProperties {
  ...
  const gridFrom = fromMorphToFrame
    ...
    : undefined;
  const vars: CSSProperties & Record<string, string | number | undefined> = {
...
function FigureImageMorphView({ morphId: anchorId, embodiment, emphasis, position }: { readonly morphId: string; readonly embodiment: FigureEmbodiment; readonly emphasis: ParticipantEmphasis; readonly position?: DispositionPosition }): ReactNode {
  ...
  const coverScroll = figureCoverScrollContentSizeAtZoom(effectivePortSize.width, effectivePortSize.height, sourceAspect, 1);
...
function MarkdownMorphView({ morphId: anchorId, embodiment, emphasis }: { readonly morphId: string; readonly embodiment: MarkdownEmbodiment; readonly emphasis: ParticipantEmphasis }): ReactNode {
  ...
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      const markdown: string = embodiment.markdown ?? (await fetch(resolvePresentationAssetUrl(embodiment.src)).then((response) => response.text()));
...
export function suppressRevealOverviewSlideNavigation(event: Event): void {
  ...
  const swallowClick = (clickEvent: Event): void => {
...
export function transformFrameStyle(transform: DispositionPosition): CSSProperties {
  return {
    position: "absolute",
...
const InteractionLayer: FC<{
  ...
  const marqueeStyle: { readonly left: string; readonly top: string; readonly width: string; readonly height: string } | undefined = marquee
    ? (() => {
...
export const PresentationDeck: FC<{
  ...
  useEffect(() => {
    ...
    const scheduleFinalizeAutoAnimateRest = (): void => {
      const configuredDuration = deck.getConfig().autoAnimateDuration;
      const durationSeconds = typeof configuredDuration === "number" ? configuredDuration : 1;
...
if (import.meta.vitest) {
  ...
  await registerPresentationReactDeckTests(import.meta.vitest, { FIGURE_WHEEL_ZOOM_MAX, jsonPreview, PRESENTATION_MANY_TO_ONE_MORPH_CLASS, PresentationDeck, SLIDE_INTERACTIVE_ENLARGE_FRAME, buildInteractiveSlideLayout, clearRevealAutoAnimateInlineLayout, clientToSectionFraction, dispositionHasEphemeralLayout, dispositionPlacementContainer, elementIsInteractiveFigureDisposition, elementIsSourceGhostAnchor, elementIsTargetGhostAnchor, figureBackgroundSizeScrollAxis, figureBackgroundSizeZoomed, figureCoverOverflowAxis, figureCoverScrollContentSize, figureCoverScrollElementStyle, figureCropBackgroundPosition, figureCropBackgroundSize, figureCropBackgroundVars, figureCropBackgroundVarsTargetGhost, figureCropScrollBackgroundSize, figureEmbodimentScrollEnabled, figureScrollOffsetForBackgroundPosition, figureScrollOverlayThumbMetrics, figureWheelZoomStep, finalizeRevealAutoAnimateRestState, flowDispositionManipulationRect, flowDispositionOffsetStyle, flowPixelOffsetToSectionRect, flowPointerDeltaToLocal, groupBoundingRect, interactiveDispositionChromeStyle, interactiveDispositionContentScale, interactiveDispositionContentScaleStyle, isFlowPixelOffsetTransform, isManyToOneMorphTransition, isNormalizedSlideFrame, isRevealAutoAnimatePairSource, isRevealSlideAutoAnimating, isSectionAutoAnimating, isUsableMeasuredRect, marqueeSelectionRule, marqueeSelects, measureDispositionBoundsInContainer, measureDispositionBoundsInSection, mediaTeaserActive, morphFrameCssVars, mosaicWindowedCoverVars, mountPresentation, normalizeMarquee, patchAutoAnimateUniformScale, patchPresentationAutoAnimateRunStyleSheet, patchPresentationAutoAnimateStyleSheet, pdfAdjacentPage, pdfCanGoToNextPage, pdfCanGoToPreviousPage, pdfCoverScale, pdfEmbodimentInitialPage, pdfPageNavEnabled, pdfScrollCoverScale, pointerNearSlideResetHotspot, prepareArrangementBeforeAutoAnimate, presentationAutoAnimateMatcher, readPresentationSlideIndicesFromUrl, rectContains, rectsIntersect, relaxHiddenPreflight, renderJsonTree, resetPdfCanvasPort, resizeDispositionRect, resolvePresentationAssetUrl, resolvePresentationAutoAnimateRunSlides, resolveRevealArrangement, revealDeckInOverview, revealInkMeasureForAutoAnimate, revealTextAutoAnimatePairOptions, scaleRectWithinGroup, setPdfCanvasPort, slideCoordinateRoot, slideHasEphemeralLayout, slidesShareAutoAnimateId, suppressRevealOverviewSlideNavigation, syncArrangementSettledState, syncManyToOneGhostMorphFramesFromDom, syncPresentationSlideUrl, tightElementBoundsRect, toggleEnlargeRect, transformFrameStyle, translateDispositionRect, unmountPresentation }, { directory: import.meta.dir, url: import.meta.url });
}
//#endregion 🧪️Tests
