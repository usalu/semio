...
if (import.meta.vitest) {
  ...
  const globalsCssSource = readFileSync(join(dirname(fileURLToPath(import.meta.url)), "🎨️.css"), "utf8");
  ...
  describe("resolveRevealArrangement", () => {
    ...
    it("keeps target-ghost rest crop on the source tile frame and morph crop on the label slot", () => {
      ...
      const embodiment = { kind: "figure" as const, id: "", src: "/catalogue.png", crop };
  ...
  describe("PresentationDeck", () => {
    ...
    it("animates figure crop and target-ghost frames only during many-to-one auto-animate", async () => {
      ...
      const { fileURLToPath } = await import("node:url");
      const css = readFileSync(resolve(dirname(fileURLToPath(import.meta.url)), "🎨️.css"), "utf8");
    ...
    it("rests morph-source ghosts with opacity only so reveal can measure FLIP targets", async () => {
      ...
      const { dirname, resolve } = await import("node:path");
      const { fileURLToPath } = await import("node:url");
      const cssPath = resolve(dirname(fileURLToPath(import.meta.url)), "🎨️.css");
    ...
    it("uses cover for full image and uniform crop zoom for partial crops", () => {
      ...
      const square = figureCropBackgroundVars({ kind: "figure", id: "", src: "/catalogue.png", crop }, crop, { x: 0, y: 0, width: 0.5, height: 0.5 });
      const wide = figureCropBackgroundVars({ kind: "figure", id: "", src: "/catalogue.png", crop }, crop, { x: 0, y: 0, width: 1, height: 0.25 });
    ...
    it("uses CSS cover for full catalogue in the catalogue frame", () => {
      ...
      const vars = figureCropBackgroundVars({ kind: "figure", id: "", src: "/🖼️bauteilbörse.png", crop, sourceAspect }, crop, frame);
  ...
  describe("presentation interaction dom", () => {
    ...
    it("resizes flow disposition from se handle when nested reveal section has zero height", () => {
      ...
      expect(disposition.querySelector<HTMLElement>(".presentation-interactive-disposition__content")?.style.transform).toContain("scale(");
    });
    ...
    it("keeps other canvas dispositions on their declared frames while one is dragged", () => {
      ...
      expect(dispositions[0]!.querySelector<HTMLElement>(".presentation-interactive-disposition__content")?.style.transform).toBe("");
    });
    ...
    it("shows tile disposition drag preview outside the declared frame without clipping", () => {
      ...
      expect(tile.querySelector<HTMLElement>(".presentation-interactive-disposition__content")?.style.transform).toBe("");
    });
