...
import { ANALOGY_PARTICIPANT_VISUAL, FIGURE_TILE_PDF_PAGE_ASPECT, INTRO_EMBODIMENT_AUTHORS_PLAIN, INTRO_EMBODIMENT_DESCRIPTION_SHORT, INTRO_PARTICIPANT_AUTHORS, INTRO_PARTICIPANT_DESCRIPTION, INTRO_PARTICIPANT_INSTITUTIONS, MEDIA_SCROLL_ORIGIN_CENTER, MEDIA_SCROLL_ORIGIN_TOP_LEFT, PRESENTATION_DEFAULT_SLIDE_ASPECT, abbreviateAuthorFirstName, affiliationEmbodimentMorphLabels, affiliationLineName, analogy, arrangementRestDispositions, authorLinesForAffiliationStep, buildResolutionScope, buildTileMorphPrompt, centerResolvedArrangement, collectPresentationSlides, expandThoughtSlides, figureFrameForSourceAspect, figureTileMediaKindFromFile, formatPresentationSlideHash, formatPresentationUrlHash, intro, introThoughtFile, isFigureTileMediaFile, isIntroArrangementId, loadPresentationFromSlideGlob, mediaScrollPercentForAxis, morphId, moveNormalizedRect, parseGridEngagement, parsePresentationSlideFilePath, parsePresentationSlideHash, parsePresentationThoughtFilePath, populateTileDraftsFromGrid, presentationSequences, presentationSlideAt, presentationSlideBookmarkParamKeys, presentationSlideFilePath, presentationThoughtFilePath, resizeNormalizedRect, resolveArrangement, resolveEmbodiment, resolveMediaScrollOrigin, resolveTextMorphRoot, split, splitFigureGrid, tile, unionSourceCrops, type Arrangement, type FigureTileDraft, type FigureTileSource, type Presentation, type ResolutionScope, type ResolvedDisposition, type TextEmbodiment, type Thought } from "../../📦️packages/🟦️typescript/🟦️.ts";
...
describe("parsePresentationSlideFilePath", () => {
	it("round-trips canonical slide module paths", () => {
		...
		expect(path).toBe("🎞️slide/Hauptteil/Einführung/Einleitung/Titel.ts");
...
describe("parsePresentationThoughtFilePath", () => {
	it("round-trips canonical thought template paths", () => {
		...
		expect(path).toBe("🎞️slide/Hauptteil/Einführung/Einleitung.ts");
