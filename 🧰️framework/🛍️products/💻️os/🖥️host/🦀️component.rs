    ...
    mod tests {
        ...
        fn hot_swap_bumps_instance_generation_and_tracks_app_changes() {
            ...
            host.load_plugin(LoadedProgram {
                plugin_id: "draw".into(),
                manifest: PluginManifest { plugin_id: "draw".into(), label: "Draw".into(), version: "0.1.0".into(), apps: vec![draw_app.clone()], capabilities: vec![], contributions: vec![], topic_contributions: vec![], examples: vec![], commands: vec![], artifact_kinds: vec![] },
            ...
            let event = host.hot_swap_plugin(LoadedProgram {
                plugin_id: "draw".into(),
                manifest: PluginManifest { plugin_id: "draw".into(), label: "Draw".into(), version: "0.2.0".into(), apps: vec![draw_app, note_app], capabilities: vec![], contributions: vec![], topic_contributions: vec![], examples: vec![], commands: vec![], artifact_kinds: vec![] },
                artifact_uri: "program://draw".into(),
            });
        ...
        fn hot_swap_rollback_on_invalid_manifest_keeps_old_plugin() {
            ...
            host.load_plugin(LoadedProgram {
                plugin_id: "draw".into(),
                manifest: PluginManifest { plugin_id: "draw".into(), label: "Draw".into(), version: "0.1.0".into(), apps: vec![draw_app], capabilities: vec![], contributions: vec![], topic_contributions: vec![], examples: vec![], commands: vec![], artifact_kinds: vec![] },
                artifact_uri: "program://draw".into(),
            });
            let instance_id = host.create_instance("draw-play", "{}".into()).expect("instance");
            let generation_before = host.instance(instance_id).expect("instance").generation;
            let event = host.hot_swap_plugin(LoadedProgram {
                plugin_id: "draw".into(),
                manifest: PluginManifest { plugin_id: "draw".into(), label: "Draw".into(), version: "".into(), apps: vec![], capabilities: vec![], contributions: vec![], topic_contributions: vec![], examples: vec![], commands: vec![], artifact_kinds: vec![] },
                artifact_uri: "program://draw".into(),
            });
            assert_eq!(event.plugin_id, "draw");
...
pub mod media_export_raster {
    ...
    #[cfg(feature = "os-host-full")]
    pub use crate::workflow::{register_os_media_export_handler, register_os_media_import_handler, OsMediaExportResult, export_os_app_instance_media, export_os_app_instance_media_kind, import_os_app_instance_media, import_os_app_instance_media_kind};
    ...
    #[cfg(not(feature = "os-host-full"))]
    static OS_MEDIA_EXPORT_HANDLERS: LazyLock<Mutex<std::collections::HashMap<(String, MediaFormat), OsMediaExportHandler>>> =
        LazyLock::new(|| Mutex::new(std::collections::HashMap::new()));
    #[cfg(not(feature = "os-host-full"))]
    static OS_MEDIA_IMPORT_HANDLERS: LazyLock<Mutex<std::collections::HashMap<(String, MediaFormat), OsMediaImportHandler>>> =
        LazyLock::new(|| Mutex::new(std::collections::HashMap::new()));
...
pub use crate::workflow_kernel::{
    ...
    pub use semio_framework::MediaFormat;
    ...
    /// 🗄️ Ticket 26/08/10/STDIO-ARTIFACTS-AND-IO W15: see the os-side twin of this function for
    /// the full rationale -- dispatches export via the typed registry's EXPORT-direction entries
    /// before falling back to the old stringly handler map.
    ...
    /// through the typed registry before falling back to the old stringly handler map.
...
pub use media_export_raster::{
    ...
    OsMediaExportResult, media_accept_filter, media_accept_filter_kinds,
};
...
pub use crate::workflow::{
    ...
    os_workflow_to_node_graph_payload, patch_workflow_parameter, placeholder_media_contract, plan_workflow, required_media_formats, MediaDirection,
    ...
    workflow_parameter_types_compatible, workflow_parameter_value, MediaContract, OsMediaCapability, MediaFormat, OsWorkflowCamera, OsWorkflowNodeGraphPayload, OsWorkflowOperatorInfo, Workflow, WorkflowDelivery, WorkflowSnapshot,
