...
impl<S: PackSink> PackWriter<S> {
    ...
    pub fn write_segment(&mut self, kind: u8, payload: &[u8]) -> Result<(), PackError> {
        ...
        if kind == pack_core::KIND_SYMBOLS {
        ...
        if kind == pack_core::KIND_DOCUMENT {
    ...
    pub fn write_chunk(&mut self, payload: &[u8]) -> Result<ChunkId, PackError> {
        let base = self.sink.position();
        let encoded = encode_segment(pack_core::KIND_CHUNK, self.options.codec, payload)?;
    ...
    pub fn finish(mut self, manifest: &Manifest) -> Result<S, PackError> {
        ...
        if !self.chunks.is_empty() {
            ...
            let encoded = encode_segment(pack_core::KIND_CHUNK_TABLE, self.options.codec, &table_bytes)?;
            self.sink.write_all(&encoded.bytes)?;
        ...
        let manifest_encoded = encode_segment(pack_core::KIND_MANIFEST, self.options.codec, &manifest_bytes)?;
        ...
        let end_encoded = encode_segment(pack_core::KIND_END, CodecId(0), &[])?;
    ...
    pub fn body_bytes(&self, verification: VerificationLevel) -> Result<Vec<u8>, PackError> {
        ...
        if manifest.doc_span.len > 0 {
            ...
            for _ in 0..frames {
                ...
                if seg.kind != pack_core::KIND_DOCUMENT {
...
/// @emoji 🔎️ Standalone helper (used by `pack::content_hash`) that reads and parses only the
    ...
    while offset < len {
        match decode_segment_at(source, offset, limits, true) {
            Ok(seg) => {
                ...
                let is_end = seg.kind == pack_core::KIND_END;
                offset += seg.consumed;
...
mod tests {
    ...
    fn open_superblock_truncated_at_every_footer_byte_boundary_errors_never_panics() {
        ...
        writer.write_segment(pack_core::KIND_DOCUMENT, b"hello world").unwrap();
    ...
    fn recover_accumulates_unknown_kind_segments_without_erroring() {
        ...
        file.extend_from_slice(&encode_segment(pack_core::KIND_END, CodecId(0), &[]).unwrap().bytes);
        let limits = PackLimits::default();
    ...
    fn segment_crc_mismatch_is_detected() {
        let encoded = encode_segment(pack_core::KIND_DOCUMENT, CodecId(0), b"payload").unwrap();
        let mut file = vec![0u8; HEADER_SIZE];
        file.extend_from_slice(&encoded.bytes);
        let last = file.len() - 1;
        file[last] ^= 0xFF;
    ...
    fn build_sample_pack(codec: CodecId) -> (Vec<u8>, ChunkId, Vec<u8>) {
        ...
        writer.write_segment(pack_core::KIND_SYMBOLS, &encode_symbols(&["widget.v1".to_string(), "name".to_string()])).unwrap();
        ...
        writer.write_segment(pack_core::KIND_DOCUMENT, &document_payload).unwrap();
