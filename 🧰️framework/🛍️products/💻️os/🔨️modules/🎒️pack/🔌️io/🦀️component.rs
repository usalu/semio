...
//! `StreamingPackWriter` (an incremental `crate::os_pack::format::PackWriter<FilePackSink>` that flushes
//! segments to disk as produced rather than buffering the whole file in memory), and
//! `recover_file` (opens a file and forward-scans it via `crate::os_pack::format::recover`).
//!
...
mod native {
    ...
    /// `crate::os_pack::format::PackWriter<FilePackSink>`.
    ...
    //#region 🔖️Recover
    /// @emoji 🩺️ Opens `path` and forward-scans it via `crate::os_pack::format::recover` — for use when a
    ...
    pub fn recover_file(path: &Path, limits: &PackLimits) -> Result<RecoveryReport, PackError> {
        ...
        crate::os_pack::format::recover(&source, limits)
    }
    //#endregion 🔖️Recover
    ...
    mod tests {
        ...
        fn no_compression_options() -> WriteOptions {
            WriteOptions { required_flags: 0, optional_flags: crate::os_pack::format::OPTIONAL_STREAMED, codec: CodecId(0) }
        }
        ...
        fn streaming_writer_full_session_multiple_segments_and_a_chunk_round_trips() {
            ...
            let manifest = Manifest {
                ...
                schema_hash: [0u8; 32],
                doc_span: ByteRange { offset: crate::os_pack::format::HEADER_SIZE as u64, len: uncompressed_segment_wire_len(doc_payload.len()) },
                doc_frame_count: 1,
            ...
            let pack_file = crate::os_pack::format::PackFile::open_manifest(source, &limits, VerificationLevel::Standard).unwrap();
        ...
        fn recover_file_with_footer_stripped_still_recovers_every_body_segment() {
            ...
            file.set_len(full_len - crate::os_pack::format::FOOTER_SIZE as u64).unwrap();
            drop(file);

            // Superblock open must fail (no valid footer) — this is the scenario recover_file is for.
            let source = FilePackSource::open(&path).unwrap();
            assert!(crate::os_pack::format::PackFile::open_superblock(source, &limits).is_err());
        ...
        fn recover_file_truncated_mid_segment_recovers_a_strict_prefix_without_panicking() {
            ...
            let cut_len = full_len - crate::os_pack::format::FOOTER_SIZE as u64 - 3;
            let file = std::fs::OpenOptions::new().write(true).open(&path).unwrap();
