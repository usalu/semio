//! 📦️ `pack_value` — DSL schema-aware wire encoding/decoding of `dsl_schema::RecordValue`
...
struct EncCtx<'a> {
    ...
    writer: &'a mut pack_format::PackWriter<Vec<u8>>,
...
pub struct DecodeOptions {
    pub verification: pack_format::VerificationLevel,
    pub preserve_unknown: bool,
    pub limits: PackLimits,
}
...
impl Default for DecodeOptions {
    fn default() -> Self {
        Self { verification: pack_format::VerificationLevel::Standard, preserve_unknown: true, limits: PackLimits::default() }
    }
}
...
pub struct DecodeReport {
    ...
    pub verified: pack_format::VerificationLevel,
}
...
pub fn encode_document(spec: &RecordSpec, record: &RecordValue, options: &EncodeOptions) -> Result<Vec<u8>, PackError> {
    ...
    writer.write_segment(pack_core::KIND_SYMBOLS, &symbols_payload)?;
    ...
    for frame in doc_payload.chunks(frame_size) {
        writer.write_segment(pack_core::KIND_DOCUMENT, frame)?;
        frame_count += 1;
    }
    let doc_end = writer.position();

    let manifest = pack_format::Manifest {
        schema_name: String::new(),
        schema_hash: schema_hash(spec),
        doc_span: pack_core::ByteRange { offset: doc_start, len: doc_end - doc_start },
        doc_frame_count: frame_count,
        symbols_span: pack_core::ByteRange { offset: 0, len: 0 },
        chunk_table_span: pack_core::ByteRange { offset: 0, len: 0 },
        field_index_span: pack_core::ByteRange { offset: 0, len: 0 },
...
pub fn decode_document(bytes: &[u8], spec: &RecordSpec, options: &DecodeOptions) -> Result<(RecordValue, DecodeReport), PackError> {
    let pack_file = pack_format::PackFile::open_manifest(bytes, &options.limits, options.verification)?;
