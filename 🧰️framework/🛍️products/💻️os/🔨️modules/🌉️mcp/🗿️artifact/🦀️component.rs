...
//! W4, closing 5 of the 9 `DECLARED_STUB_TOOL_NAMES` at `🦀️component.rs:244`. Every tool is always
...
//! protocol has no "create a document of schema X" command yet (see `🏠️workspace/🦀️component.rs`'s
...
/// (`🏠️workspace/🦀️component.rs` `read_artifact_resource`'s `validation` arm), so no shape can be
...
/// gateway tools (`🦀️component.rs`'s `core_tool_capabilities`) so they compile into the catalog for
...
/// unioned across every app it declares) — read straight from `🔣️descriptor.json` via
