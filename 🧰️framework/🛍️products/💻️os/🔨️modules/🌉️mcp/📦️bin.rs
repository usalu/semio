...
//! [--audit-dir <dir>] [--allow-origin <origin>]…` (P1b + P7-headless-workspace) — this binary owns
//! argv parsing only; all real logic lives in `semio_framework_os_mcp::{run_stdio, run_http}` (P1a's
//! brief §2.5, "keep main thin, all logic in the lib" — mirrors `🏃️run/📦️bin.rs`'s own split).
//! Unknown modes exit with a clear message rather than silently doing nothing. `--folder`/`--hub`
//! are mutually exclusive (`📋️master.md` §2.1: "`--folder <space dir>`…`--hub <url> --space <id>
//! --token <t>`"); `http`'s own `--token` (the bridge/HTTP bearer) is a distinct flag from the hub's
//! `--token`, so a hub-bound `http` session passes `--token <bearer> --hub <url> --space <id>
//! --token <hub token>` — the SECOND `--token` after `--hub`/`--space` binds the hub, matching
//! `--folder`'s own "whichever flag is still empty when this one is seen" parsing order below.
...
fn parse_http_args(argv: &mut impl Iterator<Item = String>) -> Result<HttpOptions, String> {
    ...
    while let Some(flag) = argv.next() {
        match flag.as_str() {
            ...
            // 🎯️ The FIRST `--token` seen binds the bridge/HTTP bearer (this mode's own required
            // flag, pre-existing since P1b); every subsequent `--token` binds the hub's own auth
            // token — matches this module doc's "second `--token`" convention.
    ...
    Ok(HttpOptions { port, bind, token, folder, hub, principal, scopes, audit_dir, allow_origin })
}
