...
impl RegistryDiscovery {
    ...
    pub fn scan(root: PathBuf) -> Self {
        ...
        for entry in &entries {
            descriptor_paths.push(entry.owner_root.join("🔣️descriptor.json"));
...
mod quick {
    ...
    fn write_descriptor(root: &Path, owner_rel: &str, descriptor: &manifest::PackageDescriptor) {
        ...
        std::fs::write(owner_root.join("🔣️descriptor.json"), serde_json::to_vec(descriptor).expect("descriptor serializes")).expect("write descriptor");
    }
    ...
    fn a_malformed_descriptor_is_skipped_with_a_diagnostic_not_a_panic_or_err() {
        ...
        std::fs::create_dir_all(&owner_root).expect("create owner root");
        std::fs::write(owner_root.join("🔣️descriptor.json"), b"not valid json").expect("write malformed descriptor");
