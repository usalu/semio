...
//! "Nothing constructs or reads one yet in this packet" — `🛂️manifest/🦀️component.rs` `🔖️PackageDescriptor`,
...
//#region 🔖️Eval
const EVAL_JSON: &str = include_str!("🔣️eval.json");

/// 📖️ Parses the embedded `🔣️eval.json` fixture — `≥60` natural-language requests (English + German,
/// CLAUDE.md's en-first/de-second rule) each mapping to a real capability id compiled from
/// `note_and_cad_source()`.
pub fn eval_cases() -> Vec<EvalCase> {
    serde_json::from_str(EVAL_JSON).expect("🔣️eval.json parses into Vec<EvalCase>")
}
//#endregion 🔖️Eval
