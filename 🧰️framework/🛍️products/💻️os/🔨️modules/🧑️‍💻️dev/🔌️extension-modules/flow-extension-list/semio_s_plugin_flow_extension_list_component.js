...
import { monotonicClock } from '../_vendor/@bytecodealliance/preview2-shim/clocks.js';
...
function exchange(arg0, arg1) {
  ...
  _debugLog('[iface="semio:framework/plugin", function="exchange"][Instruction::CallWasm] enter', {
    funcName: 'exchange',
    paramCount: 3,
    async: false,
    postReturn: true,
  ...
  try {
    ret =   _withGlobalCurrentTaskMeta({
      taskID: task.id(),
      componentIdx: task.componentIdx(),
      fn: () => pluginExchange(toUint32(arg0), result1, len1),
    });
  } catch (err) {
  ...
  _debugLog('[iface="semio:framework/plugin", function="exchange"][Instruction::Return]', {
    funcName: 'exchange',
    paramCount: 1,
    async: false,
    postReturn: true
...
const $init = (() => {
  let gen = (function* _initGenerator () {
    const module0 = fetchCompile(new URL('./semio_s_plugin_flow_extension_list_component.core.wasm', import.meta.url));
    ...
    postReturn2 = exports1['cabi_post_semio:framework/plugin#exchange'];
    
    try {
      postReturn2Async = WebAssembly.promising(exports1['cabi_post_semio:framework/plugin#exchange']);
    } catch(err) {
      postReturn2Async = exports1['cabi_post_semio:framework/plugin#exchange'];
    }
    
    postReturn3 = exports1['cabi_post_semio:framework/plugin#migrate-document'];
    
    try {
      postReturn3Async = WebAssembly.promising(exports1['cabi_post_semio:framework/plugin#migrate-document']);
    } catch(err) {
      postReturn3Async = exports1['cabi_post_semio:framework/plugin#migrate-document'];
    }
    ...
    pluginExchange = exports1['semio:framework/plugin#exchange'];
