...
class DistributionBundleScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    ...
    runCmd("bun", ["--eval", "const api = await import(process.argv[1]); await api.materializeDistributionBundle(process.argv[2], process.argv[3]);", import.meta.url, this.repoRoot, compiled], { cwd: this.repoRoot, budgetMs: buildBudgetMs() });
...
const module1 = fetchCompile(new URL('./plugin_component.core2.wasm', import.meta.url));`;
  ...
  describe("deployed vendor transport", () => {
    itLong("closes static compiler source imports through explicit neutral entry points", async () => {
      ...
      const validate = new Ajv({ strict: true }).compile(JSON.parse(readFileSync(join(authority, "🔐️inputs.schema.json"), "utf8")));
    ...
    it.runIf(Boolean(process.env.SEMIO_DISTRIBUTION_AUDIT_PATH))("checks actual compiled distribution bytes and preserves the entire old primary distribution", async () => {
      const { default: Ajv } = await import("ajv"), { pathEmojiStatuteFindings, loadTaxonomy } = await import("../../../../../../../🧰️framework/🛍️products/🦑️repo/🔨️modules/📚️library/🔍️discovery/🟦️.ts");
      ...
      const manifest = parseDistributionManifest(JSON.parse(readFileSync(join(compiled, DISTRIBUTION_LAYOUT.manifest), "utf8")), DISTRIBUTION_LAYOUT);
      const validate = new Ajv({ strict: true }).compile(JSON.parse(readFileSync(resolve(dirname(fileURLToPath(import.meta.url)), "../../🚚️distribution/🧬️manifest.schema.json"), "utf8")));
      ...
      const playDirectory = relative(repoRoot, resolve(dirname(fileURLToPath(import.meta.url)), "../..")).replaceAll("\\", "/"), primary = `${playDirectory}/dist`;
    ...
    itLong("validates exact distribution owner manifests against a neutral digest oracle", async () => {
      ...
      const fixture = JSON.parse(readFileSync(join(authority, "🔬️manifest-cases.json"), "utf8"));
      const { layout } = JSON.parse(readFileSync(join(authority, "🧪️cases.json"), "utf8"));
      const validate = new Ajv({ strict: true }).compile(JSON.parse(readFileSync(join(authority, "🧬️manifest.schema.json"), "utf8")));
    ...
    itLong("admits only hand-authored collision-free distribution output owners", async () => {
      ...
      const validate = new Ajv({ strict: true }).compile(JSON.parse(readFileSync(join(root, "📐️schema.json"), "utf8")));
    ...
    itLong("validates neutral read-only build inspection records against an independent digest oracle", async () => {
      ...
      const validate = new Ajv({ strict: true }).compile(JSON.parse(readFileSync(join(fixtureRoot, "📐️build-inspection.schema.json"), "utf8")));
    ...
    it.runIf(Boolean(process.env.SEMIO_BUILD_INSPECTION_OUTPUT))("inspects the actual production Rollup output without writing or replacing distribution bytes", async () => {
      ...
      const configPath = join(dirname(fileURLToPath(import.meta.url)), "⚙️vite.config.ts");
    ...
    itLong("excludes test-only Node imports from production output while preserving runtime and test-mode branches", async () => {
      ...
      const config = ts.createSourceFile("vite.config.ts", readFileSync(join(dirname(fileURLToPath(import.meta.url)), "⚙️vite.config.ts"), "utf8"), ts.ScriptTarget.Latest, true);
    ...
    itLong("serves every declared component import and tool-fixed vendor file byte-identically", async () => {
      ...
      const { loadCatalogTaxonomy, fixedFilenameContractIdsForPath } = await import("../../../../../../../🧰️framework/🛍️products/🦑️repo/🔨️modules/📚️library/🔍️discovery/🟦️.ts");
