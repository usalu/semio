...
///
/// 🧹️ This is a SECOND `wit_bindgen::generate!` for the same world (the root `🦀️.rs`'s
...
/// `⚛️reactor/🦀️.rs`'s `kernel_effect_to_wit` uses for every Rust-only field type crossing
...
// `⚛️reactor/🦀️.rs` already made for its own identical `pack` helper. `pack` is consumed
...
/// `🖥️host/⚡️effects/🦀️.rs`'s `encode_http_response`/`HttpResponseWire` (that struct is
...
impl Host {
    ...
    /// `🖥️host/⚡️effects/🦀️.rs::encode_http_response` writes, and hands back the WHOLE body
    ...
    /// (the same A3 gap `🦀️.rs`'s own `wit_effect_to_kernel` already documents, and the
    ...
    /// resolves against (`⚛️reactor/🦀️.rs`'s `Event::JobCompleted` routing step), so no
    ...
    /// needs — see `🖥️host/🧵️shard/🦀️.rs`'s `ShardLoop::pump`. The Direct arm has no
    ...
    /// is host-side, replaying a shell action — see `⚛️reactor/🦀️.rs`'s
...
/// `⚛️reactor/🦀️.rs`'s `kernel_effect_to_wit` for the ~22 variants `Host` itself ever
