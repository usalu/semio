...
//! `ShardTransport` impl `🎭️actor/🦀️component.rs`'s own doc comment names as host-supplied
...
//! (`../👶️child/🦀️main.rs`) hosts its own `ShardLoop` over — same [`framing`] module, same
...
/// 👶️ Child-side mirror of [`ProcessTransport`] — hosted by `semio-shard` (`../👶️child/🦀️main.rs`)
/// so its own `ShardLoop` can be driven exactly like a `ThreadTransport`-backed one, per
/// `🧵️shard/🦀️component.rs`'s own module doc ("this file never branches on which [`ShardTransport`]
...
/// `🧵️shard/🦀️component.rs`'s own `#[cfg(test)]` module and `🦀️component.rs`'s wasmtime tests. This
