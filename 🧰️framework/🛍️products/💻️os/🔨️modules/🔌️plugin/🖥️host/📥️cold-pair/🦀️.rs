...
mod tests;
use semio_framework::kernel::{ColdArtifactPairApplied, ColdArtifactPairCursor, ColdArtifactPairFrontier, ColdArtifactPairPage, ColdPairIngressStatus};
...
fn frontier(value: wit_reactor::ColdArtifactPairFrontier) -> Result<ColdArtifactPairFrontier, TurnFault> {
    if value.artifact_id.is_empty() || value.artifact_id.len() > 512 || value.head_edit_id.is_empty() || value.head_edit_id.len() > 512 || value.last_commit_seq > value.head_edit_ordinal {
    ...
    Ok(ColdArtifactPairFrontier { artifact_id: value.artifact_id, head_edit_ordinal: value.head_edit_ordinal, head_edit_id: value.head_edit_id, last_commit_seq: value.last_commit_seq, chain_sha256 })
}

fn cursor(value: wit_reactor::ColdArtifactPairCursor) -> Result<ColdArtifactPairCursor, TurnFault> {
    ...
    Ok(ColdArtifactPairCursor { lifetime, transfer_generation: value.transfer_generation, page_index: value.page_index, page_count: value.page_count })
}

pub(super) fn validate_cold_page(page: &ColdArtifactPairPage) -> Result<(), TurnFault> {
...
pub(super) fn kernel_cold_page_to_wit(page: &ColdArtifactPairPage) -> Result<wit_reactor::ColdArtifactPairPage, TurnFault> {
    validate_cold_page(page)?;
    let header = &page.header;
    let baseline = &header.baseline_frontier;
    Ok(wit_reactor::ColdArtifactPairPage {
        header: wit_reactor::ColdArtifactPairHeader {
            ...
            baseline_frontier: wit_reactor::ColdArtifactPairFrontier {
...
pub(super) fn wit_cold_ingress_to_kernel(value: wit_reactor::ColdPairIngressStatus) -> Result<ColdPairIngressStatus, TurnFault> {
    ...
    let status = match value {
        ...
        W::Applied(value) => {
            ...
            ColdPairIngressStatus::Applied(ColdArtifactPairApplied { lifetime, transfer_generation: value.transfer_generation, baseline_frontier: frontier(value.baseline_frontier)?, aggregate_sha256 })
        }
    };
