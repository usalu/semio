...
use semio_framework::kernel::{
    ActorInstanceLifecycleAck, ActorInstanceLifecycleReceipt, ActorInstanceLifetime, ActorInstanceOpenRequest, ActorUiPatchReceipt, AppInstanceId, QuotaSchema,
};
...
async fn genuine_component_returned_and_imported_patches_keep_channel_order_and_exact_authority() {
    for (config, root, session) in [
        (serde_json::json!({ "profile": "ui" }), 201, SESSION),
        (serde_json::json!({ "profile": "ui", "uiImportSink": true }), 101, SESSION + 1),
    ] {
