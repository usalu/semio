...
async function nativeCatalogSelectionOracleV1(): Promise<void> {
  ...
  for (const row of fixture.cases) {
    ...
    const vcs = candidate.packages[1];
    const stdio = candidate.packages[0];
    const root = candidate.profiles[1].roots[0];
    switch (row.mutation) {
      ...
      case "missing-dependency": candidate.packages.shift(); break;
      ...
      case "missing-provider": candidate.availableProviders.pop(); break;
      case "provider-package": candidate.availableProviders[1].packageId = "semio:other"; break;
      case "provider-count": candidate.availableProviders[1].receiptCount = 2; break;
    }
    const calls: string[] = [];
    let receiptCount = 0;
    let code = "denied";
  ...
  console.log(`native-catalog-selection-oracle cases=${fixture.cases.length} positive=2 denied=17 authority=planning-only published=0`);
}
