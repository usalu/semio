...
function emitRustArtifacts(entries: PluginRegistryEntry[], repoRoot: string): string {
...
function renderCatalogFiles(repoRoot: string, view: RegistryCatalogInputView = registryCatalogInputView(repoRoot, TAXONOMY)): { files: Record<string, string>; entries: PluginRegistryEntry[]; playgrounds: PlaygroundEntry[]; frameworkPackages: FrameworkPackageEntry[]; componentLaunchers: { project: string; pluginId: string }[] } {
  ...
  return {
    ...
    files: {
      ...
      "🗿️artifacts.rs": emitRustArtifacts(entries, repoRoot),
...
const WASM_PUBLICATION_PROFILE = "wasm-release";
...
type NativeCatalogSelectionPackageV1 = NativeCatalogSelectionIdentityV1 & Readonly<{ dependencies: readonly NativeCatalogSelectionIdentityV1[]; receiptCount: number }>;
type NativeCatalogProviderPreviewV1 = Readonly<{ pluginId: string; packageId: string; receiptCount: number }>;
type NativeCatalogSelectionInputV1 = Readonly<{
...
async function previewNativeCatalogSelectionV1(
...
const TRUSTED_STDIO_FACTORY_SCHEMA = "semio.stdio.native-openable-catalog-provider/v1";
const TRUSTED_STDIO_VERSION = "0.1.0";
const TRUSTED_STDIO_CATALOG_FILENAME = "trusted-stdio-catalog.json";

/** 🗄️ Resolves the stdio owner root from the discovered Cargo crate path. */
function stdioOwnerRoot(repoRoot: string, entries?: readonly PluginRegistryEntry[]): string {
...
function stdioNativeCodecFactoriesPath(stdioRoot: string): string {
...
function stdioComponentAdmission(repoRoot: string): TrustedStdioComponentAdmissionV1 {
