mod extension_bundle_dependency_tests {
    ...
    async fn extends_mismatching_the_first_dependency_panics() {
        let result = std::panic::catch_unwind(|| ExtensionBundle::new("ext-mismatch", "Ext Mismatch", "0.1.0").depends_on("primary-dep", semio_framework::tree_pin!()).extends("someone-else"));
    ...
    async fn extends_set_before_a_mismatching_dependency_also_panics() {
        let result = std::panic::catch_unwind(|| ExtensionBundle::new("ext-mismatch-2", "Ext Mismatch 2", "0.1.0").extends("primary-dep").depends_on("someone-else", semio_framework::tree_pin!()));
