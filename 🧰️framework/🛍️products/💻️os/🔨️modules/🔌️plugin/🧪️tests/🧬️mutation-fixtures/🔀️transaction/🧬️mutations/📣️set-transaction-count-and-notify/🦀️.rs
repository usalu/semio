...
pub(crate) struct SetTransactionCountAndNotify {
    pub value: i32,
}
impl SetTransactionCountAndNotify {
    const OPCODE: &'static str = "set-transaction-count-and-notify";
    const TAG: u8 = 0x64;
}
    ...
    fn parse_op(line: &str) -> Result<Self, crate::store::TextError> {
        Ok(Self {
            value: line
                ...
                .ok_or_else(|| crate::store::TextError::new("expected set-transaction-count-and-notify", crate::store::TextSpan::at(1, 1)))?
                .parse()
...
impl OpBinary for SetTransactionCountAndNotify {
    ...
    fn decode_op(bytes: &[u8]) -> Result<Self, ProtocolError> {
        if bytes.len() != 5 || bytes.first() != Some(&Self::TAG) {
            return Err(ProtocolError::Malformed { what: "set-transaction-count-and-notify", offset: 0, detail: "expected tag 0x64 and four i32 bytes".into() });
        }
        Ok(Self { value: i32::from_be_bytes(bytes[1..].try_into().expect("exact i32 width")) })
    }
}
impl MutationKind<TxnSnapshot, TxnMutation> for SetTransactionCountAndNotify {
    const SEMANTICS: SemanticDescriptor = SemanticDescriptor { verb: "set", entity: "transaction-count-and-notify", kind: "set-transaction-count-and-notify", record: "SetTransactionCountAndNotify" };
