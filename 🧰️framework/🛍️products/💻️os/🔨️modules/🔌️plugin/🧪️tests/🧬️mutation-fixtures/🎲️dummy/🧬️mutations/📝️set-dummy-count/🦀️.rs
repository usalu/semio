...
pub(crate) struct SetDummyCount {
    pub value: i32,
}
impl SetDummyCount {
    const OPCODE: &'static str = "set-dummy-count";
    const TAG: u8 = 0x61;
}
...
impl OpBinary for SetDummyCount {
    ...
    fn decode_op(bytes: &[u8]) -> Result<Self, ProtocolError> {
        if bytes.len() != 5 || bytes.first() != Some(&Self::TAG) {
            return Err(ProtocolError::Malformed { what: "set-dummy-count", offset: 0, detail: "expected tag 0x61 and four i32 bytes".into() });
        }
        Ok(Self { value: i32::from_be_bytes(bytes[1..].try_into().expect("exact i32 width")) })
    }
}
impl MutationKind<DummySnapshot, DummyMutation> for SetDummyCount {
    const SEMANTICS: SemanticDescriptor = SemanticDescriptor { verb: "set", entity: "dummy-count", kind: "set-dummy-count", record: "SetDummyCount" };
    ...
    fn label(&self) -> String {
        format!("Set dummy count to {}", self.value)
    }
}
