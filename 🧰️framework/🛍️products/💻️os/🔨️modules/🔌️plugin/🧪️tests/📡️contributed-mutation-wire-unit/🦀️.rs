...
fn descriptor_and_provenance_are_direct() {
    assert_eq!(serde_json::Value::from(protocol::ToValue::to_value(&AddValue::DESCRIPTOR)), serde_json::from_str::<serde_json::Value>(include_str!("../📡️contributed-mutation-wire/🧬️mutations/➕️add-value/🔣️.json")).expect("owned descriptor JSON"));
