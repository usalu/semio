...
//#region 🧫️Snapshot
#[derive(Clone, Debug, Default, PartialEq, Serialize, semio_framework_value_derive::ToValue, Deserialize, semio_framework_value_derive::FromValue, dsl::DslArtifact)]
...
//#region 🧬️Mutations
#[path = "../../🧪️testkit/🖥️test-app-mutations/🎚️config/🧬️mutations/🦀️.rs"]
pub mod mutations;
...
pub(crate) enum TestConfigDiff {
    Identity,
    Clear,
    Set(String),
}
impl Default for TestConfigDiff {
    fn default() -> Self {
        Self::Identity
    }
}
impl MutationDiff<TestConfig> for TestConfigDiff {
    fn apply(&self, base: &TestConfig) -> protocol::MutationApplyResult<TestConfig> {
        Ok(match self {
            ...
            Self::Set(value) => TestConfig { selected: Some(value.clone()) },
        })
    }
    fn absorb(&mut self, other: Self) {
        if !matches!(other, Self::Identity) {
            *self = other;
        }
    }
}
