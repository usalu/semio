...
fn descriptor_and_provenance_are_direct() {
    assert_eq!(serde_json::to_value(AddValue::DESCRIPTOR).expect("descriptor JSON"), serde_json::from_str::<serde_json::Value>(include_str!("../🧬️mutations/➕️add-value/🔣️.json")).expect("owned descriptor JSON"));
