...
where M: Mutation<S> + SemanticMutation<S>, L: MutationLeaf {
    ...
    assert_eq!(serde_json::to_value(L::DESCRIPTOR).expect("descriptor JSON"), descriptor_json);
...
where S: Clone + Debug + PartialEq, M: Mutation<S> + Debug + PartialEq {
    ...
    for row in cases()["composition"].as_array().expect("composition cases") {
        ...
        for value in row["steps"].as_array().expect("steps") {
            let diff: M::Diff = serde_json::from_value(serde_json::json!({"value": value})).expect("replacement diff");
        ...
        assert_eq!(serde_json::to_value(&combined).expect("composed shape"), serde_json::json!({"value":row["combined"]}));
    }
    for a in [None, Some(i32::MIN), Some(0), Some(i32::MAX)] {
        for b in [None, Some(i32::MIN), Some(0), Some(i32::MAX)] {
            for c in [None, Some(i32::MIN), Some(0), Some(i32::MAX)] {
                let diff = |value: Option<i32>| serde_json::from_value::<M::Diff>(serde_json::json!({"value":value})).expect("diff");
                let mut left = diff(a);
                left.absorb(diff(b));
                left.absorb(diff(c));
                ...
                right.absorb(middle);
                assert_eq!(serde_json::to_value(left).expect("left"), serde_json::to_value(right).expect("right"));
            }
        }
    }
}
...
where S: Clone + Debug + PartialEq + Serialize + DeserializeOwned + store::ArtifactDsl + store::ArtifactPack,
    ...
    for value in vectors["values"].as_array().expect("values") {
        ...
        let leaf: L = serde_json::from_value(payload.clone()).expect("leaf JSON");
        assert_eq!(serde_json::to_value(leaf).expect("leaf encode"), payload);
        let envelope = serde_json::json!({"SetValue":payload});
        assert_eq!(serde_json::to_value(&mutation).expect("operation JSON"), envelope);
        assert_eq!(serde_json::from_value::<M>(envelope.clone()).expect("JSON decode"), mutation);
    ...
    for value in vectors["invalidPayloads"].as_array().expect("invalid payloads") {
        assert!(serde_json::from_value::<L>(value.clone()).is_err(), "{value}");
    ...
    for value in vectors["invalidEnvelopes"].as_array().expect("invalid envelopes") {
        assert!(serde_json::from_value::<M>(value.clone()).is_err(), "{value}");
    ...
    for value in vectors["snapshots"].as_array().expect("snapshots") {
        let snapshot: S = serde_json::from_value(value.clone()).expect("snapshot");
    ...
    for value in vectors["invalidSnapshots"].as_array().expect("invalid snapshots") { assert!(serde_json::from_value::<S>(value.clone()).is_err(), "{value}"); }
    assert_eq!(serde_json::to_value(S::parse_dsl(" ").expect("empty snapshot text")).expect("default snapshot"), serde_json::json!({"value":0}));
    assert_eq!(serde_json::to_value(S::decode_pack(&[]).expect("empty snapshot pack")).expect("default snapshot"), serde_json::json!({"value":0}));
    for value in vectors["diffs"].as_array().expect("diffs") { assert!(serde_json::from_value::<M::Diff>(value.clone()).is_ok(), "{value}"); }
    for value in vectors["invalidDiffs"].as_array().expect("invalid diffs") { assert!(serde_json::from_value::<M::Diff>(value.clone()).is_err(), "{value}"); }
}
