#[path = "../🧬️mutation-fixtures-transaction-mutations-set-transaction-count/🦀️.rs"]
pub mod set_transaction_count;
#[path = "../🧬️mutation-fixtures-transaction-mutations-set-transaction-count-and-notify/🦀️.rs"]
pub mod set_transaction_count_and_notify;
#[path = "../🧬️mutation-fixtures-transaction-mutations-set-transaction-count-without-preflight/🦀️.rs"]
...
pub(crate) enum TxnMutation {
    SetTransactionCount(SetTransactionCount),
    SetTransactionCountWithoutPreflight(SetTransactionCountWithoutPreflight),
...
impl protocol::OpText for TxnMutation {
    fn parse_op(line: &str) -> Result<Self, store::TextError> {
        if line.starts_with("set-transaction-count-and-notify ") {
            ...
            Ok(SetTransactionCountWithoutPreflight::parse_op(line)?.into())
        } else {
            Ok(SetTransactionCount::parse_op(line)?.into())
        }
    }
    fn print_op(&self) -> String {
        match self {
            Self::SetTransactionCount(value) => value.print_op(),
            Self::SetTransactionCountWithoutPreflight(value) => value.print_op(),
            Self::SetTransactionCountAndNotify(value) => value.print_op(),
        }
    }
}
impl protocol::OpBinary for TxnMutation {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        match self {
            Self::SetTransactionCount(value) => value.encode_op(),
    ...
    fn decode_op(bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        match bytes.first() {
            Some(0x62) => Ok(SetTransactionCount::decode_op(bytes)?.into()),
            Some(0x63) => Ok(SetTransactionCountWithoutPreflight::decode_op(bytes)?.into()),
            Some(0x64) => Ok(SetTransactionCountAndNotify::decode_op(bytes)?.into()),
            _ => Err(protocol::ProtocolError::Malformed { what: "transaction-counter", offset: 0, detail: "unknown transaction counter tag".into() }),
        }
    }
}
