...
fn close(store: &mut InteractionStore, query: &mut LocalInteractionQuery, bytes: usize) {
    ...
    for _ in 0..1_000_000 {
        ...
        if query.terminal_is_empty() && retirement.is_none() && store.snapshot_read_leases_terminal_is_empty() {
            break;
        }
    }
...
fn local_interaction_query_partial_encoder_failure_keeps_exact_byte_ownership() {
    ...
    for bytes in [1, 64, 4096] {
        ...
        for _ in 0..10_000 {
            if let Err(error) = query.advance(grant) {
                assert_eq!(error, laws["partialError"]["error"].as_str().unwrap());
                failed = true;
                break;
            }
            if let Some(page) = query.page() {
        ...
        for _ in 0..10_000 {
            if matches!(query.close_step(ArtifactStoreOneItemGrant { maximum_items: 1, maximum_bytes: 1 }).unwrap(), SnapshotRetirementStep::Complete) {
                break;
            }
        }
        assert!(query.terminal_is_empty());
