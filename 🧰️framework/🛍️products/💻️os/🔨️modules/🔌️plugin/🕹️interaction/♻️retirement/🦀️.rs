...
impl ErasedSnapshotRetirement for InteractionRetirement {
    fn close_step(&mut self, maximum_items: usize, maximum_bytes: usize) -> Result<SnapshotRetirementStep, String> {
        ...
        if let Some(selection) = owned.selection.as_mut() {
            if let Some(id) = selection.ids.pop() {
                owned.text = Some(id);
            } else if selection.ids.capacity() != 0 {
                selection.ids = Vec::new();
            } else if selection.granularity.capacity() != 0 {
                owned.text = Some(std::mem::take(&mut selection.granularity));
            } else if let Some(anchor) = selection.anchor_id.take() {
                owned.text = Some(anchor);
            } else {
        ...
        if let Some(hover) = owned.hover.as_mut() {
            if let Some(id) = hover.ids.pop() {
                owned.text = Some(id);
            } else if hover.ids.capacity() != 0 {
                hover.ids = Vec::new();
            } else if hover.channel.capacity() != 0 {
                owned.text = Some(std::mem::take(&mut hover.channel));
            } else {
                owned.hover = None;
            }
            return Ok(SnapshotRetirementStep::Pending { released_items: 1, released_bytes: 0 });
        ...
        if let Some(state) = owned.state.as_mut() {
            if let Some((domain, selection)) = state.selection.pop_first() {
                owned.text = Some(domain);
                owned.selection = Some(selection);
            } else if let Some((domain, hover)) = state.hover.pop_first() {
                owned.text = Some(domain);
                owned.hover = Some(hover);
            } else if let Some((domain, _)) = state.active_mode.pop_first() {
                owned.text = Some(domain);
            } else if let Some((domain, value)) = state.active_granularity.pop_first() {
...
impl ArtifactOwnedValueRetirementFactory<InteractionState> for InteractionRetirementFactory {
    fn retire_owned(&self, root: InteractionState) -> Box<dyn ErasedSnapshotRetirement> {
        Box::new(InteractionRetirement::owned(root))
    }
}
...
pub(crate) fn interaction_store_owners() -> store::MemberStoreOwners<InteractionState, InteractionConfigMutation> {
    store::MemberStoreOwners::new(
        ...
        Arc::new(InteractionRetirementFactory),
        Arc::new(InteractionRetirementFactory),
        Box::new(store::ArtifactStoreCursorDisposer::<InteractionState, InteractionConfigMutation>::new()),
    )
}
