 ...
 * hub session, an offline mutation outbox) belongs entirely to `🏪️store/👷️worker/🟦️.ts`'s own state
 ...
 * Rust `PublishOutcome` (`🛎️services/🦀️.rs`; also not `Serialize` there, in-process only),
 ...
 * `🏪️store/👷️worker/🟦️.ts`'s own `rejectMutationQueueOverflow` already uses for its outbound mutation
 ...
 * `ShellHost/🟦️.tsx` already documents for its own `🏪️store/👷️worker/🟦️.ts` import. */
 ...
 * through `🏪️store/👷️worker/🟦️.ts`'s existing document transport. */
...
/** 🌉️ terra-web-effect-backbone: "route through `🏪️store/👷️worker/🟦️.ts`'s existing document/blob
 ...
 * `🏪️store/👷️worker/🟦️.ts` nor `🟦️.ts` is edited by this packet.
 *
 ...
 * `ArtifactActorMsg` kind (`localMutations`, `localDocumentArchive`, ...) is document-shaped, not a fit
