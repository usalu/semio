...
//! `world actor` (the poll world). `🖥️host/🦀️.rs`'s `PluginInstanceHandle::
...
/// 📤️ Called by `PluginBuilder::try_build()` (`🏗️builder/🦀️.rs`) once per `.job(kind, run)`
...
pub struct JobCtx {
    ...
    /// (the poll world): `🖥️host/🦀️.rs`'s `run_job_to_completion` never pumps `poll`
...
/// (`✏️s/🔌️plugins/🌀️procedural/🦀️.rs`'s own comment) — but the JOB ITSELF must still be
