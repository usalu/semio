...
#[test]
fn actual_descriptor_provenance(){let provenance=AddValue::PROVENANCE;assert_eq!(provenance.owner,AddValue::DESCRIPTOR.owner);assert_eq!(provenance.source_path,format!("{}/🦀️.rs",provenance.owner));assert_eq!(provenance.descriptor_path,format!("{}/🔣️.json",provenance.owner));let scope=protocol::MutationLeafSourceScope{workspace_token:provenance.workspace_token,mutation_root:provenance.mutation_root,owner_layout:protocol::MutationOwnerLayout::Flat,taxonomy_path:provenance.taxonomy_path,mutation_payload_facet:"🦠️mutation",source_filename:"🦀️.rs",descriptor_filename:"🔣️.json"};assert!(protocol::validate_mutation_leaf_source(&AddValue::DESCRIPTOR,&provenance,&scope).is_ok());}
...
fn neutral_snapshot_and_diff_schema_vectors_match_serde() {
    let vectors:serde_json::Value=serde_json::from_str(include_str!("🔣️.json")).expect("neutral vectors");
