...
fn cache_retirement_preserves_shared_roots_and_drains_replaced_nested_values() {
    let fixture: serde_json::Value = serde_json::from_str(include_str!("🧪️fixtures/🧠️cache-retirement.json")).unwrap();
    for maximum_bytes in [1, 64, 4096] {
...
fn evaluation_snapshot_and_channel_owners_retire_exact_long_key_and_payload_bytes() {
    let fixture: serde_json::Value = serde_json::from_str(include_str!("🧪️fixtures/📸️evaluation-owners.json")).unwrap();
    for maximum_bytes in [1, 64, 4096] {
