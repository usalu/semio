...
/// 🔮️ Test-only — production JSON encoding of a `ColdOwner<T>`'s inner value routes through
/// `pack::json::to_json_string(&*owner)` instead (RUNTIME-DEPENDENCY-ELIMINATION-FOR-S-PLUGINS-AND-ARTIFACTS, 26/09/01, tenth-seam pass).
#[cfg(test)]
