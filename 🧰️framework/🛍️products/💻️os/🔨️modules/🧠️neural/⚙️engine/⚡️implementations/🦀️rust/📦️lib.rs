...
// 🚧️ Not migrated to `math::graph::algorithms::topo_levels`: doing so would create a circular crate
// dependency (`neural_engine` → `math::graph` → `math::graph::manifest` → `neural_engine`,
