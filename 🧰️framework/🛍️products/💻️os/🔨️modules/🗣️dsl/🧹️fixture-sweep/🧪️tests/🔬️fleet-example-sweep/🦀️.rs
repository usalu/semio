//! 🧭️ Fleet-wide example laws: every committed `📚️examples/<slug>/🖼️assets/*.semio` DSL fixture must satisfy its registered
//! artifact's text laws. Public kernel APIs and production providers only; the kernel-only M5 grammar/protocol laws live
//! in the kernel's own `os_dsl::fixture_sweep` module.
...
mod tests {
    ...
    use semio_framework_artifact_workflow_workflow::WorkflowSnapshot;
    ...
    fn registry() -> Vec<(&'static str, &'static str, CheckFn)> {
        vec![
            ...
            ("flow_app", <FlowHostSnapshot as semio_framework_os_kernel::os_store::ArtifactDsl>::envelope_id(), semio_framework_os_kernel::os_store::test_support::check_dsl_fixture_text_laws::<FlowHostSnapshot>),
            ...
            ("sequence", <SequenceSnapshot as semio_framework_os_kernel::os_store::ArtifactDsl>::envelope_id(), semio_framework_os_kernel::os_store::test_support::check_dsl_fixture_text_laws::<SequenceSnapshot>),
            ...
            for artifact in read_dir(&artifacts) {
                ...
                for slug in example_slug_dirs(&examples) {
                    ...
                    if assets.is_dir() {
                        ...
                        eprintln!("[dsl-fixture-sweep] soft-skip example coverage {}: missing {}/ with ≥1 .semio — mid-migration ({})", slug.display(), ASSETS_DIR_NAME, legacy_hint);
                    }
                }
            }
        }
