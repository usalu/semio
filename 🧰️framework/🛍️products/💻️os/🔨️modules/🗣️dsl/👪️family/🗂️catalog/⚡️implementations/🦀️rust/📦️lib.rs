...
mod tests {
    ...
    fn compat_pair_reuses_the_edge_grammar_directly() {
        let value = dsl_notation::parse_edge_text("b-l--b-s").expect("parse_edge_text");
    ...
    fn grammar_file_is_syntactically_valid() {
        ...
        let grammar = dsl_grammar::parse_grammar(source).expect("family-catalog.grammar must parse");
