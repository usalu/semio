...
/// @emoji 🎭️ The union of `protocol_core::REQUIRED_*` bits this crate understands; any bit outside
/// this mask makes a header (and therefore the whole file) unreadable.
const REQUIRED_KNOWN_MASK: u32 = protocol_core::REQUIRED_HASH_CHAIN | protocol_core::REQUIRED_SIGNED | protocol_core::REQUIRED_ENCRYPTED;

/// @emoji ✍️ Serializes the 32-byte header: magic, version, flags, `header_crc32` over bytes
/// `0..20` (CRC-32C, `pack_core::crc32c`), 8 reserved zero bytes.
fn build_header_bytes(required_flags: u32, optional_flags: u32) -> [u8; HEADER_SIZE] {
    ...
    let crc = pack_core::crc32c(&buf[0..20]);
...
/// `pack_core::PackError` variant wrapped in `ProtocolError::Pack` — all are directly constructible
...
fn validate_header<S: PackSource>(source: &S) -> Result<(), ProtocolError> {
    ...
    let computed_crc = pack_core::crc32c(&buf[0..20]);
...
fn varint_width(value: u64) -> u64 {
    ...
    pack_core::write_varint_u64(&mut buf, value);
    buf.len() as u64
}

/// @emoji 🚨️ Builds a `protocol_core::ProtocolError::Malformed` for this crate's own structural
/// checks (distinct from `pack_core::PackError`-wrapped errors, which cover pack-primitive-level
...
/// call site, mirroring `pack_format::encode_segment`'s proven pattern (the contract's own cited
/// living example) — `pack_core::crc32c` only accepts one contiguous slice, and this crate has no
...
fn encode_frame_into(scratch: &mut Vec<u8>, kind: u8, flags: u8, raw_len: Option<u64>, stored_payload: &[u8]) -> Result<(), ProtocolError> {
    ...
    pack_core::write_varint_u64(scratch, body_len);
    let body_start = scratch.len();
    scratch.push(kind);
    scratch.push(flags);
    if let Some(rl) = raw_len {
        pack_core::write_varint_u64(scratch, rl);
    }
    scratch.extend_from_slice(stored_payload);
    let crc = pack_core::crc32c(&scratch[body_start..]);
...
impl<'a> RecordFrame<'a> {
    ...
    /// `!compressed`; otherwise the caller must decompress via a `pack_core::CompressionCodec`
    /// keyed by `protocol_core::frame_codec_id(self.flags)`, targeting `self.raw_len` bytes.
...
fn decode_frame_in_slice(bytes: &[u8], pos: usize) -> Result<(RecordFrame<'_>, usize), ProtocolError> {
    let mut cursor = pos;
    let body_len = pack_core::read_varint_u64(bytes, &mut cursor)?;
    if body_len < 2 {
    ...
    let raw_len = if compressed {
        ...
        let v = pack_core::read_varint_u64(body, &mut p)?;
        payload_start = p;
        Some(v)
    } else {
        None
    ...
    let computed_crc = pack_core::crc32c(body);
    if stored_crc != computed_crc {
...
/// once a caller has confirmed `kind == protocol_core::REC_COMMIT` via `FrameCursor`/`ReverseFrameCursor`.
...
fn prepare_payload(codec: pack_core::CodecId, payload: &[u8]) -> Result<PreparedPayload<'_>, ProtocolError> {
    match codec.0 {
        0 => Ok((false, None, std::borrow::Cow::Borrowed(payload))),
        1 => {
            #[cfg(feature = "deflate")]
            {
                let compressed = pack_format::DeflateCodec.compress(payload)?;
...
impl<S: PackSink> SprWriter<S> {
    ...
    pub fn write_record(&mut self, kind: u8, critical: bool, payload: &[u8], codec: pack_core::CodecId) -> Result<u64, ProtocolError> {
    ...
    pub fn commit(&mut self) -> Result<u64, ProtocolError> {
        ...
        encode_frame_into(&mut self.scratch, protocol_core::REC_COMMIT, flags, None, &payload)?;
        self.sink.write_all(&self.scratch)?;
...
/// over-reads a legitimately short remaining source. Mirrors `pack_format::read_varint_u64_at`.
fn read_varint_via_source<S: PackSource>(source: &S, offset: u64, total_len: u64) -> Result<(u64, u64), ProtocolError> {
    ...
    let mut pos = 0usize;
    let value = pack_core::read_varint_u64(&tmp, &mut pos)?;
    Ok((value, i))
}
...
fn try_fast_path<S: PackSource>(source: &S, total_len: u64, limits: &ProtocolLimits) -> Option<RecoveryReport> {
    ...
    if kind != protocol_core::REC_COMMIT || frame_len != COMMIT_FRAME_LEN || flags & FRAME_FLAG_CRITICAL == 0 {
        return None;
    }
    ...
    loop {
        ...
        if prev_kind != protocol_core::REC_COMMIT || prev_flags & FRAME_FLAG_CRITICAL == 0 {
            return None;
        }
    ...
    while pos < total_len {
        ...
        match read_frame_via_source(source, pos, limits) {
            Ok((kind, _flags, _raw_len, payload, frame_len)) => {
                ...
                pos += frame_len;
                last_valid_end = pos;
                if kind == protocol_core::REC_COMMIT {
...
mod tests {
    ...
    fn read_header_exposes_decoded_fields_to_downstream_crates() {
        let bytes = build_header_bytes(protocol_core::REQUIRED_HASH_CHAIN, protocol_core::OPTIONAL_CANONICAL).to_vec();
        let header = read_header(&bytes).unwrap();
        assert_eq!(header, Header { version_major: FORMAT_VERSION_MAJOR, version_minor: FORMAT_VERSION_MINOR, required_flags: protocol_core::REQUIRED_HASH_CHAIN, optional_flags: protocol_core::OPTIONAL_CANONICAL });
    }
    ...
    fn build_small_file() -> Vec<u8> {
        ...
        writer.write_record(protocol_core::REC_DOC, true, b"doc-payload", pack_core::CodecId(0)).unwrap();
        writer.write_record(protocol_core::REC_EDIT, true, b"edit-payload", pack_core::CodecId(0)).unwrap();
    ...
    fn frame_round_trips_kind_flags_and_payload() {
        ...
        assert_eq!(doc.kind, protocol_core::REC_DOC);
        ...
        assert_eq!(edit.kind, protocol_core::REC_EDIT);
        assert_eq!(edit.payload(), b"edit-payload");

        let commit = cursor.next_frame().unwrap().unwrap();
        assert_eq!(commit.kind, protocol_core::REC_COMMIT);
    ...
    fn cursor_surfaces_unrecognized_extension_kind_transparently() {
        ...
        let options = WriteOptions::default();
        let mut writer = SprWriter::begin(Vec::new(), &options).unwrap();
        writer.write_record(0x50, false, b"extension-payload", pack_core::CodecId(0)).unwrap();
        writer.write_record(protocol_core::REC_DOC, true, b"doc-payload", pack_core::CodecId(0)).unwrap();
        let bytes = writer.into_sink();
    ...
    fn compressed_frame_round_trips_stored_and_raw_len() {
        ...
        writer.write_record(protocol_core::REC_DOC, true, payload, pack_core::CodecId(1)).unwrap();
        let bytes = writer.into_sink();
        ...
        let decompressed = pack_format::DeflateCodec.decompress(frame.stored, frame.raw_len.unwrap(), 1024).unwrap();
    ...
    fn commit_chain_links_prev_commit_offset_across_multiple_commits() {
        let options = WriteOptions::default();
        let mut writer = SprWriter::begin(Vec::new(), &options).unwrap();
        writer.write_record(protocol_core::REC_DOC, true, b"a", pack_core::CodecId(0)).unwrap();
        let commit_1_offset = writer.commit().unwrap();
        writer.write_record(protocol_core::REC_EDIT, true, b"b", pack_core::CodecId(0)).unwrap();
    ...
    fn parse_commit_payload_is_public_for_downstream_frontier_summaries() {
        // Mirrors how `protocol_history::FrontierSummary`/`protocol_materialize` are expected to
        // pull `last_commit_seq`/`chain_hash` out of a `REC_COMMIT` frame found via `FrameCursor`.
        let bytes = build_small_file();
        let mut cursor = FrameCursor::new(&bytes, HEADER_SIZE as u64);
        let commit_frame = std::iter::from_fn(|| cursor.next_frame().transpose()).map(Result::unwrap).find(|frame| frame.kind == protocol_core::REC_COMMIT).unwrap();
    ...
    fn recover_fast_path_walks_multi_commit_chain_to_genesis() {
        ...
        let mut writer = SprWriter::begin(Vec::new(), &options).unwrap();
        for i in 0..5u8 {
            writer.write_record(protocol_core::REC_EDIT, true, &[i], pack_core::CodecId(0)).unwrap();
            writer.commit().unwrap();
        }
        let bytes = writer.into_sink();
        let report = recover(&bytes, &ProtocolLimits::default(), RecoveryMode::LastCommit).unwrap();
    ...
    fn recover_forward_scan_truncates_to_last_commit_on_torn_tail() {
        ...
        writer.write_record(protocol_core::REC_EDIT, true, b"uncommitted", pack_core::CodecId(0)).unwrap();
    ...
    fn recover_last_valid_record_mode_trusts_past_the_last_commit() {
        ...
        writer.write_record(protocol_core::REC_DOC, true, b"a", pack_core::CodecId(0)).unwrap();
        writer.commit().unwrap();
        writer.write_record(protocol_core::REC_EDIT, true, b"uncommitted-but-well-formed", pack_core::CodecId(0)).unwrap();
        let bytes = writer.into_sink();
    ...
    fn recover_never_panics_on_truncation_at_every_byte() {
        ...
        let mut writer = SprWriter::begin(Vec::new(), &options).unwrap();
        for i in 0..4u8 {
            writer.write_record(protocol_core::REC_EDIT, true, &[i; 5], pack_core::CodecId(0)).unwrap();
        }
        writer.commit().unwrap();
        writer.write_record(protocol_core::REC_EDIT, true, b"tail-record-after-commit", pack_core::CodecId(0)).unwrap();
        let bytes = writer.into_sink();
    ...
    fn position_tracks_sink_length_across_writes() {
        ...
        writer.write_record(protocol_core::REC_DOC, true, b"x", pack_core::CodecId(0)).unwrap();
