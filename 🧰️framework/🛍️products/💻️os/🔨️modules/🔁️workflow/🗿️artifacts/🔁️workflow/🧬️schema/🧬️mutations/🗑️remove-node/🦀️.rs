    ...
    fn inverse(&self, base: &WorkflowSnapshot) -> Vec<WorkflowMutation> {
        {
            ...
            let mut ops: Vec<WorkflowMutation> =
                base.graph.edges.iter().filter(|edge| edge.source_node_id == self.node_id || edge.target_node_id == self.node_id).map(|edge| WorkflowMutation::ConnectPorts(ConnectPorts { edge: edge.clone() })).collect();
            ops.extend(base.parameter_bindings.iter().filter(|binding| binding.node_id == self.node_id).map(|binding| WorkflowMutation::BindParameterField(BindParameterField { binding: binding.clone() })));
            ops.extend(base.input_bindings.iter().filter(|binding| binding.node_id == self.node_id).map(|binding| WorkflowMutation::BindInput(BindInput { binding: binding.clone() })));
            ops.extend(base.output_bindings.iter().filter(|binding| binding.node_id == self.node_id).map(|binding| WorkflowMutation::BindOutput(BindOutput { binding: binding.clone() })));
            ops.push(WorkflowMutation::AddNode(AddNode { node: node.clone() }));
            ops
        }
    }
    fn label(&self) -> protocol::LocalizedLabel {
        protocol::LocalizedLabel::native(&format!("Remove workflow node {}", self.node_id), &format!("Arbeitsablaufknoten {} entfernen", self.node_id))
    }
