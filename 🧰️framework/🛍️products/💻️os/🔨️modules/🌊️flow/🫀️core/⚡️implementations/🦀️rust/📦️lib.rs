...
/// 🔢️ `BTreeSet<String>` has no blanket `dsl::DslField` impl (only `Vec`/`BTreeMap`/arrays do) — a
...
/// 🔵️ Local twin of `neural::Neuron` — a one-variant `dsl::DslEnum` (not a plain `DslRecord`) purely
...
mod flow_vcs_tests {
    ...
    /// 📜️ Exercises `store::OpText` for every `FlowOperation` variant — the ground-truth proof for the
    ...
    fn default_flow_example_dsl_round_trips() {
        ...
        let fixture = <FlowFixture as store::DocumentDsl>::parse_dsl(text).expect("🌊️default.flow must parse");
        store::test_support::assert_dsl_round_trip(&fixture);
        store::test_support::assert_dsl_pack_equivalence(&fixture);
    }
}
...
mod tests {
    ...
    fn flow_fixture_with_synapses_builds_dag_edges_and_ports() {
        let mut host = host_with_test_bridge();
        host.set_neuron_kind_infos_json(&flow_neuron_kind_infos_json());
        host.replace_fixture(<FlowFixture as store::DocumentDsl>::parse_dsl(include_str!("../../../📚️examples/🌊️default.flow")).expect("fixture"));
