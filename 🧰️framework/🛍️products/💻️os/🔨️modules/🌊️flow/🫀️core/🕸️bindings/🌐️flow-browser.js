...
var defaultHost;
async function createFlowBrowserFeatures({ source, imports = {}, instantiate, ...hostOptions } = {}) {
  if (source === undefined)
...
class FlowSession {
  constructor() {
    if (!defaultHost)
      throw new Error("Flow browser ABI must be initialized before opening a session");
