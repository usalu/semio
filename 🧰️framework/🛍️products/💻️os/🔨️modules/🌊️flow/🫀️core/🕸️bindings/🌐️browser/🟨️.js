...
/* ../../../🕸️wasm/🌐️browser/🏃️runtime/🟨️.js */
import { FlowFeatureGroups, FlowOperation, FlowOperationFields, attachFlowSurface, createFlowFeatures, createFlowHost, isFlowSessionOpenRejected, renderFlowSurface } from "../🖥️host/🟨️.js";
...
async function createFlowBrowserRuntime({ source, imports = {}, instantiate, bindings, ...hostOptions } = {}) {
  ...
  if (!exports || !(memory instanceof WebAssembly.Memory))
    throw new Error("Flow Wasm instance must export memory");
...
var invokeFeature = Symbol("FlowSession.invokeFeature");
var sessionAuthority = Symbol("FlowSession.owner");
...
function deferredFlowTask(ready, start) {
  ...
  const result = ready.then((features) => {
    if (cancelled)
      throw new Error("Flow feature cancelled before admission");
...
for (const name of Object.keys(FlowOperation).slice(1)) {
  if (name === "attachSurface" || name === "renderFrame")
    continue;
