...
const testSourceUrl = new URL("../../🕸️wasm/📦️packages/🟨️javascript/📜️script.ts", import.meta.url);
...
export const FLOW_WASM_SCHEMA_URL = new URL("../../🧬️schema/🔣️.json", testSourceUrl.href);
...
const contractPath = join(testFileUrlToPath(new URL(".", testSourceUrl)), "../../../🕸️wasm/🧫️fixtures/📡️abi.json");
...
export function writeFlowBrowserDeclaration(): string {
  const path = join(testFileUrlToPath(new URL(".", testSourceUrl)), declarationName);
...
export async function testFlowBrowserDeclaration(packageRoot: string): Promise<void> {
  ...
  const { FlowSession } = await import("../../🕸️wasm/📦️packages/🟨️javascript/🌐️flow-browser.js");
  const fixture = JSON.parse(readFileSync(join(testFileUrlToPath(new URL(".", testSourceUrl)), "../../🧫️fixtures/📝️browser-types.json"), "utf8"));
  ...
  for (const [subpath, entry] of Object.entries(fixture.package.exports)) {
    ...
    const resolved = ts.resolveModuleName(name, join(testFileUrlToPath(new URL(".", testSourceUrl)), "consumer.ts"), { moduleResolution: ts.ModuleResolutionKind.Bundler, module: ts.ModuleKind.ESNext }, ts.sys).resolvedModule;
  ...
  const declarationPath = join(testFileUrlToPath(new URL(".", testSourceUrl)), declarationName);
  ...
  assert.equal(readFileSync(join(testFileUrlToPath(new URL(".", testSourceUrl)), declarationName), "utf8"), text);
