...
export async function testFlowBrowserClock() {
  const fixture = JSON.parse(await readFile(new URL("../../../🧪️fixtures/⏱️browser-startup.json", import.meta.url), "utf8"));
  const schema = JSON.parse(await readFile(new URL("../../../🧪️fixtures/⏱️browser-startup.schema.json", import.meta.url), "utf8"));
  ...
  try {
    const { features } = await createFlowBrowserFeatures({ source: await readFile(new URL("../../../../🫀️core/pkg/flow_core_bg.wasm", import.meta.url)) });
