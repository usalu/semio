...
fn flow_retirement_typed_serde_oracle_and_exact_bytes_survive_worker_transfer() {
    ...
    for maximum in [1, 4096] {
        ...
        while !matches!(oracle_retirement.close_step(1, maximum).unwrap(), SnapshotRetirementStep::Complete) {}
        ...
        let released = std::thread::spawn(move || {
            ...
            for _ in 0..100_000 {
                match retirement.close_step(1, maximum).unwrap() {
                    ...
                    SnapshotRetirementStep::Blocked => panic!("positive Flow retirement grant blocked"),
                }
            }
