...
fn graph_parameter_preserves_labels_and_existing_range_expansion() {
    ...
    for case in fixture.get("cases").and_then(crate::os_pack::json::Value::as_array).unwrap() {
        ...
        let mut widget = Widget::InputSlider {
            id: case.get("widgetId").and_then(crate::os_pack::json::Value::as_str).unwrap().into(),
            label: case.get("label").and_then(crate::os_pack::json::Value::as_str).unwrap().into(),
            value: before.get("value").and_then(crate::os_pack::json::Value::as_f64).unwrap(),
            min: before.get("min").and_then(crate::os_pack::json::Value::as_f64).unwrap(),
            max: before.get("max").and_then(crate::os_pack::json::Value::as_f64).unwrap(),
            step: before.get("step").and_then(crate::os_pack::json::Value::as_f64).unwrap(),
        };
        ...
        assert_eq!(wire.get("id"), case.get("widgetId"));
        assert_eq!(wire.get("label"), case.get("label"));
        let after = case.get("after").unwrap();
        for field in ["value", "min", "max", "step"] {
            assert_eq!(wire.get(field).and_then(crate::os_pack::json::Value::as_f64), after.get(field).and_then(crate::os_pack::json::Value::as_f64));
        }
