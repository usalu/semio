...
impl<'a> ArtifactCanonicalJsonArray<'a> {
    pub fn new(values: impl Iterator<Item = ArtifactCanonicalJsonValue<'a>> + Send + 'a) -> Self {
...
impl Default for ArtifactCanonicalEditEncoder {
    fn default() -> Self {
        Self { frames: Vec::with_capacity(ARTIFACT_CANONICAL_JSON_DEPTH), depth: 0, root_address: 0, started: false }
    }
}
...
impl ArtifactCanonicalEditEncoder {
    ...
    fn bind<T: ArtifactCanonicalJson>(&mut self, root: &T) -> Result<(), String> {
        ...
        if self.frames.is_empty() {
            self.frames.push(Some(BorrowedFrame::Pending(value)));
        } else {
            self.frames[0] = Some(BorrowedFrame::Pending(value));
        }
        self.depth = 1;
    ...
    fn push(&mut self, value: BorrowedFrame<'static>) -> Result<(), String> {
        ...
        if self.depth == self.frames.len() {
            ...
            self.frames[self.depth] = Some(value);
        }
        self.depth += 1;
        Ok(())
    }
        ...
        for _ in 0..ARTIFACT_CANONICAL_JSON_DEPTH * 8 {
            ...
            match frame {
                ...
                BorrowedFrame::Array { mut values, started, emitted } => {
                    if !started {
                        self.frames[top] = Some(BorrowedFrame::Array { values, started: true, emitted });
                        return Ok(Some(b'['));
                    }
                    let Some(value) = values.values.next() else {
                        self.depth -= 1;
                        return Ok(Some(b']'));
                    };
                ...
                BorrowedFrame::Object { mut values, phase, emitted, mut current } => match phase {
                    0 => {
                        self.frames[top] = Some(BorrowedFrame::Object { values, phase: 1, emitted, current });
                        return Ok(Some(b'{'));
                    }
                    1 => {
                        let Some((key, value)) = values.values.next() else {
                            self.depth -= 1;
                            return Ok(Some(b'}'));
                        };
                    ...
                    2 => {
                        self.frames[top] = Some(BorrowedFrame::Object { values, phase: 3, emitted, current });
                        return Ok(Some(b':'));
                    }
                    3 => {
