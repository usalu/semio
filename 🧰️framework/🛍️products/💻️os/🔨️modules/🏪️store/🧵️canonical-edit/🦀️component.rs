...
pub trait ArtifactCanonicalJson: Sync {
    fn canonical_json_node(&self, _path: &[usize]) -> Result<ArtifactCanonicalJsonNode<'_>, String> { Err(invalid_path()) }
    fn canonical_json_key(&self, _object_path: &[usize], _index: usize) -> Result<&str, String> { Err(invalid_path()) }
    fn canonical_json_borrowed_root(&self) -> Result<Option<ArtifactCanonicalJsonValue<'_>>, String> { Ok(None) }
}

fn invalid_path() -> String { "canonical-edit.invalid-typed-path".into() }
...
impl<'a, M: ArtifactCanonicalJson> CanonicalEditNode<'a, M> {
        ...
        match self {
            Self::Edit(edit) => {
                fields[..10].copy_from_slice(&[
                    ("id", true), ("actor", edit.actor.is_some()), ("forwards", true), ("inverse", true),
                    ("mutationMeta", !edit.mutation_meta.is_empty()), ("description", edit.description.is_some()), ("coalesceKey", edit.coalesce_key.is_some()),
                    ("sequenceNumber", true), ("startedAt", true), ("finishedAt", edit.finished_at.is_some()),
                ]);
            }
            Self::Meta(meta) => fields = [
                ("mutation_id", meta.mutation_id.is_some()), ("dependencies", !meta.dependencies.is_empty()), ("base_version", true),
                ("author_id", meta.author_id.is_some()), ("timestamp", true), ("undo_policy", true), ("payload_hash", meta.payload_hash.is_some()),
                ("semantic_kind", meta.semantic_kind.is_some()), ("label", meta.label.is_some()), ("group_id", meta.group_id.is_some()),
                ("origin", !meta.origin.is_owner()), ("", false),
            ],
    ...
    fn child(&self, index: usize) -> Result<Self, String> {
        ...
        Ok(match self {
            ...
            Self::Clock(clock) => Self::Scalar(N::U64(match index { 0 => clock.actor, 1 => clock.physical_ms, 2 => clock.logical, _ => return Err(invalid_path()) })),
            ...
            Self::Origin(origin) => match origin {
                ...
                crate::os_spr::MutationOrigin::Contributed { plugin_id, mutation_id, payload_hash } => match index {
                    0 => Self::Scalar(N::String("contributed")), 1 => Self::Scalar(N::String(plugin_id)),
                    2 => Self::Scalar(N::String(&mutation_id.0)), 3 => Self::Hash(&payload_hash.0), _ => return Err(invalid_path()),
                },
                crate::os_spr::MutationOrigin::Transaction { initiator } => match index {
                    0 => Self::Scalar(N::String("transaction")), 1 => Self::Target(initiator), _ => return Err(invalid_path()),
                },
                _ => return Err(invalid_path()),
            },
            Self::Target(target) => match field_at(&self.fields(), index)?.0 {
                0 => Self::Scalar(N::String(&target.artifact_id)), 1 => Self::Scalar(N::String(&target.artifact_kind)),
                2 => Self::Scalar(N::String(target.dialect.as_deref().ok_or_else(invalid_path)?)), _ => return Err(invalid_path()),
            },
            _ => return Err(invalid_path()),
        })
    }
    ...
    fn node(&self, path: &[usize]) -> Result<ArtifactCanonicalJsonNode<'a>, String> {
        if let Self::Mutation(value) = self { return value.canonical_json_node(path); }
        if let Some((first, rest)) = path.split_first() { return self.child(*first)?.node(rest); }
        Ok(match self {
    ...
    fn key(&self, path: &[usize], index: usize) -> Result<&'a str, String> {
        if let Self::Mutation(value) = self { return value.canonical_json_key(path, index); }
        if let Some((first, rest)) = path.split_first() { return self.child(*first)?.key(rest, index); }
...
impl<M: ArtifactCanonicalJson> ArtifactCanonicalJson for Edit<M> {
    fn canonical_json_node(&self, path: &[usize]) -> Result<ArtifactCanonicalJsonNode<'_>, String> { CanonicalEditNode::Edit(self).node(path) }
    fn canonical_json_key(&self, path: &[usize], index: usize) -> Result<&str, String> { CanonicalEditNode::Edit(self).key(path, index) }
    fn canonical_json_borrowed_root(&self) -> Result<Option<ArtifactCanonicalJsonValue<'_>>, String> { Ok(Some(CanonicalEditNode::Edit(self).borrowed_value())) }
}
//#endregion 🧬️TypedCanonicalSource

//#region 🔣️ByteEncoder
#[derive(Clone, Copy, Default)]
struct JsonFrame { kind: u8, phase: u8, index: usize, length: usize, offset: usize }

fn canonical_escape(byte: u8, escape: &mut [u8; 6]) -> usize {
    match byte {
            b'"' => { escape[..2].copy_from_slice(b"\\\""); 2 }
            b'\\' => { escape[..2].copy_from_slice(b"\\\\"); 2 }
            b'\n' => { escape[..2].copy_from_slice(b"\\n"); 2 }
            b'\r' => { escape[..2].copy_from_slice(b"\\r"); 2 }
            b'\t' => { escape[..2].copy_from_slice(b"\\t"); 2 }
            8 => { escape[..2].copy_from_slice(b"\\b"); 2 }
            12 => { escape[..2].copy_from_slice(b"\\f"); 2 }
            0..=31 => {
...
struct ScalarBytes { bytes: [u8; 64], length: usize }
...
impl Default for ArtifactCanonicalJsonCursor {
    fn default() -> Self {
        Self { frames: [JsonFrame::default(); ARTIFACT_CANONICAL_JSON_DEPTH], path: [0; ARTIFACT_CANONICAL_JSON_DEPTH], depth: 1, maximum_depth: ARTIFACT_CANONICAL_JSON_DEPTH, scalar: ScalarBytes { bytes: [0; 64], length: 0 }, escape: [0; 6], escape_length: 0, escape_offset: 0 }
    }
}

impl ArtifactCanonicalJsonCursor {
    fn push(&mut self, index: usize) -> Result<(), String> {
        if self.depth >= self.maximum_depth { return Err("canonical-edit.depth-limit".into()); }
        ...
        loop {
            ...
            if frame.kind == 0 {
                ...
                self.frames[top].kind = match node {
                    ...
                    value => { self.scalar_node(value)?; 4 }
                };
                continue;
            }
            match (frame.kind, frame.phase) {
                (1, 0) => { self.frames[top].phase = 1; return Ok(Some(b'"')); }
                (1, 1) => {
                    let ArtifactCanonicalJsonNode::String(text) = source.canonical_json_node(&self.path[..top])? else { return Err("canonical-edit.source-shape-changed".into()); };
                    if let Some(byte) = self.escaped_byte(text, top) { return Ok(Some(byte)); }
                    self.frames[top].phase = 2;
                }
                (1, 2) => { self.depth -= 1; return Ok(Some(b'"')); }
                (2, 0) => { self.frames[top].phase = 1; return Ok(Some(b'[')); }
                (2, 1) if frame.index == frame.length => { self.depth -= 1; return Ok(Some(b']')); }
                (2, 1) => { self.frames[top].phase = 2; self.push(frame.index)?; }
                (2, 2) => {
                    self.frames[top].index += 1; self.frames[top].phase = 1;
                    if self.frames[top].index < frame.length { return Ok(Some(b',')); }
                }
                (3, 0) => { self.frames[top].phase = 1; return Ok(Some(b'{')); }
                (3, 1) if frame.index == frame.length => { self.depth -= 1; return Ok(Some(b'}')); }
                (3, 1) => { self.frames[top].phase = 2; self.frames[top].offset = 0; return Ok(Some(b'"')); }
                (3, 2) => {
                    let text = source.canonical_json_key(&self.path[..top], frame.index)?;
                    if let Some(byte) = self.escaped_byte(text, top) { return Ok(Some(byte)); }
                    self.frames[top].phase = 3;
                }
                (3, 3) => { self.frames[top].phase = 4; return Ok(Some(b'"')); }
                (3, 4) => { self.frames[top].phase = 5; return Ok(Some(b':')); }
                (3, 5) => { self.frames[top].phase = 6; self.push(frame.index)?; }
                (3, 6) => {
                    self.frames[top].index += 1; self.frames[top].phase = 1;
                    if self.frames[top].index < frame.length { return Ok(Some(b',')); }
                }
                (4, _) => {
                    if frame.offset == self.scalar.length { self.depth -= 1; continue; }
                    let byte = self.scalar.bytes[frame.offset]; self.frames[top].offset += 1; return Ok(Some(byte));
                }
...
impl ArtifactStoreOneItemAuthorityRetirement {
    pub(super) fn new(authority: Arc<ArtifactStoreOneItemLiveAuthority>) -> Self { Self { authority: Some(authority), strings: Default::default(), active: None } }
}
    ...
    fn close_step(&mut self, items: usize, bytes: usize) -> Result<SnapshotRetirementStep, String> {
        ...
        if let Some(active) = self.active.as_mut() {
            return match active.close_step(1, bytes)? {
                SnapshotRetirementStep::Complete => { assert!(active.terminal_is_empty()); self.active = None; Ok(SnapshotRetirementStep::Pending { released_items: 1, released_bytes: 0 }) }
                step => Ok(step),
            };
        }
        ...
        if let Some(authority) = self.authority.take() {
            if let Some(authority) = Arc::into_inner(authority) { self.strings = [Some(authority.actor), authority.group_id]; }
            return Ok(SnapshotRetirementStep::Pending { released_items: 1, released_bytes: 0 });
        }
        Ok(SnapshotRetirementStep::Complete)
    }
    fn terminal_is_empty(&self) -> bool { self.authority.is_none() && self.strings.iter().all(Option::is_none) && self.active.is_none() }
}

impl Drop for ArtifactStoreOneItemAuthorityRetirement {
    fn drop(&mut self) { assert!(self.terminal_is_empty(), "Store live authority dropped before bounded string retirement completed"); }
}
...
impl<P, M> ArtifactStoreOneItemSealer<P, M> {
    pub(super) fn new(authority: Arc<ArtifactStoreOneItemLiveAuthority>, edit: Edit<M>, post: Arc<P>, mutation_retirement: Arc<dyn ArtifactOwnedValueRetirementFactory<M>>, snapshot_retirement: Arc<dyn SnapshotRetirementFactory<P>>) -> Self {
        Self {
            authority: Some(authority), edit: Some(Box::new(edit)), post: Some(post), prepared: None, encoder: ArtifactCanonicalEditEncoder::default(),
            hash: semio_framework_hash::Sha256::new(), transcript: semio_framework_hash::Sha256::new(), phase: 0, header_offset: 0,
            canonical_bytes: 0, hashed_canonical_bytes: 0, completed_bytes: 0, turns: 0, last_chunk: [0; ARTIFACT_CANONICAL_JSON_CHUNK_BYTES], last_length: 0, last_canonical: false,
            replay: None, mutation_retirement: Some(mutation_retirement), snapshot_retirement: Some(snapshot_retirement), active_retirement: None,
            retirement_strings: Default::default(), identities: std::array::from_fn(|_| Vec::with_capacity(ARTIFACT_STORE_ONE_ITEM_ID_BYTES)), identity_index: 0, cancelled: false, closing: false,
        }
    }

    pub fn prepared(&self) -> Option<&ArtifactStoreOneItemPrepared<P, M>> { self.prepared.as_ref() }
    pub fn take_prepared(&mut self) -> Option<ArtifactStoreOneItemPrepared<P, M>> { self.prepared.take() }
    pub fn cancel(&mut self) { self.cancelled = true; }
    pub fn begin_close(&mut self) { self.closing = true; self.replay = None; }
    pub fn canonical_chunk(&self) -> &[u8] { if self.last_canonical { &self.last_chunk[..self.last_length] } else { &[] } }
    ...
    pub fn checkpoint(&self) -> ArtifactStoreOneItemSealCheckpoint {
        ...
        ArtifactStoreOneItemSealCheckpoint {
            version: 1, operation: authority.operation.0, generation: authority.generation.0, base_revision: authority.base_revision,
            authority_digest: self.authority_digest(), phase: self.phase, completed_bytes: self.completed_bytes, canonical_bytes: self.canonical_bytes,
    ...
    pub fn restore_checkpoint(&mut self, checkpoint: ArtifactStoreOneItemSealCheckpoint) -> Result<(), String> {
        ...
        if self.phase != 0 || self.completed_bytes != 0 || self.cancelled || self.closing || checkpoint.version != 1 || checkpoint.phase > 6
            || checkpoint.operation != authority.operation.0 || checkpoint.generation != authority.generation.0 || checkpoint.base_revision != authority.base_revision
            || checkpoint.authority_digest != self.authority_digest()
            || checkpoint.canonical_bytes > CANONICAL_EDIT_MAXIMUM_BYTES || checkpoint.completed_bytes > CANONICAL_EDIT_MAXIMUM_BYTES * 2 + CANONICAL_EDIT_MAXIMUM_OVERHEAD_BYTES {
    ...
    fn verify_replay(&mut self) -> Result<(), String> {
        let Some(target) = self.replay else { return Ok(()); };
        if self.completed_bytes == target.completed_bytes && self.phase == target.phase {
            if self.canonical_bytes != target.canonical_bytes || self.transcript.clone().finalize() != target.prefix_digest { return Err("canonical-edit.checkpoint-prefix".into()); }
            self.replay = None;
    ...
    fn header_byte(&self, offset: usize) -> Option<u8> {
        ...
        for segment in [fixed.as_slice(), &4u64.to_be_bytes(), b"edit".as_slice(), &(id.len() as u64).to_be_bytes(), id, &self.canonical_bytes.to_be_bytes()] {
            if at < segment.len() { return Some(segment[at]); }
            at -= segment.len();
        }
        None
    }
    ...
    fn authority_digest(&self) -> [u8; 32] {
        ...
        CursorRevisionAccumulator::hash_record(b"one-item-authority", &[&authority.operation.0.to_be_bytes(), &authority.generation.0.to_be_bytes(), &authority.base_revision, &authority.next_sequence_number.to_be_bytes(), &authority.next_clock.actor.to_be_bytes(), &authority.next_clock.physical_ms.to_be_bytes(), &authority.next_clock.logical.to_be_bytes(), authority.actor.as_bytes(), &[u8::from(authority.group_id.is_some())], authority.group_id.as_deref().unwrap_or("").as_bytes()])
    }

    pub fn terminal_is_empty(&self) -> bool {
        self.closing && self.authority.is_none() && self.edit.is_none() && self.post.is_none() && self.prepared.is_none()
            && self.active_retirement.is_none() && self.retirement_strings.iter().all(Option::is_none)
            && self.mutation_retirement.is_none() && self.snapshot_retirement.is_none()
...
impl<P: Send + Sync + 'static, M: ArtifactCanonicalJson + Send + 'static> ArtifactStoreOneItemSealer<P, M> {
    pub fn advance(&mut self, grant: ArtifactStoreOneItemGrant) -> Result<ArtifactStoreOneItemPreparationStep, String> {
        ...
        match self.phase {
            0 => {
                self.authority.as_ref().ok_or_else(|| "canonical-edit.authority-missing".to_string())?
                    .validate_semantic_edit(self.edit.as_ref().ok_or_else(|| "canonical-edit.owner-missing".to_string())?)?;
                self.phase = 1;
            }
            1 | 3 => {
                ...
                if self.encoder.is_complete() {
                    if self.phase == 1 { self.phase = 2; self.encoder.reset()?; } else { self.phase = 4; }
                }
            }
            2 => {
                while self.last_length < maximum {
                    let Some(byte) = self.header_byte(self.header_offset) else { self.phase = 3; break };
                    self.last_chunk[self.last_length] = byte;
                    self.last_length += 1; self.header_offset += 1;
                }
                self.hash.update(&self.last_chunk[..self.last_length]);
            }
            4 => {
                if self.hashed_canonical_bytes != self.canonical_bytes { return Err("canonical-edit.source-length-changed".into()); }
                ...
                while self.last_length < maximum && self.identity_index < 3 {
                    ...
                    if target.len() == source.len() { self.identity_index += 1; continue; }
                    if target.len() == ARTIFACT_STORE_ONE_ITEM_ID_BYTES { return Err("canonical-edit.identity-limit".into()); }
                    let byte = source[target.len()];
                    target.push(byte);
                    self.last_chunk[self.last_length] = byte;
                    self.last_length += 1;
                }
                if self.identity_index == 3 { self.phase = 5; }
            }
            5 => { self.phase = 6; }
    ...
    pub fn close_step(&mut self, grant: ArtifactStoreOneItemGrant) -> Result<SnapshotRetirementStep, String> {
        ...
        if let Some(active) = self.active_retirement.as_mut() {
            return match active.close_step(grant.maximum_items.min(1), grant.maximum_bytes)? {
                ...
                SnapshotRetirementStep::Complete => {
                    if !active.terminal_is_empty() { return Err("canonical-edit.retirement-witness".into()); }
        ...
        if self.mutation_retirement.take().is_some() || self.snapshot_retirement.take().is_some() { return Ok(SnapshotRetirementStep::Pending { released_items: 1, released_bytes: 0 }); }
        Ok(SnapshotRetirementStep::Complete)
    }
}

impl<P, M> Drop for ArtifactStoreOneItemSealer<P, M> {
    fn drop(&mut self) { assert!(self.terminal_is_empty(), "canonical edit sealer dropped before exact owners were transferred or retired"); }
}
//#endregion 🔏️Sealing
...
mod tests {
    ...
    impl ArtifactCanonicalJson for FixtureMutation {
        fn canonical_json_node(&self, path: &[usize]) -> Result<ArtifactCanonicalJsonNode<'_>, String> {
            ...
            Ok(match path {
                [] => N::Object(1), [0] => N::Object(4), [0, 0] => N::String(text),
                [0, 1] => N::Array(nested.len()), [0, 1, index] => N::String(nested.get(*index).ok_or_else(invalid_path)?),
                [0, 2] => N::Bool(*enabled), [0, 3] => N::I64(*amount), _ => return Err(invalid_path()),
            })
        }
        fn canonical_json_key(&self, path: &[usize], index: usize) -> Result<&str, String> {
            match path { [] if index == 0 => Ok("Replace"), [0] => ["text", "nested", "enabled", "amount"].get(index).copied().ok_or_else(invalid_path), _ => Err(invalid_path()) }
        }
    }
    ...
    struct FixtureRetirement { text: Option<String>, nested: Vec<String>, active: Option<ArtifactStoreStringRetirement> }
    ...
    impl ErasedSnapshotRetirement for FixtureRetirement {
        fn close_step(&mut self, items: usize, bytes: usize) -> Result<SnapshotRetirementStep, String> {
            ...
            if let Some(active) = self.active.as_mut() {
                ...
                if matches!(step, SnapshotRetirementStep::Complete) { assert!(active.terminal_is_empty()); self.active = None; }
        ...
        fn terminal_is_empty(&self) -> bool { self.text.is_none() && self.nested.is_empty() && self.active.is_none() }
    }
    ...
    impl SnapshotRetirementFactory<u64> for FixtureSnapshotRetirement {
        fn retire(&self, snapshot: Arc<u64>) -> Box<dyn ErasedSnapshotRetirement> { Box::new(FixtureRootRetirement(Some(snapshot))) }
    }
    impl ErasedSnapshotRetirement for FixtureRootRetirement {
        fn close_step(&mut self, items: usize, bytes: usize) -> Result<SnapshotRetirementStep, String> {
            if items == 0 || bytes == 0 { return Ok(SnapshotRetirementStep::Blocked); }
            if self.0.take().is_some() { return Ok(SnapshotRetirementStep::Pending { released_items: 1, released_bytes: 0 }); }
            Ok(SnapshotRetirementStep::Complete)
        }
    ...
    pub(super) fn authority() -> Arc<ArtifactStoreOneItemLiveAuthority> {
        Arc::new(ArtifactStoreOneItemLiveAuthority {
            operation: semio_framework_job::OperationId(11), generation: semio_framework_job::Generation(7), base_revision: [9; 32],
            base_applied_edit_count: 3, next_sequence_number: 4, next_clock: HybridLogicalTimestamp { actor: 1, physical_ms: 42, logical: 2 },
            actor: "actor-1".into(), group_id: Some("group-1".into()),
        })
    }
    ...
    fn close(sealer: &mut ArtifactStoreOneItemSealer<u64, FixtureMutation>, bytes: usize) {
        ...
        for _ in 0..100_000 {
            match sealer.close_step(ArtifactStoreOneItemGrant { maximum_items: 1, maximum_bytes: bytes }).unwrap() {
                SnapshotRetirementStep::Complete => { assert!(sealer.terminal_is_empty()); return; }
    ...
    fn finish(sealer: &mut ArtifactStoreOneItemSealer<u64, FixtureMutation>, bytes: usize) -> [u8; 32] {
        ...
        for _ in 0..100_000 {
            ...
            if matches!(step, ArtifactStoreOneItemPreparationStep::Prepared(_)) { return sealer.prepared().unwrap().edit_digest(); }
        }
    ...
    fn canonical_sealer_tiny_grants_replay_and_cross_worker_transfer_preserve_exact_digest() {
        ...
        for bytes in [1, 2, 7, 256, 4096] {
            ...
            for _ in 0..19 { owner.advance(ArtifactStoreOneItemGrant { maximum_items: 1, maximum_bytes: bytes }).unwrap(); }
            ...
            close(&mut replay, 1);
            let mut moved = std::thread::spawn(move || { assert_eq!(finish(&mut owner, bytes), oracle); owner }).join().unwrap();
    ...
    fn canonical_sealer_rejects_stale_checkpoint_forged_prefix_and_rebound_owners() {
        let authority = authority();
        let mut owner = sealer(&authority);
        for _ in 0..4 { owner.advance(ArtifactStoreOneItemGrant { maximum_items: 1, maximum_bytes: 7 }).unwrap(); }
        let checkpoint = owner.checkpoint();
        for hostile in 0..5 {
            let mut altered = checkpoint;
            match hostile { 0 => altered.operation += 1, 1 => altered.generation += 1, 2 => altered.base_revision[0] ^= 1, 3 => altered.authority_digest[0] ^= 1, _ => altered.version += 1 }
            let mut replay = sealer(&authority);
        ...
        for _ in 0..100 { if replay.advance(ArtifactStoreOneItemGrant { maximum_items: 1, maximum_bytes: 1 }).is_err() { rejected = true; break; } }
    ...
    fn canonical_sealer_cancellation_at_every_phase_retires_exact_owners_and_allows_retry() {
        ...
        for phase in 0..=6 {
            let mut owner = sealer(&authority);
            while owner.phase < phase { owner.advance(ArtifactStoreOneItemGrant { maximum_items: 1, maximum_bytes: 4096 }).unwrap(); }
            owner.cancel();
            let checkpoint = owner.checkpoint();
        ...
        for length in fixture["largeTextBytes"].as_array().unwrap() {
            for origin in fixture["origins"].as_array().unwrap() {
                ...
                *text = "x".repeat(length.as_u64().unwrap() as usize);
                ...
                let mut encoder = ArtifactCanonicalJsonCursor::default();
                let mut chunk = [0; 256];
                while !encoder.is_complete() { let count = encoder.encode_chunk(&edit, &mut chunk).unwrap(); actual.extend_from_slice(&chunk[..count]); }
    ...
    fn canonical_authority_final_unicode_strings_retire_under_single_byte_grants() {
        ...
        for _ in 0..100 {
            match retirement.close_step(1, 1).unwrap() {
                SnapshotRetirementStep::Pending { released_items, released_bytes } => { assert!(released_items <= 1 && released_bytes <= 1); released += released_bytes; }
                SnapshotRetirementStep::Complete => { assert!(retirement.terminal_is_empty()); assert_eq!(released, "actor-🧵".len() + "group-✓".len()); return; }
