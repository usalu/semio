...
#[must_use]
pub struct ArtifactCanonicalValue<R: DslValueSource> { source: R }

impl<R: DslValueSource> ArtifactCanonicalValue<R> {
    pub fn into_source(self) -> R { self.source }
    pub(super) fn value(&self) -> &DslValue { self.source.value() }
}
...
#[derive(Clone, Copy, Debug)]
pub struct ArtifactCanonicalValueGrant { pub maximum_items: usize, pub maximum_bytes: usize }
...
enum Phase { Inspect, FinishNode, InitializeTable, HashKey, ProbeKey, CompareKey, ClearTable, Complete, Closing, Closed }

#[derive(Clone, Copy)]
struct KeySlot { hash: u64, index: usize }

struct Frame { next: usize, length: usize }
...
fn reserve_table_with(length: usize, remaining: usize, reserve: impl FnOnce(&mut Vec<Option<KeySlot>>, usize) -> Result<(), std::collections::TryReserveError>) -> Result<Vec<Option<KeySlot>>, &'static str> {
    ...
    if bytes > remaining { return Err("canonical-value.capacity-limit"); }
    let mut table = Vec::new();
    reserve(&mut table, length).map_err(|_| "canonical-value.allocation-failed")?;
    if table.capacity().saturating_mul(size_of::<Option<KeySlot>>()) > remaining { return Err("canonical-value.capacity-limit"); }
    Ok(table)
}
...
impl<R: DslValueSource> ArtifactCanonicalValueAdmission<R> {
    pub fn new(source: R, limits: ArtifactCanonicalValueLimits) -> Result<Self, (R, &'static str)> {
        ...
        Ok(Self { source: ManuallyDrop::new(Some(source)), limits, checkpoint: ArtifactCanonicalValueCheckpoint::default(), phase: Phase::Inspect, path: Vec::with_capacity(limits.maximum_depth), frames: Vec::with_capacity(limits.maximum_depth), table: Vec::new(), table_length: 0, key_index: 0, key_offset: 0, probe_slot: 0, compare_offset: 0, key_hash: 0, hasher: DefaultHasher::new() })
    }

    pub fn checkpoint(&self) -> ArtifactCanonicalValueCheckpoint { self.checkpoint }

    pub fn advance(&mut self, grant: ArtifactCanonicalValueGrant) -> Result<ArtifactCanonicalValueStep, &'static str> {
        if grant.maximum_items == 0 || grant.maximum_bytes == 0 { return Ok(ArtifactCanonicalValueStep::Blocked); }
        if self.phase == Phase::Complete { return Ok(ArtifactCanonicalValueStep::Complete(self.checkpoint)); }
        if matches!(self.phase, Phase::Closing | Phase::Closed) { return Err("canonical-value.closing"); }
        if self.checkpoint.processed_items >= self.limits.maximum_work_items { self.phase = Phase::Closing; return Err("canonical-value.work-limit"); }
        if let Err(error) = self.advance_item(grant.maximum_bytes.min(256)) { self.phase = Phase::Closing; return Err(error); }
    ...
    fn enter_children(&mut self, length: usize) -> Result<(), &'static str> {
        if length == 0 { self.phase = Phase::FinishNode; return Ok(()); }
        if self.path.len() + 1 >= self.limits.maximum_depth { return Err("canonical-value.depth-limit"); }
    ...
    fn advance_item(&mut self, bytes: usize) -> Result<(), &'static str> {
            ...
            if let Some(frame) = self.frames.last_mut() {
                ...
                if frame.next < frame.length {
                    *self.path.last_mut().ok_or("canonical-value.invalid-path")? = frame.next;
                    self.phase = Phase::Inspect;
                } else { self.frames.pop(); self.path.pop(); }
            } else { self.phase = Phase::Complete; }
            return Ok(());
        }
        ...
        match self.phase {
            ...
            Phase::InitializeTable => {
                ...
                if self.table.len() == self.table_length { self.phase = Phase::HashKey; }
            }
            Phase::HashKey => {
                let DslValue::Object(values) = value else { return Err("canonical-value.invalid-path"); };
            ...
            Phase::ProbeKey => {
                if let Some(slot) = self.table[self.probe_slot] {
                    if slot.hash == self.key_hash { self.compare_offset = 0; self.phase = Phase::CompareKey; }
                    else { self.probe_slot = (self.probe_slot + 1) % self.table.len(); }
                } else {
                    let DslValue::Object(values) = value else { return Err("canonical-value.invalid-path"); };
            ...
            Phase::CompareKey => {
                ...
                if !equal { self.probe_slot = (self.probe_slot + 1) % self.table.len(); self.phase = Phase::ProbeKey; }
                else if end == left.len() { return Err("canonical-value.duplicate-key"); }
            }
            Phase::ClearTable => {
    ...
    pub fn close_step(&mut self, grant: ArtifactCanonicalValueGrant) -> ArtifactCanonicalValueCloseStep<R> {
        if self.terminal_is_empty() { return ArtifactCanonicalValueCloseStep::Complete(self.checkpoint); }
        if grant.maximum_items == 0 { return ArtifactCanonicalValueCloseStep::Blocked(self.checkpoint); }
        self.cancel();
        let progress = |checkpoint| ArtifactCanonicalValueCloseStep::Progress { structural_items: 1, checkpoint };
        if !self.table.is_empty() { self.table.pop(); return progress(self.checkpoint); }
        if self.table.capacity() > 0 { self.table = Vec::new(); self.checkpoint.retained_bytes = 0; return progress(self.checkpoint); }
        if self.frames.pop().is_some() { self.path.pop(); return progress(self.checkpoint); }
    ...
    pub fn terminal_is_empty(&self) -> bool { self.source.is_none() && self.frames.is_empty() && self.path.is_empty() && self.table.is_empty() && self.table.capacity() == 0 && self.checkpoint.retained_bytes == 0 }
}

impl<R: DslValueSource> Drop for ArtifactCanonicalValueAdmission<R> {
    fn drop(&mut self) {
        if !std::thread::panicking() { assert!(self.terminal_is_empty(), "canonical value admission dropped before returning its source"); }
    }
}
