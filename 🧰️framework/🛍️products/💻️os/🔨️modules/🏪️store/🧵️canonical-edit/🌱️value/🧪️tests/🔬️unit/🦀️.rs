...
use crate::DslValue;
use super::{ArtifactCanonicalValueCloseStep, ArtifactCanonicalValueAdmission, ArtifactCanonicalValueGrant, ArtifactCanonicalValueLimits, ArtifactCanonicalValueStep};
use std::sync::Arc;
...
fn grant(bytes: usize) -> ArtifactCanonicalValueGrant { ArtifactCanonicalValueGrant { maximum_items: 1, maximum_bytes: bytes } }
    ...
    for bytes in [1, 7, 256] {
        for stop in vectors["cancelSteps"].as_array().unwrap().iter().map(|value| value.as_u64().unwrap() as usize).chain([100_000]) {
            ...
            for _ in 0..stop {
                ...
                if matches!(step, ArtifactCanonicalValueStep::Complete(_)) { break; }
            }
            if stop == 100_000 {
...
fn shared_value_canonical_json_admission_returns_owners_after_limits_and_late_cancel() {
    ...
    for row in vectors["limitsCases"].as_array().unwrap() {
        ...
        let value = match row["kind"].as_str().unwrap() {
            "capacity" => { bounds.maximum_retained_bytes = row["maximumRetainedBytes"].as_u64().unwrap() as usize; entries(&vectors["validEntries"]) }
            "work" => { bounds.maximum_work_items = row["maximumWorkItems"].as_u64().unwrap() as usize; DslValue::Array(vec![DslValue::Null]) }
            "depth" => { bounds.maximum_depth = row["maximumDepth"].as_u64().unwrap() as usize; DslValue::Array(vec![DslValue::Null]) }
        ...
        let mut failure = None;
        for _ in 0..100_000 { if let Err(error) = admission.advance(grant(7)) { failure = Some(error); break; } }
    ...
    let identity = Arc::as_ptr(&source);
    let mut admission = ArtifactCanonicalValueAdmission::new(source, limits()).unwrap_or_else(|(_, error)| panic!("{error}"));
    for _ in 0..100_000 { if matches!(admission.advance(grant(7)).unwrap(), ArtifactCanonicalValueStep::Complete(_)) { break; } }
    admission.cancel();
    assert!(admission.take_value().is_none());
    assert_eq!(Arc::as_ptr(&close_and_return(&mut admission)), identity);
    assert!(admission.terminal_is_empty());
...
fn shared_value_canonical_json_matches_neutral_vectors_and_serde_json_at_each_chunk_size() {
    ...
    for source in values {
        ...
        for chunk_size in [1, 7, ARTIFACT_CANONICAL_JSON_CHUNK_BYTES] {
            ...
            for _ in 0..100_000 { if matches!(admission.advance(grant(chunk_size)).unwrap(), ArtifactCanonicalValueStep::Complete(_)) { break; } }
            let accepted = admission.take_value().unwrap();
            assert!(admission.terminal_is_empty());
