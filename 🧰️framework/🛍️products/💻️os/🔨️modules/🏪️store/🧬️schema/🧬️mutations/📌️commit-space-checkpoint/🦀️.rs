...
use super::super::{SpaceHistoryMutation, RemoveSpaceCheckpoint};
use super::super::{SpaceCheckpoint, SpaceHistoryDiff, SpaceHistorySnapshot};
use serde::{Deserialize, Serialize};

//#region 🔖️Payload
// 🔀️ No ToValue/FromValue here: SpaceCheckpoint embeds foreign types that cannot derive them
...
pub struct CommitSpaceCheckpoint { pub checkpoint: SpaceCheckpoint }
//#endregion 🔖️Payload
...
impl crate::os_spr::MutationKind<SpaceHistorySnapshot, SpaceHistoryMutation> for CommitSpaceCheckpoint {
    ...
    fn diff(&self, _base: &SpaceHistorySnapshot) -> crate::os_spr::MutationOutcome<SpaceHistoryDiff> { crate::os_spr::MutationOutcome::new(SpaceHistoryDiff { add_checkpoint: Some(self.checkpoint.clone()), ..Default::default() }) }
    fn inverse(&self, _base: &SpaceHistorySnapshot) -> Vec<SpaceHistoryMutation> { vec![SpaceHistoryMutation::RemoveSpaceCheckpoint(RemoveSpaceCheckpoint { checkpoint_id: self.checkpoint.id.clone() })] }
    fn label(&self) -> String { format!("Commit space checkpoint {}", self.checkpoint.id) }
    fn target(&self) -> Vec<String> { vec!["checkpoints".into(), self.checkpoint.id.clone()] }
}
