...
const testSourceUrl = new URL("../../🧩️composition/🗄️durable-group/📜️script.ts", import.meta.url);
...
export function testDurableOwnedGroupDecisionFixture(): void {
  ...
  assert.deepEqual(fixture.publication.host, {
    slots: 1,
    operationVisibility: "private",
    requestOwnsOperation: false,
    replaceWhileOccupied: "reject",
    ackSource: "retained-receipt",
    terminalHandoff: "once",
  });
  for (const row of fixture.committedRecovery.cases) {
    const bases = row.frontiers.filter((frontier: string) => frontier === "base").length;
    const posts = row.frontiers.length - bases;
    const expected = row.receipt === "foreign" || (bases !== 0 && posts !== 0) ? "reject" : bases === 3 ? "apply" : "already-applied";
    assert.equal(row.expected, expected, row.id);
  }
  assert.deepEqual(
    [fixture.committedRecovery.witness.recordKind, fixture.committedRecovery.witness.transaction, fixture.committedRecovery.witness.bodyRecords, fixture.committedRecovery.journalBegins, fixture.committedRecovery.cancellation],
    ["event", "committed", 1, 0, "forbidden-after-commit"],
  );
