...
pub fn default_value_for_block(block: &PlaybookBlock) -> DslValue {
    match block.kind.as_str() {
        ...
        "number" | "slider" => block.default.clone().or_else(|| block.min.map(DslValue::float)).unwrap_or(DslValue::float(0.0)),
        ...
        "vector" => {
            let values: Vec<DslValue> = block.fields.as_ref().map(|fields| fields.iter().map(|field| DslValue::float(field.value.unwrap_or(0.0))).collect()).unwrap_or_default();
...
pub mod generation_forms {
    ...
    #[cfg(test)]
    include!("🧪️tests/🔬️generation-forms/🦀️.rs");
}
...
pub mod builder_kit {
    ...
    #[cfg(test)]
    include!("🧪️tests/🔬️builder-kit/🦀️.rs");
}
