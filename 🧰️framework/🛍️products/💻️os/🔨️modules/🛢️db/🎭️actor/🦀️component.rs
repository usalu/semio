...
enum Admission<M> {
    ...
    Full(M),
    Closed(M),
    /// 🪦️ `(payload, current_generation, presented_generation)` — the sender's `Address` was
...
pub enum TrySendError<M> {
    /// 🧱️ The lane is at `MailboxCapacities`' bound and is not `Priority::sheddable()`.
    Full(M),
    /// 🚪️ The mailbox has been closed (see `Address::close`/`Receiver::close`).
    Closed(M),
    /// 🪦️ See `Admission::Stale` — `(payload, expected_current, actual_presented)`.
    Stale(M, GenerationId, GenerationId),
}
...
struct MailboxState<M> {
    ...
    deficits: [i64; 6],
    /// 👉️ Which of the four DRR lanes (indices into the local `DRR_LANES` table) is currently
...
/// 🚦️ The four lanes that share deficit-round-robin scheduling once `System`/`Recovery` are
...
// 🩹️ `Unpin` is required (not just `Send + 'static`) because `poll` below uses `get_mut` for
...
// 🩹️ Same `Unpin` justification as `SendFuture`'s impl — `AskFuture` embeds a `SendFuture<M>`
...
pub enum RestartStrategy {
    ...
    OneForOne,
    /// 👯️ Every child under the same supervisor restarts together.
    OneForAll,
    ...
    pub fn reap(&self) -> Option<SupervisionDecision> {
        ...
        match decision {
            ...
            SupervisionDecision::Escalate => {
                // 🚧️ Extension seam: this crate has no parent-supervisor concept of its own —
