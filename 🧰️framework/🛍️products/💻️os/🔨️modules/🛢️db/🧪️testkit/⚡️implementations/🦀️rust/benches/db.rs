...
//! `nx run @db/testkit:bench` (`bun ./📜️script.ts bench`).
...
fn single_envelope(document: &protocol::DocumentId, index: usize) -> protocol::OperationEnvelope {
    ...
    protocol::OperationEnvelope {
        ...
        inverse: protocol::InverseOperation { schema: protocol::SchemaId(db_document::DB_PATHMAP_SCHEMA.to_string()), payload: serde_json::to_vec(&serde_json::Value::Object(serde_json::Map::new())).unwrap_or_default() },
...
//#region 🔖️CommandSubmit
/// @emoji 🎯️ Target invariant: a single small command's submit cost must not scale with how many
...
//#endregion 🔖️CommandSubmit

//#region 🔖️Recovery
/// @emoji 🎯️ Target invariants: `open` cost tracks catalog + snapshot root + WAL tail (not the
...
//#region 🔖️PreviewCommit
/// @emoji 🎯️ Target invariant: publishing previews must never delay a real commit — the group's
