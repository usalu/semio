...
//!
//! 🎯️ Design choice (compatibility surface): `db_document` (a concurrent sibling session) commits
...
//! `db_document::DocumentAuthority` genuinely work end to end (see `//#region 🧪️Tests`), composing
...
//#region 🔖️Query
/// @emoji 🔎️ What `DocumentHandle::query` can ask for — this crate's own choice (the contract fixes
...
//#region 🔖️History
/// @emoji 📜️ One committed batch's identity plus the frontier it produced — `DocumentHandle::history`'s
...
/// @emoji 📡️ What `DocumentHandle::subscribe` would filter on — defined for API-shape completeness
...
//#region 🔖️Snapshot
/// @emoji 📸️ What kind of snapshot `DocumentHandle::snapshot_now` was asked to build — defined for
...
//#region 🔖️Security
/// @emoji 🛂️ A real `db_document::AuthzHook` built on `db_security::SecurityGate`: resolves the
...
impl Database {
    ...
    /// @emoji 🚪️ The frozen `shutdown`: gracefully joins every open `DocumentAuthority` this
    /// `Database` still exclusively owns.
    ///
    /// 🧩️ Extension seam: `deadline` is currently advisory — `db_document::DocumentAuthority::shutdown`
...
//#region 🔖️DocumentHandle
pub type SubmitFuture = db_actor::ReplyReceiver<Result<CommandReceipt, DbError>>;

/// @emoji 🎭️ The frozen `DocumentHandle`: a clone-cheap live handle to one open document.
#[derive(Clone)]
