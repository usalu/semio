...
fn rename_node_cascades_edge_endpoints() {
    ...
    document.edges = vec![DagHostSnapshotEdge { id: "e1".into(), source: "a@out".into(), target: "b@in".into(), ..Default::default() }];
...
fn delete_node_severs_and_reconnects_edges() {
    let mut document = empty_dag_document();
    document.nodes = vec![sample_node("a"), sample_node("b")];
    document.edges = vec![DagHostSnapshotEdge { id: "e1".into(), source: "a@out".into(), target: "b@in".into(), route_style: EdgeRouteStyle::SharpSz, properties: PropertyBag::from([("weight".to_string(), PropertyValue::Number(2.0))]) }];
...
fn kitchen_sink_snapshot() -> DagSnapshot {
    ...
    let edges = vec![
        DagHostSnapshotEdge { id: "e1".into(), source: "slider@out".into(), target: "comp@in".into(), ..Default::default() },
        DagHostSnapshotEdge { id: "e2".into(), source: "comp@out".into(), target: "screen@in".into(), route_style: EdgeRouteStyle::SharpSz, properties: PropertyBag::from([("weight".to_string(), PropertyValue::Number(2.0))]) },
    ];
