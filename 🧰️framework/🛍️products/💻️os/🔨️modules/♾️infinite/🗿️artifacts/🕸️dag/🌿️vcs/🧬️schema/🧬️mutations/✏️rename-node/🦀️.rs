...
impl protocol::MutationKind<DagSnapshot, DagMutation> for RenameNode {
    ...
    fn diff(&self, base: &DagSnapshot) -> protocol::MutationOutcome<DagDiff> {
        ...
        let rewrites: Vec<RewrittenEdgeEndpoint> = base
            .edges
            .iter()
            .filter_map(|edge| {
                let source = split_dag_endpoint(&edge.source).0;
                let target = split_dag_endpoint(&edge.target).0;
                (source == self.id || target == self.id).then(|| RewrittenEdgeEndpoint {
                    id: edge.id.clone(),
                    new_source: (source == self.id).then(|| format!("{}{}", self.new_id, &edge.source[self.id.len()..])),
                    new_target: (target == self.id).then(|| format!("{}{}", self.new_id, &edge.target[self.id.len()..])),
                })
            })
            .collect();
    ...
    fn label(&self) -> protocol::LocalizedLabel {
        protocol::LocalizedLabel::native(&format!("Rename node {}", self.id), &format!("Knoten {} umbenennen", self.id))
    }
