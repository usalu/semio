            ...
            "wire" => {
                ...
                if let Some(edges) = case["expectedEdges"].as_array() {
                    for edge in edges {
                        ...
                        assert!(host.host_snapshot.edges.iter().any(|candidate| candidate.source == source && candidate.target == target), "{name}: {source} -> {target} must be a live wire, got {:?}", host.host_snapshot.edges.iter().map(|edge| (edge.source.as_str(), edge.target.as_str())).collect::<Vec<_>>());
                    }
                    assert_eq!(host.host_snapshot.edges.len(), edges.len(), "{name}: an input port holds exactly one incoming wire");
                }
...
fn a_minimap_click_moves_the_camera() {
    ...
    for case in minimap["cases"].as_array().expect("minimap cases") {
        ...
        let before = (host.host_snapshot.camera.x, host.host_snapshot.camera.y);
        host.pointer_down_screen(x, y, 0, false, false, false, false);
        let after = (host.host_snapshot.camera.x, host.host_snapshot.camera.y);
...
fn case_endpoint_centre(host: &DagHost, case: &Value) -> (f64, f64) {
    ...
    let centres: Vec<(f64, f64)> = endpoints
        ...
        .filter_map(|node_id| host.host_snapshot.nodes.iter().find(|node| node.id == node_id))
    ...
    for band in grab["zooms"].as_array().expect("grab zooms") {
        ...
        for case in grab["cases"].as_array().expect("grab cases") {
            ...
            for setup in case["setup"].as_array().unwrap_or(&Vec::new()) {
                ...
                host.pointer_move_screen(to_x, to_y, false, false, false);
                host.pointer_up_screen(to_x, to_y, false, false, false);
                assert!(!host.host_snapshot.edges.is_empty(), "{name}: the setup wire must exist before the case runs");
                let _ = host.take_graph_edits();
            }
            let gesture = &case["gesture"];
            match gesture["kind"].as_str().expect("gesture kind") {
                ...
                "grabSides" => {
                    ...
                    let node = host.host_snapshot.nodes.iter().find(|node| node.id == node_id).expect("fixture node").clone();
...
fn assert_case_edges(host: &DagHost, case: &Value, name: &str) {
    ...
    assert_eq!(host.host_snapshot.edges.len(), edges.len(), "{name}: the live wire count must be exactly what the case declares");
}
