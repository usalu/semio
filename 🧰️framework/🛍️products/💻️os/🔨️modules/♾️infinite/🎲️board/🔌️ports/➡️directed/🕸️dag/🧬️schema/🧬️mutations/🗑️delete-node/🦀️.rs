...
impl protocol::MutationKind<DagSnapshot, DagMutation> for DeleteNode {
    ...
    fn inverse(&self, base: &DagSnapshot) -> Vec<DagMutation> {
        ...
        let mut inverse: Vec<DagMutation> = base
            .edges
            .iter()
            .enumerate()
            .rev()
            .filter(|(_, edge)| split_dag_endpoint(&edge.source).0 == self.id || split_dag_endpoint(&edge.target).0 == self.id)
            .map(|(index, edge)| {
                DagMutation::ConnectNodes(ConnectNodes { id: edge.id.clone(), source: edge.source.clone(), target: edge.target.clone(), route_style: edge.route_style, properties: edge.properties.clone(), index: dag_index_to_wire(index) })
            })
            .collect();
