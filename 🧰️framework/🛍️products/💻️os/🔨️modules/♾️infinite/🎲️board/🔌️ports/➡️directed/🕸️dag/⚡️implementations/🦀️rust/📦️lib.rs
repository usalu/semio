...
impl Patchable<DagNodePatch> for DagNodeSpec {
    ...
    /// 🧮️ `self`-relative-to-`other` diff (protocol::Patchable convention) — used by
...
//#region 🔖️Dsl
// 🧬️ `.dag` document DSL via the `dsl::` derive engine (see `🔖️DslMirror` below) — every persisted
...
/// 📦️ Binary counterpart of the `DocumentDsl` impl above — `DagDocument` can't `#[derive(dsl::
