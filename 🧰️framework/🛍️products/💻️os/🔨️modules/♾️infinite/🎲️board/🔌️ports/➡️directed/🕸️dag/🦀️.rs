...
/// 🔢 How large a slider's value readout is against the board's own label size — the ratio the GPU
/// readout carried before it moved onto the HTML overlay beside the knob it belongs to.
const DAG_SLIDER_VALUE_LABEL_RATIO: f64 = 0.9;
...
pub struct DagHost {
    pub host_snapshot: DagHostSnapshot,
...
impl DagHost {
    ...
    fn node_id_for_widget_id(&self, widget_id: &str) -> Option<NodeId> {
        let idx = self.host_snapshot.nodes.iter().position(|n| n.id == widget_id)?;
    ...
    fn widget_id_for_node_id_ref(&self, node_id: NodeId) -> Option<&str> {
        ...
        self.host_snapshot.nodes.get(idx).map(|node| node.id.as_str())
    }
    ...
    pub fn bounded_interaction_projection(&self, revision: u64) -> Result<DagInteractionProjection, DagInteractionPlanFault> {
        if self.host_snapshot.nodes.len() > DAG_INTERACTION_NODE_CAPACITY {
    ...
    pub fn derive_pointer_plan(&self, projection: DagInteractionProjection, intent: DagPointerIntent) -> Result<DagPointerPlan, DagInteractionPlanFault> {
        if projection.revision == u64::MAX || self.host_snapshot.nodes.len() > DAG_INTERACTION_NODE_CAPACITY {
            return Err(DagInteractionPlanFault::NodeCredits);
        }
    ...
    fn derive_projection_down(&self, next: &mut DagInteractionProjection, intent: DagPointerIntent) -> Result<(), DagInteractionPlanFault> {
        ...
        let mut len = 0usize;
        for node_index in 0..self.host_snapshot.nodes.len() {
            if !dag_bit_contains(&next.selected, node_index) {
                continue;
            }
            let node = &self.host_snapshot.nodes[node_index];
        ...
        match next.gesture {
            ...
            DagProjectionGesture::Select { start_x, start_y, initial } => {
                ...
                for index in 0..self.host_snapshot.nodes.len() {
                    let node = &self.host_snapshot.nodes[index];
    ...
    fn world_to_screen_point(&self, x: f64, y: f64) -> (f64, f64) {
        ...
        let camera = Camera { x: self.host_snapshot.camera.x, y: self.host_snapshot.camera.y, zoom: self.host_snapshot.camera.zoom };
    ...
    pub fn apply_pointer_plan(&mut self, plan: &DagPointerPlan) {
        ...
        for index in 0..usize::from(plan.move_len) {
            ...
            if let Some(node) = self.host_snapshot.nodes.get_mut(index) {
                node.x = delta.x;
                node.y = delta.y;
            }
    ...
    pub fn projection_selected_id_refs<'a>(&'a self, projection: &'a DagInteractionProjection) -> impl Iterator<Item = &'a str> + 'a {
        self.host_snapshot.nodes.iter().enumerate().filter(move |(index, _)| dag_bit_contains(&projection.selected, *index)).map(|(_, node)| node.id.as_str())
    }

    pub fn projection_hovered_id_ref<'a>(&'a self, projection: &DagInteractionProjection) -> Option<&'a str> {
        projection.hover.and_then(|index| self.host_snapshot.nodes.get(usize::from(index))).map(|node| node.id.as_str())
    }
    ...
    pub fn content_world_bounds(&self) -> Option<canvas::camera::ContentBounds> {
        let first = self.host_snapshot.nodes.first()?;
        let mut union = Self::dag_node_world_bounds(first);
        for node in self.host_snapshot.nodes.iter().skip(1) {
    ...
    fn camera_state(&self) -> canvas::camera::Camera {
        canvas::camera::Camera { x: self.host_snapshot.camera.x, y: self.host_snapshot.camera.y, zoom: self.host_snapshot.camera.zoom }
    }
    ...
    pub fn set_widget_position(&mut self, widget_id: &str, x: f64, y: f64) -> Result<(), DagError> {
        ...
        self.host_snapshot.nodes[idx].y = y;
    ...
    pub fn delete_selected(&mut self) {
        ...
        self.host_snapshot.nodes.retain(|node| !widget_ids.contains(&node.id));
    ...
    pub fn port_insert_hit(&self, world_x: f64, world_y: f64, zoom: f64) -> Option<(DagPortSide, String, usize)> {
        ...
        for node in self.host_snapshot.nodes.iter().rev() {
            if node.variadic_inputs() {
                let inputs = node.inputs();
                let hw = node.width * 0.5;
    ...
    fn resolved_draw_lod(&self) -> DagDrawLod {
        ...
        dag_draw_lod(self.host_snapshot.camera.zoom)
    }
    ...
    fn rebuild_engine_with_layout(&mut self, apply_layout: bool) {
        ...
        self.edge_engine_ids.resize(self.host_snapshot.edges.len(), None);
        self.edge_route_style.clear();
        for node in &mut self.host_snapshot.nodes {
            fit_node_size(node);
        }
        ...
        for (idx, node) in self.host_snapshot.nodes.iter().enumerate() {
        ...
        let existing: Vec<(String, String)> = self
            ...
            .collect();
        let mut eid: u64 = 100;
        for (edge_index, edge) in self.host_snapshot.edges.iter().enumerate() {
    ...
    fn sync_edges_from_engine(&mut self) {
        ...
        for (eid, edge) in &self.engine.edges {
            ...
            edges.push(DagHostSnapshotEdge { id, source, target, ..Default::default() });
            edge_engine_ids.push(Some(*eid));
        }
        self.host_snapshot.edges = edges;
    ...
    pub fn set_camera(&mut self, x: f64, y: f64, zoom: f64) {
        self.host_snapshot.camera = DagCamera { x, y, zoom };
    ...
    pub fn screen_hit(&self, sx: f64, sy: f64) -> DagScreenHit {
        ...
        DagScreenHit {
            ...
            port_insert: self.port_insert_hit(world.x, world.y, self.host_snapshot.camera.zoom).is_some(),
    ...
    pub fn begin_note_edit(&mut self, node_id: &str, world_x: f64, _world_y: f64) -> bool {
        let Some(node) = self.host_snapshot.nodes.iter().find(|n| n.id == node_id) else {
            return false;
        };
        let DagNodeKind::Note { text, .. } = &node.kind else {
            return false;
        };
        let lod_index = dag_lod_index(self.host_snapshot.camera.zoom);
        let font_px = dag_label_paint_px(self.host_snapshot.camera.zoom, lod_index) * 1.05;
    ...
    pub fn note_insert_text(&mut self, chunk: &str) -> bool {
        ...
        let Some(idx) = self.host_snapshot.nodes.iter().position(|n| n.id == node_id) else {
            return false;
        };
        let DagNodeKind::Note { text, .. } = &mut self.host_snapshot.nodes[idx].kind else {
            return false;
        };
    ...
    pub fn note_move_caret(&mut self, direction: &str, extend: bool) -> bool {
        ...
        let DagNodeKind::Note { text, .. } = &self.host_snapshot.nodes[idx].kind else {
            return false;
        };
        let Some(edit) = self.editing_note.as_mut() else {
            return false;
    ...
    fn try_widget_pointer_down(&mut self, world_x: f64, world_y: f64) -> bool {
        ...
        match kind {
            ...
            WidgetPointerKind::ExportClick => {
                self.pending_export_click = Some(self.host_snapshot.nodes[idx].id.clone());
            }
        }
        true
    }
    ...
    pub fn pointer_down_screen(&mut self, sx: f64, sy: f64, button: u8, shift: bool, ctrl_or_meta: bool, alt: bool, pan: bool) {
                    ...
                    if let Some(widget_id) = self.widget_id_for_node_id(node_id) {
                        if let Some(node) = self.host_snapshot.nodes.iter().find(|entry| entry.id == widget_id) {
                            if let DagNodeKind::AppInstance { instance_id, .. } = &node.kind {
                                ...
                                let zoom = self.host_snapshot.camera.zoom.max(1e-9);
        ...
        if let Some(hit) = self.port_insert_hit(world.x, world.y, self.host_snapshot.camera.zoom) {
        ...
        if button == 0 && !merge_from_modifiers && self.lod_uses_bounded_drag() {
            let pad = DAG_BOUNDED_DRAG_HIT_PAD_PX / self.host_snapshot.camera.zoom.max(1e-9);
    ...
    /// 🎚️ Slider track anchors for the HTML slider overlay — the ONE row that carries a knob's value.
    ///
    /// 🔢 `value` is published exactly once and the overlay renders it twice: as the control's own
    /// value (hence `aria-valuenow`) and as the readout beside the track. The readout used to be
    /// painted on the GPU instead, which made it a second producer that could only move when a frame
    /// was drawn — a released knob then read `10` to a screen reader and `0.0` on the canvas. The
    /// readout's typography rides along as `fontScreenPx`/`gapScreenPx` so the DOM lands where the
    /// paint did without re-deriving the board's own metrics
...
mod wasm_session {
    ...
    impl DagSession {
        ...
        #[wasm_bindgen(js_name = loadHostSnapshotJson)]
