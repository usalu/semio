...
impl MutationDag {
    ...
    pub fn ready(&self) -> Vec<crate::os_spr::ids::MutationId> {
    ...
    /// other route.
    pub fn seed_applied(&mut self, mutation_id: crate::os_spr::ids::MutationId) {
...
pub trait MutationTransform<P>: crate::os_spr::command::Mutation<P> {
...
// only works because `Op: crate::os_spr::command::Mutation<P>` supplies each op's own
...
pub fn mutation_ids_for_edit<P, Op: crate::os_spr::command::Mutation<P>>(edit: &crate::os_spr::command::Edit<Op>) -> Vec<crate::os_spr::ids::MutationId> {
    edit.forwards.iter().enumerate().map(|(index, op)| edit.mutation_meta.get(index).and_then(|m| m.mutation_id.clone()).or_else(|| op.mutation_id()).unwrap_or_else(|| crate::os_spr::ids::MutationId(format!("{}#{index}", edit.id)))).collect()
}
...
pub fn mutation_envelope_from_edit<P, Op: crate::os_spr::command::Mutation<P> + crate::os_spr::command::OpBinary>(
    ...
    edit.forwards
        ...
        .map(|(index, op)| {
            ...
            let inverse_payload = edit.inverse.get(index).map(crate::os_spr::command::OpBinary::encode_op).transpose()?.unwrap_or_default();
            Ok(MutationEnvelope {
                mutation_id,
...
pub fn encode_envelope(envelope: &MutationEnvelope, out: &mut Vec<u8>) {
    crate::os_spr::write_str(out, &envelope.mutation_id.0);
...
pub fn decode_envelope(bytes: &[u8], pos: &mut usize) -> Result<MutationEnvelope, crate::os_spr::ProtocolError> {
    ...
    for _ in 0..dependency_count {
        dependencies.push(crate::os_spr::ids::MutationId(crate::os_spr::read_str(bytes, pos)?));
    }
...
pub fn decode_envelopes(bytes: &[u8]) -> Result<Vec<MutationEnvelope>, crate::os_spr::ProtocolError> {
    let mut pos = 0usize;
...
mod tests {
    ...
    fn seed_applied_unblocks_pending_envelopes_that_reference_out_of_band_deps() {
        ...
        dag.seed_applied(crate::os_spr::ids::MutationId("operation-1".to_string()));
    ...
    fn mutation_envelope_from_edit_derives_one_envelope_per_forward_op_using_explicit_meta() {
        let edit = crate::os_spr::command::Edit::<CausalAddOp> {
            ...
            mutation_meta: vec![
                crate::os_spr::command::MutationMeta {
                    mutation_id: Some(crate::os_spr::ids::MutationId("op-a".into())),
                ...
                crate::os_spr::command::MutationMeta {
                    ...
                    dependencies: vec![crate::os_spr::ids::MutationId("op-a".into())],
                    base_version: 1,
                    author_id: None,
        ...
        assert_eq!(envelopes[0].mutation_id, crate::os_spr::ids::MutationId("op-a".into()));
        assert_eq!(envelopes[0].actor, crate::os_spr::ids::ActorId("actor-explicit".into()));
        assert_eq!(envelopes[0].dependencies, vec![crate::os_spr::ids::MutationId("op-0".into())]);
        ...
        assert_eq!(envelopes[1].mutation_id, crate::os_spr::ids::MutationId("op-b".into()));
    ...
    fn mutation_envelope_from_edit_falls_back_to_op_trait_and_structural_defaults_without_meta() {
        ...
        assert_eq!(envelopes[0].mutation_id, crate::os_spr::ids::MutationId("edit-2#0".into()));
