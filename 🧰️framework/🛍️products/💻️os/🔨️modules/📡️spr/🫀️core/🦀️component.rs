...
pub enum ProtocolError {
    #[error(transparent)]
    Pack(#[from] crate::os_pack::core::PackError),
...
/// against BEFORE allocating — mirrors `crate::os_pack::core::PackLimits`'s stated invariant.
#[derive(Clone, Debug)]
...
//#region 🔖️RecordKinds
// Plain `pub const` u8s (mirrors crate::os_pack::core::SegmentKind convention but this family uses bare
...
/// payload codecs. `out`/`input` follow `crate::os_pack::core::ByteWriter`/`ByteReader` conventions exactly
/// (they take `&mut crate::os_pack::core::ByteWriter` / `&mut crate::os_pack::core::ByteReader<'_>` directly — no
/// reimplementation of the varint/byte primitives).
pub mod scalar {
    use crate::os_pack::core::{ByteReader, ByteWriter, PackError};
    ...
    // Minimal-varint enforcement reuses crate::os_pack::core::is_minimal_varint at Full verification.
}
//#endregion 🔖️Scalars
...
pub fn write_varint_u64(out: &mut Vec<u8>, value: u64) {
    crate::os_pack::core::write_varint_u64(out, value);
}

pub fn read_varint_u64(bytes: &[u8], pos: &mut usize) -> Result<u64, ProtocolError> {
    Ok(crate::os_pack::core::read_varint_u64(bytes, pos)?)
}
...
mod tests {
    ...
    fn pack_error_converts_into_protocol_error_via_from() {
        let pack_err = crate::os_pack::core::PackError::Truncated(7);
    ...
    mod scalars {
        ...
        fn id_round_trips_via_edit_ordinal_tag() {
            ...
            let mut reader = ByteReader::new(&bytes);
            let decoded = read_id(&mut reader, |_| unreachable!("must not resolve"), |ordinal| if ordinal == 7 { Ok("edit-7") } else { Err(crate::os_pack::core::PackError::Truncated(0)) }).unwrap();
        ...
        fn id_round_trips_via_prefix_uuid_tag() {
            ...
            let mut reader = ByteReader::new(&bytes);
            let decoded = read_id(&mut reader, |idx| if idx == 0 { Ok("actor") } else { Err(crate::os_pack::core::PackError::Truncated(0)) }, |_| unreachable!("must not resolve ordinal")).unwrap();
        ...
        fn id_falls_back_to_dictref_tag_for_plain_strings() {
            ...
            let mut reader = ByteReader::new(&bytes);
            let decoded = read_id(&mut reader, |idx| if idx == 42 { Ok("hello-world") } else { Err(crate::os_pack::core::PackError::Truncated(0)) }, |_| unreachable!("must not resolve ordinal")).unwrap();
