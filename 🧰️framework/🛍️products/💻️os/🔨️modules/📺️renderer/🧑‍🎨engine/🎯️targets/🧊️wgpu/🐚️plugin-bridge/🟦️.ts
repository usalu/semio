 ...
 *   package, so they're imported by relative path — `🧵️shard-runtime/🟦️.ts`/`🖼️wire-turn/🟦️.ts` (both NEW,
 ...
 *
 * ⚖️ {@link submitActorWork} only keeps two TURNS from overlapping, which is all the shard worker
 * demands. It is not enough once a standing typed-operation drain shares the actor with host calls: a
 * drain poll slipping between two of a render's turns collects that render's own effects, so the
...
async function settleInstanceLifecycle(lifecycle: ShardInstanceLifecycleLease, route: WgpuOwnedUiInstanceRoute, initial: WireTurnResult, execute: WgpuActorExecutor, instanceId: number): Promise<void> {
  let current = initial;
 ...
 *
 * ⚖️ `pendingTurnEffects` is the ONE place a turn's leftover effects wait for the host, and
 * `performInvocation` is the only drain. Turns that ran outside a command — the instance-open
 * lifecycle and the acknowledgement turns its patch intake produced — used to be settled for their UI
 * patches alone, so a `flowEvalTick` the guest armed while the instance opened was dropped by the host
 * while the guest's own latch stayed armed, and nothing could ever re-arm it
 * (ticket 26/09/09/PROCEDURAL-3D-END-TO-END, `📓️wgpu-blank-paint-2026-09-12.md` §6). Appends rather
 * than replaces: one instance can accumulate leftovers across several settles before the next drain. */
function stashLeftoverEffects(instanceId: number, turns: readonly WireTurnResult[]): number {
  return stashLeftoverHostEffects(instanceId, turns.flatMap((turn) => turn.effects));
}

/** 🧩️ {@link stashLeftoverEffects} over an already-scanned effect list — the form the typed-operation
 * drive uses, since its result pages are transport that was consumed before the host ever sees them. */
function stashLeftoverHostEffects(instanceId: number, effects: readonly WireVariant[]): number {
...
//#region 📬️TypedOperationDrive
/** 📏️ Continuation turns ONE host call may spend settling the typed operations it started before
 * it hands the rest to {@link drainTypedOperationTurns}. A `Migrated` interactive job returns its
 * "started" reply immediately, so this only has to carry the operation far enough to publish its
 * first pages and their acknowledgements. */
export const WGPU_TYPED_OPERATION_SETTLE_LIMIT = 64;

/** 📏️ Continuation polls the standing drain submits for one instance before it gives up — the wgpu
 * twin of `PLUGIN_OPERATION_DRAIN_BUDGET`. */
export const WGPU_TYPED_OPERATION_DRAIN_BUDGET = 4_096;

/** 📏️ Retained authority for the host effects a drain accumulates between drains. An operation
 * publishes at most one host effect per continuation turn, so a completion that never arrives would
 * otherwise grow without bound; exceeding it fails closed. */
export const WGPU_TYPED_OPERATION_EFFECT_CAPACITY = 4_096;

/** 📬️ The wgpu host's half of the typed-operation result exchange.
 *
 * ⚖️ `route_exchange_output` (`⚛️reactor/🔄️turn/🦀️.rs`) publishes a mounted operation's result pages as
 * `Effect::SendMessage{Shell{instance}}` beside the ordinary `AppFrame` replies, and the guest PARKS
 * the operation until the host answers each page with its ACK token. A host that does not answer
 * freezes the operation mid-flight — which is why `flowEvalTick` could reach the guest's command door
 * on this target and never produce a `flowEvalResolve`. This type is the whole obligation in one
 * place: collect the pages out of a turn's effects, hand back the acknowledgements the next
 * submission owes, surface a fault page by name, and leave the remaining effects untouched for the
 * host's own routing. The page codec itself is `🖼️wire-turn/🟦️.ts`'s, shared with React. */
export class WgpuTypedOperationDrive {
  #acknowledgements: TypedOperationAckEvent[] = [];
  #pages = 0;
  #terminal = false;
  readonly #hostEffects = new WeakMap<WireTurnResult, readonly WireVariant[]>();

  constructor(private readonly instanceId: number) {}

  /** 📬️ Scans every accepted turn once, keeping its non-page effects and queueing its ACKs. */
  observe(turns: readonly WireTurnResult[]): void {
    for (const turn of turns) {
      if (this.#hostEffects.has(turn)) continue;
      const scan = scanTypedOperationPages(turn.effects);
      this.#hostEffects.set(turn, scan.kept);
  ...
  /** 📬️ The acknowledgements the NEXT submitted turn owes, handed over exactly once. */
  takeAcknowledgements(): readonly TypedOperationAckEvent[] {
    const owed = this.#acknowledgements;
    this.#acknowledgements = [];
    return owed;
  }

  /** 🧩️ Every effect of `turns` that was NOT transport — pages already consumed by {@link observe}. */
  hostEffects(turns: readonly WireTurnResult[]): WireVariant[] {
    return turns.flatMap((turn) => this.#hostEffects.get(turn) ?? turn.effects);
  }

  /** 🔄️ Whether the actor still owes this call a turn: an unanswered page, or a guest that says it
   * has work left this activation. */
  owesASettle(last: WireTurnResult | undefined): boolean {
    return this.#acknowledgements.length > 0 || wireTurnStatusTag(last?.status) === "more-work";
  }

  /** 🔄️ Whether the turns just accepted did anything at all — a settle loop that keeps submitting
   * against a silent actor is the drain's job, not a host call's. */
  progressed(turns: readonly WireTurnResult[]): boolean {
...
export async function loadPluginModule(pluginId: string, moduleUrl: string, signal?: AbortSignal): Promise<WgpuPluginHandle> {
  ...
  const renderSurface = (instanceId: number, surfaceId: string, bodyKey: string, viewState: unknown): Promise<WgpuOwnedUiProjection> =>
    serializeWgpuActorCall(requireActorId(instanceId), () => renderSurfaceSerialized(instanceId, surfaceId, bodyKey, viewState));

  const renderSurfaceSerialized = async (instanceId: number, surfaceId: string, bodyKey: string, viewState: unknown): Promise<WgpuOwnedUiProjection> => {
    ...
    try {
      ...
      for (let commandIndex = 0; commandIndex < events.length; commandIndex += 1) {
        ...
        for (const commandPage of pages) {
          const submitted = await submitTurn(actorId, drive.takeAcknowledgements(), commandPage);
      ...
      for (let settle = 0; drive.owesASettle(results.at(-1)) && settle < WGPU_TYPED_OPERATION_SETTLE_LIMIT; settle += 1) {
        ...
        await accept(await submitTurn(actorId, drive.takeAcknowledgements()));
        if (!drive.progressed(results.slice(before))) break;
        await yieldWgpuUi();
      }
  ...
  /** 🔁️ Keeps an instance's mounted typed operations advancing once no host call is left to drive
   * them — the wgpu twin of `PluginRuntime`'s own `drainTypedOperations`, on the SAME shared
   * {@link drainTypedOperationTurns} loop. A `flowEvalTick` is a `Migrated` interactive job: it
   * returns its "started" reply on the first turn and then needs hundreds more reactor turns to reach
   * its terminal page, so without this poll the actor simply stops and `flowEvalResolve` never
   * arrives. Frames a poll uncovers are published on this instance's own outcome stream, where
   * `AppChannelClient`'s pump routes them exactly as a command reply's. */
  ...
  const drainTypedOperations = async (instanceId: number): Promise<void> => {
    ...
    const drive = new WgpuTypedOperationDrive(instanceId);
    try {
      const outcome = await drainTypedOperationTurns(
        WGPU_TYPED_OPERATION_DRAIN_BUDGET,
        live,
        async () => serializeWgpuActorCall(requireActorId(instanceId), async () => {
          const actorId = requireActorId(instanceId);
          const route = requireUiRoute(instanceId);
          const execute = executeFor(actorId);
          const turn = await submitTurn(actorId, drive.takeAcknowledgements());
          const accepted = [turn, ...await route.accept(turn, execute)];
          drive.observe(accepted);
 ...
 * `🌉️ProgramBridge/🎯️targets/🧊️wgpu/🦀️.rs` parses back into `kernel::InvocationResult`.
 *
 * ⚖️ `req` is a `u64` and crosses this seam as a JS `bigint`, which `JSON.stringify` refuses outright.
 * Two of the four verbs carried a replacer and two did not, so the FIRST command whose reply
 * requested an `invokeExtension` died with `Do not know how to serialize a BigInt` — the same
 * `InvocationResponse` shape, projected two different ways (ticket
 * 26/09/09/PROCEDURAL-3D-END-TO-END). One projection, all four verbs. */
export function invocationResponseJson(result: InvocationResponse): string {
  return JSON.stringify(result, (_key, value: unknown) => (typeof value === "bigint" ? Number(value) : value));
}
//#endregion 🔖️JsBridge
