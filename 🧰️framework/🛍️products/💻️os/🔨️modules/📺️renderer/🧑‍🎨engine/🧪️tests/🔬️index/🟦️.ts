...
const ownedExports = new Ajv({ strict: true, allErrors: true }).addSchema(rendererSchema).addSchema(directorySchema);
...
describe("app-owned surface session factories", () => {
  ...
  it("joins exact plugin and app ownership while keeping instance scopes distinct", () => {
    const validate = new Ajv({ strict: true, allErrors: true }).addSchema(boardSessionSchema).compile({ $ref: `${boardSessionSchema.$id}#/$defs/Puzzle2dWasmSessionFactory` });
...
import graphSliderSchema from "../../../../♾️infinite/🗿️artifacts/🕸️dag/🌿️vcs/🧬️schema/📐️.schema.json";
...
import graphParameterSchema from "../../../../🌊️flow/🗿️artifacts/🌊️flow/🎚️parameter/🧬️schema/🔣️.schema.json";
