    ...
    for step in &row.steps {
        for index in 0..step.repeat.unwrap_or(1) {
            ...
            match step.op.as_str() {
                "input" => {
                    assert_eq!(enqueue_host_event(&mut events, &mut scheduler, token, &mut generation, hold, DispatchEvent::PointerMove { pointer, x: index as f32, y: 0.0 }), ui_host::EnqueueOutcome::Accepted, "{}: input {index} was refused", row.id);
                    tally.delivered += 1;
                }
                "metrics" => {
