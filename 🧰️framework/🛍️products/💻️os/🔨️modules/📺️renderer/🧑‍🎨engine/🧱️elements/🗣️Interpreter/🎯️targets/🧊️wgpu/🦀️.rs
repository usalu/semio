//! 🧩️ framework/products/os/modules/renderer/engine/elements/🟦️Interpreter/component.rs — wgpu
...
//! via the renderer's exact `../../../🧱️elements/🗣️Interpreter/🎯️targets/🧊️wgpu/🦀️.rs` module path in
...
struct UiDocumentCloseOwner {
    window_id: SurfaceId,
    token: ui_wgpu::wgpu::engine::UiSurfaceToken,
    generation: u64,
}

#[derive(Default)]
struct UiDocumentCloseQueue {
    owners: std::collections::VecDeque<UiDocumentCloseOwner>,
}

impl UiDocumentCloseQueue {
    fn try_upsert(&mut self, owner: UiDocumentCloseOwner) -> bool {
        if let Some(existing) = self.owners.iter_mut().find(|existing| existing.window_id == owner.window_id) {
            *existing = owner;
            return true;
        }
        if self.owners.len() == ui_wgpu::wgpu::engine::UI_LAYOUT_SURFACE_SLOTS {
            return false;
        }
    ...
    fn owns_window(&self, window_id: &str) -> bool {
        self.owners.iter().any(|owner| owner.window_id.as_ref() == window_id)
    }
}
...
pub(crate) fn request_ui_document_close(window_id: &str) -> bool {
    ...
    let admitted = UI_DOCUMENT_CLOSE_QUEUE.with(|cell| {
        ...
        queue.try_upsert(UiDocumentCloseOwner { window_id, token, generation })
    });
    admitted && UI_ENGINE.with(|cell| cell.borrow_mut().begin_surface_close(token))
}

pub(crate) fn ui_document_close_pending() -> bool {
    UI_DOCUMENT_CLOSE_QUEUE.with(|cell| !cell.borrow().owners.is_empty())
}

pub(crate) fn ui_document_close_pending_for(window_id: &str) -> bool {
    UI_DOCUMENT_CLOSE_QUEUE.with(|cell| cell.borrow().owns_window(window_id))
}
...
pub(crate) fn close_ui_document_one() -> bool {
    ...
    if SCENE_POINTER_OWNERS.with(|cell| {
        let mut owners = cell.borrow_mut();
        let Some(index) = owners.slots.iter().position(|entry| entry.target.window_id == window_id && entry.target.window_generation == owner.generation) else { return false };
        owners.slots.remove(index);
        true
    }) {
        return true;
    }
    let mounted = UI_ENGINE.with(|cell| {
        let engine = cell.borrow();
    ...
    let (complete, retirement) = UI_ENGINE.with(|cell| {
        ...
        if engine.surface_token(window_id) != Some(owner.token) || engine.surface_generation(window_id) != Some(owner.generation) {
            return (true, None);
        }
    ...
    if complete {
        UI_DOCUMENT_CLOSE_QUEUE.with(|cell| {
            let mut queue = cell.borrow_mut();
            if queue.owners.front() == Some(&owner) {
                queue.owners.pop_front();
            }
        });
    }
    true
...
impl SceneIntentQueue {
    fn push(
        ...
        if window_id.len() > SCENE_INTENT_ID_BYTES || host_id.len() > SCENE_INTENT_ID_BYTES {
    ...
    fn close_window_step(&mut self, window_id: &str, generation: u64) -> bool {
        if let Some(intent) = self.retiring.as_mut().filter(|intent| intent.window_id == window_id && intent.surface_generation == generation) {
            if intent.close_step() {
                self.retiring = None;
            }
            return true;
        }
        let Some(offset) = (0..self.len).find(|offset| self.slots[(self.head + offset) % SCENE_INTENT_CAPACITY].as_ref().is_some_and(|intent| intent.window_id == window_id && intent.surface_generation == generation)) else { return false };
        let index = (self.head + offset) % SCENE_INTENT_CAPACITY;
        let intent = self.slots[index].as_mut().expect("occupied window intent");
        intent.retiring = true;
        if intent.close_step() {
            self.slots[index] = None;
            for cursor in offset..self.len - 1 {
                let next = (self.head + cursor + 1) % SCENE_INTENT_CAPACITY;
                self.slots[(self.head + cursor) % SCENE_INTENT_CAPACITY] = self.slots[next].take();
            }
            self.len -= 1;
        }
        true
    ...
    fn cancel_pointer(&mut self, pointer_id: ui_render::PointerId, owners: &[ScenePointerOwner]) {
        for slot in self.slots.iter_mut().filter_map(Option::as_mut) {
            if slot.pointer_id == Some(pointer_id)
                && owners.iter().any(|owner| owner.target.window_id == slot.window_id && owner.target.host_id == slot.host_id && owner.target.window_generation == slot.surface_generation)
                && slot.ink_job.as_ref().is_none_or(|job| job.matches_pointer_owner(slot.surface_generation, pointer_id))
            {
                slot.retiring = true;
            }
        }
...
pub struct ScenePointerTarget {
    ...
    pub(crate) key: ui_wgpu::wgpu::NodeKey,
...
impl From<ui_wgpu::wgpu::reconcile::UiRetiredComponentScene> for ScenePointerTarget {
    fn from(retired: ui_wgpu::wgpu::reconcile::UiRetiredComponentScene) -> Self {
        Self {
            ...
            node: retired.node,
            key: retired.key,
            window_generation: retired.window_generation,
            component_generation: retired.component_generation,
        }
    }
}

#[derive(Default)]
struct ScenePointerOwners {
...
pub fn retained_scene_target_at(window_id: &str, x: f32, y: f32) -> Option<ScenePointerTarget> {
    UI_ENGINE.with(|cell| {
        let engine = cell.borrow();
        let node = engine.scene_at(window_id, x, y)?.node;
        retained_scene_target_in(&engine, window_id, node)
    })
}
...
pub(crate) fn fixture_scene_pointer_target(window_id: &str, surface_id: &str, key: &str) -> ScenePointerTarget {
    ...
    ScenePointerTarget {
        ...
        kind: ui_wgpu::wgpu::SurfaceKind::TiledMap,
        node: arena.insert(()),
        key: ui_wgpu::wgpu::NodeKey::Explicit(key.into()),
        window_generation: 1,
...
pub fn claim_scene_pointer_owner(target: ScenePointerTarget, pointer_id: ui_render::PointerId) -> bool {
    ...
    SCENE_POINTER_OWNERS.with(|cell| {
        ...
        if owners.slots.len() == SCENE_POINTER_OWNER_CAPACITY {
            return false;
        }
        owners.slots.push(ScenePointerOwner { pointer_id, target });
        true
    })
}

pub fn claim_scene_pointer_at(window_id: &str, x: f32, y: f32, pointer_id: ui_render::PointerId) -> bool {
    retained_scene_target_at(window_id, x, y).is_some_and(|target| claim_scene_pointer_owner(target, pointer_id))
}

pub fn captured_scene_pointer(pointer_id: ui_render::PointerId) -> Option<ScenePointerTarget> {
    SCENE_POINTER_OWNERS.with(|cell| cell.borrow().slots.iter().find(|owner| owner.pointer_id == pointer_id).map(|owner| owner.target.clone())).filter(scene_pointer_target_is_live)
}
...
pub fn cancel_scene_pointer(pointer_id: ui_render::PointerId, _input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> Vec<ScenePointerTarget> {
    let owners = SCENE_POINTER_OWNERS.with(|cell| {
        ...
        registry.slots.retain(|owner| {
            if owner.pointer_id == pointer_id {
                cancelled.push(owner.clone());
                false
            } else {
                true
            }
...
pub fn register_clipped_retained_hit_targets(window_id: &str, clip: ui_wgpu::wgpu::Rect, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> Vec<(String, Option<ScenePointerTarget>, Rect)> {
    UI_ENGINE.with(|cell| {
        let engine = cell.borrow();
        let mut ids = Vec::new();
        for registration in engine.window_hit_targets(window_id) {
            let mut target = registration.to_hit_target();
            if !registration.overlay {
                let x = registration.rect.x.max(clip.x);
                let y = registration.rect.y.max(clip.y);
                let right = (registration.rect.x + registration.rect.w).min(clip.x + clip.w);
                let bottom = (registration.rect.y + registration.rect.h).min(clip.y + clip.h);
                if right <= x || bottom <= y {
                    continue;
                }
                target.rect = ui_wgpu::wgpu::Rect::new(x, y, right - x, bottom - y);
            }
            let scene = registration.scene.as_ref().and_then(|_| candidate_scene_target_in(&engine, window_id, registration.node));
            ids.push((registration.control_id.clone(), scene, target.rect));
            input.register_hit(target);
        }
        ids
    })
}
...
pub fn retained_pointer_capture_window(pointer_id: ui_render::PointerId) -> Option<String> {
    UI_ENGINE.with(|cell| cell.borrow().window_with_pointer_capture(pointer_id.0).map(str::to_owned))
}

pub fn retained_any_pointer_capture() -> Option<(String, ui_render::PointerId)> {
    UI_ENGINE.with(|cell| cell.borrow().any_pointer_capture().map(|(window_id, pointer_id)| (window_id.to_owned(), ui_render::PointerId(pointer_id))))
}
...
#[cfg(test)]
/// 🌳️ Whether production reconcile retained `window_id` with a Tree root.
pub(crate) fn retained_root_is_tree(window_id: &str) -> bool {
    UI_ENGINE.with(|cell| {
        let engine = cell.borrow();
        let Some(tree) = engine.tree(window_id) else { return false };
        tree.root.and_then(|root| tree.node(root)).is_some_and(|node| matches!(&node.spec.0, ui_wgpu::wgpu::UiNode::Tree(_)))
    })
}
...
pub fn canvas_catalogue_drag_over(window_id: &str, x: f32, y: f32, drag_data: &std::collections::HashMap<String, String>, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> bool {
    ...
    let mut types = drag_data.keys().cloned().collect::<Vec<_>>();
    types.sort();
    let outcome = UI_ENGINE.with(|cell| {
        let engine = cell.borrow();
        let hit = engine.scene_at(window_id, x, y)?;
        if hit.kind != ui_wgpu::wgpu::SurfaceKind::Canvas2d {
            return None;
        }
        let generation = engine.surface_generation(window_id)?;
        let tree = engine.tree(window_id)?;
        let UiNode::ComponentScene(scene) = &tree.node(hit.node)?.spec.0 else { return None };
        Some(crate::scenes::canvas_catalogue_drag_over_into(scene, hit.rect, window_id, generation, x, y, &types, input))
    });
    match outcome {
        ...
        None => {
            crate::scenes::cancel_canvas_catalogue_drag(input);
            false
        }
    }
}
...
pub fn canvas_catalogue_drop(window_id: &str, x: f32, y: f32, drag_data: &std::collections::HashMap<String, String>, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> bool {
    ...
    let outcome = UI_ENGINE.with(|cell| {
        ...
        let tree = engine.tree(window_id)?;
        let UiNode::ComponentScene(scene) = &tree.node(hit.node)?.spec.0 else { return None };
        Some(crate::scenes::canvas_catalogue_drop_into(scene, hit.rect, window_id, generation, x, y, raw_payload, input))
    });
    match outcome {
        Some(Ok(accepted)) => accepted,
        Some(Err(fault)) => {
...
pub fn dispatch_presented_ui_pointer_event(window_id: &str, pointer_id: ui_render::PointerId, event: ui_wgpu::wgpu::UiEvent, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> Vec<ui_wgpu::wgpu::UiCommand> {
    if matches!(event, ui_wgpu::wgpu::UiEvent::PointerDown { .. }) {
...
pub fn dispatch_accessibility_event(window_id: &str, window_generation: u64, node_id: u64, node_key: &str, event: ui_wgpu::wgpu::AccessibilityUiEvent, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> Option<Vec<ui_wgpu::wgpu::UiCommand>> {
    ...
    let text_editor = UI_ENGINE.with(|cell| {
        let engine = cell.borrow();
        if engine.surface_generation(window_id) != Some(window_generation) {
            return None;
        }
        text_editor_accessibility_entries(engine.tree(window_id)?).into_iter().find(|entry| entry.key == node_key && scene_virtual_accessibility_node_id(&entry.key) == node_id)
    });
    ...
    let table_stepper = UI_ENGINE.with(|cell| {
        let engine = cell.borrow();
        if engine.surface_generation(window_id) != Some(window_generation) {
            return None;
        }
    ...
    if let Some(entry) = table_stepper {
        match event {
            ui_wgpu::wgpu::AccessibilityUiEvent::Focus | ui_wgpu::wgpu::AccessibilityUiEvent::Activate => focus_table_stepper(window_id, window_generation, entry.scene_node, &entry.host_id, entry.cell.target),
            ui_wgpu::wgpu::AccessibilityUiEvent::Blur => {
                FOCUSED_TABLE_STEPPER.with(|cell| {
                    ...
                    if focus.as_ref().is_some_and(|focus| {
                        focus.window_id == window_id
                            ...
                            && focus.row_id == entry.cell.target.row_id
                            && focus.column_id == entry.cell.target.column_id
                    }) {
                        *focus = None;
                    }
                });
    ...
    let vfs_control = UI_ENGINE.with(|cell| {
        let engine = cell.borrow();
        if engine.surface_generation(window_id) != Some(window_generation) {
            return None;
        }
        vfs_accessibility_entries(engine.tree(window_id)?).into_iter().find(|entry| entry.control.key == node_key && scene_virtual_accessibility_node_id(&entry.control.key) == node_id)
    });
    if let Some(entry) = vfs_control {
        let target = crate::scenes::VfsAccessibilityFocusTarget { row_id: entry.control.row_id, kind: entry.control.kind };
        match event {
            ui_wgpu::wgpu::AccessibilityUiEvent::Focus => focus_vfs_control(window_id, window_generation, entry.scene_node, &entry.host_id, target),
            ui_wgpu::wgpu::AccessibilityUiEvent::Activate => {
                focus_vfs_control(window_id, window_generation, entry.scene_node, &entry.host_id, target.clone());
                let outcome = UI_ENGINE.with(|cell| {
                    let engine = cell.borrow();
                    let retained = engine.tree(window_id).and_then(|tree| tree.node(entry.scene_node))?;
                    let UiNode::ComponentScene(scene) = &retained.spec.0 else { return None };
                    (scene.host_id == entry.host_id && scene.component_kind == ui_wgpu::wgpu::SurfaceKind::VirtualFileSystem).then_some(())?;
                    crate::scenes::vfs_accessibility_activate(scene, &target, input)
                });
                match outcome {
                    Some(Ok(())) => {}
                    Some(Err(fault)) => input.record_action_fault(fault),
...
pub fn retained_node_is_select(window_id: &str, node_id: NodeId) -> bool {
    UI_ENGINE.with(|cell| cell.borrow().tree(window_id).and_then(|tree| tree.node(node_id)).is_some_and(|node| matches!(node.spec.0, UiNode::Select(_))))
}

/// 🧩️ Closed component kind retained with a focused reconciliation key.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum RetainedNodeFocusKind {
    Stack,
    Text,
    Button,
    Separator,
...
impl RetainedNodeFocusKind {
    fn from_node(node: &UiNode) -> Self {
        match node {
            UiNode::Stack(_) => Self::Stack,
            UiNode::Text(_) => Self::Text,
            UiNode::Button(_) => Self::Button,
            UiNode::Separator(_) => Self::Separator,
            UiNode::Input(_) => Self::Input,
            UiNode::Select(_) => Self::Select,
            UiNode::Toggle(_) => Self::Toggle,
            UiNode::KeyValue(_) => Self::KeyValue,
            UiNode::Slider(_) => Self::Slider,
            UiNode::NumberStepper(_) => Self::NumberStepper,
            UiNode::Ring(_) => Self::Ring,
            UiNode::IconSelect(_) => Self::IconSelect,
            UiNode::Progress(_) => Self::Progress,
            UiNode::Field(_) => Self::Field,
            UiNode::Section(_) => Self::Section,
            UiNode::Group(_) => Self::Group,
            UiNode::Tree(_) => Self::Tree,
            UiNode::Image(_) => Self::Image,
            UiNode::ComponentScene(_) => Self::ComponentScene,
            UiNode::ExternalSlot(_) => Self::ExternalSlot,
        }
    }
}
...
fn apply_ui_commands(commands: &[ui_wgpu::wgpu::UiCommand], pointer_id: Option<ui_render::PointerId>, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) {
    for command in commands {
        match command {
            ...
            ui_wgpu::wgpu::UiCommand::Scene { window_id, node, kind, rect, event, .. } => {
                apply_scene_ui_command(window_id, *node, *kind, *rect, event, pointer_id, input);
            }
...
struct MountedClipboardIo {
    ...
    complete: Option<Box<dyn FnOnce(Result<Option<ui_wgpu::wgpu::ClipboardContent>, String>)>>,
}
...
#[cfg(all(not(target_arch = "wasm32"), not(test)))]
fn submit_clipboard_io(job: ui_wgpu::wgpu::ClipboardIoJob, complete: impl FnOnce(Result<Option<ui_wgpu::wgpu::ClipboardContent>, String>) + 'static) {
    ...
    let mut complete = Some(Box::new(complete) as Box<dyn FnOnce(Result<Option<ui_wgpu::wgpu::ClipboardContent>, String>)>);
...
fn apply_clipboard_paste_requested(window_id: &str, _input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) {
    ...
    submit_clipboard_io(ui_wgpu::wgpu::ClipboardIoJob::read(), move |result| {
        let Ok(Some(ui_wgpu::wgpu::ClipboardContent::Text(text))) = result else { return };
        UI_ENGINE.with(|cell| {
            let mut engine = cell.borrow_mut();
            let commands = engine.dispatch_event(&window_id, ui_wgpu::wgpu::UiEvent::Paste { text });
            engine.enqueue_async_commands(commands);
        });
    });
}
 ...
 * call produces are queued once as asynchronous commands and consumed by the next interpreter
 * dispatch. Synchronous event commands are returned directly and are never cloned into that queue. */
#[cfg(target_arch = "wasm32")]
fn apply_clipboard_paste_requested(window_id: &str, _input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) {
    let window_id = window_id.to_string();
    semio_framework_async::browser::spawn_local(async move {
...
/// `✏️TextEditor`'s right-click context menu, …) already assume. 🐛️ W4 fix: this used to have `1`/`2`
...
fn apply_scene_ui_command(window_id: &str, node: NodeId, kind: ui_wgpu::wgpu::SurfaceKind, rect: Rect, event: &ui_wgpu::wgpu::UiEvent, pointer_id: Option<ui_render::PointerId>, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) {
    ...
    let witness = UI_ENGINE.with(|cell| {
        ...
        if scene.component_kind != kind || scene.host_id.len() > SCENE_INTENT_ID_BYTES {
            return None;
        }
        Some((revision, engine.surface_generation(window_id)?, scene.host_id.clone(), engine.driver_drag()))
    });
    let Some((tree_revision, surface_generation, host_id, driver_drag)) = witness else {
        input.record_action_fault(ui_wgpu::wgpu::BoundedActionFault::Structure);
        return;
    };
    if matches!(event, SceneIntentEvent::PointerDown { .. }) {
        if let Some(pointer_id) = pointer_id {
            if !retained_scene_target(window_id, node).is_some_and(|target| claim_scene_pointer_owner(target, pointer_id)) {
                input.record_action_fault(ui_wgpu::wgpu::BoundedActionFault::ItemCredits);
                return;
            }
        }
    }
    if let Err(fault) = SCENE_INTENTS.with(|cell| cell.borrow_mut().push(tree_revision, surface_generation, pointer_id, window_id, &host_id, node, kind, rect, event, driver_drag)) {
        input.record_action_fault(fault);
    }
    if matches!(event, SceneIntentEvent::PointerUp { .. }) {
        if let Some(pointer_id) = pointer_id {
            release_scene_pointer(pointer_id);
        }
    }
}
...
fn focused_text_editor() -> Option<FocusedTextEditor> {
    FOCUSED_TEXT_EDITOR.with(|cell| cell.borrow().clone())
}
...
fn apply_text_editor_text_at(focus: &FocusedTextEditor, text: &str, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> bool {
    if !focused_text_editor_matches(focus) {
        return false;
    }
    let outcome = with_live_text_editor(focus, |scene| crate::engine_canvas::text_editor_replace_selection_into(scene, text, input));
    match outcome {
...
fn apply_focused_text_editor_clipboard_key(key: &ui_wgpu::wgpu::KeyAction, modifiers: &ui_wgpu::wgpu::PointerModifiers, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> Option<bool> {
    if !modifiers.ctrl_or_meta() || modifiers.alt || modifiers.shift {
        return None;
    }
    let ui_wgpu::wgpu::KeyAction::Char(ch) = key else { return None };
    let cut = ch.eq_ignore_ascii_case("x");
    let paste = ch.eq_ignore_ascii_case("v");
    if !cut && !paste && !ch.eq_ignore_ascii_case("c") {
        return None;
    }
    let Some(focus) = focused_text_editor() else { return Some(false) };
    if paste {
        if with_live_text_editor(&focus, |_| ()).is_none() {
            FOCUSED_TEXT_EDITOR.with(|cell| *cell.borrow_mut() = None);
            return Some(false);
        }
        #[cfg(all(not(target_arch = "wasm32"), test))]
        if let Some(text) = read_os_clipboard() {
            apply_text_editor_text_at(&focus, &text, input);
        }
        #[cfg(all(not(target_arch = "wasm32"), not(test)))]
        if let Err(fault) = queue_text_editor_clipboard_read(focus) {
            input.record_action_fault(fault);
        }
        return Some(true);
    }
    let outcome = with_live_text_editor(&focus, |scene| {
        let Some(text) = crate::engine_canvas::text_editor_selection_text(scene) else { return Ok(false) };
        write_os_clipboard(&text);
        if cut {
            crate::engine_canvas::text_editor_replace_selection_into(scene, "", input)
        } else {
            Ok(true)
        }
    });
...
/// ⌨️ Commits host-delivered paste or composition text into the exact accepted editor focus.
...
/// 🖱️ A primary press outside the focused mounted editor transfers keyboard ownership.
pub fn blur_focused_text_editor_for_pointer(target: Option<&ScenePointerTarget>, down: bool, button: i16) -> bool {
    if !down || button != 0 {
        return false;
    }
    FOCUSED_TEXT_EDITOR.with(|cell| {
...
thread_local! {
    static FOCUSED_TABLE_STEPPER: std::cell::RefCell<Option<FocusedTableStepper>> = const { std::cell::RefCell::new(None) };
}

fn focus_table_stepper(window_id: &str, window_generation: u64, node: NodeId, host_id: &str, target: crate::scenes::TableStepperFocusTarget) {
    FOCUSED_TABLE_STEPPER.with(|cell| {
        *cell.borrow_mut() = Some(FocusedTableStepper { window_id: window_id.to_owned(), window_generation, node, host_id: host_id.to_owned(), row_id: target.row_id, column_id: target.column_id });
    });
}

/// 🖱️ A primary press transfers DOM-style focus away from the current Table spinbutton;
/// the accepted centre-segment intent re-arms it later in the same pointer turn when appropriate.
pub fn blur_focused_table_stepper_for_pointer(down: bool, button: i16) -> bool {
    down && button == 0 && FOCUSED_TABLE_STEPPER.with(|cell| cell.borrow_mut().take().is_some())
}

/** ⌨️ Offers a key to the accepted Table stepper focus. The retained row/column address is
 * re-resolved against the live scene before the current cell descriptor is dispatched, so a
 * replaced row, column, component or window generation cannot receive a delayed key. */
pub fn apply_focused_table_stepper_key(key: &ui_wgpu::wgpu::KeyAction, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> bool {
    let focus = FOCUSED_TABLE_STEPPER.with(|cell| cell.borrow().as_ref().map(|focus| (focus.window_id.clone(), focus.window_generation, focus.node, focus.host_id.clone(), focus.row_id.clone(), focus.column_id.clone())));
    let Some((window_id, window_generation, node, host_id, row_id, column_id)) = focus else {
        return false;
    };
    let outcome = UI_ENGINE.with(|cell| {
        let engine = cell.borrow();
        ...
        crate::scenes::table_stepper_apply_key(scene, &row_id, &column_id, key, input)
    });
    match outcome {
        Some(Ok(crate::scenes::TableStepperKeyOutcome::Consumed)) => true,
        Some(Ok(crate::scenes::TableStepperKeyOutcome::Unhandled)) => false,
        Some(Err(fault)) => {
            input.record_action_fault(fault);
            true
        }
...
//#endregion 📊️TableStepperFocus

//#region 🗂️VirtualFileSystemFocus
struct FocusedVfsControl {
    window_id: String,
    window_generation: u64,
    node: NodeId,
    host_id: String,
    target: crate::scenes::VfsAccessibilityFocusTarget,
}

thread_local! {
    static FOCUSED_VFS_CONTROL: std::cell::RefCell<Option<FocusedVfsControl>> = const { std::cell::RefCell::new(None) };
}

fn focus_vfs_control(window_id: &str, window_generation: u64, node: NodeId, host_id: &str, target: crate::scenes::VfsAccessibilityFocusTarget) {
    FOCUSED_VFS_CONTROL.with(|cell| {
        *cell.borrow_mut() = Some(FocusedVfsControl { window_id: window_id.to_owned(), window_generation, node, host_id: host_id.to_owned(), target });
    });
}
...
pub fn blur_focused_vfs_control_for_pointer(down: bool, button: i16) -> bool {
    down && button == 0 && FOCUSED_VFS_CONTROL.with(|cell| cell.borrow_mut().take().is_some())
}

pub fn apply_focused_vfs_control_key(key: &ui_wgpu::wgpu::KeyAction, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> bool {
    let Some((window_id, window_generation, node, host_id, target)) = focused_vfs_control() else { return false };
    let outcome = UI_ENGINE.with(|cell| {
        let engine = cell.borrow();
        if engine.surface_generation(&window_id) != Some(window_generation) || ui_document_close_pending_for(&window_id) {
            return None;
        ...
        let retained = engine.tree(&window_id).and_then(|tree| tree.node(node))?;
        let UiNode::ComponentScene(scene) = &retained.spec.0 else { return None };
        if scene.component_kind != ui_wgpu::wgpu::SurfaceKind::VirtualFileSystem || scene.host_id != host_id || crate::scenes::scene_host_retiring(&host_id) {
            return None;
        }
        crate::scenes::vfs_accessibility_apply_key(scene, &target, key, input)
    });
    match outcome {
        Some(Ok(crate::scenes::VfsAccessibilityKeyOutcome::Consumed)) => true,
        Some(Ok(crate::scenes::VfsAccessibilityKeyOutcome::Unhandled)) => false,
        Some(Err(fault)) => {
            input.record_action_fault(fault);
            true
        }
...
//#endregion 🗂️VirtualFileSystemFocus

//#region 🖋️InkEditorFocus
struct FocusedInkEditor {
    window_id: String,
    window_generation: u64,
    node: NodeId,
    host_id: String,
...
thread_local! {
    static FOCUSED_INK_EDITOR: std::cell::RefCell<Option<FocusedInkEditor>> = const { std::cell::RefCell::new(None) };
}

fn focus_ink_editor(window_id: &str, window_generation: u64, node: NodeId, host_id: &str) {
    FOCUSED_INK_EDITOR.with(|cell| *cell.borrow_mut() = Some(FocusedInkEditor { window_id: window_id.to_owned(), window_generation, node, host_id: host_id.to_owned() }));
}

fn focused_ink_editor() -> Option<(String, u64, NodeId, String)> {
    FOCUSED_INK_EDITOR.with(|cell| cell.borrow().as_ref().map(|focus| (focus.window_id.clone(), focus.window_generation, focus.node, focus.host_id.clone())))
}

fn clear_focused_ink_editor(host_id: &str, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) {
    crate::scenes::cancel_ink_edit(host_id, input);
    FOCUSED_INK_EDITOR.with(|cell| *cell.borrow_mut() = None);
}

pub fn apply_focused_ink_editor_key(key: &ui_wgpu::wgpu::KeyAction, modifiers: &ui_wgpu::wgpu::PointerModifiers, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> bool {
    let Some((window_id, generation, node, host_id)) = focused_ink_editor() else { return false };
    let outcome = UI_ENGINE.with(|cell| {
        let engine = cell.borrow();
        ...
        Some(crate::scenes::ink_edit_key_into(scene, key, modifiers, input))
    });
    match outcome {
        Some(Ok(consumed)) => {
            if !crate::scenes::ink_edit_active(&host_id) {
                FOCUSED_INK_EDITOR.with(|cell| *cell.borrow_mut() = None);
            }
            consumed
        }
        ...
        None => {
            clear_focused_ink_editor(&host_id, input);
            false
        }
    }
}

pub fn blur_focused_ink_editor_at(x: f32, y: f32, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> bool {
    let Some((window_id, generation, node, host_id)) = focused_ink_editor() else { return false };
    let outcome = UI_ENGINE.with(|cell| {
        let engine = cell.borrow();
        if engine.surface_generation(&window_id) != Some(generation) {
        ...
        Some(crate::scenes::ink_edit_blur_into(scene, x, y, input))
    });
    match outcome {
        Some(Ok(committed)) => {
            if committed {
...
//#endregion 🖋️InkEditorFocus

//#region 📋️InkClipboardFocus
#[derive(Clone)]
struct InkClipboardAddress {
    window_id: String,
    window_generation: u64,
    node: NodeId,
    host_id: String,
...
thread_local! {
    static FOCUSED_INK_SURFACE: std::cell::RefCell<Option<InkClipboardAddress>> = const { std::cell::RefCell::new(None) };
}

fn focus_ink_surface(window_id: &str, window_generation: u64, node: NodeId, host_id: &str, rect: Rect) {
    FOCUSED_INK_SURFACE.with(|cell| *cell.borrow_mut() = Some(InkClipboardAddress { window_id: window_id.to_owned(), window_generation, node, host_id: host_id.to_owned(), rect }));
}

fn focused_ink_surface() -> Option<InkClipboardAddress> {
    FOCUSED_INK_SURFACE.with(|cell| cell.borrow().clone())
}
...
fn with_live_ink_surface<T>(address: &InkClipboardAddress, apply: impl FnOnce(&UiComponentSceneNode) -> T) -> Option<T> {
    UI_ENGINE.with(|cell| {
        ...
        let retained = engine.tree(&address.window_id).and_then(|tree| tree.node(address.node))?;
        let UiNode::ComponentScene(scene) = &retained.spec.0 else { return None };
        (scene.component_kind == ui_wgpu::wgpu::SurfaceKind::InkCanvas && scene.host_id == address.host_id && !crate::scenes::scene_host_retiring(&address.host_id)).then(|| apply(scene))
    })
}
...
fn apply_ink_clipboard_content(address: &InkClipboardAddress, kind: crate::scenes::InkClipboardContentKind, content: &str, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> bool {
    ...
    let result = with_live_ink_surface(address, |scene| {
        if editor {
            if kind == crate::scenes::InkClipboardContentKind::Text {
                crate::scenes::ink_edit_paste_into(scene, content)
            } else {
                Ok(false)
            }
        } else {
            let target = crate::scenes::ink_clipboard_target(scene, address.rect)?;
            crate::scenes::ink_clipboard_paste_into(scene, target, kind, content, input)
        }
    });
    match result {
        Some(Ok(consumed)) => consumed,
...
pub fn apply_focused_ink_paste(text: &str, image_data_url: bool, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> bool {
    let Some(address) = focused_ink_surface() else { return false };
    let kind = if image_data_url { crate::scenes::InkClipboardContentKind::ImageDataUrl } else { crate::scenes::InkClipboardContentKind::Text };
    apply_ink_clipboard_content(&address, kind, text, input)
}

#[cfg(not(target_arch = "wasm32"))]
struct PendingNativeInkClipboard {
    address: InkClipboardAddress,
    outcome: Option<Result<Option<ui_wgpu::wgpu::ClipboardContent>, ui_wgpu::wgpu::BoundedActionFault>>,
}

#[cfg(not(target_arch = "wasm32"))]
struct PendingNativeInkClipboardSlot {
    generation: u64,
    mounted: Option<PendingNativeInkClipboard>,
}

#[cfg(not(target_arch = "wasm32"))]
#[derive(Clone, Copy)]
struct PendingNativeInkClipboardToken {
    slot: u8,
    generation: u64,
}
...
#[cfg(not(target_arch = "wasm32"))]
thread_local! {
    static PENDING_NATIVE_INK_CLIPBOARD: std::cell::RefCell<[PendingNativeInkClipboardSlot; 32]> = std::cell::RefCell::new(std::array::from_fn(|_| PendingNativeInkClipboardSlot { generation: 0, mounted: None }));
}
...
fn complete_pending_native_ink_clipboard(token: PendingNativeInkClipboardToken, result: Result<Option<ui_wgpu::wgpu::ClipboardContent>, String>) {
    PENDING_NATIVE_INK_CLIPBOARD.with(|cell| {
        ...
        if let Some(pending) = slot.mounted.as_mut() {
            pending.outcome = Some(result.map_err(|_| ui_wgpu::wgpu::BoundedActionFault::Structure));
        }
    });
}

#[cfg(not(target_arch = "wasm32"))]
fn reserve_pending_native_ink_clipboard(address: InkClipboardAddress) -> Option<PendingNativeInkClipboardToken> {
    PENDING_NATIVE_INK_CLIPBOARD.with(|cell| {
        let mut slots = cell.borrow_mut();
        let (index, slot) = slots.iter_mut().enumerate().find(|(_, slot)| slot.mounted.is_none() && slot.generation != u64::MAX)?;
        slot.generation = slot.generation.checked_add(1)?;
        slot.mounted = Some(PendingNativeInkClipboard { address, outcome: None });
        Some(PendingNativeInkClipboardToken { slot: u8::try_from(index).ok()?, generation: slot.generation })
    })
}

#[cfg(all(not(target_arch = "wasm32"), not(test)))]
fn queue_ink_clipboard_read(address: InkClipboardAddress) -> Result<(), ui_wgpu::wgpu::BoundedActionFault> {
    let Some(token) = reserve_pending_native_ink_clipboard(address) else { return Err(ui_wgpu::wgpu::BoundedActionFault::ItemCredits) };
    submit_clipboard_io(ui_wgpu::wgpu::ClipboardIoJob::read(), move |result| complete_pending_native_ink_clipboard(token, result));
    Ok(())
}

#[cfg(not(target_arch = "wasm32"))]
struct PendingNativeTextEditorClipboard {
    focus: FocusedTextEditor,
    outcome: Option<Result<Option<ui_wgpu::wgpu::ClipboardContent>, ui_wgpu::wgpu::BoundedActionFault>>,
}

#[cfg(not(target_arch = "wasm32"))]
struct PendingNativeTextEditorClipboardSlot {
    generation: u64,
    mounted: Option<PendingNativeTextEditorClipboard>,
}

#[cfg(not(target_arch = "wasm32"))]
#[derive(Clone, Copy)]
struct PendingNativeTextEditorClipboardToken {
    slot: u8,
    generation: u64,
}
...
#[cfg(not(target_arch = "wasm32"))]
thread_local! {
    static PENDING_NATIVE_TEXT_EDITOR_CLIPBOARD: std::cell::RefCell<[PendingNativeTextEditorClipboardSlot; 32]> = std::cell::RefCell::new(std::array::from_fn(|_| PendingNativeTextEditorClipboardSlot { generation: 0, mounted: None }));
}

#[cfg(not(target_arch = "wasm32"))]
fn complete_pending_native_text_editor_clipboard(token: PendingNativeTextEditorClipboardToken, result: Result<Option<ui_wgpu::wgpu::ClipboardContent>, String>) {
    PENDING_NATIVE_TEXT_EDITOR_CLIPBOARD.with(|cell| {
        let mut slots = cell.borrow_mut();
        let Some(slot) = slots.get_mut(usize::from(token.slot)).filter(|slot| slot.generation == token.generation) else { return };
        if let Some(pending) = slot.mounted.as_mut() {
            pending.outcome = Some(result.map_err(|_| ui_wgpu::wgpu::BoundedActionFault::Structure));
...
#[cfg(all(not(target_arch = "wasm32"), not(test)))]
fn queue_text_editor_clipboard_read(focus: FocusedTextEditor) -> Result<(), ui_wgpu::wgpu::BoundedActionFault> {
    let token = PENDING_NATIVE_TEXT_EDITOR_CLIPBOARD.with(|cell| {
        let mut slots = cell.borrow_mut();
        let (index, slot) = slots.iter_mut().enumerate().find(|(_, slot)| slot.mounted.is_none() && slot.generation != u64::MAX)?;
        slot.generation = slot.generation.checked_add(1)?;
        slot.mounted = Some(PendingNativeTextEditorClipboard { focus, outcome: None });
        Some(PendingNativeTextEditorClipboardToken { slot: u8::try_from(index).ok()?, generation: slot.generation })
    });
    let Some(token) = token else { return Err(ui_wgpu::wgpu::BoundedActionFault::ItemCredits) };
    submit_clipboard_io(ui_wgpu::wgpu::ClipboardIoJob::read(), move |result| complete_pending_native_text_editor_clipboard(token, result));
    Ok(())
}

#[cfg(not(target_arch = "wasm32"))]
fn rgba_png_data_url(width: u32, height: u32, bytes: Vec<u8>) -> Option<String> {
    use base64::Engine as _;
    let image = image::RgbaImage::from_raw(width, height, bytes)?;
    let mut png = Vec::new();
    image::DynamicImage::ImageRgba8(image).write_to(&mut std::io::Cursor::new(&mut png), image::ImageFormat::Png).ok()?;
    Some(format!("data:image/png;base64,{}", base64::engine::general_purpose::STANDARD.encode(png)))
}

#[cfg(all(not(target_arch = "wasm32"), not(test)))]
pub fn drive_clipboard_io_step(input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> bool {
    pump_clipboard_io_one();
    let pending_text = PENDING_NATIVE_TEXT_EDITOR_CLIPBOARD.with(|cell| {
        let mut slots = cell.borrow_mut();
        let slot = slots.iter_mut().find(|slot| slot.mounted.as_ref().is_some_and(|pending| pending.outcome.is_some()))?;
        slot.mounted.take()
    });
    if let Some(pending) = pending_text {
        return match pending.outcome.expect("ready text editor clipboard completion") {
            Ok(Some(ui_wgpu::wgpu::ClipboardContent::Text(text))) => apply_text_editor_text_at(&pending.focus, &text, input),
            Ok(Some(ui_wgpu::wgpu::ClipboardContent::ImageRgba8 { .. })) | Ok(None) => false,
            Err(fault) => {
                input.record_action_fault(fault);
                true
            }
    ...
    let pending = PENDING_NATIVE_INK_CLIPBOARD.with(|cell| {
        let mut slots = cell.borrow_mut();
        let slot = slots.iter_mut().find(|slot| slot.mounted.as_ref().is_some_and(|pending| pending.outcome.is_some()))?;
        slot.mounted.take()
    });
    let Some(pending) = pending else { return false };
    match pending.outcome.expect("ready clipboard completion") {
        Ok(Some(ui_wgpu::wgpu::ClipboardContent::Text(text))) => apply_ink_clipboard_content(&pending.address, crate::scenes::InkClipboardContentKind::Text, &text, input),
        Ok(Some(ui_wgpu::wgpu::ClipboardContent::ImageRgba8 { width, height, bytes })) => match rgba_png_data_url(width, height, bytes) {
            Some(data) => apply_ink_clipboard_content(&pending.address, crate::scenes::InkClipboardContentKind::ImageDataUrl, &data, input),
            None => {
                input.record_action_fault(ui_wgpu::wgpu::BoundedActionFault::Structure);
                true
            }
...
#[cfg(any(target_arch = "wasm32", test))]
pub fn drive_clipboard_io_step(_input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> bool {
    false
}

pub fn apply_focused_ink_surface_key(key: &ui_wgpu::wgpu::KeyAction, modifiers: &ui_wgpu::wgpu::PointerModifiers, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> bool {
    let Some(address) = focused_ink_surface() else { return false };
    if focused_ink_editor_owns(&address) || !modifiers.ctrl_or_meta() || modifiers.alt || modifiers.shift {
        return false;
    }
    let ui_wgpu::wgpu::KeyAction::Char(key) = key else { return false };
    if key.eq_ignore_ascii_case("c") {
        let result = with_live_ink_surface(&address, crate::scenes::ink_clipboard_copy_text);
        match result {
            Some(Ok(Some(text))) => write_os_clipboard(&text),
            Some(Err(fault)) => input.record_action_fault(fault),
            None => FOCUSED_INK_SURFACE.with(|cell| *cell.borrow_mut() = None),
            Some(Ok(None)) => {}
        }
        return true;
    }
    ...
    #[cfg(all(not(target_arch = "wasm32"), test))]
    if let Some(text) = read_os_clipboard() {
        apply_ink_clipboard_content(&address, crate::scenes::InkClipboardContentKind::Text, &text, input);
    }
    #[cfg(all(not(target_arch = "wasm32"), not(test)))]
    if let Err(fault) = queue_ink_clipboard_read(address) {
        input.record_action_fault(fault);
    }
    true
}

struct InkClipboardStream {
    id: u64,
    address: InkClipboardAddress,
    image_data_url: bool,
    text: String,
...
thread_local! {
    static INK_CLIPBOARD_STREAMS: std::cell::RefCell<[Option<InkClipboardStream>; 32]> = std::cell::RefCell::new(std::array::from_fn(|_| None));
}

pub fn start_focused_ink_clipboard_stream(stream: u64, image_data_url: bool, declared_bytes: usize) -> Result<bool, &'static str> {
    if declared_bytes > 3 * (16 * 1024 - 1) {
        return Err("ink clipboard stream exceeded declared byte credits");
    }
    let Some(address) = focused_ink_surface() else { return Ok(false) };
    let live = with_live_ink_surface(&address, |_| ()).is_some();
    if !live || (image_data_url && focused_ink_editor_owns(&address)) {
        return Ok(false);
    }
    INK_CLIPBOARD_STREAMS.with(|cell| {
        let mut slots = cell.borrow_mut();
        if slots.iter().flatten().any(|mounted| mounted.id == stream) {
            return Err("ink clipboard stream identifier is already mounted");
        }
        let Some(slot) = slots.iter_mut().find(|slot| slot.is_none()) else { return Err("ink clipboard stream credits exhausted") };
        *slot = Some(InkClipboardStream { id: stream, address, image_data_url, text: String::new() });
        Ok(true)
    })
}

pub fn push_focused_ink_clipboard_stream(stream: u64, text: &str) -> Result<bool, &'static str> {
    INK_CLIPBOARD_STREAMS.with(|cell| {
        let mut slots = cell.borrow_mut();
        let Some(mounted) = slots.iter_mut().flatten().find(|mounted| mounted.id == stream) else { return Ok(false) };
        if mounted.text.len().checked_add(text.len()).is_none_or(|bytes| bytes >= 16 * 1024) {
            return Err("ink clipboard stream exceeded payload byte credits");
        }
        mounted.text.push_str(text);
        Ok(true)
    })
}

pub fn commit_focused_ink_clipboard_stream(stream: u64, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> bool {
    let mounted = INK_CLIPBOARD_STREAMS.with(|cell| {
        let mut slots = cell.borrow_mut();
        let slot = slots.iter().position(|slot| slot.as_ref().is_some_and(|mounted| mounted.id == stream))?;
        slots[slot].take()
    });
    mounted.is_some_and(|mounted| {
        let kind = if mounted.image_data_url { crate::scenes::InkClipboardContentKind::ImageDataUrl } else { crate::scenes::InkClipboardContentKind::Text };
        apply_ink_clipboard_content(&mounted.address, kind, &mounted.text, input)
    })
}

pub fn abort_focused_ink_clipboard_stream(stream: u64) -> bool {
    INK_CLIPBOARD_STREAMS.with(|cell| {
        let mut slots = cell.borrow_mut();
        let Some(slot) = slots.iter().position(|slot| slot.as_ref().is_some_and(|mounted| mounted.id == stream)) else { return false };
        slots[slot] = None;
        true
    })
}

struct TextEditorInputStream {
    id: u64,
    focus: FocusedTextEditor,
    text: String,
}
...
thread_local! {
    static TEXT_EDITOR_INPUT_STREAMS: std::cell::RefCell<[Option<TextEditorInputStream>; 32]> = std::cell::RefCell::new(std::array::from_fn(|_| None));
}

pub fn start_focused_text_editor_stream(stream: u64, declared_bytes: usize) -> Result<bool, &'static str> {
    if declared_bytes >= 16 * 1024 {
        return Err("text editor stream exceeded declared byte credits");
    }
    let Some(focus) = focused_text_editor() else { return Ok(false) };
    if with_live_text_editor(&focus, |_| ()).is_none() {
        return Ok(false);
    }
    TEXT_EDITOR_INPUT_STREAMS.with(|cell| {
        let mut slots = cell.borrow_mut();
        if slots.iter().flatten().any(|mounted| mounted.id == stream) {
            return Err("text editor stream identifier is already mounted");
        }
        let Some(slot) = slots.iter_mut().find(|slot| slot.is_none()) else { return Err("text editor stream credits exhausted") };
        *slot = Some(TextEditorInputStream { id: stream, focus, text: String::new() });
        Ok(true)
    })
}

pub fn push_focused_text_editor_stream(stream: u64, text: &str) -> Result<bool, &'static str> {
    TEXT_EDITOR_INPUT_STREAMS.with(|cell| {
        let mut slots = cell.borrow_mut();
        let Some(mounted) = slots.iter_mut().flatten().find(|mounted| mounted.id == stream) else { return Ok(false) };
        if mounted.text.len().checked_add(text.len()).is_none_or(|bytes| bytes >= 16 * 1024) {
            return Err("text editor stream exceeded payload byte credits");
        }
        mounted.text.push_str(text);
        Ok(true)
    })
...
pub fn commit_focused_text_editor_stream(stream: u64, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> bool {
    let mounted = TEXT_EDITOR_INPUT_STREAMS.with(|cell| {
        let mut slots = cell.borrow_mut();
        let slot = slots.iter().position(|slot| slot.as_ref().is_some_and(|mounted| mounted.id == stream))?;
        slots[slot].take()
    });
...
pub fn abort_focused_text_editor_stream(stream: u64) -> bool {
    TEXT_EDITOR_INPUT_STREAMS.with(|cell| {
        let mut slots = cell.borrow_mut();
        let Some(slot) = slots.iter().position(|slot| slot.as_ref().is_some_and(|mounted| mounted.id == stream)) else { return false };
        slots[slot] = None;
...
fn close_window_focus_clipboard_one(window_id: &str, generation: u64) -> bool {
    ...
    if TEXT_EDITOR_INPUT_STREAMS.with(|cell| {
        let mut slots = cell.borrow_mut();
        let Some(slot) = slots.iter_mut().find(|slot| slot.as_ref().is_some_and(|stream| stream.focus.window_id == window_id && stream.focus.window_generation == generation)) else { return false };
        slot.take();
        true
    }) {
        return true;
    ...
    #[cfg(not(target_arch = "wasm32"))]
    if PENDING_NATIVE_TEXT_EDITOR_CLIPBOARD.with(|cell| {
        let mut slots = cell.borrow_mut();
        let Some(slot) = slots.iter_mut().find(|slot| slot.mounted.as_ref().is_some_and(|pending| pending.focus.window_id == window_id && pending.focus.window_generation == generation)) else { return false };
        slot.mounted.take();
        true
    }) {
        return true;
...
fn close_retiring_focus_clipboard_one() -> bool {
    ...
    if TEXT_EDITOR_INPUT_STREAMS.with(|cell| {
        let mut slots = cell.borrow_mut();
        let Some(slot) = slots.iter_mut().find(|slot| slot.as_ref().is_some_and(|stream| crate::scenes::scene_host_retiring(&stream.focus.host_id))) else { return false };
        slot.take();
        true
    }) {
        return true;
    ...
    if PENDING_NATIVE_TEXT_EDITOR_CLIPBOARD.with(|cell| {
        let mut slots = cell.borrow_mut();
        let Some(slot) = slots.iter_mut().find(|slot| slot.mounted.as_ref().is_some_and(|pending| crate::scenes::scene_host_retiring(&pending.focus.host_id))) else { return false };
        slot.mounted.take();
        true
    }) {
        return true;
...
fn process_scene_interaction(intent: &mut SceneInteractionIntent, input: &mut ui_wgpu::wgpu::InputState<ActionDescriptor>) -> Result<SceneIntentProgress, ui_wgpu::wgpu::BoundedActionFault> {
    ...
    let live = UI_ENGINE.with(|cell| {
        ...
        engine.tree_revision(window_id) == Some(intent.tree_revision)
            && engine.surface_generation(window_id) == Some(intent.surface_generation)
            && engine.tree(window_id).and_then(|tree| tree.node(node)).and_then(|retained| match &retained.spec.0 {
                UiNode::ComponentScene(scene) => Some(scene.component_kind == kind && scene.host_id == intent.host_id),
                _ => None,
            }) == Some(true)
    });
    if !live || ui_document_close_pending_for(window_id) || crate::scenes::scene_host_retiring(&intent.host_id) {
        intent.retiring = true;
    }
    if intent.retiring {
    ...
    if kind == ui_wgpu::wgpu::SurfaceKind::Canvas2d {
        return UI_ENGINE.with(|cell| {
            ...
            let result = match intent.event {
                SceneIntentEvent::PointerDown { x, y, button, modifiers } => crate::scenes::canvas_pointer_button_into(
                    scene,
                    rect,
                    intent.pointer_id.ok_or(ui_wgpu::wgpu::BoundedActionFault::ItemCredits)?,
                    window_id,
                    intent.surface_generation,
                    x,
                    y,
                    true,
                    button,
                ...
                SceneIntentEvent::PointerUp { x, y, button, modifiers } => crate::scenes::canvas_pointer_button_into(
                    scene,
                    rect,
                    intent.pointer_id.ok_or(ui_wgpu::wgpu::BoundedActionFault::ItemCredits)?,
                    window_id,
    ...
    if kind == ui_wgpu::wgpu::SurfaceKind::InkCanvas {
        return UI_ENGINE.with(|cell| {
            ...
            if matches!(intent.event, SceneIntentEvent::PointerDown { button: 0, .. }) {
                if let Some(generation) = engine.surface_generation(window_id) {
                    focus_ink_surface(window_id, generation, node, &scene.host_id, rect);
                }
            }
            match intent.event {
                ...
                event => {
                    if intent.ink_job.is_none() {
                        ...
                        intent.ink_job = crate::scenes::InkInteractionJob::new(operation_generation, intent.surface_generation, intent.pointer_id, scene, event)?;
                        if intent.ink_job.is_none() {
                            return Ok(SceneIntentProgress::Complete);
                        }
                    ...
                    match intent.ink_job.as_mut().expect("ink job retained").step(operation_generation, scene, rect, input)? {
                        ...
                        crate::scenes::InkInteractionStep::Complete => {
                            if intent.ink_job.as_ref().is_some_and(crate::scenes::InkInteractionJob::opened_edit) {
                                if let Some(generation) = engine.surface_generation(window_id) {
                                    focus_ink_editor(window_id, generation, node, &scene.host_id);
                                }
                            }
                            intent.retiring = true;
    ...
    UI_ENGINE.with(|cell| {
        ...
        match intent.event {
            SceneIntentEvent::PointerDown { x, y, button, modifiers } => {
                ...
                if kind == ui_wgpu::wgpu::SurfaceKind::VirtualFileSystem && button == 0 {
                    if let Some(target) = crate::scenes::vfs_accessibility_focus_target(scene, x, y) {
                        focus_vfs_control(window_id, intent.surface_generation, node, &scene.host_id, target);
                    }
                }
                let pointer_id = intent.pointer_id.ok_or(ui_wgpu::wgpu::BoundedActionFault::ItemCredits)?;
...
struct FrameworkSceneHost<'ctx> {
    ...
    chrome_labels: crate::scenes::SceneChromeLabels,
...
impl ui_wgpu::wgpu::SceneHost for FrameworkSceneHost<'_> {
    ...
    fn paint_slot_step(
        ...
        let mut hosts = crate::scenes::SceneEngineHosts { chrome_labels: self.chrome_labels, world3d_states: &mut *self.world3d_states, world_resources: &mut *self.world_resources, window_id: self.window_id };
        match &slot.content {
            ui_wgpu::wgpu::SlotContent::Scene(scene) => {
                ...
                render_component_scene_step(scene, slot.rect, &mut ctx, cursor, &mut hosts, self.driver_drag, self.window_generation)
            }
...
impl UiDocumentFrameCursor {
    ...
    /// 📐️ Whether intrinsic layout is accepted and the cursor has reached retained paint.
    pub fn layout_is_accepted(&self) -> bool {
        matches!(self.phase, UiDocumentFramePhase::Paint | UiDocumentFramePhase::Complete)
    }

    /// 🧭️ Re-solves accepted content against the compact host box before its first paint.
    pub fn restart_viewport_after_host_reflow(&mut self) {
        if matches!(self.phase, UiDocumentFramePhase::Paint) {
            self.phase = UiDocumentFramePhase::Viewport;
            self.stalled = 0;
        }
    }
}
        ...
        match cursor.phase {
            ...
            UiDocumentFramePhase::Paint => {
                ...
                let mut scene_host = FrameworkSceneHost {
                    ...
                    chrome_labels: hosts.chrome_labels,
                    window_id,
                    driver_drag,
                    document_generation,
                    window_generation: engine.surface_generation(window_id).unwrap_or(generation),
                ...
                match paint {
                    ...
                    ui_wgpu::wgpu::UiFrameStep::Fault => {
                        document_debug_log(&format!(
                            ...
                            engine.paint_stall_census(window_id)
                        ));
                        cursor.phase = UiDocumentFramePhase::Fault;
                    }
                }
...
/// 🖼️ Returns pixels only for the requested image source while preserving the fallback during loading.
pub(crate) fn current_ui_image_key(id: &str, src: &str) -> Option<String> {
    if id.len() > 256 || src.is_empty() || src.len() > RETAINED_UI_IMAGE_SOURCE_BYTES {
        return None;
    }
    if src.starts_with("data:") {
        resolve_ui_image_data_url(id, src);
    } else {
        queue_ui_image_url_fetch(id, src);
    }
    if !UI_IMAGE_LAST_URL.with(|cell| cell.borrow().get(id).is_some_and(|current| current == src)) {
        return None;
    }
    UI_IMAGE_URL_CACHE.with(|cell| cell.borrow().get(id).cloned())
}
...
#[cfg(test)]
#[path = "../../🧪️tests/🔬️wgpu-render-plan-validator/🦀️.rs"]
...
struct DumpFrameStats {
    ...
    frame_latency: crate::frame_latency::FrameLatencySnapshot,
}
...
struct TableStepperAccessibilityEntry {
    scene_node: NodeId,
    host_id: String,
    depth: usize,
    cell: crate::scenes::TableStepperAccessibilityCell,
}

fn scene_virtual_accessibility_node_id(key: &str) -> u64 {
    let hash = key.as_bytes().iter().fold(0xcbf29ce484222325_u64, |hash, byte| (hash ^ u64::from(*byte)).wrapping_mul(0x100000001b3));
...
fn append_table_stepper_accessibility_nodes(window_id: &str, tree: &ui_wgpu::wgpu::UiTree, driver_drag: ui_wgpu::wgpu::UiDriverDrag, nodes: &mut Vec<ui_contract::AccessibilityProjectionNode>) {
    ...
    for entry in table_stepper_accessibility_entries(tree, driver_drag) {
        ...
        nodes.push(ui_contract::AccessibilityProjectionNode {
            ...
            value_min: Some(entry.cell.min),
            value_max: Some(entry.cell.max),
            value_now: Some(entry.cell.value),
            value_text: Some(entry.cell.value.to_string()),
            busy: false,
        });
    }
}
...
struct VfsAccessibilityEntry {
    ...
    host_id: String,
    depth: usize,
    control: crate::scenes::VfsAccessibilityControl,
}

fn collect_vfs_accessibility_entries(tree: &ui_wgpu::wgpu::UiTree, node: NodeId, depth: usize, entries: &mut Vec<VfsAccessibilityEntry>) {
    let Some(retained) = tree.node(node) else { return };
    if let UiNode::ComponentScene(scene) = &retained.spec.0 {
    ...
    for child in tree.children(node) {
        collect_vfs_accessibility_entries(tree, child, depth.saturating_add(1), entries);
    }
}

fn vfs_accessibility_entries(tree: &ui_wgpu::wgpu::UiTree) -> Vec<VfsAccessibilityEntry> {
    let mut entries = Vec::new();
    if let Some(root) = tree.root {
        collect_vfs_accessibility_entries(tree, root, 0, &mut entries);
    }
    entries
}

fn append_vfs_accessibility_nodes(window_id: &str, tree: &ui_wgpu::wgpu::UiTree, nodes: &mut Vec<ui_contract::AccessibilityProjectionNode>) {
    let focus = focused_vfs_control();
    for entry in vfs_accessibility_entries(tree) {
        if nodes.len() >= ui_contract::UI_DOCUMENT_NODES {
            break;
        }
...
fn collect_text_editor_accessibility_entries(tree: &ui_wgpu::wgpu::UiTree, node: NodeId, origin: (f32, f32), depth: usize, entries: &mut Vec<TextEditorAccessibilityEntry>) {
    let Some(retained) = tree.node(node) else { return };
    let (x, y, width, height) = tree.mounted_layout(node).unwrap_or((retained.layout.x, retained.layout.y, retained.layout.width, retained.layout.height));
    let absolute = (origin.0 + x, origin.1 + y);
    if let UiNode::ComponentScene(scene) = &retained.spec.0 {
        if scene.component_kind == ui_wgpu::wgpu::SurfaceKind::TextEditor && scene.presence.visible() && !crate::scenes::scene_host_retiring(&scene.host_id) {
            if let Some(value) = crate::engine_canvas::text_editor_accessibility_value(scene) {
                entries.push(TextEditorAccessibilityEntry {
                    scene_node: node,
                    host_id: scene.host_id.clone(),
                    key: format!("{}.editor", scene.host_id),
                    label: scene.text_editor.as_ref().and_then(|editor| editor.language.as_ref()).map_or_else(|| "Editor".into(), |language| format!("{language} editor")),
                    depth: depth.saturating_add(1),
                    rect: Rect::new(absolute.0, absolute.1, width, height),
                    value,
                    read_only: crate::engine_canvas::text_editor_is_read_only(scene),
                });
            }
        }
    }
    ...
    for child in tree.children(node) {
        collect_text_editor_accessibility_entries(tree, child, absolute, depth.saturating_add(1), entries);
    }
}

fn text_editor_accessibility_entries(tree: &ui_wgpu::wgpu::UiTree) -> Vec<TextEditorAccessibilityEntry> {
    let mut entries = Vec::new();
    if let Some(root) = tree.root {
        collect_text_editor_accessibility_entries(tree, root, (0.0, 0.0), 0, &mut entries);
    }
    entries
}

fn append_text_editor_accessibility_nodes(window_id: &str, tree: &ui_wgpu::wgpu::UiTree, nodes: &mut Vec<ui_contract::AccessibilityProjectionNode>) {
    let focus = focused_text_editor();
    for entry in text_editor_accessibility_entries(tree) {
        if nodes.len() >= ui_contract::UI_DOCUMENT_NODES {
            break;
        }
        ...
        nodes.push(ui_contract::AccessibilityProjectionNode {
            ...
            selected: None,
            expanded: None,
            editable: !entry.read_only,
            multiline: true,
            controls: None,
            active_descendant: None,
            level: None,
            rect: Some([entry.rect.x, entry.rect.y, entry.rect.w, entry.rect.h]),
            value_min: None,
            value_max: None,
            value_now: None,
            value_text: Some(entry.value),
            busy: false,
        });
    }
}
...
fn build_accessibility_dump(engine: &ui_wgpu::wgpu::Ui, requested: Option<&str>) -> DumpAccessibility {
    ...
    let announced: Vec<String> = match requested.filter(|id| !id.is_empty()) {
        ...
        None => ACCESSIBILITY_VISIBLE_WINDOWS.borrow().published.iter().filter(|visible| window_ids.iter().any(|live| live == *visible)).cloned().collect(),
    };
...
fn build_frame_stats(engine: &ui_wgpu::wgpu::Ui, requested: Option<&str>) -> DumpFrameStats {
    ...
    if let Some(census) = engine.paint_census(&window_id).filter(|census| census.layers > 0 || census.scene_passes > 0) {
        return DumpFrameStats {
            ...
            frame_latency: crate::frame_latency::snapshot(),
        };
    }
    let Some(draw) = engine.draw_list(&window_id) else {
        return DumpFrameStats { window_id: Some(window_id), window_ids, draw_calls: 0, quad_count: 0, glyph_count: 0, scene_passes: 0, scene_draws: 0, scene_instances: 0, frame_latency: crate::frame_latency::snapshot() };
    };
    ...
    let scene_draws = draw.scene_passes.iter().map(|pass| pass.draws.len() + pass.translucent_draws.len() + pass.material_draws.len() + pass.textured_draws.len() + pass.line_draws.len()).sum();
    let scene_instances = draw
        .scene_passes
        .iter()
        .map(|pass| {
            pass.draws.iter().chain(pass.translucent_draws.iter()).map(|scene_draw| scene_draw.instances.len()).sum::<usize>()
                + pass.material_draws.iter().map(|scene_draw| scene_draw.instances.len()).sum::<usize>()
                + pass.textured_draws.iter().map(|scene_draw| scene_draw.instances.len()).sum::<usize>()
        })
        .sum();
    DumpFrameStats { window_id: Some(window_id), window_ids, draw_calls, quad_count, glyph_count, scene_passes, scene_draws, scene_instances, frame_latency: crate::frame_latency::snapshot() }
}
//#region 🔬️MeshStats
...
/// ♿️ Every announced window's accessibility projection — the chrome's and each visible document window's, in reading
/// order — for the native shell's platform bridge (`♿️native-accessibility`): the same dump the browser build's DOM
/// mirror renders, so both shells announce one tree (ticket 26/09/23 slice WG10).
#[cfg(not(target_arch = "wasm32"))]
pub(crate) fn native_accessibility_windows() -> Vec<crate::native_accessibility::NativeAccessibilityWindow> {
    UI_ENGINE
        .with(|cell| build_accessibility_dump(&cell.borrow(), None))
        .windows
        .into_iter()
        .map(|window| crate::native_accessibility::NativeAccessibilityWindow { window_id: window.window_id, window_generation: window.window_generation, nodes: window.nodes })
        .collect()
}
...
#[derive(Default)]
struct AccessibilityVisibleWindows {
...
pub fn note_accessibility_visible_document(window_id: &str) {
    let mut windows = ACCESSIBILITY_VISIBLE_WINDOWS.borrow_mut();
    if windows.staging.len() < ACCESSIBILITY_VISIBLE_WINDOW_CAPACITY && !windows.staging.iter().any(|visible| visible == window_id) {
        windows.staging.push(window_id.to_string());
    }
}

pub fn publish_accessibility_visible_documents() {
    let mut windows = ACCESSIBILITY_VISIBLE_WINDOWS.borrow_mut();
    let AccessibilityVisibleWindows { published, staging } = &mut *windows;
    std::mem::swap(published, staging);
}

pub fn seal_presented_input_candidate(witness: u64) -> bool {
    let visible = ACCESSIBILITY_VISIBLE_WINDOWS.borrow().staging.clone();
    UI_ENGINE.with(|cell| cell.borrow_mut().seal_presented_input_candidate(witness, &visible))
}

pub fn presented_input_candidate_matches(witness: u64) -> bool {
...
fn prepare_presented_editor_rebase(engine: &ui_wgpu::wgpu::Ui, witness: u64) -> PresentedEditorRebase {
    ...
    let text_streams = TEXT_EDITOR_INPUT_STREAMS.with(|cell| {
        ...
        std::array::from_fn(|index| {
            ...
            Some((stream.id, prepare_presented_editor_node_rebase(engine, witness, &stream.focus.window_id, stream.focus.window_generation, stream.focus.node, &stream.focus.host_id, ui_wgpu::wgpu::SurfaceKind::TextEditor)?))
        })
    });
    #[cfg(not(target_arch = "wasm32"))]
    let native_text = PENDING_NATIVE_TEXT_EDITOR_CLIPBOARD.with(|cell| {
        let slots = cell.borrow();
        std::array::from_fn(|index| {
            let slot = &slots[index];
            let pending = slot.mounted.as_ref()?;
            Some((slot.generation, prepare_presented_editor_node_rebase(engine, witness, &pending.focus.window_id, pending.focus.window_generation, pending.focus.node, &pending.focus.host_id, ui_wgpu::wgpu::SurfaceKind::TextEditor)?))
        })
    });
    let table =
...
fn commit_presented_editor_rebase(rebase: PresentedEditorRebase) {
    ...
    PENDING_NATIVE_TEXT_EDITOR_CLIPBOARD.with(|cell| {
        let mut slots = cell.borrow_mut();
        for (index, prepared) in rebase.native_text.into_iter().enumerate() {
    ...
    if let Some(rebase) = rebase.table {
        FOCUSED_TABLE_STEPPER.with(|cell| {
            ...
            if matches {
                match rebase.candidate_node {
                    Some(node) => focus.as_mut().expect("matched Table stepper focus").node = node,
                    None => *focus = None,
                }
            }
        });
    }
    if let Some(rebase) = rebase.vfs {
        FOCUSED_VFS_CONTROL.with(|cell| {
            let mut focus = cell.borrow_mut();
            let matches = focus.as_ref().is_some_and(|focus| focus.window_id == rebase.window_id && focus.window_generation == rebase.window_generation && focus.node == rebase.old_node && focus.host_id == rebase.host_id);
            if matches {
                match rebase.candidate_node {
                    Some(node) => focus.as_mut().expect("matched VFS focus").node = node,
                    None => *focus = None,
                }
            }
        });
...
pub fn discard_presented_input_candidate(witness: u64) -> bool {
    UI_ENGINE.with(|cell| cell.borrow_mut().discard_presented_input_candidate(witness))
}
...
#[cfg(test)]
pub(crate) fn accessibility_visible_window_ids() -> Vec<String> {
    ACCESSIBILITY_VISIBLE_WINDOWS.borrow().published.clone()
}
...
/// 🟦️.ts`) waits for `window.wasmBindings` then calls exports straight off it, so these land at
