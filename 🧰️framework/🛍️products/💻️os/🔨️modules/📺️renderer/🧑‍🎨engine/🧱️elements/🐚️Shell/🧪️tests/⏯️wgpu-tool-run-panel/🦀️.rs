...
fn paint_panel(shell: &mut ShellState, surface: &str, document: &UiDocumentLease) -> PaintedPanel {
    ...
    let complete = (0..SHELL_WINDOW_PAINT_OPPORTUNITIES.min(1 << 20)).any(|_| {
        ...
        let done = render_ui_document_step(&mut cursor, document, Rect::new(0.0, 0.0, 360.0, 720.0), &mut ctx, surface, "s.test.tool-run", ui_wgpu::wgpu::UiDriverDrag::Handle, &mut hosts);
    ...
    shell.register_retained_body_hits(surface, Rect::new(0.0, 0.0, 360.0, 720.0), &mut input);
...
fn press(shell: &mut ShellState, painted: &mut PaintedPanel, control: &str) {
    let hit = painted.input.hits().iter().find(|hit| hit.control_id.as_deref() == Some(control)).unwrap_or_else(|| panic!("`{control}` published a pointer target")).rect;
    let (x, y) = (hit.x + hit.w * 0.5, hit.y + hit.h * 0.5);
    semio_framework_async::block_on(shell.handle_pointer_button(x, y, true, 0, &mut painted.input, &Theme::default())).expect("published ToolRun press");
    semio_framework_async::block_on(shell.handle_pointer_button(x, y, false, 0, &mut painted.input, &Theme::default())).expect("published ToolRun release");
}

fn close_panel(surface: &str, document: &mut UiDocumentLease) {
    assert!(crate::interpreter::request_ui_document_close(surface));
    for _ in 0..262_144 {
        if !crate::interpreter::ui_document_close_pending_for(surface) {
            break;
        }
        assert!(crate::interpreter::close_ui_document_one(), "each admitted ToolRun close opportunity advances");
    }
...
fn tool_run_panel_of_a_running_run_paints_and_its_buttons_dispatch_the_run() {
    ...
    press(&mut shell, &mut painted, &pause);
    press(&mut shell, &mut painted, &step);
    press(&mut shell, &mut painted, &finalize);
    press(&mut shell, &mut painted, &abort);
    assert_eq!(
    ...
    close_panel(FRAMEWORK_PANEL_TAB_TOOL_RUN_ID, &mut document);
}
