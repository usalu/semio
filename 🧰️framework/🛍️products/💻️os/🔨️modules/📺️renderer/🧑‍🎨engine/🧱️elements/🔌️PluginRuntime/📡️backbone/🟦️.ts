...
function validateDocumentBackboneControlV1(value: DocumentBackboneControlV1): void {
  ...
  if (!Number.isSafeInteger(value.instanceId) || value.instanceId < 1 || value.instanceId > 0xffffffff || typeof value.bindingGeneration !== "bigint" || value.bindingGeneration < 0n || value.bindingGeneration > 0xffffffffffffffffn) throw new Error("actor-document-control.owner");
...
export class ActorDocumentMessagePortV1 {
  ...
  constructor(owner: ActorDocumentOwnerV1, ports: ActorDocumentMessagePortsV1) {
    if (!owner.actorId || !owner.runtimeKey || !owner.clientInstanceId || typeof owner.activationGeneration !== "bigint" || owner.activationGeneration < 0n || !Number.isSafeInteger(owner.instanceId) || owner.instanceId < 1 || owner.instanceId > 0xffffffff || (owner.scope !== null && (!owner.scope.spaceId || !owner.scope.documentId))) throw new Error("actor-document-port.owner");
