 ...
 * `Table/🟦️component.tsx`'s `TableHost` already parses on the React side, and that the wgpu
 ...
 * `Table/🟦️component.tsx` and `Interpreter/🧊️component.rs` before choosing this: reusing an already
 ...
 * only). `TableColumnRecord`/`TableRowRecord`/`TableCellButton` are private to `Table/🟦️component.tsx`
...
/** 🔖️ Hand-stated mirror of `Table/🟦️component.tsx`'s private `TableColumnRecord` — see this file's
...
/** 🔖️ Hand-stated mirror of `Table/🟦️component.tsx`'s private `TableCellButton`. */
...
/** 🔖️ Hand-stated mirror of `Table/🟦️component.tsx`'s private `TableCellRecord`. */
...
/** 🔖️ Hand-stated mirror of `Table/🟦️component.tsx`'s private `TableRowRecord` — `id` is what
 ...
 * `dispatchCellAction`'s own merge convention in `Table/🟦️component.tsx` (base descriptor + a patch
 ...
 * `Table/🟦️component.tsx`'s `renderTableCell`, the same convention every other button cell in that
...
/** 🖼️ The exact `{columnsJson, rowsJson}` pair a `TableScene` (`Table/🟦️component.tsx`'s `scene.table`,
 ...
 * `Table/🟦️component.tsx`'s `TableHost` uses, so it inherits that component's table semantics/
