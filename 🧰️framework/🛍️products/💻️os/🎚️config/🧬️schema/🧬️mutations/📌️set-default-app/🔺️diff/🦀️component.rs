...
pub fn diff(payload: &SetDefaultApp, base: &OpeningPreferences) -> protocol::MutationOutcome<OpeningPreferences> {
    if base.defaults.iter().any(|entry| entry.dialect == payload.dialect && entry.role == payload.role && entry.app == payload.app) {
        ...
        return protocol::MutationOutcome::new(base.clone()).warn("mutation.no-op", format!("\"{}\" is already the default {} for \"{}\".", payload.app.app_id, role, payload.dialect.to_coordinate()));
    }
    let mut defaults: Vec<DefaultApp> = base.defaults.iter().filter(|entry| !(entry.dialect == payload.dialect && entry.role == payload.role)).cloned().collect();
