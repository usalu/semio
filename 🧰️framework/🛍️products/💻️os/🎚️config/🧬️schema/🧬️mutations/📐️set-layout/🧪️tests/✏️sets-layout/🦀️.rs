//! 🧪️ `set-layout` fixture — `✏️sets-layout`.
//!
//! Setting the layout on untouched preferences changes exactly that preference and nothing else.
//!
...
//! `../../🧫️fixtures/✏️sets-layout/`.

use crate::opening_config::mutations::UiPreferencesConfigMutation;
use crate::opening_config::{UiPreferences, UiPreferencesDiff};

const BEFORE: &str = include_str!("../../🧫️fixtures/✏️sets-layout/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../🧫️fixtures/✏️sets-layout/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../🧫️fixtures/✏️sets-layout/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../🧫️fixtures/✏️sets-layout/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../🧫️fixtures/✏️sets-layout/🎯️outcome/🔣️.json");
...
/// ▶️ Setting the layout writes it into the record and leaves every other preference as it was.
#[test]
fn applies_to_committed_after() {
    ...
    assert_eq!(applied, expected_after(), "set-layout/sets-layout: the applied preferences differ from the committed after-snapshot");
}

/// 🎯️ The declared outcome is `applied` with no diagnostics: the layout was not set before.
#[test]
fn declared_outcome_holds() {
    let declared: serde_json::Value = serde_json::from_str(OUTCOME).expect("outcome decodes");
    assert_eq!(declared.get("status").and_then(serde_json::Value::as_str), Some("applied"), "set-layout/sets-layout: this fixture declares a applied outcome");
    let produced = <UiPreferencesConfigMutation as protocol::Mutation<UiPreferences>>::diff(&mutation(), &before());
    assert!(produced.messages().is_empty(), "set-layout/sets-layout: a changed preference emits no diagnostics");
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(json_value(outcome.diff()), committed, "set-layout/sets-layout: produced diff differs from the committed 🔺️diff");
    let decoded: UiPreferencesDiff = dsl::os_pack::json::from_json_str(DIFF).expect("committed diff decodes as UiPreferencesDiff");
    assert_eq!(protocol::MutationDiff::apply(&decoded, &before()).expect("committed diff applies"), expected_after(), "set-layout/sets-layout: the committed diff does not carry before to after");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "set-layout/sets-layout: the inverse did not restore the before-preferences");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_value(&decoded), original, "set-layout/sets-layout: committed {label} JSON is not canonical");
    }
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("payload reparses");
    assert_eq!(json_value(&mutation()), original, "set-layout/sets-layout: committed payload JSON is not canonical");
}
