//! 🧪️ `set-locale` fixture — `✏️sets-locale`.
//!
//! Setting the locale on untouched preferences changes exactly that preference and nothing else.
//!
...
//! `../../🧫️fixtures/✏️sets-locale/`.

use crate::opening_config::mutations::UiPreferencesConfigMutation;
use crate::opening_config::{UiPreferences, UiPreferencesDiff};

const BEFORE: &str = include_str!("../../🧫️fixtures/✏️sets-locale/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../🧫️fixtures/✏️sets-locale/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../🧫️fixtures/✏️sets-locale/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../🧫️fixtures/✏️sets-locale/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../🧫️fixtures/✏️sets-locale/🎯️outcome/🔣️.json");
...
/// ▶️ Setting the locale writes it into the record and leaves every other preference as it was.
#[test]
fn applies_to_committed_after() {
    let base = before();
    let outcome = <UiPreferencesConfigMutation as protocol::Mutation<UiPreferences>>::diff(&mutation(), &base);
    let applied = protocol::MutationDiff::apply(outcome.diff(), &base).expect("set-locale applies to its committed before-preferences");
    assert_eq!(applied, expected_after(), "set-locale/sets-locale: the applied preferences differ from the committed after-snapshot");
}

/// 🎯️ The declared outcome is `applied` with no diagnostics: the locale was not set before.
#[test]
fn declared_outcome_holds() {
    let declared: serde_json::Value = serde_json::from_str(OUTCOME).expect("outcome decodes");
    assert_eq!(declared.get("status").and_then(serde_json::Value::as_str), Some("applied"), "set-locale/sets-locale: this fixture declares a applied outcome");
    let produced = <UiPreferencesConfigMutation as protocol::Mutation<UiPreferences>>::diff(&mutation(), &before());
    assert!(produced.messages().is_empty(), "set-locale/sets-locale: a changed preference emits no diagnostics");
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(json_value(outcome.diff()), committed, "set-locale/sets-locale: produced diff differs from the committed 🔺️diff");
    let decoded: UiPreferencesDiff = dsl::os_pack::json::from_json_str(DIFF).expect("committed diff decodes as UiPreferencesDiff");
    assert_eq!(protocol::MutationDiff::apply(&decoded, &before()).expect("committed diff applies"), expected_after(), "set-locale/sets-locale: the committed diff does not carry before to after");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "set-locale/sets-locale: the inverse did not restore the before-preferences");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_value(&decoded), original, "set-locale/sets-locale: committed {label} JSON is not canonical");
    }
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("payload reparses");
    assert_eq!(json_value(&mutation()), original, "set-locale/sets-locale: committed payload JSON is not canonical");
}
