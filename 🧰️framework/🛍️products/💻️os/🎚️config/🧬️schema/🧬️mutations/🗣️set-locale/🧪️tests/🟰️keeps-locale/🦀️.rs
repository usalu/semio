//! 🧪️ `set-locale` fixture — `🟰️keeps-locale`.
//!
//! Setting the locale to the value the preferences already hold is the leaf's `mutation.no-op` guard: a `no-op` outcome whose whole-record diff restates the unchanged record.
//!
...
//! `../../🧫️fixtures/🟰️keeps-locale/`.

use crate::opening_config::mutations::UiPreferencesConfigMutation;
use crate::opening_config::{UiPreferences, UiPreferencesDiff};

const BEFORE: &str = include_str!("../../🧫️fixtures/🟰️keeps-locale/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../🧫️fixtures/🟰️keeps-locale/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../🧫️fixtures/🟰️keeps-locale/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../🧫️fixtures/🟰️keeps-locale/🔺️diff/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    assert_eq!(applied, expected_after(), "set-locale/keeps-locale: the applied preferences differ from the committed after-snapshot");
}

/// 🎯️ The declared outcome is `no-op` with one `warning`-level `mutation.no-op`: the locale already holds the requested value.
#[test]
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "set-locale/keeps-locale: the inverse did not restore the before-preferences");
}
...
fn committed_json_is_canonical() {
    ...
    assert_eq!(json_value(&mutation()), original, "set-locale/keeps-locale: committed payload JSON is not canonical");
}
