...
const BEFORE: &str = include_str!("../../🧫️fixtures/🟰️keeps-driver/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../🧫️fixtures/🟰️keeps-driver/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../🧫️fixtures/🟰️keeps-driver/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../🧫️fixtures/🟰️keeps-driver/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../🧫️fixtures/🟰️keeps-driver/🎯️outcome/🔣️.json");
...
/// ▶️ Re-setting the driver to its current value leaves the preferences exactly as they were.
#[test]
...
/// 🎯️ The declared outcome is `no-op` with one `warning`-level `mutation.no-op`: the driver already holds the requested value.
#[test]
fn declared_outcome_holds() {
    let declared: serde_json::Value = serde_json::from_str(OUTCOME).expect("outcome decodes");
    assert_eq!(declared.get("status").and_then(serde_json::Value::as_str), Some("no-op"), "set-driver/keeps-driver: this fixture declares a no-op outcome");
    let produced = <UiPreferencesConfigMutation as protocol::Mutation<UiPreferences>>::diff(&mutation(), &before());
    assert_eq!(produced.worst_level(), Some(protocol::Severity::Warning), "set-driver/keeps-driver: an unchanged preference is a warned no-op, never a refusal");
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_value(&decoded), original, "set-driver/keeps-driver: committed {label} JSON is not canonical");
    }
