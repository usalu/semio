 ...
 * Bash per the ticket's binding rules. Serves `📞️out-callback/` (the jco-transpiled component + the
 * Web Worker harness) with correct MIME types for `.js`/`.wasm`/`.html`. */
import { existsSync } from "node:fs";
import { join, extname, sep } from "node:path";

const ROOT = join(import.meta.dir, "../../../🧫️fixtures/🧩️jcoprobe/🌐️harness/📞️out-callback");
const PORT = 8846;
...
Bun.serve({
  ...
  async fetch(req) {
    ...
    let path = url.pathname === "../../../../../../../../../.." ? "/🌐️.html" : decodeURIComponent(url.pathname);
