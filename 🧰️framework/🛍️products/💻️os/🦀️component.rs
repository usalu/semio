...
pub mod media_export_raster {
    ...
    use crate::workflow::{register_os_media_export_handler_kind, register_os_media_import_handler_kind, OsMediaExportResult, media_accept_filter_kinds};
    use base64::Engine;
...
pub mod workflow {
    ...
    pub use crate::workflow_kernel::{
        apply_workflow_operation, create_default_workflow_parameter, empty_workflow, empty_workflow_snapshot, patch_workflow_parameter, placeholder_media_contract, plan_workflow, sync_workflow_parameter_ports,
    ...
    /// 🗂️ Defined in `semio_framework`; re-exported here verbatim.
    pub use semio_framework::{MediaClass, MediaForm, MediaType};
    ...
    /// 🗄️ Ticket 26/08/10/STDIO-ARTIFACTS-AND-IO W15: dispatch export via `io_dispatch` (real
    /// subset validation + one-hop fallback guard, ticket 26/08/11/
    /// SEMIO-ARTIFACT-UNIFIED-IMPORT-EXPORT-AND-MEDIA-FORMAT-RETIREMENT W6) hitting the typed
    /// registry's EXPORT-direction entries (`generators/w15_add_export_entries.py`, populated for
    /// all 54 domain artifacts) before falling back to the old stringly handler map.
    /// `source_document` is already-deserialized `Value`, not this artifact's own wire text/binary,
    /// so the source is built in the universal `s.stdio.json` bridge dialect every domain
    /// artifact's generated `rebuild_native_snapshot` accepts (see that function's own doc comment
    /// for why). Returns `None` (not an error) when the registry genuinely has no entry for this
    /// pair, or when composition fails for any reason -- either way the caller falls through to the
    /// pre-existing handler-map path unchanged, so this can only ADD successful dispatches, never
    /// remove one.
    ...
    /// 🗄️ Ticket 26/08/10/STDIO-ARTIFACTS-AND-IO W15: dispatch import via `io_dispatch`,
...
pub use crate::workflow::{
    ...
    workflow_parameter_types_compatible, workflow_parameter_value, MediaContract, OsMediaCapability, OsMediaExportResult, media_accept_filter_kinds, OsWorkflowCamera, OsWorkflowNodeGraphPayload, OsWorkflowOperatorInfo, Workflow, WorkflowDelivery, WorkflowSnapshot,
