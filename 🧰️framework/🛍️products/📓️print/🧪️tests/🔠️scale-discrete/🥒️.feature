...
Feature: Discrete and binned scales place their bands and buckets exactly as d3-scale does
  ...
  Scenario: Band starts, widths and steps follow the padding, alignment and range direction
    Given the committed probe document local://scale-discrete.tex and the band scales
  ...
  Scenario: A point scale is the band scale with an inner padding of one
    Given the committed probe document local://scale-discrete.tex and the point scales
  ...
  Scenario: An ordinal scale cycles its range over its domain
    Given the committed probe document local://scale-discrete.tex and the ordinal scale
      | domain    | range    | inputs    |
  ...
  Scenario: A quantize scale cuts its domain into uniform buckets
    Given the committed probe document local://scale-discrete.tex and the quantize scales
  ...
  Scenario: A quantile scale cuts its domain sample at its R-7 quantiles
    Given the committed probe document local://scale-discrete.tex and the quantile scale
      | domain               | range          | inputs       |
  ...
  Scenario: A threshold scale bisects its explicit cuts
    Given the committed probe document local://scale-discrete.tex and the threshold scale
      | domain | range        | inputs         |
