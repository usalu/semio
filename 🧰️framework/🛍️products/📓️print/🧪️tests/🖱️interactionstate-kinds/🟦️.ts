/** 🖱️ Adapter for `interactionstate-kinds`: the three families of `semio-viz-interactionstate`.
 *
 * Subject: the LaTeX families, probed through `semio-viz-probe` with one scenario per catalogue
 * kind. Oracle: the catalogue itself — d3 has interaction behaviours, not printed depictions of
 * them, so the promise a catalogue entry makes is the specification.
 */
...
//#region 🔖️Vectors
const CASE = "interactionstate-kinds";
const DECIMALS = 6;

/** 🗂️ The three families the §45 catalogue kinds are served from. */
const FAMILIES = ["state-overview", "state-selection", "state-sequence"] as const;
...
export function catalogueProjection(families: readonly string[]): ProbeProjection {
  ...
  for (const family of families) {
    ...
    projection[`state/${family}/kinds`] = variants;
    projection[`state/${family}/drawing`] = [variants.length];
    projection[`state/${family}/distinct`] = [variants.length];
  }
  return projection;
}

/** 🧮️ Folds the probe records into the same three answers, per family. */
export function stateProjection(records: readonly ProbeRecord[], families: readonly string[]): ProbeProjection {
  ...
  for (const family of families) {
    ...
    projection[`state/${family}/kinds`] = drawn;
    projection[`state/${family}/drawing`] = [drawn.length];
    projection[`state/${family}/distinct`] = [new Set(drawn.map((variant) => perFamily.get(variant)!)).size];
  }
  return projection;
}
...
export default defineTestAdapter({
  ...
  scenarios: {
    kinds: {
      /** 🗂️ The catalogue itself: every registered §45 kind of the three state families. */
      oracle: () => ({ projection: catalogueProjection(FAMILIES) }),
      /** 🎯️ Every interaction-state kind drawn once, reduced to presence and pairwise distinctness. */
