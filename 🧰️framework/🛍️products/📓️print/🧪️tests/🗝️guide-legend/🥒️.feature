...
Feature: A legend generates its entries from a scale
  ...
  Scenario: A swatch legend enumerates the scale domain
    Given the committed probe document local://categorical.tex and the ordinal scale
      | domain    |
      | A,B,C,D,E |
  ...
  Scenario: A vertical swatch legend stacks its entries by swatch and gap
    Given the committed probe document local://categorical.tex with at=4,36 swatchSize=2.6 itemGap=1.2
  ...
  Scenario: A size legend takes each circle's radius from the scale
    Given the committed probe document local://size.tex with ticks=3 over the linear scale 0..10 mapped to 0..5
