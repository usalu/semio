...
Feature: The stack transform produces the baselines d3-shape's stack produces
  ...
  Scenario: Without an order the series stack in the order they appear
    Given the committed probe document shared://🥞️transform-stack/transform-stack.tex and the series
      | series | values                |
  ...
  Scenario: The ascending order stacks the smallest total first
    Given the committed probe document shared://🥞️transform-stack/transform-stack.tex and the same series
      | order     |
      | ascending |
  ...
  Scenario: The diverging offset stacks negative values below the axis
    Given the committed probe document shared://🥞️transform-stack/transform-stack.tex and the signed series
      | series | values  |
      | up     | 3,4,2   |
      | down   | -2,-5,-1|
      | mixed  | 1,-3,4  |
