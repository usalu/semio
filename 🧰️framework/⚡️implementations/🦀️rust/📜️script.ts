...
function generatedManifestPath(root: string): string {
  return join(root, "..", "🟦️typescript", "generated", "manifest.ts");
}
...
function consolidateBindings(dir: string): string {
  ...
  return `/** @generated by \`bun nx run @semio-tech/framework-core:generate\` from framework/⚡️implementations/🦀️rust/📦️lib.rs via ts-rs. Do not edit. */\n\n${blocks.join("\n\n")}\n`;
}
...
class GenerateScript extends BundleScript {
  run(_segments: string[]): void {
    ...
    mkdirSync(join(this.root, "..", "🟦️typescript", "generated"), { recursive: true });
    writeFileSync(outPath, manifest);
    console.log(`framework-core typescript mirror refreshed -> ${outPath}`);
  }
}

/** 🔎️ Rebuilds the manifest in memory and byte-compares it against `generated/manifest.ts` — never writes that file (a lint/verify step must never let the auto-commit daemon land regenerated files). */
