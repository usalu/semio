#!/usr/bin/env bun
/** 🏘️ `@semio-tech/math-graph-community` — Community detection: Louvain, Leiden, label propagation, Girvan-Newman, and partition-quality metrics. */
...
class TestScript extends BundleScript {
  run(segments: string[]): void {
    ...
    runCargoTestBudgeted(["mathematical_graph_community"], join(this.root, "rs"), rest);
  }
}

class LintScript extends BundleScript {
  run(): void {
    runCargoLint(["mathematical_graph_community"], join(this.root, "rs"));
  }
}
