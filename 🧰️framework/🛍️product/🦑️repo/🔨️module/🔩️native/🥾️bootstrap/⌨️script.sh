...
ensure_java_runtime() {
  ...
  if [ "$COMPOSE_SESSION_START" = "1" ]; then
    log "Java 21+ is required for Neo4j. Run bash ./⌨️script.sh setup to install it."
    return 1
  fi
  case "$(uname -s)" in
  Darwin)
...
ensure_cpp_toolchain() {
  ...
  if command -v bun >/dev/null 2>&1; then
    (cd "$REPO_ROOT" && bun ./📜️script.ts cpp setup) || log "C++ vcpkg bootstrap via bun ./📜️script.ts cpp setup failed."
  fi
}
#endregion 🔖️EnsureCpp
