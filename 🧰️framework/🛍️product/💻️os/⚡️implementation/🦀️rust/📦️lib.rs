...
use store::{create_document_envelope, document_backbone_ref, materialize_document_projection, DocumentBackboneRef, DocumentCommand, DocumentEnvelope, DocumentStore, SpaceConflict};
    ...
    pub enum OsOperation {
        ...
        /// 🆔️ `node.id` is minted once at dispatch time (`OsStore::add_workflow_node`) and carried in
        /// the node itself so replay never re-mints it — see `apply_os_operation`'s `AddWorkflowNode`
        /// arm. `node` is minted FULLY (ports included) via `workflow::workflow_node_for_app`.
        AddWorkflowNode {
    ...
    pub fn default_os_projection() -> OsProjection {
        OsProjection { programs: Vec::new(), active_plugin_id: None, active_alternative_id: None, workflow: workflow::empty_workflow(), parameters: Vec::new(), parameter_bindings: Vec::new() }
    }
    ...
    pub fn apply_os_operation(projection: &OsProjection, operation: &OsOperation) -> OsProjection {
        ...
        match operation {
            ...
            OsOperation::AddWorkflowNode { node } => {
                if !next.programs.contains(&node.plugin_id) {
                    next.programs.push(node.plugin_id.clone());
                }
                let node = sync_workflow_node_parameter_ports(node, &next.parameter_bindings);
                next.workflow.nodes.push(node);
            }
            OsOperation::RemoveWorkflowNode { node_id } => {
                next.parameter_bindings.retain(|binding| binding.node_id != *node_id);
                next.workflow.nodes.retain(|node| node.id != *node_id);
            ...
            OsOperation::PatchWorkflowNode { node_id, label } => {
                for node in &mut next.workflow.nodes {
                    if node.id == *node_id {
    ...
    fn sync_workflow_node_parameter_ports(node: &workflow::WorkflowNode, bindings: &[OsParameterFieldBinding]) -> workflow::WorkflowNode {
        sync_workflow_parameter_ports(&workflow::Workflow { schema: workflow::WORKFLOW_SCHEMA.into(), nodes: vec![node.clone()], edges: Vec::new() }, bindings).nodes.into_iter().next().unwrap_or_else(|| node.clone())
    }
    ...
    impl OperationDiff<OsProjection> for OsDiff {
        fn apply(&self, projection: &OsProjection) -> OsProjection {
            let operation = match self {
                ...
                OsDiff::AddWorkflowNode { node } => OsOperation::AddWorkflowNode { node: node.clone() },
                OsDiff::RemoveWorkflowNode { node_id } => OsOperation::RemoveWorkflowNode { node_id: node_id.clone() },
                ...
                OsDiff::PatchWorkflowNode { node_id, label } => OsOperation::PatchWorkflowNode { node_id: node_id.clone(), label: label.clone() },
                ...
                OsDiff::UnbindParameterField { node_id, field_path } => OsOperation::UnbindParameterField { node_id: node_id.clone(), field_path: field_path.clone() },
                OsDiff::SyncNodePorts => OsOperation::SyncNodePorts,
            };
    ...
    impl Operation<OsProjection> for OsOperation {
        ...
        fn diff(&self, _projection: &OsProjection) -> OsDiff {
            match self {
                ...
                OsOperation::AddWorkflowNode { node } => OsDiff::AddWorkflowNode { node: node.clone() },
                OsOperation::RemoveWorkflowNode { node_id } => OsDiff::RemoveWorkflowNode { node_id: node_id.clone() },
                ...
                OsOperation::PatchWorkflowNode { node_id, label } => OsDiff::PatchWorkflowNode { node_id: node_id.clone(), label: label.clone() },
                ...
                OsOperation::UnbindParameterField { node_id, field_path } => OsDiff::UnbindParameterField { node_id: node_id.clone(), field_path: field_path.clone() },
                OsOperation::SyncNodePorts => OsDiff::SyncNodePorts,
            }
        }
        ...
        fn backwards(&self, projection: &OsProjection) -> Vec<Self> {
            match self {
                ...
                OsOperation::AddWorkflowNode { node } => vec![OsOperation::RemoveWorkflowNode { node_id: node.id.clone() }],
                OsOperation::RemoveWorkflowNode { node_id } => projection.workflow.nodes.iter().find(|node| node.id == *node_id).map(|node| vec![OsOperation::AddWorkflowNode { node: node.clone() }]).unwrap_or_default(),
                ...
                OsOperation::PatchWorkflowNode { node_id, .. } => {
                ...
                OsOperation::BindParameterField { binding } => vec![OsOperation::UnbindParameterField { node_id: binding.node_id.clone(), field_path: binding.field_path.clone() }],
                OsOperation::UnbindParameterField { node_id, field_path } => {
                    projection.parameter_bindings.iter().find(|binding| binding.node_id == *node_id && binding.field_path == *field_path).map(|binding| vec![OsOperation::BindParameterField { binding: binding.clone() }]).unwrap_or_default()
                }
                OsOperation::SyncNodePorts => Vec::new(),
            }
        }
    ...
    fn reconcile_os_workflow(mut projection: OsProjection) -> (OsProjection, Vec<SpaceConflict>) {
        ...
        let node_by_id: HashMap<&str, &workflow::WorkflowNode> = projection.workflow.nodes.iter().map(|node| (node.id.as_str(), node)).collect();
        ...
        // 🤝️ Re-negotiates each edge's stored `contract` against its live endpoints via
        // `workflow::negotiate_media_contract` — a concurrent re-typing (or a stale import) that no
        // longer negotiates the same contract drops the edge instead of silently carrying a stale wire.
        edges.retain(|edge| {
        ...
        let nodes = projection.workflow.nodes.clone();
    ...
    fn os_operation_to_dsl(operation: &OsOperation) -> OsOperationDsl {
        match operation {
            ...
            OsOperation::AddWorkflowNode { node } => OsOperationDsl::AddWorkflowNode { node: node.clone() },
            OsOperation::RemoveWorkflowNode { node_id } => OsOperationDsl::RemoveWorkflowNode { node_id: node_id.clone() },
            ...
            OsOperation::PatchWorkflowNode { node_id, label } => OsOperationDsl::PatchWorkflowNode { node_id: node_id.clone(), label: label.clone() },
            ...
            OsOperation::UnbindParameterField { node_id, field_path } => OsOperationDsl::UnbindParameterField { node_id: node_id.clone(), field_path: field_path.clone() },
            OsOperation::SyncNodePorts => OsOperationDsl::SyncNodePorts,
        }
    }
    ...
    fn os_operation_from_dsl(operation: OsOperationDsl) -> OsOperation {
        match operation {
            ...
            OsOperationDsl::AddWorkflowNode { node } => OsOperation::AddWorkflowNode { node },
            OsOperationDsl::RemoveWorkflowNode { node_id } => OsOperation::RemoveWorkflowNode { node_id },
            ...
            OsOperationDsl::PatchWorkflowNode { node_id, label } => OsOperation::PatchWorkflowNode { node_id, label },
            ...
            OsOperationDsl::UnbindParameterField { node_id, field_path } => OsOperation::UnbindParameterField { node_id, field_path },
            OsOperationDsl::SyncNodePorts => OsOperation::SyncNodePorts,
        }
    }
    ...
    impl OsStore {
        ...
        /// `workflow::workflow_node_for_app`, at dispatch time, so replay never re-derives it. Returns
        /// the node's own id, which IS the app-instance identity now (see the kernel `🔁️workflow`
        /// crate's `🔖️InstanceIdentity` doc).
        pub fn add_workflow_node(&mut self, plugin_id: &str, app_id: &str, label: Option<&str>, x: f64, y: f64) -> Result<String, VcsError> {
            ...
            self.dispatch_apply(vec![OsOperation::AddWorkflowNode { node }])?;
            Ok(node_id)
        }
    ...
    fn os_space_catalog_entry_from_document(backbone_uri: &str, document: &OsDocument) -> Result<OsSpaceCatalogEntry, VcsError> {
        ...
        Ok(OsSpaceCatalogEntry { id: space_id, name: document.name.clone(), backbone_uri: backbone_uri.into(), app_count: projection.workflow.nodes.len(), node_count: projection.workflow.nodes.len(), updated_at })
    }
    ...
    mod tests {
        ...
        fn spawns_and_removes_app_instances() {
            ...
            store.add_workflow_node("draw", "draw", None, 40.0, 40.0).expect("spawn");
            assert_eq!(store.projection().expect("projection").workflow.nodes.len(), 1);
            store.dispatch_text("undo").expect("undo");
            assert_eq!(store.projection().expect("projection").workflow.nodes.len(), 0);
        }
        ...
        fn concurrent_delete_and_wire_reconciles_without_a_dangling_edge() {
            ...
            let node_a_id = store_a.add_workflow_node("draw", "draw", None, 0.0, 0.0).expect("spawn a");
            let node_b_id = store_a.add_workflow_node("sink", "sink", None, 200.0, 0.0).expect("spawn b");
            ...
            let node_a = projection.workflow.nodes.iter().find(|node| node.id == node_a_id).expect("node a");
            let node_b = projection.workflow.nodes.iter().find(|node| node.id == node_b_id).expect("node b");
            ...
            store_a.dispatch_apply(vec![OsOperation::RemoveWorkflowNode { node_id: node_b_id.clone() }]).expect("remove node b");
            store_b
            ...
            assert!(converged_a.workflow.nodes.iter().all(|node| node.id != node_b_id), "node b must stay removed");
        ...
        /// 🧵️ A representative `OsProjection` exercising every collection: two workflow nodes (node
        /// absorbs instance now — one with an input port, one with an output port) wired by one edge,
        /// one of each `OsParameter` variant, and one parameter binding — so the DSL round trip actually
        /// covers the workflow encoding, not just an empty-projection fixpoint.
        fn sample_os_projection() -> OsProjection {
            ...
            OsProjection {
                ...
                workflow: workflow::Workflow { schema: workflow::WORKFLOW_SCHEMA.into(), nodes: vec![node_a, node_b], edges: vec![edge] },
                parameters: vec![
                ...
                parameter_bindings: vec![OsParameterFieldBinding { parameter_id: "p1".into(), node_id: "app-1".into(), field_path: "/zoom".into() }],
            }
        }
        ...
        fn op_text_round_trips_add_workflow_node() {
            store::test_support::assert_op_line_round_trip(&OsOperation::AddWorkflowNode {
                node: workflow::WorkflowNode {
                    id: "node-1".into(),
                    plugin_id: "puzzle".into(),
                    app_id: "puzzle2d".into(),
        ...
        fn op_text_round_trips_remove_workflow_node() {
            store::test_support::assert_op_line_round_trip(&OsOperation::RemoveWorkflowNode { node_id: "app-1".into() });
        }
        ...
        fn op_text_round_trips_patch_workflow_node() {
            store::test_support::assert_op_line_round_trip(&OsOperation::PatchWorkflowNode { node_id: "app-1".into(), label: "Renamed \"Board\"".into() });
        }
        ...
        fn op_text_round_trips_bind_parameter_field() {
            store::test_support::assert_op_line_round_trip(&OsOperation::BindParameterField { binding: OsParameterFieldBinding { parameter_id: "p1".into(), node_id: "app-1".into(), field_path: "/zoom".into() } });
        }

        #[test]
        fn op_text_round_trips_unbind_parameter_field() {
            store::test_support::assert_op_line_round_trip(&OsOperation::UnbindParameterField { node_id: "app-1".into(), field_path: "/zoom".into() });
        }

        #[test]
        fn op_text_round_trips_sync_node_ports() {
            store::test_support::assert_op_line_round_trip(&OsOperation::SyncNodePorts);
        }
...
pub mod workflow {
    ...
    use crate::instance::{create_os_id, is_parameter_port_id, media_port_spec_id, parameter_id_from_port_id, parameter_port_id, OsParameter, OsParameterFieldBinding};
    use crate::registry::{os_app_registration, os_artifact_descriptor, OsAppRegistration, OsArtifactDescriptor};
    ...
    /// through this).
    fn sync_workflow_node_parameter_ports(node: &WorkflowNode, bindings: &[OsParameterFieldBinding]) -> WorkflowNode {
        ...
        let parameter_inputs: Vec<_> = node_bindings
            ...
            .map(|binding| WorkflowMediaPort {
                id: parameter_port_id(&node.id, &binding.parameter_id),
                spec: semio_framework_core::MediaPortSpec {
                    id: format!("param.{}", binding.parameter_id),
                    label: "Parameter".into(),
                    direction: semio_framework_core::MediaPortDirection::In,
                    media_type: semio_framework_core::MediaType { class: semio_framework_core::MediaClass::Data, form: semio_framework_core::MediaForm::Value },
    ...
    pub fn sync_workflow_parameter_ports(graph: &Workflow, bindings: &[OsParameterFieldBinding]) -> Workflow {
    ...
    pub fn apply_flow_fixture_to_os_workflow(graph: &Workflow, fixture_json: &str) -> Vec<OsOperation> {
        ...
        if let Some(widgets) = fixture.get("widgets").and_then(Value::as_array) {
            ...
            for node in &graph.nodes {
                if !widget_ids.contains(node.id.as_str()) {
                    ...
                    operations.push(OsOperation::RemoveWorkflowNode { node_id: node.id.clone() });
                }
            }
        }
        ...
        for synapse in &synapses {
            ...
            operations.push(OsOperation::ConnectWorkflowPorts { edge: WorkflowEdge { id, source_node_id, source_port_id, target_node_id, target_port_id, contract } });
        }
    ...
    /// from a separate `OsAppInstance` join).
    pub fn build_os_workflow_operator_infos(graph: &Workflow, parameters: &[OsParameter]) -> Vec<OsWorkflowOperatorInfo> {
    ...
    pub fn list_os_workflow_vfs_children(parent_id: &str, graph: &Workflow, bindings: &[OsParameterFieldBinding], parameters: &[OsParameter]) -> Vec<OsWorkflowVfsNodeRecord> {
            ...
            return graph
                ...
                .map(|node| {
                    ...
                    OsWorkflowVfsNodeRecord {
                        id: os_workflow_vfs_instance_folder_id(&node.id),
                        file_node_kind_id: "instance".into(),
                        name: format!("{} ({}.{}))", node.label, node.plugin_id, node.app_id),
                        path: format!("/{}", node.label),
                        parent_id: Some(OS_WORKFLOW_VFS_ROOT_ID.into()),
                        has_children: true,
                        icon: registration.as_ref().map(|entry| entry.component_kind.clone()),
                        navigate_uri: None,
                        descriptor_values: HashMap::from([("binding".into(), node.yields.clone())]),
                    }
                })
                .collect();
        }
        let Some(node_id) = os_workflow_vfs_instance_id(parent_id) else {
            return Vec::new();
        };
        let Some(node) = graph.nodes.iter().find(|entry| entry.id == node_id) else {
            return Vec::new();
        };
        let registration = os_app_registration(&node.plugin_id, &node.app_id);
        if parent_id == os_workflow_vfs_instance_folder_id(&node_id) {
            return vec![
                OsWorkflowVfsNodeRecord {
                    id: os_workflow_vfs_source_id(&node_id),
                    file_node_kind_id: "source".into(),
                    name: "source.json".into(),
                    path: format!("/{}/source.json", node.label),
                    parent_id: Some(parent_id.into()),
                    has_children: false,
                    icon: Some("json".into()),
                    navigate_uri: Some(format!("os://instance/{}", node.id)),
                    descriptor_values: HashMap::from([("binding".into(), registration.as_ref().map(|entry| entry.source_format.clone()).unwrap_or_else(|| node.yields.clone()))]),
                },
                OsWorkflowVfsNodeRecord {
                    id: os_workflow_vfs_inputs_folder_id(&node_id),
                    file_node_kind_id: "folder".into(),
                    name: "inputs".into(),
                    path: format!("/{}/inputs", node.label),
                    parent_id: Some(parent_id.into()),
                    has_children: registration.as_ref().map(|entry| !entry.inputs.is_empty()).unwrap_or(false) || bindings.iter().any(|binding| binding.node_id == node_id),
                    icon: Some("folder-input".into()),
                    navigate_uri: None,
                    descriptor_values: HashMap::new(),
                },
                OsWorkflowVfsNodeRecord {
                    id: os_workflow_vfs_outputs_folder_id(&node_id),
                    file_node_kind_id: "folder".into(),
                    name: "outputs".into(),
                    path: format!("/{}/outputs", node.label),
                    parent_id: Some(parent_id.into()),
        ...
        if parent_id == os_workflow_vfs_inputs_folder_id(&node_id) {
            let mut rows = Vec::new();
            if let Some(registration) = registration.as_ref() {
                for spec in &registration.inputs {
                    rows.push(OsWorkflowVfsNodeRecord {
                        id: os_workflow_vfs_input_port_id(&node_id, &spec.id),
                        file_node_kind_id: "input".into(),
                        name: spec.id.clone(),
                        path: format!("/{}/inputs/{}", node.label, spec.id),
                        parent_id: Some(parent_id.into()),
                        has_children: false,
                        icon: Some("plug".into()),
                        navigate_uri: None,
                        descriptor_values: HashMap::from([("binding".into(), spec.kind_id.clone().unwrap_or_default())]),
                    });
                }
            }
            for binding in bindings.iter().filter(|entry| entry.node_id == node_id) {
            ...
            let descriptor = os_artifact_descriptor(&node.yields);
            for format in required_os_media_import_formats(&descriptor.dimension, os_resource_media_capability(&descriptor.kind)) {
                let ext = format.as_str();
                rows.push(OsWorkflowVfsNodeRecord {
                    id: os_workflow_vfs_import_id(&node_id, &format),
                    file_node_kind_id: "import".into(),
                    name: format!("import.{ext}"),
                    path: format!("/{}/inputs/import.{ext}", node.label),
                    parent_id: Some(parent_id.into()),
                    has_children: false,
                    icon: Some(ext.into()),
                    navigate_uri: Some(format!("os://import/{}/{}/{}", node.id, descriptor.kind, format.as_str())),
                    descriptor_values: HashMap::from([("format".into(), format.as_str().into())]),
                });
            }
            return rows;
        }
        if parent_id == os_workflow_vfs_outputs_folder_id(&node_id) {
            let descriptor = os_artifact_descriptor(&node.yields);
            ...
            if let Some(registration) = registration.as_ref() {
                for spec in &registration.outputs {
                    for format in &formats {
                        ...
                        rows.push(OsWorkflowVfsNodeRecord {
                            id: os_workflow_vfs_export_id(&node_id, &spec.id, format),
                            file_node_kind_id: "export".into(),
                            name: format!("{}.{}", spec.id, ext),
                            path: format!("/{}/outputs/{}.{}", node.label, spec.id, ext),
                            parent_id: Some(parent_id.into()),
                            has_children: false,
                            icon: Some(ext.into()),
                            navigate_uri: Some(format!("os://export/{}/{}/{}", node.id, spec.id, format.as_str())),
                            descriptor_values: HashMap::from([("binding".into(), spec.kind_id.clone().unwrap_or_default()), ("format".into(), format.as_str().into())]),
                        });
                    }
                }
            }
    ...
    mod tests {
        ...
        fn vfs_inputs_folder_lists_a_dwg_import_row_for_2d_kinds() {
            ...
            let mut graph = empty_workflow();
            graph.nodes.push(media_node("node-vfs-1", 0.0, 0.0));
            let inputs_folder = os_workflow_vfs_inputs_folder_id("node-vfs-1");
            let rows = list_os_workflow_vfs_children(&inputs_folder, &graph, &[], &[]);
            ...
            assert_eq!(import_row.navigate_uri, Some("os://import/node-vfs-1/2d.drawing/dwg".to_string()));
        }
        ...
        fn flow_fixture_diff_connects_disconnects_and_removes() {
            ...
            assert!(matches!(
                &operations[0],
                OsOperation::ConnectWorkflowPorts { edge } if edge.source_node_id == "node-2" && edge.target_port_id == "node-1:in" && !edge.id.is_empty()
            ));
            ...
            assert!(removal_operations.contains(&OsOperation::RemoveWorkflowNode { node_id: "node-2".into() }));
