//! 🧬️ `dsl_editor` — the token-transaction editor layer: undo/redo history over `dsl_token`'s
...
//#region 🔖️Document
/// @emoji 📝️ One undo/redo step: a group of inverse transactions applied together (a coalesced
...
impl EditorDocument {
    ...
    /// @emoji ⚡️ Applies `transaction` as its own undo step (equivalent to `apply_coalesced` with
    /// `coalesce_key: None`).
    pub fn apply(&mut self, transaction: &TokenTransaction) -> Result<(), TransactionError> {
        self.apply_coalesced(transaction, None)
    }

    /// @emoji 🪢️ Applies `transaction`. If `coalesce_key` is `Some` and matches the key of the
...
/// @emoji 🧭️ Finds the token index matching the delimiter at `index` (its closer if `index` is an
...
//#region 🔖️Completion
/// @emoji 💡️ Turns a `LanguageService` completion choice into a `TokenTransaction` that inserts
...
/// @emoji 💡️ Convenience: builds a `LanguageService` for `spec` and returns its completions at
