...
/// the label-adjacent dash-fusion issue `dsl_notation::print_edge` documents.
...
fn node_error(message: &str, tokens: &[dsl_core::SpannedToken], pos: usize) -> TextError {
    let span = tokens.get(pos).or_else(|| tokens.last()).map(|t| t.span).unwrap_or(dsl_core::TextSpan::at(1, 1));
    TextError::new(message.to_string(), span)
}

fn parse_node(tokens: &[dsl_core::SpannedToken], mut pos: usize) -> Result<(EdgeNode, usize), TextError> {
