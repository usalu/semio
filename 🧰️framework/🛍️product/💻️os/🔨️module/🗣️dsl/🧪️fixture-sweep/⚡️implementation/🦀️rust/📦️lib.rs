...
//! real shipped `📚️example/**` fixture file across every plugin/app that derives
...
//! `🔨️module/🗣️dsl` (or core) crate purely as a `[dev-dependencies]` fan-in so this ONE `cargo
...
//! lives (plugin-root `📚️example/`, a nested per-app `⚡️implementation/🦀️rust/📚️example/`, or a
...
mod tests {
    ...
    fn registry() -> Vec<(&'static str, &'static str, CheckFn)> {
        vec![
            ...
            ("semio_framework_os", <OsProjection as store::DocumentDsl>::EXTENSION, store::test_support::check_dsl_fixture_text_laws::<OsProjection>),
    ...
    /// @emoji 📚️ Recursively finds every directory literally named `📚️example` under `root`,
    ...
    /// nest one level, e.g. `norm/📚️example/📘️en1990/...`).
    ...
    fn repo_wide_dsl_fixture_law_sweep() {
        ...
        assert!(!dirs.is_empty(), "found zero 📚️example directories under {root:?} — sweep would vacuously pass");

        let mut fixture_files = Vec::new();
        for dir in &dirs {
            collect_files(dir, &mut fixture_files);
        }
        assert!(!fixture_files.is_empty(), "found {} 📚️example dir(s) but zero fixture files under {root:?}", dirs.len());
