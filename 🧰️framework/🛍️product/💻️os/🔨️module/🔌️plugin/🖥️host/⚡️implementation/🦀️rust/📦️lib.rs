...
dsl_core::fault_from_thiserror!(PluginHostError, dsl_core::FaultOrigin::Os, "os.plugin-host");
//#endregion ⚠️ Errors

fn host_fault_bytes(code: &str, message: impl Into<String>) -> Vec<u8> {
    dsl_core::encode_fault_bytes(&dsl_core::Fault::new(dsl_core::FaultOrigin::Os, code, message))
}
...
impl semio::framework::host::Host for HostState {
    ...
    fn read_asset(&mut self, handle: u64) -> Result<Vec<u8>, Vec<u8>> {
        match handle {
            ...
            _ => Err(host_fault_bytes("os.host.read-asset", format!("read-asset: unknown handle {handle}"))),
        }
    }
