...
/** @emoji 🗂️ Emits `PLUGIN_WASM_ARTIFACTS`: each plugin crate's id paired with the relative path its
 * built `.wasm` lands at under `framework/os/dev/js/🔌️plugin-modules/` (see `buildPlugin` in
 * `framework/os/dev/ts/script.ts`, which writes `join(pluginOutRoot, target.pluginId, target.wasmOut)`
 * — `pluginOutRoot` is that same `🔌️plugin-modules` dir). Headless Rust runners (`framework/os/run`)
 * need this mapping to locate a plugin's compiled artifact without re-deriving the dev build script's
 * output-path convention themselves. */
function emitRustArtifacts(entries: PluginRegistryEntry[]): string {
  const rows = entries.map((entry) => `    (${JSON.stringify(entry.pluginId)}, ${JSON.stringify(`🔌️plugin-modules/${entry.pluginId}/${entry.wasmOut}`)}),`).join("\n");
  return `// @generated by framework/plugin/registry/script.ts — do not edit.
