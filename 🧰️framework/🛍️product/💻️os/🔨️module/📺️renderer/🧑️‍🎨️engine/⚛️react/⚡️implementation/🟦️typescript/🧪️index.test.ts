...
describe("declarative forms parity", () => {
  ...
  it("dispatches the tree drop action with payload, target and position", async () => {
    const { declarativeTreeDragController } = await import("../../../../🧑️‍🎨️engine/⚛️react/⚡️implementation/🟦️typescript/📦️index.tsx");
...
describe("registry-derived utilities and activation (P5)", () => {
  ...
  it("panelTabDefinitionToNode maps the framework-injected History panel tab through its rendered body", () => {
    ...
    // by `AppBuilder::build_definition` (see `🧰️framework/🛍️product/💻️os/🔨️module/🔌️plugin`).
