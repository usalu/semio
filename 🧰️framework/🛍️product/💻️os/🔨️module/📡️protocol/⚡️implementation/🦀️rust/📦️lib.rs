...
pub fn extract_range<'a>(protocol_bytes: &'a [u8], ordinals: std::ops::Range<u64>) -> Result<RecordSlice<'a>, ProtocolError> {
    ...
    let recovery = protocol_format::recover(&protocol_bytes, &ProtocolLimits::default(), RecoveryMode::LastCommit)?;
    let trusted = &protocol_bytes[..recovery.bytes_recovered as usize];

    let mut cursor = FrameCursor::new(trusted, protocol_format::HEADER_SIZE as u64);
...
fn slice_content_chain(slice: &[u8]) -> Result<[u8; 32], ProtocolError> {
    let hasher = protocol_format::Blake3Hasher;
...
pub fn content_frontier(protocol_bytes: &[u8]) -> Result<FrontierSummary, ProtocolError> {
    ...
    let chain_hash = if recovery.last_commit_seq == 0 {
        protocol_format::Blake3Hasher.hash(&protocol_bytes[..protocol_format::HEADER_SIZE])
    } else {
        ...
        protocol_format::parse_commit_payload(frame.payload())?.chain_hash
    };
...
mod tests {
    ...
    fn extract_range_returns_contiguous_slice_covering_requested_edits() {
        ...
        let mut cursor = FrameCursor::new(&bytes, protocol_format::HEADER_SIZE as u64);
    ...
    fn content_frontier_reports_head_edit_and_chain_tip() {
        ...
        let mut reverse = ReverseFrameCursor::at_end(&bytes[protocol_format::HEADER_SIZE..]);
        ...
        let expected = protocol_format::parse_commit_payload(last_commit.payload()).unwrap();
