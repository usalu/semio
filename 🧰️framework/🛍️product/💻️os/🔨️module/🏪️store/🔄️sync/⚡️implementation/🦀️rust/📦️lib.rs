...
pub enum DocumentEvent {
    ...
    /// @emoji 👻️ A peer published an ephemeral preview blob (`protocol_wire::ServerFrame::Preview`)
...
/// @emoji 🆔️ One `HistoryEdit`'s op ids, matching `protocol::operation_envelope_from_edit`'s own
...
/// @emoji 📡️ `PresencePeer` -> the binary blob `protocol_wire::ClientFrame::Presence` carries
...
/// @emoji ⏰️ Advances `counter` and stamps a fresh {@link protocol::HybridLogicalTimestamp} for an
...
mod tests {
    ...
    fn wire_fixtures_stay_byte_identical_across_rust_and_ts() {
        ...
        write_server(&fixtures_dir, "📦️server-preview.bin", &ServerFrame::Preview { actor: protocol::ActorId("actor-1".to_string()), key: "cursor".to_string(), seq: 3, payload: vec![5, 6] }, Lane::Preview);
