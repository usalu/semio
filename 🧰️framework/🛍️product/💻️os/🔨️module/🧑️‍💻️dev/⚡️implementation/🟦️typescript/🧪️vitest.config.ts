/** 🧪️ Vitest config for `@semio-tech/framework-os-dev` — no in-source unit tests live in this
 * bundle root today (its `js/index.ts` boot entry is guarded by `!import.meta.vitest` rather than
 * carrying a test block), so this exists purely so `bun ./📜️script.ts test` (→ `runVitest`) has a
