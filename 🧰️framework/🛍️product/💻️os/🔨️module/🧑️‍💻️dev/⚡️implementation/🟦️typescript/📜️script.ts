 ...
 * parent (the main thread) which relays it into `🟦️backbone-🟦️worker.ts` (the real sync actor), and
...
function pluginWatchRoot(target: PluginRegistryEntry): string {
  ...
  // 🏛️ Post-restructure: sibling crate families live under `✏️s/🔌️plugin/<p>/...` (was `s/plugin/<p>/...`).
...
export async function buildEngineWasm(variant: string, renderer: string): Promise<void> {
  ...
  const graphScript = join(repoRoot, "./🧰️framework/🔨️module/🗺️surface/🕸️node-graph/⚡️implementation/🦀️rust/📜️script.ts");
  ...
  for (const engineCratePath of row?.engines ?? []) {
    if (engineCratePath === "./🧰️framework/🛍️product/💻️os/🔨️module/🌊️flow/🫀️core/⚡️implementation/🦀️rust" || engineCratePath === "./🧰️framework/🛍️product/💻️os/🔨️module/🌊️flow/⚡️implementation/🟦️typescript/🫀️core") continue;
...
class DevScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    ...
    if (renderer === "wgpu") {
      ...
      const wgpuScript = join(repoRoot, "./🧰️framework/🛍️product/💻️os/🔨️module/📺️renderer/🧑️‍🎨️engine/🧊️wgpu/⚡️implementation/🦀️rust/📜️script.ts");
      const serveStatus = runCmdStatus("bun", [wgpuScript, "serve"], {
        cwd: join(repoRoot, "./🧰️framework/🛍️product/💻️os/🔨️module/📺️renderer/🧑️‍🎨️engine/🧊️wgpu/⚡️implementation/🦀️rust"),
        env: {
          ...process.env,
          SEMIO_PLUGIN: plugin,
...
async function startParityDevServer(renderer: ParityRenderer, variant: string, port: number): Promise<ParityServerHandle> {
  const devScript = join(repoRoot, "./🧰️framework/🛍️product/💻️os/🔨️module/🧑️‍💻️dev/⚡️implementation/🟦️typescript/📜️script.ts");
  const logPath = join(parityOutDir(), `boot-${renderer}-${variant}.log`);
  const logStream = createWriteStream(logPath);
  const daemon = spawnDaemon("bun", [devScript, "dev"], {
    cwd: join(repoRoot, "./🧰️framework/🛍️product/💻️os/🔨️module/🧑️‍💻️dev/⚡️implementation/🟦️typescript"),
