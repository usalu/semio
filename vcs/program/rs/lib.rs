//! 🗂️ VCS program — declarative version-control play app bundled as a hot-swappable WASM program.

use semio_framework_program::{SurfaceKind,
    ...
    ArtifactKindSpec, UiButtonNode, UiFieldNode, UiInputNode, UiNode, UiPresence, UiStackNode,
...
fn register_vcs_exports() {
    ...
    semio_framework_program::program_runtime::register_document_codec_for_app::<VcsPlayApp>(VCS_DEMO_SCHEMA);
}
...
mod tests {
    ...
    /// 📦 Parses `document_pack()` (the full envelope) for tests that need to inspect raw
