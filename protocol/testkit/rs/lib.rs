...
/// 🎲 Deterministic seeded splitmix64 state, advanced on every draw — see
...
/// 🪪 A guaranteed-unique identifier (`"{prefix}-{index}"`, optionally adversarial-unicode-suffixed)
...
impl HistoryLogGen {
    ...
    /// 🌱 Fabricates a `HistoryLog` matching `profile`: a doc identity, `edit_count` edits (each
...
impl OpDagGen {
    ...
    /// 🌱 Generates `node_count` envelopes `"op-0".."op-{node_count-1}"`, each depending on 0..=2
...
/// ✅ LAW: `decode_history(&encode_history(log, _)) == log`.
...
/// ✅ LAW: `encode_history` is byte-for-byte deterministic — encoding the same `HistoryLog` twice
...
/// ✅ LAW: `parse_ops_text -> encode -> decode -> print_ops_text` is a fixpoint — reprinting the
...
/// ✅ LAW: every `RecordFrame::payload()` returned while sweeping `bytes` borrows zero-copy from
/// `bytes` itself.
...
/// ✅ LAW (softened to match the family's actual recovery-then-verify layering, see below): a
...
/// 📐 Up to `cap` roughly-evenly-spaced indices from `[floor, total)` — shared sampling core for
...
/// 📐 Up to `cap` roughly-evenly-spaced truncation lengths in `[0, total]`, always including `0`
/// and `total`.
...
/// ✅ LAW: truncating `bytes` at every sampled length (density per `level`) then running
...
/// 🧮 The `(kind, payload bytes)` multiset of every structural record (`REC_DOC`/`REC_EDIT`/
...
/// ✅ LAW: `protocol::compact` is physically-rewriting-but-identity-preserving — the
...
/// ✅ LAW: `OpText::print_op` output never contains `'\n'`, and `parse_op(op.print_op())` recovers
/// an equal operation.
...
/// ✅ LAW: `OpDag` converges to the same fully-applied set regardless of insertion order, for
...
/// ✅ LAW: `merge_concurrent_diffs(strategy, a, b, ..) == merge_concurrent_diffs(strategy, b, a, ..)`.
...
/// ✅ LAW: `merge_concurrent_diffs(strategy, a, a, ..) == a`.
...
/// ✅ LAW: `decode(encode(frame)) == frame`, for either wire direction.
