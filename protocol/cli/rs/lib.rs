...
/// `protocol_format::HEADER_SIZE`. `inspect`'s header dump and record-kind walk are the only
...
// Record-kind byte table, mirrored from `protocol_core::REC_*` (frozen contract values, `##
// protocol_core`, `//#region 🔖RecordKinds`) for the same not-a-direct-dependency reason as
...
/// reason as `HEADER_SIZE` above (`protocol_format::CommitPayload`/`parse_commit_payload` are not
...
fn parse_level(flags: &HashMap<String, String>) -> Result<protocol::VerificationLevel, String> {
    match flags.get("level").map(String::as_str) {
        None => Ok(protocol::VerificationLevel::Standard),
        Some("trusted") => Ok(protocol::VerificationLevel::Trusted),
        Some("standard") => Ok(protocol::VerificationLevel::Standard),
        Some("full") => Ok(protocol::VerificationLevel::Full),
...
/// #️⃣ A short non-cryptographic content fingerprint for `diff`'s "payload-hash mismatch" lines.
///
/// 🎯 Design choice: the contract's real chain-hash primitive is `blake3`, but the only `blake3`
/// impl in this family (`protocol_format::Blake3Hasher`) is not part of the facade's re-export
/// surface (the trait `protocol::RecordHasher` is re-exported, an implementation is not) — and
...
fn fingerprint(edit: &protocol::HistoryEdit) -> String {
...
//#region 🔖Inspect
/// 🔍 `protocol inspect <file>` — header, commit chain (all generations), record counts by kind,
...
fn cmd_inspect(rest: &[String]) -> i32 {
    ...
    let mut cursor = protocol::FrameCursor::new(&bytes, HEADER_SIZE);
    loop {
...
/// commit hash chain (see `protocol_history::decode_history_from`'s `VerificationLevel::Full`
...
fn cmd_verify(rest: &[String]) -> i32 {
    ...
    let options = protocol::DecodeOptions { verification: level, limits: protocol::ProtocolLimits::default() };
    match protocol::HistoryReader::open(&bytes, &options).and_then(|reader| reader.log()) {
        Ok(log) => {
            println!("OK");
...
//#region 🔖Hash
/// #️⃣ `protocol hash <file>` — prints `(commit_seq, chain_hash)`, the file's current commit
/// identity, via `protocol::content_frontier`.
fn cmd_hash(rest: &[String]) -> i32 {
    ...
    match protocol::content_frontier(&bytes) {
        Ok(frontier) => {
...
//#region 🔖Log
/// 📜 `protocol log <file> [--limit N] [--actor ID] [--alternative ID] [--reverse]` — a timeline
...
fn cmd_log(rest: &[String]) -> i32 {
    ...
    let options = protocol::DecodeOptions::default();
    let log = match protocol::HistoryReader::open(&bytes, &options).and_then(|reader| reader.log()) {
        Ok(log) => log,
        Err(error) => {
            eprintln!("protocol: {error}");
            return 1;
...
//#region 🔖Compile
/// 🔨 `protocol compile <doc.ops> [--out doc.spr]` — ops text -> `.spr` binary via
/// `protocol::compile_ops`. Writes to `--out` when given, else emits the raw bytes to stdout.
fn cmd_compile(rest: &[String]) -> i32 {
    ...
    let bytes = match protocol::compile_ops(&text, &protocol::EncodeOptions::default()) {
        Ok(bytes) => bytes,
        Err(error) => {
...
//#region 🔖Decompile
/// 🔧 `protocol decompile <doc.spr> [--out doc.ops]` — `.spr` binary -> ops text via
/// `protocol::decompile_ops`. Writes to `--out` when given, else prints the text to stdout.
fn cmd_decompile(rest: &[String]) -> i32 {
    ...
    let text = match protocol::decompile_ops(&bytes, &protocol::DecodeOptions::default()) {
        Ok(text) => text,
        Err(error) => {
...
//#region 🔖Diff
/// 🌗 `protocol diff <a.spr> <b.spr>` — record-level edit diff: the longest common prefix, then
...
fn cmd_diff(rest: &[String]) -> i32 {
    ...
    let options = protocol::DecodeOptions::default();
    let log_a = match protocol::HistoryReader::open(&bytes_a, &options).and_then(|reader| reader.log()) {
        Ok(log) => log,
        Err(error) => {
    ...
    let log_b = match protocol::HistoryReader::open(&bytes_b, &options).and_then(|reader| reader.log()) {
        Ok(log) => log,
        Err(error) => {
    ...
    let by_id_b: HashMap<&str, (usize, &protocol::HistoryEdit)> = remaining_b.iter().enumerate().map(|(i, edit)| (edit.id.as_str(), (common_prefix + i, edit))).collect();
...
//#region 🔖Compact
/// ♻️ `protocol compact <file> [--out FIXED]` — calls `protocol::compact` (atomic, in-place
...
fn cmd_compact(rest: &[String]) -> i32 {
    ...
    match protocol::compact(&target, &options, &protocol::ProtocolLimits::default()) {
        Ok(()) => {
...
//#region 🔖Repair
/// 🩹 `protocol repair <file> [--truncate-torn-tail] [--rebuild-indexes]` — runs
/// `protocol::recover_file` and prints its `RecoveryReport`; `--truncate-torn-tail` then physically
/// truncates the file to the recovered prefix.
///
/// 🎯 `--rebuild-indexes` is a documented no-op in this build: rebuilding `REC_INDEX` payloads
/// needs `protocol_history::IndexBuilder`/`IndexReader`, which are not part of the facade's
...
fn cmd_repair(rest: &[String]) -> i32 {
    ...
    let report = match protocol::recover_file(Path::new(path), &limits, protocol::RecoveryMode::LastCommit) {
        Ok(report) => report,
        Err(error) => {
...
mod tests {
    ...
    fn sample_edit(id: &str, actor: Option<&str>, description: Option<&str>, coalesce_key: Option<&str>) -> protocol::HistoryEdit {
    ...
    /// 🧪 Builds a small `.spr` file on disk with `edit_count` edits (ids `"e00".."eNN"`, one per
    ...
    fn build_history_file(name: &str, edit_count: usize, with_checkpoint_and_alternative: bool) -> (PathBuf, Vec<u8>) {
        let mut appender = protocol::HistoryAppender::begin(Vec::new(), "doc-1", "schema-1", &protocol::WriteOptions::default()).unwrap();
    ...
    /// 🧪 Round-trips a small `HistoryLog` through `HistoryAppender` -> `decompile_ops` to obtain
    ...
    fn sample_ops_text() -> String {
        ...
        appender.commit().unwrap();
        let bytes = appender.into_sink();
        protocol::decompile_ops(&bytes, &protocol::DecodeOptions::default()).unwrap()
    }
    //#endregion 🔖Fixtures
    ...
    fn parse_level_accepts_known_values_and_rejects_unknown() {
        let mut flags = HashMap::new();
        assert!(matches!(parse_level(&flags), Ok(protocol::VerificationLevel::Standard)));
        flags.insert("level".to_string(), "full".to_string());
        assert!(matches!(parse_level(&flags), Ok(protocol::VerificationLevel::Full)));
    ...
    fn parse_commit_fields_matches_a_real_commit_frame() {
        ...
        // 🎯 `HistoryAppender::begin` writes a `REC_STR_DICT` delta (doc_id + schema interned)
    ...
    fn cli_verify_rejects_file_with_corrupted_header() {
        // 🎯 Design note: this family's read path (`HistoryReader::open` -> `protocol_format::
    ...
    fn cli_diff_reports_only_in_a_when_b_is_a_shorter_prefix() {
        let mut appender_a = protocol::HistoryAppender::begin(Vec::new(), "doc-1", "schema-1", &protocol::WriteOptions::default()).unwrap();
        ...
        let mut appender_b = protocol::HistoryAppender::begin(Vec::new(), "doc-1", "schema-1", &protocol::WriteOptions::default()).unwrap();
