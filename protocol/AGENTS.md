...
- `framework/plugin/rs::protocol_mode` — shared strict-list builder engine (add/remove/move-step, add/remove/move-block op-builders, `build_protocol_list_scene`, `render_protocol_builder`), reused by `protocol/plugin/rs` and `forms/plugin/rs` (forms' "Blueprint" mode)
