...
pub use generation_forms::{
    add_generation, apply_generation_op, generation_ops, handle_generation_action, initial_generation_values,
    invert_generation_op, remove_generation, rename_generation, render_generation_form_body,
    render_generation_preview_text, render_generations_tree, select_generation, selected_generation,
    selected_generation_mut, update_generation_values, FormGeneration, GenerationOp, GenerationPlayState,
};
pub use builder_kit::{
...
pub fn build_palette(builtin: &[(&str, &str, &str)], extensions: &[(String, String, String)]) -> Vec<BlockPaletteEntry> {
    let mut entries: Vec<BlockPaletteEntry> = builtin
        .iter()
        ...
        .collect();
    entries.extend(extensions.iter().map(|(kind, label, icon_id)| BlockPaletteEntry {
