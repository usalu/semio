...
/// 🔗 Extracts the minimal contiguous byte span (over the file's trusted, recovered prefix) that
...
/// 🔐 Content-integrity check for a `RecordSlice`'s bytes against a caller-supplied expected digest.
///
/// 🎯 Design choice: the contract does not pin an exact algorithm for a slice-level chain (the
...
/// 🔐 Shared by `verify_slice` and this crate's own tests: folds every frame's `blake3(full frame
...
/// 🧭 Decodes just enough of a `.spr` file (trusted prefix + a full `HistoryLog` decode) to report
...
///
/// 🎯 Design choice: `AlternativeHead.checkpoint_id` picks the LAST id in
...
/// 🧭 See `content_frontier`'s design-choice note for the derivation this implements.
...
/// 🧭 Highest edit ordinal transitively reachable through `checkpoint.change_ids -> edit_ids`.
...
mod tests {
    ...
    /// 🧪 Builds a tiny in-memory `.spr` buffer with `edit_count` edits (ids `"e0".."eN"`), each in
