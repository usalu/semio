#!/usr/bin/env bun
/** 🧭 `@semio-tech/gis-2d-play` task router: `bun ./script.ts <dev|build|build-tiles|test|tiles|fixture>`. */
...
class DevScript extends BundleScript {
  run(segments: string[]): void {
    ...
    runViteBunxDev(this.root, segments, {
      portEnv: playgroundPortEnv("gis-2d"),
      defaultPort: playgroundDevPortString("gis-2d"),
      fixedPort: true,
    });
  }
}
...
class TestScript extends BundleScript {
  run(segments: string[]): void {
    runCargo(["test", "-p", "gis_2d"], this.repoRoot, playPollingEnv());
    runBun([wasmScript, "wasm"], this.root, playPollingEnv());
