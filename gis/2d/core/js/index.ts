...
export const GIS_MAP_PLAY_SURFACE_ID = "gis.map.play";
...
export type GisMapPlayExampleHostConfig = {
  readonly defaultFixture: GisMapFixture;
...
export interface GisMapFixturePosition {
  readonly id: string;
  readonly lon: number;
  readonly lat: number;
...
export interface GisMapFixtureRoute {
  readonly id: string;
...
export interface GisMapFixture {
  readonly schema: "gis.map.fixture";
  readonly name: string;
  readonly positions: readonly GisMapFixturePosition[];
  readonly routes: readonly GisMapFixtureRoute[];
...
function parseGisMapFixturePosition(raw: unknown): GisMapFixturePosition | null {
...
function parseGisMapFixtureRoute(raw: unknown): GisMapFixtureRoute | null {
  if (!isRecord(raw)) return null;
...
export function parseGisMapFixture(raw: unknown): GisMapFixture | null {
  if (!isRecord(raw) || raw.schema !== "gis.map.fixture" || typeof raw.name !== "string") return null;
  const positions = Array.isArray(raw.positions)
    ? raw.positions.map(parseGisMapFixturePosition).filter((row): row is GisMapFixturePosition => row !== null)
    : [];
  const routes = Array.isArray(raw.routes)
    ? raw.routes.map(parseGisMapFixtureRoute).filter((row): row is GisMapFixtureRoute => row !== null)
    : [];
  return { schema: "gis.map.fixture", name: raw.name, positions, routes, regions: [] };
}

/** @emoji 🗺️ Maps a GIS map fixture into declarative overlay props. */
export function gisMapFixtureToDescriptor(fixture: GisMapFixture): MapDescriptor {
...
export interface MapPlaySnapshot {
  ...
  readonly activeFixture: GisMapFixture | null;
...
function mapPlayInspectorPositionGroup(positions: readonly GisMapFixturePosition[]): UiInspectorFieldGroup {
...
function mapPlayInspectorRouteGroup(routes: readonly GisMapFixtureRoute[]): UiInspectorFieldGroup {
...
export function buildMapPlayInspectorTree(
  ...
  if (selectedPositionIds.length > 0) {
    const positions = selectedPositionIds
      ...
      .filter((entry): entry is GisMapFixturePosition => Boolean(entry));
  ...
  if (selectedRouteIds.length > 0) {
    const routes = selectedRouteIds
      ...
      .filter((entry): entry is GisMapFixtureRoute => Boolean(entry));
    if (routes.length > 0) {
...
type GisMapFixtureEditOp =
  | { readonly op: "setDocument"; readonly document: GisMapFixture | null }
...
function patchGisMapPositionField(position: GisMapFixturePosition, field: string, value: unknown): GisMapFixturePosition {
...
function applyGisMapFixtureEditOp(fixture: GisMapFixture | null, op: GisMapFixtureEditOp): GisMapFixture | null {
  switch (op.op) {
    case "setDocument":
      return op.document;
    case "patchPositions": {
...
function gisMapFixtureSnapshot(fixture: GisMapFixture | null): GisMapFixtureEditOp {
  return { op: "setDocument", document: fixture };
}

/** @emoji ↩️ Inverts a GIS map fixture edit from the pre-apply projection. */
export function backwardsGisMapFixtureEditOp(fixture: GisMapFixture | null, op: GisMapFixtureEditOp): readonly GisMapFixtureEditOp[] {
  switch (op.op) {
    case "setDocument":
...
export function diffGisMapFixtureEditOp(_fixture: GisMapFixture | null, operation: GisMapFixtureEditOp): unknown {
  return operation;
}
...
export class MapPlayController extends Controller implements PlaygroundExampleHost {
  ...
  private readonly docStore: DocumentVcsStore<GisMapFixture | null, GisMapFixtureEditOp>;
  ...
  constructor(commandBus: CommandBus, notify: () => void, private readonly exampleHost: GisMapPlayExampleHostConfig) {
    super(GIS_MAP_PLAY_CONTROLLER_ID, commandBus, notify);
    this.activeExampleId = exampleHost.reuseFixtureId;
    this.docStore = new DocumentVcsStore<GisMapFixture | null, GisMapFixtureEditOp>({
      envelope: createDocumentVcsEnvelope("gis.map.fixture", "gis-map-play", exampleHost.defaultFixture),
  ...
  getActiveFixture(): GisMapFixture | null {
    return this.activeFixture;
  }

  getDocumentVcsStore(): DocumentVcsStore<GisMapFixture | null, GisMapFixtureEditOp> {
    return this.docStore;
  }

  private get activeFixture(): GisMapFixture | null {
  ...
  private setActiveExampleProjection(next: GisMapFixture | null): void {
    this.applyFixtureEdit({ op: "setDocument", document: next });
  }

  private patchPositionField(position: GisMapFixturePosition, field: string, value: unknown): GisMapFixturePosition {
...
if (import.meta.vitest) {
  ...
  function mapPlayTestFixtureHost(fixture?: GisMapFixture): GisMapPlayExampleHostConfig {
    const defaultFixture = fixture ?? { schema: "gis.map.fixture", name: "test", positions: [], routes: [], regions: [] };
    return {
      defaultFixture,
...
export const GIS_MAP_PLAY_DEFAULT_FIXTURE: GisMapFixture = reuseMapFixtureJson as GisMapFixture;
...
if (import.meta.vitest) {
	...
	describe("MapPlayController fixtures", () => {
		it("loads the reuse fixture by default", () => {
			...
			expect(ctrl.getSnapshot().activeFixture?.schema).toBe("gis.map.fixture");
...
//#region 🔖MediaExport
function gisMapFixtureToSvg(fixture: GisMapFixture): string {
	const scale = 8;
	const padding = 40;
...
export function registerGisMediaExportHandlers(): void {
	registerOsMediaExportHandler("2d.map", "svg", async (doc) => {
		const fixture = doc as GisMapFixture;
