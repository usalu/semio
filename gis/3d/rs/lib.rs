...
//! `gis_2d` crate's tile/session architecture, but produces renderable mesh buffers (for the
...
/// `gis_2d`'s `MapDescriptorJson`/`PositionData` pattern. Consumed by `gis-plugin`'s `app_3d`.
...
/// plugin needs to reach into `gis_3d` beyond the wasm session itself.
...
/// `gis_2d`'s `MapSession`, but yields mesh JSON rather than driving a canvas itself — actual
