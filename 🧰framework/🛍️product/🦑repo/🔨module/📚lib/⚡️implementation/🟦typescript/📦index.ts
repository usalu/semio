...
export function devToolingEnv(extra: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  ...
  env.NX_TUI ??= "false";
  // 🧩 Nx plugin-worker IPC corrupts non-BMP emoji path segments (🔨→U+FFFD); keep plugins in-process.
...
export function appendGitDeltaSuffix(line: string, d: { added: number; removed: number; edited: number }): string {
  if (d.added > 0) line += `➕${formatMetricLocCount(d.added)}`;
  if (d.edited > 0) line += `✏️${formatMetricLocCount(d.edited)}`;
  if (d.removed > 0) line += `➖${formatMetricLocCount(d.removed)}`;
  const sum = gitDeltaLineTotal(d);
  if (sum > 0) line += `🟰${formatMetricLocCount(sum)}`;
  return line;
}

/** 📊Full `📊uloc💯…` metrics line from bloc and git deltas (bundle header, footer, languages). */
...
//#region Format
/** 🔢Rounds to `digits` significant figures (preserves sign; 0 stays 0). */
...
/** 🔤Renders a coefficient already near 3 significant figures (no scientific notation). */
...
/** 🔤Expands a tiny positive value without scientific notation (1 significant digit). */
...
/** 🔤Decimal string for a finite number without `e` notation. */
...
/** 🔢Formats metric counts with 3 significant digits and k/M (e.g. 422377 → 422k, 1300000 → 1.3M). */
...
/** ➗Formats a ratio with 3 significant digits as a short decimal; never formats a positive value as 0. */
...
/** 📊Renders one explicit `📊uloc[emoji slug]💯…` metrics line; omits empty segments. */
export function formatUlocMetricsBody(input: UlocMetricsBodyInput): string {
  ...
  if (bloc > 0) line += `💯${formatMetricLocCount(bloc)}`;
  const net = added - removed;
  if (net !== 0) {
    const absNet = Math.abs(net);
    line += net > 0 ? `📈${formatMetricLocCount(absNet)}` : `📉${formatMetricLocCount(absNet)}`;
    const previous = bloc - net;
    if (previous > 0) line += `➗${formatMetricRatio(absNet / previous)}`;
  }
  if (added > 0) line += `➕${formatMetricLocCount(added)}`;
  if (edited > 0) line += `✏️${formatMetricLocCount(edited)}`;
  if (removed > 0) line += `➖${formatMetricLocCount(removed)}`;
  const sum = gitDeltaLineTotal({ added, edited, removed });
  if (sum > 0) line += `🟰${formatMetricLocCount(sum)}`;
  return line;
}

/** 📂Sums unified LOC for tracked paths under prefixes (bundle-scoped 💯). */
...
/** 📊Formats one per-language `📊uloc{emoji}{slug}💯…` row. */
...
/** 📊Renders the unified LOC block: total `📊uloc💯…`, then per-language rows. */
...
const MICRO_COMMIT_ULOC_ROW_RE = /^📊uloc/u;
...
/** 🎆Bundle squash only: `🎆YY🌙MM☀️DD` + full per-day `📊uloc💯…` suffix. */
...
export function parseCommitBundleBody(text: string): CommitBundleSection[] {
  ...
  for (const raw of text.split("\n")) {
    ...
    if (line.startsWith("📊uloc") || line.startsWith("Signed-off-by:")) continue;
