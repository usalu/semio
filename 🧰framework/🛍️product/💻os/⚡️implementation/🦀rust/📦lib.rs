...
use store::{create_document_envelope, document_backbone_ref, materialize_document_projection, DocumentBackboneRef, DocumentCommand, DocumentEnvelope, DocumentStore, SpaceConflict};
    ...
    fn reconcile_os_workflow(mut projection: OsProjection) -> (OsProjection, Vec<SpaceConflict>) {
        ...
        // 🩹 Baseline comparison: straight `artifact_kind` string equality, since `OsWorkflowEdge` has
    ...
    /// @emoji 📦 Binary pack+spr payload for the whole `OsDocument` (name + applied-edit cursor +
    ...
    /// @emoji 📥 Inverse of `encode_os_space_payload`.
    ...
    //#region 🔖Backbone
    /// @emoji 🔌 Byte-oriented studio persistence port — `read`/`write` carry `encode_os_space_payload`'s
    ...
    /// @emoji 🌉 `store::BackbonePort` is the shared string-typed transport for every document kind
    ...
    /// @emoji 🌉 Shared admission tail for `import_os_space_from_dsl`/`import_os_space_from_pack`:
    ...
    /// @emoji 📥 Imports a studio document dsl text (`export_os_space_dsl`'s counterpart — the
    ...
    /// @emoji 📦 Pack counterpart of `import_os_space_from_dsl`: decodes `pack`+`spr` directly via the
    ...
    /// @emoji 📤 DSL-text counterpart of `export_os_space_pack` — exercises the text export path
...
pub mod backbone {
    ...
    /// @emoji 🌉 `read`/`write`'s `payload` is the same `encode_os_space_payload` blob every
    ...
    /// @emoji 🔓 Strips `encode_os_space_payload`'s `name` wrapper, returning the inner `pack`+`spr`
    ...
    impl SpaceBackbonePort {
        /// @emoji 🌉 `self.memory` is a plain `store::BackbonePort` (string-typed) fallback for any uri
...
pub mod media_export_raster {
    ...
    /// @emoji 🧊 Exports `shapes` from `kernel` through the solid exporter registered for `artifact_kind` + `format`.
    ...
    /// @emoji 🧊 Imports bytes into `kernel` through the solid importer registered for `artifact_kind` + `format`.
...
pub mod workflow {
    ...
    /// 🔬 One planner test vector: a workflow graph, the instances marked dirty, and the deliveries
    ...
    //#endregion 🔖WorkflowInstanceRegistry
    ...
    mod tests {
        ...
        /// 🔬 Shared fixtures replay (`framework/product/os/core/fixtures/*.dsl`) — the same files
        ...
        /// 🧬 Every fixture ships as a `.dsl`/`.spk` pair: both must decode to the identical
...
pub mod wasm_exports {
    ...
    /// 📦 Decodes a `WorkflowFixture` from its binary `.spk` pack form into a plain JS object.
    ...
    /// 📖 Parses a `WorkflowFixture` from its `.dsl` text form into a plain JS object.
...
pub mod registry {
    ...
    pub struct OsArtifactDescriptor {
        ...
        pub media_type: MediaType,
        /// 🔌 Structured-payload schema id, mirrored from `ArtifactKindSpec::schema` — see
