//! 🧬 `dsl_token` — token-native document substrate: tokens carry a `StableId` that survives
...
//#region 🔖StableId
/// @emoji 🪪 A token's identity across its whole lifetime — assigned once on creation, never
...
/// @emoji 🧬 How a token came to exist, for editors that want to explain provenance.
...
//#region 🔖RopeToken
/// @emoji 🧾 One token as stored in the rope: stable identity plus the same kind/text/span a
...
//#region 🔖Rope
/// @emoji 🧵 An immutable, cheaply-clonable (`Arc`-backed) sequence of tokens — `Send + Sync` by
...
impl TokenRopeSnapshot {
    ...
    /// @emoji 📝 Renders the token text back into source text (concatenation — trivia tokens
...
//#region 🔖Position
/// @emoji 📌 A position addressed by token identity, never a character offset — survives
...
impl Position {
    /// @emoji 🎯 Resolves to a concrete token-vector index in `snapshot`. Falls back to the
...
//#region 🔖Patch
/// @emoji 🩹 One atomic change to the rope: replace the tokens in `[start, end)` with `inserted`.
...
/// @emoji 📦 An ordered batch of patches applied atomically, with preconditions on the base
...
impl TokenRope {
    /// @emoji ⚡ Applies a transaction atomically (all patches or none), returning the inverse
    ...
    /// @emoji 🔬 Builds fresh `RopeToken`s (new stable ids) from lexing `text` — used both to seed
...
//#region 🔖Incremental
/// @emoji 🔂 Re-lexes only the region touched by a `[byte_start, byte_end)` text replacement,
