...
pub fn assert_constitutional_crates(manifest_dir: &str) {
    const SLOTS: [(&str, &str); 6] = [("engine", "⚙️engine"), ("dsl", "🗣️dsl"), ("op", "🔧op"), ("pack", "🎒pack"), ("protocol", "📡protocol"), ("ui", "🖱️ui")];
    let normalized = manifest_dir.replace('\\', "/");
    if !normalized.contains("/✏️s/🔌plugin/") {
        return;
    }
    let base = std::path::Path::new(manifest_dir);
    // 🏛️ `../../../🎛️app` covers the constitutional-split bundle layout
    // `✏️s/🔌plugin/<p>/🛂manifest/🗿artifact/⚡️implementation/🦀rust`, whose app tree lives three
    // levels up at `✏️s/🔌plugin/<p>/🎛️app`.
    let app_root = [base.join("../🎛️app"), base.join("../../🎛️app"), base.join("../../../🎛️app")]
        .into_iter()
        .find(|candidate| candidate.is_dir());
    let Some(app_root) = app_root else {
        panic!("constitutional-crate gate: no `🎛️app/` directory found next to {manifest_dir} (tried ../🎛️app, ../../🎛️app, ../../../🎛️app)");
    };
    let is_flat = app_root.join("⚡️implementation").join("🦀rust").join("Cargo.toml").is_file();
    ...
    for app_dir in &app_dirs {
        ...
        if !app_dir.join("⚡️implementation").join("🦀rust").join("Cargo.toml").is_file() {
            missing.push("rs".to_string());
        }
        for (slot, slot_dir) in SLOTS {
            if !app_dir.join("🔨module").join(slot_dir).join("⚡️implementation").join("🦀rust").join("Cargo.toml").is_file() {
