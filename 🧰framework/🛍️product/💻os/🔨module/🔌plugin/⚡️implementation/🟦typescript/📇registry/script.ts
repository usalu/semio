...
function findPluginCargoFiles(root: string): string[] {
  ...
  function walk(dir: string) {
    for (const name of readdirSync(dir)) {
      ...
      if (st.isDirectory()) {
        walk(path);
      } else if (name === "Cargo.toml" && !path.includes("/🧰framework/🛍️product/💻os/🔨module/🔌plugin/⚡️implementation/🦀rust/")) {
        const isModuleCrate = /\/🔨module\/[^/]+\/⚡️implementation\/🦀rust\/Cargo\.toml$/.test(path);
        // 🏛️ Contribution bundle crate: `✏️s/🔌plugin/<p>/🧩extension/<e>/⚡️implementation/🦀rust/Cargo.toml` (new node/document
        // types the plugin contributes, e.g. imperative's control/logic/math/text).
        const isExtensionCrate = /\/✏️s\/🔌plugin\/[^/]+\/🧩extension\/[^/]+\/⚡️implementation\/🦀rust\/Cargo\.toml$/.test(path);
        // 🏛️ Plugin bundle crate lives at `✏️s/🔌plugin/<p>/🛂manifest/🗿artifact/⚡️implementation/🦀rust/Cargo.toml`.
        const isPluginBundleCrate = /\/✏️s\/🔌plugin\/[^/]+\/🛂manifest\/🗿artifact\/⚡️implementation\/🦀rust\/Cargo\.toml$/.test(path);
...
function discoverExamplesForPlayground(repoRoot: string, cratePath: string, pluginId: string, variant: string): string[] {
  ...
  // 🏛️ Under `✏️s/🔌plugin/<p>/...` the tech root is the plugin folder (3 segments), not `✏️s` itself.
  const techRoot = segments[0] === "✏️s" && segments[1] === "🔌plugin" ? segments.slice(0, 3).join("/") : segments[0];
  ...
  if (variant.startsWith(pluginId) && variant.length > pluginId.length) {
    ...
    const dir = join(repoRoot, techRoot, "🎛️app", suffix, "📚example");
...
/** @emoji 🔤 Slot word -> its emoji+name directory segment (matches the repo's emoji+name naming scheme). */
...
function validateConstitutionalCrates(repoRoot: string): string[] {
  const errors: string[] = [];
  const pluginRoot = join(repoRoot, "✏️s", "🔌plugin");
  ...
  for (const pluginName of readdirSync(pluginRoot).sort()) {
    ...
    const isFlatSingleApp = existsSync(join(appRoot, "⚡️implementation", "🦀rust", "Cargo.toml"));
    ...
    for (const { label, dir } of appDirs) {
      ...
      if (!existsSync(join(dir, "⚡️implementation", "🦀rust", "Cargo.toml"))) missing.push("rs");
      for (const slot of CONSTITUTIONAL_SLOTS) {
        if (!existsSync(join(dir, "🔨module", CONSTITUTIONAL_SLOT_DIRNAME[slot], "⚡️implementation", "🦀rust", "Cargo.toml"))) missing.push(slot);
      }
      if (missing.length > 0) {
        errors.push(`✏️s/🔌plugin/${label} is missing constitutional crate slot(s): ${missing.join(", ")} (expected 7: rs, ${CONSTITUTIONAL_SLOTS.join(", ")})`);
      }
    }
  }
  return errors;
