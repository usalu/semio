...
//
// 🎯 Design choices (contract leaves these to the implementer, documented once here):
...
//
// 🎯 Design choice: dictionaries are flushed INCREMENTALLY (a REC_STR_DICT delta record right
...
pub fn encode_history(log: &HistoryLog, options: &EncodeOptions) -> Result<Vec<u8>, ProtocolError> {
    ...
    // 🎯 Built incrementally (an edit's own id is inserted only AFTER it is encoded), matching
...
fn decode_history_from(trusted: &[u8], options: &DecodeOptions) -> Result<HistoryLog, ProtocolError> {
    ...
    while let Some(frame) = cursor.next_frame()? {
        ...
        match frame.kind {
            ...
            protocol_core::REC_ACTOR_DICT => {} // v1 never splits an actor dictionary — see 🔖Payloads note
...
/// @emoji 🚧 `edits_rev` cannot return `Result` per the frozen contract signature, so a prescan
...
/// @emoji 🔎 Builds the FULL dictionary and the FULL forward-ordered edit-id table in one forward
...
/// @emoji 🧭 Compares two frontiers by head edit ordinal, then head edit id/chain hash for the
