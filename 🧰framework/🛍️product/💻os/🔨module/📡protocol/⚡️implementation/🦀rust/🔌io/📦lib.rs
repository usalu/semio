...
mod native {
    ...
    //#region 🔖File
    /// @emoji 📍 Where a `.spr` file's trusted content currently ends, and the commit-chain state
    ...
    /// @emoji 📖 Re-derives a `ResumeState` from whatever `path` currently contains: runs
    /// `protocol_format::recover` (`RecoveryMode::LastCommit`), then reads back either the
    ...
    fn resume_state_for(path: &Path, limits: &ProtocolLimits) -> Result<ResumeState, ProtocolError> {
        let source = pack_io::FilePackSource::open(path)?;
    ...
    /// @emoji 📼 A live `.spr` file handle: either a fresh/resumed write path (`appender` set) or a
    ...
    pub struct HistoryFile {
        appender: Option<HistoryAppender<pack_io::FilePackSink>>,
        resume: ResumeState,
    }

    impl HistoryFile {
        /// @emoji 🆕 Creates `path` fresh (truncating any existing file) and writes the header plus
        ...
        pub fn create(path: &Path, doc_id: &str, schema: &str, options: &WriteOptions) -> Result<Self, ProtocolError> {
            let sink = pack_io::FilePackSink::create(path)?;
        ...
        ///
        /// 🎯 Design choice (forced by the crate boundary): `protocol_format::SprWriter::begin` and
        /// `protocol_history::HistoryAppender::begin` are the ONLY public constructors for those
        ...
        /// physical file (`pack_io::FilePackSink::create` truncates), and replay every edit/change/
        ...
        /// tolerates gracefully (`protocol_materialize::resolve_plan` falls back to full replay from
        ...
        /// @emoji 👓 Opens an existing `.spr` file purely for inspection: computes `resume_state()`
        /// via `protocol_format::recover` without writing a single byte to `path`. `appender()` must
        ...
        pub fn appender(&mut self) -> &mut HistoryAppender<pack_io::FilePackSink> {
    ...
    //#region 🔖Sidecar
    /// @emoji 🧾 `.sprc` sidecar checkpoint bodies live beside the `.spr` file, named
    ...
    /// @emoji 💾 Writes a complete `.spk` pack file as a sidecar, atomically (`pack_io::write_atomic`
    ...
    pub fn write_sidecar(protocol_path: &Path, body_hash: &[u8; 32], pack_bytes: &[u8]) -> Result<(), ProtocolError> {
        pack_io::write_atomic(&sidecar_path(protocol_path, body_hash), pack_bytes)?;
        Ok(())
    }
    ...
    //#region 🔖Recover
    pub fn recover_file(path: &Path, limits: &ProtocolLimits, mode: RecoveryMode) -> Result<protocol_format::RecoveryReport, ProtocolError> {
        let source = pack_io::FilePackSource::open(path)?;
    ...
    //#region 🔖Sync
    /// @emoji 🐌 Poll-based live tailing, runtime-neutral (no tokio dependency in the type itself —
    /// the caller drives `poll()` from whatever scheduler it likes).
    pub struct TailFollower {
        path: PathBuf,
        /// @emoji 🔖 The ordinal boundary this follower has consumed through: `poll()` returns
    ...
    impl TailFollower {
        /// @emoji 📖 Validates `path` looks like a real `.spr` file (header check) up front, so a
        ...
        /// @emoji 🔁 Re-reads the whole file and re-decodes from the start every call.
        ///
        /// 🎯 Design choice: `protocol_history::HistoryReader::edits()` always begins its own fresh
    ...
    fn encode_compaction_payload(options: &CompactOptions) -> Vec<u8> {
        let mut out = pack_core::ByteWriter::new();
        out.write_u8(1);
    ...
    /// `compact` cannot drive `protocol_history::encode_history`/`HistoryAppender` as a black box (it
    ...
    fn flush_dict(writer: &mut SprWriter<Vec<u8>>, dict: &DictBuilder, base: &mut u32) -> Result<(), ProtocolError> {
        ...
        if len > *base {
            ...
            let mut payload = pack_core::ByteWriter::new();
            payload.write_u8(1);
    ...
    /// @emoji 🧹 Atomic rewrite via `pack_io::write_atomic` (temp file + fsync + rename — a reader
    ...
    ///
    /// 🎯 `keep_snapshots`/`drop_ephemeral` are accepted and recorded verbatim into the
    ...
    pub fn compact(path: &Path, options: &CompactOptions, limits: &ProtocolLimits) -> Result<(), ProtocolError> {
        ...
        writer.write_record(protocol_core::REC_COMPACTION, true, &compaction_payload, CodecId(0))?;
        ...
        writer.commit()?;
        pack_io::write_atomic(path, &writer.into_sink())?;
        Ok(())
    }
    //#endregion 🔖Compact
    ...
    mod tests {
        use super::*;
        use protocol_history::{HistoryAlternative, HistoryChange, HistoryCheckpoint};
        use std::sync::atomic::{AtomicU64, Ordering};

        /// @emoji 🎲 Per-test unique scratch directory under `std::env::temp_dir()` — no external
        ...
        fn compact_preserves_every_edit_and_records_a_compaction_provenance_marker() {
            ...
            let doc_index = kinds.iter().position(|(kind, _)| *kind == protocol_core::REC_DOC).expect("REC_DOC present");
            let (compaction_kind, compaction_payload) = &kinds[doc_index + 1];
            assert_eq!(*compaction_kind, protocol_core::REC_COMPACTION);
