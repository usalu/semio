...
//#region 🔖Header
/// @emoji 🧲 The 8-byte magic every `.spr` file begins with — distinct from pack's `.spk` magic
/// (own root-of-trust model; see the module doc).
pub const MAGIC: [u8; 8] = [0x89, b'S', b'P', b'R', 0x0D, 0x0A, 0x1A, 0x0A];
/// @emoji 📏 Fixed wire size of the header, in bytes.
pub const HEADER_SIZE: usize = 32;
/// @emoji 🔢 The container format version this crate writes and reads.
pub const FORMAT_VERSION_MAJOR: u16 = 1;
/// @emoji 🔢 The container format minor version this crate writes.
pub const FORMAT_VERSION_MINOR: u16 = 0;

/// @emoji 🎭 The union of `protocol_core::REQUIRED_*` bits this crate understands; any bit outside
...
/// @emoji 📖 Validates a source's 32-byte header in place: magic, self-CRC, `required_flags`
...
/// @emoji 🪪 A header's decoded fields — exposed for downstream crates (`protocol_io`'s
...
/// @emoji 📖 Validates (see `validate_header`) and returns a source's 32-byte header fields.
...
//#region 🔖Frame
/// @emoji 🔢 The wire width, in bytes, of `value` encoded as an unsigned LEB128 varint.
...
/// @emoji 🚨 Builds a `protocol_core::ProtocolError::Malformed` for this crate's own structural
...
///
/// 🎯 Design choice: builds one contiguous buffer and issues a single `PackSink::write_all` at the
...
/// @emoji 📦 One parsed, CRC-verified record frame borrowed zero-copy from the buffer it was read
...
impl<'a> RecordFrame<'a> {
    /// @emoji 📤 The on-disk payload bytes: identical to the caller's original data iff
    ...
    /// @emoji 📏 Total on-disk bytes this frame occupies (the value its trailing `back_len` field
...
/// @emoji 👓 Zero-copy forward parse of one frame starting at `bytes[pos..]`. Validates `body_len`
...
impl<'a> FrameCursor<'a> {
    /// @emoji 🚀 Positions a cursor over `bytes`, ready to read the frame starting at `start_offset`
...
/// @emoji 📏 The smallest possible on-disk frame: a 1-byte `body_len` varint (value 2, the minimum
...
impl<'a> ReverseFrameCursor<'a> {
    /// @emoji 🏁 Positions a cursor just past the last byte of `bytes`, ready to read the final frame.
...
//#region 🔖Commit
/// @emoji 📏 Every commit frame's fixed total on-disk size: `1(body_len, single byte since 65 <
/// 128) + 1(kind) + 1(flags) + 64(payload) + 4(crc32c) + 4(back_len)`.
pub const COMMIT_FRAME_LEN: u64 = 75;
/// @emoji 📏 Fixed size of a `REC_COMMIT` frame's payload, in bytes.
...
/// @emoji 📖 Parses a commit frame's payload; the only structural requirement is the exact 64-byte
...
impl<S: PackSink> SprWriter<S> {
    /// @emoji 🚀 Writes the 32-byte header and seeds `chain_0 = blake3(header bytes)`.
    ...
    /// @emoji 📍 Current absolute write position — the offset the next record/commit will start at.
    ...
    /// @emoji 📤 Unwraps the underlying sink (e.g. to hand a `Vec<u8>` or file handle onward).
...
//#region 🔖Recovery
/// @emoji 📋 What a `recover` call found: how much of the file is trustworthy and where.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct RecoveryReport {
    pub records_recovered: u64,
    /// @emoji 📏 Design choice: the absolute count of trusted bytes from the start of the file —
...
/// @emoji 🔀 Which recovered boundary a caller trusts.
...
/// @emoji 🔍 How much a reader verifies before trusting decoded content.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Default)]
...
/// @emoji 🔢 Reads one LEB128 varint at an absolute source offset, one byte at a time so it never
...
/// @emoji 👓 `(kind, flags, raw_len, owned payload, frame_len)` — the owning twin of `RecordFrame`
...
/// @emoji 👓 Source-backed (owning, non-zero-copy) frame read at an absolute offset — the
...
/// @emoji ⚡ Fast path: probes the last `COMMIT_FRAME_LEN` bytes for a valid `REC_COMMIT` frame
...
/// @emoji 🚑 Recovers a `.spr` source: validates the header, then tries the O(commits) fast path
...
//#region 🔖Crypto
/// @emoji 🔗 The commit chain's hash primitive: `blake3`, owned here so `protocol_core` stays
...
mod tests {
    ...
    fn cursor_surfaces_unrecognized_extension_kind_transparently() {
        // 🎯 "Skip-unknown" for this crate means: an unrecognized kind (extension range, critical
