...
//! structure.
//!
//! 🎯 Design choice: internal hash-bucket routing (the HAMT's `PMap` and the graph's adjacency
...
//! `ContentAddressed`/`PageStore` mechanism in the `📇Pages` region below.

use db_core::DbError;
use std::rc::Rc;

//#region 🔖Pages
/// @emoji 📇 A type that can serialize itself into a byte buffer in a canonical, deterministic
...
/// @emoji 🔑 Hashes `bytes` with blake3, the family's hashing algorithm throughout (matches
/// `pack`/`protocol`'s `ContentHash`).
fn hash_bytes(bytes: &[u8]) -> pack_core::ContentHash {
    pack_core::ContentHash(*blake3::hash(bytes).as_bytes())
}

/// @emoji 📦 An immutable, content-addressed byte page: its `hash` is the blake3 digest of
...
impl PageStore {
    ...
    /// @emoji ➕ Interns `bytes`, returning its content hash. A byte-identical page already
...
/// @emoji 🪜 5 bits routed per HAMT level (32-way branching, per the contract).
const HAMT_BITS: u32 = 5;
/// @emoji 🛑 Beyond this depth, the 64-bit routing hash is exhausted (13 × 5 = 65 > 64); further
...
enum HamtNode<K, V> {
    ...
    /// @emoji 💥 A bucket of entries that share a routing hash (or that ran out of depth to
...
impl<K, V> PMap<K, V> {
    ...
    /// @emoji 🚶 Eagerly materializes every entry — simple to reason about at this crate's scope;
...
impl<K: Clone + Eq + std::hash::Hash + Ord + CanonicalEncode, V: Clone + CanonicalEncode> PMap<K, V> {
    /// @emoji 🔑 Content hash over `(key, value)` pairs in sorted-by-key order — sorted so the
...
//#region 🔖PVec
/// @emoji 🪜 5 bits routed per `PVec` trie level (32-way branching, matching `PMap`).
...
/// @emoji 🌱 Builds a fresh single-child chain down to a one-element leaf holding `value`. Only
...
/// @emoji 🧵 A persistent (immutable, structurally-shared) vector: a bitmapped trie, 32-way
...
impl<T: Clone + CanonicalEncode> PVec<T> {
    /// @emoji 🔑 Content hash over elements in index order (order-sensitive, unlike `PMap`'s).
    pub fn content_hash(&self) -> pack_core::ContentHash {
        let mut buf = Vec::new();
...
/// @emoji ➕ Concatenates two rope nodes, dropping an empty side rather than wrapping it — keeps
...
/// @emoji 📜 A persistent (immutable, structurally-shared) text rope. `insert`/`delete`/`slice`
/// are expressed as `split` + `concat`, sharing every untouched leaf with the rope(s) they were
/// derived from.
///
/// 🧩 Extension seam: no active rebalancing — a long run of single-character edits at the same
...
impl PText {
    ...
    /// @emoji 🔑 Content hash of the rope's flattened UTF-8 bytes.
    pub fn content_hash(&self) -> pack_core::ContentHash {
...
/// @emoji 🌳 A persistent (immutable, structurally-shared) ordered map: an AVL tree. Unlike
...
impl<K, V> PTree<K, V> {
    ...
    /// @emoji 📏 The tree's current height — exposed so tests (and callers with their own
...
impl<K: Clone + Ord + CanonicalEncode, V: Clone + CanonicalEncode> PTree<K, V> {
    /// @emoji 🔑 Content hash over `(key, value)` pairs in ascending key order (already the
...
impl<N: Clone + Eq + std::hash::Hash, ND: Clone, ED: Clone> PGraph<N, ND, ED> {
    ...
    /// @emoji 🧹 Removes `id` and every edge touching it (both directions) — the persistent
...
impl<N: Clone + Eq + std::hash::Hash + Ord + CanonicalEncode, ND: Clone + CanonicalEncode, ED: Clone + CanonicalEncode>
    PGraph<N, ND, ED>
{
    /// @emoji 🔑 Content hash over the node set (sorted, via `PMap::content_hash`) followed by
...
//#region 🔖Overlay
/// @emoji 🫧 The read side of a document's immutable base — typically a lazily-decoded `pack`
...
/// @emoji 🫙 A `BaseSource` with nothing in it — the base for a brand-new document, and useful in
...
impl<B: BaseSource> OverlayRoot<B> {
    ...
    /// @emoji 🔢 How many paths the overlay has explicitly recorded (set or tombstoned) — every
...
//#region 🔖TouchedRegion
/// @emoji 👣 Whether a `TouchedRegion` records a read or a write — two reads of the same region
...
impl TouchedRegion {
    ...
    /// @emoji 🔀 True iff `self` and `other` name the same path, or one path is a `/`-boundary
...
/// @emoji 🧾 The accumulated reads/writes of one command/transaction against an `OverlayRoot` —
...
mod tests {
    ...
    fn ptext_handles_multibyte_unicode_by_char_index() {
        ...
        assert_eq!(inserted.to_string(), "héllo→🎉wörld");
    }
