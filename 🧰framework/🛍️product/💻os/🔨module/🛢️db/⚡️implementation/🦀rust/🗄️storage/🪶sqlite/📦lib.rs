...
//!
//! 🔒 Durability choice: the connection is opened with `PRAGMA synchronous = FULL` (in `WAL`
...
//! is this wave's goal.
//!
//! 🔐 CAS choice: `CatalogStorage::cas_root` and every `LeaseStorage` mutation run inside an
...
mod sqlite_storage {
    ...
    //#region 🔖Schema
    /// @emoji 🧱 One document's WAL segment, keyed `(document, segment_index)`. `sealed` is
...
";
    ...
    //#region 🔖Errors
    /// @emoji 🚨 Wraps a `rusqlite::Error` into `DbError::Io` — the only place a foreign driver
    ...
    // 🔒 Used as a bare fn-pointer error mapper (`.map_err(sqlite_err)`) throughout this file —
    ...
    /// @emoji 🔢 SQLite's `INTEGER` column type is a signed 64-bit value; this crate's `u64`
    ...
    impl SqliteStorage {
        /// @emoji 🚀 Opens (creating the file and its parent directories if absent) a
        ...
        /// @emoji 🧪 Opens a private, in-memory `SqliteStorage` — never durable across process
        /// exit; exists for fast unit tests that don't care about on-disk persistence (the
        /// crash/reopen laws are exercised against a real file in `//#region 🧪Tests` instead).
        ...
        fn init(conn: Connection) -> Result<Self, DbError> {
            // 🎯 `journal_mode = WAL` is a no-op (silently stays `memory`) on an in-memory
        ...
        /// @emoji 🩹 Recovers the connection mutex from a poisoned lock instead of panicking — a
    ...
    impl WalStorage for SqliteStorage {
        ...
        fn append(&self, document: &DocumentId, index: u64, bytes: &[u8]) -> Result<u64, DbError> {
            ...
            // 🎯 `CAST(... AS BLOB)`: SQLite's `||` operator degrades a BLOB||BLOB concatenation
        ...
        fn sync(&self, _document: &DocumentId, _index: u64, _class: DurabilityClass) -> Result<(), DbError> {
            // 🎯 See module doc's "Durability choice": `synchronous = FULL` already fsyncs every
        ...
        fn seal(&self, document: &DocumentId, index: u64) -> Result<(), DbError> {
            ...
            let conn = self.lock();
            // 🎯 `changes()` counts rows matched by the WHERE clause regardless of whether
    ...
    impl CatalogStorage for SqliteStorage {
        ...
        fn cas_root(&self, expected: EpochFence, new_bytes: &[u8]) -> Result<EpochFence, DbError> {
            ...
            // 🎯 `IMMEDIATE` acquires SQLite's write lock before the read, so a concurrent writer
    ...
    mod tests {
        ...
        /// @emoji 🎲 A fresh `SqliteStorage` rooted at a unique scratch file under
