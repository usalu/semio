...
//#region 🔖RecordKinds
/// @emoji 📍 First record marker in a fresh segment: document identity, segment index, and the
...
/// @emoji 🚪 Opens a logical transaction: one group of records (typically one `submit()` batch)
/// that must be applied atomically on replay.
pub const WAL_TX_BEGIN: u8 = 0x41;
/// @emoji 🏁 Closes a transaction successfully; carries the record count written since the
/// matching `WAL_TX_BEGIN` as a replay sanity check.
pub const WAL_TX_COMMIT: u8 = 0x42;
/// @emoji 🚫 Closes a transaction as rolled back; replay must discard every record since the
...
/// @emoji 🫙 A command payload, either inlined or referenced by CAS hash (see `WalPayloadRef`).
pub const WAL_PAYLOAD: u8 = 0x45;
/// @emoji 🔀 An opaque `protocol::OperationDiff`-shaped byte blob (db crates below `db_document`
/// never interpret operation semantics, per the contract's hard dependency rule).
pub const WAL_DIFF: u8 = 0x46;
/// @emoji ⏪ An opaque inverse/undo byte blob for `db_document`'s inverse-undo pipeline.
pub const WAL_INVERSE: u8 = 0x47;
/// @emoji 📣 An opaque effect/notification byte blob.
pub const WAL_EVENT: u8 = 0x48;
/// @emoji 📤 An opaque outgoing-effect outbox entry byte blob.
pub const WAL_OUTBOX: u8 = 0x49;
/// @emoji 🧭 A structured `db_core::Frontier` snapshot, written periodically so recovery can
/// cross-check replay against a known-good checkpoint.
pub const WAL_FRONTIER: u8 = 0x4A;
/// @emoji 🌿 An opaque vcs change/checkpoint id string, recorded alongside the WAL entry that
/// produced it (`db_engine`'s `VersionGraph` seam; this crate stores the id, never interprets it).
pub const WAL_VCS_REF: u8 = 0x4B;
/// @emoji 📸 Marks that a snapshot generation was published, with the frontier it covers.
pub const WAL_SNAPSHOT_PUB: u8 = 0x4C;
/// @emoji 🔖 Marks an index checkpoint: the set of `db_index` run ids current as of this point.
pub const WAL_INDEX_CKPT: u8 = 0x4D;
/// @emoji ⏳ A lease grant/renewal/release record (`db_cluster`'s ownership-lease durability).
pub const WAL_LEASE: u8 = 0x4E;
/// @emoji 🚚 An opaque schema/data migration descriptor byte blob.
pub const WAL_MIGRATION: u8 = 0x4F;

/// @emoji ✅ True iff `kind` falls in this crate's SPR extension range — the test every reader
...
/// @emoji 📖 `read_field_bytes` plus a utf-8 validation, for text fields.
...
/// @emoji 🫙 `WAL_PAYLOAD`'s two shapes: small payloads inline, large ones by CAS reference into
...
/// @emoji 📜 One decoded WAL record — the typed shape every `WAL_*` kind decodes to/encodes from.
...
impl WalRecord {
    /// @emoji 🔢 The transaction id carried by a `TxBegin`/`TxCommit`/`TxAbort` record, or `None`
    ...
    /// @emoji 📖 Inverse of `encode`. Errors `DbError::Corrupt` on an unrecognized `kind` (a
...
impl GroupCommitPolicy {
    /// @emoji ⏰ True iff any bound is currently exceeded — `DocumentWal::submit` also always
...
//#region 🔖Sink
/// @emoji 🪞 A `pack_core::PackSink` over a shared, growing in-memory buffer. `protocol::SprWriter`
...
/// @emoji 🩹 Recovers a poisoned lock instead of panicking — one panicking document actor must
...
//#region 🔖Recovery
/// @emoji 🚨 Maps `protocol::ProtocolError` (this crate never leaks it in a public signature, per
...
/// @emoji 👓 Decodes every `WAL_*`-kind frame in `trusted` (a segment's already-recovered,
...
/// @emoji 📋 What `DocumentWal::open` found while recovering a document's WAL.
...
/// @emoji 🔁 Decodes every `WAL_*` record across a document's ENTIRE WAL (every sealed segment in
...
//#region 🔖Segment
/// @emoji 📦 The live write path for exactly one WAL segment: an in-memory `protocol::SprWriter`
...
impl SegmentWriter {
    /// @emoji 🆕 Creates segment `index` in `storage`, writes its `WAL_SEGMENT_HEADER` record, and
    ...
    /// @emoji 📏 Bytes written since the last flush — not yet visible to `WalStorage`.
    fn pending_bytes(&self) -> u64 {
        self.buf.len() - self.flushed_len
    }

    /// @emoji 📏 The segment's total logical length so far, flushed or not — what
...
//#region 🔖DocumentWal
/// @emoji 📏 Default segment-rotation threshold (this crate's own choice — the contract fixes
...
/// @emoji 🧾 What `DocumentWal::submit` did with one transaction's records.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct WalAppendReceipt {
    pub segment_index: u64,
    pub tx_id: u64,
    /// @emoji ✅ True iff this call caused (or piggybacked on) a physical `commit()`+`sync` —
    /// false means the transaction is durable only up to `DurabilityClass::Memory` so far, still
    /// batched behind `GroupCommitPolicy`.
    pub committed: bool,
}

/// @emoji 📼 One document's write-ahead log: an ordered chain of per-segment `.spr` files over
...
impl DocumentWal {
    /// @emoji 🌱 Creates a brand new WAL for `document` (segment 0, genesis — no prior segment to
    ...
    /// @emoji 🚑 Opens `document`'s existing WAL, recovering it: every sealed segment (all but the
    ...
    pub fn open(
        ...
        // 🎯 Design choice (forced by `protocol::SprWriter`'s API — the same constraint
    ...
    /// @emoji 🚿 Forces a commit+flush of whatever is currently pending, regardless of policy —
    ...
    /// @emoji 🔄 Seals the active segment (after a final commit+flush) and begins a fresh one,
...
mod tests {
    ...
    fn record_kinds_fill_the_extension_range_uniquely() {
        ...
        // 🧪 `0x0C` (protocol_core::REC_COMMIT) hard-coded rather than depending on protocol_core
