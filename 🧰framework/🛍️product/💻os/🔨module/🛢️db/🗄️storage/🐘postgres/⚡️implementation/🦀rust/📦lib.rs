...
//! every trait method blocks on it via `block_on` (see `//#region 🔖Runtime`). This is the
//! contract-sanctioned reason this crate (unlike its sqlite/neo4j-with-sync-driver siblings) pulls
//! in `tokio` below `db_engine`.
//!
//! 🐘 On-disk shape: six tables (`db_wal_segment`, `db_snapshot_generation`, `db_payload`,
...
//#region 🔖Schema
/// @emoji 🧱 Idempotent DDL bootstrapped by `PostgresStorage::connect` — one statement per table,
...
/// @emoji 🐘 A `db_storage::DbStorage` backend over PostgreSQL. `pool` is the connection pool;
...
impl PostgresStorage {
    /// @emoji 🔌 Connects to `database_url`, bootstraps the schema (idempotent, no migration
    ...
    /// @emoji 🌉 Bridges this crate's synchronous `db_storage` trait methods onto `sqlx`'s
    ...
    /// 🚧 Extension seam: `block_in_place` requires the ambient runtime to be multi-threaded — a
...
//#region 🔖ErrorMapping
/// @emoji 🚨 Maps a `sqlx::Error` to this family's `DbError` — the only place `sqlx::Error` is
...
/// @emoji 🆕 Like `map_sqlx_error`, but a unique-violation becomes `DbError::AlreadyExists(what())`
...
//#region 🔖Conversions
/// @emoji 🔢 Every dense index/generation/run/epoch/timestamp this crate stores is `u64` at the
...
impl WalStorage for PostgresStorage {
    ...
    fn sync(&self, _document: &DocumentId, _index: u64, class: DurabilityClass) -> Result<(), DbError> {
        // 🎯 Every write above already ran as a committed statement/transaction, and Postgres
...
impl CatalogStorage for PostgresStorage {
    ...
    fn cas_root(&self, expected: EpochFence, new_bytes: &[u8]) -> Result<EpochFence, DbError> {
        ...
        self.block_on(async {
            let mut tx = self.pool.begin().await.map_err(map_sqlx_error)?;
            // 🎯 The bootstrap-seeded singleton row (`id = 1`) always exists, so `SELECT ... FOR
...
//#region 🔖LeaseStorage
/// @emoji ⏳ The row currently held on a resource, as read from `db_lease` — the DB-shaped input to
...
/// @emoji 🤝 Pure decision for `LeaseStorage::acquire` — identical state machine to
...
/// @emoji 🔒 Reads `resource`'s current lease row through `executor`, taking a `FOR UPDATE` row
...
mod tests {
    ...
    // 🔬 Live-Postgres integration testing (schema bootstrap round-trip, real CAS races across
    ...
    // itself is exercised by inspection against the schema in `//#region 🔖Schema`.

    //#region 🔖RangeAndTruncate
    #[test]
    ...
    //#endregion 🔖SchemaAndCapabilities

    //#region 🔖ContentAddressing
    #[test]
