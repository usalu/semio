...
//! `db_engine`'s own `//🎯 Design choice (scope)` note) drive `db_document::DocumentEngine`
...
impl SplitMix64 {
    ...
    /// @emoji 🎯 A uniform draw in `0..bound`, or `0` if `bound == 0` (never divides by zero).
...
//#region 🔖Generators
/// @emoji 🎲 Deterministic seeded fabricator for the primitive pieces of a `protocol::
...
impl CommandGen {
    ...
    /// @emoji 🆔 A fresh, seed-derived but still call-order-unique operation id — unique because
...
/// @emoji 🎲 Builds whole, self-contained `protocol::OperationEnvelope` sequences from `CommandGen`
...
impl WorkloadGen {
    ...
    /// @emoji 🧩 `count` sequential, mutually DISJOINT-path envelopes (`path-0` .. `path-{count-1}`,
...
/// @emoji 📋 One schedulable unit of `SimRuntime` work: a name (surfaced in the run order for
...
/// @emoji 🎲 Seeded deterministic scheduler: collects named tasks, then runs them in a
...
/// @emoji 🔭 Bounded interleaving explorer for model checks: derives `permutations` distinct seeds
...
//#endregion 🔖SimRuntime

//#region 🔖FaultStorage
/// @emoji 💥 One knob of `FaultStorage`'s injectable fault script — every field independently
/// optional/off by default, so a freshly `FaultScript::default()`ed `FaultStorage` behaves exactly
/// like its inner backend.
#[derive(Clone, Copy, Default, Debug)]
pub struct FaultScript {
    /// @emoji 🚫 The Nth call to `WalStorage::append` (1-indexed, counting EVERY append this
    ...
    /// @emoji 🤥 `WalStorage::sync` returns `Ok(())` without ever forwarding to the inner backend —
...
/// @emoji 💥 Wraps a real `db_storage::DbStorage` backend with a scriptable `FaultScript`, injected
...
impl FaultStorage {
    ...
    /// @emoji 🔢 Every `WalStorage::append` call this storage has seen so far, faulted or not —
    ...
    /// @emoji 🔢 How many `WalStorage::sync` calls actually reached the inner backend (as opposed
...
//#endregion 🔖FaultStorage

//#region 🔖CrashHarness
/// @emoji 📋 What `CrashHarness::run_crash_after_every_write` found across every injected write
/// boundary.
#[derive(Clone, Debug, Default)]
pub struct CrashHarnessReport {
    pub writes_tested: u64,
    /// @emoji 🚨 `(write index crashed after, error)` for every crash point where reopening the
    /// faulted storage itself errored (recovery must never do this).
    pub reopen_failures: Vec<(u64, String)>,
    /// @emoji 🚨 Write indices where the reopened, recovered document's state did not match the
...
/// @emoji 💥 Drives `db_document::DocumentEngine` through a fixed, seeded workload once per WAL
...
impl CrashHarness {
    /// @emoji 💥 For `seed`/`op_count`'s deterministic `WorkloadGen::disjoint_batch` workload:
    ...
    ///
    /// 🧩 Extension seam (deliberately, honestly scoped): write boundary #1 — the document's own
...
/// @emoji 💥 Like `run_workload_against`, but stops silently at the first injected fault instead of
...
/// @emoji 🔁 The replay-determinism law: (1) a document's WAL replay after a clean shutdown+reopen
...
/// @emoji 📸 The "snapshot + WAL suffix == full replay" law: a replica that snapshots partway
...
/// @emoji 🧬 A minimal, always-triggered projection: counts how many committed envelopes it has
...
/// @emoji 🧬 The "rebuild == incremental apply" law: applying a projection envelope-by-envelope via
...
/// @emoji 🔀 The sync-convergence law: a replica that catches up in one shot, and a replica that
...
/// @emoji 🚧 The fencing law: once a writer's `cas_root` succeeds, a second writer presenting the
...
/// @emoji 🫧 The overlay structural-sharing law: each `db_state::OverlayRoot::set` grows the
...
mod tests {
    ...
    //#region 🔖exhaustive
    /// @emoji 🐌 Genuinely slower, exhaustive-only corruption fuzzing — real db_wal recovery driven
