//! 📦 `pack_value` — DSL schema-aware wire encoding/decoding of `dsl_schema::RecordValue`
...
/// @emoji 📊 Extracts a `Table` field's lazy element-spec constructor, if `shape` is `Table`.
...
/// @emoji 🧾 Resolves a `Record` field's nested spec, if `shape` is `Record`.
...
/// @emoji 🔢 Canonical `f64` normalization: `-0.0` → `0.0`, any `NaN` → the single canonical
...
/// @emoji 🔢 Which packed form a homogeneous numeric sequence is eligible for.
enum NumKind {
    F64,
    Varint,
}

/// @emoji 🧮 A sequence is packed-eligible iff every element is the same numeric `FieldValue`
...
/// @emoji 📌 Forces `TAG_STR_INLINE` regardless of the interning decision — the wire rule for
...
/// @emoji 🔎 Deterministic string-interning pre-pass: walks the whole document once (shape-aware
...
/// @emoji 🌱 `DslValue::Object` keys are always inline (never interned) per the wire contract, so
...
//#region 🔖Encode
/// @emoji 🧾 Encodes one record's fields as `field_count varint, (field_id varint, value)*` —
...
/// @emoji 🧾 Decodes one record's fields: `field_count, (field_id, value)*`. Any field id not
...
/// @emoji 📖 Decodes one tag-prefixed value. `shape`, when known, disambiguates `Tuple` vs
...
/// @emoji 🔢 Decodes a `PackedVarint` body. `elem_shape` (the field's `List(UInt)`/`List(Enum)`/
...
/// @emoji 📊 Encodes `Shape::Table`'s `List(Record)` value as columnar `TableSoA`: `row_count,
...
/// @emoji 🔑 `blake3` over a canonical serialization of `spec`'s `(field id, key, shape-tag)`
...
/// @emoji 🩺 What [`decode_document`] observed beyond the plain `RecordValue`: field ids present
...
/// @emoji 🚪 The single entry point every other `pack_*`/`vcs`/`dsl_derive` crate encodes a
...
/// @emoji 🚪 The single entry point every other `pack_*`/`vcs`/`dsl_derive` crate decodes a
...
mod tests {
    ...
    /// @emoji 🧬 One field of every `Shape` variant, exercising every wire tag in a single spec.
    ...
    fn round_trips_scalar_edge_cases() {
        ...
        fields.insert(6, FieldValue::Text("héllo wörld 🔖 日本語".to_string()));
        ...
        assert_eq!(decoded.get(6), Some(&FieldValue::Text("héllo wörld 🔖 日本語".to_string())));
