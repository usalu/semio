...
import { generatePluginRegistry, isStudioPluginFilter, writePlaygroundSession, type PluginRegistryEntry } from "../../../../../../../🧰framework/🛍️product/💻os/🔨module/🔌plugin/⚡️implementation/🟦typescript/📇registry/📜script.ts";
...
async function ensurePluginRegistry(filterPlugin?: string): Promise<void> {
  const registryScript = join(repoRoot, "./🧰framework/🛍️product/💻os/🔨module/🔌plugin/⚡️implementation/🟦typescript/📇registry/📜script.ts");
...
async function buildPlugins(filterPlugin?: string): Promise<void> {
  ...
  const stalePublicPlugins = join(repoRoot, "🧰framework/🛍️product/💻os/🔨module/🧑‍💻dev/⚡️implementation/🟦typescript/public/plugin-modules");
...
function pluginWatchRoot(target: PluginRegistryEntry): string {
  ...
  if (topLevel === "🧰framework" || topLevel === "framework") return join(repoRoot, target.cratePath);
  // 🏛️ Post-restructure: sibling crate families live under `✏️s/🔌plugin/<p>/...` (was `s/plugin/<p>/...`).
  // Widening to `✏️s/` would watch every plugin's tree on every crate's edit.
  if ((topLevel === "✏️s" || topLevel === "s") && (segments[1] === "🔌plugin" || segments[1] === "plugin")) {
...
/** 🎭 Playwright end-to-end workflow verification for the `s` studio shell (folded in from the former `.🦑repo/🎫tickets/26/07/04/RUST-PLUGIN-FRAMEWORK-MIGRATION/s-studio-e2e-verify.mjs`). */
 ...
 * `.🦑repo/🎫tickets/26/07/11/WGPU-RENDERER-FULL-PARITY/`. */
...
async function startParityDevServer(renderer: ParityRenderer, variant: string, port: number): Promise<ParityServerHandle> {
  const devScript = join(repoRoot, "./🧰framework/🛍️product/💻os/🔨module/🧑‍💻dev/⚡️implementation/🟦typescript/📜script.ts");
  const logPath = join(parityOutDir(), `boot-${renderer}-${variant}.log`);
  const logStream = createWriteStream(logPath);
  const daemon = spawnDaemon("bun", [devScript, "dev"], {
    cwd: join(repoRoot, "./🧰framework/🛍️product/💻os/🔨module/🧑‍💻dev/⚡️implementation/🟦typescript"),
...
function parityOutDir(): string {
  const dir = process.env.PARITY_OUT_DIR ?? join(repoRoot, ".🦑repo/🎫tickets/26/07/11/WGPU-RENDERER-FULL-PARITY");
