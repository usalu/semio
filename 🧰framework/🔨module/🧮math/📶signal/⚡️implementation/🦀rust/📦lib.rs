//! 📡 Signal processing: FFT, windows, spectra, correlation, smoothing and peak analysis for time-series and modal analysis.
...
/// 🌀 In-place iterative radix-2 Cooley-Tukey DFT with bit-reversal permutation; asserts equal power-of-two lengths.
...
/// 🔄 In-place inverse DFT via the conjugate trick with `1/N` scaling; asserts equal power-of-two lengths.
...
/// 🔄 In-place 2D inverse DFT of a row-major `w * h` grid via the conjugate trick with `1/(w*h)` scaling.
...
// #region 🔖Spectrum
/// 📊 Welch one-sided power spectral density: Hann-windowed segments with fractional `overlap`, zero-padded to a power of two, averaged periodograms with window-power normalization; bin `k` maps to frequency `k * fs / next_pow2(seg_len)`.
...
/// 🔀 Averaged one-sided cross-spectral density `S_ab = conj(A) * B` over Hann-windowed overlapping segments, returned as `(magnitude, phase)`; a delay of `d` samples in `b` shows as phase `-2 * PI * k * d / next_pow2(seg_len)` at bin `k`.
...
// #region 🔖Correlate
/// 🔗 Mean-removed, unit-energy normalized cross-correlation for lags `-max_lag..=max_lag`, computed via FFT with zero-padding to `next_pow2(len_a + len_b)`; a delay of `d` samples in `b` peaks at output index `max_lag + d`.
...
/// 🎯 Index of the maximum refined by parabolic 3-point interpolation; `None` when `c` is empty or the peak sits on an edge.
...
// #region 🔖Smooth
/// 📉 Savitzky-Golay filter with mirror-padded edges: least-squares polynomial of `order` on an odd `window`, returning the `deriv`-th derivative (0..=2) scaled by `deriv! / dt^deriv`; coefficients come from the Vandermonde normal equations solved once via `MatD::lu_solve`.
...
/// 📏 Centered moving average over `window` samples with mirror-padded edges.
