//! 🔬 Graph motif extraction via iterative color refinement (1-dimensional Weisfeiler-Leman):
...
// #region 🔖Refine
/// 🔬 Runs `rounds` steps of color refinement starting from `initial_labels` (one per node,
...
/// 🔬 FNV-1a-style mixing (matching `CompiledModel::fingerprint`'s own convention), folding in a
...
/// 🔬 Relabels arbitrary `u64` colors into dense small integers `0..k`, in first-seen order (so
