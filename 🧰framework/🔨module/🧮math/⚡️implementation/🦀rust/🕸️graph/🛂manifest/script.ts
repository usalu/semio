...
import { BundleScript, getWorkspaceRoot, ScriptRouter, runBundleScriptMain, runCargoLint, runCargoTestBudgeted, resolveTestLevel } from "../../../../../../../🧰framework/🛍️product/🦑repo/🔨module/📚lib/⚡️implementation/🟦typescript/📦index.ts";
...
function findManifestFiles(root: string): string[] {
  ...
  function walk(dir: string) {
    for (const name of readdirSync(dir)) {
      ...
      if (st.isDirectory()) {
        walk(path);
      } else if (name.endsWith(".manifest.json") && path.includes("/manifest/")) {
        out.push(path);
      }
    }
  }
...
function emitTsManifest(doc: ManifestDocument): string {
  const prefix = pascalCase(doc.id);
  let out = `// Generated from ${doc.id}.manifest.json\n\nimport type { GraphManifestDocument, KindCatalogBundle } from "../../../../../../../🧰framework/🔨module/🧮math/⚡️implementation/🦀rust/🕸️graph/🛂manifest/🤖generated/types.ts";\n\n`;
  ...
  out += `  const doc = ${prefix.toUpperCase()}_MANIFEST_DOCUMENT;\n`;
  out += `  return {\n`;
  ...
  run(): void {
    ...
    for (const path of files) {
      ...
      if (doc.schema !== "manifest") {
        console.log(`[graph-manifest] skip ${relative(root, path)} (${doc.schema})`);
        continue;
      }
      console.log(`[graph-manifest] ${relative(root, path)}`);
      docs.push(doc);
    }
    if (docs.length === 0) {
    ...
    const rustModules = docs.map((doc) => {
      const modName = rustModName(doc.id);
      writeFileSync(join(outDir, `${modName}.rs`), emitRustManifest(doc));
      return modName;
    });
    const registryRs =
      `// Generated manifest registry\n\n` +
      rustModules.map((m) => `pub mod ${m};`).join("\n") +
    ...
    writeFileSync(
      ...
      `export * from "../../../../../../../🧰framework/🔨module/🧮math/⚡️implementation/🦀rust/🕸️graph/🛂manifest/🤖generated/types.ts";\n` +
        docs.map((d) => `export * from "./${d.id}.js";`).join("\n") +
    ...
    for (const doc of docs) {
      writeFileSync(join(outDir, `${doc.id}.ts`), emitTsManifest(doc));
    }
    console.log(`[graph-manifest] wrote ${docs.length} manifests to ${relative(root, outDir)}`);
  }
}
