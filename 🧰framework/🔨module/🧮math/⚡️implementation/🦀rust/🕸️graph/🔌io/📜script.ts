#!/usr/bin/env bun
/** 💾 `@semio-tech/math-graph-io` — Graph serialization: edge/adjacency lists, GML, GraphML, GEXF, Pajek, JSON encodings, and relabeling. */
...
class TestScript extends BundleScript {
  run(segments: string[]): void {
    ...
    runCargoTestBudgeted(["mathematical_graph_io"], join(this.root, "rs"), rest);
  }
}

class LintScript extends BundleScript {
  run(): void {
    runCargoLint(["mathematical_graph_io"], join(this.root, "rs"));
  }
}
