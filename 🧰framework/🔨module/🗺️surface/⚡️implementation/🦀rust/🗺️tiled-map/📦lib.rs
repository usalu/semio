...
mod tests {
    ...
    #[test]
    #[ignore = "requires .🦑repo/🎫tickets/26/06/03/MAP-VECTOR-TILES/sample-2-2-1.pbf from demotiles"]
    fn decode_demotile_fixture_has_named_layers() {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../../../../.🦑repo/🎫tickets/26/06/03/MAP-VECTOR-TILES/sample-2-2-1.pbf");
    ...
    fn fixture_linestrings_split_at_moveto() {
        let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../../../../.🦑repo/🎫tickets/26/06/03/MAP-VECTOR-TILES");
        let mut found = false;
    ...
    fn demotile_z5_has_countries_and_centroids() {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../../../../.🦑repo/🎫tickets/26/06/03/MAP-VECTOR-TILES/sample-5-17-11.pbf");
        let bytes = std::fs::read(path).expect("fixture pbf");
        let tile = super::vector_tiles::decode_mvt(&bytes).expect("decode");
    ...
    fn demotile_z0_has_many_country_features() {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../../../../.🦑repo/🎫tickets/26/06/03/MAP-VECTOR-TILES/sample-0-0-0.pbf");
        let bytes = std::fs::read(path).expect("fixture pbf");
        let tile = super::vector_tiles::decode_mvt(&bytes).expect("decode");
        let countries = tile.layers.iter().find(|l| l.name == "countries").expect("countries");
    ...
    fn figure_country_lod_uses_land_mass_backdrop() {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../../../../.🦑repo/🎫tickets/26/06/03/MAP-VECTOR-TILES/sample-3-4-2.pbf");
        let bytes = std::fs::read(path).expect("fixture pbf");
        let mut host = super::MapHost::new();
        host.set_size(800, 600, 1.0);
        host.set_render_mode("vector");
