//! 🔗 Sequence program — declarative sequence play app bundled as a hot-swappable WASM program.
...
/// — out of scope to change, since both are defined in `framework/program`.
...
fn register_sequence_exports() {
    semio_framework_program::program_runtime::register_document_codec_for_app::<SequencePlayApp>(SEQUENCE_FIXTURE_SCHEMA);
}
