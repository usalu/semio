...
export function assembleSplitInitialKitFromDirectory(initialKitDir: string): Record<string, unknown> {
  ...
  const typesDir = fs.existsSync(path.join(initialKitDir, "type")) ? path.join(initialKitDir, "type") : path.join(initialKitDir, "types");
  const designsDir = fs.existsSync(path.join(initialKitDir, "design")) ? path.join(initialKitDir, "design") : path.join(initialKitDir, "designs");
...
export function readInitialKitFixtureFromPath(kitJsonPath: string): Record<string, unknown> {
  ...
  if (fs.existsSync(path.join(initialKitDir, "type")) || fs.existsSync(path.join(initialKitDir, "types"))) {
...
function annotateKitSemantics(kit: Record<string, unknown>): void {
  const types = [...fixtureItemsOf(kit.types), ...fixtureItemsOf(kit.typologies).flatMap((topo) => fixtureItemsOf(topo.types))];
