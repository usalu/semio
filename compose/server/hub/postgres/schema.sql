-- #region 🧲Header
-- compose/postgres/schema.sql
-- 2026 Ueli Saluz <ueli@compose-tech.de>
-- Canonical PostgreSQL schema for compose kit persistence, including normalized kit snapshots and kit version-control state.
-- #endregion 🧲Header
...
-- #region 🗂️Catalog
CREATE TABLE IF NOT EXISTS core.compose_schema (
...
CREATE TABLE IF NOT EXISTS core.connection (
	...
	FOREIGN KEY (snapshot_id, design_id) REFERENCES core.design (snapshot_id, design_id) ON DELETE CASCADE,
	FOREIGN KEY (snapshot_id, parent_piece_id) REFERENCES core.piece (snapshot_id, piece_id) ON DELETE CASCADE,
	FOREIGN KEY (snapshot_id, parent_design_piece_id) REFERENCES core.piece (snapshot_id, piece_id) ON DELETE SET NULL,
	FOREIGN KEY (snapshot_id, parent_connector_id) REFERENCES core.connector (snapshot_id, connector_id) ON DELETE SET NULL,
	FOREIGN KEY (snapshot_id, child_piece_id) REFERENCES core.piece (snapshot_id, piece_id) ON DELETE CASCADE,
	FOREIGN KEY (snapshot_id, child_design_piece_id) REFERENCES core.piece (snapshot_id, piece_id) ON DELETE SET NULL,
	FOREIGN KEY (snapshot_id, child_connector_id) REFERENCES core.connector (snapshot_id, connector_id) ON DELETE SET NULL,
	CHECK (parent_piece_id <> child_piece_id)
);
