...
class DevScript extends BundleScript {
  run(segments: string[]): void {
    ...
    runCmd("bun", ["./📜️script.ts", "dev", "storybook", "compose/algorithm", ...segments], { cwd: this.repoRoot, env });
  }
}

class BuildScript extends BundleScript {
  run(segments: string[]): void {
    runCmd("bun", ["./📜️script.ts", "build", "storybook", ...segments], { cwd: this.repoRoot, env: devToolingEnv({ STORYBOOK_SCOPE: "compose/algorithm" }) });
  }
}
