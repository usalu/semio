...
import { fixtureItemsOf } from "@semio-tech/repo-lib";
...
export async function dragPieces(kit: unknown, rawDesign: Design, pieceIds: readonly string[], offset: CoordinatePlain): Promise<{ inputDesign: Design; output: Design; dragDiff: DesignDiff }> {
  ...
  const designs = fixtureItemsOf<Record<string, unknown>>(surface["designs"]).map((d) => (String(d["id"] ?? "") === designId ? __plainFromDesign(updatedRaw) : d));
...
export function mergeKitDesigns(kit: unknown, ...extraDesigns: Record<string, unknown>[]): unknown {
  ...
  const designs = [...fixtureItemsOf<Record<string, unknown>>(surface["designs"]), ...extraDesigns];
  surface["designs"] = designs;
...
if (import.meta.vitest) {
  ...
  describe("compose-algorithms public surface", () => {
    it("exports async kit runners used by compose algorithm stories", () => {
