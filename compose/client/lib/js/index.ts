...
async function readComposeWasmBytesFromMonorepoCandidates(): Promise<Uint8Array | undefined> {
  try {
...
function isVitestOrEmbeddedTestRuntime(): boolean {
  try {
    ...
    if (env) {
      ...
      if (env["COMPOSE_JS_RUN_EMBEDDED_TESTS"] === "1" || env["COMPOSE_JS_RUN_EMBEDDED_TESTS"] === true) return true;
      if (env["COMPOSE_REACT_RUN_EMBEDDED_TESTS"] === "1" || env["COMPOSE_REACT_RUN_EMBEDDED_TESTS"] === true) return true;
    }
  } catch {
    /* ignore */
  }
  ...
  if (pe) {
    ...
    if (pe["COMPOSE_JS_RUN_EMBEDDED_TESTS"] === "1") return true;
    if (pe["COMPOSE_REACT_RUN_EMBEDDED_TESTS"] === "1") return true;
  }
  return false;
}
...
function installKitFieldMethods<E extends KitPathEntity>(ctor: abstract new (...args: never[]) => E, specs: readonly BoundKitFieldSpec<unknown, E>[]): void {
  for (const spec of specs) {
    ...
    Object.defineProperty(ctor.prototype, method, {
      configurable: true,
      value: async function composeKitField(this: E): Promise<unknown> {
...
function installNodeFieldMethods<E extends Entity>(ctor: abstract new (...args: never[]) => E, typename: string, specs: readonly BoundNodeFieldSpec<unknown>[]): void {
  for (const spec of specs) {
    ...
    Object.defineProperty(ctor.prototype, method, {
      configurable: true,
      value: function composeNodeField(this: E): Promise<unknown> {
...
function installKitOperationMethods<E extends KitPathEntity>(ctor: abstract new (...args: never[]) => E, specs: readonly BoundKitOperationSpec<E>[]): void {
  for (const spec of specs) {
    Object.defineProperty(ctor.prototype, spec.method, {
      configurable: true,
      value: async function composeKitOperation(this: E, ...args: readonly unknown[]): Promise<SetResult> {
...
function installKitEventMethods<E extends KitPathEntity>(ctor: abstract new (...args: never[]) => E, specs: readonly BoundKitFieldSpec<unknown, E>[]): void {
  ...
  for (const spec of specs) {
    ...
    Object.defineProperty(ctor.prototype, eventMethod, {
      configurable: true,
      value: function composeKitFieldEvent(this: E, cb: (next: unknown) => void): Unsubscribe {
        const run = (): void => {
...
function installEntityNodeMethods<E extends Entity>(ctor: abstract new (...args: never[]) => E, typename: string, fields: readonly BoundNodeFieldSpec<unknown>[]): void {
  ...
  for (const spec of fields) {
    ...
    Object.defineProperty(ctor.prototype, eventMethod, {
      configurable: true,
      value: function composeNodeFieldEvent(this: E, cb: (next: unknown) => void): Unsubscribe {
...
function installStoreBranchFieldMethods<E extends StoreBranchEntity>(ctor: abstract new (...args: never[]) => E, specs: readonly BoundStoreBranchFieldSpec<unknown, E>[]): void {
  ...
  for (const spec of specs) {
    ...
    Object.defineProperty(ctor.prototype, method, {
      configurable: true,
      value: async function composeStoreBranchField(this: E): Promise<unknown> {
    ...
    Object.defineProperty(ctor.prototype, eventMethod, {
      configurable: true,
      value: function composeStoreBranchFieldEvent(this: E, cb: (next: unknown) => void): Unsubscribe {
        const run = (): void => {
...
function installEntityScopedNodeMethods<E extends ScopedNodeEntity>(ctor: abstract new (...args: never[]) => E, fields: readonly BoundScopedNodeFieldSpec<unknown>[]): void {
  ...
  for (const spec of fields) {
    ...
    Object.defineProperty(ctor.prototype, method, {
      configurable: true,
      value: async function composeScopedNodeField(this: E): Promise<unknown> {
    ...
    Object.defineProperty(ctor.prototype, eventMethod, {
      configurable: true,
      value: function composeScopedNodeFieldEvent(this: E, cb: (next: unknown) => void): Unsubscribe {
        const run = (): void => {
          const read = (this as unknown as Record<string, () => Promise<unknown>>)[method];
          if (typeof read !== "function") return;
          void read.call(this).then(cb);
...
function installWeakKitFieldMethods<E extends object>(
  ...
  for (const spec of specs) {
    Object.defineProperty(ctor.prototype, spec.method, {
      configurable: true,
      value: async function composeWeakKitField(this: E): Promise<unknown> {
    ...
    Object.defineProperty(ctor.prototype, eventMethod, {
      configurable: true,
      value: function composeWeakKitFieldEvent(this: E, cb: (next: unknown) => void): Unsubscribe {
        const run = (): void => {
...
function parseFileSystemChildrenRefs(frag: JsonObject | null | undefined): readonly ComposeFileSystemChildRef[] {
  ...
  return Object.freeze(
    edges.map((edge) => {
      ...
      } satisfies ComposeFileSystemChildRef;
    }),
  );
}
...
type ComposeRelayVfsBranchSpec = {
  readonly connectionKey: string;
  readonly kind: string;
  readonly nameFrom: (node: JsonObject) => string;
  readonly hasChildren?: (node: JsonObject) => boolean;
  readonly mapChild?: (child: ComposeFileSystemChildRef, node: JsonObject) => ComposeFileSystemChildRef;
...
function composeRelayVfsChildrenFromFrag(frag: JsonObject | null | undefined, branches: readonly ComposeRelayVfsBranchSpec[]): readonly ComposeFileSystemChildRef[] {
  const out: ComposeFileSystemChildRef[] = [];
  for (const branch of branches) {
    ...
    for (const edge of edges) {
      ...
      let child: ComposeFileSystemChildRef = {
        id,
        kind: branch.kind,
        name,
        path: `/${name}`,
...
function composeRelayEdgesNonEmpty(node: JsonObject, key: string): boolean {
  const edges = (node[key] as JsonObject | undefined)?.["edges"];
  return Array.isArray(edges) && edges.length > 0;
}

function composeFileSystemChildrenInner(parent: ComposeFileSystemParentRef): string {
  switch (parent.kind) {
    case "KIT":
      return `
...
function composeParseRelayVfsChildren(parent: ComposeFileSystemParentRef, frag: JsonObject | null | undefined): readonly ComposeFileSystemChildRef[] {
  switch (parent.kind) {
    case "KIT":
      return composeRelayVfsChildrenFromFrag(frag, [
        {
          connectionKey: "hasTypologies",
          kind: "TYPOLOGY",
          nameFrom: (node) => String(node["name"] ?? node["id"] ?? ""),
          hasChildren: (node) => composeRelayEdgesNonEmpty(node, "hasTypes") || composeRelayEdgesNonEmpty(node, "hasDesigns"),
        },
        ...
        },
        {
          ...
          hasChildren: () => false,
          skipNode: composeKitRootFileHiddenFromVfs,
        },
      ]);
    case "TYPOLOGY": {
      const topo = readKitPathNode(frag, ["typology"]);
      return composeRelayVfsChildrenFromFrag(topo, [
        {
        ...
        },
        {
          ...
          kind: "DESIGN",
          nameFrom: (node) => String(node["name"] ?? node["id"] ?? ""),
          hasChildren: (node) => composeRelayEdgesNonEmpty(node, "hasPieces") || composeRelayEdgesNonEmpty(node, "hasConnections"),
    ...
    case "FOLDER": {
      ...
      return composeRelayVfsChildrenFromFrag(folder, [
        {
    ...
    case "DESIGN": {
      ...
      return composeRelayVfsChildrenFromFrag(design, [
        {
    ...
    case "TYPE": {
      ...
      return composeRelayVfsChildrenFromFrag(type, [
        {
...
export class Session {
  ...
  static async open(uri: string, opts?: SessionOpenOptions): Promise<Session> {
    ...
    const wasmSpecifier = opts?.wasmSpecifier ?? (globalThis as { __COMPOSE_WASM_SPECIFIER__?: string }).__COMPOSE_WASM_SPECIFIER__;
...
if (typeof process !== "undefined" && !!process.env && process.env["COMPOSE_JS_RUN_EMBEDDED_TESTS"] === "1") {
  ...
  describe("compose/js", () => {
    ...
    it("fetchComposeFileSystemChildren selects kit relay fields at kit root", () => {
      const inner = composeFileSystemChildrenInner({ kind: "KIT", id: "kit-1" });
      expect(inner).toContain("hasTypologies");
      expect(inner).not.toContain("fileSystemChildren");
      const topoInner = composeFileSystemChildrenInner({ kind: "TYPOLOGY", id: "topo-1" });
  ...
  describe("compose/js e2e", () => {
    it("fetchComposeFileSystemChildren returns typologies at kit root after installProjection", async () => {
      ...
      const fixturePath = join(dirname(fileURLToPath(import.meta.url)), "../../../fixture/nakagin-capsule-tower.filtered.kit.compose.json");
      ...
      try {
        ...
        const children = await eventually(
          () => fetchComposeFileSystemChildren(store, { kind: "KIT", id: kitId }),
    ...
    it("fetchComposeFileSystemChildren lists capsule kinds under typology-capsule", async () => {
      const { readFile } = await import("node:fs/promises");
      const { dirname, join } = await import("node:path");
      const { fileURLToPath } = await import("node:url");
      const fixturePath = join(dirname(fileURLToPath(import.meta.url)), "../../../fixture/architect.harness.kit.compose.json");
      const session = await Session.openInMemory({ timeoutMs: 120_000 });
      try {
        const store = (await session.stores())[0]!;
        const installed = await store.installProjection(await readFile(fixturePath, "utf8"));
        ...
        const children = await eventually(
          () => fetchComposeFileSystemChildren(store, { kind: "TYPOLOGY", id: capsuleTypologyId }),
    ...
    it("installProjection preserves metabolism file names for vfs", async () => {
      ...
      const { dirname, join } = await import("node:path");
      const { fileURLToPath } = await import("node:url");
      const fixturePath = join(dirname(fileURLToPath(import.meta.url)), "../../../fixture/metabolism.shallow.kit.compose.json");
      ...
      try {
        ...
        const kitRoot = await eventually(
          () => fetchComposeFileSystemChildren(store, { kind: "KIT", id: kit.id }),
