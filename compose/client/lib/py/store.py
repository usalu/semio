"""
JSON-RPC 2.0 (NDJSON) client for the :mod:`compose` ``compose-store`` sidecar.
Transport matches :file:`compose/store/jsonrpc.rs` and the wasm ``KitStoreHandle`` method surface.
"""
...
_DEFAULT_EXE = "compose-store.exe" if sys.platform == "win32" else "compose-store"
...
class StoreClient:
    ...
    def __init__(self, binary: str | None = None) -> None:
        self._binary = binary or resolve_compose_store_binary()
    ...
    def _ensure_proc(self) -> None:
        with self._lock:
            ...
            if os.environ.get("COMPOSE_STORE_ENABLE_EVENTS", "").lower() not in (
                "1",
                "true",
                "yes",
            ):
                uenv["COMPOSE_STORE_NO_EVENTS"] = "1"
...
def load_kit_via_io(
    ...
    (``COMPOSE_STORE_NO_EVENTS=1``; no ``event`` flood). ``method`` is e.g. ``"io.importFromFile"``.
    """
    b = binary or resolve_compose_store_binary()
