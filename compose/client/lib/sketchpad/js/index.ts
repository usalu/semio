...
function applyComposeSketchpadToolbarParentEntries(bundles: Record<UiLocale, { translation: ComposeSketchpadTranslationTree }>): void {
...
applyComposeSketchpadToolbarParentEntries(composeSketchpadTranslationBundles as Record<UiLocale, { translation: ComposeSketchpadTranslationTree }>);
...
export async function sketchpadPickKitImportFile(): Promise<File | null> {
  ...
  if ("showOpenFilePicker" in window) {
    try {
      const handles = await (window as Window & { showOpenFilePicker: (o: unknown) => Promise<FileSystemFileHandle[]> }).showOpenFilePicker({
        multiple: false,
...
export async function sketchpadBrowserFolderKitFactory(): Promise<ComposeJsKitStore> {
  ...
  const kitFile =
    (await dir
      .getFileHandle("kit.compose.json", { create: false })
      .then((h) => h.getFile())
      .catch(() => null)) ??
    (await dir
      .getFileHandle("wip/initialKit/kit.compose.json", { create: false })
      .then((h) => h.getFile())
      .catch(() => null));
...
export async function openSketchpadKitFromImport(data: ArrayBuffer | Blob | File | string, options?: { readonly kind?: SketchpadKitPersistenceKind; readonly navigate?: boolean; readonly assetBaseUrl?: string }): Promise<string> {
  ...
  const assetBaseUrl = options?.assetBaseUrl ?? (typeof data === "string" ? sketchpadKitAssetBaseUrlFromImportUrl(data) : undefined);
  ...
  const parseDesigns = (): Design[] => {
    ...
    return edges
      ...
      .map((node) => {
        ...
        const connections = connectionEdges
          .map((ce) => {
            const raw = ce.node;
...
function sketchpadTypologyDesignDestinations(kitId: string, typology: ReturnType<typeof sketchpadKitTypologyRows>[number]): NavigationDestination[] {
  ...
  for (const design of typology.designs) {
    ...
    out.push(sketchpadNavigationDestination(`sketchpad.nav.design.${designId}`, sketchpadKitRowEntityName(design, designId), `/kits/${kitId}/design/${designId}`));
  }
  return out;
}
...
function sketchpadTypologyTypeDestinations(kitId: string, typology: ReturnType<typeof sketchpadKitTypologyRows>[number]): NavigationDestination[] {
  ...
  for (const type of typology.types) {
    ...
    if (!typeId) continue;
    out.push(sketchpadNavigationDestination(`sketchpad.nav.type.${typeId}`, sketchpadKitRowEntityName(type, typeId), `/kits/${kitId}/type/${typeId}`));
  }
  return out;
}
...
function sketchpadKitVfsPushFolderRow(rows: VirtualFileSystemNodeRecord[], kitId: string, parentId: string, folder: Record<string, unknown>): void {
...
function sketchpadKitVfsPushFileRow(rows: VirtualFileSystemNodeRecord[], kitId: string, parentId: string, file: Record<string, unknown>): void {
  const id = String(file["id"] ?? "");
  if (!id) return;
...
export function sketchpadMergeKitDtoFromBundleProjection(target: Kit, source: Kit): Kit {
  ...
  const files = sourceFiles.length === 0 ? targetFiles : [...targetFiles, ...sourceFiles.filter((file) => !targetFiles.some((row) => row.id === file.id))];
  const sourceTypes = sketchpadKitTypeRows(source);
  const types = sketchpadKitTypeRows(target).map((type) =>
    sketchpadMergeKitTypeRepresentations(
      type,
      sourceTypes.find((row) => row.id === type.id),
    ),
  );
  const targetTypologies = sketchpadKitTypologyRows(target);
  const sourceTypologies = sketchpadKitTypologyRows(source);
  const typologies = sourceTypologies.length === 0 ? (target as { typologies?: unknown }).typologies : targetTypologies.length === 0 ? (source as { typologies?: unknown }).typologies : (target as { typologies?: unknown }).typologies;
  ...
  const designs =
    targetDesignRows.length === 0
      ? sourceDesignRows
      : targetDesignRows.map((design) =>
          sketchpadMergeKitDesignRows(
            design,
            sourceDesignRows.find((row) => row.id === design.id),
          ),
        );
...
function sketchpadFlatPartCenterFromTopLeft(position: { readonly x: number; readonly y: number }, frame: { readonly width: number; readonly height: number }): { x: number; y: number } {
...
export function sketchpadKitEntityAvailableIcon(kit: Kit, nodeId: string, fileNodeKindId: string): string | undefined {
  switch (fileNodeKindId) {
    case "kit":
      return sketchpadKitEntityIconField((kit as { icon?: unknown }).icon) ?? sketchpadKitEntityIconField((kit.metadata as { icon?: unknown } | undefined)?.icon);
    case "type":
...
function sketchpadKitWiresIdentityKindCatalogRow(kit: Kit, node: VirtualFileSystemVisibleNode): NonNullable<WiresFixture["kindCatalogs"]>["identityKinds"][number] {
...
function sketchpadExpandedDesignIds(scope: VirtualFileSystemScope, expandedIds: readonly string[], vfsMeta: ReadonlyMap<string, { readonly fileNodeKindId: string }>): ReadonlySet<string> {
...
function sketchpadCollectDesignTransitiveTypesFromDto(design: Design, kit: Kit, rootDesignId: string): readonly string[] {
  ...
  const visit = (row: Design) => {
    for (const piece of row.pieces ?? []) {
      ...
      const nestedId = (piece as { design?: { id?: string } }).design?.id ?? (piece as { ref?: { design?: { id?: string } } }).ref?.design?.id;
...
export function sketchpadReadKitFamilyRows(kit: Kit): readonly Record<string, unknown>[] {
  ...
  if (raw == null) return [];
  const asRow = (entry: unknown): entry is Record<string, unknown> => entry != null && typeof entry === "object" && !Array.isArray(entry);
...
export function sketchpadExtractPortCompatById(kit: Kit): Map<string, readonly { readonly id: string }[]> {
  ...
  for (const port of sketchpadCollectKitPortRecords(kit)) {
    ...
    if (id && compatIds.length > 0)
      map.set(
        id,
        compatIds.map((compatId) => ({ id: compatId })),
      );
  }
  return map;
}
...
function sketchpadKitDiagramPushEdge(edges: SketchpadPuzzle2dFixture["edges"], edgeIds: Set<string>, id: string, source: string, target: string): void {
  if (edgeIds.has(id)) return;
  edgeIds.add(id);
  edges.push({ id, source, target });
}

function sketchpadKitDiagramNode(kind: SketchpadKitDiagramNodeKind, entityId: string, label: string, root: boolean): { node: SketchpadPuzzle2dFixture["nodes"][number]; center: { x: number; y: number } } {
...
export function sketchpadKitPuzzle2dFixtureFromKit(kit: Kit): SketchpadPuzzle2dFixture {
  ...
  for (const kind of kindGroups) {
    ...
    for (const item of items) {
      ...
      if (item.parentId) {
        ...
        sketchpadKitDiagramPushEdge(edges, edgeIds, `${kind}-${item.parentId}-${item.id}`, `${parentKind}:${item.parentId}`, `${kind}:${item.id}`);
      }
    }
  }
  for (const design of kit.designs ?? []) {
    for (const piece of design.pieces ?? []) {
      ...
      if (typeId) {
        sketchpadKitDiagramPushEdge(edges, edgeIds, `ref-type:${typeId}-design:${design.id}`, `type:${typeId}`, `design:${design.id}`);
      }
    }
  }
  for (const type of kit.types ?? []) {
    for (const connector of type.connectors ?? []) {
      const portId = sketchpadReadEntityId((connector as { port?: unknown }).port);
      if (!portId) continue;
      sketchpadKitDiagramPushEdge(edges, edgeIds, `ref-port:${portId}-type:${type.id}`, `port:${portId}`, `type:${type.id}`);
    }
  }
...
function sketchpadConnectionEndpoints(connection: SketchpadKitConnection): {
  ...
  const sourcePieceId = sketchpadReadEntityId(connection.connecting?.piece) ?? sketchpadReadEntityId(connection.parent?.piece);
  const targetPieceId = sketchpadReadEntityId(connection.connected?.piece) ?? sketchpadReadEntityId(connection.child?.piece);
  const sourceConnectorId = sketchpadReadEntityId(connection.connecting?.connector) ?? sketchpadReadEntityId(connection.parent?.connector);
  const targetConnectorId = sketchpadReadEntityId(connection.connected?.connector) ?? sketchpadReadEntityId(connection.child?.connector);
...
export function sketchpadApplyPuzzle2dSelection(instanceId: string, puzzle2dIds: readonly string[], controller?: SketchpadShellController): void {
  ...
  if (scope.pane === "diagram" || scope.pane === "scene") {
    ...
    const connectionIdSet = new Set((((design as { connections?: readonly SketchpadKitConnection[] } | undefined)?.connections ?? []) as readonly SketchpadKitConnection[]).map((connection) => connection.id).filter((id): id is string => Boolean(id)));
...
function sketchpadTopologyPayloadForDesignScene(design: Design, kit?: Kit): PlatformTopologyPayload {
  return sketchpadTopologyPayload(sketchpadDesignPuzzle2dFixtureFromDesign(design, kit), sketchpadDesignVolumeFixtureFromDesign(design, kit));
}
...
function sketchpadVfsNavigateUri(kitId: string, route: ReturnType<typeof parseSketchpadRouteScopeFromPath>, fileNodeKindId: string, nodeId: string, child: ComposeFileSystemChildRef): string | undefined {
  switch (fileNodeKindId) {
    case "type":
      return `/kits/${kitId}/type/${nodeId}`;
    case "design":
...
function sketchpadVfsRecordsFromRsChildren(kitId: string, route: ReturnType<typeof parseSketchpadRouteScopeFromPath>, parentId: string, children: readonly ComposeFileSystemChildRef[]): readonly VirtualFileSystemNodeRecord[] {
...
function sketchpadKitTypologyRows(kit: Kit): readonly { readonly id: string; readonly name: string; readonly types: readonly unknown[]; readonly designs: readonly unknown[] }[] {
  ...
  const items = Array.isArray(block) ? block : block && typeof block === "object" && Array.isArray((block as { items?: unknown[] }).items) ? (block as { items: unknown[] }).items : [];
  return items
    .map((topo) => {
      ...
      const types = Array.isArray(typesBlock) ? typesBlock : typesBlock && typeof typesBlock === "object" && Array.isArray((typesBlock as { items?: unknown[] }).items) ? (typesBlock as { items: unknown[] }).items : [];
      const designs = Array.isArray(designsBlock) ? designsBlock : designsBlock && typeof designsBlock === "object" && Array.isArray((designsBlock as { items?: unknown[] }).items) ? (designsBlock as { items: unknown[] }).items : [];
...
function sketchpadKitVfsChildren(kit: Kit, parentId: string): readonly VirtualFileSystemNodeRecord[] {
  ...
  if (parentId === kitId) {
    ...
    for (const type of kit.types ?? []) {
      ...
      const typeHasChildren = (t.representations?.length ?? 0) > 0 || (t.ports?.length ?? 0) > 0 || (t.connectors?.length ?? 0) > 0;
...
function sketchpadHomeVfsChildren(openKitIds: readonly string[], kitById: (kitId: string) => Kit | undefined, kitKind: (kitId: string) => string, home: SketchpadHomeUiState, parentId: string): readonly VirtualFileSystemNodeRecord[] {
    ...
    kitEntries.sort((left, right) => {
      ...
      switch (column) {
        ...
        case "kind":
          ...
          break;
        case "updated":
          comparison = sketchpadFormatKitTimestamp(left.kit.updatedAt ?? left.kit.createdAt).localeCompare(sketchpadFormatKitTimestamp(right.kit.updatedAt ?? right.kit.createdAt));
          break;
        default:
          comparison = (left.kit.name ?? left.kitId).localeCompare(right.kit.name ?? right.kitId);
      }
...
export function sketchpadSyncTypeAppChrome(platform: Platform): void {
  ...
  if (representations.length === 0) {
    ...
    typeApp.windowKinds = representations.map((rep, index) => new WindowKindRuntime(windowKindIds[index]!, labels[index]!, SKETCHPAD_BODY_TYPE_REP));
  ...
  constructor(componentKind: ComponentKind, surfaceId: string, controllerId: string, initialSnapshot: TSnapshot, platform: Platform) {
    ...
    this.detachRoute = platform.subscribe(() => {
      ...
      const routeStructureChanged =
        nextRoute.kitId !== this.route.kitId || nextRoute.designId !== this.route.designId || nextRoute.typeId !== this.route.typeId || nextRoute.docsPath !== this.route.docsPath || nextRoute.qualityId !== this.route.qualityId;
...
function sketchpadKitWiresPreselectSnapshotsEqual(left: SketchpadKitWiresPreselectSnapshot, right: SketchpadKitWiresPreselectSnapshot): boolean {
...
export class SketchpadKitWires extends SketchpadRoutedComponent<Puzzle5dModel> {
  ...
  constructor(platform: Platform) {
    super("puzzle5d", SKETCHPAD_SURFACE_KIT_WIRES, SKETCHPAD_SHELL_CONTROLLER_ID, { presentation: "flat", instanceId: SKETCHPAD_SURFACE_KIT_WIRES }, platform);
    const ctrl = getSketchpadShellController();
    const shellStore = ctrl?.getStore<SketchpadShellSnapshot>(SKETCHPAD_SHELL_STORE_SHELL);
    if (!ctrl || !shellStore) return;
    this.detachKitWiresHover = ctrl.kitWiresHoverStore.subscribe(() => {
...
export class SketchpadTypeRepresentationScene extends SketchpadRoutedComponent<Puzzle5dModel> {
  ...
  constructor(platform: Platform, surfaceId: string, representationId: string) {
    super("puzzle5d", surfaceId, SKETCHPAD_SHELL_CONTROLLER_ID, { presentation: "volume", instanceId: surfaceId }, platform);
...
function buildSketchpadCataloguePanelBody(ctx: WindowBodyViewContext): UiTreeNode {
  ...
  const kit = ctrl?.getKitStore(kitId)?.getSnapshot().kit;
  if (!kit) {
    return uiDeclarativeSectionsToTree([{ type: "section", id: "sketchpad.catalogue.loading", label: FRAMEWORK_PANEL_TAB_CATALOGUE_LABEL, children: [{ type: "text", value: "Kit loading…" }] }]);
  }
...
function buildSketchpadInspectionPanelBody(ctx: WindowBodyViewContext): UiTreeNode {
  ...
  const kit = ctrl?.getKitStore(kitId)?.getSnapshot().kit;
  if (!kit) {
    return uiDeclarativeSectionsToTree([{ type: "section", id: "sketchpad.inspection.loading", label: FRAMEWORK_PANEL_TAB_INSPECTION_LABEL, children: [{ type: "text", value: "Kit loading…" }] }]);
  }
  if (designId && selection.pieceIds.length > 0) {
    const design = findDesignInKit(kit, designId);
    const pieces = selection.pieceIds.map((pieceId) => findPieceInDesign(design!, pieceId)).filter((piece): piece is NonNullable<typeof piece> => piece != null);
  ...
  if (children.length === 1 && selection.pieceIds.length === 0 && selection.connectionIds.length === 0 && selection.kitWiresNodeIds.length === 0 && !typeId && !qualityId) {
...
export class SketchpadShellController extends VirtualFileSystemController {
  ...
  private readonly vfsNodeMetaByScope = new Map<string, Map<string, { readonly fileNodeKindId: string; readonly typeId?: string; readonly designId?: string }>>();
  ...
  syncTopologyForSurface(surfaceId: string, route: { readonly kitId: string | null; readonly designId: string | null; readonly typeId: string | null }): void {
    ...
    if (typeRepSurface && typeRepSurface.kitId === kitId && typeRepSurface.typeId === typeId) {
      const type = findTypeInKit(kit, typeId);
      const representation = type ? sketchpadListTypeRepresentations(type).find((row) => row.id === typeRepSurface.representationId) : undefined;
      if (type && representation) {
        this.upsertTopologyStore(sketchpadTypeRepresentationSceneInstanceId(kitId, typeId, representation.id), sketchpadTopologyPayloadForTypeRepresentation(type, representation, kit));
      }
      return;
    }
  ...
  navigateTo(path: string): void {
    ...
    const routeSelection = sketchpadPathSupportsRouteSelectionQuery(pathOnly) ? parseSketchpadRouteSelectionQuery(path) : sketchpadEmptyRouteSelection();
  ...
  protected override loadChildrenAsync(parentId: string, scope: VirtualFileSystemScope): Promise<readonly VirtualFileSystemNodeRecord[]> {
    const route = parseSketchpadRouteScopeFromPath(this.shellStore.get().navigationPath);
  ...
  private vfsMetaForScope(scope: VirtualFileSystemScope): ReadonlyMap<string, { readonly fileNodeKindId: string; readonly typeId?: string; readonly designId?: string }> {
    return this.vfsNodeMetaByScope.get(virtualFileSystemScopeKey(scope)) ?? new Map();
  }

  private rememberVfsNodes(scope: VirtualFileSystemScope, rows: readonly VirtualFileSystemNodeRecord[], route: ReturnType<typeof parseSketchpadRouteScopeFromPath>): void {
    ...
    for (const row of rows) {
      ...
      const designId = row.fileNodeKindId === "piece" || row.fileNodeKindId === "connection" ? (parentMeta?.fileNodeKindId === "design" ? row.parentId! : (route.designId ?? parentMeta?.designId)) : (route.designId ?? parentMeta?.designId);
      map.set(row.id, {
  ...
  override run(action: string, args?: unknown): void {
    ...
    switch (action) {
      case "setNavigation": {
        ...
        const pathOnly = path.split("?")[0] ?? "/";
        const home = pathOnly === "/" ? parseSketchpadHomeQuery(path) : shell.home;
        const routeSelection = pathOnly === "/" ? sketchpadEmptyRouteSelection() : sketchpadPathSupportsRouteSelectionQuery(pathOnly) ? parseSketchpadRouteSelectionQuery(path) : sketchpadEmptyRouteSelection();
...
function registerSketchpadWindowBodies(): void {
  ...
  registerWindowBody(SKETCHPAD_BODY_KIT_VFS, () => buildVirtualFileSystemWindowBody(SKETCHPAD_SURFACE_KIT_VFS, SKETCHPAD_SHELL_CONTROLLER_ID, "vfs"));
  registerWindowBody(SKETCHPAD_BODY_KIT_WIRES, () => buildPuzzle5dWindowBody(SKETCHPAD_SURFACE_KIT_WIRES, SKETCHPAD_SHELL_CONTROLLER_ID, "wires"));
  registerWindowBody(SKETCHPAD_BODY_DESIGN_SCENE, () => buildPuzzle5dWindowBody(SKETCHPAD_SURFACE_DESIGN_SCENE, SKETCHPAD_SHELL_CONTROLLER_ID, "scene"));
  registerWindowBody(SKETCHPAD_BODY_DESIGN_DIAGRAM, () => buildPuzzle5dWindowBody(SKETCHPAD_SURFACE_DESIGN_DIAGRAM, SKETCHPAD_SHELL_CONTROLLER_ID, "diagram"));
...
export function sketchpadNavigation(platform: Platform, uri: string): NavigationLevel[] {
  ...
  if (pathOnly === "/feedback" || pathOnly.startsWith("/feedback/")) {
    return [homeLevel, sketchpadNavigationLevel(sketchpadNavigationDestination("sketchpad.nav.feedback", "Feedback", "/feedback"), [])];
  }

  if (pathOnly.startsWith("/docs")) {
    const registry = sketchpadBuildDocsRegistry();
    const sectionAlternatives = registry.map((section) => sketchpadNavigationDestination(`sketchpad.nav.docs.section.${section.id}`, section.label, `/doc/${section.pages[0]?.path ?? section.id}`));
    const trail: NavigationLevel[] = [homeLevel, sketchpadNavigationLevel(sketchpadNavigationDestination("sketchpad.nav.documentation", "Documentation", "/docs"), sectionAlternatives)];
    const sectionId = scope.docsPath.split("/")[0] ?? "";
    const section = registry.find((entry) => entry.id === sectionId);
    if (section) {
      trail.push(sketchpadNavigationLevel(sketchpadNavigationDestination(`sketchpad.nav.docs.section.${section.id}`, section.label, `/doc/${section.pages[0]?.path ?? section.id}`), sectionAlternatives));
      const page = section.pages.find((entry) => entry.path === scope.docsPath) ?? section.pages[0];
      if (page && scope.docsPath !== section.pages[0]?.path) {
        trail.push(sketchpadNavigationLevel(sketchpadNavigationDestination(`sketchpad.nav.docs.page.${page.path}`, page.title, `/doc/${page.path}`), []));
      }
    }
    return trail;
  }
  ...
  const trail: NavigationLevel[] = [
    ...
    sketchpadNavigationLevel(sketchpadNavigationDestination(`sketchpad.nav.kit.${scope.kitId}`, kitName, kitUri), [sketchpadNavigationDestination("sketchpad.nav.typologies", "Typologies", kitUri)]),
  ];

  const typologies = kit ? sketchpadKitTypologyRows(kit) : [];
  const typologyAlternatives = typologies.map((typology) => sketchpadNavigationDestination(`sketchpad.nav.typology.${typology.id}`, typology.name, kitUri));
  ...
  if (scope.designId && kit) {
    ...
    if (typology && typologies.length > 0) {
      ...
      trail.push(
        ...
        sketchpadNavigationLevel(sketchpadNavigationDestination(`sketchpad.nav.typology.${typology.id}`, typology.name, kitUri), typologySectionAlternatives),
        sketchpadNavigationLevel(sketchpadNavigationDestination("sketchpad.nav.designs", "Designs", kitUri), sketchpadTypologyDesignDestinations(scope.kitId, typology)),
      ...
      const designAlternatives = (kit.designs ?? [])
        ...
        .map((row) => sketchpadNavigationDestination(`sketchpad.nav.design.${row.id}`, row.name ?? String(row.id), `/kits/${scope.kitId}/design/${row.id}`));
      trail.push(
  ...
  if (scope.typeId && kit) {
    ...
    if (typology && typologies.length > 0) {
      ...
      trail.push(
        sketchpadNavigationLevel(sketchpadNavigationDestination("sketchpad.nav.typologies", "Typologies", kitUri), typologyAlternatives),
        sketchpadNavigationLevel(sketchpadNavigationDestination(`sketchpad.nav.typology.${typology.id}`, typology.name, kitUri), typologySectionAlternatives),
        sketchpadNavigationLevel(sketchpadNavigationDestination("sketchpad.nav.types", "Types", kitUri), sketchpadTypologyTypeDestinations(scope.kitId, typology)),
      ...
      const typeAlternatives = (kit.types ?? [])
        ...
        .map((row) => sketchpadNavigationDestination(`sketchpad.nav.type.${row.id}`, row.name ?? String(row.id), `/kits/${scope.kitId}/type/${row.id}`));
      trail.push(
...
if (import.meta.vitest) {
  ...
  describe("Sketchpad virtual file system", () => {
    ...
    it("maps rs vfs records from graphql child refs", () => {
      const rows = sketchpadVfsRecordsFromRsChildren("kit-1", parseSketchpadRouteScopeFromPath("/kits/kit-1"), "kit-1", [
        {
          id: "type-1",
          kind: "TYPE",
          name: "Window",
    ...
    it("home vfs kit and docs page rows expose navigateUri", () => {
      ...
      const docsSection = sketchpadHomeVfsChildren(
        ["metabolism-id"],
        () => kit,
        () => "temporary",
        sketchpadEmptyHomeUiState(),
      ...
      const pages = sketchpadHomeVfsChildren(
        ["metabolism-id"],
        () => kit,
        () => "temporary",
        sketchpadEmptyHomeUiState(),
  ...
  describe("sketchpad preload kits", () => {
    it("parses comma- and whitespace-separated preload kit URLs", () => {
      expect(sketchpadPreloadKitUrls("/a/kit.compose.json, /b/kit.compose.json")).toEqual(["/a/kit.compose.json", "/b/kit.compose.json"]);
  ...
  describe("sketchpadKitWiresVisibleNodes", () => {
    it("prepends kit root when the platform helper omits it", () => {
      ...
      const visible: VirtualFileSystemVisibleNode[] = [{ id: typeId, fileNodeKindId: "type", name: "T", path: "/T", parentId: kitId, hasChildren: false }];
  ...
  describe("sketchpadKitWiresFixtureFromVisible", () => {
    it("emits one identity per visible vfs node and owns containment edges", () => {
      ...
      const fixture = sketchpadKitWiresFixtureFromVisible(kit, kitId, visible, { designTransitiveTypes: new Map(), designTransitiveDesigns: new Map(), pieceBlueprints: new Map() }, new Set());
    ...
    it("uses references for collapsed design and has+is when design is expanded", () => {
      ...
      const visibleExpanded: VirtualFileSystemVisibleNode[] = [...visibleCollapsed, { id: pieceId, fileNodeKindId: "piece", name: "P", path: "/P", parentId: designId, hasChildren: false }];
  ...
  describe("sketchpadSyncTypeAppChrome", () => {
    it("creates one window kind per representation", () => {
      ...
      platform.addApp(new AppRuntime(SKETCHPAD_TYPE_APP_ID, "Type", undefined, ctrl, createTabStackLayout(["type-empty"], ["Type"]), [new WindowKindRuntime("type-empty", "Type", SKETCHPAD_BODY_TYPE_REP)]));
      ctrl.registerKitStore(kitId, new InMemoryComposeKitStore(kit));
  ...
  describe("sketchpadApplyPortCompatById", () => {
    ...
    it("reads port compat from kit families and wires type adjacency via connectors", () => {
      ...
      const merged = sketchpadApplyPortCompatById(graphqlKit, compat);
      expect(
        sketchpadCollectKitPorts(merged)
          .map((p) => p.id)
          .sort(),
      ).toEqual(["p1", "p2"]);
      const fixture = sketchpadKitPuzzle2dFixtureFromKit(merged);
  ...
  describe("parseSketchpadRouteSelectionQuery", () => {
    it("reads piece connection and diagram ids from query params", () => {
      const kitId = "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee";
      const designId = "11111111-2222-3333-4444-555555555555";
      const selection = parseSketchpadRouteSelectionQuery(`/kits/${kitId}/design/${designId}?piece=p1&piece=p2&conn=c1&wires=type:t1`);
  ...
  describe("Sketchpad inspection panel", () => {
    ...
    it("reflects kit virtual file system row selection", async () => {
      ...
      const platform = await buildSketchpadPlatform();
      const ctrl = getSketchpadShellController()!;
      ctrl.registerKitStore(kitId, new InMemoryComposeKitStore({ id: kitId, name: "Kit", types: [{ id: typeId, name: "Column" }] } as Kit));
  ...
  describe("Sketchpad home virtual file system", () => {
    it("lists open kits under the home root", async () => {
      const platform = await buildSketchpadPlatform();
      const ctrl = getSketchpadShellController()!;
      ctrl.registerKitStore("k-home", new InMemoryComposeKitStore({ id: "k-home", name: "Demo Kit", version: "r1", updatedAt: "2025-06-01T12:00:00.000Z" } as Kit), { kind: "fixture" });
      ctrl.navigateTo("/");
      platform.uri = "/";
      const scope = sketchpadVfsScope(SKETCHPAD_KIT_APP_ID);
  ...
  describe("sketchpadCreatePortGroupMap", () => {
    it("unions ports linked by compatiblePorts", () => {
      const groups = sketchpadCreatePortGroupMap([{ id: "p1", compatiblePorts: [{ id: "p2" }] }, { id: "p2", compatiblePorts: [{ id: "p1" }] }, { id: "p3" }]);
  ...
  describe("sketchpadApplyPuzzle2dSelection", () => {
    ...
    it("stores kit wires selection on single click without navigating", () => {
      ...
      expect(ctrl.navigationPath).toBe(`/kits/${kitId}${sketchpadRouteSelectionUriFilters({ pieceIds: [], connectionIds: [], kitWiresNodeIds: [typeId], kitWiresHoveredNodeId: null })}`);
      ctrl.dispose();
    });
    ...
    it("syncs kit wires selection to vfs component snapshot without manual refresh", async () => {
      ...
      const platform = await buildSketchpadPlatform();
      const ctrl = getSketchpadShellController()!;
      ctrl.registerKitStore(kitId, new InMemoryComposeKitStore({ id: kitId, name: "K", types: [{ id: typeId, name: "Column" }], designs: [] } as Kit));
      ctrl.navigateTo(`/kits/${kitId}`);
      platform.uri = `/kits/${kitId}`;
      ctrl.syncVirtualFileSystemRoute(sketchpadVfsScope(SKETCHPAD_KIT_APP_ID), kitId);
...
const SketchpadInstanceHost: AppInstanceHostComponent = ({ instance }: { readonly instance: OsAppInstance }): ReactElement => {
  ...
  if (!platform) {
    return reactHostPort.createElement("div", { className: "flex h-full items-center justify-center p-6 text-sm text-muted-foreground" }, "Loading sketchpad…");
  }
