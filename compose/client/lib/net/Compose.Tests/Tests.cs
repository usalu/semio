...
public class Tests
{
    ...
    private static string ResolveAssetsPath()
    {
        ...
        throw new DirectoryNotFoundException($"compose test fixtures not found (started from {AppContext.BaseDirectory})");
    }
    ...
    public class KitKindTests
    {
        ...
        {
            var json = ComposeJson.Codec.Serialize(kind);
        ...
        {
            var kind = ComposeJson.Codec.Deserialize<KitKind>(json);
        ...
        public void KitKind_Json_Roundtrip()
        {
            ...
            {
                var json = ComposeJson.Codec.Serialize(kind);
                var deserialized = ComposeJson.Codec.Deserialize<KitKind>(json);
    ...
    public class KitWorkflow
    {
        ...
        public void Transport_Kit_Roundtrip_Preserves_Kit()
        {
            ...
            Assert.True(ComposeDiff.AreKitsEqual(kit, roundtripped));
        }
    ...
    public class Roundtrip
    {
        ...
        public void Metabolism_Json_Memory_Json_Json_Zip_Zip_Json()
        {
            ...
            Assert.True(ComposeDiff.AreKitsEqual(kit, zipKit), "Kit loaded from pre-existing zip does not semantically match kit loaded from JSON");
            ...
            try
            {
                ...
                Assert.True(ComposeDiff.AreKitsEqual(kit, result2.Kit), "Kit exported to zip and reimported does not semantically match original kit");
            }
            finally
            {
        ...
        public class FolderRoundtrip
        {
            ...
            public void Metabolism_Kit_Export_Import_Matches_Json()
            {
                ...
                try
                {
                    ...
                    Assert.True(ComposeDiff.AreKitsEqual(kit, loaded.Kit));
                }
                finally
                {
    ...
    public class Flatten
    {
        private static JArray LoadFlattenCases() =>
            (JArray)JObject.Parse(System.IO.File.ReadAllText(Path.Combine(Tests.AssetsPath, "flatten.cases.compose.json")))["cases"]!;
    ...
    public class FlattenMerkle
    {
        private static JObject LoadCasesDoc() =>
            JObject.Parse(System.IO.File.ReadAllText(Path.Combine(Tests.AssetsPath, "flatten-merkle.cases.compose.json")));
    ...
    public class Change
    {
        ...
        public void Metabolism_Kit_Change_Forward_Backward_Inverse_Behavior()
        {
            ...
            var kitDiffInverted = Tests.LoadAsset<KitDiff>("metabolism.kit.diff.inverted.compose.json");
            var kitDiffed = Tests.LoadAsset<Kit>("metabolism.kit.diffed.compose.json");
            ...
            Assert.True(ComposeDiff.AreKitsEqual(appliedForward, kitDiffed), "ApplyKitDiff forward: applied kit doesn't match expected diffed kit");
            ...
            Assert.True(ComposeDiff.AreKitsEqual(appliedInverse, kitOriginal), "ApplyKitDiff inverse: applied inverse kit doesn't match original kit");
        }
    }
    ...
    public class ValidateKitDiffFacts
    {
        ...
        public void Shared_Asset_Cases()
        {
            ...
            {
                var r = ComposeDiff.ValidateKitDiff(asset.TinyKit, c.Diff, false);
    ...
    public class Validation
    {
        ...
        {
            var kit = Tests.LoadAsset<Kit>("invalid.kit.compose.json");
            var result = ComposeValidator.ValidateKit(kit);
            var filePath = Path.Combine(Tests.AssetsPath, "validation.compose.json");
    ...
    public class Drag
    {
        ...
        public void Design_Pieces_Offset_DiffDesign()
        {
            ...
            var offset = Tests.LoadAsset<Coordinate>("drag/offset.compose.json");
            var expectedDiff = Tests.LoadAsset<DesignDiff>("drag/diff.design.compose.json");
    ...
    public class Move
    {
        ...
        public void Design_Pieces_MoveVector_DiffDesign()
        {
            ...
            var expectedDiff = Tests.LoadAsset<DesignDiff>("move/diff.design.compose.json");
    ...
    public class CopyAndPaste
    {
        ...
        public void Nakagin_Capsule_Tower_Copy_Paste_Roundtrip()
        {
            ...
            System.IO.File.WriteAllText(Path.Combine(Tests.AssetsPath, "nakagin-capsule-tower.copy.design.compose.json"), copyJson);
            System.IO.File.WriteAllText(Path.Combine(Tests.AssetsPath, "nakagin-capsule-tower.paste.design.diff.compose.json"), pasteJson);
            System.IO.File.WriteAllText(Path.Combine(Tests.AssetsPath, "nakagin-capsule-tower.paste.with-coordinate.design.diff.compose.json"), pasteWithCoordinateJson);
            ...
            // Verify external pieces have compose.piece.origin and compose.center attributes
            var externalPieces = copyDesign.Pieces.Where(p => p.Attributes.Any(a => a.Key == "compose.piece.origin" && a.Value == "external")).ToList();
            Assert.Single(externalPieces);
            foreach (var ext in externalPieces)
            {
                Assert.True(ext.Attributes.Any(a => a.Key == "compose.center"), $"External piece {ext.Id} missing compose.center");
            }
            ...
            var ppExclPcInclPieces = copyDesign.Pieces.Where(p =>
                p.Attributes.Any(a => a.Key == "compose.center") && !p.Attributes.Any(a => a.Key == "compose.piece.origin")).ToList();
            Assert.Single(ppExclPcInclPieces);
            foreach (var pp in ppExclPcInclPieces)
            {
                Assert.True(pp.Attributes.Any(a => a.Key == "compose.plane"), $"Pp-excl-pc-incl piece {pp.Id} missing compose.plane");
            }
            ...
            {
                Assert.False(addedPiece.Attributes.Any(a => a.Key == "compose.piece.origin" && a.Value == "external"),
    ...
    {
        private static Representation? SelectBestRepresentationLikeComposeTs(List<Representation> representations, List<string> selectedTagIds)
        {
        ...
        [Fact]
        public void Representation_Selection_From_Shared_Compose_Assets()
        {
            var payload = Tests.LoadAsset<RepresentationSelectionAsset>("representation.selection.compose.json");
            foreach (var testCase in payload.Cases)
            {
                ...
                var selected = SelectBestRepresentationLikeComposeTs(representations, testCase.SelectedTagIds);
    ...
    public class KitFilterDesign
    {
        private static JObject LoadFilterKitAsset() =>
            JObject.Parse(System.IO.File.ReadAllText(Path.Combine(Tests.AssetsPath, "filter-kit.cases.compose.json")));
    ...
    public class DesignQualitySum
    {
        ...
        {
            var asset = JObject.Parse(System.IO.File.ReadAllText(Path.Combine(Tests.AssetsPath, "quality-sum.cases.compose.json")));
    ...
    public class GetGeometricInsightsForRepresentation
    {
        ...
        public void Nakagin_Capsule_Tower_Gltf_Returns_Insights()
        {
            ...
            var canonicalPath = Path.Combine(AssetsPath, "nakagin.kpi.representation.compose.json");
    ...
    public class ExportDesignRepresentation
    {
        ...
        public void Nakagin_Capsule_Tower_Export_Gltf_Valid_Json()
        {
            ...
            var parsed = ComposeJson.Codec.Deserialize<object>(json);
    ...
    public class HashTests
    {
        private static JObject LoadHashAsset() =>
            JObject.Parse(System.IO.File.ReadAllText(Path.Combine(Tests.AssetsPath, "hash.cases.compose.json")));
    ...
    public class FindReplaceableTypesInDesignsTests
    {
        ...
        private static JObject LoadFindReplaceableAsset() =>
            JObject.Parse(System.IO.File.ReadAllText(Path.Combine(Tests.AssetsPath, "find-replaceable-types.cases.compose.json")));
