...
class DevScript extends BundleScript {
  run(segments: string[]): void {
    if (segments[0] !== "mcp") {
      console.error("usage: bun ./📜️script.ts dev mcp");
      process.exit(1);
    }
...
class BuildPostScript extends BundleScript {
  run(): void {
    ...
    console.log("✅️ Post-build complete");
  }
}
...
class BuildScript extends BundleScript {
  run(segments: string[]): void {
    ...
    if (!process.argv.includes("--skip-post-build")) {
      runCmd("bun", ["./📜️script.ts", "build", "post"], { cwd: this.root });
    }
...
class TestScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    ...
    const graphqlSchema = readFileSync(join(this.root, "..", "graphql", "🔗️schema.graphql"), "utf8");
    ...
    await runTestBudgeted(pythonCommand, ["-c", "from pathlib import Path; from ariadne import gql, make_executable_schema; s=Path('../graphql/🔗️schema.graphql').read_text(encoding='utf-8'); make_executable_schema(gql(s))"], {
      cwd: this.root,
      env,
    });
