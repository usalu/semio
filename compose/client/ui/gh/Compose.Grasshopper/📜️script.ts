...
function runGenerateValueList(root: string): void {
  ...
  convertCsvToValueList(join(root, "..", "..", "..", "..", "..", "elements", "assets", "lists", "📋️mimes.csv"), join(buildDir, "mimes.txt"), "Extension", "MIME");
  convertCsvToValueList(join(root, "..", "..", "..", "..", "..", "elements", "assets", "lists", "📋️licenses.csv"), join(buildDir, "licenses.txt"), "Name", "SPDX");
  console.log("✅️ Value lists generated");
}
...
function runYakBuild(root: string): void {
  ...
  console.log("✅️ Yak package built");
}
...
class GenerateScript extends BundleScript {
  run(segments: string[]): void {
    ...
    console.error("usage: bun ./📜️script.ts generate value-list");
    process.exit(1);
  }
}
...
class TestScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    if (segments[0] === "push") {
      ...
      console.log(`✅️ Pushed ${packageFile} to test server`);
      return;
    }
...
class BuildScript extends BundleScript {
  run(): void {
    ...
    console.log("✅️ Grasshopper build complete");
  }
}
...
class PublishScript extends BundleScript {
  run(segments: string[]): void {
    if (segments[0] === "yank") {
      ...
      console.log(`✅️ Yanked compose ${version}`);
      return;
    }
    if (segments[0] === "unyank") {
      const version = segments[1] || "5.1.0-beta";
      runCmd(yakWin7, ["unyank", "compose", version]);
      console.log(`✅️ Unyanked compose ${version}`);
      return;
    }
    ...
    console.log("✅️ Yak package published");
  }
}
...
class SetupScript extends BundleScript {
  run(segments: string[]): void {
    if (segments[0] !== "login") {
      console.error("usage: bun ./📜️script.ts setup login");
      process.exit(1);
    }
    runCmd(yakWin8, ["login"]);
    console.log("✅️ Logged in to Yak");
  }
}
