...
using Point = Compose.Point;
using Vector = Compose.Vector;
using Plane = Compose.Plane;
using ComposeGroup = Compose.Group;
using Attribute = Compose.Attribute;
using Type = Compose.Type;
using File = Compose.File;
using ComposeRepresentation = Compose.Representation;
using Compose.Store;
using AttributeDiffUpdate = Compose.AttributeModification;
using AuthorDiffUpdate = Compose.AuthorModification;
using ConceptDiffUpdate = Compose.ConceptModification;
using ConnectionDiffUpdate = Compose.ConnectionModification;
using ConnectorDiffUpdate = Compose.ConnectorModification;
using DesignDiffUpdate = Compose.DesignModification;
using FileDiffUpdate = Compose.FileModification;
using FolderDiffUpdate = Compose.FolderModification;
using KitDiffUpdate = Compose.KitModification;
using PieceDiffUpdate = Compose.PieceModification;
using PortDiffUpdate = Compose.PortModification;
using RepresentationDiffUpdate = Compose.RepresentationModification;
using TagDiffUpdate = Compose.TagModification;
...
public class Compose_GrasshopperInfo : GH_AssemblyInfo
{
...
public class ComposeCategoryIcon : GH_AssemblyPriority
{
    public override GH_LoadingInstruction PriorityLoad()
    {
        Instances.ComponentServer.AddCategoryIcon("compose", IconResources.ResolveOrPlaceholder("compose_24x24"));
        Instances.ComponentServer.AddCategorySymbolName("compose", 'S');
...
public static class IconResources
{
    ...
    {
        var fallback = Resources.ResourceManager.GetObject("compose_24x24", CultureInfo.InvariantCulture) as Bitmap;
...
{
    internal const string ComposeImportRepresentationBlobKey = "compose.import-representation.blob";
    internal const string ComposeImportRepresentationObjectIdKey = "compose.import-representation.object-id";
    private static readonly object composeImportDocumentsLock = new();
    private static readonly List<RhinoDoc> composeImportDocuments = new();

    private static RhinoDoc CreateTrackedHeadlessImportDocument()
    {
        var doc = RhinoDoc.CreateHeadless(null);
        lock (composeImportDocumentsLock)
        {
            composeImportDocuments.Add(doc);
            return doc;
        }
    }
    ...
    public static string Serialize<TEntity>(this TEntity value, string indent = "") => Compose.Utility.Serialize(value, indent);
    public static TEntity? Deserialize<TEntity>(this string value) => Compose.Utility.Deserialize<TEntity>(value);
    public static TEntity? DeepClone<TEntity>(this TEntity value) where TEntity : Compose.Entity<TEntity>
        => Compose.Entity<TEntity>.DeepClone(value);
        ...
        double tilt)
    {
        ...
        var areDirectionsSame = parentDirectionR.IsParallelTo(childDirection.Convert(), Compose.Constants.Tolerance) == 1;

        Transform directionT;
        if (areDirectionsSame)
        {

            if (Math.Abs(parentDirectionR.Z) < Compose.Constants.Tolerance)
    ...
    public static RhinoRepresentationObjectData ImportRhinoRepresentationObjectDataFromComposeFile(File file)
    {
        if (file is null)
            throw new ArgumentNullException(nameof(file));
        if (string.IsNullOrWhiteSpace(file.Blob))
            throw new ArgumentException("Compose file blob cannot be empty.", nameof(file));

        var importedContext = ImportRhinoRepresentationContextFromComposeFile(file);
        return ConvertRhinoRepresentationContextToRhinoRepresentationObjectData(importedContext, file.Blob);
    }

    public static Rhino.DocObjects.RhinoObject ImportRhinoDocumentObjectFromComposeFile(File file, ComposeRepresentation? representation = null)
    {
        return ImportRhinoDocumentObjectsFromComposeFile(file, representation).First();
    }

    public static List<Rhino.DocObjects.RhinoObject> ImportRhinoDocumentObjectsFromComposeFile(File file, ComposeRepresentation? representation = null)
    {
        if (file is null)
            throw new ArgumentNullException(nameof(file));
        if (string.IsNullOrWhiteSpace(file.Blob))
    ...
    public static RhinoRepresentationObject ImportRhinoRepresentationContextFromBlob(string fileBlob, string? sourceName = null)
    {
        ...
        AttachComposeImportMetadata(representationObject, fileBlob);
        return new RhinoRepresentationObject(file3dm, representationObject);
    }

    public static RhinoRepresentationObject ImportRhinoRepresentationContextFromComposeFile(File file)
    {
        if (file is null)
            throw new ArgumentNullException(nameof(file));
        if (string.IsNullOrWhiteSpace(file.Blob))
    ...
    public static void AttachComposeImportMetadata(Rhino.FileIO.File3dmObject? representationObject, string fileBlob)
    {
        if (representationObject?.Attributes is null || string.IsNullOrWhiteSpace(fileBlob))
            return;

        representationObject.Attributes.SetUserString(ComposeImportRepresentationBlobKey, fileBlob);
        representationObject.Attributes.SetUserString(ComposeImportRepresentationObjectIdKey, ResolveRhinoObjectId(representationObject, -1));
    }
    ...
    public static RhinoRepresentationObjectData ConvertRhinoRepresentationContextToRhinoRepresentationObjectData(RhinoRepresentationObject rhinoRepresentationObject, string fileBlob)
    {
        ...
        if (string.IsNullOrWhiteSpace(fileBlob))
            throw new ArgumentException("Compose file blob cannot be empty.", nameof(fileBlob));
        ...
        var metadata = new[]
        {
            new KeyValuePair<string, string>(ComposeImportRepresentationBlobKey, fileBlob),
            new KeyValuePair<string, string>(
                ComposeImportRepresentationObjectIdKey,
        ...
        objectAttributes.SetUserString(ComposeImportRepresentationBlobKey, fileBlob);
    ...
    private static Rhino.FileIO.File3dm ImportRhinoRepresentationViaHeadlessDocument(byte[] fileBytes, string extension)
    {
        ...
        var tempDirectory = Path.Combine(Path.GetTempPath(), "compose-gh-import");
        ...
        UnitSystem? targetUnitSystem)
    {
        ...
        try
        {
            ...
            foreach (var importedRhinoObject in importedRhinoObjects)
                AttachComposeImportMetadata(importedRhinoObject, fileBlob, importedRhinoObject.Id.ToString());
        ...
        UnitSystem? targetUnitSystem)
    {
        ...
        for (var sourceIndex = 0; sourceIndex < sourceRepresentationObjects.Count; sourceIndex++)
        {
            ...
            AttachComposeImportMetadata(importedRhinoObject, fileBlob, ResolveRhinoObjectId(sourceRepresentationObject, sourceIndex));
    ...
    private static UnitSystem? ResolveRepresentationUnitSystem(ComposeRepresentation? representation)
    {
    ...
    private static void AttachComposeImportMetadata(
        ...
        string importedObjectId)
    {
        ...
        objectAttributes.SetUserString(ComposeImportRepresentationBlobKey, fileBlob);
        objectAttributes.SetUserString(ComposeImportRepresentationObjectIdKey, importedObjectId);
    ...
    public static bool TryResolveRhinoRepresentationContext(object representationObjectInput, out RhinoRepresentationObject rhinoRepresentationObject)
    {
        ...
        if (representationObjectInput is RhinoRepresentationObjectData representationObjectData &&
            TryResolveComposeImportMetadata(representationObjectData, out var representationBlob, out var representationObjectId))
        {
        ...
        if (representationObjectInput is RhinoRepresentationObjectData scriptedRepresentationObjectData &&
            TryResolveComposeImportMetadata(scriptedRepresentationObjectData, out var scriptedBlob, out var scriptedRepresentationObjectId))
        {
        ...
        if (representationObjectInput is not Rhino.FileIO.File3dmObject importedRepresentationObject)
        {
            ...
            {
                var rhinoFileBlob = rhinoObject.Attributes?.GetUserString(ComposeImportRepresentationBlobKey);
                if (string.IsNullOrWhiteSpace(rhinoFileBlob))
                    return false;

                var rhinoObjectId = rhinoObject.Attributes?.GetUserString(ComposeImportRepresentationObjectIdKey);
                return TryResolveRhinoRepresentationContextFromMetadata(rhinoFileBlob, rhinoObjectId, out rhinoRepresentationObject);
            }
            return false;
        }

        var fileBlob = importedRepresentationObject.Attributes?.GetUserString(ComposeImportRepresentationBlobKey);
        if (string.IsNullOrWhiteSpace(fileBlob))
            return false;

        var importedContext = ImportRhinoRepresentationContextFromBlob(fileBlob);
        var importedObjectId = importedRepresentationObject.Attributes?.GetUserString(ComposeImportRepresentationObjectIdKey);
    ...
    private static bool TryResolveComposeImportMetadata(
        ...
        out string? importedObjectId)
    {
        ...
        if (!userText.TryGetValue(ComposeImportRepresentationBlobKey, out fileBlob) || string.IsNullOrWhiteSpace(fileBlob))
            return false;
        userText.TryGetValue(ComposeImportRepresentationObjectIdKey, out importedObjectId);
        return true;
    }
    ...
    public static ComposeGroup TranslateRhinoRepresentationObjectToSingleGroup(RhinoRepresentationObject rhinoRepresentationObject)
    {
        if (rhinoRepresentationObject is null)
            throw new ArgumentNullException(nameof(rhinoRepresentationObject));
        ...
        return new ComposeGroup
        {
            ...
            Description = "Single compose group translated from Rhino representation object with recursive named layer groups.",
            Pieces = allPieceIds.Distinct().Select(pieceId => new PieceId { Id = pieceId }).ToList(),
            Attributes = attributes
        };
    }

    public static ComposeGroup TranslateRhinoRepresentationObjectsToSingleGroup(IEnumerable<RhinoRepresentationObject> rhinoRepresentationObjects)
    {
...
public sealed class RhinoRepresentationObjectData
{
    ...
    public RhinoRepresentationObjectData(Rhino.FileIO.File3dmObject? rhinoObject)
    {
        ...
        var representationBlob = rhinoObject?.Attributes?.GetUserString(Utility.ComposeImportRepresentationBlobKey);
        if (!string.IsNullOrWhiteSpace(representationBlob))
            Metadata.UserText[Utility.ComposeImportRepresentationBlobKey] = representationBlob;

        var representationObjectId = rhinoObject?.Attributes?.GetUserString(Utility.ComposeImportRepresentationObjectIdKey);
        if (!string.IsNullOrWhiteSpace(representationObjectId))
            Metadata.UserText[Utility.ComposeImportRepresentationObjectIdKey] = representationObjectId;
    }
...
/// Generic Grasshopper parameter for compose entity types.
...
{
    protected EnumParam(Guid id) : base(typeof(TEnum).Name, typeof(TEnum).Name, typeof(TEnum).Name, "Compose", "Param", GH_ParamAccess.item)
    {
        ComponentGuid = id;
    }
...
public abstract class Component : GH_Component
{
    ...
    private void ApplyNamingPolicy()
    {
        ...
        Description = $"{Name} operation in compose Grasshopper.";
    ...
    where TParam : Param<TGoo, TRepresentation>, new() where TGoo : Goo<TRepresentation>, new() where TRepresentation : Entity<TRepresentation>, new()
{
    ...
    public override string Description => $"{Name} operation in compose Grasshopper.";
...
/// Generic Grasshopper data wrapper for compose change types.
...
/// Generic Grasshopper parameter for compose change types.
/// Implementations MUST provide component exposure and icon metadata.
...
public class TypeGoo : Goo<Type>
{
    ...
    internal override bool CustomCastTo<Q>(ref Q target)
    {
        ...
        if (target is PieceGoo piece)
        {
            ...
            {
                Id = Compose.Utility.GenerateRandomId(new Random().Next()),
...
{
    public GroupGoo() { }
    public GroupGoo(ComposeGroup value) : base(value) { }
...
public class GroupParam : Param<GroupGoo, ComposeGroup>
{
...
public class GroupComponent : PassthroughComponent<GroupParam, GroupGoo, ComposeGroup>
{
...
public class GroupChangeComponent : ChangeComponent<GroupParam, GroupGoo, ComposeGroup, GroupDiffParam, GroupDiffGoo, GroupDiff, GroupChangeParam, GroupChangeGoo, GroupChange>
{
...
public class EncodeTextComponent : ScriptingComponent
{
    ...
    protected override void SolveInstance(IGH_DataAccess DA)
    {
        ...
        DA.SetData(0, Compose.Utility.Encode(text, (EncodeMode)mode, new Tuple<List<string>, List<string>>(forbidden, replace)));
    }
}
...
public class DecodeTextComponent : ScriptingComponent
{
    ...
    protected override void SolveInstance(IGH_DataAccess DA)
    {
        ...
        DA.SetData(0, Compose.Utility.Decode(text, (EncodeMode)mode, new Tuple<List<string>, List<string>>(replace, forbidden)));
    }
}

public class ImportRepresentationComponent : ScriptingComponent
{
    public ImportRepresentationComponent() : base("Import Representation", "ImpRepresentation", "Imports a Rhino representation object from a compose file.") { }
    ...
    protected override void RegisterInputParams(GH_InputParamManager pManager)
    {
        ...
        pManager.AddParameter(new FileParam(), "File", "Fi", "Compose file consumed by Import Representation that contains a Rhino file blob.", GH_ParamAccess.item);
    }
    ...
    protected override void SolveInstance(IGH_DataAccess DA)
    {
        ...
        try
        {
            var importedRhinoObjects = Utility.ImportRhinoDocumentObjectsFromComposeFile(fileGoo.Value, representationGoo.Value);
...
public class ExportDesignToBlocksComponent : ScriptingComponent
{
    ...
    protected override void SolveInstance(IGH_DataAccess DA)
    {
        ...
        try
        {
            ...
            var piecePlanes = new Dictionary<string, Compose.Plane>();
            ...
            var identityPlane = new Compose.Plane
            {
                Origin = new Compose.Point { X = 0, Y = 0, Z = 0 },
                XAxis = new Compose.Vector { X = 1, Y = 0, Z = 0 },
                YAxis = new Compose.Vector { X = 0, Y = 1, Z = 0 }
            };
            ...
            foreach (var piece in pieces)
            {
                ...
                var blockName = $"compose::{type.Name}::{type.Id}";
            ...
            foreach (var piece in pieces)
            {
                ...
                if (!piecePlanes.TryGetValue(piece.Id, out var composePlane)) continue;

                var rhinoPlane = RhinoConverter.Convert(composePlane);
