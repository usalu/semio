...
app.whenReady().then(async () => {
  ...
  ipcMain.handle("read-kit", async (_event, folderPath: string) => {
    const kitPath = path.join(folderPath, ".compose", "kit.db");
    try {
  ...
  ipcMain.handle("write-kit", async (_event, folderPath: string, data: ArrayBuffer) => {
    const composeDir = path.join(folderPath, ".compose");
    if (!fs.existsSync(composeDir)) {
      fs.mkdirSync(composeDir, { recursive: true });
    }
    fs.writeFileSync(path.join(composeDir, "kit.db"), Buffer.from(data));
  });
  ...
  ipcMain.handle("select-file", async () => {
    const result = await dialog.showOpenDialog({
      ...
      filters: [{ name: "Compose Kit JSON", extensions: ["json"] }],
    });
    if (result.canceled || result.filePaths.length === 0) return null;
    return result.filePaths[0];
  });
  ...
  // Specs: One recursive watcher per kit folder; debounced IPC notifies renderer to reload `.compose/kit.db` and asset files.
  ...
  if (extensionTestsPath) {
    try {
      ...
      console.error("[compose desktop integration tests]", err);
      app.exit(1);
    }
  }
