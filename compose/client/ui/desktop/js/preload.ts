...
const e2eKitFolder = process.env.COMPOSE_E2E_KIT_FOLDER?.trim() ?? "";
if (e2eKitFolder.length > 0) {
  contextBridge.exposeInMainWorld("__COMPOSE_E2E_KIT_FOLDER__", e2eKitFolder);
}

const e2eKitFile = process.env.COMPOSE_E2E_KIT_FILE?.trim() ?? "";
