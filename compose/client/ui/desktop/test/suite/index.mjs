...
// Default integration suite: runs in Electron main after the window loads (see main.ts COMPOSE_EXTENSION_TESTS_PATH).
...
export async function run(ctx) {
  ...
  const readPersistedKits = async () =>
    await win.webContents.executeJavaScript(`
      (async () => {
        ...
        const raw = window.localStorage.getItem('compose.sketchpad.kits.' + userId);
  ...
  await waitFor(
    ...
    async () =>
      await win.webContents.executeJavaScript(`
        (() => Boolean(
          document.getElementById("compose.sketchpad.app.home.toolbar.openFolder") &&
          document.getElementById("compose.sketchpad.app.home.toolbar.openFile")
        ))()
      `),
    120_000,
  );

  await clickById("compose.sketchpad.app.home.toolbar.openFolder");
