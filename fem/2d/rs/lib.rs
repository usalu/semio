...
use store::{create_document_envelope, DocumentDsl, DocumentCommand};
    ...
    fn support_op_round_trips() {
        let base = simply_supported_beam_doc();
        round_trip(&base, &Fem2dOperation::SetSupport { index: 0, support: FemSupport { id: "s1".into(), node_id: "n1".into(), fixed: vec![FemDof::Ty] } });
    ...
    fn combination_op_round_trips() {
        ...
        let updated = FemCombination { id: "uls".into(), name: "ULS v2".into(), terms: vec![FemCombinationTerm { case_id: "dead".into(), factor: 1.4 }] };
