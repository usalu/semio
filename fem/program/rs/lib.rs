//! 🏗️ FEM WASM program: `fem2d-play` and `fem3d-play` apps registered as one hot-swappable component.
...
fn register_fem_exports() {
    semio_framework_program::program_runtime::register_document_codec_for_app::<Fem2dPlayApp>(fem_2d::FEM_2D_SCHEMA);
    semio_framework_program::program_runtime::register_document_codec_for_app::<Fem3dPlayApp>(fem_3d::FEM_3D_SCHEMA);
}
...
mod tests {
    ...
    /// operations" guard would otherwise reject it (mirrors `gis/program/rs`'s equivalent test).
    #[test]
    fn set_active_example_is_declared_as_operation_2d() {
        use semio_framework_program::ActionKind;
