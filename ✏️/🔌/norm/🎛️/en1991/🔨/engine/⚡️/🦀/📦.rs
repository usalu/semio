...
use en1990_engine::{na_de::NaDe, na_en::NaEn};
use en1991::{part_1_2::FireCurve, Document};
use en1991_op::Operation;
use norm_core::{AnnexChoice, CheckReport, CheckResult, ClauseId, ImposedCategory, NationalAnnex, NormFamily, NormFamilyId, NormHost, Quantity};

// #region 🔖NaDe
pub mod na_de {
    pub use en1990_engine::na_de::NaDe;
...
pub mod part_1_1 {
    ...
    /// 🧱 Unit weight [kN/m³] per EN 1991-1-1 Annex A.
    ...
    /// 🧱 Self-weight per unit area [kN/m²] of a layer of given thickness.
    ...
    /// ✅ Verify the assumed design dead load covers the material self-weight.
...
pub mod part_1_2 {
    ...
    /// ✅ Verify the member's rated fire-resistance temperature capacity exceeds the gas temperature at t_min.
    pub fn check_fire_action(curve: FireCurve, t_min: f64, member_capacity_c: f64, annex: AnnexChoice) -> CheckResult {
        ...
        CheckResult::from_utilization(ClauseId::new("EN 1991-1-2", "§3.2", "3.4"), Quantity::new(norm_core::QuantityKind::Temperature, theta_g), Quantity::new(norm_core::QuantityKind::Temperature, member_capacity_c), "fire gas temperature", annex)
    }
}
// #endregion 🔖Part1_2
...
pub mod part_2 {
    ...
    /// 🌉 α adjustment factor for LM1 tandem/UDL: DE-NA reduces lane 1 vs EN recommended 1.0 (DIN EN 1991-2/NA §4.3.2).
    ...
    /// 🌉 Design tandem-system axle load [kN] including α_Q adjustment.
    pub fn lm1_design_tandem_kn(annex: AnnexChoice, lane: u8) -> f64 {
        alpha_q(annex, lane) * lm1_tandem_kn(lane)
    }

    /// 🌉 Simply-supported mid-span bending moment [kNm] from LM1 tandem + UDL over a span.
    ...
    /// ✅ Check LM1-derived mid-span moment against section resistance.
    pub fn check_lm1_moment(annex: AnnexChoice, span_m: f64, lane: u8, lane_width_m: f64, resistance_knm: f64) -> CheckResult {
        ...
        CheckResult::from_utilization(ClauseId::new("EN 1991-2", "§4.3.2", "4.4"), Quantity::new(norm_core::QuantityKind::Moment, m_ed * 1000.0), Quantity::new(norm_core::QuantityKind::Moment, resistance_knm * 1000.0), "LM1 mid-span moment", annex)
    }
}
// #endregion 🔖Part2
...
pub mod part_4 {
    ...
    /// 🌾 Janssen horizontal wall pressure p_h(z) [kPa] per EN 1991-4 Annex C Eq. C.4 (asymptotic silo pressure).
    ...
    /// 🌾 Legacy linear wall pressure surrogate, retained for simple hand checks.
