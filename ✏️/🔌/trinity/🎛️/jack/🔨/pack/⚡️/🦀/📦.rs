//! 📦 Trinity Jack app — binary document surface + laws (constitutional: pack).

use trinity_ram::GraphFixture;

/// 📦 Encodes a `GraphFixture` to its binary pack form.
pub fn encode(document: &GraphFixture) -> Vec<u8> {
    store::DocumentPack::encode_pack(document)
}

/// 📖 Decodes a `GraphFixture` from its binary pack form.
