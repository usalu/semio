//! 🌍 Georeferencing and survey products: GCPs, geodetic transforms, DSM/DTM, orthomosaics, contours, volumes and quality reports.
...
// #region 🔖Geodesy
/// 🌐 WGS84 semi-major axis (metres). <https://en.wikipedia.org/wiki/World_Geodetic_System>
const WGS84_A: f64 = 6_378_137.0;
/// 🌐 WGS84 flattening `f = (a - b) / a`.
const WGS84_F: f64 = 1.0 / 298.257_223_563;
/// 📐 UTM central-meridian scale factor.
const UTM_K0: f64 = 0.9996;
/// 📐 UTM false easting (metres), added to every zone's central-meridian-relative easting.
const UTM_FALSE_EASTING: f64 = 500_000.0;
/// 📐 UTM false northing (metres) added south of the equator so northings stay positive.
...
/// 🌐 WGS84 third flattening `n = f / (2 - f)`, the small parameter the Krüger series is expanded in.
fn wgs84_n() -> f64 {
    WGS84_F / (2.0 - WGS84_F)
}

/// 🌐 Converts geodetic `(lat, lon, height)` (radians, radians, metres above the ellipsoid) to
...
/// 🌐 Converts ECEF Cartesian coordinates to geodetic `(lat, lon, height)` via the standard fixed-point
...
/// 🧭 3x3 rotation columns mapping an ECEF delta vector into East-North-Up at the given geodetic reference.
...
/// 🧭 Converts an ECEF point into the local East-North-Up tangent plane centred at the given geodetic reference.
...
/// 🧭 Converts a local East-North-Up point back to ECEF, the inverse of [`ecef_to_enu`].
...
/// 🧭 Geodetic `(lat, lon, height)` straight to a local ENU frame, composing [`geodetic_to_ecef`] and [`ecef_to_enu`].
...
/// 🧭 Local ENU straight to geodetic `(lat, lon, height)`, composing [`enu_to_ecef`] and [`ecef_to_geodetic`].
...
/// 📐 UTM zone number (1-60) covering a longitude, ignoring the Svalbard/Norway irregular-zone exceptions.
...
/// 📐 A projected UTM coordinate: easting/northing in metres plus the zone/hemisphere it was computed in.
...
/// 📐 Krüger meridional-arc scale constant `A = a/(1+n)(1 + n²/4 + n⁴/64 + n⁶/256)`.
...
/// 📐 The six Krüger forward-series coefficients `alpha_1..alpha_6`, hard-coded to 6th order in `n`
...
/// 📐 Ellipsoidal transverse-Mercator forward projection (pre false-easting/northing, pre-`k0` origin
...
/// 📐 Inverts [`tm_forward_raw`] via Newton polishing (central-difference 2x2 Jacobian) seeded from a
...
/// 📐 Projects a geodetic point (radians) to UTM, auto-selecting the zone from the longitude.
...
/// 📐 Inverse of [`geodetic_to_utm`]: recovers `(lat_rad, lon_rad)` from a UTM coordinate.
...
// #region 🔖Gcp
/// 📍 A ground control point: a known world/geodetic-or-local survey position plus the pixel
...
pub struct GroundControlPoint {
    ...
    /// 📸 `(camera_index, pixel)` pairs, `camera_index` indexing directly into the `cameras` slice
...
/// 🧷 Refines each GCP's scene-frame 3D position and the scene→world [`Sim3`] jointly: each outer pass
...
/// 🧷 Registers an SfM reconstruction (arbitrary monocular gauge/scale) into a known world/geodetic or
...
/// 📏 Root-mean-square checkpoint residual of a converged georeferencing fit: for each `(refined scene
...
impl Raster {
    ...
    /// 🔢 Row-major flat index of cell `(x, y)`.
    pub fn index(&self, x: u32, y: u32) -> usize {
        y as usize * self.width as usize + x as usize
    }

    /// 🔍 The cell's value, or `None` if out of bounds or not yet written.
    ...
    /// 📍 World-space `(x, y)` of a cell's center.
    ...
    /// 🎯 The integer cell containing a world-space point, or `None` if outside the raster's extent.
...
/// 🌄 Per-cell density raster: number of point-cloud points binned into each cell.
...
/// 🔆 Standard illumination-angle hillshade (Horn's central-difference gradient, `shade = sin(alt)
...
// #region 🔖Ortho
/// 🧵 Per-cell overlap count: how many cameras' image bounds a DSM cell's height re-projects into.
...
// #region 🔖Contours
/// 🧩 A grid-edge crossing's canonical identity, shared by the two marching-squares cells that border
...
enum EdgeKey {
    /// ↔️ Horizontal edge between `(x, y)` and `(x + 1, y)`.
    H(u32, u32),
    /// ↕️ Vertical edge between `(x, y)` and `(x, y + 1)`.
    V(u32, u32),
}

/// 📐 Linear-interpolation fraction along an edge where the field crosses `level`.
...
/// 📍 World-space position of an edge crossing at the given iso level.
...
/// 🧵 Marching-squares segments for one cell, keyed on canonical [`EdgeKey`]s; saddle cases 5/10 resolve
...
/// 🧵 Chains a bag of `(EdgeKey, position)`-terminated segments into polylines by endpoint matching:
...
/// 🗻 Marching-squares isocontour extraction at each of `levels`, chaining segments into connected
...
// #region 🔖Volume
/// 📦 Cut/fill/net volume between two elevation states, with cut and fill always non-negative and `net = fill - cut`.
...
/// 📦 Cut/fill volume between two same-shaped rasters, summing `cell_area * (after - before)` over
...
/// 📦 Cut/fill volume of a raster against a flat reference plane at `plane_z`.
...
// #region 🔖Quality
/// 🧵 Aggregate feature-track length statistics.
#[derive(Clone, Copy, Debug, Default, PartialEq)]
...
/// 📊 Whole-reconstruction quality/QC report: reprojection accuracy, GCP checkpoint agreement, track
...
pub struct QualityReport {
    ...
    /// 🧮 Flattened row-major 6x6 marginal covariance per camera (from [`camera_covariance_diagonals`]).
    pub per_camera_covariance: Vec<[f64; 36]>,
    pub per_point_sigma: Vec<f64>,
    pub watertight: Option<WatertightReport>,
}

/// 📏 Overall and per-camera reprojection RMS (pixels) of `observations` — `(camera_index, point_index,
...
/// 🧵 [`TrackStats`] derived from `observations` by grouping observation counts per `point_index`.
...
/// 🧮 Per-camera marginal 6x6 covariance (flattened row-major), estimated by constructing a
...
/// 🧮 Per-point isotropic position-uncertainty proxy: accumulates the fixed-camera reprojection normal
...
/// 📊 Builds a full [`QualityReport`] from a reconstruction and its observations, computing
...
mod tests {
    ...
    // #endregion 🔖OrthoTests

    // #region 🔖ContoursTests
    #[test]
