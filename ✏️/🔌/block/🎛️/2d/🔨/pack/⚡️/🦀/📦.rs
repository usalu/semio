//! 📦 Block 2D app — binary document surface + laws (constitutional: pack).

use block_2d::Block2dDefinition;
use store::PackError;

/// 📦 Encodes a `Block2dDefinition` to its binary pack form.
pub fn encode(document: &Block2dDefinition) -> Vec<u8> {
    store::DocumentPack::encode_pack(document)
}

/// 📖 Decodes a `Block2dDefinition` from its binary pack form.
