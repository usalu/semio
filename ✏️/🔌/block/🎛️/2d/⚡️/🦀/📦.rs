//! 🩻 Block 2D app — document entities (constitutional: general). Edits exactly one `NodeKind`: its
...
// #region 🔖Document
/// 🔵 The node's own rim presentation — mirrors `Puzzle2dNode`'s shape fields, minus placement (a
...
/// 🔘 One handle-kind catalog row this node kind ships with.
...
/// 🌱 One rim-handle template — where a handle of `handle_kind` sits on the node's rim.
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize, dsl::DslRecord)]
#[serde(rename_all = "camelCase")]
...
/// 🩻 The block-2d projection: a typed single-`NodeKind`-definition document.
