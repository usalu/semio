//! 📦 Layout app — binary document surface + laws (constitutional: pack).

use layout::LayoutDocument;
use store::PackError;

/// 📦 Encodes a `LayoutDocument` to its binary pack form.
pub fn encode(document: &LayoutDocument) -> Vec<u8> {
    store::DocumentPack::encode_pack(document)
}

/// 📖 Decodes a `LayoutDocument` from its binary pack form.
...
mod tests {
    ...
    fn pack_round_trips_and_agrees_with_dsl() {
        let document = layout_dsl::parse_dsl(layout_dsl::LAYOUT_SAMPLE_TEXT).expect("parse sample layout fixture");
