...
/// 📄 The `metabolism` example, handcrafted in the `.wires` DSL (see
...
/// are opaque `dsl::Shape::Value` literals (see `reasoning_wires::MindmapWiresDocument`), whose
...
mod tests {
    ...
    fn metabolism_fixture_hydrates_extension() {
        // 📜 The `.wires` fixture is handcrafted in `reasoning_wires_dsl`'s DSL — parse it, then
