// #region 🧲Header
/** @emoji 📸 Shooting play app — model and icon viewport. */
// #endregion 🧲Header
...
export const SHOOTING_PLAY_SURFACE_ID_MODEL = "shooting.play.model";
export const SHOOTING_PLAY_SURFACE_ID_ICON = "shooting.play.icon";
...
const SHOOTING_PLAY_STORE_KEY = "shooting.fixture";
...
function shootingPlayInspectorShotGroup(shots: readonly ShootingShot[]): UiInspectorFieldGroup {
...
export function buildShootingPlayInspectorTree(
	...
	if (selectedShotIds.length > 0) {
		const shots = selectedShotIds
			...
			.filter((entry): entry is ShootingShot => Boolean(entry));
		if (shots.length > 0) {
	...
	if (selectedAssetIds.length > 0) {
		const assets = selectedAssetIds
			...
			.filter((entry): entry is ShootingFixture["assets"][number] => Boolean(entry));
		if (assets.length > 0) {
...
function shootingShotPointerKey(id: string): string {
	return `shooting:shot:${id}`;
}

function shootingAssetPointerKey(id: string): string {
	return `shooting:asset:${id}`;
}

function shootingPointerKeysToShotIds(keys: readonly string[]): string[] {
	return keys.filter((key) => key.startsWith("shooting:shot:")).map((key) => key.slice("shooting:shot:".length));
}

function shootingPointerKeysToAssetIds(keys: readonly string[]): string[] {
...
export class ShootingPlayController extends Controller implements PlaygroundExampleHost {
	...
	private readonly docStore = new DocumentVcsStore<ShootingFixture, ShootingFixtureEditOp>({
	...
	getFixture(): ShootingFixture {
		return this.projection();
	}

	getDocumentVcsStore(): DocumentVcsStore<ShootingFixture, ShootingFixtureEditOp> {
		return this.docStore;
	}

	private projection(): ShootingFixture {
	...
	getSelectedShotIds(): readonly string[] {
		return shootingPointerKeysToShotIds(this.pointerFocus.getSnapshot().selection);
	}

	getSelectedAssetIds(): readonly string[] {
		return shootingPointerKeysToAssetIds(this.pointerFocus.getSnapshot().selection);
	}
	...
	private patchScene(patch: Partial<ShootingScene>): void {
	...
	override run(command: string, args?: unknown): void {
		if (command === "setSelection") {
			...
			let nextShotIds = this.getSelectedShotIds();
			let nextAssetIds = this.getSelectedAssetIds();
			if (Array.isArray(payload.ids)) {
				...
				if (mode === "additive") {
					nextShotIds = [...new Set([...this.getSelectedShotIds(), ...incomingShots])];
					nextAssetIds = [...new Set([...this.getSelectedAssetIds(), ...incomingAssets])];
				} else {
			...
			if (
				nextShotIds.length === this.getSelectedShotIds().length &&
				nextAssetIds.length === this.getSelectedAssetIds().length &&
				nextShotIds.every((id, index) => id === this.getSelectedShotIds()[index]) &&
				nextAssetIds.every((id, index) => id === this.getSelectedAssetIds()[index])
			) {
				return;
			}
			this.pointerFocus.setSelection([
				...nextShotIds.map(shootingShotPointerKey),
				...nextAssetIds.map(shootingAssetPointerKey),
			]);
			this.interactionRevision += 1;
			this.notifySnapshot();
			this.emit();
		...
		if (command === "setCamera") {
			const camera = (args as { camera?: ShootingCamera }).camera;
			if (!camera) return;
		...
		if (command === "importAsset") {
			const asset = (args as { asset?: ShootingFixture["assets"][number] }).asset;
			if (!asset) return;
...
if (import.meta.vitest) {
	...
	describe("@semio-tech/shooting-play", () => {
		...
		it("controller stores fixture json", () => {
			...
			expect(ctrl.getFixtureJson()).toContain("shooting.fixture");
		});
...
export function registerShootingMediaExportHandlers(): void {
	registerOsMediaExportHandler("2d.shooting", "svg", async (doc) => {
		const fixture = doc as ShootingFixture;
