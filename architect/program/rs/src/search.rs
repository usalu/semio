...
pub fn search_program(
    ...
    search_history: Option<&mut Vec<SearchQuery>>,
) -> Vec<SearchHit> {
...
fn merge_query(query: &SearchQuery, filter: Option<&SearchFilter>) -> SearchQuery {
    ...
    SearchQuery {
        keywords: if filter.keywords.is_empty() {
            query.keywords.clone()
        } else {
            filter.keywords.clone()
        },
        categories: if filter.categories.is_empty() {
            query.categories.clone()
        } else {
            filter.categories.clone()
        },
        owner_ids: if filter.owner_ids.is_empty() {
            query.owner_ids.clone()
        } else {
            filter.owner_ids.clone()
        },
        statuses: if filter.statuses.is_empty() {
            query.statuses.clone()
        } else {
            filter.statuses.clone()
        },
        priorities: if filter.priorities.is_empty() {
            query.priorities.clone()
        } else {
            filter.priorities.clone()
        },
        entity_kinds: if filter.entity_kinds.is_empty() {
            query.entity_kinds.clone()
        } else {
            filter.entity_kinds.clone()
        },
        tag_filters: if filter.tag_filters.is_empty() {
            query.tag_filters.clone()
        } else {
            filter.tag_filters.clone()
        },
        sources: if filter.sources.is_empty() {
            query.sources.clone()
        } else {
...
fn push_if_match(hits: &mut Vec<SearchHit>, register: &str, header: &EntityHeader, query: &SearchQuery) {
    ...
    if !query.sources.is_empty() {
        let source_match = header
            .notes
            .iter()
            .any(|n| query.sources.iter().any(|s| n.tag.contains(s) || n.text.contains(s)))
            || header.tags.iter().any(|t| query.sources.contains(t));
        if !source_match {
            return;
        }
    }
    ...
    let haystack = format!(
        "{} {} {:?}",
        header.name,
        header.description.as_ref().map(|d| d.text.as_str()).unwrap_or(""),
        header.tags
    )
    .to_lowercase();
    ...
    if query.keywords.is_empty() || score > 0.0 {
        hits.push(SearchHit {
            ...
            entity_id: header.id.clone(),
            name: header.name.clone(),
            score: if score == 0.0 { 0.1 } else { score },
        });
    }
}
// #endregion
...
mod tests {
    ...
    fn entity_kind_filter_limits_registers() {
        let hits = search_program(
            &sample_program(),
            &SearchQuery {
                entity_kinds: vec!["elements".into()],
                ..Default::default()
            },
            None,
            None,
