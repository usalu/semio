...
pub fn run_analysis_and_record(program: &mut Program, kind: AnalysisKind) -> AnalysisResult {
    ...
    let record = AnalysisRecord {
        ...
        metrics: result
            .metrics
            .iter()
            .map(|m| format!("{}={}{}", m.name, m.value, m.unit.as_deref().unwrap_or("")))
            .collect(),
        charts: Vec::new(),
        run_by: None,
...
fn analyze_gap(program: &Program) -> AnalysisResult {
    ...
    let unlinked: Vec<_> = program
        .requirements
        .iter()
        .filter(|req| req.element_ids.is_empty() && req.function_ids.is_empty())
        .map(|req| req.header.id.clone())
        .collect();
    for id in &unlinked {
        findings.push(format!("requirement {id} is not linked to elements or functions"));
    }
    let elements_without_functions: Vec<_> = program
        .elements
        .iter()
        .filter(|e| e.function_ids.is_empty())
    ...
    AnalysisResult {
        ...
        metrics: vec![
            AnalysisMetric {
                name: "unlinked_requirements".into(),
                value: unlinked.len() as f64,
                unit: None,
            },
            AnalysisMetric {
                name: "elements_without_functions".into(),
                value: elements_without_functions.len() as f64,
                unit: None,
            },
        ],
        diagnostics: Vec::new(),
...
fn analyze_conflict(program: &Program) -> AnalysisResult {
    ...
    let mut findings: Vec<String> = adjacency_conflicts
        .iter()
        .map(|c| format!("{}: {}", c.adjacency_a_id, c.message))
        .collect();
    findings.extend(
        program
            .conflicts
            .iter()
            .map(|c| format!("{} — {:?} between {} and {}", c.header.name, c.kind, c.entity_a_id, c.entity_b_id)),
    );
    let open_conflicts = program
        .conflicts
        .iter()
        .filter(|c| c.resolution_status != ValidationStatus::Passed)
        .count();
    ...
    AnalysisResult {
        ...
        metrics: vec![
            AnalysisMetric {
                name: "total_conflicts".into(),
                value: findings_len as f64,
                unit: None,
            },
            AnalysisMetric {
                name: "open_conflicts".into(),
                value: open_conflicts as f64,
                unit: None,
            },
        ],
        diagnostics: adjacency_conflicts
            .into_iter()
            .map(|c| ProgramDiagnostic {
                severity: DiagnosticSeverity::Error,
                code: "analysis.conflict".into(),
                message: c.message,
                entity_id: Some(c.adjacency_a_id),
...
fn analyze_dependency(program: &Program) -> AnalysisResult {
    let depends: Vec<String> = program
        .relationships
        .iter()
        .filter(|r| matches!(r.kind, RelationshipKind::DependsOn))
        .map(|r| format!("{} depends on {}", r.source_id, r.target_id))
        .collect();
    ...
    AnalysisResult {
        ...
        metrics: vec![
            AnalysisMetric {
                ...
                value: program.relationships.len() as f64,
                unit: None,
            },
            AnalysisMetric {
                name: "process_dependency_count".into(),
                value: process_deps as f64,
                unit: None,
            },
        ],
        diagnostics: Vec::new(),
...
fn analyze_capacity(program: &Program) -> AnalysisResult {
    ...
    let total_occupancy: f64 = program
        .elements
        .iter()
        .filter_map(|e| e.occupancy.target.or(e.occupancy.peak))
        .sum();
    let area_per_person = if total_occupancy > 0.0 {
        total_area / total_occupancy
    } else {
        0.0
    };
    AnalysisResult {
        ...
        summary: format!(
            "total target area {total_area:.1} m², {total_occupancy:.0} persons, {:.1} m²/person",
            area_per_person
        ),
        findings: program
            .elements
            .iter()
            .filter_map(|e| e.area.target.map(|a| format!("{}: {a:.1} m²", e.header.name)))
            .collect(),
        metrics: vec![
            AnalysisMetric {
                name: "element_count".into(),
                value: program.elements.len() as f64,
                unit: None,
            },
            AnalysisMetric {
                name: "total_target_area".into(),
                value: total_area,
                unit: Some("m2".into()),
            },
            AnalysisMetric {
                name: "area_per_person".into(),
                value: area_per_person,
                unit: Some("m2/person".into()),
            },
        ],
        diagnostics: Vec::new(),
        entity_ids: Vec::new(),
...
fn analyze_utilization(program: &Program) -> AnalysisResult {
    ...
    let ratio = if elements == 0 {
        0.0
    } else {
        activities as f64 / elements as f64
    };
    let equipped = program
        .equipment
        .iter()
        .filter(|e| !e.element_ids.is_empty())
        .count();
    AnalysisResult {
        ...
        findings: program
            .equipment
            .iter()
            .map(|e| format!("{} serves {} element(s)", e.header.name, e.element_ids.len()))
            .collect(),
        metrics: vec![AnalysisMetric {
            name: "activity_element_ratio".into(),
            value: ratio,
            unit: None,
        }],
        diagnostics: Vec::new(),
...
fn analyze_workflow(program: &Program) -> AnalysisResult {
    ...
    AnalysisResult {
        ...
        findings: program
            ...
            .collect(),
        metrics: vec![AnalysisMetric {
            name: "critical_path_processes".into(),
            value: critical.len() as f64,
            unit: None,
        }],
        diagnostics: Vec::new(),
...
fn analyze_risk(program: &Program) -> AnalysisResult {
    let high: Vec<_> = program
        .risks
        .iter()
        .filter(|r| {
            matches!(r.probability, RiskLevel::High | RiskLevel::Critical)
                || matches!(r.impact, RiskLevel::High | RiskLevel::Critical)
        })
        .map(|r| r.header.id.clone())
        .collect();
    let score_sum: f64 = program
        .risks
        .iter()
        .map(|r| risk_score(&r.probability) * risk_score(&r.impact))
        .sum();
    AnalysisResult {
        ...
        metrics: vec![
            AnalysisMetric {
                name: "risk_count".into(),
                value: program.risks.len() as f64,
                unit: None,
            },
            AnalysisMetric {
                name: "aggregate_risk_score".into(),
                value: score_sum,
                unit: None,
            },
        ],
...
fn analyze_cost(program: &Program) -> AnalysisResult {
    ...
    AnalysisResult {
        ...
        findings: program
            .costs
            .iter()
            .filter_map(|c| c.amount.map(|v| format!("{}: {v:.0}", c.header.name)))
            .collect(),
        metrics: vec![AnalysisMetric {
            name: "capital_cost_total".into(),
            value: total_capital,
...
fn analyze_scenario(program: &Program) -> AnalysisResult {
    let evaluated = program
        .options
        .iter()
        .filter(|o| o.evaluation_status == ValidationStatus::Passed)
        .count();
    AnalysisResult {
        kind: AnalysisKind::Scenario,
        title: "Scenario Analysis".into(),
        summary: format!(
            "{} scenario(s), {} options ({} selected)",
            program.scenarios.len(),
            program.options.len(),
            evaluated
        ),
        findings: program.scenarios.iter().map(|s| s.header.name.clone()).collect(),
        metrics: vec![AnalysisMetric {
            name: "selected_options".into(),
            value: evaluated as f64,
            unit: None,
        }],
        diagnostics: Vec::new(),
...
fn analyze_sensitivity(program: &Program) -> AnalysisResult {
    ...
    AnalysisResult {
        ...
        metrics: vec![AnalysisMetric {
            name: "mandatory_requirement_count".into(),
            value: mandatory as f64,
            unit: None,
        }],
        diagnostics: Vec::new(),
        entity_ids: Vec::new(),
...
fn analyze_impact(program: &Program) -> AnalysisResult {
    let impacted: usize = program
        .decisions
        .iter()
        .map(|d| d.impacted_requirement_ids.len() + d.impacted_element_ids.len())
        .sum();
    AnalysisResult {
        ...
        findings: program
            .decisions
            .iter()
            .map(|d| format!("{} impacts {} requirements", d.header.name, d.impacted_requirement_ids.len()))
            .collect(),
        metrics: vec![AnalysisMetric {
            name: "impacted_links".into(),
            value: impacted as f64,
            unit: None,
        }],
        diagnostics: Vec::new(),
...
fn analyze_trend(program: &Program) -> AnalysisResult {
    ...
    AnalysisResult {
        ...
        summary: format!(
            "{} analysis records, {} changes, {} audit events",
            program.analyses.len(),
            change_velocity,
            program.audit_events.len()
        ),
        findings: program
            .changes
            .iter()
            .take(5)
            .map(|c| format!("change {} — {}", c.header.name, c.header.timestamps.updated))
            .collect(),
        metrics: vec![AnalysisMetric {
            name: "change_count".into(),
            value: change_velocity as f64,
            unit: None,
        }],
        diagnostics: Vec::new(),
        entity_ids: Vec::new(),
...
fn analyze_requirement_comparison(program: &Program) -> AnalysisResult {
    ...
    let findings: Vec<String> = by_kind
        .iter()
        .map(|(kind, count)| format!("{kind}: {count} requirement(s)"))
        .collect();
    AnalysisResult {
        ...
        findings,
        metrics: vec![AnalysisMetric {
            name: "requirement_kind_count".into(),
            value: by_kind.len() as f64,
            unit: None,
        }],
        diagnostics: Vec::new(),
...
fn analyze_requirement_clustering(program: &Program) -> AnalysisResult {
    ...
    let findings: Vec<String> = clusters
        .iter()
        .map(|(key, ids)| format!("cluster {key}: {} requirement(s)", ids.len()))
        .collect();
    AnalysisResult {
...
fn analyze_requirement_filtering(program: &Program) -> AnalysisResult {
    ...
    AnalysisResult {
        ...
        findings,
        metrics: vec![AnalysisMetric {
            name: "pending_validation_count".into(),
...
fn analyze_requirement_sorting(program: &Program) -> AnalysisResult {
    ...
    let findings: Vec<String> = sorted
        .iter()
        .map(|r| format!("{:?} — {}", r.header.priority, r.header.name))
        .collect();
    AnalysisResult {
        ...
        findings,
        metrics: vec![AnalysisMetric {
            name: "sorted_requirement_count".into(),
            value: sorted.len() as f64,
            unit: None,
        }],
        diagnostics: Vec::new(),
...
fn analyze_requirement_scoring(program: &Program) -> AnalysisResult {
    ...
    let findings: Vec<String> = scored
        .iter()
        .take(10)
        .map(|(id, score)| format!("{id}: score {score:.2}"))
        .collect();
    ...
    AnalysisResult {
        ...
        findings,
        metrics: vec![AnalysisMetric {
            name: "total_requirement_score".into(),
            value: total,
            unit: None,
        }],
        diagnostics: Vec::new(),
...
fn analyze_requirement_weighting(program: &Program) -> AnalysisResult {
    ...
    let findings: Vec<String> = weights
        .iter()
        .map(|(id, w)| format!("{id}: weight {w:.2}"))
        .collect();
...
fn analyze_relationship(program: &Program) -> AnalysisResult {
    ...
    let conflicts = program
        .relationships
        .iter()
        .filter(|r| matches!(r.kind, RelationshipKind::ConflictsWith))
        .count();
    AnalysisResult {
        kind: AnalysisKind::RelationshipAnalysis,
        title: "Relationship Analysis".into(),
        summary: format!(
            "{} relationships across {} nodes ({} depends, {} conflicts)",
            program.relationships.len(),
            nodes.len(),
            depends,
            conflicts
        ),
        findings: program
            .relationships
            .iter()
            .map(|r| format!("{:?}: {} → {}", r.kind, r.source_id, r.target_id))
            .collect(),
        metrics: vec![
            AnalysisMetric {
                name: "relationship_node_count".into(),
                value: nodes.len() as f64,
                unit: None,
            },
            AnalysisMetric {
                name: "dependency_edge_count".into(),
                value: depends as f64,
                unit: None,
            },
        ],
        diagnostics: Vec::new(),
