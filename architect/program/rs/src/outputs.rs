...
fn requirement_lists(program: &Program) -> ProgramOutput {
    ProgramOutput {
        ...
        lines: program
            .requirements
            .iter()
            .map(|r| format!("[{:?}] {} — {}", r.kind, r.header.name, r.statement.text))
            .collect(),
...
fn functional_hierarchies(program: &Program) -> ProgramOutput {
    ...
    for root in roots {
        ...
        for child in program
            .functions
            .iter()
            .filter(|f| f.hierarchy_parent_id.as_ref() == Some(&root.header.id))
        {
            lines.push(format!("  └─ {}", child.header.name));
        }
    }
    ProgramOutput {
        kind: OutputKind::FunctionalHierarchies,
        title: "Functional Hierarchies".into(),
        lines,
...
fn activity_taxonomies(program: &Program) -> ProgramOutput {
    ...
    ProgramOutput {
        kind: OutputKind::ActivityTaxonomies,
        title: "Activity Taxonomies".into(),
        lines,
        entity_ids: program.activities.iter().map(|a| a.header.id.clone()).collect(),
    }
}
...
fn relationship_matrices(program: &Program) -> ProgramOutput {
    let lines: Vec<String> = program
        ...
        .collect();
    ProgramOutput {
        kind: OutputKind::RelationshipMatrices,
        title: "Relationship Matrices".into(),
        lines,
        entity_ids: program.relationships.iter().map(|r| r.header.id.clone()).collect(),
    }
}
...
fn adjacency_matrices(program: &Program) -> ProgramOutput {
    ...
    ProgramOutput {
        kind: OutputKind::AdjacencyMatrices,
        title: "Adjacency Matrices".into(),
        lines: report.sections.into_iter().flat_map(|s| s.bullets).collect(),
...
fn dependency_networks(program: &Program) -> ProgramOutput {
    ...
    ProgramOutput {
        kind: OutputKind::DependencyNetworks,
        title: "Dependency Networks".into(),
...
fn priority_matrices(program: &Program) -> ProgramOutput {
    let lines: Vec<String> = program
        .priorities
        .iter()
        .map(|p| format!("{} — {:?} rank {:?} weight {:?}", p.header.name, p.ranked_priority, p.rank, p.weight))
        .collect();
    ProgramOutput {
        kind: OutputKind::PriorityMatrices,
        title: "Priority Matrices".into(),
        lines,
...
fn responsibility_matrices(program: &Program) -> ProgramOutput {
    let lines: Vec<String> = program
        ...
        .responsibilities
        .iter()
        .chain(program.governance.roles.iter())
        .cloned()
        .collect();
    ProgramOutput {
        kind: OutputKind::ResponsibilityMatrices,
        title: "Responsibility Matrices".into(),
        lines,
        entity_ids: vec![program.governance.id.clone()],
    }
}
...
fn decision_trees(program: &Program) -> ProgramOutput {
    let lines: Vec<String> = program
        ...
        .map(|d| {
            format!(
                "{} → option {:?} ({})",
                d.header.name, d.selected_option_id, d.decision_statement.text
            )
        })
        .collect();
    ProgramOutput {
        kind: OutputKind::DecisionTrees,
        title: "Decision Trees".into(),
        lines,
...
fn process_maps(program: &Program) -> ProgramOutput {
    let lines: Vec<String> = program
        .processes
        .iter()
        .map(|p| format!("{}: {}", p.header.name, p.steps.join(" → ")))
        .collect();
    ProgramOutput {
        kind: OutputKind::ProcessMaps,
        title: "Process Maps".into(),
        lines,
        entity_ids: program.processes.iter().map(|p| p.header.id.clone()).collect(),
    }
}
...
fn workflow_descriptions(program: &Program) -> ProgramOutput {
    ...
    ProgramOutput {
        kind: OutputKind::WorkflowDescriptions,
        title: "Workflow Descriptions".into(),
        lines: analysis.findings,
        entity_ids: analysis.entity_ids,
    }
}
...
fn user_journeys(program: &Program) -> ProgramOutput {
    ...
    for user in &program.users {
        let activities: Vec<_> = program
            .activities
            .iter()
            .filter(|a| a.user_profile_ids.contains(&user.header.id))
            .map(|a| a.header.name.as_str())
            .collect();
        lines.push(format!("{}: {}", user.header.name, activities.join(" → ")));
    }
    ProgramOutput {
        kind: OutputKind::UserJourneys,
        title: "User Journeys".into(),
        lines,
...
fn scenario_narratives(program: &Program) -> ProgramOutput {
    let lines: Vec<String> = program
        .scenarios
        .iter()
        .map(|s| format!("{} — {}", s.header.name, s.hypothesis.text))
        .collect();
    ProgramOutput {
        kind: OutputKind::ScenarioNarratives,
        title: "Scenario Narratives".into(),
        lines,
...
fn risk_matrices(program: &Program) -> ProgramOutput {
    let lines: Vec<String> = program
        ...
        .collect();
    ProgramOutput {
        kind: OutputKind::RiskMatrices,
        title: "Risk Matrices".into(),
        lines,
...
fn compliance_matrices(program: &Program) -> ProgramOutput {
    let lines: Vec<String> = program
        .regulatory
        .iter()
        .map(|r| format!("{} {} — {:?}", r.code, r.title, r.verification_status))
        .collect();
    ProgramOutput {
        kind: OutputKind::ComplianceMatrices,
        title: "Compliance Matrices".into(),
        lines,
...
fn capacity_schedules(program: &Program) -> ProgramOutput {
    ...
    ProgramOutput {
        kind: OutputKind::CapacitySchedules,
        title: "Capacity Schedules".into(),
        lines: analysis.findings.into_iter().chain(schedule_lines).collect(),
...
fn equipment_schedules(program: &Program) -> ProgramOutput {
    let lines: Vec<String> = program
        .equipment
        .iter()
        .map(|e| format!("{} — qty {:?}", e.header.name, e.quantity.target))
        .collect();
    ProgramOutput {
        kind: OutputKind::EquipmentSchedules,
        title: "Equipment Schedules".into(),
        lines,
        entity_ids: program.equipment.iter().map(|e| e.header.id.clone()).collect(),
    }
}
...
fn evaluation_frameworks(program: &Program) -> ProgramOutput {
    let lines: Vec<String> = program
        .performance
        .iter()
        .map(|p| format!("{} — {}", p.header.name, p.criterion))
        .collect();
    ProgramOutput {
        kind: OutputKind::EvaluationFrameworks,
        title: "Evaluation Frameworks".into(),
        lines,
        entity_ids: program.performance.iter().map(|p| p.header.id.clone()).collect(),
    }
}
...
fn performance_specifications(program: &Program) -> ProgramOutput {
    let lines: Vec<String> = program
        .performance
        .iter()
        .map(|p| format!("{} target {:?} {:?}", p.header.name, p.target, p.unit))
        .collect();
    ProgramOutput {
        kind: OutputKind::PerformanceSpecifications,
        title: "Performance Specifications".into(),
        lines,
        entity_ids: program.performance.iter().map(|p| p.header.id.clone()).collect(),
    }
}
...
fn program_reports(program: &Program) -> ProgramOutput {
    let lines: Vec<String> = program
        .reports
        .iter()
        .map(|r| format!("{:?} — {}", r.kind, r.title))
        .collect();
    ProgramOutput {
        kind: OutputKind::ProgramReports,
        title: "Program Reports".into(),
        lines,
        entity_ids: program.reports.iter().map(|r| r.header.id.clone()).collect(),
    }
}
// #endregion
