...
/// 📦 Encodes a `SourcingOperation` to its binary command form.
pub fn encode_op(operation: &SourcingOperation) -> Result<Vec<u8>, protocol::ProtocolError> {
    operation.encode_op()
}

/// 📖 Decodes a `SourcingOperation` from its binary command form.
...
mod tests {
    ...
    /// 🌱 Mirrors `sourcing_engine`'s private test-only helper (see that crate's tests for why this
