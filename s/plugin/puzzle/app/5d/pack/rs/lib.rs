//! 📦 Puzzle 5d app — binary document surface + laws (constitutional: pack).

use puzzle_5d::Puzzle5dProjection;
use store::PackError;

/// 📦 Encodes a `Puzzle5dProjection` to its binary pack form.
pub fn encode(document: &Puzzle5dProjection) -> Vec<u8> {
    store::DocumentPack::encode_pack(document)
}

/// 📖 Decodes a `Puzzle5dProjection` from its binary pack form.
