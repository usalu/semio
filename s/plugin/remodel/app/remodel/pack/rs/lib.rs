//! 📦 Remodel app — binary document surface + laws (constitutional: pack).

use remodel::RemodelScene;
use store::PackError;

/// 📦 Encodes a `RemodelScene` to its binary pack form.
pub fn encode(scene: &RemodelScene) -> Vec<u8> {
    store::DocumentPack::encode_pack(scene)
}

/// 📖 Decodes a `RemodelScene` from its binary pack form.
