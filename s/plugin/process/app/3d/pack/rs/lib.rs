//! 📦 Process 3d app — binary document surface + laws (constitutional: pack).

use process_3d::Process3dDocument;
use store::PackError;

/// 📦 Encodes a `Process3dDocument` to its binary pack form.
pub fn encode(document: &Process3dDocument) -> Vec<u8> {
    store::DocumentPack::encode_pack(document)
}

/// 📖 Decodes a `Process3dDocument` from its binary pack form.
