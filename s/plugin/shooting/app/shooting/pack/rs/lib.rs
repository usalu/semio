//! 📦 Shooting app — binary document surface + laws (constitutional: pack).

use shooting::ShootingFixture;
use store::PackError;

/// 📦 Encodes a `ShootingFixture` to its binary pack form.
pub fn encode(fixture: &ShootingFixture) -> Vec<u8> {
    store::DocumentPack::encode_pack(fixture)
}

/// 📖 Decodes a `ShootingFixture` from its binary pack form.
