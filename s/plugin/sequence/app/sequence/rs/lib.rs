//! 🧩 Sequence app — document entities (constitutional: general).
...
/// 🎥 Camera state for the sequence canvas — the DAG kernel's own `DagCamera` conversions
...
/// `🔖Dsl`) instead of deriving `dsl::DslRecord` here directly, so this struct (and every consumer
