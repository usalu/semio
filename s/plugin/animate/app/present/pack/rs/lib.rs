//! 📦 Animate present app — binary document surface + laws (constitutional: pack).

use present::PresentDeck;
use store::PackError;

/// 📦 Encodes a `PresentDeck` to its binary pack form.
pub fn encode(deck: &PresentDeck) -> Vec<u8> {
    store::DocumentPack::encode_pack(deck)
}

/// 📖 Decodes a `PresentDeck` from its binary pack form.
