//! ⚡ Flow app — operation type facade (constitutional: op).
//!
...
//! `s/kernel/flow/core/rs`, `🔖Operations` region) alongside the `FlowFixture` projection they mutate —
//! see `s/plugin/flow/app/flow/rs/lib.rs` for why. Re-exported here so sibling constitutional crates
