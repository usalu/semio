//! 📦 Flow app — binary document surface + laws (constitutional: pack).
//!
//! `store::DocumentPack for FlowFixture` is implemented directly in the flow kernel crate (`flow_core`);
//! see `s/plugin/flow/app/flow/rs/lib.rs` for why. This crate only adds the thin app-facing
//! `encode`/`decode` wrappers plus the pack↔dsl equivalence law.

use flow::FlowFixture;
use store::PackError;

/// 📦 Encodes a `FlowFixture` to its binary pack form.
pub fn encode(fixture: &FlowFixture) -> Vec<u8> {
    store::DocumentPack::encode_pack(fixture)
}

/// 📖 Decodes a `FlowFixture` from its binary pack form.
