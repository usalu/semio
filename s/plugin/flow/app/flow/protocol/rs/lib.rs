...
//! see `s/plugin/flow/app/flow/op/rs/lib.rs` for why. This crate only adds the thin app-facing
...
/// 📦 Encodes a `FlowOperation` to its binary command form.
pub fn encode_op(operation: &FlowOperation) -> Result<Vec<u8>, protocol::ProtocolError> {
    operation.encode_op()
}

/// 📖 Decodes a `FlowOperation` from its binary command form.
