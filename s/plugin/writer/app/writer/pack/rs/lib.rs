//! 📦 Writer app — binary document surface + laws (constitutional: pack).

use store::PackError;
use writer::WriterProjection;

/// 📦 Encodes a `WriterProjection` to its binary pack form.
pub fn encode(projection: &WriterProjection) -> Vec<u8> {
    store::DocumentPack::encode_pack(projection)
}

/// 📖 Decodes a `WriterProjection` from its binary pack form.
