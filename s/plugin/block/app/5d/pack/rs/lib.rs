//! 📦 Block 5D app — binary document surface + laws (constitutional: pack).

use block_5d::Block5dDefinition;
use store::PackError;

/// 📦 Encodes a `Block5dDefinition` to its binary pack form.
pub fn encode(document: &Block5dDefinition) -> Vec<u8> {
    store::DocumentPack::encode_pack(document)
}

/// 📖 Decodes a `Block5dDefinition` from its binary pack form.
