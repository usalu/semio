//! 📦 GIS 2D app — binary document surface + laws (constitutional: pack).

use gis2d::GisMapDocument;
use store::PackError;

/// 📦 Encodes a `GisMapDocument` to its binary pack form.
pub fn encode(document: &GisMapDocument) -> Vec<u8> {
    store::DocumentPack::encode_pack(document)
}

/// 📖 Decodes a `GisMapDocument` from its binary pack form.
