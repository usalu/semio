//! 📦 Trinity Rewrite app — binary document surface + laws (constitutional: pack).

use rewrite::{LayoutPoint, RewriteRuleState};
use store::PackError;

/// 📦 Encodes a `RewriteRuleState` to its binary pack form.
pub fn encode(document: &RewriteRuleState) -> Vec<u8> {
    store::DocumentPack::encode_pack(document)
}

/// 📖 Decodes a `RewriteRuleState` from its binary pack form.
