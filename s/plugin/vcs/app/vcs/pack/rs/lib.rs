//! 📦 VCS app — binary document surface + laws (constitutional: pack).

use store::PackError;
use vcs::VcsDemoProjection;

/// 📦 Encodes a `VcsDemoProjection` to its binary pack form.
pub fn encode(projection: &VcsDemoProjection) -> Vec<u8> {
    store::DocumentPack::encode_pack(projection)
}

/// 📖 Decodes a `VcsDemoProjection` from its binary pack form.
