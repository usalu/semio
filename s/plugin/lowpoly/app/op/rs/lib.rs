//! ⚡ Lowpoly app — operation enum + laws (constitutional: op).

use lowpoly::{LowpolyObject, LowpolyObjectPatch, LowpolyProjection};
use lowpoly_engine::{layer_pixels_at, object_mut};
...
fn apply_paint_layer_patch(layer: &mut lowpoly::LowpolyPaintLayer, patch: &LowpolyPaintLayerPatch) -> LowpolyPaintLayerPatch {
...
pub fn invert_lowpoly_operation(projection: &LowpolyProjection, operation: &LowpolyOperation) -> LowpolyOperation {
    match operation {
        ...
        LowpolyOperation::PatchPaintLayer { object_id, index, patch } => {
            let mut probe = projection.objects.iter().find(|object| object.id == *object_id).and_then(|object| object.paint_layers.get(*index)).cloned().unwrap_or_else(|| lowpoly::LowpolyPaintLayer::new("Layer"));
...
mod tests {
    use super::*;
    use lowpoly_engine::default_projection;
    ...
    fn apply_operations_on_missing_object_are_no_ops() {
        ...
        apply_lowpoly_operation(&mut mutated, &LowpolyOperation::AddPaintLayer { object_id: "missing".into(), index: 0, layer: lowpoly::LowpolyPaintLayer::new("X") });
    ...
    fn apply_set_projection_replaces_entire_projection() {
        ...
        let replacement = lowpoly::projection_from_mesh_json(&tiny_mesh_json(), "obj-x", "X");
    ...
    fn invert_add_paint_layer_produces_remove_at_same_index() {
        let projection = default_projection();
        let operation = LowpolyOperation::AddPaintLayer { object_id: projection.objects[0].id.clone(), index: 1, layer: lowpoly::LowpolyPaintLayer::new("New") };
    ...
    fn paint_layer_patch_apply_mutates_and_inverse_restores_all_fields() {
        let mut layer = lowpoly::LowpolyPaintLayer::new("Base");
    ...
    fn tiny_object(id: &str, name: &str) -> LowpolyObject {
        LowpolyObject { id: id.into(), name: name.into(), transform: Default::default(), smooth_shading: false, mesh_json: tiny_mesh_json(), paint_layers: vec![lowpoly::LowpolyPaintLayer::new("Base")] }
    }
    ...
    fn op_text_round_trip_objects_patch_without_mesh() {
        let operation = LowpolyOperation::ObjectsPatch {
            id: "obj-1".into(),
            patch: LowpolyObjectPatch { name: Some("Renamed".into()), smooth_shading: Some(true), transform: Some(lowpoly::LowpolyTransform { position: [1.0, 2.0, 3.0], ..Default::default() }), mesh_json: None },
        };
        store::test_support::assert_op_line_round_trip(&operation);
    }
    ...
    fn op_text_round_trip_add_paint_layer() {
        let operation = LowpolyOperation::AddPaintLayer { object_id: "obj-1".into(), index: 1, layer: lowpoly::LowpolyPaintLayer::new("Detail") };
        store::test_support::assert_op_line_round_trip(&operation);
    }
