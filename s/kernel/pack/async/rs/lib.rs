//! 📦 `pack_async` — runtime-neutral async access layer for `pack` files: the `AsyncPackSource`
...
/// 📥 A random-access read source reachable only through `async`, e.g. a network range-fetcher
...
/// 🛑 A clone-cheap, thread-safe flag a caller can flip to short-circuit an in-flight
...
/// 👀 Busy-polls `token` (with a short sleep between polls to avoid pegging a core) and resolves
...
//#endregion 🔖AsyncSource

//#region 🔖Scheduler
/// 🚦 Relative urgency of a read, highest (`Critical`) to lowest (`Background`); consulted by
...
/// 📨 One caller's request for a byte range, tagged with the priority `BoundedDemand` should
...
/// 🤝 True iff `a` and `b` overlap or sit back-to-back (touching), so a single physical read of
...
/// ➕ The smallest `ByteRange` spanning both `a` and `b`.
...
/// 🌱 Lifecycle of one coalesced physical read.
enum GroupState {
...
/// ⏳ Non-leader wait: parks on a `Group` until it reaches `Done`, then hands back a clone of
...
/// 🧵 Coalesces adjacent/overlapping `AsyncPackSource` reads into a single physical fetch,
...
impl<S: AsyncPackSource> ReadScheduler<S> {
    /// 🆕 A scheduler with a sensible default concurrency cap. Use `with_capacity` to tune it.
    pub fn new(source: S) -> Self {
        Self::with_capacity(source, 4)
    }

    /// 🆕 A scheduler that admits at most `max_concurrent_reads` physical reads at once.
    ...
    /// 📖 Reads `request.range`, coalescing with any compatible in-flight read and honoring
    ...
    /// 🔎 Finds a `Gathering` group whose range touches `range` and folds `range` into it
    ...
    /// 🚀 Closes `group` for merging, waits its turn under `demand`, and performs the physical
    ...
    /// 🏁 Records `result` as the group's outcome, wakes every waiter, and drops the group from
...
/// 🚧 A semaphore-like backpressure primitive capping concurrent in-flight reads, with priority-
...
impl BoundedDemand {
    ...
    /// 🎫 Waits for a free slot, weighing contention by `priority`, and returns an RAII permit
    ...
    /// 🔓 Frees one slot: transfers it to the highest-priority queued waiter if any are waiting,
...
mod tests {
    ...
    //#region 🔖AsyncSource
    /// 🧪 An in-memory `AsyncPackSource` test double that counts how many physical reads it
    /// actually served.
    ...
    /// 🧪 A test double whose `read_at` never resolves, used to prove `CancellationToken`
    ...
    //#region 🔖Backpressure
    /// 🧵 Polls `future` exactly once with a no-op waker, without consuming it if it is not
