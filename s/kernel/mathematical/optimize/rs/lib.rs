//! 🎯 Nonlinear least squares and robust estimation: Gauss-Newton, Levenberg-Marquardt, Schur-complement bundle solvers, robust losses and RANSAC consensus.

use mathematical_algebra::{cholesky, cholesky_solve, weighted_normal_equations, MatD, VecD};
use mathematical_random::Rng;
use std::collections::HashMap;

// #region 🔖Problem
/// 🧩 A nonlinear least-squares problem: `parameter_count()` unknowns, `residual_count()` residuals,
...
pub trait LeastSquaresProblem {
    ...
    /// 🧭 Retracts a local update `dx` onto the parameter manifold at `x`; ordinary vector addition
...
/// 🔬 Central-difference Jacobian via [`LeastSquaresProblem::plus`], for problems where an analytic
...
impl RobustLoss {
    ...
    /// 📉 Robust cost `rho(r)` for a squared residual `r2`, for reporting total cost under the loss.
...
// #endregion 🔖RobustLoss

// #region 🔖GaussNewton
/// 🧮 Assembles the IRLS-weighted `(JᵀWJ, JᵀWr)` normal-equation pieces and the robustified cost at
...
/// 🧮 Solves the (possibly damped) normal-equation system `A dx = b` via Cholesky, falling back to
...
/// 📐 Local quadratic model's predicted cost reduction `0.5 * dx . (lambda * diag(H) * dx - g)`
...
/// 🚶 Undamped Gauss-Newton: repeatedly solves the IRLS-reweighted normal equations `JᵀWJ dx = -JᵀWr`
...
/// 📦 Outcome of [`levenberg_marquardt`]: the solution, its cost, and the final (undamped) `JᵀWJ`
...
/// 🎯 Damped Gauss-Newton (Levenberg-Marquardt) minimization of `problem`, robustified via IRLS
...
// #endregion 🔖LevenbergMarquardt

// #region 🔖SchurBundle
/// 🌉 One residual term in a bipartite least-squares problem: touches an "A" block, a "B" block, or
...
/// 🌉 A bundle-adjustment-shaped bipartite least-squares problem: many small "A" blocks (e.g. cameras)
...
/// 📦 Outcome of [`schur_lm`]: refined A/B blocks, final cost, and the marginal covariance diagonal
...
/// 🧮 Accumulates `Jᵀ W J` into `target` (`target` sized `j.cols x j.cols`) for a single term's block Jacobian.
...
/// 🧮 Accumulates the `ja^T W jb` cross block (sized `ja.cols x jb.cols`) for a term touching both sides.
...
/// 🧮 Accumulates `Jᵀ W r` into `target`.
...
/// 🧮 One robustified pass over every residual term, accumulating the block-diagonal `Haa`/`Hbb`
...
/// 🧮 One damped Schur-complement LM step: eliminates every B block analytically (each `Hbb` is small
...
/// 🧮 Marginal covariance diagonal blocks: inverts the dense reduced system once (column-by-column
...
/// 🎯 Levenberg-Marquardt over a [`BipartiteResiduals`] problem via Schur-complement elimination of
...
/// 📊 The marginal covariance diagonal blocks computed during `result`'s final accepted iteration.
pub fn camera_covariances(result: &SchurResult) -> &[MatD] {
    &result.a_block_covariance_diagonals
}
// #endregion 🔖SchurBundle

// #region 🔖Consensus
/// 🎲 A minimal-sample model solver for RANSAC-family consensus: fits a model (possibly several
...
/// 📏 How a candidate model's fit to the full dataset is scored.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// 📦 The winning model, its full inlier index set (re-evaluated over all of `data`, not just the
...
/// 📏 `(msac_cost, inlier_count)` for `model` against every datum — both tracked together so
...
/// 🎲 Shared RANSAC sampling loop: draws minimal samples, scores every hypothesis `solver.solve`
...
/// 🎲 Standard RANSAC: repeatedly fits a minimal-sample model and keeps the best-scoring hypothesis,
...
/// 🎲 Locally-optimized RANSAC: identical to [`ransac`], except every time a new best model is found,
...
/// 🔍 Golden-section search for the minimum of a unimodal `f` on `[a, b]`, shrinking the bracket by
...
/// 🔍 Brent's method on a bounded interval `[a, b]`: combines golden-section steps with safeguarded
...
mod tests {
    use super::*;
    use mathematical_algebra::solve_llsq;

    // #region 🔖RosenbrockTests
    struct Rosenbrock;
    ...
    // #endregion 🔖RosenbrockTests

    // #region 🔖GaussNewtonTests
    ...
    // #endregion 🔖GaussNewtonTests

    // #region 🔖RobustRegressionTests
    ...
    // #endregion 🔖RobustRegressionTests

    // #region 🔖SchurConsistencyTests
    ...
    // #endregion 🔖SchurConsistencyTests
