//! 🧵 `std::thread`-based parallel multi-start: launches several independent solve attempts
...
// #region 🔖Seed
/// 🧵 Deterministically derives attempt `i`'s seed from `base_seed` — a single splitmix64-style
/// mixing step, not a call into `mathematical_random::Rng` (this only needs to be a fast,
...
// #region 🔖MultiStart
/// 🧵 Runs `attempts` independent [`search::solve`] calls in parallel (each with a seed derived
...
mod tests {
    ...
    fn k_graph(n: usize, k: usize) -> (CompiledModel, crate::topology::GraphTopology) {
        let mut b = ModelBuilder::new();
