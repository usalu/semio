#!/usr/bin/env bun
/** 🧭 `@semio-tech/s-play` task router: `bun ./script.ts <dev|build|test> [fixture <id>] [args…]`. */
import {
	BundleScript,
	ScriptRouter,
...
import { resolveSPlayFixtureSlug } from "./fixture-slugs.ts";

class DevScript extends BundleScript {
	run(segments: string[]): void {
		const { segments: viteSegments, fixtureEnv } = consumePlaygroundFixtureArgv(segments, resolveSPlayFixtureSlug);
