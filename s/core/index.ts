...
/** @emoji 🖥️ `@semio-tech/s-core` — studio CQRS store, programs, resources, media graph, dev JSON backbone. */
// #endregion 🧲Header
...
//#region 🔖Schemas
export const S_STUDIO_SCHEMA = "s.studio" as const;
export const S_MEDIA_GRAPH_SCHEMA = "s.media-graph" as const;

export type SResourceKindId = SResourcesDescriptorKindId;
export { SRESOURCES_DESCRIPTOR_IDS as S_RESOURCE_KIND_IDS };

export interface SBackboneRef {
...
export interface SSourceDocument {
...
export interface SAppInstance {
	readonly id: string;
	readonly programId: string;
	readonly appId: string;
	readonly label: string;
	readonly yields: SResourceKindId;
	readonly sourceDocument: SSourceDocument;
}

export interface SMediaPort {
	readonly id: string;
...
export interface SMediaGraphNode {
	...
	readonly inputs: readonly SMediaPort[];
	readonly outputs: readonly SMediaPort[];
}

export interface SMediaGraphEdge {
	readonly id: string;
...
export interface SMediaGraph {
	readonly schema: typeof S_MEDIA_GRAPH_SCHEMA;
	readonly nodes: readonly SMediaGraphNode[];
	readonly edges: readonly SMediaGraphEdge[];
}

export interface SStudioProjection {
	...
	readonly mediaGraph: SMediaGraph;
}

export interface SStudioOperation {
	readonly op:
		| "spawnAppInstance"
		| "removeAppInstance"
		| "connectMediaPorts"
...
export interface SStudioChange {
	readonly id: string;
	readonly forwards: readonly SStudioOperation[];
	readonly backwards: readonly SStudioOperation[];
	readonly description?: string;
	readonly savedAt?: string;
}

export interface SStudioCheckpoint {
	readonly id: string;
...
export interface SStudioAlternative {
	readonly id: string;
	readonly name: string;
	readonly checkpointIds: readonly string[];
}

export interface SStudioVcs {
	readonly initialProjection: SStudioProjection;
	readonly operations: readonly SStudioChange[];
	readonly checkpoints: readonly SStudioCheckpoint[];
	readonly alternatives: readonly SStudioAlternative[];
}

export interface SStudioDocument {
	readonly schema: typeof S_STUDIO_SCHEMA;
	readonly id: string;
	readonly name: string;
	readonly vcs: SStudioVcs;
	readonly backbone?: SBackboneRef;
}
//#endregion 🔖Schemas
...
export interface SProgramDefinition extends PlatformDefinition {
	readonly apps: readonly (SAppRegistration & { readonly modes: readonly { readonly id: string; readonly label: string }[] })[];
}

export interface SResourceDescriptor {
...
function descriptorPresentation(kind: SResourceKindId): SResourceDescriptor {
...
export function listSResourceDescriptors(): readonly SResourceDescriptor[] {
	return SRESOURCES_DESCRIPTOR_IDS.map(descriptorPresentation);
}

export function sResourceDescriptor(kind: SResourceKindId): SResourceDescriptor {
	return descriptorPresentation(kind);
}

export function resourcesCompatible(left: SResourceKindId, right: SResourceKindId): boolean {
	return left === right;
}
...
export function sProgramById(programId: string): SProgramDefinition | undefined {
	return listSPrograms().find((program) => program.id === programId);
}

export function sAppRegistration(programId: string, appId: string): SAppRegistration | undefined {
	const program = sProgramById(programId);
...
export function registerSFixtureJsonResolver(resolver: (slug: string) => string | null): void {
	sFixtureJsonResolver = resolver;
}

function resolveSourceDocument(source: SSourceDocument, context?: AppMaterializeContext): SSourceDocument {
	const resolveFixture = context?.resolveFixtureJson ?? sFixtureJsonResolver ?? undefined;
...
export function materializeAppInstanceProjection(instance: SAppInstance, context?: AppMaterializeContext): unknown {
...
export interface SMediaGraphValidation {
	readonly ok: boolean;
	readonly errors: readonly string[];
}

export function emptyMediaGraph(): SMediaGraph {
	return { schema: S_MEDIA_GRAPH_SCHEMA, nodes: [], edges: [] };
}

export function validateMediaGraph(graph: SMediaGraph): SMediaGraphValidation {
	const errors: string[] = [];
...
export function createSId(prefix = "s"): string {
	sIdCounter += 1;
	return `${prefix}-${sIdCounter}`;
}

export function defaultStudioProjection(): SStudioProjection {
	return { programs: [], activeProgramId: null, activeAlternativeId: null, appInstances: [], mediaGraph: emptyMediaGraph() };
}

export function createEmptyStudioDocument(id = "studio", name = "Studio"): SStudioDocument {
	return {
		schema: S_STUDIO_SCHEMA,
		id,
		name,
		vcs: {
...
function cloneProjection(projection: SStudioProjection): SStudioProjection {
	return {
...
function applyStudioOperation(projection: SStudioProjection, operation: SStudioOperation): SStudioProjection {
	...
	switch (operation.op) {
		...
		case "applyAppOperation": {
			...
			const nextSource = operation.payload.nextSource as SSourceDocument;
		...
		case "spawnAppInstance": {
			const instance = operation.payload.instance as SAppInstance;
		...
		case "connectMediaPorts": {
			const edge = operation.payload.edge as SMediaGraphEdge;
...
export function materializeStudioProjection(document: SStudioDocument, appliedChangeIds: readonly string[] = []): SStudioProjection {
...
export function parseSStudioDocument(raw: unknown): SStudioDocument {
	const value = raw as Partial<SStudioDocument>;
	if (value.schema !== S_STUDIO_SCHEMA) throw new Error(`expected schema ${S_STUDIO_SCHEMA}`);
	...
	return value as SStudioDocument;
}

export function sStudioDocumentToJson(document: SStudioDocument): string {
	return JSON.stringify(document, null, 2);
}

export function sStudioDocumentFromJson(json: string): SStudioDocument {
...
export class StudioStore {
	private document: SStudioDocument;
	private appliedChangeIds: string[] = [];
	private redoChangeIds: string[] = [];
	private listeners = new Set<() => void>();
	private generation = 0;
	private projectionSnapshot: SStudioProjection | undefined;
	private projectionSnapshotGeneration = -1;
	private onAfterMutation?: () => void;

	constructor(document: SStudioDocument, options?: { readonly onAfterMutation?: () => void }) {
	...
	getDocument(): SStudioDocument {
		return this.document;
	}

	projection(): SStudioProjection {
	...
	mediaGraphSnapshot(): SMediaGraph {
	...
	dispatch(command: StudioCommand): void {
		...
		if (command.kind === "createAlternative") {
			...
			const alternative: SStudioAlternative = {
				id: createSId("alt"),
				name: command.name,
	...
	private commandToChange(command: StudioCommand): SStudioChange | null {
		switch (command.kind) {
			case "undo":
			case "redo":
			...
			case "spawnAppInstance": {
				const registration = sAppRegistration(command.programId, command.appId);
				if (!registration) throw new Error(`unknown app ${command.programId}/${command.appId}`);
				const instance: SAppInstance = {
					id: createSId("app"),
				...
				const spawn: SStudioOperation = { op: "spawnAppInstance", payload: { instance, position: command.position ?? { x: 40, y: 40 } } };
				const remove: SStudioOperation = { op: "removeAppInstance", payload: { instanceId: instance.id } };
				return { id: createSId("change"), forwards: [spawn], backwards: [remove] };
			}
			case "removeAppInstance": {
				...
				const forwards: SStudioOperation[] = [{ op: "removeAppInstance", payload: { instanceId: instance.id } }];
				const backwards: SStudioOperation[] = [
				...
				return { id: createSId("change"), forwards, backwards };
			}
			case "connectMediaPorts": {
				const edge: SMediaGraphEdge = {
					id: createSId("edge"),
			...
			case "patchAppInstances": {
				...
				const projection = this.projection();
				const forwards: SStudioOperation[] = [];
				const backwards: SStudioOperation[] = [];
...
export class DevJsonBackbone {
	...
	load(json: string): SStudioDocument {
		return sStudioDocumentFromJson(json);
	}

	loadAttached(): SStudioDocument | null {
	...
	sync(document: SStudioDocument): string {
		const synced =
			this.uri != null ? { ...document, backbone: { kind: "dev" as const, uri: this.uri } } : document;
		const json = sStudioDocumentToJson(synced);
...
export class RemoteJsonBackbone {
	...
	sync(_document: SStudioDocument): never {
		this.lastConflict = {
...
if (import.meta.vitest) {
	...
	describe("s studio", () => {
		...
		it("round-trips dev JSON backbone", () => {
			...
			backbone.attach("dev://demo.s.json");
			...
			expect(loaded.backbone?.uri).toBe("dev://demo.s.json");
		});
		...
		it("spawns and materializes every technology program id", async () => {
			...
			for (const program of listSPrograms()) {
				if (program.id === "s.system") continue;
			...
			for (const row of spawned) {
				...
				if (row.sourceFormat === "forms.form") {
					expect((projection as { schema?: string }).schema).toBe("forms.form");
				}
			}
		});
	});
