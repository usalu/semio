//! 🎛️ S Studio program — designer OS shell bundled as a hot-swappable WASM program.
...
fn ensure_space_fixtures_registered() {
...
/// builds a blank one.
fn parse_demo_space_document() -> OsDocument {
...
fn demo_space_projection() -> OsProjection {
...
pub mod app_home {
    ...
    use super::{ensure_space_fixtures_registered, parse_demo_space_document};
    ...
    pub(crate) fn resolve_studio_document(space_id: &str) -> Option<OsDocument> {
    ...
    pub(crate) fn space_document_envelope_pack(document: &OsDocument) -> Option<store::DocumentPackFiles> {
        export_os_space_pack(document).ok()
    }

    #[cfg(test)]
    pub(crate) fn register_studio_port_for_test(space_id: &str, port: Arc<dyn OsBackbonePort>) {
    ...
    mod tests {
        use super::*;
        use semio_framework_program::{testkit, HistoryView, ProgramApp, VcsDocumentApp};
        ...
        fn create_space_navigates_without_download_and_opens_empty() {
            ...
            let space_id = uri.trim_start_matches("/spaces/");
            let document = resolve_studio_document(space_id).expect("created studio");
            assert_eq!(document.name, "Fresh Studio");
            assert!(document.backbone.is_none(), "ephemeral studio must not attach backbone");
            assert!(document.vcs.initial_projection.app_instances.is_empty());

            let mut studio = super::super::app_space::SpaceApp::new();
...
pub mod app_space {
    ...
    use super::{demo_space_projection, ensure_space_fixtures_registered, parse_demo_space_document};
    use semio_framework_os::{
        ...
        OS_WORKFLOW_VFS_ROOT_ID, OS_SPACE_SCHEMA,
    };
    ...
    #[serde(rename_all = "camelCase")]
    struct SpacePanelState {
        #[serde(default)]
        active_panel_tab: String,
        #[serde(default)]
        programs: Vec<SpaceProgramEntry>,
        #[serde(default)]
    ...
    //#region 🔖DocumentHelpers
    fn parse_panel_state(view_state: &ViewState) -> SpacePanelState {
        view_state
            .panel_json
            .as_deref()
    ...
    fn panel_json(panel: &SpacePanelState) -> String {
    ...
    /// The store-free operation-builder the program uses in place of os-core's `OsStore::spawn_app_instance`
    ...
    fn build_catalogue_tree(panel: &SpacePanelState, labels: &SStudioLabels) -> UiNode {
        let programs: Vec<SpaceProgramEntry> = if panel.programs.is_empty() {
            let mut entries = Vec::new();
            ...
            match action {
                ...
                "setActiveExample" => {
                        ...
                        .filter(|value| !value.is_empty())
                    {
                        ...
                        // in-place whole-document swap on the program side anymore); an empty id is the
                ...
                "exportStudioPack" => {
                    ...
                    if let Some(document) = super::app_home::resolve_studio_document(&space_id) {
                ...
                "importSpacePackPayload" => {
                    if let Some(payload) = args.and_then(|value| value.get("payload")).and_then(|value| value.as_str()) {
                        ...
                        if let Ok(bytes) = base64::engine::general_purpose::STANDARD.decode(base64_part) {
                            ...
                            let _ = import_os_space_from_pack(&bytes, &empty_spr, super::app_home::catalog_port());
                        }
                    }
                    return ActionEmit::default();
                }
                ...
                "openSpace" => {
                        ...
                        .and_then(|value| value.as_str())
                    {
                        ...
                        let document = super::app_home::resolve_studio_document(space_id)
                            .or_else(|| {
                                if space_id == "demo" {
                        ...
                        if let Some(files) = super::app_home::space_document_envelope_pack(&document) {
                            eprintln!(
    ...
    mod tests {
        use super::*;
        use semio_framework_os::{
            apply_os_operation, merge_os_program_definition, os_baseline_resource, os_in_port, os_out_port, register_artifact_descriptor, validate_workflow, MediaClass, MediaForm, MediaType, MediaWireFormat, OsAppResourceSpec,
            OsMediaFormat, OsWorkflowNode, OsMediaPort, OsPlatformAppInput, OsPlatformInput, ArtifactKindSpec,
        };
        use semio_framework_program::{testkit, HistoryView, ModeDefinition, ProgramApp, UiControlNode, UiNode, VcsDocumentApp};
        ...
        fn space_emit(app: &mut SpaceApp, projection: &OsProjection, action: &str, args: Value) -> ActionEmit<OsOperation> {
            let history = empty_history();
        ...
        #[test]
        fn open_instance_emits_open_program_instance_effect_matching_instance() {
            seed_draw_program();
            let mut app = SpaceApp::new();
            let projection = demo_space_projection();
...
fn register_s_exports() {
    semio_framework_program::program_runtime::register_document_codec_for_app::<app_home::HomeApp>("s.home");
    semio_framework_program::program_runtime::register_document_codec_for_app::<app_space::SpaceApp>(OS_SPACE_SCHEMA);
}
//#endregion 🔖DocumentCodecs

//#region 🔖Manifest
fn bundle() -> semio_framework_program::ProgramBundle {
    register_s_exports();
    semio_framework_program::ProgramBundle::new("s", "S Studio", "0.1.0")
        .local_backbone_storage()
        .register_document_app(app_home::create_home_app(), || app_home::HomeApp)
        .register_document_app(app_space::create_space_app(), app_space::SpaceApp::new)
}
