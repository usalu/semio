...
export function actorNetworkData(markdown = readFileSync(ACTOR_NETWORK_LEDGER, "utf8")): ActorNetworkData {
  ...
  for (const country of COUNTRIES) {
    ...
    for (const type of [...new Set(countryNodes.map((node) => node.entityType))].sort()) {
      countryNodes.filter((node) => node.entityType === type)
        .sort((a, b) => normalizedName(a.name).localeCompare(normalizedName(b.name), "en") || a.key.localeCompare(b.key, "en"))
...
export function validateActorNetwork(data = actorNetworkData()): void {
  const errors: string[] = [];
...
function countryLayout(nodes: ActorNode[], edges: ActorEdge[], width: number, height: number, attempt: number): LayoutNode[] {
  ...
  const unresolved = nodes.filter((node) => !anchors.has(node.key)).sort((a, b) => a.key.localeCompare(b.key, "en"));
...
function connectedComponents(nodes: ActorNode[], edges: ActorEdge[]): ActorNode[][] {
  ...
  for (const start of [...nodes].sort((a, b) => a.key.localeCompare(b.key, "en"))) {
...
function countryPanels(country: string, nodes: ActorNode[], edges: ActorEdge[]): LayoutPanel[] {
  ...
  const components = connectedComponents(nodes, edges).sort((a, b) => b.length - a.length || a[0].key.localeCompare(b[0].key, "en"));
...
function edgeFingerprint(edges: ActorEdge[]): string {
  const normalized = [...edges].sort((a, b) => a.key.localeCompare(b.key, "en")).map((edge) => JSON.stringify(edge)).join("\n");
...
function renderActorTables(data: ActorNetworkData, ids: Map<string, string>): string {
  ...
  for (const country of COUNTRIES) {
    const nodes = data.nodes.filter((node) => node.country === country)
      .sort((a, b) => (ids.get(a.key) ?? "").localeCompare(ids.get(b.key) ?? "", "en", { numeric: true }));
    const urls: string[] = [];
...
function renderActorPrograms(data: ActorNetworkData): string {
  ...
  for (const program of [...data.programs].sort((a, b) => a.country.localeCompare(b.country, "en") || a.name.localeCompare(b.name, "en")))
...
export function validateActorNetworkRenderSync(data = actorNetworkData()): void {
...
export function validateActorNetworkAssets(data: ActorNetworkData): void {
  for (const node of data.nodes) if (node.assetPath) assert.ok(existsSync(join(berichtRoot, "📑️forschungsbericht", node.assetPath)), `Missing actor asset ${node.key}: ${node.assetPath}`);
}

function actorNetworkArtifacts(ledger: string): ReadonlyMap<string, string> {
  ...
  return new Map([[ACTOR_NETWORK_FIGURES, figures], [ACTOR_NETWORK_TABLES, tables], [ACTOR_NETWORK_PROGRAMS, programs], [ACTOR_NETWORK_INTRO, intro]]);
}
...
class ActorNetworkScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    ...
    if (command === "preview") {
      ...
      const nodes = [...files].map(([path, content]) => ({ bytesBase64: Buffer.from(content).toString("base64"), mode: 0o644, nodeKind: "file", path: relative(this.repoRoot, path).replaceAll("\\", "/").normalize("NFC") })).sort((left, right) => left.path < right.path ? -1 : left.path > right.path ? 1 : 0);
      process.stdout.write(`${JSON.stringify({ contractId: "report-actor-network", schemaVersion: 1, nodes, staleRemovals: [] })}\n`);
      return;
    }
    if (command === "check") {
      const files = actorNetworkArtifacts(readFileSync(ACTOR_NETWORK_LEDGER, "utf8"));
