...
import { stagePrintSources, printCompilerName as compilerName } from "../../../../🧰️framework/🛍️products/📓️print/🔨️modules/📥️source-staging/🟦️.ts";
...
const berichtRoot = ownerRoot;
const DOCUMENTS = { zwischenbericht: "📋️zwischenbericht/📋️zwischenbericht.tex", forschungsbericht: "📑️forschungsbericht/📑️forschungsbericht.tex", kompaktbericht: "📝️kompaktbericht/📝️kompaktbericht.tex" } as const;
//#region Akteursnetz
const ACTOR_NETWORK_ROOT = join(berichtRoot, "📎️anhang");
const ACTOR_NETWORK_LEDGER = join(ACTOR_NETWORK_ROOT, "📇️akteursnetz-erweiterung-kandidaten.md");
const ACTOR_NETWORK_FIGURES = join(ACTOR_NETWORK_ROOT, "🕸️akteursnetz-figuren.tex");
const ACTOR_NETWORK_TABLES = join(ACTOR_NETWORK_ROOT, "📊️akteursnetz-tabellen.tex");
const ACTOR_NETWORK_PROGRAMS = join(ACTOR_NETWORK_ROOT, "🎯️akteursnetz-programme.tex");
const ACTOR_NETWORK_INTRO = join(ACTOR_NETWORK_ROOT, "👥️akteursnetz.tex");
...
function actorNetworkData(markdown = readFileSync(ACTOR_NETWORK_LEDGER, "utf8")): ActorNetworkData {
...
function validateActorNetwork(data = actorNetworkData()): void {
  ...
  for (const node of data.nodes) {
    ...
    if (node.assetPath && !existsSync(join(berichtRoot, "📑️forschungsbericht", node.assetPath))) errors.push(`missing asset ${node.key}: ${node.assetPath}`);
...
function renderActorFigures(data: ActorNetworkData, ids: Map<string, string>): string {
  const out = ["\\section{Akteursnetze nach Land}",
    ...
    "\\begin{GraphSpread}"];
  for (const country of COUNTRIES) {
    console.log(`[DEBUG] Akteursnetz layout: ${country}`);
...
function validateActorNetworkRenderSync(data = actorNetworkData()): void {
  const stored = new Map([ACTOR_NETWORK_FIGURES, ACTOR_NETWORK_TABLES, ACTOR_NETWORK_PROGRAMS].map((path) => {
    if (!existsSync(path)) throw new Error(`missing Akteursnetz fragment: ${path}`);
  ...
  for (const [path, content] of expected) {
    const actual = stored.get(path)!;
    if (actual === content) continue;
    const lines = content.split("\n"), actualLines = actual.split("\n");
    const index = lines.findIndex((line, index) => line !== actualLines[index]);
    throw new Error(`stale Akteursnetz fragment: ${path}:${index + 1}\nexpected: ${lines[index]}\nactual: ${actualLines[index]}`);
  }
}

function renderActorNetwork(): void {
  const ledger = readFileSync(ACTOR_NETWORK_LEDGER, "utf8");
  const data = actorNetworkData(ledger);
  validateActorNetwork(data);
  const edgesBefore = edgeFingerprint(data.edges);
  const ids = displayIds(data.nodes);
  const figures = renderActorFigures(data, ids);
  const tables = renderActorTables(data, ids);
  const programs = renderActorPrograms(data);
  if (readFileSync(ACTOR_NETWORK_LEDGER, "utf8") !== ledger) throw new Error("Akteursnetz source changed during rendering; rerun akteursnetz render.");
  writeFileSync(ACTOR_NETWORK_FIGURES, figures);
  writeFileSync(ACTOR_NETWORK_TABLES, tables);
  writeFileSync(ACTOR_NETWORK_PROGRAMS, programs);
...
function stageReportSources(texAbs: string, outDir: string): string {
  const documentRoot = dirname(texAbs);
  const family = Object.values(DOCUMENTS).some((path) => join(ownerRoot, path) === texAbs);
  const sourceRoot = family ? ownerRoot : documentRoot;
  const entries = [basename(texAbs), "📚️references.bib", "🖼️asset", "📎️anhang"]
    .map((entry) => join(documentRoot, entry)).filter(existsSync).map((path) => relative(sourceRoot, path));
  if (family) entries.push("📎️anhang");
  return stagePrintSources(sourceRoot, entries, join(outDir, "source")).get(relative(sourceRoot, texAbs).replaceAll("\\", "/"))!;
}

function resolveTexPath(segments: string[]): string {
  const name = segments[0] ?? "zwischenbericht";
  const raw = name in DOCUMENTS ? join(ownerRoot, DOCUMENTS[name as keyof typeof DOCUMENTS]) : name;
...
function resolveOutDir(texAbs: string, segments: string[]): string {
  ...
  return process.env.SEMIO_PRINT_OUTPUT_DIR ? join(resolve(process.env.SEMIO_PRINT_OUTPUT_DIR), compilerName(basename(texAbs, ".tex"))) : join(dirname(texAbs), "dist");
}

async function buildDocument(segments: string[]): Promise<void> {
  await provisionPrintFonts();
  const texAbs = resolveTexPath(segments);
  if (texAbs === join(ownerRoot, DOCUMENTS.forschungsbericht)) {
...
async function watchDocument(segments: string[]): Promise<void> {
  const texAbs = resolveTexPath(segments);
  const outDir = resolveOutDir(texAbs, segments);
  const roots = [dirname(texAbs), ACTOR_NETWORK_ROOT, join(packageRoot, "../../../../🧰️framework/🛍️products/📓️print/🖋️latex")];
  ...
  for (const root of roots) {
    ...
    watch(root, { recursive: true }, (_event, file) => {
      ...
      const outputRelative = relative(outDir, abs);
      if (outputRelative === "" || (!outputRelative.startsWith("..") && !isAbsolute(outputRelative))) return;
      if (abs.includes(".semio-dark") || /-dark\.tex$/i.test(abs)) return;
      if (!/\.(tex|bib|sty|cls|ttf|json|md|png|jpe?g|pdf)$/i.test(abs)) return;
      try {
...
class BuildScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    if (segments.length) await buildDocument(segments);
    else for (const name of Object.keys(DOCUMENTS)) await buildDocument([name]);
  }
}
...
class TestScript extends BundleScript {
  run(segments: string[]): void {
    ...
    validateActorNetwork(data);
    if (level !== "quick") validateActorNetworkRenderSync(data);
  }
}
...
const router = new ScriptRouter(packageRoot).register("build", BuildScript).register("watch", WatchScript).register("latex", LatexScript).register("akteursnetz", ActorNetworkScript).register("test", TestScript);
