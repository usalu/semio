...
export class SetupScript extends Script {
  ...
  private runWorkspaceCodegen(): void {
    ...
    runCmd("bun", ["./📜script.ts", "build"], {
      cwd: join(this.root, "🧰framework/🔨module/🖼️asset/⚡️implementation/🟦typescript"),
      ...orchestratorBudgetOpts(),
    });
  ...
  run(_segments: string[]): void {
    ...
    const runGenerate = () => {
      if (runCmdStatus(BUN, [join(this.root, "📜script.ts"), "generate"], { cwd: this.root, ...orchestratorBudgetOpts() }) !== 0) {
        console.log("[start] `bun run generate` did not refresh all `.🦑repo/🛂manifest` bundles (Neo4j may be offline).");
      }
    };
...
export class GenerateScript extends Script {
  run(segments: string[]): void {
    ...
    console.log(`[generate] Neo4j Cypher export finished (${successes} ok, ${failures} skipped/failed) under .🦑repo/🛂manifest.`);
  }
}
...
export class TestScript extends Script {
  ...
  /** 📊Walks every `*.lcov`/`lcov.info`/`coverage.info`/`*.cover` file under `.🦑repo/📊metrics/coverage/`, merges them into one repo-wide LCOV, writes `summary.json`, and hard-fails below the 95% threshold — the exhaustive-level gate. */
...
/**
 * 🛂 Neo4j → `.🦑repo/🛂manifest/<graph>.cypher` export (pure module; invoked from root `script.ts`). Product graphs are fixed specs; extra Bolt graphs use `NEO4J_EXTRA_GRAPH_DATABASES` (comma-separated). Argv segments join with `-` via `joinNeo4jGraphDatabaseName`.
 */
...
/** 🗑️Env key: comma-separated extra Bolt graph names for `bun run generate` and native `.🦑repo/🛂manifest/*.cypher` stubs. */
 ...
 * V2 (structure), V3 (coupling) audit findings under `.🦑repo/🎫tickets/26/07/18/WAVE-4-*-AUDIT`, wired via
...
/**
 * 🎫 Wave 4 V1 duplication audit (`.🦑repo/🎫tickets/26/07/18/WAVE-4-V1-DUPLICATION-HUNTER-AUDIT`): both crates
...
/**
 * 🎫 Wave 4 V3 coupling audit (`.🦑repo/🎫tickets/26/07/18/WAVE-4-APP-TO-APP-COUPLING-AND-FRAMEWORK-IDENTITY-LEAK-AUDIT`):
...
/**
 * 🎫 pack/ binary-document-layer rollout lock step (`.🦑repo/🎫tickets/26/07/27/
...
/**
 * 🎫 CW7 command-envelope law lock step (`.🦑repo/🎫tickets/26/07/27/
...
function policyLabelsStructBreaches(scope: string, content: string): BreachRecord[] {
  ...
  while ((m = re.exec(content))) {
    ...
    breaches.push({
      ...
      solution: allowed ? `See .🦑repo/🎫tickets/26/07/18/WAVE-4-V1-DUPLICATION-HUNTER-AUDIT for the pending decision; if formally accepted, keep this allowlisted with that citation.` : `Route ${structName} through semio_framework_plugin::app_labels! { ... }, or if it needs a second axis, add it to POLICY_LABELS_TWO_AXIS_ALLOWLIST citing a ticket.`,
    });
  }
  return breaches;
}
