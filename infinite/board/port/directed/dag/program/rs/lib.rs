//! 🔀 DAG program — declarative DAG play app bundled as a hot-swappable WASM program.
...
fn build_document_tree(document: &DagDocument, selected: &[String], labels: &DagPlayLabels) -> UiNode {
    ...
    semio_framework_program::ui_tree_stamp_presence(&mut sections, &selected_ids, &std::collections::HashSet::new());
...
fn register_dag_exports() {
    ...
    semio_framework_program::program_runtime::register_document_codec_for_app::<DagPlayApp>(DAG_DOCUMENT_SCHEMA);
}
