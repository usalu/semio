//! ⚡ Imperative program — declarative imperative play app bundled as a hot-swappable WASM program.
...
fn build_inspector_tree(document: &ImperativeDocument, selected: &[String], labels: &ImperativeLabels) -> UiNode {
    if selected.is_empty() {
        return ui_declarative_sections_to_tree(&[semio_framework_program::UiSectionNode {
...
fn register_imperative_exports() {
    semio_framework_program::program_runtime::register_document_codec_for_app::<ImperativePlayApp>(IMPERATIVE_DOCUMENT_SCHEMA);
}
