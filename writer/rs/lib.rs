...
impl OperationDiff<WriterProjection> for WriterDiff {
    fn apply(&self, projection: &WriterProjection) -> WriterProjection {
        ...
        WriterProjection {
            ...
            camera: self.camera.clone().unwrap_or_else(|| projection.camera.clone()),
            ..projection.clone()
        }
    }
...
mod writer_vcs_tests {
    ...
    fn seeded_store() -> WriterStore {
        WriterStore::new(create_document_vcs_envelope(
            "writer.document",
            "writer",
    ...
    fn writer_document_vcs_replays_camera_and_document_ops() {
        ...
        store
            .dispatch(DocumentVcsCommand::Apply {
                operations: vec![WriterOp::SetCamera { camera: WriterCamera { x: 4.0, y: 5.0, zoom: 2.0 } }],
                description: None,
            })
            .expect("apply camera");
        ...
        let replacement = WriterProjection {
            schema: "writer.document".into(),
            id: "jack".into(),
            language_id: "jack".into(),
            uri: "writer://jack".into(),
            text: "MATCH (a) RETURN a".into(),
            camera: default_camera(),
        };
        store
            .dispatch(DocumentVcsCommand::Apply {
                operations: vec![WriterOp::SetDocument { document: replacement }],
                description: None,
            })
