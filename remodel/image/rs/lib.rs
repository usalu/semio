...
impl ImageGray {
    ...
    /// 🔍 Pixel value at integer coordinates `(x, y)`.
    ...
    /// 📐 Bilinear sample at subpixel `(x, y)`; coordinates are clamped to the image border, and an empty image samples to `0`.
...
/// 🎨 Row-major 8-bit RGBA image with interleaved channels; pixel `(x, y)` occupies `data[(y * width + x) * 4 ..][..4]`.
#[derive(Clone, Debug, PartialEq)]
...
impl ImageRgba8 {
    /// 🎨 Zero-filled (transparent black) RGBA image of the given size.
    pub fn new(width: u32, height: u32) -> Self {
        Self { width, height, data: vec![0; (width as usize) * (height as usize) * 4] }
    }

    /// 🔍 RGBA pixel at integer coordinates `(x, y)`.
    pub fn get_rgba(&self, x: u32, y: u32) -> [u8; 4] {
        let idx = ((y * self.width + x) * 4) as usize;
        [self.data[idx], self.data[idx + 1], self.data[idx + 2], self.data[idx + 3]]
    }

    /// 📐 Bilinear RGB sample at subpixel `(x, y)`, normalized to `[0, 1]` per channel; coordinates are clamped to the image border, and an empty image samples to black.
...
/// 📥 Decodes a PNG byte stream into RGBA; 8-bit gray, gray-alpha, RGB and RGBA (plus paletted) inputs are expanded to RGBA, and 16-bit inputs are downconverted to 8-bit.
...
/// 📤 Encodes an RGBA image as an 8-bit RGBA PNG byte stream.
...
/// 📤 Encodes row-major 16-bit grayscale samples as a 16-bit grayscale PNG byte stream (big-endian per the PNG spec), for lossless DSM/heightfield export.
...
// #region 🔖JpegTables
/// 🔀 Zig-zag scan position `k` (0..64) to natural row-major block index `y * 8 + x`, per JPEG's DCT coefficient serialization order.
...
/// 🧮 Annex K.1 luminance quantization base table, natural (row-major) order.
...
/// 🧮 Annex K.2 chrominance quantization base table, natural (row-major) order.
...
/// 🌳 Canonical Huffman spec as (code-length counts for lengths 1..=16, symbols in code order); shared wire format for DHT segments and the Annex K.3 recommended tables.
struct JpegHuffSpec {
    counts: [u8; 16],
    symbols: &'static [u8],
}
...
/// 🧮 IJG Annex K quality scaling: `quality < 50` scales coarser than 100%, `quality > 50` scales finer, `quality == 50` is identity; each base entry is scaled and clamped to the 8-bit DQT range `[1, 255]`.
...
// #endregion 🔖JpegTables

// #region 🔖JpegHuffman
/// 🌳 Decode-side canonical Huffman table built via the JPEG spec's `mincode`/`maxcode`/`valptr` algorithm (Annex F, Figure F.16 / F.22).
...
impl JpegHuffDecodeTable {
    /// 🌳 Assigns canonical codes to `symbols` in bit-length order (owned, so DHT-parsed tables never leak memory) and records the per-length decode boundaries.
    ...
    /// 🔍 Symbol at zero-based `valptr`-relative `index`, bounds-checked against corrupt/truncated-derived indices.
    fn symbol_at(&self, index: i32) -> Option<u8> {
        usize::try_from(index).ok().and_then(|i| self.symbols.get(i).copied())
    }
}

/// 🌳 Encode-side canonical code table: symbol byte value → `(code, bit length)`, built with the same canonical assignment as the decode table.
...
/// 🧮 JPEG magnitude category (`SSSS`): the number of bits needed to represent `abs(value)`, `0` for `value == 0`.
fn jpeg_category(value: i32) -> u8 {
    (32 - value.unsigned_abs().leading_zeros()) as u8
}

/// 🧮 EXTEND procedure (Annex F, Figure F.12): reconstructs a signed coefficient from its `size`-bit unsigned code word.
...
/// 🧮 Inverse of `jpeg_extend`: the `size`-bit code word for a signed coefficient already known to fit its category.
fn jpeg_signed_bits(value: i32, size: u8) -> u16 {
    if value < 0 { (value + (1 << size) - 1) as u16 } else { value as u16 }
}
// #endregion 🔖JpegHuffman

// #region 🔖JpegBits
/// 📖 MSB-first bit reader over an entropy-coded JPEG scan; transparently undoes byte stuffing (`0xFF 0x00` → literal `0xFF`) and stops (without consuming) at any other marker so restart markers can be handled by the caller.
...
impl<'a> JpegBitReader<'a> {
    /// 📖 Reader starting at byte offset `pos` in `data`.
    fn new(data: &'a [u8], pos: usize) -> Self {
        Self { data, pos, acc: 0, nbits: 0 }
    }

    /// 📖 Next destuffed entropy byte, or `Ok(None)` when the stream is sitting at an unconsumed marker.
    ...
    /// 📖 Single next bit, or `Ok(None)` at a marker boundary.
    ...
    /// 📖 `n` bits as an unsigned value, MSB first; a marker boundary before `n` bits are collected is a decode error.
    ...
    /// 📖 Decodes one Huffman symbol by walking bit-by-bit through `table`'s canonical code ranges.
    ...
    /// 🔁 Discards any partially-consumed byte and reads a `0xFFDn` restart marker at the now byte-aligned position.
...
// #region 🔖JpegDct
/// 🌊 8-point orthonormal DCT-II basis value `cos[(2x+1) u π / 16]`.
fn jpeg_dct_basis(x: usize, u: usize) -> f64 {
    (((2 * x + 1) as f64) * (u as f64) * std::f64::consts::PI / 16.0).cos()
}

/// 🌊 8-point forward DCT-II, orthonormally scaled (`C(0) = 1/√2`, `C(u > 0) = 1`, overall factor `1/2`) so it is its own exact matrix inverse via `jpeg_idct_1d`.
...
/// 🌊 8-point inverse DCT-III, the exact transpose of `jpeg_dct_1d`'s orthonormal matrix.
...
/// 🌊 Separable 8×8 transform: applies `pass` to each row then to each column of the resulting rows.
...
/// 🌊 Forward 2D DCT of an 8×8, level-shifted spatial block in natural row-major order.
fn jpeg_forward_dct_block(block: &[f64; 64]) -> [f64; 64] {
    jpeg_transform_2d(block, jpeg_dct_1d)
}

/// 🌊 Inverse 2D DCT back to an 8×8 spatial block in natural row-major order.
...
// #region 🔖JpegDecode
/// 📖 Bounds-checked byte cursor over a JPEG marker stream; every read fails cleanly on truncation instead of panicking.
struct JpegCursor<'a> {
    data: &'a [u8],
    pos: usize,
}
...
/// 📥 Parses an SOF0/SOF1 segment (length already positioned after the marker) into dimensions and component sampling/quantization ids; rejects non-8-bit precision and component counts other than 1 (grayscale) or 3 (YCbCr).
...
/// 📥 Parses a DQT segment (length already positioned after the marker) into `[natural-order table; up to 4]`, converting from the wire's zig-zag serialization.
...
/// 📥 Parses a DHT segment (length already positioned after the marker), which may carry multiple tables back to back, into `dc`/`ac` table slots by id.
...
/// 📥 Decodes one dequantized, natural-order spatial-domain 8×8 block from the entropy stream, updating `dc_pred` in place.
...
/// 📥 Decodes the entropy-coded scan into one padded sample plane per frame component, honoring restart markers every `restart_interval` MCUs (`0` disables restarts).
fn jpeg_decode_scan(
...
/// 📥 Decodes a baseline (SOF0/SOF1) JFIF byte stream into RGBA; chroma is nearest/box-upsampled per its sampling factors and combined via BT.601 full-range JFIF coefficients. Progressive (SOF2), lossless, arithmetic-coded and other non-baseline markers return `ImageError::UnsupportedJpeg`.
...
// #endregion 🔖JpegDecode

// #region 🔖JpegEncode
/// 🎨 Edge-clamped RGBA sample, defensive against a mismatched `data` buffer so `encode_jpeg` never panics on a malformed `ImageRgba8`.
...
/// 🎨 Builds full-resolution (padded to a multiple of 16) `Y`, `Cb`, `Cr` planes via BT.601 full-range JFIF coefficients, then box-downsamples `Cb`/`Cr` by 2× for 4:2:0.
...
/// 🎨 Level-shifted (`- 128`) natural-order 8×8 block read from a plane at `(bx, by)`.
...
/// 🎨 Forward-DCTs, quantizes (round-to-nearest) and zig-zag reorders one 8×8 block.
...
/// 📤 Encodes an RGBA image as a baseline (SOF0) JFIF byte stream: 4:2:0 chroma subsampling, the Annex K.3 recommended Huffman tables, and Annex K.1/K.2 quantization tables scaled by `quality` (`1..=100`, clamped). Degenerate `image` buffers (zero-sized or length-mismatched) never panic — out-of-range pixels read as opaque black.
/// <https://www.w3.org/Graphics/JPEG/itu-t81.pdf>
...
/// 🌀 Separable Gaussian blur with kernel radius `ceil(3 sigma)` and half-sample mirror padding (which preserves the image mean exactly for symmetric kernels); `sigma <= 0` returns the input unchanged.
...
/// 📦 Box blur over a `(2 radius + 1)²` window via an integral image; the window is clamped at the borders and the average divides by the number of covered pixels.
...
/// ➕ Summed-area table over pixel values and squared values, with a zero top row and left column: `sum`/`sq_sum` have `(width + 1) * (height + 1)` entries and entry `(x, y)` holds the sum over pixels `[0, x) × [0, y)`.
...
impl IntegralImage {
    /// ➕ Builds the summed-area tables in one row-prefix pass.
    ...
    /// ➕ Sum of pixel values over the inclusive-exclusive rectangle `x in [x0, x1), y in [y0, y1)`.
    ...
    /// ➕ Sum of squared pixel values over the inclusive-exclusive rectangle `x in [x0, x1), y in [y0, y1)`.
...
/// 🔻 Half-resolution downsample by 2×2 averaging; odd dimensions round up (`div_ceil`) and edge blocks clamp to the last row/column.
...
// #region 🔖Gradient
/// 🧭 Per-pixel image gradients, row-major with `gx`/`gy` of length `width * height`.
#[derive(Clone, Debug, PartialEq)]
...
/// 🧭 3×3 Scharr gradients with mirror padding, normalized by 1/32 so a unit-slope ramp yields a gradient of exactly 1 per pixel.
...
/// 🧭 Per-pixel gradient magnitude `√(gx² + gy²)` and orientation `atan2(gy, gx)` in radians, both row-major.
...
// #region 🔖Pyramid
/// 🗻 Coarse-to-fine image pyramid; `levels[0]` is the original and each next level halves the resolution (`scale = 0.5`).
#[derive(Clone, Debug, PartialEq)]
pub struct Pyramid {
...
/// 🗻 Builds up to `n_levels` pyramid levels: each level is the previous one Gaussian-blurred with `sigma = 1` then 2×2-downsampled; stops early once a level reaches 1×1.
...
// #region 🔖Patch
/// 🧩 Square intensity patch of side `2 radius + 1`, row-major in `data`.
#[derive(Clone, Debug, PartialEq)]
pub struct Patch {
    pub radius: u32,
    pub data: Vec<f32>,
...
/// 🧩 Bilinearly samples a square patch centered at `(cx, cy)`, with the sampling grid rotated by `rotation` radians (counter-clockwise offset `(dx, dy)` maps to `(cx + dx cos − dy sin, cy + dx sin + dy cos)`).
...
/// 🎯 Zero-mean normalized cross correlation in `[-1, 1]`; returns `0` for mismatched sizes or degenerate (near-zero) variance.
...
/// 📏 Sum of squared differences between two patches (mismatched sizes compare only the overlapping prefix).
pub fn ssd(a: &Patch, b: &Patch) -> f32 {
    a.data.iter().zip(b.data.iter()).map(|(&pa, &pb)| (pa - pb) * (pa - pb)).sum()
}

/// 🧮 Census transform per pixel: bit `(dy + r)(2r + 1) + (dx + r)` is set when the neighbor at offset `(dx, dy)` is strictly darker than the center. `radius` is clamped to 3 (7×7 window, 49 bits) to fit `u64`; pixels whose window leaves the image get code `0`.
...
/// 🧮 Hamming distance between two census codes.
...
// #region 🔖Warp
/// 🔀 Affine warp by inverse mapping: `m` maps OUTPUT pixel coordinates to SOURCE coordinates (`sx = m[0]·[x, y, 1]`, `sy = m[1]·[x, y, 1]`), sampled bilinearly with clamped borders.
...
/// 🔀 Homography warp by inverse mapping with perspective divide: `h` maps OUTPUT pixel coordinates to SOURCE coordinates in homogeneous form; outputs with a near-zero denominator are set to `0`.
...
/// 🔀 Generic remap: output pixel `i` (row-major over `out_w × out_h`) samples the source bilinearly at `(map_x[i], map_y[i])`.
...
// #region 🔖Nms
/// 📌 Strict local maxima of a row-major score map: keeps pixels strictly greater than every other score in their `(2 radius + 1)²` window (clamped at borders) and strictly above `threshold`, returned as `(x, y, score)` sorted by descending score, then row-major position, for determinism.
