//! 🏃 Videogrammetry: multi-frame tracking, 3D trajectories, kinematics, vibration and modal analysis, camera sync, stabilization and non-rigid capture.
...
// #endregion 🔖Vec3Helpers

// #region 🔖Track2d
/// 🧵 One 2D feature's optical-flow trajectory across frames: track identity plus `(frame_idx, x, y)`
...
/// 🎥 Stateful multi-frame 2D point tracker: advances every live [`Track2d`] one frame at a time via
...
impl Tracker2d {
    /// 🆕 An empty tracker with no live or historical tracks.
    pub fn new() -> Self {
        Self { tracks: Vec::new(), live: Vec::new(), next_id: 0 }
    }

    /// 👣 Advances every live track from `pyr_prev` into `pyr_curr`, drops points that fail forward-backward
    ...
    /// 📜 Every track this tracker has ever produced (live or dropped), in creation order.
...
// #region 🔖Mot
/// 📍 One raw 2D detection to associate against live tracks, with an optional external id hint (unused by
...
/// 🚦 One multi-object track's constant-velocity state.
#[derive(Clone, Copy, Debug, PartialEq)]
...
impl MultiObjectTracker {
    /// 🎬 An empty tracker with no live tracks.
    pub fn new() -> Self {
        Self { tracks: Vec::new(), next_id: 0 }
    }

    /// 🔮 Constant-velocity prediction step: advances every live track's `(x, y)` by `(vx, vy) * dt`.
    ...
    /// 🔄 Associates `detections` against live tracks: builds a Euclidean-distance cost matrix, gates out
...
/// 📐 Triangulates every track into a moving 3D trajectory from a **single moving camera** whose per-frame
...
/// 📈 Shared Savitzky-Golay derivative kernel for [`velocity`]/[`acceleration`]: assumes roughly-uniform
...
/// ⚡ Per-sample velocity of a [`Trajectory3d`] via first-order Savitzky-Golay differentiation, assuming
/// roughly-uniform timestamp spacing (see [`sg_derivative`]).
pub fn velocity(traj: &Trajectory3d) -> Vec<[f64; 3]> {
    sg_derivative(traj, 1)
}

/// 🚀 Per-sample acceleration of a [`Trajectory3d`] via second-order Savitzky-Golay differentiation,
...
/// 📏 Polyline arc length: the sum of Euclidean distances between consecutive samples.
pub fn arc_length(traj: &Trajectory3d) -> f64 {
    traj.samples.windows(2).map(|w| dist3(w[0].1, w[1].1)).sum()
}

/// 🪢 One neighbourhood edge's Green-Lagrange strain magnitude from [`neighborhood_affine_strain`].
#[derive(Clone, Copy, Debug, PartialEq)]
...
/// 🧩 Solves the best-fit local linear map `F` (`d1_k ≈ F d0_k`) for a neighbourhood's offset vectors via
...
/// 🧵 Local (neighbourhood-level) affine deformation-gradient strain: for every point `i` in `cloud_t0`
...
// #region 🔖Modal
/// 🎼 Extracted vibration modes from [`modal_analysis`]: parallel arrays indexed by mode, plus mode shapes
...
/// 🎵 Per-track Welch-PSD modal analysis: extracts each track's y-displacement signal, computes
...
/// 🔗 Coarse sync-offset estimate between two per-frame motion-activity signals (e.g. mean optical-flow
...
/// 🔬 Sub-frame polish of a coarse sync offset: a local golden-section search
...
/// 🌀 Simplified 2D-only rolling-shutter velocity estimate: a real 3D SE(3) tangent is not observable from
...
// #endregion 🔖RollingShutterComp

// #region 🔖Stabilize
const STABILIZE_SG_ORDER: usize = 2;

/// 🧘 Savitzky-Golay smoothing of a camera pose sequence in the SE(3) Lie algebra: consecutive relative
...
/// 🪟 Per-frame pure-rotation stabilization homography `H = K · ΔR · K⁻¹` warping the original jittery
...
/// 💨 Simple linear motion-blur kernel: a line segment of length `speed * exposure_fraction` (clamped to at
...
/// 🔍 Frequency-domain Wiener deconvolution against a known [`Psf`]: `G = conj(H) / (|H|² + 1/snr)` applied
...
/// 🧩 Fits each node's local rigid [`Se3`] transform independently via Levenberg-Marquardt over its nearby
...
/// 🔮 Embedded-deformation query: blends every graph node within `radius` of `point` by inverse distance,
...
/// 📦 Per-frame rigid-body 6DoF pose from partially-visible model-point observations: for each frame, gathers
...
mod tests {
    ...
    // #endregion 🔖Trajectory3dTests

    // #region 🔖KinematicsTests
    #[test]
    ...
    // #region 🔖RollingShutterCompTests
    #[test]
    ...
    // #endregion 🔖RollingShutterCompTests

    // #region 🔖StabilizeTests
    #[test]
    ...
    // #endregion 🔖DeblurTests

    // #region 🔖NonRigidTests
    #[test]
