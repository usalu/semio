...
app_labels! {
    ...
    struct RemodelLabels {
        ...
        no_source_video: &'static str = en: "No source video imported yet", de: "Noch kein Quellvideo importiert";
        ...
        mesh_none: &'static str = en: "none", de: "keine";
...
mod tests {
    ...
    fn view_actions_mutate_runtime_without_emitting_ops() {
        let mut app = testkit::new_app::<RemodelPlayApp>();
        app.handle_action("setCamera", Some(&json!({ "camera": { "position": [1.0, 2.0, 3.0], "target": [0.0, 0.0, 0.0], "fov": 60.0 } })), &ViewState::default(), &testkit::meta("local")).expect("set camera");
    ...
    /// `testkit::assert_two_instances_converge` pairs the instances, applies each side's edit, pumps
    /// both with a neutral `commitCheckpoint` (RemodelOp does not override `Operation::author_id`, so a
    /// history-neutral action is needed to drain inbound ops without touching `applied_edit_ids`), then
    /// asserts the probe agrees on both sides.
    #[test]
    fn two_instances_converge_disjoint_edits_via_backbone() {
        testkit::assert_two_instances_converge::<RemodelPlayApp, _>(
            "mem://remodel-convergence",
            ("importVideo", Some(&json!({ "video": { "assetId": "vid-1", "filename": "scan.mp4", "frameCount": 120, "fps": 30.0 } }))),
            ("clearResult", None),
            |app| {
                let projection = app.projection().expect("materialize projection");
                (projection.source_video.map(|video| video.filename), projection.result.is_none())
            },
        );
    }
}
