...
//! hot-swappable WASM component. Wires `remodel_document`'s field-granular scene schema to
//! `remodel_engine`'s cooperative staged reconstruction pipeline via the framework's `DispatchAction`
//! re-dispatch loop: `runReconstruction` ingests already-imported media into a fresh
//! `ReconstructionEngine` and schedules the first `advanceReconstruction` tick; each tick advances a
//! bounded budget of work and reschedules itself until `Done`/`Failed`/cancelled, coalescing the whole
//! run into one undo step via a shared `coalesce_key` (every tick — including the terminal one — uses
//! `ActionEmit::amend` with the same key; see `🔖StagedReconstruction` for why `ActionEmit::commit` on
//! the final tick would NOT satisfy the one-undo-step contract).
...
use semio_framework_plugin::{
    ...
    HostEffect, MeshData, MediaClass, MediaForm, MediaType, OsMediaCapability, OsMediaFormat, PanelGroup, ResourceKindSpec, SurfaceKind, TableScene, UiNode, UtilityCategory, UtilityDefinition, ViewState, WorldSunConfig,
...
/// 📥 The drop zone's accepted extensions: still-image formats plus every container `remodel_video`
...
//#region 🔖Runtime
/// 🎥 Ephemeral viewport orbit camera — never persisted, mirrors `world3d_camera_json`'s shape.
...
/// 📥 Rolling blur-gate scratch for one in-progress `importVideo`/`RequestMediaFrames` batch — mirrors
...
/// 🧭 Gradient-energy sharpness proxy — a local mirror of `remodel_engine`'s private `sharpness_score`
...
/// 🚦 Whether the sample should be rejected by the relative blur gate, given `scratch`'s rolling window
...
struct RemodelPlayRuntime {
    ...
    /// 📥 Which `MediaStream` the current `importFrames`/`importVideo` file-picker batch is appending
...
//#region 🔖ArgHelpers
/// 🔢 Small accessors over a `handle_action` `args` object — every action handler below reads a
...
/// 📊 `remodel-report`'s runtime-selected `(columns_json, rows_json)` for one dataset name.
...
/// 📦 Decodes a `requestFileOpen(readAs: "dataUrl")`/`RequestMediaFrames` payload into `(mime, bytes)`.
...
fn build_engine_params(params: &ReconstructionParams) -> remodel_engine::EngineParams {
...
fn map_engine_stage(stage: remodel_engine::EngineStage) -> ReconstructionStage {
    match stage {
...
fn stage_display(stage: ReconstructionStage) -> &'static str {
    match stage {
...
fn camera_pose_preview(index: u32, pose: &remodel_camera::CameraPose) -> CameraPosePreview {
...
fn watertight_snapshot(report: &remodel_mesh::WatertightReport) -> WatertightReportSnapshot {
...
/// a documented gap.
fn build_qc_snapshot(quality: &remodel_geo::QualityReport, registered_count: usize, accepted_count: usize, gcp_count: usize) -> QcReportSnapshot {
...
fn raster_to_png_asset(raster: &remodel_geo::Raster) -> ImageAsset {
...
fn video_codec_to_document(codec: remodel_video::VideoCodec) -> DocumentVideoCodec {
    match codec {
...
fn video_codec_from_label(label: &str) -> DocumentVideoCodec {
...
fn describe_video_probe(probe: &remodel_video::VideoProbe) -> (remodel_video::VideoCodec, u32, u32, f64, &'static str) {
    match probe {
...
struct PlyExporter;
impl semio_framework_plugin::MeshExporter for PlyExporter {
...
struct LasExporter;
impl semio_framework_plugin::MeshExporter for LasExporter {
    fn format(&self) -> OsMediaFormat {
        OsMediaFormat::Las
    }
    fn export(&self, mesh: &MeshData) -> Result<Vec<u8>, String> {
...
fn remodel_png_export(doc: &Value) -> Result<semio_framework_os::OsMediaExportResult, String> {
    let scene: RemodelScene = serde_json::from_value(doc.clone()).map_err(|error| error.to_string())?;
    let asset_id = scene
        .results
        .geo
...
/// 🚦 `remodel.pipeline` — job status/progress plus live viewport session state.
fn build_pipeline_panel(scene: &RemodelScene, runtime: &RemodelPlayRuntime, active_utility: &str, labels: &RemodelLabels) -> UiNode {
    let job = &scene.job;
    let job_label = format!("{}: {} ({:.0}%){}", labels.reconstruction, stage_display(job.stage), job.progress_0_1 * 100.0, job.error.as_ref().map(|error| format!(" - {}: {error}", labels.error)).unwrap_or_default());
...
/// 🧵 `remodel.results` — the products a run (partially) produced.
...
/// 🎯 `remodel.calibration` — per-camera calibration, rig extrinsics, ground control points.
...
/// 🏃 `remodel.tracks` — moving-object motion tracks. `remodel_engine` does not yet drive
...
/// ✅ `remodel.qc` — the whole-run quality report, including the watertight sub-report.
fn build_qc_panel(scene: &RemodelScene, labels: &RemodelLabels) -> UiNode {
    ...
    for warning in &qc.warnings {
        lines.push(ui_text(format!("⚠ {warning}")));
    }
    ui_stack_vertical(lines)
}
//#endregion 🔖PanelBuilders
...
impl RemodelPlayApp {
    ...
    fn handle_import_video_done(&mut self, args: Option<&Value>, doc: &DocumentView<'_, RemodelScene>) -> ActionEmit<RemodelOperation> {
        ...
        let codec = video_codec_from_label(arg_str(args, "codec").unwrap_or("unknown"));
    ...
    //#region 🔖StagedReconstruction
    /// 🚀 Validates ≥2 accepted frames, builds an engine from the current document params, pushes every
    ...
    fn handle_run_reconstruction(&mut self, doc: &DocumentView<'_, RemodelScene>) -> ActionEmit<RemodelOperation> {
        ...
        let engine_params = build_engine_params(&doc.projection.params);
    ...
    fn handle_advance_reconstruction(&mut self, doc: &DocumentView<'_, RemodelScene>) -> ActionEmit<RemodelOperation> {
        ...
        match engine.advance(RECONSTRUCTION_STEP_BUDGET) {
            remodel_engine::EngineStatus::Working { stage, progress } => {
                ...
                let mut job = doc.projection.job.clone();
                job.stage = map_engine_stage(stage);
...
impl DocumentApp for RemodelPlayApp {
    ...
    fn handle_action(&mut self, action: &str, args: Option<&Value>, doc: &DocumentView<'_, RemodelScene>, _view_state: &ViewState) -> ActionEmit<RemodelOperation> {
        match action {
            ...
            // (no calibration target detection is wired into this plugin).
            "calibrateCameras" => {
                let mut calibration = doc.projection.calibration.clone();
                for stream in &doc.projection.streams {
...
fn create_remodel_app() -> App {
    ...
    App::from_builder(
        App::builder(REMODEL_PLAY_APP_ID, "Remodel")
            ...
            // 🚀 Staged reconstruction: `runReconstruction` starts the run, `advanceReconstruction` is the
...
mod tests {
    use super::*;
    use semio_framework_plugin::{testkit, MeshExporter, PluginApp};

    //#region 🔖Fixtures
    /// 🏁 High-contrast `cell`-pixel checkerboard PNG, base64-wrapped as a `requestFileOpen(readAs:
    ...
    /// 🏁 The same checkerboard, real-JPEG-encoded — mirrors what a `RequestMediaFrames` host actually
    ...
    /// 📥 Imports `n` checker frames as one new image-sequence stream via `importFramePayload`, mirroring
    ...
    /// 🔁 Drives `advanceReconstruction` until the job reaches a terminal stage (`Done`/`Failed`) or
    ...
    /// 🧬 Type alias so the fixtures above don't need to spell out `VcsDocumentApp<RemodelPlayApp>`.
    mod testkit_app {
    ...
    fn import_video_bytes_payload_extracts_frames_in_process() {
        let mut app = testkit::new_app::<RemodelPlayApp>();
        // 🎯 `IngestParams::default().frame_sample_stride == 5`; force stride 1 so all 5 synthesized
    ...
    //#region 🔖StagedReconstructionTests
    /// 🚀 The staged execution model, end-to-end: `runReconstruction` ingests two imported checker
    ...
    /// 🛑 Cancelling mid-run must fold into the SAME undo entry the run itself started (both amends
    ...
    /// 🔁 `retryStage` after a run reached a terminal stage starts a fresh run (a new job id, back at
    /// `Ingesting`).
    #[test]
    ...
    /// 📦 The whole run — start tick, every working tick, and the terminal tick — must collapse into
