//! 🏺 Remodel program — photogrammetry/videogrammetry play app (video → watertight mesh) bundled as a
//! hot-swappable WASM program. Wires `remodel_document`'s field-granular scene schema to
...
/// ✅ Maps `remodel_geo::QualityReport` (plus counts only the program can compute — accepted vs.
...
struct PlyExporter;
impl semio_framework_program::MeshExporter for PlyExporter {
...
struct LasExporter;
impl semio_framework_program::MeshExporter for LasExporter {
    fn format(&self) -> OsMediaFormat {
        OsMediaFormat::Las
    }
    fn export(&self, mesh: &MeshData) -> Result<Vec<u8>, String> {
...
fn register_remodel_exports() {
    ...
    semio_framework_program::program_runtime::register_document_codec_for_app::<RemodelPlayApp>(REMODEL_DOCUMENT_SCHEMA);
    semio_framework_os::register_mesh_exporter("3d.remodel", "remodel", remodel_mesh_from_document, Box::new(semio_framework_program::ObjExporter));
    semio_framework_os::register_mesh_exporter("3d.remodel", "remodel", remodel_mesh_from_document, Box::new(semio_framework_program::GlbExporter));
    semio_framework_os::register_mesh_exporter("3d.remodel", "remodel", remodel_mesh_from_document, Box::new(semio_framework_program::StlExporter));
...
impl DocumentApp for RemodelPlayApp {
    ...
    /// `lowpoly-program`'s `world3d_sun_measures`/`window_measures` for the identical pattern this mirrors).
    fn window_measures(&self, _doc: &DocumentView<'_, RemodelScene>, view_state: &ViewState) -> std::collections::HashMap<String, Vec<semio_framework_program::WindowMeasure>> {
...
fn remodel_layer_measures(layers: &RemodelLayerVisibility, labels: &RemodelLabels) -> semio_framework_program::WindowMeasure {
    let toggle = |id: &str, icon: &str, label: &'static str, pressed: bool, layer: &str| semio_framework_program::WindowMeasure::Toggle {
    ...
    semio_framework_program::WindowMeasure::Group {
...
mod tests {
    use super::*;
    use semio_framework_program::{testkit, MeshExporter, ProgramApp};
    ...
    mod testkit_app {
        pub type App = semio_framework_program::VcsDocumentApp<super::RemodelPlayApp>;
    }
    //#endregion 🔖Fixtures
    ...
    fn render_does_not_panic_for_known_body_keys() {
        ...
        let doc = DocumentView { projection: &store_projection, history: &semio_framework_program::HistoryView { columns: Vec::new(), can_undo: false, can_redo: false, active_alternative_id: None, current_checkpoint_id: None, recent_ops: Vec::new() } };
