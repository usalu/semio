//! 📐 Layout program — blueprint/preview document editor bundled as a hot-swappable WASM program.
...
/// 🌉 JSON bridge for `semio_framework_program`'s `App::example`, which hardcodes `serde_json::from_str`
...
/// persistent state.
///
/// 🚧 Deliberately unwired beyond `LayoutPlayApp::gesture_preview` below — same gap as `draw-program`'s
...
impl LayoutPlayApp {
    ...
    /// sandboxed program. `#[allow(dead_code)]`: no caller exists inside this crate today; exercised by
...
fn register_layout_exports() {
    ...
    semio_framework_program::program_runtime::register_document_codec_for_app::<LayoutPlayApp>(LAYOUT_FIXTURE_SCHEMA);
