...
3. Repo binary (`./repo/cli/main.go:5787-5788`) skips directories starting with `.` and uses .gitignore patterns
4. Added `isInIgnoredDirectory()` function that:
   - Checks against explicit ignore list (node_modules, **pycache**, site-packages, etc.)
   - Skips any directory starting with `.` unless explicitly allowed (.github, .devcontainer, .repo)
