...
type SyncState = {
  ...
  /** 🚦️ Per-document sync health fed by `backbone-worker.ts`'s `DocumentEvent::Status` events, keyed by `documentId`. */
...
/** @emoji 🧭️ Parses `/studios/:id` and `/studios/:id/instances/:iid` shell paths; null for any other route (e.g. home). */
...
/** @emoji 🧭️ Default anchor a plugin-declared panel-tab `group` docks into — groups only ever map to the four corners; the two middle anchors start empty and are user-populated via drag-and-drop or a dock skeleton override. */
...
/** 🧰️ One `UtilityDefinition` → the lean `DerivedUtilitySpec` consumed by {@link deriveUtilityNodes}, resolving the label through the app's locale/terminology overlay. */
...
/** 🧰️ Stamps the owning `windowId` onto every `setActiveUtility` descriptor in a derived toolbar tree so the shell's `onAction` interceptor targets the right window regardless of which window is globally active. */
...
/**
 * 🧰️ Builds the window toolbar `UtilityNode[]` for one window purely from the static utility registry plus
...
/** @emoji 💬️ Builds spawned-window engagement, measures, and utility-options chrome for one window kind. */
...
/** @emoji 🌐️ Resolves a chrome translation key outside hook context (tree builders run there). */
...
/**
 * 🎛️ The per-window Actions rail body (P1/P2). Zero-arg actions ARE the execute button; arg-carrying
...
/**
 * 🎛️ Os-level built-in commands — app introduction/theme/layout/locale/appearance/expertise,
 * `scope: "os"`, handled
...
/** @emoji 🐢️ Structural equality over plain JSON-shaped values (the shape every `UiNode`/`WindowEngagement`/`WindowMeasure` plugin payload takes) — no cycles, no non-JSON types. */
...
/**
 * @emoji 🐢️ Reuses `previous`'s object identity when it's structurally equal to `next` — every plugin
...
export function FrameworkOsShell({
  ...
  /** 🧵️ Lazily-created worker running `backbone-worker.ts` — one per shell instance, reused across `openDocument` calls. */
  ...
  // 🎓️ Auto-starts an app's introduction the first time it launches on this device; replaying stays
    ...
    async (effects: readonly HostEffect[], baseSession: ActiveSession, uiScope: UiDirtyScope = { kind: "full" }) => {
      ...
      for (const effect of effects) {
        ...
        if ("setActiveUtility" in effect) {
          // 🧰️ A plugin programmatically switched utility: mirror it into the host-owned store slice and,
  ...
  const onAction = useCallback(
    (action: ActionDescriptor) => {
      ...
      // 🧰️ Utility activation (P5): host-owned session state, never a document op. Re-clicking the active
  ...
  //#region 💾️ uiPrefs persistence (skips localStorage writes for any locked preference)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      // 🧰️ Escape deactivates the active window's active utility (P5) when nothing is being typed.
      if (event.key === "Escape") {
  ...
  //#region 🧰️FooterUtilityLeaves — bottom-right's History tab, now sourced from the framework-owned History
  ...
  /**
   * 🎛️ Dispatches a resolved command: os-scope commands are handled locally (no plugin round trip);
  ...
  /** @emoji 🎓️ The current introduction step's anchor, decomposed by kind — `null` unless that kind is
...
export type Board2dWasmSession = {
  ...
  /** @emoji 🐢️ Silent cross-pane mirror setters (WS-live-sync round 4) — move nodes/set preselect/set the marquee outline without emitting board events or a fixture reset, so a peer pane can mirror another pane's live gesture without round-tripping through the plugin. */
...
type UtilityTreeProps = {
  ...
  readonly onAction: (action: ActionDescriptor) => void;
  readonly id?: string;
  /** @emoji 🎀️ `up` stacks a new ribbon line above the base row per pressed collection (window toolbar); `inline` keeps the horizontal drill-down (footer). */
...
/** @emoji 🔢️ Sorts toolbar nodes by `order`. */
