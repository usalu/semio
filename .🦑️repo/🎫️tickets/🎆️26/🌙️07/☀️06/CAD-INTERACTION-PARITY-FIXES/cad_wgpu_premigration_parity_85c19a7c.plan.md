...
The `premigration` git tag (`f8376e8486`, `🐙️ueli🎆️26🌙️06☀️04🚩️125`) marks the commit right before CAD's renderer was rewritten. In the 12 commits since (`126`→`137`), the entire React/JS CAD renderer was deleted and replaced by a much smaller Rust/wgpu plugin:
...
- Open a ticket under goal `🎯️r2602🎯️runningsketchpad` (existing CAD tickets like `CAD-PLAY-PER-MODEL-INTERACTIONS`, `CAD-TRANSFORM-TOOL-GUMBALL` use this goal) before starting implementation.
