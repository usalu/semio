...
- `repo/server/coordinator/project.json`: `test` target command changed from `bunx vitest run` to `bun ./📜️script.ts test` (added `"dependsOn": []` to match sibling projects, e.g. `cad/core/project.json`).
