...
/// `.repo/🎫️/26/07/27/PACK-BINARY-DOCUMENT-LAYER-ACROSS-ALL-APPS/` for the full container-format
...
pub mod pack_rt {
    ...
    /// @emoji 🌱️ Encodes an arbitrary `serde_json::Value` as a complete pack file. Infallible for any
    ...
    /// @emoji 🌱️ Inverse of `encode_json_value`.
...
/// @emoji 📦️ Binary counterpart to `DocumentTextFiles`: `pack` is the encoded initial projection
...
/// @emoji 📤️ Pack counterpart of `print_document_text`: identical op-log body (`print_ops_log`), but
...
/// @emoji 📥️ Pack counterpart of `parse_document_text`: obtains the initial projection via
