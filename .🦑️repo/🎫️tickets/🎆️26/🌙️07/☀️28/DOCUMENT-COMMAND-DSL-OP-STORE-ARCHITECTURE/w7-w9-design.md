...
**Step 1 — `store/rs/lib.rs`, `🔖️Backbone` region (after `BackboneChannelPort`, ~:2104): add the
factory registry.**
...
**Step 4 — `store/sync/rs/lib.rs`: native factories (new `🔖️Factories` region after `🔖️Host`,
...
- `store/rs/project.json`: delete the `wasm` target; `store/rs/script.ts`: delete the wasm
  ...
  workspaces.
- `.vscode/launch.json`: remove/adjust `📦️build🗃️store` entry (currently
...
2. **New test-support assert** (`store/rs/lib.rs` `🔖️TestSupport`, after `assert_operation_round_trip`
   :2827-2839):
   ```rust
...
4. Update `conformance.md` legend: `Auth ✓️` = "OperationMeta.author_id stamped from
...
| B3 | gis/plugin, infinite/board/port/directed/dag/plugin (+rs), layout/plugin (+layout/rs), reasoning/mindmap/plugin (+rs), remodel/plugin (+remodel/rs), architect/plugin (+architect/program) | mostly ✓️-verification + law swaps |
...
1. `protocol/rs/lib.rs` `🔖️Reexports` (:10-29), add: from `protocol_core`: `REC_END, REC_DOC,
...
2. **Duplicated `//#region` markers**: delete the duplicate opening line of each doubled pair
   ...
   BlobStore) + one doubled `//#endregion 🧪️Tests`.
3. **Mismatched endregion**: fix the `🔖️ReconcileAlternative` / `🔖️ContentAddressedCheckpoint
