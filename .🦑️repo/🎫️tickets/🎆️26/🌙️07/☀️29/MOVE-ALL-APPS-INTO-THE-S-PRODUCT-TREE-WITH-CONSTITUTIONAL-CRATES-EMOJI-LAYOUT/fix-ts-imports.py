...
def multi_segment_match(rel_base, n_segments=2):
    ...
    Also tries the entry-renamed basename (index.ts -> 📦️.ts) since the original no longer exists."""
...
def resolves(path):
    ...
    candidates = [path, path + '.ts', path + '.tsx', path + '.js',
                  ...
                  path + '.wasm', path + '.css', path + '/📦️.ts', path + '/📦️.tsx']
...
def process(f):
    ...
    def repl(m):
        ...
        if resolves(resolved):
            return m.group(0)
        # Try same-directory entry-rename first (index.ts/tsx -> 📦️.ts/tsx), no translation needed.
        base = os.path.basename(rel_base)
        if base in ('index.ts', 'index.tsx'):
            renamed_rel = rel_base[: -len(base)] + ('📦️.tsx' if base.endswith('.tsx') else '📦️.ts')
