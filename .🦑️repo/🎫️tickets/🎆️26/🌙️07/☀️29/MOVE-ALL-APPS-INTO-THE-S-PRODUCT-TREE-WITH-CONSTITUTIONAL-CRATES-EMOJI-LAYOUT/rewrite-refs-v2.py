...
def do_root_cargo_members():
    ...
    for line in lines:
        ...
        if m and m.group(2).startswith(("🧰️/", "✏️/", "🌎️/", "♻️/")):
            old = m.group(2)
...
# resolve(X, "🧰️/...") or path.resolve(X, "✏️/...") style ABSOLUTE-from-repo-root
# string args used in vite/vitest configs and root script.ts / dev script.ts.
RESOLVE_ABS_RE = re.compile(r'"((?:🧰️|✏️|🌎️|♻️)/[^"]+)"')
...
def do_go_work():
    ...
    def repl(m):
        ...
        if not core.startswith(("🧰️/", "✏️/", "🌎️/", "♻️/")):
            return m.group(0)
        new_core = tp.translate(core)
        if new_core == core:
            return m.group(0)
    ...
    new_content = re.sub(r"(\./(?:🧰️|✏️|🌎️|♻️)/\S+)", repl, content)
...
def do_nx_json():
    f = "nx.json"
    process_repo_root_relative(f, re.compile(r'"(\./(?:🧰️|✏️|🌎️|♻️)/[^"]+)"'), group=1)
...
def do_dependency_cruiser():
    ...
    new_content = re.sub(r'"((?:🧰️|✏️|🌎️|♻️)/[^"]*)"', repl, content)
    if new_content != content:
        open(f, "w", encoding="utf-8").write(new_content)
        stats["files_changed"] += 1
