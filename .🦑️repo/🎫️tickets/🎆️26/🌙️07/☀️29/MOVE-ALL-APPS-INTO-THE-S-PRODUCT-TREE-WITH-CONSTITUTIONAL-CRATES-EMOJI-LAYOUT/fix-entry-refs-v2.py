#!/usr/bin/env python3
"""Fix references to bare 📦️.rs/📦️.ts/📦️.tsx (first-round entry names) now that
they've been physically renamed to 📦️lib.rs/📦️index.ts/📦️index.tsx. Runs AFTER the
physical rename, operating on the current (already-renamed-for-directories) tree.
Only rewrites a match when the OLD bare-📦️ form no longer exists but the NEW form
