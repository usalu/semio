...
- `.raster` DocumentDsl grammar (hand-rolled lexer: Word/Str/LBrace/RBrace/LBracket/RBracket/Eof),
  ...
  `raster/plugin/rs/lib.rs`, inserted inside `pub(crate) mod domain` right after `//#region 🔖️Operations`.
...
## Tests added (existing `//#region 🧪️Tests` → `mod tests`, new `//#region 🔖️DslAndOpText` subregion)
