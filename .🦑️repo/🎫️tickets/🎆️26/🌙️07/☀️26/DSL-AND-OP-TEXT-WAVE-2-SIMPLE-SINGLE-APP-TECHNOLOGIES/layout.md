...
Design mirrors draw_rs's 🔖️Dsl/🔖️OpText style (hand-rolled tokenizer, tuple/list/brace grammar,
...
- New `mod dsl` in layout/rs/lib.rs (after `mod operations`), containing:
  - 🔖️Dsl: lexer (LayoutTok/lex_layout_dsl), parser (LayoutDslParser + kv/opt helpers), value grammars
  ...
  - 🔖️OpText: parse_page_patch/print_page_patch, parse_frame_patch/print_frame_patch (sparse attr-loop),
  ...
  - #[cfg(test)] mod tests at the end of `mod dsl` (layout/rs/lib.rs has no unifying `//#region 🧪️Tests`
