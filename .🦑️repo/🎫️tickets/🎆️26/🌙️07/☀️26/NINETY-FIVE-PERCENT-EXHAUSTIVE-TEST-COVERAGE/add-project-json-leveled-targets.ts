#!/usr/bin/env bun
/** 📊️One-off codemod: appends `test-quick`/`test-long`/`test-exhaustive` nx targets (cloned from each
 * project.json's own `test` target, with the level word spliced into its `bun ./📜️script.ts test …` command)
 ...
 * whose `test.options.command` starts with the standard `bun ./📜️script.ts test` prefix; anything else (a raw
...
for (const path of files) {
  ...
  if (!command.startsWith("bun ./📜️script.ts test")) {
  ...
  const newBlocks = needed
    .map((level) => {
      const levelCommand = command === "bun ./📜️script.ts test" ? `bun ./📜️script.ts test ${level}` : command.replace("bun ./📜️script.ts test", `bun ./📜️script.ts test ${level}`);
