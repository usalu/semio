...
impl RationalBezier3 {
    ...
    pub fn elevate(&self) -> RationalBezier3 {
        ...
        for i in 0..=m {
            ...
            let left = if i > 0 { self.controls[i - 1].to_vec() * a } else { crate::brep::vec::Vec3::ZERO };
            let right = if i <= n { self.controls[i].to_vec() * b } else { crate::brep::vec::Vec3::ZERO };
