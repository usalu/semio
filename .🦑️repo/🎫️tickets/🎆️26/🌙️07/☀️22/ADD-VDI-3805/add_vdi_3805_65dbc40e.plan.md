...
5. **Launch:** extend `🧪️test📏️norm` in [`.vscode/launch.json`](.vscode/launch.json) (and `.claude/launch.json` if it mirrors) with `-p norm_vdi_3805`.
6. **Docs:** update [`norm/AGENTS.md`](norm/AGENTS.md) with the VDI family path.
7. **Ticket:** `ticket_open` under goal `NORM` (title **Add VDI 3805**, emoji `📏️`); temps/checklists only under `.repo/🎫️/.../`. Bind plan id on open for archival.
...
Then full family pack via `🧪️test📏️norm`.
