...
2. **Missing expand UX** — Premigration collapsed to the top match and revealed **every** match when the ▾️ chevron (or ArrowDown) expanded the list with a capped scroll container (`flowSpotlightSuggestionListScrollClass`). Current `FlowSpotlight` always renders the truncated list with no expand control.
...
- Goal: `🎯️r2602/🎯️runningsketchpad` (same as prior spotlight ticket)
- Reopen [`.repo/🎫️/26/07/22/FLOW-DOUBLE-CLICK-CATALOGUE-SPOTLIGHT`](.repo/🎫️/26/07/22/FLOW-DOUBLE-CLICK-CATALOGUE-SPOTLIGHT) and bind this plan; put logs/notes in that ticket folder
...
- Header row: search input + ▾️/▴️ button (`aria-label` Show all / Collapse) when `hasMore`
...
- Click ▾️ (or ArrowDown) → full catalogue list, scrollable, every node reachable
