...
pub enum PersistenceBinding {
    ...
    /// @emoji ☁️ A semio_hub node reachable over WebSocket (`remote://host:port` → `ws://host:port/documents/{id}/ws`).
    Hub {
        base_url: String,
...
pub struct DocumentActorConfig {
    ...
    /// @emoji 🖋️ The authoring actor id used for semio_hub `Hello`/presence and op origin filtering.
    pub actor: String,
}
...
pub enum DocumentActorMsg {
    ...
    /// @emoji 📸️ Same as {@link LocalOps} for a full-envelope snapshot (structural commands / seeding).
...
pub enum DocumentEvent {
    /// @emoji 🕸️ Remote ops (semio_hub fan-out or appended external edits) — also pushed into the store's
    /// inbound queue so `store.tick()` materializes them.
    RemoteOps { envelopes: Vec<OpEnvelope> },
    /// @emoji 📸️ The whole envelope was replaced (divergent external history / semio_hub snapshot swap).
    ...
    /// @emoji ⚠️ A structural conflict (external divergence with local pending ops / semio_hub CAS reject).
    Conflict(StudioConflict),
}
//#endregion 🔖️Protocol

//#region 🔖️Endpoints
/// @emoji 🧱️ Core wire types used only when reconstructing an {@link OpEnvelope} from a stored edit,
...
/// @emoji 📇️ Edit ids present in an envelope's `vcs.edits` array (schema-agnostic JSON read).
#[cfg(not(target_arch = "wasm32"))]
fn envelope_edit_ids(value: Option<&Value>) -> Vec<String> {
    value
        ...
        .map(|edits| {
            edits
                .iter()
                .filter_map(|edit| edit.get("id").and_then(|id| id.as_str()).map(String::from))
                .collect()
        })
        .unwrap_or_default()
}

/// @emoji 📜️ The `vcs.edits` entries of an envelope, as raw JSON values.
#[cfg(not(target_arch = "wasm32"))]
...
/// @emoji 📦️ Rebuilds an {@link OpEnvelope} from a stored `Edit` JSON so an appended external edit can
    ...
    OpEnvelope {
        ...
        inverse: InverseOperation {
            ...
            inverse_diff: DocumentDiff {
                schema_id: SchemaId(schema.to_string()),
                payload: serde_json::json!({ "backwards": backwards }),
            },
    ...
    Op: Clone + serde::Serialize + serde::de::DeserializeOwned + Operation<P>,
{
    pub fn new(store: DocumentStore<P, Op>) -> Self {
        Self {
            ...
            cmd_tx: None,
            events: None,
            status: DocumentSyncStatus::default(),
        }
    }
    ...
    pub fn attach(
        ...
        self.store
            .attach_backbone(Box::new(channels.channel_backbone))
...
mod native_actor {
    ...
    /// @emoji 📁️ A folder/file binding's storage driver, keyed for multi-document sqlite or single blob.
    enum FolderEndpoint {
        Sqlite {
    ...
    impl FolderEndpoint {
        fn read(&self) -> Option<String> {
            match self {
                FolderEndpoint::Sqlite { storage, document_id, .. } => {
                    storage.read(document_id).ok().flatten()
                }
    ...
    impl DocumentActor {
        ...
        /// @emoji 🌱️ Seeds persistence state from any already-stored envelope and installs the file watcher.
        fn setup(&mut self) {
        ...
        /// @emoji 📸️ Records a full-envelope snapshot as the canonical persisted state.
        ...
        /// @emoji ➕️ Appends locally-applied ops to the persisted envelope's `vcs.edits` (append-only),
        ...
        fn handle_external_change(&mut self) {
            ...
            if lost.is_empty() && !new_ids.is_empty() {
                let appended: Vec<OpEnvelope> = envelope_edits(&value)
                    .into_iter()
                    .filter(|edit| {
                        edit.get("id")
                    ...
                    .map(|edit| op_envelope_from_stored_edit(&self.schema, &self.document_id, edit))
                    .collect();
                ...
                if !self.pending_hub.is_empty() {
                    self.emit(DocumentEvent::Conflict(StudioConflict {
                        kind: "externalDivergence".into(),
                        uri: format!("folder://{}", self.document_id),
                        message: "external history diverged while local ops are pending".into(),
                    }));
                } else {
        ...
        async fn try_connect_hub(&mut self) {
            ...
            match tokio_tungstenite::connect_async(url).await {
                Ok((stream, _response)) => {
                    ...
                    self.backoff_ms = 500;
                    let hello = HubClientFrame::Hello {
                        actor: self.actor.clone(),
                        token,
                        since_version: self.hub_version,
                    };
        ...
        fn on_hub_frame(&mut self, frame: HubServerFrame) {
            match frame {
                ...
                HubServerFrame::Conflict { message } => {
                    self.emit(DocumentEvent::Conflict(StudioConflict {
                        kind: "hubCas".into(),
                        uri: self.hub_base_url.clone().unwrap_or_default(),
                        message,
                    }));
                }
        ...
        /// @emoji 📸️ Pushes a full-envelope snapshot into the store's inbound queue and notifies subscribers.
        ...
        fn status(&self) -> DocumentSyncStatus {
            DocumentSyncStatus {
                persisted: self.last_written_hash.is_some() || self.hub_version > 0,
                pending_ops: self.pending_hub.len(),
                remote: self.remote_state.clone(),
            }
        }
    ...
    /// @emoji 📍️ The on-disk path a folder binding writes to: the `*.json` blob itself, or the
...
mod wasm_actor {
    ...
    impl WasmActor {
        ...
        fn drain_and_relay(&mut self) -> bool {
            ...
            for message in messages {
                match message {
                    ...
                    BackboneMessage::Snapshot { envelope_json } => {
                        if let Ok(envelope) = serde_json::from_str::<Value>(&envelope_json) {
                            self.send_frame(&HubClientFrame::PutEnvelope {
        ...
        fn on_text(&mut self, text: &str) {
            ...
            match frame {
                ...
                HubServerFrame::Conflict { message } => {
                    let _ = self.events.send(DocumentEvent::Conflict(StudioConflict {
                        kind: "hubCas".into(),
                        uri: self.hub_base_url.clone().unwrap_or_default(),
                        message,
                    }));
    ...
    pub(super) fn spawn_actor(
        config: DocumentActorConfig,
        remote: ChannelBackboneRemote,
        mut cmd_rx: mpsc::UnboundedReceiver<DocumentActorMsg>,
        events: broadcast::Sender<DocumentEvent>,
    ) {
...
pub enum FixtureInbound {
    /// @emoji 📬️ A raw `HubServerFrame` delivered as if received over the semio_hub WebSocket. Driven by
    /// WS-E's TS twin; the folder-only Rust harness skips these.
    HubFrame { frame: HubServerFrame },
    /// @emoji 📁️ An external folder edit: append these edit JSON objects to `vcs.edits` out-of-band.
...
/// @emoji 📂️ Loads every `*.json` fixture from `framework/sync/fixtures/`.
#[cfg(not(target_arch = "wasm32"))]
...
mod tests {
    ...
    fn hub_ws_url_derives_ws_endpoint_from_remote_uri() {
        ...
        assert_eq!(
            hub_ws_url("https://semio_hub.example.com", "doc-2"),
            "wss://semio_hub.example.com/documents/doc-2/ws"
        );
        assert_eq!(
    ...
    fn op_envelope_from_stored_edit_round_trips_through_ingest() {
        ...
        let recovered: Edit<DemoOp> =
            serde_json::from_value(envelope.diff.payload.clone()).expect("recover edit");
    ...
    mod actor_tests {
        ...
        async fn wait_for_event(
            events: &mut broadcast::Receiver<DocumentEvent>,
            mut predicate: impl FnMut(&DocumentEvent) -> bool,
        ) -> DocumentEvent {
        ...
        // 🔬️ External folder edit → RemoteOps event + the store timeline grows on tick().
        #[tokio::test]
        async fn folder_external_edit_delivers_remote_ops() {
            ...
            let channels = host.open(DocumentActorConfig {
                ...
                schema: "demo/v1".into(),
                bindings: vec![PersistenceBinding::Folder { path: dir.path().to_path_buf() }],
                watch_external: true,
                actor: "local".into(),
            });
            ...
            storage
                .write("doc-a", "demo/v1", &serde_json::to_string(&value).unwrap())
        ...
        //#region 🔖️MockHub
        /// @emoji 🧪️ A minimal in-process semio_hub speaking the real `HubClientFrame`/`HubServerFrame`
        /// protocol, so the semio_hub endpoint is exercised end-to-end without linking WS-C's `os-semio_hub` bin.
        struct MockHub {
        ...
        async fn mock_hub_connection(
            ws: tokio_tungstenite::WebSocketStream<tokio::net::TcpStream>,
            semio_hub: Arc<MockHub>,
        ) {
            ...
            let (version, backlog) = {
                ...
                let backlog: Vec<OpEnvelope> = log
                    .iter()
                                ...
                                match serde_json::from_str::<HubClientFrame>(text.as_str()) {
                                    Ok(HubClientFrame::Ops { envelopes }) => {
                                        for envelope in envelopes {
                                            ...
                                            let _ = semio_hub.broadcast.send(HubServerFrame::Ops {
                                                version: assigned,
        ...
        // 🔬️ Two DocumentHosts converge through a semio_hub: A's op fans out to B, whose store materializes it.
        #[tokio::test]
        ...
        // 🔬️ Reconnect with `since` catch-up: after A appends ops while B is offline, B reconnects and
        ...
        // 🔬️ Detach drains the outbox: an op applied right before close still reaches the semio_hub (and B).
        #[tokio::test]
        ...
        async fn replay_fixture(fixture: &ActorFixture) {
            ...
            assert_eq!(
                fixture.inbound.len(),
                fixture.expected_events.len(),
                "fixture {} must pair each inbound stimulus with one expected event",
                fixture.name
            );
            ...
            for (inbound, expected) in fixture.inbound.iter().zip(fixture.expected_events.iter()) {
                match inbound {
                    FixtureInbound::ExternalEdits { edits } => {
                        ...
                        storage
                            .write(&fixture.document_id, &fixture.schema, &serde_json::to_string(&value).unwrap())
                            .expect("write");
                        channels.cmd_tx.send(DocumentActorMsg::ExternalChanged).expect("poke");
                    }
                    FixtureInbound::ReplaceEnvelope { envelope } => {
                        storage
                            .write(&fixture.document_id, &fixture.schema, &serde_json::to_string(envelope).unwrap())
            ...
            for expected_id in &fixture.expected_edit_ids {
                assert!(
                    timeline_ids.contains(expected_id),
                    "fixture {} expected edit id {expected_id} in timeline {timeline_ids:?}",
                    fixture.name
                );
            }
