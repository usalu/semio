#!/usr/bin/env bun
/** @emoji 🛝️ Merge core/playground.ts into index.ts via createPlaygroundApp and delete playground.ts. */
...
function mergeOne(coreDir: string): void {
  ...
  const helperSection = playground
    .replace(/\/\/ #region 🧲️Header[\s\S]*?\/\/ #endregion 🧲️Header\n\n?/g, "")
    ...
    .replace(/\/\/#region 🔖️PlaygroundAppDefinition[\s\S]*$/g, "")
    .replace(/\/\/ #region 🧪️Tests[\s\S]*/g, "")
    .trim();

  const vitestSection = playground.match(/\/\/ #region 🧪️Tests[\s\S]*/)?.[0] ?? "";

  const playRegion = `//#region 🔖️Play
