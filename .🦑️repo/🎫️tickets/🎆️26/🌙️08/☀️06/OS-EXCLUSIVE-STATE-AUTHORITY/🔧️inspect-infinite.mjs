...
for (const f of files) {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes("crate::infinite") && n < 35) {
...
console.log("\n--- board component first 40 ---");
const board = path.join(root, "🎲️board/🦀️component.rs");
console.log(fs.readFileSync(board, "utf8").split("\n").slice(0, 40).join("\n"));

console.log("\n--- dag crate mid-path ---");
const dag = path.join(root, "🎲️board/🔌️ports/➡️directed/🕸️dag/🦀️component.rs");
const dagLines = fs.readFileSync(dag, "utf8").split("\n");
dagLines.forEach((l, i) => {
  if (/::crate::|crate::infinite|use crate::os_/.test(l) && i < 50) console.log(`${i + 1}: ${l}`);
});
// find crate mid-path patterns
dagLines.forEach((l, i) => {
  if (/\w::crate::/.test(l) || /board::crate::/.test(l)) console.log(`MID ${i + 1}: ${l}`);
});
