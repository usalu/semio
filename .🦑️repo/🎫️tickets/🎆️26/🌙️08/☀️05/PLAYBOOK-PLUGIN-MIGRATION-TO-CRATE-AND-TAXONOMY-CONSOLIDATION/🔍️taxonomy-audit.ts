#!/usr/bin/env bun
/** 🔍️ Standalone mirror of `📇️registry/📜️script.ts`'s `validateTaxonomyTree`, so the mathematical migration
 * can audit ➗️mathematical's tree while the real `check` still bails early on registry staleness
 * (regeneration is registrar work, not the migrating agent's). Delete once the registrar has regenerated
 * the registry and `bun …/📇️registry/📜️script.ts check` reports the taxonomy audit itself. */
...
const pluginId = process.argv[3] ?? "➗️mathematical";
