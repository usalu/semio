#!/usr/bin/env bun
/** 🗿️ Local mirror of the registry script's `validateTaxonomyTree` for ONE plugin — lets this ticket
 * self-verify the taxonomy shape without running the real `registry check`, which bails on staleness
 * until the registrar regenerates. Kept in the ticket folder, never in the plugin tree. */
...
const pluginRoot = join(repoRoot, "✏️s", "🔌️plugins", "🌍️gis");
const findings = validateTaxonomyTree(pluginRoot, "🌍️gis");
if (findings.length === 0) console.log("✅ taxonomy audit clean for 🌍️gis");
else {
