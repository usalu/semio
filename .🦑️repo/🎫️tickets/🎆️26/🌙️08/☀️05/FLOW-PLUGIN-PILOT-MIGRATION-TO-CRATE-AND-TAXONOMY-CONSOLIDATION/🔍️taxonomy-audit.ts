...
function validateTaxonomyTree(pluginRoot: string, pluginId: string): string[] {
  ...
  if (existsSync(libRsPath)) {
    ...
    for (const p of declaredPaths) if (p.endsWith(".rs") && !existsSync(join(pluginRoot, p))) findings.push(`${pluginId}: 📦️lib.rs declares #[path = "${p}"] but the file does not exist on disk`);
  } else {
