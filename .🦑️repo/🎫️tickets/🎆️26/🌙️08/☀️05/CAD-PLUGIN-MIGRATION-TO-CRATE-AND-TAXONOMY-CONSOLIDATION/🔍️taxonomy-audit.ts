#!/usr/bin/env bun
/** 🔍️ Standalone mirror of `📇️registry/📜️script.ts`'s `validateTaxonomyTree`, so the cad migration can audit
 * 📐️cad's tree while the real `check` still bails early on registry staleness (regeneration is
...
const pluginId = process.argv[3] ?? "📐️cad";
