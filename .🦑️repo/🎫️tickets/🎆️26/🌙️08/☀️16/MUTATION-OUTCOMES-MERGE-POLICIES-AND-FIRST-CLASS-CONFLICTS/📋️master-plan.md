...
8. Reject (`policy.rejects(worst)`): dag not advanced, state unchanged, push `Conflict{Quarantined{envelopes}, Open, messages}` → `MergeReport{accepted:false}`. The deciding `MergePolicy` is runtime-local authority state, returned in reports/errors but never persisted in a conflict or artifact history.
