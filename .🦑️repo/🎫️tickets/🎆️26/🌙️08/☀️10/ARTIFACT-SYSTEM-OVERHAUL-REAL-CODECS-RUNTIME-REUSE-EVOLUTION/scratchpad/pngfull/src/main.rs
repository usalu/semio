// Standalone port of the real png engine (byte-for-byte logic, only the
// crc32/zlib_compress/zlib_decompress calls and the PngSnapshot/RasterImage/register()
// glue are stubbed/removed) to get real cargo test execution while the workspace crate
// is blocked by an unrelated concurrent zip-engine refactor (see STATUS.md).

//#region LocalStandIns
#[derive(Clone, Debug, PartialEq, Default)]
struct RasterImage { width: u32, height: u32, rgba: Vec<u8> }

#[derive(Clone, Debug, PartialEq, Default)]
struct PngSnapshot { schema: String, image: RasterImage }

const STDIO_PNG_DOCUMENT_SCHEMA: &str = "stdio.png";
...
// zlib stream wrapping "stored" (uncompressed) deflate blocks — sufficient to validate
// chunk/filter/bit-depth/Adam7 logic without re-testing the real LZ77 compressor
// (already verified separately, per STATUS.md, and unchanged).
...
fn zlib_decompress(data: &[u8]) -> Result<Vec<u8>, String> {
    ...
    loop {
        ...
        let btype = (data[pos] >> 1) & 0b11;
        if btype != 0 { return Err("scratch zlib stub only supports stored blocks".into()); }
        pos += 1;
...
//#region Crc
fn png_crc32(data: &[u8]) -> u32 { crc32(data) }
//#endregion Crc
...
//#region Codec
fn rgba_len(img: &RasterImage) -> Result<usize, String> {
...
fn encode_png(snap: &PngSnapshot) -> Result<Vec<u8>, String> {
    let img = &snap.image;
    rgba_len(img)?;
    let bpp = 4usize;
    ...
    let compressed = zlib_compress(&idat)?;
    let mut out = Vec::new();
...
fn decode_png(data: &[u8]) -> Result<PngSnapshot, String> {
...
mod tests {
    ...
    fn hand_encode(width: u32, height: u32, bit_depth: u8, color_type: u8, plte: Option<&[u8]>, trns: Option<&[u8]>, raw_rows: &[u8]) -> Vec<u8> {
        ...
        let compressed = zlib_compress(&idat).unwrap();
        let mut out = Vec::new();
        out.extend_from_slice(&PNG_SIGNATURE);
        let mut ihdr = Vec::with_capacity(13);
