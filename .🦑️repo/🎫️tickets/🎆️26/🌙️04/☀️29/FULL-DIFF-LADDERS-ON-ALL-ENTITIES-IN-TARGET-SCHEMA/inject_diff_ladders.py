#!/usr/bin/env python3
"""
🔖️ Injects per-entity Modification→DiffsConnection ladders into compose/graphql/target.schema.graphql
...
# 🔖️ Mirrors existing PlaneDiffsOwner concrete roots (Operation implementations omitted — unions stay object-only).
...
# 🔖️ Attribute region already defines modification/diff paths; only AttributesDiff→AttributeDiffs rename is applied.
SKIP_ENTITIES = frozenset({"Attribute"})

WEAK_ENTITIES = frozenset({"Vector", "Point", "Coordinate", "Offset"})

# 🔖️ Connector / Representation / Type share one `#region Type` block.
# 🔖️ Connection lives under `#region Piece`; VCS entities under `#region VCS`.
...
def region_bounds(text: str, region: str) -> tuple[int, int]:
    """🔖️ [start, end) covering `#region {region}` … `#endregion {region}` with nested `#region` blocks."""
...
def rename_attributes_diff_typenames(text: str) -> str:
    """🔖️ AttributesDiff → AttributeDiffs for naming alignment with *Diffs."""
...
def entity_ladder(name: str, *, weak_entity: bool) -> str:
    """🔖️ Nine prefixed SDL fragments in fixed internal order."""
...
def find_brace_block_end(s: str, open_brace: int) -> int:
    """🔖️ First index after outer `{` … `}` starting at open_brace."""
    depth = 0
    i = open_brace
    while i < len(s):
        c = s[i]
...
def inject_after_primary_connection(text: str, entity: str, region: str, ladder: str) -> str:
    """🔖️ Insert ladder immediately after `type {entity}Connection …` inside region."""
...
def merge_union(text: str, union_name: str, new_members: list[str]) -> str:
    """🔖️ Replace `union Name = …` body with sorted unique members (multiline)."""
    m = re.search(
...
def merge_aggregate_union(text: str, union_pat: str, extras: list[str]) -> str:
    """🔖️ Merge AggregateEntityEdge / EntityConnection / OwnerEntity / OwnedEntityConnection."""
...
def remove_piece_mod_stub(text: str) -> str:
    """🔖️ Drop minimal PieceModification stub — replaced by generated ladder."""
    return re.sub(
