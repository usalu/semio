...
todos:
 - id: open-ticket
   ...
   status: completed
 - id: wire-types
   content: "Write `🔌️WireTypes` region: handwritten TS for KitFullDto, every GraphQL Input/Output, KitEvent, SetError/SetResult, Read/Write/Backbone/Conflict types"
   status: completed
 - id: graphql-worker
   content: Implement `🪜️GraphQLWorker` (per-store dedicated Worker, string-only postMessage, request-id demultiplexing into RxJS Subjects)
   status: completed
 - id: graphql-helpers
   content: "Implement private typesafe `🧰️GraphQL` helpers: `mutation<T>`, `query<T>`, `subscription<T>` over the worker"
   status: pending
 - id: kitstore-class
   content: "Implement `📦️KitStore` thick API: lifecycle, snapshot/theKit/materializeAt, events$, all writes, reads, VCS/backbone/conflict, undo/redo"
   status: pending
 ...
 - id: migrate-others
   ...
   status: completed
 - id: tests
   content: Extend `🧪️EmbeddedTests` to cover the new `KitStore` end-to-end; run `npm test` in `compose/js`, `compose/react`, `compose/sketchpad` and fix until green
   status: pending
 - id: close-ticket
...
- Every public method is `async` and typesafe; every argument is a typed TS shape from the new internal `🔌️WireTypes` region.
...
- [compose/js/index.ts](compose/js/index.ts) — full rewrite. New region layout (per AGENTS):
  - `🧲️Header`
  - `📥️Imports` (only `rxjs`)
  - `🔌️WireTypes` — handwritten TS mirroring `compose/rs` schema (`KitFullDto`, every Input/Output, `KitEvent`, `SetError`, `SetResult`, `ReadCommandBatch`, `ReadCommandBatchResult`, `BackboneConfig`, `BackboneStatusDto`, `ConflictResolution`, `KitConflict`, `VcsState`, …)
  - `🪜️GraphQLWorker` — owns `Worker`, request-id → `Subject<string>` map, persistent `eventStream` subscription
  - `🧰️GraphQL` — private typed helpers `mutation<T>`, `query<T>`, `subscription<T>`
  - `📦️KitStore` — sole exported class
  - `🧪️EmbeddedTests` — vitest suite rewritten in place (no new test files)
...
Extend `🧪️EmbeddedTests` in [compose/js/index.ts](compose/js/index.ts) (no new test files):
...
`ticket_open` → goal `🎯️r2602🎯️runningsketchpad`, title `Single Async KitStore Export In compose/js`. All temporary files inside the ticket folder. Close with summary + file list when tests pass.
