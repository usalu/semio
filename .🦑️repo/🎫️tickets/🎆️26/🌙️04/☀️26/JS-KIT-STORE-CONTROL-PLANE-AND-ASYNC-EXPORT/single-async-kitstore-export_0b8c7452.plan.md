...
todos:
 - id: wire-types
   content: "Rewrite `🔌️WireTypes` region in `compose/js/index.ts`: handwritten TS for KitFullDto, every Input/Result, KitEvent, BackboneConfig, BackboneStatus, VcsState, KitConflict, Unsubscribe, ReadKitCommandBatchInput/Result"
   status: completed
 - id: worker-transport
   content: "Implement `🪜️WorkerTransport` (private): per-KitStore dedicated Worker, string-only postMessage, reqId demux into private `Subject<string>`, persistent `kitEvents` subscription"
   status: completed
 - id: graphql-helpers
   content: "Implement private typesafe `🧰️Graphql` helpers: `mutation<TIn,TOut>`, `query<TIn,TOut>`, `subscription<TOut>` over the worker (rxjs internal only)"
   status: completed
 - id: kitstore-class
   content: "Implement `📦️KitStore` sole exported class: lifecycle, snapshot/theKit/materializeAt/read, all typed writes, callback-based `subscribe`/`subscribeFiltered`, VCS/backbone/conflict/undo/redo"
   status: completed
 ...
 - id: delete-kit-entities
   content: "Removed `kitEntities.ts` as a separate module: body merged into `compose/react/kitWasmClient.ts` (`🧩️KitEntitiesMerged`); `@semio-tech/compose-js` wire DTO aliased `ComposeKitWireDto` to avoid clashing with zod `KitFullDto`. Domain still lives in that region until hooks migrate to rs-only."
   status: completed
 - id: migrate-react
 ...
 - id: delete-native-adapter
   content: "Delete standalone `nativeAlgorithmAdapter.ts`: logic merged into `compose/algorithms/index.ts` under `🧮️NativeKitStoreRunners`; stories/main import `../index` / `../../index`. TS path still uses `KitStore` + `@semio-tech/compose-react` entities for diff display until rs preview commands exist."
   status: completed
 - id: migrate-ui
...
- [compose/js/index.ts](compose/js/index.ts) — full rewrite. Region layout per AGENTS:
  - `🧲️Header`
  - `📥️Imports` (only `rxjs`)
  - `🔌️WireTypes` — handwritten TS mirroring rs schema (`KitFullDto`, every `*Input`/`*Result`, `KitEvent`, `KitEventFilter`, `BackboneConfig`, `BackboneStatus`, `VcsState`, `KitConflict`, `ReadKitCommandBatchInput`, `ReadKitCommandBatchResult`, `Unsubscribe`)
  - `🪜️WorkerTransport` — owns the `Worker`, sends `{reqId, payload}` strings, dispatches incoming `{reqId, line}` strings to private `Subject<string>` per reqId, opens the persistent `kitEvents` subscription on construction
  - `🧰️Graphql` (private) — typed `mutation<TIn, TOut>(query, vars): Promise<TOut>`, `query<TIn, TOut>(...)`, `subscription<TOut>(query, onLine): Unsubscribe` — all use rxjs `firstValueFrom`/`take(1)`/`Subject` internally
  - `📦️KitStore` — sole exported class; methods are 1-line wrappers over `🧰️Graphql`; `subscribe(handler)` is a thin callback wrapper over a private `Subject<KitEvent>` (`.subscribe(handler).unsubscribe`)
  - `🧪️EmbeddedTests` — vitest suite rewritten in place (single file rule)
...
- [compose/react/kitWasmClient.ts](compose/react/kitWasmClient.ts) — former `kitEntities.ts` merged under `🧩️KitEntitiesMerged` (file removed); long-term: strip entity graph from React per §Hard rules.
...
- [compose/js/index.ts](compose/js/index.ts) `🧪️EmbeddedTests`:
...
- **Domain in `@semio-tech/compose-react`**: former `kitEntities` code now lives under `kitWasmClient.ts` → `🧩️KitEntitiesMerged`; hooks in `index.tsx` still use zod/entity classes until migrated to DTO + `KitStore` only.
