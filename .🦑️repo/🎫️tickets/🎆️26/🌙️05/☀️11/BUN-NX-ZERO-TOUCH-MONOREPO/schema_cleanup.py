...
def ensure_diff_banners(text: str) -> str:
    """🧩️ Insert `# Diff` before `# <Name>Diff` on every concrete Diff type (idempotent)."""
...
def narrow_placeholder_operation_scopes(text: str) -> str:
    """🧩️ Replace generic `scope: … // Entity` with the real scope kind for kit/type/design entry ops."""
