...
// #region 🧵️InlineWorker

/** @emoji 🧵️ Bundled worker chunk — Vite resolves `@semio-tech/compose-rs-wasm`; Blob workers cannot import bare specifiers. */
function createKitStoreWorker(): Worker {
  return new Worker(new URL("./kit-store.worker.ts", import.meta.url), { type: "module" });
}

// #endregion 🧵️InlineWorker
// #region 🔌️JsonGraphQlDtoTypes

/** @emoji 🪪️ Correlates kit command lifecycle events on the dto. */
...
/** @emoji 🧾️ Normalized set/mutation error from Rust `SetError`. */
...
/**
 * @emoji 🧾️ `KitChangeKind` on the dto (camelCase from `compose/rs`), plus any `other` label inside `other`.
 */
...
/** @emoji 🧾️ GraphQL `KitChangeSemanticKind` enum (SCREAMING_SNAKE); pair with {@linkcode KitChangeKind} via {@linkcode kitChangeSemanticKindToGraphQl}. */
...
/** @emoji 🧾️ Maps batch `changeKind` + `changeKindOther` into {@linkcode KitChangeKind} (camelCase unit or `{ other }` for extension labels). */
...
/** @emoji 🪢️ Object branch for GraphQL / serde JSON trees (explicit string slots, no `Record` alias). */
export type KitJsonObjectDto = { readonly [slot: string]: KitJsonTreeDto };
/** @emoji 🪢️ Recursive JSON tree from GraphQL / serde kit scalars. */
export type KitJsonTreeDto = string | number | boolean | null | readonly KitJsonTreeDto[] | KitJsonObjectDto;

/** @emoji 🧱️ Parsed JSON tree (strict surface; no open index-signature object typing or untyped values). */
export type JsonValue = string | number | boolean | null | readonly JsonValue[] | JsonObject;
/** @emoji 🧱️ JSON object node (string-keyed). */
export type JsonObject = { readonly [key: string]: JsonValue };

/** @emoji 🔒️ Recursive readonly DTO view for Zod-inferred and GraphQL-shaped values. */
...
/** @emoji 🧱️ Mutable JSON object for local construction. */
type JsonObjectMutable = { [key: string]: JsonValue };
/** @emoji 🧱️ Mutable `variables` map for GraphQL / kit batch (alias of the JSON builder surface). */
type GraphQlObjectMutable = JsonObjectMutable;

/** @emoji 🧾️ `JSON.parse` with a {@link JsonValue} root (GraphQL transport, config knobs). */
function parseJsonValue(text: string): JsonValue {
  return JSON.parse(text) as JsonValue;
}

/** @emoji 🧾️ GraphQL HTTP/FFI response envelope before unwrapping `data`. */
...
/**
 * @emoji 🧾️ Tags accepted on `KitStore` control-plane nested `Mutation.session` mappers and {@link WasmKitStoreClient} routing.
 * @public
 */
...
/** @emoji 🧭️ Backbone / conflict dto shapes (serde-tagged, matches `kit_backbone_dto` in `compose/rs`). */
...
/** @emoji 🧾️ GraphQL `ConflictResolutionBatchInput` (matches `compose/graphql/schema.graphql`). */
...
/** @emoji 🧾️ One read command in a `KitStore.read` batch (matches `compose/rs` read kit dto, serde camelCase). */
...
/**
 * @emoji 🧭️ Materialization anchor for target-schema versions (`wip.theKit.kit`, checkpoints, or alternatives).
 */
...
/** @emoji 🧭️ Main committed kit line (default read point). */
export const theKitReadPoint: KitReadPoint = { theKit: null };

/** @emoji 🧭️ Maps {@link KitReadPoint} to GraphQL `KitReadPointInput` (flat camelCase fields). */
...
/** @emoji 🧭️ Read point used for scoped kit reads on a {@link KitStoreClient}. */
...
/** @emoji 🧪️ Stable string for cache keys (JSON of the point branch). */
...
// #region 🔖️KitWriteScope
/** @emoji 🧭️ Target-schema unsaved **Change** id (`VersionCommandInput.unsavedChange(id)`). */
export type ChangeId = string;

/** @emoji 🧭️ Active unsaved change anchor for `Mutation.session { theKit { unsavedChange(id:) { kit { … } } } }`. */
export type KitWriteScope = { readonly changeId: ChangeId };

/**
 * @emoji 🧾️ VCS helpers aligned with `SessionCommandInput` / `VersionCommandInput` (`compose/graphql/target.schema.graphql`).
 * @public
 */
...
/** @emoji 🧾️ Maps a loose plane DTO into GraphQL `PlaneInputBatch` (camelCase axes). */
...
// #endregion 🔖️KitWriteScope

// #region 🔖️ReadBatchAndKitRead
/** @emoji 🧾️ One `design.flattenMap` row (`DesignFlattenMapEntryObject`). */
...
/** @emoji 🧾️ Per-piece document + flat pose row from `design.pieces` (`PieceStore` GraphQL fields). */
...
/** @emoji 🧾️ Connector color row from the live kit read model. */
...
/** @emoji 🧾️ One `design.includedDesigns` entry (`IncludedDesignObject`). */
...
/** @emoji 🧾️ `KitMetadataObject` root fields from GraphQL. */
...
// #endregion 🔖️ReadBatchAndKitRead

/** @emoji 🧾️ Batch input for {@link KitStore.read} (per-command, same for all entries in a batch). */
export type ReadBatch = readonly ReadKitCommand[];

/** @emoji 🧾️ One entry in a {@link ReadBatch} (alias for consumers that say “read dto item”). */
...
/** @emoji 🧾️ One command’s read output object (per-command payload shape from `compose/rs` GraphQL). */
...
/** @emoji 🧾️ Batch output from {@link KitStore.read}. */
export type ReadBatchResult = readonly ReadKitCommandOutput[];

/**
 * @emoji 📣️ GraphQL `KitEvent` scalar + synthetic invalidation rows used by {@link WasmKitStoreClient};
...
/** @emoji 🧾️ Optional filter for {@link KitStore.subscribeFiltered}. */
export type KitEventFilter = (event: KitEvent) => boolean;

/** @emoji 🧾️ Unsubscribe handle returned by {@link KitStore.subscribe}. */
...
// #region 🔖️ChangeKitCommand
/** @emoji 🧾️ `ChangePieceCommand` JSON (externally tagged, camelCase variant keys) for `kitStore.batch` live `changeKitCommands` (or `ChangeKitCommand` GraphQL scalars). */
...
/** @emoji 🧾️ `ChangeConnectionCommand` JSON for nested design commands. */
...
/** @emoji 🧾️ Nested `ChangeDesignCommand` entries. */
...
/** @emoji 🧾️ Nested `ChangeTypeCommand` entries used by stores / React. */
...
/** @emoji 🧾️ `ChangeFamilyCommand` JSON. */
...
/** @emoji 🧾️ Top-level `ChangeKitCommand` JSON for `changeKitCommands` batch variables. */
...
/** @emoji 🧾️ True for object-shaped JSON / kit tree nodes (excludes arrays and `null`). */
...
/** @emoji 🧾️ GraphQL `ConflictBatchRecord` row (matches `compose/graphql/schema.graphql`). */
...
/** @emoji 🧾️ One row from GraphQL `KitStoreResult` (camelCase dto). */
...
/** @emoji 🧾️ Forward + inverse command atoms on the subscription bus (`KitChange` from `compose/rs`). */
...
/** @emoji 🧾️ True when {@link KitEvent} is a {@link KitClassifiedMutationEvent}. */
...
/** @emoji 🧾️ Wrap nested piece commands under one design id. */
...
/** @emoji 🧾️ Wrap nested connection commands under one design id. */
...
/** @emoji 🧾️ Maps schema/UI data keys onto connection dto keys (`u`→`x`, `v`→`y`). */
...
/** @emoji 🧾️ UI/schema partial for piece field writes (maps to {@link ChangePieceCommand}). */
export type PieceFieldPatchInput = Readonly<{
  name?: string | null;
  description?: string | null;
  /** @emoji 📐️ Optional explicit placement; expands to `plane` / `center` change commands. */
...
/** @emoji 🧾️ UI/schema partial for connection field writes. */
...
/** @emoji 🧾️ Value bucket for `buildSchemaEntityChangeCommands` (schema hooks). */
export type SchemaEntityFieldValue = KitJsonTreeDto | string | number | boolean | null | object;

/** @emoji 🧾️ Converts a piece field patch into nested `changePieceCommands` dto entries. */
...
/** @emoji 🧾️ Converts a connection field patch into nested `changeConnectionCommands` dto entries. */
...
/**
 * @emoji 🧾️ Maps a schema entity + field to `changeKitCommands` for `submitChangeKitCommands` (React + kit store).
...
/** @emoji 🧾️ Shorthand for `client.submitChangeKitCommands` (React / tests). */
...
/** @emoji 🧾️ Locates the parent design id for a piece or connection via the current kit snapshot. */
...
/** @emoji 🧾️ Applies a piece field patch under one design (dto construction stays in JS). */
...
/** @emoji 🧾️ Applies a connection field patch under one design (dto construction stays in JS). */
...
/**
 * @emoji 🧾️ Writes a single field on a top-level or nested entity via `changeKitCommands` (React / kit store).
...
// #endregion 🔖️ChangeKitCommand

// #endregion 🔌️JsonGraphQlDtoTypes

// #region 🔖️ScopedKitMutations

/** @emoji 🧾️ GraphQL string literal (escaped). */
function __gqlStr(s: string): string {
  return JSON.stringify(s);
}

/** @emoji 🧾️ Inline `[ID!]!` list. */
function __gqlIds(ids: readonly string[]): string {
  return `[${ids.map((x) => __gqlStr(x)).join(",")}]`;
}

/** @emoji 🧾️ Wraps kit selection under `unsavedChange` + `theKit` (target command tree). */
...
/** @emoji 🧾️ One scoped mutation for a legacy {@link ChangeKitCommand}, or `null` when unsupported on `KitOperationInput`. */
...
// #endregion 🔖️ScopedKitMutations

// #region 🪢️InternalReadBatch
...
/** @emoji 🧾️ GraphQL selection on a target-schema version's nested `kit` (`wip.theKit.kit`, checkpoint root, or alternative kit). */
...
/** @emoji 🧾️ Extract the scoped `Kit` JSON payload for a {@link KitReadPoint}. */
...
/** @emoji 🧾️ Flattens Relay `*Connection` lists on `Query.wip` for VCS helpers (`canUndo`, tests). */
...
/** @emoji 🧾️ Maps GraphQL `TypeMetadataObject` / `DesignMetadataObject` field names to {@link TypeSchema} / {@link DesignSchema} dto keys (`createdAt`, `virtual`, …). */
...
//#region 🔌️KitGraphqlReadSelections
...
/** @emoji 🧾️ GraphQL JSON uses `null` for absent scalars; Zod `.optional()` expects omission — drop top-level `null` entries before parse. */
...
/** @emoji 🧾️ JSON, `KitJsonTreeDto`, or typed bus {@link KitEvent} in subscription and lifecycle helpers. */

/** @emoji 🧾️ Narrows subscription payloads to compose kit command lifecycle rows. */
...
/**
 * @emoji 🌐️ Typed GraphQL execute: same transport as {@link kitGraphqlRun} with a parametric `data` root for call sites.
...
/** @emoji 🧵️ `Worker` `error` events are often `ErrorEvent` with empty `message` when a module script fails to load (e.g. Vite down → net::ERR_CONNECTION_REFUSED). */
...
class WorkerStringTransport {
  ...
  /** @emoji 🧵️ Resolves on `ready`; rejects fast on worker-thread `error` (e.g. `@semio-tech/compose-rs-wasm` not resolvable from Blob worker) instead of waiting the full timeout. */
...
/** @emoji 🔗️ Smoke query: `Query.wip` + `Graph.theKit` (`compose/graphql/target.schema.graphql`). */
export const KIT_SESSION_QUERY_ENTRY = `query { wip { id theKit { id } } }` as const;

/** @emoji 🔗️ Multiplexed kit stream: `Subscription.event: Json!` (`compose/graphql/target.schema.graphql`). */
export const KIT_EVENT_STREAM_SUBSCRIPTION = `subscription { event }` as const;

/** @emoji 🧾️ Envelope kinds inside {@link KIT_EVENT_STREAM_SUBSCRIPTION} (Worker D contract). */
export type KitEventEnvelope = Readonly<{ readonly kind: string; readonly payload?: unknown }>;

/** @emoji 🔗️ @deprecated Prefer {@link KIT_EVENT_STREAM_SUBSCRIPTION}; correlator listens via `event` envelope. */
export const KIT_COMMAND_SUCCEEDED_SUBSCRIPTION = KIT_EVENT_STREAM_SUBSCRIPTION;

/** @emoji 🔗️ @deprecated Prefer {@link KIT_EVENT_STREAM_SUBSCRIPTION}. */
export const KIT_OPERATION_FAILED_SUBSCRIPTION = KIT_EVENT_STREAM_SUBSCRIPTION;

/**
 * @emoji 🔗️ Main-line full kit DTO query.
 */
export const KIT_SCOPED_FULL_DTO_QUERY = `query { wip { theKit { kit { fullSnapshot } } } }` as const;

/** @emoji 🧾️ Typed `data` shape for {@link KIT_SCOPED_FULL_DTO_QUERY} on {@link theKitReadPoint}. */
...
//#region 🧱️StorePrimitives

/** @emoji 📛️ Write lane status for {@link StoreCommand} (stable object identities for triads). */
...
/** @emoji 🧾️ Frozen idle — stable identity for `useSyncExternalStore` snapshots. */
export const WRITE_STATUS_IDLE: WriteStatus = Object.freeze({ kind: "idle", pending: 0 });
/** @emoji 🧾️ Frozen readonly — stable identity when the lane is read-only. */
export const WRITE_STATUS_READONLY: WriteStatus = Object.freeze({ kind: "readonly", pending: 0 });
/** @emoji 🪪️ Frozen pending for rs-backed mutations — stable identity while a command is in flight. */
export const WRITE_STATUS_PENDING: WriteStatus = Object.freeze({ kind: "pending", pending: 1 });

/** @emoji 🪪️ Same semantic {@link WriteStatus} → reuse prior reference (avoids React #520 from fresh object identity). */
...
/** @emoji 🧾️ Pushes live values into {@link StoreField}; return cleanup unsubscriber. */
export type StoreFieldSource<T> = (push: (next: T) => void) => Unsubscribe;

/** @emoji 📥️ Read-only typed mirror; values enter only through the optional `source` `push` callback. */
...
/** @emoji 📝️ Async side-effect carrier with render-ready {@link WriteStatus} (domain-agnostic). */
...
/** @emoji 🚦️ Generic request-id ↔ Promise-resolver correlator for mutations that complete on the rs output stream. */
...
//#endregion 🧱️StorePrimitives

/** @emoji 🧾️ Uniform `renameKit` args; scope is unused (session `theKit` path). */
export type RenameKitCommandArgs = { readonly scope: Record<string, never>; readonly input: { readonly name: string } };

/**
 * @emoji 🌐️ Single kit control plane: GraphQL strings over one dedicated `Worker` running `compose/rs` WASM (`KitStoreHandle`).
 */
export class KitStore {
  ...
  /** @emoji 🧭️ Active unsaved {@link Change} id for `VersionCommandInput.unsavedChange(id)`. */
  private kitWriteChangeId: string | null = null;

  /** @emoji 🧭️ Active {@link KitReadPoint} for scoped kit reads (see {@link WasmKitStoreClient.setKitReadPoint}). */
  private activeReadPoint: KitReadPoint = theKitReadPoint;

  /** @emoji 🧾️ Cached {@link StoreField}s for {@link kitField} (disposed with store). */
  private readonly kitFields = new Map<string, StoreField<unknown>>();

  private readonly correlator: RequestCorrelator;
  /** @emoji 🪪️ Scoped `session { theKit { unsavedChange { kit { rename } } } }` + version-level save. */
  ...
  /** @emoji 🪪️ Live kit name via {@link kitField}. */
  ...
  /**
   * @emoji 🧾️ Live {@link StoreField} over `Query.wip.theKit(at:)` + `innerOnKit` GraphQL selection (cache keyed by {@link cacheKey}).
  ...
  static async open(initialKit: KitFullDto, opts?: KitStoreOpenOptions): Promise<KitStore> {
    ...
    if (useDedicatedWorker) {
      ...
      try {
        ...
        /** @emoji 🧵️ Blob worker can't resolve `@semio-tech/compose-rs-wasm` bare specifier → fall back to inline WASM on the main thread (still real rust authority). */
  ...
  /** @emoji 🧭️ Pin materialization reads to `at`; bumps {@link invalidations}. */
  ...
  /** @emoji 🧾️ `Query.wip.theKit` materialized {@link KitStore} selection for read helpers. */
  ...
  /** @emoji 🧾️ Session/wip {@link KitStore} JSON fragment for a read scope (used by live reads such as connector colors). */
  ...
  /** @emoji 🧾️ Runs a session/wip/store query merging `extra` variables (e.g. `id` for `design(id: $id)`). */
  ...
  /** @emoji 📣️ Subscribe to kit GraphQL subscription events (RxJS-free public surface). */
  ...
  /** @emoji 📣️ Subscribe only when {@link KitEventFilter} returns true. */
  ...
  /** @emoji 📣️ Fires only after coalescing dto `Changed` / synthetic `{ Changed: null }` rows. */
  ...
  /** @emoji 📣️ Kit command lifecycle scalar rows (`composeKitCommand` / `ComposeKitCommand`). */
  ...
  //#region 🪪️KitNameAndRename

  /** @emoji 🧾️ GraphQL-backed read; pushes `parse(data)` initially and on each {@link invalidations} tick. */
  ...
  /** @emoji 🧾️ Ensures {@link kitWriteChangeId} via `VersionCommandInput.startNewChange`. */
  ...
  /** @emoji 🪪️ Cacheless Promise read of kit name via GraphQL into rs (honours {@link activeReadPoint}). */
  ...
  /** @emoji 🧾️ `Subscription.event` JSON envelope (`kind` + `payload`) or legacy stream rows. */
  ...
  /** @emoji 🧾️ Full DTO for a {@link KitReadPoint} via target-schema version `kit.fullSnapshot`; sole full-kit read path (no WASM `snapshot` fallback). */
  ...
  /** @emoji 🧾️ Main-line live kit DTO from `Query.wip.theKit.kit` (`fullSnapshot` / materialized JSON). */
  ...
  /** @emoji 🧾️ Undo stack is owned by `compose/rs` edit log; JS does not read a draft cursor on the target schema. */
  async canUndo(): Promise<boolean> {
    void (await this.vcsState());
    return false;
  }

  /** @emoji 🧾️ Redo stack is owned by `compose/rs` edit log; JS does not read a draft cursor on the target schema. */
  async canRedo(): Promise<boolean> {
    void (await this.vcsState());
    return false;
  }

  /** @emoji 🧾️ Maps control-plane canvas `commandKind` + `variables` to a `changeKitCommands` batch (`session.theKit.unsavedChange`). */
  ...
  /** @emoji 🌱️ Ensures an unsaved change exists for bundle hosts (`startNewChange`); target schema has no `kitStoreInitializeDefaults` root field. */
  async kitStoreInitializeDefaults(): Promise<string> {
    this.ensureAlive();
    return this.ensureKitWriteChangeId();
  }

  /** @emoji 🌱️ `Mutation.session { startAlternative }`; `sourceAlternativeId` is not represented on the target command tree yet. */
  ...
  /** @emoji 📸️ Serialize the RS-owned kit store bundle (`schema`, three graphs, checkpoints, changes, edits). */
  ...
  /** @emoji 🩻️ Hydrate the RS graph from a metabolism-shaped bundle read by the host adapter. */
  ...
  /** @emoji 🟢️ Starts a fresh unsaved change (`startNewChange`); sketchpad may call this on input focus. */
  ...
  /** @emoji 🧾️ Runs each {@link ChangeKitCommand} as `session { theKit { unsavedChange(id:) { kit { … } } } } }` (see {@link buildScopedChangeKitMutation}). */
  ...
  /** @emoji 🧭️ Exposes the active unsaved change id (or {@link KitStore.setKitWriteScope}). */
  getKitWriteScope(): KitWriteScope | null {
    const c = this.kitWriteChangeId;
    return c ? { changeId: c } : null;
  }

  /** @emoji 🧭️ Pins the unsaved change id (`null` clears the local handle only). */
  setKitWriteScope(scope: KitWriteScope | null): void {
    this.kitWriteChangeId = scope?.changeId ?? null;
  }

  /** @emoji ✅️ Persists the version lane (`session { theKit { save } }`). */
  ...
  /** @emoji ⛔️ Drops the local unsaved-change handle without calling save (server-side discard is not on the target command tree). */
  ...
  /** @emoji 🧾️ Top-level `session.backbone` mutations returning {@link SetResult}. */
  ...
  /** @emoji 🧾️ Backbone reads: conflicts list or attach status via GraphQL. */
  ...
  /** @emoji 🧾️ Undo/redo are not on the target `SessionCommandInput` tree; callers should use rs edit-log APIs when exposed. */
  ...
  /** @emoji 🧾️ Canvas ops mapped into `changeKitCommands` + {@link runScopedTransactionBatch}. */
  ...
  /**
   * @emoji 🧭️ Read-only flatten map rows for one design (`compose/rs` `flatten_map`), for algorithm / MCP tooling.
   */
  ...
  /** @emoji 🧾️ Apply typed `ChangeKitCommand` batch inside the active unsaved version change (`kitStore.batch`). */
  ...
  /** @emoji 🧭️ Sync handle for a connection within a design (no I/O). */
  ...
  /** @emoji 🧭️ All design ids in the live kit as {@link DesignStore} handles. */
  ...
  /** @emoji 🧭️ All kind ids in the live kit as {@link TypeStore} handles. */
  ...
  /** @emoji 🧾️ Design row id strings from `readKitDesignIdsCommand` (no {@link DesignStore} allocation). */
  ...
  /** @emoji 🧾️ Kind row id strings from `readKitTypeIdsCommand` (no {@link TypeStore} allocation). */
  ...
  /** @emoji 🧾️ Per-kind metadata rows (`readKitTypesMetadataCommand`). */
  ...
  /** @emoji 🧾️ Per-design metadata rows (`readKitDesignsMetadataCommand`). */
...
/**
 * @emoji 🧰️ Convenience alias for {@link KitStore.open}.
 */
...
/** @emoji 🧾️ Whether a {@link KitChange} references a design id in forward or inverse commands. */
...
/** @emoji 🧾️ Whether a {@link KitClassifiedMutationEvent} concerns the given design id. */
...
/** @emoji 🧾️ Whether a {@link KitClassifiedMutationEvent} concerns the given kind id. */
...
/** @emoji 🧾️ Whether a {@link KitClassifiedMutationEvent} concerns the given piece in a design. */
...
/** @emoji 🧾️ Whether a {@link KitClassifiedMutationEvent} concerns the given connection in a design. */
...
/** @emoji 🧪️ True when JSON-like subtree (including kit command atoms) contains a string field `key` equal to `id`. */
...
/** @emoji 🧭️ Design-scoped kit events (excludes bare `Changed` and `FlattenInvalidated`, which are handled separately per subscriber). */
...
/** @emoji 🧭️ Whether a subscription {@link KitEvent} likely concerns the given design (includes kit-wide invalidations). */
...
/** @emoji 🧭️ Type-scoped events (no bare `Changed`). */
...
/** @emoji 🧭️ Whether a subscription {@link KitEvent} likely concerns the given kind id. */
...
/** @emoji 🧭️ Piece-scoped events (design-scoped strict + piece id + flatten rows). */
...
/** @emoji 🧭️ Connection-scoped events (design-scoped strict + connection id). */
...
/** @emoji 🧭️ Family / file / folder entity filters (ChildAdded paths + id fields). */
...
// #region 🧩️KitWasmBridgeMerged
// #region 🔌️KitStoreClientTypes

export type KitStoreExecuteResult = { ok: true; result: JsonValue } | { ok: false; error: SetError };

/** @emoji 🧾️ Sketchpad string-command context/result (opaque JSON). */
export type KitCommandContext = JsonObject;
export type KitCommandResult = JsonObject;

/** @emoji 🧾️ Typed kit mutation envelope for React facades (`kitStore.batch` version change `changeKitCommands`). */
type KitTypedChangeKitCommandsBatch = { readonly kind: "changeKitCommands"; readonly commands: readonly ChangeKitCommand[] };

/** @emoji 🧾️ Typed `changeKitCommands` batch facade for React (opaque to string command routers). */
...
/** @emoji 🧾️ Minimal async bridge for pulling authoritative kit JSON into a {@link KitHostStore}. */
export type ComposeKitBridge = { fetchFullKit(): Promise<KitFullDto> };

/** @emoji 🧾️ Browser / test kit RPC surface used by React hooks (wraps {@link KitStore}). */
export type KitStoreClient = ComposeKitBridge & {
  /** @emoji 🧾️ Scoped read point (see {@link WasmKitStoreClient#kitReadPoint} / {@link getKitClientReadPoint}). */
  ...
  /** @emoji 🪪️ Live kit name field + async rename (see {@link KitStore.kitName} / {@link KitStore.renameKit}). */
  readonly kitName: StoreField<string>;
  readonly renameKit: StoreCommand<RenameKitCommandArgs>;
  readKitName(): Promise<string>;
  /** @emoji 🧾️ Live scoped kit field selection (see {@link KitStore.kitField}). */
  kitField<T>(
    cacheKey: string,
    spec: {
      extraVariableDecl?: string;
  ...
  /** @emoji 🌱️ Fork a named alternative from the current checkpoint tip (`null` = kit main line). */
  ...
  /** @emoji 🧭️ Switch scoped kit read root (matches {@link WasmKitStoreClient.setKitReadPoint}). */
  setKitReadPoint(scope: KitReadPoint): void;
  dispose(): void;
};

// #endregion 🔌️KitStoreClientTypes
...
// #endregion 🧰️ReadHelpers

// #region 📦️LiveKitRoot

/** @emoji 🧭️ Graph-shaped reads routed through {@link KitStore.read} (no legacy JS kit graph). */
...
// #endregion 📦️LiveKitRoot

// #region 🪜️LiveReadHub
/** @emoji 🧾️ Live-read snapshot hub: {@link getComposeKitLiveReadStore} in 🪜️ComposeKitLiveReadHub (after {@link WasmKitStoreClient} / {@link kitStoreFromKitStoreClient}). */
// #endregion 🪜️LiveReadHub

// #region 🧰️EventFilters

/** @emoji 🧭️ Any kit graph mutation may flip undo/redo eligibility. */
export function kitEventAffectsCanUndoRedo(ev: KitEvent): boolean {
  void ev;
  return true;
}

/** @emoji 🧭️ Live piece reads invalidate when the piece or its design changes. */
...
/** @emoji 🧭️ Replaceable catalog reads are design-scoped. */
...
/** @emoji 🧭️ Design quality sum reads follow design-scoped invalidation. */
...
/** @emoji 🧭️ Type-scoped reads follow kind-scoped invalidation. */
...
/** @emoji 🧭️ Colored connector rows follow kit-wide GraphQL reads; invalidate on any kit event. */
export function kitEventAffectsKitColoredConnectorsRead(ev: KitEvent): boolean {
  void ev;
  return true;
}

// #endregion 🧰️EventFilters

// #region 📦️WasmKitStoreClient
...
export class WasmKitStoreClient implements KitStoreClient {
  ...
  /** @emoji 🧭️ Active read scope for {@link getPieces} and {@link fetchFullKit}. */
  ...
  /** @emoji 🧾️ Authoritative full kit from `compose/rs` via GraphQL (no local DTO cache). */
...
/** @emoji 🧾️ Resolves the live {@link KitStore} behind a {@link KitStoreClient}, or `null` for non-WASM bridges. */
...
// #region 🪜️ComposeKitLiveReadHub

/** 🧾️ Default {@link ComposeKitLiveReadStore#getSnapshot} when a key has not polled yet (stable identity for consumers). */
...
/**
 * @emoji 🧾️ Hub for async GraphQL-backed kit reads: {@link KitStoreClient} subscription fan-out, per-key
...
/** @emoji 🧾️ View-catalog reads via async {@link KitStore}. */
...
/** @emoji 🧾️ Per-design list/metadata reads on {@link KitStoreClient}. */
...
/** @emoji 🧾️ Shallow entity lists (designs / kinds / authors). */
...
// #endregion 🪜️ComposeKitLiveReadHub
...
// #region 🔖️CommandBuilder

/** @emoji 🧭️ Fluent `Mutation.session` navigator matching the target command tree; uses {@link KitWriteScope.changeId} / {@link KitStore.openKitWriteTransaction}. */
export class CommandBuilder {
  constructor(private readonly client: KitStoreClient) {}

  /** @emoji 🧭️ Root `Mutation.session` scope. */
  session(): SessionCommandNav {
    return new SessionCommandNav(this.client);
  }
}

/** @emoji 🧭️ `SessionCommandInput` façade (subset wired to rs today). */
...
/** @emoji 🧭️ `VersionCommandInput` façade (`Session.theKit` version lane). */
...
/** @emoji 🧭️ `UnsavedChangeCommandInput` façade (nested kit ops land in Worker E). */
...
/** @emoji 🧭️ `KitOperationInput` façade — delegates to existing {@link KitStoreClient} RPCs where they exist. */
...
/** @emoji 🧭️ `AlternativeCommandInput` façade. */
...
// #endregion 🔖️CommandBuilder

// #endregion 📦️WasmKitStoreClient

// #region 🧩️KitEntitiesMerged
// #region Constants
...
export class Piece {
  ...
  /** @emoji 🧭️ Whether this piece dtos a nested design id (schema hooks). */
  dtoDesignAsPieceId(): boolean {
    return Boolean(this.design?.id);
  }

  /** @emoji 🧭️ Wired type id for schema hooks. */
  dtoTypeId(): { id: string } | undefined {
    return this.type ? { id: this.type.id } : undefined;
  }

  /** @emoji 🧭️ Flat plane DTO for UI (structural truth in `compose/rs` reads). */
  flatPlane(): unknown {
    return this.plane ? this.plane.toDto() : undefined;
  }

  /** @emoji 🧭️ Flat center UV for UI. */
  flatCenter(): unknown {
    return this.center ? this.center.toDto() : undefined;
  }

  /** @emoji 🧭️ Alternative types for replaceable UI (populated from reads in full hosts). */
...
/** @emoji 🔧️ Gap/shift/rise knobs for structural move previews (algorithms UI). */
export type MoveVector = { readonly gap: number; readonly shift: number; readonly rise: number };

/** @emoji 📌️ Paste anchoring modes for copy/paste algorithm stories. */
export type PasteDesignAnchoringKind = "original" | "middle" | "centroid" | "bottomLeft" | "bottomRight" | "topLeft" | "topRight";

/** @emoji 🧠️ Optional per-piece flatten cache row (TS algorithm path; opaque to callers). */
...
export class Design {
  ...
  /** @emoji 🧭️ Included / sibling designs for nested-design UI (DTO navigation). */
  getDesignFamily(): Design[] {
    return [];
  }

  /** @emoji 🧾️ Legacy alias for diagram consumers (`@compose/ui`). */
  getConnections(): Connection[] {
    return [...(this._connections ?? [])];
  }

  /** @emoji 🧾️ Non-mutating diff overlay for MCP / diagram previews. */
  ...
  /** @emoji 🧩️ Merges a structural {@link DesignDiff} into this design (pieces + connections). */
  ...
  /** @emoji 🧾️ Selection drag in flat UV space (piece centers only; algorithm preview). */
...
/** @emoji 🧾️ Coerces native REST flatten payloads into {@link DesignOperationResult}. */
export function normalizeDesignFlattenResult(raw: unknown): DesignOperationResult {
  return raw as DesignOperationResult;
}
/** @emoji 🧾️ Coerces native REST diff payloads into {@link DesignDiffOperationResult}. */
export function normalizeDesignDiffResult(raw: unknown): DesignDiffOperationResult {
  return raw as DesignDiffOperationResult;
}
/** @emoji 🧾️ Coerces native REST copy payloads into {@link OperationResult}<{@link Design}>. */
...
// #region 🧾️KitStoreClientChildCommands

/** @emoji 🧾️ Child DTO kinds for kit-root add/remove commands (port rows use type-scoped flows, not top-level add). */
export type KitChildEntityKind = "Family" | "Author" | "Concept" | "Tag" | "Quality" | "File" | "Folder" | "Type" | "Design" | "Port";

/** @emoji 🧾️ Parses `dto` with the matching zod schema and runs `add*` kit dtos. */
...
/** @emoji 🧾️ Emits matching `remove*` kit dto for a kit-root child id. */
...
/** @emoji 🧾️ Adds a piece under a design (`addPiece` dto). */
...
/** @emoji 🧾️ Removes a piece from a design (`removePiece` dto). */
...
/** @emoji 🧾️ Adds a connection under a design (`addConnection` dto). */
...
// #endregion 🧾️KitStoreClientChildCommands
...
/** @emoji 🧾️ Fills missing `folders[].path` from legacy `name` + `parent` before {@link FolderSchema} parse. */
...
export class Kit {
  /** @emoji 📌️ Anchoring kinds exposed to copy/paste algorithm UI. */
  ...
  /** @emoji 🧭️ Normalizes dto/DTO kit records to a {@link Kit} entity (replaces legacy `Kit.ensure`). */
  static ensure(kit: Kit | KitFullDto): Kit {
    return kit instanceof Kit ? kit : Kit.fromDto(kit as KitFullDto);
  }

  /** @emoji 📋️ Copy selection (TS path stub — use REST language or extend with KitStore batch). */
  ...
  /** @emoji 📋️ Paste selection (TS path stub). */
  ...
  /** @emoji 🧭️ Resolve a design by id (DTO graph navigation for React schema hooks). */
  findDesign(id: string): Design | undefined {
    return this.designs?.find((d) => d.id === id);
  }

  /** @emoji 🧭️ Resolve a type by id. */
  findType(id: string): Type | undefined {
    return this.types?.find((t) => t.id === id);
  }

  /** @emoji 🧭️ Flatten / parent metadata map (DTO host; WASM bridge may supply richer maps). */
  ...
  /** @emoji 🧭️ Parent piece for `pieceId` via connection graph (connecting → connected). */
  ...
  /** @emoji 🧭️ Parent connection whose connecting side matches `pieceId`. */
  ...
  /** @emoji 🧭️ Child pieces: connections where connected side is `parentPieceId` and connecting side is another piece. */
  ...
  /**
   * @emoji 🧭️ Sync flatten preview for MCP / `@compose/ui` (identity plane fallback until async WASM is threaded here).
   */
...
/** @emoji 🧭️ Client-side v7/UUID id for empty kit records when not using WASM. */
...
/** @emoji 🧭️ DTO/entity to `Kit` (react / kit registry). */
export function asKitInstance(input: KitLike): Kit {
  return input instanceof Kit ? input : Kit.fromDto(input as KitFullDto);
}

/**
 * @emoji 🧾️ Pulls the authoritative DTO from `kitClient` into a host {@link KitHostStore} (no React; call after GQL events).
 */
/** @emoji 🧾️ Minimal bridge surface used when applying WASM snapshots onto a host store.
 ...
 *  state and looks like `assets/compose/metabolism.new.kit.compose.json`. */
...
export async function applyKitClientSnapshotToLocalStore(kitClient: KitStoreClient, store: KitHostStore): Promise<void> {
  ...
  // 🌱️ First-time wiring for bundle-persisting hosts: hydrate rs from the file (if any), then ensure rs has a seed checkpoint + default unsaved change.
...
/** @emoji 🧭️ Local/sync facet on every kit store snapshot (WASM or file-backed; hooks read `sync.readonly` etc). */
...
/** @emoji 🧾️ Alias for hosts that still import `KitHostStoreSnapshot` from `@semio-tech/compose-js`. */
export type KitHostStoreSnapshot = KitStoreSnapshot;
/** @emoji 🧾️ Plain DTO aliases for React/schema bridges (same as `*Dto` types). */
...
// NOTE 🚧️ The on-disk kit-store bundle format (see `assets/compose/metabolism.new.kit.compose.json`)
...
/** @emoji 🧾️ Folder persistence adapter (Electron passes two path segments for `createDirectory`). */
...
/** @emoji 🧭️ Marker interface for host stores that want the kit client to push the rs-produced bundle bytes
...
export interface KitBundlePersisting {
  /** @emoji 📥️ Bytes the host read off the backing file at mount time (empty string if the file is new / empty). */
  readonly initialBundleJson: string;
  /** @emoji 📤️ Atomically write the rs-produced bundle JSON to the backing file/folder. */
...
export class JsonFileKitStore implements KitHostStore, KitBundlePersisting {
  ...
  /** @emoji 📥️ Snapshot of the file at mount time (rs uses it for `kitStoreBundleHydrate`). */
  readonly initialBundleJson: string;
  private constructor(
  ...
  static async create(adapter: KitJsonFileAdapter) {
    // 🚧️ Bundle format is Rust-owned. We read the file once to capture the bytes for rs hydration,
  ...
  replace(kit: Kit) {
    // 🚧️ No JS-side persistence: bundle writing happens through `persistBundle(rs-json)` driven by the kit client subscription.
    this._kit = kit;
...
export class FolderKitStore implements KitHostStore, KitBundlePersisting {
  ...
  static async create(adapter: KitFolderAdapter, initial?: KitFullDto) {
    // 🚧️ Bundle format is Rust-owned. Folder host reads the canonical kit bytes once to feed rs hydration;
...
/** @emoji 🧭️ True when the host store wants the kit client to drive bundle persistence (read at mount, write after each change). */
...
/** @emoji 🧭️ Placeholder session store: in-memory until hub sync is host-dtod. */
...
// @emoji 🧾️ Transport-side kit file URLs, object URLs, and flattened kit ports (no domain diffs; mirrors kit JSON shape).

/**
 * @emoji 🧾️ Upload/download surface used by `getKitFileProvider` / sketchpad `FileProvider` (aligned names, not re-exporting sketchpad).
 */
...
/**
 * @emoji 🧾️ Factory resolved once per opened kit; sketchpad sets this on `KitFileState`.
 */
export type KitFileProviderFactory = (kitId: string) => Promise<KitFileProvider>;

/**
 * @emoji 🧾️ Per-`KitHostStore` blob/object URL and provider resolution cache (host-only; not serialized in kit).
 */
...
/** @emoji 🧾️ Lazily created host cache keyed by the live `KitHostStore` (same identity as open kit). */
...
/** @emoji 🧾️ Async resolve + cache; warms {@link getExistingKitFileProvider} after first await. */
...
/** @emoji 🧾️ Synchronous best-effort provider (after at least one {@link getKitFileProvider} call for this store). */
...
/** @emoji 🧾️ Relative path segment for sidecar / provider I/O (matches sketchpad memory layout `kitId/fileId/path`). */
...
/** @emoji 🧾️ Prefer in-memory object URL, then embedded data/file URL fields. */
...
/**
 * @emoji 🧾️ Merged file-id → best readable URL for UI maps (`useKitStoredFileUrls`).
 */
...
/** @emoji 🧾️ Registers a `blob:` URL in {@link KitFileState.objectUrls} (revokes prior for same `fileId`). */
...
/**
 * @emoji 🧾️ All ports defined on families (read-only helper for schema/UI).
 */
...
/** @emoji 🧾️ Recursively turns `{ items: [...] }` / Relay `{ edges: [{ node }] }` containers into plain JSON arrays for Zod DTO parsing. */
...
/** @emoji 🧾️ Lifts `*.kit.compose.json` envelope (`initialKit` / `wip.initialKit`) and flattens bundle `items` lists to {@link KitFullDto}. */
...
/** @emoji 🧾️ Decode UTF-8 kit JSON bytes (flat or `*.kit.compose.json` envelope) to {@link KitFullDto}. */
...
/** @emoji 🧾️ Decode kit bytes as a flat `KitFullDto` (accepts plain kit JSON or compose bundle envelope with `initialKit` / relay `items`). */
...
/** @emoji 🧭️ Arbitrary kit entity handle: patch fields and subscribe to rs {@link KitEvent} stream. */
...
/** @emoji 🧭️ Per-design kit handle: GraphQL reads and semantic design mutations on {@link KitStore}. */
export class DesignStore {
  ...
  /** @emoji 🧾️ Full design DTO from a kit snapshot (rs materialized truth). */
  ...
  /** @emoji 🧾️ Live design graph reads routed like {@link LiveDesign}. */
  ...
  /** @emoji 🧾️ Per-piece document + flat pose metadata (`getPiecesMetadata` / `PieceStore` GraphQL). */
  ...
  /** @emoji 🧾️ Full piece DTO rows for this design (`getPieces`). */
  readPiecesFullRows(): Promise<readonly PieceDto[]> {
    return this.root.getPieces(this.readPoint, this.id);
  }

  /** @emoji 🧾️ Full connection DTO rows for this design (`getConnections`). */
...
/** @emoji 🧾️ GraphQL `TypeMetadataObject` uses JSON `null` for absent fields; strip those before Zod DTO parse. */
...
/** @emoji 🧭️ Per-kind kit handle (compose domain kind, not TS typeof). */
export class TypeStore {
  ...
  /** @emoji 🧾️ Best representation for tag ids (`readTypeBestRepresentationCommand`). */
  readBestRepresentation(tagIds: readonly string[]): Promise<RepresentationDto | null> {
...
/** @emoji 🧭️ Piece scoped to one design id plus piece id. */
export class PieceStore {
  ...
  /** @emoji 🧾️ Flattened placement plane in world space (`readPieceFlatPlaneCommand`). */
  readFlatPlane(): Promise<PlaneDto | null> {
    return new LivePiece(this.root, this.readPoint, this.designId, this.id).readFlatPlane();
  }

  /** @emoji 🧾️ Flattened placement center (`readPieceFlatCenterCommand`). */
  readFlatCenter(): Promise<CoordinateDto | null> {
    return new LivePiece(this.root, this.readPoint, this.designId, this.id).readFlatCenter();
  }

  /** @emoji 🧾️ Parent connection row when connected (`readPieceParentConnectionFullCommand`). */
  readParentConnectionFull(): Promise<ConnectionDto | null> {
...
/** @emoji 🧭️ Connection scoped to one design id plus connection id. */
...
/** @emoji 🧭️ Kit family row (ports live under families in rs). */
...
// #endregion 🧩️KitEntitiesMerged

// #endregion 🧩️KitWasmBridgeMerged
...
if (typeof process !== "undefined" && !!process.env && process.env["COMPOSE_JS_RUN_EMBEDDED_TESTS"] === "1") {
  ...
  describe("compose-js KitStore", () => {
    ...
    describe("wasm GraphQL integration · KitStore.read / vcs", () => {
      ...
      it("serializes a fresh dev-json bundle with three graphs, a seed checkpoint, and an open unsaved change", async () => {
        ...
        expect(bundle["schema"]).toBe("🎆️26🌙️06⬆️1");
  ...
  describe("compose kit-store fixtures (US-001)", () => {
    ...
    it("metabolism.new kit bundle has metabolism on-disk shape (Rust-owned)", async () => {
      // 🚧️ The on-disk bundle format is owned by `compose/rs` — JS only verifies the metabolism shape exists on the asset.
      const { readFileSync } = await import("node:fs");
      const { resolve, dirname } = await import("node:path");
      const { fileURLToPath } = await import("node:url");
      const here = dirname(fileURLToPath(import.meta.url));
