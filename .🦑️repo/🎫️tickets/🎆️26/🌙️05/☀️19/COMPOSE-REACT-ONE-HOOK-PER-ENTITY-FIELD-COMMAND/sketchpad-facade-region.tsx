...
export { COMPOSE_IN_MEMORY_KIT_URI, kitStoreFromKitStoreClient } from "../../js";

/** @emoji 📏️ Sketchpad geometric tolerance constant. */
...
/** @emoji 📌️ Tab-scoped kit id for sketchpad shell / scene bridge. */
...
/** @emoji 📌️ Active sketchpad kit tab id (`{ id }`) or `null` outside {@link ActiveKitTabContextProvider}. */
export function useActiveKitTab(): ActiveKitTabScope | null {
  const id = React.useContext(ActiveKitTabContext);
  return id == null || id === "" ? null : { id };
}

/** @emoji 📌️ Whether {@link ActiveKitTabContextProvider} is mounted. */
...
/** @emoji 🌱️ Sketchpad alternative picker scope (WIP: empty until graph alternatives wire through). */
...
/** @emoji 🌱️ Selected alternative id + setter for host dropdowns. */
...
/** @emoji 🌱️ Alternatives list for host dropdowns. */
...
/** @emoji 🧠️ In-memory kit host for sketchpad temporary kits and imports. */
...
/** @emoji 🧾️ Legacy string-command entry for DTO host stores; prefer {@link useKitCommand} on GraphQL kits. */
export async function executeComposeKitCommand(store: KitHostStore, command: string, _origin: string, ...args: unknown[]): Promise<unknown> {
  if (command === "compose.kit.undo" || command === "compose.kit.redo") {
...
/** @emoji 🧾️ Memoized kit string-command engine for legacy sketchpad callers. */
...
/** @emoji 📣️ Subscribe to registry open/close list changes. */
...
/** @emoji 🧾️ Registry bridge for {@link SketchpadStore} and other non-hook callers. */
export function getKitRegistryBridge(): KitRegistryValue | null {
  return _composeKitRegistryBridge;
}

/** @emoji 📦️ Host registry for open kit tabs (sketchpad). */
...
/** @emoji 📦️ Sketchpad host root: {@link KitRegistryProvider}. */
...
/** @emoji 📦️ Like {@link useKitRegistry} but returns null outside provider. */
export function useKitRegistrySafe(): KitRegistryValue | null {
  return React.useContext(KitRegistryContext);
}
//#endregion 📦️KitRegistry

//#region 🔌️KitRuntimeBridge
...
/** @emoji 📌️ Live kit host snapshot for registry tab or {@link KitRuntimeContext}. */
...
/** @emoji 📥️ Active kit host store for the resolved tab / runtime scope. */
...
/** @emoji 📂️ Open kit ids from {@link KitRegistryProvider}. */
...
/** @emoji 📌️ Kit `files` rows for sketchpad panels (`const [files] = useFilesFull()`). */
...
/** @emoji 📌️ Kit `tags` rows for sketchpad panels (`const [tags] = useTagsFull()`). */
...
/** @emoji 📌️ Included design ids for explode / replace UI (WIP: empty without kit-store client). */
...
/** @emoji 🧭️ Maps entity id prefix to schema scope kind for sketchpad property panels. */
...
/** @emoji 🔗️ Attach backbone via {@link useStoreCommand} (alias for sketchpad docs). */
...
//#endregion 🔌️KitRuntimeBridge
// #endregion 🎨️SketchpadFacade
