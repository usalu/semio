...
/// @emoji ­ƒº▒️ `entity_bare!` ÔÇö splices `item` tokens (structs, impls, consts) with **no** relay shells; use for roots and `LocalProvider`-class bare nodes.
#[macro_export]
...
//#region ÔÅ▒️´©Å timestamp

pub mod timestamp {
    //! ÔÅ▒️´©Å ISO-8601 millisecond-precision timestamp scalar.
    use async_graphql::{InputValueError, InputValueResult, Scalar, ScalarType, Value};
...
//#region ­ƒ®╣️ schema_gap_surfaces

pub mod schema_gap_surfaces {
    //! ­ƒ®╣️ SDL-only synthetic relay surfaces for long-tail golden declarations; registered into `Schema::sdl()` so the exported schema reaches the current target declaration set.
...
//#endregion ­ƒ®╣️ schema_gap_surfaces
...
pub mod meta {
    ...
    impl Attribute {
        /// @emoji ­ƒî┐️ Blake3 leaf over persisted attribute columns (no owner weak refs).
        pub fn compute_entity_hash(&self) -> String {
...
pub mod hash {
    ...
    /// @emoji ­ƒî│️ Merkle fold: concatenates `own` in order, then **sorted** `children` hex digests (order-independent set hashing).
...
pub mod kit {
    ...
    pub mod design {
        ...
        //#region ­ƒº▒️ clump
        /// @emoji ­ƒº▒️ SDL `Clump` ÔÇö connected-component bucket for layout (`WeakEntity` projection hook).
        pub struct Clump {
            pub id: Id,
...
pub mod vcs {
    ...
    impl Checkpoint {
        ...
        /// @emoji ­ƒî▒️ SDL `Checkpoint.initial` ÔÇö graph-level [`Graph::initial_kit`] baseline (saved once per graph, not duplicated on each checkpoint).
    ...
    impl Graph {
        ...
        /// @emoji ­ƒî▒️ Ensure a seed [`Checkpoint`] exists and [`TheKit`](../../../schema/graphql/schema.golden.graphql) is anchored (idempotent).
        ...
        /// @emoji ­ƒî▒️ Fork a new named [`Alternative`](../../../schema/graphql/schema.golden.graphql) from [`TheKit`](../../../schema/graphql/schema.golden.graphql) or another alternative's tip [`Checkpoint`](../../../schema/graphql/schema.golden.graphql).
        ...
        /// ­ƒø░️´©Å WIP bootstrap: hydrate [`Graph::mutable_kit`] from initial kit projection JSON via [`crate::kit_backbone::graph_new_overlay_from_initial_projection_json`].
...
pub mod operation {
    ...
    impl BackboneKind {
        /// @emoji ­ƒù║️´©Å Parse `dev://`, `local://`, `remote://`, or legacy `file://` (dev file path).
...
pub mod kit_backbone {
    ...
    /// @emoji ­ƒî┐️ Alternative version entity; each alternative owns its own version-scoped changes.
    #[derive(Clone, Debug, serde::Deserialize, serde::Serialize)]
    ...
    /// @emoji ­ƒî▒️ Empty `initialKit` JSON snapshot for fresh dev backbones until a full metabolism-shaped [`Kit`] snapshot is persisted.
    ...
    impl DevBackboneGraphHead {
        /// @emoji ­ƒî▒️ Empty graph snapshot stamped with `kit_id` (used for fresh `wip` / `authoritative` / `stage` heads).
    ...
    impl DevBackboneBundleDoc {
        /// @emoji ­ƒî▒️ Fresh empty bundle stamped with [`KIT_STORE_BUNDLE_SCHEMA`]; kit ids fill in once the live kit projects into `initialKit`.
        ...
        /// @emoji ­ƒº╣️ Drop [`blobs`] entities whose digest is not referenced by any `files[].blobHash` on `wip` / `authoritative` / `stage` `initialKit` snapshots.
        ...
        /// @emoji ­ƒ®╗️ Hydrate the live graph from a previously-persisted bundle JSON and keep version changes available for  replay.
        ...
        /// @emoji ­ƒî▒️ Initialize an empty bundle with a non-empty `wip` head: empty `initialKit` projection stamped with `kit_id`,
...
pub mod worker {
    ...
    /// ­ƒø░️´©Å Parent runtime hosting the GraphQL schema + routing logic.
    ...
    impl ParentStore {
        /// ­ƒø░️´©Å Spawn parent + two child runtimes (in-process on native).
        ...
        /// @emoji ­ƒø░️´©Å `POST /install` bootstrap: full `DevBackboneBundleDoc` JSON (schema `KIT_STORE_BUNDLE_SCHEMA`) or bare `initialKit` projection (same contract as wasm `bootstrap_runtime_from_json_value`).
        ...
        /// @emoji ÔÅ│️ Enqueues on wip and waits for `CommandSucceeded` / `OperationFailed` on the bus.
...
pub mod gql {
    ...
    pub mod interfaces {
        ...
        /// @emoji ­ƒº▒️ SDL `Modifications` ÔÇö aggregate removed / live / added modifications (minimal projection from live graph where wired).
        #[derive(Clone, Debug, Default)]
        ...
        //#region ­ƒº▒️ golden_interface_minimals
        ...
        //#endregion ­ƒº▒️ golden_interface_minimals
    ...
    /// @emoji ­ƒø░️´©Å Golden `interface Provider` ÔÇö concrete [`LocalProvider`] / [`RemoteProvider`].
    #[derive(Clone, async_graphql::Interface)]
    #[graphql(
        name = "Provider",
        field(name = "id", ty = "crate::id::Id"),
...
pub mod wasm_bridge {
    ...
    /// ­ƒø░️´©Å Wasm entry shim (workers may call explicitly); runtime is rooted by [`KitStoreHandle`].
    ...
    /// ­ƒø░️´©Å Boot the parent runtime inside the current (parent) web worker ÔÇö no-op host hook until iframe wiring lands.
...
mod tests {
    ...
    /// @emoji ­ƒº▒️ `entity_bare!` / `operation_with_input!` / `operation_no_input!` expand to real `item` tokens (compile-time splice, not `$( )* => {}` stubs).
    #[test]
    ...
    /// @emoji ­ƒî▒️ Opens an unsaved kit change via golden `Mutation.session.store.theKit.startNewChange` (see `schema.golden.graphql` Commands block).
    ...
    /// @emoji ­ƒî▒️ GraphQL tests open a target-schema unsaved change; the internal draft anchor stays hidden.
    ...
    fn kit_store_bundle_initialize_with_unsaved_change_seeds_checkpoint_and_change() {
        // ­ƒî▒️ Bundle bootstrap matches sketchpad "create dev kit (json)": one `initialKit`, one checkpoint, one unsaved change on the version.
