...
// #endregion ­ƒº▓️Header

// #region ­ƒôÑImports
import { Line, OrbitControls, Text } from "@react-three/drei";
import { Canvas, useFrame, useThree, type ThreeEvent } from "@react-three/fiber";
import { Suspense, useCallback, useEffect, useMemo, useRef, useState, useSyncExternalStore, type CSSProperties, type KeyboardEvent, type ReactNode } from "react";
...
export function useTessellation(kernel: SpatialKernel | null, cell: ReturnType<typeof cellRef> | null, tolerance: number): MeshTransfer | null {
...
export function useDocumentMeshes(kernel: SpatialKernel | null, model: Model, tolerance: number): readonly { readonly cell: CellRef; readonly mesh: MeshTransfer }[] {
...
/** @emoji ­ƒû╝️´©Å Maps declarative `previewKind` + params to a point transform for geometry wireframes. */
...
/** @emoji ­ƒû╝️´©Å Active geometry point transform from move/copy/mirror/rotate/scale preview display items. */
...
export const SPATIAL_PICK_TARGET_KINDS: readonly SpatialPickTargetKind[] = ["object", "objectFace", "objectEdge", "objectVertex"];
...
function spatialPickKindsForSelectionAccept(accept: readonly ModelEntityKind[]): ReadonlySet<SpatialPickTargetKind> | null {
  ...
  for (const kind of accept) {
    if (kind === "object" || kind === "objectFace" || kind === "objectEdge" || kind === "objectVertex") {
      out.add(kind);
      continue;
    }
...
function kernelGeometryKindForObjectPick(kind: SpatialObjectPickTargetKind, geometryKind?: ModelEntityKind): ModelEntityKind {
...
export function filterSpatialPickTargetsForActiveView(targets: readonly SpatialPickTarget[], activeViewId: string | null): SpatialPickTarget[] {
...
/** @emoji ­ƒº▓️ Normalizes `ModelJson` array buckets to the record shape used by interaction math. */
...
function geometryCellPoints(vertices: Record<string, VertexRecord>, edges: Record<string, EdgeRecord>, wires: Record<string, WireRecord>, faces: Record<string, FaceRecord>, shells: Record<string, ShellRecord>, cell: CellRecord): readonly Vec3[] {
  return uniqueGeometryPoints(cell.shellIds.flatMap((id) => (shells[id] ? geometryShellPoints(vertices, edges, wires, faces, shells[id]!) : [])));
}
...
function geometryCellComplexPoints(
  ...
  return uniqueGeometryPoints(complex.cellIds.flatMap((id) => (cells[id] ? geometryCellPoints(vertices, edges, wires, faces, shells, cells[id]!) : [])));
}
...
function geometryEntityPoints(buckets: ReturnType<typeof geometryBuckets>, kind: ModelEntityKind, id: string): readonly Vec3[] {
  ...
  if (kind === "cellComplex" && buckets.cellComplexes[id]) {
    return geometryCellComplexPoints(buckets.vertices, buckets.edges, buckets.wires, buckets.faces, buckets.shells, buckets.cells, buckets.cellComplexes[id]!);
  }
  return [];
}
...
function createViewObjectSpatialPickTargets(views: ExtensionViewService, model: Model, activeViewId: string): readonly SpatialPickTarget[] {
...
/** @emoji ­ƒº▓️ Builds renderer-side snap/select targets from factory geometry and optional extension view objects. */
export function createSpatialPickTargets(geometry: SpatialPickGeometry | null | undefined, views?: ExtensionViewService | null, activeViewId?: string | null): readonly SpatialPickTarget[] {
  if (!geometry) return [];
  const buckets = geometryBuckets(geometry);
...
export function filterSpatialPickTargets(targets: readonly SpatialPickTarget[], accept: readonly ModelEntityKind[] = [], toggles: SpatialPickKindToggles = {}): SpatialPickTarget[] {
  ...
  return targets.filter((target) => toggles[target.kind] !== false && (!acceptKinds || acceptKinds.has(target.kind)));
}

/** @emoji ­ƒº▓️ Creates a statechart event carrying snapped point plus selected geometry metadata. */
export function createSpatialPickEvent(kind: SpatialPickKind, point: Vec3, target: SpatialPickTarget | null, modifiers: InteractionEvent["modifiers"] = {}): InteractionEvent {
  const geometryKind = target?.kind === "object" && !target.geometryKind ? "object" : target ? kernelGeometryKindForObjectPick(target.kind as SpatialObjectPickTargetKind, target.geometryKind) : undefined;
...
// #endregion ­ƒº▓️GeometryTargets

// #region ­ƒû╝️´©ÅDisplayPrimitives
...
/** @emoji ­ƒû╝️´©Å Host hook that renders one resolved `DisplayItem` inside `<InteractionDisplay>`. */
...
/** @emoji ­ƒû╝️´©Å Registers a custom display kind; returns unregister. Libraries extend without forking the package. */
...
/** @emoji ­ƒû╝️´©Å Looks up a host-registered display kind renderer. */
...
/** @emoji ­ƒû╝️´©Å Maps `DisplayModel.items` to R3F nodes (must live under `<Canvas>`). */
...
// #endregion ­ƒû╝️´©ÅDisplayPrimitives

// #region ­ƒû▒️´©ÅInteraction
...
/** @emoji ­ƒû▒️´©Å Ground hit-test on the **XY** working plane at fixed world **Z** (= spatial footprint plane; factory height is world Z). */
...
/** @emoji ­ƒû▒️´©Å YZ wall at the second corner so `pointer.move` changes world Z (factory height uses |╬️öz|). */
...
/** @emoji ­ƒû▒️´©Å Z-aligned rod at `origin` so `pointer.move` drives peak height without XY drift. */
...
// #endregion ­ƒû▒️´©ÅInteraction

// #region ­ƒº▓️GeometryInteraction
...
/** @emoji ­ƒû▒️´©Å Returns the closest pick target eligible for hover highlighting along a ray. */
...
/** @emoji ­ƒº▓️ Renders optional factory geometry as pickable snap/select targets. */
export function SpatialPickGeometryLayer({
  ...
  const targets = useMemo(() => createSpatialPickTargets(geometry, views, activeViewId), [geometry, topoRevision, views, viewsRevision, activeViewId]);
...
// #endregion ­ƒº▓️GeometryInteraction
...
export function TessellatedCommitMesh({ mesh: data, pickable = false, showFaces = true, showEdges = true, onFacePointerMove, onFacePointerDown }: TessellatedCommitMeshProps): ReactNode {
  const geometry = useMemo(() => buildBufferGeometryFromMeshTransfer(data), [data.position, data.normal, data.index, data.faceGroups]);
  useEffect(() => () => geometry.dispose(), [geometry]);
...
export function CommittedMeshLayer({
  ...
  return (
    <group>
      {meshes.map((row, i) => (
        <TessellatedCommitMesh key={`${row.cell}:${meshTransferContentKey(row.mesh, i)}`} mesh={row.mesh} pickable={pickable} showFaces={showFaces} showEdges={showEdges} onFacePointerMove={onFacePointerMove} onFacePointerDown={onFacePointerDown} />
      ))}
    </group>
  );
}
...
/** @emoji ­ƒø░️´©Å Frames the camera to fit committed meshes and/or factory geometry (playground auto-fit). */
export function SpatialAutoFit({
  ...
  const bounds = useMemo(() => mergeSpatialSceneBounds(boundsFromMeshTransfers(meshes), boundsFromSpatialPickGeometry(geometry)), [meshes, geometry, geometryRevision]);
...
/** @emoji ­ƒø░️´©Å Orbit controls that repaint on demand and never block R3F pointer routing. */
...
export interface InteractionSpatialViewProps {
  ...
  /** @emoji ­ƒû▒️´©Å `pointer.move` hits ground (XY at fixed Z); height slab passes full 3D. */
  ...
  readonly views?: ExtensionViewService | null;
  readonly viewsRevision?: number;
  /** @emoji ­ƒû╝️´©Å When set, drives `InteractionDisplay` instead of `snapshot.display` (e.g. merged archived footprints). */
  ...
  /** @emoji ­ƒû▒️´©Å Hover raycast kind filter; defaults to `selectionKindToggles` when omitted. */
  ...
  /** @emoji ­ƒº▓️ When false, skips pick-target meshes (during active interaction sessions). */
...
/** @emoji ­ƒû▒️´©Å Ground-plane picking is command input and must stay independent from host geometry selection. */
...
export function InteractionSpatialView({
  ...
  const geometryPreviewTransform = useMemo(() => geometryPreviewTransformFromDisplay(displayModel ?? snapshot.display), [displayModel, snapshot.display]);
  ...
  const geometryRevision = geometry && typeof geometry === "object" && "revision" in geometry ? Number((geometry as { revision?: unknown }).revision) : 0;
  const sceneVisibility = useMemo(() => resolveSpatialSceneVisibility(activeViewId, filterKindToggles), [activeViewId, filterKindToggles]);
  return (
    <>
      {slots?.beforeScene}
      <InvalidateOnRevision revision={`${snapshot.revision}:${viewsRevision}:${geometryRevision}:${hoveredTargetKey ?? ""}`} />
      <SpatialInvalidator />
...
export function replPruneSelectionByKind(selection: readonly SelectionTarget[], pickViewKind: SpatialPickViewKind, kind: SpatialPickTargetKind): SelectionTarget[] {
  if (!spatialPickViewKindSet(pickViewKind).has(kind)) return [...selection];
  const geometryKind = kind === "objectVertex" ? "vertex" : kind === "objectEdge" ? "edge" : kind === "objectFace" ? "face" : kind === "object" ? "cell" : kind;
...
export function replPruneSelectionAnalytic(selection: readonly SelectionTarget[], pickTargets: readonly SpatialPickTarget[], analyticToggles: SpatialAnalyticToggles): SelectionTarget[] {
...
export function replFinalizeSelection(rendererSelection: readonly SelectionTarget[], result: InteractionSnapshot["lastResponse"]): readonly SelectionTarget[] {
...
export interface InteractionReplLayoutProps {
  ...
  /** @emoji ­ƒû╝️´©Å Spread after REPL wiring; overrides win (use for theme/slots/face handlers, not session pick state). */
...
export function InteractionRepl({
  ...
  const allArchivedBoxLayouts = useMemo(() => [...documentArchivedBoxLayouts, ...archivedBoxLayouts], [documentArchivedBoxLayouts, archivedBoxLayouts]);
  const baseDisplay = useMemo(() => replBaseDisplayForHistory(snapshot), [snapshot]);
  const mergedDisplay = useMemo(() => mergeDisplayWithArchivedBoxes(baseDisplay, allArchivedBoxLayouts), [baseDisplay, allArchivedBoxLayouts]);
  ...
  const [analyticSelectionToggles, setAnalyticSelectionToggles] = useHostState(analyticSelectionTogglesProp, onAnalyticSelectionTogglesChange, () => chromeDefaults.analyticSelectionToggles);
  ...
  const [rendererSelection, setRendererSelection] = useHostState(rendererSelectionProp, onRendererSelectionChange, () => [...chromeDefaults.rendererSelection]);
  const [interactionSelection, setInteractionSelection] = useHostState(interactionSelectionProp, onInteractionSelectionChange, () => [...chromeDefaults.interactionSelection]);
  ...
  const displayedSelectionTargets = useMemo(() => replDisplayedSelectionTargets(interactionActive, pickViewKind, rendererSelection, interactionSelection), [interactionActive, pickViewKind, rendererSelection, interactionSelection]);
  const viewFilterKindToggles = useMemo(() => spatialPickKindTogglesForView(filterKindToggles, pickViewKind), [filterKindToggles, pickViewKind]);
  const viewSelectionKindToggles = useMemo(() => intersectSpatialPickKindToggles(viewFilterKindToggles, spatialPickKindTogglesForView(selectionKindToggles, pickViewKind)), [selectionKindToggles, pickViewKind, viewFilterKindToggles]);
  const viewAnalyticFilterToggles = useMemo(() => spatialAnalyticTogglesForView(analyticFilterToggles, pickViewKind), [analyticFilterToggles, pickViewKind]);
  const viewAnalyticSelectionToggles = useMemo(
    () => intersectSpatialAnalyticToggles(viewAnalyticFilterToggles, spatialAnalyticTogglesForView(analyticSelectionToggles, pickViewKind)),
  ...
  const pickGeometryRevision = geometry && typeof geometry === "object" && "revision" in geometry ? Number((geometry as { revision?: unknown }).revision) : 0;
  ...
  const activeSelectionAccept = useMemo(() => (runtimeSelectionAccept.length > 0 ? runtimeSelectionAccept : interactionActive ? [] : SPATIAL_PICK_TARGET_KINDS), [runtimeSelectionAccept, interactionActive]);
  ...
  const dispatchSelectionTargets = useCallback(
    (targets: readonly SpatialPickTarget[], modifiers: InteractionEvent["modifiers"] = {}, point?: Vec3) => {
      ...
      const nextSelection = replMergeSelectionPickInView(interactionActive, pickViewKind, rendererSelection, interactionSelection, picked, modifiers);
  ...
  useEffect(() => {
    ...
    const onMove = (event: PointerEvent) => {
      ...
      const hits = spatialPickTargetsFromClientPoint({ x: event.clientX, y: event.clientY }, camera, rect, viewPickTargets, [], viewSelectionKindToggles, viewAnalyticSelectionToggles);
  ...
  const onSpatialInteractionEvent = useCallback(
    (ev: InteractionEvent) => {
      ...
      if (ev.kind === "pointer.down") {
        ...
        if (snapEv && activeSelectionAccept.length > 0 && activeSelectionAccept.includes(snapEv.kind as ModelEntityKind) && viewSelectionKindToggles[snapEv.kind as SpatialPickTargetKind] !== false) {
          ...
          commitSelection(replMergeSelectionPickInView(interactionActive, pickViewKind, rendererSelection, interactionSelection, [selection], modifiers));
    ...
    [rt, activeSelectionAccept, commitSelection, interactionActive, interactionSelection, rendererSelection, pointerMoveActive, pickViewKind, viewSelectionKindToggles, viewAnalyticSelectionToggles, onInteractionEventProp],
  );
  ...
  useEffect(() => {
    ...
    const beginDragSelection = (event: PointerEvent) => {
      ...
      const finishSelection = (upEvent: PointerEvent) => {
        ...
        if (distance < 4) {
          const candidates = spatialPickTargetsFromClientPoint(finalState.currentClient, camera, rect, viewPickTargets, activeSelectionAccept, viewSelectionKindToggles, viewAnalyticSelectionToggles);
        ...
        if (targets.length === 0) {
          if (spatialSelectionModeFromModifiers(finalState.modifiers as { readonly alt?: boolean; readonly ctrl?: boolean; readonly meta?: boolean; readonly shift?: boolean }) === "default") {
            commitSelection(replMergeSelectionPickInView(interactionActive, pickViewKind, rendererSelection, interactionSelection, [], finalState.modifiers));
          }
          return;
        }
  ...
  const completionSuffix = useMemo(() => replActiveCompletionSuffix(cmdLine, filtered, activeIndex), [cmdLine, filtered, activeIndex]);
            ...
            {selectionMenu.targets.map((target) => {
              ...
              return (
                <button
                  key={key}
                  type="button"
                  onPointerEnter={() => setHoveredPickKey(viewSelectionKindToggles[target.kind] !== false && spatialPickTargetMatchesAnalyticToggles(target, viewAnalyticSelectionToggles) ? key : null)}
                ...
                >
                  <span
                    style={{
                      ...
                      marginRight: 6,
                      background:
                        target.kind === "surface" && target.exposure && target.stance ? surfaceSemanticStyle(target.exposure, target.stance).color : target.kind === "part" && target.overlap ? partSemanticStyle(target.overlap).color : "#888",
                    }}
                  />
                  <span style={{ opacity: 0.7 }}>{target.kind}</span> {target.exposure ? <span style={{ opacity: 0.65 }}>{target.exposure}</span> : null}
                  {target.stance ? <span style={{ opacity: 0.65 }}>┬️À{target.stance}</span> : null}
                  {target.overlap ? <span style={{ opacity: 0.65 }}>{target.overlap}</span> : null} <code style={{ color: "#ffffff" }}>{target.id}</code>
                </button>
              );
            })}
          </div>
      ...
      {showAside ? (
        ...
        >
            ...
            {analyticSummary ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 4, maxHeight: 160, overflowY: "auto" }}>
                <span style={{ opacity: 0.75 }}>
                  {analyticSummary.surfaces.length} surfaces ┬️À {analyticSummary.parts.length} parts
                </span>
                {analyticSummary.parts.map((part) => (
                  <span key={String(part.id)} style={{ fontSize: 11, opacity: 0.85 }}>
                    {part.overlap} ┬️À {String(part.id)}
                  </span>
                ))}
              </div>
            ) : null}
          ...
          <div style={{ fontSize: 12, opacity: 0.85 }}>
            {interactionId ? (
              <>
                Interaction <code>{interactionId}</code> ┬️À state <code>{snapshot.state}</code> ┬️À rev {snapshot.revision}
              </>
            ) : (
              <>
                No interaction selected ┬️À state <code>{snapshot.state}</code> ┬️À rev {snapshot.revision}
              </>
            )}
          </div>
...
/** @emoji ­ƒû╝️´©Å Canvas-only {@link InteractionRepl} (no built-in aside); full host props and `on*` callbacks. */
...
if (import.meta.vitest) {
  ...
  describe("@spatial/js-renderer-r3f interaction adapter", () => {
    ...
    it("creates selectable targets for every editable geometry kind", () => {
      ...
      expect(targets.map((target) => target.kind)).toEqual(["objectVertex", "objectVertex", "objectVertex", "objectEdge", "objectEdge", "objectEdge", "objectFace", "object"]);
    ...
    it("play geometry punch through host exposes one difference pick target per cell", async () => {
      ...
      await derived.refresh(model);
      const partTargets = filterSpatialPickTargetsByView(createSpatialPickTargets(model, derived), "analytic").filter((t) => t.kind === "part");
    ...
    it("partitions raw and analytic pick targets", () => {
      ...
      expect(filterSpatialPickTargetsByView(targets, "raw").map(spatialPickTargetKey)).toEqual(["objectVertex:v0", "objectFace:f0"]);
      expect(filterSpatialPickTargetsByView(targets, "analytic").map(spatialPickTargetKey)).toEqual(["surface:surface-f0", "part:part-c0"]);
    });
    ...
    it("filters selectable targets by active accept list and kind toggles", () => {
      ...
      expect(filterSpatialPickTargets(targets, ["vertex", "edge"], { objectEdge: false }).map(spatialPickTargetKey)).toEqual(["objectVertex:v0"]);
    });
    ...
    it("replHostGeometryPickingEnabled stays on for play REPL and committed palette sessions", () => {
      ...
      expect(replHostGeometryPickingEnabled("box", { ...playSpec, machine: { initial: "done", states: [{ name: "done", final: true }] } }, "done")).toBe(true);
    });
    ...
    it("effective pick toggles require both visibility and selection toggles", () => {
      ...
      expect(filterSpatialPickTargetsForVisibility(targets, { objectVertex: true, objectEdge: false, objectFace: true }).map(spatialPickTargetKey)).toEqual(["objectVertex:v0", "objectFace:f0"]);
      expect(filterSpatialPickTargets(targets, [], { objectVertex: false, objectEdge: true, objectFace: false }).map(spatialPickTargetKey)).toEqual(["objectEdge:e0"]);
      expect(intersectSpatialPickKindToggles({ objectEdge: false, objectFace: true }, { objectEdge: true, objectFace: false })).toEqual({
        objectEdge: false,
        objectFace: false,
      });
      expect(filterSpatialPickTargets(targets, [], intersectSpatialPickKindToggles({ objectEdge: false, objectFace: true }, { objectEdge: true, objectFace: true })).map(spatialPickTargetKey)).toEqual(["objectVertex:v0", "objectFace:f0"]);
    });

    it("effective analytic pick toggles require both visibility and selection analytic filters", () => {
      expect(intersectSpatialAnalyticToggles({ exposure: { external: false, internal: true }, stance: { vertical: true, horizontal: true } }, { exposure: { external: true, internal: true }, stance: { vertical: false, horizontal: true } })).toEqual({
        exposure: { external: false },
        stance: { vertical: false },
      });
    });
    ...
    it("renders pinned selection-menu targets even when filter visibility hides them", () => {
      ...
      expect(resolveSpatialPickTargetsToRender(targets, { objectVertex: true, objectFace: false, surface: false }, { exposure: { internal: false }, stance: { vertical: true } }, new Set(["face:f0", "surface:s0"])).map(spatialPickTargetKey)).toEqual([
        "objectVertex:v0",
        "objectFace:f0",
        "surface:s0",
      ]);
    ...
    it("end-to-end host pick uses selection toggles while layer visibility uses filter toggles", async () => {
      const onlyVertices = Object.fromEntries(SPATIAL_PICK_TARGET_KINDS.map((k) => [k, k === "objectVertex"])) as Record<SpatialPickTargetKind, boolean>;
      const onlyFaces = Object.fromEntries(SPATIAL_PICK_TARGET_KINDS.map((k) => [k, k === "objectFace"])) as Record<SpatialPickTargetKind, boolean>;
      const model = new Model();
      ...
      const layerVisible = filterSpatialPickTargetsAnalytic(filterSpatialPickTargetsForVisibility(rawViewTargets, rawFilterToggles), spatialAnalyticTogglesForView({}, "raw"));
      ...
      const ray = new THREE.Ray(new THREE.Vector3(face!.point[0], face!.point[1], face!.point[2] + 4), new THREE.Vector3(0, 0, -1));
      ...
      const onlySurfaces = Object.fromEntries(SPATIAL_PICK_TARGET_KINDS.map((k) => [k, k === "surface"])) as Record<SpatialPickTargetKind, boolean>;
      const analyticFilter = defaultSpatialAnalyticToggles();
      const analyticVisible = filterSpatialPickTargetsAnalytic(filterSpatialPickTargetsForVisibility(analyticViewTargets, spatialPickKindTogglesForView(onlySurfaces, "analytic")), spatialAnalyticTogglesForView(analyticFilter, "analytic"));
      ...
      const analyticRay = new THREE.Ray(new THREE.Vector3(surface.point[0], surface.point[1], surface.point[2] + 4), new THREE.Vector3(0, 0, -1));
      const analyticSelectionToggles = spatialPickKindTogglesForView(onlySurfaces, "analytic");
      const analyticPick = spatialPickTargetsFromRay(analyticRay, analyticViewTargets, ["surface", "part"], analyticSelectionToggles, spatialAnalyticTogglesForView(analyticFilter, "analytic"));
    ...
    it("end-to-end raw and analytic pick view keeps cross-view selection through picks display and clear", async () => {
      ...
      const faceRay = new THREE.Ray(new THREE.Vector3(face!.point[0], face!.point[1], face!.point[2] + 4), new THREE.Vector3(0, 0, -1));
      const surfaceRay = new THREE.Ray(new THREE.Vector3(surface!.point[0], surface!.point[1], surface!.point[2] + 4), new THREE.Vector3(0, 0, -1));
      ...
      const analyticPicked = spatialPickTargetsFromRay(surfaceRay, analyticViewTargets, SPATIAL_ANALYTIC_PICK_TARGET_KINDS, spatialPickKindTogglesForView(onlySurfaces, "analytic"), spatialAnalyticTogglesForView(analyticToggles, "analytic"));
      ...
      const rawPicked = spatialPickTargetsFromRay(faceRay, rawViewTargets, SPATIAL_RAW_PICK_TARGET_KINDS, spatialPickKindTogglesForView(onlyFaces, "raw"), spatialAnalyticTogglesForView(analyticToggles, "raw"));
      ...
      const blockedRaw = spatialPickTargetsFromRay(faceRay, rawViewTargets, SPATIAL_RAW_PICK_TARGET_KINDS, spatialPickKindTogglesForView({ ...kindToggles, objectFace: false }, "raw"), spatialAnalyticTogglesForView(analyticToggles, "raw"));
    ...
    it("ray-picks overlapping face and surface candidates", () => {
      ...
      expect(spatialPickTargetsFromRay(ray, targets, ["face", "surface"], {}).map(spatialPickTargetKey)).toEqual(["objectFace:f0", "surface:surface-f0"]);
    });
    ...
    it("merges picks within the active pick view without clearing the other view", () => {
      ...
      expect(replMergeSelectionPickInView(false, "raw", renderer, [], [{ kind: "wire", id: "w1", editable: true }], {})).toEqual([
        { kind: "surface", id: "s0", editable: false },
    ...
    it("scopes displayed selection to the active pick view", () => {
      ...
      expect(replDisplayedSelectionTargets(false, "analytic", renderer, interaction)).toEqual([{ kind: "surface", id: "s0", editable: false }]);
      expect(replDisplayedSelectionTargets(true, "raw", renderer, interaction)).toEqual([{ kind: "wire", id: "w0", editable: true }]);
      expect(replDisplayedSelectionTargets(true, "analytic", renderer, interaction)).toEqual([{ kind: "part", id: "p0", editable: false }]);
    });
    ...
    describe("selection commands end-to-end", () => {
      it.each(listSelectionOperationInteractionDefs())("$id runs startÔåÆcommit and replFinalizeSelection updates renderer selection", async (defn) => {
        const model = new Model();
        ...
        if (defn.id === "selection.selectVolumes") {
          ...
          const pickView: SpatialPickViewKind = defn.kinds?.some((k) => k === "surface" || k === "part") ? "analytic" : "raw";
    ...
    it("reads interaction selection from machine context targets", () => {
      ...
      expect(replSelectionTargetsEqual([{ kind: "wire", id: "w0", editable: true }], [{ kind: "wire", id: "w0", editable: true }])).toBe(true);
      expect(replSelectionTargetsEqual([{ kind: "wire", id: "w0", editable: true }], [{ kind: "wire", id: "w1", editable: true }])).toBe(false);
    });
    ...
    it("merges host selections according to modifiers", () => {
      ...
      expect(mergeSelectionTargets([...current, ...next], next, spatialSelectionModeFromModifiers({ ctrl: true }))).toEqual(current);
      expect(
    ...
    it("switches drag coverage by initial horizontal direction", () => {
      expect(
        spatialSelectionCoverageFromPath([
          { x: 10, y: 10 },
          { x: 20, y: 12 },
        ]),
      ).toBe("full");
