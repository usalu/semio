...
/**
 * 📐️ Declarative `measure` entries for a window: read-only readouts (`display`, `reading`) or interactive controls; rendered as compact floats on the right.
 **/
...
/**
 * 📐️ Maps declarative `UIWindowMeasure` entries into compact floating tiles aligned to the right edge.
 **/
...
// #endregion 🪟️WindowMeasuresOverlay
...
const UICanvas: React.FC<{
  ...
  /** @emoji 🪟️ Stable registry key so measure/control-only `windowKinds` updates do not destroy Golden Layout. */
  const windowKindRegistryKey = React.useMemo(() => windowKinds.map((wk) => wk.id).join("\0"), [windowKinds]);

  /** @emoji 📐️ Keeps floating measures/controls in sync when `windowKinds` change without tearing down Golden Layout. */
  React.useEffect(() => {
...
/** @emoji 🧰️ Toolbar category ids shared by every App registration surface. */
export type AppToolCategory = "history" | "hand" | "selection" | "lasso" | "filter" | "open" | "create" | "view" | "actions" | "settings";

/** @emoji 📋️ Default toolbar category order (history and hand first when present). */
...
/** @emoji 🔢️ Counts registered toolbar items across all populated categories. */
...
/** @emoji 🔀️ Merges base and extension tool maps per category (extension appends within each category). */
...
/** @emoji 📂️ Lists categories that have at least one non-separator tool. */
