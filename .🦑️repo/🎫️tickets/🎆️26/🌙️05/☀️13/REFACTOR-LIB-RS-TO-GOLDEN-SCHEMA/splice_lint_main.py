...
def main() -> None:
    ...
    c_end = text.index("// 🟩️headerPolicy holds the data fields for a headerPolicy record.", c_start)
...
// 🟨️CheckPoliciesWithContext is a legacy no-op; breachs come from lint scripts and `.repo/cache/breaches`.
...
"""
    ...
    # 3) Remove headerPolicy … composePolicy block (keep #endregion 🧊️Policies)
    start = text.index("// 🟩️headerPolicy holds the data fields for a headerPolicy record.")
    end = text.index("// #endregion 🧊️Policies", start)
    text = text[:start] + "\n" + text[end:]
    ...
    a_end = text.index("// 📬️TicketOpen MUST return a non-nil error", a_start)
