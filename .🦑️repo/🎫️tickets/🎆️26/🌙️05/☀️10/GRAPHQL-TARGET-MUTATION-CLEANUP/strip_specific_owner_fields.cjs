// 🧹️ Removes specific narrow-typed owner/spine fields from `target.schema.graphql`,
//    leaving only the general interface fields (`ownerEntity`, `ownedEntities`).
//    Run from repo root: `node .repo/🎫️/26/05/10/GRAPHQL-TARGET-MUTATION-CLEANUP/strip_specific_owner_fields.js`.
...
// 🚧️ Whitelisted spine-reference field names. Only these are removed; everything else
