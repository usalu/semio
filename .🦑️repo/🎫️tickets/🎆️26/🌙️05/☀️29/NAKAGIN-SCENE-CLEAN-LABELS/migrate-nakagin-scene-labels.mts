#!/usr/bin/env bun
/** @emoji 🧾️ One-off: bake human-readable labels and clean kind ids into nakagin scene fixture (no runtime compose links). */
...
function vortexDisplayLabel(vortex: SceneVortex, objectId: string, handleLabelByKind: Map<string, string>): string {
...
function main(): void {
  ...
  const objects = scene.objects.map((obj) => {
    ...
    const label = pieceLabel && typeLabel && pieceLabel !== typeLabel ? `${typeLabel} · ${pieceLabel}` : pieceLabel || typeLabel || obj.id;
  ...
  console.log(`[migrate-nakagin-scene-labels] wrote ${scenePath} (${objectLabels} object labels, ${vortexLabels} vortex labels, ${idMap.size} kind ids cleaned)`);
}
