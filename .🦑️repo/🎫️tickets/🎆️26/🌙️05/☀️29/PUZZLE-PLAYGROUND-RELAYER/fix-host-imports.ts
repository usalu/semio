...
const boardHostHeader = `/** @emoji 🛝️ Board play React host — entry-only via play/main.ts. */
import {
	Button,
...
h2 = h2.replace(/^\/\*\* @emoji 🛝️ Board play[\s\S]*?\/\/ #region 🛝️PlayHost\n/, "");
writeFileSync(path2d, boardHostHeader + h2);

const sceneHostHeader = `/** @emoji 🛝️ Scene play React host — entry-only via play/main.ts. */
...
h3 = h3.replace(/^\/\*\* @emoji 🛝️ Scene play[\s\S]*?\/\/ #endregion 🧲️Header\n\n/, "");
h3 = h3.replace(/^\/\*\* @emoji 🛝️ Scene play[\s\S]*?\/\/ #region 🛝️PlayHost\n/, "");
