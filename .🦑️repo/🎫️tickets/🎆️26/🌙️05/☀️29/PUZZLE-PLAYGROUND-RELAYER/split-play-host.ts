...
function splitReact(dim: "2d" | "3d" | "5d", marker: string): void {
  ...
  const hostHeader =
    dim === "2d"
      ? `/** @emoji 🛝️ Board play React host — entry-only via play/main.ts; wires @puzzle/2d-react into @semio-tech/framework-playground-core-renderer-react. */\n`
      : dim === "3d"
        ? `/** @emoji 🛝️ Scene play React host — entry-only via play/main.ts. */\n`
        : `/** @emoji 🛝️ Topology play React host — entry-only via play/main.ts. */\n`;
  writeFileSync(reactIndex, `${head}\n`);
  writeFileSync(playHost, `${hostHeader}${hostBody.replaceAll("../index.tsx", "../react/index.tsx").replaceAll("./play/", "./")}\n`);
  console.log(`[split] ${dim}: react ${head.split("\n").length} lines, host ${hostBody.split("\n").length} lines`);
}

splitReact("2d", "// #region 🛝️PlayHost");
splitReact("3d", "// #region 🛝️PlayHost");
splitReact("5d", "// #region 🛝️PlayHost");
