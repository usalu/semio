...
function stripHeader(src: string): string {
  ...
  let i = 0;
  if (lines[i]?.includes("#region 🧲️Header")) {
    i++;
...
function stripContractsImport(src: string): string {
  return src.replace(/^import type \{ MeshTransfer \} from "\.\/contracts\.ts";\r?\n/m, "").replace(/^import \{[\s\S]*?\} from "\.\/contracts\.ts";\r?\n/m, "");
}

const indexPath = join(dir, "index.ts");
const testsMatch = readFileSync(indexPath, "utf8").match(/\/\/ #region 🧪️Tests[\s\S]*$/);
...
/** @emoji 🧭️ \`@semio-tech/geometry-brep-js\` — cad-free brepjs + OpenCascade kernel and contracts. */
// #endregion 🧲️Header

// #region 📐️Contracts
${contracts}// #endregion 📐️Contracts

// #region 🧭️Kernel
${kernel}// #endregion 🧭️Kernel
