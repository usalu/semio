#!/usr/bin/env bun
/**
 * @emoji 🧩️ Splices concrete `Operation` commands from `schema.golden.graphql` into `compose/client/schema/compose/schema.yaml` (camelCase keys, `implements: [*operation]`, empty `fields`); emits Neo4j `Command` merge/reparent/rename + `Data` argument kit from golden `*Input` types (imperative command names only).
 */
...
/** @emoji 🧭️ Past-participle operation stem → imperative stem (must match rename-operations-imperative.ts). */
...
/** @emoji 🧭️ Golden `type Op implements Operation` carries `input: OpInput!` only when the operation has a typed argument bag. */
...
/** @emoji 🧭️ Reads `# Arguments` section of `type ${pastOp}Input` in golden SDL. */
...
/** @emoji 🧭️ Pairs `HEAD` golden name → working-tree name when both share the same imperative stem (migration rename on existing DB). */
...
/** @emoji 🧭️ Owning kit `Class` / `Interface` for each concrete `Command` (imperative names). */
...
/** @emoji 🧭️ Neo4j `Data` argument kit: one node per `(Command.name, field.name)` via `soleOwnerKey` + detach/rewire so kit `Data` is never shared across commands. */
