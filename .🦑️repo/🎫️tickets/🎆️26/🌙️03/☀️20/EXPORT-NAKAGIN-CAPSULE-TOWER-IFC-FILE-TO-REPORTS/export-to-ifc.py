...
try:
    ...
    print(f"❌️ Failed to import export_design_model from compose.py: {e}")
    print(f"❌️ Tried to import from: {compose_py_path}")
    sys.exit(1)
...
def extract_nakagin_capsule_tower_design():
    ...
    try:
        ...
        if not kit_path.exists():
            print(f"❌️ Metabolism kit file not found: {kit_path}")
            return None, None
        ...
        if not nakagin_designs:
            print('❌️ Nakagin Capsule Tower design not found in Metabolism kit')
            return None, None
        
        print(f"✅️ Found {len(nakagin_designs)} Nakagin Capsule Tower design(s)")
        return kit_data, nakagin_designs[0]  # Return kit and main design
        
    except Exception as error:
        print(f"❌️ Failed to read Metabolism kit: {error}")
        return None, None

def export_ifc_model(kit, design, output_path):
    """Export design to IFC format using compose.py export_design_model"""
    try:
        print(f"🔄️ Exporting {design['name']} to IFC format...")
        ...
        print(f"✅️ IFC exported to: {output_path}")
        print(f"📊️ Model size: {len(ifc_bytes)} bytes")
        
        return True
        
    except Exception as error:
        print(f"❌️ Failed to export IFC: {error}")
        import traceback
        traceback.print_exc()
        return False
...
def main():
    ...
    print('🚀️ Starting Nakagin Capsule Tower design model export to IFC format...')
    ...
    print(f"📝️ Design: {design.get('name')}")
    print(f"🆔️ GUID: {design.get('guid')}")
    print(f"🧩️ Pieces: {len(design.get('pieces', []))}")
    ...
    if success:
        print('🎉️ IFC export completed successfully!')
        print(f'💡️ IFC file saved to: {output_path}')
        print('🏗️  The IFC file can be opened in any IFC-compatible BIM software')
    else:
        print('❌️ IFC export failed!')
        sys.exit(1)
