...
def test_ifc_export():
    ...
    print("🧪️ Testing Nakagin Capsule Tower IFC export for reports...")
    
    # Check if IFC file exists
    ifc_path = Path('/workspaces/semio/assets/models/nakagin-capsule-tower.ifc')
    
    if not ifc_path.exists():
        print("❌️ IFC file not found. Run export-to-ifc.py first.")
        return False
    
    # Check file size
    file_size = ifc_path.stat().st_size
    print(f"📊️ IFC file size: {file_size:,} bytes")
    ...
    try:
        with open(ifc_path, 'r', encoding='utf-8') as f:
            ...
            if first_line == "ISO-10303-21;":
                print("✅️ Valid IFC file format detected")
            else:
                print(f"❌️ Invalid IFC format: {first_line}")
                return False
    except Exception as e:
        print(f"❌️ Error reading IFC file: {e}")
        return False
    ...
    try:
        ...
        if "IFCPROJECT" in content and "IFC4" in content:
            print("✅️ IFC file contains expected IFC4 schema elements")
        else:
            print("❌️ IFC file missing expected schema elements")
            return False
    except Exception as e:
        print(f"❌️ Error parsing IFC content: {e}")
        return False
    ...
    if glb_path.exists():
        ...
        print(f"📊️ GLB file size: {glb_size:,} bytes")
        print(f"📈️ IFC/GLB size ratio: {file_size/glb_size:.2f}")
    
    print("✅️ IFC export test completed successfully!")
    return True
