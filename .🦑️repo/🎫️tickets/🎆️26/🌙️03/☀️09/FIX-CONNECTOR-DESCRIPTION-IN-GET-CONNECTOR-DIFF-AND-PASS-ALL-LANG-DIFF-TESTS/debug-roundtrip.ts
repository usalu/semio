import { MetabolismKit } from "@semio-tech/assets";
...
for (const kt of kit.types || []) {
  ...
  if (!zt) {
    console.log("type missing:", kt.name);
    continue;
  }
  ...
  if ((kt.connectors?.length || 0) !== (zt.connectors?.length || 0)) {
    console.log("type connectors diff:", kt.name, kt.connectors?.length, zt.connectors?.length);
    continue;
  }
  for (const kc of kt.connectors || []) {
    const zc = (zt.connectors || []).find((c: any) => c.guid === kc.guid);
    if (!zc) {
      console.log("connector missing:", kt.name, kc.guid);
      continue;
    }
    if (Math.abs(kc.point.x - zc.point.x) > 0.001 || Math.abs(kc.point.y - zc.point.y) > 0.001 || Math.abs(kc.point.z - zc.point.z) > 0.001) console.log("connector point diff:", kt.name, kc.name, kc.point, zc.point);
