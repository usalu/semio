...
3. `compose/js/sketchpad.test.ts`:
...
- Root cause: Selection flows through XState machine (`actor.send({ type: "DESIGN.SET_SELECTION" })`), but kit mutations flow through DesignStore (`store.execute("compose.designApp.deleteSelected")`). The `executeCommand` method was reading selection from `this.snapshot()` (local PlainAppStore state) which was never updated by XState machine selection events.
