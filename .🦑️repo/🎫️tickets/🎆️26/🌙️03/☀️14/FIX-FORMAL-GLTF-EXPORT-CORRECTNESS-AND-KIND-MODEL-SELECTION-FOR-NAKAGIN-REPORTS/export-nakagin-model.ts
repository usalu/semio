...
function exportModelToAssets(model: any, outputPath: string): void {
  try {
    ...
    console.log(`✅️ Model exported to: ${outputPath}`);
    console.log(`📊️ Model contains ${model.pieces?.length || 0} pieces and ${model.props?.length || 0} props`);
  } catch (error) {
 ...
 */
function main(): void {
  console.log("🚀️ Starting Nakagin Capsule Tower design model export...");
