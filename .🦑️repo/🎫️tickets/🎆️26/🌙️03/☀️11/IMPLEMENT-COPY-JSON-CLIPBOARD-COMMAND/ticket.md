...
Adjusted clipboard behavior to copy active kit JSON by default, removed design payload override from diagram Cmd/Ctrl+C, and updated existing clipboard e2e assertions to validate kit payload shape. TSC passes; focused Playwright remains blocked by existing @semio-tech/assets named-export runtime mismatch.
...
- `cd compose/js && npm run test:e2e -- --grep "Copy Json To Clipboard Command"` failed before test execution with existing `@semio-tech/assets` named export interop error (`CodeIcon`).
...
- `SyntaxError: Named export 'CodeIcon' not found` from `@semio-tech/assets` CommonJS interop in Playwright runtime.

## Todos

- Re-run `npm run test:e2e -- --grep "Copy Json To Clipboard Command"` after resolving the pre-existing `@semio-tech/assets` named export runtime mismatch.
- Resolve the existing `@semio-tech/assets` ESM/CJS named export mismatch in the Playwright test runtime.
