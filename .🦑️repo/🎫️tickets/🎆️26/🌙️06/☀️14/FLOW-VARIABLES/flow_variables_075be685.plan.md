...
- **Exhaustive match arms**: `widget_chrome` (251), `widget_io_ports` (575) - one input `requires(name,[schema])` + one output `provides(name,[schema])`, `widget_node_size` (606), `widget_to_dag_node` (636), `widget_id_for` (2874), `widget_label`/`widget_display_meta` (label = name, icon e.g. `🔣️`).
