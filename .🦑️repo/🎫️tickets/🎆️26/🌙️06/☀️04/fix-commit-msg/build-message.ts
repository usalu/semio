...
function findRemainderBundleIndex(bundles: CommitBundleSection[]): number {
  ...
  if (i < 0) throw new Error("commit: missing 🌐️workspace… remainder bundle in body");
  return i;
}
...
function extractBodyFromMessage(raw: string): string {
  ...
  for (const line of raw.split("\n")) {
    if (line.startsWith("🐙️") && line.endsWith("🔀️")) {
      inBody = true;
      continue;
    }
    if (!inBody) continue;
    if (/^📊️uloc➕️/.test(line) && !/^🎆️/.test(line) && bodyLines.length > 0) break;
    const stripped = line.replace(/📊️uloc.*$/u, "").trim();
    if (!stripped) {
...
🎆️26🌙️06☀️02
🌐️Storybook, cursor plans, and workspace scaffolding touched during the sprint
`;

const raw = readFileSync("/tmp/current-msg.txt", "utf8");
let body = extractBodyFromMessage(raw);
if (!body.includes("🌐️workspace")) body += remainderBody;
...
for (const row of gitRangeNumstat(root, wip, head)) {
  ...
  if (!dateLine) dateLine = bundles[bi]!.dates[0]?.dateLine ?? "🎆️26🌙️06☀️04";
...
for (let bi = 0; bi < bundles.length; bi++) {
  ...
  if (!fallbackDay) throw new Error(`bundle ${bundle.label} has no 🎆️ dates`);
