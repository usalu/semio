...
const head = "HEAD";
const microHead = "🐙️ueli🎆️26🌙️06☀️04🚩️";
...
const BUNDLE_DEFS: BundleDef[] = [
  ...
  { label: "🎬️presentation📽️", keywords: ["presentation", "projektetage", "reveal", "disposition", "morph", "slide", "chapter", "fullscreen tile", "embodiment"], pathTokens: ["presentation"] },
  {
    label: "🧩️puzzle🎮️play",
  ...
  { label: "🥅️framework", keywords: ["@framework", "framework/", "playground", "platform shell", "playgroundcontroller", "uinode", "product/base"], pathTokens: ["framework"] },
  ...
  { label: "🫡️agents", keywords: ["agents.md", "agent instructions", "goals mcp"], pathTokens: ["agents"] },
  { label: "🌐️workspace🧩️other", keywords: [], pathTokens: [] },
];
...
function assignBulletToBundle(bullet: string): string {
  ...
  if (/\bcad\b|spatial play|brep|extrude|construct query|model-definition/.test(lower)) return "📐️cad🪟️spatial";
  if (/presentation|projektetage|reveal\.js|disposition|morph/.test(lower)) return "🎬️presentation📽️";
  if (/maplibre|mvt|vector tile|figure-ground|gis layer|ancestor map/.test(lower)) return "🗺️gis🧭️map";
  if (/ghost provider|golden layout|engagement input|@ui\/react/.test(lower)) return "🖱️ui⚛️react";
  if (/svg icon|storybook|scrollbar|vite-elements/.test(lower)) return "🖱️ui🎨️assets";
...
function parseChronology(text: string): DaySection[] {
  ...
  for (const raw of text.split("\n")) {
    ...
    const day = /^🎆️(\d{2}🌙️\d{2}☀️\d{2})(?:📊️uloc.*)?$/u.exec(line);
    if (day) {
    ...
    if (text.startsWith("📊️uloc") || text.startsWith("🧾️")) continue;
    const normalized = text.startsWith("-") ? text.slice(2).trim() : text.replace(/^- /, "");
    const emojiBullet = normalized.match(/^\p{Extended_Pictographic}/u) ? normalized : `🔧️${normalized}`;
    current.bullets.push(emojiBullet.startsWith("🔧️") && /^[\p{Extended_Pictographic}]/u.test(normalized) ? normalized : emojiBullet);
  }
  return days;
}
...
function parseLegacyBundles(text: string): Map<string, Map<string, string[]>> {
  ...
  for (const raw of text.split("\n")) {
    ...
    if (/^📊️uloc➕️/.test(line) && !/^🎆️/.test(line)) break;
    if (/^Signed-off-by:/i.test(line)) break;
    const dayM = /^🎆️(\d{2}🌙️\d{2}☀️\d{2})/u.exec(line.replace(/📊️uloc.*$/u, "").trim());
    if (dayM) {
      day = dayM[0]!;
      continue;
    }
    if (line.match(/^\p{Extended_Pictographic}/u) && !line.startsWith("🎆️") && !line.startsWith("-")) {
      const scope = normalizeBundleScopeLabel(line.replace(/📊️uloc.*$/u, ""));
      if (scope && !scope.startsWith("📊️")) {
        bundle = scope;
...
const footerIdx = rawMsg.indexOf("📊️uloc➕️42518");
const legacyPart = footerIdx > 0 ? rawMsg.slice(0, footerIdx) : rawMsg.split("Signed-off-by:")[0]!;
const chronPart = rawMsg.includes("🎆️26🌙️06☀️02\n- ") ? rawMsg.slice(rawMsg.indexOf("🎆️26🌙️06☀️02\n- ")) : rawMsg.slice(footerIdx > 0 ? footerIdx : 0);
...
for (const row of gitRangeNumstat(root, wip, head)) {
  ...
  const fallback = bundleDays ? [...bundleDays.keys()].sort(compareDaysDesc)[0] : "🎆️26🌙️06☀️04";
  if (!dateLine) dateLine = fallback ?? "🎆️26🌙️06☀️04";
...
const lines: string[] = [formatBundleSubject({ alias: "ueli", emoji: "🐙️", name: "Ueli Saluz", email: "ueli@semio-tech.com" }, new Date("2026-06-04")), ""];
...
for (let ri = 0; ri < ranked.length; ri++) {
  ...
  if (!sections) {
    sections = new Map([["🎆️26🌙️06☀️04", new Set([`🔧️Work in ${def.label}`])]]);
  }
  ...
  for (const dateLine of listedDays) {
    ...
    for (const b of bulletSet) lines.push(b.startsWith("🔧️") || /^\p{Extended_Pictographic}/u.test(b) ? b : `🔧️${b}`);
  }
...
console.error(`[build] bundles=${ranked.length} range ➕️${fa}✏️${fe}➖️${fr} sum headers ➕️${sum.added}✏️${sum.edited}➖️${sum.removed}`);
