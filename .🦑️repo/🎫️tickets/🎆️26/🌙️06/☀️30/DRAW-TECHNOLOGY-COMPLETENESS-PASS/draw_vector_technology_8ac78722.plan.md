...
- Each new package gets `package.json` (`@semio-tech/draw-{core,react,play}`), `project.json` (delegating to `bun ./📜️script.ts ...`), `script.ts`, and `vitest.config.ts` mirroring raster. `draw-react` test builds flow_core WASM first (trace/boolean exports).
