...
```css
[data-level="base"] {
 --border-element-color: var(--element-base);
}
[data-level="canvas"] {
 --border-element-color: var(--element-canvas);
}
[data-level="window"] {
 --border-element-color: var(--element-window);
}
[data-level="panel"] {
 --border-element-color: var(--element-panel);
}
[data-level="overlay"] {
 --border-element-color: var(--element-overlay);
}
[data-level="temporary"] {
 --border-element-color: var(--element-temporary);
}
```
...
- Exact panel rest shades: light `--color-dark-5-9` (`#666e6b`) and dark `--color-gray-600` (`#a2a59d`) are proposed defaults; can be nudged to neighboring palette steps if more/less contrast is wanted.
  ...
  <todo>
  <id>tokens-light</id>
  <content>Add --element-_ family to :root in ui.css and repoint --border-element-color to var(--element-base)</content>
  </todo>
  <todo>
  <id>tokens-dark</id>
  <content>Add dark-mode --element-_ overrides in .dark block (panel = lighter readable gray)</content>
  </todo>
  <todo>
  <id>level-scoping</id>
