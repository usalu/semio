...
Conversions:
- Goal references (ticket.json "goal", goal.json "parent"): path → 🎯️flat1🎯️flat2...
- Contributor references (interaction "author", agent "contributor"): github → 🧑️‍💻️flatgithub
"""
...
def contributor_to_compose_id(github_handle: str) -> str:
...
def is_already_compose_id(value: str, emoji: str) -> bool:
...
def build_contributor_lookup(compose_repo_dir: str) -> dict:
    """Build a mapping of all identifiable strings to github handles."""
    lookup = {}
    contrib_dir = os.path.join(compose_repo_dir, "🧑️‍💻️")
...
def resolve_contributor(value: str, lookup: dict) -> str:
    ...
    # Already a compose ID
    if is_already_compose_id(value, EMOJI_CONTRIBUTOR):
        return value
...
def convert_contributor_ref(value: str, lookup: dict) -> str:
    ...
    if github and github not in ("unknown", "Migration Script", "GitHub Copilot"):
        return contributor_to_compose_id(github)
...
def main():
    ...
    compose_repo_dir = "/workspaces/semio/.repo"

    print("Building contributor lookup...")
    lookup = build_contributor_lookup(compose_repo_dir)
    ...
    ticket_files = glob.glob(
        os.path.join(compose_repo_dir, "🎫️", "**", "ticket.json"), recursive=True
    )
    for f in sorted(ticket_files):
        changes = process_ticket_json(f, lookup, dry_run)
        if changes:
            rel = os.path.relpath(f, compose_repo_dir)
            print(f"\n{rel}:")
            for c in changes:
                print(c)
    ...
    goal_files = glob.glob(
        os.path.join(compose_repo_dir, "🎯️", "**", "goal.json"), recursive=True
    )
