...
def fix_content(content):
    # This is a bit risky but let's try to find ctx.CreateBreach calls
    # that only have 5 arguments after the change.

    # We look for ctx.CreateBreach(
    # Then we find the balanced closing parenthesis.

    new_content = ""
    i = 0
    while i < len(content):
        if content.startswith("ctx.CreateBreach(", i):
            # Find the balanced closing paren
            start = i
            i += len("ctx.CreateBreach(")
            depth = 1
            ...
            # Now we have something like ctx.CreateBreach(arg1, arg2, arg3, arg4, arg5)
            ...
            commas_indices = []
            depth = 0
            # skip 'ctx.CreateBreach('
            for k in range(len("ctx.CreateBreach("), len(call_content) - 1):
                char = call_content[k]
                if char == "(":
                    depth += 1
                elif char == ")":
