...
# We need:
# line 29: # #region 🔖️Header Preamble  -- actually, simpler: move 'from __future__' after '# region Imports'
...
section_summaries = {
    ...
    "Exceptions": "# Custom exception document for server, client and specification errors.",
...
while i < len(lines):
    line = lines[i]
    stripped = line.strip()
    # Match # region XYZ or # #region 🔖️XYZ
    m = re.match(r"^# (?:#)?region\s+(?:🔖️)?(.+)$", stripped)
    if m:
...
def make_summary(name, kind, parent_classes="", body_hint=""):
    ...
    if kind == "def" or kind == "async def":
        summaries = {
            ...
            "findFixedPieces": "# Find all pieces that are fixed in the design document.",
            "getConnectedComponents": "# Get connected components of the piece graph.",
            "getPieceDocument": "# Get the hierarchical ordering of pieces from root to leaf.",
            ...
            "flattenDesignDict": "# Flatten a nested design document into a single flat coordinate space.",
    ...
    class_summaries = {
        ...
        "Entity": "# Abstract base for all domain entities with identity and document.",
...
def make_spec(name, kind):
    ...
    if kind == "def" or kind == "async def":
        func_requirements = {
            ...
            "getPieceDocument": "# getPieceDocument MUST return a topological ordering of pieces.",
