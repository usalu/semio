...
- `repo/cli/main.go`: Added EntityKinds, ResourceKinds, DiffableKinds, RelatedToFileKinds constant slices (~line 4285). Renamed TreeNodeStatuteNode→TreeNodeStatute, added TreeNodeTodo/TreeNodeBreach. Updated treeNodeKindToEntityKind: statute→"", category→"", todo→"todo", breach→"breach".
- `repo/cli/main_test.go`: Added 12 new test functions (TestEntityKinds, TestResourceKinds, TestDiffableKinds, TestRelatedToFileKinds, TestProjectListIDs, TestBundleListIDs, TestSectionListIDs, TestContributorListIDs, TestGoalListIDs, TestTicketListIDs, TestDraftListIDs, TestAllSpecIDExamples). Fixed TestGoalListIDs type assertion []Goal→[]\*Goal. Fixed TestTreeNodeKindToEntityKindCoversAll expected values for statute and category. Renamed TreeNodeStatuteNode→TreeNodeStatute globally.
