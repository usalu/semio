...
- `.vscode/launch.json`: Reordered sections to BUILD → PUBLISH:TEST → PUBLISH → DEV. Renamed `Go` → `Go dev`, `Python` → `Python dev`. Removed separate `repo/vscode package` entry (unified into build).
- `.vscode/tasks.json`: Reordered sections to BUILD → TEST → PREFLIGHT → ANALYZE & FIX → PUBLISH:TEST → PUBLISH → UPDATE → DEV → MCP. Removed separate `repo/vscode package` task.
...
- Merged repo/vscode package into build section

## Todos

- [x] Reorder launch.json by task kind
- [x] Rename generic debuggers to Go dev, Python dev
- [x] Unify repo/vscode package into build
...
5. Remove separate repo/vscode package entry, keep only build
