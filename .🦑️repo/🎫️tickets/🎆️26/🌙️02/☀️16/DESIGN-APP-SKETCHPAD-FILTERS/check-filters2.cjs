...
(async () => {
  ...
  await page.screenshot({ path: "/workspaces/semio/.repo/🎫️/26/02/16/DESIGN-APP-SKETCHPAD-FILTERS/after-upload.png" });
  ...
  if (filterGroupCount > 0) {
    ...
    if (pCount > 0) {
      ...
      await page.screenshot({ path: "/workspaces/semio/.repo/🎫️/26/02/16/DESIGN-APP-SKETCHPAD-FILTERS/filter-test.png" });
    }
  }
