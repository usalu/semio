...
1. ✅️ Analyze hover state subscription patterns to identify overfetching
2. ✅️ Fix `useIsPieceHovered` to use granular subscription instead of full hover object
3. ✅️ Fix `useDesignAppIsPieceTransitiveHovered` to use `useSyncField` with "hover" key
4. ✅️ Fix `useIsPieceTransitiveHovered` and `useIsConnectionHovered` similarly
5. ✅️ Add granular hooks for connector hover and connector selection
6. ✅️ Fix `PieceNodeComponent` and `DesignNodeComponent` to use granular hooks
7. ✅️ Add hover performance test to verify 100ms hover/unhover timing
