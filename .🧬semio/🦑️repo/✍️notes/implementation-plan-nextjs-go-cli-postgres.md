...
8. **Ticket workflow requirement:**
   - opening a ticket creates a temporary folder under `.repo/🎫️/{{ticket-id}}`,
...
- manage temporary ticket folders under `.repo/🎫️/{{ticket-id}}`,
...
1. CLI compresses `.repo/🎫️/{{ticket-id}}` to `tar.gz`.
...
6. Only after 200/confirmed response, CLI deletes `.repo/🎫️/{{ticket-id}}`.

### Reopen ticket

1. Server reopens ticket.
2. CLI recreates `.repo/🎫️/{{ticket-id}}` if absent.
...
- Opening a ticket creates `.repo/🎫️/{{ticket-id}}` locally.
...
- [ ] Keep `.repo/🎫️/{{ticket-id}}` temp workspace logic
