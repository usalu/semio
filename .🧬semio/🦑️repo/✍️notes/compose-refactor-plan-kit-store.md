# Compose Refactor Plan: Replaceable Kit Store Architecture

## Executive summary

This refactor must **remove all storage-engine knowledge from `compose/sketchpad` and public core APIs** so the kit backend can be swapped without changing Sketchpad. The current code still mixes UI state, kit editing logic, synchronization, persistence, and storage-engine primitives in the same bundle. The end state is:

- `compose/sketchpad` becomes a **storage-agnostic editor UI**.
- `compose/js` becomes **domain/core only**.
- `compose/studio` becomes the **provider bundle** that contains the collaborative kit provider and the other store implementations.
...
- the string and dependency for the current collaboration engine disappear from `compose/sketchpad` after the refactor.
...
1. **No engine-specific imports in `compose/sketchpad`.**
2. **No engine-specific imports in public `compose/js` APIs.**
...
4. **All kit providers live in `compose/studio`** and are exported from `studio.ts`.
5. `Sketchpad` receives the backend via a root prop, e.g.:
   - `<Sketchpad kitStore={kitStore} />`
6. Add these store implementations:
   - **CollaborativeKitStore** in `compose/studio`
   - **JsonFileKitStore** in `compose/studio`
   - **FolderKitStore** in `compose/studio`
7. The **VS Code bundle is a separate extension** and **must not live in `repo`**.
8. After the refactor, the collaboration engine name must **not appear anywhere in `compose/sketchpad`**.
...
- Move all provider implementations into `compose/studio`.
...
- Support three backends with the same public capability surface:
  ...
  - folder-backed store using `.compose/` and sqlite
...
- letting `compose/sketchpad` choose storage strategy internally
...
compose-vscode (separate repository)
...
`compose/sketchpad` may depend on `KitStore`, `UndoableKitStore`, and other storage-agnostic capability interfaces. It may **not** depend on any provider implementation or provider primitive.
...
- add lint rules / forbidden imports:
  - forbid the collaboration engine package in `compose/sketchpad`
  - forbid provider packages in `compose/js`
  - forbid importing `compose/studio` from `compose/js`
- add CI grep checks:
  - the collaboration engine name must not appear in `compose/sketchpad/**`
- define public interfaces in `compose/js`
...
# Phase 1 — extract storage-agnostic contracts into `compose/js`

## Outcomes

- `compose/js` becomes the home of the portable contracts
...
```text
compose/js/store/KitStore.ts
compose/js/store/UndoStore.ts
compose/js/store/AssetStore.ts
compose/js/store/SyncState.ts
compose/js/store/StoreEvents.ts
compose/js/store/transactions.ts
```

### 1.2 Move domain logic out of provider-backed store classes

Move pure logic into `compose/js`:
...
- `compose/js` exports only neutral store contracts and domain logic
- no provider implementation exists in `compose/js`
- a dummy in-memory store can satisfy the new interfaces

---

# Phase 2 — decontaminate `compose/sketchpad`
...
Anything in Sketchpad that creates, persists, or manages a concrete backend must move to `compose/studio`.

## Exit criteria

- no collaboration engine import in `compose/sketchpad`
- no persistence import in `compose/sketchpad`
- `Sketchpad` boots from injected `kitStore`
- the editor runs against a fake in-memory `KitStore`

---

# Phase 3 — build `compose/studio`
...
- provider internals stay inside `compose/studio/providers/collaborative/**`
...
- keep metadata/state in `.compose/`
- use sqlite in `.compose/` in the same model as the python engine
...
- `compose/studio` is the only bundle with provider implementations
- all providers return the same neutral interface
- `studio.ts` is the single public export point for kit providers

---

# Phase 4 — migrate current collaborative backend into `compose/studio`

## Outcomes

- current collaborative support still works
- it no longer contaminates Sketchpad or `compose/js`
...
The extension package is not a new bundle inside `repo`. It is a separate project consuming published Compose packages.
...
- no engine reference in `compose/sketchpad`

### Workstream C — collaborative provider in `compose/studio`
...
- grep check for engine string inside `compose/sketchpad`
...
4. extract collaborative provider into `compose/studio`
...
9. delete old provider code from `compose/sketchpad` and `compose/js`
...
- `compose/sketchpad` contains no collaboration-engine import, symbol, helper, or terminology
- `compose/js` contains no concrete kit provider implementation
- `compose/studio` exports all kit providers from `studio.ts`
...
- [ ] `compose/js` exports only neutral store contracts and core domain logic
- [ ] `compose/sketchpad` has no provider imports
- [ ] `compose/sketchpad` has no collaboration-engine terminology
- [ ] `compose/studio/studio.ts` exports all kit providers
...
- [ ] separate `compose-vscode` repository created
...
The right refactor is to make `KitStore` the boundary and move every concrete backend behind it in `compose/studio`. `compose/sketchpad` must become a pure editor shell that consumes a kit store by injection. `compose/js` must become pure portable core. The VS Code editor must be delivered as a separate extension using the JSON file store, not by embedding extension code into the main repository.
...
There should be **no** public `AssetStore`, and `compose/studio` should not export `createBlobStore(...)`. The refactor plan needs to be corrected there. Files and folders already belong in the kit domain, and the store should expose them exactly like types, designs, qualities, and authors.
...
- remove `AssetStore` from `compose/js`
- remove `createBlobStore(...)` from `compose/studio`
...
I can rewrite the store contract into a concrete `compose/js/store/KitStore.ts` draft next.

---

Here is the concrete JSON store I would ship for Electron.

It matches the refactor direction you set: the JSON file store lives in `compose/studio`, Sketchpad only sees a neutral store, and JSON is a one-kit-per-file backend. It also treats files and folders as normal kit entities, which your current command layer and import/export already do, and it relies on stable `guid` identity across entities for subscriptions and merge targeting.

I am assuming the JSON file contains the full kit and that file payloads are JSON-safe values inside the kit, such as base64 or data-URI strings on file fields. That is consistent with the existing file/folder handling and the current blob decoding logic in `compose.ts`.

### 1) Electron host bridge

```ts
// compose/studio/providers/json-file/JsonFileHost.ts
...
```ts
// compose/studio/providers/json-file/electronJsonFileHost.ts
...
```ts
// compose/studio/providers/json-file/JsonFileKitStore.ts
import { createHash } from "node:crypto";
import { deepEqual, type Kit } from "../../../js/compose";
...
```ts
import { createJsonFileKitStore } from "@compose/studio/providers/json-file/JsonFileKitStore";
import { electronJsonFileHost } from "@compose/studio/providers/json-file/electronJsonFileHost";
