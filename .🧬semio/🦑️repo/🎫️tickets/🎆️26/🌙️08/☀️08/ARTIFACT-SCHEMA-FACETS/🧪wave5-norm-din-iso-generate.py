#!/usr/bin/env python3
"""Generate artifact/snapshot/diff schema leaves and refactor norm din/iso artifacts (wave-5 din-iso agent)."""
...
ARTIFACTS = [
    ("📓️iso16757", "iso16757", "Iso16757", "norm.iso16757", "ISO 16757", True),
    ("📔️vdi3805", "vdi3805", "Vdi3805", "norm.vdi3805", "VDI 3805", True),
    ("📕️din4108", "din4108", "Din4108", "norm.din4108", "DIN 4108", False),
    ("📗️din16798", "din16798", "Din16798", "norm.din16798", "DIN EN 16798", False),
    ("📙️din18599", "din18599", "Din18599", "norm.din18599", "DIN V 18599", False),
]
...
def parse_document_struct(path: Path) -> tuple[list[Field], str, list[str]]:
    ...
    if not m:
        m = re.search(r"pub struct BalancingInputs \{([^}]+)\}", text, re.S)
    if not m:
        raise SystemExit(f"no Document/BalancingInputs struct in {path}")
    body = m.group(1)
    fields: list[Field] = []
...
def json_type(field: Field, prefix: str) -> dict:
    ...
    if "FireRating" in ty:
        return {"type": "string"}
    if "ClimateZoneDe" in ty:
        return {"type": "string"}
    if "UseClass" in ty:
        return {"type": "string"}
    if "MonthlyClimate" in ty:
        return {"$ref": f"#/$defs/{prefix}MonthlyClimate"}
    if "LayerDocument" in ty:
        return {"type": "array", "items": {"$ref": f"#/$defs/{prefix}LayerDocument"}}
    if ty.startswith("BTreeMap"):
        return {"type": "object", "additionalProperties": {"type": "string"}}
    if "::" in ty and not ty.startswith("Vec<"):
        return {"type": "object"}
...
def rust_schema_type(field: Field, prefix: str, key: str) -> str:
    ty = field.ty
    if ty == "f64":
        return "f64"
    if ty == "u8":
    ...
    if "ClimateZoneDe" in ty:
        return "crate::document::ClimateZoneDe"
    if "UseClass" in ty:
        return f"crate::artifacts::{key}::UseClass"
    if "MonthlyClimate" in ty:
        return f"crate::artifacts::{key}::MonthlyClimate"
    if "LayerDocument" in ty:
        return f"Vec<crate::artifacts::{key}::LayerDocument>"
    if ty.startswith("BTreeMap"):
        return ty
    if "::" in ty and not ty.startswith("Vec<"):
        return ty.replace("part_", f"crate::artifacts::{key}::part_")
    if ty.startswith("Vec<QkEntry>"):
...
def write_rust_artifact_schema(key: str, prefix: str, mod_key: str, fields: list[Field], path: Path) -> None:
    ...
    for f in fields:
        rt = rust_schema_type(f, prefix, key)
...
def strip_document_surgical(path: Path, key: str, prefix: str, label: str, orig: str) -> None:
    text = path.read_text(encoding="utf-8")
    text = re.sub(
        r"#\[derive[^\n]*dsl::DslRecord[^\n]*\]\s*\n(?:#\[serde[^\n]*\]\s*\n)?#\[dsl\(id = \"norm\." + re.escape(key) + r"\"[^\n]*\]\s*\npub struct (?:Document|BalancingInputs) \{[\s\S]*?//#endregion 🔖️HandcraftedDocumentCodecs\s*\n",
        "",
        text,
        count=1,
    )
    text = re.sub(r"impl Default for (?:Document|BalancingInputs) \{[\s\S]*?\n\}\s*\n", "", text, count=1)
    re_export = f"\n/// 📸️ Persisted snapshot — defined in `📸️snapshot/🧬️schema`, re-exported here.\npub use crate::artifacts::{key}::snapshot::schema::{prefix}Snapshot;\n"
    if re_export.strip() not in text:
        if "// #endregion Part5" in text:
            text = text.replace("// #endregion Part5", f"// #endregion Part5{re_export}", 1)
        elif "// #endregion Session" in text:
            text = text.replace("// #region Session", f"{re_export}// #region Session", 1)
        elif "#endregion 🔖️Types" in text:
            text = text.replace("#endregion 🔖️Types", f"{re_export}#endregion 🔖️Types", 1)
    path.write_text(text, encoding="utf-8")


def refactor_root_component(folder: Path, key: str, prefix: str, label: str, orig: str, nested: list[str], complex: bool) -> None:
    strip_document_surgical(folder / "🦀️component.rs", key, prefix, label, orig)


def write_snapshot_rust(folder: Path, key: str, prefix: str, dsl_id: str, orig: str, fields: list[Field], nested: list[str], complex: bool) -> None:
    nested_blocks = "" if complex else "\n".join(m.group(1) for m in re.finditer(r"(pub mod \w+ \{[\s\S]*?\n\})", orig))
    struct_body = re.search(r"pub struct (?:Document|BalancingInputs) \{([^}]+)\}", orig, re.S).group(1)
    ...
    default_impl = re.search(r"impl Default for (?:Document|BalancingInputs) \{[\s\S]*?\n\}", orig)
    default_text = default_impl.group(0).replace("Document", f"{prefix}Snapshot").replace("BalancingInputs", f"{prefix}Snapshot").replace("QkEntry", f"{prefix}QkEntry") if default_impl else ""
    qk = ""
...
{codecs.group(0).replace('Document', prefix + 'Snapshot').replace('BalancingInputs', prefix + 'Snapshot').replace('En1990SnapshotDsl', 'DocumentDsl').replace('En1990SnapshotPack', 'DocumentPack') if codecs else ''}
...
def generate_one(folder_name: str, key: str, prefix: str, dsl_id: str, label: str, complex: bool) -> None:
    ...
    write_snapshot_rust(folder, key, prefix, dsl_id, orig, fields, nested, complex)
    ...
    move_pack(folder)
    refactor_root_component(folder, key, prefix, label, orig, nested, complex)


def main() -> None:
    for folder_name, key, prefix, dsl_id, label, complex in ARTIFACTS:
        generate_one(folder_name, key, prefix, dsl_id, label, complex)
        print("generated", key)
