/**
 * ⚖️ Writes the canonical law file for every converted bundle and repoints its leaf.
 *
 * The six laws are the SAME six every 13-node bundle in this repository states, so they are
 * rendered from one template rather than restated by hand 36 times; the seventh — the op-codec
 * round trip — is the assertion the pre-contract flat fixture already carried and is preserved
 * verbatim rather than dropped in the move.
 */
import { existsSync, readdirSync, readFileSync, writeFileSync, rmSync } from "node:fs";
import { join } from "node:path";

type Family = { name: string; root: string; snapshot: string; mutation: string; scenario: string };

const families: Family[] = JSON.parse(readFileSync(process.argv[2], "utf8"));
const repoRoot = process.cwd();

const rustModule = (id: string): string => id.replace(/[^a-z0-9]+/g, "_");
const kindOf = (dir: string): string => dir.match(/[a-z0-9][a-z0-9-]*$/)?.[0] ?? dir;

const law = (family: Family, kind: string, scenario: string): string => `//! 🧪️ \`${kind}\` fixture — \`${scenario}\`.
//!
...
fn applies_to_committed_after() {
    let base = before();
    let produced = mutation().diff(&base).diff().apply(&base).expect("${kind} applies to its committed before-snapshot");
    assert_eq!(produced, expected_after(), "${kind}/${scenario}: applied state differs from the committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    let mut current = payload.diff(&base).diff().apply(&base).expect("forward ${kind} applies");
    for step in payload.inverse(&base) {
        current = step.diff(&current).diff().apply(&current).expect("the ${kind} inverse step applies");
    }
    assert_eq!(current, base, "${kind}/${scenario}: the undo did not restore the committed before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ${family.snapshot} = serde_json::from_str(text).expect("snapshot decodes");
...
fn op_codecs_round_trip() {
    let payload = mutation();
    assert_eq!(${family.mutation}::parse_op(&payload.print_op()).expect("the text op parses"), payload, "${kind}/${scenario}: the text op form does not round-trip");
    let bytes = payload.encode_op().expect("the binary op encodes");
    assert_eq!(bytes[1], super::binary::BINARY_TAG, "${kind}/${scenario}: the binary frame must carry this schema's tag");
    assert_eq!(${family.mutation}::decode_op(&bytes).expect("the binary op decodes"), payload, "${kind}/${scenario}: the binary op form does not round-trip");
    assert!(${family.mutation}::decode_op(&bytes[..1]).is_err(), "${kind}/${scenario}: a truncated binary frame must be rejected");
}
`;

let written = 0;
for (const family of families) {
  const mutationsRoot = join(repoRoot, family.root);
  for (const leaf of readdirSync(mutationsRoot, { withFileTypes: true })) {
    if (!leaf.isDirectory()) continue;
    const tests = join(mutationsRoot, leaf.name, "🧪️tests");
    const bundle = join(tests, family.scenario);
    if (!existsSync(join(bundle, "🦠️mutation", "🔣️component.json"))) continue;
    const kind = kindOf(leaf.name);
    writeFileSync(join(bundle, "🦀️component.rs"), law(family, kind, family.scenario));
    for (const stale of ["🦀️component.rs", "🔣️component.json"]) {
      const path = join(tests, stale);
      if (existsSync(path)) rmSync(path);
    }
    const leafSource = join(mutationsRoot, leaf.name, "🦀️component.rs");
    let source = readFileSync(leafSource, "utf8");
    source = source
      .replace(/#\[path = "🧪️tests\/🦀️component\.rs"\]\nmod tests;/, `#[path = "🧪️tests/${family.scenario}/🦀️component.rs"]\nmod tests_${rustModule(family.scenario)};`)
      .replace(/let vector: serde_json::Value = serde_json::from_str\(include_str!\("🧪️tests\/🔣️component\.json"\)\)\.expect\("authored mutation vector"\);\n\s*serde_json::from_value\(vector\["mutation"\]\.clone\(\)\)\.expect\("[^"]*"\)/, `serde_json::from_str(include_str!("🧪️tests/${family.scenario}/🦠️mutation/🔣️component.json")).expect("committed ${kind} payload")`);
    writeFileSync(leafSource, source);
    written += 1;
  }
}
console.log(`${written} bundle law file(s) written and mounted`);
