"""🐍️ Independent Python implementation of the VDI 3805 mutation vocabulary — the second producer
this case's differential comparison needs.

`s.norm.vdi3805` is a semio-NATIVE artifact and no third-party library, in any ecosystem, reads or
writes it. That was checked rather than assumed: PyPI serves no `vdi3805`, `eurocode`, `vdi3805` or
...
judge this subset's `Vdi3805Mutation` vocabulary. The reference is therefore a second IMPLEMENTATION,
...
def identity_handler(ctx):
    """🔁️ The real committed VDI 3805 document through the carrier it is committed in. That carrier
