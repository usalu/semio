"""🐍️ Independent Python implementation of the EN 1993 mutation vocabulary — the second producer
this case's differential comparison needs.

`s.norm.en1993` is a semio-NATIVE artifact and no third-party library, in any ecosystem, reads or
writes it. That was checked rather than assumed: PyPI serves no `en1993`, `eurocode`, `vdi3805` or
`iso16757` distribution at all, and the nearest real packages (`structuralcodes`, `concreteproperties`,
`anastruct`) implement design-code FORMULAE and speak no interchange format, so none of them could
judge this subset's `En1993Mutation` vocabulary. The reference is therefore a second IMPLEMENTATION,
written from the repository's own written specification of what a semantic mutation MEANS:
...
* this subset's committed mutation catalog, for the closed list of kinds.
* the committed specification vectors, for the JSON wire spelling of each payload.
...
⚠️ Honest boundary, the CARRIER. `.dsl.semio` has no specification here: this subset's committed
...
envelope preamble and the ordered `key=value` fields as written — a shape derived from that artifact's
own committed bytes and PINNED by re-emitting the file byte for byte, which a misreading could not do.
It deliberately does not map carrier tokens onto the JSON snapshot's enum spellings: that mapping is
stated nowhere, and inferring it would be reverse-engineering rather than a second reading.
"""
...
VECTORS = {
    "change-annex": ("🎢change-annex", "switches-the-national-annex-from-de-to-en"),
    "update-member-properties": ("🧊update-member-properties", "re-grades-the-base-member-to-s460-under-a-heavier-load"),
    "update-fire-inputs": ("🔆update-fire-inputs", "raises-the-fire-protection-to-r90"),
    "update-cold-formed-inputs": ("🔥update-cold-formed-inputs", "thickens-the-cold-formed-flange-and-reverses-its-stress-gradient"),
    "update-stainless-inputs": ("🎚️update-stainless-inputs", "upsizes-the-stainless-section-to-a-duplex-grade"),
    "update-plated-inputs": ("🧭update-plated-inputs", "makes-the-plate-panel-more-slender-and-more-stressed"),
    "update-silo-shell-inputs": ("🪟update-silo-shell-inputs", "deepens-the-silo-and-thickens-its-shell"),
    "update-bolt-inputs": ("🌞update-bolt-inputs", "moves-the-connection-to-four-m24-grade-10-9-bolts"),
    "update-weld-inputs": ("🧱update-weld-inputs", "lengthens-the-fillet-weld-and-re-grades-it-to-s460"),
    ...
    "update-tension-component-inputs": ("⚡update-tension-component-inputs", "derates-the-tension-rod-to-a-400-kn-characteristic-strength"),
    "update-hss-inputs": ("💧update-hss-inputs", "reclassifies-the-hollow-section-to-class-3-in-s355"),
    "update-bridge-inputs": ("🏗️update-bridge-inputs", "raises-the-bridge-damage-equivalence-and-dynamic-factors"),
    "update-tower-inputs": ("🌗update-tower-inputs", "raises-the-tower-wind-factor-and-leg-force"),
...
#: 📂 The `asset://` prefix every specification vector hangs off.
...
DSL_ASSET = "asset://🏅️standards/🔖️1/🪆️subsets/✳️any/📚️examples/📕️high-strength-connection/🖼️assets/🗣️high-strength-connection.dsl.semio"
...
def inverse_mutation(document, kind, arguments):
    ...
    if verb in ("create", "add"):
        ...
        partner = declared_partner(("delete", "remove", "insert"), noun)
    ...
    if verb in ("delete", "remove"):
        ...
        partner = declared_partner(("create", "add", "insert"), noun)
        ...
        setter = declared_partner(("change", "set", "replace"), noun)
        if setter is None:
            return []
...
def declared_partner(verbs, noun):
...
def vector(ctx, kind):
    ...
    directory, fixture = VECTORS[kind]
    stem = "%s/%s/🧪️tests/%s" % (VECTOR_ROOT, directory, fixture)
    return (
...
def mutate_handler(kind):
    ...
    def handler(ctx):
        base, wire, expected, outcome = vector(ctx, kind)
...
def inverse_handler(kind):
    ...
    def handler(ctx):
        base, wire, _expected, outcome = vector(ctx, kind)
        _tag, arguments = unwrap(wire)
        try:
        ...
        steps = inverse_mutation(base, kind, arguments)
...
def identity_handler(ctx):
    """🔁️ The real committed EN 1993 document through the carrier it is committed in. That carrier
    ...
    text = ctx.fixture_bytes(DSL_ASSET).decode("utf-8")
    ...
    if parse_dsl(reprinted) != document:
...
def adapter():
    ...
    for kind in KINDS:
        built = built.oracle("mutate-" + kind, mutate_handler(kind)).oracle("inverse-" + kind, inverse_handler(kind))
