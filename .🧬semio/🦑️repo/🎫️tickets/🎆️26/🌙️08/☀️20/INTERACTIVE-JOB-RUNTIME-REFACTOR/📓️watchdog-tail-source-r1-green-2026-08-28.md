# Watchdog Tail Source R1

Actual canonical source target exit0 with6 tail vectors/5 hostiles, alongside input22/root6/writer9-copy frontiers. This historical source run predates the two added sticky-clock vectors and consuming checkpoint refinement; no native tail tests executed.

```sh
set -o pipefail
NX_DAEMON=false NX_CACHE_PROJECT_GRAPH=false NX_ISOLATE_PLUGINS=false bun x nx run @semio-tech/ui-host-rs:test-source --skip-nx-cache 2>&1 | tee '/Users/ueli/Documents/semio/.🧬semio/🦑️repo/🎫️tickets/🎆️26/🌙️08/☀️20/INTERACTIVE-JOB-RUNTIME-REFACTOR/🧪️member-watchdog-tail-source-r1-2026-08-28.md'
```
...
[DEBUG] input writer oracle: 9 byte-copy frontiers, 3 retained-backing frontiers, 6 schema hostiles; native writer, admission, unwind and allocation remain unexecuted
[DEBUG] watchdog tail oracle: 6 same-window vectors, 5 schema hostiles; actual WGPU publication and native guard execution remain separate
