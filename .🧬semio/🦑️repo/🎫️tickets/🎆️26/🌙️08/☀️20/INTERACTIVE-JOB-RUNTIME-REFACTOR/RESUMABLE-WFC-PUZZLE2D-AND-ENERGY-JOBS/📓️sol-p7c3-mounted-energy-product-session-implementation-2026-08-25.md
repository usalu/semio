...
## Second independent RED remediation

- The final commit lease now remains the exact mounted consumer owner until adoption drains its retained pages and explicitly ACKs it. `EnergyJob::ack_commit_packet` records that exact successful ACK as the only external-consumer terminal witness. The next numerical `Complete` step consumes the witness and emits an empty terminal candidate; the mounted wrapper retains that terminal outcome, returns process `Done`, and its unconditional Drop publishes the exact shell for fixed recovery. The headless no-lease terminal path still transfers the prepared commit payload through `take_terminal` unchanged.
- A completed capture now passes a production transfer guard immediately before `Model` ownership moves into `EnergyJob::admit` or `EnergyRestoreJob::admit`. The guard rechecks exact mounted identity, live render/application/document provenance, the registry's live request, retained config digest, snapshot-store generation/revision, and cancellation. Failure leaves the complete `ModelCapture` mounted, retains any already-transferred checkpoint packet in the same shell, begins bounded close, returns the snapshot read witness, and retires through the shell's pre-reserved slot.
- Adopted results now use a fixed `AdoptedProjectionAuthority` containing the application-qualified render authority, complete mounted identity, and projection. Lookup goes directly through the requesting application's fixed registry slot, requires the slot's live request to equal the adopted request, and validates request/operation/generation/config plus every tier's app/document/operation/generation/config provenance before exposing a borrow. Same-document provenance in another application can no longer match.
- Executable hostile laws now drive the real P7c2 commit queue and `EnergyJob`: an unacknowledged final lease keeps the job nonterminal, exact ACK advances the queue once, and the next numerical grant becomes terminal. Production capture-transfer mutations change application, document revision, document generation, canonical revision, request, config digest, snapshot freshness, cancellation, and expected mounted generation; every mutation retains the exact capture and drains it through the one-unit close cursor. Fixed adopted storage is queried from two applications with identical document provenance and is mutated across request, operation, generation, config, tier application, and newer-request ABA dimensions; every stale query is refused.
- Final source-only freshness reran scoped `rustfmt --check`, JSON parsing, Energy/report `git diff --check`, terminal/admission/adoption mutation census, forbidden mounted-authority census, and the single Energy registration census. No runtime/build command was used.
