# Plugin Native Inventory R8

Actual compile-only GREEN: Nx exit 0; Cargo finished the unoptimized test profile in 44.02s and produced `semio_framework_plugin-d81b8cb7f98afff8`. No tests ran in this invocation. There is no guest/Wasm, lifecycle, output admission, or callback timing claim. The 590 warnings remain visible, including unused Future and Result diagnostics.

R7's 11 E0599 errors were corrected by the owning Mutation lane adding only `PluginApp` to the transaction fixture's existing import. [R8 selected input capture](./📓️plugin-native-inventory-r8-source-inputs-2026-08-27.md) contains 926 entries, with main/transaction hashes matching the explicit release. Selected capture is not an atomic full dependency closure.

The inventory hold was released on completion; the construction/lifetime and mounted-output source was then held unchanged for the two genuine construction tests followed by the three live-output tests. This report does not pre-credit those executions.

## Command

```sh
set -o pipefail
SEMIO_COVERAGE=0 NX_DAEMON=false NX_CACHE_PROJECT_GRAPH=false NX_ISOLATE_PLUGINS=false CARGO_BUILD_JOBS=2 CARGO_TARGET_DIR='/Users/ueli/Documents/semio/.🧬semio/🦑️repo/🎫️tickets/🎆️26/🌙️08/☀️20/INTERACTIVE-JOB-RUNTIME-REFACTOR/🧱️cargo-target-cad' SEMIO_TEST_ARTIFACT_DIR='/Users/ueli/Documents/semio/.🧬semio/🦑️repo/🎫️tickets/🎆️26/🌙️08/☀️20/INTERACTIVE-JOB-RUNTIME-REFACTOR/🧪️native-artifacts' SEMIO_BUILD_BUDGET_MS=3600000 RUST_BACKTRACE=1 bun x nx run @semio-tech/framework-plugin:test --skip-nx-cache --args='--no-run' 2>&1 | tee '/Users/ueli/Documents/semio/.🧬semio/🦑️repo/🎫️tickets/🎆️26/🌙️08/☀️20/INTERACTIVE-JOB-RUNTIME-REFACTOR/🧪️member-plugin-native-inventory-r8-2026-08-27.txt'
```

## Exact Output

The [raw output](./🧪️member-plugin-native-inventory-r8-2026-08-27.txt) has 8,147 lines, copied directly below. Existing target, jobs2 and budgets were unchanged; no cleanup, repin, or generated shared output writes occurred.
...
warning: `semio-framework-plugin` (lib test) generated 590 warnings (run `cargo fix --lib -p semio-framework-plugin --tests` to apply 536 suggestions)
    Finished `test` profile [unoptimized] target(s) in 44.02s
