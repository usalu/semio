...
## Canonical Source Census After Exact Helper Binding

The canonical full census executed and wrote `📊️member-mounted-source-full-census-2026-08-27.json`; the command intentionally exits red because real failures remain. It reports773command rows,350source-admitted bounded rows,316batch-only rows,2forbidden rows, and269remaining live registrations. This is source admission evidence, not all-app native or browser completion. The823source self-tests include exact real-source joins and hostile removal/replacement cases for synchronous move-only publication, dispatch-to-start preparation, missing generic reducer denial, mounted maintenance, retained ephemeral preparation, and extracted Store admission without hidden replay.

The still-red ledger includes missing child-root retirement authority, Jack/Trinity envelope-owner gaps, close/peer/interaction ownership,16process-global candidates,6missing publication contracts,4missing exact Store preparation authorities,35unmigrated media factories, and269remaining live commands. Generic whole-decode/drop/preparation exceptions were not introduced. The full stdout/stderr is retained as `🧪️member-mounted-source-full-census-2026-08-27.txt`.

Native r10 did not execute tests: Cargo failed writing `debug/.fingerprint/semio-framework-ui-11743043be8af4e9/dep-lib-semio_framework_ui` because the directory disappeared. Read-only inspection confirmed the entire warm target fingerprint directory was absent, with114GiBfree disk. The captured evidence is `🧪️member-latestwins-registered-r10-native-2026-08-27.txt`. No deletion or cleanup command was issued by this executor. Native retries are paused pending the coordinator's shared artifact-state investigation.
