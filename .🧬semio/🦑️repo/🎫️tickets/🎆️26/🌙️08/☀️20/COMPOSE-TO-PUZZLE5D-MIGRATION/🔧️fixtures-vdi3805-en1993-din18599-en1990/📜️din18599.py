...
def camel(name):
    """🐫️ serde's `RenameRule::CamelCase` for a FIELD: PascalCase, then lowercase the first char."""
    pascal = "".join(part[:1].upper() + part[1:] for part in name.split("_"))
    return pascal[:1].lower() + pascal[1:]


def variant(name):
    """🐫️ serde's `RenameRule::CamelCase` for a VARIANT: lowercase the first char only."""
    return name[:1].lower() + name[1:]


# 🏷️ `Din18599Mutation` is `#[serde(tag = "mutation", rename_all = "camelCase")]` — INTERNALLY
# tagged, so a payload encodes as `{"mutation":"changeHT","newHT":118.0}`: the camelCased variant
# name under the `mutation` key, with the payload struct's own (also camelCased) fields flattened
# alongside it. Every one of this artifact's payload structs carries `#[serde(rename_all =
# "camelCase")]` of its own.
...
def case(leaf, kind, module, variant_name, name, summary, field, payload_key, value, extra_applied, extra_diff):
    ...
    CASES.append(
        dict(
            ...
            summary=summary,
            after=after,
            mutation={"mutation": variant(variant_name), camel(payload_key): value},
            diff=diff,
...
REJECTED_CASE = dict(
    ...
    mutation={"mutation": "updateClimate", "newClimate": BAD_CLIMATE},
