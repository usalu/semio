...
/// @emoji 📖️ Parses one `.grammar` file. Protocol dialect sources project through [`parse_protocol`].
...
/// @emoji 📡️ Parses one `.protocol.semio` file into a typed [`ProtocolFile`].
        ...
        match head.text.as_str() {
            ...
            "chain" => {
                ...
                if cursor.peek().kind == GKind::Ident {
                    ...
                    let prim_names = ["u8", "u16", "u32", "u64", "i32", "i64", "f32", "f64", "varint", "zigzag", "bytes", "utf8", "tag", "fixed", "Fixed", "Array", "Ref"];
...
/// @emoji ♻️ `canonicalize(canonicalize(x)) == canonicalize(x)` — protocol sources round-trip through
/// [`parse_protocol`]/[`print_protocol`].
