#!/usr/bin/env bun
/** [DEBUG] P6: inject remaining DslDocument/DslOps handcrafted codecs (line-anchored derive match). */
...
const report = { injectedDocs: [], injectedOps: [], skippedDocs: [], skippedOps: [], errors: [] };
...
function hasImplFor(content, traitName, typeName) {
  return new RegExp(`impl(?:\\s*<[^>]*>)?\\s+(?:[\\w:]+::)*${traitName}\\s+for\\s+${typeName}\\b`).test(content);
}
...
function findSites(content) {
  ...
  for (let li = 0; li < lines.length; li++) {
    ...
    // only real attributes at line start (after whitespace), not doc/comments
    ...
    // look ahead for struct/enum name
    let name = null;
    let kind = null;
    ...
    if (!name) {
      report.errors.push({ line: li + 1, attrs, err: "no type after derive" });
    } else {
      sites.push({ hasDoc, hasOps, name, kind, line: li + 1 });
    }
    offset += line.length + 1;
  }
  return sites;
...
function documentImplBlockOs(typeName) {
  // for store / dsl kernel using crate::os_* and local trait names
  return `
//#region 🔖️DocumentCodec
/// 📜️ Handcrafted DocumentDsl (P6).
impl crate::os_store::DocumentDsl for ${typeName} {
    const EXTENSION: &'static str = Self::__DSL_EXTENSION;
    fn envelope_id() -> &'static str {
        Self::__DSL_ENVELOPE_ID
    }
...
impl crate::os_store::DocumentPack for ${typeName} {
...
function opsImplBlockOs(typeName) {
  return `
//#region 🔖️OpCodec
/// 🎞️ Handcrafted OpText (P6).
impl crate::os_spr::OpText for ${typeName} {
...
/// 🎯️ Handcrafted OpBinary (P6).
impl crate::os_spr::OpBinary for ${typeName} {
...
function processFile(absPath, mode) {
  ...
  let changed = false;
  const sites = findSites(content);
  for (const site of sites) {
    if (site.hasDoc) {
      if (hasImplFor(content, "DocumentDsl", site.name)) {
        report.skippedDocs.push({ rel, name: site.name });
      } else {
        const b = mode === "plugin" ? documentImplBlockPlugin(site.name) : documentImplBlockOs(site.name);
    ...
    if (site.hasOps) {
      if (hasImplFor(content, "OpText", site.name)) {
        ...
        const b = mode === "plugin" ? opsImplBlockPlugin(site.name) : opsImplBlockOs(site.name);
        const next = injectAfterType(content, site.name, b);
...
// store uses local DocumentDsl names via use - prefer store-local blocks from previous script style
// Re-run store with os paths may double-inject if already done; hasImplFor guards.
try { processFile(STORE_FILE, "store"); } catch (e) { report.errors.push({ file: "store", err: String(e) }); }

// dsl component: uses store:: and protocol:: via reexports often as DocumentDsl - check
try { processFile(DSL_FILE, "store"); } catch (e) { report.errors.push({ file: "dsl", err: String(e) }); }

writeFileSync(join(ticket, "🧪p6-inject-remaining-report.json"), JSON.stringify(report, null, 2));
console.log(JSON.stringify({
  ...
  errorSample: report.errors.slice(0, 15),
  injectedDocSample: report.injectedDocs.slice(0, 10),
  injectedOpsSample: report.injectedOps.slice(0, 10),
}, null, 2));
