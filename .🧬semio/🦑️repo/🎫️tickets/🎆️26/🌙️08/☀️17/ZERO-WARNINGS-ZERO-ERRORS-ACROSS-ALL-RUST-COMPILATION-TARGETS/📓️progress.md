...
## Remaining work (large, not completed this session)
- `cargo check --workspace --all-targets --keep-going` running to get complete, current data for
  the ~94 non-blocked crates (was in progress in-background as of this note).
- `stdio` plugin (originally ~988 warnings) not yet re-verified post-cargo-fix; still needs the
  dead-code triage pass per the method above — this is the single biggest remaining chunk.
- The 3 pre-existing broken crates above are out of scope for this ticket unless/until the other
  session's migration lands (or the dev explicitly asks to take them over).
- Full wasm32 target builds not yet attempted (native `cargo check` only so far).
