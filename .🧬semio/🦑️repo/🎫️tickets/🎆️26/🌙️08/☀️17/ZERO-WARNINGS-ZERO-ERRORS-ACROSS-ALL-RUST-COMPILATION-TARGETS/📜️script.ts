...
if (["native-start", "native-worker", "native-cancel", "native-protect", "native-lease"].includes(command)) {
  ...
  if (command === "native-start" || command === "native-protect") {
    ...
    const worker = spawn(process.execPath, [import.meta.path, protection ? "native-lease" : "native-worker", name], { cwd: process.cwd(), env: process.env, detached: true, stdio: ["ignore", stdout, stderr] });
    closeSync(stdout);
    closeSync(stderr);
...
if (command === "laws") {
  ...
  const groups = [
    ...
    {"package":"semio-s-artifact-stdio-gltf","laws":["mutation_rejection_messages_match_the_language_neutral_json_oracle","mutation_restore_preserves_the_language_neutral_wire_and_inverse"],"cargoArgs":["--features","semio-s-artifact-stdio-gltf/component-app-assembly"]},
    ...
    {"package":"semio-s-artifact-puzzle-3d","laws":["spatial_capacity_plus_one_refusal_preserves_exact_old_state","spatial_stale_owner_cannot_finish_partial_replacement","spatial_multi_cell_oversized_replacement_and_removal_make_bounded_progress","spatial_fixed_collections_use_the_credited_pages_and_return_identical_plus_one_owners","overlap_checkpoint_resumes_exact_rng_and_sample_cursor","overlap_is_deterministic_across_batch_sizes","blocked_vortex_full_ids_and_enumeration_excludes_them","weighted_sample_without_replacement_edge_cases"],"cargoArgs":["--features","semio-s-artifact-puzzle-3d/component-app-assembly"]},
    ...
    {"package":"semio-s-artifact-fem-2d","laws":["vector_layer_vectors_match_the_json_oracle","process_owner_inventory_admits_exact_maximum_and_returns_exact_credit"],"cargoArgs":["--features","semio-s-artifact-fem-2d/component-app-assembly"]},
    ...
    {"package":"semio-s-artifact-mathematical-equation","laws":["language_neutral_mutations_match_json_oracle_and_restore_base"]},
