...
if (["laws", "laws-fleet"].includes(command)) {
  ...
  const groups = [
    ...
    {"package":"semio-s-artifact-fem-2d","laws":["bounded_mesh_plus_one_fault_retains_the_exact_domain_for_cursor_close","mesh_mounted_classification_indexes_admit_maximum_reject_plus_one_and_close_exactly","vector_layer_vectors_match_the_json_oracle","process_owner_inventory_admits_exact_maximum_and_returns_exact_credit","admitted_maximum_and_production_grant_make_bounded_progress"],"cargoArgs":["--features","semio-s-artifact-fem-2d/component-app-assembly"]},
    ...
    {"package":"semio-s-artifact-trinity-jack","laws":["query_ownership_resumable_matches_neutral_results_and_single_mutation_publication","query_preparation_layout_matches_neutral_budget","query_ownership_cancelled_preparation_closes_while_source_scene_remains_live","query_ownership_preparation_rejects_oversized_node_and_edge_before_clone","query_ownership_output_admission_rejects_oversized_table_before_publication","editor::jack::config::component::contract_vectors::configuration_and_presence_contract_vectors_match_the_json_oracle","editor::jack::presence::component::contract_vectors::configuration_and_presence_contract_vectors_match_the_json_oracle"],"cargoArgs":["--features","semio-s-artifact-trinity-jack/component-app-assembly"]},
  ...
  if (command === "laws-fleet") {
    ...
    const batches = [...new Set(groups.map(group => JSON.stringify("target" in group ? group.target : { kind: "lib" })))].map(key => {
      ...
      return { target, groups: selected, args: ["test", ...(target.kind === "lib" ? ["--lib"] : ["--" + target.kind, target.name]), ...selected.flatMap(group => ["-p", group.package]), ...(features.length ? ["--features", [...new Set(features)].sort().join(",")] : []), "--no-run", "--message-format=json", "-j2"] };
    });
