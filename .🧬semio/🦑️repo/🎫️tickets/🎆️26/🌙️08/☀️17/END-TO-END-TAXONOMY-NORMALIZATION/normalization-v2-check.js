...
function publishCanonicalJsonCandidate(container, finalName, value, preparationName) {
...
`, "utf8");
  ...
  if (lstatOrNull(final)) {
    if (!lstatSync(final).isFile() || lstatSync(final).isSymbolicLink() || !readFileSync2(final).equals(readFileSync2(leaf)))
      throw new Error(`Canonical JSON destination is occupied: ${final}`);
  } else
    durableRename(leaf, final);
  durableRemove(root, true);
}
function recoverCanonicalJsonCandidates(container, finalName, preparationName, validate, serialized, validateOnly = false) {
  ...
  for (const name of readdirSync2(container).sort(generatorPathCompare)) {
    ...
    if (children.length > 1 || children.length === 1 && children[0] !== finalName)
    ...
    if (children.length === 0) {
      ...
      actions.push({ root, publish: false });
      continue;
    }
    const leaf = join2(root, finalName), leafStat = lstatOrNull(leaf);
    ...
    if (finalStat && (!finalStat.isFile() || finalStat.isSymbolicLink() || !readFileSync2(final).equals(readFileSync2(leaf))))
      throw new Error(`Canonical JSON write candidate conflicts with durable evidence: ${leaf}`);
    actions.push({ root, leaf, publish: !finalStat });
  }
  if (actions.length > 1)
  ...
  if (!validateOnly)
    for (const action of actions) {
      if (action.publish)
        durableRename(action.leaf, final);
...
function acquireTransactionLease(repoRoot, attemptRelative, backupRelative, leaseDirectory, leasePreparationName, jsonWritePreparationName, filename, planDigest, attemptOrdinal, beforePublish) {
  ...
  const leasePreparations = () => {
    ...
    for (const name of readdirSync2(backupRoot).sort(generatorPathCompare)) {
      ...
      recoverCanonicalJsonCandidates(root, filename, jsonWritePreparationName, (path) => {
  ...
  try {
    beforePublish?.();
    publishCanonicalJsonCandidate(preparing, filename, record2, jsonWritePreparationName);
    ...
    for (const preparation of leasePreparations()) {
      recoverCanonicalJsonCandidates(preparation.root, filename, jsonWritePreparationName, (path) => {
    ...
    if (lstatOrNull(preparing)) {
      ...
      recoverCanonicalJsonCandidates(preparing, filename, jsonWritePreparationName, (path) => {
...
function persistJournal(repoRoot, path, journal) {
  ...
  publishCanonicalJsonCandidate(walRoot, basename2(walPath), journalSnapshot(next), journal.jsonWritePreparationName);
  durableRename(walPath, path);
  journal.revision = next.revision;
  durableRemove(walRoot, true);
}
function readJournal(path, journalWriteDirectory, jsonWritePreparationName) {
  const bytes = readFileSync2(path, "utf8");
...
function reconcileJournalWal(repoRoot, path, current, plan, taxonomy, validateOnly = false) {
  ...
  recoverCanonicalJsonCandidates(walRoot, basename2(path), current.jsonWritePreparationName, (candidatePath) => {
    const candidate = readJournal(candidatePath, current.journalWriteDirectory, current.jsonWritePreparationName);
  ...
  const canonicalChildren = children.filter((entry) => entry === basename2(path));
  if (validateOnly && canonicalChildren.length === 0)
    return current;
  if (children.length === 0) {
    if (!validateOnly)
      durableRemove(walRoot, true);
  ...
  if (!validateOnly && canonicalJson(children) !== canonicalJson([basename2(path)]) || canonicalChildren.length !== 1)
  ...
  const next = readJournal(walPath, current.journalWriteDirectory, current.jsonWritePreparationName);
...
function validateLeasePreparationEvidence(backupRoot, leasePreparationName, jsonWritePreparationName, filename, planDigest, attemptOrdinal) {
  for (const name of readdirSync2(backupRoot).sort(generatorPathCompare)) {
    const match = /^lease-([1-9][0-9]*)-([0-9a-f-]+)-(preparing|stale)$/u.exec(splitLeadingEmoji(name).rest);
    if (!match)
      continue;
...
`);
  ...
  if (attemptsStat) {
    ...
    for (const name of readdirSync2(attemptsAbsolute).sort(generatorPathCompare)) {
      ...
      const attemptJournal = readJournal(attemptJournalAbsolute, journalWriteDirectory, journalJsonWritePreparationName);
  ...
  const validateUnpublishedAttempt = (preparation) => {
    ...
    for (const name of children) {
      ...
      if (name === journalFilename) {
        ...
        assertPreparedIdentity(readJournal(path, journalWriteDirectory, journalJsonWritePreparationName), path);
      } else {
        if (!stat?.isDirectory() || stat.isSymbolicLink())
          throw new Error(`Dead attempt preparation child is not a no-follow directory: ${path}`);
        if (name === leaseDirectory) {
          recoverCanonicalJsonCandidates(path, journalFilename, leaseJsonWritePreparationName, (candidate) => {
          ...
          recoverCanonicalJsonCandidates(wal, journalFilename, journalJsonWritePreparationName, (candidate) => assertPreparedIdentity(readJournal(candidate, journalWriteDirectory, journalJsonWritePreparationName), candidate), false, true);
          ...
          if (walChildren.length > 0) {
            ...
            if (lstatOrNull(walPath)) {
              ...
              assertPreparedIdentity(readJournal(walPath, journalWriteDirectory, journalJsonWritePreparationName), walPath);
            }
          }
  ...
  for (const attempt of existingAttempts) {
    ...
    if (hasResidue || lstatOrNull(leaseAbsolute)) {
      ...
      terminalLease = acquireTransactionLease(repoRoot, attempt.attemptRelative, attempt.journal.backupRoot, leaseDirectory, leasePreparationName, leaseJsonWritePreparationName, journalFilename, digest, attempt.ordinal);
  ...
  const acquireLease = (backupRoot, beforePublish) => {
    leaseHandle = acquireTransactionLease(repoRoot, attemptRelative, backupRoot, leaseDirectory, leasePreparationName, leaseJsonWritePreparationName, journalFilename, digest, attemptOrdinal, beforePublish);
  };
  const releaseLease = () => {
    if (!leaseHandle)
      return;
  ...
  if (options.resumeJournal) {
    try {
      ...
      if (journal.state !== "committed" && journal.state !== "rolled-back") {
        const validateRecoveryEvidence = () => {
          ...
          validateLeasePreparationEvidence(absolutePath(repoRoot, journal.backupRoot), leasePreparationName, leaseJsonWritePreparationName, journalFilename, digest, attemptOrdinal);
    ...
    try {
      ...
      publishCanonicalJsonCandidate(preparationLease, journalFilename, leaseRecord, leaseJsonWritePreparationName);
      ...
      publishCanonicalJsonCandidate(initialWalRoot, journalFilename, journalSnapshot(journal), journalJsonWritePreparationName);
