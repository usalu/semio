...
function validateTaxonomy(taxonomy = readTaxonomyUnchecked()) {
  ...
  if (record(taxonomy.semanticDirectoryKinds, "semanticDirectoryKinds"))
    for (const [id, spec] of Object.entries(taxonomy.semanticDirectoryKinds)) {
      ...
      ids(spec.parentKindIds ?? [], { ...taxonomy.semanticDirectoryKinds, ...taxonomy.semanticProjectedMemberKinds }, `semanticDirectoryKinds[${JSON.stringify(id)}].parentKindIds`);
    }
  ...
  if (record(taxonomy.pathExclusions, "pathExclusions")) {
    ...
    if (entries.length !== 1 || entries[0]?.[0] !== "compose")
      problems.push('pathExclusions must contain only "compose".');
  ...
  if (taxonomy.areaEnforcement?.opaquePathExclusionIds?.join("\x00") !== "compose")
    problems.push('areaEnforcement.opaquePathExclusionIds must be exactly ["compose"].');
  if (record(taxonomy.areas, "areas"))
    for (const [area, state] of Object.entries(taxonomy.areas)) {
      if (area === "compose" || area.startsWith("compose/"))
        problems.push("compose must exist only in pathExclusions.");
      if (state !== "clean")
  ...
  if (record(taxonomy.areaLayers, "areaLayers"))
    for (const [area, layer] of Object.entries(taxonomy.areaLayers)) {
      if (area === "compose" || area.startsWith("compose/"))
        problems.push("compose must not appear in areaLayers.");
...
`);
  const start2 = lines.findIndex((line) => line.trim() === header);
  if (start2 === -1)
    return;
  const body = [];
  for (let i = start2 + 1;i < lines.length; i++) {
...
function parseTaxonomy(raw, path) {
  ...
  for (const [id, value] of Object.entries(directoryKindRows)) {
    ...
    semanticDirectoryKinds[id] = { emoji, slugPattern, allowEmojiOnly: spec.allowEmojiOnly, ...spec.parentKindIds === undefined ? {} : { parentKindIds: stringArray(spec.parentKindIds, `semanticDirectoryKinds.${id}.parentKindIds`) } };
  }
...
function jsonTokens(path, content, adapter) {
  ...
  for (const match of content.matchAll(/"((?:\\.|[^"\\])*)"/g)) {
    ...
    if (workspaceGlob)
      rows.push({ adapter, structuredLocation: `${key ? "/@key" : "/@value"}[${Math.max(0, ordinal - 1)}]/workspace-glob@${start2}`, start: start2, end: start2 + raw.length, value, targetValues: [workspaceGlob[1]], rewriteKind: "path-prefix", rewriteData: { prefix: "{workspaceRoot}/", suffix: workspaceGlob[2] } });
    if (!key && raw !== value && /^\{workspaceRoot\}\/.+\/\*\*/u.test(value))
      rows.push({ adapter, structuredLocation: `/@value[${Math.max(0, ordinal - 1)}]/workspace-glob@${start2}`, start: start2, end: start2 + raw.length, value: raw, rewriteKind: "path-prefix", unsupportedReason: "Escaped workspace projection glob has no proven decoded-to-raw offset map" });
    if (!key && raw === value)
...
function rustTokens(path, content) {
  ...
  for (const match of content.matchAll(/\.join\(\s*"([^"]*\uD83D\uDDBC\uFE0Fassets\/\uD83C\uDFD7\uFE0FmodelDefinitions)"\s*\)/gu)) {
    if (match.index === undefined)
      continue;
    const start2 = match.index + match[0].indexOf(match[1]);
    rows.push({ adapter: "rust", structuredLocation: lineLocation(content, start2, "artifact-catalog-root-join"), start: start2, end: start2 + match[1].length, value: match[1], rewriteKind: "artifact-catalog-prose", rewriteData: { form: "relative-root" } });
  }
  ...
  for (const match of content.matchAll(/(?:^|\n)[ \t]*(?:(?:\/\/\/)|(?:\/\/!)|(?:\/\/))([^\r\n]*)/gu)) {
    ...
    for (const quoted of fragment.matchAll(/`([^`]+)`/gu)) {
      if (quoted.index === undefined || !/[/.]/u.test(quoted[1]))
        continue;
      const tokenStart = start2 + quoted.index + quoted[0].indexOf(quoted[1]);
    ...
    if (catalog) {
      const tokenStart = start2 + fragment.indexOf(catalog[1]);
...
function typescriptTokens(path, content) {
  ...
  for (const match of content.matchAll(/^([^\r\n]*\bimport\.meta\.glob\s*\([^\r\n]+(?:\r?\n|$))/gmu)) {
    ...
    rows.push({ adapter: "typescript", structuredLocation: lineLocation(content, start2, "artifact-catalog-glob"), start: start2, end: start2 + match[1].length, value: match[1], rewriteKind: "artifact-catalog-glob", rewriteData: { selectors: JSON.stringify(selectors) } });
  }
...
function buildReferenceEdits(inventory, moves, taxonomy, options, known) {
  ...
  for (const context of artifactContexts) {
    ...
    for (const required of requirements) {
      const matches = concrete.filter((edit) => edit.path === required.path && edit.adapter === required.adapter && semanticLocationMatches(edit.structuredLocation, required.structuredLocation) && edit.oldValue === required.oldValue && edit.newValue === required.newValue && edit.preimageHash === required.preimageHash);
      if (matches.length !== 1)
...
function inventoryTaxonomy(options) {
  ...
  const activeExclusions = taxonomy.exclusions.filter((excluded) => existsSync2(absolutePath(repoRoot, excluded.path))).map((entry) => entry.path);
...
function generatorNodeRecord(repoRoot, path, taxonomy) {
  ...
  const contentHash = nodeKind === "directory" ? sha256("directory") : nodeKind === "symlink" ? sha256(readlinkSync(absolute)) : sha256(readFileSync2(absolute));
...
function planTaxonomy(inventory, options) {
  ...
  const preliminaryMoves = inventory.entries.filter((entry) => entry.nodeKind !== "directory" && entry.sourcePath !== entry.normalizedPath && generatorContractsForOutputPath(entry.sourcePath, taxonomy).length === 0).map((entry) => ({
    ...
    referenceEdits: []
  })).sort((a, b) => generatorPathCompare(a.sourcePath, b.sourcePath) || generatorPathCompare(a.destinationPath, b.destinationPath));
...
function opaqueNodeDigest(path, counts) {
  ...
  if (stat.isFile()) {
    counts.files++;
    return sha256(Buffer.concat([Buffer.from(`file\x00${mode}\x00`), readFileSync2(path)]));
  }
  if (stat.isDirectory()) {
    counts.directories++;
    const children = readdirSync2(path).sort((a, b) => Buffer.from(a).compare(Buffer.from(b))).map((name) => `${Buffer.from(name).toString("hex")}\x00${opaqueNodeDigest(join2(path, name), counts)}`);
  ...
  try {
    ...
    for (let index = 0;index < sortedEditGroups.length; index++) {
      ...
      if (!journal.appliedEditPaths.includes(path)) {
        ...
        const result = applyEditsToContent(readFileSync2(target, "utf8"), edits);
