...
function parseTaxonomy(raw, path) {
  ...
  for (const [id, spec] of Object.entries(semanticDirectoryKinds))
    for (const parentId of spec.parentKindIds ?? [])
      if (!semanticDirectoryKinds[parentId])
  ...
  for (const [id, value] of Object.entries(generatorRows)) {
    ...
    if (!["owned", "unsafe", "unknown", "external"].includes(String(spec.ownership)))
    ...
    if ((ownership === "owned" || ownership === "unsafe") !== (ownerPath !== null && target !== null))
    ...
    generatorContracts[id] = { ownership, ownerPath, target, checkTarget, inputPatterns: [...new Set(inputPatterns)].sort(), outputRoots, reason: requiredString(spec.reason, `generatorContracts.${id}.reason`) };
  }
...
function referenceTokens(path, content) {
  ...
  if (lower.endsWith(".py"))
    return regexTokens(content, "python", "python-reference", [/^\s*from\s+([\w.]+)\s+import\s+/gmu, /^\s*import\s+([\w.]+)(?:\s+as\s+\w+)?\s*$/gmu, /\b(?:open|Path|joinpath|files|read_text|read_bytes)\s*\(\s*["']([^"']+)["']/gu, /__file__[^\r\n]*?\/\s*["']([^"']+)["']/gu]);
...
function textualPath(path) {
  return /(?:\.rs|\.tsx?|\.jsx?|\.mjs|\.cjs|\.mts|\.cts|\.go|\.mod|\.work|\.py|\.cs|\.fs|\.vb|\.csproj|\.fsproj|\.vbproj|\.sln|\.props|\.targets|\.c|\.cc|\.cpp|\.cxx|\.h|\.hh|\.hpp|\.hxx|\.cmake|\.jsonc?|\.toml|\.ya?ml|\.xml|\.html?|\.mdx?)$/iu.test(path) || basename2(path) === "CMakeLists.txt";
}
  ...
  for (let index = 0;index < candidates.length; index++) {
    ...
    for (const { token, target: oldTarget } of supported) {
      ...
      if (!oldTarget || !destination)
        continue;
      const newValue = rewriteReferenceToken(finalReferencePath, entry.sourcePath, token, oldTarget, destination);
...
function planTaxonomy(inventory, options) {
  ...
  const preliminaryMoves = inventory.entries.filter((entry) => entry.nodeKind !== "directory" && entry.sourcePath !== entry.normalizedPath && generatorContractsForOutputPath(entry.sourcePath, taxonomy).length === 0).map((entry) => ({
    ...
    rationaleRule: entry.semanticStem ? "semantic-stem-resolution" : entry.fixedContractId ? "fixed-contract-preservation" : "canonical-kind-name",
    ownerId: entry.ownerId,
