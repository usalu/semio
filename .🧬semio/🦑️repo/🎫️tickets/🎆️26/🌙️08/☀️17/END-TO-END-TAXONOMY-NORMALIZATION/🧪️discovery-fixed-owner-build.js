...
function taxonomyCliEditPreparationProblems(facts, taxonomy = loadTaxonomy()) {
  const problems = [];
  const kindId = semanticDirectoryKindId(facts.directoryName, taxonomy, { parentKindId: facts.parentKindId });
  if (kindId !== "transaction-edit-preparation")
    problems.push("Edit preparation directory must resolve below transaction-stage.");
  const operationHash = facts.directoryName.match(/^\uD83D\uDEA7\uFE0Fedit-([0-9a-f]{24})-/u)?.[1];
  if (!operationHash)
    problems.push("Edit preparation directory must carry one lowercase 24-hex operation hash.");
  const allowed = operationHash ? new Map([[`${operationHash}.edit`, "transaction-edit-candidate"], [`${operationHash}.pre`, "transaction-edit-preimage"]]) : new Map;
  if (new Set(facts.candidateLeafNames).size !== facts.candidateLeafNames.length)
    problems.push("Edit preparation leaves must be unique.");
  for (const leaf of facts.candidateLeafNames) {
    const expectedKindId = allowed.get(leaf);
    if (!expectedKindId)
      problems.push(`Edit preparation leaf ${JSON.stringify(leaf)} is not hash-bound to its directory.`);
    else if (scopedFileKindIdForSourcePath(`\uD83D\uDEA7\uFE0Fstage/${facts.directoryName}/${leaf}`, taxonomy, { parentDirectoryKindId: kindId ?? undefined }) !== expectedKindId)
      problems.push(`Edit preparation leaf ${JSON.stringify(leaf)} has no scoped exchange authority.`);
  }
  return problems;
}
function taxonomyCliRestorePreparationProblems(facts, taxonomy = loadTaxonomy()) {
  const problems = [];
  const kindId = semanticDirectoryKindId(facts.directoryName, taxonomy, { parentKindId: facts.parentKindId });
  if (kindId !== "transaction-restore-preparation")
    problems.push("Restore preparation directory must resolve below transaction-backup.");
  const operationHash = facts.directoryName.match(/^\uD83D\uDEA7\uFE0Frestore-([0-9a-f]{24})-/u)?.[1];
  if (!operationHash)
    problems.push("Restore preparation directory must carry one lowercase 24-hex operation hash.");
  const allowed = operationHash ? new Set([`${operationHash}.backup`, `${operationHash}.post`]) : new Set;
  if (new Set(facts.leafNames).size !== facts.leafNames.length)
    problems.push("Restore preparation leaves must be unique.");
  for (const leaf of facts.leafNames) {
    if (!allowed.has(leaf))
      problems.push(`Restore preparation leaf ${JSON.stringify(leaf)} is not hash-bound to its directory.`);
    ...
    if (scopedId !== "transaction-backup-candidate" && scopedId !== "transaction-postimage-candidate")
      problems.push(`Restore preparation leaf ${JSON.stringify(leaf)} has no scoped candidate authority.`);
  }
  return problems;
}
function taxonomyCliJsonWritePreparationProblems(facts, taxonomy = loadTaxonomy()) {
  ...
  if (kindId !== "transaction-json-write-preparation")
    problems.push("JSON write preparation must resolve below transaction-journal-write or transaction-lease-preparation.");
...
function semanticOwnedFileProjectionAuthority(facts, taxonomy = loadTaxonomy()) {
  if (!Number.isSafeInteger(facts.sourceByteLength) || facts.sourceByteLength < 0)
    throw new Error("sourceByteLength must be a non-negative safe integer.");
  const entries = Object.entries(taxonomy.semanticOwnedFileProjectionContracts);
  if (entries.length !== 1)
    throw new Error("Exactly one semantic owned-file projection contract is required.");
  const [contractId, contract] = entries[0];
  const contentState = facts.sourceByteLength === 0 ? "zero-byte" : "nonzero";
  const core = { contractId, ownerPath: facts.ownerPath, manifestPath: facts.manifestPath, manifestContentHash: createHash("sha256").update(facts.manifestContent).digest("hex"), sourcePath: facts.sourcePath, contentState };
  const manifestContract = taxonomy.fixedFilenameContracts[contract.requiredSiblingFixedFilenameContractId];
  const claimed = facts.ownerFixedDirectoryContractIds.includes(contract.ownerFixedDirectoryContractId) && facts.manifestFixedFilenameContractIds.includes(contract.requiredSiblingFixedFilenameContractId) && dirname(facts.sourcePath) === facts.ownerPath && basename(facts.sourcePath) === contract.sourceFilename && facts.sourceFileKindId === contract.sourceFileKindId && dirname(facts.manifestPath) === facts.ownerPath && manifestContract !== undefined && basename(facts.manifestPath) === fixedContractFilename(manifestContract);
  if (!claimed)
    return { ...core, disposition: "unclaimed", problems: [] };
  let status;
  try {
    const manifest = JSON.parse(facts.manifestContent);
    if (typeof manifest === "object" && manifest !== null && !Array.isArray(manifest) && Object.hasOwn(manifest, contract.manifestStatusLocation)) {
      const candidate = manifest[contract.manifestStatusLocation];
      if (candidate === "closed" || candidate === "open")
        status = candidate;
    }
  } catch {}
  if (!status)
    return { ...core, disposition: "problem", problems: ["Ticket manifest must own an explicit status equal to closed or open."] };
  const destinationPath = `${facts.ownerPath}/${contract.destinationDirectoryName}/${contract.destinationFilename}`;
  if (status === "open")
    return { ...core, destinationPath, status, disposition: "project", problems: [] };
  if (contentState === "zero-byte")
    return { ...core, status, disposition: "remove", problems: [] };
...
function fixedScopeMatches(contract, path, context) {
  ...
  if (contract.scope.kind === "fixed-directory-contract")
    return context.parentFixedDirectoryContractIds?.includes(contract.scope.fixedDirectoryContractId) === true;
  if (contract.scope.kind === "sibling-fixed-filename-contract")
    return context.siblingFixedFilenameContractIds?.includes(contract.scope.fixedFilenameContractId) === true;
  return true;
}
...
function validateTaxonomy(taxonomy = readTaxonomyUnchecked()) {
  ...
  const taxonomyCliArtifactDirectoryKinds = {
    ...
    "transaction-stage": { name: "\uD83D\uDEA7\uFE0Fstage", emoji: "\uD83D\uDEA7\uFE0F", slugPattern: "^stage$", parentKindIds: ["transaction-attempt"] },
    "transaction-backup": { name: "\uD83D\uDCBE\uFE0Fbackup", emoji: "\uD83D\uDCBE\uFE0F", slugPattern: "^backup$", parentKindIds: ["transaction-attempt"] },
    "transaction-lease": { name: "\uD83D\uDD12\uFE0Flease", emoji: "\uD83D\uDD12\uFE0F", slugPattern: "^lease$", parentKindIds: ["transaction-attempt"] },
    ...
    "transaction-json-write-preparation": { name: "\uD83D\uDEA7\uFE0Fwrite-42-123e4567-e89b-42d3-a456-426614174000", emoji: "\uD83D\uDEA7\uFE0F", slugPattern: "^write-[1-9][0-9]*-[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$", parentKindIds: ["transaction-journal-write", "transaction-lease-preparation"] },
  ...
  if (semanticDirectoryKindId("\uD83D\uDEA7\uFE0Fwrite-42-123e4567-e89b-42d3-a456-426614174000", taxonomy, { parentKindId: "transaction-lease-preparation" }) !== "transaction-json-write-preparation")
    problems.push("semanticDirectoryKinds.transaction-json-write-preparation must resolve below both exact parent kinds.");
...
function classifyPackageSourceRole(content, grammar) {
  ...
  if (grammar.analyzer === "rust") {
    ...
    if (/^(?:[A-Za-z_]\w*::)*[A-Za-z_]\w*!\s*\([^{}]*\)$/u.test(rest2))
      return "registration";
      ...
      for (const entry of readdirSafe(dir).sort((a, b) => a.name.localeCompare(b.name))) {
        ...
        if (entry.isDirectory()) {
          if (DISCOVERY_SKIP_DIRS.has(entry.name))
            continue;
          if (existsSync(join(path, "CACHEDIR.TAG")) && /cache directory tag created by cargo/iu.test(readFileSync(join(path, "CACHEDIR.TAG"), "utf8")))
            continue;
...
export {
  ...
  semanticOwnedFileProjectionAuthority,
