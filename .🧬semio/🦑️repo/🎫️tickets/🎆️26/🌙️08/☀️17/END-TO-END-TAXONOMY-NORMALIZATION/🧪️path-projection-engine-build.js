...
function parseTaxonomy(raw, path) {
  ...
  for (const [id, value] of Object.entries(projectedMemberRows)) {
    ...
    if (spec.identityField !== "mutationDirectoryName")
  ...
  const parseDescendantNode = (value, name) => {
    ...
    const pathSegments = pathRows.map((value2, index) => {
      ...
      const kindId2 = requiredString(segment.kindId, `${name}.pathSegments[${index}].kindId`);
      const literal = requiredString(segment.literal, `${name}.pathSegments[${index}].literal`).normalize("NFC");
      const kind = semanticDirectoryKinds[kindId2];
      ...
      return { kindId: kindId2, literal };
    });
    const kindId = requiredString(spec.kindId, `${name}.kindId`);
    if (spec.nodeType === "directory" ? !allDirectoryContextIds.has(kindId) : !fileKinds[kindId])
      throw new Error(`Taxonomy v7 ${name} references unknown ${spec.nodeType} kind ${kindId}`);
    return { pathSegments, nodeType: spec.nodeType, kindId };
  };
  ...
  const expectedCatalogContract = { registryField: "vectors", required: true, allowEmpty: true, runtimeKindsField: "kinds", runtimeKindsRelation: "independent", mutationIdField: "mutationId", mutationDirectoryNameField: "mutationDirectoryName", scenariosField: "scenarios", scenarioIdField: "id", scenarioDirectoryNameField: "directoryName", physicalBundleUniquenessFields: ["mutationId", "mutationDirectoryName", "scenarioId"], coverage: "every-physical-bundle-exactly-once" };
  ...
  const captureFields = new Set(["standardVersion", "subsetId", "mutationId", "scenarioId"]);
  const parseProjectionSegment = (value, name, destination) => {
    ...
    if ((kindId ? 1 : 0) + (projectedMemberKindId ? 1 : 0) !== 1)
  ...
  for (const [id, value] of Object.entries(projectionRows)) {
    ...
    if (!Array.isArray(spec.sourceSegments) || !Array.isArray(spec.destinationSegments) || spec.rationaleRule !== "artifact-mutation-test-projection-v1")
    ...
    semanticPathProjectionContracts[id] = { sourceOwnerKindId, sourceSegments: spec.sourceSegments.map((segment, index) => parseProjectionSegment(segment, `semanticPathProjectionContracts.${id}.sourceSegments[${index}]`, false)), profileRendererId, destinationOwnerKindId, destinationSegments: spec.destinationSegments.map((segment, index) => parseProjectionSegment(segment, `semanticPathProjectionContracts.${id}.destinationSegments[${index}]`, true)), descendantContractId, catalogContractId, rationaleRule: "artifact-mutation-test-projection-v1" };
  }
...
function projectionCatalogVectors(path, source) {
    ...
    for (let catalogIndex = 0;catalogIndex < root.mutationCatalogs.length; catalogIndex++) {
      ...
      for (let vectorIndex = 0;vectorIndex < catalog.vectors.length; vectorIndex++) {
        ...
        for (let scenarioIndex = 0;scenarioIndex < vector.scenarios.length; scenarioIndex++) {
          ...
          const key = `${mutationId}\x00${mutationDirectoryName}\x00${scenarioId}`;
          if (seen.has(key))
            throw new Error(`Duplicate physical vector identity ${key.replaceAll("\x00", "/")}`);
          seen.add(key);
          vectors.push({ mutationId, mutationDirectoryName, scenarioId, scenarioDirectoryName });
        }
      }
    }
  } catch (error) {
  ...
  return { vectors: vectors.sort((left, right) => left.mutationDirectoryName.localeCompare(right.mutationDirectoryName) || left.scenarioDirectoryName.localeCompare(right.scenarioDirectoryName)) };
}
...
function mutationProjectionRationale(sourcePath, destinationPath) {
  ...
  return relativeDestination[0] === "\uD83E\uDDEA\uFE0Ftests" && relativeDestination[1] === `\uD83E\uDE86\uFE0F${structural.standard}-${structural.subset}` && emojiFold(relativeDestination[2] ?? "") === emojiFold(structural.mutation) ? "artifact-mutation-test-projection-v1" : null;
}
function projectMutationTestBundles(repoRoot, scope, entries, kinds, taxonomy) {
  ...
  const descendant = taxonomy.schema.semanticDescendantContracts[ids.descendantContractId];
  ...
  for (const [subsetRoot, subsetSources] of [...bySubset.entries()].sort(([left], [right]) => left.localeCompare(right))) {
    ...
    for (const vector of catalog.vectors) {
      const key = emojiFold(vector.mutationDirectoryName);
      vectorsByMutation.set(key, [...vectorsByMutation.get(key) ?? [], vector]);
    }
    const sourcesByMutation = new Map;
    for (const source of subsetSources) {
      const key = emojiFold(source.mutationDirectoryName);
    ...
    for (const [mutationKey, mutationSources] of sourcesByMutation) {
      ...
      const mutationName = canonicalProjectedMemberName(mutationSources[0].mutationDirectoryName, taxonomy);
      if (!mutationName || vectors.some((vector) => emojiFold(vector.mutationDirectoryName) !== mutationKey)) {
      ...
      for (const [source, vector] of assignments) {
        const vectorKey = `${vector.mutationId}\x00${vector.mutationDirectoryName}\x00${vector.scenarioId}`;
        ...
        if (mutation) {
          ...
          setProjectedPath(mutation, destinationSegments.slice(0, -1).reduce((path, segment) => `${path}/${segment}`, source.artifactRoot), taxonomy);
        }
      }
    }
    for (const vector of catalog.vectors) {
      const key = `${vector.mutationId}\x00${vector.mutationDirectoryName}\x00${vector.scenarioId}`;
      if (!consumed.has(key))
...
function generatorTreeInventory(repoRoot, roots, taxonomy) {
  ...
  for (const root of [...new Set(roots.map(normalizeRelative))].sort())
    walk(root);
...
function generatorPlanningViolations(inventory, moves, edits, taxonomy) {
  ...
  const rows = [];
  for (const [id, contract] of Object.entries(taxonomy.schema.generatorContracts).sort(([left], [right]) => left.localeCompare(right))) {
    const roots = contract.outputRoots.map((root) => root.path);
    ...
    if (!outputProblem && !outputMutation && !inputMutation)
      continue;
    const inputDigest = sha256(canonicalJson(generatorInputInventory(inventory, contract, taxonomy)));
    const preOutputDigest = sha256(canonicalJson(generatorTreeInventory(inventory.repoRoot, roots, taxonomy)));
    const path = roots[0];
    ...
    rows.push(violation("generator-preview-protocol-missing", path, `Owned generator ${id} (${contract.target}) is affected but has no read-only preview manifest of exact output paths, bytes, modes, and stale removals; input ${inputDigest}, output ${preOutputDigest}`));
  }
...
function planTaxonomy(inventory, options) {
  ...
  const preliminaryMoves = inventory.entries.filter((entry) => entry.nodeKind !== "directory" && entry.sourcePath !== entry.normalizedPath && generatorContractsForOutputPath(entry.sourcePath, taxonomy).length === 0).map((entry) => ({
    ...
    rationaleRule: mutationProjectionRationale(entry.sourcePath, entry.normalizedPath) ?? (entry.semanticStem ? "semantic-stem-resolution" : entry.fixedContractId ? "fixed-contract-preservation" : "canonical-kind-name"),
    ownerId: entry.ownerId,
  ...
  const unresolved = [
    ...
    ...generatorPlanningViolations(inventory, moves, references.edits, taxonomy)
  ];
  for (const group of groups)
  ...
  const provisional = {
    ...
    expectedPostStateDigest: affectedPostStateDigest({ moves, edits: references.edits, regenerations: [] }, references.resultHashes),
    planDigest: ""
  };
  const plan = { ...provisional, planDigest: taxonomyPlanDigest(provisional) };
  report(options.progress, "plan", "complete", moves.length + references.edits.length, moves.length + references.edits.length);
  return plan;
}
...
function assertGeneratorNodeRecords(records, roots, label) {
  ...
  if (records.some((record2, index) => index > 0 && records[index - 1].path.localeCompare(record2.path) > 0))
...
function assertNxTarget(repoRoot, ownerPath, target) {
  ...
  if (manifest.name !== project || !Object.hasOwn(record(manifest.targets, `Nx manifest ${manifestPath}.targets`), targetName))
...
function assertRegenerationContract(regeneration, taxonomy, repoRoot) {
  ...
  if (!contract || contract.ownership !== "owned" || !contract.ownerPath || !contract.target)
    throw new Error(`Regeneration ${regeneration.id} does not reference an owned generator contract`);
  const roots = contract.outputRoots.map((output) => output.path).sort();
  ...
  const identity = sha256(canonicalJson({ contractId: regeneration.contractId, cwd: regeneration.cwd, command: regeneration.command, verifyCommand: regeneration.verifyCommand, outputRoots: roots, inputs: regeneration.inputs, preOutputs: regeneration.preOutputs, outputs: regeneration.outputs })).slice(0, 24);
...
function assertPlanOutsideTransaction(plan, transactionRoot, taxonomy, repoRoot) {
  const paths = [
    ...
    ...plan.regenerations.flatMap((regeneration) => [...regeneration.outputRoots, ...regeneration.inputs.map((input) => input.path), ...regeneration.preOutputs.map((output) => output.path), ...regeneration.outputs.map((output) => output.path)])
  ];
