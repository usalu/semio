...
function validateTaxonomy(taxonomy = readTaxonomyUnchecked()) {
  ...
  if (record(taxonomy.semanticOwnedFileProjectionContracts, "semanticOwnedFileProjectionContracts")) {
    ...
    for (const [id, contract] of Object.entries(taxonomy.semanticOwnedFileProjectionContracts)) {
      ...
      if (contract.allowedStatuses?.join("\x00") !== "closed\x00open")
      ...
      if (!taxonomy.fileKinds[contract.sourceFileKindId] || fileKindIdForSourcePath(contract.sourceFilename, taxonomy) !== contract.sourceFileKindId)
      ...
      if (!destinationKind || destinationKind.projectionOnly !== true || semanticDirectoryKindId(contract.destinationDirectoryName, taxonomy) !== contract.destinationDirectoryKindId)
        problems.push(`${scope}.destination directory must resolve to one projectionOnly kind.`);
      if (contract.destinationFilename !== "\uD83D\uDCDD\uFE0F.md" || fileKindIdForSourcePath(contract.destinationFilename, taxonomy) !== contract.sourceFileKindId)
