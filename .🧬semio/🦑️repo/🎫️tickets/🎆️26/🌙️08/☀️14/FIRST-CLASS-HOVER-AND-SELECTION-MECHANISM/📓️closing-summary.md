...
`semio-framework-plugin` is 167 passed / 1 failed. The failure is **not from this work**:
`plugin_builder_contract_tests::view_action_emitting_ops_is_rejected` asserts that a `View`-kind command emitting document operations is rejected. The guard (`🔌️plugin/🦀️component.rs:7877-7881`) is intact and still matches `ActionKind::View | ActionKind::Shell`, but it only fires when `registry.get_command(verb)` resolves, and `get_command` searches `app_commands: HashMap<String, CommandDefinition>`. A concurrent session is merging actions into commands — `CommandDefinition` now carries `kind: ActionKind` and has lost `scope`, and `CommandRef`/`CommandScope` were removed from the manifest (their commits `2420304f4c` 08:39 and `5756202c43` 12:44 on 2026-08-14). Under that in-flight merge, `.view_action("badView", …)` no longer lands in `app_commands`, so the lookup returns `None` and the guard is skipped. This crate was verified at 165 passed / 0 failed earlier in this ticket, before their 12:44 commit; the only edits this ticket made to that file afterwards were adding the six interaction verbs to the testkit skip list and adding `ArtifactPack` to a test-module import, neither of which touches action registration. Left for the owning session — chasing an in-progress refactor would conflict with live work.
