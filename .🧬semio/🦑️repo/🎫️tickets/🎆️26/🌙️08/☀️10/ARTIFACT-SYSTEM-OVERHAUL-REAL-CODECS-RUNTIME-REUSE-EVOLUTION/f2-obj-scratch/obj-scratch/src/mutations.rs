...
use crate::{
    ObjFace, ObjNormal, ObjSmoothingRange, ObjSnapshot, ObjTexCoord, ObjUnknownStatement, ObjUsemtlRange,
    ObjVertex,
};
use crate::{Mutation, MutationDiff};
...
pub enum ObjMutation {
    #[default]
    NoMutation,
    SetSnapshot { snapshot: ObjSnapshot },

    /// ➕️ Inserts a whole `v` row at `index` (clamped to the end on apply).
    InsertVertex { index: usize, vertex: ObjVertex },
...
pub fn apply_obj_mutation(snapshot: &mut ObjSnapshot, mutation: &ObjMutation) -> ObjDiff {
    let diff = <ObjMutation as crate::Mutation<ObjSnapshot>>::diff(mutation, snapshot);
    *snapshot = <ObjDiff as crate::MutationDiff<ObjSnapshot>>::apply(&diff, snapshot);
    diff
}
//#endregion 🔖️Apply
...
mod tests {
    use super::*;
    use crate::{ObjFaceVertex, ObjGroup, ObjObject};
