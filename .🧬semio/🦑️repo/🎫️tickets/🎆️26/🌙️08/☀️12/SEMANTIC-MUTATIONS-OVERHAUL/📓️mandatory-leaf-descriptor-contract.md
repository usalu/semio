...
`SemanticDescriptor` remains the distinct semantic/history vocabulary also used by inference contracts. It is not replaced by, or confused with, the full mutation leaf metadata. The end-state `MutationKind: MutationLeaf` requires the inherited `MutationLeaf::DESCRIPTOR` and its own `SEMANTICS`, with no defaults; their semantic kinds must match. There is no duplicate independently declared descriptor constant. The aggregate derive must expose full descriptor iteration and `From<Leaf>` conversions, reject invalid/duplicate metadata, retain mechanical behavior delegation, and never infer missing metadata from a variant name.
...
The metadata boundary is `MutationLeaf` with a by-value `const DESCRIPTOR: MutationLeafDescriptor` and `PROVENANCE`, and an end-state mandatory `MutationKind: MutationLeaf` retaining its own explicit `SEMANTICS`. Aggregate code qualifies the descriptor through `MutationLeaf`, the declaring trait. Provenance contains mutation-root, full owner and source-path facts. No behavior belongs in the metadata trait/derive. This is the next contract direction, not an implemented optional compatibility path. Aggregate derive must independently validate its own source/root and every wrapped leaf connection; a handwritten provenance literal alone is not ownership proof.
...
The bounded review is now recorded in `📓️lower-mutation-contract-audit.md`. The coordinator freezes mandatory base `DESCRIPTORS` and `descriptor(&self)`, by-value leaf `DESCRIPTOR`, and metadata-supertrait inheritance without duplicate constants. The lower type move precedes implementation; physical-workspace provenance, all consumer propagation, and production compile/runtime gates remain required. No generic owner or sentinel descriptor may be fabricated to satisfy the new trait.
