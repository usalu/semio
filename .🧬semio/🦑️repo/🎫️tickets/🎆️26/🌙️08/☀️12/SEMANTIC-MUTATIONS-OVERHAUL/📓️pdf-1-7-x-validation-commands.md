# PDF 1.7/X Validation Commands

Exact non-Cargo validation commands executed for the X cutover. All filesystem roots are explicit and exclude `compose/**`.

## ajv

```sh
bun -e 'const fs=require("fs"),path=require("path"),Ajv=require("ajv"); const root="✏️s/🔌️plugins/🗄️stdio/🗿️artifacts/📄️pdf/🏅️standards/🔖️1.7/🪆️subsets/✳️x/🧬️schema/🧬️mutations"; const meta=JSON.parse(fs.readFileSync("🧰️framework/🛍️products/🦑️repo/🔨️modules/📚️library/🔣️mutation-descriptor.schema.json","utf8")); const ajv=new Ajv({strict:false,allErrors:true}); const check=ajv.compile(meta); const dirs=fs.readdirSync(root,{withFileTypes:true}).filter(e=>e.isDirectory()&&fs.existsSync(path.join(root,e.name,"🔣️component.json"))); const files=["🦀️component.rs","🟦️component.ts","🔗️component.graphql","🛰️component.proto","🔣️payload.schema.json","📝️text/🦀️component.rs","💾️binary/🦀️component.rs"]; let errors=[],surfaces=0,payloadCases=0,validSamples=[]; const aggregate=JSON.parse(fs.readFileSync(root+"/🔣️component.json","utf8")); for(const d of dirs){  const owner=root+"/"+d.name;  const desc=JSON.parse(fs.readFileSync(owner+"/🔣️component.json","utf8"));  if(!check(desc))errors.push([d.name,check.errors]);  if(desc.owner!==owner)errors.push([d.name,"owner mismatch"]);  for(const file of files){if(fs.existsSync(owner+"/"+file))surfaces++;else errors.push([d.name,"missing "+file]);}  const payload=JSON.parse(fs.readFileSync(owner+"/"+desc.payloadSchema,"utf8"));  const validatePayload=ajv.compile(payload);
...
const root="✏️s/🔌️plugins/🗄️stdio/🗿️artifacts/📄️pdf/🏅️standards/🔖️1.7/🪆️subsets/✳️x/🧬️schema/🧬️mutations";
...
for(const d of descriptors){
 ...
 if(!leaf.includes("impl MutationKind<PdfSnapshot, PdfXMutation> for "+d.aggregateVariant))errors.push("leaf behavior "+d.semanticKind);
...
if(rootSchema.oneOf.length!==14)errors.push("root schema count");
const manifest=JSON.parse(read("✏️s/🔌️plugins/🗄️stdio/🗿️artifacts/📄️pdf/🏅️standards/🔖️1.7/🪆️subsets/✳️x/🧪️oracle/🔣️component.json"));
if(JSON.stringify(manifest.mutationCatalogs[0].kinds)!==JSON.stringify(kinds))errors.push("oracle manifest kinds");
for(const file of ["✏️s/🔌️plugins/🗄️stdio/🗿️artifacts/📄️pdf/🏅️standards/🔖️1.7/🪆️subsets/✳️x/🧪️oracle/🦀️component.rs","✏️s/🔌️plugins/🗄️stdio/🗿️artifacts/📄️pdf/🧪️tests/mutate-pdf-1-7-x/🦀️component.rs"]){
...
const rows=[...read("✏️s/🔌️plugins/🗄️stdio/🗿️artifacts/📄️pdf/🧪️tests/mutate-pdf-1-7-x/component.feature").matchAll(/^\s*\| ([a-z][a-z-]+)\s+\|/gm)].map(m=>m[1]).filter(k=>k!=="id");
if(rows.length!==28||new Set(rows).size!==14||rows.some(k=>!kinds.includes(k)))errors.push("feature rows");
...
console.log(JSON.stringify({leaves:descriptors.length,files:all.length,rust:14,typescript:14,graphql:14,protobuf:14,jsonSchema:14,text:14,binary:14,oracle:kinds.length,featureRows:rows.length,featureKinds:new Set(rows).size,tags:descriptors.map(d=>d.binaryTag),errors},null,2));
process.exit(errors.length?1:0);'
```
...
const base="✏️s/🔌️plugins/🗄️stdio/🗿️artifacts/📄️pdf/🏅️standards/🔖️1.7/🪆️subsets/✳️x",adapter="✏️s/🔌️plugins/🗄️stdio/🗿️artifacts/📄️pdf/🧪️tests/mutate-pdf-1-7-x/🦀️component.rs";
const walk=d=>fs.readdirSync(d,{withFileTypes:true}).flatMap(e=>e.isDirectory()?walk(d+"/"+e.name):[d+"/"+e.name]);
...
console.log(JSON.stringify({xRustFiles:local.length,adapterFiles:1,errors},null,2));
process.exit(errors.length?1:0);'
```
