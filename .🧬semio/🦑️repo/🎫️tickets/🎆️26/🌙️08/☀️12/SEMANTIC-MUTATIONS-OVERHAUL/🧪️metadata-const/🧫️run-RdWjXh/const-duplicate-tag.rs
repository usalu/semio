...
static DUPLICATE: MutationLeafDescriptor = MutationLeafDescriptor { semantic_kind: "remove-page", display_name: "Remove Page", emoji: "➖️", aggregate_variant: "RemovePage", payload_schema: "🦀️.rs#RemovePage", text_opcode: Some("remove-page"), ..FIRST }; static ROSTER: [MutationLeafDescriptor; 2] = [FIRST, DUPLICATE]; const _: () = match validate_mutation_leaf_descriptor_roster(FIRST.owner, &ROSTER) { Ok(()) => (), Err(_) => panic!("duplicate tag") };
fn main() {}
