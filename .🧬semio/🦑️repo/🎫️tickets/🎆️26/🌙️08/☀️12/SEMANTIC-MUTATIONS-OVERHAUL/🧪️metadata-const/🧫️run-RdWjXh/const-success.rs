...
pub const fn validate_mutation_leaf_descriptor_roster(owner: &'static str, descriptors: &'static [MutationLeafDescriptor]) -> Result<(), MutationLeafDescriptorRosterValidationError> {
    let mut index = 0;
    while index < descriptors.len() {
        let descriptor = &descriptors[index];
        if let Err(error) = validate_mutation_leaf_descriptor(descriptor) {
            return Err(MutationLeafDescriptorRosterValidationError { owner, field: error.field, first_index: index, index });
        }
        if !mutation_leaf_descriptor_str_eq(owner, descriptor.owner) {
            return Err(MutationLeafDescriptorRosterValidationError { owner, field: "owner", first_index: index, index });
        }
        index += 1;
    }
    let mut index = 0;
    while index < descriptors.len() {
        ...
        while duplicate < descriptors.len() {
            ...
            if mutation_leaf_descriptor_str_eq(descriptor.semantic_kind, other.semantic_kind) {
                return Err(MutationLeafDescriptorRosterValidationError { owner, field: "semanticKind", first_index: index, index: duplicate });
            }
            if let (Some(left), Some(right)) = (descriptor.text_opcode, other.text_opcode) {
                if mutation_leaf_descriptor_str_eq(left, right) {
                    return Err(MutationLeafDescriptorRosterValidationError { owner, field: "textOpcode", first_index: index, index: duplicate });
                }
            }
            if let (Some(left), Some(right)) = (descriptor.binary_tag, other.binary_tag) {
                if left == right {
                    return Err(MutationLeafDescriptorRosterValidationError { owner, field: "binaryTag", first_index: index, index: duplicate });
                }
            }
            duplicate += 1;
        }
