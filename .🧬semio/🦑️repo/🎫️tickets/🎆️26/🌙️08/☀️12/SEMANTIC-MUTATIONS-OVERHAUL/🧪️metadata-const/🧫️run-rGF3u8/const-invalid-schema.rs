...
static INVALID: MutationLeafDescriptor = MutationLeafDescriptor { schema_version: 2, ..FIRST }; const _: () = match validate_mutation_leaf_descriptor(&INVALID) { Ok(()) => (), Err(_) => panic!("invalid schema") };
fn main() {}
