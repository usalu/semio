...
fn mutation_authority_relative(root: &Path, path: &Path) -> Result<String, String> {
    ...
    for component in relative.components() {
        ...
        if segment.eq_ignore_ascii_case("compose") { return Err("opaque compose owner rejected".to_string()); }
...
mod mutation_source_authority_tests {
    ...
    fn materialize(case: &str, fixture: &serde_json::Value) -> (PathBuf, PathBuf, PathBuf) {
        ...
        match case {
            ...
            "raw-nonutf8" => {
                ...
                #[cfg(not(unix))] return (workspace.clone(), workspace, PathBuf::from("missing-raw-source"));
            },
    ...
    fn validates_mutation_source_authority_fixture() {
        ...
        assert_eq!(fixture["descriptor"].as_object().unwrap().keys().map(String::as_str).collect::<Vec<_>>(), ["aggregateVariant", "binaryTag", "composition", "diffParticipation", "displayName", "emoji", "invertibility", "outcomeClasses", "owner", "payloadSchema", "requiredLanguageSurfaces", "schemaVersion", "semanticKind", "textOpcode"]);
        for vector in fixture["cases"].as_array().unwrap() {
            ...
            #[cfg(unix)] if name == "raw-nonutf8" { assert!(result.as_ref().unwrap_err().contains("not UTF-8")); }
