...
## Flow Assembly Follow-up

The first package-local Nx quick test reached a pre-existing stale import in the clean Flow TypeScript package barrel. All barrel targets still used the artifact's pre-standard path even though the current authored leaves live under standard `1`, subset `any`. The same owner-local lease therefore added the clean barrel at baseline hash `0704b6b6e0b903a2cc51adfed8ee3cb7543fc20812c144386541f39aafc38aff`:

- the eleven real schema, snapshot, diff, mutation, and I/O namespace exports now point directly at their canonical standard/subset leaves;
- `flow_decomposer` was removed because its target does not exist and live search found no production consumer;
- no compatibility alias or forwarding component was added.

The updated barrel hash is `513022a3aa593288d08638e3eb56fdba7f53bf1f31e74e530830aa3c8ce61428`. A coordinator rerun of `bun nx run @semio-tech/flow-js:test-quick --skip-nx-cache` exited `0`: one file loaded successfully, with no tests remaining after the zero-consumer component and its in-source tests were deleted.

Unrelated concurrent work dirtied Flow Rust, app, schema, and mutation paths after this lease's clean baseline. Those files were neither read as implementation authority nor modified by this lease. A broad moving-graph validation must not be attributed to the deletion lease.
