...
const inspector = async (page) => {
  await click(page, '[data-slot="panel-tab-button"][id="framework.panel.inspection"], [id="framework.panel.inspection"]');
...
async function authorEdit(session, action = "addFeature", staged = {}) {
...
for (const session of sessions) {
  await click(session.page, '[data-slot="panel-tab-button"][id="framework.panel.history"], [id="framework.panel.history"]');
...
if (wanted("8")) {
  ...
  await click(B.page, '[data-slot="panel-tab-button"][id="framework.panel.history"], [id="framework.panel.history"]');
