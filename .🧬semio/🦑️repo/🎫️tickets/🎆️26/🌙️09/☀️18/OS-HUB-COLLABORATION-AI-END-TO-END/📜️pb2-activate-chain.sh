#!/bin/zsh
# 🔗️ Slice PB2: re-activate one variant at a time, EACH step through the fleet wasm mutex
# (preamble rule 27). Never `dev <variant>` (that is `nx watch --all`).
# Usage: 📜️pb2-activate-chain.sh <variant>...
TICKET="/Users/ueli/Documents/semio/.🧬semio/🦑️repo/🎫️tickets/🎆️26/🌙️09/☀️18/OS-HUB-COLLABORATION-AI-END-TO-END"
CHAIN="$TICKET/🗑️generated/pb2-activate-chain.txt"
...
for VARIANT in "$@"; do
  LOG="$TICKET/🗑️generated/pb2-${VARIANT}-activate.txt"
  echo "→ $VARIANT $(date -u +%FT%TZ)" >> "$CHAIN"
  echo "start $(date -u +%FT%TZ)" > "$LOG"
  zsh "$MUTEX" pb2 -- bun nx run "@semio-tech/framework-os-dev:activate-${VARIANT}-react-dev" >> "$LOG" 2>&1
