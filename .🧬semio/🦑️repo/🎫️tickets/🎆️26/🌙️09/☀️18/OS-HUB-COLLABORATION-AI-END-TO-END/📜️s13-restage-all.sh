...
COUNT=${#MANIFESTS[@]}
echo "[s13-restage] $COUNT component manifests  $(date '+%F %T')"
if [ "$COUNT" -ne 60 ]; then
  echo "[s13-restage] ABORT: expected 60 components, found $COUNT — the session roster moved, re-derive before building"
  echo "[s13-restage] FAILED $(date '+%F %T')"; exit 1
fi
...
for manifest in "${MANIFESTS[@]}"; do
  ...
  echo "[s13-restage] ($i/$COUNT) component dev $name  $(date '+%F %T')"
  bun "$CARGO_SCRIPT" native component dev --manifest "$manifest" > "$OUT/s13-component-$name.txt" 2>&1
  rc=$?
  echo "[s13-restage] ($i/$COUNT) component rc=$rc $(date '+%F %T')"
  ...
  if [ "$rc" -ne 0 ]; then
    echo "[s13-restage] ABORT: $name component build failed — see 🗑️generated/s13-component-$name.txt"
    tail -5 "$OUT/s13-component-$name.txt"
    echo "[s13-restage] FAILED $(date '+%F %T')"; exit 1
  fi
  echo "[s13-restage] ($i/$COUNT) materialize $name  $(date '+%F %T')"
  bun "$PLUGIN_SCRIPT" materialize dev --manifest "$manifest" > "$OUT/s13-materialize-$name.txt" 2>&1
  rc=$?
  echo "[s13-restage] ($i/$COUNT) materialize rc=$rc $(date '+%F %T')"
  if [ "$rc" -ne 0 ]; then
    echo "[s13-restage] ABORT: $name materialize failed — see 🗑️generated/s13-materialize-$name.txt"
    tail -5 "$OUT/s13-materialize-$name.txt"
    echo "[s13-restage] FAILED $(date '+%F %T')"; exit 1
  fi
done
echo "[s13-restage] activate  $(date '+%F %T')"
( cd "🧰️framework/🛍️products/💻️os/🔨️modules/🧑‍💻dev/📦️packages/🟦️typescript" && bun ./📜️script.ts activate s react dev ) > "$OUT/s13-activate.txt" 2>&1
echo "[s13-restage] activate rc=$? $(date '+%F %T')"
tail -1 "$OUT/s13-activate.txt"
echo "[s13-restage] DONE $(date '+%F %T')"
