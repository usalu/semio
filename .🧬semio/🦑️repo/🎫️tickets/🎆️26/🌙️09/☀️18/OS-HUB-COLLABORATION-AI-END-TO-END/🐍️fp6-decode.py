...
tag=sys.argv[1]
p=os.path.join(G,f"fp6-{tag}-serial.txt")
txt=open(p,encoding='utf-8',errors='replace').read()
names=sorted(set(m.group(1) for m in re.finditer(r"^test (\S+) \.\.\. FAILED$", txt, re.M)))
open(os.path.join(G,f"fp6-{tag}.names"),"w",encoding='utf-8').write("\n".join(names)+"\n")
blocks=re.split(r"\n---- (\S+) stdout ----\n", txt)
res={}
for i in range(1,len(blocks),2):
    ...
    res[name]=msg
with open(os.path.join(G,f"fp6-{tag}-panics.txt"),"w",encoding='utf-8') as w:
    for n in names: w.write(f"{n}\n    {res.get(n,'?')}\n")
prev=sys.argv[2] if len(sys.argv)>2 else None
if prev:
    old=set(open(os.path.join(G,f"fp6-{prev}.names"),encoding='utf-8').read().split())
    new=set(names)
