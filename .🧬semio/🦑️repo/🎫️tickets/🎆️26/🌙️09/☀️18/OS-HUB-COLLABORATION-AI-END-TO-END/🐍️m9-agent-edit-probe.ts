/** ✍️ M9 — the MCP agent EDITS a hub document.
 *
 * M8's probe proved the agent can READ a hub document and stopped at `action_prepare`
 * (`repo root not found`). This one drives the same chain with two differences that matter:
 *  - it matches a mutation verb by the document's `artifactKind` (`s.gis.gismap`), not by its pack
 *    `kind`/schema (`gis.map`) — two vocabularies M9 §5.1 separates;
 *  - it snapshots BEFORE and AFTER the invoke and compares the bytes, so "the agent edited it" is a
 *    measured difference and never an inferred one.
 *
 * Usage: bun 🐍️m9-agent-edit-probe.ts <origin> <spaceId> <credentialPath> <humanToken> [documentId]
 *
 * Original header of the M8 probe this is derived from:
...
const binary = process.env.M9_MCP_BINARY ?? join(repoRoot, "🧰️framework", "🛍️products", "💻️os", "🔨️modules", "🌉️mcp", "📦️packages", "🦀️rust", "dist", "build", "semio-os-mcp");
...
try {
  const initialized = await session.request("initialize", { protocolVersion: "2025-06-18", capabilities: { roots: { listChanged: true }, sampling: {}, elicitation: {} }, clientInfo: { name: "semio-m9-agent-edit", title: "M9 agent edit", version: "1" } });
  ...
  const openedKind = String(opened.structuredContent?.artifactKind ?? "");
  row("4b artifact_open names the artifact KIND, not only the pack schema", openedKind.startsWith("s."), `artifactKind=${openedKind || "<none>"} kind/schema=${opened.structuredContent?.kind ?? "<none>"}`);
  const snapshotBefore = await session.call("artifact_snapshot", { artifactId: documentId });
  const packBefore = String(snapshotBefore.structuredContent?.packBase64 ?? snapshotBefore.structuredContent?.packBytes ?? "");
  ...
  const packAfter = String(snapshot.structuredContent?.packBase64 ?? snapshot.structuredContent?.packBytes ?? "");
  row("7b the snapshot SHOWS the agent's mutation", packBefore.length > 0 && packAfter.length > 0 && packBefore !== packAfter, `before=${packBefore.length} chars after=${packAfter.length} chars identical=${packBefore === packAfter}`);
...
console.log(`M9 agent-edit probe: ${failures} red row(s)`);
