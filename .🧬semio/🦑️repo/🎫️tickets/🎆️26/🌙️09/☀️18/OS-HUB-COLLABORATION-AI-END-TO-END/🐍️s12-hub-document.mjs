#!/usr/bin/env bun
/** 🗂️ S12 — the hub-catalog document journey inside `s`, driven the way a PERSON drives it.
 *
...
const log = (...parts) => console.log("[s12]", ...parts);
...
async function ensureSpace(page) {
  ...
  await name.fill(`S12 Space ${Date.now() % 100000}`).catch(() => undefined);
...
const browser = await chromium.launch({ headless: process.env.S12_HEADED !== "1", args: ["--use-angle=metal"] });
...
page.on("console", (message) => {
  ...
  if (/refused:|dropped|rejected|\[os-shell\]/iu.test(text)) shellLines.push(text.slice(0, 240));
});
...
try {
  ...
  await page.screenshot({ path: `${generated}s12-hub-document-${tag}-spaces.png` }).catch(() => undefined);
  if (result.spaces.length === 0) throw new Error("hub workspace listed no space to open");

  const spaceId = (result.spaces.find((row) => new RegExp(process.env.S12_SPACE ?? kindFilter, "iu").test(row.text)) ?? result.spaces[0]).id;
  ...
  while (Date.now() < deadline) {
    ...
    if (shell.uri.includes(`/spaces/${spaceId}`) && shell.windowIds.length > 0) break;
    await page.waitForTimeout(2_000);
  }
  ...
  await page.screenshot({ path: `${generated}s12-hub-document-${tag}-index.png` }).catch(() => undefined);
  ...
  result.rowsAfter = await indexRows(page);
  await page.screenshot({ path: `${generated}s12-hub-document-${tag}-created.png` }).catch(() => undefined);
  ...
  await page.screenshot({ path: `${generated}s12-hub-document-${tag}-opened.png` }).catch(() => undefined);
  ...
  await page.screenshot({ path: `${generated}s12-hub-document-${tag}-mutated.png` }).catch(() => undefined);
} catch (error) {
...
result.shellLines = shellLines.slice(0, 20);
result.faults = faults.slice(0, 8);
writeFileSync(`${generated}s12-hub-document-${tag}.txt`, JSON.stringify(result, null, 2));
log(`=== ${generated}s12-hub-document-${tag}.txt ===`);
await browser.close();
