/** 🧪️ AU3 live sign-in probe — boots the `os-hub` development binary against a **fresh**
...
const binaryPath = join(repoRoot, ".🧬semio", "🦑️repo", "⚡️cache", "cargo", "target", "debug", "os-hub");
if (!existsSync(binaryPath)) throw new Error(`build the hub first: cargo build -p semio-hub --bin os-hub (missing ${binaryPath})`);
const port = Number(process.env.AU3_HUB_PORT ?? 8831);
