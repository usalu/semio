/** 🩺️ Shared batch-B interaction probe (ticket 26/09/18/OS-HUB-COLLABORATION-AI-END-TO-END, slice B2b).
 *
 ...
 *
 * Slice B3c adds two things B2b did not need: the dev shell's own `window.__semioOsCatalogProbe`
 * row set (`🏛️ShellHost/🟦️.tsx:705`), which is the only witness that an EXTENSION plugin actually
 * installed into the running shell next to its parent, and a candidate list for `action` so one
 * probe can discover which of several plausible verb ids a plugin actually declares.
 *
...
const readShell = (page) => page.evaluate(() => {
  ...
  return {
    ...
    catalog: (() => {
      ...
      return {
        shellPluginId: probe.shellPluginId,
        ready: probe.ready,
        plugins: probe.plugins.map((row) => `${row.pluginId}:${row.status}${row.routerFault ? `/${row.routerFault.code}` : ""}`),
        programs: probe.programs.map((row) => `${row.pluginId}|${row.appId}`),
      };
    })(),
  };
});
...
export async function runInteractionProbe(config) {
  const outDir = join(OUT, `b3c-${config.plugin}`);
  ...
  // ⏳️ A COLD vite dev server transforms the whole ~850-module graph on the first document request;
  // playwright's 30 s default navigation budget is shorter than that legitimately takes, so a cold
  // serve reads as a wedge. The budget is the boot budget, not a retry.
  ...
  // 🧩️ Which extension plugins installed into THIS shell, and which of them own a declared action.
  {
    const from = lines.length;
    const rows = shell.catalog?.plugins ?? [];
    const extensions = rows.filter((row) => /-(extension|module)-/.test(row));
    const extensionActions = Object.fromEntries((config.extensions ?? []).map((extension) => [
      extension.id,
      { installed: rows.find((row) => row.startsWith(`${extension.id}:`)) ?? "absent", actions: shell.actions.filter((id) => (extension.actions ?? []).some((name) => id === `action.${name}`)) },
    ]));
    note("extensions", { shellPluginId: shell.catalog?.shellPluginId, installed: rows.length, extensions, extensionActions, programs: shell.catalog?.programs ?? [] }, from);
  }

  // 🎯️ `action` may be a candidate list — the first id the shell actually declares wins.
  const candidates = Array.isArray(config.action) ? config.action : [config.action];
  const action = candidates.find((id) => shell.actions.includes(`action.${id}`)) ?? candidates[0];
  ...
  await page.screenshot({ path: join(OUT, `b3c-${config.plugin}.png`) });
  ...
  writeFileSync(join(outDir, "report.json"), JSON.stringify(report, null, 2));
  writeFileSync(join(OUT, `b3c-${config.plugin}-console.txt`), [
