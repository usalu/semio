#!/usr/bin/env zsh
# 🀄️ Slice GJ1 — ONE fleet-wasm-mutex hold that (1) type-checks the whole 🀄️wfc guest for
...
check_log="$GEN/gj1-wasm-check.txt"
echo "=== wasm32-wasip2 check of $owner at $(date -Iseconds) ===" > "$check_log"
( cd "$ROOT" && CARGO_TARGET_DIR="$ROOT/.🧬semio/🦑️repo/⚡️cache/cargo/target-gj1" cargo check --target wasm32-wasip2 \
