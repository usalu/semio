...
PLUGINS=(🪐️space 📖️playbook)
PROJECTS=(space-plugin playbook-plugin)
for i in {1..2}; do
  plugin="${PLUGINS[$i]}"; project="${PROJECTS[$i]}"
  echo "[s10-restage] ($i/2) component dev $plugin  $(date '+%F %T')"
  ...
  rc=$?
  echo "[s10-restage] ($i/2) component rc=$rc $(date '+%F %T')"
  ...
  echo "[s10-restage] ($i/2) materialize $project  $(date '+%F %T')"
  zsh "$MUTEX" s10 -- bun nx run "@semio-tech/$project:materialize-dev" > "$OUT/s10-materialize-$project.txt" 2>&1
  rc=$?
  echo "[s10-restage] ($i/2) materialize rc=$rc $(date '+%F %T')"
  if [ "$rc" -ne 0 ]; then
