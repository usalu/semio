...
try {
  await req("initialize", { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "gj1-trap-probe", version: "1" } });
  const ctx = await req("tools/call", { name: "context_resolve", arguments: {} });
  console.log(`channel=${(ctx.result?.structuredContent as any)?.channel}`);
  const started = Date.now();
  const ran = await call("inference_run", { artifactKind: "s.wfc.bitmap", inferenceSchema: "s.wfc.bitmap.solve", pluginId: "wfc", payload: { snapshot }, cancellationId: `gj1-trap-${Date.now().toString(36)}` });
  console.log(`inference_run(snapshot): ${Math.round((Date.now() - started) / 1000)}s isError=${ran.result?.isError}`);
  console.log(JSON.stringify(ran.result?.structuredContent ?? ran, null, 2));
} finally { child.kill(); }
