#!/bin/zsh
# 🏗️ One-shot react dev activation for a flow/process variant (slice FL1).
...
# Usage: 📜️fl1-activate.sh <variant> <port>
VARIANT="$1"; PORT="$2"
...
LOG="$TICKET/🗑️generated/fl1-${VARIANT}-activate.txt"
echo "start $(date -u +%FT%TZ)" > "$LOG"
zsh "$TICKET/📜️wasm-build-mutex.sh" fl1 -- bun nx run "@semio-tech/framework-os-dev:activate-${VARIANT}-react-dev" >> "$LOG" 2>&1
