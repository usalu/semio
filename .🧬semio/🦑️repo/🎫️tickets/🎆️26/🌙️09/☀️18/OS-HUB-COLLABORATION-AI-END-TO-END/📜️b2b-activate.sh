#!/bin/zsh
# 🏗️ One-shot react dev activation for a batch-B dormant plugin (slice B2b; no `nx watch`, which
# re-activates on every peer edit and takes the Vite server down mid-probe).
# Usage: 📜️b2b-activate.sh <variant> <port>
VARIANT="$1"; PORT="$2"
cd /Users/ueli/Documents/semio || exit 1
export CARGO_PROFILE_WASM_DEV_DEBUG=false NX_DAEMON=false SEMIO_RENDERER=react S_OS_PORT="$PORT"
LOG=".🧬semio/🦑️repo/🎫️tickets/🎆️26/🌙️09/☀️18/OS-HUB-COLLABORATION-AI-END-TO-END/🗑️generated/b2b-${VARIANT}-activate.txt"
