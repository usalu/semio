...
const THESIS: &[u8] = include_bytes!("../../../../../../../✏️s/🔌️plugins/🗄️stdio/🗿️artifacts/📖️pdf/🏅️standards/7️⃣1.7/🪆️subsets/🧱️base/🖼️assets/🎓️bachelor-thesis/🎓️bachelor-thesis.pdf");
const REPORT_STRIP: &[u8] = include_bytes!("../../../../../../../✏️s/🔌️plugins/🗄️stdio/🗿️artifacts/📖️pdf/🏅️standards/7️⃣1.7/🪆️subsets/🧱️base/🧫️fixtures/📊️report-strip/📊️report-strip.pdf");
...
#[test]
fn debug_dump_thesis_text() {
    let decoded = decode_pdf(THESIS).unwrap();
    let mut report = String::new();
    for index in 0..6 {
        report.push_str(&format!("=== page {index}\n{}\n", decoded.page_text(index)));
    }
    report.push_str(&format!("fonts: {}\n", decoded.fonts.len()));
    for font in decoded.fonts.iter().take(12) {
        let kind = match &font.kind { PdfFontKind::Type1 { base_font, encoding, program, .. } => format!("Type1 {base_font} enc={encoding:?} prog={}", program.is_some()), PdfFontKind::TrueType { base_font, .. } => format!("TrueType {base_font}"), PdfFontKind::Type3 { .. } => "Type3".into(), PdfFontKind::Type0 { base_font, cmap, .. } => format!("Type0 {base_font} {cmap:?}") };
        report.push_str(&format!("{}: {kind} tounicode={}\n", font.id, font.to_unicode.is_some()));
    }
    for font in decoded.fonts.iter().filter(|f| f.id == "F27" || f.id == "F39") {
        if let PdfFontKind::Type3 { encoding, .. } | PdfFontKind::Type1 { encoding, .. } = &font.kind {
            report.push_str(&format!("{} encoding base={:?} differences={:?}\n", font.id, encoding.base, encoding.differences.iter().take(12).collect::<Vec<_>>()));
        }
        if let PdfFontKind::Type1 { program: Some(PdfFontProgram::Type1 { data, .. }), .. } = &font.kind {
            report.push_str(&format!("{} program head: {:?}\n", font.id, String::from_utf8_lossy(&data[..data.len().min(1200)])));
            report.push_str(&format!("{} builtin: {:?}\n", font.id, crate::standards::v1_7::subsets::base::modules::fonts::type1_builtin_encoding(data).map(|e| e.len())));
        }
        let codec = FontCodec::new(font);
        report.push_str(&format!("{} decode(65): {:?}\n", font.id, codec.decode(&[65, 49])));
    }
    let ops = &decoded.pages[1].content;
    report.push_str(&format!("page1 ops: {}\n", ops.len()));
    for op in ops.iter().take(60) {
        report.push_str(&format!("{op:?}\n"));
    }
    std::fs::write("/Users/ueli/Documents/semio/.🧬semio/🦑️repo/🎫️tickets/🎆️26/🌙️09/☀️18/PDF-ARTIFACT-SPEC-COMPLETE/🗑️generated/thesis-text.txt", report).unwrap();
}

#[test]
fn debug_dump_exports_for_external_validation() {
    let dir = "/Users/ueli/Documents/semio/.🧬semio/🦑️repo/🎫️tickets/🎆️26/🌙️09/☀️18/PDF-ARTIFACT-SPEC-COMPLETE/🗑️generated";
    std::fs::write(format!("{dir}/rich.pdf"), encode_pdf(&rich_document()).unwrap()).unwrap();
    std::fs::write(format!("{dir}/rich-objstm.pdf"), encode_pdf_with(&rich_document(), &EncodeOptions { lower: LowerOptions::default(), write: WriteOptions { xref_stream: true, encryption: None } }).unwrap()).unwrap();
    let mut encrypted = rich_document();
    encrypted.encryption = Some(PdfEncryption { algorithm: PdfEncryptionAlgorithm::Aes256, permissions: -1, user_password: "pw".into(), owner_password: Some("owner".into()), encrypt_metadata: true });
    std::fs::write(format!("{dir}/rich-aes256.pdf"), encode_pdf(&encrypted).unwrap()).unwrap();
    encrypted.encryption = Some(PdfEncryption { algorithm: PdfEncryptionAlgorithm::Rc4_128, permissions: -1, user_password: "".into(), owner_password: Some("owner".into()), encrypt_metadata: true });
    std::fs::write(format!("{dir}/rich-rc4.pdf"), encode_pdf(&encrypted).unwrap()).unwrap();
    let thesis = decode_pdf(THESIS).unwrap();
    std::fs::write(format!("{dir}/thesis-rewrite.pdf"), encode_pdf(&thesis).unwrap()).unwrap();
    std::fs::write(format!("{dir}/text.pdf"), encode_pdf(&text_document(&[(300.0, 200.0, "Hello Semio — wrapped text with real Helvetica metrics that should break across several lines of this small page.")])).unwrap()).unwrap();
}
