/** 🔄️ Wfc panel-refresh probe: boots the wfc react playground, opens the Media panel, runs `addStream`
 ...
 * Usage: cd <ticket> && SEMIO_PROBE_OUT=wfc-panel-refresh-1 bun 🐍️wfc-panel-refresh-probe.mjs
 */
...
const outDir = join(import.meta.dir, "🗑️generated", process.env.SEMIO_PROBE_OUT ?? "wfc-panel-refresh");
...
const state = () => page.evaluate(() => {
  ...
  return {
    ...
    mediaPressed: document.getElementById("wfcing.media")?.getAttribute("aria-pressed") ?? null,
...
const openMedia = async () => { const t = page.locator('[id="wfcing.media"]').first(); if ((await t.getAttribute("aria-pressed")) !== "true") await t.click({ timeout: 8000, force: true }); await page.waitForTimeout(2500); };
...
await page.locator('[id="framework.window.wfcingFrames.engagement.toggle"]').first().click({ timeout: 8000, force: true });
...
await page.locator('[id="wfcing.media"]').first().click({ timeout: 8000, force: true });
await page.waitForTimeout(1500);
await openMedia();
await note("after-media-reopen", await state());

// (c) another app panel (Results) then back
await page.locator('[id="wfcing.results"]').first().click({ timeout: 8000, force: true }).catch(() => {});
