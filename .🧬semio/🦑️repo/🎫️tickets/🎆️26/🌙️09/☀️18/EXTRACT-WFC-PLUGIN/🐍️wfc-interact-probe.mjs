/** 🎛️ Wfc interaction probe: boots the wfc react playground (6063), then (1) records the boot state
 ...
 * Usage: cd <ticket> && SEMIO_PROBE_OUT=wfc-interact-1 bun 🐍️wfc-interact-probe.mjs
 */
import { chromium } from "playwright";
import { mkdirSync, writeFileSync } from "node:fs";
import { join } from "node:path";

const url = process.env.SEMIO_PROBE_URL ?? "http://127.0.0.1:6063/?plugin=wfc";
const outDir = join(import.meta.dir, "🗑️generated", process.env.SEMIO_PROBE_OUT ?? "wfc-interact");
...
{
  const from = lines.length;
  const engagement = before.engagements.find((id) => /wfcingFrames/i.test(id)) ?? before.engagements[0];
  let toggled = "absent";
  if (engagement) {
...
// ── undo through the keybinding ──────────────────────────────────────────
{
  ...
  await page.locator('[id="wfcing.media"]').first().focus().catch(() => {});
  let after = null;
  let presses = 0;
...
{
  const from = lines.length;
  const options = page.locator('[id="framework.window.wfcingMain.measures.unfold"]').first();
  ...
  const toggle = page.locator('[id="wfcing-main/wfcing-measure-layer-mesh"]').first();
  ...
  let published = null;
  for (let i = 0; i < settleSeconds * 2; i++) { await page.waitForTimeout(500); published = await page.evaluate(() => document.getElementById("wfcing-main/wfcing-measure-layer-mesh")?.getAttribute("data-published-value") ?? document.getElementById("wfcing-main/wfcing-measure-layer-mesh")?.getAttribute("aria-pressed") ?? null); if (published === "false") break; }
  const after = await state();
...
// ── example switch → Synthetic Orbit (10 committed frames) ────────────────
{
  ...
  const media = page.locator('[id="wfcing.media"]').first();
