...
| S12-3 | hub documents for every B2 kind inside `s` (7800 on catalog B2), lazy-install row | **B2 de re-run `b4` on the restarted 7800 (16:14–17:16, same 02:31 binary): de 9/16 PASS** (draw, block3d, wfc3d, grid3d, block5d, animate, gis (first de PASS), note, writer). All 7 FAILs are hub-side. block2d, wfc2d and grid2d: the creation took 150–166 s, then `execution-target/component` answered 503 on every request; the client retried with the announced "Der Hub ist ausgelastet…" status (S12-3h), and grid2d spent all 4 attempts. S12-3e fixes this route, but 7800 does not run it. bitmap, puzzle2d, puzzle3d and puzzle5d stayed accepted past 180 s. **Across b3 + b4, 14 of 16 B2 kinds passed in at least one run; puzzle2d and puzzle3d never did (the slow puzzle creation also reproduces on 8040, H9).** **Earlier re-run `b3` measured (09:4x–11:34, fresh space `S15 B2 Sweep 2`, fresh profiles, per-kind verb pins, name-targeted row open): en 10/16, de 9/16; 13 of 16 kinds PASS in at least one locale** (block2d, draw, wfc2d, bitmap, grid2d, block3d, wfc3d, grid3d, block5d, puzzle5d, animate, note, writer). Each PASS opened the right program through the saga and ran verb/undo/redo `[0,1,0,1]`. Every FAIL is hub-side on 7800 (old binary): creations that stay accepted (puzzle2d both locales, puzzle3d de, and de rows 10–14 during the 11:0x load), a creation POST 503 (grid3d en), and `execution-target/component` 503 after a 2–5 min creation (wfc2d en, gis en, writer en). S12-3e fixes the 503s, but 7800 does not run it yet; S12-3h retries them client-side from 10:4x. puzzle3d en opened; its verb needed the selectAll pre-verb, which the journey only gained afterwards. Earlier first pass `b2`: (`s15-b2-sweep.sh`, serve 6543 → 7800, space `S15 B2 Sweep`). Rows are judged by the program that actually loaded (`hub program <plugin>@<bundle> … loaded from hub\|store`), not the journey's own verdict. **en: 8/16 genuine PASS**: block2d, draw, block3d, wfc3d, block5d, animate, note, writer. Each ran verb/undo/redo `[0,1,0,1]`, with the right program from generation `f485bf7e…`. **Lazy install**: the first kind of each plugin was fetched from the hub (block 22 files / 27.5 MB, draw 5 / 15.6 MB, wfc 5 / 25.8 MB, animate 5 / 21.1 MB, note 5 / 15.7 MB, writer 5 / 20.1 MB). Every later kind of that plugin came from the device store with 0 hub bytes. **en failures**: bitmap, grid2d and grid3d took the wrong rail verb (now pinned per kind; de bitmap `change-seed` and grid2d `create-tile` PASS). puzzle2d stayed accepted until "outcome unknown" (H9). puzzle3d: the creation waited 5 min (the elapsed line was shown), then the open got `component` 503 (S12-3e) and the verb did not round-trip. gis: the saga did not open within 180 s. Two en rows were false PASSes of the old journey and are now marked FAIL: wfc2d opened the late puzzle2d document and puzzle5d opened block5d, because the fallback row open took the last row. That open now targets the created name only. **de: 5/16 PASS** (block2d, draw, wfc2d, bitmap, grid2d). puzzle2d has the same H9 issue. Rows 6–14 opened the right program through the saga, but it was then replaced by the space index (host regression S12-3g). The writer saga did not open. The creation notice's elapsed line was seen live in en and de | `s15-hub-document-b2-*.txt`, log `s12-s15-logs/b2-sweep.txt` |
...
| S12-3i | coordinator (C10 hypothesis): the 60 s `trusted-catalog/plugin-modules` refresh invalidates freshly mounted hub documents | **refuted for mounted documents; the real root was fixed**. The refresh only feeds the Marketplace. Live: a writer document held 190 s across 3 refreshes, each of whose first read answered 503 (front 6544 `S15_FLAKY_INDEX=503`), kept one window state throughout. **Root found live**: the OPENING reads the catalog itself (`installHubDocumentProgram` → `hubPluginSource.catalog()`), and one failed index read (503, or a timeout under load) returned `null`. The opening then ended "The artifact was created, but it could not be opened": reproduced with the index answering 503 at the moment of opening, and no open-plan was ever sent. **Fix**: the hub source reads the index and every bundle manifest under the declared transfer policy (`withTransferRetry`: 502/503/504 and lost connections, ≤ 4 attempts, backoff; the per-file loop now uses the same helper). The shell keeps its last catalog when a refresh fails instead of blanking it. After the fix the same flaky front opens the document through the 503s. Store law +1 (index 503 ×1 → answered; reset → answered; 404 → null after 1 request; 503 ×∞ → null after 4; manifest 502 ×1 → installed); the offline reinstall law now runs on the fake clock. Store 26/26 | `s12-s15-logs/journey-hold-en{,2}.txt`, `front-6544-flaky-index.txt` |
| S12-3j | coordinator: Ajv strict `unknown keyword "x-semio-note"` (2 worker laws) | **fixed TS-side, repo-consistent**. `x-semio-*` vendor annotations ARE the convention: 24 keywords over 23 332 schema files (x-semio-state ×3254, -formats ×205, -mutation ×120, -derived ×118, -child-kind ×91, -note ×10, …). Every Ajv site used to register them by hand. There is now one shared oracle, `💻️os/🧪️tests/🧬️schema-oracle` (`🔣️.json` = the declared vocabulary with each value's meta-schema; `semioSchemaAjvV1(options)` = strict Ajv plus that vocabulary). The draft-07 Ajv sites of the os laws, the os/renderer package scripts and the renderer laws that registered x-semio keywords by hand now use it (14 files, codemod `wp-s15/s15-apply-schema-oracle.py`). Law `🧪️tests/🏷️schema-vocabulary` 3/3: every annotation in the 1 064 os + hub schemas is declared and well-typed; the directory and two-client schemas compile strict; an undeclared keyword and an ill-typed annotation still refuse. os tsc: only the peer's `⌨️text-input-oracle`; renderer tsc rc 0; renderer engine-contract/opening/package-integration 698/702 (3 load timeouts pass alone). The remaining 1 is the next item. It lives in the rule-21 freeze (framework `🧬️schema/🔮️oracles` would be the long-term home; the plugins' own Ajv sites are rule-20 frozen) | `s12-s15-logs/os-suite-vocabulary.txt`, `renderer-vocab-suites.txt` |
| S12-3k | **for the rule-21 landing window**: `📇️directory/🧬️schema/🔣️.json` lost my S12-3c key (a peer's overwrite; HEAD f7791a96178 11:22 and the tree lack it) | exact JSON change to re-apply once the freeze lifts: in `$defs.ArtifactCreationProgressCopyV1` add `"waiting"` to `required` and `"waiting": { "type": "string", "minLength": 1, "maxLength": 256, "pattern": "\\{elapsed\\}" }` to `properties`. Until then engine-contract "renders the schema-owned English and German lifecycle…" fails on `additionalProperty: waiting` (the ShellHost fixture and copy still carry it) | — |
...
- 11:34 **B2 re-run `b3` finished**: en 10/16, de 9/16, 13/16 kinds in at least one locale (S12-3). Classified from the durable journey
  logs by the opened program's window ids and the sweep verdict.
- 14:59 the app restarted: every process of mine died (serves 6540/6541/6543, front 6542, hub 8040 hold). **Evidence loss**: `wp-s15/generated/`
  (every JSON/PNG named in this report's evidence column, plus the private cargo target) was deleted by an external cleanup. The durable logs
  under `.🧬semio/🌐hub/s12-s15-logs/` (sweep, journey, tool-run and front logs) survive, and the numbers above were read from them before
  the loss or re-derived from them.

- 15:0x–15:5x after the restart: hub 8040 (hold **46236** → hub 46302, current binary) and serve 6541 → 8040 (46264 → vite 51046)
  restarted with `setopt no_bg_nice`. Serve 6543 → 7800 was restarted too. It spawned a dev local-hub owner on 7800 while W2's hub was
  rebooting (`ensureDevLocalHub` squats a not-ready S_HUB_URL port). The owner bound ~15:41 and died. Reported to the coordinator;
  6543 stopped; U5 owns the fix. Front 6544 (pid 96043) runs `S15_FLAKY_INDEX=503` for S12-3i.

- 15:54–16:13 a proxy sweep on my own hub 8040 (streamed routes; catalog e8167ce8, 12 kinds; `kinds-8040.txt`), de rows 0–4, while 7800 was
  down: block2d, draw and block3d PASS; puzzle2d and puzzle3d creations stay accepted past 180 s on 8040 as well, so the slow puzzle creation
  is hub-side (H9) and not specific to 7800. The sweep driver now derives its plugin list from a kinds file (`S15_KINDS_FILE`). The kinds
  lists live in `s12-s15-logs/kinds-{b2-7800,8040}.txt`: the originals were in the deleted `generated/`. Stopped at 16:14, when 7800 was READY.
- 16:14 serve 6543 → 7800 restarted (7800 answered first, so no local-hub spawn: `joined the hub at http://127.0.0.1:7800`). B2 de sweep `b4`
  (space `S15 B2 Sweep 2`) launched, chain pid **45703**, log `s12-s15-logs/b4-sweep.txt`. The client now retries execution-target 503s
  (S12-3h) and catalog reads (S12-3i) against the old 7800 binary.

- 17:16 **B2 de `b4` finished: 9/16** (S12-3). Every failure is hub-side on 7800's old binary: component 503 on each retry after a 2.5–3 min
  creation, or creations still accepted after 3 min.

- 17:2x stopped every process of mine (serves 6541/6543, hub 8040 hold 46236 → 46302); no S15 process runs. Open: **S12-4** (the hub-document
  sweep for every kind of every package) waits for W2's `--packages all` publish; its restart recipe is `s15-b2-sweep.sh` with
  `S15_KINDS_FILE` from `s15-creation-kinds.ts`. The rule-21 landing needs the JSON patch in **S12-3k**. 7800 needs a rebuild with the
  streamed execution-target route (S12-3e) to clear the component 503 rows.
