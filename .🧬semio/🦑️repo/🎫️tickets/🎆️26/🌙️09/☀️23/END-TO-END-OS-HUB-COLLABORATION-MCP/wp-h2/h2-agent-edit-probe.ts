...
try {
  ...
  for (const frame of frames.filter((f: any) => "Commands" in f)) console.log(`INFO  commands mutation_ids=${JSON.stringify(frame.Commands.envelopes.map((e: any) => e.mutation_id))} deps=${JSON.stringify(frame.Commands.envelopes.map((e: any) => e.dependencies))} frontier=${JSON.stringify(frame.Commands.frontier)}`);
