#!/usr/bin/env bun
/** 🧭️ G11 S4: the user path for an AI client, zero-touch and timed, from a CLEAN browser profile. A human signs in to the
 ...
 * usage: bun g11-user-path.ts <shellOrigin> <hubOrigin> <en|de> <captureDir> <email> <password>
...
if (!SHELL || !HUB || !OUT || !EMAIL || !PASSWORD) throw new Error("usage: bun g11-user-path.ts <shellOrigin> <hubOrigin> <en|de> <captureDir> <email> <password>");
const WORDS = {
...
const setup = await hub("POST", "/auth/sessions", undefined, JSON.stringify({ schema: "semio.hub.auth.credential-sign-in/v1", email: EMAIL, password: PASSWORD, deviceInstanceId: `g11userpath${randomBytes(10).toString("hex")}`, clientClass: "browser" }));
const token = String(setup.json?.token ?? "");
const spaceName = `G11 user path ${LOCALE} ${randomBytes(3).toString("hex")}`;
...
try {
  ...
  client = new Client({ name: "g11-user-path", version: "1" }, { capabilities: {} });
  ...
  await page.screenshot({ path: join(OUT, `g11-user-path-approval-${LOCALE}.png`) });
  if (approvalId) {
    await skipTours(page);
  ...
  const beforeRevoke = join(mkdtempSync(join(tmpdir(), "g11-user-path-cred-")), "credential.json");
  ...
  const staleClient = new Client({ name: "g11-user-path-revoked", version: "1" }, { capabilities: {} });
  ...
  row("8 the client that was already connected is refused on its next request after the withdrawal (H9 fence, G12-P2-1)", afterRevoke.isError === true, afterRevoke.isError ? `refused ${JSON.stringify(afterRevoke.structuredContent).slice(0, 240)}` : `still edits: status=${afterRevoke.structuredContent?.status}`);
} catch (error) {
  row("run", false, error instanceof Error ? (error.stack ?? error.message) : String(error));
  await page.screenshot({ path: join(OUT, `g11-user-path-${LOCALE}-error.png`) }).catch(() => undefined);
} finally {
  await client?.close().catch(() => undefined);
  writeFileSync(join(OUT, `g11-user-path-${LOCALE}-console.txt`), lines.slice(-400).join("\n"));
  await browser.close();
}
const red = rows.filter((entry) => !entry.ok);
const userPath = rows.filter((entry) => /^[0-7] /u.test(entry.step));
console.log(`g11-user-path (${LOCALE}): user path ${userPath.filter((entry) => entry.ok).length}/8, revocation row 8 ${rows.find((entry) => entry.step.startsWith("8 "))?.ok ? "PASS" : "FAIL"}, ${rows.length - red.length}/${rows.length} rows green in ${seconds()} s`);
