#!/usr/bin/env bun
/** 💼️ G11 live proof (G9's `g9-quartet-live.ts`, re-pointed): the generic inference quartet over the stdio semio MCP against a hub.
 ...
 * human reads the advanced ledger head. The run owns a fresh space. Usage: bun g11-quartet-live.ts <hubOrigin> [binary] */
...
const OUT = "/Users/ueli/Documents/semio/.🧬semio/🌐hub/s13-g11-logs";
...
class Agent {
  ...
  async start(): Promise<void> {
    await this.request("initialize", { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: `g11-${this.name}`, version: "1" } });
...
const delegate = async (human: string, spaceId: string, label: string) => {
  ...
  const path = join(mkdtempSync(join(tmpdir(), "semio-g11-agent-")), "agent-credential.json");
...
try {
  const signIn = await hub("POST", "/auth/sessions", undefined, { schema: "semio.hub.auth.credential-sign-in/v1", email: EMAIL, password: PASSWORD, deviceInstanceId: `g11human${randomBytes(12).toString("hex")}`, clientClass: "browser" });
  ...
  const spaceName = `G11 quartet ${randomBytes(3).toString("hex")}`;
  ...
  const grid = await createDocument(human, spaceId, "wfcgrid3d", "G11 wfc grid3d solve");
  row("2 human creates a wfc grid3d document on the hub", grid.documentId.length > 0, grid.detail);
  const map = await createDocument(human, spaceId, "gis.gismap", "G11 gis map");
  row("3 human creates a gis map document on the hub", map.documentId.length > 0, map.detail);

  const a = await delegate(human, spaceId, "G11 agent A");
  const b = await delegate(human, spaceId, "G11 agent B");
  ...
  writeFileSync(join(OUT, "g11-quartet-live-transcript.json"), JSON.stringify({ rows, transcript, progressA: agentA?.progress, stderrA: agentA?.stderr.slice(-6000), stderrB: agentB?.stderr.slice(-3000) }, null, 2));
  agentA?.stop();
  agentB?.stop();
  ...
  console.log(`g11-quartet-live: ${rows.length - red}/${rows.length} rows green against ${ORIGIN}`);
