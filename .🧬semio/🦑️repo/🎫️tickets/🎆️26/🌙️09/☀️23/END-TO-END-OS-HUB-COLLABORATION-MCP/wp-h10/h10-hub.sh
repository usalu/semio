#!/bin/zsh
# 🌎️ H10 hub lifecycle on a copy of catalog B2 (production mode, loopback, credential sign-in).
#   h10-hub.sh prepare <root-name>                 fresh data root under .🧬semio/🌐hub/<root-name> (APFS clone of B2 + 2 users)
#   h10-hub.sh start <root-name> <port> <binary>   detached hub (NI 0), pid → <root>.pid, log → s12-h10-logs/<root-name>-<port>-<n>.log
#   h10-hub.sh stop <root-name>                    SIGTERM the recorded pid, wait for exit (≤ 60 s), print exit timing
#   h10-hub.sh wait-ready <port> <start-epoch-ms> <timeout-s>   poll /readyz until 200; prints elapsed ms
set -u
...
case $cmd in
  prepare)
    ...
    BIN=${2:-"$H/s12-h10-bin/os-hub-before"}
