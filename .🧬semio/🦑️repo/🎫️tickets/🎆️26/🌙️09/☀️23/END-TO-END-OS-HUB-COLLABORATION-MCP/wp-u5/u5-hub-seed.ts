 ...
 * command route, then lists what the directory answers for her. Usage: bun u5-hub-seed.ts <origin> [--email=<email>] [--delete=<namePrefix>...] <spaceName...> */
...
const email = process.argv.slice(3).find((arg) => arg.startsWith("--email="))?.slice("--email=".length) ?? "ada@example.org";
const names = process.argv.slice(3).filter((arg) => !arg.startsWith("--delete=") && !arg.startsWith("--email="));

const minted = await fetch(`${origin}/auth/sessions`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ schema: "semio.hub.auth.credential-sign-in/v1", email, password: process.env.U5_PASSWORD ?? "correct horse battery staple", deviceInstanceId: "u5-seed", clientClass: "browser" }) });
...
for (const [index, name] of names.entries()) {
  ...
  const response = await fetch(`${origin}/directory/commands`, { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${session.token}`, origin: process.env.U5_ORIGIN ?? "http://127.0.0.1:6580" }, body: sealed });
