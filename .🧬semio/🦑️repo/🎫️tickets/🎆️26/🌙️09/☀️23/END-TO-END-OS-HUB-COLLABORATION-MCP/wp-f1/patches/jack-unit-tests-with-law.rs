...
/// ⌨️ A typing burst longer than the store's fixed applied-edit ledger (64) keeps saving: every keystroke's `text-edit`
/// amends the burst's one coalesced window-config edit (ticket 26/09/23 F1: the 65th character was refused).
#[semio_framework_async_macros::async_test]
async fn a_typing_burst_longer_than_the_edit_ledger_keeps_saving() {
    let mut app = new_app().await;
    let view = query_windows();
    let editor = view.for_window_instance("editor-main").unwrap();
    let typed: String = "MATCH (a:Piece) WHERE a.name = 'core' RETURN a.name, a.label, a.kind ORDER BY a.name LIMIT 25".chars().take(96).collect();
    assert!(typed.chars().count() > 64);
    for end in 1..=typed.chars().count() {
        let text: String = typed.chars().take(end).collect();
        app.dispatch_typed(TrinityJackCommand::TextEdit { text }, &semio_framework_plugin::ActionMeta { view_state: Some(editor.clone()), ..meta("local") }).await.unwrap_or_else(|error| panic!("keystroke {end} was refused: {error:?}"));
        drive_query_ownership_operations(&mut app).await.unwrap_or_else(|error| panic!("keystroke {end} did not publish: {error}"));
    }
    let node = app.render(TRINITY_JACK_PLAY_BODY_EDITOR, None, &editor).await.expect("render");
    let json = artifact_app_laws::project_and_retire_fixture_tree(node).expect("project semantic UI test tree");
    let scene = artifact_app_laws::decode_fixture_scene_with_lanes::<semio_framework_plugin::TextEditorScene>(&json).expect("text-editor scene");
    assert_eq!(scene.buffer, typed);
}
