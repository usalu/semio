...
const log = (...parts) => console.log("[s16-hub]", ...parts);
...
try {
  ...
  await page.screenshot({ path: `${generated}s16-hub-document-${tag}-spaces.png` }).catch(() => undefined);
  ...
  await page.screenshot({ path: `${generated}s16-hub-document-${tag}-index.png` }).catch(() => undefined);
  ...
  result.rowsAfter = await indexRows(page);
  await page.screenshot({ path: `${generated}s16-hub-document-${tag}-created.png` }).catch(() => undefined);
  ...
  await page.screenshot({ path: `${generated}s16-hub-document-${tag}-opened.png` }).catch(() => undefined);
  ...
  await page.screenshot({ path: `${generated}s16-hub-document-${tag}-mutated.png` }).catch(() => undefined);
  ...
  if (process.env.S15_REOPEN === "1" && result.openedWindows.length > 0) {
    ...
    if (process.env.S15_OPEN_TRACE === "1" && (result.reopenedWindows ?? []).length > 0) {
      result.reopenTrace = [];
      for (let sample = 0; sample < 60; sample += 1) {
        ...
        if (result.reopenTrace.at(-1)?.row !== row) result.reopenTrace.push({ atMs: sample * 250, row });
        await page.waitForTimeout(250);
      }
      log(`reopen trace ${JSON.stringify(result.reopenTrace).slice(0, 1500)}`);
    }
    await page.screenshot({ path: `${generated}s16-hub-document-${tag}-reopened.png` }).catch(() => undefined);
  }
} catch (error) {
...
writeFileSync(`${generated}s16-hub-document-${tag}.txt`, JSON.stringify(result, null, 2));
log(`=== ${generated}s16-hub-document-${tag}.txt ===`);
