#!/usr/bin/env bun
/** 🌎️ S15 hub hold (a copy of W2's `w2-hub-hold.ts` whose readiness no-progress bound is `S15_READINESS_STALL_MS`, default 30 min, because a debug hub compiling seven guest codecs under fleet load reports nothing for longer than the shared 5 min): owns the local-bootstrap pipe for one hub and never exits while it runs.
...
try {
  const readiness = await waitForReadiness(run, false, Number(process.env.S15_READINESS_STALL_MS ?? 1_800_000));
