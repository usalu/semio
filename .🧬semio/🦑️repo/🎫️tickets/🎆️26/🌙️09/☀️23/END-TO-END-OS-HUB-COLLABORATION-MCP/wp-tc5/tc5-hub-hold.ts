/** 🌎️ Slice TC5 — a copy of `tc5-hub-hold.ts` with ONE difference: the readiness stall bound.
 ...
 *
 * Usage: `bun tc5-hub-hold.ts <port> <absoluteDataDir> <absoluteBinaryPath>` */
...
const TC5_READINESS_STALL_BOUND_MS = 1_800_000;
...
const run = await startLocalHub(repoRoot, hubRustRoot, [{ profileId: "developer", subject: "local-developer-tc5", displayName: "Local Developer", allowedClientClasses: ["native", "mcp"] }], {
  port,
  dataDir: dataRoot,
  binaryPath,
  capture: false,
});
const readiness = await waitForReadiness(run, true, TC5_READINESS_STALL_BOUND_MS);
