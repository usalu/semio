#!/usr/bin/env bun
/** 🎯️ W3 (from W2): proves a hub serves the catalog's creation catalog and issues a document-open plan for each named kind.
 ...
 * usage: bun w3-open-plan-probe.ts <origin> <kindId|all> [kindId…] */
...
for (const kind of selected) {
  ...
  const accepted = await call("POST", route, token, sealSpaceArtifactCreateV1({ requestId, expectedCatalogGenerationId: catalog.json.catalogGenerationId, kindId: kind.kindId, name: `W3 ${kind.kindId}` }));
  ...
  const plan = await call("POST", `/spaces/${encodeURIComponent(spaceId)}/documents/${encodeURIComponent(documentId)}/open-plan`, token, { schema: "semio.hub.document-open-intent/v1", version: 1, scope: { spaceId, documentId }, clientInstanceId: "w3-open-plan-probe" });
