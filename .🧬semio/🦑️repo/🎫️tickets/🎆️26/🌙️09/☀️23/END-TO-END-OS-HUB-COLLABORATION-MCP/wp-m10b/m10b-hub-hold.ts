#!/usr/bin/env bun
/** 🌎️ M10b hub hold — detached-safe. Usage:
 *   nohup bun m10b-hub-hold.ts <port> <dataDir> <binary> <pidFile> <statusFile> >>log 2>&1 </dev/null &
 */
...
const run = await startLocalHub(repoRoot, hubRust, [{ profileId: "developer", subject: "local-developer-m10b", displayName: "Local Developer", allowedClientClasses: ["native", "mcp"] }], {
  port,
  dataDir: dataRoot,
  binaryPath,
  capture: true,
