...
echo "=== start $(date +%T)"
cargo build -p semio-hub --bin os-hub --no-default-features --features sqlite,postgres,neo4j,native-artifact-execution 2>&1 | /usr/bin/grep -E "^error|Finished|warning: unused" | head -20
