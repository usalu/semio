...
async function awaitPropagation(session, baseline, budgetMs = 60_000) {
  ...
  while (Date.now() - startedAt < budgetMs) {
    ...
    if (baseline.inspector === undefined && (last.ledgerCount !== baseline.ledgerCount || last.edits !== baseline.edits)) return { changed: true, ...last };
    await session.page.waitForTimeout(2_000);
  }
  const canvas = await canvasHash(session.page);
  const inspectorRows = await inspector(session.page);
  const inspectorChanged = baseline.inspector !== null && baseline.inspector !== undefined && JSON.stringify(inspectorRows) !== JSON.stringify(baseline.inspector);
  return { changed: inspectorChanged, ...last, canvas, canvasChanged: canvas !== baseline.canvas, inspectorChanged, inspectorTail: inspectorRows.slice(-3) };
}
...
if (wanted("2")) {
  ...
  const propagated = await awaitPropagation(B, { ledgerCount: before.ledgerCount, edits: before.edits, canvas: before.canvas, inspector: before.inspector });
  const afterB = await witness(B);
  const afterA = await witness(A);
  report.step2 = { authoredByA: authored, bBefore: before, propagated, bAfter: afterB, aAfter: afterA };
  console.log(`[C8] step2 A inspector ${JSON.stringify(afterA.inspector.slice(0, 3))} B inspector ${JSON.stringify(afterB.inspector.slice(0, 3))}`);
  save();
...
// 3 — live edit B→A
if (wanted("3")) {
  const before = await witness(A);
  const authored = await authorEdit(B, "addFeature");
  const propagated = await awaitPropagation(A, { ledgerCount: before.ledgerCount, edits: before.edits, canvas: before.canvas, inspector: before.inspector });
...
if (wanted("6")) {
  ...
  const propagated = await awaitPropagation(B, { ledgerCount: bBefore.ledgerCount, edits: bBefore.edits, canvas: bBefore.canvas, inspector: bBefore.inspector }, 90_000);
