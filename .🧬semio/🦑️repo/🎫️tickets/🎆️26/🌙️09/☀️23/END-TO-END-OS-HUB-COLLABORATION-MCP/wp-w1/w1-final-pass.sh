...
# wasm-mutex hold running ONE product verb; Nx runs the per-component tasks inside it with --parallel=2 over the shared
...
if [ $from -le 1 ]; then
  step activate-s; retry activate-s "$OUT/final-1-activate.txt" bun nx run @semio-tech/framework-os-dev:activate-s-react-dev --parallel=2 --outputStyle=stream; done_ activate-s $? "$OUT/final-1-activate.txt"
fi
if [ $from -le 2 ]; then
  step describe-all; retry describe-all "$OUT/final-2-describe.txt" bun nx run-many -t describe --projects="$DESCRIBE" --parallel=2 --outputStyle=stream; done_ describe-all $? "$OUT/final-2-describe.txt"
fi
if [ $from -le 3 ]; then
...
if [ $from -le 4 ]; then
  step release-all; retry release-all "$OUT/final-4-release.txt" bun nx run-many -t component-release --projects="$RELEASE" --parallel=2 --outputStyle=stream; done_ release-all $? "$OUT/final-4-release.txt"
fi
if [ $from -le 5 ]; then
