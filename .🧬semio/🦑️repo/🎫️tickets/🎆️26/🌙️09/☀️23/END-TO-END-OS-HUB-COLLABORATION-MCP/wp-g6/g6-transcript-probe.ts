...
const RUN = process.env.G6_RUN ?? "probe";
const SOLVE_BUDGET_MS = Number(process.env.G6_SOLVE_BUDGET_MS ?? 300_000);
...
try {
  const init = await request("initialize", { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "g6-probe", version: "1" } });
  ...
  if (process.env.G6_ONLY_GENESIS_PAYLOAD) {
    const token = `g6-gen-${Date.now().toString(36)}`;
    const genesis = { schema: "s.wfc.bitmap", seed: 0, input: { width: 2, height: 2, palette: [{ r: 0, g: 0, b: 0, a: 255 }, { r: 255, g: 255, b: 255, a: 255 }], pixels: Buffer.from([0, 1, 1, 0]).toString("base64") }, output: { width: Number(process.env.G6_OUT_W ?? 2), height: Number(process.env.G6_OUT_H ?? 2), periodic: process.env.G6_OUT_PERIODIC === "1" }, model: { patternSize: 2, symmetry: 1, periodicInput: true }, pinned: [] };
  ...
  if (process.env.G6_ONLY_ARTIFACT_SOLVE) {
    const wfcId = `g6-wfc-${Date.now().toString(36)}`;
    ...
    writeFileSync(join(OUT, "g6-wfc-genesis-pair.json"), JSON.stringify({ pack: snap.structuredContent?.packBase64, spr: snap.structuredContent?.sprBase64 }));
    ...
    if (process.env.G6_EXPORT_ONLY) throw new Error("export-only");
    const token = `g6-art-${Date.now().toString(36)}`;
    const ran = await issue("tools/call", { name: "inference_run", arguments: { artifactKind: "s.wfc.bitmap", inferenceSchema: "s.wfc.bitmap.solve", pluginId: "wfc", artifactId: wfcId, cancellationId: token, ...(process.env.G6_WORK_UNITS ? { workUnits: Number(process.env.G6_WORK_UNITS) } : {}) }, _meta: { progressToken: token } }, SOLVE_BUDGET_MS).reply;
  ...
  const artifactId = `g6-note-${Date.now().toString(36)}`;
  ...
  const solveToken = `g6-solve-${Date.now().toString(36)}`;
  ...
  const cancelToken = `g6-cancel-${Date.now().toString(36)}`;
  ...
  console.log(`g6-probe: ${rows.length - failed}/${rows.length} rows green`);
  writeFileSync(join(OUT, `g6-${RUN}-transcript.json`), JSON.stringify({ bin, folder, rows, transcript: transcript.map((entry) => { const text = JSON.stringify(entry.msg); return text.length > 20_000 ? { ...entry, msg: { truncated: text.slice(0, 2_000), bytes: text.length } } : entry; }), stderrTail: stderr.slice(-8_000) }, null, 2));
