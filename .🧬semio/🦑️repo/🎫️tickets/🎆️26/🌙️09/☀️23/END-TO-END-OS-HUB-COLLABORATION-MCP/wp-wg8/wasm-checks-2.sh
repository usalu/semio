#!/bin/zsh
# WG8: wasm32 compile gates after the frame-decode, creation-door and shell edits, one wasm hold.
...
cargo check -p semio-framework-os-renderer-wgpu --lib --target wasm32-unknown-unknown --message-format=short 2>&1 | grep -E "^error|error\[|: error|Finished|warning: .semio-framework-os-renderer-wgpu. \(lib\)"
