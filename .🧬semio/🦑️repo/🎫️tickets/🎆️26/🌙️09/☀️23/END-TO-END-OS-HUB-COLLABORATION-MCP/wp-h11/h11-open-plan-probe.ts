 ...
 * H11: logs every creation phase change and then requests the four execution-target assets of the created document.
 * usage: bun h11-open-plan-probe.ts <origin> <kindId|all> [kindId…] */
...
const signIn = await call("POST", "/auth/sessions", undefined, { schema: "semio.hub.auth.credential-sign-in/v1", email: "user1@semio.dev", password: "gm1-local-dev-pass-1", deviceInstanceId: `h11openplan${randomBytes(11).toString("hex")}`, clientClass: "browser" });
const token = String(signIn.json?.token ?? "");
if (signIn.status !== 200 || !token) throw new Error(`sign-in failed ${signIn.status}`);
const created = await call("POST", "/directory/commands", token, { schema: "semio.directory.command-request.v1", requestId: randomBytes(16).toString("hex"), command: { kind: "create-space", name: `H11 open-plan probe ${new Date().toISOString()}`, spaceKind: "studio", visibility: "private" } });
...
for (const kind of selected) {
  ...
  const accepted = await call("POST", route, token, sealSpaceArtifactCreateV1({ requestId, expectedCatalogGenerationId: catalog.json.catalogGenerationId, kindId: kind.kindId, name: `H11 ${kind.kindId}` }));
  let status = accepted.json ?? {};
  let seen = "";
  while (accepted.status < 300 && ["accepted", "preparing", "indeterminate"].includes(status.phase) && Date.now() - started < 900_000) {
    const line = `${status.phase} ${JSON.stringify(status.progress ?? null)}`;
    if (line !== seen) console.log(`  ${kind.kindId} +${Date.now() - started}ms ${line}`);
    seen = line;
    await Bun.sleep(500);
  ...
  const plan = await call("POST", `/spaces/${encodeURIComponent(spaceId)}/documents/${encodeURIComponent(documentId)}/open-plan`, token, { schema: "semio.hub.document-open-intent/v1", version: 1, scope: { spaceId, documentId }, clientInstanceId: "h11-open-plan-probe" });
  ...
  if (ok) {
    for (const asset of ["manifest", "component", "descriptor", "browser-actor"]) {
      ...
      const response = await fetch(`${origin}/spaces/${encodeURIComponent(spaceId)}/documents/${encodeURIComponent(documentId)}/execution-target/${asset}`, { method: "POST", headers: { authorization: `Bearer ${token}`, "content-type": "application/json" }, body: JSON.stringify({ schema: "semio.hub.document-open-intent/v1", version: 1, scope: { spaceId, documentId }, clientInstanceId: "h11-open-plan-probe" }), signal: AbortSignal.timeout(120_000) });
      const body = new Uint8Array(await response.arrayBuffer());
      const declared = Number(response.headers.get("content-length") ?? -1);
      const good = response.status === 200 && (declared < 0 || declared === body.byteLength);
      if (!good && !(asset === "browser-actor" && response.status === 404)) failed += 1;
      console.log(`  ${kind.kindId} execution-target/${asset} ${response.status} ${body.byteLength}B ${Math.round(performance.now() - t0)}ms${response.status === 200 ? "" : ` ${response.headers.get("x-semio-refusal")} ${new TextDecoder().decode(body).slice(0, 160)}`}`);
    }
  }
