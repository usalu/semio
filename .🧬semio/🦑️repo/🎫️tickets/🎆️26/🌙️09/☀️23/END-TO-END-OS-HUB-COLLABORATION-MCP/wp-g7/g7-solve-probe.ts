#!/usr/bin/env bun
/** ⏱️ G7 over-MCP throughput probe: artifact_create s.wfc.bitmap → inference_run on that artifact (the client-e2e genesis solve), timed end to end, with the relay's stderr lines streamed; G7_CANCEL_AFTER_MS sends notifications/cancelled mid-solve. */
...
const RUN = process.env.G7_RUN ?? "solve";
const BIN = process.env.SEMIO_OS_MCP_BIN ?? "/Users/ueli/Documents/semio/.tmp-ticket/wp-g7/target/debug/semio-os-mcp";
const BUDGET_MS = Number(process.env.G7_BUDGET_MS ?? 600_000);
const CANCEL_AFTER_MS = Number(process.env.G7_CANCEL_AFTER_MS ?? 0);
const CANCEL_AT_PROGRESS = Number(process.env.G7_CANCEL_AT_PROGRESS ?? 0);
...
const child = spawn(BIN, ["stdio", "--folder", mkdtempSync(join(tmpdir(), "semio-g7-probe-")), "--no-bridge", "--scopes", SCOPES], { stdio: ["pipe", "pipe", "pipe"] });
createInterface({ input: child.stderr }).on("line", (line) => {
  if (line.includes("[DEBUG] g7") || /error|panic|fault/i.test(line)) say(`stderr ${line.slice(0, 400)}`);
});
...
let ok = false;
try {
  await request("initialize", { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "g7-probe", version: "1" } }).reply;
  child.stdin.write(`${JSON.stringify({ jsonrpc: "2.0", method: "notifications/initialized", params: {} })}\n`);
  const artifactId = `g7-wfc-${Date.now().toString(36)}`;
  if (process.env.G7_CREATE_CANCEL) {
    const createToken = `g7-create-${Date.now().toString(36)}`;
    const cancelledId = `${artifactId}-cancelled`;
    const create = request("tools/call", { name: "artifact_create", arguments: { artifactId: cancelledId, kind: "s.wfc.bitmap" }, _meta: { progressToken: createToken } }, BUDGET_MS);
    const workers = () => { try { return execFileSync("pgrep", ["-P", String(child.pid), "-f", "compile-component"]).toString().trim().split("\n").filter(Boolean); } catch { return []; } };
    cancelArmed = () => {
      const last = log[log.length - 1] ?? "";
      if (cancelSentAt === 0 && last.includes(process.env.G7_CREATE_CANCEL ?? "compiling-component")) {
        setTimeout(() => {
          say(`compile workers before cancel: ${JSON.stringify(workers())}`);
          cancelSentAt = Date.now();
          say(`sending notifications/cancelled for artifact_create requestId=${create.id}`);
          child.stdin.write(`${JSON.stringify({ jsonrpc: "2.0", method: "notifications/cancelled", params: { requestId: create.id, reason: "g7 create probe" } })}\n`);
        }, Number(process.env.G7_CANCEL_DELAY_MS ?? 2000));
      }
    };
    const cancelledReply = await create.reply;
    say(`artifact_create answered ${Date.now() - cancelSentAt} ms after the cancel: ${JSON.stringify(cancelledReply.result?.structuredContent ?? cancelledReply.error).slice(0, 240)}`);
    say(`compile workers after cancel: ${JSON.stringify(workers())}`);
    const listed = await call("artifact_open", { artifactId: cancelledId });
    say(`cancelled artifact open: ${listed?.isError ? "absent (error)" : "PRESENT"}`);
    cancelArmed = null;
    cancelSentAt = 0;
  }
  ...
  if (process.env.G7_IDLE_AFTER_CREATE_MS) {
    await new Promise((resolve) => setTimeout(resolve, 5_000));
    const idleBefore = cpuSeconds();
    const idleMs = Number(process.env.G7_IDLE_AFTER_CREATE_MS);
    await new Promise((resolve) => setTimeout(resolve, idleMs));
    say(`idle with a live engine and guest: process cpu ${(cpuSeconds() - idleBefore).toFixed(2)} s over ${(idleMs / 1000).toFixed(0)} s`);
  }
  for (let repeat = Number(process.env.G7_REPEAT ?? 1); repeat > 1; repeat--) {
    const warmToken = `g7-warm-${repeat}-${Date.now().toString(36)}`;
  ...
  const token = `g7-${Date.now().toString(36)}`;
  ...
  const sendCancel = () => {
    ...
    child.stdin.write(`${JSON.stringify({ jsonrpc: "2.0", method: "notifications/cancelled", params: { requestId: ran.id, reason: "g7 probe" } })}\n`);
  };
  if (CANCEL_AT_PROGRESS > 0) {
    cancelArmed = (progress) => {
      if (Math.abs(progress - CANCEL_AT_PROGRESS) < 1e-6 && cancelSentAt === 0) setTimeout(sendCancel, Number(process.env.G7_CANCEL_DELAY_MS ?? 0));
    };
  }
  ...
  child.kill();
  writeFileSync(join(OUT, `g7-${RUN}.txt`), `${log.join("\n")}\n`);
}
process.exit(ok ? 0 : 1);
