 ...
 *
 * Usage: bun s15-ux.mjs <baseUrl> [persist|locale|keyboard|devices ...]  → generated/s15-ux-<section>.json (+ png)
 */
...
async function persist() {
  ...
  await page.screenshot({ path: out("s15-ux-persist-dark.png") });
...
async function locale() {
  ...
  await page.screenshot({ path: out("s15-ux-locale-de.png") });
...
async function keyboard() {
  ...
  await page.screenshot({ path: out("s15-ux-keyboard-dag.png") });
  await context.close();
...
async function devices() {
  ...
  for (const [name, viewport] of [["phone", { width: 375, height: 812 }], ["tablet", { width: 768, height: 1024 }]]) {
    ...
    await page.screenshot({ path: out(`s15-ux-${name}-home.png`) });
    const before = await windowIds(page);
    await page.evaluate(() => document.body.focus());
    await page.keyboard.press(process.platform === "darwin" ? "Meta+p" : "Control+p");
    await page.waitForTimeout(1_200);
    ...
    await page.screenshot({ path: out(`s15-ux-${name}-program.png`) });
...
for (const section of sections) {
  ...
  writeFileSync(out(`s15-ux-${section}.json`), JSON.stringify({ section, ms: Date.now() - started, result }, null, 1));
