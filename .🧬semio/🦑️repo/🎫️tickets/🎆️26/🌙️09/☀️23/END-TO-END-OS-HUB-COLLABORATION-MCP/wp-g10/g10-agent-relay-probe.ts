#!/usr/bin/env bun
/** 🔬️ G10: does a hub-bound agent's committed edit reach the hub? API-only (no browser): a fresh space + note by the
 * hub's authorities, a delegated `semio-os-mcp --hub` agent opens it and commits `addBlock`, then the hub's own document
 * head is polled. Prints the gateway's stderr tail on a miss. usage: bun g10-agent-relay-probe.ts <hub> <rounds> */
...
const token = String((await hub("POST", "/auth/sessions", undefined, JSON.stringify({ schema: "semio.hub.auth.credential-sign-in/v1", email: "user2@semio.dev", password: "gm1-local-dev-pass-2", deviceInstanceId: `g10relay${randomBytes(12).toString("hex")}`, clientClass: "browser" }))).json?.token ?? "");
for (let round = 0; round < Number(ROUNDS); round += 1) {
  const spaceName = `G10 relay ${randomBytes(3).toString("hex")}`;
  ...
  const credentialPath = join(mkdtempSync(join(tmpdir(), "g10-relay-")), "credential.json");
  ...
  await agent.request("initialize", { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "g10-relay", version: "1" } }, 600_000);
