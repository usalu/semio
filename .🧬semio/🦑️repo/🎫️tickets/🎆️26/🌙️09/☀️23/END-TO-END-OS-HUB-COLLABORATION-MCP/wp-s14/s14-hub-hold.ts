/** 🌎️ Slice S14 — a copy of `tc5-hub-hold.ts` with ONE difference: the readiness stall bound.
 ...
 *
 * Usage: `bun s14-hub-hold.ts <port> <absoluteDataDir> <absoluteBinaryPath>` */
...
const S14_READINESS_STALL_BOUND_MS = 1_800_000;
...
const run = await startLocalHub(repoRoot, hubRustRoot, [{ profileId: "developer", subject: "local-developer-s14", displayName: "Local Developer", allowedClientClasses: ["native", "mcp"] }], {
  port,
  dataDir: dataRoot,
  binaryPath,
  capture: false,
});
const readiness = await waitForReadiness(run, true, S14_READINESS_STALL_BOUND_MS);
