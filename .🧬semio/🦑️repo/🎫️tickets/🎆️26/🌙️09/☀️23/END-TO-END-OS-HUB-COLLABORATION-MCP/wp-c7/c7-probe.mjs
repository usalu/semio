...
const lines = [];
page.on("console", (m) => lines.push(`${ms()} ${m.type()} ${m.text().slice(0, 900)}`));
page.on("pageerror", (e) => lines.push(`${ms()} pageerror ${String(e).slice(0, 900)}`));
...
await signIn(page, USERS[0].email, USERS[0].password);
await page.waitForTimeout(2000);
const attach = await attachRemote(page);
for (let i = 0; i < 60; i++) { const r = await read(page); if (/Persisted/.test(r.syncPill ?? "")) break; await page.waitForTimeout(2000); }
...
console.log("candidates", JSON.stringify(await page.evaluate(() => [...document.querySelectorAll("*")].filter((e) => e.children.length < 3 && /Add Feature/.test(e.textContent ?? "")).slice(0, 12).map((e) => [e.tagName, e.id, e.getAttribute("data-row-id"), e.getAttribute("role"), e.closest("[id]")?.id]))));
const opened = await click(page, '[id="action.addFeature"]');
await page.waitForTimeout(2500);
const ids = await page.evaluate(() => [...document.querySelectorAll("[id]")].map((e) => e.id).filter((id) => /addFeature|dialog|execute|arg\./i.test(id)).slice(0, 60));
console.log("opened", opened, JSON.stringify(ids));
await page.screenshot({ path: join(OUT, `${TAG}-form.png`) });
console.log("form", JSON.stringify(await page.evaluate(() => { const q = (id) => document.getElementById(id); const ex = q("framework.window.gis2dMain.action.addFeature.execute"); const args = ["collection","label","lon","lat","span"].map((k) => { const el = q(`action.addFeature.arg.${k}`); return [k, el?.tagName, el?.getAttribute("aria-invalid"), el?.value ?? el?.textContent?.slice(0, 40), el?.getAttribute("aria-required"), el?.offsetParent !== null]; }); return { ex: ex ? [ex.tagName, ex.getAttribute("disabled"), ex.getAttribute("aria-disabled"), ex.offsetParent !== null, ex.textContent] : null, args, panel: ex?.closest("[data-slot]")?.getAttribute("data-slot") }; })));
for (const [k, v] of [["label", "C7 probe"], ["lon", "8.5"], ["lat", "47.4"]]) { const input = page.locator(`[id="action.addFeature.arg.${k}"] input, [id="action.addFeature.arg.${k}"] textarea`).first(); if (await input.count()) await input.fill(v).catch((e) => console.log("fill", k, String(e).slice(0, 80))); else console.log("noinput", k); await page.waitForTimeout(300); }
const submitted = await (async () => { const sel = '[id$=".action.addFeature.execute"], [id*="addFeature"][id$="execute"], button:has-text("Run"), button:has-text("Execute"), button:has-text("Apply")'; const n = await page.locator(sel).count(); if (!n) return "absent"; return page.locator(sel).first().click({ timeout: 8000, force: true }).then(() => "ok").catch((e) => String(e).slice(0, 120)); })();
await page.waitForTimeout(8000);
console.log("submitted", submitted);
await page.screenshot({ path: join(OUT, `${TAG}-after.png`) });
