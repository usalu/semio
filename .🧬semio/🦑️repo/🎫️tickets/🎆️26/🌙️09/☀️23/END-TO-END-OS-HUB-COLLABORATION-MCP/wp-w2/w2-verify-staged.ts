/** 🔍️ W2: per component, committed descriptor wasm hash == dist/component-dev bytes == shared wasm-dev bytes ==
...
for (const entry of readdirSync(modules)) {
  const manifest = join(modules, entry, ".nx-artifact.json");
  if (!existsSync(manifest)) continue;
  const artifact = JSON.parse(readFileSync(manifest, "utf8")) as { owner: string; files: string[] };
  if (!artifact.owner.startsWith("✏️s/🔌️plugins/")) continue;
