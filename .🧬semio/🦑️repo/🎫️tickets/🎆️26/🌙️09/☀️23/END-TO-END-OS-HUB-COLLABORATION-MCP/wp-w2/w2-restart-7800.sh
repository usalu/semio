...
# Data root, binary copy and hold state live under `.🧬semio/🌐hub/s12-w2-*` (the canonical hub data home, never swept).
...
OLDHUB=$(sed -n 's/^hub=//p' "$STATE/pids.txt" 2>/dev/null || true)
if [ "$OLD" != "-" ]; then touch "$STATE/stop"; for i in $(seq 1 20); do ps -p "$OLD" >/dev/null || break; sleep 1; done; kill "$OLD" 2>/dev/null || true; fi
