/** 🌱️ C10 directory commands through the product's own sealed request (the same bytes the shell posts).
 * Usage (from wp-c10): bun c10-seed.ts <hubOrigin> spaces
...
async function spaces(token: string): Promise<Array<{ id: string; name: string }>> {
  ...
  return rows.map((row) => row as { id: string; name: string });
}
