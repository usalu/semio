...
if (process.env.C3_ROLE_USER2) execFileSync("bun", ["c10-seed.ts", `http://${HUB}`, "member", SPACE, "user2@semio.dev", process.env.C3_ROLE_USER2], { cwd: import.meta.dir, encoding: "utf8" });
...
if (wanted("outage")) {
  ...
  const hubHead = () => { try { return /"head_seq":(\d+)/.exec(execFileSync("bun", ["c10-doc-state.ts", `http://${HUB}`, SPACE, DOCUMENT], { cwd: import.meta.dir, encoding: "utf8" }))?.[1] ?? null; } catch { return null; } };
...
const seedAs = (...args) => execFileSync("bun", ["c10-seed.ts", `http://${HUB}`, ...args], { cwd: import.meta.dir, encoding: "utf8" });
