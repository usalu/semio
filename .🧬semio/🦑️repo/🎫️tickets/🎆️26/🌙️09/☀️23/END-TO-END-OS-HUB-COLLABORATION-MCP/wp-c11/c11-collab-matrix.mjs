 ...
 * usage: S_MATRIX_LOCALE=de-DE bun c11-collab-matrix.mjs <tag> <user1Url> <user2Url> [spaceId|-] [kindId,kindId,…]
 *        S_MATRIX_PASSWORDS='pw1,pw2' overrides the two humans' passwords (default: hub 7800's). */
import { activate, boot, clickRowAction, dialog, openSessions, read, recorder, selectOption, signIn, submitDialog, waitNamedRow, waitRow, shot, USERS } from "./c11-lib.mjs";
import { PLUGIN_BY_KIND, awaitMounted, awaitText, createArtifact, creatableKinds, docText, edit, faultsSince, hubHead, openRow, openSpace, pause, personalArgs, short, undo, until, awaitSharedSpace } from "./c11-journey.mjs";

const [tag = "c11matrix", url1 = "http://127.0.0.1:6520/", url2 = "http://127.0.0.1:6523/", givenSpace = "-", kindFilter = ""] = process.argv.slice(2);
...
try {
  ...
  if (spaceId === null) {
    const spaceName = `C11 Matrix ${Date.now() % 100000}`;
  ...
  for (const kind of kinds) {
    ...
    try {
      ...
      const aEdit = await edit(A, plugin, personalArgs(plugin, A.user.label));
      ...
      const bEdit = await edit(B, plugin, personalArgs(plugin, B.user.label));
