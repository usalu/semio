/** 🌱️ C11 (from C10) directory commands through the product's own sealed request (the same bytes the shell posts).
 * Usage (from wp-c11): bun c11-seed.ts <hubOrigin> spaces
 *                      bun c10-seed.ts <hubOrigin> create <name>
 *                      bun c10-seed.ts <hubOrigin> member <spaceId> <email> <author|spectator>
 *                      bun c10-seed.ts <hubOrigin> remove <spaceId> <userId>
 *                      bun c10-seed.ts <hubOrigin> artifact <spaceId> <kindId> <name>   (prints `DOCUMENT <id>`)
...
async function mint(email: string): Promise<string> {
  const response = await fetch(`${origin}/auth/sessions`, {
    ...
    body: JSON.stringify({ schema: "semio.hub.auth.credential-sign-in/v1", email, password: USERS[email], deviceInstanceId: `c10seed${email.replace(/\W/g, "")}0000000000000000000000`.slice(0, 32), clientClass: "browser" }),
  });
