/** 🤝️ C10 (from C3/C8) — the ten-step two-human collaboration scenario on ONE shared hub document.
 *
...
const OUT = "/Users/ueli/Documents/semio/.tmp-ticket/wp-c11/generated";
...
const read = (page) =>
  page.evaluate(() => {
    ...
    return {
      ...
      executionTarget: [...document.querySelectorAll("[data-semio-execution-target-status]")].map((el) => `${text(el).slice(0, 110)}${el.getAttribute("data-semio-execution-target-diagnostic") ? ` <${el.getAttribute("data-semio-execution-target-diagnostic")}>` : ""}`),
...
const hubOf = (page) => (page === sessions[1]?.page && process.env.C3_HUB_USER2) || HUB;
async function attachRemote(page, settleMs = Number(process.env.C3_SETTLE_MS ?? 180_000)) {
  ...
  await input.fill(`${hubOf(page)}/${SPACE}/${DOCUMENT}`);
  ...
  return { card: "ok", trail: opened.trail, value: `${hubOf(page)}/${SPACE}/${DOCUMENT}`, attach: pressed };
}

/** ✍️ One authored gis edit: `addFeature` mints the lowest free `position-N` with no argument typed. */
async function authorEdit(session, action = process.env.C3_ACTION ?? "addFeature", staged = action === "addFeature" ? { label: `${session.user.label} ${ms()}`, lon: (Math.random() * 300 - 150).toFixed(3), lat: (Math.random() * 120 - 60).toFixed(3) } : {}) {
...
if (process.env.C3_ROLE_USER2) execFileSync("bun", ["c11-seed.ts", `http://${HUB}`, "member", SPACE, "user2@semio.dev", process.env.C3_ROLE_USER2], { cwd: import.meta.dir, encoding: "utf8" });

// ── run ────────────────────────────────────────────────────────────────────────────────────────
const browser = await chromium.launch({ headless: true, args: ["--use-angle=metal", "--enable-unsafe-webgpu", "--ignore-gpu-blocklist"] });
const sessions = [];
for (const user of USERS) {
  const context = await browser.newContext({ viewport: { width: 1600, height: 1000 }, ...(user.label === "user2" && process.env.C3_LOCALE_USER2 ? { locale: process.env.C3_LOCALE_USER2 } : {}) });
  const page = await context.newPage();
  const lines = [];
  const sockets = [];
  page.on("console", (message) => lines.push(`${ms()} ${message.type()} ${message.text().slice(0, message.text().includes("c10 child fault") ? 4096 : 700)}`));
  page.on("worker", (worker) => worker.on("console", (message) => lines.push(`${ms()} worker-${message.type()} ${message.text().slice(0, 4096)}`)));
  ...
  await page.goto(`${(user.label === "user2" && process.env.C3_SHELL_USER2) || SHELL}${process.env.C3_PATH ?? "/?plugin=gis2d"}`, { waitUntil: "domcontentloaded", timeout: 180_000 });
  let shell = null;
...
if (wanted("2")) {
  ...
  const authored = await authorEdit(A);
...
if (wanted("3")) {
  ...
  const authored = await authorEdit(B);
...
// G-P1-4 — both users hold the inspector open; A authors once. A's own inspector and B's (remote) inspector must
// change in place, B's within ~2 s of A's submit. Rows are polled every 150 ms; no tab is switched while watching.
if (wanted("gp14")) {
  const inspection = '[data-slot="panel-tab-button"][id="framework.panel.inspection"]';
  await Promise.all([click(A.page, inspection), click(B.page, inspection)]);
  await Promise.all([A.page.waitForTimeout(1_500), B.page.waitForTimeout(1_500)]);
  const baseline = { A: (await read(A.page)).panelRows, B: (await read(B.page)).panelRows };
  const action = process.env.C3_ACTION ?? "addFeature";
  const opened = await click(A.page, `[id="action.${action}"]`);
  await A.page.waitForTimeout(1_200);
  const staged = { label: `gp14 ${ms()}`, lon: (Math.random() * 300 - 150).toFixed(3), lat: (Math.random() * 120 - 60).toFixed(3) };
  if (action === "addFeature") for (const [key, value] of Object.entries(staged)) {
    const input = A.page.locator(`[id$=".arg.${key}"] input, [id$=".arg.${key}"] textarea, [id$=".arg.${key}"]:is(input,textarea), [name="${key}"]`).first();
    if (await input.count()) await input.fill(String(value)).catch(() => {});
  }
  const submitted = await click(A.page, `[id$=".action.${action}.execute"]`);
  const submittedAt = Date.now();
  const watch = async (session, rows) => {
    while (Date.now() - submittedAt < 15_000) {
      const now = (await read(session.page)).panelRows;
      if (JSON.stringify(now) !== JSON.stringify(rows)) return { changed: true, ms: Date.now() - submittedAt, rowsBefore: rows.length, rowsAfter: now.length, tail: now.slice(-2) };
      await session.page.waitForTimeout(150);
    }
    return { changed: false, ms: Date.now() - submittedAt, rowsBefore: rows.length, rowsAfter: rows.length, tail: rows.slice(-2) };
  };
  const [own, remote] = await Promise.all([watch(A, baseline.A), watch(B, baseline.B)]);
  report.gp14 = { opened, submitted, staged, baselineRows: { A: baseline.A.length, B: baseline.B.length }, own, remote };
  save();
  await shot(A, "gp14-a-inspector");
  await shot(B, "gp14-b-inspector");
  record("gp14-author-inspector-live", submitted === "ok" && own.changed, JSON.stringify(own));
  record("gp14-peer-inspector-within-2s", submitted === "ok" && remote.changed && remote.ms <= 2_000, JSON.stringify(remote));
  await Promise.all([click(A.page, '[data-slot="panel-tab-button"][id="framework.panel.history"]'), click(B.page, '[data-slot="panel-tab-button"][id="framework.panel.history"]')]);
}
...
if (wanted("6")) {
  ...
  const interim = await authorEdit(A);
...
if (wanted("7")) {
  const burst = async (session, count) => {
    const rows = [];
    for (let index = 0; index < count; index += 1) rows.push(await authorEdit(session));
    return rows;
  };
...
// outage — B's link cut for C3_OUTAGE_MS (5–20 s): no freeze, B edits offline, status shown, reconcile both ways
if (wanted("outage")) {
  const outageMs = Number(process.env.C3_OUTAGE_MS ?? 15_000);
  const hubHead = () => { try { return /"head_seq":(\d+)/.exec(execFileSync("bun", ["c11-doc-state.ts", `http://${HUB}`, SPACE, DOCUMENT], { cwd: import.meta.dir, encoding: "utf8" }))?.[1] ?? null; } catch { return null; } };
  const heads = { beforeCut: hubHead() };
  const aBefore = await read(A.page);
  const bBefore = await read(B.page);
  const linkMode = process.env.C3_LINK_MODE ?? "";
  const cut = linkMode ? await fetch(`${process.env.C3_LINK_CONTROL}/cut?mode=${linkMode}&ms=${outageMs}`, { method: "POST" }).then((r) => r.json()) : await B.context.setOffline(true);
  const cutAt = ms();
  const samples = [];
  const probe = async () => {
    const started = Date.now();
    const shell = await read(B.page);
    const frame = await B.page.evaluate(() => new Promise((resolve) => { const t = performance.now(); requestAnimationFrame(() => resolve(performance.now() - t)); }));
    samples.push({ at: ms() - cutAt, readMs: Date.now() - started, frameMs: Math.round(frame), pill: shell.syncPill, target: shell.executionTarget.join(" ").slice(0, 80) });
  };
  await B.page.waitForTimeout(1_500);
  await probe();
  const offlineEdit = await authorEdit(B);
  await probe();
  const interim = await authorEdit(A);
  heads.afterInterim = hubHead();
  while (ms() - cutAt < outageMs) {
    await probe();
    await B.page.waitForTimeout(1_000);
  }
  if (!linkMode) await B.context.setOffline(false);
  const restoredAt = ms();
  const bSawA = { changed: false, waitedMs: 0 };
  const aSawB = { changed: false, waitedMs: 0 };
  const aMarker = interim.newRow === null ? null : `user1`;
  for (let i = 0; i < 60 && !(bSawA.changed && aSawB.changed); i += 1) {
    await probe();
    const aNow = await read(A.page);
    const bNow = await read(B.page);
    if (!bSawA.changed && bNow.ledger.some((row, index) => index >= bBefore.ledger.length - 3 && /create-position/.test(row) && /user1/.test(row) && !bBefore.ledger.includes(row))) Object.assign(bSawA, { changed: true, waitedMs: ms() - restoredAt, row: bNow.ledger.find((row) => /create-position/.test(row) && /user1/.test(row) && !bBefore.ledger.includes(row)) });
    if (!aSawB.changed && aNow.ledger.some((row) => /create-position/.test(row) && /user2/.test(row) && !aBefore.ledger.includes(row))) Object.assign(aSawB, { changed: true, waitedMs: ms() - restoredAt, row: aNow.ledger.find((row) => /create-position/.test(row) && /user2/.test(row) && !aBefore.ledger.includes(row)) });
    await B.page.waitForTimeout(1_500);
  }
  heads.afterRestore = hubHead();
  report.outage = { heads, linkMode: linkMode || "playwright-offline", cut, outageMs, offlineEdit, interim, samples, bSawA, aSawB, aMarker, after: { a: await read(A.page), b: await read(B.page) } };
  save();
  const worstFrame = Math.max(...samples.map((row) => row.frameMs));
  const pills = [...new Set(samples.map((row) => row.pill))];
  record("outage-no-freeze", worstFrame < 1_000, `B ${linkMode || "playwright-offline"} for ${outageMs} ms: ${samples.length} samples, worst frame ${worstFrame} ms, worst read ${Math.max(...samples.map((row) => row.readMs))} ms; pills ${JSON.stringify(pills)}`);
  record("outage-offline-edit", offlineEdit.mutated, `B authored while offline: ${JSON.stringify({ opened: offlineEdit.opened, submitted: offlineEdit.submitted, editsBefore: offlineEdit.editsBefore, editsAfter: offlineEdit.editsAfter, newRow: offlineEdit.newRow })}`);
  record("outage-reconcile-b-sees-a", bSawA.changed, JSON.stringify(bSawA));
  record("outage-reconcile-a-sees-b", aSawB.changed, JSON.stringify(aSawB));
  record("outage-hub-heads", Number(heads.afterRestore) > Number(heads.afterInterim) && Number(heads.afterInterim) > Number(heads.beforeCut), JSON.stringify(heads));
  await shot(B, "outage-after");
}

// perm — B is a spectator (seeded before boot via C3_ROLE_USER2=spectator): no edit reaches the document; then user1
// removes B from the space while B's document socket is live, and B must lose it.
const seedAs = (...args) => execFileSync("bun", ["c11-seed.ts", `http://${HUB}`, ...args], { cwd: import.meta.dir, encoding: "utf8" });
if (wanted("perm-viewer")) {
  const bBefore = await read(B.page);
  const aBefore = await read(A.page);
  const affordance = await B.page.locator('[id="action.addFeature"]').count();
  const attempt = await authorEdit(B);
  await A.page.waitForTimeout(8_000);
  const aAfter = await read(A.page);
  const aSawB = aAfter.ledger.some((row) => /create-position/.test(row) && /user2/.test(row) && !aBefore.ledger.includes(row));
  report.permViewer = { affordance, attempt, bBefore: bBefore.syncPill, aSawB, bFaults: B.lines.filter((line) => FAULT.test(line) && !NOISE.test(line)).slice(-5) };
  save();
  record("perm-viewer-cannot-write", !aSawB && !(attempt.editsAfter > attempt.editsBefore), JSON.stringify(report.permViewer));
}
if (wanted("perm-remove")) {
  const seed = seedAs;
  const liveBefore = B.sockets.filter((row) => row.closedAt === null).map((row) => row.url.slice(0, 90));
  const removedAt = ms();
  report.permRemove = { seed: seed("remove", SPACE, process.env.C3_USER2_ID ?? "01a0d597-1656-7d96-8437-f836856faea4").trim().slice(0, 300), liveBefore };
  let closed = null;
  for (let i = 0; i < 40 && closed === null; i += 1) {
    await B.page.waitForTimeout(1_000);
    const documentSocket = B.sockets.find((row) => row.url.includes("/document/ws") && row.closedAt !== null && row.closedAt >= removedAt);
    if (documentSocket) closed = documentSocket.closedAt - removedAt;
  }
  await B.page.waitForTimeout(3_000);
  const bAfter = await read(B.page);
  const reopened = B.sockets.filter((row) => row.url.includes("/document/ws") && row.openedAt > removedAt && row.closedAt === null).length;
  report.permRemove = { ...report.permRemove, closedAfterMs: closed, reopenedLive: reopened, bPill: bAfter.syncPill, bTarget: bAfter.executionTarget, bPeers: bAfter.peers.length, bLines: B.lines.slice(-8) };
  save();
  await shot(B, "perm-removed");
  record("perm-removed-member-loses-socket", closed !== null && reopened === 0, JSON.stringify(report.permRemove).slice(0, 1500));
  report.permRestore = seed("member", SPACE, "user2@semio.dev", "author").trim().slice(0, 200);
  save();
}

// presence-life — lease expiry with a live socket, leave, re-join (replay)
if (wanted("presence-life")) {
  const rosterOf = async (session) => (await read(session.page)).peers.length;
  const waitRoster = async (session, expected, budgetMs) => {
    const started = Date.now();
    let seen = -1;
    while (Date.now() - started < budgetMs) {
      seen = await rosterOf(session);
      if (seen === expected) return { seen, ms: Date.now() - started };
      await session.page.waitForTimeout(500);
    }
    return { seen, ms: null };
  };
  const start = { a: await rosterOf(A), b: await rosterOf(B) };
  B.page.evaluate(() => { const until = Date.now() + 22_000; while (Date.now() < until) {} }).catch(() => {});
  await A.page.waitForTimeout(20_000);
  const duringStall = await rosterOf(A);
  await B.page.waitForTimeout(4_000).catch(() => {});
  const afterStall = await waitRoster(A, 2, 10_000);
  const socketsOpen = B.sockets.filter((row) => row.url.includes("/document/ws") && row.closedAt === null).length;
  const reload = B.page.reload({ waitUntil: "domcontentloaded", timeout: 180_000 });
  const leave = await waitRoster(A, 1, 20_000);
  await reload;
  const page = B.page;
  for (let attempt = 0; attempt < 200; attempt += 1) {
    await page.waitForTimeout(1_000);
  ...
  if (await page.locator('[data-semio-hub-sign-in=""]').count()) await signIn(page, B.user.email, B.user.password);
  const rejoinedAt = ms();
  await attachRemote(page);
  const rejoinA = await waitRoster(A, 2, 60_000);
  const rejoinB = await waitRoster(B, 2, 60_000);
  report.presenceLife = { start, duringStall, afterStall, socketsOpenDuringStall: socketsOpen, leave, rejoinA, rejoinB, rejoinedAt };
  save();
  record("presence-lease-live-socket-keeps-row", duringStall === 2 && afterStall.seen === 2, `B's main thread blocked 22 s (no beats, socket open: ${socketsOpen}); A's roster during=${duringStall} after=${afterStall.seen}`);
  record("presence-leave", leave.seen === 1, `B reloaded (its document socket closed); A's roster → ${leave.seen} after ${leave.ms} ms`);
  record("presence-rejoin-symmetric", rejoinA.seen === 2 && rejoinB.seen === 2, `B re-attached; A sees ${rejoinA.seen} (${rejoinA.ms} ms), B sees ${rejoinB.seen} (${rejoinB.ms} ms, join replay)`);
}
...
if (wanted("9")) {
  ...
  if (!script) record("9-mid-edit-hub-restart", null, "C3_HUB_RESTART not set");
    ...
    const midEdit = await authorEdit(A);
    let restart = "";
    ...
    const resumedA = await authorEdit(A);
    const resumedB = await authorEdit(B);
