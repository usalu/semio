#!/usr/bin/env bun
/** 🌎️ C10 hub hold (a copy of S15's `w2-hub-hold.ts` whose readiness no-progress bound is `C10_READINESS_STALL_MS`, default 30 min, because a debug hub compiling seven guest codecs under fleet load reports nothing for longer than the shared 5 min): owns the local-bootstrap pipe for one hub and never exits while it runs.
...
try {
  const readiness = await waitForReadiness(run, false, Number(process.env.C10_READINESS_STALL_MS ?? 1_800_000));
