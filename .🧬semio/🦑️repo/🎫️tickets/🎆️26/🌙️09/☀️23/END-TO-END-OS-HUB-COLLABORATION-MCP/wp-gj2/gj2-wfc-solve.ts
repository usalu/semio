#!/usr/bin/env bun
/** GJ2 — drive s.wfc.bitmap.solve to a RESULT (GJ1 root-fix verification). */
...
const BUDGET_MS = Number(process.env.GJ2_BUDGET_MS ?? 600_000);
const T0 = Date.now();
...
try {
  await mcp.request("initialize", { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "gj2-solve-probe", version: "1" } }, BUDGET_MS);
  ...
  const ran = await mcp.request(
    "tools/call",
    {
      ...
      arguments: {
        ...
        pluginId: "wfc",
        payload: { snapshot },
        cancellationId: `gj2-${Date.now().toString(36)}`,
      },
      _meta: { progressToken: "gj2-solve" },
    },
    BUDGET_MS,
  );
  ...
  writeFileSync(join(OUT, "gj2-wfc-solve.json"), JSON.stringify({ ran, stderr: mcp.stderrText().slice(0, 8000) }, null, 2));
} catch (error) {
  log(`[${at()}] FAILED ${(error as Error).message}`);
  writeFileSync(join(OUT, "gj2-wfc-solve-error.txt"), String(error));
  process.exitCode = 1;
} finally {
  writeFileSync(join(OUT, "gj2-wfc-solve.txt"), lines.join("\n") + "\n");
  writeFileSync(join(OUT, "gj2-wfc-stderr.txt"), mcp.stderrText());
  await mcp.close();
}
