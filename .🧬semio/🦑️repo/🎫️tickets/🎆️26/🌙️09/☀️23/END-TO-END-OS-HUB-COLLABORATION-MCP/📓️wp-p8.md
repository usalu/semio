...
| 11 | measured table: laws, offenders, fixed, per plugin | **partial** (§ Measured) | below |
...
| 14 | Coordinator add-on (A): dead BatchOnly commands | **census done** (46 non-Migrated verbs published: architect 8 ✔ migrated, cad 5 (+5 via demonstrator), animate 1, space 26 (home 4, studio 22), procedural `listFlowExtensions` unclassified); architect migrated + law; the rest FROZEN → patch set | `generated/census-committed.json` |
| 15 | law (e, new): no verb orphans a composed child | **patch set** (`p8-law.py` part B): `composedChildOrphaned` — after every settled probe the law lists each child the parent names that no store holds and did not before (flow's class, generic for every composed surface); verdict fixture +1 case (29) | clone: verdict tests green (`clone-verdicts-orphan`); patches `p8-agent-lane.py` + `p8-law.py` reproduce the clone SDK/action-bus/retained-command/verdict files bit-for-bit |
| 16 | third-party oracle of the verdict fixture | **open**: the fixture's `why` names an AJV twin (`🔌️plugin/📦️packages/🟦️typescript/🧪️tests/⚖️declared-verb-verdicts/🟦️.ts`) that was never written; to write in the landing window (the plugin TS package has no test runner wiring yet) | source |
| 17 | patch-set inventory (land in this order, `cargo check -p` after each) | `p8-agent-lane.py` (3 files) → `p8-law.py` (3) → `p8-flow.py` (23) → `p8-cad.py` (4) → `p8-space-studio.py` (8); all dry-run clean on the tree 2026-09-26 ~11:5x; generator `wp-p8/p8-hunks.py` rebuilds a set from the clone | `patches/*.py --dry-run` |
