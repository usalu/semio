...
| 17 | landing window = ONE command | `python3 .tmp-ticket/wp-p8/p8-land.py --write --test` (plain run = dry run of all sets). Order: `p8-agent-lane` (3 files) → `p8-law` (5) → `p8-flow` (23) → `p8-cad` (4) → `p8-space-studio` (8) → `p8-space-home` (3). Preflight dry-runs every set; each set is backed up, written and gated by `cargo check -p` of the crates it touches (after agent-lane: semio-framework, plugin SDK + flow, cad, sequence guests natively); a failing gate restores only that set (files still byte-equal to what was written) and stops; `--from`/`--only`/`--restore <set>`. Machinery exercised on a scratch root (land flow, forced gate failure on space-home → restored bit-exact; `--restore flow` → tree bytes). All 6 sets dry-run clean on the tree 2026-09-26 15:0x | `p8-land.py` |
