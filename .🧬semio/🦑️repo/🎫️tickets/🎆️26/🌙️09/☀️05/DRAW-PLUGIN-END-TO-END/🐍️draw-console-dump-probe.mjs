/** 🩺️ Draw boot probe: loads a draw react dev playground headless, captures console + page errors,
 * the shell's readiness beacon, every rendered window host and a screenshot.
 * Usage: cd <ticket> && SEMIO_PROBE_URL=http://127.0.0.1:6064/?plugin=draw SEMIO_PROBE_OUT=draw-boot bun 🐍️draw-console-dump-probe.mjs
 */
...
const outDir = join(import.meta.dir, "🗑️generated", process.env.SEMIO_PROBE_OUT ?? "draw-boot");
...
// 🧵️ Guest `eprintln!` lands on the plugin WORKER's console, which the page hook never sees.
page.on("worker", (worker) => { lines.push(`${Date.now() - t0} worker ${worker.url().slice(-80)}`); worker.on("console", (msg) => lines.push(`${Date.now() - t0} worker:${msg.type()} ${msg.text().slice(0, 4000)}`)); });
