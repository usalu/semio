...
### 5.3 `cargo test -p semio-s-plugin-norm --lib --test surface_render` — BLOCKED by a target wipe

```
$ RUSTC_WRAPPER="" CARGO_TARGET_DIR=/Users/ueli/Documents/semio/target-s-e2e \
    cargo test -p semio-s-plugin-norm --lib --test surface_render --message-format=short -- \
    retained_command_dispositions_match_the_language_neutral_oracle \
    set_snapshot_dispatches_through_the_tool_job_path_and_publishes_the_payload_document \
    the_proof_catalog_covers_exactly_the_shared_retained_tool_ids \
    every_norm_editor_action_is_migrated_onto_the_shared_owned_factory
    Blocking waiting for file lock on build directory        # ~50 min behind a peer's wasm check
…                                                           # then ~4 h compiling semio-s-plugin-stdio (test profile)
error: failed to create file encoder: No such file or directory (os error 2)
```

The run died after ~5h48m because **`target-s-e2e` was deleted out from under it by another session**
(`ls -d target-s-e2e` → `No such file or directory`; the ticket's target has since been split into
`target-s-e2e-c`, `target-s-e2e-h`, `target-s-e2e-wasm`). This is the known
"shared cargo target wiped mid-build" failure mode, not a fault in this lane's code — the run never
reached `semio-s-plugin-norm`'s own test target. Lane D's own cargo/rustc processes were killed
(`kill 32112 32106`); no peer process was touched.

Re-launched against the warmest successor cache, `target-s-e2e-h` (463 rmetas, warm
`semio-s-plugin-stdio` fingerprint):

```
$ RUSTC_WRAPPER="" CARGO_TARGET_DIR=/Users/ueli/Documents/semio/target-s-e2e-h \
    cargo test -p semio-s-plugin-norm --lib --test surface_render --message-format=short -- <the four tests>
```

**Status at hand-off: still compiling.** The four tests named above are therefore WRITTEN AND
TYPE-UNVERIFIED — `cargo check --lib` (§5.1) does not compile `#[cfg(test)]` code, so nothing in this
report should be read as "the norm tests pass". That is the single open item; see §6.



## 6. Remaining blockers / notes

1. **`cargo test -p semio-s-plugin-norm --lib` is not yet green — it has never completed.** Two
   attempts were consumed by infrastructure, not by code: the first spent ~50 min behind the
   `target-s-e2e` lock and ~4 h compiling `semio-s-plugin-stdio` in the test profile before the target
   directory was deleted by a peer; the second is queued behind pid 21995 on `target-s-e2e-h`'s lock as
   of hand-off, and is still running detached
   (`scratchpad/lane-d/norm-test2.txt`, look for the `EXIT` line). Until that finishes, the four new
   Rust tests are written but type-unverified. `cargo check --lib` is EXIT 0, so the shared factory,
   the fifteen editors and the `#[cfg(test)]`-free code are proven; the test-only code is not.
2. **Target-directory policy needs a decision from the coordinator.** `target-s-e2e` no longer exists;
   `target-s-e2e-c` / `-h` / `-wasm` replaced it. Lane D picked `-h` (warmest norm dep cache) rather
   than create a sixteenth target root, but nothing stops another session from wiping that one too.
   A stable, named target for this ticket's native checks would remove ~5 h of repeated cold builds.
3. **Descriptor pair still stale** (§4) — the TS oracle's `interactiveJob` comparison stays vacuous
   until the Wave 2 catalog rebuild re-emits `📕️norm/🔣️.json`. No code change needed then.
4. **`setSnapshot` undo granularity changes on the retained lane.** The one-item publication path
   commits one store edit per mutation, whereas `Emit::commit` bundled ~20 field mutations into a single
   undoable edit. Document content is identical (that is what the LIFO reversal in
   `norm_retained_reduce` guarantees), but a user undoing a `setSnapshot` on the migrated path will step
   through the field mutations individually. If that is unacceptable, the fix belongs in the framework's
   `publish_mounted_typed_operation_unit` (batch the drain into one edit), not in norm.
5. **Not attempted, by explicit instruction:** the norm `wasm32-wasip2` build and
   `bun ./📜️script.ts describe`. Verified that no `cargo rustc --target wasm32-wasip2` process belonged
   to this lane before standing down.
6. Peers renamed norm artifact directories throughout the session (`📘️en1995`→…, `📔️vdi3805`→`🏭️vdi3805`,
   `📘️en1999`→`🪶️en1999`, `📓️iso16757`→`📇️iso16757`). Every script in this lane re-globbed the fifteen
   editor paths from disk at run time rather than hard-coding them, and the final on-disk state was
   re-verified after each interruption (15/15 wired, 0 `BatchOnlyPendingRewrite` left anywhere in the
   plugin).
