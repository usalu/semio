...
- 19:15 lane G's boot proof on 6076 (`📓️opus-shard-liveness.md` §9): 0 worker errors, 0 watchdog kills, 0 load timeouts; React mounts and renders the shell chrome; `s#1` activates and its first guest turn traps with `app-definition.interactive-job-classification: unclassified interactive command 's.space.studio@1/*#editor:setAppRegistrations'` → `unreachable`. Current source classifies that command `BatchOnlyPendingRewrite` (`🪐️space/⚙️engine/🪐️space/🦀️.rs:975`, 30 such rows in the engine), so the studio host surface needs the owned-factory migration (lane J) and then a rebuild of the `s` core before the beacon can set.

## Rebuild attempts of the `s` core — 15:35-21:55

| Build | Source state | Cargo | Outcome |
|---|---|---|---|
| 3 (`plugin-build-s-3.txt`) | HEAD 03:53 (`builder("space")`, `package_id("semio:space")`) | killed by the 2 h budget in `target-s-e2e` while stdio codegen queued behind the check lock | no artifact |
| 4 (`plugin-build-s-4.txt`) | same | 97 min in `target-s-e2e-wasm` | core + component JS in cache 19:26; `describeBuiltPlugin`: `Plugin descriptor identity mismatch` (manifest `space` vs registry `s`) |
| 5 (`plugin-build-s-5.txt`) | `builder("s")` restored, `package_id("semio:space")` still present, lane J edits in flight | 97 min | stub descriptor: `component package identity must exactly match semio:<plugin-id>` |
