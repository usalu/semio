...
```
not run by W1 (agent killed by the Opus session limit before this ran; the coordinator's crate-wide runs are authoritative — see `📓️status.md` and `🗑️generated/check-lib-*.txt` / `test-lib-*.txt`)
```
