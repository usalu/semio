/** 🩺️ [DEBUG] temp: diagnose one committed pair of the w6 fill e2e probe (geometry region copied). */
...
const url = (id: string) => dump.meshes.find((m: any) => m.id === id)?.url;
const bodyOf = (id: string) => { const i = dump.instances.find((x: any) => x.id === id); return body(id, meshForUrl(url(i.meshId)), i.position, i.rotation); };
const A = bodyOf(ida), B = bodyOf(idb);
const m = A.mesh;
// watertightness: count edge uses
const key = (p: number[]) => p.map((v) => v.toFixed(5)).join(",");
const edges = new Map<string, number>();
for (let t = 0; t < m.count; t++) { const o = 9 * t; const vs = [0, 1, 2].map((k) => key([m.tris[o + 3 * k], m.tris[o + 3 * k + 1], m.tris[o + 3 * k + 2]])); for (let k = 0; k < 3; k++) { const e = [vs[k], vs[(k + 1) % 3]].sort().join("|"); edges.set(e, (edges.get(e) ?? 0) + 1); } }
const hist: Record<number, number> = {}; for (const v of edges.values()) hist[v] = (hist[v] ?? 0) + 1;
console.log("tris", m.count, "bounds", m.min, m.max, "edge-use histogram", hist);
for (const [a, b] of [[A, B], [B, A]] as const) {
  const inverse = conj(b.rotation); let best: any = null;
  for (const sample of a.mesh.samples) {
    const world = rotate(a.rotation, sample); const w: V3 = [world[0] + a.position[0], world[1] + a.position[1], world[2] + a.position[2]];
    const local = rotate(inverse, [w[0] - b.position[0], w[1] - b.position[1], w[2] - b.position[2]]);
    if (!inside(b.mesh, local)) continue;
    let nearest = Infinity; for (let t = 0; t < b.mesh.count; t++) nearest = Math.min(nearest, pointTriangleDistance(local, b.mesh.tris, 9 * t));
    if (!best || nearest > best.nearest) best = { nearest, sample, world: w, local };
  }
  console.log(`reach ${a.id.slice(-8)} into ${b.id.slice(-8)}`, JSON.stringify(best));
}
writeFileSync(`${import.meta.dir}/🗑️generated/W5-mac-react-e2e/debug-pair.json`, JSON.stringify({ positions: Array.from(m.tris), a: { position: A.position, rotation: A.rotation }, b: { position: B.position, rotation: B.rotation } }));
