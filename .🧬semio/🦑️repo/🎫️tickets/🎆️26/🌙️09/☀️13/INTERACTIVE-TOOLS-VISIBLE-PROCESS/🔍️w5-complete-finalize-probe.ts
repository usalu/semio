/** 🩺️ (semio-91) Uninterrupted fill to Complete, then Finalize and Undo of a puzzle 3d fill in the React shell (:6013, Metal). For each action: start a long fill,
...
const lines: string[] = [];
page.on("console", (msg) => {
  if (msg.type() === "error" || /toolRun|tool run|reject|fault|stale|illegal|intake|busy/i.test(msg.text())) lines.push(`+${Date.now() - t0}ms ${msg.type()} ${(/intake duplicate/.test(msg.text()) || (/intake-rejected/.test(msg.text()) && !lines.some((l) => l.includes("intake-rejected")))) ? msg.text().slice(0, 6000) : msg.text().slice(0, 300)}`);
});
...
const report: string[] = [];
// 🏁️ semio-91: uninterrupted run to Complete, then Finalize, then Undo.
{
  report.push(`== Complete: before start ${JSON.stringify(await sample(page, Date.now()))}`);
  const started = Date.now();
  await page.getByRole("button", { name: "Start", exact: true }).first().click({ timeout: 30000 });
  let last = "";
  let complete = false;
  for (let index = 0; index < 480 && !complete; index += 1) {
    const now = await sample(page, started);
    const key = JSON.stringify({ ...now, t: 0 });
    if (key !== last) report.push(`running ${JSON.stringify(now)}`);
    last = key;
    complete = /^Complete/i.test(now.panel);
    if (!complete) await page.waitForTimeout(250);
  }
  report.push(complete ? `COMPLETE after ${Date.now() - started} ms` : "NEVER COMPLETED within 120 s");
  const clicked = Date.now();
  await page.locator('[id^="panel:framework.toolRun"] button', { hasText: "Finalize" }).first().click({ timeout: 5000 }).then(() => report.push("Finalize clicked"), (error) => report.push(`Finalize click failed: ${String(error).slice(0, 200)}`));
  last = "";
  for (let index = 0; index < 40; index += 1) {
    const now = await sample(page, clicked);
    const key = JSON.stringify({ ...now, t: 0 });
    if (key !== last) report.push(`after Finalize ${JSON.stringify(now)}`);
    last = key;
  ...
  report.push(`after undo ${JSON.stringify(await sample(page, clicked))}`);
}
report.push("== console", ...lines);
writeFileSync(`${out}/complete-finalize-${Date.now()}.txt`, report.join("\n"));
