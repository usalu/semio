#!/bin/zsh
# 🔒 Play fleet NATIVE cargo mutex: one cargo test/check of the play fleet at a time (shared build dir). usage: zsh 📜️native-test-mutex.sh <topic> -- <cmd …>
# usage: zsh 📜️wasm-build-mutex.sh <slice> -- <command …>
slice="$1"; shift; [ "$1" = "--" ] && shift
lock="/tmp/semio-play-native-test.lock"
