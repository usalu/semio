...
OUT="$T/🗑️generated/h-convergence-results.txt"
: > "$OUT"
while read -r crate; do
  ...
  for filter in converge backbone idempotent; do
    log="$T/🗑️generated/h-$crate-$filter.txt"
...
done < "$T/🗑️generated/h-convergence-crates.txt"
echo DONE >> "$OUT"
