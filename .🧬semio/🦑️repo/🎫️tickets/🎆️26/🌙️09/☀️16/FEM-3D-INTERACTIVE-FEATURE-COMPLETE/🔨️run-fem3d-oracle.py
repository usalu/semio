...
Run: `uv run python .🧬semio/…/FEM-3D-INTERACTIVE-FEATURE-COMPLETE/🔨️run-fem3d-oracle.py` (fixtures now live under
`📈️analysis/🧫️fixtures/<case>/`, the host module under `🧪️test/🖥️host/`, and fixture URIs are `shared://<case>/<file>`).
"""
...
def run_case(name):
    ...
    if failures:
        return False
    path = os.path.join(SUBSETS, "📈️analysis", "🧫️fixtures", name, "📊️expected.results.json")
