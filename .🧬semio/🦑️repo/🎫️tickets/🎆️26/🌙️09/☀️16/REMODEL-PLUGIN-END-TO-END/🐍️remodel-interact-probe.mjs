 ...
 * (shell beacon, Model window World3d host, panel toggles, Actions rows), (2) opens the Artifact/Inspection
 * panels, (3) runs the `addGcp` action from the Actions pane (document mutation) and checks the history
 * ledger + Artifact tree, (4) presses mod+z and checks the GCP is removed again (undo proves the mutation
 * landed in the document store), (5) flips the Model window's `mesh` layer toggle measure
 * (`setLayerVisibility`, window-config lane) and checks the measure re-publishes, (6) switches to the
...
const actionId = process.env.SEMIO_PROBE_ACTION ?? "addGcp";
...
// ── open the Artifact + Inspection panels ──
{
  const from = lines.length;
  const opened = [];
  for (const id of ["framework.panel.artifact", "framework.panel.inspection"]) {
...
{
  const from = lines.length;
  const engagement = before.engagements.find((id) => /remodeling-main|remodelingMain|main/i.test(id)) ?? before.engagements[0];
  let toggled = "absent";
  if (engagement) {
  ...
  let after = null;
  const applied = () => lines.slice(from).filter((l) => /history patch applied/.test(l)).length;
  for (let i = 0; i < settleSeconds * 2; i++) { await page.waitForTimeout(500); after = await state(); if (applied() > 0) break; }
  await page.waitForTimeout(1500);
  after = await state();
  await note("action-add-gcp", { engagement, toggled, rowId, clicked, submitted, applied: applied(), actionRows: opened.actionRows, gcpRows: after.gcpRows, treeRows: after.treeRows, hosts: after.hosts, bodyHead: after.bodyHead }, from);
  before = after;
}
...
// ── undo through the keybinding ──────────────────────────────────────────
{
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    await page.waitForTimeout(1500);
    after = await state();
    if (lines.slice(from).some((l) => /history patch applied.*"Undo"/.test(l))) break;
  }
  await page.waitForTimeout(1500);
  after = await state();
  const undone = lines.slice(from).filter((l) => /history patch applied.*"Undo"/.test(l)).length;
  await note("undo", { presses, undoPatches: undone, gcpRows: after.gcpRows, treeRows: after.treeRows, hosts: after.hosts, bodyHead: after.bodyHead }, from);
  before = after;
}
