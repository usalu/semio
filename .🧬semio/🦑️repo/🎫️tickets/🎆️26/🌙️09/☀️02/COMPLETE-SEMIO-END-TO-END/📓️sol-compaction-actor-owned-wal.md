...
This is an AJV/source oracle, not a Rust compile or native runtime receipt. The root-owned native attempt `tldqJV` reached the changed DB tree but failed compilation before any law; it is not a compaction verdict. The neutral fixture states queued-submit behavior, while the current native law proves pre/post command acceptance and cancellation restoration; it does not deterministically park a concurrent submit mid-compaction. The generic actor future still terminally closes the authority on a panic instead of restoring its engine, so no panic-restoration claim is made. Payload GC is not complete: only its unsafe document-local use has been disabled. No durable Map publication or retained Home/WGPU qualification is implied by this slice.
