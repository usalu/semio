...
# Current Qualified Writer Checkpoint — 2026-09-05

Session25178 is terminal. Its writer group `🗑️generated/exact-cargo-laws-Uv1AeE/00` passed all26 exact native assertions with executable SHA-256 `befad876424ba2de5861855280ad71f641d90f94defbd19ec6c5e41bf2969b8a`. This includes the captured SQLite child process law, all four filesystem directory durability laws, mounted controller laws and all five replication laws. The snapshot transfer law passed its36000-byte/three-page distinct-input aggregate check, explicit source/final lease retirement and follower writer reacquisition.

The same Nx run exited1 because its preceding compaction group `exact-cargo-laws-DOCr0L/00` passed2 then aborted in the live actor law. Its primary failure was encoded WAL backing capacity, not the earlier history factory failure; a nonterminal Store destructor then panicked during unwinding. The execution lane repaired exact backing admission and is replacing consuming Database/VCS shutdown with a retained bounded-step API. Those repairs have no native acceptance yet.

The writer26 selector list was captured before the new mounted deferred-wake law was registered. It therefore does not qualify that law or newly edited shutdown APIs. Root session59353 now runs the separate async deferred-wake native group on the same warm target. Typed WAL-open rejection and snapshot fault/cancel exact-owner transfer remain explicit nonclaims.

Session59353 is terminal GREEN3 in `exact-cargo-laws-Pj2Y8N/00`, executable SHA-256 `cd3a8448cf9099e37da12a70cc59ec032b6929607825aa55cc14163a162be552`. It qualifies fixed2048-slot capacity/generation/shutdown, native non-inline/draining dispatch, and cooperative later-pump shutdown behavior. It does not qualify the mounted DB writer wake law.

Typed WAL-open is now implemented at the source boundary, including same-permit acquired retry and self-acquiring retained release, with Engine/Authority/Database/Cluster/CLI propagation. Its registered source oracle is GREEN, and writer-native now contains30 laws (three new typed rejection laws). First Hub type compilation58634 exposed two remaining source mappings; those were repaired. Native writer30, shutdown3 and compaction4 remain pending. A newly audited Database concurrent first-mount race is assigned to the same execution lane and remains a separate runtime requirement.
