...
| `cargo check -p semio-framework-plugin-host --lib --message-format=short` | **exit 0**, 56 m 11 s. Zero errors; every warning is pre-existing (`unnecessary qualification`, deprecated `fetch_update`, unused `FromValue` import, unused `cancel_scheduled`, dead `terminal_is_empty`). This is the compile authority for all three exhaustive WIT conversion matches on the native side, including the `📥️imports/🦀️.rs` arm whose absence caused the incident above |
...
| `cargo test -p semio-framework-os-renderer-wgpu --lib inference_driver` | WGPU_PLACEHOLDER |
...
| Site | State |
|---|---|
| `🔌️plugin/⚛️reactor/🦀️.rs:2567` (kernel → WIT, guest side) | arm present (this lane) — **wasm-gated, so no native build compiles it**; see the wasm note below |
...
- A bounded rectangular `GisMapInferencePreviewV1` (`semio.hub.gis-map-inference-preview/v1`) in both
  ...
  **This lane then repaired one divergence that extension opened.** The peer's gate lives in the shared
  reducer, but the WGPU driver's own `turn()` had its own hand-written approve condition (offered +
  job id + hash, no preview), so the native port would have asked the hub to approve a proposal the
  browser port refuses. `GisMapInferenceDriverV1::turn` now applies the `Approve` intent through the
  shared reducer FIRST and emits the call only if that reducer actually entered `Approving`, which
  makes the native path structurally incapable of diverging from it again. A new assertion in
  `inference_driver_approves_only_an_offered_proposal_and_is_terminal_once` drives an offered page whose
  preview was stripped and requires the next turn to be an ordinary `Poll`, never an `Approve`.
...
2. **This lane's private target directories are large.**
...
### One conversion site no native build can reach

`⚛️reactor/🦀️.rs`'s `kernel_effect_to_wit` — the fourth `Effect` conversion — lives inside
`mod wit_bridge`, gated `#[cfg(all(any(feature = "component-guest", feature = "component-extension-guest"),
target_arch = "wasm32", target_env = "p2"))]`. **No native `cargo check` compiles it**, including the
`semio-framework-plugin-host` run above. Its arm and its `kernel_inference_proposal_kind_to_wit` helper
were verified only structurally: they are bare-fn siblings of the existing `kernel_placement_to_wit` in
the same module, using the same `wit_effects` alias (`crate::component::component::semio::framework::effects`),
and the WIT `enum inference-proposal-kind` / `record request-inference-proposal-effect` they name were
...
- **No React component was rendered in a test.** The packet asked for ShellHost unit tests; the os
  renderer package has no jsdom/React harness and this lane did not add one. The port's own laws — lease
  refusal, no state before receipt, non-optimistic cancellation, `stale`, approval-once, the closed
  failure vocabulary, no document mutation, and EN/DE totality — are asserted against the real
  host-owned port in `🧵️backbone-worker.ts` through a fake already-authenticated transport, which is the
