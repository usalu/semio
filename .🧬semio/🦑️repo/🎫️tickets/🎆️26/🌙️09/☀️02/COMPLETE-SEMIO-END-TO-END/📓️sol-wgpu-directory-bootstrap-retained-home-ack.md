...
- After the shared GIS inference page gained an optional floating-point preview, the WGPU test page constructor was updated to project the exact proposal hash into a deterministic preview ring when a proposal exists; the remaining WGPU status constructor uses `..Default`, and no WGPU equality-derived type embeds the status DTO. The current-tree registered source gate remains green at 39 checks after that parity edit. The cancelled native process predates this edit and therefore cannot qualify the new source snapshot.
