...
The worker physical-lifecycle law extends the strict neutral authority-fence row with a six-field close/reopen projection. A genuinely different accepted canonical `/me` authority closes and removes the original `ArtifactState`; reopening the identical Space/document creates a distinct state and client, and inference opens only after a fresh verified execution-target lease is installed on that successor. The law also proves no request was sealed before its terminal cleanup. The intentional RED reached Vitest and failed only at the absent neutral projection (`🗑️generated/worker-physical-reopen-red.log`). The sealed fixture/schema rerun passed one law with 335 skipped, exit 0 (`🗑️generated/worker-physical-reopen-green.log`). This is worker lifecycle evidence, not React Shell `WeakSet` or full trusted-browser journey evidence.

The current full OS rerun passed 336/336 laws across five files, exit 0 (`🗑️generated/worker-physical-reopen-full-os-recheck.log`). Its first graph attempt was correctly left unqualified after a moved Writer test imported two existing TypeScript modules without their ESM suffixes and read four fixture paths in place of the actual schemas (`🗑️generated/worker-physical-reopen-full-os.log`). The narrow current-path repair passed the registered Writer oracle against Ajv, TypeScript folds, and independent JSON Patch before the OS rerun (`🗑️generated/writer-window-state-oracle-current.log`).
