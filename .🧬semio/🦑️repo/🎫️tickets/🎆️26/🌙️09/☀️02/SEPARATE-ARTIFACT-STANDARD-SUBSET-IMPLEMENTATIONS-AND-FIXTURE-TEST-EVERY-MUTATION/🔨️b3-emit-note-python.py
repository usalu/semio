...
UNSTATED_BY_SUBSET = {"text": ["edit-block-text"]}

TEMPLATE = '''#!/usr/bin/env python3
"""🗒️ An INDEPENDENT second implementation of the `s.note.note` document and this subset's typed
mutations ({kind_list}), in Python, serving as this case's differential oracle. Relocated out of the
...
for subset, kinds in SUBSET_KINDS.items():
    ...
    unstated = ", ".join(f'"{k}"' for k in UNSTATED_BY_SUBSET.get(subset, []))
    out = TEMPLATE.format(subset=subset, kinds_tuple=kinds_tuple, kind_list=kind_list, unstated=unstated)
    case_dir = f"{ART}/✳️{subset}/🧪️tests/mutate-note-1-{subset}"
    with open(f"{case_dir}/🐍️.py", "w", encoding="utf-8") as f:
        f.write(out)
    print("wrote", f"{case_dir}/🐍️.py")
