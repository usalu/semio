...
subset's typed mutations ({kind_list}), in Python, serving as this case's differential oracle.
...
owns {kind_count}. A finite-element solver (`code_aster`, `OpenSees`, `anastruct`, `PyNite`) computes
...
for subset, kinds in SUBSET_KINDS.items():
    kinds_tuple = ", ".join(f'"{k}"' for k in kinds) + ("," if len(kinds) == 1 else "")
    kind_list = ", ".join(f"`{k}`" for k in kinds)
    out = TEMPLATE.format(subset=subset, kinds_tuple=kinds_tuple, kind_list=kind_list, kind_count=len(kinds))
    path = f"{ART}/✳️{subset}/🧪️tests/mutate-fem2d-1-{subset}/🐍️.py"
