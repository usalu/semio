...
arms = json.load(open("/tmp/_draw_arms.json", encoding="utf-8"))

ART = "✏️s/🔌️plugins/🖍️draw/🗿️artifacts/🖍️draw/🏅️standards/🔖️1/🪆️subsets"

SUBSET_KINDS = {
    "metadata": ["rename-layer", "set-layer-locked", "set-layer-visible"],
    "structure": ["create-layer", "delete-layer", "duplicate-layer", "reorder-layer"],
    "style": ["replace-layer-fill", "replace-layer-stroke", "set-layer-blend-mode", "set-layer-opacity"],
    "transform": ["set-layer-boolean-operation", "update-layer-trace-params", "update-layer-transform"],
}
CAPABILITY = {s: f"draw-1-{s}-mutate" for s in SUBSET_KINDS}
UNOBSERVABLE_BY_SUBSET = {"structure": ["duplicate-layer"]}
DECLARED_CODE_BY_SUBSET = {"structure": [("duplicate-layer", "mutation.target-missing")]}

RUST_TEMPLATE = '''//! 🖍️ Draw-document {subset} mutation case — Rust adapter. Relocated out of the artifact-level
//! `mutate-draw-1` case in ticket
...
//! `draw-mutation-semantics` (`../../../✳️any/🧪️oracle/🔣️.json`): `s.draw.draw` is a
...
//! implementation of draw semantics — and `subject` drives this repository's own vocabulary over
//! this subset's `DrawMutation` variants.
//!
...
fn fixture_text(kind: &str) -> (&'static str, &'static str, &'static str) {{
    match kind {{
{arms}        other => panic!("mutate-draw-1-{subset}: {{other:?}} is not a declared kind of this subset"),
    }}
}}

/// 🔣️ A committed fixture parsed through the platform's own JSON reader.
fn canonical(text: &str) -> Json {{
    parse_json(text).unwrap_or_else(|error| panic!("mutate-draw-1-{subset}: a committed fixture must be valid JSON: {{error}}"))
}}
{declared_code_fn}
//#endregion 🔖️Fixtures
...
mod subject {{
    ...
    use semio_s_plugin_stdio_test_oracle::law;
    use semio_s_plugin_draw::artifacts::draw::standards::v1::subsets::any::schema::mutations::{{apply_draw_mutation_json, undo_draw_mutation_json}};
...
for subset, kinds in SUBSET_KINDS.items():
    ...
    unobs = UNOBSERVABLE_BY_SUBSET.get(subset, [])
    unobs_list = ", ".join(f'"{k}"' for k in unobs)
    unobservable_block = (
    ...
    declared = DECLARED_CODE_BY_SUBSET.get(subset, [])
    if declared:
        declared_list = ", ".join(f'("{k}", "{c}")' for k, c in declared)
        declared_code_fn = f'''
/// 🚨️ The diagnostic code a declared no-op or refusal must raise, from the leaf's own committed
/// `🎯️outcome/🔣️.json`.
#[cfg(feature = "sut")]
const DECLARED_CODE: &[(&str, &str)] = &[{declared_list}];
...
'''
    ...
    out = RUST_TEMPLATE.format(subset=subset, kinds_list=kinds_list, unobservable_block=unobservable_block, arms=body, declared_code_fn=declared_code_fn)
    case_dir = f"{ART}/✳️{subset}/🧪️tests/mutate-draw-1-{subset}"
    os.makedirs(case_dir, exist_ok=True)
    with open(f"{case_dir}/🦀️.rs", "w", encoding="utf-8") as f:
        f.write(out)
    print("wrote", f"{case_dir}/🦀️.rs")
