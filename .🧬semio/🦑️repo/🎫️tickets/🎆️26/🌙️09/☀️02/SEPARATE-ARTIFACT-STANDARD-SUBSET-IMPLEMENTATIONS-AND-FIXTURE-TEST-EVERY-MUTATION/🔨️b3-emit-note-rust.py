...
SUBSET_KINDS = {
    "document": ["rename-note"],
    "canvas": ["change-grid-visible", "change-grid-spacing", "change-grid-subdivisions", "change-grid-opacity", "change-snap-enabled", "change-snap-grid-spacing"],
    "ink": ["change-pencil-width", "change-eraser-radius", "change-block-ink-width", "edit-block-ink-stroke"],
    "asset": ["create-asset", "replace-asset-payload", "delete-asset"],
    "block": ["create-block", "delete-block", "delete-blocks", "duplicate-block", "duplicate-blocks", "move-block-to-container", "drag-blocks", "rename-block", "change-block-visible", "change-block-locked", "move-block", "resize-block", "change-block-font-size"],
    "text": ["edit-block-text"],
    "math": ["edit-block-math"],
    "table": ["insert-table-row", "remove-table-row", "insert-table-column", "remove-table-column"],
}
assert sum(len(v) for v in SUBSET_KINDS.values()) == 33

TEMPLATE = '''//! 🦀️ Note document {subset} mutation case — Rust adapter. Relocated out of the artifact-level
...
for subset, kinds in SUBSET_KINDS.items():
    ...
    out = TEMPLATE.format(subset=subset, kinds_list=kinds_list)
