...
arms = json.load(open("/tmp/_fem2d_arms.json", encoding="utf-8"))
...
TEMPLATE = '''//! 🏗️ `s.fem.fem2d` {subset} mutation case — Rust SUBJECT adapter. Relocated out of the
...
fn vector(kind: &str) -> Vector {{
    match kind {{
{arms}        other => panic!("mutate-fem2d-1-{subset}: no committed specification vector is registered for kind {{other:?}}"),
    }}
}}
...
fn canonical(text: &str) -> Json {{
    parse_json(text).unwrap_or_else(|error| panic!("mutate-fem2d-1-{subset}: a committed fixture must be valid JSON: {{error}}"))
}}
//#endregion 🔖️Fixtures
...
for subset, kinds in SUBSET_KINDS.items():
    ...
    assert "🪆️subsets" not in body, (subset, body[:300])
    kinds_list = ", ".join(f'"{k}"' for k in kinds)
    out = TEMPLATE.format(subset=subset, kinds_list=kinds_list, arms=body)
    path = f"{ART}/✳️{subset}/🧪️tests/mutate-fem2d-1-{subset}/🦀️.rs"
