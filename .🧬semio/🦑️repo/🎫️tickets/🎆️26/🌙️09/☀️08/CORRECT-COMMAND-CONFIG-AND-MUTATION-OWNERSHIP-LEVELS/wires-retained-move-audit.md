...
## Captured Gesture RED Checkpoint

`wires-pointer-move-red-2.log` reached the registered runtime after explicit bounded retirement of its cold seed envelope. It failed as intended at the first pointer move: `interactive-job.missing-factory`, because canvasPointerMove has no exact controller/owner/factory/tool/schema proof. The app closed without the previous envelope-owner abort. This establishes a clean regression before migration; the five-step gesture is not yet green.

The independent shared-cursor audit now requires caller grants for clone and duplicate-free admission before canonical encoding. These fixes are being implemented at their shared owners before Wires adopts them. Store also already supplies `empty_document_spr`; reset helpers can reuse it rather than manufacture a document envelope only to print fresh history.

## Captured Move and App Config Source Migration

The thirteen-file Wires app config subtree and its synchronous config preparation were removed. The editor and all nine command signatures now use the framework NoConfig/NoConfigMutation, with framework ownership/disposal factories. Pointer movement now captures WiresCanvasTransientOwner, scans camera and node fields one at a time, rejects non-finite output, ignores a different canvas with no active drag, and resets a missing target only in the captured window. Its retained route/proof/publication contract declares Artifact and WindowTransient.

This is a source checkpoint, not a completed runtime migration: the bounded Wires document preparation and content-hash assembly are still missing. The five-step registered-app test must pass before this gesture is considered complete. LoadDocument currently only resets the document Store and render cache in the shared SDK; it does not reset attached window transient owners. That lifecycle must be addressed and validated for replacement/deletion. The generated Reasoning manifest also still has inherited config read semantics for pointer-down; its resource-addressing contract needs correction after inspecting the canonical manifest schema. No success is claimed from the migrated route label alone.

The focused ownership policy gate is GREEN after app config removal: `ownership-wires-no-config-1.log` reports 11 direct ownership vectors, four nested schema vectors, three command-source vectors agreeing with Ajv, and 104 artifact contracts checked across five schema formats. Nx completed in 2.6 seconds. This gate validates declaration ownership and document-only artifact fields; it does not establish Wires document preparation, gesture runtime, history grouping, or replacement lifecycle correctness.

## Gesture History Decision

The current Store `amend_command` appends to a tail edit by performing whole mutation replay; it is not the bounded publication seam. The existing `group_id` and `SpaceCoordinator::undo_group` coordinate one tail edit per member, not a contiguous sequence of moves within one document. Merely stamping the same group or coalesce key on fresh retained edits therefore cannot implement one undo for a drag.

The selected Wires lifecycle is an exact-window transient preview while dragging and one durable MoveNode at release. The concrete Canvas renderer must apply that window's preview delta, so live visual movement remains available without repeatedly cloning or hashing the whole document. Release resolves the current target through the captured operation input, applies total screen delta with the frozen gesture zoom, and publishes the document change with transient clear. Cancellation and missing target clear the gesture without history. This partitions preview and persisted edits by their actual lifetime and gives one undoable edit per completed drag.

The Wires agent is implementing the overlay/lifecycle and replacing the earlier per-move-durability fixture with separate preview/durable-coordinate expectations. The previous five-step runtime fixture is a baseline, not the final history contract. Native confirmation, including one-step undo and cancellation with no history, remains pending. Wires camera ownership also remains under producer tracing because its existing root documentation identifies it as pan/zoom view state while its schema persists it in the document.
