...
| `…/🧪️test/🧬️schema/🔣️.json` | **Row 44.** `$id` `https://semio-tech.com/schema/repo/test/v2` → `https://json.schemas.assets.semio-tech.com/repo/test/schema.json`. Plus `$defs.TestLayoutCases` + its root `oneOf` branch (73 → 74 defs), taking over the contract a new case had defined for itself — §6.1. |
...
Applied here: `🧪️test/🧬️schema/🔣️.json` → `"$id": "https://json.schemas.assets.semio-tech.com/repo/test/schema.json"`
...
| **R-5** | W2c (`📚️library/🧪️tests/🗺️testing-readme-coordinates/🟦️.ts:127`) | Change exactly this line, in this wave: `expect(JSON.parse(schemaBytes.toString("utf8")).$id).toBe("https://semio-tech.com/schema/repo/test/v2");` → `…toBe("https://json.schemas.assets.semio-tech.com/repo/test/schema.json");`. The `$id` is already migrated on my side. |
