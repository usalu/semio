...
- Exact Rust/Cargo/source-router paths are recorded in `📓️artifact-execution-file-ledger.md` (650 paths), including all 53 artifact Cargo manifests and taxonomy roots (36 stdio plus 17 multi-artifact), initial GIS2 roots, five taxonomy parent roots, the two shared-core roots, Puzzle's checked-in icon table, parent/package removals, legacy test import fixes, workspace/profile wiring, the final stdio taxonomy-root move, and the three feature-gated Semio import repairs. A second ownership audit removed 55 locale/presence/mutation/obsolete-wasm deletions and three standalone-test creations belonging to concurrent work; the six parent/package wrapper and Puzzle build-script removals performed by this extraction are recorded explicitly.
...
- PASS (CURRENT FEATURE CLOSURES): the locked `semio-s-plugin-stdio` tree with only `full-artifact-catalog` contains all 36 artifact packages. The locked `home-io` tree contains exactly Binary, CSV, Deflate, JSON, TXT, XLSX, XML, and ZIP plus the shared contract; it does not select the full catalog (`🗑️generated/stdio-full-tree-current.txt`, `🗑️generated/stdio-home-tree-current.txt`).
...
| stdio Semio | default normal dependency closure | Current pass | Current locked normal-edge tree contains Semio plus contract and zero of the other 35 format crates |
| stdio Semio | default library and tests | Earlier library pass; current tests pending | Current-source native verification queued |
...
| stdio catalog | `full-artifact-catalog` | Current tree and prior compile pass; runtime laws pending | Current locked tree selects all 36; coordinator's GIS integration compiled full stdio and Semio after extraction |
| stdio catalog | `home-io` | Current tree and source contracts pass; native gate pending | Locked tree selects the exact eight-package closure plus contract; source laws pass at checks 11, 23, 27, and 54 |
