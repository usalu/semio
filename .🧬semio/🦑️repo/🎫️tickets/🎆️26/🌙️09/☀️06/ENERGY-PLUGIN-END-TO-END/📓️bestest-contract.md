...
## Amendments (coordinator, 2026-09-06 06:30)
- `Fenestration.u_value_w_m2k` / `shgc` MEAN the whole-assembly simple-glazing inputs (EnergyPlus `WindowMaterial:SimpleGlazingSystem` semantics). BESTEST fixtures use the ASHRAE 140 spec values 3.0 W/m²K / 0.787. The engine's own window model and the honeybee translation both consume them that way, so the two producers stay comparable; the residual gap to a layer-by-layer EnergyPlus window (2.740 / 0.760) is a known, documented ~2.5-point cooling offset, not a bug.
- Cooling tolerance widened to 10 % relative against the translated EnergyPlus run (heating stays at 20 %; hourly RMSE ≤ 2.0 K unchanged), because the simple-glazing route itself sits +5.7…+8.1 % above the layered reference.
- Long-term (envelope mutation group): add an optional layered glazing construction (`glazing_construction_id`) to `Fenestration` so a case can be modelled layer-by-layer; when present, both producers use the layers.
- Implemented reference cases: 600, 600FF, 610, 620, 640, 900, 900FF, 910, 920, 940. Not implemented and not faked: 630/930 (fins), 650/950 (night ventilation), 960 (sunspace).
