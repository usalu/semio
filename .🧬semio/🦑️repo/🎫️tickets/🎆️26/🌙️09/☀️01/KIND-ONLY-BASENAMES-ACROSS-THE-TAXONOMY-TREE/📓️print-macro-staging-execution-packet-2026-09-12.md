...
# Coordinator Source Review During Execution

At 20:01 UTC root reproduced a source-kind enforcement gap in the in-progress scoped rule. The canonical graph leaf classified `scoped:print-graph-macro-source`, but same-owner `📐️reconcile.tex`, `reconcile.tex` and `_.tex` all returned no scoped source kind, physical documentation kind tex and no kind-only-basename finding. Raw evidence is `🗑️generated/coordinator/print-macro-bad-basename-probe.json`. The exact path pattern and sourceFilenamePattern both constrained role recognition to the already-correct filename. Sol must add portable hostile-name controls and separate semantic macro-source role recognition from canonical filename enforcement, keeping ordinary TeX elsewhere documentation. Widening only pathPattern is insufficient while role selection still filters the canonical sourceFilenamePattern. This is a current in-progress blocker, not acceptance of the initial scoped rule or a claim about all normalization diagnostics.

The worker repaired this role/name coupling with an exact graph-owner TeX scope. Root independently reran the four-name probe and a neighboring document control: canonical source remains accepted, all three wrong names now require 📐️.tex, and neighboring TeX remains outside the source scope. Root also reran the updated portable staging fixture, including preservation of bare 📐️ prose; both checks exited zero with explicit DEBUG pass messages. Repaired evidence is retained in `print-macro-bad-basename-repaired.json` and `print-macro-portable-repaired.txt` under coordinator generated output. This closes that specific filename-gate defect; normalization, shim and registered acceptance still belong to the final lane report.

Root independently invoked the existing `verifyPrintMacroStaging()` with SEMIO_PRINT_OUTPUT_DIR under this ticket's coordinator generated root. It completed with exit zero and emitted `[DEBUG] Print macro staging: schema, references and collision atomicity PASS`. This covers the portable JSON/AJV reference and three pre-mutation collision cases only; it does not duplicate the native PDF test or establish dark/panel rendering.

At 19:55 UTC root reread the changed production source-staging, catalog and compilation owners. `publishPrintArtifact` now creates a private library root for each invocation and stages the catalog's explicit librarySources from the print product root, independently of the caller's document sourceRoot. The same libraryRoot is passed through light, dark and panel recompilation. Both compiler search paths and TEXINPUTS use the staged latex directory plus staged library root; font search remains separately owned. The canonical graph shim contains only native format/package declarations and its semantic input reference, and the stager maps exact repository `📐️.tex` to compiler-only `_.tex` while rewriting .tex/.bib/.sty/.cls inputs. This is a current code review, not a new independent native execution or completed source-kind acceptance. The worker's final exact tests/report are still required.
