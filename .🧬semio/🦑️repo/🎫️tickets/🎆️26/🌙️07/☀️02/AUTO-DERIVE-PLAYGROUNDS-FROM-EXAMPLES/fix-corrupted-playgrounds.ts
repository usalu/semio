/** @emoji 🔧️ Repairs merge-script corruption in playground core index files. */
...
function movePlayRegionBeforeController(source: string, controllerMarker: string): string {
  const playStart = source.indexOf("//#region 🔖️Play");
  if (playStart < 0) return source;
  const playEnd = source.indexOf("//#endregion 🔖️Play", playStart);
  if (playEnd < 0) return source;
  const playBlock = source.slice(playStart, playEnd + "//#endregion 🔖️Play".length);
  ...
  const examplesBlock = playBlock
    ...
    .replace(/^\/\/#region 🔖️Play\n/, "//#region 🔖️PlayExamples\n")
    .replace(/\/\/#endregion 🔖️Play$/, "//#endregion 🔖️PlayExamples");
  const appDefOnly = `//#region 🔖️Play\n${appDefMatch[0]}//#endregion 🔖️Play\n`;
  const withoutPlay = source.slice(0, playStart) + source.slice(playEnd + "//#endregion 🔖️Play".length);
