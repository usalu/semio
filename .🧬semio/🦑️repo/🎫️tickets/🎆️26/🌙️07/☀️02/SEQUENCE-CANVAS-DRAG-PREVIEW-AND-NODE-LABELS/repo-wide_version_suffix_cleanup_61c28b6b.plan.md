...
- **Icon `image:` prefix stripping**: delete `stripLegacyImageDataPrefixForIcon` (`ui/react/index.tsx:956-973`) and `strip_legacy_image_data_prefix` (`infinite/canvas/rs/lib.rs:1130-1140,1283,1325`); no fixture uses the prefix. Update the one test asserting the old prefix (`puzzle/2d/react/index.tsx:10245`).
...
- `flow/react/index.tsx` + `flow/core/lib.rs` + `flow/module/wasm/lib.rs`: ~20 `Flow*V1` types (`FlowFixture`, `FlowDocument`, `FlowGui`/`FlowUi`, `FlowExtensionManifest`, etc.) and drag helpers `encodeFlowWidgetDescriptorForDrag`/`decodeFlowWidgetDescriptorFromDrag`, `FLOW_WIDGET_DRAG_MIME`.
