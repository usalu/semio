...
const MIXED_BUNDLES = new Set(["flow/core", "coda/client/bin/assistant", "compose/client/bin/engine", "repo/server/coordinator"]);
...
function isBundleDir(dir: string): boolean {
  ...
  return names.includes("package.json") || names.includes("Cargo.toml") || names.includes("pyproject.toml") || names.includes("go.mod") || names.some((n) => n.endsWith(".csproj"));
}
...
function detectLanguages(bundlePath: string): Lang[] {
  ...
  if (MIXED_BUNDLES.has(bundleRel))
    return ["js", "rs", "py", "go", "cs"].filter((l) => {
