...
2. `cargo test -p vcs -p protocol_core -p protocol_format -p protocol_history -p
   ...
   green, including the law-based tests (encode/decode identity, canonical stability, ops⇄️binary
...
3. `cargo clippy` on every touched crate, clean (or only pre-existing unrelated warnings — note
   which).
4. `bun ./📜️script.ts lint` — protocol-completeness allowlist empty and firing correctly, no
