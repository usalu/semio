...
/** 🧩️ A palette entry for a block kind insertable into a {@link BlockListScene}, contributed either by the host app's own built-ins or by a `protocolBlockKind` module contribution. */
...
/** 🧩️ A strict, ordered list of steps/blocks for the Blockly-like list editor. `stepsJson` is a `ProtocolStep[]` array, `paletteJson` is a `BlockPaletteEntry[]` array of the block kinds available to insert. */
...
/** 🧭️ The dispatch key on {@link UiComponentSceneNode} — matches the lazy-loaded host component per `framework/renderer/react/components/*-host.tsx`. */
...
/** 🧷️ Shared prop shape for every `framework/renderer/react/components/*-host.tsx` component. */
...
/** 🐳️ The full persisted dock arrangement, one tab tree per anchor — anchor ids mirror `PanelAnchor` in `ui/js/react/index.tsx` (kept inline here to stay dependency-free of that package). */
...
/** 🌱️ The full persisted dock UI state: per-anchor visibility/size/active-path, per-branch drill-down memory, and tree section/group expansion. Anchor ids mirror `PanelAnchor` (kept inline here to stay dependency-free of the `ui` package, same convention as {@link DockSkeleton}). */
...
export type PluginViewState = {
  ...
  /** 🧰️ Host-owned active utility for the active window kind (never a document field, never a VCS op). */
...
/** @emoji 🐢️ One batched, hash-conditional refresh request — one round trip for the window/panel/engagements/measures/labels sections. Toolbars are no longer a plugin section: the renderer derives them from the utility registry via {@link deriveUtilityNodes}. */
...
/** 🧰️ A resolved utility ready for the toolbar — the TS twin of Rust `DerivedUtilitySpec` in `ui_wgpu`. */
...
/**
 * 🧰️ Hand-written twin of Rust `derive_utility_nodes` (`ui/wgpu/rs/lib.rs`): builds the toolbar node tree
...
/**
 * 🎯️ Hand-written twin of Rust `partition_window_measures` (`ui/wgpu/rs/lib.rs`): splits a window's
...
/** @emoji 🐢️ Normalizes a possibly-absent `UiDirtyScope` — missing (older plugin, or a response built without one) means `full`. */
...
/** @emoji 📥️ Parses a raw plugin `handle-action`/`handle-command` response string into a typed {@link InvocationResponse}. */
...
/** @emoji 🔒️ Serializes wasm plugin entry points — the host keeps instances in one RefCell. */
...
/** @emoji 🧵️ Message types the generated `plugin-worker.js` dispatches (framework/product/os/dev/script.ts `pluginWorkerSource`). */
