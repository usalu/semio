...
Added 5 narrow one-line-delegation wrapper functions mirroring the existing `text_editor_pointer_down`/`_move`/`_up` pattern exactly, each exposing an already-existing `EditorHost` capability past the module boundary (in region `🔖️ScenesInteropAdditions`):
