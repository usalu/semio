...
Scope: `framework/product/os/dev/script.ts`'s `#region 🔬️ParityScript` only. Added a new
`#region 🔖️ProbeCatalog` sub-region (between `🔖️Triage` and `🔖️ServerPool`), wired it into
...
`framework/product/os/dev/script.ts` — `#region 🔬️ParityScript` only (`ParityPlaygroundReport` type,
new `🔖️ProbeCatalog` sub-region, `writeParityReport`, `verifyParityVariant`, `ParityProbeScript`,
