...
```bash
cd framework/product/os/dev && bun ./📜️script.ts plugin forms forms-module-procedural flow procedural
```
...
```bash
node .repo/🎫️/26/07/08/FORMS-BLUEPRINT-TRY-MODULE-PREVIEW/plugin-runtime-check.mjs
```
...
`bun ./📜️script.ts dev forms` → `runViteBunxDev` → `bun x vite` runs Vite under Node; `vite.config.ts` imports `script.ts` which uses `bun:sqlite` → `ERR_UNSUPPORTED_ESM_URL_SCHEME: Received protocol 'bun:'`.
