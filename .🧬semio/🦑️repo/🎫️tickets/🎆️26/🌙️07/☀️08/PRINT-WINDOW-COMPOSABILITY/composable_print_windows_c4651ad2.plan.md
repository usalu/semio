...
Check `repo://tickets` again for freshness, then open a new ticket (no existing one covers window composability) under goal `🎯️r2602🎯️updateddocs` (same bucket as the sibling doc tickets), e.g. slug `PRINT-WINDOW-COMPOSABILITY`.
...
- `cd print && bun ./📜️script.ts test` to build all template PDFs (light+dark).
