...
function getPluginId(command: string): string {
  ...
  if (command.startsWith("bun ./📜️script.ts dev ")) {
    const rest = command.replace("bun ./📜️script.ts dev ", "").split(" ");
...
for (const config of configs) {
  if (config.name.endsWith("🧊️wgpu")) {
    const baseName = config.name.replace("🧊️wgpu", "");
    ...
    wasmObj.name = baseName + "🧊️wgpu🌐️wasm";
    ...
    nativeObj.name = baseName + "🧊️wgpu🖥️native";
