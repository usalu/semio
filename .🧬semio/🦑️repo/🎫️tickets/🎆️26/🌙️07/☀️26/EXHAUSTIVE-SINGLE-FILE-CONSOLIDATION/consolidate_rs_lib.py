...
def consolidate(crate_dir: Path) -> list[str]:
    ...
    if "// #region 🔖️Modules" in text:
        text = text.replace("// #region 🔖️Modules\n", "", 1)
        text = text.replace("// #endregion 🔖️Modules\n", "", 1)

    if "// #region 🔖️NativeModules" in text:
        text = text.replace("// #region 🔖️NativeModules\n", "", 1)
        text = text.replace("// #endregion 🔖️NativeModules\n", "", 1)
