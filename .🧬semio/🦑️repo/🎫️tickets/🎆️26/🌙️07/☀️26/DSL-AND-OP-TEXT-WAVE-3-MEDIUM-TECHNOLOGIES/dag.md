...
## Files touched
- `infinite/board/port/directed/dag/rs/lib.rs`: added `//#region 🔖️Dsl` (DocumentDsl for DagDocument)
  and `//#region 🔖️OpText` (OpText for DagOperation), both inside the existing
  `// #region 🔖️DocumentVcs` region, right before `//#region 🔖️WasmBridge`. Extended
  `#[cfg(test)] mod dag_vcs_tests` (inside the same DocumentVcs region) with a `//#region 🔖️DslTests`
