...
### `.writer` DSL (`WriterProjection`, `writer/rs/lib.rs`, region `🔖️Dsl` inside `mod document_vcs`)
...
### OpText (`WriterOperation`, region `🔖️OpText`)
...
- New `🔖️Examples` region: `jack_example_document()`/`jack_example_json()` and
