...
- `bun ./📜️script.ts test --run` in `framework/renderer/react` — full suite passing (61-62 tests depending on concurrent unrelated raster-host work landing in the same file; the "note canvas host" describe block's 8 tests pass 100% of the time, isolated via `-t "note canvas host"`).
