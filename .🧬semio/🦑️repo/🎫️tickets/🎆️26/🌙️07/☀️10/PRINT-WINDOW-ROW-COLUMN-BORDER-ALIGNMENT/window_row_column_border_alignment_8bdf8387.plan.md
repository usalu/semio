...
- Open a new ticket (goal `🎯️r2602🎯️updateddocs`, same bucket as the prior `PRINT-WINDOW-COMPOSABILITY`/`PRINT-WINDOW-BORDER-GAP` tickets which are both closed and don't cover this).
...
- new ticket folder under `.repo/🎫️/26/07/10/...` (bisect/verify temp files, screenshots, ticket.json)
