...
// puzzle 2d: strip Inner + boot tail, add AppRendererContribution export
{
  ...
  content = content.replace(/\/\*\* @emoji 🛝️ Playground play host for Puzzle2d/, "/** @emoji 🛝️ Puzzle 2D app renderer contribution");
  ...
  content = content.replace(/\nlet puzzle2dPlayChromeRegistered = false;\n\n\/\*\* @emoji 🧊️ Registers puzzle 2d play surface host[\s\S]*?registerTabIcon\("puzzle\.2d-play\.icon\.settings", "settings"\);\n\}/, "");
  const innerStart = content.indexOf("\nfunction Puzzle2dPlayInner(");
  const entryEnd = content.indexOf("// #endregion 🔖️Entrypoint");
...
/** @emoji 🛝️ Puzzle 2D app renderer for playground and OS shells. */
...
/** @emoji 🔗️ WIRES play renderer — same surfaces and panels as puzzle 2D play. */
