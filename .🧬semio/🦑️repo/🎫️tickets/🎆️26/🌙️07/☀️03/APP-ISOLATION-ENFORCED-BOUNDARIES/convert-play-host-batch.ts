...
function fixImports(content: string, label: string): string {
  let c = content;
  c = c.replace(/\/\*\* @emoji 🛝️ Playground play host for ([^—]+) — loaded only via `\.\/play` subpath\. \*\//, `/** @emoji 🛝️ ${label} app renderer contribution — loaded only via \`./play\` subpath. */`);
...
/** @emoji 🛝️ Trinity Jack app renderer for playground and OS shells. */
...
/** @emoji 🛝️ Trinity Rewrite app renderer for playground and OS shells. */
