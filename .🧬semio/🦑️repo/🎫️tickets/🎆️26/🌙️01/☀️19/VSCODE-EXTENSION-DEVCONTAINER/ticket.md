...
- Added TicketOpen input fields (noIssue, planPath) to repo GraphQL schema and resolver in ./repo/cli/main.go to align with CLI payload.
- Updated graphql/repo/schema.graphql TicketOpenInput to match repo schema fields.
- Rebuilt ./repo/cli/cli binary to apply schema changes.
- Opened ticket in repo root and removed mislocated ./repo/cli/tickets folder via repo folder delete.
