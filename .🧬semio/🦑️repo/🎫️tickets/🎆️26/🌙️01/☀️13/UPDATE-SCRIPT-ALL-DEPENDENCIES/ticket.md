...
```
npm run update
  └️─️> npx tsx update.ts
        ├️─️> Load update.config.json
        ├️─️> npm update -S (root)
        ├️─️> Restore "*" versions in workspaces
        ├️─️> cd py/engine && uv lock --upgrade
        ├️─️> cd rs/compose && cargo update
        ├️─️> For each go.mod: go get -u && go mod tidy
        └️─️> For each .csproj: dotnet outdated --upgrade (skip excluded)
```
