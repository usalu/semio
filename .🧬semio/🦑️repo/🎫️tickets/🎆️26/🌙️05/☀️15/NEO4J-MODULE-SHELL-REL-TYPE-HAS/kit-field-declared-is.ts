#!/usr/bin/env bun
/**
 * @emoji 🧭️ Rebuilds declared `IS` from `compose/client/schema/compose/schema.yaml`: kit members get one `IS` per field; `Class`/`Interface` types keep **only** direct `implements` targets (no transitive `IS` to supertypes like Entity).
 */
...
//#region 🧭️Types
/** @emoji 📌️ One kit field's owner (YAML map key, any casing), member name, and FieldIs target node name in Neo4j. */
...
/** @emoji 📐️ One `Class`/`Interface` type’s YAML map key and direct `implements` targets (Neo4j node names). */
...
/** @emoji 🎯️ True when this map is a leaf field spec (has `is`); sibling keys like `cached` or `constraints` do not affect declared-`IS` repair. */
...
//#region 🧭️Cypher
/**
 * @emoji 🔗️ Kit members (`Data` / `Derived` / `Reference`) keep **only** the single declared `IS` from YAML (concrete `Class`, `Interface`, or `Scalar`). `Class`/`Interface`/`Command` types keep **only** direct `implements` edges (see {@link buildTypeDeclaredIsRepairCypher}).
 */
...
/**
 * @emoji 🧷️ Drops every `IS` from a type that is not listed under YAML `implements`, then `MERGE`s each declared edge (direct supertypes only).
 */
