// 🦋️ Removes every `union` and rewrites union-typed fields into general-interface fields whose
...
// ─️ 1) Collect all unions: name → [memberTypes] ─️
...
// ─️ 2) Find every line owned by a `union ... = …` declaration so we can drop them. ─️
...
// ─️ 3) Track the "current concrete operation type name" so we can narrow `scope`/`input`. ─️
