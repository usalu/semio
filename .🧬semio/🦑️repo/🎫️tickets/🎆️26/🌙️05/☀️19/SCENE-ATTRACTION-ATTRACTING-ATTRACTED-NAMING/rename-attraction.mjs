...
const pairs = [
  ...
  ["//#region 🔗️LinkGesture", "//#region 🧲️AttractionGesture"],
  ["//#endregion 🔗️LinkGesture", "//#endregion 🧲️AttractionGesture"],
...
s = s.replace(/\{ source: session\.attractingFullId, target: snapId \}/g, "{ attracting: session.attractingFullId, attracted: snapId }");
s = s.replace(/return \{ source: `\$\{movingObjectId\}:link`, target: best\.id \}/g, "return { attracting: `${movingObjectId}:link`, attracted: best.id }");
...
const fnEnd = s.indexOf("/** @emoji 🤝️ WASM-style filtered attraction compatibility");
const fnBlock = s.slice(fnStart, fnEnd);
const fnFixed = fnBlock.replaceAll("source.vortexKind", "attracting.vortexKind").replaceAll("target.vortexKind", "attracted.vortexKind").replaceAll("source.objectKind", "attracting.objectKind").replaceAll("target.objectKind", "attracted.objectKind");
...
const dragEnd = s.indexOf("//#endregion 🧲️Helpers", dragStart);
const dragBlock = s.slice(dragStart, dragEnd);
const dragFixed = dragBlock.replace("sceneAttractionGestureRuleApplies(r, source, target, catalogs)", "sceneAttractionGestureRuleApplies(r, attracting, attracted, catalogs)");
...
s = s.replace("const targetCtx: SceneAttractionHandleContext", "const attractedCtx: SceneAttractionHandleContext");
s = s.replace(
...
s = s.replace(/\{ source: awaitPick\.attractingFullId, target: vf \}/g, "{ attracting: awaitPick.attractingFullId, attracted: vf }");

s = s.replace("function sceneAttractionSnapCommitProximityOk(\n\ttargetFullId: string,", "function sceneAttractionSnapCommitProximityOk(\n\tattractedFullId: string,");
