// #region ­ƒº▓️Header
/** ­ƒº▒️ `@elements/ui-shell` ÔÇö Framework-free workbench graph, declarative {@link UiNode} bodies, {@link ShellExtensionHost} (VS CodeÔÇôstyle `contributes` + `activate`), and {@link Workbench} ÔåÆ {@link WorkbenchApp} ÔåÆ {@link WorkbenchMode}. */
...
export type UiNode = UiStackNode | UiTextNode | UiButtonNode | UiSeparatorNode | UiScene3DHostSurfaceNode | UiBoardHostSurfaceNode | UiTableHostSurfaceNode;
...
function assertCanvasOnlyWindowBody(bodyKey: string, node: UiNode): void {
  ...
  throw new Error(`Declarative window body "${bodyKey}" must be a single table, scene3d, or board surface (optional none padding stack wrapper). Found "${node.type}". Use WorkbenchMode.tools, side tabs, or window measures for chrome.`);
}
//#endregion ­ƒöûUiNode
...
/** @emoji ­ƒº▒️ Default row/column of one stack per window kind (Golden-style ownership). */
...
//#region ­ƒöûToolbar
/** @emoji ­ƒº░️ Toolbar category ids shared by every app registration surface. */
...
/** @emoji ­ƒº╣️ Removes a declarative window registration (tests / hot reload). */
...
/** @emoji ­ƒº╣️ Removes a declarative side-panel registration (tests). */
...
/** @emoji ­ƒº░️ Activation context: workbench, manifest, and registration helpers (VS Code `ExtensionContext` analogue). */
