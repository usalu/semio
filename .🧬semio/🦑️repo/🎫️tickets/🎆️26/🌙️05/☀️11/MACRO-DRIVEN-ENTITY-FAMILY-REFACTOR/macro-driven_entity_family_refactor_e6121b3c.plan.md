...
todos:
 - id: bootstrap
   content: "Coordinator: read `repo://goals`, open ticket via repo MCP `ticket_open`, snapshot baseline `cargo check`, inject `//#region 🧬️ entity_dsl` and `//#region 🤖️ W1`..`//#region 🤖️ W8` markers into [compose/rs/lib.rs](compose/rs/lib.rs)."
   status: completed
 - id: w0-foundation
...
Everything below lives in a new `//#region 🧬️ entity_dsl` section of [compose/rs/lib.rs](compose/rs/lib.rs).
...
pub mod sdl_registry {
    ...
    /// 📜️ Concatenate all entity + operation + interface fragments + the
...
```rust
/// 🧬️ Emit the full 12-type ladder for one entity:
///   X, XEdge, XConnection,
    ...
    (
        ...
        paste::paste! {
            ...
            impl $name {
                ...
                /// 🪪️ Merkle leaf: type tag + id + each data field + sorted child digests.
            ...
            // ─️ 🪢️ Relay Edge / Connection ─️
            $crate::__entity_relay!($name);

            // ─️ 📐️ Diff / DiffEdge / DiffConnection ─️
            $crate::__entity_diff!($name, $tag, { $( $fname : $fty @ $fclass ),* });

            // ─️ ✏️ Modification / Edge / Connection ─️
            $crate::__entity_modification!($name, $tag);

            // ─️ 📚️ Modifications / Edge / Connection ─️
...
```rust
/// 🎬️ Emit XInput (optional), X (Operation), XEdge, XConnection,
...
macro_rules! operation_family {
    (
        ...
        paste::paste! {
            ...
            impl $name {
                /// 🧮️ Default no-op apply; concrete operations override via `kit_op_apply!`.
...
macro_rules! kit_operation_enum {
    ( $( $variant:ident { scope: $scope_arm:ident, input: $input_arm:ident } ),* $(,)? ) => {
        ...
        /// 🔗️ Async-graphql Interface enum exposing each concrete operation as a node.
...
```rust
/// 🧾️ Generate `XInput` (async-graphql InputObject) and `into_X` / `into_X_with_id`
...
```rust
/// 🪢️ Relay shells whose `node` is a Union (Blueprint, OperationIface, OwnedEntity).
...
```rust
/// 📜️ Canonical SDL = static header + interface fragments + entity fragments
...
/// 🔎️ Pull `type Query`, `type Mutation`, `type Subscription`, `schema { ... }` only
...
```rust
/// 📜️ Generated SDL must round-trip against the on-disk golden.
#[test]
...
1. Carve out `//#region 🧬️ entity_dsl` in [compose/rs/lib.rs](compose/rs/lib.rs) with all macros from blueprints sections 1-14.
...
| ---------- | ------------- | --------------------------------------------------------------------------- | ---- | --------------------- | ----------- |
| W0         | Foundation    | new `//#region 🧬️ entity_dsl` (top of file, after `pub mod hash`)           | 1    | bootstrap             | 60 min      |
...
- Foundation worker (W0) is the sole exception: it inserts the `//#region 🧬️ entity_dsl` block above all other regions, plus rewrites `gql::sdl()` in `pub mod gql`. W0 finishes before any wave-2 worker starts.
- Each worker prepends a `//#region 🤖️ W<id> in progress` and appends `//#endregion 🤖️ W<id> in progress` marker around any in-flight scratch code; the integrator removes them at integration time.
...
Hard rules:
  ...
  - Use sub-regions (`//#region 🔖️<sub>`) for grouping inside your region.
...
1. **Bootstrap (coordinator, serial).**
   ...
   - Inject the empty `//#region 🧬️ entity_dsl` and `//#region 🤖️ W1`..`//#region 🤖️ W8` markers into [compose/rs/lib.rs](compose/rs/lib.rs) so subagents have unambiguous landing zones.
...
- **Disjoint regions**: each worker writes to one contiguous byte range delimited by `//#region 🤖️ W<id>` markers. Re-greps after each wave detect any out-of-region writes.
...
2. Open a ticket via `ticket_open` titled "Macro-Driven Entity Family Refactor", associated with the closest existing goal (likely the GraphQL/control-plane goal). Ticket folder will be `.repo/🎫️/26/05/11/macro-driven-entity-family-refactor` — all temp scripts/logs land there.
...
Macro definitions added: ~1,200 lines under `//#region 🧬️ entity_dsl` (one-time cost).
