...
- Tests in the `🧪️ tests` region MUST keep passing (`bun scripts/export-schema.ts` is still the gate). When a name changes (`Transaction` -> `Edit`, etc.) tests are updated in the same region.
...
- `pub struct Transaction` -> `pub struct Edit` (region `💼️ transaction` -> `💼️ edit`); fields stay (`owner_draft` -> `owner_alternative_wip` rolled into `Alternative` state in step 2).
...
## 2. New shared interfaces (new region `🧷️ interfaces`)
...
## 3. Entity-type pass (region `🏷️ meta` + `📦️ kit`)
...
## 7. Modifications-aggregate pass (new region `📦️ modifications`)
...
## 8. Input pass (new region `📥️ input`)
