...
golden_paths = '''
    /// @emoji 📎️ US-001 replay fixtures (`kit-store.golden.*`) under `assets/compose/`.
...
mod = f"""//#region 📋️ kit_store_comprehensive_e2e

/// @emoji 📋️ In-process runner for `kit-store.comprehensive.compose.json` (native hosts + compose-store E2E).
...
start = text.index("    /// @emoji 📋️ Full store scenario catalog")
end = text.index("    /// @emoji 📜️ gql::sdl() is code-first")
