#!/usr/bin/env bun
/** @emoji 🏢️ Converts temp/*_Building.json BREPs to spatial.topology/v1 fixtures (centered, array buckets). */
...
for (const item of sources) {
  ...
  console.log(`[DEBUG] ${item.target}: ${asset.vertices.length} vertices, ${asset.edges.length} edges, ${asset.cells.length} cells, cellComplex=${asset.cellComplexes[0]?.cellIds.length ?? 0}`);
}
