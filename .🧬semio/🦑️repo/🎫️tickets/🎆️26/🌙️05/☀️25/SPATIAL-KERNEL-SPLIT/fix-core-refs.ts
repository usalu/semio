...
s = s.replace("StateEngine` + `KernelAdapter`", "StateEngine` + `SpatialKernel`");
s = s.replace(
...
s = s.replace('return typeof v === "number" ? Math.abs(v) : undefined;', 'return typeof v === "number" ? env.preview.abs(v) : undefined;');
s = s.replace("return vec3Distance(va, vb);", "return env.preview.vec3Distance(va, vb);");
s = s.replace(
  `return expr.op === "min"
...
s = s.replace("opts?: { readonly derived?: DerivedViewService }", "opts?: { readonly derived?: DerivedViewService; readonly preview?: SpatialPreviewKernel }");
s = s.replace('if (name === "position") return evaluateAnchorPosition(topo, anchor);', 'if (name === "position") return opts?.preview?.evaluateAnchorPosition(topo, anchor) ?? anchor.position;');
s = s.replace(`ctx: { readonly kernel: SpatialKernel; readonly topology: TopologyGraph }`, `ctx: { readonly kernel: SpatialKernel; readonly preview: SpatialPreviewKernel; readonly topology: TopologyGraph }`);
s = s.replace(/constructor\(private readonly kernel\?: SpatialKernel\) \{\}/, "constructor(private readonly kernel: SpatialKernel) {}");
s = s.replace("const sr = this.kernel?.computeSurfaceViews?.(topo);\n\t\tthis.surfaces = sr ? await Promise.resolve(sr) : computeSurfaceViewsFromTopology(topo);", "this.surfaces = await Promise.resolve(this.kernel.computeSurfaceViews(topo));");
s = s.replace("const pr = this.kernel?.computePartViews?.(topo);\n\t\tthis.parts = pr ? await Promise.resolve(pr) : computePartViewsFromTopology(topo);", "this.parts = await Promise.resolve(this.kernel.computePartViews(topo));");
s = s.replace(
...
s = s.replace("private cloneCtx(c: Record<string, unknown>): Record<string, unknown> {", runtimeHelper + "\n\tprivate cloneCtx(c: Record<string, unknown>): Record<string, unknown> {");

s = s.replace("return evalGuard(g, { context: this.sm.getContext() });", "return evalGuard(g, this.exprEnv());");
s = s.replace("const env: ExprEnv = { context: ctx };", "const env: ExprEnv = { context: ctx, preview: this.previewMath() };");
s = s.replace("const ar = await Promise.resolve(def.run(paramBag, { kernel: k, topology: topo }));", "const ar = await Promise.resolve(def.run(paramBag, { kernel: k, preview: this.previewMath(), topology: topo }));");
