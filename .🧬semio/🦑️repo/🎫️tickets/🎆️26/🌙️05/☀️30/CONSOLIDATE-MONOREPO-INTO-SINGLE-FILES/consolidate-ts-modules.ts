...
function stripHeader(src: string): string {
  ...
  let i = 0;
  if (lines[i]?.includes("#region") && (lines[i]?.includes("Header") || lines[i]?.includes("🧲️Header"))) {
    i++;
...
for (const satellitePath of shellSatellites) {
  ...
  shellBlocks.push(`//#region 🔖️${name}\n${body.trimEnd()}\n//#endregion 🔖️${name}`);
  unlinkSync(satellitePath);
}
...
for (const file of kitStoreFiles) {
  ...
  kitStoreBlocks.push(`//#region 🔖️${name}\n${body.trimEnd()}\n//#endregion 🔖️${name}`);
  unlinkSync(satellitePath);
}
...
if (existsSync(join(osDevDir, "plugin-registry.ts"))) {
  ...
  index = index
    ...
    .replace(/import "\.\.\/globals\.css";\n/, `import "../globals.css";\n\n//#region 🔖️plugin-registry\n${registry.trimEnd()}\n//#endregion 🔖️plugin-registry\n`);
  ...
  const scriptPath = join(osDevDir, "../📜️script.ts");
