...
s = s.replace(
  ...
  /** @emoji 🔗️ Flat graph port model; WIRES surfaces use \`normal\` (node-id edges, no handles). */
...
s = s.replace(/\/\*\* @emoji 🔗️ Builds a full anchor id/g, "/** @emoji 🔗️ Builds a full grip id");
s = s.replace(/\/\*\* @emoji 🔍️ Splits a full anchor id/g, "/** @emoji 🔍️ Splits a full grip id");
