...
# 🧭️Invoke: `bash ./script.sh setup` (full bootstrap) or `bash ./script.sh start` (IDE session / Neo4j-only path).
...
ensure_native_neo4j() {
  ...
  log "Neo4j: clearing graph in ${graph_db}, then loading generated .repo/🛂️/*.cypher (from bun run generate) …"
  ...
  for technology in "${technologies[@]}"; do
    local schema_file="$REPO_ROOT/.repo/🛂️/${technology}.cypher"
