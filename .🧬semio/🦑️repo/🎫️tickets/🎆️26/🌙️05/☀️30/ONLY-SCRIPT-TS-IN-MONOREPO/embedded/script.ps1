...
function Ensure-NativeNeo4j {
    ...
    Write-Step "Neo4j: clearing graph in $graphDb, then loading generated .repo/🛂️/*.cypher (from `bun run generate`) …"
    ...
    foreach ($technology in $technologies) {
        $schemaFile = Join-Path $RepoRoot ".repo\🛂️\$technology.cypher"
