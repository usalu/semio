...
// #region 🔖️KitHostCommandDispatch
/** @emoji 🪪️ Authoritative kit DTO from a host store (plain `kit` snapshots may omit class `toJSON`). */
...
/** @emoji 🪪️ Resolves {@link FolderKitStore} adapter when present (runtime field on class instance). */
...
/** @emoji 🪪️ Relative folder path for adapter I/O (prefer resolved `path` from {@link normalizeKitFullDtoFolderPaths}). */
...
/** @emoji 🪪️ Normalizes {@link Id} handles and plain strings for kit graph RPCs. */
...
/**
 * @emoji 📣️ Typed kit graph side-effect for kit/design apps (replaces string `kitWire`).
...
/** @emoji 🧾️ Applies {@link KitHostGraphOperation} through the live {@link KitStoreClient} bridge. */
...
/** @emoji 🧾️ VCS undo via the optional {@link KitStoreClient} bridge (typed alternative to `executeComposeKitCommand` undo). */
...
/** @emoji 🧾️ VCS redo via the optional {@link KitStoreClient} bridge (typed alternative to `executeComposeKitCommand` redo). */
...
/** @emoji 🧾️ String-command entry retained for sketchpad host flows; prefer {@link applyKitHostGraphOperation} for graph edits. */
...
/** @emoji 🧾️ Memoized kit string-command engine for legacy callers (sketchpad). */
...
/** @emoji 🧾️ Default {@link createKitCommandEngineExplicitOrigin} wrapper. */
...
// #endregion 🔖️KitHostCommandDispatch
...
/** @emoji 📌️Persistence metadata for an open kit in the registry. */
export type KitPersistenceInfo = { kind: "temporary" | "file" | "folder" | "remote"; path?: string; url?: string };

/** @emoji 📌️Desktop / webview: inject folder/file/session kit store creation (shell owns I/O). */
...
/** @emoji 🧾️ `useSyncExternalStore` with a derived snapshot and a custom equality for fewer rerenders. */
...
/** @emoji 📌️Derives file/folder/remote/temporary from backbone or store constructor. */
...
/**
 * @emoji 🧭️ One bridge: host kit id, active read scope, and optional VCS write anchors (set by {@link KitScope}).
 */
export type ComposeKitScopedView = {
  kitId: string;
  kitReadPoint: KitReadPoint;
  kitWriteScope: KitWriteScope | null;
  /** @emoji 🌱️ When {@link KitAlternativeSelectionProvider} is present: chosen alternative id, or `null` for the kit main line. */
  selectedAlternativeId: string | null;
};

const ComposeKitScopedViewContext = React.createContext<ComposeKitScopedView | null>(null);

/** @emoji 🧭️ `null` outside {@link KitScope}. */
export function useComposeKitScopedView(): ComposeKitScopedView | null {
  return React.useContext(ComposeKitScopedViewContext);
}

/**
 * @emoji 🧭️ Active {@link KitReadPoint} for read hooks: {@link ComposeKitScopedViewContext} when inside {@link KitScope}, else this default (main line).
 */
...
/** @emoji 📌️ Current {@link SchemaScope} from nearest entity scope provider (TypeScope, DesignScope, …). */
...
export type KitRegistryEntry = {
  ...
  refs: number;
  /** @emoji 📌️How this kit is persisted; derived at {@link KitRegistryValue.open} time. */
...
export type KitRegistryValue = {
  ...
  /** @emoji 📌️Open or bump refcount for a kit. */
...
/** @emoji 🧾️ Subscribe to registry open/close (new or removed kit ids), for {@link useOpenKitShallows}. */
...
// #region 🌱️KitAlternativeSelection
/** @emoji 🌱️ One row in the host alternative dropdown (id + display name from rs). */
...
/** @emoji 🌱️ Normalizes `wip.alternatives` whether rs returns a flat list or a relay {@link AlternativeConnection}. */
...
/**
 * @emoji 🌱️ Host wires the open kit id so {@link kitStoreFromKitStoreClient} can list graph alternatives (rs `wip.alternatives`).
...
/** @emoji 🌱️ Current alternative id, or `null` for the kit main line. */
...
/** @emoji 🌱️ Alternatives from rs (read-only), for host dropdowns. */
...
// #endregion 🌱️KitAlternativeSelection
  ...
  const open = React.useCallback(
    async (kitId: string, init: { backbone?: KitBackboneConfig; initialKit?: KitLike; store?: KitHostStore; kitClient?: KitStoreClient; readPoint?: KitReadPoint }) => {
      ...
      try {
        ...
        await applyKitClientSnapshotToLocalStore(kitClient, store);
...
/**
 * @emoji 📦️ Host root for kit scopes: delegates to {@link KitRegistryProvider}. Pass `initialKit` for future eager `openKit` bootstrap (thin hooks plan).
 */
...
/** @emoji 📄️ Browser SPA: pick a `.json` kit via File System Access API or a read-only file input fallback. */
...
/** @emoji 🌐️ Browser SPA: remote kit via session transport; `kit.name` must hold the server URL. */
...
/** @emoji 🧩️ VS Code webview: JSON read/write via extension `postMessage` and injected `__COMPOSE_KIT_JSON__`. */
...
/** @emoji 📌️ Shell / tab kit id (DOM bridge, R3F); complements {@link useActiveKitId} from {@link KitRuntimeContext}. */
export type KitShellScopeValue = { id: string };

const KitShellScopeContext = React.createContext<KitShellScopeValue | null>(null);

/** @emoji 📌️ Provider for tab-scoped kit id (sketchpad shell, scene bridge). */
...
/** @emoji 📌️ Same context as {@link KitShellScopeContext}; stable name for kit tab scope. */
export const KitScopeContext = KitShellScopeContext;

/**
 * @emoji 📌️ Resolves kit id: explicit argument, then {@link useKitShellScope}, then {@link useActiveKitId}.
 */
...
/** @emoji 📌️ Active kit scope `{ id }` from shell context or runtime (same resolution as sketchpad `useKitScope`). */
...
/**
 * @emoji 📌️ Live {@link KitHostStoreSnapshot} for the resolved kit when it matches the current {@link KitRuntimeContext}.
 */
...
/**
 * @emoji 📌️ `useSyncExternalStore` on a `getSnapshot`/`subscribe` pair with a stable server snapshot.
...
/** 🧾️ Stable {@link KitStoreReadSnap} identities for {@link useComposeReadSnap} idle branches (avoid React #520). */
...
/** @emoji 📌️ Type ids from live kit graph (`ReadKitTypeIdsCommand`); async hub + {@link KitStore.kindRowIds}. */
...
/** @emoji 📌️ Per-kind metadata rows (`ReadKitTypesMetadataCommand`) via {@link KitStore.kindMetadataRows}. */
...
/** @emoji 📌️ Design ids from live kit graph (`ReadKitDesignIdsCommand`) via {@link KitStore.designRowIds}. */
...
/** @emoji 📌️ Per-design metadata rows (`ReadKitDesignsMetadataCommand`) via {@link KitStore.designMetadataRows}. */
...
/** @emoji 📌️ Kit `types` from `KitStore.read` `readKitFullCommand` (RS materialization, not host DTO scan). */
...
/** @emoji 📌️ Kit `designs` from `KitStore.read` `readKitFullCommand` (RS materialization). */
...
/** @emoji 📌️ Kit `files` from `KitStore.read` `readKitFullCommand` (RS materialization). */
...
/** @emoji 📌️ Kit `tags` from `KitStore.read` `readKitFullCommand` (RS materialization). */
...
export type KitScopeProps = {
  ...
  kitClient?: KitStoreClient | null;
  /** @emoji 🧭️ Initial {@link KitReadPoint} for `createKitStoreClient` / read materialization (omit to follow {@link KitAlternativeSelectionProvider}, else main line). */
  kitReadPoint?: KitReadPoint;
  /** @emoji 🧭️ When set, pins {@link KitStoreClient.setKitWriteScope} for batched kit mutations (omit to auto-bootstrap per gesture). */
  kitWriteScope?: KitWriteScope | null;
...
/** @emoji 📌️ App-route aliases matching sketchpad `*ScopeProvider` names; same components as entity scopes. */
...
/** @emoji 📌️ Minimal {@link SchemaScope} when only entity id is known (e.g. R3F context bridge). */
 ...
 * @emoji 🧾️ Replaces the confusing `useDraft` name (conflicted with removed Draft entity semantics).
 */
...
// #region 🔖️ChangeLifecycleAndCommandBuilder

/** @emoji 🧭️ Stable {@link CommandBuilder} for nested `Mutation.session` writes (see `@semio-tech/compose-js` implementation). */
...
/**
 * @emoji 🧭️ **Change** lifecycle façade for the current kit write scope.
...
export function useChange(): {
  ...
  const currentChangeId = (client?.getKitWriteScope()?.changeId ?? null) as ChangeId | null;
...
/** @emoji 🧭️ Opens a new kit-write transaction (`VersionCommandInput.startNewChange`). */
...
/** @emoji 🧭️ Saves the active unsaved change (`VersionCommandInput.save` / `UnsavedChangeCommandInput.save`). */
...
/** @emoji 🧭️ Placeholder until {@link Alternative.unsavedChanges} live-read selection lands in Worker E. */
export function useUnsavedChanges(): KitFieldBinding<readonly unknown[]> {
  return kitReadonlyTriad(EMPTY_UNSAVED_CHANGES_LIST);
}

/** @emoji 🧭️ Session hub auth (stub until rs hub mutations are wired). */
...
/** @emoji 🧭️ Session hub logout (stub until rs hub mutations are wired). */
...
/** @emoji 🧭️ `Session.startAlternative` wrapper around {@link KitStoreClient.createAlternativeFromTip}. */
...
/** @emoji 🧭️ `AlternativeCommandInput.integrateIntoTheKit` (stub until rs merges alternatives). */
...
// #endregion 🔖️ChangeLifecycleAndCommandBuilder
...
/** @emoji 🧾️ Patch root {@link Kit} fields via `submitKitChangeCommands` (value lane separate from reads). */
...
/** @emoji 🧾️ Normalizes live-read snapshot data into a piece document metadata map (Map or plain record from older hubs). */
...
/** @emoji 📌️ Piece DTO rows for a design via {@link DesignStore.readPiecesFullRows}. */
...
/** @emoji 📌️ Connection DTO rows for a design via {@link DesignStore.readConnectionsFullRows}. */
...
/** @emoji 📌️ Shallow design catalog rows via {@link KitStore.getDesigns}. */
...
/** @emoji 📌️ Shallow kind catalog rows via {@link KitStore.getTypes}. */
...
/** @emoji 📌️ Shallow author rows via {@link KitStore.getAuthors}. */
...
/** @emoji 📌️ Piece rows for the active {@link DesignScope} (sketchpad-style, no KitFieldBinding). */
...
/** @emoji 📌️ Connection rows for the active {@link DesignScope} (sketchpad-style, no KitFieldBinding). */
...
/** @emoji 📌️ Author rows (`authorsShallowJson`) — use {@link useKitAuthorsShallow} for the raw `[value, status]` pair. */
...
/** @emoji 📥️ Read-only host store handle for the active kit scope (value, no setter, no status). */
...
/** @emoji 📌️ Full kit store snapshot (read-only); prefer over tuple wrappers. */
...
/** @emoji 📌️Alias for {@link useKitStoredFileUrls} (sketchpad compatibility). */
export const useFileUrls = useKitStoredFileUrls;

/**
 * @emoji 📌️Readable URL for a kit file (provider, embedded, or remote) — thin wrapper over `@semio-tech/compose-js` kit file helpers.
 */
...
/**
 * @emoji 📌️Resolves a blob: / object URL for a file (IndexedDB, binary store, provider, or fetch).
 */
...
/**
 * @emoji 📌️Embeds a dropped blob as data URL on the kit file record (`JsonFileKitStore` / compatible).
 */
...
/**
 * @emoji 📌️Binary sidecar I/O on folder-backed or compatible stores.
 */
...
/**
 * @emoji 📌️Live `KitFileState` (provider URLs, blobs, object URLs) for the current `KitHostStore`.
 */
...
/**
 * @emoji 📌️File / folder / remote / temporary kind for the current kit (registry or backbone).
 */
...
/**
 * @emoji 📌️Path/url metadata for the current kit’s persistence.
 */
...
/** @emoji 📌️Listed kit ids in the registry (empty if no provider); uses bridge so LayoutCanvas windows see ids without React context. */
...
/** @emoji 📌️Active kit: registry selection first, else current {@link KitScope} id. */
...
/**
 * @emoji 🪝️ Attach read-only `snapshot` / `fileUrls` on {@link KitHostStore} for legacy design-store selectors; graph mutations use {@link applyKitHostGraphOperation} / {@link executeComposeKitCommand}.
 */
...
/** @emoji 🧾️ Memoized {@link createKitCommandEngineExplicitOrigin} for a host {@link KitHostStore} (null when no store). */
...
/** @emoji 📌️ Shallow kit rows for every open registry kit; subscribes per {@link KitHostStore} + registry list changes via bridge (canvas portals lack {@link KitRegistryContext}). */
...
/** @emoji 📌️ True when {@link KitRegistryValue} holds the kit id (updates when kits open/close). */
export function useRegistryHasKit(kitId: string): boolean {
  const ids = useOpenKitGuids();
  return ids.includes(kitId);
}

/** @emoji 📌️ Persistence kind for a registry kit (undefined if not open); uses bridge for canvas portals without registry context. */
...
/** @emoji 📌️Alias hooks for explicit "by id" call sites. */
...
// #region 🔖️SchemaReadWriteSegregation
/**
 * @emoji 📥️ Read-only schema object value (no setter, no status). Returns `undefined` outside {@link KitScope}.
 */
...
/**
 * @emoji 📥️ Read-only schema field value (no setter, no status). Returns `undefined` outside {@link KitScope}.
 */
...
/**
 * @emoji 📝️ Schema object mutation `[run, status]` (no reads). Returns no-op outside {@link KitScope}.
 */
...
/**
 * @emoji 📝️ Schema field mutation `[run, status]` (no reads). Routes through Rust `submitKitChangeCommands` when supported, falls back to host store DTO writes.
 */
...
// #endregion 🔖️SchemaReadWriteSegregation
...
/** @emoji 📌️ Scoped entity read: merges {@link useAuthorScope} id with optional explicit id; selector optional. */
...
/** @emoji 📌️ Scoped entity read for {@link TypeScope}. */
...
/** @emoji 📌️ Scoped entity read for {@link QualityScope}. */
...
/** @emoji 📌️ Scoped entity read for {@link DesignScope}; middle arg kept for sketchpad call shape (unused). */
...
/** @emoji 📌️ Scoped entity read for {@link PieceScope}. */
...
/** @emoji 📌️ Scoped entity read for {@link ConnectionScope}. */
...
/** @emoji 🧾️ One `useSyncExternalStore` over a {@link StoreField} (no local mechanism). */
export function useStoreField<T>(field: StoreField<T>): T {
  return React.useSyncExternalStore(field.subscribe, field.getSnapshot, field.getSnapshot);
}

/** @emoji 🧾️ One `useSyncExternalStore` on command status + stable {@link StoreCommand.run}. */
...
/** @emoji 🪪️ Live kit name string from {@link KitStoreClient.kitName} (requires {@link KitScope} with kit client). */
...
/** @emoji 🪪️ Rename kit command + {@link WriteStatus} from {@link KitStoreClient.renameKit} (string arg maps to uniform `scope`/`input`). */
...
/** @emoji 🪪️ Kit `version` string from the active kit snapshot (legacy name {@link useKitRelease}). */
...
/** @emoji 📝️ **Edit** record with forward/backward operation lists. */
...
// [BLOCKED] §7.1.2 schema hooks + Scope→Context migration: see .repo/🎫️/26/05/12/field-only-kit-reads-cqrs-classes/react-blocker.json (JS export / tsc parity required first).
