  ...
  for (let i = 0; i < Math.min(tooltipTriggers.length, 30); i++) {
    ...
    if (tooltipCount > 0) {
      ...
      if (tooltipBox) {
        ...
        if (atTopLeft) {
          console.log(`[DEBUG] ❌️ TOOLTIP AT TOP-LEFT! Trigger at (${box.x.toFixed(0)},${box.y.toFixed(0)}) but tooltip at (${tooltipBox.x.toFixed(0)},${tooltipBox.y.toFixed(0)})`);
        } else {
          console.log(`[DEBUG] ✅️ Tooltip near trigger (dx=${dx.toFixed(0)}, dy=${dy.toFixed(0)})`);
        }
      } else {
