...
def strip_header(content):
    match = re.search(r"// #endregion\s+🔖️?Header\b", content)
    if match:
...
def main():
    ...
    if "export const sketchpadCommands" not in sketchpad_content:
        ...
        return
    print("✓️ Sketchpad pre-processing done")
    ...
    if const_dupes:
        print(f"⚠️ Duplicate const: {const_dupes}")
    else:
        print("✓️ No duplicate const declarations")
    if enum_dupes:
        print(f"⚠️ Duplicate enum: {enum_dupes}")
    else:
        print("✓️ No duplicate enum declarations")
    if func_dupes:
        print(f"⚠️ Duplicate function: {func_dupes}")
    else:
        print("✓️ No duplicate function declarations")
    if let_dupes:
        print(f"⚠️ Duplicate let/var: {let_dupes}")
    else:
        print("✓️ No duplicate let/var declarations")
    ...
    print(f"✓️ Written to {SKETCHPAD}")
