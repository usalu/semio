...
function exportModelToAssets(model: any, outputPath: string): void {
  try {
    ...
    console.log(`✅️ Compose Model exported to: ${outputPath}`);
    console.log(`📊️ Model contains ${model.attributes?.length || 0} attributes and ${model.tags?.length || 0} tags`);
  } catch (error) {
    console.error(`❌️ Failed to export model: ${error}`);
    throw error;
  }
}
 ...
 */
function main(): void {
  console.log("🚀️ Starting Nakagin Capsule Tower design model export with compose functions...");
  ...
  if (design.description) {
    console.log(`📄️ Description: ${design.description.substring(0, 100)}...`);
  }
  ...
  console.log("🎉️ Export completed successfully!");
}
