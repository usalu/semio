...
def debug_export():
    ...
    try:
        ...
        if not nakagin_designs:
            print('❌️ Nakagin Capsule Tower design not found')
            return
        ...
        if pieces:
            piece = pieces[0]
            print(f"🔍️ Sample piece: {piece.get('name')}")
            print(f"🔍️ Piece type: {piece.get('type')}")
            
        # Try the export
        print("🔄️ Attempting IFC export...")
        ...
        print(f"✅️ Export successful! Size: {len(ifc_bytes)} bytes")
        
        # Save to test file
        output_path = Path('/workspaces/semio/.repo/🎫️/26/03/20/EXPORT-NAKAGIN-CAPSULE-TOWER-IFC-FILE-TO-REPORTS/test-nakagin.ifc')
        with open(output_path, 'wb') as f:
            f.write(ifc_bytes)
        print(f"💾️ Test file saved to: {output_path}")
