...
await page.evaluate(() => {
  window.__LT.tasks = [];
  window.__LT.markers.push({ name: "START", t: performance.now() });
});
...
await page.evaluate(() => {
  window.__LT.markers.push({ name: "PANEL_CLOSED", t: performance.now() });
});
...
await page.waitForTimeout(500);
await page.evaluate(() => {
  window.__LT.markers.push({ name: "ZOOM_IN", t: performance.now() });
});
await page.mouse.wheel(0, 600);
await page.waitForTimeout(500);
await page.evaluate(() => {
  window.__LT.markers.push({ name: "ZOOM_OUT", t: performance.now() });
});
...
await page.evaluate(() => {
  ...
  if (nodesEl) {
    const mo = new MutationObserver((muts) => {
      ...
      window.__LT.mutPhases.push({
        t: performance.now(),
        n: muts.length,
        types: muts
          .slice(0, 5)
          .map((m) => m.type + ":" + m.attributeName)
          .join(","),
      });
    });
...
await page.evaluate(() => {
  window.__LT.markers.push({ name: "BEFORE_DOWN", t: performance.now() });
});
await page.mouse.down();
await page.waitForTimeout(50);
await page.evaluate(() => {
  window.__LT.markers.push({ name: "AFTER_DOWN", t: performance.now() });
});
await page.mouse.move(sx + 100, sy, { steps: 20 });
await page.evaluate(() => {
  window.__LT.markers.push({ name: "AFTER_MOVE", t: performance.now() });
});
await page.evaluate(() => {
  window.__LT.markers.push({ name: "BEFORE_UP", t: performance.now() });
});
await page.mouse.up();
await page.waitForTimeout(200);
await page.evaluate(() => {
  window.__LT.markers.push({ name: "AFTER_UP", t: performance.now() });
});
await page.waitForTimeout(5000);
await page.evaluate(() => {
  window.__LT.markers.push({ name: "SETTLE", t: performance.now() });
});
...
for (const t of data.tasks) {
  ...
  for (let i = data.markers.length - 1; i >= 0; i--) {
    if (t.s >= data.markers[i].t) {
      phase = data.markers[i].name;
      break;
    }
  }
