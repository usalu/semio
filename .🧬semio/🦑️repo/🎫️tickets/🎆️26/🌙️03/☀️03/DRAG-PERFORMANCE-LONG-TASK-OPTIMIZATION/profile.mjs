...
(async () => {
  ...
  if ((await fileInput.count()) === 0) {
    ...
    const [fileChooser] = await Promise.all([
      ...
      page
        .locator('button:has-text("Open"), [aria-label*="open"], [data-testid*="open"]')
        .first()
        .click()
        .catch(() => {}),
    ]).catch(() => [null]);
  ...
  await page.evaluate(() => {
    performance.mark("zoom-in-start");
    window.__PERF__.marks.push({ name: "zoom-in-start", time: performance.now() });
  });
  ...
  await page.waitForTimeout(500);
  await page.evaluate(() => {
    performance.mark("zoom-in-end");
    window.__PERF__.marks.push({ name: "zoom-in-end", time: performance.now() });
  });

  // Phase 2: Zoom Out
  await page.evaluate(() => {
    performance.mark("zoom-out-start");
    window.__PERF__.marks.push({ name: "zoom-out-start", time: performance.now() });
  });
  await page.mouse.wheel(0, 600);
  await page.waitForTimeout(500);
  await page.evaluate(() => {
    performance.mark("zoom-out-end");
    window.__PERF__.marks.push({ name: "zoom-out-end", time: performance.now() });
  });
  ...
  await page.evaluate(() => {
    performance.mark("mousedown-start");
    window.__PERF__.marks.push({ name: "mousedown-start", time: performance.now() });
  });
  ...
  await page.waitForTimeout(50);
  await page.evaluate(() => {
    performance.mark("mousedown-end");
    window.__PERF__.marks.push({ name: "mousedown-end", time: performance.now() });
  });

  // Phase 4: Mouse move (drag) - 20 steps
  await page.evaluate(() => {
    performance.mark("mousemove-start");
    window.__PERF__.marks.push({ name: "mousemove-start", time: performance.now() });
  });
  await page.mouse.move(startX + 100, startY, { steps: 20 });
  await page.evaluate(() => {
    performance.mark("mousemove-end");
    window.__PERF__.marks.push({ name: "mousemove-end", time: performance.now() });
  });

  // Phase 5: Mouse up
  await page.evaluate(() => {
    performance.mark("mouseup-start");
    window.__PERF__.marks.push({ name: "mouseup-start", time: performance.now() });
  });
  await page.mouse.up();
  await page.waitForTimeout(200);
  await page.evaluate(() => {
    performance.mark("mouseup-end");
    window.__PERF__.marks.push({ name: "mouseup-end", time: performance.now() });
  });
