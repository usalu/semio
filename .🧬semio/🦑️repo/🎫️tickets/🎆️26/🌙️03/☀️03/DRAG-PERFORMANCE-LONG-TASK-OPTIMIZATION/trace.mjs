...
(async () => {
  ...
  await sleep(50);
  await page.evaluate(() => {
    performance.mark("drag-moving");
  });
  await page.mouse.move(startX + 100, startY, { steps: 20 });
  await page.evaluate(() => {
    performance.mark("drag-moved");
  });
  await page.mouse.up();
  await sleep(200);
  await page.evaluate(() => {
  ...
  for (const phase of phases) {
    longTasks.filter((t) => t.startTime >= phase.start - 100 && t.startTime <= phase.end + 100).forEach((t) => allPhasedStarts.add(t.startTime));
  }
