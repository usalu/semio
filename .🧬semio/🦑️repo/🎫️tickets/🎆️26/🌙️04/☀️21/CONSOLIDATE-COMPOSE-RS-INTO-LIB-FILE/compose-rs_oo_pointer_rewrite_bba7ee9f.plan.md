---
name: compose-rs OO pointer rewrite
overview: "Ground-up rewrite of `compose/rs` as a purely object-oriented, pointer-first domain model: `Arc<RwLock<T>>` for parent-owned children, `Weak<RwLock<T>>` for immutable back-references, no free functions, no GUID-based lookups in the in-memory graph, and lazy derived caches behind `OnceLock` / `Cell<stale>` on each entity. DTOs survive only as a thin `serde` boundary for JSON I/O."
todos:
 - id: scaffold
   content: Move compose/rs/lib.rs to compose/rs/src/ with per-concept files; update Cargo.toml path; set up empty module tree.
   status: completed
 - id: core_entities
 ...
 - id: wasm
   ...
   status: completed
 - id: consumer
   content: Update compose/algorithms/native-bridges/rs/src/main.rs to the new API (kit.design(g), design.flatten(), etc.).
   status: completed
 - id: tests
   content: Port the existing ~84 tests (mod tests, mod benchmark, mod kit_backbone_async) to the new OO API, split into compose/rs/src/tests/; cargo test --lib -p compose green.
   status: completed
 - id: verify
   content: cargo build for host and wasm32; cargo test; smoke-test compose-algorithms-native-bridges-rs with a fixture kit.
   status: completed
isProject: false
---
...
From [`compose/rs/lib.rs`](compose/rs/lib.rs), the following is deleted outright (no deprecated wrappers):
...
- `ComposeUtil` class with static free-style helpers — methods move onto the natural owner (`Kit::new_guid()`, `Quality::round`, etc.) except `generate_guid`, which becomes `Guid::new()` on a new `Guid` newtype.
...
Move from `compose/rs/lib.rs` (1 file, 27K lines) to `compose/rs/src/` with one file per concept (update `Cargo.toml` `path = "src/lib.rs"`):
...
- [`compose/algorithms/native-bridges/rs/src/main.rs`](compose/algorithms/native-bridges/rs/src/main.rs) updated in the same commit: `req.kit.design_by_guid(&g)` → `req.kit.design(&g)` returning `Arc<RwLock<Design>>`; `design.flatten(&req.kit)` becomes `design.flatten()` (parent is reachable via `Weak`); piece/connection lookups become `design.piece(guid)` / `design.connection(guid)` which are the internal index helpers (kept for DTO-shaped input but not for graph traversal).
- [`compose/hub`](compose/hub/Cargo.toml) — does not `use compose::*` despite the dep; left untouched, builds unchanged.
...
- `cargo build -p compose` and `cargo build -p compose --target wasm32-unknown-unknown`.
- `cargo test --lib -p compose` — the ~84 existing tests are ported to the new API in the same PR; red tests fail the plan.
- `cargo build -p compose-algorithms-native-bridges-rs` (name per `Cargo.toml`).
...
- No change to `compose/hub`, `compose/py`, `compose/ts`, `compose/gh`, `compose/3dm`, or any non-Rust bundle.
