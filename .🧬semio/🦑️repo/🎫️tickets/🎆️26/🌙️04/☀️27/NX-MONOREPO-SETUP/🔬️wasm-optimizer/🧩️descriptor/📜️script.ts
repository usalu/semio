...
const runtime = await import(join(owner, "🟦️.ts")), { PLUGIN_DESCRIPTOR_PROBE_SOURCE } = await import(join(owner, "📜️script.ts"));
const { pluginModulesRoot } = await import(resolve(workspace, "🧰️framework/🛍️products/💻️os/🔨️modules/🧑‍💻dev/♻️activation/🟦️.ts"));
...
for (const optimize of [false, true]) {
  ...
  await runtime.transpilePluginComponentAsync(artifact, path, "note_component", { repoRoot: workspace, preview2VendorDir: join(pluginModulesRoot("release"), runtime.PREVIEW2_VENDOR_RELATIVE), signal: controller.signal, optimize });
