...
## Concurrent Consumer Regression

The 2026-09-08 template/gallery consumer run finished after 98m33s with all 87 document producers complete, but both consumer tasks failed before PDF parsing. Their quick preflight shared `print-validation/.staging-test`; one task removed files while the other was reading them. Both ENOENT stacks point to that staging fixture. The fixture now allocates a unique native mkdtemp directory for each invocation and removes only its own directory. The producer implementation is unchanged. Concurrent quick verification, PDF consumption, and final-input collection cache restoration remain pending.

Concurrent `test-quick,test-viz` now passes through Nx with both tasks actually executing (0/2 cache hits), in 6.9s. This replays the shared language-neutral staging fixture in both concurrent processes; the unique staging-directory fix prevents cross-removal. The final-input 87-document collection proof is compiling with NX_DAEMON=false. Its three phases will separately check population, identical warm reuse, and clean-output restoration. Full PDF consumption remains pending.
