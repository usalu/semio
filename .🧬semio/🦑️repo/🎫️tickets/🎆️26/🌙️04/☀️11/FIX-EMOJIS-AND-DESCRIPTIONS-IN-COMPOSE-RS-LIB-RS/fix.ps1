$file = "c:\git\compose\compose\rs\lib.rs"
...
for ($i = 0; $i -lt $lines.Count; $i++) {
    ...
    # Pattern: /// 🔖️<summary>🔖️funcname holds the data fields for a funcname record.</summary>
    # ============================================================
    if ($line -match '^\s*///\s*.{0,8}<summary>.{0,8}(\w+)\s+holds the data fields for a \w+ record\.</summary>') {
        ...
        if (-not $allDescMap.ContainsKey($funcName)) {
            ...
            elseif ($funcName -match '^(import|export)_') {
                if ($funcName -match '^import') { $funcEmoji = [char]::ConvertFromUtf32(0x1F4E5) } # 📥️
                else { $funcEmoji = [char]::ConvertFromUtf32(0x1F4E4) } # 📤️
                $funcDesc = "$($funcName -replace '_', ' ')"
            }
            # Edit functions
            ...
            elseif ($funcName -eq "jaccard") {
                $funcEmoji = [char]::ConvertFromUtf32(0x1F4CA)  # 📊️
            ...
            elseif ($funcName -eq "generate_unique_name") {
                $funcEmoji = [char]::ConvertFromUtf32(0x1F4DD)  # 📝️
            ...
            elseif ($funcName -eq "SUPPORTED_MODEL_EXTENSIONS") {
                $funcEmoji = [char]::ConvertFromUtf32(0x1F5FF)  # 🗿️
            ...
            elseif ($funcName -eq "zip_roundtrip") {
                $funcEmoji = [char]::ConvertFromUtf32(0x1F4E6)  # 📦️
                $funcDesc = "ZIP archive round-trip import and export"
            }
            elseif ($funcName -eq "wasm") {
                $funcEmoji = [char]::ConvertFromUtf32(0x1F310)  # 🌐️
                $funcDesc = "WebAssembly bindings for the compose library"
            }
            elseif ($funcName -eq "tests") {
                $funcEmoji = [char]::ConvertFromUtf32(0x1F9EA)  # 🧪️
                $funcDesc = "unit and integration tests for the compose library"
            }
            elseif ($funcName -eq "planes_equal_approx") {
                $funcEmoji = [char]0x25FB + [char]0xFE0F  # ◻ 
                $funcDesc = "compares two planes for approximate equality"
            }
            elseif ($funcName -match '^compute_') {
                $funcEmoji = [char]::ConvertFromUtf32(0x1F4BB)  # 💻️
                $funcDesc = "$($funcName -replace '_', ' ')"
            }
            elseif ($funcName -match '^make_') {
                $funcEmoji = [char]::ConvertFromUtf32(0x1F527)  # 🔧️
                $funcDesc = "$($funcName -replace '_', ' ')"
            }
    ...
    # ============================================================
    # Fix 8: ComposeError doc comment
    # ============================================================
    if ($line -match '^\s*///\s*.+<summary>.+ComposeError\s+holds the data fields') {
        $indent = ""
        if ($line -match '^(\s*)') { $indent = $Matches[1] }
        $newLines.Add("$indent/// <summary>" + [char]0x274C + "ComposeError represents a domain error with context message.</summary>")
        $fixCount++
        continue
    }
    ...
    if ($line -match '^\s*///\s*.+<summary>.+Result\s+holds the data fields') {
        $indent = ""
        if ($line -match '^(\s*)') { $indent = $Matches[1] }
        $newLines.Add("$indent/// <summary>" + [char]0x2705 + "Result represents a success or ComposeError outcome.</summary>")
        $fixCount++
        continue
    }
    ...
    # Also: /// 🔖️<summary>📖️find_type_in_kit ....
    ...
    # like: /// 💘️All valid KitKind values.
    # ============================================================
    ...
    # e.g. // 📆️Model Types - Attribute → // 💎️Model Types - Attribute
    # ============================================================
    ...
    # e.g. "/// 📚️<summary>🔖️Converts a nalgebra..."
