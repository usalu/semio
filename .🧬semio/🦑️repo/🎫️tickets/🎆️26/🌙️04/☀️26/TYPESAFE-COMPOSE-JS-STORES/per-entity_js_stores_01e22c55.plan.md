...
overview: "Replace the stub entity classes in `compose/js/index.ts` with real per-entity stores that hold only `{ root: KitStore, id: string }` (plus design scope where needed), delegate reads/mutations to the existing `KitStore` GraphQL paths, and add filtered `subscribe` based on serialized `KitEvent` shapes from `compose/rs`. React/sketchpad migration to `useSyncExternalStore` hooks stays a follow-on once these exports are stable."
todos:
 ...
 - id: agents-embedded-migrate
   content: Update compose/js/AGENTS.md export note if needed; migrate embedded tests off public patchEntityField where replaced
   status: completed
isProject: false
---

# Implement per-entity stores (`compose/js`)

## Current state

- [`compose/js/index.ts`](c:\git\compose\compose\js\index.ts): [`KitStore`](c:\git\compose\compose\js\index.ts) (~lines 466–1100+) already implements worker/wasm transport, `submitShell` / `submitShellJson`, `read()`, `patchEntityField` / `addChild` / `removeChild`, design mutations (`clusterPieces`, `dragPieces`, …), VCS, backbone, and `openKit` (re-export of `KitStore.open`).
- Lines **3540–3597** (`#region EntityStoreStubs`): [`DesignStore`](c:\git\compose\compose\js\index.ts), [`TypeStore`](c:\git\compose\compose\js\index.ts), [`PieceStore`](c:\git\compose\compose\js\index.ts), [`ConnectionStore`](c:\git\compose\compose\js\index.ts), [`FamilyStore`](c:\git\compose\compose\js\index.ts), [`FileStore`](c:\git\compose\compose\js\index.ts), [`FolderStore`](c:\git\compose\compose\js\index.ts), [`KitEntityStore`](c:\git\compose\compose\js\index.ts) are **empty constructors** with `unknown` fields — re-exported from [`compose/react/index.tsx`](c:\git\compose\compose\react\index.tsx) but unusable.
- [`compose/rs/lib.rs`](c:\git\compose\compose\rs\lib.rs) `KitEvent` (externally tagged JSON) carries entity ids (`Design { design_id }`, `Type { type_id }`, nested design events, etc.) — suitable for **client-side filtering** without a new rs field.

## Target (aligned with [strict layering plan](c:\git\compose.cursor\plans\strict_compose_layering_refactor_205dc73c.plan.md) §3.1)
...
- **Mutators**: delegate to existing `KitStore` methods (`clusterPieces`, `patchEntityField`, …) until every call site has a semantic `ChangeKitCommand` shell; return type can stay `SetResult` initially or add a thin `receiptFromSetResult` mapping to [`KitCommandReceipt`](c:\git\compose\compose\js\index.ts) for API parity with the written plan.
- **`subscribe`**: `root.subscribe` wrapped with a predicate `kitEventTouchesStore(ev: KitEvent): boolean` that understands serde JSON tags (`Design`, `Type`, `Piece`, `Connection`, `ChildAdded`, `Changed`, …) and the entity’s ids; always forward `composeKitCommand` lifecycle events if the store initiated a mutation (optional: correlate by `requestId` if you pass it through).

## Implementation steps (all in existing [`compose/js/index.ts`](c:\git\compose\compose\js\index.ts) regions per repo rules)
...
5. **`FamilyStore`, `FileStore`, `FolderStore`, `KitEntityStore`**
   - Same pattern: minimal read + `patchEntityField` / child ops + filtered subscribe. Expand surface only where [`compose/react`](c:\git\compose\compose\react\index.tsx) or sketchpad imports symbols (grep before deleting stubs).

6. **Event filter helper**
   - One internal module region (e.g. `#region KitEventEntityFilter`) with exhaustive-ish matching on keys present in JSON from [`KitEvent`](c:\git\compose\compose\rs\lib.rs) (~15586); unit-test with fixture objects (no wasm) in the **existing** embedded vitest block at the bottom of `index.ts`.
...
8. **Repo process (when executing, not in plan-only mode)**
   - Per [AGENTS.md](c:\git\compose\AGENTS.md): `repo` MCP `search`, read `repo://goals`, `ticket_open` for e.g. `refactor-js-per-entity-stores`, work only under `.repo/🎫️/...` for scratch, `ticket_close` with file list.
...
- **`compose/react`**: `useDesignStore` / `useSyncExternalStore` thin hooks ([plan §4](c:\git\compose.cursor\plans\strict_compose_layering_refactor_205dc73c.plan.md)) — depends on stable store API; [`useKitStore`](c:\git\compose\compose\react\index.tsx) still returns `KitHostStore` today.
- **Migrate** [`patchEntityField`](c:\git\compose\compose\react\index.tsx) call sites to typed store methods incrementally.
...
- **AGENTS.md** ([`compose/js/AGENTS.md`](c:\git\compose\compose\js\AGENTS.md)) says root exports only `KitStore`, `openKit`, and needed types — entity store classes **are** part of the public API once real; update that sentence if it currently forbids exporting them.
