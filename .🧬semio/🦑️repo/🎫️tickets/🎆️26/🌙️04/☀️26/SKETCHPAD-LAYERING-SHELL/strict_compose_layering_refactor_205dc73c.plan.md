---
name: Strict compose layering refactor
overview: Refactor the `compose/rs` → `compose/js` → `compose/react` → `compose/sketchpad` stack so each layer only knows the implementation details of the layer immediately below it, with all domain logic, caching and persistence concentrated in `compose/rs`, async fire-and-forget semantic commands at the boundary, central diff-based application internally, per-entity typed stores in JS, and `useSyncExternalStore` / `useCallback` hooks in React.
todos:
 - id: rs_shell
...
# Strict Compose Layering Refactor
...
## 2. compose/rs (single mutating boundary, diff-only application)

Existing entry: [`compose/rs/lib.rs`](compose/rs/lib.rs) (1.5MB; keep as one file with `pub mod` blocks; do NOT split).
...
Extend [`pub mod kit_backbone_wire`](compose/rs/lib.rs) with `BackboneConfig::Memory` (no-op storage; default after `KitStoreHandle::create`). Keep `Dev` (single JSON), `Local` (`.compose/kit.db` + blobs), `Remote` (hub).

Add `attachBackbone(cfg)` / `detachBackbone` / `backboneStatus` / `listConflicts` / `resolveConflict` / `syncNow` to the `submitKitCommand` shell as `KitCommand` variants (not separate GraphQL fields). Wasm executes Dev/Memory natively; Local/Remote return `NotSupported` on wasm and are routed to a native compose-store sidecar.
...
## 3. compose/js (per-entity Store classes, thin GraphQL client)

Refactor [`compose/js/index.ts`](compose/js/index.ts) (currently a single ~1100-line `KitStore` god class).
...
- Delete the entire `// #region 🪢️InternalReadWire` block plus all `mapReadCommand` / `mapDesignRead` / `mapPieceRead` / `mapTypeRead` shape-mapping (rs returns the canonical shape directly).

### 3.2 Transport

Keep `WorkerStringTransport` + `InlineWasmTransport` (current files [`compose/js/worker.ts`](compose/js/worker.ts), [`compose/js/index.ts`](compose/js/index.ts) lines 270-416). Tighten to GraphQL strings only. RxJS becomes a private `Subject`; never appears in `.d.ts`.

## 4. compose/react (thin hooks bundle)

Reduce [`compose/react/index.tsx`](compose/react/index.tsx) (currently 635KB) and delete [`compose/react/kitWasmClient.ts`](compose/react/kitWasmClient.ts) (118KB).
...
## 5. compose/sketchpad (UI + local selection only)

Edit [`compose/sketchpad/index.tsx`](compose/sketchpad/index.tsx) and the VS Code adapter section.

### 5.1 Remove

- All imports of `InMemoryKitStore`, `JsonFileKitStore`, `FolderKitStore`, `createSessionKitStore`, `applyKitClientSnapshotToLocalStore`, `acquireComposeKitCommandFacade`, `createKitCommandEngine*`, `KitHostStore`, `KitHostStoreSnapshot`, `KitBinaryStore`, `KitFileState`.
