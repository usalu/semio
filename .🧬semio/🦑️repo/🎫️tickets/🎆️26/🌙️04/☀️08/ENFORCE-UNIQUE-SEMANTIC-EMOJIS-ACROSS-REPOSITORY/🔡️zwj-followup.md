# ZWJ Naming Follow-up

## 2026-09-05 Stale Producer Completion

The stale Raster process was later terminated after its 1,200-second Cargo discovery budget expired. The recreated legacy Dev tree was re-inspected: its only physical top-level owners were `🔌️plugin-modules` and `🤖️generated`, totalling 19 MB, while all authored source and current generated outputs live under canonical `🧑‍💻dev`. No live source reference to `🧑️‍💻️dev` remained. The legacy generated tree was moved to the macOS Trash, so it is recoverable and no Git operation was used.

The normalization reader's package-source disposition vocabulary was brought into parity with discovery by registering the already-declared one-to-one `vscode-test-configuration` → `vscode-test-cli-config` validator authority. Both TypeScript interfaces now include the complete current tool-configuration validator union. The launch seed and generated launch catalog use the registered `🔤️taxonomy-leading-grapheme` identity instead of the stale `🧪️` identity.

After these repairs, `bun nx run @semio-tech/repo-lib:test-taxonomy-leading-grapheme` passes all 9 tests and 27,497 assertions. A dedicated rerun of `bun nx run @semio-tech/repo-lib:test-path-emoji-statutes` passes all 38 tests and 1,336 assertions in 10.46 seconds.
...
No listed path, central identity, or incoming reference was changed during the initial audit above. The coordinated completion below supersedes that initial hold. Retained historical witnesses remain separate from current source authorities, as in the completed WGPU catalog work.
...
The fourteen emoji identity cases are injected into one isolated taxonomy validation, avoiding fourteen redundant full-catalog validations. The focused result is two tests, 62 assertions and zero failures in 1.43 seconds. The existing taxonomy-leading-grapheme Nx target passes nine tests and 25,283 assertions. A direct current-catalog validation returns `[]`.
...
Canonical assets generation wrote 286 deterministic outputs and its check confirmed all 286 fresh. Plugin-registry generation completed for 59 plugin crates, 60 playgrounds and 45 framework packages and regenerated `.vscode/launch.json`; its broader check reached 4,297 unrelated pre-existing plugin-tree structural violations, so no clean plugin-tree claim is made. Canonical WGPU generation and the following check both exit zero; `🎞️frame-worker.js` is fresh and was not hand-edited. The OS kernel Cargo check exits zero, proving the recipe `#[path]` and grammar `include_str!` consumers compile; existing warnings remain unchanged.

The registered path-statute Nx route reaches 26 green tests before its 15-second process budget under concurrent workspace load; no assertion fails before the budget kill. Its two new focused tests pass as recorded above. A separate external end-to-end publisher (PIDs 36524/36533, with active Cargo/rustc descendants) was launched from a historical ticket with the old Dev path already loaded and has recreated 47 generated files under `🧑️‍💻️dev`. That live output is untouched. It is the sole current duplicate finding against the canonical `🧑‍💻dev` sibling and the sole reason the physical seven-owner assertion cannot yet be green; the statute is not weakened and no external process is stopped.
