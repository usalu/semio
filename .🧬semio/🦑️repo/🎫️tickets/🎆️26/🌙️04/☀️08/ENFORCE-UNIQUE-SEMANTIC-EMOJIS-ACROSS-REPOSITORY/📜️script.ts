...
const gitPathspecs = requestedPrefix ? [requestedPrefix] : [".", ...taxonomy.pathEmojiPolicy.reservedSubtreeDirectoryNames.map((name) => `:(exclude)${name}`)];
const raw = Bun.spawnSync(["git", "ls-files", "-co", "--exclude-standard", "-z", "--", ...gitPathspecs], { cwd: root }).stdout.toString();
const observedInventory = raw
  .split("\0")
  .filter(Boolean)
  .map((path) => path.replace(/\/$/u, "").normalize("NFC"))
  .filter((path) => requestedPrefix === "" || path === requestedPrefix || path.startsWith(`${requestedPrefix}/`))
  .flatMap((path) => {
    try {
      return [{ path, directory: lstatSync(join(root, path)).isDirectory() }];
    } catch {
      return [];
    }
  });
...
const packageContext = (path: string): { packageRoot: boolean; ecosystemId?: string; parentDirectoryKindId?: string; siblingFixedFilenameContractIds?: readonly string[] } => {
  ...
  ).find(([manifest]) => basename(path) === manifest || existsSync(join(root, parent, manifest)))?.[1];
...
const statuteEntries = entries.map((entry) => {
  ...
  const fixed = entry.directory ? (fixedDirectoryIds.get(entry.path) ?? resolver.directoryIdsForPath(entry.path, context)).length > 0 : resolver.filenameIdsForPath(entry.path, context).length > 0;
