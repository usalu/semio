...
if (command === "test") {
  ...
  const rows = readFileSync(session, "utf8")
    .trim()
    .split("\n")
    .map((line) => JSON.parse(line));
  const recorded = rows.find((row) => row.timestamp === "2026-09-04T00:41:00.366Z")?.payload.item.stdout;
  const findings = [...recorded.matchAll(/\{\s*"kind":\s*"[^"]+",\s*"path":\s*"(?:[^"\\]|\\.)*"[^{}]*\}/gu)].flatMap((match) => {
    try {
  ...
  const oldByIdentity = indexed(original),
    currentByIdentity = indexed(current);
  ...
  for (const move of moves) {
    const name = basename(move.source),
      names = mappings.get(name) ?? new Set<string>();
    names.add(basename(move.destination));
    mappings.set(name, names);
  }
  const document = {
    baseline,
    audit,
    moves,
    conflicts,
    ambiguousNames: [...mappings].filter(([, names]) => names.size > 1).map(([name, names]) => ({ name, originals: [...names] })),
    pairs: [...mappings].filter(([, names]) => names.size === 1).map(([name, names]) => [name, [...names][0]]),
    inventory,
  };
  ...
  console.log(
    JSON.stringify(
      {
        moves: moves.length,
        recorded: moves.filter((move) => move.evidence.startsWith("recorded")).length,
        conflicts: conflicts.length,
        ambiguousNames: document.ambiguousNames,
        pairs: document.pairs.length,
        samples: moves.filter((move) => !move.evidence.startsWith("recorded")).slice(0, 25),
      },
      null,
      2,
    ),
  ...
  for (const path of inventory) {
    ...
    const repaired = reverse(text),
      updated = repaired.text,
      replacements = repaired.count;
    if (updated === text) continue;
    const after = Buffer.from(updated),
      beforeHash = digest(bytes);
  ...
  const tree = git("ls-tree", "-r", "-z", baseline)
    .split("\0")
    .filter(Boolean)
    .map((row) => {
      const tab = row.indexOf("\t");
      return { path: row.slice(tab + 1), hash: row.slice(0, tab).split(" ")[2]! };
    });
  const treeByIdentity = new Map<string, typeof tree>();
  for (const row of tree) {
    const key = identity(row.path),
      rows = treeByIdentity.get(key) ?? [];
    rows.push(row);
    treeByIdentity.set(key, rows);
  }
  ...
  for (let start = 0; start < candidates.length; start += 80) {
    ...
    for (const item of batch) {
      const eol = response.stdout.indexOf(10, offset),
        header = response.stdout.subarray(offset, eol).toString(),
        size = Number(header.split(" ")[2]);
      if (!Number.isFinite(size)) throw new Error(`Invalid historical blob header: ${header}`);
      const old = response.stdout.subarray(eol + 1, eol + 1 + size).toString("utf8");
      offset = eol + 1 + size + 1;
      const oldLiterals = new Map<string, Set<string>>();
      for (const literal of literals(old)) {
        const key = reverse(literal.raw).text,
          choices = oldLiterals.get(key) ?? new Set<string>();
        choices.add(literal.raw);
        oldLiterals.set(key, choices);
      }
      const bytes = readFileSync(item.path),
        current = bytes.toString("utf8"),
        replacements: { before: string; after: string; index: number }[] = [];
      ...
      const before = createHash("sha256").update(bytes).digest("hex"),
        after = createHash("sha256").update(updated).digest("hex");
  ...
  console.log(
    JSON.stringify({ files: edits.length, tokens: edits.reduce((sum, row) => sum + row.tokens.length, 0), unresolved: unresolved.length, samples: edits.slice(0, 10).map((row) => ({ path: row.path, tokens: row.tokens.slice(0, 3) })) }, null, 2),
  );
} else if (["handpicked-preview", "handpicked-apply", "reserved-preview", "reserved-apply"].includes(command)) {
  const reserved = command.startsWith("reserved-"),
    apply = command.endsWith("-apply");
  const groups = reserved ? [] : JSON.parse(readFileSync(join(import.meta.dir, "🖐️names.json"), "utf8")).groups;
  const allMoves: { source: string; destination: string }[] = reserved
    ? JSON.parse(readFileSync(join(import.meta.dir, "🔒️reserved.json"), "utf8")).moves
    : groups.flatMap((group: any) => group.names.map((name: any) => ({ source: posix.join(group.parent, name.from), destination: posix.join(group.parent, name.to) })));
    ...
    let updated = content.replace(/(["'`])([^"'`\r\n]+)\1/gu, (literal, quote: string, value: string, index: number) => {
      ...
      if (project(value) !== value) replacement = project(value);
        ...
        for (let base = dirname(path); ; base = dirname(base)) {
          const target = posix.normalize(posix.join(base, value)),
            next = project(target);
      ...
      if (replacement === undefined) {
        unresolved.push({ path, literal });
        return literal;
      }
      replacements.push({ before: value, after: replacement, index });
  ...
  if (apply)
    for (const move of [...moves].sort((a, b) => b.source.length - a.source.length)) {
      assert(!existsSync(move.destination), `Concurrent destination: ${move.destination}`);
      const before = lstatSync(move.source);
      renameSync(move.source, move.destination);
      assert.equal(lstatSync(move.destination).ino, before.ino);
    }
