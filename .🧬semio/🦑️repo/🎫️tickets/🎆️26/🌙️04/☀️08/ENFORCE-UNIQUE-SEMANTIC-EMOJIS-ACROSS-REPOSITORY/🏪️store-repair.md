# Store Handpicked Emoji Repair

Scope: `🧰️framework/🛍️products/💻️os/🔨️modules/🏪️store`. Inspected fixture structures, cases, laws, and source consumers before all twenty-four explicit moves. No fixture expected result, opcode, binary tag, or assertion was removed or regenerated.
...
Exact native includes and test mounts, fourteen mutation descriptor owner strings, five current authority roots, and root-script oracle inputs follow the physical moves. Five severity descriptor owners were already correct when re-read during the repair, so the pending stale hunk was not forced over concurrent correct contents. CAD's separate `🧪️retirement.json` source remains unchanged. The root Store retirement oracle previously selected a nonexistent `🧬️schema/🔣️.schema.json`; it now selects the actual titled retirement schema.

Scoped exact member lists register domain owners, presence rejection, owned-schema tests, fixture cases, and mutation names. The actual ancestor walk resolves all eighty-nine directories. Complete physical traversal includes all 217 authored entries and reports zero path-emoji statute findings; no authored subtree is excluded.

## Verification

The first native invocation incorrectly supplied `long` to the plain Cargo test route; it stopped on argument parsing and is not test evidence. The corrected existing Nx route compiled the entire kernel test binary and started store tests. Four assertion failures appeared: large-domain canonical sealing, both rejected-page grant cases, and restore-active-space nullable wire. A destructor panic then aborted the process in `amend_last_incremental_cache_survives_undo_redo_round_trip` with SIGABRT, preventing a complete test total. These failures remain visible and are not called passing.

Direct read-only calls of the existing source oracles reach unrelated exact source-guard failures: close-factory substitution/returned-read retirement, and canonical error-progress encoder/reader/sealer linkage. Their positive data parsing and schema checks do not establish overall source-oracle success. Assertions and source API behavior were preserved.
