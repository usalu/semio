...
## Resumed Model, Audio, Graph, and Image Repair

- Model's existing manifest expected `🔧️set-relation-applied`, but its physical fixture directory was still bare. That final directory now matches the manifest without changing either payload.
- Audio is `🔊️audio`. Its eight canonical applied-fixture directories use their mutation meanings (microphone, tag, mute, scissors, waveform, disc, sample-rate slider, and tag value); each pair is `⬅️before.json`/`➡️after.json`. The ten mutation-test fixture directories use those same meanings plus pause and snapshot. The recording fixture is `🏚️bauen-mit-bestand-ausschnitt`. The snapshot scenario uses `🎧️rerates-to-48-khz-and-rewrites-the-right-channel`, and its oracle declaration now resolves to that physical name.
- Graph is `🕸️graph`. Connectivity is `🕸️connectivity` and edge creation is `🌉️create-edge`, eliminating collisions with the GraphQL carrier. Eleven JSON Schema sidecars are `🧬️.schema.json`. Eight arbitrary animal/color scenario markers were replaced with their actual node, edge, port, weight, and movement meanings; all eleven scenario directory declarations were aligned with the exact physical names.
- Image is `🖼️image`. Pixel painting is `🖌️set-frame-pixels`; the thirteen mutation-test fixtures use their operation meanings. Eleven arbitrary scenario markers were replaced with frame delay, append, colorspace, ICC, author, width, grayscale, order, depth, comment, and black-pixel meanings. The twelve oracle scenario declarations now match their physical directories.
- Each repaired subset uses `🔮️oracle`, `☑️options` for both views, and exact source/shared Rust path patches. Snapshot sidecars are `🧬️.schema.json` and their descriptor/oracle payload-schema references agree. Reserved names remain literal.
- Focused pre-root-cutover audits: Audio 189 files / 136 directories / 325 governed; Graph 269 / 201 / 470; Image 288 / 272 / 560. Every one of the eight finding categories was zero in all three checks.
- The canonical Semio fixture verifier at the Audio/Graph checkpoint reports 272 fixtures and zero file problems (exit 0). Further full-scope verification follows after the remaining subsets are repaired.

## Final Semio Semantic Review

- Remaining subset roots are individually meaningful and sibling-unique: `🔤️text`, `📊️table`, `📦️object`, `🧰️kit`, `🎬️video`, and `🔢️value`. Each uses `🔮️oracle` and two distinct `☑️options` directories. Text, Table, Object, and Kit received 7, 8, 9, and 15 schema-sidecar identities respectively. Video and Value received seven canonical operation fixture pairs each; all canonical pairs use `⬅️before.json` and `➡️after.json`.
- Value's flat mutation corpus was reviewed as fifteen independent siblings. Mutation leaves use the corresponding action (➕, ➖, ✖, ✂, 🗝, 🧷, 🔁, ⏸); expected results use insertion, hole, cleanup, deletion, key, pin, and typed-value meanings (📥, 🕳, 🧹, 🗑, 🔑, 📌, 🔢). Its real source document is cited at the extant `local://🔣️.json`; the absent historical JSON artifact path is no longer presented as an existing source.
- The semantic pass inspected every mutation scenario and mutation-test corpus directory, beyond the structural audit. It replaced unrelated flowers, animals, and colored markers in the previously repaired Mesh, B-rep, Drawing, Base, CAD, and Model trees. Mesh's material/texture/geometry scenarios now use metal, blue paint, material, image, byte carrier, quad, topology, primitive, mesh, texture, and roughness meanings. B-rep's revised cases identify arc replacement, apex creation, vertex movement, face creation, diagonal edge, cylinder surface, and solid creation. Drawing's revised cases identify layering, hierarchy restoration, ungrouping, movement, scaling, stroke width, ordering, rotation, dragging, triangle path, grouping, and flattening.
- Test-suite directories now identify the subset they test rather than using unrelated clover, lemon, pear, orange, frog, kite, sunflower, colored squares, or anchor markers. Real named source fixtures use building/forest identities. All brown-circle snapshot owners and fixture directories were removed. Document image insertion is `🖼️insert-image`, image bytes are `📀️set-image-bytes`, and style removal is `🧽️remove-style`; these remain distinct from the snapshot camera. CAD block-layer assignment is `🧷️set-block-entity-layer`. Presentation removal uses `🧹️remove-layout`, `🚫️remove-master`, and `📤️remove-slide`, distinguishing both specification fixtures and real-deck fixture siblings.
- The 233 mutation descriptors were independently compared with their physical owner names. Sixteen stale declared emoji values were corrected through explicit field edits, including old generic document/test markers and pre-cutover mutation identities, so descriptor-driven consumers agree with the chosen names.
- All 119 oracle catalog scenario directory declarations resolve to the physical mutation scenario directory. All 795 Semio mounts in the shared Stdio Rust barrel resolve. All 1,314 literal Rust `include_str!` / `include_bytes!` dependencies in Semio resolve. All 1,770 Semio JSON files parse.
- The final whole-Semio naming audit covers 5,591 files, 3,732 directories, and 9,257 governed nodes; all eight finding categories are zero. The final canonical fixture verifier reports 286 fixtures and zero file problems (exit 0).
- Exact source, generator, fixture-manifest, and documentation references were corrected, including cross-subset references missed by prior tracked-only searches. The semantic fixture IDs and payload contents remain stable; no automatic emoji selection or migration implementation was added.

## Final Live-Reference and Runtime Checkpoint

This section supersedes earlier pending checkpoints and broad statements about unchanged payload contents. It distinguishes repaired naming references from the remaining source-contract limitations; this is not an all-runtime-tests-pass report.

Production `discoverTestCases`, `parseFeature`, `fixtureUrisIn`, and `resolveFixtures` initially found 546 missing references among 628 unique-per-case declared URIs. Exact feature fields now reference the already handpicked physical names. Explicit fixture columns retain logical scenario IDs, and Presentation's distinct real-talk and specification corpora remain separate. The final check resolves all 625 unique-per-case URIs across 19 cases, with zero missing references and zero parse errors. The count drops by three because corrected repeated no-mutation references deduplicate, not because scenarios were removed. Another agent independently repeated this production-API check with the same 19 / 625 / zero result.

The bounded runtime pass also repaired exact adapter carrier constants, mutation-role URI selection, and Mesh's incorrect relative shared-test import. Mutation readers now use the declared current scenario URI rather than constructing an obsolete generic coordinate. No compatibility aliases or filesystem fallback paths were introduced. Two Presentation insert-slide mutation payloads contained the polluted property `🎞️slide`; only that property was restored to the schema/handler's literal `slide`. No numeric data, pinned hashes, expected projections, or mutation semantics were rebaselined.

The existing fixture verification target, `bun nx run @semio-tech/repo-test-domain:test-fixture-verify -- --artifact s.stdio.semio`, passes with 286 fixtures and zero file problems after these edits. The bounded naming audit remains 5,591 files, 3,732 directories, and 9,257 governed entries, with all eight categories zero.

The committed Python host executed all 640 planned oracle scenarios in the 17 cases with Python adapters: 610 passed, six failed, and 24 errored. B-rep, CAD, Document, Graph, Kit, Object, Table, and Text pass in full. There are no remaining missing-file or undeclared-fixture errors in these final runs. Mesh's existing TypeScript oracle, including its independent Three.js geometry checks, separately executed and passed all 52 scenarios through the committed host. All 19 Rust adapters pass a parser-only `rustc -Zunpretty=ast-tree` check; this does not establish typechecking or native execution. Base has no Python adapter and was not executed natively.

### Remaining Source-Contract Limits

- Animation, Audio, Flow, Image, Model, Presentation, Value, and Video each lack oracle registrations for `no-mutation-baseline-mutate` and `no-mutation-baseline-inverse`: 16 errors total. Those registrations were not invented or skipped to make the result green.
- Drawing has two `group` key errors, six failed hierarchy assertions that group nodes carry no children, and six unregistered specification-vector oracle scenarios (flatten, group, rotate, scale, unflatten, ungroup). Its 38 passing scenarios are reported separately from those six failures and eight errors. The inspected group payloads use the literal `group`, not an emoji-polluted runtime token; no unrelated hierarchy behavior was changed.
- The 12 absent Animation payload schemas recorded before its cutover remain absent on the final direct filesystem check: every mutation descriptor still declares `🔣️.schema.json`, while each owner contains only its `🔣️.json` descriptor. These are absent source assets, not a renamed schema available at another coordinate. The fixture-file verifier does not cover this contract debt; no schemas were fabricated and no validation was weakened.

All bounded Semio verification processes have completed. Historical red results remain useful evidence but are superseded by the final counts above; temporary diagnostics are ready for the parent agent's ticket-scoped recoverable cleanup.
