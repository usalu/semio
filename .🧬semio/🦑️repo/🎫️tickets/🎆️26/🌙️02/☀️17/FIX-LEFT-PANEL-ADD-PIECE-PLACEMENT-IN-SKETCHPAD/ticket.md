...
- Rewire workbench row actions to add type/design pieces from left sidepanel. ✅️
- Keep child creation available without removing functionality. ✅️
- Add/adjust Design test assertions for `+` piece placement. ✅️
- Run sketchpad e2e tests and report result. ✅️

## Plan

1. Update `Design.tsx` workbench tree item actions to call `addPiece` for primary row action. ✅️
2. Preserve child creation as secondary action. ✅️
3. Extend `sketchpad.test.ts` Design test to click left workbench type row `+` and assert node count increments. ✅️
4. Run tests and close ticket with changed files. ✅️
