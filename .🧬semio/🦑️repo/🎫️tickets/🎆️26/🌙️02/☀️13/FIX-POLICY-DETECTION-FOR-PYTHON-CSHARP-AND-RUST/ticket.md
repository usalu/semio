...
- `repo/cli/main.go`: Python `sectionStart`/`sectionEnd`/`policySectionStart`/`policySectionEnd` regexes now use `#?` to match both `# region` and `# #region` formats. Updated `sectionStartFmt`/`sectionEndFmt`/`sectionBothFmt` to `# #region 🔖️%s` format.
- `repo/cli/main.go`: CSharp `sectionStart`/`sectionEnd`/`policySectionStart`/`policySectionEnd` regexes now use `(?://\s*)?` to match both `#region` and `// #region` formats. Updated `sectionStartFmt`/`sectionEndFmt`/`sectionBothFmt` to `// #region 🔖️%s` format.
...
- Test content uses: `# #region 🔖️Functions`
...
- Test content uses: `// #region 🔖️Functions` (comment-based format)
