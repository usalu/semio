...
- Enhanced `TypeScriptLanguage.ScanComments` to:
  ...
  - Correctly report the column position for all comment breachs.
...
- Refactored `Breach` struct and `CreateBreach` to support `Column` tracking across 26 callsites.
...
- [x] Add `InTodoBlock` to `CommentScanState` in `repo/cli/main.go`
- [x] Update `TypeScriptLanguage.ScanComments` to handle TODO blocks and ignore config files
- [x] Update `CreateBreach` to include `column` (or set it manually)
...
2. Update `TypeScriptLanguage.ScanComments`:
   ...
   - Record `Column` for comment breachs.
3. Update `applyAutofixes`:
   - For `BreachCodeCommentInline`, if `v.Column > 1`, only remove the comment portion of the line.
