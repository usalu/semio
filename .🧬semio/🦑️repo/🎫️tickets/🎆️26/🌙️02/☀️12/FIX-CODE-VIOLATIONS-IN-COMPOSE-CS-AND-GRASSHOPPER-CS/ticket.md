...
Fixed all code breachs in compose/net/Compose/Compose.cs (47→0) and compose/gh/Compose.Grasshopper/Compose.Grasshopper.cs (85→0). Added Imports/Namespace section wrappers for orphan definitions, RFC2119-keyword section summaries for 70 sections, and definition summary+spec comments for 29 class definitions across both files.

## Changes

### compose/net/Compose/Compose.cs

- Added `#region 🔖️Imports` / `#endregion 🔖️Imports` around using statements (orphan fix)
- Added `#region 🔖️Namespace` / `#endregion 🔖️Namespace` around namespace declaration (orphan fix)
- Added RFC2119-keyword section summaries to 39 sections (Constants, Utility, Expressions, Entitying, ComposeValidation, Attribute, Coord, Point, Vector, Plane, Location, Author, File, Folder, Benchmark, QualityKind, Quality, Tag, Concept, Port, Prop, Model, Connector, Type, Layer, Group, Piece, Side, Connection, Stat, Design, Kit, Design Family Helpers, Type Family Helpers, Api, ZipRoundtrip, KitImporter, KitExporter, ComposeDiff)
...
- Ran analyze on both files to identify breachs
- Created Python script to automate section summary, orphan wrapping, and definition comment insertion
- Initial pass fixed 39+3 breachs in Compose.cs and 31+26 in Grasshopper.cs
...
- Verified 0 breachs on both files
- Verified Compose.cs builds successfully (0 errors)
...
1. Run analyze to identify all breachs
...
6. Verify 0 breachs and successful build
