...
def fix_file(filepath, section_summaries, definition_summaries):
    ...
    for line_num_1based, summary in section_summaries.items():
        ...
        if idx < len(lines):
            line = lines[idx]
            if "#region" in line and "🔖️" in line:
...
def fix_design():
    ...
    orphan_line = 7839
    idx = orphan_line - 1
    insertions.append((idx, "// #region 🔖️Windows\n"))
    insertions.append(
        (
            idx,
    ...
    for i in range(len(lines) - 1, -1, -1):
        if "// #endregion Components" in lines[i]:
            for j in range(i + 1, min(i + 5, len(lines))):
                if "// #region App" in lines[j] or "// #region 🔖️App" in lines[j]:
                    lines.insert(i, "// #endregion 🔖️Windows\n\n")
                    break
            break
