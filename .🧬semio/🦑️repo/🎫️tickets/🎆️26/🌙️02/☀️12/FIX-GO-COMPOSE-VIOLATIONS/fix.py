...
filepath = "/workspaces/semio/compose/go/compose.go"
...
# Line 30 is `package compose`, line 32 starts `import (`
# Need to insert `// #region 🔖️Imports` before line 30 and `// #endregion 🔖️Imports` after the import block
# The import block ends at line 41 `)`, then line 42 is blank
# Actually, let me find the exact lines
for i, line in enumerate(lines):
    if line.strip() == "package compose":
...
for i in range(len(lines)):
    if lines[i].strip() == "package compose":
        pkg_line_idx = i
...
add_insert(
    pkg_line_idx,
    "// #region 🔖️Imports\n// Imports MUST include all required packages for the compose domain library.\n\n",
)
add_insert(import_close + 1, "\n// #endregion 🔖️Imports\n")

# === 2. Section summaries ===
section_summaries = {
    "Constants": "Constants MUST define shared constant values for the compose domain.",
    "Utils": "Utils MUST provide general-purpose utility functions for the compose domain.",
    "Entity IDs": "Entity IDs MUST define identifier types for all compose domain entities.",
...
for i, line in enumerate(lines):
    m = re.match(r"^// #region 🔖️(.+)$", line.strip())
    if m:
...
def_summaries = {
    ...
    # Validation types
    "type ComposeEntityKind": (
        "ComposeEntityKind enumerates the kinds of compose domain entities.",
    ),
