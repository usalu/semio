...
Added `assembleSplitInitialKitFromDirectory` / `readInitialKitFixtureFromPath` in `compose/fixtures/script.ts` and matching loaders in C# (`Utility.ReadKitFixtureJson`), Python (`_test_load_json`), Go tests, sketchpad Vite fixture middleware, and `@semio-tech/assets` bootstrap.
