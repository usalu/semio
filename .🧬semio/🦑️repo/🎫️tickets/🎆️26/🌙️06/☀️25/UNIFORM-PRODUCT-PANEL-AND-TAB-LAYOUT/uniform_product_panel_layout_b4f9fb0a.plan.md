...
overview: "Standardize every product onto one shell layout: Workbench (Document default + Catalogue), Details (Inspection default), Settings (Mode default + App + General), and fill every Inspection tree with fully-wired editable input controls bound to the live selection."
todos:
 - id: ticket
 ...
 - id: foundation-playground
   content: Fix PlaygroundView to merge settings-kind tabs + augmentPanelTabs.settings and wire App-tab display host; add canonical Document/Catalogue/Inspection tab constants+icons
   status: completed
 - id: puzzle3d
 ...
 - id: puzzle5d
   ...
   status: completed
 - id: flow
   content: "Flow: add controller selection + canvas bridge; Document + Catalogue(Kinds+Extensions) + editable Inspection"
   status: completed
 - id: procedural
   content: "Procedural: Document + Catalogue + editable Inspection using existing selection"
   status: completed
 - id: dag-map-shooting
   content: "DAG, Map, Shooting: build Document + Catalogue + editable Inspection (shot/asset, layer/feature, node)"
   status: completed
 - id: sketchpad
   content: "Sketchpad: replace text-stack details with editable Inspection bound to routeSelection; ensure Document + Catalogue"
   status: completed
 - id: coda
...
Standardize all products onto one shell: Workbench [Document (default), Catalogue], Details [Inspection (default)], Settings [Mode (default), App, General], with fully two-way editable Inspection controls per product.
...
Shared tab identity: add canonical tab id/icon/label constants (Document, Catalogue, Inspection) so every product references the same names/icons via `shellTabIconComponent`.

## Phase 2 - Rename + restructure products already on the shell

For each: Workbench = Document (default) + Catalogue; Details = single Inspection tab; drop per-product Settings tabs (content moves to App).
...
## Phase 3 - Build missing Document / Catalogue / Inspection
...
- Flow [flow/play/index.ts](flow/play/index.ts): add `selectedNodeIds`+revision to `FlowPlayController`, bridge `FlowCanvas.onSelectionChange/selectedNodeIds` in `FlowPlayPaneSurfaceHost`; add Document (widgets/synapses from `fixtureJson`), fold Kinds+Extensions into Catalogue sections, add editable Inspection.
- Procedural [procedural/play/index.ts](procedural/play/index.ts): controller already has selection; add Document + Catalogue (Kinds+Extensions) + editable Inspection.
...
[compose/client/lib/sketchpad/js/index.ts](compose/client/lib/sketchpad/js/index.ts): replace the static `SketchpadDetailsPanel` text-stack (~14348-14466) with an editable Inspection panel bound to `routeSelection` (pieces/connections) dispatching compose-react patch commands; ensure Document + Catalogue workbench tabs. Settings already flow through `PlatformView`.
...
- Workbench: Document = project/run/iteration tree (or existing `OntologyTree`/`ValidationTree`); Catalogue = frameworks/properties/platforms templates.
