#!/usr/bin/env bun
/** @emoji 🧪️ Smoke-check Eingabeprozess Eingabearten slide deck wiring. */
import { PRESENTATION_DEFAULT_SLIDE_ASPECT, collectPresentationSlides, loadPresentationFromSlideGlob } from "@semio-tech/framework-presentation-core";
...
const slideModules = import.meta.glob<{ default: unknown }>("../../../mit-bestand/präsentation/33.projektetage/slide/**/*.ts", { eager: true, root: projektetageRoot });
...
expect(systematik?.thoughts.map((thought) => thought.name)).toEqual(["Eingabeprozess", "Konnektivität", "Typologien"]);
...
const bookmark = collectPresentationSlides(deck).find((slide) => slide.thought === "Eingabeprozess" && slide.slide === "Eingabearten");
