...
todos:
 - id: ticket
   ...
   status: completed
 - id: rust-union
   content: Add canonical structured Icon enum (serde tag=kind) + encode_icon/decode_icon string codec in infinite/canvas/vello/icon_codec.rs; route board_resolve_icon_kind via decode
   status: completed
 - id: rust-newkinds
...
### 2. Canonical Rust Icon + codec — `[infinite/canvas/vello/icon_codec.rs](infinite/canvas/vello/icon_codec.rs)`
...
### 3. Extend Rust resolution (new kinds) — same file + `[lib.rs](infinite/canvas/vello/lib.rs)`

- `text:` -> render via Typst with the default text font (new `board_typst_text_to_svg`, distinct from the emoji path), centered.
- `shortcode` `:code:` -> map to emoji via a shortcode->emoji table, then reuse the emoji Typst path. Generate the table at build time in `[infinite/canvas/vello](infinite/canvas/vello)` `build.rs` from a committed JSON asset under `infinite/canvas/vello/assets/` (no runtime external dep).
...
- Extend the `Icon` component to render every kind: catalog->vendored `ICONS`, svg->inline, url/data->, emoji/shortcode-> with Noto Color Emoji, text->, typst->lazy Typst-wasm render helper exported from `infinite_canvas` (fallback to text). Export `encodeIcon`/`decodeIcon` from the TS side as the single source of truth.
...
- Keep `icon: Option<String>` columns but define them as the canonical icon-string grammar; add `infinite_canvas` dependency where needed and resolve via `decode_icon`/`board_resolve_icon_kind` when rendered on a canvas. No GraphQL schema change.

### 8. Wire/refactor remaining canvases

- Confirm `dag`/`flow`/`gis`/`reasoning` (all depend on `infinite_canvas`) route node icons through `board_resolve_icon_kind`; add `iconKind` support where a canvas only had text/media today, reusing the shared resolver.
