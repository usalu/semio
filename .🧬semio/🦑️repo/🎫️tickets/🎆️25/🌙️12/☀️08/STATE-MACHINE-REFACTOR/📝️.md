...
1. ✅️ `useDesignAppCommands` routes app state operations through XState `actor.send()`
2. ✅️ `useTypeAppCommands` already uses XState for all operations
3. ✅️ All triadic hooks use `actor.can()` for `canSetState`
4. ✅️ `useSelector` from `@xstate/react` added to Design.tsx
5. ✅️ Example component `ToolsToggleGroup` refactored to use only triadic hooks
