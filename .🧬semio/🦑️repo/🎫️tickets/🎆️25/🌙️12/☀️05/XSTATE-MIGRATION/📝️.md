...
Add to `js/compose/package.json`:
...
│️   ├️─️ viewing
│️   └️─️ editing
│️       ├️─️ transaction.active
│️       └️─️ transaction.idle
└️─️ error
```
...
✅️ Generate a **PDF** / **Notion-friendly** version
✅️ Turn this into an **interactive checklist**
✅️ Create **diagrams** (statecharts, actor document, migration flow)
✅️ Produce an **engineering roadmap with timeline & team assignments**
...
- `js/compose/package.json` - Added xstate@^5.20.0, @xstate/react@^6.0.0
- `js/compose/sketchpad/machines.ts` - ~1900 lines of XState machines
- `js/compose/sketchpad/shared.ts` - XState types and Y.js bridge utilities
- `js/compose/sketchpad/Sketchpad.tsx` - XState actor integration and hooks
...
- `js/compose/sketchpad/machines.ts` - All app machines refactored
- `js/compose/sketchpad/Design.tsx` - Hooks use stores, actor disabled
...
Y.js Store (source of truth)
    │️
    ├️─️►️ useDesignApp hook (watch changes)
    │️       │️
    │️       └️─️►️ DESIGN.SYNC event ─️►️ XState Machine
    │️                                     │️
    │️                                     └️─️►️ Context updated
    │️
    └️─️►️ useDesignAppCommands ─️►️ store.execute() ─️►️ Y.js mutation
```
