...
Three separate policies exist: `header-region`, `empty-region`, `inline-comment`. Each produces a single breach type.

# Plan

Generalize policies into scope-based groups that produce multiple related breachs:

1. **header** policy (replaces header-region):
   - `header:missing-region` - No #region 🔖️Header found
...
2. **region** policy (replaces empty-region):
   ...
   - `region:missing-start-name` - #region 🔖️without name
   - `region:missing-end-name` - #endregion 🔖️without name
...
Statutes use `:` separator (e.g., `header:missing-license`).
