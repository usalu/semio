...
2. ✅️ Store diagram ref using `setDropZoneRef` callback
3. ✅️ Check pointer coordinates against diagram bounds in `handleDragEnd`
4. ✅️ Create piece at pointer position when drop is within bounds
5. ✅️ Clear `activeDraggedType`/`activeDraggedDesign` after drop
6. ✅️ Repeat similar pattern for DesignAppScene (3D scene)
7. ✅️ For scene: use normalized screen coordinates to calculate world position on y=0 plane
8. ✅️ Remove debug logs
