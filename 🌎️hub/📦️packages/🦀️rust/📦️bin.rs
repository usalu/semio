...
use directory::os_directory::{
    ...
    AdminPageV1, AdminRecordedConnectionV1, ConnectionView, DirectoryActor, DirectoryActorKind, DirectoryCommand, DirectoryConnectionPhase, DirectoryEvent, DirectoryPresenceActor, DirectoryReadModel, DirectorySpaceRole, DirectorySpaceVisibility,
    DirectoryStreamMessage, DocumentDescriptor, DocumentOpenArtifactV1, DocumentOpenCatalogV1, DocumentOpenCheckpointV1, DocumentOpenGrantV1, DocumentOpenIntentV1, DocumentOpenPackageV1, DocumentOpenPlanErrorCodeV1, DocumentOpenPlanErrorV1, DocumentOpenPlanV1, DocumentOpenRevalidationV1,
    DocumentPlanSocketGrantIntentV1,
    DocumentOpenSurfaceV1, DocumentView, InviteView, MemberView, SpaceView, DOCUMENT_OPEN_PLAN_MAX_TTL_MS,
};
...
use semio_hub::artifact_authority::trusted_catalog::{NativeCodecBinding, TrustedCatalogLoader, VerifiedDocumentOpenSelectionV1, VerifiedTrustedCatalog};
#[cfg(test)]
...
use semio_hub::directory::model::{AdminOperationAuditRecord, AuthSessionKind, DocumentScope, InviteRecord, NewAdminOperationAuditRecord, SocketSessionBindingStatus, SocketShareBindingStatus, SpaceRole, SyncSessionRecord};
#[cfg(feature = "sqlite")]
...
use semio_hub::directory::{
    ArtifactCasSweepContinuation, ArtifactCasSweepRequest, ArtifactCasSweepResult, CommandResult, DirectoryService, HubDirectories, HubDirectory, HubVerifiedCheckpointPublisher, ProjectionRebuildControl, ProjectionRebuildProgress,
...
async fn configured_artifact_authority(bundle_path: Option<std::path::PathBuf>, profile: Option<String>, bindings: &[NativeCodecBinding]) -> Result<Option<ConfiguredArtifactAuthority>, AuthorityError> {
...
impl SocketGrantLedgerV1 {
    ...
    fn register_live(&self, record: &SocketGrantRecordV1) -> Result<(String, Arc<tokio::sync::Notify>), SocketGrantLedgerErrorV1> {
        ...
        if stored.state != SocketGrantStateV1::Consumed
    ...
    fn is_live(&self, record: &SocketGrantRecordV1, live_id: &str) -> bool {
        ...
        inner.records.get(&record.selector).is_some_and(|stored| {
            stored.state == SocketGrantStateV1::Consumed
                ...
                && stored.subject == record.subject
                && stored.document_plan == record.document_plan
                && inner.live_by_binding.get(&record.subject.binding()).and_then(|live| live.get(live_id)).is_some_and(|(selector, _)| selector == &record.selector)
        })
    }
    ...
    fn invalidate_binding(&self, binding: SocketBindingKeyV1) {
        let notifiers = {
            ...
            let invalidated = inner.records.iter().filter_map(|(selector, record)| (record.subject.binding() == binding).then(|| selector.clone())).collect::<Vec<_>>();
...
impl DocumentOpenPlanLedgerV1 {
    ...
    fn authority_for_authenticated_exchange(
        &self,
        receipt: &str,
        scope: &DocumentScope,
        subject: &SocketSubjectV1,
        now_ms: u64,
    ) -> Result<DocumentOpenPlanAuthorityV1, DocumentOpenPlanErrorCodeV1> {
    ...
    fn exchange_to_socket_grant(
        &self,
        receipt: &str,
        current: &DocumentOpenPlanAuthorityV1,
        now_ms: u64,
        socket_grants: &SocketGrantLedgerV1,
    ) -> Result<SocketGrantReceiptV1, DocumentOpenPlanErrorCodeV1> {
        self.exchange_record(receipt, current, now_ms, |authority, plan_expires_at_ms| {
            ...
            let expires_at_ms = issued_at_ms
                .checked_add(SOCKET_GRANT_TTL_MS)
                .ok_or(DocumentOpenPlanErrorCodeV1::DeadlineExceeded)?
                .min(plan_expires_at_ms)
                .min(binding_expires_at_ms);
            ...
            socket_grants
                .issue_with_document_plan(&capability, audience, authority.server_actor_id.clone(), authority.subject.clone(), issued_at_ms, expires_at_ms, Some(Arc::new(authority.clone())))
            ...
            let response = SocketGrantReceiptV1 {
                schema: "semio.hub.socket-grant/v1",
                protocol: SOCKET_PROTOCOL_V1,
                grant: capability.expose_once(),
                actor_id: authority.server_actor_id.clone(),
                expires_at_ms,
            };
...
async fn socket_live_authority(state: &HubState, record: &SocketGrantRecordV1, live_id: &str) -> Result<Vec<tokio::sync::OwnedMutexGuard<()>>, SocketBindingValidityV1> {
    let admission = tokio::time::timeout(std::time::Duration::from_secs(2), state.socket_binding_gates.acquire_subject(&record.subject)).await.map_err(|_| SocketBindingValidityV1::Unavailable)?;
    let validity = tokio::time::timeout(std::time::Duration::from_secs(2), record.subject.revalidate(state.directory.as_ref(), &record.audience, now_ms())).await.unwrap_or(SocketBindingValidityV1::Unavailable);
...
async fn issue_socket_grant(state: &HubState, subject: SocketSubjectV1, audience: SocketAudienceV1, stable_actor_material: Option<[u8; 32]>) -> Result<Json<SocketGrantReceiptV1>, StatusCode> {
    let binding = subject.binding();
    let _admission = tokio::time::timeout(std::time::Duration::from_secs(2), state.socket_binding_gates.acquire_subject(&subject)).await.map_err(|_| StatusCode::SERVICE_UNAVAILABLE)?;
...
async fn authenticate_document_socket_subject(
    state: &HubState,
    scope: &DocumentScope,
    headers: &HeaderMap,
) -> Result<(SocketSubjectV1, Option<[u8; 32]>), DocumentOpenPlanErrorCodeV1> {
...
async fn issue_document_open_plan_inner(
    space_id: String,
    document_id: String,
    headers: HeaderMap,
    state: HubState,
    ...
    let selected = catalog
        .resolve_document_open(&descriptor, intent.requested_surface_id.as_deref(), writable)
        .ok_or_else(|| document_open_plan_exchange_error(DocumentOpenPlanErrorCodeV1::ComponentUnavailable))?;
...
async fn issue_document_open_plan(
    ...
    tokio::time::timeout(std::time::Duration::from_millis(deadline_ms), async move {
        ...
        let body = axum::body::to_bytes(body, DOCUMENT_OPEN_PLAN_REQUEST_MAX_BYTES)
            .await
            .map_err(|_| document_open_plan_route_error(StatusCode::PAYLOAD_TOO_LARGE, DocumentOpenPlanErrorCodeV1::Denied))?;
...
async fn issue_document_plan_socket_grant_inner(
    space_id: String,
    document_id: String,
    headers: HeaderMap,
    state: HubState,
    ...
    let authority = state
        .document_open_plans
        .authority_for_authenticated_exchange(&intent.plan_receipt, &scope, &subject, exchange_at_ms)
        .map_err(document_open_plan_exchange_error)?;
    let descriptor = state
        .directory
        .get_document_descriptor(&scope)
        .await
    ...
    if catalog.generation_id() != authority.catalog.generation_id
        || catalog.resolve_document_open(&authority.descriptor, Some(&authority.surface.surface_id), authority.grant.write).is_none_or(|selected| {
            selected.package != authority.package || selected.artifact != authority.artifact || selected.surface != authority.surface || selected.grant != authority.grant
        })
    {
        return Err(document_open_plan_exchange_error(DocumentOpenPlanErrorCodeV1::Stale));
    }
    ...
    state
        .document_open_plans
        .exchange_to_socket_grant(&intent.plan_receipt, &authority, exchange_at_ms, state.socket_grants.as_ref())
        .map(Json)
        .map_err(document_open_plan_exchange_error)
}
...
async fn issue_document_plan_socket_grant(
    ...
    tokio::time::timeout(
        ...
        async move {
            let (parts, body) = request.into_parts();
            let body = axum::body::to_bytes(body, DOCUMENT_OPEN_PLAN_EXCHANGE_REQUEST_MAX_BYTES)
                .await
                .map_err(|_| document_open_plan_route_error(StatusCode::PAYLOAD_TOO_LARGE, DocumentOpenPlanErrorCodeV1::Denied))?;
...
#[cfg(test)]
async fn issue_document_socket_grant_fixture(
    Path((space_id, document_id)): Path<(String, String)>,
    headers: HeaderMap,
    State(state): State<HubState>,
...
async fn document_plan_socket_validity(state: &HubState, record: &SocketGrantRecordV1, surface: Option<&str>) -> SocketBindingValidityV1 {
    ...
    if record.subject != authority.subject
        || record.actor_id != authority.server_actor_id
        || record.audience != SocketAudienceV1::Document(authority.scope.clone())
        || surface.is_some_and(|surface| surface != authority.surface.surface_id)
    {
        return SocketBindingValidityV1::Unauthorized;
    }
    ...
    if catalog.generation_id() != authority.catalog.generation_id
        || catalog.resolve_document_open(&descriptor, Some(&authority.surface.surface_id), authority.grant.write).is_none_or(|selection| {
            selection.package != authority.package || selection.artifact != authority.artifact || selection.surface != authority.surface || selection.grant != authority.grant
        })
    {
        return SocketBindingValidityV1::Unauthorized;
    }
...
async fn consume_socket_grant(state: &HubState, headers: &HeaderMap, audience: SocketAudienceV1, surface: Option<&str>) -> Result<SocketGrantAdmissionV1, StatusCode> {
    ...
    let validity = tokio::time::timeout(std::time::Duration::from_secs(2), candidate.subject.revalidate(state.directory.as_ref(), &candidate.audience, now_ms())).await.unwrap_or(SocketBindingValidityV1::Unavailable);
    match validity {
...
async fn handle_ws(socket: WebSocket, space_id: String, document_id: String, surface: String, state: HubState, socket_admission: SocketGrantAdmissionV1) {
    ...
    let socket_binding_gates = match tokio::time::timeout(std::time::Duration::from_secs(2), state.socket_binding_gates.acquire_subject(&socket_grant.subject)).await {
        Ok(admission) => admission,
        Err(_) => {
                    ...
                    Some(Ok(Message::Binary(bytes))) => {
                        let Ok((_lane, frame)) = decode_client_frame(&bytes).await else {
                            ...
                            match tokio::time::timeout(
                                std::time::Duration::from_secs(2),
                                handle_client_frame(&state, &handle, &db_id, &key, &space_id, &document_id, &fanout, &actor, &gate, &principal, &tenant, frame, &mut sender),
                            )
                            .await
                            {
                                Ok(true) => {}
...
async fn send_socket_directory_message(sender: &mut SplitSink<WebSocket, Message>, state: &HubState, record: &SocketGrantRecordV1, live_id: &str, message: &DirectoryStreamMessage) -> SocketBindingValidityV1 {
    let _admission = match socket_live_authority(state, record, live_id).await {
        Ok(admission) => admission,
        Err(validity) => return validity,
    };
    let validity = tokio::time::timeout(std::time::Duration::from_secs(2), socket_directory_message_visible(state, record, message)).await.unwrap_or(SocketBindingValidityV1::Unavailable);
    if validity != SocketBindingValidityV1::Active {
        return validity;
    }
...
async fn execute_admin_intent(state: &HubState, principal: &AdminPrincipalV1, intent: AdminIntentV1, operation_runtime: Option<Arc<AdminOperationRuntime>>) -> AdminIntentExecution {
    ...
    if let Some(command) = admin_directory_command(&intent) {
        ...
        return match state.directory_service.execute(principal.event_actor(), command).await {
...
mod tests {
    ...
    fn document_open_catalog_for_descriptor_with_generation(descriptor: &DocumentDescriptor, generation_id: String) -> Arc<dyn DocumentOpenCatalogAuthorityV1> {
        ...
        Arc::new(TestDocumentOpenCatalog { generation_id, open_targets: vec![
    ...
    async fn document_open_plan_authority_for_session(
        state: &HubState,
        fixture: &DocumentOpenPlanLedgerFixture,
        token: &str,
        scope: DocumentScope,
    ) -> DocumentOpenPlanAuthorityV1 {
    ...
    async fn issue_and_exchange_document_open_plan_for_test(
        state: &HubState,
        token: &str,
        scope: &DocumentScope,
        client_instance_id: &str,
    ) -> (DocumentOpenPlanV1, SocketGrantReceiptV1) {
        ...
        let DirectoryJson(plan) = match issue_document_open_plan_inner(
            scope.space_id.clone(),
            scope.document_id.clone(),
            headers.clone(),
            state.clone(),
            Bytes::from(directory::os_pack::json::to_json_string(&intent)),
        )
        .await {
            Ok(plan) => plan,
            Err(_) => panic!("issue document open plan"),
        };
        let exchange = DocumentPlanSocketGrantIntentV1 {
            schema: "semio.hub.document-plan-socket-grant-intent/v1".into(),
            version: 1,
            plan_receipt: plan.receipt.clone(),
        };
        let Json(grant) = match issue_document_plan_socket_grant_inner(
            scope.space_id.clone(),
            scope.document_id.clone(),
            headers,
            state.clone(),
            Bytes::from(directory::os_pack::json::to_json_string(&exchange)),
        )
        .await {
            Ok(grant) => grant,
    ...
    fn document_open_plan_receipt_exchange_mints_one_exact_bounded_socket_grant() {
        ...
        let public = plans
            .issue_with_capability(authority.clone(), fixture.now_ms, fixture.now_ms + 100, DocumentOpenPlanCapabilityV1::from_secret(document_open_plan_secret(30)))
            .expect("route-inaccessible plan fixture");
        ...
        let capacity_plan = capacity_plans
            .issue_with_capability(authority.clone(), fixture.now_ms, fixture.now_ms + 100, DocumentOpenPlanCapabilityV1::from_secret(document_open_plan_secret(31)))
        ...
        assert!(matches!(
            capacity_plans.exchange_to_socket_grant(&capacity_plan.receipt, &authority, fixture.now_ms + 1, &capacity_sockets),
    ...
    async fn document_open_plan_issue_route_is_catalog_bound_authenticated_bounded_cancel_safe_and_exchangeable() {
        ...
        let grant_body = |receipt: &str| {
            directory::os_pack::json::to_json_string(&DocumentPlanSocketGrantIntentV1 {
                schema: "semio.hub.document-plan-socket-grant-intent/v1".into(),
                version: 1,
                plan_receipt: receipt.into(),
            })
        };
        let addr = spawn_server(state.clone()).await;
        ...
        let oversized_request = axum::http::Request::builder()
            .uri(&plan_route)
        ...
        assert!(matches!(
            issue_document_open_plan(OriginalUri(plan_route.parse().expect("plan URI")), Path((STUDIO.into(), document_id.into())), State(state.clone()), oversized_request).await,
    ...
    async fn document_open_plan_exchange_route_is_authenticated_exact_hostile_and_single_use() {
        ...
        let public = state
            .document_open_plans
            .issue_with_capability(authority.clone(), now, now + 10_000, DocumentOpenPlanCapabilityV1::from_secret(document_open_plan_secret(40)))
            .expect("private issuer fixture");
        ...
        let bound = state
            .document_open_plans
            .issue_with_capability(authority.clone(), now + 1, now + 10_001, DocumentOpenPlanCapabilityV1::from_secret(document_open_plan_secret(41)))
            .expect("binding fixture plan");
        ...
        let scoped = state
            .document_open_plans
            .issue_with_capability(authority.clone(), now + 2, now + 10_002, DocumentOpenPlanCapabilityV1::from_secret(document_open_plan_secret(42)))
            .expect("scope fixture plan");
        ...
        let strict = state
            .document_open_plans
            .issue_with_capability(authority.clone(), now + 3, now + 10_003, DocumentOpenPlanCapabilityV1::from_secret(document_open_plan_secret(43)))
            .expect("strict request fixture plan");
        ...
        let query_plan = state
            .document_open_plans
            .issue_with_capability(authority.clone(), now + 4, now + 10_004, DocumentOpenPlanCapabilityV1::from_secret(document_open_plan_secret(44)))
            .expect("query fixture plan");
        ...
        let bounded = state
            .document_open_plans
            .issue_with_capability(authority.clone(), now + 5, now + 10_005, DocumentOpenPlanCapabilityV1::from_secret(document_open_plan_secret(45)))
            .expect("bounded request fixture plan");
        ...
        share_authority.subject = SocketSubjectV1::Share {
            share_id: issued_share.record.id,
            selector: issued_share.record.selector,
            scope: scope.clone(),
            expires_at_ms: issued_share.record.expires_at,
        };
        ...
        let share_plan = state
            .document_open_plans
            .issue_with_capability(share_authority.clone(), now + 6, now + 10_006, DocumentOpenPlanCapabilityV1::from_secret(document_open_plan_secret(46)))
            .expect("share fixture plan");
