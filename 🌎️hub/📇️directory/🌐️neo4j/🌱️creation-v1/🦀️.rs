...
struct GenesisFixture {
    directory: std::sync::Arc<Neo4jDirectory>,
...
async fn genesis_commit_ack_and_locked_expiry_are_atomic_neo4j() {
    ...
    expired.directory.graph.run(query("MATCH (a:AuthSession {id: $id}) SET a.expiresAt = $expires_at").param("id", expired.intent.actor.session_id.clone()).param("expires_at", expires_at)).await.unwrap();
    ...
    expired.directory.graph.run(query("MATCH (a:AuthSession {id: $id}) SET a.expiresAt = 0").param("id", expired.intent.actor.session_id.clone())).await.unwrap();
    ...
    println!("[DEBUG] Neo4j genesis: post-commit ACK loss=1 exact receipt/events=3 retry existing=1 lock-crossed expiry refusal=1 two-service terminalization=2 facts=3; no DirectoryService broadcast or real factory execution");
}
