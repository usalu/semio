...
use crate::artifact_authority::{ArtifactPair, checkpoint_id_encoding_v1, ArtifactBlobIntegrity, AuthorityLimits, AuthorityOperationControl, AuthorityProgress, ImmutableArtifactBlobStore, OperationContext};
...
impl GenesisFixture {
    ...
    fn packet(&self) -> DocumentGenesisAppendV1 {
        ...
        DocumentGenesisAppendV1 { intent: self.intent.clone(), checkpoint: self.checkpoint.clone(), reservation: self.reservation.clone(), now_ms: now_ms() as u64, events: [
            NewDirectoryEvent { hlc: clock.tick(), actor: author.clone(), space_id: Some(self.intent.scope.space_id.clone()), user_id: Some(self.intent.actor.user_id.clone()), body: DirectoryEventBody::DocumentAnnounced { descriptor: self.prepared.descriptor.clone() } },
            NewDirectoryEvent { hlc: clock.tick(), actor: author, space_id: Some(self.intent.scope.space_id.clone()), user_id: Some(self.intent.actor.user_id.clone()), body: DirectoryEventBody::DocumentIndexed { scope: self.intent.scope.clone(), descriptor_digest_v1: self.checkpoint.descriptor_digest_v1, entry: directory::os_directory::DocumentIndexEntryV1 { name: self.intent.request.name.clone(), dialect: self.intent.parent_dialect.clone() } } },
            NewDirectoryEvent { hlc: clock.tick(), actor: system(), space_id: Some(self.intent.scope.space_id.clone()), user_id: None, body: DirectoryEventBody::ArtifactCheckpointPublished { checkpoint: published_artifact_checkpoint(&self.checkpoint) } },
        ] }
    }

    fn snapshot(&self) -> Vec<Vec<Vec<String>>> {
        let conn = self.sqlite().lock().unwrap();
        ["hub_directory_event", "hub_document_descriptor", "hub_document_index", "hub_artifact_checkpoint", "hub_artifact_checkpoint_private", "hub_artifact_authority_journal", "hub_artifact_cas_ledger_head", "hub_artifact_cas_ledger_journal", "hub_artifact_cas_reservation", "hub_artifact_cas_reservation_object", "hub_artifact_cas_reference", "hub_artifact_cas_reference_object", "hub_artifact_creation_fact"].iter().map(|table| {
    ...
    async fn create_at(path: &str) -> Self {
        ...
        sqlite.append_artifact_creation_fact(&ArtifactCreationFactAppendV1 { actor: intent.actor.clone(), space_id: intent.scope.space_id.clone(), request_id: intent.request.request_id.clone(), command_sha256: intent.command_sha256.clone(), expected_revision: 1, recorded_at_ms: now_ms() as u64, body: ArtifactCreationFactBodyV1::Prepared { candidate: prepared.clone() } }).await.unwrap();
...
fn system() -> DirectoryActor { DirectoryActor { kind: DirectoryActorKind::System, id: "system:artifact-creation".into() } }

struct GenesisControl;

impl AuthorityOperationControl for GenesisControl {
    fn now_ms(&self) -> u64 { now_ms() as u64 }
...
async fn genesis_physical_pair_and_sqlite_restart_preserve_exact_receipt() {
    ...
    drop(blobs); drop(storage); drop(f);
    ...
    for stage in ["reopened", "rebuilt"] {
        if stage == "rebuilt" { sqlite.rebuild_projections().await.unwrap(); }
...
async fn genesis_accepted_only_recovery_has_no_prepared_or_public_side_effects() {
    ...
    for row in fixture["cases"].as_array().unwrap() {
        ...
        if kind == "revoked" { sqlite.revoke_auth_session(&intent.actor.session_id, "test", Some("seed"), "accepted-recovery").await.unwrap(); }
        if kind == "expired" {
        ...
        if !candidates.is_empty() { assert_eq!(candidates[0], intent); }
...
async fn genesis_neutral_transactions_are_atomic_replayable_and_authorized() {
    ...
    for row in fixture["cases"].as_array().unwrap() {
        ...
        let table = match kind { "fault-descriptor" => Some("hub_document_descriptor"), "fault-index" => Some("hub_document_index"), "fault-checkpoint" => Some("hub_artifact_checkpoint"), "fault-authority" => Some("hub_artifact_authority_journal"), "fault-cas-ledger" => Some("hub_artifact_cas_ledger_journal"), "fault-cas-reference" => Some("hub_artifact_cas_reference"), "fault-completion" => Some("hub_artifact_creation_fact"), _ => None };
        ...
        match kind {
            "revoked-session" => { f.sqlite().revoke_auth_session(&f.intent.actor.session_id, "test", Some("seed"), "genesis-revoke").await.unwrap(); }
            "expired-session" => { f.sqlite().lock().unwrap().execute("UPDATE hub_auth_session SET expires_at = 0 WHERE id = ?1", [&f.intent.actor.session_id]).unwrap(); }
            "changed-generation" => { f.sqlite().lock().unwrap().execute("UPDATE hub_auth_session SET authorization_generation = authorization_generation + 1 WHERE id = ?1", [&f.intent.actor.session_id]).unwrap(); }
            "archived-space" => { f.sqlite().lock().unwrap().execute("UPDATE hub_space SET kind = 'archive' WHERE id = 'default'", []).unwrap(); }
            "removed-member" => { f.sqlite().lock().unwrap().execute("DELETE FROM hub_space_membership WHERE space_id = 'default' AND user_id = 'seed'", []).unwrap(); }
            "spectator" => { f.sqlite().lock().unwrap().execute("UPDATE hub_space_membership SET role = 'spectator' WHERE space_id = 'default' AND user_id = 'seed'", []).unwrap(); }
            ...
            "wrong-index-name" => if let DirectoryEventBody::DocumentIndexed { entry, .. } = &mut packet.events[1].body { entry.name = "Other".into(); },
            ...
            "cancelled" => { f.sqlite().append_artifact_creation_fact(&ArtifactCreationFactAppendV1 { actor: f.intent.actor.clone(), space_id: f.intent.scope.space_id.clone(), request_id: f.intent.request.request_id.clone(), command_sha256: f.intent.command_sha256.clone(), expected_revision: 2, recorded_at_ms: now_ms() as u64, body: ArtifactCreationFactBodyV1::Cancelled }).await.unwrap(); }
...
async fn genesis_delivery_lease_serializes_invalidation_and_requires_replay() {
    ...
    std::future::poll_fn(|context| match invalidate.as_mut().poll(context) { Poll::Pending => Poll::Ready(()), Poll::Ready(()) => panic!("invalidation crossed an active send lease") }).await;
    assert_eq!(f.service.delivery_epoch(Some("default")), epoch);
    assert!(invalidations.try_recv().is_err());
    drop(lease);
    invalidate.await;
    ...
    for index in 0..crate::directory::DIRECTORY_DELIVERY_SCOPE_MAX { f.service.invalidate_delivery_locked(&clock, &format!("space-{index}")).await; }
