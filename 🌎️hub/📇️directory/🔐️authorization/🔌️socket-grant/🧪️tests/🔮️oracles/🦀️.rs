...
fn test_declarations(source: &str) -> Vec<String> {
    let file = syn::parse_file(source).expect("socket-grant source must parse through syn");
    let mut names = Vec::new();
    collect_test_declarations(&file.items, &mut names);
    names
}
...
fn collect_test_declarations(items: &[Item], names: &mut Vec<String>) {
    for item in items {
        match item {
            Item::Fn(function) if function.attrs.iter().any(test_attribute) => names.push(function.sig.ident.to_string()),
            Item::Mod(module) => {
                if let Some((_, nested)) = &module.content {
                    collect_test_declarations(nested, names);
                }
            }
            _ => {}
        }
