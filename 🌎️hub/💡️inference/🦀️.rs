...
pub enum InferenceErrorV1 { Invalid, Bounds, Denied, Conflict, Capacity, Expired, Cancelled, Storage }
...
impl InferenceOperationControlV1 {
    pub fn new(lifetime_ms: u64, work_limit: u64) -> Result<Self, InferenceErrorV1> {
        if lifetime_ms == 0 || lifetime_ms > schema::JOB_MAX_LIFETIME_MS || work_limit == 0 || work_limit > 65_536 { return Err(InferenceErrorV1::Bounds); }
    ...
    pub fn cancel(&self) { self.cancelled.store(true, Ordering::Release); self.interrupted.notify_waiters(); }

    pub fn checkpoint(&self, completed: u64) -> Result<(), InferenceErrorV1> {
        if self.cancelled.load(Ordering::Acquire) { return Err(InferenceErrorV1::Cancelled); }
        if Instant::now() >= self.deadline { return Err(InferenceErrorV1::Expired); }
        if completed > self.work_limit { return Err(InferenceErrorV1::Bounds); }
        self.progress.fetch_max(completed, Ordering::AcqRel);
        Ok(())
    }

    pub fn progress(&self) -> (u64, u64) { (self.progress.load(Ordering::Acquire), self.work_limit) }

    pub(super) async fn interruption(&self) -> InferenceErrorV1 {
        let notified = self.interrupted.notified();
        tokio::pin!(notified);
        notified.as_mut().enable();
        if let Err(error) = self.checkpoint(self.progress().0) { return error; }
        tokio::select! {
...
impl InferencePrivateBytesV1 {
    pub fn new(bytes: Vec<u8>, maximum: usize) -> Result<Self, InferenceErrorV1> {
        ...
        if value.0.len() > maximum { return Err(InferenceErrorV1::Bounds); }
        Ok(value)
    }
