...
use crate::directory::{model::{SocketSessionBindingStatus, SpaceRole}, HubDirectories, HubDirectory};
use directory::os_directory::DocumentScope;

pub(crate) async fn check_live_inference_author(
    directory: &HubDirectories, identity: &InferenceIdentityV1, scope: &DocumentScope,
    now: impl Fn() -> i64, control: &InferenceOperationControlV1,
    ...
    if returned_at_ms < now_ms { return Err(InferenceErrorV1::Denied); }
    match binding {
