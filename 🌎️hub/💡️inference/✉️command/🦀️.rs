...
impl<'a> CanonicalInferenceCommandV1<'a> {
    pub(super) fn decode(bytes: &'a [u8]) -> Result<Self, InferenceErrorV1> {
        if bytes.is_empty() || bytes.len() > COMMAND_MAX_BYTES { return Err(InferenceErrorV1::Bounds); }
        ...
        if count > DEPENDENCY_MAX_COUNT as u64 { return Err(InferenceErrorV1::Bounds); }
        ...
        for index in 0..dependency_count {
            ...
            if dependencies[..index].contains(&value) { return Err(InferenceErrorV1::Invalid); }
            dependencies[index] = value;
        }
        let value = Self {
            mutation_id, document_id, actor, dependencies, dependency_count,
            diff_schema: cursor.text()?, diff_payload: cursor.field(PAYLOAD_MAX_BYTES)?,
            inverse_schema: cursor.text()?, inverse_payload: cursor.field(PAYLOAD_MAX_BYTES)?,
            timestamp: protocol::HybridLogicalTimestamp { actor: cursor.integer()?, physical_ms: cursor.integer()?, logical: cursor.integer()? },
        };
        if cursor.position != bytes.len() { return Err(InferenceErrorV1::Invalid); }
        let mut canonical = InferencePrivateBytesV1::new(Vec::with_capacity(bytes.len()), COMMAND_MAX_BYTES)?;
        value.encode(&mut canonical.0);
        if canonical.as_slice() != bytes { return Err(InferenceErrorV1::Invalid); }
        Ok(value)
    }
    ...
    fn encode(&self, output: &mut Vec<u8>) {
        ...
        for dependency in &self.dependencies[..self.dependency_count] { protocol::write_str(output, dependency); }
        ...
        for value in [self.timestamp.actor, self.timestamp.physical_ms, self.timestamp.logical] { protocol::wire::write_varint_u64(output, value); }
    }
}
...
impl<'a> Cursor<'a> {
    fn integer(&mut self) -> Result<u64, InferenceErrorV1> {
        ...
        for shift in (0..=63).step_by(7) {
            ...
            self.position += 1;
            if shift == 63 && byte > 1 { return Err(InferenceErrorV1::Invalid); }
            value |= u64::from(byte & 127) << shift;
            if byte & 128 == 0 {
                if shift != 0 && byte == 0 { return Err(InferenceErrorV1::Invalid); }
    ...
    fn field(&mut self, maximum: usize) -> Result<&'a [u8], InferenceErrorV1> {
        ...
        if length > maximum as u64 || length > (self.bytes.len() - self.position) as u64 { return Err(InferenceErrorV1::Bounds); }
    ...
    fn text(&mut self) -> Result<&'a str, InferenceErrorV1> {
        ...
        if value.is_empty() || value.chars().any(char::is_control) { return Err(InferenceErrorV1::Invalid); }
        Ok(value)
    }
}
