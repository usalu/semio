...
pub(super) struct TrustedCatalogRelativePath {
    segments: Box<[String]>,
}

impl TrustedCatalogRelativePath {
...
impl TrustedCatalogDataRoot {
    ...
    pub(super) fn open_current(&self) -> Result<Option<TrustedCatalogOpenedFile>, AuthorityError> {
        let path = TrustedCatalogRelativePath::parse("trusted-catalog/current.json")?;
...
impl TrustedCatalogGenerationRoot {
    ...
    pub(super) fn open_regular(&self, path: &TrustedCatalogRelativePath) -> Result<TrustedCatalogOpenedFile, AuthorityError> {
        TrustedCatalogOpenedFile::new(platform::open_regular_at(&self.directory, path.os_segments()).map_err(catalog_error)?)
    }

    pub(super) async fn read_regular(&self, path: &TrustedCatalogRelativePath, maximum: u64, context: &OperationContext<'_>) -> Result<Vec<u8>, AuthorityError> {
