...
pub struct ArtifactCreationExecutionV1 {
    pub(super) intent: ArtifactCreationIntentV1,
}

impl ArtifactCreationExecutionV1 {
    /// 🔎️ Inspection cannot construct, clone or renew the factory execution grant.
    pub fn intent(&self) -> &ArtifactCreationIntentV1 {
        &self.intent
    }
}
...
pub(crate) fn decide_artifact_creation_fact_append_v1(facts: &[ArtifactCreationFactV1], append: &ArtifactCreationFactAppendV1, observed_now_ms: u64) -> DirectoryResult<Option<ArtifactCreationFactV1>> {
    ...
    if append.recorded_at_ms > observed_now_ms || cancellation && (append.expected_revision == 0 || append.expected_revision > operation.revision) {
        return Err(rejected("artifact creation transition clock or observed revision is invalid"));
    }
    match &append.body {
    ...
    if matches!(operation.phase, SpaceArtifactCreationPhaseV1::Ready | SpaceArtifactCreationPhaseV1::Cancelled | SpaceArtifactCreationPhaseV1::Failed) {
        return Ok(None);
    }
    if !cancellation && append.expected_revision != operation.revision {
        return Err(rejected("artifact creation transition revision changed"));
    }
...
impl ArtifactCreationIntentV1 {
    ...
    pub fn validate(&self) -> DirectoryResult<()> {
        let descriptor = DocumentDescriptor {
            ...
            document_id: self.scope.document_id.clone(),
            artifact_kind: self.request.kind_id.clone(),
            artifact_schema: self.artifact_schema.clone(),
            owner: self.owner.clone(),
            pack_schema_hash: self.pack_schema_hash.clone(),
            bootstrap_version: 1,
        ...
        if !text(&self.actor.user_id)
            ...
            || !(SpaceArtifactCreationStatusV1 {
                schema: "semio.hub.space-artifact-creation-status/v1".into(),
...
impl ArtifactCreationPreparedV1 {
    ...
    pub fn validate(&self, intent: &ArtifactCreationIntentV1) -> DirectoryResult<()> {
        ...
        if self.pack.is_empty()
            || self.spr.is_empty()
            || self.pack.len().checked_add(self.spr.len()).is_none_or(|length| length > ARTIFACT_CREATION_PAIR_MAX_BYTES)
            || d.space_id != intent.scope.space_id
            || d.document_id != intent.scope.document_id
            || d.artifact_kind != intent.request.kind_id
            || d.artifact_schema != intent.artifact_schema
            || d.owner != intent.owner
...
impl ArtifactCreationOperationV1 {
    ...
    pub fn fold(facts: &[ArtifactCreationFactV1]) -> DirectoryResult<Self> {
        ...
        for (index, fact) in facts.iter().enumerate() {
            if fact.actor_user_id != intent.actor.user_id
                || fact.request_id != intent.request.request_id
                || fact.revision != index as u64 + 1
                || fact.recorded_at_ms < timestamp
                || fact.recorded_at_ms > directory::os_directory::schema::DOCUMENT_OPEN_MAX_SAFE_INTEGER
            {
                return Err(rejected("artifact creation fact identity or order differs"));
            }
            match (&fact.body, result.phase, index) {
                ...
                (ArtifactCreationFactBodyV1::Committed { receipt }, SpaceArtifactCreationPhaseV1::Preparing, _) => {
                    ...
                    if receipt.ready != intent.ready()
                        || receipt.checkpoint_id != candidate.checkpoint.checkpoint_id
                        || receipt.descriptor_digest_v1 != candidate.checkpoint.descriptor_digest_v1
                        || receipt.event_seq_first == 0
                        || receipt.event_seq_first.checked_add(2) != Some(receipt.event_seq_last)
                        || receipt.event_seq_last > directory::os_directory::schema::DOCUMENT_OPEN_MAX_SAFE_INTEGER
                        || receipt.event_ids.len() != 3
                        || receipt.event_ids.iter().any(|id| !text(id) || !id.is_ascii())
                        || receipt.event_ids.iter().collect::<std::collections::BTreeSet<_>>().len() != 3
                    {
                        return Err(rejected("artifact creation completion differs from its prepared triple"));
                    }
                    result.receipt = Some(receipt.clone());
                    result.phase = SpaceArtifactCreationPhaseV1::Ready;
                }
            ...
            result.revision = fact.revision;
            timestamp = fact.recorded_at_ms;
        }
        Ok(result)
    }
    ...
    pub fn status(&self) -> SpaceArtifactCreationStatusV1 {
        SpaceArtifactCreationStatusV1 {
            schema: "semio.hub.space-artifact-creation-status/v1".into(),
            request_id: self.intent.request.request_id.clone(),
            space_id: self.intent.scope.space_id.clone(),
