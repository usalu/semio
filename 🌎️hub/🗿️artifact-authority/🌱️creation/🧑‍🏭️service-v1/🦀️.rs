...
impl ArtifactCreationServiceV1 {
    ...
    pub fn new(directory: Arc<DirectoryService>, catalog: Arc<VerifiedTrustedCatalog>, storage: Arc<ArtifactChunkCasStores>) -> Self {
        Self { directory, catalog, storage }
    }
    ...
    async fn authorize(&self, actor: &ArtifactCreationActorV1, space_id: &str, now_ms: u64) -> DirectoryResult<()> {
        ...
        if !matches!(self.directory.backend().socket_session_binding(&actor.session_id, &actor.user_id, actor.authorization_generation, Some(space_id), now_ms).await?, SocketSessionBindingStatus::Active { role: Some(SpaceRole::Author), .. })
            || self.directory.backend().get_space(space_id).await?.is_none_or(|space| space.kind == "archive")
        {
    ...
    fn catalog_matches(&self, intent: &ArtifactCreationIntentV1) -> bool {
        self.catalog.generation_id() == intent.catalog_generation
            && intent.request.expected_catalog_generation_id == intent.catalog_generation
            && self.catalog.artifact_creation_selection(&intent.request.kind_id).is_some_and(|selected| {
                selected.package.plugin_id == intent.owner.plugin_id
                    && selected.package.package_id == intent.owner.package_id
                    && selected.package.version == intent.owner.version
                    && selected.package.component_sha256 == intent.owner.package_hash
                    && selected.artifact.schema == intent.artifact_schema
                    && selected.artifact.pack_schema_hash == intent.pack_schema_hash
                    && selected.parent_dialect == intent.parent_dialect
            })
    }
    ...
    pub async fn accept(&self, actor: &ArtifactCreationActorV1, space_id: &str, request: SpaceArtifactCreateV1, context: &OperationContext<'_>) -> DirectoryResult<ArtifactCreationAcceptanceV1> {
        ...
        if request.expected_catalog_generation_id != self.catalog.generation_id() {
            return Err(DirectoryError::Conflict("artifact creation catalog generation changed".into()));
        }
        ...
        directory::os_identity::fill_entropy(&mut entropy).map_err(|_| DirectoryError::Backend("artifact creation identity entropy is unavailable".into()))?;
        if entropy == [0; 16] {
            return Err(DirectoryError::Backend("artifact creation identity entropy is empty".into()));
        }
        ...
        let intent = ArtifactCreationIntentV1 {
            ...
            artifact_schema: selected.artifact.schema.clone(),
            pack_schema_hash: selected.artifact.pack_schema_hash.clone(),
            parent_dialect: selected.parent_dialect.clone(),
            command_sha256,
            accepted_at_ms,
    ...
    pub async fn cancel(&self, actor: &ArtifactCreationActorV1, space_id: &str, request_id: &str, now_ms: u64) -> DirectoryResult<SpaceArtifactCreationStatusV1> {
        ...
        let append = ArtifactCreationFactAppendV1 {
            ...
            command_sha256: operation.intent.command_sha256,
    ...
    async fn terminal(&self, intent: &ArtifactCreationIntentV1, cancelled: bool, now_ms: u64) -> DirectoryResult<SpaceArtifactCreationStatusV1> {
        ...
        let append = ArtifactCreationFactAppendV1 {
            ...
            expected_revision: operation.revision,
            recorded_at_ms: now_ms,
            body: if cancelled { ArtifactCreationFactBodyV1::Cancelled } else { ArtifactCreationFactBodyV1::Failed },
        };
    ...
    pub async fn recovery_candidates(&self, now_ms: u64, limit: usize) -> DirectoryResult<Vec<ArtifactCreationIntentV1>> {
        self.directory.backend().artifact_creation_recovery_candidates(now_ms, limit).await
    }
    ...
    pub async fn recover<A: ArtifactCreationCommitAuthorityV1>(&self, intent: ArtifactCreationIntentV1, authority: &A, context: &OperationContext<'_>) -> DirectoryResult<SpaceArtifactCreationStatusV1> {
        ...
        if operation.intent != intent {
            return Err(DirectoryError::Conflict("artifact creation recovery intent differs".into()));
        }
        ...
        let revoked = match self.authorize(&intent.actor, &intent.scope.space_id, context.now_ms()).await {
            Ok(()) => false,
            Err(DirectoryError::Unauthorized) => true,
    ...
    pub async fn execute<A: ArtifactCreationCommitAuthorityV1>(&self, execution: ArtifactCreationExecutionV1, authority: &A, context: &OperationContext<'_>) -> DirectoryResult<SpaceArtifactCreationStatusV1> {
        ...
        let operation = self.read(&intent.actor.user_id, &intent.scope.space_id, &intent.request.request_id).await?;
        if operation.intent != intent {
            return Err(DirectoryError::Conflict("artifact creation execution identity differs".into()));
        }
        if operation.phase != SpaceArtifactCreationPhaseV1::Accepted {
            return Ok(operation.status());
        }
        ...
        let append = ArtifactCreationFactAppendV1 {
            ...
            command_sha256: intent.command_sha256.clone(),
            expected_revision: operation.revision,
            recorded_at_ms: context.now_ms(),
            body: ArtifactCreationFactBodyV1::Prepared { candidate: prepared },
        };
    ...
    pub async fn recover_prepared<A: ArtifactCreationCommitAuthorityV1>(&self, intent: ArtifactCreationIntentV1, authority: &A, context: &OperationContext<'_>) -> DirectoryResult<SpaceArtifactCreationStatusV1> {
        let operation = self.read(&intent.actor.user_id, &intent.scope.space_id, &intent.request.request_id).await?;
        if operation.intent != intent || operation.phase == SpaceArtifactCreationPhaseV1::Accepted {
            return Err(DirectoryError::Conflict("artifact creation recovery cannot execute an unprepared factory".into()));
        }
        let mut limits = context.limits();
        limits.max_pair_bytes = limits.max_pair_bytes.min(ARTIFACT_CREATION_PAIR_MAX_BYTES as u64);
    ...
    async fn publish_prepared<A: ArtifactCreationCommitAuthorityV1>(&self, operation: ArtifactCreationOperationV1, authority: &A, context: &OperationContext<'_>) -> DirectoryResult<SpaceArtifactCreationStatusV1> {
        ...
        match CheckpointPublicationOrchestrator::new(ArtifactChunkBlobStore::new(self.storage.clone()), publisher).publish_candidate(candidate, context).await {
            ...
            Err(AuthorityError::Publication(_)) => {
                ...
                Ok(SpaceArtifactCreationStatusV1 {
...
fn authority_error(error: AuthorityError) -> DirectoryError {
    DirectoryError::Backend(error.to_string())
}

struct GenesisPublisher<'a, A> {
    owner: &'a ArtifactCreationServiceV1,
    intent: &'a ArtifactCreationIntentV1,
    prepared: &'a ArtifactCreationPreparedV1,
    authority: &'a A,
}
...
impl<A: ArtifactCreationCommitAuthorityV1> VerifiedCheckpointPublisher for GenesisPublisher<'_, A> {
    ...
    async fn publish_reserved(&self, checkpoint: &ArtifactCheckpoint, reservation: &ArtifactCasReservation, context: &OperationContext<'_>) -> Result<(), AuthorityError> {
        ...
        context.checkpoint()?;
        if !self.owner.catalog_matches(self.intent) {
            return Err(AuthorityError::Publication("artifact creation catalog changed".into()));
        }
