//! 📝 Note program — infinite canvas note board bundled as a hot-swappable WASM program.

use semio_framework_program::{SurfaceKind, PanelGroup,
...
/// 📄 The `semio` example document, handcrafted in the `.note` DSL (see `🔖Dsl`) instead of JSON —
/// {@link semio_example_document}/{@link semio_example_json} are the only ways it should be consumed.
const SEMIO_NOTE_EXAMPLE_TEXT: &str = include_str!("../../example/semio.note");
...
///
/// See {@link protocol::Operation} and {@link https://../../../vcs/program/rs/lib.rs VcsDemoOperation}.
...
struct NoteDiff {
    #[serde(default, skip_serializing_if = "Option::is_none")]
    operation: Option<NoteOperation>,
}
...
//#region 🔖Dsl
// `NoteDocument`'s `store::DocumentDsl` and `NoteOperation`'s `store::OpText` implementations are now
...
/// 📄 The `semio` example, parsed once from {@link SEMIO_NOTE_EXAMPLE_TEXT} — the source of truth for
...
fn semio_example_document() -> NoteDocument {
...
/// contractually require JSON text (`ProgramApp::render`'s `projection_override_json`, `App::example`'s
/// manifest `document_json`) — out of scope to change, since both are defined in `framework/program`.
...
fn register_note_exports() {
    ...
    semio_framework_program::program_runtime::register_document_codec_for_app::<NotePlayApp>(NOTE_DOCUMENT_SCHEMA);
}
...
mod tests {
    use super::*;
    use semio_framework_program::{DwgColor, DwgEntity, DwgLayer, ProgramApp};
    use semio_framework_program::testkit::{assert_undo_redo_round_trip, meta, new_app, new_app_with_registry};
        ...
        let document = NoteDocument {
            ...
            blocks: vec![
                ...
                NoteBlockNode::Table {
                    ...
                    rows: vec![
                        vec![NoteTableCell { content: "a1".into() }, NoteTableCell { content: "b1".into() }],
                        vec![NoteTableCell { content: "a2".into() }, NoteTableCell { content: "with \"quotes\" and \\ backslash".into() }],
                    ],
                },
                NoteBlockNode::Math {
                    id: "math-1".into(),
    ...
    #[test]
    fn note_op_text_round_trips_every_variant() {
        store::test_support::assert_op_line_round_trip(&NoteOperation::SetCamera { camera: NoteCamera { x: 1.0, y: -2.5, zoom: 2.0 } });
        store::test_support::assert_op_line_round_trip(&NoteOperation::SetGridVisible { visible: Some(true) });
        store::test_support::assert_op_line_round_trip(&NoteOperation::SetGridVisible { visible: None });
        store::test_support::assert_op_line_round_trip(&NoteOperation::SetGridSpacing { spacing: Some(16.0) });
        store::test_support::assert_op_line_round_trip(&NoteOperation::SetGridSpacing { spacing: None });
        store::test_support::assert_op_line_round_trip(&NoteOperation::SetGridSubdivisions { value: Some(8.0) });
        store::test_support::assert_op_line_round_trip(&NoteOperation::SetGridOpacity { opacity: Some(0.6) });
        store::test_support::assert_op_line_round_trip(&NoteOperation::SetSnapEnabled { enabled: Some(false) });
        store::test_support::assert_op_line_round_trip(&NoteOperation::SetSnapGridSpacing { spacing: Some(4.0) });
        store::test_support::assert_op_line_round_trip(&NoteOperation::SetPencilWidth { width: Some(5.0) });
        store::test_support::assert_op_line_round_trip(&NoteOperation::SetEraserRadius { radius: Some(20.0) });

        let mut stroke_with_points = create_block_by_kind("stroke", 0.0, 0.0);
        if let NoteBlockNode::Ink { points, .. } = &mut stroke_with_points {
            *points = vec![[0.0, 0.0], [1.5, 2.5], [3.0, -1.0]];
        }
        store::test_support::assert_op_line_round_trip(&NoteOperation::SetBlocks {
            blocks: vec![create_block_by_kind("text", 5.0, 5.0), stroke_with_points],
        });

        store::test_support::assert_op_line_round_trip(&NoteOperation::PutAsset {
            key: "asset-2".into(),
            asset: NoteImageAsset { mime: "image/jpeg".into(), data: "data:image/jpeg;base64,xyz".into(), width: None, height: None },
        });

        store::test_support::assert_op_line_round_trip(&NoteOperation::SetDocument { document: semio_example_document() });
    }

    #[test]
    fn note_document_text_round_trips_store_with_applied_operation() {
        let envelope = store::create_document_envelope::<NoteDocument, NoteOperation>("note.document", "doc-text-test", empty_note_document(), None);
        let mut store = store::DocumentStore::new(envelope);
        store
            .dispatch(store::DocumentCommand::Apply {
                operations: vec![NoteOperation::SetBlocks { blocks: vec![create_block_by_kind("text", 12.0, 24.0)] }],
                description: None,
            })
            .expect("apply");
