//! ✏️ Draw program — declarative draw app bundled as a hot-swappable WASM program.
...
use semio_framework_program::{SurfaceKind, ActionDefinition, ActionEmit, ActionKind, DocumentApp, DocumentView,
...
use semio_framework_program::kernel::HostEffect;
...
/// 📄 Parses the handcrafted DSL fixture once per call — used both for `setActiveExample`'s in-program
...
/// 🌉 JSON bridge for `semio_framework_program`'s `App::example`/`VcsDocumentApp::render` override,
...
/// one is a `vcs`/WIT/`framework/program/host` change outside this ticket's plugin-crate-only file
...
impl DrawPlayApp {
    ...
    /// this sandboxed program. `#[allow(dead_code)]`: no caller exists inside this crate today (see the
...
fn register_draw_exports() {
    ...
    semio_framework_program::program_runtime::register_document_codec_for_app::<DrawPlayApp>(DRAW_DOCUMENT_SCHEMA);
