...
pub type DrawEnvelope = DocumentEnvelope<DrawDocument, DrawOperation>;
pub type DrawStore = DocumentStore<DrawDocument, DrawOperation>;
...
//#region 🔖Dsl
// `DrawDocument`'s `store::DocumentDsl` and `DrawOperation`'s `store::OpText` are now generated by
...
// `store::OpText for DrawOperation` is now generated by `#[derive(dsl::DslOps)]` on the type
...
mod tests {
    ...
    fn document_text_round_trips_a_store_with_an_applied_operation() {
        ...
        let envelope = store::create_document_envelope(DRAW_DOCUMENT_SCHEMA, "doc-text-test", initial, None);
        let mut store: DrawStore = DocumentStore::new(envelope);
        ...
        store.dispatch(store::DocumentCommand::Apply { operations: vec![DrawOperation::SetLayerOpacity { layer_id: layer_id_value, opacity: 0.5 }], description: Some("set opacity".into()) }).expect("apply set opacity");
        store::test_support::assert_document_text_round_trip(&store);
        store::test_support::assert_document_pack_round_trip(&store);
        store::test_support::assert_live_equals_replay(&store);
    }
    //#endregion 🔖DslTests
