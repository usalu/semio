// #region 🧲Header
/** @emoji ✏️ Draw play app — layer editor shell. */
// #endregion 🧲Header
...
import {
	...
	encodeDrawPointerFocusKey,
	drawHoverPayloadFromPointerFocusKey,
...
export class DrawPlayController extends Controller implements PlaygroundExampleHost {
	...
	private canvasMeasures(): readonly WindowMeasure[] {
		...
		const selectedLayer = selected.length === 1 ? findDrawLayer(doc, selected[0]!) : null;
		return [
			{
				kind: "slider",
				id: "draw-canvas-zoom",
	...
	private canvasEngagement(): WindowEngagement {
		...
		return {
			...
			status: [{ id: "draw-layer-count", text: `${flattenDrawLayers(doc.layers).length} layers · ${this.getSelectedIds().length} selected` }],
		};
	}
	...
	getHoveredId(): string | null {
		return drawHoverPayloadFromPointerFocusKey(this.pointerFocus.getSnapshot().hover).id;
	}
		...
		switch (command) {
			...
			case "engagementSubmit": {
				...
				if (value && selected.length === 1) {
					this.run("patchLayers", { layerIds: [...selected], field: "name", value });
				}
				return;
			}
			case "setCameraZoom": {
			...
			case "setSelectedOpacity": {
				...
				const selected = this.getSelectedIds();
				if (!Number.isFinite(opacity) || !selected.length) return;
			...
			case "setFixtureJson": {
				...
				if (!json.includes("draw.document")) {
			...
			case "setSelection": {
				...
				console.log("[DEBUG] draw selection", this.getSelectedIds());
				this.bump();
				return;
			}
			case "setHover": {
				...
				const kind = (args.kind as DrawKindHover | null) ?? null;
				const hoverKey = id ? encodeDrawPointerFocusKey(kind?.domain ?? "layer", id) : null;
			...
			case "commitDocument": {
				...
				if (!document || document.schema !== "draw.document") return;
			...
			case "deleteLayer": {
				...
				this.pointerFocus.setSelection(this.getSelectedIds().filter((id) => id !== layerId));
				return;
			}
			case "duplicateLayer": {
			...
			case "combineBoolean": {
				const ids = Array.isArray(args.ids) ? args.ids.map(String) : [...this.getSelectedIds()];
			...
			case "selectAll": {
				this.pointerFocus.setSelection(flattenDrawLayers(this.projection().layers).map((layer) => layer.id));
				this.bump();
				return;
			}
			default:
