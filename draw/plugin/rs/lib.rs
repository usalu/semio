...
use semio_framework_plugin::{SurfaceKind,
    ...
    ui_inspector_readonly_field, ui_stack_vertical, ui_text, tool_toggle, App, Canvas2dScene,
    ...
    fn tools(&self, document_json: &str, _view_state: &ViewState) -> Vec<ToolNode> {
        ...
        TOOLS
            ...
            .map(|(index, (id, icon, label))| {
                let mut node = tool_toggle(format!("draw-play-tools.{id}"), icon, label, active == id, draw_play_cmd("setActiveTool", Some(json!({ "tool": id }))));
                if let ToolNode::Toggle { order, .. } = &mut node {
