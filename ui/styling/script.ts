...
function emitCSharp(tokens: Tokens): string {
  const lines: string[] = ["// <auto-generated />", "// Generated from ui/styling/tokens.json — run `bun ./script.ts generate`.", "using System;", "", "namespace Elements.Styling;", "", "public static class Palette", "{"];
  for (const [k, v] of Object.entries(tokens.colors)) {
...
/** @emoji 🗺️ Color scan walks its own root set: same as {@link PX_SCAN_ROOTS} minus `ui/styling` (the source of truth
 * for tokens), plus `.storybook` and the whole `coda/client` tree (superseding the narrower `coda/client/ui` root
 * so it isn't walked twice). */
const COLOR_SCAN_ROOTS = [...PX_SCAN_ROOTS.filter((root) => !root.startsWith("ui/styling") && root !== "coda/client/ui"), ".storybook", "coda/client"] as const;
...
/** @emoji 🚫 Fails when hardcoded hex/rgb/hsl colors, full-palette Tailwind color classes, or manual `dark:`-variant color utilities remain outside `ui/styling` (design tokens/appearances are the single source of color truth). */
