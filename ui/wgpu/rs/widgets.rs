...
pub fn measure_widget<E>(atlas: &mut FontAtlas, theme: &Theme, node: &WidgetNode<E>) -> (f32, f32) {
    match node {
        ...
        WidgetNode::Text { value, emphasize } => {
            let size = if *emphasize { theme.font_size_body + 1.0 } else { theme.font_size_body };
...
pub fn render_widget<E: Clone>(
    ...
    match node {
        ...
        WidgetNode::Text { value, emphasize } => {
            let size = if *emphasize { ctx.theme.font_size_body + 1.0 } else { ctx.theme.font_size_body };
        ...
        WidgetNode::Section { label, children, .. } => {
            if let Some(label) = label {
                draw_text(ctx, label, bounds.x, bounds.y + ctx.theme.font_size_body, ctx.theme.font_size_body, ctx.theme.text);
            }
        ...
        WidgetNode::Tree { sections } => render_tree(sections, bounds, ctx),
    }
}
...
fn render_button<E: Clone>(
    ...
    ctx.input.register_hit(HitTarget {
        rect: bounds,
        event,
        control_id: id.or_else(|| Some(label.to_string())),
        kind: HitKind::Button,
    });
}
...
fn render_input<E: Clone>(id: &str, value: &str, placeholder: Option<&str>, bounds: Rect, ctx: &mut WidgetContext<'_, E>) {
    ...
    let (display, muted) = if ctx.input.focused_id.as_deref() == Some(id) {
...
fn render_toggle<E: Clone>(
    ...
    let bg = if pressed { ctx.theme.accent } else { ctx.theme.button };
    ctx.draw.push_rounded([bounds.x, bounds.y, bounds.w.min(36.0), bounds.h], bg, ctx.theme.border_radius);
...
fn render_ring<E: Clone>(id: &str, t: f64, event: Option<E>, bounds: Rect, ctx: &mut WidgetContext<'_, E>) {
    ...
    let knob_angle = std::f32::consts::TAU * t as f32;
...
fn render_tree_item<E: Clone>(item: &TreeItem<E>, bounds: Rect, ctx: &mut WidgetContext<'_, E>, depth: u32) {
    if item.selected {
        ctx.draw.push_rounded([bounds.x, bounds.y, bounds.w, bounds.h], ctx.theme.selected, 4.0);
    }
    draw_text(
        ctx,
        &item.label,
