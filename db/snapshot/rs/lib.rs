//! 🗄️ 📸 `db_snapshot` — pack-file-based document snapshots for the `db` crate family:
...
//!
//! 🎯 Design choice — locating the `KIND_SNAPSHOT` descriptor segment: `pack::Manifest` has no
...
//!
//! 🎯 Scope boundary: retention (`SnapshotManager::retain_from`) only allows pruning generations
...
/// @emoji 📇 One generation's manifest of a document's snapshot state: the frontier it was taken
...
pub struct SnapshotDescriptor {
    ...
    pub generation: u64,
    /// @emoji 🌳 `None` for a full baseline (self-sufficient, chains to nothing); `Some(g)` for an
    ...
    /// @emoji 🧱 Page hashes with a `KIND_CHUNK` in *this* blob, in the exact order they were
...
impl SnapshotDescriptor {
    /// @emoji 🧭 Reconstructs the `db_core::Frontier` this generation was taken at.
    ...
    /// @emoji 📖 Inverse of `encode`. Never panics on malformed input — every field read is
...
//#region 🔖SegmentIo
/// @emoji 🪟 A length-bounded, base-shifted view over a borrowed byte buffer that implements
...
/// @emoji 🔎 Reads the `KIND_SNAPSHOT` segment this crate always writes as the very first segment
...
/// @emoji 🩹 `pack::PackWriter::finish` always writes `prev_footer_offset = 0` (there is no public
...
/// @emoji 🪪 One opened generation: its decoded descriptor plus enough footer/position state to
...
/// @emoji 🔚 Opens the LAST generation physically present in `combined` (the one whose footer sits
...
/// @emoji 📄 Reads one page's raw bytes by content hash, starting at `handle` and walking to
...
//#region 🔖Manager
/// @emoji 🌱 Which lineage a `SnapshotManager::publish` call starts: `FullBaseline` is
...
/// @emoji 📦 Everything about a generation that isn't derived from `new_pages`/plumbing — the
...
/// @emoji 🧑‍💼 Orchestrates `db_snapshot`'s pack-encoding logic on top of a
...
impl<'storage> SnapshotManager<'storage> {
    ...
    pub fn publish(&self, document: &DocumentId, origin: SnapshotOrigin, new_pages: &[Page], body: SnapshotBody) -> Result<u64, DbError> {
        ...
        self.storage.write_generation(document, generation, &bytes)?;
        Ok(generation)
    }

    /// @emoji 🔗 Fetches every generation from the chain's root through `through_generation`
    ...
    /// @emoji 🥇 The document's latest generation number and descriptor, or `None` if it has no
    /// snapshot yet.
    ...
    /// @emoji 🎯 Selection: the highest-numbered generation whose `head_seq` does not exceed
    ...
    pub fn retain_from(&self, document: &DocumentId, floor_generation: u64) -> Result<(), DbError> {
        ...
        for generation in self.storage.list_generations(document)? {
            if generation < floor_generation {
                self.storage.delete_generation(document, generation)?;
            }
        }
        Ok(())
    }

    /// @emoji 🔬 Verifies generation `generation` decodes cleanly at `level`: the `KIND_SNAPSHOT`
...
mod tests {
    ...
    fn single_generation_round_trips_through_pack_public_api() {
        ...
        // 🔬 A real `pack::PackFile`, unrelated to this crate's own reader, must accept the bytes.
    ...
    fn footer_chain_flag_and_prev_offset_are_genuine_pack_wire_data() {
        ...
        // 🔬 Read gen1's footer via `pack`'s own public superblock API, not this crate's helpers.
    ...
    fn manager_publishes_full_baseline_then_loads_it_back() {
        ...
        let (loaded_generation, descriptor) = manager.load_latest(&document).unwrap().unwrap();
    ...
    fn manager_incremental_chain_materializes_and_resolves_inherited_pages() {
        ...
        let gen1 = manager.publish(&document, SnapshotOrigin::Incremental, &gen1_pages, body(5)).unwrap();
        assert_eq!(gen1, 1);

        let (latest_generation, descriptor) = manager.load_latest(&document).unwrap().unwrap();
        assert_eq!(latest_generation, 1);
        assert_eq!(descriptor.parent_generation, Some(0));

        let combined = manager.materialize_chain(&document, 1).unwrap();
    ...
    fn manager_retain_from_requires_full_baseline_floor() {
        ...
        assert!(matches!(manager.retain_from(&document, 1), Err(DbError::InvalidArgument(_))));
    }
    ...
    fn manager_retain_from_deletes_generations_below_a_valid_baseline_floor() {
        ...
        manager.retain_from(&document, 2).unwrap();
    ...
    fn manager_select_generation_picks_highest_head_seq_at_most_target() {
        ...
        assert_eq!(manager.select_generation(&document, 15).unwrap(), Some(1));
