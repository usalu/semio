...
//#region 🔖Util
/// @emoji 🔓 Locks `mutex`, recovering the inner value even if a prior holder panicked while
...
//#region 🔖Json
/// @emoji 📝 Escapes `raw` per RFC 8259 into `out` (no surrounding quotes).
    ...
    match field {
        ...
        db_core::EmitField::F64(v) => {
            if v.is_finite() {
                ...
                // 🔒 JSON has no NaN/Infinity literal — never emit a syntactically invalid line.
                out.push_str("null");
            }
        }
...
/// @emoji 🧾 Encodes `event` as one JSON object line (no trailing newline) — the wire shape both
...
/// @emoji 🧠 An in-memory `EventSink` — the default for tests and for introspecting what a sink
...
impl MemorySink {
    ...
    /// @emoji 📜 A snapshot of every line written so far, oldest first.
...
/// @emoji 📄 An `EventSink` over any `std::io::Write` (a file, a pipe, `Vec<u8>`, …) — one JSON
...
//#region 🔖Structured
/// @emoji 📡 A `db_core::Emit` implementation that JSON-lines-encodes every event into an
...
impl<S: EventSink> StructuredSink<S> {
    ...
    /// @emoji 🚨 How many `emit` calls lost their event to a sink write failure. `Emit::emit`
...
//#region 🔖Audit
/// @emoji 🔗 One link in `AuditSink`'s tamper-evident chain: the sequence number, and the
...
impl<S: EventSink> AuditSink<S> {
    /// @emoji 🆕 `max_retained` bounds the in-memory chain window `verify_chain` can check
    ...
    /// @emoji 🚨 See `StructuredSink::failed_writes` — same rationale (`Emit::emit` has no
    ...
    /// @emoji 📜 A snapshot of the retained chain window, oldest first.
    pub fn chain(&self) -> Vec<AuditLink> {
        lock(&self.state).links.iter().copied().collect()
    }

    /// @emoji ✅ Recomputes the checksum chain over `lines` (as read back from durable storage,
...
/// @emoji 🚧 Bounds how many distinct label sets ("series") a metric name may accumulate — an
...
impl CardinalityLimiter {
    ...
    /// @emoji 🚪 Admits `labels` for `metric`: returns `labels` unchanged if it's already a known
    ...
    pub fn admit(&self, metric: &'static str, labels: Labels) -> Labels {
        ...
        if series.contains(&labels) || series.len() < self.max_series_per_metric {
            ...
            // 🔒 Only claim a tracked slot for the overflow marker if one is actually free —
...
/// @emoji 📊 A point-in-time read of one histogram series: `bucket_counts[i]` counts
...
/// @emoji 📈 Counter/gauge/histogram series, each keyed by `(name, Labels)` and admitted through
...
impl MetricRegistry {
    ...
    /// @emoji ➕ Monotonically increments the counter series `(name, labels)` by `delta`.
    ...
    /// @emoji 📊 Records `value` into the histogram series `(name, labels)`, creating it with
...
/// @emoji 🔖 A span's identity within its owning `SpanRegistry` (not globally unique — scope it
/// with the registry that issued it).
#[derive(Clone, Copy, PartialEq, Eq, Hash, Debug)]
pub struct SpanId(u64);

/// @emoji 🌳 A finished span: name, optional parent (for nesting) and document scope, and timing.
#[derive(Clone, Debug)]
...
/// @emoji 🌲 Tracks in-flight spans and retains the most recently completed ones in a bounded
...
impl<C: Clock> SpanRegistry<C> {
    ...
    /// @emoji 🏁 Ends `id`, moving it from active into the completed ring buffer and returning
    ...
    /// @emoji 📜 A snapshot of the retained completed spans, oldest first.
...
//#region 🔖Health
/// @emoji 🩺 One component's health, worst-of-aggregated by `HealthRegistry::report` into the
...
/// @emoji 📋 A point-in-time health read: every component's individual state plus the worst-of
/// overall.
#[derive(Clone, Debug)]
...
/// @emoji 🩺 Aggregates named component health into one worst-of-the-set overall status —
...
impl HealthRegistry {
    ...
    /// @emoji 📊 Snapshots every component (sorted by name for determinism) plus the worst-of
...
/// @emoji 🧬 Runtime cross-check that two (or more) independently-produced state-hash streams
...
impl DeterminismVerifier {
    /// @emoji 🆕 `expected_labels` names every stream that must agree (e.g. `["primary",
    ...
    /// @emoji 📮 Records `label`'s digest for `seq`. Once every expected label has reported for
...
/// @emoji 🚫 An `OtelSpanExporter` that reports `DbError::Unimplemented` rather than silently
...
mod tests {
    ...
    fn cardinality_limiter_admits_up_to_the_limit_then_collapses_to_overflow() {
        ...
        // 🔒 a previously-admitted series stays itself, never gets swept into overflow later.
