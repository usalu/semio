...
//!
//! 🌉 `db_projection` (this crate's fourth declared dependency, per the frozen `Cargo.toml`) has now
...
//! `db_index`'s `CommitIndex`/`FrontierIndex`. What plugs in now, per the `🔖ProjectionBridge`
...
//#region 🔖Value
/// @emoji 🧬 The dynamic value model every `Query` evaluates against. Deliberately this crate's
...
/// @emoji 🥇 Cross-type ordering rank, used only as `compare_values`'s tie-breaker between values
...
/// @emoji 🧵 A best-effort text rendering of any `Value`, used only by `Predicate::FullText`'s
...
/// @emoji 🧩 One step of a `Path`: a named field into a `Value::Map`, or a positional index into a
/// `Value::List`.
...
impl Path {
    ...
    /// @emoji 🔤 Splits a dotted string into segments, treating any segment that parses as a plain
    ...
    /// @emoji 🔎 Walks `value` through this path's segments. `None` on any type mismatch (e.g. an
...
//#region 🔖Consistency
/// @emoji 🧭 The six read-consistency modes `DocumentHandle::query` accepts (frozen in the db
...
/// @emoji 🔌 What `resolve_consistency` needs from its caller: the document's current frontier, and
...
/// @emoji 🧮 Resolves `consistency` against `resolver` into a concrete `ResolvedConsistency`.
...
/// @emoji 🔗 A `ConsistencyResolver` backed by real `db_index` typed indexes: `Historical`
...
//#region 🔖Query
/// @emoji 🎯 A single filter condition. `And`/`Or`/`Not` compose the rest into an arbitrary boolean
...
pub enum Predicate {
    ...
    Exists(Path),
    Contains(Path, Value),
    /// @emoji 🔤 Case-insensitive substring match. `Path::empty()` matches against the whole
...
/// @emoji ✅ Evaluates `predicate` against one materialized row `value`. A missing path is treated
...
/// @emoji 🔀 One sort key: a `Path` to compare by, and its direction. A missing path sorts as
...
/// @emoji 📜 A complete query: what to return (`select`), which rows qualify (`filter`), in what
...
/// @emoji 📐 A conservative byte-size estimate of `value` — sums scalar widths and container
...
//#region 🔖QuerySource
/// @emoji 🆔 An opaque per-document row identifier. `u64` (not a `String`) to match `db_index`'s
...
/// @emoji 🚰 What the planner/evaluator need from a materialized document: every row (for a full
...
pub trait QuerySource {
    ...
    /// @emoji 🎯 Default: linear `scan` + find. Override when a cheaper direct lookup exists (e.g.
...
/// @emoji 🧵 The natural `QuerySource` over a `db_state::PVec`: row id = element index. This is the
...
/// @emoji 🔌 What a full-text pushdown needs: term → candidate row ids. `db_index::FullTextIndex`
...
/// @emoji 👉 A read-only cursor over `bytes`, used only by `decode_value` — every `take*` call
...
/// @emoji 📖 Inverse of `encode_value`. Rejects an unknown tag, a truncated buffer, or an
...
/// @emoji 🔌 `Value`'s `db_projection::ProjectionState` impl — lets any `db_projection::ProjectionClass`
...
/// @emoji 🌉 The bridge the module doc's `db_projection`-integration note describes: decodes
...
/// @emoji 🔍 Finds the first `Predicate::FullText` term reachable through a top-level conjunction
...
/// @emoji 🧠 Chooses a `QueryPlan` for `query`. Currently a single heuristic (full-text pushdown);
...
//#region 🔖Execute
/// @emoji 📊 What `execute` observed while producing a `QueryResult` — the contract's "diagnostics"
/// bullet.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// @emoji 📦 A fully materialized query result: the projected, sorted, paginated rows, plus
...
//#region 🔖Stream
/// @emoji 🌊 A `QueryResult`'s rows as an `Iterator`, for callers (`DocumentHandle::query`'s
...
//#region 🔖LiveQuery
/// @emoji 📡 A `Query` plus the `Consistency` it should be (re-)evaluated under — what a caller
...
/// @emoji 🔀 The change between two successive evaluations of a `LiveQuery`'s `Query`: rows newly
...
/// @emoji 📺 Tracks one live query's last-seen result set so `refresh` can emit a `QueryDiff`
...
impl LiveQuery {
    ...
    /// @emoji 🔁 Re-executes `self.spec.query` against `source` and diffs the result against the
...
mod tests {
    ...
    mod execute_tests {
        ...
        /// @emoji 🧪 A hand-rolled `FullTextLookup` double — exercises pushdown without needing a
    ...
    mod live_query {
        ...
        /// @emoji 🆔 `PVec`'s `RowId` is positional (index-based — see its `QuerySource` impl's
