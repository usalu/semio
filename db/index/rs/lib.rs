...
impl IndexKind {
    /// @emoji 📋 Every kind, for tests and for callers that want to enumerate/verify a document's
...
/// @emoji 🔢 How many low bits of a `run_id` are the within-kind sequence — the remaining high
...
/// @emoji 🧮 Packs `kind` and `sequence` into one `run_id`. Errors `LimitExceeded` if `sequence`
...
//#region 🔖SortedRun
/// @emoji 📇 One entry's value in a sorted run: either a live payload or a tombstone recording that
...
/// @emoji 📌 One `(key, value)` pair inside a sorted run. A well-formed run's entries are strictly
...
/// @emoji 🧱 Sorts an unordered batch of writes into a well-formed run's entry list: ascending by
...
/// @emoji 🪧 A run's 6-byte header: 4-byte magic, 1-byte format version, 1-byte `IndexKind` tag —
/// see `read_run_header`.
const RUN_MAGIC: [u8; 4] = *b"DBIR";
const RUN_VERSION: u8 = 1;

/// @emoji 📐 A run header's parsed fields plus how many bytes of `body` it occupied, so the caller
...
/// @emoji 📖 Parses `body`'s header (magic/version/kind tag/entry count) WITHOUT touching the entry
...
/// @emoji 👀 Reads just a run's `entry_count` (no checksum verification, no entry decode) — the
...
/// @emoji 📖 Inverse of `encode_run`: verifies the trailing crc32c over everything before it FIRST
...
//#region 🔖Merge
/// @emoji 🌀 The LSM-lite k-way merge: `runs` MUST be ordered oldest-first/newest-last (index 0 is
...
///
/// 🎯 Design choice: a straightforward multi-pointer scan (`O(total_entries * runs.len())`) rather
...
//#endregion 🔖MergePolicy

//#region 🔖Stats
/// @emoji 📊 A kind's current shape: how many runs it's spread across, how many live entries (each
...
//#region 🔖IndexHandle
/// @emoji 🔍 One `(document, kind)`'s view onto its sorted runs — every typed wrapper below
...
impl<'a> IndexHandle<'a> {
    /// @emoji 🚀 Opens a handle with the default `MergePolicy`.
    ...
    /// @emoji 🚀 Opens a handle with an explicit `MergePolicy` (e.g. a tighter threshold for a
    ...
    /// @emoji 📋 This handle's live run ids, ascending by sequence (oldest first) — every other id
    ...
    /// @emoji ➕ Convenience over `put_batch` for a single key.
    pub fn put(&self, key: Vec<u8>, value: Vec<u8>) -> Result<(), DbError> {
        self.put_batch(vec![(key, RunValue::Put(value))])
    }

    /// @emoji 🪦 Convenience over `put_batch` for a single tombstone.
    pub fn delete(&self, key: &[u8]) -> Result<(), DbError> {
        self.put_batch(vec![(key.to_vec(), RunValue::Tombstone)])
    }

    /// @emoji 🔎 Resolves `key` by scanning runs newest-to-oldest and returning the first match —
    ...
    /// @emoji 📜 Every live (non-tombstoned) `(key, value)` whose key starts with `prefix`,
    ...
    /// @emoji 🌀 `MergePolicy`'s enforcement: while this kind has more live runs than
    ...
    /// @emoji 🧹 Merges EVERY live run for this kind into exactly one (dropping tombstones, since
    ...
    /// @emoji 📊 Current shape of this kind's runs — see `IndexStats`'s doc for what `entry_count`
    ...
    /// @emoji ✅ Fully decodes (checksum + structural validation) every live run for this kind,
...
//#endregion 🔖IndexHandle

//#region 🔖RecordLocation
/// @emoji 📍 A pointer into a document's WAL: which segment, what byte offset, how many bytes.
...
/// @emoji 🔢 `u64 -> RecordLocation`, keyed big-endian so byte order matches numeric order — the
...
//#endregion 🔖InverseIndex

//#region 🔖ActorSeqIndex
/// @emoji 👤 `(actor, actor_seq) -> command_seq` — resolves an actor's own local operation sequence
...
impl<'a> ActorSeqIndex<'a> {
    ...
    /// @emoji 🥇 The highest `(actor_seq, command_seq)` pair recorded for `actor`, or `None` if
...
//#endregion 🔖ActorSeqIndex

//#region 🔖FrontierIndex
/// @emoji 🧭 `commit_seq -> Frontier` — a per-commit snapshot of `db_core::Frontier`, letting
...
impl<'a> FrontierIndex<'a> {
    ...
    /// @emoji 🥇 The frontier recorded under the highest `commit_seq`, or `None` if none recorded.
...
//#endregion 🔖FrontierIndex

//#region 🔖TouchedRegionIndex
/// @emoji 🎯 `region -> [command_seq]` (ascending, deduplicated) — `db_conflict`'s reverse index:
...
impl<'a> TouchedRegionIndex<'a> {
    ...
    /// @emoji ➕ Records that `command_seq` touched `region` — read-modify-write over the region's
...
//#endregion 🔖TouchedRegionIndex

//#region 🔖CommitIndex
/// @emoji 🏁 `commit_id -> command_seq` — resolves a VCS-facing commit id (`vcs::Checkpoint.id`,
...
//#endregion 🔖CommitIndex

//#region 🔖FullTextIndex
/// @emoji 🔤 `term -> [doc_ref]` — a minimal inverted index: `index_document` tokenizes text into
...
impl<'a> FullTextIndex<'a> {
    ...
    /// @emoji ➕ Tokenizes `text` and records `doc_ref` against every distinct term it contains.
    ...
    /// @emoji 🔎 The posting list for `term` (case-folded to match `index_document`'s tokenizer),
...
//#endregion 🔖FullTextIndex

//#region 🔖BlobList
/// @emoji 📦 Encodes a list of opaque byte blobs (`ConflictIndex`'s per-command conflict records)
...
impl<'a> ConflictIndex<'a> {
    ...
    /// @emoji ➕ Appends `record` to `command_seq`'s conflict list.
    ...
    /// @emoji 📋 Every conflict record recorded for `command_seq`, in the order they were
...
//#endregion 🔖ConflictIndex

//#region 🔖ProjectionIndex
...
impl<'a> ProjectionIndex<'a> {
    ...
    /// @emoji 🎯 The exact state recorded for `projection_id` at `frontier_seq`, or `None` if
...
impl<'a> PreviewIndex<'a> {
    ...
    /// @emoji 📋 Every currently-live `(preview_key, value)` published by `actor`.
...
mod tests {
    ...
    fn projection_index_resolves_exact_and_floor_lookups_scoped_to_projection_id() {
        ...
        // 🎯 "by-color" sorts after "by-author" but has no entries at all — must not fall back to
