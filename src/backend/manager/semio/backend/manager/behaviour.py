...
def getLocalPointOfView(pose:Pose ,worldPointOfView:Point, considerPointOfView = True, considerView = True)->Point:
    ...
    """
    transformedPointOfView = convert(worldPointOfView)
    if considerPointOfView:
        transformedPointOfView = applyTransforms(transformedPointOfView,BlenderMatrix.Translation(-convert(pose.point_of_view)))
    if considerView:
        transformedPointOfView = applyTransforms(transformedPointOfView,convert(pose.view).to_matrix().to_4x4())
...
def getWorldPointOfView(pose:Pose, localPointOfView:Point,considerPointOfView = True, considerView = True)->Point:
    ...
    if considerView:
        transformedPointOfView = applyTransforms(transformedPointOfView,convert(pose.view).inverted().to_matrix().to_4x4())
    if considerPointOfView:
        transformedPointOfView = applyTransforms(transformedPointOfView,BlenderMatrix.Translation(convert(pose.point_of_view)))
    return convert(transformedPointOfView)