...
fs.writeFileSync(path.resolve(process.cwd(), ".repo/tickets/2026/03/03/DESIGN-DRAG-PERFORMANCE-LONG-TASK-OPTIMIZATION/mousedown.cpuprofile"), JSON.stringify(p1));
// PROFILE DRAG
await cdp.send("Profiler.start");
await page.mouse.move(startX + 100, startY, { steps: 20 });
await page.waitForTimeout(200);
const { profile: p2 } = await cdp.send("Profiler.stop");
fs.writeFileSync(path.resolve(process.cwd(), ".repo/tickets/2026/03/03/DESIGN-DRAG-PERFORMANCE-LONG-TASK-OPTIMIZATION/drag.cpuprofile"), JSON.stringify(p2));
// PROFILE MOUSEUP
await cdp.send("Profiler.start");
await page.mouse.up();
await page.waitForTimeout(500);
const { profile: p3 } = await cdp.send("Profiler.stop");
fs.writeFileSync(path.resolve(process.cwd(), ".repo/tickets/2026/03/03/DESIGN-DRAG-PERFORMANCE-LONG-TASK-OPTIMIZATION/mouseup3.cpuprofile"), JSON.stringify(p3));
