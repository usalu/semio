...
(async () => {
  ...
  await page.evaluate(() => {
    ...
    window.__MO__ = new MutationObserver((ms) => {
      for (const m of ms) {
        if (m.type === "childList") {
          window.__MUT_LOG__.added += m.addedNodes.length;
          window.__MUT_LOG__.removed += m.removedNodes.length;
  ...
  await page.evaluate(() => {
    window.__MUT_LOG__ = { added: 0, removed: 0, attrs: 0 };
    window.__MO__.observe(document.body, { childList: true, attributes: true, subtree: true });
  });
  await page.mouse.click(10, 10);
  await sleep(5000);
  await page.evaluate(() => {
    window.__MO__.disconnect();
  });
