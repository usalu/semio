...
async function measure() {
  ...
  if (sectionCount === 0 && itemCount === 0) {
    ...
    const panelDivs = await page.evaluate(() => {
      return Array.from(document.querySelectorAll("[class*='side-panel'], [class*='panel'], [role='tree']"))
        .map((el) => ({
    ...
    document.querySelectorAll('[data-slot="tree-item-row"]').forEach((row) => {
      ...
      row.querySelectorAll("button").forEach((btn) => {
        ...
        if (svg) {
          ...
          ci = { svgCenter: r.left + r.width / 2 - rowRect.left, svgW: r.width };
        }
      });
  ...
  if (measurements.rows.length > 0) {
    ...
    for (const l of measurements.lines) {
      ...
      if (!seen.has(k)) {
        seen.add(k);
        console.log(`Row[${l.label}] line relLeft=${l.relLeft.toFixed(2)} style="${l.style}"`);
      }
    }
  }
