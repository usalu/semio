...
File: `[neural/engine/lib.rs](neural/engine/lib.rs)` (region `🔖Evaluator`)
...
- `project.json`: nx `wasm` target calling `bun ./script.ts wasm` (copy `[flow/core/project.json](flow/core/project.json)`).
...
- `FlowSession` (region `🔖WasmSession`): add `#[wasm_bindgen]` methods:
...
- launch.json: add per-module `🛠️dev🌊flow🦀module-<name>` wasm-build entries and update `🛠️dev🌊flow🦀test` to include all module crates (already lists them). Follow existing order/grouping.
