...
function extractPlayHostRegion(content: string): { before: string; region: string; after: string; regionName: string } | null {
  ...
  const regionName = marker[0].replace("//#region 🔖", "");
...
/** @emoji 🛝 Playground play host for ${regionName.replace(/PlayHost$/, "")} — loaded only via \`./play\` subpath. */
// #endregion 🧲Header
...
for (const { path, pkg } of TARGETS) {
  ...
  const playHostContent = playHostHeader(parts.regionName) + parts.region.replace(/^\/\/#region 🔖\w+PlayHost\n/, "");
