...
- **Ticket:** open a new ticket (e.g. `COMPLETE-3D-PROJECTION-GIZMO`) — distinct from open `[ALL-3D-WINDOWS-SHOW-CORNER-GIZMO](.repo/🎫/26/07/23/ALL-3D-WINDOWS-SHOW-CORNER-GIZMO/)` which only covered mounting the widget. Reuse that ticket’s wiring; this work extends the control itself.
