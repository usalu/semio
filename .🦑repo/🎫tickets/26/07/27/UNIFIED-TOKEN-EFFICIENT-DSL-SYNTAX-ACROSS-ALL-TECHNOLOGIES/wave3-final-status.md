...
- **`puzzle/plugin/rs/lib.rs`** — `Puzzle2dPlayApp`, `Puzzle3dPlayApp`, `Puzzle5dPlayApp` all declared
  ...
  `Puzzle5dPlayProjection`, one per crate in each crate's `🔖ValueBridge` region (new `🔖PlayProjection`
...
- Also noted in passing (not `[DEBUG]`, not residue, just an observation): `bun nx run
  ...
  generated/playgrounds.json, generated/playgrounds.ts"), blocking `bun ./script.ts verify` past its
...
- `puzzle/2d/rs/lib.rs` — new `🔖PlayProjection` subregion: `Puzzle2dPlayProjection` newtype +
