...
## Implemented
- `s/rs/lib.rs`: added `//#region 🔖Dsl` (`mod studio_dsl` hand-rolled lexer/printer +
  `impl vcs::DocumentDsl for SStudioProjection`, extension `sstudio`) and
  `//#region 🔖OpText` (`impl vcs::OpText for StudioOperation`), plus round-trip tests
  in the existing `//#region 🧪Tests`.
- `s/plugin/rs/lib.rs`: added `//#region 🔖Dsl` (`mod home_dsl` + `impl vcs::DocumentDsl for
  SHomeDocument`, extension `shome`) and `//#region 🔖OpText` (`impl vcs::OpText for
