...
Compared against the React reference (`ui/js/react/index.tsx`'s `ModeDropZone`/`computeModeDropZone`/`applyModeDrop`/`extractStackFromLayout`/`splitWithStack`/`mergeStackTabsIntoStack` family, `#region 🧭ModeDockDrag`/`🧭ModeLayoutUtils`) — this is the actual sibling algorithm (the `PanelDock` 6-anchor system in `os-shell.tsx` the original brief pointed at is a different, unrelated panel system; the file's own top-of-file doc comment already says `dock` corresponds to React's `Mode` component, not `PanelDock`).
