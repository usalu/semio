#!/usr/bin/env bun
/** 🧪 React OS dev functional smoke + fresh reference capture for this ticket's parity harness.
 * Thin variant of `.repo/🎫/26/07/05/FIX-WGPU-WORLD3D-EMPTY-PREVIEW/verify-react-playgrounds-e2e.ts`
 * (same plugin list, same functional assertions) copied into this ticket folder rather than patched
 * in place — the old script hardcodes its screenshot/log output paths via `import.meta.dir`, and this
 * ticket needs its own, fresher `screenshot-react-<plugin>.png` set that `parity-verify.ts`'s
 * `REFERENCE_DIR` can point at without touching the older ticket's files. */
...
const canvas2dPlugins = new Set(["draw", "note", "layout", "puzzle2d", "gis2d", "procedural2d", "reasoning-wires", "presentation"]);
...
function analyzePngBuffer(png: Buffer): PaintStats {
  ...
  const bgB = 15;
  const tolerance = 8;
  const isBg = (r: number, g: number, b: number) => Math.abs(r - bgR) <= tolerance && Math.abs(g - bgG) <= tolerance && Math.abs(b - bgB) <= tolerance;
...
const tier1Plugins = new Set(["cad", "gis2d", "puzzle2d", "puzzle3d", "puzzle5d", "flow", "procedural2d", "procedural3d", "trinity", "trinity-rewrite", "sequence"]);
...
async function assertTier1BodyContent(page: import("playwright").Page, pluginId: string): Promise<void> {
  const selector = world3dPlugins.has(pluginId) ? ".semio-world-3d-host canvas" : canvas2dPlugins.has(pluginId) ? "canvas" : graphPlugins.has(pluginId) ? '[data-component-kind="node-graph"], canvas, svg' : "#root";
...
try {
  ...
  const logPath = join(import.meta.dir, "capture-react-reference.log");
