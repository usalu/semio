...
- `#[cfg(test)] mod tests` with `🔖FacadeTests` (unit tests for the façade's own contract) and
  `🔖GoldenHarness` (the DrawList-parity acceptance gate).
...
1. **Duplicate `fn action()`** — I had defined a test helper `fn action() -> ActionDescriptor` twice
   in `mod tests` (once under `🔖FacadeTests`, once under `🔖GoldenHarness` — both are just comment
...
`.repo/🎫/26/07/17/GENERALIZE-RASTER-SURFACE-KIND-TO-PAINT2D` ticket touching related types). Flagging
...
Location: `#[cfg(test)] mod tests` inside `engine`, sub-region `🔖GoldenHarness`. Approach: each of
