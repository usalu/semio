...
session, with 40-48 concurrent `cargo build`/`bun script.ts dev`/`parity triage` processes observed
...
`findFreeParityPortPair()`; (2) `bun ./script.ts plugin <variant>` silently dropped the variant filter
