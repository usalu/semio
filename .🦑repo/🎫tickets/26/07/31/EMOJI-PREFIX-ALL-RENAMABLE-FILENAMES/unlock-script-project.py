#!/usr/bin/env python3
"""Rename script.ts → 📜script.ts and project.json → 📋project.json; path-aware text rewrites."""
import json
import os
import sys
...
def rewrite_text():
    ...
    safe = [
        ("bun ./📜script.ts", "bun ./📜script.ts"),
        ("./📜script.ts", "./📜script.ts"),
        ('"script.ts"', '"📜script.ts"'),
        ("**/📜script.ts", "**/📜script.ts"),
        ("base !== \"script.ts\"", "base !== \"📜script.ts\""),
        ("expected script.ts, got", "expected 📜script.ts, got"),
        ("ent.name === \"script.ts\"", "ent.name === \"📜script.ts\""),
        ("endsWith(\"/script.ts\")", "endsWith(\"/📜script.ts\")"),
        ("endsWith('/📜script.ts')", "endsWith('/📜script.ts')"),
        ("=== \"script.ts\"", "=== \"📜script.ts\""),
        ("=== 'script.ts'", "=== '📜script.ts'"),
        ("name !== \"script.ts\"", "name !== \"📜script.ts\""),
        ("includes(\"script.ts\")", "includes(\"📜script.ts\")"),
        ("entry.name !== \"project.json\"", "entry.name !== \"📋project.json\""),
        ("/project.json", "/📋project.json"),
        ("{projectRoot}/project.json", "{projectRoot}/📋project.json"),
        ("glob '**/project.json'", "glob '**/📋project.json'"),
        ("glob \"**/project.json\"", "glob \"**/📋project.json\""),
    ]
    changed = 0
    for dirpath, dirs, files in os.walk(".", topdown=True):
