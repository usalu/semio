...
1. **Read the ENTIRE original `lib.rs` file first**, including deep into any `//#region 🧪Tests`/nested sub-regions — test regions are often much larger than the initial `#region` marker suggests (in `note` a whole extra `🔖DslAndOpText` sub-region with ~10 more tests sat past where a naive line-count estimate would stop). Missing this means silently losing test coverage.
