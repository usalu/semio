...
- `writer/rs/project.json` `test` target already just called `bun ./script.ts test` (no change needed there).
...
- `writer/rs/project.json` — reviewed only, not modified (already `bun ./script.ts test`, no `test-e2e` needed).
- `.repo/🎫/26/07/17/OVERHAUL-TEST-SUITES-TO-30S-BUDGET-PER-APP/writer-rs-scratch/build.log` — left in place as evidence of the stalled isolated-build attempt (the isolated `target/` directory itself was deleted to reclaim disk space).
- `.repo/🎫/26/07/17/OVERHAUL-TEST-SUITES-TO-30S-BUDGET-PER-APP/report-semio-tech-writer-rs.md` — this report.
