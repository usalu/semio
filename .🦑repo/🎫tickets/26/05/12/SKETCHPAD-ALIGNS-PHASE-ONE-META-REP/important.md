...
- **Done (this pass):** `compose/js/index.ts` rewritten to plan regions: `🌐Transport` (GqlTransport + EventBus + worker wiring), `🧬Entity` (Entity + defineField/defineOperation/defineFields/defineOperations), `🧱Classes` (Kit + all entity classes + weak-entity interfaces), `🚀PublicAPI` (`openKit`), `🧪Tests` (negative grep + Piece.drag stub + EventBus).
