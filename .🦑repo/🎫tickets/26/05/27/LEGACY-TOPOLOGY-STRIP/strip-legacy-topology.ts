...
text = text.replace(/const MODEL_ENTITY_KINDS = new Set<string>\(\[[^\]]+\]\);/, 'const MODEL_ENTITY_KINDS = new Set<string>(["anchor", "vertex", "edge", "wire", "face", "solid", "object", "geometry", "attribute"]);');
...
text = text.replace(/export type CellComplexRecordDiff[\s\S]*?export type ClusterRecordDiff[\s\S]*?;\n\n/, "");
...
text = text.replace(/    const cell = model\.cells\[row\.geometryRef\];/, "    const cell = model.solids[row.geometryRef];");
...
text = text.replace(/if \(defn\.id === "selection\.selectAnchors" \|\| defn\.id === "selection\.selectCellComplexes" \|\| defn\.id === "selection\.selectClusters"\)/, 'if (defn.id === "selection.selectAnchors")');
...
schemaText = schemaText.replace(/"required": \["anchors", "vertices", "edges", "wires", "faces", "shells", "cells", "cellComplexes", "clusters"\]/, '"required": ["anchors", "vertices", "edges", "wires", "faces", "shells", "solids"]');
...
is = is.replace(/"enum": \[[\s\S]*?"anchor"\s*\]/, `"enum": ["vertex", "edge", "wire", "face", "solid", "anchor"]`);
