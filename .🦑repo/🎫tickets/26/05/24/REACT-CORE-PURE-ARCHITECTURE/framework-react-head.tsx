// #region ­ƒº▓Header
/** @emoji ÔÜø´©Å `@elements/framework-react` ÔÇö React renderer for {@link @elements/framework}: declarative {@link UiNode} host (monolith). */
// #endregion ­ƒº▓Header

export type { Workbench } from "@elements/framework";

export type { Level } from "@elements/ui";
export { LevelProvider, useLevel, getLevelBgClass, getLevelHoverClass, getLevelActiveHoverClass, getLevelZClass, getLevelBorderElementClass, getLevelDivideElementClass } from "@elements/ui";

//#region ­ƒôªshell-chrome-types.tsx
import type * as React from "react";

/** @emoji ­ƒæú Footer row rendered by the workbench shell. */
...
/** @emoji ­ƒî▓ Minimal tree panel payload for declarative side tabs. */
...
/** @emoji ­ƒôæ Side panel tab registration consumed by {@link WorkbenchView}. */
...
/** @emoji ­ƒôÉ Floating window measure/control descriptors (golden-layout chrome). */
...
/** @emoji ­ƒ¬ƒ Golden-layout window kind registration. */
...
/** @emoji ­ƒº░ Toolbar item for a single category slot. */
...
/** @emoji ­ƒº░ Toolbar category ids shared by framework shell tool maps. */
...
/** @emoji ­ƒùé´©Å Per-category toolbar tools for the workbench shell. */
export type AppTools = Partial<Record<AppToolCategory, readonly UIToolbarItem[]>>;

//#endregion ­ƒôªshell-chrome-types.tsx

//#region ­ƒôªworkbench-app-context.tsx
/** @emoji ­ƒº¡ Props for {@link WorkbenchView} (navbar, panels, golden-layout canvas). */
...
/** @emoji ­ƒº¡ @deprecated Use {@link WorkbenchViewProps}. */
...
import type { UiBoardHostSurfaceNode, UiButtonNode, UiNode, UiPanelHostSurfaceNode, UiTableHostSurfaceNode, UiScene3DHostSurfaceNode, UiSeparatorNode, UiStackNode, UiTextNode } from "@elements/framework";
...
function renderText(node: UiTextNode): React.ReactElement {
  const dataProps = node.dataAttributes ? Object.fromEntries(Object.entries(node.dataAttributes).map(([k, v]) => [`data-${k}`, v])) : {};
  return (
    <span className={cn("text-muted-foreground px-1 text-xs", node.emphasize && "font-semibold uppercase tracking-wide")} {...dataProps}>
      {node.value}
    </span>
  );
}
...
function renderSeparator(_node: UiSeparatorNode, horizontalParent: boolean): React.ReactElement {
  return <span role="separator" className={cn("shrink-0 bg-border", horizontalParent ? "mx-1 h-4 w-px self-center" : "my-1 h-px w-full")} aria-hidden />;
}
...
function renderPanel(node: UiPanelHostSurfaceNode): React.ReactElement {
  ...
  if (!Host) {
    return <div className="flex flex-1 items-center justify-center p-4 text-xs text-muted-foreground">Unsupported panel surface &quot;{node.surfaceId}&quot;</div>;
  }
  return (
    <div className="relative min-h-0 min-w-0 flex-1 overflow-auto">
      <Host node={node} />
...
function renderNode(node: UiNode, commandBus: CommandBus, horizontalParent: boolean): React.ReactElement {
  switch (node.type) {
    ...
    default:
      return <div className="p-2 text-xs text-destructive">Unsupported UiNode {(node as { type?: string }).type ?? "unknown"}</div>;
  }
}
...
export function shellWindowKindsToGolden(windowKinds: readonly WorkbenchWindowKind[], bus: CommandBus): UIWindowKindDefinition[] {
  ...
  return windowKinds.map((wk) => {
    ...
    const Body = windowBodyByKey.get(wk.bodyKey) ?? (() => <div className="p-2 text-xs text-muted-foreground">Missing window body &quot;{wk.bodyKey}&quot;</div>);
...
export function shellSideTabsToPanelTabs(tabs: readonly ShellSideTabSpec[], bus: CommandBus): SidePanelTabConfig[] {
  ...
  return tabs.map((tab, orderIndex) => {
    ...
    const Body = declarativeFactory ? getDeclarativeSidePanelBodyComponent(tab.id, tab.bodyKey) : (sidePanelBodyByKey.get(tab.bodyKey) ?? (() => <div className="p-2 text-xs">Missing panel {tab.bodyKey}</div>));
    return {
      id: tab.id,
...
const AppPanelStatePreview: React.FC<{
  ...
  if (!hasAppPanelValue(value)) {
    return (
      <div data-testid={`${testId}.empty`} className="text-sm text-muted-foreground">
        {emptyMessage}
      </div>
    );
  }
...
const UIPanelToggleGroup: React.FC<{
  ...
  <div data-slot="app-panel-toggle-group" className="flex items-stretch border border-element overflow-hidden h-medium">
    {items.map((item, index) => (
      <Toggle key={item.id} kind="icon" id={item.id} pressed={item.pressed} onPressedChange={item.onPressedChange} className={cn("border-0 rounded-none", index > 0 && "border-l")} icon={item.icon} />
    ))}
  </div>
);
...
export const WorkbenchView: React.FC<WorkbenchViewProps> = ({
  ...
  const mergedTools = React.useMemo(() => mergeAppTools(shellToolsToAppTools(workbench.globalTools, workbench.commandBus), shellToolsToAppTools(activeApp.tools, workbench.commandBus)), [activeApp.tools, workbench, shellGen]);
  ...
  const mergedFooterItems = [...shellFooterToFooterItems(workbench.globalFooterItems, workbench.commandBus), ...shellFooterToFooterItems(activeApp.footerItems, workbench.commandBus), ...(extraFooterItems ?? [])].sort(
  ...
  const goldenWindowKinds = React.useMemo(() => resolvedWindowKindsOverride ?? shellWindowKindsToGolden(activeApp.windowKinds, workbench.commandBus), [activeApp.windowKinds, resolvedWindowKindsOverride, workbench.commandBus]);
...
if (import.meta.vitest) {
  ...
  describe("WorkbenchView", () => {
    it("synthesizes default panel toggles for a single-app workbench", () => {
      ...
      const app = new WorkbenchApp("test", "Test", undefined, new TCtrl(), createTabStackLayout(["main"], ["Main"]), [new WorkbenchWindowKind("main", "Main", "test.workbench-view.main")]);
    ...
    it("merges appwide tools, selection, options, and window kinds with the active mode", () => {
      ...
      const app = new WorkbenchApp("app", "App", undefined, new TCtrl(), createTabStackLayout(["base"], ["Base"]), [new WorkbenchWindowKind("base", "Base", "test.workbench-view.base")]);
    ...
    it("renders a leading mode dropdown when an app has multiple modes", () => {
      ...
      const app = new WorkbenchApp("app", "App", undefined, new TCtrl(), createTabStackLayout(["main"], ["Main"]), [new WorkbenchWindowKind("main", "Main", "test.workbench-view.mm.main")]);
