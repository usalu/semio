...
- **Tests run:** `bun ./script.ts test` in `elements/lib/framework/core` — 7 passed. Board package Vitest fails on pre-existing duplicate exports in `index.tsx` (unrelated).
