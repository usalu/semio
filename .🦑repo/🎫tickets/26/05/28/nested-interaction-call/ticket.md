...
- **Shape hubs** — `surface.construct` / `curve.construct` callable interactions (sync via `bun ./script.ts sync-shape-construct`); typology construct calls `surface.construct` with full output map.
