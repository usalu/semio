...
for (const { from, to, playImport, reactImport, fixturePrefix } of moves) {
  ...
  c = c.replace(/from "@framework\/playground-renderer-react"/g, 'from "../index.tsx"');
  c = c.replace(
    /^\/\/ #region 🧲Header[\s\S]*?\/\/ #endregion 🧲Header\n\n/,
    `// #region 🧲Header\n/** @emoji 🛝 Puzzle play React chrome in \`@semio-tech/framework-playground-core-renderer-react\` (not in play packages). */\n// #endregion 🧲Header\n\n`,
  );
