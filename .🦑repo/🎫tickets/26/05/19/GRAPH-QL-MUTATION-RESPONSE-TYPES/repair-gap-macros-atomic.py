"""Atomically repair schema_gap_surfaces name-list/register macros in lib.rs."""
import re
...
if not fam_m or not relay_m:
    raise SystemExit("could not extract @names lists")
family_names = fam_m.group(1).strip()
relay_names = relay_m.group(1).strip()
if not family_names or not relay_names:
    raise SystemExit("empty name lists")
...
pat = re.compile(
    r"    macro_rules! define_gap_surface_families_from_list \{.*?"
...
if n != 1:
    pat2 = re.compile(
        r"    macro_rules! gap_surface_families \{.*?"
        r"with_gap_surface_existing_relay_names!\(gap_surface_existing_relays\);\s*",
        re.DOTALL,
    )
    new_text, n = pat2.subn(block, text, count=1)
if n != 1:
    raise SystemExit(f"replace failed count={n}")

# Ensure mod closes once
new_text = re.sub(
    r"(with_gap_surface_existing_relay_names!\(gap_surface_existing_relays\);\s*)\}\s*\}\s*(//#endregion)",
    r"\1}\n\2",
    new_text,
    count=1,
)
