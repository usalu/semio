...
- Rebuilt CLI binary at `repo/cli/cli` so extension runtime uses patched parser.
...
- Ran `./repo/cli/cli goal tree` initially and hit: `json: cannot unmarshal object into Go struct field Interaction.interactions.author of type string`.
- Verified widespread legacy payloads via grep for object-form `"author": { ... }` in `.repo/tickets`.
...
- Rebuilt CLI binary:
  - `go build -o cli main.go` in `repo/cli`.
- Re-verified CLI data loading:
  - `./repo/cli/cli goal tree` (pass).
  - `./repo/cli/cli --json graphql 'query Goals { ... }'` (pass).
  - `./repo/cli/cli --json graphql 'query RepoStructure { ... }'` (returns data without parse error).
