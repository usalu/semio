...
- `repo/cli/main.go`: Fixed `SectionIdValueToUriPath` to call `PathToUriPath(filePath)` instead of using raw file path. Fixed `DefinitionIdValueToUriPath` in all three branches (no hash/paragraph, paragraph only, hash+paragraph) to call `PathToUriPath(filePath)`.
- `repo/cli/main_test.go`: Updated `TestSectionIdValueToUriPath` expectations: "no hash", "single section", "nested sections" — file paths now uppercase. Updated `TestDefinitionIdValueToUriPath` expectations: "no hash", "with section and def", "def only" — file paths now uppercase.
