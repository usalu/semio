...
Fixed two issues preventing Windsurf hooks from calling repo CLI: (1) Created missing .windsurf/hooks.json with all 10 Windsurf hook events mapped to CLI commands. (2) Fixed exit code 2 bug: hookCommand returned pointer &ExitError{Code:2} but main() used errors.As with value type ExitError, causing match failure and fallthrough to os.Exit(1). Changed to ExitError{Code:2}. (3) Updated generateWindsurfConfig with show_output fields. All 27+ hook tests pass. Blocked commands (git checkout, git stash, git reset --hard, git clean -fd) verified returning exit code 2.
...
- **Fixed** `repo/cli/main.go` line 32975 — Changed `&ExitError{Code: 2}` to `ExitError{Code: 2}` so `errors.As` in `main()` matches and `os.Exit(2)` is called (required by Windsurf to block pre-hook actions).
- **Updated** `repo/cli/main.go` `generateWindsurfConfig` — Added `show_output` fields to all hook entries.
