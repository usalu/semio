...
| File                    | Lines  | Role                                                                    |
| ----------------------- | ------ | ----------------------------------------------------------------------- |
| `repo/cli/main.go`      | 40,523 | Monolithic CLI — contains ALL hook types, dispatch, config gen, command |
| `repo/cli/main_test.go` | 17,659 | All tests including hook tests                                          |
| `repo/go/events.go`     | 291    | Shared event kinds for CLI→server communication (EventKind)             |
| `repo/go/emit.go`       | 51     | HTTP POST helper to send events to repo server                          |

### 2. Hook Configuration Files (Generated)

| File                      | Client        | Format                                                                                |
| ------------------------- | ------------- | ------------------------------------------------------------------------------------- |
| `.github/hooks/repo.json` | copilot-chat  | JSON: `{hooks: {EventName: [{type,command,timeout}]}}`                                |
...
### B. Shared EventKind (`repo/go/events.go` L17-47)
...
| Line  | Name                      | Purpose                                                  |
| ----- | ------------------------- | -------------------------------------------------------- |
| 35420 | `logHook()`               | Audit log to `.repo/📜/🪝/🤖/<session>/<ts>_<kind>.json` |
...
| 9953  | `CopilotEditorProvider`     | copilot-chat → `.github/hooks/repo.json`                                                                                    |
...
Editor Native Event (e.g. Copilot "PreToolUse") + stdin JSON payload
    ↓
./repo/cli/cli hook <nativeEvent> <client> [--tool-name X] [--tool-args Y]
    ↓
...
RunHook(hctx)
    ...
    ├─ logHook(hctx, result) → writes .repo/📜/🪝/🤖/<session>/<ts>_<kind>.json
