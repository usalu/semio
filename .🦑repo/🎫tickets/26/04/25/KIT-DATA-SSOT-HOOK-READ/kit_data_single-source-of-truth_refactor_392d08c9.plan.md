...
todos:
 ...
 - id: rs_compose_command_port
   content: "compose/rs: port every handler from @semio-tech/compose-js composeKitCommandHandlers (~28222–28664) into typed KitStoreCommand variants + impls."
   status: pending
 - id: js_strip_domain
 ...
 - id: react_scopes
   ...
   status: completed
 - id: react_hooks_thin
   content: "compose/react: rewrite every per-field hook as a 1:1 forwarder to the matching @semio-tech/compose-js entity store; read-only computed hooks return [value, status]; writable hooks return [value, setValue, status]. Rename useFlatPiecePlane -> usePieceFlatPlane, useFlatPieceCenter -> usePieceFlatCenter. Delete useMemo+@semio-tech/compose-js derivations (useIncludedDesigns, useReplacableTypes, useReplacableDesigns, useExplodeableDesignNodes, usePieceParentConnection)."
   status: in_progress
 ...
 - id: sketchpad_provider_tree
   ...
   status: pending
 - id: package_cleanup
   content: "@semio-tech/compose-js removed from sketchpad/package.json (done). Add eslint (or nx) rule forbidding @semio-tech/compose-js in sketchpad sources; audit react/js dep lists."
   status: in_progress
 - id: tests_and_verify
   content: cargo test (rs, incl. wasm module); pnpm -F @semio-tech/compose-js test (per-entity stores); pnpm -F @semio-tech/compose-react test (Scopes + hooks + draft/rollback); pnpm -F @semio-tech/compose-sketchpad test (Playwright — kit boot temporary/file/folder/remote, piece-name edit, undo/redo, file blob, no @semio-tech/compose-js import); desktop smoke on metabolism.zip.
   status: pending
isProject: false
---
...
3. **compose/react** exports clean Scope components (`KitScope`, `PieceScope`, `TypeScope`, `DesignScope`, `ConnectionScope`, `AuthorScope`, `QualityScope`, `FileScope`, `FolderScope`, `PortScope`, `ConnectorScope`, `RepresentationScope`, `FamilyScope`, `TagScope`, `ConceptScope`, `LayerScope`, `GroupScope`, `StatScope`, `PropScope`, `AttributeScope`) and per-field hooks (`usePieceFlatPlane`, `usePieceFlatCenter`, `useConnectionGap`, `useKitName`, …). Hooks forward 1:1 to the corresponding `*Store` method in `@semio-tech/compose-js`. No `useMemo`+find, no `@semio-tech/compose-js` helper imports other than pure value types, no command semantics, no `useKitCommands`/`executeKitCommand`/`useKitCommandDispatchersWithOrigin` exports. Read-only computed hooks return `[value, status]` pair; writable field hooks return `[value, setValue, status]` triad (unchanged).
4. **compose/sketchpad** contains ZERO kit data, zero kit stores, zero kit hooks, zero command strings. Only UI-selection / tools / panels / navigation state lives in its `SketchpadStore` + `sketchpadMachine`. Every kit read/write happens in a component under a `<PieceScope id=…>` (or sibling) via a `@semio-tech/compose-react` hook.
...
1. Audit every domain helper currently in `@semio-tech/compose-js` and add the matching per-entity method + `Read*Command` variant + `Read*CommandOutput` variant + execute arm. Concretely:
...
## Phase R2 — strip computed/diff accessors off `@semio-tech/compose-js` value-type classes
...
Result: `@semio-tech/compose-js` value types are pure POJO wrappers. Any caller still importing these helpers is refactored in Phases J1, K, and S.

## Phase J1 — add per-entity TS stores in `@semio-tech/compose-js`
...
## Phase K1 — thin-wrapperize `@semio-tech/compose-react`
...
All edits in [compose/sketchpad/index.tsx](compose/sketchpad/index.tsx). Delete the following (each redefined locally today — kill the local definition, add import from `@semio-tech/compose-react`):
...
All now resolve to `@semio-tech/compose-react` exports (with renamed `usePieceFlatPlane` / `usePieceFlatCenter`).
...
4. Replace dynamic `require("@semio-tech/compose-js")` / `import("@semio-tech/compose-js")` at 47386 / 47552 (sql-related import/export path) with a call into `@semio-tech/compose-react` `useKitExport()` / `useKitImport()` that forwards to a rs-side `KitStoreCommand::ExportAs { format }` / `ImportFrom { … }`.

## Phase S4 — provider tree & machine

All edits in [compose/sketchpad/index.tsx](compose/sketchpad/index.tsx).

1. Root: wrap `Sketchpad` in `<KitRegistryProvider>` (already exists in react); for each active tab mount `<KitScope kitId={guid} backbone={…}>`. Inside the tab, the design panel wraps `<DesignScope id={designId}>`, piece inspector wraps `<PieceScope id={pieceId}>`, etc. Every per-field input reads/writes via `@semio-tech/compose-react` hooks in its local Scope.
2. `sketchpadMachine` (8312) keeps `openKitGuids` / `activeKitGuid` (8679–8681) and `UI.OPEN_KIT.PUSH|CLOSE|ACTIVATE` events (7674–7676, 8612–8632). Add any new UI events introduced to replace kit-mutation command strings. The machine invokes `fromPromise` actors that call `kitRegistry.open*` from `@semio-tech/compose-react`.

## Phase P — package / dist cleanup

- [compose/sketchpad/package.json](compose/sketchpad/package.json): drop `@semio-tech/compose-js` from direct deps (sketchpad no longer imports any js symbol). Keep `@semio-tech/compose-react`, `@compose/ui`.
- [compose/react/package.json](compose/react/package.json): `@semio-tech/compose-js` stays (peer + dev), not `@semio-tech/compose-sketchpad`.
...
2. `pnpm -F @semio-tech/compose-js test`: per-entity TS stores (`PieceStore.flatPlane()` returns Plane, `setFlatPlane` does not exist as it is read-only, `PieceStore.setName()` routes to `ChangePieceCommand::Name`).
3. `pnpm -F @semio-tech/compose-react test`: `<PieceScope>` + `usePieceFlatPlane()` returns `[plane, status]`; `<PieceScope>` + `usePieceName()` returns `[name, setName, status]`; rollback on rejection.
4. `pnpm -F @semio-tech/compose-sketchpad test`: existing Playwright regions pass after the rewire; kit boot (temporary/file/folder/remote) via `useKitRegistry`; piece-name input edit+commit+rollback; no `@semio-tech/compose-js` import in the bundle (add an eslint rule).
...
- Renaming the existing `KitStore` interface in `@semio-tech/compose-js` to `KitPersistenceStore` is optional; if too invasive, keep the interface name and give the new per-entity class a different name (e.g. `KitRef`).
