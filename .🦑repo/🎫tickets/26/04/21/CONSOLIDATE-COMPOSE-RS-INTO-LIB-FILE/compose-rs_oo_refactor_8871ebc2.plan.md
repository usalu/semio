...
todos:
 ...
 - id: side_entity
   ...
   status: completed
 - id: delete_smells
   content: Delete DesignStore::delete_pieces_and_connections_ref, DesignStore::invalidate_piece_pose_caches, PieceStore::bubble_pose_invalidation_to_design, KitStore::compose_type_mut, KitStore::design_mut, diff.rs _keep_arc.
   status: completed
 - id: oo_factory
 ...
 - id: setters_everywhere
   ...
   status: completed
 - id: complete_flatten
   content: Implement DesignStore::flatten with BFS + ConnectionStore::compute_child_plane mirroring compose/py/main.py computeChildPlaneDict; PieceStore::flat_plane/flat_center delegate to it.
   status: completed
 - id: complete_diff
 ...
 - id: tests
   ...
   status: completed
 - id: verify
   content: cargo build (host + wasm32-unknown-unknown), cargo test --lib -p compose, spot-check kit round-trip determinism (from_json_str(s).to_json_pretty() == s, hash stable).
   status: completed
isProject: false
---

# compose-rs OO refactor
...
Add to [`compose/rs/src/hash.rs`](compose/rs/src/hash.rs) (or new `cache.rs`):
...
- [`compose/rs/src/diff.rs`](compose/rs/src/diff.rs): `_keep_arc`, `DesignStore::delete_pieces_and_connections_ref(&DesignStoreRef, ...)`.
- [`compose/rs/src/kit.rs`](compose/rs/src/kit.rs): `compose_type_mut`, `design_mut` (aliases of the read getter).
...
Replace the stub in [`compose/rs/src/piece.rs`](compose/rs/src/piece.rs) (`flat_plane` / `flat_center` copying the neighbour) with a proper design-level BFS cache:

- `DesignStore::flatten()` computes `HashMap<Guid, (Plane, Coord)>` by BFS over pieces, resolving parent/child connector via `Connection.connected/connecting`, then calling a private `ConnectionStore::compute_child_plane(&self, parent_plane: &Plane, parent_connector: &ConnectorStore, child_connector: &ConnectorStore) -> (Plane, Coord)` that mirrors [`compose/py/main.py`](compose/py/main.py) `computeChildPlaneDict` (gap / shift / rise / rotation / turn / tilt, port-anchored frame using `nalgebra`).
...
Move all change logic onto entities (it already lives in [`compose/rs/src/diff.rs`](compose/rs/src/diff.rs) as `impl DesignStore`, but it is incomplete). Each method takes `&mut self` on the owning entity:
...
- [`compose/rs/src/io/sqlite.rs`](compose/rs/src/io/sqlite.rs): `impl KitStore { pub fn save_sqlite(&self, path: &Path) -> Result<()>; pub fn load_sqlite(path: &Path) -> Result<KitStoreRef>; }` with a normalized schema, one table per entity (guid PK, JSON payload column + FK columns for graph relations). Uses `rusqlite` (already in `Cargo.toml`).
- [`compose/rs/src/io/zip.rs`](compose/rs/src/io/zip.rs): `impl KitStore { pub fn save_zip(&self, path: &Path) -> Result<()>; pub fn load_zip(path: &Path) -> Result<KitStoreRef>; }`. Bundle = `kit.json` + `assets/<file.guid>.<ext>` for representation payloads.
...
[`compose/rs/src/wasm.rs`](compose/rs/src/wasm.rs) keeps identical JS-visible names (`generateGuid`, `kitFromJson`, `kitToJson`, `kitValidate`, `kitsAreEqual`, `flattenDesign`, `composeRound`, `composeNormalizeName`) — bodies become one-liner delegations to the new OO API.

## Tests

Move inline tests into [`compose/rs/src/tests/`](compose/rs/src/tests/) (currently empty). New integration tests, one file per concern:
...
- No changes to `compose/py`, `compose/js`, `compose/hub`, or the WASM JS-visible names.
