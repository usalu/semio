---
name: compose rs new arch
overview: "Rebuild [compose/rs/lib.rs](compose/rs/lib.rs) as a clean greenfield skeleton matching [compose/graphql/target.schema.graphql](compose/graphql/target.schema.graphql): one struct per GraphQL type with a main impl + an `#[Object]` impl, all `pub fn` async, a single global `emit_event`, and a parent/child web-worker topology where the parent routes `Query.wip` and `Query.authoritative` to two child workers. Wire one end-to-end operation (`CreatedFixedPiece`) through the full path as the reference flow."
todos:
 - id: scaffold-modules
...
# compose/rs new architecture (greenfield skeleton + 1 end-to-end op)

All edits land inside [compose/rs/lib.rs](compose/rs/lib.rs). Nothing else (matches workspace rule: edit existing files, no new files outside ticket folders). [compose/rs/lib.old.rs](compose/rs/lib.old.rs) is left untouched as historical reference. Cargo deps (`async-graphql`, `async-broadcast`, `async-channel`, `async-executor`, `async-lock`, `futures-util`, `wasm-bindgen-futures`, `web-sys`) are already present in [compose/rs/Cargo.toml](compose/rs/Cargo.toml); add `web-sys` features `Worker`, `MessageEvent`, `DedicatedWorkerGlobalScope` (wasm32-only) — no new crates.
...
- `parses_target_schema()` — builds the schema and `assert!(schema.sdl()` contains the key types from [target.schema.graphql](compose/graphql/target.schema.graphql)).
...
- Porting change-commands, diff merge, flatten algorithm, hash computation, VCS persistence, backbone (dev/local/remote), conflict resolution from [compose/rs/lib.old.rs](compose/rs/lib.old.rs). These remain in `lib.old.rs` and will be ported in follow-up tickets onto this skeleton.
- All `Operation` types other than `CreatedFixedPiece` are skeleton-only.
- No JS bridge changes ([compose/js](compose/js)) — that follows the strict layering rule once the GraphQL surface stabilises.
