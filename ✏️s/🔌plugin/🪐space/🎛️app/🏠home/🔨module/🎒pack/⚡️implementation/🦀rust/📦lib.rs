//! 📦 S Home launcher app — binary document surface + laws (constitutional: pack).

use home::SHomeDocument;
use store::PackError;

/// 📦 Encodes an `SHomeDocument` to its binary pack form.
pub fn encode(document: &SHomeDocument) -> Vec<u8> {
    store::DocumentPack::encode_pack(document)
}

/// 📖 Decodes an `SHomeDocument` from its binary pack form.
