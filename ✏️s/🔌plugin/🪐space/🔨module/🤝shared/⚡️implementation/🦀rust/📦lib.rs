//! 🌱 S Studio plugin — fixtures + document helpers shared by the `home` and `space` apps
...
/// 🧵 Registers the draw/writer fixture documents referenced by the demo space's app instances —
...
/// @emoji 🌱 The demo space's bare `OsProjection` — the studio app's `initial_projection`, parsed
