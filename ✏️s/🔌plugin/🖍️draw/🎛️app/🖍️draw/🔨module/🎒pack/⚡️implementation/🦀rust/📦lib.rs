//! 📦 Draw app — binary document surface + laws (constitutional: pack).

use draw::DrawDocument;
use store::PackError;

/// 📦 Encodes a `DrawDocument` to its binary pack form.
pub fn encode(document: &DrawDocument) -> Vec<u8> {
    store::DocumentPack::encode_pack(document)
}

/// 📖 Decodes a `DrawDocument` from its binary pack form.
