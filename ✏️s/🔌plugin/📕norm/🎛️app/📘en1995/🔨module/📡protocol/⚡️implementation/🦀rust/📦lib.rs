//! ⚖️ EN 1995 app — binary command protocol surface + laws (constitutional: protocol).

use en1995_op::Operation;
use protocol::OpBinary;

/// 📦 Encodes an `Operation` to its binary command form.
pub fn encode_op(operation: &Operation) -> Result<Vec<u8>, protocol::ProtocolError> {
    operation.encode_op()
}

/// 📖 Decodes an `Operation` from its binary command form.
