...
//!
//! 🔀 `MasonryClass`/`part_2::ExposureClass`/`part_2::MortarClass` are Document field types, so they
...
// #region 🔖Part2
/// 🧱 EN 1996-2 selection of materials & execution: exposure-class durability admissibility and bed-joint execution checks. `ExposureClass`/`MortarClass` live in `en1996::part_2` (Document field types); this submodule holds only the compute functions.
pub mod part_2 {
    use super::*;
...
/// 🧮 Headless per-document evaluation — the `NormFamily::evaluate` body for `En1996Family` (defined
