//! 📦 Raster app — binary document surface + laws (constitutional: pack).

use raster::RasterProjection;
use store::PackError;

/// 📦 Encodes a `RasterProjection` to its binary pack form.
pub fn encode(document: &RasterProjection) -> Vec<u8> {
    store::DocumentPack::encode_pack(document)
}

/// 📖 Decodes a `RasterProjection` from its binary pack form.
