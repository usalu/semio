//! 🔗 Sequence plugin — declarative sequence play app bundled as a hot-swappable WASM plugin.

//#region 🔖WasmSession
/// 🌉 Direct-canvas WASM bridge — a bespoke JS API (`SequenceSession`) for driving the sequence DAG
/// canvas outside the generic `DocumentApp`/`PluginApp` render pipeline (raw GPU frame painting,
/// pointer/wheel routing). Lives in the plugin bundle rather than any constitutional app crate: it
/// isn't part of the entity/op/dsl/pack/protocol/ui split — it's WASM-only glue wired directly onto
/// `sequence_engine::SequenceHost`.
#[cfg(target_arch = "wasm32")]
mod wasm_session {
    use infinite_board_port_directed_dag::DagLayoutOptions;
    use sequence::{SequenceFixture, SlotRef};
    use sequence_engine::{sequence_camera_from_dag, SequenceHost};
