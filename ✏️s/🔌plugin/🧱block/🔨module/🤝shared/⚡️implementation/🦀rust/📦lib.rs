//! 🤝 Block plugin — record types shared by all three apps' entity (`rs`) crates (non-constitutional;
...
//#region 🔖Identity
/// 🪪 The single kind definition a block document edits — name/label/variant/description/icon/unit
...
/// 🔗 One allowed (or, unidirectional, one-way-allowed) compatibility pair between two handle/vortex/
...
/// 🧱 One representation (mesh at a LOD/tag combination) a kind ships with — compose's "Representation".
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize, dsl::DslRecord)]
#[serde(rename_all = "camelCase")]
...
//#region 🔖Meta
/// 📝 Free-text description carried alongside a block document (distinct from the kind's own
