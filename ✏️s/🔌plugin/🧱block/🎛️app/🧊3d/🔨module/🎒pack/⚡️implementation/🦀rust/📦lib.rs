//! 📦 Block 3D app — binary document surface + laws (constitutional: pack).

use block_3d::Block3dDefinition;
use store::PackError;

/// 📦 Encodes a `Block3dDefinition` to its binary pack form.
pub fn encode(document: &Block3dDefinition) -> Vec<u8> {
    store::DocumentPack::encode_pack(document)
}

/// 📖 Decodes a `Block3dDefinition` from its binary pack form.
