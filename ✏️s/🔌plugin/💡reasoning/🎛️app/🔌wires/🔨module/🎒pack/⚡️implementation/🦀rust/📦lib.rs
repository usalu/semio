//! 📦 Reasoning wires app — binary document surface + laws (constitutional: pack).

use reasoning_wires::MindmapWiresDocument;
use store::PackError;

/// 📦 Encodes a `MindmapWiresDocument` to its binary pack form.
pub fn encode(document: &MindmapWiresDocument) -> Vec<u8> {
    store::DocumentPack::encode_pack(document)
}

/// 📖 Decodes a `MindmapWiresDocument` from its binary pack form.
