...
pub fn fem2d_solve_all(doc: &Fem2dDocument) -> Result<HashMap<String, fem_core::StaticResult>, Fem2dError> {
    ...
    let combinations: Vec<fem_core::analyses::Combination> = doc.combinations.iter().map(|c| fem_core::analyses::Combination { id: c.id.clone(), terms: c.terms.iter().map(|t| (t.case_id.clone(), t.factor)).collect() }).collect();
...
mod tests {
    ...
    fn example_fixture_parses_and_solves() {
        ...
        let doc: Fem2dDocument = Fem2dDocument::parse_dsl(fem2d_dsl::FEM2D_EXAMPLE_TEXT).expect("example fixture parses");
