//! 📦 FEM 2D app — binary document surface + laws (constitutional: pack).

use fem2d::Fem2dDocument;
use store::PackError;

/// 📦 Encodes a `Fem2dDocument` to its binary pack form.
pub fn encode(document: &Fem2dDocument) -> Vec<u8> {
    store::DocumentPack::encode_pack(document)
}

/// 📖 Decodes a `Fem2dDocument` from its binary pack form.
