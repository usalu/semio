//! 📦 GIS 3D app — binary document surface + laws (constitutional: pack).

use gis3d::Gis3dTerrainDocument;
use store::PackError;

/// 📦 Encodes a `Gis3dTerrainDocument` to its binary pack form.
pub fn encode(document: &Gis3dTerrainDocument) -> Vec<u8> {
    store::DocumentPack::encode_pack(document)
}

/// 📖 Decodes a `Gis3dTerrainDocument` from its binary pack form.
