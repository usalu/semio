...
pub type Puzzle3dStore = store::DocumentStore<Puzzle3dProjection, Puzzle3dOperation>;
...
/// 🌱️ `puzzle-plugin`'s `Puzzle3dPlayApp` predates the typed `Puzzle3dProjection` above and stays on
...
/// same local-bridge shape as `puzzle_2d`'s `Puzzle2dPlayProjection` and `semio_compose_rs`'s `KitSnapshot`.
...
mod tests {
    ...
    /// 🪢️ Regression guard for the linear (`HashMap`-based) rewrite of `puzzle3d_collect_collection_delta`
