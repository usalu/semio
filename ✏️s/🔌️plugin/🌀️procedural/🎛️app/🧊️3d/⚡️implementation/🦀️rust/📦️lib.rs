//! 📐️ Procedural 3D app — document entities (constitutional: general).
...
/// 📜️ `.procedural3d` textual document — derive-engine grammar via `Procedural3dDocumentDsl`
...
/// 📦️ `.procedural3d` binary pack — same `Procedural3dDocumentDsl` mirror as `DocumentDsl` above (see
/// `🔖️DslMirror`); `dsl::DslDocument`'s derive already gives `Procedural3dDocumentDsl` its own
