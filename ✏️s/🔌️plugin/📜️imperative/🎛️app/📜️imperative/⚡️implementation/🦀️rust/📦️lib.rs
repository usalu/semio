...
//#region 🔖️Dsl
/// 🌱️ `Value`/`Atom`/`Dictionary`/`Step`/`Path` are all defined in `neural_engine`/`imperative_engine`
...
/// 📄️ Local mirror of `ImperativeDocument` — see the region's opening doc comment for why `path:
...
//#region 🔖️Pack
/// 📦️ Binary counterpart of the `store::DocumentDsl` impl above. `ImperativeDocument` carries a manual
