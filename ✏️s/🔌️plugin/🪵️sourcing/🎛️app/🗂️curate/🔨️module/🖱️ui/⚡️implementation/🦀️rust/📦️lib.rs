//! 🎨️ Sourcing curate app — `DocumentApp` impl, render, manifest (constitutional: ui).
...
struct ProgramContributionEntry {
    #[allow(dead_code, reason = "🧩️ present to match the manifest contribution entry JSON shape for Deserialize; never read after parsing, only `contribution` is")]
    plugin_id: String,
    contribution: Contribution,
}

/// 🧩️ One module's typology + catalogue kinds, resolved either from a contributed plugin or, as a
...
/// 🧩️ Contributed module catalogues if any plugin has contributed, else the built-in modules — so the
...
pub fn create_sourcing_curate_app() -> App {
    App::from_builder(
        App::builder(SOURCING_CURATE_APP_ID, "Curate")
            ...
            // 🔧️ Every curate edit — filters, sort, selection, curation counts — is persisted in the
