...
/** @emoji 🧭️ `@semio-tech/cad-js-kernel-brepjs` — `SpatialKernel` backed by brepjs + OpenCascade WASM. */
// #endregion 🧲️Header
...
function stepParseLineEndpoints(entities: ReadonlyMap<number, string>, lineRef: number): readonly [Vec3, Vec3] | null {
  ...
  const dirParts = inlineDir
    ...
    : (() => {
        ...
        return nestedMatch ? nestedMatch[1]!.split(",").map((value) => Number(value.trim().replace(/E\+/i, "e+"))) : [];
      })();
...
function modelFromStepLineCurve(start: Vec3, end: Vec3, options: { readonly prefix: string; readonly objectId: string; readonly typology: TypologyRef }): Model {
...
function modelFromStepShellSurface(points: readonly Vec3[], options: { readonly prefix: string; readonly objectId: string; readonly typology: TypologyRef }): Model {
  if (points.length < 3) return new Model();
  const model = new Model();
  const normal = polygonNormalNewell(points);
  const origin: Vec3 = [points.reduce((sum, point) => sum + point[0], 0) / points.length, points.reduce((sum, point) => sum + point[1], 0) / points.length, points.reduce((sum, point) => sum + point[2], 0) / points.length];
...
function wireframePartFromStepPresentationEntity(entities: ReadonlyMap<number, string>, entityRef: number, options: { readonly prefix: string; readonly objectId: string; readonly typology: TypologyRef }): Model | null {
  const body = entities.get(entityRef);
  if (!body) return null;
...
function importStepPresentationLayersToModel(stepText: string, options: { readonly basePrefix: string; readonly lengthScale: number; readonly modelDefinitionId: string }): Model {
...
/** @emoji 🪜️ Parses AP242 presentation layers and declared solid order from raw STEP text. */
export function stepPresentationLayers(stepText: string): {
  ...
  for (const [, body] of entities) {
    ...
    const solids = stepParseHashRefList(itemsMatch[1]!).filter((ref) => entities.get(ref)?.startsWith("MANIFOLD_SOLID_BREP("));
    if (solids.length > 0) {
      solidOrder = solids;
      break;
    }
...
/** @emoji 📏️ Raw Rhino/AP242 BREP STEP fixtures are authored in millimeters; CAD model space uses meters. */
export const STEP_BREP_IMPORT_MM_TO_M = 0.001;

/** @emoji 📐️ Uniformly scales vertex positions and length-bearing surface data on a model graph. */
...
function modelFromImportedBrepSolid(brepSolid: ValidSolid, options: { readonly prefix: string; readonly objectId: string; readonly typology?: TypologyRef; readonly modelDefinitionId?: string }): Model {
...
class BrepjsWasmEngine {
  ...
  /** @emoji 🪜️ Exports a `ModelSpace` to AP242 STEP (six pillars + brepjs solids). */
  async exportModelSpaceToStep(space: ModelSpace, modelSpaceId = "default"): Promise<string> {
    ...
    emitSpatialUdaProperty(writer, msPdId, "spatial.modelspace", JSON.stringify({ revision: space.revision, modelIds }), "System_Generated");
    ...
    for (const modelId of modelIds) {
      ...
      for (const [entityId, fields] of model.metadata.entries()) {
        emitSpatialUdaProperty(writer, modelPdId, `spatial.attribute.${entityId}`, JSON.stringify(fields), "Authored Attribute");
      }
      for (const obj of Object.values(model.objects)) {
        const objPdId = writer.emitNew(`PRODUCT_DEFINITION(${stepEscape(String(obj.id))}, ${stepEscape(String(obj.typology))}, #${formationId}, #${contextId})`);
        ...
        for (const defn of listApplicablePropertyDefinitionsForModelDefinition(modelId, model, obj)) {
          ...
          for (const [key, value] of Object.entries(derived)) {
            emitSpatialUdaProperty(writer, shapeId, `spatial.property.${String(obj.id)}.${key}`, JSON.stringify(value), "Derived Property");
          }
        }
      }
    }
  ...
  /** @emoji 🪜️ Exports one `Model` as a single-model `ModelSpace` STEP file. */
  ...
  async importStepBimToModelSpace(stepText: string, options?: { readonly prefix?: string; readonly lengthScale?: number; readonly modelDefinitionId?: string }): Promise<ModelSpace> {
    ...
    for (let i = 0; i < brepSolids.length; i++) {
      ...
      const prefix = `${basePrefix}${suffix}`;
      const objectId = `object-${prefix}`;
      mergeImportedBrepPart(model, modelFromImportedBrepSolid(brepSolids[i]!, { prefix, objectId, typology }));
    }
    scaleModelPositions(model, lengthScale);
    const space = new ModelSpace();
    space.link(modelDefinitionId, model);
  ...
  /** @emoji 🪜️ Imports AP242 STEP produced by `exportModelSpaceToStep` into a `ModelSpace`. */
...
export async function exportModelSpaceToDwg(space: ModelSpace, deflection = 0.1): Promise<Uint8Array> {
  ...
  const flowCore = (await import("../../../../../../../🧰️framework/🛍️product/💻️os/🔨️module/🌊️flow/🫀️core/pkg/⚡️implementation/🦀️rust/flow_core.js")) as { dwg_encode_mesh_json?: (meshJson: string) => string };
    ...
    const composeStepFixtures = [
      {
        ...
        file: "📐️hexagonal-cut-concrete-forest-left.stp",
      },
      {
        name: "hexagonal-cut-concrete-forest-right",
        file: "📐️hexagonal-cut-concrete-forest-right.stp",
      },
    ] as const;
    ...
    it("generates play fixtures from compose STEP sources", async () => {
      ...
      for (const fixture of composeStepFixtures) {
        ...
        if (fixture.name === "hexagonal-cut-concrete-forest-left") {
          const bimPath = resolve(fixtureRoot, "📐️hexagonal-cut-concrete-forest-left-bim.stp");
          const bimText = await readFile(bimPath, "utf8");
          const bimSpace = await kernel.importStepBimToModelSpace(bimText, {
            prefix: "hexagonal-cut-concrete-forest-left-bim",
            modelDefinitionId: AEC_BUILDING_MODEL_DEFINITION_ID,
          });
          const energyPath = resolve(fixtureRoot, "📐️hexagonal-cut-concrete-forest-left-energy.stp");
          ...
          const structurePath = resolve(fixtureRoot, "📐️hexagonal-cut-concrete-forest-left-classic-structure.stp");
          ...
          const outPath = resolve(playRoot, `${fixture.name}.🔣️model.json`);
