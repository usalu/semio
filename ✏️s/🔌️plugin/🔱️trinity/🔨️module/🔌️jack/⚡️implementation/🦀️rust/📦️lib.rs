...
/// 🔤️ Fine-grained jack token kind, shared with editor consumers (e.g. writer's outline/hover heuristics) that
/// need exact keyword/punctuation identity rather than the coarse [`TokenClass`] used for syntax highlighting.
#[derive(Clone, Debug, PartialEq)]
pub enum Token {
    KwMatch,
    KwWhere,
...
// #region 🔖️SpannedAst
/// 🌳️ Span-tracked jack AST node for editor tree/hover/outline surfaces (e.g. writer's document panel),
/// which need source positions the semantic [`Query`] AST from [`parse`] discards.
#[derive(Clone, Debug, PartialEq)]
...
/// 🌳️ Parse jack source into a span-tracked AST for hierarchy/outline panels (moved from writer's premigration
/// `parseJackAst`; lexing never fails — only parsing can, in which case a single `"error"` node spans the source).
...
pub fn execute(graph: &Graph, query: &Query) -> Result<(QueryResult, Vec<TrinityGraphOperation>), String> {
    ...
    for clause in &query.clauses {
        match clause {
            ...
            Clause::Create(pattern) => {
                ...
                fixture = apply_trinity_graph_operations(fixture, &batch).map_err(|e| e.to_string())?;
            ...
            Clause::Delete(vars) => {
                for var in vars {
                    if let Some(id) = bindings.first().and_then(|b| b.nodes.get(var).cloned()) {
                        let operation = TrinityGraphOperation::DeleteNode { id };
                        operations.push(operation.clone());
                        fixture = apply_trinity_graph_operations(fixture, std::slice::from_ref(&operation)).map_err(|e| e.to_string())?;
                        view = Graph::from_fixture(fixture.clone()).map_err(|e| e.to_string())?;
                    }
                }
            }
...
pub fn run(graph: &mut Graph, source: &str) -> Result<QueryResult, String> {
    ...
    if !operations.is_empty() {
        let fixture = apply_trinity_graph_operations(graph.to_fixture(), &operations).map_err(|e| e.to_string())?;
...
fn emit_create_operations(fixture: &GraphFixture, pattern: &Pattern) -> Result<Vec<TrinityGraphOperation>, String> {
...
mod tests {
    ...
    fn format_is_idempotent() {
        // 🩹️ this crate's `format()` delegates to `mathematical_graph_dsl::format`, which now
