...
use semio_framework_plugin::{
    ...
    ActionDefinition, ActionDescriptor, ActionKind, App, BlockListScene, ConfigView, DocumentApp, DocumentView, Emit, HostEffect, Label, LocalizedLabel, NodeGraphEdgeRecord, NodeGraphNodeRecord, NodeGraphPortRecord, NodeGraphScene,
...
impl DocumentApp for ArchitectApp {
    ...
    fn command_from_action(&self, action: &str, args: Option<&Value>) -> Result<ArchitectCommand, String> {
    ...
    fn handle(&self, command: &ArchitectCommand, doc: &DocumentView<'_, Program>, cfg: &ConfigView<'_, ArchitectConfig>) -> Result<Emit<ProgramOperation, ArchitectConfigOperation>, Fault> {
        ...
        match command {
            ...
            ArchitectCommand::AddRegisterItem { register_id, name, template_id } => {
                if let Some(template_id) = template_id {
                    ...
                    if let Some(template) = program.templates.iter().find(|row| row.header.id == template_id).cloned() {
                        let mut scratch = program.clone();
                        return Emit::operations(apply_template(&mut scratch, &template));
                    }
                }
            ...
            ArchitectCommand::ApplyTemplate { template_id } => {
                ...
                let mut scratch = program.clone();
                Emit::operations(apply_template(&mut scratch, &template))
            }
            ArchitectCommand::ExportRegistersCsv => {
                let csv = export_registers_csv(program).unwrap_or_default();
                Emit::effect(HostEffect::DownloadMediaExport { filename: format!("{}.registers.csv", program.meta.document_id), mime_type: "text/csv".into(), data: csv, encoding: None })
            }
            ArchitectCommand::ImportRegistersCsv { csv, strategy } => {
                ...
                Emit::operations(vec![ProgramOperation::SetProgram { program: Box::new(next_program) }])
            }
            ...
            ArchitectCommand::ExportProgram => {
                ...
                Emit::effect(HostEffect::DownloadMediaExport { filename: format!("{}.architect.dsl", program.meta.document_id), mime_type: "text/plain".into(), data: dsl_text, encoding: None })
            }
            ArchitectCommand::ImportProgramRequest => {
                Emit::effect(HostEffect::RequestFileOpen { accept: ".dsl,.architect.dsl,.spk,.ops,application/octet-stream,text/plain".into(), read_as: None, import_action: "importProgram".into(), multiple: false })
            }
            ...
            ArchitectCommand::SetAdjacencyKind { element_a_id, element_b_id, kind, cycle } => {
                ...
                match next_kind {
                    Some(kind) => {
                        ...
                        Emit::operations(vec![ProgramOperation::SetAdjacency { adjacency }])
                    }
                    None => {
                        if let Some(row) = existing {
                            Emit::operations(vec![ProgramOperation::ClearAdjacency { id: row.header.id.clone() }])
                        } else {
                            Emit::default()
                        }
                    }
