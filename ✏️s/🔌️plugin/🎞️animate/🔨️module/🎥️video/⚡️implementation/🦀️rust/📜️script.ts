#!/usr/bin/env bun
/** 🎥️ `@semio-tech/animate-video-rs` router: `bun ./📜️script.ts test|render|preview|flush-cache`. */
import { existsSync, rmSync } from "node:fs";
import { join } from "node:path";
import { BundleScript, ScriptRouter, runBundleScriptMain, runCargoTestBudgeted, runCmd } from "../../../../../../../🧰️framework/🛍️product/🦑️repo/🔨️module/📚️lib/⚡️implementation/🟦️typescript/📦️index.ts";

const TICKET_TARGET_REL = ".🦑️repo/🎫️tickets/26/07/18/ANIMATE-VIDEO-CLI-PRESENT-BRIDGE/target";
...
/** 🎛️ Parses animate video CLI flags from argv segments. */
...
class RenderScript extends RenderCliScript {
  run(segments: string[]): void {
    ...
    if (options.flushCache) {
      runCmd("bun", ["./📜️script.ts", "flush-cache"], { cwd: this.root, env: this.cargoEnv() });
    }
    if (options.preview) {
      runCmd("bun", ["./📜️script.ts", "preview", "--quality", options.quality, ...(options.scene ? ["--scene", options.scene] : [])], {
        cwd: this.root,
        env: this.cargoEnv(),
      });
      return;
