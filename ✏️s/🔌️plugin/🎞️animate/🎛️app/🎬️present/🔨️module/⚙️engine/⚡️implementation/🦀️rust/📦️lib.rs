//! ⚙️ Animate present app — headless compute (constitutional: engine).
...
pub mod compiler {
    ...
    mod tests {
        use super::*;
        use crate::{populate_tile_drafts_from_grid, FigureTileGridSeedSpec};
...
pub mod slide {
    ...
    pub struct PresentScene {
        ...
        pub deck: Option<present::PresentDeck>,
    }
...
pub fn empty_present_deck() -> present::PresentDeck {
    present::PresentDeck { schema: present::PRESENT_DECK_SCHEMA.into(), source: present::default_figure_tile_source(), tiles: Vec::new() }
}
//#endregion 🔖️Domain
...
pub struct SplitFigureGridSpec<'a> {
    pub rows: u32,
    pub columns: u32,
    pub frame: &'a present::FigureTileFrame,
    pub gap: f64,
...
pub struct SplitGridCell {
    pub key: String,
    pub crop: present::FigureTileFrame,
}

pub struct FigureTileGridSeedSpec<'a> {
    pub source: &'a present::FigureTileSource,
    pub rows: u32,
    pub columns: u32,
    pub gap: f64,
    pub key_prefix: &'a str,
...
pub fn clamp_tile_crop(crop: present::FigureTileFrame) -> present::FigureTileFrame {
    ...
    present::FigureTileFrame { x, y, width, height }
}
...
pub fn split_figure_grid(spec: SplitFigureGridSpec<'_>) -> Vec<SplitGridCell> {
    ...
    for row in 0..rows {
        for column in 0..columns {
            cells.push(SplitGridCell { key: format!("{}-r{row}-c{column}", spec.key_prefix), crop: present::FigureTileFrame { x: frame.x + column as f64 * crop_width, y: frame.y + row as f64 * crop_height, width: crop_width, height: crop_height } });
        }
    }
...
pub fn populate_tile_drafts_from_grid(spec: FigureTileGridSeedSpec<'_>) -> Vec<present::FigureTileDraft> {
    split_figure_grid(SplitFigureGridSpec { rows: spec.rows, columns: spec.columns, frame: &spec.source.frame, gap: spec.gap, key_prefix: spec.key_prefix })
        .into_iter()
        .map(|cell| present::FigureTileDraft { id: cell.key.clone(), name: cell.key, crop: cell.crop })
        .collect()
}

pub fn build_tile_morph_prompt(source: &present::FigureTileSource, drafts: &[present::FigureTileDraft]) -> String {
    fn format_frame(frame: &present::FigureTileFrame) -> String {
...
pub fn animate_present_document_json_from_dwg(drawing: &semio_framework_core::DwgDrawing) -> Result<serde_json::Value, String> {
    ...
    let deck = present::PresentDeck {
        schema: present::PRESENT_DECK_SCHEMA.into(),
        source: present::FigureTileSource { src: format!("data:image/png;base64,{png_base64}"), kind: "image".into(), frame: frame.clone(), source_aspect: Some(width as f64 / height.max(1) as f64), pdf_page: None },
        tiles: vec![present::FigureTileDraft { id: "imported-drawing".into(), name: "Imported Drawing".into(), crop: frame }],
    };
...
mod tests {
    use super::*;
    use present::default_figure_tile_source;
    use serde_json::json;
    ...
    fn morph_prompt_lists_tiles() {
        let source = default_figure_tile_source();
        let tiles = vec![present::FigureTileDraft { id: "t1".into(), name: "t1".into(), crop: present::FigureTileFrame { x: 0.1, y: 0.1, width: 0.2, height: 0.2 } }];
    ...
    fn from_dwg_builds_single_slide_deck_from_entity() {
        ...
        assert_eq!(deck.schema, present::PRESENT_DECK_SCHEMA);
