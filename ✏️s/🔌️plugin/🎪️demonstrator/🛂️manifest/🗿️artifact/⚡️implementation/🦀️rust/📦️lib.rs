...
//! `semio_plugin!` — this crate owns no `🎛️app/` of its own; every app it registers is a
//! transitively-shared dependency of its own source plugin crate, same pattern as `s`'s bundle, see
//! `✏️s/🔌️plugin/🪐️space/🛂️manifest/🗿️artifact/⚡️implementation/🦀️rust/📦️lib.rs`). Each source
