...
//! operations (constitutional: general). The actual algorithms live in sibling `remodel_image`/
//! `remodel_video`/`remodel_camera`/`remodel_feature`/`remodel_sfm`/`remodel_dense`/`remodel_mesh`/
//! `remodel_motion`/`remodel_geo`/`remodel_engine` crates, none of which this crate depends on:
...
//! reference to their shape below is a plain-JSON (or `Packed*`) snapshot the plugin runtime fills
//! in, never the library type itself.
