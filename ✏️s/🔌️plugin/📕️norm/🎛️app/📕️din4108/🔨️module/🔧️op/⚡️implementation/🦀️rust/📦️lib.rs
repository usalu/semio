...
//! so `Operation` is a re-export of `norm_core`'s generic `SetDocumentOperation<Document>`, which
//! already carries its own `Operation`/`OpText`/`OpBinary` impls — nothing to implement here.
