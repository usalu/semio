...
use iso16757::{CatalogueValue, Document};
use norm_core::{AnnexChoice, CheckReport, CheckResult, ClauseId, NormError, Quantity, QuantityKind};
...
pub mod part_5 {
    ...
    pub fn build_ifc_catalogue(catalogue: &iso16757::part_1::Catalogue) -> IfcCatalogue {
    ...
    pub fn validate_exchange(catalogue: &iso16757::part_1::Catalogue, ifc: &IfcCatalogue) -> Vec<String> {
        let mut issues = Vec::new();
...
pub mod io {
    ...
    pub fn catalogue_to_json(catalogue: &iso16757::part_1::Catalogue) -> Result<String, NormError> {
        serde_json::to_string_pretty(catalogue).map_err(|e| NormError::InvalidValue { field: "catalogue".into(), reason: e.to_string() })
    }

    pub fn catalogue_from_json(json: &str) -> Result<iso16757::part_1::Catalogue, NormError> {
        serde_json::from_str(json).map_err(|e| NormError::InvalidValue { field: "catalogue".into(), reason: e.to_string() })
    }

    pub fn dictionary_to_json(dictionary: &iso16757::part_4::Dictionary) -> Result<String, NormError> {
...
mod tests {
    ...
    fn part_number_script_is_deterministic() {
        let runtime = part_5::DefaultScriptRuntime;
        let rule = iso16757::part_5::PartNumberRule::Script { function_id: "partno".into(), source: "dn * 10 + 50".into() };
    ...
    fn composition_cycle_detected() {
        ...
        doc.catalogue.compositions.insert("product.a".into(), vec![iso16757::part_1::CompositionRelationship { component_product_id: "product.b".into(), quantity: 1 }]);
        doc.catalogue.compositions.insert("product.b".into(), vec![iso16757::part_1::CompositionRelationship { component_product_id: "product.a".into(), quantity: 1 }]);
    ...
    fn script_rejects_forbidden_import() {
        let runtime = part_5::DefaultScriptRuntime;
        let err = runtime.execute("import fs", &HashMap::new(), iso16757::part_5::ScriptLimits::default()).unwrap_err();
        assert!(matches!(err, iso16757::part_5::ScriptError::InvalidExpression(_)));
    }

    #[test]
    fn evaluate_constraint_operators() {
        let dec = |v: f64| CatalogueValue::Decimal { value: v };
        let mk = |op, value| iso16757::part_1::SelectionConstraint { property_id: "p".into(), operator: op, value };
        assert!(part_1::evaluate_constraint(&dec(5.0), &mk(iso16757::part_1::ConstraintOperator::NotEqual, dec(6.0))));
        assert!(!part_1::evaluate_constraint(&dec(5.0), &mk(iso16757::part_1::ConstraintOperator::NotEqual, dec(5.0))));
        assert!(part_1::evaluate_constraint(&dec(5.0), &mk(iso16757::part_1::ConstraintOperator::LessThan, dec(6.0))));
        assert!(!part_1::evaluate_constraint(&dec(5.0), &mk(iso16757::part_1::ConstraintOperator::LessThan, dec(4.0))));
        assert!(part_1::evaluate_constraint(&dec(5.0), &mk(iso16757::part_1::ConstraintOperator::GreaterThan, dec(4.0))));
        assert!(!part_1::evaluate_constraint(&dec(5.0), &mk(iso16757::part_1::ConstraintOperator::GreaterThan, dec(6.0))));
        let range = CatalogueValue::Range { min: 1.0, max: 10.0, unit: None };
        assert!(part_1::evaluate_constraint(&dec(5.0), &mk(iso16757::part_1::ConstraintOperator::InRange, range.clone())));
        assert!(!part_1::evaluate_constraint(&dec(50.0), &mk(iso16757::part_1::ConstraintOperator::InRange, range)));
    }
    ...
    fn validate_catalogue_structure_flags_issues() {
        ...
        let mut doc = Document::default();
        doc.catalogue.compositions.insert("product.cv".into(), vec![iso16757::part_1::CompositionRelationship { component_product_id: "product.cv".into(), quantity: 1 }]);
        let issues = part_1::validate_catalogue_structure(&doc.catalogue);
    ...
    fn substitute_parameters_recurses_through_node_kinds() {
        let primitive = iso16757::part_2::GeometryNode::Primitive { kind: "box".into(), parameters: BTreeMap::from([("width".into(), 1.0)]) };
        let transform = iso16757::part_2::GeometryNode::Transform { translation: [1.0, 0.0, 0.0], rotation_deg: [0.0, 0.0, 0.0], child: Box::new(primitive.clone()) };
        let boolean = iso16757::part_2::GeometryNode::Boolean { operator: iso16757::part_2::BooleanOperator::Union, children: vec![primitive.clone(), transform.clone()] };
        let reference = iso16757::part_2::GeometryNode::Reference { geometry_id: "geom.x".into() };

        let values = HashMap::from([("width".into(), 2.0)]);
        match part_2::substitute_parameters(&primitive, &values) {
            iso16757::part_2::GeometryNode::Primitive { parameters, .. } => assert_eq!(parameters["width"], 2.0),
            _ => panic!("expected primitive"),
        }
        match part_2::substitute_parameters(&transform, &values) {
            iso16757::part_2::GeometryNode::Transform { child, .. } => match *child {
                iso16757::part_2::GeometryNode::Primitive { parameters, .. } => assert_eq!(parameters["width"], 2.0),
        ...
        match part_2::substitute_parameters(&boolean, &values) {
            iso16757::part_2::GeometryNode::Boolean { children, .. } => assert_eq!(children.len(), 2),
            _ => panic!("expected boolean"),
        }
        match part_2::substitute_parameters(&reference, &values) {
            iso16757::part_2::GeometryNode::Reference { geometry_id } => assert_eq!(geometry_id, "geom.x"),
    ...
    fn evaluate_bounding_box_error_paths() {
        ...
        let missing_width = iso16757::part_2::GeometryNode::Primitive { kind: "box".into(), parameters: BTreeMap::new() };
        assert!(matches!(part_2::evaluate_bounding_box(&missing_width, &catalogue), Err(NormError::IncompleteInput { field }) if field == "width"));

        let unknown_kind = iso16757::part_2::GeometryNode::Primitive { kind: "cone".into(), parameters: BTreeMap::new() };
        assert!(matches!(part_2::evaluate_bounding_box(&unknown_kind, &catalogue), Err(NormError::OutOfScope { .. })));

        let empty_boolean = iso16757::part_2::GeometryNode::Boolean { operator: iso16757::part_2::BooleanOperator::Union, children: Vec::new() };
        ...
        let unresolved_ref = iso16757::part_2::GeometryNode::Reference { geometry_id: "missing".into() };
    ...
    fn evaluate_bounding_box_cylinder_sphere_boolean_transform() {
        let catalogue = iso16757::part_2::GeometryCatalogue::default();
        let cylinder = iso16757::part_2::GeometryNode::Primitive { kind: "cylinder".into(), parameters: BTreeMap::from([("radius".into(), 1.0), ("height".into(), 2.0)]) };
        let bbox = part_2::evaluate_bounding_box(&cylinder, &catalogue).expect("cylinder bbox");
        assert_eq!(bbox.max, [2.0, 2.0, 2.0]);

        let sphere = iso16757::part_2::GeometryNode::Primitive { kind: "sphere".into(), parameters: BTreeMap::from([("radius".into(), 1.5)]) };
        let bbox = part_2::evaluate_bounding_box(&sphere, &catalogue).expect("sphere bbox");
        assert_eq!(bbox.max, [3.0, 3.0, 3.0]);

        let a = iso16757::part_2::GeometryNode::Primitive { kind: "box".into(), parameters: BTreeMap::from([("width".into(), 1.0), ("height".into(), 1.0), ("depth".into(), 1.0)]) };
        let b = iso16757::part_2::GeometryNode::Transform {
            translation: [5.0, 5.0, 5.0],
            rotation_deg: [0.0, 0.0, 0.0],
            child: Box::new(iso16757::part_2::GeometryNode::Primitive { kind: "box".into(), parameters: BTreeMap::from([("width".into(), 1.0), ("height".into(), 1.0), ("depth".into(), 1.0)]) }),
        };
        let boolean = iso16757::part_2::GeometryNode::Boolean { operator: iso16757::part_2::BooleanOperator::Union, children: vec![a, b] };
        ...
        objects.insert(
            "geom.ref".to_string(),
            iso16757::part_2::GeometryObject {
                id: "geom.ref".into(),
                shape: Some(iso16757::part_2::GeometryNode::Primitive { kind: "box".into(), parameters: BTreeMap::from([("width".into(), 1.0), ("height".into(), 1.0), ("depth".into(), 1.0)]) }),
                symbolic: None,
                spaces: Vec::new(),
                surfaces: Vec::new(),
                ports: Vec::new(),
        ...
        let ref_catalogue = iso16757::part_2::GeometryCatalogue { objects, primitive_registry: Vec::new() };
        let reference = iso16757::part_2::GeometryNode::Reference { geometry_id: "geom.ref".into() };
    ...
    fn validate_geometry_graph_self_reference_and_cycle() {
        let mut objects = BTreeMap::new();
        let self_ref = iso16757::part_2::GeometryObject {
            id: "geom.self".into(),
            shape: Some(iso16757::part_2::GeometryNode::Reference { geometry_id: "geom.self".into() }),
            symbolic: None,
            spaces: Vec::new(),
            surfaces: Vec::new(),
            ports: Vec::new(),
        ...
        let catalogue = iso16757::part_2::GeometryCatalogue { objects, primitive_registry: Vec::new() };
        let mut visited = HashSet::new();
        ...
        let mut objects = BTreeMap::new();
        let a = iso16757::part_2::GeometryObject {
            id: "geom.a".into(),
            shape: Some(iso16757::part_2::GeometryNode::Reference { geometry_id: "geom.b".into() }),
            symbolic: None,
            spaces: Vec::new(),
            surfaces: Vec::new(),
            ports: Vec::new(),
        ...
        let b = iso16757::part_2::GeometryObject {
            id: "geom.b".into(),
            shape: Some(iso16757::part_2::GeometryNode::Reference { geometry_id: "geom.a".into() }),
            symbolic: None,
            spaces: Vec::new(),
            surfaces: Vec::new(),
            ports: Vec::new(),
    ...
    fn subtype_closure_is_transitive() {
        let dictionary = iso16757::part_4::Dictionary {
            reference: iso16757::DictionaryRef { id: "d".into(), version: "1".into() },
            subjects: Vec::new(),
            relationships: vec![
                iso16757::part_4::Relationship { id: "r1".into(), kind: iso16757::part_4::RelationshipKind::IsSubtypeOf, source_id: "a".into(), target_id: "b".into(), cardinality: iso16757::Cardinality::optional() },
                iso16757::part_4::Relationship { id: "r2".into(), kind: iso16757::part_4::RelationshipKind::IsSubtypeOf, source_id: "b".into(), target_id: "c".into(), cardinality: iso16757::Cardinality::optional() },
            ],
            properties: Vec::new(),
    ...
    fn detect_subtype_cycle_true() {
        let subject = |id: &str| iso16757::part_4::Subject {
            id: id.into(),
            kind: iso16757::part_4::SubjectKind::ProductClass,
            names: iso16757::Names { preferred: iso16757::LocalizedText { locale: "en".into(), text: id.into() }, short_name: None, alternatives: Vec::new() },
            definition: iso16757::LocalizedText { locale: "en".into(), text: String::new() },
            parent_id: None,
        };
        let dictionary = iso16757::part_4::Dictionary {
            reference: iso16757::DictionaryRef { id: "d".into(), version: "1".into() },
            ...
            relationships: vec![
                iso16757::part_4::Relationship { id: "r1".into(), kind: iso16757::part_4::RelationshipKind::IsSubtypeOf, source_id: "a".into(), target_id: "b".into(), cardinality: iso16757::Cardinality::optional() },
                iso16757::part_4::Relationship { id: "r2".into(), kind: iso16757::part_4::RelationshipKind::IsSubtypeOf, source_id: "b".into(), target_id: "a".into(), cardinality: iso16757::Cardinality::optional() },
            ],
            properties: Vec::new(),
            controlled_lists: Vec::new(),
            meta_subjects: Vec::new(),
    ...
    fn validate_dictionary_flags_dangling_and_cardinality_review() {
        let mut doc = Document::default();
        doc.dictionary.relationships.push(iso16757::part_4::Relationship {
            id: "r.dangling".into(),
            kind: iso16757::part_4::RelationshipKind::IsDependentOn,
            source_id: "subject.valve".into(),
            target_id: "subject.unknown".into(),
            cardinality: iso16757::Cardinality::optional(),
        });
        doc.dictionary.relationships.push(iso16757::part_4::Relationship {
            id: "r.cardinality".into(),
            kind: iso16757::part_4::RelationshipKind::HasPart,
            source_id: "subject.valve".into(),
            target_id: "subject.valve".into(),
            cardinality: iso16757::Cardinality { min: 2, max: Some(3) },
        });
    ...
    fn calculate_part_number_table_rule_paths() {
        ...
        let rule = iso16757::part_5::PartNumberRule::Table { rows: rows.clone(), output_column: "code".into() };
        let inputs = BTreeMap::from([("dn".into(), CatalogueValue::Decimal { value: 50.0 })]);
        ...
        let literal_rule = iso16757::part_5::PartNumberRule::Literal { value: "LIT-1".into() };
    ...
    fn script_runtime_timeout() {
        let runtime = part_5::DefaultScriptRuntime;
        let limits = iso16757::part_5::ScriptLimits { max_steps: 100, max_recursion: 10, timeout_ms: 0 };
        let err = runtime.execute("1 + 1", &HashMap::new(), limits).unwrap_err();
        assert!(matches!(err, iso16757::part_5::ScriptError::Timeout(0)));
    }

    #[test]
    fn script_runtime_limit_errors() {
        let runtime = part_5::DefaultScriptRuntime;
        let recursion_limits = iso16757::part_5::ScriptLimits { max_steps: 1000, max_recursion: 2, timeout_ms: 5_000 };
        let err = runtime.execute("(((1)))", &HashMap::new(), recursion_limits).unwrap_err();
        assert!(matches!(err, iso16757::part_5::ScriptError::RecursionLimit(2)));

        let step_limits = iso16757::part_5::ScriptLimits { max_steps: 1, max_recursion: 100, timeout_ms: 5_000 };
        let err = runtime.execute("1+1", &HashMap::new(), step_limits).unwrap_err();
        assert!(matches!(err, iso16757::part_5::ScriptError::StepLimit(0)));

        let err = runtime.execute("unknownVar", &HashMap::new(), iso16757::part_5::ScriptLimits::default()).unwrap_err();
        assert!(matches!(err, iso16757::part_5::ScriptError::InvalidExpression(ref e) if e == "unknownVar"));
    }
    ...
    fn script_runtime_arithmetic_operators() {
        ...
        let result = runtime.execute("(10 - 4) / 2 * 3", &inputs, iso16757::part_5::ScriptLimits::default()).expect("arithmetic");
