...
use norm_core::{NormError, QuantityKind};
...
/// 🔒️ A `QuantityKind` tag mirroring `norm_core::QuantityKind`'s 19 variants, kept locally: the DSL
...
/// 🔧️ `dsl::DslValue`'s `Number` variant is f64-only (no int/float distinction, per its own doc
