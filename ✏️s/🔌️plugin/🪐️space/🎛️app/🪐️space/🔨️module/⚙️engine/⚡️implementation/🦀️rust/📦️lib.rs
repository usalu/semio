...
/// @emoji ✨️ Builds the `AddWorkflowNode` operation (minting a fresh node — id, ports, document/config
/// refs, everything — via `workflow_node_for_app`, so replay never re-derives it) plus the new node's
/// id for the caller to focus. The store-free operation-builder the plugin uses in place of os-core's
/// `OsStore::add_workflow_node` (a `DocumentApp` owns no store — its wrapper does). Renamed from the
/// pre-merge `spawn_app_instance_operation`: a node IS the app instance now, there is no separate
/// `OsAppInstance` to mint alongside it.
pub fn add_workflow_node_operation(plugin_id: &str, app_id: &str, label: Option<&str>, x: f64, y: f64) -> Option<(OsOperation, String)> {
    ...
    Some((OsOperation::AddWorkflowNode { node }, node_id))
}
...
/// (`"connectMediaPorts"` and the `nodeGraphEdit`/`"connect"` fixture edit) so neither can push an
/// `OsOperation::ConnectWorkflowPorts` for an incompatible or unresolved pair of ports. Operates
...
pub fn flatten_media_vfs_rows(
    ...
    for child in &children {
        ...
        if child.has_children {
            flatten_media_vfs_rows(&child.id, graph, bindings, parameters, rows);
        }
    }
}

pub fn vfs_node_to_row(node: &semio_framework_os::OsWorkflowVfsNodeRecord) -> Value {
    json!({
        "id": node.id,
