...
fn catalog_shot_item(id: &str, label: impl Into<Label>, format: &str, shape: &str) -> UiTreeItemNode {
    crate::apps::shooting::tree_item_with_icon(format!("shooting-play-catalogue.{id}"), label, "camera", crate::apps::shooting::shooting_action("addShot", Some(json!({ "format": format, "shape": shape }))))
}
...
pub fn render(labels: &ShootingLabels) -> UiNode {
    ...
    let asset_items = vec![crate::apps::shooting::tree_item_with_icon("shooting-play-catalogue.asset.glb", labels.glb_asset, "box", crate::apps::shooting::shooting_action("addAsset", Some(json!({ "format": "glb" }))))];
