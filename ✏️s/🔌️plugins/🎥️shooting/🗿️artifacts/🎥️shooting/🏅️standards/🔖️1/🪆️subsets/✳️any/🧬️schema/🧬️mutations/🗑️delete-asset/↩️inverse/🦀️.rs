...
pub async fn inverse(payload: &DeleteAsset, base: &ShootingSnapshot) -> Vec<ShootingMutation> {
    match base.assets.iter().position(|asset| asset.id == payload.id) {
        Some(index) => vec![ShootingMutation::CreateAsset(crate::artifacts::shooting::mutations::create_asset::CreateAsset { asset: base.assets[index].clone(), index: Some(index) })],
        None => Vec::new(),
    }
}
