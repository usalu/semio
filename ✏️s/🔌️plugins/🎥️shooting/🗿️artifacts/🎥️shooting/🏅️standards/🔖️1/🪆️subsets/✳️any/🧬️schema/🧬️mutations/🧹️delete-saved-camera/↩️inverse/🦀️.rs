...
pub async fn inverse(payload: &DeleteSavedCamera, base: &ShootingSnapshot) -> Vec<ShootingMutation> {
    match base.saved_cameras.iter().position(|entry| entry.id == payload.id) {
        Some(index) => vec![ShootingMutation::CreateSavedCamera(crate::artifacts::shooting::mutations::create_saved_camera::CreateSavedCamera { saved_camera: base.saved_cameras[index].clone(), index: Some(index) })],
        None => Vec::new(),
    }
}
