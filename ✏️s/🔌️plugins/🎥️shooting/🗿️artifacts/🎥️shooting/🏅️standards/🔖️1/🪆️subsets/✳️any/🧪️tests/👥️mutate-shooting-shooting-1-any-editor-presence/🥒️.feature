@capability-shooting-shooting-1-any-editor-presence-mutate
@no-oracle-shooting-shooting-1-any-editor-presence-state-lane-semantics
@comparison-ordered-json-v1
@mutations-shooting-shooting-1-any-editor-presence
Feature: Apply every presence state-lane mutation of s.shooting.shooting's ✏️editor/👥️presence to its committed vector
  `ShootingPresenceMutation` is the presence lane of this subset's editor: it holds the shot selection and live camera this collaborator broadcasts to peers. It is this
  repository's own state record, so the case records the no-oracle decision `shooting-shooting-1-any-editor-presence-state-lane-semantics` in
  ...
  before-snapshot. Production dispatch is reached through `shooting_presence_mutation_report_json`, which runs
  ...
  Scenario Outline: Apply <id> and land on the committed after-snapshot, diff and outcome
    ...
    When <id> is applied to that vector's before-snapshot through shooting_presence_mutation_report_json
