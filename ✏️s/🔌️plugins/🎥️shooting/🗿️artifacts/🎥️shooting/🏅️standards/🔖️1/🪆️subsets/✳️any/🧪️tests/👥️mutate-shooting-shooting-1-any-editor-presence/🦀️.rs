//! 👥️ `s.shooting.shooting` ✏️editor/👥️presence state-lane mutation case — Rust adapter.
//!
//! Recorded no-oracle decision `shooting-shooting-1-any-editor-presence-state-lane-semantics`: the runner dispatches no oracle role, so every law is asserted inside
//! the subject handlers through `semio_s_plugin_stdio_test_oracle::law::vector` over the report of this crate's
//! production bridge `shooting_presence_mutation_report_json`. The oracle handlers answer with the committed after- and before-snapshots read
...
//#region 🔖️Vectors
/// 🧫️ The committed applied vector of one kind, read literally from `✏️editor/👥️presence/🧫️fixtures`.
...
mod subject {
    ...
    use semio_s_artifact_shooting_shooting::editor::shooting::presence::shooting_presence_mutation_report_json;

    fn report(committed: &Vector) -> Result<String, String> {
        shooting_presence_mutation_report_json(committed.before, committed.mutation, committed.after)
    }
