@capability-shooting-shooting-1-any-editor-config-mutate
@no-oracle-shooting-shooting-1-any-editor-config-state-lane-semantics
@comparison-ordered-json-v1
@mutations-shooting-shooting-1-any-editor-config
Feature: Apply every config state-lane mutation of s.shooting.shooting's ✏️editor/🎚️config to its committed vector
  `ShootingConfigMutation` is the config lane of this subset's editor: it holds the default shot and asset formats, the shot selection, the centre-model toggle, the viewport fit revision, the camera-label draft and the live camera. It is this
  repository's own state record, so the case records the no-oracle decision `shooting-shooting-1-any-editor-config-state-lane-semantics` in
  ...
  before-snapshot. Production dispatch is reached through `shooting_config_mutation_report_json`, which runs
  ...
  Scenario Outline: Apply <id> and land on the committed after-snapshot, diff and outcome
    ...
    When <id> is applied to that vector's before-snapshot through shooting_config_mutation_report_json
