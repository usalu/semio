...
//!
//! `store::DocumentPack for ShootingSnapshot` is implemented directly on the artifact type (see
...
/// 📦️ Encodes a `ShootingSnapshot` to its binary pack form.
pub fn encode(snapshot: &ShootingSnapshot) -> Vec<u8> {
    store::DocumentPack::encode_pack(snapshot)
}

/// 📖️ Decodes a `ShootingSnapshot` from its binary pack form.
pub fn decode(bytes: &[u8]) -> Result<ShootingSnapshot, PackError> {
...
mod tests {
    ...
    fn pack_round_trips_representative_fixture() {
        store::os_store::test_support::assert_dsl_pack_equivalence(&representative_snapshot());
        let bytes = encode(&representative_snapshot());
        assert_eq!(decode(&bytes).expect("decode"), representative_snapshot());
    }

    #[test]
    fn pack_round_trips_empty_fixture() {
        store::os_store::test_support::assert_dsl_pack_equivalence(&crate::artifacts::shooting::empty_shooting_snapshot());
    }

    #[test]
    fn pack_round_trips_base_icon_example_and_agrees_with_dsl() {
        let snapshot = dsl::parse_dsl(dsl::SHOOTING_EXAMPLE_TEXT).expect("base-icon example parses");
    ...
    fn command_envelope_round_trip_holds_for_an_applied_operation() {
        ...
        let mut store: DocumentStore<ShootingSnapshot, ShootingMutation> = DocumentStore::new(create_document_envelope(SHOOTING_DOCUMENT_SCHEMA, "shooting", crate::artifacts::shooting::empty_shooting_snapshot(), None));
        store
        ...
        store::os_store::test_support::assert_command_envelope_round_trip::<ShootingSnapshot, ShootingMutation>(edit, &DocumentId(store.envelope().id.clone()), &SchemaId(store.envelope().schema.clone()));
    }
