...
where
    PA: semio_framework_plugin::PluginApp
        + From<semio_framework_plugin::VcsArtifactApp<semio_framework_plugin::EditorApp<editor::remodeling::RemodelingPlayApp>>>
...
fn apply_mesh_chunk(mesh: &mut MeshData, last_field: Option<u8>, bytes: &[u8]) -> bool {
    ...
    match field {
        0 | 1 | 2 | 4 | 7 | 9 if values.len() % 4 == 0 => {
            ...
            target.extend(values.as_chunks::<4>().0.iter().map(|value| f32::from_le_bytes(*value)));
        }
        3 | 5 | 6 | 8 if values.len() % 4 == 0 => {
            ...
            target.extend(values.as_chunks::<4>().0.iter().map(|value| u32::from_le_bytes(*value)));
        }
        10 => {
