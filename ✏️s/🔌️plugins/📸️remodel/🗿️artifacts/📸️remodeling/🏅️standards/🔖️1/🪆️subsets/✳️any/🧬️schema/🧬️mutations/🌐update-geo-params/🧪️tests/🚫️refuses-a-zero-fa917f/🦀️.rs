//! 🧪️ `update-geo-params` fixture — `🚫️refuses-a-zero-fa917f`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `update-geo-params-invariant` is this vector's scenario id in
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌐update-geo-params/🚫️refuses-a-zero-fa917f/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌐update-geo-params/🚫️refuses-a-zero-fa917f/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌐update-geo-params/🚫️refuses-a-zero-fa917f/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌐update-geo-params/🚫️refuses-a-zero-fa917f/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `update-geo-params` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "update-geo-params/refuses-a-zero-fa917f: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "update-geo-params/refuses-a-zero-fa917f: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "update-geo-params/refuses-a-zero-fa917f declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "update-geo-params/refuses-a-zero-fa917f: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "update-geo-params/refuses-a-zero-fa917f: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.invariant", "update-geo-params/refuses-a-zero-fa917f: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Fatal, "update-geo-params/refuses-a-zero-fa917f: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "update-geo-params/refuses-a-zero-fa917f: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🌐update-geo-params/🚫️refuses-a-zero-fa917f/🔺️diff/🚫️.absent").is_empty(), "update-geo-params/refuses-a-zero-fa917f: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "update-geo-params/refuses-a-zero-fa917f: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "update-geo-params/refuses-a-zero-fa917f: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "update-geo-params/refuses-a-zero-fa917f: committed mutation JSON is not canonical");
}
