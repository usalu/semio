//! 🧪️ `create-rig-extrinsic` fixture — `🚫️refuses-a-second-95e04d`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `create-rig-extrinsic-duplicate` is this vector's scenario id in
...
//!
//! 🏞️ duplicate-id is checked before the unknown-camera invariant
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⛓️create-rig-extrinsic/🚫️refuses-a-second-95e04d/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⛓️create-rig-extrinsic/🚫️refuses-a-second-95e04d/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⛓️create-rig-extrinsic/🚫️refuses-a-second-95e04d/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⛓️create-rig-extrinsic/🚫️refuses-a-second-95e04d/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `create-rig-extrinsic` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "create-rig-extrinsic/refuses-a-second-95e04d: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "create-rig-extrinsic/refuses-a-second-95e04d: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "create-rig-extrinsic/refuses-a-second-95e04d declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "create-rig-extrinsic/refuses-a-second-95e04d: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "create-rig-extrinsic/refuses-a-second-95e04d: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.duplicate-id", "create-rig-extrinsic/refuses-a-second-95e04d: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Fatal, "create-rig-extrinsic/refuses-a-second-95e04d: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "create-rig-extrinsic/refuses-a-second-95e04d: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/⛓️create-rig-extrinsic/🚫️refuses-a-second-95e04d/🔺️diff/🚫️.absent").is_empty(), "create-rig-extrinsic/refuses-a-second-95e04d: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "create-rig-extrinsic/refuses-a-second-95e04d: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "create-rig-extrinsic/refuses-a-second-95e04d: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "create-rig-extrinsic/refuses-a-second-95e04d: committed mutation JSON is not canonical");
}
