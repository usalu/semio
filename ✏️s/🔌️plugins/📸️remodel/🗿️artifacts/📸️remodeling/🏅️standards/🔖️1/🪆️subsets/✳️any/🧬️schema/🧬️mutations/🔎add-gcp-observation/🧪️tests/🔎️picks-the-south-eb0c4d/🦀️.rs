//! 🧪️ `add-gcp-observation` fixture — `🔎️picks-the-south-eb0c4d`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `add-gcp-observation-realworld` is this vector's scenario id in
...
//!
//! 🏞️ a third pick strengthens a two-view control point into a three-view one
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔎add-gcp-observation/🔎️picks-the-south-eb0c4d/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔎add-gcp-observation/🔎️picks-the-south-eb0c4d/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔎add-gcp-observation/🔎️picks-the-south-eb0c4d/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔎add-gcp-observation/🔎️picks-the-south-eb0c4d/🎯️outcome/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔎add-gcp-observation/🔎️picks-the-south-eb0c4d/🔺️diff/🔣️.json");
...
async fn reaches_the_committed_after_document() {
    ...
    assert_eq!(applied, expected_after(), "add-gcp-observation/picks-the-south-eb0c4d: applied state differs from committed after-snapshot");
    assert_ne!(applied, before(), "add-gcp-observation/picks-the-south-eb0c4d: an applied vector must move the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(json_of(outcome.diff()), committed, "add-gcp-observation/picks-the-south-eb0c4d: produced diff differs from the committed 🔺️diff/🔣️.json");
    let decoded: RemodelingDiff = pack::from_json_str(DIFF).expect("committed diff decodes into the diff type");
    assert_eq!(json_of(&decoded), committed, "add-gcp-observation/picks-the-south-eb0c4d: committed diff JSON is not canonical");
}
...
async fn committed_diff_carries_before_to_after() {
    ...
    assert_eq!(applied, expected_after(), "add-gcp-observation/picks-the-south-eb0c4d: committed diff did not carry before to after");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "add-gcp-observation/picks-the-south-eb0c4d declares an applied outcome");
    ...
    assert_eq!(emitted, declared_codes, "add-gcp-observation/picks-the-south-eb0c4d: emitted diagnostics differ from the declared ones");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "add-gcp-observation/picks-the-south-eb0c4d: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "add-gcp-observation/picks-the-south-eb0c4d: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "add-gcp-observation/picks-the-south-eb0c4d: committed mutation JSON is not canonical");
}
