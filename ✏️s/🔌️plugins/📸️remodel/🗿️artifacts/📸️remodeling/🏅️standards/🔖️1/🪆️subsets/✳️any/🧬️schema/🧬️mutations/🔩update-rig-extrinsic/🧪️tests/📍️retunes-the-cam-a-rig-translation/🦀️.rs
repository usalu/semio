//! 🧪️ `update-rig-extrinsic` fixture — `📍️retunes-the-cam-a-rig-translation`.
//!
...
async fn inverse_is_the_same_verb_carrying_the_base_pose() {
    ...
    assert!(
        matches!(inverse.as_slice(), [RemodelingMutation::UpdateRigExtrinsic(payload)] if payload.extrinsic.camera_id == "cam-a" && payload.extrinsic.translation_m == [0.0, 0.0, 0.0]),
...
async fn produces_committed_diff() {
    ...
    assert_eq!(encoded, committed, "update-rig-extrinsic/retunes-the-cam-a-rig-translation: produced diff differs from the committed 🔺️diff/🔣️.json");
