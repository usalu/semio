//! 🧪️ `replace-trajectory` fixture — `🛣️swaps-in-a-three-49b17f`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `replace-trajectory-realworld` is this vector's scenario id in
...
//!
//! 🏞️ a shorter solve replaces the whole pose list, positions included
...
async fn reaches_the_committed_after_document() {
    ...
    assert_eq!(applied, expected_after(), "replace-trajectory/swaps-in-a-three-49b17f: applied state differs from committed after-snapshot");
    assert_ne!(applied, before(), "replace-trajectory/swaps-in-a-three-49b17f: an applied vector must move the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(json_of(outcome.diff()), committed, "replace-trajectory/swaps-in-a-three-49b17f: produced diff differs from the committed 🔺️diff/🔣️.json");
    let decoded: RemodelingDiff = pack::from_json_str(DIFF).expect("committed diff decodes into the diff type");
    assert_eq!(json_of(&decoded), committed, "replace-trajectory/swaps-in-a-three-49b17f: committed diff JSON is not canonical");
}
...
async fn committed_diff_carries_before_to_after() {
    ...
    assert_eq!(applied, expected_after(), "replace-trajectory/swaps-in-a-three-49b17f: committed diff did not carry before to after");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "replace-trajectory/swaps-in-a-three-49b17f declares an applied outcome");
    ...
    assert_eq!(emitted, declared_codes, "replace-trajectory/swaps-in-a-three-49b17f: emitted diagnostics differ from the declared ones");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "replace-trajectory/swaps-in-a-three-49b17f: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "replace-trajectory/swaps-in-a-three-49b17f: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "replace-trajectory/swaps-in-a-three-49b17f: committed mutation JSON is not canonical");
}
