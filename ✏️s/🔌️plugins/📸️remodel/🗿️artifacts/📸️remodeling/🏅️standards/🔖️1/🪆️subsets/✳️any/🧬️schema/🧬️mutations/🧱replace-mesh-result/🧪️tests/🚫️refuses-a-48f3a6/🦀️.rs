...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱replace-mesh-result/🚫️refuses-a-48f3a6/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱replace-mesh-result/🚫️refuses-a-48f3a6/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱replace-mesh-result/🚫️refuses-a-48f3a6/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱replace-mesh-result/🚫️refuses-a-48f3a6/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `replace-mesh-result` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "replace-mesh-result/refuses-a-48f3a6: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "replace-mesh-result/refuses-a-48f3a6: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "replace-mesh-result/refuses-a-48f3a6 declares a rejected outcome");
    ...
    assert_eq!(declared_path, messages[0].target, "replace-mesh-result/refuses-a-48f3a6: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱replace-mesh-result/🚫️refuses-a-48f3a6/🔺️diff/🚫️.absent").is_empty(), "replace-mesh-result/refuses-a-48f3a6: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "replace-mesh-result/refuses-a-48f3a6: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "replace-mesh-result/refuses-a-48f3a6: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "replace-mesh-result/refuses-a-48f3a6: committed mutation JSON is not canonical");
}
