//! 🧪️ `replace-stream-source` fixture — `📼️attaches-a-607df8`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `replace-stream-source-attaches` is this vector's scenario id in
...
//!
//! 🏞️ moving a source slot from null to a value is the empty-boundary case
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁replace-stream-source/📼️attaches-a-607df8/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁replace-stream-source/📼️attaches-a-607df8/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁replace-stream-source/📼️attaches-a-607df8/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁replace-stream-source/📼️attaches-a-607df8/🎯️outcome/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁replace-stream-source/📼️attaches-a-607df8/🔺️diff/🔣️.json");
...
async fn reaches_the_committed_after_document() {
    ...
    assert_eq!(applied, expected_after(), "replace-stream-source/attaches-a-607df8: applied state differs from committed after-snapshot");
    assert_ne!(applied, before(), "replace-stream-source/attaches-a-607df8: an applied vector must move the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(json_of(outcome.diff()), committed, "replace-stream-source/attaches-a-607df8: produced diff differs from the committed 🔺️diff/🔣️.json");
    let decoded: RemodelingDiff = pack::from_json_str(DIFF).expect("committed diff decodes into the diff type");
    assert_eq!(json_of(&decoded), committed, "replace-stream-source/attaches-a-607df8: committed diff JSON is not canonical");
}
...
async fn committed_diff_carries_before_to_after() {
    ...
    assert_eq!(applied, expected_after(), "replace-stream-source/attaches-a-607df8: committed diff did not carry before to after");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "replace-stream-source/attaches-a-607df8 declares an applied outcome");
    ...
    assert_eq!(emitted, declared_codes, "replace-stream-source/attaches-a-607df8: emitted diagnostics differ from the declared ones");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "replace-stream-source/attaches-a-607df8: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "replace-stream-source/attaches-a-607df8: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "replace-stream-source/attaches-a-607df8: committed mutation JSON is not canonical");
}
