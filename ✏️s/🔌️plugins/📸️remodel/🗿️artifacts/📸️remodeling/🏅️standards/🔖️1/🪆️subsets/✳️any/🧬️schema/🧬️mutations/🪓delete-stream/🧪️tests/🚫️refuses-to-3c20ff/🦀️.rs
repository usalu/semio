//! 🧪️ `delete-stream` fixture — `🚫️refuses-to-3c20ff`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `delete-stream-missing` is this vector's scenario id in
...
//!
//! 🏞️ an unknown id is an Error, never a silent no-op
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪓delete-stream/🚫️refuses-to-3c20ff/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪓delete-stream/🚫️refuses-to-3c20ff/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪓delete-stream/🚫️refuses-to-3c20ff/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪓delete-stream/🚫️refuses-to-3c20ff/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `delete-stream` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "delete-stream/refuses-to-3c20ff: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "delete-stream/refuses-to-3c20ff: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "delete-stream/refuses-to-3c20ff declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "delete-stream/refuses-to-3c20ff: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "delete-stream/refuses-to-3c20ff: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.target-missing", "delete-stream/refuses-to-3c20ff: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Error, "delete-stream/refuses-to-3c20ff: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "delete-stream/refuses-to-3c20ff: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🪓delete-stream/🚫️refuses-to-3c20ff/🔺️diff/🚫️.absent").is_empty(), "delete-stream/refuses-to-3c20ff: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "delete-stream/refuses-to-3c20ff: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "delete-stream/refuses-to-3c20ff: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "delete-stream/refuses-to-3c20ff: committed mutation JSON is not canonical");
}
