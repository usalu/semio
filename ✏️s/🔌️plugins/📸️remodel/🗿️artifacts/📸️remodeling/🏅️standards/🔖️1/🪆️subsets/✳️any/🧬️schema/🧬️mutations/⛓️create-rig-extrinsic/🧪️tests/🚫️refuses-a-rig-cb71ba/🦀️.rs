//! 🧪️ `create-rig-extrinsic` fixture — `🚫️refuses-a-rig-cb71ba`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `create-rig-extrinsic-unknown-camera` is this vector's scenario id in
...
//!
//! 🏞️ a rig entry may only name a calibrated camera
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⛓️create-rig-extrinsic/🚫️refuses-a-rig-cb71ba/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⛓️create-rig-extrinsic/🚫️refuses-a-rig-cb71ba/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⛓️create-rig-extrinsic/🚫️refuses-a-rig-cb71ba/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⛓️create-rig-extrinsic/🚫️refuses-a-rig-cb71ba/🎯️outcome/🔣️.json");
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "create-rig-extrinsic/refuses-a-rig-cb71ba: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "create-rig-extrinsic/refuses-a-rig-cb71ba: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "create-rig-extrinsic/refuses-a-rig-cb71ba declares a rejected outcome");
    ...
    assert_eq!(declared_path, messages[0].target, "create-rig-extrinsic/refuses-a-rig-cb71ba: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/⛓️create-rig-extrinsic/🚫️refuses-a-rig-cb71ba/🔺️diff/🚫️.absent").is_empty(), "create-rig-extrinsic/refuses-a-rig-cb71ba: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "create-rig-extrinsic/refuses-a-rig-cb71ba: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "create-rig-extrinsic/refuses-a-rig-cb71ba: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "create-rig-extrinsic/refuses-a-rig-cb71ba: committed mutation JSON is not canonical");
}
