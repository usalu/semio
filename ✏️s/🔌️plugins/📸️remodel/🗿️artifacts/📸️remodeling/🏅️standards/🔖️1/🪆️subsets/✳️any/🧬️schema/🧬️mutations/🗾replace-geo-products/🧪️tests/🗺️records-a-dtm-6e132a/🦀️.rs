//! 🧪️ `replace-geo-products` fixture — `🗺️records-a-dtm-6e132a`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `replace-geo-products-realworld` is this vector's scenario id in
...
//!
//! 🏞️ a georeferencing pass fills the two slots the survey had left null
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗾replace-geo-products/🗺️records-a-dtm-6e132a/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗾replace-geo-products/🗺️records-a-dtm-6e132a/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗾replace-geo-products/🗺️records-a-dtm-6e132a/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗾replace-geo-products/🗺️records-a-dtm-6e132a/🎯️outcome/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗾replace-geo-products/🗺️records-a-dtm-6e132a/🔺️diff/🔣️.json");
...
async fn reaches_the_committed_after_document() {
    ...
    assert_eq!(applied, expected_after(), "replace-geo-products/records-a-dtm-6e132a: applied state differs from committed after-snapshot");
    assert_ne!(applied, before(), "replace-geo-products/records-a-dtm-6e132a: an applied vector must move the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(json_of(outcome.diff()), committed, "replace-geo-products/records-a-dtm-6e132a: produced diff differs from the committed 🔺️diff/🔣️.json");
    let decoded: RemodelingDiff = pack::from_json_str(DIFF).expect("committed diff decodes into the diff type");
    assert_eq!(json_of(&decoded), committed, "replace-geo-products/records-a-dtm-6e132a: committed diff JSON is not canonical");
}
...
async fn committed_diff_carries_before_to_after() {
    ...
    assert_eq!(applied, expected_after(), "replace-geo-products/records-a-dtm-6e132a: committed diff did not carry before to after");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "replace-geo-products/records-a-dtm-6e132a declares an applied outcome");
    ...
    assert_eq!(emitted, declared_codes, "replace-geo-products/records-a-dtm-6e132a: emitted diagnostics differ from the declared ones");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "replace-geo-products/records-a-dtm-6e132a: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "replace-geo-products/records-a-dtm-6e132a: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "replace-geo-products/records-a-dtm-6e132a: committed mutation JSON is not canonical");
}
