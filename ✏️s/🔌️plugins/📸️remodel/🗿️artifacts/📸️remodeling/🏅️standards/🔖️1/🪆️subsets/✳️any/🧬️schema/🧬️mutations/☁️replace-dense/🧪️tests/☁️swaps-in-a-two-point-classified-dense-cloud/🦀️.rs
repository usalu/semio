//! 🧪️ `replace-dense` fixture — `☁️swaps-in-a-two-point-classified-dense-cloud`.
//!
...
async fn inverse_restores_the_single_point_base_cloud() {
    ...
    assert!(
        matches!(inverse.as_slice(), [RemodelingMutation::ReplaceDense(payload)] if payload.dense.as_ref().is_some_and(|dense| dense.positions.to_f32_vec().len() == 3)),
...
async fn produces_committed_diff() {
    ...
    assert_eq!(encoded, committed, "replace-dense/swaps-in-a-two-point-classified-dense-cloud: produced diff differs from the committed 🔺️diff/🔣️.json");
