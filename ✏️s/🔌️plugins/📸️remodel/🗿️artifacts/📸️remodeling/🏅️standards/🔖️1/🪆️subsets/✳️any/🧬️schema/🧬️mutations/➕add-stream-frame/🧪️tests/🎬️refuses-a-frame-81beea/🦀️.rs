//! 🧪️ `add-stream-frame` fixture — `🎬️refuses-a-frame-81beea`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `add-stream-frame-kind` is this vector's scenario id in
...
//!
//! 🏞️ `kind` asserts the owner stream's provenance instead of rewriting it, which is what gives this verb an inverse
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕add-stream-frame/🎬️refuses-a-frame-81beea/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕add-stream-frame/🎬️refuses-a-frame-81beea/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕add-stream-frame/🎬️refuses-a-frame-81beea/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕add-stream-frame/🎬️refuses-a-frame-81beea/🎯️outcome/🔣️.json");
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "add-stream-frame/refuses-a-frame-81beea: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "add-stream-frame/refuses-a-frame-81beea: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "add-stream-frame/refuses-a-frame-81beea declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "add-stream-frame/refuses-a-frame-81beea: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "add-stream-frame/refuses-a-frame-81beea: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.invariant", "add-stream-frame/refuses-a-frame-81beea: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Fatal, "add-stream-frame/refuses-a-frame-81beea: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "add-stream-frame/refuses-a-frame-81beea: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/➕add-stream-frame/🎬️refuses-a-frame-81beea/🔺️diff/🚫️.absent").is_empty(), "add-stream-frame/refuses-a-frame-81beea: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "add-stream-frame/refuses-a-frame-81beea: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "add-stream-frame/refuses-a-frame-81beea: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "add-stream-frame/refuses-a-frame-81beea: committed mutation JSON is not canonical");
}
