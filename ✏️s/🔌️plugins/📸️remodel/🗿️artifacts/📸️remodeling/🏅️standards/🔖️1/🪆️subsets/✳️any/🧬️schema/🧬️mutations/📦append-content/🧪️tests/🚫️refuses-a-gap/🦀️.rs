//! 🧪️ `append-content` fixture — `🚫️refuses-a-gap`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `append-content-gap` is this vector's scenario id in
...
//!
//! 🏞️ a leaf index past the stored leaf count would leave a gap and is refused
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📦append-content/🚫️refuses-a-gap/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📦append-content/🚫️refuses-a-gap/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📦append-content/🚫️refuses-a-gap/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📦append-content/🚫️refuses-a-gap/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `commit-reconstruction` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "append-content/refuses-a-gap: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "append-content/refuses-a-gap: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "append-content/refuses-a-gap declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "append-content/refuses-a-gap: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "append-content/refuses-a-gap: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.content-gap", "append-content/refuses-a-gap: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Error, "append-content/refuses-a-gap: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "append-content/refuses-a-gap: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/📦append-content/🚫️refuses-a-gap/🔺️diff/🚫️.absent").is_empty(), "append-content/refuses-a-gap: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "append-content/refuses-a-gap: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "append-content/refuses-a-gap: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "append-content/refuses-a-gap: committed mutation JSON is not canonical");
}
