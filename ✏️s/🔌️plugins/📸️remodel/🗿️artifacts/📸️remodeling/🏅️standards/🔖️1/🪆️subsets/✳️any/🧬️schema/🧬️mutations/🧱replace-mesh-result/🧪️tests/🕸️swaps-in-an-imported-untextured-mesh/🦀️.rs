//! 🧪️ `replace-mesh-result` fixture — `🕸️swaps-in-an-imported-untextured-mesh`.
//!
...
use crate::artifacts::remodeling::MeshSource;
...
async fn stores_the_payload_child_handle_and_clears_the_texture_reference() {
    ...
    assert_eq!(applied.results.mesh.mesh.child_id, "remodeling-mesh-2f6c81b0d4a37e59", "the payload's composed child handle is stored verbatim, not re-minted");
...
async fn inverse_restores_the_reconstructed_textured_mesh() {
    ...
    assert!(
        matches!(inverse.as_slice(), [RemodelingMutation::ReplaceMeshResult(payload)] if payload.mesh.source == MeshSource::Reconstructed && payload.mesh.texture_asset_id.as_deref() == Some("asset-a")),
...
async fn produces_committed_diff() {
    ...
    assert_eq!(encoded, committed, "replace-mesh-result/swaps-in-an-imported-untextured-mesh: produced diff differs from the committed 🔺️diff/🔣️.json");
