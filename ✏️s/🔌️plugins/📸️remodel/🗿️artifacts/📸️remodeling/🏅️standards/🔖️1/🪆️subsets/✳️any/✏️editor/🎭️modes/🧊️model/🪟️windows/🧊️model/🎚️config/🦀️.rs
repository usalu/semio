...
impl protocol::Mutation<RemodelingModelWindowConfig> for RemodelingModelWindowConfigMutation {
    ...
    fn inverse(&self, base: &RemodelingModelWindowConfig) -> Vec<Self> { vec![Self::Snapshot { config: base.clone() }] }
}

impl store::ArtifactDsl for RemodelingModelWindowConfig {
    const EXTENSION: &'static str = "remodelingmodelwindowcfg";
    fn envelope_id() -> &'static str { "s.remodel.remodeling.modelwindowconfig" }
    ...
    fn print_dsl(&self) -> String {
        ...
        let body = serde_json::to_string_pretty(&value).expect("Remodeling Model window config JSON");
...
impl store::ArtifactPack for RemodelingModelWindowConfig {
    ...
    fn decode_pack_with(bytes: &[u8], _options: &store::PackDecodeOptions) -> Result<Self, store::PackError> {
        ...
        if !envelope.matches_identity(<Self as store::ArtifactDsl>::envelope_id(), store::semio_format::Component::Pack, 1) { return Err(store::PackError::Schema("Remodeling Model window pack envelope mismatch".into())); }
