//! 🧪️ `create-asset` fixture — `🖼️stores-an-9f39e1`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `create-asset-realworld` is this vector's scenario id in
...
//!
//! 🏞️ the accepted branch writes BOTH the assets map and the durable-artifact store, which is the only verb that writes two lanes
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧷create-asset/🖼️stores-an-9f39e1/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧷create-asset/🖼️stores-an-9f39e1/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧷create-asset/🖼️stores-an-9f39e1/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧷create-asset/🖼️stores-an-9f39e1/🎯️outcome/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧷create-asset/🖼️stores-an-9f39e1/🔺️diff/🔣️.json");
...
async fn reaches_the_committed_after_document() {
    ...
    assert_eq!(applied, expected_after(), "create-asset/stores-an-9f39e1: applied state differs from committed after-snapshot");
    assert_ne!(applied, before(), "create-asset/stores-an-9f39e1: an applied vector must move the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(json_of(outcome.diff()), committed, "create-asset/stores-an-9f39e1: produced diff differs from the committed 🔺️diff/🔣️.json");
    let decoded: RemodelingDiff = pack::from_json_str(DIFF).expect("committed diff decodes into the diff type");
    assert_eq!(json_of(&decoded), committed, "create-asset/stores-an-9f39e1: committed diff JSON is not canonical");
}
...
async fn committed_diff_carries_before_to_after() {
    ...
    assert_eq!(applied, expected_after(), "create-asset/stores-an-9f39e1: committed diff did not carry before to after");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "create-asset/stores-an-9f39e1 declares an applied outcome");
    ...
    assert_eq!(emitted, declared_codes, "create-asset/stores-an-9f39e1: emitted diagnostics differ from the declared ones");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "create-asset/stores-an-9f39e1: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "create-asset/stores-an-9f39e1: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "create-asset/stores-an-9f39e1: committed mutation JSON is not canonical");
}
