//! 🧪️ `commit-reconstruction` fixture — `⭐️rejects-an-2e5568`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `commit-reconstruction-sparse` is this vector's scenario id in
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏁commit-reconstruction/⭐️rejects-an-2e5568/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏁commit-reconstruction/⭐️rejects-an-2e5568/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏁commit-reconstruction/⭐️rejects-an-2e5568/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏁commit-reconstruction/⭐️rejects-an-2e5568/🎯️outcome/🔣️.json");
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "commit-reconstruction/rejects-an-2e5568: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "commit-reconstruction/rejects-an-2e5568: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "commit-reconstruction/rejects-an-2e5568 declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "commit-reconstruction/rejects-an-2e5568: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "commit-reconstruction/rejects-an-2e5568: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.invalid-reconstruction-sparse", "commit-reconstruction/rejects-an-2e5568: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Error, "commit-reconstruction/rejects-an-2e5568: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "commit-reconstruction/rejects-an-2e5568: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🏁commit-reconstruction/⭐️rejects-an-2e5568/🔺️diff/🚫️.absent").is_empty(), "commit-reconstruction/rejects-an-2e5568: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "commit-reconstruction/rejects-an-2e5568: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "commit-reconstruction/rejects-an-2e5568: committed {label} JSON is not canonical");
    }
