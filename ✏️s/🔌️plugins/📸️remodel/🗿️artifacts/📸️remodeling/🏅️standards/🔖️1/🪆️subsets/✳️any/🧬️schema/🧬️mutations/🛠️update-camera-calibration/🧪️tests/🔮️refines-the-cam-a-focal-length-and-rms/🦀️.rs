//! 🧪️ `update-camera-calibration` fixture — `🔮️refines-the-cam-a-focal-length-and-rms`.
//!
...
async fn inverse_is_the_same_verb_carrying_the_base_camera_record() {
    ...
    assert!(matches!(inverse.as_slice(), [RemodelingMutation::UpdateCameraCalibration(payload)] if payload.camera.id == "cam-a" && payload.camera.fx == 1000.0), "update-camera-calibration inverts to itself with the base fx of 1000, got {inverse:?}");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(encoded, committed, "update-camera-calibration/refines-the-cam-a-focal-length-and-rms: produced diff differs from the committed 🔺️diff/🔣️.json");
