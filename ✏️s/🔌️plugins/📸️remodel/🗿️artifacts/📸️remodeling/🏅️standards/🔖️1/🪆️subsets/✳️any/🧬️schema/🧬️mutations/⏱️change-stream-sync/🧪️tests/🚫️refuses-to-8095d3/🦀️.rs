//! 🧪️ `change-stream-sync` fixture — `🚫️refuses-to-8095d3`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `change-stream-sync-missing` is this vector's scenario id in
...
//!
//! 🏞️ the missing-target guard runs before the identical-offset guard
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⏱️change-stream-sync/🚫️refuses-to-8095d3/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⏱️change-stream-sync/🚫️refuses-to-8095d3/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⏱️change-stream-sync/🚫️refuses-to-8095d3/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⏱️change-stream-sync/🚫️refuses-to-8095d3/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `change-stream-sync` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "change-stream-sync/refuses-to-8095d3: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "change-stream-sync/refuses-to-8095d3: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "change-stream-sync/refuses-to-8095d3 declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "change-stream-sync/refuses-to-8095d3: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "change-stream-sync/refuses-to-8095d3: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.target-missing", "change-stream-sync/refuses-to-8095d3: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Error, "change-stream-sync/refuses-to-8095d3: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "change-stream-sync/refuses-to-8095d3: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/⏱️change-stream-sync/🚫️refuses-to-8095d3/🔺️diff/🚫️.absent").is_empty(), "change-stream-sync/refuses-to-8095d3: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "change-stream-sync/refuses-to-8095d3: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "change-stream-sync/refuses-to-8095d3: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "change-stream-sync/refuses-to-8095d3: committed mutation JSON is not canonical");
}
