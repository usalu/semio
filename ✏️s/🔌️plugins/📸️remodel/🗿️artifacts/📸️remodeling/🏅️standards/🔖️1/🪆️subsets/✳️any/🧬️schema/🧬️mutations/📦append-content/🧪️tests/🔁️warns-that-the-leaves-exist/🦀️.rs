//! 🧪️ `append-content` fixture — `🔁️warns-that-the-leaves-exist`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `append-content-noop` is this vector's scenario id in
...
//!
//! 🏞️ leaves already stored byte for byte re-land as a Warning, not a rewrite
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📦append-content/🔁️warns-that-the-leaves-exist/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📦append-content/🔁️warns-that-the-leaves-exist/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📦append-content/🔁️warns-that-the-leaves-exist/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📦append-content/🔁️warns-that-the-leaves-exist/🎯️outcome/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📦append-content/🔁️warns-that-the-leaves-exist/🔺️diff/🔣️.json");
...
async fn no_op_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "append-content/warns-that-the-leaves-exist: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "append-content/warns-that-the-leaves-exist: a no-op must not move the document");
}
...
async fn declared_warning_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "append-content/warns-that-the-leaves-exist declares an applied outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "append-content/warns-that-the-leaves-exist: a no-op leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "append-content/warns-that-the-leaves-exist: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.no-op", "append-content/warns-that-the-leaves-exist: an identical resubmission is a no-op");
    assert_eq!(messages[0].level, protocol::Severity::Warning, "append-content/warns-that-the-leaves-exist: a no-op is a Warning, never an Error");
    let committed = pack::parse_json(DIFF).expect("committed diff decodes");
    assert_eq!(json_of(produced.diff()), committed, "append-content/warns-that-the-leaves-exist: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "append-content/warns-that-the-leaves-exist: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "append-content/warns-that-the-leaves-exist: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "append-content/warns-that-the-leaves-exist: committed mutation JSON is not canonical");
}
