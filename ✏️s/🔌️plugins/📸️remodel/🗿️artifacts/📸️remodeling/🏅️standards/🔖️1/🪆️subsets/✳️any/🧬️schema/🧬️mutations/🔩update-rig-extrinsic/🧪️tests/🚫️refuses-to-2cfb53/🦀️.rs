//! 🧪️ `update-rig-extrinsic` fixture — `🚫️refuses-to-2cfb53`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `update-rig-extrinsic-missing` is this vector's scenario id in
...
//!
//! 🏞️ missing-target precedes the finiteness invariant and the no-op Warning alike
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔩update-rig-extrinsic/🚫️refuses-to-2cfb53/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔩update-rig-extrinsic/🚫️refuses-to-2cfb53/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔩update-rig-extrinsic/🚫️refuses-to-2cfb53/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔩update-rig-extrinsic/🚫️refuses-to-2cfb53/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `update-rig-extrinsic` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "update-rig-extrinsic/refuses-to-2cfb53: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "update-rig-extrinsic/refuses-to-2cfb53: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "update-rig-extrinsic/refuses-to-2cfb53 declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "update-rig-extrinsic/refuses-to-2cfb53: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "update-rig-extrinsic/refuses-to-2cfb53: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.target-missing", "update-rig-extrinsic/refuses-to-2cfb53: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Error, "update-rig-extrinsic/refuses-to-2cfb53: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "update-rig-extrinsic/refuses-to-2cfb53: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🔩update-rig-extrinsic/🚫️refuses-to-2cfb53/🔺️diff/🚫️.absent").is_empty(), "update-rig-extrinsic/refuses-to-2cfb53: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "update-rig-extrinsic/refuses-to-2cfb53: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "update-rig-extrinsic/refuses-to-2cfb53: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "update-rig-extrinsic/refuses-to-2cfb53: committed mutation JSON is not canonical");
}
