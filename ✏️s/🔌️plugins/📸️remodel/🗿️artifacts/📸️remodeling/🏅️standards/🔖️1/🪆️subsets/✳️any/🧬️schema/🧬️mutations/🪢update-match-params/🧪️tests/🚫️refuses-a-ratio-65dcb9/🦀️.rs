//! 🧪️ `update-match-params` fixture — `🚫️refuses-a-ratio-65dcb9`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `update-match-params-invariant` is this vector's scenario id in
...
//!
//! 🏞️ the block's own Fatal invariant, checked before the identical-resubmission Warning
...
/// 🚫️ A refused `update-match-params` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "update-match-params/refuses-a-ratio-65dcb9: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "update-match-params/refuses-a-ratio-65dcb9: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "update-match-params/refuses-a-ratio-65dcb9 declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "update-match-params/refuses-a-ratio-65dcb9: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "update-match-params/refuses-a-ratio-65dcb9: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.invariant", "update-match-params/refuses-a-ratio-65dcb9: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Fatal, "update-match-params/refuses-a-ratio-65dcb9: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "update-match-params/refuses-a-ratio-65dcb9: the declared path must be the emitted target");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "update-match-params/refuses-a-ratio-65dcb9: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "update-match-params/refuses-a-ratio-65dcb9: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "update-match-params/refuses-a-ratio-65dcb9: committed mutation JSON is not canonical");
}
