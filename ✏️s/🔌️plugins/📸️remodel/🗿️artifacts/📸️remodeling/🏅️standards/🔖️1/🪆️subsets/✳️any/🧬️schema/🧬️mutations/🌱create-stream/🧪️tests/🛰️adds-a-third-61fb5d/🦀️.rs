//! 🧪️ `create-stream` fixture — `🛰️adds-a-third-61fb5d`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `create-stream-realworld` is this vector's scenario id in
...
//!
//! 🏞️ appends a third stream to a survey that already carries two, bound to the rig's spare camera
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-stream/🛰️adds-a-third-61fb5d/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-stream/🛰️adds-a-third-61fb5d/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-stream/🛰️adds-a-third-61fb5d/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-stream/🛰️adds-a-third-61fb5d/🎯️outcome/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-stream/🛰️adds-a-third-61fb5d/🔺️diff/🔣️.json");
...
async fn reaches_the_committed_after_document() {
    ...
    assert_eq!(applied, expected_after(), "create-stream/adds-a-third-61fb5d: applied state differs from committed after-snapshot");
    assert_ne!(applied, before(), "create-stream/adds-a-third-61fb5d: an applied vector must move the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(json_of(outcome.diff()), committed, "create-stream/adds-a-third-61fb5d: produced diff differs from the committed 🔺️diff/🔣️.json");
    let decoded: RemodelingDiff = pack::from_json_str(DIFF).expect("committed diff decodes into the diff type");
    assert_eq!(json_of(&decoded), committed, "create-stream/adds-a-third-61fb5d: committed diff JSON is not canonical");
}
...
async fn committed_diff_carries_before_to_after() {
    ...
    assert_eq!(applied, expected_after(), "create-stream/adds-a-third-61fb5d: committed diff did not carry before to after");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "create-stream/adds-a-third-61fb5d declares an applied outcome");
    ...
    assert_eq!(emitted, declared_codes, "create-stream/adds-a-third-61fb5d: emitted diagnostics differ from the declared ones");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "create-stream/adds-a-third-61fb5d: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "create-stream/adds-a-third-61fb5d: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "create-stream/adds-a-third-61fb5d: committed mutation JSON is not canonical");
}
