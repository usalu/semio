//! 🧪️ `update-camera-calibration` fixture — `🚫️refuses-to-b60a39`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `update-camera-calibration-missing` is this vector's scenario id in
...
//!
//! 🏞️ missing-target precedes both the no-op and the finiteness guard
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🛠️update-camera-calibration/🚫️refuses-to-b60a39/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🛠️update-camera-calibration/🚫️refuses-to-b60a39/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🛠️update-camera-calibration/🚫️refuses-to-b60a39/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🛠️update-camera-calibration/🚫️refuses-to-b60a39/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `update-camera-calibration` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "update-camera-calibration/refuses-to-b60a39: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "update-camera-calibration/refuses-to-b60a39: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "update-camera-calibration/refuses-to-b60a39 declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "update-camera-calibration/refuses-to-b60a39: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "update-camera-calibration/refuses-to-b60a39: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.target-missing", "update-camera-calibration/refuses-to-b60a39: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Error, "update-camera-calibration/refuses-to-b60a39: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "update-camera-calibration/refuses-to-b60a39: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🛠️update-camera-calibration/🚫️refuses-to-b60a39/🔺️diff/🚫️.absent").is_empty(), "update-camera-calibration/refuses-to-b60a39: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "update-camera-calibration/refuses-to-b60a39: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "update-camera-calibration/refuses-to-b60a39: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "update-camera-calibration/refuses-to-b60a39: committed mutation JSON is not canonical");
}
