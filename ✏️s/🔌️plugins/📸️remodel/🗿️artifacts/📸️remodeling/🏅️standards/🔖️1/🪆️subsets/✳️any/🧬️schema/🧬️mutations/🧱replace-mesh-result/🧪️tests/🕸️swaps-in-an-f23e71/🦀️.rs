//! 🧪️ `replace-mesh-result` fixture — `🕸️swaps-in-an-f23e71`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `replace-mesh-result` is this vector's scenario id in
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱replace-mesh-result/🕸️swaps-in-an-f23e71/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱replace-mesh-result/🕸️swaps-in-an-f23e71/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱replace-mesh-result/🕸️swaps-in-an-f23e71/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱replace-mesh-result/🕸️swaps-in-an-f23e71/🎯️outcome/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱replace-mesh-result/🕸️swaps-in-an-f23e71/🔺️diff/🔣️.json");
...
async fn reaches_the_committed_after_document() {
    let applied = apply_remodeling_mutation(&before(), &mutation()).expect("replace-mesh-result applies to its committed before-snapshot");
    assert_eq!(applied, expected_after(), "replace-mesh-result/swaps-in-an-f23e71: applied state differs from committed after-snapshot");
    assert_ne!(applied, before(), "replace-mesh-result/swaps-in-an-f23e71: an applied vector must move the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(json_of(outcome.diff()), committed, "replace-mesh-result/swaps-in-an-f23e71: produced diff differs from the committed 🔺️diff/🔣️.json");
    let decoded: RemodelingDiff = pack::from_json_str(DIFF).expect("committed diff decodes into the diff type");
    assert_eq!(json_of(&decoded), committed, "replace-mesh-result/swaps-in-an-f23e71: committed diff JSON is not canonical");
}
...
async fn committed_diff_carries_before_to_after() {
    ...
    assert_eq!(applied, expected_after(), "replace-mesh-result/swaps-in-an-f23e71: committed diff did not carry before to after");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "replace-mesh-result/swaps-in-an-f23e71 declares an applied outcome");
    ...
    assert_eq!(emitted, declared_codes, "replace-mesh-result/swaps-in-an-f23e71: emitted diagnostics differ from the declared ones");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "replace-mesh-result/swaps-in-an-f23e71: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "replace-mesh-result/swaps-in-an-f23e71: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "replace-mesh-result/swaps-in-an-f23e71: committed mutation JSON is not canonical");
}
