//! 🧪️ `delete-gcp` fixture — `🚫️removes-gcp-corner-and-cascades-its-observation`.
//!
...
async fn inverse_recreates_the_gcp_with_its_observation() {
    ...
    assert!(
        matches!(inverse.as_slice(), [RemodelingMutation::CreateGcp(payload)] if payload.gcp.id == "gcp-corner" && payload.gcp.observations.len() == 1),
...
async fn produces_committed_diff() {
    ...
    assert_eq!(encoded, committed, "delete-gcp/removes-gcp-corner-and-cascades-its-observation: produced diff differs from the committed 🔺️diff/🔣️.json");
