//! 🧪️ `delete-stream` fixture — `⛓️refuses-to-remove-422a37`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `delete-stream-referenced` is this vector's scenario id in
...
//!
//! 🏞️ observations belong to their GCPs, not to the stream, so the delete refuses instead of severing another record's data
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪓delete-stream/⛓️refuses-to-remove-422a37/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪓delete-stream/⛓️refuses-to-remove-422a37/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪓delete-stream/⛓️refuses-to-remove-422a37/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪓delete-stream/⛓️refuses-to-remove-422a37/🎯️outcome/🔣️.json");
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "delete-stream/refuses-to-remove-422a37: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "delete-stream/refuses-to-remove-422a37: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "delete-stream/refuses-to-remove-422a37 declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "delete-stream/refuses-to-remove-422a37: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "delete-stream/refuses-to-remove-422a37: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.referenced", "delete-stream/refuses-to-remove-422a37: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Error, "delete-stream/refuses-to-remove-422a37: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "delete-stream/refuses-to-remove-422a37: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🪓delete-stream/⛓️refuses-to-remove-422a37/🔺️diff/🚫️.absent").is_empty(), "delete-stream/refuses-to-remove-422a37: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "delete-stream/refuses-to-remove-422a37: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "delete-stream/refuses-to-remove-422a37: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "delete-stream/refuses-to-remove-422a37: committed mutation JSON is not canonical");
}
