//! 🧪️ `create-gcp` fixture — `🚫️refuses-a-19c1ab`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `create-gcp-duplicate` is this vector's scenario id in
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧿create-gcp/🚫️refuses-a-19c1ab/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧿create-gcp/🚫️refuses-a-19c1ab/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧿create-gcp/🚫️refuses-a-19c1ab/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧿create-gcp/🚫️refuses-a-19c1ab/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `create-gcp` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "create-gcp/refuses-a-19c1ab: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "create-gcp/refuses-a-19c1ab: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "create-gcp/refuses-a-19c1ab declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "create-gcp/refuses-a-19c1ab: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "create-gcp/refuses-a-19c1ab: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.duplicate-id", "create-gcp/refuses-a-19c1ab: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Fatal, "create-gcp/refuses-a-19c1ab: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "create-gcp/refuses-a-19c1ab: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🧿create-gcp/🚫️refuses-a-19c1ab/🔺️diff/🚫️.absent").is_empty(), "create-gcp/refuses-a-19c1ab: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "create-gcp/refuses-a-19c1ab: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "create-gcp/refuses-a-19c1ab: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "create-gcp/refuses-a-19c1ab: committed mutation JSON is not canonical");
}
