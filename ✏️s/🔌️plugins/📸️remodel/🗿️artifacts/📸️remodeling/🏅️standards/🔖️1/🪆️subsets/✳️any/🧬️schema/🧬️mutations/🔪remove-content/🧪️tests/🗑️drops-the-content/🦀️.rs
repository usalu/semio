//! 🧪️ `remove-content` fixture — `🗑️drops-the-content`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `remove-content` is this vector's scenario id in
...
//!
//! 🏞️ removing from leaf 0 drops the whole content entry the append published
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔪remove-content/🗑️drops-the-content/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔪remove-content/🗑️drops-the-content/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔪remove-content/🗑️drops-the-content/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔪remove-content/🗑️drops-the-content/🎯️outcome/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔪remove-content/🗑️drops-the-content/🔺️diff/🔣️.json");
...
async fn reaches_the_committed_after_document() {
    let applied = apply_remodeling_mutation(&before(), &mutation()).expect("remove-content applies to its committed before-snapshot");
    assert_eq!(applied, expected_after(), "remove-content/drops-the-content: applied state differs from committed after-snapshot");
    assert_ne!(applied, before(), "remove-content/drops-the-content: an applied vector must move the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(json_of(outcome.diff()), committed, "remove-content/drops-the-content: produced diff differs from the committed 🔺️diff/🔣️.json");
    let decoded: RemodelingDiff = pack::from_json_str(DIFF).expect("committed diff decodes into the diff type");
    assert_eq!(json_of(&decoded), committed, "remove-content/drops-the-content: committed diff JSON is not canonical");
}
...
async fn committed_diff_carries_before_to_after() {
    ...
    assert_eq!(applied, expected_after(), "remove-content/drops-the-content: committed diff did not carry before to after");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "remove-content/drops-the-content declares an applied outcome");
    ...
    assert_eq!(emitted, declared_codes, "remove-content/drops-the-content: emitted diagnostics differ from the declared ones");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "remove-content/drops-the-content: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "remove-content/drops-the-content: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "remove-content/drops-the-content: committed mutation JSON is not canonical");
}
