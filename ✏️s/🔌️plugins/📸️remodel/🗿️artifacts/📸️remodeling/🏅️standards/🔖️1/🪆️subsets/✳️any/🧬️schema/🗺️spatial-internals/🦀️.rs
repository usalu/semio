...
//! Moved wholesale from `🧮️math/🗺️spatial` — 📸️remodeling is its sole repo-wide consumer (verified: symbol-level grep of every exported type/fn across the whole tree outside math and remodeling returned zero hits), per `26/08/12/DISSOLVE-KERNELS-AND-MODULES-INTO-EVENT-SOURCED-ARTIFACTS` wave M3d.
