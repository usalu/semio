//! 🧪️ `replace-stream-source` fixture — `🚫️refuses-to-f7f40d`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `replace-stream-source-missing` is this vector's scenario id in
...
//!
//! 🏞️ this verb has no no-op guard at all, so target-missing is its only diagnostic
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁replace-stream-source/🚫️refuses-to-f7f40d/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁replace-stream-source/🚫️refuses-to-f7f40d/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁replace-stream-source/🚫️refuses-to-f7f40d/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁replace-stream-source/🚫️refuses-to-f7f40d/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `replace-stream-source` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "replace-stream-source/refuses-to-f7f40d: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "replace-stream-source/refuses-to-f7f40d: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "replace-stream-source/refuses-to-f7f40d declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "replace-stream-source/refuses-to-f7f40d: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "replace-stream-source/refuses-to-f7f40d: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.target-missing", "replace-stream-source/refuses-to-f7f40d: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Error, "replace-stream-source/refuses-to-f7f40d: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "replace-stream-source/refuses-to-f7f40d: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁replace-stream-source/🚫️refuses-to-f7f40d/🔺️diff/🚫️.absent").is_empty(), "replace-stream-source/refuses-to-f7f40d: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "replace-stream-source/refuses-to-f7f40d: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "replace-stream-source/refuses-to-f7f40d: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "replace-stream-source/refuses-to-f7f40d: committed mutation JSON is not canonical");
}
