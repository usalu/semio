//! 🧪️ `update-ingest-params` fixture — `🚫️refuses-an-59752a`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `update-ingest-params-invariant` is this vector's scenario id in
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🥣update-ingest-params/🚫️refuses-an-59752a/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🥣update-ingest-params/🚫️refuses-an-59752a/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🥣update-ingest-params/🚫️refuses-an-59752a/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🥣update-ingest-params/🚫️refuses-an-59752a/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `update-ingest-params` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "update-ingest-params/refuses-an-59752a: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "update-ingest-params/refuses-an-59752a: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "update-ingest-params/refuses-an-59752a declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "update-ingest-params/refuses-an-59752a: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "update-ingest-params/refuses-an-59752a: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.invariant", "update-ingest-params/refuses-an-59752a: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Fatal, "update-ingest-params/refuses-an-59752a: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "update-ingest-params/refuses-an-59752a: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🥣update-ingest-params/🚫️refuses-an-59752a/🔺️diff/🚫️.absent").is_empty(), "update-ingest-params/refuses-an-59752a: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "update-ingest-params/refuses-an-59752a: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "update-ingest-params/refuses-an-59752a: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "update-ingest-params/refuses-an-59752a: committed mutation JSON is not canonical");
}
