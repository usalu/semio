//! 🧪️ `replace-mesh-result` fixture — `🕸️swaps-the-c43d9c`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `replace-mesh-result-realworld` is this vector's scenario id in
...
//!
//! 🏞️ an imported mesh drops the watertight report the reconstructed one carried
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱replace-mesh-result/🕸️swaps-the-c43d9c/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱replace-mesh-result/🕸️swaps-the-c43d9c/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱replace-mesh-result/🕸️swaps-the-c43d9c/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱replace-mesh-result/🕸️swaps-the-c43d9c/🎯️outcome/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱replace-mesh-result/🕸️swaps-the-c43d9c/🔺️diff/🔣️.json");
...
async fn reaches_the_committed_after_document() {
    ...
    assert_eq!(applied, expected_after(), "replace-mesh-result/swaps-the-c43d9c: applied state differs from committed after-snapshot");
    assert_ne!(applied, before(), "replace-mesh-result/swaps-the-c43d9c: an applied vector must move the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(json_of(outcome.diff()), committed, "replace-mesh-result/swaps-the-c43d9c: produced diff differs from the committed 🔺️diff/🔣️.json");
    let decoded: RemodelingDiff = pack::from_json_str(DIFF).expect("committed diff decodes into the diff type");
    assert_eq!(json_of(&decoded), committed, "replace-mesh-result/swaps-the-c43d9c: committed diff JSON is not canonical");
}
...
async fn committed_diff_carries_before_to_after() {
    ...
    assert_eq!(applied, expected_after(), "replace-mesh-result/swaps-the-c43d9c: committed diff did not carry before to after");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "replace-mesh-result/swaps-the-c43d9c declares an applied outcome");
    ...
    assert_eq!(emitted, declared_codes, "replace-mesh-result/swaps-the-c43d9c: emitted diagnostics differ from the declared ones");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "replace-mesh-result/swaps-the-c43d9c: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "replace-mesh-result/swaps-the-c43d9c: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "replace-mesh-result/swaps-the-c43d9c: committed mutation JSON is not canonical");
}
