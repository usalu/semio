//! 🧪️ `update-match-params` fixture — `🔁️warns-that-the-414aae`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `update-match-params-noop` is this vector's scenario id in
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪢update-match-params/🔁️warns-that-the-414aae/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪢update-match-params/🔁️warns-that-the-414aae/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪢update-match-params/🔁️warns-that-the-414aae/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪢update-match-params/🔁️warns-that-the-414aae/🎯️outcome/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪢update-match-params/🔁️warns-that-the-414aae/🔺️diff/🔣️.json");
...
async fn no_op_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "update-match-params/warns-that-the-414aae: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "update-match-params/warns-that-the-414aae: a no-op must not move the document");
}
...
async fn declared_warning_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "update-match-params/warns-that-the-414aae declares an applied outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "update-match-params/warns-that-the-414aae: a no-op leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "update-match-params/warns-that-the-414aae: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.no-op", "update-match-params/warns-that-the-414aae: an identical resubmission is a no-op");
    assert_eq!(messages[0].level, protocol::Severity::Warning, "update-match-params/warns-that-the-414aae: a no-op is a Warning, never an Error");
    let committed = pack::parse_json(DIFF).expect("committed diff decodes");
    assert_eq!(json_of(produced.diff()), committed, "update-match-params/warns-that-the-414aae: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "update-match-params/warns-that-the-414aae: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "update-match-params/warns-that-the-414aae: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "update-match-params/warns-that-the-414aae: committed mutation JSON is not canonical");
}
