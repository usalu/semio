//! 🧪️ `delete-asset` fixture — `🖼️refuses-to-f9541f`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `delete-asset-referenced-frames` is this vector's scenario id in
...
//!
//! 🏞️ a stream frame is a reference, so the document never ends up naming a leaf that is gone
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗞️delete-asset/🖼️refuses-to-f9541f/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗞️delete-asset/🖼️refuses-to-f9541f/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗞️delete-asset/🖼️refuses-to-f9541f/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗞️delete-asset/🖼️refuses-to-f9541f/🎯️outcome/🔣️.json");
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "delete-asset/refuses-to-f9541f: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "delete-asset/refuses-to-f9541f: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "delete-asset/refuses-to-f9541f declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "delete-asset/refuses-to-f9541f: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "delete-asset/refuses-to-f9541f: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.referenced", "delete-asset/refuses-to-f9541f: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Error, "delete-asset/refuses-to-f9541f: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "delete-asset/refuses-to-f9541f: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🗞️delete-asset/🖼️refuses-to-f9541f/🔺️diff/🚫️.absent").is_empty(), "delete-asset/refuses-to-f9541f: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "delete-asset/refuses-to-f9541f: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "delete-asset/refuses-to-f9541f: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "delete-asset/refuses-to-f9541f: committed mutation JSON is not canonical");
}
