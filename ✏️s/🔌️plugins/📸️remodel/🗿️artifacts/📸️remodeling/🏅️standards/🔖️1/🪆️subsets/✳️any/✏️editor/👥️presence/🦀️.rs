//! 👥️ Remodeling presence — shareable live ephemeral state + mutations.
...
#[dsl(extension = "remodeling.presence")]
#[dsl(layout = "lines")]
...
impl protocol::MutationDiff<RemodelingPresence> for RemodelingPresence {
    async fn apply(&self, _base: &RemodelingPresence) -> protocol::MutationApplyResult<RemodelingPresence> {
        Ok({ self.clone() })
    }
...
impl store::ArtifactDsl for RemodelingPresence {
...
impl ArtifactPack for RemodelingPresence {
...
pub enum RemodelingPresenceMutation {
    ...
    Snapshot {
        #[dsl(block)]
        presence: RemodelingPresence,
    },
}

impl Mutation<RemodelingPresence> for RemodelingPresenceMutation {
    type Diff = RemodelingPresence;

    async fn diff(&self, _base: &RemodelingPresence) -> protocol::MutationOutcome<RemodelingPresence> {
        match self {
            Self::Snapshot { presence } => protocol::MutationOutcome::new(presence.clone()),
        }
    }

    async fn inverse(&self, base: &RemodelingPresence) -> Vec<Self> {
