//! 🧪️ `replace-job` fixture — `🔤️advances-the-job-to-texturing`.
//!
...
use crate::artifacts::remodeling::ReconstructionStage;
...
async fn stores_the_whole_job_record_verbatim() {
    let applied = apply_remodeling_mutation(&before(), &mutation()).expect("replace-job applies to its committed before-snapshot");
...
async fn inverse_restores_the_bundle_adjusting_job() {
    ...
    assert!(
        matches!(inverse.as_slice(), [RemodelingMutation::ReplaceJob(payload)] if payload.job.stage == ReconstructionStage::BundleAdjusting && payload.job.stage_cursor == 3),
...
async fn produces_committed_diff() {
    ...
    assert_eq!(encoded, committed, "replace-job/advances-the-job-to-texturing: produced diff differs from the committed 🔺️diff/🔣️.json");
