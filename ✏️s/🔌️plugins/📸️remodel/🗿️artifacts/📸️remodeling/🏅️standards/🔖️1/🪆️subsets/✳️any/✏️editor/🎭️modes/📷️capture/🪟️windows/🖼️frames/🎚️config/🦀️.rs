...
impl protocol::Mutation<RemodelingFramesWindowConfig> for RemodelingFramesWindowConfigMutation {
    ...
    fn inverse(&self, base: &RemodelingFramesWindowConfig) -> Vec<Self> { vec![Self::Snapshot { config: base.clone() }] }
}
...
impl protocol::OpText for RemodelingFramesWindowConfigMutation { fn print_op(&self) -> String { dsl::json::to_json_string(self) } fn parse_op(line: &str) -> Result<Self, store::TextError> { dsl::json::from_json_str(line).map_err(|error| store::TextError::new(error.to_string(), store::TextSpan::at(1, 1))) } }
