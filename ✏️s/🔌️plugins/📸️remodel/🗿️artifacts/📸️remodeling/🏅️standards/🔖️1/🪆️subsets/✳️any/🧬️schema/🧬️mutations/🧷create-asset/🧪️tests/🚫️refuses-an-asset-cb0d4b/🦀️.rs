...
//!
//! 🏞️ a replayable content handle is accepted only by commit-reconstruction, never by create-asset
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧷create-asset/🚫️refuses-an-asset-cb0d4b/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧷create-asset/🚫️refuses-an-asset-cb0d4b/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧷create-asset/🚫️refuses-an-asset-cb0d4b/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧷create-asset/🚫️refuses-an-asset-cb0d4b/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `create-asset` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "create-asset/refuses-an-asset-cb0d4b: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "create-asset/refuses-an-asset-cb0d4b: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "create-asset/refuses-an-asset-cb0d4b declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "create-asset/refuses-an-asset-cb0d4b: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "create-asset/refuses-an-asset-cb0d4b: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.invalid-asset-payload", "create-asset/refuses-an-asset-cb0d4b: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Error, "create-asset/refuses-an-asset-cb0d4b: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "create-asset/refuses-an-asset-cb0d4b: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🧷create-asset/🚫️refuses-an-asset-cb0d4b/🔺️diff/🚫️.absent").is_empty(), "create-asset/refuses-an-asset-cb0d4b: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "create-asset/refuses-an-asset-cb0d4b: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "create-asset/refuses-an-asset-cb0d4b: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "create-asset/refuses-an-asset-cb0d4b: committed mutation JSON is not canonical");
}
