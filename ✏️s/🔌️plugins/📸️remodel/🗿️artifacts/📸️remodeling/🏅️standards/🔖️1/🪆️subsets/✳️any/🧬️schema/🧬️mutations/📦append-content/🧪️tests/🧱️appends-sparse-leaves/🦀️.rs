//! 🧪️ `append-content` fixture — `🧱️appends-sparse-leaves`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `append-content` is this vector's scenario id in
...
//!
//! 🏞️ the first leaf of new sparse content lands in the durable store under its digest id
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📦append-content/🧱️appends-sparse-leaves/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📦append-content/🧱️appends-sparse-leaves/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📦append-content/🧱️appends-sparse-leaves/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📦append-content/🧱️appends-sparse-leaves/🎯️outcome/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📦append-content/🧱️appends-sparse-leaves/🔺️diff/🔣️.json");
...
async fn reaches_the_committed_after_document() {
    let applied = apply_remodeling_mutation(&before(), &mutation()).expect("append-content applies to its committed before-snapshot");
    assert_eq!(applied, expected_after(), "append-content/appends-sparse-leaves: applied state differs from committed after-snapshot");
    assert_ne!(applied, before(), "append-content/appends-sparse-leaves: an applied vector must move the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(json_of(outcome.diff()), committed, "append-content/appends-sparse-leaves: produced diff differs from the committed 🔺️diff/🔣️.json");
    let decoded: RemodelingDiff = pack::from_json_str(DIFF).expect("committed diff decodes into the diff type");
    assert_eq!(json_of(&decoded), committed, "append-content/appends-sparse-leaves: committed diff JSON is not canonical");
}
...
async fn committed_diff_carries_before_to_after() {
    ...
    assert_eq!(applied, expected_after(), "append-content/appends-sparse-leaves: committed diff did not carry before to after");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "append-content/appends-sparse-leaves declares an applied outcome");
    ...
    assert_eq!(emitted, declared_codes, "append-content/appends-sparse-leaves: emitted diagnostics differ from the declared ones");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "append-content/appends-sparse-leaves: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "append-content/appends-sparse-leaves: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "append-content/appends-sparse-leaves: committed mutation JSON is not canonical");
}
