//! 🧪️ `commit-reconstruction` fixture — `🖼️rejects-an-e9fa51`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `commit-reconstruction-asset` is this vector's scenario id in
...
//!
//! 🏞️ the asset guard runs after the sparse guard, so a null sparse reaches it
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏁commit-reconstruction/🖼️rejects-an-e9fa51/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏁commit-reconstruction/🖼️rejects-an-e9fa51/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏁commit-reconstruction/🖼️rejects-an-e9fa51/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏁commit-reconstruction/🖼️rejects-an-e9fa51/🎯️outcome/🔣️.json");
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "commit-reconstruction/rejects-an-e9fa51: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "commit-reconstruction/rejects-an-e9fa51: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "commit-reconstruction/rejects-an-e9fa51 declares a rejected outcome");
    ...
    assert_eq!(declared_path, messages[0].target, "commit-reconstruction/rejects-an-e9fa51: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🏁commit-reconstruction/🖼️rejects-an-e9fa51/🔺️diff/🚫️.absent").is_empty(), "commit-reconstruction/rejects-an-e9fa51: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "commit-reconstruction/rejects-an-e9fa51: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "commit-reconstruction/rejects-an-e9fa51: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "commit-reconstruction/rejects-an-e9fa51: committed mutation JSON is not canonical");
}
