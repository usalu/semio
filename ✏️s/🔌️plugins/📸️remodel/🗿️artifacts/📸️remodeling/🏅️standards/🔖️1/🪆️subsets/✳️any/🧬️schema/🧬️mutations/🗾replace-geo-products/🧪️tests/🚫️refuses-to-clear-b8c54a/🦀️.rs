//! 🧪️ `replace-geo-products` fixture — `🚫️refuses-to-clear-b8c54a`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `replace-geo-products-absent` is this vector's scenario id in
...
//!
//! 🏞️ clearing an already-absent value is an Error against the DOCUMENT id
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗾replace-geo-products/🚫️refuses-to-clear-b8c54a/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗾replace-geo-products/🚫️refuses-to-clear-b8c54a/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗾replace-geo-products/🚫️refuses-to-clear-b8c54a/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗾replace-geo-products/🚫️refuses-to-clear-b8c54a/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `replace-geo-products` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "replace-geo-products/refuses-to-clear-b8c54a: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "replace-geo-products/refuses-to-clear-b8c54a: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "replace-geo-products/refuses-to-clear-b8c54a declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "replace-geo-products/refuses-to-clear-b8c54a: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "replace-geo-products/refuses-to-clear-b8c54a: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.target-missing", "replace-geo-products/refuses-to-clear-b8c54a: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Error, "replace-geo-products/refuses-to-clear-b8c54a: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "replace-geo-products/refuses-to-clear-b8c54a: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🗾replace-geo-products/🚫️refuses-to-clear-b8c54a/🔺️diff/🚫️.absent").is_empty(), "replace-geo-products/refuses-to-clear-b8c54a: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "replace-geo-products/refuses-to-clear-b8c54a: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "replace-geo-products/refuses-to-clear-b8c54a: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "replace-geo-products/refuses-to-clear-b8c54a: committed mutation JSON is not canonical");
}
