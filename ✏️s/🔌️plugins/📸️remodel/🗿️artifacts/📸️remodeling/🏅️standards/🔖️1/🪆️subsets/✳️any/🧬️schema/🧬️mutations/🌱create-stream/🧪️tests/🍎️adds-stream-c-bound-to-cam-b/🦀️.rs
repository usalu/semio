//! 🧪️ `create-stream` fixture — `🍎️adds-stream-c-bound-to-cam-b`.
//!
...
async fn appends_stream_c_after_the_two_existing_streams() {
    let applied = apply_remodeling_mutation(&before(), &mutation()).expect("create-stream applies to its committed before-snapshot");
...
async fn inverse_is_a_single_delete_of_stream_c() {
    ...
    assert!(matches!(inverse.as_slice(), [RemodelingMutation::DeleteStream(payload)] if payload.id == "stream-c"), "create-stream's inverse for a fresh id is one delete-stream for that id, got {inverse:?}");
