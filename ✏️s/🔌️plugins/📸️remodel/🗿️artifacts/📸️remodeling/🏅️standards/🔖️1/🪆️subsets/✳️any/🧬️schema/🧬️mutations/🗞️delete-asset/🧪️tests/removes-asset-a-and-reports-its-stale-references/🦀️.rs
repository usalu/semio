...
async fn drops_the_handle_and_leaves_every_reference_dangling() {
    let applied = apply_remodeling_mutation(&before(), &mutation()).expect("delete-asset applies to its committed before-snapshot");
...
async fn inverse_is_empty_against_a_cold_working_scene_cache() {
    ...
    let applied = apply_remodeling_mutation(&base, &mutation()).expect("forward applies");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(encoded, committed, "delete-asset/removes-asset-a-and-reports-its-stale-references: produced diff differs from the committed 🔺️diff/🔣️.json");
