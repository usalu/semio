...
async fn inverse_recreates_the_captured_cam_b_record() {
    ...
    assert!(
        matches!(inverse.as_slice(), [RemodelingMutation::CreateCameraCalibration(payload)] if payload.camera.id == "cam-b" && payload.camera.locked),
...
async fn produces_committed_diff() {
    ...
    assert_eq!(encoded, committed, "delete-camera-calibration/removes-the-cam-b-calibration: produced diff differs from the committed 🔺️diff/🔣️.json");
