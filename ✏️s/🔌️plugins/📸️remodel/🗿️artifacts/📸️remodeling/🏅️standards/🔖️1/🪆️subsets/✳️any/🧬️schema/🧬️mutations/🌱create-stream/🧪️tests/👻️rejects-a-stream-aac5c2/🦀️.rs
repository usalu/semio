//! 🧪️ `create-stream` fixture — `👻️rejects-a-stream-aac5c2`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `create-stream-unknown-camera` is this vector's scenario id in
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-stream/👻️rejects-a-stream-aac5c2/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-stream/👻️rejects-a-stream-aac5c2/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-stream/👻️rejects-a-stream-aac5c2/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-stream/👻️rejects-a-stream-aac5c2/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `create-stream` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "create-stream/rejects-a-stream-aac5c2: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "create-stream/rejects-a-stream-aac5c2: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "create-stream/rejects-a-stream-aac5c2 declares a rejected outcome");
    ...
    assert_eq!(declared_path, messages[0].target, "create-stream/rejects-a-stream-aac5c2: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-stream/👻️rejects-a-stream-aac5c2/🔺️diff/🚫️.absent").is_empty(), "create-stream/rejects-a-stream-aac5c2: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "create-stream/rejects-a-stream-aac5c2: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "create-stream/rejects-a-stream-aac5c2: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "create-stream/rejects-a-stream-aac5c2: committed mutation JSON is not canonical");
}
