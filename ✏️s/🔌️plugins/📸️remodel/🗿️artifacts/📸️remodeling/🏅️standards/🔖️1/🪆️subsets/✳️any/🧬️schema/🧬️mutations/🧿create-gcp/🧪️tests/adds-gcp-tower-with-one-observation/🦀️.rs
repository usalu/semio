...
async fn inverse_is_a_single_delete_of_gcp_tower() {
    ...
    assert!(matches!(inverse.as_slice(), [RemodelingMutation::DeleteGcp(payload)] if payload.id == "gcp-tower"), "create-gcp's inverse for a fresh id is one delete-gcp for that id, got {inverse:?}");
