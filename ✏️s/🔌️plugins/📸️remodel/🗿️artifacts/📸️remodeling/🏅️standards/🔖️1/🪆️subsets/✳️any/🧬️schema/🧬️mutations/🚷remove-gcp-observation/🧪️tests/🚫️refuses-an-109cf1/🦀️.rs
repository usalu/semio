//! 🧪️ `remove-gcp-observation` fixture — `🚫️refuses-an-109cf1`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `remove-gcp-observation-out-of-range` is this vector's scenario id in
...
//!
//! 🏞️ an out-of-range index is reported as target-missing against the POINT id
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚷remove-gcp-observation/🚫️refuses-an-109cf1/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚷remove-gcp-observation/🚫️refuses-an-109cf1/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚷remove-gcp-observation/🚫️refuses-an-109cf1/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚷remove-gcp-observation/🚫️refuses-an-109cf1/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `remove-gcp-observation` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "remove-gcp-observation/refuses-an-109cf1: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "remove-gcp-observation/refuses-an-109cf1: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "remove-gcp-observation/refuses-an-109cf1 declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "remove-gcp-observation/refuses-an-109cf1: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "remove-gcp-observation/refuses-an-109cf1: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.target-missing", "remove-gcp-observation/refuses-an-109cf1: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Error, "remove-gcp-observation/refuses-an-109cf1: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "remove-gcp-observation/refuses-an-109cf1: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🚷remove-gcp-observation/🚫️refuses-an-109cf1/🔺️diff/🚫️.absent").is_empty(), "remove-gcp-observation/refuses-an-109cf1: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "remove-gcp-observation/refuses-an-109cf1: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "remove-gcp-observation/refuses-an-109cf1: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "remove-gcp-observation/refuses-an-109cf1: committed mutation JSON is not canonical");
}
