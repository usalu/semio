//! 🧪️ `delete-gcp` fixture — `🚫️refuses-to-12366b`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `delete-gcp-missing` is this vector's scenario id in
...
//!
//! 🏞️ an unknown id is an Error against the id itself
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚮delete-gcp/🚫️refuses-to-12366b/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚮delete-gcp/🚫️refuses-to-12366b/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚮delete-gcp/🚫️refuses-to-12366b/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚮delete-gcp/🚫️refuses-to-12366b/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `delete-gcp` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "delete-gcp/refuses-to-12366b: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "delete-gcp/refuses-to-12366b: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "delete-gcp/refuses-to-12366b declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "delete-gcp/refuses-to-12366b: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "delete-gcp/refuses-to-12366b: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.target-missing", "delete-gcp/refuses-to-12366b: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Error, "delete-gcp/refuses-to-12366b: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "delete-gcp/refuses-to-12366b: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🚮delete-gcp/🚫️refuses-to-12366b/🔺️diff/🚫️.absent").is_empty(), "delete-gcp/refuses-to-12366b: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "delete-gcp/refuses-to-12366b: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "delete-gcp/refuses-to-12366b: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "delete-gcp/refuses-to-12366b: committed mutation JSON is not canonical");
}
