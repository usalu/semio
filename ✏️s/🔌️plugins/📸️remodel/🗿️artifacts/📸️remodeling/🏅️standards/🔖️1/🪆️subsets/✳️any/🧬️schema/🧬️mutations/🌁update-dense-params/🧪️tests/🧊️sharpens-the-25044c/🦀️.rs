//! 🧪️ `update-dense-params` fixture — `🧊️sharpens-the-25044c`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `update-dense-params-realworld` is this vector's scenario id in
...
//!
//! 🏞️ a real retune of one parameter block; every other block is left untouched
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌁update-dense-params/🧊️sharpens-the-25044c/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌁update-dense-params/🧊️sharpens-the-25044c/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌁update-dense-params/🧊️sharpens-the-25044c/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌁update-dense-params/🧊️sharpens-the-25044c/🎯️outcome/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌁update-dense-params/🧊️sharpens-the-25044c/🔺️diff/🔣️.json");
...
async fn reaches_the_committed_after_document() {
    ...
    assert_eq!(applied, expected_after(), "update-dense-params/sharpens-the-25044c: applied state differs from committed after-snapshot");
    assert_ne!(applied, before(), "update-dense-params/sharpens-the-25044c: an applied vector must move the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(json_of(outcome.diff()), committed, "update-dense-params/sharpens-the-25044c: produced diff differs from the committed 🔺️diff/🔣️.json");
    let decoded: RemodelingDiff = pack::from_json_str(DIFF).expect("committed diff decodes into the diff type");
    assert_eq!(json_of(&decoded), committed, "update-dense-params/sharpens-the-25044c: committed diff JSON is not canonical");
}
...
async fn committed_diff_carries_before_to_after() {
    ...
    assert_eq!(applied, expected_after(), "update-dense-params/sharpens-the-25044c: committed diff did not carry before to after");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "update-dense-params/sharpens-the-25044c declares an applied outcome");
    ...
    assert_eq!(emitted, declared_codes, "update-dense-params/sharpens-the-25044c: emitted diagnostics differ from the declared ones");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "update-dense-params/sharpens-the-25044c: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "update-dense-params/sharpens-the-25044c: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "update-dense-params/sharpens-the-25044c: committed mutation JSON is not canonical");
}
