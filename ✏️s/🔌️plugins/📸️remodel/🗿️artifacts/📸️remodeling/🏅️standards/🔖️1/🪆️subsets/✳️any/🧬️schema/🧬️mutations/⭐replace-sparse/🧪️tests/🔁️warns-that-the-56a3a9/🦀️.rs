//! 🧪️ `replace-sparse` fixture — `🔁️warns-that-the-56a3a9`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `replace-sparse-noop` is this vector's scenario id in
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⭐replace-sparse/🔁️warns-that-the-56a3a9/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⭐replace-sparse/🔁️warns-that-the-56a3a9/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⭐replace-sparse/🔁️warns-that-the-56a3a9/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⭐replace-sparse/🔁️warns-that-the-56a3a9/🎯️outcome/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⭐replace-sparse/🔁️warns-that-the-56a3a9/🔺️diff/🔣️.json");
...
async fn no_op_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "replace-sparse/warns-that-the-56a3a9: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "replace-sparse/warns-that-the-56a3a9: a no-op must not move the document");
}
...
async fn declared_warning_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "replace-sparse/warns-that-the-56a3a9 declares an applied outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "replace-sparse/warns-that-the-56a3a9: a no-op leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "replace-sparse/warns-that-the-56a3a9: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.no-op", "replace-sparse/warns-that-the-56a3a9: an identical resubmission is a no-op");
    assert_eq!(messages[0].level, protocol::Severity::Warning, "replace-sparse/warns-that-the-56a3a9: a no-op is a Warning, never an Error");
    let committed = pack::parse_json(DIFF).expect("committed diff decodes");
    assert_eq!(json_of(produced.diff()), committed, "replace-sparse/warns-that-the-56a3a9: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "replace-sparse/warns-that-the-56a3a9: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "replace-sparse/warns-that-the-56a3a9: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "replace-sparse/warns-that-the-56a3a9: committed mutation JSON is not canonical");
}
