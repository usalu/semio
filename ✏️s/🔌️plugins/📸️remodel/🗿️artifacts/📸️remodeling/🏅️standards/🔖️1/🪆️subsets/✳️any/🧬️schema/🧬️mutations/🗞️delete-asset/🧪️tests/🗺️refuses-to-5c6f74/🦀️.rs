//! 🧪️ `delete-asset` fixture — `🗺️refuses-to-5c6f74`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `delete-asset-geo-product` is this vector's scenario id in
...
//!
//! 🏞️ the guard reaches results.geo's asset slots and the mesh texture, not only stream frames
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗞️delete-asset/🗺️refuses-to-5c6f74/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗞️delete-asset/🗺️refuses-to-5c6f74/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗞️delete-asset/🗺️refuses-to-5c6f74/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗞️delete-asset/🗺️refuses-to-5c6f74/🎯️outcome/🔣️.json");
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "delete-asset/refuses-to-5c6f74: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "delete-asset/refuses-to-5c6f74: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "delete-asset/refuses-to-5c6f74 declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "delete-asset/refuses-to-5c6f74: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "delete-asset/refuses-to-5c6f74: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.referenced", "delete-asset/refuses-to-5c6f74: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Error, "delete-asset/refuses-to-5c6f74: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "delete-asset/refuses-to-5c6f74: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🗞️delete-asset/🗺️refuses-to-5c6f74/🔺️diff/🚫️.absent").is_empty(), "delete-asset/refuses-to-5c6f74: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "delete-asset/refuses-to-5c6f74: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "delete-asset/refuses-to-5c6f74: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "delete-asset/refuses-to-5c6f74: committed mutation JSON is not canonical");
}
