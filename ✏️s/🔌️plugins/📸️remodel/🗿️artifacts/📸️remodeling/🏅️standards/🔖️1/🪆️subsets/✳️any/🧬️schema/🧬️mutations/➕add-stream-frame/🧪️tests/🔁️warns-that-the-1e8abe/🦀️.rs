//! 🧪️ `add-stream-frame` fixture — `🔁️warns-that-the-1e8abe`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `add-stream-frame-noop` is this vector's scenario id in
...
//!
//! 🏞️ equality is on the WHOLE frame record, not on its index alone
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕add-stream-frame/🔁️warns-that-the-1e8abe/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕add-stream-frame/🔁️warns-that-the-1e8abe/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕add-stream-frame/🔁️warns-that-the-1e8abe/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕add-stream-frame/🔁️warns-that-the-1e8abe/🎯️outcome/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕add-stream-frame/🔁️warns-that-the-1e8abe/🔺️diff/🔣️.json");
...
async fn no_op_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "add-stream-frame/warns-that-the-1e8abe: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "add-stream-frame/warns-that-the-1e8abe: a no-op must not move the document");
}
...
async fn declared_warning_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "add-stream-frame/warns-that-the-1e8abe declares an applied outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "add-stream-frame/warns-that-the-1e8abe: a no-op leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "add-stream-frame/warns-that-the-1e8abe: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.no-op", "add-stream-frame/warns-that-the-1e8abe: an identical resubmission is a no-op");
    assert_eq!(messages[0].level, protocol::Severity::Warning, "add-stream-frame/warns-that-the-1e8abe: a no-op is a Warning, never an Error");
    let committed = pack::parse_json(DIFF).expect("committed diff decodes");
    assert_eq!(json_of(produced.diff()), committed, "add-stream-frame/warns-that-the-1e8abe: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "add-stream-frame/warns-that-the-1e8abe: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "add-stream-frame/warns-that-the-1e8abe: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "add-stream-frame/warns-that-the-1e8abe: committed mutation JSON is not canonical");
}
