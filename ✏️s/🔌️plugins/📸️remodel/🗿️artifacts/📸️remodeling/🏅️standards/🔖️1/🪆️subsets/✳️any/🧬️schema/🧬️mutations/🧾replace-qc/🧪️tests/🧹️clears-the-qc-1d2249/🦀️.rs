//! 🧪️ `replace-qc` fixture — `🧹️clears-the-qc-1d2249`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `replace-qc-clears` is this vector's scenario id in
...
//!
//! 🏞️ moving a present value to null is admitted
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧾replace-qc/🧹️clears-the-qc-1d2249/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧾replace-qc/🧹️clears-the-qc-1d2249/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧾replace-qc/🧹️clears-the-qc-1d2249/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧾replace-qc/🧹️clears-the-qc-1d2249/🎯️outcome/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧾replace-qc/🧹️clears-the-qc-1d2249/🔺️diff/🔣️.json");
...
async fn reaches_the_committed_after_document() {
    ...
    assert_eq!(applied, expected_after(), "replace-qc/clears-the-qc-1d2249: applied state differs from committed after-snapshot");
    assert_ne!(applied, before(), "replace-qc/clears-the-qc-1d2249: an applied vector must move the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(json_of(outcome.diff()), committed, "replace-qc/clears-the-qc-1d2249: produced diff differs from the committed 🔺️diff/🔣️.json");
    let decoded: RemodelingDiff = pack::from_json_str(DIFF).expect("committed diff decodes into the diff type");
    assert_eq!(json_of(&decoded), committed, "replace-qc/clears-the-qc-1d2249: committed diff JSON is not canonical");
}
...
async fn committed_diff_carries_before_to_after() {
    ...
    assert_eq!(applied, expected_after(), "replace-qc/clears-the-qc-1d2249: committed diff did not carry before to after");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "replace-qc/clears-the-qc-1d2249 declares an applied outcome");
    ...
    assert_eq!(emitted, declared_codes, "replace-qc/clears-the-qc-1d2249: emitted diagnostics differ from the declared ones");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "replace-qc/clears-the-qc-1d2249: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "replace-qc/clears-the-qc-1d2249: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "replace-qc/clears-the-qc-1d2249: committed mutation JSON is not canonical");
}
