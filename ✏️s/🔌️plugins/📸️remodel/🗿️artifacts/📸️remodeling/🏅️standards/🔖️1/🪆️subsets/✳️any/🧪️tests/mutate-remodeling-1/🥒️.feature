...
Feature: Apply every typed remodeling-scene mutation to its committed specification vector and against an independent Python implementation
  ...
  Scenario Outline: Applying <id> reaches its committed after-document
    ...
    Examples:
      | id                        | dir                         | fixture                                                     |
      | create-stream             | 🌱create-stream              | adds-stream-c-bound-to-cam-b                                |
      | delete-stream             | 🪓delete-stream              | removes-stream-b-and-cascades-its-gcp-observation           |
      | change-stream-sync        | ⏱️change-stream-sync        | shifts-stream-a-sync-offset-to-minus-seven-and-a-half       |
      | add-stream-frame          | ➕add-stream-frame           | appends-a-third-frame-to-stream-a                           |
      | remove-stream-frame       | ➖remove-stream-frame        | removes-the-last-frame-of-stream-a                          |
      | replace-stream-source     | 🔁replace-stream-source      | clears-the-video-source-of-stream-a                         |
      | create-asset              | 🧷create-asset               | stores-a-new-jpeg-frame-asset                               |
      | delete-asset              | 🗞️delete-asset              | removes-asset-a-and-reports-its-stale-references            |
      | create-camera-calibration | 🔭create-camera-calibration  | adds-the-cam-c-fisheye-calibration                          |
      | update-camera-calibration | 🛠️update-camera-calibration | refines-the-cam-a-focal-length-and-rms                      |
      | delete-camera-calibration | 🚫delete-camera-calibration  | removes-the-cam-b-calibration                               |
      | create-rig-extrinsic      | ⛓️create-rig-extrinsic      | adds-a-rig-extrinsic-for-cam-b                              |
      | delete-rig-extrinsic      | ✂️delete-rig-extrinsic      | drops-the-cam-a-rig-extrinsic                               |
      | update-rig-extrinsic      | 🔩update-rig-extrinsic       | retunes-the-cam-a-rig-translation                           |
      | create-gcp                | 🧿create-gcp                 | adds-gcp-tower-with-one-observation                         |
      | delete-gcp                | 🚮delete-gcp                 | removes-gcp-corner-and-cascades-its-observation             |
      | add-gcp-observation       | 🔎add-gcp-observation        | adds-the-first-observation-to-gcp-ridge                     |
      | remove-gcp-observation    | 🚷remove-gcp-observation     | removes-the-only-observation-of-gcp-corner                  |
      | update-ingest-params      | 🥣update-ingest-params       | tightens-the-ingest-sharpness-gate                          |
      | update-feature-params     | 🌠update-feature-params      | switches-the-detector-to-akaze                              |
      | update-match-params       | 🪢update-match-params        | switches-the-matcher-to-a-kd-tree                           |
      | update-sfm-params         | 🧮update-sfm-params          | switches-the-robust-loss-to-cauchy                          |
      | update-dense-params       | 🌁update-dense-params        | raises-the-dense-resolution-and-confidence-gate             |
      | update-mesh-params        | 🕸️update-mesh-params        | doubles-the-texture-size-and-drops-the-watertight-guarantee |
      | update-motion-params      | 🏎️update-motion-params      | enables-motion-tracking                                     |
      | update-geo-params         | 🌐update-geo-params          | enables-georeferencing-with-an-origin                       |
      | replace-job               | 🏗️replace-job               | advances-the-job-to-texturing                               |
      | replace-sparse            | ⭐replace-sparse             | swaps-in-an-uncolored-four-point-sparse-cloud               |
      | replace-dense             | ☁️replace-dense             | swaps-in-a-two-point-classified-dense-cloud                 |
      | replace-mesh-result       | 🧱replace-mesh-result        | swaps-in-an-imported-untextured-mesh                        |
      | replace-trajectory        | 🛣️replace-trajectory        | clears-the-camera-trajectory                                |
      | replace-tracks            | 🚂replace-tracks             | replaces-the-moving-track-with-two-static-tracks            |
      | replace-geo-products      | 🗾replace-geo-products       | adds-the-dtm-and-ortho-rasters                              |
      | replace-qc                | 🧾replace-qc                 | records-a-qc-report-carrying-a-watertight-summary           |
