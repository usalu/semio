//! ⚙️ Remodeling app engine — the `RemodelingPlayApp`'s own compute: bridges between the document's
//! `RemodelingSnapshot` param/result types and the ten sibling photogrammetry topic modules
//! (`images`/`video`/`camera`/`feature`/`sfm`/`dense`/`mesh`/`motion`/`geo`/`reconstruction`), none of
//! which reference `RemodelingSnapshot` themselves (pure numeric/geometry algorithms).
//!
...
pub fn build_engine_params(params: &ReconstructionParams) -> remodeling_engine::EngineParams {
    ...
    engine_params.sfm.robust_loss = match params.sfm.robust_loss {
        RobustLossKind::L2 => remodeling_sfm::RobustLoss::Trivial,
        RobustLossKind::Huber => remodeling_sfm::RobustLoss::Huber(f64::from(params.sfm.huber_delta_px)),
        RobustLossKind::Cauchy => remodeling_sfm::RobustLoss::Cauchy(f64::from(params.sfm.huber_delta_px)),
    };
...
fn camera_world_position(pose: &remodeling_camera::CameraPose) -> [f32; 3] {
...
pub fn camera_pose_preview(index: u32, pose: &remodeling_camera::CameraPose) -> CameraPosePreview {
...
pub fn watertight_snapshot(report: &remodeling_mesh::WatertightReport) -> WatertightReportSnapshot {
...
/// ✅️ Maps `remodeling_geo::QualityReport` (plus counts only the plugin can compute — accepted vs.
...
/// a documented gap.
pub fn build_qc_snapshot(quality: &remodeling_geo::QualityReport, registered_count: usize, accepted_count: usize, gcp_count: usize) -> QcReportSnapshot {
...
/// 🗺️ Normalizes a `remodeling_geo::Raster`'s valid cells to a 16-bit grayscale PNG `ImageAsset` (invalid
/// cells encode as `0`) — `remodeling_image::encode_png_gray16` is the shared 16-bit writer every other
/// raster-shaped asset in this codebase uses.
#[cfg(test)]
pub fn raster_to_png_asset(raster: &remodeling_geo::Raster) -> ImageAsset {
    ...
    let bytes = remodeling_image::encode_png_gray16(&data, raster.width, raster.height).unwrap_or_default();
...
impl RasterPngPreparation {
    pub fn new(raster: remodeling_geo::Raster) -> Self {
        Self {
            raster,
...
pub fn video_codec_to_artifact(codec: remodeling_video::VideoCodec) -> DocumentVideoCodec {
    match codec {
        remodeling_video::VideoCodec::Avc => DocumentVideoCodec::Avc,
        remodeling_video::VideoCodec::Hevc => DocumentVideoCodec::Hevc,
        remodeling_video::VideoCodec::Vp9 => DocumentVideoCodec::Vp9,
        remodeling_video::VideoCodec::Av1 => DocumentVideoCodec::Av1,
        remodeling_video::VideoCodec::Mjpeg => DocumentVideoCodec::Mjpeg,
        remodeling_video::VideoCodec::Unknown(_) => DocumentVideoCodec::Unknown,
    }
}
