//! 🧪️ `remove-content` fixture — `🚫️refuses-missing-content`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `remove-content-missing` is this vector's scenario id in
...
//!
//! 🏞️ content the store never held is a missing target
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔪remove-content/🚫️refuses-missing-content/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔪remove-content/🚫️refuses-missing-content/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔪remove-content/🚫️refuses-missing-content/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔪remove-content/🚫️refuses-missing-content/🎯️outcome/🔣️.json");
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "remove-content/refuses-missing-content: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "remove-content/refuses-missing-content: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "remove-content/refuses-missing-content declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "remove-content/refuses-missing-content: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "remove-content/refuses-missing-content: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.target-missing", "remove-content/refuses-missing-content: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Error, "remove-content/refuses-missing-content: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "remove-content/refuses-missing-content: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🔪remove-content/🚫️refuses-missing-content/🔺️diff/🚫️.absent").is_empty(), "remove-content/refuses-missing-content: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "remove-content/refuses-missing-content: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "remove-content/refuses-missing-content: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "remove-content/refuses-missing-content: committed mutation JSON is not canonical");
}
