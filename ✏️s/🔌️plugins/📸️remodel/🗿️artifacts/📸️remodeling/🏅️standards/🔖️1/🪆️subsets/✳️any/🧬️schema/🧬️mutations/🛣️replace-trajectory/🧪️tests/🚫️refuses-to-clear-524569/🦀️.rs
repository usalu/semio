//! 🧪️ `replace-trajectory` fixture — `🚫️refuses-to-clear-524569`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `replace-trajectory-absent` is this vector's scenario id in
...
//!
//! 🏞️ clearing an already-absent value is an Error against the DOCUMENT id, not a no-op
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🛣️replace-trajectory/🚫️refuses-to-clear-524569/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🛣️replace-trajectory/🚫️refuses-to-clear-524569/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🛣️replace-trajectory/🚫️refuses-to-clear-524569/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🛣️replace-trajectory/🚫️refuses-to-clear-524569/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `replace-trajectory` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "replace-trajectory/refuses-to-clear-524569: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "replace-trajectory/refuses-to-clear-524569: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "replace-trajectory/refuses-to-clear-524569 declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "replace-trajectory/refuses-to-clear-524569: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "replace-trajectory/refuses-to-clear-524569: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.target-missing", "replace-trajectory/refuses-to-clear-524569: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Error, "replace-trajectory/refuses-to-clear-524569: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "replace-trajectory/refuses-to-clear-524569: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🛣️replace-trajectory/🚫️refuses-to-clear-524569/🔺️diff/🚫️.absent").is_empty(), "replace-trajectory/refuses-to-clear-524569: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "replace-trajectory/refuses-to-clear-524569: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "replace-trajectory/refuses-to-clear-524569: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "replace-trajectory/refuses-to-clear-524569: committed mutation JSON is not canonical");
}
