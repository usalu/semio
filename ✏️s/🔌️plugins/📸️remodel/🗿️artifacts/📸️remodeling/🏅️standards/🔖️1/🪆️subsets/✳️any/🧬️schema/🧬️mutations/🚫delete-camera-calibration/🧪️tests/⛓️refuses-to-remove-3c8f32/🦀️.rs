//! 🧪️ `delete-camera-calibration` fixture — `⛓️refuses-to-remove-3c8f32`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `delete-camera-calibration-referenced` is this vector's scenario id in
...
//!
//! 🏞️ a calibration owns nothing, so it may not leave while a stream binding or a rig entry depends on it
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚫delete-camera-calibration/⛓️refuses-to-remove-3c8f32/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚫delete-camera-calibration/⛓️refuses-to-remove-3c8f32/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚫delete-camera-calibration/⛓️refuses-to-remove-3c8f32/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚫delete-camera-calibration/⛓️refuses-to-remove-3c8f32/🎯️outcome/🔣️.json");
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "delete-camera-calibration/refuses-to-remove-3c8f32: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "delete-camera-calibration/refuses-to-remove-3c8f32: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "delete-camera-calibration/refuses-to-remove-3c8f32 declares a rejected outcome");
    ...
    assert_eq!(declared_path, messages[0].target, "delete-camera-calibration/refuses-to-remove-3c8f32: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🚫delete-camera-calibration/⛓️refuses-to-remove-3c8f32/🔺️diff/🚫️.absent").is_empty(), "delete-camera-calibration/refuses-to-remove-3c8f32: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "delete-camera-calibration/refuses-to-remove-3c8f32: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "delete-camera-calibration/refuses-to-remove-3c8f32: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "delete-camera-calibration/refuses-to-remove-3c8f32: committed mutation JSON is not canonical");
}
