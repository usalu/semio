...
pub async fn definition() -> PanelTabDefinition {
    PanelTabDefinition { kind: PanelTabKind::App(REMODELING_PANEL_PARAMETERS_ID.into()), label: LocalizedLabel::native("Parameters", "Parameter"), group: PanelGroup::Details, body_key: Some(REMODELING_PLAY_BODY_PARAMETERS.into()), children: Vec::new() }
}
...
mod tests {
    ...
    async fn the_parameters_panel_reflects_a_live_param_edit() {
        let mut app = app();
        dispatch(&mut app, RemodelingCommand::SetIngestParams(SetIngestParams { frame_sample_stride: 9, max_frames: 200, downscale_long_edge_px: 1600, min_sharpness: 0.3 }));
        assert!(render_body(&mut app, REMODELING_PLAY_BODY_PARAMETERS).contains("stride 9"));
    }
}
//#endregion 🧪️Tests
