//! 🧪️ `add-gcp-observation` fixture — `🚫️refuses-to-pick-3c0570`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `add-gcp-observation-missing` is this vector's scenario id in
...
//!
//! 🏞️ the owner point is resolved before the observation is inspected
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔎add-gcp-observation/🚫️refuses-to-pick-3c0570/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔎add-gcp-observation/🚫️refuses-to-pick-3c0570/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔎add-gcp-observation/🚫️refuses-to-pick-3c0570/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔎add-gcp-observation/🚫️refuses-to-pick-3c0570/🎯️outcome/🔣️.json");
...
/// 🚫️ A refused `add-gcp-observation` leaves the document byte-identical to its committed after-document, which
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "add-gcp-observation/refuses-to-pick-3c0570: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "add-gcp-observation/refuses-to-pick-3c0570: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "add-gcp-observation/refuses-to-pick-3c0570 declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "add-gcp-observation/refuses-to-pick-3c0570: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "add-gcp-observation/refuses-to-pick-3c0570: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.target-missing", "add-gcp-observation/refuses-to-pick-3c0570: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Error, "add-gcp-observation/refuses-to-pick-3c0570: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "add-gcp-observation/refuses-to-pick-3c0570: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🔎add-gcp-observation/🚫️refuses-to-pick-3c0570/🔺️diff/🚫️.absent").is_empty(), "add-gcp-observation/refuses-to-pick-3c0570: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "add-gcp-observation/refuses-to-pick-3c0570: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "add-gcp-observation/refuses-to-pick-3c0570: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "add-gcp-observation/refuses-to-pick-3c0570: committed mutation JSON is not canonical");
}
