//! 🧪️ `change-stream-sync` fixture — `🔁️warns-that-the-a98c13`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `change-stream-sync-noop` is this vector's scenario id in
...
//!
//! 🏞️ an identical resubmission is a Warning with an empty diff, not an Error
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⏱️change-stream-sync/🔁️warns-that-the-a98c13/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⏱️change-stream-sync/🔁️warns-that-the-a98c13/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⏱️change-stream-sync/🔁️warns-that-the-a98c13/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⏱️change-stream-sync/🔁️warns-that-the-a98c13/🎯️outcome/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⏱️change-stream-sync/🔁️warns-that-the-a98c13/🔺️diff/🔣️.json");
...
async fn no_op_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "change-stream-sync/warns-that-the-a98c13: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "change-stream-sync/warns-that-the-a98c13: a no-op must not move the document");
}
...
async fn declared_warning_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "change-stream-sync/warns-that-the-a98c13 declares an applied outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "change-stream-sync/warns-that-the-a98c13: a no-op leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "change-stream-sync/warns-that-the-a98c13: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.no-op", "change-stream-sync/warns-that-the-a98c13: an identical resubmission is a no-op");
    assert_eq!(messages[0].level, protocol::Severity::Warning, "change-stream-sync/warns-that-the-a98c13: a no-op is a Warning, never an Error");
    let committed = pack::parse_json(DIFF).expect("committed diff decodes");
    assert_eq!(json_of(produced.diff()), committed, "change-stream-sync/warns-that-the-a98c13: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "change-stream-sync/warns-that-the-a98c13: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "change-stream-sync/warns-that-the-a98c13: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "change-stream-sync/warns-that-the-a98c13: committed mutation JSON is not canonical");
}
