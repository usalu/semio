//! 🧪️ `replace-stream-source` fixture — `🎥️reingests-the-311c32`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `replace-stream-source-realworld` is this vector's scenario id in
...
//!
//! 🏞️ the replacement keeps the raster consistent with the camera's own principal point
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁replace-stream-source/🎥️reingests-the-311c32/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁replace-stream-source/🎥️reingests-the-311c32/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁replace-stream-source/🎥️reingests-the-311c32/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁replace-stream-source/🎥️reingests-the-311c32/🎯️outcome/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁replace-stream-source/🎥️reingests-the-311c32/🔺️diff/🔣️.json");
...
async fn reaches_the_committed_after_document() {
    ...
    assert_eq!(applied, expected_after(), "replace-stream-source/reingests-the-311c32: applied state differs from committed after-snapshot");
    assert_ne!(applied, before(), "replace-stream-source/reingests-the-311c32: an applied vector must move the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(json_of(outcome.diff()), committed, "replace-stream-source/reingests-the-311c32: produced diff differs from the committed 🔺️diff/🔣️.json");
    let decoded: RemodelingDiff = pack::from_json_str(DIFF).expect("committed diff decodes into the diff type");
    assert_eq!(json_of(&decoded), committed, "replace-stream-source/reingests-the-311c32: committed diff JSON is not canonical");
}
...
async fn committed_diff_carries_before_to_after() {
    ...
    assert_eq!(applied, expected_after(), "replace-stream-source/reingests-the-311c32: committed diff did not carry before to after");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("applied"), "replace-stream-source/reingests-the-311c32 declares an applied outcome");
    ...
    assert_eq!(emitted, declared_codes, "replace-stream-source/reingests-the-311c32: emitted diagnostics differ from the declared ones");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "replace-stream-source/reingests-the-311c32: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "replace-stream-source/reingests-the-311c32: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "replace-stream-source/reingests-the-311c32: committed mutation JSON is not canonical");
}
