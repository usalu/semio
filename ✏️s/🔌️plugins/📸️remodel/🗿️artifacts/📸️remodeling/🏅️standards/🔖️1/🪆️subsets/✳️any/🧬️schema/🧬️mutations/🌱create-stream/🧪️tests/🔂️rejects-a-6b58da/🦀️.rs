//! 🧪️ `create-stream` fixture — `🔂️rejects-a-6b58da`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). `create-stream-duplicate-id` is this vector's scenario id in
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-stream/🔂️rejects-a-6b58da/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-stream/🔂️rejects-a-6b58da/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-stream/🔂️rejects-a-6b58da/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-stream/🔂️rejects-a-6b58da/🎯️outcome/🔣️.json");
...
async fn refusal_leaves_the_document_untouched() {
    ...
    assert_eq!(applied, expected_after(), "create-stream/rejects-a-6b58da: applied state differs from committed after-snapshot");
    assert_eq!(applied, base, "create-stream/rejects-a-6b58da: a refused mutation must not move the document");
}
...
async fn declared_refusal_holds() {
    ...
    assert_eq!(declared.get("status").and_then(|status| status.as_str()), Some("rejected"), "create-stream/rejects-a-6b58da declares a rejected outcome");
    let produced = produced();
    assert_eq!(produced.diff(), &RemodelingDiff::default(), "create-stream/rejects-a-6b58da: a refusing leaf must carry an empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "create-stream/rejects-a-6b58da: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.duplicate-id", "create-stream/rejects-a-6b58da: the declared code must be the emitted one");
    assert_eq!(messages[0].level, protocol::Severity::Fatal, "create-stream/rejects-a-6b58da: the declared level must be the emitted one");
    ...
    assert_eq!(declared_path, messages[0].target, "create-stream/rejects-a-6b58da: the declared path must be the emitted target");
}
...
async fn no_diff_is_committed() {
    assert!(include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-stream/🔂️rejects-a-6b58da/🔺️diff/🚫️.absent").is_empty(), "create-stream/rejects-a-6b58da: the absent-diff marker must stay empty");
}
...
async fn inverse_restores_the_before_document() {
    ...
    assert_eq!(snapshot, base, "create-stream/rejects-a-6b58da: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(json_of(&decoded), original, "create-stream/rejects-a-6b58da: committed {label} JSON is not canonical");
    }
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert_eq!(json_of(&mutation()), original, "create-stream/rejects-a-6b58da: committed mutation JSON is not canonical");
}
