meta:
  id: remodeling_inference
  title: semio remodeling inference — value-bridge record body
  endian: le
doc: |
  `RemodelingInference` carries `ToValue`/`FromValue` only — no `dsl::DslRecord`/`DslDiff` derive, therefore no
