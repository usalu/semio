...
async fn inverse_is_a_single_delete_of_the_cam_b_pose() {
    ...
    assert!(matches!(inverse.as_slice(), [RemodelingMutation::DeleteRigExtrinsic(payload)] if payload.camera_id == "cam-b"), "create-rig-extrinsic's inverse for an unposed camera is one delete-rig-extrinsic, got {inverse:?}");
