...
/// 📦️ Encodes a `RemodelSnapshot` to its binary pack form.
pub fn encode(scene: &RemodelSnapshot) -> Vec<u8> {
    store::DocumentPack::encode_pack(scene)
}

/// 📖️ Decodes a `RemodelSnapshot` from its binary pack form.
pub fn decode(bytes: &[u8]) -> Result<RemodelSnapshot, PackError> {
    <RemodelSnapshot as store::DocumentPack>::decode_pack(bytes)
}
...
mod tests {
    ...
    fn default_scene_roundtrips_through_dsl_pack_equivalence() {
        store::os_store::test_support::assert_dsl_pack_equivalence(&default_remodel_scene());
    }

    #[test]
    fn populated_scene_roundtrips_through_dsl_pack_equivalence() {
        let scene = populated_scene_fixture();
        store::os_store::test_support::assert_dsl_pack_equivalence(&scene);
