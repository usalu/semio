...
mod exporters_tests {
    ...
    fn png_export_round_trips_a_stored_texture_asset() {
        // 🧪️ `remodel_png_export` only looks up and passes through already-stored asset bytes — it
        // never validates PNG-ness — so a stand-in base64 payload exercises the same path without
        // this artifact-side test depending on the app's `images` engine (artifacts must never
        // depend on apps).
        use base64::Engine as _;
        let mut scene = default_remodel_scene();
        let data = base64::engine::general_purpose::STANDARD.encode(b"not-really-a-png-but-4x4");
        scene.assets.insert("tex-1".into(), crate::artifacts::remodel::ImageAsset { mime: "image/png".into(), data: data.clone(), width: 4, height: 4 });
