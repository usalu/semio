...
async fn clears_the_source_while_keeping_the_frames() {
    let applied = apply_remodel_mutation(&before(), &mutation()).expect("replace-stream-source applies to its committed before-snapshot");
...
async fn inverse_restores_the_captured_mp4_source() {
    ...
    assert!(
        matches!(inverse.as_slice(), [RemodelMutation::ReplaceStreamSource(payload)] if payload.id == "stream-a" && payload.source.as_ref().is_some_and(|source| source.name == "front.mp4")),
...
async fn produces_committed_diff() {
    ...
    assert_eq!(encoded, committed, "replace-stream-source/clears-the-video-source-of-stream-a: produced diff differs from the committed 🔺️diff/🔣️component.json");
