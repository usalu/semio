//! 🧪️ `add-gcp-observation` fixture — `adds-the-first-observation-to-gcp-ridge`.
//!
...
async fn pushes_the_observation_onto_gcp_ridge_only() {
    let applied = apply_remodel_mutation(&before(), &mutation()).expect("add-gcp-observation applies to its committed before-snapshot");
...
async fn inverse_removes_the_observation_at_the_base_count() {
    ...
    assert!(
        matches!(inverse.as_slice(), [RemodelMutation::RemoveGcpObservation(payload)] if payload.id == "gcp-ridge" && payload.observation_index == 0),
...
async fn produces_committed_diff() {
    ...
    assert_eq!(encoded, committed, "add-gcp-observation/adds-the-first-observation-to-gcp-ridge: produced diff differs from the committed 🔺️diff/🔣️component.json");
