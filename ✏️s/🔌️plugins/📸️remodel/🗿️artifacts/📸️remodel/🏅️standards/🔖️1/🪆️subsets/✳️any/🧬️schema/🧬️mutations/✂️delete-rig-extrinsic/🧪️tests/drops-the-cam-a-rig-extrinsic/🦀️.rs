//! 🧪️ `delete-rig-extrinsic` fixture — `drops-the-cam-a-rig-extrinsic`.
//!
...
async fn empties_the_rig_while_keeping_the_cam_a_record() {
    let applied = apply_remodel_mutation(&before(), &mutation()).expect("delete-rig-extrinsic applies to its committed before-snapshot");
...
async fn inverse_recreates_the_captured_identity_pose() {
    ...
    assert!(
        matches!(inverse.as_slice(), [RemodelMutation::CreateRigExtrinsic(payload)] if payload.extrinsic.camera_id == "cam-a" && payload.extrinsic.rotation_wxyz == [1.0, 0.0, 0.0, 0.0]),
