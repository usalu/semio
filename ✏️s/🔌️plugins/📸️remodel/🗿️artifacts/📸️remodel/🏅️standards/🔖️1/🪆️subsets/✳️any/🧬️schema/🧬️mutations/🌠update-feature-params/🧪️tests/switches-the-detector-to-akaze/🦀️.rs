//! 🧪️ `update-feature-params` fixture — `switches-the-detector-to-akaze`.
//!
...
use crate::artifacts::remodel::FeatureDetector;
...
async fn writes_the_akaze_detector_and_its_new_budget() {
    let applied = apply_remodel_mutation(&before(), &mutation()).expect("update-feature-params applies to its committed before-snapshot");
...
async fn inverse_restores_the_base_orb_detector() {
    ...
    assert!(
        matches!(inverse.as_slice(), [RemodelMutation::UpdateFeatureParams(payload)] if payload.params.detector == FeatureDetector::Orb && payload.params.target_count == 4000),
