...
//#region 🔖️Inverse
pub fn inverse(payload: &super::DeleteAsset, base: &RemodelSnapshot) -> Vec<RemodelMutation> {
    match remodel_asset(base, &payload.key) {
        Some(old) => vec![crate::artifacts::remodel::mutations::create_asset::create_asset(payload.key.clone(), old)],
        None => Vec::new(),
    }
}
//#endregion 🔖️Inverse
