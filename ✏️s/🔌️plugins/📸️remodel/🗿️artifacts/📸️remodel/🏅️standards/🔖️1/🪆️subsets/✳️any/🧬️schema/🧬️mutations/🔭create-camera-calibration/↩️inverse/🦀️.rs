...
//#region 🔖️Inverse
pub fn inverse(payload: &super::CreateCameraCalibration, base: &RemodelSnapshot) -> Vec<RemodelMutation> {
    if base.calibration.cameras.iter().any(|camera| camera.id == payload.camera.id) {
        return Vec::new();
    }
    vec![crate::artifacts::remodel::mutations::delete_camera_calibration::delete_camera_calibration(payload.camera.id.clone())]
}
//#endregion 🔖️Inverse
