//! 🧮️ Remodel play app — the `ArtifactEditor::Config` view state and its operation vocabulary.
//!
...
/// 🧮️ Remodel's `ArtifactEditor::Config` — absorbs every former `RemodelPlayRuntime` view/session field
...
impl Default for RemodelConfig {
    fn default() -> Self {
        Self { camera: RemodelWorldCamera::default(), layers: RemodelLayerVisibility::default(), frame_cursor: RemodelFrameCursor::default(), report_table: "frames".into(), active_utility_id: "select".into(), locale: "en-US".into() }
    }
}
