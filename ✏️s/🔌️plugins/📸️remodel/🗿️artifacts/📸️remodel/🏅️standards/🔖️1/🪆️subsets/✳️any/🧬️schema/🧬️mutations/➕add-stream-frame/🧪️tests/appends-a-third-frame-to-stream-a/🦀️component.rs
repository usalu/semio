//! 🧪️ `add-stream-frame` fixture — `appends-a-third-frame-to-stream-a`.
//!
...
async fn appends_frame_two_and_restamps_the_stream_kind() {
    let applied = apply_remodel_mutation(&before(), &mutation()).expect("add-stream-frame applies to its committed before-snapshot");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(encoded, committed, "add-stream-frame/appends-a-third-frame-to-stream-a: produced diff differs from the committed 🔺️diff/🔣️component.json");
