...
async fn enables_tracking_without_touching_the_existing_track() {
    let applied = apply_remodel_mutation(&before(), &mutation()).expect("update-motion-params applies to its committed before-snapshot");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(encoded, committed, "update-motion-params/enables-motion-tracking: produced diff differs from the committed 🔺️diff/🔣️component.json");
