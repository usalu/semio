...
//#region 🔖️Diff
pub fn diff(payload: &super::DeleteAsset, base: &RemodelSnapshot) -> protocol::MutationOutcome<RemodelDiff> {
    ...
    let outcome = protocol::MutationOutcome::new(RemodelDiff { assets: Some(assets), ..Default::default() });
    if stale_refs == 0 {
        outcome
    } else {
