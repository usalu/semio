//! 🎹️ RemodelComposer (1/✳️any) — analyzer + builder glued. Reads native `s.remodel` sources
//! plus any of: stdio.dwg, stdio.glb, stdio.gltf, stdio.json, stdio.las, stdio.obj, stdio.ply, stdio.png, stdio.stl, stdio.txt. Writes one `s.remodel` (1/✳️any) snapshot.
...
use crate::artifacts::remodel::standards::v1::subsets::any::analyzer::RemodelAnalyzer;
...
pub struct RemodelComposer;

impl ArtifactComposer for RemodelComposer {
    ...
    fn compose(sources: &[ComposeSource]) -> Result<Composition<Self::Snapshot>, ComposeError> {
        for source in sources {
            ...
            if source.dialect == DEP_GLB {
                ...
                if let Ok(snapshot) = crate::artifacts::remodel::io::import::deserializers::artifacts::glb::v2_0::any::deserialize_bytes(&bytes) {
                    return Ok(Composition { snapshot, confidence: semio_framework_plugin::IoConfidence::Medium, diagnostics: Vec::new() });
                }
            }
        ...
        Err(ComposeError { message: "RemodelComposer: no source in a known read dialect".into(), diagnostics: Vec::new() })
    }
}
