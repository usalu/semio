...
async fn fills_in_the_two_absent_raster_references() {
    let applied = apply_remodel_mutation(&before(), &mutation()).expect("replace-geo-products applies to its committed before-snapshot");
...
async fn inverse_restores_the_dsm_only_base_record() {
    ...
    assert!(
        matches!(inverse.as_slice(), [RemodelMutation::ReplaceGeoProducts(payload)] if payload.geo.as_ref().is_some_and(|geo| geo.dtm_asset_id.is_none() && geo.ortho_asset_id.is_none())),
...
async fn produces_committed_diff() {
    ...
    assert_eq!(encoded, committed, "replace-geo-products/adds-the-dtm-and-ortho-rasters: produced diff differs from the committed 🔺️diff/🔣️component.json");
