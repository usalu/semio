...
//#region 🔖️Inverse
pub fn inverse(payload: &super::CreateRigExtrinsic, base: &RemodelSnapshot) -> Vec<RemodelMutation> {
    if base.calibration.rig.iter().any(|extrinsic| extrinsic.camera_id == payload.extrinsic.camera_id) {
        return Vec::new();
    }
    vec![crate::artifacts::remodel::mutations::delete_rig_extrinsic::delete_rig_extrinsic(payload.extrinsic.camera_id.clone())]
}
//#endregion 🔖️Inverse
