//! 🧪️ `replace-trajectory` fixture — `clears-the-camera-trajectory`.
//!
...
async fn clears_the_trajectory_while_the_job_pose_preview_survives() {
    let applied = apply_remodel_mutation(&before(), &mutation()).expect("replace-trajectory applies to its committed before-snapshot");
...
async fn inverse_restores_the_two_pose_trajectory() {
    ...
    assert!(
        matches!(inverse.as_slice(), [RemodelMutation::ReplaceTrajectory(payload)] if payload.trajectory.as_ref().is_some_and(|trajectory| trajectory.poses.len() == 2)),
...
async fn produces_committed_diff() {
    ...
    assert_eq!(encoded, committed, "replace-trajectory/clears-the-camera-trajectory: produced diff differs from the committed 🔺️diff/🔣️component.json");
