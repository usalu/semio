...
impl ImageGray {
    ...
    pub async fn get(&self, x: u32, y: u32) -> f32 {
    ...
    pub async fn from_rgba8_luma(src: &ImageRgba8) -> Self {
    ...
    pub async fn sample(&self, x: f32, y: f32) -> f32 {
...
impl ImageRgba8 {
    ...
    pub async fn get_rgba(&self, x: u32, y: u32) -> [u8; 4] {
    ...
    pub async fn sample_rgb(&self, x: f32, y: f32) -> [f32; 3] {
        if self.width == 0 || self.height == 0 {
            return [0.0; 3];
        }
        let xf = x.clamp(0.0, (self.width - 1) as f32);
...
pub async fn decode_png(bytes: &[u8]) -> Result<ImageRgba8, ImageError> {
...
pub async fn encode_png(img: &ImageRgba8) -> Result<Vec<u8>, ImageError> {
...
pub async fn encode_png_gray16(data: &[u16], width: u32, height: u32) -> Result<Vec<u8>, ImageError> {
...
pub async fn decode_jpeg(bytes: &[u8]) -> Result<ImageRgba8, ImageError> {
...
// #region 🔖️Filter
async fn mirror_index(i: i64, n: i64) -> i64 {
    let mut i = i;
    loop {
        if i < 0 {
            i = -1 - i;
...
pub async fn gaussian_blur(img: &ImageGray, sigma: f32) -> ImageGray {
...
pub async fn box_blur_integral(img: &ImageGray, radius: u32) -> ImageGray {
...
impl IntegralImage {
    ...
    pub async fn build(img: &ImageGray) -> Self {
    ...
    pub async fn rect_sum(&self, x0: u32, y0: u32, x1: u32, y1: u32) -> f64 {
    ...
    pub async fn rect_sq_sum(&self, x0: u32, y0: u32, x1: u32, y1: u32) -> f64 {
        let stride = self.width as usize + 1;
        let (x0, y0, x1, y1) = (x0 as usize, y0 as usize, x1 as usize, y1 as usize);
...
pub async fn downsample_half(img: &ImageGray) -> ImageGray {
    if img.width == 0 || img.height == 0 {
        return img.clone();
    }
...
pub async fn scharr_gradients(img: &ImageGray) -> GradientField {
    let (w, h) = (img.width as i64, img.height as i64);
...
pub async fn gradient_magnitude_orientation(g: &GradientField) -> (Vec<f32>, Vec<f32>) {
...
pub async fn build_pyramid(img: &ImageGray, n_levels: usize) -> Pyramid {
...
pub async fn extract_patch(img: &ImageGray, cx: f32, cy: f32, radius: u32, rotation: f32) -> Patch {
...
pub async fn zncc(a: &Patch, b: &Patch) -> f32 {
...
pub async fn ssd(a: &Patch, b: &Patch) -> f32 {
...
pub async fn census_transform(img: &ImageGray, radius: u32) -> Vec<u64> {
...
pub async fn hamming_cost_census(a: u64, b: u64) -> u32 {
    (a ^ b).count_ones()
}
// #endregion 🔖️Patch
...
pub async fn warp_affine(img: &ImageGray, m: &[[f32; 3]; 2], out_w: u32, out_h: u32) -> ImageGray {
    let mut out = ImageGray::new(out_w, out_h);
    for y in 0..out_h {
        for x in 0..out_w {
            let fx = x as f32;
...
pub async fn warp_homography(img: &ImageGray, h: &[[f64; 3]; 3], out_w: u32, out_h: u32) -> ImageGray {
    let mut out = ImageGray::new(out_w, out_h);
    for y in 0..out_h {
        for x in 0..out_w {
            let fx = f64::from(x);
...
pub async fn remap(img: &ImageGray, map_x: &[f32], map_y: &[f32], out_w: u32, out_h: u32) -> ImageGray {
    let mut out = ImageGray::new(out_w, out_h);
...
pub async fn non_max_suppression(scores: &[f32], width: u32, height: u32, radius: u32, threshold: f32) -> Vec<(u32, u32, f32)> {
...
mod tests {
    ...
    async fn lcg_image(width: u32, height: u32, seed: u32) -> ImageGray {
    ...
    async fn smooth_image(width: u32, height: u32) -> ImageGray {
        let mut img = ImageGray::new(width, height);
        for y in 0..height {
            for x in 0..width {
    ...
    async fn mean_and_variance(img: &ImageGray) -> (f64, f64) {
    ...
    async fn invert3(h: &[[f64; 3]; 3]) -> [[f64; 3]; 3] {
    ...
    async fn gradient_rgba8(width: u32, height: u32) -> ImageRgba8 {
    ...
    async fn checkerboard_rgba8(width: u32, height: u32, cell: u32) -> ImageRgba8 {
        let mut img = ImageRgba8::new(width, height);
        for y in 0..height {
            for x in 0..width {
                let idx = ((y * width + x) * 4) as usize;
    ...
    async fn flat_rgba8(width: u32, height: u32, rgb: [u8; 3]) -> ImageRgba8 {
        let mut img = ImageRgba8::new(width, height);
    ...
    async fn psnr_rgba8(a: &ImageRgba8, b: &ImageRgba8) -> f64 {
