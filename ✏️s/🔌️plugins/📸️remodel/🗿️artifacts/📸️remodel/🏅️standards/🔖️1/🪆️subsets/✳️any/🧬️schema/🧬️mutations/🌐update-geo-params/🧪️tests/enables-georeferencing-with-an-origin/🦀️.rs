...
async fn fills_in_the_origin_while_enabling_georeferencing() {
    let applied = apply_remodel_mutation(&before(), &mutation()).expect("update-geo-params applies to its committed before-snapshot");
...
async fn inverse_restores_the_null_origin() {
    ...
    assert!(
        matches!(inverse.as_slice(), [RemodelMutation::UpdateGeoParams(payload)] if !payload.params.enabled && payload.params.origin_lat.is_none()),
