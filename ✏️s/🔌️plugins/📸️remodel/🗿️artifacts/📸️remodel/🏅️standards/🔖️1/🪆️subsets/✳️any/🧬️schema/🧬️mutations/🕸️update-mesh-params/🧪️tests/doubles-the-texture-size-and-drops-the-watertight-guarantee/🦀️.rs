//! 🧪️ `update-mesh-params` fixture — `doubles-the-texture-size-and-drops-the-watertight-guarantee`.
//!
...
async fn drops_the_guarantee_without_clearing_the_recorded_watertight_report() {
    let applied = apply_remodel_mutation(&before(), &mutation()).expect("update-mesh-params applies to its committed before-snapshot");
