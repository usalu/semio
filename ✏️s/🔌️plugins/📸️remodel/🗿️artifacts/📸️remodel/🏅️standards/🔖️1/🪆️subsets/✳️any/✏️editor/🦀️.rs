...
pub fn remodel_action(action: &str, args: Option<semio_framework_plugin::UiValue>) -> semio_framework_plugin::UiAssemblyResult<(semio_framework_plugin::ActionId, Option<semio_framework_plugin::UiValue>)> {
...
/// `artifact_kind()` (`🗿️artifacts/📸️remodel/🦀️.rs`), which already carries the real dialect
...
impl ArtifactEditor for RemodelPlayApp {
    ...
    semio_framework_plugin::bounded_first_step_tool_proofs! {
        ...
        owner_file: "✏️s/🔌️plugins/📸️remodel/🗿️artifacts/📸️remodel/🏅️standards/🔖️1/🪆️subsets/✳️any/✏️editor/🦀️.rs",
    ...
    async fn render(body_key: &str, doc: &ArtifactView<'_, RemodelSnapshot>, cfg: &ConfigView<'_, RemodelConfig>) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::ComponentTree> {
        let scene = doc.snapshot;
...
mod tests {
    ...
    async fn retained_command_catalog_matches_the_serde_json_oracle() {
        let oracle = SerdeJsonRemodelRetainedCatalogOracle.summarize(include_str!("🧪️fixtures/🧫️retained-command-limits/🔣️.json"));
    ...
    //#region 🔖️IoTests
    /// 🧪️ Relocated from the artifact's `⚙️engine/🦀️.rs` (#2553): `remodel_io()` returns
