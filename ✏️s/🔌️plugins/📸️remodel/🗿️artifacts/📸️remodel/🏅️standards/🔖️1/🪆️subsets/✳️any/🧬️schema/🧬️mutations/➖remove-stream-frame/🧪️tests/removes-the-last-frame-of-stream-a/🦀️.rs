//! 🧪️ `remove-stream-frame` fixture — `removes-the-last-frame-of-stream-a`.
//!
...
async fn removes_the_frame_at_base_position_one() {
    let applied = apply_remodel_mutation(&before(), &mutation()).expect("remove-stream-frame applies to its committed before-snapshot");
...
async fn inverse_re_appends_the_captured_last_frame() {
    ...
    assert!(
        matches!(inverse.as_slice(), [RemodelMutation::AddStreamFrame(payload)] if payload.id == "stream-a" && payload.frame.index == 1 && payload.kind == MediaKind::Video),
