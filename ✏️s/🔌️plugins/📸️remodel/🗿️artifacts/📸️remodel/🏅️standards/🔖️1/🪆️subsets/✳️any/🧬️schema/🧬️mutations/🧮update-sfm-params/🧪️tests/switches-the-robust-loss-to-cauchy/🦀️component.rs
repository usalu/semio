//! 🧪️ `update-sfm-params` fixture — `switches-the-robust-loss-to-cauchy`.
//!
...
use crate::artifacts::remodel::RobustLossKind;
...
async fn writes_the_cauchy_loss_and_the_longer_ransac_budget() {
    let applied = apply_remodel_mutation(&before(), &mutation()).expect("update-sfm-params applies to its committed before-snapshot");
...
async fn inverse_restores_the_base_huber_loss() {
    ...
    assert!(
        matches!(inverse.as_slice(), [RemodelMutation::UpdateSfmParams(payload)] if payload.params.robust_loss == RobustLossKind::Huber && payload.params.ransac_iterations == 1000),
...
async fn produces_committed_diff() {
    ...
    assert_eq!(encoded, committed, "update-sfm-params/switches-the-robust-loss-to-cauchy: produced diff differs from the committed 🔺️diff/🔣️component.json");
