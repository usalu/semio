...
use crate::editor::remodel::engine::{camera as remodel_camera, dense as remodel_dense, images as remodel_image, mesh as remodel_mesh, sfm as remodel_sfm};

use std::collections::HashMap;

pub use crate::lie::Sim3;
pub use remodel_camera::{CameraPose, Intrinsics};
pub use remodel_mesh::WatertightReport;
pub use remodel_sfm::Reconstruction;

use crate::algebra::{pseudo_inverse, solve_llsq, vec3d_normalize, vec3d_sub, MatD, VecD};
use crate::lie::umeyama;
use crate::optimize::{camera_covariances, LmConfig, ResidualTerm, SchurResult};
...
impl Raster {
    /// 🧱️ Starts an owned raster without performing cell-count-sized initialization.
    pub fn empty(width: u32, height: u32, cell_size: f64, origin: [f64; 2]) -> Self {
        Self { width, height, cell_size, origin, values: Vec::new(), valid: Vec::new() }
    }

    /// ⏱️ Appends at most `cell_budget` invalid cells and reports whether allocation is complete.
    pub fn extend_invalid(&mut self, cell_budget: usize) -> bool {
        let target = self.width as usize * self.height as usize;
        let end = self.values.len().saturating_add(cell_budget).min(target);
    ...
    pub fn new(width: u32, height: u32, cell_size: f64, origin: [f64; 2]) -> Self {
        let mut raster = Self::empty(width, height, cell_size, origin);
        raster.extend_invalid(usize::MAX);
        raster
    }
...
/// [`SfmBundleProblem`] from `recon`+`observations` and running [`crate::optimize::schur_lm`] for a
...
pub fn camera_covariance_diagonals(recon: &Reconstruction, observations: &[(usize, usize, [f64; 2])]) -> Vec<[f64; 36]> {
    ...
    let result: SchurResult = crate::optimize::schur_lm(&problem, a0, b0, &cfg);
