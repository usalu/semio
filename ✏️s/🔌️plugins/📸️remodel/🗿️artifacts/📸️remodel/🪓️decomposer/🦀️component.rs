...
pub struct RemodelParts { pub snapshot: Option<WatertightReportSnapshot> }
...
impl ArtifactDecomposer for RemodelDecomposer {
    ...
    fn decompose(sources: &[DecomposeSource<'_>]) -> Decomposition<Self::Parts> {
        ...
        for source in sources {
            match source {
                DecomposeSource::Text(text) => match <WatertightReportSnapshot as store::DocumentDsl>::parse_dsl(text) {
                ...
                DecomposeSource::Binary(bytes) => match <WatertightReportSnapshot as store::DocumentPack>::decode_pack(bytes) {
                    Ok(snapshot) => parts.snapshot = Some(snapshot),
                    Err(err) => {
                        confidence = Confidence::Low;
