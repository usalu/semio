#!/usr/bin/env bun
/** 🌍️ `@semio-tech/remodel-geo` — Georeferencing and survey products: GCPs, geodetic transforms, DSM/DTM, orthomosaics, contours, volumes and quality reports. */
...
class TestScript extends BundleScript {
  run(segments: string[]): void {
    ...
    runCargoTestBudgeted(["remodel_geo"], join(this.root, "rs"), rest);
  }
}

class LintScript extends BundleScript {
  run(): void {
    runCargoLint(["remodel_geo"], join(this.root, "rs"));
  }
}
