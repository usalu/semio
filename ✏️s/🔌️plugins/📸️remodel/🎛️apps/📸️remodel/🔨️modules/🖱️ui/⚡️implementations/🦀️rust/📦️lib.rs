...
impl RemodelPlayApp {
    ...
    fn handle_import_frame_payload(&self, payload: &str, name: &str, index: u32, doc: &DocumentView<'_, RemodelScene>) -> Emit<RemodelOperation, RemodelConfigOperation> {
        let Some((mime, bytes)) = payload_from_data_url(payload) else { return Ok(Emit::default() };
    ...
    fn handle_import_video_frame_payload(&self, payload: &str, name: &str, index: u32, frame_index: u32, timestamp_ms: f64, doc: &DocumentView<'_, RemodelScene>) -> Emit<RemodelOperation, RemodelConfigOperation> {
        let Some((_mime, bytes)) = payload_from_data_url(payload) else { return Ok(Emit::default() };
    ...
    fn handle(&self, command: &RemodelCommand, doc: &DocumentView<'_, RemodelScene>, _cfg: &ConfigView<'_, RemodelConfig>) -> Result<Emit<RemodelOperation, RemodelConfigOperation>, Fault> {
        ...
        match command {
            ...
            RemodelCommand::PlaceGcpObservation { gcp_id, stream_id, frame_index, pixel_x, pixel_y } => {
                let mut gcps = scene.gcps.clone();
                let Some(gcp) = gcps.iter_mut().find(|gcp| &gcp.id == gcp_id) else { return Ok(Emit::default() };
            ...
            RemodelCommand::SetIngestParams { frame_sample_stride, max_frames, downscale_long_edge_px, min_sharpness } => {
                Ok(Emit::operations(vec![RemodelOperation::SetIngestParams { params: IngestParams { frame_sample_stride: *frame_sample_stride, max_frames: *max_frames, downscale_long_edge_px: *downscale_long_edge_px, min_sharpness: *min_sharpness } }])
            }
