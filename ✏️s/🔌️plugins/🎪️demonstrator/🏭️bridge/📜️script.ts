...
// 🏭️ Language-neutral production mutation bridge for every subset under `✏️s/🔌️plugins/🎪️demonstrator`.
//
...
function main(argv: readonly string[]): number {
  ...
  const built = spawnSync("cargo", ["run", "--quiet", "--offline", "--bin", "semio-demonstrator-mutation-bridge", "--", command, artifact, standard, subset, ...surface], {
    cwd: import.meta.dir,
    encoding: "utf8",
  });
  if (built.status !== 0) {
