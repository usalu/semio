//! 🎪️ The demonstrator's six panes, assembled into one plugin bundle.
//!
...
mod tests {
    ...
    fn bundle_registers_the_six_demonstrator_panes() {
        let ids: Vec<String> = bundle().manifest.apps.iter().map(|app| app.id.clone()).collect();
    ...
    fn every_pane_declares_a_document_schema() {
        for app in bundle().manifest.apps {
