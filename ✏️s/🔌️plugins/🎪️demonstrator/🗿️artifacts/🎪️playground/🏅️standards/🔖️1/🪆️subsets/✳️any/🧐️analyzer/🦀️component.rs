//! 🧐️ PlaygroundAnalyzer (1/✳️any) — read-only analysis, successor to the pre-migration
//! PlaygroundDecomposer. Real logic; artifact/standard levels delegate here.
...
impl ArtifactAnalyzer for PlaygroundAnalyzer {
    ...
    fn analyze(sources: &[AnalyzeSource<'_>]) -> Analysis<Self::Parts> {
        ...
        for source in sources {
            match source {
                AnalyzeSource::Text(text) => match <PlaygroundSnapshot as store::DocumentDsl>::parse_dsl(text) {
                ...
                AnalyzeSource::Binary(bytes) => match <PlaygroundSnapshot as store::DocumentPack>::decode_pack(bytes) {
                    Ok(snapshot) => parts.snapshot = Some(snapshot),
                    Err(err) => {
                        confidence = IoConfidence::Low;
