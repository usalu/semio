...
//! guarding is the WIRE SHAPE: `PlaygroundMutation` is the only mutation enum in the repository that
//! encodes EXTERNALLY tagged with a snake_case payload field, `{"ChangeSchema": {"new_schema": …}}`.
//! The committed vector is read literally here and decoded by the production bridge on the subject
//! side, so a serde attribute added to that enum in passing breaks this case rather than silently
//! changing the format.
//!
...
fn canonical(text: &str) -> Json {
    parse_json(text).unwrap_or_else(|error| panic!("mutate-playground-1: a committed fixture must be valid JSON: {error}"))
}
...
mod subject {
    ...
    use semio_s_plugin_stdio_test_oracle::law;
    use semio_s_plugin_demonstrator::artifacts::playground::standards::v1::subsets::any::schema::mutations::{apply_playground_mutation_json, round_trip_playground_dsl, undo_playground_mutation_json};
    ...
    pub fn mutate(kind: &'static str) -> impl Fn(&Context) -> Result<Outcome, String> {
        move |_ctx: &Context| {
            ...
            let (document, codes) = answer(&apply_playground_mutation_json(before, mutation)?)?;
    ...
    pub fn inverse(kind: &'static str) -> impl Fn(&Context) -> Result<Outcome, String> {
        move |_ctx: &Context| {
            ...
            let (document, _codes) = answer(&undo_playground_mutation_json(before, mutation)?)?;
