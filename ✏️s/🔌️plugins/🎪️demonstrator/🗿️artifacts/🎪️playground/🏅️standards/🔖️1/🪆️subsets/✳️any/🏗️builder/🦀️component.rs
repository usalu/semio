...
impl ArtifactBuilder for PlaygroundBuilder {
    ...
    fn from_text(text: &str) -> Result<Self, store::TextError> {
        Ok(Self::from_snapshot(<PlaygroundSnapshot as store::DocumentDsl>::parse_dsl(text)?))
    }
    fn from_binary(bytes: &[u8]) -> Result<Self, store::PackError> {
        Ok(Self::from_snapshot(<PlaygroundSnapshot as store::DocumentPack>::decode_pack(bytes)?))
    }
