//! 👁️ Playground viewer — the `view` mode: a single read-only window over the document's one
//! `schema` field, the read-only counterpart of the editor's `edit` mode.
...
pub const PLAYGROUND_VIEW_MODE_VIEW: &str = "view";
