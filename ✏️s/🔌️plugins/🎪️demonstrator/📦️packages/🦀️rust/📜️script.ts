#!/usr/bin/env bun
/** 🎪️ `@semio-tech/demonstrator-plugin` router: `bun ./📜️script.ts test`. */
...
class TestScript extends BundleScript {
  run(_segments: string[]): void {
    runCargoTestBudgeted(["semio-s-plugin-demonstrator"], this.repoRoot);
  }
}
...
class DescribeScript extends BundleScript {
  run(): void {
    process.exit(describePluginComponent(this.repoRoot, "semio-s-plugin-demonstrator", join(this.root, "..", "..")));
  }
}
