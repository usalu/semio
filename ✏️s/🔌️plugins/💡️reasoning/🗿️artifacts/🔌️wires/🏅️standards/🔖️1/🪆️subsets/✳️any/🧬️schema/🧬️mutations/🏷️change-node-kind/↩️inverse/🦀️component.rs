...
//#region 🔖️Inverse
pub async fn inverse(payload: &super::mutation::ChangeNodeKind, base: &WiresSnapshot) -> Vec<WiresMutation> {
    match find_board_node(base, &payload.node_id).and_then(|node| node.get("nodeKind").and_then(|value| value.as_str()).map(str::to_string)) {
        Some(old_kind) => vec![crate::artifacts::wires::mutations::change_node_kind::mutation::change_node_kind(payload.node_id.clone(), old_kind)],
        None => Vec::new(),
    }
}
//#endregion 🔖️Inverse
