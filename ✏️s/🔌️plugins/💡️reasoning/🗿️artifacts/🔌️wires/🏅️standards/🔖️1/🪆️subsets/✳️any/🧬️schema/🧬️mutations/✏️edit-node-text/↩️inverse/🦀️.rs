...
//#region 🔖️Inverse
pub async fn inverse(payload: &super::EditNodeText, base: &WiresSnapshot) -> Vec<WiresMutation> {
    match find_board_node(base, &payload.node_id).and_then(|node| node.get("text").and_then(|value| value.as_str()).map(str::to_string)) {
        Some(old_text) => vec![crate::artifacts::wires::mutations::edit_node_text::edit_node_text(payload.node_id.clone(), old_text)],
        None => Vec::new(),
    }
}
//#endregion 🔖️Inverse
