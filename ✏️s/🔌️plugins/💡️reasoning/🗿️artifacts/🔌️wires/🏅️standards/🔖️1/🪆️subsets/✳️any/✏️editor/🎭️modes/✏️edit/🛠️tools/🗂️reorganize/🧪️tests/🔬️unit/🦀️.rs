...
use semio_framework_tool_run::{ToolRunId, ToolRunIdentity, ToolRunVerdict, TOOL_RUN_ACTION_IDS};
...
/// the synchronous `forceLayout` verb is gone.
#[test]
fn reorganize_tool_declares_the_shared_layout_run_definition() {
    ...
    let app = crate::editor::wires::create_wires_app();
    ...
    for removed in tool["removedActions"].as_array().expect("removed").iter().chain(std::iter::once(&tool["id"])) {
        let removed = removed.as_str().expect("removed id");
        assert!(<ReasoningWiresPlayApp as ArtifactEditor>::command_from_action(removed, None).is_err(), "{removed} no longer resolves to a synchronous command");
    }
}

/// 🕸️ Visible nodes keep their first occurrence, rectangles repel by half their diagonal, locked placed nodes pin,
/// unplaced nodes are seeded; edges resolve handles, drop self loops, hidden, dangling and duplicate springs.
#[test]
fn wires_board_maps_to_the_language_neutral_layout_graph() {
    let fixture = fixture();
    ...
    for (node, row) in layout.graph.nodes.iter().zip(expected_nodes) {
        ...
        assert_eq!((node.radius, node.pinned, node.anchor), (row["radius"].as_f64().expect("radius"), row["pinned"].as_bool().expect("pinned"), None));
    }
...
/// 🎞️ The metabolism run: every iteration is one fuel unit and one tick, the provisional list ends with exactly one
/// `move-node` per moved node carrying its entity, and every node ends with its settled verdict.
#[test]
fn reorganize_run_over_the_metabolism_example_matches_the_fixture() {
    let fixture = fixture();
    let law = &fixture["run"];
    let document = metabolism();
    ...
    let mut job = LayoutRunJob::new(identity(), &layout.graph, LayoutRunConfig::default(), move_encoder(layout.node_ids.clone())).expect("job");
    ...
    assert_eq!(format!("{digest:016x}"), law["positionsDigest"].as_str().expect("digest"), "the metabolism layout is bit-deterministic");
    ...
    assert!(matches!(job.stop(), Some(LayoutRunStop::Converged | LayoutRunStop::IterationLimit)));
    let per_iteration = recording.step_micros.len() as u64;
    assert!(per_iteration >= law["iterations"].as_u64().expect("iterations"), "at least one step per iteration");
}
...
fn reorganize_layout_quality_matches_the_fdg_sim_oracle() {
    ...
    let layout = layout_graph(&metabolism());
    ...
    assert!(mine <= reference * oracle["stressRatioMax"].as_f64().expect("ratio"), "stress {mine:.4} vs fdg-sim {reference:.4}");
    assert!(mine <= start * (1.0 - oracle["improvementMin"].as_f64().expect("improvement")), "stress {mine:.4} vs initial {start:.4}");
}
...
#[test]
fn reorganize_step_stays_below_the_interactive_ceiling_on_the_metabolism_example() {
    let fixture = fixture();
...
fn pump_until(app: &mut WiresApp, what: &str, done: impl Fn(&WiresApp) -> bool) {
...
/// them as one history entry at the run's final positions, and one undo restores every committed position.
#[test]
