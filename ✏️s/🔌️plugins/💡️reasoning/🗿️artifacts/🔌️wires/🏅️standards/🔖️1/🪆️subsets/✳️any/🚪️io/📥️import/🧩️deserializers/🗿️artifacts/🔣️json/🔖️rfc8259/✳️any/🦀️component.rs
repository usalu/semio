...
pub fn deserialize(from: &JsonSnapshot) -> Result<WiresSnapshot, store::TextError> {
    let _ = MINDMAP_WIRES_SCHEMA;
    let mut out: WiresSnapshot = serde_json::from_value(from.value.clone())
        .map_err(|e| store::TextError::new(format!("wires<-json: {e}"), dsl::TextSpan::at(1, 1)))?;
    Ok(out)
}
