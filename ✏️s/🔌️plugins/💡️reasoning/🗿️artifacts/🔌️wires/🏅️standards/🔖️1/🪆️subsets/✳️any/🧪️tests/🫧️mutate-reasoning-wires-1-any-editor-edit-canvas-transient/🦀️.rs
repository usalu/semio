//! 🫧️ `s.reasoning.wires` ✏️editor/🎭️modes/✏️edit/🪟️windows/🕸️canvas/🫧️transient state-lane mutation case — Rust adapter.
//!
//! Recorded no-oracle decision `reasoning-wires-1-any-editor-edit-canvas-transient-state-lane-semantics`: the runner dispatches no oracle role, so every law is asserted inside
//! the subject handlers through `semio_s_plugin_stdio_test_oracle::law::vector` over the report of this crate's
//! production bridge `wires_canvas_transient_mutation_report_json`. The oracle handlers answer with the committed after- and before-snapshots read
...
//#region 🔖️Vectors
/// 🧫️ The committed applied vector of one kind, read literally from `✏️editor/🎭️modes/✏️edit/🪟️windows/🕸️canvas/🫧️transient/🧫️fixtures`.
...
mod subject {
    ...
    use semio_s_artifact_reasoning_wires::editor::wires::window_transient::wires_canvas_transient_mutation_report_json;

    fn report(committed: &Vector) -> Result<String, String> {
        wires_canvas_transient_mutation_report_json(committed.before, committed.mutation, committed.after)
    }
