//! 🧪️ `set-node-root` fixture — `📓️reports-a-no-op-when-an-unflagged-node-is-set-to-not-root`.
//!
...
use crate::mutations::{SetNodeRoot, WiresMutation};
...
/// the exact child owner from the snapshot's own persisted `wiresFixture.board` mirror — no node is invented here.
