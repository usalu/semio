...
//#region 🔖️Inverse
pub async fn inverse(payload: &super::ChangeNodeShape, base: &WiresSnapshot) -> Vec<WiresMutation> {
    match find_board_node(base, &payload.node_id).and_then(|node| node.get("shape").and_then(|value| value.as_str()).map(str::to_string)) {
        Some(old_shape) => vec![crate::artifacts::wires::mutations::change_node_shape::change_node_shape(payload.node_id.clone(), old_shape)],
        None => Vec::new(),
    }
}
//#endregion 🔖️Inverse
