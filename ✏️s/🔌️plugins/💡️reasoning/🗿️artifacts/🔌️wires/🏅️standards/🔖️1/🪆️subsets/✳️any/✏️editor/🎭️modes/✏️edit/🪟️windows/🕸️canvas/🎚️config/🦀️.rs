//! 🎚️ Persisted local configuration for one concrete Wires canvas.

#[path = "🧬️schema/🦀️.rs"]
mod schema;
pub use schema::*;

impl store::ArtifactDsl for WiresCanvasWindowConfig {
    ...
    fn print_dsl(&self) -> String {
        ...
        let envelope = store::semio_format::SemioEnvelope::from_envelope_id(<Self as store::ArtifactDsl>::envelope_id(), store::semio_format::Component::Dsl, 1).expect("valid Wires canvas config envelope");
        store::semio_format::wrap_text(&envelope, &body)
    }
}

impl store::ArtifactPack for WiresCanvasWindowConfig {
...
impl semio_framework_plugin::WindowConfigOwner for WiresCanvasWindowConfigOwner {
    const WINDOW_KIND_ID: &'static str = super::WIRES_PLAY_WINDOW_CANVAS;
    const SCHEMA: &'static str = "reasoning.wirescanvaswindowconfig";
