...
impl protocol::MutationDiff<WiresPresence> for WiresPresence {
    async fn apply(&self, _base: &WiresPresence) -> protocol::MutationApplyResult<WiresPresence> {
        Ok({
            self.clone()
        })
    }
...
pub enum WiresPresenceMutation {
    ...
    Snapshot {
        #[dsl(block)]
        presence: WiresPresence,
    },
}

impl Mutation<WiresPresence> for WiresPresenceMutation {
    type Diff = WiresPresence;

    async fn diff(&self, _base: &WiresPresence) -> protocol::MutationOutcome<WiresPresence> {
        match self {
            Self::Snapshot { presence } => protocol::MutationOutcome::new(presence.clone()),
        }
    }

    async fn inverse(&self, base: &WiresPresence) -> Vec<Self> {
        vec![Self::Snapshot { presence: base.clone() }]
    }
}

impl protocol::OpText for WiresPresenceMutation {
...
impl protocol::OpBinary for WiresPresenceMutation {
