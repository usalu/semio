...
#[test]
fn wires_window_transient_contract_vectors_match_the_json_oracle() {
    ...
    let base: WiresCanvasTransient = pack::from_json_str(&vectors["base"].to_string()).expect("owned base decoder");
    assert_eq!(<WiresCanvasTransientMutation as Mutation<WiresCanvasTransient>>::DESCRIPTORS.len(), vectors["cases"].as_array().expect("cases").len());
    for vector in vectors["cases"].as_array().expect("cases") {
        let mutation: WiresCanvasTransientMutation = pack::from_json_str(&vector["mutation"].to_string()).expect("owned operation decoder");
        ...
        assert_eq!(WiresCanvasTransientMutation::parse_op(&mutation.print_op()).expect("operation text"), mutation);
        assert_eq!(WiresCanvasTransientMutation::decode_op(&mutation.encode_op().expect("operation binary")).expect("binary decode"), mutation);
