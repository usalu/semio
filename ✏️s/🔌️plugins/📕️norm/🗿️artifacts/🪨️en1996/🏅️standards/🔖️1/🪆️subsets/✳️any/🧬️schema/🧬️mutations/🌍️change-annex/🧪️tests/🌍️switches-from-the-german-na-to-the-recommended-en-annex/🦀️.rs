//! 🧪️ `change-annex` fixture — `🌍️switches-from-the-german-na-to-the-recommended-en-annex`.
//!
...
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌍️change-annex/🌍️switches-from-the-german-na-to-the-recommended-en-annex/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌍️change-annex/🌍️switches-from-the-german-na-to-the-recommended-en-annex/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌍️change-annex/🌍️switches-from-the-german-na-to-the-recommended-en-annex/🔺️diff/🔣️.json");
...
/// ▶️ Switching the national annex from `De` to `En` rewrites `annex` alone. γ_M swings from the DIN NA's flat
/// 1.5 to the EN table's class-dependent value, but that is COMPUTED in `💡️inferences`, so the masonry class
/// the EN table reads must ride through untouched.
...
async fn switches_from_the_german_na_to_the_recommended_en_annex() {
    ...
    assert_eq!(applied.masonry_class, before().masonry_class, "change-annex/switches-from-the-german-na-to-the-recommended-en-annex: the manufacturing-control class is the OTHER input to γ_M and must survive an annex switch untouched");
}
...
/// 🎯️ `change-annex` has only an equality guard (an enum cannot be non-finite), and `En` differs
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(produced.worst_level(), None, "change-annex/switches-from-the-german-na-to-the-recommended-en-annex: `change-annex` has no numeric-finiteness guard at all — only the equality guard — and `AnnexChoice::En` differs from the committed committed `De`, so `mutation.no-op` must not fire");
...
/// 🔣️ The committed diff decodes to `En1996Diff`, re-encodes unchanged, and carries the annex choice and nothing
/// else.
#[semio_framework_async_macros::async_test]
async fn committed_diff_is_canonical() {
    let decoded: En1996Diff = serde_json::from_str(DIFF).expect("the committed change-annex diff decodes");
    ...
    assert!(decoded.masonry_class.is_none(), "change-annex/switches-from-the-german-na-to-the-recommended-en-annex: change-annex writes annex and must leave `masonry_class` untouched");
    assert!(decoded.design_situation.is_none(), "change-annex/switches-from-the-german-na-to-the-recommended-en-annex: change-annex writes annex and must leave `design_situation` untouched");
