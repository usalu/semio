...
use crate::editor::en1996::commands::{apply_remedy, evaluate, insert_item, remove_item, selected_check, set_active_example, set_field, set_snapshot};
...
pub const CONTROLLER_ID: &str = "s.norm.en1996@1/*#editor";
...
semio_framework_plugin::app_commands! {
    ...
    pub enum En1996Command for En1996Snapshot, En1996Mutation, NoConfig, NoConfigMutation {
...
impl ArtifactEditor for En1996PlayApp {
    ...
    fn examples() -> Vec<semio_framework_plugin::ExampleSource> {
        vec![crate::loadbearing_wall::source(), crate::multi_fail_masonry::source()]
    }
    ...
    semio_framework_plugin::bounded_first_step_tool_proofs! {
        ...
        artifact_schema: "semio.norm.en1996/v1",
    ...
    semio_s_artifact_norm_contract::norm_command_from_action!(En1996Command, crate::standards::v1::subsets::any::schema::snapshot::decode_en1996_snapshot_json);
    ...
    ) -> Result<Emit<En1996Mutation, NoConfigMutation, Self::DraftMutation>, Fault> {
        crate::app_surface::dispatch_norm_command::<Self>(command, doc, cfg, view_state)
    }

    fn render(body_key: &str, doc: &ArtifactView<'_, En1996Snapshot>, cfg: &ConfigView<'_, NoConfig>, view_state: &semio_framework_plugin::ViewModel) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::ComponentTree> {
        let mut host = NormHost::<En1996Family>::from_artifact(doc.snapshot.clone());
    ...
    fn import_media(port: &str, media: &Media, doc: &ArtifactView<'_, En1996Snapshot>) -> Result<Emit<En1996Mutation, NoConfigMutation, Self::DraftMutation>, MediaError> {
        let base = doc.snapshot.clone();
        crate::app_surface::import_media(port, media, move |snapshot: En1996Snapshot| En1996Mutation::from_snapshot(&base, &snapshot))
    }
...
impl crate::app_surface::NormRetainedEditor for En1996PlayApp {
    type Family = En1996Family;

    type ResultsWindowConfigOwner = results::ResultsWindowConfigOwner;

    fn selected_check_window_mutation(command: &En1996Command) -> Option<crate::results_window_config::NormResultsWindowConfigMutation> {
        match command {
            En1996Command::SetSelectedCheckIndex(payload) => Some(selected_check::window_mutation(payload)),
            _ => None,
        }
    }

    fn dispatch_retained(command: &En1996Command, doc: &ArtifactView<'_, En1996Snapshot>, cfg: &ConfigView<'_, NoConfig>) -> Result<Emit<En1996Mutation, NoConfigMutation, NoDraftMutation>, Fault> {
...
pub fn create_en1996_app() -> semio_framework_plugin::AppDefinition {
    Editor::builder(crate::EN1996_DIALECT)
            ...
            .action_with(
                semio_framework_plugin::ActionDefinition::new_catalog("setSnapshot", LocalizedLabel::native("Set En1996Snapshot", "Dokument setzen"), semio_framework_plugin::ActionKind::Mutation)
            ...
            .action_describe("setSnapshot", LocalizedLabel::native("Replaces the whole EN 1996 (Eurocode 6, masonry structures) compliance document with the supplied document JSON; the previous inputs are discarded.", "Ersetzt das gesamte Nachweisdokument nach EN 1996 (Eurocode 6, Mauerwerksbau) durch das übergebene Dokument-JSON; die bisherigen Eingaben werden verworfen."))
            .action_describe("evaluate", LocalizedLabel::native("Recomputes every EN 1996 (Eurocode 6, masonry structures) check from the document's inputs and refreshes the results window; the document is not changed.", "Berechnet alle Nachweise nach EN 1996 (Eurocode 6, Mauerwerksbau) aus den Eingaben des Dokuments neu und aktualisiert das Ergebnisfenster; das Dokument ändert sich nicht."))
            ...
            .action_describe("setActiveExample", LocalizedLabel::native("Loads one of the bundled EN 1996 (Eurocode 6, masonry structures) examples into the open compliance document, replacing its inputs, by example id.", "Lädt eines der mitgelieferten Beispiele nach EN 1996 (Eurocode 6, Mauerwerksbau) in das offene Nachweisdokument und ersetzt dessen Eingaben, anhand der Beispiel-Id."))
            .build_definition()
}
