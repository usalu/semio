...
pub(crate) mod testkit {
    ...
    pub async fn app_with_registry() -> NormApp {
        new_app_with_registry::<EditorApp<En1996PlayApp>>(en1996_manifest_for_testkit).await
    }
...
mod tests {
    ...
    async fn an_unknown_body_key_falls_back_to_a_text_node() {
        ...
        assert!(testkit::render(&mut app, "norm.en1996.play.nope").await.contains("Unknown body"));
    }
    ...
    async fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::app_with_registry().await;
        testkit::dispatch(&mut app, En1996Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1996Snapshot::default() })).await;
    ...
    async fn evaluate_recommits_the_current_projection_without_changing_it() {
        ...
        testkit::dispatch(&mut app, En1996Command::Evaluate(evaluate::Evaluate {})).await;
    ...
    async fn selected_check_index_is_a_config_only_edit() {
        let mut app = testkit::app_with_registry().await;
        let before = app.snapshot().expect("projection");
        let result = testkit::dispatch(&mut app, En1996Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) })).await;
    ...
    async fn view_actions_never_emit_artifact_mutations_under_the_real_registry() {
        let mut app = testkit::app_with_registry().await;
        let result = testkit::dispatch(&mut app, En1996Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(1) })).await;
