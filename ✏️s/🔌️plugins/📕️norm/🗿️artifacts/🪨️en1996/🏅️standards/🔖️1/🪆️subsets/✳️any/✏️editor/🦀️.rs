//! ð§¿ï¸ EN 1996 play app â the `ArtifactApp` impl (dispatch-only), the aggregated command enum and
//! the manifest stitch.
//!
...
use crate::editor::en1996::commands::{evaluate, selected_check, set_snapshot};
...
pub const LABEL: &str = "EN 1996";
/// ðï¸ The playground/registry variant key â every body key, window id and schema is derived from it.
pub const VARIANT: &str = "en1996";
pub const DOCUMENT_SCHEMA: &str = "semio.norm.en1996/v1";
pub const CONFIG_SCHEMA: &str = "config.norm.en1996";
//#endregion ðï¸Constants

//#region ðï¸Commands
semio_framework_plugin::app_commands! {
    /// ð¯ï¸ `En1996PlayApp::Command` â the SOLE dispatch surface for this app's own behavior, covering every
    /// action `create_en1996_app` declares. Row order IS the binary variant ordinal (appending is safe,
    ...
    pub enum En1996Command for En1996Snapshot, En1996Mutation, NormConfig, NormConfigMutation {
...
//#region ðï¸En1996PlayApp
#[derive(Default)]
pub struct En1996PlayApp;


impl ArtifactEditor for En1996PlayApp {
    ...
    type Command = En1996Command;

    const DIALECT: Dialect = crate::artifacts::en1996::EN1996_DIALECT;
    const DOCUMENT_SCHEMA: &'static str = "semio.norm.en1996/v1";
    ...
    fn register_tool_job_factories(registry: &mut semio_framework_plugin::ArtifactToolFactoryRegistry<'_, semio_framework_plugin::EditorApp<Self>>) -> Result<(), Fault> {
        En1996BoundedCommandJobFactory::register(registry)
    }
    ...
    semio_framework_plugin::bounded_first_step_tool_proofs! {
        owner: semio_framework_plugin::EditorApp<En1996PlayApp>,
        owner_file: "✏️s/🔌️plugins/📕️norm/🗿️artifacts/🪨️en1996/🏅️standards/🔖️1/🪆️subsets/✳️any/✏️editor/🦀️.rs",
        controller: "s.norm.en1996@1/*#editor",
        document_schema: "semio.norm.en1996/v1",
        factory: "En1996BoundedCommandJobFactory",
        factory_type: En1996BoundedCommandJobFactory,
    ...
    fn initial_snapshot() -> En1996Snapshot {
    ...
    fn command_id(command: &En1996Command) -> &'static str {
        command.command_id()
    }

    fn handle(
        command: &En1996Command,
        doc: &ArtifactView<'_, En1996Snapshot>,
        cfg: &ConfigView<'_, NormConfig>,
        _interaction: &InteractionView<'_>,
        _draft: &DraftView<'_, Self::Draft>,
        _engines: &EngineHandles,
    ) -> Result<Emit<En1996Mutation, NormConfigMutation, Self::DraftMutation>, Fault> {
    ...
    fn export_media(port: &str, doc: &ArtifactView<'_, En1996Snapshot>) -> Result<Media, MediaError> {
        crate::app_surface::export_media::<En1996Family>(port, VARIANT, DOCUMENT_SCHEMA, doc.snapshot)
    }

    /// ðï¸ `"model:in"`/`"document:in"` â see `crate::app_surface::import_media`.
    fn import_media(port: &str, media: &Media, _doc: &ArtifactView<'_, En1996Snapshot>) -> Result<Emit<En1996Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
        crate::app_surface::import_media(port, media, |snapshot: En1996Snapshot| En1996Mutation::from_snapshot(&snapshot))
    }
    //#endregion ðï¸MediaPorts
}
//#endregion ðï¸En1996PlayApp

//#region 🧵️RetainedCommands
crate::norm_owned_tool_job_factory!(En1996BoundedCommandJobFactory, En1996PlayApp);

impl crate::app_surface::NormRetainedEditor for En1996PlayApp {
    fn dispatch_retained(command: &En1996Command, doc: &ArtifactView<'_, En1996Snapshot>, cfg: &ConfigView<'_, NormConfig>) -> Result<Emit<En1996Mutation, NormConfigMutation, NoDraftMutation>, Fault> {
        command.dispatch(doc, cfg)
    }
}
...
impl crate::document::NormFamily for En1996Family {
    ...
    fn evaluate(document: &En1996Snapshot) -> crate::document::CheckReport {
        crate::artifacts::en1996::standards::v1::subsets::any::schema::inferences::evaluate(document)
    }
}
...
//#region ðï¸Manifest
pub fn create_en1996_app() -> semio_framework_plugin::AppDefinition {
    Editor::builder(crate::artifacts::en1996::EN1996_DIALECT)
            .document(["semio", "norm", VARIANT])
            .artifact_kind(crate::artifacts::en1996::artifact_kind())
            ...
            .mutation("setSnapshot", LocalizedLabel::native("Set En1996Snapshot", "Dokument setzen"))
...
pub(crate) mod testkit {
    ...
    /// ✏️ Adapts `create_en1996_app`'s `AppDefinition` (contract §2.4) into the `App { definition,
    ...
    pub fn en1996_manifest_for_testkit() -> semio_framework_plugin::App {
        semio_framework_plugin::App { definition: create_en1996_app(), examples: Vec::new() }
    }

    pub type NormApp = VcsArtifactApp<EditorApp<En1996PlayApp>>;
    ...
    pub fn app_with_registry() -> NormApp {
        new_app_with_registry::<EditorApp<En1996PlayApp>>(en1996_manifest_for_testkit)
    }

    pub fn dispatch(app: &mut NormApp, command: En1996Command) -> InvocationResult {
...
mod tests {
    ...
    /// ð¯ï¸ One value per `En1996Command` row â the whole-command-surface laws below iterate it, so a new row
    /// that is not listed here fails `command_ids_cover_every_row`.
    fn every_command() -> Vec<En1996Command> {
        vec![
            En1996Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1996Snapshot::default() }),
            En1996Command::Evaluate(evaluate::Evaluate {}),
            En1996Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) }),
        ]
    }
    ...
    fn command_ids_cover_every_row_and_are_unique() {
        ...
        let ids: Vec<&str> = commands.iter().map(En1996Command::command_id).collect();
    ...
    /// of the three payload shapes involves the per-standard `En1996Snapshot`.
    #[semio_framework_async_macros::async_test]
    fn optional_field_rows_keep_their_pre_migration_bytes() {
        let hex = |command: &En1996Command| protocol::OpBinary::encode_op(command).expect("encode").iter().map(|byte| format!("{byte:02x}")).collect::<String>();
        assert_eq!(hex(&En1996Command::Evaluate(evaluate::Evaluate {})), "01010000");
        assert_eq!(hex(&En1996Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) })), "01020001000402");
        assert_eq!(hex(&En1996Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: None })), "01020000");
    }
    ...
    fn declares_model_in_and_report_out_ports() {
        let ports = create_en1996_app().io.ports;
    ...
    fn an_unknown_body_key_falls_back_to_a_text_node() {
        ...
        assert!(testkit::render(&mut app, "norm.en1996.play.nope").contains("Unknown body"));
    }
    ...
    fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::app_with_registry();
        testkit::dispatch(&mut app, En1996Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1996Snapshot::default() }));
        let host = NormHost::<En1996Family>::from_document(app.snapshot().expect("projection"));
    ...
    fn evaluate_recommits_the_current_projection_without_changing_it() {
        ...
        testkit::dispatch(&mut app, En1996Command::Evaluate(evaluate::Evaluate {}));
    ...
    fn selected_check_index_is_a_config_only_edit() {
        let mut app = testkit::app_with_registry();
        let before = app.snapshot().expect("projection");
        let result = testkit::dispatch(&mut app, En1996Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) }));
    ...
    fn view_actions_never_emit_artifact_mutations_under_the_real_registry() {
        let mut app = testkit::app_with_registry();
        let result = testkit::dispatch(&mut app, En1996Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(1) }));
