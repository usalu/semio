...
fn norm_results_window_ownership_runtime_isolates_focused_inspection_and_reopens() {
    std::thread::Builder::new()
        ...
        .spawn(|| block_on_norm_results_windows(async {
            ...
            async fn dispatch(app: &mut NormApp, index: Option<u32>, view: &ViewModel) -> Result<semio_framework_plugin::artifact_app_laws::TypedOperationFixtureReceipt, String> {
                let meta = ActionMeta { instance_id: 204, view_state: Some(view.clone()), ..artifact_app_laws::meta("norm-results-window-ownership") };
            ...
            let mut app = Box::new(artifact_app_laws::new_app_with_registry::<EditorApp<En1996PlayApp>>(manifest).await);
            ...
            let outcome: Result<(), String> = async {
                ...
                let left_receipt = dispatch(&mut app, Some(1), &left_panel).await?;
                ...
                let right_receipt = dispatch(&mut app, Some(2), &right).await?;
                ...
                let mut reopened = Box::new(artifact_app_laws::new_app_with_registry::<EditorApp<En1996PlayApp>>(manifest).await);
                ...
                dispatch(&mut reopened, Some(3), &left_panel).await?;
