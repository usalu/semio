//! 🧪️ `🗑️retire-product-index` fixture — `🗑️removes-the-pr600-index`.

use crate::{Iso16757Diff, Iso16757Mutation, Iso16757Snapshot};

const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️retire-product-index/🗑️removes-the-pr600-index/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️retire-product-index/🗑️removes-the-pr600-index/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️retire-product-index/🗑️removes-the-pr600-index/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️retire-product-index/🗑️removes-the-pr600-index/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️retire-product-index/🗑️removes-the-pr600-index/🎯️outcome/🔣️.json");
...
async fn removes_the_pr600_index() {
    let applied = protocol::MutationDiff::apply(built_outcome().diff(), &before()).expect("apply");
    assert_eq!(applied, expected_after());
    assert_eq!(applied.catalogue.product_indexes.len(), before().catalogue.product_indexes.len().saturating_sub(1));
    assert!(applied.catalogue.product_indexes.iter().all(|x| x.id != "index-pr600"));
}
