//! 🧪️ `📚introduce-product-series` fixture — `📚appends-a-pr-plus-series`.

use crate::{Iso16757Diff, Iso16757Mutation, Iso16757Snapshot};

const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📚introduce-product-series/📚appends-a-pr-plus-series/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📚introduce-product-series/📚appends-a-pr-plus-series/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📚introduce-product-series/📚appends-a-pr-plus-series/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📚introduce-product-series/📚appends-a-pr-plus-series/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📚introduce-product-series/📚appends-a-pr-plus-series/🎯️outcome/🔣️.json");
...
async fn appends_a_pr_plus_series() {
    let applied = protocol::MutationDiff::apply(built_outcome().diff(), &before()).expect("apply");
    assert_eq!(applied, expected_after());
    assert_eq!(applied.catalogue.product_series.len(), before().catalogue.product_series.len() + 1);
    assert!(applied.catalogue.product_series.iter().any(|x| x.id == "series-pr-plus"));
}
