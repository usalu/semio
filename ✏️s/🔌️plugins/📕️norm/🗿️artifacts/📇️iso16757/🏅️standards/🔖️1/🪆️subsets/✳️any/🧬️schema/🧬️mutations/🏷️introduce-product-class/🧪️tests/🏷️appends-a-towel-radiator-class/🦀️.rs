//! 🧪️ `🏷️introduce-product-class` fixture — `🏷️appends-a-towel-radiator-class`.

use crate::{Iso16757Diff, Iso16757Mutation, Iso16757Snapshot};

const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏷️introduce-product-class/🏷️appends-a-towel-radiator-class/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏷️introduce-product-class/🏷️appends-a-towel-radiator-class/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏷️introduce-product-class/🏷️appends-a-towel-radiator-class/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏷️introduce-product-class/🏷️appends-a-towel-radiator-class/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏷️introduce-product-class/🏷️appends-a-towel-radiator-class/🎯️outcome/🔣️.json");
...
async fn appends_a_towel_radiator_class() {
    let applied = protocol::MutationDiff::apply(built_outcome().diff(), &before()).expect("apply");
    assert_eq!(applied, expected_after());
    assert_eq!(applied.catalogue.product_classes.len(), before().catalogue.product_classes.len() + 1);
    assert!(applied.catalogue.product_classes.iter().any(|x| x.id == "class-towel-radiator"));
}
