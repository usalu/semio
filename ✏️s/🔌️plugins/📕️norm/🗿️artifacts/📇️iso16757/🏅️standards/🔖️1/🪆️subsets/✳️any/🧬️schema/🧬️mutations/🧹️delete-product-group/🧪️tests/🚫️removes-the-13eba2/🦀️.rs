//! 🧪️ `delete-product-group` fixture — `🚫️removes-the-radiators-group-and-strands-its-class`.
//!
...
async fn removes_the_radiators_group_and_strands_its_class() {
...
#[semio_framework_async_macros::async_test]
async fn recreating_the_radiators_group_restores_before() {
    let base = before();
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-product-group/removes-the-radiators-group-and-strands-its-class: the produced diff differs from the committed 🔺️diff/🔣️.json");
}
