//! 🧪️ `🗑️retire-product-class` fixture — `🗑️removes-the-panel-radiator-class`.

use crate::{Iso16757Diff, Iso16757Mutation, Iso16757Snapshot};

const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️retire-product-class/🗑️removes-the-panel-radiator-class/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️retire-product-class/🗑️removes-the-panel-radiator-class/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️retire-product-class/🗑️removes-the-panel-radiator-class/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️retire-product-class/🗑️removes-the-panel-radiator-class/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️retire-product-class/🗑️removes-the-panel-radiator-class/🎯️outcome/🔣️.json");
...
async fn removes_the_panel_radiator_class() {
    let applied = protocol::MutationDiff::apply(built_outcome().diff(), &before()).expect("apply");
    assert_eq!(applied, expected_after());
    assert_eq!(applied.catalogue.product_classes.len(), before().catalogue.product_classes.len().saturating_sub(1));
    assert!(applied.catalogue.product_classes.iter().all(|x| x.id != "class.panel-radiator"));
}
