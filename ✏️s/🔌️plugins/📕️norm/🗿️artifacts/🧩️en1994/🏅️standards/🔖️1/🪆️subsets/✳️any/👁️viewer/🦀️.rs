//! 👁️ EN 1994 viewer — the read-only counterpart of `✏️editor` for this subset (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `En1994Viewer` implements
//! `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<En1994Viewer>` (framework SDK) is
//! the sole runtime adapter, so this file can never structurally emit an artifact or draft mutation.
//! MUST NOT import anything from the sibling editor module (`policyViewerPurityBreaches`).

use crate::artifacts::en1994::En1994Snapshot;
use crate::artifacts::en1994::{EN1994_DIALECT, EN1994_DOCUMENT_SCHEMA};
use crate::viewer::en1994::modes::view;
...
pub struct En1994Viewer;

impl ArtifactViewer for En1994Viewer {
    type Snapshot = En1994Snapshot;
    type Mutation = crate::artifacts::en1994::op::En1994Mutation;
    ...
    type Command = En1994ViewCommand;
...
mod tests {
    ...
    async fn an_unknown_body_key_falls_back_to_a_text_node() {
        ...
        let json = semio_framework_plugin::testkit::project_and_retire_fixture_tree(<En1994Viewer as ArtifactViewer>::render("nope", &doc, &ConfigView { snapshot: &NoConfig::default() }).expect("viewer assembly")).expect("json");
