...
async fn an_unknown_body_key_falls_back_to_a_text_node() {
    ...
    let json = semio_framework_plugin::artifact_app_laws::project_and_retire_fixture_tree(
        <En1994Viewer as ArtifactViewer>::render("nope", &doc, &ConfigView { snapshot: &NoConfig::default(), window: None }, &semio_framework_plugin::ViewModel::default()).expect("viewer assembly"),
    )
    .expect("json");
