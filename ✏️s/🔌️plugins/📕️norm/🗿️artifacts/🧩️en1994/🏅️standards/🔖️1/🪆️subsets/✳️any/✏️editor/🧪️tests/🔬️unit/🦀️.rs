pub(crate) mod context {
    ...
    pub async fn app_with_registry() -> NormApp {
        let mut app = new_app_with_registry::<EditorApp<En1994PlayApp>>(en1994_manifest_for_tests).await;
...
fn retained_command_dispositions_match_the_language_neutral_oracle() {
    ...
    let definition = create_en1994_app();
...
fn every_command() -> Vec<En1994Command> {
    vec![
        ...
        En1994Command::SetActiveExample(set_active_example::SetActiveExample { example_id: String::new() }),
    ]
}
...
async fn declares_model_in_and_report_out_ports() {
    let ports = create_en1994_app().io.ports;
