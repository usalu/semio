...
def action_internals(action, span_m, support, spacing_m):
    ...
    m_k = float(action.get("mKNm") or 0)
    v_k = float(action.get("vKN") or 0)
    n_k = float(action.get("nKN") or 0)
    if abs(q) > 1e-12:
...
def compare_report(snapshot, report):
    ...
    for beam in beams:
        ...
        # eta itself needs n_req — skip absolute eta, compare eta_min presence via etamin check limit side
        cid = f"en1994.6.6.1.2.etamin.{bid}"
        if cid in checks:
            # compare limit-side eta_min encoded as utilization = eta_min/eta inverted — use computed/limit from report
            # Instead recompute eta roughly using beff concrete force
            compared += 1  # counted via presence; detailed below
            ru = float(checks[cid]["utilization"])
            # report utilization for minimum() is limit/computed style — skip tight compare if shape differs
            _ = ru
