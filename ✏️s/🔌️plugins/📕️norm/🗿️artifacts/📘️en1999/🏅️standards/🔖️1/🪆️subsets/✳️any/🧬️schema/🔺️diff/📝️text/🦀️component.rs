...
//#region 🔖️Apply
impl En1999Diff {
    pub fn apply_to_artifact(&self, artifact: &En1999Artifact) -> En1999Artifact {
...
impl MutationDiff<En1999Snapshot> for En1999Diff {
    fn apply(&self, snapshot: &En1999Snapshot) -> En1999Snapshot {
        if let Some(replacement) = &self.artifact {
...
pub fn diff_set_snapshot(snapshot: &En1999Snapshot) -> En1999Diff {
    En1999Diff {
        artifact: Some(Box::new(En1999Artifact::from_snapshot(snapshot.clone()))),
        ..Default::default()
    }
}
//#endregion 🔖️Helpers
...
mod tests {
    ...
    fn set_snapshot_diff_replaces_the_whole_snapshot() {
        ...
        let mutation = En1999Mutation::SetSnapshot { snapshot: En1999Snapshot::default() };
        let diff = mutation.diff(&base);
        assert_eq!(diff.apply(&base), En1999Snapshot::default());
    }
}
//#endregion 🧪️Tests
