...
async fn committed_diff_is_canonical() {
    ...
    assert!(produced.as_object().is_some_and(|fields| fields.values().all(serde_json::Value::is_null)), "change-variable-action-value/refuses-to-revalue-a-missing-action-0: the default diff must serialize as an all-null object, never as an omitted-field one");
}
