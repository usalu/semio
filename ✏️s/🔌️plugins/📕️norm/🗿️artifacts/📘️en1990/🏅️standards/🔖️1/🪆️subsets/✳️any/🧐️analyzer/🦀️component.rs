//! 🧐️ En1990Analyzer (1/✳️any) — read-only analysis, successor to the pre-migration
//! En1990Decomposer. Real logic; artifact/standard levels delegate here.
...
impl ArtifactAnalyzer for En1990Analyzer {
    ...
    fn analyze(sources: &[AnalyzeSource<'_>]) -> Analysis<Self::Parts> {
        ...
        for source in sources {
            match source {
                AnalyzeSource::Text(text) => match <En1990Snapshot as store::DocumentDsl>::parse_dsl(text) {
                ...
                AnalyzeSource::Binary(bytes) => match <En1990Snapshot as store::DocumentPack>::decode_pack(bytes) {
                    Ok(snapshot) => parts.snapshot = Some(snapshot),
                    Err(err) => {
                        confidence = IoConfidence::Low;
