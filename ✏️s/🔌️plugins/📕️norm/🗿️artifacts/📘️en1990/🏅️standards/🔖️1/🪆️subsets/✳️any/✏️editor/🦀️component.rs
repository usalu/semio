...
use semio_framework_plugin::{
    AppIo, AppOperationContext, ArtifactEditor, ArtifactOwnedToolJobRequest, ArtifactToolFactoryRegistry, ArtifactView, ConfigView, DraftView, Editor, EditorApp, Emit, Fault, InteractiveJobClassification, LocalizedLabel, Media, MediaError, NoDraft,
...
const NORM_BOUNDED_PAYLOAD_SCHEMA: &str = "semio.norm.en1990/v1.tool-command.v1";
...
fn norm_bounded_extent(command: &En1990Command, _snapshot: &En1990Snapshot, _interaction: &protocol::InteractionState) -> Option<usize> {
...
) -> Result<Emit<En1990Mutation, NormConfigMutation, NoDraftMutation>, Fault> {
...
impl semio_framework::ToolJobFactory for BoundedFirstStepCommandJobFactory {
    type Payload = ArtifactRetainedCommandPayload<EditorApp<En1990PlayApp>>;
    type Job = ArtifactRetainedCommandJob<EditorApp<En1990PlayApp>>;
...
impl semio_framework_plugin::ArtifactOwnedToolJobFactory for BoundedFirstStepCommandJobFactory {
    type Owner = EditorApp<En1990PlayApp>;
...
impl ArtifactEditor for En1990PlayApp {
    ...
    semio_framework_plugin::bounded_first_step_tool_proofs! {
        ...
        owner_file: "✏️s/🔌️plugins/📕️norm/🗿️artifacts/📘️en1990/🏅️standards/🔖️1/🪆️subsets/✳️any/✏️editor/🦀️component.rs",
...
mod tests {
    ...
    #[test]
    fn retained_view_command_contract_matches_the_language_neutral_cohort_fixture() {
        let fixture: serde_json::Value = serde_json::from_str(include_str!("../../../../../../../🧪️fixtures/🎯️bounded-view-command-limits.json")).expect("retained command fixture");
        let contract = norm_bounded_contract();
        assert_eq!(fixture["contract"]["maximumRawBytes"], contract.max_raw_wire_bytes);
        assert_eq!(fixture["contract"]["maximumDecodedItems"], contract.max_decoded_items);
        assert_eq!(fixture["contract"]["maximumWorkItemsPerStep"], contract.max_work_units_per_step);
        assert_eq!(fixture["contract"]["maximumOutputBytes"], contract.max_output_bytes);
        assert_eq!(fixture["contract"]["maximumStepMicros"], contract.max_step_micros);
        assert_eq!(fixture["apps"].as_array().map(Vec::len), Some(15));
        assert_eq!(fixture["tools"], serde_json::json!(NORM_BOUNDED_TOOL_IDS));
        for case in fixture["wireCases"].as_array().expect("wire cases") {
            let bytes = case["bytes"].as_u64().expect("wire bytes") as usize;
            let has_checkpoint = case["checkpoint"].as_bool().unwrap_or(false);
            assert_eq!(case["accepted"].as_bool(), Some(norm_bounded_wire_admitted(bytes, has_checkpoint)), "{}", case["name"]);
        }
        let factory = BoundedFirstStepCommandJobFactory::new("s.norm.en1990@1/*#editor");
        assert_eq!(semio_framework::ToolJobFactory::payload_schema_id(&factory), "semio.norm.en1990/v1.tool-command.v1");
        assert_eq!(semio_framework::ToolJobFactory::keys(&factory).len(), NORM_BOUNDED_TOOL_IDS.len());
    }
