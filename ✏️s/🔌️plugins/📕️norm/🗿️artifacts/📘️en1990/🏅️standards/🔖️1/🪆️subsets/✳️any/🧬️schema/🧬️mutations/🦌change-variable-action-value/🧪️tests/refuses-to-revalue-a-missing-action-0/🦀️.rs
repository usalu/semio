//! 🧪️ `change-variable-action-value` fixture — `refuses-to-revalue-a-missing-action-0`.
//!
//! `change-variable-action-value` runs its finiteness invariant FIRST (18.75 kN is finite, so that guard passes), then looks the entry up by index. Against the empty list an unseeded working-scene cache reads back, the lookup misses and the builder answers `mutation.target-missing` — an `error`, not the `fatal` its invariant guard would have raised.
//!
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "change-variable-action-value/refuses-to-revalue-a-missing-action-0: applied state differs from committed after-snapshot");
    assert_eq!(expected_after(), before(), "change-variable-action-value/refuses-to-revalue-a-missing-action-0: a rejected case's after-snapshot must be its before-snapshot verbatim");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "change-variable-action-value/refuses-to-revalue-a-missing-action-0: replaying the inverse of a rejection must still leave the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-variable-action-value/refuses-to-revalue-a-missing-action-0: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "change-variable-action-value/refuses-to-revalue-a-missing-action-0: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "change-variable-action-value/refuses-to-revalue-a-missing-action-0: raised diagnostics differ from the committed 🎯️outcome messages");
    assert!(produced.iter().any(|(_, raised_code)| raised_code == code), "change-variable-action-value/refuses-to-revalue-a-missing-action-0: the outcome's declared code must be one the builder actually raised");
    assert_eq!(applied(), before(), "change-variable-action-value/refuses-to-revalue-a-missing-action-0: a rejected mutation must leave the snapshot untouched");
}
...
fn produces_committed_diff() {
    ...
    assert!(DIFF_ABSENT.is_empty(), "change-variable-action-value/refuses-to-revalue-a-missing-action-0: 🔺️diff/🚫️.absent must be a zero-byte marker");
}
...
fn committed_diff_is_canonical() {
    ...
    assert!(
        ...
        "change-variable-action-value/refuses-to-revalue-a-missing-action-0: the default diff must serialize as an all-null object, never as an omitted-field one"
    );
}
