...
/// order and spelling — the list `../../🔣️oracle.json` publishes as the `en1990-1-any`
/// mutation catalog and `../../../../../🧪️tests/mutate-en1990-1` registers its scenarios from. The
...
impl En1990Mutation {
    ...
    pub fn from_snapshot(base: &En1990Snapshot, target: &En1990Snapshot) -> Vec<En1990Mutation> {
        ...
        mutations.push(En1990Mutation::ChangeAnnex(set_snapshot::ChangeAnnex { new_annex: target.annex.clone() }));
...
mod tests {
    ...
    fn every_mutation() -> Vec<En1990Mutation> {
        vec![
            En1990Mutation::ChangeAnnex(set_snapshot::ChangeAnnex { new_annex: AnnexChoice::En }),
    ...
    fn change_annex_satisfies_the_inverse_and_absorb_laws() {
        let base = En1990Snapshot::default();
        let mutation = En1990Mutation::ChangeAnnex(set_snapshot::ChangeAnnex { new_annex: AnnexChoice::En });
...
#[path = "."]
mod fixture_tests {
    #[path = "🐷set-snapshot/🧪️tests/switches-the-national-annex-from-de-to-en/🦀️.rs"]
    mod tests_change_annex_switches_the_national_annex_from_de_to_en;
    #[path = "🐑change-consequence-class/🧪️tests/escalates-the-building-from-cc2-to-cc3/🦀️.rs"]
    mod tests_change_consequence_class_escalates_the_building_from_cc2_to_cc3;
    #[path = "🐐change-permanent-action/🧪️tests/raises-the-permanent-action-to-62-5-kn/🦀️.rs"]
    mod tests_change_permanent_action_raises_the_permanent_action_to_62_5_kn;
    #[path = "🐘change-resistance/🧪️tests/raises-the-design-resistance-to-320-kn/🦀️.rs"]
    mod tests_change_resistance_raises_the_design_resistance_to_320_kn;
    #[path = "🦄change-seismic-action/🧪️tests/enables-the-seismic-situation-with-an-85-kn-a-ed/🦀️.rs"]
    mod tests_change_seismic_action_enables_the_seismic_situation_with_an_85_kn_a_ed;
    #[path = "🐮change-variable-action-category/🧪️tests/refuses-to-recategorise-a-missing-action-0/🦀️.rs"]
    mod tests_change_variable_action_category_refuses_to_recategorise_a_missing_action_0;
    #[path = "🦌change-variable-action-value/🧪️tests/refuses-to-revalue-a-missing-action-0/🦀️.rs"]
    mod tests_change_variable_action_value_refuses_to_revalue_a_missing_action_0;
    #[path = "🐴insert-variable-action/🧪️tests/seeds-the-first-variable-action-q-snow-at-12-5-kn/🦀️.rs"]
    mod tests_insert_variable_action_seeds_the_first_variable_action_q_snow_at_12_5_kn;
    #[path = "🐎remove-variable-action/🧪️tests/refuses-to-remove-action-0-from-an-unseeded-child-slot/🦀️.rs"]
    mod tests_remove_variable_action_refuses_to_remove_action_0_from_an_unseeded_child_slot;
    #[path = "🐗reorder-variable-actions/🧪️tests/refuses-to-move-action-0-to-slot-1-in-an-empty-list/🦀️.rs"]
...
/// `../../../../../🧪️tests/mutate-en1990-1` links only this crate, so `serde_json` is unreachable
...
/// `mutate-en1990-1`'s `inverse-<kind>` scenarios assert, exposed under a name the test adapter can
...
mod kinds_catalog {
    ...
    /// committed `en1990-1-any` catalog. The framework never parses Rust, so this is the only thing
    /// standing between a renamed variant and a completeness gate that silently measures the wrong
    /// set.
    #[test]
    fn kinds_match_the_enum_and_the_catalog() {
        let descriptors = <En1990Mutation as protocol::SemanticMutation<En1990Snapshot>>::kinds();
        assert_eq!(KINDS.len(), descriptors.len(), "KINDS must name exactly one entry per declared En1990Mutation variant");
