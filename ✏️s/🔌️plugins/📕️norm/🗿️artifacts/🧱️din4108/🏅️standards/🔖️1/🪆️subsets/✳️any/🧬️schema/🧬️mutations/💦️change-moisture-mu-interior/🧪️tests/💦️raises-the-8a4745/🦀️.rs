//! 🧪️ `change-moisture-mu-interior` fixture — `💦️raises-the-interior-mu-value-to-2-point-5`.
//!
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(
        snapshot.moisture_mu_exterior,
        before().moisture_mu_exterior,
        "change-moisture-mu-interior/raises-the-interior-mu-value-to-2-point-5: moistureMuExterior must stay exactly as the before-snapshot had it — change-moisture-mu-interior owns moistureMuInterior and nothing else"
    );
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-moisture-mu-interior/raises-the-interior-mu-value-to-2-point-5: produced diff differs from the committed 🔺️diff/🔣️.json");
    ...
    assert!(
        raised.diff().moisture_mu_exterior.is_none(),
        "change-moisture-mu-interior/raises-the-interior-mu-value-to-2-point-5: the sparse delta must leave moistureMuExterior unset — a delta that rewrote it would be a bug this assertion exists to catch"
    );
}
...
async fn committed_diff_is_canonical() {
    ...
    assert!(
        ...
        "change-moisture-mu-interior/raises-the-interior-mu-value-to-2-point-5: change-moisture-mu-interior is an artifact-lane edit and must never carry the presence-lane selectedCheckIndex"
    );
