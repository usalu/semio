//! 🔺️ `change-zone-night-ventilation` diff — whole-list rewrite via Din4108Diff list wrappers.
...
fn apply_in_place(payload: &ChangeZoneNightVentilation, snap: &mut Din4108Snapshot) -> Result<(), String> {
    
    let z = snap.zones.iter_mut().find(|z| z.id == payload.zone_id).ok_or("zone not found")?;
    z.night_ventilation = payload.new_night_ventilation.clone();
