//! 🔺️ `change-element-adjacent` diff — whole-list rewrite via Din4108Diff list wrappers.
...
fn apply_in_place(payload: &ChangeElementAdjacent, snap: &mut Din4108Snapshot) -> Result<(), String> {
    
    let e = snap.elements.iter_mut().find(|e| e.id == payload.element_id).ok_or("element not found")?;
    e.adjacent = payload.new_adjacent.clone();
