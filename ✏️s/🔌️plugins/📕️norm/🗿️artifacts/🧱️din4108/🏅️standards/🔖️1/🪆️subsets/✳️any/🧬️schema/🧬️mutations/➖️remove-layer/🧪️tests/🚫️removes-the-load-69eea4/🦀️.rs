//! 🧪️ `remove-layer` fixture — `🚫️removes-the-load-bearing-masonry-layer`.
//!
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "remove-layer/removes-the-load-bearing-masonry-layer: produced diff differs from the committed 🔺️diff/🔣️.json");
...
async fn committed_diff_is_canonical() {
    ...
    assert!(
        ...
        "remove-layer/removes-the-load-bearing-masonry-layer: remove-layer is an artifact-lane edit and must never carry the presence-lane selectedCheckIndex"
    );
