...
fn apply_in_place(payload: &RemoveZoneWindow, snap: &mut Din4108Snapshot) -> Result<(), String> {

    let z = snap.zones.iter_mut().find(|z| z.id == payload.zone_id).ok_or("zone not found")?;
    if payload.index >= z.windows.len() { return Err("window index out of range".into()); }
    z.windows.remove(payload.index);
