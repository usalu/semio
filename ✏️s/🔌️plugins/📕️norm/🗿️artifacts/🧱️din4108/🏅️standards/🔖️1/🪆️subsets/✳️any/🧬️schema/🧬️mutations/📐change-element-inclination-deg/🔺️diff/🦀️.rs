//! Diff for `change-element-inclination-deg`.
...
pub fn diff(payload: &ChangeElementInclinationDeg, base: &Din4108Snapshot) -> protocol::MutationOutcome<Din4108Diff> {
...
fn apply_in_place(payload: &ChangeElementInclinationDeg, snap: &mut Din4108Snapshot) -> Result<(), String> {
    let e = snap.elements.iter_mut().find(|e| e.id == payload.element_id).ok_or("element not found")?;
    e.inclination_deg = payload.new_inclination_deg;
    Ok(())
}
