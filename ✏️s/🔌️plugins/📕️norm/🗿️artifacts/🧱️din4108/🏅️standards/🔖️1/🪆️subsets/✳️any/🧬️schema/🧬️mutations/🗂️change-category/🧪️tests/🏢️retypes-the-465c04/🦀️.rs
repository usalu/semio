...
async fn committed_diff_is_canonical() {
    ...
    assert!(serde_json::to_value(&decoded).expect("diff JSON").get("selectedCheckIndex").is_none(), "change-category/retypes-the-assembly-as-office: change-category is an artifact-lane edit and must never carry the presence-lane selectedCheckIndex");
