//! Diff for `change-element-delta-ur`.
...
pub fn diff(payload: &ChangeElementDeltaUr, base: &Din4108Snapshot) -> protocol::MutationOutcome<Din4108Diff> {
...
fn apply_in_place(payload: &ChangeElementDeltaUr, snap: &mut Din4108Snapshot) -> Result<(), String> {
    let e = snap.elements.iter_mut().find(|e| e.id == payload.element_id).ok_or("element not found")?;
    e.delta_u_r = payload.new_delta_u_r;
    Ok(())
}
