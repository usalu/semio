//! 🔺️ `insert-zone` diff — whole-list rewrite via Din4108Diff list wrappers.
...
fn apply_in_place(payload: &InsertZone, snap: &mut Din4108Snapshot) -> Result<(), String> {
    
    let i = payload.index.min(snap.zones.len());
    snap.zones.insert(i, payload.zone.clone());
