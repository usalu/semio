//! Diff for `change-element-delta-uf`.
...
fn apply_in_place(payload: &ChangeElementDeltaUf, snap: &mut Din4108Snapshot) -> Result<(), String> {
    let e = snap.elements.iter_mut().find(|e| e.id == payload.element_id).ok_or("element not found")?;
    e.delta_u_f = payload.new_delta_u_f;
    Ok(())
}
