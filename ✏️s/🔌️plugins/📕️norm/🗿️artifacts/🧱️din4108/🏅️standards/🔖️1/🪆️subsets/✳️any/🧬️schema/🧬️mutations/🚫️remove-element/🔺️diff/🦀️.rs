...
fn apply_in_place(payload: &RemoveElement, snap: &mut Din4108Snapshot) -> Result<(), String> {

    if payload.index >= snap.elements.len() { return Err("element index out of range".into()); }
    snap.elements.remove(payload.index);
