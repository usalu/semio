//! Diff for `change-zone-window-orientation`.
...
fn apply_in_place(payload: &ChangeZoneWindowOrientation, snap: &mut Din4108Snapshot) -> Result<(), String> {
    ...
    w.orientation = payload.new_orientation.clone();
    Ok(())
}
