//! 🔺️ `insert-element` diff — whole-list rewrite via Din4108Diff list wrappers.
...
fn apply_in_place(payload: &InsertElement, snap: &mut Din4108Snapshot) -> Result<(), String> {
    
    let i = payload.index.min(snap.elements.len());
    snap.elements.insert(i, payload.element.clone());
