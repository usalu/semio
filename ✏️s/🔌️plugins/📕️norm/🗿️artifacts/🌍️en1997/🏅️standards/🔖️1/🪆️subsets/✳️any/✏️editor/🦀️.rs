...
use crate::editor::en1997::commands::{apply_remedy, evaluate, insert_item, remove_item, selected_check, set_active_example, set_field, set_snapshot};
...
pub const CONTROLLER_ID: &str = "s.norm.en1997@1/*#editor";
...
semio_framework_plugin::app_commands! {
    ...
    pub enum En1997Command for En1997Snapshot, En1997Mutation, NoConfig, NoConfigMutation {
...
impl ArtifactEditor for En1997PlayApp {
    ...
    semio_framework_plugin::bounded_first_step_tool_proofs! {
        ...
        artifact_schema: "semio.norm.en1997/v1",
    ...
    semio_s_artifact_norm_contract::norm_command_from_action!(En1997Command, crate::standards::v1::subsets::any::schema::snapshot::decode_en1997_snapshot_json);
    ...
    ) -> Result<Emit<En1997Mutation, NoConfigMutation, Self::DraftMutation>, Fault> {
        crate::app_surface::dispatch_norm_command::<Self>(command, doc, cfg, view_state)
    }

    fn render(body_key: &str, doc: &ArtifactView<'_, En1997Snapshot>, cfg: &ConfigView<'_, NoConfig>, view_state: &semio_framework_plugin::ViewModel) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::ComponentTree> {
        let host = NormHost::<En1997Family>::from_artifact(doc.snapshot.clone());
        match body_key {
    ...
    fn import_media(port: &str, media: &Media, doc: &ArtifactView<'_, En1997Snapshot>) -> Result<Emit<En1997Mutation, NoConfigMutation, Self::DraftMutation>, MediaError> {
        let base = doc.snapshot.clone();
        crate::app_surface::import_media(port, media, move |snapshot: En1997Snapshot| En1997Mutation::from_snapshot(&base, &snapshot))
    }
...
impl crate::app_surface::NormRetainedEditor for En1997PlayApp {
    type Family = En1997Family;

    type ResultsWindowConfigOwner = results::ResultsWindowConfigOwner;

    fn selected_check_window_mutation(command: &En1997Command) -> Option<crate::results_window_config::NormResultsWindowConfigMutation> {
        match command {
            En1997Command::SetSelectedCheckIndex(payload) => Some(selected_check::window_mutation(payload)),
            _ => None,
        }
    }

    fn dispatch_retained(command: &En1997Command, doc: &ArtifactView<'_, En1997Snapshot>, cfg: &ConfigView<'_, NoConfig>) -> Result<Emit<En1997Mutation, NoConfigMutation, NoDraftMutation>, Fault> {
...
pub fn create_en1997_app() -> semio_framework_plugin::AppDefinition {
    Editor::builder(crate::EN1997_DIALECT)
            ...
            .action_with(
                semio_framework_plugin::ActionDefinition::new_catalog("setSnapshot", LocalizedLabel::native("Set En1997Snapshot", "Dokument setzen"), semio_framework_plugin::ActionKind::Mutation)
            ...
            .action_describe("setSnapshot", LocalizedLabel::native("Replaces the whole EN 1997 (Eurocode 7, geotechnical design) compliance document with the supplied document JSON; the previous inputs are discarded.", "Ersetzt das gesamte Nachweisdokument nach EN 1997 (Eurocode 7, Geotechnik) durch das übergebene Dokument-JSON; die bisherigen Eingaben werden verworfen."))
            .action_describe("evaluate", LocalizedLabel::native("Recomputes every EN 1997 (Eurocode 7, geotechnical design) check from the document's inputs and refreshes the results window; the document is not changed.", "Berechnet alle Nachweise nach EN 1997 (Eurocode 7, Geotechnik) aus den Eingaben des Dokuments neu und aktualisiert das Ergebnisfenster; das Dokument ändert sich nicht."))
            ...
            .action_describe("setActiveExample", LocalizedLabel::native("Loads one of the bundled EN 1997 (Eurocode 7, geotechnical design) examples into the open compliance document, replacing its inputs, by example id.", "Lädt eines der mitgelieferten Beispiele nach EN 1997 (Eurocode 7, Geotechnik) in das offene Nachweisdokument und ersetzt dessen Eingaben, anhand der Beispiel-Id."))
            .build_definition()
}
