//! 🧪️ `change-qs-kpa` fixture — `🪵️raises-the-unit-shaft-resistance-to-120-kpa`.
//!
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(
        ...
        None,
        "change-qs-kpa/raises-the-unit-shaft-resistance-to-120-kpa: the payload is finite, so `change-qs-kpa`'s `mutation.invariant` fatal cannot fire, and 120.0 differs from the committed 80.0, so its `mutation.no-op` warning cannot either"
    );
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-qs-kpa/raises-the-unit-shaft-resistance-to-120-kpa: the produced diff differs from the committed 🔺️diff/🔣️.json");
}
