//! 🧪️ `change-footing-embedment` named test.

use crate::{En1997Mutation, En1997Snapshot};
use protocol::MutationDiff;

#[semio_framework_async_macros::async_test]
async fn applies_change_footing_embedment() {
    let base = En1997Snapshot::default();
    let mutation = sample_mutation(&base);
    let outcome = <En1997Mutation as protocol::Mutation<En1997Snapshot>>::diff(&mutation, &base);
    assert_eq!(outcome.worst_level(), None, "change-footing-embedment should apply cleanly on the default subject");
    let after = MutationDiff::apply(outcome.diff(), &base).expect("applies");
    assert_ne!(after, base, "change-footing-embedment must change the snapshot");
    ...
    assert_eq!(restored, base, "change-footing-embedment inverse restores the base snapshot");
}

fn sample_mutation(base: &En1997Snapshot) -> En1997Mutation {
    use crate::mutations::*;
    En1997Mutation::ChangeFootingEmbedment(change_footing_embedment::ChangeFootingEmbedment { id: base.footings[0].id.clone(), new_embedment: base.footings[0].embedment + 0.3 })
}
