...
async fn an_out_of_range_selected_index_falls_back_to_the_first_check() {
    let host = NormHost::<En1997Family>::from_artifact(crate::En1997Snapshot::default());
    let first = semio_framework_plugin::artifact_app_laws::project_and_retire_fixture_tree(semio_framework_plugin::ComponentTree { root: render(&host, None, Default::default(), None).expect("node assembly") }).expect("json");
    let clamped = semio_framework_plugin::artifact_app_laws::project_and_retire_fixture_tree(semio_framework_plugin::ComponentTree { root: render(&host, Some(9_999), Default::default(), None).expect("node assembly") }).expect("json");
