pub(crate) mod context {
    ...
    /// ✏️ Adapts `create_en1997_app`'s `AppDefinition` (contract §2.4) into the `App { definition,
    ...
    pub fn en1997_manifest_for_tests() -> semio_framework_plugin::App {
        semio_framework_plugin::App { definition: create_en1997_app(), examples: Vec::new() }
    }
    
    pub type NormApp = VcsArtifactApp<EditorApp<En1997PlayApp>>;
    ...
    pub async fn dispatch(app: &mut NormApp, command: En1997Command) -> InvocationResult {
...
fn retained_command_dispositions_match_the_language_neutral_oracle() {
    ...
    let definition = create_en1997_app();
...
/// ð¯ï¸ One value per `En1997Command` row â the whole-command-surface laws below iterate it, so a new row
/// that is not listed here fails `command_ids_cover_every_row`.
fn every_command() -> Vec<En1997Command> {
    vec![
        En1997Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1997Snapshot::default() }),
        En1997Command::Evaluate(evaluate::Evaluate {}),
        En1997Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) }),
        En1997Command::SetActiveExample(set_active_example::SetActiveExample { example_id: String::new() }),
    ]
}
...
async fn command_ids_cover_every_row_and_are_unique() {
    ...
    let ids: Vec<&str> = commands.iter().map(En1997Command::command_id).collect();
...
/// of the three payload shapes involves the per-standard `En1997Snapshot`.
#[semio_framework_async_macros::async_test]
async fn optional_field_rows_keep_their_pre_migration_bytes() {
    let hex = |command: &En1997Command| protocol::OpBinary::encode_op(command).expect("encode").iter().map(|byte| format!("{byte:02x}")).collect::<String>();
    assert_eq!(hex(&En1997Command::Evaluate(evaluate::Evaluate {})), "01010000");
    assert_eq!(hex(&En1997Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) })), "01020001000402");
    assert_eq!(hex(&En1997Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: None })), "01020000");
}
...
async fn declares_model_in_and_report_out_ports() {
    let ports = create_en1997_app().io.ports;
...
async fn an_unknown_body_key_falls_back_to_a_text_node() {
    ...
    assert!(context::render(&mut app, "norm.en1997.play.nope").await.contains("Unknown body"));
...
async fn set_snapshot_commits_a_host_backed_report() {
    let mut app = context::app_with_registry().await;
    context::dispatch(&mut app, En1997Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1997Snapshot::default() })).await;
    context::dispatch(&mut app, En1997Command::Evaluate(evaluate::Evaluate {})).await;
    context::settle(&mut app).await;
    let host = NormHost::<En1997Family>::from_artifact(app.snapshot().expect("projection"));
...
async fn selected_check_index_is_a_config_only_edit() {
    let mut app = context::app_with_registry().await;
    let before = app.snapshot().expect("projection");
    let result = context::dispatch(&mut app, En1997Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) })).await;
...
async fn view_actions_never_emit_artifact_mutations_under_the_real_registry() {
    let mut app = context::app_with_registry().await;
    let result = context::dispatch(&mut app, En1997Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(1) })).await;
...
async fn undo_redo_round_trips_through_the_wrapper() {
    ...
    context::settle(&mut app).await;
    assert_eq!(app.snapshot().expect("projection"), En1997Snapshot::default());
    context::close(&mut app);
}
