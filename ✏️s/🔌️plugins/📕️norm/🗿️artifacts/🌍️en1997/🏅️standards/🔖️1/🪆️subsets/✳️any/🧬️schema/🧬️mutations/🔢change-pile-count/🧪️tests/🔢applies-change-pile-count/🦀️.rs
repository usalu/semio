...
async fn applies_change_pile_count() {
    ...
    assert_eq!(restored, base, "change-pile-count inverse restores the base snapshot");
}

fn sample_mutation(base: &En1997Snapshot) -> En1997Mutation {
    use crate::mutations::*;
    En1997Mutation::ChangePileCount(change_pile_count::ChangePileCount { id: base.piles[0].id.clone(), new_count: base.piles[0].count + 1 })
}
