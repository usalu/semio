//! 🧪️ `change-h-ed-kn` fixture — `➡️raises-the-design-horizontal-load-to-120-kn`.
//!
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(
        ...
        None,
        "change-h-ed-kn/raises-the-design-horizontal-load-to-120-kn: the payload is finite, so `change-h-ed-kn`'s `mutation.invariant` fatal cannot fire, and 120.0 differs from the committed 80.0, so its `mutation.no-op` warning cannot either"
    );
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-h-ed-kn/raises-the-design-horizontal-load-to-120-kn: the produced diff differs from the committed 🔺️diff/🔣️.json");
}
