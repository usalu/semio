...
impl ArtifactBuilder for Din4108Builder {
    ...
    fn from_text(text: &str) -> Result<Self, store::TextError> {
        Ok(Self::from_snapshot(<Din4108Snapshot as store::DocumentDsl>::parse_dsl(text)?))
    }
    fn from_binary(bytes: &[u8]) -> Result<Self, store::PackError> {
        Ok(Self::from_snapshot(<Din4108Snapshot as store::DocumentPack>::decode_pack(bytes)?))
    }
