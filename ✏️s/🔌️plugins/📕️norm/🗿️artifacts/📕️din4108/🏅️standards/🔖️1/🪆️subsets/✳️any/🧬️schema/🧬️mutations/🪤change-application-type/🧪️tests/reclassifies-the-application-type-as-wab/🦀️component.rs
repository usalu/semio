//! 🧪️ `change-application-type` fixture — `reclassifies-the-application-type-as-wab`.
//!
...
/// ▶️ `change-application-type` carries `before` to exactly the committed `after`.
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "change-application-type/reclassifies-the-application-type-as-wab: inverse did not restore the before-snapshot");
    assert_eq!(inverse.len(), 1, "change-application-type/reclassifies-the-application-type-as-wab: undoing one applicationType edit is exactly one step");
    assert_eq!(snapshot.application_type, "DEO", "change-application-type/reclassifies-the-application-type-as-wab: the undo step must put applicationType back to 'DEO'");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-application-type/reclassifies-the-application-type-as-wab: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "change-application-type/reclassifies-the-application-type-as-wab: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome — status AND every diagnostic `change-application-type`'s own diff builder
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "change-application-type/reclassifies-the-application-type-as-wab: raised diagnostics differ from the committed 🎯️outcome messages");
    ...
    match status {
        "applied" if declared.iter().any(|(_, code)| code == "mutation.no-op") => {
            assert!(applied, "change-application-type/reclassifies-the-application-type-as-wab: declared applied but the mutation was rejected");
            assert_eq!(snapshot, before(), "change-application-type/reclassifies-the-application-type-as-wab: a no-op outcome is applied with an EMPTY diff — the snapshot must come back untouched");
        }
        "applied" => {
            assert!(applied, "change-application-type/reclassifies-the-application-type-as-wab: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "change-application-type/reclassifies-the-application-type-as-wab: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "change-application-type/reclassifies-the-application-type-as-wab: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("change-application-type/reclassifies-the-application-type-as-wab: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta `change-application-type` produces is exactly the committed diff — the single most
...
/// `Some(None)` across a JSON round trip — `change-application-type` never writes it, so the committed
...
async fn committed_diff_is_canonical() {
    ...
    assert!(decoded.selected_check_index.is_none(), "change-application-type/reclassifies-the-application-type-as-wab: change-application-type is an artifact-lane edit and must never carry the presence-lane selectedCheckIndex");
    let reencoded = serde_json::to_value(&decoded).expect("diff re-encodes");
    let original: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff reparses");
    assert_eq!(reencoded, original, "change-application-type/reclassifies-the-application-type-as-wab: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `change-application-type` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
