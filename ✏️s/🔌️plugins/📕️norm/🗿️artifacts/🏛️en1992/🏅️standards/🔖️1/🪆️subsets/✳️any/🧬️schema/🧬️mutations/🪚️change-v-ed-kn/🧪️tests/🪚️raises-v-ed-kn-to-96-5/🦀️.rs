//! 🧪️ `change-v-ed-kn` fixture — `🪚️raises-v-ed-kn-to-96-5`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below was transcribed from this
//! leaf's own `../../🔺️diff/🦀️.rs`, which writes `En1992Diff.v_ed_kn` and nothing else,
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪚️change-v-ed-kn/🪚️raises-v-ed-kn-to-96-5/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪚️change-v-ed-kn/🪚️raises-v-ed-kn-to-96-5/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪚️change-v-ed-kn/🪚️raises-v-ed-kn-to-96-5/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪚️change-v-ed-kn/🪚️raises-v-ed-kn-to-96-5/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪚️change-v-ed-kn/🪚️raises-v-ed-kn-to-96-5/🎯️outcome/🔣️.json");
...
async fn change_v_ed_kn_applies_to_committed_after() {
...
#[semio_framework_async_macros::async_test]
async fn change_v_ed_kn_inverse_restores_before() {
    let base = before();
...
#[semio_framework_async_macros::async_test]
async fn change_v_ed_kn_committed_json_is_canonical() {
...
#[semio_framework_async_macros::async_test]
async fn change_v_ed_kn_declared_outcome_holds() {
...
#[semio_framework_async_macros::async_test]
async fn change_v_ed_kn_produces_committed_diff() {
    let outcome = <En1992Mutation as protocol::Mutation<En1992Snapshot>>::diff(&mutation(), &before());
    ...
    assert_eq!(produced, committed, "change-v-ed-kn/raises-v-ed-kn-to-96-5: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
#[semio_framework_async_macros::async_test]
async fn change_v_ed_kn_committed_diff_is_canonical() {
    ...
    assert!(serde_json::to_value(&decoded).expect("diff JSON").get("selectedCheckIndex").is_none(), "change-v-ed-kn/raises-v-ed-kn-to-96-5: the committed diff must leave the presence-lane selectedCheckIndex absent");
...
#[semio_framework_async_macros::async_test]
async fn change_v_ed_kn_committed_diff_applies_to_after() {
    let decoded: En1992Diff = serde_json::from_str(DIFF).expect("change-v-ed-kn committed diff decodes");
