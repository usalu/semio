...
async fn change_anchor_f_uk_mpa_committed_diff_is_canonical() {
    ...
    assert!(serde_json::to_value(&decoded).expect("diff JSON").get("selectedCheckIndex").is_none(), "change-anchor-f-uk-mpa/raises-anchor-f-uk-mpa-to-900-0: the committed diff must leave the presence-lane selectedCheckIndex absent");
