...
async fn change_anchor_c1_mm_committed_diff_is_canonical() {
    ...
    assert!(serde_json::to_value(&decoded).expect("diff JSON").get("selectedCheckIndex").is_none(), "change-anchor-c1-mm/raises-anchor-c1-mm-to-137-5: the committed diff must leave the presence-lane selectedCheckIndex absent");
