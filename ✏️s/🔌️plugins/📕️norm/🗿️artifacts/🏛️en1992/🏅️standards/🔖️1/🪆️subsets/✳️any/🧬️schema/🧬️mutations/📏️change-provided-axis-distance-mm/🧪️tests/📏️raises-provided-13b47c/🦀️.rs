...
async fn change_provided_axis_distance_mm_applies_to_committed_after() {
...
#[semio_framework_async_macros::async_test]
async fn change_provided_axis_distance_mm_inverse_restores_before() {
    let base = before();
...
#[semio_framework_async_macros::async_test]
async fn change_provided_axis_distance_mm_committed_json_is_canonical() {
...
#[semio_framework_async_macros::async_test]
async fn change_provided_axis_distance_mm_declared_outcome_holds() {
...
#[semio_framework_async_macros::async_test]
async fn change_provided_axis_distance_mm_produces_committed_diff() {
    let outcome = <En1992Mutation as protocol::Mutation<En1992Snapshot>>::diff(&mutation(), &before());
...
#[semio_framework_async_macros::async_test]
async fn change_provided_axis_distance_mm_committed_diff_is_canonical() {
    ...
    assert!(
        ...
        "change-provided-axis-distance-mm/raises-provided-axis-distance-mm-to-42-5: the committed diff must leave the presence-lane selectedCheckIndex absent"
    );
...
#[semio_framework_async_macros::async_test]
async fn change_provided_axis_distance_mm_committed_diff_applies_to_after() {
    let decoded: En1992Diff = serde_json::from_str(DIFF).expect("change-provided-axis-distance-mm committed diff decodes");
