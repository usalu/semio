...
use semio_s_artifact_fem_2d::elements2d::BeamEb2;
#[cfg(feature = "cross-fem")]
use semio_s_artifact_fem_2d::model::{Dof, ElementResult, Elements, FemError, MemberUdl, Model, Node, StaticResult, Support};

#[cfg(feature = "cross-fem")]
fn max_beam_moment_knm(result: &StaticResult, element_id: &str) -> f64 {
    let (_, ElementResult::Beam { stations }) = result.elements.iter().find(|(id, _)| id == element_id).expect("beam element result") else {
...
#[cfg(feature = "cross-fem")]
fn max_beam_shear_kn(result: &StaticResult, element_id: &str) -> f64 {
    let (_, ElementResult::Beam { stations }) = result.elements.iter().find(|(id, _)| id == element_id).expect("beam element result") else {
        panic!("expected beam element result");
    };
...
#[cfg(feature = "cross-fem")]
#[allow(clippy::too_many_arguments, reason = "one argument per parameter the published clause formula itself names; bundling them into a struct would break the 1:1 reading against the standard")]
pub fn check_rc_beam_from_fem(span_m: f64, udl_kn_m: f64, f_ck: f64, b_mm: f64, d_mm: f64, a_s_mm2: f64, f_yk: f64, rho_l: f64, annex: AnnexChoice) -> Result<CheckReport, FemError> {
    ...
    model.elements.push(Elements::BeamEb2(BeamEb2 { id: "b1".into(), start: "n0".into(), end: "n1".into(), e: 30e9, area: b_mm * d_mm / 1e6, iy: b_mm * d_mm.powi(3) / 12e12, density: 2500.0 }));
    model.member_loads.push(("b1".into(), MemberUdl { wx: 0.0, wy: -udl_kn_m * 1000.0, wz: 0.0 }));

    let result = semio_s_artifact_fem_2d::model::solve_linear_static(&model)?;
