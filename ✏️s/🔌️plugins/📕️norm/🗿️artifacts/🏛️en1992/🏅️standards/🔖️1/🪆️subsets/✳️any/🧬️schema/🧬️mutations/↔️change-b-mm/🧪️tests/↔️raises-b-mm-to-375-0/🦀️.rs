...
async fn change_b_mm_applies_to_committed_after() {
...
#[semio_framework_async_macros::async_test]
async fn change_b_mm_inverse_restores_before() {
    let base = before();
...
#[semio_framework_async_macros::async_test]
async fn change_b_mm_committed_json_is_canonical() {
...
#[semio_framework_async_macros::async_test]
async fn change_b_mm_declared_outcome_holds() {
...
#[semio_framework_async_macros::async_test]
async fn change_b_mm_produces_committed_diff() {
    let outcome = <En1992Mutation as protocol::Mutation<En1992Snapshot>>::diff(&mutation(), &before());
...
#[semio_framework_async_macros::async_test]
async fn change_b_mm_committed_diff_is_canonical() {
    ...
    assert!(serde_json::to_value(&decoded).expect("diff JSON").get("selectedCheckIndex").is_none(), "change-b-mm/raises-b-mm-to-375-0: the committed diff must leave the presence-lane selectedCheckIndex absent");
...
#[semio_framework_async_macros::async_test]
async fn change_b_mm_committed_diff_applies_to_after() {
    let decoded: En1992Diff = serde_json::from_str(DIFF).expect("change-b-mm committed diff decodes");
