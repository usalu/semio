...
async fn change_liquid_rho_p_eff_committed_diff_is_canonical() {
    ...
    assert!(serde_json::to_value(&decoded).expect("diff JSON").get("selectedCheckIndex").is_none(), "change-liquid-rho-p-eff/raises-liquid-rho-p-eff-to-0-0078125: the committed diff must leave the presence-lane selectedCheckIndex absent");
