pub(crate) mod context {
    ...
    pub fn en1992_manifest_for_tests() -> semio_framework_plugin::App {
...
fn every_command() -> Vec<En1992Command> {
    vec![
        ...
        En1992Command::SetActiveExample(set_active_example::SetActiveExample { example_id: String::new() }),
    ]
}
...
async fn an_unknown_body_key_falls_back_to_a_text_node() {
    ...
    assert!(context::render(&mut app, "norm.en1992.play.nope").await.contains("Unknown body"));
...
async fn set_snapshot_commits_a_host_backed_report() {
    let mut app = context::app_with_registry().await;
    context::dispatch(&mut app, En1992Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1992Snapshot::default() })).await;
    let mut host = NormHost::<En1992Family>::from_artifact(app.snapshot().expect("projection"));
    host.evaluate();
...
async fn evaluate_recommits_the_current_projection_without_changing_it() {
    ...
    context::dispatch(&mut app, En1992Command::Evaluate(evaluate::Evaluate {})).await;
...
async fn selected_check_index_is_a_config_only_edit() {
    let mut app = context::app_with_registry().await;
    let before = app.snapshot().expect("projection");
    let result = context::dispatch(&mut app, En1992Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) })).await;
...
async fn view_actions_never_emit_artifact_mutations_under_the_real_registry() {
    let mut app = context::app_with_registry().await;
    let result = context::dispatch(&mut app, En1992Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(1) })).await;
...
async fn report_out_exports_the_computed_check_report() {
    ...
    let checks = value.get("checks").or_else(|| value.get("Checks")).expect("checks array");
    assert!(checks.as_array().map(|a| !a.is_empty()).unwrap_or(false));
    context::close(&mut app);
}
