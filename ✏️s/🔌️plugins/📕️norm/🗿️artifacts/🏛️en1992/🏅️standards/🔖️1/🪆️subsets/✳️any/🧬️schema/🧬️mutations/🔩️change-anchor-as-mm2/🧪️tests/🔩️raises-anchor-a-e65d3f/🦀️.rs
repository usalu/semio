//! 🧪️ `change-anchor-as-mm2` fixture — `🔩️raises-anchor-a-s-mm2-to-157-0`.
//!
...
async fn change_anchor_as_mm2_applies_to_committed_after() {
...
#[semio_framework_async_macros::async_test]
async fn change_anchor_as_mm2_inverse_restores_before() {
    let base = before();
...
#[semio_framework_async_macros::async_test]
async fn change_anchor_as_mm2_committed_json_is_canonical() {
...
#[semio_framework_async_macros::async_test]
async fn change_anchor_as_mm2_declared_outcome_holds() {
...
#[semio_framework_async_macros::async_test]
async fn change_anchor_as_mm2_produces_committed_diff() {
    let outcome = <En1992Mutation as protocol::Mutation<En1992Snapshot>>::diff(&mutation(), &before());
...
#[semio_framework_async_macros::async_test]
async fn change_anchor_as_mm2_committed_diff_is_canonical() {
    ...
    assert!(serde_json::to_value(&decoded).expect("diff JSON").get("selectedCheckIndex").is_none(), "change-anchor-as-mm2/raises-anchor-a-s-mm2-to-157-0: the committed diff must leave the presence-lane selectedCheckIndex absent");
...
#[semio_framework_async_macros::async_test]
async fn change_anchor_as_mm2_committed_diff_applies_to_after() {
    let decoded: En1992Diff = serde_json::from_str(DIFF).expect("change-anchor-as-mm2 committed diff decodes");
