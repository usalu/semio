...
impl ArtifactEditor for En1995PlayApp {
    ...
    semio_framework_plugin::bounded_first_step_tool_proofs! {
        ...
        artifact_schema: "semio.norm.en1995/v1",
    ...
    semio_s_artifact_norm_contract::norm_command_from_action!(En1995Command, crate::standards::v1::subsets::any::schema::snapshot::decode_en1995_snapshot_json);
    ...
    ) -> Result<Emit<En1995Mutation, NoConfigMutation, Self::DraftMutation>, Fault> {
        crate::app_surface::dispatch_norm_command::<Self>(command, doc, cfg, view_state)
    }

    fn render(body_key: &str, doc: &ArtifactView<'_, En1995Snapshot>, cfg: &ConfigView<'_, NoConfig>, view_state: &semio_framework_plugin::ViewModel) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::ComponentTree> {
        let host = NormHost::<En1995Family>::from_artifact(doc.snapshot.clone());
        match body_key {
...
impl crate::app_surface::NormRetainedEditor for En1995PlayApp {
    ...
    fn selected_check_window_mutation(command: &En1995Command) -> Option<crate::results_window_config::NormResultsWindowConfigMutation> {
        match command {
            En1995Command::SetSelectedCheckIndex(payload) => Some(selected_check::window_mutation(payload)),
            _ => None,
        }
    }

    fn dispatch_retained(command: &En1995Command, doc: &ArtifactView<'_, En1995Snapshot>, cfg: &ConfigView<'_, NoConfig>) -> Result<Emit<En1995Mutation, NoConfigMutation, NoDraftMutation>, Fault> {
