...
fn raises_the_section_modulus_to_9500000_mm3() {
...
/// back on `z_mm3`.
#[semio_framework_async_macros::async_test]
fn restoring_8000000_mm3_restores_before() {
    let base = before();
...
fn declared_outcome_holds() {
    ...
    assert_eq!(
        ...
        None,
        "change-z-mm3/raises-the-section-modulus-to-9500000-mm3: the payload is a finite number, so the `is_finite` fatal guard stays shut, and 9500000.0 differs from the committed 8000000.0, so the `mutation.no-op` warning guard stays shut too"
    );
