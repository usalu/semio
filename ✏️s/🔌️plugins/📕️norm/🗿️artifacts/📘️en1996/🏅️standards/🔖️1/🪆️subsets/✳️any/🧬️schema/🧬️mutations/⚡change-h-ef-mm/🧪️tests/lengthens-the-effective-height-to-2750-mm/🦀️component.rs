...
fn lengthens_the_effective_height_to_2750_mm() {
...
#[semio_framework_async_macros::async_test]
fn restoring_2500_mm_restores_before() {
    let base = before();
...
fn declared_outcome_holds() {
    ...
    assert_eq!(
        ...
        None,
        "change-h-ef-mm/lengthens-the-effective-height-to-2750-mm: the payload is a finite number, so the `is_finite` fatal guard stays shut, and 2750.0 differs from the committed 2500.0, so the `mutation.no-op` warning guard stays shut too"
    );
