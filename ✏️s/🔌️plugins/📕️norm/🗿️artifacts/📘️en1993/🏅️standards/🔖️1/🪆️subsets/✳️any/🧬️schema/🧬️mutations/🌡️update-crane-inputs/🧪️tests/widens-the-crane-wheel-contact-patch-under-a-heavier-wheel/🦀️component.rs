//! 🧪️ `update-crane-inputs` fixture — `widens-the-crane-wheel-contact-patch-under-a-heavier-wheel`.
//!
//! EN 1993-6's four crane-runway fields: the wheel load 50→75 kN, the wheel contact length 100→125 mm, the rail load-dispersion length 50→62.5 mm and the web thickness 10→12 mm.
//!
...
fn applied() -> En1993Snapshot {
    ...
    <En1993Diff as protocol::MutationDiff<En1993Snapshot>>::apply(raised.diff(), &base).expect("update-crane-inputs applies to its committed before-snapshot")
}
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "update-crane-inputs/widens-the-crane-wheel-contact-patch-under-a-heavier-wheel: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert!(!inverse.is_empty(), "update-crane-inputs/widens-the-crane-wheel-contact-patch-under-a-heavier-wheel: this mutation changes state, so its inverse must not be empty");
    ...
    assert_eq!(snapshot, base, "update-crane-inputs/widens-the-crane-wheel-contact-patch-under-a-heavier-wheel: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "update-crane-inputs/widens-the-crane-wheel-contact-patch-under-a-heavier-wheel: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "update-crane-inputs/widens-the-crane-wheel-contact-patch-under-a-heavier-wheel: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome — status AND every diagnostic `update-crane-inputs`'s own diff builder raises —
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "update-crane-inputs/widens-the-crane-wheel-contact-patch-under-a-heavier-wheel: raised diagnostics differ from the committed 🎯️outcome messages");
    let snapshot = applied();
    match status {
        "applied" => assert_ne!(snapshot, before(), "update-crane-inputs/widens-the-crane-wheel-contact-patch-under-a-heavier-wheel: declared applied but the snapshot came back unchanged"),
        "rejected" => assert_eq!(snapshot, before(), "update-crane-inputs/widens-the-crane-wheel-contact-patch-under-a-heavier-wheel: a rejected mutation must leave the snapshot untouched"),
        other => panic!("update-crane-inputs/widens-the-crane-wheel-contact-patch-under-a-heavier-wheel: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH fields `update-crane-inputs` is allowed to
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "update-crane-inputs/widens-the-crane-wheel-contact-patch-under-a-heavier-wheel: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "update-crane-inputs/widens-the-crane-wheel-contact-patch-under-a-heavier-wheel: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `update-crane-inputs` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "update-crane-inputs/widens-the-crane-wheel-contact-patch-under-a-heavier-wheel: committed diff did not carry before to after");
}
