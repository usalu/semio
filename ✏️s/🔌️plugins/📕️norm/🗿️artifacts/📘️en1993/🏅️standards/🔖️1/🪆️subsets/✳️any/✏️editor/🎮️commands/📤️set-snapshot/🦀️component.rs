//! 📤️ EN 1993 play app command — replace the whole compliance document.
//!
...
//! `En1993Mutation::from_snapshot`, bundled into a single atomic edit.
...
pub struct ReplaceSnapshot {
    #[dsl(block)]
    pub snapshot: En1993Snapshot,
}
//#endregion 🔖️Payload

//#region 🔖️Handler
pub async fn handle(payload: &ReplaceSnapshot, _doc: &ArtifactView<'_, En1993Snapshot>, _cfg: &ConfigView<'_, NormConfig>) -> Result<Emit<En1993Mutation, NormConfigMutation>, Fault> {
    crate::app_surface::commit_snapshot_fields(En1993Mutation::from_snapshot(&payload.snapshot), "setSnapshot")
}
//#endregion 🔖️Handler
...
mod tests {
    ...
    async fn handle_commits_the_payload_document_under_its_action_id() {
        ...
        let emit = handle(&ReplaceSnapshot { snapshot: En1993Snapshot::default() }, &ArtifactView::new(&projection, &HistoryView::empty()), &ConfigView { snapshot: &config }).expect("handle");
        assert_eq!(emit.artifact_mutations, En1993Mutation::from_snapshot(&En1993Snapshot::default()));
