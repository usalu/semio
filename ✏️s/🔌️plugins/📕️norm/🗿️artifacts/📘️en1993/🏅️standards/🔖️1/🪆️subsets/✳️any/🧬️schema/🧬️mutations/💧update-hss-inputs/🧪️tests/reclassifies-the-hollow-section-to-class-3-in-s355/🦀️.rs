//! 🧪️ `update-hss-inputs` fixture — `reclassifies-the-hollow-section-to-class-3-in-s355`.
//!
//! EN 1993-1-12's hollow-section group: elastic modulus 400000→320000 mm3, grade 460→355 MPa, section class 2→3 (a `u8`) and design moment 100→75 kNm. Dropping to class 3 is what forces the elastic rather than plastic modulus into the check.
//!
...
fn applied() -> En1993Snapshot {
    ...
    <En1993Diff as protocol::MutationDiff<En1993Snapshot>>::apply(raised.diff(), &base).expect("update-hss-inputs applies to its committed before-snapshot")
}
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "update-hss-inputs/reclassifies-the-hollow-section-to-class-3-in-s355: applied state differs from committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    assert!(!inverse.is_empty(), "update-hss-inputs/reclassifies-the-hollow-section-to-class-3-in-s355: this mutation changes state, so its inverse must not be empty");
    ...
    assert_eq!(snapshot, base, "update-hss-inputs/reclassifies-the-hollow-section-to-class-3-in-s355: inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "update-hss-inputs/reclassifies-the-hollow-section-to-class-3-in-s355: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "update-hss-inputs/reclassifies-the-hollow-section-to-class-3-in-s355: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome — status AND every diagnostic `update-hss-inputs`'s own diff builder raises —
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "update-hss-inputs/reclassifies-the-hollow-section-to-class-3-in-s355: raised diagnostics differ from the committed 🎯️outcome messages");
    let snapshot = applied();
    match status {
        "applied" => assert_ne!(snapshot, before(), "update-hss-inputs/reclassifies-the-hollow-section-to-class-3-in-s355: declared applied but the snapshot came back unchanged"),
        "rejected" => assert_eq!(snapshot, before(), "update-hss-inputs/reclassifies-the-hollow-section-to-class-3-in-s355: a rejected mutation must leave the snapshot untouched"),
        other => panic!("update-hss-inputs/reclassifies-the-hollow-section-to-class-3-in-s355: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH fields `update-hss-inputs` is allowed to
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "update-hss-inputs/reclassifies-the-hollow-section-to-class-3-in-s355: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "update-hss-inputs/reclassifies-the-hollow-section-to-class-3-in-s355: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `update-hss-inputs` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "update-hss-inputs/reclassifies-the-hollow-section-to-class-3-in-s355: committed diff did not carry before to after");
}
