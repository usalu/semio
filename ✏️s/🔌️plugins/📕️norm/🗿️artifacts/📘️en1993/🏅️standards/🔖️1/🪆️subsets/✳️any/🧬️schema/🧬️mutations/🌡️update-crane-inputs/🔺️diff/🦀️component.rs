...
pub async fn diff(payload: &UpdateCraneInputs, base: &En1993Snapshot) -> protocol::MutationOutcome<En1993Diff> {
    ...
    if base.crane_f_z_ed_kn == payload.new_crane_f_z_ed_kn && base.crane_wheel_contact_length_mm == payload.new_crane_wheel_contact_length_mm && base.crane_dispersion_mm == payload.new_crane_dispersion_mm && base.crane_t_w_mm == payload.new_crane_t_w_mm {
        return protocol::MutationOutcome::empty().warn("mutation.no-op", "This facet already has these values.");
    }
    protocol::MutationOutcome::new(En1993Diff { crane_f_z_ed_kn: Some(payload.new_crane_f_z_ed_kn), crane_wheel_contact_length_mm: Some(payload.new_crane_wheel_contact_length_mm), crane_dispersion_mm: Some(payload.new_crane_dispersion_mm), crane_t_w_mm: Some(payload.new_crane_t_w_mm), ..Default::default() })
}
//#endregion 🔖️Diff
