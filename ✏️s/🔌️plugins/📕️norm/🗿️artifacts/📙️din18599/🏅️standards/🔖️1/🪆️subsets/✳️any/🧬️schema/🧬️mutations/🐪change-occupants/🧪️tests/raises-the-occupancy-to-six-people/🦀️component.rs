//! 🧪️ `change-occupants` fixture — `raises-the-occupancy-to-six-people`.
//!
//! The occupant count goes 4 → 6. `occupants` is the snapshot's only `u32`, so both the mutation payload and the diff must carry a JSON integer here, never a float — and this builder has NO finiteness guard, only the `mutation.no-op` equality check, because an integer cannot be NaN.
//!
...
fn applied() -> Din18599Snapshot {
    ...
    <Din18599Diff as protocol::MutationDiff<Din18599Snapshot>>::apply(raised.diff(), &base).expect("change-occupants applies to its committed before-snapshot")
}
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "change-occupants/raises-the-occupancy-to-six-people: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert!(!inverse.is_empty(), "change-occupants/raises-the-occupancy-to-six-people: this mutation changes state, so its inverse must not be empty");
    ...
    assert_eq!(snapshot, base, "change-occupants/raises-the-occupancy-to-six-people: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-occupants/raises-the-occupancy-to-six-people: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "change-occupants/raises-the-occupancy-to-six-people: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome — status AND every diagnostic `change-occupants`'s own diff builder raises —
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "change-occupants/raises-the-occupancy-to-six-people: raised diagnostics differ from the committed 🎯️outcome messages");
    let snapshot = applied();
    match status {
        "applied" => assert_ne!(snapshot, before(), "change-occupants/raises-the-occupancy-to-six-people: declared applied but the snapshot came back unchanged"),
        "rejected" => assert_eq!(snapshot, before(), "change-occupants/raises-the-occupancy-to-six-people: a rejected mutation must leave the snapshot untouched"),
        other => panic!("change-occupants/raises-the-occupancy-to-six-people: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH fields `change-occupants` is allowed to
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-occupants/raises-the-occupancy-to-six-people: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "change-occupants/raises-the-occupancy-to-six-people: committed diff JSON is not canonical");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "change-occupants/raises-the-occupancy-to-six-people: committed diff did not carry before to after");
}
