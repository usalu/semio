//! 🧪️ `update-tower-inputs` fixture — `🌬️raises-the-tower-wind-factor-and-leg-force`.
//!
//! EN 1993-3's two-field lattice-tower group: the wind amplification factor 1.25→1.75 and the leg design force 300→480 kN.
//!
...
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗼️update-tower-inputs/🌬️raises-the-tower-wind-factor-and-leg-force/🔺️diff/🔣️.json");
...
fn applied() -> En1993Snapshot {
    ...
    <En1993Diff as protocol::MutationDiff<En1993Snapshot>>::apply(raised.diff(), &base).expect("update-tower-inputs applies to its committed before-snapshot")
}
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "update-tower-inputs/raises-the-tower-wind-factor-and-leg-force: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert!(!inverse.is_empty(), "update-tower-inputs/raises-the-tower-wind-factor-and-leg-force: this mutation changes state, so its inverse must not be empty");
    ...
    assert_eq!(snapshot, base, "update-tower-inputs/raises-the-tower-wind-factor-and-leg-force: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "update-tower-inputs/raises-the-tower-wind-factor-and-leg-force: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "update-tower-inputs/raises-the-tower-wind-factor-and-leg-force: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome — status AND every diagnostic `update-tower-inputs`'s own diff builder raises —
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "update-tower-inputs/raises-the-tower-wind-factor-and-leg-force: raised diagnostics differ from the committed 🎯️outcome messages");
    let snapshot = applied();
    match status {
        "applied" => assert_ne!(snapshot, before(), "update-tower-inputs/raises-the-tower-wind-factor-and-leg-force: declared applied but the snapshot came back unchanged"),
        "rejected" => assert_eq!(snapshot, before(), "update-tower-inputs/raises-the-tower-wind-factor-and-leg-force: a rejected mutation must leave the snapshot untouched"),
        other => panic!("update-tower-inputs/raises-the-tower-wind-factor-and-leg-force: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH fields `update-tower-inputs` is allowed to
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "update-tower-inputs/raises-the-tower-wind-factor-and-leg-force: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "update-tower-inputs/raises-the-tower-wind-factor-and-leg-force: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `update-tower-inputs` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "update-tower-inputs/raises-the-tower-wind-factor-and-leg-force: committed diff did not carry before to after");
}
