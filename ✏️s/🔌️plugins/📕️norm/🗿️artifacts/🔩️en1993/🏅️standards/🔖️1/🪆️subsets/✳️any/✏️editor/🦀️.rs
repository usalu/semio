...
use crate::editor::en1993::commands::{evaluate, selected_check, set_snapshot};
...
impl ArtifactEditor for En1993PlayApp {
    ...
    const DIALECT: Dialect = crate::artifacts::en1993::EN1993_DIALECT;
    ...
    semio_framework_plugin::bounded_first_step_tool_proofs! {
        ...
        document_schema: "semio.norm.en1993/v1",
    ...
    ) -> Result<Emit<En1993Mutation, NormConfigMutation, Self::DraftMutation>, Fault> {
        command.dispatch(doc, cfg)
    }

    fn render(body_key: &str, doc: &ArtifactView<'_, En1993Snapshot>, cfg: &ConfigView<'_, NormConfig>) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::ComponentTree> {
...
impl crate::app_surface::NormRetainedEditor for En1993PlayApp {
    fn dispatch_retained(command: &En1993Command, doc: &ArtifactView<'_, En1993Snapshot>, cfg: &ConfigView<'_, NormConfig>) -> Result<Emit<En1993Mutation, NormConfigMutation, NoDraftMutation>, Fault> {
        command.dispatch(doc, cfg)
    }
}
...
pub fn create_en1993_app() -> semio_framework_plugin::AppDefinition {
    Editor::builder(crate::artifacts::en1993::EN1993_DIALECT)
...
pub(crate) mod testkit {
    ...
    /// ✏️ Adapts `create_en1993_app`'s `AppDefinition` (contract §2.4) into the `App { definition,
    ...
    pub fn en1993_manifest_for_testkit() -> semio_framework_plugin::App {
        semio_framework_plugin::App { definition: create_en1993_app(), examples: Vec::new() }
    }

    pub type NormApp = VcsArtifactApp<EditorApp<En1993PlayApp>>;
    ...
    pub async fn app_with_registry() -> NormApp {
        new_app_with_registry::<EditorApp<En1993PlayApp>>(en1993_manifest_for_testkit).await
    }

    pub async fn dispatch(app: &mut NormApp, command: En1993Command) -> InvocationResult {
...
mod tests {
    ...
    fn retained_command_dispositions_match_the_language_neutral_oracle() {
        ...
        let definition = create_en1993_app();
    ...
    async fn declares_model_in_and_report_out_ports() {
        let ports = create_en1993_app().io.ports;
    ...
    async fn an_unknown_body_key_falls_back_to_a_text_node() {
        ...
        assert!(testkit::render(&mut app, "norm.en1993.play.nope").await.contains("Unknown body"));
    }
    ...
    async fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::app_with_registry().await;
        testkit::dispatch(&mut app, En1993Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1993Snapshot::default() })).await;
    ...
    async fn evaluate_recommits_the_current_projection_without_changing_it() {
        ...
        testkit::dispatch(&mut app, En1993Command::Evaluate(evaluate::Evaluate {})).await;
    ...
    async fn selected_check_index_is_a_config_only_edit() {
        let mut app = testkit::app_with_registry().await;
        let before = app.snapshot().expect("projection");
        let result = testkit::dispatch(&mut app, En1993Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) })).await;
    ...
    async fn view_actions_never_emit_artifact_mutations_under_the_real_registry() {
        let mut app = testkit::app_with_registry().await;
        let result = testkit::dispatch(&mut app, En1993Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(1) })).await;
