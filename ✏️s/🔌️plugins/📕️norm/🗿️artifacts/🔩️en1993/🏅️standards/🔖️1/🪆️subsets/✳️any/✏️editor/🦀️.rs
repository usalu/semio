...
use crate::editor::en1993::commands::{apply_remedy, evaluate, insert_item, remove_item, selected_check, set_active_example, set_field, set_snapshot};
use crate::editor::en1993::modes::edit as edit_mode;
use crate::editor::en1993::modes::edit::windows::{inputs, results};
use crate::editor::en1993::panels::{catalogue as catalogue_panel, document as document_panel, inspection as inspection_panel};
...
pub const CONTROLLER_ID: &str = "s.norm.en1993@1/*#editor";
...
semio_framework_plugin::app_commands! {
    ...
    pub enum En1993Command for En1993Snapshot, En1993Mutation, NoConfig, NoConfigMutation {
...
impl ArtifactEditor for En1993PlayApp {
    /// 📚️ Artifact catalogue stamped by `PluginBuilder::editor` onto the navbar dropdown.
    fn examples() -> Vec<semio_framework_plugin::ExampleSource> {
        vec![crate::heb240_compliant::source(), crate::high_strength_connection::source()]
    }
    ...
    const DIALECT: Dialect = crate::EN1993_DIALECT;
    ...
    fn register_tool_job_factories(registry: &mut semio_framework_plugin::ArtifactToolFactoryRegistry<'_, semio_framework_plugin::EditorApp<Self>>) -> Result<(), Fault> {
        En1993BoundedCommandJobFactory::register(registry)
    }
    ...
    semio_framework_plugin::bounded_first_step_tool_proofs! {
        owner: semio_framework_plugin::EditorApp<En1993PlayApp>,
        owner_file: "✏️s/🔌️plugins/📕️norm/🗿️artifacts/🔩️en1993/🏅️standards/🔖️1/🪆️subsets/✳️any/✏️editor/🦀️.rs",
        controller: "s.norm.en1993@1/*#editor",
        artifact_schema: "semio.norm.en1993/v1",
        factory: "En1993BoundedCommandJobFactory",
        factory_type: En1993BoundedCommandJobFactory,
    ...
    semio_s_artifact_norm_contract::norm_command_from_action!(En1993Command, crate::standards::v1::subsets::any::schema::snapshot::decode_en1993_snapshot_json);

    fn handle(
        command: &En1993Command,
        doc: &ArtifactView<'_, En1993Snapshot>,
        cfg: &ConfigView<'_, NoConfig>,
        _interaction: &InteractionView<'_>,
        view_state: Option<&semio_framework_plugin::ViewModel>,
        _draft: &DraftView<'_, Self::Draft>,
        _engines: &EngineHandles,
    ) -> Result<Emit<En1993Mutation, NoConfigMutation, Self::DraftMutation>, Fault> {
        crate::app_surface::dispatch_norm_command::<Self>(command, doc, cfg, view_state)
    }

    fn render(body_key: &str, doc: &ArtifactView<'_, En1993Snapshot>, cfg: &ConfigView<'_, NoConfig>, view_state: &semio_framework_plugin::ViewModel) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::ComponentTree> {
        let host = NormHost::<En1993Family>::from_artifact(doc.snapshot.clone());
        match body_key {
    ...
    fn import_media(port: &str, media: &Media, _doc: &ArtifactView<'_, En1993Snapshot>) -> Result<Emit<En1993Mutation, NoConfigMutation, Self::DraftMutation>, MediaError> {
        crate::app_surface::import_media(port, media, |snapshot: En1993Snapshot| En1993Mutation::from_snapshot(&En1993Snapshot::default(), &snapshot))
    }
...
crate::norm_owned_tool_job_factory!(En1993BoundedCommandJobFactory, En1993PlayApp);

impl crate::app_surface::NormRetainedEditor for En1993PlayApp {
    type Family = En1993Family;

    type ResultsWindowConfigOwner = results::ResultsWindowConfigOwner;

    fn selected_check_window_mutation(command: &En1993Command) -> Option<crate::results_window_config::NormResultsWindowConfigMutation> {
        match command {
            En1993Command::SetSelectedCheckIndex(payload) => Some(selected_check::window_mutation(payload)),
            _ => None,
        }
    }

    fn dispatch_retained(command: &En1993Command, doc: &ArtifactView<'_, En1993Snapshot>, cfg: &ConfigView<'_, NoConfig>) -> Result<Emit<En1993Mutation, NoConfigMutation, NoDraftMutation>, Fault> {
...
//#region ðï¸Manifest
pub fn create_en1993_app() -> semio_framework_plugin::AppDefinition {
    Editor::builder(crate::EN1993_DIALECT)
            ...
            .action_describe("setSnapshot", LocalizedLabel::native("Replaces the whole EN 1993 (Eurocode 3, steel structures) compliance document with the supplied document JSON; the previous inputs are discarded.", "Ersetzt das gesamte Nachweisdokument nach EN 1993 (Eurocode 3, Stahlbau) durch das übergebene Dokument-JSON; die bisherigen Eingaben werden verworfen."))
            .action_describe("evaluate", LocalizedLabel::native("Recomputes every EN 1993 (Eurocode 3, steel structures) check from the document's inputs and refreshes the results window; the document is not changed.", "Berechnet alle Nachweise nach EN 1993 (Eurocode 3, Stahlbau) aus den Eingaben des Dokuments neu und aktualisiert das Ergebnisfenster; das Dokument ändert sich nicht."))
            ...
            .action_describe("setActiveExample", LocalizedLabel::native("Loads one of the bundled EN 1993 (Eurocode 3, steel structures) examples into the open compliance document, replacing its inputs, by example id.", "Lädt eines der mitgelieferten Beispiele nach EN 1993 (Eurocode 3, Stahlbau) in das offene Nachweisdokument und ersetzt dessen Eingaben, anhand der Beispiel-Id."))
            .build_definition()
}
