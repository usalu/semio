...
pub fn diff(payload: &UpdateColdFormedInputs, base: &En1993Snapshot) -> protocol::MutationOutcome<En1993Diff> {
    ...
    if base.cf_b_bar_mm == payload.new_cf_b_bar_mm
        && base.cf_t_mm == payload.new_cf_t_mm
        && base.cf_k_sigma == payload.new_cf_k_sigma
        && base.cf_psi == payload.new_cf_psi
        && base.cf_n_ed_kn == payload.new_cf_n_ed_kn
        && base.cf_gross_resistance_kn == payload.new_cf_gross_resistance_kn
    {
