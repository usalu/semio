//! 🧪️ `update-tension-component-inputs` fixture — `📉️derates-the-tension-rod-to-a-400-kn-characteristic-strength`.
//!
//! EN 1993-1-11's tension-component triple: the ultimate characteristic strength 500→400 kN, the characteristic resistance 350→280 kN and the design force 250→190 kN. All three guards in this builder are finiteness checks; every value here is finite, so no `mutation.invariant` fires.
//!
...
fn applied() -> En1993Snapshot {
    ...
    <En1993Diff as protocol::MutationDiff<En1993Snapshot>>::apply(raised.diff(), &base).expect("update-tension-component-inputs applies to its committed before-snapshot")
}
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "update-tension-component-inputs/derates-the-tension-rod-to-a-400-kn-characteristic-strength: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert!(!inverse.is_empty(), "update-tension-component-inputs/derates-the-tension-rod-to-a-400-kn-characteristic-strength: this mutation changes state, so its inverse must not be empty");
    ...
    assert_eq!(snapshot, base, "update-tension-component-inputs/derates-the-tension-rod-to-a-400-kn-characteristic-strength: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "update-tension-component-inputs/derates-the-tension-rod-to-a-400-kn-characteristic-strength: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "update-tension-component-inputs/derates-the-tension-rod-to-a-400-kn-characteristic-strength: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "update-tension-component-inputs/derates-the-tension-rod-to-a-400-kn-characteristic-strength: raised diagnostics differ from the committed 🎯️outcome messages");
    let snapshot = applied();
    match status {
        "applied" => assert_ne!(snapshot, before(), "update-tension-component-inputs/derates-the-tension-rod-to-a-400-kn-characteristic-strength: declared applied but the snapshot came back unchanged"),
        "rejected" => assert_eq!(snapshot, before(), "update-tension-component-inputs/derates-the-tension-rod-to-a-400-kn-characteristic-strength: a rejected mutation must leave the snapshot untouched"),
        other => panic!("update-tension-component-inputs/derates-the-tension-rod-to-a-400-kn-characteristic-strength: unknown outcome status {other:?}"),
    }
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "update-tension-component-inputs/derates-the-tension-rod-to-a-400-kn-characteristic-strength: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "update-tension-component-inputs/derates-the-tension-rod-to-a-400-kn-characteristic-strength: committed diff JSON is not canonical");
}
