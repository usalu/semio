//! 🧪️ `update-fire-inputs` fixture — `🧯️raises-the-fire-protection-to-r90`.
//!
//! EN 1993-1-2's whole fire facet moves together: the required rating goes r60→r90, the protection thickness 20→32 mm, section-factor massivity 150→175 1/m, the load level mu0 0.5→0.625 and the design temperature 550→620 degC. `fire_rating` is one of only four `Option<String>` fields in this diff.
//!
...
fn applied() -> En1993Snapshot {
    ...
    <En1993Diff as protocol::MutationDiff<En1993Snapshot>>::apply(raised.diff(), &base).expect("update-fire-inputs applies to its committed before-snapshot")
}
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "update-fire-inputs/raises-the-fire-protection-to-r90: applied state differs from committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    assert!(!inverse.is_empty(), "update-fire-inputs/raises-the-fire-protection-to-r90: this mutation changes state, so its inverse must not be empty");
    ...
    assert_eq!(snapshot, base, "update-fire-inputs/raises-the-fire-protection-to-r90: inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "update-fire-inputs/raises-the-fire-protection-to-r90: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "update-fire-inputs/raises-the-fire-protection-to-r90: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome — status AND every diagnostic `update-fire-inputs`'s own diff builder raises —
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "update-fire-inputs/raises-the-fire-protection-to-r90: raised diagnostics differ from the committed 🎯️outcome messages");
    let snapshot = applied();
    match status {
        "applied" => assert_ne!(snapshot, before(), "update-fire-inputs/raises-the-fire-protection-to-r90: declared applied but the snapshot came back unchanged"),
        "rejected" => assert_eq!(snapshot, before(), "update-fire-inputs/raises-the-fire-protection-to-r90: a rejected mutation must leave the snapshot untouched"),
        other => panic!("update-fire-inputs/raises-the-fire-protection-to-r90: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH fields `update-fire-inputs` is allowed to
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "update-fire-inputs/raises-the-fire-protection-to-r90: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "update-fire-inputs/raises-the-fire-protection-to-r90: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `update-fire-inputs` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "update-fire-inputs/raises-the-fire-protection-to-r90: committed diff did not carry before to after");
}
