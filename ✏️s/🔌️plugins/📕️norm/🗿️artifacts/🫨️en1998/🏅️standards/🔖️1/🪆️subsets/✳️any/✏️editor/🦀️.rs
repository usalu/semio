...
pub(crate) mod testkit {
    ...
    pub fn en1998_manifest_for_testkit() -> semio_framework_plugin::App {
        semio_framework_plugin::App { definition: create_en1998_app(), examples: Vec::new() }
    }
    ...
    pub async fn app_with_registry() -> NormApp {
        new_app_with_registry::<EditorApp<En1998PlayApp>>(en1998_manifest_for_testkit).await
    }

    pub async fn dispatch(app: &mut NormApp, command: En1998Command) -> InvocationResult {
...
mod tests {
    ...
    /// ð¯ï¸ One value per `En1998Command` row â the whole-command-surface laws below iterate it, so a new row
    /// that is not listed here fails `command_ids_cover_every_row`.
    fn every_command() -> Vec<En1998Command> {
        vec![
            En1998Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1998Snapshot::default() }),
            En1998Command::Evaluate(evaluate::Evaluate {}),
            En1998Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) }),
        ]
    }
    ...
    async fn optional_field_rows_keep_their_pre_migration_bytes() {
        ...
        assert_eq!(hex(&En1998Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) })), "01020001000402");
        assert_eq!(hex(&En1998Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: None })), "01020000");
    }
    ...
    async fn an_unknown_body_key_falls_back_to_a_text_node() {
        ...
        assert!(testkit::render(&mut app, "norm.en1998.play.nope").await.contains("Unknown body"));
    }
    ...
    async fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::app_with_registry().await;
        testkit::dispatch(&mut app, En1998Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1998Snapshot::default() })).await;
    ...
    async fn evaluate_recommits_the_current_projection_without_changing_it() {
        ...
        testkit::dispatch(&mut app, En1998Command::Evaluate(evaluate::Evaluate {})).await;
    ...
    async fn selected_check_index_is_a_config_only_edit() {
        let mut app = testkit::app_with_registry().await;
        let before = app.snapshot().expect("projection");
        let result = testkit::dispatch(&mut app, En1998Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) })).await;
    ...
    async fn view_actions_never_emit_artifact_mutations_under_the_real_registry() {
        let mut app = testkit::app_with_registry().await;
        let result = testkit::dispatch(&mut app, En1998Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(1) })).await;
