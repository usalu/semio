//! 🧪️ `change-foundation-h-rd-kn` fixture — `🧲️raises-foundation-h-rd-kn-to-475-0`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below was transcribed from this
//! leaf's own `../../🔺️diff/🦀️.rs`, which writes `En1998Diff.foundation_h_rd_kn` and nothing else,
...
async fn change_foundation_h_rd_kn_applies_to_committed_after() {
...
#[semio_framework_async_macros::async_test]
async fn change_foundation_h_rd_kn_inverse_restores_before() {
    let base = before();
...
#[semio_framework_async_macros::async_test]
async fn change_foundation_h_rd_kn_committed_json_is_canonical() {
...
#[semio_framework_async_macros::async_test]
async fn change_foundation_h_rd_kn_declared_outcome_holds() {
...
#[semio_framework_async_macros::async_test]
async fn change_foundation_h_rd_kn_produces_committed_diff() {
    let outcome = <En1998Mutation as protocol::Mutation<En1998Snapshot>>::diff(&mutation(), &before());
    ...
    assert_eq!(produced, committed, "change-foundation-h-rd-kn/raises-foundation-h-rd-kn-to-475-0: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
#[semio_framework_async_macros::async_test]
async fn change_foundation_h_rd_kn_committed_diff_is_canonical() {
    ...
    assert!(serde_json::to_value(&decoded).expect("diff JSON").get("selectedCheckIndex").is_none(), "change-foundation-h-rd-kn/raises-foundation-h-rd-kn-to-475-0: the committed diff must leave the presence-lane selectedCheckIndex absent");
...
#[semio_framework_async_macros::async_test]
async fn change_foundation_h_rd_kn_committed_diff_applies_to_after() {
    let decoded: En1998Diff = serde_json::from_str(DIFF).expect("change-foundation-h-rd-kn committed diff decodes");
