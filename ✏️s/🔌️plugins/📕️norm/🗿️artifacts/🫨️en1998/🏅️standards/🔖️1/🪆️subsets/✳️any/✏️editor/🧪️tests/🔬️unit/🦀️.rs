pub(crate) mod context {
    ...
    pub fn en1998_manifest_for_tests() -> semio_framework_plugin::App {
    ...
    pub async fn app_with_registry() -> NormApp {
        let mut app = new_app_with_registry::<EditorApp<En1998PlayApp>>(en1998_manifest_for_tests).await;
...
fn every_command() -> Vec<En1998Command> {
    vec![
        ...
        En1998Command::SetActiveExample(set_active_example::SetActiveExample { example_id: String::new() }),
    ]
}
...
async fn an_unknown_body_key_falls_back_to_a_text_node() {
    ...
    assert!(context::render(&mut app, "norm.en1998.play.nope").await.contains("Unknown body"));
...
async fn set_snapshot_commits_a_host_backed_report() {
    let mut app = context::app_with_registry().await;
    context::dispatch(&mut app, En1998Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1998Snapshot::default() })).await;
    context::dispatch(&mut app, En1998Command::Evaluate(evaluate::Evaluate {})).await;
    let host = NormHost::<En1998Family>::from_artifact(app.snapshot().expect("projection"));
...
async fn selected_check_index_is_a_config_only_edit() {
    let mut app = context::app_with_registry().await;
    let before = app.snapshot().expect("projection");
    let result = context::dispatch(&mut app, En1998Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) })).await;
...
async fn view_actions_never_emit_artifact_mutations_under_the_real_registry() {
    let mut app = context::app_with_registry().await;
    let result = context::dispatch(&mut app, En1998Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(1) })).await;
...
async fn report_out_exports_the_computed_check_report() {
    ...
    let report: crate::document::CheckReport = pack::json::from_json_str(&json).expect("report json parses via ToValue/FromValue");
