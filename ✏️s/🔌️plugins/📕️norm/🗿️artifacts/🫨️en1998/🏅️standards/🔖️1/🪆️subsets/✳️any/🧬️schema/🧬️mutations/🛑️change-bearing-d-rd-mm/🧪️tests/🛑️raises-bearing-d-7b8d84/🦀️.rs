...
async fn change_bearing_d_rd_mm_committed_diff_is_canonical() {
    ...
    assert!(serde_json::to_value(&decoded).expect("diff JSON").get("selectedCheckIndex").is_none(), "change-bearing-d-rd-mm/raises-bearing-d-rd-mm-to-312-5: the committed diff must leave the presence-lane selectedCheckIndex absent");
