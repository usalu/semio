//! 🧪️ `change-silo-radius-m` fixture — `⭕️raises-silo-radius-m-to-6-25`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⭕️change-silo-radius-m/⭕️raises-silo-radius-m-to-6-25/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⭕️change-silo-radius-m/⭕️raises-silo-radius-m-to-6-25/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⭕️change-silo-radius-m/⭕️raises-silo-radius-m-to-6-25/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⭕️change-silo-radius-m/⭕️raises-silo-radius-m-to-6-25/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⭕️change-silo-radius-m/⭕️raises-silo-radius-m-to-6-25/🎯️outcome/🔣️.json");
...
async fn change_silo_radius_m_applies_to_committed_after() {
...
#[semio_framework_async_macros::async_test]
async fn change_silo_radius_m_inverse_restores_before() {
    let base = before();
...
#[semio_framework_async_macros::async_test]
async fn change_silo_radius_m_committed_json_is_canonical() {
    ...
    assert_eq!(reencoded, original, "change-silo-radius-m/raises-silo-radius-m-to-6-25: committed mutation JSON is not the canonical internally-tagged ChangeSiloRadiusM form carrying mutation and newSiloRadiusM");
}

/// 🎯️ The declared outcome holds: `change-silo-radius-m` at 6.25 is applied, not rejected, and carries no
/// diagnostic of its own.
#[semio_framework_async_macros::async_test]
async fn change_silo_radius_m_declared_outcome_holds() {
...
#[semio_framework_async_macros::async_test]
async fn change_silo_radius_m_produces_committed_diff() {
    let outcome = <En1998Mutation as protocol::Mutation<En1998Snapshot>>::diff(&mutation(), &before());
...
#[semio_framework_async_macros::async_test]
async fn change_silo_radius_m_committed_diff_is_canonical() {
    ...
    assert!(serde_json::to_value(&decoded).expect("diff JSON").get("selectedCheckIndex").is_none(), "change-silo-radius-m/raises-silo-radius-m-to-6-25: the committed diff must leave the presence-lane selectedCheckIndex absent");
...
#[semio_framework_async_macros::async_test]
async fn change_silo_radius_m_committed_diff_applies_to_after() {
    let decoded: En1998Diff = serde_json::from_str(DIFF).expect("change-silo-radius-m committed diff decodes");
