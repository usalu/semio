//! 🧪️ `change-t1-s` fixture — `🕐️raises-t1-s-to-0-75`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕐️change-t1-s/🕐️raises-t1-s-to-0-75/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕐️change-t1-s/🕐️raises-t1-s-to-0-75/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕐️change-t1-s/🕐️raises-t1-s-to-0-75/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕐️change-t1-s/🕐️raises-t1-s-to-0-75/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕐️change-t1-s/🕐️raises-t1-s-to-0-75/🎯️outcome/🔣️.json");
...
async fn change_t1_s_applies_to_committed_after() {
...
#[semio_framework_async_macros::async_test]
async fn change_t1_s_inverse_restores_before() {
    let base = before();
...
#[semio_framework_async_macros::async_test]
async fn change_t1_s_committed_json_is_canonical() {
    ...
    assert_eq!(reencoded, original, "change-t1-s/raises-t1-s-to-0-75: committed mutation JSON is not the canonical internally-tagged ChangeT1S form carrying mutation and newT1S");
}

/// 🎯️ The declared outcome holds: `change-t1-s` at 0.75 is applied, not rejected, and carries no
/// diagnostic of its own.
#[semio_framework_async_macros::async_test]
async fn change_t1_s_declared_outcome_holds() {
...
#[semio_framework_async_macros::async_test]
async fn change_t1_s_produces_committed_diff() {
    let outcome = <En1998Mutation as protocol::Mutation<En1998Snapshot>>::diff(&mutation(), &before());
...
#[semio_framework_async_macros::async_test]
async fn change_t1_s_committed_diff_is_canonical() {
    ...
    assert!(serde_json::to_value(&decoded).expect("diff JSON").get("selectedCheckIndex").is_none(), "change-t1-s/raises-t1-s-to-0-75: the committed diff must leave the presence-lane selectedCheckIndex absent");
...
#[semio_framework_async_macros::async_test]
async fn change_t1_s_committed_diff_applies_to_after() {
    let decoded: En1998Diff = serde_json::from_str(DIFF).expect("change-t1-s committed diff decodes");
