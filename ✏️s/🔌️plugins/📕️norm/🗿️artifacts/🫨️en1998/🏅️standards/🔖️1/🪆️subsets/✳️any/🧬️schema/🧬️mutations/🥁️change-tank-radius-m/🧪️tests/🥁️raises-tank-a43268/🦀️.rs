//! 🧪️ `change-tank-radius-m` fixture — `🥁️raises-tank-radius-m-to-5-75`.
//!
...
async fn change_tank_radius_m_committed_diff_is_canonical() {
    ...
    assert!(serde_json::to_value(&decoded).expect("diff JSON").get("selectedCheckIndex").is_none(), "change-tank-radius-m/raises-tank-radius-m-to-5-75: the committed diff must leave the presence-lane selectedCheckIndex absent");
