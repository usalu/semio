//! 🧪️ `change-wall-h-rd-kn` fixture — `🏋️raises-wall-h-rd-kn-to-187-5`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below was transcribed from this
//! leaf's own `../../🔺️diff/🦀️.rs`, which writes `En1998Diff.wall_h_rd_kn` and nothing else,
...
async fn change_wall_h_rd_kn_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-wall-h-rd-kn/raises-wall-h-rd-kn-to-187-5: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn change_wall_h_rd_kn_committed_diff_is_canonical() {
    ...
    assert!(serde_json::to_value(&decoded).expect("diff JSON").get("selectedCheckIndex").is_none(), "change-wall-h-rd-kn/raises-wall-h-rd-kn-to-187-5: the committed diff must leave the presence-lane selectedCheckIndex absent");
