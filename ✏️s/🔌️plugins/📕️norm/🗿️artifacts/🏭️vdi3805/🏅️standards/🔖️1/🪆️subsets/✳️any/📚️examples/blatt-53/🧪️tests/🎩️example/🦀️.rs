...
async fn primary_asset_decodes_and_complies() {
    ...
    let text = include_str!("../../../../🖼️assets/blatt-53/🗣️.dsl.semio");
    let decoded = decode_vdi3805_dsl(text).expect("blatt-53 dsl");
    assert_eq!(decoded.catalog.products[0].sheet.0, 53);
    assert!(evaluate(&decoded).complies(), "blatt-53 must comply");
    let expected = conforming_blatt_dataset(53);
