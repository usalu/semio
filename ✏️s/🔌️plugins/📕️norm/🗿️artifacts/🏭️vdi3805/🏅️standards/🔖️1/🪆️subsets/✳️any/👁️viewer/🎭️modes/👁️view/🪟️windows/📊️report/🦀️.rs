//! 📊️ VDI 3805 viewer — the Report window: a read-only table of every computed compliance
...
//#region 🔖️Definition
/// 🧱️ Stitched into the viewer manifest by `crate::viewer::vdi3805::create_vdi3805_viewer`. Read-only variant
...
//#region 🔖️Render
/// 👁️ Pure `Vdi3805Snapshot -> UiNode` read: recomputes the compliance report straight off the document
/// (the same pure inference the editor's results window renders through `NormHost`), then tables it.
pub fn render(document: &Vdi3805Snapshot, locale: semio_framework_plugin::Locale) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::BuiltNode> {
