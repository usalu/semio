...
async fn primary_asset_decodes_and_complies() {
    ...
    let text = include_str!("../../../../🖼️assets/blatt-6/🗣️.dsl.semio");
    let decoded = decode_vdi3805_dsl(text).expect("blatt-6 dsl");
    assert_eq!(decoded.catalog.products[0].sheet.0, 6);
    assert!(evaluate(&decoded).complies(), "blatt-6 must comply");
    let expected = conforming_blatt_dataset(6);
