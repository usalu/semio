...
async fn primary_asset_decodes_and_complies() {
    ...
    let text = include_str!("../../../../🖼️assets/blatt-5/🗣️.dsl.semio");
    let decoded = decode_vdi3805_dsl(text).expect("blatt-5 dsl");
    assert_eq!(decoded.catalog.products[0].sheet.0, 5);
    assert!(evaluate(&decoded).complies(), "blatt-5 must comply");
    let expected = conforming_blatt_dataset(5);
