//! 🧪️ `update-manufacturer-file` fixture — `✏️renames-the-header-manufacturer-to-acme`.
//!
//! The whole `010` header facet is swapped atomically: `manufacturer` becomes `ACME` and `record_count` becomes 3. `catalog.file` is a SEPARATE copy of the header and is deliberately left alone — the diff builder writes `manufacturer_file` only.
//!
...
fn applied() -> Vdi3805Snapshot {
    ...
    <Vdi3805Diff as protocol::MutationDiff<Vdi3805Snapshot>>::apply(raised.diff(), &base).expect("update-manufacturer-file applies to its committed before-snapshot")
}
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "update-manufacturer-file/renames-the-header-manufacturer-to-acme: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert!(!inverse.is_empty(), "update-manufacturer-file/renames-the-header-manufacturer-to-acme: this mutation changes state, so its inverse must not be empty");
    ...
    assert_eq!(snapshot, base, "update-manufacturer-file/renames-the-header-manufacturer-to-acme: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "update-manufacturer-file/renames-the-header-manufacturer-to-acme: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "update-manufacturer-file/renames-the-header-manufacturer-to-acme: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome — status AND every diagnostic `update-manufacturer-file`'s own diff builder raises —
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "update-manufacturer-file/renames-the-header-manufacturer-to-acme: raised diagnostics differ from the committed 🎯️outcome messages");
    let snapshot = applied();
    match status {
        "applied" => assert_ne!(snapshot, before(), "update-manufacturer-file/renames-the-header-manufacturer-to-acme: declared applied but the snapshot came back unchanged"),
        "rejected" => assert_eq!(snapshot, before(), "update-manufacturer-file/renames-the-header-manufacturer-to-acme: a rejected mutation must leave the snapshot untouched"),
        other => panic!("update-manufacturer-file/renames-the-header-manufacturer-to-acme: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH fields `update-manufacturer-file` is allowed to
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "update-manufacturer-file/renames-the-header-manufacturer-to-acme: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "update-manufacturer-file/renames-the-header-manufacturer-to-acme: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `update-manufacturer-file` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "update-manufacturer-file/renames-the-header-manufacturer-to-acme: committed diff did not carry before to after");
}
