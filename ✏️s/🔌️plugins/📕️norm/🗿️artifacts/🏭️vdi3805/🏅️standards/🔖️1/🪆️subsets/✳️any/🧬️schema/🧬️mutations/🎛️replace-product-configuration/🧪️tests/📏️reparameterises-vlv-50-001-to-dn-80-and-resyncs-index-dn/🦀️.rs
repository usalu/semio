//! 🧪️ `replace-product-configuration` fixture — `📏️reparameterises-vlv-50-001-to-dn-80-and-resyncs-index-dn`.
//!
//! The product's whole configuration block is swapped (dn 50→80, kvs 4.5→8.0 m3/h) and `extract_dn` recomputes the persisted index entry's `dn` from the NEW parameter bag. `tags` are not touched — only `dn` is derived from the configuration.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🎛️replace-product-configuration/📏️reparameterises-vlv-50-001-to-dn-80-and-resyncs-index-dn/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🎛️replace-product-configuration/📏️reparameterises-vlv-50-001-to-dn-80-and-resyncs-index-dn/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🎛️replace-product-configuration/📏️reparameterises-vlv-50-001-to-dn-80-and-resyncs-index-dn/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🎛️replace-product-configuration/📏️reparameterises-vlv-50-001-to-dn-80-and-resyncs-index-dn/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🎛️replace-product-configuration/📏️reparameterises-vlv-50-001-to-dn-80-and-resyncs-index-dn/🎯️outcome/🔣️.json");
...
fn applied() -> Vdi3805Snapshot {
    ...
    <Vdi3805Diff as protocol::MutationDiff<Vdi3805Snapshot>>::apply(raised.diff(), &base).expect("replace-product-configuration applies to its committed before-snapshot")
}
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "replace-product-configuration/reparameterises-vlv-50-001-to-dn-80-and-resyncs-index-dn: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert!(!inverse.is_empty(), "replace-product-configuration/reparameterises-vlv-50-001-to-dn-80-and-resyncs-index-dn: this mutation changes state, so its inverse must not be empty");
    ...
    assert_eq!(snapshot, base, "replace-product-configuration/reparameterises-vlv-50-001-to-dn-80-and-resyncs-index-dn: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-product-configuration/reparameterises-vlv-50-001-to-dn-80-and-resyncs-index-dn: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "replace-product-configuration/reparameterises-vlv-50-001-to-dn-80-and-resyncs-index-dn: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "replace-product-configuration/reparameterises-vlv-50-001-to-dn-80-and-resyncs-index-dn: raised diagnostics differ from the committed 🎯️outcome messages");
    let snapshot = applied();
    match status {
        "applied" => assert_ne!(snapshot, before(), "replace-product-configuration/reparameterises-vlv-50-001-to-dn-80-and-resyncs-index-dn: declared applied but the snapshot came back unchanged"),
        "rejected" => assert_eq!(snapshot, before(), "replace-product-configuration/reparameterises-vlv-50-001-to-dn-80-and-resyncs-index-dn: a rejected mutation must leave the snapshot untouched"),
        other => panic!("replace-product-configuration/reparameterises-vlv-50-001-to-dn-80-and-resyncs-index-dn: unknown outcome status {other:?}"),
    }
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-product-configuration/reparameterises-vlv-50-001-to-dn-80-and-resyncs-index-dn: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "replace-product-configuration/reparameterises-vlv-50-001-to-dn-80-and-resyncs-index-dn: committed diff JSON is not canonical");
}
