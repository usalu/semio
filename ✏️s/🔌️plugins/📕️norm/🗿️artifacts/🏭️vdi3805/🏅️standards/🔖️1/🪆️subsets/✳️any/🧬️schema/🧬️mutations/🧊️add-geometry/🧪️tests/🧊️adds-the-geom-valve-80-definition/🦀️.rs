...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧊️add-geometry/🧊️adds-the-geom-valve-80-definition/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧊️add-geometry/🧊️adds-the-geom-valve-80-definition/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧊️add-geometry/🧊️adds-the-geom-valve-80-definition/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧊️add-geometry/🧊️adds-the-geom-valve-80-definition/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧊️add-geometry/🧊️adds-the-geom-valve-80-definition/🎯️outcome/🔣️.json");
...
async fn inverse_restores_before() {
    ...
    assert!(!inverse.is_empty(), "add-geometry/adds-the-geom-valve-80-definition: this mutation changes state, so its inverse must not be empty");
    ...
    assert_eq!(snapshot, base, "add-geometry/adds-the-geom-valve-80-definition: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "add-geometry/adds-the-geom-valve-80-definition: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "add-geometry/adds-the-geom-valve-80-definition: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome — status AND every diagnostic `add-geometry`'s own diff builder raises —
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "add-geometry/adds-the-geom-valve-80-definition: raised diagnostics differ from the committed 🎯️outcome messages");
    let snapshot = applied();
    match status {
        "applied" => assert_ne!(snapshot, before(), "add-geometry/adds-the-geom-valve-80-definition: declared applied but the snapshot came back unchanged"),
        "rejected" => assert_eq!(snapshot, before(), "add-geometry/adds-the-geom-valve-80-definition: a rejected mutation must leave the snapshot untouched"),
        other => panic!("add-geometry/adds-the-geom-valve-80-definition: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH fields `add-geometry` is allowed to
/// touch, not merely that the end state matches.
#[semio_framework_async_macros::async_test]
async fn produces_committed_diff() {
    let raised = <Vdi3805Mutation as protocol::Mutation<Vdi3805Snapshot>>::diff(&mutation(), &before());
    let raised_diff = raised.diff();
    assert_eq!(raised_diff.geometry.as_ref().map(|map| map.len()), Some(2), "add-geometry/adds-the-geom-valve-80-definition: the diff republishes the whole geometry map, now holding two definitions");
    assert!(raised_diff.catalog.is_none(), "add-geometry/adds-the-geom-valve-80-definition: creating geometry does not touch the product catalogue that may later reference it");
    let produced = serde_json::to_value(raised_diff).expect("produced diff encodes");
    let committed: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff decodes");
    assert_eq!(produced, committed, "add-geometry/adds-the-geom-valve-80-definition: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "add-geometry/adds-the-geom-valve-80-definition: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `add-geometry` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "add-geometry/adds-the-geom-valve-80-definition: committed diff did not carry before to after");
}
