...
async fn primary_asset_decodes_and_fails() {
    ...
    let text = include_str!("../../../../🖼️assets/blatt-6-fail/🗣️.dsl.semio");
    let decoded = decode_vdi3805_dsl(text).expect("blatt-6-fail dsl");
    assert_eq!(decoded.catalog.products[0].sheet.0, 6);
    let report = evaluate(&decoded);
    assert!(report.failing().count() >= 2, "blatt-6-fail expected ≥2 fails");
