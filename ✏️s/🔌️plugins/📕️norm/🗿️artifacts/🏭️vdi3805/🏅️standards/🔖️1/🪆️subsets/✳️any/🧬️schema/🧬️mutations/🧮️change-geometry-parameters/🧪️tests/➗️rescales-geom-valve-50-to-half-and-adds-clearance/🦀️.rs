//! 🧪️ `change-geometry-parameters` fixture — `➗️rescales-geom-valve-50-to-half-and-adds-clearance`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧮️change-geometry-parameters/➗️rescales-geom-valve-50-to-half-and-adds-clearance/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧮️change-geometry-parameters/➗️rescales-geom-valve-50-to-half-and-adds-clearance/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧮️change-geometry-parameters/➗️rescales-geom-valve-50-to-half-and-adds-clearance/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧮️change-geometry-parameters/➗️rescales-geom-valve-50-to-half-and-adds-clearance/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧮️change-geometry-parameters/➗️rescales-geom-valve-50-to-half-and-adds-clearance/🎯️outcome/🔣️.json");
...
async fn inverse_restores_before() {
    ...
    assert!(!inverse.is_empty(), "change-geometry-parameters/rescales-geom-valve-50-to-half-and-adds-clearance: this mutation changes state, so its inverse must not be empty");
    ...
    assert_eq!(snapshot, base, "change-geometry-parameters/rescales-geom-valve-50-to-half-and-adds-clearance: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-geometry-parameters/rescales-geom-valve-50-to-half-and-adds-clearance: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "change-geometry-parameters/rescales-geom-valve-50-to-half-and-adds-clearance: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome — status AND every diagnostic `change-geometry-parameters`'s own diff builder raises —
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "change-geometry-parameters/rescales-geom-valve-50-to-half-and-adds-clearance: raised diagnostics differ from the committed 🎯️outcome messages");
    let snapshot = applied();
    match status {
        "applied" => assert_ne!(snapshot, before(), "change-geometry-parameters/rescales-geom-valve-50-to-half-and-adds-clearance: declared applied but the snapshot came back unchanged"),
        "rejected" => assert_eq!(snapshot, before(), "change-geometry-parameters/rescales-geom-valve-50-to-half-and-adds-clearance: a rejected mutation must leave the snapshot untouched"),
        other => panic!("change-geometry-parameters/rescales-geom-valve-50-to-half-and-adds-clearance: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH fields `change-geometry-parameters` is allowed to
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-geometry-parameters/rescales-geom-valve-50-to-half-and-adds-clearance: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "change-geometry-parameters/rescales-geom-valve-50-to-half-and-adds-clearance: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `change-geometry-parameters` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "change-geometry-parameters/rescales-geom-valve-50-to-half-and-adds-clearance: committed diff did not carry before to after");
}
