...
//! directory name stays `🟤️set-snapshot` while its content becomes `ChangeAnnex` — see the
...
impl protocol::MutationKind<Din16798Snapshot, Din16798Mutation> for ChangeAnnex {
    ...
    fn label(&self) -> protocol::LocalizedLabel {
        protocol::LocalizedLabel::native(&format!("Change national annex to {}", self.new_annex.label()), &format!("Nationalen Anhang auf {} ändern", self.new_annex.label()))
    }
}
