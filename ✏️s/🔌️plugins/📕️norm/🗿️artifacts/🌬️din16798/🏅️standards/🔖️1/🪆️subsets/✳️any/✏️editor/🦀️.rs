...
use crate::editor::din16798::commands::{apply_remedy, evaluate, insert_item, remove_item, selected_check, set_active_example, set_field, set_snapshot};
...
pub const CONTROLLER_ID: &str = "s.norm.din16798@1/*#editor";
...
semio_framework_plugin::app_commands! {
    ...
    pub enum Din16798Command for Din16798Snapshot, Din16798Mutation, NoConfig, NoConfigMutation {
...
impl ArtifactEditor for Din16798PlayApp {
    ...
    fn examples() -> Vec<semio_framework_plugin::ExampleSource> {
        vec![crate::examples::compliant_office::source(), crate::examples::noncompliant_office::source(), crate::examples::residential_method3::source(), crate::examples::demo::source()]
    }
    ...
    semio_framework_plugin::bounded_first_step_tool_proofs! {
        ...
        artifact_schema: "semio.norm.din16798/v1",
    ...
    semio_s_artifact_norm_contract::norm_command_from_action!(Din16798Command, crate::standards::v1::subsets::any::schema::snapshot::decode_din16798_snapshot_json);
    ...
    ) -> Result<Emit<Din16798Mutation, NoConfigMutation, Self::DraftMutation>, Fault> {
        crate::app_surface::dispatch_norm_command::<Self>(command, doc, cfg, view_state)
    }

    fn render(body_key: &str, doc: &ArtifactView<'_, Din16798Snapshot>, cfg: &ConfigView<'_, NoConfig>, view_state: &semio_framework_plugin::ViewModel) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::ComponentTree> {
        let mut host = NormHost::<DinEn16798Family>::from_artifact(doc.snapshot.clone());
        if matches!(body_key, results::BODY_RESULTS | document_panel::BODY_ARTIFACT | inspection_panel::BODY_INSPECTION) && host.report().checks.is_empty() {
            host.evaluate();
        }
        match body_key {
    ...
    fn import_media(port: &str, media: &Media, doc: &ArtifactView<'_, Din16798Snapshot>) -> Result<Emit<Din16798Mutation, NoConfigMutation, Self::DraftMutation>, MediaError> {
        let base = doc.snapshot.clone();
        crate::app_surface::import_media(port, media, move |snapshot: Din16798Snapshot| Din16798Mutation::from_snapshot(&base, &snapshot))
    }
...
impl crate::app_surface::NormRetainedEditor for Din16798PlayApp {
    type Family = DinEn16798Family;

    type ResultsWindowConfigOwner = results::ResultsWindowConfigOwner;

    fn selected_check_window_mutation(command: &Din16798Command) -> Option<crate::results_window_config::NormResultsWindowConfigMutation> {
        match command {
            Din16798Command::SetSelectedCheckIndex(payload) => Some(selected_check::window_mutation(payload)),
            _ => None,
        }
    }

    fn dispatch_retained(command: &Din16798Command, doc: &ArtifactView<'_, Din16798Snapshot>, cfg: &ConfigView<'_, NoConfig>) -> Result<Emit<Din16798Mutation, NoConfigMutation, NoDraftMutation>, Fault> {
...
pub fn create_din16798_app() -> semio_framework_plugin::AppDefinition {
    Editor::builder(crate::DIN16798_DIALECT)
            ...
            .action_describe("setSnapshot", LocalizedLabel::native("Replaces the whole DIN EN 16798 (indoor environment and ventilation) compliance document with the supplied document JSON; the previous inputs are discarded.", "Ersetzt das gesamte Nachweisdokument nach DIN EN 16798 (Innenraumklima und Lüftung) durch das übergebene Dokument-JSON; die bisherigen Eingaben werden verworfen."))
            .action_describe("evaluate", LocalizedLabel::native("Recomputes every DIN EN 16798 (indoor environment and ventilation) check from the document's inputs and refreshes the results window; the document is not changed.", "Berechnet alle Nachweise nach DIN EN 16798 (Innenraumklima und Lüftung) aus den Eingaben des Dokuments neu und aktualisiert das Ergebnisfenster; das Dokument ändert sich nicht."))
            ...
            .action_describe("setActiveExample", LocalizedLabel::native("Loads one of the bundled DIN EN 16798 (indoor environment and ventilation) examples into the open compliance document, replacing its inputs, by example id.", "Lädt eines der mitgelieferten Beispiele nach DIN EN 16798 (Innenraumklima und Lüftung) in das offene Nachweisdokument und ersetzt dessen Eingaben, anhand der Beispiel-Id."))
            .build_definition()
}
