//! 🧪️ `change-liquid-rho-p-eff` fixture — `raises-liquid-rho-p-eff-to-0-0078125`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below was transcribed from this
//! leaf's own `../../🔺️diff/🦀️component.rs`, which writes `En1992Diff.liquid_rho_p_eff` and nothing else,
...
async fn change_liquid_rho_p_eff_applies_to_committed_after() {
...
#[semio_framework_async_macros::async_test]
async fn change_liquid_rho_p_eff_inverse_restores_before() {
    let base = before();
...
#[semio_framework_async_macros::async_test]
async fn change_liquid_rho_p_eff_committed_json_is_canonical() {
...
#[semio_framework_async_macros::async_test]
async fn change_liquid_rho_p_eff_declared_outcome_holds() {
...
#[semio_framework_async_macros::async_test]
async fn change_liquid_rho_p_eff_produces_committed_diff() {
    let outcome = <En1992Mutation as protocol::Mutation<En1992Snapshot>>::diff(&mutation(), &before());
    ...
    assert_eq!(produced, committed, "change-liquid-rho-p-eff/raises-liquid-rho-p-eff-to-0-0078125: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
#[semio_framework_async_macros::async_test]
async fn change_liquid_rho_p_eff_committed_diff_is_canonical() {
...
#[semio_framework_async_macros::async_test]
async fn change_liquid_rho_p_eff_committed_diff_applies_to_after() {
    let decoded: En1992Diff = serde_json::from_str(DIFF).expect("change-liquid-rho-p-eff committed diff decodes");
