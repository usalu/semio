//! 📊️ EN 1992 viewer — the Report window: a read-only table of every computed compliance
...
//#region 🔖️Definition
/// 🧱️ Stitched into the viewer manifest by `crate::viewer::en1992::create_en1992_viewer`. Read-only variant
...
//#region 🔖️Render
/// 👁️ Pure `En1992Snapshot -> UiNode` read: recomputes the compliance report straight off the document
/// (the same pure inference the editor's results window renders through `NormHost`), then tables it.
pub async fn render(document: &En1992Snapshot) -> UiNode {
    let report = crate::artifacts::en1992::standards::v1::subsets::any::schema::inferences::evaluate(document);
