...
//! leaf's own `../../🔺️diff/🦀️component.rs`, which writes `En1992Diff.a_c_mm2` and nothing else,
...
async fn change_ac_mm2_applies_to_committed_after() {
...
#[semio_framework_async_macros::async_test]
async fn change_ac_mm2_inverse_restores_before() {
    let base = before();
...
#[semio_framework_async_macros::async_test]
async fn change_ac_mm2_committed_json_is_canonical() {
...
#[semio_framework_async_macros::async_test]
async fn change_ac_mm2_declared_outcome_holds() {
...
#[semio_framework_async_macros::async_test]
async fn change_ac_mm2_produces_committed_diff() {
    let outcome = <En1992Mutation as protocol::Mutation<En1992Snapshot>>::diff(&mutation(), &before());
    ...
    assert_eq!(produced, committed, "change-ac-mm2/raises-a-c-mm2-to-168750-0: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
#[semio_framework_async_macros::async_test]
async fn change_ac_mm2_committed_diff_is_canonical() {
...
#[semio_framework_async_macros::async_test]
async fn change_ac_mm2_committed_diff_applies_to_after() {
    let decoded: En1992Diff = serde_json::from_str(DIFF).expect("change-ac-mm2 committed diff decodes");
