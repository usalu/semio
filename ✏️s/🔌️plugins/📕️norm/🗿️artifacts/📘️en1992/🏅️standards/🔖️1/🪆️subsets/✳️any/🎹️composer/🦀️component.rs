//! 🎹️ En1992Composer (1/✳️any) — analyzer + builder glued. Reads native `s.en1992` sources
//! plus any of: stdio.csv, stdio.json, stdio.txt, stdio.xlsx, stdio.zip. Writes one `s.en1992` (1/✳️any) snapshot.
    ...
    fn compose(sources: &[ComposeSource]) -> Result<Composition<Self::Snapshot>, ComposeError> {
        for source in sources {
            ...
            if source.dialect == DEP_CSV {
                ...
                if let Ok(snapshot) = crate::artifacts::en1992::io::import::deserializers::artifacts::csv::v_rfc4180::any::deserialize_bytes(&bytes) {
            ...
            if source.dialect == DEP_JSON {
                ...
                if let Ok(snapshot) = crate::artifacts::en1992::io::import::deserializers::artifacts::json::v_rfc8259::any::deserialize_bytes(&bytes) {
                    return Ok(Composition { snapshot, confidence: semio_framework_plugin::IoConfidence::Medium, diagnostics: Vec::new() });
                }
            }
            if source.dialect == DEP_TXT {
                ...
                if let Ok(snapshot) = crate::artifacts::en1992::io::import::deserializers::artifacts::txt::v_utf_8::any::deserialize_bytes(&bytes) {
                    return Ok(Composition { snapshot, confidence: semio_framework_plugin::IoConfidence::Medium, diagnostics: Vec::new() });
                }
            }
            if source.dialect == DEP_XLSX {
                ...
                if let Ok(snapshot) = crate::artifacts::en1992::io::import::deserializers::artifacts::xlsx::v_ecma_376::any::deserialize_bytes(&bytes) {
                    return Ok(Composition { snapshot, confidence: semio_framework_plugin::IoConfidence::Medium, diagnostics: Vec::new() });
                }
            }
            if source.dialect == DEP_ZIP {
                ...
                if let Ok(snapshot) = crate::artifacts::en1992::io::import::deserializers::artifacts::zip::v2_0::any::deserialize_bytes(&bytes) {
                    return Ok(Composition { snapshot, confidence: semio_framework_plugin::IoConfidence::Medium, diagnostics: Vec::new() });
                }
            }
