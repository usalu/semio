...
//! leaf's own `../../🔺️diff/🦀️component.rs`, which writes `En1992Diff.annex` and nothing else,
...
//! ⚠️ This leaf still lives in the pre-migration `🐝set-snapshot/` directory: `📦️glue.rs` path-includes
//! that exact name, so the directory keeps it while its content is `ChangeAnnex`.
