//! 🧪️ `change-use-fem` fixture — `turns-use-fem-on`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below was transcribed from this
//! leaf's own `../../🔺️diff/🦀️component.rs`, which writes `En1992Diff.use_fem` and nothing else,
...
async fn change_use_fem_applies_to_committed_after() {
...
#[semio_framework_async_macros::async_test]
async fn change_use_fem_inverse_restores_before() {
    let base = before();
...
#[semio_framework_async_macros::async_test]
async fn change_use_fem_committed_json_is_canonical() {
...
#[semio_framework_async_macros::async_test]
async fn change_use_fem_declared_outcome_holds() {
...
#[semio_framework_async_macros::async_test]
async fn change_use_fem_produces_committed_diff() {
    let outcome = <En1992Mutation as protocol::Mutation<En1992Snapshot>>::diff(&mutation(), &before());
    ...
    assert_eq!(produced, committed, "change-use-fem/turns-use-fem-on: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
#[semio_framework_async_macros::async_test]
async fn change_use_fem_committed_diff_is_canonical() {
...
#[semio_framework_async_macros::async_test]
async fn change_use_fem_committed_diff_applies_to_after() {
    let decoded: En1992Diff = serde_json::from_str(DIFF).expect("change-use-fem committed diff decodes");
