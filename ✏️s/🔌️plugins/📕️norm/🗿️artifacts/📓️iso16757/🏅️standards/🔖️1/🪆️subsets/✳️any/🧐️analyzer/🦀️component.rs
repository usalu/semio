//! 🧐️ Iso16757Analyzer (1/✳️any) — read-only analysis, successor to the pre-migration
//! Iso16757Decomposer. Real logic; artifact/standard levels delegate here.
...
impl ArtifactAnalyzer for Iso16757Analyzer {
    ...
    fn analyze(sources: &[AnalyzeSource<'_>]) -> Analysis<Self::Parts> {
        ...
        for source in sources {
            match source {
                AnalyzeSource::Text(text) => match <Iso16757Snapshot as store::DocumentDsl>::parse_dsl(text) {
                ...
                AnalyzeSource::Binary(bytes) => match <Iso16757Snapshot as store::DocumentPack>::decode_pack(bytes) {
                    Ok(snapshot) => parts.snapshot = Some(snapshot),
                    Err(err) => {
                        confidence = IoConfidence::Low;
