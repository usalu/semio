//! 🧪️ `change-annex` fixture — `🌍️switches-national-annex-to-en` (EN 1999 aluminium).
//!
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(produced.n_ed_kn, base.n_ed_kn, "change-annex/switches-national-annex-to-en: `n_ed_kn` is not addressed by this mutation and must survive untouched");
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: En1999Snapshot = serde_json::from_str(text).expect("change-annex/switches-national-annex-to-en: snapshot decodes");
...
async fn produces_committed_diff() {
    ...
    assert!(outcome.diff().n_ed_kn.is_none(), "change-annex/switches-national-annex-to-en: the diff must leave `n_ed_kn` unset");
...
async fn committed_diff_is_canonical() {
    let decoded: En1999Diff = serde_json::from_str(DIFF).expect("change-annex/switches-national-annex-to-en: committed diff decodes");
