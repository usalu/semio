pub(crate) mod context {
    ...
    /// ✏️ Adapts `create_en1999_app`'s `AppDefinition` (contract §2.4) into the `App { definition,
...
fn retained_command_dispositions_match_the_language_neutral_oracle() {
    ...
    let definition = create_en1999_app();
...
/// ð¯ï¸ One value per `En1999Command` row â the whole-command-surface laws below iterate it, so a new row
/// that is not listed here fails `command_ids_cover_every_row`.
fn every_command() -> Vec<En1999Command> {
    vec![
        En1999Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1999Snapshot::default() }),
        En1999Command::Evaluate(evaluate::Evaluate {}),
        En1999Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) }),
        En1999Command::SetActiveExample(set_active_example::SetActiveExample { example_id: String::new() }),
    ]
}
...
async fn command_ids_cover_every_row_and_are_unique() {
    ...
    let ids: Vec<&str> = commands.iter().map(En1999Command::command_id).collect();
...
/// of the three payload shapes involves the per-standard `En1999Snapshot`.
#[semio_framework_async_macros::async_test]
async fn optional_field_rows_keep_their_pre_migration_bytes() {
    let hex = |command: &En1999Command| protocol::OpBinary::encode_op(command).expect("encode").iter().map(|byte| format!("{byte:02x}")).collect::<String>();
    assert_eq!(hex(&En1999Command::Evaluate(evaluate::Evaluate {})), "01010000");
    assert_eq!(hex(&En1999Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) })), "01020001000402");
    assert_eq!(hex(&En1999Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: None })), "01020000");
}
...
async fn declares_model_in_and_report_out_ports() {
    let ports = create_en1999_app().io.ports;
...
async fn an_unknown_body_key_falls_back_to_a_text_node() {
    ...
    assert!(context::render(&mut app, "norm.en1999.play.nope").await.contains("Unknown body"));
...
async fn set_snapshot_commits_a_host_backed_report() {
    let mut app = context::app_with_registry().await;
    context::dispatch(&mut app, En1999Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1999Snapshot::default() })).await;
    let host = NormHost::<En1999Family>::from_artifact(app.snapshot().expect("projection"));
...
async fn evaluate_recommits_the_current_projection_without_changing_it() {
    ...
    context::dispatch(&mut app, En1999Command::Evaluate(evaluate::Evaluate {})).await;
...
async fn selected_check_index_is_a_config_only_edit() {
    let mut app = context::app_with_registry().await;
    let before = app.snapshot().expect("projection");
    let result = context::dispatch(&mut app, En1999Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) })).await;
...
async fn view_actions_never_emit_artifact_mutations_under_the_real_registry() {
    let mut app = context::app_with_registry().await;
    let result = context::dispatch(&mut app, En1999Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(1) })).await;
...
async fn undo_redo_round_trips_through_the_wrapper() {
    ...
    context::settle(&mut app).await;
    assert_eq!(app.snapshot().expect("projection"), En1999Snapshot::default());
    context::close(&mut app);
}
