//! 👁️ EN 1999 viewer — the read-only counterpart of `✏️editor` for this subset (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `En1999Viewer` implements
//! `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<En1999Viewer>` (framework SDK) is
//! the sole runtime adapter, so this file can never structurally emit an artifact or draft mutation.
//! MUST NOT import anything from the sibling editor module (`policyViewerPurityBreaches`).

use crate::artifacts::en1999::En1999Snapshot;
use crate::artifacts::en1999::{EN1999_DIALECT, EN1999_DOCUMENT_SCHEMA};
use crate::viewer::en1999::modes::view;
...
pub enum En1999ViewCommand {
    #[default]
    Noop,
}

impl protocol::OpBinary for En1999ViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(En1999ViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
pub struct En1999Viewer;

impl ArtifactViewer for En1999Viewer {
    type Snapshot = En1999Snapshot;
    type Mutation = crate::artifacts::en1999::op::En1999Mutation;
    ...
    type Command = En1999ViewCommand;

    const DIALECT: Dialect = EN1999_DIALECT;
    const DOCUMENT_SCHEMA: &'static str = EN1999_DOCUMENT_SCHEMA;

    fn initial_snapshot() -> En1999Snapshot {
        En1999Snapshot::default()
    }

    /// 👁️ Structurally read-only: the sole `En1999ViewCommand::Noop` variant never carries a config
...
//#region 🔖️Manifest
pub fn create_en1999_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(EN1999_DIALECT)
        .document(["semio", "norm", "en1999"])
...
mod tests {
    ...
    fn create_en1999_viewer_builds_a_definition_for_this_dialect() {
    ...
    fn an_unknown_body_key_falls_back_to_a_text_node() {
        ...
        let json = serde_json::to_string(&<En1999Viewer as ArtifactViewer>::render("nope", &doc, &ConfigView { snapshot: &NoConfig::default() })).expect("json");
