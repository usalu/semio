//! â¨ï¸ EN 1999 play app â the `ArtifactApp` impl (dispatch-only), the aggregated command enum and
//! the manifest stitch.
//!
...
use crate::editor::en1999::commands::{apply_remedy, evaluate, insert_item, remove_item, selected_check, set_active_example, set_field, set_snapshot};
use crate::editor::en1999::modes::edit as edit_mode;
use crate::editor::en1999::modes::edit::windows::{inputs, results};
use crate::editor::en1999::panels::{catalogue as catalogue_panel, document as document_panel, inspection as inspection_panel};
...
pub const LABEL: &str = "EN 1999";
/// ðï¸ The playground/registry variant key â every body key, window id and schema is derived from it.
pub const VARIANT: &str = "en1999";
/// 🆔️ Retained-command / UI action controller id.
pub const CONTROLLER_ID: &str = "s.norm.en1999@1/*#editor";
pub const DOCUMENT_SCHEMA: &str = "semio.norm.en1999/v1";
//#endregion ðï¸Constants

//#region ðï¸Commands
semio_framework_plugin::app_commands! {
    /// ð¯ï¸ `En1999PlayApp::Command` â the SOLE dispatch surface for this app's own behavior, covering every
    /// action `create_en1999_app` declares. Row order IS the binary variant ordinal (appending is safe,
    ...
    pub enum En1999Command for En1999Snapshot, En1999Mutation, NoConfig, NoConfigMutation {
...
//#region ðï¸En1999PlayApp
#[derive(Default)]
pub struct En1999PlayApp;

impl ArtifactEditor for En1999PlayApp {
    /// 📚️ Artifact catalogue stamped by `PluginBuilder::editor` onto the navbar dropdown.
    fn examples() -> Vec<semio_framework_plugin::ExampleSource> {
        vec![crate::aluminium_roof_purlin::source(), crate::noncompliant_multi_fail::source()]
    }
    ...
    type Command = En1999Command;

    const DIALECT: Dialect = crate::EN1999_DIALECT;
    const DOCUMENT_SCHEMA: &'static str = "semio.norm.en1999/v1";
    ...
    fn register_tool_job_factories(registry: &mut semio_framework_plugin::ArtifactToolFactoryRegistry<'_, semio_framework_plugin::EditorApp<Self>>) -> Result<(), Fault> {
        En1999BoundedCommandJobFactory::register(registry)
    }
    ...
    semio_framework_plugin::bounded_first_step_tool_proofs! {
        owner: semio_framework_plugin::EditorApp<En1999PlayApp>,
        owner_file: "✏️s/🔌️plugins/📕️norm/🗿️artifacts/🪶️en1999/🏅️standards/🔖️1/🪆️subsets/✳️any/✏️editor/🦀️.rs",
        controller: "s.norm.en1999@1/*#editor",
        artifact_schema: "semio.norm.en1999/v1",
        factory: "En1999BoundedCommandJobFactory",
        factory_type: En1999BoundedCommandJobFactory,
    ...
    fn command_id(command: &En1999Command) -> &'static str {
        command.command_id()
    }

    semio_s_artifact_norm_contract::norm_command_from_action!(En1999Command, crate::standards::v1::subsets::any::schema::snapshot::decode_en1999_snapshot_json);

    fn handle(
        command: &En1999Command,
        doc: &ArtifactView<'_, En1999Snapshot>,
        cfg: &ConfigView<'_, NoConfig>,
        _interaction: &InteractionView<'_>,
        view_state: Option<&semio_framework_plugin::ViewModel>,
        _draft: &DraftView<'_, Self::Draft>,
        _engines: &EngineHandles,
    ) -> Result<Emit<En1999Mutation, NoConfigMutation, Self::DraftMutation>, Fault> {
        crate::app_surface::dispatch_norm_command::<Self>(command, doc, cfg, view_state)
    }

    fn render(body_key: &str, doc: &ArtifactView<'_, En1999Snapshot>, cfg: &ConfigView<'_, NoConfig>, view_state: &semio_framework_plugin::ViewModel) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::ComponentTree> {
        let host = NormHost::<En1999Family>::from_artifact(doc.snapshot.clone());
        match body_key {
    ...
    fn export_media(port: &str, doc: &ArtifactView<'_, En1999Snapshot>) -> Result<Media, MediaError> {
        crate::app_surface::export_media::<En1999Family>(port, VARIANT, DOCUMENT_SCHEMA, doc.snapshot)
    }

    /// ðï¸ `"model:in"`/`"artifact:in"` â see `crate::app_surface::import_media`.
    fn import_media(port: &str, media: &Media, _doc: &ArtifactView<'_, En1999Snapshot>) -> Result<Emit<En1999Mutation, NoConfigMutation, Self::DraftMutation>, MediaError> {
        crate::app_surface::import_media(port, media, |snapshot: En1999Snapshot| En1999Mutation::from_snapshot(&En1999Snapshot::empty(), &snapshot))
    }
    //#endregion ðï¸MediaPorts
}
//#endregion ðï¸En1999PlayApp

//#region 🧵️RetainedCommands
crate::norm_owned_tool_job_factory!(En1999BoundedCommandJobFactory, En1999PlayApp);

impl crate::app_surface::NormRetainedEditor for En1999PlayApp {
    type Family = En1999Family;

    type ResultsWindowConfigOwner = results::ResultsWindowConfigOwner;

    fn selected_check_window_mutation(command: &En1999Command) -> Option<crate::results_window_config::NormResultsWindowConfigMutation> {
        match command {
            En1999Command::SetSelectedCheckIndex(payload) => Some(selected_check::window_mutation(payload)),
            _ => None,
        }
    }

    fn dispatch_retained(command: &En1999Command, doc: &ArtifactView<'_, En1999Snapshot>, cfg: &ConfigView<'_, NoConfig>) -> Result<Emit<En1999Mutation, NoConfigMutation, NoDraftMutation>, Fault> {
...
impl crate::document::NormFamily for En1999Family {
    ...
    fn evaluate(document: &En1999Snapshot) -> crate::document::CheckReport {
...
//#region ðï¸Manifest
pub fn create_en1999_app() -> semio_framework_plugin::AppDefinition {
    Editor::builder(crate::EN1999_DIALECT)
            ...
            .action_describe("setSnapshot", LocalizedLabel::native("Replaces the whole EN 1999 (Eurocode 9, aluminium structures) compliance document with the supplied document JSON; the previous inputs are discarded.", "Ersetzt das gesamte Nachweisdokument nach EN 1999 (Eurocode 9, Aluminiumbau) durch das übergebene Dokument-JSON; die bisherigen Eingaben werden verworfen."))
            .action_describe("evaluate", LocalizedLabel::native("Recomputes every EN 1999 (Eurocode 9, aluminium structures) check from the document's inputs and refreshes the results window; the document is not changed.", "Berechnet alle Nachweise nach EN 1999 (Eurocode 9, Aluminiumbau) aus den Eingaben des Dokuments neu und aktualisiert das Ergebnisfenster; das Dokument ändert sich nicht."))
            ...
            .action_describe("setActiveExample", LocalizedLabel::native("Loads one of the bundled EN 1999 (Eurocode 9, aluminium structures) examples into the open compliance document, replacing its inputs, by example id.", "Lädt eines der mitgelieferten Beispiele nach EN 1999 (Eurocode 9, Aluminiumbau) in das offene Nachweisdokument und ersetzt dessen Eingaben, anhand der Beispiel-Id."))
            .build_definition()
}
