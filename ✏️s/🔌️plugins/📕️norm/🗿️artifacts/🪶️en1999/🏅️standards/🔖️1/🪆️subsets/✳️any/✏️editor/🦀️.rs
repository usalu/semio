...
use crate::editor::en1999::commands::{evaluate, selected_check, set_snapshot};
...
pub const CONFIG_SCHEMA: &str = "config.norm.en1999";
...
semio_framework_plugin::app_commands! {
    ...
    pub enum En1999Command for En1999Snapshot, En1999Mutation, NormConfig, NormConfigMutation {
...
impl ArtifactEditor for En1999PlayApp {
    ...
    const DIALECT: Dialect = crate::artifacts::en1999::EN1999_DIALECT;
    ...
    semio_framework_plugin::bounded_first_step_tool_proofs! {
        ...
        document_schema: "semio.norm.en1999/v1",
    ...
    ) -> Result<Emit<En1999Mutation, NormConfigMutation, Self::DraftMutation>, Fault> {
        command.dispatch(doc, cfg)
    }

    fn render(body_key: &str, doc: &ArtifactView<'_, En1999Snapshot>, cfg: &ConfigView<'_, NormConfig>) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::ComponentTree> {
        let host = NormHost::<En1999Family>::from_document(doc.snapshot.clone());
        match body_key {
    ...
    fn import_media(port: &str, media: &Media, _doc: &ArtifactView<'_, En1999Snapshot>) -> Result<Emit<En1999Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
...
impl crate::app_surface::NormRetainedEditor for En1999PlayApp {
    fn dispatch_retained(command: &En1999Command, doc: &ArtifactView<'_, En1999Snapshot>, cfg: &ConfigView<'_, NormConfig>) -> Result<Emit<En1999Mutation, NormConfigMutation, NoDraftMutation>, Fault> {
        command.dispatch(doc, cfg)
    }
}
...
impl crate::document::NormFamily for En1999Family {
    ...
    fn evaluate(document: &En1999Snapshot) -> crate::document::CheckReport {
        crate::artifacts::en1999::standards::v1::subsets::any::schema::inferences::evaluate(document)
    }
}
...
pub fn create_en1999_app() -> semio_framework_plugin::AppDefinition {
    Editor::builder(crate::artifacts::en1999::EN1999_DIALECT)
            .document(["semio", "norm", VARIANT])
            .artifact_kind(crate::artifacts::en1999::artifact_kind())
...
pub(crate) mod testkit {
    ...
    pub fn en1999_manifest_for_testkit() -> semio_framework_plugin::App {
    ...
    pub fn app_with_registry() -> NormApp {
        new_app_with_registry::<EditorApp<En1999PlayApp>>(en1999_manifest_for_testkit)
    }

    pub fn dispatch(app: &mut NormApp, command: En1999Command) -> InvocationResult {
...
mod tests {
    ...
    fn an_unknown_body_key_falls_back_to_a_text_node() {
        ...
        assert!(testkit::render(&mut app, "norm.en1999.play.nope").contains("Unknown body"));
    }
    ...
    fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::app_with_registry();
        testkit::dispatch(&mut app, En1999Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1999Snapshot::default() }));
    ...
    fn evaluate_recommits_the_current_projection_without_changing_it() {
        ...
        testkit::dispatch(&mut app, En1999Command::Evaluate(evaluate::Evaluate {}));
    ...
    fn selected_check_index_is_a_config_only_edit() {
        let mut app = testkit::app_with_registry();
        let before = app.snapshot().expect("projection");
        let result = testkit::dispatch(&mut app, En1999Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) }));
    ...
    fn view_actions_never_emit_artifact_mutations_under_the_real_registry() {
        let mut app = testkit::app_with_registry();
        let result = testkit::dispatch(&mut app, En1999Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(1) }));
