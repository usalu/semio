...
//#region 🔖️Handler
pub async fn handle(payload: &ReplaceSnapshot, _doc: &ArtifactView<'_, En1994Snapshot>, _cfg: &ConfigView<'_, NormConfig>) -> Result<Emit<En1994Mutation, NormConfigMutation>, Fault> {
...
mod tests {
    ...
    async fn handle_commits_the_payload_document_under_its_action_id() {
        ...
        let emit = handle(&ReplaceSnapshot { snapshot: En1994Snapshot::default() }, &ArtifactView::new(&projection, &HistoryView::empty()), &ConfigView { snapshot: &config }).expect("handle");
        assert_eq!(emit.artifact_mutations, En1994Mutation::from_snapshot(&En1994Snapshot::default()));
