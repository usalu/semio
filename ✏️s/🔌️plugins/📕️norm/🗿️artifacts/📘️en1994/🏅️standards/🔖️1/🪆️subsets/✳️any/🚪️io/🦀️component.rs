//! 🚪️ IO s.en1994 (1/✳️any) — no stdio format bridges. W5a (ticket 26/08/11/SEMIO-ARTIFACT-UNIFIED-IMPORT-EXPORT-AND-MEDIA-FORMAT-RETIREMENT) deleted the five
//! degenerate leaves (csv/json/txt/xlsx/zip) that either fabricated a one-cell-CSV/raw-DSL-dump
//! shape or silently defaulted to `En1994Snapshot::default()` on import (an honesty bug, not a
//! real codec). En1994Snapshot is a compliance document (scalar fields plus a handful of nested
...
//! ⚙️engine::register) for the native `s.en1994` dialect only.
...
pub mod derived_composition {
    use crate::artifacts::en1994::standards::v1::subsets::any::schema::En1994Analyzer;
    ...
    pub struct En1994ComposerComposition;

    impl ArtifactComposition for En1994ComposerComposition {
        ...
        fn compose(sources: &[ComposeSource<'_>]) -> Result<Composition<Self::Snapshot>, ComposeError> {
            ...
            Err(ComposeError { message: "En1994ComposerComposition: no source in a known read dialect".into(), diagnostics: Vec::new() })
        }
    }
}
