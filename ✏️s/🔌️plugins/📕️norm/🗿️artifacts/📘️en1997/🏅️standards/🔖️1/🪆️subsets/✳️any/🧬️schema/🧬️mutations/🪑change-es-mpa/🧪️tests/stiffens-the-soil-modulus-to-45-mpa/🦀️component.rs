...
async fn declared_outcome_holds() {
    ...
    assert_eq!(produced.worst_level(), None, "change-es-mpa/stiffens-the-soil-modulus-to-45-mpa: the payload is finite, so `change-es-mpa`'s `mutation.invariant` fatal cannot fire, and 45.0 differs from the committed 30.0, so its `mutation.no-op` warning cannot either");
