//! 🧪️ `change-phi-deg` fixture — `raises-the-friction-angle-to-35-degrees`.
//!
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(produced.worst_level(), None, "change-phi-deg/raises-the-friction-angle-to-35-degrees: the payload is finite, so `change-phi-deg`'s `mutation.invariant` fatal cannot fire, and 35.0 differs from the committed 30.0, so its `mutation.no-op` warning cannot either");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-phi-deg/raises-the-friction-angle-to-35-degrees: the produced diff differs from the committed 🔺️diff/🔣️component.json");
}
