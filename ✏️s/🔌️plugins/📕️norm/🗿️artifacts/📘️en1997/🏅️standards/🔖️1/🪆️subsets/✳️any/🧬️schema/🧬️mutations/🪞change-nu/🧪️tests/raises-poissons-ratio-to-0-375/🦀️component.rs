...
async fn declared_outcome_holds() {
    ...
    assert_eq!(produced.worst_level(), None, "change-nu/raises-poissons-ratio-to-0-375: the payload is finite, so `change-nu`'s `mutation.invariant` fatal cannot fire, and 0.375 differs from the committed 0.25, so its `mutation.no-op` warning cannot either");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-nu/raises-poissons-ratio-to-0-375: the produced diff differs from the committed 🔺️diff/🔣️component.json");
}
