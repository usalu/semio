//! 🧪️ `change-sfp-wm3-s` fixture — `improves-the-specific-fan-power-to-1250-w-per-m3-s`.
//!
//! Improves the measured specific fan power. The REQUIRED SFP class it is checked against is a separate leaf and is not re-derived, which this fixture asserts explicitly. Guards read off this leaf's own diff builder: `is_finite` then `==` no-op.
//!
...
/// ▶️ `change-sfp-wm3-s` carries `before` to exactly the committed `after`.
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "change-sfp-wm3-s/improves-the-specific-fan-power-to-1250-w-per-m3-s: inverse did not restore the before-snapshot");
    assert_eq!(inverse.len(), 1, "change-sfp-wm3-s/improves-the-specific-fan-power-to-1250-w-per-m3-s: undoing one sfpWM3S edit is exactly one step");
    assert_eq!(snapshot.sfp_w_m3_s, 1500.0, "change-sfp-wm3-s/improves-the-specific-fan-power-to-1250-w-per-m3-s: the undo step must put sfpWM3S back to 1500.0");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-sfp-wm3-s/improves-the-specific-fan-power-to-1250-w-per-m3-s: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "change-sfp-wm3-s/improves-the-specific-fan-power-to-1250-w-per-m3-s: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome — status AND every diagnostic `change-sfp-wm3-s`'s own diff builder
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "change-sfp-wm3-s/improves-the-specific-fan-power-to-1250-w-per-m3-s: raised diagnostics differ from the committed 🎯️outcome messages");
    ...
    match status {
        "applied" if declared.iter().any(|(_, code)| code == "mutation.no-op") => {
            assert!(applied, "change-sfp-wm3-s/improves-the-specific-fan-power-to-1250-w-per-m3-s: declared applied but the mutation was rejected");
            assert_eq!(snapshot, before(), "change-sfp-wm3-s/improves-the-specific-fan-power-to-1250-w-per-m3-s: a no-op outcome is applied with an EMPTY diff — the snapshot must come back untouched");
        }
        "applied" => {
            assert!(applied, "change-sfp-wm3-s/improves-the-specific-fan-power-to-1250-w-per-m3-s: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "change-sfp-wm3-s/improves-the-specific-fan-power-to-1250-w-per-m3-s: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "change-sfp-wm3-s/improves-the-specific-fan-power-to-1250-w-per-m3-s: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("change-sfp-wm3-s/improves-the-specific-fan-power-to-1250-w-per-m3-s: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta `change-sfp-wm3-s` produces is exactly the committed diff — the single most
...
/// `Some(None)` across a JSON round trip — `change-sfp-wm3-s` never writes it, so the committed
...
async fn committed_diff_is_canonical() {
    ...
    assert!(decoded.selected_check_index.is_none(), "change-sfp-wm3-s/improves-the-specific-fan-power-to-1250-w-per-m3-s: change-sfp-wm3-s is an artifact-lane edit and must never carry the presence-lane selectedCheckIndex");
    let reencoded = serde_json::to_value(&decoded).expect("diff re-encodes");
    let original: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff reparses");
    assert_eq!(reencoded, original, "change-sfp-wm3-s/improves-the-specific-fan-power-to-1250-w-per-m3-s: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `change-sfp-wm3-s` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
