...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot.years_since_inspection, before().years_since_inspection, "change-system-type/switches-to-a-decentral-mechanical-system: yearsSinceInspection must stay exactly as the before-snapshot had it — change-system-type owns systemType and nothing else");
}
...
async fn produces_committed_diff() {
    ...
    assert!(raised.diff().years_since_inspection.is_none(), "change-system-type/switches-to-a-decentral-mechanical-system: the sparse delta must leave yearsSinceInspection unset — a delta that rewrote it would be a bug this assertion exists to catch");
}
