...
//! leaf's own `../../🔺️diff/🦀️component.rs`, which writes `En1998Diff.tank_mass_t` and nothing else,
...
async fn change_tank_mass_t_applies_to_committed_after() {
...
#[semio_framework_async_macros::async_test]
async fn change_tank_mass_t_inverse_restores_before() {
    let base = before();
...
#[semio_framework_async_macros::async_test]
async fn change_tank_mass_t_committed_json_is_canonical() {
...
#[semio_framework_async_macros::async_test]
async fn change_tank_mass_t_declared_outcome_holds() {
...
#[semio_framework_async_macros::async_test]
async fn change_tank_mass_t_produces_committed_diff() {
    let outcome = <En1998Mutation as protocol::Mutation<En1998Snapshot>>::diff(&mutation(), &before());
    ...
    assert_eq!(produced, committed, "change-tank-mass-t/raises-tank-mass-t-to-425-0: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
#[semio_framework_async_macros::async_test]
async fn change_tank_mass_t_committed_diff_applies_to_after() {
    let decoded: En1998Diff = serde_json::from_str(DIFF).expect("change-tank-mass-t committed diff decodes");
