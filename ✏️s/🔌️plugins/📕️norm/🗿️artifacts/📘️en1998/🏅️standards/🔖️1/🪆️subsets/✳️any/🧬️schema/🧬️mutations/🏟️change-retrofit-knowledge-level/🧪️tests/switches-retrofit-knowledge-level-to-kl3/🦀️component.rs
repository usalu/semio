...
//! leaf's own `../../🔺️diff/🦀️component.rs`, which writes `En1998Diff.retrofit_knowledge_level` and nothing else,
...
async fn change_retrofit_knowledge_level_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-retrofit-knowledge-level/switches-retrofit-knowledge-level-to-kl3: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
