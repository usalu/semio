...
//! leaf's own `../../🔺️diff/🦀️.rs`, which writes `En1998Diff.retrofit_limit_state` and nothing else,
...
fn change_retrofit_limit_state_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-retrofit-limit-state/switches-retrofit-limit-state-to-near-collapse: produced diff differs from the committed 🔺️diff/🔣️.json");
}
