...
//! leaf's own `../../🔺️diff/🦀️.rs`, which writes `En1998Diff.tank_v_rd_kn` and nothing else,
...
fn change_tank_v_rd_kn_applies_to_committed_after() {
...
#[semio_framework_async_macros::async_test]
fn change_tank_v_rd_kn_inverse_restores_before() {
    let base = before();
...
#[semio_framework_async_macros::async_test]
fn change_tank_v_rd_kn_committed_json_is_canonical() {
...
#[semio_framework_async_macros::async_test]
fn change_tank_v_rd_kn_declared_outcome_holds() {
...
#[semio_framework_async_macros::async_test]
fn change_tank_v_rd_kn_produces_committed_diff() {
    let outcome = <En1998Mutation as protocol::Mutation<En1998Snapshot>>::diff(&mutation(), &before());
    ...
    assert_eq!(produced, committed, "change-tank-v-rd-kn/raises-tank-v-rd-kn-to-537-5: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
#[semio_framework_async_macros::async_test]
fn change_tank_v_rd_kn_committed_diff_is_canonical() {
...
#[semio_framework_async_macros::async_test]
fn change_tank_v_rd_kn_committed_diff_applies_to_after() {
    let decoded: En1998Diff = serde_json::from_str(DIFF).expect("change-tank-v-rd-kn committed diff decodes");
