...
//! leaf's own `../../🔺️diff/🦀️.rs`, which writes `En1998Diff.en_a_gr` and nothing else,
...
fn change_en_a_gr_applies_to_committed_after() {
...
#[semio_framework_async_macros::async_test]
fn change_en_a_gr_inverse_restores_before() {
    let base = before();
...
#[semio_framework_async_macros::async_test]
fn change_en_a_gr_committed_json_is_canonical() {
...
#[semio_framework_async_macros::async_test]
fn change_en_a_gr_declared_outcome_holds() {
...
#[semio_framework_async_macros::async_test]
fn change_en_a_gr_produces_committed_diff() {
    let outcome = <En1998Mutation as protocol::Mutation<En1998Snapshot>>::diff(&mutation(), &before());
    ...
    assert_eq!(produced, committed, "change-en-a-gr/raises-en-a-gr-to-0-25: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
#[semio_framework_async_macros::async_test]
fn change_en_a_gr_committed_diff_is_canonical() {
...
#[semio_framework_async_macros::async_test]
fn change_en_a_gr_committed_diff_applies_to_after() {
    let decoded: En1998Diff = serde_json::from_str(DIFF).expect("change-en-a-gr committed diff decodes");
