//! 🧪️ `change-silo-n-rd-kn` fixture — `raises-silo-n-rd-kn-to-640-0`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below was transcribed from this
//! leaf's own `../../🔺️diff/🦀️component.rs`, which writes `En1998Diff.silo_n_rd_kn` and nothing else,
//! behind an `is_finite` `mutation.invariant` guard and a `base.silo_n_rd_kn == payload.new_silo_n_rd_kn` `mutation.no-op` guard.
...
fn before() -> En1998Snapshot {
    serde_json::from_str(BEFORE).expect("change-silo-n-rd-kn before-snapshot decodes")
}
fn expected_after() -> En1998Snapshot {
    serde_json::from_str(AFTER).expect("change-silo-n-rd-kn after-snapshot decodes")
}
fn mutation() -> En1998Mutation {
    serde_json::from_str(MUTATION).expect("change-silo-n-rd-kn mutation decodes")
}
...
fn change_silo_n_rd_kn_applies_to_committed_after() {
    ...
    assert!(messages.is_empty(), "change-silo-n-rd-kn/raises-silo-n-rd-kn-to-640-0: a real 500.0 to 640.0 change must raise neither the leaf's `mutation.invariant` nor its `mutation.no-op` message");
}
...
fn change_silo_n_rd_kn_inverse_restores_before() {
    ...
    assert_eq!(inverse.len(), 1, "change-silo-n-rd-kn/raises-silo-n-rd-kn-to-640-0: a scalar change inverts with exactly one counter-mutation");
    let mut restored = forward;
    for step in &inverse {
        let (next, _messages) = vcs::apply_mutation(&restored, step).expect("inverse change-silo-n-rd-kn step applies");
        restored = next;
    }
    assert_eq!(restored.silo_n_rd_kn, 500.0, "change-silo-n-rd-kn/raises-silo-n-rd-kn-to-640-0: the inverse must put silo_n_rd_kn back to 500.0");
    assert_eq!(restored, base, "change-silo-n-rd-kn/raises-silo-n-rd-kn-to-640-0: the inverse did not restore the committed before-snapshot");
}

/// 🔣️ Both committed snapshots and the committed `ChangeSiloNRdKn` payload are already canonical:
...
fn change_silo_n_rd_kn_committed_json_is_canonical() {
    for (side, text) in [("⬅️before", BEFORE), ("➡️after", AFTER)] {
        ...
        let original: serde_json::Value = serde_json::from_str(text).expect("change-silo-n-rd-kn snapshot reparses");
        assert_eq!(reencoded, original, "change-silo-n-rd-kn/raises-silo-n-rd-kn-to-640-0: committed {side} snapshot JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("change-silo-n-rd-kn mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("change-silo-n-rd-kn mutation reparses");
    assert_eq!(reencoded, original, "change-silo-n-rd-kn/raises-silo-n-rd-kn-to-640-0: committed mutation JSON is not the canonical externally-tagged ChangeSiloNRdKn form carrying newSiloNRdKn");
}
...
fn change_silo_n_rd_kn_declared_outcome_holds() {
    ...
    assert!(outcome.messages().is_empty(), "change-silo-n-rd-kn/raises-silo-n-rd-kn-to-640-0: moving silo_n_rd_kn from 500.0 to 640.0 must raise neither the leaf's `mutation.invariant` nor its `mutation.no-op` message");
    assert!(vcs::apply_mutation(&before(), &mutation()).is_ok(), "change-silo-n-rd-kn/raises-silo-n-rd-kn-to-640-0: an applied outcome must survive the diff-apply seam");
}

/// 🔺️ The sparse delta `change-silo-n-rd-kn` produces is exactly the committed diff — the load-bearing
...
fn change_silo_n_rd_kn_produces_committed_diff() {
    ...
    assert!(outcome.diff().silo_v_ed_kn.is_none(), "change-silo-n-rd-kn/raises-silo-n-rd-kn-to-640-0: change-silo-n-rd-kn must leave silo_v_ed_kn untouched");
    let produced = serde_json::to_value(outcome.diff()).expect("change-silo-n-rd-kn produced diff encodes");
    let committed: serde_json::Value = serde_json::from_str(DIFF).expect("change-silo-n-rd-kn committed diff decodes");
    assert_eq!(produced, committed, "change-silo-n-rd-kn/raises-silo-n-rd-kn-to-640-0: produced diff differs from the committed 🔺️diff/🔣️component.json");
}

/// 🔣️ The committed diff is canonical and decodes back into `En1998Diff` with `siloNRdKn` set.
...
fn change_silo_n_rd_kn_committed_diff_is_canonical() {
    ...
    let reencoded = serde_json::to_value(&decoded).expect("change-silo-n-rd-kn committed diff re-encodes");
    let original: serde_json::Value = serde_json::from_str(DIFF).expect("change-silo-n-rd-kn committed diff reparses");
    assert_eq!(reencoded, original, "change-silo-n-rd-kn/raises-silo-n-rd-kn-to-640-0: committed diff JSON is not canonical");
}
...
fn change_silo_n_rd_kn_committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "change-silo-n-rd-kn/raises-silo-n-rd-kn-to-640-0: the committed diff did not carry before to after");
}
