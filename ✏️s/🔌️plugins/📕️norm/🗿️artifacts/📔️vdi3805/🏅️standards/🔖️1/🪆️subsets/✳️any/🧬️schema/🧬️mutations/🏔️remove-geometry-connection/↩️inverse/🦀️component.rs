...
//#region 🔖️Inverse
pub async fn inverse(payload: &RemoveGeometryConnection, base: &Vdi3805Snapshot) -> Vec<Vdi3805Mutation> {
    let existing = base.geometry.get(&payload.id).and_then(|geometry| geometry.connections.iter().find(|c| c.id == payload.connection_id));
    match existing {
        Some(old) => vec![Vdi3805Mutation::AddGeometryConnection(add_geometry_connection::mutation::AddGeometryConnection { id: payload.id.clone(), connection: old.clone() })],
        None => Vec::new(),
    }
}
//#endregion 🔖️Inverse
