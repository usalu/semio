//! 🧪️ `rename-product` fixture — `retitles-vlv-50-001-and-resyncs-its-index-tags`.
//!
//! The bilingual product title is replaced, and the persisted `catalog.index` entry's `tags` are rebuilt from the new title's texts in order. Only `catalog` and `index` are republished.
//!
...
fn applied() -> Vdi3805Snapshot {
    ...
    <Vdi3805Diff as protocol::MutationDiff<Vdi3805Snapshot>>::apply(raised.diff(), &base).expect("rename-product applies to its committed before-snapshot")
}
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "rename-product/retitles-vlv-50-001-and-resyncs-its-index-tags: applied state differs from committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    assert!(!inverse.is_empty(), "rename-product/retitles-vlv-50-001-and-resyncs-its-index-tags: this mutation changes state, so its inverse must not be empty");
    ...
    assert_eq!(snapshot, base, "rename-product/retitles-vlv-50-001-and-resyncs-its-index-tags: inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "rename-product/retitles-vlv-50-001-and-resyncs-its-index-tags: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "rename-product/retitles-vlv-50-001-and-resyncs-its-index-tags: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome — status AND every diagnostic `rename-product`'s own diff builder raises —
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "rename-product/retitles-vlv-50-001-and-resyncs-its-index-tags: raised diagnostics differ from the committed 🎯️outcome messages");
    let snapshot = applied();
    match status {
        "applied" => assert_ne!(snapshot, before(), "rename-product/retitles-vlv-50-001-and-resyncs-its-index-tags: declared applied but the snapshot came back unchanged"),
        "rejected" => assert_eq!(snapshot, before(), "rename-product/retitles-vlv-50-001-and-resyncs-its-index-tags: a rejected mutation must leave the snapshot untouched"),
        other => panic!("rename-product/retitles-vlv-50-001-and-resyncs-its-index-tags: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH fields `rename-product` is allowed to
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rename-product/retitles-vlv-50-001-and-resyncs-its-index-tags: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "rename-product/retitles-vlv-50-001-and-resyncs-its-index-tags: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `rename-product` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "rename-product/retitles-vlv-50-001-and-resyncs-its-index-tags: committed diff did not carry before to after");
}
