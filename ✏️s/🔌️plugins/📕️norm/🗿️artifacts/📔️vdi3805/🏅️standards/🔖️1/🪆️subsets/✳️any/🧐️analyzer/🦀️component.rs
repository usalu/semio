//! 🧐️ Vdi3805Analyzer (1/✳️any) — read-only analysis, successor to the pre-migration
//! Vdi3805Decomposer. Real logic; artifact/standard levels delegate here.
...
impl ArtifactAnalyzer for Vdi3805Analyzer {
    ...
    fn analyze(sources: &[AnalyzeSource<'_>]) -> Analysis<Self::Parts> {
        ...
        for source in sources {
            match source {
                AnalyzeSource::Text(text) => match <Vdi3805Snapshot as store::DocumentDsl>::parse_dsl(text) {
                ...
                AnalyzeSource::Binary(bytes) => match <Vdi3805Snapshot as store::DocumentPack>::decode_pack(bytes) {
                    Ok(snapshot) => parts.snapshot = Some(snapshot),
                    Err(err) => {
                        confidence = IoConfidence::Low;
