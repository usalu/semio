//! 🧪️ `create-geometry` fixture — `adds-the-geom-valve-80-definition`.
//!
...
fn applied() -> Vdi3805Snapshot {
    ...
    <Vdi3805Diff as protocol::MutationDiff<Vdi3805Snapshot>>::apply(raised.diff(), &base).expect("create-geometry applies to its committed before-snapshot")
}
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "create-geometry/adds-the-geom-valve-80-definition: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert!(!inverse.is_empty(), "create-geometry/adds-the-geom-valve-80-definition: this mutation changes state, so its inverse must not be empty");
    ...
    assert_eq!(snapshot, base, "create-geometry/adds-the-geom-valve-80-definition: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "create-geometry/adds-the-geom-valve-80-definition: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "create-geometry/adds-the-geom-valve-80-definition: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome — status AND every diagnostic `create-geometry`'s own diff builder raises —
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "create-geometry/adds-the-geom-valve-80-definition: raised diagnostics differ from the committed 🎯️outcome messages");
    let snapshot = applied();
    match status {
        "applied" => assert_ne!(snapshot, before(), "create-geometry/adds-the-geom-valve-80-definition: declared applied but the snapshot came back unchanged"),
        "rejected" => assert_eq!(snapshot, before(), "create-geometry/adds-the-geom-valve-80-definition: a rejected mutation must leave the snapshot untouched"),
        other => panic!("create-geometry/adds-the-geom-valve-80-definition: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH fields `create-geometry` is allowed to
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "create-geometry/adds-the-geom-valve-80-definition: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "create-geometry/adds-the-geom-valve-80-definition: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `create-geometry` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "create-geometry/adds-the-geom-valve-80-definition: committed diff did not carry before to after");
}
