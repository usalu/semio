//! 🧪️ `remove-geometry-connection` fixture — `detaches-the-out-connection-from-geom-valve-50`.
//!
//! `geom.valve.50`'s outlet connection is retained-out by id, leaving only `in`. A missing geometry id or a missing connection id would both be `mutation.target-missing`; both exist here.
//!
...
fn applied() -> Vdi3805Snapshot {
    ...
    <Vdi3805Diff as protocol::MutationDiff<Vdi3805Snapshot>>::apply(raised.diff(), &base).expect("remove-geometry-connection applies to its committed before-snapshot")
}
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "remove-geometry-connection/detaches-the-out-connection-from-geom-valve-50: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert!(!inverse.is_empty(), "remove-geometry-connection/detaches-the-out-connection-from-geom-valve-50: this mutation changes state, so its inverse must not be empty");
    ...
    assert_eq!(snapshot, base, "remove-geometry-connection/detaches-the-out-connection-from-geom-valve-50: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "remove-geometry-connection/detaches-the-out-connection-from-geom-valve-50: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "remove-geometry-connection/detaches-the-out-connection-from-geom-valve-50: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome — status AND every diagnostic `remove-geometry-connection`'s own diff builder raises —
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "remove-geometry-connection/detaches-the-out-connection-from-geom-valve-50: raised diagnostics differ from the committed 🎯️outcome messages");
    let snapshot = applied();
    match status {
        "applied" => assert_ne!(snapshot, before(), "remove-geometry-connection/detaches-the-out-connection-from-geom-valve-50: declared applied but the snapshot came back unchanged"),
        "rejected" => assert_eq!(snapshot, before(), "remove-geometry-connection/detaches-the-out-connection-from-geom-valve-50: a rejected mutation must leave the snapshot untouched"),
        other => panic!("remove-geometry-connection/detaches-the-out-connection-from-geom-valve-50: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH fields `remove-geometry-connection` is allowed to
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "remove-geometry-connection/detaches-the-out-connection-from-geom-valve-50: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "remove-geometry-connection/detaches-the-out-connection-from-geom-valve-50: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `remove-geometry-connection` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "remove-geometry-connection/detaches-the-out-connection-from-geom-valve-50: committed diff did not carry before to after");
}
