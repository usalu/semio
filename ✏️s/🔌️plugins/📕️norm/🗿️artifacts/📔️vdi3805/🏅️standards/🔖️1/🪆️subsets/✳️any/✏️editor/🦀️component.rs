...
impl ArtifactEditor for Vdi3805PlayApp {
    ...
    async fn initial_snapshot() -> Vdi3805Snapshot {
    ...
    async fn command_id(command: &Vdi3805Command) -> &'static str {
        command.command_id()
    }
    ...
    async fn render(body_key: &str, doc: &ArtifactView<'_, Vdi3805Snapshot>, cfg: &ConfigView<'_, NormConfig>) -> UiNode {
    ...
    async fn export_media(port: &str, doc: &ArtifactView<'_, Vdi3805Snapshot>) -> Result<Media, MediaError> {
    ...
    async fn import_media(port: &str, media: &Media, doc: &ArtifactView<'_, Vdi3805Snapshot>) -> Result<Emit<Vdi3805Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
...
impl crate::document::NormFamily for Vdi3805Family {
    ...
    async fn evaluate(document: &Vdi3805Snapshot) -> crate::document::CheckReport {
...
//#region ðï¸Manifest
pub async fn create_vdi3805_app() -> semio_framework_plugin::AppDefinition {
...
pub(crate) mod testkit {
    ...
    /// ✏️ Adapts `create_vdi3805_app`'s `AppDefinition` (contract §2.4) into the `App { definition,
    ...
    pub async fn vdi3805_manifest_for_testkit() -> semio_framework_plugin::App {
        semio_framework_plugin::App { definition: create_vdi3805_app(), examples: Vec::new() }
    }

    pub type NormApp = VcsArtifactApp<EditorApp<Vdi3805PlayApp>>;

    pub async fn new_app() -> NormApp {
        sdk_new_app::<EditorApp<Vdi3805PlayApp>>()
    }
    ...
    pub async fn app_with_registry() -> NormApp {
        new_app_with_registry::<EditorApp<Vdi3805PlayApp>>(vdi3805_manifest_for_testkit)
    }

    pub async fn dispatch(app: &mut NormApp, command: Vdi3805Command) -> InvocationResult {
...
mod tests {
    ...
    async fn every_command() -> Vec<Vdi3805Command> {
        vec![
