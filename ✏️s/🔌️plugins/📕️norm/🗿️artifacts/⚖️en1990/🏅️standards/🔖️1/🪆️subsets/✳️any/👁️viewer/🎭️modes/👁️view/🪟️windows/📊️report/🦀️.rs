//! 📊️ EN 1990 viewer — the Report window: a read-only table of every computed compliance
...
//#region 🔖️Definition
/// 🧱️ Stitched into the viewer manifest by `crate::viewer::en1990::create_en1990_viewer`. Read-only variant
...
//#region 🔖️Render
/// 👁️ Pure `En1990Snapshot -> UiNode` read: recomputes the compliance report straight off the document
/// (the same pure inference the editor's results window renders through `NormHost`), then tables it.
pub fn render(document: &En1990Snapshot, locale: semio_framework_plugin::Locale) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::BuiltNode> {
