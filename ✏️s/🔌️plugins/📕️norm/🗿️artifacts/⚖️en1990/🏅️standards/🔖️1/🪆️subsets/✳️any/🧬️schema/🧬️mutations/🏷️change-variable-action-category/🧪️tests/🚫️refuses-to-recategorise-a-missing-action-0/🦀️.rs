//! 🧪️ `change-variable-action-category` fixture — `🚫️refuses-to-recategorise-a-missing-action-0`.
//!
//! `change-variable-action-category` looks the entry up with `q_k.get_mut(index)` before it compares categories, so against the empty list an unseeded working-scene cache reads back, the lookup misses and the builder answers `mutation.target-missing` with the index as its target address. The `mutation.no-op` same-category guard is never reached.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏷️change-variable-action-category/🚫️refuses-to-recategorise-a-missing-action-0/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏷️change-variable-action-category/🚫️refuses-to-recategorise-a-missing-action-0/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏷️change-variable-action-category/🚫️refuses-to-recategorise-a-missing-action-0/🦠️mutation/🔣️.json");
const DIFF_ABSENT: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏷️change-variable-action-category/🚫️refuses-to-recategorise-a-missing-action-0/🔺️diff/🚫️.absent");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏷️change-variable-action-category/🚫️refuses-to-recategorise-a-missing-action-0/🎯️outcome/🔣️.json");
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "change-variable-action-category/refuses-to-recategorise-a-missing-action-0: applied state differs from committed after-snapshot");
    assert_eq!(expected_after(), before(), "change-variable-action-category/refuses-to-recategorise-a-missing-action-0: a rejected case's after-snapshot must be its before-snapshot verbatim");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "change-variable-action-category/refuses-to-recategorise-a-missing-action-0: replaying the inverse of a rejection must still leave the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-variable-action-category/refuses-to-recategorise-a-missing-action-0: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "change-variable-action-category/refuses-to-recategorise-a-missing-action-0: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome — the `rejected` status AND the exact diagnostic `change-variable-action-category`'s own
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "change-variable-action-category/refuses-to-recategorise-a-missing-action-0: raised diagnostics differ from the committed 🎯️outcome messages");
    assert!(produced.iter().any(|(_, raised_code)| raised_code == code), "change-variable-action-category/refuses-to-recategorise-a-missing-action-0: the outcome's declared code must be one the builder actually raised");
    assert_eq!(applied(), before(), "change-variable-action-category/refuses-to-recategorise-a-missing-action-0: a rejected mutation must leave the snapshot untouched");
}
...
async fn produces_committed_diff() {
    let raised = <En1990Mutation as protocol::Mutation<En1990Snapshot>>::diff(&mutation(), &before());
    assert_eq!(raised.diff(), &En1990Diff::default(), "change-variable-action-category/refuses-to-recategorise-a-missing-action-0: a fatal rejection must publish the default (all-null) diff");
    assert!(DIFF_ABSENT.is_empty(), "change-variable-action-category/refuses-to-recategorise-a-missing-action-0: 🔺️diff/🚫️.absent must be a zero-byte marker");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "change-variable-action-category/refuses-to-recategorise-a-missing-action-0: the rejection's empty diff must leave before exactly as committed in after");
}
