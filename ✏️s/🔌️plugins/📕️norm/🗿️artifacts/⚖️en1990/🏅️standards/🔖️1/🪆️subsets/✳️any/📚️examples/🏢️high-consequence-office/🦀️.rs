...
/// `q_k` composed-child handle over the SAME content, so its content-addressed `child_id` equals
/// whatever `PRIMARY_TEXT`'s `qK=` line decodes to — the standard way a caller (e.g. a test parsing
/// `PRIMARY_TEXT` fresh) identifies the entries behind a parsed-from-text handle, whose own local
/// owner stays absent until the host materializes the child document.
pub fn reference_snapshot() -> crate::En1990Snapshot {
    let q_k =
        crate::en1990_qk_child_from_entries(&[crate::En1990QkEntry { category: "office".into(), value: 60.0 }, crate::En1990QkEntry { category: "partition-walls".into(), value: 12.0 }, crate::En1990QkEntry { category: "snow".into(), value: 18.0 }]);
