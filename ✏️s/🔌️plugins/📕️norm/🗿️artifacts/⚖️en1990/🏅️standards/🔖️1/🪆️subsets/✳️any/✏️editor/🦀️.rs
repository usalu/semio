...
use crate::editor::en1990::commands::{evaluate, selected_check, set_snapshot};
...
impl ArtifactEditor for En1990PlayApp {
    ...
    const DIALECT: Dialect = crate::artifacts::en1990::EN1990_DIALECT;
    ...
    semio_framework_plugin::bounded_first_step_tool_proofs! {
        ...
        document_schema: "semio.norm.en1990/v1",
...
impl crate::app_surface::NormRetainedEditor for En1990PlayApp {
    fn dispatch_retained(command: &En1990Command, doc: &ArtifactView<'_, En1990Snapshot>, cfg: &ConfigView<'_, NormConfig>) -> Result<Emit<En1990Mutation, NormConfigMutation, NoDraftMutation>, Fault> {
        command.dispatch(doc, cfg)
    }
}
...
impl crate::document::NormFamily for En1990Family {
    ...
    fn evaluate(document: &En1990Snapshot) -> crate::document::CheckReport {
        crate::artifacts::en1990::standards::v1::subsets::any::schema::inferences::evaluate(document)
    }
}
...
pub fn create_en1990_app() -> semio_framework_plugin::AppDefinition {
    Editor::builder(crate::artifacts::en1990::EN1990_DIALECT)
            .document(["semio", "norm", VARIANT])
            .artifact_kind(crate::artifacts::en1990::artifact_kind())
...
mod tests {
    ...
    fn retained_command_dispositions_match_the_language_neutral_oracle() {
        ...
        let definition = create_en1990_app();
    ...
    async fn command_ids_cover_every_row_and_are_unique() {
        ...
        let ids: Vec<&str> = commands.iter().map(En1990Command::command_id).collect();
    ...
    async fn optional_field_rows_keep_their_pre_migration_bytes() {
        let hex = |command: &En1990Command| protocol::OpBinary::encode_op(command).expect("encode").iter().map(|byte| format!("{byte:02x}")).collect::<String>();
        assert_eq!(hex(&En1990Command::Evaluate(evaluate::Evaluate {})), "01010000");
    ...
    async fn declares_model_in_and_report_out_ports() {
        let ports = create_en1990_app().io.ports;
    ...
    async fn an_unknown_body_key_falls_back_to_a_text_node() {
        ...
        assert!(testkit::render(&mut app, "norm.en1990.play.nope").await.contains("Unknown body"));
    }
    ...
    async fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::app_with_registry().await;
        testkit::dispatch(&mut app, En1990Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { text: crate::document::escape_op_text_field(&<En1990Snapshot as store::ArtifactDsl>::print_dsl(&En1990Snapshot::default())) })).await;
    ...
    async fn evaluate_recommits_the_current_projection_without_changing_it() {
        ...
        testkit::dispatch(&mut app, En1990Command::Evaluate(evaluate::Evaluate {})).await;
    ...
    async fn selected_check_index_is_a_config_only_edit() {
        let mut app = testkit::app_with_registry().await;
        let before = app.snapshot().expect("projection");
        let result = testkit::dispatch(&mut app, En1990Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) })).await;
    ...
    async fn view_actions_never_emit_artifact_mutations_under_the_real_registry() {
        let mut app = testkit::app_with_registry().await;
        let result = testkit::dispatch(&mut app, En1990Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(1) })).await;
