//! 🎹️ En1995Composer (1 standard) — aggregates its subsets' composer entries value-level. W5a
...
//! history) — a fabricated shape, not a real codec. Only the native `s.en1995` entry remains.

use std::sync::OnceLock;
use semio_framework_plugin::{ComposerEntry, composer_entry_of};
use crate::artifacts::en1995::standards::v1::subsets::any::composer::En1995Composer as En1995AnyComposer;

static ENTRIES: OnceLock<Vec<ComposerEntry>> = OnceLock::new();

pub fn entries() -> &'static [ComposerEntry] {
    ENTRIES.get_or_init(|| vec![
        composer_entry_of::<En1995AnyComposer>(),
    ]).as_slice()
}
