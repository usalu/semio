...
async fn declared_outcome_holds() {
    ...
    assert_eq!(produced.worst_level(), None, "change-f-ed-kn/raises-the-design-fastener-force-to-24-kn: the payload is finite, so `change-f-ed-kn`'s `mutation.invariant` fatal cannot fire, and 24.0 differs from the committed 18.0, so its `mutation.no-op` warning cannot either");
