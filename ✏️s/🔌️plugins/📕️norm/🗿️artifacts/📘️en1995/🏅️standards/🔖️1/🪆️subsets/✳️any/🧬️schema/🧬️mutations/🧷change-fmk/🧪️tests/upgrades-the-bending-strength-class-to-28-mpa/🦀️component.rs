//! 🧪️ `change-fmk` fixture — `upgrades-the-bending-strength-class-to-28-mpa`.
//!
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(produced.worst_level(), None, "change-fmk/upgrades-the-bending-strength-class-to-28-mpa: the payload is finite, so `change-fmk`'s `mutation.invariant` fatal cannot fire, and 28.0 differs from the committed 24.0, so its `mutation.no-op` warning cannot either");
