...
impl From<En1995Snapshot> for En1995Artifact {
    async fn from(snapshot: En1995Snapshot) -> Self {
        Self::from_snapshot(snapshot)
    }
}

impl En1995Artifact {
    pub async fn to_snapshot(&self) -> En1995Snapshot {
        En1995Snapshot {
    ...
    pub async fn from_snapshot(snapshot: En1995Snapshot) -> Self {
        Self {
    ...
    pub async fn set_snapshot(&mut self, snapshot: En1995Snapshot) {
...
//#region 🔖️Descriptor
pub async fn en1995_artifact_schema_descriptor() -> schema::ArtifactSchemaDescriptor {
...
impl AnnexParams {
    ...
    pub async fn gamma_m(&self, material: TimberMaterial) -> f64 {
        match material {
    ...
    pub async fn k_cr(&self, f_v_k_mpa: f64) -> f64 {
        match self.choice {
            AnnexChoice::En => 0.67,
...
pub async fn k_mod(service: ServiceClass, duration: LoadDuration) -> f64 {
...
pub async fn k_def(service: ServiceClass, duration: LoadDuration) -> f64 {
    match (service, duration) {
...
pub async fn k_crit(lambda_rel_m: f64) -> f64 {
...
pub async fn lambda_rel_m(w_eff_mm3: f64, f_m_k_mpa: f64, m_crit_knm: f64) -> f64 {
...
pub mod part_1_1 {
    ...
    pub async fn bending_resistance_knm(w_mm3: f64, f_m_k_mpa: f64, k_mod: f64, k_crit: f64, annex: AnnexChoice) -> f64 {
        let gamma_m = AnnexParams::for_annex(annex).gamma_m(TimberMaterial::Glulam);
        k_mod * k_crit * w_mm3 * f_m_k_mpa / gamma_m / 1_000_000.0
    }

    pub async fn compression_resistance_kn(a_mm2: f64, f_c_0_k_mpa: f64, k_mod: f64, annex: AnnexChoice) -> f64 {
        let gamma_m = AnnexParams::for_annex(annex).gamma_m(TimberMaterial::Glulam);
        k_mod * a_mm2 * f_c_0_k_mpa / gamma_m / 1000.0
    }

    pub async fn connection_bearing_resistance_kn(a_ef_mm2: f64, f_v_k_mpa: f64, k_mod: f64, annex: AnnexChoice) -> f64 {
    ...
    pub async fn shear_stress_mpa(v_ed_kn: f64, b_mm: f64, h_mm: f64, k_cr: f64) -> f64 {
        let b_ef_mm = k_cr * b_mm;
        1.5 * v_ed_kn * 1000.0 / (b_ef_mm * h_mm)
    }

    /// 📐️ Shear design resistance f_v,d [MPa] per EN 1995-1-1 §2.4.1, Eq. 2.14.
    pub async fn shear_resistance_mpa(f_v_k_mpa: f64, k_mod: f64, annex: AnnexChoice) -> f64 {
        let gamma_m = AnnexParams::for_annex(annex).gamma_m(TimberMaterial::Solid);
        k_mod * f_v_k_mpa / gamma_m
    }

    pub async fn check_bending(m_ed: f64, m_rd: f64, annex: AnnexChoice) -> CheckResult {
    ...
    pub async fn check_compression(n_ed_kn: f64, n_rd_kn: f64, annex: AnnexChoice) -> CheckResult {
    ...
    pub async fn check_connection_bearing(f_ed_kn: f64, f_rd_kn: f64, annex: AnnexChoice) -> CheckResult {
    ...
    pub async fn check_shear(v_ed_kn: f64, b_mm: f64, h_mm: f64, f_v_k_mpa: f64, k_mod: f64, annex: AnnexChoice) -> CheckResult {
...
pub mod part_1_2 {
    ...
    pub async fn charred_depth_mm(fire_duration_min: f64) -> f64 {
        CHARRING_RATE_MM_MIN * fire_duration_min
    }

    pub async fn residual_section_mm(original_mm: f64, charred_depth_mm: f64) -> f64 {
        (original_mm - 2.0 * charred_depth_mm).max(0.0)
    }

    pub async fn check_fire(charred_depth_mm: f64, remaining_mm: f64) -> CheckResult {
...
pub mod part_2 {
    ...
    pub async fn bridge_bending_resistance_knm(w_mm3: f64, f_m_k_mpa: f64, service: ServiceClass, duration: LoadDuration, k_crit: f64, annex: AnnexChoice) -> f64 {
        part_1_1::bending_resistance_knm(w_mm3, f_m_k_mpa, k_mod(service, duration), k_crit, annex)
    }

    pub async fn check_bridge_timber(m_ed: f64, m_rd: f64) -> CheckResult {
    ...
    pub async fn check_pedestrian_vibration(a_vert_m_s2: f64) -> CheckResult {
    ...
    pub async fn fatigue_reduction_factor(n_cycles: f64) -> f64 {
    ...
    pub async fn check_bridge_fatigue(m_ed_knm: f64, m_rd_knm: f64, n_cycles: f64) -> CheckResult {
...
pub async fn check_glulam_beam(
    m_ed_knm: f64,
    n_ed_kn: f64,
    v_ed_kn: f64,
    w_mm3: f64,
