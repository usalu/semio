...
use crate::editor::en1995::commands::{evaluate, selected_check, set_snapshot};
...
pub const CONFIG_SCHEMA: &str = "config.norm.en1995";
...
const NORM_BOUNDED_PAYLOAD_SCHEMA: &str = "semio.norm.en1995/v1.tool-command.v1";
...
fn norm_bounded_extent(command: &En1995Command, _snapshot: &En1995Snapshot, _interaction: &protocol::InteractionState) -> Option<usize> {
    NORM_BOUNDED_TOOL_IDS.contains(&command.command_id()).then_some(1)
}

fn norm_bounded_reduce(
    command: &En1995Command,
...
) -> Result<Emit<En1995Mutation, NormConfigMutation, NoDraftMutation>, Fault> {
...
impl semio_framework::ToolJobFactory for BoundedFirstStepCommandJobFactory {
    type Payload = ArtifactRetainedCommandPayload<EditorApp<En1995PlayApp>>;
    type Job = ArtifactRetainedCommandJob<EditorApp<En1995PlayApp>>;
...
impl semio_framework_plugin::ArtifactOwnedToolJobFactory for BoundedFirstStepCommandJobFactory {
    type Owner = EditorApp<En1995PlayApp>;
    const TOOL_IDS: &'static [&'static str] = NORM_BOUNDED_TOOL_IDS;
    const DOCUMENT_SCHEMA: &'static str = "semio.norm.en1995/v1";
}
//#endregion 🧵️RetainedCommands

impl ArtifactEditor for En1995PlayApp {
    ...
    type Command = En1995Command;

    const DIALECT: Dialect = crate::artifacts::en1995::EN1995_DIALECT;
    const DOCUMENT_SCHEMA: &'static str = "semio.norm.en1995/v1";

    semio_framework_plugin::bounded_first_step_tool_proofs! {
        owner: semio_framework_plugin::EditorApp<En1995PlayApp>,
        owner_file: "✏️s/🔌️plugins/📕️norm/🗿️artifacts/📘️en1995/🏅️standards/🔖️1/🪆️subsets/✳️any/✏️editor/🦀️component.rs",
        controller: "s.norm.en1995@1/*#editor",
        document_schema: "semio.norm.en1995/v1",
    ...
    fn command_id(command: &En1995Command) -> &'static str {
        command.command_id()
    }

    fn handle(
        command: &En1995Command,
        doc: &ArtifactView<'_, En1995Snapshot>,
        cfg: &ConfigView<'_, NormConfig>,
        _interaction: &InteractionView<'_>,
        _draft: &DraftView<'_, Self::Draft>,
        _engines: &EngineHandles,
    ) -> Result<Emit<En1995Mutation, NormConfigMutation, Self::DraftMutation>, Fault> {
        command.dispatch(doc, cfg)
    }

    fn render(body_key: &str, doc: &ArtifactView<'_, En1995Snapshot>, cfg: &ConfigView<'_, NormConfig>) -> UiNode {
        let host = NormHost::<En1995Family>::from_document(doc.snapshot.clone());
        match body_key {
...
impl crate::document::NormFamily for En1995Family {
    ...
    fn evaluate(document: &En1995Snapshot) -> crate::document::CheckReport {
        crate::artifacts::en1995::standards::v1::subsets::any::schema::inferences::evaluate(document)
    }
}
...
pub fn create_en1995_app() -> semio_framework_plugin::AppDefinition {
    Editor::builder(crate::artifacts::en1995::EN1995_DIALECT)
            .document(["semio", "norm", VARIANT])
            .artifact_kind(crate::artifacts::en1995::artifact_kind())
            ...
            .mutation("setSnapshot", LocalizedLabel::native("Set En1995Snapshot", "Dokument setzen"))
...
pub(crate) mod testkit {
    ...
    pub fn en1995_manifest_for_testkit() -> semio_framework_plugin::App {
    ...
    pub fn new_app() -> NormApp {
        sdk_new_app::<EditorApp<En1995PlayApp>>()
    }
    ...
    pub fn app_with_registry() -> NormApp {
        new_app_with_registry::<EditorApp<En1995PlayApp>>(en1995_manifest_for_testkit)
    }

    pub fn dispatch(app: &mut NormApp, command: En1995Command) -> InvocationResult {
...
mod tests {
    ...
    fn the_manifest_stitches_every_taxonomy_node() {
        let definition = create_en1995_app().definition;
    ...
    fn declares_model_in_and_report_out_ports() {
        let ports = create_en1995_app().definition.io.ports;
    ...
    fn an_unknown_body_key_falls_back_to_a_text_node() {
        ...
        assert!(testkit::render(&mut app, "norm.en1995.play.nope").contains("Unknown body"));
    }
    ...
    fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, En1995Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1995Snapshot::default() }));
    ...
    fn evaluate_recommits_the_current_projection_without_changing_it() {
        ...
        testkit::dispatch(&mut app, En1995Command::Evaluate(evaluate::Evaluate {}));
    ...
    fn selected_check_index_is_a_config_only_edit() {
        let mut app = testkit::new_app();
        let before = app.snapshot().expect("projection");
        let result = testkit::dispatch(&mut app, En1995Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) }));
    ...
    fn view_actions_never_emit_artifact_mutations_under_the_real_registry() {
        ...
        let result = testkit::dispatch(&mut app, En1995Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(1) }));
