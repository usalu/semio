...
fn raises_the_design_shear_force_to_22_5_kn() {
...
/// `v_ed_kn`.
#[semio_framework_async_macros::async_test]
fn restoring_15_kn_restores_before() {
    let base = before();
...
fn declared_outcome_holds() {
    ...
    assert_eq!(
        ...
        None,
        "change-v-ed-kn/raises-the-design-shear-force-to-22-5-kn: the payload is finite, so `change-v-ed-kn`'s `mutation.invariant` fatal cannot fire, and 22.5 differs from the committed 15.0, so its `mutation.no-op` warning cannot either"
    );
