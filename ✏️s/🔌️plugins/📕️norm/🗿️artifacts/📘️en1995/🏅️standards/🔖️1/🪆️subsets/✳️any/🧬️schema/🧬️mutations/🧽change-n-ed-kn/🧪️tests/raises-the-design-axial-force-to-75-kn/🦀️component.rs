...
async fn raises_the_design_axial_force_to_75_kn() {
...
/// `n_ed_kn`.
#[semio_framework_async_macros::async_test]
async fn restoring_50_kn_restores_before() {
    let base = before();
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(produced.worst_level(), None, "change-n-ed-kn/raises-the-design-axial-force-to-75-kn: the payload is finite, so `change-n-ed-kn`'s `mutation.invariant` fatal cannot fire, and 75.0 differs from the committed 50.0, so its `mutation.no-op` warning cannot either");
