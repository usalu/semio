pub(crate) mod context {
    ...
    pub fn din18599_manifest_for_tests() -> semio_framework_plugin::App {
    ...
    pub async fn app_with_registry() -> NormApp {
        new_app_with_registry::<EditorApp<Din18599PlayApp>>(din18599_manifest_for_tests).await
    }
...
async fn an_unknown_body_key_falls_back_to_a_text_node() {
    ...
    assert!(context::render(&mut app, "norm.din18599.play.nope").await.contains("Unknown body"));
}
...
async fn selected_check_index_is_a_config_only_edit() {
    let mut app = context::app_with_registry().await;
    let before = app.snapshot().expect("projection");
    let result = context::dispatch(&mut app, Din18599Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) })).await;
...
async fn view_actions_never_emit_artifact_mutations_under_the_real_registry() {
    let mut app = context::app_with_registry().await;
    let result = context::dispatch(&mut app, Din18599Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(1) })).await;
