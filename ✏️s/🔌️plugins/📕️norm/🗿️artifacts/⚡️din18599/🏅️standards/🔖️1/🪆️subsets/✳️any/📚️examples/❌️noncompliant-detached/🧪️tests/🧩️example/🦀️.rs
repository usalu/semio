...
async fn primary_asset_decodes_and_evaluates_claimed_verdict() {
    ...
    let text = crate::examples::noncompliant_detached::PRIMARY_TEXT;
    let doc = <crate::Din18599Snapshot as ArtifactDsl>::parse_dsl(text).expect("parse noncompliant-detached");
    let report = evaluate_document(&doc);
    assert!(!report.complies()); assert!(report.failing().count() >= 2);
}
