...
impl ArtifactEditor for Din18599PlayApp {
    ...
    fn render(body_key: &str, doc: &ArtifactView<'_, Din18599Snapshot>, cfg: &ConfigView<'_, NormConfig>) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::ComponentTree> {
...
impl crate::app_surface::NormRetainedEditor for Din18599PlayApp {
    fn dispatch_retained(command: &Din18599Command, doc: &ArtifactView<'_, Din18599Snapshot>, cfg: &ConfigView<'_, NormConfig>) -> Result<Emit<Din18599Mutation, NormConfigMutation, NoDraftMutation>, Fault> {
        command.dispatch(doc, cfg)
    }
}
...
pub(crate) mod testkit {
    ...
    pub async fn app_with_registry() -> NormApp {
        new_app_with_registry::<EditorApp<Din18599PlayApp>>(din18599_manifest_for_testkit).await
    }

    pub async fn dispatch(app: &mut NormApp, command: Din18599Command) -> InvocationResult {
...
mod tests {
    ...
    fn retained_command_dispositions_match_the_language_neutral_oracle() {
        ...
        let definition = create_din18599_app();
    ...
    async fn declares_model_in_and_report_out_ports() {
        let ports = create_din18599_app().io.ports;
    ...
    async fn an_unknown_body_key_falls_back_to_a_text_node() {
        ...
        assert!(testkit::render(&mut app, "norm.din18599.play.nope").await.contains("Unknown body"));
    }
    ...
    async fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::app_with_registry().await;
        testkit::dispatch(&mut app, Din18599Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { text: crate::document::escape_op_text_field(&<Din18599Snapshot as store::ArtifactDsl>::print_dsl(&Din18599Snapshot::default())) })).await;
    ...
    async fn evaluate_recommits_the_current_projection_without_changing_it() {
        ...
        testkit::dispatch(&mut app, Din18599Command::Evaluate(evaluate::Evaluate {})).await;
    ...
    async fn selected_check_index_is_a_config_only_edit() {
        let mut app = testkit::app_with_registry().await;
        let before = app.snapshot().expect("projection");
        let result = testkit::dispatch(&mut app, Din18599Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) })).await;
    ...
    async fn view_actions_never_emit_artifact_mutations_under_the_real_registry() {
        let mut app = testkit::app_with_registry().await;
        let result = testkit::dispatch(&mut app, Din18599Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(1) })).await;
