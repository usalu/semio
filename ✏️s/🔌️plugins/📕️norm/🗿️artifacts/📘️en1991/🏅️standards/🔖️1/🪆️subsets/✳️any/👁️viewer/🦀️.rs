//! 👁️ EN 1991 viewer — the read-only counterpart of `✏️editor` for this subset (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `En1991Viewer` implements
//! `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<En1991Viewer>` (framework SDK) is
//! the sole runtime adapter, so this file can never structurally emit an artifact or draft mutation.
//! MUST NOT import anything from the sibling editor module (`policyViewerPurityBreaches`).

use crate::artifacts::en1991::En1991Snapshot;
use crate::artifacts::en1991::{EN1991_DIALECT, EN1991_DOCUMENT_SCHEMA};
use crate::viewer::en1991::modes::view;
use crate::viewer::en1991::modes::view::windows::report;
...
pub enum En1991ViewCommand {
    #[default]
    Noop,
}

impl protocol::OpBinary for En1991ViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(En1991ViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
pub struct En1991Viewer;

impl ArtifactViewer for En1991Viewer {
    type Snapshot = En1991Snapshot;
    type Mutation = crate::artifacts::en1991::op::En1991Mutation;
    ...
    type Command = En1991ViewCommand;

    const DIALECT: Dialect = EN1991_DIALECT;
    const DOCUMENT_SCHEMA: &'static str = EN1991_DOCUMENT_SCHEMA;

    fn initial_snapshot() -> En1991Snapshot {
        En1991Snapshot::default()
    }

    /// 👁️ Structurally read-only: the sole `En1991ViewCommand::Noop` variant never carries a config
...
//#region 🔖️Manifest
pub fn create_en1991_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(EN1991_DIALECT)
        .document(["semio", "norm", "en1991"])
...
mod tests {
    ...
    fn create_en1991_viewer_builds_a_definition_for_this_dialect() {
        let def = create_en1991_viewer();
        assert_eq!(def.dialect, EN1991_DIALECT.into());
    }

    #[semio_framework_async_macros::async_test]
    fn viewer_dialect_matches_the_artifact_coordinate() {
        assert_eq!(<En1991Viewer as ArtifactViewer>::DIALECT, EN1991_DIALECT);
    }
    ...
    fn an_unknown_body_key_falls_back_to_a_text_node() {
        ...
        let json = serde_json::to_string(&<En1991Viewer as ArtifactViewer>::render("nope", &doc, &ConfigView { snapshot: &NoConfig::default() })).expect("json");
