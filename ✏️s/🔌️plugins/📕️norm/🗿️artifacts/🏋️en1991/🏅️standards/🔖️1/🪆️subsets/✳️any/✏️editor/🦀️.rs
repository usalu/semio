...
use crate::editor::en1991::commands::{apply_remedy, evaluate, insert_item, remove_item, selected_check, set_active_example, set_field, set_snapshot};
...
pub const CONTROLLER_ID: &str = "s.norm.en1991@1/*#editor";
...
semio_framework_plugin::app_commands! {
    ...
    pub enum En1991Command for En1991Snapshot, En1991Mutation, NoConfig, NoConfigMutation {
...
impl ArtifactEditor for En1991PlayApp {
    ...
    fn examples() -> Vec<semio_framework_plugin::ExampleSource> {
        vec![crate::de_office_compliant::source(), crate::multi_fail_noncompliant::source()]
    }
    ...
    const DIALECT: Dialect = crate::EN1991_DIALECT;
    ...
    semio_framework_plugin::bounded_first_step_tool_proofs! {
        ...
        artifact_schema: "semio.norm.en1991/v1",
    ...
    semio_s_artifact_norm_contract::norm_command_from_action!(En1991Command, crate::standards::v1::subsets::any::schema::snapshot::decode_en1991_snapshot_json);
    ...
    ) -> Result<Emit<En1991Mutation, NoConfigMutation, Self::DraftMutation>, Fault> {
    ...
    fn import_media(port: &str, media: &Media, doc: &ArtifactView<'_, En1991Snapshot>) -> Result<Emit<En1991Mutation, NoConfigMutation, Self::DraftMutation>, MediaError> {
        let base = doc.snapshot.clone();
        crate::app_surface::import_media(port, media, move |snapshot: En1991Snapshot| En1991Mutation::from_snapshot(&base, &snapshot))
    }
...
impl crate::app_surface::NormRetainedEditor for En1991PlayApp {
    type Family = En1991Family;

    type ResultsWindowConfigOwner = results::ResultsWindowConfigOwner;

    fn selected_check_window_mutation(command: &En1991Command) -> Option<crate::results_window_config::NormResultsWindowConfigMutation> {
        match command {
            En1991Command::SetSelectedCheckIndex(payload) => Some(selected_check::window_mutation(payload)),
            _ => None,
        }
    }

    fn dispatch_retained(command: &En1991Command, doc: &ArtifactView<'_, En1991Snapshot>, cfg: &ConfigView<'_, NoConfig>) -> Result<Emit<En1991Mutation, NoConfigMutation, NoDraftMutation>, Fault> {
...
pub fn create_en1991_app() -> semio_framework_plugin::AppDefinition {
    Editor::builder(crate::EN1991_DIALECT)
            ...
            .action_describe("setSnapshot", LocalizedLabel::native("Replaces the whole EN 1991 (Eurocode 1, actions on structures) compliance document with the supplied document JSON; the previous inputs are discarded.", "Ersetzt das gesamte Nachweisdokument nach EN 1991 (Eurocode 1, Einwirkungen auf Tragwerke) durch das übergebene Dokument-JSON; die bisherigen Eingaben werden verworfen."))
            .action_describe("evaluate", LocalizedLabel::native("Recomputes every EN 1991 (Eurocode 1, actions on structures) check from the document's inputs and refreshes the results window; the document is not changed.", "Berechnet alle Nachweise nach EN 1991 (Eurocode 1, Einwirkungen auf Tragwerke) aus den Eingaben des Dokuments neu und aktualisiert das Ergebnisfenster; das Dokument ändert sich nicht."))
            ...
            .action_describe("setActiveExample", LocalizedLabel::native("Loads one of the bundled EN 1991 (Eurocode 1, actions on structures) examples into the open compliance document, replacing its inputs, by example id.", "Lädt eines der mitgelieferten Beispiele nach EN 1991 (Eurocode 1, Einwirkungen auf Tragwerke) in das offene Nachweisdokument und ersetzt dessen Eingaben, anhand der Beispiel-Id."))
            .build_definition()
}
