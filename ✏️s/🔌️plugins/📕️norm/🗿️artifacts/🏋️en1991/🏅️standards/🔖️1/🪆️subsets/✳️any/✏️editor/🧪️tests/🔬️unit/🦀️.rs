pub(crate) mod context {
    ...
    pub fn en1991_manifest_for_tests() -> semio_framework_plugin::App {
    ...
    pub async fn app_with_registry() -> NormApp {
        let mut app = new_app_with_registry::<EditorApp<En1991PlayApp>>(en1991_manifest_for_tests).await;
    ...
    /// driven to its terminal-empty shallow shell, so every fixture that mounts an app must end here.
...
fn every_command() -> Vec<En1991Command> {
    vec![
        ...
        En1991Command::SetActiveExample(set_active_example::SetActiveExample { example_id: String::new() }),
    ]
}
...
async fn an_unknown_body_key_falls_back_to_a_text_node() {
    ...
    assert!(context::render(&mut app, "norm.en1991.play.nope").await.contains("Unknown body"));
...
async fn set_snapshot_commits_a_host_backed_report() {
    let mut app = context::app_with_registry().await;
    context::dispatch(&mut app, En1991Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1991Snapshot::default() })).await;
    context::dispatch(&mut app, En1991Command::Evaluate(evaluate::Evaluate {})).await;
    let host = NormHost::<En1991Family>::from_artifact(app.snapshot().expect("projection"));
...
async fn selected_check_index_is_a_config_only_edit() {
    let mut app = context::app_with_registry().await;
    let before = app.snapshot().expect("projection");
    let result = context::dispatch(&mut app, En1991Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) })).await;
...
async fn view_actions_never_emit_artifact_mutations_under_the_real_registry() {
    let mut app = context::app_with_registry().await;
    let result = context::dispatch(&mut app, En1991Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(1) })).await;
