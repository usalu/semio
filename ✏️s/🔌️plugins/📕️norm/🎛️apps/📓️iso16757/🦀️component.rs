//! 📗️ ISO 16757 play app — the `DocumentApp` impl (dispatch-only), the aggregated command enum and
//! the manifest stitch.
//!
...
use crate::apps::iso16757::commands::{evaluate, selected_check, set_document};
...
//#region 🔖️Constants
pub const APP_ID: &str = "norm-iso-16757-play";
...
semio_framework_plugin::app_commands! {
    ...
    pub enum Iso16757Command for Document, Iso16757Mutation, NormConfig, NormConfigMutation {
...
impl DocumentApp for Iso16757PlayApp {
    ...
    const APP_ID: &'static str = "norm-iso-16757-play";
    ...
    fn handle(command: &Iso16757Command, doc: &DocumentView<'_, Document>, cfg: &ConfigView<'_, NormConfig>, _draft: &DraftView<'_, Self::Draft>, _engines: &EngineHandles) -> Result<Emit<Iso16757Mutation, NormConfigMutation, Self::DraftMutation>, Fault> {
        command.dispatch(doc, cfg)
    }

    fn render(body_key: &str, doc: &DocumentView<'_, Document>, cfg: &ConfigView<'_, NormConfig>) -> UiNode {
        let host = NormHost::<Iso16757Family>::from_document(doc.projection.clone());
        match body_key {
    ...
    fn export_media(port: &str, doc: &DocumentView<'_, Document>) -> Result<Media, MediaError> {
        crate::app_surface::export_media::<Iso16757Family>(port, VARIANT, DOCUMENT_SCHEMA, doc.projection)
    }

    /// 🎞️ `"model:in"`/`"document:in"` — see `crate::app_surface::import_media`.
    fn import_media(port: &str, media: &Media, _doc: &DocumentView<'_, Document>) -> Result<Emit<Iso16757Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
...
pub(crate) mod testkit {
    ...
    pub type NormApp = VcsDocumentApp<Iso16757PlayApp>;

    pub fn new_app() -> NormApp {
        sdk_new_app::<Iso16757PlayApp>()
    }
    ...
    pub fn app_with_registry() -> NormApp {
        new_app_with_registry::<Iso16757PlayApp>(create_iso16757_app)
    }
...
mod tests {
    ...
    fn every_command() -> Vec<Iso16757Command> {
        vec![
            Iso16757Command::SetDocument(set_document::SetDocument { document: Document::default() }),
    ...
    fn set_document_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, Iso16757Command::SetDocument(set_document::SetDocument { document: Document::default() }));
        let host = NormHost::<Iso16757Family>::from_document(app.projection().expect("projection"));
