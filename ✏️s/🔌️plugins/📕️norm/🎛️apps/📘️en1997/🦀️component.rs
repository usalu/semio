//! ⛰️ EN 1997 play app — the `DocumentApp` impl (dispatch-only), the aggregated command enum and
//! the manifest stitch.
//!
...
//! `crate::artifacts::en1997::engine`, and everything the fifteen norm apps share verbatim (config,
//! media ports, render primitives, manifest constructors) in `crate::document::app` / `crate::document::config`.

use crate::apps::en1997::commands::{evaluate, selected_check, set_snapshot};
use crate::apps::en1997::modes::edit as edit_mode;
use crate::apps::en1997::modes::edit::windows::{inputs, results};
use crate::apps::en1997::panels::{catalogue as catalogue_panel, document as document_panel, inspection as inspection_panel};
...
//#region 🔖️Constants
pub const APP_ID: &str = "norm-en-1997-play";
...
semio_framework_plugin::app_commands! {
    /// 🎯️ `En1997PlayApp::Command` — the SOLE dispatch surface for this app's own behavior, covering every
...
//#region 🔖️En1997PlayApp
#[derive(Default)]
pub struct En1997PlayApp;

impl DocumentApp for En1997PlayApp {
    ...
    const APP_ID: &'static str = "norm-en-1997-play";
    ...
    fn handle(command: &En1997Command, doc: &DocumentView<'_, En1997Snapshot>, cfg: &ConfigView<'_, NormConfig>, _draft: &DraftView<'_, Self::Draft>, _engines: &EngineHandles) -> Result<Emit<En1997Mutation, NormConfigMutation, Self::DraftMutation>, Fault> {
        command.dispatch(doc, cfg)
    }

    fn render(body_key: &str, doc: &DocumentView<'_, En1997Snapshot>, cfg: &ConfigView<'_, NormConfig>) -> UiNode {
    ...
    fn export_media(port: &str, doc: &DocumentView<'_, En1997Snapshot>) -> Result<Media, MediaError> {
    ...
    fn import_media(port: &str, media: &Media, _doc: &DocumentView<'_, En1997Snapshot>) -> Result<Emit<En1997Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
        crate::app_surface::import_media::<En1997Snapshot>(port, media)
    }
    //#endregion 🔖️MediaPorts
}
//#endregion 🔖️En1997PlayApp

//#region 🔖️Manifest
pub fn create_en1997_app() -> App {
    App::from_builder(
        App::builder(APP_ID, LocalizedLabel::data(LABEL))
            .document(["semio", "norm", VARIANT])
            .artifact_kind(crate::artifacts::en1997::artifact_kind())
    ...
    .example("default", LocalizedLabel::native("Default", "Standard"), serde_json::to_string(&En1997Snapshot::default()).expect("default document serializes"), "file")
...
pub(crate) mod testkit {
    ...
    pub type NormApp = VcsDocumentApp<En1997PlayApp>;

    pub fn new_app() -> NormApp {
        sdk_new_app::<En1997PlayApp>()
    }
    ...
    pub fn app_with_registry() -> NormApp {
        new_app_with_registry::<En1997PlayApp>(create_en1997_app)
    }

    pub fn dispatch(app: &mut NormApp, command: En1997Command) -> InvocationResult {
...
mod tests {
    ...
    /// 🎯️ One value per `En1997Command` row — the whole-command-surface laws below iterate it, so a new row
    /// that is not listed here fails `command_ids_cover_every_row`.
    fn every_command() -> Vec<En1997Command> {
        vec![
            En1997Command::SetSnapshot(set_snapshot::SetSnapshot { snapshot: En1997Snapshot::default() }),
    ...
    fn the_manifest_stitches_every_taxonomy_node() {
        let definition = create_en1997_app().definition;
    ...
    fn declares_model_in_and_report_out_ports() {
        let ports = create_en1997_app().definition.io.ports;
    ...
    fn an_unknown_body_key_falls_back_to_a_text_node() {
        ...
        assert!(testkit::render(&mut app, "norm.en1997.play.nope").contains("Unknown body"));
    }
    ...
    fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, En1997Command::SetSnapshot(set_snapshot::SetSnapshot { snapshot: En1997Snapshot::default() }));
        let host = NormHost::<En1997Family>::from_document(app.projection().expect("projection"));
    ...
    fn evaluate_recommits_the_current_projection_without_changing_it() {
        ...
        testkit::dispatch(&mut app, En1997Command::Evaluate(evaluate::Evaluate {}));
    ...
    fn selected_check_index_is_a_config_only_edit() {
        let mut app = testkit::new_app();
        let before = app.projection().expect("projection");
        let result = testkit::dispatch(&mut app, En1997Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) }));
    ...
    fn view_actions_never_emit_document_mutations_under_the_real_registry() {
        ...
        let result = testkit::dispatch(&mut app, En1997Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(1) }));
    ...
    fn undo_redo_round_trips_through_the_wrapper() {
        ...
        assert_eq!(app.projection().expect("projection"), En1997Snapshot::default());
    }
