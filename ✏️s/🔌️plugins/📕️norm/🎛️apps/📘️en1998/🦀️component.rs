//! 🌍️ EN 1998 play app — the `ArtifactApp` impl (dispatch-only), the aggregated command enum and
//! the manifest stitch.
//!
...
use crate::apps::en1998::commands::{evaluate, selected_check, set_snapshot};
...
impl ArtifactApp for En1998PlayApp {
    ...
    fn handle(command: &En1998Command, doc: &ArtifactView<'_, En1998Snapshot>, cfg: &ConfigView<'_, NormConfig>, _draft: &DraftView<'_, Self::Draft>, _engines: &EngineHandles) -> Result<Emit<En1998Mutation, NormConfigMutation, Self::DraftMutation>, Fault> {
        command.dispatch(doc, cfg)
    }

    fn render(body_key: &str, doc: &ArtifactView<'_, En1998Snapshot>, cfg: &ConfigView<'_, NormConfig>) -> UiNode {
...
pub fn create_en1998_app() -> App {
    ...
    .example("default", LocalizedLabel::native("Default", "Standard"), serde_json::to_string(&En1998Snapshot::default()).expect("default document serializes"), "file")
...
pub(crate) mod testkit {
    ...
    pub type NormApp = VcsArtifactApp<En1998PlayApp>;
...
mod tests {
    ...
    fn every_command() -> Vec<En1998Command> {
        vec![
            En1998Command::SetSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1998Snapshot::default() }),
    ...
    fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, En1998Command::SetSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1998Snapshot::default() }));
