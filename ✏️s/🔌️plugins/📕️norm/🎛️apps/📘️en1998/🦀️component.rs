//! 🌍️ EN 1998 play app — the `DocumentApp` impl (dispatch-only), the aggregated command enum and
//! the manifest stitch.
//!
...
//! `crate::artifacts::en1998::engine`, and everything the fifteen norm apps share verbatim (config,
//! media ports, render primitives, manifest constructors) in `crate::document::app` / `crate::document::config`.

use crate::apps::en1998::commands::{evaluate, selected_check, set_document};
use crate::apps::en1998::modes::edit as edit_mode;
use crate::apps::en1998::modes::edit::windows::{inputs, results};
use crate::apps::en1998::panels::{catalogue as catalogue_panel, document as document_panel, inspection as inspection_panel};
...
//#region 🔖️Constants
pub const APP_ID: &str = "norm-en-1998-play";
...
semio_framework_plugin::app_commands! {
    /// 🎯️ `En1998PlayApp::Command` — the SOLE dispatch surface for this app's own behavior, covering every
    ...
    pub enum En1998Command for Document, En1998Mutation, NormConfig, NormConfigMutation {
...
//#region 🔖️En1998PlayApp
#[derive(Default)]
pub struct En1998PlayApp;

impl DocumentApp for En1998PlayApp {
    ...
    const APP_ID: &'static str = "norm-en-1998-play";
    ...
    fn handle(command: &En1998Command, doc: &DocumentView<'_, Document>, cfg: &ConfigView<'_, NormConfig>, _draft: &DraftView<'_, Self::Draft>, _engines: &EngineHandles) -> Result<Emit<En1998Mutation, NormConfigMutation, Self::DraftMutation>, Fault> {
        command.dispatch(doc, cfg)
    }

    fn render(body_key: &str, doc: &DocumentView<'_, Document>, cfg: &ConfigView<'_, NormConfig>) -> UiNode {
        let host = NormHost::<En1998Family>::from_document(doc.projection.clone());
        match body_key {
    ...
    fn export_media(port: &str, doc: &DocumentView<'_, Document>) -> Result<Media, MediaError> {
        crate::app_surface::export_media::<En1998Family>(port, VARIANT, DOCUMENT_SCHEMA, doc.projection)
    }

    /// 🎞️ `"model:in"`/`"document:in"` — see `crate::app_surface::import_media`.
    fn import_media(port: &str, media: &Media, _doc: &DocumentView<'_, Document>) -> Result<Emit<En1998Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
        crate::app_surface::import_media::<Document>(port, media)
    }
    //#endregion 🔖️MediaPorts
}
//#endregion 🔖️En1998PlayApp

//#region 🔖️Manifest
pub fn create_en1998_app() -> App {
    App::from_builder(
...
pub(crate) mod testkit {
    ...
    pub type NormApp = VcsDocumentApp<En1998PlayApp>;

    pub fn new_app() -> NormApp {
        sdk_new_app::<En1998PlayApp>()
    }
    ...
    pub fn app_with_registry() -> NormApp {
        new_app_with_registry::<En1998PlayApp>(create_en1998_app)
    }
...
mod tests {
    ...
    /// 🎯️ One value per `En1998Command` row — the whole-command-surface laws below iterate it, so a new row
    /// that is not listed here fails `command_ids_cover_every_row`.
    fn every_command() -> Vec<En1998Command> {
        vec![
            En1998Command::SetDocument(set_document::SetDocument { document: Document::default() }),
    ...
    fn set_document_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, En1998Command::SetDocument(set_document::SetDocument { document: Document::default() }));
