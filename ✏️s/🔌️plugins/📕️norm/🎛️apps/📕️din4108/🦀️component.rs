...
impl ArtifactApp for Din4108PlayApp {
    ...
    fn import_media(port: &str, media: &Media, _doc: &ArtifactView<'_, Din4108Snapshot>) -> Result<Emit<Din4108Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
        crate::app_surface::import_media(port, media, |snapshot| vec![Din4108Mutation::SetSnapshot { snapshot }])
    }
...
pub fn create_din4108_app() -> App {
    ...
    .example("default", LocalizedLabel::native("Default", "Standard"), serde_json::to_string(&Din4108Snapshot::default()).expect("default document serializes"), "file")
...
mod tests {
    ...
    fn every_command() -> Vec<Din4108Command> {
        vec![
            Din4108Command::SetSnapshot(set_snapshot::SetSnapshot { snapshot: Din4108Snapshot::default() }),
    ...
    fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, Din4108Command::SetSnapshot(set_snapshot::SetSnapshot { snapshot: Din4108Snapshot::default() }));
