...
impl DocumentApp for En1993PlayApp {
    ...
    fn import_media(port: &str, media: &Media, _doc: &DocumentView<'_, En1993Snapshot>) -> Result<Emit<En1993Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
        crate::app_surface::import_media(port, media, |snapshot| En1993Mutation::SetSnapshot { snapshot })
    }
...
mod tests {
    ...
    fn every_command() -> Vec<En1993Command> {
        vec![
            En1993Command::SetSnapshot(set_snapshot::SetSnapshot { snapshot: En1993Snapshot::default() }),
    ...
    fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, En1993Command::SetSnapshot(set_snapshot::SetSnapshot { snapshot: En1993Snapshot::default() }));
