//! 🔩️ EN 1993 play app — the `ArtifactApp` impl (dispatch-only), the aggregated command enum and
//! the manifest stitch.
//!
...
impl ArtifactApp for En1993PlayApp {
    ...
    fn handle(command: &En1993Command, doc: &ArtifactView<'_, En1993Snapshot>, cfg: &ConfigView<'_, NormConfig>, _draft: &DraftView<'_, Self::Draft>, _engines: &EngineHandles) -> Result<Emit<En1993Mutation, NormConfigMutation, Self::DraftMutation>, Fault> {
    ...
    fn import_media(port: &str, media: &Media, _doc: &ArtifactView<'_, En1993Snapshot>) -> Result<Emit<En1993Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
        crate::app_surface::import_media(port, media, |snapshot| vec![En1993Mutation::SetSnapshot { snapshot }])
    }
