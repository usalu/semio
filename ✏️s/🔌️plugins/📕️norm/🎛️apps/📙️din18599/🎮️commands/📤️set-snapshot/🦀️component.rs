...
//! impl already uses), NOT a nested `#[dsl(block)]` struct field — `Din18599Snapshot` no longer
...
mod tests {
    ...
    fn handle_preserves_full_f64_precision_through_the_payload() {
        ...
        let config = NormConfig::default();
        let text = crate::document::escape_op_text_field(&<Din18599Snapshot as store::ArtifactDsl>::print_dsl(&Din18599Snapshot::default()));
        let emit = handle(&ReplaceSnapshot { text }, &ArtifactView { snapshot: &projection, history: &HistoryView::empty() }, &ConfigView { snapshot: &config }).expect("handle");
