...
semio_framework_plugin::app_commands! {
    ...
    pub enum Din18599Command for Document, Din18599Mutation, NormConfig, NormConfigMutation {
...
impl DocumentApp for Din18599PlayApp {
    ...
    fn handle(command: &Din18599Command, doc: &DocumentView<'_, Document>, cfg: &ConfigView<'_, NormConfig>, _draft: &DraftView<'_, Self::Draft>, _engines: &EngineHandles) -> Result<Emit<Din18599Mutation, NormConfigMutation, Self::DraftMutation>, Fault> {
    ...
    fn export_media(port: &str, doc: &DocumentView<'_, Document>) -> Result<Media, MediaError> {
        crate::app_surface::export_media::<DinV18599Family>(port, VARIANT, DOCUMENT_SCHEMA, doc.projection)
    }

    /// 🎞️ `"model:in"`/`"document:in"` — see `crate::app_surface::import_media`.
    fn import_media(port: &str, media: &Media, _doc: &DocumentView<'_, Document>) -> Result<Emit<Din18599Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
