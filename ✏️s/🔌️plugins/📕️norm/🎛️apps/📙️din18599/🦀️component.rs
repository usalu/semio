...
use crate::apps::din18599::commands::{evaluate, selected_check, set_snapshot};
...
impl DocumentApp for Din18599PlayApp {
    ...
    fn handle(command: &Din18599Command, doc: &DocumentView<'_, Din18599Snapshot>, cfg: &ConfigView<'_, NormConfig>, _draft: &DraftView<'_, Self::Draft>, _engines: &EngineHandles) -> Result<Emit<Din18599Mutation, NormConfigMutation, Self::DraftMutation>, Fault> {
        command.dispatch(doc, cfg)
    }

    fn render(body_key: &str, doc: &DocumentView<'_, Din18599Snapshot>, cfg: &ConfigView<'_, NormConfig>) -> UiNode {
    ...
    fn export_media(port: &str, doc: &DocumentView<'_, Din18599Snapshot>) -> Result<Media, MediaError> {
    ...
    fn import_media(port: &str, media: &Media, _doc: &DocumentView<'_, Din18599Snapshot>) -> Result<Emit<Din18599Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
        crate::app_surface::import_media(port, media, |snapshot| Din18599Mutation::SetSnapshot { snapshot })
    }
...
pub fn create_din18599_app() -> App {
    ...
    .example("default", LocalizedLabel::native("Default", "Standard"), serde_json::to_string(&Din18599Snapshot::default()).expect("default document serializes"), "file")
...
mod tests {
    ...
    fn every_command() -> Vec<Din18599Command> {
        vec![
            Din18599Command::SetSnapshot(set_snapshot::SetSnapshot { snapshot: Din18599Snapshot::default() }),
    ...
    fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, Din18599Command::SetSnapshot(set_snapshot::SetSnapshot { snapshot: Din18599Snapshot::default() }));
