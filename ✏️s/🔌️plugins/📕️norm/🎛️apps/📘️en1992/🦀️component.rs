...
use crate::apps::en1992::commands::{evaluate, selected_check, set_snapshot};
...
impl DocumentApp for En1992PlayApp {
    ...
    fn import_media(port: &str, media: &Media, _doc: &DocumentView<'_, En1992Snapshot>) -> Result<Emit<En1992Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
        crate::app_surface::import_media(port, media, |snapshot| En1992Mutation::SetSnapshot { snapshot })
    }
...
mod tests {
    ...
    fn every_command() -> Vec<En1992Command> {
        vec![
            En1992Command::SetSnapshot(set_snapshot::SetSnapshot { snapshot: En1992Snapshot::default() }),
    ...
    fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, En1992Command::SetSnapshot(set_snapshot::SetSnapshot { snapshot: En1992Snapshot::default() }));
