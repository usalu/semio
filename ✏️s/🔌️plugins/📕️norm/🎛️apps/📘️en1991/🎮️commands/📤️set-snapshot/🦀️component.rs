//! 📤️ EN 1991 play app command — replace the whole compliance document.
//!
...
//! `En1991Mutation::from_snapshot`, bundled into a single atomic edit.
...
pub struct ReplaceSnapshot {
    #[dsl(block)]
    pub snapshot: En1991Snapshot,
}
//#endregion 🔖️Payload

//#region 🔖️Handler
pub fn handle(payload: &ReplaceSnapshot, _doc: &ArtifactView<'_, En1991Snapshot>, _cfg: &ConfigView<'_, NormConfig>) -> Result<Emit<En1991Mutation, NormConfigMutation>, Fault> {
    crate::app_surface::commit_snapshot_fields(En1991Mutation::from_snapshot(&payload.snapshot), "setSnapshot")
}
//#endregion 🔖️Handler
...
mod tests {
    ...
    fn handle_commits_the_payload_document_under_its_action_id() {
        ...
        let emit = handle(&ReplaceSnapshot { snapshot: En1991Snapshot::default() }, &ArtifactView::new(&projection, &HistoryView::empty()), &ConfigView { snapshot: &config }).expect("handle");
        assert_eq!(emit.artifact_mutations, En1991Mutation::from_snapshot(&En1991Snapshot::default()));
