...
use crate::apps::en1991::commands::{evaluate, selected_check, set_snapshot};
...
impl DocumentApp for En1991PlayApp {
    ...
    fn import_media(port: &str, media: &Media, _doc: &DocumentView<'_, En1991Snapshot>) -> Result<Emit<En1991Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
        crate::app_surface::import_media(port, media, |snapshot| En1991Mutation::SetSnapshot { snapshot })
    }
...
mod tests {
    ...
    fn every_command() -> Vec<En1991Command> {
        vec![
            En1991Command::SetSnapshot(set_snapshot::SetSnapshot { snapshot: En1991Snapshot::default() }),
    ...
    fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, En1991Command::SetSnapshot(set_snapshot::SetSnapshot { snapshot: En1991Snapshot::default() }));
