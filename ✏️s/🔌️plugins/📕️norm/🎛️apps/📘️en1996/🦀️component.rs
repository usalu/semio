//! 🧿️ EN 1996 play app — the `DocumentApp` impl (dispatch-only), the aggregated command enum and
//! the manifest stitch.
//!
...
//! `crate::artifacts::en1996::engine`, and everything the fifteen norm apps share verbatim (config,
...
use crate::artifacts::en1996::engine::En1996Family;
...
semio_framework_plugin::app_commands! {
    /// 🎯️ `En1996PlayApp::Command` — the SOLE dispatch surface for this app's own behavior, covering every
...
//#region 🔖️En1996PlayApp
#[derive(Default)]
pub struct En1996PlayApp;

impl DocumentApp for En1996PlayApp {
    ...
    fn handle(command: &En1996Command, doc: &DocumentView<'_, En1996Snapshot>, cfg: &ConfigView<'_, NormConfig>, _draft: &DraftView<'_, Self::Draft>, _engines: &EngineHandles) -> Result<Emit<En1996Mutation, NormConfigMutation, Self::DraftMutation>, Fault> {
        command.dispatch(doc, cfg)
    }

    fn render(body_key: &str, doc: &DocumentView<'_, En1996Snapshot>, cfg: &ConfigView<'_, NormConfig>) -> UiNode {
    ...
    fn export_media(port: &str, doc: &DocumentView<'_, En1996Snapshot>) -> Result<Media, MediaError> {
    ...
    fn import_media(port: &str, media: &Media, _doc: &DocumentView<'_, En1996Snapshot>) -> Result<Emit<En1996Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
        crate::app_surface::import_media::<En1996Snapshot>(port, media)
    }
    //#endregion 🔖️MediaPorts
}
//#endregion 🔖️En1996PlayApp
...
pub(crate) mod testkit {
    ...
    pub type NormApp = VcsDocumentApp<En1996PlayApp>;
...
mod tests {
    ...
    /// 🎯️ One value per `En1996Command` row — the whole-command-surface laws below iterate it, so a new row
    /// that is not listed here fails `command_ids_cover_every_row`.
    fn every_command() -> Vec<En1996Command> {
        vec![
            En1996Command::SetSnapshot(set_snapshot::SetSnapshot { snapshot: En1996Snapshot::default() }),
    ...
    fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, En1996Command::SetSnapshot(set_snapshot::SetSnapshot { snapshot: En1996Snapshot::default() }));
        let host = NormHost::<En1996Family>::from_document(app.projection().expect("projection"));
    ...
    fn undo_redo_round_trips_through_the_wrapper() {
        ...
        assert_eq!(app.projection().expect("projection"), En1996Snapshot::default());
    }
