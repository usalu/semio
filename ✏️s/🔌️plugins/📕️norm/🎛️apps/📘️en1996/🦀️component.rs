...
use crate::apps::en1996::commands::{evaluate, selected_check, set_document};
...
semio_framework_plugin::app_commands! {
    ...
    pub enum En1996Command for Document, En1996Mutation, NormConfig, NormConfigMutation {
...
impl DocumentApp for En1996PlayApp {
    ...
    fn handle(command: &En1996Command, doc: &DocumentView<'_, Document>, cfg: &ConfigView<'_, NormConfig>, _draft: &DraftView<'_, Self::Draft>, _engines: &EngineHandles) -> Result<Emit<En1996Mutation, NormConfigMutation, Self::DraftMutation>, Fault> {
        command.dispatch(doc, cfg)
    }

    fn render(body_key: &str, doc: &DocumentView<'_, Document>, cfg: &ConfigView<'_, NormConfig>) -> UiNode {
        let host = NormHost::<En1996Family>::from_document(doc.projection.clone());
        match body_key {
    ...
    fn export_media(port: &str, doc: &DocumentView<'_, Document>) -> Result<Media, MediaError> {
        crate::app_surface::export_media::<En1996Family>(port, VARIANT, DOCUMENT_SCHEMA, doc.projection)
    }

    /// 🎞️ `"model:in"`/`"document:in"` — see `crate::app_surface::import_media`.
    fn import_media(port: &str, media: &Media, _doc: &DocumentView<'_, Document>) -> Result<Emit<En1996Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
...
mod tests {
    ...
    fn every_command() -> Vec<En1996Command> {
        vec![
            En1996Command::SetDocument(set_document::SetDocument { document: Document::default() }),
    ...
    fn set_document_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, En1996Command::SetDocument(set_document::SetDocument { document: Document::default() }));
