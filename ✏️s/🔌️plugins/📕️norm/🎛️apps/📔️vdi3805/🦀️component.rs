...
semio_framework_plugin::app_commands! {
    /// 🎯️ `Vdi3805PlayApp::Command` — the SOLE dispatch surface for this app's own behavior, covering every
...
mod tests {
    ...
    /// 🎯️ One value per `Vdi3805Command` row — the whole-command-surface laws below iterate it, so a new row
    /// that is not listed here fails `command_ids_cover_every_row`.
    fn every_command() -> Vec<Vdi3805Command> {
        vec![
            Vdi3805Command::SetSnapshot(set_snapshot::ReplaceSnapshot { snapshot: Vdi3805Snapshot::default() }),
    ...
    fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, Vdi3805Command::SetSnapshot(set_snapshot::ReplaceSnapshot { snapshot: Vdi3805Snapshot::default() }));
