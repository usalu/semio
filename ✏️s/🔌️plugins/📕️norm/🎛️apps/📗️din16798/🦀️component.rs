...
use crate::apps::din16798::commands::{evaluate, selected_check, set_document};
...
impl DocumentApp for Din16798PlayApp {
    ...
    fn command_id(&self, command: &Din16798Command) -> &str {
        command.command_id()
    }

    fn handle(&self, command: &Din16798Command, doc: &DocumentView<'_, Document>, cfg: &ConfigView<'_, NormConfig>) -> Result<Emit<Operation, NormConfigOperation>, Fault> {
        command.dispatch(doc, cfg)
    }

    fn render(&self, body_key: &str, doc: &DocumentView<'_, Document>, cfg: &ConfigView<'_, NormConfig>) -> UiNode {
        let host = NormHost::<DinEn16798Family>::from_document(doc.projection.clone());
        match body_key {
    ...
    fn export_media(&self, port: &str, doc: &DocumentView<'_, Document>) -> Result<Media, MediaError> {
        crate::core::app::export_media::<DinEn16798Family>(port, VARIANT, DOCUMENT_SCHEMA, doc.projection)
    }
...
mod tests {
    ...
    fn every_command() -> Vec<Din16798Command> {
        vec![
            Din16798Command::SetDocument(set_document::SetDocument { document: Document::default() }),
    ...
    fn set_document_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, Din16798Command::SetDocument(set_document::SetDocument { document: Document::default() }));
        let host = NormHost::<DinEn16798Family>::from_document(app.projection().expect("projection"));
