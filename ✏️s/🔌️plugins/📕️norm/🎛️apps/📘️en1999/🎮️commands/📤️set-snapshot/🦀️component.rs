...
//#region 🔖️Handler
pub fn handle(payload: &ReplaceSnapshot, _doc: &ArtifactView<'_, En1999Snapshot>, _cfg: &ConfigView<'_, NormConfig>) -> Result<Emit<En1999Mutation, NormConfigMutation>, Fault> {
...
mod tests {
    ...
    fn handle_commits_the_payload_document_under_its_action_id() {
        ...
        let emit = handle(
            &ReplaceSnapshot { snapshot: En1999Snapshot::default() },
            &ArtifactView { snapshot: &projection, history: &HistoryView::empty() },
            &ConfigView { snapshot: &config },
        )
        .expect("handle");
        assert_eq!(emit.artifact_mutations, En1999Mutation::from_snapshot(&En1999Snapshot::default()));
