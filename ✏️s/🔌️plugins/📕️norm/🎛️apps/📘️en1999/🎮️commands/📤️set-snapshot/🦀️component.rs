...
//#region 🔖️Handler
pub fn handle(payload: &SetSnapshot, _doc: &DocumentView<'_, En1999Snapshot>, _cfg: &ConfigView<'_, NormConfig>) -> Result<Emit<En1999Mutation, NormConfigMutation>, Fault> {
    crate::app_surface::commit_snapshot(En1999Mutation::SetSnapshot { snapshot: payload.snapshot.clone() }, "setSnapshot")
}
//#endregion 🔖️Handler
...
mod tests {
    ...
    fn handle_commits_the_payload_document_under_its_action_id() {
        ...
        let emit = handle(
            &SetSnapshot { snapshot: En1999Snapshot::default() },
            &DocumentView { snapshot: &projection, history: &HistoryView::empty() },
            &ConfigView { snapshot: &config },
        )
        .expect("handle");
        assert_eq!(emit.document_mutations, vec![En1999Mutation::SetSnapshot { snapshot: En1999Snapshot::default() }]);
