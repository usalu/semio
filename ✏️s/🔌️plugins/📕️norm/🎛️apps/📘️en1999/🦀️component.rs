//! ✨️ EN 1999 play app — the `ArtifactApp` impl (dispatch-only), the aggregated command enum and
//! the manifest stitch.
//!
...
mod tests {
    ...
    fn every_command() -> Vec<En1999Command> {
        vec![
            En1999Command::SetSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1999Snapshot::default() }),
    ...
    fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, En1999Command::SetSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1999Snapshot::default() }));
