//! ✨️ EN 1999 play app — the `DocumentApp` impl (dispatch-only), the aggregated command enum and
//! the manifest stitch.
//!
...
//! `crate::artifacts::en1999::engine`, and everything the fifteen norm apps share verbatim (config,
//! media ports, render primitives, manifest constructors) in `crate::document::app` / `crate::document::config`.

use crate::apps::en1999::commands::{evaluate, selected_check, set_snapshot};
use crate::apps::en1999::modes::edit as edit_mode;
use crate::apps::en1999::modes::edit::windows::{inputs, results};
use crate::apps::en1999::panels::{catalogue as catalogue_panel, document as document_panel, inspection as inspection_panel};
...
//#region 🔖️Constants
pub const APP_ID: &str = "norm-en-1999-play";
...
semio_framework_plugin::app_commands! {
    /// 🎯️ `En1999PlayApp::Command` — the SOLE dispatch surface for this app's own behavior, covering every
...
//#region 🔖️En1999PlayApp
#[derive(Default)]
pub struct En1999PlayApp;

impl DocumentApp for En1999PlayApp {
    ...
    const APP_ID: &'static str = "norm-en-1999-play";
    ...
    fn handle(command: &En1999Command, doc: &DocumentView<'_, En1999Snapshot>, cfg: &ConfigView<'_, NormConfig>, _draft: &DraftView<'_, Self::Draft>, _engines: &EngineHandles) -> Result<Emit<En1999Mutation, NormConfigMutation, Self::DraftMutation>, Fault> {
        command.dispatch(doc, cfg)
    }

    fn render(body_key: &str, doc: &DocumentView<'_, En1999Snapshot>, cfg: &ConfigView<'_, NormConfig>) -> UiNode {
    ...
    fn export_media(port: &str, doc: &DocumentView<'_, En1999Snapshot>) -> Result<Media, MediaError> {
    ...
    fn import_media(port: &str, media: &Media, _doc: &DocumentView<'_, En1999Snapshot>) -> Result<Emit<En1999Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
        crate::app_surface::import_media::<En1999Snapshot>(port, media)
    }
    //#endregion 🔖️MediaPorts
}
//#endregion 🔖️En1999PlayApp

//#region 🔖️Manifest
pub fn create_en1999_app() -> App {
    App::from_builder(
    ...
    .example("default", LocalizedLabel::native("Default", "Standard"), serde_json::to_string(&En1999Snapshot::default()).expect("default document serializes"), "file")
...
pub(crate) mod testkit {
    ...
    pub type NormApp = VcsDocumentApp<En1999PlayApp>;

    pub fn new_app() -> NormApp {
        sdk_new_app::<En1999PlayApp>()
    }
    ...
    pub fn app_with_registry() -> NormApp {
        new_app_with_registry::<En1999PlayApp>(create_en1999_app)
    }
...
mod tests {
    ...
    /// 🎯️ One value per `En1999Command` row — the whole-command-surface laws below iterate it, so a new row
    /// that is not listed here fails `command_ids_cover_every_row`.
    fn every_command() -> Vec<En1999Command> {
        vec![
            En1999Command::SetSnapshot(set_snapshot::SetSnapshot { snapshot: En1999Snapshot::default() }),
    ...
    fn the_manifest_stitches_every_taxonomy_node() {
        let definition = create_en1999_app().definition;
    ...
    fn declares_model_in_and_report_out_ports() {
        let ports = create_en1999_app().definition.io.ports;
    ...
    fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, En1999Command::SetSnapshot(set_snapshot::SetSnapshot { snapshot: En1999Snapshot::default() }));
        let host = NormHost::<En1999Family>::from_document(app.projection().expect("projection"));
    ...
    fn undo_redo_round_trips_through_the_wrapper() {
        ...
        assert_eq!(app.projection().expect("projection"), En1999Snapshot::default());
    }
