//! 🧲️ EN 1994 play app — the `DocumentApp` impl (dispatch-only), the aggregated command enum and
//! the manifest stitch.
//!
...
impl DocumentApp for En1994PlayApp {
    ...
    fn handle(command: &En1994Command, doc: &DocumentView<'_, Document>, cfg: &ConfigView<'_, NormConfig>, _draft: &DraftView<'_, Self::Draft>, _engines: &EngineHandles) -> Result<Emit<En1994Mutation, NormConfigMutation, Self::DraftMutation>, Fault> {
    ...
    fn import_media(port: &str, media: &Media, _doc: &DocumentView<'_, Document>) -> Result<Emit<En1994Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
...
pub(crate) mod testkit {
    ...
    pub type NormApp = VcsDocumentApp<En1994PlayApp>;
...
mod tests {
    ...
    fn every_command() -> Vec<En1994Command> {
        vec![
            En1994Command::SetDocument(set_document::SetDocument { document: Document::default() }),
    ...
    fn set_document_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, En1994Command::SetDocument(set_document::SetDocument { document: Document::default() }));
