//! ð§²ï¸ EN 1994 play app â the `ArtifactApp` impl (dispatch-only), the aggregated command enum and
//! the manifest stitch.
//!
...
//! `crate::apps::en1994`, and everything the fifteen norm apps share verbatim (config,
...
pub const LABEL: &str = "EN 1994";
/// ðï¸ The playground/registry variant key â every body key, window id and schema is derived from it.
pub const VARIANT: &str = "en1994";
pub const DOCUMENT_SCHEMA: &str = "semio.norm.en1994/v1";
pub const CONFIG_SCHEMA: &str = "config.norm.en1994";
//#endregion ðï¸Constants

//#region ðï¸Commands
semio_framework_plugin::app_commands! {
    /// ð¯ï¸ `En1994PlayApp::Command` â the SOLE dispatch surface for this app's own behavior, covering every
    /// action `create_en1994_app` declares. Row order IS the binary variant ordinal (appending is safe,
...
//#region ðï¸En1994PlayApp
#[derive(Default)]
pub struct En1994PlayApp;

impl ArtifactApp for En1994PlayApp {
    ...
    fn command_id(command: &En1994Command) -> &'static str {
        command.command_id()
    }

    fn handle(command: &En1994Command, doc: &ArtifactView<'_, En1994Snapshot>, cfg: &ConfigView<'_, NormConfig>, _interaction: &InteractionView<'_>, _draft: &DraftView<'_, Self::Draft>, _engines: &EngineHandles) -> Result<Emit<En1994Mutation, NormConfigMutation, Self::DraftMutation>, Fault> {
        command.dispatch(doc, cfg)
    }

    fn render(body_key: &str, doc: &ArtifactView<'_, En1994Snapshot>, cfg: &ConfigView<'_, NormConfig>) -> UiNode {
    ...
    fn export_media(port: &str, doc: &ArtifactView<'_, En1994Snapshot>) -> Result<Media, MediaError> {
    ...
    fn import_media(port: &str, media: &Media, _doc: &ArtifactView<'_, En1994Snapshot>) -> Result<Emit<En1994Mutation, NormConfigMutation, Self::DraftMutation>, MediaError> {
        crate::app_surface::import_media(port, media, |snapshot: En1994Snapshot| En1994Mutation::from_snapshot(&snapshot))
    }
    //#endregion ðï¸MediaPorts
}
//#endregion ðï¸En1994PlayApp
...
impl crate::document::NormFamily for En1994Family {
    type Document = En1994Snapshot;
    type Mutation = En1994Mutation;

    fn family_id() -> crate::document::NormFamilyId {
        crate::document::NormFamilyId::En1994
    }

    fn evaluate(document: &En1994Snapshot) -> crate::document::CheckReport {
        crate::artifacts::en1994::standards::v1::subsets::any::schema::inferences::evaluate(document)
    }
}

pub type Host = crate::document::NormHost<En1994Family>;
//#endregion 🧩️ComplianceFamily

//#region ðï¸Manifest
pub fn create_en1994_app() -> App {
    App::from_builder(
        App::builder(APP_ID, LocalizedLabel::data(LABEL))
            .document(["semio", "norm", VARIANT])
            .artifact_kind(crate::artifacts::en1994::artifact_kind())
    ...
    .example("default", LocalizedLabel::native("Default", "Standard"), serde_json::to_string(&En1994Snapshot::default()).expect("default document serializes"), "file")
...
pub(crate) mod testkit {
    ...
    pub type NormApp = VcsArtifactApp<En1994PlayApp>;

    pub fn new_app() -> NormApp {
        sdk_new_app::<En1994PlayApp>()
    }
    ...
    pub fn app_with_registry() -> NormApp {
        new_app_with_registry::<En1994PlayApp>(create_en1994_app)
    }

    pub fn dispatch(app: &mut NormApp, command: En1994Command) -> InvocationResult {
...
mod tests {
    ...
    /// ð¯ï¸ One value per `En1994Command` row â the whole-command-surface laws below iterate it, so a new row
    /// that is not listed here fails `command_ids_cover_every_row`.
    fn every_command() -> Vec<En1994Command> {
        vec![
            En1994Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1994Snapshot::default() }),
    ...
    fn command_ids_cover_every_row_and_are_unique() {
        ...
        let ids: Vec<&str> = commands.iter().map(En1994Command::command_id).collect();
    ...
    fn optional_field_rows_keep_their_pre_migration_bytes() {
        ...
        assert_eq!(hex(&En1994Command::Evaluate(evaluate::Evaluate {})), "01010000");
        assert_eq!(hex(&En1994Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(2) })), "01020001000402");
        assert_eq!(hex(&En1994Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: None })), "01020000");
    }
    ...
    fn the_manifest_stitches_every_taxonomy_node() {
        let definition = create_en1994_app().definition;
    ...
    fn declares_model_in_and_report_out_ports() {
        let ports = create_en1994_app().definition.io.ports;
    ...
    fn an_unknown_body_key_falls_back_to_a_text_node() {
        ...
        assert!(testkit::render(&mut app, "norm.en1994.play.nope").contains("Unknown body"));
    }
    ...
    fn set_snapshot_commits_a_host_backed_report() {
        let mut app = testkit::new_app();
        testkit::dispatch(&mut app, En1994Command::ReplaceSnapshot(set_snapshot::ReplaceSnapshot { snapshot: En1994Snapshot::default() }));
    ...
    fn view_actions_never_emit_artifact_mutations_under_the_real_registry() {
        ...
        let result = testkit::dispatch(&mut app, En1994Command::SetSelectedCheckIndex(selected_check::SetSelectedCheckIndex { index: Some(1) }));
