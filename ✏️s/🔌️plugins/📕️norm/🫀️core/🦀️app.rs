...
use crate::core::{CheckReport, NormFamily, NormHost};
...
pub fn import_media<D>(port: &str, media: &Media) -> Result<Emit<crate::core::SetDocumentOperation<D>, crate::core::NormConfigOperation>, MediaError>
where
    D: Clone + Default + PartialEq + Serialize + DeserializeOwned + store::DocumentPack,
{
    if port == "model:in" {
        if let MediaPayload::Structured { json, .. } = &media.payload {
            if let Ok(document) = serde_json::from_str::<D>(json) {
                return Ok(Emit::operations(vec![crate::core::SetDocumentOperation::SetDocument { document }]));
            }
        }
    ...
    Ok(Emit::operations(vec![crate::core::SetDocumentOperation::SetDocument { document }]))
}
...
pub fn commit_document<D>(document: D, description: &str) -> Result<Emit<crate::core::SetDocumentOperation<D>, crate::core::NormConfigOperation>, Fault> {
    Ok(Emit::commit(vec![crate::core::SetDocumentOperation::SetDocument { document }], description))
}

/// ☑️ The one config-only edit every app's `selected-check` command emits.
pub fn commit_selected_check_index<D>(index: Option<u32>) -> Result<Emit<crate::core::SetDocumentOperation<D>, crate::core::NormConfigOperation>, Fault> {
    Ok(Emit::config(vec![crate::core::NormConfigOperation::SetSelectedCheckIndex { index }]))
}
...
pub fn selected_check_index(cfg: &ConfigView<'_, crate::core::NormConfig>) -> Option<u32> {
...
mod tests {
    ...
    fn render_inspection_falls_back_to_the_first_check_for_an_out_of_range_index() {
        ...
        report.push(crate::core::CheckResult::from_utilization(
            crate::core::ClauseId::new("demo", "§1", "1.1"),
            crate::core::Quantity::new(crate::core::QuantityKind::Dimensionless, 0.5),
            crate::core::Quantity::new(crate::core::QuantityKind::Dimensionless, 1.0),
            "demo check",
            crate::core::AnnexChoice::De,
        ));
