...
pub fn print_set_snapshot_op<D: ArtifactDsl>(snapshot: &D) -> String {
    format!("{SET_SNAPSHOT_OP_KEYWORD} \"{}\"", escape_op_text_field(&snapshot.print_dsl()))
}

/// 📖️ Shared `set-snapshot "<dsl>"` parser.
pub fn parse_set_snapshot_op<D: ArtifactDsl>(line: &str) -> Result<D, TextError> {
    let trimmed = line.trim();
    let rest = trimmed
...
pub fn encode_set_snapshot_op<D: ArtifactPack>(snapshot: &D) -> Result<Vec<u8>, ProtocolError> {
    let mut out = vec![1u8];
...
pub fn decode_set_snapshot_op<D: ArtifactPack>(bytes: &[u8]) -> Result<D, ProtocolError> {
