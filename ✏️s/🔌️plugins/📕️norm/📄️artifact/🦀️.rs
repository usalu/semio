...
/// ⚡️ One `OpText` implementation shared by every norm family: `SetArtifactMutation<D>`'s only variant
...
/// recovered via `D::parse_dsl` after {@link unescape_op_text_field}. Bounded on `store::ArtifactDsl` (for
/// `print_dsl`/`parse_dsl`) plus the same bounds `SetArtifactMutation<D>`'s own `Mutation<D>` impl
...
/// which has no line-boundary to protect). Bounded on `store::ArtifactPack` in addition to the
...
mod tests {
    ...
    fn set_artifact_operation_op_text_escapes_multiline_dsl_text() {
        ...
        let parsed = <SetArtifactMutation<DemoDocument> as OpText>::parse_op(&printed).expect("parse_op");
    ...
    /// `SetArtifactMutation<DemoDocument>`'s `Edit` round-trips through
