        ...
        match command {
            ...
            DagCommand::Disconnect { edge_id } => {
                if document.edges.iter().any(|edge| &edge.id == edge_id) {
                    Ok(Emit::operations(vec![DagOperation::Edges(CollectionOperation::Remove { id: edge_id.clone() })])
                } else {
                    Ok(Emit::default()
                }
            }
            DagCommand::MoveMediaNode { node_id, x, y } => {
                if document.nodes.iter().any(|node| &node.id == node_id) {
                    Ok(Emit::amend(vec![DagOperation::Nodes(CollectionOperation::Patch { id: node_id.clone(), patch: DagNodePatch { x: Some(*x), y: Some(*y), ..Default::default() } })], format!("move-{node_id}"))
                } else {
                    Ok(Emit::default()
                }
            }
            ...
            DagCommand::PatchDagNodes { node_ids, field, value } => {
                ...
                if operations.is_empty() {
                    ...
                    Ok(Emit::amend(operations, format!("patch-{field}-{}", node_ids.join(",")))
                }
            }
        }
    }
