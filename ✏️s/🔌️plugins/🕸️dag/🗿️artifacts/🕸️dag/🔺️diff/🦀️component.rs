...
//!
//! `MutationDiff<DagDocument> for DagDiff` and its `apply`/`absorb` logic are implemented directly in
//! the DAG kernel crate (`infinite_board_port_directed_dag`, `🔖️DocumentVcs` region) alongside
//! `DagDocument`/`DagMutation` themselves — see `crate::artifacts::dag::op`'s doc for why. This module
