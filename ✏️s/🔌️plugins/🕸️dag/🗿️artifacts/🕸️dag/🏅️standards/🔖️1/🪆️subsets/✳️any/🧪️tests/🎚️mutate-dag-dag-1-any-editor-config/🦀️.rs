//! 🎚️ `s.dag.dag` ✏️editor/🎚️config state-lane mutation case — Rust adapter.
//!
//! Recorded no-oracle decision `dag-dag-1-any-editor-config-state-lane-semantics`: the runner dispatches no oracle role, so every law is asserted inside
//! the subject handlers through `semio_s_plugin_stdio_test_oracle::law::vector` over the report of this crate's
//! production bridge `dag_config_mutation_report_json`. The oracle handlers answer with the committed after- and before-snapshots read
...
fn vector(kind: &str) -> Result<Vector, String> {
    Ok(match kind {
        ...
        "change-camera" => Vector {
            before: include_str!("../../✏️editor/🎚️config/🧫️fixtures/🎥️change-camera/✅️change-camera-applied/📸️snapshot/⬅️before/🔣️.json"),
            mutation: include_str!("../../✏️editor/🎚️config/🧫️fixtures/🎥️change-camera/✅️change-camera-applied/🦠️mutation/🔣️.json"),
            after: include_str!("../../✏️editor/🎚️config/🧫️fixtures/🎥️change-camera/✅️change-camera-applied/📸️snapshot/➡️after/🔣️.json"),
            diff: include_str!("../../✏️editor/🎚️config/🧫️fixtures/🎥️change-camera/✅️change-camera-applied/🔺️diff/🔣️.json"),
            outcome: include_str!("../../✏️editor/🎚️config/🧫️fixtures/🎥️change-camera/✅️change-camera-applied/🎯️outcome/🔣️.json"),
            observable: true,
        },
