...
fn dag_document_maps_to_the_language_neutral_layout_graph() {
    ...
    let edges = mapping["edges"].as_array().expect("edges").iter().map(|edge| DagFixtureEdge { id: edge["id"].as_str().expect("id").into(), source: edge["source"].as_str().expect("source").into(), target: edge["target"].as_str().expect("target").into(), ..DagFixtureEdge::default() }).collect();
