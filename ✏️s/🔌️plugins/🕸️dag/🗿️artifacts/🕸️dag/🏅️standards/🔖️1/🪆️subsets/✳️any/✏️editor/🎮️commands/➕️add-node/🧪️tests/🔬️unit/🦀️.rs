...
async fn add_node_action_updates_document_with_the_new_node() {
    ...
    app.dispatch_typed(DagCommand::AddNode(AddNode { kind: "slider".into(), x: None, y: None }), &semio_framework_plugin::artifact_app_laws::meta("local")).await.expect("add node");
...
async fn rename_dag_node_rewrites_nodes_and_edges() {
    ...
    app.dispatch_typed(DagCommand::RenameDagNode(rename_dag_node::RenameDagNode { old_id: old_id.clone(), value: "renamed-node".into() }), &semio_framework_plugin::artifact_app_laws::meta("local")).await.expect("rename");
    let document = app.snapshot().expect("projection");
    let nodes = document.nodes();
...
async fn rename_dag_node_is_a_no_op_for_an_empty_or_duplicate_id() {
    ...
    let result = app.dispatch_typed(DagCommand::RenameDagNode(rename_dag_node::RenameDagNode { old_id: first_id.clone(), value: "   ".into() }), &semio_framework_plugin::artifact_app_laws::meta("local")).await.expect("rename blank");
    assert!(result.mutations.is_empty());
    let result = app.dispatch_typed(DagCommand::RenameDagNode(rename_dag_node::RenameDagNode { old_id: first_id, value: second_id }), &semio_framework_plugin::artifact_app_laws::meta("local")).await.expect("rename duplicate");
    assert!(result.mutations.is_empty());
}
...
async fn remove_node_deletes_node_and_connected_edges() {
    ...
    app.dispatch_typed(DagCommand::RemoveNode(remove_node::RemoveNode { node_id: node_id.clone() }), &semio_framework_plugin::artifact_app_laws::meta("local")).await.expect("remove");
    let document = app.snapshot().expect("projection");
...
async fn add_node_then_undo_restores_document() {
    ...
    app.dispatch_typed(DagCommand::AddNode(AddNode { kind: "note".into(), x: None, y: None }), &semio_framework_plugin::artifact_app_laws::meta("local")).await.expect("add");
...
async fn patch_slider_value_coalesces_into_one_edit() {
    let mut app = context::new_app().await;
    app.dispatch_typed(DagCommand::AddNode(AddNode { kind: "slider".into(), x: None, y: None }), &semio_framework_plugin::artifact_app_laws::meta("local")).await.expect("add slider");
    ...
    for value in [1.0, 2.0, 5.0] {
        app.dispatch_typed(DagCommand::PatchDagNodes(patch_dag_nodes::PatchDagNodes { node_ids: vec![node_id.clone()], field: "value".into(), value: value.to_string() }), &semio_framework_plugin::artifact_app_laws::meta("local")).await.expect("patch slider");
    }
