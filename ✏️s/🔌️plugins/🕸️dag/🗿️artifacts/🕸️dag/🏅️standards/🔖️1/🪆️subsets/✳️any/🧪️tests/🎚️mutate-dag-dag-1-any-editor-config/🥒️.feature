@capability-dag-dag-1-any-editor-config-mutate
@no-oracle-dag-dag-1-any-editor-config-state-lane-semantics
@comparison-ordered-json-v1
@mutations-dag-dag-1-any-editor-config
Feature: Apply every config state-lane mutation of s.dag.dag's ✏️editor/🎚️config to its committed vector
  `DagConfigMutation` is the config lane of this subset's editor: it holds the graph camera (x, y, zoom). It is this
  repository's own state record, so the case records the no-oracle decision `dag-dag-1-any-editor-config-state-lane-semantics` in
  ...
  before-snapshot. Production dispatch is reached through `dag_config_mutation_report_json`, which runs
  ...
  Scenario Outline: Apply <id> and land on the committed after-snapshot, diff and outcome
    ...
    When <id> is applied to that vector's before-snapshot through dag_config_mutation_report_json
