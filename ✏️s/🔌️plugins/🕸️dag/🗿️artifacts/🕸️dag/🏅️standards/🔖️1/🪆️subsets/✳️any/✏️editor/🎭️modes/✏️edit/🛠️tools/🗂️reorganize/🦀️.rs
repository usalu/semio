...
pub fn layered_targets(snapshot: &DagSnapshot, config: &DagConfig) -> Result<HashMap<String, LayoutRunPoint>, Fault> {
    let fixture = DagHostSnapshot { schema: DAG_DOCUMENT_SCHEMA.into(), ..dag_host_snapshot_from_document(&semio_framework_artifact_infinite_dag::DagSnapshot::from(snapshot), dag_config_camera(config)) };
    let mut host = DagHost::load_host_snapshot_json(&dsl::json::to_json_string(&fixture)).map_err(|error| Fault::from(format!("dag.layout-run.layered-load: {error}")))?;
    host.reorganize(&DagLayoutOptions::default()).map_err(|error| Fault::from(format!("dag.layout-run.layered-layout: {error}")))?;
    let layered: DagHostSnapshot = host.host_snapshot_json().ok().and_then(|json| dsl::json::from_json_str(&json).ok()).ok_or_else(|| Fault::from("dag.layout-run.layered-fixture"))?;
