...
pub async fn inverse(payload: &super::mutation::DeleteNode, base: &DagSnapshot) -> Vec<DagMutation> {
    ...
    let mut mutations = vec![crate::artifacts::dag::mutations::create_node::mutation::create_node(node.clone()), crate::artifacts::dag::mutations::reorder_nodes::mutation::reorder_nodes(scene.nodes.iter().map(|node| node.id.clone()).collect())];
