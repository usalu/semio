//! 👥️ `s.dag.dag` ✏️editor/👥️presence state-lane mutation case — Rust adapter.
//!
//! Recorded no-oracle decision `dag-dag-1-any-editor-presence-state-lane-semantics`: the runner dispatches no oracle role, so every law is asserted inside
//! the subject handlers through `semio_s_plugin_stdio_test_oracle::law::vector` over the report of this crate's
//! production bridge `dag_presence_mutation_report_json`. The oracle handlers answer with the committed after- and before-snapshots read
...
mod subject {
    ...
    use semio_s_artifact_dag_dag::editor::dag::presence::mutations::dag_presence_mutation_report_json;

    fn report(committed: &Vector) -> Result<String, String> {
        dag_presence_mutation_report_json(committed.before, committed.mutation, committed.after)
    }
