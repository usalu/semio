...
/// `crate::artifacts::mathematical::artifact`/`🚪️io/🦀️.rs::io`.
...
pub use crate::artifacts::mathematical::snapshot::schema::{EquationSnapshot, MathematicalFixture, MathematicalSnapshot};
//#endregion 🔖️Document
...
pub fn require_mathematical_scene(snapshot: &MathematicalSnapshot) -> Result<Arc<MathematicalWorkingScene>, store::ArtifactChildMaterializationError> {
    snapshot.results.require_local_owner::<MathematicalWorkingScene>()
}

/// 📤️ Projects every mathematical field for a foreign carrier when the composed scene is present.
pub fn mathematical_fixture(snapshot: &MathematicalSnapshot) -> Result<MathematicalFixture, store::ArtifactChildMaterializationError> {
    let scene = require_mathematical_scene(snapshot)?;
    Ok(MathematicalFixture { graph: scene.graph.clone(), geometry: scene.geometry.clone(), equation: snapshot.equation.clone() })
}
...
pub fn mathematical_snapshot_from_fixture(fixture: MathematicalFixture) -> MathematicalSnapshot {
    let mut snapshot = mathematical_snapshot_with_state(fixture.graph, fixture.geometry);
...
pub async fn artifact() -> semio_framework_plugin::app::declarations::ArtifactDeclaration {
    ...
    ArtifactDeclaration { kind: ArtifactKindId::parse("s.mathematical.mathematical").expect("canonical mathematical.mathematical kind"), localization: &[], standards: vec![crate::artifacts::mathematical::standards::v1::standard()] }
}
...
mod tests {
    ...
    async fn carrier_fixture_contains_child_state_and_rejects_a_wire_only_parent() {
        ...
        let fixture = mathematical_fixture(&snapshot).expect("owned scene projects");
        assert_eq!(fixture.graph, mathematical_scene_owner(&snapshot).unwrap().graph);
        assert_eq!(fixture.geometry, MathematicalGeometry::default());

        let wire = snapshot.to_value();
        let decoded = MathematicalSnapshot::from_value(wire).expect("parent wire decodes");
        assert_eq!(mathematical_fixture(&decoded), Err(store::ArtifactChildMaterializationError::Absent));
    }
        ...
        for case in cases {
            ...
            match law {
                ...
                "instanceIsolation" => {
                    ...
                    assert_eq!(mathematical_graph(&left).directed, left_directed);
                    assert_eq!(mathematical_graph(&right).directed, right_directed);
                }
                "abaIsolation" => {
                    ...
                    assert_eq!(mathematical_graph(&reused_identity_b).directed, right_directed);
                    drop(reused_identity_b);
                    assert_eq!(mathematical_graph(&stale_a).directed, left_directed);
                }
                "wireOmission" => {
                    let left = owned_snapshot(left_directed).await;
                    let mut right = left.clone();
