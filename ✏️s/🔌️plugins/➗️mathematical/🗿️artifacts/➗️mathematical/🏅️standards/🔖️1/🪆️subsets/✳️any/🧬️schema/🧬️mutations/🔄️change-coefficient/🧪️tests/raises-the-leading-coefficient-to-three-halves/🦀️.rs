...
//!
//! ✅️ This is the ONE mathematical leaf that gets a fully hand-authored APPLIED case with a real
//! `🔺️diff`: `change-coefficient` edits `MathematicalSnapshot.equation`, the plain (non-`#[child]`)
//! persistent field, and therefore never calls `mathematical_children_from_state`. Every other verb
...
async fn applies_to_committed_after() {
    ...
    let applied = <MathematicalDiff as protocol::MutationDiff<MathematicalSnapshot>>::apply(produced().diff(), &base).expect("change-coefficient applies to its committed before-snapshot");
...
/// mathematical verb that leaves the three co-derived composed children completely alone.
#[semio_framework_async_macros::async_test]
...
async fn inverse_restores_before() {
    ...
    assert_eq!(
        inverse,
        vec![MathematicalMutation::ChangeCoefficient(ChangeCoefficient { label: COEFFICIENT, numer: "2".to_string(), denom: "1".to_string() })],
...
async fn a_non_numeric_target_and_a_zero_denominator_are_refused() {
    let base = before();
    let on_the_sum = MathematicalMutation::ChangeCoefficient(ChangeCoefficient { label: EquationNodeLabel(0), numer: "1".to_string(), denom: "1".to_string() });
    let outcome = <MathematicalMutation as protocol::Mutation<MathematicalSnapshot>>::diff(&on_the_sum, &base);
    ...
    let absent = MathematicalMutation::ChangeCoefficient(ChangeCoefficient { label: EquationNodeLabel(999), numer: "1".to_string(), denom: "1".to_string() });
    let outcome = <MathematicalMutation as protocol::Mutation<MathematicalSnapshot>>::diff(&absent, &base);
    assert_eq!(outcome.messages()[0].target, vec!["999".to_string()], "the diagnostic names the unresolved LABEL, rendered decimal");

    let zero_denominator = MathematicalMutation::ChangeCoefficient(ChangeCoefficient { label: COEFFICIENT, numer: "1".to_string(), denom: "0".to_string() });
    let outcome = <MathematicalMutation as protocol::Mutation<MathematicalSnapshot>>::diff(&zero_denominator, &base);
    ...
    assert_eq!(outcome.diff(), &MathematicalDiff::default(), "every refused change-coefficient carries the empty diff");
