//! ➗️ `s.mathematical.mathematical` whole-document identity round trip — Rust adapter. Relocated
//! out of the artifact-level `mutate-mathematical-1` case in ticket
...
//!
//! Recorded no-oracle decision `mathematical-mutation-semantics` (`../../🧪️oracle/🔣️.json`)
...
mod subject {
    ...
    pub fn round_trip(ctx: &Context) -> Result<Outcome, String> {
        ...
        let report = parse_json(&mathematical_identity_report_json(&dsl_text).map_err(|error| format!("identity-round-trip: the committed example did not reach this subset's own codec: {error}"))?)?;
