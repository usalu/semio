...
Feature: Parse the real committed mathematical document, print it back and cross it against its binary encoding
  This case carries the whole-document identity law that used to live inside the artifact-level
  `mutate-mathematical-1` case alongside the `✳️graph`, `✳️geometry` and `✳️equation` mutation
  ...
  decision (`mathematical-mutation-semantics`, in `../../🧪️oracle/🔣️.json`) because the same debt
  ...
  `mathematical-scene-ed395b82221de2b2` — the one committed document where the composition is
