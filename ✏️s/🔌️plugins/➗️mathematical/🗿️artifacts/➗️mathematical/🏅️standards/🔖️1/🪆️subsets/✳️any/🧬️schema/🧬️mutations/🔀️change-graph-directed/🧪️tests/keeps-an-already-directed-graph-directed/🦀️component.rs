...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-graph-directed/keeps-an-already-directed-graph-directed: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "change-graph-directed/keeps-an-already-directed-graph-directed: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(serde_json::Value::as_str), Some("applied"), "change-graph-directed/keeps-an-already-directed-graph-directed declares an applied outcome");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced_value, committed, "change-graph-directed/keeps-an-already-directed-graph-directed: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "change-graph-directed/keeps-an-already-directed-graph-directed: committed diff JSON is not canonical");
