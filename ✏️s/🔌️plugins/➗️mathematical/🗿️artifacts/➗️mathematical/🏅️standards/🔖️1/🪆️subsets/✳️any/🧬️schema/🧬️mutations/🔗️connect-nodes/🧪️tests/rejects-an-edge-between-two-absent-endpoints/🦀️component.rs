...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert!(pack::json::value_eq_ignoring_object_order(&reencoded, &original), "connect-nodes/rejects-an-edge-between-two-absent-endpoints: committed {label} JSON is not canonical ({reencoded:?} vs {original:?})");
    }
    let reencoded = pack::json_from_dsl_value(&(mutation()).to_value());
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert!(pack::json::value_eq_ignoring_object_order(&reencoded, &original), "connect-nodes/rejects-an-edge-between-two-absent-endpoints: committed mutation JSON is not canonical ({reencoded:?} vs {original:?})");
    assert_eq!(original.pointer("/ConnectNodes/source").and_then(pack::JsonValue::as_str), Some("n-alpha"), "the payload addresses its endpoints by bare node id");
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(pack::JsonValue::as_str), Some("rejected"), "connect-nodes/rejects-an-edge-between-two-absent-endpoints declares a rejected outcome");
    let emitted = produced();
