...
async fn produces_committed_diff() {
    ...
    assert!(pack::json::value_eq_ignoring_object_order(&produced_value, &committed), "replace-graph/replays-the-identical-empty-graph: produced diff differs from the committed 🔺️diff/🔣️component.json ({produced_value:?} vs {committed:?})");
}
