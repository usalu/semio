...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-node-label/rejects-relabelling-a-node-that-is-not-in-the-graph: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "change-node-label/rejects-relabelling-a-node-that-is-not-in-the-graph: committed mutation JSON is not canonical");
    assert_eq!(original.pointer("/ChangeNodeLabel/new_label").and_then(serde_json::Value::as_str), Some("Alpha"), "the payload's label field commits snake_case");
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(serde_json::Value::as_str), Some("rejected"), "change-node-label/rejects-relabelling-a-node-that-is-not-in-the-graph declares a rejected outcome");
    let emitted = produced();
