//! 🚪️ mathematical -> md. The complete carrier fixture is embedded in a canonical JSON code block.
...
impl Serializer<MathematicalSnapshot> for MathematicalIntoMd {
    ...
    async fn serialize(from: &MathematicalSnapshot) -> IoResult<IoPayload> {
        let fixture = mathematical_fixture(from).map_err(|error| IoError { message: format!("MathematicalIntoMd: {error}"), diagnostics: Vec::new() })?;
