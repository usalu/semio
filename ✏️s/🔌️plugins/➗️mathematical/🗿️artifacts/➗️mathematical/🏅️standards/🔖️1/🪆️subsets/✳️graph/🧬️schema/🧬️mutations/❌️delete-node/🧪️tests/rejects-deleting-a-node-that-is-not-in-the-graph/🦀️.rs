//! 🧪️ `delete-node` fixture — `rejects-deleting-a-node-that-is-not-in-the-graph`.
//!
...
async fn rejection_leaves_the_document_at_the_committed_after() {
    let base = before();
    assert!(mathematical_graph(&base).nodes.is_empty(), "rejects-deleting-a-node-that-is-not-in-the-graph's before-snapshot must decode to an unresolved, node-less graph");
...
async fn a_missing_node_is_reported_alone_with_no_cascade_notice() {
    ...
    assert_eq!(emitted.diff(), &MathematicalDiff::default(), "a rejecting delete-node must carry an empty diff");
