...
async fn rejection_leaves_the_document_at_the_committed_after() {
    let base = before();
    assert!(mathematical_graph(&base).nodes.is_empty(), "rejects-a-bulk-delete-where-every-id-is-absent's before-snapshot must decode to an unresolved, node-less graph");
...
async fn every_requested_id_travels_in_one_all_missing_diagnostic() {
    ...
    assert_eq!(emitted.diff(), &MathematicalDiff::default(), "a rejecting delete-nodes must carry an empty diff");
