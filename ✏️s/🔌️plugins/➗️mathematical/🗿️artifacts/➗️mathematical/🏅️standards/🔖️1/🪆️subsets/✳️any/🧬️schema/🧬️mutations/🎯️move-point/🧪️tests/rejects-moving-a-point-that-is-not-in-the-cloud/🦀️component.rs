...
use crate::artifacts::mathematical::mutations::move_point::mutation::MovePoint;
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "move-point/rejects-moving-a-point-that-is-not-in-the-cloud: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "move-point/rejects-moving-a-point-that-is-not-in-the-cloud: committed mutation JSON is not canonical");
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(serde_json::Value::as_str), Some("rejected"), "move-point/rejects-moving-a-point-that-is-not-in-the-cloud declares a rejected outcome");
    let emitted = produced();
