//! 🚪️ mathematical <- md. Reads the canonical JSON carrier fixture from its code block.
...
impl Deserializer<MathematicalSnapshot> for MdIntoMathematical {
    ...
    async fn deserialize(payload: &IoPayload) -> IoResult<MathematicalSnapshot> {
        ...
        let literal = match md.blocks.as_slice() {
            ...
            _ => return Err(IoError { message: "MdIntoMathematical: expected one json code block".into(), diagnostics: Vec::new() }),
        };
        let fixture: MathematicalFixture = pack::json::from_json_str(literal).map_err(|error| IoError { message: format!("MdIntoMathematical: {error}"), diagnostics: Vec::new() })?;
