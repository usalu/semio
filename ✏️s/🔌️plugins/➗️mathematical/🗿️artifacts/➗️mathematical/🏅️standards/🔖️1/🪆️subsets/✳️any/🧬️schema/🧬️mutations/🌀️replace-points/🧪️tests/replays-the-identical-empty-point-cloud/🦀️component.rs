//! 🧪️ `replace-points` fixture — `replays-the-identical-empty-point-cloud`.
//!
...
//! `mathematical_scene` fails soft to an EMPTY point cloud, which this committed payload replays
//! verbatim, taking `replace-points`' own whole-value `mutation.no-op` guard.

use crate::artifacts::mathematical::mutations::replace_points::mutation::ReplacePoints;
use crate::artifacts::mathematical::{mathematical_geometry, MathematicalDiff, MathematicalMutation, MathematicalPoint, MathematicalSnapshot};
...
/// ▶️ `replace-points` compares the whole `Vec<MathematicalPoint>` against base, so replaying an
/// identical cloud carries `before` to exactly the committed `after`, i.e. leaves it untouched.
#[semio_framework_async_macros::async_test]
async fn applies_to_committed_after() {
    let base = before();
    let MathematicalMutation::ReplacePoints(payload) = mutation() else {
        panic!("replays-the-identical-empty-point-cloud's committed mutation must be a replace-points");
    };
    assert_eq!(mathematical_geometry(&base).points, payload.points, "the committed payload must be exactly the point cloud BASE resolves to, or the no-op guard is never reached");
...
async fn inverse_restores_before() {
    ...
    assert_eq!(inverse, vec![MathematicalMutation::ReplacePoints(ReplacePoints { points: Vec::new() })], "replace-points inverts to a replace-points carrying BASE's whole prior cloud, got {inverse:?}");
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-points/replays-the-identical-empty-point-cloud: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "replace-points/replays-the-identical-empty-point-cloud: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(serde_json::Value::as_str), Some("applied"), "replace-points/replays-the-identical-empty-point-cloud declares an applied outcome");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(outcome.diff(), &MathematicalDiff::default(), "a no-op replace-points must carry the empty diff, never a re-minted child triple");
    let produced_value = serde_json::to_value(outcome.diff()).expect("produced diff encodes");
    let committed: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff decodes");
    assert_eq!(produced_value, committed, "replace-points/replays-the-identical-empty-point-cloud: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "replace-points/replays-the-identical-empty-point-cloud: committed diff JSON is not canonical");
}
...
async fn one_added_point_is_a_real_replacement() {
    let base = before();
    let loaded = MathematicalMutation::ReplacePoints(ReplacePoints { points: vec![MathematicalPoint { x: 0.5, y: 0.25 }] });
    let outcome = <MathematicalMutation as protocol::Mutation<MathematicalSnapshot>>::diff(&loaded, &base);
    assert!(outcome.messages().is_empty(), "a differing cloud is a real replacement, not a no-op, got {:?}", outcome.messages());
    assert!(outcome.diff().notation.is_some() && outcome.diff().results.is_some() && outcome.diff().computed.is_some(), "a geometry-scoped mathematical mutation still regenerates all three co-derived children");
