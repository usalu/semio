...
impl protocol::MutationDiff<EquationPresence> for EquationPresence {
    fn apply(&self, base: &EquationPresence) -> protocol::MutationApplyResult<EquationPresence> {
        Ok({ base.clone() })
    }
...
impl Mutation<EquationPresence> for EquationPresenceMutation {
    ...
    fn diff(&self, _base: &EquationPresence) -> protocol::MutationOutcome<EquationPresence> {
        protocol::MutationOutcome::new(EquationPresence::default())
    }

    fn inverse(&self, _base: &EquationPresence) -> Vec<Self> {
