...
//!
//! ⚠️ Why this leaf pins the NO-OP branch rather than a state change: `EquationSnapshot` keeps
//! its graph and its point cloud in three co-derived composed CHILDREN (`notation`/`results`/
//! `computed`, `🔖️WorkingScene`), and every state-changing equation diff re-mints all three
//! through `equation_children_from_state`, whose `child_id` is a `DefaultHasher` digest of the
...
//! and `equation_scene` fails soft to an empty — but DIRECTED — graph. Re-asserting
...
async fn applies_to_committed_after() {
    let base = before();
    assert!(equation_graph(&base).directed, "change-graph-directed/keeps-an-already-directed-graph-directed: the unresolved base scene must already be directed for this fixture to reach the no-op guard");
...
async fn inverse_restores_before() {
    ...
    let inverse = <EquationMutation as protocol::Mutation<EquationSnapshot>>::inverse(&forward, &base);
...
/// This is the assertion that proves the guard fires BEFORE `equation_children_from_state`:
...
async fn produces_committed_diff() {
    ...
    assert!(pack::json::value_eq_ignoring_object_order(&produced_value, &committed), "change-graph-directed/keeps-an-already-directed-graph-directed: produced diff differs from the committed 🔺️diff/🔣️.json ({produced_value:?} vs {committed:?})");
}
