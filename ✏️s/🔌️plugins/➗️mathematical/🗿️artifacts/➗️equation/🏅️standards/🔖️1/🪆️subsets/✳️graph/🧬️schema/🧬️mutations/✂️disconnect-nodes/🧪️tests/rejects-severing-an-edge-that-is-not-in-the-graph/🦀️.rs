...
//! UNRESOLVED handle and `equation_scene` fails soft to an edge-less graph — the state this
...
async fn rejection_leaves_the_document_at_the_committed_after() {
    let base = before();
    assert!(equation_graph(&base).edges.is_empty(), "rejects-severing-an-edge-that-is-not-in-the-graph's before-snapshot must decode to an unresolved, edge-less graph");
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert!(pack::json::value_eq_ignoring_object_order(&reencoded, &original), "disconnect-nodes/rejects-severing-an-edge-that-is-not-in-the-graph: committed {label} JSON is not canonical ({reencoded:?} vs {original:?})");
    }
    let reencoded = pack::json_from_dsl_value(&(mutation()).to_value());
    let original = pack::parse_json(MUTATION).expect("mutation reparses");
    assert!(pack::json::value_eq_ignoring_object_order(&reencoded, &original), "disconnect-nodes/rejects-severing-an-edge-that-is-not-in-the-graph: committed mutation JSON is not canonical ({reencoded:?} vs {original:?})");
    assert_eq!(original.pointer("/DisconnectNodes").and_then(pack::JsonValue::as_object).map(|fields| fields.len()), Some(1), "the payload carries the edge id and nothing else");
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(pack::JsonValue::as_str), Some("rejected"), "disconnect-nodes/rejects-severing-an-edge-that-is-not-in-the-graph declares a rejected outcome");
    let emitted = produced();
