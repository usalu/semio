...
async fn applies_to_committed_after() {
    ...
    assert_eq!(equation_graph(&base), payload.graph, "the committed payload must be byte-for-byte the graph BASE resolves to, or the no-op guard is never reached");
...
async fn inverse_restores_before() {
    ...
    assert_eq!(inverse, vec![EquationMutation::ReplaceGraph(ReplaceGraph { graph: equation_graph(&base) })], "replace-graph inverts to a replace-graph carrying BASE's whole prior graph, got {inverse:?}");
...
async fn a_graph_differing_in_one_field_is_a_real_replacement() {
    let base = before();
    let retitled = EquationMutation::ReplaceGraph(ReplaceGraph { graph: EquationGraph { algorithm: "bfs".into(), ..equation_graph(&base) } });
