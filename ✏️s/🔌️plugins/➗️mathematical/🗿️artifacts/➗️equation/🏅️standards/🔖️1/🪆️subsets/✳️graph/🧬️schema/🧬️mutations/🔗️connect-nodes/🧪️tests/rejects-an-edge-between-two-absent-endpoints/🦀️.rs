...
async fn rejection_leaves_the_document_at_the_committed_after() {
    let base = before();
    assert!(equation_graph(&base).nodes.is_empty() && equation_graph(&base).edges.is_empty(), "rejects-an-edge-between-two-absent-endpoints' before-snapshot must decode to an unresolved, empty graph");
