...
//! UNRESOLVED handle and `equation_scene` fails soft to an empty graph and an EMPTY point
...
async fn rejection_leaves_the_document_at_the_committed_after() {
    let base = before();
    assert!(equation_geometry(&base).points.is_empty(), "rejects-removing-a-point-from-an-empty-cloud's before-snapshot must decode to an unresolved, point-less cloud");
