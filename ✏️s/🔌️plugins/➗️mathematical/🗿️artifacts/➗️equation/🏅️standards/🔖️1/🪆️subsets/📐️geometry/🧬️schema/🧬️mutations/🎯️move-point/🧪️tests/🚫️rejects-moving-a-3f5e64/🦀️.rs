//! 🧪️ `move-point` fixture — `🚫️rejects-moving-a-point-that-is-not-in-the-cloud`.
//!
...
use crate::standards::v1::subsets::geometry::schema::mutations::move_point::MovePoint;
...
async fn rejection_leaves_the_document_at_the_committed_after() {
    let base = before();
    assert!(equation_geometry(&base).points.is_empty(), "rejects-moving-a-point-that-is-not-in-the-cloud's before-snapshot must decode to an unresolved, point-less cloud");
...
async fn the_index_lookup_precedes_the_finiteness_invariant() {
    ...
    assert_eq!(emitted.diff(), &EquationDiff::default(), "a rejecting move-point must carry an empty diff");
    ...
    let non_finite = EquationMutation::MovePoint(MovePoint { index: 0, x: f64::NAN, y: 0.0 });
