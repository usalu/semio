/** 📐️ Equation viewer — Geometry window: typed twin of `🦀️.rs`'s view-model. Mirrors
 * the Rust `render(document: &EquationSnapshot) -> UiNode` boundary — a flat table of the point
 * cloud's coordinates, built on the framework `TableWindowKit` (no hull/centroid overlay, no
 * editable cell: the read-only counterpart of `✏️editor/…/📐️geometry/🟦️.ts`). */

/** 👁️ The Geometry window's typed view-model — one row per point, in document order. */
export interface EquationGeometryTableViewModel {
