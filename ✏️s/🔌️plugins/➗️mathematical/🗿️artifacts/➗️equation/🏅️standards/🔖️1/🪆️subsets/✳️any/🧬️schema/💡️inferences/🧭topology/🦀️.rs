...
//! `EquationNode` ids; edges are `EquationEdge`'s own `source`/`target` id pair, taken as
...
async fn topological_sort(nodes: Vec<String>, edges: Vec<(String, String)>) -> EquationTopology {
    ...
    EquationTopology { topo_order, depth, cycle_free, node_count }
}
//#endregion 🔖️Compute
