...
async fn rejection_leaves_the_document_at_the_committed_after() {
    let base = before();
    assert!(equation_graph(&base).nodes.is_empty(), "rejects-relabelling-a-node-that-is-not-in-the-graph's before-snapshot must decode to an unresolved, node-less graph");
