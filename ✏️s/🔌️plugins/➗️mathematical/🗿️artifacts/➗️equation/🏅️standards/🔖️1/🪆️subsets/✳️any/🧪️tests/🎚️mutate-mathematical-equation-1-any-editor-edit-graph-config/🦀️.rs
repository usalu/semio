//! 🎚️ `s.mathematical.equation` ✏️editor/🎭️modes/✏️edit/🪟️windows/🕸️graph/🎚️config state-lane mutation case — Rust adapter.
//!
//! Recorded no-oracle decision `mathematical-equation-1-any-editor-edit-graph-config-state-lane-semantics`: the runner dispatches no oracle role, so every law is asserted inside
//! the subject handlers through `semio_s_plugin_stdio_test_oracle::law::vector` over the report of this crate's
//! production bridge `equation_graph_window_config_mutation_report_json`. The oracle handlers answer with the committed after- and before-snapshots read
