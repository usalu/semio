//! 🧮️ Mathematical app — document entities (constitutional: general).
...
/// `nodeGraphEdit`/`setDocument` payloads. The DSL-facing shape is `MathEdgeDsl` (see `🔖️Dsl`),
...
/// JSON-facing only now — see `MathGraphDsl` in `🔖️Dsl` for the DSL-facing twin.
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// `🔖️Dsl`), a manual `store::DocumentDsl` impl instead of the direct derive, since `MathGraph`'s
...
//
// 🧭️ These DSL-mirror types and the `DocumentDsl`/`DocumentPack` impls below live here (not in the
...
/// 📦️ Manual `store::DocumentPack` mirror of the manual `DocumentDsl` impl above — `MathProjectionDsl`
