#!/usr/bin/env bun
/** 🔺️ `@semio-tech/trinity-plugin` router: `bun ./📜️script.ts test`. */
...
class TestScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    ...
    await runCargoTestBudgeted(["semio-s-plugin-trinity"], this.repoRoot, rest);
  }
}
...
class DescribeScript extends BundleScript {
  run(): void {
    process.exit(describePluginComponent(this.repoRoot, "semio-s-plugin-trinity", join(this.root, "..", "..")));
  }
}
