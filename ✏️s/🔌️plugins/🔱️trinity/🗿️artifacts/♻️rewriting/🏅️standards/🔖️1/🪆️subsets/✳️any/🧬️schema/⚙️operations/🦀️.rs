...
/// VCS edit.
pub fn dispatch_rewrite_rule_mutations(store: &mut RewriteRuleStore, mutations: Vec<RewriteRuleMutation>) -> Result<(), TrinityRewritingError> {
    if mutations.is_empty() {
        return Ok(());
    }
    store.dispatch(ArtifactCommand::Apply { mutations, description: None }).map_err(TrinityRewritingError::from).map(|_| ())
}
...
mod tests {
    ...
    async fn document_text_round_trip_rewrite_rule_store() {
        ...
        next.lhs_json = "{}".into();
        dispatch_rewrite_rule_mutations(&mut store, rewriting_snapshot_mutations(&base, &next)).unwrap();
    ...
    async fn command_envelope_round_trip_holds_for_an_applied_operation() {
        ...
        ::store::os_store::test_support::assert_command_envelope_round_trip::<RewritingSnapshot, RewriteRuleMutation>(edit, &ArtifactId(store.envelope().id.clone()), &SchemaId(store.envelope().schema.clone()));
    }
