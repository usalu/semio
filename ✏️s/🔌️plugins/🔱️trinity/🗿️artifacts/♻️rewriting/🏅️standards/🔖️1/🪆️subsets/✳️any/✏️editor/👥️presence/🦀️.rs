...
//#region 🔖️Presence
/// 👥️ Shareable live subset of rewriting view state (before-pane camera, LOD) — node selection/hover
...
impl store::ArtifactDsl for RewritingPresence {
...
impl ArtifactPack for RewritingPresence {
...
mod contract_vectors {
    ...
    fn configuration_and_presence_contract_vectors_match_the_json_oracle() {
        ...
        let base: RewritingPresence = pack::from_json_str(&vectors["base"].to_string()).unwrap();
        assert_eq!(<RewritingPresenceMutation as Mutation<RewritingPresence>>::DESCRIPTORS.len(), vectors["cases"].as_array().unwrap().len());
        for vector in vectors["cases"].as_array().unwrap() {
            let mutation: RewritingPresenceMutation = pack::from_json_str(&vector["mutation"].to_string()).unwrap();
            ...
            assert_eq!(RewritingPresenceMutation::parse_op(&mutation.print_op()).unwrap(), mutation);
            assert_eq!(RewritingPresenceMutation::decode_op(&mutation.encode_op().unwrap()).unwrap(), mutation);
