...
pub fn validate_trinity_graph_operation(operation: &TrinityGraphMutation, fixture: &JackSnapshot) -> Result<(), crate::artifacts::jack::TrinityRamError> {
    ...
    match operation {
            ...
            if let Some(node_def) = fixture.manifest.node_kind(kind) {
                for port in ports {
                    ...
                    if !node_def.port_kinds.is_empty() && !node_def.port_kinds.iter().any(|p| p == &port.kind) {
                        return Err(TrinityRamError::PortKindNotDeclaredOnMutation { node_id: id.clone(), port_id: port.id.clone(), port_kind: port.kind.clone(), node_kind: kind.clone() });
                    }
                }
            }
        }
        ...
        TrinityGraphMutation::Rename { id, .. } | TrinityGraphMutation::Reposition { id, .. } => {
            if !fixture.nodes.iter().any(|node| node.id == *id) {
                return Err(TrinityRamError::NodeNotFound(id.clone()));
            }
        }
        ...
        TrinityGraphMutation::SetFixture { .. } => {}
    }
    Ok(())
}
        ...
        match self {
            ...
            TrinityGraphMutation::DeleteNode { id } => {
                ...
                if let Some(node) = node {
                    ...
                    for edge in edges {
                        out.push(TrinityGraphMutation::CreateEdge { id: edge.id, kind: edge.kind, source: edge.source, target: edge.target, properties: edge.properties });
                    }
                }
                out
            }
            ...
            TrinityGraphMutation::SetDataProperty { entity, key, .. } => {
                ...
                match (entity, prior) {
                    ...
                    (entity, None) => vec![TrinityGraphMutation::ClearDataProperty { entity: entity.clone(), key: key.clone() }],
                }
            }
            TrinityGraphMutation::ClearDataProperty { entity, key } => {
...
mod tests {
    ...
    fn graph_op_rejects_port_kind_not_declared_on_operation() {
        ...
        let op = TrinityGraphMutation::CreateNode {
            id: "new".into(),
            kind: "Piece".into(),
            name: "x".into(),
            x: 0.0,
    ...
    fn graph_op_create_edge_rejects_invalid_port_keys() {
        ...
        let bad_source = TrinityGraphMutation::CreateEdge { id: "e2".into(), kind: "Connection".into(), source: "noAt".into(), target: crate::artifacts::jack::port_key("child", "in-a"), properties: PropertyBag::new() };
        ...
        let bad_target = TrinityGraphMutation::CreateEdge { id: "e3".into(), kind: "Connection".into(), source: crate::artifacts::jack::port_key("root", "out-a"), target: "noAt".into(), properties: PropertyBag::new() };
    ...
    fn graph_op_create_edge_rejects_missing_source_and_target_nodes() {
        let fixture = mini_fixture();
        let missing_source = TrinityGraphMutation::CreateEdge { id: "e2".into(), kind: "Connection".into(), source: crate::artifacts::jack::port_key("ghost", "out"), target: crate::artifacts::jack::port_key("child", "in-a"), properties: PropertyBag::new() };
        ...
        let missing_target = TrinityGraphMutation::CreateEdge { id: "e3".into(), kind: "Connection".into(), source: crate::artifacts::jack::port_key("root", "out-a"), target: crate::artifacts::jack::port_key("ghost", "in"), properties: PropertyBag::new() };
    ...
    fn graph_op_rejects_duplicate_node_and_edge_ids() {
        let fixture = mini_fixture();
        let dup_node = TrinityGraphMutation::CreateNode { id: "root".into(), kind: "Piece".into(), name: "x".into(), x: 0.0, y: 0.0, width: 80.0, height: 40.0, ports: vec![] };
        ...
        let dup_edge = TrinityGraphMutation::CreateEdge { id: "e1".into(), kind: "Connection".into(), source: crate::artifacts::jack::port_key("root", "out-a"), target: crate::artifacts::jack::port_key("child", "in-a"), properties: PropertyBag::new() };
    ...
    fn graph_op_rejects_missing_entities_on_delete_rename_reposition() {
        let fixture = mini_fixture();
        assert!(matches!(validate_trinity_graph_operation(&TrinityGraphMutation::DeleteNode { id: "ghost".into() }, &fixture), Err(crate::artifacts::jack::TrinityRamError::NodeNotFound(_))));
        assert!(matches!(validate_trinity_graph_operation(&TrinityGraphMutation::DeleteEdge { id: "ghost".into() }, &fixture), Err(crate::artifacts::jack::TrinityRamError::EdgeNotFound(_))));
        assert!(matches!(validate_trinity_graph_operation(&TrinityGraphMutation::Rename { id: "ghost".into(), name: "x".into() }, &fixture), Err(crate::artifacts::jack::TrinityRamError::NodeNotFound(_))));
        assert!(matches!(validate_trinity_graph_operation(&TrinityGraphMutation::Reposition { id: "ghost".into(), x: 0.0, y: 0.0 }, &fixture), Err(crate::artifacts::jack::TrinityRamError::NodeNotFound(_))));
    }
    ...
    fn graph_op_set_data_property_rejects_unknown_entity_kind() {
        ...
        let err = validate_trinity_graph_operation(&TrinityGraphMutation::SetDataProperty { entity: EntityRef::Node("root".into()), key: "label".into(), value: PropertyValue::String("x".into()) }, &fixture).expect_err("unknown entity kind");
    ...
    fn graph_op_set_data_property_rejects_unknown_property_key() {
        let fixture = mini_fixture();
        let err = validate_trinity_graph_operation(&TrinityGraphMutation::SetDataProperty { entity: EntityRef::Node("root".into()), key: "bogus".into(), value: PropertyValue::Null }, &fixture).expect_err("unknown key");
    ...
    fn graph_op_set_data_property_rejects_type_mismatch() {
        let fixture = mini_fixture();
        let err = validate_trinity_graph_operation(&TrinityGraphMutation::SetDataProperty { entity: EntityRef::Node("root".into()), key: "label".into(), value: PropertyValue::Number(1.0) }, &fixture).expect_err("type mismatch");
    ...
    fn graph_op_clear_data_property_rejects_missing_entities() {
        let fixture = mini_fixture();
        assert!(matches!(validate_trinity_graph_operation(&TrinityGraphMutation::ClearDataProperty { entity: EntityRef::Node("ghost".into()), key: "label".into() }, &fixture), Err(crate::artifacts::jack::TrinityRamError::NodeNotFound(_))));
        assert!(matches!(validate_trinity_graph_operation(&TrinityGraphMutation::ClearDataProperty { entity: EntityRef::Edge("ghost".into()), key: "u".into() }, &fixture), Err(crate::artifacts::jack::TrinityRamError::EdgeNotFound(_))));
    }

    #[test]
    fn apply_trinity_graph_mutations_applies_valid_sequence_and_rejects_invalid() {
        let fixture = mini_fixture();
        let ok = apply_trinity_graph_mutations(fixture.clone(), &[TrinityGraphMutation::Rename { id: "root".into(), name: "renamed".into() }]).expect("rename applies");
        assert_eq!(ok.nodes.iter().find(|n| n.id == "root").unwrap().name, "renamed");

        let err = apply_trinity_graph_mutations(fixture, &[TrinityGraphMutation::DeleteNode { id: "ghost".into() }]).expect_err("missing node");
