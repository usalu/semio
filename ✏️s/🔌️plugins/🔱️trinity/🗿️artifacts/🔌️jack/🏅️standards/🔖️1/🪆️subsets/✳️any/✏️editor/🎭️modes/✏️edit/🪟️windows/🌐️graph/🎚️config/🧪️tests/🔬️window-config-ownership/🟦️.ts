...
export function testJackGraphWindowConfigOracle(): void {
  ...
  const appSchema = JSON.parse(readFileSync(new URL("../../../../../../../🎚️config/🧬️schema/🔣️.json", import.meta.url), "utf8"));
  assert(!("camera" in appSchema.properties));
  assert(!("lodModeByWindow" in appSchema.properties));
  ...
  console.log("[DEBUG] Jack graph-window config oracle: independent camera/LOD updates, per-window generations, app schema excludes view state");
}
