//! 🫧️ `s.trinity.jack` ✏️editor/🎭️modes/✏️edit/🪟️windows/📝️editor/🫧️transient state-lane mutation case — Rust adapter.
//!
//! Recorded no-oracle decision `trinity-jack-1-any-editor-edit-editor-transient-state-lane-semantics`: the runner dispatches no oracle role, so every law is asserted inside
//! the subject handlers through `semio_s_plugin_stdio_test_oracle::law::vector` over the report of this crate's
//! production bridge `jack_editor_window_transient_mutation_report_json`. The oracle handlers answer with the committed after- and before-snapshots read
...
//#region 🔖️Vectors
/// 🧫️ The committed applied vector of one kind, read literally from `✏️editor/🎭️modes/✏️edit/🪟️windows/📝️editor/🫧️transient/🧫️fixtures`.
...
mod subject {
    ...
    use semio_s_artifact_trinity_jack::editor::jack::transient::jack_editor_window_transient_mutation_report_json;

    fn report(committed: &Vector) -> Result<String, String> {
        jack_editor_window_transient_mutation_report_json(committed.before, committed.mutation, committed.after)
    }
