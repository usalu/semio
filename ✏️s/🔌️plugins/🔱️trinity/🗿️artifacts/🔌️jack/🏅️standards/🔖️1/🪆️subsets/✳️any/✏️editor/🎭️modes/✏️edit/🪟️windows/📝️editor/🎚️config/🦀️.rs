//! 📝️ Persisted-local authored query for one concrete Jack editor window.

#[path = "🧬️schema/🦀️.rs"]
mod schema;
pub use schema::JackEditorWindowConfig;

impl store::ArtifactDsl for JackEditorWindowConfig {
...
impl store::ArtifactPack for JackEditorWindowConfig {
...
store::impl_whole_record_config!(JackEditorWindowConfig);

#[path = "🧬️schema/🧬️mutations/🦀️.rs"]
mod mutations;
pub use mutations::*;

pub struct JackEditorWindowConfigOwner;

impl semio_framework_plugin::WindowConfigOwner for JackEditorWindowConfigOwner {
    const WINDOW_KIND_ID: &'static str = super::TRINITY_JACK_PLAY_WINDOW_EDITOR;
    const SCHEMA: &'static str = "trinity.jackeditorwindowcfg";
...
pub fn register(registry: &mut semio_framework_plugin::WindowConfigOwnerRegistry) -> Result<(), semio_framework_plugin::Fault> {
    registry.register::<JackEditorWindowConfigOwner>()
}

pub fn current<'a>(view: &'a semio_framework_plugin::ConfigView<'_, semio_framework_plugin::NoConfig>) -> Option<&'a JackEditorWindowConfig> {
    view.window::<JackEditorWindowConfigOwner>()
}

pub fn addressed(view: &semio_framework_plugin::ViewModel, mutation: JackEditorWindowConfigMutation) -> Result<semio_framework_plugin::WindowConfigMutation, semio_framework_plugin::Fault> {
    use semio_framework_plugin::{Fault, FaultCode, FaultOrigin, WindowConfigMutation};
    let window_id = view.window_id.as_deref().ok_or_else(|| Fault::new(FaultOrigin::App, FaultCode::new("jack.editor-window-required"), "Jack query changes require a concrete editor window"))?;
    let kind = view.window_instances.iter().find(|window| window.id == window_id).map(|window| window.window_kind_id.as_str());
    if kind != Some(super::TRINITY_JACK_PLAY_WINDOW_EDITOR) {
        return Err(Fault::new(FaultOrigin::App, FaultCode::new("jack.editor-window-required"), "Jack query changes require an editor window"));
    }
    Ok(WindowConfigMutation::of::<JackEditorWindowConfigOwner>(window_id, mutation))
}
