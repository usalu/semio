...
//! branch of this verb that mints nothing — an NO-OP case with the artifact's `Default` diff,
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📍️move-node/📍️keeps-a-node-at-the-point-it-already-occupies/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📍️move-node/📍️keeps-a-node-at-the-point-it-already-occupies/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📍️move-node/📍️keeps-a-node-at-the-point-it-already-occupies/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📍️move-node/📍️keeps-a-node-at-the-point-it-already-occupies/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📍️move-node/📍️keeps-a-node-at-the-point-it-already-occupies/🎯️outcome/🔣️.json");
...
/// `move-node` really emits here. A no-op is a NO-OP with an empty diff, never rejected.
#[semio_framework_async_macros::async_test]
async fn declared_outcome_holds() {
    let outcome: serde_json::Value = serde_json::from_str(OUTCOME).expect("outcome decodes");
    assert_eq!(outcome.get("status").and_then(serde_json::Value::as_str), Some("no-op"), "move-node/keeps-a-node-at-the-point-it-already-occupies declares a no-op outcome");
    ...
    apply_trinity_graph_mutation(&mut snapshot, &mutation()).expect("move-node/keeps-a-node-at-the-point-it-already-occupies: declared no-op but the mutation was rejected");
