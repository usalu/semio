//! 🧪️ `move-node` fixture — `📍️keeps-a-node-at-the-point-it-already-occupies`.
//!
...
use crate::{jack_working_scene, materialize_jack_content, JackSnapshot, Node, PropertyBag};
...
async fn produces_committed_diff() {
    ...
    assert_eq!(typed, JackDiff::default(), "move-node's no-op delta is the artifact's Default diff, every one of its seven document slots left None");
}
