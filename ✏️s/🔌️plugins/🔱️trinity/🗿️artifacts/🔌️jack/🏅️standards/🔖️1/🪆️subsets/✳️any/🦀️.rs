...
//! `serializer_entry`/`deserializer_entry` typed-trait shape `🗒️note`'s own `🚪️io/🦀️.rs`
//! uses) to replace the OLD `ComposerEntry`/`deserialize_bytes` machinery still live in this
//! artifact's own `🚪️io/🦀️.rs` — real migration work that belongs in that excluded file.
//! See `📓️terra-fleet-trinity-recipe-report.md`'s lease-request for the follow-up: once
//! `io-async-signatures` lands, relocate `io_declaration()` into `🚪️io/🦀️.rs` as `io()`
