//! ♻️ `s.trinity.rewrite` exhaustive mutation case — Rust SUBJECT adapter. Ticket
...
mod subject {
    ...
    fn snapshot_of(text: &str, label: &str, kind: &str) -> Result<RewriteSnapshot, String> {
        decode_rewrite_snapshot_json(text).map_err(|error| format!("mutate-rewrite-1: the {label} rule for {kind:?} must decode: {error}"))
    }

    fn mutation_of(text: &str, label: &str, kind: &str) -> Result<RewriteRuleMutation, String> {
        decode_rewrite_mutation_json(text).map_err(|error| format!("mutate-rewrite-1: the {label} payload for {kind:?} must decode: {error}"))
    }
    ...
    fn touches_one(scenario: &str, kind: &str, before: &RewriteSnapshot, after: &RewriteSnapshot) -> Result<(), String> {
    ...
    pub fn mutate(kind: &'static str) -> impl Fn(&Context) -> Result<Outcome, String> {
        move |ctx: &Context| {
            ...
            let raised = apply_rewrite_mutation_reporting(&mut current, &payload);
            if !raised.is_empty() {
    ...
    pub fn inverse(kind: &'static str) -> impl Fn(&Context) -> Result<Outcome, String> {
        move |ctx: &Context| {
            ...
            for step in inverse_rewrite_mutation_steps(&payload, &base) {
    ...
    pub fn spec_vector(kind: &'static str) -> impl Fn(&Context) -> Result<Outcome, String> {
        move |ctx: &Context| {
            ...
            let mut current = base.clone();
            let raised = apply_rewrite_mutation_reporting(&mut current, &vector);
            if !raised.is_empty() {
                return Err(format!("spec-vector-{kind}: every committed rewrite vector is a clean applied vector, but the implementation raised {raised:?}"));
            }
            if current != expected {
            ...
            for step in inverse_rewrite_mutation_steps(&vector, &base) {
                let undone = apply_rewrite_mutation_reporting(&mut current, &step);
                if !undone.is_empty() {
    ...
    /// example: `.rewrite.dsl.semio` is a fixed-layout document and the committed example is this
    ...
    pub fn round_trip(ctx: &Context) -> Result<Outcome, String> {
        ...
        if printed != text {
            ...
            return Err(format!(
                "exact-bytes law violated: `.rewrite.dsl.semio` is a fixed-layout document and the committed example is this codec's own output, so the re-printed text was required to reproduce it — {} byte(s) out against {} byte(s) in{}",
                printed.len(),
                text.len(),
                match at {
        ...
        for (rule, what) in [(&ground_floor, "ground floor"), (&derived, "capsule tower")] {
            let reread = snapshot_of(&encode_rewrite_snapshot_json(rule), "re-encoded", "identity-round-trip")?;
            if &reread != rule {
