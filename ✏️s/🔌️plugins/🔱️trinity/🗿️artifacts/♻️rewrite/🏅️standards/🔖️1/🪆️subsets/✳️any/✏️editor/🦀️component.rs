//! ♻️ Trinity Rewrite editor — parametric rewrite editor bundled as a hot-swappable WASM plugin.
//!
//! 📌️ Pure-trait `ArtifactEditor`: `TrinityRewritePlayApp` is a unit struct; every former
...
pub(crate) const TRINITY_REWRITE_PLAY_WINDOW_JACK: &str = "trinity-rewrite-jack";
pub(crate) const TRINITY_REWRITE_PLAY_WINDOW_PARAMETERS: &str = "trinity-rewrite-parameters";
const TRINITY_REWRITE_PLAY_RULE_NAME: &str = "label-core";

const NAKAGIN_FIXTURE_DSL: &str = include_str!("../../../../../../🔌️jack/🏅️standards/🔖️1/🪆️subsets/✳️any/📚️examples/🎬️demo/🖼️assets/🗣️example.dsl.semio");
...
/// ♻️ Trinity Rewrite play app — a parametric-rewrite editor over a {@link RewriteSnapshot} projection.
#[derive(Default)]
pub struct TrinityRewritePlayApp;

impl ArtifactEditor for TrinityRewritePlayApp {
    ...
    fn handle(command: &TrinityRewriteCommand, doc: &ArtifactView<'_, RewriteSnapshot>, cfg: &ConfigView<'_, RewriteConfig>, interaction: &InteractionView<'_>, _draft: &DraftView<'_, Self::Draft>, _engines: &EngineHandles) -> Result<Emit<RewriteRuleMutation, RewriteConfigMutation, Self::DraftMutation>, Fault> {
        ...
        match command {
            TrinityRewriteCommand::NodeGraphEdit { surface_id, operations_json } => crate::editor::rewrite::commands::node_graph_edit(state, &interaction.selection("graph").ids, surface_id, operations_json),
            TrinityRewriteCommand::SetLhsJson { value } => crate::editor::rewrite::commands::set_lhs_json(state, value),
            TrinityRewriteCommand::SetRhsJson { value } => crate::editor::rewrite::commands::set_rhs_json(state, value),
            TrinityRewriteCommand::SetParameter { name, value } => crate::editor::rewrite::commands::set_parameter(state, name, value),
            TrinityRewriteCommand::AddRuleClause { kind } => crate::editor::rewrite::commands::add_rule_clause_command(state, kind),
            TrinityRewriteCommand::ResetRule => crate::editor::rewrite::commands::reset_rule(state),
            TrinityRewriteCommand::PatchNodes { node_ids, field, value } => crate::editor::rewrite::commands::patch_nodes(state, node_ids, field, value),
            TrinityRewriteCommand::SetViewport { surface_id, viewport_json } => crate::editor::rewrite::commands::set_viewport(surface_id, viewport_json),
            TrinityRewriteCommand::Reorganize => crate::editor::rewrite::commands::reorganize(config.reorganize_epoch),
            TrinityRewriteCommand::SetLodMode { window_id, value } => crate::editor::rewrite::commands::set_lod_mode(window_id, value),
            TrinityRewriteCommand::SetLocale { value } => crate::editor::rewrite::commands::set_locale(value),
        }
    }
...
//#region 🔖️Manifest
use crate::editor::rewrite::modes::edit;
