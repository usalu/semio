//! 🧪️ `edit-lhs` fixture — `narrows-the-lhs-pattern-to-a-shaft-neighbour`.
//!
...
//! subgraph the rewrite fires on. Unlike its `edit-rhs` sibling it carries no `#[dsl(lang = "json")]`
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot.rhs_json, base.rhs_json, "edit-lhs must not touch the RHS rewrite program");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "edit-lhs/narrows-the-lhs-pattern-to-a-shaft-neighbour: produced diff differs from the committed 🔺️diff/🔣️component.json");
