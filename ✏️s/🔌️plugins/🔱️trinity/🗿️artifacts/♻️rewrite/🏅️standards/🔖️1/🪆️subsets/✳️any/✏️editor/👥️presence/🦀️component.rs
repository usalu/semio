//! 👥️ Rewrite presence — shareable live ephemeral state + mutations.
...
impl protocol::MutationDiff<RewritePresence> for RewritePresence {
    async fn apply(&self, _base: &RewritePresence) -> protocol::MutationApplyResult<RewritePresence> {
        Ok({
            self.clone()
        })
    }
...
impl store::ArtifactDsl for RewritePresence {
...
impl ArtifactPack for RewritePresence {
...
pub enum RewritePresenceMutation {
    ...
    Snapshot {
        #[dsl(block)]
        presence: RewritePresence,
    },
}

impl Mutation<RewritePresence> for RewritePresenceMutation {
    type Diff = RewritePresence;

    async fn diff(&self, _base: &RewritePresence) -> protocol::MutationOutcome<RewritePresence> {
        match self {
            Self::Snapshot { presence } => protocol::MutationOutcome::new(presence.clone()),
        }
    }

    async fn inverse(&self, base: &RewritePresence) -> Vec<Self> {
        vec![Self::Snapshot { presence: base.clone() }]
    }
}

impl protocol::OpText for RewritePresenceMutation {
...
impl protocol::OpBinary for RewritePresenceMutation {
