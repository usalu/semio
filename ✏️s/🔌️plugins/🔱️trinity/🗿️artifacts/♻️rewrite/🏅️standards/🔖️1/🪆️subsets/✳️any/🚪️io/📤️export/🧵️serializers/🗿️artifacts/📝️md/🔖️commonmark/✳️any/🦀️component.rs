...
pub async fn serialize(snapshot: &RewriteSnapshot) -> Result<MdSnapshot, store::TextError> {
    Ok(MdSnapshot { schema: STDIO_MD_DOCUMENT_SCHEMA.into(), blocks: parse_markdown_blocks(&<RewriteSnapshot as store::ArtifactDsl>::print_dsl(snapshot)) })
}
