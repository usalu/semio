...
fn catalogue_add_item(id: &str, label: impl AsRef<str>, clause_kind: &str) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::BuiltNode> {
    ...
    tree_item_with_action(id, crate::editor::rewrite::ui_label(label)?, None, crate::editor::rewrite::rewrite_action("addRuleClause", Some(args))?)
}
    ...
    PanelTreeBuilder::new("trinity-catalogue")?
        .section(
            ...
            crate::editor::rewrite::ui_node_list([
                tree_item("trinity-catalogue.piece", crate::editor::rewrite::ui_label(labels.piece.as_str())?),
                tree_item("trinity-catalogue.connection", crate::editor::rewrite::ui_label(labels.connection.as_str())?),
                tree_item("trinity-catalogue.connector", crate::editor::rewrite::ui_label(labels.connector.as_str())?),
            ])?,
        )?
        .section("trinity-catalogue.lhs", Some(labels.add_to_lhs.as_str().into()), true, crate::editor::rewrite::ui_node_list([catalogue_add_item("trinity-catalogue.add-where", "Where clause", "where")])?)?
        .section(
