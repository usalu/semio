...
pub mod set_active_utility {
    ...
    pub fn handle(payload: &SetActiveUtility, _doc: &DocumentView<'_, Process3dDocument>, _cfg: &ConfigView<'_, Process3dConfig>) -> Result<Emit<Process3dOperation, Process3dConfigOperation>, Fault> {
        Ok(Emit::config(vec![Process3dConfigOperation::SetActiveUtility { utility_id: payload.utility_id.clone() }, Process3dConfigOperation::SetSelectedFaceId { value: None }]))
    }
}
