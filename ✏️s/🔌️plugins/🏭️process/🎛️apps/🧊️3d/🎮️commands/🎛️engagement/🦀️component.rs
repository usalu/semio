...
pub mod engagement_submit {
    ...
    pub fn handle(_payload: &EngagementSubmit, doc: &ArtifactView<'_, Process3dSnapshot>, cfg: &ConfigView<'_, Process3dConfig>) -> Result<Emit<Process3dMutation, Process3dConfigMutation>, Fault> {
        ...
        match command_word.split_whitespace().next() {
            ...
            Some("back") => Ok(Emit { artifact_mutations: vec![Process3dMutation::SetCursor { resolved_up_to: Some(current.saturating_sub(1)) }], config_mutations: vec![clear_input], ..Default::default() }),
            Some("forward") => Ok(Emit { artifact_mutations: vec![Process3dMutation::SetCursor { resolved_up_to: Some((current + 1).min(len)) }], config_mutations: vec![clear_input], ..Default::default() }),
            Some("all") => Ok(Emit { artifact_mutations: vec![Process3dMutation::SetCursor { resolved_up_to: None }], config_mutations: vec![clear_input], ..Default::default() }),
...
pub mod engagement_input {
    ...
    pub fn handle(payload: &EngagementInput, _doc: &ArtifactView<'_, Process3dSnapshot>, _cfg: &ConfigView<'_, Process3dConfig>) -> Result<Emit<Process3dMutation, Process3dConfigMutation>, Fault> {
...
pub mod engagement_abort {
    ...
    pub fn handle(_payload: &EngagementAbort, _doc: &ArtifactView<'_, Process3dSnapshot>, _cfg: &ConfigView<'_, Process3dConfig>) -> Result<Emit<Process3dMutation, Process3dConfigMutation>, Fault> {
