//! 🧪️ `rename-step` fixture — `🔤️accepts-a-new-label-and-applies-it`.
//!
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "rename-step/accepts-a-new-label-and-applies-it: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "rename-step/accepts-a-new-label-and-applies-it: inverse did not restore the before-snapshot");
}
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "rename-step/accepts-a-new-label-and-applies-it: raised diagnostics differ from the committed 🎯️outcome messages");
    ...
    match status {
        "applied" if declared.iter().any(|(_, code)| code == "mutation.no-op") => {
            assert!(applied, "rename-step/accepts-a-new-label-and-applies-it: declared applied but the mutation was rejected");
            assert_eq!(snapshot, before(), "rename-step/accepts-a-new-label-and-applies-it: a no-op outcome is applied with an EMPTY diff — the snapshot must come back untouched");
        }
        "applied" => {
            assert!(applied, "rename-step/accepts-a-new-label-and-applies-it: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "rename-step/accepts-a-new-label-and-applies-it: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "rename-step/accepts-a-new-label-and-applies-it: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("rename-step/accepts-a-new-label-and-applies-it: unknown outcome status {other:?}"),
    }
}
