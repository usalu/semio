...
mod wasm_bridge {
    ...
    impl Process3dSnapshotVcs {
        ...
        pub fn new(envelope_json: Option<String>) -> Result<Process3dSnapshotVcs, JsValue> {
            let store = match envelope_json {
                Some(json) => {
                    ...
                    semio_framework_plugin::resolve_ready(Process3dStore::new(envelope)).map_err(|e| JsValue::from_str(&e.to_string()))?
                }
                None => semio_framework_plugin::resolve_ready(Process3dStore::new(create_document_envelope(PROCESS_3D_SCHEMA, "process3d", crate::artifacts::process3d::empty_process3d_snapshot(), None))).map_err(|e| JsValue::from_str(&e.to_string()))?,
            };
        ...
        pub fn envelope_json(&self) -> Result<String, JsValue> {
            semio_framework_plugin::resolve_ready(self.store.borrow().envelope_json()).map_err(|e| JsValue::from_str(&e.to_string()))
        }

        #[wasm_bindgen(js_name = generation)]
        pub fn generation(&self) -> u32 {
            semio_framework_plugin::resolve_ready(self.store.borrow().generation()) as u32
        }
    }
}
