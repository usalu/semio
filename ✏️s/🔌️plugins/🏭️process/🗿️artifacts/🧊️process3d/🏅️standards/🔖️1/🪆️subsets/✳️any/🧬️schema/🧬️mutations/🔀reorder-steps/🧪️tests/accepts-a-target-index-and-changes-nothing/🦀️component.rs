...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "reorder-steps/accepts-a-target-index-and-changes-nothing: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "reorder-steps/accepts-a-target-index-and-changes-nothing: inverse did not restore the before-snapshot");
}
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "reorder-steps/accepts-a-target-index-and-changes-nothing: raised diagnostics differ from the committed 🎯️outcome messages");
    ...
    match status {
        "applied" if declared.iter().any(|(_, code)| code == "mutation.no-op") => {
            assert!(applied, "reorder-steps/accepts-a-target-index-and-changes-nothing: declared applied but the mutation was rejected");
            assert_eq!(snapshot, before(), "reorder-steps/accepts-a-target-index-and-changes-nothing: a no-op outcome is applied with an EMPTY diff — the snapshot must come back untouched");
        }
        "applied" => {
            assert!(applied, "reorder-steps/accepts-a-target-index-and-changes-nothing: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "reorder-steps/accepts-a-target-index-and-changes-nothing: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "reorder-steps/accepts-a-target-index-and-changes-nothing: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("reorder-steps/accepts-a-target-index-and-changes-nothing: unknown outcome status {other:?}"),
    }
}
