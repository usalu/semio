//! 🧪️ `change-cursor` fixture — `pins-the-replay-cursor-to-two-steps`.
//!
//! `change-cursor` moves the timeline scrub point that bounds kernel replay; its diff field is a double-`Option` whose outer level means "this diff touches the cursor" — here it is set to a concrete `2`, the inner-`Some` case that survives a JSON round trip.
//!
...
async fn applies_to_committed_after() {
    let (snapshot, _) = protocol::apply_mutation(&before(), &mutation()).expect("change-cursor applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "change-cursor/pins-the-replay-cursor-to-two-steps: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "change-cursor/pins-the-replay-cursor-to-two-steps: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "change-cursor/pins-the-replay-cursor-to-two-steps: committed {side} JSON is not canonical");
    }
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "change-cursor/pins-the-replay-cursor-to-two-steps: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "change-cursor/pins-the-replay-cursor-to-two-steps: raised diagnostics differ from the committed 🎯️outcome messages");
    ...
    match status {
        "applied" if declared.iter().any(|(_, code)| code == "mutation.no-op") => {
            assert!(applied, "change-cursor/pins-the-replay-cursor-to-two-steps: declared applied but the mutation was rejected");
            assert_eq!(snapshot, before(), "change-cursor/pins-the-replay-cursor-to-two-steps: a no-op outcome is applied with an EMPTY diff — the snapshot must come back untouched");
        }
        "applied" => {
            assert!(applied, "change-cursor/pins-the-replay-cursor-to-two-steps: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "change-cursor/pins-the-replay-cursor-to-two-steps: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "change-cursor/pins-the-replay-cursor-to-two-steps: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("change-cursor/pins-the-replay-cursor-to-two-steps: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH collections and fields `change-cursor` is
...
async fn produces_committed_diff() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&produced, &committed), "change-cursor/pins-the-replay-cursor-to-two-steps: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "change-cursor/pins-the-replay-cursor-to-two-steps: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `change-cursor` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "change-cursor/pins-the-replay-cursor-to-two-steps: committed diff did not carry before to after");
}
