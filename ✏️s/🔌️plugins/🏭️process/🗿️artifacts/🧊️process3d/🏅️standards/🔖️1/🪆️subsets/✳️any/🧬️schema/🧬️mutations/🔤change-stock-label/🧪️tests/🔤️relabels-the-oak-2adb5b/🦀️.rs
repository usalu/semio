//! 🧪️ `change-stock-label` fixture — `🔤️relabels-the-oak-beam-as-planed`.
//!
//! `change-stock-label` no-ops on an identical string; otherwise it sets `stockLabel` and nothing else — notably NOT `stockId`, which is the stock's stable identity and has no mutation at all in this vocabulary.
//!
...
async fn applies_to_committed_after() {
    let (snapshot, _) = protocol::apply_mutation(&before(), &mutation()).expect("change-stock-label applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "change-stock-label/relabels-the-oak-beam-as-planed: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "change-stock-label/relabels-the-oak-beam-as-planed: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "change-stock-label/relabels-the-oak-beam-as-planed: committed {side} JSON is not canonical");
    }
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "change-stock-label/relabels-the-oak-beam-as-planed: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "change-stock-label/relabels-the-oak-beam-as-planed: raised diagnostics differ from the committed 🎯️outcome messages");
    ...
    match status {
        "applied" if declared.iter().any(|(_, code)| code == "mutation.no-op") => {
            assert!(applied, "change-stock-label/relabels-the-oak-beam-as-planed: declared applied but the mutation was rejected");
            assert_eq!(snapshot, before(), "change-stock-label/relabels-the-oak-beam-as-planed: a no-op outcome is applied with an EMPTY diff — the snapshot must come back untouched");
        }
        "applied" => {
            assert!(applied, "change-stock-label/relabels-the-oak-beam-as-planed: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "change-stock-label/relabels-the-oak-beam-as-planed: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "change-stock-label/relabels-the-oak-beam-as-planed: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("change-stock-label/relabels-the-oak-beam-as-planed: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH collections and fields `change-stock-label` is
...
async fn produces_committed_diff() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&produced, &committed), "change-stock-label/relabels-the-oak-beam-as-planed: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "change-stock-label/relabels-the-oak-beam-as-planed: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `change-stock-label` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "change-stock-label/relabels-the-oak-beam-as-planed: committed diff did not carry before to after");
}
