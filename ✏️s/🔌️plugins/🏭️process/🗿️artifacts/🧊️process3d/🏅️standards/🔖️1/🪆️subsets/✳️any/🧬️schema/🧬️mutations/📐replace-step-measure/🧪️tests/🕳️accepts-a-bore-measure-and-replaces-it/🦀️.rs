//! 🧪️ `replace-step-measure` fixture — `🕳️accepts-a-bore-measure-and-replaces-it`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐replace-step-measure/🕳️accepts-a-bore-measure-and-replaces-it/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐replace-step-measure/🕳️accepts-a-bore-measure-and-replaces-it/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐replace-step-measure/🕳️accepts-a-bore-measure-and-replaces-it/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐replace-step-measure/🕳️accepts-a-bore-measure-and-replaces-it/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐replace-step-measure/🕳️accepts-a-bore-measure-and-replaces-it/🎯️outcome/🔣️.json");
...
async fn applies_to_committed_after() {
    let (snapshot, _) = protocol::apply_mutation(&before(), &mutation()).expect("replace-step-measure applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "replace-step-measure/accepts-a-bore-measure-and-replaces-it: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "replace-step-measure/accepts-a-bore-measure-and-replaces-it: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "replace-step-measure/accepts-a-bore-measure-and-replaces-it: committed {side} JSON is not canonical");
    }
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "replace-step-measure/accepts-a-bore-measure-and-replaces-it: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "replace-step-measure/accepts-a-bore-measure-and-replaces-it: raised diagnostics differ from the committed 🎯️outcome messages");
    ...
    match status {
        "applied" if declared.iter().any(|(_, code)| code == "mutation.no-op") => {
            assert!(applied, "replace-step-measure/accepts-a-bore-measure-and-replaces-it: declared applied but the mutation was rejected");
            assert_eq!(snapshot, before(), "replace-step-measure/accepts-a-bore-measure-and-replaces-it: a no-op outcome is applied with an EMPTY diff — the snapshot must come back untouched");
        }
        "applied" => {
            assert!(applied, "replace-step-measure/accepts-a-bore-measure-and-replaces-it: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "replace-step-measure/accepts-a-bore-measure-and-replaces-it: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "replace-step-measure/accepts-a-bore-measure-and-replaces-it: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("replace-step-measure/accepts-a-bore-measure-and-replaces-it: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH collections and fields `replace-step-measure` is
...
async fn produces_committed_diff() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&produced, &committed), "replace-step-measure/accepts-a-bore-measure-and-replaces-it: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "replace-step-measure/accepts-a-bore-measure-and-replaces-it: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `replace-step-measure` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "replace-step-measure/accepts-a-bore-measure-and-replaces-it: committed diff did not carry before to after");
}
