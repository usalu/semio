//! 🧪️ `reorder-steps` fixture — `🔀️accepts-a-target-index-and-reorders-them`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔀reorder-steps/🔀️accepts-a-target-index-and-reorders-them/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔀reorder-steps/🔀️accepts-a-target-index-and-reorders-them/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔀reorder-steps/🔀️accepts-a-target-index-and-reorders-them/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔀reorder-steps/🔀️accepts-a-target-index-and-reorders-them/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔀reorder-steps/🔀️accepts-a-target-index-and-reorders-them/🎯️outcome/🔣️.json");
...
async fn applies_to_committed_after() {
    let (snapshot, _) = protocol::apply_mutation(&before(), &mutation()).expect("reorder-steps applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "reorder-steps/accepts-a-target-index-and-reorders-them: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "reorder-steps/accepts-a-target-index-and-reorders-them: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "reorder-steps/accepts-a-target-index-and-reorders-them: committed {side} JSON is not canonical");
    }
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "reorder-steps/accepts-a-target-index-and-reorders-them: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "reorder-steps/accepts-a-target-index-and-reorders-them: raised diagnostics differ from the committed 🎯️outcome messages");
    ...
    match status {
        "applied" if declared.iter().any(|(_, code)| code == "mutation.no-op") => {
            assert!(applied, "reorder-steps/accepts-a-target-index-and-reorders-them: declared applied but the mutation was rejected");
            assert_eq!(snapshot, before(), "reorder-steps/accepts-a-target-index-and-reorders-them: a no-op outcome is applied with an EMPTY diff — the snapshot must come back untouched");
        }
        "applied" => {
            assert!(applied, "reorder-steps/accepts-a-target-index-and-reorders-them: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "reorder-steps/accepts-a-target-index-and-reorders-them: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "reorder-steps/accepts-a-target-index-and-reorders-them: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("reorder-steps/accepts-a-target-index-and-reorders-them: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH collections and fields `reorder-steps` is
...
async fn produces_committed_diff() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&produced, &committed), "reorder-steps/accepts-a-target-index-and-reorders-them: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "reorder-steps/accepts-a-target-index-and-reorders-them: committed diff JSON is not canonical");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "reorder-steps/accepts-a-target-index-and-reorders-them: committed diff did not carry before to after");
}
