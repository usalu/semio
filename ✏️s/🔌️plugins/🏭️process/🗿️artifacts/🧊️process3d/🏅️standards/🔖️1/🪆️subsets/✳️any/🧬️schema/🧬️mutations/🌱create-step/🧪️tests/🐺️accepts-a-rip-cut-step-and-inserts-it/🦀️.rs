//! 🧪️ `create-step` fixture — `🐺️accepts-a-rip-cut-step-and-inserts-it`.
//!
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "create-step/accepts-a-rip-cut-step-and-inserts-it: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "create-step/accepts-a-rip-cut-step-and-inserts-it: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "create-step/accepts-a-rip-cut-step-and-inserts-it: committed {side} JSON is not canonical");
    }
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "create-step/accepts-a-rip-cut-step-and-inserts-it: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "create-step/accepts-a-rip-cut-step-and-inserts-it: raised diagnostics differ from the committed 🎯️outcome messages");
    ...
    match status {
        "applied" if declared.iter().any(|(_, code)| code == "mutation.no-op") => {
            assert!(applied, "create-step/accepts-a-rip-cut-step-and-inserts-it: declared applied but the mutation was rejected");
            assert_eq!(snapshot, before(), "create-step/accepts-a-rip-cut-step-and-inserts-it: a no-op outcome is applied with an EMPTY diff — the snapshot must come back untouched");
        }
        "applied" => {
            assert!(applied, "create-step/accepts-a-rip-cut-step-and-inserts-it: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "create-step/accepts-a-rip-cut-step-and-inserts-it: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "create-step/accepts-a-rip-cut-step-and-inserts-it: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("create-step/accepts-a-rip-cut-step-and-inserts-it: unknown outcome status {other:?}"),
    }
}
...
async fn produces_committed_diff() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&produced, &committed), "create-step/accepts-a-rip-cut-step-and-inserts-it: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "create-step/accepts-a-rip-cut-step-and-inserts-it: committed diff JSON is not canonical");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "create-step/accepts-a-rip-cut-step-and-inserts-it: committed diff did not carry before to after");
}
