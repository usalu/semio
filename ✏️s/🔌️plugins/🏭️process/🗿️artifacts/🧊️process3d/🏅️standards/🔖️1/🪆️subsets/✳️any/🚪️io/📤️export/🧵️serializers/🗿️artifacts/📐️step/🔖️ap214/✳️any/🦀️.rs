...
pub fn serialize(snapshot: &Process3dSnapshot) -> Result<StepSnapshot, store::TextError> {
    ...
    <StepSnapshot as store::ArtifactPack>::decode_pack(&bytes).map_err(|e| store::TextError::new(e.to_string(), dsl::TextSpan::at(1, 1)))
}

pub fn serialize_bytes(snapshot: &Process3dSnapshot) -> Result<Vec<u8>, store::TextError> {
    Ok(<StepSnapshot as store::ArtifactPack>::encode_pack(&serialize(snapshot)?))
}
