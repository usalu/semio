//! 🧪️ `replace-stock-solid` fixture — `reissues-the-stock-brep-child-handle`.
//!
//! `replace-stock-solid` compares the whole `ArtifactChild` handle for equality and no-ops when unchanged; the diff it writes carries ONLY the two-string handle — the parent never stores brep topology, so swapping geometry is a handle swap.
//!
...
async fn applies_to_committed_after() {
    let (snapshot, _) = protocol::apply_mutation(&before(), &mutation()).expect("replace-stock-solid applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "replace-stock-solid/reissues-the-stock-brep-child-handle: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "replace-stock-solid/reissues-the-stock-brep-child-handle: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "replace-stock-solid/reissues-the-stock-brep-child-handle: committed {side} JSON is not canonical");
    }
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "replace-stock-solid/reissues-the-stock-brep-child-handle: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "replace-stock-solid/reissues-the-stock-brep-child-handle: raised diagnostics differ from the committed 🎯️outcome messages");
    ...
    match status {
        "applied" if declared.iter().any(|(_, code)| code == "mutation.no-op") => {
            assert!(applied, "replace-stock-solid/reissues-the-stock-brep-child-handle: declared applied but the mutation was rejected");
            assert_eq!(snapshot, before(), "replace-stock-solid/reissues-the-stock-brep-child-handle: a no-op outcome is applied with an EMPTY diff — the snapshot must come back untouched");
        }
        "applied" => {
            assert!(applied, "replace-stock-solid/reissues-the-stock-brep-child-handle: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "replace-stock-solid/reissues-the-stock-brep-child-handle: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "replace-stock-solid/reissues-the-stock-brep-child-handle: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("replace-stock-solid/reissues-the-stock-brep-child-handle: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH collections and fields `replace-stock-solid` is
...
async fn produces_committed_diff() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&produced, &committed), "replace-stock-solid/reissues-the-stock-brep-child-handle: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "replace-stock-solid/reissues-the-stock-brep-child-handle: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `replace-stock-solid` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "replace-stock-solid/reissues-the-stock-brep-child-handle: committed diff did not carry before to after");
}
