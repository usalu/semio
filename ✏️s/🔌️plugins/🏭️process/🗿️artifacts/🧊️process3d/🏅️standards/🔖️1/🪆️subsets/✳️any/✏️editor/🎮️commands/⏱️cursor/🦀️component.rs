...
pub mod step_cursor_back {
    ...
    pub fn handle(_payload: &StepCursorBack, doc: &ArtifactView<'_, Process3dSnapshot>, _cfg: &ConfigView<'_, Process3dConfig>, _ctx: &mut crate::editor::process3d::Process3dDispatchCtx) -> Result<Emit<Process3dMutation, Process3dConfigMutation>, Fault> {
        let fixture = doc.snapshot;
        let current = fixture.resolved_up_to.unwrap_or(0) as i64;
...
pub mod step_cursor_forward {
    ...
    pub fn handle(_payload: &StepCursorForward, doc: &ArtifactView<'_, Process3dSnapshot>, _cfg: &ConfigView<'_, Process3dConfig>, _ctx: &mut crate::editor::process3d::Process3dDispatchCtx) -> Result<Emit<Process3dMutation, Process3dConfigMutation>, Fault> {
        let fixture = doc.snapshot;
        let current = fixture.resolved_up_to.unwrap_or(0) as i64;
        Ok(Emit::mutations(vec![Process3dMutation::ChangeCursor(ChangeCursor { new_resolved_up_to: Some((current + 1).max(0) as usize) })]))
    }
