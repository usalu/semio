...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "create-step/accepts-a-rip-cut-step-and-changes-nothing: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "create-step/accepts-a-rip-cut-step-and-changes-nothing: committed mutation JSON is not canonical");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "create-step/accepts-a-rip-cut-step-and-changes-nothing: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "create-step/accepts-a-rip-cut-step-and-changes-nothing: committed diff JSON is not canonical");
}
