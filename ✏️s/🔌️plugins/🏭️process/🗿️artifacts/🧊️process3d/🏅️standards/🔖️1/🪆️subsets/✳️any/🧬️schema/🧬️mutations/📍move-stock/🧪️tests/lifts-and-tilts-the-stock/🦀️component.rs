//! 🧪️ `move-stock` fixture — `lifts-and-tilts-the-stock`.
//!
//! `move-stock` rejects a non-finite position/axis/angle and no-ops on an identical pose; otherwise it sets the single scalar `stockPose` field — the stock's brep child handle is NOT reissued, because a pose is applied by the kernel, not baked into geometry.
//!
...
async fn applies_to_committed_after() {
    let (snapshot, _) = protocol::apply_mutation(&before(), &mutation()).expect("move-stock applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "move-stock/lifts-and-tilts-the-stock: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "move-stock/lifts-and-tilts-the-stock: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "move-stock/lifts-and-tilts-the-stock: committed {side} JSON is not canonical");
    }
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "move-stock/lifts-and-tilts-the-stock: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "move-stock/lifts-and-tilts-the-stock: raised diagnostics differ from the committed 🎯️outcome messages");
    ...
    match status {
        "applied" if declared.iter().any(|(_, code)| code == "mutation.no-op") => {
            assert!(applied, "move-stock/lifts-and-tilts-the-stock: declared applied but the mutation was rejected");
            assert_eq!(snapshot, before(), "move-stock/lifts-and-tilts-the-stock: a no-op outcome is applied with an EMPTY diff — the snapshot must come back untouched");
        }
        "applied" => {
            assert!(applied, "move-stock/lifts-and-tilts-the-stock: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "move-stock/lifts-and-tilts-the-stock: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "move-stock/lifts-and-tilts-the-stock: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("move-stock/lifts-and-tilts-the-stock: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH collections and fields `move-stock` is
...
async fn produces_committed_diff() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&produced, &committed), "move-stock/lifts-and-tilts-the-stock: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "move-stock/lifts-and-tilts-the-stock: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `move-stock` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "move-stock/lifts-and-tilts-the-stock: committed diff did not carry before to after");
}
