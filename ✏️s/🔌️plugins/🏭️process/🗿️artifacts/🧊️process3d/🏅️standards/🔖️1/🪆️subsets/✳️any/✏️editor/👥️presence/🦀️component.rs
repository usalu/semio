...
impl Default for Process3dPresence {
    fn default() -> Self {
        Self {
            ...
            camera_fov: 45.0,
            active_utility_id: crate::editor::process3d::config::PROCESS3D_DEFAULT_UTILITY.into(),
        }
    }
}

impl protocol::MutationDiff<Process3dPresence> for Process3dPresence {
    async fn apply(&self, _base: &Process3dPresence) -> protocol::MutationApplyResult<Process3dPresence> {
        Ok({
            self.clone()
        })
    }
...
impl Mutation<Process3dPresence> for Process3dPresenceMutation {
    ...
    async fn diff(&self, _base: &Process3dPresence) -> protocol::MutationOutcome<Process3dPresence> {
        protocol::MutationOutcome::new(match self {
            Self::Snapshot { presence } => presence.clone(),
        })
    }

    async fn inverse(&self, base: &Process3dPresence) -> Vec<Self> {
