//! 🧪️ `rename-machine` fixture — `🏷️retitles-the-saw`.
//!
//! `rename-machine` no-ops on an identical label; otherwise it clones the machine list, rewrites that one machine's `label`, and republishes the whole `Workshop` — its icon and capabilities ride through unchanged.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔖rename-machine/🏷️retitles-the-saw/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔖rename-machine/🏷️retitles-the-saw/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔖rename-machine/🏷️retitles-the-saw/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔖rename-machine/🏷️retitles-the-saw/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔖rename-machine/🏷️retitles-the-saw/🎯️outcome/🔣️.json");
...
async fn applies_to_committed_after() {
    let (snapshot, _) = protocol::apply_mutation(&before(), &mutation()).expect("rename-machine applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "rename-machine/retitles-the-saw: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "rename-machine/retitles-the-saw: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "rename-machine/retitles-the-saw: committed {side} JSON is not canonical");
    }
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "rename-machine/retitles-the-saw: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "rename-machine/retitles-the-saw: raised diagnostics differ from the committed 🎯️outcome messages");
    ...
    match status {
        "applied" if declared.iter().any(|(_, code)| code == "mutation.no-op") => {
            assert!(applied, "rename-machine/retitles-the-saw: declared applied but the mutation was rejected");
            assert_eq!(snapshot, before(), "rename-machine/retitles-the-saw: a no-op outcome is applied with an EMPTY diff — the snapshot must come back untouched");
        }
        "applied" => {
            assert!(applied, "rename-machine/retitles-the-saw: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "rename-machine/retitles-the-saw: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "rename-machine/retitles-the-saw: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("rename-machine/retitles-the-saw: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH collections and fields `rename-machine` is
...
async fn produces_committed_diff() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&produced, &committed), "rename-machine/retitles-the-saw: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "rename-machine/retitles-the-saw: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `rename-machine` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "rename-machine/retitles-the-saw: committed diff did not carry before to after");
}
