//! 🧪️ `change-step-enabled` fixture — `accepts-a-disable-flag-and-changes-nothing`.
//!
//! `change-step-enabled` sets the id-keyed step's `enabled` flag in `step_payloads` and re-mints `steps`/`tool_solids` (`process3d_step_timeline_diff`) — the flow node's `enabled` param changes, gating the step out of kernel replay; `resolvedUpTo` is untouched.
//!
...
async fn applies_to_committed_after() {
    let (snapshot, _) = protocol::apply_mutation(&before(), &mutation()).expect("change-step-enabled applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "change-step-enabled/accepts-a-disable-flag-and-changes-nothing: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "change-step-enabled/accepts-a-disable-flag-and-changes-nothing: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "change-step-enabled/accepts-a-disable-flag-and-changes-nothing: committed {side} JSON is not canonical");
    }
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "change-step-enabled/accepts-a-disable-flag-and-changes-nothing: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "change-step-enabled/accepts-a-disable-flag-and-changes-nothing: raised diagnostics differ from the committed 🎯️outcome messages");
    ...
    match status {
        "applied" if declared.iter().any(|(_, code)| code == "mutation.no-op") => {
            assert!(applied, "change-step-enabled/accepts-a-disable-flag-and-changes-nothing: declared applied but the mutation was rejected");
            assert_eq!(snapshot, before(), "change-step-enabled/accepts-a-disable-flag-and-changes-nothing: a no-op outcome is applied with an EMPTY diff — the snapshot must come back untouched");
        }
        "applied" => {
            assert!(applied, "change-step-enabled/accepts-a-disable-flag-and-changes-nothing: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "change-step-enabled/accepts-a-disable-flag-and-changes-nothing: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "change-step-enabled/accepts-a-disable-flag-and-changes-nothing: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("change-step-enabled/accepts-a-disable-flag-and-changes-nothing: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH collections and fields `change-step-enabled` is
...
async fn produces_committed_diff() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&produced, &committed), "change-step-enabled/accepts-a-disable-flag-and-changes-nothing: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert!(semio_framework_os_kernel::json::value_eq_ignoring_object_order(&reencoded, &original), "change-step-enabled/accepts-a-disable-flag-and-changes-nothing: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `change-step-enabled` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "change-step-enabled/accepts-a-disable-flag-and-changes-nothing: committed diff did not carry before to after");
}
