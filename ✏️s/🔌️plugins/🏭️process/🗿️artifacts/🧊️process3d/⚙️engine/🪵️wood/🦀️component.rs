//! 🪵️ Process3d engine — built-in wood-shop machine catalog (saws, drill press, CNC router, fasteners).
//!
//! Folded in from the old, standalone `semio-s-plugin-process-wood` crate (formerly a plain `rlib`
//! helper crate under `🧩️extensions/🪵️wood`, never independently loadable — no `[package.metadata.semio]
//! role`, no `project.json`, `crate-type = ["rlib"]` only). Merging it here as an engine topic file
//! avoids the compile-time cycle a separate crate would now create: this engine module depends on the
//! catalog for `installed_catalogs()`, and the catalog needs the artifact's `Capability`/`WorkshopMachine`
//! types — which now live in THIS crate, so a satellite crate depending back on it would be circular.
