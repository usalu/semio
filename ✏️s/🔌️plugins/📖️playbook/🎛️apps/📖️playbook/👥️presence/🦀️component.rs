...
//#region 🔖️Presence
/// 👥️ Peer selection on the block-list builder surface (typed replacement for `selection_json`).
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize, dsl::DslDocument)]
#[serde(rename_all = "camelCase", default)]
#[dsl(extension = "playbook.presence")]
#[dsl(layout = "lines")]
pub struct PlaybookPresence {
    pub selected_ids: Vec<String>,
}

impl Default for PlaybookPresence {
    fn default() -> Self {
...
impl protocol::MutationDiff<PlaybookPresence> for PlaybookPresence {
    fn apply(&self, _base: &PlaybookPresence) -> PlaybookPresence {
        self.clone()
    }
...
impl store::DocumentDsl for PlaybookPresence {
...
impl DocumentPack for PlaybookPresence {
...
impl Mutation<PlaybookPresence> for PlaybookPresenceMutation {
    ...
    fn diff(&self, _base: &PlaybookPresence) -> PlaybookPresence {
        match self {
            Self::Snapshot { presence } => presence.clone(),
        }
    }

    fn inverse(&self, base: &PlaybookPresence) -> Vec<Self> {
