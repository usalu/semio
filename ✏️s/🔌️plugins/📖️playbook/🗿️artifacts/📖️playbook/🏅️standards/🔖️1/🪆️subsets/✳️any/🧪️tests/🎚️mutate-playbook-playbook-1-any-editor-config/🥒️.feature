@capability-playbook-playbook-1-any-editor-config-mutate
@no-oracle-playbook-playbook-1-any-editor-config-state-lane-semantics
@comparison-ordered-json-v1
@mutations-playbook-playbook-1-any-editor-config
Feature: Apply every config state-lane mutation of s.playbook.playbook's ✏️editor/🎚️config to its committed vector
  `PlaybookConfigMutation` is the config lane of this subset's editor: it holds the host-pushed block-kind contribution list (`contributionsJson`) that hot-swaps `playbook.blockKind` installs. It is this
  repository's own state record, so the case records the no-oracle decision `playbook-playbook-1-any-editor-config-state-lane-semantics` in
  ...
  before-snapshot. Production dispatch is reached through `playbook_config_mutation_report_json`, which runs
  ...
  Scenario Outline: Apply <id> and land on the committed after-snapshot, diff and outcome
    ...
    When <id> is applied to that vector's before-snapshot through playbook_config_mutation_report_json
