...
//! fix (`🗒️note/.../🔣️json/🔖️rfc8259/✳️any/🦀️component.rs`) with the reverse structural converter
//! and stdio's own real `write_json_pretty` for `serialize_bytes` (the previous
//! `serde_json::to_vec_pretty(&value)` would have serialized the internally-tagged `JsonValue`
//! shape verbatim, not real JSON text — a latent bug this fix also corrects).
