//! 🧭 `topology` — one named inference: the playbook's step/block dependency structure. Nodes are
//! step ids and block ids (flattened across all steps); edges are (a) sequential document order —
//! each step follows the previous step, and each block follows the previous node within its step —
//! plus (b) `condition` data-dependency edges, added whenever a block's visibility condition
//! references another block's id via `PlaybookExpr::Var`. Topologically sorted with Kahn's
...
async fn collect_condition_vars(expr: &PlaybookExpr, names: &mut Vec<String>) {
    match expr {
        ...
        PlaybookExpr::Eq { left, right } => {
            collect_condition_vars(left, names);
            collect_condition_vars(right, names);
        }
        PlaybookExpr::And { items } | PlaybookExpr::Or { items } => {
            for item in items {
                collect_condition_vars(item, names);
            }
        }
        PlaybookExpr::Truthy { expr: inner } => collect_condition_vars(inner, names),
    }
}
...
async fn topological_sort(nodes: Vec<String>, edges: Vec<(String, String)>) -> PlaybookTopology {
    ...
    PlaybookTopology { topo_order, depth, cycle_free, node_count }
}
//#endregion 🔖️Compute
