...
pub mod derived_composition {
    ...
    pub struct PlaybookComposerComposition;

    impl ArtifactComposition for PlaybookComposerComposition {
        ...
        fn compose(sources: &[ComposeSource<'_>]) -> Result<Composition<Self::Snapshot>, ComposeError> {
            ...
            Err(ComposeError { message: "PlaybookComposerComposition: no source in a known read dialect".into(), diagnostics: Vec::new() })
        }
    }
}
...
pub mod io_registry {
    ...
    const PLAYBOOK_DIALECT: Dialect = Dialect { artifact_kind: "s.playbook.playbook", standard: StandardId("1"), subset: SubsetId("*") };
    ...
    fn rebuild_native_snapshot(sources: &[ErasedComposeSource]) -> Result<crate::PlaybookSnapshot, ComposeError> {
    ...
    fn compose_export_docx(sources: &[ErasedComposeSource]) -> semio_framework_plugin::ComposeFuture<'_> {
        Box::pin(async move {
            let snapshot = rebuild_native_snapshot(sources)?;
            let bytes = crate::io::export::serializers::artifacts::docx::v_ecma_376::any::serialize_bytes(&snapshot).map_err(|e| ComposeError { message: e, diagnostics: Vec::new() })?;
    ...
    fn compose_export_md(sources: &[ErasedComposeSource]) -> semio_framework_plugin::ComposeFuture<'_> {
        Box::pin(async move {
            let snapshot = rebuild_native_snapshot(sources)?;
            let bytes = crate::io::export::serializers::artifacts::md::v_commonmark::any::serialize_bytes(&snapshot).map_err(|e| ComposeError { message: e, diagnostics: Vec::new() })?;
