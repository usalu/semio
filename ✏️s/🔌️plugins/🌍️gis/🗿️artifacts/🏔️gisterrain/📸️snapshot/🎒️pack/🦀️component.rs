...
pub fn decode(bytes: &[u8]) -> Result<GisTerrainSnapshot, PackError> {
    <GisTerrainSnapshot as store::DocumentPack>::decode_pack(bytes)
}
...
mod tests {
    ...
    fn gis3d_terrain_document_pack_agrees_with_dsl_for_arbitrary_exaggeration() {
        store::os_store::test_support::assert_dsl_pack_equivalence(&GisTerrainSnapshot { exaggeration: 2.75, imported_features_json: String::new() });
    }

    #[test]
    fn gis3d_terrain_document_pack_agrees_with_dsl_for_imported_features_json() {
        store::os_store::test_support::assert_dsl_pack_equivalence(&GisTerrainSnapshot { exaggeration: 1.0, imported_features_json: r#"{"positions":[{"id":"p1","lon":1.0,"lat":2.0}],"routes":[],"regions":[]}"#.into() });
    }
}
//#endregion 🧪️Tests
