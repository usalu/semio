//! 📤️ Export GisTerrainSnapshot as .las.

use semio_framework_plugin::{DocumentCodec, IoError, JsonCodec, MediaFormat};

//#region 🔖️Export
pub fn export(snapshot: &crate::artifacts::gisterrain::GisTerrainSnapshot) -> Result<Vec<u8>, IoError> {
...
pub fn register() {
    let kind = "3d.mesh";
    let format = MediaFormat::Las;
    semio_framework_os::register_os_media_export_handler(kind, format, move |doc| {
        let snapshot: crate::artifacts::gisterrain::GisTerrainSnapshot =
