//! 🎹️ GisTerrainComposer (1/✳️any) — analyzer + builder glued. Reads native `s.gisterrain` sources
//! plus any of: stdio.dwg, stdio.glb, stdio.gltf, stdio.json, stdio.las, stdio.obj, stdio.ply, stdio.png, stdio.stl, stdio.txt. Writes one `s.gisterrain` (1/✳️any) snapshot.
...
use crate::artifacts::gisterrain::standards::v1::subsets::any::analyzer::GisTerrainAnalyzer;
...
pub struct GisTerrainComposer;

impl ArtifactComposer for GisTerrainComposer {
    type Snapshot = GisTerrainSnapshot;
    const WRITES: Dialect = DIALECT;

    fn reads() -> &'static [Dialect] {
        &[DIALECT, DEP_DWG, DEP_GLB, DEP_GLTF, DEP_JSON, DEP_LAS, DEP_OBJ, DEP_PLY, DEP_PNG, DEP_STL, DEP_TXT]
    }
    ...
    fn compose(sources: &[ComposeSource]) -> Result<Composition<Self::Snapshot>, ComposeError> {
        for source in sources {
            if source.dialect == DIALECT {
                ...
                let analysis = GisTerrainAnalyzer::analyze(&[native]);
            ...
            if source.dialect == DEP_GLB {
                ...
                if let Ok(snapshot) = crate::artifacts::gisterrain::io::import::deserializers::artifacts::glb::v2_0::any::deserialize_bytes(&bytes) {
                    return Ok(Composition { snapshot, confidence: semio_framework_plugin::IoConfidence::Medium, diagnostics: Vec::new() });
                }
            }
        ...
        Err(ComposeError { message: "GisTerrainComposer: no source in a known read dialect".into(), diagnostics: Vec::new() })
    }
}
