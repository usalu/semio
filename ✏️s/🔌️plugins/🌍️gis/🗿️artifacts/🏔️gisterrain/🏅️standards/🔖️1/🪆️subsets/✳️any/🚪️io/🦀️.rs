//! 🚪️ IO s.gis.gisterrain (1/✳️any) — registration now flows through 🎹️composer::register
//! (called once from ⚙️engine::register), not per-leaf register().
pub fn import_stdio_kinds() -> &'static [&'static str] {
    &["stdio.json", "stdio.txt"]
}
// 🗺️ No "stdio.png"/"stdio.json"/"stdio.dwg" here: gismap owns those EXPORT claims, see `🚪️IoRegistry`
// below. Imports are this document's own json and txt carriers only: the terrain surface is derived from
// the document, so no mesh file can be read into it.
...
pub mod io_registry {
    ...
    const GISTERRAIN_DIALECT: Dialect = Dialect { artifact_kind: "s.gis.gisterrain", standard: StandardId("1"), subset: SubsetId("*") };
    ...
    fn rebuild_native_snapshot(sources: &[ErasedComposeSource]) -> Result<crate::GisTerrainSnapshot, ComposeError> {
