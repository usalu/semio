...
pub fn serialize_bytes(snapshot: &GisTerrainSnapshot) -> Result<Vec<u8>, store::TextError> {
    encode_mesh(&gis_terrain_mesh_from_snapshot(snapshot), SemioMeshFormat::Las).map_err(|error| store::TextError::new(format!("gisterrain→las: {error}"), dsl::TextSpan::at(1, 1)))
}
