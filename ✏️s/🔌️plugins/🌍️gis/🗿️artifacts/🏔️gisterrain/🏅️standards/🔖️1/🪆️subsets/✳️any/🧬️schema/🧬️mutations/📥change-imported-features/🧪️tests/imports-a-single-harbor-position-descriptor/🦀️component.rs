//! 🧪️ `change-imported-features` fixture — `imports-a-single-harbor-position-descriptor`.
//!
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "change-imported-features/imports-a-single-harbor-position-descriptor: applied state differs from committed after-snapshot");
    assert_ne!(snapshot.mesh, base.mesh, "change-imported-features/imports-a-single-harbor-position-descriptor: editing a content-key field must re-mint the derived mesh handle");
    assert!(snapshot.mesh.is_some(), "change-imported-features/imports-a-single-harbor-position-descriptor: the terrain always owns a mesh child — the slot is never emptied by an edit");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "change-imported-features/imports-a-single-harbor-position-descriptor: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-imported-features/imports-a-single-harbor-position-descriptor: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "change-imported-features/imports-a-single-harbor-position-descriptor: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(status, "applied", "change-imported-features/imports-a-single-harbor-position-descriptor: this fixture pins an applied outcome");
    let produced = <GisTerrainMutation as protocol::Mutation<GisTerrainSnapshot>>::diff(&mutation(), &before());
    assert!(produced.messages().is_empty(), "change-imported-features/imports-a-single-harbor-position-descriptor: an applied outcome with no declared messages must emit none, got {:?}", produced.messages());
    let mut snapshot = before();
    apply_gis_terrain_mutation(&mut snapshot, &mutation()).expect("change-imported-features/imports-a-single-harbor-position-descriptor: declared applied but the mutation was rejected");
    assert_ne!(snapshot, before(), "change-imported-features/imports-a-single-harbor-position-descriptor: an applied change-imported-features must actually change the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-imported-features/imports-a-single-harbor-position-descriptor: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "change-imported-features/imports-a-single-harbor-position-descriptor: committed diff JSON is not canonical");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "change-imported-features/imports-a-single-harbor-position-descriptor: committed diff did not carry before to after");
}
...
async fn changes_only_the_imported_descriptor_string_and_inverts_to_the_base_string() {
    ...
    assert!(produced.messages().is_empty(), "change-imported-features/imports-a-single-harbor-position-descriptor: a genuinely different import must be diagnostic-free (the no-op warning is the other branch), got {:?}", produced.messages());
    assert_eq!(produced.diff().imported_features_json.as_deref(), Some(after.imported_features_json.as_str()), "change-imported-features/imports-a-single-harbor-position-descriptor: the delta carries the payload's own descriptor string verbatim");
    assert!(produced.diff().exaggeration.is_none(), "change-imported-features/imports-a-single-harbor-position-descriptor: the terrain's OTHER artifact-lane field must stay out of the delta");
    assert_eq!(after.exaggeration, base.exaggeration, "change-imported-features/imports-a-single-harbor-position-descriptor: the untouched exaggeration must survive the edit verbatim");
    assert_eq!(base.imported_features_json, "null", "change-imported-features/imports-a-single-harbor-position-descriptor: the pre-import state is the literal JSON string \"null\", not an empty field");
    let inverse = inverse_gis_terrain_mutation(&base, &mutation());
    assert_eq!(inverse.len(), 1, "change-imported-features/imports-a-single-harbor-position-descriptor: an import undoes with exactly one step, got {inverse:?}");
    let GisTerrainMutation::ChangeImportedFeatures(undo) = &inverse[0] else {
        panic!("change-imported-features/imports-a-single-harbor-position-descriptor: the inverse must be another change-imported-features, got {:?}", inverse[0]);
    };
