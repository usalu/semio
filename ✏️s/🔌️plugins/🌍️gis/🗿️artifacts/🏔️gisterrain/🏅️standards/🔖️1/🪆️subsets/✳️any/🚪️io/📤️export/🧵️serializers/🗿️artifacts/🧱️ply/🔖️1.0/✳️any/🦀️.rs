//! 🏔️ gisterrain → ply — the terrain's composed surface mesh (`gis_terrain_mesh_from_snapshot`) as
//! PLY: one shared vertex element plus a triangle face list, written by `s.stdio.semio/v1/mesh`'s own export leaf.
//!
...
pub fn serialize_bytes(snapshot: &GisTerrainSnapshot) -> Result<Vec<u8>, store::TextError> {
    encode_mesh(&gis_terrain_mesh_from_snapshot(snapshot), SemioMeshFormat::Ply).map_err(|error| store::TextError::new(format!("gisterrain→ply: {error}"), dsl::TextSpan::at(1, 1)))
}
