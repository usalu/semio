//! 🧪️ `replace-route-data` fixture — `rewrites-the-ferry-route-payload`.
//!
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "replace-route-data/rewrites-the-ferry-route-payload: applied state differs from committed after-snapshot");
    assert_ne!(snapshot.drawing.child_id, base.drawing.child_id, "replace-route-data/rewrites-the-ferry-route-payload: editing routes must re-mint the derived drawing handle");
    assert_ne!(snapshot.value.child_id, base.value.child_id, "replace-route-data/rewrites-the-ferry-route-payload: editing routes must re-mint the derived value handle");
    assert!(snapshot.image.is_none(), "replace-route-data/rewrites-the-ferry-route-payload: gis carries no raster basemap, so the image child stays absent");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "replace-route-data/rewrites-the-ferry-route-payload: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-route-data/rewrites-the-ferry-route-payload: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "replace-route-data/rewrites-the-ferry-route-payload: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(status, "applied", "replace-route-data/rewrites-the-ferry-route-payload: this fixture pins an applied outcome");
    let produced = <GisMapMutation as protocol::Mutation<GisMapSnapshot>>::diff(&mutation(), &before());
    assert!(produced.messages().is_empty(), "replace-route-data/rewrites-the-ferry-route-payload: an applied outcome with no declared messages must emit none, got {:?}", produced.messages());
    let mut snapshot = before();
    apply_gis_map_mutation(&mut snapshot, &mutation()).expect("replace-route-data/rewrites-the-ferry-route-payload: declared applied but the mutation was rejected");
    assert_ne!(snapshot, before(), "replace-route-data/rewrites-the-ferry-route-payload: an applied replace-route-data must actually change the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-route-data/rewrites-the-ferry-route-payload: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "replace-route-data/rewrites-the-ferry-route-payload: committed diff JSON is not canonical");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "replace-route-data/rewrites-the-ferry-route-payload: committed diff did not carry before to after");
}
...
async fn patches_only_the_ferry_payload_and_inverts_to_the_base_payload() {
    ...
    assert!(produced.messages().is_empty(), "replace-route-data/rewrites-the-ferry-route-payload: a genuinely different payload must be diagnostic-free (the no-op warning is the other branch), got {:?}", produced.messages());
    ...
    let inverse = inverse_gis_map_mutation(&base, &mutation());
    assert_eq!(inverse.len(), 1, "replace-route-data/rewrites-the-ferry-route-payload: a payload swap undoes with exactly one step, got {inverse:?}");
    let GisMapMutation::ReplaceRouteData(undo) = &inverse[0] else {
        panic!("replace-route-data/rewrites-the-ferry-route-payload: the inverse must be another replace-route-data, got {:?}", inverse[0]);
    };
