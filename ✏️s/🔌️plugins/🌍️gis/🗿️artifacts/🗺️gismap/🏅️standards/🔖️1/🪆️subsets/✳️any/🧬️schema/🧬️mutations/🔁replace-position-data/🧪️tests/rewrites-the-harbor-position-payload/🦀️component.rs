//! 🧪️ `replace-position-data` fixture — `rewrites-the-harbor-position-payload`.
//!
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "replace-position-data/rewrites-the-harbor-position-payload: applied state differs from committed after-snapshot");
    assert_ne!(snapshot.drawing.child_id, base.drawing.child_id, "replace-position-data/rewrites-the-harbor-position-payload: editing positions must re-mint the derived drawing handle");
    assert_ne!(snapshot.value.child_id, base.value.child_id, "replace-position-data/rewrites-the-harbor-position-payload: editing positions must re-mint the derived value handle");
    assert!(snapshot.image.is_none(), "replace-position-data/rewrites-the-harbor-position-payload: gis carries no raster basemap, so the image child stays absent");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "replace-position-data/rewrites-the-harbor-position-payload: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-position-data/rewrites-the-harbor-position-payload: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "replace-position-data/rewrites-the-harbor-position-payload: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(status, "applied", "replace-position-data/rewrites-the-harbor-position-payload: this fixture pins an applied outcome");
    let produced = <GisMapMutation as protocol::Mutation<GisMapSnapshot>>::diff(&mutation(), &before());
    assert!(produced.messages().is_empty(), "replace-position-data/rewrites-the-harbor-position-payload: an applied outcome with no declared messages must emit none, got {:?}", produced.messages());
    let mut snapshot = before();
    apply_gis_map_mutation(&mut snapshot, &mutation()).expect("replace-position-data/rewrites-the-harbor-position-payload: declared applied but the mutation was rejected");
    assert_ne!(snapshot, before(), "replace-position-data/rewrites-the-harbor-position-payload: an applied replace-position-data must actually change the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-position-data/rewrites-the-harbor-position-payload: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "replace-position-data/rewrites-the-harbor-position-payload: committed diff JSON is not canonical");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "replace-position-data/rewrites-the-harbor-position-payload: committed diff did not carry before to after");
}
...
async fn patches_only_the_harbor_payload_and_inverts_to_the_base_payload() {
    ...
    assert!(produced.messages().is_empty(), "replace-position-data/rewrites-the-harbor-position-payload: a genuinely different payload must be diagnostic-free (the no-op warning is the other branch), got {:?}", produced.messages());
    let delta = produced.diff().positions.as_ref().expect("replace-position-data writes a positions delta");
    assert_eq!(delta.patched.len(), 1, "replace-position-data/rewrites-the-harbor-position-payload: exactly one feature is patched, got {delta:?}");
    assert_eq!(delta.patched[0].id, "pos-harbor", "replace-position-data/rewrites-the-harbor-position-payload: the patch is addressed by the payload's own id");
    assert_eq!(delta.patched[0].patch.data.as_ref(), Some(&after.positions[0].data), "replace-position-data/rewrites-the-harbor-position-payload: the patch carries the committed replacement payload verbatim");
    assert!(delta.added.is_empty() && delta.removed.is_empty() && delta.reordered.is_none(), "replace-position-data/rewrites-the-harbor-position-payload: a payload swap must not add, remove or reorder anything, got {delta:?}");
    assert!(produced.diff().routes.is_none() && produced.diff().regions.is_none(), "replace-position-data/rewrites-the-harbor-position-payload: replace-position-data must never touch the routes or regions collections");
    let inverse = inverse_gis_map_mutation(&base, &mutation());
    assert_eq!(inverse.len(), 1, "replace-position-data/rewrites-the-harbor-position-payload: a payload swap undoes with exactly one step, got {inverse:?}");
    let GisMapMutation::ReplacePositionData(undo) = &inverse[0] else {
        panic!("replace-position-data/rewrites-the-harbor-position-payload: the inverse must be another replace-position-data, got {:?}", inverse[0]);
    };
