//! 🧪️ `delete-region` fixture — `removes-the-old-town-region`.
//!
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "delete-region/removes-the-old-town-region: applied state differs from committed after-snapshot");
    assert_ne!(snapshot.drawing.child_id, base.drawing.child_id, "delete-region/removes-the-old-town-region: editing regions must re-mint the derived drawing handle");
    assert_ne!(snapshot.value.child_id, base.value.child_id, "delete-region/removes-the-old-town-region: editing regions must re-mint the derived value handle");
    assert!(snapshot.image.is_none(), "delete-region/removes-the-old-town-region: gis carries no raster basemap, so the image child stays absent");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "delete-region/removes-the-old-town-region: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "delete-region/removes-the-old-town-region: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "delete-region/removes-the-old-town-region: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(status, "applied", "delete-region/removes-the-old-town-region: this fixture pins an applied outcome");
    let produced = <GisMapMutation as protocol::Mutation<GisMapSnapshot>>::diff(&mutation(), &before());
    assert!(produced.messages().is_empty(), "delete-region/removes-the-old-town-region: an applied outcome with no declared messages must emit none, got {:?}", produced.messages());
    let mut snapshot = before();
    apply_gis_map_mutation(&mut snapshot, &mutation()).expect("delete-region/removes-the-old-town-region: declared applied but the mutation was rejected");
    assert_ne!(snapshot, before(), "delete-region/removes-the-old-town-region: an applied delete-region must actually change the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-region/removes-the-old-town-region: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "delete-region/removes-the-old-town-region: committed diff JSON is not canonical");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "delete-region/removes-the-old-town-region: committed diff did not carry before to after");
}
...
async fn removes_exactly_the_addressed_region_and_leaves_its_kind_sibling() {
    ...
    let inverse = inverse_gis_map_mutation(&base, &mutation());
    assert_eq!(inverse.len(), 1, "delete-region/removes-the-old-town-region: a delete undoes with exactly one step, got {inverse:?}");
    let GisMapMutation::CreateRegion(undo) = &inverse[0] else {
        panic!("delete-region/removes-the-old-town-region: the inverse must be a create-region, got {:?}", inverse[0]);
    };
    assert_eq!(undo.index, 1, "delete-region/removes-the-old-town-region: the inverse restores the region at the index BASE held it at");
    assert_eq!(undo.item, base.regions[1], "delete-region/removes-the-old-town-region: the inverse re-creates the whole BASE feature, payload included");
    let semantics = <GisMapMutation as protocol::SemanticMutation<GisMapSnapshot>>::semantics(&mutation());
    assert_eq!((semantics.verb, semantics.entity, semantics.kind, semantics.record), ("delete", "region", "delete-region", "DeletedRegion"), "delete-region/removes-the-old-town-region: the fixture must be bound to delete-region's own descriptor");
}
