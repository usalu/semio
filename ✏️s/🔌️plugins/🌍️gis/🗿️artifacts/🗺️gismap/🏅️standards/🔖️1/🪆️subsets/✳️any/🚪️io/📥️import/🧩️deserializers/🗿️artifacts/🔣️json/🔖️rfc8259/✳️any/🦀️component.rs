...
use semio_s_plugin_stdio::artifacts::json::standards::v_rfc8259::subsets::any::schema::snapshot::JsonValue;

pub fn register() {}

/// 🌉️ `JsonSnapshot.value` is stdio's own `JsonValue` (never `serde_json::Value` — see that
/// type's own doc comment). Both directions live here since this leaf needs both: `deserialize`
/// reads `JsonValue -> serde_json::Value` (to hand to `serde_json::from_value`), `deserialize_bytes`
/// reads raw JSON text `-> serde_json::Value -> JsonValue` (to build the `JsonSnapshot` this
/// leaf's own `deserialize` expects). Mirrors the sibling export leaf's own copy (same
/// dependency-free, per-leaf convention every other io leaf in this tree already uses, e.g. the
/// semio/drawing svg leaves' mirrored base64 encoder/decoder).
...
fn serde_value_to_json(value: &serde_json::Value) -> JsonValue {
    match value {
        ...
        serde_json::Value::Object(members) => JsonValue::Object {
            members: members.iter().map(|(key, value)| semio_s_plugin_stdio::artifacts::json::standards::v_rfc8259::subsets::any::schema::snapshot::JsonMember { key: key.clone(), value: serde_value_to_json(value) }).collect(),
        },
    }
}
...
pub fn deserialize(from: &JsonSnapshot) -> Result<GisMapSnapshot, store::TextError> {
    ...
    let mut snap: GisMapSnapshot = serde_json::from_value(json_value_to_serde(&from.value))
        .map_err(|e| store::TextError::new(format!("gismap<-json: {e}"), dsl::TextSpan::at(1, 1)))?;
