//! 🧪️ `reorder-regions` fixture — `moves-the-park-region-between-the-two-districts`.
//!
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "reorder-regions/moves-the-park-region-between-the-two-districts: applied state differs from committed after-snapshot");
    assert_ne!(snapshot.drawing.child_id, base.drawing.child_id, "reorder-regions/moves-the-park-region-between-the-two-districts: editing regions must re-mint the derived drawing handle");
    assert_ne!(snapshot.value.child_id, base.value.child_id, "reorder-regions/moves-the-park-region-between-the-two-districts: editing regions must re-mint the derived value handle");
    assert!(snapshot.image.is_none(), "reorder-regions/moves-the-park-region-between-the-two-districts: gis carries no raster basemap, so the image child stays absent");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "reorder-regions/moves-the-park-region-between-the-two-districts: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "reorder-regions/moves-the-park-region-between-the-two-districts: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "reorder-regions/moves-the-park-region-between-the-two-districts: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(status, "applied", "reorder-regions/moves-the-park-region-between-the-two-districts: this fixture pins an applied outcome");
    let produced = <GisMapMutation as protocol::Mutation<GisMapSnapshot>>::diff(&mutation(), &before());
    assert!(produced.messages().is_empty(), "reorder-regions/moves-the-park-region-between-the-two-districts: an applied outcome with no declared messages must emit none, got {:?}", produced.messages());
    let mut snapshot = before();
    apply_gis_map_mutation(&mut snapshot, &mutation()).expect("reorder-regions/moves-the-park-region-between-the-two-districts: declared applied but the mutation was rejected");
    assert_ne!(snapshot, before(), "reorder-regions/moves-the-park-region-between-the-two-districts: an applied reorder-regions must actually change the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "reorder-regions/moves-the-park-region-between-the-two-districts: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "reorder-regions/moves-the-park-region-between-the-two-districts: committed diff JSON is not canonical");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "reorder-regions/moves-the-park-region-between-the-two-districts: committed diff did not carry before to after");
}
...
async fn permutes_the_whole_region_order_including_the_id_that_stayed_put() {
    ...
    let inverse = inverse_gis_map_mutation(&base, &mutation());
    assert_eq!(inverse.len(), 1, "reorder-regions/moves-the-park-region-between-the-two-districts: a reorder undoes with exactly one step, got {inverse:?}");
    let GisMapMutation::ReorderRegions(undo) = &inverse[0] else {
        panic!("reorder-regions/moves-the-park-region-between-the-two-districts: the inverse must be another reorder-regions, got {:?}", inverse[0]);
    };
