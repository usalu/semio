//! 🧪️ `create-route` fixture — `adds-a-tram-route-after-the-ferry`.
//!
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "create-route/adds-a-tram-route-after-the-ferry: applied state differs from committed after-snapshot");
    assert_ne!(snapshot.drawing.child_id, base.drawing.child_id, "create-route/adds-a-tram-route-after-the-ferry: editing routes must re-mint the derived drawing handle");
    assert_ne!(snapshot.value.child_id, base.value.child_id, "create-route/adds-a-tram-route-after-the-ferry: editing routes must re-mint the derived value handle");
    assert!(snapshot.image.is_none(), "create-route/adds-a-tram-route-after-the-ferry: gis carries no raster basemap, so the image child stays absent");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "create-route/adds-a-tram-route-after-the-ferry: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "create-route/adds-a-tram-route-after-the-ferry: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "create-route/adds-a-tram-route-after-the-ferry: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(status, "applied", "create-route/adds-a-tram-route-after-the-ferry: this fixture pins an applied outcome");
    let produced = <GisMapMutation as protocol::Mutation<GisMapSnapshot>>::diff(&mutation(), &before());
    assert!(produced.messages().is_empty(), "create-route/adds-a-tram-route-after-the-ferry: an applied outcome with no declared messages must emit none, got {:?}", produced.messages());
    let mut snapshot = before();
    apply_gis_map_mutation(&mut snapshot, &mutation()).expect("create-route/adds-a-tram-route-after-the-ferry: declared applied but the mutation was rejected");
    assert_ne!(snapshot, before(), "create-route/adds-a-tram-route-after-the-ferry: an applied create-route must actually change the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "create-route/adds-a-tram-route-after-the-ferry: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "create-route/adds-a-tram-route-after-the-ferry: committed diff JSON is not canonical");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "create-route/adds-a-tram-route-after-the-ferry: committed diff did not carry before to after");
}
...
async fn adds_exactly_one_route_and_inverts_to_a_delete_of_that_id() {
    ...
    assert!(produced.messages().is_empty(), "create-route/adds-a-tram-route-after-the-ferry: creating a fresh id must be diagnostic-free, got {:?}", produced.messages());
    let delta = produced.diff().routes.as_ref().expect("create-route writes a routes delta");
    assert_eq!(delta.added.iter().map(|feature| feature.id.as_str()).collect::<Vec<_>>(), vec!["route-tram"], "create-route/adds-a-tram-route-after-the-ferry: exactly the payload's own feature is added");
    assert!(delta.removed.is_empty() && delta.patched.is_empty() && delta.reordered.is_none(), "create-route/adds-a-tram-route-after-the-ferry: a create must not remove, patch or reorder anything, got {delta:?}");
    assert!(produced.diff().positions.is_none() && produced.diff().regions.is_none(), "create-route/adds-a-tram-route-after-the-ferry: create-route must never touch the positions or regions collections");
    let inverse = inverse_gis_map_mutation(&base, &mutation());
    assert_eq!(inverse.len(), 1, "create-route/adds-a-tram-route-after-the-ferry: a create undoes with exactly one step, got {inverse:?}");
    let GisMapMutation::DeleteRoute(undo) = &inverse[0] else {
        panic!("create-route/adds-a-tram-route-after-the-ferry: the inverse must be a delete-route, got {:?}", inverse[0]);
    };
    assert_eq!(undo.id, "route-tram", "create-route/adds-a-tram-route-after-the-ferry: the inverse deletes exactly the id the payload carried");
    let semantics = <GisMapMutation as protocol::SemanticMutation<GisMapSnapshot>>::semantics(&mutation());
    assert_eq!((semantics.verb, semantics.entity, semantics.kind, semantics.record), ("create", "route", "create-route", "CreatedRoute"), "create-route/adds-a-tram-route-after-the-ferry: the fixture must be bound to create-route's own descriptor");
}
