//! 🧪️ `reorder-routes` fixture — `moves-the-bus-route-to-the-front`.
//!
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "reorder-routes/moves-the-bus-route-to-the-front: applied state differs from committed after-snapshot");
    assert_ne!(snapshot.drawing.child_id, base.drawing.child_id, "reorder-routes/moves-the-bus-route-to-the-front: editing routes must re-mint the derived drawing handle");
    assert_ne!(snapshot.value.child_id, base.value.child_id, "reorder-routes/moves-the-bus-route-to-the-front: editing routes must re-mint the derived value handle");
    assert!(snapshot.image.is_none(), "reorder-routes/moves-the-bus-route-to-the-front: gis carries no raster basemap, so the image child stays absent");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "reorder-routes/moves-the-bus-route-to-the-front: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "reorder-routes/moves-the-bus-route-to-the-front: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "reorder-routes/moves-the-bus-route-to-the-front: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(status, "applied", "reorder-routes/moves-the-bus-route-to-the-front: this fixture pins an applied outcome");
    let produced = <GisMapMutation as protocol::Mutation<GisMapSnapshot>>::diff(&mutation(), &before());
    assert!(produced.messages().is_empty(), "reorder-routes/moves-the-bus-route-to-the-front: an applied outcome with no declared messages must emit none, got {:?}", produced.messages());
    let mut snapshot = before();
    apply_gis_map_mutation(&mut snapshot, &mutation()).expect("reorder-routes/moves-the-bus-route-to-the-front: declared applied but the mutation was rejected");
    assert_ne!(snapshot, before(), "reorder-routes/moves-the-bus-route-to-the-front: an applied reorder-routes must actually change the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "reorder-routes/moves-the-bus-route-to-the-front: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "reorder-routes/moves-the-bus-route-to-the-front: committed diff JSON is not canonical");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "reorder-routes/moves-the-bus-route-to-the-front: committed diff did not carry before to after");
}
...
async fn permutes_the_whole_route_order_without_touching_any_payload() {
    ...
    assert!(produced.messages().is_empty(), "reorder-routes/moves-the-bus-route-to-the-front: a genuine move must be diagnostic-free (the no-op warning is the other branch), got {:?}", produced.messages());
    let delta = produced.diff().routes.as_ref().expect("reorder-routes writes a routes delta");
    assert_eq!(delta.reordered.as_deref(), Some(["route-bus".to_string(), "route-ferry".to_string(), "route-tram".to_string()].as_slice()), "reorder-routes/moves-the-bus-route-to-the-front: the delta is the full recomputed id order");
    assert!(delta.added.is_empty() && delta.removed.is_empty() && delta.patched.is_empty(), "reorder-routes/moves-the-bus-route-to-the-front: a reorder must not add, remove or patch anything, got {delta:?}");
    assert!(produced.diff().positions.is_none() && produced.diff().regions.is_none(), "reorder-routes/moves-the-bus-route-to-the-front: reorder-routes must never touch the positions or regions collections");
    let inverse = inverse_gis_map_mutation(&base, &mutation());
    assert_eq!(inverse.len(), 1, "reorder-routes/moves-the-bus-route-to-the-front: a reorder undoes with exactly one step, got {inverse:?}");
    let GisMapMutation::ReorderRoutes(undo) = &inverse[0] else {
        panic!("reorder-routes/moves-the-bus-route-to-the-front: the inverse must be another reorder-routes, got {:?}", inverse[0]);
    };
