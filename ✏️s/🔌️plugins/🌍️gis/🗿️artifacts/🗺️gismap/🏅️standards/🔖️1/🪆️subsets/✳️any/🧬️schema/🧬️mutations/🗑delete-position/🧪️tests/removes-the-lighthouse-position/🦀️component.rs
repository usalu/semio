//! 🧪️ `delete-position` fixture — `removes-the-lighthouse-position`.
//!
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "delete-position/removes-the-lighthouse-position: applied state differs from committed after-snapshot");
    assert_ne!(snapshot.drawing.child_id, base.drawing.child_id, "delete-position/removes-the-lighthouse-position: editing positions must re-mint the derived drawing handle");
    assert_ne!(snapshot.value.child_id, base.value.child_id, "delete-position/removes-the-lighthouse-position: editing positions must re-mint the derived value handle");
    assert!(snapshot.image.is_none(), "delete-position/removes-the-lighthouse-position: gis carries no raster basemap, so the image child stays absent");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "delete-position/removes-the-lighthouse-position: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "delete-position/removes-the-lighthouse-position: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "delete-position/removes-the-lighthouse-position: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(status, "applied", "delete-position/removes-the-lighthouse-position: this fixture pins an applied outcome");
    let produced = <GisMapMutation as protocol::Mutation<GisMapSnapshot>>::diff(&mutation(), &before());
    assert!(produced.messages().is_empty(), "delete-position/removes-the-lighthouse-position: an applied outcome with no declared messages must emit none, got {:?}", produced.messages());
    let mut snapshot = before();
    apply_gis_map_mutation(&mut snapshot, &mutation()).expect("delete-position/removes-the-lighthouse-position: declared applied but the mutation was rejected");
    assert_ne!(snapshot, before(), "delete-position/removes-the-lighthouse-position: an applied delete-position must actually change the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-position/removes-the-lighthouse-position: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "delete-position/removes-the-lighthouse-position: committed diff JSON is not canonical");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "delete-position/removes-the-lighthouse-position: committed diff did not carry before to after");
}
...
async fn removes_exactly_one_position_and_inverts_to_a_base_derived_create() {
    ...
    assert!(produced.messages().is_empty(), "delete-position/removes-the-lighthouse-position: deleting a present id must be diagnostic-free, got {:?}", produced.messages());
    let delta = produced.diff().positions.as_ref().expect("delete-position writes a positions delta");
    assert_eq!(delta.removed, vec!["pos-lighthouse".to_string()], "delete-position/removes-the-lighthouse-position: exactly the payload's own id is removed");
    assert!(delta.added.is_empty() && delta.patched.is_empty() && delta.reordered.is_none(), "delete-position/removes-the-lighthouse-position: a delete must not add, patch or reorder anything, got {delta:?}");
    assert!(produced.diff().routes.is_none() && produced.diff().regions.is_none(), "delete-position/removes-the-lighthouse-position: delete-position must never touch the routes or regions collections");
    let inverse = inverse_gis_map_mutation(&base, &mutation());
    assert_eq!(inverse.len(), 1, "delete-position/removes-the-lighthouse-position: a delete undoes with exactly one step, got {inverse:?}");
    let GisMapMutation::CreatePosition(undo) = &inverse[0] else {
        panic!("delete-position/removes-the-lighthouse-position: the inverse must be a create-position, got {:?}", inverse[0]);
    };
