...
use crate::editor::gis2d::{gis2d_action, gis2d_layer_tree_item, ui_label, ui_node_list, ui_value_map, ui_value_text, GIS_MAP_LAYER_IDS};
...
pub fn render(labels: &Gis2dPlayLabels) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::BuiltNode> {
    ...
    let items = ui_node_list(GIS_MAP_LAYER_IDS.iter().map(|(id, _, icon)| {
        ...
        gis2d_layer_tree_item(builder.item_id("layer", id)?, ui_label(gis2d_layer_label(id, labels))?, None, icon, Some(gis2d_action("toggleLayerVisibility", Some(args))?))
    }))?;
    builder.section("gis2d-play-catalogue.layers", Some(ui_label(FRAMEWORK_PANEL_TAB_CATALOGUE_LABEL)?), true, items)?.build()
}
//#endregion 🔖️Render
