...
fn fixture() -> serde_json::Value {
    serde_json::from_str(include_str!("../../🧫️fixtures/🧬️mutations/🪟️map-window-config-direct/🔣️.json")).expect("configuration law fixture")
}
    ...
    T: MutationLeaf + dsl::ToValue + dsl::FromValue + PartialEq + std::fmt::Debug,
{
    ...
    assert_eq!(MapWindowConfigMutation::decode_op(&bytes).unwrap(), operation);
...
fn neutral_envelopes_share_json_text_binary_and_inverse_contracts() {
    ...
    assert!(MapWindowConfigMutation::decode_op(&[0, 0]).is_err());
...
//#region 🧪️Identity
fn populated() -> MapWindowConfig {
    let mut value = MapWindowConfig::default();
...
fn visibility_inverse_distinguishes_absence_and_explicit_default() {
    for previous in [None, Some(false), Some(true)] {
        ...
        for visible in [None, Some(false), Some(true)] {
            let operation = MapWindowConfigMutation::SetLayerVisibility(SetLayerVisibility { layer_id: "water".into(), visible });
...
fn stroke_inverse_distinguishes_absence_and_explicit_default() {
    for previous in [None, Some(1.0), Some(2.0)] {
        ...
        for value in [None, Some(1.0), Some(2.0)] {
            let operation = MapWindowConfigMutation::SetLayerStrokeScale(SetLayerStrokeScale { layer_id: "roads".into(), value });
            assert_eq!(undo(&base, &operation), base);
...
fn independent_sparse_writes_compose_and_serde_retains_removal() {
    let base = populated();
    let camera = MapWindowConfigMutation::SetCamera(SetCamera { camera_json: "{}".into() });
