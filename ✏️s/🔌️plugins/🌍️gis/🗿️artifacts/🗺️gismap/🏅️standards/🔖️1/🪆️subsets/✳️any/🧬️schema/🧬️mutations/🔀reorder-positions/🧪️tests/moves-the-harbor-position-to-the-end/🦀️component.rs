//! 🧪️ `reorder-positions` fixture — `moves-the-harbor-position-to-the-end`.
//!
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "reorder-positions/moves-the-harbor-position-to-the-end: applied state differs from committed after-snapshot");
    assert_ne!(snapshot.drawing.child_id, base.drawing.child_id, "reorder-positions/moves-the-harbor-position-to-the-end: editing positions must re-mint the derived drawing handle");
    assert_ne!(snapshot.value.child_id, base.value.child_id, "reorder-positions/moves-the-harbor-position-to-the-end: editing positions must re-mint the derived value handle");
    assert!(snapshot.image.is_none(), "reorder-positions/moves-the-harbor-position-to-the-end: gis carries no raster basemap, so the image child stays absent");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "reorder-positions/moves-the-harbor-position-to-the-end: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "reorder-positions/moves-the-harbor-position-to-the-end: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "reorder-positions/moves-the-harbor-position-to-the-end: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(status, "applied", "reorder-positions/moves-the-harbor-position-to-the-end: this fixture pins an applied outcome");
    let produced = <GisMapMutation as protocol::Mutation<GisMapSnapshot>>::diff(&mutation(), &before());
    assert!(produced.messages().is_empty(), "reorder-positions/moves-the-harbor-position-to-the-end: an applied outcome with no declared messages must emit none, got {:?}", produced.messages());
    let mut snapshot = before();
    apply_gis_map_mutation(&mut snapshot, &mutation()).expect("reorder-positions/moves-the-harbor-position-to-the-end: declared applied but the mutation was rejected");
    assert_ne!(snapshot, before(), "reorder-positions/moves-the-harbor-position-to-the-end: an applied reorder-positions must actually change the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "reorder-positions/moves-the-harbor-position-to-the-end: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "reorder-positions/moves-the-harbor-position-to-the-end: committed diff JSON is not canonical");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "reorder-positions/moves-the-harbor-position-to-the-end: committed diff did not carry before to after");
}
...
async fn permutes_the_whole_position_order_without_touching_any_payload() {
    ...
    assert!(produced.messages().is_empty(), "reorder-positions/moves-the-harbor-position-to-the-end: a genuine move must be diagnostic-free (the no-op warning is the other branch), got {:?}", produced.messages());
    let delta = produced.diff().positions.as_ref().expect("reorder-positions writes a positions delta");
    assert_eq!(delta.reordered.as_deref(), Some(["pos-lighthouse".to_string(), "pos-quay".to_string(), "pos-harbor".to_string()].as_slice()), "reorder-positions/moves-the-harbor-position-to-the-end: the delta is the full recomputed id order");
    assert!(delta.added.is_empty() && delta.removed.is_empty() && delta.patched.is_empty(), "reorder-positions/moves-the-harbor-position-to-the-end: a reorder must not add, remove or patch anything, got {delta:?}");
    assert!(produced.diff().routes.is_none() && produced.diff().regions.is_none(), "reorder-positions/moves-the-harbor-position-to-the-end: reorder-positions must never touch the routes or regions collections");
    let inverse = inverse_gis_map_mutation(&base, &mutation());
    assert_eq!(inverse.len(), 1, "reorder-positions/moves-the-harbor-position-to-the-end: a reorder undoes with exactly one step, got {inverse:?}");
    let GisMapMutation::ReorderPositions(undo) = &inverse[0] else {
        panic!("reorder-positions/moves-the-harbor-position-to-the-end: the inverse must be another reorder-positions, got {:?}", inverse[0]);
    };
    assert_eq!((undo.id.as_str(), undo.to_index), ("pos-harbor", 0), "reorder-positions/moves-the-harbor-position-to-the-end: the inverse sends the feature back to the index BASE held it at");
    let semantics = <GisMapMutation as protocol::SemanticMutation<GisMapSnapshot>>::semantics(&mutation());
    assert_eq!((semantics.verb, semantics.entity, semantics.kind, semantics.record), ("reorder", "positions", "reorder-positions", "ReorderedPositions"), "reorder-positions/moves-the-harbor-position-to-the-end: the fixture must be bound to reorder-positions' own descriptor");
}
