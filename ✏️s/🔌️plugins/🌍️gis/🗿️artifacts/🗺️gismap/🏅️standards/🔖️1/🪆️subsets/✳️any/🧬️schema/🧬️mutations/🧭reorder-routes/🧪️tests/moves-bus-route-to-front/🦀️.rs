//! 🧪️ `reorder-routes` fixture — `moves-bus-route-to-front`.
//!
...
/// ▶️ `reorder-routes` carries `before` to exactly the committed `after`, and — because the edited
...
async fn applies_to_committed_after() {
    ...
    assert_ne!(snapshot.drawing.child_id, base.drawing.child_id, "reorder-routes/moves-bus-route-to-front: editing routes must re-mint the derived drawing handle");
    assert_ne!(snapshot.value.child_id, base.value.child_id, "reorder-routes/moves-bus-route-to-front: editing routes must re-mint the derived value handle");
...
/// ↩️ Applying `reorder-routes` then its inverse restores `before` exactly — including both derived
...
async fn permutes_the_whole_route_order_without_touching_any_payload() {
    ...
    assert_eq!(
        ...
        "reorder-routes/moves-bus-route-to-front: the fixture must be bound to reorder-routes' own descriptor"
    );
}
