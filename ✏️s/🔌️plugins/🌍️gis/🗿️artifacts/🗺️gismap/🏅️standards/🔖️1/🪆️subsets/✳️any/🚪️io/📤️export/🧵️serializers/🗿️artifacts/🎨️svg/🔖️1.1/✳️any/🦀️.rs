//! 🗺️ gismap → svg — the map's page drawing (`gis_map_snapshot_to_drawing`: markers, routes and
//! regions, north up) written as SVG 1.1 by `s.stdio.semio/v1/drawing`'s own export leaf.
//!
...
pub fn serialize_bytes(snapshot: &GisMapSnapshot) -> Result<Vec<u8>, store::TextError> {
    encode_drawing(&gis_map_snapshot_to_drawing(snapshot), SemioDrawingFormat::Svg).map_err(|error| store::TextError::new(format!("gismap→svg: {error}"), dsl::TextSpan::at(1, 1)))
}
