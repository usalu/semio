...
impl protocol::MutationDiff<Gis2dPresence> for Gis2dPresence {
    async fn apply(&self, _base: &Gis2dPresence) -> protocol::MutationApplyResult<Gis2dPresence> {
        Ok({
            self.clone()
        })
    }
...
impl Mutation<Gis2dPresence> for Gis2dPresenceMutation {
    ...
    async fn diff(&self, base: &Gis2dPresence) -> protocol::MutationOutcome<Gis2dPresence> {
        match self {
    ...
    async fn inverse(&self, base: &Gis2dPresence) -> Vec<Self> {
