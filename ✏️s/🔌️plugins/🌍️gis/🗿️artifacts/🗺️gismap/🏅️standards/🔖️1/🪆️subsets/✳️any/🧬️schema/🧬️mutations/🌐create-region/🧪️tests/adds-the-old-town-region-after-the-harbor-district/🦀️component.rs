//! 🧪️ `create-region` fixture — `adds-the-old-town-region-after-the-harbor-district`.
//!
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "create-region/adds-the-old-town-region-after-the-harbor-district: applied state differs from committed after-snapshot");
    assert_ne!(snapshot.drawing.child_id, base.drawing.child_id, "create-region/adds-the-old-town-region-after-the-harbor-district: editing regions must re-mint the derived drawing handle");
    assert_ne!(snapshot.value.child_id, base.value.child_id, "create-region/adds-the-old-town-region-after-the-harbor-district: editing regions must re-mint the derived value handle");
    assert!(snapshot.image.is_none(), "create-region/adds-the-old-town-region-after-the-harbor-district: gis carries no raster basemap, so the image child stays absent");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "create-region/adds-the-old-town-region-after-the-harbor-district: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "create-region/adds-the-old-town-region-after-the-harbor-district: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "create-region/adds-the-old-town-region-after-the-harbor-district: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(status, "applied", "create-region/adds-the-old-town-region-after-the-harbor-district: this fixture pins an applied outcome");
    let produced = <GisMapMutation as protocol::Mutation<GisMapSnapshot>>::diff(&mutation(), &before());
    assert!(produced.messages().is_empty(), "create-region/adds-the-old-town-region-after-the-harbor-district: an applied outcome with no declared messages must emit none, got {:?}", produced.messages());
    let mut snapshot = before();
    apply_gis_map_mutation(&mut snapshot, &mutation()).expect("create-region/adds-the-old-town-region-after-the-harbor-district: declared applied but the mutation was rejected");
    assert_ne!(snapshot, before(), "create-region/adds-the-old-town-region-after-the-harbor-district: an applied create-region must actually change the document");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "create-region/adds-the-old-town-region-after-the-harbor-district: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "create-region/adds-the-old-town-region-after-the-harbor-district: committed diff JSON is not canonical");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "create-region/adds-the-old-town-region-after-the-harbor-district: committed diff did not carry before to after");
}
...
async fn adds_exactly_one_region_and_inverts_to_a_delete_of_that_id() {
    ...
    assert!(produced.messages().is_empty(), "create-region/adds-the-old-town-region-after-the-harbor-district: creating a fresh id must be diagnostic-free even when an existing region shares its kind, got {:?}", produced.messages());
    let delta = produced.diff().regions.as_ref().expect("create-region writes a regions delta");
    assert_eq!(delta.added.iter().map(|feature| feature.id.as_str()).collect::<Vec<_>>(), vec!["region-old-town"], "create-region/adds-the-old-town-region-after-the-harbor-district: exactly the payload's own feature is added");
    assert!(delta.removed.is_empty() && delta.patched.is_empty() && delta.reordered.is_none(), "create-region/adds-the-old-town-region-after-the-harbor-district: a create must not remove, patch or reorder anything, got {delta:?}");
    assert!(produced.diff().positions.is_none() && produced.diff().routes.is_none(), "create-region/adds-the-old-town-region-after-the-harbor-district: create-region must never touch the positions or routes collections");
    let inverse = inverse_gis_map_mutation(&base, &mutation());
    assert_eq!(inverse.len(), 1, "create-region/adds-the-old-town-region-after-the-harbor-district: a create undoes with exactly one step, got {inverse:?}");
    let GisMapMutation::DeleteRegion(undo) = &inverse[0] else {
        panic!("create-region/adds-the-old-town-region-after-the-harbor-district: the inverse must be a delete-region, got {:?}", inverse[0]);
    };
