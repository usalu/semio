...
use semio_framework_plugin::{
    ...
    ActionKind, App, AppIo, AppLabels, ArtifactKindSpec, ConfigView, DocumentApp, DocumentView, Emit, Fault, Label, Locale, LocalizedLabel, MeasureSelectItem, Media, MediaClass, MediaError, MediaForm, MediaPayload, MediaType, Menu, OsMediaCapability,
    ...
    fn handle(&self, command: &Gis2dCommand, doc: &DocumentView<'_, gis2d::GisMapDocument>, cfg: &ConfigView<'_, Gis2dConfig>) -> Result<Emit<GisMapOperation, Gis2dConfigOperation>, Fault> {
        ...
        match command {
            ...
            Gis2dCommand::SetActiveExample { example_id } => {
                ...
                Ok(Emit {
                    document_operations: vec![GisMapOperation::SetDocument { document: next }],
                    config_operations,
                    ..Default::default()
                })
            }
            Gis2dCommand::PatchPositions { positions_json } => {
                ...
                Ok(Emit::operations(positions_operations(&document.positions, &next)))
            }
            Gis2dCommand::PatchRoutes { route_ids, field, value } => Ok(patch_routes_operations(document, route_ids, field, value)),
            Gis2dCommand::PatchRoute { route_id, field, value } => Ok(patch_routes_operations(document, std::slice::from_ref(route_id), field, value)),
            ...
            Gis2dCommand::ToggleLayerVisibility { layer_id } => {
                ...
                Ok(Emit::config(vec![Gis2dConfigOperation::SetLayerVisibility { layer_id: layer_id.clone(), visible }]))
            }
