...
/// `Mutation::Diff` is the WHOLE `Gis2dConfig` (not a granular patch type): `diff()` returns "the full
/// config after this op", and `MutationDiff<Gis2dConfig>::apply` for `Gis2dConfig` itself (generated by
/// `store::impl_whole_record_config!`) just returns that snapshot verbatim, ignoring `base`.
// 🧯️ `clippy::large_enum_variant` — `Snapshot` deliberately carries the WHOLE `Gis2dConfig` (12
// fields) while every other row carries one scalar: that whole-config snapshot IS the inverse
// mechanism every variant's `backwards()` returns. Boxing it would change the derived `dsl::DslOps`
// wire encoding, which this migration must preserve byte-for-byte, so the size skew is accepted by
// design rather than papered over.
...
impl Mutation<Gis2dConfig> for Gis2dConfigMutation {
    ...
    fn diff(&self, base: &Gis2dConfig) -> Gis2dConfig {
        ...
        match self {
            Gis2dConfigMutation::Snapshot { config } => return config.clone(),
    ...
    fn inverse(&self, base: &Gis2dConfig) -> Vec<Self> {
        vec![Gis2dConfigMutation::Snapshot { config: base.clone() }]
    }
}
...
mod tests {
    ...
    fn gis2d_config_operation_backwards_restores_the_pre_operation_snapshot() {
        ...
        assert_eq!(backwards, vec![Gis2dConfigMutation::Snapshot { config: base.clone() }]);
