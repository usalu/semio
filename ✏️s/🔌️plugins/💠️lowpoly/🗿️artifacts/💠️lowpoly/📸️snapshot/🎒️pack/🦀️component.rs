...
/// 📦️ Encodes a `LowpolySnapshot` to its binary pack form.
pub fn encode(projection: &LowpolySnapshot) -> Vec<u8> {
    store::DocumentPack::encode_pack(projection)
}

/// 📖️ Decodes a `LowpolySnapshot` from its binary pack form.
pub fn decode(bytes: &[u8]) -> Result<LowpolySnapshot, PackError> {
...
mod tests {
    ...
    fn command_envelope_round_trip_holds_for_an_applied_operation() {
        ...
        let mut store: DocumentStore<LowpolySnapshot, LowpolyMutation> = DocumentStore::new(create_document_envelope(LOWPOLY_DOCUMENT_SCHEMA, "test-doc", projection, None));
        ...
        semio_framework_os_kernel::os_store::test_support::assert_command_envelope_round_trip::<LowpolySnapshot, LowpolyMutation>(edit, &DocumentId(store.envelope().id.clone()), &SchemaId(store.envelope().schema.clone()));
    }
