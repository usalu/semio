...
Feature: Round-trip the lowpoly document through every stdio format the Rust IO layer declares
  ...
  `ply`, `png`, `stl`, `txt`. Every scenario here takes the SAME committed `LowpolySnapshot` fixture
  ...
  comment in `🗿️artifacts/💠️lowpoly/🦀️component.rs`. Because of that, four of these nine exporters
  ...
  `🧪️oracle/📦️packages/🦀️rust` crate) could still validate that the five working exporters (`obj`,
  `ply`, `json`, `png`, `txt`) emit well-formed bytes in their target grammar, but reaching that crate
  ...
  📌️ FOUR OF NINE SCENARIOS ARE EXPECTED TO FAIL TODAY, ON PURPOSE: `dwg`, `gltf`, `las`, `stl` are
