//! 🎹️ LowpolyComposer (1/✳️any) — analyzer + builder glued. Reads native `s.lowpoly` sources
//! plus any of: stdio.dwg, stdio.glb, stdio.gltf, stdio.json, stdio.las, stdio.obj, stdio.ply, stdio.png, stdio.stl, stdio.txt. Writes one `s.lowpoly` (1/✳️any) snapshot.
...
use crate::artifacts::lowpoly::standards::v1::subsets::any::analyzer::LowpolyAnalyzer;
...
pub struct LowpolyComposer;

impl ArtifactComposer for LowpolyComposer {
    ...
    fn compose(sources: &[ComposeSource]) -> Result<Composition<Self::Snapshot>, ComposeError> {
        for source in sources {
            ...
            if source.dialect == DEP_GLB {
                ...
                if let Ok(snapshot) = crate::artifacts::lowpoly::io::import::deserializers::artifacts::glb::v2_0::any::deserialize_bytes(&bytes) {
                    return Ok(Composition { snapshot, confidence: semio_framework_plugin::IoConfidence::Medium, diagnostics: Vec::new() });
                }
            }
        ...
        Err(ComposeError { message: "LowpolyComposer: no source in a known read dialect".into(), diagnostics: Vec::new() })
    }
}
