...
use crate::editor::lowpoly::{lowpoly_action, ui_label, ui_node_list, ui_value_map, ui_value_number};
use semio_framework_plugin::{tree_item_with_action, LocalizedLabel, PanelGroup, PanelTabDefinition, PanelTabKind, PanelTreeBuilder, PluginAssemblyError, UiText};
...
pub fn render(view: LowpolyView<'_>, labels: &LowpolyLabels) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::BuiltNode> {
    ...
    PanelTreeBuilder::new("lowpoly-play-layers")?.section("lowpoly-play-layers.paint", Some(ui_label(labels.paint_layers.as_str())?), true, items)?.selected([format!("lowpoly-layer:{active_layer}")])?.build()
}
//#endregion 🔖️Render
