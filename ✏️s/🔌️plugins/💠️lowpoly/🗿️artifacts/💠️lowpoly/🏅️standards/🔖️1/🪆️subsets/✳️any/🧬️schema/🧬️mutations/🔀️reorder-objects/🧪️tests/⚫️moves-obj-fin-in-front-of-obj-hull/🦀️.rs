//! 🧪️ `reorder-objects` fixture — `⚫️moves-obj-fin-in-front-of-obj-hull`.
//!
...
async fn applies_to_committed_after() {
    let (snapshot, _) = protocol::apply_mutation(&before(), &mutation()).expect("reorder-objects applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "reorder-objects/moves-obj-fin-in-front-of-obj-hull: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "reorder-objects/moves-obj-fin-in-front-of-obj-hull: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "reorder-objects/moves-obj-fin-in-front-of-obj-hull: committed {side} JSON is not canonical");
    }
    let reencoded = to_json(&mutation());
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "reorder-objects/moves-obj-fin-in-front-of-obj-hull: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "reorder-objects/moves-obj-fin-in-front-of-obj-hull: raised diagnostics differ from the committed 🎯️outcome messages");
    ...
    match status {
        "applied" if declared.iter().any(|(_, code)| code == "mutation.no-op") => {
            assert!(applied, "reorder-objects/moves-obj-fin-in-front-of-obj-hull: declared applied but the mutation was rejected");
            assert_eq!(snapshot, before(), "reorder-objects/moves-obj-fin-in-front-of-obj-hull: a no-op outcome is applied with an EMPTY diff — the snapshot must come back untouched");
        }
        "applied" => {
            assert!(applied, "reorder-objects/moves-obj-fin-in-front-of-obj-hull: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "reorder-objects/moves-obj-fin-in-front-of-obj-hull: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "reorder-objects/moves-obj-fin-in-front-of-obj-hull: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("reorder-objects/moves-obj-fin-in-front-of-obj-hull: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH collections and fields `reorder-objects` is
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "reorder-objects/moves-obj-fin-in-front-of-obj-hull: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "reorder-objects/moves-obj-fin-in-front-of-obj-hull: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields the committed `after` — the diff is a
/// complete description of what `reorder-objects` changed, not a summary of it.
#[semio_framework_async_macros::async_test]
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "reorder-objects/moves-obj-fin-in-front-of-obj-hull: committed diff did not carry before to after");
}
