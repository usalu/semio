...
impl protocol::MutationDiff<LowpolyPresence> for LowpolyPresence {
    async fn apply(&self, _base: &LowpolyPresence) -> protocol::MutationApplyResult<LowpolyPresence> {
        Ok({
            self.clone()
        })
    }
...
impl Mutation<LowpolyPresence> for LowpolyPresenceMutation {
    ...
    async fn diff(&self, _base: &LowpolyPresence) -> protocol::MutationOutcome<LowpolyPresence> {
        protocol::MutationOutcome::new(match self {
            Self::Snapshot { presence } => presence.clone(),
        })
    }

    async fn inverse(&self, base: &LowpolyPresence) -> Vec<Self> {
