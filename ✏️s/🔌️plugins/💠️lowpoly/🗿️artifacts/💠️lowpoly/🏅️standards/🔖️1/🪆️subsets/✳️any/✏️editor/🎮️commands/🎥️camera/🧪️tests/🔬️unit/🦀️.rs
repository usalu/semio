...
async fn set_camera_updates_config() {
    ...
    assert!(a.dispatch_typed(LowpolyCommand::SetCamera(super::set_camera::SetCamera { position: [1.0, 2.0, 3.0], target: [0.0, 0.0, 0.0], fov: 45.0 }), &semio_framework_plugin::artifact_app_laws::meta("a")).await.is_ok());
}
