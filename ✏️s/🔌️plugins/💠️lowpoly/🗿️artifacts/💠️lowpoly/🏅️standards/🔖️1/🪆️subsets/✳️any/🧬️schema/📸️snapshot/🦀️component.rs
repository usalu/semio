...
pub(crate) fn enc_lowpoly_transform(t: &LowpolyTransform) -> String {
    format!("[{},{},{},{},{},{},{},{},{}]", t.position[0], t.position[1], t.position[2], t.rotation[0], t.rotation[1], t.rotation[2], t.scale[0], t.scale[1], t.scale[2],)
}
pub(crate) fn dec_lowpoly_transform(s: &str) -> Result<LowpolyTransform, String> {
    let parts = split_top_level(strip_brackets(s)?, ',');
...
pub(crate) fn dec_paint_layer_list(s: &str) -> Result<Vec<LowpolyPaintLayer>, String> {
...
pub(crate) fn dec_object_list(s: &str) -> Result<Vec<LowpolyObject>, String> {
    split_top_level(strip_brackets(s)?, ',').into_iter().filter(|s| !s.is_empty()).map(dec_object).collect()
}
//#endregion 🔖️CodecPrimitives

//#region 🔖️TextPrimitives
fn print_lowpoly_snapshot_body(s: &LowpolySnapshot) -> String {
    format!("schema={}\nobjects={}", enc_str(&s.schema), enc_object_list(&s.objects))
}
fn parse_lowpoly_snapshot_body(body: &str) -> Result<LowpolySnapshot, String> {
    let mut schema = None;
    ...
    for line in body.lines() {
        ...
        if let Some(rest) = line.strip_prefix("schema=") {
            ...
            objects = dec_object_list(rest)?;
        } else {
            return Err(format!("lowpoly snapshot: unknown line {line:?}"));
        }
    }
    Ok(LowpolySnapshot { schema: schema.ok_or_else(|| "lowpoly snapshot: missing schema line".to_string())?, objects })
}
...
fn write_lowpoly_transform(out: &mut Vec<u8>, t: &LowpolyTransform) {
    for v in t.position.iter().chain(t.rotation.iter()).chain(t.scale.iter()) {
        out.extend_from_slice(&v.to_le_bytes());
    }
}
fn read_lowpoly_transform(reader: &mut store::ByteReader<'_>) -> Result<LowpolyTransform, String> {
...
fn write_paint_layer(out: &mut Vec<u8>, l: &LowpolyPaintLayer) {
...
fn read_paint_layer(reader: &mut store::ByteReader<'_>) -> Result<LowpolyPaintLayer, String> {
...
fn write_paint_layer_list(out: &mut Vec<u8>, list: &[LowpolyPaintLayer]) {
    store::pack_rt::write_varint_u64(out, list.len() as u64);
    for l in list {
        write_paint_layer(out, l);
    }
}
fn read_paint_layer_list(reader: &mut store::ByteReader<'_>) -> Result<Vec<LowpolyPaintLayer>, String> {
    let count = reader.read_varint_u64().map_err(|e| e.to_string())?;
    (0..count).map(|_| read_paint_layer(reader)).collect()
}

fn write_object(out: &mut Vec<u8>, o: &LowpolyObject) {
