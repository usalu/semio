//! 🧪️ `change-paint-layer-opacity` fixture — `🌫️fades-the-base-layer-to-half`.
//!
//! `change-paint-layer-opacity` compares the stored `f32` for exact equality before writing, and its indexed layer patch sets only `opacity` — the layer stays visible and keeps its blend mode.
//!
...
const BEFORE: &str = include_str!("../../../../../📄️documents/🧬️mutations/🌫️change-paint-layer-opacity/🌫️fades-the-base-layer-to-half/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../📄️documents/🧬️mutations/🌫️change-paint-layer-opacity/🌫️fades-the-base-layer-to-half/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../📄️documents/🧬️mutations/🌫️change-paint-layer-opacity/🌫️fades-the-base-layer-to-half/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../📄️documents/🧬️mutations/🌫️change-paint-layer-opacity/🌫️fades-the-base-layer-to-half/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../📄️documents/🧬️mutations/🌫️change-paint-layer-opacity/🌫️fades-the-base-layer-to-half/🎯️outcome/🔣️.json");
...
async fn applies_to_committed_after() {
    let (snapshot, _) = protocol::apply_mutation(&before(), &mutation()).expect("change-paint-layer-opacity applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "change-paint-layer-opacity/fades-the-base-layer-to-half: applied state differs from committed after-snapshot");
}
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "change-paint-layer-opacity/fades-the-base-layer-to-half: inverse did not restore the before-snapshot");
}
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "change-paint-layer-opacity/fades-the-base-layer-to-half: raised diagnostics differ from the committed 🎯️outcome messages");
    ...
    match status {
        "applied" if declared.iter().any(|(_, code)| code == "mutation.no-op") => {
            assert!(applied, "change-paint-layer-opacity/fades-the-base-layer-to-half: declared applied but the mutation was rejected");
            assert_eq!(snapshot, before(), "change-paint-layer-opacity/fades-the-base-layer-to-half: a no-op outcome is applied with an EMPTY diff — the snapshot must come back untouched");
        }
        "applied" => {
            assert!(applied, "change-paint-layer-opacity/fades-the-base-layer-to-half: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "change-paint-layer-opacity/fades-the-base-layer-to-half: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "change-paint-layer-opacity/fades-the-base-layer-to-half: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("change-paint-layer-opacity/fades-the-base-layer-to-half: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH collections and fields `change-paint-layer-opacity` is
