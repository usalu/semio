use crate::editor::lowpoly::unit_tests::context::app;
...
async fn gumball_drag_coalesces_to_one_committed_edit() {
    ...
    a.dispatch_typed(LowpolyCommand::TransformBegin(super::transform_begin::TransformBegin {}), &artifact_app_laws::meta("a")).await.unwrap();
    let tick_a = a.dispatch_typed(LowpolyCommand::TranslateSelection(super::translate_selection::TranslateSelection { mode: Some("mesh".into()), ids: Some(vec![]), dx: 0.5, dy: 0.0, dz: 0.0 }), &artifact_app_laws::meta("a")).await.unwrap();
    let tick_b = a.dispatch_typed(LowpolyCommand::TranslateSelection(super::translate_selection::TranslateSelection { mode: Some("mesh".into()), ids: Some(vec![]), dx: 0.25, dy: 0.0, dz: 0.0 }), &artifact_app_laws::meta("a")).await.unwrap();
    ...
    let end = a.dispatch_typed(LowpolyCommand::TransformEnd(super::transform_end::TransformEnd {}), &artifact_app_laws::meta("a")).await.unwrap();
