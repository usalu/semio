...
impl LowpolyPlayApp {
    ...
    fn mesh_edit(&self, projection: &LowpolyProjection, config: &LowpolyConfig, edit: impl FnOnce(&mut LowpolyDocument) -> Result<(), String>) -> Emit<LowpolyOperation, LowpolyConfigOperation> {
        ...
        Ok(Emit::operations(vec![LowpolyOperation::ObjectsPatch { id: object_id, patch }])
    }
    ...
    fn commit_stroke(&self) -> Emit<LowpolyOperation, LowpolyConfigOperation> {
        ...
        *self.stroke_dirty.borrow_mut() += 1;
        ...
        Ok(Emit::commit(vec![LowpolyOperation::PaintStroke { object_id: session.object_id, layer_index: session.layer_index, runs }], "Paint stroke")
    }
    ...
    fn paint_tick(&self, projection: &LowpolyProjection, config: &LowpolyConfig, object_id: String, u: f32, v: f32) -> Emit<LowpolyOperation, LowpolyConfigOperation> {
        ...
        if utility == "eyedropper" {
            ...
            return Ok(Emit::config(vec![LowpolyConfigOperation::SetPaintColor { r: color[0], g: color[1], b: color[2], a: color[3] }]);
        }
    ...
    fn fill_at(&self, projection: &LowpolyProjection, config: &LowpolyConfig, object_id: String, u: f32, v: f32) -> Emit<LowpolyOperation, LowpolyConfigOperation> {
        ...
        *self.stroke_dirty.borrow_mut() += 1;
        Ok(Emit::commit(vec![LowpolyOperation::PaintStroke { object_id, layer_index, runs }], "Fill")
    }
    ...
    fn transform_selection(&self, projection: &LowpolyProjection, config: &LowpolyConfig, mode: &str, ids: Vec<u32>, transform: Transform, description: &str) -> Emit<LowpolyOperation, LowpolyConfigOperation> {
        ...
        if emitted.document_operations.is_empty() {
            ...
            Ok(Emit::commit(emitted.document_operations, description)
        }
    }
    ...
    fn commit_transform(&self) -> Emit<LowpolyOperation, LowpolyConfigOperation> {
        ...
        Ok(Emit::commit(vec![LowpolyOperation::ObjectsPatch { id: session.object_id, patch }], "Transform selection")
    }
        ...
        match command {
            ...
            LowpolyCommand::SetActiveObject { object_id } => {
                if projection.objects.iter().any(|object| &object.id == object_id) {
                    Ok(Emit::config(vec![LowpolyConfigOperation::SetActiveObject { object_id: object_id.clone() }])
                } else {
                    Ok(Emit::default()
                }
            }
            LowpolyCommand::SetSelection { mode, ids } => {
                let normalized = LowpolyDocument::normalize_selection_mode(mode);
                let keys = selection_keys_for(projection, config, &normalized, ids);
                Ok(Emit::config(vec![LowpolyConfigOperation::SetSelection { mode: normalized, ids: ids.clone() }, LowpolyConfigOperation::SetSelectionKeys { keys }])
            }
            ...
            LowpolyCommand::SetHover { object_id, mode, id } => {
                Ok(Emit::config(vec![LowpolyConfigOperation::SetHoveredObject { object_id: object_id.clone() }, LowpolyConfigOperation::SetHoveredTarget { object_id: object_id.clone(), mode: mode.clone(), id: *id }])
            }
            LowpolyCommand::WorldPick { granularity, merge, id } => match id {
                None => {
                    if merge == "replace" {
                        ...
                        Ok(Emit::config(vec![LowpolyConfigOperation::SetSelection { mode: config.selection_mode.clone(), ids: Vec::new() }, LowpolyConfigOperation::SetSelectionKeys { keys }])
                    } else {
                        Ok(Emit::default()
                    }
                }
            ...
            LowpolyCommand::PaintSample { object_id, u, v, x, y } => {
                let Some((uu, vv)) = paint_uv_from_command(*u, *v, *x, *y) else { return Ok(Emit::default() };
                let object_id = object_id.clone().unwrap_or_else(|| resolve_active_object_id(projection, config));
                let Some(object) = projection.objects.iter().find(|object| object.id == object_id) else { return Ok(Emit::default() };
                let composite = composite_layer_pixels(&object.paint_layers);
                let color = sample_pixel_from(&composite, uu, vv);
                Ok(Emit::config(vec![LowpolyConfigOperation::SetPaintColor { r: color[0], g: color[1], b: color[2], a: color[3] }])
            }
            ...
            LowpolyCommand::AddPaintLayer { object_id, name } => {
                ...
                Ok(Emit::operations(vec![LowpolyOperation::AddPaintLayer { object_id, index, layer: LowpolyPaintLayer::new(name) }])
            }
            ...
            LowpolyCommand::AddPrimitive { kind } => {
                ...
                let Some(mut build) = build_doc(projection, config) else { return Ok(Emit::default() };
            ...
            LowpolyCommand::PatchObject { object_id, field, value_json } => {
                ...
                Ok(Emit::operations(vec![LowpolyOperation::ObjectsPatch { id: object_id.clone(), patch }])
            }
