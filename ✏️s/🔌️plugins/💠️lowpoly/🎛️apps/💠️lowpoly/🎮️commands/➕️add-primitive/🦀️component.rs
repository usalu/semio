//! ➕️ Lowpoly play app command — appends a new primitive mesh object and makes it active.
...
pub mod add_primitive {
    ...
    pub fn handle(payload: &AddPrimitive, doc: &DocumentView<'_, LowpolyProjection>, _cfg: &ConfigView<'_, LowpolyConfig>, _ctx: &mut LowpolyScratch) -> Result<Emit<LowpolyOperation, LowpolyConfigOperation>, Fault> {
        let projection = doc.projection;
        let kind = primitive_kind(payload.kind.as_deref().unwrap_or("box")).to_string();
        let Some(mut build) = build_doc(projection, _cfg.projection) else { return Ok(Emit::default()) };
...
mod tests {
    ...
    fn add_primitive_emits_objects_add_operation() {
        let mut a = app();
        dispatch(&mut a, LowpolyCommand::AddPrimitive(super::add_primitive::AddPrimitive { kind: Some("box".into()) }));
    ...
    fn add_primitive_supports_every_known_kind() {
        ...
        for kind in ["plane", "cylinder", "cone", "ico_sphere"] {
            dispatch(&mut a, LowpolyCommand::AddPrimitive(super::add_primitive::AddPrimitive { kind: Some(kind.into()) }));
        }
