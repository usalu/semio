...
pub mod world_select {
    ...
    pub fn handle(payload: &WorldSelect, _doc: &DocumentView<'_, LowpolyProjection>, cfg: &ConfigView<'_, LowpolyConfig>, _ctx: &mut LowpolyScratch) -> Result<Emit<LowpolyOperation, LowpolyConfigOperation>, Fault> {
...
pub mod world_hover {
    ...
    pub fn handle(payload: &WorldHover, _doc: &DocumentView<'_, LowpolyProjection>, _cfg: &ConfigView<'_, LowpolyConfig>, _ctx: &mut LowpolyScratch) -> Result<Emit<LowpolyOperation, LowpolyConfigOperation>, Fault> {
        let target = payload.object_id.as_ref().map(|id| (id.clone(), "mesh".to_string(), 0u32));
        Ok(Emit::config(vec![
            LowpolyConfigOperation::SetHoveredObject { object_id: payload.object_id.clone() },
...
pub mod set_hover {
    ...
    pub fn handle(payload: &SetHover, _doc: &DocumentView<'_, LowpolyProjection>, _cfg: &ConfigView<'_, LowpolyConfig>, _ctx: &mut LowpolyScratch) -> Result<Emit<LowpolyOperation, LowpolyConfigOperation>, Fault> {
        Ok(Emit::config(vec![
            LowpolyConfigOperation::SetHoveredObject { object_id: payload.object_id.clone() },
            LowpolyConfigOperation::SetHoveredTarget { object_id: payload.object_id.clone(), mode: payload.mode.clone(), id: payload.id },
        ]))
    }
}
//#endregion 🔖️SetHover
...
pub mod world_pick {
    ...
    pub fn handle(payload: &WorldPick, doc: &DocumentView<'_, LowpolyProjection>, cfg: &ConfigView<'_, LowpolyConfig>, _ctx: &mut LowpolyScratch) -> Result<Emit<LowpolyOperation, LowpolyConfigOperation>, Fault> {
