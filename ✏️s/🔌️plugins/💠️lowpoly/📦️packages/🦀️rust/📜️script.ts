#!/usr/bin/env bun
/** 💠️ `@semio-tech/lowpoly-plugin` router: `bun ./📜️script.ts test`. */
...
class TestScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    ...
    await runCargoTestBudgeted(["semio-s-plugin-lowpoly"], this.repoRoot, rest);
  }
}
...
class DescribeScript extends BundleScript {
  run(): void {
    process.exit(describePluginComponent(this.repoRoot, "semio-s-plugin-lowpoly", join(this.root, "..", "..")));
  }
}
