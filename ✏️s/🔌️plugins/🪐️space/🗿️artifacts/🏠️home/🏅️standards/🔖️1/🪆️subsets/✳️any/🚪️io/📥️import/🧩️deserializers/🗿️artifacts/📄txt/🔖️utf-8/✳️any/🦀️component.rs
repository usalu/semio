...
pub async fn deserialize(_from: &semio_s_plugin_stdio::artifacts::txt::TxtSnapshot) -> Result<SHomeSnapshot, String> {
    Err("txt import not yet implemented".into())
}
pub async fn deserialize_bytes(_bytes: &[u8]) -> Result<SHomeSnapshot, String> {
    Err("txt import not yet implemented".into())
}
