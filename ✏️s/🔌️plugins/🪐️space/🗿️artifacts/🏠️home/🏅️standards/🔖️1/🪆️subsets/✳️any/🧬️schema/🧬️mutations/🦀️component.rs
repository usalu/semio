...
/// (`../../🧪️oracle/🔣️component.json`) declares and the `mutate-s-home-1` exhaustive test case measures
...
/// `protocol`/`store` extern-crate aliases are private — so neither `SHomeMutation` nor `SHomeSnapshot`
...
pub fn s_home_mutation_report_json(base_json: &str, mutation_json: &str) -> Result<String, String> {
    let decode_snapshot = |text: &str| -> Result<SHomeSnapshot, String> { Ok(serde_json::from_str(text).map_err(|error| error.to_string())?) };
