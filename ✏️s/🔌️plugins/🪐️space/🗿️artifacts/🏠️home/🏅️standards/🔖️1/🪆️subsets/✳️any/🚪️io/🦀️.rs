...
pub mod derived_composition {
    ...
    impl ArtifactComposition for SHomeComposerComposition {
        ...
        fn compose(sources: &[ComposeSource<'_>]) -> Result<Composition<Self::Snapshot>, ComposeError> {
            ...
            Err(ComposeError { message: "SHomeComposerComposition: no source in a known read dialect".into(), diagnostics: Vec::new() })
        }
    }
}
...
/// rule 5: `io_registry`/`ComposerEntry`/serializers/deserializers live in `🚪️io/`). `declaration()` at
/// the artifact root already calls `…::io::io_registry::entries()` — see this artifact's own
/// `🦀️.rs`, both the `.composers(…)` call and the root's own shadowing `io_registry` shim
/// (which returns `&'static [&'static ComposerEntry]`, a different type than this module's
...
pub mod io_registry {
    ...
    const HOME_DIALECT: Dialect = Dialect { artifact_kind: "s.space.home", standard: StandardId("1"), subset: SubsetId("*") };
    const HOME_JSON_BRIDGE_DIALECT: Dialect = Dialect { artifact_kind: "s.stdio.json", standard: StandardId("rfc8259"), subset: SubsetId("*") };

    fn rebuild_native_snapshot(sources: &[ErasedComposeSource]) -> Result<crate::SHomeSnapshot, ComposeError> {
