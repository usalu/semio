...
mod tests {
    ...
    /// 🧮️ `HomeConfig` round-trips dsl<->pack independently of the `SHomeDocument` document grammar above.
    #[test]
    fn home_config_dsl_pack_equivalence() {
        store::test_support::assert_dsl_pack_equivalence(&HomeConfig::default());
    }
}
//#endregion 🧪️Tests
