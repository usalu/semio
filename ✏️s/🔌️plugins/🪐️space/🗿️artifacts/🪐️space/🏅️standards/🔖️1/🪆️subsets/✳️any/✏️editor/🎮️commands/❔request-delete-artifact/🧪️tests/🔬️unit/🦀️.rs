...
async fn request_delete_opens_the_confirm_dialog_without_mutating() {
    ...
    let result = app.dispatch_typed(SpaceIndexCommand::RequestDeleteArtifact(RequestDeleteArtifact { id: id.clone() }), &semio_framework_plugin::artifact_app_laws::meta("local")).await.expect("request delete");
