...
impl protocol::MutationDiff<SpacePresence> for SpacePresence {
    async fn apply(&self, _base: &SpacePresence) -> protocol::MutationApplyResult<SpacePresence> {
        Ok({ self.clone() })
    }
...
impl Mutation<SpacePresence> for SpacePresenceMutation {
    ...
    async fn diff(&self, _base: &SpacePresence) -> protocol::MutationOutcome<SpacePresence> {
        match self {
            Self::Snapshot { presence } => protocol::MutationOutcome::new(presence.clone()),
        }
    }

    async fn inverse(&self, base: &SpacePresence) -> Vec<Self> {
