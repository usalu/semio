...
use semio_framework_os::{apply_flow_host_snapshot_to_os_workflow, WorkflowMutation, WorkflowSnapshot};
use semio_framework_plugin::{app::InteractionView, ArtifactView, ConfigView, Emit, Fault};

/// 🚧️ TEMP(Wave 3): `operations_json` stays an opaque JSON-array string, mirroring
/// `apply_flow_host_snapshot_to_os_workflow`'s still-JSON `fixture_json` bridge — typed once the flow
        ...
        match edit.get("operation").and_then(pack::JsonValue::as_str).unwrap_or("") {
            "setHostSnapshot" => {
                if let Some(host_snapshot_json) = edit.get("hostSnapshotJson").and_then(pack::JsonValue::as_str) {
                    ...
                    artifact_mutations.extend(apply_flow_host_snapshot_to_os_workflow(&projection.graph, host_snapshot_json));
                }
            }
            "move" => {
...
/// as empty; every other sub-operation (`setHostSnapshot`/`move`/`connect`) is unaffected.
