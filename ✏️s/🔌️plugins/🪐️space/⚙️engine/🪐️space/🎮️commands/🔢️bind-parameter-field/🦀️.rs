...
use semio_framework_os::workflow::{BindParameterField as BindParameterFieldMutation, UnbindParameterField as UnbindParameterFieldMutation};
...
pub fn handle(payload: &BindParameterField, _doc: &ArtifactView<'_, WorkflowSnapshot>, _cfg: &ConfigView<'_, SpaceConfig>) -> Result<Emit<WorkflowMutation, SpaceConfigMutation>, Fault> {
    if payload.parameter_id.is_empty() || payload.parameter_id == "__direct__" {
        ...
        Ok(Emit::mutations(vec![WorkflowMutation::BindParameterField(BindParameterFieldMutation { binding: WorkflowParameterBinding { parameter_id: payload.parameter_id.clone(), node_id: payload.node_id.clone(), field_path: payload.field_path.clone() } })]))
    }
}
