...
mod tests {
    ...
    async fn patch_app_instances_updates_labels() {
        ...
        let emit = studio_emit(&projection, &config, &SpaceCommand::PatchAppInstances(crate::engine::space::commands::patch_app_instances::PatchAppInstances { node_ids: ids.clone(), field: "label".into(), value: "Batch Label".into() }))
            .await
            .expect("handle");
        let next = apply_mutations(&projection, &emit.artifact_mutations).await;
