...
mod tests {
    ...
    fn space_command_op_text_round_trips_every_variant() {
        ...
        store::os_store::test_support::assert_op_line_round_trip(&SpaceCommand::AddParameter(crate::apps::space::commands::add_parameter::AddParameter { name: "Parameter".into(), kind: "numeric".into() }));
        store::os_store::test_support::assert_op_line_round_trip(&SpaceCommand::RemoveParameter(crate::apps::space::commands::remove_parameter::RemoveParameter { parameter_id: "p1".into() }));
        store::os_store::test_support::assert_op_line_round_trip(&SpaceCommand::BindParameterField(crate::apps::space::commands::bind_parameter_field::BindParameterField {
            node_id: "n1".into(),
            field_path: "label".into(),
            parameter_id: "p1".into(),
        }));
        store::os_store::test_support::assert_op_line_round_trip(&SpaceCommand::UnbindParameterField(crate::apps::space::commands::unbind_parameter_field::UnbindParameterField { node_id: "n1".into(), field_path: "label".into() }));
    }
    ...
    fn unbind_parameter_field_removes_binding() {
        ...
        let cfg = ConfigView { snapshot: &config };
        let emit =
            crate::apps::space::commands::bind_parameter_field::handle(&crate::apps::space::commands::bind_parameter_field::BindParameterField { node_id: node.id.clone(), field_path: "label".into(), parameter_id }, &doc, &cfg).expect("handle");
        ...
        let doc = ArtifactView::new(&projection, &history);
        let emit = crate::apps::space::commands::unbind_parameter_field::handle(&crate::apps::space::commands::unbind_parameter_field::UnbindParameterField { node_id: node.id.clone(), field_path: "label".into() }, &doc, &cfg).expect("handle");
        projection = apply_mutations(&projection, &emit.artifact_mutations);
        assert!(!projection.parameter_bindings.iter().any(|row| row.node_id == node.id && row.field_path == "label"));
    }
}
