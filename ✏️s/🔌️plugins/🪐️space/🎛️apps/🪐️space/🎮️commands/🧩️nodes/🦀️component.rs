...
pub mod spawn_app {
    ...
    pub fn handle(payload: &SpawnApp, _doc: &DocumentView<'_, WorkflowDocument>, _cfg: &ConfigView<'_, SpaceConfig>) -> Result<Emit<WorkflowOperation, SpaceConfigOperation>, Fault> {
...
pub mod move_media_node {
    ...
    pub fn handle(payload: &MoveMediaNode, _doc: &DocumentView<'_, WorkflowDocument>, _cfg: &ConfigView<'_, SpaceConfig>) -> Result<Emit<WorkflowOperation, SpaceConfigOperation>, Fault> {
        Ok(Emit::amend(vec![WorkflowOperation::MoveNode { node_id: payload.node_id.clone(), x: payload.x, y: payload.y }], format!("moveMediaNode:{}", payload.node_id)))
    }
}
...
pub mod remove_app_instance {
    ...
    pub fn handle(payload: &RemoveAppInstance, _doc: &DocumentView<'_, WorkflowDocument>, cfg: &ConfigView<'_, SpaceConfig>) -> Result<Emit<WorkflowOperation, SpaceConfigOperation>, Fault> {
        ...
        match payload.node_id.clone().or_else(|| crate::apps::space::primary_selected_node_id(config)) {
            Some(node_id) => {
                ...
                Ok(Emit { document_operations: vec![WorkflowOperation::RemoveNode { node_id }], config_operations, ..Default::default() })
            }
            None => Ok(Emit::default()),
        }
    }
...
pub mod delete_selection {
    ...
    pub fn handle(_payload: &DeleteSelection, _doc: &DocumentView<'_, WorkflowDocument>, cfg: &ConfigView<'_, SpaceConfig>) -> Result<Emit<WorkflowOperation, SpaceConfigOperation>, Fault> {
        let config = cfg.projection;
        let document_operations = config.selected_node_ids.iter().cloned().map(|node_id| WorkflowOperation::RemoveNode { node_id }).collect();
        Ok(Emit {
            document_operations,
...
pub mod copy_app_instance {
    ...
    pub fn handle(_payload: &CopyAppInstance, _doc: &DocumentView<'_, WorkflowDocument>, cfg: &ConfigView<'_, SpaceConfig>) -> Result<Emit<WorkflowOperation, SpaceConfigOperation>, Fault> {
        Ok(Emit::config(vec![SpaceConfigOperation::SetClipboard { node_ids: cfg.projection.selected_node_ids.clone() }]))
    }
}
...
fn duplicate_nodes(source_ids: Vec<String>, projection: &WorkflowDocument) -> Emit<WorkflowOperation, SpaceConfigOperation> {
...
pub mod rename_app_instance {
    ...
    pub fn handle(payload: &RenameAppInstance, doc: &DocumentView<'_, WorkflowDocument>, cfg: &ConfigView<'_, SpaceConfig>) -> Result<Emit<WorkflowOperation, SpaceConfigOperation>, Fault> {
        match crate::apps::space::primary_selected_node_id(cfg.projection) {
            Some(node_id) => {
                ...
                match next_label {
                    Some(next_label) => Ok(Emit::operations(vec![WorkflowOperation::PatchNode { node_id, label: next_label }])),
                    None => Ok(Emit::default()),
                }
            }
            None => Ok(Emit::default()),
        ...
        if payload.field == "position" {
            ...
            let document_operations = payload
                ...
                .filter_map(|node_id| {
                    ...
                    Some(WorkflowOperation::MoveNode { node_id: node_id.clone(), x, y })
                })
                .collect();
            Ok(Emit::operations(document_operations))
        } else {
            Ok(Emit::default())
        }
    }
...
pub mod patch_app_instances {
    ...
    pub fn handle(payload: &PatchAppInstances, _doc: &DocumentView<'_, WorkflowDocument>, _cfg: &ConfigView<'_, SpaceConfig>) -> Result<Emit<WorkflowOperation, SpaceConfigOperation>, Fault> {
        if payload.field == "label" {
            Ok(Emit::operations(payload.node_ids.iter().cloned().map(|node_id| WorkflowOperation::PatchNode { node_id, label: payload.value.clone() }).collect()))
        } else {
            Ok(Emit::default())
        }
    }
...
pub mod reorganize_workflow {
    ...
    pub fn handle(_payload: &ReorganizeWorkflow, doc: &DocumentView<'_, WorkflowDocument>, cfg: &ConfigView<'_, SpaceConfig>) -> Result<Emit<WorkflowOperation, SpaceConfigOperation>, Fault> {
        let config = cfg.projection;
        ...
        let document_operations = node_ids
            ...
            .map(|(index, node_id)| {
                ...
                WorkflowOperation::MoveNode { node_id: node_id.clone(), x: 80.0 + col * 220.0, y: 80.0 + row * 160.0 }
            })
            .collect();
        Ok(Emit::operations(document_operations))
    }
