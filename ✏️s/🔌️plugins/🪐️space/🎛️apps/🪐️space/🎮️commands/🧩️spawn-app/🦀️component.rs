...
mod tests {
    ...
    fn space_command_op_text_round_trips_every_variant() {
        ...
        store::os_store::test_support::assert_op_line_round_trip(&SpaceCommand::MoveMediaNode(crate::apps::space::commands::move_media_node::MoveMediaNode { node_id: "n1".into(), x: 1.0, y: 2.0 }));
        store::os_store::test_support::assert_op_line_round_trip(&SpaceCommand::RemoveAppInstance(crate::apps::space::commands::remove_app_instance::RemoveAppInstance { node_id: Some("n1".into()) }));
        store::os_store::test_support::assert_op_line_round_trip(&SpaceCommand::RemoveAppInstance(crate::apps::space::commands::remove_app_instance::RemoveAppInstance { node_id: None }));
        store::os_store::test_support::assert_op_line_round_trip(&SpaceCommand::DeleteSelection(crate::apps::space::commands::delete_selection::DeleteSelection {}));
        store::os_store::test_support::assert_op_line_round_trip(&SpaceCommand::CopyAppInstance(crate::apps::space::commands::copy_app_instance::CopyAppInstance {}));
        store::os_store::test_support::assert_op_line_round_trip(&SpaceCommand::DuplicateAppInstance(crate::apps::space::commands::duplicate_app_instance::DuplicateAppInstance {}));
        store::os_store::test_support::assert_op_line_round_trip(&SpaceCommand::PasteAppInstance(crate::apps::space::commands::paste_app_instance::PasteAppInstance {}));
        store::os_store::test_support::assert_op_line_round_trip(&SpaceCommand::RenameAppInstance(crate::apps::space::commands::rename_app_instance::RenameAppInstance { label: Some("Renamed".into()) }));
        store::os_store::test_support::assert_op_line_round_trip(&SpaceCommand::PatchMediaNodes(crate::apps::space::commands::patch_media_nodes::PatchMediaNodes {
            node_ids: vec!["n1".into()],
            field: "position".into(),
            axis: Some("x".into()),
            value: "120".into(),
        }));
        store::os_store::test_support::assert_op_line_round_trip(&SpaceCommand::PatchAppInstances(crate::apps::space::commands::patch_app_instances::PatchAppInstances {
            node_ids: vec!["n1".into()],
            field: "label".into(),
            value: "Batch Label".into(),
        }));
        store::os_store::test_support::assert_op_line_round_trip(&SpaceCommand::ReorganizeWorkflow(crate::apps::space::commands::reorganize_workflow::ReorganizeWorkflow {}));
    }
    ...
    fn move_media_node_emits_coalesced_move_operation() {
        ...
        let emit = studio_emit(&projection, &config, &SpaceCommand::MoveMediaNode(crate::apps::space::commands::move_media_node::MoveMediaNode { node_id: node_id.clone(), x: 120.0, y: 160.0 })).expect("handle");
    ...
    fn spawns_draw_app_instance() {
        ...
        let projection = demo_space_projection();
        let config = SpaceConfig::default();
        let emit = studio_emit(&projection, &config, &SpaceCommand::SpawnApp(SpawnApp { plugin_id: "draw".into(), app_id: "draw".into(), x: 80.0, y: 80.0 })).expect("handle");
    ...
    fn spawns_draw_app_instance_at_drop_position() {
        ...
        let emit = studio_emit(&projection, &config, &SpaceCommand::SpawnApp(SpawnApp { plugin_id: "draw".into(), app_id: "draw".into(), x: 321.0, y: 654.0 })).expect("handle");
        let next = apply_mutations(&projection, &emit.artifact_mutations);
    ...
    fn patch_app_instances_updates_labels() {
        ...
        let emit =
            studio_emit(&projection, &config, &SpaceCommand::PatchAppInstances(crate::apps::space::commands::patch_app_instances::PatchAppInstances { node_ids: ids.clone(), field: "label".into(), value: "Batch Label".into() })).expect("handle");
        let next = apply_mutations(&projection, &emit.artifact_mutations);
    ...
    fn spawns_puzzle5d_and_shooting_with_multi_port_registrations() {
        ...
        let config = SpaceConfig::default();
        let emit = studio_emit(&projection, &config, &SpaceCommand::SpawnApp(SpawnApp { plugin_id: "puzzle.5d".into(), app_id: "puzzle5d".into(), x: 200.0, y: 100.0 })).expect("handle");
        projection = apply_mutations(&projection, &emit.artifact_mutations);
        let emit = studio_emit(&projection, &config, &SpaceCommand::SpawnApp(SpawnApp { plugin_id: "shooting".into(), app_id: "shooting".into(), x: 300.0, y: 100.0 })).expect("handle");
        projection = apply_mutations(&projection, &emit.artifact_mutations);
    ...
    fn undo_redo_round_trip_on_spawn() {
        ...
        testkit::assert_undo_redo_round_trip(&mut app, SpaceCommand::SpawnApp(SpawnApp { plugin_id: "draw".into(), app_id: "draw".into(), x: 80.0, y: 80.0 }), |app| app.snapshot().expect("projection").graph.nodes.len(), before, before + 1);
    }
}
//#endregion 🧪️Tests
