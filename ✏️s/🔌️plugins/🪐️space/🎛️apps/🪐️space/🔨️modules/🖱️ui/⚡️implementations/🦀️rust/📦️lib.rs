...
//! `projection.graph.nodes` directly.
//!
//! 🕳️ Deviation from the usual "ui" content: this app's `DocumentApp::Projection`/`Operation` are
//! `semio_framework_os::{WorkflowDocument, WorkflowOperation}` — see `space_op`'s doc comment. This crate also
...
use semio_framework_plugin::{
    ...
    ActionDefinition, ActionDescriptor, ActionKind, App, ConfigView, DocumentApp, DocumentView, Emit, Fault, FaultCode, FaultOrigin, HostEffect, IconName, Label, Locale, LocalizedLabel, MeasureSelectItem, NodeGraphEdgeRecord, NodeGraphFindItem, NodeGraphHover,
...
// `os_app_registration`'s `registration.parameter_fields`, `os_parameter_value`,
// `materialize_os_app_instance_document_json`) were deliberately left untouched by the os-core dissolve
// (the plan's 10-step checklist only dissolved `OsProjection`/`OsOperation`, not `instance::Parameters`
// — see the ticket's own w3-progress notes) while `WorkflowDocument.parameters`/`.parameter_bindings`
// now carry the kernel `workflow` crate's own, structurally-identical `WorkflowParameter`/
// `WorkflowParameterBinding`/`WorkflowParameterType`. These two parameter vocabularies are a known,
// intentionally-deferred duplication (flagged for a follow-up ticket) — this bridge converts between
// them at the few call sites that still need the os-core registry-facing shape.
fn workflow_parameter_type_to_os(kind: &WorkflowParameterType) -> semio_framework_os::OsParameterType {
    match kind {
...
fn workflow_parameter_to_os(parameter: &WorkflowParameter) -> semio_framework_os::OsParameter {
    match parameter {
...
fn workflow_parameters_to_os(parameters: &[WorkflowParameter]) -> Vec<semio_framework_os::OsParameter> {
    parameters.iter().map(workflow_parameter_to_os).collect()
}

fn workflow_parameter_binding_to_os(binding: &WorkflowParameterBinding) -> semio_framework_os::OsParameterFieldBinding {
...
fn workflow_parameter_bindings_to_os(bindings: &[WorkflowParameterBinding]) -> Vec<semio_framework_os::OsParameterFieldBinding> {
...
/// `primary_selected_instance_id(&StudioRuntimeState, &WorkflowDocument)`; no join needed anymore (node IS
/// the instance).
...
fn build_parameters_tree(projection: &WorkflowDocument, labels: &SStudioLabels) -> UiNode {
...
fn build_inspector_tree(projection: &WorkflowDocument, config: &SpaceConfig, term_labels: &SStudioLabels) -> UiNode {
                        ...
                        let compatible: Vec<_> = projection
                            ...
                            .filter(|parameter| {
                                os_parameter_types_compatible(
                                    &workflow_parameter_type_to_os(match parameter {
...
fn render_workflow(projection: &WorkflowDocument, config: &SpaceConfig, labels: &SStudioLabels) -> UiNode {
    let _ = labels;
...
fn render_compiled_dag(projection: &WorkflowDocument) -> UiNode {
...
fn render_media_vfs(projection: &WorkflowDocument, labels: &SStudioLabels) -> UiNode {
    ...
    // 🚧️ `flatten_media_vfs_rows` is a no-op stub (os-core dissolve step 6 deleted the
    // `🔖️WorkflowVfs` region it depended on) — the media VFS window shows only its root node until a
    // full collection-browser UI replaces it in a later wave.
    flatten_media_vfs_rows(OS_WORKFLOW_VFS_ROOT_ID, &projection.graph, &projection.parameter_bindings, &projection.parameters, &mut rows);
...
fn negotiate_connect_or_notify(projection: &WorkflowDocument, source_node_id: &str, source_port_id: &str, target_node_id: &str, target_port_id: &str) -> Result<MediaContract, HostEffect> {
...
fn connect_edge_operation(source_node_id: &str, source_port_id: &str, target_node_id: &str, target_port_id: &str, contract: MediaContract) -> WorkflowOperation {
...
impl DocumentApp for SpaceApp {
    ...
    fn handle(&self, command: &SpaceCommand, doc: &DocumentView<'_, WorkflowDocument>, cfg: &ConfigView<'_, SpaceConfig>) -> Result<Emit<WorkflowOperation, SpaceConfigOperation>, Fault> {
        ...
        match command {
            ...
            SpaceCommand::AddParameter { name, kind } => {
                ...
                Ok(Emit::operations(vec![add_parameter_operation(&parameter_type, name)]))
            }
            SpaceCommand::RemoveParameter { parameter_id } => Ok(Emit::operations(vec![WorkflowOperation::RemoveParameter { parameter_id: parameter_id.clone() }])),
            ...
            SpaceCommand::MoveMediaNode { node_id, x, y } => Ok(Emit::amend(vec![WorkflowOperation::MoveNode { node_id: node_id.clone(), x: *x, y: *y }], format!("moveMediaNode:{node_id}"))),
            ...
            SpaceCommand::DisconnectMediaEdge { edge_id } => Ok(Emit::operations(vec![WorkflowOperation::DisconnectEdge { edge_id: edge_id.clone() }])),
            ...
            SpaceCommand::RenameAppInstance { label } => match primary_selected_node_id(config) {
                Some(node_id) => {
                    let next_label = label.clone().or_else(|| projection.graph.nodes.iter().find(|row| row.id == node_id).map(|node| format!("{} (renamed)", node.label)));
                    match next_label {
            ...
            SpaceCommand::PatchAppInstances { node_ids, field, value } => {
                if field == "label" {
                    Ok(Emit::operations(node_ids.iter().cloned().map(|node_id| WorkflowOperation::PatchNode { node_id, label: value.clone() }).collect()))
                } else {
                    Ok(Emit::default())
                }
            }
            SpaceCommand::BindParameterField { node_id, field_path, parameter_id } => {
                if parameter_id.is_empty() || parameter_id == "__direct__" {
                    Ok(Emit::operations(vec![WorkflowOperation::UnbindParameterField { node_id: node_id.clone(), field_path: field_path.clone() }]))
                } else {
                    Ok(Emit::operations(vec![WorkflowOperation::BindParameterField { binding: WorkflowParameterBinding { parameter_id: parameter_id.clone(), node_id: node_id.clone(), field_path: field_path.clone() } }]))
                }
            }
            SpaceCommand::UnbindParameterField { node_id, field_path } => Ok(Emit::operations(vec![WorkflowOperation::UnbindParameterField { node_id: node_id.clone(), field_path: field_path.clone() }])),
            SpaceCommand::ReorganizeWorkflow => {
                let node_ids: Vec<String> = if config.selected_node_ids.is_empty() { projection.graph.nodes.iter().map(|node| node.id.clone()).collect() } else { config.selected_node_ids.clone() };
                let document_operations = node_ids
                    .iter()
                    .enumerate()
                    .map(|(index, node_id)| {
            ...
            SpaceCommand::SelectInstance { node_id } => {
                ...
                Ok(Emit::config(vec![SpaceConfigOperation::SetActiveNode { node_id: node_id.clone() }, SpaceConfigOperation::SetSelection { node_ids }]))
            }
            SpaceCommand::NodeGraphSelect { node_ids, select_all } | SpaceCommand::SetMediaNodeSelection { node_ids, select_all } => {
                let node_ids = if *select_all { projection.graph.nodes.iter().map(|node| node.id.clone()).collect() } else { node_ids.clone() };
            ...
            SpaceCommand::NodeGraphHover { hover_json } | SpaceCommand::TextHover { hover_json } => {
                ...
                Ok(Emit::config(vec![SpaceConfigOperation::SetHover { node_id }]))
            }
            ...
            SpaceCommand::SetActiveExample { example_id } => {
                if example_id.is_empty() {
                    ...
                    Ok(Emit::effect(HostEffect::Navigate { uri: format!("/spaces/{example_id}") }))
                }
            }
            SpaceCommand::ExportMedia { node_id, format } => match projection.graph.nodes.iter().find(|row| &row.id == node_id) {
                Some(node) => {
                ...
                if let (Some(node_id), Some(format_name)) = (config.pending_import_node_id.clone(), config.pending_import_format.clone()) {
                    ...
                    if let Some(format) = semio_framework_os::OsMediaFormat::parse(&format_name) {
                        ...
                        if let Ok(bytes) = base64::engine::general_purpose::STANDARD.decode(base64_part) {
                            if let Some(node) = projection.graph.nodes.iter().find(|row| row.id == node_id) {
            ...
            SpaceCommand::OpenSpace { space_id } => {
                // 🚧️ `parse_demo_space_document()` now yields a `workflow::WorkflowDocument` (see `##
                // The inversion`), not a `space::SpaceProjection`-backed catalog entry — the "demo" id
                // fallback below synthesizes a minimal ephemeral space manifest with the same id/name
                // instead, so `"demo"` still resolves to *something* openable.
                let document = home_ui::resolve_studio_document(space_id).or_else(|| {
                    if space_id == "demo" {
                        let name = { let demo = parse_demo_space_document(); if demo.name.trim().is_empty() { "Demo Studio".into() } else { demo.name } };
            ...
            SpaceCommand::OpenInstance { node_id } => match node_id.clone().or_else(|| primary_selected_node_id(config)) {
                Some(node_id) => match projection.graph.nodes.iter().find(|row| row.id == node_id) {
                    Some(node) => Ok(Emit {
    ...
    fn window_measures(&self, doc: &DocumentView<'_, WorkflowDocument>, cfg: &ConfigView<'_, SpaceConfig>) -> HashMap<String, Vec<WindowMeasure>> {
        let labels = semio_framework_plugin::resolve_labels_for_locale::<SStudioLabels>(&cfg.projection.locale);
        HashMap::from([(S_PLAY_WINDOW_WORKFLOW.into(), workflow_measures(cfg.projection, &doc.projection.graph.nodes, labels))])
    }

    fn context_menu(
        &self,
        request: &semio_framework_plugin::ContextMenuRequest,
        _doc: &DocumentView<'_, WorkflowDocument>,
...
pub fn create_space_app() -> App {
    ...
    let measures = workflow_measures(&config, &projection.graph.nodes, semio_framework_plugin::resolve_labels_for_locale::<SStudioLabels>(&config.locale));
...
mod tests {
    use super::*;
    use semio_framework_os::{
        apply_workflow_operation, register_app_io, register_artifact_descriptor, validate_workflow, AppDefinition, ArtifactKindSpec, ArtifactPresentation, MediaClass, MediaForm, MediaPortDirection, MediaPortSpec, MediaType, MediaWireFormat, OsMediaFormat,
        PortMultiplicity,
    };
    ...
    fn studio_emit(projection: &WorkflowDocument, config: &SpaceConfig, command: SpaceCommand) -> Result<Emit<WorkflowOperation, SpaceConfigOperation>, Fault> {
    ...
    fn apply_operations(projection: &WorkflowDocument, operations: &[WorkflowOperation]) -> WorkflowDocument {
    ...
    fn demo_document_has_instances_and_edges() {
        ...
        assert!(validate_workflow(&projection.graph).ok);
    }
    ...
    fn connect_media_ports_rejects_incompatible_types_via_notice() {
        ...
        projection.graph.nodes.push(test_node("contract-src", vec![], vec![src_out]));
        projection.graph.nodes.push(test_node("contract-dst", vec![dst_in], vec![]));
        let config = SpaceConfig::default();
    ...
    fn connect_media_ports_negotiates_a_contract_for_compatible_types() {
        ...
        projection.graph.nodes.push(test_node("contract-src-2", vec![], vec![src_out]));
        projection.graph.nodes.push(test_node("contract-dst-2", vec![dst_in], vec![]));
        let config = SpaceConfig::default();
        let emit = studio_emit(
            &projection,
            &config,
        ...
        assert!(validate_workflow(&next.graph).ok, "a freshly negotiated edge must pass validate_workflow's contract-consistency check");
    }
    ...
    fn undo_redo_round_trip_on_spawn() {
        ...
        let mut app = VcsDocumentApp::new(SpaceApp);
        let before = app.projection().expect("projection").graph.nodes.len();
        testkit::assert_undo_redo_round_trip(&mut app, SpaceCommand::SpawnApp { plugin_id: "draw".into(), app_id: "draw".into(), x: 80.0, y: 80.0 }, |app| app.projection().expect("projection").graph.nodes.len(), before, before + 1);
    }
