//! 🧪️ `delete-function` fixture — `🗑️deletes-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️.rs`, which drops `function-a` from `program.functions` as `removed = ["function-a"]`, after a target-missing guard.
//!
...
const BEFORE: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/⚙️function/🗑️delete/🗑️deletes-a/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/⚙️function/🗑️delete/🗑️deletes-a/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/⚙️function/🗑️delete/🗑️deletes-a/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/⚙️function/🗑️delete/🗑️deletes-a/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/⚙️function/🗑️delete/🗑️deletes-a/🎯️outcome/🔣️.json");

fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("delete-function/deletes-function-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("delete-function/deletes-function-a: after snapshot decodes")
}
...
async fn delete_function_applies_to_committed_after() {
    ...
    let applied = outcome.diff().apply(&base).expect("delete-function/deletes-function-a: delete-function applies to its committed before-snapshot");
...
async fn delete_function_inverse_restores_before() {
    ...
    undo.reverse();
    let mut state = forward.diff(&base).diff().apply(&base).expect("delete-function/deletes-function-a: forward diff applies");
    for step in &undo {
        state = step.diff(&state).diff().apply(&state).expect("delete-function/deletes-function-a: inverse step applies");
    }
...
async fn delete_function_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("delete-function/deletes-function-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("delete-function/deletes-function-a: snapshot re-encodes");
...
async fn delete_function_declared_outcome_holds() {
    ...
    assert!(outcome.diff().apply(&base).is_ok(), "delete-function/deletes-function-a: delete-function was rejected by apply on its own before-snapshot");
}
...
async fn delete_function_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("delete-function/deletes-function-a: produced diff encodes");
...
async fn delete_function_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("delete-function/deletes-function-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("delete-function/deletes-function-a: committed diff re-encodes");
...
async fn delete_function_committed_diff_applies_to_after() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("delete-function/deletes-function-a: committed diff decodes");
    let produced = decoded.apply(&before()).expect("delete-function/deletes-function-a: committed diff applies to the before-snapshot");
