...
//! leaf's own `🔺️diff/🦀️component.rs`, which appends the payload row to `program.organizational` as `added = [row]`, after a duplicate-id guard.
//!
...
async fn create_organizational_requirement_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "create-organizational-requirement/creates-organizational-requirement-a: the diff create-organizational-requirement builds differs from the committed 🔺️diff/🔣️component.json");
}
