...
//! leaf's own `🔺️diff/🦀️component.rs`, which appends the payload row to `program.resources` as `added = [row]`, after a duplicate-id guard.
//!
...
async fn create_resource_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "create-resource/creates-resource-a: the diff create-resource builds differs from the committed 🔺️diff/🔣️component.json");
}
