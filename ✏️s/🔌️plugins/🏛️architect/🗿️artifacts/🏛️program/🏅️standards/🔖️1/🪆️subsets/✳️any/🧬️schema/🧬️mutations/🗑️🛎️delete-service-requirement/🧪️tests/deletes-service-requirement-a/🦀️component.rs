//! 🧪️ `delete-service-requirement` fixture — `deletes-service-requirement-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️component.rs`, which drops `service-requirement-a` from `program.services` as `removed = ["service-requirement-a"]`, after a target-missing guard.
//!
...
async fn delete_service_requirement_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-service-requirement/deletes-service-requirement-a: the diff delete-service-requirement builds differs from the committed 🔺️diff/🔣️component.json");
}
