//! 🧪️ `rename-delivery-constraint` fixture — `renames-delivery-constraint-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️component.rs`, which patches only `name` on `delivery-constraint-a` in `program.delivery` — every other patch field stays `null`.
//!
...
async fn rename_delivery_constraint_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rename-delivery-constraint/renames-delivery-constraint-a: the diff rename-delivery-constraint builds differs from the committed 🔺️diff/🔣️component.json");
}
