//! 🧪️ `delete-delivery-constraint` fixture — `🗑️deletes-a`.
//!
...
fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("delete-delivery-constraint/deletes-delivery-constraint-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("delete-delivery-constraint/deletes-delivery-constraint-a: after snapshot decodes")
}
...
async fn delete_delivery_constraint_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("delete-delivery-constraint/deletes-delivery-constraint-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("delete-delivery-constraint/deletes-delivery-constraint-a: snapshot re-encodes");
...
async fn delete_delivery_constraint_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("delete-delivery-constraint/deletes-delivery-constraint-a: produced diff encodes");
...
async fn delete_delivery_constraint_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("delete-delivery-constraint/deletes-delivery-constraint-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("delete-delivery-constraint/deletes-delivery-constraint-a: committed diff re-encodes");
