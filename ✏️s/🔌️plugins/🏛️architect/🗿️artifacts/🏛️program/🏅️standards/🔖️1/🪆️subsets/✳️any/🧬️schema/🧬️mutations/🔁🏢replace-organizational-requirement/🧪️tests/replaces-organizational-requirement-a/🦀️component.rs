//! 🧪️ `replace-organizational-requirement` fixture — `replaces-organizational-requirement-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️component.rs`, which patches `organizational-requirement-a` in `program.organizational` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `department`).
//!
...
async fn replace_organizational_requirement_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-organizational-requirement/replaces-organizational-requirement-a: the diff replace-organizational-requirement builds differs from the committed 🔺️diff/🔣️component.json");
}
