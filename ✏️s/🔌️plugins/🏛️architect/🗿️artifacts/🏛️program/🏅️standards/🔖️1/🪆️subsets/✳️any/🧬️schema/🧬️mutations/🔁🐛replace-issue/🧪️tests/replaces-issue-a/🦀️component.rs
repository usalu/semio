...
//! leaf's own `🔺️diff/🦀️component.rs`, which patches `issue-a` in `program.issues` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `issue_type`).
//!
...
async fn replace_issue_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-issue/replaces-issue-a: the diff replace-issue builds differs from the committed 🔺️diff/🔣️component.json");
}
