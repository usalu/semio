//! 🧪️ `delete-cost-requirement` fixture — `🗑️deletes-a`.
//!
...
fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("delete-cost-requirement/deletes-cost-requirement-a: after snapshot decodes")
}
...
async fn delete_cost_requirement_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("delete-cost-requirement/deletes-cost-requirement-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("delete-cost-requirement/deletes-cost-requirement-a: snapshot re-encodes");
...
async fn delete_cost_requirement_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("delete-cost-requirement/deletes-cost-requirement-a: produced diff encodes");
...
async fn delete_cost_requirement_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("delete-cost-requirement/deletes-cost-requirement-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("delete-cost-requirement/deletes-cost-requirement-a: committed diff re-encodes");
