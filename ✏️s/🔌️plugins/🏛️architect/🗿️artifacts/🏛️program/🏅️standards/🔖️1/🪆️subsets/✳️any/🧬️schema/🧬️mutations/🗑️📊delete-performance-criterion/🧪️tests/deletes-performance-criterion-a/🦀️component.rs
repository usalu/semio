...
//! leaf's own `🔺️diff/🦀️component.rs`, which drops `performance-criterion-a` from `program.performance` as `removed = ["performance-criterion-a"]`, after a target-missing guard.
//!
...
async fn delete_performance_criterion_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-performance-criterion/deletes-performance-criterion-a: the diff delete-performance-criterion builds differs from the committed 🔺️diff/🔣️component.json");
}
