//! 🧪️ `rename-template-record` fixture — `renames-template-record-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️component.rs`, which patches only `name` on `template-record-a` in `program.templates` — every other patch field stays `null`.
//!
...
async fn rename_template_record_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rename-template-record/renames-template-record-a: the diff rename-template-record builds differs from the committed 🔺️diff/🔣️component.json");
}
