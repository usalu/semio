...
//! leaf's own `🔺️diff/🦀️component.rs`, which reads the live `knowledge` rows off the working-scene cache, appends the payload row, and re-mints a content-addressed `table` child handle whose `childId` is a hash of the row list's JSON.
//!
...
async fn create_knowledge_record_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "create-knowledge-record/creates-knowledge-record-a: the diff create-knowledge-record builds differs from the committed 🔺️diff/🔣️component.json");
}
