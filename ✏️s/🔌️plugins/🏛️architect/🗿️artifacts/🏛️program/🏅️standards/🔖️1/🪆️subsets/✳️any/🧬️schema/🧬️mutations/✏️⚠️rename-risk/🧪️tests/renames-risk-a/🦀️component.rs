//! 🧪️ `rename-risk` fixture — `renames-risk-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️component.rs`, which patches only `name` on `risk-a` in `program.risks` — every other patch field stays `null`.
//!
...
async fn rename_risk_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rename-risk/renames-risk-a: the diff rename-risk builds differs from the committed 🔺️diff/🔣️component.json");
}
