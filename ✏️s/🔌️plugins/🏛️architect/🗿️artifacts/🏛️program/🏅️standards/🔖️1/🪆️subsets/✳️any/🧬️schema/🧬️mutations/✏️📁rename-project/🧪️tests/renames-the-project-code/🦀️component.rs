...
//! leaf's own `🔺️diff/🦀️component.rs`, which clones `base.project`, writes the single `code` field, and emits it as the whole `project` facet — the diff carries the complete replacement value, not a patch.
//!
...
async fn rename_project_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rename-project/renames-the-project-code: the diff rename-project builds differs from the committed 🔺️diff/🔣️component.json");
}
