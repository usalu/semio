...
fn before() -> ProgramSnapshot {
    serde_json::from_str(BEFORE).expect("delete-storage-requirement/deletes-storage-requirement-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    serde_json::from_str(AFTER).expect("delete-storage-requirement/deletes-storage-requirement-a: after snapshot decodes")
}
...
async fn delete_storage_requirement_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = serde_json::from_str(text).expect("delete-storage-requirement/deletes-storage-requirement-a: snapshot decodes");
        let reencoded = serde_json::to_value(&decoded).expect("delete-storage-requirement/deletes-storage-requirement-a: snapshot re-encodes");
...
async fn delete_storage_requirement_produces_committed_diff() {
    let produced = serde_json::to_value(mutation().diff(&before()).diff()).expect("delete-storage-requirement/deletes-storage-requirement-a: produced diff encodes");
...
async fn delete_storage_requirement_committed_diff_is_canonical() {
    let decoded: ProgramDiff = serde_json::from_str(DIFF).expect("delete-storage-requirement/deletes-storage-requirement-a: committed diff decodes");
    let reencoded = serde_json::to_value(&decoded).expect("delete-storage-requirement/deletes-storage-requirement-a: committed diff re-encodes");
