...
//! leaf's own `🔺️diff/🦀️component.rs`, which drops `status-record-a` from `program.status_records` as `removed = ["status-record-a"]`, after a target-missing guard.
//!
...
async fn delete_status_record_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-status-record/deletes-status-record-a: the diff delete-status-record builds differs from the committed 🔺️diff/🔣️component.json");
}
