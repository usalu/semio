//! 🧪️ `delete-audit-event` fixture — `deletes-audit-event-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️component.rs`, which drops `audit-event-a` from `program.audit_events` as `removed = ["audit-event-a"]`, after a target-missing guard.
//!
...
async fn delete_audit_event_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-audit-event/deletes-audit-event-a: the diff delete-audit-event builds differs from the committed 🔺️diff/🔣️component.json");
}
