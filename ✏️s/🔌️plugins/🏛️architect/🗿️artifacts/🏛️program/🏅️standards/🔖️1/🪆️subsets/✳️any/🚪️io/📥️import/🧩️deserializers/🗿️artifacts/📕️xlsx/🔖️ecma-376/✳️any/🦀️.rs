...
pub async fn deserialize(from: &XlsxSnapshot) -> Result<ProgramSnapshot, store::TextError> {
    let _ = STDIO_XLSX_DOCUMENT_SCHEMA;
    dsl::FromValue::from_value(dsl::ToValue::to_value(from)).map_err(|e: dsl::ValueError| store::TextError::new(format!("program<-xlsx: {e}"), dsl::TextSpan::at(1, 1)))
}
