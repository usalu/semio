...
//! leaf's own `🔺️diff/🦀️component.rs`, which drops `growth-plan-a` from `program.growth` as `removed = ["growth-plan-a"]`, after a target-missing guard.
//!
...
async fn delete_growth_plan_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-growth-plan/deletes-growth-plan-a: the diff delete-growth-plan builds differs from the committed 🔺️diff/🔣️component.json");
}
