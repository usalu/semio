...
//! leaf's own `🔺️diff/🦀️component.rs`, which patches `relationship-a` in `program.relationships` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `directional`).
//!
...
async fn replace_relationship_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-relationship/replaces-relationship-a: the diff replace-relationship builds differs from the committed 🔺️diff/🔣️component.json");
}
