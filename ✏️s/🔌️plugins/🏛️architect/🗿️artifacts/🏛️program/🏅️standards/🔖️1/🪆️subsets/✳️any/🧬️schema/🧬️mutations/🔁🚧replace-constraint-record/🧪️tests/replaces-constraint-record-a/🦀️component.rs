...
//! leaf's own `🔺️diff/🦀️component.rs`, which patches `constraint-record-a` in `program.constraints` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `constraint_type`).
//!
...
async fn replace_constraint_record_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-constraint-record/replaces-constraint-record-a: the diff replace-constraint-record builds differs from the committed 🔺️diff/🔣️component.json");
}
