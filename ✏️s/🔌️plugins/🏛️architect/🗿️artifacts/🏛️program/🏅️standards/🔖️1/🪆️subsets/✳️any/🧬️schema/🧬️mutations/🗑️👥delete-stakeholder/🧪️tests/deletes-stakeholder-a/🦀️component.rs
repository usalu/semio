...
//! leaf's own `🔺️diff/🦀️component.rs`, which drops `stakeholder-a` from `program.stakeholders` as `removed = ["stakeholder-a"]`, after a target-missing guard.
//!
...
async fn delete_stakeholder_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-stakeholder/deletes-stakeholder-a: the diff delete-stakeholder builds differs from the committed 🔺️diff/🔣️component.json");
}
