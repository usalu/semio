//! 🔺️ Sparse diff construction for `ConnectAdjacency` — real handcrafted `ProgramDiff`
//! builder, never apply-then-capture. Endpoint normalization mirrors (does not call)
//! `engine::adjacency::set_adjacency`'s canonicalization — that function mutates a snapshot
//! in place (apply-first), which this diff-first mutation cannot delegate to.
...
pub fn diff_connect(payload: &ConnectAdjacency, base: &ProgramSnapshot) -> ProgramDiff {
