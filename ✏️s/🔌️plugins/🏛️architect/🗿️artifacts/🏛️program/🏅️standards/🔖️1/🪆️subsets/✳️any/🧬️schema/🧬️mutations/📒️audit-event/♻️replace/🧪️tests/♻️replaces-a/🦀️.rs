//! 🧪️ `replace-audit-event` fixture — `♻️replaces-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️.rs`, which patches `audit-event-a` in `program.audit_events` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `subject_kind`).
//!
...
const BEFORE: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📒️audit-event/♻️replace/♻️replaces-a/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📒️audit-event/♻️replace/♻️replaces-a/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📒️audit-event/♻️replace/♻️replaces-a/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📒️audit-event/♻️replace/♻️replaces-a/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📒️audit-event/♻️replace/♻️replaces-a/🎯️outcome/🔣️.json");

fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("replace-audit-event/replaces-audit-event-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("replace-audit-event/replaces-audit-event-a: after snapshot decodes")
}
...
async fn replace_audit_event_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("replace-audit-event/replaces-audit-event-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("replace-audit-event/replaces-audit-event-a: snapshot re-encodes");
...
async fn replace_audit_event_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("replace-audit-event/replaces-audit-event-a: produced diff encodes");
    ...
    assert_eq!(produced, committed, "replace-audit-event/replaces-audit-event-a: the diff replace-audit-event builds differs from the committed 🔺️diff/🔣️.json");
}

/// 🔣️ The committed diff is itself canonical and decodes to ProgramDiff.
#[semio_framework_async_macros::async_test]
async fn replace_audit_event_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("replace-audit-event/replaces-audit-event-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("replace-audit-event/replaces-audit-event-a: committed diff re-encodes");
