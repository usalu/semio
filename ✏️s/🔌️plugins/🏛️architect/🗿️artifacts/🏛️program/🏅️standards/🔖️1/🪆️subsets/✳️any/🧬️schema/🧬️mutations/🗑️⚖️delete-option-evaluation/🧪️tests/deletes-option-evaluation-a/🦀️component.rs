//! 🧪️ `delete-option-evaluation` fixture — `deletes-option-evaluation-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️component.rs`, which drops `option-evaluation-a` from `program.options` as `removed = ["option-evaluation-a"]`, after a target-missing guard.
//!
...
async fn delete_option_evaluation_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-option-evaluation/deletes-option-evaluation-a: the diff delete-option-evaluation builds differs from the committed 🔺️diff/🔣️component.json");
}
