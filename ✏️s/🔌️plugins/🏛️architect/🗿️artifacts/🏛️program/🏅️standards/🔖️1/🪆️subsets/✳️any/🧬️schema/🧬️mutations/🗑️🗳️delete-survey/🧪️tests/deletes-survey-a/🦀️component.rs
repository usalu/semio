...
//! leaf's own `🔺️diff/🦀️component.rs`, which drops `survey-a` from `program.surveys` as `removed = ["survey-a"]`, after a target-missing guard.
//!
...
async fn delete_survey_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-survey/deletes-survey-a: the diff delete-survey builds differs from the committed 🔺️diff/🔣️component.json");
}
