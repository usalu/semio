...
//! leaf's own `🔺️diff/🦀️.rs`, which patches `process-a` in `program.processes` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `code`).
//!
...
async fn replace_process_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-process/replaces-process-a: the diff replace-process builds differs from the committed 🔺️diff/🔣️.json");
}
