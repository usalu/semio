...
//! leaf's own `🔺️diff/🦀️component.rs`, which appends the payload row to `program.constraints` as `added = [row]`, after a duplicate-id guard.
//!
...
async fn create_constraint_record_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "create-constraint-record/creates-constraint-record-a: the diff create-constraint-record builds differs from the committed 🔺️diff/🔣️component.json");
}
