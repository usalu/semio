...
async fn the_overview_lists_every_non_empty_register_with_its_counts() {
    let json = semio_framework_plugin::artifact_app_laws::project_and_retire_fixture_tree(semio_framework_plugin::built_to_component_tree(render(&sample_plugin()).expect("render"))).expect("retire viewer tree");
...
async fn an_empty_program_renders_the_placeholder() {
    let json = semio_framework_plugin::artifact_app_laws::project_and_retire_fixture_tree(semio_framework_plugin::built_to_component_tree(render(&empty_plugin()).expect("render"))).expect("retire viewer tree");
