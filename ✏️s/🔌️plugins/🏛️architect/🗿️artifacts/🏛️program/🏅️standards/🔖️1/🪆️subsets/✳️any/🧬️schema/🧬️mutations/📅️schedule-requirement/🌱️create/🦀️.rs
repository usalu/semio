//! 🦠️ ProgramSnapshot mutation — `create-schedule-requirement` leaf (create). Split from the
...
use crate::registers::ScheduleRequirement;
...
impl MutationKind<ProgramSnapshot, ProgramMutation> for CreateScheduleRequirement {
    ...
    fn label(&self) -> protocol::LocalizedLabel {
        protocol::LocalizedLabel::native(&format!("Create schedule requirement \"{}\"", self.schedule_requirement.header.name), &format!("Zeitplananforderung \"{}\" erstellen", self.schedule_requirement.header.name))
    }
