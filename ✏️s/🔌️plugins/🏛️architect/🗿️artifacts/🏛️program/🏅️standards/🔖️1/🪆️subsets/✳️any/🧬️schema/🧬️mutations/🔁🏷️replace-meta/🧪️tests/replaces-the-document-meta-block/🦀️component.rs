//! 🧪️ `replace-meta` fixture — `replaces-the-document-meta-block`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️component.rs`, which emits the payload value as the whole `meta` facet verbatim.
//!
...
async fn replace_meta_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-meta/replaces-the-document-meta-block: the diff replace-meta builds differs from the committed 🔺️diff/🔣️component.json");
}
