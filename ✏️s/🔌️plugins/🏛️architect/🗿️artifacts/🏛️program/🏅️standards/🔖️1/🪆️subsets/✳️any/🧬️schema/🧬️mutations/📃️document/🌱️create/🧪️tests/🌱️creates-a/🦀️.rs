//! 🧪️ `create-document` fixture — `🌱️creates-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️.rs`, which appends the payload row to `program.artifacts` as `added = [row]`, after a duplicate-id guard.
//!
...
const BEFORE: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📃️document/🌱️create/🌱️creates-a/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📃️document/🌱️create/🌱️creates-a/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📃️document/🌱️create/🌱️creates-a/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📃️document/🌱️create/🌱️creates-a/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📃️document/🌱️create/🌱️creates-a/🎯️outcome/🔣️.json");

fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("create-document/creates-document-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("create-document/creates-document-a: after snapshot decodes")
}
...
async fn create_document_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("create-document/creates-document-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("create-document/creates-document-a: snapshot re-encodes");
...
async fn create_document_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("create-document/creates-document-a: produced diff encodes");
    let committed: serde_json::Value = serde_json::from_str(DIFF).expect("create-document/creates-document-a: committed diff decodes");
    assert_eq!(produced, committed, "create-document/creates-document-a: the diff create-document builds differs from the committed 🔺️diff/🔣️.json");
}

/// 🔣️ The committed diff is itself canonical and decodes to ProgramDiff.
#[semio_framework_async_macros::async_test]
async fn create_document_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("create-document/creates-document-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("create-document/creates-document-a: committed diff re-encodes");
