...
//! leaf's own `🔺️diff/🦀️component.rs`, which patches only `name` on `infrastructure-requirement-a` in `program.infrastructure` — every other patch field stays `null`.
//!
...
async fn rename_infrastructure_requirement_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rename-infrastructure-requirement/renames-infrastructure-requirement-a: the diff rename-infrastructure-requirement builds differs from the committed 🔺️diff/🔣️component.json");
}
