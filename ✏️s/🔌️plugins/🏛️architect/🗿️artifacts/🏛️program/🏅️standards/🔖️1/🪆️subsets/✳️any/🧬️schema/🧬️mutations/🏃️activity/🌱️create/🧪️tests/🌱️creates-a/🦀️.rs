...
//! leaf's own `🔺️diff/🦀️.rs`, which appends the payload row to `program.activities` as `added = [row]`, after a duplicate-id guard.
//!
...
fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("create-activity/creates-activity-a: after snapshot decodes")
}
...
async fn create_activity_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("create-activity/creates-activity-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("create-activity/creates-activity-a: snapshot re-encodes");
...
async fn create_activity_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("create-activity/creates-activity-a: produced diff encodes");
    let committed: serde_json::Value = serde_json::from_str(DIFF).expect("create-activity/creates-activity-a: committed diff decodes");
    assert_eq!(produced, committed, "create-activity/creates-activity-a: the diff create-activity builds differs from the committed 🔺️diff/🔣️.json");
}

/// 🔣️ The committed diff is itself canonical and decodes to ProgramDiff.
#[semio_framework_async_macros::async_test]
async fn create_activity_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("create-activity/creates-activity-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("create-activity/creates-activity-a: committed diff re-encodes");
