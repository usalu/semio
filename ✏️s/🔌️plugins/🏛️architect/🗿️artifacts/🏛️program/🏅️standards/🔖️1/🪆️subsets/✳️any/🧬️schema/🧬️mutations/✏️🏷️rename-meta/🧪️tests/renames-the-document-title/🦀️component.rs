...
//! leaf's own `🔺️diff/🦀️component.rs`, which clones `base.meta`, writes the single `title` field, and emits it as the whole `meta` facet — the diff carries the complete replacement value, not a patch.
//!
...
async fn rename_meta_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rename-meta/renames-the-document-title: the diff rename-meta builds differs from the committed 🔺️diff/🔣️component.json");
}
