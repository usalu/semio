...
//! leaf's own `🔺️diff/🦀️component.rs`, which patches `delivery-constraint-a` in `program.delivery` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `constraint_type`).
//!
...
async fn replace_delivery_constraint_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-delivery-constraint/replaces-delivery-constraint-a: the diff replace-delivery-constraint builds differs from the committed 🔺️diff/🔣️component.json");
}
