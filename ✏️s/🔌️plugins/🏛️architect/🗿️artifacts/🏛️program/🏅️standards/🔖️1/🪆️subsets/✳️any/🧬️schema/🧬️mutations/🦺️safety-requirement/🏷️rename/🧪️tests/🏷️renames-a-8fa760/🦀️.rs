//! 🧪️ `rename-safety-requirement` fixture — `🏷️renames-a`.
//!
...
fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("rename-safety-requirement/renames-safety-requirement-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("rename-safety-requirement/renames-safety-requirement-a: after snapshot decodes")
}
...
async fn rename_safety_requirement_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("rename-safety-requirement/renames-safety-requirement-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("rename-safety-requirement/renames-safety-requirement-a: snapshot re-encodes");
...
async fn rename_safety_requirement_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("rename-safety-requirement/renames-safety-requirement-a: produced diff encodes");
...
async fn rename_safety_requirement_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("rename-safety-requirement/renames-safety-requirement-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("rename-safety-requirement/renames-safety-requirement-a: committed diff re-encodes");
