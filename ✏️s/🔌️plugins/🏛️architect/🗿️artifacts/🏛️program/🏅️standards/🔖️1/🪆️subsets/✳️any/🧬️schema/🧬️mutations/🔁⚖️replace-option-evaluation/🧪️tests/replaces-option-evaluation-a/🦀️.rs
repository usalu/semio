...
//! leaf's own `🔺️diff/🦀️.rs`, which patches `option-evaluation-a` in `program.options` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `option_name`).
//!
