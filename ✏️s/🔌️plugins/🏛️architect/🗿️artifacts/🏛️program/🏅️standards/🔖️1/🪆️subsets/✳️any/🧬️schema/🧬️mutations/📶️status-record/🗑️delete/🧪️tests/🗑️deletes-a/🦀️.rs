...
fn before() -> ProgramSnapshot {
    serde_json::from_str(BEFORE).expect("delete-status-record/deletes-status-record-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    serde_json::from_str(AFTER).expect("delete-status-record/deletes-status-record-a: after snapshot decodes")
}
...
async fn delete_status_record_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = serde_json::from_str(text).expect("delete-status-record/deletes-status-record-a: snapshot decodes");
        let reencoded = serde_json::to_value(&decoded).expect("delete-status-record/deletes-status-record-a: snapshot re-encodes");
...
async fn delete_status_record_produces_committed_diff() {
    let produced = serde_json::to_value(mutation().diff(&before()).diff()).expect("delete-status-record/deletes-status-record-a: produced diff encodes");
...
async fn delete_status_record_committed_diff_is_canonical() {
    let decoded: ProgramDiff = serde_json::from_str(DIFF).expect("delete-status-record/deletes-status-record-a: committed diff decodes");
    let reencoded = serde_json::to_value(&decoded).expect("delete-status-record/deletes-status-record-a: committed diff re-encodes");
