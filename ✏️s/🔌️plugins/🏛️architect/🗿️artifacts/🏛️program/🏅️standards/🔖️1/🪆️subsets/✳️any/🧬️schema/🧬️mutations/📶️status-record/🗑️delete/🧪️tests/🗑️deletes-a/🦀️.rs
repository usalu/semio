//! 🧪️ `delete-status-record` fixture — `🗑️deletes-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️.rs`, which drops `status-record-a` from `program.status_records` as `removed = ["status-record-a"]`, after a target-missing guard.
//!
...
const BEFORE: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📶️status-record/🗑️delete/🗑️deletes-a/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📶️status-record/🗑️delete/🗑️deletes-a/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📶️status-record/🗑️delete/🗑️deletes-a/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📶️status-record/🗑️delete/🗑️deletes-a/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📶️status-record/🗑️delete/🗑️deletes-a/🎯️outcome/🔣️.json");

fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("delete-status-record/deletes-status-record-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("delete-status-record/deletes-status-record-a: after snapshot decodes")
}
...
async fn delete_status_record_applies_to_committed_after() {
    ...
    let applied = outcome.diff().apply(&base).expect("delete-status-record/deletes-status-record-a: delete-status-record applies to its committed before-snapshot");
...
async fn delete_status_record_inverse_restores_before() {
    ...
    undo.reverse();
    let mut state = forward.diff(&base).diff().apply(&base).expect("delete-status-record/deletes-status-record-a: forward diff applies");
    for step in &undo {
        state = step.diff(&state).diff().apply(&state).expect("delete-status-record/deletes-status-record-a: inverse step applies");
    }
...
async fn delete_status_record_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("delete-status-record/deletes-status-record-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("delete-status-record/deletes-status-record-a: snapshot re-encodes");
...
async fn delete_status_record_declared_outcome_holds() {
    ...
    assert!(outcome.diff().apply(&base).is_ok(), "delete-status-record/deletes-status-record-a: delete-status-record was rejected by apply on its own before-snapshot");
}
...
async fn delete_status_record_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("delete-status-record/deletes-status-record-a: produced diff encodes");
...
async fn delete_status_record_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("delete-status-record/deletes-status-record-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("delete-status-record/deletes-status-record-a: committed diff re-encodes");
...
async fn delete_status_record_committed_diff_applies_to_after() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("delete-status-record/deletes-status-record-a: committed diff decodes");
    let produced = decoded.apply(&before()).expect("delete-status-record/deletes-status-record-a: committed diff applies to the before-snapshot");
