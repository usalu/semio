//! 🦠️ ProgramSnapshot mutation — `replace-regulatory-requirement` leaf (replace). Split from the
...
use crate::registers::RegulatoryRequirement;
...
impl MutationKind<ProgramSnapshot, ProgramMutation> for ReplaceRegulatoryRequirement {
    ...
    fn label(&self) -> protocol::LocalizedLabel {
        protocol::LocalizedLabel::native(&format!("Replace regulatory requirement \"{}\"", self.regulatory_requirement.header.name), &format!("Regulatorische Anforderung \"{}\" ersetzen", self.regulatory_requirement.header.name))
    }
