//! 🧪️ `delete-access-rule` fixture — `🗑️deletes-a`.
//!
...
const BEFORE: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🔑️access-rule/🗑️delete/🗑️deletes-a/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🔑️access-rule/🗑️delete/🗑️deletes-a/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🔑️access-rule/🗑️delete/🗑️deletes-a/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🔑️access-rule/🗑️delete/🗑️deletes-a/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🔑️access-rule/🗑️delete/🗑️deletes-a/🎯️outcome/🔣️.json");

fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("delete-access-rule/deletes-access-rule-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("delete-access-rule/deletes-access-rule-a: after snapshot decodes")
}
...
async fn delete_access_rule_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("delete-access-rule/deletes-access-rule-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("delete-access-rule/deletes-access-rule-a: snapshot re-encodes");
...
async fn delete_access_rule_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("delete-access-rule/deletes-access-rule-a: produced diff encodes");
...
async fn delete_access_rule_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("delete-access-rule/deletes-access-rule-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("delete-access-rule/deletes-access-rule-a: committed diff re-encodes");
