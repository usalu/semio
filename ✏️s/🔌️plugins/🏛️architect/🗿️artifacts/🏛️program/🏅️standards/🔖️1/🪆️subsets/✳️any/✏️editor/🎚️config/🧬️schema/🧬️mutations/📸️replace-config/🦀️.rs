...
impl protocol::MutationKind<ArchitectConfig, ArchitectConfigMutation> for ReplaceConfig {
    ...
    fn diff(&self, base: &ArchitectConfig) -> protocol::MutationOutcome<ArchitectConfig> {
        if &self.config == base { return protocol::MutationOutcome::empty().warn("mutation.no-op", "Requested config already matches."); }
        protocol::MutationOutcome::new(self.config.clone())
    }
    fn inverse(&self, base: &ArchitectConfig) -> Vec<ArchitectConfigMutation> { vec![ArchitectConfigMutation::ReplaceConfig(Self { config: base.clone() })] }
