//! 🧪️ `rename-human-factor-requirement` fixture — `🏷️renames-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️.rs`, which patches only `name` on `human-factor-requirement-a` in `program.human_factors` — every other patch field stays `null`.
//!
...
fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("rename-human-factor-requirement/renames-human-factor-requirement-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("rename-human-factor-requirement/renames-human-factor-requirement-a: after snapshot decodes")
}
...
async fn rename_human_factor_requirement_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("rename-human-factor-requirement/renames-human-factor-requirement-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("rename-human-factor-requirement/renames-human-factor-requirement-a: snapshot re-encodes");
...
async fn rename_human_factor_requirement_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("rename-human-factor-requirement/renames-human-factor-requirement-a: produced diff encodes");
    ...
    assert_eq!(produced, committed, "rename-human-factor-requirement/renames-human-factor-requirement-a: the diff rename-human-factor-requirement builds differs from the committed 🔺️diff/🔣️.json");
}

/// 🔣️ The committed diff is itself canonical and decodes to ProgramDiff.
#[semio_framework_async_macros::async_test]
async fn rename_human_factor_requirement_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("rename-human-factor-requirement/renames-human-factor-requirement-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("rename-human-factor-requirement/renames-human-factor-requirement-a: committed diff re-encodes");
