//! 🧪️ `replace-delivery-constraint` fixture — `replaces-delivery-constraint-a`.
//!
...
fn before() -> ProgramSnapshot {
    serde_json::from_str(BEFORE).expect("replace-delivery-constraint/replaces-delivery-constraint-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    serde_json::from_str(AFTER).expect("replace-delivery-constraint/replaces-delivery-constraint-a: after snapshot decodes")
}
...
async fn replace_delivery_constraint_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = serde_json::from_str(text).expect("replace-delivery-constraint/replaces-delivery-constraint-a: snapshot decodes");
        let reencoded = serde_json::to_value(&decoded).expect("replace-delivery-constraint/replaces-delivery-constraint-a: snapshot re-encodes");
...
async fn replace_delivery_constraint_produces_committed_diff() {
    let produced = serde_json::to_value(mutation().diff(&before()).diff()).expect("replace-delivery-constraint/replaces-delivery-constraint-a: produced diff encodes");
...
async fn replace_delivery_constraint_committed_diff_is_canonical() {
    let decoded: ProgramDiff = serde_json::from_str(DIFF).expect("replace-delivery-constraint/replaces-delivery-constraint-a: committed diff decodes");
    let reencoded = serde_json::to_value(&decoded).expect("replace-delivery-constraint/replaces-delivery-constraint-a: committed diff re-encodes");
