...
//! leaf's own `🔺️diff/🦀️component.rs`, which drops `user-profile-a` from `program.users` as `removed = ["user-profile-a"]`, after a target-missing guard.
//!
...
async fn delete_user_profile_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-user-profile/deletes-user-profile-a: the diff delete-user-profile builds differs from the committed 🔺️diff/🔣️component.json");
}
