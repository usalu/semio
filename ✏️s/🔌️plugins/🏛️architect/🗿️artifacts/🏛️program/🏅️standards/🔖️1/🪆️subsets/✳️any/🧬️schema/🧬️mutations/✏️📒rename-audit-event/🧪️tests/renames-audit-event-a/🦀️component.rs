//! 🧪️ `rename-audit-event` fixture — `renames-audit-event-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️component.rs`, which patches only `name` on `audit-event-a` in `program.audit_events` — every other patch field stays `null`.
//!
...
async fn rename_audit_event_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rename-audit-event/renames-audit-event-a: the diff rename-audit-event builds differs from the committed 🔺️diff/🔣️component.json");
}
