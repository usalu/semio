//! 🧪️ `connect-trace` fixture — `🧵️requirement-decision`.
//!
...
const MUTATION: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🧵️trace/🧵️connect/🧵️requirement-decision/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🧵️trace/🧵️connect/🧵️requirement-decision/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🧵️trace/🧵️connect/🧵️requirement-decision/🎯️outcome/🔣️.json");

fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("connect-trace/connects-requirement-a-to-decision-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("connect-trace/connects-requirement-a-to-decision-a: after snapshot decodes")
}
...
async fn connect_trace_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("connect-trace/connects-requirement-a-to-decision-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("connect-trace/connects-requirement-a-to-decision-a: snapshot re-encodes");
...
async fn connect_trace_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("connect-trace/connects-requirement-a-to-decision-a: produced diff encodes");
...
async fn connect_trace_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("connect-trace/connects-requirement-a-to-decision-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("connect-trace/connects-requirement-a-to-decision-a: committed diff re-encodes");
