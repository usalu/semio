//! 🧪️ `delete-storage-requirement` fixture — `deletes-storage-requirement-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️component.rs`, which drops `storage-requirement-a` from `program.storage` as `removed = ["storage-requirement-a"]`, after a target-missing guard.
//!
...
async fn delete_storage_requirement_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-storage-requirement/deletes-storage-requirement-a: the diff delete-storage-requirement builds differs from the committed 🔺️diff/🔣️component.json");
}
