...
//! leaf's own `🔺️diff/🦀️component.rs`, which drops `access-rule-a` from `program.access_rules` as `removed = ["access-rule-a"]`, after a target-missing guard.
//!
...
async fn delete_access_rule_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-access-rule/deletes-access-rule-a: the diff delete-access-rule builds differs from the committed 🔺️diff/🔣️component.json");
}
