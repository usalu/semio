...
//! leaf's own `🔺️diff/🦀️component.rs`, which drops `function-a` from `program.functions` as `removed = ["function-a"]`, after a target-missing guard.
//!
...
async fn delete_function_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-function/deletes-function-a: the diff delete-function builds differs from the committed 🔺️diff/🔣️component.json");
}
