//! 🧪️ `replace-flow-requirement` fixture — `replaces-flow-requirement-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️component.rs`, which patches `flow-requirement-a` in `program.flows` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `flow_type`).
//!
...
async fn replace_flow_requirement_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-flow-requirement/replaces-flow-requirement-a: the diff replace-flow-requirement builds differs from the committed 🔺️diff/🔣️component.json");
}
