...
//! leaf's own `🔺️diff/🦀️.rs`, which drops `delivery-constraint-a` from `program.delivery` as `removed = ["delivery-constraint-a"]`, after a target-missing guard.
//!
...
async fn delete_delivery_constraint_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-delivery-constraint/deletes-delivery-constraint-a: the diff delete-delivery-constraint builds differs from the committed 🔺️diff/🔣️.json");
}
