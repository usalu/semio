...
pub fn diff(payload: &ConnectTrace, base: &ProgramSnapshot) -> protocol::MutationOutcome<ProgramDiff> {
    match base.traces.iter().find(|row| row.id == payload.trace.id) {
        Some(existing) => {
            ...
            protocol::MutationOutcome::new(ProgramDiff { traces: Some(ProgramTracesDelta { patched: vec![crate::diff::ProgramTracesPatchEntry { id: payload.trace.id.0.clone(), patch }], ..Default::default() }), ..Default::default() })
        }
