...
//! leaf's own `🔺️diff/🦀️component.rs`, which patches `decision-a` in `program.decisions` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `approval_status`).
//!
...
async fn replace_decision_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-decision/replaces-decision-a: the diff replace-decision builds differs from the committed 🔺️diff/🔣️component.json");
}
