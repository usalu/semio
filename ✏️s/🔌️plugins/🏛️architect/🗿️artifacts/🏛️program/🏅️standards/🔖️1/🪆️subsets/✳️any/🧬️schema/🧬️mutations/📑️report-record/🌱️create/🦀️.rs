//! 🦠️ ProgramSnapshot mutation — `create-report-record` leaf (create). Split from the
...
use crate::registers::ReportRecord;
...
impl MutationKind<ProgramSnapshot, ProgramMutation> for CreateReportRecord {
    ...
    fn label(&self) -> protocol::LocalizedLabel {
        protocol::LocalizedLabel::native(&format!("Create report record \"{}\"", self.report_record.header.name), &format!("Berichtsdatensatz \"{}\" erstellen", self.report_record.header.name))
    }
