...
//! leaf's own `🔺️diff/🦀️component.rs`, which patches only `name` on `workshop-a` in `program.workshops` — every other patch field stays `null`.
//!
...
async fn rename_workshop_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rename-workshop/renames-workshop-a: the diff rename-workshop builds differs from the committed 🔺️diff/🔣️component.json");
}
