//! 🧪️ `rename-knowledge-record` fixture — `rejects-renaming-absent-knowledge-record-a`.
//!
...
async fn rename_knowledge_record_carries_the_absent_diff_marker() {
    assert!(ABSENT.is_empty(), "rename-knowledge-record/rejects-renaming-absent-knowledge-record-a: 🔺️diff/🚫️component.absent must be an empty marker file");
}
