...
//! leaf's own `🔺️diff/🦀️.rs`, which patches `quality-record-a` in `program.quality` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `quality_topic`).
//!
