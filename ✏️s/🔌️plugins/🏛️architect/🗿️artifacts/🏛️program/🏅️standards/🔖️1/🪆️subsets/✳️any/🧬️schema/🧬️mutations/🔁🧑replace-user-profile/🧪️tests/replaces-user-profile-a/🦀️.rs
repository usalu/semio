...
//! leaf's own `🔺️diff/🦀️.rs`, which patches `user-profile-a` in `program.users` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `validated`).
//!
...
async fn replace_user_profile_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-user-profile/replaces-user-profile-a: the diff replace-user-profile builds differs from the committed 🔺️diff/🔣️.json");
}
