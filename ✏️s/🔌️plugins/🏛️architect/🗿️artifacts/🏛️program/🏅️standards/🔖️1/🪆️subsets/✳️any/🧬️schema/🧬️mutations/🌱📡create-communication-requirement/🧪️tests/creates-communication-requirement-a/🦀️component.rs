...
//! leaf's own `🔺️diff/🦀️component.rs`, which appends the payload row to `program.communication` as `added = [row]`, after a duplicate-id guard.
//!
...
async fn create_communication_requirement_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "create-communication-requirement/creates-communication-requirement-a: the diff create-communication-requirement builds differs from the committed 🔺️diff/🔣️component.json");
}
