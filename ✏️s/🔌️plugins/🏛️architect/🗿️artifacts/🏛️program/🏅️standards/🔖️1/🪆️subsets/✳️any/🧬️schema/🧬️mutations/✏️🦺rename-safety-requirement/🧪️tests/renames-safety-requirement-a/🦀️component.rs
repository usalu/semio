...
//! leaf's own `🔺️diff/🦀️component.rs`, which patches only `name` on `safety-requirement-a` in `program.safety` — every other patch field stays `null`.
//!
...
async fn rename_safety_requirement_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rename-safety-requirement/renames-safety-requirement-a: the diff rename-safety-requirement builds differs from the committed 🔺️diff/🔣️component.json");
}
