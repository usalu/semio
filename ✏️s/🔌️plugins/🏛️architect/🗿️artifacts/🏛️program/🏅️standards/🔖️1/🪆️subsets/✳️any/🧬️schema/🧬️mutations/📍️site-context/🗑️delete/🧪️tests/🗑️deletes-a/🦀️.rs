//! 🧪️ `delete-site-context` fixture — `🗑️deletes-a`.
//!
...
const BEFORE: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📍️site-context/🗑️delete/🗑️deletes-a/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📍️site-context/🗑️delete/🗑️deletes-a/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📍️site-context/🗑️delete/🗑️deletes-a/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📍️site-context/🗑️delete/🗑️deletes-a/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📍️site-context/🗑️delete/🗑️deletes-a/🎯️outcome/🔣️.json");

fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("delete-site-context/deletes-site-context-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("delete-site-context/deletes-site-context-a: after snapshot decodes")
}
...
async fn delete_site_context_applies_to_committed_after() {
    ...
    let applied = outcome.diff().apply(&base).expect("delete-site-context/deletes-site-context-a: delete-site-context applies to its committed before-snapshot");
...
async fn delete_site_context_inverse_restores_before() {
    ...
    undo.reverse();
    let mut state = forward.diff(&base).diff().apply(&base).expect("delete-site-context/deletes-site-context-a: forward diff applies");
    for step in &undo {
        state = step.diff(&state).diff().apply(&state).expect("delete-site-context/deletes-site-context-a: inverse step applies");
    }
...
async fn delete_site_context_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("delete-site-context/deletes-site-context-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("delete-site-context/deletes-site-context-a: snapshot re-encodes");
...
async fn delete_site_context_declared_outcome_holds() {
    ...
    assert!(outcome.diff().apply(&base).is_ok(), "delete-site-context/deletes-site-context-a: delete-site-context was rejected by apply on its own before-snapshot");
}
...
async fn delete_site_context_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("delete-site-context/deletes-site-context-a: produced diff encodes");
...
async fn delete_site_context_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("delete-site-context/deletes-site-context-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("delete-site-context/deletes-site-context-a: committed diff re-encodes");
...
async fn delete_site_context_committed_diff_applies_to_after() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("delete-site-context/deletes-site-context-a: committed diff decodes");
    let produced = decoded.apply(&before()).expect("delete-site-context/deletes-site-context-a: committed diff applies to the before-snapshot");
