...
fn before() -> ProgramSnapshot {
    serde_json::from_str(BEFORE).expect("delete-site-context/deletes-site-context-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    serde_json::from_str(AFTER).expect("delete-site-context/deletes-site-context-a: after snapshot decodes")
}
...
async fn delete_site_context_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = serde_json::from_str(text).expect("delete-site-context/deletes-site-context-a: snapshot decodes");
        let reencoded = serde_json::to_value(&decoded).expect("delete-site-context/deletes-site-context-a: snapshot re-encodes");
...
async fn delete_site_context_produces_committed_diff() {
    let produced = serde_json::to_value(mutation().diff(&before()).diff()).expect("delete-site-context/deletes-site-context-a: produced diff encodes");
...
async fn delete_site_context_committed_diff_is_canonical() {
    let decoded: ProgramDiff = serde_json::from_str(DIFF).expect("delete-site-context/deletes-site-context-a: committed diff decodes");
    let reencoded = serde_json::to_value(&decoded).expect("delete-site-context/deletes-site-context-a: committed diff re-encodes");
