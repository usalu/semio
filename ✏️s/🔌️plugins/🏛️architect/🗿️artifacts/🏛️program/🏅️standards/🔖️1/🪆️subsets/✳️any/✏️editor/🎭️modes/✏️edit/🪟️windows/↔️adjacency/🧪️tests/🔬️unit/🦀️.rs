...
async fn the_matrix_renders_a_triangle_strip_with_element_labels() {
    let json = crate::editor::architect::unit_tests::context::project_render(render(&sample_plugin(), &config::ArchitectAdjacencyWindowConfig::default(), &semio_framework_plugin::TreeWindows::unhosted()));
...
async fn an_empty_program_renders_the_placeholder() {
    let json = crate::editor::architect::unit_tests::context::project_render(render(&empty_plugin(), &config::ArchitectAdjacencyWindowConfig::default(), &semio_framework_plugin::TreeWindows::unhosted()));
