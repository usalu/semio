...
//! leaf's own `🔺️diff/🦀️component.rs`, which drops `cost-requirement-a` from `program.costs` as `removed = ["cost-requirement-a"]`, after a target-missing guard.
//!
...
async fn delete_cost_requirement_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-cost-requirement/deletes-cost-requirement-a: the diff delete-cost-requirement builds differs from the committed 🔺️diff/🔣️component.json");
}
