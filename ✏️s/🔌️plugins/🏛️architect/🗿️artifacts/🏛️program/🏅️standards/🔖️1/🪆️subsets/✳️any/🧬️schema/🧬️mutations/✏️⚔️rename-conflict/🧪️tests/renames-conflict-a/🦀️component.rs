...
//! leaf's own `🔺️diff/🦀️component.rs`, which patches only `name` on `conflict-a` in `program.conflicts` — every other patch field stays `null`.
//!
...
async fn rename_conflict_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rename-conflict/renames-conflict-a: the diff rename-conflict builds differs from the committed 🔺️diff/🔣️component.json");
}
