//! 🧪️ `create-communication-requirement` fixture — `🌱️creates-a`.
//!
...
fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("create-communication-requirement/creates-communication-requirement-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("create-communication-requirement/creates-communication-requirement-a: after snapshot decodes")
}
...
async fn create_communication_requirement_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("create-communication-requirement/creates-communication-requirement-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("create-communication-requirement/creates-communication-requirement-a: snapshot re-encodes");
...
async fn create_communication_requirement_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("create-communication-requirement/creates-communication-requirement-a: produced diff encodes");
...
async fn create_communication_requirement_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("create-communication-requirement/creates-communication-requirement-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("create-communication-requirement/creates-communication-requirement-a: committed diff re-encodes");
