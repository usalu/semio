...
//! leaf's own `🔺️diff/🦀️component.rs`, which patches `performance-criterion-a` in `program.performance` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `criterion`).
//!
...
async fn replace_performance_criterion_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-performance-criterion/replaces-performance-criterion-a: the diff replace-performance-criterion builds differs from the committed 🔺️diff/🔣️component.json");
}
