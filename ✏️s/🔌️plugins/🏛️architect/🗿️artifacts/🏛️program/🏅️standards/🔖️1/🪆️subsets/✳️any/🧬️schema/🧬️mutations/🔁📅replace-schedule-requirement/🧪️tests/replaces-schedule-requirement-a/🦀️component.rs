...
//! leaf's own `🔺️diff/🦀️component.rs`, which patches `schedule-requirement-a` in `program.schedules` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `milestone`).
//!
...
async fn replace_schedule_requirement_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-schedule-requirement/replaces-schedule-requirement-a: the diff replace-schedule-requirement builds differs from the committed 🔺️diff/🔣️component.json");
}
