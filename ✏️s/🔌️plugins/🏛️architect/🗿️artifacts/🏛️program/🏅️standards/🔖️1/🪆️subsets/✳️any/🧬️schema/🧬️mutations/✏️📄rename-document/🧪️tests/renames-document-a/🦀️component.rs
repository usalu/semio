...
//! leaf's own `🔺️diff/🦀️component.rs`, which patches only `name` on `document-a` in `program.artifacts` — every other patch field stays `null`.
//!
...
async fn rename_document_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rename-document/renames-document-a: the diff rename-document builds differs from the committed 🔺️diff/🔣️component.json");
}
