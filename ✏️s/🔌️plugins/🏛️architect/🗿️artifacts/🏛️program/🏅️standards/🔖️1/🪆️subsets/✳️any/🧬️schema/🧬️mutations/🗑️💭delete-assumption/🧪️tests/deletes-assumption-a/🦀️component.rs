...
//! leaf's own `🔺️diff/🦀️component.rs`, which drops `assumption-a` from `program.assumptions` as `removed = ["assumption-a"]`, after a target-missing guard.
//!
...
async fn delete_assumption_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-assumption/deletes-assumption-a: the diff delete-assumption builds differs from the committed 🔺️diff/🔣️component.json");
}
