//! 🧪️ `replace-option-evaluation` fixture — `♻️replaces-a`.
//!
...
const BEFORE: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/⚖️option-evaluation/♻️replace/♻️replaces-a/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/⚖️option-evaluation/♻️replace/♻️replaces-a/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/⚖️option-evaluation/♻️replace/♻️replaces-a/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/⚖️option-evaluation/♻️replace/♻️replaces-a/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/⚖️option-evaluation/♻️replace/♻️replaces-a/🎯️outcome/🔣️.json");

fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("replace-option-evaluation/replaces-option-evaluation-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("replace-option-evaluation/replaces-option-evaluation-a: after snapshot decodes")
}
...
async fn replace_option_evaluation_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("replace-option-evaluation/replaces-option-evaluation-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("replace-option-evaluation/replaces-option-evaluation-a: snapshot re-encodes");
...
async fn replace_option_evaluation_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("replace-option-evaluation/replaces-option-evaluation-a: produced diff encodes");
...
async fn replace_option_evaluation_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("replace-option-evaluation/replaces-option-evaluation-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("replace-option-evaluation/replaces-option-evaluation-a: committed diff re-encodes");
