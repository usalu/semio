//! 🧪️ `replace-relationship` fixture — `♻️replaces-a`.
//!
...
fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("replace-relationship/replaces-relationship-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("replace-relationship/replaces-relationship-a: after snapshot decodes")
}
...
async fn replace_relationship_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("replace-relationship/replaces-relationship-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("replace-relationship/replaces-relationship-a: snapshot re-encodes");
...
async fn replace_relationship_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("replace-relationship/replaces-relationship-a: produced diff encodes");
...
async fn replace_relationship_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("replace-relationship/replaces-relationship-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("replace-relationship/replaces-relationship-a: committed diff re-encodes");
