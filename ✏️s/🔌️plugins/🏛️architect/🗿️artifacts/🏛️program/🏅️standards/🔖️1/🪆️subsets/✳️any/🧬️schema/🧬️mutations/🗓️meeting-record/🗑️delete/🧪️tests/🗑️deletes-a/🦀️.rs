//! 🧪️ `delete-meeting-record` fixture — `🗑️deletes-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️.rs`, which drops `meeting-record-a` from `program.meetings` as `removed = ["meeting-record-a"]`, after a target-missing guard.
//!
...
fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("delete-meeting-record/deletes-meeting-record-a: after snapshot decodes")
}
...
async fn delete_meeting_record_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("delete-meeting-record/deletes-meeting-record-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("delete-meeting-record/deletes-meeting-record-a: snapshot re-encodes");
...
async fn delete_meeting_record_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("delete-meeting-record/deletes-meeting-record-a: produced diff encodes");
    ...
    assert_eq!(produced, committed, "delete-meeting-record/deletes-meeting-record-a: the diff delete-meeting-record builds differs from the committed 🔺️diff/🔣️.json");
}

/// 🔣️ The committed diff is itself canonical and decodes to ProgramDiff.
#[semio_framework_async_macros::async_test]
async fn delete_meeting_record_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("delete-meeting-record/deletes-meeting-record-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("delete-meeting-record/deletes-meeting-record-a: committed diff re-encodes");
