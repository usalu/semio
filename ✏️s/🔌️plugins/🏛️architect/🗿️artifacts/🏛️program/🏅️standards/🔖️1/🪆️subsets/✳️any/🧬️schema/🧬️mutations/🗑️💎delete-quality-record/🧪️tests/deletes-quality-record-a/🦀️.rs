...
//! leaf's own `🔺️diff/🦀️.rs`, which drops `quality-record-a` from `program.quality` as `removed = ["quality-record-a"]`, after a target-missing guard.
//!
...
async fn delete_quality_record_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-quality-record/deletes-quality-record-a: the diff delete-quality-record builds differs from the committed 🔺️diff/🔣️.json");
}
