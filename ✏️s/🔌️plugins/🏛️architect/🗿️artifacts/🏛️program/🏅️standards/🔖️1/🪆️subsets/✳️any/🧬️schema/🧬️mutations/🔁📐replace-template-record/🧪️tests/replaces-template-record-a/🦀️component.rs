...
//! leaf's own `🔺️diff/🦀️component.rs`, which patches `template-record-a` in `program.templates` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `template_type`).
//!
...
async fn replace_template_record_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-template-record/replaces-template-record-a: the diff replace-template-record builds differs from the committed 🔺️diff/🔣️component.json");
}
