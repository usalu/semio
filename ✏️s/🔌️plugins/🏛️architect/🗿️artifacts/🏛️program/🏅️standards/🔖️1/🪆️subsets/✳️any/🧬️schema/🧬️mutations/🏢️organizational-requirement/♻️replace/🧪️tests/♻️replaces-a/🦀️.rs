...
fn before() -> ProgramSnapshot {
    serde_json::from_str(BEFORE).expect("replace-organizational-requirement/replaces-organizational-requirement-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    serde_json::from_str(AFTER).expect("replace-organizational-requirement/replaces-organizational-requirement-a: after snapshot decodes")
}
...
async fn replace_organizational_requirement_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = serde_json::from_str(text).expect("replace-organizational-requirement/replaces-organizational-requirement-a: snapshot decodes");
        let reencoded = serde_json::to_value(&decoded).expect("replace-organizational-requirement/replaces-organizational-requirement-a: snapshot re-encodes");
...
async fn replace_organizational_requirement_produces_committed_diff() {
    let produced = serde_json::to_value(mutation().diff(&before()).diff()).expect("replace-organizational-requirement/replaces-organizational-requirement-a: produced diff encodes");
...
async fn replace_organizational_requirement_committed_diff_is_canonical() {
    let decoded: ProgramDiff = serde_json::from_str(DIFF).expect("replace-organizational-requirement/replaces-organizational-requirement-a: committed diff decodes");
    let reencoded = serde_json::to_value(&decoded).expect("replace-organizational-requirement/replaces-organizational-requirement-a: committed diff re-encodes");
