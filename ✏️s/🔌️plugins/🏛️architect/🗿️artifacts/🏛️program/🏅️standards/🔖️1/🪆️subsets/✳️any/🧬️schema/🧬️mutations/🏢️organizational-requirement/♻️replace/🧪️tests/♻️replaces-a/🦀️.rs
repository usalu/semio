//! 🧪️ `replace-organizational-requirement` fixture — `♻️replaces-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️.rs`, which patches `organizational-requirement-a` in `program.organizational` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `department`).
//!
...
const BEFORE: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🏢️organizational-requirement/♻️replace/♻️replaces-a/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🏢️organizational-requirement/♻️replace/♻️replaces-a/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🏢️organizational-requirement/♻️replace/♻️replaces-a/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🏢️organizational-requirement/♻️replace/♻️replaces-a/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🏢️organizational-requirement/♻️replace/♻️replaces-a/🎯️outcome/🔣️.json");

fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("replace-organizational-requirement/replaces-organizational-requirement-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("replace-organizational-requirement/replaces-organizational-requirement-a: after snapshot decodes")
}
...
async fn replace_organizational_requirement_applies_to_committed_after() {
    ...
    let applied = outcome.diff().apply(&base).expect("replace-organizational-requirement/replaces-organizational-requirement-a: replace-organizational-requirement applies to its committed before-snapshot");
...
async fn replace_organizational_requirement_inverse_restores_before() {
    ...
    undo.reverse();
    let mut state = forward.diff(&base).diff().apply(&base).expect("replace-organizational-requirement/replaces-organizational-requirement-a: forward diff applies");
    for step in &undo {
        state = step.diff(&state).diff().apply(&state).expect("replace-organizational-requirement/replaces-organizational-requirement-a: inverse step applies");
    }
...
async fn replace_organizational_requirement_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("replace-organizational-requirement/replaces-organizational-requirement-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("replace-organizational-requirement/replaces-organizational-requirement-a: snapshot re-encodes");
...
async fn replace_organizational_requirement_declared_outcome_holds() {
    ...
    assert!(outcome.diff().apply(&base).is_ok(), "replace-organizational-requirement/replaces-organizational-requirement-a: replace-organizational-requirement was rejected by apply on its own before-snapshot");
}
...
async fn replace_organizational_requirement_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("replace-organizational-requirement/replaces-organizational-requirement-a: produced diff encodes");
...
async fn replace_organizational_requirement_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("replace-organizational-requirement/replaces-organizational-requirement-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("replace-organizational-requirement/replaces-organizational-requirement-a: committed diff re-encodes");
...
async fn replace_organizational_requirement_committed_diff_applies_to_after() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("replace-organizational-requirement/replaces-organizational-requirement-a: committed diff decodes");
    let produced = decoded.apply(&before()).expect("replace-organizational-requirement/replaces-organizational-requirement-a: committed diff applies to the before-snapshot");
