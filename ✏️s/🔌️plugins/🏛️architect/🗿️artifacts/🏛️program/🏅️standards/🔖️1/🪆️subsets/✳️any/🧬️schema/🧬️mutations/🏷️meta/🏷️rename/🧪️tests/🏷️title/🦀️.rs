...
fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("rename-meta/renames-the-document-title: after snapshot decodes")
}
...
async fn rename_meta_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("rename-meta/renames-the-document-title: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("rename-meta/renames-the-document-title: snapshot re-encodes");
...
async fn rename_meta_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("rename-meta/renames-the-document-title: produced diff encodes");
...
async fn rename_meta_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("rename-meta/renames-the-document-title: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("rename-meta/renames-the-document-title: committed diff re-encodes");
