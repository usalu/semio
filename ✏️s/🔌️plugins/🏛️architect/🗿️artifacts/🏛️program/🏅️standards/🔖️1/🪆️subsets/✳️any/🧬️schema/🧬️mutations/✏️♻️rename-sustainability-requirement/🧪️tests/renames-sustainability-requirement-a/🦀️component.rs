...
//! leaf's own `🔺️diff/🦀️component.rs`, which patches only `name` on `sustainability-requirement-a` in `program.sustainability` — every other patch field stays `null`.
//!
...
async fn rename_sustainability_requirement_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rename-sustainability-requirement/renames-sustainability-requirement-a: the diff rename-sustainability-requirement builds differs from the committed 🔺️diff/🔣️component.json");
}
