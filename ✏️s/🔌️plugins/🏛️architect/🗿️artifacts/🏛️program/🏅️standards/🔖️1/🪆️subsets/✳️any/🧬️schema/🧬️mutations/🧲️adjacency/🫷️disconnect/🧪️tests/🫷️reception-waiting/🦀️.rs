...
const BEFORE: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🧲️adjacency/🫷️disconnect/🫷️reception-waiting/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🧲️adjacency/🫷️disconnect/🫷️reception-waiting/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🧲️adjacency/🫷️disconnect/🫷️reception-waiting/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🧲️adjacency/🫷️disconnect/🫷️reception-waiting/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🧲️adjacency/🫷️disconnect/🫷️reception-waiting/🎯️outcome/🔣️.json");

fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("disconnect-adjacency/disconnects-reception-from-waiting: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("disconnect-adjacency/disconnects-reception-from-waiting: after snapshot decodes")
}
...
async fn disconnect_adjacency_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("disconnect-adjacency/disconnects-reception-from-waiting: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("disconnect-adjacency/disconnects-reception-from-waiting: snapshot re-encodes");
...
async fn disconnect_adjacency_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("disconnect-adjacency/disconnects-reception-from-waiting: produced diff encodes");
...
async fn disconnect_adjacency_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("disconnect-adjacency/disconnects-reception-from-waiting: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("disconnect-adjacency/disconnects-reception-from-waiting: committed diff re-encodes");
