...
//! leaf's own `🔺️diff/🦀️component.rs`, which finds no trace with this id and therefore takes the `added = [trace]` branch (endpoints are free-form, unchecked).
//!
...
async fn connect_trace_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "connect-trace/connects-requirement-a-to-decision-a: the diff connect-trace builds differs from the committed 🔺️diff/🔣️component.json");
}
