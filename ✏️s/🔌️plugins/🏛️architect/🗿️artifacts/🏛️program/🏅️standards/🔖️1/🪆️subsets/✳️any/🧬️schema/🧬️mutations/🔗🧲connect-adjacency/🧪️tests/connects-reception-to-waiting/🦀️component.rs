...
//! leaf's own `🔺️diff/🦀️component.rs`, which checks both endpoint elements exist, normalizes the pair, forces `normalized = true`, and — because no edge yet joins `element-a`/`element-b` — takes the `added = [normalized edge]` branch.
//!
...
async fn connect_adjacency_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "connect-adjacency/connects-reception-to-waiting: the diff connect-adjacency builds differs from the committed 🔺️diff/🔣️component.json");
}
