//! 🧪️ `disconnect-adjacency` fixture — `disconnects-reception-from-waiting`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️component.rs`, which drops `adjacency-a` from `program.adjacencies` as `removed = ["adjacency-a"]`.
//!
...
async fn disconnect_adjacency_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "disconnect-adjacency/disconnects-reception-from-waiting: the diff disconnect-adjacency builds differs from the committed 🔺️diff/🔣️component.json");
}
