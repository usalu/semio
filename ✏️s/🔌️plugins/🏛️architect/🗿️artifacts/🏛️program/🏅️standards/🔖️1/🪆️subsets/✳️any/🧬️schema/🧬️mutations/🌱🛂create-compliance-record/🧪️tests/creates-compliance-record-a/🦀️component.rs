...
//! leaf's own `🔺️diff/🦀️component.rs`, which appends the payload row to `program.compliance_records` as `added = [row]`, after a duplicate-id guard.
//!
...
async fn create_compliance_record_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "create-compliance-record/creates-compliance-record-a: the diff create-compliance-record builds differs from the committed 🔺️diff/🔣️component.json");
}
