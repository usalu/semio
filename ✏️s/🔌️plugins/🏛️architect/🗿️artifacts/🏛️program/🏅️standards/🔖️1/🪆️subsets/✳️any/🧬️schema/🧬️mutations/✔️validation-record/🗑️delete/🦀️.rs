//! 🦠️ ProgramSnapshot mutation — `delete-validation-record` leaf (delete). Split from the
...
impl MutationKind<ProgramSnapshot, ProgramMutation> for DeleteValidationRecord {
    ...
    fn label(&self) -> protocol::LocalizedLabel {
        protocol::LocalizedLabel::native(&format!("Delete validation record \"{}\"", self.id.0), &format!("Validierungsdatensatz \"{}\" löschen", self.id.0))
    }
