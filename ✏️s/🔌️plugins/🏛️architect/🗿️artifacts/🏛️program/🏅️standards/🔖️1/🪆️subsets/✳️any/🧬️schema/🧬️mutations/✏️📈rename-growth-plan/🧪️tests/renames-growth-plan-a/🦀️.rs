...
//! leaf's own `🔺️diff/🦀️.rs`, which patches only `name` on `growth-plan-a` in `program.growth` — every other patch field stays `null`.
//!
...
async fn rename_growth_plan_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rename-growth-plan/renames-growth-plan-a: the diff rename-growth-plan builds differs from the committed 🔺️diff/🔣️.json");
}
