//! 🧪️ `replace-delivery-constraint` fixture — `♻️replaces-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️.rs`, which patches `delivery-constraint-a` in `program.delivery` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `constraint_type`).
//!
...
const BEFORE: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🚚️delivery-constraint/♻️replace/♻️replaces-a/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🚚️delivery-constraint/♻️replace/♻️replaces-a/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🚚️delivery-constraint/♻️replace/♻️replaces-a/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🚚️delivery-constraint/♻️replace/♻️replaces-a/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🚚️delivery-constraint/♻️replace/♻️replaces-a/🎯️outcome/🔣️.json");

fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("replace-delivery-constraint/replaces-delivery-constraint-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("replace-delivery-constraint/replaces-delivery-constraint-a: after snapshot decodes")
}
...
async fn replace_delivery_constraint_applies_to_committed_after() {
    ...
    let applied = outcome.diff().apply(&base).expect("replace-delivery-constraint/replaces-delivery-constraint-a: replace-delivery-constraint applies to its committed before-snapshot");
...
async fn replace_delivery_constraint_inverse_restores_before() {
    ...
    undo.reverse();
    let mut state = forward.diff(&base).diff().apply(&base).expect("replace-delivery-constraint/replaces-delivery-constraint-a: forward diff applies");
    for step in &undo {
        state = step.diff(&state).diff().apply(&state).expect("replace-delivery-constraint/replaces-delivery-constraint-a: inverse step applies");
    }
...
async fn replace_delivery_constraint_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("replace-delivery-constraint/replaces-delivery-constraint-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("replace-delivery-constraint/replaces-delivery-constraint-a: snapshot re-encodes");
...
async fn replace_delivery_constraint_declared_outcome_holds() {
    ...
    assert!(outcome.diff().apply(&base).is_ok(), "replace-delivery-constraint/replaces-delivery-constraint-a: replace-delivery-constraint was rejected by apply on its own before-snapshot");
}
...
async fn replace_delivery_constraint_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("replace-delivery-constraint/replaces-delivery-constraint-a: produced diff encodes");
...
async fn replace_delivery_constraint_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("replace-delivery-constraint/replaces-delivery-constraint-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("replace-delivery-constraint/replaces-delivery-constraint-a: committed diff re-encodes");
...
async fn replace_delivery_constraint_committed_diff_applies_to_after() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("replace-delivery-constraint/replaces-delivery-constraint-a: committed diff decodes");
    let produced = decoded.apply(&before()).expect("replace-delivery-constraint/replaces-delivery-constraint-a: committed diff applies to the before-snapshot");
