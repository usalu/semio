//! 🧪️ `replace-quality-record` fixture — `replaces-quality-record-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️component.rs`, which patches `quality-record-a` in `program.quality` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `quality_topic`).
//!
...
fn before() -> ProgramSnapshot {
    serde_json::from_str(BEFORE).expect("replace-quality-record/replaces-quality-record-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    serde_json::from_str(AFTER).expect("replace-quality-record/replaces-quality-record-a: after snapshot decodes")
}
...
async fn replace_quality_record_applies_to_committed_after() {
    ...
    let applied = outcome.diff().apply(&base).expect("replace-quality-record/replaces-quality-record-a: replace-quality-record applies to its committed before-snapshot");
...
async fn replace_quality_record_inverse_restores_before() {
    ...
    undo.reverse();
    let mut state = forward.diff(&base).diff().apply(&base).expect("replace-quality-record/replaces-quality-record-a: forward diff applies");
    for step in &undo {
        state = step.diff(&state).diff().apply(&state).expect("replace-quality-record/replaces-quality-record-a: inverse step applies");
    }
...
async fn replace_quality_record_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = serde_json::from_str(text).expect("replace-quality-record/replaces-quality-record-a: snapshot decodes");
        let reencoded = serde_json::to_value(&decoded).expect("replace-quality-record/replaces-quality-record-a: snapshot re-encodes");
...
async fn replace_quality_record_declared_outcome_holds() {
    ...
    assert!(outcome.diff().apply(&base).is_ok(), "replace-quality-record/replaces-quality-record-a: replace-quality-record was rejected by apply on its own before-snapshot");
}
...
async fn replace_quality_record_produces_committed_diff() {
    let produced = serde_json::to_value(mutation().diff(&before()).diff()).expect("replace-quality-record/replaces-quality-record-a: produced diff encodes");
    ...
    assert_eq!(produced, committed, "replace-quality-record/replaces-quality-record-a: the diff replace-quality-record builds differs from the committed 🔺️diff/🔣️component.json");
}

/// 🔣️ The committed diff is itself canonical and decodes to ProgramDiff.
#[semio_framework_async_macros::async_test]
async fn replace_quality_record_committed_diff_is_canonical() {
    let decoded: ProgramDiff = serde_json::from_str(DIFF).expect("replace-quality-record/replaces-quality-record-a: committed diff decodes");
    let reencoded = serde_json::to_value(&decoded).expect("replace-quality-record/replaces-quality-record-a: committed diff re-encodes");
...
async fn replace_quality_record_committed_diff_applies_to_after() {
    let decoded: ProgramDiff = serde_json::from_str(DIFF).expect("replace-quality-record/replaces-quality-record-a: committed diff decodes");
    let produced = decoded.apply(&before()).expect("replace-quality-record/replaces-quality-record-a: committed diff applies to the before-snapshot");
