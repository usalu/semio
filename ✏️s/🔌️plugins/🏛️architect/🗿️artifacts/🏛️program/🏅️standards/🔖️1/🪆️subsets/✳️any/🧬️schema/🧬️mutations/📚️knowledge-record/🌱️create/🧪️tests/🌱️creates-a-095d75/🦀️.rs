//! 🧪️ `create-knowledge-record` fixture — `🌱️creates-a`.
//!
...
fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("create-knowledge-record/creates-knowledge-record-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("create-knowledge-record/creates-knowledge-record-a: after snapshot decodes")
}
...
async fn create_knowledge_record_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("create-knowledge-record/creates-knowledge-record-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("create-knowledge-record/creates-knowledge-record-a: snapshot re-encodes");
...
async fn create_knowledge_record_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("create-knowledge-record/creates-knowledge-record-a: produced diff encodes");
...
async fn create_knowledge_record_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("create-knowledge-record/creates-knowledge-record-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("create-knowledge-record/creates-knowledge-record-a: committed diff re-encodes");
