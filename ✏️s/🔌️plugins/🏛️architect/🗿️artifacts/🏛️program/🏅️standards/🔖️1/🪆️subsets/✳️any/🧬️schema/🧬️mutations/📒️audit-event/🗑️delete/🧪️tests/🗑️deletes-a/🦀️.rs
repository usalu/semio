...
fn before() -> ProgramSnapshot {
    serde_json::from_str(BEFORE).expect("delete-audit-event/deletes-audit-event-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    serde_json::from_str(AFTER).expect("delete-audit-event/deletes-audit-event-a: after snapshot decodes")
}
...
async fn delete_audit_event_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = serde_json::from_str(text).expect("delete-audit-event/deletes-audit-event-a: snapshot decodes");
        let reencoded = serde_json::to_value(&decoded).expect("delete-audit-event/deletes-audit-event-a: snapshot re-encodes");
...
async fn delete_audit_event_produces_committed_diff() {
    let produced = serde_json::to_value(mutation().diff(&before()).diff()).expect("delete-audit-event/deletes-audit-event-a: produced diff encodes");
...
async fn delete_audit_event_committed_diff_is_canonical() {
    let decoded: ProgramDiff = serde_json::from_str(DIFF).expect("delete-audit-event/deletes-audit-event-a: committed diff decodes");
    let reencoded = serde_json::to_value(&decoded).expect("delete-audit-event/deletes-audit-event-a: committed diff re-encodes");
