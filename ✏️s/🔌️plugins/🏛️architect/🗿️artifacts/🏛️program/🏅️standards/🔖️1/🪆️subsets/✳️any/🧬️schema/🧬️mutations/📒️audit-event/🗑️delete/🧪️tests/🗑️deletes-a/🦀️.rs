//! 🧪️ `delete-audit-event` fixture — `🗑️deletes-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️.rs`, which drops `audit-event-a` from `program.audit_events` as `removed = ["audit-event-a"]`, after a target-missing guard.
//!
...
const BEFORE: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📒️audit-event/🗑️delete/🗑️deletes-a/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📒️audit-event/🗑️delete/🗑️deletes-a/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📒️audit-event/🗑️delete/🗑️deletes-a/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📒️audit-event/🗑️delete/🗑️deletes-a/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/📒️audit-event/🗑️delete/🗑️deletes-a/🎯️outcome/🔣️.json");

fn before() -> ProgramSnapshot {
    dsl::json::from_json_str(BEFORE).expect("delete-audit-event/deletes-audit-event-a: before snapshot decodes")
}

fn expected_after() -> ProgramSnapshot {
    dsl::json::from_json_str(AFTER).expect("delete-audit-event/deletes-audit-event-a: after snapshot decodes")
}
...
async fn delete_audit_event_applies_to_committed_after() {
    ...
    let applied = outcome.diff().apply(&base).expect("delete-audit-event/deletes-audit-event-a: delete-audit-event applies to its committed before-snapshot");
...
async fn delete_audit_event_inverse_restores_before() {
    ...
    undo.reverse();
    let mut state = forward.diff(&base).diff().apply(&base).expect("delete-audit-event/deletes-audit-event-a: forward diff applies");
    for step in &undo {
        state = step.diff(&state).diff().apply(&state).expect("delete-audit-event/deletes-audit-event-a: inverse step applies");
    }
...
async fn delete_audit_event_committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: ProgramSnapshot = dsl::json::from_json_str(text).expect("delete-audit-event/deletes-audit-event-a: snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("delete-audit-event/deletes-audit-event-a: snapshot re-encodes");
...
async fn delete_audit_event_declared_outcome_holds() {
    ...
    assert!(outcome.diff().apply(&base).is_ok(), "delete-audit-event/deletes-audit-event-a: delete-audit-event was rejected by apply on its own before-snapshot");
}
...
async fn delete_audit_event_produces_committed_diff() {
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(mutation().diff(&before()).diff())).expect("delete-audit-event/deletes-audit-event-a: produced diff encodes");
...
async fn delete_audit_event_committed_diff_is_canonical() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("delete-audit-event/deletes-audit-event-a: committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("delete-audit-event/deletes-audit-event-a: committed diff re-encodes");
...
async fn delete_audit_event_committed_diff_applies_to_after() {
    let decoded: ProgramDiff = dsl::json::from_json_str(DIFF).expect("delete-audit-event/deletes-audit-event-a: committed diff decodes");
    let produced = decoded.apply(&before()).expect("delete-audit-event/deletes-audit-event-a: committed diff applies to the before-snapshot");
