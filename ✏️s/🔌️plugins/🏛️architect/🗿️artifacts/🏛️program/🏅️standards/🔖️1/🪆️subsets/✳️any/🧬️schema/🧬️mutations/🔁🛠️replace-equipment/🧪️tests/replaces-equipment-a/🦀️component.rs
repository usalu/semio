//! 🧪️ `replace-equipment` fixture — `replaces-equipment-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️component.rs`, which patches `equipment-a` in `program.equipment` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `code`).
//!
...
async fn replace_equipment_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-equipment/replaces-equipment-a: the diff replace-equipment builds differs from the committed 🔺️diff/🔣️component.json");
}
