//! 🧪️ `replace-safety-requirement` fixture — `replaces-safety-requirement-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️component.rs`, which patches `safety-requirement-a` in `program.safety` with the FULL `None` that `Patchable::diff_patch` snapshots off the payload row (this fixture moves `hazard`).
//!
...
async fn replace_safety_requirement_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-safety-requirement/replaces-safety-requirement-a: the diff replace-safety-requirement builds differs from the committed 🔺️diff/🔣️component.json");
}
