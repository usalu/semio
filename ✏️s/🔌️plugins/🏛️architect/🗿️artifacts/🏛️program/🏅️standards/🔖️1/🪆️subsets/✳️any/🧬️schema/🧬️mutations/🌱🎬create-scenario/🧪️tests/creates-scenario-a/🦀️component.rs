...
//! leaf's own `🔺️diff/🦀️component.rs`, which appends the payload row to `program.scenarios` as `added = [row]`, after a duplicate-id guard.
//!
...
async fn create_scenario_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "create-scenario/creates-scenario-a: the diff create-scenario builds differs from the committed 🔺️diff/🔣️component.json");
}
