//! 🧪️ `create-meeting-record` fixture — `creates-meeting-record-a`.
//!
//! Hand-authored source of truth is the JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). Every expectation below is transcribed from THIS
//! leaf's own `🔺️diff/🦀️component.rs`, which appends the payload row to `program.meetings` as `added = [row]`, after a duplicate-id guard.
//!
...
async fn create_meeting_record_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "create-meeting-record/creates-meeting-record-a: the diff create-meeting-record builds differs from the committed 🔺️diff/🔣️component.json");
}
