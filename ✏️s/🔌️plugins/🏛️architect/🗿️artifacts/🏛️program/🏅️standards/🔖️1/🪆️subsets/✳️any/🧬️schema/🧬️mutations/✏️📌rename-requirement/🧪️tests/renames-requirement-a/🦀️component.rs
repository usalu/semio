...
//! leaf's own `🔺️diff/🦀️component.rs`, which patches only `name` on `requirement-a` in `program.requirements` — every other patch field stays `null`.
//!
...
async fn rename_requirement_produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rename-requirement/renames-requirement-a: the diff rename-requirement builds differs from the committed 🔺️diff/🔣️component.json");
}
