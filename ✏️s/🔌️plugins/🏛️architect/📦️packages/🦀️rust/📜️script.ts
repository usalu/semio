#!/usr/bin/env bun
/** 🏛️ `@semio-tech/architect-plugin` router: `bun ./📜️script.ts test`. */
...
class TestScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    ...
    await runCargoTestBudgeted(["semio-s-plugin-architect"], this.repoRoot, rest);
  }
}
...
class DescribeScript extends BundleScript {
  run(): void {
    process.exit(describePluginComponent(this.repoRoot, "semio-s-plugin-architect", join(this.root, "..", "..")));
  }
}
