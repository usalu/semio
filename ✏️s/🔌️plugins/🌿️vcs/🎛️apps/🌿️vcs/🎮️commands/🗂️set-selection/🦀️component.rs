//! 🗂️ 🗂️ VCS play app commands command — `set-selection`.
...
mod tests {
    ...
    fn vcs_demo_command_op_text_round_trips() {
        store::os_store::test_support::assert_op_line_round_trip(&VcsCommand::SetSelection(SetSelection { ids: vec!["checkpoint-1".into()] }));
    }
    ...
    fn set_selection_drives_config_and_emits_no_mutations() {
        ...
        let result = dispatch(&mut instance, VcsCommand::SetSelection(SetSelection { ids: vec![checkpoint_id.clone()] }));
    ...
    fn set_selection_never_creates_a_document_undo_step() {
        ...
        dispatch(&mut instance, VcsCommand::SetSelection(SetSelection { ids: vec!["checkpoint-1".into()] }));
