...
mod tests {
    ...
    fn vcs_demo_command_op_text_round_trips() {
        store::test_support::assert_op_line_round_trip(&VcsCommand::SetSelection(set_selection::SetSelection { ids: vec!["checkpoint-1".into()] }));
    }
    ...
    fn set_selection_never_creates_a_document_undo_step() {
        ...
        assert_eq!(instance.projection().expect("materialize projection").counter, before, "document undo has nothing to revert — selection never touched the document");
    }
}
//#endregion 🧪️Tests
