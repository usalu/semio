...
//! preserved verbatim (a plain structural `DslValue` coercion, kept as-is — reshaping the domain
...
impl Serializer<VcsSnapshot> for VcsIntoCsv {
    ...
    async fn serialize(from: &VcsSnapshot) -> IoResult<IoPayload> {
        let csv = CsvSnapshot::from_value(from.to_value()).map_err(|error| IoError { message: format!("VcsIntoCsv: {error}"), diagnostics: Vec::new() })?;
