...
mod structural_correspondence_tests {
    ...
    fn direct_owners_descriptors_surfaces_and_catalog_correspond() {
        ...
        assert_eq!(descriptor_kinds, catalog_kinds);
        {
            ...
            {
                let surface_source = std::fs::read_to_string(owner.join("🔣️payload.schema.json")).expect("direct language surface");
                assert!(surface_source.contains(kind) || surface_source.contains(variant));
            }
            {
                let surface_source = std::fs::read_to_string(owner.join("🔣️wire.schema.json")).expect("direct language surface");
                assert!(surface_source.contains(kind) || surface_source.contains(variant));
            }
        }
        {
