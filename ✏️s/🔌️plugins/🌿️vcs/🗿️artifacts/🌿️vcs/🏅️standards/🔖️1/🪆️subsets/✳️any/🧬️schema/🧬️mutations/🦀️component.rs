...
mod structural_correspondence_tests {
    ...
    fn direct_owners_descriptors_surfaces_and_catalog_correspond() {
        let direct_owners = [
            ("rename-vcs", "RenameVcs", "✏️rename-vcs"),
            ("change-counter", "ChangeCounter", "🔢change-counter"),
            ("change-notes", "ChangeNotes", "📝change-notes"),
            ("change-status", "ChangeStatus", "🚦change-status"),
            ("add-tag", "AddTag", "🏷️add-tag"),
        ...
        for (kind, variant, directory) in direct_owners {
            ...
            for surface in ["🟦️component.ts", "🔗️component.graphql", "🛰️component.proto", "🔣️payload.schema.json", "🔣️wire.schema.json"] {
