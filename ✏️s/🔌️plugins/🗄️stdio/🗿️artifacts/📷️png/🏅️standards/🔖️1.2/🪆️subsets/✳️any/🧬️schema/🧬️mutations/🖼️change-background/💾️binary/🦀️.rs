...
pub const BINARY_TAG: u8 = dsl::protocol_record::tag_u8(include_str!("../../💾️binary/📡️.protocol.semio"), "change-background");
...
pub fn encode_payload(payload: &ChangeBackgroundMutation) -> Result<Vec<u8>, protocol::ProtocolError> {
    ...
    write_bin_option(&mut w, bkgd, write_bin_background);
    Ok(w.into_bytes())
}
...
pub fn decode(bytes: &[u8]) -> Result<PngMutation, protocol::ProtocolError> {
    ...
    let result: Result<PngMutation, protocol::ProtocolError> = Ok(PngMutation::ChangeBackground(ChangeBackgroundMutation { bkgd: read_bin_option(&mut r, read_bin_background).map_err(|error| op_pack_err(&error))? }));
