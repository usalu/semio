//! 🧪️ `insert-text-chunk` fixture — `direct-behavior`.
//!
...
fn mutation() -> PngMutation {
    serde_json::from_str(MUTATION).expect("committed insert-text-chunk payload decodes")
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "insert-text-chunk/direct-behavior: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
fn op_codecs_round_trip() {
    ...
    assert_eq!(bytes[1], super::binary::BINARY_TAG, "insert-text-chunk/direct-behavior: the binary frame must carry this schema's tag");
