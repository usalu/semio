...
pub const BINARY_TAG: u8 = dsl::protocol_record::tag_u8(include_str!("../../💾️binary/📡️.protocol.semio"), "replace-palette");
...
pub fn decode(bytes: &[u8]) -> Result<PngMutation, protocol::ProtocolError> {
    ...
    let result: Result<PngMutation, protocol::ProtocolError> = Ok(PngMutation::ReplacePalette(ReplacePaletteMutation { plte: read_bin_option(&mut r, |r| read_bin_vec(r, read_bin_rgb)).map_err(|error| op_pack_err(&error))? }));
