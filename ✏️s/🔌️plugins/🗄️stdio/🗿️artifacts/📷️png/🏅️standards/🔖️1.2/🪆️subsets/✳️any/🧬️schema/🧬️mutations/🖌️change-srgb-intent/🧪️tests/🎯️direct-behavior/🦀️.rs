...
fn mutation() -> PngMutation {
    serde_json::from_str(MUTATION).expect("committed change-srgb-intent payload decodes")
}
...
fn op_codecs_round_trip() {
    ...
    assert_eq!(bytes[1], super::binary::BINARY_TAG, "change-srgb-intent/direct-behavior: the binary frame must carry this schema's tag");
