...
/// `PngMutation`'s own hand-rolled `OpText`/`OpBinary` (`🧬️mutations/🦀️.rs`) reuses them
...
// primitives, `🧰️framework/…/🎒️pack/🧾️codec/🦀️.rs`, reachable exactly like csv's own
// precedent — `dsl`/`store`/`protocol` all alias the same kernel crate root). `pub(crate)` so
// `🧬️mutations/🦀️.rs`'s own `OpBinary` impl reuses these instead of duplicating (same
...
impl protocol::DiffCodec for PngDiff {
    ...
    /// text-as-binary shortcut. Matches `../💾️binary/📡️.protocol.semio`'s real
...
/// (`⚙️engine/🦀️.rs`'s `conformance_laws` module) call this directly instead of
