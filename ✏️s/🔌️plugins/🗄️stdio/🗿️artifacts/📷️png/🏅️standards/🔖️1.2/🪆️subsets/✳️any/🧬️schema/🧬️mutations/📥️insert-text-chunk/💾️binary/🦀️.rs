...
pub const BINARY_TAG: u8 = dsl::protocol_record::tag_u8(include_str!("../../💾️binary/📡️.protocol.semio"), "insert-text-chunk");
...
pub fn decode(bytes: &[u8]) -> Result<PngMutation, protocol::ProtocolError> {
    ...
    let result: Result<PngMutation, protocol::ProtocolError> = Ok({
        ...
        PngMutation::InsertTextChunk(InsertTextChunkMutation { index, chunk })
    });
