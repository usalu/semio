//! 🧪️ `remove-unknown-chunk` fixture — `🎯️direct-behavior`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📤️remove-unknown-chunk/🎯️direct-behavior/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📤️remove-unknown-chunk/🎯️direct-behavior/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📤️remove-unknown-chunk/🎯️direct-behavior/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📤️remove-unknown-chunk/🎯️direct-behavior/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📤️remove-unknown-chunk/🎯️direct-behavior/🎯️outcome/🔣️.json");
...
fn mutation() -> PngMutation {
    dsl::json::from_json_str(MUTATION).expect("committed remove-unknown-chunk payload decodes")
}
...
fn applies_to_committed_after() {
    let base = before();
    let produced = mutation().diff(&base).diff().apply(&base).expect("remove-unknown-chunk applies to its committed before-snapshot");
    assert_eq!(produced, expected_after(), "remove-unknown-chunk/direct-behavior: applied state differs from the committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    let mut current = payload.diff(&base).diff().apply(&base).expect("forward remove-unknown-chunk applies");
    for step in payload.inverse(&base) {
        current = step.diff(&current).diff().apply(&current).expect("the remove-unknown-chunk inverse step applies");
    }
    assert_eq!(current, base, "remove-unknown-chunk/direct-behavior: the undo did not restore the committed before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: PngSnapshot = dsl::json::from_json_str(text).expect("snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("snapshot encodes");
        let original: serde_json::Value = serde_json::from_str(text).expect("snapshot reparses");
        assert_eq!(reencoded, original, "remove-unknown-chunk/direct-behavior: committed {label} JSON is not canonical");
    }
    ...
    assert_eq!(reencoded, original, "remove-unknown-chunk/direct-behavior: committed payload JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(raised.is_empty(), "remove-unknown-chunk/direct-behavior: declared applied, but dispatch raised {raised:?}"),
        _ => assert!(!raised.is_empty(), "remove-unknown-chunk/direct-behavior: declared {status}, but dispatch raised nothing"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "remove-unknown-chunk/direct-behavior: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "remove-unknown-chunk/direct-behavior: committed diff did not carry before to after");
}
...
fn op_codecs_round_trip() {
    let payload = mutation();
    assert_eq!(PngMutation::parse_op(&payload.print_op()).expect("the text op parses"), payload, "remove-unknown-chunk/direct-behavior: the text op form does not round-trip");
    let bytes = payload.encode_op().expect("the binary op encodes");
    assert_eq!(bytes[1], binary::BINARY_TAG, "remove-unknown-chunk/direct-behavior: the binary frame must carry this schema's tag");
    assert_eq!(PngMutation::decode_op(&bytes).expect("the binary op decodes"), payload, "remove-unknown-chunk/direct-behavior: the binary op form does not round-trip");
    assert!(PngMutation::decode_op(&bytes[..1]).is_err(), "remove-unknown-chunk/direct-behavior: a truncated binary frame must be rejected");
}
