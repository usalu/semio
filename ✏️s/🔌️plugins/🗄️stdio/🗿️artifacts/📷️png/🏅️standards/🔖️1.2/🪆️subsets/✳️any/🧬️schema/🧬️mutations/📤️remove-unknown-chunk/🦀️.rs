...
impl protocol::MutationKind<PngSnapshot, PngMutation> for RemoveUnknownChunkMutation {
    ...
    fn inverse(&self, base: &PngSnapshot) -> Vec<PngMutation> {
        ...
        match base.unknown_chunks.get(*index) {
            Some(chunk) => vec![PngMutation::InsertUnknownChunk(crate::schema::mutations::InsertUnknownChunkMutation { index: *index, chunk: chunk.clone() })],
            None => Vec::new(),
        }
    }
...
pub(crate) fn test_case() -> PngMutation {
    dsl::json::from_json_str(include_str!("../../../🧫️fixtures/🧬️mutations/📤️remove-unknown-chunk/🎯️direct-behavior/🦠️mutation/🔣️.json")).expect("committed remove-unknown-chunk payload")
}
#[cfg(test)]
