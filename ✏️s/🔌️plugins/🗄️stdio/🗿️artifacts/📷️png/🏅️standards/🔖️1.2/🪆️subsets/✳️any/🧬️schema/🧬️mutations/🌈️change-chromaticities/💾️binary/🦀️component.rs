...
pub fn decode(bytes: &[u8]) -> Result<PngMutation, protocol::ProtocolError> {
    ...
    let result: Result<PngMutation, protocol::ProtocolError> =
        Ok(PngMutation::ChangeChromaticities(crate::artifacts::png::schema::mutations::ChangeChromaticitiesMutation { chrm: diff::read_bin_option(&mut r, diff::read_bin_chromaticities).map_err(op_pack_err)? }));
