//! 🧪️ `insert-text-chunk` fixture — `🎯️direct-behavior`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📥️insert-text-chunk/🎯️direct-behavior/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📥️insert-text-chunk/🎯️direct-behavior/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📥️insert-text-chunk/🎯️direct-behavior/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📥️insert-text-chunk/🎯️direct-behavior/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📥️insert-text-chunk/🎯️direct-behavior/🎯️outcome/🔣️.json");
...
fn mutation() -> PngMutation {
    dsl::json::from_json_str(MUTATION).expect("committed insert-text-chunk payload decodes")
}
...
fn applies_to_committed_after() {
    let base = before();
    let produced = mutation().diff(&base).diff().apply(&base).expect("insert-text-chunk applies to its committed before-snapshot");
    assert_eq!(produced, expected_after(), "insert-text-chunk/direct-behavior: applied state differs from the committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    let mut current = payload.diff(&base).diff().apply(&base).expect("forward insert-text-chunk applies");
    for step in payload.inverse(&base) {
        current = step.diff(&current).diff().apply(&current).expect("the insert-text-chunk inverse step applies");
    }
    assert_eq!(current, base, "insert-text-chunk/direct-behavior: the undo did not restore the committed before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "insert-text-chunk/direct-behavior: committed {label} JSON is not canonical");
    }
    ...
    assert_eq!(reencoded, original, "insert-text-chunk/direct-behavior: committed payload JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(raised.is_empty(), "insert-text-chunk/direct-behavior: declared applied, but dispatch raised {raised:?}"),
        _ => assert!(!raised.is_empty(), "insert-text-chunk/direct-behavior: declared {status}, but dispatch raised nothing"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "insert-text-chunk/direct-behavior: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "insert-text-chunk/direct-behavior: committed diff did not carry before to after");
}
...
fn op_codecs_round_trip() {
    let payload = mutation();
    assert_eq!(PngMutation::parse_op(&payload.print_op()).expect("the text op parses"), payload, "insert-text-chunk/direct-behavior: the text op form does not round-trip");
    let bytes = payload.encode_op().expect("the binary op encodes");
    assert_eq!(bytes[1], binary::BINARY_TAG, "insert-text-chunk/direct-behavior: the binary frame must carry this schema's tag");
    assert_eq!(PngMutation::decode_op(&bytes).expect("the binary op decodes"), payload, "insert-text-chunk/direct-behavior: the binary op form does not round-trip");
    assert!(PngMutation::decode_op(&bytes[..1]).is_err(), "insert-text-chunk/direct-behavior: a truncated binary frame must be rejected");
}
