...
pub const BINARY_TAG: u8 = dsl::protocol_record::tag_u8(include_str!("../../💾️binary/📡️.protocol.semio"), "change-transparency");
...
pub fn encode_payload(payload: &ChangeTransparencyMutation) -> Result<Vec<u8>, protocol::ProtocolError> {
    ...
    write_bin_option(&mut w, trns, write_bin_transparency);
    Ok(w.into_bytes())
}
...
pub fn decode(bytes: &[u8]) -> Result<PngMutation, protocol::ProtocolError> {
    ...
    let result: Result<PngMutation, protocol::ProtocolError> = Ok(PngMutation::ChangeTransparency(ChangeTransparencyMutation { trns: read_bin_option(&mut r, read_bin_transparency).map_err(|error| op_pack_err(&error))? }));
