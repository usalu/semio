//! 🧪️ `change-transparency` fixture — `direct-behavior`.
//!
...
fn mutation() -> PngMutation {
    serde_json::from_str(MUTATION).expect("committed change-transparency payload decodes")
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-transparency/direct-behavior: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
fn op_codecs_round_trip() {
    ...
    assert_eq!(bytes[1], super::binary::BINARY_TAG, "change-transparency/direct-behavior: the binary frame must carry this schema's tag");
