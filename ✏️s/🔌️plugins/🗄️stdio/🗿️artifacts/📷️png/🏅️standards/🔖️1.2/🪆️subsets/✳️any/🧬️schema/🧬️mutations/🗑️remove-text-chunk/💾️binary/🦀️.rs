...
pub const BINARY_TAG: u8 = dsl::protocol_record::tag_u8(include_str!("../../💾️binary/📡️.protocol.semio"), "remove-text-chunk");
...
pub fn decode(bytes: &[u8]) -> Result<PngMutation, protocol::ProtocolError> {
    ...
    let result: Result<PngMutation, protocol::ProtocolError> = Ok(PngMutation::RemoveTextChunk(RemoveTextChunkMutation { index: r.read_varint_u64().map_err(|error| op_pack_err(&error))? as usize }));
