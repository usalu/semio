...
pub fn decode(bytes: &[u8]) -> Result<PngMutation, protocol::ProtocolError> {
    ...
    let result: Result<PngMutation, protocol::ProtocolError> = Ok(PngMutation::RemoveTextChunk(RemoveTextChunkMutation { index: r.read_varint_u64().map_err(op_pack_err)? as usize }));
