...
pub fn decode(bytes: &[u8]) -> Result<PngMutation, protocol::ProtocolError> {
    ...
    let result: Result<PngMutation, protocol::ProtocolError> =
        Ok(PngMutation::ChangePhysicalDims(crate::artifacts::png::schema::mutations::ChangePhysicalDimsMutation { phys: diff::read_bin_option(&mut r, diff::read_bin_physical_dims).map_err(op_pack_err)? }));
