...
fn mutation() -> PngMutation {
    dsl::json::from_json_str(MUTATION).expect("committed change-header payload decodes")
}
...
fn op_codecs_round_trip() {
    ...
    assert_eq!(bytes[1], binary::BINARY_TAG, "change-header/direct-behavior: the binary frame must carry this schema's tag");
