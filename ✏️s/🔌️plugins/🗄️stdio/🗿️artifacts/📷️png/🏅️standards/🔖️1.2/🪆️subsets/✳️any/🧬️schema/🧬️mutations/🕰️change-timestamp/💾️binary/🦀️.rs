...
pub const BINARY_TAG: u8 = dsl::protocol_record::tag_u8(include_str!("../../💾️binary/📡️.protocol.semio"), "change-timestamp");
...
pub fn encode_payload(payload: &ChangeTimestampMutation) -> Result<Vec<u8>, protocol::ProtocolError> {
    ...
    write_bin_option(&mut w, time, write_bin_timestamp);
    Ok(w.into_bytes())
}
...
pub fn decode(bytes: &[u8]) -> Result<PngMutation, protocol::ProtocolError> {
    ...
    let result: Result<PngMutation, protocol::ProtocolError> = Ok(PngMutation::ChangeTimestamp(ChangeTimestampMutation { time: read_bin_option(&mut r, read_bin_timestamp).map_err(|error| op_pack_err(&error))? }));
