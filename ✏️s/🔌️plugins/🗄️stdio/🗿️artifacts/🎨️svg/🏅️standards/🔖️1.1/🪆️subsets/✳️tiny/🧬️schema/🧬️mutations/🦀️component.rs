...
pub fn kind_of(mutation: &SvgTinyMutation) -> &'static str {
    match mutation {
        SvgTinyMutation::NoMutation => "no-mutation",
        SvgTinyMutation::SetSnapshot { .. } => "set-snapshot",
        SvgTinyMutation::StampBaseProfile { .. } => "stamp-base-profile",
        SvgTinyMutation::InsertTinyElement { .. } => "insert-tiny-element",
        SvgTinyMutation::RemoveElement { .. } => "remove-element",
        SvgTinyMutation::SetTinyAttribute { .. } => "set-tiny-attribute",
        SvgTinyMutation::SetText { .. } => "set-text",
        SvgTinyMutation::SetViewBox { .. } => "set-view-box",
        SvgTinyMutation::SetTransform { .. } => "set-transform",
        SvgTinyMutation::StripNonTiny => "strip-non-tiny",
    }
}
...
impl Mutation<SvgSnapshot> for SvgTinyMutation {
    type Diff = SvgDiff;

    fn diff(&self, base: &SvgSnapshot) -> protocol::MutationOutcome<Self::Diff> {
        match self {
            SvgTinyMutation::NoMutation => protocol::MutationOutcome::new(SvgDiff::default()),
            SvgTinyMutation::SetSnapshot { snapshot } => protocol::MutationOutcome::new(diff_set_snapshot(base, snapshot)),
            SvgTinyMutation::StampBaseProfile { base_profile, version } => protocol::MutationOutcome::new(attributes_diff_at_path(base, &[], &[("baseProfile", base_profile.clone()), ("version", version.clone())])),
            SvgTinyMutation::InsertTinyElement { parent, index, node } => match subtree_profile_violation(node) {
            ...
            SvgTinyMutation::RemoveElement { parent, index } => protocol::MutationOutcome::new(diff_at_path(
                parent,
            ...
            SvgTinyMutation::SetTinyAttribute { path, name, value } => {
            ...
            SvgTinyMutation::SetText { path, text } => protocol::MutationOutcome::new(diff_at_path(path, SvgNodeDiff::Text { text: Some(text.clone()) })),
            SvgTinyMutation::SetViewBox { path, view_box } => protocol::MutationOutcome::new(attributes_diff_at_path(base, path, &[("viewBox", view_box.as_ref().map(view_box_to_string))])),
            SvgTinyMutation::SetTransform { path, transform } => protocol::MutationOutcome::new(attributes_diff_at_path(base, path, &[("transform", transform.as_ref().map(|ops| transform_list_to_string(ops)))])),
            SvgTinyMutation::StripNonTiny => protocol::MutationOutcome::new(diff_set_snapshot(base, &stripped_to_tiny(base))),
        }
    }
    ...
    fn inverse(&self, base: &SvgSnapshot) -> Vec<Self> {
        match self {
            SvgTinyMutation::NoMutation => vec![SvgTinyMutation::NoMutation],
            SvgTinyMutation::SetSnapshot { .. } | SvgTinyMutation::StripNonTiny => vec![SvgTinyMutation::SetSnapshot { snapshot: base.clone() }],
            SvgTinyMutation::StampBaseProfile { .. } => vec![SvgTinyMutation::StampBaseProfile { base_profile: prior_attribute(base, &[], "baseProfile"), version: prior_attribute(base, &[], "version") }],
            SvgTinyMutation::InsertTinyElement { parent, index, .. } => vec![SvgTinyMutation::RemoveElement { parent: parent.clone(), index: *index }],
            SvgTinyMutation::RemoveElement { parent, index } => match node_at(&base.doc, parent) {
                Ok(XmlNode::Element { children, .. }) => match children.get(*index) {
                    Some(node) => vec![SvgTinyMutation::InsertTinyElement { parent: parent.clone(), index: *index, node: node.clone() }],
                    None => vec![SvgTinyMutation::NoMutation],
                },
                _ => vec![SvgTinyMutation::NoMutation],
            },
            SvgTinyMutation::SetTinyAttribute { path, name, .. } => vec![SvgTinyMutation::SetTinyAttribute { path: path.clone(), name: name.clone(), value: prior_attribute(base, path, name) }],
            SvgTinyMutation::SetText { path, .. } => {
                let old = match node_at(&base.doc, path) {
                    Ok(XmlNode::Text { text }) => text.clone(),
                    _ => String::new(),
                };
                vec![SvgTinyMutation::SetText { path: path.clone(), text: old }]
            }
            SvgTinyMutation::SetViewBox { path, .. } => vec![SvgTinyMutation::SetViewBox { path: path.clone(), view_box: prior_attribute(base, path, "viewBox").and_then(|v| parse_view_box(&v).ok()) }],
            SvgTinyMutation::SetTransform { path, .. } => vec![SvgTinyMutation::SetTransform { path: path.clone(), transform: prior_attribute(base, path, "transform").and_then(|v| parse_transform_list(&v).ok()) }],
        }
    }
}
...
mod tests {
    ...
    fn kinds_matches_enum_variants_and_manifest() {
        let every = vec![
            SvgTinyMutation::NoMutation,
            SvgTinyMutation::SetSnapshot { snapshot: SvgSnapshot::default() },
            SvgTinyMutation::StampBaseProfile { base_profile: None, version: None },
            SvgTinyMutation::InsertTinyElement { parent: Vec::new(), index: 0, node: elem("rect", vec![], vec![]) },
            SvgTinyMutation::RemoveElement { parent: Vec::new(), index: 0 },
            SvgTinyMutation::SetTinyAttribute { path: Vec::new(), name: "fill".into(), value: None },
            SvgTinyMutation::SetText { path: Vec::new(), text: String::new() },
            SvgTinyMutation::SetViewBox { path: Vec::new(), view_box: None },
            SvgTinyMutation::SetTransform { path: Vec::new(), transform: None },
            SvgTinyMutation::StripNonTiny,
        ];
    ...
    fn insert_tiny_element_rejects_an_excluded_subtree() {
        ...
        let outcome = apply_svg_tiny_mutation(&mut snapshot, &SvgTinyMutation::InsertTinyElement { parent: Vec::new(), index: 0, node: elem("filter", vec![], vec![elem("feTurbulence", vec![], vec![])]) });
    ...
    fn set_tiny_attribute_rejects_a_forbidden_presentation_attribute() {
        let mut snapshot = document(elem("svg", vec![], vec![]));
        let outcome = apply_svg_tiny_mutation(&mut snapshot, &SvgTinyMutation::SetTinyAttribute { path: Vec::new(), name: "opacity".into(), value: Some("0.5".into()) });
    ...
    fn strip_non_tiny_removes_excluded_elements_and_attributes() {
        ...
        apply_svg_tiny_mutation(&mut snapshot, &SvgTinyMutation::StripNonTiny);
    ...
    fn strip_non_tiny_is_invertible_through_its_own_inverse() {
        ...
        let mutation = SvgTinyMutation::StripNonTiny;
    ...
    fn stamp_base_profile_is_invertible_when_the_root_declared_neither_attribute() {
        ...
        let mut snapshot = base.clone();
        let mutation = SvgTinyMutation::StampBaseProfile { base_profile: Some("tiny".into()), version: Some("1.1".into()) };
        let undo = Mutation::inverse(&mutation, &base);
        apply_svg_tiny_mutation(&mut snapshot, &mutation);
