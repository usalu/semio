...
fn svgTinyEditor_example_snapshot(example_id: &str) -> SvgSnapshot {
...
fn svgTinyEditor_command_id(command: &SvgTinyEditCommand) -> &'static str {
...
fn svgTinyEditor_retained_extent(command: &SvgTinyEditCommand, _snapshot: &SvgSnapshot, _interaction: &protocol::InteractionState) -> Option<usize> {
    matches!(command, SvgTinyEditCommand::SetActiveExample { .. }).then_some(1)
}
fn svgTinyEditor_retained_reduce(command: &SvgTinyEditCommand, _snapshot: &SvgSnapshot, _config: &NoConfig, _history: &semio_framework_plugin::HistoryView, _interaction: &protocol::InteractionState, _hover: &semio_framework_plugin::app::InteractionHoverState, _context: Option<&semio_framework_plugin::app::ArtifactOwnedToolJobContext<EditorApp<SvgTinyEditor>>>, _operation: &AppOperationContext) -> Result<Emit<SvgTinyMutation, NoConfigMutation, NoDraftMutation>, Fault> {
    match command {
        SvgTinyEditCommand::SetActiveExample { example_id } => Ok(Emit { effects: vec![semio_s_artifact_stdio_contract::load_example_effect(&svgTinyEditor_example_snapshot(example_id), STDIO_SVG_DOCUMENT_SCHEMA)], description: Some(format!("Load example {example_id}")), ..Default::default() }),
        _ => Err(Fault::from("stdio-example-retained-route-mismatch")),
    }
}
struct SvgTinyEditorExampleFactory { keys: Vec<ToolFactoryKey> }
impl SvgTinyEditorExampleFactory { fn new(controller_id: &str) -> Self { Self { keys: STDIO_SVG_DOCUMENT_SCHEMA_EXAMPLE_TOOL_IDS.iter().map(|tool_id| ToolFactoryKey::new(controller_id, *tool_id)).collect() } } }
impl ToolJobFactory for SvgTinyEditorExampleFactory {
    type Payload = ArtifactRetainedCommandPayload<EditorApp<SvgTinyEditor>>;
    type Job = ArtifactRetainedCommandJob<EditorApp<SvgTinyEditor>>;
...
impl ArtifactOwnedToolJobFactory for SvgTinyEditorExampleFactory {
    type Owner = EditorApp<SvgTinyEditor>;
    ...
    semio_framework_plugin::bounded_first_step_tool_proofs! {
        ...
        factory: "SvgTinyEditorExampleFactory",
        factory_type: SvgTinyEditorExampleFactory,
    ...
    fn register_tool_job_factories(registry: &mut ArtifactToolFactoryRegistry<'_, EditorApp<Self>>) -> Result<(), Fault> {
        registry.register(SvgTinyEditorExampleFactory::new(registry.controller_id()))
    }
    fn build_tool_job(request: ArtifactOwnedToolJobRequest<EditorApp<Self>>) -> Result<Option<ToolOperationSpec>, Fault> {
        if !STDIO_SVG_DOCUMENT_SCHEMA_EXAMPLE_TOOL_IDS.contains(&request.tool_id.as_str()) { return Ok(None); }
        if svgTinyEditor_command_id(&request.command) != request.tool_id { return Err(Fault::from("stdio-example-tool-mismatch")); }
        ...
        let payload = ArtifactRetainedCommandPayload::try_new(ArtifactRetainedCommandInputs { command: *request.command, snapshot: request.snapshot, config: request.config, history: request.history, interaction_state: request.interaction_state, interaction_hover: request.interaction_hover, context: Some(request.context), operation, completion: request.completion }, svgTinyEditor_command_id, STDIO_SVG_DOCUMENT_SCHEMA_EXAMPLE_BYTES, 1, Box::new(BoundedArtifactCommandWork::new(semio_s_artifact_stdio_contract::SET_ACTIVE_EXAMPLE_ACTION_ID, svgTinyEditor_retained_reduce, svgTinyEditor_retained_extent)))?;
...
impl ArtifactEditor for SvgTinyEditor {
    ...
    fn handle(
        ...
        match command {
            SvgTinyEditCommand::SetActiveExample { example_id } => Ok(Emit {
                effects: vec![semio_s_artifact_stdio_contract::load_example_effect(&svgTinyEditor_example_snapshot(example_id), STDIO_SVG_DOCUMENT_SCHEMA)],
...
pub fn create_svg_tiny_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(SVG_TINY_DIALECT).document(["semio", "svg"]).icon_id("image").mode_def(edit::definition()).default_mode_id(edit::MODE_ID).window_kind_def(main::definition()).default_layout(edit::layout()).action_with(semio_s_artifact_stdio_contract::set_active_example_action())
