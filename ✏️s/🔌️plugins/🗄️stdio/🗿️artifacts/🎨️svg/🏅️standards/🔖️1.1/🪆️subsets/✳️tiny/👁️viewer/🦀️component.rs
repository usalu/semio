//! 👁️ `svg` viewer (tiny) — the read-only counterpart of `✏️editor` for this
//! subset (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `SvgTinyViewer`
//! implements `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<SvgTinyViewer>` is
...
pub enum SvgTinyViewCommand {
    Noop,
}

impl protocol::OpBinary for SvgTinyViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(SvgTinyViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
impl ArtifactViewer for SvgTinyViewer {
    ...
    type Command = SvgTinyViewCommand;
...
pub fn create_svg_tiny_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(SVG_TINY_DIALECT).document(["semio", "svg"]).icon_id("image").mode_def(view::definition()).default_mode_id(view::MODE_ID).window_kind_def(main::definition()).default_layout(view::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn create_viewer_builds_a_definition_for_the_viewer_role() {
        let def = create_svg_tiny_viewer();
    ...
    async fn viewer_dialect_matches_the_artifact_coordinate() {
        assert_eq!(<SvgTinyViewer as ArtifactViewer>::DIALECT, SVG_TINY_DIALECT);
    }
}
//#endregion 🧪️Tests
