...
impl protocol::DiffCodec for SvgDiff {
    ...
    /// ⚡️ Binary = the text bytes verbatim, same simplification as `GifDiff`'s hand-rolled codec
    /// (and the repo's only other hand-rolled `DiffCodec`, `WriterDiff`) — satisfies every
    /// `DiffCodec` law without inventing a second wire format.
...
mod handcrafted_diff_codec_tests {
    ...
    /// 🧪️ F6-PILOT: `DiffCodec` round-trip laws over the hand-rolled `SvgDiff` grammar —
    /// exercises the recursive enum tree (`Element`/`Text`/`Replace` `SvgNodeDiff` variants),
    /// both top-level tri-states, attribute add/remove/modify, and nested child add/remove/modify.
    #[test]
    fn diff_codec_text_binary_roundtrip_law() {
        ...
        let b = snapshot(XmlDocument {
            root: Some(elem("svg", vec![("width", "20"), ("height", "30")], vec![
                elem("circle", vec![("r", "5")], vec![]),
