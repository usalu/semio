//! 🏗️ SvgBasicBuilder (1.1/✳️basic) — a builder whose `build()` can only return an
//! `SvgSnapshot` that is SVG Basic 1.1-conformant BY CONSTRUCTION (D5 requirement #3): it
//! unconditionally injects `baseProfile="basic"`/`version="1.1"` onto the document root, then
//! re-runs the SAME `check_svg_basic_conformance` used by `SvgBasicComposer`, regardless of which
//! path (`from_snapshot`/`from_text`/`from_binary`/`mutate`) produced the in-flight snapshot -- so
//! a hard Basic 1.1 violation can never leave this builder as an `Ok(SvgSnapshot)`.
...
pub struct SvgBasicBuilder {
    snapshot: SvgSnapshot,
}

impl ArtifactBuilder for SvgBasicBuilder {
...
mod tests {
    ...
    fn hard_violation_injected_via_raw_mutate_still_fails_build() {
        let mut snapshot = SvgBasicBuilder::empty().build().unwrap();
        ...
        let (mutated, _diff) = SvgBasicBuilder::from_snapshot(SvgSnapshot::default()).mutate(SvgMutation::SetSnapshot { snapshot });
    ...
    fn from_text_round_trips_through_basic_build() {
        ...
        let built = SvgBasicBuilder::from_text(text).expect("parses").build().expect("conforming document builds");
