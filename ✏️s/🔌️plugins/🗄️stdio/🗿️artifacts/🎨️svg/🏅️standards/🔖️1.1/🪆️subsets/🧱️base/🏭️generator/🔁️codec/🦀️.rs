...
//! registration (`../../../🔮️oracles/🔣️.json`, oracle id `quick-xml-svg-1-1-mutate-reader`) needs.
...
//! sibling `🔬️probes/📜️script.ts` header and `../../../🔮️oracles/🔣️.json`'s
//! `svg-1-1-quick-xml-reader-v1` comparisonProfile for exactly this scoping.
//!
//! Three subcommands:
//!   build   <recipe-id> <out-dir> <physical-dir> <before-file> <after-file>
...
//#region 🔖️Entry
fn cmd_build(id: &str, out_dir: &str, physical_dir: &str, before_file: &str, after_file: &str) -> i32 {
    let Some((before, after)) = recipe(id) else {
        eprintln!("[quick-xml-svg-codec] unknown recipe {id:?} — known: {}", RECIPE_IDS.join(", "));
        return 1;
    };
    let dir = Path::new(out_dir).join(physical_dir);
    fs::create_dir_all(&dir).expect("create fixture recipe directory");
    fs::write(dir.join(before_file), write_svg(&before)).expect("write before SVG");
    fs::write(dir.join(after_file), write_svg(&after)).expect("write after SVG");
    eprintln!("[quick-xml-svg-codec] {id}: {before_file} + {after_file} -> {}", dir.display());
    0
}
...
fn main() {
    ...
    let code = match args.get(1).map(String::as_str) {
        Some("build") => {
            let (Some(id), Some(out_dir), Some(physical_dir), Some(before_file), Some(after_file)) = (args.get(2), args.get(3), args.get(4), args.get(5), args.get(6)) else {
                eprintln!("usage: quick-xml-svg-codec build <recipe-id> <out-dir> <physical-dir> <before-file> <after-file>");
                std::process::exit(2);
            };
            cmd_build(id, out_dir, physical_dir, before_file, after_file)
        }
        Some("project") => {
        ...
        _ => {
            eprintln!("usage: quick-xml-svg-codec build <recipe-id> <out-dir> <physical-dir> <before-file> <after-file> | project <path-to-svg> | list-recipes");
            2
        }
    };
