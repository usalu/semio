...
//
//   bun 📜️script.ts generate  [--only <fixture-id>]     # writes <outDir>/<id>/{before,after}.svg
...
// @see ./🦀️quick-xml-svg-codec/src/main.rs — the actual codec; `build <recipe-id> <out-dir>`,
...
function fileEntry(role: string, dir: string, filename: string, id: string): { role: string; path: string; mediaType: string; sha256: string; bytes: number } {
  ...
  return { role, path: `${FIXTURE_PATH_PREFIX}${id}/${filename}`, mediaType: "image/svg+xml", sha256: contentDigest(bytes), bytes: bytes.length };
}
...
function generateOne(recipe: Recipe, outDir: string): Record<string, unknown> {
  ...
  if (!existsSync(join(dir, "before.svg")) || !existsSync(join(dir, "after.svg"))) {
    throw new Error(`recipe ${recipe.id} did not produce both before.svg and after.svg`);
  }
  const files = [fileEntry("expected-before-svg", dir, "before.svg", recipe.id), fileEntry("expected-after-svg", dir, "after.svg", recipe.id)];
