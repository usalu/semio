//! 🎹️ SvgBasicComposer (1.1/✳️basic) — reads the same sources the ✳️any subset does (native
//! `stdio.svg` 1.1, plus its `xml` DAG dependency), delegates the actual parse to the ✳️any
//! composer, injects `baseProfile="basic"`/`version="1.1"` onto the root, then HARD-GATES the
//! `basic` dialect stamp on real SVG Basic 1.1 conformance (D5 requirement #2): a blocklisted
//! raster filter primitive or a clip-path-to-text reference fails composition outright with
//! specific `Diagnostic`s naming what's wrong; a soft one (a nested `<svg>`) passes through as an
...
//! SAME `check_svg_basic_conformance` function backs both: the hard gate here runs
//! pre-serialization against the typed `SvgSnapshot` (authoritative), while the registered
//! validator re-runs it post-hoc against the wire `IoPayload`.
...
use crate::artifacts::svg::standards::v1_1::subsets::basic::analyzer::check_svg_basic_conformance;
...
//#region 🔖️Composer
pub struct SvgBasicComposer;

impl ArtifactComposer for SvgBasicComposer {
...
/// (`crate::artifacts::svg::standards::v1_1::composer::entries()`).
pub fn register() {
...
mod tests {
    ...
    use crate::artifacts::svg::standards::v1_1::subsets::basic::analyzer::CODE_FILTER_PRIMITIVE;
    use crate::artifacts::svg::standards::v1_1::subsets::basic::builder::SvgBasicBuilder;
    ...
    fn conforming_document_composes_and_stamps_basic() {
        ...
        let composed = SvgBasicComposer::compose(&sources).expect("clean document must compose to basic");
    ...
    fn blocklisted_filter_primitive_fails_compose_with_real_diagnostic() {
        ...
        let sources = vec![ComposeSource { dialect: DIALECT_ANY, payload: AnalyzeSource::Text(text) }];
        let err = SvgBasicComposer::compose(&sources).expect_err("a document with feTurbulence must not stamp basic");
