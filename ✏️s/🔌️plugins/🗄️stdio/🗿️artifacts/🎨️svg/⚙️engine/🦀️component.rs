...
pub fn register() {
    crate::artifacts::svg::io::register();
    register_artifact_schema();
    register_pilot_languages();
    store::register_document_codec(store::DocumentCodec::of::<SvgSnapshot, SvgMutation>(STDIO_SVG_DOCUMENT_SCHEMA));
}
...
impl protocol::ArtifactEngine for SvgEngine {
    type Artifact = SvgArtifact;
...
mod tests {
    ...
    fn codec_round_trip() {
        ...
        let parsed = <SvgSnapshot as store::DocumentDsl>::parse_dsl(&text).expect("parse");
        assert_eq!(parsed.schema, snap.schema);
        let bytes = store::DocumentPack::encode_pack(&snap);
        let decoded = <SvgSnapshot as store::DocumentPack>::decode_pack(&bytes).expect("decode");
