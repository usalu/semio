...
const LEGACY_ENGINE_BIN = join(LEGACY_ENGINE_DIR, "target", "release", "generate");
const LEGACY_RECIPE = "pattern-shell";
const LEGACY_FIXTURE_FILE = "pattern-shell.obj";

const READER_MANIFEST = join(HERE, "🦀️tobj-obj-reader", "Cargo.toml");
const READER_BIN = join(HERE, "🦀️tobj-obj-reader", "target", "release", "tobj-obj-reader");
 ...
 *  parses Rust. */
const READER_RECIPES: readonly { id: string; hasAfter: boolean }[] = [
  { id: "no-mutation-no-op", hasAfter: true },
  { id: "set-snapshot-applied", hasAfter: true },
  { id: "set-vertex-applied", hasAfter: true },
  { id: "set-vertex-rejected-out-of-bounds", hasAfter: false },
  { id: "set-texcoord-applied", hasAfter: true },
  { id: "set-texcoord-rejected-out-of-bounds", hasAfter: false },
  { id: "set-normal-applied", hasAfter: true },
  { id: "set-normal-rejected-out-of-bounds", hasAfter: false },
  { id: "insert-face-applied", hasAfter: true },
  { id: "insert-face-rejected-out-of-bounds", hasAfter: false },
  { id: "remove-face-applied", hasAfter: true },
  { id: "remove-face-rejected-missing", hasAfter: false },
  { id: "set-face-applied", hasAfter: true },
  { id: "set-face-rejected-out-of-bounds", hasAfter: false },
  { id: "set-group-applied", hasAfter: true },
  { id: "remove-group-applied", hasAfter: true },
  { id: "remove-group-rejected-missing", hasAfter: false },
  { id: "set-object-applied", hasAfter: true },
  { id: "remove-object-applied", hasAfter: true },
  { id: "remove-object-rejected-missing", hasAfter: false },
];
//#endregion 🧬️Contract
...
function generateReaderRecipe(recipeId: string, fixturesRoot: string): void {
  ensureReaderBuilt();
  mkdirSync(fixturesRoot, { recursive: true });
  const result = spawnSync(READER_BIN, ["build", recipeId, fixturesRoot], { stdio: "inherit" });
  if (result.status !== 0) throw new Error(`tobj-obj-reader build ${recipeId} failed with status ${result.status}`);
}
...
async function generate(outFlagDir: string | null, only: string | null): Promise<void> {
  ...
  if (only === null || only === LEGACY_RECIPE) {
    await generateLegacy(join(root, LEGACY_RECIPE));
    return;
  }
  ...
  generateReaderRecipe(recipe.id, root);
}
//#endregion 🚪️Generate
...
async function manifestForLegacy(): Promise<Record<string, unknown>> {
  const dir = join(FIXTURES_DIR, LEGACY_RECIPE);
  ...
  return {
    ...
    files: [await fileEntry("primary-obj", dir, LEGACY_FIXTURE_FILE, LEGACY_RECIPE)],
...
async function manifestForReaderRecipe(recipe: { id: string; hasAfter: boolean }): Promise<Record<string, unknown>> {
  const dir = join(FIXTURES_DIR, recipe.id);
  const beforePath = join(dir, "before.obj");
  if (!existsSync(beforePath)) throw new Error(`${beforePath} does not exist — run "generate --only ${recipe.id}" first`);
  const files = [await fileEntry("expected-before-obj", dir, "before.obj", recipe.id)];
  if (recipe.hasAfter) files.push(await fileEntry("expected-after-obj", dir, "after.obj", recipe.id));
  return {
    schema: "semio.repository-test.fixture/v2",
    id: recipe.id,
    class: "third-party-generated",
  ...
  if (command === "document" || command === "document-manifests") {
    ...
    if (command === "document") {
      ...
      for (const [kind, after] of Object.entries(PAIRS)) {
        const dir = join(outFlagDir ?? process.env.SEMIO_FIXTURE_OUT ?? FIXTURES_DIR, kind);
        mkdirSync(dir, { recursive: true });
        writeFileSync(join(dir, "before.obj"), BASE);
        writeFileSync(join(dir, "after.obj"), after);
        // 🔍️Observability through the READER, before the pair is allowed to stand.
        if (projectionOf(join(dir, "before.obj")) === projectionOf(join(dir, "after.obj"))) failures.push(`${kind}: not observable in the document projection`);
    ...
    for (const kind of Object.keys(PAIRS)) {
      ...
      const files = [];
      for (const [role, name] of [["expected-before-obj", "before.obj"], ["expected-after-obj", "after.obj"]] as const) {
        const path = join(dir, name);
        files.push({ role, path: `../🧫️fixtures/${kind}/${name}`, mediaType: "model/obj", sha256: await sha256(path), bytes: statSync(path).size });
      }
      entries.push({
        schema: "semio.repository-test.fixture/v2",
        id: `document-${kind}`,
