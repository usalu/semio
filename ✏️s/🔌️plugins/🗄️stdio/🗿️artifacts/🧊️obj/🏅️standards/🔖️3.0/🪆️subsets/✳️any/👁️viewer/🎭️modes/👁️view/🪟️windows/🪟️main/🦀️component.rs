...
async fn entity_count(document: &ObjSnapshot) -> usize {
...
async fn world_instances_json(document: &ObjSnapshot) -> String {
...
/// 👁️ Pure `ObjSnapshot -> UiNode` read: default camera (a viewer has no persisted
/// per-session camera — `Config = NoConfig`), no selection/gumball/engagement overlay.
pub async fn render(document: &ObjSnapshot) -> UiNode {
    ...
    let view = MeshView {
        camera_json: world3d_camera_json(OBJ_ANY_VIEW_DEFAULT_CAMERA_POSITION, OBJ_ANY_VIEW_DEFAULT_CAMERA_TARGET, OBJ_ANY_VIEW_DEFAULT_CAMERA_FOV).await,
        meshes_json,
