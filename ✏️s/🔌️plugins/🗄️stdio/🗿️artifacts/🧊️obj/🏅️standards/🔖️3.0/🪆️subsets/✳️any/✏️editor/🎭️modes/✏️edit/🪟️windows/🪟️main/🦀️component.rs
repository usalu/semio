//! 🧊 OBJ editor — the Main window: the SAME world-3d mesh scene shape as the sibling
...
const OBJ_ANY_EDIT_FALLBACK_MESH_KIND: &str = "box";
const OBJ_ANY_EDIT_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const OBJ_ANY_EDIT_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const OBJ_ANY_EDIT_DEFAULT_CAMERA_FOV: f64 = 45.0;
    ...
    let instances: Vec<pack::JsonValue> = (0..count)
        .map(|index| {
            pack::json!({
                ...
                "meshId": OBJ_ANY_EDIT_FALLBACK_MESH_KIND,
...
pub fn render(document: &ObjSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = pack::json_to_string(&pack::json_array([pack::json!({ "id": OBJ_ANY_EDIT_FALLBACK_MESH_KIND, "data": mesh_from_kind(OBJ_ANY_EDIT_FALLBACK_MESH_KIND) })]));
    let view = MeshView {
        camera_json: world3d_camera_json(OBJ_ANY_EDIT_DEFAULT_CAMERA_POSITION, OBJ_ANY_EDIT_DEFAULT_CAMERA_TARGET, OBJ_ANY_EDIT_DEFAULT_CAMERA_FOV),
        meshes_json,
