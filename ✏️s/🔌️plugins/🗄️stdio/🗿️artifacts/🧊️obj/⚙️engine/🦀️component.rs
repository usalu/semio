...
pub fn register() {
    crate::artifacts::obj::io::register();
    register_artifact_schema();
    register_pilot_languages();
    store::register_document_codec(store::DocumentCodec::of::<ObjSnapshot, ObjMutation>(STDIO_OBJ_DOCUMENT_SCHEMA));
}
...
mod tests {
    ...
    fn codec_round_trip() {
        ...
        let parsed = <ObjSnapshot as store::DocumentDsl>::parse_dsl(&text).expect("parse");
        assert_eq!(parsed.schema, snap.schema);
        let bytes = store::DocumentPack::encode_pack(&snap);
        let decoded = <ObjSnapshot as store::DocumentPack>::decode_pack(&bytes).expect("decode");
