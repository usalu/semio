//! 🏗️ TxtBuilder — local ArtifactBuilder until SDK Wave 3.
...
impl ArtifactBuilder for TxtBuilder {
    ...
    fn from_text(text: &str) -> Result<Self, store::TextError> {
        Ok(Self::from_snapshot(<TxtSnapshot as store::DocumentDsl>::parse_dsl(text)?))
    }
    fn from_binary(bytes: &[u8]) -> Result<Self, store::PackError> {
        Ok(Self::from_snapshot(<TxtSnapshot as store::DocumentPack>::decode_pack(bytes)?))
    }
    fn mutate(mut self, mutation: Self::Mutation) -> Self {
        crate::artifacts::txt::schema::mutations::apply_txt_mutation(&mut self.snapshot, &mutation);
        self
    }
