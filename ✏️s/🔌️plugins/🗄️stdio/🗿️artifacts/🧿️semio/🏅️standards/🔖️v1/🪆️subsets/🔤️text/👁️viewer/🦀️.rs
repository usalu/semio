//! 👁️ Semio Text viewer — the read-only counterpart of `✏️editor` for this subset (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `SemioTextViewer`
//! implements `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<SemioTextViewer>`
...
pub enum SemioTextViewCommand {
    #[default]
    Noop,
}

impl protocol::OpBinary for SemioTextViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(SemioTextViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
pub struct SemioTextViewer;

impl ArtifactViewer for SemioTextViewer {
    ...
    type Command = SemioTextViewCommand;
...
pub fn create_semio_text_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(SEMIO_TEXT_DIALECT).document(["stdio", "semio"]).icon_id("box").mode_def(view::definition()).default_mode_id(view::SEMIO_TEXT_VIEW_MODE_ID).window_kind_def(main::definition()).default_layout(view::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn viewer_never_mutates_the_document_or_draft_store() {
        semio_framework_plugin::testkit::assert_viewer_never_mutates::<SemioTextViewer>();
    }
}
//#endregion 🧪️Tests
