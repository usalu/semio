//! 🧪️ `scale` fixture — `📏️scales-the-nested-group-non-uniformly`.
//!
//! Transcribed from `../../🔺️diff/🦀️.rs`: the exact mirror of `rotate` — unresolvable
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📏scale-node/📏️scales-the-nested-group-non-uniformly/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📏scale-node/📏️scales-the-nested-group-non-uniformly/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📏scale-node/📏️scales-the-nested-group-non-uniformly/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📏scale-node/📏️scales-the-nested-group-non-uniformly/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📏scale-node/📏️scales-the-nested-group-non-uniformly/🎯️outcome/🔣️.json");

fn before() -> SemioDrawingSnapshot {
    dsl::json::from_json_str(BEFORE).expect("scale before snapshot decodes")
}
fn expected_after() -> SemioDrawingSnapshot {
    dsl::json::from_json_str(AFTER).expect("scale after snapshot decodes")
}
fn mutation() -> SemioDrawingMutation {
    dsl::json::from_json_str(MUTATION).expect("scale mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(mutation()))).expect("scale mutation encodes");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "scale/scales-the-nested-group-non-uniformly: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical_and_narrowly_scoped() {
    let decoded: SemioDrawingDiff = dsl::json::from_json_str(DIFF).expect("committed scale diff decodes");
    ...
    let crate::standards::v1::subsets::drawing::schema::diff::DrawNodeDiff::Group(node_group) = &children.modified[0].diff else { panic!("scaling a Group must produce the Group arm") };
