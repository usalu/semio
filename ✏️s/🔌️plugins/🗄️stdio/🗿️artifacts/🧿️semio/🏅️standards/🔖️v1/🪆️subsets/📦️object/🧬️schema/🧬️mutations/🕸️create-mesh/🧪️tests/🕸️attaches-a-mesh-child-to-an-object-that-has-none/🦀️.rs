//! 🧪️ `🕸️create-mesh` fixture — `🕸️attaches-a-mesh-child-to-an-object-that-has-none`.
//!
//! Transcribed from `../../🔺️diff/🦀️.rs`: an object that ALREADY has a mesh child is
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕸️create-mesh/🕸️attaches-a-mesh-child-to-an-object-that-has-none/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕸️create-mesh/🕸️attaches-a-mesh-child-to-an-object-that-has-none/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕸️create-mesh/🕸️attaches-a-mesh-child-to-an-object-that-has-none/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕸️create-mesh/🕸️attaches-a-mesh-child-to-an-object-that-has-none/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕸️create-mesh/🕸️attaches-a-mesh-child-to-an-object-that-has-none/🎯️outcome/🔣️.json");

fn before() -> SemioObjectSnapshot {
    dsl::json::from_json_str(BEFORE).expect("🕸️create-mesh before snapshot decodes")
}
fn expected_after() -> SemioObjectSnapshot {
    dsl::json::from_json_str(AFTER).expect("🕸️create-mesh after snapshot decodes")
}
fn mutation() -> SemioObjectMutation {
    dsl::json::from_json_str(MUTATION).expect("🕸️create-mesh mutation decodes")
}
...
async fn the_undo_delete_mesh_detaches_the_handle_again() {
    ...
    assert_eq!(undo, vec![SemioObjectMutation::DeleteMesh(crate::standards::v1::subsets::object::schema::mutations::delete_mesh::DeleteMesh {})], "creating a child into an EMPTY slot must undo as the matching delete, not as another create");
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(mutation()))).expect("🕸️create-mesh mutation encodes");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "🕸️create-mesh/attaches-a-mesh-child-to-an-object-that-has-none: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical() {
    let decoded: SemioObjectDiff = dsl::json::from_json_str(DIFF).expect("committed 🕸️create-mesh diff decodes");
