...
//! defined in ITS OWN aggregate file, not under `✳️any/🧬️schema/🧬️mutations/`, so it cannot serve as
...
mod tests {
    ...
    async fn wrapped_brep_kind_diff_and_inverse_route_correctly() {
        ...
        let m = SemioMutation::ApplyBrep(apply_brep::ApplyBrep { mutation: SemioBrepMutation::CreateVertex(create_vertex::CreateVertex { id: "v1".into(), point: crate::artifacts::semio::standards::v1::subsets::any::schema::geometry::SemioPoint3 { x: 1.0, y: 2.0, z: 3.0 } }) });
        let diff = <SemioMutation as Mutation<SemioSnapshot>>::diff(&m, &base);
