...
fn before() -> SemioGraphSnapshot {
    dsl::json::from_json_str(BEFORE).expect("delete-edge before snapshot decodes")
}
fn expected_after() -> SemioGraphSnapshot {
    dsl::json::from_json_str(AFTER).expect("delete-edge after snapshot decodes")
}
fn mutation() -> SemioGraphMutation {
    dsl::json::from_json_str(MUTATION).expect("delete-edge mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(mutation()))).expect("delete-edge mutation encodes");
...
async fn committed_diff_is_canonical_and_omits_nodes_entirely() {
    let decoded: SemioGraphDiff = dsl::json::from_json_str(DIFF).expect("committed delete-edge diff decodes");
