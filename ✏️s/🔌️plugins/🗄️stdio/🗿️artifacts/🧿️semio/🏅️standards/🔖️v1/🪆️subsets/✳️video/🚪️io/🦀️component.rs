...
pub mod derived_composition {
    ...
    impl SubsetValidator for SemioVideoValidator {
        ...
        async fn validate(payload: &IoPayload) -> Vec<dsl::Diagnostic> {
            let decoded = match payload {
                IoPayload::Binary(bytes) => <SemioVideoSnapshot as store::ArtifactPack>::decode_pack(bytes).await.ok(),
    ...
    mod tests {
        ...
        async fn subset_validator_recheck_agrees_with_direct_invariant_check() {
            ...
            assert!(diagnostics.await.is_empty(), "wire recheck must agree with the direct check for a clean snapshot: {diagnostics:?}");
        }
        ...
        mod conformance_laws {
            ...
            async fn fixture_honesty_law() {
                ...
                let parsed = <snapshot::SemioVideoSnapshot as store::ArtifactDsl>::parse_dsl(FIXTURE_DSL).await.expect("parse shipped .dsl.semio fixture");
                ...
                let decoded = <snapshot::SemioVideoSnapshot as store::ArtifactPack>::decode_pack(FIXTURE_PACK).await.expect("decode shipped .pack.semio fixture");
