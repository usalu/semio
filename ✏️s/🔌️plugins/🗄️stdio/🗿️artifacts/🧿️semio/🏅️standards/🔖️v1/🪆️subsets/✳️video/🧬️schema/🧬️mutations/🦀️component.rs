//! 🧬️ SemioVideoMutation — 🚧 scaffolded by W1b: a single `SetSnapshot` full-replace mutation
...
pub enum SemioVideoMutation {
    ...
    SetSnapshot { snapshot: SemioVideoSnapshot },
}

impl Mutation<SemioVideoSnapshot> for SemioVideoMutation {
    type Diff = SemioVideoDiff;

    fn diff(&self, _base: &SemioVideoSnapshot) -> Self::Diff {
        match self {
            SemioVideoMutation::NoMutation => SemioVideoDiff::default(),
            SemioVideoMutation::SetSnapshot { snapshot } => SemioVideoDiff { replacement: Some(snapshot.clone()) },
        }
    }
...
pub fn apply_semio_video_mutation(snapshot: &mut SemioVideoSnapshot, mutation: &SemioVideoMutation) -> SemioVideoDiff {
    let diff = <SemioVideoMutation as Mutation<SemioVideoSnapshot>>::diff(mutation, snapshot);
    *snapshot = <SemioVideoDiff as protocol::MutationDiff<SemioVideoSnapshot>>::apply(&diff, snapshot);
    diff
}
//#endregion 🔖️Mutation
...
mod tests {
    ...
    fn mutation_diff_law_set_snapshot_matches_diff() {
        let base = SemioVideoSnapshot::default();
        let mut next = SemioVideoSnapshot::default();
        next.schema = format!("{}-mutated", base.schema);
        let mutation = SemioVideoMutation::SetSnapshot { snapshot: next.clone() };
        let diff = <SemioVideoMutation as Mutation<SemioVideoSnapshot>>::diff(&mutation, &base);
        assert_eq!(<SemioVideoDiff as protocol::MutationDiff<SemioVideoSnapshot>>::apply(&diff, &base), next);
        let inv = <SemioVideoMutation as Mutation<SemioVideoSnapshot>>::inverse(&mutation, &base);
        assert_eq!(inv.len(), 1);
        let mut round = next.clone();
        let _ = apply_semio_video_mutation(&mut round, &inv[0]);
    ...
    fn op_text_binary_roundtrip_law() {
        let base = SemioVideoSnapshot::default();
        for m in [SemioVideoMutation::NoMutation, SemioVideoMutation::SetSnapshot { snapshot: base.clone() }] {
