...
//#region 🔖️Diff
pub async fn diff(payload: &RemoveNodeProperty, base: &SemioGraphSnapshot) -> protocol::MutationOutcome<SemioGraphDiff> {
    ...
    if payload.index >= node.properties.len() {
        return protocol::MutationOutcome::error("mutation.target-missing", format!("Node \"{}\" has no property at index {}.", payload.node_id.value, payload.index), [payload.node_id.value.clone(), payload.index.to_string()]).await;
    }
