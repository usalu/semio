//! 🧊 Semio Audio editor — the Main window: the SAME world-3d mesh scene shape as the sibling
...
//! artifact-level `SemioAudioSnapshot` — this window itself never mutates; mutation is the surface
...
const SEMIO_AUDIO_EDIT_FALLBACK_MESH_KIND: &str = "box";
const SEMIO_AUDIO_EDIT_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const SEMIO_AUDIO_EDIT_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const SEMIO_AUDIO_EDIT_DEFAULT_CAMERA_FOV: f64 = 45.0;
...
/// other by design).
async fn entity_count(document: &SemioAudioSnapshot) -> usize {
...
async fn world_instances_json(document: &SemioAudioSnapshot) -> String {
...
pub async fn render(document: &SemioAudioSnapshot) -> UiNode {
    ...
    let view = MeshView {
        camera_json: world3d_camera_json(SEMIO_AUDIO_EDIT_DEFAULT_CAMERA_POSITION, SEMIO_AUDIO_EDIT_DEFAULT_CAMERA_TARGET, SEMIO_AUDIO_EDIT_DEFAULT_CAMERA_FOV).await,
        meshes_json,
