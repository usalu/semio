/** 🔺️ SemioDrawingDiff — mirrors the real Rust 🔺️diff/🦀️component.rs (handcrafted sparse diff,
 * source of truth). Collection triples reuse the shared engine's own TS shape (see
 * `⚙️engine/🧰️triples/🦀️component.rs`'s facet mirrors) — `removed`/`modified`/`added`.
...
// 🧭️ Mutation-level node addressing (`NodePath`) lives in ../🧬️mutations/🟦️component.ts.
