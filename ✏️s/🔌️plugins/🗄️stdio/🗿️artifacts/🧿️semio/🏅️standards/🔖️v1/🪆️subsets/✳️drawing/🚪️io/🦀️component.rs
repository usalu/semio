...
pub mod derived_composition {
    ...
    impl ArtifactComposition for SemioDrawingComposerComposition {
        ...
        async fn compose(sources: &[ComposeSource<'_>]) -> Result<Composition<Self::Snapshot>, ComposeError> {
            ...
            let analysis = SemioDrawingAnalyzer::analyze(&native).await;
    ...
    impl SubsetValidator for SemioDrawingValidator {
        ...
        async fn validate(payload: &IoPayload) -> Vec<dsl::Diagnostic> {
            let decoded = match payload {
                IoPayload::Binary(bytes) => <SemioDrawingSnapshot as store::ArtifactPack>::decode_pack(bytes).await.ok(),
    ...
    mod tests {
        ...
        mod conformance_laws {
            ...
            async fn fixture_honesty_law() {
                const FIXTURE_DSL: &str = include_str!("../../✳️any/📚️examples/🖍️sketch/🖼️assets/🗣️example.dsl.semio");
                const FIXTURE_PACK: &[u8] = include_bytes!("../../✳️any/📚️examples/🖍️sketch/🖼️assets/🎒️example.pack.semio");

                let demo = snapshot::demo_drawing_snapshot();

                let parsed = <snapshot::SemioDrawingSnapshot as store::ArtifactDsl>::parse_dsl(FIXTURE_DSL).await.expect("parse shipped .dsl.semio fixture");
                ...
                let decoded = <snapshot::SemioDrawingSnapshot as store::ArtifactPack>::decode_pack(FIXTURE_PACK).await.expect("decode shipped .pack.semio fixture");
