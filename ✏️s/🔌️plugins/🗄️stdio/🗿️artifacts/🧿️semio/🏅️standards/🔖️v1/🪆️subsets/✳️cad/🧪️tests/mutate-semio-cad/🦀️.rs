...
//! `../../🏅️standards/🔖️v1/🪆️subsets/✳️base/📚️examples/📐️drawing/🖼️assets/🗣️.dsl.semio` and its
...
mod subject {
    ...
    const DSL_ASSET: &str = "asset://📚️examples/📐️drawing/🖼️assets/🗣️.dsl.semio";
    const PACK_ASSET: &str = "asset://📚️examples/📐️drawing/🖼️assets/🎒️.pack.semio";
    ...
    fn snapshot_of(vector: &Json, name: &str) -> Result<SemioCadSnapshot, String> {
        ...
        decode_semio_cad_snapshot_json(&member.to_string())
    }

    fn projection(snapshot: &SemioCadSnapshot) -> Result<Json, String> {
        parse_json(&encode_semio_cad_snapshot_json(snapshot))
    }

    fn disagreement(what: &str, got: &SemioCadSnapshot, expected: &SemioCadSnapshot) -> String {
        format!("{what}\n     got: {}\nexpected: {}", encode_semio_cad_snapshot_json(got), encode_semio_cad_snapshot_json(expected))
    }
    //#endregion 🔖️Fixtures
    ...
    pub fn identity_round_trip(ctx: &Context) -> Result<Outcome, String> {
        ...
        let printed = print_semio_cad_dsl(&parsed);
        carrier_is_exact(printed.as_bytes(), text.as_bytes())?;
        let reparsed = parse_semio_cad_dsl(&printed)?;
        ...
        let repacked_bytes = encode_semio_cad_pack(&parsed);
        carrier_is_exact(&repacked_bytes, &pack_bytes)?;
        let repacked = decode_semio_cad_pack(&repacked_bytes)?;
