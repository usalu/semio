...
impl protocol::MutationKind<SemioGraphSnapshot, SemioGraphMutation> for AddNodePort {
    ...
    fn label(&self) -> protocol::LocalizedLabel {
        protocol::LocalizedLabel::native(&format!("Add port to node \"{}\" at #{}", self.node_id.value, self.index), &format!("Anschluss zu Knoten \"{}\" an #{} hinzufügen", self.node_id.value, self.index))
    }
