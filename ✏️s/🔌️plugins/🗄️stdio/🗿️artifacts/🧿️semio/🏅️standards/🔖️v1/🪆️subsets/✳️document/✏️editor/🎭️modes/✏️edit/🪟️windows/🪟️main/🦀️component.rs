//! 🧊 Semio Document editor — the Main window: the SAME world-3d mesh scene shape as the sibling
...
//! artifact-level `SemioDocumentSnapshot` — this window itself never mutates; mutation is the surface
...
const SEMIO_DOCUMENT_EDIT_FALLBACK_MESH_KIND: &str = "box";
const SEMIO_DOCUMENT_EDIT_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const SEMIO_DOCUMENT_EDIT_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const SEMIO_DOCUMENT_EDIT_DEFAULT_CAMERA_FOV: f64 = 45.0;
...
/// other by design).
async fn entity_count(document: &SemioDocumentSnapshot) -> usize {
...
async fn world_instances_json(document: &SemioDocumentSnapshot) -> String {
    let count = entity_count(document);
    let instances: Vec<serde_json::Value> = (0..count.await)
        .map(|index| {
            serde_json::json!({
                "id": format!("semio_document-{index}"),
                "meshId": SEMIO_DOCUMENT_EDIT_FALLBACK_MESH_KIND,
                "position": [index as f64 * 2.0, 0.0, 0.0],
                "rotation": [0.0, 0.0, 0.0, 1.0],
                "scale": [1.0, 1.0, 1.0],
                "label": format!("Semio Document {index}"),
                "smoothShading": false,
            })
        })
        .collect();
...
pub async fn render(document: &SemioDocumentSnapshot) -> UiNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": SEMIO_DOCUMENT_EDIT_FALLBACK_MESH_KIND, "data": mesh_from_kind(SEMIO_DOCUMENT_EDIT_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(SEMIO_DOCUMENT_EDIT_DEFAULT_CAMERA_POSITION, SEMIO_DOCUMENT_EDIT_DEFAULT_CAMERA_TARGET, SEMIO_DOCUMENT_EDIT_DEFAULT_CAMERA_FOV).await,
        meshes_json,
...
mod tests {
    ...
    async fn render_produces_a_scene_node_for_the_default_document() {
        let document = SemioDocumentSnapshot::default();
