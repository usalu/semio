...
pub fn render(document: &SemioPresentationSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = pack::json_to_string(&pack::JsonValue::from(vec![pack::json_object([
        ("id".to_string(), pack::JsonValue::from(SEMIO_PRESENTATION_VIEW_FALLBACK_MESH_KIND)),
        ("data".to_string(), pack::json_from_dsl_value(&dsl::to_dsl_value(&mesh_from_kind(SEMIO_PRESENTATION_VIEW_FALLBACK_MESH_KIND)).expect("MeshData serializes"))),
    ])]));
    let view = MeshView {
