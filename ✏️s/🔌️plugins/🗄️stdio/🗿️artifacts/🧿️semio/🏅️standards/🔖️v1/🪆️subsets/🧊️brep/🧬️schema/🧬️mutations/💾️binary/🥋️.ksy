...
doc: |
  ...
  fixed `format` byte + `tag` byte (the record tag in `📡️.protocol.semio`), then the variant's own
  ...
  `../📝️text/🔤️.ebnf` for that text's real grammar). Replaces the old whole-enum
