...
//!
//! Transcribed from `../../🔺️diff/🦀️component.rs`: the row lands at `min(index, rows.len())` and
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "insert-row/inserts-a-row-between-the-two-existing-rows: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
