...
mod subject {
    ...
    const GRAPH_DSL: &str = "asset://🕸️graph/🗣️.dsl.semio";
    const GRAPH_PACK: &str = "asset://🕸️graph/🎒️.pack.semio";
    /// 🌲️ The real 424 KB building model, its source and its binary twin.
    const FOREST_JSON: &str = "shared://🔢️mutate-semio-value/🔣️.json";
    const FOREST_DSL: &str = "shared://🔢️mutate-semio-value/🌲️hexagonal-cut-concrete-forest/🗣️.dsl.semio";
    const FOREST_PACK: &str = "shared://🔢️mutate-semio-value/🌲️hexagonal-cut-concrete-forest/🎒️.pack.semio";
    ...
    fn step_fixtures(ctx: &Context) -> Vec<String> {
        ...
        for (_, text) in &ctx.scenario.steps {
            ...
            loop {
                let Some(at) = ["shared://🔢️mutate-semio-value/", "asset://", "shared://"].iter().filter_map(|scheme| rest.find(scheme)).min() else {
                    break;
                };
                let tail = &rest[at..];
