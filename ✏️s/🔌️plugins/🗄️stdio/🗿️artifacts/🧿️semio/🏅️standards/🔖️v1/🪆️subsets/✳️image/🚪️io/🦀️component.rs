...
pub mod derived_composition {
    ...
    impl ArtifactComposition for SemioImageComposerComposition {
        ...
        async fn compose(sources: &[ComposeSource<'_>]) -> Result<Composition<Self::Snapshot>, ComposeError> {
            ...
            let analysis = SemioImageAnalyzer::analyze(&native).await;
    ...
    impl SubsetValidator for SemioImageValidator {
        ...
        async fn validate(payload: &IoPayload) -> Vec<dsl::Diagnostic> {
            let decoded = match payload {
                IoPayload::Binary(bytes) => <SemioImageSnapshot as store::ArtifactPack>::decode_pack(bytes).await.ok(),
                IoPayload::Text(text) => <SemioImageSnapshot as store::ArtifactDsl>::parse_dsl(text).await.ok(),
            };
            match decoded {
                Some(_) => Vec::new(),
    ...
    pub async fn register() {
        ::schema::register_artifact_schema_descriptor(crate::artifacts::semio::standards::v1::subsets::image::schema::semio_image_artifact_schema_descriptor().await);
    ...
    pub async fn register_artifact_inferences() {
        ::schema::register_artifact_inference_descriptor(crate::artifacts::semio::standards::v1::subsets::image::schema::inferences::semio_image_artifact_inference_descriptor().await);
    }
    //#endregion 🔖️Register
