//! 🧊 Semio Brep editor — the Main window: the SAME world-3d mesh scene shape as the sibling
...
//! artifact-level `SemioBrepSnapshot` — this window itself never mutates; mutation is the surface
...
const SEMIO_BREP_EDIT_FALLBACK_MESH_KIND: &str = "box";
const SEMIO_BREP_EDIT_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const SEMIO_BREP_EDIT_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const SEMIO_BREP_EDIT_DEFAULT_CAMERA_FOV: f64 = 45.0;
...
/// other by design).
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
fn entity_count(document: &SemioBrepSnapshot) -> usize {
...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn render(document: &SemioBrepSnapshot) -> BuiltNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": SEMIO_BREP_EDIT_FALLBACK_MESH_KIND, "data": mesh_from_kind(SEMIO_BREP_EDIT_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(SEMIO_BREP_EDIT_DEFAULT_CAMERA_POSITION, SEMIO_BREP_EDIT_DEFAULT_CAMERA_TARGET, SEMIO_BREP_EDIT_DEFAULT_CAMERA_FOV),
        meshes_json,
