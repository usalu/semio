//! 🧊 Semio Text viewer — the Main window: a read-only world-3d mesh scene built with the shared
//! `MeshWindowKit` (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.6), from the
//! artifact-level `SemioTextSnapshot` — this file imports nothing from the sibling editor module
...
const SEMIO_TEXT_VIEW_FALLBACK_MESH_KIND: &str = "box";
const SEMIO_TEXT_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const SEMIO_TEXT_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const SEMIO_TEXT_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
...
async fn entity_count(document: &SemioTextSnapshot) -> usize {
...
/// 👁️ Pure `SemioTextSnapshot -> UiNode` read: default camera (a viewer has no persisted
/// per-session camera — `Config = NoConfig`), no selection/gumball/engagement overlay.
pub async fn render(document: &SemioTextSnapshot) -> UiNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": SEMIO_TEXT_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(SEMIO_TEXT_VIEW_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(SEMIO_TEXT_VIEW_DEFAULT_CAMERA_POSITION, SEMIO_TEXT_VIEW_DEFAULT_CAMERA_TARGET, SEMIO_TEXT_VIEW_DEFAULT_CAMERA_FOV).await,
        meshes_json,
