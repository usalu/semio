//! 📤️ `SemioObjectToCsv` — mirror of `SemioObjectFromCsv`. `root` must be a `List` of rows shaped
...
//#region 🔖️Serializer
pub struct SemioObjectToCsv;

impl ArtifactSerializer for SemioObjectToCsv {
    ...
    fn serialize(from: &Self::From) -> Result<Self::Into, store::PackError> {
        ...
        csv_from_semio(&from.root, &objects, &mut visiting)
    }
}
    ...
    match v {
        SemioValue::Ref { id } => {
            if !visiting.insert(id.clone()) {
                return Err(err(format!("object->csv: reference cycle detected at id {:?}", id.value)));
            }
            let target = *objects.get(id).ok_or_else(|| err(format!("object->csv: dangling Ref{{id: {:?}}} — not found in `objects`", id.value)))?;
...
fn scalar_to_field(v: &SemioValue) -> Result<CsvField, store::PackError> {
    match v {
        ...
        other => Err(err(format!("object->csv: cell value must be a scalar (Str/Int/Float/Bool/Null), got {other:?}"))),
    }
}

pub fn csv_from_semio(root: &SemioValue, objects: &HashMap<&ObjectId, &SemioValue>, visiting: &mut HashSet<ObjectId>) -> Result<CsvSnapshot, store::PackError> {
    let resolved_root = resolve(root, objects, visiting)?;
    let items = match resolved_root {
        SemioValue::List { items } => items,
        other => return Err(err(format!("object->csv: root must be a List of rows, got {other:?}"))),
    };
    if items.is_empty() {
        return Ok(CsvSnapshot { schema: STDIO_CSV_DOCUMENT_SCHEMA.into(), has_header: true, records: Vec::new() });
    }

    let first = resolve(&items[0], objects, visiting)?;
    match first {
        SemioValue::Map { entries: first_entries } => {
            ...
            for item in &items {
                let resolved = resolve(item, objects, visiting)?;
                let entries = match resolved {
                    SemioValue::Map { entries } => entries,
                    other => return Err(err(format!("object->csv: every row must be a Map since the first row was, got {other:?}"))),
                };
                let keys: Vec<String> = entries.iter().map(|e| e.key.clone()).collect();
                if keys != header {
                    return Err(err(format!("object->csv: row key set/order {keys:?} does not match the first row's {header:?} — RFC 4180 has exactly one column set per file")));
                }
                let fields = entries.iter().map(|e| scalar_to_field(&resolve(&e.value, objects, visiting)?)).collect::<Result<Vec<_>, store::PackError>>()?;
        ...
        SemioValue::List { .. } => {
            ...
            for item in &items {
                ...
                let row_items = match resolved {
                    SemioValue::List { items } => items,
                    other => return Err(err(format!("object->csv: every row must be a List since the first row was, got {other:?}"))),
                };
                let fields = row_items.iter().map(|v| scalar_to_field(&resolve(v, objects, visiting)?)).collect::<Result<Vec<_>, store::PackError>>()?;
                records.push(CsvRecord { fields });
            }
            Ok(CsvSnapshot { schema: STDIO_CSV_DOCUMENT_SCHEMA.into(), has_header: false, records })
        }
        other => Err(err(format!("object->csv: each row must be a Map or List, got {other:?}"))),
    }
}
//#endregion 🔖️Convert
...
mod tests {
    use super::*;
    use crate::artifacts::semio::standards::v1::subsets::object::io::import::deserializers::artifacts::csv::v_rfc4180::any::semio_value_from_csv;
    ...
    fn round_trip(snapshot: &CsvSnapshot) -> CsvSnapshot {
        ...
        csv_from_semio(&value, &objects, &mut visiting).expect("object->csv")
    }

    /// 🧪️ Required proof: csv -> object -> csv -> object round trip preserves everything the
    /// object subset can represent (the `quoted` flag excepted — documented lossy field).
    #[test]
    fn csv_to_object_to_csv_to_object_round_trips() {
    ...
    fn mismatched_row_shape_is_a_hard_error() {
        let value = SemioValue::List {
            items: vec![
                SemioValue::Map { entries: vec![crate::artifacts::semio::standards::v1::subsets::object::schema::snapshot::SemioObjectEntry { key: "a".into(), value: SemioValue::Str { value: "1".into() } }] },
                SemioValue::Map { entries: vec![crate::artifacts::semio::standards::v1::subsets::object::schema::snapshot::SemioObjectEntry { key: "b".into(), value: SemioValue::Str { value: "2".into() } }] },
            ],
        };
        let objects = HashMap::new();
        let mut visiting = HashSet::new();
        assert!(csv_from_semio(&value, &objects, &mut visiting).is_err());
    }
