...
pub async fn diff(_payload: &DeleteMesh, base: &SemioObjectSnapshot) -> protocol::MutationOutcome<SemioObjectDiff> {
    if base.mesh.is_none() {
        return protocol::MutationOutcome::error("mutation.target-missing", "The object has no mesh to delete.".to_string(), ["mesh".to_string()]).await;
    }
    protocol::MutationOutcome::new(SemioObjectDiff { mesh: Some(None), ..Default::default() }).await
}
//#endregion 🔖️Diff
