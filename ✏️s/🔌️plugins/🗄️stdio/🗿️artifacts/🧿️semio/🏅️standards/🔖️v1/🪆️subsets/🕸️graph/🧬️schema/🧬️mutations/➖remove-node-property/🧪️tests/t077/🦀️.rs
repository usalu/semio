...
fn expected_after() -> SemioGraphSnapshot {
    dsl::json::from_json_str(AFTER).expect("remove-node-property after snapshot decodes")
}
fn mutation() -> SemioGraphMutation {
    dsl::json::from_json_str(MUTATION).expect("remove-node-property mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(mutation()))).expect("remove-node-property mutation encodes");
...
async fn committed_diff_is_canonical_and_omits_edges_entirely() {
    let decoded: SemioGraphDiff = dsl::json::from_json_str(DIFF).expect("committed remove-node-property diff decodes");
