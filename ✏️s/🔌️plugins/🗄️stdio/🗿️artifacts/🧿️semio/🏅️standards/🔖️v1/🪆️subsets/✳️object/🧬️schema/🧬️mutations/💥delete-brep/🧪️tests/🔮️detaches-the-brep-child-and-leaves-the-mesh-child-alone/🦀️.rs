//! 🧪️ `delete-brep` fixture — `🔮️detaches-the-brep-child-and-leaves-the-mesh-child-alone`.
//!
//! Transcribed from `../../🔺️diff/🦀️.rs`: an object with no brep child is Error
...
fn before() -> SemioObjectSnapshot {
    serde_json::from_str(BEFORE).expect("delete-brep before snapshot decodes")
}
fn expected_after() -> SemioObjectSnapshot {
    serde_json::from_str(AFTER).expect("delete-brep after snapshot decodes")
}
fn mutation() -> SemioObjectMutation {
    serde_json::from_str(MUTATION).expect("delete-brep mutation decodes")
}

/// ▶️ The brep handle is cleared and the sibling mesh handle is deliberately left in place — the
...
async fn clears_the_brep_slot_and_leaves_the_mesh_slot_alone() {
    ...
    assert_eq!(produced.transform, base.transform, "delete-brep must not touch the object's placement");
}
...
/// 🔣️ Snapshots and the empty-payload `{"DeleteBrep":{}}` mutation are canonical — the after-snapshot
/// OMITS the `brep` key entirely (the snapshot field is a plain `Option` with
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "delete-brep/detaches-the-brep-child-and-leaves-the-mesh-child-alone: committed {label} JSON is not canonical");
    }
    let after_json: serde_json::Value = serde_json::from_str(AFTER).expect("after reparses");
    assert!(after_json.get("brep").is_none(), "a cleared snapshot slot is an ABSENT key, never an explicit null");
    let reencoded = serde_json::to_value(mutation()).expect("delete-brep mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("delete-brep mutation reparses");
    assert_eq!(reencoded, original, "delete-brep/detaches-the-brep-child-and-leaves-the-mesh-child-alone: committed mutation JSON is not canonical");
}

/// 🎯️ Declared `applied`: the base really has a brep child, so the `mutation.target-missing`
/// rejection must not fire.
#[semio_framework_async_macros::async_test]
async fn declared_outcome_holds_without_a_target_missing_rejection() {
    let outcome: serde_json::Value = serde_json::from_str(OUTCOME).expect("outcome decodes");
    assert_eq!(outcome.get("status").and_then(serde_json::Value::as_str), Some("applied"), "delete-brep/detaches-the-brep-child-and-leaves-the-mesh-child-alone: this case is declared applied");
    let produced = mutation().diff(&before());
    assert!(produced.messages().is_empty(), "clearing a populated slot must raise no diagnostics");
}

/// 🔺️ The produced delta ENCODES to exactly the committed diff: `{"brep": null}`, and nothing
...
async fn produces_committed_diff() {
    ...
    assert!(matches!(outcome.diff().brep, Some(None)), "the in-memory diff must be Some(None) — write the slot, clear it");
    let produced = serde_json::to_value(outcome.diff()).expect("produced diff encodes");
    let committed: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff decodes");
    assert_eq!(produced, committed, "delete-brep/detaches-the-brep-child-and-leaves-the-mesh-child-alone: produced diff differs from the committed 🔺️diff/🔣️.json");
...
async fn committed_diff_json_pins_the_option_option_collapse() {
    let decoded: SemioObjectDiff = serde_json::from_str(DIFF).expect("committed delete-brep diff decodes");
