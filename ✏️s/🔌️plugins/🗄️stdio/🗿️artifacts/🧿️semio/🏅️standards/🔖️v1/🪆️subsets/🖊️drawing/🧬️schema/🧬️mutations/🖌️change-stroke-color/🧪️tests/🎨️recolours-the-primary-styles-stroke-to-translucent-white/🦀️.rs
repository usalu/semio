//! 🧪️ `change-stroke-color` fixture — `🎨️recolours-the-primary-styles-stroke-to-translucent-white`.
//!
//! Transcribed from `../../🔺️diff/🦀️.rs`: the same three guards as `replace-fill` —
...
fn expected_after() -> SemioDrawingSnapshot {
    dsl::json::from_json_str(AFTER).expect("change-stroke-color after snapshot decodes")
}
fn mutation() -> SemioDrawingMutation {
    dsl::json::from_json_str(MUTATION).expect("change-stroke-color mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(mutation()))).expect("change-stroke-color mutation encodes");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-stroke-color/recolours-the-primary-styles-stroke-to-translucent-white: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical_and_narrowly_scoped() {
    let decoded: SemioDrawingDiff = dsl::json::from_json_str(DIFF).expect("committed change-stroke-color diff decodes");
