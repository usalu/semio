...
impl protocol::OpBinary for SemioCadEditCommand {
    ...
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(SemioCadEditCommand::SetVertex)
    }
}
//#endregion 🔖️Command
...
pub fn create_semio_cad_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(SEMIO_CAD_DIALECT).document(["stdio", "semio"]).icon_id("box").mode_def(edit::definition()).default_mode_id(edit::SEMIO_CAD_EDIT_MODE_ID).window_kind_def(main::definition()).default_layout(edit::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn create_editor_builds_a_definition_for_the_editor_role() {
        let def = create_semio_cad_editor();
    ...
    async fn editor_dialect_matches_the_artifact_coordinate() {
        assert_eq!(<SemioCadEditor as ArtifactEditor>::DIALECT, SEMIO_CAD_DIALECT);
    }

    #[semio_framework_async_macros::async_test]
    async fn editor_and_viewer_share_one_dialect() {
        semio_framework_plugin::testkit::assert_editor_and_viewer_share_dialect::<SemioCadEditor, crate::viewer::semio_cad::SemioCadViewer>().await;
    }
}
//#endregion 🧪️Tests
