...
const FIXTURE_DIRECTORY_BY_KIND: &[(&str, &str)] = &[("add-layer", "🗂️add-layer-applied")];
...
fn main() {
    ...
    for kind in KINDS {
        ...
        let fixture_directory = FIXTURE_DIRECTORY_BY_KIND.iter().find_map(|(registered_kind, directory)| (*registered_kind == *kind).then_some(*directory)).expect("every kind has one reviewed fixture directory");
        let dir = PathBuf::from(&out_root).join(fixture_directory);
        fs::create_dir_all(&dir).expect("mkdir");
        fs::write(dir.join("⬅️before.json"), &base_bytes).expect("write reviewed before carrier");
        fs::write(dir.join("➡️after.json"), &mutated_bytes).expect("write reviewed after carrier");
        written += 1;
