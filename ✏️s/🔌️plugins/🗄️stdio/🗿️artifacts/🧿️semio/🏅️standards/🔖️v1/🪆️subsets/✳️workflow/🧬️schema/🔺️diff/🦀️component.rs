//! 🔺️ SemioWorkflowDiff — handcrafted sparse diff over `SemioWorkflowSnapshot`
//! (`nodes: Vec<WorkflowNode>` + `edges: Vec<WorkflowEdge>`, both id-keyed). No
//! `snapshot: Option<SemioWorkflowSnapshot>` full-replace slot — even `SetSnapshot`'s diff is the
//! sparse field-by-field `SemioWorkflowDiff::between(base, next)`.
//!
...
//! `WorkflowNodeDiff` and `from`/`to: Option<PortRef>` on `WorkflowEdgeDiff` are whole-value-
...
use crate::artifacts::semio::standards::v1::subsets::workflow::schema::snapshot::{
    PortRef, SemioWorkflowSnapshot, WorkflowEdge, WorkflowNode, WorkflowParam,
};
...
pub type WorkflowParamsDiff = NamedTripleDiff<String, WorkflowParamDiff, WorkflowParam>;
pub type WorkflowNodesDiff = NamedTripleDiff<String, WorkflowNodeDiff, WorkflowNode>;
pub type WorkflowEdgesDiff = NamedTripleDiff<String, WorkflowEdgeDiff, WorkflowEdge>;

#[derive(Clone, Debug, Default, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowParamDiff {
...
#[derive(Clone, Debug, Default, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct WorkflowNodeDiff {
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub kind: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub label: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub params: Option<WorkflowParamsDiff>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
//#region 🔖️Diff
/// 🔺️ Diff for `s.stdio.semio.workflow`.
#[derive(Clone, Debug, Default, PartialEq, Serialize, Deserialize, ArtifactSchema)]
#[serde(rename_all = "camelCase")]
#[artifact_schema(id = "s.stdio.semio.workflow.diff")]
pub struct SemioWorkflowDiff {
    #[state(persistent)]
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub nodes: Option<WorkflowNodesDiff>,
    #[state(persistent)]
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub edges: Option<WorkflowEdgesDiff>,
}
//#endregion 🔖️Diff
...
//#region 🔖️ParamLogic
fn diff_param(old: &WorkflowParam, new: &WorkflowParam) -> Option<WorkflowParamDiff> {
    if old == new {
        return None;
    }
    Some(WorkflowParamDiff { value: (old.value != new.value).then(|| new.value.clone()) })
}
fn apply_param(param: &mut WorkflowParam, diff: &WorkflowParamDiff) {
    if let Some(v) = &diff.value {
        param.value = v.clone();
    }
}
fn inverse_param(base: &WorkflowParam, diff: &WorkflowParamDiff) -> WorkflowParamDiff {
    WorkflowParamDiff { value: diff.value.as_ref().map(|_| base.value.clone()) }
}
fn absorb_param_diff(mut a: WorkflowParamDiff, b: WorkflowParamDiff) -> WorkflowParamDiff {
    if b.value.is_some() {
        a.value = b.value;
    }
    a
}
fn diff_params(old: &[WorkflowParam], new: &[WorkflowParam]) -> Option<WorkflowParamsDiff> {
    between_named(old, new, |p| p.key.clone(), diff_param)
}
//#endregion 🔖️ParamLogic

//#region 🔖️NodeLogic
fn diff_node(old: &WorkflowNode, new: &WorkflowNode) -> Option<WorkflowNodeDiff> {
    if old == new {
        return None;
    }
    ...
    if kind.is_none() && label.is_none() && params.is_none() && position.is_none() { None } else { Some(WorkflowNodeDiff { kind, label, params, position }) }
}

fn apply_node(node: &mut WorkflowNode, diff: &WorkflowNodeDiff) {
...
fn inverse_node(base: &WorkflowNode, diff: &WorkflowNodeDiff) -> WorkflowNodeDiff {
    WorkflowNodeDiff {
...
fn absorb_node_diff(mut a: WorkflowNodeDiff, b: WorkflowNodeDiff) -> WorkflowNodeDiff {
    if b.kind.is_some() {
        a.kind = b.kind;
    }
    if b.label.is_some() {
...
fn diff_nodes(old: &[WorkflowNode], new: &[WorkflowNode]) -> Option<WorkflowNodesDiff> {
    between_named(old, new, |n| n.id.clone(), diff_node)
}
//#endregion 🔖️NodeLogic

//#region 🔖️EdgeLogic
fn diff_edge(old: &WorkflowEdge, new: &WorkflowEdge) -> Option<WorkflowEdgeDiff> {
    if old == new {
        return None;
    }
    ...
    let kind = (old.kind != new.kind).then(|| new.kind.clone());
    if from.is_none() && to.is_none() && kind.is_none() { None } else { Some(WorkflowEdgeDiff { from, to, kind }) }
}

fn apply_edge(edge: &mut WorkflowEdge, diff: &WorkflowEdgeDiff) {
...
fn inverse_edge(base: &WorkflowEdge, diff: &WorkflowEdgeDiff) -> WorkflowEdgeDiff {
    WorkflowEdgeDiff {
...
fn absorb_edge_diff(mut a: WorkflowEdgeDiff, b: WorkflowEdgeDiff) -> WorkflowEdgeDiff {
    if b.from.is_some() {
        a.from = b.from;
    }
    if b.to.is_some() {
...
fn diff_edges(old: &[WorkflowEdge], new: &[WorkflowEdge]) -> Option<WorkflowEdgesDiff> {
...
impl DiffAlgebra<SemioWorkflowSnapshot> for SemioWorkflowDiff {
    ...
    fn between(base: &SemioWorkflowSnapshot, other: &SemioWorkflowSnapshot) -> Self {
        SemioWorkflowDiff { nodes: diff_nodes(&base.nodes, &other.nodes), edges: diff_edges(&base.edges, &other.edges) }
    }
...
/// `snapshot: Option<SemioWorkflowSnapshot>` full-replace slot — this IS `SemioWorkflowDiff::between`.
pub fn diff_set_snapshot(base: &SemioWorkflowSnapshot, next: &SemioWorkflowSnapshot) -> SemioWorkflowDiff {
    SemioWorkflowDiff::between(base, next)
}

pub fn diff_insert_node(node: WorkflowNode) -> SemioWorkflowDiff {
    SemioWorkflowDiff { nodes: Some(WorkflowNodesDiff { added: vec![node], ..Default::default() }), edges: None }
}
pub fn diff_remove_node(id: &str) -> SemioWorkflowDiff {
    SemioWorkflowDiff { nodes: Some(WorkflowNodesDiff { removed: vec![id.to_string()], ..Default::default() }), edges: None }
}
pub fn diff_set_node_kind(id: &str, kind: &str) -> SemioWorkflowDiff {
    let d = WorkflowNodeDiff { kind: Some(kind.to_string()), ..Default::default() };
    SemioWorkflowDiff { nodes: Some(WorkflowNodesDiff { modified: vec![NamedModified { key: id.to_string(), diff: d }], ..Default::default() }), edges: None }
}
pub fn diff_set_node_label(id: &str, label: &str) -> SemioWorkflowDiff {
    let d = WorkflowNodeDiff { label: Some(label.to_string()), ..Default::default() };
    SemioWorkflowDiff { nodes: Some(WorkflowNodesDiff { modified: vec![NamedModified { key: id.to_string(), diff: d }], ..Default::default() }), edges: None }
}
pub fn diff_set_node_position(id: &str, position: SemioPoint2) -> SemioWorkflowDiff {
    let d = WorkflowNodeDiff { position: Some(position), ..Default::default() };
    SemioWorkflowDiff { nodes: Some(WorkflowNodesDiff { modified: vec![NamedModified { key: id.to_string(), diff: d }], ..Default::default() }), edges: None }
}
/// 🧩 Upserts one param on node `id` — a `WorkflowParamsDiff` `modified` entry if `key` already
/// exists on that node, an `added` entry (full `WorkflowParam`) otherwise.
pub fn diff_set_node_param(base: &SemioWorkflowSnapshot, id: &str, key: &str, value: &str) -> SemioWorkflowDiff {
    let Some(node) = base.nodes.iter().find(|n| n.id == id) else { return SemioWorkflowDiff::default() };
    let params_diff = match node.params.iter().find(|p| p.key == key) {
        Some(existing) if existing.value == value => return SemioWorkflowDiff::default(),
        Some(_) => WorkflowParamsDiff { modified: vec![NamedModified { key: key.to_string(), diff: WorkflowParamDiff { value: Some(value.to_string()) } }], ..Default::default() },
        None => WorkflowParamsDiff { added: vec![WorkflowParam { key: key.to_string(), value: value.to_string() }], ..Default::default() },
    };
    let node_diff = WorkflowNodeDiff { params: Some(params_diff), ..Default::default() };
    SemioWorkflowDiff { nodes: Some(WorkflowNodesDiff { modified: vec![NamedModified { key: id.to_string(), diff: node_diff }], ..Default::default() }), edges: None }
}
pub fn diff_remove_node_param(id: &str, key: &str) -> SemioWorkflowDiff {
    let params_diff = WorkflowParamsDiff { removed: vec![key.to_string()], ..Default::default() };
    let node_diff = WorkflowNodeDiff { params: Some(params_diff), ..Default::default() };
    SemioWorkflowDiff { nodes: Some(WorkflowNodesDiff { modified: vec![NamedModified { key: id.to_string(), diff: node_diff }], ..Default::default() }), edges: None }
}
pub fn diff_insert_edge(edge: WorkflowEdge) -> SemioWorkflowDiff {
    SemioWorkflowDiff { nodes: None, edges: Some(WorkflowEdgesDiff { added: vec![edge], ..Default::default() }) }
}
pub fn diff_remove_edge(id: &str) -> SemioWorkflowDiff {
    SemioWorkflowDiff { nodes: None, edges: Some(WorkflowEdgesDiff { removed: vec![id.to_string()], ..Default::default() }) }
}
pub fn diff_set_edge_endpoints(id: &str, from: PortRef, to: PortRef) -> SemioWorkflowDiff {
    let d = WorkflowEdgeDiff { from: Some(from), to: Some(to), kind: None };
    SemioWorkflowDiff { nodes: None, edges: Some(WorkflowEdgesDiff { modified: vec![NamedModified { key: id.to_string(), diff: d }], ..Default::default() }) }
}
pub fn diff_set_edge_kind(id: &str, kind: &str) -> SemioWorkflowDiff {
    let d = WorkflowEdgeDiff { from: None, to: None, kind: Some(kind.to_string()) };
    SemioWorkflowDiff { nodes: None, edges: Some(WorkflowEdgesDiff { modified: vec![NamedModified { key: id.to_string(), diff: d }], ..Default::default() }) }
}
...
pub(crate) fn enc_param(p: &WorkflowParam) -> String {
    format!("[{},{}]", enc_str(&p.key), enc_str(&p.value))
}
pub(crate) fn dec_param(s: &str) -> Result<WorkflowParam, String> {
    let inner = strip_brackets(s)?;
    let parts = split_top_level(inner, ',');
...
pub(crate) fn enc_node(n: &WorkflowNode) -> String {
    format!(
        "[{},{},{},{},{}]",
        enc_str(&n.id),
        enc_str(&n.kind),
...
pub(crate) fn dec_node(s: &str) -> Result<WorkflowNode, String> {
    let inner = strip_brackets(s)?;
    let parts = split_top_level(inner, ',');
...
pub(crate) fn enc_edge(e: &WorkflowEdge) -> String {
    format!("[{},{},{},{}]", enc_str(&e.id), enc_port_ref(&e.from), enc_port_ref(&e.to), enc_str(&e.kind))
}
pub(crate) fn dec_edge(s: &str) -> Result<WorkflowEdge, String> {
    let inner = strip_brackets(s)?;
    let parts = split_top_level(inner, ',');
...
fn enc_param_diff(d: &WorkflowParamDiff) -> String {
    format!("[{}]", encode_option(&d.value, |v| enc_str(v)))
}
fn dec_param_diff(s: &str) -> Result<WorkflowParamDiff, String> {
    let inner = strip_brackets(s)?;
    Ok(WorkflowParamDiff { value: decode_option(inner, dec_str)? })
}
fn enc_params_diff(d: &WorkflowParamsDiff) -> String { enc_named_triple(d, |k| enc_str(k), enc_param_diff, enc_param) }
fn dec_params_diff(s: &str) -> Result<WorkflowParamsDiff, String> { dec_named_triple(s, dec_str, dec_param_diff, dec_param) }

fn enc_node_diff(d: &WorkflowNodeDiff) -> String {
    format!(
        "[{},{},{},{}]",
...
fn dec_node_diff(s: &str) -> Result<WorkflowNodeDiff, String> {
    let inner = strip_brackets(s)?;
    let parts = split_top_level(inner, ',');
    let [kind, label, params, position] = parts.as_slice() else { return Err(format!("node diff: expected 4 fields, got {}", parts.len())) };
    Ok(WorkflowNodeDiff { kind: decode_option(kind, dec_str)?, label: decode_option(label, dec_str)?, params: decode_option(params, dec_params_diff)?, position: decode_option(position, dec_point2)? })
}
fn enc_nodes_diff(d: &WorkflowNodesDiff) -> String { enc_named_triple(d, |k| enc_str(k), enc_node_diff, enc_node) }
fn dec_nodes_diff(s: &str) -> Result<WorkflowNodesDiff, String> { dec_named_triple(s, dec_str, dec_node_diff, dec_node) }

fn enc_edge_diff(d: &WorkflowEdgeDiff) -> String {
    format!("[{},{},{}]", encode_option(&d.from, |v| enc_port_ref(v)), encode_option(&d.to, |v| enc_port_ref(v)), encode_option(&d.kind, |v| enc_str(v)))
}
fn dec_edge_diff(s: &str) -> Result<WorkflowEdgeDiff, String> {
    let inner = strip_brackets(s)?;
    let parts = split_top_level(inner, ',');
    let [from, to, kind] = parts.as_slice() else { return Err(format!("edge diff: expected 3 fields, got {}", parts.len())) };
    Ok(WorkflowEdgeDiff { from: decode_option(from, dec_port_ref)?, to: decode_option(to, dec_port_ref)?, kind: decode_option(kind, dec_str)? })
}
fn enc_edges_diff(d: &WorkflowEdgesDiff) -> String { enc_named_triple(d, |k| enc_str(k), enc_edge_diff, enc_edge) }
fn dec_edges_diff(s: &str) -> Result<WorkflowEdgesDiff, String> { dec_named_triple(s, dec_str, dec_edge_diff, dec_edge) }
//#endregion 🔖️DiffValueCodecs

//#region 🔖️TopLevel
fn print_workflow_diff(d: &SemioWorkflowDiff) -> String {
...
fn parse_workflow_diff(line: &str) -> Result<SemioWorkflowDiff, String> {
    let mut d = SemioWorkflowDiff::default();
    if line.is_empty() {
        return Ok(d);
    }
    for token in line.split(' ') {
        ...
        else { return Err(format!("workflow diff: unknown token {token:?}")); }
    }
    Ok(d)
}

impl protocol::DiffCodec for SemioWorkflowDiff {
    fn print_diff(&self) -> String {
        print_workflow_diff(self)
    }
...
mod tests {
    ...
    fn base_snapshot() -> SemioWorkflowSnapshot {
    ...
    fn sweep_a() -> SemioWorkflowSnapshot {
        SemioWorkflowSnapshot {
            schema: crate::artifacts::semio::standards::v1::subsets::workflow::schema::snapshot::STDIO_SEMIOWORKFLOW_DOCUMENT_SCHEMA.into(),
            nodes: vec![
    ...
    fn sweep_b() -> SemioWorkflowSnapshot {
        SemioWorkflowSnapshot {
            schema: crate::artifacts::semio::standards::v1::subsets::workflow::schema::snapshot::STDIO_SEMIOWORKFLOW_DOCUMENT_SCHEMA.into(),
            nodes: vec![
    ...
    fn between_roundtrip_law() {
        let a = sweep_a();
        let b = sweep_b();
        assert_eq!(MutationDiff::apply(&<SemioWorkflowDiff as DiffAlgebra<SemioWorkflowSnapshot>>::between(&a, &b), &a), b);
        assert_eq!(MutationDiff::apply(&<SemioWorkflowDiff as DiffAlgebra<SemioWorkflowSnapshot>>::between(&b, &a), &b), a);

        let sample = base_snapshot();
        assert_eq!(MutationDiff::apply(&<SemioWorkflowDiff as DiffAlgebra<SemioWorkflowSnapshot>>::between(&sample, &sample), &sample), sample);
    }
    ...
    fn field_sweep() {
        ...
        let diff_ab = <SemioWorkflowDiff as DiffAlgebra<SemioWorkflowSnapshot>>::between(&a, &b);
        ...
        let diff_ba = <SemioWorkflowDiff as DiffAlgebra<SemioWorkflowSnapshot>>::between(&b, &a);
        assert_eq!(MutationDiff::apply(&diff_ba, &b), a);
    }
    //#endregion 🔖️FieldSweep

    //#region 🔖️AbsorbLaw
    fn assert_absorb_matches_sequential(base: &SemioWorkflowSnapshot, d1: &SemioWorkflowDiff, d2: &SemioWorkflowDiff) -> SemioWorkflowDiff {
    ...
    fn diff_codec_text_binary_roundtrip_law() {
        ...
        for d in diffs {
            ...
            let parsed = SemioWorkflowDiff::parse_diff(&printed).unwrap_or_else(|e| panic!("parse_diff({printed:?}) failed: {e}"));
            ...
            let decoded = SemioWorkflowDiff::decode_diff(&encoded).unwrap_or_else(|e| panic!("decode_diff failed: {e}"));
