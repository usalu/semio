//! 🧪️ `change-run-language` fixture — `🇩🇪retags-the-second-run-as-german`.
//!
...
fn before() -> SemioTextSnapshot {
    dsl::json::from_json_str(BEFORE).expect("change-run-language before snapshot decodes")
}
fn expected_after() -> SemioTextSnapshot {
    dsl::json::from_json_str(AFTER).expect("change-run-language after snapshot decodes")
}
fn change_run_language() -> SemioTextMutation {
    dsl::json::from_json_str(MUTATION).expect("change-run-language mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(change_run_language()))).expect("change-run-language mutation encodes");
...
async fn committed_diff_is_canonical() {
    let decoded: SemioTextDiff = dsl::json::from_json_str(DIFF).expect("committed change-run-language diff decodes");
