//! 🧪️ `flatten` fixture — `flattens-an-identity-nested-group-into-its-leaves`.
//!
//! Transcribed from `../../🔺️diff/🦀️component.rs` and its `collect_flattened_leaves` helper:
...
async fn the_undo_unflatten_carries_the_captured_hierarchy() {
    ...
    let SemioDrawingMutation::Unflatten(restore) = &undo[0] else { panic!("flatten must undo as unflatten") };
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "flatten/flattens-an-identity-nested-group-into-its-leaves: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
