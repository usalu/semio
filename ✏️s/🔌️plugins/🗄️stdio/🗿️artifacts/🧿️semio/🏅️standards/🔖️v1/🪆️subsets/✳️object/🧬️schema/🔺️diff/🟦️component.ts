/** 🔺️ SemioObjectDiff facet mirror — the `🦀️component.rs` sibling is the real source of truth;
 * this interface tracks its fields 1:1 (see `POLICY_FACET_MIRROR_DRIFT`). */
import type { ObjectId, SemioObjectEntry, SemioObjectNode, SemioValue } from "../📸️snapshot/🟦️component";
...
export type SemioValueDiff =
  ...
  | { kind: "map"; diff: NamedTripleDiff<string, SemioValueDiff, NamedAdded<SemioObjectEntry>> }
  | { kind: "ref"; id: ObjectId };

export interface SemioObjectDiff {
  /** @state persistent */ root?: SemioValueDiff;
  /** @state persistent */ objects?: NamedTripleDiff<ObjectId, SemioValueDiff, NamedAdded<SemioObjectNode>>;
}
