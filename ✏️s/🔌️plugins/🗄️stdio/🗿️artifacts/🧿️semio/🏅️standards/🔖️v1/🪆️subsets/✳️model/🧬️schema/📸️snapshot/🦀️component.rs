...
async fn dec_spatial_kind(s: &str) -> Result<SpatialKind, String> {
    match s {
...
async fn enc_element_class(c: &ElementClass) -> String {
    match c {
...
async fn dec_element_class(s: &str) -> Result<ElementClass, String> {
    match s {
...
async fn enc_geometry_ref(g: &GeometryRef) -> String {
    match g {
...
async fn dec_geometry_ref(s: &str) -> Result<GeometryRef, String> {
    if s == "N" {
...
async fn enc_pset_value(v: &PsetValue) -> String {
    match v {
...
async fn enc_property(p: &Property) -> String {
...
async fn dec_relation_kind(s: &str) -> Result<RelationKind, String> {
    match s {
...
async fn enc_relation(r: &ModelRelation) -> String {
...
/// readable.
async fn print_semio_model_snapshot_body(s: &SemioModelSnapshot) -> String {
    format!(
        "schema={}\nspatial=[{}]\nelements=[{}]\nrelations=[{}]",
        enc_str(&s.schema),
...
async fn parse_semio_model_snapshot_body(body: &str) -> Result<SemioModelSnapshot, String> {
    let mut schema = None;
...
async fn write_spatial_kind(out: &mut Vec<u8>, k: &SpatialKind) {
    out.push(match k {
        SpatialKind::Site => 0,
...
async fn write_element_class(out: &mut Vec<u8>, c: &ElementClass) {
    match c {
...
async fn write_geometry_ref(out: &mut Vec<u8>, g: &GeometryRef) {
    match g {
...
async fn write_pset_value(out: &mut Vec<u8>, v: &PsetValue) {
    match v {
...
async fn write_property(out: &mut Vec<u8>, p: &Property) {
...
async fn write_property_set(out: &mut Vec<u8>, ps: &PropertySet) {
...
async fn write_spatial_node(out: &mut Vec<u8>, n: &SpatialNode) {
...
async fn write_element(out: &mut Vec<u8>, e: &SemioModelElement) {
...
async fn write_relation_kind(out: &mut Vec<u8>, k: &RelationKind) {
    match k {
...
async fn write_relation(out: &mut Vec<u8>, r: &ModelRelation) {
...
async fn encode_model_snapshot_binary(s: &SemioModelSnapshot) -> Vec<u8> {
...
/// `📚️examples/🏢️building/🖼️assets/🗣️example.dsl.semio`/`🎒️example.pack.semio` and for the
/// conformance-law tests in `🎹️composer/🦀️component.rs`.
#[cfg(test)]
pub(crate) async fn demo_semio_model_snapshot() -> SemioModelSnapshot {
    SemioModelSnapshot {
