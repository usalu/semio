...
Feature: Apply every typed semio ANIMATION mutation to the real committed walk cycle, against an independent Python implementation
  ...
  `semio-animation-python-independent` in `…/🎞️animation/🔮️oracles/🔣️.json`; the recorded
  ...
  Scenario Outline: Apply <id> to its committed specification vector
    ...
    Examples:
      ...
      | remove-channel | 🗑️remove-channel |
      | set-channel-target | 🎯set-channel-target |
      | set-channel-interpolation | 📈set-channel-interpolation |
      | insert-keyframe | 🔑insert-keyframe |
      | remove-keyframe | 🔓remove-keyframe |
      | set-keyframe-time | 🕐set-keyframe-time |
      | set-keyframe-value | 🔢set-keyframe-value |
