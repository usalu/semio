//! 👁️ Semio Kit viewer — the read-only counterpart of `✏️editor` for this subset (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `SemioKitViewer`
//! implements `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<SemioKitViewer>`
...
use crate::viewer::semio_kit::modes::view;
use crate::viewer::semio_kit::modes::view::windows::main;
...
pub enum SemioKitViewCommand {
    #[default]
    Noop,
}

impl protocol::OpBinary for SemioKitViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(SemioKitViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
pub struct SemioKitViewer;

impl ArtifactViewer for SemioKitViewer {
    ...
    type Command = SemioKitViewCommand;
...
pub fn create_semio_kit_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(SEMIO_KIT_DIALECT).document(["stdio", "semio"]).icon_id("box").mode_def(view::definition()).default_mode_id(view::SEMIO_KIT_VIEW_MODE_ID).window_kind_def(main::definition()).default_layout(view::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn viewer_never_mutates_the_document_or_draft_store() {
        semio_framework_plugin::testkit::assert_viewer_never_mutates::<SemioKitViewer>();
    }
}
//#endregion 🧪️Tests
