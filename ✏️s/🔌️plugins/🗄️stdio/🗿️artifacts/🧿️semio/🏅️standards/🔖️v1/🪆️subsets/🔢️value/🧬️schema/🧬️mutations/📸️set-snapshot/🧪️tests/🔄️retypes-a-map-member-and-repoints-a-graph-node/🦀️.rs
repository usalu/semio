//! 🧪️ `📸️set-snapshot` fixture — `🔄️retypes-a-map-member-and-repoints-a-graph-node`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📸️set-snapshot/🔄️retypes-a-map-member-and-repoints-a-graph-node/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📸️set-snapshot/🔄️retypes-a-map-member-and-repoints-a-graph-node/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📸️set-snapshot/🔄️retypes-a-map-member-and-repoints-a-graph-node/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📸️set-snapshot/🔄️retypes-a-map-member-and-repoints-a-graph-node/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📸️set-snapshot/🔄️retypes-a-map-member-and-repoints-a-graph-node/🎯️outcome/🔣️.json");

fn before() -> SemioValueSnapshot {
    dsl::json::from_json_str(BEFORE).expect("before value-graph snapshot decodes")
}
fn expected_after() -> SemioValueSnapshot {
    dsl::json::from_json_str(AFTER).expect("after value-graph snapshot decodes")
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: SemioValueSnapshot = dsl::json::from_json_str(text).expect("value-graph snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("value-graph snapshot encodes");
...
async fn produces_committed_diff() {
    ...
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(outcome.diff())).expect("produced value-graph diff encodes");
...
async fn committed_diff_is_canonical() {
    let decoded: SemioValueTreeDiff = dsl::json::from_json_str(DIFF).expect("committed value-graph diff decodes");
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("value-graph diff re-encodes");
