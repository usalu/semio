//! 🧪️ `remove-run` fixture — `removes-the-middle-run`.
//!
//! Transcribed from `../../🔺️diff/🦀️component.rs`: `remove-run` rejects an out-of-range BASE
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "remove-run/removes-the-middle-run: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
