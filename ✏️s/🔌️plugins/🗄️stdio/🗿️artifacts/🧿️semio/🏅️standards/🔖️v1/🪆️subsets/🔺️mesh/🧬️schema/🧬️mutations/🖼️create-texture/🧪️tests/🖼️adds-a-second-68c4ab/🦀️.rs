...
fn before() -> SemioMeshSnapshot {
    dsl::json::from_json_str(BEFORE).expect("create-texture before snapshot decodes")
}
fn expected_after() -> SemioMeshSnapshot {
    dsl::json::from_json_str(AFTER).expect("create-texture after snapshot decodes")
}
fn mutation() -> SemioMeshMutation {
    dsl::json::from_json_str(MUTATION).expect("create-texture mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(mutation()))).expect("create-texture mutation encodes");
...
async fn committed_diff_is_canonical_and_narrowly_scoped() {
    let decoded: SemioMeshDiff = dsl::json::from_json_str(DIFF).expect("committed create-texture diff decodes");
