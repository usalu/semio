...
pub async fn diff(payload: &CreatePrimitive, base: &SemioMeshSnapshot) -> protocol::MutationOutcome<SemioMeshDiff> {
    let Some(mesh) = crate::artifacts::semio::standards::v1::subsets::mesh::schema::diff::mesh_at(base, &payload.mesh_id).await else {
        return protocol::MutationOutcome::error("mutation.target-missing", format!("Mesh \"{}\" does not exist.", payload.mesh_id), [payload.mesh_id.clone()]).await;
    };
    if mesh.primitives.iter().any(|p| p.id == payload.primitive.id) {
        return protocol::MutationOutcome::fatal("mutation.duplicate-id", format!("Primitive \"{}\" already exists in mesh \"{}\".", payload.primitive.id, payload.mesh_id), [payload.primitive.id.clone()]).await;
    }
