...
fn before() -> SemioDrawingSnapshot {
    dsl::json::from_json_str(BEFORE).expect("flatten before snapshot decodes")
}
fn expected_after() -> SemioDrawingSnapshot {
    dsl::json::from_json_str(AFTER).expect("flatten after snapshot decodes")
}
fn mutation() -> SemioDrawingMutation {
    dsl::json::from_json_str(MUTATION).expect("flatten mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(mutation()))).expect("flatten mutation encodes");
...
async fn committed_diff_is_canonical_and_narrowly_scoped() {
    let decoded: SemioDrawingDiff = dsl::json::from_json_str(DIFF).expect("committed flatten diff decodes");
    ...
    assert!(matches!(children.modified[0].diff, crate::standards::v1::subsets::drawing::schema::diff::DrawNodeDiff::Replace { .. }), "a hierarchy change goes through Replace, never through the per-field Group arm");
