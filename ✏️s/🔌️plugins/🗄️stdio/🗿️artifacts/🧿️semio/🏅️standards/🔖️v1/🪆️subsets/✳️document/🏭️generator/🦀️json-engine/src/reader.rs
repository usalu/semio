//! 🔍️ The READER: projects a committed semio_document JSON carrier through `serde_json` and compares two of
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        Some("project") => match read(args.get(2).expect("usage: reader project <file.json>")) {
            ...
            Err(error) => {
                println!("{}", report("semio-document-json-project", "failed", json!({"error": error})));
                exit(1);
            }
        },
        Some("compare") => {
            ...
            match (read(expected), read(actual)) {
                (Ok(left), Ok(right)) => println!("{}", report("semio-document-json-compare", "ok", json!({"equal": left == right, "expected": left, "actual": right}))),
                (Err(error), _) | (_, Err(error)) => {
                    println!("{}", report("semio-document-json-compare", "failed", json!({"error": error})));
                    exit(1);
                }
            }
        }
