...
pub async fn diff(payload: &DeleteVertex, base: &SemioBrepSnapshot) -> protocol::MutationOutcome<SemioBrepDiff> {
    if !base.vertices.iter().any(|v| v.id == payload.id) {
        return protocol::MutationOutcome::error("mutation.target-missing", format!("Vertex \"{}\" does not exist.", payload.id), [payload.id.clone()]).await;
    }
    ...
    if dependent_edges.is_empty() {
        ...
        outcome.info("mutation.cascade", format!("Deleting vertex \"{}\" also removed {} connected edge(s): {}.", payload.id, dependent_edges.len(), dependent_edges.join(", "))).await
    }
}
//#endregion 🔖️Diff
