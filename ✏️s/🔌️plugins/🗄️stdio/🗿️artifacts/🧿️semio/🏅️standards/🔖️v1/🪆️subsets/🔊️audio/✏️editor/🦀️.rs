...
impl protocol::OpBinary for SemioAudioEditCommand {
    ...
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(SemioAudioEditCommand::SetVertex)
    }
}
//#endregion 🔖️Command
...
pub fn create_semio_audio_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(SEMIO_AUDIO_DIALECT).document(["stdio", "semio"]).icon_id("box").mode_def(edit::definition()).default_mode_id(edit::SEMIO_AUDIO_EDIT_MODE_ID).window_kind_def(main::definition()).default_layout(edit::layout()).build_definition()
}
//#endregion 🔖️Manifest
