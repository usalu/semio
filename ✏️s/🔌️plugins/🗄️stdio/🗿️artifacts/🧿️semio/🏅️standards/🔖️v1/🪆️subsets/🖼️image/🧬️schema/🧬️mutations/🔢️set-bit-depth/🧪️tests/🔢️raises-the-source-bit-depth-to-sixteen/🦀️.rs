...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔢️set-bit-depth/🔢️raises-the-source-bit-depth-to-sixteen/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔢️set-bit-depth/🔢️raises-the-source-bit-depth-to-sixteen/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔢️set-bit-depth/🔢️raises-the-source-bit-depth-to-sixteen/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔢️set-bit-depth/🔢️raises-the-source-bit-depth-to-sixteen/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔢️set-bit-depth/🔢️raises-the-source-bit-depth-to-sixteen/🎯️outcome/🔣️.json");

fn before() -> SemioImageSnapshot {
    dsl::json::from_json_str(BEFORE).expect("set-bit-depth before snapshot decodes")
}
fn expected_after() -> SemioImageSnapshot {
    dsl::json::from_json_str(AFTER).expect("set-bit-depth after snapshot decodes")
}
fn mutation() -> SemioImageMutation {
    dsl::json::from_json_str(MUTATION).expect("set-bit-depth mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(mutation()))).expect("set-bit-depth mutation encodes");
...
async fn committed_diff_applies_to_after() {
    let decoded: SemioImageDiff = dsl::json::from_json_str(DIFF).expect("committed set-bit-depth diff decodes");
