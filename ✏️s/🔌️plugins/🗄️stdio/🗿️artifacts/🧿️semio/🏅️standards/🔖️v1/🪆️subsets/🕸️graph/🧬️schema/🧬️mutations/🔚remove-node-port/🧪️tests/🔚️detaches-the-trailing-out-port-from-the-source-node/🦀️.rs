...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔚remove-node-port/🔚️detaches-the-trailing-out-port-from-the-source-node/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔚remove-node-port/🔚️detaches-the-trailing-out-port-from-the-source-node/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔚remove-node-port/🔚️detaches-the-trailing-out-port-from-the-source-node/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔚remove-node-port/🔚️detaches-the-trailing-out-port-from-the-source-node/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔚remove-node-port/🔚️detaches-the-trailing-out-port-from-the-source-node/🎯️outcome/🔣️.json");

fn before() -> SemioGraphSnapshot {
    dsl::json::from_json_str(BEFORE).expect("remove-node-port before snapshot decodes")
}
fn expected_after() -> SemioGraphSnapshot {
    dsl::json::from_json_str(AFTER).expect("remove-node-port after snapshot decodes")
}
fn mutation() -> SemioGraphMutation {
    dsl::json::from_json_str(MUTATION).expect("remove-node-port mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(mutation()))).expect("remove-node-port mutation encodes");
...
async fn committed_diff_is_canonical_and_omits_edges_entirely() {
    let decoded: SemioGraphDiff = dsl::json::from_json_str(DIFF).expect("committed remove-node-port diff decodes");
