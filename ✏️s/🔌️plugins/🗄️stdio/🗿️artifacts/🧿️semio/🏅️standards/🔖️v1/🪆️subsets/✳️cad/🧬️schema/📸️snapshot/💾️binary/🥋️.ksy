...
doc: |
  ...
  sibling `📡️.protocol.semio`'s own comment; `blocks[].entities`/`entities` also embed a
  data-carrying tagged `entity` union via a real per-variant tag byte). Not a JSON blob — see
  `📸️snapshot/🦀️.rs`'s `encode_cad_snapshot_binary` for the payload's real internal
