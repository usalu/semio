//! 🧪️ `create-face` fixture — `🔷️adds-an-opposing-face-over-the-same-loop`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔷create-face/🔷️adds-an-opposing-face-over-the-same-loop/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔷create-face/🔷️adds-an-opposing-face-over-the-same-loop/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔷create-face/🔷️adds-an-opposing-face-over-the-same-loop/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔷create-face/🔷️adds-an-opposing-face-over-the-same-loop/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔷create-face/🔷️adds-an-opposing-face-over-the-same-loop/🎯️outcome/🔣️.json");

fn before() -> SemioBrepSnapshot {
    decode_semio_brep_snapshot_json(BEFORE).expect("create-face before snapshot decodes")
}
fn expected_after() -> SemioBrepSnapshot {
    decode_semio_brep_snapshot_json(AFTER).expect("create-face after snapshot decodes")
}
fn mutation() -> SemioBrepMutation {
    decode_semio_brep_mutation_json(MUTATION).expect("create-face mutation decodes")
}
...
async fn adds_the_opposing_face_over_the_existing_loop() {
    ...
    assert_eq!(created.tol, 4e-7, "the face carries the payload's nonzero tolerance");
...
async fn committed_json_is_canonical() {
    ...
    let original = pack::json::parse(MUTATION).expect("create-face mutation reparses");
...
async fn declared_outcome_holds_as_committed() {
    ...
    assert_eq!(outcome.get("status").and_then(pack::json::Value::as_str), Some("applied"), "create-face/adds-an-opposing-face-over-the-same-loop: this case is declared applied");
...
async fn committed_diff_is_canonical_and_narrowly_scoped() {
    let decoded = decode_semio_brep_diff_json(DIFF).expect("committed create-face diff decodes");
