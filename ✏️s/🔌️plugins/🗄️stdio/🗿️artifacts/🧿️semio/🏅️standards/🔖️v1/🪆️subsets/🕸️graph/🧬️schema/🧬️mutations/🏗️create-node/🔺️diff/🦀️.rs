...
pub fn diff(payload: &super::CreateNode, base: &SemioGraphSnapshot) -> protocol::MutationOutcome<SemioGraphDiff> {
    ...
    nodes.push(SemioGraphNode { id: payload.id.clone(), kind: payload.kind.clone(), label: payload.label.clone(), position: payload.position, ports: payload.ports.clone(), properties: payload.properties.clone() });
