//! 🧪️ `rotate-object` fixture — `🔄️rotates-the-object-a-half-turn-about-z`.
//!
//! Transcribed from `../../🔺️diff/🦀️.rs`: any non-finite quaternion component is FATAL
...
fn before() -> SemioObjectSnapshot {
    dsl::json::from_json_str(BEFORE).expect("rotate-object before snapshot decodes")
}
fn expected_after() -> SemioObjectSnapshot {
    dsl::json::from_json_str(AFTER).expect("rotate-object after snapshot decodes")
}
fn rotate_object() -> SemioObjectMutation {
    dsl::json::from_json_str(MUTATION).expect("rotate-object mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(rotate_object()))).expect("rotate-object mutation encodes");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rotate-object/rotates-the-object-a-half-turn-about-z: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical_and_carries_the_whole_transform() {
    let decoded: SemioObjectDiff = dsl::json::from_json_str(DIFF).expect("committed rotate-object diff decodes");
