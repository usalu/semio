//! 🔺️ SemioObjectDiff — 🚧 scaffolded by W1b: a minimal, genuinely law-abiding full-replace
...
pub struct SemioObjectDiff {
    ...
    pub replacement: Option<SemioObjectSnapshot>,
}
...
pub fn diff_set_snapshot(base: &SemioObjectSnapshot, snapshot: &SemioObjectSnapshot) -> SemioObjectDiff {
    <SemioObjectDiff as DiffAlgebra<SemioObjectSnapshot>>::between(base, snapshot)
}
//#endregion 🔖️Diff
...
mod tests {
    ...
    fn field_sweep_full_replace_round_trip() {
        let a = SemioObjectSnapshot::default();
        let mut b = SemioObjectSnapshot::default();
        b.schema = format!("{}-swept", a.schema);
        let d = <SemioObjectDiff as DiffAlgebra<SemioObjectSnapshot>>::between(&a, &b);
        assert_eq!(d.apply(&a), b);
        let inv = d.inverse(&a);
        assert_eq!(inv.apply(&d.apply(&a)), a);
        assert!(<SemioObjectDiff as DiffAlgebra<SemioObjectSnapshot>>::between(&a, &a).is_empty());
    }

    #[test]
    fn absorb_composes_two_sequential_diffs() {
        let a = SemioObjectSnapshot::default();
        let mut mid = SemioObjectSnapshot::default();
        mid.schema = format!("{}-mid", a.schema);
        let mut after = SemioObjectSnapshot::default();
        after.schema = format!("{}-after", a.schema);
        let mut d1 = <SemioObjectDiff as DiffAlgebra<SemioObjectSnapshot>>::between(&a, &mid);
        let d2 = <SemioObjectDiff as DiffAlgebra<SemioObjectSnapshot>>::between(&mid, &after);
