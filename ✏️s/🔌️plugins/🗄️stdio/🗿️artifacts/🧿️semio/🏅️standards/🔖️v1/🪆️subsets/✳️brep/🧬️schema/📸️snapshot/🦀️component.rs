//! 🧬️ SemioBrepSnapshot — id-keyed solids with a typed surface enum — from step's brep engine + StepSnapshot.
...
/// 🧊 Owned by the `brep` subset: `BrepSolid`, `BrepSurface`. 🚧 minimal — `BrepCurve` and the
/// full Nurbs/loop/shell/face id-keyed graph land in W2.
#[derive(Clone, Debug, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BrepSolid { pub id: String, pub surface: BrepSurface }

#[derive(Clone, Debug, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(tag = "kind", rename_all = "camelCase")]
pub enum BrepSurface {
    Plane { origin: SemioPoint3, normal: SemioPoint3 },
    Cylinder { axis: SemioPoint3, radius: f64 },
}
