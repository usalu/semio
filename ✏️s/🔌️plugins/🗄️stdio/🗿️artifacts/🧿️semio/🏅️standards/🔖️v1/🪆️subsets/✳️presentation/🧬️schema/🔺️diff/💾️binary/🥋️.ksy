...
doc: |
  ...
  varint-length-prefixed collection blobs (see the sibling `📡️.protocol.semio`'s own
  comment on the `protocol-cond-cannot-chain` boundary). Not a JSON blob — see
  `🔺️diff/🦀️.rs`'s `DiffCodec::encode_diff` for the payload's real internal layout.
seq:
  - id: format
    type: u1
  - id: presence
