...
//!
//! Transcribed from `../../🔺️diff/🦀️component.rs`: an unresolvable path is Error
...
async fn the_undo_flatten_needs_no_capture_of_its_own() {
    ...
    let SemioDrawingMutation::Flatten(reflatten) = &undo[0] else { panic!("unflatten must undo as flatten") };
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "unflatten/restores-the-captured-hierarchy-over-the-flat-group: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
