//! 🧊 Semio Mesh viewer — the Main window: a read-only world-3d mesh scene built with the shared
//! `MeshWindowKit` (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.6), from the
//! artifact-level `SemioMeshSnapshot` — this file imports nothing from the sibling editor module
...
const SEMIO_MESH_VIEW_FALLBACK_MESH_KIND: &str = "box";
const SEMIO_MESH_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const SEMIO_MESH_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const SEMIO_MESH_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
    ...
    let instances: Vec<pack::JsonValue> = (0..count)
        .map(|index| {
            pack::json!({
                ...
                "meshId": SEMIO_MESH_VIEW_FALLBACK_MESH_KIND,
...
/// 👁️ Pure `SemioMeshSnapshot -> BuiltNode` read: default camera (a viewer has no persisted
...
pub fn render(document: &SemioMeshSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = pack::json_to_string(&pack::JsonValue::from(vec![pack::json_object([
        ("id".to_string(), pack::JsonValue::from(SEMIO_MESH_VIEW_FALLBACK_MESH_KIND)),
        ("data".to_string(), pack::json_from_dsl_value(&dsl::to_dsl_value(&mesh_from_kind(SEMIO_MESH_VIEW_FALLBACK_MESH_KIND)).expect("MeshData serializes"))),
    ])]));
    let view = MeshView {
        camera_json: world3d_camera_json(SEMIO_MESH_VIEW_DEFAULT_CAMERA_POSITION, SEMIO_MESH_VIEW_DEFAULT_CAMERA_TARGET, SEMIO_MESH_VIEW_DEFAULT_CAMERA_FOV),
        meshes_json,
