...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➖remove-type/🚫️removes-the-column-type-and-keeps-the-beam-type/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➖remove-type/🚫️removes-the-column-type-and-keeps-the-beam-type/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➖remove-type/🚫️removes-the-column-type-and-keeps-the-beam-type/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➖remove-type/🚫️removes-the-column-type-and-keeps-the-beam-type/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➖remove-type/🚫️removes-the-column-type-and-keeps-the-beam-type/🎯️outcome/🔣️.json");

fn before() -> SemioKitSnapshot {
    dsl::json::from_json_str(BEFORE).expect("remove-type before snapshot decodes")
}
fn expected_after() -> SemioKitSnapshot {
    dsl::json::from_json_str(AFTER).expect("remove-type after snapshot decodes")
}
fn mutation() -> SemioKitMutation {
    dsl::json::from_json_str(MUTATION).expect("remove-type mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(mutation()))).expect("remove-type mutation encodes");
...
async fn committed_diff_is_canonical_and_narrowly_scoped() {
    let decoded: SemioKitDiff = dsl::json::from_json_str(DIFF).expect("committed remove-type diff decodes");
