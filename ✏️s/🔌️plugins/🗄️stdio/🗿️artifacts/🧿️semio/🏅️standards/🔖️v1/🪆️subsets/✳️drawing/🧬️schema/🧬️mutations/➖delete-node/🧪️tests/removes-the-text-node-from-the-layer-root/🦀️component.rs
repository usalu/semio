//! 🧪️ `delete-node` fixture — `removes-the-text-node-from-the-layer-root`.
//!
//! Transcribed from `../../🔺️diff/🦀️component.rs`: a path with no parent (a bare layer root) is
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-node/removes-the-text-node-from-the-layer-root: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
