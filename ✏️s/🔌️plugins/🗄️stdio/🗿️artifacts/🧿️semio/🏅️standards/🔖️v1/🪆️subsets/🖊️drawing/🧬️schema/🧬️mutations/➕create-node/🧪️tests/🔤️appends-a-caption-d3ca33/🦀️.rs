//! 🧪️ `create-node` fixture — `🔤️appends-a-caption-text-node-to-the-layer-root`.
//!
...
async fn appends_the_caption_as_the_fourth_child() {
    ...
    assert!(matches!(children[3], crate::standards::v1::subsets::drawing::schema::snapshot::DrawNode::Text { .. }), "the payload's own node kind lands");
...
async fn committed_diff_is_canonical_and_narrowly_scoped() {
    let decoded: SemioDrawingDiff = dsl::json::from_json_str(DIFF).expect("committed create-node diff decodes");
