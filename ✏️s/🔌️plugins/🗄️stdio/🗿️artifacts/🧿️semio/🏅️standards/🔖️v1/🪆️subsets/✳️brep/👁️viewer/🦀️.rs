//! 👁️ Semio Brep viewer — the read-only counterpart of `✏️editor` for this subset (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `SemioBrepViewer`
//! implements `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<SemioBrepViewer>`
...
use crate::viewer::semio_brep::modes::view;
use crate::viewer::semio_brep::modes::view::windows::main;
...
pub enum SemioBrepViewCommand {
    #[default]
    Noop,
}

impl protocol::OpBinary for SemioBrepViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(SemioBrepViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
pub struct SemioBrepViewer;

impl ArtifactViewer for SemioBrepViewer {
    ...
    type Command = SemioBrepViewCommand;
...
pub fn create_semio_brep_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(SEMIO_BREP_DIALECT).document(["stdio", "semio"]).icon_id("box").mode_def(view::definition()).default_mode_id(view::SEMIO_BREP_VIEW_MODE_ID).window_kind_def(main::definition()).default_layout(view::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn create_viewer_builds_a_definition_for_the_viewer_role() {
        let def = create_semio_brep_viewer();
    ...
    async fn viewer_dialect_matches_the_artifact_coordinate() {
        assert_eq!(<SemioBrepViewer as ArtifactViewer>::DIALECT, SEMIO_BREP_DIALECT);
    }

    #[semio_framework_async_macros::async_test]
    async fn viewer_never_mutates_the_document_or_draft_store() {
        semio_framework_plugin::testkit::assert_viewer_never_mutates::<SemioBrepViewer>();
    }
}
//#endregion 🧪️Tests
