//! 🧪️ `rotate` fixture — `rotates-the-nested-group-a-half-turn-about-z`.
//!
//! Transcribed from `../../🔺️diff/🦀️component.rs`: an unresolvable path ⇒ Error
...
async fn the_undo_rotate_restores_the_identity_rotation() {
    ...
    let SemioDrawingMutation::Rotate(restore) = &undo[0] else { panic!("rotate must undo as rotate") };
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rotate/rotates-the-nested-group-a-half-turn-about-z: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
