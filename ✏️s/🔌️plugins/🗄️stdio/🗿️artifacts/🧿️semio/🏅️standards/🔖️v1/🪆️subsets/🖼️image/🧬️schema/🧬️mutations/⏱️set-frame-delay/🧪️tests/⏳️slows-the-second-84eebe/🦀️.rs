//! 🧪️ `set-frame-delay` fixture — `⏳️slows-the-second-frame-down`.
//!
...
fn before() -> SemioImageSnapshot {
    dsl::json::from_json_str(BEFORE).expect("set-frame-delay before snapshot decodes")
}
fn expected_after() -> SemioImageSnapshot {
    dsl::json::from_json_str(AFTER).expect("set-frame-delay after snapshot decodes")
}
fn mutation() -> SemioImageMutation {
    dsl::json::from_json_str(MUTATION).expect("set-frame-delay mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(mutation()))).expect("set-frame-delay mutation encodes");
...
async fn committed_diff_is_canonical_and_narrowly_scoped() {
    let decoded: SemioImageDiff = dsl::json::from_json_str(DIFF).expect("committed set-frame-delay diff decodes");
