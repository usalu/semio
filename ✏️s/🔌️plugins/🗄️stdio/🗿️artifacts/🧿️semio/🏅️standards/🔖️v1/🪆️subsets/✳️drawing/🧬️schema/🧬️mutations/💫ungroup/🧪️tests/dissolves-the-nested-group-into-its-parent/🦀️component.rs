//! 🧪️ `ungroup` fixture — `dissolves-the-nested-group-into-its-parent`.
//!
//! Transcribed from `../../🔺️diff/🦀️component.rs`: a path with no parent (a layer root) is Error
...
async fn the_undo_group_recaptures_the_range_and_the_transform() {
    ...
    let SemioDrawingMutation::Group(regroup) = &undo[0] else { panic!("ungroup must undo as group") };
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "ungroup/dissolves-the-nested-group-into-its-parent: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
