...
//#region 🔖️Diff
pub async fn diff(payload: &ReplacePath, base: &SemioDrawingSnapshot) -> protocol::MutationOutcome<SemioDrawingDiff> {
    match node_at(base, &payload.at).await {
        Some(DrawNode::Path { segments, .. }) if *segments == payload.new_segments => protocol::MutationOutcome::empty().await.warn("mutation.no-op", format!("Path in layer #{} already has those segments.", payload.at.layer)).await,
