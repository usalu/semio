...
pub fn diff(payload: &ReplacePath, base: &SemioDrawingSnapshot) -> protocol::MutationOutcome<SemioDrawingDiff> {
    match node_at(base, &payload.at) {
        Some(DrawNode::Path { segments, .. }) if *segments == payload.new_segments => protocol::MutationOutcome::empty().await.warn("mutation.no-op", format!("Path in layer #{} already has those segments.", payload.at.layer)),
