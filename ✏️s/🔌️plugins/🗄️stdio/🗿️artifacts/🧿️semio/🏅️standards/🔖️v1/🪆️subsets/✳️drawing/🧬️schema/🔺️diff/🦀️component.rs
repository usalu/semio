//! 🔺️ SemioDrawingDiff — 🚧 scaffolded by W1b: a minimal, genuinely law-abiding full-replace
...
pub struct SemioDrawingDiff {
    ...
    pub replacement: Option<SemioDrawingSnapshot>,
}
...
pub fn diff_set_snapshot(base: &SemioDrawingSnapshot, snapshot: &SemioDrawingSnapshot) -> SemioDrawingDiff {
    <SemioDrawingDiff as DiffAlgebra<SemioDrawingSnapshot>>::between(base, snapshot)
}
//#endregion 🔖️Diff
...
mod tests {
    ...
    fn field_sweep_full_replace_round_trip() {
        let a = SemioDrawingSnapshot::default();
        let mut b = SemioDrawingSnapshot::default();
        b.schema = format!("{}-swept", a.schema);
        let d = <SemioDrawingDiff as DiffAlgebra<SemioDrawingSnapshot>>::between(&a, &b);
    ...
    fn absorb_composes_two_sequential_diffs() {
        let a = SemioDrawingSnapshot::default();
        let mut mid = SemioDrawingSnapshot::default();
        mid.schema = format!("{}-mid", a.schema);
        let mut after = SemioDrawingSnapshot::default();
        after.schema = format!("{}-after", a.schema);
        let mut d1 = <SemioDrawingDiff as DiffAlgebra<SemioDrawingSnapshot>>::between(&a, &mid);
        let d2 = <SemioDrawingDiff as DiffAlgebra<SemioDrawingSnapshot>>::between(&mid, &after);
