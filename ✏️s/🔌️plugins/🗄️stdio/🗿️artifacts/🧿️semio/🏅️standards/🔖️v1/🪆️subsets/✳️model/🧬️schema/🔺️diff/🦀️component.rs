//! 🔺️ SemioModelDiff — 🚧 scaffolded by W1b: a minimal, genuinely law-abiding full-replace
...
pub struct SemioModelDiff {
    ...
    pub replacement: Option<SemioModelSnapshot>,
}
...
pub fn diff_set_snapshot(base: &SemioModelSnapshot, snapshot: &SemioModelSnapshot) -> SemioModelDiff {
    <SemioModelDiff as DiffAlgebra<SemioModelSnapshot>>::between(base, snapshot)
}
//#endregion 🔖️Diff
...
mod tests {
    ...
    fn field_sweep_full_replace_round_trip() {
        let a = SemioModelSnapshot::default();
        let mut b = SemioModelSnapshot::default();
        b.schema = format!("{}-swept", a.schema);
        let d = <SemioModelDiff as DiffAlgebra<SemioModelSnapshot>>::between(&a, &b);
        assert_eq!(d.apply(&a), b);
        let inv = d.inverse(&a);
        assert_eq!(inv.apply(&d.apply(&a)), a);
        assert!(<SemioModelDiff as DiffAlgebra<SemioModelSnapshot>>::between(&a, &a).is_empty());
    }

    #[test]
    fn absorb_composes_two_sequential_diffs() {
        let a = SemioModelSnapshot::default();
        let mut mid = SemioModelSnapshot::default();
        mid.schema = format!("{}-mid", a.schema);
        let mut after = SemioModelSnapshot::default();
        after.schema = format!("{}-after", a.schema);
        let mut d1 = <SemioModelDiff as DiffAlgebra<SemioModelSnapshot>>::between(&a, &mid);
        let d2 = <SemioModelDiff as DiffAlgebra<SemioModelSnapshot>>::between(&mid, &after);
