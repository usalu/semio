...
pub mod derived_composition {
    ...
    use crate::artifacts::semio::standards::v1::subsets::workflow::schema::SemioWorkflowAnalyzer;
    use super::super::import::deserializers::artifacts::json::v_rfc8259::any::SemioWorkflowFromJson;
    use super::super::export::serializers::artifacts::json::v_rfc8259::any::SemioWorkflowToJson;

    const DIALECT: Dialect = Dialect { artifact_kind: "s.stdio.semio", standard: StandardId("v1"), subset: SubsetId("workflow") };

    //#region 🔖️Composer
    pub struct SemioWorkflowComposerComposition;

    impl ArtifactComposition for SemioWorkflowComposerComposition {
        ...
        fn compose(sources: &[ComposeSource]) -> Result<Composition<Self::Snapshot>, ComposeError> {
            let native: Vec<AnalyzeSource<'_>> = sources
                ...
                .collect();
            if native.is_empty() {
                return Err(ComposeError { message: "SemioWorkflowComposerComposition: no source in a known read dialect".into(), diagnostics: Vec::new() });
            }
            let analysis = SemioWorkflowAnalyzer::analyze(&native);
            let snapshot = analysis.parts.snapshot.ok_or_else(|| ComposeError {
                message: "SemioWorkflowComposerComposition: analysis produced no snapshot".into(),
