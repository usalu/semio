...
impl protocol::OpBinary for SemioAnimationEditCommand {
    const TOOL_JOB_IDS: &'static [&'static str] = SEMIO_ANIMATION_DOCUMENT_SCHEMA_EXAMPLE_TOOL_IDS;

    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(pack::to_json_string(self).into_bytes())
    }
    fn decode_op(bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        let parsed = pack::parse_json_bytes(bytes).map_err(|error| protocol::ProtocolError::Malformed { what: "SemioAnimationEditCommand", offset: 0, detail: error.to_string() })?;
        <Self as dsl::FromValue>::from_value(pack::json_to_dsl_value(&parsed)).map_err(|error| protocol::ProtocolError::Malformed { what: "SemioAnimationEditCommand", offset: 0, detail: error.to_string() })
    }
}
//#endregion 🔖️Command
...
fn semioAnimationEditor_command_from_action(action: &str, args: Option<&dsl::DslValue>) -> Result<SemioAnimationEditCommand, Fault> {
    match action {
...
impl ArtifactOwnedToolJobFactory for SemioAnimationEditorExampleFactory {
    ...
    fn build_document_store_initialization_job(
        ...
        Ok(semio_framework_plugin::bounded_document_store_initialization_job(envelope, SEMIO_ANIMATION_DOCUMENT_SCHEMA, operation, generation))
    }

    fn command_id(command: &Self::Command) -> &'static str { semioAnimationEditor_command_id(command) }

    fn command_from_action(action: &str, args: Option<&dsl::DslValue>) -> Result<Self::Command, Fault> { semioAnimationEditor_command_from_action(action, args) }
