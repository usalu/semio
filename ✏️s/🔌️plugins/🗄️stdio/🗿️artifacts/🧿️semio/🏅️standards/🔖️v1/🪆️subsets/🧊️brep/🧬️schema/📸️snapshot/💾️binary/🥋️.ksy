...
doc: |
  ...
  untagged, see the sibling `📡️.protocol.semio`'s own comment; `edges`/`faces` also embed
  a data-carrying tagged `curve`/`surface` union via a real per-variant tag byte). Not a JSON blob —
  see `📸️snapshot/🦀️.rs`'s `encode_brep_snapshot_binary` for the payload's real internal
