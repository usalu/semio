//! 🧊 Semio Cad viewer — the Main window: a read-only world-3d mesh scene built with the shared
//! `MeshWindowKit` (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.6), from the
//! artifact-level `SemioCadSnapshot` — this file imports nothing from the sibling editor module
...
const SEMIO_CAD_VIEW_FALLBACK_MESH_KIND: &str = "box";
const SEMIO_CAD_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const SEMIO_CAD_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const SEMIO_CAD_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
fn entity_count(document: &SemioCadSnapshot) -> usize {
...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
fn world_instances_json(document: &SemioCadSnapshot) -> String {
    let count = entity_count(document);
    let instances: Vec<pack::JsonValue> = (0..count)
        .map(|index| {
            pack::json!({
                "id": format!("semio_cad-{index}"),
                "meshId": SEMIO_CAD_VIEW_FALLBACK_MESH_KIND,
                "position": [index as f64 * 2.0, 0.0, 0.0],
                "rotation": [0.0, 0.0, 0.0, 1.0],
                "scale": [1.0, 1.0, 1.0],
                "label": format!("Semio Cad {index}"),
                "smoothShading": false,
            })
        })
        .collect();
...
/// 👁️ Pure `SemioCadSnapshot -> BuiltNode` read: default camera (a viewer has no persisted
/// per-session camera — `Config = NoConfig`), no selection/gumball/engagement overlay.
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn render(document: &SemioCadSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let mesh_data_value = dsl::to_dsl_value(&mesh_from_kind(SEMIO_CAD_VIEW_FALLBACK_MESH_KIND)).unwrap_or(dsl::DslValue::Null);
    let meshes_json = pack::json_to_string(&pack::JsonValue::Array(vec![pack::json!({ "id": SEMIO_CAD_VIEW_FALLBACK_MESH_KIND, "data": pack::json_from_dsl_value(&mesh_data_value) })]));
    let view = MeshView {
        camera_json: world3d_camera_json(SEMIO_CAD_VIEW_DEFAULT_CAMERA_POSITION, SEMIO_CAD_VIEW_DEFAULT_CAMERA_TARGET, SEMIO_CAD_VIEW_DEFAULT_CAMERA_FOV),
        meshes_json,
...
mod tests {
    ...
    async fn render_produces_a_scene_node_for_the_default_document() {
        let document = SemioCadSnapshot::default();
