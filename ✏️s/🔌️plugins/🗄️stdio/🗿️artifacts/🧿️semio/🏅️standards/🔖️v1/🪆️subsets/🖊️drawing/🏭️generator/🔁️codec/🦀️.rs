...
//! predicting one.
//!
//! @see ../../🔮️oracles/🔣️.json — the manifest whose 17 kinds this corpus binds to, one fixture each.
...
fn fixture_directory(kind: &str) -> Option<&'static str> {
    match kind {
        ...
        "delete-layer" => Some("🗑️delete-layer"),
        "delete-node" => Some("➖️delete-node"),
        "drag-nodes" => Some("🖐️drag-nodes"),
        "flatten-node" => Some("🫓️flatten-node"),
        "group-nodes" => Some("🧷️group-nodes"),
        "move-node" => Some("📍️move-node"),
        "reorder-nodes" => Some("🔀️reorder-nodes"),
        "replace-fill" => Some("🪣️replace-fill"),
        "replace-path" => Some("🛤️replace-path"),
        "rotate-node" => Some("🔄️rotate-node"),
        "scale-node" => Some("📏️scale-node"),
        "unflatten-node" => Some("🎈️unflatten-node"),
        "ungroup-node" => Some("💫️ungroup-node"),
        _ => None,
    }
}
...
fn main() {
    ...
    for (kind, before, after) in corpus() {
        let directory = fixture_directory(kind).expect("registered fixture kind");
        let dir = std::path::Path::new(&out).join(directory);
        std::fs::create_dir_all(&dir).expect("fixture directory");
        for (name, scene) in [("⬅️before.svg", &before), ("➡️after.svg", &after)] {
