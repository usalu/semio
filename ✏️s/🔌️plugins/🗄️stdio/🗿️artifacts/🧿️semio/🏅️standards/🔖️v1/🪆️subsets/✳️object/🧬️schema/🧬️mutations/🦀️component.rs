//! 🧬️ SemioObjectMutation — 🚧 scaffolded by W1b: a single `SetSnapshot` full-replace mutation
...
impl Mutation<SemioObjectSnapshot> for SemioObjectMutation {
    ...
    fn diff(&self, _base: &SemioObjectSnapshot) -> Self::Diff {
        match self {
            SemioObjectMutation::NoMutation => SemioObjectDiff::default(),
            SemioObjectMutation::SetSnapshot { snapshot } => SemioObjectDiff { replacement: Some(snapshot.clone()) },
        }
    }
...
pub fn apply_semio_object_mutation(snapshot: &mut SemioObjectSnapshot, mutation: &SemioObjectMutation) -> SemioObjectDiff {
    ...
    *snapshot = <SemioObjectDiff as protocol::MutationDiff<SemioObjectSnapshot>>::apply(&diff, snapshot);
    diff
}
//#endregion 🔖️Mutation
...
mod tests {
    ...
    fn mutation_diff_law_set_snapshot_matches_diff() {
        let base = SemioObjectSnapshot::default();
        let mut next = SemioObjectSnapshot::default();
        next.schema = format!("{}-mutated", base.schema);
        let mutation = SemioObjectMutation::SetSnapshot { snapshot: next.clone() };
        let diff = <SemioObjectMutation as Mutation<SemioObjectSnapshot>>::diff(&mutation, &base);
        assert_eq!(<SemioObjectDiff as protocol::MutationDiff<SemioObjectSnapshot>>::apply(&diff, &base), next);
        let inv = <SemioObjectMutation as Mutation<SemioObjectSnapshot>>::inverse(&mutation, &base);
        assert_eq!(inv.len(), 1);
        let mut round = next.clone();
        let _ = apply_semio_object_mutation(&mut round, &inv[0]);
    ...
    fn op_text_binary_roundtrip_law() {
        let base = SemioObjectSnapshot::default();
        for m in [SemioObjectMutation::NoMutation, SemioObjectMutation::SetSnapshot { snapshot: base.clone() }] {
            let printed = m.print_op();
            assert!(!printed.contains('\n'), "print_op must be one line, got {printed:?}");
            let parsed = SemioObjectMutation::parse_op(&printed).unwrap_or_else(|e| panic!("parse_op({printed:?}) failed: {e}"));
