...
impl ArtifactSerializer for SemioAnimationToMp4 {
    ...
    fn serialize(from: &Self::From) -> Result<Self::Into, store::PackError> {
        ...
        let tracks = if samples.is_empty() {
            ...
            vec![Mp4Track { track_id: 1, timescale: SYNTHETIC_TIMESCALE, codec: Mp4Codec::Other { fourcc: "anim".into(), raw: Vec::new() }, width: 0, height: 0, samples }]
        };
        Ok(Mp4Snapshot { schema: "stdio.mp4".into(), ftyp: Mp4Ftyp { major_brand: "isom".into(), minor_version: 0, compatible_brands: Vec::new() }, tracks, unknown_boxes: Vec::new() })
    }
}
