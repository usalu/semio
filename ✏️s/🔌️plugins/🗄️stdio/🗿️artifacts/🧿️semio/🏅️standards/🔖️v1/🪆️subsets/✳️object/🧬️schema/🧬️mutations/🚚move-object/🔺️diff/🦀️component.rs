...
pub async fn diff(payload: &MoveObject, base: &SemioObjectSnapshot) -> protocol::MutationOutcome<SemioObjectDiff> {
    ...
    if !t.x.is_finite() || !t.y.is_finite() || !t.z.is_finite() {
        return protocol::MutationOutcome::fatal("mutation.invariant", "Object translation has a non-finite component.".to_string(), ["transform".to_string()]).await;
    }
    if base.transform.translation == t {
        return protocol::MutationOutcome::empty().await.warn("mutation.no-op", "Object is already at this translation.".to_string()).await;
    }
