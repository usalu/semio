//! 🧪️ `reorder-rows` fixture — `🔃️moves-the-last-row-to-the-front`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔃reorder-rows/🔃️moves-the-last-row-to-the-front/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔃reorder-rows/🔃️moves-the-last-row-to-the-front/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔃reorder-rows/🔃️moves-the-last-row-to-the-front/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔃reorder-rows/🔃️moves-the-last-row-to-the-front/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔃reorder-rows/🔃️moves-the-last-row-to-the-front/🎯️outcome/🔣️.json");

fn before() -> SemioTableSnapshot {
    dsl::json::from_json_str(BEFORE).expect("reorder-rows before snapshot decodes")
}
fn expected_after() -> SemioTableSnapshot {
    dsl::json::from_json_str(AFTER).expect("reorder-rows after snapshot decodes")
}
fn reorder_rows() -> SemioTableMutation {
    dsl::json::from_json_str(MUTATION).expect("reorder-rows mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(reorder_rows()))).expect("reorder-rows mutation encodes");
...
async fn committed_diff_is_canonical_and_omits_columns_entirely() {
    let decoded: SemioTableDiff = dsl::json::from_json_str(DIFF).expect("committed reorder-rows diff decodes");
