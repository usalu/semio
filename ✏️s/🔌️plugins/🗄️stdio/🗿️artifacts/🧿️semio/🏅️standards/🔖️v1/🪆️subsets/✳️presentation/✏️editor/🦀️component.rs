//! ✏️ Semio Presentation editor — thin, kit-based editor surface (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.1). `SemioPresentationEditor`
...
use crate::editor::semio_presentation::modes::edit;
use crate::editor::semio_presentation::modes::edit::windows::main;
...
pub enum SemioPresentationEditCommand {
    #[default]
    SetVertex,
}

impl protocol::OpBinary for SemioPresentationEditCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(SemioPresentationEditCommand::SetVertex)
    }
}
//#endregion 🔖️Command
...
pub struct SemioPresentationEditor;

impl ArtifactEditor for SemioPresentationEditor {
...
//#region 🔖️Manifest
pub fn create_semio_presentation_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(SEMIO_PRESENTATION_DIALECT)
        .document(["stdio", "semio"])
        .icon_id("box")
        .mode_def(edit::definition())
        .default_mode_id(edit::SEMIO_PRESENTATION_EDIT_MODE_ID)
...
mod tests {
    ...
    fn create_editor_builds_a_definition_for_the_editor_role() {
        let def = create_semio_presentation_editor();
    ...
    fn editor_dialect_matches_the_artifact_coordinate() {
        assert_eq!(<SemioPresentationEditor as ArtifactEditor>::DIALECT, SEMIO_PRESENTATION_DIALECT);
    }

    #[test]
    fn editor_and_viewer_share_one_dialect() {
        semio_framework_plugin::testkit::assert_editor_and_viewer_share_dialect::<SemioPresentationEditor, crate::viewer::semio_presentation::SemioPresentationViewer>();
    }
}
//#endregion 🧪️Tests
