...
//#region 🔖️Diff
pub async fn diff(payload: &ReplaceCurve, base: &SemioBrepSnapshot) -> protocol::MutationOutcome<SemioBrepDiff> {
    let Some(edge) = base.edges.iter().find(|e| e.id == payload.edge_id) else {
        return protocol::MutationOutcome::error("mutation.target-missing", format!("Edge \"{}\" does not exist.", payload.edge_id), [payload.edge_id.clone()]).await;
    };
    if edge.curve == payload.new_curve {
        return protocol::MutationOutcome::empty().await.warn("mutation.no-op", format!("Edge \"{}\" already has this curve.", payload.edge_id)).await;
    }
