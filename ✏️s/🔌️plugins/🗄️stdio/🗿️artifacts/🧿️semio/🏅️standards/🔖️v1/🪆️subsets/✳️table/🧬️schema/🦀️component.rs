...
pub mod derived_construction {
    ...
    impl ArtifactBuilder for SemioTableBuilderConstruction {
        ...
        fn mutate(mut self, mutation: Self::Mutation) -> (Self, Self::Diff) {
            let diff = <Self::Mutation as protocol::Mutation<SemioTableSnapshot>>::diff(&mutation, &self.snapshot);
            self.snapshot = <Self::Diff as protocol::MutationDiff<SemioTableSnapshot>>::apply(&diff, &self.snapshot);
            (self, diff)
        }
...
semio_framework_plugin::derive_artifact_facets!(
    pub spec SemioTableBuilderFacets {
        construction: derived_construction::SemioTableBuilderConstruction,
        analysis: derived_analysis::SemioTableAnalyzerAnalysis,
