...
pub fn inverse(payload: &DeleteColumn, base: &SemioTableSnapshot) -> Vec<SemioTableMutation> {
    ...
    let mut mutations = vec![SemioTableMutation::CreateColumn(create_column::mutation::CreateColumn {
    ...
    for (row_index, row) in base.rows.iter().enumerate() {
        if let Some(cell) = row.cells.get(at) {
            mutations.push(SemioTableMutation::EditCell(edit_cell::mutation::EditCell {
                row_index,
                column_name: payload.name.clone(),
                new_value: cell.clone(),
            }));
        }
    }
    mutations
