//! 🧊 Semio Value editor — the Main window: the SAME world-3d mesh scene shape as the sibling
...
//! artifact-level `SemioValueSnapshot` — this window itself never mutates; mutation is the surface
...
const SEMIO_VALUE_EDIT_FALLBACK_MESH_KIND: &str = "box";
const SEMIO_VALUE_EDIT_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const SEMIO_VALUE_EDIT_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const SEMIO_VALUE_EDIT_DEFAULT_CAMERA_FOV: f64 = 45.0;
    ...
    let instances: Vec<pack::JsonValue> = (0..count)
        .map(|index| {
            pack::json!({
                ...
                "meshId": SEMIO_VALUE_EDIT_FALLBACK_MESH_KIND,
                "position": [index as f64 * 2.0, 0.0, 0.0],
                "rotation": [0.0, 0.0, 0.0, 1.0],
                "scale": [1.0, 1.0, 1.0],
                "label": format!("Semio Value {index}"),
                "smoothShading": false,
            })
        })
        .collect();
...
pub fn render(document: &SemioValueSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = pack::json_to_string(&pack::JsonValue::from(vec![pack::json_object([
        ("id".to_string(), pack::JsonValue::from(SEMIO_VALUE_EDIT_FALLBACK_MESH_KIND)),
        ("data".to_string(), pack::json_from_dsl_value(&dsl::to_dsl_value(&mesh_from_kind(SEMIO_VALUE_EDIT_FALLBACK_MESH_KIND)).expect("MeshData serializes"))),
    ])]));
    let view = MeshView {
        camera_json: world3d_camera_json(SEMIO_VALUE_EDIT_DEFAULT_CAMERA_POSITION, SEMIO_VALUE_EDIT_DEFAULT_CAMERA_TARGET, SEMIO_VALUE_EDIT_DEFAULT_CAMERA_FOV),
        meshes_json,
