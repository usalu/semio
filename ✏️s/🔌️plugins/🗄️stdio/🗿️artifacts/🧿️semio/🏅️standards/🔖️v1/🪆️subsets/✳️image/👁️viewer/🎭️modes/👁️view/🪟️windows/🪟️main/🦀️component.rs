...
pub fn render(document: &SemioImageSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": SEMIO_IMAGE_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(SEMIO_IMAGE_VIEW_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
