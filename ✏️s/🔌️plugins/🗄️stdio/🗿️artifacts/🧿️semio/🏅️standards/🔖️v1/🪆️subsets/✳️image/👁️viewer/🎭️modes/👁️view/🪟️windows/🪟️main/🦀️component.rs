...
async fn entity_count(document: &SemioImageSnapshot) -> usize {
...
async fn world_instances_json(document: &SemioImageSnapshot) -> String {
...
/// 👁️ Pure `SemioImageSnapshot -> UiNode` read: default camera (a viewer has no persisted
/// per-session camera — `Config = NoConfig`), no selection/gumball/engagement overlay.
pub async fn render(document: &SemioImageSnapshot) -> UiNode {
    ...
    let view = MeshView {
        camera_json: world3d_camera_json(SEMIO_IMAGE_VIEW_DEFAULT_CAMERA_POSITION, SEMIO_IMAGE_VIEW_DEFAULT_CAMERA_TARGET, SEMIO_IMAGE_VIEW_DEFAULT_CAMERA_FOV).await,
        meshes_json,
