...
pub async fn diff(payload: &RemoveNodePort, base: &SemioGraphSnapshot) -> protocol::MutationOutcome<SemioGraphDiff> {
    ...
    if payload.index >= node.ports.len() {
        return protocol::MutationOutcome::error("mutation.target-missing", format!("Node \"{}\" has no port at index {}.", payload.node_id.value, payload.index), [payload.node_id.value.clone(), payload.index.to_string()]).await;
    }
