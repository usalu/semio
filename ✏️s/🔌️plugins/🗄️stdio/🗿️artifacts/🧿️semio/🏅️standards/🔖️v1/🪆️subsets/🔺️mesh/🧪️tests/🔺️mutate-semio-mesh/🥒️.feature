...
Feature: Apply every typed semio MESH mutation to a real architectural model, against three.js and an independent TypeScript implementation
  ...
  **`🟦️component.ts` beside this file is the second IMPLEMENTATION, for the half no third party
  ...
  `🧰️framework/🛍️products/💻️os/🔨️modules/🧬️semio/🦀️.rs`, and the verbs from
  ...
  **The model under test is a real one, and its provenance is written down.**
  `shared://🔺️mutate-semio-mesh/🗣️.dsl.semio` and its binary twin were derived ONCE from the real committed glTF
  binary `🗿️artifacts/🧊️gltf/🏅️standards/🔖️2.0/🪆️subsets/♾️any/📚️examples/🌱️metabolism/🖼️assets/🏙️base/🧊️.glb`
  ...
  Scenario Outline: Apply <id> to the real derived model
    Given the real derived mesh artifact shared://🔺️mutate-semio-mesh/🗣️.dsl.semio
    And the committed mutation payload shared://🔺️mutate-semio-mesh/<fixture>/🦠️mutation/🔣️.json
    ...
    Examples:
      | id | fixture |
      | create-mesh | 🕸️create-mesh |
      | delete-mesh | 🗑️delete-mesh |
      | create-primitive | 🔺️create-primitive |
      | delete-primitive | ✂️delete-primitive |
      | set-primitive-topology | 🔀️set-primitive-topology |
      | replace-primitive-geometry | 📐️replace-primitive-geometry |
      | set-primitive-material | 🧲️set-primitive-material |
      | create-material | 🎨️create-material |
      | delete-material | 🚮️delete-material |
      | change-material-base-color | 🌈️change-material-base-color |
      | change-material-metallic | ⚙️change-material-metallic |
      | change-material-roughness | 🧱️change-material-roughness |
      | create-texture | 🖼️create-texture |
      | delete-texture | 🕳️delete-texture |
  ...
  Scenario: Re-emit both committed encodings of the real derived model from the parsed document
    Given the real derived mesh artifact shared://🔺️mutate-semio-mesh/🗣️.dsl.semio
    And its committed binary twin shared://🔺️mutate-semio-mesh/🎒️.pack.semio
