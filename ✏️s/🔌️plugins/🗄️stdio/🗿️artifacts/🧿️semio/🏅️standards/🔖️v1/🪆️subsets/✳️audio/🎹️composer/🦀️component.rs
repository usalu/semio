//! 🎹️ SemioAudioComposer (s.stdio.semio/v1/audio) — 🚧 scaffolded by W1b:
    ...
    fn validate(payload: &IoPayload) -> Vec<dsl::Diagnostic> {
        ...
        match decoded {
            ...
            None => vec![dsl::Diagnostic::error(
                "stdio.semio_audio.validate-decode-failed",
                dsl::TextSpan::at(1, 1),
                "SemioAudioValidator: payload did not decode as a SemioAudioSnapshot".to_string(),
            )],
        }
    }
}
