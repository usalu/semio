...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕add-type/📇️appends-a-slab-type-to-the-catalogue/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕add-type/📇️appends-a-slab-type-to-the-catalogue/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕add-type/📇️appends-a-slab-type-to-the-catalogue/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕add-type/📇️appends-a-slab-type-to-the-catalogue/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕add-type/📇️appends-a-slab-type-to-the-catalogue/🎯️outcome/🔣️.json");

fn before() -> SemioKitSnapshot {
    dsl::json::from_json_str(BEFORE).expect("add-type before snapshot decodes")
}
fn expected_after() -> SemioKitSnapshot {
    dsl::json::from_json_str(AFTER).expect("add-type after snapshot decodes")
}
fn mutation() -> SemioKitMutation {
    dsl::json::from_json_str(MUTATION).expect("add-type mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(mutation()))).expect("add-type mutation encodes");
...
async fn committed_diff_is_canonical_and_narrowly_scoped() {
    let decoded: SemioKitDiff = dsl::json::from_json_str(DIFF).expect("committed add-type diff decodes");
