...
pub mod derived_composition {
    ...
    impl SubsetValidator for SemioFlowValidator {
        ...
        async fn validate(payload: &IoPayload) -> Vec<dsl::Diagnostic> {
            let decoded = match payload {
                IoPayload::Binary(bytes) => <SemioFlowSnapshot as store::ArtifactPack>::decode_pack(bytes).await.ok(),
    ...
    mod tests {
        ...
        async fn validator_recheck_on_wire_payload_flags_the_same_invariants() {
            ...
            assert!(diagnostics.await.iter().any(|d| d.code.0 == "stdio.semio_flow.dangling-edge-endpoint"), "got {diagnostics:?}");
        }
        ...
        mod conformance_laws {
            ...
            async fn fixture_honesty_law() {
                ...
                let parsed = <snapshot::SemioFlowSnapshot as store::ArtifactDsl>::parse_dsl(FIXTURE_DSL).await.expect("parse shipped .dsl.semio fixture");
                ...
                let decoded = <snapshot::SemioFlowSnapshot as store::ArtifactPack>::decode_pack(FIXTURE_PACK).await.expect("decode shipped .pack.semio fixture");
