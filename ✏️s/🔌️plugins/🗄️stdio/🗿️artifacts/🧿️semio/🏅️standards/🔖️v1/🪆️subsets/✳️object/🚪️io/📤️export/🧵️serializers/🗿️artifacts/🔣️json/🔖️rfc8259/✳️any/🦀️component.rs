//! 📤️ `SemioObjectToJson` — mirror of `SemioObjectFromJson`. `Null`/`Bool`/`Str`/`List`/`Map` map
//! directly back onto `Null`/`Bool`/`String`/`Array`/`Object`; `Int{lexeme}`/`Float{lexeme}` both
...
//!   object always becomes `Str`, never round-trips back to `Bytes`. Documented, never silently
//!   "fixed" by inventing a JSON binary convention this codec doesn't otherwise use.
//! - `Ref{id}` is DEREFERENCED: the referenced `objects` entry's value is walked and inlined
//!   recursively (json has no graph — every `JsonValue` is a tree). A `Ref` that does not resolve
//!   in `objects`, or a reference cycle (a node reachable from itself through one or more `Ref`
...
//#region 🔖️Serializer
pub struct SemioObjectToJson;

impl ArtifactSerializer for SemioObjectToJson {
    ...
    fn serialize(from: &Self::From) -> Result<Self::Into, store::PackError> {
        ...
        let value = json_value_from_semio(&from.root, &objects, &mut visiting)?;
...
//#region 🔖️Convert
fn json_value_from_semio(v: &SemioValue, objects: &HashMap<&ObjectId, &SemioValue>, visiting: &mut HashSet<ObjectId>) -> Result<JsonValue, store::PackError> {
    match v {
        ...
        SemioValue::List { items } => {
            let items = items.iter().map(|item| json_value_from_semio(item, objects, visiting)).collect::<Result<Vec<_>, _>>()?;
            Ok(JsonValue::Array { items })
        }
        SemioValue::Map { entries } => {
            let members = entries
                .iter()
                .map(|e| Ok(JsonMember { key: e.key.clone(), value: json_value_from_semio(&e.value, objects, visiting)? }))
                .collect::<Result<Vec<_>, store::PackError>>()?;
            Ok(JsonValue::Object { members })
        }
        SemioValue::Ref { id } => {
            if !visiting.insert(id.clone()) {
                return Err(store::PackError::Schema(format!("object->json: reference cycle detected at id {:?} (json has no graph, cannot represent a cycle)", id.value)));
            }
            let target = objects.get(id).ok_or_else(|| store::PackError::Schema(format!("object->json: dangling Ref{{id: {:?}}} — not found in `objects`", id.value)))?;
            let result = json_value_from_semio(target, objects, visiting);
            visiting.remove(id);
            result
        }
    }
...
mod tests {
    use super::*;
    use crate::artifacts::semio::standards::v1::subsets::object::io::import::deserializers::artifacts::json::v_rfc8259::any::semio_value_from_json;
    use crate::artifacts::semio::standards::v1::subsets::object::schema::snapshot::{SemioObjectEntry, SemioObjectNode};

    #[test]
    fn int_and_float_lexemes_reemit_as_a_plain_number_verbatim() {
        let objects = HashMap::new();
        let mut visiting = HashSet::new();
        assert_eq!(json_value_from_semio(&SemioValue::Int { lexeme: "9007199254740993".into() }, &objects, &mut visiting).unwrap(), JsonValue::Number { lexeme: "9007199254740993".into() });
        assert_eq!(json_value_from_semio(&SemioValue::Float { lexeme: "1.2300".into() }, &objects, &mut visiting).unwrap(), JsonValue::Number { lexeme: "1.2300".into() });
    }
    ...
    fn bytes_become_a_base64_string() {
        let objects = HashMap::new();
        let mut visiting = HashSet::new();
        let out = json_value_from_semio(&SemioValue::Bytes { value: vec![0, 1, 2, 255] }, &objects, &mut visiting).unwrap();
        assert_eq!(out, JsonValue::String { value: base64_encode(&[0, 1, 2, 255]) });
    }

    #[test]
    fn ref_is_dereferenced_inline() {
        let target_id = ObjectId::new("n1");
        let target_value = SemioValue::Str { value: "leaf".into() };
        let mut objects_owned: HashMap<&ObjectId, &SemioValue> = HashMap::new();
        objects_owned.insert(&target_id, &target_value);
        let mut visiting = HashSet::new();
        let out = json_value_from_semio(&SemioValue::Ref { id: target_id.clone() }, &objects_owned, &mut visiting).unwrap();
        assert_eq!(out, JsonValue::String { value: "leaf".into() });
    }

    #[test]
    fn dangling_ref_is_a_hard_error() {
        let objects: HashMap<&ObjectId, &SemioValue> = HashMap::new();
        let mut visiting = HashSet::new();
        let err = json_value_from_semio(&SemioValue::Ref { id: ObjectId::new("missing") }, &objects, &mut visiting);
        assert!(err.is_err());
    }

    #[test]
    fn self_cycle_is_a_hard_error() {
        let id = ObjectId::new("n1");
        let value = SemioValue::Ref { id: id.clone() };
        let mut objects_owned: HashMap<&ObjectId, &SemioValue> = HashMap::new();
        objects_owned.insert(&id, &value);
        let mut visiting = HashSet::new();
        let err = json_value_from_semio(&SemioValue::Ref { id: id.clone() }, &objects_owned, &mut visiting);
        assert!(err.is_err(), "a Ref pointing to itself must error, not infinitely recurse");
    }

    /// 🧪️ Required proof: json -> object -> json -> object round trip preserves everything the
    /// object subset can represent (Bytes/Ref excepted — documented one-directional gaps, proven
    /// separately above).
    #[test]
    fn json_to_object_to_json_to_object_round_trips() {
        ...
        let s1 = SemioObjectSnapshot { schema: crate::artifacts::semio::standards::v1::subsets::object::schema::snapshot::STDIO_SEMIOOBJECT_DOCUMENT_SCHEMA.into(), root: s1_value, objects: Vec::new() };
        let json_x = SemioObjectToJson::serialize(&s1).expect("serialize");
        let s2_value = semio_value_from_json(&json_x.value);
        assert_eq!(s1.root, s2_value);
    }

    #[test]
    fn objects_graph_round_trips_through_dereferenced_json() {
        let s1 = SemioObjectSnapshot {
            schema: crate::artifacts::semio::standards::v1::subsets::object::schema::snapshot::STDIO_SEMIOOBJECT_DOCUMENT_SCHEMA.into(),
            root: SemioValue::Map { entries: vec![SemioObjectEntry { key: "linked".into(), value: SemioValue::Ref { id: ObjectId::new("n1") } }] },
            objects: vec![SemioObjectNode { id: ObjectId::new("n1"), value: SemioValue::Int { lexeme: "7".into() } }],
        };
        let json_x = SemioObjectToJson::serialize(&s1).expect("serialize");
        match &json_x.value {
