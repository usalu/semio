...
pub fn render(document: &SemioDocumentSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let mesh_data_value = dsl::to_dsl_value(&mesh_from_kind(SEMIO_DOCUMENT_EDIT_FALLBACK_MESH_KIND)).unwrap_or(dsl::DslValue::Null);
    let meshes_json = pack::json_to_string(&pack::JsonValue::Array(vec![pack::json!({ "id": SEMIO_DOCUMENT_EDIT_FALLBACK_MESH_KIND, "data": pack::json_from_dsl_value(&mesh_data_value) })]));
    let view = MeshView {
