...
doc: |
  ...
  sibling `📡️component.protocol.semio`'s own comment; `blocks` also embeds a data-carrying tagged
  `DocBlock` union with recursive `List`/`Table`/`Quote` nesting via a real per-variant tag byte).
  Not a JSON blob — see `📸️snapshot/🦀️component.rs`'s `encode_document_snapshot_binary` for the
