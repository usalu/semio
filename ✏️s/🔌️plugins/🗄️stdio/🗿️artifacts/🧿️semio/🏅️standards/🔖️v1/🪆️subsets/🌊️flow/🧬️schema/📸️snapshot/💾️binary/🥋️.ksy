meta:
  id: stdio_semio_flow_snapshot
  endian: le
doc: |
  `pack` (binary) form of a `stdio.semio.flow` snapshot, past the `semio_format` envelope: a
  ...
  `📡️.protocol.semio`'s own comment). Not a JSON blob — see
  `📸️snapshot/🦀️.rs`'s `encode_flow_snapshot_binary` for the payload's real internal
