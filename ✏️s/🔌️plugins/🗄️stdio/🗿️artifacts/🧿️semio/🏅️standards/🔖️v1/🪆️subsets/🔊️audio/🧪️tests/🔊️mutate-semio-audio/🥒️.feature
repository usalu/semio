...
Feature: Apply every typed semio AUDIO mutation to a real recording, against an independent Python implementation
  ...
  `semio-audio-python-independent` in `…/🔊️audio/🔮️oracles/🔣️.json`; the recorded no-oracle
  ...
  Scenario Outline: Apply <id> to its committed specification vector
    ...
    Examples:
      ...
      | set-channel-samples | 🌊️set-channel-samples |
      | insert-tag | 🏷️insert-tag |
      | remove-tag | ✂️remove-tag |
      | set-tag-value | 💬️set-tag-value |
