...
//#region 🔖️Diff
pub async fn diff(payload: &MoveVertex, base: &SemioMeshSnapshot) -> protocol::MutationOutcome<SemioMeshDiff> {
    ...
    if *current == payload.new_point {
        return protocol::MutationOutcome::empty().await.warn("mutation.no-op", format!("Vertex {} of primitive \"{}\" is already at that position.", payload.vertex_index, payload.primitive_id)).await;
    }
