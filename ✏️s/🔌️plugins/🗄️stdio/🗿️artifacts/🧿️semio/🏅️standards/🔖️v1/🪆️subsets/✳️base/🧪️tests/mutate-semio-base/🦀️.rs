...
//! 🪆️subsets/✳️base/🔣️oracle.json`): `s.stdio.semio` is the ENVELOPE union over all
...
//! mutate-semio-base` really executes every scenario below. The gate keeps the two BUILDS apart; it has
...
//#region 🔖️Kinds
/// 🏷️ Mirrors `SemioMutation::KINDS` (`../../🏅️standards/🔖️v1/🪆️subsets/✳️base/🧬️schema/
...
const LEAF_DIR: &str = "🏅️standards/🔖️v1/🪆️subsets/✳️base/🧬️schema/🧬️mutations/📄set-snapshot/🧪️tests/replaces-the-envelope-wrapping-a-value-subset";
...
fn expected_routing(scenario: &str, kind: &str) -> (String, Vec<String>, bool) {
    match scenario {
        ...
        other => panic!("mutate-semio-base: no declared routing law for scenario {other:?}"),
    }
}
...
mod subject {
    ...
    use semio_s_plugin_stdio::artifacts::semio::standards::v1::subsets::base::schema::snapshot::{decode_semio_envelope_pack, encode_semio_envelope_pack, parse_semio_envelope_dsl, print_semio_envelope_dsl};
    use semio_s_plugin_stdio::artifacts::semio::standards::v1::subsets::base::schema::mutations::semio_mutation_refusal_codes;
    use semio_s_plugin_stdio::artifacts::semio::standards::v1::subsets::base::schema::geometry::{SemioPoint2, SemioPoint3};
    use semio_s_plugin_stdio::artifacts::semio::standards::v1::subsets::base::schema::mutations::{
    ...
    use semio_s_plugin_stdio::artifacts::semio::standards::v1::subsets::base::schema::mutations::apply_animation as any_animation;
    use semio_s_plugin_stdio::artifacts::semio::standards::v1::subsets::base::schema::snapshot::{SemioSnapshot, SemioSubsetSnapshot};
    ...
    fn empty_envelope(subset: &str) -> SemioSnapshot {
        let arm = match subset {
            ...
            other => panic!("mutate-semio-base: no empty envelope for subset {other:?}"),
        };
    ...
    fn delegated(subset: &str) -> SemioMutation {
        match subset {
            ...
            other => panic!("mutate-semio-base: no delegated verb for subset {other:?}"),
        }
    }
    //#endregion 🔖️Arms
    ...
    fn bytes_field(json: &Json, key: &str) -> Vec<u8> {
        json.array(key)
            ...
            .map(|entry| match entry {
                ...
                other => panic!("mutate-semio-base: expected a byte number, found {other:?}"),
            })
            .collect()
    }
    ...
    fn decode_value_snapshot(json: &Json) -> semio_s_plugin_stdio::artifacts::semio::standards::v1::subsets::value::schema::snapshot::SemioValueSnapshot {
        ...
        SemioValueSnapshot {
            ...
            root: decode_value(json.get("root").expect("mutate-semio-base: a value snapshot must carry a root")),
            nodes: json
                .array("nodes")
                .iter()
                .map(|node| SemioValueNode { id: decode_id(node.get("id").expect("mutate-semio-base: a graph node must carry an id")), value: decode_value(node.get("value").expect("mutate-semio-base: a graph node must carry a value")) })
                .collect(),
        }
    }
    fn decode_envelope(json: &Json) -> SemioSnapshot {
        let arm = json.get("subset").expect("mutate-semio-base: an envelope must carry a subset");
        let subset = match arm.str("subset").as_str() {
            "value" => SemioSubsetSnapshot::Value(decode_value_snapshot(arm)),
            other => panic!("mutate-semio-base: the committed leaf vector carries the value arm; no decoder for {other:?}"),
        };
...
/// 🏷️ `no-mutation` is a TEST-ONLY control row — no `SemioMutation` variant exists for it
/// (`#[derive(dsl::Mutations)]` requires every variant to wrap exactly one leaf payload, and a unit
/// variant wraps none) — so its own catalog kind was dropped in the ticket
/// `SEPARATE-ARTIFACT-STANDARD-SUBSET-IMPLEMENTATIONS-AND-FIXTURE-TEST-EVERY-MUTATION` (shard E3)
/// once this subset's own v2 manifest was written from the 19 real leaf descriptors. The feature's
/// own two scenarios were retagged `@id-no-mutation-baseline-mutate`/`-inverse` to match (not
/// `mutate-`/`inverse-` prefixed, so `mutationCoverageBreaches`'s stray check never claims them
/// against the catalog); this registration mirrors that rename precisely, everything else unchanged.
fn scenario_id(kind: &str, verb: &str) -> String {
    if kind == "no-mutation" {
...
pub fn adapter() -> Adapter {
    ...
    for kind in KINDS {
        built = built.oracle(&scenario_id(kind, "mutate"), mutate_oracle_for(kind)).oracle(&scenario_id(kind, "inverse"), inverse_oracle_for(kind));
        #[cfg(feature = "sut")]
        {
