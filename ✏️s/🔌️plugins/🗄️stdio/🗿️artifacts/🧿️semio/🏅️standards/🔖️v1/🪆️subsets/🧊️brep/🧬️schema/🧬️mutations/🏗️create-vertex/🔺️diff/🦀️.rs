...
pub fn diff(payload: &super::CreateVertex, base: &SemioBrepSnapshot) -> protocol::MutationOutcome<SemioBrepDiff> {
    ...
    protocol::MutationOutcome::new(SemioBrepDiff { vertices: Some(NamedTripleDiff { removed: vec![], modified: vec![], added: vec![BrepVertex { id: payload.id.clone(), point: payload.point, tol: 0.0 }] }), ..Default::default() })
}
//#endregion 🔖️Diff
