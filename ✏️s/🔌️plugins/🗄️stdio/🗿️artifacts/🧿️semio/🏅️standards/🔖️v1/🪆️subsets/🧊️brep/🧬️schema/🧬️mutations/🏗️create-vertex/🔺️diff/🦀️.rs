...
use crate::standards::v1::subsets::brep::schema::snapshot::{BrepVertex, SemioBrepSnapshot};
...
pub fn diff(payload: &super::CreateVertex, base: &SemioBrepSnapshot) -> protocol::MutationOutcome<SemioBrepDiff> {
    ...
    protocol::MutationOutcome::new(SemioBrepDiff { vertices: Some(NamedTripleDiff { removed: vec![], modified: vec![], added: vec![BrepVertex { id: payload.id.clone(), point: payload.point, tol: payload.tol }] }), ..Default::default() })
}
//#endregion 🔖️Diff
