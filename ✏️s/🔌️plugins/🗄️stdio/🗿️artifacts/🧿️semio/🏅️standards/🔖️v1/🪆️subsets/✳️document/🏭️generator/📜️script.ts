...
async function generateOne(recipe: Recipe, outDir: string): Promise<Record<string, unknown>[]> {
  ...
  for (const carrier of recipe.carriers) {
    ...
    for (const [variant, doc] of variants) {
      ...
      const filename = `${variant}.${carrier}`;
      write(join(dir, filename), bytes);
      files.push({ role: `${variant}-${carrier}`, path: `${FIXTURE_PATH_PREFIX}${recipe.id}/${filename}`, mediaType: spec.mediaType, sha256: await contentDigest(bytes), bytes: bytes.length });
    }
    manifests.push({
