//! 🔺️ SemioCadDiff — 🚧 scaffolded by W1b: a minimal, genuinely law-abiding full-replace
...
pub struct SemioCadDiff {
    ...
    pub replacement: Option<SemioCadSnapshot>,
}
...
pub fn diff_set_snapshot(base: &SemioCadSnapshot, snapshot: &SemioCadSnapshot) -> SemioCadDiff {
    <SemioCadDiff as DiffAlgebra<SemioCadSnapshot>>::between(base, snapshot)
}
//#endregion 🔖️Diff
...
mod tests {
    ...
    fn field_sweep_full_replace_round_trip() {
        let a = SemioCadSnapshot::default();
        let mut b = SemioCadSnapshot::default();
        b.schema = format!("{}-swept", a.schema);
        let d = <SemioCadDiff as DiffAlgebra<SemioCadSnapshot>>::between(&a, &b);
        assert_eq!(d.apply(&a), b);
        let inv = d.inverse(&a);
        assert_eq!(inv.apply(&d.apply(&a)), a);
        assert!(<SemioCadDiff as DiffAlgebra<SemioCadSnapshot>>::between(&a, &a).is_empty());
    }

    #[test]
    fn absorb_composes_two_sequential_diffs() {
        let a = SemioCadSnapshot::default();
        let mut mid = SemioCadSnapshot::default();
        mid.schema = format!("{}-mid", a.schema);
        let mut after = SemioCadSnapshot::default();
        after.schema = format!("{}-after", a.schema);
        let mut d1 = <SemioCadDiff as DiffAlgebra<SemioCadSnapshot>>::between(&a, &mid);
        let d2 = <SemioCadDiff as DiffAlgebra<SemioCadSnapshot>>::between(&mid, &after);
