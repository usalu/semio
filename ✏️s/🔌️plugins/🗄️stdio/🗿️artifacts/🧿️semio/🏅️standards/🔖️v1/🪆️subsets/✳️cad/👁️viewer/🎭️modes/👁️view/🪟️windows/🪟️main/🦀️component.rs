...
async fn entity_count(document: &SemioCadSnapshot) -> usize {
...
async fn world_instances_json(document: &SemioCadSnapshot) -> String {
...
/// 👁️ Pure `SemioCadSnapshot -> UiNode` read: default camera (a viewer has no persisted
/// per-session camera — `Config = NoConfig`), no selection/gumball/engagement overlay.
pub async fn render(document: &SemioCadSnapshot) -> UiNode {
    ...
    let view = MeshView {
        camera_json: world3d_camera_json(SEMIO_CAD_VIEW_DEFAULT_CAMERA_POSITION, SEMIO_CAD_VIEW_DEFAULT_CAMERA_TARGET, SEMIO_CAD_VIEW_DEFAULT_CAMERA_FOV).await,
        meshes_json,
