//! 🧪️ `📄set-snapshot` fixture — `replaces-the-envelope-wrapping-a-value-subset`.
//!
//! Source of truth is the committed JSON quintet beside this file (contract D1, ticket
//! `26/08/20/COMPOSE-TO-PUZZLE5D-MIGRATION`). The `.op.semio`/`.spr.semio`/`.dsl.semio`/
//! `.pack.semio`/`.patch.semio` encodings are derived from it by `fixtures generate`.
//!
//! 🧿️ `✳️any` is the ENVELOPE union, and `SetSnapshot` is the only mutation it owns that is not a
