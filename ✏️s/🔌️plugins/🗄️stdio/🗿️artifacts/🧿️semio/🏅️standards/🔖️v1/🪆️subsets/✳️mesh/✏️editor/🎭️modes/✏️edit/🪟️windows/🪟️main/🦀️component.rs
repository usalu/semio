//! 🧊 Semio Mesh editor — the Main window: the SAME world-3d mesh scene shape as the sibling
...
//! artifact-level `SemioMeshSnapshot` — this window itself never mutates; mutation is the surface
...
const SEMIO_MESH_EDIT_FALLBACK_MESH_KIND: &str = "box";
const SEMIO_MESH_EDIT_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const SEMIO_MESH_EDIT_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const SEMIO_MESH_EDIT_DEFAULT_CAMERA_FOV: f64 = 45.0;
...
/// other by design).
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
fn entity_count(document: &SemioMeshSnapshot) -> usize {
...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
fn world_instances_json(document: &SemioMeshSnapshot) -> String {
    let count = entity_count(document);
    let instances: Vec<pack::JsonValue> = (0..count)
        .map(|index| {
            pack::json!({
                "id": format!("semio_mesh-{index}"),
                "meshId": SEMIO_MESH_EDIT_FALLBACK_MESH_KIND,
                "position": [index as f64 * 2.0, 0.0, 0.0],
                "rotation": [0.0, 0.0, 0.0, 1.0],
                "scale": [1.0, 1.0, 1.0],
                "label": format!("Semio Mesh {index}"),
                "smoothShading": false,
            })
        })
        .collect();
...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn render(document: &SemioMeshSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = pack::json_to_string(&pack::JsonValue::from(vec![pack::json_object([
        ("id".to_string(), pack::JsonValue::from(SEMIO_MESH_EDIT_FALLBACK_MESH_KIND)),
        ("data".to_string(), pack::json_from_dsl_value(&dsl::to_dsl_value(&mesh_from_kind(SEMIO_MESH_EDIT_FALLBACK_MESH_KIND)).expect("MeshData serializes"))),
    ])]));
    let view = MeshView {
        camera_json: world3d_camera_json(SEMIO_MESH_EDIT_DEFAULT_CAMERA_POSITION, SEMIO_MESH_EDIT_DEFAULT_CAMERA_TARGET, SEMIO_MESH_EDIT_DEFAULT_CAMERA_FOV),
        meshes_json,
