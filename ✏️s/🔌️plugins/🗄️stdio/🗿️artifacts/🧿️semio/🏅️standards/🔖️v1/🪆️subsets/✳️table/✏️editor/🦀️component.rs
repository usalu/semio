//! ✏️ Semio Table editor — thin, kit-based editor surface (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.1). `SemioTableEditor`
...
pub enum SemioTableEditCommand {
    #[default]
    SetVertex,
}

impl protocol::OpBinary for SemioTableEditCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(SemioTableEditCommand::SetVertex)
    }
}
//#endregion 🔖️Command
...
impl ArtifactEditor for SemioTableEditor {
...
pub fn create_semio_table_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(SEMIO_TABLE_DIALECT).document(["stdio", "semio"]).icon_id("box").mode_def(edit::definition()).default_mode_id(edit::SEMIO_TABLE_EDIT_MODE_ID).window_kind_def(main::definition()).default_layout(edit::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn editor_and_viewer_share_one_dialect() {
        semio_framework_plugin::testkit::assert_editor_and_viewer_share_dialect::<SemioTableEditor, crate::viewer::semio_table::SemioTableViewer>();
    }
}
//#endregion 🧪️Tests
