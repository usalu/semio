//! 🧪️ `set-frame-pixels` fixture — `⬛️repaints-the-only-frame-black`.
//!
//! Transcribed from `../../🔺️diff/🦀️.rs`, the strictest guard pair in this subset: an
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🖌️set-frame-pixels/⬛️repaints-the-only-frame-black/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🖌️set-frame-pixels/⬛️repaints-the-only-frame-black/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🖌️set-frame-pixels/⬛️repaints-the-only-frame-black/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🖌️set-frame-pixels/⬛️repaints-the-only-frame-black/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🖌️set-frame-pixels/⬛️repaints-the-only-frame-black/🎯️outcome/🔣️.json");

fn before() -> SemioImageSnapshot {
    dsl::json::from_json_str(BEFORE).expect("set-frame-pixels before snapshot decodes")
}
fn expected_after() -> SemioImageSnapshot {
    dsl::json::from_json_str(AFTER).expect("set-frame-pixels after snapshot decodes")
}
fn mutation() -> SemioImageMutation {
    dsl::json::from_json_str(MUTATION).expect("set-frame-pixels mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(mutation()))).expect("set-frame-pixels mutation encodes");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "set-frame-pixels/repaints-the-only-frame-black: produced diff differs from the committed 🔺️diff/🔣️.json");
}

/// 🔣️ The committed diff is a decode→encode fixed point and is scoped as narrowly as the leaf builds it.
#[semio_framework_async_macros::async_test]
async fn committed_diff_is_canonical_and_narrowly_scoped() {
    let decoded: SemioImageDiff = dsl::json::from_json_str(DIFF).expect("committed set-frame-pixels diff decodes");
