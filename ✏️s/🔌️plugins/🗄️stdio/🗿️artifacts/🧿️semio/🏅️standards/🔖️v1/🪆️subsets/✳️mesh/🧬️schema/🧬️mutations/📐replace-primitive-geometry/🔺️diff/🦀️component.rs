...
pub fn diff(payload: &ReplacePrimitiveGeometry, base: &SemioMeshSnapshot) -> protocol::MutationOutcome<SemioMeshDiff> {
    ...
    if primitive.positions == payload.positions && primitive.normals == payload.normals && primitive.uvs == payload.uvs && primitive.colors == payload.colors && primitive.indices == payload.indices {
        return protocol::MutationOutcome::empty().await.warn("mutation.no-op", format!("Primitive \"{}\" geometry is unchanged.", payload.primitive_id));
    }
