//! 🔺️ SemioVideoDiff — 🚧 scaffolded by W1b: a minimal, genuinely law-abiding full-replace
...
pub struct SemioVideoDiff {
    ...
    pub replacement: Option<SemioVideoSnapshot>,
}
...
pub fn diff_set_snapshot(base: &SemioVideoSnapshot, snapshot: &SemioVideoSnapshot) -> SemioVideoDiff {
    <SemioVideoDiff as DiffAlgebra<SemioVideoSnapshot>>::between(base, snapshot)
}
//#endregion 🔖️Diff
...
mod tests {
    ...
    fn field_sweep_full_replace_round_trip() {
        let a = SemioVideoSnapshot::default();
        let mut b = SemioVideoSnapshot::default();
        b.schema = format!("{}-swept", a.schema);
        let d = <SemioVideoDiff as DiffAlgebra<SemioVideoSnapshot>>::between(&a, &b);
    ...
    fn absorb_composes_two_sequential_diffs() {
        let a = SemioVideoSnapshot::default();
        let mut mid = SemioVideoSnapshot::default();
        mid.schema = format!("{}-mid", a.schema);
        let mut after = SemioVideoSnapshot::default();
        after.schema = format!("{}-after", a.schema);
        let mut d1 = <SemioVideoDiff as DiffAlgebra<SemioVideoSnapshot>>::between(&a, &mid);
        let d2 = <SemioVideoDiff as DiffAlgebra<SemioVideoSnapshot>>::between(&mid, &after);
