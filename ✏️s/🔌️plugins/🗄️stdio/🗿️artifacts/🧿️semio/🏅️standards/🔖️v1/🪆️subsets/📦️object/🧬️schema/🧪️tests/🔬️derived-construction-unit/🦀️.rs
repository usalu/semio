mod tests {
    ...
    async fn typed_constructors_build_a_populated_snapshot() {
        let snapshot = SemioObjectBuilderConstruction::new()
            ...
            .with_brep("brep-a", store::os_io::ArtifactRef { artifact_id: "brep-a".into(), dialect: store::os_io::ArtifactDialect { artifact_kind: "s.stdio.semio".into(), standard: "v1".into(), subset: "brep".into() } })
            .expect("valid exact brep child")
            .build()
            .expect("build");
        assert_eq!(snapshot.transform.translation.x, 1.0);
        assert_eq!(snapshot.brep.unwrap().child_id, "brep-a");
    }
}
