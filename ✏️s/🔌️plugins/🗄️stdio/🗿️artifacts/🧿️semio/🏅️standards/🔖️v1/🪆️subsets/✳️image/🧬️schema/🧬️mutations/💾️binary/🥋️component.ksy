...
doc: |
  ...
  ../📡️component.protocol.semio, walked by dsl::walk_protocol) for `SemioImageMutation`'s real
  binary op frame (`../../🦀️component.rs`'s `OpBinary::encode_op`/`decode_op` — NOT
...
seq:
  ...
  - id: tag
    type: u1
    doc: "SemioImageMutation variant ordinal, 0-12 — see OP_KEYWORDS in ../../🦀️component.rs"
  - id: payload
    size-eos: true
    doc: |
