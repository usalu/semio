...
fn before() -> SemioGraphSnapshot {
    dsl::json::from_json_str(BEFORE).expect("change-node-kind before snapshot decodes")
}
fn expected_after() -> SemioGraphSnapshot {
    dsl::json::from_json_str(AFTER).expect("change-node-kind after snapshot decodes")
}
fn mutation() -> SemioGraphMutation {
    dsl::json::from_json_str(MUTATION).expect("change-node-kind mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(mutation()))).expect("change-node-kind mutation encodes");
...
async fn committed_diff_is_canonical_and_omits_edges_entirely() {
    let decoded: SemioGraphDiff = dsl::json::from_json_str(DIFF).expect("committed change-node-kind diff decodes");
