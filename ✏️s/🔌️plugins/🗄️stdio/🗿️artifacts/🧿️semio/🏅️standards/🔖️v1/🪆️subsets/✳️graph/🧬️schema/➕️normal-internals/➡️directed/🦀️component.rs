...
impl DirectedGraph {
    ...
    pub async fn add_path(&mut self, nodes: &[NodeId]) -> Vec<EdgeId> {
        for &n in nodes {
            self.ensure_node(n);
        }
    ...
    pub async fn add_cycle(&mut self, nodes: &[NodeId]) -> Vec<EdgeId> {
        if nodes.is_empty() {
            return Vec::new();
        }
    ...
    pub async fn add_star(&mut self, nodes: &[NodeId]) -> Vec<EdgeId> {
        let mut ids = Vec::new();
    ...
    pub async fn set_edge_attributes(&mut self, name: &str, values: impl IntoIterator<Item = ((NodeId, NodeId), PropertyValue)>) {
    ...
    pub async fn path_weight(&self, nodes: &[NodeId]) -> Option<f64> {
    ...
    pub async fn non_neighbors(&self, node: NodeId) -> impl Iterator<Item = NodeId> + '_ {
