//! 🧬️ SemioMutation — 🚧 scaffolded by W1b: a single `SetSnapshot` full-replace mutation
...
impl Mutation<SemioSnapshot> for SemioMutation {
    ...
    fn diff(&self, _base: &SemioSnapshot) -> Self::Diff {
        match self {
            SemioMutation::NoMutation => SemioDiff::default(),
            SemioMutation::SetSnapshot { snapshot } => SemioDiff { replacement: Some(snapshot.clone()) },
        }
    }

    fn inverse(&self, base: &SemioSnapshot) -> Vec<Self> {
        match self {
            SemioMutation::NoMutation => vec![SemioMutation::NoMutation],
            SemioMutation::SetSnapshot { .. } => vec![SemioMutation::SetSnapshot { snapshot: base.clone() }],
        }
    }
}
...
pub fn apply_semio_mutation(snapshot: &mut SemioSnapshot, mutation: &SemioMutation) -> SemioDiff {
    ...
    *snapshot = <SemioDiff as protocol::MutationDiff<SemioSnapshot>>::apply(&diff, snapshot);
    diff
}
//#endregion 🔖️Mutation
...
mod tests {
    ...
    fn mutation_diff_law_set_snapshot_matches_diff() {
        let base = SemioSnapshot::default();
        let mut next = SemioSnapshot::default();
        next.schema = format!("{}-mutated", base.schema);
        let mutation = SemioMutation::SetSnapshot { snapshot: next.clone() };
        let diff = <SemioMutation as Mutation<SemioSnapshot>>::diff(&mutation, &base);
        assert_eq!(<SemioDiff as protocol::MutationDiff<SemioSnapshot>>::apply(&diff, &base), next);
        let inv = <SemioMutation as Mutation<SemioSnapshot>>::inverse(&mutation, &base);
        assert_eq!(inv.len(), 1);
        let mut round = next.clone();
        let _ = apply_semio_mutation(&mut round, &inv[0]);
    ...
    fn op_text_binary_roundtrip_law() {
        let base = SemioSnapshot::default();
        for m in [SemioMutation::NoMutation, SemioMutation::SetSnapshot { snapshot: base.clone() }] {
