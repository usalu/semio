//! 🔺️ SemioMeshDiff — 🚧 scaffolded by W1b: a minimal, genuinely law-abiding full-replace
...
pub struct SemioMeshDiff {
    ...
    pub replacement: Option<SemioMeshSnapshot>,
}
...
mod tests {
    ...
    fn field_sweep_full_replace_round_trip() {
        let a = SemioMeshSnapshot::default();
        let mut b = SemioMeshSnapshot::default();
    ...
    fn absorb_composes_two_sequential_diffs() {
        let a = SemioMeshSnapshot::default();
        let mut mid = SemioMeshSnapshot::default();
        mid.schema = format!("{}-mid", a.schema);
        let mut after = SemioMeshSnapshot::default();
