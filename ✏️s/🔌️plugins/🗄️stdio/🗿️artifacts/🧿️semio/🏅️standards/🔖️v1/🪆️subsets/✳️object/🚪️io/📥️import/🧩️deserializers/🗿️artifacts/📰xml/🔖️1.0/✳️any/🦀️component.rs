//! 📥️ `SemioObjectFromXml` — element/attribute tree -> typed object graph. More lossy than the
...
//! test). `objects` always decodes empty — XML has no id-graph/reference concept for `Ref` to come
//! from.
...
pub struct SemioObjectFromXml;

impl ArtifactDeserializer for SemioObjectFromXml {
    ...
    fn deserialize(from: &Self::From) -> Result<Self::Into, store::PackError> {
        Ok(SemioObjectSnapshot { schema: STDIO_SEMIOOBJECT_DOCUMENT_SCHEMA.into(), root: semio_value_from_xml_document(&from.doc), objects: Vec::new() })
    }
}
...
//#region 🔖️Convert
fn entry(key: &str, value: SemioValue) -> SemioObjectEntry {
    SemioObjectEntry { key: key.into(), value }
}
    ...
    fn element_with_attrs_and_children_maps_to_a_kind_tagged_structure() {
        ...
        match &value {
            SemioValue::Map { entries } => {
                assert_eq!(entries[0], SemioObjectEntry { key: "kind".into(), value: str_value("document") });
                let root = entries.iter().find(|e| e.key == "root").expect("root entry").value.clone();
                match root {
                    SemioValue::Map { entries } => {
                        assert!(entries.contains(&SemioObjectEntry { key: "tag".into(), value: str_value("svg") }));
