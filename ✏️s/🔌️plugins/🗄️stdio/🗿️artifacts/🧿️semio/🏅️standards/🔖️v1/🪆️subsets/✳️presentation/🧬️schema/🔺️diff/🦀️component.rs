...
/// pairing below.
async fn absorb_opt<T>(a: Option<T>, b: Option<T>, f: impl FnOnce(T, T) -> T) -> Option<T> {
    match (a, b) {
        (None, x) => x,
        (x, None) => x,
...
async fn diff_frame(old: &SlideFrame, new: &SlideFrame) -> Option<SlideFrameDiff> {
    if old == new {
        return None;
    }
...
async fn apply_frame(frame: &mut SlideFrame, diff: &SlideFrameDiff) {
...
async fn inverse_frame(base: &SlideFrame, diff: &SlideFrameDiff) -> SlideFrameDiff {
...
async fn absorb_frame(mut a: SlideFrameDiff, b: SlideFrameDiff) -> SlideFrameDiff {
    if b.origin.is_some() {
        a.origin = b.origin;
    }
    if b.width.is_some() {
...
async fn diff_image(old: &SlidePictureImage, new: &SlidePictureImage) -> Option<SlidePictureImageDiff> {
    if old == new {
        return None;
    }
...
async fn apply_image(image: &mut SlidePictureImage, diff: &SlidePictureImageDiff) {
...
async fn inverse_image(base: &SlidePictureImage, diff: &SlidePictureImageDiff) -> SlidePictureImageDiff {
...
async fn absorb_image(mut a: SlidePictureImageDiff, b: SlidePictureImageDiff) -> SlidePictureImageDiff {
...
async fn diff_doc_block(old: &DocBlock, new: &DocBlock) -> Option<DocBlock> {
    (old != new).then(|| new.clone())
}
async fn apply_doc_block(block: &mut DocBlock, diff: &DocBlock) {
    *block = diff.clone();
}
async fn doc_block_with_diff_applied(_block: &DocBlock, diff: &DocBlock) -> DocBlock {
    diff.clone()
}
async fn inverse_doc_block(base: &DocBlock, _diff: &DocBlock) -> DocBlock {
    base.clone()
}
async fn absorb_doc_block(_a: DocBlock, b: DocBlock) -> DocBlock {
    b
}
//#endregion 🔖️ValueDiffLogic

//#region 🔖️ShapeDiffLogic
async fn diff_shape(old: &SlideShape, new: &SlideShape) -> Option<SlideShapeDiff> {
    if old == new {
        return None;
    }
    match (old, new) {
...
async fn apply_shape(shape: &mut SlideShape, diff: &SlideShapeDiff) {
    match diff {
...
async fn apply_table_row(row: &mut SlideTableRow, diff: &SlideTableRowDiff) {
    if let Some(cd) = &diff.cells {
        apply_indexed(&mut row.cells, cd, apply_table_cell);
    }
}
async fn apply_table_cell(cell: &mut SlideTableCell, diff: &SlideTableCellDiff) {
    if let Some(bd) = &diff.blocks {
        apply_indexed(&mut cell.blocks, bd, apply_doc_block);
    }
}
async fn shape_with_diff_applied(shape: &SlideShape, diff: &SlideShapeDiff) -> SlideShape {
    let mut out = shape.clone();
    apply_shape(&mut out, diff);
    out
}
async fn table_row_with_diff_applied(row: &SlideTableRow, diff: &SlideTableRowDiff) -> SlideTableRow {
    let mut out = row.clone();
    apply_table_row(&mut out, diff);
    out
}

async fn inverse_shape(base: &SlideShape, diff: &SlideShapeDiff) -> SlideShapeDiff {
    match diff {
...
async fn inverse_table_row(base: &SlideTableRow, diff: &SlideTableRowDiff) -> SlideTableRowDiff {
    SlideTableRowDiff { cells: diff.cells.as_ref().map(|cd| inverse_indexed(&base.cells, cd, inverse_table_cell)) }
}
async fn inverse_table_cell(base: &SlideTableCell, diff: &SlideTableCellDiff) -> SlideTableCellDiff {
...
async fn apply_master(master: &mut SlideMaster, diff: &SlideMasterDiff) {
    if let Some(sd) = &diff.shapes {
        apply_indexed(&mut master.shapes, sd, apply_shape);
    }
}
async fn apply_layout(layout: &mut SlideLayout, diff: &SlideLayoutDiff) {
...
async fn apply_slide(slide: &mut Slide, diff: &SlideDiff) {
...
async fn slide_with_diff_applied(slide: &Slide, diff: &SlideDiff) -> Slide {
    let mut out = slide.clone();
    apply_slide(&mut out, diff);
    out
}

async fn inverse_master(base: &SlideMaster, diff: &SlideMasterDiff) -> SlideMasterDiff {
    SlideMasterDiff { shapes: diff.shapes.as_ref().map(|sd| inverse_indexed(&base.shapes, sd, inverse_shape)) }
}
async fn inverse_layout(base: &SlideLayout, diff: &SlideLayoutDiff) -> SlideLayoutDiff {
...
async fn inverse_slide(base: &Slide, diff: &SlideDiff) -> SlideDiff {
    SlideDiff {
...
pub async fn diff_insert_slide(index: usize, slide: Slide) -> SemioPresentationDiff {
...
pub async fn diff_remove_slide(index: usize) -> SemioPresentationDiff {
...
pub async fn diff_set_slide_layout(base: &SemioPresentationSnapshot, index: usize, layout_id: Option<String>) -> SemioPresentationDiff {
...
async fn shape_diff_frame_only(shape: &SlideShape, frame_diff: SlideFrameDiff) -> SlideShapeDiff {
    match shape {
...
pub async fn diff_insert_master(master: SlideMaster) -> SemioPresentationDiff {
...
pub async fn diff_remove_master(id: &str) -> SemioPresentationDiff {
...
pub async fn diff_insert_layout(layout: SlideLayout) -> SemioPresentationDiff {
...
pub async fn diff_remove_layout(id: &str) -> SemioPresentationDiff {
...
pub async fn diff_set_layout_master(id: &str, master_id: &str) -> SemioPresentationDiff {
...
//#region 🔖️ValueCodecs
pub(crate) async fn enc_semio_point2(p: &SemioPoint2) -> String {
...
pub(crate) async fn dec_placeholder_kind(s: &str) -> Result<PlaceholderKind, String> {
    match s {
...
pub(crate) async fn enc_shape(shape: &SlideShape) -> String {
    match shape {
...
pub(crate) async fn enc_master(m: &SlideMaster) -> String {
...
async fn enc_frame_diff(d: &SlideFrameDiff) -> String {
...
async fn enc_shape_diff(d: &SlideShapeDiff) -> String {
    match d {
...
async fn enc_master_diff(d: &SlideMasterDiff) -> String {
...
//#region 🔖️TopLevel
async fn print_presentation_diff(d: &SemioPresentationDiff) -> String {
...
async fn parse_presentation_diff(line: &str) -> Result<SemioPresentationDiff, String> {
...
#[cfg(test)]
pub(crate) async fn snapshot_a() -> SemioPresentationSnapshot {
...
#[cfg(test)]
pub(crate) async fn snapshot_b() -> SemioPresentationSnapshot {
    SemioPresentationSnapshot {
        schema: "s.stdio.semio.presentation".into(),
...
#[cfg(test)]
pub(crate) async fn demo_diff_cases() -> Vec<SemioPresentationDiff> {
    let a = snapshot_a();
    let b = snapshot_b();
