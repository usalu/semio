//! 🔺️ SemioPresentationDiff — 🚧 scaffolded by W1b: a minimal, genuinely law-abiding full-replace
...
pub struct SemioPresentationDiff {
    ...
    pub replacement: Option<SemioPresentationSnapshot>,
}
...
pub fn diff_set_snapshot(base: &SemioPresentationSnapshot, snapshot: &SemioPresentationSnapshot) -> SemioPresentationDiff {
    <SemioPresentationDiff as DiffAlgebra<SemioPresentationSnapshot>>::between(base, snapshot)
}
//#endregion 🔖️Diff
...
mod tests {
    ...
    fn field_sweep_full_replace_round_trip() {
        let a = SemioPresentationSnapshot::default();
        let mut b = SemioPresentationSnapshot::default();
        b.schema = format!("{}-swept", a.schema);
        let d = <SemioPresentationDiff as DiffAlgebra<SemioPresentationSnapshot>>::between(&a, &b);
    ...
    fn absorb_composes_two_sequential_diffs() {
        let a = SemioPresentationSnapshot::default();
        let mut mid = SemioPresentationSnapshot::default();
        mid.schema = format!("{}-mid", a.schema);
        let mut after = SemioPresentationSnapshot::default();
        after.schema = format!("{}-after", a.schema);
        let mut d1 = <SemioPresentationDiff as DiffAlgebra<SemioPresentationSnapshot>>::between(&a, &mid);
        let d2 = <SemioPresentationDiff as DiffAlgebra<SemioPresentationSnapshot>>::between(&mid, &after);
