//! 🧬️ SemioDrawingSnapshot — a recursive DrawNode{Group/Path/Text} scene graph — from svg; replaces DwgDrawing-as-neutral.
...
/// 🖍️ Owned by the `drawing` subset: `DrawNode` — a recursive scene graph, matching svg's
/// `SvgNodeDiff` recursive-diff template per the master plan.
#[derive(Clone, Debug, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(tag = "kind", rename_all = "camelCase")]
pub enum DrawNode {
    Group { children: Vec<DrawNode> },
    Path { d: String },
    Text { value: String, at: SemioPoint2 },
}

impl Default for DrawNode {
    fn default() -> Self { DrawNode::Group { children: Vec::new() } }
}
