//! 📥️ `SemioObjectFromCsv` — tabular rows -> a `List` of `Map`s. A genuine SHAPE mismatch,
...
//! - `objects` always decodes empty — CSV has no graph/reference concept.
...
pub struct SemioObjectFromCsv;

impl ArtifactDeserializer for SemioObjectFromCsv {
    ...
    fn deserialize(from: &Self::From) -> Result<Self::Into, store::PackError> {
        Ok(SemioObjectSnapshot { schema: STDIO_SEMIOOBJECT_DOCUMENT_SCHEMA.into(), root: semio_value_from_csv(from), objects: Vec::new() })
    }
}
        ...
        let items = records
            .map(|record| {
                let entries = header
                    ...
                    .map(|(key, field)| SemioObjectEntry { key: key.clone(), value: SemioValue::Str { value: field.value.clone() } })
                    .collect();
    ...
    fn header_rows_become_a_list_of_keyed_maps() {
        ...
        match value {
            SemioValue::List { items } => {
                ...
                assert_eq!(items[0], SemioValue::Map { entries: vec![SemioObjectEntry { key: "name".into(), value: SemioValue::Str { value: "Ada".into() } }, SemioObjectEntry { key: "age".into(), value: SemioValue::Str { value: "36".into() } }] });
            }
