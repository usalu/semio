...
pub mod derived_composition {
    ...
    impl ArtifactComposition for SemioKitComposerComposition {
        ...
        fn compose(sources: &[ComposeSource]) -> Result<Composition<Self::Snapshot>, ComposeError> {
            ...
            let snapshot = analysis.parts.snapshot.ok_or_else(|| ComposeError {
                message: "SemioKitComposerComposition: analysis produced no snapshot".into(),
        ...
        fn validate(payload: &IoPayload) -> Vec<dsl::Diagnostic> {
            ...
            let Some(snapshot) = decoded else {
                return vec![dsl::Diagnostic::error(
                    "stdio.semio_kit.validate-decode-failed",
                    dsl::TextSpan::at(1, 1),
                    "SemioKitValidator: payload did not decode as a SemioKitSnapshot".to_string(),
                )];
            };
            let mut diagnostics = Vec::new();
            for object in &snapshot.objects { diagnostics.extend(wrong_kind("objects", "object", &object.target)); }
            for model in &snapshot.models { diagnostics.extend(wrong_kind("models", "model", &model.target)); }
    ...
    fn validator_entry() -> &'static SubsetValidatorEntry { VALIDATOR_ENTRY.get_or_init(subset_validator_entry_of::<SemioKitValidator>) }
    ...
    pub fn register() {
        ...
        store::register_document_codec(store::ArtifactCodec::of::<SemioKitSnapshot, crate::artifacts::semio::standards::v1::subsets::kit::schema::mutations::SemioKitMutation>(crate::artifacts::semio::standards::v1::subsets::kit::schema::snapshot::STDIO_SEMIOKIT_DOCUMENT_SCHEMA));
    ...
    mod tests {
        ...
        mod conformance_laws {
            ...
            fn fixture_honesty_law() {
                const FIXTURE_DSL: &str = include_str!("../📚️examples/🪑️furniture/🖼️assets/🗣️example.dsl.semio");
