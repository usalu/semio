//! 🏗️ Writes the `semio@v1/drawing` JSON-carrier fixture corpus. Every pair is checked for
//! OBSERVABILITY before it is written — a mutation whose projection does not move is refused, not
//! committed.
use std::fs;
use std::path::PathBuf;
use std::process::exit;

use semio_drawing_json::{apply, build_seed, project, KINDS};

fn fixture_directory(kind: &str) -> Option<&'static str> {
    match kind {
        "create-layer" => Some("🗂️create-layer-applied"),
        _ => None,
    }
}
