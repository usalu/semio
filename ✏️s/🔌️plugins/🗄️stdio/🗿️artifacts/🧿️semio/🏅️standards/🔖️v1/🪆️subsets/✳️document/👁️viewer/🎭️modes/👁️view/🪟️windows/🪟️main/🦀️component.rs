//! 🧊 Semio Document viewer — the Main window: a read-only world-3d mesh scene built with the shared
//! `MeshWindowKit` (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.6), from the
//! artifact-level `SemioDocumentSnapshot` — this file imports nothing from the sibling editor module
...
const SEMIO_DOCUMENT_VIEW_FALLBACK_MESH_KIND: &str = "box";
const SEMIO_DOCUMENT_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const SEMIO_DOCUMENT_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const SEMIO_DOCUMENT_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
    ...
    let instances: Vec<serde_json::Value> = (0..count.await)
        .map(|index| {
            serde_json::json!({
                ...
                "meshId": SEMIO_DOCUMENT_VIEW_FALLBACK_MESH_KIND,
...
/// 👁️ Pure `SemioDocumentSnapshot -> UiNode` read: default camera (a viewer has no persisted
...
pub async fn render(document: &SemioDocumentSnapshot) -> UiNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": SEMIO_DOCUMENT_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(SEMIO_DOCUMENT_VIEW_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(SEMIO_DOCUMENT_VIEW_DEFAULT_CAMERA_POSITION, SEMIO_DOCUMENT_VIEW_DEFAULT_CAMERA_TARGET, SEMIO_DOCUMENT_VIEW_DEFAULT_CAMERA_FOV).await,
        meshes_json,
