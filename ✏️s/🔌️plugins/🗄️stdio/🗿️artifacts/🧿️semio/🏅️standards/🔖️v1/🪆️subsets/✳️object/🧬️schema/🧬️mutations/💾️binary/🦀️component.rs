...
//! follows as one opaque trailing `bytes` chain — reuses the already-real `../📝️text/🦀️component.rs`
...
mod tests {
    ...
    async fn op_binary_roundtrip_law() {
        for m in demo_mutation_cases() {
            ...
            let decoded = SemioObjectMutation::decode_op(&encoded).await.unwrap_or_else(|e| panic!("decode_op failed: {e}"));
