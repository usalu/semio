...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✏️edit-run/✍️rewrites-the-marked-runs-content/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✏️edit-run/✍️rewrites-the-marked-runs-content/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✏️edit-run/✍️rewrites-the-marked-runs-content/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✏️edit-run/✍️rewrites-the-marked-runs-content/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✏️edit-run/✍️rewrites-the-marked-runs-content/🎯️outcome/🔣️.json");

fn before() -> SemioTextSnapshot {
    dsl::json::from_json_str(BEFORE).expect("edit-run before snapshot decodes")
}
fn expected_after() -> SemioTextSnapshot {
    dsl::json::from_json_str(AFTER).expect("edit-run after snapshot decodes")
}
fn edit_run() -> SemioTextMutation {
    dsl::json::from_json_str(MUTATION).expect("edit-run mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(edit_run()))).expect("edit-run mutation encodes");
...
async fn committed_diff_is_canonical() {
    let decoded: SemioTextDiff = dsl::json::from_json_str(DIFF).expect("committed edit-run diff decodes");
