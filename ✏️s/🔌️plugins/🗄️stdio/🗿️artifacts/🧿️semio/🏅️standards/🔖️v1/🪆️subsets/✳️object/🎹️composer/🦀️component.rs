...
mod tests {
    ...
    /// "engine-equivalent" home — `object` has no per-subset `⚙️engine/` dir, only the SHARED
    ...
    mod conformance_laws {
        ...
        fn grammar_conformance_law() {
            ...
            let text = store::ArtifactDsl::print_dsl(&snapshot::demo_semio_object_snapshot());
        ...
        /// output for every `SemioObjectMutation` variant (`mutations::demo_mutation_cases()`),
        /// incl. nested list/map payload values and a multi-segment mixed `SemioObjectPath`.
        #[test]
        ...
        /// for every representative `SemioObjectDiff` (`diff::demo_diff_cases()`), incl. the empty
        ...
        fn protocol_walk_law() {
            ...
            let packed = store::ArtifactPack::encode_pack(&snapshot::demo_semio_object_snapshot());
        ...
        /// `print_dsl`/`encode_pack` output of `snapshot::demo_semio_object_snapshot()` —
        ...
        fn fixture_honesty_law() {
            const FIXTURE_DSL: &str = include_str!("../../../../../📚️examples/🕸️graph/🖼️assets/🗣️example.dsl.semio");
            const FIXTURE_PACK: &[u8] = include_bytes!("../../../../../📚️examples/🕸️graph/🖼️assets/🎒️example.pack.semio");

            let demo = snapshot::demo_semio_object_snapshot();
            ...
            assert_eq!(parsed, demo, "shipped .dsl.semio fixture does not parse back to demo_semio_object_snapshot()");
            assert_eq!(store::ArtifactDsl::print_dsl(&demo), FIXTURE_DSL, "print_dsl(demo_semio_object_snapshot()) drifted from the shipped .dsl.semio fixture");
            ...
            assert_eq!(decoded, demo, "shipped .pack.semio fixture does not decode back to demo_semio_object_snapshot()");
            assert_eq!(store::ArtifactPack::encode_pack(&demo), FIXTURE_PACK, "encode_pack(demo_semio_object_snapshot()) drifted from the shipped .pack.semio fixture");
        }
    }
