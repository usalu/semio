...
pub(crate) fn dec_pin(s: &str) -> Result<store::LinkPin, String> {
    ...
    match parts.as_slice() {
        ...
        [tag, hash, size, media_type] if *tag == "s" => Ok(store::LinkPin::Snapshot {
            blob: store::BlobRef { hash: dec_str(hash)?, size: size.trim().parse().map_err(|e: std::num::ParseIntError| e.to_string())?, media_type: dec_str(media_type)? },
        }),
...
pub(crate) fn enc_type(t: &SemioKitType) -> String { format!("[{},{},{}]", enc_str(&t.id), enc_str(&t.name), enc_str(&t.category)) }
...
pub(crate) fn enc_type_list(list: &[SemioKitType]) -> String { format!("[{}]", list.iter().map(enc_type).collect::<Vec<_>>().join(",")) }
...
pub(crate) fn enc_piece(p: &SemioKitPiece) -> String { format!("[{},{},{}]", enc_str(&p.id), enc_str(&p.type_id), enc_transform(&p.transform)) }
...
pub(crate) fn enc_design_list(list: &[SemioKitDesign]) -> String { format!("[{}]", list.iter().map(enc_design).collect::<Vec<_>>().join(",")) }
...
fn print_kit_snapshot_body(s: &SemioKitSnapshot) -> String {
    format!(
        "schema={}\ntypes={}\ndesigns={}\nobjects={}\nmodels={}\nproperties={}\nrepresentations={}",
        enc_str(&s.schema), enc_type_list(&s.types), enc_design_list(&s.designs),
        enc_child_list(&s.objects), enc_child_list(&s.models),
        enc_child_opt(&s.properties), enc_link_list(&s.representations),
    )
}
fn parse_kit_snapshot_body(body: &str) -> Result<SemioKitSnapshot, String> {
    ...
    for line in body.lines() {
        ...
        else if let Some(rest) = line.strip_prefix("types=") { types = dec_type_list(rest)?; }
        else if let Some(rest) = line.strip_prefix("designs=") { designs = dec_design_list(rest)?; }
        else if let Some(rest) = line.strip_prefix("objects=") { objects = dec_child_list(rest)?; }
        else if let Some(rest) = line.strip_prefix("models=") { models = dec_child_list(rest)?; }
        else if let Some(rest) = line.strip_prefix("properties=") { properties = dec_child_opt(rest)?; }
        else if let Some(rest) = line.strip_prefix("representations=") { representations = dec_link_list(rest)?; }
        else { return Err(format!("semio kit snapshot: unknown line {line:?}")); }
    }
    Ok(SemioKitSnapshot {
        schema: schema.ok_or_else(|| "semio kit snapshot: missing schema line".to_string())?,
        types, designs, objects, models, properties, representations,
    })
}
...
pub(crate) fn read_ref(reader: &mut store::ByteReader<'_>) -> Result<store::os_io::ArtifactRef, String> { store::os_io::ArtifactRef::parse_uri(&read_str_lp(reader)?) }

pub(crate) fn write_child<S>(out: &mut Vec<u8>, c: &store::ArtifactChild<S>) { write_str_lp(out, &c.child_id); write_ref(out, &c.target); }
...
pub(crate) fn write_child_list<S>(out: &mut Vec<u8>, list: &[store::ArtifactChild<S>]) {
    ...
    for c in list { write_child(out, c); }
}
...
pub(crate) fn write_child_opt<S>(out: &mut Vec<u8>, c: &Option<store::ArtifactChild<S>>) {
    match c { Some(c) => { out.push(1); write_child(out, c); } None => out.push(0) }
}
...
pub(crate) fn write_pin(out: &mut Vec<u8>, p: &store::LinkPin) {
    match p {
        ...
        store::LinkPin::Checkpoint { id } => { out.push(1); write_str_lp(out, id); }
        store::LinkPin::Snapshot { blob } => { out.push(2); write_str_lp(out, &blob.hash); store::pack_rt::write_varint_u64(out, blob.size); write_str_lp(out, &blob.media_type); }
    }
}
...
pub(crate) fn write_link(out: &mut Vec<u8>, l: &store::ArtifactLink) { write_ref(out, &l.target); write_pin(out, &l.pin); write_str_lp(out, &l.role); }
...
pub(crate) fn write_link_list(out: &mut Vec<u8>, list: &[store::ArtifactLink]) {
    store::pack_rt::write_varint_u64(out, list.len() as u64);
    for l in list { write_link(out, l); }
}
...
pub(crate) fn write_type(out: &mut Vec<u8>, t: &SemioKitType) { write_str_lp(out, &t.id); write_str_lp(out, &t.name); write_str_lp(out, &t.category); }
...
pub(crate) fn write_type_list(out: &mut Vec<u8>, list: &[SemioKitType]) {
    store::pack_rt::write_varint_u64(out, list.len() as u64);
    for t in list { write_type(out, t); }
}
...
pub(crate) fn write_piece(out: &mut Vec<u8>, p: &SemioKitPiece) { write_str_lp(out, &p.id); write_str_lp(out, &p.type_id); write_transform(out, &p.transform); }
...
pub(crate) fn write_connection(out: &mut Vec<u8>, c: &SemioKitConnection) {
    write_str_lp(out, &c.id); write_str_lp(out, &c.connecting_piece_id); write_str_lp(out, &c.connecting_port); write_str_lp(out, &c.connected_piece_id); write_str_lp(out, &c.connected_port);
}
pub(crate) fn read_connection(reader: &mut store::ByteReader<'_>) -> Result<SemioKitConnection, String> {
    Ok(SemioKitConnection {
        id: read_str_lp(reader)?, connecting_piece_id: read_str_lp(reader)?, connecting_port: read_str_lp(reader)?,
        connected_piece_id: read_str_lp(reader)?, connected_port: read_str_lp(reader)?,
    })
}
pub(crate) fn write_design(out: &mut Vec<u8>, d: &SemioKitDesign) {
    ...
    for p in &d.pieces { write_piece(out, p); }
    store::pack_rt::write_varint_u64(out, d.connections.len() as u64);
    for c in &d.connections { write_connection(out, c); }
}
...
pub(crate) fn write_design_list(out: &mut Vec<u8>, list: &[SemioKitDesign]) {
    store::pack_rt::write_varint_u64(out, list.len() as u64);
    for d in list { write_design(out, d); }
}
...
impl store::ArtifactDsl for SemioKitSnapshot {
    ...
    fn envelope_id() -> &'static str { STDIO_SEMIOKIT_DOCUMENT_SCHEMA }
...
pub(crate) fn demo_kit_snapshot() -> SemioKitSnapshot {
    ...
    SemioKitSnapshot {
        ...
        designs: vec![SemioKitDesign {
            ...
            pieces: vec![
                SemioKitPiece { id: "piece-1".into(), type_id: "chair".into(), transform: SemioTransform::identity() },
                SemioKitPiece { id: "piece-2".into(), type_id: "chair".into(), transform: SemioTransform::identity() },
            ],
        ...
        representations: vec![store::ArtifactLink {
            target: store::os_io::ArtifactRef { artifact_id: "chair-repr".into(), dialect: dialect("mesh") },
...
mod tests {
    ...
    #[test]
    fn parent_snapshot_stores_only_handles_never_child_content() {
        let snap = demo_kit_snapshot();
        let text = <SemioKitSnapshot as store::ArtifactDsl>::print_dsl(&snap);
