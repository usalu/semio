...
//!
//! Transcribed from `../../🔺️diff/🦀️component.rs`: an unknown solid id is Error
...
fn before() -> SemioBrepSnapshot {
    serde_json::from_str(BEFORE).expect("delete-solid before snapshot decodes")
}
fn expected_after() -> SemioBrepSnapshot {
    serde_json::from_str(AFTER).expect("delete-solid after snapshot decodes")
}
fn mutation() -> SemioBrepMutation {
    serde_json::from_str(MUTATION).expect("delete-solid mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::to_value(mutation()).expect("delete-solid mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("delete-solid mutation reparses");
...
async fn declared_outcome_holds_as_committed() {
    ...
    assert_eq!(outcome.get("status").and_then(serde_json::Value::as_str), Some("applied"), "delete-solid/removes-the-only-solid-and-leaves-its-shell-behind: this case is declared applied");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-solid/removes-the-only-solid-and-leaves-its-shell-behind: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
