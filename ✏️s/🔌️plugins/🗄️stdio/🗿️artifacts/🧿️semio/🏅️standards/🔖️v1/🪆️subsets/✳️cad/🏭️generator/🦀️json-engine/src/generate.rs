//! 🏭️ Writes the `semio@v1/cad` JSON-carrier fixture corpus. Every pair is checked for OBSERVABILITY
//! before it is written — a mutation whose projection does not move is refused, not committed.
use std::fs;
use std::path::PathBuf;
use std::process::exit;

use semio_cad_json::{apply, build_seed, project, KINDS};
...
fn main() {
    ...
    for kind in KINDS {
        ...
        let dir = PathBuf::from(&out_root).join(format!("{kind}-applied"));
