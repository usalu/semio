//! 🧊 Semio Animation editor — the Main window: the SAME world-3d mesh scene shape as the sibling
...
//! artifact-level `SemioAnimationSnapshot` — this window itself never mutates; mutation is the surface
...
const SEMIO_ANIMATION_EDIT_FALLBACK_MESH_KIND: &str = "box";
const SEMIO_ANIMATION_EDIT_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const SEMIO_ANIMATION_EDIT_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const SEMIO_ANIMATION_EDIT_DEFAULT_CAMERA_FOV: f64 = 45.0;
    ...
    let instances: Vec<pack::JsonValue> = (0..count)
        .map(|index| {
            pack::json!({
                ...
                "meshId": SEMIO_ANIMATION_EDIT_FALLBACK_MESH_KIND,
...
pub fn render(document: &SemioAnimationSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let mesh_data_value = dsl::to_dsl_value(&mesh_from_kind(SEMIO_ANIMATION_EDIT_FALLBACK_MESH_KIND)).unwrap_or(dsl::DslValue::Null);
    let meshes_json = pack::json_to_string(&pack::JsonValue::Array(vec![pack::json!({ "id": SEMIO_ANIMATION_EDIT_FALLBACK_MESH_KIND, "data": pack::json_from_dsl_value(&mesh_data_value) })]));
    let view = MeshView {
        camera_json: world3d_camera_json(SEMIO_ANIMATION_EDIT_DEFAULT_CAMERA_POSITION, SEMIO_ANIMATION_EDIT_DEFAULT_CAMERA_TARGET, SEMIO_ANIMATION_EDIT_DEFAULT_CAMERA_FOV),
        meshes_json,
