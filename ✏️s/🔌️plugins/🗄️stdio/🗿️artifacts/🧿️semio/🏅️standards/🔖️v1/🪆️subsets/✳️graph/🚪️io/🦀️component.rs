...
pub mod derived_composition {
    ...
    impl ArtifactComposition for SemioGraphComposerComposition {
        ...
        async fn compose(sources: &[ComposeSource<'_>]) -> Result<Composition<Self::Snapshot>, ComposeError> {
            ...
            let analysis = SemioGraphAnalyzer::analyze(&native).await;
    ...
    impl SubsetValidator for SemioGraphValidator {
        ...
        async fn validate(payload: &IoPayload) -> Vec<dsl::Diagnostic> {
            let decoded = match payload {
                IoPayload::Binary(bytes) => <SemioGraphSnapshot as store::ArtifactPack>::decode_pack(bytes).await.ok(),
                IoPayload::Text(text) => <SemioGraphSnapshot as store::ArtifactDsl>::parse_dsl(text).await.ok(),
            };
            match decoded {
                Some(_) => Vec::new(),
    ...
    mod tests {
        ...
        mod conformance_laws {
            ...
            async fn fixture_honesty_law() {
                ...
                let parsed = <snapshot::SemioGraphSnapshot as store::ArtifactDsl>::parse_dsl(FIXTURE_DSL).await.expect("parse shipped .dsl.semio fixture");
                ...
                let decoded = <snapshot::SemioGraphSnapshot as store::ArtifactPack>::decode_pack(FIXTURE_PACK).await.expect("decode shipped .pack.semio fixture");
