...
pub fn create_semio_cad_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(SEMIO_CAD_DIALECT).document(["stdio", "semio"]).icon_id("box").mode_def(view::definition()).default_mode_id(view::SEMIO_CAD_VIEW_MODE_ID).window_kind_def(main::definition()).default_layout(view::layout()).build_definition()
}
//#endregion 🔖️Manifest
