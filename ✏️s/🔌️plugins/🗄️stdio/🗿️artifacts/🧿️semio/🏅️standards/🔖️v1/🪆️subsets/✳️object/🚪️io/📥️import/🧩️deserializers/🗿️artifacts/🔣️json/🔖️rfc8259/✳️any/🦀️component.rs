//! 📥️ `SemioObjectFromJson` — the cleanest pair in this wave: `object`'s `SemioValue` was
...
//!
//! - `Null`/`Bool`/`String`/`Array`/`Object` map 1:1 onto `Null`/`Bool`/`Str`/`List`/`Map`.
...
//!   by this direction (the `objects` backing store always decodes empty) — see the serializer's
//!   own doc comment for what happens going the other way.

use crate::artifacts::json::JsonSnapshot;
use crate::artifacts::json::schema::snapshot::JsonValue;
use crate::artifacts::semio::standards::v1::subsets::object::schema::snapshot::{SemioObjectEntry, SemioObjectSnapshot, SemioValue, STDIO_SEMIOOBJECT_DOCUMENT_SCHEMA};
use semio_framework_plugin::{ArtifactDeserializer, Dialect, StandardId, SubsetId};

//#region 🔖️Deserializer
pub struct SemioObjectFromJson;

impl ArtifactDeserializer for SemioObjectFromJson {
    type From = JsonSnapshot;
    type Into = SemioObjectSnapshot;
    const FROM: Dialect = Dialect { artifact_kind: "s.stdio.json", standard: StandardId("rfc8259"), subset: SubsetId::ANY };
    const INTO: Dialect = Dialect { artifact_kind: "s.stdio.semio", standard: StandardId("v1"), subset: SubsetId("object") };

    fn deserialize(from: &Self::From) -> Result<Self::Into, store::PackError> {
        Ok(SemioObjectSnapshot { schema: STDIO_SEMIOOBJECT_DOCUMENT_SCHEMA.into(), root: semio_value_from_json(&from.value), objects: Vec::new() })
    }
}
...
pub fn semio_value_from_json(v: &JsonValue) -> SemioValue {
    match v {
        ...
        JsonValue::Object { members } => SemioValue::Map { entries: members.iter().map(|m| SemioObjectEntry { key: m.key.clone(), value: semio_value_from_json(&m.value) }).collect() },
    }
}
//#endregion 🔖️Convert
