...
//!
//! Transcribed from `../../🔺️diff/🦀️component.rs`: an out-of-range BASE `from` is
...
async fn the_undo_reorder_moves_the_run_back_to_the_head() {
    ...
    assert_eq!(undo, vec![SemioTextMutation::ReorderRuns(crate::artifacts::semio::standards::v1::subsets::text::schema::mutations::reorder_runs::mutation::ReorderRuns { from: 2, to: 0 })], "the undo must address the landed index #2 and send it back to #0");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "reorder-runs/moves-the-first-run-to-the-end: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
