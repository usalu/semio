//! 🧊 Semio Table viewer — the Main window: a read-only world-3d mesh scene built with the shared
//! `MeshWindowKit` (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.6), from the
//! artifact-level `SemioTableSnapshot` — this file imports nothing from the sibling editor module
...
const SEMIO_TABLE_VIEW_FALLBACK_MESH_KIND: &str = "box";
const SEMIO_TABLE_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const SEMIO_TABLE_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const SEMIO_TABLE_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
...
async fn entity_count(document: &SemioTableSnapshot) -> usize {
...
async fn world_instances_json(document: &SemioTableSnapshot) -> String {
    let count = entity_count(document);
    let instances: Vec<serde_json::Value> = (0..count.await)
        .map(|index| {
            serde_json::json!({
                "id": format!("semio_table-{index}"),
                "meshId": SEMIO_TABLE_VIEW_FALLBACK_MESH_KIND,
...
/// 👁️ Pure `SemioTableSnapshot -> UiNode` read: default camera (a viewer has no persisted
/// per-session camera — `Config = NoConfig`), no selection/gumball/engagement overlay.
pub async fn render(document: &SemioTableSnapshot) -> UiNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": SEMIO_TABLE_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(SEMIO_TABLE_VIEW_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(SEMIO_TABLE_VIEW_DEFAULT_CAMERA_POSITION, SEMIO_TABLE_VIEW_DEFAULT_CAMERA_TARGET, SEMIO_TABLE_VIEW_DEFAULT_CAMERA_FOV).await,
        meshes_json,
