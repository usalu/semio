//! 🔍️ The READER: projects a committed semio_document JSON carrier through `json` (json-rust) and compares
...
use semio_document_json::project;
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        Some("project") => match read(args.get(2).expect("usage: reader project <file.json>")) {
            Ok(value) => println!("{}", report("semio-document-json-project", "ok", value)),
            Err(error) => {
                println!("{}", report("semio-document-json-project", "failed", object! {"error": error}));
                exit(1);
            }
        },
        Some("compare") => {
            ...
            match (read(expected), read(actual)) {
                (Ok(left), Ok(right)) => println!("{}", report("semio-document-json-compare", "ok", object! {"equal": left == right, "expected": left, "actual": right})),
                (Err(error), _) | (_, Err(error)) => {
                    println!("{}", report("semio-document-json-compare", "failed", object! {"error": error}));
                    exit(1);
                }
            }
        }
