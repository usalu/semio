...
pub fn intersect_curve_curve(a: &Curve3, b: &Curve3, tol: f64) -> Result<Vec<CurveCurveHit>, IntersectError> {
    ...
    match (a, b) {
        (Curve3::Line { origin: o1, dir: d1 }, Curve3::Line { origin: o2, dir: d2 }) => {
            intersect_line_line(*o1, *d1, *o2, *d2, tol)
        }
        (Curve3::Line { origin, dir }, Curve3::Circle { frame, radius }) => {
            intersect_line_circle(*origin, *dir, frame, *radius, tol, false)
        }
        (Curve3::Circle { frame, radius }, Curve3::Line { origin, dir }) => {
            intersect_line_circle(*origin, *dir, frame, *radius, tol, true)
        }
        (Curve3::Circle { frame: f1, radius: r1 }, Curve3::Circle { frame: f2, radius: r2 }) => {
            intersect_circle_circle(f1, *r1, f2, *r2, tol)
        }
...
fn intersect_line_line(o1: Pnt3, d1: Vec3, o2: Pnt3, d2: Vec3, tol: f64) -> Result<Vec<CurveCurveHit>, IntersectError> {
    ...
    Ok(vec![CurveCurveHit {
        point: Pnt3::new((p_a.x + p_b.x) * 0.5, (p_a.y + p_b.y) * 0.5, (p_a.z + p_b.z) * 0.5),
        t_a,
        t_b,
    }])
}
...
fn intersect_line_circle(
    ...
    tol: f64,
    swap: bool,
) -> Result<Vec<CurveCurveHit>, IntersectError> {
...
fn intersect_line2_circle(
    ...
    if a <= tol * tol {
        return Err(IntersectError::Degenerate(
            "line direction parallel to circle normal with zero in-plane speed".into(),
        ));
    }
...
fn pack_hit(point: Pnt3, t_line: f64, t_circle: f64, swap: bool) -> CurveCurveHit {
    if swap {
...
fn intersect_circle_circle(
    ...
    if f1.z.cross(f2.z).norm() > tol {
        return intersect_general(
            &Curve3::Circle { frame: *f1, radius: r1 },
            &Curve3::Circle { frame: *f2, radius: r2 },
            tol,
        );
    }
    let c1 = f1.origin;
...
fn clip_pair(
    bez_a: &RationalBezier3,
    a0: f64,
    a1: f64,
    bez_b: &RationalBezier3,
    b0: f64,
    b1: f64,
    curve_a: &Curve3,
    curve_b: &Curve3,
...
fn newton_refine(a: &Curve3, b: &Curve3, mut t_a: f64, mut t_b: f64, tol: f64) -> Option<CurveCurveHit> {
    ...
    if pa.distance(pb) <= tol * 10.0 {
        Some(CurveCurveHit {
            point: Pnt3::new((pa.x + pb.x) * 0.5, (pa.y + pb.y) * 0.5, (pa.z + pb.z) * 0.5),
            t_a,
            t_b,
        })
    } else {
...
mod tests {
    ...
    #[test]
    fn perpendicular_lines_meet_at_origin() {
        let a = Curve3::Line {
            origin: Pnt3::new(-1.0, 0.0, 0.0),
            dir: Vec3::new(1.0, 0.0, 0.0),
        };
        let b = Curve3::Line {
            origin: Pnt3::new(0.0, -1.0, 0.0),
    ...
    #[test]
    fn unit_circle_with_diameter_line() {
    ...
    #[test]
    fn line_circle_order_preserves_parameters() {
        let circle = Curve3::Circle {
            frame: Frame3::WORLD,
            radius: 1.0,
        };
