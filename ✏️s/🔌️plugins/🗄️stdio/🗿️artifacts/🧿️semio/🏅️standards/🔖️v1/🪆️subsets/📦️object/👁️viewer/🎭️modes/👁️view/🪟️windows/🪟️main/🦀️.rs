//! 🧊 Semio Object viewer — the Main window: a read-only world-3d mesh scene built with the shared
//! `MeshWindowKit` (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.6), from the
//! artifact-level `SemioObjectSnapshot` — this file imports nothing from the sibling editor module
...
const SEMIO_OBJECT_VIEW_FALLBACK_MESH_KIND: &str = "box";
const SEMIO_OBJECT_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const SEMIO_OBJECT_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const SEMIO_OBJECT_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
fn entity_count(document: &SemioObjectSnapshot) -> usize {
...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
fn world_instances_json(document: &SemioObjectSnapshot) -> String {
    let count = entity_count(document);
    let instances: Vec<serde_json::Value> = (0..count)
        .map(|index| {
            serde_json::json!({
                "id": format!("semio_object-{index}"),
                "meshId": SEMIO_OBJECT_VIEW_FALLBACK_MESH_KIND,
                "position": [index as f64 * 2.0, 0.0, 0.0],
                "rotation": [0.0, 0.0, 0.0, 1.0],
                "scale": [1.0, 1.0, 1.0],
                "label": format!("Semio Object {index}"),
                "smoothShading": false,
            })
        })
        .collect();
...
/// 👁️ Pure `SemioObjectSnapshot -> BuiltNode` read: default camera (a viewer has no persisted
/// per-session camera — `Config = NoConfig`), no selection/gumball/engagement overlay.
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn render(document: &SemioObjectSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = world3d_meshes_json_from_kinds(&[SEMIO_OBJECT_VIEW_FALLBACK_MESH_KIND.to_string()]);
    let view = MeshView {
        camera_json: world3d_camera_json(SEMIO_OBJECT_VIEW_DEFAULT_CAMERA_POSITION, SEMIO_OBJECT_VIEW_DEFAULT_CAMERA_TARGET, SEMIO_OBJECT_VIEW_DEFAULT_CAMERA_FOV),
        meshes_json,
...
mod tests {
    ...
    async fn render_produces_a_scene_node_for_the_default_document() {
        let document = SemioObjectSnapshot::default();
