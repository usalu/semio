//! 🧪️ `📸️set-snapshot` fixture — `🔥️slides-the-wall-and-attaches-a-fire-rating-pset`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📸️set-snapshot/🔥️slides-the-wall-and-attaches-a-fire-rating-pset/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📸️set-snapshot/🔥️slides-the-wall-and-attaches-a-fire-rating-pset/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📸️set-snapshot/🔥️slides-the-wall-and-attaches-a-fire-rating-pset/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📸️set-snapshot/🔥️slides-the-wall-and-attaches-a-fire-rating-pset/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📸️set-snapshot/🔥️slides-the-wall-and-attaches-a-fire-rating-pset/🎯️outcome/🔣️.json");

fn before() -> SemioModelSnapshot {
    dsl::json::from_json_str(BEFORE).expect("before model snapshot decodes")
}
fn expected_after() -> SemioModelSnapshot {
    dsl::json::from_json_str(AFTER).expect("after model snapshot decodes")
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: SemioModelSnapshot = dsl::json::from_json_str(text).expect("model snapshot decodes");
        let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("model snapshot encodes");
...
async fn produces_committed_diff() {
    ...
    let produced = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(outcome.diff())).expect("produced model diff encodes");
...
async fn committed_diff_is_canonical() {
    let decoded: SemioModelDiff = dsl::json::from_json_str(DIFF).expect("committed model diff decodes");
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("model diff re-encodes");
