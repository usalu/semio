    ...
    match node_at(base, &payload.parent) {
        Some(DrawNode::Group { children, .. }) if payload.from < children.len() => {
            if payload.from == payload.to {
                return protocol::MutationOutcome::empty().await.warn("mutation.no-op", format!("Node #{} in layer #{} is already at position #{}.", payload.from, payload.parent.layer, payload.to));
            }
