...
//! nest here rather than adding new entries to stdio's `📦️glue.rs`, mirroring the `⚙️engine` →
//! `📄️step`/`📦️mesh-io` local-mount pattern from wave PEEL3).

// #region 🔖️Submodules

#[path = "🔢️matrix/🦀️component.rs"]
pub mod matrix;
#[path = "⚖️predicates/🦀️component.rs"]
pub mod predicates;
...
impl std::ops::Add for Vec2 {
    type Output = Vec2;
    async fn add(self, o: Vec2) -> Vec2 {
        Vec2::new(self.x + o.x, self.y + o.y)
    }
}
impl std::ops::Sub for Vec2 {
    type Output = Vec2;
    async fn sub(self, o: Vec2) -> Vec2 {
        Vec2::new(self.x - o.x, self.y - o.y)
    }
}
impl std::ops::Mul<f64> for Vec2 {
    type Output = Vec2;
    async fn mul(self, s: f64) -> Vec2 {
        Vec2::new(self.x * s, self.y * s)
    }
}
impl std::ops::Neg for Vec2 {
    type Output = Vec2;
    async fn neg(self) -> Vec2 {
...
impl std::ops::Sub for Pnt2 {
    type Output = Vec2;
    async fn sub(self, o: Pnt2) -> Vec2 {
        Vec2::new(self.x - o.x, self.y - o.y)
    }
}
impl std::ops::Add<Vec2> for Pnt2 {
    type Output = Pnt2;
    async fn add(self, v: Vec2) -> Pnt2 {
...
impl std::ops::Add for Vec3 {
    type Output = Vec3;
    async fn add(self, o: Vec3) -> Vec3 {
        Vec3::new(self.x + o.x, self.y + o.y, self.z + o.z)
    }
}
impl std::ops::Sub for Vec3 {
    type Output = Vec3;
    async fn sub(self, o: Vec3) -> Vec3 {
        Vec3::new(self.x - o.x, self.y - o.y, self.z - o.z)
    }
}
impl std::ops::Mul<f64> for Vec3 {
    type Output = Vec3;
    async fn mul(self, s: f64) -> Vec3 {
        Vec3::new(self.x * s, self.y * s, self.z * s)
    }
}
impl std::ops::Neg for Vec3 {
    type Output = Vec3;
    async fn neg(self) -> Vec3 {
