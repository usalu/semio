//! 🧪️ `create-vertex` fixture — `🐺️adds-an-apex-vertex-above-the-square`.
//!
...
fn before() -> SemioBrepSnapshot {
    decode_semio_brep_snapshot_json(BEFORE).expect("create-vertex before snapshot decodes")
}
fn expected_after() -> SemioBrepSnapshot {
    decode_semio_brep_snapshot_json(AFTER).expect("create-vertex after snapshot decodes")
}
fn mutation() -> SemioBrepMutation {
    decode_semio_brep_mutation_json(MUTATION).expect("create-vertex mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let original = pack::json::parse(MUTATION).expect("create-vertex mutation reparses");
...
async fn declared_outcome_holds_as_committed() {
    ...
    assert_eq!(outcome.get("status").and_then(pack::json::Value::as_str), Some("applied"), "create-vertex/adds-an-apex-vertex-above-the-square: this case is declared applied");
