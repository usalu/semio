//! 🧬️ SemioAnimationMutation — 🚧 scaffolded by W1b: a single `SetSnapshot` full-replace mutation
...
impl Mutation<SemioAnimationSnapshot> for SemioAnimationMutation {
    ...
    fn diff(&self, _base: &SemioAnimationSnapshot) -> Self::Diff {
        match self {
            SemioAnimationMutation::NoMutation => SemioAnimationDiff::default(),
            SemioAnimationMutation::SetSnapshot { snapshot } => SemioAnimationDiff { replacement: Some(snapshot.clone()) },
        }
    }

    fn inverse(&self, base: &SemioAnimationSnapshot) -> Vec<Self> {
        match self {
            SemioAnimationMutation::NoMutation => vec![SemioAnimationMutation::NoMutation],
            SemioAnimationMutation::SetSnapshot { .. } => vec![SemioAnimationMutation::SetSnapshot { snapshot: base.clone() }],
        }
    }
}
...
mod tests {
    ...
    fn mutation_diff_law_set_snapshot_matches_diff() {
        let base = SemioAnimationSnapshot::default();
        let mut next = SemioAnimationSnapshot::default();
        next.schema = format!("{}-mutated", base.schema);
        let mutation = SemioAnimationMutation::SetSnapshot { snapshot: next.clone() };
        let diff = <SemioAnimationMutation as Mutation<SemioAnimationSnapshot>>::diff(&mutation, &base);
        assert_eq!(<SemioAnimationDiff as protocol::MutationDiff<SemioAnimationSnapshot>>::apply(&diff, &base), next);
        let inv = <SemioAnimationMutation as Mutation<SemioAnimationSnapshot>>::inverse(&mutation, &base);
        assert_eq!(inv.len(), 1);
        let mut round = next.clone();
        let _ = apply_semio_animation_mutation(&mut round, &inv[0]);
    ...
    fn op_text_binary_roundtrip_law() {
        let base = SemioAnimationSnapshot::default();
        for m in [SemioAnimationMutation::NoMutation, SemioAnimationMutation::SetSnapshot { snapshot: base.clone() }] {
