//! 🏭️ Writes the `semio_document@1/any` JSON-carrier fixture corpus. Every pair is checked for OBSERVABILITY
//! before it is written — a mutation whose projection does not move is refused, not committed.
use std::fs;
use std::path::PathBuf;
use std::process::exit;

use semio_document_json::{apply, arrange, build_seed, project, KINDS};
...
fn main() {
    ...
    for kind in KINDS {
        ...
        fs::write(dir.join("⬅️before.json"), &base_bytes).expect("write before JSON");
        fs::write(dir.join("➡️after.json"), &mutated_bytes).expect("write after JSON");
        written += 1;
