...
//! `../../🔺️diff/🦀️component.rs`: `insert-run` clones `base.runs`, inserts the payload run at
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "insert-run/inserts-a-german-run-between-two-english-runs: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
