//! 🧪️ `add-node-port` fixture — `🔌️inserts-an-in-port-ahead-of-the-existing-out-port`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔌add-node-port/🔌️inserts-an-in-port-ahead-of-the-existing-out-port/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔌add-node-port/🔌️inserts-an-in-port-ahead-of-the-existing-out-port/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔌add-node-port/🔌️inserts-an-in-port-ahead-of-the-existing-out-port/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔌add-node-port/🔌️inserts-an-in-port-ahead-of-the-existing-out-port/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔌add-node-port/🔌️inserts-an-in-port-ahead-of-the-existing-out-port/🎯️outcome/🔣️.json");

fn before() -> SemioGraphSnapshot {
    dsl::json::from_json_str(BEFORE).expect("add-node-port before snapshot decodes")
}
fn expected_after() -> SemioGraphSnapshot {
    dsl::json::from_json_str(AFTER).expect("add-node-port after snapshot decodes")
}
fn mutation() -> SemioGraphMutation {
    dsl::json::from_json_str(MUTATION).expect("add-node-port mutation decodes")
}
...
async fn committed_json_is_canonical() {
    ...
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&(mutation()))).expect("add-node-port mutation encodes");
...
async fn committed_diff_is_canonical_and_omits_edges_entirely() {
    let decoded: SemioGraphDiff = dsl::json::from_json_str(DIFF).expect("committed add-node-port diff decodes");
