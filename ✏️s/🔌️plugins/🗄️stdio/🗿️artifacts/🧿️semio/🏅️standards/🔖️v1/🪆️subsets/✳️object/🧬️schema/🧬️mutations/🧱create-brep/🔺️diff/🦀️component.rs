...
pub async fn diff(payload: &CreateBrep, base: &SemioObjectSnapshot) -> protocol::MutationOutcome<SemioObjectDiff> {
    if base.brep.is_some() {
        return protocol::MutationOutcome::fatal("mutation.duplicate-id", "The object already has a brep child.".to_string(), ["brep".to_string()]).await;
    }
    protocol::MutationOutcome::new(SemioObjectDiff { brep: Some(Some(store::ArtifactChild::new(payload.child_id.clone(), payload.target.clone()).await)), ..Default::default() }).await
}
//#endregion 🔖️Diff
