//! 👁️ Semio Model viewer — the read-only counterpart of `✏️editor` for this subset (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `SemioModelViewer`
//! implements `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<SemioModelViewer>`
...
use crate::viewer::semio_model::modes::view;
use crate::viewer::semio_model::modes::view::windows::main;
...
pub enum SemioModelViewCommand {
    #[default]
    Noop,
}

impl protocol::OpBinary for SemioModelViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(SemioModelViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
pub struct SemioModelViewer;

impl ArtifactViewer for SemioModelViewer {
...
pub fn create_semio_model_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(SEMIO_MODEL_DIALECT).document(["stdio", "semio"]).icon_id("box").mode_def(view::definition()).default_mode_id(view::SEMIO_MODEL_VIEW_MODE_ID).window_kind_def(main::definition()).default_layout(view::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn create_viewer_builds_a_definition_for_the_viewer_role() {
        let def = create_semio_model_viewer();
    ...
    async fn viewer_dialect_matches_the_artifact_coordinate() {
        assert_eq!(<SemioModelViewer as ArtifactViewer>::DIALECT, SEMIO_MODEL_DIALECT);
    }

    #[semio_framework_async_macros::async_test]
    async fn viewer_never_mutates_the_document_or_draft_store() {
        semio_framework_plugin::testkit::assert_viewer_never_mutates::<SemioModelViewer>();
    }
}
//#endregion 🧪️Tests
