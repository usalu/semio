...
/// 👁️ Pure `SemioAudioSnapshot -> UiNode` read: default camera (a viewer has no persisted
...
pub async fn render(document: &SemioAudioSnapshot) -> UiNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": SEMIO_AUDIO_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(SEMIO_AUDIO_VIEW_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(SEMIO_AUDIO_VIEW_DEFAULT_CAMERA_POSITION, SEMIO_AUDIO_VIEW_DEFAULT_CAMERA_TARGET, SEMIO_AUDIO_VIEW_DEFAULT_CAMERA_FOV).await,
        meshes_json,
