...
//! `../../🏅️standards/🔖️v1/🪆️subsets/✳️document/🧪️oracle/🔣️component.json`. Registering oracle
...
mod subject {
    ...
    use semio_s_plugin_stdio::artifacts::semio::standards::v1::subsets::document::schema::mutations::{apply_semio_document_mutation, decode_semio_document_mutation_json, inverse_semio_document_mutation, SemioDocumentMutation};
    ...
    pub fn spec_vector(kind: &'static str) -> impl Fn(&Context) -> Result<Outcome, String> {
        move |ctx: &Context| {
            ...
            run(&mut current, &decode_semio_document_mutation_json(&member_text(&vector, "mutation")?)?, ctx.scenario.id.as_str())?;
            if current != expected {
