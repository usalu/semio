...
TAG_TO_KIND = {
    ...
    "setTextBoxBlocks": "set-textbox-blocks",
...
def apply_mutation(document: dict, mutation: dict) -> dict:
    ...
    if kind == "set-shape-frame":
        ...
        return result
    if kind == "set-textbox-blocks":
        shape = shape_at(result, int(mutation["slide_index"]), int(mutation["shape_index"]), "set-textbox-blocks")
        if shape["shapeKind"] != "textBox":
            raise AssertionError("set-textbox-blocks addresses a %s shape" % shape["shapeKind"])
...
def inverse_mutation(document: dict, mutation: dict) -> dict:
    ...
    if kind == "set-textbox-blocks":
        slide_index = int(mutation["slide_index"])
        shape_index = int(mutation["shape_index"])
        shape = shape_at(document, slide_index, shape_index, "set-textbox-blocks")
        if shape["shapeKind"] != "textBox":
            raise AssertionError("set-textbox-blocks addresses a %s shape" % shape["shapeKind"])
