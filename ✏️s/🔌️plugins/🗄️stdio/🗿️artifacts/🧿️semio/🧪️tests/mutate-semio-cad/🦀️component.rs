...
mod subject {
    ...
    pub fn round_trip(ctx: &Context) -> Result<Outcome, String> {
        ...
        let reparsed = parse_semio_cad_dsl(&print_semio_cad_dsl(&parsed))?;
        ...
        let unpacked = decode_semio_cad_pack(&ctx.fixture_bytes(PACK_ASSET)?)?;
        ...
        let repacked = decode_semio_cad_pack(&encode_semio_cad_pack(&parsed))?;
