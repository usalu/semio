...
//! a `serde` derive is nameable from here, and hand-transcribing a recursive scene graph into a Rust
//! literal would be a second copy of the fixture free to drift away from it. The subset's own production
//! code therefore exports the bridges this adapter needs, whose signatures name only reachable types:
//! `decode_semio_drawing_snapshot_json`/ `encode_semio_drawing_snapshot_json` and the DSL/pack
//! pass-throughs (`../../🏅️standards/🔖️v1/ 🪆️subsets/✳️drawing/🧬️schema/📸️snapshot/🦀️component.rs`),
//! `decode_semio_drawing_mutation_json`/ `inverse_semio_drawing_mutation`
//! (`…/🧬️mutations/🦀️component.rs`). The subject half is gated behind the generated host's `sut` feature
...
//! subject exhaustive --owner 🗄️stdio --case mutate-semio-drawing` really executes every scenario below.
