//! 🦀️ STEP AP214 `✳️cc1` mutation case — Rust adapter. ISO 10303-214 CC1 (config data only).
//!
...
//! it registers are one per axis `check_cc1_conformance` reads, and the projection it compares by
...
use semio_s_plugin_stdio_test_oracle::artifacts::step::standards::v_ap214::subsets::cc1::{oracle_apply_mutation, oracle_inverse_spec, oracle_round_trip, project_step_ap214_cc1};
...
/// 🧾️ Case-local mirror of the `step-ap214-cc1` catalog. Duplicated rather than imported: `KINDS`
...
const KINDS: &[&str] = &["no-mutation", "set-snapshot", "set-file-schema", "set-product-identity", "remove-shape-representation"];
//#endregion 🔖️Kinds
...
//#region 🔖️Claim
/// 🎯️ CC1's own claim, asserted per row: `remove-shape-representation` must actually take one of
/// the real export's two representations OFF the ladder, so `aboveCeiling` falls from 2 to 1. CC1
/// counts every representation as above its ceiling, because its ceiling is below the ladder.
fn class_claim(kind: &str, before: &Json, after: &Json) -> Result<(), String> {
    if kind != "remove-shape-representation" {
        return Ok(());
    }
    match (before.get("aboveCeiling"), after.get("aboveCeiling")) {
        (Some(Json::Number(was)), Some(Json::Number(now))) if *now < *was => Ok(()),
        (was, now) => Err(format!("removing a representation must lower the count above CC1's ceiling; it went from {was:?} to {now:?}")),
    }
}
//#endregion 🔖️Claim
...
fn mutate_oracle(ctx: &Context) -> Result<Outcome, String> {
    ...
    let before = project_step_ap214_cc1(&oracle_apply_mutation(&input, &no_mutation())?)?;
    let bytes = oracle_apply_mutation(&input, &spec)?;
    let projection = project_step_ap214_cc1(&bytes)?;
...
fn inverse_oracle(ctx: &Context) -> Result<Outcome, String> {
    ...
    let spec = ctx.doc_json()?;
    let kind = spec.str("kind");
    let baseline = project_step_ap214_cc1(&oracle_apply_mutation(&input, &no_mutation())?)?;
...
fn identity_round_trip_oracle(ctx: &Context) -> Result<Outcome, String> {
    let input = mutable_input(ctx)?;
    let before = project_step_ap214_cc1(&input)?;
...
mod subject {
    ...
    /// 🦠️ The `(kind, params)` wire shape read into a real `StepCc1Mutation`. `set-snapshot` builds
    ...
    pub fn mutate(ctx: &Context) -> Result<Outcome, String> {
        ...
        let spec = ctx.doc_json()?;
        let kind = spec.str("kind");
        let before = project_step_ap214_cc1(&apply_and_encode(&input, &no_mutation())?)?;
    ...
    pub fn inverse(ctx: &Context) -> Result<Outcome, String> {
        ...
        let spec = ctx.doc_json()?;
        let kind = spec.str("kind");
        let baseline = project_step_ap214_cc1(&apply_and_encode(&input, &no_mutation())?)?;
        let text = std::str::from_utf8(&input).map_err(|error| format!("input is not UTF-8: {error}"))?;
