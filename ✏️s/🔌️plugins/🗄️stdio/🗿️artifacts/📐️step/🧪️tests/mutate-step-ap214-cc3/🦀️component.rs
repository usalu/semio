...
mod subject {
    ...
    fn mutation_from_spec(spec: &Json, base: &StepSnapshot) -> Result<StepCc3Mutation, String> {
        ...
        Ok(match spec.str("kind").as_str() {
            "no-mutation" => StepCc3Mutation::NoMutation,
            "set-snapshot" => {
                ...
                let _ = base;
                StepCc3Mutation::SetSnapshot { snapshot }
            }
            "set-file-schema" => {
                ...
                StepCc3Mutation::SetFileSchema { schemas }
            }
            "set-product-identity" => StepCc3Mutation::SetProductIdentity {
                identity: match params.get("identity").filter(|value| !matches!(value, Json::Null)) {
                    Some(value) => Some(identity_from(value)?),
                    None => None,
                },
            },
            "set-shape-representation" => StepCc3Mutation::SetShapeRepresentation {
            ...
            "demote-shape-representation" => StepCc3Mutation::DemoteShapeRepresentation { id: u64_field(&params, "id")? },
