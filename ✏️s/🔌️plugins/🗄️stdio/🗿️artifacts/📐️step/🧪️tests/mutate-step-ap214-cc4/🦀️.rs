...
mod subject {
    ...
    fn mutation_from_spec(spec: &Json, base: &StepSnapshot) -> Result<StepCc4Mutation, String> {
        ...
        Ok(match spec.str("kind").as_str() {
            "no-mutation" => StepCc4Mutation::NoMutation,
            "set-snapshot" => {
                ...
                let _ = base;
                StepCc4Mutation::SetSnapshot { snapshot }
            }
            "set-file-schema" => {
                ...
                StepCc4Mutation::SetFileSchema { schemas }
            }
            "set-product-identity" => StepCc4Mutation::SetProductIdentity {
                identity: match params.get("identity").filter(|value| !matches!(value, Json::Null)) {
                    Some(value) => Some(identity_from(value)?),
                    None => None,
                },
            },
            "set-shape-representation" => StepCc4Mutation::SetShapeRepresentation {
            ...
            "demote-shape-representation" => StepCc4Mutation::DemoteShapeRepresentation { id: u64_field(&params, "id")? },
