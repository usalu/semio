//! 🦀️ STEP AP214 `✳️cc6` mutation case — Rust adapter. ISO 10303-214 CC6 (advanced B-Rep, top of the ladder).
//!
...
//! it registers are one per axis `check_cc6_conformance` reads, and the projection it compares by
...
use semio_s_plugin_stdio_test_oracle::artifacts::step::standards::v_ap214::subsets::cc6::{oracle_apply_mutation, oracle_inverse_spec, oracle_round_trip, project_step_ap214_cc6};
...
/// 🧾️ Case-local mirror of the `step-ap214-cc6` catalog. Duplicated rather than imported: `KINDS`
...
const KINDS: &[&str] = &["no-mutation", "set-snapshot", "set-file-schema", "set-product-identity", "set-shape-representation"];
//#endregion 🔖️Kinds
...
//#region 🔖️Claim
/// 🎯️ CC6's own claim, asserted per row: it is the only class that can WRITE a rung-6
/// representation, so `set-shape-representation` must leave the addressed instance carrying the top
/// of the ladder — and the document must still conform, because nothing can be above this ceiling.
fn class_claim(kind: &str, _before: &Json, after: &Json) -> Result<(), String> {
    if after.get("conformsToClass") != Some(&Json::Bool(true)) {
        return Err(format!("{kind:?} left the document non-conformant to CC6, which is impossible: nothing the ladder classifies is above rung 6"));
    }
    if kind != "set-shape-representation" {
        return Ok(());
    }
    let promoted = after.array("representations").iter().any(|entry| entry.get("rung") == Some(&Json::Number(6.0)) && entry.get("id") == Some(&Json::Number(836.0)));
    if promoted {
        Ok(())
    } else {
        Err("set-shape-representation was supposed to promote #836 to the top of the ladder; the census does not show it there".to_string())
    }
}
//#endregion 🔖️Claim
...
fn mutate_oracle(ctx: &Context) -> Result<Outcome, String> {
    ...
    let before = project_step_ap214_cc6(&oracle_apply_mutation(&input, &no_mutation())?)?;
    let bytes = oracle_apply_mutation(&input, &spec)?;
    let projection = project_step_ap214_cc6(&bytes)?;
...
fn inverse_oracle(ctx: &Context) -> Result<Outcome, String> {
    ...
    let spec = ctx.doc_json()?;
    let kind = spec.str("kind");
    let baseline = project_step_ap214_cc6(&oracle_apply_mutation(&input, &no_mutation())?)?;
...
fn identity_round_trip_oracle(ctx: &Context) -> Result<Outcome, String> {
    let input = mutable_input(ctx)?;
    let before = project_step_ap214_cc6(&input)?;
...
mod subject {
    ...
    use semio_s_plugin_stdio::artifacts::step::standards::v_ap214::subsets::cc6::schema::mutations::{apply_step_cc6_mutation_checked, inverse_step_cc6_mutation, StepCc6Mutation};
    use semio_s_plugin_stdio::artifacts::step::StepSnapshot;
    use semio_s_plugin_stdio_test_oracle::artifacts::step::standards::v_ap214::subsets::cc6::project_step_ap214_cc6;
    use semio_s_plugin_stdio_test_oracle::law::{inverse_restores, reparsed_not_copied, round_trip_preserves};
    ...
    /// 🦠️ The `(kind, params)` wire shape read into a real `StepCc6Mutation`. `set-snapshot` builds
    ...
    fn mutation_from_spec(spec: &Json, base: &StepSnapshot) -> Result<StepCc6Mutation, String> {
        ...
        Ok(match spec.str("kind").as_str() {
            ...
            "set-file-schema" => {
                ...
                StepCc6Mutation::SetFileSchema(semio_s_plugin_stdio::artifacts::step::standards::v_ap214::subsets::cc6::schema::mutations::set_file_schema::SetFileSchema { schemas })
            }
            "set-product-identity" => StepCc6Mutation::SetProductIdentity(semio_s_plugin_stdio::artifacts::step::standards::v_ap214::subsets::cc6::schema::mutations::set_product_identity::SetProductIdentity {
                identity: match params.get("identity").filter(|value| !matches!(value, Json::Null)) {
                    Some(value) => Some(identity_from(value)?),
                    None => None,
                },
            }),
            "set-shape-representation" => StepCc6Mutation::SetShapeRepresentation(semio_s_plugin_stdio::artifacts::step::standards::v_ap214::subsets::cc6::schema::mutations::set_shape_representation::SetShapeRepresentation {
    ...
    pub fn mutate(ctx: &Context) -> Result<Outcome, String> {
        ...
        let spec = ctx.doc_json()?;
        let kind = spec.str("kind");
        let before = project_step_ap214_cc6(&apply_and_encode(&input, &no_mutation())?)?;
