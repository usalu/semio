...
pub fn register() {
    crate::artifacts::step::io::register();
    register_artifact_schema();
    register_pilot_languages();
    store::register_document_codec(store::DocumentCodec::of::<StepSnapshot, StepMutation>(STDIO_STEP_DOCUMENT_SCHEMA));
}
...
mod tests {
    ...
    fn codec_round_trip() {
        ...
        let parsed = <StepSnapshot as store::DocumentDsl>::parse_dsl(&text).expect("parse");
        assert_eq!(parsed.schema, snap.schema);
        let bytes = store::DocumentPack::encode_pack(&snap);
        let decoded = <StepSnapshot as store::DocumentPack>::decode_pack(&bytes).expect("decode");
