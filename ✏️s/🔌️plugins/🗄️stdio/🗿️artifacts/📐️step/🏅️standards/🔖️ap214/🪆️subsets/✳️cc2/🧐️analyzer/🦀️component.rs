//! 🧐️ StepCc2Analyzer (ap214/✳️cc2) — real ISO 10303-214 CC2 (bounded wireframe/basic surfaces) conformance checks against the
...
//! - HARD: any `*_SHAPE_REPRESENTATION` instance whose ladder rung exceeds 2 (see
//!   `⚙️engine::ladder::ladder_rung_of` for the classification and rationale; cc2 allows rungs
//!   up to 2).
...
pub fn check_cc2_conformance(snapshot: &StepSnapshot) -> Vec<Diagnostic> {
    ...
    for (id, type_name, rung) in ladder_violations(&doc, MAX_RUNG) {
        out.push(hard(
            CODE_LADDER,
            format!("instance #{id} is a {type_name} (ladder rung {rung}) -- exceeds cc2's max rung 2"),
        ));
    }
...
pub struct StepCc2Analyzer;

impl ArtifactAnalyzer for StepCc2Analyzer {
