//! 🎹️ StepCc2Composer (ap214/✳️cc2) — reads the same sources the ✳️any subset does (native
...
//! roster: "set file_schema=AUTOMOTIVE_DESIGN"), then HARD-GATES the `cc2` dialect stamp on
//! real ISO 10303-214 CC2 (bounded wireframe/basic surfaces) conformance. A hard violation (missing `FILE_SCHEMA` declaration after
//! injection is impossible; a `*_SHAPE_REPRESENTATION` instance above rung 2) fails composition
...
//! SAME `check_cc2_conformance` function backs both: the hard gate here runs pre-serialization
...
use crate::artifacts::step::standards::v_ap214::subsets::cc2::analyzer::check_cc2_conformance;
...
const DIALECT_SELF: Dialect = Dialect { artifact_kind: "s.stdio.step", standard: StandardId("ap214"), subset: SubsetId("cc2") };
...
//#region 🔖️Composer
pub struct StepCc2Composer;

impl ArtifactComposer for StepCc2Composer {
    ...
    fn compose(sources: &[ComposeSource]) -> Result<Composition<Self::Snapshot>, ComposeError> {
        ...
        if !hard.is_empty() {
            ...
            return Err(ComposeError {
                message: format!("ISO 10303-214 CC2 (bounded wireframe/basic surfaces) conformance violated: {} hard issue(s) -- not stamping the cc2 dialect", hard.len()),
                diagnostics: all,
            });
        }
...
/// 🛡️ The registered `SubsetValidator` for `ap214/cc2` -- see the module doc comment for how
/// this relates to (and honestly differs from) the composer's own pre-serialization hard gate.
pub struct StepCc2Validator;

impl SubsetValidator for StepCc2Validator {
    ...
    fn validate(payload: &IoPayload) -> Vec<Diagnostic> {
        ...
        match decoded {
            Some(snapshot) => check_cc2_conformance(&snapshot),
            None => vec![Diagnostic {
                code: FaultCode::new("stdio.step.cc2.validate-decode-failed"),
                severity: Severity::Warning,
                span: TextSpan::at(1, 1),
                message: "StepCc2Validator: payload did not decode as a StepSnapshot -- skipped".into(),
                expected: None,
...
fn validator_entry() -> &'static SubsetValidatorEntry {
    VALIDATOR_ENTRY.get_or_init(subset_validator_entry_of::<StepCc2Validator>)
}
...
mod tests {
    ...
    fn composer_injects_file_schema_and_stamps_clean_document() {
        ...
        let composed = StepCc2Composer::compose(&sources).expect("a document with no illegal representation must compose to cc2");
    ...
    fn subset_validator_recheck_flags_missing_file_schema_on_the_raw_wire_payload() {
        ...
        let bytes = clean_bytes();
        let diagnostics = StepCc2Validator::validate(&IoPayload::Binary(bytes));
        assert!(diagnostics.iter().any(|d| d.code.0 == crate::artifacts::step::standards::v_ap214::subsets::cc2::analyzer::CODE_FILE_SCHEMA), "got {diagnostics:?}");
    }
}
