//! ✏️ Step CC4 editor — thin, kit-based editor surface (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.1). `StepCc4Editor`
//! implements `ArtifactEditor`, wiring the shared `MeshWindowKit` to a single Main window.

use crate::artifacts::step::standards::v_ap214::subsets::cc4::schema::mutations::StepMutation;
...
pub const STEP_CC4_DOCUMENT_SCHEMA: &str = "stdio.step";
//#endregion 🔖️Dialect
...
pub enum StepCc4EditCommand {
    #[default]
    SetVertex,
}

impl protocol::OpBinary for StepCc4EditCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(StepCc4EditCommand::SetVertex)
    }
}
//#endregion 🔖️Command
...
pub struct StepCc4Editor;

impl ArtifactEditor for StepCc4Editor {
    ...
    type Command = StepCc4EditCommand;
...
pub fn create_step_cc4_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(STEP_CC4_DIALECT).document(["stdio", "step"]).icon_id("box").mode_def(edit::definition()).default_mode_id(edit::STEP_CC4_EDIT_MODE_ID).window_kind_def(main::definition()).default_layout(edit::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn editor_and_viewer_share_one_dialect() {
        semio_framework_plugin::testkit::assert_editor_and_viewer_share_dialect::<StepCc4Editor, crate::viewer::step_cc4::StepCc4Viewer>();
    }
}
//#endregion 🧪️Tests
