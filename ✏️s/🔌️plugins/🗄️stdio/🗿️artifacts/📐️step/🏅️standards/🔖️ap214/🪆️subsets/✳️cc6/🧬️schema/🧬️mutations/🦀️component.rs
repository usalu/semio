...
//! filter. Every variant below is one rule of `check_cc6_conformance` (`../🦀️component.rs`'s
...
pub enum StepCc6Mutation {
    ...
    /// 🪜️ Writes a representation this class admits, or — with `None` — deletes one. Every
    /// classified rung passes the guard here; what it still refuses is a type that is not a
    /// `*_SHAPE_REPRESENTATION` at all, which is the only ladder error CC6 can commit.
...
impl StepCc6Mutation {
    ...
    pub fn kind(&self) -> &'static str {
        match self {
            StepCc6Mutation::NoMutation => "no-mutation",
            StepCc6Mutation::SetSnapshot { .. } => "set-snapshot",
            StepCc6Mutation::SetFileSchema { .. } => "set-file-schema",
            StepCc6Mutation::SetProductIdentity { .. } => "set-product-identity",
            StepCc6Mutation::SetShapeRepresentation { .. } => "set-shape-representation",
        }
    }
    ...
    fn class_edit(&self) -> Option<ClassEdit> {
        match self {
            StepCc6Mutation::NoMutation | StepCc6Mutation::SetSnapshot { .. } => None,
            StepCc6Mutation::SetFileSchema { schemas } => Some(ClassEdit::FileSchema { schemas: schemas.clone() }),
            StepCc6Mutation::SetProductIdentity { identity } => Some(ClassEdit::ProductIdentity { identity: identity.clone() }),
            StepCc6Mutation::SetShapeRepresentation { id, representation } => Some(ClassEdit::Representation { id: *id, row: representation.clone() }),
        }
    }
}
...
impl Mutation<StepSnapshot> for StepCc6Mutation {
    type Diff = StepDiff;

    fn diff(&self, base: &StepSnapshot) -> protocol::MutationOutcome<Self::Diff> {
        match self.class_edit() {
            None => match self {
                StepCc6Mutation::SetSnapshot { snapshot } => protocol::MutationOutcome::new(<StepDiff as DiffAlgebra<StepSnapshot>>::between(base, snapshot)),
    ...
    /// ↩️ A real per-axis inverse read off the base, for every axis. The `restore` arm exists because
    /// the shared inversion is class-neutral and can answer `None` for a class whose ceiling is below
    /// the base's own representation; at rung 6 there is no such representation, so that arm is
    /// unreachable here — kept rather than deleted, because a class must not assume its own
    /// reachability argument holds for a helper it shares with five other classes.
    fn inverse(&self, base: &StepSnapshot) -> Vec<Self> {
        let restore = || vec![StepCc6Mutation::SetSnapshot { snapshot: base.clone() }];
        match self.class_edit() {
            None => match self {
                StepCc6Mutation::SetSnapshot { .. } => restore(),
                _ => vec![StepCc6Mutation::NoMutation],
            },
            Some(edit) => match ladder::invert_class_edit(&base.to_part21_document(), MAX_RUNG, &edit) {
                Some(ClassEdit::FileSchema { schemas }) => vec![StepCc6Mutation::SetFileSchema { schemas }],
                Some(ClassEdit::ProductIdentity { identity }) => vec![StepCc6Mutation::SetProductIdentity { identity }],
                Some(ClassEdit::Representation { id, row }) => vec![StepCc6Mutation::SetShapeRepresentation { id, representation: row }],
                _ => restore(),
            },
        }
    }
...
mod tests {
    ...
    fn every_conformance_axis_round_trips_through_its_own_inverse() {
        round_trip(StepCc6Mutation::SetFileSchema { schemas: vec!["CONFIG_CONTROL_DESIGN".into()] });
        round_trip(StepCc6Mutation::SetProductIdentity { identity: None });
        round_trip(StepCc6Mutation::SetShapeRepresentation { id: 13, representation: None });
    }
    ...
    fn a_type_that_is_not_on_the_ladder_is_still_refused() {
        ...
        let outcome = apply_step_cc6_mutation(&mut snapshot, &StepCc6Mutation::SetShapeRepresentation { id: 13, representation: Some(off_ladder) });
        ...
        assert!(apply_step_cc6_mutation(&mut snapshot, &StepCc6Mutation::SetShapeRepresentation { id: 13, representation: Some(at_ceiling) }).messages().is_empty(), "and the fixture's own rung-6 type is admitted");
    }
    ...
    fn each_verb_moves_the_diagnostic_it_was_derived_from() {
        ...
        apply_step_cc6_mutation(&mut snapshot, &StepCc6Mutation::SetShapeRepresentation { id: 13, representation: None });
        ...
        apply_step_cc6_mutation(&mut snapshot, &StepCc6Mutation::SetProductIdentity { identity: None });
        assert!(!has_product_definition_chain(&snapshot.to_part21_document()));
        apply_step_cc6_mutation(&mut snapshot, &StepCc6Mutation::SetFileSchema { schemas: vec!["IFC4".into()] });
    ...
    fn a_rejected_mutation_leaves_the_snapshot_untouched() {
        let mut snapshot = base();
        assert!(!apply_step_cc6_mutation(&mut snapshot, &StepCc6Mutation::SetShapeRepresentation { id: 827, representation: None }).messages().is_empty(), "a conformance repair must never delete a product record");
        assert!(!apply_step_cc6_mutation(&mut snapshot, &StepCc6Mutation::SetFileSchema { schemas: vec![] }).messages().is_empty());
    ...
    fn no_mutation_is_the_identity() {
        let mut snapshot = base();
        assert!(apply_step_cc6_mutation(&mut snapshot, &StepCc6Mutation::NoMutation).messages().is_empty());
        assert_eq!(snapshot, base());
    }
    ...
    fn kinds_const_matches_enum_variants_in_declaration_order() {
        let one_per_variant = vec![
            StepCc6Mutation::NoMutation,
            StepCc6Mutation::SetSnapshot { snapshot: StepSnapshot::default() },
            StepCc6Mutation::SetFileSchema { schemas: Vec::new() },
            StepCc6Mutation::SetProductIdentity { identity: None },
            StepCc6Mutation::SetShapeRepresentation { id: 0, representation: None },
        ];
