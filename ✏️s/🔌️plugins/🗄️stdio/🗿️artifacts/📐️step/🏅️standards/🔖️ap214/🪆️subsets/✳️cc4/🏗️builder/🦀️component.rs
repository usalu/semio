//! 🏗️ StepCc4Builder (ap214/✳️cc4) — a local `ArtifactBuilder` (until SDK Wave 3, same as
//! the artifact-level `StepBuilder`) whose `build()` re-runs `check_cc4_conformance` as a hard
//! gate, unconditionally, regardless of which construction path produced the in-flight snapshot
//! -- so a hard ISO 10303-214 CC4 (manifold surfaces with topology) violation can never leave this builder as an `Ok(StepSnapshot)`.

use dsl::{Diagnostic, Severity};
use semio_framework_plugin::ArtifactBuilder;
use crate::artifacts::step::{StepDiff, StepMutation, StepSnapshot};
use crate::artifacts::step::standards::v_ap214::subsets::cc4::analyzer::check_cc4_conformance;

//#region 🔖️Builder
#[derive(Clone, Debug, Default)]
pub struct StepCc4Builder {
    snapshot: StepSnapshot,
}

impl ArtifactBuilder for StepCc4Builder {
...
mod tests {
    ...
    use crate::artifacts::step::standards::v_ap214::subsets::cc4::analyzer::CODE_LADDER;
    ...
    fn conforming_construction_builds() {
        let snapshot = StepCc4Builder::from_snapshot(conforming_snapshot()).build().expect("conforming construction must build");
    ...
    fn hard_violation_injected_via_raw_mutate_still_fails_build() {
        ...
        let (mutated, _diff) = StepCc4Builder::from_snapshot(StepSnapshot::default()).mutate(StepMutation::SetSnapshot { snapshot });
