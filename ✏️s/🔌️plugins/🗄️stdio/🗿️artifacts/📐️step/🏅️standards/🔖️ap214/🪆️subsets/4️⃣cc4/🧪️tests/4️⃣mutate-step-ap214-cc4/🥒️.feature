@capability-step-ap214-cc4-mutate
@oracle-ruststep-step-ap214-cc4-mutate
@comparison-semantic-step-conformance-class-v1
@mutations-step-ap214-cc4
Feature: Apply every typed ISO 10303-214 CC4 (manifold surfaces with topology) mutation to a real AP214 exchange structure
  ...
  it is a FILTER, and the 6 kinds here are one per axis `check_cc4_conformance` actually reads:
  ...
  🪜️ What this class is. CC4 is the first class with TOPOLOGY: a manifold surface representation
  carries shells, and a shell is a topological object rather than a bag of surfaces. What CC4 refuses
  is the step to a SOLID — `FACETED_BREP_SHAPE_REPRESENTATION` bounds a volume, and a class scoped to
  surfaces has no conformant state containing one. Against the real export, `#13` is two rungs above
  this ceiling and `#836` is two rungs below it.
  ...
  🧬️ Why this catalog is identical to `2️⃣mutate-step-ap214-cc2`, `3️⃣mutate-step-ap214-cc3`, `5️⃣mutate-step-ap214-cc5`'s, and
  ...
  what this subset supplies is `MAX_RUNG = 4` and the ceiling type MANIFOLD_SURFACE_SHAPE_REPRESENTATION
  it selects.
  ...
  Scenario Outline: Apply <id> and read the conformance projection back
    ...
    Examples:
      ...
      | set-shape-representation    | {"id": 836, "representation": {"typeName": "MANIFOLD_SURFACE_SHAPE_REPRESENTATION", "name": "Document", "items": [837, 895], "context": 835}}                                 |
