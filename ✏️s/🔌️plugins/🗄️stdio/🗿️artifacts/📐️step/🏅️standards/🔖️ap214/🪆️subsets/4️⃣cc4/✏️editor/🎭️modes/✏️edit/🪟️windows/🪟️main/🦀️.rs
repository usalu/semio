...
pub fn render(document: &StepSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = pack::json_to_string(&pack::json_array([pack::json!({ "id": STEP_CC4_EDIT_FALLBACK_MESH_KIND, "data": mesh_from_kind(STEP_CC4_EDIT_FALLBACK_MESH_KIND) })]));
    let view = MeshView {
