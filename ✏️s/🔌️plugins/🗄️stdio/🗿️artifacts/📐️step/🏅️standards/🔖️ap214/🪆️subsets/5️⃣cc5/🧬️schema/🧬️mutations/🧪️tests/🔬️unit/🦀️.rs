...
use crate::standards::v_ap214::subsets::cc5::schema::check_cc5_conformance;
...
fn the_class_ceiling_is_the_line_this_vocabulary_draws() {
    ...
    assert!(
        apply_step_cc5_mutation(&mut snapshot, &StepCc5Mutation::SetShapeRepresentation(set_shape_representation::SetShapeRepresentation { id: 13, representation: Some(at_ceiling) })).messages().is_empty(),
...
fn a_rejected_mutation_leaves_the_snapshot_untouched() {
    let mut snapshot = base();
    assert!(
        !apply_step_cc5_mutation(&mut snapshot, &StepCc5Mutation::SetShapeRepresentation(set_shape_representation::SetShapeRepresentation { id: 827, representation: None })).messages().is_empty(),
