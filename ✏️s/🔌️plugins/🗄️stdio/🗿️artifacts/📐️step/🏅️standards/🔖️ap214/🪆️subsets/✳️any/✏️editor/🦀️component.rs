...
//#region 🔖️Manifest
pub async fn create_step_any_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(STEP_ANY_DIALECT)
        .await.document(["stdio", "step"])
        .await.icon_id("box")
        .await.mode_def(edit::definition().await)
        .await.default_mode_id(edit::STEP_ANY_EDIT_MODE_ID)
