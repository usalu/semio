//! 🧬️ StepSnapshot schema (ap214/✳️cc5) — reuses the ✳️any subset's `StepSnapshot` verbatim
//! (the SAME Rust type, same `stdio.step` schema id). ISO 10303-214 CC5 (faceted B-Rep) is a validation-gated
...
//! `🪆️subsets/✳️cc5/🧬️schema/` is present per `🔣️taxonomy.json`'s `subsetChildDirs`, without
//! duplicating the schema definition.

pub use crate::artifacts::step::standards::v_ap214::subsets::any::schema::*;

//#region 🧬️Mutations
/// 🧬️ This subset's OWN mutation vocabulary — one kind per ISO 10303-214 CC5 (faceted B-Rep) conformance
/// rule, derived from `check_cc5_conformance` below rather than copied from a sibling class, and
...
pub mod derived_construction {
    ...
    mod tests {
        ...
        use crate::artifacts::step::standards::v_ap214::subsets::cc5::schema::CODE_LADDER;
        ...
        async fn conforming_construction_builds() {
            let snapshot = StepCc5BuilderConstruction::from_snapshot(conforming_snapshot()).await.build().await.expect("conforming construction must build");
        ...
        async fn hard_violation_injected_via_raw_mutate_still_fails_build() {
            ...
            let (mutated, _diff) = StepCc5BuilderConstruction::from_snapshot(StepSnapshot::default()).await.mutate(StepMutation::SetSnapshot { snapshot }).await;
            let err = mutated.build().await.expect_err("an ADVANCED_BREP_SHAPE_REPRESENTATION instance above rung 5 must fail build()");
