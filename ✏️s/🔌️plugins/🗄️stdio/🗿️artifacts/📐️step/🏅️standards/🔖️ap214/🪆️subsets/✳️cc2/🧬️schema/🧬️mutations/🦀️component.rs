...
use crate::artifacts::step::standards::v_ap214::subsets::cc2::schema::MAX_RUNG;
...
/// 📐️ Typed conformance-class mutation for `stdio.step` at `ap214/✳️cc2`.
...
pub enum StepCc2Mutation {
    ...
    /// is this class's own: `GEOMETRICALLY_BOUNDED_WIREFRAME_SHAPE_REPRESENTATION` passes, anything above it is refused with a message naming
    /// the rung and the ceiling.
    SetShapeRepresentation {
        id: u64,
        representation: Option<ShapeRepresentationRow>,
    },
    /// ⬇️ The conformance repair: rewrite an over-rung representation onto `GEOMETRICALLY_BOUNDED_WIREFRAME_SHAPE_REPRESENTATION`, keeping its
...
impl StepCc2Mutation {
    ...
    pub fn kind(&self) -> &'static str {
        match self {
            StepCc2Mutation::NoMutation => "no-mutation",
            StepCc2Mutation::SetSnapshot { .. } => "set-snapshot",
            StepCc2Mutation::SetFileSchema { .. } => "set-file-schema",
            StepCc2Mutation::SetProductIdentity { .. } => "set-product-identity",
            StepCc2Mutation::SetShapeRepresentation { .. } => "set-shape-representation",
            StepCc2Mutation::DemoteShapeRepresentation { .. } => "demote-shape-representation",
        }
    }
    ...
    fn class_edit(&self) -> Option<ClassEdit> {
        match self {
            StepCc2Mutation::NoMutation | StepCc2Mutation::SetSnapshot { .. } => None,
            StepCc2Mutation::SetFileSchema { schemas } => Some(ClassEdit::FileSchema { schemas: schemas.clone() }),
            StepCc2Mutation::SetProductIdentity { identity } => Some(ClassEdit::ProductIdentity { identity: identity.clone() }),
            StepCc2Mutation::SetShapeRepresentation { id, representation } => Some(ClassEdit::Representation { id: *id, row: representation.clone() }),
            StepCc2Mutation::DemoteShapeRepresentation { id } => Some(ClassEdit::Demotion { id: *id }),
        }
    }
}
...
impl Mutation<StepSnapshot> for StepCc2Mutation {
    type Diff = StepDiff;

    fn diff(&self, base: &StepSnapshot) -> protocol::MutationOutcome<Self::Diff> {
        match self.class_edit() {
            None => match self {
                StepCc2Mutation::SetSnapshot { snapshot } => protocol::MutationOutcome::new(<StepDiff as DiffAlgebra<StepSnapshot>>::between(base, snapshot)),
    ...
    fn inverse(&self, base: &StepSnapshot) -> Vec<Self> {
        let restore = || vec![StepCc2Mutation::SetSnapshot { snapshot: base.clone() }];
        match self.class_edit() {
            None => match self {
                StepCc2Mutation::SetSnapshot { .. } => restore(),
                _ => vec![StepCc2Mutation::NoMutation],
            },
            Some(edit) => match ladder::invert_class_edit(&base.to_part21_document(), MAX_RUNG, &edit) {
                Some(ClassEdit::FileSchema { schemas }) => vec![StepCc2Mutation::SetFileSchema { schemas }],
                Some(ClassEdit::ProductIdentity { identity }) => vec![StepCc2Mutation::SetProductIdentity { identity }],
                Some(ClassEdit::Representation { id, row }) => vec![StepCc2Mutation::SetShapeRepresentation { id, representation: row }],
                _ => restore(),
            },
        }
    }
...
mod tests {
    ...
    fn every_conformance_axis_round_trips_through_its_own_inverse() {
        round_trip(base(), StepCc2Mutation::SetFileSchema { schemas: vec!["CONFIG_CONTROL_DESIGN".into()] });
        round_trip(base(), StepCc2Mutation::SetProductIdentity { identity: None });
        round_trip(base(), StepCc2Mutation::SetShapeRepresentation { id: 13, representation: None });
        round_trip(conforming(), StepCc2Mutation::SetShapeRepresentation { id: 13, representation: None });
        round_trip(base(), StepCc2Mutation::DemoteShapeRepresentation { id: 13 });
    }
    ...
    fn the_class_ceiling_is_the_line_this_vocabulary_draws() {
        ...
        assert!(apply_step_cc2_mutation(&mut snapshot, &StepCc2Mutation::SetShapeRepresentation { id: 13, representation: Some(at_ceiling) }).messages().is_empty(), "this class admits its own ceiling type");
        ...
        let outcome = apply_step_cc2_mutation(&mut snapshot, &StepCc2Mutation::SetShapeRepresentation { id: 13, representation: Some(above) });
    ...
    fn each_verb_moves_the_diagnostic_it_was_derived_from() {
        ...
        apply_step_cc2_mutation(&mut snapshot, &StepCc2Mutation::DemoteShapeRepresentation { id: 13 });
        ...
        apply_step_cc2_mutation(&mut snapshot, &StepCc2Mutation::SetProductIdentity { identity: None });
        assert!(!has_product_definition_chain(&snapshot.to_part21_document()));
        apply_step_cc2_mutation(&mut snapshot, &StepCc2Mutation::SetFileSchema { schemas: vec!["IFC4".into()] });
    ...
    fn a_rejected_mutation_leaves_the_snapshot_untouched() {
        let mut snapshot = base();
        assert!(!apply_step_cc2_mutation(&mut snapshot, &StepCc2Mutation::SetShapeRepresentation { id: 827, representation: None }).messages().is_empty(), "a conformance repair must never delete a product record");
        assert!(!apply_step_cc2_mutation(&mut snapshot, &StepCc2Mutation::SetFileSchema { schemas: vec![] }).messages().is_empty());
    ...
    fn no_mutation_is_the_identity() {
        let mut snapshot = base();
        assert!(apply_step_cc2_mutation(&mut snapshot, &StepCc2Mutation::NoMutation).messages().is_empty());
        assert_eq!(snapshot, base());
    }
    ...
    fn kinds_const_matches_enum_variants_in_declaration_order() {
        let one_per_variant = vec![
            StepCc2Mutation::NoMutation,
            StepCc2Mutation::SetSnapshot { snapshot: StepSnapshot::default() },
            StepCc2Mutation::SetFileSchema { schemas: Vec::new() },
            StepCc2Mutation::SetProductIdentity { identity: None },
            StepCc2Mutation::SetShapeRepresentation { id: 0, representation: None },
            StepCc2Mutation::DemoteShapeRepresentation { id: 0 },
        ];
