//! 🧬️ StepSnapshot schema (ap214/✳️cc4) — reuses the ✳️any subset's `StepSnapshot` verbatim
...
pub mod derived_analysis {
    ...
    pub async fn check_cc4_conformance(snapshot: &StepSnapshot) -> Vec<Diagnostic> {
    ...
    impl ArtifactAnalysis for StepCc4AnalyzerAnalysis {
        ...
        async fn analyze(sources: &[AnalyzeSource<'_>]) -> Analysis<Self::Parts> {
            ...
            if let Some(snapshot) = &inner.parts.snapshot {
                let checks = check_cc4_conformance(snapshot).await;
