//! 🧊 Step CC1 editor — the Main window: the SAME world-3d mesh scene shape as the sibling
...
const STEP_CC1_EDIT_FALLBACK_MESH_KIND: &str = "box";
const STEP_CC1_EDIT_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const STEP_CC1_EDIT_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const STEP_CC1_EDIT_DEFAULT_CAMERA_FOV: f64 = 45.0;
...
fn world_instances_json(document: &StepSnapshot) -> String {
    ...
    let instances: Vec<serde_json::Value> = (0..count)
        .map(|index| {
            serde_json::json!({
                "id": format!("step_cc1-{index}"),
                "meshId": STEP_CC1_EDIT_FALLBACK_MESH_KIND,
                "position": [index as f64 * 2.0, 0.0, 0.0],
                "rotation": [0.0, 0.0, 0.0, 1.0],
                "scale": [1.0, 1.0, 1.0],
                "label": format!("Step CC1 {index}"),
                "smoothShading": false,
            })
        })
        .collect();
...
pub fn render(document: &StepSnapshot) -> UiNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": STEP_CC1_EDIT_FALLBACK_MESH_KIND, "data": mesh_from_kind(STEP_CC1_EDIT_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(STEP_CC1_EDIT_DEFAULT_CAMERA_POSITION, STEP_CC1_EDIT_DEFAULT_CAMERA_TARGET, STEP_CC1_EDIT_DEFAULT_CAMERA_FOV),
        meshes_json,
