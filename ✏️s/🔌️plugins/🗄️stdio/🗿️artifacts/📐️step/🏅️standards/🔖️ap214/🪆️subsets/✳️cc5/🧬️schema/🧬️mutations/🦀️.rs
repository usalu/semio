...
impl StepCc5Mutation {
    ...
    pub fn kind(&self) -> &'static str {
        match self {
            StepCc5Mutation::SetSnapshot(_) => "set-snapshot",
...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub(crate) fn class_inverse(base: &StepSnapshot, edit: &ClassEdit) -> Vec<StepCc5Mutation> {
    match ladder::invert_class_edit(&base.to_part21_document(), MAX_RUNG, edit) {
        Some(ClassEdit::FileSchema { schemas }) => vec![StepCc5Mutation::SetFileSchema(set_file_schema::SetFileSchema { schemas })],
        Some(ClassEdit::ProductIdentity { identity }) => vec![StepCc5Mutation::SetProductIdentity(set_product_identity::SetProductIdentity { identity })],
        Some(ClassEdit::Representation { id, row }) => vec![StepCc5Mutation::SetShapeRepresentation(set_shape_representation::SetShapeRepresentation { id, representation: row })],
        _ => vec![StepCc5Mutation::SetSnapshot(set_snapshot::SetSnapshot { snapshot: base.clone() })],
    }
}
...
mod tests {
    ...
    fn every_conformance_axis_round_trips_through_its_own_inverse() {
        round_trip(base(), StepCc5Mutation::SetFileSchema(set_file_schema::SetFileSchema { schemas: vec!["CONFIG_CONTROL_DESIGN".into()] }));
        round_trip(base(), StepCc5Mutation::SetProductIdentity(set_product_identity::SetProductIdentity { identity: None }));
        round_trip(base(), StepCc5Mutation::SetShapeRepresentation(set_shape_representation::SetShapeRepresentation { id: 13, representation: None }));
        round_trip(conforming(), StepCc5Mutation::SetShapeRepresentation(set_shape_representation::SetShapeRepresentation { id: 13, representation: None }));
        round_trip(base(), StepCc5Mutation::DemoteShapeRepresentation(demote_shape_representation::DemoteShapeRepresentation { id: 13 }));
    }
    ...
    fn the_class_ceiling_is_the_line_this_vocabulary_draws() {
        ...
        assert!(apply_step_cc5_mutation(&mut snapshot, &StepCc5Mutation::SetShapeRepresentation(set_shape_representation::SetShapeRepresentation { id: 13, representation: Some(at_ceiling) })).messages().is_empty(), "this class admits its own ceiling type");
        ...
        let outcome = apply_step_cc5_mutation(&mut snapshot, &StepCc5Mutation::SetShapeRepresentation(set_shape_representation::SetShapeRepresentation { id: 13, representation: Some(above) }));
    ...
    fn each_verb_moves_the_diagnostic_it_was_derived_from() {
        ...
        apply_step_cc5_mutation(&mut snapshot, &StepCc5Mutation::DemoteShapeRepresentation(demote_shape_representation::DemoteShapeRepresentation { id: 13 }));
        ...
        apply_step_cc5_mutation(&mut snapshot, &StepCc5Mutation::SetProductIdentity(set_product_identity::SetProductIdentity { identity: None }));
        assert!(!has_product_definition_chain(&snapshot.to_part21_document()));
        apply_step_cc5_mutation(&mut snapshot, &StepCc5Mutation::SetFileSchema(set_file_schema::SetFileSchema { schemas: vec!["IFC4".into()] }));
    ...
    fn a_rejected_mutation_leaves_the_snapshot_untouched() {
        let mut snapshot = base();
        assert!(!apply_step_cc5_mutation(&mut snapshot, &StepCc5Mutation::SetShapeRepresentation(set_shape_representation::SetShapeRepresentation { id: 827, representation: None })).messages().is_empty(), "a conformance repair must never delete a product record");
        assert!(!apply_step_cc5_mutation(&mut snapshot, &StepCc5Mutation::SetFileSchema(set_file_schema::SetFileSchema { schemas: vec![] })).messages().is_empty());
    ...
    fn kinds_const_matches_enum_variants_in_declaration_order() {
        let one_per_variant = vec![
            StepCc5Mutation::SetSnapshot(set_snapshot::SetSnapshot { snapshot: StepSnapshot::default() }),
            StepCc5Mutation::SetFileSchema(set_file_schema::SetFileSchema { schemas: Vec::new() }),
            StepCc5Mutation::SetProductIdentity(set_product_identity::SetProductIdentity { identity: None }),
            StepCc5Mutation::SetShapeRepresentation(set_shape_representation::SetShapeRepresentation { id: 0, representation: None }),
            StepCc5Mutation::DemoteShapeRepresentation(demote_shape_representation::DemoteShapeRepresentation { id: 0 }),
        ];
