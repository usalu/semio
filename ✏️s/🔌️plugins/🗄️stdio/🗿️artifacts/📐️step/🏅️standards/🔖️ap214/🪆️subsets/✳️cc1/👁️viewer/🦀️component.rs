//! 👁️ Step CC1 viewer — the read-only counterpart of `✏️editor` for this subset (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `StepCc1Viewer`
//! implements `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<StepCc1Viewer>`
...
use crate::viewer::step_cc1::modes::view;
use crate::viewer::step_cc1::modes::view::windows::main;
...
pub enum StepCc1ViewCommand {
    #[default]
    Noop,
}

impl protocol::OpBinary for StepCc1ViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(StepCc1ViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
pub struct StepCc1Viewer;

impl ArtifactViewer for StepCc1Viewer {
    ...
    type Command = StepCc1ViewCommand;
...
pub fn create_step_cc1_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(STEP_CC1_DIALECT).document(["stdio", "step"]).icon_id("box").mode_def(view::definition()).default_mode_id(view::STEP_CC1_VIEW_MODE_ID).window_kind_def(main::definition()).default_layout(view::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn create_viewer_builds_a_definition_for_the_viewer_role() {
        let def = create_step_cc1_viewer();
    ...
    async fn viewer_dialect_matches_the_artifact_coordinate() {
        assert_eq!(<StepCc1Viewer as ArtifactViewer>::DIALECT, STEP_CC1_DIALECT);
    }

    #[semio_framework_async_macros::async_test]
    async fn viewer_never_mutates_the_document_or_draft_store() {
        semio_framework_plugin::testkit::assert_viewer_never_mutates::<StepCc1Viewer>();
    }
}
//#endregion 🧪️Tests
