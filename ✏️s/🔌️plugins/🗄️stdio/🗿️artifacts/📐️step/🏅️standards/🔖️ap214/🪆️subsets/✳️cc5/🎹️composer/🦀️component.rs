//! 🎹️ StepCc5Composer (ap214/✳️cc5) — reads the same sources the ✳️any subset does (native
...
//! roster: "set file_schema=AUTOMOTIVE_DESIGN"), then HARD-GATES the `cc5` dialect stamp on
//! real ISO 10303-214 CC5 (faceted B-Rep) conformance. A hard violation (missing `FILE_SCHEMA` declaration after
//! injection is impossible; a `*_SHAPE_REPRESENTATION` instance above rung 5) fails composition
...
//! SAME `check_cc5_conformance` function backs both: the hard gate here runs pre-serialization
...
//#region 🔖️Composer
pub struct StepCc5Composer;

impl ArtifactComposer for StepCc5Composer {
...
mod tests {
    ...
    fn composer_injects_file_schema_and_stamps_clean_document() {
        ...
        let composed = StepCc5Composer::compose(&sources).expect("a document with no illegal representation must compose to cc5");
    ...
    fn subset_validator_recheck_flags_missing_file_schema_on_the_raw_wire_payload() {
        ...
        assert!(diagnostics.iter().any(|d| d.code.0 == crate::artifacts::step::standards::v_ap214::subsets::cc5::analyzer::CODE_FILE_SCHEMA), "got {diagnostics:?}");
    }
}
