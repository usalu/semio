//! 👁️ Step CC4 viewer — the read-only counterpart of `✏️editor` for this subset (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `StepCc4Viewer`
//! implements `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<StepCc4Viewer>`
...
use crate::viewer::step_cc4::modes::view;
use crate::viewer::step_cc4::modes::view::windows::main;
...
pub enum StepCc4ViewCommand {
    #[default]
    Noop,
}

impl protocol::OpBinary for StepCc4ViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(StepCc4ViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
pub struct StepCc4Viewer;

impl ArtifactViewer for StepCc4Viewer {
    ...
    type Command = StepCc4ViewCommand;
...
pub fn create_step_cc4_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(STEP_CC4_DIALECT).document(["stdio", "step"]).icon_id("box").mode_def(view::definition()).default_mode_id(view::STEP_CC4_VIEW_MODE_ID).window_kind_def(main::definition()).default_layout(view::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn create_viewer_builds_a_definition_for_the_viewer_role() {
        let def = create_step_cc4_viewer();
    ...
    async fn viewer_dialect_matches_the_artifact_coordinate() {
        assert_eq!(<StepCc4Viewer as ArtifactViewer>::DIALECT, STEP_CC4_DIALECT);
    }

    #[semio_framework_async_macros::async_test]
    async fn viewer_never_mutates_the_document_or_draft_store() {
        semio_framework_plugin::testkit::assert_viewer_never_mutates::<StepCc4Viewer>().await;
    }
}
//#endregion 🧪️Tests
