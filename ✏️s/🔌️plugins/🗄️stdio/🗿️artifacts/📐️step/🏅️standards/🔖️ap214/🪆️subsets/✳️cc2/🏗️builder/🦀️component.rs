//! 🏗️ StepCc2Builder (ap214/✳️cc2) — a local `ArtifactBuilder` (until SDK Wave 3, same as
//! the artifact-level `StepBuilder`) whose `build()` re-runs `check_cc2_conformance` as a hard
//! gate, unconditionally, regardless of which construction path produced the in-flight snapshot
//! -- so a hard ISO 10303-214 CC2 (bounded wireframe/basic surfaces) violation can never leave this builder as an `Ok(StepSnapshot)`.
...
pub struct StepCc2Builder {
    snapshot: StepSnapshot,
}

impl ArtifactBuilder for StepCc2Builder {
    ...
    fn build(self) -> Result<Self::Snapshot, Vec<Diagnostic>> {
        let hard: Vec<Diagnostic> = check_cc2_conformance(&self.snapshot).into_iter().filter(|d| matches!(d.severity, Severity::Error | Severity::Fatal)).collect();
        if hard.is_empty() {
            Ok(self.snapshot)
        } else {
            Err(hard)
...
mod tests {
    ...
    use crate::artifacts::step::standards::v_ap214::subsets::cc2::analyzer::CODE_LADDER;
    ...
    fn conforming_construction_builds() {
        let snapshot = StepCc2Builder::from_snapshot(conforming_snapshot()).build().expect("conforming construction must build");
    ...
    fn hard_violation_injected_via_raw_mutate_still_fails_build() {
        ...
        let (mutated, _diff) = StepCc2Builder::from_snapshot(StepSnapshot::default()).mutate(StepMutation::SetSnapshot { snapshot });
