//! 🔮️ Mutation oracle for `ap214/✳️cc6` — ISO 10303-214 CC6 (advanced B-Rep, top of the ladder).
//!
...
//! `check_cc6_conformance` actually reads, and the projection reports those axes and nothing else.
//!
//! 🪜️ **CC6 sits at the top of the ladder, and the committed fixture already conforms to it.**
//! `rung_of` classifies into 2..=6 and no higher, so `violations(doc, 6)` is empty for every
//! document that can be written and `aboveCeiling` is always 0 here. Two consequences, both
//! structural rather than cosmetic:
//!
//! * There is no `demote-shape-representation` arm. `✳️cc2`..`✳️cc5` have instances above their
//!   ceiling to bring down; CC6 has none, so a demotion kind would be one that can never move the
//!   projection — a scenario that always passes and proves nothing.
//! * `set-shape-representation`'s guard still runs, and what it genuinely refuses here is a type
//!   that is not on the ladder at all. The tests assert that, rather than pretending a rung 7 exists.
//!
...
//! @see ../🧪️oracle/🔣️component.json — the `step-ap214-cc6` catalog and its no-oracle-free entry.
...
mod oracles {
    ...
    /// and mirroring `StepCc6Mutation::inverse()`'s own base-relative semantics.
    ///
/// A ladder edit inverts to the base's own representation, always: nothing is above this ceiling, so
/// the whole-document degradation the other five classes need is unreachable here. It is written
/// anyway, because the inversion rule is shared with five classes for which it IS reachable.
    ...
    /// 👁️ The conformance-class projection — the three axes `check_cc6_conformance` reads, measured
...
#[cfg(feature = "oracles")]
pub fn project_step_ap214_cc6(bytes: &[u8]) -> Result<Json, String> {
...
#[cfg(not(feature = "oracles"))]
pub fn project_step_ap214_cc6(_bytes: &[u8]) -> Result<Json, String> {
    Err("the `oracles` feature is not enabled, so no reference implementation is linked".to_string())
}
//#endregion 🔖️Dispatch
...
mod tests {
    ...
    fn the_real_export_reads_the_way_this_class_predicts() {
        ...
        assert_eq!(before.get("aboveCeiling"), Some(&Json::Number(0.0)), "nothing can be above the top of the ladder");
        assert_eq!(before.get("conformsToClass"), Some(&Json::Bool(true)), "CC6 is the one class the committed real export already conforms to");
    ...
    fn exercised_specs() -> Vec<Json> {
        vec![
            ...
            spec("set-shape-representation", object(vec![("id", Json::Number(13.0)), ("representation", Json::Null)])),
        ]
    }
    ...
    fn kinds_match_the_catalog_and_the_vocabulary() {
        ...
        let feature = include_str!("../../../../../🧪️tests/mutate-step-ap214-cc6/component.feature");
        for kind in KINDS {
    ...
    fn a_type_that_is_not_on_the_ladder_is_still_refused() {
        let row = object(vec![("typeName", Json::String("MANIFOLD_SOLID_BREP".to_string())), ("name", Json::String("not a representation".to_string()))]);
        let refusal = oracle_apply_mutation(&fixture(), &spec("set-shape-representation", object(vec![("id", Json::Number(13.0)), ("representation", row)]))).unwrap_err();
        assert!(refusal.contains("not a *_SHAPE_REPRESENTATION type"), "{refusal}");
    }
}
//#endregion 🧪️Tests
