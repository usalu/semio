//! 🧬️ StepSnapshot schema (ap214/6️⃣cc6) — reuses the 🧱️base subset's `StepSnapshot` verbatim
...
//! `🪆️subsets/6️⃣cc6/🧬️schema/` is present per `🔣️taxonomy.json`'s `subsetChildDirs`, without
//! duplicating the schema definition.

pub use crate::artifacts::step::standards::v_ap214::subsets::base::schema::*;

//#region 🧬️Mutations
/// 🧬️ This subset's OWN mutation vocabulary — one kind per ISO 10303-214 CC6 (advanced B-Rep, top of the ladder) conformance
/// rule, derived from `check_cc6_conformance` below rather than copied from a sibling class, and
...
pub mod derived_construction {
    ...
    mod tests {
        ...
        async fn top_of_ladder_representation_injected_via_raw_mutate_still_builds() {
            ...
            let (mutated, _diff) = StepCc6BuilderConstruction::from_snapshot(StepSnapshot::default()).mutate(StepMutation::SetSnapshot(crate::artifacts::step::standards::v_ap214::subsets::base::schema::mutations::set_snapshot::SetSnapshot { snapshot }));
...
pub mod derived_analysis {
    ...
    //#region 🔖️Analyzer
    /// 🧐️ Analyzes `stdio.step` (ap214/6️⃣cc6): delegates the real parse to the 🧱️base subset's
