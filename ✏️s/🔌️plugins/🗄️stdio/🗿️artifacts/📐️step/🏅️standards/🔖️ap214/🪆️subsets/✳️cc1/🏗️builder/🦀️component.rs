//! 🏗️ StepCc1Builder (ap214/✳️cc1) — a local `ArtifactBuilder` (until SDK Wave 3, same as
//! the artifact-level `StepBuilder`) whose `build()` re-runs `check_cc1_conformance` as a hard
//! gate, unconditionally, regardless of which construction path produced the in-flight snapshot
//! -- so a hard ISO 10303-214 CC1 (config data only) violation can never leave this builder as an `Ok(StepSnapshot)`.
...
pub struct StepCc1Builder {
    snapshot: StepSnapshot,
}

impl ArtifactBuilder for StepCc1Builder {
...
mod tests {
    ...
    use crate::artifacts::step::standards::v_ap214::subsets::cc1::analyzer::CODE_SHAPE_REPRESENTATION_PRESENT;
    ...
    fn conforming_construction_builds() {
        let snapshot = StepCc1Builder::from_snapshot(conforming_snapshot()).build().expect("conforming construction must build");
    ...
    fn hard_violation_injected_via_raw_mutate_still_fails_build() {
        ...
        let (mutated, _diff) = StepCc1Builder::from_snapshot(StepSnapshot::default()).mutate(StepMutation::SetSnapshot { snapshot });
