//! 🔮️ Mutation oracle for `ap214/4️⃣cc4` — ISO 10303-214 CC4 (manifold surfaces with topology).
//!
...
//! `check_cc4_conformance` actually reads, and the projection reports those axes and nothing else.
//!
//! 🪜️ **CC4 is the first class with topology.** A manifold surface representation carries shells,
//! and a shell is a topological object rather than a bag of surfaces. What CC4 refuses is the step
//! to a SOLID: `FACETED_BREP_SHAPE_REPRESENTATION` bounds a volume, and a class scoped to surfaces
//! has no conformant state that contains one.
//!
//! 🧬️ **This catalog declares exactly the same six kinds as `2️⃣cc2`, `3️⃣cc3`, `5️⃣cc5`, and that is a
...
//! ladder, and the product identity chain — because §4.3 varies only the ceiling. `1️⃣cc1` sits below
//! the ladder and `6️⃣cc6` on top of it, and each declares five kinds instead. The derivation is
...
//! support. What distinguishes THIS module from those three siblings is `MAX_RUNG = 4` and the
...
//! @see 🔣️.json — the `step-ap214-cc4` catalog and the `ruststep` registration it carries.
...
/// 🏷️ The declared vocabulary, mirroring `StepCc4Mutation`'s own variants in declaration order.
...
mod oracles {
    ...
    /// and mirroring `StepCc4Mutation::inverse()`'s own base-relative semantics.
    ///
/// A ladder edit inverts to the base's own representation when that representation is admissible in
/// THIS class, and to a whole-document restore when it is not. The real export's `#13` is two rungs
/// above this ceiling, so it takes the second path.
    ...
    /// 👁️ The conformance-class projection — the three axes `check_cc4_conformance` reads, measured
...
#[cfg(feature = "oracles")]
pub fn project_step_ap214_cc4(bytes: &[u8]) -> Result<Json, String> {
...
#[cfg(not(feature = "oracles"))]
pub fn project_step_ap214_cc4(_bytes: &[u8]) -> Result<Json, String> {
    Err("the `oracles` feature is not enabled, so no reference implementation is linked".to_string())
}
//#endregion 🔖️Dispatch
