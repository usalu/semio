//! 🔮️ Mutation oracle for `ap214/3️⃣cc3` — ISO 10303-214 CC3 (wireframe with topology).
//!
...
//! `check_cc3_conformance` actually reads, and the projection reports those axes and nothing else.
//!
//! 🪜️ **CC3 adds surfaces to CC2's curves, still carried geometrically.** Its ceiling is
//! `GEOMETRICALLY_BOUNDED_SURFACE_SHAPE_REPRESENTATION`; the step it refuses is to
//! `MANIFOLD_SURFACE_SHAPE_REPRESENTATION`, where the same surfaces acquire a topological shell.
//! The line this class draws is therefore about topology, not about geometry, and the guard below
//! is what makes that concrete.
//!
//! 🧬️ **This catalog declares exactly the same six kinds as `2️⃣cc2`, `4️⃣cc4`, `5️⃣cc5`, and that is a
...
//! @see 🔣️.json — the `step-ap214-cc3` catalog and the `ruststep` registration it carries.
...
/// 🏷️ The declared vocabulary, mirroring `StepCc3Mutation`'s own variants in declaration order.
...
mod oracles {
    ...
    /// and mirroring `StepCc3Mutation::inverse()`'s own base-relative semantics.
    ///
/// A ladder edit inverts to the base's own representation when that representation is admissible in
/// THIS class, and to a whole-document restore when it is not — which for the real export's rung-6
/// `#13` is three rungs of difference.
    ...
    /// 👁️ The conformance-class projection — the three axes `check_cc3_conformance` reads, measured
...
#[cfg(feature = "oracles")]
pub fn project_step_ap214_cc3(bytes: &[u8]) -> Result<Json, String> {
...
#[cfg(not(feature = "oracles"))]
pub fn project_step_ap214_cc3(_bytes: &[u8]) -> Result<Json, String> {
    Err("the `oracles` feature is not enabled, so no reference implementation is linked".to_string())
}
//#endregion 🔖️Dispatch
