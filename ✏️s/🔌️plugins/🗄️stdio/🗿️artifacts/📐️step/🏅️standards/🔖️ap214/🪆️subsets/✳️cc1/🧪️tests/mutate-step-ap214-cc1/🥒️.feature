@capability-step-ap214-cc1-mutate
@oracle-ruststep-step-ap214-cc1-mutate
...
Feature: Apply every typed ISO 10303-214 CC1 (config data only) mutation to a real AP214 exchange structure
  ...
  it is a FILTER, and the 5 kinds here are one per axis `check_cc1_conformance` actually reads:
  ...
  🪜️ What this class is, and what that costs the real file. CC1 is "configuration data only": it
  admits NO `*_SHAPE_REPRESENTATION` instance whatsoever. `ladder_rung_of` never answers below 2, so
  CC1's ceiling of 1 is below every rung there is, and BOTH of the real export's representations are
  violations of it — `#13`, a real `ADVANCED_BREP_SHAPE_REPRESENTATION` (rung 6), and `#836`, the
  bare `SHAPE_REPRESENTATION` (rung 2). That is why this case's vocabulary has a `remove-shape-
  representation` row and no `set-shape-representation` row at all: there is no representation this
  class could be asked to write. It is the only one of the six with that shape.
  ...
  Scenario Outline: Apply <id> and read the conformance projection back
    ...
    Examples:
      ...
      | remove-shape-representation | {"id": 13}                                                                                                                                                                    |
