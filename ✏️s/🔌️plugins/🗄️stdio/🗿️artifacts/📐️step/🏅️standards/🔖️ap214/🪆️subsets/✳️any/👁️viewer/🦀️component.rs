...
//#region 🔖️Manifest
pub async fn create_step_any_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(STEP_ANY_DIALECT)
        .await.document(["stdio", "step"])
        .await.icon_id("box")
        .await.mode_def(view::definition().await)
        .await.default_mode_id(view::STEP_ANY_VIEW_MODE_ID)
