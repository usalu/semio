//! 🔮️ Mutation oracle for `ap214/✳️cc2` — ISO 10303-214 CC2 (bounded wireframe/basic surfaces).
//!
...
//! `check_cc2_conformance` actually reads, and the projection reports those axes and nothing else.
//!
//! 🪜️ **CC2 is the first class that admits geometry, and it admits only one kind of it.** Its
//! ceiling is `GEOMETRICALLY_BOUNDED_WIREFRAME_SHAPE_REPRESENTATION` — curves and points carried
//! directly, with no topology and no surfaces — so `set-shape-representation` refuses all four types
//! above it. Against the real export, `#13` (rung 6) is above the ceiling and `#836` (the bare
//! `SHAPE_REPRESENTATION`, rung 2) already sits on it, so one demotion is the whole repair.
//!
...
//! support. What distinguishes THIS module from those three siblings is `MAX_RUNG = 2` and the
...
//! @see 🔣️.json — the `step-ap214-cc2` catalog and the `ruststep` registration it carries.
...
/// 🏷️ The declared vocabulary, mirroring `StepCc2Mutation`'s own variants in declaration order.
...
mod oracles {
    ...
    /// and mirroring `StepCc2Mutation::inverse()`'s own base-relative semantics.
    ///
/// A ladder edit inverts to the base's own representation when that representation is admissible in
/// THIS class, and to a whole-document restore when it is not. Against the real export the second
/// case is what `#13` takes: rung 6 is four rungs above CC2's ceiling.
    ...
    /// 👁️ The conformance-class projection — the three axes `check_cc2_conformance` reads, measured
...
#[cfg(feature = "oracles")]
pub fn project_step_ap214_cc2(bytes: &[u8]) -> Result<Json, String> {
...
#[cfg(not(feature = "oracles"))]
pub fn project_step_ap214_cc2(_bytes: &[u8]) -> Result<Json, String> {
    Err("the `oracles` feature is not enabled, so no reference implementation is linked".to_string())
}
//#endregion 🔖️Dispatch
...
mod tests {
    ...
    fn the_real_export_reads_the_way_this_class_predicts() {
        ...
        assert_eq!(before.get("aboveCeiling"), Some(&Json::Number(1.0)), "only the rung-6 #13 is above CC2; the bare SHAPE_REPRESENTATION at #836 sits exactly on the ceiling");
    ...
    fn kinds_match_the_catalog_and_the_vocabulary() {
        ...
        let feature = include_str!("../../../../../🧪️tests/mutate-step-ap214-cc2/🥒️.feature");
        for kind in KINDS {
            assert!(manifest.contains(&format!("\"{kind}\"")), "the catalog is missing kind {kind:?}");
            assert!(vocabulary.contains(&format!("\"{kind}\"")), "StepCc2Mutation::KINDS is missing {kind:?}");
