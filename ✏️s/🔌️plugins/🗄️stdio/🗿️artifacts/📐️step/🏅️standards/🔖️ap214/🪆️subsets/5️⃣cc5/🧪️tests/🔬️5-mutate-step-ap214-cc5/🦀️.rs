//! 🦀️ STEP AP214 `5️⃣cc5` mutation case — Rust adapter. ISO 10303-214 CC5 (faceted B-Rep).
//!
...
//! it registers are one per axis `check_cc5_conformance` reads, and the projection it compares by
...
use semio_s_plugin_stdio_test_oracle::artifacts::step::standards::v_ap214::subsets::cc5::{oracle_apply_mutation, oracle_inverse_spec, oracle_round_trip, project_step_ap214_cc5};
...
const CEILING: &str = "FACETED_BREP_SHAPE_REPRESENTATION";
//#endregion 🔖️Kinds
...
///   repaired nothing.
/// * `set-shape-representation` must leave `#836` carrying this class's own ceiling type, `FACETED_BREP_SHAPE_REPRESENTATION`
...
fn mutate_oracle(ctx: &Context) -> Result<Outcome, String> {
    ...
    let before = project_step_ap214_cc5(&oracle_apply_mutation(&input, &no_mutation())?)?;
    let bytes = oracle_apply_mutation(&input, &spec)?;
    let projection = project_step_ap214_cc5(&bytes)?;
...
fn inverse_oracle(ctx: &Context) -> Result<Outcome, String> {
    ...
    let spec = ctx.doc_json()?;
    let kind = spec.str("kind");
    let baseline = project_step_ap214_cc5(&oracle_apply_mutation(&input, &no_mutation())?)?;
...
fn identity_round_trip_oracle(ctx: &Context) -> Result<Outcome, String> {
    let input = mutable_input(ctx)?;
    let before = project_step_ap214_cc5(&input)?;
...
mod subject {
    ...
    use semio_s_artifact_stdio_step::standards::v_ap214::subsets::cc5::schema::mutations::{apply_step_cc5_mutation_checked, inverse_step_cc5_mutation, StepCc5Mutation};
    ...
    /// 🦠️ The `(kind, params)` wire shape read into a real `StepCc5Mutation`. `set-snapshot` builds
    ...
    fn mutation_from_spec(spec: &Json, base: &StepSnapshot) -> Result<StepCc5Mutation, String> {
        ...
        Ok(match spec.str("kind").as_str() {
            "set-snapshot" => {
                ...
                let _ = base;
                StepCc5Mutation::SetSnapshot(semio_s_artifact_stdio_step::standards::v_ap214::subsets::cc5::schema::mutations::set_snapshot::SetSnapshot { snapshot })
            }
            "set-file-schema" => {
                ...
                StepCc5Mutation::SetFileSchema(semio_s_artifact_stdio_step::standards::v_ap214::subsets::cc5::schema::mutations::set_file_schema::SetFileSchema { schemas })
            }
            "set-product-identity" => StepCc5Mutation::SetProductIdentity(semio_s_artifact_stdio_step::standards::v_ap214::subsets::cc5::schema::mutations::set_product_identity::SetProductIdentity {
                identity: match params.get("identity").filter(|value| !matches!(value, Json::Null)) {
                    Some(value) => Some(identity_from(value)?),
                    None => None,
                },
            }),
            "set-shape-representation" => StepCc5Mutation::SetShapeRepresentation(semio_s_artifact_stdio_step::standards::v_ap214::subsets::cc5::schema::mutations::set_shape_representation::SetShapeRepresentation {
            ...
            "demote-shape-representation" => StepCc5Mutation::DemoteShapeRepresentation(semio_s_artifact_stdio_step::standards::v_ap214::subsets::cc5::schema::mutations::demote_shape_representation::DemoteShapeRepresentation { id: u64_field(&params, "id")? }),
    ...
    fn apply_and_encode(input: &[u8], spec: &Json) -> Result<Vec<u8>, String> {
        ...
        apply_step_cc5_mutation_checked(&mut snapshot, &mutation)?;
    ...
    pub fn mutate(ctx: &Context) -> Result<Outcome, String> {
        ...
        let spec = ctx.doc_json()?;
        let kind = spec.str("kind");
        let before = project_step_ap214_cc5(&apply_and_encode(&input, &no_mutation())?)?;
    ...
    pub fn inverse(ctx: &Context) -> Result<Outcome, String> {
        ...
        let spec = ctx.doc_json()?;
        let kind = spec.str("kind");
        let baseline = project_step_ap214_cc5(&apply_and_encode(&input, &no_mutation())?)?;
        let text = std::str::from_utf8(&input).map_err(|error| format!("input is not UTF-8: {error}"))?;
        ...
        if kind != "no-mutation" {
            for step in inverse_step_cc5_mutation(&base, &mutation_from_spec(&spec, &base)?) {
                apply_step_cc5_mutation_checked(&mut snapshot, &step)?;
            }
        }
