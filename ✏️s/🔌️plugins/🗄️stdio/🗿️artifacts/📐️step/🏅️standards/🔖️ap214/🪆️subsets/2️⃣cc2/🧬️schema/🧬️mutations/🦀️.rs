...
mod tests {
    ...
    fn the_class_ceiling_is_the_line_this_vocabulary_draws() {
        ...
        assert!(apply_step_cc2_mutation(&mut snapshot, &StepCc2Mutation::SetShapeRepresentation(set_shape_representation::SetShapeRepresentation { id: 13, representation: Some(at_ceiling) })).messages().is_empty(), "this class admits its own ceiling type");
    ...
    fn a_rejected_mutation_leaves_the_snapshot_untouched() {
        let mut snapshot = base();
        assert!(!apply_step_cc2_mutation(&mut snapshot, &StepCc2Mutation::SetShapeRepresentation(set_shape_representation::SetShapeRepresentation { id: 827, representation: None })).messages().is_empty(), "a conformance repair must never delete a product record");
