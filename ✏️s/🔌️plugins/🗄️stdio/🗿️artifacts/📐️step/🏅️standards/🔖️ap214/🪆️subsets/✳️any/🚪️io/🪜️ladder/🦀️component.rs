...
/// deletion — which is why `✳️cc1`'s vocabulary carries `remove-shape-representation` where
/// `✳️cc2`..`✳️cc5` carry `demote-shape-representation`.
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
...
/// (`✳️cc1` has no way to write a representation at all; `✳️cc6` has nothing to demote from), and
