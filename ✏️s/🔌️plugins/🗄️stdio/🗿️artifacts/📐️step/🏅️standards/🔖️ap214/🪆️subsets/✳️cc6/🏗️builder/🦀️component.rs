//! 🏗️ StepCc6Builder (ap214/✳️cc6) — a local `ArtifactBuilder` (until SDK Wave 3, same as
//! the artifact-level `StepBuilder`) whose `build()` re-runs `check_cc6_conformance` as a hard
//! gate, unconditionally, regardless of which construction path produced the in-flight snapshot
//! -- so a hard ISO 10303-214 CC6 (advanced B-Rep, top of the ladder) violation can never leave this builder as an `Ok(StepSnapshot)`.

use dsl::{Diagnostic, Severity};
use semio_framework_plugin::ArtifactBuilder;
use crate::artifacts::step::{StepDiff, StepMutation, StepSnapshot};
use crate::artifacts::step::standards::v_ap214::subsets::cc6::analyzer::check_cc6_conformance;

//#region 🔖️Builder
#[derive(Clone, Debug, Default)]
pub struct StepCc6Builder {
    snapshot: StepSnapshot,
}

impl ArtifactBuilder for StepCc6Builder {
...
mod tests {
    ...
    fn conforming_construction_builds() {
        let snapshot = StepCc6Builder::from_snapshot(conforming_snapshot()).build().expect("conforming construction must build");
    ...
    fn top_of_ladder_representation_injected_via_raw_mutate_still_builds() {
        ...
        let (mutated, _diff) = StepCc6Builder::from_snapshot(StepSnapshot::default()).mutate(StepMutation::SetSnapshot { snapshot });
