//! 👁️ Step CC2 viewer — the read-only counterpart of `✏️editor` for this subset (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `StepCc2Viewer`
//! implements `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<StepCc2Viewer>`
...
pub enum StepCc2ViewCommand {
    #[default]
    Noop,
}

impl protocol::OpBinary for StepCc2ViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(StepCc2ViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
impl ArtifactViewer for StepCc2Viewer {
    ...
    type Command = StepCc2ViewCommand;
...
pub fn create_step_cc2_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(STEP_CC2_DIALECT).document(["stdio", "step"]).icon_id("box").mode_def(view::definition()).default_mode_id(view::STEP_CC2_VIEW_MODE_ID).window_kind_def(main::definition()).default_layout(view::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn create_viewer_builds_a_definition_for_the_viewer_role() {
        let def = create_step_cc2_viewer();
    ...
    async fn viewer_dialect_matches_the_artifact_coordinate() {
        assert_eq!(<StepCc2Viewer as ArtifactViewer>::DIALECT, STEP_CC2_DIALECT);
    }

    #[semio_framework_async_macros::async_test]
    async fn viewer_never_mutates_the_document_or_draft_store() {
        semio_framework_plugin::testkit::assert_viewer_never_mutates::<StepCc2Viewer>();
    }
}
//#endregion 🧪️Tests
