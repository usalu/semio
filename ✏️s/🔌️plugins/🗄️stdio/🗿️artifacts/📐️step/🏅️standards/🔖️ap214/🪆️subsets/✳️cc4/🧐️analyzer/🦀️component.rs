//! 🧐️ StepCc4Analyzer (ap214/✳️cc4) — real ISO 10303-214 CC4 (manifold surfaces with topology) conformance checks against the
...
//! - HARD: any `*_SHAPE_REPRESENTATION` instance whose ladder rung exceeds 4 (see
//!   `⚙️engine::ladder::ladder_rung_of` for the classification and rationale; cc4 allows rungs
//!   up to 4).
...
pub fn check_cc4_conformance(snapshot: &StepSnapshot) -> Vec<Diagnostic> {
    ...
    for (id, type_name, rung) in ladder_violations(&doc, MAX_RUNG) {
        out.push(hard(
            CODE_LADDER,
            format!("instance #{id} is a {type_name} (ladder rung {rung}) -- exceeds cc4's max rung 4"),
        ));
    }
...
//#region 🔖️Analyzer
/// 🧐️ Analyzes `stdio.step` (ap214/✳️cc4): delegates the real parse to the ✳️any subset's
...
pub struct StepCc4Analyzer;

impl ArtifactAnalyzer for StepCc4Analyzer {
