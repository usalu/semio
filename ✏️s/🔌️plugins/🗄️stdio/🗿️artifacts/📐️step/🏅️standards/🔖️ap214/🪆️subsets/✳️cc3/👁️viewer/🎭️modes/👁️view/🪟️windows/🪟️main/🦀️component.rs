...
pub async fn render(document: &StepSnapshot) -> UiNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": STEP_CC3_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(STEP_CC3_VIEW_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(STEP_CC3_VIEW_DEFAULT_CAMERA_POSITION, STEP_CC3_VIEW_DEFAULT_CAMERA_TARGET, STEP_CC3_VIEW_DEFAULT_CAMERA_FOV).await,
        meshes_json,
