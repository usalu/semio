//! 🎹️ StepCc4Composer (ap214/✳️cc4) — reads the same sources the ✳️any subset does (native
...
//! roster: "set file_schema=AUTOMOTIVE_DESIGN"), then HARD-GATES the `cc4` dialect stamp on
//! real ISO 10303-214 CC4 (manifold surfaces with topology) conformance. A hard violation (missing `FILE_SCHEMA` declaration after
//! injection is impossible; a `*_SHAPE_REPRESENTATION` instance above rung 4) fails composition
...
//! SAME `check_cc4_conformance` function backs both: the hard gate here runs pre-serialization
...
//#region 🔖️Composer
pub struct StepCc4Composer;

impl ArtifactComposer for StepCc4Composer {
...
mod tests {
    ...
    fn composer_injects_file_schema_and_stamps_clean_document() {
        ...
        let composed = StepCc4Composer::compose(&sources).expect("a document with no illegal representation must compose to cc4");
    ...
    fn subset_validator_recheck_flags_missing_file_schema_on_the_raw_wire_payload() {
        ...
        assert!(diagnostics.iter().any(|d| d.code.0 == crate::artifacts::step::standards::v_ap214::subsets::cc4::analyzer::CODE_FILE_SCHEMA), "got {diagnostics:?}");
    }
}
