//! 🧊 Step CC3 viewer — the Main window: a read-only world-3d mesh scene built with the shared
//! `MeshWindowKit` (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.6), from the
//! artifact-level `StepSnapshot` — this file imports nothing from the sibling editor module
...
const STEP_CC3_VIEW_FALLBACK_MESH_KIND: &str = "box";
const STEP_CC3_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const STEP_CC3_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const STEP_CC3_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
fn entity_count(document: &StepSnapshot) -> usize {
...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
fn world_instances_json(document: &StepSnapshot) -> String {
    let count = entity_count(document);
    let instances: Vec<pack::JsonValue> = (0..count)
        .map(|index| {
            pack::json!({
                "id": format!("step_cc3-{index}"),
                "meshId": STEP_CC3_VIEW_FALLBACK_MESH_KIND,
                "position": [index as f64 * 2.0, 0.0, 0.0],
                "rotation": [0.0, 0.0, 0.0, 1.0],
                "scale": [1.0, 1.0, 1.0],
                "label": format!("Step CC3 {index}"),
                "smoothShading": false,
            })
        })
        .collect();
...
pub fn render(document: &StepSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = pack::json_to_string(&pack::json_array([pack::json!({ "id": STEP_CC3_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(STEP_CC3_VIEW_FALLBACK_MESH_KIND) })]));
    let view = MeshView {
        camera_json: world3d_camera_json(STEP_CC3_VIEW_DEFAULT_CAMERA_POSITION, STEP_CC3_VIEW_DEFAULT_CAMERA_TARGET, STEP_CC3_VIEW_DEFAULT_CAMERA_FOV),
        meshes_json,
