...
//! filter. Every variant below is one rule of `check_cc5_conformance` (`../🦀️component.rs`'s
...
//! @see ../🧪️oracle/🔣️component.json — the `step-ap214-cc5` catalog `KINDS` is checked against.
...
use crate::artifacts::step::standards::v_ap214::subsets::cc5::schema::MAX_RUNG;
...
/// 📐️ Typed conformance-class mutation for `stdio.step` at `ap214/✳️cc5`.
...
pub enum StepCc5Mutation {
    ...
    /// is this class's own: `FACETED_BREP_SHAPE_REPRESENTATION` passes, anything above it is refused with a message naming
    /// the rung and the ceiling.
    SetShapeRepresentation {
        id: u64,
        representation: Option<ShapeRepresentationRow>,
    },
    /// ⬇️ The conformance repair: rewrite an over-rung representation onto `FACETED_BREP_SHAPE_REPRESENTATION`, keeping its
...
/// `step-ap214-cc5` catalog in `../../🧪️oracle/🔣️component.json` must match verbatim.
...
impl StepCc5Mutation {
    ...
    pub fn kind(&self) -> &'static str {
        match self {
            StepCc5Mutation::NoMutation => "no-mutation",
            StepCc5Mutation::SetSnapshot { .. } => "set-snapshot",
            StepCc5Mutation::SetFileSchema { .. } => "set-file-schema",
            StepCc5Mutation::SetProductIdentity { .. } => "set-product-identity",
            StepCc5Mutation::SetShapeRepresentation { .. } => "set-shape-representation",
            StepCc5Mutation::DemoteShapeRepresentation { .. } => "demote-shape-representation",
        }
    }
    ...
    fn class_edit(&self) -> Option<ClassEdit> {
        match self {
            StepCc5Mutation::NoMutation | StepCc5Mutation::SetSnapshot { .. } => None,
            StepCc5Mutation::SetFileSchema { schemas } => Some(ClassEdit::FileSchema { schemas: schemas.clone() }),
            StepCc5Mutation::SetProductIdentity { identity } => Some(ClassEdit::ProductIdentity { identity: identity.clone() }),
            StepCc5Mutation::SetShapeRepresentation { id, representation } => Some(ClassEdit::Representation { id: *id, row: representation.clone() }),
            StepCc5Mutation::DemoteShapeRepresentation { id } => Some(ClassEdit::Demotion { id: *id }),
        }
    }
}
...
impl Mutation<StepSnapshot> for StepCc5Mutation {
    type Diff = StepDiff;

    fn diff(&self, base: &StepSnapshot) -> protocol::MutationOutcome<Self::Diff> {
        match self.class_edit() {
            None => match self {
                StepCc5Mutation::SetSnapshot { snapshot } => protocol::MutationOutcome::new(<StepDiff as DiffAlgebra<StepSnapshot>>::between(base, snapshot)),
    ...
    fn inverse(&self, base: &StepSnapshot) -> Vec<Self> {
        let restore = || vec![StepCc5Mutation::SetSnapshot { snapshot: base.clone() }];
        match self.class_edit() {
            None => match self {
                StepCc5Mutation::SetSnapshot { .. } => restore(),
                _ => vec![StepCc5Mutation::NoMutation],
            },
            Some(edit) => match ladder::invert_class_edit(&base.to_part21_document(), MAX_RUNG, &edit) {
                Some(ClassEdit::FileSchema { schemas }) => vec![StepCc5Mutation::SetFileSchema { schemas }],
                Some(ClassEdit::ProductIdentity { identity }) => vec![StepCc5Mutation::SetProductIdentity { identity }],
                Some(ClassEdit::Representation { id, row }) => vec![StepCc5Mutation::SetShapeRepresentation { id, representation: row }],
                _ => restore(),
            },
        }
    }
...
/// ▶️ [`apply_step_cc5_mutation`] in a signature that names only this subset's own public types, so
...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn apply_step_cc5_mutation_checked(snapshot: &mut StepSnapshot, mutation: &StepCc5Mutation) -> Result<(), String> {
    let outcome = apply_step_cc5_mutation(snapshot, mutation);
...
/// ↩️ `Mutation::inverse` for `StepCc5Mutation`, reachable without naming the `protocol` alias — the
/// production inverse itself, never a copy of its rules.
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn inverse_step_cc5_mutation(base: &StepSnapshot, mutation: &StepCc5Mutation) -> Vec<StepCc5Mutation> {
    <StepCc5Mutation as Mutation<StepSnapshot>>::inverse(mutation, base)
}
...
mod tests {
    ...
    use crate::artifacts::step::standards::v_ap214::subsets::cc5::schema::check_cc5_conformance;
    ...
    fn conforming() -> StepSnapshot {
        let mut doc = base().to_part21_document();
        ladder::demote_shape_representation(&mut doc, 13, "FACETED_BREP_SHAPE_REPRESENTATION").expect("the base carries a real representation");
        StepSnapshot::from_part21_document(doc)
    }

    fn round_trip(start: StepSnapshot, mutation: StepCc5Mutation) {
        let mut mutated = start.clone();
        let outcome = apply_step_cc5_mutation(&mut mutated, &mutation);
        ...
        for step in Mutation::inverse(&mutation, &start) {
            apply_step_cc5_mutation(&mut mutated, &step);
        }
    ...
    fn every_conformance_axis_round_trips_through_its_own_inverse() {
        round_trip(base(), StepCc5Mutation::SetFileSchema { schemas: vec!["CONFIG_CONTROL_DESIGN".into()] });
        round_trip(base(), StepCc5Mutation::SetProductIdentity { identity: None });
        round_trip(base(), StepCc5Mutation::SetShapeRepresentation { id: 13, representation: None });
        round_trip(conforming(), StepCc5Mutation::SetShapeRepresentation { id: 13, representation: None });
        round_trip(base(), StepCc5Mutation::DemoteShapeRepresentation { id: 13 });
    }
    ...
    fn the_class_ceiling_is_the_line_this_vocabulary_draws() {
        ...
        assert_eq!(at_ceiling.type_name, "FACETED_BREP_SHAPE_REPRESENTATION");
        assert!(apply_step_cc5_mutation(&mut snapshot, &StepCc5Mutation::SetShapeRepresentation { id: 13, representation: Some(at_ceiling) }).messages().is_empty(), "this class admits its own ceiling type");

        let above = ShapeRepresentationRow { type_name: "ADVANCED_BREP_SHAPE_REPRESENTATION".into(), name: "too high".into(), items: vec![12], context: Some(835) };
        let outcome = apply_step_cc5_mutation(&mut snapshot, &StepCc5Mutation::SetShapeRepresentation { id: 13, representation: Some(above) });
        let message = &outcome.messages().first().expect("rung 6 is above this class").message;
        assert!(message.contains("rung 6") && message.contains("ceiling of 5"), "the refusal must name both rungs: {message}");
    ...
    fn each_verb_moves_the_diagnostic_it_was_derived_from() {
        ...
        apply_step_cc5_mutation(&mut snapshot, &StepCc5Mutation::DemoteShapeRepresentation { id: 13 });
        ...
        assert!(check_cc5_conformance(&snapshot).is_empty(), "and with the schema and the chain already right, nothing else is left to report");

        apply_step_cc5_mutation(&mut snapshot, &StepCc5Mutation::SetProductIdentity { identity: None });
        assert!(!has_product_definition_chain(&snapshot.to_part21_document()));
        apply_step_cc5_mutation(&mut snapshot, &StepCc5Mutation::SetFileSchema { schemas: vec!["IFC4".into()] });
    ...
    fn a_rejected_mutation_leaves_the_snapshot_untouched() {
        let mut snapshot = base();
        assert!(!apply_step_cc5_mutation(&mut snapshot, &StepCc5Mutation::SetShapeRepresentation { id: 827, representation: None }).messages().is_empty(), "a conformance repair must never delete a product record");
        assert!(!apply_step_cc5_mutation(&mut snapshot, &StepCc5Mutation::SetFileSchema { schemas: vec![] }).messages().is_empty());
    ...
    fn no_mutation_is_the_identity() {
        let mut snapshot = base();
        assert!(apply_step_cc5_mutation(&mut snapshot, &StepCc5Mutation::NoMutation).messages().is_empty());
        assert_eq!(snapshot, base());
    }
    ...
    fn kinds_const_matches_enum_variants_in_declaration_order() {
        let one_per_variant = vec![
            StepCc5Mutation::NoMutation,
            StepCc5Mutation::SetSnapshot { snapshot: StepSnapshot::default() },
            StepCc5Mutation::SetFileSchema { schemas: Vec::new() },
            StepCc5Mutation::SetProductIdentity { identity: None },
            StepCc5Mutation::SetShapeRepresentation { id: 0, representation: None },
            StepCc5Mutation::DemoteShapeRepresentation { id: 0 },
        ];
