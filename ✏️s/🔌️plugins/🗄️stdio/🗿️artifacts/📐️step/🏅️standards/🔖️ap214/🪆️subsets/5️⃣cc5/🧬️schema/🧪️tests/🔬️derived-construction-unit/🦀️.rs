mod tests {
    ...
    use crate::standards::v_ap214::subsets::cc5::schema::CODE_LADDER;
    ...
    async fn conforming_construction_builds() {
        let snapshot = StepCc5BuilderConstruction::from_snapshot(conforming_snapshot()).build().expect("conforming construction must build");
    ...
    async fn hard_violation_injected_via_raw_mutate_still_fails_build() {
        ...
        let (mutated, _diff) = StepCc5BuilderConstruction::from_snapshot(StepSnapshot::default()).mutate(StepMutation::SetSnapshot(crate::standards::v_ap214::subsets::base::schema::mutations::set_snapshot::SetSnapshot { snapshot }));
