...
/// 🧾️ Case-local mirror of the `step-ap214-cc2` catalog. Duplicated rather than imported: `KINDS`
...
fn mutate_oracle(ctx: &Context) -> Result<Outcome, String> {
    ...
    let before = project_step_ap214_cc2(&oracle_apply_mutation(&input, &no_mutation())?)?;
...
fn inverse_oracle(ctx: &Context) -> Result<Outcome, String> {
    ...
    let spec = ctx.doc_json()?;
    let kind = spec.str("kind");
    let baseline = project_step_ap214_cc2(&oracle_apply_mutation(&input, &no_mutation())?)?;
...
mod subject {
    ...
    /// 🦠️ The `(kind, params)` wire shape read into a real `StepCc2Mutation`. `set-snapshot` builds
    ...
    fn mutation_from_spec(spec: &Json, base: &StepSnapshot) -> Result<StepCc2Mutation, String> {
        ...
        Ok(match spec.str("kind").as_str() {
            "set-snapshot" => {
                ...
                let _ = base;
                StepCc2Mutation::SetSnapshot(semio_s_artifact_stdio_step::standards::v_ap214::subsets::cc2::schema::mutations::set_snapshot::SetSnapshot { snapshot })
            }
            "set-file-schema" => {
                ...
                StepCc2Mutation::SetFileSchema(semio_s_artifact_stdio_step::standards::v_ap214::subsets::cc2::schema::mutations::set_file_schema::SetFileSchema { schemas })
            }
            "set-product-identity" => StepCc2Mutation::SetProductIdentity(semio_s_artifact_stdio_step::standards::v_ap214::subsets::cc2::schema::mutations::set_product_identity::SetProductIdentity {
                identity: match params.get("identity").filter(|value| !matches!(value, Json::Null)) {
                    Some(value) => Some(identity_from(value)?),
                    None => None,
                },
            }),
            "set-shape-representation" => StepCc2Mutation::SetShapeRepresentation(semio_s_artifact_stdio_step::standards::v_ap214::subsets::cc2::schema::mutations::set_shape_representation::SetShapeRepresentation {
            ...
            "demote-shape-representation" => StepCc2Mutation::DemoteShapeRepresentation(semio_s_artifact_stdio_step::standards::v_ap214::subsets::cc2::schema::mutations::demote_shape_representation::DemoteShapeRepresentation { id: u64_field(&params, "id")? }),
    ...
    fn apply_and_encode(input: &[u8], spec: &Json) -> Result<Vec<u8>, String> {
        ...
        apply_step_cc2_mutation_checked(&mut snapshot, &mutation)?;
    ...
    pub fn mutate(ctx: &Context) -> Result<Outcome, String> {
        ...
        let spec = ctx.doc_json()?;
        let kind = spec.str("kind");
        let before = project_step_ap214_cc2(&apply_and_encode(&input, &no_mutation())?)?;
    ...
    pub fn inverse(ctx: &Context) -> Result<Outcome, String> {
        ...
        let spec = ctx.doc_json()?;
        let kind = spec.str("kind");
        let baseline = project_step_ap214_cc2(&apply_and_encode(&input, &no_mutation())?)?;
        let text = std::str::from_utf8(&input).map_err(|error| format!("input is not UTF-8: {error}"))?;
        ...
        if kind != "no-mutation" {
            for step in inverse_step_cc2_mutation(&base, &mutation_from_spec(&spec, &base)?) {
                apply_step_cc2_mutation_checked(&mut snapshot, &step)?;
            }
        }
