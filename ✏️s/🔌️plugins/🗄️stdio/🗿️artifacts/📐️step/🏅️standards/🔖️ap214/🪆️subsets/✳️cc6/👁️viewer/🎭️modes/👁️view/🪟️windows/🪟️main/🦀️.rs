//! 🧊 Step CC6 viewer — the Main window: a read-only world-3d mesh scene built with the shared
...
const STEP_CC6_VIEW_FALLBACK_MESH_KIND: &str = "box";
const STEP_CC6_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const STEP_CC6_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const STEP_CC6_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
    ...
    let instances: Vec<pack::JsonValue> = (0..count)
        .map(|index| {
            pack::json!({
                ...
                "meshId": STEP_CC6_VIEW_FALLBACK_MESH_KIND,
...
pub fn render(document: &StepSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = pack::json_to_string(&pack::json_array([pack::json!({ "id": STEP_CC6_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(STEP_CC6_VIEW_FALLBACK_MESH_KIND) })]));
    let view = MeshView {
