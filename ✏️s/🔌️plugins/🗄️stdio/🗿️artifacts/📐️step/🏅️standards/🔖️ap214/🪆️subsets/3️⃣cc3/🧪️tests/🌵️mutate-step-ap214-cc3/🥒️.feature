@capability-step-ap214-cc3-mutate
@oracle-ruststep-step-ap214-cc3-mutate
@comparison-semantic-step-conformance-class-v1
@mutations-step-ap214-cc3
Feature: Apply every typed ISO 10303-214 CC3 (wireframe with topology) mutation to a real AP214 exchange structure
  ...
  it is a FILTER, and the 6 kinds here are one per axis `check_cc3_conformance` actually reads:
  ...
  🪜️ What this class is. CC3 adds surfaces to CC2's curves and still carries them GEOMETRICALLY —
  bounded by their own geometry rather than by a topological shell. Its ceiling is
  `GEOMETRICALLY_BOUNDED_SURFACE_SHAPE_REPRESENTATION`, and the step it refuses is the one to
  `MANIFOLD_SURFACE_SHAPE_REPRESENTATION`, where the same surfaces acquire topology. The line this
  class draws is about topology, not about geometry, which is exactly what the `set-shape-
  representation` row below exercises: it writes CC3's own ceiling type onto the real `#836`.
  ...
  🧬️ Why this catalog is identical to `🔴️mutate-step-ap214-cc2`, `🧿️mutate-step-ap214-cc4`, `🟣️mutate-step-ap214-cc5`'s, and
  ...
  Scenario Outline: Apply <id> and read the conformance projection back
    ...
    Examples:
      ...
      | set-shape-representation    | {"id": 836, "representation": {"typeName": "GEOMETRICALLY_BOUNDED_SURFACE_SHAPE_REPRESENTATION", "name": "Document", "items": [837, 895], "context": 835}}                    |
