//! 🧐️ StepCc1Analyzer (ap214/✳️cc1) — real ISO 10303-214 CC1 (config data only) conformance
//! checks against the retained lossless `Part21Document` graph (`StepSnapshot::to_part21_document()`). CC1 is
//! the bottom rung: no shape geometry at all, only administrative/config product data. Checks
//! implemented as real, honest scans over `by_type`/`instances` (never fabricated against an
//! unmodeled field), shared with the other five `✳️ccN` subsets via `⚙️engine::ladder`:
//! - HARD: `FILE_SCHEMA` does not declare `AUTOMOTIVE_DESIGN` — every AP214 file must.
//! - HARD: ANY `*_SHAPE_REPRESENTATION` instance is present at all (CC1 permits none — modeled as
//!   `MAX_RUNG = 1`, a rung no real `*_SHAPE_REPRESENTATION` subtype ever classifies as, since the
//!   ladder starts at rung 2; see `⚙️engine::ladder::ladder_rung_of`).
//! - SOFT: no `PRODUCT` + `PRODUCT_DEFINITION_FORMATION` + `PRODUCT_DEFINITION` chain found — real
//!   AP214 config data normally carries one.
...
pub struct StepCc1Analyzer;

impl ArtifactAnalyzer for StepCc1Analyzer {
