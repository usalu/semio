//! 🎹️ StepCc6Composer (ap214/✳️cc6) — reads the same sources the ✳️any subset does (native
...
//! roster: "set file_schema=AUTOMOTIVE_DESIGN"), then HARD-GATES the `cc6` dialect stamp on
//! real ISO 10303-214 CC6 (advanced B-Rep, top of the ladder) conformance. A hard violation (missing `FILE_SCHEMA` declaration after
//! injection is impossible; a `*_SHAPE_REPRESENTATION` instance above rung 6) fails composition
...
//! SAME `check_cc6_conformance` function backs both: the hard gate here runs pre-serialization
...
//#region 🔖️Composer
pub struct StepCc6Composer;

impl ArtifactComposer for StepCc6Composer {
...
mod tests {
    ...
    fn composer_injects_file_schema_and_stamps_clean_document() {
        ...
        let composed = StepCc6Composer::compose(&sources).expect("a document with no illegal representation must compose to cc6");
    ...
    fn subset_validator_recheck_flags_missing_file_schema_on_the_raw_wire_payload() {
        ...
        assert!(diagnostics.iter().any(|d| d.code.0 == crate::artifacts::step::standards::v_ap214::subsets::cc6::analyzer::CODE_FILE_SCHEMA), "got {diagnostics:?}");
    }
}
