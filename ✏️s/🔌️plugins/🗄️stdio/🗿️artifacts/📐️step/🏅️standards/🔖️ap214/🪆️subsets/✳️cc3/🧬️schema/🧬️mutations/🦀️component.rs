...
//! filter. Every variant below is one rule of `check_cc3_conformance` (`../🦀️component.rs`'s
...
//! @see ../🧪️oracle/🔣️component.json — the `step-ap214-cc3` catalog `KINDS` is checked against.
...
/// 📐️ Typed conformance-class mutation for `stdio.step` at `ap214/✳️cc3`.
...
pub enum StepCc3Mutation {
    ...
    /// is this class's own: `GEOMETRICALLY_BOUNDED_SURFACE_SHAPE_REPRESENTATION` passes, anything above it is refused with a message naming
    /// the rung and the ceiling.
    SetShapeRepresentation {
        id: u64,
        representation: Option<ShapeRepresentationRow>,
    },
    /// ⬇️ The conformance repair: rewrite an over-rung representation onto `GEOMETRICALLY_BOUNDED_SURFACE_SHAPE_REPRESENTATION`, keeping its
...
/// `step-ap214-cc3` catalog in `../../🧪️oracle/🔣️component.json` must match verbatim.
...
impl StepCc3Mutation {
    ...
    pub fn kind(&self) -> &'static str {
        match self {
            StepCc3Mutation::NoMutation => "no-mutation",
            StepCc3Mutation::SetSnapshot { .. } => "set-snapshot",
            StepCc3Mutation::SetFileSchema { .. } => "set-file-schema",
            StepCc3Mutation::SetProductIdentity { .. } => "set-product-identity",
            StepCc3Mutation::SetShapeRepresentation { .. } => "set-shape-representation",
            StepCc3Mutation::DemoteShapeRepresentation { .. } => "demote-shape-representation",
        }
    }
    ...
    fn class_edit(&self) -> Option<ClassEdit> {
        match self {
            StepCc3Mutation::NoMutation | StepCc3Mutation::SetSnapshot { .. } => None,
            StepCc3Mutation::SetFileSchema { schemas } => Some(ClassEdit::FileSchema { schemas: schemas.clone() }),
            StepCc3Mutation::SetProductIdentity { identity } => Some(ClassEdit::ProductIdentity { identity: identity.clone() }),
            StepCc3Mutation::SetShapeRepresentation { id, representation } => Some(ClassEdit::Representation { id: *id, row: representation.clone() }),
            StepCc3Mutation::DemoteShapeRepresentation { id } => Some(ClassEdit::Demotion { id: *id }),
        }
    }
}
...
impl Mutation<StepSnapshot> for StepCc3Mutation {
    type Diff = StepDiff;

    fn diff(&self, base: &StepSnapshot) -> protocol::MutationOutcome<Self::Diff> {
        match self.class_edit() {
            None => match self {
                StepCc3Mutation::SetSnapshot { snapshot } => protocol::MutationOutcome::new(<StepDiff as DiffAlgebra<StepSnapshot>>::between(base, snapshot)),
    ...
    fn inverse(&self, base: &StepSnapshot) -> Vec<Self> {
        let restore = || vec![StepCc3Mutation::SetSnapshot { snapshot: base.clone() }];
        match self.class_edit() {
            None => match self {
                StepCc3Mutation::SetSnapshot { .. } => restore(),
                _ => vec![StepCc3Mutation::NoMutation],
            },
            Some(edit) => match ladder::invert_class_edit(&base.to_part21_document(), MAX_RUNG, &edit) {
                Some(ClassEdit::FileSchema { schemas }) => vec![StepCc3Mutation::SetFileSchema { schemas }],
                Some(ClassEdit::ProductIdentity { identity }) => vec![StepCc3Mutation::SetProductIdentity { identity }],
                Some(ClassEdit::Representation { id, row }) => vec![StepCc3Mutation::SetShapeRepresentation { id, representation: row }],
                _ => restore(),
            },
        }
    }
...
mod tests {
    ...
    fn every_conformance_axis_round_trips_through_its_own_inverse() {
        round_trip(base(), StepCc3Mutation::SetFileSchema { schemas: vec!["CONFIG_CONTROL_DESIGN".into()] });
        round_trip(base(), StepCc3Mutation::SetProductIdentity { identity: None });
        round_trip(base(), StepCc3Mutation::SetShapeRepresentation { id: 13, representation: None });
        round_trip(conforming(), StepCc3Mutation::SetShapeRepresentation { id: 13, representation: None });
        round_trip(base(), StepCc3Mutation::DemoteShapeRepresentation { id: 13 });
    }
    ...
    fn the_class_ceiling_is_the_line_this_vocabulary_draws() {
        ...
        assert!(apply_step_cc3_mutation(&mut snapshot, &StepCc3Mutation::SetShapeRepresentation { id: 13, representation: Some(at_ceiling) }).messages().is_empty(), "this class admits its own ceiling type");
        ...
        let outcome = apply_step_cc3_mutation(&mut snapshot, &StepCc3Mutation::SetShapeRepresentation { id: 13, representation: Some(above) });
    ...
    fn each_verb_moves_the_diagnostic_it_was_derived_from() {
        ...
        apply_step_cc3_mutation(&mut snapshot, &StepCc3Mutation::DemoteShapeRepresentation { id: 13 });
        ...
        apply_step_cc3_mutation(&mut snapshot, &StepCc3Mutation::SetProductIdentity { identity: None });
        assert!(!has_product_definition_chain(&snapshot.to_part21_document()));
        apply_step_cc3_mutation(&mut snapshot, &StepCc3Mutation::SetFileSchema { schemas: vec!["IFC4".into()] });
    ...
    fn a_rejected_mutation_leaves_the_snapshot_untouched() {
        let mut snapshot = base();
        assert!(!apply_step_cc3_mutation(&mut snapshot, &StepCc3Mutation::SetShapeRepresentation { id: 827, representation: None }).messages().is_empty(), "a conformance repair must never delete a product record");
        assert!(!apply_step_cc3_mutation(&mut snapshot, &StepCc3Mutation::SetFileSchema { schemas: vec![] }).messages().is_empty());
    ...
    fn no_mutation_is_the_identity() {
        let mut snapshot = base();
        assert!(apply_step_cc3_mutation(&mut snapshot, &StepCc3Mutation::NoMutation).messages().is_empty());
        assert_eq!(snapshot, base());
    }
    ...
    fn kinds_const_matches_enum_variants_in_declaration_order() {
        let one_per_variant = vec![
            StepCc3Mutation::NoMutation,
            StepCc3Mutation::SetSnapshot { snapshot: StepSnapshot::default() },
            StepCc3Mutation::SetFileSchema { schemas: Vec::new() },
            StepCc3Mutation::SetProductIdentity { identity: None },
            StepCc3Mutation::SetShapeRepresentation { id: 0, representation: None },
            StepCc3Mutation::DemoteShapeRepresentation { id: 0 },
        ];
