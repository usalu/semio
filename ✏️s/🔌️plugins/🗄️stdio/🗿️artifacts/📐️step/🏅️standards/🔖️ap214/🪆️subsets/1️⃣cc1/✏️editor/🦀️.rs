...
impl protocol::OpBinary for StepCc1EditCommand {
    const TOOL_JOB_IDS: &'static [&'static str] = STEP_CC1_DOCUMENT_SCHEMA_EXAMPLE_TOOL_IDS;

    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(pack::to_json_string(self).into_bytes())
    }
    fn decode_op(bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        let parsed = pack::parse_json_bytes(bytes).map_err(|error| protocol::ProtocolError::Malformed { what: "StepCc1EditCommand", offset: 0, detail: error.to_string() })?;
        <Self as dsl::FromValue>::from_value(pack::json_to_dsl_value(&parsed)).map_err(|error| protocol::ProtocolError::Malformed { what: "StepCc1EditCommand", offset: 0, detail: error.to_string() })
    }
}
//#endregion 🔖️Command
...
fn stepCc1Editor_command_from_action(action: &str, args: Option<&dsl::DslValue>) -> Result<StepCc1EditCommand, Fault> {
    match action {
...
impl ArtifactEditor for StepCc1Editor {
    ...
    fn build_document_store_initialization_job(
        ...
        Ok(semio_framework_plugin::bounded_document_store_initialization_job(envelope, STEP_CC1_DOCUMENT_SCHEMA, operation, generation))
    }

    fn command_id(command: &Self::Command) -> &'static str { stepCc1Editor_command_id(command) }

    fn command_from_action(action: &str, args: Option<&dsl::DslValue>) -> Result<Self::Command, Fault> { stepCc1Editor_command_from_action(action, args) }
...
pub fn create_step_cc1_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(STEP_CC1_DIALECT).document(["stdio", "step"]).icon_id("box").mode_def(edit::definition()).default_mode_id(edit::STEP_CC1_EDIT_MODE_ID).window_kind_def(main::definition()).default_layout(edit::layout()).action_with(semio_s_artifact_stdio_contract::set_active_example_action())
