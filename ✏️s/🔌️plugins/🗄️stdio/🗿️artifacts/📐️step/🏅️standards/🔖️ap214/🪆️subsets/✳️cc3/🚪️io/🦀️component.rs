//! 🚪️ IO stdio.step (ap214/✳️cc3) — reuses the ✳️any subset's import/export DAG leaves (same
...
pub mod derived_composition {
    ...
    const DIALECT_SELF: Dialect = Dialect { artifact_kind: "s.stdio.step", standard: StandardId("ap214"), subset: SubsetId("cc3") };
    ...
    //#region 🔖️Composer
    pub struct StepCc3ComposerComposition;

    impl ArtifactComposition for StepCc3ComposerComposition {
        ...
        async fn compose(sources: &[ComposeSource<'_>]) -> Result<Composition<Self::Snapshot>, ComposeError> {
            ...
            if !hard.is_empty() {
                ...
                all.extend(soft);
                return Err(ComposeError { message: format!("ISO 10303-214 CC3 (wireframe with topology) conformance violated: {} hard issue(s) -- not stamping the cc3 dialect", hard.len()), diagnostics: all });
            }
    ...
    /// 🛡️ The registered `SubsetValidator` for `ap214/cc3` -- see the module doc comment for how
    /// this relates to (and honestly differs from) the composer's own pre-serialization hard gate.
    pub struct StepCc3Validator;

    impl SubsetValidator for StepCc3Validator {
        ...
        async fn validate(payload: &IoPayload) -> Vec<Diagnostic> {
            ...
            match decoded {
                Some(snapshot) => check_cc3_conformance(&snapshot).await,
                None => vec![Diagnostic {
                    code: FaultCode::new("stdio.step.cc3.validate-decode-failed"),
                    severity: Severity::Warning,
                    span: TextSpan::at(1, 1),
                    message: "StepCc3Validator: payload did not decode as a StepSnapshot -- skipped".into(),
                    expected: None,
    ...
    async fn validator_entry() -> &'static SubsetValidatorEntry {
        VALIDATOR_ENTRY.get_or_init(subset_validator_entry_of::<StepCc3Validator>)
    }
    ...
    mod tests {
        ...
        async fn composer_injects_file_schema_and_stamps_clean_document() {
            ...
            let composed = StepCc3ComposerComposition::compose(&sources).expect("a document with no illegal representation must compose to cc3");
        ...
        async fn subset_validator_recheck_flags_missing_file_schema_on_the_raw_wire_payload() {
            ...
            let bytes = clean_bytes();
            let diagnostics = StepCc3Validator::validate(&IoPayload::Binary(bytes));
            assert!(diagnostics.iter().any(|d| d.code.0 == crate::artifacts::step::standards::v_ap214::subsets::cc3::schema::CODE_FILE_SCHEMA), "got {diagnostics:?}");
        }
    }
}
