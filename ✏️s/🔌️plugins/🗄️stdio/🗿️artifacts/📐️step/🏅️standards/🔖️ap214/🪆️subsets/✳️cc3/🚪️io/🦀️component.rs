...
pub mod derived_composition {
    ...
    mod tests {
        ...
        async fn composer_injects_file_schema_and_stamps_clean_document() {
            ...
            let composed = StepCc3ComposerComposition::compose(&sources).await.expect("a document with no illegal representation must compose to cc3");
        ...
        async fn subset_validator_recheck_flags_missing_file_schema_on_the_raw_wire_payload() {
            ...
            assert!(diagnostics.await.iter().any(|d| d.code.0 == crate::artifacts::step::standards::v_ap214::subsets::cc3::schema::CODE_FILE_SCHEMA), "got {diagnostics:?}");
        }
    }
}
