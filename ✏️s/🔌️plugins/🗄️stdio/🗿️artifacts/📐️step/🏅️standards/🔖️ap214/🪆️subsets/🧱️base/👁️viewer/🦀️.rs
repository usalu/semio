//! 👁️ Step Any viewer — the read-only counterpart of `✏️editor` for this subset (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `StepAnyViewer`
//! implements `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<StepAnyViewer>`
...
use crate::viewer::step_any::modes::view;
use crate::viewer::step_any::modes::view::windows::main;
...
pub enum StepAnyViewCommand {
    #[default]
    Noop,
}

impl protocol::OpBinary for StepAnyViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(StepAnyViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
pub struct StepAnyViewer;

impl ArtifactViewer for StepAnyViewer {
    ...
    type Command = StepAnyViewCommand;
...
pub fn create_step_any_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(STEP_ANY_DIALECT).document(["stdio", "step"]).icon_id("box").mode_def(view::definition()).default_mode_id(view::STEP_ANY_VIEW_MODE_ID).window_kind_def(main::definition()).default_layout(view::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn create_viewer_builds_a_definition_for_the_viewer_role() {
        let def = create_step_any_viewer();
        assert_eq!(def.role, semio_framework_plugin::AppRole::Viewer);
        assert_eq!(def.dialect, STEP_ANY_DIALECT.into());
    }

    #[semio_framework_async_macros::async_test]
    async fn viewer_dialect_matches_the_artifact_coordinate() {
        assert_eq!(<StepAnyViewer as ArtifactViewer>::DIALECT, STEP_ANY_DIALECT);
    }

    #[semio_framework_async_macros::async_test]
    async fn viewer_never_mutates_the_document_or_draft_store() {
        semio_framework_plugin::testkit::assert_viewer_never_mutates::<StepAnyViewer>().await;
    }
}
//#endregion 🧪️Tests
