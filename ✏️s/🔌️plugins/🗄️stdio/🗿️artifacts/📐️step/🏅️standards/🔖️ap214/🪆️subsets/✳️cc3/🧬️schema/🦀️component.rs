...
pub mod derived_construction {
    ...
    mod tests {
        ...
        async fn conforming_construction_builds() {
            let snapshot = StepCc3BuilderConstruction::from_snapshot(conforming_snapshot()).await.build().await.expect("conforming construction must build");
        ...
        async fn hard_violation_injected_via_raw_mutate_still_fails_build() {
            ...
            let (mutated, _diff) = StepCc3BuilderConstruction::from_snapshot(StepSnapshot::default()).await.mutate(StepMutation::SetSnapshot { snapshot }).await;
            let err = mutated.build().await.expect_err("an ADVANCED_BREP_SHAPE_REPRESENTATION instance above rung 3 must fail build()");
