...
pub mod derived_construction {
    ...
    mod tests {
        ...
        fn hard_violation_injected_via_raw_mutate_still_fails_build() {
            ...
            let (mutated, _diff) = StepCc3BuilderConstruction::from_snapshot(StepSnapshot::default()).mutate(StepMutation::SetSnapshot { snapshot });
...
pub mod derived_analysis {
    ...
    pub fn check_cc3_conformance(snapshot: &StepSnapshot) -> Vec<Diagnostic> {
        ...
        for (id, type_name, rung) in ladder_violations(&doc, MAX_RUNG) {
            out.push(hard(
                CODE_LADDER,
                format!("instance #{id} is a {type_name} (ladder rung {rung}) -- exceeds cc3's max rung 3"),
            ));
        }
    ...
    //#region 🔖️Analyzer
    /// 🧐️ Analyzes `stdio.step` (ap214/✳️cc3): delegates the real parse to the ✳️any subset's
...
semio_framework_plugin::derive_artifact_facets!(
    pub spec StepCc3BuilderFacets {
        construction: derived_construction::StepCc3BuilderConstruction,
        analysis: derived_analysis::StepCc3AnalyzerAnalysis,
