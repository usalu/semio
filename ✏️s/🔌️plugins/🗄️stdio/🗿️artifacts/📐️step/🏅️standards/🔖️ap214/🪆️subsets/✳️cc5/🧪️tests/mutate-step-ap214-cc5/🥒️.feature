@capability-step-ap214-cc5-mutate
@oracle-ruststep-step-ap214-cc5-mutate
@comparison-semantic-step-conformance-class-v1
@mutations-step-ap214-cc5
Feature: Apply every typed ISO 10303-214 CC5 (faceted B-Rep) mutation to a real AP214 exchange structure
  The input is `shared://🧪️hexagonal-cut-concrete-forest-left-ap214/📐️.stp`, the same real committed
  export the `✳️base` case reads: a real Rhino 8.31 / ST-Developer v19.2 BIM export whose 1,396-entity
  ...
  it is a FILTER, and the 6 kinds here are one per axis `check_cc5_conformance` actually reads:
  ...
  🪜️ What this class is, and why it is the sharpest of the six against THIS file. CC5 admits solids,
  but only FACETED ones — planar faces bounded by polylines. Exactly one of the six classified types
  is out of reach, `ADVANCED_BREP_SHAPE_REPRESENTATION`, where curved faces and B-spline surfaces
  enter — and that is precisely what the real committed export carries at `#13`. The gap between this
  class and the real file is a single rung, and it is still a gap: a class does not admit the type
  directly above it. The `demote-shape-representation` row is a one-rung step and it is the entire
  repair this file needs.
  ...
  🧬️ Why this catalog is identical to `mutate-step-ap214-cc2`, `mutate-step-ap214-cc3`, `mutate-step-ap214-cc4`'s, and
  ...
  Scenario Outline: Apply <id> and read the conformance projection back
    ...
    Examples:
      ...
      | set-shape-representation    | {"id": 836, "representation": {"typeName": "FACETED_BREP_SHAPE_REPRESENTATION", "name": "Document", "items": [837, 895], "context": 835}}                                     |
