//! 🚪️ IO stdio.step (ap214/✳️cc5) — reuses the ✳️any subset's import/export DAG leaves (same
...
pub mod derived_composition {
    ...
    mod tests {
        ...
        async fn composer_injects_file_schema_and_stamps_clean_document() {
            ...
            let composed = StepCc5ComposerComposition::compose(&sources).await.expect("a document with no illegal representation must compose to cc5");
        ...
        async fn subset_validator_recheck_flags_missing_file_schema_on_the_raw_wire_payload() {
            ...
            let bytes = clean_bytes();
            let diagnostics = StepCc5Validator::validate(&IoPayload::Binary(bytes));
            assert!(diagnostics.await.iter().any(|d| d.code.0 == crate::artifacts::step::standards::v_ap214::subsets::cc5::schema::CODE_FILE_SCHEMA), "got {diagnostics:?}");
        }
    }
}
