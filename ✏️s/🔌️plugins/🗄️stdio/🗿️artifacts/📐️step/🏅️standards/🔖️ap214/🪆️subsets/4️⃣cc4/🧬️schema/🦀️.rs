//! 🧬️ StepSnapshot schema (ap214/4️⃣cc4) — reuses the 🧱️base subset's `StepSnapshot` verbatim
//! (the SAME Rust type, same `stdio.step` schema id). ISO 10303-214 CC4 (manifold surfaces with topology) is a validation-gated
...
//! `🪆️subsets/4️⃣cc4/🧬️schema/` is present per `🔣️taxonomy.json`'s `subsetChildDirs`, without
//! duplicating the schema definition.

pub use crate::artifacts::step::standards::v_ap214::subsets::base::schema::*;

//#region 🧬️Mutations
/// 🧬️ This subset's OWN mutation vocabulary — one kind per ISO 10303-214 CC4 (manifold surfaces with topology) conformance
/// rule, derived from `check_cc4_conformance` below rather than copied from a sibling class, and
...
pub mod derived_construction {
    ...
    mod tests {
        ...
        use crate::artifacts::step::standards::v_ap214::subsets::cc4::schema::CODE_LADDER;
        ...
        async fn hard_violation_injected_via_raw_mutate_still_fails_build() {
            ...
            let (mutated, _diff) = StepCc4BuilderConstruction::from_snapshot(StepSnapshot::default()).mutate(StepMutation::SetSnapshot(crate::artifacts::step::standards::v_ap214::subsets::base::schema::mutations::set_snapshot::SetSnapshot { snapshot }));
...
pub mod derived_analysis {
    ...
    pub const CODE_PRODUCT_CHAIN: &str = "stdio.step.cc4.product-definition-chain";
    pub const CODE_LADDER: &str = "stdio.step.cc4.representation-above-rung";
    ...
    /// 🛡️ Real ISO 10303-214 CC4 (manifold surfaces with topology) conformance checks against one already-decoded `StepSnapshot`. Shared
    /// single source of truth: `StepCc4Composer::compose` hard-gates on this (pre-serialization,
    /// authoritative), `StepCc4Builder::build` hard-gates on this too, and the registered
    /// `SubsetValidator` (from `🎹️composer::register`) re-runs it post-hoc against the wire payload
    /// for the D5 validate-on-build hook.
    // 🚫️async: E1 pure inherent-impl helper (file verified I/O-free, consumed via opaque-type-hostile call site) — see R9
    pub fn check_cc4_conformance(snapshot: &StepSnapshot) -> Vec<Diagnostic> {
        ...
        for (id, type_name, rung) in ladder_violations(&doc, MAX_RUNG) {
            out.push(hard(CODE_LADDER, format!("instance #{id} is a {type_name} (ladder rung {rung}) -- exceeds cc4's max rung 4")));
        }
    ...
    //#region 🔖️Analyzer
    /// 🧐️ Analyzes `stdio.step` (ap214/4️⃣cc4): delegates the real parse to the 🧱️base subset's
    /// analyzer (same `StepSnapshot`), then folds real ISO 10303-214 CC4 (manifold surfaces with topology) conformance diagnostics on top.
    ...
    /// answered by `analyze`/`check_cc4_conformance`, not by `sniff`.
    pub struct StepCc4AnalyzerAnalysis;

    impl ArtifactAnalysis for StepCc4AnalyzerAnalysis {
    ...
    mod tests {
        ...
        async fn representation_at_max_rung_is_clean() {
            let mut doc = base_doc();
            doc.instances.push(Part21Instance { id: 4, entities: vec![("MANIFOLD_SURFACE_SHAPE_REPRESENTATION".into(), vec![])] });
            let snapshot = StepSnapshot::from_part21_document(doc);
            let diagnostics = check_cc4_conformance(&snapshot);
...
semio_framework_plugin::derive_artifact_facets!(
    ...
    analyzer: StepCc4Analyzer,
    composer: StepCc4Composer,
);
