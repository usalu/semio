...
pub mod derived_composition {
    ...
    //#region 🔖️Composer
    pub struct StepCc2ComposerComposition;

    impl ArtifactComposition for StepCc2ComposerComposition {
        ...
        async fn compose(sources: &[ComposeSource<'_>]) -> Result<Composition<Self::Snapshot>, ComposeError> {
            ...
            if !hard.is_empty() {
                ...
                all.extend(soft);
                return Err(ComposeError { message: format!("ISO 10303-214 CC2 (bounded wireframe/basic surfaces) conformance violated: {} hard issue(s) -- not stamping the cc2 dialect", hard.len()), diagnostics: all });
            }
    ...
    mod tests {
        ...
        async fn composer_injects_file_schema_and_stamps_clean_document() {
            ...
            let composed = StepCc2ComposerComposition::compose(&sources).await.expect("a document with no illegal representation must compose to cc2");
        ...
        async fn subset_validator_recheck_flags_missing_file_schema_on_the_raw_wire_payload() {
            ...
            assert!(diagnostics.await.iter().any(|d| d.code.0 == crate::artifacts::step::standards::v_ap214::subsets::cc2::schema::CODE_FILE_SCHEMA), "got {diagnostics:?}");
        }
    }
}
