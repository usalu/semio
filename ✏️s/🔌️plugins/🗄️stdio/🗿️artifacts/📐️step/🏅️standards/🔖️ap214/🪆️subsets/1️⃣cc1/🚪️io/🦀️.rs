//! 🚪️ IO stdio.step (ap214/1️⃣cc1) — reuses the 🧱️base subset's import/export DAG leaves (same
...
pub mod derived_composition {
    ...
    //#region 🔖️Composer
    pub struct StepCc1ComposerComposition;

    impl ArtifactComposition for StepCc1ComposerComposition {
        ...
        fn compose(sources: &[ComposeSource<'_>]) -> Result<Composition<Self::Snapshot>, ComposeError> {
            ...
            if !hard.is_empty() {
                ...
                all.extend(soft);
                return Err(ComposeError { message: format!("ISO 10303-214 CC1 (config data only) conformance violated: {} hard issue(s) -- not stamping the cc1 dialect", hard.len()), diagnostics: all });
            }
    ...
    mod tests {
        ...
        async fn subset_validator_recheck_flags_missing_file_schema_on_the_raw_wire_payload() {
            ...
            let bytes = clean_bytes();
            let diagnostics = StepCc1Validator::validate(&IoPayload::Binary(bytes)).await;
            assert!(diagnostics.iter().any(|d| d.code.0 == crate::standards::v_ap214::subsets::cc1::schema::CODE_FILE_SCHEMA), "got {diagnostics:?}");
        }
    }
}
