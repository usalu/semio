//! 🧊 Step Any editor — the Main window: the SAME world-3d mesh scene shape as the sibling
...
const STEP_ANY_EDIT_FALLBACK_MESH_KIND: &str = "box";
const STEP_ANY_EDIT_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const STEP_ANY_EDIT_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const STEP_ANY_EDIT_DEFAULT_CAMERA_FOV: f64 = 45.0;
    ...
    let instances: Vec<serde_json::Value> = (0..count.await)
        .map(|index| {
            serde_json::json!({
                ...
                "meshId": STEP_ANY_EDIT_FALLBACK_MESH_KIND,
...
pub async fn render(document: &StepSnapshot) -> UiNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": STEP_ANY_EDIT_FALLBACK_MESH_KIND, "data": mesh_from_kind(STEP_ANY_EDIT_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(STEP_ANY_EDIT_DEFAULT_CAMERA_POSITION, STEP_ANY_EDIT_DEFAULT_CAMERA_TARGET, STEP_ANY_EDIT_DEFAULT_CAMERA_FOV).await,
        meshes_json,
