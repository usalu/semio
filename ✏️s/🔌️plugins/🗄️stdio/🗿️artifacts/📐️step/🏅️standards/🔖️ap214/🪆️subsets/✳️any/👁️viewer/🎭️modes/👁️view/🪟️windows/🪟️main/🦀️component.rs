//! 🧊 Step Any viewer — the Main window: a read-only world-3d mesh scene built with the shared
...
const STEP_ANY_VIEW_FALLBACK_MESH_KIND: &str = "box";
const STEP_ANY_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const STEP_ANY_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const STEP_ANY_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
...
fn world_instances_json(document: &StepSnapshot) -> String {
    ...
    let instances: Vec<serde_json::Value> = (0..count)
        .map(|index| {
            serde_json::json!({
                "id": format!("step_any-{index}"),
                "meshId": STEP_ANY_VIEW_FALLBACK_MESH_KIND,
                "position": [index as f64 * 2.0, 0.0, 0.0],
                "rotation": [0.0, 0.0, 0.0, 1.0],
                "scale": [1.0, 1.0, 1.0],
                "label": format!("Step Any {index}"),
                "smoothShading": false,
            })
        })
        .collect();
...
/// 👁️ Pure `StepSnapshot -> UiNode` read: default camera (a viewer has no persisted
/// per-session camera — `Config = NoConfig`), no selection/gumball/engagement overlay.
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn render(document: &StepSnapshot) -> UiNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": STEP_ANY_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(STEP_ANY_VIEW_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(STEP_ANY_VIEW_DEFAULT_CAMERA_POSITION, STEP_ANY_VIEW_DEFAULT_CAMERA_TARGET, STEP_ANY_VIEW_DEFAULT_CAMERA_FOV),
        meshes_json,
