//! ✏️ Step CC5 editor — thin, kit-based editor surface (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.1). `StepCc5Editor`
...
use crate::editor::step_cc5::modes::edit;
...
pub enum StepCc5EditCommand {
    #[default]
    SetVertex,
}

impl protocol::OpBinary for StepCc5EditCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(StepCc5EditCommand::SetVertex)
    }
}
//#endregion 🔖️Command
...
pub struct StepCc5Editor;

impl ArtifactEditor for StepCc5Editor {
    ...
    type Command = StepCc5EditCommand;
...
pub fn create_step_cc5_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(STEP_CC5_DIALECT).document(["stdio", "step"]).icon_id("box").mode_def(edit::definition()).default_mode_id(edit::STEP_CC5_EDIT_MODE_ID).window_kind_def(main::definition()).default_layout(edit::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn editor_and_viewer_share_one_dialect() {
        semio_framework_plugin::testkit::assert_editor_and_viewer_share_dialect::<StepCc5Editor, crate::viewer::step_cc5::StepCc5Viewer>();
    }
}
//#endregion 🧪️Tests
