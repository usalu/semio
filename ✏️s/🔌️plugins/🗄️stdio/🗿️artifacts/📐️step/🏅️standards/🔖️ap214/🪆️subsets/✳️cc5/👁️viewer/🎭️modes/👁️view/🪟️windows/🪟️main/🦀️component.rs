//! 🧊 Step CC5 viewer — the Main window: a read-only world-3d mesh scene built with the shared
...
const STEP_CC5_VIEW_FALLBACK_MESH_KIND: &str = "box";
const STEP_CC5_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const STEP_CC5_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const STEP_CC5_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
    ...
    let instances: Vec<serde_json::Value> = (0..count.await)
        .map(|index| {
            serde_json::json!({
                ...
                "meshId": STEP_CC5_VIEW_FALLBACK_MESH_KIND,
...
pub async fn render(document: &StepSnapshot) -> UiNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": STEP_CC5_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(STEP_CC5_VIEW_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(STEP_CC5_VIEW_DEFAULT_CAMERA_POSITION, STEP_CC5_VIEW_DEFAULT_CAMERA_TARGET, STEP_CC5_VIEW_DEFAULT_CAMERA_FOV).await,
        meshes_json,
