...
//! committed file is never written to. `oracle` handlers drive the registered `serde_json`
...
//! the SAME independent reader (`project_json_value`, `serde_json` underneath) before the
...
/// 🔁️ The oracle's own decode/re-encode, through the SAME independent `serde_json` reader/writer
/// this subset's mutations use — proves the reference library itself is stable on the real fixture
/// before the subject's own codec is asked to be.
...
mod subject {
    ...
    /// not link the oracle role's `serde_json`-shaped `PathSeg`.
