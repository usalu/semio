...
pub mod derived_composition {
    ...
    impl SubsetValidator for JsonIJsonValidator {
        ...
        async fn validate(payload: &IoPayload) -> Vec<Diagnostic> {
            let decoded = match payload {
                IoPayload::Binary(bytes) => <JsonSnapshot as store::ArtifactPack>::decode_pack(bytes).await.ok(),
                IoPayload::Text(text) => <JsonSnapshot as store::ArtifactDsl>::parse_dsl(text).await.ok(),
            };
            match decoded {
    ...
    mod tests {
        ...
        async fn subset_validator_recheck_flags_only_soft_diagnostics_for_a_clean_document() {
            ...
            let snapshot = <JsonSnapshot as store::ArtifactDsl>::parse_dsl(&text).await.expect("parses");
            ...
            assert!(diagnostics.await.iter().all(|d| d.severity != Severity::Error), "wire recheck must never report a hard violation for a duplicate/overflow-free document: {diagnostics:?}");
            assert!(diagnostics.await.iter().any(|d| d.code.0 == "stdio.json.i-json.top-level-scalar"), "got {diagnostics:?}");
        }
    }
}
