...
impl Mutation<JsonSnapshot> for JsonIJsonMutation {
    type Diff = JsonDiff;
    ...
    fn inverse(&self, base: &JsonSnapshot) -> Vec<Self> {
        match self {
            JsonIJsonMutation::NoMutation => vec![JsonIJsonMutation::NoMutation],
            JsonIJsonMutation::SetSnapshot { .. } => vec![JsonIJsonMutation::SetSnapshot { snapshot: base.clone() }],
            JsonIJsonMutation::SetTopLevel { .. } => match JsonIJsonRoot::from_value(&base.value) {
                Some(root) => vec![JsonIJsonMutation::SetTopLevel { root }],
                None => vec![JsonIJsonMutation::SetSnapshot { snapshot: base.clone() }],
            },
            JsonIJsonMutation::UpsertMember { path, key, .. } => match resolve(&base.value, path) {
                Some(JsonValue::Object { members }) => match members.iter().find(|member| &member.key == key) {
                    Some(existing) => vec![JsonIJsonMutation::UpsertMember { path: path.clone(), key: key.clone(), value: existing.value.clone() }],
                    None => vec![JsonIJsonMutation::RemoveMember { path: path.clone(), key: key.clone() }],
                },
                _ => vec![JsonIJsonMutation::NoMutation],
            },
            JsonIJsonMutation::RemoveMember { path, key } => match resolve(&base.value, path) {
                Some(JsonValue::Object { members }) => match members.iter().find(|member| &member.key == key) {
                    Some(existing) => vec![JsonIJsonMutation::UpsertMember { path: path.clone(), key: key.clone(), value: existing.value.clone() }],
                    None => vec![JsonIJsonMutation::NoMutation],
                },
                _ => vec![JsonIJsonMutation::NoMutation],
            },
            JsonIJsonMutation::RenameMember { path, from, to } => vec![JsonIJsonMutation::RenameMember { path: path.clone(), from: to.clone(), to: from.clone() }],
            JsonIJsonMutation::SetSafeNumber { path, .. } => match resolve(&base.value, path) {
                Some(JsonValue::Number { lexeme }) => vec![JsonIJsonMutation::SetSafeNumber { path: path.clone(), lexeme: lexeme.clone() }],
                _ => vec![JsonIJsonMutation::NoMutation],
            },
            JsonIJsonMutation::SetString { path, .. } => match resolve(&base.value, path) {
                Some(JsonValue::String { value }) => vec![JsonIJsonMutation::SetString { path: path.clone(), value: value.clone() }],
                _ => vec![JsonIJsonMutation::NoMutation],
            },
            JsonIJsonMutation::InsertArrayElement { path, index, .. } => match resolve(&base.value, path) {
                Some(JsonValue::Array { items }) => vec![JsonIJsonMutation::RemoveArrayElement { path: path.clone(), index: (*index).min(items.len()) }],
                _ => vec![JsonIJsonMutation::NoMutation],
            },
            JsonIJsonMutation::RemoveArrayElement { path, index } => match resolve(&base.value, path) {
                Some(JsonValue::Array { items }) => match items.get(*index) {
                    Some(item) => vec![JsonIJsonMutation::InsertArrayElement { path: path.clone(), index: *index, value: item.clone() }],
                    None => vec![JsonIJsonMutation::NoMutation],
                },
                _ => vec![JsonIJsonMutation::NoMutation],
            },
...
mod tests {
    ...
    /// declaration order, and the same list the `json-rfc8259-i-json` catalog declares. The framework
    ...
    fn kinds_match_the_enum_and_the_catalog() {
        ...
        let sample = vec![
            JsonIJsonMutation::NoMutation,
            JsonIJsonMutation::SetSnapshot { snapshot: JsonSnapshot::default() },
            JsonIJsonMutation::SetTopLevel { root: JsonIJsonRoot::Array { items: Vec::new() } },
            JsonIJsonMutation::UpsertMember { path: Vec::new(), key: String::new(), value: JsonValue::Null },
            JsonIJsonMutation::RemoveMember { path: Vec::new(), key: String::new() },
            JsonIJsonMutation::RenameMember { path: Vec::new(), from: String::new(), to: String::new() },
            JsonIJsonMutation::SetSafeNumber { path: Vec::new(), lexeme: "0".to_string() },
            JsonIJsonMutation::SetString { path: Vec::new(), value: String::new() },
            JsonIJsonMutation::InsertArrayElement { path: Vec::new(), index: 0, value: JsonValue::Null },
            JsonIJsonMutation::RemoveArrayElement { path: Vec::new(), index: 0 },
        ];
    ...
    fn set_safe_number_at_the_boundary_is_accepted() {
        ...
        let outcome = apply_json_i_json_mutation(&mut snapshot, &JsonIJsonMutation::SetSafeNumber { path: key("revision"), lexeme: "9007199254740991".to_string() });
    ...
    fn set_safe_number_one_past_the_boundary_is_refused_and_never_applied() {
        let mut snapshot = base();
        let outcome = apply_json_i_json_mutation(&mut snapshot, &JsonIJsonMutation::SetSafeNumber { path: key("revision"), lexeme: "9007199254740992".to_string() });
    ...
    fn set_string_refuses_a_unicode_noncharacter() {
        let mut snapshot = base();
        let outcome = apply_json_i_json_mutation(&mut snapshot, &JsonIJsonMutation::SetString { path: key("title"), value: "before\u{FFFE}after".to_string() });
        assert!(outcome.messages().iter().any(|message| message.code.0 == CODE_INVARIANT), "got {:?}", outcome.messages());
    ...
    fn rename_member_is_atomic_and_position_preserving() {
        let mut snapshot = base();
        apply_json_i_json_mutation(&mut snapshot, &JsonIJsonMutation::RenameMember { path: Vec::new(), from: "revision".to_string(), to: "version".to_string() });
    ...
    fn rename_member_onto_an_existing_name_is_refused() {
        let mut snapshot = base();
        let outcome = apply_json_i_json_mutation(&mut snapshot, &JsonIJsonMutation::RenameMember { path: Vec::new(), from: "revision".to_string(), to: "title".to_string() });
        assert!(outcome.messages().iter().any(|message| message.code.0 == CODE_INVARIANT), "got {:?}", outcome.messages());
        let JsonValue::Object { members } = &snapshot.value else { panic!("root stays an object") };
    ...
    /// on, proved here for all ten kinds against one snapshot rather than only end-to-end.
    #[test]
    fn applying_a_mutation_and_then_its_inverse_restores_the_snapshot() {
        let original = base();
        let mutations = vec![
            JsonIJsonMutation::NoMutation,
            JsonIJsonMutation::SetSnapshot { snapshot: JsonSnapshot { value: JsonValue::Array { items: vec![number("1")] }, ..JsonSnapshot::default() } },
            JsonIJsonMutation::SetTopLevel { root: JsonIJsonRoot::Array { items: vec![JsonValue::Null] } },
            JsonIJsonMutation::UpsertMember { path: Vec::new(), key: "revision".to_string(), value: number("9") },
            JsonIJsonMutation::UpsertMember { path: Vec::new(), key: "fresh".to_string(), value: JsonValue::Null },
            JsonIJsonMutation::RemoveMember { path: Vec::new(), key: "title".to_string() },
            JsonIJsonMutation::RenameMember { path: Vec::new(), from: "title".to_string(), to: "heading".to_string() },
            JsonIJsonMutation::SetSafeNumber { path: key("revision"), lexeme: "9007199254740991".to_string() },
            JsonIJsonMutation::SetString { path: key("title"), value: "Ünïcödé, mit Sonderzeichen".to_string() },
            JsonIJsonMutation::InsertArrayElement { path: key("tags"), index: 1, value: JsonValue::String { value: "inserted".to_string() } },
            JsonIJsonMutation::RemoveArrayElement { path: key("tags"), index: 0 },
        ];
