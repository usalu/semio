...
//!
//! Every fixture is a `(before.json, after.json)` PAIR for exactly one mutation kind. Both halves are
...
fn corpus() -> Vec<(&'static str, Value, Value)> {
    ...
    vec![
        ("set-member", base(), set_member),
        ("remove-member", base(), remove_member),
        ("insert-array-element", base(), insert_array_element),
        ("remove-array-element", base(), remove_array_element),
        ("set-scalar", base(), set_scalar),
    ]
}
