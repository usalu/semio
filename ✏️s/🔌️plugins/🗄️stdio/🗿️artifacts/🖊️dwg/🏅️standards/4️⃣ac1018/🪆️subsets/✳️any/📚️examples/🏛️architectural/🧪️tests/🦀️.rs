...
async fn persisted_dwg_facets_have_no_parallel_entity_projection() {
    ...
    {
        let facet = std::fs::read_to_string(artifact_root.join("🔗️.graphql")).unwrap_or_else(|error| panic!("missing schema facet: {error}"));
        assert!(!facet.contains("DwgLogicalEntity"), "a schema facet retains DwgLogicalEntity");
        assert!(!facet.contains(" entities:"), "a schema facet retains the parallel entities field");
        assert!(!facet.contains("\"entities\""), "a schema facet retains the parallel entities property");
    }
    {
        let facet = std::fs::read_to_string(artifact_root.join("🛰️.proto")).unwrap_or_else(|error| panic!("missing schema facet: {error}"));
        assert!(!facet.contains("DwgLogicalEntity"), "a schema facet retains DwgLogicalEntity");
        assert!(!facet.contains(" entities:"), "a schema facet retains the parallel entities field");
        assert!(!facet.contains("\"entities\""), "a schema facet retains the parallel entities property");
    }
    {
        let facet = std::fs::read_to_string(artifact_root.join("🔣️.json")).unwrap_or_else(|error| panic!("missing schema facet: {error}"));
        assert!(!facet.contains("DwgLogicalEntity"), "a schema facet retains DwgLogicalEntity");
        assert!(!facet.contains(" entities:"), "a schema facet retains the parallel entities field");
        assert!(!facet.contains("\"entities\""), "a schema facet retains the parallel entities property");
    }
    {
        let facet = std::fs::read_to_string(artifact_root.join("📸️snapshot/🔗️.graphql")).unwrap_or_else(|error| panic!("missing schema facet: {error}"));
        assert!(!facet.contains("DwgLogicalEntity"), "a schema facet retains DwgLogicalEntity");
        assert!(!facet.contains(" entities:"), "a schema facet retains the parallel entities field");
        assert!(!facet.contains("\"entities\""), "a schema facet retains the parallel entities property");
    }
    {
        let facet = std::fs::read_to_string(artifact_root.join("📸️snapshot/🛰️.proto")).unwrap_or_else(|error| panic!("missing schema facet: {error}"));
        assert!(!facet.contains("DwgLogicalEntity"), "a schema facet retains DwgLogicalEntity");
        assert!(!facet.contains(" entities:"), "a schema facet retains the parallel entities field");
        assert!(!facet.contains("\"entities\""), "a schema facet retains the parallel entities property");
    }
    {
        let facet = std::fs::read_to_string(artifact_root.join("🔺️diff/🟦️.ts")).unwrap_or_else(|error| panic!("missing schema facet: {error}"));
        assert!(!facet.contains("DwgLogicalEntity"), "a schema facet retains DwgLogicalEntity");
        assert!(!facet.contains(" entities:"), "a schema facet retains the parallel entities field");
        assert!(!facet.contains("\"entities\""), "a schema facet retains the parallel entities property");
    }
    {
        let facet = std::fs::read_to_string(artifact_root.join("🔺️diff/🔗️.graphql")).unwrap_or_else(|error| panic!("missing schema facet: {error}"));
        assert!(!facet.contains("DwgLogicalEntity"), "a schema facet retains DwgLogicalEntity");
        assert!(!facet.contains(" entities:"), "a schema facet retains the parallel entities field");
        assert!(!facet.contains("\"entities\""), "a schema facet retains the parallel entities property");
    }
    {
        let facet = std::fs::read_to_string(artifact_root.join("🔺️diff/🛰️.proto")).unwrap_or_else(|error| panic!("missing schema facet: {error}"));
        assert!(!facet.contains("DwgLogicalEntity"), "a schema facet retains DwgLogicalEntity");
        assert!(!facet.contains(" entities:"), "a schema facet retains the parallel entities field");
        assert!(!facet.contains("\"entities\""), "a schema facet retains the parallel entities property");
    }
    let rust = std::fs::read_to_string(artifact_root.join("📸️snapshot/🦀️.rs")).expect("Rust snapshot schema");
