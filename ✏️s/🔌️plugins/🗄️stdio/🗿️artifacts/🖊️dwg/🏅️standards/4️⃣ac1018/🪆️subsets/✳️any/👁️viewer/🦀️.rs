...
pub fn create_dwg_ac1018_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(DWG_AC1018_DIALECT).document(["stdio", "dwgac1018"]).icon_id("box").mode_def(view::definition()).default_mode_id(view::DWG_AC1018_VIEW_MODE_ID).window_kind_def(main::definition()).default_layout(view::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn viewer_never_mutates_the_document_or_draft_store() {
        semio_framework_plugin::testkit::assert_viewer_never_mutates::<DwgAc1018Viewer>().await;
    }
}
//#endregion 🧪️Tests
