...
pub fn render(document: &DwgSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = pack::json_to_string(&pack::json_array([pack::json!({ "id": DWG_AC1024_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(DWG_AC1024_VIEW_FALLBACK_MESH_KIND) })]));
    let view = MeshView {
