...
impl protocol::OpBinary for DwgAc1024EditCommand {
    ...
    fn decode_op(bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        let parsed = pack::parse_json_bytes(bytes).map_err(|error| protocol::ProtocolError::Malformed { what: "DwgAc1024EditCommand", offset: 0, detail: error.to_string() })?;
        <Self as dsl::FromValue>::from_value(pack::json_to_dsl_value(&parsed)).map_err(|error| protocol::ProtocolError::Malformed { what: "DwgAc1024EditCommand", offset: 0, detail: error.to_string() })
    }
}
//#endregion 🔖️Command
...
fn dwgAc1024Editor_example_snapshot(example_id: &str) -> DwgSnapshot {
...
fn dwgAc1024Editor_retained_extent(command: &DwgAc1024EditCommand, _snapshot: &DwgSnapshot, _interaction: &protocol::InteractionState) -> Option<usize> {
    matches!(command, DwgAc1024EditCommand::SetActiveExample { .. }).then_some(1)
}

fn dwgAc1024Editor_retained_reduce(
    command: &DwgAc1024EditCommand,
    _snapshot: &DwgSnapshot,
    _config: &NoConfig,
    _history: &semio_framework_plugin::HistoryView,
    _interaction: &protocol::InteractionState,
    _hover: &semio_framework_plugin::app::InteractionHoverState,
    _context: Option<&semio_framework_plugin::app::ArtifactOwnedToolJobContext<EditorApp<DwgAc1024Editor>>>,
...
struct DwgAc1024EditorExampleFactory {
    keys: Vec<ToolFactoryKey>,
}

impl DwgAc1024EditorExampleFactory {
    fn new(controller_id: &str) -> Self {
        Self { keys: DWG_AC1024_DOCUMENT_SCHEMA_EXAMPLE_TOOL_IDS.iter().map(|tool_id| ToolFactoryKey::new(controller_id, *tool_id)).collect() }
    }
}

impl ToolJobFactory for DwgAc1024EditorExampleFactory {
    type Payload = ArtifactRetainedCommandPayload<EditorApp<DwgAc1024Editor>>;
    type Job = ArtifactRetainedCommandJob<EditorApp<DwgAc1024Editor>>;
    fn keys(&self) -> &[ToolFactoryKey] { &self.keys }
    fn payload_schema_id(&self) -> &str { DWG_AC1024_DOCUMENT_SCHEMA_EXAMPLE_SCHEMA }
    fn classification(&self) -> InteractiveJobClassification { InteractiveJobClassification::Migrated }
    fn execution_contract(&self) -> ToolExecutionContract { ToolExecutionContract::bounded_first_step(DWG_AC1024_DOCUMENT_SCHEMA_EXAMPLE_BYTES, 64, 1, 65_536, 7_500) }
    ...
    fn create_job_from_wire_pages_with_payload(&mut self, _operation: semio_framework_job::Operation, payload: Self::Payload, input: semio_framework_plugin::action_bus::RetainedToolWireInput, checkpoint: Option<semio_framework_plugin::action_bus::RetainedToolWireInput>) -> Result<Self::Job, (ToolJobFactoryError, semio_framework_plugin::action_bus::RetainedToolWireInput, Option<semio_framework_plugin::action_bus::RetainedToolWireInput>)> {
        if input.declared_bytes() > DWG_AC1024_DOCUMENT_SCHEMA_EXAMPLE_BYTES || checkpoint.is_some() {
...
impl ArtifactOwnedToolJobFactory for DwgAc1024EditorExampleFactory {
    type Owner = EditorApp<DwgAc1024Editor>;
    const TOOL_IDS: &'static [&'static str] = DWG_AC1024_DOCUMENT_SCHEMA_EXAMPLE_TOOL_IDS;
    const DOCUMENT_SCHEMA: &'static str = DWG_AC1024_DOCUMENT_SCHEMA;
    const PUBLICATION_CONTRACTS: &'static [ArtifactToolPublicationContract] = &[DWG_AC1024_DOCUMENT_SCHEMA_EXAMPLE_CONTRACT];
}
//#region 🔖️Editor
...
impl ArtifactEditor for DwgAc1024Editor {
    ...
    semio_framework_plugin::bounded_first_step_tool_proofs! {
        ...
        factory: "DwgAc1024EditorExampleFactory",
        factory_type: DwgAc1024EditorExampleFactory,
        contract: ToolExecutionContract::bounded_first_step(DWG_AC1024_DOCUMENT_SCHEMA_EXAMPLE_BYTES, 64, 1, 65_536, 7_500),
    ...
    fn build_tool_job(request: ArtifactOwnedToolJobRequest<EditorApp<Self>>) -> Result<Option<ToolOperationSpec>, Fault> {
        ...
        if dwgAc1024Editor_command_id(&request.command) != request.tool_id {
        ...
        let payload = ArtifactRetainedCommandPayload::try_new(
            ...
            dwgAc1024Editor_command_id,
            DWG_AC1024_DOCUMENT_SCHEMA_EXAMPLE_BYTES,
            1,
            Box::new(BoundedArtifactCommandWork::new(semio_s_artifact_stdio_contract::SET_ACTIVE_EXAMPLE_ACTION_ID, dwgAc1024Editor_retained_reduce, dwgAc1024Editor_retained_extent)),
        )?;
    ...
    fn build_document_store_initialization_job(
        ...
        Ok(semio_framework_plugin::bounded_document_store_initialization_job(envelope, DWG_AC1024_DOCUMENT_SCHEMA, operation, generation))
    }

    fn command_id(command: &Self::Command) -> &'static str { dwgAc1024Editor_command_id(command) }

    fn command_from_action(action: &str, args: Option<&dsl::DslValue>) -> Result<Self::Command, Fault> { dwgAc1024Editor_command_from_action(action, args) }
