//! 🧊 DWG AC1018 viewer — the Main window: a read-only world-3d mesh scene built with the shared
...
const DWG_AC1018_VIEW_FALLBACK_MESH_KIND: &str = "box";
const DWG_AC1018_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const DWG_AC1018_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const DWG_AC1018_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
    ...
    let instances: Vec<serde_json::Value> = (0..count.await)
        .map(|index| {
            serde_json::json!({
                ...
                "meshId": DWG_AC1018_VIEW_FALLBACK_MESH_KIND,
...
pub async fn render(document: &DwgSnapshot) -> UiNode {
    ...
    let view = MeshView {
        camera_json: world3d_camera_json(DWG_AC1018_VIEW_DEFAULT_CAMERA_POSITION, DWG_AC1018_VIEW_DEFAULT_CAMERA_TARGET, DWG_AC1018_VIEW_DEFAULT_CAMERA_FOV).await,
        meshes_json,
