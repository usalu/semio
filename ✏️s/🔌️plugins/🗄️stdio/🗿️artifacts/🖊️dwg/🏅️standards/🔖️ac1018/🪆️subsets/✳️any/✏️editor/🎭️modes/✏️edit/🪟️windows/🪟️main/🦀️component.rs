//! 🧊 DWG AC1018 editor — the Main window: the SAME world-3d mesh scene shape as the sibling
...
//! artifact-level `DwgSnapshot` — this window itself never mutates; mutation is the surface
...
const DWG_AC1018_EDIT_FALLBACK_MESH_KIND: &str = "box";
const DWG_AC1018_EDIT_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const DWG_AC1018_EDIT_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const DWG_AC1018_EDIT_DEFAULT_CAMERA_FOV: f64 = 45.0;
    ...
    let instances: Vec<pack::JsonValue> = (0..count)
        .map(|index| {
            pack::json!({
                ...
                "meshId": DWG_AC1018_EDIT_FALLBACK_MESH_KIND,
                "position": [index as f64 * 2.0, 0.0, 0.0],
                "rotation": [0.0, 0.0, 0.0, 1.0],
                "scale": [1.0, 1.0, 1.0],
                "label": format!("DWG AC1018 {index}"),
                "smoothShading": false,
            })
        })
        .collect();
...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn render(document: &DwgSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = pack::json_to_string(&pack::json_array([pack::json!({ "id": DWG_AC1018_EDIT_FALLBACK_MESH_KIND, "data": mesh_from_kind(DWG_AC1018_EDIT_FALLBACK_MESH_KIND) })]));
    let view = MeshView {
