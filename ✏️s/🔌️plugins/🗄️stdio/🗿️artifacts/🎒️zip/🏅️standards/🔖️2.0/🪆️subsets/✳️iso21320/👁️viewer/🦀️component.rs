//! 👁️ Zip viewer (2.0/✳️iso21320) — the read-only counterpart of `✏️editor` for this subset (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `ZipIso21320Viewer` implements
//! `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<ZipIso21320Viewer>` (framework
...
//#region 🔖️Dialect
/// 🎯️ This surface's dialect coordinate — `s.stdio.zip@2.0/iso21320`. Kept as its own independent
...
pub const ZIP_ISO21320_VIEWER_DIALECT: Dialect = Dialect { artifact_kind: "s.stdio.zip", standard: StandardId("2.0"), subset: SubsetId("iso21320") };
//#endregion 🔖️Dialect
...
pub enum ZipIso21320ViewCommand {
    #[default]
    Noop,
}

impl protocol::OpBinary for ZipIso21320ViewCommand {
    async fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    async fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(ZipIso21320ViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
pub struct ZipIso21320Viewer;

impl ArtifactViewer for ZipIso21320Viewer {
    ...
    type Command = ZipIso21320ViewCommand;

    const DIALECT: Dialect = ZIP_ISO21320_VIEWER_DIALECT;
    ...
    /// 👁️ Structurally read-only: the sole `ZipIso21320ViewCommand::Noop` variant never carries a
...
mod tests {
    ...
    async fn create_zip_iso21320_viewer_builds_a_definition_for_the_viewer_role() {
        let def = create_zip_iso21320_viewer();
        assert_eq!(def.role, semio_framework_plugin::AppRole::Viewer);
        assert_eq!(def.dialect, ZIP_ISO21320_VIEWER_DIALECT.into());
    }

    #[semio_framework_async_macros::async_test]
    async fn viewer_dialect_matches_the_artifact_coordinate() {
        assert_eq!(<ZipIso21320Viewer as ArtifactViewer>::DIALECT, ZIP_ISO21320_VIEWER_DIALECT);
    }
