//! 🎒️ Zip editor (2.0/✳️any) — the FIRST authored `ArtifactEditor` surface for `s.stdio.zip@2.0/*`
//! (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET). `ZipSnapshot` is a flat logical
//! archive: entries (`name`+`data`) plus an archive-level `comment` — no composed-child indirection
//! needed. One window, `🪟️main` (`TreeWindowKit`), renders the archive as a tree; its `set-node`
//! action funnels through the one typed command this surface declares, `ZipEditorCommand::SetNode`,
...
/// `🏅️standards/🔖️2.0/🪆️subsets/✳️any` location on disk. No reusable standalone `pub const DIALECT`
/// exists for this exact subset (the artifact root has none; the ✳️any subset's own schema module
...
pub async fn create_zip_any_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(ZIP_ANY_EDITOR_DIALECT)
        .await.document(["stdio", "zip", "any"])
        .await.icon_id("archive")
        .await.mode_def(edit::definition().await)
        .await.default_mode_id(edit::ZIP_ANY_EDIT_MODE_ID)
