//! 🧪️ `set-snapshot` fixture — `🟣️slows-the-second-frame-and-marks-it-do-not-dispose`.
//!
...
async fn inverse_restores_before() {
    ...
    assert!(matches!(inverse[0], GifMutation::SetSnapshot(_)), "set-snapshot/slows-the-second-frame-and-marks-it-do-not-dispose: the undo step must itself be a SetSnapshot carrying the pre-state");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "set-snapshot/slows-the-second-frame-and-marks-it-do-not-dispose: produced diff differs from the committed 🔺️diff/🔣️.json");
