...
//! reader corpus. Sibling to `main.rs` (which builds the untouched `🎬️pattern-strip.gif` behind
...
//!
//! Two subcommands:
//!   build   <recipe-id> <out-dir> <fixture-directory> <before-file> <after-file>
...
/// `🔮️oracles/🖼️raster/🦀️.rs`'s `gif_interlace_row_order`, on the same "a fixed grammar
...
//#region 🔖️Entry
fn cmd_build(id: &str, out_dir: &str, fixture_directory: &str, before_file: &str, after_file: &str) -> i32 {
    let Some((before, after)) = recipe(id) else {
        eprintln!("[gif-89a-reader] unknown recipe {id:?} — known: {}", RECIPE_IDS.join(", "));
        return 1;
    };
    let dir = Path::new(out_dir).join(fixture_directory);
    fs::create_dir_all(&dir).expect("create fixture recipe directory");
    fs::write(dir.join(before_file), encode_gif(&before)).expect("write before fixture");
    fs::write(dir.join(after_file), encode_gif(&after)).expect("write after fixture");
    eprintln!("[gif-89a-reader] {id}: {before_file} + {after_file} -> {}", dir.display());
    0
}
...
fn main() {
    ...
    let code = match args.get(1).map(String::as_str) {
        Some("build") => {
            let (Some(id), Some(out_dir), Some(fixture_directory), Some(before_file), Some(after_file)) = (args.get(2), args.get(3), args.get(4), args.get(5), args.get(6)) else {
                eprintln!("usage: reader build <recipe-id> <out-dir> <fixture-directory> <before-file> <after-file>");
                std::process::exit(2);
            };
            cmd_build(id, out_dir, fixture_directory, before_file, after_file)
        }
        Some("project") => {
        ...
        _ => {
            eprintln!("usage: reader build <recipe-id> <out-dir> <fixture-directory> <before-file> <after-file> | project <path-to-gif> | list-recipes");
            2
        }
    };
...
#[cfg(test)]
#[path = "./🧪️tests/🧪️process-local-determinism/🦀️.rs"]
