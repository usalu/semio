...
mod subject {
    ...
    /// 🧭️ Registers all 20 kinds' `mutate`/`inverse` scenario ids plus the round trip, mirroring
    /// `super::adapter`'s oracle registration.
    pub fn register(mut built: Adapter) -> Adapter {
        built = built.subject("mutate", mutate).subject("no-mutation-baseline-mutate", mutate);
        built = built.subject("inverse", inverse).subject("no-mutation-baseline-inverse", inverse);
...
pub fn adapter() -> Adapter {
    ...
    built = built.oracle("mutate", mutate_oracle).oracle("no-mutation-baseline-mutate", mutate_oracle);
    built = built.oracle("inverse", inverse_oracle).oracle("no-mutation-baseline-inverse", inverse_oracle);
