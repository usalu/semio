...
use crate::standards::v89a::subsets::any::schema::mutations::{set_frame_pixels, GifMutation};
...
impl protocol::OpBinary for Gif89aEditCommand {
    ...
    fn decode_op(bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        let parsed = pack::parse_json_bytes(bytes).map_err(|error| protocol::ProtocolError::Malformed { what: "gif_89a-edit-command", offset: 0, detail: error.to_string() })?;
        <Self as dsl::FromValue>::from_value(pack::json_to_dsl_value(&parsed)).map_err(|error| protocol::ProtocolError::Malformed { what: "gif_89a-edit-command", offset: 0, detail: error.to_string() })
    }
}
//#endregion 🔖️Command
...
fn gif89aEditor_example_snapshot(example_id: &str) -> GifSnapshot {
...
fn gif89aEditor_command_id(command: &Gif89aEditCommand) -> &'static str {
    match command { Gif89aEditCommand::SetActiveExample { .. } => semio_s_artifact_stdio_contract::SET_ACTIVE_EXAMPLE_ACTION_ID, _ => "other" }
}
fn gif89aEditor_command_from_action(action: &str, args: Option<&dsl::DslValue>) -> Result<Gif89aEditCommand, Fault> {
    match action {
        semio_s_artifact_stdio_contract::SET_ACTIVE_EXAMPLE_ACTION_ID => Ok(Gif89aEditCommand::SetActiveExample { example_id: semio_s_artifact_stdio_contract::example_id_argument(args, "") }),
        _ => Err(Fault::from(format!("action '{action}' is not setActiveExample"))),
    }
}
fn gif89aEditor_retained_extent(command: &Gif89aEditCommand, _snapshot: &GifSnapshot, _interaction: &protocol::InteractionState) -> Option<usize> {
    matches!(command, Gif89aEditCommand::SetActiveExample { .. }).then_some(1)
}
...
struct Gif89aEditorExampleFactory { keys: Vec<ToolFactoryKey> }
impl Gif89aEditorExampleFactory { fn new(controller_id: &str) -> Self { Self { keys: STDIO_GIF_DOCUMENT_SCHEMA_EXAMPLE_TOOL_IDS.iter().map(|tool_id| ToolFactoryKey::new(controller_id, *tool_id)).collect() } } }
impl ToolJobFactory for Gif89aEditorExampleFactory {
    type Payload = ArtifactRetainedCommandPayload<EditorApp<Gif89aEditor>>;
    type Job = ArtifactRetainedCommandJob<EditorApp<Gif89aEditor>>;
...
impl ArtifactOwnedToolJobFactory for Gif89aEditorExampleFactory {
    ...
    fn register_tool_job_factories(registry: &mut ArtifactToolFactoryRegistry<'_, EditorApp<Self>>) -> Result<(), Fault> {
        registry.register(Gif89aEditorExampleFactory::new(registry.controller_id()))
    }
    fn build_tool_job(request: ArtifactOwnedToolJobRequest<EditorApp<Self>>) -> Result<Option<ToolOperationSpec>, Fault> {
        if !STDIO_GIF_DOCUMENT_SCHEMA_EXAMPLE_TOOL_IDS.contains(&request.tool_id.as_str()) { return Ok(None); }
        if gif89aEditor_command_id(&request.command) != request.tool_id { return Err(Fault::from("stdio-example-tool-mismatch")); }
        ...
        let payload = ArtifactRetainedCommandPayload::try_new(ArtifactRetainedCommandInputs { command: *request.command, snapshot: request.snapshot, config: request.config, history: request.history, interaction_state: request.interaction_state, interaction_hover: request.interaction_hover, context: Some(request.context), operation, completion: request.completion }, gif89aEditor_command_id, STDIO_GIF_DOCUMENT_SCHEMA_EXAMPLE_BYTES, 1, Box::new(BoundedArtifactCommandWork::new(semio_s_artifact_stdio_contract::SET_ACTIVE_EXAMPLE_ACTION_ID, gif89aEditor_retained_reduce, gif89aEditor_retained_extent)))?;
...
pub fn create_gif_89a_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(GIF_89A_DIALECT).document(["semio", "gif"]).icon_id("image").mode_def(edit::definition()).default_mode_id(edit::MODE_ID).window_kind_def(main::definition()).default_layout(edit::layout()).action_with(semio_s_artifact_stdio_contract::set_active_example_action())
