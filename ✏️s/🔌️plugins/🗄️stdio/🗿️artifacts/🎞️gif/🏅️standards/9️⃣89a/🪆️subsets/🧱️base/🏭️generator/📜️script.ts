...
// `gif` 0.13 dependency:
//   generate/manifests           — 🎬️pattern-strip.gif, UNTOUCHED, backs `gif-89a-any-mutate`
...
const GRAPHIC_CONTROL_FIXTURES_DIR = join(HERE, "..", "..", "🎛️graphic-control", "🧫️fixtures");
const RECIPE = "pattern-strip";
const FIXTURE_DIRECTORY = "📼️pattern-strip";
const FIXTURE_FILE = "🎬️pattern-strip.gif";
...
const FIXTURE_DIRECTORY_NAMES: Readonly<Record<string, string>> = {
  "add-app-extension": "🧩️add-app-extension",
  "insert-comment": "💬️insert-comment",
  "insert-frame-applied": "➕️insert-frame-applied",
  "move-frame-applied": "🔀️move-frame-applied",
  "no-mutation-no-op": "⏸️no-mutation-no-op",
  "remove-app-extension": "🧹️remove-app-extension",
  "remove-comment": "🗑️remove-comment",
  "remove-frame-applied": "➖️remove-frame-applied",
  "set-background-color-index-applied": "🖌️set-background-color-index-applied",
  "set-frame-delay-applied": "⏱️set-frame-delay-applied",
  "set-frame-disposal-applied": "♻️set-frame-disposal-applied",
  "set-frame-geometry-applied": "📐️set-frame-geometry-applied",
  "set-frame-interlace-applied": "🪜️set-frame-interlace-applied",
  "set-frame-pixels-applied": "🧱️set-frame-pixels-applied",
  "set-frame-transparency-applied": "👻️set-frame-transparency-applied",
  "set-frame-user-input-applied": "🕹️set-frame-user-input-applied",
  "set-global-color-table-applied": "🌈️set-global-color-table-applied",
...
const COMMENT_RECIPES = new Set(["insert-comment", "remove-comment"]);
const GRAPHIC_CONTROL_RECIPES = new Set(["set-frame-delay-applied", "set-frame-disposal-applied", "set-frame-transparency-applied", "set-frame-user-input-applied"]);

function committedFixtureRoot(id: string): string {
  if (APPLICATION_RECIPES.has(id)) return APPLICATION_FIXTURES_DIR;
  if (COMMENT_RECIPES.has(id)) return COMMENT_FIXTURES_DIR;
  if (GRAPHIC_CONTROL_RECIPES.has(id)) return GRAPHIC_CONTROL_FIXTURES_DIR;
  return FIXTURES_DIR;
}
...
async function manifests(): Promise<void> {
  const outPath = join(FIXTURES_DIR, FIXTURE_DIRECTORY, FIXTURE_FILE);
  ...
  const entry = {
    ...
    family: "mechanical",
    files: [{ path: `../🧫️fixtures/${FIXTURE_DIRECTORY}/${FIXTURE_FILE}`, mediaType: "image/gif", sha256: digest }],
...
function readerBuild(id: string, outDir: string): void {
  const result = spawnSync("cargo", ["run", "--quiet", "--manifest-path", ENGINE_MANIFEST, "--bin", "reader", "--", "build", id, outDir, FIXTURE_DIRECTORY_NAMES[id]!, BEFORE_FILE, AFTER_FILE], { encoding: "utf8" });
  if (result.status !== 0) throw new Error(`reader build ${id} failed (exit ${result.status}): ${result.stderr}`);
}

function readerFileEntry(role: string, dir: string, filename: string, directory: string): { role: string; path: string; mediaType: string; sha256: string; bytes: number } {
  const abs = join(dir, filename);
  const bytes = readFileSync(abs);
  return { role, path: `../🧫️fixtures/${directory}/${filename}`, mediaType: "image/gif", sha256: contentDigest(bytes), bytes: bytes.length };
}
...
async function build(outDir: string | null, only: string | null): Promise<number> {
  ...
  for (const recipe of recipes) {
    try {
      readerBuild(recipe.id, outDir ?? committedFixtureRoot(recipe.id));
  ...
  console.error(`[gif reader generator] ${recipes.length - failed}/${recipes.length} bundle(s) generated${outDir === null ? " into their committed subset roots" : ` into ${outDir}`}${failed > 0 ? `, ${failed} failed` : ""}`);
  return failed > 0 ? 1 : 0;
}
...
async function buildManifests(only: string | null): Promise<number> {
  ...
  const manifests = recipes.map((recipe) => {
    const fixtureRoot = committedFixtureRoot(recipe.id);
    const dir = join(fixtureRoot, FIXTURE_DIRECTORY_NAMES[recipe.id]!);
    if (!existsSync(join(dir, BEFORE_FILE)) || !existsSync(join(dir, AFTER_FILE))) throw new Error(`${dir} is missing ${BEFORE_FILE}/${AFTER_FILE} — run "build --only ${recipe.id}" first`);
    return readerManifestEntry(recipe, fixtureRoot);
  });
...
async function main(argv: readonly string[]): Promise<number> {
  ...
  if (command === "generate") {
    ...
    const outDir = outFlag ?? (fixtureOutRoot !== undefined ? join(fixtureOutRoot, FIXTURE_DIRECTORY) : join(FIXTURES_DIR, FIXTURE_DIRECTORY));
  ...
  if (command === "build") {
    const outFlag = flagValue(rest, "--out");
    const outDir = outFlag ?? fixtureOutRoot ?? null;
...
out, kind, directory_name, before_name, after_name = sys.argv[1:6]
d = os.path.join(out, directory_name); os.makedirs(d, exist_ok=True)
before = os.path.join(d, before_name); after = os.path.join(d, after_name)
...
`;
    ...
    if (command === "extensions") {
      ...
      for (const kind of KINDS) {
        const directory = FIXTURE_DIRECTORY_NAMES[kind]!;
        const root = explicitRoot ?? committedFixtureRoot(kind);
        const written = spawnSync("python3", ["-c", WRITER, root, kind, directory, BEFORE_FILE, AFTER_FILE], { stdio: "inherit" });
        if (written.status !== 0) { failures.push(`${kind}: writer failed`); continue; }
        const cmp = spawnSync(readerBin, ["compare", join(root, directory, BEFORE_FILE), join(root, directory, AFTER_FILE)], { encoding: "utf8" });
    ...
    for (const kind of KINDS) {
      const directory = FIXTURE_DIRECTORY_NAMES[kind]!;
      const root = committedFixtureRoot(kind);
      const files = [];
  ...
  const ASPECT_OUT = process.env.SEMIO_FIXTURE_OUT ?? FIXTURES_DIR;
