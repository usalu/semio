...
Feature: Apply every typed GIF87a mutation to a real derived document
  ...
  not fabricated, from the real animated ../../🏅️standards/7️⃣87a/🪆️subsets/✳️any/📚️examples/💃️dancing/🖼️assets/🖼️dancing.gif
  ...
  (`../../🏅️standards/7️⃣87a/🪆️subsets/✳️any/🚪️io/🦀️component.rs`) pins that refusal as deliberate.
  ...
  @level-exhaustive
  @mode-differential
  Scenario: Undoing no-mutation restores the original document
    Given the real GIF87a input document shared://🐘️dancing-87a-large/🖼️.gif
    When the no-mutation mutation is applied and then its own inverse, computed from the pre-mutation document, is applied on top
      """
      {"kind": "no-mutation", "params": {}}
      """
    Then the oracle and the subject agree on the semantic projection of the restored document
