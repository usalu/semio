...
impl protocol::OpBinary for Gif87aEditCommand {
    const TOOL_JOB_IDS: &'static [&'static str] = STDIO_GIF_DOCUMENT_SCHEMA_EXAMPLE_TOOL_IDS;
...
impl ArtifactEditor for Gif87aEditor {
    ...
    fn build_tool_job(request: ArtifactOwnedToolJobRequest<EditorApp<Self>>) -> Result<Option<ToolOperationSpec>, Fault> {
        ...
        if gif87aEditor_command_id(&request.command) != request.tool_id { return Err(Fault::from("stdio-example-tool-mismatch")); }
...
pub fn create_gif_87a_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(GIF_87A_DIALECT).document(["semio", "gif"]).icon_id("image").mode_def(edit::definition()).default_mode_id(edit::MODE_ID).window_kind_def(main::definition()).default_layout(edit::layout()).action_with(semio_s_artifact_stdio_contract::set_active_example_action())
