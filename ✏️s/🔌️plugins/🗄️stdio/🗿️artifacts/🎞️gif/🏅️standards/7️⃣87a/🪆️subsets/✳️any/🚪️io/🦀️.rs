...
/// recipe's own exemplar pattern (`🔤️txt/…/🚪️io/🦀️.rs`'s
...
/// `../🪆️subsets/✳️any/🧬️schema/📸️snapshot/🦀️.rs`), matching txt's own real-call
/// precedent. `GifDiff` has NO such call: it's hand-rolled (no `dsl::DslDiff` derive — the
/// tri-state `gct`/nested `lct` fields block it, see `🔺️diff/🦀️.rs`'s own doc
