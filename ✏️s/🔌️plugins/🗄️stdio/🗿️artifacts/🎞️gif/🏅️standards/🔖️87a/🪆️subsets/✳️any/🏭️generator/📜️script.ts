...
out_root, kind = sys.argv[1], sys.argv[2]
before, after = pair(kind)
d = os.path.join(out_root, kind)
os.makedirs(d, exist_ok=True)
open(os.path.join(d, 'before.gif'), 'wb').write(before)
open(os.path.join(d, 'after.gif'), 'wb').write(after)
...
function generate(outRoot: string): number {
  ...
  for (const kind of KINDS) {
    const written = spawnSync("python3", ["-c", WRITER, outRoot, kind], { stdio: "inherit" });
    if (written.status !== 0) {
    ...
    const before = projectionOf(binary, join(outRoot, kind, "before.gif"));
    const after = projectionOf(binary, join(outRoot, kind, "after.gif"));
...
async function manifests(): Promise<void> {
  ...
  for (const kind of KINDS) {
    ...
    entries.push({
      ...
      generator: { oracle: ORACLE_ID, packageVersion: "11.3.0", engineFamily: "pillow", engineVersion: "11.3.0", command: "bun ✏️s/🔌️plugins/🗄️stdio/🗿️artifacts/🎞️gif/🏅️standards/🔖️87a/🪆️subsets/✳️any/🏭️generator/📜️script.ts generate", platform: process.platform },
...
async function aspectMode(command: string): Promise<number> {
  ...
  if (command === "aspect" || command === "aspect-manifests") {
    ...
    const probes = join(import.meta.dir, "..", "..", "..", "..", "🔖️89a", "🪆️subsets", "✳️any", "🔬️probes", "📜️script.ts");
    if (command === "aspect") {
