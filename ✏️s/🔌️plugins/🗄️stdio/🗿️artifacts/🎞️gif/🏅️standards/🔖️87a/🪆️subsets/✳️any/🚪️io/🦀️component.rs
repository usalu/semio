...
// real GIF87a codec — variable 3–12 bit LZW, GCT/LCT, interlace de-row on decode — relocated
// here verbatim (destination rule 2: codecs → `🚪️io/`; rule 6: pure format algorithms with no
// snapshot dependency stay WITH the codec here, since they're GIF-specific, not
// artifact-independent). The LZW core (`lzw_encode`/`lzw_decode`) and the surrounding byte-level
// helpers (sub-blocks, color tables, RGBA quantization) stay `pub` so the 89a standard's own
// (also-relocated) codec reuses them verbatim instead of duplicating — see
...
mod tests {
    ...
    mod conformance_laws {
        ...
        async fn fixture_honesty_law() {
            ...
            let parsed = <GifSnapshot as store::ArtifactDsl>::parse_dsl(FIXTURE_DSL).await.expect("parse shipped .dsl.semio fixture");
            ...
            let decoded = <GifSnapshot as store::ArtifactPack>::decode_pack(FIXTURE_PACK).await.expect("decode shipped .pack.semio fixture");
