...
pub enum XlsxStrictMutation {
    ...
    /// 🏷️ Retargets the main SpreadsheetML namespace declaration wherever it is declared, from whichever member of the `[transitional, strict]` pair the package currently carries to `namespace`.
    SetMainNamespace {
        namespace: String,
    },
    /// 🔗️ Retargets the `officeDocument` relationships MARKUP namespace, the `xmlns:r` an `r:id` attribute resolves against — a different axis from the relationship TYPE base.
    SetRelationshipsNamespace {
        namespace: String,
    },
    ...
    /// ➕️ Adds a legacy VML drawing part together with its content-type override — the markup ISO/IEC 29500-1 Strict removes entirely and ISO/IEC 29500-4 Transitional retains.
    InsertVmlPart {
        path: String,
        markup: String,
    },
    /// ➖️ Removes the legacy VML drawing part at `path` and its content-type override.
    RemoveVmlPart {
        path: String,
    },
    /// 🏷️ Sets the resolved content type of the worksheet part at `path` — the one soft axis this subset's conformance check reads.
    SetWorksheetContentType {
        path: String,
        content_type: String,
    },
}
...
/// mutation catalog `xlsx-ecma-376-strict` (`../../🧪️oracle/🔣️component.json`) is measured against
...
impl Mutation<XlsxSnapshot> for XlsxStrictMutation {
    type Diff = XlsxDiff;

    fn diff(&self, base: &XlsxSnapshot) -> protocol::MutationOutcome<Self::Diff> {
        protocol::MutationOutcome::new(match self {
            Self::NoMutation => XlsxDiff::default(),
            Self::SetSnapshot { snapshot } => <XlsxDiff as DiffAlgebra<XlsxSnapshot>>::between(base, snapshot),
            Self::SetMainNamespace { namespace } => diff_retarget_namespace(base, MAIN_NAMESPACES, namespace),
            Self::SetRelationshipsNamespace { namespace } => diff_retarget_namespace(base, RELATIONSHIP_NAMESPACES, namespace),
            ...
            Self::InsertVmlPart { path, markup } => diff_insert_vml_part(base, path, markup),
            Self::RemoveVmlPart { path } => diff_remove_vml_part(base, path),
            Self::SetWorksheetContentType { path, content_type } => diff_set_content_type(base, path, content_type),
        })
    }
    ...
    fn inverse(&self, base: &XlsxSnapshot) -> Vec<Self> {
        vec![match self {
            ...
            Self::SetRelationshipsNamespace { .. } => match declared_pair_member(base, RELATIONSHIP_NAMESPACES) {
                Some(namespace) => Self::SetRelationshipsNamespace { namespace },
                None => Self::NoMutation,
            },
            ...
            Self::InsertVmlPart { path, .. } => Self::RemoveVmlPart { path: path.clone() },
            Self::RemoveVmlPart { path } => match part_text(base, path) {
                Some(markup) => Self::InsertVmlPart { path: path.clone(), markup },
                None => Self::NoMutation,
            },
            Self::SetWorksheetContentType { path, .. } => match resolved_content_type(base, path) {
                Some(content_type) => Self::SetWorksheetContentType { path: path.clone(), content_type },
                None => Self::NoMutation,
            },
        }]
    }
...
mod tests {
    ...
    fn kinds_match_enum_and_catalog() {
        fn kind_of(mutation: &XlsxStrictMutation) -> &'static str {
            match mutation {
                XlsxStrictMutation::NoMutation => "no-mutation",
                XlsxStrictMutation::SetSnapshot { .. } => "set-snapshot",
                XlsxStrictMutation::SetMainNamespace { .. } => "set-main-namespace",
                XlsxStrictMutation::SetRelationshipsNamespace { .. } => "set-relationships-namespace",
                XlsxStrictMutation::SetConformanceAttribute { .. } => "set-conformance-attribute",
                XlsxStrictMutation::RemoveConformanceAttribute => "remove-conformance-attribute",
                XlsxStrictMutation::InsertVmlPart { .. } => "insert-vml-part",
                XlsxStrictMutation::RemoveVmlPart { .. } => "remove-vml-part",
                XlsxStrictMutation::SetWorksheetContentType { .. } => "set-worksheet-content-type",
            }
        }
        let samples = [
            XlsxStrictMutation::NoMutation,
            XlsxStrictMutation::SetSnapshot { snapshot: XlsxSnapshot::default() },
            XlsxStrictMutation::SetMainNamespace { namespace: String::new() },
            XlsxStrictMutation::SetRelationshipsNamespace { namespace: String::new() },
            XlsxStrictMutation::SetConformanceAttribute { value: String::new() },
            XlsxStrictMutation::RemoveConformanceAttribute,
            XlsxStrictMutation::InsertVmlPart { path: String::new(), markup: String::new() },
            XlsxStrictMutation::RemoveVmlPart { path: String::new() },
            XlsxStrictMutation::SetWorksheetContentType { path: String::new(), content_type: String::new() },
        ];
