...
pub async fn deserialize(from: &BinarySnapshot) -> Result<XlsxSnapshot, store::PackError> {
    let mut snap = crate::artifacts::xlsx::standards::v_ecma_376::subsets::any::io::import::deserializers::decode_xlsx(&from.bytes).await.map_err(|e| store::PackError::Schema(e.to_string()))?;
