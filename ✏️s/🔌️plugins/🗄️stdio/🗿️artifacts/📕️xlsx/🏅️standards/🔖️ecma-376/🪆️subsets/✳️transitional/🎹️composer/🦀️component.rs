//! 🎹️ XlsxTransitionalComposer (ecma-376/✳️transitional) — reads the same sources the ✳️any
//! subset does (native `stdio.xlsx` ecma-376, plus its `zip`/`xml` DAG deps), delegates the
//! actual parse to the ✳️any composer, then HARD-GATES the `transitional` dialect stamp on real
//! ISO/IEC 29500-4 Transitional conformance (namespace/relationship-base/`conformance`
//! attribute). A hard violation (Strict-shaped or unparsable workbook.xml) fails composition
//! outright with specific `Diagnostic`s; a soft one (worksheet content-type mismatch) passes
...
//! SAME `check_transitional_conformance` function backs both, mirroring ✳️strict's composer
//! exactly. Unlike ✳️strict, today's ecma-376 writer (`⚙️engine::encode_xlsx`) always re-emits
//! Transitional-shaped `xl/workbook.xml` on regenerate, so a wire round trip through
//! `ArtifactPack::encode_pack` does NOT break this dialect's own hard gate the way it does
//! ✳️strict's — see that module's doc comment for the shared writer-scope-cut background.
...
use crate::artifacts::xlsx::standards::v_ecma_376::subsets::transitional::analyzer::check_transitional_conformance;
...
//#region 🔖️Composer
pub struct XlsxTransitionalComposer;

impl ArtifactComposer for XlsxTransitionalComposer {
...
/// (`crate::artifacts::xlsx::standards::v_ecma_376::composer::entries()`).
pub fn register() {
...
mod tests {
    ...
    use crate::artifacts::xlsx::standards::v_ecma_376::subsets::transitional::analyzer::CODE_NAMESPACE_MISMATCH;
    use crate::artifacts::xlsx::standards::v_ecma_376::subsets::transitional::builder::XlsxTransitionalBuilder;
    ...
    fn conforming_builder_snapshot_composes_and_stamps_transitional() {
        ...
        let composed = XlsxTransitionalComposer::compose(&sources).expect("clean document must compose to transitional");
    ...
    fn strict_shaped_document_fails_compose_with_real_diagnostic() {
        ...
        use crate::artifacts::xlsx::standards::v_ecma_376::subsets::strict::builder::stamp_strict_namespace;
        ...
        let err = XlsxTransitionalComposer::compose(&sources).expect_err("a Strict-shaped workbook.xml must not stamp transitional");
