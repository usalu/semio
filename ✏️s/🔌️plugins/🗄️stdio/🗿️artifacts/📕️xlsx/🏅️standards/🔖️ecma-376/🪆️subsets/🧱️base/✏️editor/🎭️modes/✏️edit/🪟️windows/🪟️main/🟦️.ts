/** 📊️ Xlsx editor (ecma-376/🧱️base) — `main` window: typed twin of `🦀️.rs`'s
...
/** ✏️ `set-cell` payload shape — mirrors `XlsxEditorCommand::SetCell`. `row` indexes the flattened
 * cell list `xlsx_flat_cells` emits (sheet order, then each sheet's own cell storage order);
 * `value` is the edited cell's raw display text. */
