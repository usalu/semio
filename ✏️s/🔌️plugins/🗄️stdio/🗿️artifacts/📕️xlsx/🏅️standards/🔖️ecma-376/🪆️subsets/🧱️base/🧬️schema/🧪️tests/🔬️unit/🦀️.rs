...
mod conformance_laws {
    ...
    /// XML parts an xlsx OPC package carries (`📸️snapshot/📝️text/📖️.grammar.semio`'s
    ...
    /// deliberately after a codec change — `cargo test -p semio-s-artifact-stdio-xlsx --lib --
    /// --ignored zzz_write` — then re-run the law. Same shape as tiff's `zzz_write_native_tiff_fixture`.
    #[semio_framework_async_macros::async_test]
    #[ignore]
