/** 📊️ Xlsx editor (ecma-376/✳️transitional) — `main` window: typed twin of `🦀️component.rs`'s
...
/** ✏️ `set-cell` payload shape — mirrors `XlsxTransitionalEditorCommand::SetCell`. `row` indexes
 * the flattened cell list `xlsx_flat_cells` emits (sheet order, then each sheet's own cell storage
