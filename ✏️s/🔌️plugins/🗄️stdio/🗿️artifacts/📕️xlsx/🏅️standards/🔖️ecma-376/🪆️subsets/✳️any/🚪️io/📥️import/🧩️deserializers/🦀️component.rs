...
async fn workbook_sheets_from_xml(doc: &XmlDocument, part: &str) -> Result<Vec<SheetRef>, XlsxError> {
    ...
    for s in sheets_el {
        ...
        let sheet_name = attr_val(attrs, "name").await.ok_or_else(|| bad("<sheet> missing name".into()))?.to_string();
        let r_id = attr_val(attrs, "r:id").await.ok_or_else(|| bad("<sheet> missing r:id".into()))?.to_string();
...
async fn extract_typed_value(children: &[XmlNode], t: Option<&str>, sst_len: usize, part: &str) -> Result<XlsxCellValue, XlsxError> {
    match t {
        Some("s") => {
            let v = find_v_text(children).await.ok_or_else(|| XlsxError::Xml { part: part.into(), detail: "t=\"s\" cell missing <v>".into() })?;
        ...
        Some("str") => Ok(XlsxCellValue::InlineString(find_v_text(children).await.unwrap_or_default())),
        Some("inlineStr") => {
        ...
        Some("b") => {
            let v = find_v_text(children).await.unwrap_or_default();
            Ok(XlsxCellValue::Boolean(v.trim() == "1" || v.trim().eq_ignore_ascii_case("true")))
        }
        Some("e") => Ok(XlsxCellValue::InlineString(find_v_text(children).await.unwrap_or_default())),
        None | Some(_) => match find_v_text(children).await {
...
async fn extract_cell_value(children: &[XmlNode], t: Option<&str>, sst_len: usize, part: &str) -> Result<XlsxCellValue, XlsxError> {
    if let Some(expr) = find_f_text(children).await {
        let cached = if find_v_text(children).await.is_some() { Some(Box::new(extract_typed_value(children, t, sst_len, part).await?)) } else { None };
        return Ok(XlsxCellValue::Formula { expr, cached });
    }
    extract_typed_value(children, t, sst_len, part).await
}
...
async fn worksheet_cells_from_xml(doc: &XmlDocument, sst_len: usize, part: &str) -> Result<Vec<XlsxCell>, XlsxError> {
    ...
    for row_node in sheet_data {
        ...
        let row = attr_val(attrs, "r").await.ok_or_else(|| bad("<row> missing r".into()))?.parse::<u32>().map_err(|_| bad("<row> r attribute is not a valid integer".into()))?;
        for c_node in row_children {
            let XmlNode::Element { name, attrs, children: c_children } = c_node else { continue };
            if name != "c" {
                continue;
            }
            let reference = attr_val(attrs, "r").await.ok_or_else(|| bad("<c> missing r".into()))?;
            let col = column_index(column_letters_of(reference).await).await.ok_or_else(|| bad(format!("<c> r={reference:?} has no valid column-letter prefix")))?;
            let t = attr_val(attrs, "t");
            let value = extract_cell_value(c_children, t.await, sst_len, part).await?;
...
pub async fn decode_xlsx(data: &[u8]) -> Result<XlsxSnapshot, XlsxError> {
    ...
    let workbook_path = opc.resolve_relationship("", REL_TYPE_OFFICE_DOCUMENT).await.or_else(|| opc.resolve_relationship("", REL_TYPE_OFFICE_DOCUMENT_STRICT)).ok_or(XlsxError::MissingWorkbookRelationship)?;
    let workbook_bytes = opc.part_bytes(&workbook_path).await.ok_or_else(|| XlsxError::MissingPart(workbook_path.clone()))?;
    ...
    let workbook_xml = xml_document_from_text(&workbook_text).await.map_err(|e| XlsxError::Xml { part: workbook_path.clone(), detail: e })?;
    let sheet_refs = workbook_sheets_from_xml(&workbook_xml, &workbook_path).await?;

    let workbook_rels = opc.relationships_for(&workbook_path);
    let shared_strings = match workbook_rels.await.iter().find(|r| r.rel_type == REL_TYPE_SHARED_STRINGS || r.rel_type == REL_TYPE_SHARED_STRINGS_STRICT) {
        Some(rel) => {
            let path = opc::resolve_relationship_target(&workbook_path, &rel.target);
            let bytes = opc.part_bytes(&path).await.ok_or_else(|| XlsxError::MissingPart(path.clone()))?;
            let text = String::from_utf8(bytes.to_vec()).map_err(|_| XlsxError::Xml { part: path.clone(), detail: "not valid utf-8".into() })?;
            let doc = xml_document_from_text(&text).await.map_err(|e| XlsxError::Xml { part: path.clone(), detail: e })?;
            shared_strings_from_xml(&doc, &path).await?
        }
        None => Vec::new(),
    };
    ...
    for sheet_ref in &sheet_refs {
        let rel = workbook_rels.await.iter().find(|r| r.id == sheet_ref.r_id).ok_or_else(|| XlsxError::Malformed(format!("sheet {:?} references unknown relationship id {}", sheet_ref.name, sheet_ref.r_id)))?;
        let path = opc::resolve_relationship_target(&workbook_path, &rel.target);
        let bytes = opc.part_bytes(&path).await.ok_or_else(|| XlsxError::MissingPart(path.clone()))?;
        let text = String::from_utf8(bytes.to_vec()).map_err(|_| XlsxError::Xml { part: path.clone(), detail: "not valid utf-8".into() })?;
        let doc = xml_document_from_text(&text).await.map_err(|e| XlsxError::Xml { part: path.clone(), detail: e })?;
        let cells = worksheet_cells_from_xml(&doc, sst_len, &path).await?;
        sheets.push(XlsxSheet { name: sheet_ref.name.clone(), cells });
    }

    Ok(XlsxSnapshot::from_parts(opc, XlsxWorkbook { sheets, shared_strings }).await)
}
//#endregion 🔖️Codec
...
pub async fn sniff_xlsx_bytes(data: &[u8]) -> bool {
    ...
    let path = opc.resolve_relationship("", REL_TYPE_OFFICE_DOCUMENT).await.or_else(|| opc.resolve_relationship("", REL_TYPE_OFFICE_DOCUMENT_STRICT));
    match path {
