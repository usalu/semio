//! 🔮️ Mutation oracle for this subset — every CONFORMANCE-CLASS mutation the `xlsx` `✳️strict`
...
//! document object at all. `check_strict_conformance` reads exactly five axes on an already-decoded `XlsxSnapshot`: `xl/workbook.xml`'s root `xmlns`, its root `xmlns:r`, its root `conformance` attribute, the presence of any part whose content type is the legacy VML drawing type, and each worksheet part's own content type. This vocabulary is one kind per axis, in that order — nothing more and nothing less.
//!
...
const PROFILE: crate::document::ooxml::OoxmlProfile = crate::document::ooxml::OoxmlProfile {
    format: "xlsx",
    main_namespaces: ["http://schemas.openxmlformats.org/spreadsheetml/2006/main", "http://purl.oclc.org/ooxml/spreadsheetml/main"],
...
pub fn oracle_apply_mutation(input: &[u8], spec: &Json) -> Result<Vec<u8>, String> {
    ...
    if !KINDS.contains(&kind.as_str()) {
        return Err(format!("mutation kind {kind:?} is not declared by the xlsx-ecma-376-strict catalog"));
    }
...
pub fn project_package(input: &[u8]) -> Result<Json, String> {
    crate::document::ooxml::project(input, "xlsx")
}
...
mod tests {
    ...
    const FIXTURE: &[u8] = include_bytes!("../../../../../🧫️fixtures/📕️reuse-marketplaces.xlsx");
    ...
    fn params_for(kind: &str) -> Json {
        match kind {
            ...
            "set-main-namespace" => json_object(vec![("namespace", Json::String("http://purl.oclc.org/ooxml/spreadsheetml/main".to_string()))]),
            ...
            "insert-vml-part" => json_object(vec![("path", Json::String("xl/drawings/vmlDrawing1.vml".to_string()))]),
            "remove-vml-part" => json_object(vec![("path", Json::String("xl/drawings/vmlDrawing1.vml".to_string()))]),
            "set-worksheet-content-type" => json_object(vec![("path", Json::String("xl/worksheets/sheet1.xml".to_string())), ("contentType", Json::String("application/xml".to_string()))]),
