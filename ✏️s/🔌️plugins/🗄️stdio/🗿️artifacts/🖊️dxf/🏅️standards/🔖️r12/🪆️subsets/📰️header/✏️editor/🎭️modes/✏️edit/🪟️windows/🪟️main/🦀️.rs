//! 🧊 DXF editor — the Main window: the SAME world-3d mesh scene shape as the sibling
...
//! artifact-level `DxfSnapshot` — this window itself never mutates; mutation is the surface
...
const DXF_ANY_EDIT_FALLBACK_MESH_KIND: &str = "box";
const DXF_ANY_EDIT_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const DXF_ANY_EDIT_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const DXF_ANY_EDIT_DEFAULT_CAMERA_FOV: f64 = 45.0;
    ...
    let instances: Vec<pack::JsonValue> = (0..count)
        .map(|index| {
            pack::json!({
                ...
                "meshId": DXF_ANY_EDIT_FALLBACK_MESH_KIND,
...
pub fn render(document: &DxfSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = pack::json_to_string(&pack::json_array([pack::json!({ "id": DXF_ANY_EDIT_FALLBACK_MESH_KIND, "data": mesh_from_kind(DXF_ANY_EDIT_FALLBACK_MESH_KIND) })]));
    let view = MeshView {
        camera_json: world3d_camera_json(DXF_ANY_EDIT_DEFAULT_CAMERA_POSITION, DXF_ANY_EDIT_DEFAULT_CAMERA_TARGET, DXF_ANY_EDIT_DEFAULT_CAMERA_FOV),
        meshes_json,
