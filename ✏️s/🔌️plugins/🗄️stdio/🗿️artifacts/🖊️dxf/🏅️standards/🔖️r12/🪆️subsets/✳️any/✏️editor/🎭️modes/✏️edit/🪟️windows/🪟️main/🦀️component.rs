...
pub async fn render(document: &DxfSnapshot) -> UiNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": DXF_ANY_EDIT_FALLBACK_MESH_KIND, "data": mesh_from_kind(DXF_ANY_EDIT_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(DXF_ANY_EDIT_DEFAULT_CAMERA_POSITION, DXF_ANY_EDIT_DEFAULT_CAMERA_TARGET, DXF_ANY_EDIT_DEFAULT_CAMERA_FOV).await,
        meshes_json,
