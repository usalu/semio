...
pub mod derived_construction {
    ...
    impl ArtifactBuilder for DxfBuilderConstruction {
        ...
        async fn from_text(text: &str) -> Result<Self, store::TextError> {
            Ok(Self::from_snapshot(<DxfSnapshot as store::ArtifactDsl>::parse_dsl(text).await?).await)
        }
        async fn from_binary(bytes: &[u8]) -> Result<Self, store::PackError> {
            Ok(Self::from_snapshot(<DxfSnapshot as store::ArtifactPack>::decode_pack(bytes).await?).await)
        }
    ...
    impl ArtifactAnalysis for DxfAnalyzerAnalysis {
        ...
        async fn analyze(sources: &[AnalyzeSource<'_>]) -> Analysis<Self::Parts> {
            ...
            for source in sources {
                match source {
                    AnalyzeSource::Text(text) => match <DxfSnapshot as store::ArtifactDsl>::parse_dsl(text).await {
                    ...
                    AnalyzeSource::Binary(bytes) => match <DxfSnapshot as store::ArtifactPack>::decode_pack(bytes).await {
                        Ok(snapshot) => parts.snapshot = Some(snapshot),
                        Err(err) => {
                            confidence = IoConfidence::Low;
...
mod tests {
    ...
    async fn codec_round_trip() {
        ...
        let parsed = <DxfSnapshot as store::ArtifactDsl>::parse_dsl(&text).await.expect("parse");
        assert_eq!(parsed.schema, snap.schema);
        let bytes = store::ArtifactPack::encode_pack(&snap);
        let decoded = <DxfSnapshot as store::ArtifactPack>::decode_pack(&bytes).await.expect("decode");
    ...
    mod conformance_laws {
        ...
        async fn fixture_honesty_law() {
            ...
            let parsed = <DxfSnapshot as store::ArtifactDsl>::parse_dsl(FIXTURE_DSL).await.expect("parse shipped .dsl.semio fixture");
            ...
            let decoded = <DxfSnapshot as store::ArtifactPack>::decode_pack(FIXTURE_PACK).await.expect("decode shipped .pack.semio fixture");
