//! 👁️ DXF viewer — the read-only counterpart of `✏️editor` for this subset (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `DxfAnyViewer`
//! implements `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<DxfAnyViewer>`
...
use crate::viewer::dxf::modes::view;
...
impl protocol::OpBinary for DxfAnyViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(DxfAnyViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
pub struct DxfAnyViewer;

impl ArtifactViewer for DxfAnyViewer {
    ...
    type Command = DxfAnyViewCommand;
...
pub fn create_dxf_any_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(DXF_ANY_DIALECT).document(["stdio", "dxf"]).icon_id("box").mode_def(view::definition()).default_mode_id(view::DXF_ANY_VIEW_MODE_ID).window_kind_def(main::definition()).default_layout(view::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn viewer_never_mutates_the_document_or_draft_store() {
        semio_framework_plugin::testkit::assert_viewer_never_mutates::<DxfAnyViewer>();
    }
}
//#endregion 🧪️Tests
