...
pub fn register() {
    crate::artifacts::dxf::io::register();
    register_artifact_schema();
    register_pilot_languages();
    store::register_document_codec(store::DocumentCodec::of::<DxfSnapshot, DxfMutation>(STDIO_DXF_DOCUMENT_SCHEMA));
}
...
impl protocol::ArtifactEngine for DxfEngine {
    type Artifact = DxfArtifact;
...
mod tests {
    ...
    fn codec_round_trip() {
        ...
        let parsed = <DxfSnapshot as store::DocumentDsl>::parse_dsl(&text).expect("parse");
        assert_eq!(parsed.schema, snap.schema);
        let bytes = store::DocumentPack::encode_pack(&snap);
        let decoded = <DxfSnapshot as store::DocumentPack>::decode_pack(&bytes).expect("decode");
