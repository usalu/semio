...
Feature: Apply every typed PPTX ECMA-376 Transitional conformance-class mutation to a real package
  ...
  PresentationML and DrawingML, each addressable on its own. No ✳️any mutation moves any of those
  axes and no mutation here touches document content, so the two vocabularies are disjoint by
  construction.
