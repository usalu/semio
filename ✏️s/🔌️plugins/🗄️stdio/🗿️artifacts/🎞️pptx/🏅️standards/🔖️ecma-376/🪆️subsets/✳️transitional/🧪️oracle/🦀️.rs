//! 🔮️ Mutation oracle for this subset — every CONFORMANCE-CLASS mutation the `pptx` `✳️transitional`
...
//#region 🔖️Vocabulary
/// 🧾️ Kebab-case spelling of every variant this subset's `PptxTransitionalMutation` declares, in
/// declaration order. The catalog `pptx-ecma-376-transitional` is measured against this exact list, and the
...
pub fn oracle_apply_mutation(input: &[u8], spec: &Json) -> Result<Vec<u8>, String> {
    ...
    if !KINDS.contains(&kind.as_str()) {
        return Err(format!("mutation kind {kind:?} is not declared by the pptx-ecma-376-transitional catalog"));
    }
