...
pub async fn deserialize(from: &BinarySnapshot) -> Result<PptxSnapshot, store::PackError> {
    let mut snap = crate::artifacts::pptx::standards::v_ecma_376::subsets::any::io::import::deserializers::decode_pptx(&from.bytes).await.map_err(|e| store::PackError::Schema(e.to_string()))?;
