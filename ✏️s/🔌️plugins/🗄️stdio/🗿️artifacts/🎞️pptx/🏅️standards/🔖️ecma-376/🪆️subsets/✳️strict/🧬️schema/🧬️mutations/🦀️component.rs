...
pub enum PptxStrictMutation {
    ...
    /// ➕️ Appends an `mc:AlternateContent` markup-compatibility fallback to the root element of the part at `path`.
    InsertAlternateContent {
        path: String,
    },
    /// ➖️ Strips every `mc:AlternateContent` root child from the part at `path`.
...
impl Mutation<PptxSnapshot> for PptxStrictMutation {
    type Diff = PptxDiff;
    ...
    fn diff(&self, base: &PptxSnapshot) -> protocol::MutationOutcome<Self::Diff> {
        protocol::MutationOutcome::new(match self {
            ...
            Self::InsertAlternateContent { path } => diff_root_children(base, path, |children| {
                children.push(alternate_content_node());
                true
            }),
            Self::RemoveAlternateContent { path } => diff_root_children(base, path, |children| {
    ...
    fn inverse(&self, base: &PptxSnapshot) -> Vec<Self> {
        vec![match self {
            ...
            Self::RemoveVmlPart { path } => match xml_part(base, path) {
                Some(part) => Self::InsertVmlPart { path: path.clone(), markup: xml_document_to_text(&part.document) },
                None => Self::NoMutation,
            },
            Self::InsertAlternateContent { path } => Self::RemoveAlternateContent { path: path.clone() },
            Self::RemoveAlternateContent { path } => Self::InsertAlternateContent { path: path.clone() },
        }]
    }
}
...
mod tests {
    ...
    fn kinds_match_enum_and_catalog() {
        fn kind_of(mutation: &PptxStrictMutation) -> &'static str {
            match mutation {
                PptxStrictMutation::NoMutation => "no-mutation",
                PptxStrictMutation::SetSnapshot { .. } => "set-snapshot",
                PptxStrictMutation::SetMainNamespace { .. } => "set-main-namespace",
                PptxStrictMutation::SetDrawingNamespace { .. } => "set-drawing-namespace",
                PptxStrictMutation::SetRelationshipBase { .. } => "set-relationship-base",
                PptxStrictMutation::SetConformanceAttribute { .. } => "set-conformance-attribute",
                PptxStrictMutation::RemoveConformanceAttribute => "remove-conformance-attribute",
                PptxStrictMutation::InsertVmlPart { .. } => "insert-vml-part",
                PptxStrictMutation::RemoveVmlPart { .. } => "remove-vml-part",
                PptxStrictMutation::InsertAlternateContent { .. } => "insert-alternate-content",
                PptxStrictMutation::RemoveAlternateContent { .. } => "remove-alternate-content",
            }
        }
        let samples = [
            PptxStrictMutation::NoMutation,
            PptxStrictMutation::SetSnapshot { snapshot: PptxSnapshot::default() },
            PptxStrictMutation::SetMainNamespace { namespace: String::new() },
            PptxStrictMutation::SetDrawingNamespace { namespace: String::new() },
            PptxStrictMutation::SetRelationshipBase { base: String::new() },
            PptxStrictMutation::SetConformanceAttribute { value: String::new() },
            PptxStrictMutation::RemoveConformanceAttribute,
            PptxStrictMutation::InsertVmlPart { path: String::new(), markup: String::new() },
            PptxStrictMutation::RemoveVmlPart { path: String::new() },
            PptxStrictMutation::InsertAlternateContent { path: String::new() },
            PptxStrictMutation::RemoveAlternateContent { path: String::new() },
        ];
