...
async fn run_from_xml(node: &XmlNode) -> Option<PptxRun> {
    ...
    for rc in children {
        ...
        match name.as_str() {
            "a:rPr" => {
                if let Some(b) = attr_val(attrs, "b").await {
                    bold = b == "1";
                }
                if let Some(i) = attr_val(attrs, "i").await {
                    italic = i == "1";
                }
                if let Some(sz) = attr_val(attrs, "sz").await.and_then(|v| v.parse::<u32>().ok()) {
...
async fn paragraph_from_xml(node: &XmlNode) -> PptxParagraph {
    ...
    for c in element_children(node) {
        if let Some(r) = run_from_xml(c).await {
            runs.push(r);
        }
    }
    PptxParagraph { runs }
...
async fn text_frame_from_xml(tx_body: &XmlNode) -> Vec<PptxParagraph> {
    element_children(tx_body).await.iter().filter(|c| matches!(c, XmlNode::Element { name, .. } if name == "a:p")).map(paragraph_from_xml).collect()
}
//#endregion 🔖️TextXml
...
async fn position_from_xml(shape_children: &[XmlNode]) -> crate::artifacts::pptx::schema::snapshot::PptxTransform {
    ...
    let Some(sp_pr) = find_child(shape_children, "p:spPr").await else { return PptxTransform::default() };
    let Some(xfrm) = find_child(element_children(sp_pr).await, "a:xfrm").await else { return PptxTransform::default() };
    let xfrm_children = element_children(xfrm);
    let (mut x, mut y, mut cx, mut cy) = (0i64, 0i64, 0i64, 0i64);
    if let Some(XmlNode::Element { attrs, .. }) = find_child(xfrm_children.await, "a:off").await {
        x = attr_val(attrs, "x").await.and_then(|v| v.parse().ok()).unwrap_or(0);
        y = attr_val(attrs, "y").await.and_then(|v| v.parse().ok()).unwrap_or(0);
    }
    if let Some(XmlNode::Element { attrs, .. }) = find_child(xfrm_children.await, "a:ext").await {
        cx = attr_val(attrs, "cx").await.and_then(|v| v.parse().ok()).unwrap_or(0);
        cy = attr_val(attrs, "cy").await.and_then(|v| v.parse().ok()).unwrap_or(0);
    }
...
async fn shape_from_xml_node(node: &XmlNode) -> PptxShape {
    ...
    match name.as_str() {
        "p:sp" => {
            let ph_type = find_child(children, "p:nvSpPr").await.and_then(|nv| find_child(element_children(nv), "p:nvPr")).and_then(|nvpr| find_child(element_children(nvpr), "p:ph")).map(|ph| match ph {
                XmlNode::Element { attrs, .. } => semio_framework_plugin::resolve_ready(attr_val(attrs, "type")).unwrap_or("body").to_string(),
                _ => "body".to_string(),
            });
            let position = position_from_xml(children);
            let text_frame = find_child(children, "p:txBody").await.map(text_frame_from_xml).unwrap_or_default();
            match ph_type {
        ...
        "p:pic" => {
            let blip_rel_id = find_child(children, "p:blipFill")
                .await.and_then(|fill| find_child(element_children(fill), "a:blip"))
...
async fn collect_shapes(root: &XmlNode) -> Vec<PptxShape> {
    ...
    let Some(c_sld) = find_child(children, "p:cSld").await else { return Vec::new() };
    let Some(sp_tree) = find_child(element_children(c_sld).await, "p:spTree").await else { return Vec::new() };
    element_children(sp_tree).await.iter().filter(|c| !matches!(c, XmlNode::Element { name, .. } if name == "p:nvGrpSpPr" || name == "p:grpSpPr")).map(shape_from_xml_node).collect()
}
//#endregion 🔖️SlideXml
...
async fn presentation_slide_rids_from_xml(doc: &XmlDocument, part: &str) -> Result<Vec<String>, PptxError> {
    ...
    if let Some(children) = sld_id_lst {
        for c in children {
            ...
            let rid = attr_val(attrs, "r:id").await.ok_or_else(|| bad("<p:sldId> missing r:id".into()))?.to_string();
            out.push(rid);
        }
    }
    Ok(out)
...
pub(crate) async fn project_presentation(opc: &opc::OpcPackage, xml_parts: &[PptxXmlPart]) -> Result<PptxPresentation, PptxError> {
    let presentation_path = resolve_office_document_relationship(opc).await.ok_or(PptxError::MissingPresentationRelationship)?;
    ...
    let slide_rids = presentation_slide_rids_from_xml(&presentation.document, &presentation_path).await?;
    let pres_rels = opc.relationships_for(&presentation_path).await;
...
pub async fn decode_pptx(data: &[u8]) -> Result<PptxSnapshot, PptxError> {
    let mut opc = opc::decode_opc(data).await?;
    let presentation_path = resolve_office_document_relationship(&opc).await.ok_or(PptxError::MissingPresentationRelationship)?;
    let bytes = opc.part_bytes(&presentation_path).await.ok_or_else(|| PptxError::MissingPart(presentation_path.clone()))?;
    let text = String::from_utf8(bytes.to_vec()).map_err(|_| PptxError::Xml { part: presentation_path.clone(), detail: "not valid utf-8".into() })?;
    let xml = xml_document_from_text(&text).await.map_err(|e| PptxError::Xml { part: presentation_path.clone(), detail: e })?;
    let slide_rids = presentation_slide_rids_from_xml(&xml, &presentation_path).await?;
    ...
    for rid in &slide_rids {
        ...
        let path = opc::resolve_relationship_target(&presentation_path, &rel.target);
        let bytes = opc.part_bytes(&path).await.ok_or_else(|| PptxError::MissingPart(path.clone()))?;
        let text = String::from_utf8(bytes.to_vec()).map_err(|_| PptxError::Xml { part: path.clone(), detail: "not valid utf-8".into() })?;
        let slide_xml = xml_document_from_text(&text).await.map_err(|e| PptxError::Xml { part: path.clone(), detail: e })?;
    ...
    for part in std::mem::take(&mut opc.parts) {
        if pptx_part_is_xml(&part.path, &part.content_type).await {
            ...
            let document = xml_document_from_text(&text).await.map_err(|detail| PptxError::Xml { part: part.path.clone(), detail })?;
    ...
    debug_assert!(matches!(project_presentation(&opc, &xml_parts).await, Ok(projected) if projected == presentation));
    Ok(PptxSnapshot::from_parts(opc, xml_parts, presentation).await)
}
//#endregion 🔖️Codec
...
pub async fn sniff_pptx_bytes(data: &[u8]) -> bool {
    ...
    match resolve_office_document_relationship(&opc).await {
