//! 🎹️ PptxTransitionalComposer (ecma-376/✳️transitional) — reads the same sources the ✳️any
//! subset does (native `stdio.pptx` ecma-376, plus its `zip`/`xml` DAG deps), delegates the
//! actual parse to the ✳️any composer, then HARD-GATES the `transitional` dialect stamp on real
//! ISO/IEC 29500-4:2016 Transitional conformance (mirrors `📄️pdf` 1.7 `✳️a`'s `PdfAComposer`, D5
//! requirement #2: "Dialect stamped only when clean"). A hard violation (wrong main namespace, or
//! any Strict namespace/relationship base present) fails composition outright with specific
//! `Diagnostic`s naming what's wrong; a soft one (explicit `conformance="strict"` attribute)
...
//! `🧰️framework/🔨️modules/🚪️io/🦀️component.rs`) — the SAME `check_transitional_conformance`
//! function backs both: the hard gate here runs pre-serialization against the typed
//! `PptxSnapshot` (authoritative), while the registered validator re-runs it post-hoc against the
//! wire `IoPayload` for the generic `io_dispatch`/`wire_artifact_compose` hook.
...
//#region 🔖️Composer
pub struct PptxTransitionalComposer;

impl ArtifactComposer for PptxTransitionalComposer {
...
mod tests {
    ...
    fn conforming_transitional_package_composes_and_stamps_transitional() {
        ...
        let composed = PptxTransitionalComposer::compose(&sources).expect("clean Transitional document must compose to transitional");
    ...
    fn strict_document_fails_compose_with_real_diagnostic() {
        ...
        let sources = vec![ComposeSource { dialect: DIALECT_ANY, payload: AnalyzeSource::Text(&hex) }];
        let err = PptxTransitionalComposer::compose(&sources).expect_err("a Strict document must not stamp transitional");
