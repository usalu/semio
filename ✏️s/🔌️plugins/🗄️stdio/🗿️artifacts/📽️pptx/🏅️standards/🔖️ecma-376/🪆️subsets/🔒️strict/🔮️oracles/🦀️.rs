//! 🔮️ Mutation oracle for this subset — every CONFORMANCE-CLASS mutation the `pptx` `🔒️strict`
...
//#region 🔖️Vocabulary
/// 🧾️ Kebab-case spelling of every variant this subset's `PptxStrictMutation` declares, in
/// declaration order. The catalog `pptx-ecma-376-strict` is measured against this exact list, and the
...
const PROFILE: crate::document::ooxml::OoxmlProfile = crate::document::ooxml::OoxmlProfile {
    format: "pptx",
    main_namespaces: ["http://schemas.openxmlformats.org/presentationml/2006/main", "http://purl.oclc.org/ooxml/presentationml/main"],
    drawing_namespaces: Some(["http://schemas.openxmlformats.org/drawingml/2006/main", "http://purl.oclc.org/ooxml/drawingml/main"]),
...
pub fn oracle_apply_mutation(input: &[u8], spec: &Json) -> Result<Vec<u8>, String> {
    ...
    if !KINDS.contains(&kind.as_str()) {
        return Err(format!("mutation kind {kind:?} is not declared by the pptx-ecma-376-strict catalog"));
    }
...
pub fn project_package(input: &[u8]) -> Result<Json, String> {
    crate::document::ooxml::project(input, "pptx")
}
