//! 👁️ Pptx strict viewer — the read-only counterpart of the mutation-capable surface for this
//! subset (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2).
//! `PptxStrictViewer` implements `ArtifactViewer`, never `ArtifactApp` — `ViewerApp<
//! PptxStrictViewer>` (framework SDK) is the sole runtime adapter, so this file can never
...
pub const PPTX_STRICT_VIEWER_DIALECT: Dialect = Dialect { artifact_kind: "s.stdio.pptx", standard: StandardId("ecma-376"), subset: SubsetId("strict") };
//#endregion 🔖️Dialect

//#region 🔖️Command
/// 👁️ The viewer declares no actions, so its typed command channel has exactly one inert variant.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub enum PptxStrictViewCommand {
    #[default]
    Noop,
}

impl protocol::OpBinary for PptxStrictViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(PptxStrictViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
pub struct PptxStrictViewer;

impl ArtifactViewer for PptxStrictViewer {
    ...
    type Command = PptxStrictViewCommand;

    const DIALECT: Dialect = PPTX_STRICT_VIEWER_DIALECT;
...
pub fn create_pptx_strict_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(PPTX_STRICT_VIEWER_DIALECT)
        .document(["semio", "stdio", "pptx"])
        .icon_id("presentation")
        .mode_def(view::definition())
        .default_mode_id(view::PPTX_STRICT_VIEW_MODE_ID)
