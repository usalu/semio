...
impl protocol::OpText for PptxStrictEditorCommand {
    fn print_op(&self) -> String {
        let PptxStrictEditorCommand::SetPage { index, text } = self;
        format!("set-page index={index} text={}", text.replace('\\', "\\\\").replace('\n', "\\n").replace(' ', "\\s"))
    }
    fn parse_op(line: &str) -> Result<Self, store::TextError> {
        let rest = line.strip_prefix("set-page ").ok_or_else(|| store::TextError::new(format!("pptx strict editor command: unknown line {line:?}"), dsl::TextSpan::at(1, 1)))?;
        let mut index = None;
        let mut text = String::new();
        for token in rest.split(' ') {
            let (key, raw) = token.split_once('=').ok_or_else(|| store::TextError::new(format!("pptx strict editor command: bad token {token:?}"), dsl::TextSpan::at(1, 1)))?;
        ...
        let index = index.ok_or_else(|| store::TextError::new("pptx strict editor command: missing index", dsl::TextSpan::at(1, 1)))?;
        Ok(PptxStrictEditorCommand::SetPage { index, text })
    }
}

impl protocol::OpBinary for PptxStrictEditorCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(<Self as protocol::OpText>::print_op(self).into_bytes())
    }
    fn decode_op(bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        let line = String::from_utf8(bytes.to_vec()).map_err(|error| protocol::ProtocolError::Malformed { what: "pptx strict editor command utf8", offset: 0, detail: error.to_string() })?;
