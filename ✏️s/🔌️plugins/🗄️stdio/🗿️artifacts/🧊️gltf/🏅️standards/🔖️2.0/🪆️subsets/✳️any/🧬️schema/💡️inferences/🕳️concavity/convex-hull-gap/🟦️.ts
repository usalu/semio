/** 💡️ convex-hull-gap atomic glTF inference leaf. */
export const gltfConvexHullGapInference = {
  id: 's.stdio.gltf.inference.convex-hull-gap.v1',
  algorithmVersion: 1,
  cacheKey: 's.stdio.gltf.inference.convex-hull-gap.v1:geometry-v2',
  reads: ['document/scene', 'document/scenes', 'document/nodes', 'document/meshes', 'document/accessors', 'document/bufferViews', 'document/buffers', 'buffers'],
} as const;
export type GltfConvexHullGapInference = typeof gltfConvexHullGapInference;

