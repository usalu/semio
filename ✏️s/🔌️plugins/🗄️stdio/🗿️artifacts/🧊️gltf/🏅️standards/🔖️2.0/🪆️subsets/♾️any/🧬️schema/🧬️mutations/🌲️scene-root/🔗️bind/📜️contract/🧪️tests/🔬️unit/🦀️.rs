use crate::schema::mutations::bind_scene_root_node as mutation;
...
fn canonical_vectors_execute_direct_mutation_and_codec_laws() {
    ...
    for vector in vectors {
        let payload: mutation::GltfBindSceneRootNodePayload = decode(&vector["mutation"]);
        ...
        assert_laws(&mutation::BindSceneRootNodeMutation::Apply(payload.clone()), &base, &expected);
        ...
        assert!(pack::from_json_str::<mutation::GltfBindSceneRootNodePayload>(malformed).is_err());
    ...
    println!("[DEBUG] bind_scene_root_node: {} canonical vectors verified through direct mutations, inverse restoration and the independent JSON oracle.", vectors.len());
}
