...
pub async fn validate(payload: &GltfCreateBufferPayload, base: &GltfSnapshot) -> Result<(), GltfTopLevelMutationRejection> { if payload.position > base.document.buffers.len() { return Err(reject("gltf.mutation.insert-out-of-range", "document/buffers", "position must be within the collection")); }  if base.document.buffers.len() != base.buffers.len() { return Err(reject("gltf.mutation.buffer-alignment", "buffers", "descriptor and bytes arrays must align")); } Ok(()) }
pub async fn apply(payload: &GltfCreateBufferPayload, base: &GltfSnapshot) -> Result<GltfSnapshot, GltfTopLevelMutationRejection> { validate(payload, base)?; let mut next = base.clone(); repair(&mut next.document, GltfTopLevelFamily::Buffers, &Change::Insert(payload.position))?; next.document.buffers.insert(payload.position, GltfBuffer { byte_length: payload.bytes.len(), uri: None, name: None, extensions: None, extras: None }); next.buffers.insert(payload.position, payload.bytes.clone()); Ok(next) }
