//! 🧪️ `transform-node` fixture — `replaces-a-translation-with-a-scaled-trs-transform`.
//!
...
use crate::artifacts::gltf::schema::mutations::transform_node::mutation::GltfTransformNodePayload;
use crate::artifacts::gltf::schema::mutations::transform_node::{diff, inverse, mutation};
use crate::artifacts::gltf::GltfSnapshot;

const CASE: &str = "transform-node/replaces-a-translation-with-a-scaled-trs-transform";
...
fn payload() -> GltfTransformNodePayload {
    serde_json::from_str(MUTATION).expect("transform-node payload decodes")
}

/// ▶️ `transform-node` writes the whole TRS triple and clears `matrix` in the same step.
#[semio_framework_async_macros::async_test]
async fn applies_to_committed_after() {
    let snapshot = mutation::apply(&payload(), &before()).expect("transform-node applies to its committed before-snapshot");
...
async fn inverse_restores_before() {
    let base = before();
    let inverse = inverse::derive(&payload(), &base).expect("transform-node inverse derives from the exact base");
...
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => {
            let snapshot = attempt.expect("transform-node declared applied");
        ...
        "rejected" => {
            ...
            assert_eq!(attempt.expect_err("transform-node declared rejected").code, code, "{CASE}: rejection code differs from the committed outcome");
        }
...
async fn produces_committed_diff() {
    let produced = diff::derive(&payload(), &before()).expect("transform-node derives its diff");
