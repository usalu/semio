...
fn mutable_input(ctx: &Context, kind: &str) -> Result<Vec<u8>, String> {
    let uri = match kind {
        "create-animation" => "shared://🌱️create-animation-applied/⬅️before.gltf",
        "delete-animation" => "shared://🗑️delete-animation-applied/⬅️before.gltf",
        "move-animation" => "shared://🚚️move-animation-applied/⬅️before.gltf",
...
/// `reorder-animations` against each kind's committed `⬅️before.gltf` selected by
/// `mutable_input` — computed independently since the oracle role must not link the
