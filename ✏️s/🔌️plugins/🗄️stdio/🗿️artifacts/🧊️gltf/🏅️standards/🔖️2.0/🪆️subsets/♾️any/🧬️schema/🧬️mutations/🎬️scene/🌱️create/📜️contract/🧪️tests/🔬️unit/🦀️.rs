...
use crate::schema::mutations::create_scene::{DESCRIPTOR, diff, inverse, mutation};
...
async fn create_scene_shared_vector_executes_descriptor_and_phase_laws() {
    let contract: Contract = serde_json::from_str(include_str!("../../🔣️.json")).expect("canonical vector decodes");
