/** 💡️ minimum-distance-to-neighbors atomic glTF inference leaf. */
export const gltfMinimumDistanceToNeighborsInference = {
  id: 's.stdio.gltf.inference.minimum-distance-to-neighbors.v1',
  algorithmVersion: 1,
  cacheKey: 's.stdio.gltf.inference.minimum-distance-to-neighbors.v1:geometry-v2',
  reads: ['document/scene', 'document/scenes', 'document/nodes', 'document/meshes', 'document/accessors', 'document/bufferViews', 'document/buffers', 'buffers'],
} as const;
export type GltfMinimumDistanceToNeighborsInference = typeof gltfMinimumDistanceToNeighborsInference;

