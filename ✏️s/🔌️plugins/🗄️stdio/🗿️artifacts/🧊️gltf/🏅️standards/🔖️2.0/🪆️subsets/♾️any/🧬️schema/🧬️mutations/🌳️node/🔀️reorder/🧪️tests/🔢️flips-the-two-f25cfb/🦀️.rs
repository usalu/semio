//! 🧪️ `reorder-nodes` fixture — `🔢️flips-the-two-nodes-and-renumbers-the-scene-roots`.
//!
...
use crate::schema::mutations::reorder_nodes::diff::GltfReorderNodesDiff;
use crate::schema::mutations::reorder_nodes::GltfReorderNodesPayload;
use crate::schema::mutations::reorder_nodes::{diff, inverse, mutation};
use crate::GltfSnapshot;

const CASE: &str = "reorder-nodes/flips-the-two-nodes-and-renumbers-the-scene-roots";
const BEFORE: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🌳️node/🔀️reorder/🔢️flips-the-two-f25cfb/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🌳️node/🔀️reorder/🔢️flips-the-two-f25cfb/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🌳️node/🔀️reorder/🔢️flips-the-two-f25cfb/🧬️operation/🔣️.json");
const DIFF: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🌳️node/🔀️reorder/🔢️flips-the-two-f25cfb/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🌳️node/🔀️reorder/🔢️flips-the-two-f25cfb/🎯️outcome/🔣️.json");
...
async fn committed_diff_is_canonical() {
    let decoded: GltfReorderNodesDiff = dsl::json::from_json_str(DIFF).expect("committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("diff re-encodes");
    let original: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff reparses");
    assert_eq!(reencoded, original, "{CASE}: committed diff JSON is not canonical");
    assert!(matches!(&decoded.operation, crate::schema::mutations::reorder_nodes::diff::GltfReorderNodesOperation::Reorder { order } if order == &vec![1usize, 0]), "{CASE}: the committed operation must be a reorder carrying the permutation");
}
