...
pub fn validate(payload: &GltfReorderPrimitivesPayload, base: &GltfSnapshot) -> Result<(), GltfTopLevelMutationRejection> { checked_index(payload.mesh, base.document.meshes.len(), "document/meshes")?; let length = base.document.meshes[payload.mesh].primitives.len(); if payload.order.len() != length || payload.order.iter().any(|index| *index >= length) || { let mut order = payload.order.clone(); order.sort_unstable(); order.dedup(); order.len() != length } { return Err(reject("gltf.mutation.invalid-permutation", "document/meshes/primitives", "order must contain each primitive once")); }
    if payload.order.iter().enumerate().all(|(index, value)| *value == index) { return Err(reject("gltf.mutation.no-observable-change", "document/meshes/primitives", "reorder must change order")); } Ok(()) }
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
