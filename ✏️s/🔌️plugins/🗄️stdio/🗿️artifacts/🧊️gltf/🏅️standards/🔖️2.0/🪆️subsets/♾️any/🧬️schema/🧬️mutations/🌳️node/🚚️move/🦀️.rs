...
pub fn validate(payload: &GltfMoveNodePayload, base: &GltfSnapshot) -> Result<(), GltfTopLevelMutationRejection> { if payload.index >= base.document.nodes.len() || payload.position >= base.document.nodes.len() { return Err(reject("gltf.mutation.index-out-of-range", "document/nodes", "indices must address items")); }
    if payload.index == payload.position { return Err(reject("gltf.mutation.no-observable-change", "document/nodes", "destination equals source")); }  Ok(()) }
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
...
impl protocol::MutationKind<GltfSnapshot, super::GltfMutation> for MoveNodeMutation {
    ...
    fn inverse(&self, base: &GltfSnapshot) -> Vec<super::GltfMutation> {
        ...
        vec![super::GltfMutation::MoveNode(Self::Restore(Box::new(inverse)))]
    }
