//! 🧪️ `move-node-parent` fixture — `adopts-the-second-scene-root-under-the-first-node`.
//!
...
use crate::artifacts::gltf::schema::mutations::move_node_parent::GltfReparentNodePayload;
use crate::artifacts::gltf::schema::mutations::move_node_parent::{diff, inverse, mutation};
use crate::artifacts::gltf::GltfSnapshot;

const CASE: &str = "move-node-parent/adopts-the-second-scene-root-under-the-first-node";
...
fn payload() -> GltfReparentNodePayload {
    serde_json::from_str(MUTATION).expect("move-node-parent payload decodes")
}

/// ▶️ `move-node-parent` detaches the child from the scene root list and re-attaches it under the parent.
#[semio_framework_async_macros::async_test]
async fn applies_to_committed_after() {
    let snapshot = mutation::apply(&payload(), &before()).expect("move-node-parent applies to its committed before-snapshot");
...
async fn inverse_restores_before() {
    let base = before();
    let inverse = inverse::derive(&payload(), &base).expect("move-node-parent inverse derives from the exact base");
...
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => {
            let snapshot = attempt.expect("move-node-parent declared applied");
        ...
        "rejected" => {
            ...
            assert_eq!(attempt.expect_err("move-node-parent declared rejected").code, code, "{CASE}: rejection code differs from the committed outcome");
        }
...
async fn produces_committed_diff() {
    let produced = diff::derive(&payload(), &before()).expect("move-node-parent derives its diff");
    ...
    assert!(produced.scenes.is_some() && produced.nodes.is_some(), "{CASE}: move-node-parent must declare both the scenes and the nodes slot");
    assert!(produced.meshes.is_none() && produced.accessors.is_none(), "{CASE}: move-node-parent must write nothing beyond scenes and nodes");
}
