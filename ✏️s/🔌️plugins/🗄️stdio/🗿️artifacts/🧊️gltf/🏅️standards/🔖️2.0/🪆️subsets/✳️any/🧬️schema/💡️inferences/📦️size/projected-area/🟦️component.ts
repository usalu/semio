...
export const inferGltfProjectedArea = (context: GltfTsGeometryContext): GltfTsMeasure<number> => { const value = (()=>{const d=bounds(context.points)?.dimensions;return d?d[0]*d[1]:undefined})(); return context.valid && value !== undefined ? exact(context, value, 'square-metre') : unavailable(context, 'square-metre'); };
export const unavailableGltfProjectedArea = (context: GltfTsGeometryContext): GltfTsMeasure<number> => unavailable(context, 'square-metre');
