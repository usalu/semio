//! 🧪️ `move-sampler` fixture — `swaps-the-linear-and-clamped-samplers`.
//!
//! `repair(Samplers, Move(0, 1))` rewrites `texture.sampler` on every texture. The committed snapshots
...
fn payload() -> GltfMoveSamplerPayload {
    serde_json::from_str(MUTATION).expect("move-sampler payload decodes")
}

/// ▶️ `move-sampler` swaps the two samplers and rewrites both texture bindings.
#[semio_framework_async_macros::async_test]
async fn applies_to_committed_after() {
    let snapshot = mutation::apply(&payload(), &before()).expect("move-sampler applies to its committed before-snapshot");
...
/// ↩️ The inverse swaps them back so the linear sampler leads again.
#[semio_framework_async_macros::async_test]
async fn inverse_restores_before() {
    let base = before();
    let inverse = inverse::derive(&payload(), &base).expect("move-sampler inverse derives from the exact base");
...
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => {
            let snapshot = attempt.expect("move-sampler declared applied");
        ...
        "rejected" => {
            ...
            assert_eq!(attempt.expect_err("move-sampler declared rejected").code, code, "{CASE}: rejection code differs from the committed outcome");
        }
        other => panic!("{CASE}: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The delta is two indices; neither sampler body appears in it.
#[semio_framework_async_macros::async_test]
async fn produces_committed_diff() {
    let produced = diff::derive(&payload(), &before()).expect("move-sampler derives its diff");
...
async fn committed_diff_is_canonical() {
    let decoded: GltfMoveSamplerDiff = serde_json::from_str(DIFF).expect("committed diff decodes");
