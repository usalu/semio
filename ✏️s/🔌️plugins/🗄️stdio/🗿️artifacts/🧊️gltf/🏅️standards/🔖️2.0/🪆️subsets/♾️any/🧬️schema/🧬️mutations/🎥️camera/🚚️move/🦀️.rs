...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn apply(payload: &GltfMoveCameraPayload, base: &GltfSnapshot) -> Result<GltfSnapshot, GltfTopLevelMutationRejection> {
    validate(payload, base)?;
    let mut next = base.clone();
    cameras_op(&mut next, GltfTopLevelFamily::Cameras, payload.index, Some(payload.position), None)?;
    Ok(next)
}
...
impl protocol::MutationKind<GltfSnapshot, super::GltfMutation> for MoveCameraMutation {
    ...
    fn label(&self) -> protocol::LocalizedLabel {
        protocol::LocalizedLabel::native("Move Camera", "Kamera verschieben")
    }
