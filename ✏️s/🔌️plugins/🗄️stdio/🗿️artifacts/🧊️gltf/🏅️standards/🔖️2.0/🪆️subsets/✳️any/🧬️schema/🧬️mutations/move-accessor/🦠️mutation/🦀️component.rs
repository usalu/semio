...
pub async fn validate(payload: &GltfMoveAccessorPayload, base: &GltfSnapshot) -> Result<(), GltfTopLevelMutationRejection> { if payload.index >= base.document.accessors.len() || payload.position >= base.document.accessors.len() { return Err(reject("gltf.mutation.index-out-of-range", "document/accessors", "indices must address items")); } if payload.index == payload.position { return Err(reject("gltf.mutation.no-observable-change", "document/accessors", "destination equals source")); }  Ok(()) }
pub async fn apply(payload: &GltfMoveAccessorPayload, base: &GltfSnapshot) -> Result<GltfSnapshot, GltfTopLevelMutationRejection> { validate(payload, base)?; let mut next = base.clone(); accessors_op(&mut next, GltfTopLevelFamily::Accessors, payload.index, Some(payload.position), None)?;  Ok(next) }
