...
pub fn descriptor() -> GltfInferenceLeafDescriptor { GltfExposedAreaInference::DESCRIPTOR }
...
pub fn from_assembly(surface_area: &GltfMeasure<f64>, part_count: usize, contact_area: f64, complete: bool, sample_count: usize, topology: Topology) -> GltfMeasure<f64> {
    if part_count <= 1 { return surface_area.clone(); }
    if complete { return estimate((surface_area.value.unwrap_or(0.0) - 2.0 * contact_area).max(0.0), GltfUnit::SquareMetre, sample_count, Some(topology)); }
...
pub fn encode_result(indicators: &GltfEntityIndicators) -> Result<serde_json::Value, serde_json::Error> {
    serde_json::to_value(&indicators.area_volume.exposedArea)
}
