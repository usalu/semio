...
//#region 🔖️Manifest
pub async fn create_gltf_any_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(GLTF_ANY_DIALECT)
        .await.document(["stdio", "gltf"])
        .await.icon_id("box")
        .await.mode_def(edit::definition().await)
        .await.default_mode_id(edit::GLTF_ANY_EDIT_MODE_ID)
