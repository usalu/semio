...
pub fn derive(payload: &GltfChangeDocumentExtensionDataPayload, base: &GltfSnapshot) -> Result<GltfDiff, GltfTopLevelMutationRejection> { let _ = apply(payload, base)?; Ok(GltfDiff { extensions: Some(payload.data.clone()), ..Default::default() }) }
