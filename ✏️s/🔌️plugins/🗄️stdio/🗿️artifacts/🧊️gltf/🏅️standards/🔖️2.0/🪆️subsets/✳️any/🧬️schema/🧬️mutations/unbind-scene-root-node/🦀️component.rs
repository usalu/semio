//! 🧬️ Executable descriptor adapter for unbind-scene-root-node.
...
async fn plan(bytes: &[u8], base: &GltfSnapshot) -> Result<GltfMutationLeafPlan, GltfMutationLeafError> {
    let payload: mutation::GltfUnbindSceneRootNodePayload = decode(bytes).await?;
    let diff = diff::derive(&payload, base).await.map_err(rejection)?;
    let inverse = inverse::derive(&payload, base).await.map_err(rejection)?;
    Ok(GltfMutationLeafPlan { diff_payload: encode(&diff).await?, inverse_payload: encode(&inverse).await?, touched_paths: vec![path(diff.scene, diff.position).await] })
}
async fn plan_inverse(bytes: &[u8], base: &GltfSnapshot) -> Result<GltfMutationLeafPlan, GltfMutationLeafError> {
    let inverse: inverse::GltfUnbindSceneRootNodeInverse = decode(bytes).await?;
    let _ = inverse::apply(base, &inverse).await.map_err(rejection)?;
    Ok(GltfMutationLeafPlan { diff_payload: encode(&inverse).await?, inverse_payload: Vec::new(), touched_paths: vec![path(inverse.scene, inverse.position).await] })
}
async fn apply_diff(bytes: &[u8], base: &GltfSnapshot) -> Result<GltfMutationLeafApplication, GltfMutationLeafError> {
    let diff: diff::GltfUnbindSceneRootNodeDiff = decode(bytes).await?;
