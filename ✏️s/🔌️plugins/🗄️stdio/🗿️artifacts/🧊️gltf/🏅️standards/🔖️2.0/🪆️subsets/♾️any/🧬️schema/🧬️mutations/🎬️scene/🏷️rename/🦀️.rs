//! 🧬️ Direct change-scene-name mutation owner: payload, validation, typed diff, inverse, and outcomes.
...
pub fn validate(payload: &GltfChangeSceneNamePayload, base: &GltfSnapshot) -> Result<(), GltfTopLevelMutationRejection> { checked_index(payload.scene, base.document.scenes.len(), "document/scenes")?; Ok(()) }
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn apply(payload: &GltfChangeSceneNamePayload, base: &GltfSnapshot) -> Result<GltfSnapshot, GltfTopLevelMutationRejection> { validate(payload, base)?; let mut next = base.clone(); next.document.scenes[payload.scene].name = payload.value.clone(); Ok(next) }
...
impl protocol::MutationKind<GltfSnapshot, super::GltfMutation> for ChangeSceneNameMutation {
    const SEMANTICS: protocol::SemanticDescriptor = protocol::SemanticDescriptor { verb: "change", entity: "scene-name", kind: "change-scene-name", record: "ChangedSceneName" };
    ...
    fn inverse(&self, base: &GltfSnapshot) -> Vec<super::GltfMutation> {
        ...
        vec![super::GltfMutation::ChangeSceneName(Self::Restore(inverse))]
    }

    fn label(&self) -> String {
        "Change Scene Name".to_string()
    }

    fn target(&self) -> Vec<String> {
        vec!["change-scene-name".to_string()]
    }
}
