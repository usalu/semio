...
fn rejection(value: mutation::GltfChangeMaterialAlphaModeRejection) -> GltfMutationLeafError {
...
fn plan(payload: &[u8], base: &GltfSnapshot) -> Result<GltfMutationLeafPlan, GltfMutationLeafError> {
    let payload = decode::<mutation::GltfChangeMaterialAlphaModePayload>(payload)?;
...
fn plan_inverse(payload: &[u8], base: &GltfSnapshot) -> Result<GltfMutationLeafPlan, GltfMutationLeafError> {
    let inverse = decode::<inverse::GltfChangeMaterialAlphaModeInverse>(payload)?;
...
fn apply_diff(payload: &[u8], base: &GltfSnapshot) -> Result<GltfMutationLeafApplication, GltfMutationLeafError> {
    let diff = decode::<diff::GltfChangeMaterialAlphaModeDiff>(payload)?;
