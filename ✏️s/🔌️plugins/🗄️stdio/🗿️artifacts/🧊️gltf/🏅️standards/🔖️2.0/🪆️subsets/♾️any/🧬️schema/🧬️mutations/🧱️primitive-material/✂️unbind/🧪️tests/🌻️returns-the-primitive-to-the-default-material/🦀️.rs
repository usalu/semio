//! 🧪️ `unbind-primitive-material` fixture — `🌻️returns-the-primitive-to-the-default-material`.
//!
//! `validate()` refuses an unbound primitive with `gltf.mutation.relation-absent`. Clearing
...
use crate::artifacts::gltf::schema::mutations::unbind_primitive_material::GltfUnbindPrimitiveMaterialPayload;
use crate::artifacts::gltf::schema::mutations::unbind_primitive_material::{diff, inverse, mutation};
...
fn payload() -> GltfUnbindPrimitiveMaterialPayload {
    serde_json::from_str(MUTATION).expect("unbind-primitive-material payload decodes")
}

/// ▶️ `unbind-primitive-material` clears the binding and leaves the material itself in place.
#[semio_framework_async_macros::async_test]
async fn applies_to_committed_after() {
    let snapshot = mutation::apply(&payload(), &before()).expect("unbind-primitive-material applies to its committed before-snapshot");
...
/// ↩️ The inverse re-binds the steel material.
#[semio_framework_async_macros::async_test]
async fn inverse_restores_before() {
    let base = before();
    let inverse = inverse::derive(&payload(), &base).expect("unbind-primitive-material inverse derives from the exact base");
...
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => {
            let snapshot = attempt.expect("unbind-primitive-material declared applied");
        ...
        "rejected" => {
            ...
            assert_eq!(attempt.expect_err("unbind-primitive-material declared rejected").code, code, "{CASE}: rejection code differs from the committed outcome");
        }
        other => panic!("{CASE}: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The delta replaces the primitive list with the `material` key absent.
#[semio_framework_async_macros::async_test]
async fn produces_committed_diff() {
    let produced = diff::derive(&payload(), &before()).expect("unbind-primitive-material derives its diff");
