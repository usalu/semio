//! 🧪️ `create-mesh` fixture — `🕸️inserts-an-empty-mesh-ahead-of-the-hull-mesh`.
//!
//! `operation()` inserts `GltfMesh::default()` — no primitives, no morph weights, no name.
...
/// ▶️ `create-mesh` inserts an empty mesh and renumbers the node that referenced mesh 0.
#[semio_framework_async_macros::async_test]
async fn applies_to_committed_after() {
    let snapshot = mutation::apply(&payload(), &before()).expect("create-mesh applies to its committed before-snapshot");
...
/// ↩️ The inverse deletes the empty mesh and puts the node back on 'hull'.
#[semio_framework_async_macros::async_test]
async fn inverse_restores_before() {
    let base = before();
    let inverse = inverse::derive(&payload(), &base).expect("create-mesh inverse derives from the exact base");
...
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => {
            let snapshot = attempt.expect("create-mesh declared applied");
        ...
        "rejected" => {
            ...
            assert_eq!(attempt.expect_err("create-mesh declared rejected").code, code, "{CASE}: rejection code differs from the committed outcome");
        }
        other => panic!("{CASE}: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The delta is one `insert` into `document/meshes` and names exactly one slot.
#[semio_framework_async_macros::async_test]
async fn produces_committed_diff() {
    let produced = diff::derive(&payload(), &before()).expect("create-mesh derives its diff");
