//! 🧪️ `reorder-cameras` fixture — `🔀️flips-the-perspective-and-orthographic-cameras`.
//!
//! `repair(Cameras, Reorder([1, 0]))` remaps `node.camera` on every node. Both nodes here hold a
...
const BEFORE: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🎥️camera/🔀️reorder/🔀️flips-the-0609c1/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🎥️camera/🔀️reorder/🔀️flips-the-0609c1/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🎥️camera/🔀️reorder/🔀️flips-the-0609c1/🧬️operation/🔣️.json");
const DIFF: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🎥️camera/🔀️reorder/🔀️flips-the-0609c1/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🎥️camera/🔀️reorder/🔀️flips-the-0609c1/🎯️outcome/🔣️.json");
...
fn payload() -> GltfReorderCamerasPayload {
    serde_json::from_str(MUTATION).expect("reorder-cameras payload decodes")
}

/// ▶️ `reorder-cameras` permutes the camera list and remaps both node bindings.
...
/// ↩️ Re-applying the same flip restores the original camera order.
#[semio_framework_async_macros::async_test]
async fn inverse_restores_before() {
    let base = before();
    let inverse = inverse::derive(&payload(), &base).expect("reorder-cameras inverse derives from the exact base");
...
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => {
            let snapshot = attempt.expect("reorder-cameras declared applied");
        ...
        "rejected" => {
            ...
            assert_eq!(attempt.expect_err("reorder-cameras declared rejected").code, code, "{CASE}: rejection code differs from the committed outcome");
        }
        other => panic!("{CASE}: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The delta is the permutation; neither camera body appears in it.
#[semio_framework_async_macros::async_test]
async fn produces_committed_diff() {
    let produced = diff::derive(&payload(), &before()).expect("reorder-cameras derives its diff");
...
async fn committed_diff_is_canonical() {
    let decoded: GltfReorderCamerasDiff = dsl::json::from_json_str(DIFF).expect("committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("diff re-encodes");
    let original: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff reparses");
    assert_eq!(reencoded, original, "{CASE}: committed diff JSON is not canonical");
    assert_eq!(decoded.id, "s.stdio.gltf.mutation.reorder-cameras.v1", "{CASE}: the committed diff must carry reorder-cameras' own canonical descriptor id");
}
