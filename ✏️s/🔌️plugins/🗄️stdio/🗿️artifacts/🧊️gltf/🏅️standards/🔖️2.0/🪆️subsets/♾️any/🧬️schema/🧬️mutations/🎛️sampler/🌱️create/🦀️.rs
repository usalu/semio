...
pub fn validate(payload: &GltfCreateSamplerPayload, base: &GltfSnapshot) -> Result<(), GltfTopLevelMutationRejection> {
    if payload.position > base.document.samplers.len() {
        return Err(reject("gltf.mutation.insert-out-of-range", "document/samplers", "position must be within the collection"));
    }
    Ok(())
}
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
