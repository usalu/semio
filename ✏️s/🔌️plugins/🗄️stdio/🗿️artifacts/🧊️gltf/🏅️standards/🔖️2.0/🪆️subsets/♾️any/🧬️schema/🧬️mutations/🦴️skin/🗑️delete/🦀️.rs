...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn apply(payload: &GltfDeleteSkinPayload, base: &GltfSnapshot) -> Result<GltfSnapshot, GltfTopLevelMutationRejection> {
    validate(payload, base)?;
    let mut next = base.clone();
    skins_op(&mut next, GltfTopLevelFamily::Skins, payload.index, None, None)?;
    Ok(next)
}
...
impl protocol::MutationKind<GltfSnapshot, super::GltfMutation> for DeleteSkinMutation {
    ...
    fn label(&self) -> protocol::LocalizedLabel {
        protocol::LocalizedLabel::native("Delete Skin", "Skin löschen")
    }
