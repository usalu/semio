...
mod tests {
    ...
    struct Rejected {
        index: mutation::GltfUnbindNodeChildPayload,
        reference: mutation::GltfUnbindNodeChildPayload,
    }
    #[derive(value_derive::FromValue)]
    #[value(rename_all = "camelCase")]
    struct Wire {
    ...
    async fn canonical_vector_enforces_forward_inverse_reference_stale_and_wire_laws() {
        ...
        assert!(serde_json::from_str::<mutation::GltfUnbindNodeChildPayload>(&vector.wire.malformed_payload).is_err());
