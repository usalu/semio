//! 🧪️ `move-required-extension` fixture — `⬇️demotes-the-unlit-requirement-behind-the-transform-requirement`.
//!
//! The `extensionsRequired` twin of `move-used-extension`, with the same by-name addressing and the same
...
const BEFORE: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/✅️required-extension/🚚️move/🔬️t043/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/✅️required-extension/🚚️move/🔬️t043/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/✅️required-extension/🚚️move/🔬️t043/🧬️operation/🔣️.json");
const DIFF: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/✅️required-extension/🚚️move/🔬️t043/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/✅️required-extension/🚚️move/🔬️t043/🎯️outcome/🔣️.json");
...
fn payload() -> GltfMoveRequiredExtensionPayload {
    serde_json::from_str(MUTATION).expect("move-required-extension payload decodes")
}

/// ▶️ `move-required-extension` rotates the requirement list and leaves the declaration list alone.
#[semio_framework_async_macros::async_test]
async fn applies_to_committed_after() {
    let snapshot = mutation::apply(&payload(), &before()).expect("move-required-extension applies to its committed before-snapshot");
...
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => {
            let snapshot = attempt.expect("move-required-extension declared applied");
        ...
        "rejected" => {
            ...
            assert_eq!(attempt.expect_err("move-required-extension declared rejected").code, code, "{CASE}: rejection code differs from the committed outcome");
        }
        other => panic!("{CASE}: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The delta is the whole reordered requirement list.
#[semio_framework_async_macros::async_test]
async fn produces_committed_diff() {
    let produced = diff::derive(&payload(), &before()).expect("move-required-extension derives its diff");
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(decoded.extensions_required.as_ref().and_then(|required| required.last().cloned()), Some("KHR_materials_unlit".to_string()), "{CASE}: the committed list must end with the moved requirement");
}
