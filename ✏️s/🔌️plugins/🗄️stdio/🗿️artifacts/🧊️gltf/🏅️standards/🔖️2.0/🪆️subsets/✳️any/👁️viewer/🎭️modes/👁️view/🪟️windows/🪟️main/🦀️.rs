//! 🧊 glTF viewer — the Main window: a read-only world-3d mesh scene built with the shared
//! `MeshWindowKit` (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.6), from the
//! artifact-level `GltfSnapshot` — this file imports nothing from the sibling editor module
...
const GLTF_ANY_VIEW_FALLBACK_MESH_KIND: &str = "box";
const GLTF_ANY_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const GLTF_ANY_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const GLTF_ANY_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
...
fn world_instances_json(document: &GltfSnapshot) -> String {
    ...
    let instances: Vec<pack::JsonValue> = (0..count)
        .map(|index| {
            pack::json!({
                "id": format!("gltf_any-{index}"),
                "meshId": GLTF_ANY_VIEW_FALLBACK_MESH_KIND,
                "position": [index as f64 * 2.0, 0.0, 0.0],
                "rotation": [0.0, 0.0, 0.0, 1.0],
                "scale": [1.0, 1.0, 1.0],
                "label": format!("glTF {index}"),
                "smoothShading": false,
            })
        })
        .collect();
...
/// 👁️ Pure `GltfSnapshot -> BuiltNode` read: default camera (a viewer has no persisted
/// per-session camera — `Config = NoConfig`), no selection/gumball/engagement overlay.
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn render(document: &GltfSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = world3d_meshes_json_from_kinds(&[GLTF_ANY_VIEW_FALLBACK_MESH_KIND.to_string()]);
    let view = MeshView {
