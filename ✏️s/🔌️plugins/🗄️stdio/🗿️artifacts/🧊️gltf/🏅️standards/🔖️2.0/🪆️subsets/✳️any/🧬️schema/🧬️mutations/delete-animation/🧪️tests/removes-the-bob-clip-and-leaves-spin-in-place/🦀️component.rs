//! 🧪️ `delete-animation` fixture — `removes-the-bob-clip-and-leaves-spin-in-place`.
//!
//! Animations are the one family with an empty `repair` arm: nothing in a glTF document holds an index
...
use crate::artifacts::gltf::schema::mutations::delete_animation::mutation::GltfDeleteAnimationPayload;
...
fn payload() -> GltfDeleteAnimationPayload {
    serde_json::from_str(MUTATION).expect("delete-animation payload decodes")
}

/// ▶️ `delete-animation` removes only the addressed clip and cascades nowhere.
#[semio_framework_async_macros::async_test]
async fn applies_to_committed_after() {
    let snapshot = mutation::apply(&payload(), &before()).expect("delete-animation applies to its committed before-snapshot");
...
/// ↩️ The inverse re-appends the recorded 'bob' clip at index 1.
#[semio_framework_async_macros::async_test]
async fn inverse_restores_before() {
    let base = before();
    let inverse = inverse::derive(&payload(), &base).expect("delete-animation inverse derives from the exact base");
...
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => {
            let snapshot = attempt.expect("delete-animation declared applied");
        ...
        "rejected" => {
            ...
            assert_eq!(attempt.expect_err("delete-animation declared rejected").code, code, "{CASE}: rejection code differs from the committed outcome");
        }
        other => panic!("{CASE}: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The delta records the removed clip in full — the only thing that makes deletion reversible.
#[semio_framework_async_macros::async_test]
async fn produces_committed_diff() {
    let produced = diff::derive(&payload(), &before()).expect("delete-animation derives its diff");
...
async fn committed_diff_is_canonical() {
    let decoded: GltfDeleteAnimationDiff = serde_json::from_str(DIFF).expect("committed diff decodes");
    let reencoded = serde_json::to_value(&decoded).expect("diff re-encodes");
    let original: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff reparses");
    assert_eq!(reencoded, original, "{CASE}: committed diff JSON is not canonical");
    assert_eq!(decoded.payload.index, 1, "{CASE}: the committed diff must echo the deletion index");
}
