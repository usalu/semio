//! 🧪️ `declare-used-extension` fixture — `declares-the-unlit-extension-ahead-of-the-transform-extension`.
//!
...
use crate::artifacts::gltf::schema::mutations::declare_used_extension::mutation::GltfDeclareUsedExtensionPayload;
use crate::artifacts::gltf::schema::mutations::declare_used_extension::{diff, inverse, mutation};
use crate::artifacts::gltf::GltfSnapshot;

const CASE: &str = "declare-used-extension/declares-the-unlit-extension-ahead-of-the-transform-extension";
...
fn payload() -> GltfDeclareUsedExtensionPayload {
    serde_json::from_str(MUTATION).expect("declare-used-extension payload decodes")
}

/// ▶️ `declare-used-extension` inserts the name at the requested position and requires nothing.
#[semio_framework_async_macros::async_test]
async fn applies_to_committed_after() {
    let snapshot = mutation::apply(&payload(), &before()).expect("declare-used-extension applies to its committed before-snapshot");
...
async fn inverse_restores_before() {
    let base = before();
    let inverse = inverse::derive(&payload(), &base).expect("declare-used-extension inverse derives from the exact base");
...
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => {
            let snapshot = attempt.expect("declare-used-extension declared applied");
        ...
        "rejected" => {
            ...
            assert_eq!(attempt.expect_err("declare-used-extension declared rejected").code, code, "{CASE}: rejection code differs from the committed outcome");
        }
...
async fn produces_committed_diff() {
    let produced = diff::derive(&payload(), &before()).expect("declare-used-extension derives its diff");
