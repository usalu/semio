...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn apply(payload: &GltfMoveRequiredExtensionPayload, base: &GltfSnapshot) -> Result<GltfSnapshot, GltfTopLevelMutationRejection> {
    validate(payload, base)?;
    let mut next = base.clone();
    let value = next.document.extensions_required.remove(next.document.extensions_required.iter().position(|value| value == &payload.extension).unwrap());
    next.document.extensions_required.insert(payload.position, value);
    Ok(next)
}
