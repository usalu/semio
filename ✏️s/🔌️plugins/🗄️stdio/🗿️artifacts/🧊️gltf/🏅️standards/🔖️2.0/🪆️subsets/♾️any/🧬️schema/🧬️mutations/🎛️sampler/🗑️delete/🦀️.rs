...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn apply(payload: &GltfDeleteSamplerPayload, base: &GltfSnapshot) -> Result<GltfSnapshot, GltfTopLevelMutationRejection> {
    validate(payload, base)?;
    let mut next = base.clone();
    samplers_op(&mut next, GltfTopLevelFamily::Samplers, payload.index, None, None)?;
    Ok(next)
}
...
impl protocol::MutationKind<GltfSnapshot, super::GltfMutation> for DeleteSamplerMutation {
    ...
    fn label(&self) -> protocol::LocalizedLabel {
        protocol::LocalizedLabel::native("Delete Sampler", "Sampler löschen")
    }
