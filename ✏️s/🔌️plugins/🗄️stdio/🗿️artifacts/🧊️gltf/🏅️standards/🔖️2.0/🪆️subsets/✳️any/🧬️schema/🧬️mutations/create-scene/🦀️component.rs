...
fn rejection(value: GltfCreateSceneRejection) -> GltfMutationLeafError {
    GltfMutationLeafError { code: value.code, path: value.path, detail: value.detail }
}

fn decode<T: DeserializeOwned>(payload: &[u8], path: &str) -> Result<T, GltfMutationLeafError> {
...
fn encode<T: Serialize>(value: &T, path: &str) -> Result<Vec<u8>, GltfMutationLeafError> {
