...
Feature: Apply every registered glTF 2.0 camera mutation to a real-world document
  ...
  directory (`../../../✳️any/🧬️schema/🧬️mutations/{🌱️🎥️create-camera,🗑️🎥️delete-camera,
  🚚️🎥️move-camera,🔀️🎥️reorder-cameras}/🦀️.rs`) stays physically owned by `✳️any`'s aggregate
  mutation root — `validate_mutation_leaf_source` requires a leaf's `owner` to be an immediate child
  of its aggregate root, so ownership here is declared through this catalog and the manifest's
  ...
  Scenario Outline: Apply <id> to the real document
    ...
    Examples:
      | id               | params                                                                       |
      | create-camera    | {"position":1,"projection":{"type":"perspective","perspective":{"yfov":1,"znear":0.1}}} |
      | delete-camera    | {"index":0}                                                                  |
      | move-camera      | {"index":1,"position":0}                                                     |
      | reorder-cameras  | {"order":[1,0]}                                                              |
