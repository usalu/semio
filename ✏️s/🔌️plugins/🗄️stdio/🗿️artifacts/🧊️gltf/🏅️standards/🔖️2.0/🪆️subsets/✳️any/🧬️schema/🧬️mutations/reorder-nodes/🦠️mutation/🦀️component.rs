...
pub async fn validate(payload: &GltfReorderNodesPayload, base: &GltfSnapshot) -> Result<(), GltfTopLevelMutationRejection> { if payload.order.len() != base.document.nodes.len() || payload.order.iter().collect::<std::collections::BTreeSet<_>>().len() != payload.order.len() || payload.order.iter().any(|index| *index >= base.document.nodes.len()) { return Err(reject("gltf.mutation.invalid-permutation", "document/nodes", "order must contain every index once")); } if payload.order.iter().enumerate().all(|(index, value)| index == *value) { return Err(reject("gltf.mutation.no-observable-change", "document/nodes", "order already matches")); }  Ok(()) }
pub async fn apply(payload: &GltfReorderNodesPayload, base: &GltfSnapshot) -> Result<GltfSnapshot, GltfTopLevelMutationRejection> { validate(payload, base)?; let mut next = base.clone(); nodes_op(&mut next, GltfTopLevelFamily::Nodes, payload.order[0], None, Some(&payload.order))?;  Ok(next) }
