...
pub async fn derive(payload: &GltfUnbindDefaultScenePayload, base: &GltfSnapshot) -> Result<GltfDiff, GltfTopLevelMutationRejection> { let _ = apply(payload, base)?; Ok(GltfDiff { scene: Some(None), ..Default::default() }) }
