...
pub fn validate(payload: &GltfReorderSkinsPayload, base: &GltfSnapshot) -> Result<(), GltfTopLevelMutationRejection> { if payload.order.len() != base.document.skins.len() || payload.order.iter().collect::<std::collections::BTreeSet<_>>().len() != payload.order.len() || payload.order.iter().any(|index| *index >= base.document.skins.len()) { return Err(reject("gltf.mutation.invalid-permutation", "document/skins", "order must contain every index once")); }
    if payload.order.iter().enumerate().all(|(index, value)| index == *value) { return Err(reject("gltf.mutation.no-observable-change", "document/skins", "order already matches")); }  Ok(()) }
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
...
impl protocol::MutationKind<GltfSnapshot, super::GltfMutation> for ReorderSkinsMutation {
    ...
    fn inverse(&self, base: &GltfSnapshot) -> Vec<super::GltfMutation> {
        ...
        vec![super::GltfMutation::ReorderSkins(Self::Restore(Box::new(inverse)))]
    }
