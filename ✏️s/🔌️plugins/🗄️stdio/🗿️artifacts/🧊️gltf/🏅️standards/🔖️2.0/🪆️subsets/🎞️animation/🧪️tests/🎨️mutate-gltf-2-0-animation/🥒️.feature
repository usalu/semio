...
Feature: Apply every registered glTF 2.0 animation mutation to a real-world document
  ...
  the same `gltf-2-0-any-reader-oracle` base document the artifact-root `🐞️mutate-gltf-2-0` case uses —
  this case claims it. Every real leaf directory (`../../../♾️any/🧬️schema/🧬️mutations/
  🎞️animation/{🌱️create,🗑️delete,🚚️move,🔀️reorder}/🦀️.rs`) stays physically owned by `♾️any`'s
  aggregate mutation root — `validate_mutation_leaf_source` requires the exact registered
  domain/operation owner beneath its aggregate root, so ownership here is
  ...
  Scenario Outline: Apply <id> to the real document
    ...
    Examples:
      ...
      | delete-animation      | 🗑️delete-animation-applied    | {"index":0} |
      | move-animation        | 🚚️move-animation-applied      | {"index":1,"position":0} |
      | reorder-animations    | 🔀️reorder-animations-applied  | {"order":[1,0]} |
