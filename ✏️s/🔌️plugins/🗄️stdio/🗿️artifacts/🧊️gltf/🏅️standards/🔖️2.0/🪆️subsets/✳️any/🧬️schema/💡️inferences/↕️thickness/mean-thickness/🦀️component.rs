...
pub fn descriptor() -> GltfInferenceLeafDescriptor { GltfMeanThicknessInference::DESCRIPTOR }

pub fn infer(context: &GltfGeometryContext<'_>) -> GltfMeasure<f64> {
    let distribution = super::distribution(context); distribution.mean.map(|value| estimate(value, GltfUnit::Metre, super::samples(context).len(), Some(context.topology))).unwrap_or_else(|| unavailable(GltfUnit::Metre, context.unavailable_volume, Vec::new(), context.sample_count, Some(context.topology)))
}
...
pub fn encode_result(indicators: &GltfEntityIndicators) -> Result<serde_json::Value, serde_json::Error> {
    serde_json::to_value(&indicators.thickness.meanThickness)
}
...
mod tests {
    ...
    fn descriptor_is_versioned_and_cacheable() {
        assert_eq!(descriptor().id, "s.stdio.gltf.inference.mean-thickness.v1");
