...
pub fn apply(base: &GltfSnapshot, diff: &GltfBindNodeChildDiff) -> Result<GltfSnapshot, GltfTopLevelMutationRejection> {
    ...
    if base.document.nodes.get(diff.child).is_none() || diff.position > parent.children.len() || parent.children.contains(&diff.child) {
        return Err(reject("gltf.mutation.stale-diff", format!("document/nodes/{}/children/{}", diff.parent, diff.position), "parent, position, or child binding is stale"));
    }
    let mut pending = vec![diff.child];
    let mut seen = std::collections::BTreeSet::new();
    while let Some(node) = pending.pop() {
        if node == diff.parent {
            return Err(reject("gltf.mutation.node-cycle", format!("document/nodes/{}/children/{}", diff.parent, diff.position), "patch closes a cycle"));
        }
        if seen.insert(node) {
