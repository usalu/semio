...
pub fn derive(payload: &GltfTransformNodePayload, base: &GltfSnapshot) -> Result<GltfDiff, GltfTopLevelMutationRejection> { let next = apply(payload, base)?; Ok(GltfDiff { nodes: Some(GltfNodesDiff::between(&next.document.nodes, &base.document.nodes)), ..Default::default() }) }
