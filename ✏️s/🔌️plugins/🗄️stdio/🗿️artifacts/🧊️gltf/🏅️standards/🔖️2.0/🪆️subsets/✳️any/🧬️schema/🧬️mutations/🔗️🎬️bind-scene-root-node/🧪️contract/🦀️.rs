//! 🧪️ Rust executor for the shared bind-scene-root-node vectors.
#[cfg(test)]
mod tests {
    ...
    struct Rejected {
        index: mutation::GltfBindSceneRootNodePayload,
        reference: mutation::GltfBindSceneRootNodePayload,
    }
    #[derive(value_derive::FromValue)]
    #[value(rename_all = "camelCase")]
    struct Wire {
    ...
    struct Vector {
        base: GltfSnapshot,
        mutation: mutation::GltfBindSceneRootNodePayload,
        diff: diff::GltfBindSceneRootNodeDiff,
        inverse: inverse::GltfBindSceneRootNodeInverse,
        after: GltfSnapshot,
    ...
    async fn canonical_vector_enforces_forward_inverse_reference_stale_and_wire_laws() {
        ...
        assert!(serde_json::from_str::<mutation::GltfBindSceneRootNodePayload>(&vector.wire.malformed_payload).is_err());
