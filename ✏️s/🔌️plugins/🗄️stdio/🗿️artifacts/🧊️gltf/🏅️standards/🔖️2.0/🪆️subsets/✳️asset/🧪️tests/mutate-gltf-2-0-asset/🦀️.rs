...
//! {✅️🧩️add-required-extension,📣️🧩️add-used-extension,✏️📦️change-asset-descriptive-metadata,
//! ✏️📦️change-asset-extension-data,✏️📦️change-asset-extra-data,✏️📦️change-asset-version,
//! ✏️📄️change-document-extension-data,✏️📄️change-document-extra-data,
//! 🚚️🧩️move-required-extension,🚚️🧩️move-used-extension,🚫️🧩️remove-required-extension,
//! 🔙️🧩️remove-used-extension,🔀️🧩️reorder-required-extensions,🔀️🧩️reorder-used-extensions}/
//! 🦀️.rs`) stays physically owned by `✳️any` — `validate_mutation_leaf_source` requires a leaf's
//! `owner` to be an immediate child of its aggregate mutation root, so this case reaches it by
//! import, never by moving the directory. The oracle performs every kind by independent GLB/JSON-tree
