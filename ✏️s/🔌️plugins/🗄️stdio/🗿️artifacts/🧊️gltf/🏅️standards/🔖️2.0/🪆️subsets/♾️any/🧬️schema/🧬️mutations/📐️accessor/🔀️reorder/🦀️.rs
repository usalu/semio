//! 🧬️ Direct reorder-accessors mutation owner: payload, validation, typed diff, inverse, and outcomes.
...
pub struct GltfReorderAccessorsPayload { pub order: Vec<usize> }
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn validate(payload: &GltfReorderAccessorsPayload, base: &GltfSnapshot) -> Result<(), GltfTopLevelMutationRejection> { if payload.order.len() != base.document.accessors.len() || payload.order.iter().collect::<std::collections::BTreeSet<_>>().len() != payload.order.len() || payload.order.iter().any(|index| *index >= base.document.accessors.len()) { return Err(reject("gltf.mutation.invalid-permutation", "document/accessors", "order must contain every index once")); } if payload.order.iter().enumerate().all(|(index, value)| index == *value) { return Err(reject("gltf.mutation.no-observable-change", "document/accessors", "order already matches")); }  Ok(()) }
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn apply(payload: &GltfReorderAccessorsPayload, base: &GltfSnapshot) -> Result<GltfSnapshot, GltfTopLevelMutationRejection> { validate(payload, base)?; let mut next = base.clone(); accessors_op(&mut next, GltfTopLevelFamily::Accessors, payload.order[0], None, Some(&payload.order))?;  Ok(next) }
...
impl protocol::MutationKind<GltfSnapshot, super::GltfMutation> for ReorderAccessorsMutation {
    const SEMANTICS: protocol::SemanticDescriptor = protocol::SemanticDescriptor { verb: "reorder", entity: "accessors", kind: "reorder-accessors", record: "ReorderedAccessors" };
    ...
    fn inverse(&self, base: &GltfSnapshot) -> Vec<super::GltfMutation> {
        ...
        vec![super::GltfMutation::ReorderAccessors(Self::Restore(inverse))]
    }
    ...
    fn target(&self) -> Vec<String> {
        vec!["reorder-accessors".to_string()]
    }
}
