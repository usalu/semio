...
//! (`../../../✳️any/🧬️schema/🧬️mutations/{🌱️🎞️create-animation,🗑️🎞️delete-animation,
//! 🚚️🎞️move-animation,🔀️🎞️reorder-animations}/🦀️.rs`) stays physically owned by `✳️any` —
//! `validate_mutation_leaf_source` requires a leaf's `owner` to be an immediate child of its
//! aggregate mutation root, so this case reaches it by import, never by moving the directory. The
...
//! file's own doc comment and `undo_delete_animation`), same shape `✳️skin`'s adapter already
//! established for `delete-skin`.

use semio_repo_test_host::{Adapter, Context, Json, Outcome};
use semio_s_plugin_stdio_test_oracle::artifacts::gltf::standards::v2_0::subsets::any::{oracle_apply_mutation, project_gltf, undo_delete_animation};
use semio_s_plugin_stdio_test_oracle::law::{inverse_restores_within, mutation_is_observable_within};

//#region 🔖️Kinds
const KINDS: &[&str] = &["create-animation", "delete-animation", "move-animation", "reorder-animations"];
//#endregion 🔖️Kinds
...
/// `✳️animation`). Copies into the work directory; the committed fixture itself is never written to.
...
//#region 🔖️Inverse
/// ↩️ The semantically correct inverse spec for `create-animation`/`move-animation`/
/// `reorder-animations` against each kind's own committed fixture (`../../🧫️fixtures/
/// <kind>-applied/before.gltf`) — computed independently since the oracle role must not link the
/// subject crate. `delete-animation` has no entry here: its inverse is special-cased in both
/// `mutate_oracle`/`inverse_oracle` below and the subject module, via `undo_delete_animation` — see
/// the feature file's own doc comment for why.
fn inverse_spec(kind: &str) -> Json {
    match kind {
        "create-animation" => json_spec("delete-animation", json_obj(vec![("index", json_num(1.0))])),
        "move-animation" => json_spec("move-animation", json_obj(vec![("index", json_num(0.0)), ("position", json_num(1.0))])),
        "reorder-animations" => json_spec("reorder-animations", json_obj(vec![("order", json_arr(vec![1.0, 0.0]))])),
...
/// `delete-animation` is special-cased through `undo_delete_animation` (the original document's own
/// real content, not a same-shaped substitute) rather than the generic `inverse_spec` dispatch.
fn inverse_oracle(ctx: &Context) -> Result<Outcome, String> {
    let spec = ctx.doc_json()?;
    let kind = spec.str("kind");
    let input = mutable_input(ctx, &kind)?;
    let mutated = oracle_apply_mutation(&input, &spec)?;
    let restored = if kind == "delete-animation" { undo_delete_animation(&mutated, &input)? } else { oracle_apply_mutation(&mutated, &inverse_spec(&kind))? };
...
mod subject {
    ...
    use semio_s_plugin_stdio::artifacts::gltf::standards::v2_0::subsets::any::schema::mutations::{create_animation, delete_animation, move_animation, reorder_animations};
    ...
    fn undo_delete_animation(before: &GltfSnapshot, mutated: &GltfSnapshot) -> GltfSnapshot {
        ...
        restored.document.animations = before.document.animations.clone();
        restored
    }
    //#endregion 🔖️Dispatch
    ...
    pub fn inverse(ctx: &Context) -> Result<Outcome, String> {
        ...
        let restored = if kind == "delete-animation" {
            undo_delete_animation(&before, &mutated)
        } else {
