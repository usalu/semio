...
//! 🧬️mutations/{🌱️🎥️create-camera,🗑️🎥️delete-camera,🚚️🎥️move-camera,🔀️🎥️reorder-cameras}/
//! 🦀️.rs`) stays physically owned by `✳️any` — `validate_mutation_leaf_source` requires a leaf's
//! `owner` to be an immediate child of its aggregate mutation root, so this case reaches it by import,
//! never by moving the directory. The oracle performs every kind by independent GLB/JSON-tree
...
/// 🎥️ The `create-camera` fixture's own inserted camera (`../../🧫️fixtures/create-camera-applied/
/// after.gltf`, `cameras[1]`) — a perspective camera with only `yfov`/`znear` set, matching the
...
//#region 🔖️Profile
/// 📏️ Mirrors `../../../✳️any/🧪️tests/mutate-gltf-2-0/🦀️.rs`'s own `GLTF_WRITER_FREEDOM` — the
...
/// `../../../✳️any/🧪️tests/mutate-gltf-2-0/🦀️.rs`'s identical structure for the artifact-root case.
