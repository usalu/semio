...
pub fn validate(payload: &GltfChangeAssetVersionPayload, base: &GltfSnapshot) -> Result<(), GltfTopLevelMutationRejection> { if payload.version.trim().is_empty() { return Err(reject("gltf.mutation.invalid-asset-version", "document/asset/version", "version must be non-empty")); }
    if payload.version == base.document.asset.version { return Err(reject("gltf.mutation.no-observable-change", "document/asset/version", "version already has this value")); } Ok(()) }
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
...
impl protocol::MutationKind<GltfSnapshot, super::GltfMutation> for ChangeAssetVersionMutation {
    ...
    fn label(&self) -> String {
        "Change Asset Version".to_string()
    }
