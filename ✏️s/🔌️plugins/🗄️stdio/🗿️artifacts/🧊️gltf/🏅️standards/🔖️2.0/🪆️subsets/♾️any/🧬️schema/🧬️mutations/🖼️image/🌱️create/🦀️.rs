...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn apply(payload: &GltfCreateImagePayload, base: &GltfSnapshot) -> Result<GltfSnapshot, GltfTopLevelMutationRejection> {
    validate(payload, base)?;
    let mut next = base.clone();
    repair(&mut next.document, GltfTopLevelFamily::Images, &Change::Insert(payload.position))?;
    next.document.images.insert(payload.position, GltfImage::default());
    Ok(next)
}
...
impl protocol::MutationKind<GltfSnapshot, super::GltfMutation> for CreateImageMutation {
    ...
    fn inverse(&self, base: &GltfSnapshot) -> Vec<super::GltfMutation> {
        ...
        vec![super::GltfMutation::CreateImage(Self::Restore(Box::new(inverse)))]
    }

    fn label(&self) -> protocol::LocalizedLabel {
        protocol::LocalizedLabel::native("Create Image", "Bild erstellen")
    }
