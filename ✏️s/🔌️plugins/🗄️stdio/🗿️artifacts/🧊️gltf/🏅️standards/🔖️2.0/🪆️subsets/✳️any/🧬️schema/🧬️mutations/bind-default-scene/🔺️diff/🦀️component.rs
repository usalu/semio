...
pub async fn derive(payload: &GltfBindDefaultScenePayload, base: &GltfSnapshot) -> Result<GltfDiff, GltfTopLevelMutationRejection> { let _ = apply(payload, base)?; Ok(GltfDiff { scene: Some(Some(payload.scene)), ..Default::default() }) }
