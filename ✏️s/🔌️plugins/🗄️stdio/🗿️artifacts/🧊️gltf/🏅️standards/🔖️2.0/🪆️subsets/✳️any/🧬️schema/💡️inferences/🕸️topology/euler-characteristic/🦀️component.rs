...
pub(crate) async fn infer(context: &GltfGeometryContext<'_>) -> GltfMeasure<i64> {
    exact(context.topology.chi, GltfUnit::Unitless, context.sample_count, Some(context.topology)).await
}

pub async fn unavailable_measure(ids: &[String]) -> GltfMeasure<i64> {
