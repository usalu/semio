...
pub async fn derive(operation:&GltfBindNodeMeshPayload,base:&GltfSnapshot)->Result<GltfBindNodeMeshDiff,GltfTopLevelMutationRejection>{validate(operation,base)?;let after=Some(operation.mesh);Ok(GltfBindNodeMeshDiff{operation:operation.clone(),after,touched_paths:["document/nodes/*/mesh"].into_iter().map(str::to_owned).collect()})}
pub async fn apply(base:&GltfSnapshot,diff:&GltfBindNodeMeshDiff)->Result<GltfSnapshot,GltfTopLevelMutationRejection>{let mut next=base.clone();next.document.nodes[diff.operation.node].mesh=diff.after;Ok(next)}
pub async fn encode(diff:&GltfBindNodeMeshDiff)->Result<Vec<u8>,serde_json::Error>{serde_json::to_vec(diff)}
