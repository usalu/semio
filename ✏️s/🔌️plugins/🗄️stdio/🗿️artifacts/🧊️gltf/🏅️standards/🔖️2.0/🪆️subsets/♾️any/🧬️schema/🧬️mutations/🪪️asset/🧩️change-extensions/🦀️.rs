...
pub fn validate(payload: &GltfChangeAssetExtensionDataPayload, base: &GltfSnapshot) -> Result<(), GltfTopLevelMutationRejection> { if payload.data == base.document.asset.extensions { return Err(reject("gltf.mutation.no-observable-change", "document/asset/extensions", "value already has this value")); } Ok(()) }
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn apply(payload: &GltfChangeAssetExtensionDataPayload, base: &GltfSnapshot) -> Result<GltfSnapshot, GltfTopLevelMutationRejection> { validate(payload, base)?; let mut next = base.clone(); next.document.asset.extensions = payload.data.clone(); Ok(next) }
...
impl protocol::MutationKind<GltfSnapshot, super::GltfMutation> for ChangeAssetExtensionDataMutation {
    ...
    fn inverse(&self, base: &GltfSnapshot) -> Vec<super::GltfMutation> {
        ...
        vec![super::GltfMutation::ChangeAssetExtensionData(Self::Restore(Box::new(inverse)))]
    }

    fn label(&self) -> String {
        "Change Asset Extension Data".to_string()
    }
