...
pub(crate) fn infer(context: &GltfGeometryContext<'_>) -> GltfMeasure<f64> {
    context.solid.map_or_else(|| unavailable(GltfUnit::CubicMetre, context.unavailable_volume, Vec::new(), context.sample_count, Some(context.topology)), |metrics| exact(metrics.0, GltfUnit::CubicMetre, context.sample_count, Some(context.topology)))
}
