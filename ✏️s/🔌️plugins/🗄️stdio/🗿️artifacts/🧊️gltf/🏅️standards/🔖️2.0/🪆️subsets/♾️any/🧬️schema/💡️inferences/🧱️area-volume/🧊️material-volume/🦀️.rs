...
impl GltfInferenceLeaf for GltfMaterialVolumeInference {
...
pub(crate) fn infer(context: &GltfGeometryContext<'_>) -> GltfMeasure<f64> {
    context
        .solid.map_or_else(|| unavailable(GltfUnit::CubicMetre, context.unavailable_volume, Vec::new(), context.sample_count, Some(context.topology)), |metrics| exact(metrics.0, GltfUnit::CubicMetre, context.sample_count, Some(context.topology)))
}
...
pub fn encode_result(indicators: &GltfEntityIndicators) -> dsl::DslValue {
    dsl::ToValue::to_value(&indicators.area_volume.material_volume)
}
...
mod tests {
    ...
    async fn descriptor_is_versioned_and_cacheable() {
        assert_eq!(descriptor().id, "s.stdio.gltf.inference.material-volume.v1");
