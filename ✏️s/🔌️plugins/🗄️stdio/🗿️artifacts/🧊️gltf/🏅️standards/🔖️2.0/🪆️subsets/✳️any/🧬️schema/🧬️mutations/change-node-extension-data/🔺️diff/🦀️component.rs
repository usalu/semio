...
pub async fn derive(operation:&GltfChangeNodeExtensionDataPayload,base:&GltfSnapshot)->Result<GltfChangeNodeExtensionDataDiff,GltfTopLevelMutationRejection>{validate(operation,base)?;let after=match &operation.data{crate::artifacts::gltf::schema::mutations::change_node_extension_data::mutation::GltfDataPresence::Absent=>None,crate::artifacts::gltf::schema::mutations::change_node_extension_data::mutation::GltfDataPresence::Present{value}=>Some(value.clone())};Ok(GltfChangeNodeExtensionDataDiff{operation:operation.clone(),after,touched_paths:["document/nodes/*/extensions"].into_iter().map(str::to_owned).collect()})}
pub async fn apply(base:&GltfSnapshot,diff:&GltfChangeNodeExtensionDataDiff)->Result<GltfSnapshot,GltfTopLevelMutationRejection>{let mut next=base.clone();next.document.nodes[diff.operation.node].extensions=diff.after.clone();Ok(next)}
pub async fn encode(diff:&GltfChangeNodeExtensionDataDiff)->Result<Vec<u8>,serde_json::Error>{serde_json::to_vec(diff)}
