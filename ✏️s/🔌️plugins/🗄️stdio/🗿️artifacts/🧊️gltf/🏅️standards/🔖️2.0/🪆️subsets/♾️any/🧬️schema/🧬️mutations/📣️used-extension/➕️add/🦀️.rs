...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn apply(payload: &GltfDeclareUsedExtensionPayload, base: &GltfSnapshot) -> Result<GltfSnapshot, GltfTopLevelMutationRejection> {
    validate(payload, base)?;
    let mut next = base.clone();
    next.document.extensions_used.insert(payload.position, payload.extension.clone());
    Ok(next)
}
...
impl protocol::MutationKind<GltfSnapshot, super::GltfMutation> for AddUsedExtensionMutation {
    ...
    fn label(&self) -> protocol::LocalizedLabel {
        protocol::LocalizedLabel::native("Add Used Extension", "genutzte Erweiterung hinzufügen")
    }
