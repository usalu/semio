...
pub fn descriptor() -> GltfInferenceLeafDescriptor {
    GltfVoidVolumeInference::DESCRIPTOR
}

// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub(crate) fn infer(context: &GltfGeometryContext<'_>) -> GltfMeasure<f64> {
    context.solid.map_or_else(|| unavailable(GltfUnit::CubicMetre, context.unavailable_volume, Vec::new(), context.sample_count, Some(context.topology)), |metrics| exact(metrics.2, GltfUnit::CubicMetre, context.sample_count, Some(context.topology)))
}
...
pub fn encode_result(indicators: &GltfEntityIndicators) -> dsl::DslValue {
    dsl::ToValue::to_value(&indicators.area_volume.void_volume)
}
