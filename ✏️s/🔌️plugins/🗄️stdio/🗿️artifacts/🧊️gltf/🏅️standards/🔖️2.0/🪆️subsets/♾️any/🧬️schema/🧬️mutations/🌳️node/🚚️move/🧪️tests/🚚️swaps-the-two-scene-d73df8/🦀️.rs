//! 🧪️ `move-node` fixture — `🚚️swaps-the-two-scene-root-nodes`.
//!
...
use crate::schema::mutations::move_node::diff::GltfMoveNodeDiff;
use crate::schema::mutations::move_node::GltfMoveNodePayload;
use crate::schema::mutations::move_node::{diff, inverse, mutation};
use crate::GltfSnapshot;

const CASE: &str = "move-node/swaps-the-two-scene-root-nodes";
const BEFORE: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🌳️node/🚚️move/🚚️swaps-the-two-scene-d73df8/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🌳️node/🚚️move/🚚️swaps-the-two-scene-d73df8/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🌳️node/🚚️move/🚚️swaps-the-two-scene-d73df8/🧬️operation/🔣️.json");
const DIFF: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🌳️node/🚚️move/🚚️swaps-the-two-scene-d73df8/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../../🧫️fixtures/🧬️mutations/🌳️node/🚚️move/🚚️swaps-the-two-scene-d73df8/🎯️outcome/🔣️.json");
...
async fn committed_diff_is_canonical() {
    let decoded: GltfMoveNodeDiff = dsl::json::from_json_str(DIFF).expect("committed diff decodes");
    let reencoded = serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&decoded)).expect("diff re-encodes");
    let original: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff reparses");
    assert_eq!(reencoded, original, "{CASE}: committed diff JSON is not canonical");
    assert!(matches!(decoded.operation, crate::schema::mutations::move_node::diff::GltfMoveNodeOperation::Move { index: 0, position: 1 }), "{CASE}: the committed operation must be a bare move, carrying no node value");
}
