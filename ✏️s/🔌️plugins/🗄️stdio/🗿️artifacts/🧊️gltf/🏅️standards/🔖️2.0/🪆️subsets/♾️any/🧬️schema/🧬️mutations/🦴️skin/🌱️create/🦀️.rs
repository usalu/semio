...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn apply(payload: &GltfCreateSkinPayload, base: &GltfSnapshot) -> Result<GltfSnapshot, GltfTopLevelMutationRejection> {
    validate(payload, base)?;
    let mut next = base.clone();
    repair(&mut next.document, GltfTopLevelFamily::Skins, &Change::Insert(payload.position))?;
    next.document.skins.insert(payload.position, GltfSkin::default());
    Ok(next)
}
