...
pub fn derive(operation:&GltfBindNodeCameraPayload,base:&GltfSnapshot)->Result<GltfBindNodeCameraInverse,GltfTopLevelMutationRejection>{validate(operation,base)?;let before=base.document.nodes[operation.node].camera;Ok(GltfBindNodeCameraInverse{operation:operation.clone(),before,touched_paths:["document/nodes/*/camera"].into_iter().map(str::to_owned).collect()})}
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn apply(base:&GltfSnapshot,inverse:&GltfBindNodeCameraInverse)->Result<GltfSnapshot,GltfTopLevelMutationRejection>{let mut next=base.clone();next.document.nodes[diff.operation.node].camera=inverse.before;Ok(next)}
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn encode(inverse:&GltfBindNodeCameraInverse)->Result<Vec<u8>,serde_json::Error>{serde_json::to_vec(inverse)}
