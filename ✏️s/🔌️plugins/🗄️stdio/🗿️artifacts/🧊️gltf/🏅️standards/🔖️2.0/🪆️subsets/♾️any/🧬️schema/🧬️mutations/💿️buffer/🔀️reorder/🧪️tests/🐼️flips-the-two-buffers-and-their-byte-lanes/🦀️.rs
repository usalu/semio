//! 🧪️ `reorder-buffers` fixture — `🐼️flips-the-two-buffers-and-their-byte-lanes`.
//!
//! The only reorder leaf that permutes two lanes: `buffers_op(…, Reorder)` for the descriptors and an
...
fn payload() -> GltfReorderBuffersPayload {
    serde_json::from_str(MUTATION).expect("reorder-buffers payload decodes")
}

/// ▶️ `reorder-buffers` permutes descriptors and bytes together and rewrites both buffer references.
#[semio_framework_async_macros::async_test]
async fn applies_to_committed_after() {
    let snapshot = mutation::apply(&payload(), &before()).expect("reorder-buffers applies to its committed before-snapshot");
...
/// ↩️ Re-applying the same flip restores both lanes.
#[semio_framework_async_macros::async_test]
async fn inverse_restores_before() {
    let base = before();
    let inverse = inverse::derive(&payload(), &base).expect("reorder-buffers inverse derives from the exact base");
...
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => {
            let snapshot = attempt.expect("reorder-buffers declared applied");
        ...
        "rejected" => {
            ...
            assert_eq!(attempt.expect_err("reorder-buffers declared rejected").code, code, "{CASE}: rejection code differs from the committed outcome");
        }
        other => panic!("{CASE}: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The delta declares four slots — descriptors then bytes — and carries no bytes itself.
#[semio_framework_async_macros::async_test]
async fn produces_committed_diff() {
    let produced = diff::derive(&payload(), &before()).expect("reorder-buffers derives its diff");
...
async fn committed_diff_is_canonical() {
    let decoded: GltfReorderBuffersDiff = serde_json::from_str(DIFF).expect("committed diff decodes");
