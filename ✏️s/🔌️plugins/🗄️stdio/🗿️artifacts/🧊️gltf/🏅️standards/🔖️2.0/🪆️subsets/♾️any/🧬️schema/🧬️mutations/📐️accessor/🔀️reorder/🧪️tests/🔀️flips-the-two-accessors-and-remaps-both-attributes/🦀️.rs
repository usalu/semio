//! 🧪️ `reorder-accessors` fixture — `🔀️flips-the-two-accessors-and-remaps-both-attributes`.
//!
//! `apply()` calls `accessors_op(…, order[0], None, Some(order))`, whose `Change::Reorder` remap is
...
fn payload() -> GltfReorderAccessorsPayload {
    serde_json::from_str(MUTATION).expect("reorder-accessors payload decodes")
}

/// ▶️ `reorder-accessors` rewrites the whole collection order and remaps every attribute that indexed it.
...
/// ↩️ The inverse is the same flip applied again, which is its own undo for a 2-cycle.
#[semio_framework_async_macros::async_test]
async fn inverse_restores_before() {
    let base = before();
    let inverse = inverse::derive(&payload(), &base).expect("reorder-accessors inverse derives from the exact base");
...
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => {
            let snapshot = attempt.expect("reorder-accessors declared applied");
        ...
        "rejected" => {
            ...
            assert_eq!(attempt.expect_err("reorder-accessors declared rejected").code, code, "{CASE}: rejection code differs from the committed outcome");
        }
        other => panic!("{CASE}: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The delta carries the permutation only — never the reordered accessor values.
#[semio_framework_async_macros::async_test]
async fn produces_committed_diff() {
    let produced = diff::derive(&payload(), &before()).expect("reorder-accessors derives its diff");
...
async fn committed_diff_is_canonical() {
    let decoded: GltfReorderAccessorsDiff = serde_json::from_str(DIFF).expect("committed diff decodes");
