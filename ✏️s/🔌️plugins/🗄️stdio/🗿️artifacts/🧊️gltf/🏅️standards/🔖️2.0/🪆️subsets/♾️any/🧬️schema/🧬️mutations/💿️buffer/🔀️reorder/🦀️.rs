...
pub fn validate(payload: &GltfReorderBuffersPayload, base: &GltfSnapshot) -> Result<(), GltfTopLevelMutationRejection> { if payload.order.len() != base.document.buffers.len() || payload.order.iter().collect::<std::collections::BTreeSet<_>>().len() != payload.order.len() || payload.order.iter().any(|index| *index >= base.document.buffers.len()) { return Err(reject("gltf.mutation.invalid-permutation", "document/buffers", "order must contain every index once")); }
    if payload.order.iter().enumerate().all(|(index, value)| index == *value) { return Err(reject("gltf.mutation.no-observable-change", "document/buffers", "order already matches")); }
...
impl protocol::MutationKind<GltfSnapshot, super::GltfMutation> for ReorderBuffersMutation {
    ...
    fn inverse(&self, base: &GltfSnapshot) -> Vec<super::GltfMutation> {
        ...
        vec![super::GltfMutation::ReorderBuffers(Self::Restore(Box::new(inverse)))]
    }
