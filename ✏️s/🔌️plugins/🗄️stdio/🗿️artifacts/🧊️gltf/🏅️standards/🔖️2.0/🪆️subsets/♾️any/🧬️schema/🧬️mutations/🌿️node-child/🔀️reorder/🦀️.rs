...
pub fn validate(payload: &GltfReorderNodeChildrenPayload, base: &GltfSnapshot) -> Result<(), GltfTopLevelMutationRejection> { checked_index(payload.parent, base.document.nodes.len(), "document/nodes")?; let children = &base.document.nodes[payload.parent].children; if payload.order.len() != children.len() || payload.order.iter().any(|child| !children.contains(child)) || { let mut order = payload.order.clone(); order.sort_unstable(); order.dedup(); order.len() != children.len() } { return Err(reject("gltf.mutation.invalid-permutation", "document/nodes/children", "order must contain every child identity once")); }
    if payload.order == *children { return Err(reject("gltf.mutation.no-observable-change", "document/nodes/children", "reorder must change order")); } Ok(()) }
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
