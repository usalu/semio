...
pub async fn derive(payload: &GltfWithdrawUsedExtensionPayload, base: &GltfSnapshot) -> Result<GltfDiff, GltfTopLevelMutationRejection> { let _ = apply(payload, base)?; Ok(GltfDiff { extensions_used: Some(base.document.extensions_used.clone()), ..Default::default() }) }
