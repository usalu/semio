...
pub async fn validate(payload: &GltfMoveSamplerPayload, base: &GltfSnapshot) -> Result<(), GltfTopLevelMutationRejection> { if payload.index >= base.document.samplers.len() || payload.position >= base.document.samplers.len() { return Err(reject("gltf.mutation.index-out-of-range", "document/samplers", "indices must address items")); } if payload.index == payload.position { return Err(reject("gltf.mutation.no-observable-change", "document/samplers", "destination equals source")); }  Ok(()) }
pub async fn apply(payload: &GltfMoveSamplerPayload, base: &GltfSnapshot) -> Result<GltfSnapshot, GltfTopLevelMutationRejection> { validate(payload, base)?; let mut next = base.clone(); samplers_op(&mut next, GltfTopLevelFamily::Samplers, payload.index, Some(payload.position), None)?;  Ok(next) }
