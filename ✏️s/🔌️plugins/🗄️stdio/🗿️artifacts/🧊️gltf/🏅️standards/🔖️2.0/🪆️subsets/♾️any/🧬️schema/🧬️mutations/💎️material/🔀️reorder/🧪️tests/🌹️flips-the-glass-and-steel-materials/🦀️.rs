//! 🧪️ `reorder-materials` fixture — `🌹️flips-the-glass-and-steel-materials`.
//!
//! `repair(Materials, Reorder([1, 0]))` remaps `primitive.material` across every mesh primitive. Both
...
fn payload() -> GltfReorderMaterialsPayload {
    serde_json::from_str(MUTATION).expect("reorder-materials payload decodes")
}

/// ▶️ `reorder-materials` permutes the material list and remaps both primitive bindings.
#[semio_framework_async_macros::async_test]
async fn applies_to_committed_after() {
    let snapshot = mutation::apply(&payload(), &before()).expect("reorder-materials applies to its committed before-snapshot");
...
/// ↩️ Re-applying the same flip restores the original material order.
#[semio_framework_async_macros::async_test]
async fn inverse_restores_before() {
    let base = before();
    let inverse = inverse::derive(&payload(), &base).expect("reorder-materials inverse derives from the exact base");
...
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => {
            let snapshot = attempt.expect("reorder-materials declared applied");
        ...
        "rejected" => {
            ...
            assert_eq!(attempt.expect_err("reorder-materials declared rejected").code, code, "{CASE}: rejection code differs from the committed outcome");
        }
        other => panic!("{CASE}: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The delta is the permutation; neither material body appears in it.
#[semio_framework_async_macros::async_test]
async fn produces_committed_diff() {
    let produced = diff::derive(&payload(), &before()).expect("reorder-materials derives its diff");
...
async fn committed_diff_is_canonical() {
    let decoded: GltfReorderMaterialsDiff = serde_json::from_str(DIFF).expect("committed diff decodes");
    let reencoded = serde_json::to_value(&decoded).expect("diff re-encodes");
    let original: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff reparses");
    assert_eq!(reencoded, original, "{CASE}: committed diff JSON is not canonical");
    assert_eq!(decoded.id, "s.stdio.gltf.mutation.reorder-materials.v1", "{CASE}: the committed diff must carry reorder-materials' own canonical descriptor id");
}
