...
pub async fn validate(payload: &GltfChangeAssetVersionPayload, base: &GltfSnapshot) -> Result<(), GltfTopLevelMutationRejection> { if payload.version.trim().is_empty() { return Err(reject("gltf.mutation.invalid-asset-version", "document/asset/version", "version must be non-empty")); } if payload.version == base.document.asset.version { return Err(reject("gltf.mutation.no-observable-change", "document/asset/version", "version already has this value")); } Ok(()) }
pub async fn apply(payload: &GltfChangeAssetVersionPayload, base: &GltfSnapshot) -> Result<GltfSnapshot, GltfTopLevelMutationRejection> { validate(payload, base)?; let mut next = base.clone(); next.document.asset.version = payload.version.clone(); Ok(next) }
