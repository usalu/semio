...
pub fn descriptor() -> GltfInferenceLeafDescriptor { GltfIrregularityInference::DESCRIPTOR }
...
pub(crate) fn from_raw(context: &GltfGeometryContext<'_>, raw: &super::GltfRoughnessRaw) -> GltfMeasure<f64> {
    raw.irregularity.map(|value| estimate(value, GltfUnit::Unitless, raw.deviations.len(), Some(context.topology))).unwrap_or_else(|| unavailable(GltfUnit::Unitless, GltfAvailability::Degenerate, Vec::new(), context.sample_count, Some(context.topology)))
}
...
pub fn encode_result(indicators: &GltfEntityIndicators) -> Result<serde_json::Value, serde_json::Error> {
    serde_json::to_value(&indicators.roughness.irregularity)
}
