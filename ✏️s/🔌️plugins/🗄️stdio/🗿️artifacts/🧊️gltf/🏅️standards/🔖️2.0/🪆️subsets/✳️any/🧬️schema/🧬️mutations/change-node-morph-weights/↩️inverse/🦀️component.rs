...
pub fn derive(operation:&GltfChangeNodeMorphWeightsPayload,base:&GltfSnapshot)->Result<GltfChangeNodeMorphWeightsInverse,GltfTopLevelMutationRejection>{validate(operation,base)?;let before=base.document.nodes[operation.node].weights.clone();Ok(GltfChangeNodeMorphWeightsInverse{operation:operation.clone(),before,touched_paths:["document/nodes/*/weights"].into_iter().map(str::to_owned).collect()})}
pub fn apply(base:&GltfSnapshot,inverse:&GltfChangeNodeMorphWeightsInverse)->Result<GltfSnapshot,GltfTopLevelMutationRejection>{let mut next=base.clone();next.document.nodes[diff.operation.node].weights=inverse.before.clone();Ok(next)}
pub fn encode(inverse:&GltfChangeNodeMorphWeightsInverse)->Result<Vec<u8>,serde_json::Error>{serde_json::to_vec(inverse)}
