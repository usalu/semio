...
pub async fn derive(payload: &GltfMoveRequiredExtensionPayload, base: &GltfSnapshot) -> Result<GltfDiff, GltfTopLevelMutationRejection> { let _ = apply(payload, base)?; Ok(GltfDiff { extensions_required: Some({ let mut values = base.document.extensions_required.clone(); let value = values.remove(values.iter().position(|value| value == &payload.extension).unwrap()); values.insert(payload.position, value); values }), ..Default::default() }) }
