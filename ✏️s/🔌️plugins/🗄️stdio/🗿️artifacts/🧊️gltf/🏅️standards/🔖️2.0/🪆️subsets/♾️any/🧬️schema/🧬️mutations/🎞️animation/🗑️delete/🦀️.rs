...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn apply(payload: &GltfDeleteAnimationPayload, base: &GltfSnapshot) -> Result<GltfSnapshot, GltfTopLevelMutationRejection> {
    validate(payload, base)?;
    let mut next = base.clone();
    animations_op(&mut next, GltfTopLevelFamily::Animations, payload.index, None, None)?;
    Ok(next)
}
...
impl protocol::MutationKind<GltfSnapshot, super::GltfMutation> for DeleteAnimationMutation {
    ...
    fn inverse(&self, base: &GltfSnapshot) -> Vec<super::GltfMutation> {
        ...
        vec![super::GltfMutation::DeleteAnimation(Self::Restore(Box::new(inverse)))]
    }

    fn label(&self) -> protocol::LocalizedLabel {
        protocol::LocalizedLabel::native("Delete Animation", "Animation löschen")
    }
