...
pub fn validate(payload: &GltfMoveImagePayload, base: &GltfSnapshot) -> Result<(), GltfTopLevelMutationRejection> { if payload.index >= base.document.images.len() || payload.position >= base.document.images.len() { return Err(reject("gltf.mutation.index-out-of-range", "document/images", "indices must address items")); }
    if payload.index == payload.position { return Err(reject("gltf.mutation.no-observable-change", "document/images", "destination equals source")); }  Ok(()) }
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
...
impl protocol::MutationKind<GltfSnapshot, super::GltfMutation> for MoveImageMutation {
    ...
    fn inverse(&self, base: &GltfSnapshot) -> Vec<super::GltfMutation> {
        ...
        vec![super::GltfMutation::MoveImage(Self::Restore(Box::new(inverse)))]
    }
