"""🐍️ IfcOpenShell differential ORACLE for stdio.ifc 2x3/✳️any — a real SECOND PRODUCER.

Ticket 26/08/23/END-TO-END-TESTING-REFACTOR. The sibling case `../mutate-ifc-2x3` registers
...
🚫️ **Nothing here reaches this repository's implementation.** The only imports are `json`, `os`,
...
(`§8.2.2`/`§8.2.3` header attribute order). It is a deliberate, stated duplication of the sibling
`../differential-ifc-4/🐍️component.py`'s reader rather than a shared module: a case adapter in this
repository is self-contained by convention, the two projections differ (this subset keeps
complex/multi-type instances whole, so an instance projects as a LIST of records), and the two cases
...
📏️ **Why IfcOpenShell reformats this one and not the IFC4 sibling.** This file was written by EDM
StepFileFactory, not by IfcOpenShell, so IfcOpenShell's own writer is NOT a fixed point of it:
193 915 bytes in, 188 288 bytes out, and the projections still agree entity for entity. That is a
much stronger no-pass-through proof than the IFC4 sibling can give, where IfcOpenShell reproduces
its own earlier export byte for byte.

📏️ **What IfcOpenShell cannot produce, measured rather than assumed.** `remove-instance` has no
differential scenario here. `ifcopenshell.file.remove` documents and performs reference repair
("in the case of a list or set of references, the reference to the deleted will be removed from the
aggregate"), and this subset's own `Ifc2x3Mutation::RemoveInstance` is a bare `retain` that
deliberately leaves dangling references — the case's own feature file records that as the chosen
...
@see ../../🏅️standards/🔖️2x3/🪆️subsets/✳️any/🔣️oracle.json — this oracle's registration.
"""
...
#: scenario in `../mutate-ifc-2x3`; nothing is lost, and nothing false is claimed here.
...
def read_document(text: str) -> tuple:
    """📥️ Header records and DATA entity instances of one exchange structure. An instance is a
    LIST of records so a complex (multi-type) instance survives the projection whole."""
    header: dict = {}
    ...
    while True:
        ...
        records = []
        if reader.text[reader.at] == "(":
            # 🧬️ A COMPLEX instance: `#5=(NAME_A(…)NAME_B(…));`. This subset keeps such an instance
            # whole, as a list of records, rather than flattening it to its first type.
            reader.at += 1
            while True:
                reader.skip()
                if reader.text[reader.at] == ")":
                ...
                name = reader.keyword()
                reader.skip()
                records.append({"name": name.upper(), "args": reader.parameter_list()})
        else:
            name = reader.keyword()
            reader.skip()
            records.append({"name": name.upper(), "args": reader.parameter_list()})
        entities.append({"id": identifier, "entities": records})
        reader.skip()
        if reader.at < reader.size and reader.text[reader.at] == ";":
            reader.at += 1
...
def project(text: str) -> dict:
    """👁️ The `semantic-ifc-v1` surface for this subset: declared FILE_SCHEMA, both header records
    under the names the standard fixes for them, and the full id-sorted entity graph — where an
    instance is a LIST of records, so a complex (multi-type) instance stays whole."""
    header, entities = read_document(text)
    schema: list = []
    for outer in header.get("FILE_SCHEMA", []):
        if outer["t"] == "aggregate":
            schema.extend(item["v"] for item in outer["v"] if item["t"] == "string")
...
# region 🔖️Producer
def set_header_records(model, header: dict) -> None:
    """📇️ Replaces all three header records wholesale, the way `Ifc2x3Mutation::SetHeader` does."""
    description = header["fileDescription"]
    model.header.file_description.description = list(to_ifcopenshell(model, description[0]))
    if len(description) > 1:
        model.header.file_description.implementation_level = to_ifcopenshell(model, description[1])
    file_name = header["fileName"]
    target = model.header.file_name
...
def apply_mutation(path: str, spec: dict) -> bytes:
    ...
    if kind == "no-mutation":
        ...
        if inverses != 0:
            raise AssertionError("#%s is referenced by %d other instance(s); `ifcopenshell.file.remove` repairs those references while Ifc2x3Mutation::RemoveInstance is a bare retain that deliberately leaves them dangling, so this oracle refuses the removal rather than silently performing a different verb" % (params["id"], inverses))
        model.remove(target)
    else:
...
def inverse(ctx: Context) -> Outcome:
    ...
    if found:
        raise AssertionError("inverse law violated for %r — undoing it did not restore the building model: %s" % (kind, found))
    return Outcome(projection, restored)


def identity_round_trip(ctx: Context) -> Outcome:
    """🔒️ IfcOpenShell decodes the whole real building model into its own typed model and
    re-serializes from that model alone. This fixture was written by EDM StepFileFactory, not by
    IfcOpenShell, so the byte tripwire is real evidence here: the output MUST NOT be bit-identical
    to the input, and the projections MUST still agree."""
    path = mutable_input(ctx)
    ...
    if produced == original:
        raise AssertionError("byte pass-through: IfcOpenShell's re-encoded output is bit-identical to the input, so nothing here proves the document was parsed")
    projection = project(produced.decode("utf-8"))
    if materialized != projection["entityCount"]:
        raise AssertionError("IfcOpenShell materialized %d typed instances but its own written bytes carry %d instances — the model was not fully parsed" % (materialized, projection["entityCount"]))
    before = project(original.decode("utf-8"))
...
def adapter() -> Adapter:
    ...
    for kind in KINDS:
        built = built.oracle("differential-%s" % kind, mutate)
    for kind in INVERSE_KINDS:
        built = built.oracle("differential-inverse-%s" % kind, inverse)
