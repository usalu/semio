...
/// `fixture_honesty_law`) and for `mutations::demo_mutation_cases()`/`diff::demo_diff_cases()`.
pub fn demo_ifc_snapshot() -> IfcSnapshot {
    ...
    IfcSnapshot {
        ...
        entities: vec![
            IfcEntity { id: 1, name: "IFCPROJECT".into(), args: vec![IfcValue::String("gid-project".into()), IfcValue::Reference(2), IfcValue::String("Demo Project".into())], complex: Vec::new() },
            IfcEntity { id: 2, name: "IFCOWNERHISTORY".into(), args: vec![IfcValue::Unset, IfcValue::Integer(0)], complex: Vec::new() },
        ],
    }
}
...
mod tests {
    ...
    /// own `conformance_laws` module and the sibling STEP artifact's own.
