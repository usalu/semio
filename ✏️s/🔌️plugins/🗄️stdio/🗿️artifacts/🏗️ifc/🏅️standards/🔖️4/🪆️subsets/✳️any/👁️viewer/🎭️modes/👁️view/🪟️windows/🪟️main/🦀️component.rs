...
/// 👁️ Pure `IfcSnapshot -> UiNode` read: default camera (a viewer has no persisted
...
pub async fn render(document: &IfcSnapshot) -> UiNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": IFC4_ANY_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(IFC4_ANY_VIEW_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(IFC4_ANY_VIEW_DEFAULT_CAMERA_POSITION, IFC4_ANY_VIEW_DEFAULT_CAMERA_TARGET, IFC4_ANY_VIEW_DEFAULT_CAMERA_FOV).await,
        meshes_json,
