...
pub async fn deserialize(from: &TxtSnapshot) -> Result<IfcSnapshot, store::TextError> {
    let document = crate::artifacts::step::engine::part21::parse_part21(from.to_body().await.trim()).await.map_err(|e| store::TextError::new(format!("ifc parse: {e}"), dsl::TextSpan::at(1, 1)))?;
    Ok(crate::artifacts::ifc::schema::snapshot::from_part21_document(STDIO_IFC_DOCUMENT_SCHEMA, &document).await)
}
