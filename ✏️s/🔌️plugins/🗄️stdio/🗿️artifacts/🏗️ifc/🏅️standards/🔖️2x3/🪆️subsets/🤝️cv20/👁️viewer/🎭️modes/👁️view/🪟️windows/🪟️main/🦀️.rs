//! 🧊 IFC 2x3 Cv20 viewer — the Main window: a read-only world-3d mesh scene built with the shared
//! `MeshWindowKit` (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.6), from the
//! artifact-level `Ifc2x3Snapshot` — this file imports nothing from the sibling editor module
...
use crate::standards::v2x3::subsets::cv20::schema::snapshot::Ifc2x3Snapshot;
...
const IFC2X3_CV20_VIEW_FALLBACK_MESH_KIND: &str = "box";
const IFC2X3_CV20_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const IFC2X3_CV20_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const IFC2X3_CV20_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
fn entity_count(document: &Ifc2x3Snapshot) -> usize {
...
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
fn world_instances_json(document: &Ifc2x3Snapshot) -> String {
    let count = entity_count(document);
    let instances: Vec<pack::JsonValue> = (0..count)
        .map(|index| {
            pack::json!({
                "id": format!("ifc2x3_cv20-{index}"),
                "meshId": IFC2X3_CV20_VIEW_FALLBACK_MESH_KIND,
                "position": [index as f64 * 2.0, 0.0, 0.0],
                "rotation": [0.0, 0.0, 0.0, 1.0],
                "scale": [1.0, 1.0, 1.0],
                "label": format!("IFC 2x3 Cv20 {index}"),
                "smoothShading": false,
            })
        })
        .collect();
    pack::json_to_string(&pack::json_array(instances))
}
...
pub fn render(document: &Ifc2x3Snapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = pack::json_to_string(&pack::json_array([pack::json!({ "id": IFC2X3_CV20_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(IFC2X3_CV20_VIEW_FALLBACK_MESH_KIND) })]));
    let view = MeshView {
        camera_json: world3d_camera_json(IFC2X3_CV20_VIEW_DEFAULT_CAMERA_POSITION, IFC2X3_CV20_VIEW_DEFAULT_CAMERA_TARGET, IFC2X3_CV20_VIEW_DEFAULT_CAMERA_FOV),
        meshes_json,
...
mod tests {
    ...
    async fn render_produces_a_scene_node_for_the_default_document() {
        let document = Ifc2x3Snapshot::default();
