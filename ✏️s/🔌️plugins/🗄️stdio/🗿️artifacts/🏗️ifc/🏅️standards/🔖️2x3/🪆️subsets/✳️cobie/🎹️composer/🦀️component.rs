//! 🎹️ Ifc2x3CobieComposer (2x3/✳️cobie) — reads the same sources ✳️any does, delegates the parse
//! to the ✳️any composer, then HARD-GATES the `cobie` dialect stamp on the two HARD Basic FM
//! Handover checks (FILE_SCHEMA + ViewDefinition); the roster's soft checks (space names,
//! building/storey presence, type assignment) pass through as advisory diagnostics. Also
//! registers this dialect's `SubsetValidator`.
...
//#region 🔖️Composer
pub struct Ifc2x3CobieComposer;

impl ArtifactComposer for Ifc2x3CobieComposer {
    ...
    fn compose(sources: &[ComposeSource]) -> Result<Composition<Self::Snapshot>, ComposeError> {
        ...
        if !hard.is_empty() {
            ...
            return Err(ComposeError {
                message: format!("Basic FM Handover (COBie) conformance violated: {} hard issue(s) -- not stamping the cobie dialect", hard.len()),
                diagnostics: all,
            });
        }
...
mod tests {
    ...
    use crate::artifacts::ifc::standards::v2x3::subsets::cobie::analyzer::CODE_VIEW_DEFINITION;
    use crate::artifacts::ifc::standards::v2x3::subsets::cobie::builder::Ifc2x3CobieBuilder;
    ...
    fn conforming_builder_snapshot_composes_and_stamps_cobie() {
        ...
        let composed = Ifc2x3CobieComposer::compose(&sources).expect("clean document must compose to cobie");
    ...
    fn wrong_view_definition_fails_compose_with_real_diagnostic() {
        ...
        snapshot.document.header.file_description[0] = crate::artifacts::step::engine::part21::Part21Value::List(vec![
            crate::artifacts::step::engine::part21::Part21Value::Str("ViewDefinition [CoordinationView]".into()),
        ]);
        let bytes = <Ifc2x3Snapshot as store::ArtifactPack>::encode_pack(&snapshot);
        let sources = vec![ComposeSource { dialect: DIALECT_ANY, payload: AnalyzeSource::Binary(&bytes) }];
        let err = Ifc2x3CobieComposer::compose(&sources).expect_err("wrong ViewDefinition must not stamp cobie");
