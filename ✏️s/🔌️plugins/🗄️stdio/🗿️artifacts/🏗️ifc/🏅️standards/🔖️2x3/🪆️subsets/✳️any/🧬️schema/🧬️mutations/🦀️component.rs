...
mod tests {
    ...
    async fn upsert_then_inverse_restores_absent_id_via_remove() {
        ...
        let mutation = Ifc2x3Mutation::UpsertInstance { instance: inst(1, "IFCWALL").await };
        let base = snap.clone();
    ...
    async fn exact_native_between_noop_inverse_absorb_and_supported_rewrite() {
        ...
        assert_exact("no-op export", &crate::artifacts::ifc::standards::v2x3::engine::encode_ifc2x3(&no_op).expect("no-op export")).await;
