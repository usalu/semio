//! 🧊 IFC 2x3 Sav viewer — the Main window: a read-only world-3d mesh scene built with the shared
...
use crate::artifacts::ifc::standards::v2x3::subsets::sav::schema::snapshot::Ifc2x3Snapshot;
...
const IFC2X3_SAV_VIEW_FALLBACK_MESH_KIND: &str = "box";
const IFC2X3_SAV_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const IFC2X3_SAV_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const IFC2X3_SAV_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
...
fn world_instances_json(document: &Ifc2x3Snapshot) -> String {
    ...
    let instances: Vec<pack::JsonValue> = (0..count)
        .map(|index| {
            pack::json!({
                "id": format!("ifc2x3_sav-{index}"),
                "meshId": IFC2X3_SAV_VIEW_FALLBACK_MESH_KIND,
                "position": [index as f64 * 2.0, 0.0, 0.0],
                "rotation": [0.0, 0.0, 0.0, 1.0],
                "scale": [1.0, 1.0, 1.0],
                "label": format!("IFC 2x3 Sav {index}"),
                "smoothShading": false,
            })
        })
        .collect();
...
pub fn render(document: &Ifc2x3Snapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = pack::json_to_string(&pack::json_array([pack::json!({ "id": IFC2X3_SAV_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(IFC2X3_SAV_VIEW_FALLBACK_MESH_KIND) })]));
    let view = MeshView {
