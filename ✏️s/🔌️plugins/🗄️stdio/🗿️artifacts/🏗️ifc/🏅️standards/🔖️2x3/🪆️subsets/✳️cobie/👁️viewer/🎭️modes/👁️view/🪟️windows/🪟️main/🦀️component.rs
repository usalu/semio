//! 🧊 IFC 2x3 Cobie viewer — the Main window: a read-only world-3d mesh scene built with the shared
...
const IFC2X3_COBIE_VIEW_FALLBACK_MESH_KIND: &str = "box";
const IFC2X3_COBIE_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const IFC2X3_COBIE_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const IFC2X3_COBIE_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
    ...
    let instances: Vec<serde_json::Value> = (0..count.await)
        .map(|index| {
            serde_json::json!({
                ...
                "meshId": IFC2X3_COBIE_VIEW_FALLBACK_MESH_KIND,
...
/// 👁️ Pure `Ifc2x3Snapshot -> UiNode` read: default camera (a viewer has no persisted
...
pub async fn render(document: &Ifc2x3Snapshot) -> UiNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": IFC2X3_COBIE_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(IFC2X3_COBIE_VIEW_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(IFC2X3_COBIE_VIEW_DEFAULT_CAMERA_POSITION, IFC2X3_COBIE_VIEW_DEFAULT_CAMERA_TARGET, IFC2X3_COBIE_VIEW_DEFAULT_CAMERA_FOV).await,
        meshes_json,
