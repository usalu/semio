...
pub async fn render(document: &Ifc2x3Snapshot) -> UiNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": IFC2X3_ANY_EDIT_FALLBACK_MESH_KIND, "data": mesh_from_kind(IFC2X3_ANY_EDIT_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(IFC2X3_ANY_EDIT_DEFAULT_CAMERA_POSITION, IFC2X3_ANY_EDIT_DEFAULT_CAMERA_TARGET, IFC2X3_ANY_EDIT_DEFAULT_CAMERA_FOV).await,
        meshes_json,
