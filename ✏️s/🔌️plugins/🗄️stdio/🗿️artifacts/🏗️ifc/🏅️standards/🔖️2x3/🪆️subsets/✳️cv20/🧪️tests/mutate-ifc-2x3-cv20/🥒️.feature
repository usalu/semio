...
Feature: Apply every typed Coordination View 2.0 mutation to a real IFC2X3 building model
  ...
  This subset's vocabulary is NOT the sibling `✳️base` subset's. `✳️base` declares generic ISO 10303-21
  ...
  @level-exhaustive
  @mode-property
  Scenario: Apply no-mutation to the real Coordination View 2.0 model
    Given the real input document shared://🧪️wellness-center-sama-street-level/🏗️.ifc
  ...
  @level-exhaustive
  @mode-property
  Scenario: Undoing no-mutation restores the real Coordination View 2.0 model
    Given the real input document shared://🧪️wellness-center-sama-street-level/🏗️.ifc
    When the no-mutation mutation is applied with its parameters
      """
      {"kind": "no-mutation", "params": {}}
