//! 👁️ IFC 2x3 Cobie viewer — the read-only counterpart of `✏️editor` for this subset (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `Ifc2x3CobieViewer`
//! implements `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<Ifc2x3CobieViewer>`
...
pub enum Ifc2x3CobieViewCommand {
    #[default]
    Noop,
}

impl protocol::OpBinary for Ifc2x3CobieViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(Ifc2x3CobieViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
pub struct Ifc2x3CobieViewer;

impl ArtifactViewer for Ifc2x3CobieViewer {
    ...
    type Command = Ifc2x3CobieViewCommand;
...
pub fn create_ifc2x3_cobie_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(IFC2X3_COBIE_DIALECT).document(["stdio", "ifc2x3"]).icon_id("box").mode_def(view::definition()).default_mode_id(view::IFC2X3_COBIE_VIEW_MODE_ID).window_kind_def(main::definition()).default_layout(view::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn create_viewer_builds_a_definition_for_the_viewer_role() {
        let def = create_ifc2x3_cobie_viewer();
    ...
    async fn viewer_dialect_matches_the_artifact_coordinate() {
        assert_eq!(<Ifc2x3CobieViewer as ArtifactViewer>::DIALECT, IFC2X3_COBIE_DIALECT);
    }

    #[semio_framework_async_macros::async_test]
    async fn viewer_never_mutates_the_document_or_draft_store() {
        semio_framework_plugin::testkit::assert_viewer_never_mutates::<Ifc2x3CobieViewer>();
    }
}
//#endregion 🧪️Tests
