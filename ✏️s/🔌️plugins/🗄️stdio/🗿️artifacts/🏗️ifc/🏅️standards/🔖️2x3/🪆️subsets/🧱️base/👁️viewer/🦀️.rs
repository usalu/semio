...
pub fn create_ifc2x3_any_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(IFC2X3_ANY_DIALECT).document(["stdio", "ifc2x3"]).icon_id("box").mode_def(view::definition()).default_mode_id(view::IFC2X3_ANY_VIEW_MODE_ID).window_kind_def(main::definition()).default_layout(view::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn viewer_never_mutates_the_document_or_draft_store() {
        semio_framework_plugin::testkit::assert_viewer_never_mutates::<Ifc2x3AnyViewer>().await;
    }
}
//#endregion 🧪️Tests
