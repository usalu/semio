...
pub fn render(document: &Ifc2x3Snapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = pack::json_to_string(&pack::json_array([pack::json!({ "id": IFC2X3_COBIE_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(IFC2X3_COBIE_VIEW_FALLBACK_MESH_KIND) })]));
    let view = MeshView {
