...
impl protocol::OpBinary for Ifc4AnyEditCommand {
    const TOOL_JOB_IDS: &'static [&'static str] = IFC4_ANY_DOCUMENT_SCHEMA_EXAMPLE_TOOL_IDS;
...
pub fn create_ifc4_any_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(IFC4_ANY_DIALECT).document(["stdio", "ifc4"]).icon_id("box").mode_def(edit::definition()).default_mode_id(edit::IFC4_ANY_EDIT_MODE_ID).window_kind_def(main::definition()).default_layout(edit::layout()).action_with(semio_s_artifact_stdio_contract::set_active_example_action())
