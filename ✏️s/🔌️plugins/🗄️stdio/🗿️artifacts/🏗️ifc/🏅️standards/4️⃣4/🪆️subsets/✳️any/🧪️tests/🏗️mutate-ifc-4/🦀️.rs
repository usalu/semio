...
/// aware, mirroring the same `IfcMutation::inverse()` semantics `../../🏅️standards/4️⃣4/🪆️subsets/
...
mod subject {
    ...
    /// (`../../🏅️standards/4️⃣4/🪆️subsets/✳️any/🦀️oracle.rs`'s own `value_from_json`),
    ...
    fn value_from_json(value: &Json) -> Result<IfcValue, String> {
        match str_field(value, "t")?.as_str() {
            ...
            // 📎️ Mirrors the oracle dispatcher's own `value_from_json` (`../../🏅️standards/4️⃣4/
    ...
    /// `📑️mutate-pdf-1-7`/`📐️mutate-step-ap214`'s own oracles use, needed here since a full
    ...
    fn mutation_from_spec(spec: &Json, base: &IfcSnapshot) -> Result<IfcMutation, String> {
        ...
        Ok(match kind.as_str() {
            ...
            // standards/4️⃣4/🪆️subsets/✳️any/🔣️oracle.json` catalog, which still declares it).
