//! 👁️ IFC 4 viewer — the read-only counterpart of `✏️editor` for this subset (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `Ifc4AnyViewer`
//! implements `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<Ifc4AnyViewer>`
...
use crate::viewer::ifc4_any::modes::view;
use crate::viewer::ifc4_any::modes::view::windows::main;
...
impl protocol::OpBinary for Ifc4AnyViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(Ifc4AnyViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
pub struct Ifc4AnyViewer;

impl ArtifactViewer for Ifc4AnyViewer {
    ...
    type Command = Ifc4AnyViewCommand;
...
pub fn create_ifc4_any_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(IFC4_ANY_DIALECT).document(["stdio", "ifc4"]).icon_id("box").mode_def(view::definition()).default_mode_id(view::IFC4_ANY_VIEW_MODE_ID).window_kind_def(main::definition()).default_layout(view::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn create_viewer_builds_a_definition_for_the_viewer_role() {
        let def = create_ifc4_any_viewer();
    ...
    async fn viewer_dialect_matches_the_artifact_coordinate() {
        assert_eq!(<Ifc4AnyViewer as ArtifactViewer>::DIALECT, IFC4_ANY_DIALECT);
    }

    #[semio_framework_async_macros::async_test]
    async fn viewer_never_mutates_the_document_or_draft_store() {
        semio_framework_plugin::testkit::assert_viewer_never_mutates::<Ifc4AnyViewer>().await;
    }
}
//#endregion 🧪️Tests
