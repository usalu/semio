...
Feature: Apply every typed raw-binary mutation to a real-world byte buffer
  ...
  matter here precisely because this subset does NOT parse structure: a byte-range replacement at offset 6 below
  ...
  @level-exhaustive
  @mode-error
  Scenario Outline: An invalid byte-range replacement fails cleanly — <id>
    Given the real input buffer shared://🏘️abbau-aufbau-masterarbeit-grundriss/🖼️.jpg
    When the replace-byte-range mutation is attempted with its parameters
      """
      {"kind": "replace-byte-range", "params": <params>}
      """
