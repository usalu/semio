...
//! `👁️viewer`/`✏️editor`/`📚️examples` children unchanged (mounted elsewhere, via `🦀️.rs`) —
//! this file only ASSEMBLES their declaration-shaped surface; it does not remount them (the
//! `#[path]` mounts stay `🦀️.rs`'s job per design.md §1's "owner mounts children" rule, and
...
/// empty by the carrier law (see `🚪️io/🦀️.rs`'s `io()` doc comment), `examples` carries
