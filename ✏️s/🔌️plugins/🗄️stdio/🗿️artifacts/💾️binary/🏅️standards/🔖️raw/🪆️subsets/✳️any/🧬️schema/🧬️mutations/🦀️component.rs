...
/// the `binary-raw-any` mutation catalog (`../../🧪️oracle/🔣️component.json`) declares and the
...
pub const KINDS: &[&str] = &["no-mutation", "set-snapshot", "splice", "append-bytes", "truncate-at"];
...
mod tests {
    ...
    /// (`../../🧪️oracle/🔣️component.json`'s `binary-raw-any` catalog `kinds`) is checked by the
    ...
    async fn kinds_cover_every_variant() {
        fn kind_of(mutation: &BinaryMutation) -> &'static str {
            match mutation {
                BinaryMutation::NoMutation => "no-mutation",
                BinaryMutation::SetSnapshot { .. } => "set-snapshot",
                BinaryMutation::Splice { .. } => "splice",
                BinaryMutation::AppendBytes { .. } => "append-bytes",
                BinaryMutation::TruncateAt { .. } => "truncate-at",
            }
        }
        ...
        assert_eq!(KINDS.len(), 5, "binary-raw-any declares 5 BinaryMutation variants");
    }
