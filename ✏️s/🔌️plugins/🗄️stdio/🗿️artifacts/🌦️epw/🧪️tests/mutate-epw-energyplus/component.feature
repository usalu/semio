...
Feature: Apply every typed EnergyPlus EPW mutation to a real weather file
  ...
  `set-ground-temperatures`/`set-holidays-dst`/`set-comments-1`/`set-comments-2`/`set-data-periods`)
  ...
  Scenario Outline: Apply <id> to the real weather file's header
    ...
    Examples:
      ...
      | set-comments-1                | {"value": "COMMENTS 1,Wave 7 mutation test comment"} |
      | set-comments-2                | {"value": "COMMENTS 2,Wave 7 mutation test comment"} |
