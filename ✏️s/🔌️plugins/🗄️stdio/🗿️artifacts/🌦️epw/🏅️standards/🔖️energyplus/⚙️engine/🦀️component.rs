//! ⚙️ EpwEngine — owns the real EnergyPlus Weather (EPW) text codec. LOSSLESS: all 8 header
//! lines + all 35 per-record columns (https://bigladdersoftware.com/epx/docs/9-6/auxiliary-programs/energyplus-weather-file-epw-data-dictionary.html)
//! — see `crate::artifacts::epw::standards::energyplus::subsets::any::schema::snapshot` module
//! doc comment for the full rationale (incl. why every column is `String`, not `f64`).
