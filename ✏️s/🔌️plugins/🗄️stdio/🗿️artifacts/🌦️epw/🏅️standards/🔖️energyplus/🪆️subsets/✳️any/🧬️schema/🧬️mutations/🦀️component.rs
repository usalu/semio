//! 🧬️ EpwMutation — 🚧 scaffolded by W1b: a single `SetSnapshot` full-replace mutation
...
pub enum EpwMutation {
    ...
    SetSnapshot { snapshot: EpwSnapshot },
}

impl Mutation<EpwSnapshot> for EpwMutation {
    type Diff = EpwDiff;

    fn diff(&self, _base: &EpwSnapshot) -> Self::Diff {
        match self {
            EpwMutation::NoMutation => EpwDiff::default(),
            EpwMutation::SetSnapshot { snapshot } => EpwDiff { replacement: Some(snapshot.clone()) },
        }
    }
...
impl protocol::OpText for EpwMutation {
...
mod tests {
    ...
    fn mutation_diff_law_set_snapshot_matches_diff() {
        let base = EpwSnapshot::default();
        let mut next = EpwSnapshot::default();
        next.schema = format!("{}-mutated", base.schema);
        let mutation = EpwMutation::SetSnapshot { snapshot: next.clone() };
        let diff = <EpwMutation as Mutation<EpwSnapshot>>::diff(&mutation, &base);
        assert_eq!(<EpwDiff as protocol::MutationDiff<EpwSnapshot>>::apply(&diff, &base), next);
        let inv = <EpwMutation as Mutation<EpwSnapshot>>::inverse(&mutation, &base);
        assert_eq!(inv.len(), 1);
        let mut round = next.clone();
        let _ = apply_epw_mutation(&mut round, &inv[0]);
    ...
    fn op_text_binary_roundtrip_law() {
        let base = EpwSnapshot::default();
        for m in [EpwMutation::NoMutation, EpwMutation::SetSnapshot { snapshot: base.clone() }] {
