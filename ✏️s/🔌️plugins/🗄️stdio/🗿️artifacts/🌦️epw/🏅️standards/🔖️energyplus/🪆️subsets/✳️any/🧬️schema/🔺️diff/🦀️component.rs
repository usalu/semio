//! 🔺️ EpwDiff — 🚧 scaffolded by W1b: a minimal, genuinely law-abiding full-replace
...
pub struct EpwDiff {
    ...
    pub replacement: Option<EpwSnapshot>,
}
...
pub fn diff_set_snapshot(base: &EpwSnapshot, snapshot: &EpwSnapshot) -> EpwDiff {
    <EpwDiff as DiffAlgebra<EpwSnapshot>>::between(base, snapshot)
}
//#endregion 🔖️Diff
...
mod tests {
    ...
    fn field_sweep_full_replace_round_trip() {
        let a = EpwSnapshot::default();
        let mut b = EpwSnapshot::default();
        b.schema = format!("{}-swept", a.schema);
        let d = <EpwDiff as DiffAlgebra<EpwSnapshot>>::between(&a, &b);
        assert_eq!(d.apply(&a), b);
        let inv = d.inverse(&a);
        assert_eq!(inv.apply(&d.apply(&a)), a);
        assert!(<EpwDiff as DiffAlgebra<EpwSnapshot>>::between(&a, &a).is_empty());
    }

    #[test]
    fn absorb_composes_two_sequential_diffs() {
        let a = EpwSnapshot::default();
        let mut mid = EpwSnapshot::default();
        mid.schema = format!("{}-mid", a.schema);
        let mut after = EpwSnapshot::default();
        after.schema = format!("{}-after", a.schema);
        let mut d1 = <EpwDiff as DiffAlgebra<EpwSnapshot>>::between(&a, &mid);
        let d2 = <EpwDiff as DiffAlgebra<EpwSnapshot>>::between(&mid, &after);
