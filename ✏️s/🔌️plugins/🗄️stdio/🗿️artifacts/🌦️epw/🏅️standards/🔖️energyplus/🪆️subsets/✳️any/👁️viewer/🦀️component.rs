...
impl ArtifactViewer for EpwViewer {
    ...
    async fn initial_snapshot() -> EpwSnapshot {
        EpwSnapshot::default()
    }
...
//#region 🔖️Manifest
pub async fn create_epw_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(EPW_VIEWER_DIALECT)
        .await.document(["stdio", "epw"])
        .await.icon_id("cloud-sun")
        .await.mode_def(view::definition().await)
        .await.default_mode_id(view::EPW_VIEW_MODE_ID)
