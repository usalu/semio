//! 🧬️ TsvSnapshot — records + trailing_newline (IANA TSV has no quoting) — REAL, complete parse today.
//! 🚧 scaffolded by W1b: minimal honest fields only (not the full spec shape) — full
//! implementation lands in W2/W3.

/// 📦️ Owned by `tsv`: `TsvRecord`. IANA text/tab-separated-values has NO quoting/escaping —
/// own types, deliberately not merged into `csv`.
#[derive(Clone, Debug, Default, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct TsvRecord { #[serde(default)] pub fields: Vec<String> }
...
mod tests {
    ...
    fn json_pack_round_trips() {
        let snap = TsvSnapshot::default();
        let bytes = <TsvSnapshot as store::ArtifactPack>::encode_pack(&snap);
        let back = <TsvSnapshot as store::ArtifactPack>::decode_pack(&bytes).expect("decode");
        assert_eq!(snap, back);
    }
    ...
    fn dsl_text_round_trips() {
        let snap = TsvSnapshot::default();
        let text = <TsvSnapshot as store::ArtifactDsl>::print_dsl(&snap);
        let back = <TsvSnapshot as store::ArtifactDsl>::parse_dsl(&text).expect("parse");
        assert_eq!(snap, back);
    }
}
//#endregion 🔖️Tests
