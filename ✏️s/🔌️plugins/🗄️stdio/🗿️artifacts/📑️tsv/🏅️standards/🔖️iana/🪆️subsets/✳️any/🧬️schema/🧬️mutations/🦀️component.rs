//! 🧬️ TsvMutation — 🚧 scaffolded by W1b: a single `SetSnapshot` full-replace mutation
...
pub enum TsvMutation {
    ...
    SetSnapshot { snapshot: TsvSnapshot },
}

impl Mutation<TsvSnapshot> for TsvMutation {
    type Diff = TsvDiff;

    fn diff(&self, _base: &TsvSnapshot) -> Self::Diff {
        match self {
            TsvMutation::NoMutation => TsvDiff::default(),
            TsvMutation::SetSnapshot { snapshot } => TsvDiff { replacement: Some(snapshot.clone()) },
        }
    }
...
pub fn apply_tsv_mutation(snapshot: &mut TsvSnapshot, mutation: &TsvMutation) -> TsvDiff {
    ...
    *snapshot = <TsvDiff as protocol::MutationDiff<TsvSnapshot>>::apply(&diff, snapshot);
    diff
}
//#endregion 🔖️Mutation
...
impl protocol::OpText for TsvMutation {
...
mod tests {
    ...
    fn mutation_diff_law_set_snapshot_matches_diff() {
        let base = TsvSnapshot::default();
        let mut next = TsvSnapshot::default();
        next.schema = format!("{}-mutated", base.schema);
        let mutation = TsvMutation::SetSnapshot { snapshot: next.clone() };
        let diff = <TsvMutation as Mutation<TsvSnapshot>>::diff(&mutation, &base);
        assert_eq!(<TsvDiff as protocol::MutationDiff<TsvSnapshot>>::apply(&diff, &base), next);
        let inv = <TsvMutation as Mutation<TsvSnapshot>>::inverse(&mutation, &base);
        assert_eq!(inv.len(), 1);
        let mut round = next.clone();
        let _ = apply_tsv_mutation(&mut round, &inv[0]);
    ...
    fn op_text_binary_roundtrip_law() {
        let base = TsvSnapshot::default();
        for m in [TsvMutation::NoMutation, TsvMutation::SetSnapshot { snapshot: base.clone() }] {
