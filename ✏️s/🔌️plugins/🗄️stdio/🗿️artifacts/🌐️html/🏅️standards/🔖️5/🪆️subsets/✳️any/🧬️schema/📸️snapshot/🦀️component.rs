//! 🧬️ HtmlSnapshot — doctype detection + the rest of the document retained as raw text (tokenizer lands in W3).
...
impl store::ArtifactDsl for HtmlSnapshot {
    ...
    fn envelope_id() -> &'static str { STDIO_HTML_DOCUMENT_SCHEMA }
...
mod tests {
    ...
    fn json_pack_round_trips() {
        ...
        let bytes = <HtmlSnapshot as store::ArtifactPack>::encode_pack(&snap);
        let back = <HtmlSnapshot as store::ArtifactPack>::decode_pack(&bytes).expect("decode");
        assert_eq!(snap, back);
    }
    ...
    fn dsl_text_round_trips() {
        let snap = HtmlSnapshot::default();
        let text = <HtmlSnapshot as store::ArtifactDsl>::print_dsl(&snap);
        let back = <HtmlSnapshot as store::ArtifactDsl>::parse_dsl(&text).expect("parse");
        assert_eq!(snap, back);
    }
}
//#endregion 🔖️Tests
