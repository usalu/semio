//! 🔺️ HtmlDiff — 🚧 scaffolded by W1b: a minimal, genuinely law-abiding full-replace
...
pub struct HtmlDiff {
    ...
    pub replacement: Option<HtmlSnapshot>,
}
...
pub fn diff_set_snapshot(base: &HtmlSnapshot, snapshot: &HtmlSnapshot) -> HtmlDiff {
    <HtmlDiff as DiffAlgebra<HtmlSnapshot>>::between(base, snapshot)
}
//#endregion 🔖️Diff
...
mod tests {
    ...
    fn field_sweep_full_replace_round_trip() {
        let a = HtmlSnapshot::default();
        let mut b = HtmlSnapshot::default();
        b.schema = format!("{}-swept", a.schema);
        let d = <HtmlDiff as DiffAlgebra<HtmlSnapshot>>::between(&a, &b);
    ...
    fn absorb_composes_two_sequential_diffs() {
        let a = HtmlSnapshot::default();
        let mut mid = HtmlSnapshot::default();
        mid.schema = format!("{}-mid", a.schema);
        let mut after = HtmlSnapshot::default();
        after.schema = format!("{}-after", a.schema);
        let mut d1 = <HtmlDiff as DiffAlgebra<HtmlSnapshot>>::between(&a, &mid);
        let d2 = <HtmlDiff as DiffAlgebra<HtmlSnapshot>>::between(&mid, &after);
