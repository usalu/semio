...
doc: |
  ...
  footer, see the sibling `📡️.protocol.semio`) wrapping a `payload` segment whose bytes
  are the canonical UTF-8 HTML5 document text -- the exact same grammar as
  `../📝️text/📖️.grammar.semio`'s `document` production. Not an opaque blob: the payload's
