//! 🧬️ HtmlMutation — 🚧 scaffolded by W1b: a single `SetSnapshot` full-replace mutation
...
pub enum HtmlMutation {
    ...
    SetSnapshot { snapshot: HtmlSnapshot },
}

impl Mutation<HtmlSnapshot> for HtmlMutation {
    type Diff = HtmlDiff;

    fn diff(&self, _base: &HtmlSnapshot) -> Self::Diff {
        match self {
            HtmlMutation::NoMutation => HtmlDiff::default(),
            HtmlMutation::SetSnapshot { snapshot } => HtmlDiff { replacement: Some(snapshot.clone()) },
        }
    }
...
pub fn apply_html_mutation(snapshot: &mut HtmlSnapshot, mutation: &HtmlMutation) -> HtmlDiff {
    let diff = <HtmlMutation as Mutation<HtmlSnapshot>>::diff(mutation, snapshot);
    *snapshot = <HtmlDiff as protocol::MutationDiff<HtmlSnapshot>>::apply(&diff, snapshot);
    diff
}
//#endregion 🔖️Mutation
...
impl protocol::OpText for HtmlMutation {
...
mod tests {
    ...
    fn mutation_diff_law_set_snapshot_matches_diff() {
        ...
        let mut next = HtmlSnapshot::default();
        next.schema = format!("{}-mutated", base.schema);
        let mutation = HtmlMutation::SetSnapshot { snapshot: next.clone() };
        let diff = <HtmlMutation as Mutation<HtmlSnapshot>>::diff(&mutation, &base);
        assert_eq!(<HtmlDiff as protocol::MutationDiff<HtmlSnapshot>>::apply(&diff, &base), next);
        let inv = <HtmlMutation as Mutation<HtmlSnapshot>>::inverse(&mutation, &base);
        assert_eq!(inv.len(), 1);
        let mut round = next.clone();
        let _ = apply_html_mutation(&mut round, &inv[0]);
    ...
    fn op_text_binary_roundtrip_law() {
        let base = HtmlSnapshot::default();
        for m in [HtmlMutation::NoMutation, HtmlMutation::SetSnapshot { snapshot: base.clone() }] {
