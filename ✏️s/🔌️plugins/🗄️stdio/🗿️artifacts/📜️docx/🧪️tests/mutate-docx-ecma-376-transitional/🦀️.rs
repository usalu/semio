//! 🦀️ DOCX ECMA-376 ✳️transitional exhaustive conformance-class mutation case — Rust adapter.
//!
//! The input is the real committed `📜️example-readme.docx`, a 7-part OPC package that ALREADY
//! satisfies this class on all three of its axes — Transitional WordprocessingML namespace, no
//! strict-family namespace anywhere, no contradicting `conformance="strict"`. This case is the
...
//! wants it. The evidence stops where ISO/IEC 29500-4 does: VML and `mc:AlternateContent` are legal
//! Transitional markup, so this case polices neither and claims nothing about them.

use semio_repo_test_host::{Adapter, Context, Json, Outcome};
use semio_s_plugin_stdio_test_oracle::artifacts::docx::standards::v_ecma_376::subsets::transitional::{oracle_apply_mutation, oracle_arrange, oracle_inverse_spec, oracle_round_trip, project_package, KINDS};
...
/// unzipping all three committed OOXML fixtures, so that kind runs on the real 7-part package after
/// the reference implementation has independently stamped one onto it. The other five kinds read
/// the committed bytes untouched.
...
/// three-axis Transitional projection of the real package, or its scenario would pass whether or
/// not the mutation ran. Nothing is exempt — every declared kind is one axis of
/// `check_transitional_conformance`, and the profile declares no writer freedom for any of them.
...
/// bytes: for `remove-conformance-attribute` the document to be restored is the stamped one, which
