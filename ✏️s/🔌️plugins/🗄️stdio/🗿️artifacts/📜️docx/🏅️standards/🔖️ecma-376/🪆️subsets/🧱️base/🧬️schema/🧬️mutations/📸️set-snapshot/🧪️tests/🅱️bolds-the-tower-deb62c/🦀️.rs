...
async fn inverse_restores_before() {
    ...
    assert!(matches!(inverse[0], DocxMutation::SetSnapshot(_)), "set-snapshot/bolds-the-tower-run-of-the-opening-paragraph: the undo step must itself be a SetSnapshot carrying the pre-state");
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: DocxSnapshot = dsl::json::from_json_str(text).expect("snapshot decodes");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "set-snapshot/bolds-the-tower-run-of-the-opening-paragraph: produced diff differs from the committed 🔺️diff/🔣️.json");
...
async fn committed_diff_is_canonical() {
    let decoded: DocxDiff = dsl::json::from_json_str(DIFF).expect("committed diff decodes");
