//! 🎹️ DocxTransitionalComposer (ecma-376/✳️transitional) — reads the same sources the ✳️any
//! subset does (native `stdio.docx` ecma-376, plus its `zip`/`xml` DAG deps), delegates the actual
//! parse to the ✳️any composer, then HARD-GATES the `transitional` dialect stamp on real ISO/IEC
//! 29500-4:2016 Transitional conformance. A hard violation (missing transitional namespace, any
//! strict-family namespace/relationship present) fails composition outright with specific
//! `Diagnostic`s naming what's wrong; the soft one (`conformance="strict"` present) passes through
//! as an advisory diagnostic on the successful `Composition`. In practice a document produced by
//! the ✳️any subset's own `engine::build_minimal_docx` is already transitional-conformant by
//! construction (that engine hardcodes the transitional namespace and relationship base) -- this
//! composer's hard gate exists for sources that didn't originate there (round-tripped
//! Strict-tagged bytes, hand-authored packages, …).
//!
//! Also registers this dialect's `SubsetValidator` (D5's generic validate-on-build hook) — the
//! SAME `check_transitional_conformance` function backs both.
...
//#region 🔖️Composer
pub struct DocxTransitionalComposer;

impl ArtifactComposer for DocxTransitionalComposer {
...
mod tests {
    ...
    use crate::artifacts::docx::standards::v_ecma_376::subsets::transitional::analyzer::CODE_STRICT_NS_PRESENT;
    ...
    fn conforming_snapshot_composes_and_stamps_transitional() {
        ...
        let composed = DocxTransitionalComposer::compose(&sources).expect("clean transitional document must compose");
    ...
    fn strict_namespace_present_fails_compose_with_real_diagnostic() {
        ...
        let sources = vec![ComposeSource { dialect: DIALECT_ANY, payload: AnalyzeSource::Binary(&bytes) }];
        let err = DocxTransitionalComposer::compose(&sources).expect_err("mixed-in strict namespace must not stamp transitional");
