...
//! built from the framework `DocumentWindowKit` (contract §2.6). One page per top-level block —
//! `Paragraph` blocks render as their joined run text; `Table` blocks render as a flattened
//! row/cell text summary (rows joined by newlines, cells within a row joined by ` | `), a read
