//! 🎹️ DocxComposer (raw/✳️any at ecma-376) — analyzer + builder glued. Reads native
//! `stdio.docx` sources (plus its DAG dependencies: zip, xml), writes one `stdio.docx` (ecma-376/✳️any) snapshot.
...
use crate::artifacts::docx::standards::v_ecma_376::subsets::any::analyzer::DocxAnalyzer;
...
impl ArtifactComposer for DocxComposer {
    ...
    fn compose(sources: &[ComposeSource]) -> Result<Composition<Self::Snapshot>, ComposeError> {
        ...
        let native: Vec<AnalyzeSource<'_>> = sources
            ...
            .collect();
        if native.is_empty() {
            return Err(ComposeError { message: "DocxComposer: no source in a known read dialect".into(), diagnostics: Vec::new() });
        }
        let analysis = DocxAnalyzer::analyze(&native);
        let snapshot = analysis.parts.snapshot.ok_or_else(|| ComposeError {
            message: "DocxComposer: analysis produced no snapshot".into(),
