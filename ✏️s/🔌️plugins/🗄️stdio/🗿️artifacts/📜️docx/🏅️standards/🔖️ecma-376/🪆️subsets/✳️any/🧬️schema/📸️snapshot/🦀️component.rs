//! 🧬️ DocxSnapshot — OOXML zip parts.
...
impl Default for DocxSnapshot {
    fn default() -> Self {
        Self { schema: STDIO_DOCX_DOCUMENT_SCHEMA.into(), entries: Vec::new() }
    }
}

impl store::DocumentDsl for DocxSnapshot {
    ...
    fn parse_dsl(text: &str) -> Result<Self, store::TextError> {
        ...
        crate::artifacts::docx::engine::decode_docx(&bytes).map_err(|e| store::TextError::new(e, dsl::TextSpan::at(1, 1)))
    }
...
impl store::DocumentPack for DocxSnapshot {
    fn encode_pack_with(&self, options: &store::PackEncodeOptions) -> Result<Vec<u8>, store::PackError> {
        let _ = options;
        let raw = crate::artifacts::docx::engine::encode_docx(self).map_err(|e| store::PackError::Schema(e))?;
        let envelope = store::semio_format::SemioEnvelope::from_envelope_id(
            <Self as store::DocumentDsl>::envelope_id(),
    ...
    fn decode_pack_with(bytes: &[u8], options: &store::PackDecodeOptions) -> Result<Self, store::PackError> {
        ...
        let _ = options;
        crate::artifacts::docx::engine::decode_docx(&inner).map_err(|e| store::PackError::Schema(e))
    }
}
