...
export const RECIPES: Recipe[] = [
  ...
  { id: "insert-block-rejected-invalid-index", mutation: "insert-block", outcome: "rejected", before: BASE_MODEL, notes: "Inserting at index 99 of a 2-block body is outside the final collection — `apply_indexed`'s `mutation.apply.invalid-index`. No legal after-state exists, so only `⬅️before.docx` is carried." },
...
const FIXTURE_DIRECTORY_NAMES: Readonly<Record<string, string>> = {
  "bolds-the-tower-run-of-the-opening-paragraph": "💪️bolds-the-tower-run-of-the-opening-paragraph",
  "insert-block-appends-a-pricing-table": "📊️insert-block-appends-a-pricing-table",
  "insert-block-rejected-invalid-index": "🚧️insert-block-rejected-invalid-index",
  "insert-style-adds-a-quote-style": "💬️insert-style-adds-a-quote-style",
  "insert-style-rejected-duplicate-id": "👯️insert-style-rejected-duplicate-id",
  "no-mutation-no-op": "⏸️no-mutation-no-op",
  "remove-block-drops-the-closing-paragraph": "✂️remove-block-drops-the-closing-paragraph",
  "remove-block-rejected-missing-index": "🕳️remove-block-rejected-missing-index",
  "remove-part-drops-core-properties": "🧹️remove-part-drops-core-properties",
  "remove-part-rejected-missing-path": "🧭️remove-part-rejected-missing-path",
  "remove-style-drops-the-quote-style": "🗨️remove-style-drops-the-quote-style",
  "remove-style-rejected-missing-id": "🪪️remove-style-rejected-missing-id",
  "set-block-content-no-op-unchanged-content": "💤️set-block-content-no-op-unchanged-content",
  "set-block-content-replaces-the-closing-paragraph": "📝️set-block-content-replaces-the-closing-paragraph",
  "set-part-adds-core-properties": "🧩️set-part-adds-core-properties",
  "set-part-no-op-identical-content": "🟰️set-part-no-op-identical-content",
  "set-run-formatting-italicizes-the-closing-paragraph": "🪄️set-run-formatting-italicizes-the-closing-paragraph",
  ...
  "set-style-name-renames-the-body-style": "🔤️set-style-name-renames-the-body-style",
};
const BEFORE_FILE = "⬅️before.docx";
const AFTER_FILE = "➡️after.docx";
//#endregion 🍳️Recipes
...
async function generateOne(id: string, outDir: string): Promise<void> {
  ...
  const dir = join(outDir, FIXTURE_DIRECTORY_NAMES[id]!);
  mkdirSync(dir, { recursive: true });
  writeFileSync(join(dir, BEFORE_FILE), await buildDocx(recipe.before));
  if (recipe.outcome !== "rejected") {
    if (!recipe.after) throw new Error(`recipe ${id} is declared ${recipe.outcome} but has no after state`);
    writeFileSync(join(dir, AFTER_FILE), await buildDocx(recipe.after));
  }
}
...
async function main(argv: readonly string[]): Promise<number> {
  ...
  for (const id of ids) {
    ...
    console.log(`[docx generator] ${id} -> ${join(outDir, FIXTURE_DIRECTORY_NAMES[id]!)}`);
  }
  return 0;
}
