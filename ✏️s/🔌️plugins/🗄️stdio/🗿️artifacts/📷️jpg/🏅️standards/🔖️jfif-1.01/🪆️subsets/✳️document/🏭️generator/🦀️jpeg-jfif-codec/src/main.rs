...
//!
//! Two subcommands:
//!   build   <recipe-id> <out-dir>   — writes <out-dir>/<recipe-id>/before.jpg [and after.jpg]
...
fn cmd_build(id: &str, out_dir: &str) -> i32 {
    ...
    fs::write(dir.join("before.jpg"), &before).expect("write before.jpg");
    fs::write(dir.join("after.jpg"), &after).expect("write after.jpg");
    eprintln!("[jpeg-jfif-codec] {id}: before.jpg ({} bytes) + after.jpg ({} bytes) -> {}", before.len(), after.len(), dir.display());
    0
}
    ...
    let code = match args.get(1).map(String::as_str) {
        Some("build") => {
            let (Some(id), Some(out_dir)) = (args.get(2), args.get(3)) else {
                eprintln!("usage: jpeg-jfif-codec build <recipe-id> <out-dir>");
                std::process::exit(2);
            };
