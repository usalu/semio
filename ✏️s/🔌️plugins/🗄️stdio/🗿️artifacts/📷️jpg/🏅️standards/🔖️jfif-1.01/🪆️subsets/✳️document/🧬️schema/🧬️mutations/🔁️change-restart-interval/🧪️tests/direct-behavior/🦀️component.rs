//! 🧪️ `change-restart-interval` fixture — `direct-behavior`.
//!
...
fn mutation() -> JpgMutation {
    serde_json::from_str(MUTATION).expect("committed change-restart-interval payload decodes")
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-restart-interval/direct-behavior: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
fn op_codecs_round_trip() {
    ...
    assert_eq!(bytes[1], super::binary::BINARY_TAG, "change-restart-interval/direct-behavior: the binary frame must carry this schema's tag");
