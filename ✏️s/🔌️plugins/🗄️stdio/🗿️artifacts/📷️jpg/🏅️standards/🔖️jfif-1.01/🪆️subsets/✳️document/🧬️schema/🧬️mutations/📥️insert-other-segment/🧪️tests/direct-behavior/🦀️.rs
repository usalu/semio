//! 🧪️ `insert-other-segment` fixture — `direct-behavior`.
//!
...
fn mutation() -> JpgMutation {
    serde_json::from_str(MUTATION).expect("committed insert-other-segment payload decodes")
}
...
fn applies_to_committed_after() {
    let base = before();
    let produced = mutation().diff(&base).diff().apply(&base).expect("insert-other-segment applies to its committed before-snapshot");
...
fn inverse_restores_before() {
    ...
    let mut current = payload.diff(&base).diff().apply(&base).expect("forward insert-other-segment applies");
    for step in payload.inverse(&base) {
        current = step.diff(&current).diff().apply(&current).expect("the insert-other-segment inverse step applies");
    }
...
fn op_codecs_round_trip() {
    ...
    assert_eq!(bytes[1], super::binary::BINARY_TAG, "insert-other-segment/direct-behavior: the binary frame must carry this schema's tag");
