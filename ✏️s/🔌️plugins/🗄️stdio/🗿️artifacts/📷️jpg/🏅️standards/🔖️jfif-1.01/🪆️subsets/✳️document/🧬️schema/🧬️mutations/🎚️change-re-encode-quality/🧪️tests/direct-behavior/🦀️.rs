//! 🧪️ `change-re-encode-quality` fixture — `direct-behavior`.
//!
...
fn mutation() -> JpgMutation {
    serde_json::from_str(MUTATION).expect("committed change-re-encode-quality payload decodes")
}
...
fn applies_to_committed_after() {
    let base = before();
    let produced = mutation().diff(&base).diff().apply(&base).expect("change-re-encode-quality applies to its committed before-snapshot");
    assert_eq!(produced, expected_after(), "change-re-encode-quality/direct-behavior: applied state differs from the committed after-snapshot");
}
...
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(raised.is_empty(), "change-re-encode-quality/direct-behavior: declared applied, but dispatch raised {raised:?}"),
        _ => assert!(!raised.is_empty(), "change-re-encode-quality/direct-behavior: declared {status}, but dispatch raised nothing"),
    }
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "change-re-encode-quality/direct-behavior: committed diff did not carry before to after");
}
...
fn op_codecs_round_trip() {
    let payload = mutation();
    assert_eq!(JpgMutation::parse_op(&payload.print_op()).expect("the text op parses"), payload, "change-re-encode-quality/direct-behavior: the text op form does not round-trip");
    let bytes = payload.encode_op().expect("the binary op encodes");
    assert_eq!(bytes[1], super::binary::BINARY_TAG, "change-re-encode-quality/direct-behavior: the binary frame must carry this schema's tag");
    assert_eq!(JpgMutation::decode_op(&bytes).expect("the binary op decodes"), payload, "change-re-encode-quality/direct-behavior: the binary op form does not round-trip");
    assert!(JpgMutation::decode_op(&bytes[..1]).is_err(), "change-re-encode-quality/direct-behavior: a truncated binary frame must be rejected");
}
