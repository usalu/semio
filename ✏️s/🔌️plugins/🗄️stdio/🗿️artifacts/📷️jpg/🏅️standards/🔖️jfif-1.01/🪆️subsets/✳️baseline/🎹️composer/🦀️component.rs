//! 🎹️ JpgBaselineComposer (jfif-1.01/✳️baseline) — reads the same sources the ✳️any subset
//! does (native `stdio.jpg` jfif-1.01, plus its `binary` DAG dep), delegates the actual parse to
//! the ✳️any composer, then HARD-GATES the `baseline` dialect stamp on real ITU-T T.81 / ISO
//! 10918-1 Annex F baseline-sequential-DCT conformance. A hard violation (no retained frame,
//! non-SOF0 marker, non-8-bit precision, arithmetic-conditioning present) fails composition
//! outright with specific `Diagnostic`s naming what's wrong; a soft one (too many Huffman tables,
//! unusual component/sampling layout) passes through as an advisory diagnostic on the successful
//! `Composition`.
//!
//! In practice this dialect stamp is "conforming by construction" for anything the composer's own
//! `✳️any` delegate actually decodes: `⚙️engine::decode_jpg` already only ever returns `Ok` for a
//! SOF0/8-bit/Huffman-only stream (every other SOFn and any DAC segment is a typed
//! `JpgError::Unsupported` — decode itself never succeeds otherwise). The hard gate here still
//! exists (rather than skipping straight to `Ok`) because `check_baseline_conformance` is also
//! the SAME function the registered `SubsetValidator` below re-runs against arbitrary wire
//! payloads, and because a snapshot could in principle reach this composer via a `SetSnapshot`
//! mutation that never went through `decode_jpg` at all (see `🏗️builder`).
//!
//! Also registers this dialect's `SubsetValidator` (D5's generic validate-on-build hook, see
//! `🧰️framework/🔨️modules/🚪️io/🦀️component.rs`) — the SAME `check_baseline_conformance` function
//! backs both: the hard gate here runs pre-serialization against the typed `JpgSnapshot`
...
use crate::artifacts::jpg::standards::v_jfif_1_01::subsets::any::composer::JpgComposer as JpgAnyComposer;
...
//#region 🔖️Composer
pub struct JpgBaselineComposer;

impl ArtifactComposer for JpgBaselineComposer {
    ...
    fn compose(sources: &[ComposeSource]) -> Result<Composition<Self::Snapshot>, ComposeError> {
        ...
        if !hard.is_empty() {
            ...
            return Err(ComposeError {
                message: format!("baseline conformance violated: {} hard issue(s) -- not stamping the baseline dialect", hard.len()),
                diagnostics: all,
            });
        }
...
/// (`crate::artifacts::jpg::standards::v_jfif_1_01::composer::entries()`), matching how `✳️any`'s
...
mod tests {
    ...
    fn engine_encoded_jpeg_composes_and_stamps_baseline() {
        ...
        // frame/sof_marker/huffman-table-count populated) the way `JpgBaselineComposer::compose`
        // itself would internally.
        let composed = JpgBaselineComposer::compose(&sources).expect("real baseline JPEG must compose and stamp baseline");
        assert!(composed.diagnostics.iter().all(|d| d.severity != Severity::Error), "no hard diagnostics expected: {:?}", composed.diagnostics);
        assert!(composed.snapshot.frame.is_some());
        assert_eq!(composed.snapshot.sof_marker, crate::artifacts::jpg::standards::v_jfif_1_01::subsets::baseline::analyzer::SOF0);
    }
