...
use crate::artifacts::jpg::standards::v_jfif_1_01::subsets::any::schema::diff::{JpgComponentDiff, JpgComponentModified, JpgComponentsDiff, JpgDiff, JpgFrameChange, JpgFrameFieldsDiff, JpgHuffmanTableAdded, JpgHuffmanTableKey, JpgHuffmanTablesDiff};
use crate::artifacts::jpg::standards::v_jfif_1_01::subsets::any::schema::snapshot::{JpgFrameComponent, JpgHuffmanTable, JpgSnapshot};
...
pub enum JpgBaselineMutation {
    ...
    /// 🖼️ Sets the frame marker. [`SOF0`] is baseline; 0xC1 (extended sequential), 0xC2
    /// (progressive) and 0xC9/0xCA (arithmetic) are the values `CODE_SOF_MARKER` reports.
    SetSofMarker {
        marker: u8,
    },
    /// 📏️ Sets the frame's sample precision. [`BASELINE_PRECISION`] is baseline; 12 is the extended
    /// sequential value `CODE_PRECISION` reports.
    SetSamplePrecision {
        precision: u8,
    },
    /// 🔢️ Sets whether the document carried a DAC (arithmetic-coding conditioning) segment. `true`
    /// is what `CODE_ARITHMETIC` reports; baseline is Huffman-entropy-coded only.
    SetArithmetic {
        arithmetic: bool,
    },
    /// ➕️ Adds one Huffman table at `index`, which is how the per-class count crosses the
    /// [`BASELINE_TABLES_PER_CLASS`] line `CODE_HUFFMAN_TABLE_COUNT` reports against. A table whose
    /// `(class, id)` the document already carries is left alone — this kind counts, it does not
    /// overwrite; `JpgMutation::SetHuffmanTable` is the kind that overwrites.
    ///
    /// `index` is carried rather than always appending because it is what makes the kind INVERTIBLE:
    /// the inverse of removing the second of four tables has to put it back where it was, and a
    /// vocabulary whose insertion can only append cannot express that.
    InsertHuffmanTable {
        index: usize,
        table: JpgHuffmanTable,
    },
    /// ➖️ Removes the Huffman table at `(class, id)`, bringing the per-class count back down.
    RemoveHuffmanTable {
        key: JpgHuffmanTableKey,
    },
    /// ➕️ Adds one frame component at `index`, which is how the component count crosses the
    /// four-component line `CODE_COMPONENT_SAMPLING` reports against. `index` is carried for the
    /// same invertibility reason as [`JpgBaselineMutation::InsertHuffmanTable`]'s.
    InsertFrameComponent {
        index: usize,
        component: JpgFrameComponent,
    },
    /// ➖️ Removes the frame component with id `id`.
    RemoveFrameComponent {
        id: u8,
    },
    /// 🔲️ Sets one component's horizontal and vertical sampling factors — the second axis
    /// `CODE_COMPONENT_SAMPLING` reads, which it reports outside JFIF's conventional 1..=4 range.
...
impl Mutation<JpgSnapshot> for JpgBaselineMutation {
    type Diff = JpgDiff;

    fn diff(&self, base: &JpgSnapshot) -> protocol::MutationOutcome<Self::Diff> {
        protocol::MutationOutcome::new(match self {
            JpgBaselineMutation::NoMutation => JpgDiff::default(),
            JpgBaselineMutation::SetSnapshot { snapshot } => crate::artifacts::jpg::standards::v_jfif_1_01::subsets::any::schema::diff::diff_set_snapshot(base, snapshot),
            JpgBaselineMutation::SetSofMarker { marker } => JpgDiff { sof_marker: (base.sof_marker != *marker).then_some(*marker), ..Default::default() },
            JpgBaselineMutation::SetSamplePrecision { precision } => {
            ...
            JpgBaselineMutation::SetArithmetic { arithmetic } => JpgDiff { arithmetic: (base.arithmetic != *arithmetic).then_some(*arithmetic), ..Default::default() },
            JpgBaselineMutation::InsertHuffmanTable { index, table } => {
            ...
            JpgBaselineMutation::RemoveHuffmanTable { key } => {
            ...
            JpgBaselineMutation::InsertFrameComponent { index, component: added } => {
                if component(base, added.id).is_some() {
                    JpgDiff::default()
                } else {
                    let at = (*index).min(base.frame.as_ref().map_or(0, |frame| frame.components.len()));
                    frame_diff(base, JpgFrameFieldsDiff { components: Some(JpgComponentsDiff { removed: Vec::new(), modified: Vec::new(), added: vec![crate::artifacts::jpg::standards::v_jfif_1_01::subsets::any::schema::diff::JpgComponentAdded { index: at, item: *added }] }), ..Default::default() })
                }
            }
            JpgBaselineMutation::RemoveFrameComponent { id } => {
            ...
            JpgBaselineMutation::SetComponentSampling { id, h_sampling, v_sampling } => match component(base, *id) {
    ...
    /// ↩️ Handcrafted, axis-aware inverses. A kind that SETS an axis inverts to whatever the base
    /// held for it; a kind that ADDS inverts to a removal and vice versa, and an addition or removal
    /// that found nothing to do inverts to `NoMutation` rather than to a fabricated counter-edit.
    fn inverse(&self, base: &JpgSnapshot) -> Vec<Self> {
        vec![match self {
            JpgBaselineMutation::NoMutation => JpgBaselineMutation::NoMutation,
            JpgBaselineMutation::SetSnapshot { .. } => JpgBaselineMutation::SetSnapshot { snapshot: base.clone() },
            JpgBaselineMutation::SetSofMarker { .. } => JpgBaselineMutation::SetSofMarker { marker: base.sof_marker },
            JpgBaselineMutation::SetSamplePrecision { .. } => match &base.frame {
                Some(frame) => JpgBaselineMutation::SetSamplePrecision { precision: frame.precision },
                None => JpgBaselineMutation::NoMutation,
            },
            JpgBaselineMutation::SetArithmetic { .. } => JpgBaselineMutation::SetArithmetic { arithmetic: base.arithmetic },
            JpgBaselineMutation::InsertHuffmanTable { table, .. } => {
                let key = JpgHuffmanTableKey { class: table.class, id: table.id };
                match huffman(base, &key) {
                    Some(_) => JpgBaselineMutation::NoMutation,
                    None => JpgBaselineMutation::RemoveHuffmanTable { key },
                }
            }
            ...
            JpgBaselineMutation::RemoveHuffmanTable { key } => match base.huffman_tables.iter().position(|table| table.class == key.class && table.id == key.id) {
                Some(at) => JpgBaselineMutation::InsertHuffmanTable { index: at, table: base.huffman_tables[at].clone() },
                None => JpgBaselineMutation::NoMutation,
            },
            JpgBaselineMutation::InsertFrameComponent { component: added, .. } => match component(base, added.id) {
                Some(_) => JpgBaselineMutation::NoMutation,
                None => JpgBaselineMutation::RemoveFrameComponent { id: added.id },
            },
            JpgBaselineMutation::RemoveFrameComponent { id } => match base.frame.as_ref().and_then(|frame| frame.components.iter().position(|found| found.id == *id)) {
                Some(at) => JpgBaselineMutation::InsertFrameComponent { index: at, component: base.frame.as_ref().expect("the frame was just read").components[at] },
                None => JpgBaselineMutation::NoMutation,
            },
            JpgBaselineMutation::SetComponentSampling { id, .. } => match component(base, *id) {
                Some(found) => JpgBaselineMutation::SetComponentSampling { id: *id, h_sampling: found.h_sampling, v_sampling: found.v_sampling },
                None => JpgBaselineMutation::NoMutation,
            },
        }]
    }
...
mod tests {
    use super::*;
    use crate::artifacts::jpg::standards::v_jfif_1_01::subsets::any::schema::snapshot::{JpgFrameHeader, JpgHuffmanClass};
    ...
    fn kinds_match_enum_variants_in_declaration_order() {
        let variants = [
            JpgBaselineMutation::NoMutation,
            JpgBaselineMutation::SetSnapshot { snapshot: JpgSnapshot::default() },
            JpgBaselineMutation::SetSofMarker { marker: SOF0 },
            JpgBaselineMutation::SetSamplePrecision { precision: BASELINE_PRECISION },
            JpgBaselineMutation::SetArithmetic { arithmetic: false },
            JpgBaselineMutation::InsertHuffmanTable { index: 4, table: table(JpgHuffmanClass::Dc, 2) },
            JpgBaselineMutation::RemoveHuffmanTable { key: JpgHuffmanTableKey { class: JpgHuffmanClass::Dc, id: 0 } },
            JpgBaselineMutation::InsertFrameComponent { index: 3, component: JpgFrameComponent { id: 4, h_sampling: 1, v_sampling: 1, quant_table_id: 0 } },
            JpgBaselineMutation::RemoveFrameComponent { id: 3 },
            JpgBaselineMutation::SetComponentSampling { id: 1, h_sampling: 2, v_sampling: 2 },
        ];
    ...
    fn each_kind_moves_exactly_the_axis_its_diagnostic_reports() {
        ...
        apply_jpg_baseline_mutation(&mut snapshot, &JpgBaselineMutation::SetSofMarker { marker: 0xC2 });
        assert_eq!(codes(&snapshot), vec![CODE_SOF_MARKER.to_string()]);

        let mut snapshot = conforming();
        apply_jpg_baseline_mutation(&mut snapshot, &JpgBaselineMutation::SetSamplePrecision { precision: 12 });
        assert_eq!(codes(&snapshot), vec![CODE_PRECISION.to_string()]);

        let mut snapshot = conforming();
        apply_jpg_baseline_mutation(&mut snapshot, &JpgBaselineMutation::SetArithmetic { arithmetic: true });
        assert_eq!(codes(&snapshot), vec![CODE_ARITHMETIC.to_string()]);

        let mut snapshot = conforming();
        apply_jpg_baseline_mutation(&mut snapshot, &JpgBaselineMutation::InsertHuffmanTable { index: 4, table: table(JpgHuffmanClass::Dc, 2) });
        assert_eq!(codes(&snapshot), vec![CODE_HUFFMAN_TABLE_COUNT.to_string()]);

        let mut snapshot = conforming();
        apply_jpg_baseline_mutation(&mut snapshot, &JpgBaselineMutation::SetComponentSampling { id: 1, h_sampling: 5, v_sampling: 2 });
    ...
    fn a_fifth_frame_component_is_what_crosses_the_component_count_line() {
        let mut snapshot = conforming();
        apply_jpg_baseline_mutation(&mut snapshot, &JpgBaselineMutation::InsertFrameComponent { index: 3, component: JpgFrameComponent { id: 4, h_sampling: 1, v_sampling: 1, quant_table_id: 1 } });
        assert!(codes(&snapshot).is_empty(), "four components is still inside the line");
        apply_jpg_baseline_mutation(&mut snapshot, &JpgBaselineMutation::InsertFrameComponent { index: 4, component: JpgFrameComponent { id: 5, h_sampling: 1, v_sampling: 1, quant_table_id: 1 } });
        assert_eq!(codes(&snapshot), vec![CODE_COMPONENT_SAMPLING.to_string()]);
    }
    ...
    fn every_kind_is_inverted_by_its_own_inverse() {
        let cases = [
            JpgBaselineMutation::NoMutation,
            JpgBaselineMutation::SetSnapshot { snapshot: JpgSnapshot::default() },
            JpgBaselineMutation::SetSofMarker { marker: 0xC1 },
            JpgBaselineMutation::SetSamplePrecision { precision: 12 },
            JpgBaselineMutation::SetArithmetic { arithmetic: true },
            JpgBaselineMutation::InsertHuffmanTable { index: 4, table: table(JpgHuffmanClass::Ac, 3) },
            JpgBaselineMutation::RemoveHuffmanTable { key: JpgHuffmanTableKey { class: JpgHuffmanClass::Dc, id: 1 } },
            JpgBaselineMutation::InsertFrameComponent { index: 3, component: JpgFrameComponent { id: 9, h_sampling: 1, v_sampling: 1, quant_table_id: 0 } },
            JpgBaselineMutation::RemoveFrameComponent { id: 2 },
            JpgBaselineMutation::SetComponentSampling { id: 1, h_sampling: 4, v_sampling: 4 },
        ];
    ...
    fn an_insertion_that_finds_its_target_present_produces_an_empty_diff() {
        let base = conforming();
        let already = JpgBaselineMutation::InsertHuffmanTable { index: 0, table: table(JpgHuffmanClass::Dc, 0) };
        assert_eq!(<JpgBaselineMutation as Mutation<JpgSnapshot>>::diff(&already, &base).diff(), &JpgDiff::default());
        assert_eq!(inverse_jpg_baseline_mutation(&already, &base), vec![JpgBaselineMutation::NoMutation]);

        let absent = JpgBaselineMutation::RemoveFrameComponent { id: 42 };
        assert_eq!(<JpgBaselineMutation as Mutation<JpgSnapshot>>::diff(&absent, &base).diff(), &JpgDiff::default());
        assert_eq!(inverse_jpg_baseline_mutation(&absent, &base), vec![JpgBaselineMutation::NoMutation]);
    }
}
//#endregion 🧪️Tests
