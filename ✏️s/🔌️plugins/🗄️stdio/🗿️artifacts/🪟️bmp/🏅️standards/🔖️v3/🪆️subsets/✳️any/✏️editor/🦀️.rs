...
pub fn create_bmp_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(BMP_DIALECT).document(["semio", "bmp"]).icon_id("image").mode_def(edit::definition()).default_mode_id(edit::MODE_ID).window_kind_def(main::definition()).default_layout(edit::layout()).build_definition()
}
//#endregion 🔖️Manifest
