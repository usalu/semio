...
impl DiffAlgebra<BmpSnapshot> for BmpDiff {
    ...
    fn is_empty(&self) -> bool {
        self.header_size.is_none()
            ...
            && self.palette.as_ref().is_none_or(BmpPaletteDiff::is_empty)
            && self.pixels.is_none()
    }
}
...
/// `demo_snap_a`/`demo_diff_cases` placement) so `⚙️engine/🦀️.rs`'s
...
/// `diff_grammar_conformance_law`/`protocol_walk_law` (`⚙️engine/🦀️.rs`'s
