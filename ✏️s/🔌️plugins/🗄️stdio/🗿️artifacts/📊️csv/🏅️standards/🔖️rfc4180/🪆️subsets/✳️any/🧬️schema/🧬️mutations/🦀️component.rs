...
mod tests {
    ...
    async fn kinds_match_enum_and_catalog() {
        fn kind_of(mutation: &CsvMutation) -> &'static str {
            match mutation {
                CsvMutation::NoMutation => "no-mutation",
                CsvMutation::SetSnapshot { .. } => "set-snapshot",
                CsvMutation::SetHasHeader { .. } => "set-has-header",
                CsvMutation::InsertRecord { .. } => "insert-record",
                CsvMutation::RemoveRecord { .. } => "remove-record",
                CsvMutation::SetField { .. } => "set-field",
            }
        }
        let samples = [
            CsvMutation::NoMutation,
            CsvMutation::SetSnapshot { snapshot: CsvSnapshot::default() },
            CsvMutation::SetHasHeader { has_header: false },
            CsvMutation::InsertRecord { index: 0, record: CsvRecord::default() },
            CsvMutation::RemoveRecord { index: 0 },
            CsvMutation::SetField { record_index: 0, field_index: 0, value: String::new(), quoted: false },
        ];
