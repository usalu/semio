...
//#region 🔖️Manifest
pub async fn create_stl_any_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(STL_ANY_DIALECT)
        .await.document(["stdio", "stl"])
        .await.icon_id("box")
        .await.mode_def(view::definition().await)
        .await.default_mode_id(view::STL_ANY_VIEW_MODE_ID)
