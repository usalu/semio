//! 🧊 STL viewer — the Main window: a read-only world-3d mesh scene built with the shared
//! `MeshWindowKit` (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.6), from the
//! artifact-level `StlSnapshot` — this file imports nothing from the sibling editor module
...
const STL_ANY_VIEW_FALLBACK_MESH_KIND: &str = "box";
const STL_ANY_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const STL_ANY_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const STL_ANY_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
    ...
    let instances: Vec<pack::JsonValue> = (0..count)
        .map(|index| {
            pack::json!({
                ...
                "meshId": STL_ANY_VIEW_FALLBACK_MESH_KIND,
...
/// 👁️ Pure `StlSnapshot -> BuiltNode` read: default camera (a viewer has no persisted
/// per-session camera — `Config = NoConfig`), no selection/gumball/engagement overlay.
// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
pub fn render(document: &StlSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let meshes_json = pack::json_to_string(&pack::json_array([pack::json!({ "id": STL_ANY_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(STL_ANY_VIEW_FALLBACK_MESH_KIND) })]));
    let view = MeshView {
        camera_json: world3d_camera_json(STL_ANY_VIEW_DEFAULT_CAMERA_POSITION, STL_ANY_VIEW_DEFAULT_CAMERA_TARGET, STL_ANY_VIEW_DEFAULT_CAMERA_FOV),
        meshes_json,
