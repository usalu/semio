...
/// 👁️ Pure `StlSnapshot -> UiNode` read: default camera (a viewer has no persisted
...
pub async fn render(document: &StlSnapshot) -> UiNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": STL_ANY_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(STL_ANY_VIEW_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(STL_ANY_VIEW_DEFAULT_CAMERA_POSITION, STL_ANY_VIEW_DEFAULT_CAMERA_TARGET, STL_ANY_VIEW_DEFAULT_CAMERA_FOV).await,
        meshes_json,
