//! 🦀️ TIFF creation and round-trip case — Rust adapter.
//!
//! The oracle writes the image with the registered `image` reference encoder; the subject decodes
//! that same artifact with this repository's `decode_tiff` and re-encodes it with `encode_tiff`.
...
mod subject {
    ...
    pub fn run(ctx: &Context) -> Result<Outcome, String> {
        ...
        let snapshot = decode_tiff(&reference).map_err(|error| format!("decode_tiff failed: {:?}", error))?;
        let bytes = encode_tiff(&snapshot).map_err(|error| format!("encode_tiff failed: {:?}", error))?;
        let projection = project_image(&bytes, FORMAT)?;
        Ok(Outcome::with_raw(bytes, projection))
    }
}
