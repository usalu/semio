...
impl protocol::OpBinary for TiffAnyEditCommand {
    async fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        serde_json::to_vec(self).map_err(|error| protocol::ProtocolError::Malformed { what: "tiff_any-edit-command", offset: 0, detail: error.to_string() })
    }
    async fn decode_op(bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        serde_json::from_slice(bytes).map_err(|error| protocol::ProtocolError::Malformed { what: "tiff_any-edit-command", offset: 0, detail: error.to_string() })
    }
}
//#endregion 🔖️Command
...
impl ArtifactEditor for TiffAnyEditor {
    ...
    async fn handle(
        ...
        match command {
            TiffAnyEditCommand::SetPixelRegion { pixels } => Ok(Emit::mutations(vec![TiffMutation::SetPixels { pixels: pixels.clone() }]).await),
        }
    }
...
//#region 🔖️Manifest
pub async fn create_tiff_any_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(TIFF_ANY_DIALECT)
        .await.document(["semio", "tiff"])
        .await.icon_id("image")
