//! 🎹️ TiffBaselineComposer (6.0/✳️baseline) — reads the same sources the ✳️any subset does
//! (native `stdio.tiff` 6.0, plus its `binary` DAG dep), delegates the actual parse to the ✳️any
//! composer, then folds the real Baseline TIFF diagnostics on top (see `🧐️analyzer`).
//! PASS-THROUGH by design: `compose` here can never fail on conformance grounds (findings are
//! all SOFT by policy), only on the same errors the ✳️any delegate itself can already return.
//!
...
use crate::artifacts::tiff::standards::v6_0::subsets::any::composer::TiffComposer as TiffAnyComposer;
...
//#region 🔖️Composer
pub struct TiffBaselineComposer;

impl ArtifactComposer for TiffBaselineComposer {
...
mod tests {
    ...
    fn compose_carries_no_findings_for_a_conformant_document() {
        ...
        let composed = TiffBaselineComposer::compose(&sources).expect("pass-through compose never fails on conformance grounds");
    ...
    fn no_ifd_diagnostic_is_reachable_via_direct_check_not_through_encode_pack() {
        let diagnostics = crate::artifacts::tiff::standards::v6_0::subsets::baseline::analyzer::check_tiff_baseline_conformance(&TiffSnapshot::default());
        assert!(diagnostics.iter().any(|d| d.code.0 == crate::artifacts::tiff::standards::v6_0::subsets::baseline::analyzer::CODE_NO_IFD), "got {diagnostics:?}");
    }
}
