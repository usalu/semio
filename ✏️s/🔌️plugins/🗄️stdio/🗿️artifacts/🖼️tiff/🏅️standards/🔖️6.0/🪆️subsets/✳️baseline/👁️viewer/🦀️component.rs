//! 👁️ `tiff` viewer (baseline) — the read-only counterpart of `✏️editor` for this
//! subset (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `TiffBaselineViewer`
//! implements `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<TiffBaselineViewer>` is
...
pub enum TiffBaselineViewCommand {
    Noop,
}

impl protocol::OpBinary for TiffBaselineViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(TiffBaselineViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
impl ArtifactViewer for TiffBaselineViewer {
    ...
    type Command = TiffBaselineViewCommand;
...
pub fn create_tiff_baseline_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(TIFF_BASELINE_DIALECT).document(["semio", "tiff"]).icon_id("image").mode_def(view::definition()).default_mode_id(view::MODE_ID).window_kind_def(main::definition()).default_layout(view::layout()).build_definition()
}
//#endregion 🔖️Manifest
