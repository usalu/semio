...
fn op_codecs_round_trip() {
    let payload = mutation();
    assert_eq!(TiffMutation::parse_op(&payload.print_op()).expect("the text op parses"), payload, "replace-pixels/direct-behavior: the text op form does not round-trip");
    ...
    assert_eq!(TiffMutation::decode_op(&bytes).expect("the binary op decodes"), payload, "replace-pixels/direct-behavior: the binary op form does not round-trip");
    assert!(TiffMutation::decode_op(&bytes[..1]).is_err(), "replace-pixels/direct-behavior: a truncated binary frame must be rejected");
}
