...
fn mutation() -> TiffMutation {
    serde_json::from_str(MUTATION).expect("committed remove-ifd payload decodes")
}
...
fn op_codecs_round_trip() {
    ...
    assert_eq!(bytes[1], super::binary::BINARY_TAG, "remove-ifd/direct-behavior: the binary frame must carry this schema's tag");
