...
pub const BINARY_TAG: u8 = dsl::protocol_record::tag_u8(include_str!("../../💾️binary/📡️.protocol.semio"), "remove-tag");
...
pub fn decode(bytes: &[u8]) -> Result<TiffMutation, protocol::ProtocolError> {
    ...
    let result: Result<TiffMutation, protocol::ProtocolError> = {
        ...
        Ok(TiffMutation::RemoveTag(RemoveTagMutation { ifd_index, tag }))
    };
