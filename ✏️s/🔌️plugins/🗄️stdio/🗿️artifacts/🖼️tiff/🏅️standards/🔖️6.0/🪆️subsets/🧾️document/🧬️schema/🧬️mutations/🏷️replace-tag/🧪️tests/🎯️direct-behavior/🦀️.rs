...
fn mutation() -> TiffMutation {
    dsl::json::from_json_str(MUTATION).expect("committed replace-tag payload decodes")
}
...
fn op_codecs_round_trip() {
    ...
    assert_eq!(bytes[1], binary::BINARY_TAG, "replace-tag/direct-behavior: the binary frame must carry this schema's tag");
