...
const SUBSET_IDS: Readonly<Record<string, string>> = { "📰️header": "header", "📊️tables": "tables", "🧩️entities": "entities", "🧱️blocks": "blocks" };
...
const FIXTURE_COORDINATES: Readonly<Record<string, readonly [string, string]>> = {
  ...
  "insert-layer-applied": ["📊️tables", "🧱️insert-layer-applied"],
  "insert-layer-rejected-duplicate": ["📊️tables", "🚫️insert-layer-rejected-duplicate"],
  "remove-layer-applied": ["📊️tables", "🪨️remove-layer-applied"],
  "remove-layer-rejected-missing": ["📊️tables", "🔍️remove-layer-rejected-missing"],
  "set-layer-applied": ["📊️tables", "🎚️set-layer-applied"],
  "set-layer-rejected-missing": ["📊️tables", "❓️set-layer-rejected-missing"],
  "insert-style-applied": ["📊️tables", "🎨️insert-style-applied"],
  "insert-style-rejected-duplicate": ["📊️tables", "👯️insert-style-rejected-duplicate"],
  "remove-style-applied": ["📊️tables", "🧽️remove-style-applied"],
  "remove-style-rejected-missing": ["📊️tables", "🫥️remove-style-rejected-missing"],
  "set-style-applied": ["📊️tables", "🖌️set-style-applied"],
  "set-style-rejected-missing": ["📊️tables", "❌️set-style-rejected-missing"],
  "insert-linetype-applied": ["📊️tables", "🧵️insert-linetype-applied"],
  "insert-linetype-rejected-duplicate": ["📊️tables", "🪢️insert-linetype-rejected-duplicate"],
  "remove-linetype-applied": ["📊️tables", "🪚️remove-linetype-applied"],
  "remove-linetype-rejected-missing": ["📊️tables", "🕳️remove-linetype-rejected-missing"],
  "set-linetype-applied": ["📊️tables", "🪡️set-linetype-applied"],
  "set-linetype-rejected-missing": ["📊️tables", "📭️set-linetype-rejected-missing"],
  "insert-entity-applied": ["🧩️entities", "➕️insert-entity-applied"],
  "insert-entity-rejected-out-of-bounds": ["🧩️entities", "🚧️insert-entity-rejected-out-of-bounds"],
  "remove-entity-applied": ["🧩️entities", "➖️remove-entity-applied"],
  "remove-entity-rejected-missing": ["🧩️entities", "🔍️remove-entity-rejected-missing"],
  "set-entity-applied": ["🧩️entities", "🧩️set-entity-applied"],
  "set-entity-rejected-out-of-bounds": ["🧩️entities", "⛔️set-entity-rejected-out-of-bounds"],
  "insert-block-applied": ["🧱️blocks", "➕️insert-block-applied"],
  "insert-block-rejected-out-of-bounds": ["🧱️blocks", "🚧️insert-block-rejected-out-of-bounds"],
  "remove-block-applied": ["🧱️blocks", "➖️remove-block-applied"],
  "remove-block-rejected-missing": ["🧱️blocks", "🔍️remove-block-rejected-missing"],
  "set-block-applied": ["🧱️blocks", "🧱️set-block-applied"],
  "set-block-rejected-out-of-bounds": ["🧱️blocks", "⛔️set-block-rejected-out-of-bounds"],
};
...
async function manifestForSingle(recipe: Recipe, outDir: string): Promise<Record<string, unknown>> {
  const name = FIXTURE_COORDINATES[recipe.id]![1];
  const dir = join(outDir, name);
  const filename = "📐️drafting-plate.dxf";
...
async function manifestForPair(recipe: Recipe, outDir: string): Promise<Record<string, unknown>> {
  const [subset, name] = FIXTURE_COORDINATES[recipe.id]!;
  const dir = join(outDir, name);
  engineBuild(recipe.id, dir);
  const files: Awaited<ReturnType<typeof fileEntry>>[] = [await fileEntry("expected-before-dxf", dir, "⬅️before.dxf", name)];
  const hasAfter = existsSync(join(dir, "➡️after.dxf"));
  if (recipe.outcome === "rejected") {
    ...
    files.push(await fileEntry("expected-after-dxf", dir, "➡️after.dxf", name));
  }
...
async function main(argv: readonly string[]): Promise<number> {
  ...
  const outDir = outFlagIndex >= 0 ? rest[outFlagIndex + 1]! : fixtureOutRoot !== undefined && fixtureOutRoot.length > 0 ? fixtureOutRoot : null;
  ...
  for (const recipe of recipes) {
    try {
      const coordinate = FIXTURE_COORDINATES[recipe.id];
      if (!coordinate) throw new Error(`Missing authored fixture coordinate: ${recipe.id}`);
      const destination = outDir ?? join(HERE, "..", "..", coordinate[0], "🧫️fixtures");
      const manifest = recipe.kind === "single" ? await manifestForSingle(recipe, destination) : await manifestForPair(recipe, destination);
