...
pub enum SetMemberMutation {
    Apply(SetMemberPayload),
    Restore(JsonDiff),
}
...
impl protocol::MutationKind<JsonSnapshot, super::JsonMutation> for SetMemberMutation {
        ...
        match self {
            Self::Apply(payload) => protocol::MutationOutcome::new(match resolve(&base.value, &payload.path) {
                Some(JsonValue::Object { members }) => match members.iter().find(|member| member.key == payload.key) {
                    Some(existing) => {
                        let leaf = crate::schema::diff::value_diff_between(&existing.value, &payload.value);
                        diff_at_path(&payload.path, leaf.map(|diff| JsonValueDiff::Object { diff: JsonObjectDiff { removed: Vec::new(), added: Vec::new(), modified: vec![JsonObjectModified { key: payload.key.clone(), diff }] } }))
                    }
                    None => diff_at_path(
                        &payload.path,
                        Some(JsonValueDiff::Object { diff: JsonObjectDiff { removed: Vec::new(), modified: Vec::new(), added: vec![JsonObjectAdded { index: members.len(), key: payload.key.clone(), item: payload.value.clone() }] } }),
                    ),
                },
                _ => JsonDiff::default(),
            }),
    ...
    fn label(&self) -> protocol::LocalizedLabel {
        protocol::LocalizedLabel::native("Set Member", "Eigenschaft setzen")
    }
