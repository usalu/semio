...
pub enum RemoveArrayElementMutation {
    Apply(RemoveArrayElementPayload),
    Restore(JsonDiff),
}
    ...
    fn diff(&self, base: &JsonSnapshot) -> protocol::MutationOutcome<JsonDiff> {
        match self {
            Self::Apply(payload) => protocol::MutationOutcome::new(match resolve(&base.value, &payload.path) {
                Some(JsonValue::Array { items }) if payload.index < items.len() => diff_at_path(&payload.path, Some(JsonValueDiff::Array { diff: JsonArrayDiff { removed: vec![payload.index], modified: Vec::new(), added: Vec::new() } })),
                _ => JsonDiff::default(),
            }),
