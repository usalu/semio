...
fn decode_hex(value: &str) -> Result<Vec<u8>, String> { fn nibble(value: u8) -> Option<u8> { if value.is_ascii_digit() { return Some(value - b'0'); } (b'a'..=b'f').contains(&value).then_some(value - b'a' + 10) } if !value.len().is_multiple_of(2) { return Err("payload must be lowercase hexadecimal".to_string()); } value.as_bytes().as_chunks::<2>().0.iter().map(|pair| Ok((nibble(pair[0]).ok_or_else(|| "invalid hexadecimal".to_string())? << 4) | nibble(pair[1]).ok_or_else(|| "invalid hexadecimal".to_string())?)).collect() }
