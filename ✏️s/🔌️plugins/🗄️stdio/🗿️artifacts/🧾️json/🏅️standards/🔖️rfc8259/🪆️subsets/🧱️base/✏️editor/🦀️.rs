...
const JSON_ANY_RETAINED_TOOL_IDS: &[&str] = &[semio_s_artifact_stdio_contract::SET_ACTIVE_EXAMPLE_ACTION_ID];
...
fn json_any_command_from_action(action: &str, args: Option<&dsl::DslValue>) -> Result<JsonAnyEditorCommand, Fault> {
    match action {
        ...
        "set-node" => Ok(JsonAnyEditorCommand::SetNode { node_id: semio_s_artifact_stdio_contract::window_kit_text_argument(args, &["nodeId", "node_id", "id"], ""), value: semio_s_artifact_stdio_contract::window_kit_text_argument(args, &["value"], "") }),
        other => Err(Fault::new(
...
fn json_any_command_id(command: &JsonAnyEditorCommand) -> &'static str {
    match command {
        JsonAnyEditorCommand::SetNode { .. } => "set-node",
        JsonAnyEditorCommand::SetActiveExample { .. } => semio_s_artifact_stdio_contract::SET_ACTIVE_EXAMPLE_ACTION_ID,
    }
}

// 🚫️async: E1 pure codec/computation helper (file verified I/O-free, consumed via Fn-bound combinator/Display) — see R9
fn json_any_retained_extent(command: &JsonAnyEditorCommand, _snapshot: &JsonSnapshot, _interaction: &protocol::InteractionState) -> Option<usize> {
    matches!(command, JsonAnyEditorCommand::SetActiveExample { .. }).then_some(1)
}
...
fn json_any_retained_reduce(
    ...
    match command {
        JsonAnyEditorCommand::SetActiveExample { example_id } => Ok(Emit {
        ...
        JsonAnyEditorCommand::SetNode { .. } => Err(Fault::from("stdio-json-retained-route-mismatch")),
    }
}
...
impl ArtifactEditor for JsonAnyEditor {
    ...
    fn build_tool_job(request: ArtifactOwnedToolJobRequest<EditorApp<Self>>) -> Result<Option<ToolOperationSpec>, Fault> {
        ...
        let payload = ArtifactRetainedCommandPayload::try_new(
            ...
            1,
            Box::new(BoundedArtifactCommandWork::new(semio_s_artifact_stdio_contract::SET_ACTIVE_EXAMPLE_ACTION_ID, json_any_retained_reduce, json_any_retained_extent)),
        )?;
