...
// `(⬅️before.json, ➡️after.json)` PAIR for exactly one mutation kind — the pair IS the expectation,
...
const KINDS = [
  ...
  { kind: "remove-member", directory: "🗑️remove-member" },
  { kind: "set-member", directory: "✏️set-member" },
  { kind: "set-scalar", directory: "🔢️set-scalar" },
] as const;
//#endregion 🧭️Paths
...
const manifests = (): void => {
  const entries = KINDS.flatMap(({ kind, directory }) => {
    ...
    return [{
      ...
      files: ["⬅️before.json", "➡️after.json"].map((name) => ({
        role: name === "⬅️before.json" ? "input-json" : "expected-json",
