...
pub enum InsertArrayElementMutation {
    Apply(InsertArrayElementPayload),
    Restore(JsonDiff),
}
        ...
        match self {
            Self::Apply(payload) => protocol::MutationOutcome::new(match resolve(&base.value, &payload.path) {
                Some(JsonValue::Array { items }) => {
                    diff_at_path(&payload.path, Some(JsonValueDiff::Array { diff: JsonArrayDiff { removed: Vec::new(), modified: Vec::new(), added: vec![JsonArrayAdded { index: payload.index.min(items.len()), item: payload.value.clone() }] } }))
                }
                _ => JsonDiff::default(),
            }),
