//! ⚙️ Mp3 (mpeg1-layer3) engine — a REAL container-level codec: ID3v2 header + frame decode
//! (regular ID3v2.3 big-endian frame sizes, syncsafe ID3v2.4), an 11-bit MPEG frame sync scan
//! with a full bitrate/sample-rate table decode (every field-level bit of the 4-byte frame
//! header, typed), and an ID3v1 128-byte trailer detector. Frame PAYLOAD bytes are retained
//! typed-raw — an HONEST boundary: no Huffman/MDCT decode, this is a container-level codec, not
//! a full audio decoder (per the master plan's own framing).
...
fn parse_frame_header(bytes: &[u8], pos: usize) -> Option<(Mp3FrameHeader, usize)> {
    ...
    let sample_rate = sample_rate_hz(mpeg_version_id, sample_rate_index)?;
...
mod tests {
    ...
    /// `📚️examples/🎬️demo/🖼️assets/example.mp3` (per ticket `fixtures/mp3/NOTES.md`), duplicated
    /// here as a literal so the test doesn't reach across an emoji-path `include_bytes!`
    /// boundary.
    fn real_fixture() -> Vec<u8> {
        include_bytes!("../../../📚️examples/🎬️demo/🖼️assets/example.mp3").to_vec()
    }
...
/// `decode_mp3`/`encode_mp3` above are used by the subset's analyzer/`ArtifactDsl`/
/// `ArtifactPack` impls; schema descriptor + document codec registration is the subset
/// composer's own job (see that module).
pub fn register() {
    crate::artifacts::mp3::standards::mpeg1_layer3::subsets::any::io::register();
}
//#endregion 🔖️Register
