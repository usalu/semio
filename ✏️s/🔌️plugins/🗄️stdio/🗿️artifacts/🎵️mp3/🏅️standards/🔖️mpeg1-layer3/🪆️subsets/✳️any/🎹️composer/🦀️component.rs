//! 🎹️ Mp3Composer (s.stdio.mp3/mpeg1-layer3/*) — 🚧 scaffolded by W1b:
...
use crate::artifacts::mp3::standards::mpeg1_layer3::subsets::any::analyzer::Mp3Analyzer;

const DIALECT: Dialect = Dialect { artifact_kind: "s.stdio.mp3", standard: StandardId("mpeg1-layer3"), subset: SubsetId("*") };

//#region 🔖️Composer
pub struct Mp3Composer;

impl ArtifactComposer for Mp3Composer {
    ...
    fn compose(sources: &[ComposeSource]) -> Result<Composition<Self::Snapshot>, ComposeError> {
        let native: Vec<AnalyzeSource<'_>> = sources
            ...
            .collect();
        if native.is_empty() {
            return Err(ComposeError { message: "Mp3Composer: no source in a known read dialect".into(), diagnostics: Vec::new() });
        }
        let analysis = Mp3Analyzer::analyze(&native);
        let snapshot = analysis.parts.snapshot.ok_or_else(|| ComposeError {
            message: "Mp3Composer: analysis produced no snapshot".into(),
