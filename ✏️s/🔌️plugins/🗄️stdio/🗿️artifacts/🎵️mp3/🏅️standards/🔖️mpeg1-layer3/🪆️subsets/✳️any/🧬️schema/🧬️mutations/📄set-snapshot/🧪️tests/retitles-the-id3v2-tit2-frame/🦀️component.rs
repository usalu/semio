...
async fn inverse_restores_before() {
    ...
    assert!(matches!(inverse[0], Mp3Mutation::SetSnapshot { .. }), "set-snapshot/retitles-the-id3v2-tit2-frame: the undo step must itself be a SetSnapshot carrying the pre-state");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "set-snapshot/retitles-the-id3v2-tit2-frame: produced diff differs from the committed 🔺️diff/🔣️component.json");
