//! 🧬️ BmpMutation — document mutation dispatch.
...
impl Mutation<BmpSnapshot> for BmpMutation {
    ...
    fn diff(&self, _base: &BmpSnapshot) -> Self::Diff {
        match self {
            BmpMutation::NoMutation => BmpDiff::default(),
            BmpMutation::SetSnapshot { snapshot } => diff_set_snapshot(snapshot),
        }
    }
    ...
    fn inverse(&self, base: &BmpSnapshot) -> Vec<Self> {
        match self {
            ...
            BmpMutation::SetSnapshot { .. } => vec![BmpMutation::SetSnapshot { snapshot: base.clone() }],
        }
    }
}
