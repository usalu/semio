...
//!
//! Two subcommands:
//!   build   <recipe-id> <out-dir>   — writes <out-dir>/<recipe-id>/before.bmp [and after.bmp]
...
//!
//! @see ../../../../../../../../📼️avi/🏅️standards/🔖️1.0/🪆️subsets/✳️any/🏭️generator/🦀️riff-avi-codec/src/main.rs
...
fn cmd_build(id: &str, out_dir: &str) -> i32 {
    ...
    fs::write(dir.join("before.bmp"), encode(&before)).expect("write before.bmp");
    fs::write(dir.join("after.bmp"), encode(&after)).expect("write after.bmp");
    eprintln!("[image-bmp-codec] {id}: before.bmp + after.bmp -> {}", dir.display());
    0
}
