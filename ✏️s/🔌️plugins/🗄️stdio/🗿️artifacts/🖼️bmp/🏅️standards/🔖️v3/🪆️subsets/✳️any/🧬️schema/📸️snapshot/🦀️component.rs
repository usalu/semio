...
impl store::ArtifactDsl for BmpSnapshot {
    ...
    async fn parse_dsl(text: &str) -> Result<Self, store::TextError> {
        ...
        crate::artifacts::bmp::engine::decode_bmp(&bytes).await.map_err(|e| store::TextError::new(e, dsl::TextSpan::at(1, 1)))
    }
    async fn print_dsl(&self) -> String {
        let raw = crate::artifacts::bmp::engine::encode_bmp(self).await.unwrap_or_default();
...
impl store::ArtifactPack for BmpSnapshot {
    async fn encode_pack_with(&self, options: &store::PackEncodeOptions) -> Result<Vec<u8>, store::PackError> {
        let _ = options;
        let raw = crate::artifacts::bmp::engine::encode_bmp(self).await.map_err(|e| store::PackError::Schema(e))?;
    ...
    async fn decode_pack_with(bytes: &[u8], options: &store::PackDecodeOptions) -> Result<Self, store::PackError> {
        ...
        let _ = options;
        crate::artifacts::bmp::engine::decode_bmp(&inner).await.map_err(|e| store::PackError::Schema(e))
    }
}
