...
//
//   bun 📜️script.ts generate  [--only <fixture-id>]     # writes <outDir>/<id>/{before,after}.bmp
...
// @see ./🦀️image-bmp-codec/src/main.rs — the actual codec; `build <recipe-id> <out-dir>` and
...
/** 🍳️ Mirrors `RECIPE_IDS`/`recipe()` in `🦀️image-bmp-codec/src/main.rs` verbatim — one entry per
...
const RECIPES: readonly Recipe[] = [
  { id: "insert-palette-entry-applied", mutation: "insert-palette-entry", notes: "A new colour is inserted into the palette table at index 2; the 16-entry index buffer is left byte-identical, matching this subset's own semantics that a palette edit changes the table and leaves the picture alone." },
  { id: "remove-palette-entry-applied", mutation: "remove-palette-entry", notes: "The unreferenced spare palette entry (index 5, colour [0,0,0]) is removed; the index buffer is left byte-identical." },
  { id: "replace-palette-entry-applied", mutation: "replace-palette-entry", notes: "The unreferenced spare palette entry (index 5) is replaced with a new colour; the index buffer is left byte-identical." },
  { id: "replace-pixel-data-applied", mutation: "replace-pixel-data", notes: "A direct-colour 4x4 BMP's entire pixel array is replaced with a different solid RGB fill." },
];
//#endregion 🧬️Contract
...
function fileEntry(role: string, dir: string, filename: string, id: string): { role: string; path: string; mediaType: string; sha256: string; bytes: number } {
  ...
  return { role, path: `${FIXTURE_PATH_PREFIX}${id}/${filename}`, mediaType: "image/bmp", sha256: contentDigest(bytes), bytes: bytes.length };
}
...
function generateOne(recipe: Recipe, outDir: string): Record<string, unknown> {
  ...
  if (!existsSync(join(dir, "after.bmp"))) throw new Error(`recipe ${recipe.id} is declared applied but the codec produced no after.bmp`);
  const files = [fileEntry("expected-before-bmp", dir, "before.bmp", recipe.id), fileEntry("expected-after-bmp", dir, "after.bmp", recipe.id)];
...
save(os.path.join(d, 'before.bmp'))
save(os.path.join(d, 'after.bmp'), dpi=(300, 300))
print(kind + ': written')
`;
      const KINDS = ["change-header-fields"];
      ...
      if (command === "header") {
        ...
        for (const kind of KINDS) {
          ...
          const cmp = spawnSync("bun", [probes, "bmp-header-compare", "--input", join(outDir, kind, "before.bmp"), "--input", join(outDir, kind, "after.bmp")], { encoding: "utf8" });
          if (cmp.status !== 0) { failures.push(`${kind}: reader refused the pair`); continue; }
          if (JSON.parse(cmp.stdout).measurements.equal === true) failures.push(`${kind}: not observable in the header projection`);
        }
      ...
      for (const kind of KINDS) {
        const files = [];
        for (const [role, name] of [["expected-before-bmp", "before.bmp"], ["expected-after-bmp", "after.bmp"]] as const) {
          const bytes = readFileSync(join(outDir, kind, name));
          files.push({ role, path: `${FIXTURE_PATH_PREFIX}${kind}/${name}`, mediaType: "image/bmp", sha256: contentDigest(bytes), bytes: bytes.length });
        }
        entries.push({
          schema: "semio.repository-test.fixture/v2",
          id: `header-${kind}`,
