//! 🧪️ `change-header-fields` fixture — `direct-behavior`.
//!
...
fn mutation() -> BmpMutation {
    serde_json::from_str(MUTATION).expect("committed change-header-fields payload decodes")
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-header-fields/direct-behavior: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
fn op_codecs_round_trip() {
    ...
    assert_eq!(bytes[1], super::binary::BINARY_TAG, "change-header-fields/direct-behavior: the binary frame must carry this schema's tag");
