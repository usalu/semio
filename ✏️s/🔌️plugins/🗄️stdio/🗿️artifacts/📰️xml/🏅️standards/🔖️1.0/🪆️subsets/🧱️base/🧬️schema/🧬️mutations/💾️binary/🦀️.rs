...
impl protocol::OpBinary for XmlMutation {
    ...
    fn decode_op(bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        let text = std::str::from_utf8(bytes).map_err(|cause| protocol::ProtocolError::Malformed { what: "xml mutation", offset: 0, detail: cause.to_string() })?;
        pack::from_json_str(text).map_err(|cause| protocol::ProtocolError::Malformed { what: "xml mutation", offset: 0, detail: cause.to_string() })
    }
}
