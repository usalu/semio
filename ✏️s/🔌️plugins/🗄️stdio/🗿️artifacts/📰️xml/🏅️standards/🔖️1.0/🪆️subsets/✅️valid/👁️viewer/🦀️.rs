//! 👁️ Xml viewer — the read-only counterpart of `✏️editor` for `s.stdio.xml@1.0/*` (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `XmlValidViewer` implements
//! `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<XmlValidViewer>` (framework SDK)
...
pub const XML_VALID_VIEWER_DIALECT: Dialect = Dialect { artifact_kind: "s.stdio.xml", standard: StandardId("1.0"), subset: SubsetId("valid") };
//#endregion 🔖️Dialect

//#region 🔖️Command
/// 👁️ The viewer declares no actions, so its typed command channel has exactly one inert variant.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub enum XmlValidViewCommand {
    #[default]
    Noop,
}

impl protocol::OpBinary for XmlValidViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(XmlValidViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
impl ArtifactViewer for XmlValidViewer {
    ...
    type Command = XmlValidViewCommand;

    const DIALECT: Dialect = XML_VALID_VIEWER_DIALECT;
...
pub fn create_xml_valid_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(XML_VALID_VIEWER_DIALECT)
        .document(["semio", "stdio", "xml"])
        .icon_id("list-tree")
        .mode_def(view::definition())
        .default_mode_id(view::XML_VALID_VIEW_MODE_ID)
