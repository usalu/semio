//! 🧬️ Mp4Snapshot — doctype + top-level ISO-BMFF boxes; ftyp typed, everything else typed-raw.
//! 🚧 scaffolded by W1b: minimal honest fields only (not the full spec shape) — full
//! implementation lands in W2/W3.

/// 📦️ Owned by `mp4`: `Mp4RawBox` — an untyped-raw top-level ISO-BMFF box (type + payload
/// bytes verbatim). `ftyp` is typed above (`major_brand`/`minor_version`); everything else,
/// including `moov`/`mdat`, stays typed-raw until W3's real box-tree descent.
#[derive(Clone, Debug, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct Mp4RawBox { pub box_type: String, #[serde(default)] pub data: Vec<u8> }
