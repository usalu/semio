...
pub mod derived_construction {
    ...
    impl ArtifactBuilder for Mp4BuilderConstruction {
        ...
        async fn from_text(text: &str) -> Result<Self, store::TextError> {
            Ok(Self::from_snapshot(<Mp4Snapshot as store::ArtifactDsl>::parse_dsl(text).await?).await)
        }
        async fn from_binary(bytes: &[u8]) -> Result<Self, store::PackError> {
            Ok(Self::from_snapshot(<Mp4Snapshot as store::ArtifactPack>::decode_pack(bytes).await?).await)
        }
        ...
        async fn absorb(mut self, diff: Self::Diff) -> protocol::MutationApplyResult<Self> {
            self.snapshot = <Mp4Diff as protocol::MutationDiff<Mp4Snapshot>>::apply(&diff, &self.snapshot).await?;
            Ok(self)
        }
    ...
    impl ArtifactAnalysis for Mp4AnalyzerAnalysis {
        ...
        async fn sniff(source: &AnalyzeSource<'_>) -> IoConfidence {
            match source {
                ...
                AnalyzeSource::Text(text) => {
                    if io::sniff_real_bytes(text.as_bytes()).await || text.contains(STDIO_MP4_DOCUMENT_SCHEMA) {
                        IoConfidence::High
                    } else {
                        IoConfidence::Low
                    }
        ...
        async fn analyze(sources: &[AnalyzeSource<'_>]) -> Analysis<Self::Parts> {
            ...
            for source in sources {
                match source {
                    AnalyzeSource::Text(text) => match <Mp4Snapshot as store::ArtifactDsl>::parse_dsl(text).await {
                    ...
                    AnalyzeSource::Binary(bytes) => match <Mp4Snapshot as store::ArtifactPack>::decode_pack(bytes).await {
                        Ok(snapshot) => parts.snapshot = Some(snapshot),
                        Err(err) => {
                            confidence = IoConfidence::Low;
