//! 🎹️ Mp4Composer (s.stdio.mp4/isobmff/*) — 🚧 scaffolded by W1b:
...
use crate::artifacts::mp4::standards::isobmff::subsets::any::analyzer::Mp4Analyzer;

const DIALECT: Dialect = Dialect { artifact_kind: "s.stdio.mp4", standard: StandardId("isobmff"), subset: SubsetId("*") };

//#region 🔖️Composer
pub struct Mp4Composer;

impl ArtifactComposer for Mp4Composer {
    ...
    fn compose(sources: &[ComposeSource]) -> Result<Composition<Self::Snapshot>, ComposeError> {
        let native: Vec<AnalyzeSource<'_>> = sources
            ...
            .collect();
        if native.is_empty() {
            return Err(ComposeError { message: "Mp4Composer: no source in a known read dialect".into(), diagnostics: Vec::new() });
        }
        let analysis = Mp4Analyzer::analyze(&native);
        let snapshot = analysis.parts.snapshot.ok_or_else(|| ComposeError {
            message: "Mp4Composer: analysis produced no snapshot".into(),
