...
impl protocol::OpBinary for Mp4EditCommand {
    const TOOL_JOB_IDS: &'static [&'static str] = STDIO_MP4_DOCUMENT_SCHEMA_EXAMPLE_TOOL_IDS;
...
fn mp4Editor_retained_reduce(command: &Mp4EditCommand, _snapshot: &Mp4Snapshot, _config: &NoConfig, _history: &semio_framework_plugin::HistoryView, _interaction: &protocol::InteractionState, _hover: &semio_framework_plugin::app::InteractionHoverState, _context: Option<&semio_framework_plugin::app::ArtifactOwnedToolJobContext<EditorApp<Mp4Editor>>>, _operation: &AppOperationContext) -> Result<Emit<Mp4Mutation, NoConfigMutation, NoDraftMutation>, Fault> {
    match command {
        Mp4EditCommand::SetActiveExample { example_id } => Ok(Emit { effects: vec![semio_s_artifact_stdio_contract::load_example_effect(&mp4Editor_example_snapshot(example_id), STDIO_MP4_DOCUMENT_SCHEMA)], description: Some(format!("Load example {example_id}")), ..Default::default() }),
...
pub fn create_mp4_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(MP4_DIALECT).document(["semio", "mp4"]).icon_id("play").mode_def(edit::definition()).default_mode_id(edit::MODE_ID).window_kind_def(main::definition()).default_layout(edit::layout()).action_with(semio_s_artifact_stdio_contract::set_active_example_action())
