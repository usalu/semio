//! 🧊 LAS viewer — the Main window: a read-only world-3d mesh scene built with the shared
//! `MeshWindowKit` (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.6), from the
//! artifact-level `LasSnapshot` — this file imports nothing from the sibling editor module
...
const LAS_ANY_VIEW_FALLBACK_MESH_KIND: &str = "box";
const LAS_ANY_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [8.0, -8.0, 6.0];
const LAS_ANY_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const LAS_ANY_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
    ...
    let instances: Vec<serde_json::Value> = (0..count.await)
        .map(|index| {
            serde_json::json!({
                ...
                "meshId": LAS_ANY_VIEW_FALLBACK_MESH_KIND,
...
/// 👁️ Pure `LasSnapshot -> UiNode` read: default camera (a viewer has no persisted
...
pub async fn render(document: &LasSnapshot) -> UiNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": LAS_ANY_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(LAS_ANY_VIEW_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(LAS_ANY_VIEW_DEFAULT_CAMERA_POSITION, LAS_ANY_VIEW_DEFAULT_CAMERA_TARGET, LAS_ANY_VIEW_DEFAULT_CAMERA_FOV).await,
        meshes_json,
