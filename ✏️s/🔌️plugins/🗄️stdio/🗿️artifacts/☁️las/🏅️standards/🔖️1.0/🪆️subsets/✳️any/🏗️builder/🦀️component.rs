//! 🏗️ LasBuilder — local ArtifactBuilder until SDK Wave 3.
...
impl ArtifactBuilder for LasBuilder {
    ...
    fn from_text(text: &str) -> Result<Self, store::TextError> {
        Ok(Self::from_snapshot(<LasSnapshot as store::DocumentDsl>::parse_dsl(text)?))
    }
    fn from_binary(bytes: &[u8]) -> Result<Self, store::PackError> {
        Ok(Self::from_snapshot(<LasSnapshot as store::DocumentPack>::decode_pack(bytes)?))
    }
    fn mutate(mut self, mutation: Self::Mutation) -> Self {
        crate::artifacts::las::schema::mutations::apply_las_mutation(&mut self.snapshot, &mutation);
        self
    }
