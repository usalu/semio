...
//! way to override either, while `../🧬️schema/📸️snapshot/🦀️.rs`'s `LasHeader` treats both
...
//! past the fixed 227 bytes `../🚪️io/🦀️.rs`'s `encode_las` always emits (its own
...
mod raw_doc {
    ...
    /// records — the same "ground truth, not a hardcoded clamp" rule `../../🚪️io/🦀️.rs`'s
    ...
    /// 🎯️ Mirrors `../../🚪️io/🦀️.rs`'s `choose_point_format` exactly: point format chosen
...
//#region 🔖️FixedStr
/// 🔤️ Null/space-padded fixed-width ASCII field helpers, matching `../../🚪️io/🦀️.rs`'s
...
/// `../../🚪️io/🦀️.rs`'s `LasPoint.classification: u8` already models — so no decomposition
...
/// STRUCTURAL ones this never carries): the same field set `../🧬️schema/📸️snapshot/🦀️.rs`'s
...
mod tests {
    ...
    /// at `../../../../../../🧪️tests/mutate-las-1-0/🦀️.rs` through `ctx.copy_fixture`.
    fn fixture() -> Vec<u8> {
    ...
    /// `../../../../../../🧪️tests/mutate-las-1-0/🦀️.rs` exercises, not of this module in
