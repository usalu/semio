...
impl protocol::OpBinary for LasAnyEditCommand {
    ...
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(LasAnyEditCommand::SetVertex)
    }
}
//#endregion 🔖️Command
...
pub fn create_las_any_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(LAS_ANY_DIALECT).document(["stdio", "las"]).icon_id("box").mode_def(edit::definition()).default_mode_id(edit::LAS_ANY_EDIT_MODE_ID).window_kind_def(main::definition()).default_layout(edit::layout()).build_definition()
}
//#endregion 🔖️Manifest
