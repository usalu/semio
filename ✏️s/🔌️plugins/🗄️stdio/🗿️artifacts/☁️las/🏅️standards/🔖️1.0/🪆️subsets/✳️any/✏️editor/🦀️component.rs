...
//#region 🔖️Manifest
pub async fn create_las_any_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(LAS_ANY_DIALECT)
        .await.document(["stdio", "las"])
        .await.icon_id("box")
        .await.mode_def(edit::definition().await)
        .await.default_mode_id(edit::LAS_ANY_EDIT_MODE_ID)
...
mod tests {
    ...
    async fn editor_and_viewer_share_one_dialect() {
        semio_framework_plugin::testkit::assert_editor_and_viewer_share_dialect::<LasAnyEditor, crate::viewer::las::LasAnyViewer>();
    }
}
//#endregion 🧪️Tests
