...
pub mod derived_construction {
    ...
    impl ArtifactBuilder for DeflateBuilderConstruction {
        ...
        async fn from_text(text: &str) -> Result<Self, store::TextError> {
            Ok(Self::from_snapshot(<DeflateSnapshot as store::ArtifactDsl>::parse_dsl(text).await?).await)
        }
        async fn from_binary(bytes: &[u8]) -> Result<Self, store::PackError> {
            Ok(Self::from_snapshot(<DeflateSnapshot as store::ArtifactPack>::decode_pack(bytes).await?).await)
        }
    ...
    impl ArtifactAnalysis for DeflateAnalyzerAnalysis {
        ...
        async fn analyze(sources: &[AnalyzeSource<'_>]) -> Analysis<Self::Parts> {
            ...
            for source in sources {
                match source {
                    AnalyzeSource::Text(text) => match <DeflateSnapshot as store::ArtifactDsl>::parse_dsl(text).await {
                    ...
                    AnalyzeSource::Binary(bytes) => match <DeflateSnapshot as store::ArtifactPack>::decode_pack(bytes).await {
                        Ok(snapshot) => parts.snapshot = Some(snapshot),
                        Err(err) => {
                            confidence = IoConfidence::Low;
