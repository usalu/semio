//! 🧪️ `set-snapshot` fixture — `🏗️lifts-the-second-vertex-and-appends-a-comment`.
//!
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot.elements[0].rows[1].values[2], crate::standards::v1_0::subsets::any::schema::snapshot::PlyValue::Float(2.0), "set-snapshot/lifts-the-second-vertex-and-appends-a-comment: the second vertex's z cell must land on 2.0");
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: PlySnapshot = dsl::json::from_json_str(text).expect("snapshot decodes");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "set-snapshot/lifts-the-second-vertex-and-appends-a-comment: produced diff differs from the committed 🔺️diff/🔣️.json");
...
async fn committed_diff_is_canonical() {
    let decoded: PlyDiff = dsl::json::from_json_str(DIFF).expect("committed diff decodes");
