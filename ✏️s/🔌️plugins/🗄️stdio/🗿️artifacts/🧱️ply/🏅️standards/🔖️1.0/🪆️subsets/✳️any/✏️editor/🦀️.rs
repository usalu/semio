...
impl protocol::OpBinary for PlyAnyEditCommand {
    const TOOL_JOB_IDS: &'static [&'static str] = PLY_ANY_DOCUMENT_SCHEMA_EXAMPLE_TOOL_IDS;

    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(pack::to_json_string(self).into_bytes())
    }
    fn decode_op(bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        let parsed = pack::parse_json_bytes(bytes).map_err(|error| protocol::ProtocolError::Malformed { what: "PlyAnyEditCommand", offset: 0, detail: error.to_string() })?;
        <Self as dsl::FromValue>::from_value(pack::json_to_dsl_value(&parsed)).map_err(|error| protocol::ProtocolError::Malformed { what: "PlyAnyEditCommand", offset: 0, detail: error.to_string() })
    }
}
//#endregion 🔖️Command
...
fn plyAnyEditor_command_from_action(action: &str, args: Option<&dsl::DslValue>) -> Result<PlyAnyEditCommand, Fault> {
    match action {
...
impl ArtifactOwnedToolJobFactory for PlyAnyEditorExampleFactory {
    ...
    fn build_document_store_initialization_job(
        ...
        Ok(semio_framework_plugin::bounded_document_store_initialization_job(envelope, PLY_ANY_DOCUMENT_SCHEMA, operation, generation))
    }

    fn command_id(command: &Self::Command) -> &'static str { plyAnyEditor_command_id(command) }

    fn command_from_action(action: &str, args: Option<&dsl::DslValue>) -> Result<Self::Command, Fault> { plyAnyEditor_command_from_action(action, args) }
...
pub fn create_ply_any_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(PLY_ANY_DIALECT).document(["stdio", "ply"]).icon_id("box").mode_def(edit::definition()).default_mode_id(edit::PLY_ANY_EDIT_MODE_ID).window_kind_def(main::definition()).default_layout(edit::layout()).action_with(semio_s_artifact_stdio_contract::set_active_example_action())
