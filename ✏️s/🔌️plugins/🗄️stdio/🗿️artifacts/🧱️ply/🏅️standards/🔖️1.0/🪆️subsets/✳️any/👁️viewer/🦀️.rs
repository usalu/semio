//! 👁️ PLY viewer — the read-only counterpart of `✏️editor` for this subset (ticket
//! 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `PlyAnyViewer`
//! implements `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` — `ViewerApp<PlyAnyViewer>`
...
use crate::viewer::ply::modes::view;
use crate::viewer::ply::modes::view::windows::main;
...
pub enum PlyAnyViewCommand {
    #[default]
    Noop,
}

impl protocol::OpBinary for PlyAnyViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(PlyAnyViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
pub struct PlyAnyViewer;

impl ArtifactViewer for PlyAnyViewer {
    ...
    type Command = PlyAnyViewCommand;
...
pub fn create_ply_any_viewer() -> semio_framework_plugin::AppDefinition {
    Viewer::builder(PLY_ANY_DIALECT).document(["stdio", "ply"]).icon_id("box").mode_def(view::definition()).default_mode_id(view::PLY_ANY_VIEW_MODE_ID).window_kind_def(main::definition()).default_layout(view::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn viewer_dialect_matches_the_artifact_coordinate() {
        assert_eq!(<PlyAnyViewer as ArtifactViewer>::DIALECT, PLY_ANY_DIALECT);
    }

    #[semio_framework_async_macros::async_test]
    async fn viewer_never_mutates_the_document_or_draft_store() {
        semio_framework_plugin::testkit::assert_viewer_never_mutates::<PlyAnyViewer>().await;
    }
}
//#endregion 🧪️Tests
