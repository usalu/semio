...
//!
//! Two subcommands:
//!   build   <recipe-id> <out-dir>   — writes <out-dir>/<recipe-id>/before.avi [and after.avi]
...
//! only `before.avi`: the attempted mutation is out-of-bounds/duplicate/missing-target by
...
/// recipe — the generator writes only `before.avi`). Every AFTER state below touches EXACTLY the
...
fn cmd_build(id: &str, out_dir: &str) -> i32 {
    ...
    fs::write(dir.join("before.avi"), encode_avi(&before)).expect("write before.avi");
    if let Some(after) = after {
        fs::write(dir.join("after.avi"), encode_avi(&after)).expect("write after.avi");
        eprintln!("[riff-avi-codec] {id}: before.avi + after.avi -> {}", dir.display());
    } else {
        eprintln!("[riff-avi-codec] {id}: before.avi only (rejected) -> {}", dir.display());
    }
    0
}
    ...
    let code = match args.get(1).map(String::as_str) {
        Some("build") => {
            let (Some(id), Some(out_dir)) = (args.get(2), args.get(3)) else {
                eprintln!("usage: riff-avi-codec build <recipe-id> <out-dir>");
                std::process::exit(2);
            };
