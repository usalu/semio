//! 🔺️ AviDiff — 🚧 scaffolded by W1b: a minimal, genuinely law-abiding full-replace
...
pub struct AviDiff {
    ...
    pub replacement: Option<AviSnapshot>,
}
...
mod tests {
    ...
    fn field_sweep_full_replace_round_trip() {
        let a = AviSnapshot::default();
        let mut b = AviSnapshot::default();
    ...
    fn absorb_composes_two_sequential_diffs() {
        let a = AviSnapshot::default();
        let mut mid = AviSnapshot::default();
        mid.schema = format!("{}-mid", a.schema);
        let mut after = AviSnapshot::default();
        after.schema = format!("{}-after", a.schema);
        let mut d1 = <AviDiff as DiffAlgebra<AviSnapshot>>::between(&a, &mid);
        let d2 = <AviDiff as DiffAlgebra<AviSnapshot>>::between(&mid, &after);
