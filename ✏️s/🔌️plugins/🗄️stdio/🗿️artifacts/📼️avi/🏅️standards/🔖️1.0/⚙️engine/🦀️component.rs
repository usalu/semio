//! ⚙️ Avi (1.0, RIFF) engine — real RIFF/AVI decode/encode. Moved from remodel's video engine
//! (`✏️s/🔌️plugins/📸️remodel/🗿️artifacts/📸️remodel/🏅️standards/🔖️1/⚙️engine/🎥️video/🦀️component.rs`
//! RIFF-half: `🔖️Avi` region lines 605-885 for the reader shape, `write_avi_mjpg` lines 3762-3823
//! for the writer shape) per the master plan's extraction map, adapted from that source's
//! "first `vids` stream only, metadata-only" `probe_avi`/`AviInfo` into this artifact's full
//! `AviSnapshot` (every stream, real chunk BYTES copied out — not just offsets — `idx1_present`
//! flag instead of raw index retention, matching the ticket's exact schema).
//!
//! **idx1 offset-base note**: remodel's own `parse_idx1` tries two candidate bases (`movi_start`
//! excluding the `movi` tag, and `0`) and falls back to `scan_movi_chunks` when neither verifies
//! — because real-world AVI encoders disagree on whether `idx1` offsets are relative to the
//! `movi` LIST's data start INCLUDING or EXCLUDING its own 4-byte `movi` tag (this ticket's own
//! `make_avi.py` fixture generator documents using the OpenDML-standard INCLUDING convention,
//! which neither of remodel's two candidates actually matches — see this ticket's own recon).
//! This engine sidesteps the ambiguity entirely for both directions of its OWN codec: `movi`
//! chunks are always discovered by direct RIFF iteration (no offset math needed to find them),
//! and `idx1` entries are matched to them POSITIONALLY (same order, same count — true for this
//! encoder's own always-written `idx1`, and for every idx1-having AVI this decoder has been
//! exercised against) purely to recover the keyframe flag.
...
mod tests {
    ...
    /// 🎬️ The handcrafted-but-real `example.avi` fixture (`generators/w0-fixtures/make_avi.py`,
    /// see `fixtures/avi/NOTES.md`): 732 bytes, 16×16 MJPG, 3 `00dc` frames, real `idx1`.
    const REAL_EXAMPLE_AVI: &[u8] = include_bytes!("../../../📚️examples/🎬️demo/🖼️assets/example.avi");
