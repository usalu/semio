...
// 🏭️ Third-party fixture generator for `s.stdio.pdf@1.7/🗄️a` — the 14-kind ISO 19005-2/19005-3
// (PDF/A-2, PDF/A-3) conformance-class catalog.
//
// Every before/after pair `🔁️codec/🏗️generate/🦀️.rs` writes is produced by the SAME registered `lopdf`
// 0.44 reference implementation named `lopdf-pdf-1-7-a-mutate` in `../🔣️oracle.json` —
...
// `../../../../../../🧪️tests/🗄️mutate-pdf-1-7-a` drives — never this repository's own production PDF
...
// itself — so `generate`/`manifests` operate on the whole 14-recipe corpus, not one recipe at a time.
//
...
const SUBSET = "a";
const ORACLE_ID = "lopdf-pdf-1-7-a-mutate-reader";
const COMPARISON_PROFILE = "semantic-pdf-conformance-a-v1";
// 🧾️ Kept in step with `🔁️codec/🏗️generate/🦀️.rs::KINDS` (itself the same list as
// `../🔣️oracle.json`'s `pdf-1-7-a` catalog) — `manifests` walks whichever recipe directories the
...
const KINDS: readonly string[] = ["insert-javascript-action", "remove-javascript-action", "insert-launch-action", "remove-launch-action", "insert-embedded-file", "remove-embedded-file", "set-af-relationship", "remove-af-relationship", "set-output-intent", "remove-output-intent", "embed-font-file", "remove-font-file"];
const FIXTURE_DIRECTORY_BY_KIND: Readonly<Record<string, string>> = {
  ...
  "insert-embedded-file": "📎️insert-embedded-file",
  "remove-embedded-file": "🗑️remove-embedded-file",
  "set-af-relationship": "🔗️set-af-relationship",
  "remove-af-relationship": "✂️remove-af-relationship",
  ...
  "insert-encryption-dictionary": "🔒️insert-encryption-dictionary",
  "remove-encryption-dictionary": "🔓️remove-encryption-dictionary",
};
...
async function generate(outRoot: string): Promise<void> {
  ...
  for (const kind of KINDS) {
    ...
    const beforePath = join(outRoot, directory, BEFORE_FILENAME);
    const afterPath = join(outRoot, directory, AFTER_FILENAME);
    if (!existsSync(beforePath) || !existsSync(afterPath)) continue;
    console.log(`${kind}: before=${await sha256(beforePath)} after=${await sha256(afterPath)}`);
  }
}
...
async function manifests(): Promise<void> {
  ...
  for (const kind of KINDS) {
    ...
    const beforePath = join(dir, BEFORE_FILENAME);
    ...
    entries.push({
      ...
      files: [
        { role: "base-pdf", path: `../🧫️fixtures/${directory}/${BEFORE_FILENAME}`, mediaType: "application/pdf", sha256: await sha256(beforePath), bytes: readFileSync(beforePath).length },
        { role: "mutated-pdf", path: `../🧫️fixtures/${directory}/${AFTER_FILENAME}`, mediaType: "application/pdf", sha256: await sha256(afterPath), bytes: readFileSync(afterPath).length },
      ],
...
seed, out, kind, directory = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4]
...
d = os.path.join(out, directory); os.makedirs(d, exist_ok=True)
before = os.path.join(d, '⬅️before.pdf'); after = os.path.join(d, '➡️after.pdf')
...
async function encryptionMode(emitManifests: boolean, root: string): Promise<number> {
  ...
  const seedKind = ["set-output-intent", "insert-javascript-action", "embed-font-file"].find((kind) => exists(j(seedDir, FIXTURE_DIRECTORY_BY_KIND[kind]!, BEFORE_FILENAME)));
  if (!seedKind) throw new Error(`no committed ${BEFORE_FILENAME} to seed the encryption pair from`);
  const seed = j(seedDir, FIXTURE_DIRECTORY_BY_KIND[seedKind]!, BEFORE_FILENAME);
  if (!emitManifests) {
    const failures: string[] = [];
    for (const kind of KINDS) {
      const directory = FIXTURE_DIRECTORY_BY_KIND[kind]!;
      const written = run("python3", ["-c", ENCRYPTION_WRITER, seed, root, kind, directory], { stdio: "inherit" });
      if (written.status !== 0) { failures.push(`${kind}: writer failed`); continue; }
      const cmp = run("bun", [probes, "pdf-encryption-compare", "--input", j(root, directory, BEFORE_FILENAME), "--input", j(root, directory, AFTER_FILENAME)], { encoding: "utf8" });
  ...
  for (const kind of KINDS) {
    ...
    for (const [role, name] of [["base-pdf", BEFORE_FILENAME], ["mutated-pdf", AFTER_FILENAME]] as const) {
      const bytes = read(j(root, directory, name));
      const digest = await crypto.subtle.digest("SHA-256", bytes);
      files.push({ role, path: `../🧫️fixtures/${directory}/${name}`, mediaType: "application/pdf", sha256: `sha256:${[...new Uint8Array(digest)].map((b) => b.toString(16).padStart(2, "0")).join("")}`, bytes: bytes.length });
    }
    entries.push({
      schema: "semio.repository-test.fixture/v2",
      id: `encryption-${kind}`,
      class: "third-party-generated",
      target: { artifact: "s.stdio.pdf", standard: "1.7", subset: "a" },
      mutation: kind,
      outcome: "applied",
      units: { length: "unitless", angle: "degree" },
      files,
