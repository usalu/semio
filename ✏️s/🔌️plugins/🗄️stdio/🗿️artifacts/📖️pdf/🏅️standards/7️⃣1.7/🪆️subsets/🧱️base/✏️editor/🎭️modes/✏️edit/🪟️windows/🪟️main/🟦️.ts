/** 🪟️ PDF Document (1.7) editor -- `main` window: typed twin of `🦀️.rs`'s `DocumentWindowKit`
...
export interface Pdf17DocumentPage {
  text: string;
}

/** ✏️ The `main` window's typed view-model -- the TS mirror of the Rust `render()` boundary's
 * input (a bare `PdfSnapshot`). */
export interface Pdf17DocumentViewModel {
  windowKindId: "framework.window.document";
  bodyKey: "framework.window.document";
  pages: Pdf17DocumentPage[];
}

/** ✏️ `set-page` payload shape -- mirrors `Pdf17EditorCommand::SetPage`. APPENDS to the page's
...
export interface Pdf17SetPage {
  index: number;
  text: string;
}

export const PDF17_MAIN_WINDOW_KIND_ID = "framework.window.document" as const;
export const PDF17_MAIN_BODY_KEY = "framework.window.document" as const;
