...
Feature: Apply every typed ISO 16612-2 (PDF/VT-1) conformance-class mutation to a real document
  ...
  reads every axis `check_x_conformance` reads, plus two of its own: `/Root/DPartRoot` (hard) and a `/DPM` metadata dictionary on every `/DPart` node reachable from it (soft). No 🧱️base mutation moves any of those axes and no mutation here touches
