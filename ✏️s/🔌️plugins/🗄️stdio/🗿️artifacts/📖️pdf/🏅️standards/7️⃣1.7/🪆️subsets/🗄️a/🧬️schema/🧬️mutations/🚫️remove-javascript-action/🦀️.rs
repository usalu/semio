...
impl MutationKind<PdfSnapshot, PdfAMutation> for RemoveJavascriptAction {
    ...
    fn inverse(&self, base: &PdfSnapshot) -> Vec<PdfAMutation> {
        support::action_with(base, "JavaScript", "JS", &self.script).map(|_| PdfAMutation::InsertJavascriptAction(InsertJavascriptAction { script: self.script.clone() })).into_iter().collect()
    }
