//! 🔤️ Authoritative PDF/H mutation for attaching a font program to a font descriptor.
...
impl MutationKind<PdfSnapshot, PdfHMutation> for EmbedFontFile {
    ...
    fn inverse(&self, base: &PdfSnapshot) -> Vec<PdfHMutation> {
        ...
        match support::font_program(base, id) {
            Some((key, program)) => vec![PdfHMutation::EmbedFontFile(EmbedFontFile { descriptor_ordinal: self.descriptor_ordinal, key, program })],
            None => vec![PdfHMutation::RemoveFontFile(RemoveFontFile { descriptor_ordinal: self.descriptor_ordinal })],
        }
    }
