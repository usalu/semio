...
fn removes_the_catalog_output_intent() {
    ...
    let outcome = <RemoveOutputIntent as MutationKind<PdfSnapshot, PdfEMutation>>::diff(&mutation, &base);
    let next = outcome.diff().apply(&base).unwrap();
    assert!(support::output_intent_identifier(&next).is_none());
    assert_eq!(<RemoveOutputIntent as MutationKind<PdfSnapshot, PdfEMutation>>::inverse(&mutation, &base).len(), 1);
}
