...
Feature: Apply every typed the PDF Healthcare Best Practices Guide (PDF/H) conformance-class mutation to a real document
  ...
  owns the PDF Healthcare Best Practices Guide (PDF/H), a CONFORMANCE CLASS, which is a property of the object graph as a whole and
  of no page at all. The catalog is one kind per axis of this subset's own check_h_conformance, which
  reads five axes: a populated `Info.title` AND `Info.author`, any `/S /JavaScript` action or bare `/JS` key, any `/S /Launch` action, an `/AcroForm` field with `/FT /Sig`, and an embedded program on every font's `/FontDescriptor`. No 🧱️base mutation moves any of those axes and no mutation here touches
  ...
  /Encrypt. That is faithful to what this subset actually checks — check_h_conformance scans the retained
  ...
  removal, still performed by the reference, still on the subset's own generated seed graph:
    ...
    remove-signature-field — the /AcroForm signature field is inserted first.
  ...
  divergence for it. What the projection carries instead is exactly the content check_h_conformance's own
