//! 🧽️ Authoritative PDF/E mutation for removing the catalog output intent.
...
impl MutationKind<PdfSnapshot, PdfEMutation> for RemoveOutputIntent {
    ...
    fn inverse(&self, base: &PdfSnapshot) -> Vec<PdfEMutation> {
        support::output_intent_identifier(base)
            .map(|identifier| PdfEMutation::SetOutputIntent(SetOutputIntent { identifier }))
            .into_iter()
            .collect()
    }

    fn label(&self) -> String {
        "Remove PDF/E output intent".to_string()
    }
