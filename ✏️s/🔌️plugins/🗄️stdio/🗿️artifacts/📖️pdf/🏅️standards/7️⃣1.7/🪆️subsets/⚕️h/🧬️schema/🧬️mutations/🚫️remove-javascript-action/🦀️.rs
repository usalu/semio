//! 🚫️ Authoritative PDF/H mutation for removing a matching JavaScript action.
...
impl MutationKind<PdfSnapshot, PdfHMutation> for RemoveJavascriptAction {
    ...
    fn inverse(&self, base: &PdfSnapshot) -> Vec<PdfHMutation> {
        support::action_with(base, "JavaScript", "JS", &self.script)
            .map(|_| PdfHMutation::InsertJavascriptAction(InsertJavascriptAction { script: self.script.clone() }))
            .into_iter()
            .collect()
    }
...
mod tests {
    ...
    fn removes_the_matching_script_action() {
        ...
        let outcome = <RemoveJavascriptAction as MutationKind<PdfSnapshot, PdfHMutation>>::diff(&mutation, &base);
