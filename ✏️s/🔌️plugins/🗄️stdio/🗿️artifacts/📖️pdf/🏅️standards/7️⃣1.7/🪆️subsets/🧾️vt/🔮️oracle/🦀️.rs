...
mod tests {
    ...
    fn params_for(kind: &str) -> Json {
        match kind {
            ...
            "set-dpart-root" => json_object(vec![("job", Json::String("run 4711, recipient block A".to_string()))]),
            "remove-dpart-root" => json_object(vec![]),
            "set-dpart-metadata" => json_object(vec![("job", Json::String("run 4712, recipient block B".to_string()))]),
            "remove-dpart-metadata" => json_object(vec![]),
    ...
    fn a_kind_this_subset_does_not_declare_is_refused_even_when_the_engine_could_perform_it() {
        let undeclared = ["insert-embedded-file", "insert-signature-field", "remove-af-relationship", "remove-display-doc-title", "remove-embedded-file", "remove-lang", "remove-mark-info", "remove-signature-field", "remove-struct-tree-root", "set-af-relationship", "set-display-doc-title", "set-info-author", "set-info-title", "set-lang", "set-mark-info", "set-struct-tree-root"];
