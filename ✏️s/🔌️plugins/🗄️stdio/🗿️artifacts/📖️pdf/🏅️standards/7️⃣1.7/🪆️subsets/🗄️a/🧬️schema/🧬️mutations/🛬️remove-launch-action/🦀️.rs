//! 🛬️ Authoritative PDF/A mutation for removing a matching launch action.
...
impl MutationKind<PdfSnapshot, PdfAMutation> for RemoveLaunchAction {
    ...
    fn inverse(&self, base: &PdfSnapshot) -> Vec<PdfAMutation> {
        support::action_with(base, "Launch", "F", &self.target)
            .map(|_| PdfAMutation::InsertLaunchAction(InsertLaunchAction { target: self.target.clone() }))
            .into_iter()
            .collect()
    }
...
mod tests {
    ...
    fn removes_the_matching_launch_target() {
        ...
        let outcome = <RemoveLaunchAction as MutationKind<PdfSnapshot, PdfAMutation>>::diff(&mutation, &base);
