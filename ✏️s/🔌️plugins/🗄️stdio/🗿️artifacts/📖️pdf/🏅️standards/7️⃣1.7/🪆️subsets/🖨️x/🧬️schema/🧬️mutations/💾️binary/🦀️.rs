...
fn identity(payload: &[u8]) -> Result<String, protocol::ProtocolError> {
    let value: pack::JsonValue = pack::parse_json_bytes(payload).map_err(|error| malformed(2, error.to_string()))?;
...
impl OpBinary for PdfXMutation {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        ...
        if payload.len() > MAX_PAYLOAD_BYTES {
            return Err(protocol::ProtocolError::LimitExceeded("PDF/X mutation payload"));
        }
    ...
    fn decode_op(bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        ...
        <PdfXMutation as dsl::FromValue>::from_value(pack::json_to_dsl_value(&parsed)).map_err(|error| malformed(2, error.to_string()))
    }
}
//#endregion 🧱️Framing
