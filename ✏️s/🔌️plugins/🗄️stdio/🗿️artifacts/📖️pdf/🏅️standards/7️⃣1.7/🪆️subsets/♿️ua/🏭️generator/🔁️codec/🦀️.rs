//! 📄️ The `pdf@1.7/ua` mutation vocabulary and conformance projection, expressed ENTIRELY through
...
//!
//! @see ../../../🔮️oracles/🦀️.rs — the subset's own vocabulary and conformance axes.
...
pub fn build_seed() -> Vec<u8> {
    ...
    let content_id = document.add_object(Stream::new(dictionary! {}, b"BT /F1 12 Tf 72 720 Td (pdf-1-7-ua fixture seed) Tj ET".to_vec()));
    ...
    let info = document.add_object(dictionary! {
        "Title" => Object::string_literal("pdf-1-7-ua fixture seed"),
        "Author" => Object::string_literal("pdf-1-7-ua-lopdf-engine"),
    });
...
pub fn arrange(kind: &str, bytes: &[u8]) -> Vec<u8> {
    ...
    match kind {
        ...
        "remove-mark-info" => set_catalog(&mut document, "MarkInfo", Object::Dictionary(dictionary! { "Marked" => true })),
        "remove-struct-tree-root" => {
...
pub fn project(bytes: &[u8]) -> Result<String, String> {
    ...
    let mut out = String::from("{\"subset\":\"ua\"");
