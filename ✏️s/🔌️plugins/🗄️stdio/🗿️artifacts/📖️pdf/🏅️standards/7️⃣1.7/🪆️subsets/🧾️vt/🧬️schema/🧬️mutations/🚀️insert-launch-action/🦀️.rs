//! 🚀️ Authoritative PDF/VT mutation for inserting a launch action.
...
impl MutationKind<PdfSnapshot, PdfVtMutation> for InsertLaunchAction {
    ...
    fn inverse(&self, _base: &PdfSnapshot) -> Vec<PdfVtMutation> {
        vec![PdfVtMutation::RemoveLaunchAction(RemoveLaunchAction { target: self.target.clone() })]
    }
...
mod tests {
    ...
    fn inserts_the_launch_target() {
        ...
        let outcome = <InsertLaunchAction as MutationKind<PdfSnapshot, PdfVtMutation>>::diff(&mutation, &base);
