//! 🦀️ PDF 1.7 ⚕️h exhaustive conformance-class mutation case — Rust adapter.
//!
...
//! `../../🏅️standards/7️⃣1.7/🪆️subsets/⚕️h/🔮️oracles/🦀️.rs`, `subject` handlers drive this
...
//!
//! @see ../../🏅️standards/7️⃣1.7/🪆️subsets/⚕️h/🧬️schema/🦀️.rs — `check_h_conformance`, the one
//!      axis list this whole case derives from.

use semio_repo_test_host::{Adapter, Context, Json, Outcome};
use semio_s_plugin_stdio_test_oracle::artifacts::pdf::standards::v1_7::subsets::h::{oracle_apply_mutation, oracle_arrange, oracle_inverse_spec, oracle_round_trip, project_conformance};

//#region 🔖️Kinds
/// 🧾️ Test-case-local mirror of the `pdf-1-7-h` catalog. Duplicated, not imported, from
/// `../../🏅️standards/7️⃣1.7/🪆️subsets/⚕️h/🧬️schema/🧬️mutations/🦀️.rs::KINDS` — that module
...
const KINDS: &[&str] = &["set-info-title", "set-info-author", "insert-javascript-action", "remove-javascript-action", "insert-launch-action", "remove-launch-action", "insert-signature-field", "remove-signature-field", "embed-font-file", "remove-font-file"];
//#endregion 🔖️Kinds
...
mod subject {
    ...
    use semio_s_artifact_stdio_pdf::standards::v1_7::subsets::h::schema::mutations::*;
    ...
    /// `PdfHMutation` — the only channel between the feature's parameters and the subject's codec.
    fn mutation_from_spec(base: &PdfSnapshot, spec: &Json) -> Result<PdfHMutation, String> {
        let params = spec.get("params").cloned().unwrap_or(Json::Null);
        Ok(match spec.str("kind").as_str() {
            "set-info-title" => PdfHMutation::SetInfoTitle(SetInfoTitle { title: params.str("title") }),
            "set-info-author" => PdfHMutation::SetInfoAuthor(SetInfoAuthor { author: params.str("author") }),
            "insert-javascript-action" => PdfHMutation::InsertJavascriptAction(InsertJavascriptAction { script: params.str("script") }),
            "remove-javascript-action" => PdfHMutation::RemoveJavascriptAction(RemoveJavascriptAction { script: params.str("script") }),
            "insert-launch-action" => PdfHMutation::InsertLaunchAction(InsertLaunchAction { target: params.str("target") }),
            "remove-launch-action" => PdfHMutation::RemoveLaunchAction(RemoveLaunchAction { target: params.str("target") }),
            "insert-signature-field" => PdfHMutation::InsertSignatureField(InsertSignatureField { name: params.str("name") }),
            "remove-signature-field" => PdfHMutation::RemoveSignatureField(RemoveSignatureField { name: params.str("name") }),
            "embed-font-file" => PdfHMutation::EmbedFontFile(EmbedFontFile { descriptor_ordinal: params.get("descriptorOrdinal").and_then(number).unwrap_or(0.0) as usize, key: params.str("key"), program: program_reference(base, &params)? }),
            "remove-font-file" => PdfHMutation::RemoveFontFile(RemoveFontFile { descriptor_ordinal: params.get("descriptorOrdinal").and_then(number).unwrap_or(0.0) as usize }),
    ...
    pub fn mutate(ctx: &Context) -> Result<Outcome, String> {
        ...
        apply_h_conformance_mutation(&mut snapshot, &mutation);
    ...
    pub fn inverse(ctx: &Context) -> Result<Outcome, String> {
        ...
        apply_h_conformance_mutation(&mut snapshot, &forward);
        let undo = oracle_inverse_spec(&base, &spec)?;
        let backward = mutation_from_spec(&snapshot, &undo)?;
        apply_h_conformance_mutation(&mut snapshot, &backward);
        let output = encode(&snapshot)?;
        let projection = project_conformance(&output)?;
        Ok(Outcome::with_raw(output, projection))
    }
