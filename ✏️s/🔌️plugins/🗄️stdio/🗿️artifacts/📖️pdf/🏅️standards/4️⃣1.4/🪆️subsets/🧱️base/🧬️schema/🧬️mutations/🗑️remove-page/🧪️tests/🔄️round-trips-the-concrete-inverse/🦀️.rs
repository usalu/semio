...
fn op_codecs_round_trip() {
    ...
    for step in std::iter::once(payload.clone()).chain(payload.inverse(&before())) {
        ...
        assert_eq!(dsl::from_dsl_value::<PdfMutation>((serde_json::from_str::<serde_json::Value>(&dsl::json::to_json_string(&step)).expect("the payload encodes")).into()).expect("the payload decodes"), step, "remove-page/round-trips-the-concrete-inverse: the JSON form does not round-trip");
    }
}
