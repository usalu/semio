...
// 🏭️ Third-party fixture generator for the two `s.stdio.pdf@1.4/x` page-box mutations.
//
// Every before/after pair `🦀️lopdf-engine/src/🏗️generate.rs` writes is produced by the SAME registered `lopdf`
// 0.44 reference implementation named `lopdf-pdf-1-4-x-mutate` in `../🔣️oracle.json` —
...
// `../../../../../../🧪️tests/🖨️mutate-pdf-1-4-x` drives — never this repository's own production PDF
...
const SUBSET = "x";
const ORACLE_ID = "lopdf-pdf-1-4-x-mutate-reader";
const COMPARISON_PROFILE = "semantic-pdf-1-4-conformance-x-v1";
// 🧾️ Kept in step with `🦀️lopdf-engine/src/lib.rs::KINDS` (itself the same list as
// `../🔣️oracle.json`'s `pdf-1-4-x` catalog) — `manifests` walks whichever recipe directories the
...
const KINDS: readonly string[] = ["set-page-size", "collapse-page-size"];
