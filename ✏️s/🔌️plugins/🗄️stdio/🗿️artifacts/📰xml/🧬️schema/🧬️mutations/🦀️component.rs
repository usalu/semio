...
impl protocol::OpText for XmlMutation {
    fn print_op(&self) -> String {
        serde_xml::to_string(self).unwrap_or_else(|_| "{}".into())
    }
    fn parse_op(line: &str) -> Result<Self, store::TextError> {
        serde_xml::from_str(line.trim()).map_err(|e| {
...
impl protocol::OpBinary for XmlMutation {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        serde_xml::to_vec(self).map_err(|e| protocol::ProtocolError::Malformed {
            what: "op encode",
            offset: 0,
            detail: e.to_string(),
        })
    ...
    fn decode_op(bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        serde_xml::from_slice(bytes).map_err(|e| protocol::ProtocolError::Malformed {
            what: "op decode",
            offset: 0,
            detail: e.to_string(),
        })
