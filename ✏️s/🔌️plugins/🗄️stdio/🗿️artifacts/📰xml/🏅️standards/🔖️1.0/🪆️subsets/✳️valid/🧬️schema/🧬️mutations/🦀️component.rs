...
use crate::artifacts::xml::standards::v1_0::subsets::any::schema::mutations::XmlNodePath;
...
pub enum XmlValidMutation {
    ...
    SetInternalSubset {
        declarations: Vec<XmlDtdDeclaration>,
    },
...
pub fn kind_of(mutation: &XmlValidMutation) -> &'static str {
    match mutation {
        XmlValidMutation::NoMutation => "no-mutation",
        XmlValidMutation::SetSnapshot { .. } => "set-snapshot",
        XmlValidMutation::DeclareDoctype { .. } => "declare-doctype",
        XmlValidMutation::RenameDocumentElement { .. } => "rename-document-element",
        XmlValidMutation::SetExternalSubset { .. } => "set-external-subset",
        XmlValidMutation::SetStandalone { .. } => "set-standalone",
        XmlValidMutation::DeclareEntity { .. } => "declare-entity",
        XmlValidMutation::SetInternalSubset { .. } => "set-internal-subset",
        XmlValidMutation::SetText { .. } => "set-text",
    }
}
...
impl Mutation<XmlSnapshot> for XmlValidMutation {
    type Diff = XmlDiff;

    fn diff(&self, base: &XmlSnapshot) -> protocol::MutationOutcome<Self::Diff> {
        match self {
            XmlValidMutation::NoMutation => protocol::MutationOutcome::new(XmlDiff::default()),
            XmlValidMutation::SetSnapshot { snapshot } => match blocked_snapshot_violation(snapshot) {
            ...
            XmlValidMutation::DeclareDoctype { external_id } => match document_element_name(base) {
            ...
            XmlValidMutation::RenameDocumentElement { name } => match (document_element_name(base), base.doc.doctype.as_ref()) {
            ...
            XmlValidMutation::SetExternalSubset { external_id } => match base.doc.doctype.as_ref() {
            ...
            XmlValidMutation::SetStandalone { standalone } => {
            ...
            XmlValidMutation::DeclareEntity { index, parameter, name, value } => match base.doc.doctype.as_ref() {
            ...
            XmlValidMutation::SetInternalSubset { declarations } => match base.doc.doctype.as_ref() {
            ...
            XmlValidMutation::SetText { path, text } => protocol::MutationOutcome::new(diff_at_path(&path.0, XmlNodeDiff::Text { text: Some(text.clone()) })),
        }
    }

    /// ↩️ Every arm restores the exact prior state rather than an approximation of it. `DeclareDoctype`
    /// and `SetExternalSubset` need only the prior external id back, because the Name is re-derived
    /// from the (untouched) document element and the internal subset is carried across unchanged;
    /// both internal-subset kinds invert to the prior declaration LIST, because §4.2's
    /// first-declaration-binds rule makes position semantic and a name-keyed undo would not restore
    /// it. A mutation with nothing to invert against inverts to `NoMutation`: it never applied, so
    /// there is nothing to undo.
    fn inverse(&self, base: &XmlSnapshot) -> Vec<Self> {
        match self {
            XmlValidMutation::NoMutation => vec![XmlValidMutation::NoMutation],
            XmlValidMutation::SetSnapshot { .. } => vec![XmlValidMutation::SetSnapshot { snapshot: base.clone() }],
            XmlValidMutation::DeclareDoctype { .. } => match base.doc.doctype.as_ref() {
                Some(doctype) => vec![XmlValidMutation::DeclareDoctype { external_id: doctype.external_id.clone() }],
                None => vec![XmlValidMutation::SetSnapshot { snapshot: base.clone() }],
            },
            XmlValidMutation::RenameDocumentElement { .. } => match document_element_name(base) {
                Some(name) => vec![XmlValidMutation::RenameDocumentElement { name: name.to_string() }],
                None => vec![XmlValidMutation::NoMutation],
            },
            XmlValidMutation::SetExternalSubset { .. } => vec![XmlValidMutation::SetExternalSubset { external_id: base.doc.doctype.as_ref().and_then(|doctype| doctype.external_id.clone()) }],
            XmlValidMutation::SetStandalone { .. } => vec![XmlValidMutation::SetStandalone { standalone: base.doc.declaration.as_ref().and_then(|declaration| declaration.standalone) }],
            XmlValidMutation::DeclareEntity { .. } | XmlValidMutation::SetInternalSubset { .. } => match base.doc.doctype.as_ref() {
                Some(doctype) => vec![XmlValidMutation::SetInternalSubset { declarations: doctype.declarations.clone() }],
                None => vec![XmlValidMutation::NoMutation],
            },
            XmlValidMutation::SetText { path, .. } => {
                let prior = path
                    ...
                    .unwrap_or_default();
                vec![XmlValidMutation::SetText { path: path.clone(), text: prior }]
            }
        }
    }
}
...
mod tests {
    ...
    fn kinds_matches_enum_variants_in_declaration_order() {
        let one_per_variant = vec![
            XmlValidMutation::NoMutation,
            XmlValidMutation::SetSnapshot { snapshot: XmlSnapshot::default() },
            XmlValidMutation::DeclareDoctype { external_id: None },
            XmlValidMutation::RenameDocumentElement { name: "x".into() },
            XmlValidMutation::SetExternalSubset { external_id: None },
            XmlValidMutation::SetStandalone { standalone: None },
            XmlValidMutation::DeclareEntity { index: 0, parameter: false, name: "e".into(), value: "v".into() },
            XmlValidMutation::SetInternalSubset { declarations: Vec::new() },
            XmlValidMutation::SetText { path: XmlNodePath(vec![0]), text: "t".into() },
        ];
    ...
    fn set_snapshot_refuses_a_replacement_that_is_not_valid() {
        ...
        let (next, outcome) = applied(&base, &XmlValidMutation::SetSnapshot { snapshot: <XmlSnapshot as store::ArtifactDsl>::parse_dsl("<plist/>").expect("parses") });
    ...
    fn declare_doctype_derives_the_name_from_the_document_element() {
        ...
        let (next, _) = applied(&base, &XmlValidMutation::DeclareDoctype { external_id: Some(XmlExternalId::System { system_id: "logreq.dtd".into() }) });
    ...
    fn rename_document_element_retags_the_doctype_in_the_same_step() {
        let base = valid_document();
        let (next, _) = applied(&base, &XmlValidMutation::RenameDocumentElement { name: "propertyList".into() });
    ...
    fn declare_entity_refuses_a_duplicate_name() {
        let base = valid_document();
        let (with_entity, _) = applied(&base, &XmlValidMutation::DeclareEntity { index: 0, parameter: false, name: "semio".into(), value: "Semio".into() });
        let (_, duplicate) = applied(&with_entity, &XmlValidMutation::DeclareEntity { index: 0, parameter: false, name: "semio".into(), value: "Other".into() });
        ...
        let (_, no_doctype) = applied(&<XmlSnapshot as store::ArtifactDsl>::parse_dsl("<plist/>").expect("parses"), &XmlValidMutation::SetInternalSubset { declarations: Vec::new() });
    ...
    fn declare_entity_inserts_at_the_declared_index_and_inverts_to_the_prior_list() {
        let base = valid_document();
        let seeded = XmlValidMutation::SetInternalSubset {
    ...
    fn every_kind_round_trips_through_its_own_inverse() {
        ...
        let cases = vec![
            XmlValidMutation::NoMutation,
            XmlValidMutation::SetSnapshot { snapshot: <XmlSnapshot as store::ArtifactDsl>::parse_dsl("<!DOCTYPE root>\n<root><child>text</child></root>").expect("parses") },
            XmlValidMutation::DeclareDoctype { external_id: Some(XmlExternalId::System { system_id: "plist.dtd".into() }) },
            XmlValidMutation::RenameDocumentElement { name: "propertyList".into() },
            XmlValidMutation::SetExternalSubset { external_id: None },
            XmlValidMutation::SetStandalone { standalone: Some(true) },
            XmlValidMutation::DeclareEntity { index: 0, parameter: false, name: "semio".into(), value: "Semio".into() },
            XmlValidMutation::SetInternalSubset { declarations: vec![XmlDtdDeclaration::Entity { parameter: true, name: "shared".into(), value: "<!ELEMENT dummy EMPTY>".into() }] },
            XmlValidMutation::SetText { path: XmlNodePath(vec![0, 0]), text: "Renamed".into() },
        ];
    ...
    fn set_standalone_is_exact_in_every_declaration_combination() {
        for source in ["<!DOCTYPE root>\n<root/>", "<?xml version=\"1.0\"?>\n<!DOCTYPE root>\n<root/>", "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\n<!DOCTYPE root>\n<root/>"] {
            ...
            for target in [None, Some(true), Some(false)] {
                let mutation = XmlValidMutation::SetStandalone { standalone: target };
