...
//!
//! Two subcommands:
//!   build   <recipe-id> <out-dir>   — writes <out-dir>/<recipe-id>/before.xml [and after.xml]
...
//! recipe here writes both `before.xml` and `after.xml`; there is no `-rejected-*` recipe in this
//! corpus.
//!
...
/// `before.xml` — the same "assert what production actually produces" discipline the sibling AVI
...
fn cmd_build(id: &str, out_dir: &str) -> i32 {
    ...
    fs::write(dir.join("before.xml"), encode_xml(&before)).expect("write before.xml");
    fs::write(dir.join("after.xml"), encode_xml(&after)).expect("write after.xml");
    eprintln!("[quick-xml-oracle-codec] {id}: before.xml + after.xml -> {}", dir.display());
    0
}
    ...
    let code = match args.get(1).map(String::as_str) {
        Some("build") => {
            let (Some(id), Some(out_dir)) = (args.get(2), args.get(3)) else {
                eprintln!("usage: quick-xml-oracle-codec build <recipe-id> <out-dir>");
                std::process::exit(2);
            };
        ...
        _ => {
            eprintln!("usage: quick-xml-oracle-codec build <recipe-id> <out-dir> | project <path-to-xml> | list-recipes");
            2
        }
    };
