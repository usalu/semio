...
pub fn register() {
    ...
    store::register_document_codec(store::DocumentCodec::of::<XmlSnapshot, XmlMutation>(STDIO_XML_DOCUMENT_SCHEMA));
}
...
mod tests {
    ...
    fn codec_round_trip() {
        ...
        let parsed = <XmlSnapshot as store::DocumentDsl>::parse_dsl(&text).expect("parse");
        assert_eq!(parsed.schema, snap.schema);
        let bytes = store::DocumentPack::encode_pack(&snap);
        let decoded = <XmlSnapshot as store::DocumentPack>::decode_pack(&bytes).expect("decode");
