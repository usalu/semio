...
const RECIPES: Recipe[] = [
  ...
  { id: "insert-topic-rejected-duplicate", subset: "🖊️markup", directory: "👯️insert-topic-rejected-duplicate", outcome: "rejected", build: () => ({ before: BASE_TOPICS }) },
  { id: "remove-topic-applied", subset: "🖊️markup", directory: "🗑️remove-topic-applied", outcome: "applied", build: () => ({ before: BASE_TOPICS, after: BASE_TOPICS.filter((t) => t.guid !== "topic-review-02") }) },
  { id: "remove-topic-rejected-missing", subset: "🖊️markup", directory: "🕳️remove-topic-rejected-missing", outcome: "rejected", build: () => ({ before: BASE_TOPICS }) },
  {
    id: "set-topic-markup-applied", subset: "🖊️markup", directory: "🖊️set-topic-markup-applied",
    outcome: "applied",
    build: () => ({ before: BASE_TOPICS, after: BASE_TOPICS.map((t) => (t.guid === "topic-clash-01" ? { ...t, status: "Closed", priority: "Low" } : t)) }),
  },
  {
    id: "insert-comment-applied", subset: "🖊️markup", directory: "🗨️insert-comment-applied",
    outcome: "applied",
    build: () => ({
      before: BASE_TOPICS,
  ...
  { id: "remove-comment-applied", subset: "🖊️markup", directory: "🧹️remove-comment-applied", outcome: "applied", build: () => ({ before: BASE_TOPICS, after: BASE_TOPICS.map((t) => (t.guid === "topic-clash-01" ? { ...t, comments: [] } : t)) }) },
  {
    id: "set-comment-applied", subset: "🖊️markup", directory: "✏️set-comment-applied",
    outcome: "applied",
    build: () => ({
      before: BASE_TOPICS,
  ...
  {
    id: "insert-viewpoint-applied", subset: "👁️viewpoint", directory: "👁️insert-viewpoint-applied",
    outcome: "applied",
    build: () => ({
      before: BASE_TOPICS,
  ...
  { id: "remove-viewpoint-applied", subset: "👁️viewpoint", directory: "🙈️remove-viewpoint-applied", outcome: "applied", build: () => ({ before: BASE_TOPICS, after: BASE_TOPICS.map((t) => (t.guid === "topic-clash-01" ? { ...t, viewpoints: [] } : t)) }) },
  { id: "remove-viewpoint-rejected-missing", subset: "👁️viewpoint", directory: "🕳️remove-viewpoint-rejected-missing", outcome: "rejected", build: () => ({ before: BASE_TOPICS }) },
  {
    id: "set-viewpoint-camera-applied", subset: "👁️viewpoint", directory: "📷️set-viewpoint-camera-applied",
    outcome: "applied",
    build: () => ({
      before: BASE_TOPICS,
  ...
  {
    id: "set-viewpoint-components-applied", subset: "👁️viewpoint", directory: "🧱️set-viewpoint-components-applied",
    outcome: "applied",
    build: () => ({
      before: BASE_TOPICS,
...
//#region 🚀️Entry
async function generateOne(id: string, outDir: string): Promise<string> {
  ...
  const dir = join(outDir, recipe.subset, "🧫️fixtures", recipe.directory);
  mkdirSync(dir, { recursive: true });
  writeFileSync(join(dir, "⬅️before.bcf"), await buildBcf(beforeVersion ?? "2.1", before));
  if (recipe.outcome === "applied") {
    if (!after) throw new Error(`recipe ${id} is declared applied but has no after state`);
    writeFileSync(join(dir, "➡️after.bcf"), await buildBcf(afterVersion ?? beforeVersion ?? "2.1", after));
  }
  return dir;
}
...
async function main(argv: readonly string[]): Promise<number> {
  ...
  const outDir = process.env.SEMIO_FIXTURE_OUT ?? join(import.meta.dir, "..", "..");
  const ids = only ? [only] : RECIPES.map((entry) => entry.id);
  for (const id of ids) {
    const directory = await generateOne(id, outDir);
    console.log(`[bcf generator] ${id} -> ${directory}`);
  }
  return 0;
}
