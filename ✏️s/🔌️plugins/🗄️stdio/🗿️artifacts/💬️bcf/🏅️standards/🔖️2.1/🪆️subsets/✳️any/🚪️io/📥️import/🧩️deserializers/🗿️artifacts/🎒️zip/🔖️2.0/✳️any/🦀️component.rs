...
pub async fn deserialize(from: &BinarySnapshot) -> Result<BcfSnapshot, store::PackError> {
    let mut snap = crate::artifacts::bcf::io::decode_bcf(&from.bytes).await.map_err(|e| store::PackError::Schema(e))?;
    snap.schema = STDIO_BCF_DOCUMENT_SCHEMA.into();
    Ok(snap)
}

/// Decode a Binary pack then parse ZIP.
pub async fn deserialize_bytes(bytes: &[u8]) -> Result<BcfSnapshot, store::PackError> {
