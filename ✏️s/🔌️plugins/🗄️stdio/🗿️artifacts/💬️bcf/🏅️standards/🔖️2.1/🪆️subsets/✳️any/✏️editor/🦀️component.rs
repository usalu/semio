...
impl ArtifactEditor for BcfAnyEditor {
    ...
    async fn initial_snapshot() -> BcfSnapshot {
        BcfSnapshot::default()
    }
...
//#region 🔖️Manifest
pub async fn create_bcf_any_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(BCF_ANY_DIALECT)
        .await.document(["stdio", "bcf"])
        .await.icon_id("box")
        .await.mode_def(edit::definition().await)
        .await.default_mode_id(edit::BCF_ANY_EDIT_MODE_ID)
...
mod tests {
    ...
    async fn editor_and_viewer_share_one_dialect() {
        semio_framework_plugin::testkit::assert_editor_and_viewer_share_dialect::<BcfAnyEditor, crate::viewer::bcf::BcfAnyViewer>();
    }
}
//#endregion 🧪️Tests
