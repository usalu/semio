//! 🎹️ BcfComposer (raw/✳️any at 2.1) — analyzer + builder glued. Reads native
//! `stdio.bcf` sources (plus its DAG dependencies: zip, xml), writes one `stdio.bcf` (2.1/✳️any) snapshot.
...
use crate::artifacts::bcf::standards::v2_1::subsets::any::analyzer::BcfAnalyzer;
...
pub struct BcfComposer;

impl ArtifactComposer for BcfComposer {
    ...
    fn compose(sources: &[ComposeSource]) -> Result<Composition<Self::Snapshot>, ComposeError> {
        ...
        let native: Vec<AnalyzeSource<'_>> = sources
            ...
            .collect();
        if native.is_empty() {
            return Err(ComposeError { message: "BcfComposer: no source in a known read dialect".into(), diagnostics: Vec::new() });
        }
        let analysis = BcfAnalyzer::analyze(&native);
        let snapshot = analysis.parts.snapshot.ok_or_else(|| ComposeError {
            message: "BcfComposer: analysis produced no snapshot".into(),
