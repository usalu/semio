...
pub fn register() {
    crate::artifacts::ply::io::register();
    register_artifact_schema();
    register_pilot_languages();
    store::register_document_codec(store::DocumentCodec::of::<PlySnapshot, PlyMutation>(STDIO_PLY_DOCUMENT_SCHEMA));
}
...
mod tests {
    ...
    fn codec_round_trip() {
        ...
        let parsed = <PlySnapshot as store::DocumentDsl>::parse_dsl(&text).expect("parse");
        assert_eq!(parsed.schema, snap.schema);
        let bytes = store::DocumentPack::encode_pack(&snap);
        let decoded = <PlySnapshot as store::DocumentPack>::decode_pack(&bytes).expect("decode");
