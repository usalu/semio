...
impl protocol::OpBinary for PlyAnyEditCommand {
    ...
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(PlyAnyEditCommand::SetVertex)
    }
}
//#endregion 🔖️Command
...
impl ArtifactEditor for PlyAnyEditor {
    ...
    async fn initial_snapshot() -> PlySnapshot {
        PlySnapshot::default()
    }
...
pub fn create_ply_any_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(PLY_ANY_DIALECT).document(["stdio", "ply"]).icon_id("box").mode_def(edit::definition()).default_mode_id(edit::PLY_ANY_EDIT_MODE_ID).window_kind_def(main::definition()).default_layout(edit::layout()).build_definition()
}
//#endregion 🔖️Manifest
...
mod tests {
    ...
    async fn editor_and_viewer_share_one_dialect() {
        semio_framework_plugin::testkit::assert_editor_and_viewer_share_dialect::<PlyAnyEditor, crate::viewer::ply::PlyAnyViewer>();
    }
}
//#endregion 🧪️Tests
