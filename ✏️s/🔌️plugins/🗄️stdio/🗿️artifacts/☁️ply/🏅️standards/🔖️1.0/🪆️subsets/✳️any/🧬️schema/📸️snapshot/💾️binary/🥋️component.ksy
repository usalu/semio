...
doc: |
  ...
  `⚙️engine/🦀️component.rs`), which parses the SAME header text this file's `header_lines`
  exposes and then reads exactly the widths the sibling `📡️component.protocol.semio` documents.
seq:
  - id: header_lines
    type: header_line
    repeat: until
