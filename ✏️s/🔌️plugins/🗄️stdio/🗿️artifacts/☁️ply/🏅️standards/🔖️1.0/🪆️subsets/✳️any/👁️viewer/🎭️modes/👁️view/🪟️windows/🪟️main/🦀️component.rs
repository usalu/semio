...
async fn entity_count(document: &PlySnapshot) -> usize {
...
async fn world_instances_json(document: &PlySnapshot) -> String {
...
/// 👁️ Pure `PlySnapshot -> UiNode` read: default camera (a viewer has no persisted
/// per-session camera — `Config = NoConfig`), no selection/gumball/engagement overlay.
pub async fn render(document: &PlySnapshot) -> UiNode {
    let meshes_json = serde_json::to_string(&[serde_json::json!({ "id": PLY_ANY_VIEW_FALLBACK_MESH_KIND, "data": mesh_from_kind(PLY_ANY_VIEW_FALLBACK_MESH_KIND) })]).unwrap_or_else(|_| "[]".into());
    let view = MeshView {
        camera_json: world3d_camera_json(PLY_ANY_VIEW_DEFAULT_CAMERA_POSITION, PLY_ANY_VIEW_DEFAULT_CAMERA_TARGET, PLY_ANY_VIEW_DEFAULT_CAMERA_FOV).await,
        meshes_json,
