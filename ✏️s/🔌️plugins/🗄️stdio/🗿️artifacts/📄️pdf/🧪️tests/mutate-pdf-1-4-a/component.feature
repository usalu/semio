...
Feature: Apply every typed ISO 19005-1 (PDF/A-1) conformance mutation to a real document
  ...
  check_pdf_a_conformance says so in as many words: it raises exactly two diagnostics,
  stdio.pdf.a.text-empty and stdio.pdf.a.schema-gap-unverifiable, and the second fires unconditionally on every document to record that full
  conformance cannot be checked from this schema at all. A vocabulary derived honestly from that
  checker therefore has exactly ONE movable axis, page 1's extractable text — whether the first page shows any text at all,
