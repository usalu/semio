...
//!
//! @see ../../🏅️standards/🔖️1.7/🪆️subsets/✳️h/🧬️schema/🦀️component.rs — `check_h_conformance`, the one
...
const KINDS: &[&str] = &["no-mutation", "set-snapshot", "set-info-title", "set-info-author", "insert-javascript-action", "remove-javascript-action", "insert-launch-action", "remove-launch-action", "insert-signature-field", "remove-signature-field", "embed-font-file", "remove-font-file"];
//#endregion 🔖️Kinds
...
mod subject {
    ...
    use semio_s_plugin_stdio::artifacts::pdf::standards::v1_7::subsets::h::schema::mutations::{apply_h_conformance_mutation, stamp_conformance, PdfHMutation};
    ...
    fn mutation_from_spec(base: &PdfSnapshot, spec: &Json) -> Result<PdfHMutation, String> {
        ...
        Ok(match spec.str("kind").as_str() {
            "no-mutation" => PdfHMutation::NoMutation,
            "set-snapshot" => PdfHMutation::SetSnapshot { snapshot: stamp_conformance(base.clone(), params.str("conformance") == "stamped") },
            "set-info-title" => PdfHMutation::SetInfoTitle { title: params.str("title") },
            "set-info-author" => PdfHMutation::SetInfoAuthor { author: params.str("author") },
            "insert-javascript-action" => PdfHMutation::InsertJavaScriptAction { script: params.str("script") },
            "remove-javascript-action" => PdfHMutation::RemoveJavaScriptAction { script: params.str("script") },
            "insert-launch-action" => PdfHMutation::InsertLaunchAction { target: params.str("target") },
            "remove-launch-action" => PdfHMutation::RemoveLaunchAction { target: params.str("target") },
            "insert-signature-field" => PdfHMutation::InsertSignatureField { name: params.str("name") },
            "remove-signature-field" => PdfHMutation::RemoveSignatureField { name: params.str("name") },
            "embed-font-file" => PdfHMutation::EmbedFontFile { descriptor_ordinal: params.get("descriptorOrdinal").and_then(number).unwrap_or(0.0) as usize, key: params.str("key"), program: program_reference(base, &params)? },
            "remove-font-file" => PdfHMutation::RemoveFontFile { descriptor_ordinal: params.get("descriptorOrdinal").and_then(number).unwrap_or(0.0) as usize },
