...
Feature: Apply every typed ISO 15930-7 (PDF/X-4) conformance-class mutation to a real document
  ...
  owns ISO 15930-7 (PDF/X-4), a CONFORMANCE CLASS, which is a property of the object graph as a whole and
  of no page at all. The catalog is one kind per axis of this subset's own check_x_conformance, which
  reads seven axes: any Standard Security Handler `/Encrypt` dictionary object, an `/S /GTS_PDFX` OutputIntent that also carries `/DestOutputProfile`, a `/TrimBox` or `/ArtBox` on EVERY `/Type /Page`, an embedded program on every font's `/FontDescriptor`, any `/S /JavaScript` action or bare `/JS` key, any `/S /Launch` action, and any `/Subtype /Movie` or `/Subtype /Sound` annotation. No ✳️any mutation moves any of those axes and no mutation here touches
  ...
  /Encrypt. That is faithful to what this subset actually checks — check_x_conformance scans the retained
  ...
  divergence for it. What the projection carries instead is exactly the content check_x_conformance's own
