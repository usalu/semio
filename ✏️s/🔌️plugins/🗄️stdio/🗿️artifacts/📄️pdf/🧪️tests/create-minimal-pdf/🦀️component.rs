...
fn oracle(ctx: &Context) -> Result<Outcome, String> {
    let spec = spec_for(&ctx.scenario.id)?;
    let bytes = oracle_create_pdf(&spec)?;
...
fn subject(ctx: &Context) -> Result<Outcome, String> {
    let spec = spec_for(&ctx.scenario.id)?;
    let bytes = encode_pdf(&snapshot_for(&spec)).map_err(|error| format!("encode_pdf failed: {:?}", error))?;
    let projection = project_pdf(&bytes)?;
    Ok(Outcome::with_raw(bytes, projection))
}

/// 🧭️ Every scenario in this feature is created the same way, so one pair of handlers serves all.
fn register(adapter: Adapter, scenario: &'static str) -> Adapter {
    adapter.oracle(scenario, oracle).subject(scenario, subject)
}
//#endregion 🔖️Roles
...
pub fn adapter() -> Adapter {
    ...
    for scenario in ["one-empty-a4-page", "three-empty-pages", "document-title-and-author"] {
        built = register(built, scenario);
    }
    built
}
