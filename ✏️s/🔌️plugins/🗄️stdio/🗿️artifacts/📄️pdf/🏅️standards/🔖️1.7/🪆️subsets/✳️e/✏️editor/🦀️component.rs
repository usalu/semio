//! 📄️ PDF/E Document (1.7) editor -- one of stdio's 10 real PDF subset editors (ticket
...
//! document schema id (confirmed by direct read before writing this file). `Pdf17EEditor`
//! implements `ArtifactEditor` over that shared type, tagged with this file's own `PDF17E_DIALECT`
//! (`standard: "1.7"`, `subset: "e"`) -- subsets are constraint PROFILES of the same
...
impl protocol::OpText for Pdf17EEditorCommand {
...
impl protocol::OpBinary for Pdf17EEditorCommand {
...
pub struct Pdf17EEditor;

impl ArtifactEditor for Pdf17EEditor {
    ...
    type Command = Pdf17EEditorCommand;
    ...
    async fn handle(
        ...
        match command {
            Pdf17EEditorCommand::SetPage { index, text } => {
...
pub fn create_pdf17_e_editor() -> semio_framework_plugin::AppDefinition {
    let builder = Editor::builder(PDF17E_DIALECT);
    let builder = semio_framework_plugin::resolve_ready(builder.document(["stdio", "pdf", "1.7", "e"]));
    ...
    let builder = semio_framework_plugin::resolve_ready(builder.default_mode_id(edit::PDF17E_EDIT_MODE_ID));
