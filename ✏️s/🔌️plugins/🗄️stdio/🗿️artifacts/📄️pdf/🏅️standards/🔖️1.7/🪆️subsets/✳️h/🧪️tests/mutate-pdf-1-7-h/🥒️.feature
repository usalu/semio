...
Feature: Apply every typed the PDF Healthcare Best Practices Guide (PDF/H) conformance-class mutation to a real document
  ...
  reads five axes: a populated `Info.title` AND `Info.author`, any `/S /JavaScript` action or bare `/JS` key, any `/S /Launch` action, an `/AcroForm` field with `/FT /Sig`, and an embedded program on every font's `/FontDescriptor`. No ✳️any mutation moves any of those axes and no mutation here touches
