...
//! (`../🧬️schema/🦀️component.rs`), which reads seven axes: any Standard Security Handler `/Encrypt` dictionary object, an `/S /GTS_PDFX` OutputIntent that also carries `/DestOutputProfile`, a `/TrimBox` or `/ArtBox` on EVERY `/Type /Page`, an embedded program on every font's `/FontDescriptor`, any `/S /JavaScript` action or bare `/JS` key, any `/S /Launch` action, and any `/Subtype /Movie` or `/Subtype /Sound` annotation.
//!
...
//! @see ../🧬️schema/🦀️component.rs — `check_x_conformance`, the one axis list everything here derives from.
...
mod tests {
    ...
    fn a_kind_this_subset_does_not_declare_is_refused_even_when_the_engine_could_perform_it() {
        let undeclared = ["insert-embedded-file", "insert-signature-field", "remove-af-relationship", "remove-display-doc-title", "remove-dpart-metadata", "remove-dpart-root", "remove-embedded-file", "remove-lang", "remove-mark-info", "remove-signature-field", "remove-struct-tree-root", "set-af-relationship", "set-display-doc-title", "set-dpart-metadata", "set-dpart-root", "set-info-author", "set-info-title", "set-lang", "set-mark-info", "set-struct-tree-root"];
