//! 🏗️ PdfA2bBuilder (1.7/✳️a-2b) — a typed builder whose ergonomic path can only produce a
//! PDF/A-2b-conforming `PdfSnapshot` BY CONSTRUCTION (D5 requirement #3):
...
//!   PDF/A-2b actually allows content-wise.
//!
...
//! conforming-only is `build()` itself: it re-runs the SAME `check_pdf_a2b_conformance` used by
//! `PdfA2bComposer`, unconditionally, regardless of which path produced the in-flight snapshot --
//! so a hard PDF/A-2b violation can never leave this builder as an `Ok(PdfSnapshot)`, no matter
...
/// (`/S /GTS_PDFA1`, ISO 19005-2's own conformance marker) -- a genuine, well-formed PDF/A
...
// 🔗 `apply_pdf_mutation` (from `any::schema::mutations`) now returns the mutation's `PdfDiff`
// (needed by `ArtifactBuilder::mutate`'s `-> (Self, Self::Diff)` signature below); the two
// ergonomic methods above only need the mutated snapshot, so they discard it deliberately.

impl ArtifactBuilder for PdfA2bBuilder {
    ...
    /// regardless. Prefer `PdfA2bBuilder::new(condition)` directly wherever the real condition is
    /// known.
    fn empty() -> Self {
    ...
    /// `from_binary`, a raw `mutate(SetSnapshot { .. })`), a hard PDF/A-2b violation fails
    /// `build()` -- soft diagnostics (missing OutputIntent, non-embedded font) pass through as
    /// advisory `Diagnostic`s on the `Err` path is NOT taken for those; only hard ones block.
    fn build(self) -> Result<Self::Snapshot, Vec<Diagnostic>> {
        let hard: Vec<Diagnostic> = check_pdf_a2b_conformance(&self.snapshot).into_iter().filter(|d| matches!(d.severity, Severity::Error | Severity::Fatal)).collect();
        if hard.is_empty() {
            Ok(self.snapshot)
        } else {
            Err(hard)
...
mod tests {
    ...
    fn new_requires_output_intent_and_builds_clean() {
        let snapshot = PdfA2bBuilder::new("sRGB IEC61966-2.1")
            .add_page(PdfPage::new(200.0, 200.0))
            .set_info(PdfInfo { title: Some("A2b Test".into()), ..PdfInfo::default() })
            .build()
            .expect("conforming construction must build");
        assert_eq!(snapshot.pages.len(), 1);
        assert_eq!(snapshot.info.title.as_deref(), Some("A2b Test"));
    }
    ...
    fn hard_violation_injected_via_raw_mutate_still_fails_build() {
        ...
        let mut snapshot = PdfA2bBuilder::new("sRGB IEC61966-2.1").add_page(PdfPage::new(100.0, 100.0)).build().unwrap();
        snapshot.objects.push(violating);
        // Even routed back in via the generic `SetSnapshot` escape hatch, `build()` still catches it.
        let (mutated, _diff) = PdfA2bBuilder::from_snapshot(PdfSnapshot::default()).mutate(PdfMutation::SetSnapshot { snapshot });
        let err = mutated.build().expect_err("a /Launch action must fail build()");
        assert!(err.iter().any(|d| d.code.0 == crate::artifacts::pdf::standards::v1_7::subsets::a2b::analyzer::CODE_LAUNCH));
    }
}
