//! 🧺️ Authoritative PDF/A mutation for detaching a font program from a font descriptor.
...
impl MutationKind<PdfSnapshot, PdfAMutation> for RemoveFontFile {
    ...
    fn inverse(&self, base: &PdfSnapshot) -> Vec<PdfAMutation> {
        support::font_descriptors(base)
            ...
            .map(|(key, program)| PdfAMutation::EmbedFontFile(EmbedFontFile { descriptor_ordinal: self.descriptor_ordinal, key, program }))
            .into_iter()
            .collect()
    }
