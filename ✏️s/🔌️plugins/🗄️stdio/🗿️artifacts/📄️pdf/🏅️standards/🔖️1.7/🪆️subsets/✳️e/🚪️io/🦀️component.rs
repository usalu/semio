...
pub mod derived_composition {
    ...
    mod tests {
        ...
        async fn conforming_builder_snapshot_composes_and_stamps_e() {
            let snapshot = PdfEBuilder::new().await.add_page(crate::artifacts::pdf::standards::v1_7::subsets::any::schema::snapshot::PdfPage::new(100.0, 100.0)).await.build().await.unwrap();
            let bytes = <PdfSnapshot as store::ArtifactPack>::encode_pack(&snapshot);
            let sources = vec![ComposeSource { dialect: DIALECT_ANY, payload: AnalyzeSource::Binary(&bytes) }];
            let composed = PdfEComposerComposition::compose(&sources).await.expect("clean document must compose to e");
            assert!(composed.diagnostics.iter().all(|d| d.severity != Severity::Error), "no hard diagnostics expected: {:?}", composed.diagnostics);
        }

        #[semio_framework_async_macros::async_test]
        async fn subset_validator_recheck_runs_the_same_check() {
            let snapshot = PdfEBuilder::new().await.add_page(crate::artifacts::pdf::standards::v1_7::subsets::any::schema::snapshot::PdfPage::new(50.0, 50.0)).await.build().await.unwrap();
            let bytes = <PdfSnapshot as store::ArtifactPack>::encode_pack(&snapshot);
