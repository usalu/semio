//! 🧽️ Authoritative PDF/VT mutation for remove trim box.
...
impl MutationKind<PdfSnapshot, PdfVtMutation> for RemoveTrimBox {
    ...
    fn inverse(&self, base: &PdfSnapshot) -> Vec<PdfVtMutation> {
        support::page_objects(base).get(self.page_index).copied()
            ...
            .map(|trim_box| PdfVtMutation::SetTrimBox(SetTrimBox { page_index: self.page_index, trim_box }))
            .into_iter()
            .collect()
    }

    fn label(&self) -> String {
        format!("Remove PDF/VT trim box on page {}", self.page_index)
    }
