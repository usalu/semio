//! 🎹️ PdfAComposer (1.7/✳️a) — reads the same sources the ✳️any subset does (native
//! `stdio.pdf` 1.7, plus its `binary`/`deflate` DAG deps), delegates the actual parse to the
//! ✳️any composer, then HARD-GATES the `a` dialect stamp on real PDF/A conformance (D5
//! requirement #2: "Dialect stamped 1.7/a only when clean"). A hard violation
//! (`/Encrypt`/JavaScript/`/Launch`/embedded file missing `/AFRelationship`) fails composition
//! outright with specific `Diagnostic`s naming what's wrong; a soft one (missing `OutputIntent`,
//! non-embedded font) passes through as an advisory diagnostic on the successful `Composition`,
//! alongside the always-informational level-detection `Diagnostic` (`stdio.pdf.a.level`).
//!
//! Also registers this dialect's `SubsetValidator` (D5's generic validate-on-build hook, see
//! `🧰️framework/🔨️modules/🚪️io/🦀️component.rs`) — the SAME `check_pdf_a_conformance` function
...
//! the authoritative "was this ever a clean `a` document" answer.
//!
//! W2 restructure (ticket 26/08/11/ARTIFACT-STANDARD-SUBSETS-REAL-VOCABULARIES): renamed from
//! `PdfA2bComposer`/`✳️a-2b` -- the conformance LEVEL (2b/2u/3b/3u) is no longer part of the
//! dialect id, see `🧐️analyzer::detect_pdfa_level`.
...
//#region 🔖️Composer
pub struct PdfAComposer;

impl ArtifactComposer for PdfAComposer {
...
mod tests {
    ...
    use crate::artifacts::pdf::standards::v1_7::subsets::a::analyzer::{CODE_JAVASCRIPT, CODE_LAUNCH};
    use crate::artifacts::pdf::standards::v1_7::subsets::a::builder::PdfABuilder;
    ...
    fn conforming_builder_snapshot_composes_and_stamps_a() {
        ...
        let composed = PdfAComposer::compose(&sources).expect("clean document must compose to a");
    ...
    fn javascript_action_reachable_from_open_action_fails_compose_with_real_diagnostic() {
        ...
        let err = PdfAComposer::compose(&sources).expect_err("a document with a JS action must not stamp a");
    ...
    fn launch_action_reachable_from_open_action_fails_compose_with_real_diagnostic() {
        ...
        let hex = hex_encode(&bytes);
        let sources = vec![ComposeSource { dialect: DIALECT_ANY, payload: AnalyzeSource::Text(&hex) }];
        let err = PdfAComposer::compose(&sources).expect_err("a document with a Launch action must not stamp a");
    ...
    fn encrypted_trailer_document_is_rejected_upstream_by_the_shared_engine() {
        ...
        let sources = vec![ComposeSource { dialect: DIALECT_ANY, payload: AnalyzeSource::Text(&hex) }];
        let err = PdfAComposer::compose(&sources).expect_err("an /Encrypt trailer must never compose, at a or any other dialect");
        assert!(
