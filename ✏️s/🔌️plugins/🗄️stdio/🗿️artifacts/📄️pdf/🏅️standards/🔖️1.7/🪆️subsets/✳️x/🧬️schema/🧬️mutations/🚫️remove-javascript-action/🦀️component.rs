//! 🚫️ Authoritative PDF/X mutation for removing a matching JavaScript action.
...
impl MutationKind<PdfSnapshot, PdfXMutation> for RemoveJavascriptAction {
    ...
    fn inverse(&self, base: &PdfSnapshot) -> Vec<PdfXMutation> {
        support::action_with(base, "JavaScript", "JS", &self.script)
            .map(|_| PdfXMutation::InsertJavascriptAction(InsertJavascriptAction { script: self.script.clone() }))
            .into_iter()
            .collect()
    }
...
mod tests {
    ...
    fn removes_the_matching_script_action() {
        ...
        let outcome = <RemoveJavascriptAction as MutationKind<PdfSnapshot, PdfXMutation>>::diff(&mutation, &base);
