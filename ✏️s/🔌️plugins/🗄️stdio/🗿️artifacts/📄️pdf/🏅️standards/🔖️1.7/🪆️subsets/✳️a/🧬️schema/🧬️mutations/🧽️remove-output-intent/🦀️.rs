//! 🧽️ Authoritative PDF/A mutation for removing the catalog output intent.
...
impl MutationKind<PdfSnapshot, PdfAMutation> for RemoveOutputIntent {
...
mod tests {
    ...
    fn removes_the_catalog_output_intent() {
        ...
        let outcome = <RemoveOutputIntent as MutationKind<PdfSnapshot, PdfAMutation>>::diff(&mutation, &base);
        let next = outcome.diff().apply(&base).unwrap();
        assert!(support::output_intent_identifier(&next).is_none());
        assert_eq!(<RemoveOutputIntent as MutationKind<PdfSnapshot, PdfAMutation>>::inverse(&mutation, &base).len(), 1);
    }
}
//#endregion 🧪️Tests
