//! 👁️ PDF/UA Document (1.7) viewer -- the read-only counterpart of the mutation-capable surface for this
//! subset (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `Pdf17UaViewer`
//! implements `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` -- `ViewerApp<Pdf17UaViewer>`
...
/// coordinate -- `s.stdio.pdf@1.7/ua` -- measured directly against `PDF_ARTIFACT_SCHEMA_ID`
...
pub const PDF17UA_DIALECT: Dialect = Dialect { artifact_kind: PDF_ARTIFACT_SCHEMA_ID, standard: StandardId("1.7"), subset: SubsetId("ua") };
//#endregion 🔖️Dialect

//#region 🔖️Command
/// 👁️ The viewer declares no actions, so its typed command channel has exactly one inert variant.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub enum Pdf17UaViewCommand {
    #[default]
    Noop,
}
...
pub struct Pdf17UaViewer;

impl ArtifactViewer for Pdf17UaViewer {
    ...
    type Command = Pdf17UaViewCommand;
...
pub fn create_pdf17_ua_viewer() -> semio_framework_plugin::AppDefinition {
    let builder = Viewer::builder(PDF17UA_DIALECT);
    let builder = semio_framework_plugin::resolve_ready(builder.document(["stdio", "pdf", "1.7", "ua"]));
    ...
    let builder = semio_framework_plugin::resolve_ready(builder.default_mode_id(view::PDF17UA_VIEW_MODE_ID));
