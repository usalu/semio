/** 🪟️ PDF/H Document (1.7) viewer -- `main` window: READ-ONLY typed twin of `component.rs`'s
...
export interface Pdf17HDocumentPage {
  text: string;
}

/** 👁️ The `main` window's typed view-model -- the TS mirror of the Rust `render()` boundary's
 * input (a bare `PdfSnapshot`). */
export interface Pdf17HDocumentViewModel {
  windowKindId: "framework.window.document";
  bodyKey: "framework.window.document";
  pages: Pdf17HDocumentPage[];
}

export const PDF17H_MAIN_WINDOW_KIND_ID = "framework.window.document" as const;
export const PDF17H_MAIN_BODY_KEY = "framework.window.document" as const;
