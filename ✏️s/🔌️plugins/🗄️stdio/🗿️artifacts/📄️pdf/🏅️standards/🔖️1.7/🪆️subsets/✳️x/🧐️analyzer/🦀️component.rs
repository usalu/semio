//! 🧐️ PdfXAnalyzer (1.7/✳️x) — real ISO 15930-7:2010 (PDF/X-4) conformance checks against the
//! retained `PdfSnapshot.objects` graph (same object-graph-backed pattern as 1.7's `✳️a`). Ticket
...
//! parse):
//! - HARD: `/Encrypt`-shaped Standard Security Handler dictionary anywhere in the retained object
//!   graph — PDF/X forbids encryption.
//! - HARD: `/Root`'s `/OutputIntents` array has no entry with `/S /GTS_PDFX` carrying a
//!   `/DestOutputProfile` (the ICC output-intent profile PDF/X-4 requires).
//! - HARD: some `/Type /Page` object has neither a `/TrimBox` nor an `/ArtBox` key — ISO 15930-7
//!   requires one of the two on every page.
//! - SOFT: a `/Type /Font` object with no reachable `/FontFile`/`/FontFile2`/`/FontFile3`.
//! - SOFT: an `/S /JavaScript` action, bare `/JS` key, or `/S /Launch` action anywhere.
//! - SOFT: a `/Subtype /Movie` or `/Subtype /Sound` annotation dict.
...
fn has_pdfx_output_intent(objects: &[PdfIndirectObject]) -> bool {
    ...
    intents.iter().any(|item| {
        resolve_item(objects, item)
            ...
            .map(|d| dict_name(d, "S") == Some("GTS_PDFX") && d.iter().any(|e| e.key == "DestOutputProfile"))
            .unwrap_or(false)
    })
}
...
mod tests {
    ...
    #[test]
    fn missing_output_intent_is_hard() {
    ...
    #[test]
    fn page_missing_trim_or_art_box_is_hard() {
        let mut objects = conforming_objects();
        objects.push(PdfIndirectObject {
            id: ObjRef { num: 4, gen: 0 },
            value: PdfObject::Dict(vec![PdfDictEntry { key: "Type".into(), value: PdfObject::Name("Page".into()) }]),
        });
    ...
    #[test]
    fn javascript_action_is_soft() {
        let mut objects = conforming_objects();
        objects.push(PdfIndirectObject {
            id: ObjRef { num: 7, gen: 0 },
    ...
    #[test]
    fn movie_annotation_is_soft() {
        let mut objects = conforming_objects();
        objects.push(PdfIndirectObject {
            id: ObjRef { num: 8, gen: 0 },
