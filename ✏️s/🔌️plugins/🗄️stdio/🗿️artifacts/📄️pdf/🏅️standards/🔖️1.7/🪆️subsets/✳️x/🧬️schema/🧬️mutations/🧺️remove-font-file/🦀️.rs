//! 🧺️ Authoritative PDF/X mutation for detaching a font program from a font descriptor.
...
impl MutationKind<PdfSnapshot, PdfXMutation> for RemoveFontFile {
    ...
    fn inverse(&self, base: &PdfSnapshot) -> Vec<PdfXMutation> {
        support::font_descriptors(base)
            ...
            .map(|(key, program)| PdfXMutation::EmbedFontFile(EmbedFontFile { descriptor_ordinal: self.descriptor_ordinal, key, program }))
            .into_iter()
            .collect()
    }
...
mod tests {
    ...
    fn detaches_and_can_restore_the_font_program() {
        ...
        let outcome = <RemoveFontFile as MutationKind<PdfSnapshot, PdfXMutation>>::diff(&mutation, &base);
        let next = outcome.diff().apply(&base).unwrap();
        let descriptor = support::font_descriptors(&next)[0];
        assert!(support::font_program(&next, descriptor).is_none());
        assert_eq!(<RemoveFontFile as MutationKind<PdfSnapshot, PdfXMutation>>::inverse(&mutation, &base).len(), 1);
    }
}
//#endregion 🧪️Tests
