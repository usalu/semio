//! 🦀️ PDF 1.7 ✳️ua exhaustive conformance-class mutation case — Rust adapter.
//!
...
//! `../../🏅️standards/🔖️1.7/🪆️subsets/✳️ua/🦀️oracle.rs`, `subject` handlers drive this
//! repository's own decode/mutate/encode round trip, and both results are read back by the SAME
//! independent `project_conformance` before the `semantic-pdf-conformance-ua-v1` profile compares
...
//!
//! @see ../../🏅️standards/🔖️1.7/🪆️subsets/✳️ua/🧬️schema/🦀️component.rs — `check_ua_conformance`, the one
//!      axis list this whole case derives from.

use semio_repo_test_host::{Adapter, Context, Json, Outcome};
use semio_s_plugin_stdio_test_oracle::artifacts::pdf::standards::v1_7::subsets::ua::{oracle_apply_mutation, oracle_arrange, oracle_inverse_spec, oracle_round_trip, project_conformance};

//#region 🔖️Kinds
/// 🧾️ Test-case-local mirror of the `pdf-1-7-ua` catalog. Duplicated, not imported, from
/// `../../🏅️standards/🔖️1.7/🪆️subsets/✳️ua/🧬️schema/🧬️mutations/🦀️.rs::KINDS` — that module
...
mod subject {
    ...
    /// `PdfUaMutation` — the only channel between the feature's parameters and the subject's codec.
    ...
    pub fn mutate(ctx: &Context) -> Result<Outcome, String> {
        ...
        apply_ua_conformance_mutation(&mut snapshot, &mutation);
    ...
    pub fn inverse(ctx: &Context) -> Result<Outcome, String> {
        ...
        apply_ua_conformance_mutation(&mut snapshot, &forward);
        let undo = oracle_inverse_spec(&base, &spec)?;
        let backward = mutation_from_spec(&snapshot, &undo)?;
        apply_ua_conformance_mutation(&mut snapshot, &backward);
        let output = encode(&snapshot)?;
        let projection = project_conformance(&output)?;
        Ok(Outcome::with_raw(output, projection))
    }
