...
impl Mutation<PdfSnapshot> for PdfMutation {
    ...
    async fn diff(&self, base: &PdfSnapshot) -> protocol::MutationOutcome<Self::Diff> {
        protocol::MutationOutcome::new(match self {
            ...
            PdfMutation::SetSnapshot { snapshot } => diff::diff_set_snapshot(base, snapshot).await,
            PdfMutation::InsertPage { index, page } => diff::diff_insert_page(*index, page.clone()).await,
            PdfMutation::RemovePage { index } => diff::diff_remove_page(*index).await,
            PdfMutation::SetPageMediaBox { index, media_box } => diff::diff_set_page_media_box(*index, *media_box).await,
            PdfMutation::SetPageCropBox { index, crop_box } => diff::diff_set_page_crop_box(*index, *crop_box).await,
            PdfMutation::AppendPageContent { index, text } => diff::diff_append_page_content(base, *index, text).await,
    ...
    async fn inverse(&self, base: &PdfSnapshot) -> Vec<Self> {
        match self {
            ...
            PdfMutation::SetDictEntry { id, path, key, .. } => match original_dict_value(base, *id, path, key).await {
            ...
            PdfMutation::RemoveDictEntry { id, path, key } => match original_dict_value(base, *id, path, key).await {
                Some(orig) => vec![PdfMutation::SetDictEntry { id: *id, path: path.clone(), key: key.clone(), value: orig }],
                None => vec![PdfMutation::NoMutation],
            },
...
async fn dec_path_segment(s: &str) -> Result<PdfPathSegment, String> {
    ...
    match tag {
        ...
        "K" => Ok(PdfPathSegment::DictKey { key: dec_str(inner).await? }),
...
async fn enc_pdf_snapshot(s: &PdfSnapshot) -> String {
    ...
    hex_encode(&bytes).await
}
async fn dec_pdf_snapshot(s: &str) -> Result<PdfSnapshot, String> {
    let bytes = hex_decode(s).await?;
    let mut reader = store::ByteReader::new(&bytes).await;
    let snapshot = dec_pdf_snapshot_bin(&mut reader).await?;
...
async fn parse_pdf_mutation(line: &str) -> Result<PdfMutation, String> {
    ...
    match keyword {
        "set-snapshot" => Ok(PdfMutation::SetSnapshot { snapshot: dec_pdf_snapshot(arg("snapshot")?).await? }),
        "insert-page" => Ok(PdfMutation::InsertPage { index: usize_arg("index")?, page: dec_pdf_page(arg("page")?).await? }),
        "remove-page" => Ok(PdfMutation::RemovePage { index: usize_arg("index")? }),
        "set-page-media-box" => Ok(PdfMutation::SetPageMediaBox { index: usize_arg("index")?, media_box: dec_box(arg("media-box")?).await? }),
        "set-page-crop-box" => Ok(PdfMutation::SetPageCropBox { index: usize_arg("index")?, crop_box: decode_option(arg("crop-box")?, dec_box).await? }),
        "append-page-content" => Ok(PdfMutation::AppendPageContent { index: usize_arg("index")?, text: dec_str(arg("text")?).await? }),
        "set-info" => Ok(PdfMutation::SetInfo { info: dec_pdf_info(arg("info")?).await? }),
        "insert-object" => Ok(PdfMutation::InsertObject { id: dec_objref(arg("id")?).await?, value: dec_pdf_object(arg("value")?).await? }),
        "remove-object" => Ok(PdfMutation::RemoveObject { id: dec_objref(arg("id")?).await? }),
        "set-object-value" => Ok(PdfMutation::SetObjectValue { id: dec_objref(arg("id")?).await?, value: dec_pdf_object(arg("value")?).await? }),
        "set-dict-entry" => Ok(PdfMutation::SetDictEntry { id: dec_objref(arg("id")?).await?, path: dec_path(arg("path")?).await?, key: dec_str(arg("key")?).await?, value: dec_pdf_object(arg("value")?).await? }),
        "remove-dict-entry" => Ok(PdfMutation::RemoveDictEntry { id: dec_objref(arg("id")?).await?, path: dec_path(arg("path")?).await?, key: dec_str(arg("key")?).await? }),
        "set-trailer-entry" => Ok(PdfMutation::SetTrailerEntry { key: dec_str(arg("key")?).await?, value: dec_pdf_object(arg("value")?).await? }),
        "remove-trailer-entry" => Ok(PdfMutation::RemoveTrailerEntry { key: dec_str(arg("key")?).await? }),
...
impl OpText for PdfMutation {
    async fn print_op(&self) -> String {
        print_pdf_mutation(self).await
    }
    async fn parse_op(line: &str) -> Result<Self, store::TextError> {
        parse_pdf_mutation(line).await.map_err(|e| store::TextError::new(e, dsl::TextSpan::at(1, 1)))
    }
}
...
impl OpBinary for PdfMutation {
    async fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        ...
        match self {
            ...
            PdfMutation::SetSnapshot { snapshot } => enc_pdf_snapshot_bin(snapshot, &mut out).await,
            ...
            PdfMutation::SetInfo { info } => enc_pdf_info_bin(info, &mut out).await,
            PdfMutation::InsertObject { id, value } => {
                enc_objref_bin(id, &mut out);
                enc_pdf_object_bin(value, &mut out);
            }
            PdfMutation::RemoveObject { id } => enc_objref_bin(id, &mut out).await,
            ...
            PdfMutation::RemoveTrailerEntry { key } => write_str_lp(&mut out, key).await,
        }
        Ok(out)
    }
    ...
    async fn decode_op(bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        ...
        let mutation = match tag {
            ...
            1 => Ok(PdfMutation::SetSnapshot { snapshot: dec_pdf_snapshot_bin(&mut reader).await.map_err(|e| malformed("op snapshot", semio_framework_plugin::resolve_ready(reader.position()), e))? }),
            2 => {
                ...
                let page = dec_pdf_page_bin(&mut reader).await.map_err(|e| malformed("op page", semio_framework_plugin::resolve_ready(reader.position()), e))?;
            ...
            4 => {
                let index = reader.read_varint_u64().await.map_err(|e| malformed("op index", semio_framework_plugin::resolve_ready(reader.position()), e.to_string()))? as usize;
                let media_box = dec_box_bin(&mut reader).await.map_err(|e| malformed("op media_box", semio_framework_plugin::resolve_ready(reader.position()), e))?;
            ...
            5 => {
                ...
                let crop_box = if has != 0 { Some(dec_box_bin(&mut reader).await.map_err(|e| malformed("op crop_box", semio_framework_plugin::resolve_ready(reader.position()), e))?) } else { None };
            ...
            7 => Ok(PdfMutation::SetInfo { info: dec_pdf_info_bin(&mut reader).await.map_err(|e| malformed("op info", semio_framework_plugin::resolve_ready(reader.position()), e))? }),
            8 => {
                let id = dec_objref_bin(&mut reader).await.map_err(|e| malformed("op id", semio_framework_plugin::resolve_ready(reader.position()), e))?;
                let value = dec_pdf_object_bin(&mut reader).await.map_err(|e| malformed("op value", semio_framework_plugin::resolve_ready(reader.position()), e))?;
                Ok(PdfMutation::InsertObject { id, value })
            }
            9 => Ok(PdfMutation::RemoveObject { id: dec_objref_bin(&mut reader).await.map_err(|e| malformed("op id", semio_framework_plugin::resolve_ready(reader.position()), e))? }),
            10 => {
                let id = dec_objref_bin(&mut reader).await.map_err(|e| malformed("op id", semio_framework_plugin::resolve_ready(reader.position()), e))?;
                let value = dec_pdf_object_bin(&mut reader).await.map_err(|e| malformed("op value", semio_framework_plugin::resolve_ready(reader.position()), e))?;
            ...
            11 => {
                let id = dec_objref_bin(&mut reader).await.map_err(|e| malformed("op id", semio_framework_plugin::resolve_ready(reader.position()), e))?;
                let path = dec_path_bin(&mut reader).await.map_err(|e| malformed("op path", semio_framework_plugin::resolve_ready(reader.position()), e))?;
            ...
            14 => Ok(PdfMutation::RemoveTrailerEntry { key: read_str_lp(&mut reader).await.map_err(|e| malformed("op key", semio_framework_plugin::resolve_ready(reader.position()), e))? }),
