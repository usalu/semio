//! 🛬️ Authoritative PDF/E mutation for removing a matching launch action.
...
impl MutationKind<PdfSnapshot, PdfEMutation> for RemoveLaunchAction {
    ...
    fn inverse(&self, base: &PdfSnapshot) -> Vec<PdfEMutation> {
        support::action_with(base, "Launch", "F", &self.target)
            .map(|_| PdfEMutation::InsertLaunchAction(InsertLaunchAction { target: self.target.clone() }))
            .into_iter()
            .collect()
    }
...
mod tests {
    ...
    fn removes_the_matching_launch_target() {
        ...
        let outcome = <RemoveLaunchAction as MutationKind<PdfSnapshot, PdfEMutation>>::diff(&mutation, &base);
