//! 📄️ PDF/VT Document (1.7) editor -- one of stdio's 10 real PDF subset editors (ticket
...
//! document schema id (confirmed by direct read before writing this file). `Pdf17VtEditor`
//! implements `ArtifactEditor` over that shared type, tagged with this file's own `PDF17VT_DIALECT`
//! (`standard: "1.7"`, `subset: "vt"`) -- subsets are constraint PROFILES of the same
...
use crate::editor::pdf17vt::modes::edit;
use crate::editor::pdf17vt::modes::edit::windows::main;
...
/// coordinate -- `s.stdio.pdf@1.7/vt` -- measured directly against `PDF_ARTIFACT_SCHEMA_ID`
...
pub const PDF17VT_DIALECT: Dialect = Dialect { artifact_kind: PDF_ARTIFACT_SCHEMA_ID, standard: StandardId("1.7"), subset: SubsetId("vt") };
//#endregion 🔖️Dialect
...
impl protocol::OpText for Pdf17VtEditorCommand {
...
impl protocol::OpBinary for Pdf17VtEditorCommand {
...
pub struct Pdf17VtEditor;

impl ArtifactEditor for Pdf17VtEditor {
    ...
    type Command = Pdf17VtEditorCommand;

    const DIALECT: Dialect = PDF17VT_DIALECT;
    ...
    async fn handle(
        ...
        match command {
            Pdf17VtEditorCommand::SetPage { index, text } => {
...
pub fn create_pdf17_vt_editor() -> semio_framework_plugin::AppDefinition {
    let builder = Editor::builder(PDF17VT_DIALECT);
    let builder = semio_framework_plugin::resolve_ready(builder.document(["stdio", "pdf", "1.7", "vt"]));
    ...
    let builder = semio_framework_plugin::resolve_ready(builder.default_mode_id(edit::PDF17VT_EDIT_MODE_ID));
