...
//
// The bytes this file produces are written entirely by the real `lopdf` 0.44 object-graph library
// (`🦀️engine/src/main.rs`) — the SAME crate registered as `lopdf-pdf-1-7-mutate` in
// `../🧪️oracle/🔣️.json` — never by this repository's own `encode_pdf`. This script only marshals: it
// builds and invokes the Rust binary and reports what it wrote; it computes no PDF bytes itself.
//
...
const RECIPE = "report-strip";
const FIXTURE_FILE = "report-strip.pdf";
//#endregion 🧬️Contract
...
async function manifests(): Promise<void> {
  ...
  const entry = {
    ...
    family: "mechanical",
    files: [{ path: `../🧫️fixtures/${RECIPE}/${FIXTURE_FILE}`, mediaType: "application/pdf", sha256: digest }],
