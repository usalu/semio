...
//! (`../🧬️schema/🦀️component.rs`), which reads five axes: a populated `Info.title` AND `Info.author`, any `/S /JavaScript` action or bare `/JS` key, any `/S /Launch` action, an `/AcroForm` field with `/FT /Sig`, and an embedded program on every font's `/FontDescriptor`.
//!
...
//! @see ../🧬️schema/🦀️component.rs — `check_h_conformance`, the one axis list everything here derives from.
...
mod tests {
    ...
    fn params_for(kind: &str) -> Json {
        match kind {
            ...
            "insert-signature-field" => json_object(vec![("name", Json::String("AttendingPhysician".to_string()))]),
            "remove-signature-field" => json_object(vec![("name", Json::String("AttendingPhysician".to_string()))]),
    ...
    fn a_kind_this_subset_does_not_declare_is_refused_even_when_the_engine_could_perform_it() {
        let undeclared = ["insert-embedded-file", "insert-encryption-dictionary", "insert-media-annotation", "remove-af-relationship", "remove-display-doc-title", "remove-dpart-metadata", "remove-dpart-root", "remove-embedded-file", "remove-encryption-dictionary", "remove-lang", "remove-mark-info", "remove-media-annotation", "remove-output-intent", "remove-struct-tree-root", "remove-trim-box", "set-af-relationship", "set-display-doc-title", "set-dpart-metadata", "set-dpart-root", "set-lang", "set-mark-info", "set-output-intent", "set-struct-tree-root", "set-trim-box"];
