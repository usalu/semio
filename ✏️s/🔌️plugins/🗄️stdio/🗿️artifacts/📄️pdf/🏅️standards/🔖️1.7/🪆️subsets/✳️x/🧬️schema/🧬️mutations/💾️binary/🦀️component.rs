//! 💾️ Generic binary framing and direct-owner registry for the visible PDF/X mutation aggregate.
...
fn malformed(offset: u64, detail: impl Into<String>) -> protocol::ProtocolError {
    protocol::ProtocolError::Malformed { what: "PDF/X mutation aggregate", offset, detail: detail.into() }
}
...
impl OpBinary for PdfXMutation {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        let payload = serde_json::to_vec(self).map_err(|error| malformed(0, error.to_string()))?;
        if payload.len() > MAX_PAYLOAD_BYTES { return Err(protocol::ProtocolError::LimitExceeded("PDF/X mutation payload")); }
    ...
    fn decode_op(bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        ...
        if payload_len > MAX_PAYLOAD_BYTES { return Err(protocol::ProtocolError::LimitExceeded("PDF/X mutation payload")); }
