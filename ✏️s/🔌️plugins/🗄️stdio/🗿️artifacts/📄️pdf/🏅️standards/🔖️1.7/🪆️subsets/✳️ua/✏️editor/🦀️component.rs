//! 📄️ PDF/UA Document (1.7) editor -- one of stdio's 10 real PDF subset editors (ticket
...
//! document schema id (confirmed by direct read before writing this file). `Pdf17UaEditor`
//! implements `ArtifactEditor` over that shared type, tagged with this file's own `PDF17UA_DIALECT`
//! (`standard: "1.7"`, `subset: "ua"`) -- subsets are constraint PROFILES of the same
...
pub struct Pdf17UaEditor;

impl ArtifactEditor for Pdf17UaEditor {
...
pub fn create_pdf17_ua_editor() -> semio_framework_plugin::AppDefinition {
    let builder = Editor::builder(PDF17UA_DIALECT);
    ...
    let builder = semio_framework_plugin::resolve_ready(builder.default_mode_id(edit::PDF17UA_EDIT_MODE_ID));
...
mod tests {
    ...
    async fn create_pdf17_ua_editor_builds_a_definition_for_the_editor_role() {
        let def = create_pdf17_ua_editor();
    ...
    async fn editor_dialect_matches_the_artifact_coordinate() {
        assert_eq!(<Pdf17UaEditor as ArtifactEditor>::DIALECT, PDF17UA_DIALECT);
    }
