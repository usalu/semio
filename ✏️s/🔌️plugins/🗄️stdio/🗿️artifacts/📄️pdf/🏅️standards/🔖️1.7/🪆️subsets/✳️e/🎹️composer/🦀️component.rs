//! 🎹️ PdfEComposer (1.7/✳️e) — reads the same sources ✳️any does, delegates parsing to it, then
//! HARD-GATES the `e` dialect stamp on real ISO 24517-1:2008 (PDF/E-1) conformance
//! (`check_e_conformance`). A hard violation (`/Encrypt`, JS/Launch action, Movie/Sound
//! annotation) fails composition; a soft one (no OutputIntent, non-embedded font) passes through
//! as an advisory diagnostic. Also registers this dialect's `SubsetValidator`. Ticket
...
//#region 🔖️Composer
pub struct PdfEComposer;

impl ArtifactComposer for PdfEComposer {
...
mod tests {
    ...
    use crate::artifacts::pdf::standards::v1_7::subsets::e::builder::PdfEBuilder;
    ...
    fn conforming_builder_snapshot_composes_and_stamps_e() {
        ...
        let composed = PdfEComposer::compose(&sources).expect("clean document must compose to e");
