/** 🪟️ PDF/E Document (1.7) editor -- `main` window: typed twin of `component.rs`'s `DocumentWindowKit`
...
export interface Pdf17EDocumentPage {
  text: string;
}

/** ✏️ The `main` window's typed view-model -- the TS mirror of the Rust `render()` boundary's
 * input (a bare `PdfSnapshot`). */
export interface Pdf17EDocumentViewModel {
  windowKindId: "framework.window.document";
  bodyKey: "framework.window.document";
  pages: Pdf17EDocumentPage[];
}

/** ✏️ `set-page` payload shape -- mirrors `Pdf17EEditorCommand::SetPage`. APPENDS to the page's
...
export interface Pdf17ESetPage {
  index: number;
  text: string;
}

export const PDF17E_MAIN_WINDOW_KIND_ID = "framework.window.document" as const;
export const PDF17E_MAIN_BODY_KEY = "framework.window.document" as const;
