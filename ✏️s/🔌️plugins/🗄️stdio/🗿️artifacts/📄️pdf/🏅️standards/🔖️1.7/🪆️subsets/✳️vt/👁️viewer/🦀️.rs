//! 👁️ PDF/VT Document (1.7) viewer -- the read-only counterpart of the mutation-capable surface for this
//! subset (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.2). `Pdf17VtViewer`
//! implements `ArtifactViewer`, never `ArtifactEditor`/`ArtifactApp` -- `ViewerApp<Pdf17VtViewer>`
...
use crate::viewer::pdf17vt::modes::view;
...
pub enum Pdf17VtViewCommand {
    #[default]
    Noop,
}

impl protocol::OpBinary for Pdf17VtViewCommand {
    fn encode_op(&self) -> Result<Vec<u8>, protocol::ProtocolError> {
        Ok(Vec::new())
    }
    fn decode_op(_bytes: &[u8]) -> Result<Self, protocol::ProtocolError> {
        Ok(Pdf17VtViewCommand::Noop)
    }
}
//#endregion 🔖️Command
...
pub struct Pdf17VtViewer;

impl ArtifactViewer for Pdf17VtViewer {
    ...
    type Command = Pdf17VtViewCommand;
...
pub fn create_pdf17_vt_viewer() -> semio_framework_plugin::AppDefinition {
    let builder = Viewer::builder(PDF17VT_DIALECT);
    let builder = builder.document(["stdio", "pdf", "1.7", "vt"]);
    let builder = builder.icon_id("file-text");
    let builder = builder.mode_def(view::definition());
    let builder = builder.default_mode_id(view::PDF17VT_VIEW_MODE_ID);
...
mod tests {
    ...
    async fn create_pdf17_vt_viewer_builds_a_definition_for_the_viewer_role() {
        let def = create_pdf17_vt_viewer();
    ...
    async fn viewer_dialect_matches_the_artifact_coordinate() {
        assert_eq!(<Pdf17VtViewer as ArtifactViewer>::DIALECT, PDF17VT_DIALECT);
    }
