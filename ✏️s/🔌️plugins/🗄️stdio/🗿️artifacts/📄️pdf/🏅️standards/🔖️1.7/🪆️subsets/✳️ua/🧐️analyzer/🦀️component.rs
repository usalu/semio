//! 🧐️ PdfUaAnalyzer (1.7/✳️ua) — real ISO 14289-1:2014 (PDF/UA-1) conformance checks against the
//! retained `PdfSnapshot.objects` graph. Ticket 26/08/11/ARTIFACT-STANDARD-SUBSETS-REAL-VOCABULARIES W3.
//!
//! Checks implemented as real, honest scans against the Catalog dict and `snapshot.info`:
//! - HARD: `/Root`'s `/MarkInfo` dict is absent, or present without `/Marked true` -- ISO 14289-1
//!   §7.1 requires the document be marked as tagged.
//! - HARD: `/Root` carries no `/StructTreeRoot` key at all -- the structure tree is PDF/UA's
//!   entire mechanism; its total absence is unrecoverable.
//! - SOFT: `/Root` carries no non-empty `/Lang` key.
//! - SOFT: `/Root`'s `/ViewerPreferences` is absent, or present without `/DisplayDocTitle true`.
//! - SOFT: `snapshot.info.title` is absent or empty.
...
//#region 🔖️Analyzer
pub struct PdfUaAnalyzer;

impl ArtifactAnalyzer for PdfUaAnalyzer {
    type Parts = PdfParts;
