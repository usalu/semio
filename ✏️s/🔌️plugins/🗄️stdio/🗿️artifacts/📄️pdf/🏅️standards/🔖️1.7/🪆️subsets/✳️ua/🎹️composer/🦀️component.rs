//! 🎹️ PdfUaComposer (1.7/✳️ua) — reads the same sources ✳️any does, delegates parsing to it,
//! then HARD-GATES the `ua` dialect stamp on real ISO 14289-1:2014 (PDF/UA-1) conformance
//! (`check_ua_conformance`). A hard violation (`/MarkInfo`/`Marked` missing, `/StructTreeRoot`
//! absent) fails composition; a soft one (missing `/Lang`, `/DisplayDocTitle`, Info.title,
//! non-embedded font) passes through as an advisory diagnostic. Also registers this dialect's
//! `SubsetValidator`. Ticket 26/08/11/ARTIFACT-STANDARD-SUBSETS-REAL-VOCABULARIES W3.
...
//#region 🔖️Composer
pub struct PdfUaComposer;

impl ArtifactComposer for PdfUaComposer {
    ...
    fn compose(sources: &[ComposeSource]) -> Result<Composition<Self::Snapshot>, ComposeError> {
        ...
        if !hard.is_empty() {
            ...
            return Err(ComposeError {
                message: format!("PDF/UA-1 conformance violated: {} hard issue(s) -- not stamping the ua dialect", hard.len()),
                diagnostics: all,
            });
        }
...
mod tests {
    ...
    fn conforming_builder_snapshot_composes_and_stamps_ua() {
        ...
        let composed = PdfUaComposer::compose(&sources).expect("clean document must compose to ua");
    ...
    fn missing_markinfo_fails_compose() {
        ...
        let err = PdfUaComposer::compose(&sources).expect_err("an untagged document must not stamp ua");
        assert!(err.diagnostics.iter().any(|d| d.code.0 == crate::artifacts::pdf::standards::v1_7::subsets::ua::analyzer::CODE_MARKINFO), "got {:?}", err.diagnostics);
    }
}
