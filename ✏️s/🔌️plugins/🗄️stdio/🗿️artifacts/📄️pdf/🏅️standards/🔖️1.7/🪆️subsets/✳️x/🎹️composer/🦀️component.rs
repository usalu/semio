//! 🎹️ PdfXComposer (1.7/✳️x) — reads the same sources the ✳️any subset does (native
//! `stdio.pdf` 1.7, plus its `binary`/`deflate` DAG deps), delegates the actual parse to the
//! ✳️any composer, then HARD-GATES the `x` dialect stamp on real ISO 15930-7:2010 (PDF/X-4)
//! conformance. A hard violation (`/Encrypt`, missing `/GTS_PDFX` OutputIntent, a page missing
//! both `/TrimBox` and `/ArtBox`) fails composition outright; a soft one (non-embedded font,
//! JavaScript/Launch action, Movie/Sound annotation) passes through as an advisory diagnostic.
//!
//! Also registers this dialect's `SubsetValidator` (D5's generic validate-on-build hook) — the
//! SAME `check_x_conformance` function backs both the pre-serialization hard gate here and the
//! post-hoc wire-payload recheck. Ticket 26/08/11/ARTIFACT-STANDARD-SUBSETS-REAL-VOCABULARIES W3.
...
mod tests {
    ...
    use crate::artifacts::pdf::standards::v1_7::subsets::x::builder::PdfXBuilder;
    ...
    fn conforming_builder_snapshot_composes_and_stamps_x() {
        ...
        let composed = PdfXComposer::compose(&sources).expect("clean document must compose to x");
    ...
    fn missing_output_intent_fails_compose() {
        ...
        let err = PdfXComposer::compose(&sources).expect_err("a document with no OutputIntent must not stamp x");
        assert!(err.diagnostics.iter().any(|d| d.code.0 == crate::artifacts::pdf::standards::v1_7::subsets::x::analyzer::CODE_OUTPUT_INTENT), "got {:?}", err.diagnostics);
    }
    ...
    fn subset_validator_recheck_runs_the_same_check() {
        ...
        assert!(diagnostics.iter().any(|d| d.code.0 == crate::artifacts::pdf::standards::v1_7::subsets::x::analyzer::CODE_OUTPUT_INTENT), "got {diagnostics:?}");
    }
}
