//! 🚀️ Authoritative PDF/A mutation for inserting a launch action.
...
impl MutationKind<PdfSnapshot, PdfAMutation> for InsertLaunchAction {
    ...
    fn inverse(&self, _base: &PdfSnapshot) -> Vec<PdfAMutation> {
        vec![PdfAMutation::RemoveLaunchAction(RemoveLaunchAction { target: self.target.clone() })]
    }
