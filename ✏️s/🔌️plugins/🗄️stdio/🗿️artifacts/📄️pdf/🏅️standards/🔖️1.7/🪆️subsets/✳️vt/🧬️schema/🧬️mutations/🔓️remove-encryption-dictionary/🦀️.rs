//! 🔓️ Authoritative PDF/VT mutation for removing a matching encryption dictionary.
...
impl MutationKind<PdfSnapshot, PdfVtMutation> for RemoveEncryptionDictionary {
    ...
    fn inverse(&self, base: &PdfSnapshot) -> Vec<PdfVtMutation> {
        support::encryption_dictionary_with(base, self.version, self.revision)
            .map(|_| PdfVtMutation::InsertEncryptionDictionary(InsertEncryptionDictionary { version: self.version, revision: self.revision }))
            .into_iter()
            .collect()
    }
...
mod tests {
    ...
    fn removes_only_a_present_security_handler() {
        ...
        let outcome = <RemoveEncryptionDictionary as MutationKind<PdfSnapshot, PdfVtMutation>>::diff(&mutation, &base);
        let next = outcome.diff().apply(&base).unwrap();
        assert!(support::encryption_dictionary_with(&next, 2, 3).is_none());
        assert_eq!(<RemoveEncryptionDictionary as MutationKind<PdfSnapshot, PdfVtMutation>>::inverse(&mutation, &base).len(), 1);
    }
}
//#endregion 🧪️Tests
