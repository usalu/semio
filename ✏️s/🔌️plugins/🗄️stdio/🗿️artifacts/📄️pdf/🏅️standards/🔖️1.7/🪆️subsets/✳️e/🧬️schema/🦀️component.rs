...
pub mod derived_construction {
    ...
    mod tests {
        ...
        async fn empty_builder_builds_clean() {
            let snapshot = PdfEBuilderConstruction::new().await.add_page(PdfPage::new(200.0, 200.0)).await.set_info(PdfInfo { title: Some("An E Test".into()), ..PdfInfo::default() }).await.build().await.expect("no hard violations by default");
        ...
        async fn hard_violation_injected_via_raw_mutate_still_fails_build() {
            ...
            let mut snapshot = PdfEBuilderConstruction::new().await.add_page(PdfPage::new(100.0, 100.0)).await.build().await.unwrap();
