//! 📄️ PDF/A Document (1.7) editor -- one of stdio's 10 real PDF subset editors (ticket
...
//! document schema id (confirmed by direct read before writing this file). `Pdf17AEditor`
//! implements `ArtifactEditor` over that shared type, tagged with this file's own `PDF17A_DIALECT`
//! (`standard: "1.7"`, `subset: "a"`) -- subsets are constraint PROFILES of the same
...
use crate::editor::pdf17a::modes::edit;
use crate::editor::pdf17a::modes::edit::windows::main;
...
impl protocol::OpText for Pdf17AEditorCommand {
...
pub struct Pdf17AEditor;

impl ArtifactEditor for Pdf17AEditor {
    ...
    type Command = Pdf17AEditorCommand;
    ...
    fn handle(
        ...
        match command {
            Pdf17AEditorCommand::SetPage { index, text } => {
...
pub fn create_pdf17_a_editor() -> semio_framework_plugin::AppDefinition {
    let builder = Editor::builder(PDF17A_DIALECT);
    let builder = builder.document(["stdio", "pdf", "1.7", "a"]);
    let builder = builder.icon_id("file-text");
    let builder = builder.mode_def(edit::definition());
    let builder = builder.default_mode_id(edit::PDF17A_EDIT_MODE_ID);
