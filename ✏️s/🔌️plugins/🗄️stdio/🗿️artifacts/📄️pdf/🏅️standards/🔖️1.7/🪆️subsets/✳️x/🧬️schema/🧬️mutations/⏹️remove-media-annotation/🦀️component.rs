//! ⏹️ Authoritative PDF/X mutation for removing a matching Movie or Sound annotation.
...
impl MutationKind<PdfSnapshot, PdfXMutation> for RemoveMediaAnnotation {
...
mod tests {
    ...
    fn removes_only_the_matching_media_annotation() {
        ...
        let outcome = <RemoveMediaAnnotation as MutationKind<PdfSnapshot, PdfXMutation>>::diff(&mutation, &base);
        let next = outcome.diff().apply(&base).unwrap();
        assert!(support::media_annotation(&next, &mutation.subtype, &mutation.title).is_none());
        assert_eq!(<RemoveMediaAnnotation as MutationKind<PdfSnapshot, PdfXMutation>>::inverse(&mutation, &base).len(), 1);
    }
}
//#endregion 🧪️Tests
