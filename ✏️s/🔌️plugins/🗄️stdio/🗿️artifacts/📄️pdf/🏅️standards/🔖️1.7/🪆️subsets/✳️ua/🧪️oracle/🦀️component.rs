...
//! (`../🧬️schema/🦀️component.rs`), which reads six axes, all of them keys of the document CATALOG or of `/Info`: `/Root/MarkInfo` with `/Marked true`, `/Root/StructTreeRoot`, a non-empty `/Root/Lang`, `/Root/ViewerPreferences` with `/DisplayDocTitle true`, a non-empty `Info.title`, and an embedded program on every font's `/FontDescriptor`.
//!
...
//! @see ../🧬️schema/🦀️component.rs — `check_ua_conformance`, the one axis list everything here derives from.
...
mod tests {
    ...
    fn params_for(kind: &str) -> Json {
        match kind {
            ...
            "set-mark-info" => json_object(vec![("marked", Json::Bool(true))]),
            "remove-mark-info" => json_object(vec![]),
            "set-struct-tree-root" => json_object(vec![]),
    ...
    fn a_kind_this_subset_does_not_declare_is_refused_even_when_the_engine_could_perform_it() {
        let undeclared = ["insert-embedded-file", "insert-encryption-dictionary", "insert-javascript-action", "insert-launch-action", "insert-media-annotation", "insert-signature-field", "remove-af-relationship", "remove-dpart-metadata", "remove-dpart-root", "remove-embedded-file", "remove-encryption-dictionary", "remove-javascript-action", "remove-launch-action", "remove-media-annotation", "remove-output-intent", "remove-signature-field", "remove-trim-box", "set-af-relationship", "set-dpart-metadata", "set-dpart-root", "set-info-author", "set-output-intent", "set-trim-box"];
