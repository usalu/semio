//! 🧬️ `PdfXMutation` — the ISO 15930-7 (PDF/X-4) CONFORMANCE-CLASS vocabulary of `stdio.pdf` 1.7. Every
...
//! own `check_x_conformance` (`../../🦀️component.rs`), which reads seven axes: any Standard Security Handler `/Encrypt` dictionary object, an `/S /GTS_PDFX` OutputIntent that also carries `/DestOutputProfile`, a `/TrimBox` or `/ArtBox` on EVERY `/Type /Page`, an embedded program on every font's `/FontDescriptor`, any `/S /JavaScript` action or bare `/JS` key, any `/S /Launch` action, and any `/Subtype /Movie` or `/Subtype /Sound` annotation.
//!
...
/// carry a `/DestOutputProfile`. Both are read straight off `check_x_conformance`.
...
pub const CONFORMANT_TITLE: &str = "A PDF/X-4 conformant document";
pub const CONFORMANT_AUTHOR: &str = "semio stdio conformance stamp";
//#endregion 🔖️Class

//#region 🔖️Mutations
/// 📐️ Typed conformance-class mutation for `stdio.pdf` 1.7 under ISO 15930-7 (PDF/X-4). Every variant
...
/// 🧾️ Kebab-case spelling of every `PdfXMutation` variant, in declaration order — the exhaustive
/// mutation catalog `pdf-1-7-x` (`../../🧪️oracle/🔣️component.json`) is measured against this
...
impl Mutation<PdfSnapshot> for PdfXMutation {
    type Diff = PdfDiff;
...
mod tests {
    ...
    /// variant is added to `PdfXMutation` without a matching kebab-case spelling here, which is what
    ...
    fn kinds_match_enum_and_catalog() {
        fn kind_of(mutation: &PdfXMutation) -> &'static str {
            match mutation {
                PdfXMutation::NoMutation => "no-mutation",
                PdfXMutation::SetSnapshot { .. } => "set-snapshot",
                PdfXMutation::InsertEncryptionDictionary { .. } => "insert-encryption-dictionary",
                PdfXMutation::RemoveEncryptionDictionary { .. } => "remove-encryption-dictionary",
                PdfXMutation::SetOutputIntent { .. } => "set-output-intent",
                PdfXMutation::RemoveOutputIntent => "remove-output-intent",
                PdfXMutation::SetTrimBox { .. } => "set-trim-box",
                PdfXMutation::RemoveTrimBox { .. } => "remove-trim-box",
                PdfXMutation::EmbedFontFile { .. } => "embed-font-file",
                PdfXMutation::RemoveFontFile { .. } => "remove-font-file",
                PdfXMutation::InsertJavaScriptAction { .. } => "insert-javascript-action",
                PdfXMutation::RemoveJavaScriptAction { .. } => "remove-javascript-action",
                PdfXMutation::InsertLaunchAction { .. } => "insert-launch-action",
                PdfXMutation::RemoveLaunchAction { .. } => "remove-launch-action",
                PdfXMutation::InsertMediaAnnotation { .. } => "insert-media-annotation",
                PdfXMutation::RemoveMediaAnnotation { .. } => "remove-media-annotation",
            }
        }
        let samples = [
            PdfXMutation::NoMutation,
            PdfXMutation::SetSnapshot { snapshot: PdfSnapshot::default() },
            PdfXMutation::InsertEncryptionDictionary { version: 2, revision: 3 },
            PdfXMutation::RemoveEncryptionDictionary { version: 2, revision: 3 },
            PdfXMutation::SetOutputIntent { identifier: String::new() },
            PdfXMutation::RemoveOutputIntent,
            PdfXMutation::SetTrimBox { page_index: 0, trim_box: [0.0; 4] },
            PdfXMutation::RemoveTrimBox { page_index: 0 },
            PdfXMutation::EmbedFontFile { descriptor_ordinal: 0, key: String::new(), program: ObjRef::default() },
            PdfXMutation::RemoveFontFile { descriptor_ordinal: 0 },
            PdfXMutation::InsertJavaScriptAction { script: String::new() },
            PdfXMutation::RemoveJavaScriptAction { script: String::new() },
            PdfXMutation::InsertLaunchAction { target: String::new() },
            PdfXMutation::RemoveLaunchAction { target: String::new() },
            PdfXMutation::InsertMediaAnnotation { subtype: String::new(), title: String::new() },
            PdfXMutation::RemoveMediaAnnotation { subtype: String::new(), title: String::new() },
        ];
        let from_enum: Vec<&'static str> = samples.iter().map(kind_of).collect();
        assert_eq!(from_enum, KINDS, "KINDS must list every PdfXMutation variant, in declaration order");
        ...
        assert_eq!(declared, KINDS, "the oracle manifest's kinds must match PdfXMutation exactly");
    }
    ...
    fn exhaustive(base: &PdfSnapshot) -> Vec<PdfXMutation> {
        ...
        vec![
            PdfXMutation::NoMutation,
            PdfXMutation::SetSnapshot { snapshot: stamp_conformance(base.clone(), true) },
            PdfXMutation::InsertEncryptionDictionary { version: 2, revision: 3 },
            PdfXMutation::RemoveEncryptionDictionary { version: 2, revision: 3 },
            PdfXMutation::SetOutputIntent { identifier: "sRGB IEC61966-2.1".to_string() },
            PdfXMutation::RemoveOutputIntent,
            PdfXMutation::SetTrimBox { page_index: 0, trim_box: [8.5, 8.5, 586.776, 833.39] },
            PdfXMutation::RemoveTrimBox { page_index: 0 },
            PdfXMutation::EmbedFontFile { descriptor_ordinal: 0, key: "FontFile2".to_string(), program: program },
            PdfXMutation::RemoveFontFile { descriptor_ordinal: 0 },
            PdfXMutation::InsertJavaScriptAction { script: "app.alert('audit');".to_string() },
            PdfXMutation::RemoveJavaScriptAction { script: "app.alert('audit');".to_string() },
            PdfXMutation::InsertLaunchAction { target: "render-plots.bat".to_string() },
            PdfXMutation::RemoveLaunchAction { target: "render-plots.bat".to_string() },
            PdfXMutation::InsertMediaAnnotation { subtype: "Movie".to_string(), title: "site walkthrough".to_string() },
            PdfXMutation::RemoveMediaAnnotation { subtype: "Movie".to_string(), title: "site walkthrough".to_string() },
        ]
    }
    //#endregion 🔖️Fixture
}
