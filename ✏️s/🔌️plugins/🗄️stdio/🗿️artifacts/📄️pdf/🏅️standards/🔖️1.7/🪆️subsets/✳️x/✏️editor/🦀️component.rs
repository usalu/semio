...
impl ArtifactEditor for Pdf17XEditor {
    ...
    type Command = Pdf17XEditorCommand;
    ...
    async fn handle(
        ...
        match command {
            Pdf17XEditorCommand::SetPage { index, text } => {
...
//#region 🔖️Manifest
pub async fn create_pdf17_x_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(PDF17X_DIALECT)
        .await.document(["stdio", "pdf", "1.7", "x"])
        .await.icon_id("file-text")
        .await.mode_def(edit::definition().await)
        .await.default_mode_id(edit::PDF17X_EDIT_MODE_ID)
