...
mod tests {
    ...
    fn detaches_and_can_restore_the_font_program() {
        ...
        let outcome = <RemoveFontFile as MutationKind<PdfSnapshot, PdfUaMutation>>::diff(&mutation, &base);
        let next = outcome.diff().apply(&base).unwrap();
        let descriptor = support::font_descriptors(&next)[0];
        assert!(support::font_program(&next, descriptor).is_none());
        assert_eq!(<RemoveFontFile as MutationKind<PdfSnapshot, PdfUaMutation>>::inverse(&mutation, &base).len(), 1);
    }
}
//#endregion 🧪️Tests
