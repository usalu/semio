//! 🧐️ PdfAAnalyzer (1.7/✳️a) — real ISO 19005-2 (PDF/A-2) / ISO 19005-3 (PDF/A-3) conformance
//! checks against the retained `PdfSnapshot.objects` graph (D5, ticket
//! 26/08/10/ARTIFACT-SYSTEM-OVERHAUL-REAL-CODECS-RUNTIME-REUSE-EVOLUTION §D5). This is the FIRST
//! real, non-`✳️any` subset in the whole repo (originally landed as `✳️a-2b`; restructured to
//! `✳️a` in ticket 26/08/11/ARTIFACT-STANDARD-SUBSETS-REAL-VOCABULARIES W2 -- see `detect_pdfa_level`
//! below for why the conformance LEVEL is no longer baked into the subset id).
//!
//! Checks implemented as real, honest scans (never fabricated against fields the engine doesn't
//! parse):
//! - HARD (blocks the `a` dialect stamp): `/Encrypt` — since decode already rejects any file
...
//! - HARD: an embedded-file `/Type /Filespec` object that carries an `/EF` (actual attached file
//!   stream) but no `/AFRelationship` key — ISO 19005-3 requires `/AFRelationship` on every
//!   embedded-file association; ISO 19005-2 forbids generic embedded files entirely, so an `/EF`
//!   without a declared relationship is non-conformant under either part. This is the A-2/A-3
//!   differentiator the roster asked for; see `embedded_files_missing_afrelationship`.
...
//! - INFO (reported via `Severity::Hint`, the softest severity this fault model has -- there is
//!   no distinct `Info` variant): the DATA-detected PDF/A conformance level, see
//!   `detect_pdfa_level`.
