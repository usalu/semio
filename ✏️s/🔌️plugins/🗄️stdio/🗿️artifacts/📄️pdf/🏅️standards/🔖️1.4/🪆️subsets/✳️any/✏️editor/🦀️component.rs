//! 📄️ PDF Document (1.4) editor -- one of stdio's 10 real PDF subset editors (ticket
...
//! document schema id (confirmed by direct read before writing this file). `Pdf14Editor`
//! implements `ArtifactEditor` over that shared type, tagged with this file's own `PDF14_DIALECT`
//! (`standard: "1.4"`, `subset: "*"`) -- subsets are constraint PROFILES of the same
...
impl ArtifactEditor for Pdf14Editor {
    ...
    type Command = Pdf14EditorCommand;
    ...
    async fn handle(
        ...
        match command {
            Pdf14EditorCommand::SetPage { index, text } => {
...
//#region 🔖️Manifest
pub async fn create_pdf14_editor() -> semio_framework_plugin::AppDefinition {
    Editor::builder(PDF14_DIALECT)
        .await.document(["stdio", "pdf", "1.4", "any"])
        .await.icon_id("file-text")
        .await.mode_def(edit::definition().await)
        .await.default_mode_id(edit::PDF14_EDIT_MODE_ID)
