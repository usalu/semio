//! 🧪️ `replace-page-text` fixture — `round-trips-the-concrete-inverse`.
//!
...
fn applies_to_committed_after() {
    ...
    assert!(outcome.messages().is_empty(), "replace-page-text/round-trips-the-concrete-inverse: the committed vector is a clean applied vector");
    assert_eq!(state, expected_after(), "replace-page-text/round-trips-the-concrete-inverse: applied state differs from the committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    assert!(!inverse.is_empty(), "replace-page-text/round-trips-the-concrete-inverse: a mutation that really moved the document must offer an undo");
    for step in &inverse {
        assert!(step.diff(&state).apply_to(&mut state).messages().is_empty(), "replace-page-text/round-trips-the-concrete-inverse: an inverse step was refused");
    }
    assert_eq!(state, base, "replace-page-text/round-trips-the-concrete-inverse: the undo did not restore the committed before-snapshot");
}
...
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(raised.is_empty(), "replace-page-text/round-trips-the-concrete-inverse: declared applied, but dispatch raised {raised:?}"),
        _ => assert!(!raised.is_empty(), "replace-page-text/round-trips-the-concrete-inverse: declared {status}, but dispatch raised nothing"),
    }
}
