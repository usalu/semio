//! 🧪️ `remove-page` fixture — `round-trips-the-concrete-inverse`.
//!
...
fn mutation() -> PdfMutation {
    serde_json::from_str(MUTATION).expect("committed remove-page payload decodes")
}
...
fn applies_to_committed_after() {
    ...
    assert!(outcome.messages().is_empty(), "remove-page/round-trips-the-concrete-inverse: the committed vector is a clean applied vector");
    assert_eq!(state, expected_after(), "remove-page/round-trips-the-concrete-inverse: applied state differs from the committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    assert!(!inverse.is_empty(), "remove-page/round-trips-the-concrete-inverse: a mutation that really moved the document must offer an undo");
    for step in &inverse {
        assert!(step.diff(&state).apply_to(&mut state).messages().is_empty(), "remove-page/round-trips-the-concrete-inverse: an inverse step was refused");
    }
    assert_eq!(state, base, "remove-page/round-trips-the-concrete-inverse: the undo did not restore the committed before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "remove-page/round-trips-the-concrete-inverse: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("payload encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("payload reparses");
    assert_eq!(reencoded, original, "remove-page/round-trips-the-concrete-inverse: committed payload JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(raised.is_empty(), "remove-page/round-trips-the-concrete-inverse: declared applied, but dispatch raised {raised:?}"),
        _ => assert!(!raised.is_empty(), "remove-page/round-trips-the-concrete-inverse: declared {status}, but dispatch raised nothing"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "remove-page/round-trips-the-concrete-inverse: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "remove-page/round-trips-the-concrete-inverse: committed diff did not carry before to after");
}
...
fn op_codecs_round_trip() {
    ...
    for step in std::iter::once(payload.clone()).chain(payload.inverse(&before())) {
        assert_eq!(PdfMutation::parse_op(&step.print_op()).expect("the text op parses"), step, "remove-page/round-trips-the-concrete-inverse: the text op form does not round-trip");
        assert_eq!(PdfMutation::decode_op(&step.encode_op().expect("the binary op encodes")).expect("the binary op decodes"), step, "remove-page/round-trips-the-concrete-inverse: the binary op form does not round-trip");
        assert_eq!(serde_json::from_value::<PdfMutation>(serde_json::to_value(&step).expect("the payload encodes")).expect("the payload decodes"), step, "remove-page/round-trips-the-concrete-inverse: the JSON form does not round-trip");
    }
}
