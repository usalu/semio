//! 📄️ PDF/A Document (1.4) editor -- one of stdio's 10 real PDF subset editors (ticket
...
//! document schema id (confirmed by direct read before writing this file). `Pdf14AEditor`
//! implements `ArtifactEditor` over that shared type, tagged with this file's own `PDF14A_DIALECT`
//! (`standard: "1.4"`, `subset: "a"`) -- subsets are constraint PROFILES of the same
...
pub struct Pdf14AEditor;

impl ArtifactEditor for Pdf14AEditor {
    ...
    type Command = Pdf14AEditorCommand;
    ...
    async fn handle(
        ...
        match command {
            Pdf14AEditorCommand::SetPage { index, text } => {
...
pub fn create_pdf14_a_editor() -> semio_framework_plugin::AppDefinition {
    let builder = Editor::builder(PDF14A_DIALECT);
    let builder = semio_framework_plugin::resolve_ready(builder.document(["stdio", "pdf", "1.4", "a"]));
    ...
    let builder = semio_framework_plugin::resolve_ready(builder.default_mode_id(edit::PDF14A_EDIT_MODE_ID));
