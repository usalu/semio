//! 🎹️ PdfAComposer (1.4/✳️a) — reads the same sources the ✳️any subset does (native `stdio.pdf`
//! 1.4, plus its `binary`/`deflate` DAG deps), delegates the actual parse to the ✳️any composer,
//! then folds the honestly-scope-limited PDF/A-1 diagnostics on top. PASS-THROUGH by design (see
...
use crate::artifacts::pdf::standards::v1_4::subsets::a::analyzer::check_pdf_a_conformance;
...
mod tests {
    ...
    use crate::artifacts::pdf::standards::v1_4::subsets::a::analyzer::CODE_SCHEMA_GAP;
    ...
    fn compose_always_carries_the_schema_gap_diagnostic() {
        ...
        let composed = PdfAComposer::compose(&sources).expect("pass-through compose never fails on conformance grounds");
