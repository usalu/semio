//! 🧪️ `insert-page` fixture — `round-trips-the-concrete-inverse`.
//!
...
fn mutation() -> PdfMutation {
    serde_json::from_str(MUTATION).expect("committed insert-page payload decodes")
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "insert-page/round-trips-the-concrete-inverse: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
fn op_codecs_round_trip() {
    ...
    for step in std::iter::once(payload.clone()).chain(payload.inverse(&before())) {
        ...
        assert_eq!(serde_json::from_value::<PdfMutation>(serde_json::to_value(&step).expect("the payload encodes")).expect("the payload decodes"), step, "insert-page/round-trips-the-concrete-inverse: the JSON form does not round-trip");
    }
}
