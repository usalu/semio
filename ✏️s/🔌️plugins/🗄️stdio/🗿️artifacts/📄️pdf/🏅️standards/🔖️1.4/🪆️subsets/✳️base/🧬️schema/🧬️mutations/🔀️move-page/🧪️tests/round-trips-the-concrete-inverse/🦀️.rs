//! 🧪️ `move-page` fixture — `round-trips-the-concrete-inverse`.
//!
...
fn mutation() -> PdfMutation {
    serde_json::from_str(MUTATION).expect("committed move-page payload decodes")
}
...
fn inverse_restores_before() {
    ...
    for step in &inverse {
        assert!(step.diff(&state).apply_to(&mut state).messages().is_empty(), "move-page/round-trips-the-concrete-inverse: an inverse step was refused");
    }
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "move-page/round-trips-the-concrete-inverse: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("payload encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("payload reparses");
    assert_eq!(reencoded, original, "move-page/round-trips-the-concrete-inverse: committed payload JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(raised.is_empty(), "move-page/round-trips-the-concrete-inverse: declared applied, but dispatch raised {raised:?}"),
        _ => assert!(!raised.is_empty(), "move-page/round-trips-the-concrete-inverse: declared {status}, but dispatch raised nothing"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "move-page/round-trips-the-concrete-inverse: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "move-page/round-trips-the-concrete-inverse: committed diff did not carry before to after");
}
...
fn op_codecs_round_trip() {
    ...
    for step in std::iter::once(payload.clone()).chain(payload.inverse(&before())) {
        ...
        assert_eq!(serde_json::from_value::<PdfMutation>(serde_json::to_value(&step).expect("the payload encodes")).expect("the payload decodes"), step, "move-page/round-trips-the-concrete-inverse: the JSON form does not round-trip");
    }
}
