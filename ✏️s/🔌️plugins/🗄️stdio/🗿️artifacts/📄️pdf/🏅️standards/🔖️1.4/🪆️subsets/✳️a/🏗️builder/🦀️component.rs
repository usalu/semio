//! 🏗️ PdfABuilder (1.4/✳️a) — PASS-THROUGH by design, matching `🎹️composer`'s honesty scope:
...
//! way 1.7's `✳️a` builder does. `build()` re-runs `check_pdf_a_conformance` and always attaches
...
mod tests {
    ...
    fn pass_through_build_never_fails_on_conformance_grounds() {
        let snapshot = PdfABuilder::empty().build().expect("no hard check exists at this schema; build must succeed");
