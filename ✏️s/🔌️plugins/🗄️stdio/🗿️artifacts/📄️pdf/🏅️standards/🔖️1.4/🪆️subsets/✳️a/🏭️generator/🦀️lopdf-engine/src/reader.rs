...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        Some("project") => match read(args.get(2).expect("usage: reader project <file.pdf>")) {
            Ok(json) => println!("{}", report("pdf-14-a-project", "ok", &json, None)),
            Err(error) => {
                println!("{}", report("pdf-14-a-project", "failed", "{}", Some(&error)));
                exit(1);
            }
        },
        Some("compare") => {
            ...
            match (read(expected), read(actual)) {
                (Ok(left), Ok(right)) => {
                    ...
                    println!("{}", report("pdf-14-a-compare", "ok", &format!("{{\"equal\":{equal},\"expected\":{left},\"actual\":{right}}}"), None));
                }
                (Err(error), _) | (_, Err(error)) => {
                    println!("{}", report("pdf-14-a-compare", "failed", "{}", Some(&error)));
                    exit(1);
                }
            }
        }
