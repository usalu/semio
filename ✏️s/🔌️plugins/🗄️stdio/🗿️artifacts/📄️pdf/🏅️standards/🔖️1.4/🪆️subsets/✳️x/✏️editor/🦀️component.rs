//! 📄️ PDF/X Document (1.4) editor -- one of stdio's 10 real PDF subset editors (ticket
...
//! document schema id (confirmed by direct read before writing this file). `Pdf14XEditor`
//! implements `ArtifactEditor` over that shared type, tagged with this file's own `PDF14X_DIALECT`
//! (`standard: "1.4"`, `subset: "x"`) -- subsets are constraint PROFILES of the same
...
impl protocol::OpText for Pdf14XEditorCommand {
...
impl protocol::OpBinary for Pdf14XEditorCommand {
...
pub struct Pdf14XEditor;

impl ArtifactEditor for Pdf14XEditor {
    ...
    type Command = Pdf14XEditorCommand;
    ...
    async fn handle(
        ...
        match command {
            Pdf14XEditorCommand::SetPage { index, text } => {
