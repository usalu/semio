...
//!   * `build <recipe-id> <out-dir>` writes `<out-dir>/<recipe-id>/⬅️before.obj` [and `after.obj`],
...
fn cmd_build(id: &str, out_dir: &str) -> i32 {
    ...
    fs::write(dir.join("⬅️before.obj"), recipe.before).expect("write before.obj");
    match recipe.after {
        Some(after) => {
            admit_or_panic(&format!("{}/after", recipe.id), after);
            fs::write(dir.join("➡️after.obj"), after).expect("write after.obj");
