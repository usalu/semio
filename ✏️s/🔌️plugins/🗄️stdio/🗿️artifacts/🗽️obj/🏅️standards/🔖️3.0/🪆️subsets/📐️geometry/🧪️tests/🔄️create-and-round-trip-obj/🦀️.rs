//! 🦀️ OBJ creation and round-trip case — Rust adapter.
//!
...
use semio_s_plugin_stdio_test_oracle::mesh::{oracle_create_obj, project_obj, MeshSpec};
...
fn oracle(ctx: &Context) -> Result<Outcome, String> {
    let bytes = oracle_create_obj(&spec(ctx)?)?;
...
mod subject {
    ...
    pub fn run(ctx: &Context) -> Result<Outcome, String> {
        ...
        let text = String::from_utf8(reference).map_err(|error| format!("reference OBJ is not UTF-8: {}", error))?;
