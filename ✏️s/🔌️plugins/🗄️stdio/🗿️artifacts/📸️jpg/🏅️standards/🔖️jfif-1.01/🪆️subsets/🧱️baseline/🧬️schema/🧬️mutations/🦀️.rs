...
//!
//! `🧾️document` owns the DOCUMENT vocabulary — the JFIF header, the quantization and Huffman tables by
...
//!
//! Two of these axes overlap the `🧾️document` vocabulary by NAME and not by meaning.
...
//!
//! `Diff` is `JpgDiff`, the SAME diff type `🧾️document` uses — the two subsets share one snapshot type,
...
//!
//! `encode_jpg` (`../../🧾️document/🚪️io/🦀️.rs`) writes a baseline file and nothing else, by
...
//! and claimed by `🟣️mutate-jpg-jfif-1-01-baseline`, and that case measures this vocabulary where its
...
//! both through the INDEPENDENT `image` reader the sibling `🧾️document` subset registers.
//!
