...
pub const BINARY_TAG: u8 = dsl::protocol_record::tag_u8(include_str!("../../💾️binary/📡️.protocol.semio"), "remove-huffman-table");
...
pub fn encode_payload(payload: &RemoveHuffmanTableMutation) -> Result<Vec<u8>, protocol::ProtocolError> {
    ...
    let mut out = Vec::new();
    enc_huffman_key_bin(key, &mut out);
    Ok(out)
}
pub fn decode(bytes: &[u8]) -> Result<JpgMutation, protocol::ProtocolError> {
    let mut reader = store::ByteReader::new(bytes);
    let malformed = |what: &'static str, offset: usize, detail: String| protocol::ProtocolError::Malformed { what, offset: offset as u64, detail };
    let result: Result<JpgMutation, protocol::ProtocolError> = Ok(JpgMutation::RemoveHuffmanTable(RemoveHuffmanTableMutation { key: dec_huffman_key_bin(&mut reader).map_err(|e| malformed("op huffman-key", reader.position(), e))? }));
