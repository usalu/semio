...
fn mutation() -> JpgMutation {
    dsl::json::from_json_str(MUTATION).expect("committed replace-quant-table payload decodes")
}
...
fn op_codecs_round_trip() {
    ...
    assert_eq!(bytes[1], binary::BINARY_TAG, "replace-quant-table/direct-behavior: the binary frame must carry this schema's tag");
