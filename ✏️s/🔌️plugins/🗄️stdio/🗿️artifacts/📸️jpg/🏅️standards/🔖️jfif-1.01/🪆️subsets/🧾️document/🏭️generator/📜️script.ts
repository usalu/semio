...
const RECIPES: readonly Recipe[] = [
  ...
  { id: "replace-quant-table-applied", directoryName: "📊️replace-quant-table-applied", mutation: "replace-quant-table", witnessable: false, notes: "Byte-identical before/after: this subset's own production encoder regenerates DQT fresh from re_encode_quality on every write, so no encoder in this repository can carry a replaced quant table into the bytes." },
  { id: "remove-quant-table-applied", directoryName: "🧹️remove-quant-table-applied", mutation: "remove-quant-table", witnessable: false, notes: "Byte-identical before/after, same reason as replace-quant-table." },
  { id: "replace-huffman-table-applied", directoryName: "🌳️replace-huffman-table-applied", mutation: "replace-huffman-table", witnessable: false, notes: "Byte-identical before/after: production regenerates DHT fresh on every write." },
  { id: "remove-huffman-table-applied", directoryName: "🪓️remove-huffman-table-applied", mutation: "remove-huffman-table", witnessable: false, notes: "Byte-identical before/after, same reason as replace-huffman-table." },
  { id: "change-restart-interval-applied", directoryName: "🔁️change-restart-interval-applied", mutation: "change-restart-interval", witnessable: false, notes: "Byte-identical before/after: production never emits a DRI/restart marker at all." },
...
async function main(argv: readonly string[]): Promise<number> {
  ...
  if (command !== "generate" && command !== "manifests") {
    ...
    if (command === "libjpeg" || command === "libjpeg-manifests") {
      const KINDS: Record<string, Readonly<{ directoryName: string; args: readonly string[] }>> = {
        "change-restart-interval": { directoryName: "⏱️change-restart-interval", args: ["-restart", "2"] },
        "replace-huffman-table": { directoryName: "🔀️replace-huffman-table", args: ["-optimize"] },
      };
      ...
      const SCAN_DIRECTORY_NAME = "🌲️remove-huffman-table";
      const SCANS = { before: "4️⃣four-tables.scan", after: "3️⃣three-tables.scan" };
      ...
      if (command === "libjpeg") {
        ...
        for (const [kind, recipe] of Object.entries(KINDS)) {
          ...
          const seed = spawnSync("python3", ["-c", "import sys\nfrom PIL import Image\nim=Image.new('RGB',(32,32))\n[im.putpixel((x,y),((x*8)%256,(y*8)%256,((x+y)*4)%256)) for x in range(32) for y in range(32)]\nim.save(sys.argv[1], quality=90)", join(dir, "⬅️before.jpg")], { stdio: "inherit" });
          if (seed.status !== 0) { failures.push(`${kind}: seed writer failed`); continue; }
          const transcoded = spawnSync("jpegtran", [...recipe.args, "-outfile", join(dir, "➡️after.jpg"), join(dir, "⬅️before.jpg")], { stdio: "inherit" });
          if (transcoded.status !== 0) { failures.push(`${kind}: jpegtran failed`); continue; }
          const cmp = spawnSync("bun", [probes, "jpg-libjpeg-compare", "--input", join(dir, "⬅️before.jpg"), "--input", join(dir, "➡️after.jpg")], { encoding: "utf8" });
        ...
        {
          const dir = join(outDir, SCAN_DIRECTORY_NAME);
          mkdirSync(dir, { recursive: true });
          ...
          if (seed.status !== 0) failures.push(`${SCAN_KIND}: seed writer failed`);
            ...
            for (const [half, script] of [["⬅️before.jpg", SCANS.before], ["➡️after.jpg", SCANS.after]] as const) {
            ...
            const before = dhtSet(join(dir, "⬅️before.jpg"));
            const after = dhtSet(join(dir, "➡️after.jpg"));
            ...
            for (const half of ["⬅️before.jpg", "➡️after.jpg"]) {
      ...
      for (const kind of [...Object.keys(KINDS), SCAN_KIND]) {
        const directoryName = kind === SCAN_KIND ? SCAN_DIRECTORY_NAME : KINDS[kind]!.directoryName;
        const files = [];
...
if kind == 'replace-quant-table':
    ...
    im.save(os.path.join(d, '➡️after.jpg'), quality=40)
    ...
    im.save(os.path.join(d, '⬅️before.jpg'), qtables=[STD, STD])
    im.save(os.path.join(d, '➡️after.jpg'), qtables=[STD])
else:
    raise SystemExit('unknown kind ' + kind)
print(kind + ': written')
`;
      const KINDS: Readonly<Record<string, string>> = {
        "replace-quant-table": "📈️replace-quant-table",
        "change-jfif-header": "🔎️change-jfif-header",
        "remove-quant-table": "📉️remove-quant-table",
      };
      const probes = join(import.meta.dir, "..", "🔬️probes", "📜️script.ts");
      const root = outDir;
      if (command === "markers") {
