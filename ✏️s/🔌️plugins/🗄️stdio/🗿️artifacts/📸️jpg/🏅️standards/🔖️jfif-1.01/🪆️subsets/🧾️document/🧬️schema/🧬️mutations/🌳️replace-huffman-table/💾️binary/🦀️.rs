...
pub fn decode(bytes: &[u8]) -> Result<JpgMutation, protocol::ProtocolError> {
    ...
    let result: Result<JpgMutation, protocol::ProtocolError> =
        Ok(JpgMutation::ReplaceHuffmanTable(ReplaceHuffmanTableMutation { table: dec_huffman_table_bin(&mut reader).map_err(|e| malformed("op huffman-table", reader.position(), e))? }));
