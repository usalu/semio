...
impl protocol::MutationKind<JpgSnapshot, JpgMutation> for ReplaceQuantTableMutation {
    ...
    fn inverse(&self, base: &JpgSnapshot) -> Vec<JpgMutation> {
        ...
        match base.quant_tables.iter().find(|t| t.id == table.id) {
            Some(existing) => vec![JpgMutation::ReplaceQuantTable(ReplaceQuantTableMutation { table: existing.clone() })],
...
pub(crate) fn test_case() -> JpgMutation {
    serde_json::from_str(include_str!("🧪️tests/🎯️direct-behavior/🦠️mutation/🔣️.json")).expect("committed replace-quant-table payload")
}
#[cfg(test)]
