...
pub fn encode_payload(payload: &ReplaceQuantTableMutation) -> Result<Vec<u8>, protocol::ProtocolError> {
    ...
    let mut out = Vec::new();
    enc_quant_table_bin(table, &mut out);
    Ok(out)
}
pub fn decode(bytes: &[u8]) -> Result<JpgMutation, protocol::ProtocolError> {
    let mut reader = store::ByteReader::new(bytes);
    let malformed = |what: &'static str, offset: usize, detail: String| protocol::ProtocolError::Malformed { what, offset: offset as u64, detail };
    let result: Result<JpgMutation, protocol::ProtocolError> = Ok(JpgMutation::ReplaceQuantTable(ReplaceQuantTableMutation { table: dec_quant_table_bin(&mut reader).map_err(|e| malformed("op quant-table", reader.position(), e))? }));
