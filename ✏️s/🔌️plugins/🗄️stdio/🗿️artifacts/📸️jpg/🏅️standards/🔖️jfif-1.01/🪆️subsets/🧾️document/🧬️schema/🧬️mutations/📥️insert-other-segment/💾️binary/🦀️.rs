...
pub const BINARY_TAG: u8 = dsl::protocol_record::tag_u8(include_str!("../../💾️binary/📡️.protocol.semio"), "insert-other-segment");
...
pub fn encode_payload(payload: &InsertOtherSegmentMutation) -> Result<Vec<u8>, protocol::ProtocolError> {
    ...
    enc_segment_bin(segment, &mut out);
    Ok(out)
}
pub fn decode(bytes: &[u8]) -> Result<JpgMutation, protocol::ProtocolError> {
    ...
    let result: Result<JpgMutation, protocol::ProtocolError> = {
        ...
        let segment = dec_segment_bin(&mut reader).map_err(|e| malformed("op segment", reader.position(), e))?;
        Ok(JpgMutation::InsertOtherSegment(InsertOtherSegmentMutation { index, segment }))
    };
