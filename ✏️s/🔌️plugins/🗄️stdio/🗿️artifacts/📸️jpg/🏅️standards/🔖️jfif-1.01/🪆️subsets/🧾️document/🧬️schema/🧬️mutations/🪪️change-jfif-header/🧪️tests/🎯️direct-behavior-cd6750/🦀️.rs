...
fn mutation() -> JpgMutation {
    serde_json::from_str(MUTATION).expect("committed change-jfif-header payload decodes")
}
...
fn op_codecs_round_trip() {
    ...
    assert_eq!(bytes[1], super::binary::BINARY_TAG, "change-jfif-header/direct-behavior: the binary frame must carry this schema's tag");
