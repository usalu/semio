...
/// 📦️ Encodes a `CurateSnapshot` to its binary pack form.
pub fn encode(document: &CurateSnapshot) -> Vec<u8> {
    store::DocumentPack::encode_pack(document)
}

/// 📖️ Decodes a `CurateSnapshot` from its binary pack form.
pub fn decode(bytes: &[u8]) -> Result<CurateSnapshot, PackError> {
