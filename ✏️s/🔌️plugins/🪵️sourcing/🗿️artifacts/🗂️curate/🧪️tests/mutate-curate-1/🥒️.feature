...
@oracle-curate-python-independent
...
Feature: Apply every typed curation mutation twice — once in Rust, once in Python — and require the same answer
  ...
  add/remove pair. No inventory or bill-of-materials library speaks `.curate.dsl.semio`, and none is
  ...
  A finding recorded while deriving it: this repository's own `parse_curate_dsl` REJECTS that
  ...
  Why the Python reference does not read the carrier. `.curate.dsl.semio` has no prose document, and
  ...
  Scenario Outline: Replay the committed <id> specification vector through both implementations
    ...
    Examples:
      ...
      | create-curated-item       | append | 🌱create-curated-item      | appends-a-steel-plate-to-the-curation      |
      | delete-curated-item       | detach | 🗑️delete-curated-item      | removes-the-clt-panel-from-the-curation    |
      | change-curated-item-count | retune | 🔢change-curated-item-count | raises-the-glulam-beam-count-to-20         |
