#!/usr/bin/env python3
"""🗂️ An INDEPENDENT second implementation of the `sourcing.curate` document and all three of its
...
curation of `(objectId, count)` pairs. No third party reads `.curate.dsl.semio`, and no bill-of-
...
`.curate.dsl.semio`. That carrier has no prose document, and its committed example is a structured
...
"""🗂️ The three members `CurateSnapshot` declares — and the cross-language projection."""
...
def mutate_handler(kind):
    ...
    def handler(ctx):
        document = document_of(json_fixture(ctx, "timber-kit"))
...
def identity_handler(ctx):
    ...
    The `.curate.dsl.semio` carrier's own laws are asserted in role on the Rust side, against the
