...
fn compose_export_glb(sources: &[ErasedComposeSource]) -> Result<ComposedArtifact, ComposeError> {
    let snapshot = rebuild_native_snapshot(sources)?;
    let bytes = crate::artifacts::curate::io::export::serializers::artifacts::glb::v2_0::any::serialize_bytes(&snapshot).map_err(|e| ComposeError { message: e.to_string(), diagnostics: Vec::new() })?;
...
pub fn entries() -> &'static [ComposerEntry] {
    ENTRIES.get_or_init(|| vec![
        ...
        ComposerEntry { writes: EXPORT_GLB_DIALECT, reads: &[CURATE_DIALECT], compose: compose_export_glb },
