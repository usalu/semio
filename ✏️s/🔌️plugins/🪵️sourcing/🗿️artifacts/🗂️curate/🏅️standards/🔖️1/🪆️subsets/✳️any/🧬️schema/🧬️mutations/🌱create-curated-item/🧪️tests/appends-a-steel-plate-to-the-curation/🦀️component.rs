//! 🧪️ `create-curated-item` fixture — `appends-a-steel-plate-to-the-curation`.
//!
//! `create-curated-item`'s diff oracle guards a single condition — the object is already curated ⇒
//! Fatal `mutation.duplicate-id` — and otherwise emits a genuinely sparse `CurateCuratedDelta`
...
fn before() -> CurateSnapshot {
    serde_json::from_str(BEFORE).expect("before curate document decodes")
}
fn expected_after() -> CurateSnapshot {
    serde_json::from_str(AFTER).expect("after curate document decodes")
}
fn mutation() -> SourcingMutation {
    serde_json::from_str(MUTATION).expect("create-curated-item mutation decodes")
}
fn built_outcome() -> protocol::MutationOutcome<CurateDiff> {
    <SourcingMutation as protocol::Mutation<CurateSnapshot>>::diff(&mutation(), &before())
}
...
async fn deleting_the_new_pick_restores_before() {
    ...
    let inverse = <SourcingMutation as protocol::Mutation<CurateSnapshot>>::inverse(&mutation(), &base);
    ...
    for step in &inverse {
        let undo = <SourcingMutation as protocol::Mutation<CurateSnapshot>>::diff(step, &snapshot);
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: CurateSnapshot = serde_json::from_str(text).expect("curate document decodes");
        let reencoded = serde_json::to_value(&decoded).expect("curate document encodes");
        let original: serde_json::Value = serde_json::from_str(text).expect("curate document reparses");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "create-curated-item/appends-a-steel-plate-to-the-curation: produced diff differs from the committed 🔺️diff/🔣️component.json");
}

/// 🔣️ The committed diff decodes to `CurateDiff` and re-encodes unchanged.
#[semio_framework_async_macros::async_test]
async fn committed_diff_is_canonical() {
    let decoded: CurateDiff = serde_json::from_str(DIFF).expect("committed create-curated-item diff decodes");
