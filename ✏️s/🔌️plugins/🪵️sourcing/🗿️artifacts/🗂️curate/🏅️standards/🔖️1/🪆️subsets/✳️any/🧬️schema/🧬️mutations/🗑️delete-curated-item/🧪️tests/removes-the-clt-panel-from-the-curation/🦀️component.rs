//! 🧪️ `delete-curated-item` fixture — `removes-the-clt-panel-from-the-curation`.
//!
//! `delete-curated-item`'s diff oracle guards a single condition — the object is not curated ⇒
//! Error `mutation.target-missing` — and otherwise emits a `CurateCuratedDelta` carrying ONE
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-curated-item/removes-the-clt-panel-from-the-curation: produced diff differs from the committed 🔺️diff/🔣️component.json");
}

/// 🔣️ The committed diff decodes to `CurateDiff` and re-encodes unchanged.
#[semio_framework_async_macros::async_test]
async fn committed_diff_is_canonical() {
    let decoded: CurateDiff = serde_json::from_str(DIFF).expect("committed delete-curated-item diff decodes");
