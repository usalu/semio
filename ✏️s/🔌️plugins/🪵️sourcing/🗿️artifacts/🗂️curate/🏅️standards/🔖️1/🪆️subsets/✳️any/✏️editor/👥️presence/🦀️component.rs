...
impl Mutation<SourcingCuratePresence> for SourcingCuratePresenceMutation {
    ...
    async fn diff(&self, _base: &SourcingCuratePresence) -> protocol::MutationOutcome<SourcingCuratePresence> {
        protocol::MutationOutcome::new(match self {
            Self::Snapshot { presence } => presence.clone(),
        })
    }

    async fn inverse(&self, base: &SourcingCuratePresence) -> Vec<Self> {
