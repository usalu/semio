//! 🚪️ curate <- png — foreign `Deserializer<CurateSnapshot>` (ticket 26/08/17/CLEAN-ARTIFACT-
...
//! `CurateSnapshot` pack rather than as a `PngSnapshot` pack, and `deserialize(from: &PngSnapshot)`
//! merely re-encodes `from` and forwards into that same (format-mismatched) path — confirmed by
//! inspection, not a regression introduced by this pass. No domain-correct "PNG image -> curate
...
impl Deserializer<CurateSnapshot> for PngIntoCurate {
    ...
    fn deserialize(payload: &IoPayload) -> IoResult<CurateSnapshot> {
        ...
        deserialize_bytes(bytes).map(IoOutcome::clean).map_err(|error| IoError { message: format!("PngIntoCurate: {error}"), diagnostics: Vec::new() })
    }
}
