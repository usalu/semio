...
pub async fn apply_stock_extra_delta(
    ...
    for (index, id) in delta.removed.iter().enumerate() {
        if !removed.insert(id.as_str()) {
            return Err(protocol::MutationApplyError::new(
                ...
                "stock entry is removed more than once",
            )
        ...
        if !stock_extra.iter().any(|extra| &extra.id == id) {
            return Err(protocol::MutationApplyError::new(
                ...
                "removed stock entry does not exist",
            )
            .at(["removed".to_string(), index.to_string()]));
        }
    }
    let mut identities: std::collections::BTreeSet<_> =
        stock_extra.iter().map(|extra| extra.id.clone()).collect();
    ...
    for (index, extra) in delta.added.iter().enumerate() {
        if !identities.insert(extra.id.clone()) {
            return Err(protocol::MutationApplyError::new(
                "mutation.apply.duplicate-target",
                "added stock entry identity already exists",
            )
    ...
    for (index, entry) in delta.patched.iter().enumerate() {
        if !patched.insert(entry.id.as_str()) {
            return Err(protocol::MutationApplyError::new(
                "mutation.apply.duplicate-target",
                "stock entry is patched more than once",
            )
        ...
        if removed.contains(entry.id.as_str()) || !identities.contains(&entry.id) {
            return Err(protocol::MutationApplyError::new(
                "mutation.apply.missing-target",
                "patched stock entry does not exist",
            )
            .at(["patched".to_string(), index.to_string()]));
        }
        if entry.extra.id != entry.id {
            return Err(protocol::MutationApplyError::new(
                ...
                "stock entry patch cannot change its identity",
            )
            .at(["patched".to_string(), index.to_string()]));
        }
    }
    let mut next: Vec<_> = stock_extra
        .iter()
        .filter(|extra| !removed.contains(extra.id.as_str()))
        .cloned()
        .collect();
    next.extend(delta.added.iter().cloned());
    for entry in &delta.patched {
        let target = next.iter_mut().find(|extra| extra.id == entry.id).ok_or_else(|| {
            protocol::MutationApplyError::new(
                "mutation.apply.missing-target",
                "patched stock entry does not exist after structural edits",
            )
...
pub async fn apply_curated_delta(
    ...
    for (index, id) in delta.removed.iter().enumerate() {
        if !removed.insert(id.as_str()) {
            return Err(protocol::MutationApplyError::new(
                "mutation.apply.duplicate-target",
                "curated item is removed more than once",
            )
            .at(["removed".to_string(), index.to_string()]));
        }
        if !curated.iter().any(|item| &item.object_id == id) {
            return Err(protocol::MutationApplyError::new(
                "mutation.apply.missing-target",
                "removed curated item does not exist",
            )
            .at(["removed".to_string(), index.to_string()]));
        }
    }
    let mut identities: std::collections::BTreeSet<_> =
        curated.iter().map(|item| item.object_id.clone()).collect();
    for id in &delta.removed {
        identities.remove(id);
    }
    for (index, item) in delta.added.iter().enumerate() {
        if !identities.insert(item.object_id.clone()) {
            return Err(protocol::MutationApplyError::new(
                "mutation.apply.duplicate-target",
                "added curated item identity already exists",
            )
            .at(["added".to_string(), index.to_string()]));
        }
    }
    ...
    for (index, entry) in delta.patched.iter().enumerate() {
        if !patched.insert(entry.object_id.as_str()) {
            return Err(protocol::MutationApplyError::new(
                "mutation.apply.duplicate-target",
                "curated item is patched more than once",
            )
            .at(["patched".to_string(), index.to_string()]));
        }
        if removed.contains(entry.object_id.as_str()) || !identities.contains(&entry.object_id) {
            return Err(protocol::MutationApplyError::new(
                "mutation.apply.missing-target",
                "patched curated item does not exist",
            )
            .at(["patched".to_string(), index.to_string()]));
        }
    }
    let mut next: Vec<_> = curated
        .iter()
        .filter(|item| !removed.contains(item.object_id.as_str()))
        .cloned()
        .collect();
    next.extend(delta.added.iter().cloned());
    for entry in &delta.patched {
        let target = next
            ...
            .ok_or_else(|| {
                protocol::MutationApplyError::new(
                    "mutation.apply.missing-target",
                    "patched curated item does not exist after structural edits",
                )
                .at(["patched".to_string(), entry.object_id.clone()])
            })?;
...
async fn reorder_named<T>(
    ...
    if order.len() != items.len()
        || order.iter().enumerate().any(|(index, target)| {
            order[..index].contains(target) || !items.iter().any(|item| id(item) == target)
        })
    {
        return Err(protocol::MutationApplyError::new(
    ...
    for target in order {
        let index = remaining.iter().position(|item| id(item) == target).ok_or_else(|| {
            protocol::MutationApplyError::new(
                "mutation.apply.missing-target",
                "reordered item does not exist",
            )
            .at(["reordered".to_string(), target.clone()])
        })?;
