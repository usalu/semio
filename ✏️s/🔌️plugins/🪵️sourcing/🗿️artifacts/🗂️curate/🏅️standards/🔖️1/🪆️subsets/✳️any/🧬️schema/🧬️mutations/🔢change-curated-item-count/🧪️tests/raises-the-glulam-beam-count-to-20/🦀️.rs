...
//! and otherwise emits a `CurateCuratedDelta` carrying ONE `patched` entry. `count` is set to a
...
/// 🔣️ The committed diff decodes to `CurateDiff` and re-encodes unchanged — the patch entry's
/// `count` is an `Option<u32>` with no skip attribute, so it is emitted as a real number.
#[semio_framework_async_macros::async_test]
async fn committed_diff_is_canonical() {
    let decoded: CurateDiff = serde_json::from_str(DIFF).expect("committed change-curated-item-count diff decodes");
