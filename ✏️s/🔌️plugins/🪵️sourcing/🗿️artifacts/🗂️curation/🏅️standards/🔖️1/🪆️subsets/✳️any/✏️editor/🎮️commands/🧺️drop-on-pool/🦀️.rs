//! 🧺️ 🧺️ Sourcing curation app commands command — `drop-on-pool`.

use crate::artifacts::curation::op::SourcingMutation;
use crate::artifacts::curation::schema::{curation_decision_for_set, CurationDecision};
...
/// 🪂️ Dropping a curated row back onto the pool mirrors `curation_remove`: zero its curated count.
pub fn handle(payload: &DropOnPool, doc: &ArtifactView<'_, CurationSnapshot>, _cfg: &ConfigView<'_, SourcingCurationConfig>) -> Result<Emit<SourcingMutation, SourcingCurationConfigMutation>, Fault> {
