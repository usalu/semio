...
//! a second implementation of the `sourcing.curation` document and all three typed mutations, written
...
//! own production code exports the bridges instead: `decode_curation_snapshot_json`/
//! `encode_curation_snapshot_json`/`parse_curation_dsl`/`print_curation_dsl`/`curation_selection_summary`
...
mod subject {
    ...
    fn uri_in(ctx: &Context, needle: &str) -> Result<String, String> {
        ctx.scenario
            ...
            .find(|token| (token.starts_with("asset://") || token.starts_with("shared://🗂️mutate-curation-1/") || token.starts_with("shared://")) && token.contains(needle))
    ...
    fn snapshot_of(text: &str, label: &str, kind: &str) -> Result<CurationSnapshot, String> {
        decode_curation_snapshot_json(text).map_err(|error| format!("mutate-curation-1: the {label} snapshot for {kind:?} must decode: {error}"))
    }

    fn mutation_of(text: &str, kind: &str) -> Result<SourcingMutation, String> {
        decode_sourcing_mutation_json(text).map_err(|error| format!("mutate-curation-1: the mutation payload for {kind:?} must decode: {error}"))
    }

    /// 📤️ What parity compares: the whole document this subset's own JSON codec writes. Every member
    /// of `CurationSnapshot` is plain content — this artifact composes no digest-derived child — so
    /// nothing has to be held back from the comparison.
    fn projection(snapshot: &CurationSnapshot) -> Result<Json, String> {
        parse_json(&encode_curation_snapshot_json(snapshot))
    }

    fn disagreement(what: &str, got: &CurationSnapshot, expected: &CurationSnapshot) -> String {
        format!("{what}\n     got: [{}] {}\nexpected: [{}] {}", curation_selection_summary(got), encode_curation_snapshot_json(got), curation_selection_summary(expected), encode_curation_snapshot_json(expected))
    }
    //#endregion 🔖️Plan
    ...
    fn effect_holds(scenario: &str, effect: &str, before: &CurationSnapshot, after: &CurationSnapshot) -> Result<(), String> {
    ...
    fn outcome_matches(kind: &str, declared: &Json, raised: &[(String, String)]) -> Result<(), String> {
        if declared.str("status") != "applied" {
            return Err(format!("spec-vector-{kind}: every committed curation vector is a clean applied vector, but its outcome declares {:?}", declared.str("status")));
        }
        if !raised.is_empty() {
    ...
    /// 🎯️ Every committed curation vector declares the same clean applied outcome, so the claim is
    ...
    /// example: `.curation.dsl.semio` is a fixed-layout document with no writer freedom and the
    /// committed example is this codec's own output, so the re-printed text is required to reproduce
    /// it byte for byte. THIS CURRENTLY FAILS — `parse_curation_dsl` rejects the committed example —
    ...
    pub fn round_trip(ctx: &Context) -> Result<Outcome, String> {
        ...
        let reread = snapshot_of(&encode_curation_snapshot_json(&derived), "re-encoded", "identity-round-trip")?;
        if reread != derived {
        ...
        let parsed = parse_curation_dsl(&text)?;
        if parsed.stock_extra.len() != 10 || !parsed.curated.is_empty() {
            return Err(format!("identity-round-trip: the committed carrier carries ten stock entries against an empty curation, but parsed {} stock entr(ies) and [{}]", parsed.stock_extra.len(), curation_selection_summary(&parsed)));
        }
        let printed = print_curation_dsl(&parsed);
        let reparsed = parse_curation_dsl(&printed)?;
        if reparsed != parsed {
        ...
        if printed != text {
            ...
            return Err(format!(
                "exact-bytes law violated: `.curation.dsl.semio` is a fixed-layout document and the committed example is this codec's own output, so the re-printed text was required to reproduce it — {} byte(s) out against {} byte(s) in{}",
                printed.len(),
                text.len(),
                match at {
