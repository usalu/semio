...
fn compose_export_glb(sources: &[ErasedComposeSource]) -> Result<ComposedArtifact, ComposeError> {
    let snapshot = rebuild_native_snapshot(sources)?;
    let text = crate::artifacts::cad::io::export::serializers::artifacts::glb::v2_0::any::serialize_text(&snapshot).map_err(|e| ComposeError { message: e.to_string(), diagnostics: Vec::new() })?;
    Ok(ComposedArtifact { dialect: EXPORT_GLB_DIALECT, payload: IoPayload::Text(text), diagnostics: Vec::new(), confidence: IoConfidence::Medium })
}
...
pub fn entries() -> &'static [ComposerEntry] {
    ENTRIES.get_or_init(|| vec![
        ...
        ComposerEntry { writes: EXPORT_GLB_DIALECT, reads: &[CAD_DIALECT], compose: compose_export_glb },
