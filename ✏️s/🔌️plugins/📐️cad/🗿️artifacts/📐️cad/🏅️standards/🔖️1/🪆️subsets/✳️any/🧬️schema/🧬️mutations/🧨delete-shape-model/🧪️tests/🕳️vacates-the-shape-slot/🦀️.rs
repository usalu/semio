//! 🧪️ `delete-shape-model` fixture — `🕳️vacates-the-shape-slot`.
//!
...
fn expected_after() -> CadSnapshot {
    dsl::json::from_json_str(AFTER).expect("delete-shape-model/vacates-the-shape-slot: after snapshot decodes")
}
fn mutation() -> CadMutation {
    dsl::json::from_json_str(MUTATION).expect("delete-shape-model/vacates-the-shape-slot: mutation decodes")
}
...
async fn inverse_reinstalls_the_escrowed_shape_handle() {
    ...
    match &inverse[0] {
        CadMutation::CreateShapeModel(step) => {
            assert_eq!(step.child_id, "cad-shape-1", "the inverse must reinstall the escrowed child id");
