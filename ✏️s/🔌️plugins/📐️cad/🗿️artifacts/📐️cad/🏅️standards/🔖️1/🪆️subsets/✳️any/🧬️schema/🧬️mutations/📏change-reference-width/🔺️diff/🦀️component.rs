...
pub async fn diff(payload: &ChangeReferenceWidth, base: &CadSnapshot) -> protocol::MutationOutcome<CadDiff> {
    ...
    if !payload.new_width_world.is_finite() || payload.new_width_world <= 0.0 {
        return protocol::MutationOutcome::fatal("mutation.invariant", format!("Reference \"{}\" width must be finite and positive, got {}.", payload.reference_id, payload.new_width_world), [payload.model_definition_id.clone(), payload.reference_id.clone()]);
    }
