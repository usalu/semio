//! 🧪️ `delete-building-model` fixture — `🏚️vacates-the-building-slot`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/💥delete-building-model/🏚️vacates-the-building-slot/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/💥delete-building-model/🏚️vacates-the-building-slot/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/💥delete-building-model/🏚️vacates-the-building-slot/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/💥delete-building-model/🏚️vacates-the-building-slot/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/💥delete-building-model/🏚️vacates-the-building-slot/🎯️outcome/🔣️.json");

fn before() -> CadSnapshot {
    dsl::json::from_json_str(BEFORE).expect("delete-building-model/vacates-the-building-slot: before snapshot decodes")
}
fn expected_after() -> CadSnapshot {
    dsl::json::from_json_str(AFTER).expect("delete-building-model/vacates-the-building-slot: after snapshot decodes")
}
fn mutation() -> CadMutation {
    dsl::json::from_json_str(MUTATION).expect("delete-building-model/vacates-the-building-slot: mutation decodes")
}
...
async fn inverse_reinstalls_the_escrowed_building_handle() {
    ...
    match &inverse[0] {
        CadMutation::CreateBuildingModel(step) => {
            assert_eq!(step.child_id, "cad-building-1", "the inverse must reinstall the escrowed child id");
...
async fn committed_diff_applies_to_after() {
    ...
    let decoded: crate::diff::CadDiff = dsl::json::from_json_str(DIFF).expect("committed diff decodes into the artifact's diff type");
    assert_eq!(decoded, crate::diff::CadDiff::default(), "delete-building-model/vacates-the-building-slot: a `null` buildingModel is indistinguishable from an untouched one, so the decoded diff is empty");
