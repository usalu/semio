//! 🧪️ `replace-reference-media` fixture — `🖼️reattaches-the-shape-reference-to-a-new-plan`.
//!
//! Proves the five-field media bundle is replaced atomically — and that a `null` orientation LEAVES the existing one alone rather than clearing it.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🖇️replace-reference-media/🖼️reattaches-the-shape-reference-to-a-new-plan/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🖇️replace-reference-media/🖼️reattaches-the-shape-reference-to-a-new-plan/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🖇️replace-reference-media/🖼️reattaches-the-shape-reference-to-a-new-plan/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🖇️replace-reference-media/🖼️reattaches-the-shape-reference-to-a-new-plan/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🖇️replace-reference-media/🖼️reattaches-the-shape-reference-to-a-new-plan/🎯️outcome/🔣️.json");

fn before() -> CadSnapshot {
    dsl::json::from_json_str(BEFORE).expect("replace-reference-media/reattaches-the-shape-reference-to-a-new-plan: before snapshot decodes")
}
fn expected_after() -> CadSnapshot {
    dsl::json::from_json_str(AFTER).expect("replace-reference-media/reattaches-the-shape-reference-to-a-new-plan: after snapshot decodes")
}
fn mutation() -> CadMutation {
    dsl::json::from_json_str(MUTATION).expect("replace-reference-media/reattaches-the-shape-reference-to-a-new-plan: mutation decodes")
}
...
async fn swaps_the_media_bundle_and_leaves_placement_alone() {
    let after = applied();
    ...
    assert_eq!(after, expected_after(), "replace-reference-media/reattaches-the-shape-reference-to-a-new-plan: applied state differs from the committed after-snapshot");
}

/// ↩️ The inverse is a `replace-reference-media` carrying all five BASE media fields.
#[semio_framework_async_macros::async_test]
async fn inverse_reattaches_the_original_plan_bundle() {
    let base = before();
    let inverse = mutation().inverse(&base);
    assert_eq!(inverse.len(), 1, "replace-reference-media inverts to exactly one step");
    match &inverse[0] {
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-reference-media/reattaches-the-shape-reference-to-a-new-plan: committed {label} JSON is not canonical");
    }
    ...
    assert_eq!(reencoded, original, "replace-reference-media/reattaches-the-shape-reference-to-a-new-plan: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome matches what `replace-reference-media`'s own diff builder actually produces.
#[semio_framework_async_macros::async_test]
async fn declared_outcome_holds() {
    let declared: serde_json::Value = serde_json::from_str(OUTCOME).expect("outcome decodes");
    assert_eq!(declared.get("status").and_then(serde_json::Value::as_str), Some("applied"), "replace-reference-media/reattaches-the-shape-reference-to-a-new-plan: this fixture declares an applied outcome");
    let base = before();
    let produced = mutation().diff(&base);
    assert!(produced.messages().is_empty(), "replace-reference-media/reattaches-the-shape-reference-to-a-new-plan: declared clean-applied but the diff builder reported {:?}", produced.messages());
    let map = produced.diff().references_by_model_definition_id.as_ref().expect("replace-reference-media fills the references map");
...
/// 🔺️ The sparse delta `replace-reference-media` produces is exactly the committed diff — the most load-bearing
/// assertion in the fixture, because it pins WHICH fields the mutation may touch, not merely that the
/// end state matches. Here the emitted row shows `orientation` still null — the patch's `orientation: None` means "unchanged", so the media bundle can never CLEAR an orientation.
#[semio_framework_async_macros::async_test]
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-reference-media/reattaches-the-shape-reference-to-a-new-plan: replace-reference-media must emit one bucket whose row keeps the placement fields and the untouched orientation");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "replace-reference-media/reattaches-the-shape-reference-to-a-new-plan: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed diff directly to `before` yields `after` — the diff is a complete
/// description of the change `replace-reference-media` makes, not a summary of it.
#[semio_framework_async_macros::async_test]
