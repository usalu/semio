    ...
    pub async fn handle(payload: &PatchCadPlayReference, doc: &ArtifactView<'_, CadSnapshot>, _cfg: &ConfigView<'_, CadConfig>, _ctx: &mut CadDispatchCtx<'_>) -> Result<Emit<CadMutation, CadConfigMutation>, Fault> {
        ...
        let mutation = match payload.field.as_str() {
            ...
            "widthWorld" => {
                ...
                resolve_number_edit(current, value_json.as_ref(), delta_json.as_ref())
                    .map(|new_width_world| CadMutation::ChangeReferenceWidth(ChangeReferenceWidth { model_definition_id: payload.model_definition_id.clone(), reference_id: payload.reference_id.clone(), new_width_world }))
            }
