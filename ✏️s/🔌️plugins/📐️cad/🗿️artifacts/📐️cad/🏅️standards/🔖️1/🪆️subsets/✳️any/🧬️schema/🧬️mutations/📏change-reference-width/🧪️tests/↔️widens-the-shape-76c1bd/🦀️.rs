...
fn before() -> CadSnapshot {
    dsl::json::from_json_str(BEFORE).expect("change-reference-width/widens-the-shape-reference-plane: before snapshot decodes")
}
fn expected_after() -> CadSnapshot {
    dsl::json::from_json_str(AFTER).expect("change-reference-width/widens-the-shape-reference-plane: after snapshot decodes")
}
fn mutation() -> CadMutation {
    dsl::json::from_json_str(MUTATION).expect("change-reference-width/widens-the-shape-reference-plane: mutation decodes")
}
