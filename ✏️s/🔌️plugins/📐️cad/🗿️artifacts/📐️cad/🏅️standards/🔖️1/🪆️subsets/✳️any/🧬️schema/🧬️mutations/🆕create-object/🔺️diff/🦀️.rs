...
pub fn diff(payload: &CreateObject, base: &CadSnapshot) -> protocol::MutationOutcome<CadDiff> {
    ...
    // 🪆️ A pane with no materialized child yet still admits its first object — the empty scene is the
    // honest base, never a fabricated one (the other three panes keep their own handles and owners).
    let scene = crate::cad_pane_local_scene(base, payload.pane).unwrap_or_else(|| std::sync::Arc::new(CadWorkingScene::default()));
    ...
    objects.insert(index, cad_object_from_spec(&payload.object));
