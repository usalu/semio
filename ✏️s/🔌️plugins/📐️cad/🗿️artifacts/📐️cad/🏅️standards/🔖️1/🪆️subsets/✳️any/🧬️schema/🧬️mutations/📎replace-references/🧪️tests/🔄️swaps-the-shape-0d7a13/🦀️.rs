...
fn before() -> CadSnapshot {
    dsl::json::from_json_str(BEFORE).expect("replace-references/swaps-the-shape-reference-list: before snapshot decodes")
}
fn expected_after() -> CadSnapshot {
    dsl::json::from_json_str(AFTER).expect("replace-references/swaps-the-shape-reference-list: after snapshot decodes")
}
fn mutation() -> CadMutation {
    dsl::json::from_json_str(MUTATION).expect("replace-references/swaps-the-shape-reference-list: mutation decodes")
}
