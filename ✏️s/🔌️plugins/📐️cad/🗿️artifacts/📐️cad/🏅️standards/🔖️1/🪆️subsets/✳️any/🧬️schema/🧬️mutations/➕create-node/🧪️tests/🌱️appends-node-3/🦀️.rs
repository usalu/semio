...
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕create-node/🌱️appends-node-3/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕create-node/🌱️appends-node-3/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕create-node/🌱️appends-node-3/🎯️outcome/🔣️.json");

fn before() -> CadSnapshot {
    dsl::json::from_json_str(BEFORE).expect("create-node/appends-node-3: before snapshot decodes")
}
fn expected_after() -> CadSnapshot {
    dsl::json::from_json_str(AFTER).expect("create-node/appends-node-3: after snapshot decodes")
}
fn mutation() -> CadMutation {
    dsl::json::from_json_str(MUTATION).expect("create-node/appends-node-3: mutation decodes")
}
