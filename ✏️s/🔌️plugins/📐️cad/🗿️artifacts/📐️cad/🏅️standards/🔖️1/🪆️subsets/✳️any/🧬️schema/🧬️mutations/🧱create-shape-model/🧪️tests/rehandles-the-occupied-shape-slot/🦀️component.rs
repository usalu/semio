//! 🧪️ `create-shape-model` fixture — `rehandles-the-occupied-shape-slot`.
//!
...
fn before() -> CadSnapshot {
    serde_json::from_str(BEFORE).expect("create-shape-model/rehandles-the-occupied-shape-slot: before snapshot decodes")
}
fn expected_after() -> CadSnapshot {
    serde_json::from_str(AFTER).expect("create-shape-model/rehandles-the-occupied-shape-slot: after snapshot decodes")
}
fn mutation() -> CadMutation {
    serde_json::from_str(MUTATION).expect("create-shape-model/rehandles-the-occupied-shape-slot: mutation decodes")
}
...
async fn replaces_the_shape_handle_in_place() {
    ...
    assert!(after.building_model.as_ref().map(|c| c.child_id.as_str()) == Some("building-model-1") && after.energy_model.as_ref().map(|c| c.child_id.as_str()) == Some("energy-model-1") && after.structure_classic_model.as_ref().map(|c| c.child_id.as_str()) == Some("structure-classic-model-1"), "create-shape-model must leave the other three fixed model slots untouched");
