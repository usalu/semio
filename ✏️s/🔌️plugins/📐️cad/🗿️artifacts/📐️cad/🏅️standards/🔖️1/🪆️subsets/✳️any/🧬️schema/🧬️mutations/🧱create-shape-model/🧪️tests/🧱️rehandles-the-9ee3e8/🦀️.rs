//! 🧪️ `create-shape-model` fixture — `🧱️rehandles-the-occupied-shape-slot`.
//!
...
fn before() -> CadSnapshot {
    dsl::json::from_json_str(BEFORE).expect("create-shape-model/rehandles-the-occupied-shape-slot: before snapshot decodes")
}
fn expected_after() -> CadSnapshot {
    dsl::json::from_json_str(AFTER).expect("create-shape-model/rehandles-the-occupied-shape-slot: after snapshot decodes")
}
fn mutation() -> CadMutation {
    dsl::json::from_json_str(MUTATION).expect("create-shape-model/rehandles-the-occupied-shape-slot: mutation decodes")
}
fn applied() -> CadSnapshot {
    let base = before();
    mutation().diff(&base).diff().apply(&base).expect("create-shape-model applies to its committed before-snapshot")
}
...
async fn inverse_reinstalls_the_displaced_shape_handle() {
    ...
    for step in &inverse {
        snapshot = step.diff(&snapshot).diff().apply(&snapshot).expect("create-shape-model/rehandles-the-occupied-shape-slot: inverse step applies");
    }
