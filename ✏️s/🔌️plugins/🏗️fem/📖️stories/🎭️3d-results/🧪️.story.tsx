// #region 🧲️Header
// 💻️ ✏️s/🔌️plugins/🏗️fem/📖️stories/🎭️3d-results/🧪️.story.tsx
...
// exact Results window result-display triple `config_result_display` maps onto `DisplayMode`.
...
function Fem3dResultsStoryHost({ initialExampleId, locale, initialMode }: { readonly initialExampleId: string; readonly locale: FemStoryLocale; readonly initialMode: string }): ReactElement {
  ...
  const node = useMemo(() => buildFem3dSceneNode(instances, state.camera, FEM3D_RESULTS_BODY_KEY, FEM3D_EDITOR_CONTROLLER_ID), [instances, state.camera]);
  const debug = useMemo(
  ...
  return (
    <div style={{ display: "flex", height: "100%", width: "100%", minHeight: "24rem", flexDirection: "column" }}>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 4, padding: 4 }}>
        ...
        <button type="button" data-testid="fem3d-results-clear-example" onClick={() => dispatch("setActiveExample", { exampleId: FEM3D_CLEARED_EXAMPLE_ID })}>
...
export const NoLoadCase: Story = {
  args: { initialExampleId: FEM3D_CLEARED_EXAMPLE_ID, locale: "en-US", initialMode: "static" },
};
