// #region 🧲️Header
// 💻️ ✏️s/🔌️plugins/🏗️fem/📖️stories/🎭️2d-model/🧪️.story.tsx
...
// A story-local reducer mirrors fem2d's own tool roster (`setActiveExample`, `addNode`) so the
    ...
    <div style={{ display: "flex", height: "100%", width: "100%", flexDirection: "column" }}>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 4, padding: 4 }}>
        ...
        {FEM_STORY_LOCALES.map((tag) => (
          <button key={tag} type="button" data-testid={`fem2d-set-locale-${tag}`} aria-pressed={state.locale === tag} onClick={() => setState((current) => ({ ...current, locale: tag }))}>
...
/** 🇩🇪️ The same document with every panel label resolved through the story host's explicit `de-DE` OS locale. */
