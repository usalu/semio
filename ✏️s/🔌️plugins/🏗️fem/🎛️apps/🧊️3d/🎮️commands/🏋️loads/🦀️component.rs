...
fn resolve_load_case(doc: &Fem3dDocument, case_id: Option<&str>) -> (usize, FemLoadCase) {
...
mod tests {
    ...
    fn add_member_udl_action_emits_op_3d() {
        ...
        let load_case = &projection.load_cases[0];
    ...
    fn add_combination_parses_terms_json() {
        ...
        let projection = app.projection().expect("projection");
        assert_eq!(projection.combinations.len(), 1);
        assert_eq!(projection.combinations[0].terms.get("case-0"), Some(&1.35));
    }
    ...
    fn add_combination_invalid_terms_json_is_a_no_op() {
        ...
        assert!(app.projection().expect("projection").combinations.is_empty());
    }
}
// #endregion 🧪️Tests
