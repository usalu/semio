...
//!
//! 🔁️ The dead case already carries self-weight, so this is `change-load-case-self-weight`'s no-op branch: a NO-OP with a Warning, an empty diff, and a document that does not move. `change-load-case-self-weight` has no Fatal branch at all.

use crate::standards::v1::subsets::any::schema::mutations::Fem2dMutation;
use crate::standards::v1::subsets::any::schema::mutations::{apply_fem2d_mutation, inverse_fem2d_mutation};
use crate::Fem2dSnapshot;

const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚖️change-load-case-self-weight/🔁️keeps-self-weight-ff696b/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚖️change-load-case-self-weight/🔁️keeps-self-weight-ff696b/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚖️change-load-case-self-weight/🔁️keeps-self-weight-ff696b/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚖️change-load-case-self-weight/🔁️keeps-self-weight-ff696b/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚖️change-load-case-self-weight/🔁️keeps-self-weight-ff696b/🎯️outcome/🔣️.json");
...
/// ▶️ A no-op `change-load-case-self-weight` is a NO-OP, not rejected — it simply changes nothing, so the document comes
...
fn no_op_leaves_the_document_untouched() {
    ...
    assert_eq!(snapshot, base, "change-load-case-self-weight/keeps-self-weight-ff696b: an accepted no-op still leaves the document exactly where it was");
}
...
fn the_no_op_is_a_warning_not_a_rejection() {
    ...
    assert_eq!(produced.diff(), &crate::standards::v1::subsets::any::schema::diff::Fem2dDiff::default(), "change-load-case-self-weight/keeps-self-weight-ff696b: a no-op change-load-case-self-weight must carry the empty diff");
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("no-op"), "change-load-case-self-weight/keeps-self-weight-ff696b declares a no-op outcome");
    let produced = <Fem2dMutation as protocol::Mutation<Fem2dSnapshot>>::diff(&mutation(), &before());
...
fn produces_committed_diff() {
    ...
    assert_eq!(typed, crate::standards::v1::subsets::any::schema::diff::Fem2dDiff::default(), "change-load-case-self-weight/keeps-self-weight-ff696b: a no-op delta is the artifact's Default diff");
}
