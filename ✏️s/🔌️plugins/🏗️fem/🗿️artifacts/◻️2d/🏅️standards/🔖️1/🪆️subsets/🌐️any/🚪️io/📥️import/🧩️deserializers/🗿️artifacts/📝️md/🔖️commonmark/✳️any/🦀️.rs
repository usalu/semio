//! 🚪️ fem2d ← md — foreign `Deserializer<Fem2dSnapshot>` on the framework's `io_mechanism`
//! channel, the exact inverse of the sibling `📤️export` leaf's fenced-code-block envelope
//! (`IoFidelity::Exact`). The `fem2d`-tagged fence wins; an untagged fence is accepted as a
...
pub fn from_md_text(text: &str) -> Result<Fem2dSnapshot, IoError> {
    let snapshot = MdSnapshot::from_text(text);
    let literal = fenced_literal(&snapshot).ok_or_else(|| IoError { message: format!("md→fem2d: not a fem2d envelope — no `{FENCE_INFO}` fenced code block and no untagged one to fall back on"), diagnostics: Vec::new() })?;
    <Fem2dSnapshot as store::ArtifactDsl>::parse_dsl(literal).map_err(|error| IoError { message: format!("md→fem2d: {error}"), diagnostics: Vec::new() })
}

/// 🧩️ `s.stdio.md@commonmark/*` → `s.fem.fem2d@1/*`.
pub struct MdIntoFem2d;

impl Deserializer<Fem2dSnapshot> for MdIntoFem2d {
    const FROM: Dialect = MD_DIALECT;
    const FIDELITY: IoFidelity = IoFidelity::Exact;
    async fn sniff(payload: &IoPayload) -> Confidence {
        match payload {
            IoPayload::Text(text) if text.contains("```fem2d") => Confidence::High,
    ...
    async fn deserialize(payload: &IoPayload) -> IoResult<Fem2dSnapshot> {
        let IoPayload::Text(text) = payload else {
            return Err(IoError { message: "md→fem2d: expected a text commonmark payload".to_string(), diagnostics: Vec::new() });
        };
