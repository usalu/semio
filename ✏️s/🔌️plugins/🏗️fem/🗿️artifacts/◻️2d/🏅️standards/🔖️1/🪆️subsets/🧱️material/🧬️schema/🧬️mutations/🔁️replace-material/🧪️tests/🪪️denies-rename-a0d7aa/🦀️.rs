//! 🧪️ `replace-material` fixture — `🪪️denies-rename-a0d7aa`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-material/🪪️denies-rename-a0d7aa/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-material/🪪️denies-rename-a0d7aa/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-material/🪪️denies-rename-a0d7aa/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-material/🪪️denies-rename-a0d7aa/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_untouched() {
    ...
    assert_eq!(snapshot, expected_after(), "replace-material/denies-rename-a0d7aa: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "replace-material/denies-rename-a0d7aa: a refused mutation must leave the snapshot exactly where it was");
}
...
/// ↩️ `replace-material`'s inverse is BASE-derived: it restores the material `before` holds, addressed by the SELECTED id.
#[test]
fn inverse_of_the_refused_mutation() {
    let inverse = inverse_fem2d_mutation(&before(), &mutation());
    assert_eq!(inverse.len(), 1, "replace-material/denies-rename-a0d7aa: replace-material undoes with exactly one step, got {inverse:?}");
    let Fem2dMutation::ReplaceMaterial(undo) = &inverse[0] else {
        panic!("replace-material's inverse must be a ReplaceMaterial, got {:?}", inverse[0]);
    };
    assert_eq!(undo.id, "concrete_c30", "the inverse addresses the selected id, not the one the replacement carried");
    assert_eq!(&undo.new_material, before().materials.iter().find(|item| item.id == "concrete_c30").expect("the target survives a refusal"), "the inverse restores the pre-mutation material");
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "replace-material/denies-rename-a0d7aa declares a rejected outcome");
    let produced = <Fem2dMutation as protocol::Mutation<Fem2dSnapshot>>::diff(&mutation(), &before());
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-material/denies-rename-a0d7aa: committed {label} JSON is not canonical");
    }
    assert_eq!(BEFORE, AFTER, "replace-material/denies-rename-a0d7aa changes nothing: the two committed snapshots must be byte-identical");
    let decoded_mutation = mutation();
    let reencoded = dsl::ToValue::to_value(&decoded_mutation);
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "replace-material/denies-rename-a0d7aa: committed mutation JSON is not canonical");
}
