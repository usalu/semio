//! 🧪️ `delete-node` fixture — `⛔️rejects-a-missing-429801`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕳️delete-node/⛔️rejects-a-missing-429801/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕳️delete-node/⛔️rejects-a-missing-429801/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕳️delete-node/⛔️rejects-a-missing-429801/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕳️delete-node/⛔️rejects-a-missing-429801/🎯️outcome/🔣️.json");
...
/// ▶️ A refused `delete-node` still applies cleanly — the refusal is carried as a diagnostic beside an
...
fn rejection_leaves_the_document_untouched() {
    ...
    assert_eq!(snapshot, expected_after(), "delete-node/rejects-a-missing-429801: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "delete-node/rejects-a-missing-429801: a refused mutation must leave the snapshot exactly where it was");
}
...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(semantics.kind, "delete-node", "the fixture must be bound to delete-node's own descriptor");
}

/// ↩️ `delete-node`'s inverse is BASE-derived: with no such record on `before` there is nothing to restore, so the inverse collapses to `Vec::new()`.
#[test]
fn inverse_of_the_refused_mutation() {
    let inverse = inverse_fem2d_mutation(&before(), &mutation());
    assert!(inverse.is_empty(), "delete-node/rejects-a-missing-429801: a target BASE never held leaves nothing to restore, got {inverse:?}");
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "delete-node/rejects-a-missing-429801 declares a rejected outcome");
    let produced = <Fem2dMutation as protocol::Mutation<Fem2dSnapshot>>::diff(&mutation(), &before());
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "delete-node/rejects-a-missing-429801: committed {label} JSON is not canonical");
    }
    assert_eq!(BEFORE, AFTER, "delete-node/rejects-a-missing-429801 changes nothing: the two committed snapshots must be byte-identical");
    let decoded_mutation = mutation();
    let reencoded = dsl::ToValue::to_value(&decoded_mutation);
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "delete-node/rejects-a-missing-429801: committed mutation JSON is not canonical");
}
