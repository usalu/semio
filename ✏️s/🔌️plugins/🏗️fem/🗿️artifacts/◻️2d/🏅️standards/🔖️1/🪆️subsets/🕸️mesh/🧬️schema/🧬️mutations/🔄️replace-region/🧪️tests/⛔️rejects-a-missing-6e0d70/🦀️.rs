//! 🧪️ `replace-region` fixture — `⛔️rejects-a-missing-6e0d70`.
//!
...
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔄️replace-region/⛔️rejects-a-missing-6e0d70/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔄️replace-region/⛔️rejects-a-missing-6e0d70/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔄️replace-region/⛔️rejects-a-missing-6e0d70/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_untouched() {
    ...
    assert_eq!(snapshot, expected_after(), "replace-region/rejects-a-missing-6e0d70: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "replace-region/rejects-a-missing-6e0d70: a refused mutation must leave the snapshot exactly where it was");
}
...
/// ↩️ `replace-region`'s inverse is BASE-derived: with no such record on `before` there is nothing to restore, so the inverse collapses to `Vec::new()`.
#[test]
fn inverse_of_the_refused_mutation() {
    let inverse = inverse_fem2d_mutation(&before(), &mutation());
    assert!(inverse.is_empty(), "replace-region/rejects-a-missing-6e0d70: a target BASE never held leaves nothing to restore, got {inverse:?}");
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "replace-region/rejects-a-missing-6e0d70 declares a rejected outcome");
    let produced = <Fem2dMutation as protocol::Mutation<Fem2dSnapshot>>::diff(&mutation(), &before());
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-region/rejects-a-missing-6e0d70: committed {label} JSON is not canonical");
    }
    assert_eq!(BEFORE, AFTER, "replace-region/rejects-a-missing-6e0d70 changes nothing: the two committed snapshots must be byte-identical");
    let decoded_mutation = mutation();
    let reencoded = dsl::ToValue::to_value(&decoded_mutation);
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "replace-region/rejects-a-missing-6e0d70: committed mutation JSON is not canonical");
}
