...
fn admitted_document_mutations_make_bounded_progress_and_retire() {
    ...
    assert_eq!(factory.preflight(&mutation, None, store::HistoryLane::Document).expect("document admission").work_items, store::ARTIFACT_STORE_ONE_ITEM_INVERTIBLE_WORK_ITEMS, "one forward row plus its single inverse row");
