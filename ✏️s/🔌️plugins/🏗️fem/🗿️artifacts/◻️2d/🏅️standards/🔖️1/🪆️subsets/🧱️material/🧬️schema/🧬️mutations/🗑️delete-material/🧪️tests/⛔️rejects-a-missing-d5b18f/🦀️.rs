//! 🧪️ `delete-material` fixture — `⛔️rejects-a-missing-d5b18f`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️delete-material/⛔️rejects-a-missing-d5b18f/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️delete-material/⛔️rejects-a-missing-d5b18f/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️delete-material/⛔️rejects-a-missing-d5b18f/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️delete-material/⛔️rejects-a-missing-d5b18f/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_untouched() {
    ...
    assert_eq!(snapshot, expected_after(), "delete-material/rejects-a-missing-d5b18f: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "delete-material/rejects-a-missing-d5b18f: a refused mutation must leave the snapshot exactly where it was");
}
...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(produced.diff(), &crate::standards::v1::subsets::any::schema::diff::Fem2dDiff::default(), "delete-material/rejects-a-missing-d5b18f: a rejecting delete-material must carry the empty diff, never a half-built delta");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.target-missing", "delete-material/rejects-a-missing-d5b18f: the refusal is reported as mutation.target-missing");
...
/// ↩️ `delete-material`'s inverse is BASE-derived: with no such record on `before` there is nothing to restore, so the inverse collapses to `Vec::new()`.
#[test]
fn inverse_of_the_refused_mutation() {
    let inverse = inverse_fem2d_mutation(&before(), &mutation());
    assert!(inverse.is_empty(), "delete-material/rejects-a-missing-d5b18f: a target BASE never held leaves nothing to restore, got {inverse:?}");
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "delete-material/rejects-a-missing-d5b18f declares a rejected outcome");
    let produced = <Fem2dMutation as protocol::Mutation<Fem2dSnapshot>>::diff(&mutation(), &before());
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "delete-material/rejects-a-missing-d5b18f: committed {label} JSON is not canonical");
    }
    assert_eq!(BEFORE, AFTER, "delete-material/rejects-a-missing-d5b18f changes nothing: the two committed snapshots must be byte-identical");
    let decoded_mutation = mutation();
    let reencoded = dsl::ToValue::to_value(&decoded_mutation);
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "delete-material/rejects-a-missing-d5b18f: committed mutation JSON is not canonical");
}
