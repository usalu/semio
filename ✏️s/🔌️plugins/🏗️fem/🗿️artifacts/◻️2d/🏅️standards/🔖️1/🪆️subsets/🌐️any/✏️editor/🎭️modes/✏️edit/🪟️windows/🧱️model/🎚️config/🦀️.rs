//! 🎚️ Persisted local state for one exact FEM 2D model window.

#[path = "🧬️schema/🦀️.rs"]
mod schema;
pub use schema::*;

impl Default for Fem2dModelWindowConfig {
    fn default() -> Self {
        Self { camera: crate::Viewport2d::default() }
    }
}

impl store::ArtifactDsl for Fem2dModelWindowConfig {
    const EXTENSION: &'static str = "fem2dmodelwindowcfg";
...
impl store::ArtifactPack for Fem2dModelWindowConfig {
...
store::impl_whole_record_config!(Fem2dModelWindowConfig);

#[derive(Clone, Debug, PartialEq, semio_framework_value_derive::ToValue, semio_framework_value_derive::FromValue, dsl::DslOps)]
pub enum Fem2dModelWindowConfigMutation {
    #[dsl(key = "snapshot")]
    Snapshot {
        #[dsl(block)]
        config: Box<Fem2dModelWindowConfig>,
    },
}

impl protocol::OpText for Fem2dModelWindowConfigMutation {
...
impl protocol::OpBinary for Fem2dModelWindowConfigMutation {
...
impl protocol::Mutation<Fem2dModelWindowConfig> for Fem2dModelWindowConfigMutation {
    type Diff = Fem2dModelWindowConfig;
    const DESCRIPTORS: &'static [protocol::MutationLeafDescriptor] = &[protocol::MutationLeafDescriptor {
        schema_version: 1,
        owner: "✏️s/🔌️plugins/🏗️fem/🗿️artifacts/◻️2d/🏅️standards/🔖️1/🪆️subsets/🌐️any/✏️editor/🎭️modes/✏️edit/🪟️windows/🧱️model/🎚️config",
        semantic_kind: "set-window-config",
        display_name: "Set FEM 2D Model Window Configuration",
        emoji: "🎚️",
        aggregate_variant: "Snapshot",
        payload_schema: "fem.2d.modelwindowconfig",
        text_opcode: None,
        binary_tag: None,
    ...
    fn diff(&self, base: &Fem2dModelWindowConfig) -> protocol::MutationOutcome<Self::Diff> {
        match self {
    ...
    fn inverse(&self, base: &Fem2dModelWindowConfig) -> Vec<Self> {
        vec![Self::Snapshot { config: Box::new(base.clone()) }]
    }
}

pub struct Fem2dModelWindowConfigOwner;

impl semio_framework_plugin::WindowConfigOwner for Fem2dModelWindowConfigOwner {
    const WINDOW_KIND_ID: &'static str = super::WINDOW_KIND_ID;
    const SCHEMA: &'static str = "fem.2d.modelwindowconfig";
    const MAXIMUM_PUBLICATION_BYTES: usize = 16_384;
    type State = Fem2dModelWindowConfig;
    type Mutation = Fem2dModelWindowConfigMutation;
...
pub fn current<C>(view: &semio_framework_plugin::ConfigView<'_, C>) -> Fem2dModelWindowConfig {
    view.window::<Fem2dModelWindowConfigOwner>().cloned().unwrap_or_default()
}

pub fn addressed(view: &semio_framework_plugin::ViewModel, config: Fem2dModelWindowConfig) -> Result<semio_framework_plugin::WindowConfigMutation, semio_framework_plugin::Fault> {
    ...
    if kind != <Fem2dModelWindowConfigOwner as semio_framework_plugin::WindowConfigOwner>::WINDOW_KIND_ID {
        return Err(semio_framework_plugin::Fault::from("fem.window.kind: addressed window has the wrong kind"));
    }
    Ok(semio_framework_plugin::WindowConfigMutation::of::<Fem2dModelWindowConfigOwner>(id, Fem2dModelWindowConfigMutation::Snapshot { config: Box::new(config) }))
}
