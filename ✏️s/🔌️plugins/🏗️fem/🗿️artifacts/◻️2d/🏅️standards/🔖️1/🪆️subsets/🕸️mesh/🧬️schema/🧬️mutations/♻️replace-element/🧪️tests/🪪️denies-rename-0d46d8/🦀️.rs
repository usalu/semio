...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(produced.diff(), &crate::artifacts::fem2d::diff::Fem2dDiff::default(), "replace-element/denies-rename-0d46d8: a rejecting replace-element must carry the empty diff, never a half-built delta");
...
fn inverse_of_the_refused_mutation() {
    ...
    assert_eq!(undo.new_element.as_ref(), before().elements.iter().find(|item| crate::artifacts::fem2d::element_id(item) == "b2").expect("the target survives a refusal"), "the inverse restores the pre-mutation element");
}
