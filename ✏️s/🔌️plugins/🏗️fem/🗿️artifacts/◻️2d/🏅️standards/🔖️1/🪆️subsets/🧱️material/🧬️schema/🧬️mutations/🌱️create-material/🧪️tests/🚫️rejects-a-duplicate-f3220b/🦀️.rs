//! 🧪️ `create-material` fixture — `🚫️rejects-a-duplicate-f3220b`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱️create-material/🚫️rejects-a-duplicate-f3220b/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱️create-material/🚫️rejects-a-duplicate-f3220b/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱️create-material/🚫️rejects-a-duplicate-f3220b/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱️create-material/🚫️rejects-a-duplicate-f3220b/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_untouched() {
    ...
    assert_eq!(snapshot, expected_after(), "create-material/rejects-a-duplicate-f3220b: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "create-material/rejects-a-duplicate-f3220b: a refused mutation must leave the snapshot exactly where it was");
}
...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(produced.diff(), &crate::standards::v1::subsets::any::schema::diff::Fem2dDiff::default(), "create-material/rejects-a-duplicate-f3220b: a rejecting create-material must carry the empty diff, never a half-built delta");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.duplicate-id", "create-material/rejects-a-duplicate-f3220b: the refusal is reported as mutation.duplicate-id");
    ...
    assert_eq!(messages[0].target, vec!["steel_s355".to_string()], "the diagnostic addresses the offending id");
...
fn inverse_of_the_refused_mutation() {
    ...
    assert_eq!(inverse.len(), 1, "create-material/rejects-a-duplicate-f3220b: create-material always undoes with exactly one step, got {inverse:?}");
    let Fem2dMutation::DeleteMaterial(undo) = &inverse[0] else {
        panic!("create-material's inverse must be a delete-material, got {:?}", inverse[0]);
    };
    assert_eq!(undo.id, "steel_s355", "the inverse addresses exactly the id the payload carried");
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "create-material/rejects-a-duplicate-f3220b declares a rejected outcome");
    let produced = <Fem2dMutation as protocol::Mutation<Fem2dSnapshot>>::diff(&mutation(), &before());
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "create-material/rejects-a-duplicate-f3220b: committed {label} JSON is not canonical");
    }
    assert_eq!(BEFORE, AFTER, "create-material/rejects-a-duplicate-f3220b changes nothing: the two committed snapshots must be byte-identical");
    let decoded_mutation = mutation();
    let reencoded = dsl::ToValue::to_value(&decoded_mutation);
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "create-material/rejects-a-duplicate-f3220b: committed mutation JSON is not canonical");
}
