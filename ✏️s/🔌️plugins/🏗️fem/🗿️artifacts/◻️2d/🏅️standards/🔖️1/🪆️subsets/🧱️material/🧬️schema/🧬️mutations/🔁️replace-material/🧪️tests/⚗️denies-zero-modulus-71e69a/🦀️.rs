//! 🧪️ `replace-material` fixture — `⚗️denies-zero-modulus-71e69a`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-material/⚗️denies-zero-modulus-71e69a/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-material/⚗️denies-zero-modulus-71e69a/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-material/⚗️denies-zero-modulus-71e69a/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-material/⚗️denies-zero-modulus-71e69a/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_untouched() {
    ...
    assert_eq!(snapshot, expected_after(), "replace-material/denies-zero-modulus-71e69a: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "replace-material/denies-zero-modulus-71e69a: a refused mutation must leave the snapshot exactly where it was");
}
...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(messages[0].target, vec!["concrete_c30".to_string()], "the diagnostic addresses the offending id");
...
fn inverse_of_the_refused_mutation() {
    ...
    assert_eq!(inverse.len(), 1, "replace-material/denies-zero-modulus-71e69a: replace-material undoes with exactly one step, got {inverse:?}");
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "replace-material/denies-zero-modulus-71e69a declares a rejected outcome");
    let produced = <Fem2dMutation as protocol::Mutation<Fem2dSnapshot>>::diff(&mutation(), &before());
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-material/denies-zero-modulus-71e69a: committed {label} JSON is not canonical");
    }
    assert_eq!(BEFORE, AFTER, "replace-material/denies-zero-modulus-71e69a changes nothing: the two committed snapshots must be byte-identical");
    let decoded_mutation = mutation();
    let reencoded = dsl::ToValue::to_value(&decoded_mutation);
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "replace-material/denies-zero-modulus-71e69a: committed mutation JSON is not canonical");
}
