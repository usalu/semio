...
Feature: Apply every typed fem2d mesh mutation twice — once in Rust, once in Python — and require the same answer
  ...
  Scenario Outline: Replay the committed <id> specification vector through both implementations
    ...
    | delete-node     | 🕳️delete-node     | 🚫️removes-node-n3-without-cascading-to-its-support |
    | create-element  | 🧩️create-element  | ➖️appends-bar-e2-between-n2-and-n3                 |
    | delete-element  | 🗑️delete-element  | 🚫️removes-bar-e2-and-keeps-its-end-nodes           |
    | replace-element | ♻️replace-element | ♻️converts-beam-e1-into-a-bar-in-place             |
    | create-section  | 📐️create-section    | 📐️appends-the-ipe300-profile                       |
    | delete-section  | ✂️delete-section  | 🚫️removes-the-spare-hollow-section                 |
    | replace-section | 📏️replace-section | 💪️stiffens-ipe200-with-a-reinforced-profile        |
    | create-region   | 🗺️create-region   | 🧱️appends-a-solid-rectangular-slab                 |
    | delete-region   | 🚫️delete-region   | 🚫️removes-the-slab-and-keeps-its-material          |
    | replace-region  | 🔄️replace-region  | 🪜️punches-a-stair-opening-through-the-slab         |
