//! 🧪️ `create-region` fixture — `🕳️denies-loose-hole-d9efa1`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗺️create-region/🕳️denies-loose-hole-d9efa1/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗺️create-region/🕳️denies-loose-hole-d9efa1/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗺️create-region/🕳️denies-loose-hole-d9efa1/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗺️create-region/🕳️denies-loose-hole-d9efa1/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_untouched() {
    ...
    assert_eq!(snapshot, expected_after(), "create-region/denies-loose-hole-d9efa1: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "create-region/denies-loose-hole-d9efa1: a refused mutation must leave the snapshot exactly where it was");
}
...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(produced.diff(), &crate::standards::v1::subsets::any::schema::diff::Fem2dDiff::default(), "create-region/denies-loose-hole-d9efa1: a rejecting create-region must carry the empty diff, never a half-built delta");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.invariant", "create-region/denies-loose-hole-d9efa1: the refusal is reported as mutation.invariant");
    ...
    assert_eq!(messages[0].target, vec!["panel_door".to_string()], "the diagnostic addresses the offending id");
...
fn inverse_of_the_refused_mutation() {
    ...
    assert_eq!(inverse.len(), 1, "create-region/denies-loose-hole-d9efa1: create-region undoes with exactly one step, got {inverse:?}");
    let Fem2dMutation::DeleteRegion(undo) = &inverse[0] else {
        panic!("create-region's inverse must be a DeleteRegion, got {:?}", inverse[0]);
    };
    assert_eq!(undo.id, "panel_door", "the inverse addresses exactly the id the payload carried");
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "create-region/denies-loose-hole-d9efa1 declares a rejected outcome");
    let produced = <Fem2dMutation as protocol::Mutation<Fem2dSnapshot>>::diff(&mutation(), &before());
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "create-region/denies-loose-hole-d9efa1: committed {label} JSON is not canonical");
    }
    assert_eq!(BEFORE, AFTER, "create-region/denies-loose-hole-d9efa1 changes nothing: the two committed snapshots must be byte-identical");
    let decoded_mutation = mutation();
    let reencoded = dsl::ToValue::to_value(&decoded_mutation);
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "create-region/denies-loose-hole-d9efa1: committed mutation JSON is not canonical");
}
