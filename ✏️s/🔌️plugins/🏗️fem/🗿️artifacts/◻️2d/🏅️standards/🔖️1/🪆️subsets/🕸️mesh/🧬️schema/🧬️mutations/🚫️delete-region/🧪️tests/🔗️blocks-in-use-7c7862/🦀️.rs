//! 🧪️ `delete-region` fixture — `🔗️blocks-in-use-7c7862`.
//!
...
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚫️delete-region/🔗️blocks-in-use-7c7862/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚫️delete-region/🔗️blocks-in-use-7c7862/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚫️delete-region/🔗️blocks-in-use-7c7862/🎯️outcome/🔣️.json");
...
/// ▶️ A refused `delete-region` still applies cleanly — the refusal is carried as a diagnostic beside an
...
fn rejection_leaves_the_document_untouched() {
    ...
    assert_eq!(snapshot, expected_after(), "delete-region/blocks-in-use-7c7862: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "delete-region/blocks-in-use-7c7862: a refused mutation must leave the snapshot exactly where it was");
}
...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(produced.diff(), &crate::standards::v1::subsets::any::schema::diff::Fem2dDiff::default(), "delete-region/blocks-in-use-7c7862: a rejecting delete-region must carry the empty diff, never a half-built delta");
    ...
    assert_eq!(messages[0].target, vec!["wall1".to_string(), "wind/lw2".to_string()], "the diagnostic addresses the target first, then every referrer");
    let semantics = <Fem2dMutation as protocol::SemanticMutation<Fem2dSnapshot>>::semantics(&mutation());
    assert_eq!(semantics.kind, "delete-region", "the fixture must be bound to delete-region's own descriptor");
}

/// ↩️ `delete-region`'s inverse is BASE-derived and the target survives the refusal, so the inverse is the `create-region` that would put the panel back.
#[test]
fn inverse_of_the_refused_mutation() {
    let inverse = inverse_fem2d_mutation(&before(), &mutation());
    assert_eq!(inverse.len(), 1, "delete-region/blocks-in-use-7c7862: delete-region undoes with exactly one step, got {inverse:?}");
    let Fem2dMutation::CreateRegion(undo) = &inverse[0] else {
        panic!("delete-region's inverse must be a CreateRegion, got {:?}", inverse[0]);
    };
    assert_eq!(undo.region.id, "wall1", "the inverse recreates exactly the region the delete was refused for");
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "delete-region/blocks-in-use-7c7862 declares a rejected outcome");
    let produced = <Fem2dMutation as protocol::Mutation<Fem2dSnapshot>>::diff(&mutation(), &before());
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "delete-region/blocks-in-use-7c7862: committed {label} JSON is not canonical");
    }
    assert_eq!(BEFORE, AFTER, "delete-region/blocks-in-use-7c7862 changes nothing: the two committed snapshots must be byte-identical");
    let decoded_mutation = mutation();
    let reencoded = dsl::ToValue::to_value(&decoded_mutation);
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "delete-region/blocks-in-use-7c7862: committed mutation JSON is not canonical");
}
