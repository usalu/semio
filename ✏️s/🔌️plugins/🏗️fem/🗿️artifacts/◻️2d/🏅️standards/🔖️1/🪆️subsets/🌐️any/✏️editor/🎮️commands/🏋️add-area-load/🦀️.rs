//! 🏋️ 🏋️ Fem2d play app commands command — `add-area-load`.
...
fn add_load_mutation(doc: &Fem2dSnapshot, case_id: Option<&str>, load: FemLoad) -> Fem2dMutation {
    match resolve_load_case(doc, case_id) {
        Some(existing) => Fem2dMutation::AddLoad(add_load::AddLoad { case_id: existing.id, load: Box::new(load) }),
        None => Fem2dMutation::CreateLoadCase(create_load_case::CreateLoadCase { load_case: FemLoadCase { id: "case-1".into(), name: "Load Case 1".into(), loads: vec![load], self_weight: false } }),
    }
}
...
pub fn handle(payload: &AddAreaLoad, doc: &ArtifactView<'_, Fem2dSnapshot>, _cfg: &ConfigView<'_, NoConfig>) -> Result<Emit<Fem2dMutation, NoConfigMutation>, Fault> {
