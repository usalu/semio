//! 🧪️ `replace-region` fixture — `🪪️denies-rename-574c91`.
//!
...
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔄️replace-region/🪪️denies-rename-574c91/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔄️replace-region/🪪️denies-rename-574c91/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔄️replace-region/🪪️denies-rename-574c91/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_untouched() {
    ...
    assert_eq!(snapshot, expected_after(), "replace-region/denies-rename-574c91: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "replace-region/denies-rename-574c91: a refused mutation must leave the snapshot exactly where it was");
}
...
fn inverse_of_the_refused_mutation() {
    ...
    assert_eq!(inverse.len(), 1, "replace-region/denies-rename-574c91: replace-region undoes with exactly one step, got {inverse:?}");
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "replace-region/denies-rename-574c91 declares a rejected outcome");
    let produced = <Fem2dMutation as protocol::Mutation<Fem2dSnapshot>>::diff(&mutation(), &before());
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-region/denies-rename-574c91: committed {label} JSON is not canonical");
    }
    assert_eq!(BEFORE, AFTER, "replace-region/denies-rename-574c91 changes nothing: the two committed snapshots must be byte-identical");
    let decoded_mutation = mutation();
    let reencoded = dsl::ToValue::to_value(&decoded_mutation);
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "replace-region/denies-rename-574c91: committed mutation JSON is not canonical");
}
