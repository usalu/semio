//! 🏗️ `s.fem.fem2d` whole-document identity round trip — Rust SUBJECT adapter. Relocated out of
//! the artifact-level `mutate-fem2d-1` case in ticket
...
mod subject {
    ...
    pub fn round_trip(ctx: &Context) -> Result<Outcome, String> {
        ...
        let report = parse_json(&fem2d_identity_report_json(&text_value).map_err(|error| format!("identity-round-trip: the committed example did not reach this subset's own codec: {error}"))?)?;
