//! 🧪️ `replace-load` fixture — `🪪️denies-rename-732de0`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-load/🪪️denies-rename-732de0/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-load/🪪️denies-rename-732de0/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-load/🪪️denies-rename-732de0/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-load/🪪️denies-rename-732de0/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_untouched() {
    ...
    assert_eq!(snapshot, expected_after(), "replace-load/🪪️denies-rename-732de0: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "replace-load/🪪️denies-rename-732de0: a refused mutation must leave the snapshot exactly where it was");
}
...
/// ↩️ `replace-load`'s inverse is BASE-derived. It restores the load `before` holds, addressed by the SELECTED id.
#[test]
fn inverse_of_the_refused_mutation() {
    let inverse = inverse_fem2d_mutation(&before(), &mutation());
    assert_eq!(inverse.len(), 1, "replace-load/🪪️denies-rename-732de0: replace-load undoes with exactly one step, got {inverse:?}");
    let Fem2dMutation::ReplaceLoad(undo) = &inverse[0] else {
        panic!("replace-load's inverse must be a ReplaceLoad, got {:?}", inverse[0]);
    };
    assert_eq!(undo.load_id, "lw1", "the inverse addresses the selected id, not the one the replacement carried");
    assert_eq!(undo.case_id, "wind", "the inverse stays inside the case the payload selected");
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "replace-load/🪪️denies-rename-732de0 declares a rejected outcome");
    let produced = <Fem2dMutation as protocol::Mutation<Fem2dSnapshot>>::diff(&mutation(), &before());
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-load/🪪️denies-rename-732de0: committed {label} JSON is not canonical");
    }
    assert_eq!(BEFORE, AFTER, "replace-load/🪪️denies-rename-732de0 changes nothing: the two committed snapshots must be byte-identical");
    let decoded_mutation = mutation();
    let reencoded = dsl::ToValue::to_value(&decoded_mutation);
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "replace-load/🪪️denies-rename-732de0: committed mutation JSON is not canonical");
}
