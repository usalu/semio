...
Feature: Apply every typed fem2d mesh mutation twice — once in Rust, once in Python — and require the same answer
  ...
  The artifact is real. `shared://🕸️mutate-fem2d-1-any-mesh/🏗️timber-portal-frame.snapshot.json` is the SAME derived timber-portal-frame model every
  ...
  Scenario Outline: Apply <id> to the real derived timber portal frame
    Given the real derived model shared://🕸️mutate-fem2d-1-any-mesh/🏗️timber-portal-frame.snapshot.json
