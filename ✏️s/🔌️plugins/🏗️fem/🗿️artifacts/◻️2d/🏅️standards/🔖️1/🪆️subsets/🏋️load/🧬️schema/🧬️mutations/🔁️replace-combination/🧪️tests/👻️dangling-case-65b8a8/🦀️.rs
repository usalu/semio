//! 🧪️ `replace-combination` fixture — `👻️dangling-case-65b8a8`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-combination/👻️dangling-case-65b8a8/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-combination/👻️dangling-case-65b8a8/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-combination/👻️dangling-case-65b8a8/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-combination/👻️dangling-case-65b8a8/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_untouched() {
    ...
    assert_eq!(snapshot, expected_after(), "replace-combination/👻️dangling-case-65b8a8: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "replace-combination/👻️dangling-case-65b8a8: a refused mutation must leave the snapshot exactly where it was");
}
...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(produced.diff(), &crate::standards::v1::subsets::any::schema::diff::Fem2dDiff::default(), "replace-combination/👻️dangling-case-65b8a8: a rejecting replace-combination must carry the empty diff, never a half-built delta");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.target-missing", "replace-combination/👻️dangling-case-65b8a8: the refusal is reported as mutation.target-missing");
    assert_eq!(messages[0].level, protocol::Severity::Error, "a missing reference is an Error: a different base may well carry that case");
...
fn inverse_of_the_refused_mutation() {
    ...
    assert_eq!(inverse.len(), 1, "replace-combination/👻️dangling-case-65b8a8: replace-combination undoes with exactly one step, got {inverse:?}");
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "replace-combination/👻️dangling-case-65b8a8 declares a rejected outcome");
    let produced = <Fem2dMutation as protocol::Mutation<Fem2dSnapshot>>::diff(&mutation(), &before());
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-combination/👻️dangling-case-65b8a8: committed {label} JSON is not canonical");
    }
    assert_eq!(BEFORE, AFTER, "replace-combination/👻️dangling-case-65b8a8 changes nothing: the two committed snapshots must be byte-identical");
    let decoded_mutation = mutation();
    let reencoded = dsl::ToValue::to_value(&decoded_mutation);
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "replace-combination/👻️dangling-case-65b8a8: committed mutation JSON is not canonical");
}
