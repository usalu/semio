...
Feature: Apply every typed fem2d load mutation twice — once in Rust, once in Python — and require the same answer
  ...
  Scenario Outline: Replay the committed <id> specification vector through both implementations
    ...
    | add-load                     | ➕️add-load                     | appends-a-member-udl-to-the-dead-case             |
    | remove-load                  | ➖️remove-load                  | strips-the-trailing-member-udl-from-the-dead-case |
    | change-load-case-self-weight | ⚖️change-load-case-self-weight | switches-self-weight-on-for-the-dead-case         |
