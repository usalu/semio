...
#[cfg(test)]
mod tests {
    /// 🧪️ The store type aliases live in `crate::mutations` (`Fem2dEnvelope`/`Fem2dStore`) and
    /// are exercised by that node's own tests plus `crate::spr`'s
    /// `fem2d_document_text_round_trips_through_the_store`.
    #[test]
    fn fem2d_store_type_alias_constructs_from_an_empty_envelope() {
        let store =
            semio_framework_plugin::resolve_ready(crate::mutations::Fem2dStore::new(store::create_document_envelope(crate::FEM_2D_SCHEMA, "fem2d", crate::schema::empty_fem2d_snapshot(), None)))
                .expect("valid store");
