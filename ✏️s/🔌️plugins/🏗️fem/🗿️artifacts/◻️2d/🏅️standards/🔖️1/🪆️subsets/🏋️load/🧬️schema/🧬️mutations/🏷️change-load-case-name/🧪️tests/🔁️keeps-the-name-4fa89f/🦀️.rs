//! 🧪️ `change-load-case-name` fixture — `🔁️keeps-the-name-4fa89f`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏷️change-load-case-name/🔁️keeps-the-name-4fa89f/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏷️change-load-case-name/🔁️keeps-the-name-4fa89f/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏷️change-load-case-name/🔁️keeps-the-name-4fa89f/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏷️change-load-case-name/🔁️keeps-the-name-4fa89f/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🏷️change-load-case-name/🔁️keeps-the-name-4fa89f/🎯️outcome/🔣️.json");
...
fn the_no_op_is_a_warning_not_a_rejection() {
    ...
    assert_eq!(produced.diff(), &crate::standards::v1::subsets::any::schema::diff::Fem2dDiff::default(), "change-load-case-name/🔁️keeps-the-name-4fa89f: a no-op change-load-case-name must carry the empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.no-op", "change-load-case-name/🔁️keeps-the-name-4fa89f: an unchanged value is reported as no-op");
    ...
    assert!(messages[0].target.is_empty(), "change-load-case-name raises its no-op through the 2-arg `warn` builder, which attaches no target address");
}

/// ↩️ `change-load-case-name` inverts from BASE: the case exists, so the inverse is a step restoring the name it already had.
#[test]
fn inverse_restores_before() {
    let base = before();
    let mutation = mutation();
    let inverse = inverse_fem2d_mutation(&base, &mutation);
    assert_eq!(inverse.len(), 1, "change-load-case-name/🔁️keeps-the-name-4fa89f: change-load-case-name always emits exactly one inverse step, even for a no-op, got {inverse:?}");
    let mut snapshot = base.clone();
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-load-case-name/🔁️keeps-the-name-4fa89f: committed {label} JSON is not canonical");
    }
    assert_eq!(BEFORE, AFTER, "change-load-case-name/🔁️keeps-the-name-4fa89f changes nothing: the two committed snapshots must be byte-identical");
    let decoded_mutation = mutation();
    let reencoded = dsl::ToValue::to_value(&decoded_mutation);
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "change-load-case-name/🔁️keeps-the-name-4fa89f: committed mutation JSON is not canonical");
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-load-case-name/🔁️keeps-the-name-4fa89f: produced diff differs from the committed 🔺️diff/🔣️.json");
    ...
    assert_eq!(typed, crate::standards::v1::subsets::any::schema::diff::Fem2dDiff::default(), "change-load-case-name/🔁️keeps-the-name-4fa89f: a no-op delta is the artifact's Default diff");
}
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "change-load-case-name/🔁️keeps-the-name-4fa89f: committed diff JSON is not canonical");
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "change-load-case-name/🔁️keeps-the-name-4fa89f: committed diff did not carry before to after");
}
