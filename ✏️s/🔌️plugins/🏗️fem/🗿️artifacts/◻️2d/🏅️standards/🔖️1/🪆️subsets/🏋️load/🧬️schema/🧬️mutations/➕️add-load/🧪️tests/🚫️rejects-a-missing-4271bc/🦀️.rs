//! 🧪️ `add-load` fixture — `🚫️rejects-a-missing-4271bc`.
//!
...
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕️add-load/🚫️rejects-a-missing-4271bc/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕️add-load/🚫️rejects-a-missing-4271bc/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➕️add-load/🚫️rejects-a-missing-4271bc/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_untouched() {
    ...
    assert_eq!(snapshot, expected_after(), "add-load/rejects-a-missing-4271bc: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "add-load/rejects-a-missing-4271bc: a refused mutation must leave the snapshot exactly where it was");
}
...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(produced.diff(), &crate::standards::v1::subsets::any::schema::diff::Fem2dDiff::default(), "add-load/rejects-a-missing-4271bc: a rejecting add-load must carry the empty diff, never a half-built delta");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.target-missing", "add-load/rejects-a-missing-4271bc: the refusal is reported as mutation.target-missing");
...
/// ↩️ `add-load`'s inverse is BASE-derived: with no such record on `before` there is nothing to restore, so the inverse collapses to `Vec::new()`.
#[test]
fn inverse_of_the_refused_mutation() {
    let inverse = inverse_fem2d_mutation(&before(), &mutation());
    assert!(inverse.is_empty(), "add-load/rejects-a-missing-4271bc: a target BASE never held leaves nothing to restore, got {inverse:?}");
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "add-load/rejects-a-missing-4271bc declares a rejected outcome");
    let produced = <Fem2dMutation as protocol::Mutation<Fem2dSnapshot>>::diff(&mutation(), &before());
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "add-load/rejects-a-missing-4271bc: committed {label} JSON is not canonical");
    }
    assert_eq!(BEFORE, AFTER, "add-load/rejects-a-missing-4271bc changes nothing: the two committed snapshots must be byte-identical");
    let decoded_mutation = mutation();
    let reencoded = dsl::ToValue::to_value(&decoded_mutation);
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "add-load/rejects-a-missing-4271bc: committed mutation JSON is not canonical");
}
