...
pub fn inverse(payload: &DeleteSupport, base: &Fem2dSnapshot) -> Vec<Fem2dMutation> {
    base.supports.iter().find(|item| item.id == payload.id).map(|item| vec![Fem2dMutation::CreateSupport(create_support::CreateSupport { support: item.clone() })]).unwrap_or_default()
}
//#endregion 🔖️Inverse
