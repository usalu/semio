...
fn compose_export_zip(sources: &[ErasedComposeSource]) -> Result<ComposedArtifact, ComposeError> {
    ...
    let bytes = crate::artifacts::fem3d::io::export::serializers::artifacts::zip::v2_0::any::export(&snapshot).map_err(|e| ComposeError { message: e.to_string(), diagnostics: Vec::new() })?;
...
fn compose_export_png(sources: &[ErasedComposeSource]) -> Result<ComposedArtifact, ComposeError> {
    let snapshot = rebuild_native_snapshot(sources)?;
    let bytes = crate::artifacts::fem3d::io::export::serializers::artifacts::png::v1_2::any::export(&snapshot).map_err(|e| ComposeError { message: e.to_string(), diagnostics: Vec::new() })?;
...
fn compose_export_glb(sources: &[ErasedComposeSource]) -> Result<ComposedArtifact, ComposeError> {
    let snapshot = rebuild_native_snapshot(sources)?;
    let bytes = crate::artifacts::fem3d::io::export::serializers::artifacts::glb::v2_0::any::export(&snapshot).map_err(|e| ComposeError { message: e.to_string(), diagnostics: Vec::new() })?;
...
pub fn entries() -> &'static [ComposerEntry] {
    ENTRIES.get_or_init(|| vec![
        ...
        ComposerEntry { writes: EXPORT_ZIP_DIALECT, reads: &[FEM3D_DIALECT], compose: compose_export_zip },
        ...
        ComposerEntry { writes: EXPORT_PNG_DIALECT, reads: &[FEM3D_DIALECT], compose: compose_export_png },
        ...
        ComposerEntry { writes: EXPORT_GLB_DIALECT, reads: &[FEM3D_DIALECT], compose: compose_export_glb },
