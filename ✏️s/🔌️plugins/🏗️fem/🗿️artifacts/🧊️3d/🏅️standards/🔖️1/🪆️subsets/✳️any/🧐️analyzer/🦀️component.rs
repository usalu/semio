...
impl ArtifactAnalyzer for Fem3dAnalyzer {
    ...
    fn analyze(sources: &[AnalyzeSource<'_>]) -> Analysis<Self::Parts> {
        ...
        for source in sources {
            match source {
                AnalyzeSource::Text(text) => match <Fem3dSnapshot as store::DocumentDsl>::parse_dsl(text) {
                ...
                AnalyzeSource::Binary(bytes) => match <Fem3dSnapshot as store::DocumentPack>::decode_pack(bytes) {
                    Ok(snapshot) => parts.snapshot = Some(snapshot),
                    Err(err) => {
                        confidence = IoConfidence::Low;
