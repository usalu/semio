...
pub fn inverse(payload: &DeleteSupport, base: &Fem3dSnapshot) -> Vec<Fem3dMutation> {
    base.supports.iter().find(|item| item.id == payload.id).map(|item| vec![Fem3dMutation::CreateSupport(create_support::CreateSupport { support: item.clone() })]).unwrap_or_default()
}
//#endregion 🔖️Inverse
