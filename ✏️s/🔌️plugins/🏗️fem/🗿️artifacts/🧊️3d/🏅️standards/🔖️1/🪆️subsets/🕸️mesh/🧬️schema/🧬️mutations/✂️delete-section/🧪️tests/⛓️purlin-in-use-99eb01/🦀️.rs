//! 🧪️ `delete-section` fixture — `⛓️purlin-in-use-99eb01`.
//!
...
//!
//! The C24 purlin profile is shared by all six eaves and ridge purlins. Deleting it would leave six elements
...
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️delete-section/⛓️purlin-in-use-99eb01/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️delete-section/⛓️purlin-in-use-99eb01/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️delete-section/⛓️purlin-in-use-99eb01/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_at_the_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "delete-section/purlin-in-use-99eb01: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "delete-section/purlin-in-use-99eb01: a refused mutation must leave every one of the nine members untouched");
}
...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(produced.diff(), &crate::standards::v1::subsets::any::schema::diff::Fem3dDiff::default(), "delete-section/purlin-in-use-99eb01: a refused mutation must carry the empty diff");
...
fn inverse_has_the_declared_shape() {
    ...
    assert_eq!(inverse.len(), 1, "delete-section/purlin-in-use-99eb01: this kind always emits exactly one undo step, got {inverse:?}");
    assert!(matches!(inverse[0], Fem3dMutation::CreateSection(_)), "delete-section/purlin-in-use-99eb01: the inverse of delete-section is a CreateSection, got {:?}", inverse[0]);
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "delete-section/purlin-in-use-99eb01: this vector declares a rejected outcome");
    let produced = <Fem3dMutation as protocol::Mutation<Fem3dSnapshot>>::diff(&mutation(), &before());
    let message = produced.messages().first().expect("a rejected outcome carries a diagnostic");
    assert_eq!(outcome.get("code").and_then(dsl::DslValue::as_str), Some(message.code.0.as_str()), "delete-section/purlin-in-use-99eb01: the declared code must match the emitted one");
    ...
    assert_eq!(declared_path, message.target, "delete-section/purlin-in-use-99eb01: the declared path must match the emitted target");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "delete-section/purlin-in-use-99eb01: committed {label} JSON is not canonical");
    }
    let reencoded = dsl::ToValue::to_value(&mutation());
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "delete-section/purlin-in-use-99eb01: committed mutation JSON is not canonical");
}
...
fn semantics_bind_the_declared_kind() {
    ...
    assert_eq!((semantics.verb, semantics.entity, semantics.kind, semantics.record), ("delete", "section", "delete-section", "DeletedSection"), "delete-section/purlin-in-use-99eb01: the fixture must be bound to delete-section's own descriptor");
}
