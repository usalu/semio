//! 🧪️ `replace-node` fixture — `🪪️renames-node-4a2286`.
//!
...
//!
//! `n2` is a leaf of the reference graph, so renaming it would orphan nothing — and the identity contract is still the
//! identity contract: a replace never renames.

use crate::standards::v1::subsets::any::schema::mutations::Fem3dMutation;
use crate::standards::v1::subsets::any::schema::mutations::{apply_fem3d_mutation, inverse_fem3d_mutation};
use crate::Fem3dSnapshot;

const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-node/🪪️renames-node-4a2286/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-node/🪪️renames-node-4a2286/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-node/🪪️renames-node-4a2286/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-node/🪪️renames-node-4a2286/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_at_the_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "replace-node/renames-node-4a2286: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "replace-node/renames-node-4a2286: a refused mutation must leave every one of the nine members untouched");
}
...
fn inverse_has_the_declared_shape() {
    ...
    for step in &inverse {
        assert!(matches!(step, Fem3dMutation::ReplaceNode(_)), "replace-node/renames-node-4a2286: the inverse of replace-node is a ReplaceNode, got {step:?}");
    }
    assert!(inverse.len() <= 1, "replace-node/renames-node-4a2286: this kind never emits more than one undo step, got {inverse:?}");
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "replace-node/renames-node-4a2286: this vector declares a rejected outcome");
    let produced = <Fem3dMutation as protocol::Mutation<Fem3dSnapshot>>::diff(&mutation(), &before());
    let message = produced.messages().first().expect("a rejected outcome carries a diagnostic");
    assert_eq!(outcome.get("code").and_then(dsl::DslValue::as_str), Some(message.code.0.as_str()), "replace-node/renames-node-4a2286: the declared code must match the emitted one");
    ...
    assert_eq!(declared_path, message.target, "replace-node/renames-node-4a2286: the declared path must match the emitted target");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-node/renames-node-4a2286: committed {label} JSON is not canonical");
    }
    let reencoded = dsl::ToValue::to_value(&mutation());
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "replace-node/renames-node-4a2286: committed mutation JSON is not canonical");
}
...
fn semantics_bind_the_declared_kind() {
    ...
    assert_eq!((semantics.verb, semantics.entity, semantics.kind, semantics.record), ("replace", "node", "replace-node", "ReplacedNode"), "replace-node/renames-node-4a2286: the fixture must be bound to replace-node's own descriptor");
}
