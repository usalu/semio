...
owns 1. A finite-element solver (`code_aster`, `OpenSees`, `anastruct`, `PyNite`) computes
...
def uri_in(ctx, needle):
    ...
    for step in ctx.scenario["steps"]:
        for token in step["text"].split():
            if token.startswith(("asset://", "shared://📈️mutate-fem3d-1-analysis/", "shared://")) and needle in token:
                return token
