...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        Some("project") => match read(args.get(2).expect("usage: reader project <file.json>")) {
            Ok(value) => println!("{}", report("fem3d-json-project", "ok", value)),
            Err(error) => {
                println!("{}", report("fem3d-json-project", "failed", json!({"error": error})));
                exit(1);
            }
        },
        Some("compare") => {
            ...
            match (read(expected), read(actual)) {
                (Ok(left), Ok(right)) => println!("{}", report("fem3d-json-compare", "ok", json!({"equal": left == right, "expected": left, "actual": right}))),
                (Err(error), _) | (_, Err(error)) => {
                    println!("{}", report("fem3d-json-compare", "failed", json!({"error": error})));
                    exit(1);
                }
            }
        }
