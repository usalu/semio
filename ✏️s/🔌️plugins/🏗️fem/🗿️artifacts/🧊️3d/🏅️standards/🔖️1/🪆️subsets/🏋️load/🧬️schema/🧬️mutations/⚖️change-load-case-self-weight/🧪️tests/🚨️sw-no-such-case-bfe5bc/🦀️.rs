//! 🧪️ `change-load-case-self-weight` fixture — `🚨️sw-no-such-case-bfe5bc`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚖️change-load-case-self-weight/🚨️sw-no-such-case-bfe5bc/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚖️change-load-case-self-weight/🚨️sw-no-such-case-bfe5bc/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚖️change-load-case-self-weight/🚨️sw-no-such-case-bfe5bc/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚖️change-load-case-self-weight/🚨️sw-no-such-case-bfe5bc/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_at_the_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "change-load-case-self-weight/sw-no-such-case-bfe5bc: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "change-load-case-self-weight/sw-no-such-case-bfe5bc: a refused mutation must leave every one of the nine members untouched");
}
...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(messages[0].level, protocol::Severity::Error, "change-load-case-self-weight/sw-no-such-case-bfe5bc: a missed target is an Error, not the Fatal a duplicate identity raises");
    assert_eq!(messages[0].target, vec!["seismic".to_string()], "change-load-case-self-weight/sw-no-such-case-bfe5bc: the diagnostic addresses exactly the identity that could not be resolved");
}
...
fn inverse_has_the_declared_shape() {
    ...
    assert!(inverse.is_empty(), "change-load-case-self-weight/sw-no-such-case-bfe5bc: a base-derived inverse of a missing target has nothing to restore, got {inverse:?}");
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "change-load-case-self-weight/sw-no-such-case-bfe5bc: this vector declares a rejected outcome");
    let produced = <Fem3dMutation as protocol::Mutation<Fem3dSnapshot>>::diff(&mutation(), &before());
    let message = produced.messages().first().expect("a rejected outcome carries a diagnostic");
    assert_eq!(outcome.get("code").and_then(dsl::DslValue::as_str), Some(message.code.0.as_str()), "change-load-case-self-weight/sw-no-such-case-bfe5bc: the declared code must match the emitted one");
    ...
    assert_eq!(declared_path, message.target, "change-load-case-self-weight/sw-no-such-case-bfe5bc: the declared path must match the emitted target");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-load-case-self-weight/sw-no-such-case-bfe5bc: committed {label} JSON is not canonical");
    }
    let reencoded = dsl::ToValue::to_value(&mutation());
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "change-load-case-self-weight/sw-no-such-case-bfe5bc: committed mutation JSON is not canonical");
}
...
fn semantics_bind_the_declared_kind() {
    ...
    assert_eq!(
        ...
        "change-load-case-self-weight/sw-no-such-case-bfe5bc: the fixture must be bound to change-load-case-self-weight's own descriptor"
    );
}
