...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(produced.diff(), &crate::artifacts::fem3d::diff::Fem3dDiff::default(), "change-load-case-self-weight/sw-no-such-case-bfe5bc: a refused mutation must carry the empty diff");
...
fn semantics_bind_the_declared_kind() {
    ...
    assert_eq!((semantics.verb, semantics.entity, semantics.kind, semantics.record), ("change", "load-case", "change-load-case-self-weight", "ChangedLoadCaseSelfWeight"), "change-load-case-self-weight/sw-no-such-case-bfe5bc: the fixture must be bound to change-load-case-self-weight's own descriptor");
}
