//! 🧪️ `replace-material` fixture — `🪪️renames-c24-b60696`.
//!
...
//!
//! This is finding 3 of W11's report turned into a refusal: `replace-material { id: "c24", newMaterial: { id:
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-material/🪪️renames-c24-b60696/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-material/🪪️renames-c24-b60696/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-material/🪪️renames-c24-b60696/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-material/🪪️renames-c24-b60696/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_at_the_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "replace-material/renames-c24-b60696: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "replace-material/renames-c24-b60696: a refused mutation must leave every one of the nine members untouched");
}
...
fn inverse_has_the_declared_shape() {
    ...
    assert_eq!(inverse.len(), 1, "replace-material/renames-c24-b60696: this kind always emits exactly one undo step, got {inverse:?}");
    assert!(matches!(inverse[0], Fem3dMutation::ReplaceMaterial(_)), "replace-material/renames-c24-b60696: the inverse of replace-material is a ReplaceMaterial, got {:?}", inverse[0]);
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "replace-material/renames-c24-b60696: this vector declares a rejected outcome");
    let produced = <Fem3dMutation as protocol::Mutation<Fem3dSnapshot>>::diff(&mutation(), &before());
    let message = produced.messages().first().expect("a rejected outcome carries a diagnostic");
    assert_eq!(outcome.get("code").and_then(dsl::DslValue::as_str), Some(message.code.0.as_str()), "replace-material/renames-c24-b60696: the declared code must match the emitted one");
    ...
    assert_eq!(declared_path, message.target, "replace-material/renames-c24-b60696: the declared path must match the emitted target");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-material/renames-c24-b60696: committed {label} JSON is not canonical");
    }
    let reencoded = dsl::ToValue::to_value(&mutation());
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "replace-material/renames-c24-b60696: committed mutation JSON is not canonical");
}
...
fn semantics_bind_the_declared_kind() {
    ...
    assert_eq!(
        ...
        "replace-material/renames-c24-b60696: the fixture must be bound to replace-material's own descriptor"
    );
}
