//! 🧪️ `remove-load` fixture — `🚨️no-such-load-5bab2d`.
//!
...
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➖️remove-load/🚨️no-such-load-5bab2d/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/➖️remove-load/🚨️no-such-load-5bab2d/🦠️mutation/🔣️.json");
...
fn rejection_leaves_the_document_at_the_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "remove-load/no-such-load-5bab2d: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "remove-load/no-such-load-5bab2d: a refused mutation must leave every one of the nine members untouched");
}
...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(produced.diff(), &crate::standards::v1::subsets::any::schema::diff::Fem3dDiff::default(), "remove-load/no-such-load-5bab2d: a refused mutation must carry the empty diff");
...
fn semantics_bind_the_declared_kind() {
    ...
    assert_eq!((semantics.verb, semantics.entity, semantics.kind, semantics.record), ("remove", "load", "remove-load", "RemovedLoad"), "remove-load/no-such-load-5bab2d: the fixture must be bound to remove-load's own descriptor");
}
