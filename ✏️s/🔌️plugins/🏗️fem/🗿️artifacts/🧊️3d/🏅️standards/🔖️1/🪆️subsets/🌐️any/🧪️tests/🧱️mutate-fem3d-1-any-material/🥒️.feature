...
Feature: Apply every typed fem3d material mutation twice — once in Rust, once in Python — and require the same answer
  ...
  The artifact is real. `shared://🧱️mutate-fem3d-1-any-material/🧊️steel-frame.snapshot.json` is the SAME derived steel frame model every fem3d
  ...
  Scenario Outline: Apply <id> to the real derived steel frame
    Given the real derived model shared://🧱️mutate-fem3d-1-any-material/🧊️steel-frame.snapshot.json
