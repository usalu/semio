//! 🧪️ `create-node` fixture — `🚨️dup-node-id-86f2e1`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚪️create-node/🚨️dup-node-id-86f2e1/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚪️create-node/🚨️dup-node-id-86f2e1/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚪️create-node/🚨️dup-node-id-86f2e1/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚪️create-node/🚨️dup-node-id-86f2e1/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_at_the_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "create-node/dup-node-id-86f2e1: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "create-node/dup-node-id-86f2e1: a refused mutation must leave every one of the nine members untouched");
}
...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(messages[0].level, protocol::Severity::Fatal, "create-node/dup-node-id-86f2e1: an identity collision is an invariant breach that no merge policy may absorb");
    assert_eq!(messages[0].target, vec!["ap_1".to_string()], "create-node/dup-node-id-86f2e1: the diagnostic addresses exactly the identity that collided");
}
...
fn inverse_has_the_declared_shape() {
    ...
    assert_eq!(inverse.len(), 1, "create-node/dup-node-id-86f2e1: a payload-derived inverse always emits exactly one step, got {inverse:?}");
    assert!(matches!(inverse[0], Fem3dMutation::DeleteNode(_)), "create-node/dup-node-id-86f2e1: the inverse of create-node is a delete-node, got {:?}", inverse[0]);
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "create-node/dup-node-id-86f2e1: this vector declares a rejected outcome");
    let produced = <Fem3dMutation as protocol::Mutation<Fem3dSnapshot>>::diff(&mutation(), &before());
    let message = produced.messages().first().expect("a rejected outcome carries a diagnostic");
    assert_eq!(outcome.get("code").and_then(dsl::DslValue::as_str), Some(message.code.0.as_str()), "create-node/dup-node-id-86f2e1: the declared code must match the emitted one");
    ...
    assert_eq!(declared_path, message.target, "create-node/dup-node-id-86f2e1: the declared path must match the emitted target");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "create-node/dup-node-id-86f2e1: committed {label} JSON is not canonical");
    }
    let reencoded = dsl::ToValue::to_value(&mutation());
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "create-node/dup-node-id-86f2e1: committed mutation JSON is not canonical");
}
...
fn semantics_bind_the_declared_kind() {
    ...
    assert_eq!((semantics.verb, semantics.entity, semantics.kind, semantics.record), ("create", "node", "create-node", "CreatedNode"), "create-node/dup-node-id-86f2e1: the fixture must be bound to create-node's own descriptor");
}
