//! 🧪️ `replace-solid` fixture — `📐️zero-height-2b131a`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔄️replace-solid/📐️zero-height-2b131a/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔄️replace-solid/📐️zero-height-2b131a/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔄️replace-solid/📐️zero-height-2b131a/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔄️replace-solid/📐️zero-height-2b131a/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_at_the_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "replace-solid/zero-height-2b131a: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "replace-solid/zero-height-2b131a: a refused mutation must leave every one of the nine members untouched");
}
...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(messages[0].level, protocol::Severity::Fatal, "replace-solid/zero-height-2b131a: an unphysical value is Fatal: no later mutation could rescue a document that took it");
    assert_eq!(messages[0].target, vec!["slab_apron".to_string()], "replace-solid/zero-height-2b131a: the diagnostic addresses exactly \"slab_apron\"");
}
...
fn inverse_has_the_declared_shape() {
    ...
    assert_eq!(inverse.len(), 1, "replace-solid/zero-height-2b131a: this kind always emits exactly one undo step, got {inverse:?}");
    assert!(matches!(inverse[0], Fem3dMutation::ReplaceSolid(_)), "replace-solid/zero-height-2b131a: the inverse of replace-solid is a ReplaceSolid, got {:?}", inverse[0]);
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "replace-solid/zero-height-2b131a: this vector declares a rejected outcome");
    let produced = <Fem3dMutation as protocol::Mutation<Fem3dSnapshot>>::diff(&mutation(), &before());
    let message = produced.messages().first().expect("a rejected outcome carries a diagnostic");
    assert_eq!(outcome.get("code").and_then(dsl::DslValue::as_str), Some(message.code.0.as_str()), "replace-solid/zero-height-2b131a: the declared code must match the emitted one");
    ...
    assert_eq!(declared_path, message.target, "replace-solid/zero-height-2b131a: the declared path must match the emitted target");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-solid/zero-height-2b131a: committed {label} JSON is not canonical");
    }
    let reencoded = dsl::ToValue::to_value(&mutation());
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "replace-solid/zero-height-2b131a: committed mutation JSON is not canonical");
}
...
fn semantics_bind_the_declared_kind() {
    ...
    assert_eq!((semantics.verb, semantics.entity, semantics.kind, semantics.record), ("replace", "solid", "replace-solid", "ReplacedSolid"), "replace-solid/zero-height-2b131a: the fixture must be bound to replace-solid's own descriptor");
}
