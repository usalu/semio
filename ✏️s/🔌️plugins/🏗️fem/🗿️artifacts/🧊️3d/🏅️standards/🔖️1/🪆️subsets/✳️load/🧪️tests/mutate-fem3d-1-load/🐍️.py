...
subset's typed mutations (`create-load-case`, `delete-load-case`, `add-load`, `remove-load`, `change-load-case-self-weight`, `create-combination`, `delete-combination`), in Python, serving as this case's differential oracle.
...
owns 7. A finite-element solver (`code_aster`, `OpenSees`, `anastruct`, `PyNite`) computes
