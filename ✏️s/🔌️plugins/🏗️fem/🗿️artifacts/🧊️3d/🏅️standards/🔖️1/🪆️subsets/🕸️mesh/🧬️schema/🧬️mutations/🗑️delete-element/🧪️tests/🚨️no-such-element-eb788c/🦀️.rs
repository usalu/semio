//! 🧪️ `delete-element` fixture — `🚨️no-such-element-eb788c`.
//!
...
fn rejection_leaves_the_document_at_the_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "delete-element/no-such-element-eb788c: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "delete-element/no-such-element-eb788c: a refused mutation must leave every one of the nine members untouched");
}
...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(produced.diff(), &crate::standards::v1::subsets::any::schema::diff::Fem3dDiff::default(), "delete-element/no-such-element-eb788c: a refused mutation must carry the empty diff");
    let messages = produced.messages();
    assert_eq!(messages.len(), 1, "delete-element/no-such-element-eb788c: exactly one diagnostic is expected, got {messages:?}");
    assert_eq!(messages[0].code.0, "mutation.target-missing", "delete-element/no-such-element-eb788c: the refusal is reported as mutation.target-missing");
    assert_eq!(messages[0].level, protocol::Severity::Error, "delete-element/no-such-element-eb788c: a missed target is an Error, not the Fatal a duplicate identity raises");
    assert_eq!(messages[0].target, vec!["col_l_9".to_string()], "delete-element/no-such-element-eb788c: the diagnostic addresses exactly the identity that could not be resolved");
}
...
fn inverse_has_the_declared_shape() {
    ...
    assert!(inverse.is_empty(), "delete-element/no-such-element-eb788c: a base-derived inverse of a missing target has nothing to restore, got {inverse:?}");
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "delete-element/no-such-element-eb788c: this vector declares a rejected outcome");
    let produced = <Fem3dMutation as protocol::Mutation<Fem3dSnapshot>>::diff(&mutation(), &before());
    let message = produced.messages().first().expect("a rejected outcome carries a diagnostic");
    assert_eq!(outcome.get("code").and_then(dsl::DslValue::as_str), Some(message.code.0.as_str()), "delete-element/no-such-element-eb788c: the declared code must match the emitted one");
    ...
    assert_eq!(declared_path, message.target, "delete-element/no-such-element-eb788c: the declared path must match the emitted target");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "delete-element/no-such-element-eb788c: committed {label} JSON is not canonical");
    }
    let reencoded = dsl::ToValue::to_value(&mutation());
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "delete-element/no-such-element-eb788c: committed mutation JSON is not canonical");
}
...
fn semantics_bind_the_declared_kind() {
    ...
    assert_eq!((semantics.verb, semantics.entity, semantics.kind, semantics.record), ("delete", "element", "delete-element", "DeletedElement"), "delete-element/no-such-element-eb788c: the fixture must be bound to delete-element's own descriptor");
}
