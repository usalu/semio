//! 🧪️ `delete-material` fixture — `⛓️glulam-in-use-1208e1`.
//!
...
//!
//! GL24h is the grade of every column and rafter in the hall. The refusal lists all twelve so a caller can see
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️delete-material/⛓️glulam-in-use-1208e1/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️delete-material/⛓️glulam-in-use-1208e1/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️delete-material/⛓️glulam-in-use-1208e1/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️delete-material/⛓️glulam-in-use-1208e1/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_at_the_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "delete-material/glulam-in-use-1208e1: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "delete-material/glulam-in-use-1208e1: a refused mutation must leave every one of the nine members untouched");
}
...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(produced.diff(), &crate::standards::v1::subsets::any::schema::diff::Fem3dDiff::default(), "delete-material/glulam-in-use-1208e1: a refused mutation must carry the empty diff");
...
fn inverse_has_the_declared_shape() {
    ...
    assert_eq!(inverse.len(), 1, "delete-material/glulam-in-use-1208e1: this kind always emits exactly one undo step, got {inverse:?}");
    assert!(matches!(inverse[0], Fem3dMutation::CreateMaterial(_)), "delete-material/glulam-in-use-1208e1: the inverse of delete-material is a CreateMaterial, got {:?}", inverse[0]);
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "delete-material/glulam-in-use-1208e1: this vector declares a rejected outcome");
    let produced = <Fem3dMutation as protocol::Mutation<Fem3dSnapshot>>::diff(&mutation(), &before());
    let message = produced.messages().first().expect("a rejected outcome carries a diagnostic");
    assert_eq!(outcome.get("code").and_then(dsl::DslValue::as_str), Some(message.code.0.as_str()), "delete-material/glulam-in-use-1208e1: the declared code must match the emitted one");
    ...
    assert_eq!(declared_path, message.target, "delete-material/glulam-in-use-1208e1: the declared path must match the emitted target");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "delete-material/glulam-in-use-1208e1: committed {label} JSON is not canonical");
    }
    let reencoded = dsl::ToValue::to_value(&mutation());
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "delete-material/glulam-in-use-1208e1: committed mutation JSON is not canonical");
}
...
fn semantics_bind_the_declared_kind() {
    ...
    assert_eq!((semantics.verb, semantics.entity, semantics.kind, semantics.record), ("delete", "material", "delete-material", "DeletedMaterial"), "delete-material/glulam-in-use-1208e1: the fixture must be bound to delete-material's own descriptor");
}
