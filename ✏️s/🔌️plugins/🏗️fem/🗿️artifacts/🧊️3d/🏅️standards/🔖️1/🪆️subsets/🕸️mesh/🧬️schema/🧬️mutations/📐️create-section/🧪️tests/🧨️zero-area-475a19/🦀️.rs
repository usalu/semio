//! 🧪️ `create-section` fixture — `🧨️zero-area-475a19`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐️create-section/🧨️zero-area-475a19/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐️create-section/🧨️zero-area-475a19/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐️create-section/🧨️zero-area-475a19/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐️create-section/🧨️zero-area-475a19/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_at_the_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "create-section/zero-area-475a19: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "create-section/zero-area-475a19: a refused mutation must leave every one of the nine members untouched");
}
...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(messages[0].level, protocol::Severity::Fatal, "create-section/zero-area-475a19: an unphysical value is Fatal: no later mutation could rescue a document that took it");
    assert_eq!(messages[0].target, vec!["sec_void".to_string()], "create-section/zero-area-475a19: the diagnostic addresses exactly \"sec_void\"");
}
...
fn inverse_has_the_declared_shape() {
    ...
    assert_eq!(inverse.len(), 1, "create-section/zero-area-475a19: this kind always emits exactly one undo step, got {inverse:?}");
    assert!(matches!(inverse[0], Fem3dMutation::DeleteSection(_)), "create-section/zero-area-475a19: the inverse of create-section is a DeleteSection, got {:?}", inverse[0]);
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "create-section/zero-area-475a19: this vector declares a rejected outcome");
    let produced = <Fem3dMutation as protocol::Mutation<Fem3dSnapshot>>::diff(&mutation(), &before());
    let message = produced.messages().first().expect("a rejected outcome carries a diagnostic");
    assert_eq!(outcome.get("code").and_then(dsl::DslValue::as_str), Some(message.code.0.as_str()), "create-section/zero-area-475a19: the declared code must match the emitted one");
    ...
    assert_eq!(declared_path, message.target, "create-section/zero-area-475a19: the declared path must match the emitted target");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "create-section/zero-area-475a19: committed {label} JSON is not canonical");
    }
    let reencoded = dsl::ToValue::to_value(&mutation());
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "create-section/zero-area-475a19: committed mutation JSON is not canonical");
}
...
fn semantics_bind_the_declared_kind() {
    ...
    assert_eq!((semantics.verb, semantics.entity, semantics.kind, semantics.record), ("create", "section", "create-section", "CreatedSection"), "create-section/zero-area-475a19: the fixture must be bound to create-section's own descriptor");
}
