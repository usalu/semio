//! 🚪️ fem3d → csv — foreign `Serializer<Fem3dSnapshot>` on the framework's `io_mechanism` channel.
//! A fem3d document is eight heterogeneous tables (nodes/elements/materials/sections/solids/
...
pub fn csv_snapshot(from: &Fem3dSnapshot) -> CsvSnapshot {
    CsvSnapshot {
        schema: STDIO_CSV_DOCUMENT_SCHEMA.into(),
        has_header: true,
        records: vec![CsvRecord { fields: vec![CsvField { value: PAYLOAD_COLUMN.into(), quoted: false }] }, CsvRecord { fields: vec![CsvField { value: <Fem3dSnapshot as store::ArtifactDsl>::print_dsl(from), quoted: true }] }],
    }
}
