//! 🔍️ The READER: projects a committed fem3d JSON carrier through `json` (json-rust) and compares two of
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        Some("project") => match read(args.get(2).expect("usage: reader project <file.json>")) {
            ...
            Err(error) => {
                println!("{}", report("fem3d-json-project", "failed", object! {"error": error}));
                exit(1);
            }
        },
        Some("compare") => {
            ...
            match (read(expected), read(actual)) {
                (Ok(left), Ok(right)) => println!("{}", report("fem3d-json-compare", "ok", object! {"equal": left == right, "expected": left, "actual": right})),
                (Err(error), _) | (_, Err(error)) => {
                    println!("{}", report("fem3d-json-compare", "failed", object! {"error": error}));
                    exit(1);
                }
            }
        }
