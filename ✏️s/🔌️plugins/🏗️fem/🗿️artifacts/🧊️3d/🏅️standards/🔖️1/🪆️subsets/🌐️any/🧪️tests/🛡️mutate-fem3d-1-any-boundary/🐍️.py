...
REJECT_VECTORS = (
    "same-support-bff8b3",
    "dangling-node-d44469",
    "renames-pin-29f41a",
    "no-such-support-edd22a",
    "dangling-node-af37e2",
)
...
def uri_in(ctx, needle):
    ...
    for step in ctx.scenario["steps"]:
        for token in step["text"].split():
            if token.startswith(("asset://", "shared://🛡️mutate-fem3d-1-any-boundary/", "shared://")) and needle in token:
                return token
