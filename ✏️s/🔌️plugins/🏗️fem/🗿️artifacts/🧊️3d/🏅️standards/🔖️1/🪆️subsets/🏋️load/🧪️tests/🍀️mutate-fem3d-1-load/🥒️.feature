...
Feature: Apply every typed fem3d load mutation twice — once in Rust, once in Python — and require the same answer
  ...
  Scenario Outline: Replay the committed <id> specification vector through both implementations
    ...
    | add-load                     | ➕️add-load                     | lays-an-area-pressure-over-the-roof-slab                    |
    | remove-load                  | ➖️remove-load                  | drops-the-trailing-member-udl-from-the-dead-case            |
    | change-load-case-self-weight | ⚖️change-load-case-self-weight | switches-self-weight-off-for-the-dead-case                  |
