...
mod tests {
    ...
    async fn add_nodal_load_targets_named_case() {
        ...
        let live_case_id = semio_framework_plugin::resolve_ready(app.snapshot()).expect("snapshot").load_cases[1].id.clone();
    ...
    async fn add_combination_invalid_terms_json_is_a_no_op() {
        ...
        assert!(semio_framework_plugin::resolve_ready(app.snapshot()).expect("snapshot").combinations.is_empty());
    }
}
