//! 🧪️ `replace-load` fixture — `🚨️no-such-case-21f1b7`.
//!
...
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-load/🚨️no-such-case-21f1b7/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-load/🚨️no-such-case-21f1b7/🦠️mutation/🔣️.json");
...
fn rejection_leaves_the_document_at_the_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "replace-load/no-such-case-21f1b7: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "replace-load/no-such-case-21f1b7: a refused mutation must leave every one of the nine members untouched");
}
...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(produced.diff(), &crate::standards::v1::subsets::any::schema::diff::Fem3dDiff::default(), "replace-load/no-such-case-21f1b7: a refused mutation must carry the empty diff");
...
fn inverse_has_the_declared_shape() {
    ...
    assert!(inverse.len() <= 1, "replace-load/no-such-case-21f1b7: this kind never emits more than one undo step, got {inverse:?}");
}
...
fn semantics_bind_the_declared_kind() {
    ...
    assert_eq!((semantics.verb, semantics.entity, semantics.kind, semantics.record), ("replace", "load", "replace-load", "ReplacedLoad"), "replace-load/no-such-case-21f1b7: the fixture must be bound to replace-load's own descriptor");
}
