  ...
  if (command === "carrier" || command === "carrier-manifests") {
    ...
    for (const [kind, fixture] of Object.entries(CARRIER_FIXTURES)) {
      ...
      entries.push({
        ...
        generator: { oracle: "serde-json-fem3d-carrier-reader", packageVersion: "1", engineFamily: "serde-json", engineVersion: "1", command: "bun ✏️s/🔌️plugins/🏗️fem/🗿️artifacts/🧊️3d/🏅️standards/🔖️1/🪆️subsets/🌐️any/🏭️generator/📜️script.ts carrier", platform: process.platform },
        comparisonProfile: "semantic-fem3d-carrier-v1",
        reproducible: true,
        family: "mechanical",
        notes: `A deterministic fem3d model carrying at least two of every collection, with the ${kind} mutation applied as an edit to the JSON CARRIER and read back through serde_json — never through this repository's own mutation engine. Observability (after projection != before projection) is checked before a pair is written, and a pair that does not move is refused rather than committed.`,
      });
    }
