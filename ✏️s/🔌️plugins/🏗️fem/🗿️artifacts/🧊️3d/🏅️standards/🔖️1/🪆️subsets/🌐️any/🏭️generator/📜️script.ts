...
// 🏭️ Third-party fixture generator for `s.fem.fem3d@1/🌐️any`.
//
// A `FemSolid` is an outline (+ zero or more holes) extruded by its own `height`, offset by `baseZ` —
// exactly what `crate::fem3d_engine::meshing::build_semio_mesh_snapshot` bridges into OBJ/STL. Every
// recipe here builds the SAME kind of shape independently: `manifold-3d`'s own `CrossSection.extrude`
// (a real solid-boolean kernel's native polygon-with-holes extrusion, not a re-derivation of our
// triangulator) BUILDS it and is the sole source of its volume/area/genus truth, translated by `baseZ`,
// and `three` (a real 3D engine, nothing of ours) EXPORTS it to OBJ/STL and RE-IMPORTS what it wrote.
// Nothing here reimplements our `crate::mesh::triangulate`/`extrude_tri_mesh`/`split_to_tets`/
// `boundary_faces` chain — that is the point: an expectation produced by two second parties proves the
// CAPABILITY (a straight-edged polygon-with-holes prism, possibly floating above the origin, round-trips
// through OBJ/STL) rather than that either library agrees with us.
//
...
export type Family = "rectangles" | "polygons" | "holes" | "scale" | "degenerate" | "elevated";

export type Toolkit = Readonly<{ Manifold: typeof Manifold; CrossSection: typeof CrossSection }>;

/** 🧪️ One corpus entry. A recipe DESCRIBES a `FemSolid`'s geometry (outline + holes + height + baseZ);
 *  it computes nothing. `build` returns the extruded, translated solid manifold-3d itself produced. */
...
/** 🌐️ `FemSolid.{outline,holes,height,baseZ}` → a real extruded, Z-translated `Manifold`, via
 *  manifold-3d's OWN polygon-with-holes handling: `EvenOdd` fill toggles fill state on every contour
 *  crossing regardless of winding, so a hole loop nested inside the outer loop subtracts itself without
 *  this generator having to agree with `crate::mesh`'s "either winding" convention on which way is
 *  which. `translate(0,0,baseZ)` mirrors the bridge fn's `z[i] + solid.base_z` offset exactly. */
function extrudeSolid(t: Toolkit, recipe: Recipe): Manifold {
  ...
  return cross.extrude(recipe.height).translate(0, 0, recipe.baseZ);
}
...
//#region 🧪️Corpus
/** 🧪️ Every recipe is a `FemSolid`'s `{outline, holes, height, baseZ}` — nothing else in the document
 *  reaches `build_semio_mesh_snapshot`, so nothing else needs a stand-in here. Units are metres,
 *  matching this subset's committed snapshot fixtures (e.g. a 6×4 m, 0.25 m thick roof slab at
 *  `baseZ: 3.5`). */
const RECIPES: readonly Recipe[] = [
  // 👪️rectangles — the plainest possible extruded footprint, resting on the origin plane (baseZ 0).
  { id: "rect-unit-cube", family: "rectangles", tolerance: "fem-polygon-exact", notes: "1×1 m square, 1 m tall, resting on z=0 — the simplest possible FemSolid.", outline: [[0, 0], [1, 0], [1, 1], [0, 1]], holes: [], height: 1, baseZ: 0 },
  { id: "rect-roof-slab", family: "elevated", tolerance: "fem-polygon-exact", notes: "6×4 m, 0.25 m tall, baseZ 3.5 m — mirrors the committed create-solid/appends-an-extruded-roof-slab scenario exactly (same outline+height+baseZ).", outline: [[0, 0], [6, 0], [6, 4], [0, 4]], holes: [], height: 0.25, baseZ: 3.5 },
  { id: "rect-thin-plate", family: "rectangles", tolerance: "fem-polygon-exact", notes: "10×6 m footprint, 0.02 m tall, baseZ 0 — a high aspect-ratio plate.", outline: [[0, 0], [10, 0], [10, 6], [0, 6]], holes: [], height: 0.02, baseZ: 0 },
  ...
  { id: "polygon-l-shape", family: "polygons", tolerance: "fem-polygon-exact", notes: "an L-shaped (non-convex, re-entrant) footprint, 0.3 m tall, baseZ 0.", outline: [[0, 0], [3, 0], [3, 1], [1, 1], [1, 3], [0, 3]], holes: [], height: 0.3, baseZ: 0 },
  { id: "polygon-triangle", family: "polygons", tolerance: "fem-polygon-exact", notes: "a triangular footprint, 2 m tall column, baseZ 0.", outline: [[0, 0], [5, 0], [2.5, 4]], holes: [], height: 2, baseZ: 0 },

  // 👪️holes — a solid with holes punched through it. A through-hole in a linearly extruded prism is a
  // genuine topological fact (genus rises by one per hole), not merely a triangle count, exactly as the
  // mesh pilot's bored cube proved for a curved solid.
  { id: "solid-one-hole", family: "holes", tolerance: "fem-polygon-exact", notes: "4×2 m slab, 0.25 m tall, one 1×1 m rectangular hole, baseZ 0 — genus 1.", outline: [[0, 0], [4, 0], [4, 2], [0, 2]], holes: [[[1, 0.5], [2, 0.5], [2, 1.5], [1, 1.5]]], height: 0.25, baseZ: 0 },
  { id: "solid-two-holes", family: "holes", tolerance: "fem-polygon-exact", notes: "4×2 m slab, 0.25 m tall, two disjoint square holes, baseZ 0 — genus 2.", outline: [[0, 0], [4, 0], [4, 2], [0, 2]], holes: [[[0.5, 0.5], [1.2, 0.5], [1.2, 1.2], [0.5, 1.2]], [[2.8, 0.5], [3.5, 0.5], [3.5, 1.2], [2.8, 1.2]]], height: 0.25, baseZ: 0 },

  // 👪️elevated — proves the `baseZ` translation lands correctly: the same one-hole footprint, resting
  // at three different elevations rather than always at the origin.
  { id: "solid-one-hole-elevated-low", family: "elevated", tolerance: "fem-polygon-exact", notes: "solid-one-hole at baseZ 2 m (a suspended floor slab).", outline: [[0, 0], [4, 0], [4, 2], [0, 2]], holes: [[[1, 0.5], [2, 0.5], [2, 1.5], [1, 1.5]]], height: 0.25, baseZ: 2 },
  { id: "solid-one-hole-elevated-high", family: "elevated", tolerance: "fem-polygon-exact", notes: "solid-one-hole at baseZ 30 m (a high floor slab).", outline: [[0, 0], [4, 0], [4, 2], [0, 2]], holes: [[[1, 0.5], [2, 0.5], [2, 1.5], [1, 1.5]]], height: 0.25, baseZ: 30 },
  { id: "solid-one-hole-elevated-negative", family: "elevated", tolerance: "fem-polygon-exact", notes: "solid-one-hole at baseZ -5 m (a basement slab below the origin).", outline: [[0, 0], [4, 0], [4, 2], [0, 2]], holes: [[[1, 0.5], [2, 0.5], [2, 1.5], [1, 1.5]]], height: 0.25, baseZ: -5 },

  // 👪️scale — the SAME shape (the one-hole slab above) at three orders of magnitude either side of 1,
  // baseZ scaled along with it, to prove the weld grid and the gate are scale-relative rather than
  // tuned to metre-scale fixtures.
  { id: "scale-one-hole-1e-3", family: "scale", tolerance: "fem-polygon-exact", notes: "solid-one-hole scaled ×1e-3 (millimetre-scale slab), baseZ scaled too.", outline: [[0, 0], [0.004, 0], [0.004, 0.002], [0, 0.002]], holes: [[[0.001, 0.0005], [0.002, 0.0005], [0.002, 0.0015], [0.001, 0.0015]]], height: 0.00025, baseZ: 0.002 },
  { id: "scale-one-hole-1e3", family: "scale", tolerance: "fem-polygon-exact", notes: "solid-one-hole scaled ×1e3 (kilometre-scale slab), baseZ scaled too.", outline: [[0, 0], [4000, 0], [4000, 2000], [0, 2000]], holes: [[[1000, 500], [2000, 500], [2000, 1500], [1000, 1500]]], height: 250, baseZ: 2000 },
  { id: "scale-one-hole-1e6", family: "scale", tolerance: "fem-polygon-exact", notes: "solid-one-hole scaled ×1e6, baseZ scaled too.", outline: [[0, 0], [4e6, 0], [4e6, 2e6], [0, 2e6]], holes: [[[1e6, 5e5], [2e6, 5e5], [2e6, 1.5e6], [1e6, 1.5e6]]], height: 250000, baseZ: 2000000 },

  // 👪️degenerate — edge cases a corpus that only ever builds "nice" shapes would never surface.
  { id: "degenerate-hairline-height", family: "degenerate", tolerance: "fem-polygon-exact", notes: "a 2×1 m footprint at 1e-6 m height, baseZ 0 — height far below the weld grid's own absolute floor at metre scale, kept as a recorded edge case rather than dropped to make the corpus look clean.", outline: [[0, 0], [2, 0], [2, 1], [0, 1]], holes: [], height: 1e-6, baseZ: 0 },
  { id: "degenerate-sliver-outline", family: "degenerate", tolerance: "fem-polygon-exact", notes: "a 5×0.01 m sliver footprint, 0.1 m tall, baseZ 0 — an extreme aspect ratio in-plane rather than through the height.", outline: [[0, 0], [5, 0], [5, 0.01], [0, 0.01]], holes: [], height: 0.1, baseZ: 0 },
];

const RECIPE_DIRECTORIES = {
  "rect-unit-cube": "🧊️rect-unit-cube",
  "rect-roof-slab": "🏠️rect-roof-slab",
  "rect-thin-plate": "🍽️rect-thin-plate",
  "polygon-l-shape": "🪜️polygon-l-shape",
  "polygon-triangle": "🔺️polygon-triangle",
  "solid-one-hole": "🍩️solid-one-hole",
  "solid-two-holes": "🕶️solid-two-holes",
  "solid-one-hole-elevated-low": "⬇️solid-one-hole-elevated-low",
  "solid-one-hole-elevated-high": "🎈️solid-one-hole-elevated-high",
  "solid-one-hole-elevated-negative": "➖️solid-one-hole-elevated-negative",
  "scale-one-hole-1e-3": "🔬️scale-one-hole-1e-3",
  "scale-one-hole-1e3": "🔭️scale-one-hole-1e3",
  "scale-one-hole-1e6": "🌌️scale-one-hole-1e6",
  "degenerate-hairline-height": "🧵️degenerate-hairline-height",
  "degenerate-sliver-outline": "🪶️degenerate-sliver-outline",
} as const;

const EXPORTED_FILE_NAMES = { stl: "🧊️expected.stl", obj: "🗿️expected.obj" } as const;

const CARRIER_FIXTURES = {
  "create-node": { subset: "mesh", path: "🕸️mesh/🧫️fixtures/⚪️create-node" },
  "delete-node": { subset: "mesh", path: "🕸️mesh/🧫️fixtures/🕳️delete-node" },
  "create-element": { subset: "mesh", path: "🕸️mesh/🧫️fixtures/🧩️create-element" },
  "delete-element": { subset: "mesh", path: "🕸️mesh/🧫️fixtures/🗑️delete-element" },
  "replace-element": { subset: "mesh", path: "🕸️mesh/🧫️fixtures/♻️replace-element" },
  "create-material": { subset: "material", path: "🧱️material/🧫️fixtures/🌱️create-material" },
  "delete-material": { subset: "material", path: "🧱️material/🧫️fixtures/🗑️delete-material" },
  "replace-material": { subset: "material", path: "🧱️material/🧫️fixtures/🔁️replace-material" },
  "create-section": { subset: "mesh", path: "🕸️mesh/🧫️fixtures/📐️create-section" },
  "delete-section": { subset: "mesh", path: "🕸️mesh/🧫️fixtures/✂️delete-section" },
  "replace-section": { subset: "mesh", path: "🕸️mesh/🧫️fixtures/📏️replace-section" },
  "create-support": { subset: "boundary", path: "🛡️boundary/🧫️fixtures/🛡️create-support" },
  "delete-support": { subset: "boundary", path: "🛡️boundary/🧫️fixtures/🗑️delete-support" },
  "replace-support": { subset: "boundary", path: "🛡️boundary/🧫️fixtures/🔁️replace-support" },
  "create-load-case": { subset: "load", path: "🏋️load/🧫️fixtures/📋️create-load-case" },
  "delete-load-case": { subset: "load", path: "🏋️load/🧫️fixtures/🗑️delete-load-case" },
  "add-load": { subset: "load", path: "🏋️load/🧫️fixtures/➕️add-load" },
  "remove-load": { subset: "load", path: "🏋️load/🧫️fixtures/➖️remove-load" },
  "change-load-case-self-weight": { subset: "load", path: "🏋️load/🧫️fixtures/⚖️change-load-case-self-weight" },
...
async function generateOne(t: NonNullable<typeof toolkit>, recipe: Recipe, outDir: string): Promise<Record<string, unknown>> {
  const directoryName = RECIPE_DIRECTORIES[recipe.id as keyof typeof RECIPE_DIRECTORIES];
  if (directoryName === undefined) throw new Error(`no semantic fixture directory is registered for ${JSON.stringify(recipe.id)}`);
  const dir = join(outDir, directoryName);
  const files: { role: string; path: string; mediaType: string; sha256: string; bytes: number }[] = [];

  const result = extrudeSolid(t, recipe);
  ...
  if (!empty) {
    ...
    for (const [format, text] of Object.entries(exported) as [keyof typeof EXPORTED_FILE_NAMES, string][]) {
      const filename = EXPORTED_FILE_NAMES[format];
      write(join(dir, filename), text);
      files.push({ role: `expected-${format}`, path: `${FIXTURE_PATH_PREFIX}${directoryName}/${filename}`, mediaType: `model/${format}`, sha256: await contentDigest(text), bytes: Buffer.byteLength(text) });
    }
    ...
    measurements.measuredFrom = "🧊️expected.stl, re-imported and welded";
    ...
    measurements.expectedBaseZ = recipe.baseZ;
    ...
    // 📐️independent analytic cross-check: shoelace area of outline minus holes, times height, plus the
    // baseZ offset the bridge fn applies AFTER the extrusion.
    ...
    measurements.analyticVolume = footprintArea * recipe.height;
    ...
    measurements.analyticMinZ = recipe.baseZ;
    measurements.minZRelativeError = Math.abs(bounds.min[2] - recipe.baseZ) / Math.max(diagonal, Number.EPSILON);
  }
  ...
  return {
    ...
    mutation: "replace-solid-geometry",
    ...
    generator: {
      ...
      command: `bun ✏️s/🔌️plugins/🏗️fem/🗿️artifacts/🧊️3d/🏅️standards/🔖️1/🪆️subsets/🌐️any/🏭️generator/📜️script.ts generate --only ${recipe.id}`,
      seed: SEED,
...
async function main(argv: readonly string[]): Promise<number> {
  ...
  const outDir = process.env.SEMIO_FIXTURE_OUT ?? value("--out") ?? join(import.meta.dir, "..", "..", "🕸️mesh", "🧫️fixtures");
  ...
  if (command === "carrier" || command === "carrier-manifests") {
    ...
    if (command === "carrier") {
      const run = spawnSync(join(cargoTargetDir, "release", "generate"), [subsetsDir], { stdio: "inherit" });
      return run.status ?? 1;
    }
    const entries = [];
    for (const [kind, fixture] of Object.entries(CARRIER_FIXTURES)) {
      const files = [];
      for (const [role, name] of [["expected-before-json", "⏮️before.json"], ["expected-after-json", "⏭️after.json"]] as const) {
        const path = join(subsetsDir, fixture.path, name);
      ...
      entries.push({
        ...
        generator: { oracle: "json-rust-fem3d-carrier-reader", packageVersion: "0.12", engineFamily: "json-rust", engineVersion: "0.12", command: "bun ✏️s/🔌️plugins/🏗️fem/🗿️artifacts/🧊️3d/🏅️standards/🔖️1/🪆️subsets/🌐️any/🏭️generator/📜️script.ts carrier", platform: currentPlatform() },
        comparisonProfile: "semantic-fem3d-carrier-v1",
        reproducible: true,
        family: "mechanical",
        notes: `A deterministic fem3d model carrying at least two of every collection, with the ${kind} mutation applied as an edit to the JSON CARRIER and read back through json-rust — never through this repository's own mutation engine. Observability (after projection != before projection) is checked before a pair is written, and a pair that does not move is refused rather than committed.`,
      });
    }
