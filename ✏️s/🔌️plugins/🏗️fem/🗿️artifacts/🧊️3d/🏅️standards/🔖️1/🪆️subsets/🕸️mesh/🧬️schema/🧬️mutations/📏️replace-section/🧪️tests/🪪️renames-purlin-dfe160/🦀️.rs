...
fn the_refusal_is_the_declared_diagnostic() {
    ...
    assert_eq!(produced.diff(), &crate::artifacts::fem3d::diff::Fem3dDiff::default(), "replace-section/renames-purlin-dfe160: a refused mutation must carry the empty diff");
...
fn semantics_bind_the_declared_kind() {
    ...
    assert_eq!((semantics.verb, semantics.entity, semantics.kind, semantics.record), ("replace", "section", "replace-section", "ReplacedSection"), "replace-section/renames-purlin-dfe160: the fixture must be bound to replace-section's own descriptor");
}
