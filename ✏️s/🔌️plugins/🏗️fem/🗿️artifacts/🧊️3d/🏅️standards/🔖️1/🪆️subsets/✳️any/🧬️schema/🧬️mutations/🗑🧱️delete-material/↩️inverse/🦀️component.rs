...
//#region 🔖️Inverse
pub async fn inverse(payload: &DeleteMaterial, base: &Fem3dSnapshot) -> Vec<Fem3dMutation> {
    base.materials
        .iter()
        .find(|item| item.id == payload.id)
        .map(|item| vec![Fem3dMutation::CreateMaterial(create_material::mutation::CreateMaterial { material: item.clone() })])
        .unwrap_or_default()
}
//#endregion 🔖️Inverse
