//! 🧪️ `replace-load` fixture — `🪪️renames-load-a535c9`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-load/🪪️renames-load-a535c9/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-load/🪪️renames-load-a535c9/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-load/🪪️renames-load-a535c9/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️replace-load/🪪️renames-load-a535c9/🎯️outcome/🔣️.json");
...
fn rejection_leaves_the_document_at_the_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "replace-load/renames-load-a535c9: applied state differs from committed after-snapshot");
    assert_eq!(snapshot, base, "replace-load/renames-load-a535c9: a refused mutation must leave every one of the nine members untouched");
}
...
fn inverse_has_the_declared_shape() {
    ...
    for step in &inverse {
        assert!(matches!(step, Fem3dMutation::ReplaceLoad(_)), "replace-load/renames-load-a535c9: the inverse of replace-load is a ReplaceLoad, got {step:?}");
    }
    assert!(inverse.len() <= 1, "replace-load/renames-load-a535c9: this kind never emits more than one undo step, got {inverse:?}");
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::DslValue::as_str), Some("rejected"), "replace-load/renames-load-a535c9: this vector declares a rejected outcome");
    let produced = <Fem3dMutation as protocol::Mutation<Fem3dSnapshot>>::diff(&mutation(), &before());
    let message = produced.messages().first().expect("a rejected outcome carries a diagnostic");
    assert_eq!(outcome.get("code").and_then(dsl::DslValue::as_str), Some(message.code.0.as_str()), "replace-load/renames-load-a535c9: the declared code must match the emitted one");
    ...
    assert_eq!(declared_path, message.target, "replace-load/renames-load-a535c9: the declared path must match the emitted target");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-load/renames-load-a535c9: committed {label} JSON is not canonical");
    }
    let reencoded = dsl::ToValue::to_value(&mutation());
    let original: dsl::DslValue = dsl::json::from_json_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "replace-load/renames-load-a535c9: committed mutation JSON is not canonical");
}
...
fn semantics_bind_the_declared_kind() {
    ...
    assert_eq!((semantics.verb, semantics.entity, semantics.kind, semantics.record), ("replace", "load", "replace-load", "ReplacedLoad"), "replace-load/renames-load-a535c9: the fixture must be bound to replace-load's own descriptor");
}
