...
//! `serde_json::to_value(snapshot)` — the real structured tree, every `Fem3dSnapshot` field. So all
...
pub fn apply(kind: &str, doc: &Value) -> Result<Value, String> {
    ...
    match kind {
        "create-node" => array(&mut doc, "nodes").push(json!({"id": "n4", "x": 0.0, "y": 3.0, "z": 2.5})),
        "delete-node" => {
        ...
        "create-material" => array(&mut doc, "materials").push(json!({"id": "m3", "name": "GL24h", "e": 11500000000.0, "g": 720000000.0, "nu": 0.2, "rho": 420.0})),
        "delete-material" => {
            array(&mut doc, "materials").retain(|m| m.get("id").and_then(Value::as_str) != Some("m2"));
        }
        "replace-material" => {
            let materials = array(&mut doc, "materials");
            materials[0] = json!({"id": "m1", "name": "S355", "e": 210000000000.0, "g": 81000000000.0, "nu": 0.3, "rho": 7850.0});
        }
        "create-section" => array(&mut doc, "sections").push(json!({"id": "s3", "name": "HEB200", "area": 0.00781, "iy": 0.0000570, "iz": 0.0000201, "j": 0.000000594})),
        "delete-section" => {
            array(&mut doc, "sections").retain(|s| s.get("id").and_then(Value::as_str) != Some("s2"));
        }
        "replace-section" => {
            let sections = array(&mut doc, "sections");
            sections[0] = json!({"id": "s1", "name": "IPE220", "area": 0.00334, "iy": 0.0000272, "iz": 0.00000205, "j": 0.0000000907});
        }
        "create-support" => array(&mut doc, "supports").push(json!({"id": "sup3", "nodeId": "n3", "fixed": ["Tx", "Tz"]})),
        "delete-support" => {
