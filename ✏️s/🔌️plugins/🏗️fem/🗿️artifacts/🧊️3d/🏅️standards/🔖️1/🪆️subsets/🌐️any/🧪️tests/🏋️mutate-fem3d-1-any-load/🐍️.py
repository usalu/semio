...
REJECT_VECTORS = (
    "sw-no-such-case-bfe5bc",
    "no-such-combo-f42cd6",
    "dup-load-id-4f4a0a",
    "no-such-member-3fe6e9",
    "no-such-load-5bab2d",
    "dangling-solid-5e04d9",
...
def uri_in(ctx, needle):
    ...
    for step in ctx.scenario["steps"]:
        for token in step["text"].split():
            if token.startswith(("asset://", "shared://🏋️mutate-fem3d-1-any-load/", "shared://")) and needle in token:
                return token
