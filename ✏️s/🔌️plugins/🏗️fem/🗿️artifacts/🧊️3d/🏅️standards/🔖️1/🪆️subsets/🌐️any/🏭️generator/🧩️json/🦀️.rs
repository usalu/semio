//! 🏗️ The `fem3d@1/any` non-geometric mutation vocabulary, expressed over this subset's own JSON
...
pub fn build_seed() -> JsonValue {
    literal(r#"{
        "nodes": [
            ...
            {"id": "n3", "x": 4.0, "y": 3.0, "z": 2.5}
        ],
        "elements": [
            {"kind": "bar", "id": "e1", "start": "n1", "end": "n2", "materialId": "m1", "sectionId": "s1"},
            {"kind": "frame", "id": "e2", "start": "n2", "end": "n3", "materialId": "m2", "sectionId": "s2", "roll": 0.0}
        ],
        "materials": [
            {"id": "m1", "name": "S235", "e": 210000000000.0, "g": 81000000000.0, "nu": 0.3, "rho": 7850.0},
            {"id": "m2", "name": "C30/37", "e": 33000000000.0, "g": 13750000000.0, "nu": 0.2, "rho": 2500.0}
        ],
        "sections": [
            {"id": "s1", "name": "IPE200", "area": 0.00285, "iy": 0.0000194, "iz": 0.00000142, "j": 0.0000000697},
        ...
        "supports": [
            ...
            {"id": "sup2", "nodeId": "n2", "fixed": ["Tz"]}
        ],
        "loadCases": [
            {"id": "lc1", "name": "dead", "loads": [
                {"kind": "nodal", "id": "l1", "nodeId": "n3", "dof": "Tz", "value": -12000.0},
                {"kind": "memberUdl", "id": "l2", "elementId": "e1", "wx": 0.0, "wy": 0.0, "wz": -3500.0}
            ], "selfWeight": true},
            {"id": "lc2", "name": "live", "loads": [
                {"kind": "area", "id": "l3", "solidId": "sol1", "pressure": -2500.0}
            ], "selfWeight": false}
        ],
        "combinations": [
...
pub fn apply(kind: &str, doc: &JsonValue) -> Result<JsonValue, String> {
    ...
    match kind {
        "create-node" => array(&mut doc, "nodes")?.push(literal(r#"{"id": "n4", "x": 0.0, "y": 3.0, "z": 2.5}"#)),
        "delete-node" => {
        ...
        "create-material" => array(&mut doc, "materials")?.push(literal(r#"{"id": "m3", "name": "GL24h", "e": 11500000000.0, "g": 720000000.0, "nu": 0.2, "rho": 420.0}"#)),
        "delete-material" => {
            array(&mut doc, "materials")?.retain(|m| m["id"] != "m2");
        }
        "replace-material" => {
            let materials = array(&mut doc, "materials")?;
            materials[0] = literal(r#"{"id": "m1", "name": "S355", "e": 210000000000.0, "g": 81000000000.0, "nu": 0.3, "rho": 7850.0}"#);
        }
        "create-section" => array(&mut doc, "sections")?.push(literal(r#"{"id": "s3", "name": "HEB200", "area": 0.00781, "iy": 0.0000570, "iz": 0.0000201, "j": 0.000000594}"#)),
        "delete-section" => {
            array(&mut doc, "sections")?.retain(|s| s["id"] != "s2");
        }
        "replace-section" => {
            let sections = array(&mut doc, "sections")?;
            sections[0] = literal(r#"{"id": "s1", "name": "IPE220", "area": 0.00334, "iy": 0.0000272, "iz": 0.00000205, "j": 0.0000000907}"#);
        }
        "create-support" => array(&mut doc, "supports")?.push(literal(r#"{"id": "sup3", "nodeId": "n3", "fixed": ["Tx", "Tz"]}"#)),
        "delete-support" => {
        ...
        "replace-node" => {
            ...
            nodes[2] = literal(r#"{"id": "n3", "x": 4.0, "y": 3.0, "z": 3.0}"#);
        }
        "replace-load" => {
            let cases = array(&mut doc, "loadCases")?;
            let loads = array(&mut cases[0], "loads")?;
            loads[1] = literal(r#"{"kind": "memberUdl", "id": "l2", "elementId": "e1", "wx": 0.0, "wy": 0.0, "wz": -5250.0}"#);
        }
        "change-load-case-name" => {
            let cases = array(&mut doc, "loadCases")?;
            cases[1]["name"] = "imposed".into();
        }
        "replace-combination" => {
            let combinations = array(&mut doc, "combinations")?;
            combinations[0] = literal(r#"{"id": "c1", "name": "ULS", "terms": [{"caseId": "lc1", "factor": 1.35}, {"caseId": "lc2", "factor": 1.05}]}"#);
        }
...
pub fn project(bytes: &[u8]) -> Result<JsonValue, String> {
    ...
    for key in ["nodes", "elements", "solids", "materials", "sections", "supports", "loadCases", "combinations", "analysis"] {
