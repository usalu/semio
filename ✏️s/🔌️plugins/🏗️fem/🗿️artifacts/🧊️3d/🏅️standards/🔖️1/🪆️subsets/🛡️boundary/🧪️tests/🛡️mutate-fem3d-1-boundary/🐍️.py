...
subset's typed mutations (`create-support`, `delete-support`, `replace-support`), in Python, serving as this case's differential oracle.
...
KINDS = ("create-support", "delete-support", "replace-support")
...
def uri_in(ctx, needle):
    ...
    for step in ctx.scenario["steps"]:
        for token in step["text"].split():
            if token.startswith(("asset://", "shared://🛡️mutate-fem3d-1-boundary/", "shared://")) and needle in token:
                return token
