#!/usr/bin/env python3
"""🧊 An INDEPENDENT second implementation of the `s.fem.fem3d` structural model and all twenty-five
...
MODEL, not the analysis: nine id-keyed collections — nodes, elements, materials, sections, solids,
...
* ``🏅️standards/🔖️1/🪆️subsets/✳️any/🧬️schema/📸️snapshot/🔣️component.json`` — `Fem3dSnapshot` is
...
is a verbatim copy of the SNAPSHOT schema with `title` changed to `Fem3dMutation`; and in the
snapshot schema itself every one of `FemNode`, `FemElement`, `FemSolid`, `FemMaterial`,
...
names are the statement: `removes-the-column-head-node-under-a-live-frame`,
`removes-the-roof-slab-and-keeps-its-material`, `removes-the-bracing-bar-and-leaves-the-frame`,
`removes-the-serviceability-combination-and-keeps-both-cases`,
`removes-the-wind-case-together-with-its-load`. A delete removes exactly its own entity; only a load
...
"""🗂️ The nine members `Fem3dSnapshot` declares — and the cross-language projection."""
...
REPLACEABLE = ("element", "solid", "material", "section", "support")
...
"""🧱️ The members each record carries, as the twenty-five committed vectors spell them. `elements`
is absent here on purpose — its shape depends on the element kind, see [`ELEMENTS`]."""
