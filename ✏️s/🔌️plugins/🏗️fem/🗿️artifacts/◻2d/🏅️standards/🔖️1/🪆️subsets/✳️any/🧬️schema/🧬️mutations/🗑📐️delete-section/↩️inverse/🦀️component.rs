...
//#region 🔖️Inverse
pub async fn inverse(payload: &DeleteSection, base: &Fem2dSnapshot) -> Vec<Fem2dMutation> {
    base.sections
        .iter()
        .find(|item| item.id == payload.id)
        .map(|item| vec![Fem2dMutation::CreateSection(create_section::mutation::CreateSection { section: item.clone() })])
        .unwrap_or_default()
}
//#endregion 🔖️Inverse
