...
//#region 🔖️Inverse
pub async fn inverse(payload: &DeleteCombination, base: &Fem2dSnapshot) -> Vec<Fem2dMutation> {
    base.combinations
        .iter()
        .find(|item| item.id == payload.id)
        .map(|item| vec![Fem2dMutation::CreateCombination(create_combination::mutation::CreateCombination { combination: item.clone() })])
        .unwrap_or_default()
}
//#endregion 🔖️Inverse
