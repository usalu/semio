...
pub async fn serialize(snapshot: &Fem2dSnapshot) -> Result<CsvSnapshot, store::TextError> {
    Ok(CsvSnapshot {
        schema: STDIO_CSV_DOCUMENT_SCHEMA.into(),
        has_header: true,
        records: vec![CsvRecord { fields: vec![CsvField { value: "payload".into(), quoted: false }] }, CsvRecord { fields: vec![CsvField { value: <Fem2dSnapshot as store::ArtifactDsl>::print_dsl(snapshot), quoted: true }] }],
    })
}
