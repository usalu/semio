...
//#region 🔖️Inverse
pub async fn inverse(payload: &AddLoad, base: &Fem2dSnapshot) -> Vec<Fem2dMutation> {
    if base.load_cases.iter().any(|case| case.id == payload.case_id) {
        vec![Fem2dMutation::RemoveLoad(remove_load::mutation::RemoveLoad { case_id: payload.case_id.clone(), load_id: load_id(&payload.load).to_string() })]
    } else {
        Vec::new()
    }
}
