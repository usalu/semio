...
//#region 🔖️Entities
/** 📍️ A structural node in plan (x, y in meters). Mirrors Rust `FemNode` (`🦀️component.rs`). */
...
/** 🔒️ A DOF tag mirroring the FEM 2D degrees of freedom. Mirrors Rust `FemDof` (`🦀️component.rs`). */
 ...
 * Mirrors Rust `FemMaterial` (`🦀️component.rs`). */
 ...
 * Mirrors Rust `FemLoad` (`🦀️component.rs`), tagged on `kind`. */
export type FemLoad =
 ...
 * Mirrors Rust `FemLoadCase` (`🦀️component.rs`). */
 ...
 * `FemCombinationTerm` (`🦀️component.rs`). */
 ...
 * Mirrors Rust `FemAnalysisSettings` (`🦀️component.rs`). */
