...
/// 📦️ Encodes a `LayoutDocument` to its binary pack form.
pub fn encode(document: &LayoutDocument) -> Vec<u8> {
    store::DocumentPack::encode_pack(document)
}

/// 📖️ Decodes a `LayoutDocument` from its binary pack form.
pub fn decode(bytes: &[u8]) -> Result<LayoutDocument, PackError> {
    <LayoutDocument as store::DocumentPack>::decode_pack(bytes)
}
...
mod tests {
    ...
    use crate::artifacts::layout::{CharacterStyle, Frame, GridSettings, Layer, LayoutBounds, Page, PageColumns, PageMargins, PageOverride, LAYOUT_FIXTURE_SCHEMA};
