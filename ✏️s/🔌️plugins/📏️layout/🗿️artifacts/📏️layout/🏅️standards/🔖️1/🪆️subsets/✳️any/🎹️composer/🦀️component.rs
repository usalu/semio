//! 🎹️ LayoutComposer (1/✳️any) — analyzer + builder glued. Reads native `s.layout` sources
//! plus any of: stdio.dwg, stdio.dxf, stdio.json, stdio.pdf, stdio.png, stdio.svg. Writes one `s.layout` (1/✳️any) snapshot.
...
use crate::artifacts::layout::standards::v1::subsets::any::analyzer::LayoutAnalyzer;
...
impl ArtifactComposer for LayoutComposer {
    ...
    fn compose(sources: &[ComposeSource]) -> Result<Composition<Self::Snapshot>, ComposeError> {
        for source in sources {
            ...
            if source.dialect == DEP_DXF {
                ...
                if let Ok(snapshot) = crate::artifacts::layout::io::import::deserializers::artifacts::dxf::v_r12::any::deserialize_bytes(&bytes) {
                    return Ok(Composition { snapshot, confidence: semio_framework_plugin::IoConfidence::Medium, diagnostics: Vec::new() });
                }
            }
            if source.dialect == DEP_JSON {
                ...
                if let Ok(snapshot) = crate::artifacts::layout::io::import::deserializers::artifacts::json::v_rfc8259::any::deserialize_bytes(&bytes) {
                    return Ok(Composition { snapshot, confidence: semio_framework_plugin::IoConfidence::Medium, diagnostics: Vec::new() });
                }
            }
            if source.dialect == DEP_PDF {
                ...
                if let Ok(snapshot) = crate::artifacts::layout::io::import::deserializers::artifacts::pdf::v1_4::any::deserialize_bytes(&bytes) {
                    return Ok(Composition { snapshot, confidence: semio_framework_plugin::IoConfidence::Medium, diagnostics: Vec::new() });
                }
            }
            if source.dialect == DEP_PNG {
                ...
                if let Ok(snapshot) = crate::artifacts::layout::io::import::deserializers::artifacts::png::v1_2::any::deserialize_bytes(&bytes) {
                    return Ok(Composition { snapshot, confidence: semio_framework_plugin::IoConfidence::Medium, diagnostics: Vec::new() });
                }
            }
            if source.dialect == DEP_SVG {
                ...
                if let Ok(snapshot) = crate::artifacts::layout::io::import::deserializers::artifacts::svg::v1_1::any::deserialize_bytes(&bytes) {
                    return Ok(Composition { snapshot, confidence: semio_framework_plugin::IoConfidence::Medium, diagnostics: Vec::new() });
                }
            }

        }
        Err(ComposeError { message: "LayoutComposer: no source in a known read dialect".into(), diagnostics: Vec::new() })
    }
}
