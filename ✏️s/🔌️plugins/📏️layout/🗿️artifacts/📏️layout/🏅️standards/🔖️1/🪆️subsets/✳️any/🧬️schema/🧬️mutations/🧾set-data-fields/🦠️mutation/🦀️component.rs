//! 🧾 Retired. `set-data-fields`'s generic `SetDataFields{json}` variant was replaced by the
//! semantic `change-data-fields` triad (`🧬️mutations/🧾change-data-fields/`) — see
//! `.🦑️repo/🎫️tickets/🎆️26/🌙️08/☀️12/SEMANTIC-MUTATIONS-OVERHAUL/📓️taxonomy.md`. This directory has
//! no active payload structs and no `SetDataFields` variant remains in `LayoutMutation`.
//!
