...
pub fn handle(payload: &CanvasDragOver, _doc: &ArtifactView<'_, LayoutSnapshot>, cfg: &ConfigView<'_, NoConfig>) -> Result<Emit<LayoutMutation, NoConfigMutation>, Fault> {
    let blueprint = surface_is_blueprint(payload.surface_id.as_deref());
    if !blueprint {
        return Ok(Emit::default());
    }
    let _world = screen_to_world_for_surface(&current(cfg), payload.x, payload.y, payload.width, payload.height);
    Ok(Emit::default())
}
