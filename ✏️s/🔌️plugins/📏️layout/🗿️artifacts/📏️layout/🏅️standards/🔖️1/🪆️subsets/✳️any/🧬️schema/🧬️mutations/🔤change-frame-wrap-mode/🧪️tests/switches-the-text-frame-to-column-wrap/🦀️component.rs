//! 🧪️ `change-frame-wrap-mode` fixture — `switches-the-text-frame-to-column-wrap`.
//!
//! Proves the Text-only `wrap_mode` string is replaced and the column count is left alone.
//!
...
fn before() -> LayoutSnapshot {
    serde_json::from_str(BEFORE).expect("change-frame-wrap-mode/switches-the-text-frame-to-column-wrap: before snapshot decodes")
}
fn expected_after() -> LayoutSnapshot {
    serde_json::from_str(AFTER).expect("change-frame-wrap-mode/switches-the-text-frame-to-column-wrap: after snapshot decodes")
}
fn mutation() -> LayoutMutation {
    serde_json::from_str(MUTATION).expect("change-frame-wrap-mode/switches-the-text-frame-to-column-wrap: mutation decodes")
}
fn applied() -> LayoutSnapshot {
    let base = before();
    mutation().diff(&base).diff().apply(&base).expect("change-frame-wrap-mode applies to its committed before-snapshot")
}

/// ▶️ `change-frame-wrap-mode` is a Text-variant-specific field patch; `columns` belongs to a different mutation.
#[semio_framework_async_macros::async_test]
async fn switches_the_wrap_mode_without_recolumning() {
    let after = applied();
    ...
    assert_eq!(after, expected_after(), "change-frame-wrap-mode/switches-the-text-frame-to-column-wrap: applied state differs from the committed after-snapshot");
}

/// ↩️ The inverse is a `change-frame-wrap-mode` carrying BASE's wrap mode string.
#[semio_framework_async_macros::async_test]
async fn inverse_restores_the_box_wrap_mode() {
    let base = before();
    let inverse = mutation().inverse(&base);
    assert_eq!(inverse.len(), 1, "change-frame-wrap-mode inverts to exactly one step on a text frame");
    match &inverse[0] {
    ...
    for step in &inverse {
        snapshot = step.diff(&snapshot).diff().apply(&snapshot).expect("change-frame-wrap-mode/switches-the-text-frame-to-column-wrap: inverse step applies");
    }
    assert_eq!(snapshot, base, "change-frame-wrap-mode/switches-the-text-frame-to-column-wrap: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-frame-wrap-mode/switches-the-text-frame-to-column-wrap: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "change-frame-wrap-mode/switches-the-text-frame-to-column-wrap: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome matches what `change-frame-wrap-mode`'s own diff builder actually produces.
#[semio_framework_async_macros::async_test]
async fn declared_outcome_holds() {
    let declared: serde_json::Value = serde_json::from_str(OUTCOME).expect("outcome decodes");
    assert_eq!(declared.get("status").and_then(serde_json::Value::as_str), Some("applied"), "change-frame-wrap-mode/switches-the-text-frame-to-column-wrap: this fixture declares an applied outcome");
    let base = before();
...
/// 🔺️ The sparse delta `change-frame-wrap-mode` produces is exactly the committed diff — the most load-bearing
/// assertion in the fixture, because it pins WHICH fields the mutation may touch, not merely that the
/// end state matches. Here only `wrap_mode` is populated — the SAME `FramePatch` type also carries `columns`, and the diff proves this verb does not touch it.
#[semio_framework_async_macros::async_test]
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-frame-wrap-mode/switches-the-text-frame-to-column-wrap: change-frame-wrap-mode must emit a nested frame patch populating only wrap_mode");
}
