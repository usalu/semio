...
fn layout_inspection_summary_matches_the_json_oracle() {
    ...
    for row in fixture["cases"].as_array().expect("locales") {
        let config = LayoutConfig { active_page_id: fixture["activePage"].as_str().expect("active page").into(), ..LayoutConfig::default() };
        ...
        let node = render(&snapshot, &config, crate::editor::layout::terminology::layout_labels(&view_state)).expect("semantic inspector");
