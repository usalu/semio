//! 🧪️ `change-frame-stroke` fixture — `🖊️adds-a-stroke-to-the-rect-frame`.
//!
...
fn before() -> LayoutSnapshot {
    dsl::os_pack::from_json_str(BEFORE).expect("change-frame-stroke/adds-a-stroke-to-the-rect-frame: before snapshot decodes")
}
fn expected_after() -> LayoutSnapshot {
    dsl::os_pack::from_json_str(AFTER).expect("change-frame-stroke/adds-a-stroke-to-the-rect-frame: after snapshot decodes")
}
