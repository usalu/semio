    ...
    fn handle(&self, command: &LayoutCommand, doc: &DocumentView<'_, LayoutDocument>, cfg: &ConfigView<'_, LayoutConfig>) -> Result<Emit<LayoutOperation, LayoutConfigOperation>, Fault> {
        ...
        match command {
            ...
            LayoutCommand::CanvasPointerMove { surface_id, x, y, width, height } => {
                ...
                Ok(Emit::config(vec![LayoutConfigOperation::SetHover { id: hit_test_at(document, config, *x, *y, *width, *height, blueprint) }])
            }
            ...
            LayoutCommand::CanvasDragOver { surface_id, kind, x, y, width, height } => {
                ...
                Ok(Emit::config(vec![LayoutConfigOperation::SetDropPreview { preview: LayoutDropPreviewState { kind: kind.clone(), x: wx, y: wy } }])
            }
            ...
            LayoutCommand::SetCamera { surface_id, camera } => {
                let blueprint = surface_is_blueprint(surface_id.as_deref());
                if blueprint {
                    Ok(Emit::config(vec![LayoutConfigOperation::SetCamera { camera: camera.clone() }])
                } else {
                    Ok(Emit::config(vec![LayoutConfigOperation::SetPreviewCamera { camera: camera.clone() }])
                }
            }
            ...
            LayoutCommand::PatchFrame { frame_id, page_id, field, value } => {
                ...
                match field.as_str() {
                    ...
                    "fill" | "stroke" => {
                        ...
                        Ok(Emit::operations(vec![LayoutOperation::PatchFrame { page_id, frame_id: frame_id.clone(), patch }])
                    }
                    ...
                    "storyContent" => {
                        ...
                        match story_id {
                            Some(story_id) if document.stories.iter().any(|story| story.id == story_id) => {
                                Ok(Emit::operations(vec![LayoutOperation::Stories(CollectionOperation::Patch { id: story_id, patch: TextStoryPatch { content: Some(value.clone()) } })])
                            }
                            _ => Ok(Emit::default()),
                        }
                    }
            ...
            LayoutCommand::CanvasDrop { surface_id, kind, x, y, width, height } => {
                let blueprint = surface_is_blueprint(surface_id.as_deref());
                let clear_preview = Ok(Emit::config(vec![LayoutConfigOperation::SetDropPreview { preview: LayoutDropPreviewState::default() }]);
                if !blueprint {
            ...
            LayoutCommand::ExportPdf { page_id } => {
                ...
                match export_document_pdf(document, &page_id) {
                    Ok(bytes) => {
                        Ok(Emit::effect(HostEffect::DownloadMediaExport { filename: format!("{page_id}.pdf"), mime_type: "application/pdf".into(), data: base64::engine::general_purpose::STANDARD.encode(bytes), encoding: Some("base64".into()) })
                    }
                    Err(_) => Ok(Emit::default()),
                }
            }
