...
// 🏭️ Language-neutral production mutation bridge for every subset under `✏️s/🔌️plugins/📏️layout`.
//
...
function main(argv: readonly string[]): number {
  ...
  const built = spawnSync("cargo", ["run", "--quiet", "--offline", "--bin", "semio-layout-mutation-bridge", "--", command, artifact, standard, subset, ...surface], {
    cwd: import.meta.dir,
    encoding: "utf8",
  });
  if (built.status !== 0) {
