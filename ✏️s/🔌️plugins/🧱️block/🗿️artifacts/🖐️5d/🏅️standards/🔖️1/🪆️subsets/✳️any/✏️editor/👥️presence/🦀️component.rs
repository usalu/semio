...
impl protocol::MutationDiff<Block5dPresence> for Block5dPresence {
    async fn apply(&self, _base: &Block5dPresence) -> protocol::MutationApplyResult<Block5dPresence> {
        Ok({
            self.clone()
        })
    }
...
impl Mutation<Block5dPresence> for Block5dPresenceMutation {
    ...
    async fn diff(&self, _base: &Block5dPresence) -> protocol::MutationOutcome<Block5dPresence> {
        protocol::MutationOutcome::new(match self {
            Self::Snapshot { presence } => presence.clone(),
        })
    }

    async fn inverse(&self, base: &Block5dPresence) -> Vec<Self> {
