//! 🚪️ block5d → txt — foreign `Serializer<Block5dSnapshot>` on the framework's `io_mechanism`
...
//! fixtures carry and `<Block5dSnapshot as store::ArtifactDsl>::parse_dsl` reads back, so the hop is
...
pub fn dsl_text(from: &Block5dSnapshot) -> String {
    <Block5dSnapshot as store::ArtifactDsl>::print_dsl(from)
}

/// 🧵️ `s.block.block5d@1/*` → `s.stdio.txt@utf-8/*`.
pub struct Block5dIntoTxt;
