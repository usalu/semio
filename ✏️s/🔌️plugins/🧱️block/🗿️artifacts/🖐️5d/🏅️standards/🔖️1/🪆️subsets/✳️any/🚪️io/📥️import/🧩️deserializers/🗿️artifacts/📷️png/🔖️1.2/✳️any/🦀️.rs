//! 🚪️ block5d ← png — foreign `Deserializer<Block5dSnapshot>` that HONESTLY REFUSES.
//!
...
/// 🧩️ `s.stdio.png@1.2/*` → `s.block.block5d@1/*` — always `Err`, see this file's module doc.
pub struct PngIntoBlock5d;

impl Deserializer<Block5dSnapshot> for PngIntoBlock5d {
    const FROM: Dialect = PNG_DIALECT;
    const FIDELITY: IoFidelity = IoFidelity::Lossy;
    async fn deserialize(_payload: &IoPayload) -> IoResult<Block5dSnapshot> {
        Err(IoError {
            message: "png import not supported for a part kind definition: a PNG carries pixels only — it has no kind identity, no handle/vortex/grip catalog and no compatibility rules to build a kind definition from".to_string(),
            diagnostics: Vec::new(),
        })
    }
}
