...
fn project(document: &Block5dSnapshot, windows: &TreeWindows<'_>) -> String {
    ...
    let node = render(document, labels, windows).expect("inspector renders");
...
fn window_view(open: Option<bool>, offset: u32, rows: u32) -> ViewModel {
    ViewModel { tree_windows: vec![TreeWindowRequest { body_key: BLOCK5D_BODY_INSPECTOR.into(), node_key: BLOCK5D_INSPECTOR_SUMMARY.into(), open, offset, rows }], ..Default::default() }
}
...
fn the_inspector_summary_stamps_its_total_and_publishes_no_continuation_row() {
    let json = project(&Block5dSnapshot::default(), &TreeWindows::unhosted());
    let (total, offset, rows) = container_of(&json, BLOCK5D_INSPECTOR_SUMMARY);
    assert_eq!(total, 3, "name, label and the grip count: {json}");
    assert_eq!(offset, 0, "a first paint starts at zero");
    assert_eq!(rows.len(), 3, "a three-row section fits one viewport: {json}");
...
fn a_closed_inspector_summary_stamps_its_total_with_no_rows() {
    ...
    let json = project(&Block5dSnapshot::default(), &TreeWindows::for_body(&view, BLOCK5D_BODY_INSPECTOR));
    let (total, offset, rows) = container_of(&json, BLOCK5D_INSPECTOR_SUMMARY);
    assert_eq!(total, 3, "a closed section still states its extent: {json}");
    assert_eq!(offset, 0);
    assert!(rows.is_empty(), "a closed section materialises nothing: {json}");
    assert!(!json.contains("Name"), "…not even its first field: {json}");
}
...
fn an_inspector_window_request_materialises_exactly_its_slice() {
    let view = window_view(Some(true), 1, 2);
    let json = project(&Block5dSnapshot::default(), &TreeWindows::for_body(&view, BLOCK5D_BODY_INSPECTOR));
    let (total, offset, rows) = container_of(&json, BLOCK5D_INSPECTOR_SUMMARY);
    assert_eq!(total, 3);
    assert_eq!(offset, 1, "the stamped offset is the requested one: {json}");
    assert_eq!(rows, vec!["block5d-play-inspector.label".to_string(), "block5d-play-inspector.grip-count".to_string()], "exactly fields [1, 3): {json}");
}
