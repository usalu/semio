//! 🧪️ `move-camera3d` fixture — `orbits-3d-camera`: the 3D camera moves to (5, 5, 5) looking at (0, 1, 0)
//!
...
async fn applies_to_committed_after() {
    ...
    apply_block5d_mutation(&mut snapshot, &mutation()).expect("move-camera3d applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "move-camera3d/orbits-3d-camera: applied state differs from committed after-snapshot");
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "move-camera3d/orbits-3d-camera: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "move-camera3d/orbits-3d-camera: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "move-camera3d/orbits-3d-camera: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "move-camera3d/orbits-3d-camera: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "move-camera3d/orbits-3d-camera: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "move-camera3d/orbits-3d-camera: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("move-camera3d/orbits-3d-camera: unknown outcome status {other:?}"),
    }
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "move-camera3d/orbits-3d-camera: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "move-camera3d/orbits-3d-camera: committed diff JSON is not canonical");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "move-camera3d/orbits-3d-camera: committed diff did not carry before to after");
}
