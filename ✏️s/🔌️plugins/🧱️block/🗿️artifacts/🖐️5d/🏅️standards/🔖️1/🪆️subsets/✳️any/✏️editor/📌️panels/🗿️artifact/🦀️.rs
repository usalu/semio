...
pub fn render(definition: &Block5dSnapshot, labels: &Block5dLabels) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::BuiltNode> {
    ...
    builder
        .section_or_placeholder("block5d-play-document.grip-kinds", Some(ui_label(labels.grip_kinds.as_str())?), true, grip_kind_items, ui_label(labels.no_grip_kinds.as_str())?)?
        .section_or_placeholder("block5d-play-document.grips", Some(ui_label(labels.grips.as_str())?), true, grip_items, ui_label(labels.no_grips.as_str())?)?
