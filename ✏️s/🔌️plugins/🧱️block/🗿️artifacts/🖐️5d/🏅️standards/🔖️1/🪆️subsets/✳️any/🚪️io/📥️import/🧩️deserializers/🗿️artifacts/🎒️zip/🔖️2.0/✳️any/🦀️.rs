//! 🚪️ block5d ← zip — foreign `Deserializer<Block5dSnapshot>` on the framework's `io_mechanism`
//! channel: decodes a REAL zip 2.0 container (stdio's own `decode_zip`) and rebuilds the snapshot
//! from its `snapshot.block5d.semio` member, falling back to `snapshot.json`. Exact inverse of the sibling
...
pub struct ZipIntoBlock5d;

impl Deserializer<Block5dSnapshot> for ZipIntoBlock5d {
