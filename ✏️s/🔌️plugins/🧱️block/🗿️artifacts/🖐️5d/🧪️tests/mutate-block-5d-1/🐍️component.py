...
them reads `.dsl.semio`. The sibling `mutate-block-2d-1` settled the same question the same way over
...
* the forty-one committed `(before, mutation, diff, outcome, after)` quintets, which are the only
  statement of the one thing this vocabulary does not share with its siblings: that `update-part2d`
...
"""🖌️ `update-part2d` rebuilds the 2d facet from these six arguments, in this order, dropping every
...
"""🧊 `update-part3d` rebuilds the 3d facet from both of its arguments; neither is optional."""
...
`update-part2d` legitimately changes which members the 2d facet holds."""
