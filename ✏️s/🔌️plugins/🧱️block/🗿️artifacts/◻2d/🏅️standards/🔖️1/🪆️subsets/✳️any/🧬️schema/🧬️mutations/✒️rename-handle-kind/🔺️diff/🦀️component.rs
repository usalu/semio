//! 🔺️ Sparse diff builder for `RenameHandleKind` — real handcrafted delta, never apply-then-capture.
...
//#region 🔖️Diff
pub async fn diff(payload: &super::mutation::RenameHandleKind, base: &Block2dSnapshot) -> protocol::MutationOutcome<Block2dDiff> {
    ...
    let replacement = Block2dHandleKind { name: payload.new_name.clone(), ..existing.clone() };
