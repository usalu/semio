...
//! migration added to `🚪️io/🦀️.rs`. Unlike trinity, `🧱️block/🚪️io/` is NOT excluded from
...
//! `🚪️io/🦀️.rs` uses) per format leaf, replacing the OLD `ComposerEntry`/`serialize_bytes`/
//! `deserialize_bytes` free-fn machinery still live in this artifact's own `🚪️io/🦀️.rs` and
