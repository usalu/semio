//! 🦀️ Block 2d 1 exhaustive mutation case — Rust adapter. Ticket
//! `26/08/23/END-TO-END-TESTING-REFACTOR`.
//!
//! Recorded no-oracle decision `block-2d-mutation-semantics`
//! (`../../🏅️standards/🔖️1/🪆️subsets/✳️any/🧪️oracle/🔣️component.json`): this is a semio-NATIVE
//! document and `Block2dMutation` IS its specification, so there is nothing third-party to register. What
...
/// 🔀️ Snapshot field → the diff field(s) allowed to declare it. `Block2dDiff` mirrors `Block2dSnapshot` name for name, so the table is empty; its `🖐️5d` and `🧊️3d` siblings, whose diffs rename `2d`/`3d` to `part2d`/`part3d` and FOLD `catalog` plus `vortexKindExtra` into one `vortexKinds` field, carry real rows here.
...
fn vector(ctx: &Context) -> Result<Vector, String> {
    ...
    if !KINDS.contains(&kind.as_str()) {
        return Err(format!("scenario doc string names {kind:?}, which is not a declared Block2dMutation kind"));
    }
...
/// 🐫️ `rename-node-kind` → `renameNodeKind`, the discriminant this subset's
...
/// 🔁️ Decode and re-encode the fully populated node-kind definition, through the platform's own dependency-free JSON reader and writer: the
...
fn round_trip(ctx: &Context) -> Result<Outcome, String> {
    const SNAPSHOT: &str = "asset://🏅️standards/🔖️1/🪆️subsets/✳️any/🧬️schema/🧬️mutations/✏️rename-node-kind/🧪️tests/renames-node-kind-to-gate/📸️snapshot/⬅️before/🔣️component.json";
    ...
    if reparsed.array("handleKinds").len() < 2 || reparsed.array("handles").is_empty() || reparsed.array("authors").is_empty() || reparsed.get("presentation").is_none() {
        return Err("the committed round-trip snapshot is the fully populated node-kind definition this scenario describes — two handle kinds, a placed handle, an author and a presentation — but at least one of those is missing".to_string());
    }
