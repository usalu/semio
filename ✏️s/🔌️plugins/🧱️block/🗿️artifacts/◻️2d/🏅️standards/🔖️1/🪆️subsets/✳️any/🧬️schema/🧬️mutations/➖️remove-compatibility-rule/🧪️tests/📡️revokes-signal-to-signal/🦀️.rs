//! 🧪️ `remove-compatibility-rule` fixture — `📡️revokes-signal-to-signal`: the `compat-signal-signal` rule is removed from the compatibility table
//!
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "remove-compatibility-rule/revokes-signal-to-signal: produced diff differs from the committed 🔺️diff/🔣️.json");
}
