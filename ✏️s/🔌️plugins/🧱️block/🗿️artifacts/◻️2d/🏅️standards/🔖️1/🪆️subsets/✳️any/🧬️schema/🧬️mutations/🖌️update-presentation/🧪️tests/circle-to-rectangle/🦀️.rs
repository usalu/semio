//! 🧪️ `update-presentation` fixture — `circle-to-rectangle`: the whole rim presentation facet is replaced, circle → rectangle
//!
...
/// ▶️ The mutation carries `before` to exactly the committed `after`: the whole rim presentation facet is replaced, circle → rectangle
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "update-presentation/circle-to-rectangle: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "update-presentation/circle-to-rectangle: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "update-presentation/circle-to-rectangle: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "update-presentation/circle-to-rectangle: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "update-presentation/circle-to-rectangle: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "update-presentation/circle-to-rectangle: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("update-presentation/circle-to-rectangle: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH collections and fields `update-presentation` is
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "update-presentation/circle-to-rectangle: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "update-presentation/circle-to-rectangle: committed diff JSON is not canonical");
}
