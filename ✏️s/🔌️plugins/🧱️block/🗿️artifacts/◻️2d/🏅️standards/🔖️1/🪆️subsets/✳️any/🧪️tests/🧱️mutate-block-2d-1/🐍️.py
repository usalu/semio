...
`change-handle-handle-kind` writes `🐙️handles`, not `handleKinds` — and a suffix rule that got one of
...
def uri_in(ctx, needle):
    ...
    for step in ctx.scenario["steps"]:
        for token in step["text"].split():
            if token.startswith(("asset://", "shared://🧱️mutate-block-2d-1/", "shared://")) and needle in token:
                return token
