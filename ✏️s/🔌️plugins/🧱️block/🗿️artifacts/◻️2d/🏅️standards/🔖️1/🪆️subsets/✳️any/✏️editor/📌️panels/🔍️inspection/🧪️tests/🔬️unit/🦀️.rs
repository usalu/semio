...
fn project(document: &Block2dSnapshot, windows: &TreeWindows<'_>) -> String {
    let labels = block2d_labels(&ViewModel::default());
...
fn window_view(open: Option<bool>, offset: u32, rows: u32) -> ViewModel {
    ViewModel { tree_windows: vec![TreeWindowRequest { body_key: BLOCK2D_BODY_INSPECTOR.into(), node_key: BLOCK2D_INSPECTOR_SUMMARY.into(), open, offset, rows }], ..Default::default() }
}
...
fn the_inspector_summary_stamps_its_total_and_publishes_no_continuation_row() {
    ...
    let (total, offset, rows) = container_of(&json, BLOCK2D_INSPECTOR_SUMMARY);
    assert_eq!(total, 5, "four identity fields plus the handle count: {json}");
    assert_eq!(offset, 0, "a first paint starts at zero");
    assert_eq!(rows.len(), 5, "a five-row section fits one viewport: {json}");
...
fn a_closed_inspector_summary_stamps_its_total_with_no_rows() {
    ...
    let json = project(&document, &TreeWindows::for_body(&view, BLOCK2D_BODY_INSPECTOR));
    let (total, offset, rows) = container_of(&json, BLOCK2D_INSPECTOR_SUMMARY);
    assert_eq!(total, 5, "a closed section still states its extent: {json}");
    assert_eq!(offset, 0);
...
fn an_inspector_window_request_materialises_exactly_its_slice() {
    ...
    assert_eq!(rows, vec!["block2d-play-inspector.variant".to_string(), "block2d-play-inspector.description".to_string()], "exactly fields [2, 4): {json}");
}
...
fn the_inspector_tree_binds_no_interaction_domain_and_its_rows_keep_their_controls() {
    ...
    assert!(fields.iter().take(4).all(|field| !field["children"].as_array().cloned().unwrap_or_default().is_empty()), "every identity field nests its own control: {json}");
}
