...
//! carries its own `io_declaration()` (see the sibling `🪆️subsets/✳️any/🦀️.rs`) with
...
pub mod io_registry {
    ...
    const BLOCK2D_DIALECT: Dialect = Dialect { artifact_kind: "s.block.block2d", standard: StandardId("1"), subset: SubsetId("*") };
    const BLOCK2D_JSON_BRIDGE_DIALECT: Dialect = Dialect { artifact_kind: "s.stdio.json", standard: StandardId("rfc8259"), subset: SubsetId("*") };

    async fn rebuild_native_snapshot(sources: &[ErasedComposeSource]) -> Result<crate::artifacts::block2d::Block2dSnapshot, ComposeError> {
