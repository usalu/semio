/** 🚪️ block2d ← json — TypeScript mirror of the sibling `🦀️.rs` leaf's `from_json_text`, the
 * exact inverse of `block2dToJsonText` in the `📤️export` leaf (whose field tables it reuses — one
 * declaration of the schema per subset, never two).
 */

import type { Block2dSnapshot } from "../../../../../../../🧬️schema/📸️snapshot/🟦️";
import { type BlockJsonField, BLOCK2D_SNAPSHOT_FIELDS, writeBlockJsonRecord } from "../../../../../../📤️export/🧵️serializers/🗿️artifacts/🔣️json/🔖️rfc8259/✳️any/🟦️";
...
// #region 🎯️Entrypoint
/** 🧩️ `s.stdio.json@rfc8259/*` → `s.block.block2d@1/*`. Throws on text that is not a JSON object. */
export function block2dFromJsonText(text: string): Block2dSnapshot {
  const parsed: unknown = JSON.parse(text);
  if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
    throw new Error("json→block2d: expected a json object at the document root");
  }
  const snapshot = readBlockJsonRecord(parsed, BLOCK2D_SNAPSHOT_FIELDS) as unknown as Block2dSnapshot;
  if (!snapshot.schema) snapshot.schema = "block.2d";
  return snapshot;
}

/** 🔁️ Canonical re-encoding of arbitrary `s.block.block2d` json — the fixed point the parity test asserts. */
export function block2dCanonicalJsonText(text: string): string {
  return writeBlockJsonRecord(block2dFromJsonText(text) as unknown as Record<string, unknown>, BLOCK2D_SNAPSHOT_FIELDS);
}
