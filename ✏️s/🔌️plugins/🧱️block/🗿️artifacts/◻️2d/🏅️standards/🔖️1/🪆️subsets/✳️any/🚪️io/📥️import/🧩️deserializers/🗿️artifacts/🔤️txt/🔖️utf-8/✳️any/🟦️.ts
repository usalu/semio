/** 🚪️ block2d ← txt — TypeScript mirror of the sibling `🦀️.rs` leaf's `from_dsl_text`.
 *
...
import { type BlockJsonField, BLOCK2D_SNAPSHOT_FIELDS } from "../../../../../../📤️export/🧵️serializers/🗿️artifacts/🔣️json/🔖️rfc8259/✳️any/🟦️";

/** 📄️ The `.semio` text preamble every `block.block2d` document opens with. */
export const BLOCK_DSL_PREAMBLE = "semio block.block2d.dsl ";
...
function scalar(token: Token, field: BlockJsonField): unknown {
  switch (field.kind) {
    ...
    default:
      throw new Error(`block2d dsl: field \`${field.key}\` is not a scalar`);
  }
}
// #endregion 🔢️Scalars
...
function readBlockBody(lines: string[], fields: readonly BlockJsonField[]): Record<string, unknown> {
  ...
  for (const line of lines) {
    for (const [, name, raw] of line.matchAll(/([A-Za-z0-9-]+)=("(?:[^"\\]|\\.)*"|[^\s]*)/g)) {
      ...
      if (!field) throw new Error(`block2d dsl: unknown member \`${name}\``);
...
function readTableRow(tokens: Token[], columns: readonly BlockJsonField[]): Record<string, unknown> {
  ...
  for (const column of columns) {
    if (column.kind === "textList" || column.kind === "table") {
      if (tokens[index]?.text !== "[") throw new Error(`block2d dsl: expected \`[\` for column \`${column.key}\``);
      index += 1;
      const items: string[] = [];
      while (index < tokens.length && tokens[index].text !== "]") {
        if (column.kind === "table") throw new Error(`block2d dsl: nested table rows are outside this reader's scope`);
...
export function block2dFromDslText(text: string): Block2dSnapshot {
  if (!text.startsWith(BLOCK_DSL_PREAMBLE)) throw new Error(`block2d dsl: missing \`${BLOCK_DSL_PREAMBLE}\` preamble`);
  const lines = text.split("\n").slice(1);
  const byDslName = new Map(BLOCK2D_SNAPSHOT_FIELDS.map((field) => [dslName(field.key), field]));
  const record: Record<string, unknown> = {};
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (opener) {
      ...
      if (!field) throw new Error(`block2d dsl: unknown member \`${opener[1]}\``);
      const body: string[] = [];
      index += 1;
      ...
      if (field.kind === "record") {
        ...
        const columns = names.map((name) => {
          ...
          if (!column) throw new Error(`block2d dsl: unknown column \`${name}\` in \`${opener[1]}\``);
          return column;
        });
        record[field.key] = body.filter((row) => row.trim().length > 0).map((row) => readTableRow(tokenize(row), columns));
      } else {
        throw new Error(`block2d dsl: \`${opener[1]}\` is not a block or table member`);
      }
      continue;
    }
    const scalarLine = line.match(/^([a-z0-9-]+)=(.*)$/);
    if (!scalarLine) throw new Error(`block2d dsl: unparsable line \`${line}\``);
    const field = byDslName.get(scalarLine[1]);
    if (!field) throw new Error(`block2d dsl: unknown member \`${scalarLine[1]}\``);
    record[field.key] = scalar(tokenize(scalarLine[2])[0] ?? { text: "", quoted: true }, field);
  }
  for (const field of BLOCK2D_SNAPSHOT_FIELDS) {
    if (field.key in record) continue;
  ...
  return record as unknown as Block2dSnapshot;
}
// #endregion 📥️Reader
