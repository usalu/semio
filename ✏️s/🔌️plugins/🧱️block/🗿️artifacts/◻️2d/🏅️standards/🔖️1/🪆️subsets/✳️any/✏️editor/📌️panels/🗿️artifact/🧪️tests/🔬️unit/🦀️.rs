...
async fn renders_document_tree() {
    ...
    assert!(render_body(&mut app, BLOCK2D_BODY_ARTIFACT).await.contains("Handle Kinds"));
}
...
fn window_view(node_key: &str, open: Option<bool>, offset: u32, rows: u32) -> ViewModel {
    ViewModel { tree_windows: vec![TreeWindowRequest { body_key: BLOCK2D_BODY_ARTIFACT.into(), node_key: node_key.into(), open, offset, rows }], ..Default::default() }
}

/// 🌾️ A document an order of magnitude past any viewport: 40 handle kinds and 160 rim handles.
fn oversized_document() -> Block2dSnapshot {
    let mut document = Block2dSnapshot::default();
    document.handle_kinds = (0..40)
        .map(|index| crate::Block2dHandleKind {
            id: format!("kind-{index:03}"),
            name: format!("kind-{index:03}"),
            label: format!("Kind {index}"),
            color: "#334455".into(),
            default_wire_kind: "wire".into(),
        })
        .collect();
    document.handles = (0..160)
        .map(|index| crate::Block2dHandleTemplate { id: format!("handle-{index:03}"), handle_kind: format!("kind-{:03}", index % 40), angle: index as f64 * 0.01, radius: 1.0 })
        .collect();
    document
}
...
fn an_oversized_document_stamps_every_sections_total() {
    ...
    let (kinds_total, kinds_offset, kinds_rows) = container_of(&json, BLOCK2D_DOCUMENT_HANDLE_KINDS);
    ...
    let (items_total, _, items_rows) = container_of(&json, BLOCK2D_DOCUMENT_HANDLES);
...
fn a_closed_document_section_stamps_its_total_with_no_rows() {
    let document = oversized_document();
    let view = window_view(BLOCK2D_DOCUMENT_HANDLES, Some(false), 0, 0);
    let json = project(&document, &TreeWindows::for_body(&view, BLOCK2D_BODY_ARTIFACT));
    let (total, offset, rows) = container_of(&json, BLOCK2D_DOCUMENT_HANDLES);
...
fn a_document_window_request_materialises_exactly_its_slice() {
    let document = oversized_document();
    let view = window_view(BLOCK2D_DOCUMENT_HANDLES, Some(true), 90, 7);
    let json = project(&document, &TreeWindows::for_body(&view, BLOCK2D_BODY_ARTIFACT));
    let (total, offset, rows) = container_of(&json, BLOCK2D_DOCUMENT_HANDLES);
    assert_eq!(total as usize, 160);
    assert_eq!(offset, 90, "the stamped offset is the requested one: {json}");
    let expected: Vec<String> = (90..97).map(|index| format!("handle:handle-{index:03}")).collect();
...
fn document_rows_declare_their_granularity_and_carry_no_row_binding() {
    ...
    let json = project(&document, &TreeWindows::unhosted());
    let projection: serde_json::Value = serde_json::from_str(&json).expect("document projection json");
    assert_eq!(projection["component"]["interactionDomain"].as_str(), Some(BLOCK2D_INTERACTION_HANDLE), "{json}");
