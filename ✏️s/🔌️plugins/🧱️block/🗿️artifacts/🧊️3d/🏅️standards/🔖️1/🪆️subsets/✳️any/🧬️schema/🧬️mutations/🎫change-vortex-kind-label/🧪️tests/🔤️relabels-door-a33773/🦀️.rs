//! 🧪️ `change-vortex-kind-label` fixture — `🔤️relabels-door-vortex-kind`: the `door` vortex kind's `label` becomes `Front Door` — block-owned overflow, so the `catalog` handle is untouched
//!
