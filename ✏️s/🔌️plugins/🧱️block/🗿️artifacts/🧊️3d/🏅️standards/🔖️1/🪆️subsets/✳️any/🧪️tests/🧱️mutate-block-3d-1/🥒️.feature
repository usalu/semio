...
Feature: Apply every typed block3d object-kind mutation twice — once in Rust, once in Python — and require the same answer
  ...
  external library could confirm or refute". The sibling `🧱️mutate-block-2d-1` — same plugin, same
  ...
  the committed answer. `🟩️mutate-program-1` reports the identical blocker over `knowledge` and
  `benchmarks`, and `🐸️mutate-en1990-1`'s two red scenarios are the same finding again: publishing the
