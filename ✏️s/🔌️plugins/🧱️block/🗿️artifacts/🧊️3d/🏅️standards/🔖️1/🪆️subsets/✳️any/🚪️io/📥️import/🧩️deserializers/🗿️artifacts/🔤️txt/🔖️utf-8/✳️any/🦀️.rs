//! 🚪️ block3d ← txt — foreign `Deserializer<Block3dSnapshot>` on the framework's `io_mechanism`
...
/// 📄️ The `.semio` text preamble every `block.block3d` document opens with — the sniff anchor.
pub const DSL_PREAMBLE: &str = "semio block.block3d.dsl ";
...
pub fn from_dsl_text(text: &str) -> Result<Block3dSnapshot, IoError> {
    <Block3dSnapshot as store::ArtifactDsl>::parse_dsl(text).map_err(|error| IoError { message: format!("txt→block3d: {error}"), diagnostics: Vec::new() })
}

/// 🧩️ `s.stdio.txt@utf-8/*` → `s.block.block3d@1/*`.
...
impl Deserializer<Block3dSnapshot> for TxtIntoBlock3d {
    ...
    async fn deserialize(payload: &IoPayload) -> IoResult<Block3dSnapshot> {
        let IoPayload::Text(text) = payload else {
            return Err(IoError { message: "txt→block3d: expected a text utf-8 payload".to_string(), diagnostics: Vec::new() });
        };
