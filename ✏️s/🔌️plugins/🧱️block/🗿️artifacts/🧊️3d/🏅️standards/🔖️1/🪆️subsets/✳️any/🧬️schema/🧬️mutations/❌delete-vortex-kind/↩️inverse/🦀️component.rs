...
pub fn inverse(payload: &super::mutation::DeleteVortexKind, base: &Block3dSnapshot) -> Vec<Block3dMutation> {
    match base.vortex_kinds.iter().find(|item| item.id == payload.id) { Some(existing) => vec![super::super::create_vortex_kind::mutation::create_vortex_kind(existing.clone())], None => Vec::new() }
}
//#endregion 🔖️Inverse
