//! 🧪️ `change-vortex-kind-label` fixture — `relabels-door-vortex-kind`: the `door` vortex kind's `label` becomes `Front Door` — block-owned overflow, so the `catalog` handle is untouched
//!
...
/// ▶️ The mutation carries `before` to exactly the committed `after`: the `door` vortex kind's `label` becomes `Front Door` — block-owned overflow, so the `catalog` handle is untouched
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "change-vortex-kind-label/relabels-door-vortex-kind: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-vortex-kind-label/relabels-door-vortex-kind: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "change-vortex-kind-label/relabels-door-vortex-kind: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "change-vortex-kind-label/relabels-door-vortex-kind: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "change-vortex-kind-label/relabels-door-vortex-kind: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "change-vortex-kind-label/relabels-door-vortex-kind: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("change-vortex-kind-label/relabels-door-vortex-kind: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH collections and fields `change-vortex-kind-label` is
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-vortex-kind-label/relabels-door-vortex-kind: produced diff differs from the committed 🔺️diff/🔣️.json");
}
