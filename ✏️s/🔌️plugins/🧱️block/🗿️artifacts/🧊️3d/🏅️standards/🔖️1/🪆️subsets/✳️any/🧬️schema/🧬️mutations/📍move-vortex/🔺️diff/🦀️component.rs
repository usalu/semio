//! 🔺️ Sparse diff builder for `MoveVortex` — real handcrafted delta, never apply-then-capture.
...
//#region 🔖️Diff
pub fn diff(payload: &super::mutation::MoveVortex, base: &Block3dSnapshot) -> protocol::MutationOutcome<Block3dDiff> {
    ...
    let replacement = Block3dVortexTemplate { position: payload.new_position, direction: payload.new_direction, ..existing.clone() };
