//! 🧪️ `rename-vortex-kind` fixture — `✏️renames-door-to-portal`: the `door` vortex kind's `name` becomes `portal` — the half that lives in the composed kit child, so the handle changes while the overflow row does not
//!
