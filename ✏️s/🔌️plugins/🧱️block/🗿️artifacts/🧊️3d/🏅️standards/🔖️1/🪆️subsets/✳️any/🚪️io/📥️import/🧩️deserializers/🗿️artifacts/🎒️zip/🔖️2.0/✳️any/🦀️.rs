//! 🚪️ block3d ← zip — foreign `Deserializer<Block3dSnapshot>` on the framework's `io_mechanism`
//! channel: decodes a REAL zip 2.0 container (stdio's own `decode_zip`) and rebuilds the snapshot
//! from its `snapshot.block3d.semio` member, falling back to `snapshot.json`. Exact inverse of the sibling
...
use crate::artifacts::block3d::io::export::serializers::artifacts::zip::v2_0::any::{ZIP_DSL_ENTRY, ZIP_JSON_ENTRY};
...
pub struct ZipIntoBlock3d;

impl Deserializer<Block3dSnapshot> for ZipIntoBlock3d {
