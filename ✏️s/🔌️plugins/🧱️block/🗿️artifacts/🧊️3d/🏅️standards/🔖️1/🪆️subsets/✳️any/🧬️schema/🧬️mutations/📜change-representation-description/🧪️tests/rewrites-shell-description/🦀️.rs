//! 🧪️ `change-representation-description` fixture — `rewrites-shell-description`: the `rep-shell` representation's `description` is rewritten
//!
...
/// ▶️ The mutation carries `before` to exactly the committed `after`: the `rep-shell` representation's `description` is rewritten
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-representation-description/rewrites-shell-description: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "change-representation-description/rewrites-shell-description: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    match status {
        ...
        "rejected" => {
            assert!(!applied, "change-representation-description/rewrites-shell-description: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "change-representation-description/rewrites-shell-description: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("change-representation-description/rewrites-shell-description: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH collections and fields `change-representation-description` is
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-representation-description/rewrites-shell-description: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "change-representation-description/rewrites-shell-description: committed diff JSON is not canonical");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "change-representation-description/rewrites-shell-description: committed diff did not carry before to after");
}
