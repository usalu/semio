//! 🔺️ Sparse diff builder for `ChangeVortexLabel` — real handcrafted delta, never apply-then-capture.
...
//#region 🔖️Diff
pub async fn diff(payload: &super::mutation::ChangeVortexLabel, base: &Block3dSnapshot) -> protocol::MutationOutcome<Block3dDiff> {
    ...
    let replacement = Block3dVortexTemplate { label: payload.new_label.clone(), ..existing.clone() };
