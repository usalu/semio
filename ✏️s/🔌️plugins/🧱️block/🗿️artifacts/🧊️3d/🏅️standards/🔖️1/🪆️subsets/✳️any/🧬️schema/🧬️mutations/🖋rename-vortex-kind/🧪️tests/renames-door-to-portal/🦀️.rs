//! 🧪️ `rename-vortex-kind` fixture — `renames-door-to-portal`: the `door` vortex kind's `name` becomes `portal` — the half that lives in the composed kit child, so the handle changes while the overflow row does not
//!
...
/// ▶️ The mutation carries `before` to exactly the committed `after`: the `door` vortex kind's `name` becomes `portal` — the half that lives in the composed kit child, so the handle changes while the overflow row does not
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "rename-vortex-kind/renames-door-to-portal: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "rename-vortex-kind/renames-door-to-portal: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "rename-vortex-kind/renames-door-to-portal: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "rename-vortex-kind/renames-door-to-portal: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "rename-vortex-kind/renames-door-to-portal: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "rename-vortex-kind/renames-door-to-portal: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("rename-vortex-kind/renames-door-to-portal: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta this mutation produces is exactly the committed diff — the single most
/// load-bearing assertion in the fixture: it pins WHICH collections and fields `rename-vortex-kind` is
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rename-vortex-kind/renames-door-to-portal: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "rename-vortex-kind/renames-door-to-portal: committed diff JSON is not canonical");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "rename-vortex-kind/renames-door-to-portal: committed diff did not carry before to after");
}
