//! 🚪️ block3d ← png — foreign `Deserializer<Block3dSnapshot>` that HONESTLY REFUSES.
//!
...
/// 🧩️ `s.stdio.png@1.2/*` → `s.block.block3d@1/*` — always `Err`, see this file's module doc.
pub struct PngIntoBlock3d;

impl Deserializer<Block3dSnapshot> for PngIntoBlock3d {
    const FROM: Dialect = PNG_DIALECT;
    const FIDELITY: IoFidelity = IoFidelity::Lossy;
    async fn deserialize(_payload: &IoPayload) -> IoResult<Block3dSnapshot> {
        Err(IoError {
            message: "png import not supported for an object kind definition: a PNG carries pixels only — it has no kind identity, no handle/vortex/grip catalog and no compatibility rules to build a kind definition from".to_string(),
            diagnostics: Vec::new(),
        })
    }
}
