...
impl protocol::MutationDiff<Block3dPresence> for Block3dPresence {
    fn apply(&self, _base: &Block3dPresence) -> protocol::MutationApplyResult<Block3dPresence> {
        Ok({
            self.clone()
        })
    }
...
impl Mutation<Block3dPresence> for Block3dPresenceMutation {
    ...
    fn diff(&self, _base: &Block3dPresence) -> protocol::MutationOutcome<Block3dPresence> {
        protocol::MutationOutcome::new(match self {
            Self::Snapshot { presence } => presence.clone(),
        })
    }

    fn inverse(&self, base: &Block3dPresence) -> Vec<Self> {
