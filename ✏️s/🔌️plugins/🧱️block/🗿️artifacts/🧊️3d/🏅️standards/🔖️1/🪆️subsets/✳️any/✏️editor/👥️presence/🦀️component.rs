...
impl protocol::MutationDiff<Block3dPresence> for Block3dPresence {
    async fn apply(&self, _base: &Block3dPresence) -> protocol::MutationApplyResult<Block3dPresence> {
        Ok({
            self.clone()
        })
    }
...
pub enum Block3dPresenceMutation {
    ...
    Snapshot {
        #[dsl(block)]
        presence: Block3dPresence,
    },
}

impl Mutation<Block3dPresence> for Block3dPresenceMutation {
    type Diff = Block3dPresence;

    async fn diff(&self, _base: &Block3dPresence) -> protocol::MutationOutcome<Block3dPresence> {
        protocol::MutationOutcome::new(match self {
            Self::Snapshot { presence } => presence.clone(),
        })
    }

    async fn inverse(&self, base: &Block3dPresence) -> Vec<Self> {
