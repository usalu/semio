...
use semio_framework_plugin::{
        create_default_layout, tree_item_with_action, ui_inspector_groups_to_tree, ui_inspector_readonly_field, ui_stack_vertical, ui_text, ActionDescriptor, App, AppLabels, ArtifactKindSpec, ConfigView, DocumentApp, DocumentView, Emit, Fault, Label, Locale,
    ...
    fn handle(&self, command: &Block5dCommand, doc: &DocumentView<'_, Block5dDefinition>, _cfg: &ConfigView<'_, Block5dConfig>) -> Result<Emit<Block5dOperation, Block5dConfigOperation>, Fault> {
        match command {
            Block5dCommand::PatchPartKind { field, value } => {
                ...
                Ok(Emit::operations(vec![Block5dOperation::SetPartKind { part_kind }])
            }
            ...
            Block5dCommand::AddGrip => {
                let Some(grip_kind_id) = doc.projection.grip_kinds.first().map(|kind| kind.id.clone()) else { return Ok(Emit::default() };
                ...
                Ok(Emit::operations(vec![Block5dOperation::SetGrip { index: doc.projection.grips.len(), grip }])
            }
