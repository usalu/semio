//! 🔺️ Sparse diff builder for `ChangeEquipmentGainLatentFraction` — the artifact's delta is built straight from the
...
//#region 🔖️Diff
pub fn diff(payload: &super::ChangeEquipmentGainLatentFraction, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    if !(0.0..=1.0).contains(&payload.new_latent_fraction) {
        return protocol::MutationOutcome::error("mutation.invariant", format!("Equipment Gain {}: latent fraction must be a fraction in [0, 1], got {}.", payload.id.0, payload.new_latent_fraction), [payload.id.0.to_string()]);
    }
