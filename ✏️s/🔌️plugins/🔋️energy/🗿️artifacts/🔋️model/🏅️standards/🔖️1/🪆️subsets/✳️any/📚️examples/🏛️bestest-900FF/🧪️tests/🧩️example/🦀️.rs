...
async fn primary_asset_is_nonempty() {
    let text = include_str!("../../../../🖼️assets/🏛️bestest-900FF/🗣️.dsl.semio");
    assert!(text.len() > 8);
}
...
async fn asset_carries_the_registered_case_model() {
    let text = include_str!("../../../../🖼️assets/🏛️bestest-900FF/🗣️.dsl.semio");
    let snapshot = <crate::EnergyModelSnapshot as store::ArtifactDsl>::parse_dsl(text).expect("bestest 900FF example parses");
