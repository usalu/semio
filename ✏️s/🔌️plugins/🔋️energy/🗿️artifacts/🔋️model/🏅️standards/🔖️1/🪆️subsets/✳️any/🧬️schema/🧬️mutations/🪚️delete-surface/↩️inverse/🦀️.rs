...
pub fn inverse(payload: &super::DeleteSurface, base: &EnergyModelSnapshot) -> Vec<EnergyModelMutation> {
    ...
    let mut steps = vec![vocabulary::create_surface(
        existing.id,
        existing.name.clone(),
        existing.zone_id,
        existing.class,
        existing.vertices_m.clone(),
        existing.construction_id,
        existing.outside_boundary_condition.kind(),
        existing.outside_boundary_condition.interzone_partner(),
    ...
    for window in base.model.fenestrations.iter().filter(|item| item.surface_id == payload.id) {
        steps.push(vocabulary::create_fenestration(
            ...
            window.frame_conductance_w_k,
            window.divider_conductance_w_k,
            window.overhang_depth_m,
            window.overhang_offset_m,
            window.fin_depth_m,
            window.fin_offset_m,
