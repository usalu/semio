...
pub fn inverse(payload: &super::ChangePeopleGainActivitySchedule, base: &EnergyModelSnapshot) -> Vec<EnergyModelMutation> {
    match base.model.people.iter().find(|item| item.id == payload.id) {
        Some(item)
            if item.activity_schedule_id != payload.new_activity_schedule_id
                && (base.model.schedules.constants.iter().any(|schedule| schedule.id == payload.new_activity_schedule_id)
        ...
        {
            vec![vocabulary::change_people_gain_activity_schedule(payload.id, item.activity_schedule_id)]
        }
        _ => Vec::new(),
    }
}
