//! 🧪️ `change-air-loop-return-node` fixture — `⛔️refuses-node-zero`: refuses the unset node id.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/↘️change-air-loop-return-node/⛔️refuses-node-zero/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/↘️change-air-loop-return-node/⛔️refuses-node-zero/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/↘️change-air-loop-return-node/⛔️refuses-node-zero/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/↘️change-air-loop-return-node/⛔️refuses-node-zero/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/↘️change-air-loop-return-node/⛔️refuses-node-zero/🎯️outcome/🔣️.json");
...
fn scenario() -> (EnergyModelSnapshot, EnergyModelMutation) {
    ...
    (snapshot(model), super::change_air_loop_return_node(crate::model::EntityId(17), 0))
}

fn case() -> Case {
    Case { kind: "change-air-loop-return-node", directory: "↘️change-air-loop-return-node/⛔️refuses-node-zero", before: BEFORE, after: AFTER, mutation: MUTATION, diff: DIFF, outcome: OUTCOME, scenario }
}
