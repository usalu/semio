...
pub fn diff(payload: &super::ChangePeopleGainActivitySchedule, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    if !(base.model.schedules.constants.iter().any(|schedule| schedule.id == payload.new_activity_schedule_id) || base.model.schedules.daily.iter().any(|schedule| schedule.id == payload.new_activity_schedule_id) || base.model.schedules.weekly.iter().any(|schedule| schedule.id == payload.new_activity_schedule_id) || base.model.schedules.annual.iter().any(|schedule| schedule.id == payload.new_activity_schedule_id) || base.model.schedules.time_series.iter().any(|schedule| schedule.id == payload.new_activity_schedule_id)) {
    ...
    if existing.activity_schedule_id == payload.new_activity_schedule_id {
        return protocol::MutationOutcome::empty().warn("mutation.no-op", format!("People Gain {} already carries this schedule reference: {}.", payload.id.0, payload.new_activity_schedule_id.0));
    }
    let mut model = base.model.clone();
    if let Some(item) = model.people.iter_mut().find(|item| item.id == payload.id) {
        item.activity_schedule_id = payload.new_activity_schedule_id;
    }
