//! 🧪️ `change-mechanical-ventilation-fan-total-efficiency` fixture — `⛔️refuses`: refuses fan total efficiency outside a fraction in (0, 1].
//!
...
fn scenario() -> (EnergyModelSnapshot, EnergyModelMutation) {
    ...
    (snapshot(model), super::change_mechanical_ventilation_fan_total_efficiency(crate::model::EntityId(60), 0.0))
}

fn case() -> Case {
    Case { kind: "change-mechanical-ventilation-fan-total-efficiency", directory: "💠️change-mechanical-ventilation-fan-total-efficiency/🧪️tests/⛔️refuses", before: BEFORE, after: AFTER, mutation: MUTATION, diff: DIFF, outcome: OUTCOME, scenario }
}
