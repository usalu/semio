//! 🔺️ Sparse diff builder for `ChangeGlazingMaterialThickness` — the artifact's delta is built straight from the
...
//#region 🔖️Diff
pub fn diff(payload: &super::ChangeGlazingMaterialThickness, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    if !payload.new_thickness_m.is_finite() || payload.new_thickness_m <= 0.0 {
        return protocol::MutationOutcome::error("mutation.invariant", format!("Glazing material {}: pane thickness (m) must be a positive finite value, got {}.", payload.id.0, payload.new_thickness_m), [payload.id.0.to_string()]);
    }
