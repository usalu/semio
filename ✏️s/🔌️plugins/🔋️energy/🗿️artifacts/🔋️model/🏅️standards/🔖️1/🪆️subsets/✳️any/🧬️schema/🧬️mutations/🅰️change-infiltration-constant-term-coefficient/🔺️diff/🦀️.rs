...
pub fn diff(payload: &super::ChangeInfiltrationConstantTermCoefficient, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    if !payload.new_constant_term_coefficient.is_finite() || payload.new_constant_term_coefficient < 0.0 {
        return protocol::MutationOutcome::error("mutation.invariant", format!("Infiltration {}: the constant term coefficient A must be a finite non-negative value, got {}.", payload.id.0, payload.new_constant_term_coefficient), [payload.id.0.to_string()]);
    }
    if existing.constant_term_coefficient == payload.new_constant_term_coefficient {
        return protocol::MutationOutcome::empty().warn("mutation.no-op", format!("Infiltration {} already carries this the constant term coefficient A: {}.", payload.id.0, payload.new_constant_term_coefficient));
    }
    let mut model = base.model.clone();
    if let Some(item) = model.infiltrations.iter_mut().find(|item| item.id == payload.id) {
        item.constant_term_coefficient = payload.new_constant_term_coefficient;
    }
