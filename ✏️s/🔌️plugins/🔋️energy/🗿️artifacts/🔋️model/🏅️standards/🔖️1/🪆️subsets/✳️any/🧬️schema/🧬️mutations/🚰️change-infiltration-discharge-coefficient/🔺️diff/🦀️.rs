...
pub fn diff(payload: &super::ChangeInfiltrationDischargeCoefficient, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    if !payload.new_discharge_coefficient.is_finite() || payload.new_discharge_coefficient <= 0.0 {
        return protocol::MutationOutcome::error("mutation.invariant", format!("Infiltration {}: the orifice discharge coefficient must be a positive finite value, got {}.", payload.id.0, payload.new_discharge_coefficient), [payload.id.0.to_string()]);
    }
