...
pub fn diff(payload: &super::ChangeRefrigerationSystemDefrostSchedule, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    if !(base.model.schedules.constants.iter().any(|schedule| schedule.id == payload.new_defrost_schedule_id) || base.model.schedules.daily.iter().any(|schedule| schedule.id == payload.new_defrost_schedule_id) || base.model.schedules.weekly.iter().any(|schedule| schedule.id == payload.new_defrost_schedule_id) || base.model.schedules.annual.iter().any(|schedule| schedule.id == payload.new_defrost_schedule_id) || base.model.schedules.time_series.iter().any(|schedule| schedule.id == payload.new_defrost_schedule_id)) {
    ...
    if let Some(item) = model.refrigeration_systems.iter_mut().find(|item| item.id == payload.id) {
        item.defrost_schedule_id = payload.new_defrost_schedule_id;
    }
