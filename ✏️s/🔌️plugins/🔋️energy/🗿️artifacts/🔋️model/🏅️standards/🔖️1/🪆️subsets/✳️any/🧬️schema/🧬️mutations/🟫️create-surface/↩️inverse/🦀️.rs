...
pub fn inverse(payload: &super::CreateSurface, base: &EnergyModelSnapshot) -> Vec<EnergyModelMutation> {
    if base.model.surfaces.iter().any(|item| item.id == payload.id)
        ...
        || !base.model.zones.iter().any(|item| item.id == payload.zone_id)
        || !base.model.constructions.iter().any(|item| item.id == payload.construction_id)
        || payload.vertices_m.len() < 3
        || payload.interzone_surface_id.is_some_and(|partner| partner == payload.id || !base.model.surfaces.iter().any(|item| item.id == partner))
