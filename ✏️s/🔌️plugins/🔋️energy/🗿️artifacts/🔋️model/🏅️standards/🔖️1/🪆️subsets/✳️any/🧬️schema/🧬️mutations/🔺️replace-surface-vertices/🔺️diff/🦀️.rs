...
pub fn diff(payload: &super::ReplaceSurfaceVertices, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    if payload.new_vertices_m.len() < 3 {
        return protocol::MutationOutcome::error("mutation.invariant", format!("A surface polygon needs at least three vertices, got {}.", payload.new_vertices_m.len()), [payload.id.0.to_string()]);
    }
