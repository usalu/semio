...
pub fn inverse(payload: &super::ChangeThermostatCoolingSetpointSchedule, base: &EnergyModelSnapshot) -> Vec<EnergyModelMutation> {
    match base.model.thermostats.iter().find(|item| item.id == payload.id) {
        Some(item)
            if item.cooling_setpoint_schedule_id != payload.new_cooling_setpoint_schedule_id
        ...
        {
            vec![vocabulary::change_thermostat_cooling_setpoint_schedule(payload.id, item.cooling_setpoint_schedule_id)]
        }
        _ => Vec::new(),
    }
}
