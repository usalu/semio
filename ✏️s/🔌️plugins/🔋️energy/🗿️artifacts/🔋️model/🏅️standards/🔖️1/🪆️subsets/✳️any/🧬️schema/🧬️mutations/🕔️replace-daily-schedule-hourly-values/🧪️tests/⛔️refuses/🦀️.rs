//! 🧪️ `replace-daily-schedule-hourly-values` fixture — `⛔️refuses`: refuses a profile that is not twenty-four hours long.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕔️replace-daily-schedule-hourly-values/⛔️refuses/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕔️replace-daily-schedule-hourly-values/⛔️refuses/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕔️replace-daily-schedule-hourly-values/⛔️refuses/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕔️replace-daily-schedule-hourly-values/⛔️refuses/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕔️replace-daily-schedule-hourly-values/⛔️refuses/🎯️outcome/🔣️.json");
...
fn scenario() -> (EnergyModelSnapshot, EnergyModelMutation) {
    ...
    (snapshot(model), super::replace_daily_schedule_hourly_values(crate::model::ScheduleId(10), vec![22.0; 12]))
}

fn case() -> Case {
    Case { kind: "replace-daily-schedule-hourly-values", directory: "🕔️replace-daily-schedule-hourly-values/⛔️refuses", before: BEFORE, after: AFTER, mutation: MUTATION, diff: DIFF, outcome: OUTCOME, scenario }
}
