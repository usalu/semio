//! 🔺️ Sparse diff builder for `DeleteDailySchedule` — the artifact's delta is built straight from the
...
//#region 🔖️Diff
pub fn diff(payload: &super::DeleteDailySchedule, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    model.schedules.daily.retain(|item| item.id != payload.id);
