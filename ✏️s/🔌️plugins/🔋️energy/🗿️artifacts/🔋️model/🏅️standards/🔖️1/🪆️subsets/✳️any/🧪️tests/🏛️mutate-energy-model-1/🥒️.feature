...
Feature: Apply the one typed s.energy.model mutation against an independent Python implementation
  ...
  under `🧬️schema/🧬️mutations/♻️replace-model/🧪️tests/🏛️degrades-an-empty-model-payload-to-a-no-op/`
  ...
  vector for this vocabulary, `🏛️degrades-an-empty-model-payload-to-a-no-op`, carries `newModelJson` of
  ...
  otherwise, so a second reader has nothing to parse from — the same finding `🚀️mutate-iso16757-1`,
  `🟣️mutate-vdi3805-1`, `🌬️mutate-din16798-1` and `🟩️mutate-program-1` record against their own subsets.
