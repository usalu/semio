...
pub fn diff(payload: &super::ChangeFaultStartSchedule, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    if existing.start_schedule_id == payload.new_start_schedule_id {
        return protocol::MutationOutcome::empty().warn("mutation.no-op", format!("Fault {} already carries this schedule reference: {}.", payload.id.0, payload.new_start_schedule_id.0));
    }
    let mut model = base.model.clone();
    if let Some(item) = model.faults.iter_mut().find(|item| item.id == payload.id) {
        item.start_schedule_id = payload.new_start_schedule_id;
    }
