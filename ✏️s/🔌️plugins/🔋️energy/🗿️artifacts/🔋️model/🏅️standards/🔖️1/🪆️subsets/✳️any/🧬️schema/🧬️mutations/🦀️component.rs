...
/// `protocol`/`store` extern-crate aliases are private — so neither `EnergyModelMutation` nor `EnergyModelSnapshot`
...
pub fn energy_model_mutation_report_json(base_json: &str, mutation_json: &str) -> Result<String, String> {
    let decode_snapshot = |text: &str| -> Result<EnergyModelSnapshot, String> { Ok(serde_json::from_str(text).map_err(|error| error.to_string())?) };
