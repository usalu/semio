...
impl protocol::MutationKind<EnergyModelSnapshot, EnergyModelMutation> for ChangeSetpointManagerSchedule {
    ...
    fn label(&self) -> protocol::LocalizedLabel {
        protocol::LocalizedLabel::native(&format!("Change setpoint manager {} schedule to {:?}", self.id.0, self.new_schedule_present.then_some(self.new_schedule_id.0)), &format!("Zeitplan von Sollwertmanager {} auf {:?} ändern", self.id.0, self.new_schedule_present.then_some(self.new_schedule_id.0)))
    }
