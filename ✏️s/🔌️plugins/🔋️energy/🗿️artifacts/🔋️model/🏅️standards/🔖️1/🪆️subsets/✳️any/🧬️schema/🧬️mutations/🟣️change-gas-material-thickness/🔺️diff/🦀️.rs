...
//#region 🔖️Diff
pub fn diff(payload: &super::ChangeGasMaterialThickness, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    if !payload.new_thickness_m.is_finite() || payload.new_thickness_m <= 0.0 {
        return protocol::MutationOutcome::error("mutation.invariant", format!("Gas material {}: gap width (m) must be a positive finite value, got {}.", payload.id.0, payload.new_thickness_m), [payload.id.0.to_string()]);
    }
    if existing.thickness_m == payload.new_thickness_m {
        return protocol::MutationOutcome::empty().warn("mutation.no-op", format!("Gas material {} already carries this gap width (m): {}.", payload.id.0, payload.new_thickness_m));
    }
