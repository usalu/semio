//! 🎚️ Persisted local state for ONE concrete `energy.model.3d` window: its orbit pose.
//!
//! 🎥️ Why a WINDOW config and not a field on `EnergyModelConfig`: two open 3d windows (a split pane,
//! a second tab) orbit independently, and a camera must never enter the document's undo history. The
//! shape is the `WindowConfigOwner` precedent — fem3d's `🧱️model/🎚️config` and the wires canvas's
//! `🕸️canvas/🎚️config`, whose `set-camera` verb this one mirrors.

#[path = "🧬️schema/🦀️.rs"]
mod schema;
pub use schema::*;

impl store::ArtifactDsl for EnergyModelWindowConfig {
...
impl store::ArtifactPack for EnergyModelWindowConfig {
...
pub struct EnergyModelWindowConfigOwner;

impl semio_framework_plugin::WindowConfigOwner for EnergyModelWindowConfigOwner {
    const WINDOW_KIND_ID: &'static str = super::WINDOW_KIND_ID;
    const SCHEMA: &'static str = "energy.model3dwindowconfig";
    ...
    type State = EnergyModelWindowConfig;
    type Mutation = EnergyModelWindowConfigMutation;
...
pub fn current<'a, C>(view: &'a semio_framework_plugin::ConfigView<'_, C>) -> Option<&'a EnergyModelWindowConfig> {
    view.window::<EnergyModelWindowConfigOwner>()
}
...
pub fn addressed(view: &semio_framework_plugin::ViewModel, mutation: EnergyModelWindowConfigMutation) -> Result<semio_framework_plugin::WindowConfigMutation, semio_framework_plugin::Fault> {
    let window_id = view
        .window_id
        .as_deref()
        .ok_or_else(|| semio_framework_plugin::Fault::new(semio_framework_plugin::FaultOrigin::App, semio_framework_plugin::FaultCode::new("energy.model.3d.window-required"), "a camera change requires a concrete 3d model window"))?;
    ...
    if kind != Some(super::WINDOW_KIND_ID) {
        return Err(semio_framework_plugin::Fault::new(semio_framework_plugin::FaultOrigin::App, semio_framework_plugin::FaultCode::new("energy.model.3d.window-kind"), "a camera change requires a 3d model window"));
    }
    Ok(semio_framework_plugin::WindowConfigMutation::of::<EnergyModelWindowConfigOwner>(window_id, mutation))
}
