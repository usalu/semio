//! 🔺️ Sparse diff builder for `ChangeSurfaceZone` — the artifact's delta is built straight from the
...
//#region 🔖️Diff
pub fn diff(payload: &super::ChangeSurfaceZone, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    if existing.zone_id == payload.new_zone_id {
        return protocol::MutationOutcome::empty().warn("mutation.no-op", format!("Surface {} already has this owning zone.", payload.id.0));
    }
