...
pub fn diff(payload: &super::ChangeLightingGainRadiantFraction, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    if !(0.0..=1.0).contains(&payload.new_radiant_fraction) {
        return protocol::MutationOutcome::error("mutation.invariant", format!("Lighting Gain {}: radiant fraction must be a fraction in [0, 1], got {}.", payload.id.0, payload.new_radiant_fraction), [payload.id.0.to_string()]);
    }
    if existing.radiant_fraction == payload.new_radiant_fraction {
        return protocol::MutationOutcome::empty().warn("mutation.no-op", format!("Lighting Gain {} already carries this radiant fraction: {}.", payload.id.0, payload.new_radiant_fraction));
    }
