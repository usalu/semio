//! ↩️ Inverse for `DeleteWeeklySchedule` — always computed from BASE, never by inverting the delta.
...
pub fn inverse(payload: &super::DeleteWeeklySchedule, base: &EnergyModelSnapshot) -> Vec<EnergyModelMutation> {
    let Some(index) = base.model.schedules.weekly.iter().position(|item| item.id == payload.id) else {
        return Vec::new();
    };
    ...
    let existing = &base.model.schedules.weekly[index];
    vec![vocabulary::create_weekly_schedule(index as u32, existing.id, existing.daily_schedule_ids.to_vec())]
}
//#endregion 🔖️Inverse
