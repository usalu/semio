...
impl protocol::MutationKind<EnergyModelSnapshot, EnergyModelMutation> for ChangePlantLoopDesignFlow {
    ...
    fn label(&self) -> protocol::LocalizedLabel {
        protocol::LocalizedLabel::native(&format!("Change plant loop {} design mass flow to {:?}", self.id.0, self.new_design_flow_kg_s), &format!("Bemessungsmassenstrom von Anlagenkreislauf {} auf {:?} ändern", self.id.0, self.new_design_flow_kg_s))
    }
