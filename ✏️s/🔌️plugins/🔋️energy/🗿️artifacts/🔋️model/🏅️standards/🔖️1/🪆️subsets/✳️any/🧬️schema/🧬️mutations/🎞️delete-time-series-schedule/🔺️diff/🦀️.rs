//! 🔺️ Sparse diff builder for `DeleteTimeSeriesSchedule` — the artifact's delta is built straight from the
...
//#region 🔖️Diff
pub fn diff(payload: &super::DeleteTimeSeriesSchedule, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    model.schedules.time_series.retain(|item| item.id != payload.id);
