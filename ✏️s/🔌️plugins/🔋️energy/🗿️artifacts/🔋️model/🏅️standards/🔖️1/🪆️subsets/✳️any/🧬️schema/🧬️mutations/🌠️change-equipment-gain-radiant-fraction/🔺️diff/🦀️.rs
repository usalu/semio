...
pub fn diff(payload: &super::ChangeEquipmentGainRadiantFraction, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    if !(0.0..=1.0).contains(&payload.new_radiant_fraction) {
        return protocol::MutationOutcome::error("mutation.invariant", format!("Equipment Gain {}: radiant fraction must be a fraction in [0, 1], got {}.", payload.id.0, payload.new_radiant_fraction), [payload.id.0.to_string()]);
    }
