//! 🧪️ `replace-model` fixture — `🏛️degrades-an-empty-model-payload-to-a-no-op`.
//!
...
async fn produces_committed_diff() {
    ...
    assert!(pack::json::value_eq_ignoring_object_order(&produced, &committed), "replace-model/degrades-an-empty-model-payload-to-a-no-op: produced diff differs from the committed 🔺️diff/🔣️.json ({produced:?} vs {committed:?})");
}
