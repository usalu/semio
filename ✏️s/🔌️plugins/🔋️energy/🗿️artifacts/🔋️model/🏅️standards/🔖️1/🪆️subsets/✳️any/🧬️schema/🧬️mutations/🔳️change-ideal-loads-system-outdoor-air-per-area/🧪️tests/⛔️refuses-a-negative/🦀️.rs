//! 🧪️ `change-ideal-loads-system-outdoor-air-per-area` fixture — `⛔️refuses-a-negative`: refuses a negative ventilation rate.
//!
...
fn scenario() -> (EnergyModelSnapshot, EnergyModelMutation) {
    ...
    (snapshot(model), super::change_ideal_loads_system_outdoor_air_per_area(crate::model::EntityId(12), -1.0))
}

fn case() -> Case {
    Case { kind: "change-ideal-loads-system-outdoor-air-per-area", directory: "🔳️change-ideal-loads-system-outdoor-air-per-area/🧪️tests/⛔️refuses-a-negative", before: BEFORE, after: AFTER, mutation: MUTATION, diff: DIFF, outcome: OUTCOME, scenario }
}
