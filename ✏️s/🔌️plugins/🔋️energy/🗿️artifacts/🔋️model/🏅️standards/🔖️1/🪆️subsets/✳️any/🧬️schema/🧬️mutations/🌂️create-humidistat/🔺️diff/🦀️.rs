...
pub fn diff(payload: &super::CreateHumidistat, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    if !(base.model.schedules.constants.iter().any(|schedule| schedule.id == payload.humidifying_setpoint_schedule_id) || base.model.schedules.daily.iter().any(|schedule| schedule.id == payload.humidifying_setpoint_schedule_id) || base.model.schedules.weekly.iter().any(|schedule| schedule.id == payload.humidifying_setpoint_schedule_id) || base.model.schedules.annual.iter().any(|schedule| schedule.id == payload.humidifying_setpoint_schedule_id) || base.model.schedules.time_series.iter().any(|schedule| schedule.id == payload.humidifying_setpoint_schedule_id)) {
    ...
    if !(base.model.schedules.constants.iter().any(|schedule| schedule.id == payload.dehumidifying_setpoint_schedule_id) || base.model.schedules.daily.iter().any(|schedule| schedule.id == payload.dehumidifying_setpoint_schedule_id) || base.model.schedules.weekly.iter().any(|schedule| schedule.id == payload.dehumidifying_setpoint_schedule_id) || base.model.schedules.annual.iter().any(|schedule| schedule.id == payload.dehumidifying_setpoint_schedule_id) || base.model.schedules.time_series.iter().any(|schedule| schedule.id == payload.dehumidifying_setpoint_schedule_id)) {
