    ...
    match base.model.setpoint_managers.iter().find(|item| item.id == payload.id) {
        Some(item)
            if item.schedule_id != value
                && !((!payload.new_schedule_present && payload.new_schedule_id.0 != 0)
                    || (payload.new_schedule_present
        ...
        {
            vec![vocabulary::change_setpoint_manager_schedule(payload.id, item.schedule_id.is_some(), item.schedule_id.unwrap_or(crate::model::ScheduleId(0)))]
        }
        _ => Vec::new(),
    }
}
