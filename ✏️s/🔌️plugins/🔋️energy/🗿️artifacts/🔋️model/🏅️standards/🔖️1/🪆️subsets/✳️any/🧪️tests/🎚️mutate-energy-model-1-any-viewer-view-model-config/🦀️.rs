//! 🎚️ `s.energy.model` 👁️viewer/🎭️modes/👁️view/🪟️windows/🧊️model/🎚️config state-lane mutation case — Rust adapter.
//!
//! Recorded no-oracle decision `energy-model-1-any-viewer-view-model-config-state-lane-semantics`: the runner dispatches no oracle role, so every law is asserted inside
//! the subject handlers through `semio_s_plugin_stdio_test_oracle::law::vector` over the report of this crate's
//! production bridge `energy_model_viewer_window_config_mutation_report_json`. The oracle handlers answer with the committed after- and before-snapshots read
...
//#region 🔖️Vectors
/// 🧫️ The committed applied vector of one kind, read literally from `👁️viewer/🎭️modes/👁️view/🪟️windows/🧊️model/🎚️config/🧫️fixtures`.
...
mod subject {
    ...
    use semio_s_artifact_energy_model::viewer::model::modes::view::windows::model::config::energy_model_viewer_window_config_mutation_report_json;

    fn report(committed: &Vector) -> Result<String, String> {
        energy_model_viewer_window_config_mutation_report_json(committed.before, committed.mutation, committed.after)
    }
