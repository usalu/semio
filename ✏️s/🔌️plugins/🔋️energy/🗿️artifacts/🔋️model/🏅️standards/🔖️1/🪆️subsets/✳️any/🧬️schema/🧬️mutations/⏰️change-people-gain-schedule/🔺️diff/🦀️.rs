//! 🔺️ Sparse diff builder for `ChangePeopleGainSchedule` — the artifact's delta is built straight from the
...
//#region 🔖️Diff
pub fn diff(payload: &super::ChangePeopleGainSchedule, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    if existing.schedule_id == payload.new_schedule_id {
        return protocol::MutationOutcome::empty().warn("mutation.no-op", format!("People Gain {} already carries this schedule reference: {}.", payload.id.0, payload.new_schedule_id.0));
    }
