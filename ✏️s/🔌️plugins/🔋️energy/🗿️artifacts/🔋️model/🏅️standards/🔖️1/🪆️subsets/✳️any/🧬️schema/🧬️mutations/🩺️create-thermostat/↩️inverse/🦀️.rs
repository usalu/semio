...
pub fn inverse(payload: &super::CreateThermostat, base: &EnergyModelSnapshot) -> Vec<EnergyModelMutation> {
    if (base.model.thermostats.iter().any(|item| item.id == payload.id))
        ...
        || (!(base.model.schedules.constants.iter().any(|schedule| schedule.id == payload.heating_setpoint_schedule_id)
        ...
        || (!(base.model.schedules.constants.iter().any(|schedule| schedule.id == payload.cooling_setpoint_schedule_id)
        ...
        || (!payload.heating_throttle_range_k.is_finite() || payload.heating_throttle_range_k <= 0.0)
        || (!payload.cooling_throttle_range_k.is_finite() || payload.cooling_throttle_range_k <= 0.0)
    {
        return Vec::new();
    }
