//! 🔺️ Sparse diff builder for `ChangeSizingObjectZone` — the artifact's delta is built straight from the
...
//#region 🔖️Diff
pub fn diff(payload: &super::ChangeSizingObjectZone, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    if existing.zone_id == payload.new_zone_id {
        return protocol::MutationOutcome::empty().warn("mutation.no-op", format!("Sizing object {} already has that zone.", payload.id.0));
    }
