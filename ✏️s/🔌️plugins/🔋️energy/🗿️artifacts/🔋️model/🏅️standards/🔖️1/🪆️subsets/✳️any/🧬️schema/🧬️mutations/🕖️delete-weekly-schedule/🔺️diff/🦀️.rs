//! 🔺️ Sparse diff builder for `DeleteWeeklySchedule` — the artifact's delta is built straight from the
...
//#region 🔖️Diff
pub fn diff(payload: &super::DeleteWeeklySchedule, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    model.schedules.weekly.retain(|item| item.id != payload.id);
