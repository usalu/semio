...
fn before() -> EnergyModelSnapshot {
    serde_json::from_str(BEFORE).expect("before energy document decodes")
}
fn expected_after() -> EnergyModelSnapshot {
    serde_json::from_str(AFTER).expect("after energy document decodes")
}
fn mutation() -> EnergyModelMutation {
    serde_json::from_str(MUTATION).expect("replace-model mutation decodes")
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        let decoded: EnergyModelSnapshot = serde_json::from_str(text).expect("energy document decodes");
        let reencoded = serde_json::to_value(&decoded).expect("energy document encodes");
        let original: serde_json::Value = serde_json::from_str(text).expect("energy document reparses");
        assert_eq!(reencoded, original, "replace-model/degrades-an-empty-model-payload-to-a-no-op: committed {label} energy JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("replaceModel payload encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("replaceModel payload reparses");
    assert_eq!(reencoded, original, "replace-model/degrades-an-empty-model-payload-to-a-no-op: committed replaceModel JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    assert!(serde_json::from_str::<crate::model::Model>("{}").is_err(), "replace-model/degrades-an-empty-model-payload-to-a-no-op: this fixture depends on \"{{}}\" NOT being a decodable Model");
    let declared: serde_json::Value = serde_json::from_str(OUTCOME).expect("outcome decodes");
    assert_eq!(declared.get("status").and_then(serde_json::Value::as_str), Some("applied"), "replace-model/degrades-an-empty-model-payload-to-a-no-op: a degraded no-op is applied, never rejected — the oracle has no rejection branch at all");
...
async fn produces_committed_diff() {
    let produced = serde_json::to_value(built_outcome().diff()).expect("produced replace-model diff encodes");
    let committed: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff decodes");
    assert_eq!(produced, committed, "replace-model/degrades-an-empty-model-payload-to-a-no-op: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    let decoded: EnergyModelDiff = serde_json::from_str(DIFF).expect("committed replace-model diff decodes");
    ...
    assert_eq!(reencoded, original, "replace-model/degrades-an-empty-model-payload-to-a-no-op: committed diff JSON is not canonical");
}
