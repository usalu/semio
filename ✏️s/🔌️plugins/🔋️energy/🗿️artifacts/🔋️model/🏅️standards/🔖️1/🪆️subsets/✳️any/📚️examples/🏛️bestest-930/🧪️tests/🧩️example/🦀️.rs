...
async fn primary_asset_is_nonempty() {
    let text = include_str!("../../../../🖼️assets/🏛️bestest-930/🗣️.dsl.semio");
    assert!(text.len() > 8);
}
...
async fn asset_carries_the_registered_case_model() {
    let text = include_str!("../../../../🖼️assets/🏛️bestest-930/🗣️.dsl.semio");
    let snapshot = <crate::EnergyModelSnapshot as store::ArtifactDsl>::parse_dsl(text).expect("bestest 930 example parses");
