...
//#region 🔖️Diff
pub fn diff(payload: &super::ChangeEquipmentGainZone, base: &EnergyModelSnapshot) -> protocol::MutationOutcome<EnergyModelDiff> {
    ...
    if existing.zone_id == payload.new_zone_id {
        return protocol::MutationOutcome::empty().warn("mutation.no-op", format!("Equipment Gain {} already carries this zone reference: {}.", payload.id.0, payload.new_zone_id.0));
    }
