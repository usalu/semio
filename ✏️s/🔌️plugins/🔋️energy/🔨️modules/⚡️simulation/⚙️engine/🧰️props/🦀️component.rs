...
pub async fn saturation_pressure_pa(t_c: f64) -> f64 {
...
pub async fn humidity_ratio_from_rh(t_c: f64, rh: f64, p_atm: f64) -> f64 {
...
pub async fn rh_from_humidity_ratio(t_c: f64, w: f64, p_atm: f64) -> f64 {
    let p_ws = saturation_pressure_pa(t_c);
...
pub async fn wet_bulb_c(t_db_c: f64, w: f64, p_atm: f64) -> f64 {
    let target = w;
...
pub async fn moist_air_enthalpy_j_per_kg(t_c: f64, w: f64) -> f64 {
    CP_DRY_AIR * t_c + w * (H_FG_0C + 1860.0 * t_c)
}

/// 🌡️ Dew point [°C] from humidity ratio.
pub async fn dew_point_c(w: f64, p_atm: f64) -> f64 {
    let p_w = w * p_atm / (0.621_945 + w);
...
pub async fn moist_air_density(t_c: f64, w: f64, p_atm: f64) -> f64 {
    let t_k = c_to_k(t_c);
    let p_w = w * p_atm / (0.621_945 + w);
    let p_d = p_atm - p_w;
...
pub async fn steam_saturation_temp_c(p_pa: f64) -> f64 {
    let ln_p = (p_pa / 611.657).ln();
    237.3 * ln_p / (17.2799 - ln_p)
}

/// 💨️ Latent heat of vaporization [J/kg] at temperature [°C].
pub async fn latent_heat_vaporization(t_c: f64) -> f64 {
    H_FG_0C - 2370.0 * t_c
}
// #endregion 🔖️Steam
...
pub async fn r410a_saturation_pressure_pa(t_c: f64) -> f64 {
    let t_k = c_to_k(t_c);
    let a = -1031.0;
    let b = 7.0;
    (a / t_k + b).exp() * P_STD
...
pub async fn r410a_saturation_temp_c(p_pa: f64) -> f64 {
    let ratio = (p_pa / P_STD).ln();
    k_to_c(-1031.0 / (ratio - 7.0))
}

async fn k_to_c(t_k: f64) -> f64 {
    t_k - 273.15
}
...
pub async fn glycol_cp_j_per_kg_k(t_c: f64, glycol_fraction: f64) -> f64 {
    let f = glycol_fraction.clamp(0.0, 0.6);
    water_cp_j_per_kg_k(t_c) * (1.0 - f) + 2400.0 * f
}

/// 🧪️ Glycol mixture density [kg/m³].
pub async fn glycol_density(t_c: f64, glycol_fraction: f64) -> f64 {
    let f = glycol_fraction.clamp(0.0, 0.6);
    water_density(t_c) * (1.0 - f) + 1110.0 * f
}

/// 🧪️ Glycol mixture dynamic viscosity [Pa·s] (simplified).
pub async fn glycol_viscosity(t_c: f64, glycol_fraction: f64) -> f64 {
    let f = glycol_fraction.clamp(0.0, 0.6);
...
mod tests {
    ...
    #[test]
    async fn water_density_near_room_temp() {
