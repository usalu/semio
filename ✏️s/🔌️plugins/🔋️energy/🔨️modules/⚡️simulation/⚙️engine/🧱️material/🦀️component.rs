//! 🧱️ Construction thermal properties: U-value, R-value, and thermal mass.

use crate::model::Material;

// #region 🔖️FilmResistance
...
// #endregion 🔖️FilmResistance

// #region 🔖️Resistance
/// 🧊️ Layer thermal resistance R = d/λ [m²·K/W].
pub async fn layer_resistance_m2k_w(layer: &Material) -> f64 {
    layer.thickness_m / layer.conductivity_w_m_k
}

/// 🧊️ Total effective resistance including film resistances [m²·K/W].
pub async fn effective_resistance(layers: &[Material], r_interior: f64, r_exterior: f64) -> f64 {
    r_interior + r_exterior + layers.iter().map(layer_resistance_m2k_w).sum::<f64>()
}
// #endregion 🔖️Resistance

// #region 🔖️UValue
/// 🔥️ Construction U-value [W/(m²·K)] = 1/R_total.
pub async fn construction_u_value(layers: &[Material], r_interior: f64, r_exterior: f64) -> f64 {
...
// #region 🔖️ThermalMass
/// 🪨️ Area-normalized thermal capacitance [J/(m²·K)] = Σ ρ·c·d.
pub async fn construction_thermal_mass(layers: &[Material]) -> f64 {
    layers.iter().map(|m| m.density_kg_m3 * m.specific_heat_j_kg_k * m.thickness_m).sum()
}

/// 🪨️ Volumetric heat capacity of a single layer [J/(m³·K)].
pub async fn layer_volumetric_heat_capacity(layer: &Material) -> f64 {
    layer.density_kg_m3 * layer.specific_heat_j_kg_k
}
// #endregion 🔖️ThermalMass

// #region 🔖️Equivalent
/// 🧱️ Equivalent single-layer properties for multi-layer constructions.
...
/// 🧱️ Collapse layers into one equivalent slab preserving R and thermal mass.
pub async fn equivalent_layer(layers: &[Material], r_interior: f64, r_exterior: f64) -> EquivalentLayer {
