...
pub async fn operative_temp_c(air_temp_c: f64, mean_radiant_temp_c: f64, air_speed_m_s: f64) -> f64 {
...
pub async fn mean_radiant_temp_c(surface_temps_k: &[f64], view_factors: &[f64]) -> f64 {
...
pub async fn mean_radiant_temp_from_enclosure(air_temp_c: f64, enclosure_delta_t_k: f64) -> f64 {
...
pub async fn pmv(input: &ComfortInput) -> f64 {
...
pub async fn ppd(pmv_value: f64) -> f64 {
...
pub async fn adaptive_comfort_range_c(standard: AdaptiveStandard, running_mean_outdoor_temp_c: f64, acceptability_class: u8) -> (f64, f64) {
...
pub async fn adaptive_comfort_ok(standard: AdaptiveStandard, operative_temp_c: f64, running_mean_outdoor_temp_c: f64, acceptability_class: u8) -> bool {
...
pub async fn radiant_asymmetry_ok(temp_high_c: f64, temp_low_c: f64, max_asymmetry_k: f64) -> bool {
...
mod tests {
    ...
    async fn neutral_input() -> ComfortInput {
    ...
    #[test]
    async fn operative_equals_air_when_equal_mrt() {
        let top = operative_temp_c(22.0, 22.0, 0.1);
        assert!((top - 22.0).abs() < 0.5);
    }

    #[test]
    async fn pmv_near_zero_at_neutral() {
        let p = pmv(&neutral_input());
        assert!(p.abs() < 1.5, "pmv={p}");
    }

    #[test]
    async fn ppd_minimum_at_zero_pmv() {
        let p = ppd(0.0);
        assert!((p - 5.0).abs() < 1.0);
    }

    #[test]
    async fn pmv_increases_when_too_warm() {
    ...
    #[test]
    async fn adaptive_range_centers_on_outdoor() {
