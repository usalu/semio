//! 🧪️ `rename-layer` fixture — `✏️renames-the-sketch-layer-to-final-linework`.
//!
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert!(dsl::json::value_eq_ignoring_object_order(&reencoded, &original), "rename-layer/renames-the-sketch-layer-to-final-linework: committed {side} JSON is not canonical");
    }
    ...
    assert!(dsl::json::value_eq_ignoring_object_order(&reencoded, &original), "rename-layer/renames-the-sketch-layer-to-final-linework: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::json::Value::as_str), Some("applied"), "rename-layer/renames-the-sketch-layer-to-final-linework declares an applied outcome");
...
async fn produces_committed_diff() {
    ...
    assert!(dsl::json::value_eq_ignoring_object_order(&encoded, &committed), "rename-layer/renames-the-sketch-layer-to-final-linework: produced diff differs from the committed 🔺️diff/🔣️.json");
...
async fn committed_diff_is_canonical() {
    ...
    assert!(dsl::json::value_eq_ignoring_object_order(&reencoded, &original), "rename-layer/renames-the-sketch-layer-to-final-linework: committed diff JSON is not canonical");
}
