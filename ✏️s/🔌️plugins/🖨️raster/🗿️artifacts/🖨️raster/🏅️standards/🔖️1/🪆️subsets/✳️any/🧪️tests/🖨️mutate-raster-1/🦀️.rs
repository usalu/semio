...
//!
//! This case is a CROSS-LANGUAGE DIFFERENTIAL. The reference is `🐍️.py` beside this file —
...
mod subject {
    ...
    fn uri_in(ctx: &Context, needle: &str) -> Result<String, String> {
        ctx.scenario
            ...
            .find(|token| (token.starts_with("asset://") || token.starts_with("shared://🖨️mutate-raster-1/") || token.starts_with("shared://")) && token.contains(needle))
