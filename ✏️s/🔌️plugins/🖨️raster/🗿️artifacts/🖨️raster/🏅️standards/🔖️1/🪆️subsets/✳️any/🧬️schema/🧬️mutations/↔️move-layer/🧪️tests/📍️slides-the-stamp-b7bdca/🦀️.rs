...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/↔️move-layer/📍️slides-the-stamp-b7bdca/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/↔️move-layer/📍️slides-the-stamp-b7bdca/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/↔️move-layer/📍️slides-the-stamp-b7bdca/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/↔️move-layer/📍️slides-the-stamp-b7bdca/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/↔️move-layer/📍️slides-the-stamp-b7bdca/🎯️outcome/🔣️.json");
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert!(dsl::json::value_eq_ignoring_object_order(&reencoded, &original), "move-layer/slides-the-stamp-layer-off-the-origin: committed {side} JSON is not canonical");
    }
    ...
    assert!(dsl::json::value_eq_ignoring_object_order(&reencoded, &original), "move-layer/slides-the-stamp-layer-off-the-origin: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::json::Value::as_str), Some("applied"), "move-layer/slides-the-stamp-layer-off-the-origin declares an applied outcome");
...
async fn produces_committed_diff() {
    ...
    assert!(dsl::json::value_eq_ignoring_object_order(&encoded, &committed), "move-layer/slides-the-stamp-layer-off-the-origin: produced diff differs from the committed 🔺️diff/🔣️.json");
...
async fn committed_diff_is_canonical() {
    ...
    assert!(dsl::json::value_eq_ignoring_object_order(&reencoded, &original), "move-layer/slides-the-stamp-layer-off-the-origin: committed diff JSON is not canonical");
}
