...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐resize-layer/📐️resizes-the-canvas-41d97f/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐resize-layer/📐️resizes-the-canvas-41d97f/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐resize-layer/📐️resizes-the-canvas-41d97f/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐resize-layer/📐️resizes-the-canvas-41d97f/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐resize-layer/📐️resizes-the-canvas-41d97f/🎯️outcome/🔣️.json");
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert!(dsl::json::value_eq_ignoring_object_order(&reencoded, &original), "resize-layer/resizes-the-canvas-layer-to-256-by-128: committed {side} JSON is not canonical");
    }
    ...
    assert!(dsl::json::value_eq_ignoring_object_order(&reencoded, &original), "resize-layer/resizes-the-canvas-layer-to-256-by-128: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::json::Value::as_str), Some("applied"), "resize-layer/resizes-the-canvas-layer-to-256-by-128 declares an applied outcome");
...
async fn produces_committed_diff() {
    ...
    assert!(dsl::json::value_eq_ignoring_object_order(&encoded, &committed), "resize-layer/resizes-the-canvas-layer-to-256-by-128: produced diff differs from the committed 🔺️diff/🔣️.json");
...
async fn committed_diff_is_canonical() {
    ...
    assert!(dsl::json::value_eq_ignoring_object_order(&reencoded, &original), "resize-layer/resizes-the-canvas-layer-to-256-by-128: committed diff JSON is not canonical");
}
