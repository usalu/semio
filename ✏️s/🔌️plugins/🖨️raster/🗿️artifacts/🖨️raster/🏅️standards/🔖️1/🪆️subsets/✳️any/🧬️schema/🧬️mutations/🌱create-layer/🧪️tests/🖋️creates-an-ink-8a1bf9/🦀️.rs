...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-layer/🖋️creates-an-ink-8a1bf9/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-layer/🖋️creates-an-ink-8a1bf9/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-layer/🖋️creates-an-ink-8a1bf9/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-layer/🖋️creates-an-ink-8a1bf9/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-layer/🖋️creates-an-ink-8a1bf9/🎯️outcome/🔣️.json");
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert!(dsl::json::value_eq_ignoring_object_order(&reencoded, &original), "create-layer/creates-an-ink-layer-inside-the-artwork-group: committed {side} JSON is not canonical");
    }
    ...
    assert!(dsl::json::value_eq_ignoring_object_order(&reencoded, &original), "create-layer/creates-an-ink-layer-inside-the-artwork-group: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(dsl::json::Value::as_str), Some("applied"), "create-layer/creates-an-ink-layer-inside-the-artwork-group declares an applied outcome");
...
async fn produces_committed_diff() {
    ...
    assert!(dsl::json::value_eq_ignoring_object_order(&encoded, &committed), "create-layer/creates-an-ink-layer-inside-the-artwork-group: produced diff differs from the committed 🔺️diff/🔣️.json");
...
async fn committed_diff_is_canonical() {
    ...
    assert!(dsl::json::value_eq_ignoring_object_order(&reencoded, &original), "create-layer/creates-an-ink-layer-inside-the-artwork-group: committed diff JSON is not canonical");
}
