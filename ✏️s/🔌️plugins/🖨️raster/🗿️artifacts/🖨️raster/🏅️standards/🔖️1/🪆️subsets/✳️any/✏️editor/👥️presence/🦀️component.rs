...
impl protocol::MutationDiff<RasterPresence> for RasterPresence {
    async fn apply(&self, _base: &RasterPresence) -> protocol::MutationApplyResult<RasterPresence> {
        Ok({
            self.clone()
        })
    }
...
impl Mutation<RasterPresence> for RasterPresenceMutation {
    ...
    async fn diff(&self, _base: &RasterPresence) -> protocol::MutationOutcome<RasterPresence> {
        match self {
            Self::Snapshot { presence } => protocol::MutationOutcome::new(presence.clone()),
        }
    }

    async fn inverse(&self, base: &RasterPresence) -> Vec<Self> {
