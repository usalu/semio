...
pub fn serialize_bytes(snapshot: &RasterSnapshot) -> Result<Vec<u8>, String> {
    ...
    let target: semio_s_plugin_stdio::artifacts::jpg::JpgSnapshot = semio_image_to_format(&image, JPG_DIALECT)?;
    semio_s_plugin_stdio::artifacts::jpg::io::encode_jpg(&target).map_err(|error| format!("{error:?}"))
}
