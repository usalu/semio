//! 📤️ Export RasterSnapshot as .gif.

use semio_framework_plugin::{DocumentCodec, IoError, JsonCodec, MediaFormat};

//#region 🔖️Export
pub fn export(snapshot: &crate::artifacts::raster::RasterSnapshot) -> Result<Vec<u8>, IoError> {
...
pub fn register() {
    let kind = "2d.raster";
    let format = MediaFormat::Gif;
    semio_framework_os::register_os_media_export_handler(kind, format, move |doc| {
        let snapshot: crate::artifacts::raster::RasterSnapshot =
