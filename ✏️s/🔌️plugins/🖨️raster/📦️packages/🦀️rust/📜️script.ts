#!/usr/bin/env bun
/** 🖨️ `@semio-tech/raster-plugin` router: `bun ./📜️script.ts test`. */
...
class TestScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    ...
    await runCargoTestBudgeted(["semio-s-plugin-raster"], this.repoRoot, rest);
  }
}
...
class DescribeScript extends BundleScript {
  run(): void {
    process.exit(describePluginComponent(this.repoRoot, "semio-s-plugin-raster", join(this.root, "..", "..")));
  }
}
