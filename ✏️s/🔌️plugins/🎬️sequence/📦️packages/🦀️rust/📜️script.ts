#!/usr/bin/env bun
/** 🎬️ `@semio-tech/sequence-plugin` router: `bun ./📜️script.ts test`. */
...
class TestScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    ...
    await runCargoTestBudgeted(["semio-s-plugin-sequence"], this.repoRoot, rest);
  }
}
...
class DescribeScript extends BundleScript {
  run(): void {
    process.exit(describePluginComponent(this.repoRoot, "semio-s-plugin-sequence", join(this.root, "..", "..")));
  }
}
