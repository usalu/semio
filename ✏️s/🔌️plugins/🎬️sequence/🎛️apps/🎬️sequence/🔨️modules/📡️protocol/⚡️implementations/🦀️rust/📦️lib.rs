...
mod tests {
    ...
    fn command_envelope_round_trip_holds_for_an_applied_operation() {
        ...
        let envelope = store::create_document_envelope::<sequence::SequenceFixture, SequenceOperation>(sequence::SEQUENCE_FIXTURE_SCHEMA, "sequence-envelope-test", default_fixture(), None);
        let mut doc_store = store::DocumentStore::new(envelope);
        doc_store
            .dispatch(store::DocumentCommand::Apply {
                operations: vec![SequenceOperation::StepsAdd { index: 2, item: sequence::SequenceStep { id: "step-7".into(), kind: "log.print".into(), params: sequence::StepParams::new(), x: 12.0, y: 24.0, slot: None, collapsed: false } }],
        ...
        store::test_support::assert_command_envelope_round_trip::<sequence::SequenceFixture, SequenceOperation>(edit, &DocumentId(doc_store.envelope().id.clone()), &SchemaId(doc_store.envelope().schema.clone()));
    }
