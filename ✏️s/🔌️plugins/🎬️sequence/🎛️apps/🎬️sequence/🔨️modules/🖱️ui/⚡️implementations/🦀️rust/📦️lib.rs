...
use semio_framework_plugin::{
    ...
    DocumentView, DslValue, Emit, Fault, Label, Locale, LocalizedLabel, Media, MediaClass, MediaError, MediaForm, MediaPayload, MediaType, NodeGraphEdgeRecord, NodeGraphNodeRecord, NodeGraphPortRecord, NodeGraphScene, NodeGraphViewport, OsMediaCapability,
    ...
    fn handle(&self, command: &SequenceCommand, doc: &DocumentView<'_, SequenceFixture>, cfg: &ConfigView<'_, SequenceConfig>) -> Result<Emit<SequenceOperation, SequenceConfigOperation>, Fault> {
        ...
        match command {
            SequenceCommand::AddStep { kind, x, y } => {
                ...
                Ok(Emit { document_operations: sequence_fixture_operations(fixture, &host.fixture), config_operations: vec![SequenceConfigOperation::SetSelection { step_ids: vec![id] }], ..Default::default() })
            }
...
//#region 🔖️WasmSession
/// 🌉️ Direct-canvas WASM bridge — a bespoke JS API (`SequenceSession`) for driving the sequence DAG
/// canvas outside the generic `DocumentApp`/`PluginApp` render pipeline (raw GPU frame painting,
/// pointer/wheel routing). Lives in the UI crate.
