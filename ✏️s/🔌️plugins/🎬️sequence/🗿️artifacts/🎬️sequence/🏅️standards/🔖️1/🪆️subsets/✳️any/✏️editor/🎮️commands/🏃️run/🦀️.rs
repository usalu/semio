...
use crate::editor::sequence::{host_from_host_snapshot, retire_run_result_cold, sequence_host_snapshot_from_children};
...
pub mod run_command {
    ...
    pub fn handle(_payload: &Run, doc: &ArtifactView<'_, SequenceSnapshot>, _cfg: &ConfigView<'_, NoConfig>) -> Result<Emit<SequenceMutation, NoConfigMutation>, Fault> {
        let fixture = sequence_host_snapshot_from_children(doc.snapshot, &doc.children)?;
        let host = neural_engine::ColdOwner::new(host_from_host_snapshot(&fixture));
