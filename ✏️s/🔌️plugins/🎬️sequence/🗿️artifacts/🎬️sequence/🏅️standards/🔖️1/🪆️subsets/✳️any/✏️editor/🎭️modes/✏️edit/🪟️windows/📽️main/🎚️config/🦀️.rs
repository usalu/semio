...
impl store::ArtifactDsl for SequenceMainWindowConfig {
    const EXTENSION: &'static str = "sequencemainwindowcfg";
    fn envelope_id() -> &'static str { "s.sequence.sequence.mainwindowconfig" }
    ...
    fn print_dsl(&self) -> String {
        ...
        let body = serde_json::to_string_pretty(&value).expect("Sequence main window config JSON");
...
impl store::ArtifactPack for SequenceMainWindowConfig {
    ...
    fn decode_pack_with(bytes: &[u8], _options: &store::PackDecodeOptions) -> Result<Self, store::PackError> {
        ...
        if !envelope.matches_identity(<Self as store::ArtifactDsl>::envelope_id(), store::semio_format::Component::Pack, 1) { return Err(store::PackError::Schema("Sequence window pack envelope mismatch".into())); }
