...
async fn add_step_command_appends_step() {
    ...
    assert!(app.snapshot().expect("projection").to_host_document().steps.len() > 2);
}

#[semio_framework_async_macros::async_test]
async fn remove_step_command_deletes_step() {
    let mut app = new_app().await;
    let step_id = app.snapshot().expect("projection").to_host_document().steps[0].id.clone();
    dispatch(&mut app, SequenceCommand::RemoveStep(RemoveStep { id: step_id.clone() })).await;
    assert!(app.snapshot().expect("projection").to_host_document().steps.iter().all(|step| step.id != step_id));
}
...
async fn delete_selection_removes_the_live_selected_step() {
    ...
    assert!(!app.snapshot().expect("projection").to_host_document().steps.iter().any(|step| step.id == "step-1"), "selected step must be deleted");
}
