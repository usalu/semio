use crate::editor::sequence::unit_tests::context::{dispatch, live_host_snapshot, new_app, new_app_with_registry_wired, select_steps};
...
async fn add_step_command_appends_step() {
    ...
    assert!(live_host_snapshot(&app).await.steps.len() > 2);
}

#[semio_framework_async_macros::async_test]
async fn remove_step_command_deletes_step() {
    let mut app = new_app().await;
    let step_id = live_host_snapshot(&app).await.steps[0].id.clone();
    dispatch(&mut app, SequenceCommand::RemoveStep(RemoveStep { id: step_id.clone() })).await;
    assert!(live_host_snapshot(&app).await.steps.iter().all(|step| step.id != step_id));
}
...
async fn delete_selection_removes_the_live_selected_step() {
    ...
    assert!(!live_host_snapshot(&app).await.steps.iter().any(|step| step.id == "step-1"), "selected step must be deleted");
}
