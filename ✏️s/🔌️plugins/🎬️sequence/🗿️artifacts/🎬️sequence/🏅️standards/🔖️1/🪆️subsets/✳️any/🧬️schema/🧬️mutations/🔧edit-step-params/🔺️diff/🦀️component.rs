...
//#region 🔖️Diff
pub fn diff(payload: &super::mutation::EditStepParams, base: &SequenceSnapshot) -> protocol::MutationOutcome<SequenceDiff> {
    ...
    if existing.params == payload.params {
        return protocol::MutationOutcome::empty().warn("mutation.no-op", format!("Step \"{}\" parameters are already unchanged.", payload.id));
    }
