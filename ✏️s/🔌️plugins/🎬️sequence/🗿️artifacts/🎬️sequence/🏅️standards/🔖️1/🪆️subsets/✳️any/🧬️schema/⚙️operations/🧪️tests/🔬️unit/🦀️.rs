...
async fn create_edit_delete_step_round_trip() {
    ...
    assert_eq!(moved.to_host_snapshot().steps.iter().find(|step| step.id == "step-99").unwrap().x, 120.0);
    let removed = round_trip(&moved, &delete_step("step-99".into()));
    assert!(!removed.to_host_snapshot().steps.iter().any(|step| step.id == "step-99"));
}
...
async fn txt_dsl_carrier_round_trips_exactly() {
    ...
    let exported = export::txt::v_utf_8::any::SequenceIntoTxt::serialize(&snapshot).await.expect("dsl txt export");
    let IoPayload::Text(text) = exported.value else { panic!("txt is a text payload") };
    let back = import::txt::v_utf_8::any::TxtIntoSequence::deserialize(&IoPayload::Text(text)).await.expect("dsl txt import");
