...
mod move_step_helper {
    ...
    pub async fn move_all_steps_to_origin(app: &mut SequenceApp) {
        let ids: Vec<String> = app.snapshot().expect("projection").to_host_document().steps.iter().map(|step| step.id.clone()).collect();
        for id in &ids {
...
async fn set_orientation_command_changes_reorganize_layout_axis() {
    ...
    let ys: Vec<f64> = app.snapshot().expect("projection").to_host_document().steps.iter().map(|step| step.y).collect();
...
async fn reorganize_command_spreads_step_positions_apart() {
    ...
    move_all_steps_to_origin(&mut app).await;
    dispatch(&mut app, SequenceCommand::Reorganize(Reorganize {})).await;
    let xs: Vec<f64> = app.snapshot().expect("projection").to_host_document().steps.iter().map(|step| step.x).collect();
