...
async fn write_child(out: &mut Vec<u8>, c: &SequenceContentChild) {
    write_str_lp(out, &c.child_id);
    write_ref(out, &c.target);
}
async fn read_child(reader: &mut store::ByteReader<'_>) -> Result<SequenceContentChild, String> {
    let child_id = read_str_lp(reader)?;
    let target = read_ref(reader)?;
    Ok(store::ArtifactChild::new(child_id, target))
}

async fn encode_sequence_snapshot_binary(s: &SequenceSnapshot) -> Vec<u8> {
...
async fn decode_sequence_snapshot_binary(bytes: &[u8]) -> Result<SequenceSnapshot, String> {
    const PACK_BINARY_FORMAT: u8 = 1;
