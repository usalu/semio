...
//#region 🔖️Inverse
pub async fn inverse(payload: &super::DisconnectSteps, base: &SequenceSnapshot) -> Vec<SequenceMutation> {
    let scene = sequence_working_scene(base);
    match scene.edges.iter().find(|edge| edge.id == payload.id) {
        Some(edge) => vec![crate::artifacts::sequence::mutations::connect_steps(edge.id.clone(), edge.from.clone(), edge.to.clone())],
        None => Vec::new(),
    }
}
//#endregion 🔖️Inverse
