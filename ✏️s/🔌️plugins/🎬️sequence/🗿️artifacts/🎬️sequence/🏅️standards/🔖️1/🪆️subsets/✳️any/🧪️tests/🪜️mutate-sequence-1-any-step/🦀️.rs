//! 🦀️ SEQUENCE shared-envelope step-vocabulary exhaustive mutation case — Rust adapter. Added in
//! ticket `26/09/02/SEPARATE-ARTIFACT-STANDARD-SUBSET-IMPLEMENTATIONS-AND-FIXTURE-TEST-EVERY-MUTATION`
//! (shard E3) to close `unregistered-mutation-vocabulary` for `✳️any`'s own
//! `🧬️schema/🧬️mutations`/`🚪️io/🧬️mutations` — the shared `SequenceMutation` aggregate enum and wire
//! codec every one of `🪜️step`'s six kinds dispatches through, but which itself owned no catalog once
//! its own real kinds moved to their smallest subset owner. This case is a full, real duplicate — not
//! a stub — of `🪜️step`'s own case: same fixtures (copied per B3's own established
//! shared-fixture-across-case-boundary precedent), same assertions, same law calls, applied through
//! the identical `apply_sequence_mutation`/`inverse_sequence_mutation` functions that case itself
//! calls.
//!
...
//#region 🔖️Vocabulary
/// 🏷️ `🪜️step`'s own `KINDS` slice, duplicated rather than imported so the oracle-only build never
/// links the subject crate.
...
const SEQUENCE_ASSET: &str = "shared://🪜️mutate-sequence-1-any-step/🗣️.dsl.semio";

/// 🧫️ The three steps and one edge the composed content child is seeded with — a local copy of the
/// identical fixture `🪜️step`'s own case already carries.
const BASE_SCENE: &str = "shared://🪜️mutate-sequence-1-any-step/🎬️base-scene.json";
...
mod subject {
    ...
    /// child re-minted around the committed base scene. The `schema` field is never a literal here:
    /// it is whatever the committed artifact itself decodes to.
    ...
    /// ⚖️ The case's OWN semantic projection, read back through production's
