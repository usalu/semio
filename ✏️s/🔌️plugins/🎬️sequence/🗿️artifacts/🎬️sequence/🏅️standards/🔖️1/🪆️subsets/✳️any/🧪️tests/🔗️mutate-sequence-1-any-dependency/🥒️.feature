...
Feature: Apply every typed SEQUENCE dependency mutation through the shared envelope-level dispatch
  ...
  child (steps and edges) is seeded from `shared://🔗️mutate-sequence-1-any-dependency/🎬️base-scene.json`, a local copy of the identical
  ...
  Scenario Outline: Apply <id> through the shared aggregate dispatch and observe it move
    Given the real committed sequence artifact shared://🔗️mutate-sequence-1-any-dependency/🗣️.dsl.semio
    And its composed content child seeded from shared://🔗️mutate-sequence-1-any-dependency/🎬️base-scene.json
