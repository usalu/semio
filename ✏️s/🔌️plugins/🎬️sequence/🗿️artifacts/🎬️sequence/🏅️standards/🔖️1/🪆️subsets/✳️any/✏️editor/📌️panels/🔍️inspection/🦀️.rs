...
//#region 🔖️Render
pub fn render(host_snapshot: &SequenceHostSnapshot, selected: &[String], labels: &SequenceLabels) -> UiAssemblyResult<BuiltNode> {
    let steps: Vec<&SequenceStep> = selected.iter().filter_map(|id| host_snapshot.steps.iter().find(|step| &step.id == id)).collect();
