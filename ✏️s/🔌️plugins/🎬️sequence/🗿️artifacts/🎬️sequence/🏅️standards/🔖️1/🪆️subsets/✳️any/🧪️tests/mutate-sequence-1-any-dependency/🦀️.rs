//! 🦀️ SEQUENCE shared-envelope dependency-vocabulary exhaustive mutation case — Rust adapter. Added
//! in ticket `26/09/02/SEPARATE-ARTIFACT-STANDARD-SUBSET-IMPLEMENTATIONS-AND-FIXTURE-TEST-EVERY-MUTATION`
...
//! codec both of `✳️dependency`'s kinds dispatch through, but which itself owned no catalog once its
//! own real kinds moved to their smallest subset owner. This case is a full, real duplicate — not a
//! stub — of `✳️dependency`'s own case: same fixtures (copied per B3's own established
...
//#region 🔖️Vocabulary
/// 🏷️ `✳️dependency`'s own `KINDS` slice, duplicated rather than imported so the oracle-only build
/// never links the subject crate.
