...
impl protocol::MutationDiff<SequencePresence> for SequencePresence {
    async fn apply(&self, _base: &SequencePresence) -> protocol::MutationApplyResult<SequencePresence> {
        Ok({ self.clone() })
    }
...
impl store::ArtifactDsl for SequencePresence {
...
impl ArtifactPack for SequencePresence {
...
impl Mutation<SequencePresence> for SequencePresenceMutation {
    ...
    async fn diff(&self, _base: &SequencePresence) -> protocol::MutationOutcome<SequencePresence> {
        match self {
            Self::Snapshot { presence } => protocol::MutationOutcome::new(presence.clone()),
        }
    }

    async fn inverse(&self, base: &SequencePresence) -> Vec<Self> {
        vec![Self::Snapshot { presence: base.clone() }]
    }
}

impl protocol::OpText for SequencePresenceMutation {
...
impl protocol::OpBinary for SequencePresenceMutation {
