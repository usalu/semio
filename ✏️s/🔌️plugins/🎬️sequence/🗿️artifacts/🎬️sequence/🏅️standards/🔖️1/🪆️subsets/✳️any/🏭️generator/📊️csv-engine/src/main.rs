...
fn main() {
    ...
    for (kind, before, after) in corpus() {
        ...
        let dir = std::path::Path::new(&out).join(directory_name(kind));
        std::fs::create_dir_all(&dir).expect("fixture directory");
        for (name, rows) in [("⬅️before.csv", &before), ("➡️after.csv", &after)] {
