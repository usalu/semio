...
async fn step_content_round_trips_through_the_composed_child_snapshot() {
    let fixture = default_snapshot().to_host_document();
...
async fn fixture_projection_rejects_a_wire_only_parent_instead_of_defaulting_empty() {
    ...
    assert!(!snapshot.try_to_host_document().expect("owned scene projects").steps.is_empty());
    ...
    assert_eq!(decoded.try_to_host_document(), Err(store::ArtifactChildMaterializationError::Absent));
}
