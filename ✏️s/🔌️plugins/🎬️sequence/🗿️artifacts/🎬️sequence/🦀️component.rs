...
/// 🔎 Reads the exact child's typed live scene. A wire-only child fails soft until its content
/// is materialized by the host.
pub fn sequence_working_scene_for_handle(handle: &SequenceContentChild) -> SequenceWorkingScene {
    handle.local_owner::<SequenceWorkingScene>().map(|scene| scene.as_ref().clone()).unwrap_or_default()
}
