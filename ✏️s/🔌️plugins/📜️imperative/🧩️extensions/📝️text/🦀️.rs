...
pub fn catalogue_json(registry: &Registry) -> String {
    ...
    to_string(&object([
        ...
        ("sections".to_string(), array([object([("id".to_string(), JsonValue::from("text")), ("title".to_string(), JsonValue::from("Text")), ("items".to_string(), items)])])),
    ]))
}
...
pub fn imperative_module_contribution() -> semio_framework::ProgramContributionEntry {
    ...
    imperative_extension_sdk::imperative_module_contribution(EXTENSION_ID, "text", "Text", "message-square", imperative_extension_sdk::build_manifest_json("text", "Text", MODULE_VERSION, &registry, Some(&catalogue)))
}
...
pub fn imperative_module_topic_contribution() -> semio_framework::TopicContribution {
    let registry = module_registry();
    let catalogue = catalogue_json(&registry);
    imperative_extension_sdk::imperative_module_topic_contribution("text", "Text", "message-square", imperative_extension_sdk::build_manifest_json("text", "Text", MODULE_VERSION, &registry, Some(&catalogue)))
}
