...
/// old body never matched a real `ProcedureSnapshot` shape anyway. Mirrors `🔱️jack`'s own stub
/// for the same reason: an arbitrary CSV grid has no tabular correspondence to a `Path` of
/// `Step`s to reconstruct.
pub fn deserialize(from: &CsvSnapshot) -> Result<ProcedureSnapshot, store::TextError> {
    let _ = (STDIO_CSV_DOCUMENT_SCHEMA, from);
    Ok(ProcedureSnapshot::default())
}

pub fn deserialize_bytes(bytes: &[u8]) -> Result<ProcedureSnapshot, store::TextError> {
    <ProcedureSnapshot as store::ArtifactPack>::decode_pack(bytes).map_err(|e| store::TextError::new(e.to_string(), dsl::TextSpan::at(1, 1)))
}
