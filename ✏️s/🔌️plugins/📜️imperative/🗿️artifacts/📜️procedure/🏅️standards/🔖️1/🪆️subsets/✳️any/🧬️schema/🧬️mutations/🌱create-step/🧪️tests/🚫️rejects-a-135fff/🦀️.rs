//! 🧪️ `create-step` fixture — `🚫️rejects-a-duplicate-step-id-at-the-root-path`.
//!
...
//! thread-local scratch cache (`ProcedureWorkingScene`): a persisted handle only round-trips to
...
async fn the_unused_inverse_is_a_delete_of_the_contested_id() {
    ...
    let ProcedureMutation::DeleteStep(undo) = &inverse[0] else {
...
async fn produces_no_diff() {
    let produced = built_outcome();
    assert_eq!(produced.diff(), &ProcedureDiff::default(), "create-step/rejects-a-duplicate-step-id-at-the-root-path: a fatal create must produce the default diff");
...
async fn committed_absent_marker_is_empty() {
    assert!(ABSENT_DIFF.is_empty(), "create-step/rejects-a-duplicate-step-id-at-the-root-path: 🚫️.absent must stay empty, never a stand-in diff");
}
