...
pub fn diff(payload: &super::CreateStep, base: &ProcedureSnapshot) -> protocol::MutationOutcome<ProcedureDiff> {
    if let Some(owner) = &payload.path_ref.owner {
        let path = crate::artifacts::procedure::procedure_working_scene(base).path;
    ...
    let existing = crate::artifacts::procedure::mutations::resolve_steps(base, &payload.path_ref);
