...
async fn the_inverse_reorders_the_tail_step_back_to_index_two() {
    ...
    let ImperativeMutation::ReorderSteps(undo) = &inverse[0] else {
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "reorder-steps/warns-that-an-over-clamped-index-leaves-the-tail-step-in-place: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    let decoded: ImperativeDiff = serde_json::from_str(DIFF).expect("committed reorder-steps diff decodes");
    assert_eq!(decoded, ImperativeDiff::default(), "reorder-steps/warns-that-an-over-clamped-index-leaves-the-tail-step-in-place: a no-op's committed diff must be the type's own default");
