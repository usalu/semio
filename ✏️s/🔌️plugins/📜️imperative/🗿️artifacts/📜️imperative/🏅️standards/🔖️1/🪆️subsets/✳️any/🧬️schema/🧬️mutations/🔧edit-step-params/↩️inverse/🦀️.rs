...
//#region 🔖️Inverse
pub fn inverse(payload: &super::EditStepParams, base: &ImperativeSnapshot) -> Vec<ImperativeMutation> {
    ...
    match steps.iter().find(|step| step.id == payload.id) {
        Some(step) => vec![crate::artifacts::imperative::mutations::edit_step_params::edit_step_params(payload.path_ref.clone(), payload.id.clone(), step.params.clone())],
        None => Vec::new(),
    }
}
//#endregion 🔖️Inverse
