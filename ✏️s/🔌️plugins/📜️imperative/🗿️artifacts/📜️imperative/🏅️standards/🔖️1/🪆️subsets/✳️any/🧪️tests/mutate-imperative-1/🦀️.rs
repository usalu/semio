...
//! (`../../🏅️standards/🔖️1/🪆️subsets/✳️any/🔣️oracle.json`): `imperative.document` is a
...
//! repository's own `apply_imperative_mutation_reporting` over the full four-kind
//! `ImperativeMutation` vocabulary.
//!
...
//! feature's own `Examples` row and is decoded by `seed_imperative_flow_json`. That restatement is
...
//! own production code exports the bridges instead: `decode_imperative_snapshot_json`/
//! `encode_imperative_snapshot_json`/`parse_imperative_dsl`/`print_imperative_dsl`
//! (`…/🧬️schema/📸️snapshot/🦀️.rs`) and `decode_imperative_mutation_json`/
//! `seed_imperative_flow_json`/`apply_imperative_mutation_reporting`/
//! `inverse_imperative_mutation_steps`/`imperative_program_summary`
//! (`…/🧬️schema/🧬️mutations/🦀️.rs`).

use semio_repo_test_host::{parse_json, Adapter, Context, Json, Outcome};

//#region 🔖️Kinds
/// 🏷️ Mirrors `ImperativeMutation::KINDS` (`../../🏅️standards/🔖️1/🪆️subsets/✳️any/🧬️schema/
...
fn fixture_text(kind: &str) -> (&'static str, &'static str, &'static str, &'static str) {
    match kind {
        ...
        other => panic!("mutate-imperative-1: no specification vector registered for kind {other:?}"),
    }
}
...
mod subject {
    ...
    use semio_s_plugin_imperative::artifacts::imperative::standards::v1::subsets::any::schema::mutations::{apply_imperative_mutation_reporting, decode_imperative_mutation_json, imperative_program_summary, inverse_imperative_mutation_steps, seed_imperative_flow_json, ImperativeMutation};
    use semio_s_plugin_imperative::artifacts::imperative::standards::v1::subsets::any::schema::snapshot::{decode_imperative_snapshot_json, encode_imperative_snapshot_json, parse_imperative_dsl, print_imperative_dsl, ImperativeSnapshot};
    ...
    fn mutation_of(text: &str, label: &str, kind: &str) -> Result<ImperativeMutation, String> {
        decode_imperative_mutation_json(text).map_err(|error| format!("mutate-imperative-1: the {label} payload for {kind:?} must decode: {error}"))
    }

    fn projection(snapshot: &ImperativeSnapshot) -> Result<Json, String> {
        parse_json(&encode_imperative_snapshot_json(snapshot))
    }

    fn disagreement(what: &str, got: &ImperativeSnapshot, expected: &ImperativeSnapshot) -> String {
        format!("{what}\n     got: {} [{}]\nexpected: {} [{}]", encode_imperative_snapshot_json(got), imperative_program_summary(got), encode_imperative_snapshot_json(expected), imperative_program_summary(expected))
    }
    ...
    fn vector_reports(kind: &str, code: &str, level: &str, declared: &Json, raised: &[(String, String)], before: &ImperativeSnapshot, after: &ImperativeSnapshot, expected: &ImperativeSnapshot) -> Result<(), String> {
    ...
    fn application_is_observable(kind: &str, raised: &[(String, String)], base: &ImperativeSnapshot, mutated: &ImperativeSnapshot) -> Result<(), String> {
        if !raised.is_empty() {
        ...
        if mutated.flow.child_id == base.flow.child_id {
            return Err(format!("mutate-{kind}: applying this kind left the content-addressed flow handle at {} — the mutation never reached the program, so the scenario would report a pass for a mutation it never observed ([{}])", base.flow.child_id, imperative_program_summary(mutated)));
        }
        Ok(())
    }
    //#endregion 🔖️Laws
    ...
    pub fn mutate(kind: &'static str) -> impl Fn(&Context) -> Result<Outcome, String> {
        move |ctx: &Context| {
            ...
            let expected = decode_imperative_snapshot_json(after).map_err(|error| format!("mutate-imperative-1: the committed after-snapshot for {kind:?} must decode: {error}"))?;
            let vector = mutation_of(vector, "committed vector", kind)?;
            let mut replayed = base.clone();
            let raised = apply_imperative_mutation_reporting(&mut replayed, &vector);
            ...
            let applied_messages = apply_imperative_mutation_reporting(&mut applied, &payload);
    ...
    pub fn inverse(kind: &'static str) -> impl Fn(&Context) -> Result<Outcome, String> {
        move |ctx: &Context| {
            ...
            let raised = apply_imperative_mutation_reporting(&mut current, &payload);
            if !raised.is_empty() {
                return Err(format!("inverse-{kind}: the forward mutation was rejected: {raised:?}"));
            }
            if current.flow.child_id == base.flow.child_id {
                return Err(format!("inverse-{kind}: the forward mutation left the flow handle untouched, so restoring it proves nothing ([{}])", imperative_program_summary(&current)));
            }
            for step in inverse_imperative_mutation_steps(&payload, &base) {
                let undone = apply_imperative_mutation_reporting(&mut current, &step);
    ...
    /// dialects that a parser returning `ImperativeSnapshot::default()` cannot satisfy.
    pub fn round_trip(ctx: &Context) -> Result<Outcome, String> {
        ...
        let parsed = parse_imperative_dsl(&text)?;
        let rendered = encode_imperative_snapshot_json(&parsed);
        ...
        let printed = print_imperative_dsl(&parsed);
        let reparsed = parse_imperative_dsl(&printed)?;
        if reparsed != parsed {
