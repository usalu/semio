...
async fn the_rejected_delete_leaves_both_scopes_intact() {
    ...
    let scene = crate::artifacts::imperative::imperative_working_scene(&carried);
...
async fn produces_no_diff() {
    let produced = built_outcome();
    assert_eq!(produced.diff(), &ImperativeDiff::default(), "delete-step/rejects-a-root-step-id-addressed-inside-a-branch-body: a target-missing delete must produce the default diff");
...
async fn committed_absent_marker_is_empty() {
    assert!(ABSENT_DIFF.is_empty(), "delete-step/rejects-a-root-step-id-addressed-inside-a-branch-body: 🚫️component.absent must stay empty, never a stand-in diff");
}
