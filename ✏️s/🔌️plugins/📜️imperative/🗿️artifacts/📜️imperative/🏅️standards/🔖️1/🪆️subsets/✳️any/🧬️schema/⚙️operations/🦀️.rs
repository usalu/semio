...
use crate::artifacts::imperative::mutations::{create_step, delete_step, edit_step_params, register_imperative_mutation_descriptors, reorder_steps, ImperativeMutation};
...
/// `mutate-imperative-1` needs the same materialization from outside, where neither `Path` nor its
/// `Dictionary`/`Value` argument types
/// can be constructed, so the program travels as JSON and is decoded here.
pub fn seed_imperative_flow_json(snapshot: &mut ImperativeSnapshot, program_json: &str) -> Result<(), String> {
    let path: Path = serde_json::from_str(program_json).map_err(|error| error.to_string())?;
    crate::artifacts::imperative::materialize_imperative_flow(&mut snapshot.flow, &path);
    Ok(())
}
...
mod tests {
    ...
    async fn dispatch_registers_semantic_descriptors() {
        register_imperative_mutation_descriptors(::semio_framework_os_kernel::StateClass::Artifact).expect("mutation descriptor registration");
