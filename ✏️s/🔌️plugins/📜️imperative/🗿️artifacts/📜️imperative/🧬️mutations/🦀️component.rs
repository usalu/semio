...
impl protocol::Mutation<ImperativeDocument> for ImperativeMutation {
    ...
    fn inverse(&self, projection: &ImperativeDocument) -> Vec<Self> {
...
pub fn resolve_steps<'a>(document: &'a ImperativeDocument, path_ref: &PathRef) -> Option<&'a [Step]> {
...
mod tests {
    ...
    fn add_step_op_round_trips() {
        ...
        let operation = ImperativeMutation { path_ref: PathRef::default(), collection: protocol::CollectionMutation::Add { index: 0, item: step("step-x", "log.print") } };
    ...
    fn move_step_op_round_trips() {
        let document = crate::artifacts::imperative::engine::default_document();
        let operation = ImperativeMutation { path_ref: PathRef::default(), collection: protocol::CollectionMutation::Move { id: "step-1".into(), to_index: 1 } };
        store::test_support::assert_operation_round_trip(&document, operation.clone());
        store::test_support::assert_op_line_round_trip(&operation);
        store::test_support::assert_store_roundtrip(document, operation);
    }
    ...
    fn patch_step_params_op_round_trips() {
        ...
        let document = crate::artifacts::imperative::engine::default_document();
        let operation = ImperativeMutation { path_ref: PathRef::default(), collection: protocol::CollectionMutation::Patch { id: "step-1".into(), patch: Dictionary::new().insert("key", Value::Atom(Atom::String("renamed".into()))) } };
        store::test_support::assert_operation_round_trip(&document, operation.clone());
        store::test_support::assert_op_line_round_trip(&operation);
        store::test_support::assert_store_roundtrip(document, operation);
    }
    ...
    fn add_step_into_nested_control_body_round_trips() {
        ...
        let operation = ImperativeMutation { path_ref, collection: protocol::CollectionMutation::Add { index: 0, item: step("step-nested", "log.print") } };
        store::test_support::assert_operation_round_trip(&document, operation.clone());
        store::test_support::assert_op_line_round_trip(&operation);
        let post = vcs::apply_mutation(&document, &operation);
    ...
    fn operation_backwards_on_unresolvable_path_ref_is_empty() {
        let document = crate::artifacts::imperative::engine::default_document();
        let operation = ImperativeMutation { path_ref: PathRef { owner: Some("missing".into()), slot: Some("then".into()) }, collection: protocol::CollectionMutation::Remove { id: "step-x".into() } };
        assert!(protocol::Mutation::inverse(&operation, &document).is_empty());
    }
}
//#endregion 🧪️Tests
