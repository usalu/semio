//! 📦️ Imperative app — binary document surface + laws (constitutional: pack).
...
mod tests {
    ...
    fn pack_round_trips_and_agrees_with_dsl() {
        let document = imperative_dsl::parse_dsl(imperative_dsl::IMPERATIVE_EXAMPLE_TEXT).expect("parse 📜️default.imperative");
    ...
    fn pack_round_trips_representative_document_with_nested_control_body() {
        ...
        let mut document = imperative_dsl::parse_dsl(imperative_dsl::IMPERATIVE_EXAMPLE_TEXT).expect("parse 📜️default.imperative");
