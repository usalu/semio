    ...
    fn handle(&self, command: &ImperativeCommand, doc: &DocumentView<'_, ImperativeDocument>, cfg: &ConfigView<'_, ImperativeConfig>) -> Result<Emit<ImperativeOperation, ImperativeConfigOperation>, Fault> {
        ...
        match command {
            ...
            ImperativeCommand::MoveStep { id, index } => {
                if resolve_contains(document, None, None, id) {
                    Ok(Emit::operations(vec![ImperativeOperation { path_ref: PathRef::default(), collection: CollectionOperation::Move { id: id.clone(), to: *index } }])
                } else {
                    Ok(Emit::default()
                }
            }
            ImperativeCommand::MoveStepAt { id, index, owner, slot } => {
                if resolve_contains(document, owner.as_deref(), slot.as_deref(), id) {
                    let path_ref = path_ref_from(owner.as_deref(), slot.as_deref(), document);
                    Ok(Emit::operations(vec![ImperativeOperation { path_ref, collection: CollectionOperation::Move { id: id.clone(), to: *index } }])
                } else {
                    Ok(Emit::default()
                }
            }
            ImperativeCommand::SetStepParams { id, params } => {
                if resolve_contains(document, None, None, id) {
                    Ok(Emit::operations(vec![ImperativeOperation { path_ref: PathRef::default(), collection: CollectionOperation::Patch { id: id.clone(), patch: value_dsl_map_to_dictionary(params) } }])
                } else {
                    Ok(Emit::default()
                }
            }
            ImperativeCommand::SetStepParamsAt { id, owner, slot, params } => {
                if resolve_contains(document, owner.as_deref(), slot.as_deref(), id) {
                    let path_ref = path_ref_from(owner.as_deref(), slot.as_deref(), document);
                    Ok(Emit::operations(vec![ImperativeOperation { path_ref, collection: CollectionOperation::Patch { id: id.clone(), patch: value_dsl_map_to_dictionary(params) } }])
                } else {
                    Ok(Emit::default()
                }
            }
