...
//#region 🔖️Presence
/// 👥️ Shareable live subset of imperative view state (peer step selection in the script surface).
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize, dsl::DslDocument)]
#[serde(rename_all = "camelCase", default)]
#[dsl(extension = "imperative.presence")]
#[dsl(layout = "lines")]
pub struct ImperativePresence {
    pub selected_step_ids: Vec<String>,
}

impl Default for ImperativePresence {
    fn default() -> Self {
        Self { selected_step_ids: Vec::new() }
    }
}

impl protocol::MutationDiff<ImperativePresence> for ImperativePresence {
    fn apply(&self, _base: &ImperativePresence) -> ImperativePresence {
        self.clone()
    }
...
impl store::DocumentDsl for ImperativePresence {
...
impl DocumentPack for ImperativePresence {
...
impl Mutation<ImperativePresence> for ImperativePresenceMutation {
    ...
    fn diff(&self, _base: &ImperativePresence) -> ImperativePresence {
        match self {
            Self::Snapshot { presence } => presence.clone(),
        }
    }

    fn inverse(&self, base: &ImperativePresence) -> Vec<Self> {
