...
impl Default for Procedural3dPresence {
    fn default() -> Self {
        Self { camera: CameraJson { x: 0.0, y: 0.0, zoom: 1.0 }, preview_camera: Procedural3dPreviewCamera::default(), active_utility_id: String::new(), show_mode: "shaded".into() }
    }
}

impl protocol::MutationDiff<Procedural3dPresence> for Procedural3dPresence {
    async fn apply(&self, _base: &Procedural3dPresence) -> protocol::MutationApplyResult<Procedural3dPresence> {
        Ok({ self.clone() })
    }
...
impl Mutation<Procedural3dPresence> for Procedural3dPresenceMutation {
    ...
    async fn diff(&self, _base: &Procedural3dPresence) -> protocol::MutationOutcome<Procedural3dPresence> {
        match self {
            Self::Snapshot { presence } => protocol::MutationOutcome::new(presence.clone()),
        }
    }

    async fn inverse(&self, base: &Procedural3dPresence) -> Vec<Self> {
