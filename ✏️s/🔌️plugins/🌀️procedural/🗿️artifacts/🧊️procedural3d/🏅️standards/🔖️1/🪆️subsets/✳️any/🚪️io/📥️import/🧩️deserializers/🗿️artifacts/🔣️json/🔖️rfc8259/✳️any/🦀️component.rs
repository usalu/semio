...
pub async fn deserialize(from: &JsonSnapshot) -> Result<Procedural3dSnapshot, store::TextError> {
    let _ = STDIO_JSON_DOCUMENT_SCHEMA;
    let snap: Procedural3dSnapshot = serde_json::from_value(from.to_serde_value()).map_err(|e| store::TextError::new(format!("procedural3d<-json: {e}"), dsl::TextSpan::at(1, 1)))?;
