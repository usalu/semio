...
pub fn render(labels: &Procedural3dLabels) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::BuiltNode> {
    ...
    for section in sections {
        for item in section.items {
            ...
            let mut node = tree_item_with_action(identity::item_key(&item.kind, item.neuron_kind.as_deref(), item.format.as_deref(), item.action.as_deref()), item.name.clone(), None, ActionFactory::new(PROCEDURAL_3D_PLAY_APP_ID).action("addWidget", Some(args))?)?;
