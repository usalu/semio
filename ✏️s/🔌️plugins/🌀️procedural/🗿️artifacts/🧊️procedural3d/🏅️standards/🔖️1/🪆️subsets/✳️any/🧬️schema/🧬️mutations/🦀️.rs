...
pub fn generation_mutation_to_procedural3d(operation: GenerationMutation) -> Procedural3dMutation {
    match operation {
        GenerationMutation::Add { generation } => Procedural3dMutation::CreateGeneration(create_generation::CreateGeneration { generation }),
        GenerationMutation::Remove { id } => Procedural3dMutation::DeleteGeneration(delete_generation::DeleteGeneration { id }),
        GenerationMutation::Rename { id, name } => Procedural3dMutation::RenameGeneration(rename_generation::RenameGeneration { id, new_name: name }),
        GenerationMutation::UpdateValues { id, question_id, value } => Procedural3dMutation::ChangeGenerationValue(change_generation_value::ChangeGenerationValue { id, question_id, new_value: value }),
    }
}
...
pub fn procedural3d_fixture_operations(before: &FlowFixture, after: &FlowFixture) -> Vec<Procedural3dMutation> {
    ...
    for widget in &before.widgets {
        if !after.widgets.iter().any(|entry| widget_id(entry) == widget_id(widget)) {
            operations.push(Procedural3dMutation::DeleteWidget(delete_widget::DeleteWidget { id: widget_id(widget).to_string() }));
        }
    }
    for (index, widget) in after.widgets.iter().enumerate() {
        ...
        match prior {
            Some(previous) if previous != widget => operations.push(Procedural3dMutation::UpdateWidget(update_widget::UpdateWidget { widget: widget.clone() })),
            None => operations.push(Procedural3dMutation::CreateWidget(create_widget::CreateWidget { index, widget: widget.clone() })),
            _ => {}
        }
    }
    for synapse in &before.synapses {
        if !after.synapses.iter().any(|entry| entry.id == synapse.id) {
            operations.push(Procedural3dMutation::DisconnectSynapse(disconnect_synapse::DisconnectSynapse { id: synapse.id.clone() }));
        }
    }
    for (index, synapse) in after.synapses.iter().enumerate() {
        let prior = before.synapses.iter().find(|entry| entry.id == synapse.id);
        match prior {
            Some(previous) if previous != synapse => operations.push(Procedural3dMutation::UpdateSynapse(update_synapse::UpdateSynapse { synapse: synapse.clone() })),
            None => operations.push(Procedural3dMutation::ConnectSynapse(connect_synapse::ConnectSynapse { index, synapse: synapse.clone() })),
            _ => {}
        }
    }
    for id in before.layout.keys() {
        if !after.layout.contains_key(id) {
            operations.push(Procedural3dMutation::DeleteWidgetPosition(delete_widget_position::DeleteWidgetPosition { id: id.clone() }));
        }
    }
    for (id, layout) in &after.layout {
        if before.layout.get(id) != Some(layout) {
            operations.push(Procedural3dMutation::MoveWidget(move_widget::MoveWidget { id: id.clone(), layout: layout.clone() }));
        }
    }
    if before.schema != after.schema {
        operations.push(Procedural3dMutation::ChangeSchema(change_schema::ChangeSchema { new_schema: after.schema.clone() }));
    }
    operations
}
