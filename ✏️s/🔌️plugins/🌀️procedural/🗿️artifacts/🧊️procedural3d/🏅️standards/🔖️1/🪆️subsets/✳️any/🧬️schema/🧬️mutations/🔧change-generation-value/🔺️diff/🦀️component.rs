...
pub fn diff(payload: &ChangeGenerationValue, base: &Procedural3dSnapshot) -> protocol::MutationOutcome<Procedural3dDiff> {
    ...
    if existing.values.get(&payload.question_id) == Some(&payload.new_value) {
        return protocol::MutationOutcome::new(Procedural3dDiff::default()).warn("mutation.no-op", format!("Generation \"{}\" question \"{}\" is already \"{}\".", payload.id, payload.question_id, payload.new_value));
    }
