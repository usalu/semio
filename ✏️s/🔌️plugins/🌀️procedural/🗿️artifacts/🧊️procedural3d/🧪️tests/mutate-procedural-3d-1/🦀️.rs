//! 🦀️ Procedural3d 1 exhaustive mutation case — Rust adapter. Ticket
...
//! document and `Procedural3dMutation` IS its specification, so there is nothing third-party to register. What
...
//#region 🔖️Vocabulary
/// 🏷️ Mirrors `Procedural3dMutation::KINDS`
...
/// 🔀️ Snapshot field → the diff field(s) allowed to declare it. `Procedural3dDiff` mirrors `Procedural3dSnapshot`'s two fields name for name, so the table is empty; the sibling `🧩️assembly` subset, whose diff splits every collection into a `<name>Removed`/`<name>Upserted` pair, carries real rows here.
...
fn vector(ctx: &Context) -> Result<Vector, String> {
    ...
    if !KINDS.contains(&kind.as_str()) {
        return Err(format!("scenario doc string names {kind:?}, which is not a declared Procedural3dMutation kind"));
    }
...
fn round_trip(ctx: &Context) -> Result<Outcome, String> {
    const SNAPSHOT: &str = "asset://🏅️standards/🔖️1/🪆️subsets/✳️any/🧬️schema/🧬️mutations/🗑delete-generation/🧪️tests/removes-the-selected-generation-2-and-falls-back/📸️snapshot/⬅️before/🔣️component.json";
