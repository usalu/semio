...
fn non_empty_canonical_snapshot_round_trips_one_grant_at_a_time() {
    ...
    expected.host_snapshot.widgets.push(semio_framework_artifact_flow_flow::Widget::Neuron { id: "retained-neuron".into(), neuron_kind: "law".into(), params, input_ports: vec!["in".into()], output_ports: vec!["out".into()], preview: true });
    ...
    expected.host_snapshot.layout.insert("retained-neuron".into(), semio_framework_artifact_flow_flow::WidgetLayout { x: 12.5, y: -8.25 });
    expected.host_snapshot.layout.insert("retained-preview".into(), semio_framework_artifact_flow_flow::WidgetLayout { x: 36.0, y: -8.25 });
    ...
    .into();
    assert!(!expected.host_snapshot.widgets.is_empty());
    assert!(!expected.host_snapshot.synapses.is_empty());
    ...
    assert_eq!(actual.host_snapshot.synapses.len(), expected.host_snapshot.synapses.len(), "typed synapse owner must retain the exact row census");
    assert_eq!(actual.host_snapshot.synapses.last(), expected.host_snapshot.synapses.last(), "typed synapse owner must retain the exact non-empty appended row");
    assert_eq!(synapse_digest(actual.host_snapshot.synapses.last().expect("typed retained synapse")), synapse_digest(expected.host_snapshot.synapses.last().expect("expected retained synapse")));
    assert_eq!(actual.host_snapshot.layout, expected.host_snapshot.layout, "typed layout owner must retain the semantically attached widget positions");
