...
fn render_emits_real_tessellated_geometry_for_the_default_fixture() {
    ...
    let eval = evaluate_host_snapshot(&document.host_snapshot);
    let payload = preview_payload(&eval, &document.host_snapshot, &config, None, &Generation3dViewMarks::default());
...
fn marks_paint_hover_and_selection_from_a_bare_widget_id() {
    ...
    let config = Generation3dViewConfig::default();
    let eval = evaluate_host_snapshot(&document.host_snapshot);
    let bare = preview_payload(&eval, &document.host_snapshot, &config, None, &Generation3dViewMarks::default());
    ...
    let marked = preview_payload(&eval, &document.host_snapshot, &config, None, &marks);
...
fn show_mode_changes_the_emitted_mesh_channels_and_edge_flag() {
    ...
    let shaded_payload = preview_payload(&eval, &document.host_snapshot, &shaded, None, &Generation3dViewMarks::default());
    let wireframe_payload = preview_payload(&eval, &document.host_snapshot, &wireframe, None, &Generation3dViewMarks::default());
...
fn render_uses_the_configured_preview_camera() {
    ...
    let node = render(&document, &config, None, None, None, &Generation3dViewMarks::default(), crate::editor::generation3d::terminology::generation3d_labels(&semio_framework_plugin::ViewModel::default())).expect("render");
...
fn a_supplied_evaluation_is_used_instead_of_re_evaluating() {
    ...
    let config = Generation3dViewConfig::default();
    let empty_eval = "{}";
    let payload = preview_payload(empty_eval, &document.host_snapshot, &config, None, &Generation3dViewMarks::default());
...
fn a_hover_only_re_render_tessellates_nothing() {
    ...
    let cold = preview_payload(&eval, &document.host_snapshot, &config, None, &Generation3dViewMarks::default());
    ...
    let warm = preview_payload(&eval, &document.host_snapshot, &config, None, &Generation3dViewMarks::default());
    ...
    let hovered = preview_payload(&eval, &document.host_snapshot, &config, None, &hovered_marks);
