...
fn one_live_wire(fixture: &FlowHostDocument) -> (String, String, String, String, String) {
...
fn wire_exists(fixture: &FlowHostDocument, from: &str, from_port: &str, to: &str, to_port: &str) -> bool {
...
async fn disconnect_then_connect_round_trips_one_wire() {
    ...
    let (before, synapse_id, from, from_port, to, to_port) = {
        ...
        let (id, from, from_port, to, to_port) = one_live_wire(&read.host_document);
        (read.host_document.synapses.len(), id, from, from_port, to, to_port)
    };
    ...
    {
        let read = snapshot(&app);
        assert_eq!(read.host_document.synapses.len(), before - 1, "disconnect must remove exactly the one wire it names");
        assert!(!wire_exists(&read.host_document, &from, &from_port, &to, &to_port), "the cut wire must be gone: {synapse_id}");
    }
    ...
    {
        let read = snapshot(&app);
        assert_eq!(read.host_document.synapses.len(), before, "connect must restore exactly one wire");
        assert!(wire_exists(&read.host_document, &from, &from_port, &to, &to_port), "connect must re-wire {from}:{from_port} -> {to}:{to_port}");
    }
}
...
async fn move_relocates_the_widget_a_node_drag_names() {
    ...
    let node_id = {
        let read = snapshot(&app);
        crate::widget_id(read.host_document.widgets.first().expect("the fixture must have a widget")).to_string()
    };
    ...
    let landed = {
        let read = snapshot(&app);
        let position = read.host_document.layout.get(node_id.as_str()).unwrap_or_else(|| panic!("widget {node_id} must have a pinned layout after a move"));
