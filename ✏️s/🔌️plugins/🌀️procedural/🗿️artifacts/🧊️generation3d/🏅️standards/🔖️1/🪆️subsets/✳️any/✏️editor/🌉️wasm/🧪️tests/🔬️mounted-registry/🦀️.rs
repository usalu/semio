...
const GENERATION3D_ENVELOPE_OUTPUT_CHANNELS: usize = 4;
const GENERATION3D_ENVELOPE_CONTROL_CREDITS: usize = 1;
const GENERATION3D_ENVELOPE_OPERATION_SLOTS: usize = 4;
const GENERATION3D_OUTPUT_PAGE_BYTES: usize = store::ARTIFACT_ENVELOPE_DECODE_PAGE_BYTES;

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
#[repr(u8)]
enum Generation3dOutputKind {
    Progress = 0,
    Checkpoint = 1,
    Preview = 2,
    Terminal = 3,
...
struct Generation3dOutputPage {
    ...
    generation: u64,
    sequence: u64,
    kind: Generation3dOutputKind,
    len: usize,
    bytes: [u8; GENERATION3D_OUTPUT_PAGE_BYTES],
}

impl Generation3dOutputPage {
    fn new(operation: u64, generation: u64, sequence: u64, kind: Generation3dOutputKind, status: u8, admitted_pages: usize, admitted_bytes: usize) -> Self {
        let mut bytes = [0; GENERATION3D_OUTPUT_PAGE_BYTES];
...
struct Generation3dOutputSlot {
    page: Option<Generation3dOutputPage>,
    lease: Option<std::sync::Arc<std::sync::atomic::AtomicU8>>,
    acknowledged: bool,
}

impl Generation3dOutputSlot {
    fn empty() -> Self {
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct Generation3dIngressCredits {
    maximum_pages: usize,
    maximum_bytes: usize,
    maximum_items: usize,
...
impl Generation3dIngressCredits {
    fn try_new(maximum_pages: usize, maximum_bytes: usize, maximum_items: usize, maximum_output_pages: usize, maximum_controls: usize) -> Result<Self, &'static str> {
        if maximum_pages == 0 || maximum_pages > GENERATION3D_ENVELOPE_MAXIMUM_PAGES {
            return Err("generation3d-envelope.page-credits");
        }
        if maximum_bytes == 0 || maximum_bytes > GENERATION3D_ENVELOPE_MAXIMUM_BYTES {
            return Err("generation3d-envelope.byte-credits");
        }
        if maximum_items == 0 || maximum_items > GENERATION3D_ENVELOPE_MAXIMUM_ITEMS {
            return Err("generation3d-envelope.item-credits");
        }
        if maximum_output_pages != GENERATION3D_ENVELOPE_OUTPUT_CHANNELS {
            return Err("generation3d-envelope.output-credits");
        }
        if maximum_controls != GENERATION3D_ENVELOPE_CONTROL_CREDITS {
            return Err("generation3d-envelope.control-credits");
        }
...
struct Generation3dMountedOperation {
    ...
    sequence: u64,
    outputs: [Generation3dOutputSlot; GENERATION3D_ENVELOPE_OUTPUT_CHANNELS],
}

impl Generation3dMountedOperation {
    ...
    fn preflight_page(&self, bytes: usize) -> Result<(), &'static str> {
        let pages = self.admitted_pages.checked_add(1).ok_or("generation3d-envelope.page-overflow")?;
        let total_bytes = self.admitted_bytes.checked_add(bytes).ok_or("generation3d-envelope.byte-overflow")?;
        if bytes > GENERATION3D_OUTPUT_PAGE_BYTES || pages > self.credits.maximum_pages || total_bytes > self.credits.maximum_bytes {
            return Err("generation3d-envelope.page-handback");
        }
        Ok(())
    }

    fn publish(&mut self, kind: Generation3dOutputKind, status: u8) {
        let slot = &mut self.outputs[kind as usize];
        slot.reclaim_lost_lease();
        if kind == Generation3dOutputKind::Terminal && (slot.page.is_some() || slot.acknowledged) {
            return;
        }
        self.sequence = self.sequence.saturating_add(1);
        slot.page = Some(Generation3dOutputPage::new(self.operation, self.generation, self.sequence, kind, status, self.admitted_pages, self.admitted_bytes));
        slot.acknowledged = false;
    }
}

struct Generation3dOutputLease {
    page: Generation3dOutputPage,
    signal: std::sync::Arc<std::sync::atomic::AtomicU8>,
    consumed: bool,
}

impl Drop for Generation3dOutputLease {
    fn drop(&mut self) {
        if !self.consumed {
...
struct Generation3dMountedRegistry {
    operations: [Option<Generation3dMountedOperation>; GENERATION3D_ENVELOPE_OPERATION_SLOTS],
}

impl Generation3dMountedRegistry {
    fn new() -> Self {
    ...
    fn operation_mut(&mut self, operation: u64, generation: u64) -> Result<&mut Generation3dMountedOperation, &'static str> {
        self.operations.iter_mut().flatten().find(|entry| entry.matches(operation, generation)).ok_or("generation3d-envelope.stale-registry-handle")
    }

    fn operation(&self, operation: u64, generation: u64) -> Result<&Generation3dMountedOperation, &'static str> {
        self.operations.iter().flatten().find(|entry| entry.matches(operation, generation)).ok_or("generation3d-envelope.stale-registry-handle")
    }
    ...
    fn publish(&mut self, operation: u64, generation: u64, kind: Generation3dOutputKind, status: u8) -> Result<(), &'static str> {
        self.operation_mut(operation, generation)?.publish(kind, status);
        Ok(())
    }

    fn take(&mut self, operation: u64, generation: u64, kind: Generation3dOutputKind) -> Result<Option<Generation3dOutputLease>, &'static str> {
        ...
        Ok(Some(Generation3dOutputLease { page: *page, signal, consumed: false }))
    }

    fn resume(&mut self, lease: &mut Generation3dOutputLease) -> Result<(), &'static str> {
        ...
        if !slot.lease.as_ref().is_some_and(|signal| std::sync::Arc::ptr_eq(signal, &lease.signal)) {
            return Err("generation3d-envelope.output-resume-stale");
        }
    ...
    fn acknowledge_output(&mut self, lease: &mut Generation3dOutputLease) -> Result<(), &'static str> {
        let slot = &mut self.operation_mut(lease.page.operation, lease.page.generation)?.outputs[lease.page.kind as usize];
        ...
        if !same_page || !slot.lease.as_ref().is_some_and(|signal| std::sync::Arc::ptr_eq(signal, &lease.signal)) {
            return Err("generation3d-envelope.output-ack-stale");
        }
    ...
    fn terminal_acknowledged(&mut self, operation: u64, generation: u64) -> Result<bool, &'static str> {
        let slot = &mut self.operation_mut(operation, generation)?.outputs[Generation3dOutputKind::Terminal as usize];
        slot.reclaim_lost_lease();
    ...
    fn prepare_load_acknowledgement(&mut self, operation: u64, generation: u64) -> Result<bool, &'static str> {
        ...
        let entry = self.operation_mut(operation, generation)?;
        for output in &mut entry.outputs[..Generation3dOutputKind::Terminal as usize] {
    ...
    fn remove(&mut self, operation: u64, generation: u64) -> Result<(), &'static str> {
        let slot = self.operations.iter_mut().find(|slot| slot.as_ref().is_some_and(|entry| entry.matches(operation, generation))).ok_or("generation3d-envelope.remove-stale")?;
        if slot.as_ref().is_some_and(|entry| entry.outputs.iter().any(|output| output.page.is_some() || output.lease.is_some())) {
            return Err("generation3d-envelope.remove-populated");
        }
        *slot = None;
        Ok(())
    }
    ...
    fn close_step(&mut self) -> bool {
        ...
        let _ = crate::standards::v1::subsets::any::schema::mutations::binary::generation3d_release_publication_authority(semio_framework_job::OperationId(operation_id), semio_framework_job::Generation(generation));
...
mod mounted_laws {
    ...
    fn exact_credits(maximum_pages: usize, maximum_bytes: usize) -> Generation3dIngressCredits {
        Generation3dIngressCredits::try_new(maximum_pages, maximum_bytes, GENERATION3D_ENVELOPE_MAXIMUM_ITEMS, GENERATION3D_ENVELOPE_OUTPUT_CHANNELS, GENERATION3D_ENVELOPE_CONTROL_CREDITS).expect("exact Generation3d mounted credits")
    }
    ...
    fn every_credit_rejects_zero_and_maximum_plus_one_before_operation_construction() {
        let valid = (GENERATION3D_ENVELOPE_MAXIMUM_PAGES, GENERATION3D_ENVELOPE_MAXIMUM_BYTES, GENERATION3D_ENVELOPE_MAXIMUM_ITEMS, GENERATION3D_ENVELOPE_OUTPUT_CHANNELS, GENERATION3D_ENVELOPE_CONTROL_CREDITS);
        assert!(Generation3dIngressCredits::try_new(0, valid.1, valid.2, valid.3, valid.4).is_err());
        assert!(Generation3dIngressCredits::try_new(valid.0 + 1, valid.1, valid.2, valid.3, valid.4).is_err());
        assert!(Generation3dIngressCredits::try_new(valid.0, 0, valid.2, valid.3, valid.4).is_err());
        assert!(Generation3dIngressCredits::try_new(valid.0, valid.1 + 1, valid.2, valid.3, valid.4).is_err());
        assert!(Generation3dIngressCredits::try_new(valid.0, valid.1, 0, valid.3, valid.4).is_err());
        assert!(Generation3dIngressCredits::try_new(valid.0, valid.1, valid.2 + 1, valid.3, valid.4).is_err());
        assert!(Generation3dIngressCredits::try_new(valid.0, valid.1, valid.2, 0, valid.4).is_err());
        assert!(Generation3dIngressCredits::try_new(valid.0, valid.1, valid.2, valid.3 + 1, valid.4).is_err());
        assert!(Generation3dIngressCredits::try_new(valid.0, valid.1, valid.2, valid.3, 0).is_err());
        assert!(Generation3dIngressCredits::try_new(valid.0, valid.1, valid.2, valid.3, valid.4 + 1).is_err());
        assert!(Generation3dIngressCredits::try_new(valid.0, valid.1, valid.2, valid.3, valid.4).is_ok());
    }
    ...
    fn repeated_rejected_controls_do_not_consume_operation_credits() {
        ...
        let mut registry = Generation3dMountedRegistry::new();
        let credits = exact_credits(1, GENERATION3D_OUTPUT_PAGE_BYTES);
    ...
    fn operation_and_page_maximum_plus_one_leave_producers_unconstructed() {
        ...
        registry.admit_page(1, 7, GENERATION3D_OUTPUT_PAGE_BYTES).expect("exact page handoff");
        assert_eq!(registry.operation(1, 7).expect("first operation").preflight_page(1), Err("generation3d-envelope.page-handback"));
    ...
    fn outputs_are_bounded_latest_wins_with_lossless_terminal_take_resume_and_ack() {
        let mut registry = Generation3dMountedRegistry::new();
        insert(&mut registry, 21, 8, exact_credits(1, GENERATION3D_OUTPUT_PAGE_BYTES));

        registry.publish(21, 8, Generation3dOutputKind::Progress, 0).expect("initial progress");
        let first = registry.operation(21, 8).expect("operation").outputs[Generation3dOutputKind::Progress as usize].page.as_ref().expect("progress").sequence;
        registry.publish(21, 8, Generation3dOutputKind::Progress, 1).expect("latest progress");
        let latest = registry.operation(21, 8).expect("operation").outputs[Generation3dOutputKind::Progress as usize].page.as_ref().expect("progress").sequence;
        assert!(latest > first);

        let mut progress = registry.take(21, 8, Generation3dOutputKind::Progress).expect("take").expect("progress lease");
        let leased_progress = progress.page.sequence;
        registry.publish(21, 8, Generation3dOutputKind::Progress, 2).expect("latest progress while leased");
        registry.resume(&mut progress).expect("resume");
        drop(progress);
        let mut progress = registry.take(21, 8, Generation3dOutputKind::Progress).expect("retake").expect("progress lease");
        assert!(progress.page.sequence > leased_progress);
        registry.acknowledge_output(&mut progress).expect("progress ACK");
        drop(progress);

        registry.publish(21, 8, Generation3dOutputKind::Checkpoint, 1).expect("checkpoint");
        let mut checkpoint = registry.take(21, 8, Generation3dOutputKind::Checkpoint).expect("take").expect("checkpoint lease");
        let checkpoint_sequence = checkpoint.page.sequence;
        registry.resume(&mut checkpoint).expect("checkpoint resume");
        drop(checkpoint);
        let mut checkpoint = registry.take(21, 8, Generation3dOutputKind::Checkpoint).expect("retake").expect("checkpoint lease");
        assert_eq!(checkpoint.page.sequence, checkpoint_sequence);
        registry.acknowledge_output(&mut checkpoint).expect("checkpoint ACK");
        drop(checkpoint);

        registry.publish(21, 8, Generation3dOutputKind::Terminal, 2).expect("terminal");
        let terminal_sequence = registry.operation(21, 8).expect("operation").outputs[Generation3dOutputKind::Terminal as usize].page.as_ref().expect("terminal").sequence;
        registry.publish(21, 8, Generation3dOutputKind::Terminal, 4).expect("lossless terminal retry");
        assert_eq!(registry.operation(21, 8).expect("operation").outputs[Generation3dOutputKind::Terminal as usize].page.as_ref().expect("terminal").sequence, terminal_sequence,);
        drop(registry.take(21, 8, Generation3dOutputKind::Terminal).expect("take").expect("lost terminal lease"));
        let mut terminal = registry.take(21, 8, Generation3dOutputKind::Terminal).expect("retake").expect("terminal retained after handle loss");
        ...
        drop(terminal);
        registry.publish(21, 8, Generation3dOutputKind::Terminal, 4).expect("terminal remains acknowledged");
        assert!(registry.operation(21, 8).expect("operation").outputs[Generation3dOutputKind::Terminal as usize].page.is_none());
    ...
    fn complete_before_ack_and_interrupted_close_retain_owners() {
        ...
        registry.publish(31, 9, Generation3dOutputKind::Terminal, 2).expect("terminal");
        assert_eq!(registry.remove(31, 9), Err("generation3d-envelope.remove-populated"));
        assert!(!registry.prepare_load_acknowledgement(31, 9).expect("complete remains unacknowledged"));
        let mut terminal = registry.take(31, 9, Generation3dOutputKind::Terminal).expect("take").expect("terminal");
        registry.acknowledge_output(&mut terminal).expect("terminal ACK");
        drop(terminal);
        ...
        for kind in [Generation3dOutputKind::Progress, Generation3dOutputKind::Checkpoint, Generation3dOutputKind::Preview, Generation3dOutputKind::Terminal] {
        ...
        for _ in 0..GENERATION3D_ENVELOPE_OUTPUT_CHANNELS + 2 {
    ...
    fn populated_ordinary_drop_panics_until_hostile_close_drains() {
        let dropped = std::panic::catch_unwind(|| {
            let mut registry = Generation3dMountedRegistry::new();
            insert(&mut registry, 33, 10, exact_credits(1, GENERATION3D_OUTPUT_PAGE_BYTES));
            registry.publish(33, 10, Generation3dOutputKind::Terminal, 4).expect("fault terminal");
        });
        assert!(dropped.is_err(), "populated ordinary drop must remain fail-loud");

        let mut registry = Generation3dMountedRegistry::new();
        insert(&mut registry, 34, 10, exact_credits(1, GENERATION3D_OUTPUT_PAGE_BYTES));
        registry.publish(34, 10, Generation3dOutputKind::Terminal, 3).expect("cancel terminal");
        for _ in 0..GENERATION3D_ENVELOPE_OUTPUT_CHANNELS + 2 {
            if registry.close_step() {
                break;
            }
    ...
    fn authoritative_publication_rejects_stale_generation_aba_and_parent() {
        ...
        assert!(crate::standards::v1::subsets::any::schema::mutations::binary::generation3d_release_publication_authority(operation, Generation(42)));
    }
    ...
    fn domain_local_static_verifier_rejects_raw_routes_and_proves_three_dimensional_coverage() {
        ...
        let lifecycle_fixture = include_str!("../../../../🧫️fixtures/🧷️p8yz-b-retained-mounted-laws.json");
        ...
        let mounted_field = owner_source.split_once("struct Generation3dPackSnapshotAuthority").expect("P3 mounted envelope snapshot authority").1.split_once("enum Generation3dMutationDecodeState").expect("P3 mounted snapshot authority boundary").0;
        let mounted_mutation =
            owner_source.split_once("struct Generation3dMutationDecodeAuthority").expect("P3 mounted mutation authority").1.split_once("struct Generation3dRejectedConflictAuthority").expect("P3 mounted mutation authority boundary").0;
        ...
        assert!(crate::standards::v1::subsets::any::schema::mutations::binary::generation3d_retained_catalog_is_complete());
    }
}
