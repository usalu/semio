...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum Generation3dMountedPackCloseStep {
...
impl Generation3dMountedPackSession {
    ...
    fn allocate_after_discriminator(&mut self) -> Result<(), &'static str> {
        ...
        *self.value = Some(
            mounted::RetainedValueCursor::try_new(limits(), store::ARTIFACT_ENVELOPE_DECODE_MAXIMUM_CLOSE_ALLOCATION_BYTES)
                .map_err(|_| "generation3d-mounted.value-preflight")?,
        );
    ...
    fn flush_page(&mut self) -> Result<(), &'static str> {
        ...
        if let Err(page) = source.admit_page(page) {
            ...
            return Err("generation3d-mounted.producer-handback");
        }
        Ok(())
    }
    ...
    pub fn reserve_retained_allocation(&mut self, maximum_bytes: usize) -> Result<mounted::RetainedPackSourceAllocationStep, mounted::RetainedPackSourceAllocationError> {
        ...
        if self.phase == Generation3dMountedPackPhase::Drive {
            ...
            let value = self
                .value
                .as_mut()
                .ok_or(mounted::RetainedPackSourceAllocationError { allocated_bytes: 0, reason: "generation3d-mounted.value-owner" })?;
            if value
                .next_allocation_bytes()
                .map_err(|_| mounted::RetainedPackSourceAllocationError { allocated_bytes: 0, reason: "generation3d-mounted.value-allocation" })?
                .is_some()
            {
                return value
                    ...
                    .map_err(|error| mounted::RetainedPackSourceAllocationError { allocated_bytes: error.allocated_bytes, reason: "generation3d-mounted.value-allocation" });
            }
            return self
                .catalog
                .as_mut()
    ...
    pub fn close_step(&mut self, maximum_items: usize, maximum_bytes: usize) -> Result<Generation3dMountedPackCloseStep, &'static str> {
        if maximum_items == 0 {
            return Ok(Generation3dMountedPackCloseStep::Pending { released_items: 0, released_bytes: 0 });
        }
        self.phase = Generation3dMountedPackPhase::Closing;
    ...
    pub fn terminal_is_empty(&self) -> bool {
        self.phase == Generation3dMountedPackPhase::Closed && self.page_len == 0 && self.source.is_none() && self.anchor.is_none() && self.segment.is_none() && self.catalog.is_none() && self.value.is_none() && self.typed.is_none() && self.catalog_value.is_none() && self.document_byte.is_none()
    }
}
