...
pub fn inverse(payload: &DisconnectSynapse, base: &Generation3dSnapshot) -> Vec<Generation3dMutation> {
    match synapse_index(&base.host_document, &payload.id) {
        Some(index) => vec![Generation3dMutation::ConnectSynapse(ConnectSynapse { index, synapse: base.host_document.synapses[index].clone() })],
        None => Vec::new(),
    }
}
