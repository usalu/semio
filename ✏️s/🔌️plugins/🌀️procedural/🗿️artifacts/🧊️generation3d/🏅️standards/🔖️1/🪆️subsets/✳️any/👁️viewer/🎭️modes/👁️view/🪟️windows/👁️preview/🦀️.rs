//! 👁️ Generation3d viewer — the Preview window: a read-only world-3d mesh scene built with the
//! shared `MeshWindowKit` (ticket 26/08/16/ARTIFACT-VIEWERS-AND-EDITORS-PER-SUBSET contract §2.6),
//! evaluated fresh from the same artifact-level `Generation3dSnapshot` the editor's own Preview
...
const GENERATION3D_VIEW_DEFAULT_CAMERA_POSITION: [f64; 3] = [4.0, -4.0, 3.0];
const GENERATION3D_VIEW_DEFAULT_CAMERA_TARGET: [f64; 3] = [0.0, 0.0, 0.0];
const GENERATION3D_VIEW_DEFAULT_CAMERA_FOV: f64 = 45.0;
/// 🧊️ Matches the other surface's own default LOD tessellation deflection (unset `lod_mode` ⇒ 0.05).
const GENERATION3D_VIEW_TOLERANCE: f64 = 0.05;
...
//#region 🔖️Render
/// 👁️ Pure `Generation3dSnapshot -> BuiltNode` read: default camera (a viewer has no persisted
/// per-session camera — `Config = NoConfig`), no selection/gumball/engagement overlay, real evaluated
/// preview geometry — not a fallback placeholder: generation3d's whole purpose is generated geometry,
/// and the pure evaluate+tessellate path needs no session/config to run once.
pub fn render(document: &Generation3dSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    let (meshes_json, instances_json) = evaluated_meshes_and_instances(&document.fixture);
    let view = MeshView {
        camera_json: world3d_camera_json(GENERATION3D_VIEW_DEFAULT_CAMERA_POSITION, GENERATION3D_VIEW_DEFAULT_CAMERA_TARGET, GENERATION3D_VIEW_DEFAULT_CAMERA_FOV),
        meshes_json,
        instances_json,
