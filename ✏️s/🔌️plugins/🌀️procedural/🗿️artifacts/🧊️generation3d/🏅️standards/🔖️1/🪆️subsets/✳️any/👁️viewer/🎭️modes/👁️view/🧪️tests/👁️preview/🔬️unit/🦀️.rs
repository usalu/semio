...
fn render_uses_the_configured_preview_camera() {
    ...
    let scene: semio_framework_ui::wgpu::World3dScene = semio_framework_plugin::testkit::built_surface_scene(&node).expect("viewer preview body must decode as an assembled world-3d scene");
...
fn a_hover_only_re_render_tessellates_nothing() {
    ...
    let cold = preview_payload(&eval, &document.fixture, &config, &Generation3dViewMarks::default());
    ...
    let warm = preview_payload(&eval, &document.fixture, &config, &Generation3dViewMarks::default());
    ...
    let hovered = preview_payload(&eval, &document.fixture, &config, &hovered_marks);
