...
async fn first_slider_question(app: &mut Generation3dApp) -> (String, String, f64) {
    ...
    let spec = flow_host_snapshot_to_form_spec(&read.host_snapshot);
    let question = spec
        .steps
        .iter()
...
/// `updateGenerationValues` writes the roster entry, and `generation_host_snapshot_for` — the one input the
...
async fn editing_a_form_value_repatches_the_generate_preview_fixture() {
    ...
    let patched = crate::standards::v1::subsets::any::schema::generation_host_snapshot_for(&read.host_snapshot, &read.generation, Some(generation_id.as_str()));
