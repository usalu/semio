...
async fn set_sun_intensity_dispatches_live_and_leaves_the_document_untouched() {
    let _serial = crate::viewer::generation3d::testkit::lock();
    let mut app = app().await;
    let before = app.snapshot().expect("snapshot");
    let command = Generation3dViewCommand::SetSunIntensity(SetSunIntensity { value: 2.0 });
    assert_eq!(command.command_id(), "setSunIntensity");
