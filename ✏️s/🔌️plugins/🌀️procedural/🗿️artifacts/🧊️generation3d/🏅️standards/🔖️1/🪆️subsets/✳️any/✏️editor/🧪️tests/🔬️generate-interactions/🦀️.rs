...
async fn every_node_graph_sub_operation_the_table_names_changes_the_fixture() {
    ...
    let node_id = {
        ...
        crate::widget_id(read.host_snapshot.widgets.first().expect("a widget")).to_string()
    };
    ...
    let landed = {
        let read = crate::editor::generation3d::unit_tests::context::snapshot(&app);
        let position = read.host_snapshot.layout.get(node_id.as_str()).expect("a moved widget keeps a pinned layout");
