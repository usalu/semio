//! 🧭 `topology` — one named inference: the DAG shape of a generation3d snapshot's `fixture`
...
pub fn compute_generation3d_topology(snapshot: &Generation3dSnapshot) -> Generation3dTopology {
    ...
    Generation3dTopology { node_count: widget_ids.len() as u32, edge_count: snapshot.fixture.synapses.len() as u32, topo_order: if cycle_free { order } else { Vec::new() }, depth: if cycle_free { depth } else { 0 }, cycle_free }
}
//#endregion 🔖️Topology
