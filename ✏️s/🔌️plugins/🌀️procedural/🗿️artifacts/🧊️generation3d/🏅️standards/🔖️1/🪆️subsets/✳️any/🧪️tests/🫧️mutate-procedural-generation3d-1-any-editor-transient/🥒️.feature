@capability-procedural-generation3d-1-any-editor-transient-mutate
@no-oracle-procedural-generation3d-1-any-editor-transient-state-lane-semantics
@comparison-ordered-json-v1
@mutations-procedural-generation3d-1-any-editor-transient
Feature: Apply every transient state-lane mutation of s.procedural.generation3d's ✏️editor/🫧️transient to its committed vector
  `Generation3dTransientMutation` is the transient lane of this subset's editor: it holds the text of the last generation preview, or none before the first one. It is this
  repository's own state record, so the case records the no-oracle decision `procedural-generation3d-1-any-editor-transient-state-lane-semantics` in
  ...
  before-snapshot. Production dispatch is reached through `generation3d_transient_mutation_report_json`, which runs
  ...
  Scenario Outline: Apply <id> and land on the committed after-snapshot, diff and outcome
    ...
    When <id> is applied to that vector's before-snapshot through generation3d_transient_mutation_report_json
