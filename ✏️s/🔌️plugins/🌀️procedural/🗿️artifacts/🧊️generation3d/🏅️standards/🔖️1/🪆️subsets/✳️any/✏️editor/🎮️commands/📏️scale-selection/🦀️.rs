...
use crate::standards::v1::subsets::any::schema::{commit_host_snapshot, ensure_gumball_node, gumball_scale_params_json, gumball_widget_number_param, with_host};
...
fn scale_ids(host_snapshot: &FlowHostSnapshot, ids: &[String], uniform_factor: f64) -> Emit<Generation3dMutation, Generation3dConfigMutation> {
    match gumball_transform(host_snapshot, ids, "scale", move |host, transform_id| {
...
pub fn handle(payload: &ScaleSelection, doc: &ArtifactView<'_, Generation3dSnapshot>, _cfg: &ConfigView<'_, Generation3dConfig>, _session: &mut FlowEvalSession) -> Result<Emit<Generation3dMutation, Generation3dConfigMutation>, Fault> {
    ...
    Ok(scale_ids(&doc.snapshot.host_snapshot, &ids, (payload.sx + payload.sy + payload.sz) / 3.0))
}
...
pub(crate) fn apply_selected(payload: &ScaleSelection, doc: &ArtifactView<'_, Generation3dSnapshot>, selected: &[String]) -> Emit<Generation3dMutation, Generation3dConfigMutation> {
    let ids = mesh_selection_ids_typed(&payload.node_ids, selected);
    scale_ids(&doc.snapshot.host_snapshot, &ids, (payload.sx + payload.sy + payload.sz) / 3.0)
}
