...
use crate::standards::v1::subsets::any::schema::{commit_host_document, ensure_gumball_node, gumball_translate_params_json, gumball_widget_offset, with_host};
...
fn translate_ids(fixture: &FlowHostDocument, ids: &[String], dx: f64, dy: f64, dz: f64) -> Emit<Generation3dMutation, Generation3dConfigMutation> {
...
pub fn handle(payload: &TranslateSelection, doc: &ArtifactView<'_, Generation3dSnapshot>, _cfg: &ConfigView<'_, Generation3dConfig>, _session: &mut FlowEvalSession) -> Result<Emit<Generation3dMutation, Generation3dConfigMutation>, Fault> {
    ...
    Ok(translate_ids(&doc.snapshot.host_document, &ids, payload.dx, payload.dy, payload.dz))
}
...
pub(crate) fn apply_selected(payload: &TranslateSelection, doc: &ArtifactView<'_, Generation3dSnapshot>, selected: &[String]) -> Emit<Generation3dMutation, Generation3dConfigMutation> {
    ...
    translate_ids(&doc.snapshot.host_document, &ids, payload.dx, payload.dy, payload.dz)
}
