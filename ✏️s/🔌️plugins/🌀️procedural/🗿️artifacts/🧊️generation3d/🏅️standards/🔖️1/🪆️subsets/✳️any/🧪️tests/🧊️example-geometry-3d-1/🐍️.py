...
a fixture error and a disagreement located in the Rust lane is a kernel error. The two boolean
examples' fixtures carry `kernelStatus: "blocked-on-boolean-kernel"`; this oracle still states what
...
def ball_cube_union(ball_radius: float, cube_side: float) -> float:
    """🧲️ |ball ∪ cube| for a ball and an axis-aligned cube sharing the origin, by quadrature."""
    half = cube_side / 2.0

    def clipped_disc_area(z: float) -> float:
        ...
        if radius >= half * math.sqrt(2.0):
            return 4.0 * half * half
        chord = math.sqrt(radius * radius - half * half)
        return 4.0 * (half * chord + radius * radius * (math.pi / 4.0 - math.atan(chord / half)))

    limit = min(ball_radius, half)
    overlap, _ = integrate.quad(clipped_disc_area, -limit, limit, limit=400, points=[-limit, 0.0, limit])
...
def expected(example: str, knob: dict[str, float], defaults: dict[tuple[str, str], dict]) -> dict:
    ...
    if example == "sphere-cut-with-torus":
        ...
        ball = knob["slider_2"]
        return {"volume": ball_minus_torus(ball, major, minor), "perimeter": None, "closed": True, "extent": (2.0 * ball,) * 3}
