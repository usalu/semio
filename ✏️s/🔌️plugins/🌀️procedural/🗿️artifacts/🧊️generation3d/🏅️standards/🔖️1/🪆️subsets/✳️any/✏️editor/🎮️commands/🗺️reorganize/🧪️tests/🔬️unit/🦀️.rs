...
async fn overlap_every_widget(app: &mut crate::editor::generation3d::unit_tests::context::Generation3dApp) -> Vec<String> {
    let widget_ids: Vec<String> = context::snapshot(&app).host_snapshot.widgets.iter().map(|widget| crate::widget_id(widget).to_string()).collect();
...
async fn reorganize_moves_at_least_one_widget_off_a_deliberately_overlapped_layout() {
    ...
    assert_eq!(after.host_snapshot.widgets.len(), widget_ids.len(), "reorganize is a layout operation and must not add or remove widgets");
    let moved: Vec<&String> = widget_ids.iter().filter(|id| after.host_snapshot.layout.get(id.as_str()).is_some_and(|position| (position.x, position.y) != OVERLAPPED)).collect();
...
async fn reorganize_leaves_no_two_widgets_sharing_one_position() {
    ...
    for id in &widget_ids {
        let Some(position) = after.host_snapshot.layout.get(id.as_str()) else { continue };
