...
async fn main_graph_scene_exports_flow_backed_node_graph_fields() {
    ...
    // 🛍️ The scene names its operators by KIND ID (inside `fixtureJson`) and carries no operator
    // records of its own: the registered catalogue is app-static and ~100 KB with the real `brep`/`math`
    // sets installed, three times the fixed 32 KiB per-surface admission this very render is checked
    // against (ticket 26/09/09/PROCEDURAL-3D-END-TO-END §3.1).
