...
async fn set_lod_mode_dispatches_live_and_leaves_the_document_untouched() {
    let _serial = crate::viewer::generation3d::testkit::lock();
    let mut app = app().await;
    let before = testkit::snapshot(&app);
    let command = Generation3dViewCommand::SetLodMode(SetLodMode { value: "coarse".into() });
    assert_eq!(command.command_id(), "setLodMode");
