...
async fn set_active_example_via_string_action_loads_fixture() {
    ...
    assert!(projection.host_snapshot.widgets.iter().any(|widget| crate::widget_id(widget).contains("fillet") || matches!(widget, Widget::Neuron { neuron_kind, .. } if neuron_kind.contains("fillet") || neuron_kind.contains("box"))));
}
