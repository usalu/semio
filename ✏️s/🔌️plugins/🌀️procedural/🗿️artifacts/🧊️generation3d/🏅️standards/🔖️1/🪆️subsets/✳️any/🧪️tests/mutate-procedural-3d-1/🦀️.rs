...
/// 🔀️ Snapshot field → the diff field(s) allowed to declare it. `Generation3dDiff` mirrors `Generation3dSnapshot`'s two fields name for name, so the table is empty; the sibling `🧩️assembly` subset, whose diff splits every collection into a `<name>Removed`/`<name>Upserted` pair, carries real rows here.
...
fn round_trip(ctx: &Context) -> Result<Outcome, String> {
    const SNAPSHOT: &str = "asset://🧬️schema/🧬️mutations/🗑delete-generation/🧪️tests/removes-the-selected-generation-2-and-falls-back/📸️snapshot/⬅️before/🔣️.json";
