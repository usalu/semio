...
fn inference_of_the_example_is_a_non_trivial_acyclic_topology() {
    ...
    assert_eq!(inferred.topology.node_count as usize, snapshot.host_document.widgets.len());
    assert_eq!(inferred.topology.edge_count as usize, snapshot.host_document.synapses.len());
    assert!(inferred.topology.cycle_free, "a bundled example must be an acyclic graph");
    assert_eq!(inferred.topology.topo_order.len(), snapshot.host_document.widgets.len());
    snapshot.retire_cold();
}
