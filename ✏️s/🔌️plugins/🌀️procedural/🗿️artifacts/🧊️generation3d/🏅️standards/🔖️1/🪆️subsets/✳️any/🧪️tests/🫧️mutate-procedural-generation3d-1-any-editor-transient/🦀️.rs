//! 🫧️ `s.procedural.generation3d` ✏️editor/🫧️transient state-lane mutation case — Rust adapter.
//!
//! Recorded no-oracle decision `procedural-generation3d-1-any-editor-transient-state-lane-semantics`: the runner dispatches no oracle role, so every law is asserted inside
//! the subject handlers through `semio_s_plugin_stdio_test_oracle::law::vector` over the report of this crate's
//! production bridge `generation3d_transient_mutation_report_json`. The oracle handlers answer with the committed after- and before-snapshots read
...
mod subject {
    ...
    use semio_s_artifact_procedural_generation3d::editor::generation3d::transient::generation3d_transient_mutation_report_json;

    fn report(committed: &Vector) -> Result<String, String> {
        generation3d_transient_mutation_report_json(committed.before, committed.mutation, committed.after)
    }
