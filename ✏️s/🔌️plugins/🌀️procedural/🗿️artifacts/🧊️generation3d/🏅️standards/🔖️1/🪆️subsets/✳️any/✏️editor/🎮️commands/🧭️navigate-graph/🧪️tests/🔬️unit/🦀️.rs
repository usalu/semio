...
fn the_keyboard_order_is_the_reading_order_the_fixture_declares() {
    let fixture = fixture();
    let document = hex_column();
    assert_eq!(document.fixture.keyboard_order(), strings(&fixture["keyboardOrder"]).iter().map(String::as_str).collect::<Vec<_>>(), "reading order");
}
...
fn every_anchor_rule_the_fixture_states_holds() {
    ...
    for row in fixture["anchors"].as_array().expect("anchors") {
        ...
        assert_eq!(anchor_node(&document.fixture, &selection), expected, "{:?}: {}", selection, row["why"].as_str().unwrap_or_default());
    }
}
...
fn every_arrow_walk_lands_where_the_fixture_says() {
    ...
    for walk in fixture["walks"].as_array().expect("walks") {
        ...
        for step in walk["steps"].as_array().expect("steps") {
            ...
            if let Some(target) = step_target(&document.fixture, &selection, step_of(named)) {
...
fn a_keyboard_step_authors_no_document_and_no_config_operation() {
    ...
    for step in [FlowGraphStep::Next, FlowGraphStep::Previous, FlowGraphStep::Upstream, FlowGraphStep::Downstream] {
        let target = step_target(&document.fixture, &["extrude".to_string()], step);
        assert!(target.is_some(), "the hex column answers every direction from `extrude`");
    }
    assert_eq!(activate_ports(&document.fixture, &ports, &["extrude".to_string()]).map(Vec::as_slice), Some(["extrude@wire".to_string(), "extrude@vector".to_string()].as_slice()), "activate opens the anchor's ports");
    assert_eq!(activate_ports(&document.fixture, &ports, &["extrude@wire".to_string(), "extrude@vector".to_string()]), None, "an already-open node opens no further");
    assert_eq!(activate_ports(&document.fixture, &ports, &["height".to_string()]), None, "a node with no published port opens nothing");
}
...
async fn an_arrow_verb_moves_the_framework_owned_graph_selection() {
    ...
    assert!(context::snapshot(&app).fixture.widgets.iter().any(|widget| crate::widget_id(widget) == "height"), "the fixture app opens the hex column");
