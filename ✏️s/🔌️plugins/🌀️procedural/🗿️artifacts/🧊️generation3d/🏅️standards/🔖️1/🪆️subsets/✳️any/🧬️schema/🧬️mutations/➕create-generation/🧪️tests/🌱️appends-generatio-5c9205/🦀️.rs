//! 🧪️ `create-generation` fixture — `🌱️appends-generation-2-and-moves-the-selection`.
//!
//! `create-generation`'s diff builder rejects a duplicate id, then folds a `GenerationMutation::Add` into the play state — which appends the generation AND re-points `selectedGenerationId` at it, both of which the committed diff must carry.
//!
...
fn applies_to_committed_after() {
    ...
    apply_generation3d_mutation(&mut snapshot, &mutation()).expect("create-generation applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "create-generation/appends-generation-2-and-moves-the-selection: applied state differs from committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "create-generation/appends-generation-2-and-moves-the-selection: inverse did not restore the before-snapshot");
}
