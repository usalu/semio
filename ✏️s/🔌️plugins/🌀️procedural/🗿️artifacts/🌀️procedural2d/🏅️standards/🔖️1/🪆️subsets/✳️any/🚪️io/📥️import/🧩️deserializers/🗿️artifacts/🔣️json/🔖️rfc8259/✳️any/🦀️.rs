...
pub fn deserialize(from: &JsonSnapshot) -> Result<Procedural2dSnapshot, store::TextError> {
    ...
    let snap = <Procedural2dSnapshot as protocol::FromValue>::from_value(protocol::DslValue::from(from.to_serde_value())).map_err(|e| store::TextError::new(format!("procedural2d<-json: {e}"), dsl::TextSpan::at(1, 1)))?;
