//! 🦀️ Procedural2d 1 exhaustive mutation case — Rust adapter. Ticket
...
//! document and `Procedural2dMutation` IS its specification, so there is nothing third-party to register. What
...
//#region 🔖️Vocabulary
/// 🏷️ Mirrors `Procedural2dMutation::KINDS`
...
/// 🔀️ Snapshot field → the diff field(s) allowed to declare it. `Procedural2dDiff` mirrors `Procedural2dSnapshot`'s two fields name for name, so the table is empty; the sibling `🧩️assembly` subset, whose diff splits every collection into a `<name>Removed`/`<name>Upserted` pair, carries real rows here.
...
fn vector(ctx: &Context) -> Result<Vector, String> {
    ...
    if !KINDS.contains(&kind.as_str()) {
        return Err(format!("scenario doc string names {kind:?}, which is not a declared Procedural2dMutation kind"));
    }
...
fn round_trip(ctx: &Context) -> Result<Outcome, String> {
    const SNAPSHOT: &str = "asset://🧬️schema/🧬️mutations/➖delete-generation/🧪️tests/removes-the-selected-generation-2-and-falls-back-to-generation-1/📸️snapshot/⬅️before/🔣️.json";
