...
pub fn deserialize(from: &JsonSnapshot) -> Result<Procedural2dSnapshot, store::TextError> {
    ...
    let snap: Procedural2dSnapshot = serde_json::from_value(json_value_to_serde(&from.value))
        .map_err(|e| store::TextError::new(format!("procedural2d<-json: {e}"), dsl::TextSpan::at(1, 1)))?;
