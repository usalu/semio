...
async fn applies_to_committed_after() {
    ...
    apply_procedural2d_mutation(&mut snapshot, &mutation()).expect("connect-synapse applies to its committed before-snapshot");
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "connect-synapse/joins-note-b-to-note-c-at-index-1: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
