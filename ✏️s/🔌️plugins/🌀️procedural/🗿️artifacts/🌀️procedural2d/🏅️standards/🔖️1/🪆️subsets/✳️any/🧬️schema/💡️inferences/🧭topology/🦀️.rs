//! 🧭 `topology` — one named inference: the DAG shape of a procedural2d snapshot's `fixture`
...
/// 🧭️ Computes `topology` from a procedural2d snapshot's `fixture.widgets`/`fixture.synapses` via
/// Kahn's algorithm: widgets are nodes, synapses (`from` → `to`) are directed edges.
pub fn compute_procedural2d_topology(snapshot: &Procedural2dSnapshot) -> Procedural2dTopology {
    ...
    Procedural2dTopology { node_count: widget_ids.len() as u32, edge_count: snapshot.fixture.synapses.len() as u32, topo_order: if cycle_free { order } else { Vec::new() }, depth: if cycle_free { depth } else { 0 }, cycle_free }
}
//#endregion 🔖️Topology
