...
Feature: Apply every typed assembly mutation twice — once in Rust, once in Python — and require the same answer
  ...
  library could confirm or refute". `mutate-fem3d-1` and `🗺️mutate-gisterrain-1` refuted that in this
  ...
  states — the blocker `🧩️mutate-block-3d-1` and `🟩️mutate-program-1` both report.
  ...
  Scenario Outline: The committed <id> vector declares its own kind and moves the document
    ...
    Examples:
      | id               | vector                                                        |
      | create-slot      | 🧩️create-slot/🧪️tests/appends-slot-c-at-index-2                |
      | delete-slot      | 🕳️delete-slot/🧪️tests/removes-slot-a-and-cascades-edge-ab     |
      | create-rule      | 📏️create-rule/🧪️tests/appends-a-rule-forbidding-roof-over-wall |
      | delete-rule      | ❌delete-rule/🧪️tests/removes-the-wall-roof-rule              |
      | change-weight    | 🔢️change-weight/🧪️tests/raises-the-wall-module-selection-bias  |
      | remove-weight    | 🪶️remove-weight/🧪️tests/drops-the-wall-module-weight-override |
      | connect-slots    | 🔗️connect-slots/🧪️tests/joins-slot-b-to-slot-c-at-index-1      |
      | disconnect-slots | ✂️disconnect-slots/🧪️tests/severs-edge-ab-leaving-both-slots  |
      | change-seed      | 🎲️change-seed/🧪️tests/reseeds-the-solve-from-7-to-99           |
