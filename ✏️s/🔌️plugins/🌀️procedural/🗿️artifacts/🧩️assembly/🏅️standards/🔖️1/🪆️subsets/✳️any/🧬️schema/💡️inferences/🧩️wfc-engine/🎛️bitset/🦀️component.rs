...
impl PatternSet {
    ...
    #[inline]
    pub async fn is_empty_universe(&self) -> bool {
        self.len == 0
    }

    #[inline]
    pub async fn get(&self, p: PatternId) -> bool {
        let idx = p.index();
    ...
    #[inline]
    pub async fn set(&mut self, p: PatternId, value: bool) {
        let idx = p.index();
        debug_assert!(idx < self.len);
    ...
    pub async fn and_with(&mut self, other: &PatternSet) {
    ...
    /// 🎭️ In-place `self |= other`.
    pub async fn or_with(&mut self, other: &PatternSet) {
        debug_assert_eq!(self.len, other.len);
        for (a, b) in self.words.iter_mut().zip(other.words.iter()) {
            *a |= *b;
        }
    ...
    /// 🎭️ In-place `self &= !other`.
    pub async fn and_not_with(&mut self, other: &PatternSet) {
        debug_assert_eq!(self.len, other.len);
        for (a, b) in self.words.iter_mut().zip(other.words.iter()) {
            *a &= !*b;
        }
    ...
    pub async fn count_ones(&self) -> u32 {
    ...
    pub async fn first_set(&self) -> Option<PatternId> {
    ...
    pub async fn iter_ones(&self) -> impl Iterator<Item = PatternId> + '_ {
    ...
    #[inline]
    pub async fn words(&self) -> &[u64] {
        &self.words
    }

    pub async fn is_subset_of(&self, other: &PatternSet) -> bool {
        debug_assert_eq!(self.len, other.len);
        self.words.iter().zip(other.words.iter()).all(|(a, b)| a & !b == 0)
    }

    pub async fn intersects(&self, other: &PatternSet) -> bool {
        debug_assert_eq!(self.len, other.len);
    ...
    pub async fn is_well_formed(&self) -> bool {
    ...
    pub async fn restrict_returning_removed(&mut self, allowed: &PatternSet, removed_out: &mut PatternSet) -> u32 {
...
mod tests {
    ...
    async fn from_indices(len: usize, idxs: &[usize]) -> PatternSet {
