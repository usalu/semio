//! 🧪️ `remove-weight` fixture — `drops-the-wall-module-weight-override`.
//!
//! `remove-weight`'s diff builder writes one `weights_removed` id, returning that module to the solver's neutral default bias; no other collection is written.
//!
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "remove-weight/drops-the-wall-module-weight-override: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "remove-weight/drops-the-wall-module-weight-override: committed {side} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "remove-weight/drops-the-wall-module-weight-override: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "remove-weight/drops-the-wall-module-weight-override: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_assembly_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "remove-weight/drops-the-wall-module-weight-override: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "remove-weight/drops-the-wall-module-weight-override: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "remove-weight/drops-the-wall-module-weight-override: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("remove-weight/drops-the-wall-module-weight-override: unknown outcome status {other:?}"),
    }
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "remove-weight/drops-the-wall-module-weight-override: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "remove-weight/drops-the-wall-module-weight-override: committed diff JSON is not canonical");
}
