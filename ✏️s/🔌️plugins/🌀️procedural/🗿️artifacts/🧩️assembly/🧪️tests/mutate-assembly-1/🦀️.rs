//! 🦀️ Assembly 1 exhaustive mutation case — Rust adapter. Ticket
//! `26/08/23/END-TO-END-TESTING-REFACTOR`.
//!
//! Recorded no-oracle decision `assembly-mutation-semantics`
//! (`../../🏅️standards/🔖️1/🪆️subsets/✳️any/🧪️oracle/🔣️.json`): this is a semio-NATIVE
//! document and `AssemblyMutation` IS its specification, so there is nothing third-party to register. What
...
//#region 🔖️Vocabulary
/// 🏷️ Mirrors `AssemblyMutation::KINDS`
...
/// 🔀️ Snapshot field → the diff field(s) allowed to declare it. This subset is the reason the table exists. `AssemblyDiff` does NOT mirror its snapshot: every collection is split into a `<name>Removed` id list and a `<name>Upserted` `(index, record)` list, so the footprint law has to know that a change to `slots` may be declared by either `slotsRemoved` or `slotsUpserted`. No other subset in this plugin family splits its deltas this way.
const DIFF_ALIASES: &[(&str, &[&str])] = &[
    ("slots", &["slotsRemoved", "slotsUpserted"]),
    ("edges", &["edgesRemoved", "edgesUpserted"]),
    ("weights", &["weightsRemoved", "weightsUpserted"]),
...
fn vector(ctx: &Context) -> Result<Vector, String> {
    ...
    if !KINDS.contains(&kind.as_str()) {
        return Err(format!("scenario doc string names {kind:?}, which is not a declared AssemblyMutation kind"));
    }
...
/// 🐫️ `create-slot` → `CreateSlot`, the Rust variant name this subset's EXTERNALLY tagged enum writes as the
...
/// 🔁️ Decode and re-encode the three-slot wave-function-collapse assembly, through the platform's own dependency-free JSON reader and writer: the
...
fn round_trip(ctx: &Context) -> Result<Outcome, String> {
    const SNAPSHOT: &str = "asset://🏅️standards/🔖️1/🪆️subsets/✳️any/🧬️schema/🧬️mutations/🔗connect-slots/🧪️tests/joins-slot-b-to-slot-c-at-index-1/📸️snapshot/⬅️before/🔣️component.json";
    ...
    if reparsed.array("slots").len() < 3 || reparsed.array("edges").is_empty() || reparsed.array("modules").len() < 2 || reparsed.array("rules").is_empty() || reparsed.array("weights").is_empty() {
        return Err("the committed round-trip snapshot is the three-slot, two-module assembly with an edge, a rule and a weight this scenario describes, but at least one of those is missing".to_string());
    }
