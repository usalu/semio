//! 🧬️ 🧬️ Generation2d play app commands command — `select-generation`.
...
pub fn handle(payload: &SelectGeneration, doc: &ArtifactView<'_, Generation2dSnapshot>, cfg: &ConfigView<'_, Generation2dConfig>, session: &mut FlowEvalSession) -> Result<Emit<Generation2dMutation, Generation2dConfigMutation>, Fault> {
    let id = payload.id.clone().map(dsl::DslValue::String).unwrap_or(dsl::DslValue::Null);
    Ok(handle_generation("selectGeneration", Some(&dsl::DslValue::object([("id".to_string(), id)])), doc, cfg, session))
}
