@capability-procedural-generation2d-1-any-editor-transient-mutate
@no-oracle-procedural-generation2d-1-any-editor-transient-state-lane-semantics
@comparison-ordered-json-v1
@mutations-procedural-generation2d-1-any-editor-transient
Feature: Apply every transient state-lane mutation of s.procedural.generation2d's ✏️editor/🫧️transient to its committed vector
  `Generation2dTransientMutation` is the transient lane of this subset's editor: it holds the text of the last generation preview, or none before the first one. It is this
  repository's own state record, so the case records the no-oracle decision `procedural-generation2d-1-any-editor-transient-state-lane-semantics` in
  ...
  before-snapshot. Production dispatch is reached through `generation2d_transient_mutation_report_json`, which runs
  ...
  Scenario Outline: Apply <id> and land on the committed after-snapshot, diff and outcome
    ...
    When <id> is applied to that vector's before-snapshot through generation2d_transient_mutation_report_json
