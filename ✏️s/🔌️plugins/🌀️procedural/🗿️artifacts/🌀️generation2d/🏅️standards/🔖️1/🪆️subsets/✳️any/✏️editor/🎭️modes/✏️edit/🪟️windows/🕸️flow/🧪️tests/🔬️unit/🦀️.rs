...
async fn main_graph_scene_exports_flow_backed_node_graph_fields() {
    ...
    // 🛍️ See the `🧊️generation3d` twin: the registered operator catalogue is app-static and rides the
    // reserved `framework.section.catalogue` surface, never this scene's fixed-capacity payload
