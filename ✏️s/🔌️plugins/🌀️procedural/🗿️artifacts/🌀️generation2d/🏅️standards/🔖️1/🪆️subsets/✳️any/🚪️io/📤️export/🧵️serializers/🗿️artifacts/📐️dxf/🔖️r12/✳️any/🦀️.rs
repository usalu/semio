//! generation2d → dxf — the page drawing (`generation2d_drawing`) written as DXF R12 entities by `s.stdio.semio/v1/drawing`'s own
...
pub fn serialize_bytes(snapshot: &Generation2dSnapshot) -> Result<Vec<u8>, store::TextError> {
    encode_drawing(&generation2d_drawing(snapshot).map_err(|error| store::TextError::new(format!("generation2d→dxf: {error}"), dsl::TextSpan::at(1, 1)))?, SemioDrawingFormat::Dxf).map_err(|error| store::TextError::new(format!("generation2d→dxf: {error}"), dsl::TextSpan::at(1, 1)))
}
