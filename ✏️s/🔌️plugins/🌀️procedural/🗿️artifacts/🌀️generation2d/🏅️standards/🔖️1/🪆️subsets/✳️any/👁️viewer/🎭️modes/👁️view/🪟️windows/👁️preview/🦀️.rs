//! 👁️ Generation2d viewer — the Preview window: a read-only schematic render of every widget's
...
pub const BODY_KEY: &str = "generation2d.view.preview";
const SURFACE_ID: &str = "generation2d.view.preview";
/// 👁️ Read-only counterpart of the editor's `GENERATION2D_PLAY_APP_ID` controller id — kept distinct
/// so a viewer session's canvas controller can never be mistaken for an editor session's.
const GENERATION2D_VIEW_CONTROLLER_ID: &str = "generation2d-view";
...
pub fn render(document: &Generation2dSnapshot) -> semio_framework_plugin::UiAssemblyResult<BuiltNode> {
    ...
    let layers: Vec<dsl::DslValue> = fixture
        ...
        .collect();
    let _ = GENERATION2D_VIEW_CONTROLLER_ID;
    crate::scene_surface(
        SURFACE_ID,
