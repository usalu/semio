...
pub fn target_digest(target: PreviewEvalTarget, doc: &ArtifactView<'_, Generation2dSnapshot>, cfg: &ConfigView<'_, Generation2dConfig>) -> u64 {
    let fixture = dsl::json::to_json_string(&doc.snapshot.fixture);
    match target {
...
pub fn evaluate(window_id: &str, window_kind_id: &str, target: PreviewEvalTarget, doc: &ArtifactView<'_, Generation2dSnapshot>, cfg: &ConfigView<'_, Generation2dConfig>, session: &mut FlowEvalSession) -> (Emit<Generation2dMutation, Generation2dConfigMutation>, FlowEvalPublication) {
    ...
    let outcome = match target {
        PreviewEvalTarget::Document => preview_eval::evaluate_tick(window_id, window_kind_id, &doc.snapshot.fixture, session, retained.as_deref()),
        PreviewEvalTarget::Generation => {
            ...
            let host = crate::standards::v1::subsets::any::schema::generation_preview_host(&doc.snapshot.fixture, &values);
            let outcome = preview_eval::evaluate_tick(window_id, window_kind_id, &host.fixture, session, retained.as_deref());
            host.retire_cold();
            outcome
        }
    };
