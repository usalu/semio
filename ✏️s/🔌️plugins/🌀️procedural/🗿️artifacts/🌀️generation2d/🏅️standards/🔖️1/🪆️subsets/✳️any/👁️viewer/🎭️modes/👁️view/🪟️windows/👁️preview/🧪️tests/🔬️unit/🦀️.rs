...
fn render_produces_a_scene_node_for_the_default_document() {
    ...
    let json = semio_framework_plugin::artifact_app_laws::project_and_retire_fixture_tree(semio_framework_plugin::ComponentTree { root: render(&document).expect("viewer fixture") }).expect("render json");
