//! 🧭 `topology` — one named inference: the DAG shape of a generation2d snapshot's `fixture`
...
pub struct Generation2dTopology {
    pub node_count: u32,
    pub edge_count: u32,
...
/// 🧭️ Computes `topology` from a generation2d snapshot's `fixture.widgets`/`fixture.synapses` via
/// Kahn's algorithm: widgets are nodes, synapses (`from` → `to`) are directed edges.
pub fn compute_generation2d_topology(snapshot: &Generation2dSnapshot) -> Generation2dTopology {
    ...
    Generation2dTopology { node_count: widget_ids.len() as u32, edge_count: snapshot.host_snapshot.synapses.len() as u32, topo_order: if cycle_free { order } else { Vec::new() }, depth: if cycle_free { depth } else { 0 }, cycle_free }
}
//#endregion 🔖️Topology
