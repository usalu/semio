...
pub fn compute_generation2d_topology(snapshot: &Generation2dSnapshot) -> Generation2dTopology {
    ...
    Generation2dTopology { node_count: widget_ids.len() as u32, edge_count: snapshot.host_document.synapses.len() as u32, topo_order: if cycle_free { order } else { Vec::new() }, depth: if cycle_free { depth } else { 0 }, cycle_free }
}
//#endregion 🔖️Topology
