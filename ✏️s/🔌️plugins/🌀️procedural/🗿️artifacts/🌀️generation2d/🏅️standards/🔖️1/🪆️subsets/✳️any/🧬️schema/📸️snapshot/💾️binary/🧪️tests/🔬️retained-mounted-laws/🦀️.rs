...
fn admit(session: &mut Generation2dMountedPackSession, value: u8) {
    loop {
        if let Some(exact) = session.next_retained_allocation_bytes().expect("P2 source allocation query") {
            let step = session.reserve_retained_allocation(exact).expect("P2 source allocation grant");
...
fn non_empty_canonical_snapshot_round_trips_one_grant_at_a_time() {
    ...
    let mut session = generation2d_mounted_pack_session(bytes.len(), 8_192).expect("P2 retained snapshot preflight");
    ...
    for _ in 0..1_000_000 {
        if let Some(exact) = session.next_retained_allocation_bytes().expect("P2 retained allocation query") {
            assert!(session.reserve_retained_allocation(exact).expect("P2 retained allocation grant").progressed);
            continue;
        }
...
fn p3d3_is_rejected_before_semantic_allocation() {
    let mut session = generation2d_mounted_pack_session(8, 8).expect("P2 hostile discriminator preflight");
