/** ✏️ Generation2d editor — subset-level typed twin. Re-exports every window's typed view-model
 * binding so a host-side TS consumer has one import surface for the whole editor manifest, mirroring
 * `🦀️.rs`'s `create_generation2d_app()` stitching every window/mode module together. */

export const GENERATION2D_EDITOR_DIALECT = { artifactKind: "s.procedural.generation2d", standard: "1", subset: "*" } as const;

export const GENERATION2D_PLAY_MODE_EDIT = "edit" as const;
export const GENERATION2D_PLAY_MODE_GENERATE = "generate" as const;
