...
Feature: Apply every typed generation2d mutation twice — once in Rust, once in Python — and require the same answer
  ...
  Scenario Outline: The committed <id> vector declares its own kind and moves the document
    ...
    Examples:
      | id                      | vector                                                                                      |
      | create-widget           | 🌱️create-widget/🧪️tests/inserts-note-c-at-index-2                                            |
      | replace-widget          | 🔁️replace-widget/🧪️tests/rewrites-the-note-b-body-in-place                                   |
      | delete-widget           | 🗑️delete-widget/🧪️tests/removes-note-a-and-flags-the-dangling-synapse                       |
      | connect-synapse         | 🔗️connect-synapse/🧪️tests/joins-note-b-to-note-c-at-index-1                                  |
      | replace-synapse         | 🔄️replace-synapse/🧪️tests/repoints-link-ab-onto-the-alt-port                                 |
      | disconnect-synapse      | ✂️disconnect-synapse/🧪️tests/severs-link-ab-leaving-both-notes                              |
      | move-widget             | 📍️move-widget/🧪️tests/repositions-note-a-on-the-canvas                                       |
      | clear-widget-layout     | 🧹clear-widget-layout/🧪️tests/drops-the-note-a-layout-entry                                  |
      | update-camera           | 🎛️set-camera/🧪️tests/pans-and-zooms-the-graph-camera                                         |
      | change-schema           | 🔤️change-schema/🧪️tests/restamps-the-fixture-schema                                          |
      ...
      | change-generation-value | 🔢️change-generation-value/🧪️tests/raises-the-height-answer-in-generation-1                   |
