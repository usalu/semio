...
export function generation2dSnapshotFixtureAssetSelfTests(): number {
  ...
  for (const source of sources) {
    ...
    assert(!/\bgeneration2d_fixture_operations\b/.test(source), "use generation2d_host_document_operations");
    assert(!/\bflow_fixture_to_form_spec\b/.test(source), "use flow_host_document_to_form_spec");
    assert(!/pub fixture:\s*FlowHostDocument/.test(source), "Generation2dSnapshot fields must be host_document");
    assert(!/\.replace_fixture\s*\(/.test(source), "FlowHost must use replace_host_document");
  }
  console.log("[DEBUG] Generation2d snapshot/fixture/asset terminology lint: 3 sources, 0 forbidden patterns");
