...
pub fn diff(payload: &super::DeleteWidget, base: &Generation2dSnapshot) -> protocol::MutationOutcome<Generation2dDiff> {
    ...
    let cascaded_synapse_ids: Vec<String> = base.host_document.synapses.iter().filter(|synapse| synapse.from == payload.id || synapse.to == payload.id).map(|synapse| synapse.id.clone()).collect();
