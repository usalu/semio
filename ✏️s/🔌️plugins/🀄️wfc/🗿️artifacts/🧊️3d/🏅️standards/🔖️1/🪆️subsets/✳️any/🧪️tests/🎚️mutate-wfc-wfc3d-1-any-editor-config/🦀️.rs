//! 🎚️ `s.wfc.wfc3d` ✏️editor/🎚️config state-lane mutation case — Rust adapter.
//!
//! Recorded no-oracle decision `wfc-wfc3d-1-any-editor-config-state-lane-semantics`: the runner dispatches no oracle role, so every law is asserted inside
//! the subject handlers through `semio_s_plugin_stdio_test_oracle::law::vector` over the report of this crate's
//! production bridge `wfc3d_config_mutation_report_json`. The oracle handlers answer with the committed after- and before-snapshots read
...
mod subject {
    ...
    use semio_s_artifact_wfc_3d::editor::wfc3d::config::mutations::wfc3d_config_mutation_report_json;

    fn report(committed: &Vector) -> Result<String, String> {
        wfc3d_config_mutation_report_json(committed.before, committed.mutation, committed.after)
    }
