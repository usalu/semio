//! 🧪️ `disconnect-slots` fixture — `✂️severs-the-corridor-b-edge`.
//!
//! `disconnect-slots` removes ONE edge and leaves both slots standing.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️disconnect-slots/✂️severs-the-corridor-b-edge/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️disconnect-slots/✂️severs-the-corridor-b-edge/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️disconnect-slots/✂️severs-the-corridor-b-edge/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️disconnect-slots/✂️severs-the-corridor-b-edge/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️disconnect-slots/✂️severs-the-corridor-b-edge/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "✂️severs-the-corridor-b-edge: applying the mutation must reach the committed ➡️after");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "✂️severs-the-corridor-b-edge: inverse() must restore the pre-mutation document");
}
...
fn committed_json_is_canonical() {
    for (label, text, snapshot) in [("⬅️before", BEFORE, before()), ("➡️after", AFTER, expected_after())] {
        ...
        assert_eq!(printed, committed, "✂️severs-the-corridor-b-edge: committed {label} is not canonical");
    }
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "✂️severs-the-corridor-b-edge: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_wfc3d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "✂️severs-the-corridor-b-edge: an applied outcome must apply");
            assert_ne!(snapshot, before(), "✂️severs-the-corridor-b-edge: an applied outcome must change the document");
        }
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "✂️severs-the-corridor-b-edge: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    let committed: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff decodes");
    assert_eq!(printed, committed, "✂️severs-the-corridor-b-edge: the committed diff is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(applied, expected_after(), "✂️severs-the-corridor-b-edge: the committed diff must be complete");
}
