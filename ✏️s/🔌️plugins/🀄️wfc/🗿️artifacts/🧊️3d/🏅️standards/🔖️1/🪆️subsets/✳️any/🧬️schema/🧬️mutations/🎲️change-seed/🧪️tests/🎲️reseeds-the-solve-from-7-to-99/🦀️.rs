...
//!
//! `change-seed` sets the scalar `seed` field only — every id-keyed collection lane stays empty.
//!
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "🎲️reseeds-the-solve-from-7-to-99: applying the mutation must reach the committed ➡️after");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "🎲️reseeds-the-solve-from-7-to-99: inverse() must restore the pre-mutation document");
}
...
fn committed_json_is_canonical() {
    for (label, text, snapshot) in [("⬅️before", BEFORE, before()), ("➡️after", AFTER, expected_after())] {
        ...
        assert_eq!(printed, committed, "🎲️reseeds-the-solve-from-7-to-99: committed {label} is not canonical");
    }
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "🎲️reseeds-the-solve-from-7-to-99: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_wfc3d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "🎲️reseeds-the-solve-from-7-to-99: an applied outcome must apply");
            assert_ne!(snapshot, before(), "🎲️reseeds-the-solve-from-7-to-99: an applied outcome must change the document");
        }
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "🎲️reseeds-the-solve-from-7-to-99: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    let committed: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff decodes");
    assert_eq!(printed, committed, "🎲️reseeds-the-solve-from-7-to-99: the committed diff is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(applied, expected_after(), "🎲️reseeds-the-solve-from-7-to-99: the committed diff must be complete");
}
