//! 🧪️ `create-tile` fixture — `🀄️adds-a-stair-tile-to-the-catalogue`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🀄️create-tile/🀄️adds-a-stair-tile-to-the-catalogue/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🀄️create-tile/🀄️adds-a-stair-tile-to-the-catalogue/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🀄️create-tile/🀄️adds-a-stair-tile-to-the-catalogue/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🀄️create-tile/🀄️adds-a-stair-tile-to-the-catalogue/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🀄️create-tile/🀄️adds-a-stair-tile-to-the-catalogue/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "🀄️adds-a-stair-tile-to-the-catalogue: applying the mutation must reach the committed ➡️after");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "🀄️adds-a-stair-tile-to-the-catalogue: inverse() must restore the pre-mutation document");
}
...
fn committed_json_is_canonical() {
    for (label, text, snapshot) in [("⬅️before", BEFORE, before()), ("➡️after", AFTER, expected_after())] {
        ...
        assert_eq!(printed, committed, "🀄️adds-a-stair-tile-to-the-catalogue: committed {label} is not canonical");
    }
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "🀄️adds-a-stair-tile-to-the-catalogue: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_wfc3d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "🀄️adds-a-stair-tile-to-the-catalogue: an applied outcome must apply");
            assert_ne!(snapshot, before(), "🀄️adds-a-stair-tile-to-the-catalogue: an applied outcome must change the document");
        }
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "🀄️adds-a-stair-tile-to-the-catalogue: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    let committed: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff decodes");
    assert_eq!(printed, committed, "🀄️adds-a-stair-tile-to-the-catalogue: the committed diff is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(applied, expected_after(), "🀄️adds-a-stair-tile-to-the-catalogue: the committed diff must be complete");
}
