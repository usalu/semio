...
impl Wfc3dFillTickPayload {
    ...
    pub fn into_transient(&self) -> crate::editor::wfc3d::transient::Wfc3dTransient {
        crate::editor::wfc3d::transient::Wfc3dTransient {
            assignments: self
                .assignments
                .iter()
                .filter_map(|(slot, tile)| {
                    tile.as_ref().map(|tile| crate::editor::wfc3d::transient::Wfc3dAssignment {
                        slot_id: slot.clone(),
                        tile_id: tile.clone(),
                    })
                })
    ...
    pub fn set_solve_mutations(&self) -> Vec<Wfc3dTransientMutation> {
        let assignments = self
            .assignments
            .iter()
            .filter_map(|(slot_id, tile_id)| {
                tile_id.as_ref().map(|tile_id| crate::editor::wfc3d::transient::Wfc3dAssignment {
...
pub fn assignments_from_job(job: &WfcJob<semio_s_plugin_wfc_engine::topology::GraphTopology>, tile_ids: &[String], snapshot: &Wfc3dSnapshot) -> BTreeMap<String, Option<String>> {
...
pub fn payload_from_commit(snapshot: &Wfc3dSnapshot, commit: &crate::inferences::Wfc3dInferenceCommit) -> Wfc3dFillTickPayload {
...
fn validate_id(id: &str) -> Result<(), String> {
    if id.is_empty() || id.len() > MAX_ID_BYTES {
        return Err("wfc3d-fill-id-invalid".into());
    }
    Ok(())
}
...
impl FillPreparation {
    fn new(snapshot: Wfc3dSnapshot) -> Self {
        Self {
            snapshot,
    ...
    fn build_model(&mut self) -> Result<(), String> {
        ...
        if self.relation_names.is_empty() {
            self.relation_names.push(crate::editor::wfc3d::WFC_3D_DEFAULT_RELATION.to_string());
        }
    ...
    fn apply_pins(&mut self, operation: Operation) -> Result<WfcJob<semio_s_plugin_wfc_engine::topology::GraphTopology>, String> {
        ...
        let model = self.model.take().ok_or("wfc3d-fill-model-missing")?;
        let topology = self.topology.take().ok_or("wfc3d-fill-topology-missing")?;
...
impl Wfc3dFillRunJob {
    pub fn new(identity: ToolRunIdentity, snapshot: Arc<Wfc3dSnapshot>, port: semio_framework_plugin::ToolRunJobPort) -> Self {
        Self {
            identity,
    ...
    pub fn take_set_solve(&mut self) -> Option<Vec<Wfc3dTransientMutation>> {
        self.set_solve.take()
    }

    fn empty_payload(snapshot: &Wfc3dSnapshot) -> Wfc3dFillTickPayload {
...
pub fn build_tool_run_job(request: ToolRunJobRequest<'_, semio_framework_plugin::EditorApp<crate::editor::wfc3d::Wfc3dEditor>>) -> Result<Option<semio_framework_plugin::ToolRunJob>, Fault> {
    if request.tool_id != TOOL_ID || request.purpose != ToolRunJobPurpose::Run {
        return Ok(None);
    }
    Ok(Some(Box::new(Wfc3dFillRunJob::new(request.identity, request.snapshot.clone(), request.port.clone()))))
}
//#endregion 🏗️Factory
