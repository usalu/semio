//! 🧪️ `delete-slot` fixture — `🕳️removes-room-a-and-cascades-its-edge`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕳️delete-slot/🕳️removes-room-a-and-cascades-its-edge/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕳️delete-slot/🕳️removes-room-a-and-cascades-its-edge/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕳️delete-slot/🕳️removes-room-a-and-cascades-its-edge/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕳️delete-slot/🕳️removes-room-a-and-cascades-its-edge/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🕳️delete-slot/🕳️removes-room-a-and-cascades-its-edge/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "🕳️removes-room-a-and-cascades-its-edge: applying the mutation must reach the committed ➡️after");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "🕳️removes-room-a-and-cascades-its-edge: inverse() must restore the pre-mutation document");
}
...
fn committed_json_is_canonical() {
    for (label, text, snapshot) in [("⬅️before", BEFORE, before()), ("➡️after", AFTER, expected_after())] {
        ...
        assert_eq!(printed, committed, "🕳️removes-room-a-and-cascades-its-edge: committed {label} is not canonical");
    }
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "🕳️removes-room-a-and-cascades-its-edge: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_wfc3d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "🕳️removes-room-a-and-cascades-its-edge: an applied outcome must apply");
            assert_ne!(snapshot, before(), "🕳️removes-room-a-and-cascades-its-edge: an applied outcome must change the document");
        }
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "🕳️removes-room-a-and-cascades-its-edge: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    let committed: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff decodes");
    assert_eq!(printed, committed, "🕳️removes-room-a-and-cascades-its-edge: the committed diff is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(applied, expected_after(), "🕳️removes-room-a-and-cascades-its-edge: the committed diff must be complete");
}
