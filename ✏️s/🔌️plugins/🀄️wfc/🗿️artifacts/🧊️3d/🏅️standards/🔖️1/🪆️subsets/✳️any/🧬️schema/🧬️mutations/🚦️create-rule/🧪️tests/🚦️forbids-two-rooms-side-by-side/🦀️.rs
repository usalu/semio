//! 🧪️ `create-rule` fixture — `🚦️forbids-two-rooms-side-by-side`.
//!
//! `create-rule` upserts ONE rule at its canonical sorted index.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚦️create-rule/🚦️forbids-two-rooms-side-by-side/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚦️create-rule/🚦️forbids-two-rooms-side-by-side/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚦️create-rule/🚦️forbids-two-rooms-side-by-side/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚦️create-rule/🚦️forbids-two-rooms-side-by-side/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚦️create-rule/🚦️forbids-two-rooms-side-by-side/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "🚦️forbids-two-rooms-side-by-side: applying the mutation must reach the committed ➡️after");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "🚦️forbids-two-rooms-side-by-side: inverse() must restore the pre-mutation document");
}
...
fn committed_json_is_canonical() {
    for (label, text, snapshot) in [("⬅️before", BEFORE, before()), ("➡️after", AFTER, expected_after())] {
        ...
        assert_eq!(printed, committed, "🚦️forbids-two-rooms-side-by-side: committed {label} is not canonical");
    }
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "🚦️forbids-two-rooms-side-by-side: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_wfc3d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "🚦️forbids-two-rooms-side-by-side: an applied outcome must apply");
            assert_ne!(snapshot, before(), "🚦️forbids-two-rooms-side-by-side: an applied outcome must change the document");
        }
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "🚦️forbids-two-rooms-side-by-side: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    let committed: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff decodes");
    assert_eq!(printed, committed, "🚦️forbids-two-rooms-side-by-side: the committed diff is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(applied, expected_after(), "🚦️forbids-two-rooms-side-by-side: the committed diff must be complete");
}
