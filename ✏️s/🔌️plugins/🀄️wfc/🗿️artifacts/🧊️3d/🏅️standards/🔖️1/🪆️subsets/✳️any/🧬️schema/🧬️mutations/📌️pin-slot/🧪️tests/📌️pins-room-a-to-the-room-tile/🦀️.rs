//! 🧪️ `pin-slot` fixture — `📌️pins-room-a-to-the-room-tile`.
//!
//! `pin-slot` replaces ONE slot at its own index, setting only its pin.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📌️pin-slot/📌️pins-room-a-to-the-room-tile/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📌️pin-slot/📌️pins-room-a-to-the-room-tile/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📌️pin-slot/📌️pins-room-a-to-the-room-tile/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📌️pin-slot/📌️pins-room-a-to-the-room-tile/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📌️pin-slot/📌️pins-room-a-to-the-room-tile/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "📌️pins-room-a-to-the-room-tile: applying the mutation must reach the committed ➡️after");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "📌️pins-room-a-to-the-room-tile: inverse() must restore the pre-mutation document");
}
...
fn committed_json_is_canonical() {
    for (label, text, snapshot) in [("⬅️before", BEFORE, before()), ("➡️after", AFTER, expected_after())] {
        ...
        assert_eq!(printed, committed, "📌️pins-room-a-to-the-room-tile: committed {label} is not canonical");
    }
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "📌️pins-room-a-to-the-room-tile: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_wfc3d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "📌️pins-room-a-to-the-room-tile: an applied outcome must apply");
            assert_ne!(snapshot, before(), "📌️pins-room-a-to-the-room-tile: an applied outcome must change the document");
        }
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "📌️pins-room-a-to-the-room-tile: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    let committed: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff decodes");
    assert_eq!(printed, committed, "📌️pins-room-a-to-the-room-tile: the committed diff is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(applied, expected_after(), "📌️pins-room-a-to-the-room-tile: the committed diff must be complete");
}
