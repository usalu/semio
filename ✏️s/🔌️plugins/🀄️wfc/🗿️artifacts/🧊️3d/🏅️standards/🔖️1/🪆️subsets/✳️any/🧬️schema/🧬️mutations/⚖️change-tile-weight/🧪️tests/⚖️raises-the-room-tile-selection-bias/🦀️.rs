//! 🧪️ `change-tile-weight` fixture — `⚖️raises-the-room-tile-selection-bias`.
//!
//! `change-tile-weight` replaces ONE tile at its own index, changing only its weight.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚖️change-tile-weight/⚖️raises-the-room-tile-selection-bias/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚖️change-tile-weight/⚖️raises-the-room-tile-selection-bias/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚖️change-tile-weight/⚖️raises-the-room-tile-selection-bias/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚖️change-tile-weight/⚖️raises-the-room-tile-selection-bias/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/⚖️change-tile-weight/⚖️raises-the-room-tile-selection-bias/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "⚖️raises-the-room-tile-selection-bias: applying the mutation must reach the committed ➡️after");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "⚖️raises-the-room-tile-selection-bias: inverse() must restore the pre-mutation document");
}
...
fn committed_json_is_canonical() {
    for (label, text, snapshot) in [("⬅️before", BEFORE, before()), ("➡️after", AFTER, expected_after())] {
        ...
        assert_eq!(printed, committed, "⚖️raises-the-room-tile-selection-bias: committed {label} is not canonical");
    }
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "⚖️raises-the-room-tile-selection-bias: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_wfc3d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "⚖️raises-the-room-tile-selection-bias: an applied outcome must apply");
            assert_ne!(snapshot, before(), "⚖️raises-the-room-tile-selection-bias: an applied outcome must change the document");
        }
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "⚖️raises-the-room-tile-selection-bias: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    let committed: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff decodes");
    assert_eq!(printed, committed, "⚖️raises-the-room-tile-selection-bias: the committed diff is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(applied, expected_after(), "⚖️raises-the-room-tile-selection-bias: the committed diff must be complete");
}
