//! 🧪️ `delete-tile` fixture — `🗑️drops-the-corridor-tile-and-its-rule`.
//!
//! `delete-tile` removes the tile, every rule naming it, and every pin on it.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️delete-tile/🗑️drops-the-corridor-tile-and-its-rule/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️delete-tile/🗑️drops-the-corridor-tile-and-its-rule/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️delete-tile/🗑️drops-the-corridor-tile-and-its-rule/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️delete-tile/🗑️drops-the-corridor-tile-and-its-rule/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🗑️delete-tile/🗑️drops-the-corridor-tile-and-its-rule/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "🗑️drops-the-corridor-tile-and-its-rule: applying the mutation must reach the committed ➡️after");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "🗑️drops-the-corridor-tile-and-its-rule: inverse() must restore the pre-mutation document");
}
...
fn committed_json_is_canonical() {
    for (label, text, snapshot) in [("⬅️before", BEFORE, before()), ("➡️after", AFTER, expected_after())] {
        ...
        assert_eq!(printed, committed, "🗑️drops-the-corridor-tile-and-its-rule: committed {label} is not canonical");
    }
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "🗑️drops-the-corridor-tile-and-its-rule: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_wfc3d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "🗑️drops-the-corridor-tile-and-its-rule: an applied outcome must apply");
            assert_ne!(snapshot, before(), "🗑️drops-the-corridor-tile-and-its-rule: an applied outcome must change the document");
        }
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "🗑️drops-the-corridor-tile-and-its-rule: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    let committed: serde_json::Value = serde_json::from_str(DIFF).expect("committed diff decodes");
    assert_eq!(printed, committed, "🗑️drops-the-corridor-tile-and-its-rule: the committed diff is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(applied, expected_after(), "🗑️drops-the-corridor-tile-and-its-rule: the committed diff must be complete");
}
