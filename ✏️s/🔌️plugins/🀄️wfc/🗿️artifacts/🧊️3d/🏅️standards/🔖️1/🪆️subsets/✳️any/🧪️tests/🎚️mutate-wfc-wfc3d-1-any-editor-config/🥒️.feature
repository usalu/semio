@capability-wfc-wfc3d-1-any-editor-config-mutate
@no-oracle-wfc-wfc3d-1-any-editor-config-state-lane-semantics
@comparison-ordered-json-v1
@mutations-wfc-wfc3d-1-any-editor-config
Feature: Apply every config state-lane mutation of s.wfc.wfc3d's ✏️editor/🎚️config to its committed vector
  `Wfc3dConfigMutation` is the config lane of this subset's editor: it holds this pane's camera (x, y, zoom) and the tile a pin gesture assigns. It is this
  repository's own state record, so the case records the no-oracle decision `wfc-wfc3d-1-any-editor-config-state-lane-semantics` in
  ...
  before-snapshot. Production dispatch is reached through `wfc3d_config_mutation_report_json`, which runs
  ...
  Scenario Outline: Apply <id> and land on the committed after-snapshot, diff and outcome
    ...
    When <id> is applied to that vector's before-snapshot through wfc3d_config_mutation_report_json
