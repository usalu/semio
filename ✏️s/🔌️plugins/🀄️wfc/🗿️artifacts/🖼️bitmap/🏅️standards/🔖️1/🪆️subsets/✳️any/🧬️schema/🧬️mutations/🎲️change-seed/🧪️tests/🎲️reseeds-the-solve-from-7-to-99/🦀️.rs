...
//!
//! the scalar `seed` field only — every other lane stays empty, so the solve re-runs without the spec itself moving.
//!
...
fn applies_to_committed_after() {
    ...
    apply_bitmap_mutation(&mut snapshot, &mutation()).expect("change-seed applies to its committed before-snapshot");
...
/// pins WHICH lanes `change-seed` is allowed to touch, not merely that the end state matches.
#[test]
