@capability-wfc-bitmap-1-any-editor-transient-mutate
@no-oracle-wfc-bitmap-1-any-editor-transient-state-lane-semantics
@comparison-ordered-json-v1
@mutations-wfc-bitmap-1-any-editor-transient
Feature: Apply every transient state-lane mutation of s.wfc.bitmap's ✏️editor/🫧️transient to its committed vector
  `BitmapTransientMutation` is the transient lane of this subset's editor: it holds the last solve's palette-index pixels, its output extent and whether it ended in a contradiction. It is this
  repository's own state record, so the case records the no-oracle decision `wfc-bitmap-1-any-editor-transient-state-lane-semantics` in
  ...
  before-snapshot. Production dispatch is reached through `bitmap_transient_mutation_report_json`, which runs
  ...
  Scenario Outline: Apply <id> and land on the committed after-snapshot, diff and outcome
    ...
    When <id> is applied to that vector's before-snapshot through bitmap_transient_mutation_report_json
