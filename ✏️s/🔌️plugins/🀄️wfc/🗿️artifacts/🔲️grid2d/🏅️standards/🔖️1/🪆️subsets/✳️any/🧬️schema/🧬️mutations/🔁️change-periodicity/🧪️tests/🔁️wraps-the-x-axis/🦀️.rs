...
//!
//! A periodic axis becomes `Boundary::Wrap` in the solve topology; the persisted document only records the two flags.
//!
...
fn applies_to_committed_after() {
    ...
    apply_grid2d_mutation(&mut snapshot, &mutation()).expect("change-periodicity applies to its committed before-snapshot");
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "change-periodicity/🔁️wraps-the-x-axis: inverse did not restore the before-snapshot");
}
...
/// `change-periodicity` is allowed to touch, not merely that the end state matches.
#[test]
