...
//!
//! `change-seed` sets the scalar `seed` lane only — every id-keyed collection delta stays empty, so the solve inference re-runs without the spec itself moving.
//!
...
fn applies_to_committed_after() {
    ...
    apply_grid2d_mutation(&mut snapshot, &mutation()).expect("change-seed applies to its committed before-snapshot");
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "change-seed/🎲️reseeds-the-solve-from-7-to-99: inverse did not restore the before-snapshot");
}
...
/// `change-seed` is allowed to touch, not merely that the end state matches.
#[test]
