//! 🧪️ `move-slot` fixture — `↔️drags-slot-b-down`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/↔️move-slot/↔️drags-slot-b-down/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/↔️move-slot/↔️drags-slot-b-down/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/↔️move-slot/↔️drags-slot-b-down/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/↔️move-slot/↔️drags-slot-b-down/🔺️diff/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "move-slot/↔️drags-slot-b-down: applied state differs from the committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "move-slot/↔️drags-slot-b-down: inverse did not restore the before-snapshot");
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "move-slot/↔️drags-slot-b-down: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_wfc2d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "move-slot/↔️drags-slot-b-down: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "move-slot/↔️drags-slot-b-down: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "move-slot/↔️drags-slot-b-down: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("move-slot/↔️drags-slot-b-down: unknown outcome status {other:?}"),
    }
}
