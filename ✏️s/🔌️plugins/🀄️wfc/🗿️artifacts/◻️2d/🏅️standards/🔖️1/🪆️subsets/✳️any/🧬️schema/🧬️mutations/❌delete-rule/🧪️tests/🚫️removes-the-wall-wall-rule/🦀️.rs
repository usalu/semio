//! 🧪️ `delete-rule` fixture — `🚫️removes-the-wall-wall-rule`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/❌delete-rule/🚫️removes-the-wall-wall-rule/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/❌delete-rule/🚫️removes-the-wall-wall-rule/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/❌delete-rule/🚫️removes-the-wall-wall-rule/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/❌delete-rule/🚫️removes-the-wall-wall-rule/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/❌delete-rule/🚫️removes-the-wall-wall-rule/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "delete-rule/🚫️removes-the-wall-wall-rule: applied state differs from the committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "delete-rule/🚫️removes-the-wall-wall-rule: inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "delete-rule/🚫️removes-the-wall-wall-rule: committed {side} JSON is not canonical");
    }
    ...
    assert_eq!(reencoded, original, "delete-rule/🚫️removes-the-wall-wall-rule: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "delete-rule/🚫️removes-the-wall-wall-rule: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_wfc2d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "delete-rule/🚫️removes-the-wall-wall-rule: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "delete-rule/🚫️removes-the-wall-wall-rule: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "delete-rule/🚫️removes-the-wall-wall-rule: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("delete-rule/🚫️removes-the-wall-wall-rule: unknown outcome status {other:?}"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "delete-rule/🚫️removes-the-wall-wall-rule: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "delete-rule/🚫️removes-the-wall-wall-rule: committed diff JSON is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "delete-rule/🚫️removes-the-wall-wall-rule: committed diff did not carry before to after");
}
