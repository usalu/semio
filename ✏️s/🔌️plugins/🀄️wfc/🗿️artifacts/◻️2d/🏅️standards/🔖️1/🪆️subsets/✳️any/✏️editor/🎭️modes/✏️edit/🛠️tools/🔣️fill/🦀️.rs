...
pub fn payload_from_commit(snapshot: &Wfc2dSnapshot, commit: &crate::inferences::Wfc2dInferenceCommit) -> Wfc2dFillTickPayload {
    let assignments = snapshot
        ...
        .collect();
    Wfc2dFillTickPayload { assignments, contradiction: commit.contradiction, done: true }
}
//#endregion 🧱️Payload
            ...
            FillPhase::Run { .. } => {
                ...
                match &outcome {
                        ...
                        let payload = match commit.as_ref() {
                            Some(commit) => {
                                let assignments = snapshot
                                    ...
                                    .collect();
                                Wfc2dFillTickPayload { assignments, contradiction: false, done: true }
                            }
                            None => Wfc2dFillTickPayload {
                                assignments: snapshot.slots.iter().map(|slot| (slot.id.clone(), None)).collect(),
                                contradiction: true,
                                done: true,
                            },
                    ...
                    StepOutcome::Fault(fault) if semio_s_plugin_wfc_engine::job::payload_bytes(&fault.detail) == b"wfc-unsatisfiable" => {
                        ...
                        close_job(child.as_mut());
                        self.phase = FillPhase::Settled;
                        self.pending_finish = Some(Wfc2dFillTickPayload {
                            assignments: snapshot.slots.iter().map(|slot| (slot.id.clone(), None)).collect(),
                            contradiction: true,
                            done: true,
                        });
                    ...
                    StepOutcome::PreviewReady(_) | StepOutcome::CheckpointReady(_) => {
                        let payload = Wfc2dFillTickPayload {
