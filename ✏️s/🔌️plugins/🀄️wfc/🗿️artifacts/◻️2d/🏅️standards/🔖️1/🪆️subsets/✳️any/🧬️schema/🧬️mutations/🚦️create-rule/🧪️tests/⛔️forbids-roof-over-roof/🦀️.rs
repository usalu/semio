//! 🧪️ `create-rule` fixture — `⛔️forbids-roof-over-roof`.
//!
...
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚦️create-rule/⛔️forbids-roof-over-roof/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚦️create-rule/⛔️forbids-roof-over-roof/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚦️create-rule/⛔️forbids-roof-over-roof/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚦️create-rule/⛔️forbids-roof-over-roof/🎯️outcome/🔣️.json");
...
fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "create-rule/⛔️forbids-roof-over-roof: committed {side} JSON is not canonical");
    }
    ...
    assert_eq!(reencoded, original, "create-rule/⛔️forbids-roof-over-roof: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => {
            ...
            assert_ne!(snapshot, before(), "create-rule/⛔️forbids-roof-over-roof: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "create-rule/⛔️forbids-roof-over-roof: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("create-rule/⛔️forbids-roof-over-roof: unknown outcome status {other:?}"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "create-rule/⛔️forbids-roof-over-roof: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "create-rule/⛔️forbids-roof-over-roof: committed diff JSON is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "create-rule/⛔️forbids-roof-over-roof: committed diff did not carry before to after");
}
