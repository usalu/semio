//! 🧪️ `unpin-slot` fixture — `🔓️releases-the-slot-c-pin`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔓️unpin-slot/🔓️releases-the-slot-c-pin/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔓️unpin-slot/🔓️releases-the-slot-c-pin/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔓️unpin-slot/🔓️releases-the-slot-c-pin/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔓️unpin-slot/🔓️releases-the-slot-c-pin/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔓️unpin-slot/🔓️releases-the-slot-c-pin/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "unpin-slot/🔓️releases-the-slot-c-pin: applied state differs from the committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "unpin-slot/🔓️releases-the-slot-c-pin: inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "unpin-slot/🔓️releases-the-slot-c-pin: committed {side} JSON is not canonical");
    }
    ...
    assert_eq!(reencoded, original, "unpin-slot/🔓️releases-the-slot-c-pin: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "unpin-slot/🔓️releases-the-slot-c-pin: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_wfc2d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "unpin-slot/🔓️releases-the-slot-c-pin: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "unpin-slot/🔓️releases-the-slot-c-pin: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "unpin-slot/🔓️releases-the-slot-c-pin: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("unpin-slot/🔓️releases-the-slot-c-pin: unknown outcome status {other:?}"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "unpin-slot/🔓️releases-the-slot-c-pin: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "unpin-slot/🔓️releases-the-slot-c-pin: committed diff JSON is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "unpin-slot/🔓️releases-the-slot-c-pin: committed diff did not carry before to after");
}
