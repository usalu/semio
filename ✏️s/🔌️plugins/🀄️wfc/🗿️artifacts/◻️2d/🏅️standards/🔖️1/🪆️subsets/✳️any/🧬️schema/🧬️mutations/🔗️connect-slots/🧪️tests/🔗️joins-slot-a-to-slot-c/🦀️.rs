//! 🧪️ `connect-slots` fixture — `🔗️joins-slot-a-to-slot-c`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔗️connect-slots/🔗️joins-slot-a-to-slot-c/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔗️connect-slots/🔗️joins-slot-a-to-slot-c/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔗️connect-slots/🔗️joins-slot-a-to-slot-c/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔗️connect-slots/🔗️joins-slot-a-to-slot-c/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔗️connect-slots/🔗️joins-slot-a-to-slot-c/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "connect-slots/🔗️joins-slot-a-to-slot-c: applied state differs from the committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "connect-slots/🔗️joins-slot-a-to-slot-c: inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "connect-slots/🔗️joins-slot-a-to-slot-c: committed {side} JSON is not canonical");
    }
    ...
    assert_eq!(reencoded, original, "connect-slots/🔗️joins-slot-a-to-slot-c: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "connect-slots/🔗️joins-slot-a-to-slot-c: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_wfc2d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "connect-slots/🔗️joins-slot-a-to-slot-c: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "connect-slots/🔗️joins-slot-a-to-slot-c: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "connect-slots/🔗️joins-slot-a-to-slot-c: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("connect-slots/🔗️joins-slot-a-to-slot-c: unknown outcome status {other:?}"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "connect-slots/🔗️joins-slot-a-to-slot-c: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "connect-slots/🔗️joins-slot-a-to-slot-c: committed diff JSON is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "connect-slots/🔗️joins-slot-a-to-slot-c: committed diff did not carry before to after");
}
