@capability-wfc-wfc2d-1-any-editor-transient-mutate
@no-oracle-wfc-wfc2d-1-any-editor-transient-state-lane-semantics
@comparison-ordered-json-v1
@mutations-wfc-wfc2d-1-any-editor-transient
Feature: Apply every transient state-lane mutation of s.wfc.wfc2d's ✏️editor/🫧️transient to its committed vector
  `Wfc2dTransientMutation` is the transient lane of this subset's editor: it holds the last solve's slot assignments and whether it ended in a contradiction. It is this
  repository's own state record, so the case records the no-oracle decision `wfc-wfc2d-1-any-editor-transient-state-lane-semantics` in
  ...
  before-snapshot. Production dispatch is reached through `wfc2d_transient_mutation_report_json`, which runs
  ...
  Scenario Outline: Apply <id> and land on the committed after-snapshot, diff and outcome
    ...
    When <id> is applied to that vector's before-snapshot through wfc2d_transient_mutation_report_json
