//! 🧪️ `resize-slot` fixture — `📐️widens-slot-b`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐️resize-slot/📐️widens-slot-b/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐️resize-slot/📐️widens-slot-b/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐️resize-slot/📐️widens-slot-b/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐️resize-slot/📐️widens-slot-b/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📐️resize-slot/📐️widens-slot-b/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "resize-slot/📐️widens-slot-b: applied state differs from the committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "resize-slot/📐️widens-slot-b: inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "resize-slot/📐️widens-slot-b: committed {side} JSON is not canonical");
    }
    ...
    assert_eq!(reencoded, original, "resize-slot/📐️widens-slot-b: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "resize-slot/📐️widens-slot-b: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_wfc2d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "resize-slot/📐️widens-slot-b: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "resize-slot/📐️widens-slot-b: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "resize-slot/📐️widens-slot-b: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("resize-slot/📐️widens-slot-b: unknown outcome status {other:?}"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "resize-slot/📐️widens-slot-b: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "resize-slot/📐️widens-slot-b: committed diff JSON is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "resize-slot/📐️widens-slot-b: committed diff did not carry before to after");
}
