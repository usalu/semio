//! 🧪️ `disconnect-slots` fixture — `✂️severs-edge-ab`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️disconnect-slots/✂️severs-edge-ab/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️disconnect-slots/✂️severs-edge-ab/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️disconnect-slots/✂️severs-edge-ab/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️disconnect-slots/✂️severs-edge-ab/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️disconnect-slots/✂️severs-edge-ab/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "disconnect-slots/✂️severs-edge-ab: applied state differs from the committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "disconnect-slots/✂️severs-edge-ab: inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "disconnect-slots/✂️severs-edge-ab: committed {side} JSON is not canonical");
    }
    ...
    assert_eq!(reencoded, original, "disconnect-slots/✂️severs-edge-ab: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "disconnect-slots/✂️severs-edge-ab: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_wfc2d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "disconnect-slots/✂️severs-edge-ab: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "disconnect-slots/✂️severs-edge-ab: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "disconnect-slots/✂️severs-edge-ab: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("disconnect-slots/✂️severs-edge-ab: unknown outcome status {other:?}"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "disconnect-slots/✂️severs-edge-ab: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "disconnect-slots/✂️severs-edge-ab: committed diff JSON is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "disconnect-slots/✂️severs-edge-ab: committed diff did not carry before to after");
}
