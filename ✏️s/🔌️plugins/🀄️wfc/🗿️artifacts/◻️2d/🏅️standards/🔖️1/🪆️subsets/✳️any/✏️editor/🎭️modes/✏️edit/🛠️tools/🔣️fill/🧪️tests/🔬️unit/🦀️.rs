...
fn drive_once(job: &mut Wfc2dFillRunJob, operation: OperationId, generation: Generation, cancel: &semio_framework_job::CancelToken, sequence: &mut u64, fuel: u64) -> StepOutcome {
    ...
    semio_framework_job::drive_step(job, "wfc.wfc2d.fill.run.test", operation, generation, semio_framework_job::InteractiveStage::InteractiveStep, budget, cancel.clone(), semio_framework_job::default_now_us, sequence, &mut verdict)
}

fn tick_payload(outcome: StepOutcome) -> Option<Wfc2dFillTickPayload> {
    match outcome {
...
fn close(job: &mut Wfc2dFillRunJob) {
    job.begin_close();
...
#[test]
fn the_tool_declares_the_read_only_wfc2d_fill_run() {
    let tool = definition();
    ...
    assert_eq!(run.windows, vec![crate::editor::wfc2d::modes::edit::windows::preview::WFC_2D_PREVIEW_WINDOW.to_string()]);
...
fn the_language_agnostic_partial_vector_decodes_to_the_normative_shape() {
    ...
    assert_eq!(schema["$id"], "s.wfc.wfc2d.fill.tick.v1");
...
fn stepping_publishes_a_strictly_increasing_partial_before_the_finish() {
    ...
    let mut job = Wfc2dFillRunJob::new(identity(), snapshot.clone(), ToolRunJobPort::default());
...
fn aborting_mid_run_cancels_and_never_writes_set_solve() {
    ...
    let mut job = Wfc2dFillRunJob::new(identity(), snapshot, port.clone());
    let operation = semio_framework_job::allocate_operation_id();
    let generation = Generation(1);
    let cancel = semio_framework_job::root_cancel_token();
    let mut sequence = 0;
