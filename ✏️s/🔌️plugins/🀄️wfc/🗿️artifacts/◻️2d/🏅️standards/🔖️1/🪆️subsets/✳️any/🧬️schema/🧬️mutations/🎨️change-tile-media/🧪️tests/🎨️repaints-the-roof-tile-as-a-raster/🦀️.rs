//! 🧪️ `change-tile-media` fixture — `🎨️repaints-the-roof-tile-as-a-raster`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🎨️change-tile-media/🎨️repaints-the-roof-tile-as-a-raster/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🎨️change-tile-media/🎨️repaints-the-roof-tile-as-a-raster/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🎨️change-tile-media/🎨️repaints-the-roof-tile-as-a-raster/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🎨️change-tile-media/🎨️repaints-the-roof-tile-as-a-raster/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🎨️change-tile-media/🎨️repaints-the-roof-tile-as-a-raster/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "change-tile-media/🎨️repaints-the-roof-tile-as-a-raster: applied state differs from the committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "change-tile-media/🎨️repaints-the-roof-tile-as-a-raster: inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-tile-media/🎨️repaints-the-roof-tile-as-a-raster: committed {side} JSON is not canonical");
    }
    ...
    assert_eq!(reencoded, original, "change-tile-media/🎨️repaints-the-roof-tile-as-a-raster: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "change-tile-media/🎨️repaints-the-roof-tile-as-a-raster: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_wfc2d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "change-tile-media/🎨️repaints-the-roof-tile-as-a-raster: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "change-tile-media/🎨️repaints-the-roof-tile-as-a-raster: declared applied but the snapshot came back unchanged");
        }
        "rejected" => {
            assert_eq!(snapshot, before(), "change-tile-media/🎨️repaints-the-roof-tile-as-a-raster: a rejected mutation must leave the snapshot untouched");
        }
        other => panic!("change-tile-media/🎨️repaints-the-roof-tile-as-a-raster: unknown outcome status {other:?}"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-tile-media/🎨️repaints-the-roof-tile-as-a-raster: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "change-tile-media/🎨️repaints-the-roof-tile-as-a-raster: committed diff JSON is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "change-tile-media/🎨️repaints-the-roof-tile-as-a-raster: committed diff did not carry before to after");
}
