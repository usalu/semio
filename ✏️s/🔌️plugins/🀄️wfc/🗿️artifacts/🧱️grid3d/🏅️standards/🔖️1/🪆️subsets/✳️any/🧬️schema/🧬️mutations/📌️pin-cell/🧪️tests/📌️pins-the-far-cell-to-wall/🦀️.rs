//! 🧪️ `pin-cell` fixture — `📌️pins-the-far-cell-to-wall`.
//!
//! `pin-cell` inserts at the canonical cell-key position, so `1:0:0` lands after `0:0:0`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📌️pin-cell/📌️pins-the-far-cell-to-wall/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📌️pin-cell/📌️pins-the-far-cell-to-wall/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📌️pin-cell/📌️pins-the-far-cell-to-wall/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📌️pin-cell/📌️pins-the-far-cell-to-wall/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📌️pin-cell/📌️pins-the-far-cell-to-wall/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    apply_grid3d_mutation(&mut snapshot, &mutation()).expect("pin-cell applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "pin-cell/📌️pins-the-far-cell-to-wall: applied state differs from the committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "pin-cell/📌️pins-the-far-cell-to-wall: the inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "pin-cell/📌️pins-the-far-cell-to-wall: committed {side} JSON is not canonical");
    }
    ...
    assert_eq!(reencoded, original, "pin-cell/📌️pins-the-far-cell-to-wall: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "pin-cell/📌️pins-the-far-cell-to-wall: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_grid3d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "pin-cell/📌️pins-the-far-cell-to-wall: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "pin-cell/📌️pins-the-far-cell-to-wall: declared applied but the snapshot came back unchanged");
        }
        "rejected" => assert_eq!(snapshot, before(), "pin-cell/📌️pins-the-far-cell-to-wall: a rejected mutation must leave the snapshot untouched"),
        other => panic!("pin-cell/📌️pins-the-far-cell-to-wall: unknown outcome status {other:?}"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "pin-cell/📌️pins-the-far-cell-to-wall: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "pin-cell/📌️pins-the-far-cell-to-wall: committed diff JSON is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "pin-cell/📌️pins-the-far-cell-to-wall: committed diff did not carry before to after");
}
