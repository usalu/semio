//! 🧪️ `create-tile` fixture — `🧱️adds-the-roof-tile`.
//!
//! `create-tile` inserts at the CANONICAL SORTED position (`roof` lands between `floor` and `wall`), never at the end — which is what makes `delete-tile`'s inverse round-trip.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱️create-tile/🧱️adds-the-roof-tile/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱️create-tile/🧱️adds-the-roof-tile/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱️create-tile/🧱️adds-the-roof-tile/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱️create-tile/🧱️adds-the-roof-tile/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧱️create-tile/🧱️adds-the-roof-tile/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    apply_grid3d_mutation(&mut snapshot, &mutation()).expect("create-tile applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "create-tile/🧱️adds-the-roof-tile: applied state differs from the committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "create-tile/🧱️adds-the-roof-tile: the inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "create-tile/🧱️adds-the-roof-tile: committed {side} JSON is not canonical");
    }
    ...
    assert_eq!(reencoded, original, "create-tile/🧱️adds-the-roof-tile: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "create-tile/🧱️adds-the-roof-tile: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_grid3d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "create-tile/🧱️adds-the-roof-tile: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "create-tile/🧱️adds-the-roof-tile: declared applied but the snapshot came back unchanged");
        }
        "rejected" => assert_eq!(snapshot, before(), "create-tile/🧱️adds-the-roof-tile: a rejected mutation must leave the snapshot untouched"),
        other => panic!("create-tile/🧱️adds-the-roof-tile: unknown outcome status {other:?}"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "create-tile/🧱️adds-the-roof-tile: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "create-tile/🧱️adds-the-roof-tile: committed diff JSON is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "create-tile/🧱️adds-the-roof-tile: committed diff did not carry before to after");
}
