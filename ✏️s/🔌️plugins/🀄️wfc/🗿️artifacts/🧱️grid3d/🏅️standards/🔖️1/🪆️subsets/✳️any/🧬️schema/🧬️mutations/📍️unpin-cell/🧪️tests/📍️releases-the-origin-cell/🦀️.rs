//! 🧪️ `unpin-cell` fixture — `📍️releases-the-origin-cell`.
//!
//! `unpin-cell` removes by cell key; the inverse restores the pin at its own index.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📍️unpin-cell/📍️releases-the-origin-cell/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📍️unpin-cell/📍️releases-the-origin-cell/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📍️unpin-cell/📍️releases-the-origin-cell/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📍️unpin-cell/📍️releases-the-origin-cell/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📍️unpin-cell/📍️releases-the-origin-cell/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    apply_grid3d_mutation(&mut snapshot, &mutation()).expect("unpin-cell applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "unpin-cell/📍️releases-the-origin-cell: applied state differs from the committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "unpin-cell/📍️releases-the-origin-cell: the inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "unpin-cell/📍️releases-the-origin-cell: committed {side} JSON is not canonical");
    }
    ...
    assert_eq!(reencoded, original, "unpin-cell/📍️releases-the-origin-cell: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    let produced: Vec<(String, String)> = raised
        ...
        .collect();
    assert_eq!(produced, declared, "unpin-cell/📍️releases-the-origin-cell: raised diagnostics differ from the committed 🎯️outcome messages");
    let mut snapshot = before();
    let applied = apply_grid3d_mutation(&mut snapshot, &mutation()).is_ok();
    match status {
        "applied" => {
            assert!(applied, "unpin-cell/📍️releases-the-origin-cell: declared applied but the mutation was rejected");
            assert_ne!(snapshot, before(), "unpin-cell/📍️releases-the-origin-cell: declared applied but the snapshot came back unchanged");
        }
        "rejected" => assert_eq!(snapshot, before(), "unpin-cell/📍️releases-the-origin-cell: a rejected mutation must leave the snapshot untouched"),
        other => panic!("unpin-cell/📍️releases-the-origin-cell: unknown outcome status {other:?}"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "unpin-cell/📍️releases-the-origin-cell: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "unpin-cell/📍️releases-the-origin-cell: committed diff JSON is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "unpin-cell/📍️releases-the-origin-cell: committed diff did not carry before to after");
}
