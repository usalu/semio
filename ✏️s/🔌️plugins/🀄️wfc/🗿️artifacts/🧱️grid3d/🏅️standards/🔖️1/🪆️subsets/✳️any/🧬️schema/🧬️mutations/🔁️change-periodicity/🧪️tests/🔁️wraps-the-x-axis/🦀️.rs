//! 🧪️ `change-periodicity` fixture — `🔁️wraps-the-x-axis`.
//!
//! `change-periodicity` writes all three flags together, because they are one setting.
//!
...
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️change-periodicity/🔁️wraps-the-x-axis/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️change-periodicity/🔁️wraps-the-x-axis/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️change-periodicity/🔁️wraps-the-x-axis/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🔁️change-periodicity/🔁️wraps-the-x-axis/🎯️outcome/🔣️.json");
...
fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-periodicity/🔁️wraps-the-x-axis: committed {side} JSON is not canonical");
    }
    ...
    assert_eq!(reencoded, original, "change-periodicity/🔁️wraps-the-x-axis: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => {
            ...
            assert_ne!(snapshot, before(), "change-periodicity/🔁️wraps-the-x-axis: declared applied but the snapshot came back unchanged");
        }
        "rejected" => assert_eq!(snapshot, before(), "change-periodicity/🔁️wraps-the-x-axis: a rejected mutation must leave the snapshot untouched"),
        other => panic!("change-periodicity/🔁️wraps-the-x-axis: unknown outcome status {other:?}"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-periodicity/🔁️wraps-the-x-axis: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "change-periodicity/🔁️wraps-the-x-axis: committed diff JSON is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "change-periodicity/🔁️wraps-the-x-axis: committed diff did not carry before to after");
}
