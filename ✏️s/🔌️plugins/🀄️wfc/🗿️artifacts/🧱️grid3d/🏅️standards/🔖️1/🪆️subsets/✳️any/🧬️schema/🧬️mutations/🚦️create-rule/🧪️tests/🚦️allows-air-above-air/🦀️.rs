//! 🧪️ `create-rule` fixture — `🚦️allows-air-above-air`.
//!
//! `create-rule` inserts at the canonical sorted position — `r-top-air-air` lands BEFORE `r-top-floor-wall`.
//!
...
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚦️create-rule/🚦️allows-air-above-air/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚦️create-rule/🚦️allows-air-above-air/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚦️create-rule/🚦️allows-air-above-air/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🚦️create-rule/🚦️allows-air-above-air/🎯️outcome/🔣️.json");
...
fn committed_json_is_canonical() {
    for (side, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "create-rule/🚦️allows-air-above-air: committed {side} JSON is not canonical");
    }
    ...
    assert_eq!(reencoded, original, "create-rule/🚦️allows-air-above-air: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => {
            ...
            assert_ne!(snapshot, before(), "create-rule/🚦️allows-air-above-air: declared applied but the snapshot came back unchanged");
        }
        "rejected" => assert_eq!(snapshot, before(), "create-rule/🚦️allows-air-above-air: a rejected mutation must leave the snapshot untouched"),
        other => panic!("create-rule/🚦️allows-air-above-air: unknown outcome status {other:?}"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "create-rule/🚦️allows-air-above-air: produced diff differs from the committed 🔺️diff/🔣️.json");
}
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "create-rule/🚦️allows-air-above-air: committed diff JSON is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "create-rule/🚦️allows-air-above-air: committed diff did not carry before to after");
}
