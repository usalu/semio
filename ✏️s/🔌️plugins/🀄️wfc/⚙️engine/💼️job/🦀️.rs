...
impl<T: Topology + Clone> WfcJob<T> {
    ...
    pub fn domain_masks(&self) -> Vec<PatternSet> {
    ...
    fn choose_one(&mut self) {
        ...
        if self.state.choice_cursor.is_none() {
            let cursor = match self.config.sampler {
                ...
                WfcSampler::WeightedRoulette => ChoiceCursor {
                    phase: ChoicePhase::Weigh,
                    pattern: 0,
                    total: 0.0,
                    target: 0.0,
                    running: 0.0,
    ...
    pub fn take_completed_commit(&mut self) -> Option<WfcCommit> {
...
impl<T: Topology + Clone> WfcRestore<T> {
    pub fn new(operation: Operation, model: CompiledModel, topology: T, config: WfcJobConfig, initial_domains: Option<Vec<PatternSet>>, fixed: Vec<(NodeId, PatternId)>, bytes: Vec<u8>) -> Result<Self, String> {
    ...
    pub fn take_job(&mut self) -> Option<WfcJob<T>> {
        self.restored.take()
    }
