...
pub struct GraphModelBuild {
    phase: GraphModelPhase,
    raw_weights: Vec<f64>,
...
impl GraphModelBuild {
    pub fn new(raw_weights: Vec<f64>, allowed_pairs: std::collections::BTreeSet<(u32, u32)>) -> Result<Self, ModelError> {
    ...
    pub fn step(&mut self) -> Result<Option<CompiledModel>, ModelError> {
        let pattern_count = self.raw_weights.len();
        let word_count = pattern_count.div_ceil(64);
        match self.phase {
            GraphModelPhase::Weights => {
                if let Some(&value) = self.raw_weights.get(self.cursor) {
                    ...
                    self.cursor = 0;
                    self.phase = GraphModelPhase::Allowed;
                }
            }
            GraphModelPhase::Allowed => {
                if self.cursor < pattern_count {
                    ...
                    self.cursor = 0;
                    {
                        self.phase = GraphModelPhase::Supporters;
                    }
                    #[cfg(not(test))]
                    {
                        self.phase = GraphModelPhase::Complete;
                    }
                }
            }
            GraphModelPhase::Supporters => {
                if self.cursor < pattern_count {
