...
use crate::constraints_card::CardinalityConstraint;
use crate::constraints_conn::{ConnectivityConstraint, ReachabilityConstraint};
...
impl AdjacencyView {
    pub fn new(neighbors: Vec<Vec<NodeId>>, regions: Vec<RegionId>) -> Self {
...
pub fn build_adjacency_view<T: crate::topology::Topology>(topo: &T) -> AdjacencyView {
...
// #region 🔖️Dispatch
impl Constraints {
    pub fn initialize(&self, domains: &DomainStore, weights: &WeightTable, adjacency: &AdjacencyView) -> Result<Vec<(NodeId, PatternSet)>, ConstraintError> {
        match self {
    ...
    pub fn validate_complete(&self, assignment: &[PatternId], adjacency: &AdjacencyView) -> Result<(), String> {
        match self {
