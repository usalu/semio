...
//! `pub`: not public API, free to evolve, and never boxed as `dyn` — internal-iteration
...
// #region 🧵️IncrementalCompiler
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum GraphTopologyPhase {
    Outgoing,
    Incoming,
    Regions,
    Complete,
...
pub struct GraphTopologyBuild {
    node_count: usize,
    phase: GraphTopologyPhase,
    node: usize,
...
impl GraphTopologyBuild {
    pub fn new(node_count: usize) -> Self {
        Self {
            node_count,
            phase: GraphTopologyPhase::Outgoing,
            node: 0,
    ...
    pub fn add_arc(&mut self, from: NodeId, to: NodeId, relation: RelationId) -> Result<(), crate::error::TopologyError> {
    ...
    fn emit_outgoing(&mut self) {
        ...
        if self.node == self.node_count {
            self.node = 0;
            self.phase = GraphTopologyPhase::Incoming;
            return;
        }
    ...
    fn emit_incoming(&mut self) {
        ...
        if self.node == self.node_count {
            self.node = 0;
            self.phase = GraphTopologyPhase::Regions;
            return;
        }
    ...
    pub fn step(&mut self) -> Option<GraphTopology> {
        match self.phase {
            GraphTopologyPhase::Outgoing => self.emit_outgoing(),
            GraphTopologyPhase::Incoming => self.emit_incoming(),
            GraphTopologyPhase::Regions => {
                if self.node < self.node_count {
                    self.regions.push(0);
                    self.node += 1;
                } else {
                    self.phase = GraphTopologyPhase::Complete;
                }
            }
            GraphTopologyPhase::Complete => {
...
// #endregion 🧵️IncrementalCompiler
...
/// in this crate).
pub fn from_graph_view(view: &impl semio_framework_graph::GraphView, rel_of: impl Fn(semio_framework_graph::EdgeRef) -> RelationId) -> Result<GraphTopology, crate::error::TopologyError> {
    use crate::error::TopologyError;
