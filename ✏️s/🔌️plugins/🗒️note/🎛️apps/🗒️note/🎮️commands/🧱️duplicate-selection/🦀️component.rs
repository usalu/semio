...
pub fn handle(_payload: &DuplicateSelection, doc: &ArtifactView<'_, NoteSnapshot>, _cfg: &ConfigView<'_, NoteConfig>, ctx: &mut crate::apps::note::NoteDispatchCtx) -> Result<Emit<NoteMutation, NoteConfigMutation>, Fault> {
    Ok(duplicate_blocks(doc.snapshot, &ctx.selected_block_ids))
}
