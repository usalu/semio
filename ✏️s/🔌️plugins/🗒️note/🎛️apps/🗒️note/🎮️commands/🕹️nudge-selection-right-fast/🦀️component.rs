...
pub fn handle(_payload: &NudgeSelectionRightFast, doc: &ArtifactView<'_, NoteSnapshot>, cfg: &ConfigView<'_, NoteConfig>) -> Result<Emit<NoteMutation, NoteConfigMutation>, Fault> {
    Ok(nudge(doc.snapshot, cfg.snapshot, NUDGE_STEP_FAST, 0.0))
}
