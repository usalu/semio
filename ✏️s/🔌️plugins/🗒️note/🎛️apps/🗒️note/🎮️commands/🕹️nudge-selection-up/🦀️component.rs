...
pub fn handle(_payload: &NudgeSelectionUp, doc: &ArtifactView<'_, NoteSnapshot>, _cfg: &ConfigView<'_, NoteConfig>, ctx: &mut crate::apps::note::NoteDispatchCtx) -> Result<Emit<NoteMutation, NoteConfigMutation>, Fault> {
...
mod tests {
    use super::*;
    use crate::apps::note::testkit::{dispatch, note_app_with_registry, select_blocks};
    ...
    fn nudge_direction_actions_move_selection_without_args() {
        for (command, expected_dx, expected_dy) in [
            ...
            (NoteCommand::NudgeSelectionDown(crate::apps::note::commands::nudge_selection_down::NudgeSelectionDown {}), 0.0, 1.0),
            (NoteCommand::NudgeSelectionLeft(crate::apps::note::commands::nudge_selection_left::NudgeSelectionLeft {}), -1.0, 0.0),
            (NoteCommand::NudgeSelectionRight(crate::apps::note::commands::nudge_selection_right::NudgeSelectionRight {}), 1.0, 0.0),
        ] {
    ...
    fn nudge_fast_actions_use_ten_pixel_step() {
        ...
        let new_id = block_id(&app.snapshot().expect("snapshot").blocks[0]).to_string();
        select_blocks(&mut app, &[&new_id]);
        dispatch(&mut app, NoteCommand::NudgeSelectionRightFast(crate::apps::note::commands::nudge_selection_right_fast::NudgeSelectionRightFast {}));
        let projection = app.snapshot().expect("snapshot");
        let (x, y, ..) = block_bounds(&projection.blocks[0]);
