...
pub mod set_active_utility {
    ...
    pub fn handle(payload: &SetActiveUtility, _doc: &DocumentView<'_, NoteDocument>, _cfg: &ConfigView<'_, NoteConfig>) -> Result<Emit<NoteOperation, NoteConfigOperation>, Fault> {
        Ok(Emit::config(vec![NoteConfigOperation::SetActiveUtility { utility_id: payload.utility_id.clone() }]))
    }
}
