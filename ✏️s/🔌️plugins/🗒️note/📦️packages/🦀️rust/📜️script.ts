#!/usr/bin/env bun
/** 📝️ `@semio-tech/note-plugin` router: `bun ./📜️script.ts <test|describe>`. */
...
class TestScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    ...
    await runCargoTestBudgeted(["semio-s-plugin-note"], this.repoRoot, rest);
  }
}
...
class DescribeScript extends BundleScript {
  run(): void {
    process.exit(describePluginComponent(this.repoRoot, "semio-s-plugin-note", join(this.root, "..", "..")));
  }
}
