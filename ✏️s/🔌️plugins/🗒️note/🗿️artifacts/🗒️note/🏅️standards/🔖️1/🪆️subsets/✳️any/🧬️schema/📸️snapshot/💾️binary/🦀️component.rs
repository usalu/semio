...
mod tests {
    ...
    fn pack_round_trips_and_agrees_with_dsl() {
        let document = crate::artifacts::note::schema::snapshot::text::parse_dsl(crate::artifacts::note::schema::snapshot::text::SEMIO_NOTE_EXAMPLE_TEXT).expect("parse semio example");
...
mod semio_protocol_conformance {
    ...
    fn verify_protocol_bytes_against_encoded_pack() {
        let document = crate::artifacts::note::schema::snapshot::text::parse_dsl(crate::artifacts::note::schema::snapshot::text::SEMIO_NOTE_EXAMPLE_TEXT).expect("parse fixture");
        let bytes = encode(&document);
