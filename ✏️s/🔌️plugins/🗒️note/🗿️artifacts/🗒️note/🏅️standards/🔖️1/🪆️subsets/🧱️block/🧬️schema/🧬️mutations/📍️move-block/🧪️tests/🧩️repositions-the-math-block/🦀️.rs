//! 🧪️ `move-block` fixture — `🧩️repositions-the-math-block`.
//!
...
async fn applies_to_committed_after() {
    let applied = apply_note_mutation(&before(), &mutation()).expect("move-block applies to its committed before-snapshot");
    assert_eq!(applied, expected_after(), "move-block/repositions-the-math-block: applied state differs from committed after-snapshot");
}

/// ↩️ The inverse re-issues `move-block` with the base block's own prior `(x, y)` from `block_bounds`.
#[semio_framework_async_macros::async_test]
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "move-block/repositions-the-math-block: inverse did not restore the before-snapshot");
}
...
/// 🎯️ The block exists, the target is finite, and it genuinely differs from the current position, so none of the `mutation.target-missing` error, `mutation.invariant` fatal or `mutation.no-op` warn fires.
#[semio_framework_async_macros::async_test]
...
/// 🔺️ One `blocks.patched` entry addressing exactly one id — no sibling appears in the delta, so no sibling can move.
///
...
/// 📍 An ABSOLUTE reposition of a single block — the new coordinates replace the old ones outright and the size is untouched.
#[semio_framework_async_macros::async_test]
async fn single_block_takes_absolute_coordinates_without_resizing() {
    let base = before();
