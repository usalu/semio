...
mutations (`rename-note`), in Python, serving as this case's differential oracle. Duplicated from
`../../../📜️document/🧪️tests/📜️mutate-note-1-document/` (shard G2, ticket
`26/09/02/SEPARATE-ARTIFACT-STANDARD-SUBSET-IMPLEMENTATIONS-AND-FIXTURE-TEST-EVERY-MUTATION`) to close
`unregistered-mutation-vocabulary` at this `✳️any` owner, reusing the already-manifested
`note-1-document-mutate` capability.
...
* ``../../🧬️schema/📸️snapshot/🔣️.json`` — the document's members and the six block
  kinds.
