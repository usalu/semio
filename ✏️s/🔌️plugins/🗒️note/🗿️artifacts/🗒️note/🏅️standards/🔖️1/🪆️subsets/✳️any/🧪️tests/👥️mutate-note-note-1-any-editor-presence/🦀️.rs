//! 👥️ `s.note.note` ✏️editor/👥️presence state-lane mutation case — Rust adapter.
//!
//! Recorded no-oracle decision `note-note-1-any-editor-presence-state-lane-semantics`: the runner dispatches no oracle role, so every law is asserted inside
//! the subject handlers through `semio_s_plugin_stdio_test_oracle::law::vector` over the report of this crate's
//! production bridge `note_presence_mutation_report_json`. The oracle handlers answer with the committed after- and before-snapshots read
