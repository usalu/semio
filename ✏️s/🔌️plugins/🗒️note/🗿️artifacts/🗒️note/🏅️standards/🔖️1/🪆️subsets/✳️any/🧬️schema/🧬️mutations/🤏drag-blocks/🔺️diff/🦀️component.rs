...
pub fn diff(payload: &DragBlocks, base: &NoteSnapshot) -> NoteDiff {
    ...
    for id in &payload.ids {
        let Some(block) = crate::artifacts::note::engine::find_block(&base.blocks, id) else { continue };
        let mut moved = block.clone();
        crate::artifacts::note::engine::offset_block_tree(&mut moved, payload.dx, payload.dy);
