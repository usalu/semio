@capability-note-note-1-any-editor-presence-mutate
@no-oracle-note-note-1-any-editor-presence-state-lane-semantics
@comparison-ordered-json-v1
@mutations-note-note-1-any-editor-presence
Feature: Apply every presence state-lane mutation of s.note.note's ✏️editor/👥️presence to its committed vector
  `NotePresenceMutation` is the presence lane of this subset's editor: it holds the canvas camera (x, y, zoom) this collaborator broadcasts to peers. It is this
  repository's own state record, so the case records the no-oracle decision `note-note-1-any-editor-presence-state-lane-semantics` in
  ...
  before-snapshot. Production dispatch is reached through `note_presence_mutation_report_json`, which runs
  ...
  Scenario Outline: Apply <id> and land on the committed after-snapshot, diff and outcome
    ...
    When <id> is applied to that vector's before-snapshot through note_presence_mutation_report_json
