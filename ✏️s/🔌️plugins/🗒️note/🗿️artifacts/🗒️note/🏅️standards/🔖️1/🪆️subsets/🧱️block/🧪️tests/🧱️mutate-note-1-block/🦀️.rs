//! 🦀️ Note document block mutation case — Rust adapter. Relocated out of the artifact-level
...
const KINDS: &[&str] = &["create-block", "delete-block", "delete-blocks", "duplicate-block", "duplicate-blocks", "move-block-to-container", "drag-blocks", "rename-block", "change-block-visible", "change-block-locked", "move-block", "resize-block", "change-block-font-size"];
