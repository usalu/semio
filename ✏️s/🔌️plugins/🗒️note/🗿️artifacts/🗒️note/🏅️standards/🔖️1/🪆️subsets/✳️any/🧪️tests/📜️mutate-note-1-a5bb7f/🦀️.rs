//! 🦀️ Note `any`-owned document-vocabulary case — Rust adapter. Duplicated from
//! `../../../📜️document/🧪️tests/📜️mutate-note-1-document/` (shard G2, this ticket) to close
//! `unregistered-mutation-vocabulary` at the `✳️any/🧬️schema/🧬️mutations` +
//! `✳️any/🚪️io/🧬️mutations` owner, reusing the already-manifested `note-1-document-mutate`
//! capability — same mechanism E3 proved on `sequence`, F4 on `drawing`/`equation`/`fem2d`/`fem3d`.
//!
...
//#region 🔖️Kinds
#[cfg(feature = "sut")]
/// 🏷️ `📜️document`'s own slice of `NoteMutation::KINDS`
/// (`../../../📜️document/🧬️schema/🧬️mutations/🦀️.rs`) — duplicated, not imported, because the
/// oracle-only build must not link the subject crate.
const KINDS: &[&str] = &["rename-note"];
