//! 🕹️ 🕹️ Note play app commands command — `nudge-selection-up-fast`.
...
pub fn handle(_payload: &NudgeSelectionUpFast, doc: &ArtifactView<'_, NoteSnapshot>, _cfg: &ConfigView<'_, NoteConfig>, ctx: &mut crate::editor::note::NoteDispatchCtx) -> Result<Emit<NoteMutation, NoteConfigMutation>, Fault> {
    Ok(nudge(doc.snapshot, &ctx.selected_block_ids, 0.0, -NUDGE_STEP_FAST))
}
