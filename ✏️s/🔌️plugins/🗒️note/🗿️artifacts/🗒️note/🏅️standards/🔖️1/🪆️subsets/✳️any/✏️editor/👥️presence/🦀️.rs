...
impl protocol::MutationDiff<NotePresence> for NotePresence {
    async fn apply(&self, _base: &NotePresence) -> protocol::MutationApplyResult<NotePresence> {
        Ok({ self.clone() })
    }
...
impl Mutation<NotePresence> for NotePresenceMutation {
    ...
    async fn diff(&self, _base: &NotePresence) -> protocol::MutationOutcome<NotePresence> {
        match self {
            Self::Snapshot { presence } => protocol::MutationOutcome::new(presence.clone()),
        }
    }

    async fn inverse(&self, base: &NotePresence) -> Vec<Self> {
