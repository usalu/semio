...
impl Serializer<NoteSnapshot> for NoteIntoDwg {
    ...
    async fn serialize(from: &NoteSnapshot) -> IoResult<IoPayload> {
        ...
        let raw = semio_framework_os::svg_to_polylines(&svg).and_then(|paths| semio_s_plugin_stdio::artifacts::dwg::standards::v_ac1024::subsets::any::io::polylines_to_dwg_bytes(paths.iter().map(|path| (path.layer.as_str(), path.vertices.as_slice(), path.closed)))).map_err(|error| IoError { message: format!("NoteIntoDwg: svg_to_dwg: {error}"), diagnostics: Vec::new() })?;
