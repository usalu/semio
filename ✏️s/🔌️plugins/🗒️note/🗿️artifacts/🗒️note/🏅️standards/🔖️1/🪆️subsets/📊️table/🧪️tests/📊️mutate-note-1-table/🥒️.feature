@capability-note-1-table-mutate
@oracle-note-python-independent
@comparison-ordered-json-v1
@mutations-note-1-table
Feature: Apply every typed note document table mutation twice — once in Rust, once in Python — and require the same answer
  ...
  This subset owns a table block's rows and columns: `insert-table-row`/`insert-table-column` append BLANK cells, which is what makes their inverses exact against the committed trailing-blank vectors, and `remove-table-row`/`remove-table-column` drop the trailing row or column.
  ...
  Scenario Outline: Applying <id> to its committed before-snapshot yields the committed after-snapshot
    ...
    Examples:
      | id                  | vector                                                      |
      | insert-table-row    | ⬇️insert-table-row/🧪️tests/⬇️appends-a-blank-third-row        |
      | remove-table-row    | ⬆️remove-table-row/🧪️tests/⬆️drops-the-trailing-blank-row     |
      | insert-table-column | ➡️insert-table-column/🧪️tests/➡️appends-the-lettered-column-c |
      | remove-table-column | ⬅️remove-table-column/🧪️tests/⬅️drops-the-trailing-column-b   |
