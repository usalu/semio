//! 🦀️ Note document canvas mutation case — Rust adapter. Relocated out of the artifact-level
...
const KINDS: &[&str] = &["change-grid-visible", "change-grid-spacing", "change-grid-subdivisions", "change-grid-opacity", "change-snap-enabled", "change-snap-grid-spacing"];
