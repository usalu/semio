...
mod tests {
    ...
    fn note_document_text_round_trips_store_with_applied_operation() {
        let envelope = store::create_document_envelope::<NoteDocument, NoteMutation>("note.document", "doc-text-test", crate::artifacts::note::engine::empty_note_document(), None);
    ...
    fn command_envelope_round_trip_holds_for_an_applied_operation() {
        ...
        let envelope = store::create_document_envelope::<NoteDocument, NoteMutation>("note.document", "command-envelope-demo", crate::artifacts::note::engine::empty_note_document(), None);
        let mut doc_store = store::DocumentStore::new(envelope);
        doc_store.dispatch(store::DocumentCommand::Apply { mutations: vec![NoteMutation::SetGridSpacing { spacing: Some(48.0) }], description: None }).expect("apply");
        let edit: &Edit<NoteMutation> = doc_store.envelope().vcs.edits.last().expect("dispatch must have recorded an edit");
        store::test_support::assert_command_envelope_round_trip::<NoteDocument, NoteMutation>(edit, &DocumentId(doc_store.envelope().id.clone()), &SchemaId(doc_store.envelope().schema.clone()));
    }
