//! 🔧️ Note artifact — operation enum + laws (constitutional: op).
...
impl protocol::OpText for NoteOperation {
...
impl protocol::OpBinary for NoteOperation {
...
/// this file's own `Operation::diff`/tests.
pub fn apply_note_operation(projection: &NoteDocument, operation: &NoteOperation) -> NoteDocument {
...
mod tests {
    ...
    fn op_text_round_trips_every_variant() {
        ...
        store::test_support::assert_op_text_binary_equivalence(&NoteOperation::SetDocument { document: crate::artifacts::note::engine::empty_note_document() });
    }
