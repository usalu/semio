...
pub mod request_completions {
    ...
    pub fn handle(_payload: &RequestCompletions, _doc: &DocumentView<'_, WriterProjection>, cfg: &ConfigView<'_, WriterConfig>) -> Result<Emit<WriterOperation, WriterConfigOperation>, Fault> {
        let config = cfg.projection;
        Ok(Emit::config(vec![WriterConfigOperation::SetRevision { value: config.revision + 1 }]))
    }
}
...
pub mod lint_document {
    ...
    pub fn handle(_payload: &LintDocument, _doc: &DocumentView<'_, WriterProjection>, cfg: &ConfigView<'_, WriterConfig>) -> Result<Emit<WriterOperation, WriterConfigOperation>, Fault> {
        let config = cfg.projection;
        Ok(Emit::config(vec![WriterConfigOperation::SetLintSignal { value: config.lint_signal + 1 }, WriterConfigOperation::SetRevision { value: config.revision + 1 }]))
    }
}
