use super::SetCamera;
use crate::editor::writer::testkit::{dispatch, main_window_view, new_app};
...
async fn set_camera_command_writes_config_not_operations() {
    ...
    let payload = semio_framework_plugin::testkit::project_and_retire_fixture_tree(node).expect("render JSON");
    let scene = semio_framework_plugin::testkit::decode_fixture_scene::<semio_framework_plugin::TextEditorScene>(&payload).expect("Writer scene");
