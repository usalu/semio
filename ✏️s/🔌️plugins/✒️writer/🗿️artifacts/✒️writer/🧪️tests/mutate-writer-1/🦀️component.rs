...
//! (`../../🏅️standards/🔖️1/🪆️subsets/✳️any/🧪️oracle/🔣️component.json`): `s.writer.writer` is a
...
mod subject {
    ...
    fn mutation_at(ctx: &Context, vector: &str, kind: &str) -> Result<WriterMutation, String> {
        decode_writer_mutation_json(&text_at(ctx, vector, "🦠️mutation/🔣️component.json")?).map_err(|error| format!("the committed mutation payload for {kind:?} must decode: {error}"))
    }
