#!/usr/bin/env bun
/** ✒️ `@semio-tech/writer-plugin` router: `bun ./📜️script.ts test`. */
...
class TestScript extends BundleScript {
  run(_segments: string[]): void {
    runCargoTestBudgeted(["semio-s-plugin-writer"], this.repoRoot);
  }
}
...
class DescribeScript extends BundleScript {
  run(): void {
    process.exit(describePluginComponent(this.repoRoot, "semio-s-plugin-writer", join(this.root, "..", "..")));
  }
}
