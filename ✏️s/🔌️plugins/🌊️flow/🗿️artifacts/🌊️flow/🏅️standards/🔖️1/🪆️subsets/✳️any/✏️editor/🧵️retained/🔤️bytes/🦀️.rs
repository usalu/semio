...
//#region 📑️Copy
pub(super) fn edit_id_length(sequence: u64, metadata: bool) -> usize {
    b"flow-retained-".len() + if sequence == 0 { 1 } else { sequence.ilog10() as usize + 1 } + if metadata { 2 } else { 0 }
}
...
pub(super) fn edit_id_byte(sequence: u64, index: usize, metadata: bool) -> u8 {
    ...
    if index < digits {
        return b'0' + ((sequence / 10u64.pow((digits - index - 1) as u32)) % 10) as u8;
    }
    if metadata {
        b"#0"[index - digits]
    } else {
        unreachable!("Flow edit id byte escaped extent")
    }
}

#[derive(Default)]
pub(super) struct TextCopy {
    bytes: Vec<u8>,
    reserved: bool,
    complete: bool,
}
...
impl TextCopy {
    ...
    pub(super) fn advance_ascii(&mut self, length: usize, byte_at: impl Fn(usize) -> u8, maximum_bytes: usize) -> Result<Option<usize>, String> {
        ...
        if !self.reserved {
            self.bytes.try_reserve_exact(length).map_err(|_| "Flow generated text allocation failed")?;
            self.reserved = true;
            return Ok(Some(0));
        }
        if self.complete {
        ...
        for _ in 0..count {
            let byte = byte_at(self.bytes.len());
            if !byte.is_ascii() {
                return Err("Flow generated identifier is not ASCII".into());
            }
    ...
    pub(super) fn advance(&mut self, left: &str, right: &str, maximum_bytes: usize) -> (Option<bool>, usize) {
        ...
        while bytes < maximum_bytes && self.index < left.len() {
            if let Some(value) = self.left.take() {
                bytes += 1;
                if value != right.as_bytes()[self.index] {
                    self.result = Some(false);
                    return (self.result, bytes);
                }
                self.index += 1;
            } else {
                self.left = Some(left.as_bytes()[self.index]);
                bytes += 1;
            }
        }
        if self.index == left.len() {
