...
//!
//! ✅️ This is an NO-OP case with an EMPTY diff. `move-widgets` is PLURAL by taxonomy design (one
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📍️move-widgets/🟰️re-applies-the-current-layout-to-both-widgets/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📍️move-widgets/🟰️re-applies-the-current-layout-to-both-widgets/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📍️move-widgets/🟰️re-applies-the-current-layout-to-both-widgets/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📍️move-widgets/🟰️re-applies-the-current-layout-to-both-widgets/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📍️move-widgets/🟰️re-applies-the-current-layout-to-both-widgets/🎯️outcome/🔣️.json");
...
/// 🎯️ The declared outcome is `no-op` WITH a `warn`-level `mutation.no-op` — a drag that ends
...
async fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(serde_json::Value::as_str), Some("no-op"), "move-widgets/re-applies-the-current-layout-to-both-widgets declares a no-op outcome");
    let mut snapshot = before();
    apply_flow_mutation(&mut snapshot, &mutation()).expect("move-widgets/re-applies-the-current-layout-to-both-widgets: declared no-op but the mutation was rejected");
