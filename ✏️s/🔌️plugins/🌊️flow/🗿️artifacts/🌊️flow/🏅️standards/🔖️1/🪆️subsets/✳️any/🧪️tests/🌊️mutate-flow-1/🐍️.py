...
def apply_duplicate_widget(graph, payload):
    ...
    counterpart at all."""
    after = copy.deepcopy(graph)
    source = after["widgets"][widget_index(after, payload["sourceId"])]
    copied = dict(source, id=payload["newId"])
    after["widgets"].append(copied)
    after["synapses"].append({"id": payload["synapseId"], "from": payload["sourceId"], "to": payload["newId"], "fromPort": payload["fromPort"], "toPort": payload["toPort"]})
    return after
...
def inverse_apply(kind, base_graph, payload, mutated_graph):
    ...
    if kind == "duplicate-widget":
        after["widgets"] = [w for w in after["widgets"] if w["id"] != payload["newId"]]
        after["synapses"] = [s for s in after["synapses"] if s["id"] != payload["synapseId"]]
        return after
