...
pub fn node_graph_edit_result(doc: &ArtifactView<'_, FlowSnapshot>, config: &FlowMainWindowConfig, session: &FlowEvalSession, operations: &[FlowNodeGraphEditOp], selected_nodes: &[String]) -> Result<Emit<FlowMutation, NoConfigMutation>, Fault> {
    crate::editor::flow::commands::node_graph_edit::node_graph_edit_result(doc, config, session, operations, selected_nodes)
}
