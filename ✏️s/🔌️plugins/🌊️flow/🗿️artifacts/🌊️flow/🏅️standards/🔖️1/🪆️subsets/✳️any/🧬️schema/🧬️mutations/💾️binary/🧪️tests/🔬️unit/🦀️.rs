...
async fn flow_document_text_round_trips_store_with_applied_operation() {
    let envelope = store::create_document_envelope::<FlowSnapshot, FlowMutation>("flow.host_snapshot", "doc-text-test", FlowSnapshot::default(), None);
...
async fn duplicate_widget_composite_round_trips_through_op_codecs_and_a_real_store_dispatch() {
    ...
    let envelope = store::create_document_envelope::<FlowSnapshot, FlowMutation>("flow.host_snapshot", "doc-composite-test", FlowSnapshot::default(), None);
    let mut doc_store = store::ArtifactStore::new(envelope).await.expect("valid artifact store fixture");
    ...
    let live = doc_store.snapshot().expect("snapshot").to_host_snapshot();
