...
use super::super::{
    bytes::{edit_id_byte, edit_id_length, TextCopy},
    Owner, Retirement,
};
...
impl store::ArtifactStoreOneItemPreparationFactory<FlowSnapshot, FlowMutation> for PreparationFactory {
    fn preflight(&self, mutation: &FlowMutation, description: Option<&str>, lane: store::HistoryLane) -> Result<store::ArtifactStoreOneItemFootprint, String> {
        if lane != store::HistoryLane::Document || description.is_some_and(|value| value.len() > store::ARTIFACT_STORE_ONE_ITEM_ID_BYTES) {
            return Err("Flow artifact preparation rejected lane or description".into());
        }
    ...
    fn begin(
        &self,
        request: store::ArtifactStoreOneItemPreparationRequest<FlowSnapshot, FlowMutation>,
    ) -> Result<Box<dyn store::ArtifactStoreOneItemPreparation<FlowSnapshot, FlowMutation>>, store::ArtifactStoreOneItemPreparationRequest<FlowSnapshot, FlowMutation>> {
        if request.lane != store::HistoryLane::Document
            || request.operation != request.authority.operation()
            || request.generation != request.authority.generation()
            || request.base_revision != request.authority.base_revision()
            || request.authority.actor().len() > store::ARTIFACT_STORE_ONE_ITEM_ID_BYTES
            || !Recipe::supported(&request.mutation)
        {
            return Err(request);
        }
        Ok(Box::new(Preparation {
            state: ManuallyDrop::new(PreparationState {
                base: Some(request.base),
                mutation: Some(Arc::new(request.mutation)),
...
struct PreparationState {
    base: Option<store::SnapshotRead<FlowSnapshot>>,
    ...
    scene_root: Option<Arc<FlowWorkingScene>>,
    inverse: Option<Vec<FlowMutation>>,
    post: Option<FlowSnapshot>,
    hash: Option<SceneHash>,
    ...
    external_retirement: Option<Box<dyn store::ErasedSnapshotRetirement>>,
...
struct Preparation {
    state: ManuallyDrop<PreparationState>,
}

fn progress(state: &mut PreparationState, bytes: usize) -> Advance {
    state.checkpoint.cursor += 1;
    state.checkpoint.completed_items += 1;
    state.checkpoint.completed_bytes += bytes as u64;
...
impl store::ArtifactStoreOneItemPreparation<FlowSnapshot, FlowMutation> for Preparation {
    fn advance(&mut self, grant: Grant) -> Result<Advance, String> {
        ...
        if state.failed {
            return Err("Flow canonical mutation encoding failed".into());
        }
        if !grant.permits_one() || state.closing || state.cancelled {
        ...
        if let Some(sealer) = state.sealer.as_mut() {
            ...
            let bytes = (sealer.checkpoint().completed_bytes - before) as usize;
            let result = progress(state, bytes);
            if matches!(step, Advance::Prepared(_)) {
                state.checkpoint.digest = state.sealer.as_ref().unwrap().prepared().unwrap().edit_digest();
                return Ok(Advance::Prepared(state.checkpoint));
            }
        ...
        match state.phase {
            0 => {
                ...
                state.hash = Some(SceneHash::new(scene));
                state.phase = 1;
            }
            1 => {
                let hash = state.hash.as_mut().unwrap();
                let bytes = hash.advance(grant)?;
                state.phase_bytes += bytes;
                if state.phase_bytes > FLOW_STORE_MAX_TEXT_BYTES + crate::FLOW_CONTENT_ID_DOMAIN.len() {
                    return Err("Flow artifact base exceeds admitted canonical byte envelope".into());
                }
                if hash.complete() {
                    let (root, digest) = hash.take().unwrap();
            ...
            4 => {
                ...
                state.phase_bytes += bytes;
                if state.phase_bytes > FLOW_STORE_MAX_TEXT_BYTES {
                    return Err("Flow artifact mutation exceeds admitted canonical byte envelope".into());
                }
                ...
                match reader.close_step(grant)? {
                    Close::Complete => {
                        if !reader.terminal_is_empty() {
                            return Err("Flow mutation reader closed with owners".into());
                        }
                        state.mutation_reader = None;
                        state.phase = 6;
                    }
                    Close::Pending { released_bytes, .. } => return Ok(progress(state, released_bytes)),
                    Close::Blocked => return Ok(Advance::Blocked),
            ...
            6 => {
                state.recipe = Some(Recipe::new(state.scene_root.take().unwrap(), Arc::clone(state.mutation.as_ref().unwrap())));
                state.phase = 7;
            }
            7 => {
                let recipe = state.recipe.as_mut().unwrap();
                if recipe.complete() {
                    let (scene, inverse) = recipe.take().unwrap();
                    state.scene = Some(scene);
                    state.inverse = Some(inverse);
                    recipe.begin_close();
                    state.phase = 8;
                } else {
                    return Ok(match recipe.advance(grant)? {
                        Some(bytes) => progress(state, bytes),
            ...
            8 => {
                ...
                match recipe.close_step(grant)? {
                    Close::Complete => {
                        if !recipe.terminal_is_empty() {
                            return Err("Flow recipe closed with owners".into());
                        }
                        state.recipe = None;
                        state.phase = 9;
                    }
            ...
            9 => {
                state.hash = Some(SceneHash::new(Arc::new(state.scene.take().unwrap())));
                state.phase = 10;
            }
            10 => {
                let hash = state.hash.as_mut().unwrap();
                ...
                if hash.complete() {
                    ...
                    hash.begin_close();
                    state.phase = 11;
                    if matches!(&**state.mutation.as_ref().unwrap(), FlowMutation::ReplaceWidget(_)) && state.source_digest == state.digest {
                        return Err("Flow widget replacement is a no-op".into());
                    }
                }
                return Ok(progress(state, bytes));
            }
                        ...
                        copy.advance_ascii(
                            ...
                            |position| {
                                if position < PREFIX.len() {
                                    ...
                                    let position = position - PREFIX.len();
                                    let byte = digest[position / 2];
                                    b"0123456789abcdef"[if position % 2 == 0 { (byte >> 4) as usize } else { (byte & 15) as usize }]
                                }
                            },
                            grant.maximum_bytes,
                        )?
            ...
            110 => {
                let target = store::os_io::ArtifactRef {
                    artifact_id: state.texts[4].take().unwrap(),
            ...
            111 => {
                let mutation = match Arc::try_unwrap(state.mutation.take().unwrap()) {
                    Ok(mutation) => mutation,
                    Err(root) => {
                        state.mutation = Some(root);
                        return Err("Flow mutation still has an active traversal owner".into());
                    }
                };
                ...
                let edit = protocol::Edit {
                    ...
                    inverse: state.inverse.take().unwrap(),
                    mutation_meta: vec![protocol::MutationMeta {
                        mutation_id: state.texts[9].take().map(protocol::MutationId),
                        dependencies: Vec::new(),
                        base_version: authority.base_applied_edit_count() as u64,
                        author_id: state.texts[2].take().map(protocol::ActorId),
    ...
    fn prepared(&self) -> Option<&store::ArtifactStoreOneItemPrepared<FlowSnapshot, FlowMutation>> {
        self.state.sealer.as_ref().and_then(|sealer| sealer.prepared())
    }
    fn take_prepared(&mut self) -> Option<store::ArtifactStoreOneItemPrepared<FlowSnapshot, FlowMutation>> {
        self.state.sealer.as_mut().and_then(|sealer| sealer.take_prepared())
    }
    fn cancel(&mut self) {
        self.state.cancelled = true;
        if let Some(sealer) = self.state.sealer.as_mut() {
            sealer.cancel();
        }
    }
    fn begin_close(&mut self) {
        ...
        if let Some(hash) = self.state.hash.as_mut() {
            hash.begin_close();
        }
        if let Some(reader) = self.state.mutation_reader.as_mut() {
            reader.begin_close();
        }
        if let Some(recipe) = self.state.recipe.as_mut() {
            recipe.begin_close();
        }
    }
    ...
    fn close_step(&mut self, grant: Grant) -> Result<Close, String> {
        let state = &mut *self.state;
        if !state.closing || !grant.permits_one() {
        ...
        if let Some(owner) = state.external_retirement.as_mut() {
            ...
            if step == Close::Complete {
                if !owner.terminal_is_empty() {
                    return Err("Flow external retirement is nonterminal".into());
                }
                state.external_retirement = None;
        ...
        macro_rules! close_cursor {
            ($field:ident) => {
                if let Some(owner) = state.$field.as_mut() {
                    let step = owner.close_step(grant)?;
                    if step == Close::Complete {
                        if !owner.terminal_is_empty() {
                            return Err("Flow preparation cursor closed with live owners".into());
                        }
                        state.$field = None;
                        return Ok(Close::Pending { released_items: 1, released_bytes: 0 });
                    }
        ...
        close_cursor!(sealer);
        close_cursor!(hash);
        close_cursor!(mutation_reader);
        close_cursor!(recipe);
        if let Some(text) = state.text.take() {
            text.retire(&mut state.retirement);
        } else if let Some(text) = state.texts.iter_mut().find_map(Option::take).or_else(|| state.description.take()) {
            state.retirement.push(Owner::Bytes(text.into_bytes()));
        } else if let Some(inverse) = state.inverse.take() {
            state.retirement.push(Owner::Mutations(inverse));
        } else if let Some(post) = state.post.take() {
            state.external_retirement = Some(store::ArtifactOwnedValueRetirementFactory::retire_owned(&SnapshotRetirementFactory, post));
        } else if let Some(scene) = state.scene.take() {
            state.retirement.push(Owner::Scene(scene));
        } else if let Some(scene) = state.scene_root.take() {
            if let Some(scene) = Arc::into_inner(scene) {
                state.retirement.push(Owner::Scene(scene));
            }
        ...
        } else if let Some(base) = state.base.take() {
            if !base.return_to_registry() {
                return Err("Flow preparation failed to return exact base root".into());
            }
        } else if let Some(authority) = state.authority.take() {
            state.external_retirement = Some(authority.retire());
        } else {
    ...
    fn terminal_is_empty(&self) -> bool {
        ...
        state.closing
            ...
            && state.scene.is_none()
            && state.scene_root.is_none()
            && state.inverse.is_none()
            && state.post.is_none()
            && state.hash.is_none()
            && state.mutation_reader.is_none()
            && state.recipe.is_none()
            && state.text.is_none()
            && state.texts.iter().all(Option::is_none)
            && state.sealer.is_none()
            && state.external_retirement.is_none()
            && state.retirement.is_empty()
            && state.source_digest.is_none()
...
impl Drop for Preparation {
    fn drop(&mut self) {
        if store::ArtifactStoreOneItemPreparation::terminal_is_empty(self) {
            unsafe {
                ManuallyDrop::drop(&mut self.state);
            }
        } else if !std::thread::panicking() {
            panic!("Flow artifact preparation must close before drop");
        }
    }
}
