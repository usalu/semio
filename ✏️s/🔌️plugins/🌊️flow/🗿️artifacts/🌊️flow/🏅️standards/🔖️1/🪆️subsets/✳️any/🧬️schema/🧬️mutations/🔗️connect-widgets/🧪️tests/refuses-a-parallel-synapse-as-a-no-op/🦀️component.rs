...
//! `s.stdio.semio.flow` CHILD by a `DefaultHasher` digest of the child content.
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "connect-widgets/refuses-a-parallel-synapse-as-a-no-op: produced diff differs from the committed 🔺️diff/🔣️component.json");
}
