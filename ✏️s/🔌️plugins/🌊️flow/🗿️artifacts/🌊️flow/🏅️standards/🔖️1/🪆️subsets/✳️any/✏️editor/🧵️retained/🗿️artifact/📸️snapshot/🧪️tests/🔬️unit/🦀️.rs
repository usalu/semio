mod tests {
    use crate::retirement::{SceneRetirementFactory, SnapshotRetirementFactory};
    ...
    fn flow_parent_projection_and_child_identity_match_neutral_corpus() {
        ...
        for row in fixture["cases"].as_array().unwrap() {
            ...
            for projection in [<EditorApp<crate::editor::flow::FlowPlayApp> as ArtifactApp>::child_restore_projection(&snapshot).unwrap(), <ViewerApp<crate::viewer::flow::FlowViewer> as ArtifactApp>::child_restore_projection(&snapshot).unwrap()] {
            ...
            for _ in 0..100_000 {
                if retirement.close_step(1, 64).unwrap() == store::SnapshotRetirementStep::Complete {
                    break;
                }
            }
    ...
    fn child_typed_handoff_preserves_mismatched_owner_then_retires_exact_scene() {
        ...
        let scene = child.take_local_owner::<FlowWorkingScene>().unwrap().unwrap();
        assert!(child.take_local_owner::<FlowWorkingScene>().unwrap().is_none());
