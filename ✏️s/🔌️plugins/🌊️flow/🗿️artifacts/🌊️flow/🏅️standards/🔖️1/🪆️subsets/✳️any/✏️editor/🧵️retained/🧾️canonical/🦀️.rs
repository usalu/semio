...
fn text(value: &str) -> Value<'_> {
    Value::Scalar(Node::String(value))
}
fn number<'a>(value: f64) -> Value<'a> {
    Value::Scalar(Node::F64(value))
}
fn index<'a>(value: usize) -> Value<'a> {
    Value::Scalar(Node::U64(value as u64))
}
fn boolean<'a>(value: bool) -> Value<'a> {
    Value::Scalar(Node::Bool(value))
}
fn null<'a>() -> Value<'a> {
    Value::Scalar(Node::Null)
}
fn object<'a, const N: usize>(mut fields: [(&'a str, Value<'a>); N]) -> Value<'a> {
    fields.sort_unstable_by(|left, right| left.0.cmp(right.0));
    Value::Object(Object::new(fields.into_iter()))
}
fn array<'a>(values: impl Iterator<Item = Value<'a>> + Send + 'a) -> Value<'a> {
    Value::Array(Array::new(values))
}
fn strings(values: &[String]) -> Value<'_> {
    array(values.iter().map(|value| text(value)))
}
fn set(values: &flow::OrderedSet) -> Value<'_> {
    array(values.iter().map(|value| text(value)))
}
fn layout(value: &semio_framework_artifact_flow_flow::WidgetLayout) -> Value<'_> {
    object([("x", number(value.x)), ("y", number(value.y))])
}
fn optional_layout(value: &Option<semio_framework_artifact_flow_flow::WidgetLayout>) -> Value<'_> {
    value.as_ref().map(layout).unwrap_or_else(null)
}
//#endregion 🔣️Scalars

//#region 🧠️Neural
fn dictionary(value: &neural::Dictionary) -> Value<'_> {
    Value::Object(Object::new(value.iter().map(|(key, value)| (key.as_str(), neural_value(value)))))
}
...
fn neural_value(value: &neural::Value) -> Value<'_> {
    match value {
        ...
        neural::Value::Atom(value) => match value {
            neural::Atom::Null => null(),
            neural::Atom::Boolean(value) => boolean(*value),
            neural::Atom::Integer(value) => Value::Scalar(Node::I64(*value)),
            neural::Atom::Decimal(value) => number(*value),
...
fn neuron(value: &neural::Neuron) -> Value<'_> {
    object([("id", text(&value.id)), ("kind", text(&value.kind)), ("params", dictionary(&value.params)), ("tree", value.tree.as_ref().map(|value| tree(value)).unwrap_or_else(null))])
}
...
fn preview(value: &FlowPreviewGui) -> Value<'_> {
    object([
        ...
        ("preview", dictionary(&value.preview)),
        ("expanded", set(&value.expanded)),
        ("layout", optional_layout(&value.layout)),
    ])
}
...
fn widget(value: &Widget) -> Value<'_> {
    match value {
        Widget::Neuron { id, neuron_kind, params, input_ports, output_ports, preview } => {
            object([("kind", text("neuron")), ("id", text(id)), ("neuronKind", text(neuron_kind)), ("params", dictionary(params)), ("inputPorts", strings(input_ports)), ("outputPorts", strings(output_ports)), ("preview", boolean(*preview))])
        }
        Widget::InputSlider { id, label, value, min, max, step } => object([("kind", text("inputSlider")), ("id", text(id)), ("label", text(label)), ("value", number(*value)), ("min", number(*min)), ("max", number(*max)), ("step", number(*step))]),
...
fn layout_entry(value: &FlowLayoutEntry) -> Value<'_> {
    object([("id", text(&value.id)), ("layout", optional_layout(&value.layout))])
}
...
impl ArtifactCanonicalJson for FlowMutation {
    fn canonical_json_borrowed_root(&self) -> Result<Option<Value<'_>>, String> {
        Ok(Some(match self {
            ...
            Self::ConnectWidgets(value) => {
                object([("mutation", text("connectWidgets")), ("index", index(value.index)), ("id", text(&value.id)), ("from", text(&value.from)), ("fromPort", text(&value.from_port)), ("to", text(&value.to)), ("toPort", text(&value.to_port))])
            }
            ...
            Self::UpdateSynapseEndpoints(value) => {
                object([("mutation", text("updateSynapseEndpoints")), ("id", text(&value.id)), ("from", text(&value.from)), ("fromPort", text(&value.from_port)), ("to", text(&value.to)), ("toPort", text(&value.to_port))])
            }
            Self::MoveWidgets(value) => object([("mutation", text("moveWidgets")), ("entries", array(value.entries.iter().map(layout_entry)))]),
            Self::DuplicateWidget(value) => {
                object([("mutation", text("duplicateWidget")), ("sourceId", text(&value.source_id)), ("newId", text(&value.new_id)), ("synapseId", text(&value.synapse_id)), ("fromPort", text(&value.from_port)), ("toPort", text(&value.to_port))])
            }
        }))
    }
}
...
impl ArtifactCanonicalJson for crate::FlowWorkingScene {
    fn canonical_json_borrowed_root(&self) -> Result<Option<Value<'_>>, String> {
        Ok(Some(object([
            ...
            ("synapses", array(self.synapses.iter().map(|value| object([("id", text(&value.id)), ("from", text(&value.from)), ("to", text(&value.to)), ("fromPort", text(&value.from_port)), ("toPort", text(&value.to_port))])))),
