...
use crate::apps::flow::{flow_graph_selection_domains, host_operations, sync_host_selection_domains, FLOW_INTERACTION_GRAPH};
...
mod tests {
    use super::*;
    use crate::apps::flow::testkit::{dispatch, dispatch_with_registry, flow_app_with_registry, select_graph};
    ...
    fn context_menu_at_is_a_no_operation() {
        use crate::apps::flow::commands::context_menu_at;
        let mut app = flow_app_with_registry();
        let result = dispatch(&mut app, FlowCommand::ContextMenuAt(context_menu_at::ContextMenuAt { id: String::new() }));
        assert!(result.mutations.is_empty());
        assert!(!crate::apps::flow::testkit::render(&mut app, FLOW_PLAY_BODY_MAIN).contains(r#""selection":["#));
    }
}
//#endregion 🧪️Tests
