...
impl protocol::MutationDiff<FlowPresence> for FlowPresence {
    fn apply(&self, _base: &FlowPresence) -> FlowPresence {
        self.clone()
    }
...
impl store::DocumentDsl for FlowPresence {
...
impl DocumentPack for FlowPresence {
...
impl Mutation<FlowPresence> for FlowPresenceMutation {
    ...
    fn diff(&self, _base: &FlowPresence) -> FlowPresence {
        match self {
            Self::Snapshot { presence } => presence.clone(),
        }
    }

    fn inverse(&self, base: &FlowPresence) -> Vec<Self> {
