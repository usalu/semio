...
mod tests {
    ...
    async fn evaluate_json_text() {
        ...
        let out_json = evaluate_json(&neural_engine::ColdOwner::new(module_registry()), "core.text", &serde_json::to_string(&input).unwrap());
...
mod extension_guest {
    ...
    fn bundle_identity_matches_catalogue_fixture() {
        ...
        assert_eq!(manifest["extensionId"], fixture["primitive"]["pluginId"]);
        ...
        for contribution in &bundle.manifest.topic_contributions {
            ...
            assert_eq!(payload["extensionId"], fixture["primitive"]["flowId"]);
        }
    }
