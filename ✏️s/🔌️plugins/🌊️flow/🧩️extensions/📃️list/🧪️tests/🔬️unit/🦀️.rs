...
async fn empty_creates_list() {
    ...
    let out = reg.dispatch_cold("list.empty", Dictionary::new()).unwrap();
...
async fn get_reads_index() {
    ...
    let out = reg.dispatch_cold("list.get", input).unwrap();
...
async fn append_adds_next_index() {
    ...
    let out = reg.dispatch_cold("list.append", input).unwrap();
...
async fn heterogeneous_list_input_rejected_at_evaluate() {
    ...
    let err = reg.dispatch_cold("list.size", input).unwrap_err();
