...
mod extension_guest {
    ...
    fn bundle() -> ExtensionBundle {
        ...
        let flow_topic = flow_extension_topic_contribution(FLOW_APP_ID, EXTENSION_ID, EXTENSION_LABEL, "dictionary", &manifest_json);
        let procedural3d_topic = flow_extension_topic_contribution(PROCEDURAL3D_APP_ID, EXTENSION_ID, EXTENSION_LABEL, "dictionary", &manifest_json);
        let bundle = ExtensionBundle::new("flow-extension-dictionary", EXTENSION_LABEL, "0.1.0").extends("flow");
    ...
    fn bundle_identity_matches_catalogue_fixture() {
        let fixture = pack::json::parse(include_str!("../🧪️fixture/🔣️s.json")).unwrap();
        let bundle = bundle();
        assert_eq!(Some(bundle.manifest.extension_id.as_str()), fixture.get("dictionary").and_then(|entry| entry.get("pluginId")).and_then(pack::json::Value::as_str));
        assert_eq!(bundle.manifest.topic_contributions.len(), 2);
        for contribution in &bundle.manifest.topic_contributions {
            assert_eq!(contribution.payload.get("extensionId").and_then(|value| value.as_str()), fixture.get("dictionary").and_then(|entry| entry.get("flowId")).and_then(pack::json::Value::as_str));
        }
    }
