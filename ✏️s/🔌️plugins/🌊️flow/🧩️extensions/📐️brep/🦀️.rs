...
mod tests {
    ...
    async fn box_of(reg: &mut Registry, size: f64) -> Dictionary {
        channel_payload(&reg.dispatch("brep.prim3d.box", &Dictionary::new().insert("width", Value::Dictionary(number_dictionary(size))).insert("depth", Value::Dictionary(number_dictionary(size))).insert("height", Value::Dictionary(number_dictionary(size)))).unwrap(), "solid").await
    }
    ...
    async fn surface_family_plane_point_stays_in_plane_and_normal_matches() {
        ...
        let evaluated = channel_payload(&reg.dispatch("brep.eval.surfPoint", &Dictionary::new().insert("surface", Value::Dictionary(surface.clone())).insert("u", Value::Dictionary(number_dictionary(1.0))).insert("v", Value::Dictionary(number_dictionary(-2.0)))).unwrap(), "point").await;
        ...
        let normal = channel_payload(&reg.dispatch("brep.eval.surfNormal", &Dictionary::new().insert("surface", Value::Dictionary(surface)).insert("u", Value::Dictionary(number_dictionary(0.0))).insert("v", Value::Dictionary(number_dictionary(0.0)))).unwrap(), "normal").await;
    ...
    async fn rotate_about_rotates_around_the_given_origin_not_the_world_origin() {
        ...
        let rotated = channel_payload(
            &reg.dispatch(
                "brep.xform.rotateAbout",
                &Dictionary::new().insert("geometry", Value::Dictionary(solid)).insert("origin", Value::Dictionary(point(5.0, 0.0, 0.0).await)).insert("axis", Value::Dictionary(vector(0.0, 0.0, 1.0).await)).insert("angle", Value::Dictionary(number_dictionary(std::f64::consts::PI))),
            )
            .unwrap(),
            "geometry",
        ).await;
