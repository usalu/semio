...
class TestScript extends BundleScript {
  async run(segments: string[]): Promise<void> {
    ...
    await runCargoTestBudgeted(["semio-s-plugin-animate"], this.repoRoot, rest);
  }
}
...
class DescribeScript extends BundleScript {
  run(): void {
    process.exit(describePluginComponent(this.repoRoot, "semio-s-plugin-animate", join(this.root, "..", "..")));
  }
}
