...
pub mod updater {
        ...
        F: Fn() -> Box<dyn Sobject> + Send + Sync + 'static,
    {
        ...
        add_updater(
            ...
            Updater::new(name, move |obj, _dt| {
                let fresh = factory();
                if let Some(v) = obj.as_any_mut().downcast_mut::<crate::apps::present::engine::scene::sobject::VSobject>() {
                    if let Some(fv) = fresh.as_any().downcast_ref::<crate::apps::present::engine::scene::sobject::VSobject>() {
    ...
    mod tests {
        ...
        fn run_updaters_recurses_into_group_children() {
            ...
            let mut group = crate::apps::present::engine::scene::sobject::Group::new(vec![Box::new(child)]);
