// #region 🧲️Header
/** @emoji 📽️ React + reveal.js renderer for `@semio-tech/animate-present-core` declarative decks. */
// #endregion 🧲️Header
...
/** @emoji 📐️ Uniform react-pdf scale so the page covers the disposition frame without distortion. */
...
/** @emoji 📐️ Uniform react-pdf scale for one-axis cover scroll (fit width or height, overflow the other). */
...
/** @emoji 📐️ Measures the react-pdf viewport from the disposition frame or enlarged slide content box. */
...
if (import.meta.vitest) {
	...
	const globalsCssSource = readFileSync(
		join(dirname(fileURLToPath(import.meta.url)), "🎨️globals.css"),
		"utf8",
	);
	...
	describe("PresentationDeck", () => {
		...
		it("animates figure crop and target-ghost frames only during many-to-one auto-animate", async () => {
			...
			const { fileURLToPath } = await import("node:url");
			const css = readFileSync(resolve(dirname(fileURLToPath(import.meta.url)), "🎨️globals.css"), "utf8");
		...
		it("rests morph-source ghosts with opacity only so reveal can measure FLIP targets", async () => {
			...
			const { dirname, resolve } = await import("node:path");
			const { fileURLToPath } = await import("node:url");
			const cssPath = resolve(dirname(fileURLToPath(import.meta.url)), "🎨️globals.css");
	...
	describe("presentation interaction geometry", () => {
		...
		it("shows catalogue full figure at rest with source ghosts and focus tiles visible", async () => {
			const { collectPresentationSlides } = await import("@semio-tech/animate-present-core");
		...
		it("places catalogue-labels target ghosts at inline label frames", async () => {
			const { collectPresentationSlides } = await import("@semio-tech/animate-present-core");
			const { deck, inlineColumnLabelPosition } =
				await import("@semio-tech/mit-bestand-praesentation-projektetage-spec");
			const focusRef = collectPresentationSlides(deck).find((slide) => slide.slide === "Bauteilarten");
