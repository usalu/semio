        ...
        match command {
            ...
            PresentCommand::ExportVideoFromDeck { output_dir, scene_json } => {
                ...
                match export_video_from_deck(&scene, output_dir) {
                    Ok(bundles) => {
                        Ok(Emit::effect(HostEffect::DownloadMediaExport { filename: "animate-video-export.ops".into(), mime_type: "text/plain".into(), data: serde_json::to_string_pretty(&bundles).unwrap_or_else(|_| "[]".into()), encoding: None })
                    }
