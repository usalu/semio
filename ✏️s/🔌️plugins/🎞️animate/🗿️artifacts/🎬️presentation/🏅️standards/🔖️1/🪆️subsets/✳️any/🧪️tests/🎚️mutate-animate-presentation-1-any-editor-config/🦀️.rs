//! 🎚️ `s.animate.presentation` ✏️editor/🎚️config state-lane mutation case — Rust adapter.
//!
//! Recorded no-oracle decision `animate-presentation-1-any-editor-config-state-lane-semantics`: the runner dispatches no oracle role, so every law is asserted inside
//! the subject handlers through `semio_s_plugin_stdio_test_oracle::law::vector` over the report of this crate's
//! production bridge `presentation_config_mutation_report_json`. The oracle handlers answer with the committed after- and before-snapshots read
...
mod subject {
    ...
    use semio_s_artifact_animate_presentation::editor::animate::config::presentation_config_mutation_report_json;

    fn report(committed: &Vector) -> Result<String, String> {
        presentation_config_mutation_report_json(committed.before, committed.mutation, committed.after)
    }
