//! 🦀️ Animate PRESENTATION exhaustive mutation case — Rust adapter. Ticket
//! `26/08/23/END-TO-END-TESTING-REFACTOR`, wave 12 (the gate hole that exempted every vocabulary
//! with no catalog).
//!
//! Recorded no-oracle decision `presentation-figure-deck-mutation-semantics`
//! (`../../🏅️standards/🔖️1/🪆️subsets/✳️any/🔣️oracle.json`): `s.animate.presentation` is a
...
//#region 🔖️Vocabulary
/// 🏷️ Mirrors `PresentationMutation::KINDS`
...
mod subject {
    ...
    fn base(ctx: &Context) -> Result<PresentationSnapshot, String> {
        ...
        Ok(presentation_snapshot_with_tiles(&source, &committed_tiles(ctx)?))
    }
    ...
    /// `encode_presentation_projection_json` and reparsed with the platform's dependency-free reader.
    /// Comparison belongs to the owner, never to an adapter, so nothing about what this document
    /// means is decided in this file.
    fn projection(snapshot: &PresentationSnapshot) -> Result<Json, String> {
        parse_json(&encode_presentation_projection_json(snapshot))
    }
    ...
    pub fn mutate(ctx: &Context) -> Result<Outcome, String> {
        ...
        let base = base(ctx)?;
        let mutated = apply_presentation_mutation(&base, &mutation(&spec)?).map_err(|error| format!("mutate-{kind}: the mutation did not apply: {error}"))?;
