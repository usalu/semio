//! 🧪️ `delete-tile` fixture — `🚫️rejects-deleting-a-missing-tile`.
//!
...
//! back as the DEFAULT source with NO tiles (`presentation_working_scene_for_handle` falls soft, never
...
fn a_missing_tile_is_an_error_target_missing() {
    ...
    assert_eq!(produced.diff(), &PresentationDiff::default(), "a rejecting delete-tile must carry the identity diff, never a half-built presentation handle");
...
fn the_committed_diff_is_declared_absent() {
    ...
    let produced = <PresentationMutation as protocol::Mutation<PresentationSnapshot>>::diff(&mutation(), &before());
    assert_eq!(produced.diff(), &PresentationDiff::default(), "delete-tile/rejects-deleting-a-missing-tile: a rejection must produce no delta at all");
}
