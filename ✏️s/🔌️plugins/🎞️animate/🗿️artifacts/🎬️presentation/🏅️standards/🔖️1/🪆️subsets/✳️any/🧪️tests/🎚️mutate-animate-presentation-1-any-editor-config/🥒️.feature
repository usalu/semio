@capability-animate-presentation-1-any-editor-config-mutate
@no-oracle-animate-presentation-1-any-editor-config-state-lane-semantics
@comparison-ordered-json-v1
@mutations-animate-presentation-1-any-editor-config
Feature: Apply every config state-lane mutation of s.animate.presentation's ✏️editor/🎚️config to its committed vector
  `PresentationConfigMutation` is the config lane of this subset's editor: it holds the engagement input text of the presentation editor. It is this
  repository's own state record, so the case records the no-oracle decision `animate-presentation-1-any-editor-config-state-lane-semantics` in
  ...
  before-snapshot. Production dispatch is reached through `presentation_config_mutation_report_json`, which runs
  ...
  Scenario Outline: Apply <id> and land on the committed after-snapshot, diff and outcome
    ...
    When <id> is applied to that vector's before-snapshot through presentation_config_mutation_report_json
