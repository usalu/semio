...
import { parseFigureTileDrafts, parseFigureTileSource, parsePresentationArtifact, type ArtifactChild, type FigureTileDraft, type FigureTileSource, type PresentationArtifact } from "../🟦️.ts";
...
export interface PresentationDiff {
  ...
  /** @state artifact */ tiles: FigureTileDraft[] | null;
...
export function parsePresentationDiff(value: unknown, at = "$"): PresentationDiff {
  ...
  if (Object.keys(row).some((key) => !["artifact", "schema", "source", "tiles", "presentation"].includes(key))) throw new Error(`${at}: fields do not match PresentationDiff`);
  ...
  return {
    ...
    source: row.source == null ? null : parseFigureTileSource(row.source, `${at}.source`),
    tiles: row.tiles == null ? null : parseFigureTileDrafts(row.tiles, `${at}.tiles`),
