//! 🚪️ presentation -> png — foreign `Serializer<PresentationSnapshot>` (ticket
...
pub struct PresentationIntoPng;

impl Serializer<PresentationSnapshot> for PresentationIntoPng {
    ...
    async fn serialize(from: &PresentationSnapshot) -> IoResult<IoPayload> {
        ...
        let wire = <PngSnapshot as store::ArtifactPack>::decode_pack(&bytes).map_err(|error| IoError { message: format!("PresentationIntoPng: {error}"), diagnostics: Vec::new() })?;
