/** 🖼️ Animate editor — tile-editor window: typed twin of `🦀️.rs`'s view-model. Mirrors the
 * canvas-2d scene `render(deck: &PresentationSnapshot)` boundary — one background source-figure layer plus
 ...
 * inputs (a bare `PresentationSnapshot`, no runtime/config/utility state carried alongside it). */
export interface AnimateTileEditorViewModel {
  windowKindId: "tile-editor";
  bodyKey: "animate.presentation.play.main";
  surfaceId: "animate.presentation.play";
  layers: AnimateTileCanvasLayer[];
}

export const ANIMATE_TILE_EDITOR_WINDOW_KIND_ID = "tile-editor" as const;
export const ANIMATE_TILE_EDITOR_BODY_KEY = "animate.presentation.play.main" as const;
export const ANIMATE_TILE_EDITOR_SURFACE_ID = "animate.presentation.play" as const;
