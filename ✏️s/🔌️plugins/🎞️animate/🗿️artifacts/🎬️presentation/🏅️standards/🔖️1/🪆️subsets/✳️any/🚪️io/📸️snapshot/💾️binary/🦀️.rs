//! 📦️ Animate presentation artifact — binary pack codec for `📸️snapshot`. Moved from
...
fn encode_presentation_snapshot_binary(s: &PresentationSnapshot) -> Vec<u8> {
...
fn decode_presentation_snapshot_binary(bytes: &[u8]) -> Result<PresentationSnapshot, String> {
    const PACK_BINARY_FORMAT: u8 = 1;
...
impl store::ArtifactPack for PresentationSnapshot {
