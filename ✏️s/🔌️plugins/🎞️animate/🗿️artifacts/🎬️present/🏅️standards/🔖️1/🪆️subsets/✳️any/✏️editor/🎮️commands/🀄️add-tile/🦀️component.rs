...
mod tests {
    ...
    async fn seed_2x2(app: &mut crate::editor::animate::testkit::PresentApp) {
        dispatch(app, PresentCommand::SeedGrid(crate::editor::animate::commands::seed_grid::SeedGrid { rows: 2, columns: 2 })).await;
    }

    #[semio_framework_async_macros::async_test]
    async fn add_delete_and_rename_tile_round_trip_through_operations() {
        let mut app = present_app().await;
        app.dispatch_typed(PresentCommand::AddTile(AddTile { crop: None }), &meta("local")).await.expect("add tile");
        let tile_id = crate::artifacts::present::present_working_scene(&app.snapshot().await.expect("projection")).1[0].id.clone();
        app.dispatch_typed(PresentCommand::RenameTiles(rename_tiles::RenameTiles { ids: vec![tile_id.clone()], value: "Hero".into() }), &meta("local")).await.expect("rename");
        assert_eq!(crate::artifacts::present::present_working_scene(&app.snapshot().await.expect("projection")).1[0].name, "Hero");
        app.dispatch_typed(PresentCommand::DeleteTile(delete_tile::DeleteTile { id: tile_id }), &meta("local")).await.expect("delete");
    ...
    async fn patch_tile_crop_clamps_and_is_reversible() {
        ...
        app.dispatch_typed(PresentCommand::AddTile(AddTile { crop: None }), &meta("local")).await.expect("add tile");
        let tile_id = crate::artifacts::present::present_working_scene(&app.snapshot().await.expect("projection")).1[0].id.clone();
        app.dispatch_typed(PresentCommand::PatchTileCrops(patch_tile_crops::PatchTileCrops { ids: vec![tile_id], field: "width".into(), value: 0.5 }), &meta("local")).await.expect("patch crop");
        assert_eq!(crate::artifacts::present::present_working_scene(&app.snapshot().await.expect("projection")).1[0].crop.width, 0.5);
        app.handle_action("undo", None, &meta("local")).await.expect("undo");
        assert_eq!(crate::artifacts::present::present_working_scene(&app.snapshot().await.expect("projection")).1[0].crop.width, 0.2);
    }
    ...
    async fn delete_selection_removes_only_the_selected_tile() {
        ...
        let first_id = crate::artifacts::present::present_working_scene(&app.snapshot().await.expect("projection")).1[0].id.clone();
        ...
        app.handle_action(INTERACTION_SELECT_ACTION_ID, Some(&serde_json::json!({ "domainId": PRESENT_INTERACTION_DOMAIN, "targets": targets, "merge": "replace", "method": "pick" })), &meta("local")).await.expect("select");
        app.dispatch_typed(PresentCommand::DeleteSelection(delete_selection::DeleteSelection {}), &meta("local")).await.expect("delete selection");
        assert_eq!(crate::artifacts::present::present_working_scene(&app.snapshot().await.expect("projection")).1.len(), 3, "only the selected tile is removed");
    }
    ...
    async fn delete_tile_with_unknown_id_is_a_no_op() {
        let mut app = present_app().await;
        seed_2x2(&mut app).await;
        app.dispatch_typed(PresentCommand::DeleteTile(delete_tile::DeleteTile { id: "does-not-exist".into() }), &meta("local")).await.expect("delete missing");
        assert_eq!(crate::artifacts::present::present_working_scene(&app.snapshot().await.expect("projection")).1.len(), 4, "unknown ids are filtered out before dispatch");
    }
    ...
    async fn rename_tiles_with_blank_value_leaves_name_unchanged() {
        ...
        app.dispatch_typed(PresentCommand::AddTile(AddTile { crop: None }), &meta("local")).await.expect("add tile");
        let tile_id = crate::artifacts::present::present_working_scene(&app.snapshot().await.expect("projection")).1[0].id.clone();
        let before = crate::artifacts::present::present_working_scene(&app.snapshot().await.expect("projection")).1[0].name.clone();
        app.dispatch_typed(PresentCommand::RenameTiles(rename_tiles::RenameTiles { ids: vec![tile_id], value: "   ".into() }), &meta("local")).await.expect("rename blank");
        assert_eq!(crate::artifacts::present::present_working_scene(&app.snapshot().await.expect("projection")).1[0].name, before, "whitespace-only rename is rejected");
    }
    ...
    async fn rename_tiles_with_unknown_ids_is_a_no_op() {
        let mut app = present_app().await;
        app.dispatch_typed(PresentCommand::AddTile(AddTile { crop: None }), &meta("local")).await.expect("add tile");
        app.dispatch_typed(PresentCommand::RenameTiles(rename_tiles::RenameTiles { ids: vec!["nope".into()], value: "Hero".into() }), &meta("local")).await.expect("rename unknown");
        assert_ne!(crate::artifacts::present::present_working_scene(&app.snapshot().await.expect("projection")).1[0].name, "Hero");
    }
    ...
    async fn patch_tile_crops_covers_all_fields_across_multiple_tiles() {
        let mut app = present_app().await;
        seed_2x2(&mut app).await;
        let ids: Vec<String> = crate::artifacts::present::present_working_scene(&app.snapshot().await.expect("projection")).1.iter().map(|tile| tile.id.clone()).collect();
        for field in ["x", "y", "width", "height"] {
            app.dispatch_typed(PresentCommand::PatchTileCrops(patch_tile_crops::PatchTileCrops { ids: ids.clone(), field: field.into(), value: 0.4 }), &meta("local")).await.expect("patch field");
        }
        for tile in &crate::artifacts::present::present_working_scene(&app.snapshot().await.expect("projection")).1 {
    ...
    async fn patch_tile_crops_targeting_no_existing_tile_is_a_no_op() {
        let mut app = present_app().await;
        app.dispatch_typed(PresentCommand::PatchTileCrops(patch_tile_crops::PatchTileCrops { ids: vec!["ghost".into()], field: "width".into(), value: 0.4 }), &meta("local")).await.expect("patch ghost");
        assert!(crate::artifacts::present::present_working_scene(&app.snapshot().await.expect("projection")).1.is_empty());
    }
    ...
    async fn app_manifest_declares_expected_operations() {
        ...
        let _ = present_app_with_registry().await;
    }
}
//#endregion 🧪️Tests
