...
//!
//! ⚠️ Why every present case pins a GUARD branch: `PresentSnapshot` keeps its `(source, tiles)` in
...
//!
//! 🆕 `create-tile` is the ONE present verb whose guard is FATAL: an id collision breaks the
...
//!
//! 🎞️ `PresentMutation` carries NO `#[serde(tag = ...)]`, so its wire shape is serde's EXTERNALLY
...
fn committed_json_is_canonical() {
    ...
    assert!(original.get("CreateTile").is_some(), "present mutations are externally tagged by their PascalCase variant name");
}
