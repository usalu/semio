//! 🧪️ `rename-tile` fixture — `no-ops-when-the-tile-already-has-that-name`.
//!
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rename-tile/no-ops-when-the-tile-already-has-that-name: produced diff differs from the committed 🔺️diff/🔣️component.json");
    assert_eq!(outcome.diff(), &PresentDiff::default(), "an identity rename must carry the identity diff");
}
...
async fn inverse_restores_the_base_name_and_is_its_own_inverse_here() {
    ...
    let PresentMutation::RenameTile(undo) = &inverse[0] else {
