...
//! `PresentMutation`. Kept as an empty placeholder only because `📦️glue.rs` (plugin-shared, out of
//! this facet's edit boundary) still `#[path]`-wires this exact file; the report's
//! `sharedFileRequests` asks a later pass to delete this directory once `📦️glue.rs` is repointed.
