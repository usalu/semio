...
/// 📦️ Encodes a `PresentSnapshot` to its binary pack form.
pub fn encode(deck: &PresentSnapshot) -> Vec<u8> {
    store::DocumentPack::encode_pack(deck)
}

/// 📖️ Decodes a `PresentSnapshot` from its binary pack form.
pub fn decode(bytes: &[u8]) -> Result<PresentSnapshot, PackError> {
