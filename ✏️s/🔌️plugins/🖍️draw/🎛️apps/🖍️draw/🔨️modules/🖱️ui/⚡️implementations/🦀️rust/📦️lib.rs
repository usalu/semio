...
fn commit_with_utility_reset(operations: Vec<DrawOperation>, description: &str) -> Emit<DrawOperation, DrawConfigOperation> {
    ...
    let mut emit = Ok(Emit::commit(operations, description);
    ...
    fn handle(&self, command: &DrawCommand, doc: &DocumentView<'_, DrawDocument>, cfg: &ConfigView<'_, DrawConfig>) -> Result<Emit<DrawOperation, DrawConfigOperation>, Fault> {
        ...
        match command {
            ...
            DrawCommand::MoveLayer { layer_id: target_id, target_row_id, drop_position } => {
                let (parent_id, index) = resolve_reorder_target(document, target_row_id, drop_position);
                Ok(Emit::operations(vec![DrawOperation::ReorderLayer { layer_id: target_id.clone(), parent_id, index }])
            }
            ...
            DrawCommand::DuplicateLayer { layer_id: target_id } => {
                ...
                Ok(Emit::operations(vec![DrawOperation::DuplicateLayer { layer_id: target_id.clone() }])
            }
            DrawCommand::ToggleLayerVisible { layer_id: target_id } => match find_draw_layer(document, target_id) {
                Some(layer) => {
                    ...
                    Ok(Emit::operations(vec![DrawOperation::SetLayerVisible { layer_id: target_id.clone(), visible }])
                }
                None => Ok(Emit::default()),
            },
            ...
            DrawCommand::SetActiveUtility { utility_id } => {
                ...
                Ok(Emit::config(vec![DrawConfigOperation::SetActiveUtility { utility_id: utility_id.clone() }])
            }
