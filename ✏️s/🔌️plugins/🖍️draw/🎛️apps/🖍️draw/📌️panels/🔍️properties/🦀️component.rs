...
/// state now and `ArtifactApp::render(body_key, doc, cfg)` is never given an `InteractionView` (only
/// `handle`/`copy_fragment`/`cut_operations` are). Documented reduced-fidelity gap, same shape as
/// `📐️cad`'s `build_properties_panel` object/primitive branches (`🎛️apps/📐️cad/📌️panels/🔍️inspection/
/// 🦀️component.rs`): falls through to the schema/utility/layer-count summary until a
