...
fn matrix_to_semio_transform(matrix: [f64; 6]) -> SemioTransform {
    let transform = draw_matrix_to_transform(matrix);
    SemioTransform {
...
/// reuses `draw_document_to_svg` (the same semio/drawing↔svg bridge the export-svg shell path
/// uses), so there is exactly one SVG renderer.
pub fn draw_vector_media(doc: &DrawSnapshot) -> Result<semio_framework::Media, semio_framework::MediaError> {
    let (svg, _width, _height) = draw_document_to_svg(doc).map_err(|error| semio_framework::MediaError::Payload("vector:out".into(), error))?;
