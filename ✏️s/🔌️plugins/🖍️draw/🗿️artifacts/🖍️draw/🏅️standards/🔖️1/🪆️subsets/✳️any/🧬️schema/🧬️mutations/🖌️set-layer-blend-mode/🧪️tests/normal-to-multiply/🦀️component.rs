...
async fn applies_to_committed_after() {
    ...
    apply_draw_mutation(&mut snapshot, &mutation()).expect("set-layer-blend-mode applies to its committed before-snapshot");
...
async fn blend_mode_is_written_verbatim() {
    ...
    apply_draw_mutation(&mut snapshot, &mutation()).expect("set-layer-blend-mode applies");
    let before_layer = layer_base(find_draw_layer(&base, "shape-a").expect("before carries shape-a"));
    let after_layer = layer_base(find_draw_layer(&snapshot, "shape-a").expect("shape-a survives a blend change"));
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "set-layer-blend-mode/normal-to-multiply: produced diff differs from the committed 🔺️diff/🔣️component.json");
