...
async fn applies_to_committed_after() {
    ...
    apply_draw_mutation(&mut snapshot, &mutation()).expect("rename-layer applies to its committed before-snapshot");
...
/// ✏️ In draw, a layer's `name` is its human label and `id` is its identity key — unlike dag, where
...
async fn the_layer_identity_survives_the_rename() {
    ...
    apply_draw_mutation(&mut snapshot, &mutation()).expect("rename-layer applies");
    assert!(find_draw_layer(&snapshot, "shape-a").is_some(), "rename-layer must not change the layer's addressable id");
    let before_layer = layer_base(find_draw_layer(&base, "shape-a").expect("before carries shape-a"));
    let after_layer = layer_base(find_draw_layer(&snapshot, "shape-a").expect("shape-a is still addressable by id"));
...
/// own `id` addresses `shape-a` and is NOT part of the patch — draw's identity key is never a patch
