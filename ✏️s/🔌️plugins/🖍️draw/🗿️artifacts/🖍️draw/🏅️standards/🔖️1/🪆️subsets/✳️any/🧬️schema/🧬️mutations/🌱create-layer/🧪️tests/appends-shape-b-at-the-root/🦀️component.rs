...
async fn applies_to_committed_after() {
    ...
    apply_draw_mutation(&mut snapshot, &mutation()).expect("create-layer applies to its committed before-snapshot");
...
async fn an_unaddressed_create_appends_at_the_root_end() {
    ...
    apply_draw_mutation(&mut snapshot, &mutation()).expect("create-layer applies");
    ...
    assert!(find_draw_layer(&base, "shape-b").is_none(), "appends-shape-b-at-the-root's before-snapshot must not already carry shape-b");
}
...
/// layer plus its resolved ADDRESS. `DrawLayerAddition.parent_id` is the one field in this diff
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "create-layer/appends-shape-b-at-the-root: produced diff differs from the committed 🔺️diff/🔣️component.json");
