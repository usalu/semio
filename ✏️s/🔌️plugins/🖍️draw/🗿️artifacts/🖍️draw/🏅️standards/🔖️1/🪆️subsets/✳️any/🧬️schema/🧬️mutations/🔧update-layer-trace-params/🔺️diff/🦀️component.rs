...
//#region 🔖️Diff
pub fn diff(payload: &super::mutation::UpdateLayerTraceParams, base: &DrawSnapshot) -> protocol::MutationOutcome<DrawDiff> {
    match find_draw_layer(base, &payload.layer_id) {
        ...
        Some(DrawLayerNode::Trace(trace)) if trace.params == payload.params => {
            protocol::MutationOutcome::empty().warn("mutation.no-op", format!("Layer \"{}\" trace params are unchanged.", payload.layer_id))
        }
