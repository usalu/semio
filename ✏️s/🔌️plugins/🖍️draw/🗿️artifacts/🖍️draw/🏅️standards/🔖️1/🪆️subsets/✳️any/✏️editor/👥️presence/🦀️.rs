//! 👥️ Draw presence — shareable live ephemeral state + mutations.
...
impl store::ArtifactDsl for DrawPresence {
...
impl ArtifactPack for DrawPresence {
...
pub enum DrawPresenceMutation {
    ...
    Snapshot {
        #[dsl(block)]
        presence: DrawPresence,
    },
}

impl Mutation<DrawPresence> for DrawPresenceMutation {
    type Diff = DrawPresence;

    fn diff(&self, _base: &DrawPresence) -> protocol::MutationOutcome<DrawPresence> {
        match self {
            Self::Snapshot { presence } => protocol::MutationOutcome::new(presence.clone()),
        }
    }

    fn inverse(&self, base: &DrawPresence) -> Vec<Self> {
        vec![Self::Snapshot { presence: base.clone() }]
    }
}

impl protocol::OpText for DrawPresenceMutation {
...
impl protocol::OpBinary for DrawPresenceMutation {
