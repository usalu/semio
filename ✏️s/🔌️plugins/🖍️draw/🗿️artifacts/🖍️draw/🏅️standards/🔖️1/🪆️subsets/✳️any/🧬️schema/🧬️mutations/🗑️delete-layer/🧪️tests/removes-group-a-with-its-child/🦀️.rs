//! 🧪️ `delete-layer` fixture — `removes-group-a-with-its-child`.
//!
...
async fn applies_to_committed_after() {
    ...
    apply_draw_mutation(&mut snapshot, &mutation()).expect("delete-layer applies to its committed before-snapshot");
...
async fn deleting_a_group_takes_its_subtree() {
    let base = before();
    assert!(find_draw_layer(&base, "text-a").is_some(), "removes-group-a-with-its-child's before-snapshot must nest text-a inside group-a");
    let mut snapshot = base.clone();
    apply_draw_mutation(&mut snapshot, &mutation()).expect("delete-layer applies");
    assert!(find_draw_layer(&snapshot, "group-a").is_none(), "the addressed group must be gone");
    assert!(find_draw_layer(&snapshot, "text-a").is_none(), "the group's child goes with it — a delete never reparents a subtree to the root");
    assert_eq!(snapshot.layers.len(), 1, "only the untouched sibling remains at the root");
    assert!(find_draw_layer(&snapshot, "shape-a").is_some(), "the sibling layer survives the delete");
}
