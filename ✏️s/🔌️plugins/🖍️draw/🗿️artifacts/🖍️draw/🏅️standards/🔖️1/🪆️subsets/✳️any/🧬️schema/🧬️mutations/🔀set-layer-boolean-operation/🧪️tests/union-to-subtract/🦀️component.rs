...
async fn applies_to_committed_after() {
    ...
    apply_draw_mutation(&mut snapshot, &mutation()).expect("set-layer-boolean-operation applies to its committed before-snapshot");
...
async fn only_the_boolean_variants_operation_changes() {
    ...
    apply_draw_mutation(&mut snapshot, &mutation()).expect("set-layer-boolean-operation applies");
    let DrawLayerNode::Boolean(before_boolean) = find_draw_layer(&base, "boolean-a").expect("before carries boolean-a") else {
        panic!("union-to-subtract's before-snapshot must carry a boolean layer");
    };
    let DrawLayerNode::Boolean(after_boolean) = find_draw_layer(&snapshot, "boolean-a").expect("boolean-a survives the operation change") else {
    ...
    assert_eq!(find_draw_layer(&snapshot, "shape-a"), find_draw_layer(&base, "shape-a"), "the sibling shape layer is out of this mutation's reach");
}
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "set-layer-boolean-operation/union-to-subtract: produced diff differs from the committed 🔺️diff/🔣️component.json");
