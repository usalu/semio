...
//! (`…🖍️draw/…/🚪️io/🦀️.rs:144-154`) and dispatches through `io_dispatch` into
//! `semio@v1/drawing`'s OWN svg bridge, which renders through the SAME `write_svg_xml`
//! (`…🖍️draw/…/🚪️io/🦀️.rs:159-179`). So the wire vocabulary this probe reads —
...
//!
//! WHAT THE BRIDGE DROPS, READ FROM `…🖍️draw/…/🚪️io/🦀️.rs`. Every `SemioDrawNode` variant
