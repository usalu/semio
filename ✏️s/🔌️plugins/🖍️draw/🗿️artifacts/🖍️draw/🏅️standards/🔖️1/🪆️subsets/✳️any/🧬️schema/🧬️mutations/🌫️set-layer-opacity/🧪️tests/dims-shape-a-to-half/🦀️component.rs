//! 🧪️ `set-layer-opacity` fixture — `dims-shape-a-to-half`.
//!
...
async fn applies_to_committed_after() {
    ...
    apply_draw_mutation(&mut snapshot, &mutation()).expect("set-layer-opacity applies to its committed before-snapshot");
...
async fn layer_opacity_is_independent_of_the_fill_alpha() {
    ...
    apply_draw_mutation(&mut snapshot, &mutation()).expect("set-layer-opacity applies");
    let before_layer = layer_base(find_draw_layer(&base, "shape-a").expect("before carries shape-a"));
    let after_layer = layer_base(find_draw_layer(&snapshot, "shape-a").expect("shape-a survives a dim"));
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "set-layer-opacity/dims-shape-a-to-half: produced diff differs from the committed 🔺️diff/🔣️component.json");
