...
async fn applies_to_committed_after() {
    ...
    apply_draw_mutation(&mut snapshot, &mutation()).expect("set-layer-locked applies to its committed before-snapshot");
...
async fn locking_does_not_hide_the_layer() {
    ...
    apply_draw_mutation(&mut snapshot, &mutation()).expect("set-layer-locked applies");
    let before_layer = layer_base(find_draw_layer(&base, "shape-a").expect("before carries shape-a"));
    let after_layer = layer_base(find_draw_layer(&snapshot, "shape-a").expect("shape-a survives a lock"));
