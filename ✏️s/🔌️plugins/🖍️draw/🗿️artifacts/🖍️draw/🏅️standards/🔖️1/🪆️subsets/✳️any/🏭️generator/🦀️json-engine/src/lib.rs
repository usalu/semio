//! 🖍️ The `draw@1/any` LAYER-METADATA mutation vocabulary, expressed over this subset's own JSON
//! carrier and read back through `serde_json` — a third-party JSON implementation and nothing of ours.
//!
//! Why these three and no others: this subset already carries `quick-xml-draw-1-mutate`, which judges
...
//! all), `DrawSnapshot::layers` is an INLINE `Vec<DrawLayerNode>`. The three fields are therefore
...
/// `DrawLayerNode` with `#[serde(rename = "shape")]` on the variant, and `DrawLayerBase` FLATTENED into
