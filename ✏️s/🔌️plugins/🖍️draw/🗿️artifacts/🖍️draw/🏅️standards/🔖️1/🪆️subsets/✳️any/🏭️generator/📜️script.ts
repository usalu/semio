...
// 🏭️ Third-party fixture generator for the three LAYER-METADATA kinds of `s.draw.draw@1/✳️any`.
//
// This subset already carries `quick-xml-draw-1-mutate`, which judges the SVG export. SVG has no
...
// it. They ride this subset's JSON carrier instead, where `DrawSnapshot::layers` is an INLINE
// `Vec<DrawLayerNode>` and every one of the three is a carrier-level fact.
//
...
const ORACLE_ID = "serde-json-draw-carrier-reader";
const COMPARISON_PROFILE = "semantic-draw-carrier-v1";
...
async function manifests(): Promise<void> {
  ...
  for (const kind of KINDS) {
    ...
    entries.push({
      ...
      target: { artifact: "s.draw.draw", standard: "1", subset: "any" },
      mutation: kind,
      outcome: "applied",
      ...
      generator: { oracle: ORACLE_ID, packageVersion: "1", engineFamily: "serde-json", engineVersion: "1", command: "bun ✏️s/🔌️plugins/🖍️draw/🗿️artifacts/🖍️draw/🏅️standards/🔖️1/🪆️subsets/✳️any/🏭️generator/📜️script.ts generate", platform: process.platform },
      comparisonProfile: COMPARISON_PROFILE,
      reproducible: true,
      family: "mechanical",
      notes: `A deterministic two-layer draw document with the ${kind} mutation applied as an edit to the JSON CARRIER and read back through serde_json — never through this repository's own mutation engine. SVG, which this subset's other reader judges, has no representation for this field. Observability is checked before a pair is written, and a pair that does not move is refused rather than committed.`,
    });
  }
