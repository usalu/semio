...
impl protocol::MutationDiff<DrawPresence> for DrawPresence {
    async fn apply(&self, _base: &DrawPresence) -> protocol::MutationApplyResult<DrawPresence> {
        Ok({ self.clone() })
    }
...
impl Mutation<DrawPresence> for DrawPresenceMutation {
    ...
    async fn diff(&self, _base: &DrawPresence) -> protocol::MutationOutcome<DrawPresence> {
        match self {
            Self::Snapshot { presence } => protocol::MutationOutcome::new(presence.clone()),
        }
    }

    async fn inverse(&self, base: &DrawPresence) -> Vec<Self> {
