//! 🪄️ Proc macros compiling `statechart!` declarations into `fsm` static tables.
//!
...
pub fn statechart(input: TokenStream) -> TokenStream {
    expand_statechart(input.into()).unwrap_or_else(|error| error.to_compile_error()).into()
}
...
pub fn derive_statechart_event(input: TokenStream) -> TokenStream {
    ...
    expand_statechart_event(&input).unwrap_or_else(|error| error.to_compile_error()).into()
}
...
pub fn derive_statechart_schema(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    expand_statechart_schema(&input).unwrap_or_else(|error| error.to_compile_error()).into()
}
...
/// 🪄️ Implements `fsm::StatechartEvent` for a consumer-authored enum, for use with
