//! 🧪️ `replace-layer-fill` fixture — `🌾️solid-to-linear-gradient`.
//!
...
async fn applies_to_committed_after() {
    ...
    apply_drawing_mutation(&mut snapshot, &mutation()).expect("replace-layer-fill applies to its committed before-snapshot");
...
async fn the_fill_variant_itself_is_swapped() {
    ...
    apply_drawing_mutation(&mut snapshot, &mutation()).expect("replace-layer-fill applies");
    let before_fill = layer_base(find_drawing_layer(&base, "shape-a").expect("before carries shape-a")).attributes.fill.clone();
    let after_fill = layer_base(find_drawing_layer(&snapshot, "shape-a").expect("shape-a survives a fill swap")).attributes.fill.clone();
    ...
    let stroke = layer_base(find_drawing_layer(&snapshot, "shape-a").expect("shape-a is present")).attributes.stroke.clone();
