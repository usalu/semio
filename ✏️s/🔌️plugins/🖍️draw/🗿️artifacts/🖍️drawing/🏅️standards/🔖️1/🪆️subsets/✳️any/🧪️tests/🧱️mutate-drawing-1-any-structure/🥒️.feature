...
@mutations-drawing-1-any-structure
Feature: Apply every typed drawing-document structure mutation to its committed specification vector
  🧩️ Duplicated from `../../../🧱️structure/🧪️tests/🧱️mutate-drawing-1-structure/` (shard F4, this ticket) to close `unregistered-mutation-vocabulary` at the `✳️any/🧬️schema/🧬️mutations` + `✳️any/🚪️io/🧬️mutations` owner — same mechanism E3 already proved on `sequence`. Reuses the ALREADY-manifested `drawing-1-structure-mutate` capability, so no new v2 manifest entry or runtime-inventory coordinate is created.
