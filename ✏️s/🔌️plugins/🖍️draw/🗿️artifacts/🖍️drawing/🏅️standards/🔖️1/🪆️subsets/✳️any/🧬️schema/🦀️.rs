...
mod tests {
    ...
    async fn layer_to_path_segments_covers_every_shape_kind_and_empty_polygon_and_unknown_kind() {
        ...
        let polygon =
            DrawingLayerNode::Shape(DrawingShapeBody { base: default_layer_base("Poly"), shape_kind: "polygon".into(), rect: None, ellipse: None, circle: None, line: None, polygon: Some(DrawingPolygon { points: vec![[0.0, 0.0], [1.0, 0.0], [0.5, 1.0]] }) });
        assert_eq!(layer_to_path_segments(&polygon).len(), 4);

        let ellipse = DrawingLayerNode::Shape(DrawingShapeBody { base: default_layer_base("Ellipse"), shape_kind: "ellipse".into(), rect: None, ellipse: Some(DrawingEllipse { cx: 0.0, cy: 0.0, rx: 1.0, ry: 1.0 }), circle: None, line: None, polygon: None });
