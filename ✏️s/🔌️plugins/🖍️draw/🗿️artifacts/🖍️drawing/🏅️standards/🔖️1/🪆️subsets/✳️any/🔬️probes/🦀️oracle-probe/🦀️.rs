//! 🔬️ Third-party carrier reader for `s.draw.drawing@1/✳️any`.
//!
...
//!
//! WHY THIS READS THE SAME ATTRIBUTE VOCABULARY AS `semio@v1/drawing`'s OWN PROBE. `drawing`'s
//! `DrawingIntoSvg` serializer does not hand-roll SVG itself — it builds a `SemioDrawingSnapshot`
//! (`…🖍️drawing/…/🚪️io/🦀️.rs:144-154`) and dispatches through `io_dispatch` into
//! `semio@v1/drawing`'s OWN svg bridge, which renders through the SAME `write_svg_xml`
//! (`…🖍️drawing/…/🚪️io/🦀️.rs:159-179`). So the wire vocabulary this probe reads —
...
//!
//! WHAT THE BRIDGE DROPS, READ FROM `…🖍️drawing/…/🚪️io/🦀️.rs`. Every `SemioDrawNode` variant
//! (`Group`/`Path`/`Text`/`Image`) carries no `id` field at all (lines 121-139), so a `drawing` layer's
...
//! `locked` has no field on `DrawingSceneNode` at all — `set-layer-locked` is NOT witnessable. An
...
//!   semio-drawing-oracle-probe fixtures      --out <dir>
//!
...
/// attributes `drawing`'s exported carrier actually carries. Nothing is inferred — an attribute the file
...
/// `drawing_document_to_svg`'s own one-group-per-node shape), wrapping one `<path>` or `<text>` leaf.
struct SceneNode<'a> {
    matrix: [f64; 6],
...
/// here (see this subset's 🔣️oracle.json `notes` on those two entries).
fn fixtures(out: &str) -> Result<Vec<String>, String> {
    ...
    // `!base.visible` layers before they ever become a DrawingSceneNode).
