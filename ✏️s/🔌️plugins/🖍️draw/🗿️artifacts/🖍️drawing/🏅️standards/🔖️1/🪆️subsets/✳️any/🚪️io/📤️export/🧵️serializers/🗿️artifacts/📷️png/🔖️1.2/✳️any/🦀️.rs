...
impl Serializer<DrawingSnapshot> for DrawingIntoPng {
    ...
    async fn serialize(from: &DrawingSnapshot) -> IoResult<IoPayload> {
        let error = |message: String| IoError { message: format!("DrawingIntoPng: {message}"), diagnostics: Vec::new() };
        let bytes = encode_drawing(&drawing_document_to_semio_drawing(from), SemioDrawingFormat::Png).map_err(error)?;
