...
async fn applies_to_committed_after() {
    ...
    apply_drawing_mutation(&mut snapshot, &mutation()).expect("reorder-layer applies to its committed before-snapshot");
...
async fn stacking_order_changes_but_geometry_does_not() {
    ...
    apply_drawing_mutation(&mut snapshot, &mutation()).expect("reorder-layer applies");
    ...
    assert_eq!(find_drawing_layer(&snapshot, "shape-a"), find_drawing_layer(&base, "shape-a"), "the moved layer's own value is carried across untouched");
}
