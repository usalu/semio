//! 🧪️ `update-layer-transform` fixture — `🔴️translates-and-scales-shape-a`.
//!
...
async fn applies_to_committed_after() {
    ...
    apply_drawing_mutation(&mut snapshot, &mutation()).expect("update-layer-transform applies to its committed before-snapshot");
...
async fn the_whole_transform_facet_moves_atomically() {
    ...
    apply_drawing_mutation(&mut snapshot, &mutation()).expect("update-layer-transform applies");
    let after_layer = layer_base(find_drawing_layer(&snapshot, "shape-a").expect("shape-a survives a transform update"));
    ...
    let (DrawingLayerNode::Shape(before_shape), DrawingLayerNode::Shape(after_shape)) = (find_drawing_layer(&base, "shape-a").expect("before carries shape-a"), find_drawing_layer(&snapshot, "shape-a").expect("after carries shape-a")) else {
...
async fn produces_committed_diff() {
    ...
    let transform: crate::artifacts::drawing::DrawingTransform = serde_json::from_str(blob).expect("the transform blob is itself valid JSON");
