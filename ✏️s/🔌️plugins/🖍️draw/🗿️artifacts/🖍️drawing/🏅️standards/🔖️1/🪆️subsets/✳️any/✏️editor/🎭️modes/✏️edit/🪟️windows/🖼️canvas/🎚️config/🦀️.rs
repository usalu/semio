...
impl store::ArtifactDsl for DrawingCanvasWindowConfig {
    const EXTENSION: &'static str = "drawingcanvaswindowcfg";
    fn envelope_id() -> &'static str { "s.draw.drawing.canvas-window.config" }
    ...
    fn print_dsl(&self) -> String {
        ...
        let body = serde_json::to_string_pretty(&value).expect("Drawing Canvas window config JSON");
...
impl store::ArtifactPack for DrawingCanvasWindowConfig {
    ...
    fn decode_pack_with(bytes: &[u8], _options: &store::PackDecodeOptions) -> Result<Self, store::PackError> {
        ...
        if !envelope.matches_identity(<Self as store::ArtifactDsl>::envelope_id(), store::semio_format::Component::Pack, 1) {
            return Err(store::PackError::Schema("Drawing Canvas window config pack envelope mismatch".into()));
        }
