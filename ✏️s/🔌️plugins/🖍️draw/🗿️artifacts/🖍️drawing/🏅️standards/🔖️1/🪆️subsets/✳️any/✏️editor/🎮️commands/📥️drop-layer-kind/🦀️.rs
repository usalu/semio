...
use crate::schema::{create_layer_by_kind, find_drawing_layer, find_drawing_layer_location};
...
fn resolve_reorder_target(document: &DrawingSnapshot, target_row_id: &str, drop_position: &str) -> (Option<String>, usize) {
    ...
    if let Some(layer_id_value) = crate::schema::drawing_play_layer_id_from_tree_row_id(target_row_id) {
        if let Some(layer) = find_drawing_layer(document, &layer_id_value) {
            if drop_position == "inside" {
                if let crate::DrawingLayerNode::Group(group) = layer {
...
pub fn handle(payload: &DropLayerKind, doc: &ArtifactView<'_, DrawingSnapshot>, _cfg: &ConfigView<'_, NoConfig>, _session: &mut DrawingSession) -> Result<Emit<DrawingMutation, NoConfigMutation>, Fault> {
    ...
    Ok(Emit { artifact_mutations: vec![crate::mutations::create_layer(parent_id, Some(index), layer)], ..Default::default() })
}
