...
//#region 🔖️Render
pub fn render(_document: &DrawingSnapshot, labels: &DrawingPlayLabels) -> semio_framework_plugin::UiAssemblyResult<semio_framework_plugin::BuiltNode> {
    let catalogue_kinds = [
    ...
    for operation in DRAWING_BOOLEAN_OPERATIONS {
        ...
        items.try_push(item).map_err(|_| PluginAssemblyError::new("ui.fixed-capacity", "drawing boolean row admission failed"))?;
    }
    ...
    PanelTreeBuilder::new("drawing-play-catalogue")?.section("drawing-play-catalogue", Some(section), true, items)?.build()
}
//#endregion 🔖️Render
