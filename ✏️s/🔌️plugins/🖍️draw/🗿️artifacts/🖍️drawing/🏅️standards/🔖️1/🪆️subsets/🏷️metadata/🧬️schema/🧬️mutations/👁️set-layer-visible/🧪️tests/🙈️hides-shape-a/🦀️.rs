//! 🧪️ `set-layer-visible` fixture — `🙈️hides-shape-a`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/👁️set-layer-visible/🙈️hides-shape-a/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/👁️set-layer-visible/🙈️hides-shape-a/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/👁️set-layer-visible/🙈️hides-shape-a/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/👁️set-layer-visible/🙈️hides-shape-a/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/👁️set-layer-visible/🙈️hides-shape-a/🎯️outcome/🔣️.json");
...
async fn applies_to_committed_after() {
    ...
    apply_drawing_mutation(&mut snapshot, &mutation()).expect("set-layer-visible applies to its committed before-snapshot");
...
async fn only_the_visible_flag_flips() {
    ...
    apply_drawing_mutation(&mut snapshot, &mutation()).expect("set-layer-visible applies");
    let before_layer = layer_base(find_drawing_layer(&base, "shape-a").expect("before carries shape-a"));
    let after_layer = layer_base(find_drawing_layer(&snapshot, "shape-a").expect("shape-a survives a visibility flip"));
