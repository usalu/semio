...
//! only generates `Mutation`/`SemanticMutation` (see `../🦀️component.rs`'s `🔖️FormMutation` region)
//! — the wire-text/wire-binary codecs stay handcrafted here, one keyword per semantic verb, grammar
//! `keyword key1=value1 key2=value2 ...`. `📦️glue.rs`'s `op` shim re-exports this module's `*`
