...
async fn extension_question_emits_external_slot_when_contribution_registered() {
    let node = render_try_question(&building_component_question(), &Object::new(), &building_component_contributions(), None, crate::editor::forms::terminology::forms_play_labels(&semio_framework_plugin::ViewModel::default()), &semio_framework_plugin::ViewModel::default());
    let json = serde_json::to_string(&node.expect("semantic component")).expect("component JSON");
...
async fn extension_question_falls_back_without_contribution() {
    let node = render_try_question(&building_component_question(), &Object::new(), &[], None, crate::editor::forms::terminology::forms_play_labels(&semio_framework_plugin::ViewModel::default()), &semio_framework_plugin::ViewModel::default());
    let json = serde_json::to_string(&node.expect("semantic component")).expect("component JSON");
