...
impl protocol::MutationKind<FormsConfig, FormsConfigMutation> for CommitTryValuesBatch {
    ...
    fn diff(&self, base: &FormsConfig) -> protocol::MutationOutcome<FormsConfig> {
        ...
        let Some(values) = commit_staged_try_values_batch(&self.staging_id, self.entry_count) else { return protocol::MutationOutcome::new(next).absorb_messages([FormsStageError::Missing.message()]); };
    ...
    fn label(&self) -> String { "Commit Try Values Batch".into() }
