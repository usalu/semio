...
impl protocol::MutationKind<FormsConfig, FormsConfigMutation> for StageTryValueChunk {
    ...
    fn diff(&self, base: &FormsConfig) -> protocol::MutationOutcome<FormsConfig> {
        let next = base.clone();
        if let Err(error) = stage_try_value_chunk(&self.staging_id, self.index, &self.chunk) {
    ...
    fn inverse(&self, _base: &FormsConfig) -> Vec<FormsConfigMutation> {
        vec![FormsConfigMutation::DiscardTryValueStaging(DiscardTryValueStaging { staging_id: self.staging_id.clone() })]
    }
    fn label(&self) -> String {
        "Stage Try Value Chunk".into()
    }
