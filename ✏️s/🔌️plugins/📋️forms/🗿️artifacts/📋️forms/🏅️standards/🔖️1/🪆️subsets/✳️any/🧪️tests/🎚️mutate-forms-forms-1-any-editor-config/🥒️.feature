@capability-forms-forms-1-any-editor-config-mutate
@no-oracle-forms-forms-1-any-editor-config-state-lane-semantics
@comparison-ordered-json-v1
@mutations-forms-forms-1-any-editor-config
Feature: Apply every config state-lane mutation of s.forms.forms's ✏️editor/🎚️config to its committed vector
  `FormsConfigMutation` is the config lane of this subset's editor: it holds the host-declared contribution list (`contributionsJson`) projected into the forms app. It is this
  repository's own state record, so the case records the no-oracle decision `forms-forms-1-any-editor-config-state-lane-semantics` in
  ...
  before-snapshot. Production dispatch is reached through `forms_config_mutation_report_json`, which runs
  ...
  Scenario Outline: Apply <id> and land on the committed after-snapshot, diff and outcome
    ...
    When <id> is applied to that vector's before-snapshot through forms_config_mutation_report_json
