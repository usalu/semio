...
mod tests {
    ...
    fn inspector_patch_updates_required() {
        let mut app = forms_app();
        dispatch(&mut app, FormsCommand::SetActiveExample(crate::apps::forms::commands::set_active_example::SetActiveExample { example_id: "default".into() }));
    ...
    fn move_question_relocates_it_to_the_target_step() {
        let mut app = forms_app();
        dispatch(&mut app, FormsCommand::AddStep(crate::apps::forms::commands::add_step::AddStep {}));
        let spec = app.snapshot().expect("projection");
        let steps = forms_steps(&spec);
