...
mod tests {
    ...
    fn vector_question_id(app: &mut crate::apps::forms::testkit::FormsApp) -> String {
        ...
        crate::artifacts::forms::engine::flatten_questions(&app.projection().expect("projection")).into_iter().map(|(_, question)| question).find(|question| question.kind == "vector").expect("vector question").id
    }
