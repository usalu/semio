...
use forms::{FormQuestion, FormQuestionOption, FormSpec, FormStep, FormVectorField, FORMS_DOCUMENT_SCHEMA, FORM_BUILTIN_KINDS};
...
fn forms_action(action: &str, args: Option<Value>) -> ActionDescriptor {
    semio_framework_plugin::ActionFactory::new(FORMS_PLAY_CONTROLLER_ID).action(action, args)
}
...
fn find_question_kind_contribution<'a>(contributions: &'a [ProgramContributionEntry], kind: &str) -> Option<(&'a str, &'a Contribution)> {
...
fn extension_params_value(question: &FormQuestion, values: &Map<String, Value>) -> Value {
    values.get(&question.id).cloned().or_else(|| question.params.as_ref().map(dsl_to_value)).unwrap_or_else(|| json!({}))
}
...
fn render_extension_question(question: &FormQuestion, values: &Map<String, Value>, contributions: &[ProgramContributionEntry], surface: &str, interactive: bool) -> UiNode {
    ...
    ui_stack_vertical(vec![ui_external_slot(plugin_id, app_id, params_body_key, &payload), ui_external_slot(plugin_id, app_id, preview_body_key, &payload)])
}
...
fn default_question_for_kind(kind: &str, id: String) -> FormQuestion {
    match kind {
        ...
        "single" | "multi" => {
            ...
            question.options = Some(vec![FormQuestionOption { value: "a".into(), label: "Option A".into() }, FormQuestionOption { value: "b".into(), label: "Option B".into() }]);
            question
        }
        "note" => {
...
fn question_kind_editor_fields(question: &FormQuestion, question_ids: &[String], contributions: &[ProgramContributionEntry], id_prefix: &str, labels: &FormsLabels) -> Vec<UiNode> {
    ...
    match question.kind.as_str() {
        ...
        "boolean" => {
            let pressed = question.default.as_ref().and_then(|default| dsl_to_value(default).as_bool()).unwrap_or(false);
...
fn build_inspector_tree(spec: &FormSpec, config: &FormsConfig, contributions: &[ProgramContributionEntry], term_labels: &FormsLabels) -> UiNode {
    let questions: Vec<FormQuestion> = config.selected_ids.iter().filter_map(|id| find_question_location(spec, id).map(|location| location.question)).collect();
    if questions.is_empty() {
        return ui_declarative_sections_to_tree(&[semio_framework_plugin::UiSectionNode {
            id: "forms-play-inspector.empty".into(),
            label: Some(Label::data(FRAMEWORK_PANEL_TAB_INSPECTION_LABEL)),
            default_open: Some(true),
            children: vec![ui_text(Label::data(format!("Schema: {FORMS_DOCUMENT_SCHEMA}"))), ui_text(Label::data(format!("Steps: {}", spec.steps.len()))), ui_text(Label::data(format!("Questions: {}", flatten_questions(spec).len())))],
            presence: UiPresence::default(),
            menu: None,
        }]);
    }
...
fn forms_playbook_builder_config() -> playbook::PlaybookBuilderConfig {
    playbook::PlaybookBuilderConfig { action_namespace: "forms-blueprint", controller_id: FORMS_PLAY_CONTROLLER_ID, labels: playbook::PLAYBOOK_BUILDER_LABELS_EN }
}
...
fn render_blueprint_builder(spec: &FormSpec, forms_config: &FormsConfig, contributions: &[ProgramContributionEntry], labels: &FormsLabels) -> UiNode {
    let palette: Vec<BlockPaletteEntry> = catalogue_kinds(contributions, labels).into_iter().map(|(kind, label, icon_id)| BlockPaletteEntry { block_kind: kind, label, icon_id }).collect();
    let builder_config = forms_playbook_builder_config();
    playbook::render_playbook_builder(FORMS_PLAY_SURFACE_BLUEPRINT, spec, &palette, forms_config.selected_ids.first().map(String::as_str), &builder_config)
}
//#endregion 🔖️Builder
...
fn render_image_question(question: &FormQuestion) -> UiNode {
    ui_image(format!("forms-try.{}.image", question.id), image_question_src(question), Some(Label::data(question.label.clone())))
}
    ...
    fn handle(&self, command: &FormsCommand, doc: &DocumentView<'_, FormSpec>, cfg: &ConfigView<'_, FormsConfig>) -> Result<Emit<FormOperation, FormsConfigOperation>, Fault> {
        ...
        match command {
            ...
            FormsCommand::PatchStep { step_id, field, value } => {
                ...
                Emit { document_operations: vec![FormOperation::UpdateStep { step }], config_operations: reset_try_config_operations(), coalesce_key: Some(format!("patch-step:{step_id}:{field}")), ..Default::default() }
            }
            ...
            FormsCommand::PatchQuestionOptions { question_ids, option_value, field, value_json } => {
                ...
                Ok(Emit::amend(operations, format!("patch-option:{option_value}:{field}"))
            }
    ...
    fn export_media(&self, port: &str, doc: &DocumentView<'_, FormSpec>) -> Result<Media, MediaError> {
        match port {
            "document:out" => {
                ...
                Ok(Media { media_type: MediaType { class: MediaClass::Data, form: MediaForm::Value }, payload: MediaPayload::Structured { schema: FORMS_DOCUMENT_SCHEMA.into(), json: store::pack_rt::pack_value_to_base64(&bytes) } })
            }
            "dictionary:out" => {
...
// `✏️s/🔌️plugins/📋️forms/🛂️manifest/🗿️artifact` crate (matching every other plugin's constitutional
// layout — the `🎛️apps/…/ui` crate exports `create_forms_app`/`FormsPlayApp` only, never registers the
...
mod tests {
    ...
    fn add_question_materializes_kind_default() {
        ...
        assert!(flatten_questions(&spec).iter().any(|(_, question)| question.kind == "text"), "kind default materialized from the registry");
    }
    ...
    fn image_question_with_url_src_emits_image_node() {
        let question = FormQuestion { src: Some("https://example.com/picture.png".into()), ..question_shell("q-image".into(), "Picture".into(), "image".into()) };
    ...
    fn drop_question_kind_inserts_and_selects() {
        let mut app = new_app();
        let step_id = app.projection().expect("projection").steps[0].id.clone();
        app.dispatch_typed(FormsCommand::DropQuestionKind { kind: "slider".into(), target_id: forms_play_step_tree_id(&step_id), drop_position: "inside".into() }, &testkit::meta("local")).expect("drop kind");
        let spec = app.projection().expect("projection");
    ...
    fn add_question_undo_redo_round_trip() {
        ...
        testkit::assert_undo_redo_round_trip(&mut app, FormsCommand::AddQuestion { kind: "text".into(), step_id: None }, |app| flatten_questions(&app.projection().expect("projection")).len(), before, before + 1);
    }
    ...
    fn inspector_patch_updates_required() {
        ...
        app.dispatch_typed(FormsCommand::PatchQuestions { question_ids: vec![name_id], field: "required".into(), value_json: "false".into(), param_key: None }, &testkit::meta("local")).expect("patch required");
        let spec = app.projection().expect("projection");
    ...
    fn two_instances_converge_disjoint_edits() {
        testkit::assert_two_instances_converge::<FormsPlayApp, (usize, usize)>("mem://forms-convergence", FormsCommand::AddQuestion { kind: "text".into(), step_id: None }, FormsCommand::AddStep, |app| {
