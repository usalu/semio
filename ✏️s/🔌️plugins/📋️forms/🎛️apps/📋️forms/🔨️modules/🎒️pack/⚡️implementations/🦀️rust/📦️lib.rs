//! 📦️ Forms app — binary document surface + laws (constitutional: pack).
...
mod tests {
    ...
    fn building_component_fixture_pack_agrees_with_dsl() {
        let spec = forms_dsl::parse_dsl(forms_dsl::BUILDING_COMPONENT_EXAMPLE_TEXT).expect("📋️building-component.forms parses");
    ...
    fn default_fixture_pack_agrees_with_dsl() {
        let spec = forms_dsl::parse_dsl(forms_dsl::DEFAULT_EXAMPLE_TEXT).expect("📋️default.forms parses");
        store::test_support::assert_dsl_pack_equivalence(&spec);
        let bytes = encode(&spec);
        assert_eq!(decode(&bytes).expect("decode"), spec);
    }
    ...
    fn onboarding_fixture_pack_agrees_with_dsl() {
        let spec = forms_dsl::parse_dsl(forms_dsl::ONBOARDING_EXAMPLE_TEXT).expect("📋️onboarding.forms parses");
        store::test_support::assert_dsl_pack_equivalence(&spec);
        let bytes = encode(&spec);
        assert_eq!(decode(&bytes).expect("decode"), spec);
    }
