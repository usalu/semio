...
impl Default for Puzzle3dPresence {
    fn default() -> Self {
        Self { camera_position: [0.0, 0.0, 0.0], camera_target: [0.0, 0.0, 0.0], camera_zoom: 1.0, active_utility_id: String::new(), active_tool_id: None }
    }
}

impl protocol::MutationDiff<Puzzle3dPresence> for Puzzle3dPresence {
    async fn apply(&self, _base: &Puzzle3dPresence) -> protocol::MutationApplyResult<Puzzle3dPresence> {
        Ok({ self.clone() })
    }
...
impl Mutation<Puzzle3dPresence> for Puzzle3dPresenceMutation {
    ...
    async fn diff(&self, _base: &Puzzle3dPresence) -> protocol::MutationOutcome<Puzzle3dPresence> {
        protocol::MutationOutcome::new(match self {
            Self::Snapshot { presence } => presence.clone(),
        })
    }

    async fn inverse(&self, base: &Puzzle3dPresence) -> Vec<Self> {
