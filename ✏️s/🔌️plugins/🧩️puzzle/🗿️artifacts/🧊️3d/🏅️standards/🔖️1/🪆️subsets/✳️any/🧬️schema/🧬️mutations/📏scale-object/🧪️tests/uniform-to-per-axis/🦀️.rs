//! 🧪️ `scale-object` fixture — `uniform-to-per-axis`.
//!
...
/// ▶️ The committed `scale-object` payload carries `before` to exactly the committed `after`, and
/// lands the change this case is named for.
#[test]
fn applies_to_committed_after() {
    let mut snapshot = before();
    apply_puzzle3d_mutation(&mut snapshot, &mutation()).expect("scale-object applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "scale-object/uniform-to-per-axis: applied state differs from committed after-snapshot");
...
/// ↩️ Applying `scale-object` then the inverse it derives from `before` restores `before` exactly.
#[test]
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "scale-object/uniform-to-per-axis: inverse did not restore the before-snapshot");
}

/// 🔣️ Both committed snapshots and the committed `scale-object` payload are already canonical:
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "scale-object/uniform-to-per-axis: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "scale-object/uniform-to-per-axis: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome matches what `scale-object` actually produces on this base.
#[test]
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "scale-object/uniform-to-per-axis: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "scale-object/uniform-to-per-axis: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "scale-object/uniform-to-per-axis: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("scale-object/uniform-to-per-axis: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta `scale-object` produces is exactly the committed diff — the single most
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "scale-object/uniform-to-per-axis: produced diff differs from the committed 🔺️diff/🔣️.json");
...
/// 🔣️ The committed `scale-object` diff is itself canonical and decodes to `Puzzle3dDiff`.
#[test]
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "scale-object/uniform-to-per-axis: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed `scale-object` diff directly to `before` yields the committed `after` —
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "scale-object/uniform-to-per-axis: committed diff did not carry before to after");
}
