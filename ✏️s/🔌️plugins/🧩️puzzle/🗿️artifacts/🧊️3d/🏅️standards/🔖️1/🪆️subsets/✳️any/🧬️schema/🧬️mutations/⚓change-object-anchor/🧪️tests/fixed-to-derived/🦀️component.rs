//! 🧪️ `change-object-anchor` fixture — `fixed-to-derived`.
//!
//! Flips `object-a` from keeping its stored plane (`fixed`) to resetting to default XY on flatten
//! (`derived`). `anchor` is a bare enum with no `Option`, so it is always present.
//!
...
/// ▶️ The committed `change-object-anchor` payload carries `before` to exactly the committed `after`, and
...
/// ↩️ Applying `change-object-anchor` then the inverse it derives from `before` restores `before` exactly.
#[test]
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "change-object-anchor/fixed-to-derived: inverse did not restore the before-snapshot");
}

/// 🔣️ Both committed snapshots and the committed `change-object-anchor` payload are already canonical:
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-object-anchor/fixed-to-derived: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "change-object-anchor/fixed-to-derived: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome matches what `change-object-anchor` actually produces on this base.
#[test]
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "change-object-anchor/fixed-to-derived: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "change-object-anchor/fixed-to-derived: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "change-object-anchor/fixed-to-derived: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("change-object-anchor/fixed-to-derived: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta `change-object-anchor` produces is exactly the committed diff — the single most
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-object-anchor/fixed-to-derived: produced diff differs from the committed 🔺️diff/🔣️component.json");
...
/// 🩹 Applying the committed `change-object-anchor` diff directly to `before` yields the committed `after` —
