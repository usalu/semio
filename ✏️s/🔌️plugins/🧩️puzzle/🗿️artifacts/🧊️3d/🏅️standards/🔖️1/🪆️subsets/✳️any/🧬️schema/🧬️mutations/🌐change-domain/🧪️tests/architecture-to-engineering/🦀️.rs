//! 🧪️ `change-domain` fixture — `architecture-to-engineering`.
//!
...
/// ▶️ The committed `change-domain` payload carries `before` to exactly the committed `after`, and
/// lands the change this case is named for.
#[test]
fn applies_to_committed_after() {
    let mut snapshot = before();
    apply_puzzle3d_mutation(&mut snapshot, &mutation()).expect("change-domain applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "change-domain/architecture-to-engineering: applied state differs from committed after-snapshot");
...
/// ↩️ Applying `change-domain` then the inverse it derives from `before` restores `before` exactly.
#[test]
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "change-domain/architecture-to-engineering: inverse did not restore the before-snapshot");
}

/// 🔣️ Both committed snapshots and the committed `change-domain` payload are already canonical:
...
fn committed_json_is_canonical() {
    ...
    assert_eq!(reencoded, original, "change-domain/architecture-to-engineering: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome matches what `change-domain` actually produces on this base.
#[test]
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "change-domain/architecture-to-engineering: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "change-domain/architecture-to-engineering: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "change-domain/architecture-to-engineering: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("change-domain/architecture-to-engineering: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta `change-domain` produces is exactly the committed diff — the single most
...
/// 🔣️ The committed `change-domain` diff is itself canonical and decodes to `Puzzle3dDiff`.
#[test]
...
/// 🩹 Applying the committed `change-domain` diff directly to `before` yields the committed `after` —
