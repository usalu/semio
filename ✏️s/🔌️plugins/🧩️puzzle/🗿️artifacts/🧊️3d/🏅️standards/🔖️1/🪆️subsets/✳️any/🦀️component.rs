...
//! `io() -> IoDeclaration` function their migration added to `🚪️io/🦀️component.rs`. puzzle3d's own
...
async fn examples() -> &'static [ExampleSource] {
    ...
    EXAMPLES
        .get_or_init(|| vec![crate::examples::puzzle3d::nakagin_capsule_tower::SOURCE.clone(), crate::examples::puzzle3d::concrete_forest::SOURCE.clone()])
        .as_slice()
}
...
pub async fn subset() -> SubsetDeclaration {
    SubsetDeclaration {
        ...
        io: io_declaration(),
        viewer: viewer_surface::<viewer::Puzzle3dViewer>(viewer::create_puzzle3d_viewer()),
        editor: editor_surface::<editor::Puzzle3dPlayApp>(editor::create_puzzle3d_app()),
        examples: examples(),
    }
}
