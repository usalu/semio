...
pub fn set_projection(ctx: &mut Puzzle3dActionCtx<'_>, action: &str, args: Option<&Value>) {
    let moves_pose = semio_framework::io::resolve_ready(world3d_projection_action_moves_pose(action, args));
    semio_framework::io::resolve_ready(apply_world3d_projection_action(&mut ctx.scene.runtime.camera.projection, action, args));
    if moves_pose {
        let distance = puzzle3d_camera_distance(&ctx.scene.runtime.camera);
        let (position, up) = semio_framework::io::resolve_ready(world3d_projection_pose(&ctx.scene.runtime.camera.projection, ctx.scene.runtime.camera.target, distance));
