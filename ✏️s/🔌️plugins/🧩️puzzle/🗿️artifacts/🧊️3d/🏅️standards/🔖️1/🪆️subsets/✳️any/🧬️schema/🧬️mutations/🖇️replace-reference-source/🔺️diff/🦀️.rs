//! 🔺️ Sparse diff builder for `ReplaceReferenceSource` — patches the one addressed reference in place.
...
//#region 🔖️Diff
pub fn diff(payload: &super::mutation::ReplaceReferenceSource, base: &Puzzle3dSnapshot) -> protocol::MutationOutcome<Puzzle3dDiff> {
    ...
    next.source = payload.new_source.clone();
    if next == *item {
