//! 🧪️ `rotate-object` fixture — `half-turn-about-z`.
//!
...
/// ▶️ The committed `rotate-object` payload carries `before` to exactly the committed `after`, and
/// lands the change this case is named for.
#[semio_framework_async_macros::async_test]
async fn applies_to_committed_after() {
    let mut snapshot = before();
    apply_puzzle3d_mutation(&mut snapshot, &mutation()).expect("rotate-object applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "rotate-object/half-turn-about-z: applied state differs from committed after-snapshot");
...
/// ↩️ Applying `rotate-object` then the inverse it derives from `before` restores `before` exactly.
#[semio_framework_async_macros::async_test]
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "rotate-object/half-turn-about-z: inverse did not restore the before-snapshot");
}

/// 🔣️ Both committed snapshots and the committed `rotate-object` payload are already canonical:
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "rotate-object/half-turn-about-z: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "rotate-object/half-turn-about-z: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome matches what `rotate-object` actually produces on this base.
#[semio_framework_async_macros::async_test]
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "rotate-object/half-turn-about-z: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "rotate-object/half-turn-about-z: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "rotate-object/half-turn-about-z: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("rotate-object/half-turn-about-z: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta `rotate-object` produces is exactly the committed diff — the single most
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "rotate-object/half-turn-about-z: produced diff differs from the committed 🔺️diff/🔣️component.json");
...
/// 🔣️ The committed `rotate-object` diff is itself canonical and decodes to `Puzzle3dDiff`.
#[semio_framework_async_macros::async_test]
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "rotate-object/half-turn-about-z: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed `rotate-object` diff directly to `before` yields the committed `after` —
