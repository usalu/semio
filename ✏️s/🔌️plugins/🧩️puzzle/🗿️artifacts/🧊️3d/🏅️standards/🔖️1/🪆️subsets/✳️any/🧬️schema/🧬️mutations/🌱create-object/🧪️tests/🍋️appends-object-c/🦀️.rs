//! 🧪️ `create-object` fixture — `🍋️appends-object-c`.
//!
...
/// ▶️ The committed `create-object` payload carries `before` to exactly the committed `after`, and
/// lands the change this case is named for.
#[test]
fn applies_to_committed_after() {
    let mut snapshot = before();
    apply_puzzle3d_mutation(&mut snapshot, &mutation()).expect("create-object applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "create-object/appends-object-c: applied state differs from committed after-snapshot");
...
/// ↩️ Applying `create-object` then the inverse it derives from `before` restores `before` exactly.
#[test]
...
/// 🔣️ Both committed snapshots and the committed `create-object` payload are already canonical:
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "create-object/appends-object-c: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "create-object/appends-object-c: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome matches what `create-object` actually produces on this base.
#[test]
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "create-object/appends-object-c: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "create-object/appends-object-c: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "create-object/appends-object-c: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("create-object/appends-object-c: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta `create-object` produces is exactly the committed diff — the single most
...
/// 🔣️ The committed `create-object` diff is itself canonical and decodes to `Puzzle3dDiff`.
#[test]
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "create-object/appends-object-c: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed `create-object` diff directly to `before` yields the committed `after` —
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "create-object/appends-object-c: committed diff did not carry before to after");
}
