//! 🧪️ `replace-kind-catalogs` fixture — `installs-vortex-kind-catalog`.
//!
...
fn applies_to_committed_after() {
    ...
    apply_puzzle3d_mutation(&mut snapshot, &mutation()).expect("replace-kind-catalogs applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "replace-kind-catalogs/installs-vortex-kind-catalog: applied state differs from committed after-snapshot");
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "replace-kind-catalogs/installs-vortex-kind-catalog: inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-kind-catalogs/installs-vortex-kind-catalog: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "replace-kind-catalogs/installs-vortex-kind-catalog: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "replace-kind-catalogs/installs-vortex-kind-catalog: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "replace-kind-catalogs/installs-vortex-kind-catalog: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "replace-kind-catalogs/installs-vortex-kind-catalog: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("replace-kind-catalogs/installs-vortex-kind-catalog: unknown outcome status {other:?}"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-kind-catalogs/installs-vortex-kind-catalog: produced diff differs from the committed 🔺️diff/🔣️component.json");
...
/// 🔣️ The committed `replace-kind-catalogs` diff is itself canonical and decodes to `Puzzle3dDiff`.
#[test]
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "replace-kind-catalogs/installs-vortex-kind-catalog: committed diff JSON is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "replace-kind-catalogs/installs-vortex-kind-catalog: committed diff did not carry before to after");
}
