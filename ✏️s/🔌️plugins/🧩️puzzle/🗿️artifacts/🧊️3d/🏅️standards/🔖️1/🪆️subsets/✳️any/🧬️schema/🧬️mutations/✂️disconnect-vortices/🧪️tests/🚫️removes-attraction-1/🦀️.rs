//! 🧪️ `disconnect-vortices` fixture — `🚫️removes-attraction-1`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️disconnect-vortices/🚫️removes-attraction-1/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️disconnect-vortices/🚫️removes-attraction-1/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️disconnect-vortices/🚫️removes-attraction-1/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️disconnect-vortices/🚫️removes-attraction-1/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/✂️disconnect-vortices/🚫️removes-attraction-1/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "disconnect-vortices/removes-attraction-1: applied state differs from committed after-snapshot");
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "disconnect-vortices/removes-attraction-1: inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "disconnect-vortices/removes-attraction-1: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "disconnect-vortices/removes-attraction-1: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "disconnect-vortices/removes-attraction-1: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "disconnect-vortices/removes-attraction-1: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "disconnect-vortices/removes-attraction-1: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("disconnect-vortices/removes-attraction-1: unknown outcome status {other:?}"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "disconnect-vortices/removes-attraction-1: produced diff differs from the committed 🔺️diff/🔣️.json");
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "disconnect-vortices/removes-attraction-1: committed diff JSON is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "disconnect-vortices/removes-attraction-1: committed diff did not carry before to after");
}
