//! 🧰️ `set-active` command.

use crate::editor::puzzle3d::modes::edit::windows::main::utilities;
use crate::editor::puzzle3d::{drive_precompute, puzzle3d_scene_active_utility, Puzzle3dActionCtx};
use dsl::os_pack::json::Value;

pub fn set_active(ctx: &mut Puzzle3dActionCtx<'_>, args: Option<&Value>) {
    let tool_id = args.and_then(|value| value.get("toolId")).and_then(|value| value.as_str()).filter(|id| !id.is_empty());
    ctx.scene.runtime.active_tool_id = tool_id.map(str::to_string);
    ctx.scene.active_utility = puzzle3d_scene_active_utility(&ctx.scene.runtime, ctx.view_state, Some(ctx.window_id));
    ctx.scene.runtime.suggestion_menu = None;
    ctx.scene.runtime.engagement_input = String::new();
    ctx.scene.runtime.brush_candidate_index = 0;
    if ctx.scene.active_utility == utilities::brush::UTILITY_ID {
        drive_precompute(&mut ctx.app.precompute.borrow_mut(), ctx.scene);
    }
}
