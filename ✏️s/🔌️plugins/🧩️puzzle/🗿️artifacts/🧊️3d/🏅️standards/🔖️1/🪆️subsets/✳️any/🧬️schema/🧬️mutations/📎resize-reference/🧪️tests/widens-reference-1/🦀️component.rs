//! 🧪️ `resize-reference` fixture — `widens-reference-1`.
//!
...
/// ▶️ The committed `resize-reference` payload carries `before` to exactly the committed `after`, and
/// lands the change this case is named for.
#[test]
fn applies_to_committed_after() {
    let mut snapshot = before();
    apply_puzzle3d_mutation(&mut snapshot, &mutation()).expect("resize-reference applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "resize-reference/widens-reference-1: applied state differs from committed after-snapshot");
...
/// ↩️ Applying `resize-reference` then the inverse it derives from `before` restores `before` exactly.
#[test]
...
/// 🔣️ Both committed snapshots and the committed `resize-reference` payload are already canonical:
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "resize-reference/widens-reference-1: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "resize-reference/widens-reference-1: committed mutation JSON is not canonical");
}

/// 🎯️ The declared outcome matches what `resize-reference` actually produces on this base.
#[test]
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "resize-reference/widens-reference-1: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "resize-reference/widens-reference-1: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "resize-reference/widens-reference-1: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("resize-reference/widens-reference-1: unknown outcome status {other:?}"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "resize-reference/widens-reference-1: produced diff differs from the committed 🔺️diff/🔣️component.json");
...
/// 🔣️ The committed `resize-reference` diff is itself canonical and decodes to `Puzzle3dDiff`.
#[test]
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "resize-reference/widens-reference-1: committed diff JSON is not canonical");
}

/// 🩹 Applying the committed `resize-reference` diff directly to `before` yields the committed `after` —
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "resize-reference/widens-reference-1: committed diff did not carry before to after");
}
