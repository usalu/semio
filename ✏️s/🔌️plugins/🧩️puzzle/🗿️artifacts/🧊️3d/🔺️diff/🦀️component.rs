...
use crate::artifacts::puzzle3d::diff::schema::{
...
//#region 🔖️DiffHelpers
/// 📍 Builds an objects-set delta, preserving insert index via `reordered` when the id is new.
pub fn diff_set_object(index: usize, object: Puzzle3dObject, base: &Puzzle3dSnapshot) -> Puzzle3dDiff {
    let mut delta = Puzzle3dObjectsDelta { added: vec![object.clone()], ..Default::default() };
    if puzzle3d_index_of(&base.objects, &object.id).is_none() {
        let mut order: Vec<String> = base.objects.iter().map(|entry| entry.id.clone()).collect();
        order.insert(index.min(order.len()), object.id.clone());
        delta.reordered = Some(order);
    }
    Puzzle3dDiff { objects: Some(delta), ..Default::default() }
}
pub fn diff_remove_object(id: String) -> Puzzle3dDiff {
    Puzzle3dDiff { objects: Some(Puzzle3dObjectsDelta { removed: vec![id], ..Default::default() }), ..Default::default() }
}
/// 📍 Builds an attractions-set delta, preserving insert index via `reordered` when the id is new.
pub fn diff_set_attraction(index: usize, attraction: Puzzle3dAttraction, base: &Puzzle3dSnapshot) -> Puzzle3dDiff {
    let mut delta = Puzzle3dAttractionsDelta { added: vec![attraction.clone()], ..Default::default() };
    if puzzle3d_index_of(&base.attractions, &attraction.id).is_none() {
        let mut order: Vec<String> = base.attractions.iter().map(|entry| entry.id.clone()).collect();
        order.insert(index.min(order.len()), attraction.id.clone());
        delta.reordered = Some(order);
    }
    Puzzle3dDiff { attractions: Some(delta), ..Default::default() }
}
pub fn diff_remove_attraction(id: String) -> Puzzle3dDiff {
    Puzzle3dDiff { attractions: Some(Puzzle3dAttractionsDelta { removed: vec![id], ..Default::default() }), ..Default::default() }
}
/// 📍 Builds a target-volumes-set delta, preserving insert index via `reordered` when the id is new.
pub fn diff_set_target_volume(index: usize, volume: Puzzle3dTargetVolume, base: &Puzzle3dSnapshot) -> Puzzle3dDiff {
    let mut delta = Puzzle3dTargetVolumesDelta { added: vec![volume.clone()], ..Default::default() };
    if puzzle3d_index_of(&base.target_volumes, &volume.id).is_none() {
        let mut order: Vec<String> = base.target_volumes.iter().map(|entry| entry.id.clone()).collect();
        order.insert(index.min(order.len()), volume.id.clone());
        delta.reordered = Some(order);
    }
    Puzzle3dDiff { target_volumes: Some(delta), ..Default::default() }
}
pub fn diff_remove_target_volume(id: String) -> Puzzle3dDiff {
    Puzzle3dDiff { target_volumes: Some(Puzzle3dTargetVolumesDelta { removed: vec![id], ..Default::default() }), ..Default::default() }
}
/// 📍 Builds a references-set delta, preserving insert index via `reordered` when the id is new.
pub fn diff_set_reference(index: usize, reference: Puzzle3dReference, base: &Puzzle3dSnapshot) -> Puzzle3dDiff {
    let mut delta = Puzzle3dReferencesDelta { added: vec![reference.clone()], ..Default::default() };
    if puzzle3d_index_of(&base.references, &reference.id).is_none() {
        let mut order: Vec<String> = base.references.iter().map(|entry| entry.id.clone()).collect();
        order.insert(index.min(order.len()), reference.id.clone());
        delta.reordered = Some(order);
    }
    Puzzle3dDiff { references: Some(delta), ..Default::default() }
}
pub fn diff_remove_reference(id: String) -> Puzzle3dDiff {
    Puzzle3dDiff { references: Some(Puzzle3dReferencesDelta { removed: vec![id], ..Default::default() }), ..Default::default() }
}
pub fn diff_set_meta(meta: crate::artifacts::puzzle3d::Puzzle3dMeta) -> Puzzle3dDiff {
    Puzzle3dDiff { meta: Some(meta), ..Default::default() }
}
pub fn diff_set_snapshot(snapshot: Puzzle3dSnapshot) -> Puzzle3dDiff {
    Puzzle3dDiff { artifact: Some(Box::new(Puzzle3dArtifact::from_snapshot(snapshot))), ..Default::default() }
}
pub(crate) fn puzzle3d_index_of<T>(items: &[T], id: &str) -> Option<usize>
where
    T: HasId,
{
...
impl HasId for crate::artifacts::puzzle3d::Puzzle3dObject { fn id(&self) -> &str { &self.id } }
impl HasId for crate::artifacts::puzzle3d::Puzzle3dAttraction { fn id(&self) -> &str { &self.id } }
impl HasId for crate::artifacts::puzzle3d::Puzzle3dTargetVolume { fn id(&self) -> &str { &self.id } }
impl HasId for crate::artifacts::puzzle3d::Puzzle3dReference { fn id(&self) -> &str { &self.id } }
