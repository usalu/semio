...
pub fn inverse(payload: &super::ReplaceFastenerGeometry, base: &Puzzle5dSnapshot) -> Vec<Puzzle5dMutation> {
    ...
    vec![crate::artifacts::puzzle5d::mutations::replace_fastener_geometry::replace_fastener_geometry(item.id.clone(), item.gap, item.shift, item.rise, item.rotation, item.turn, item.tilt, item.x, item.y)]
}
//#endregion 🔖️Inverse
