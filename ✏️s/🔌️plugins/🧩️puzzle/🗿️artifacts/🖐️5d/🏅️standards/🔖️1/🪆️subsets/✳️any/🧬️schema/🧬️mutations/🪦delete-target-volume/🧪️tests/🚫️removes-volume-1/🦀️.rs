...
//!
//! Removes the first of two constraining boxes; the second is untouched and keeps its index.
//!
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot.target_volumes.len(), 1, "delete-target-volume/removes-volume-1: exactly one volume must remain");
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(status, "applied", "delete-target-volume/removes-volume-1: this vector declares an applied outcome");
    let mut snapshot = before();
    apply_puzzle5d_mutation(&mut snapshot, &mutation()).expect("delete-target-volume/removes-volume-1: declared applied but the mutation was rejected");
}
...
fn produces_committed_diff() {
    ...
    assert!(committed["parts"].is_null(), "delete-target-volume/removes-volume-1: a target volume is not a part");
    assert!(committed["fasteners"].is_null(), "delete-target-volume/removes-volume-1: a target volume is not a fastener");
}
