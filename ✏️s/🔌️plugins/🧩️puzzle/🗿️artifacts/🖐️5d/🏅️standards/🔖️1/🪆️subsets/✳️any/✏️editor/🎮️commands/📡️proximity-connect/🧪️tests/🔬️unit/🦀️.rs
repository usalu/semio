    ...
    for case in fixture["cases"].as_array().expect("cases") {
        ...
        for _ in 0..rounds {
            for (index, part) in sampled.iter().enumerate() {
                ...
                let mut ctx = Puzzle5dActionCtx { scene: &mut scene, snapshot: &snapshot, instance_owner: None, window_id: "puzzle5d-3d", window_kind: "puzzle5d-3d", selection: &selection, abort: false };
