...
//!
//! Collapses a per-axis `[x, y, z]` extent to a single uniform factor — the `Puzzle5dScale` union's other arm.
//!
...
fn declared_outcome_holds() {
    ...
    assert_eq!(status, "applied", "scale-target-volume/per-axis-to-uniform: this vector declares an applied outcome");
    let mut snapshot = before();
    apply_puzzle5d_mutation(&mut snapshot, &mutation()).expect("scale-target-volume/per-axis-to-uniform: declared applied but the mutation was rejected");
}
...
fn produces_committed_diff() {
    ...
    assert!(committed["parts"].is_null(), "scale-target-volume/per-axis-to-uniform: a target volume is not a part");
    assert!(committed["fasteners"].is_null(), "scale-target-volume/per-axis-to-uniform: a target volume is not a fastener");
}
