...
/// 🔍️ `focusSelection`: recenters both cameras on the selection, preserving the volume camera's orbit
/// offset. With NOTHING selected it frames the whole document instead of completing silently — a camera
/// verb with no selection has an obvious subject (everything), and the silent no-op made the `f`
/// keybinding dead on every boot before the user had picked anything (ported from puzzle 3d's
/// `Puzzle3dFocusSelectionWork::frames`).
pub fn focus_selection(ctx: &mut Puzzle5dActionCtx<'_>) {
    let selected = ctx.selected_part_ids();
    let part_ids: Vec<String> = if selected.is_empty() { ctx.scene.document.parts.iter().map(|part| part.id.clone()).collect() } else { selected };
