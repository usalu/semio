//! 🧪️ `scale-part3d` fixture — `📏️uniform-double`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📏scale-part3d/📏️uniform-double/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📏scale-part3d/📏️uniform-double/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📏scale-part3d/📏️uniform-double/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📏scale-part3d/📏️uniform-double/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/📏scale-part3d/📏️uniform-double/🎯️outcome/🔣️.json");
...
fn applies_to_committed_after() {
    ...
    apply_puzzle5d_mutation(&mut snapshot, &mutation()).expect("scale-part3d applies to its committed before-snapshot");
    assert_eq!(snapshot, expected_after(), "scale-part3d/uniform-double: applied state differs from committed after-snapshot");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "scale-part3d/uniform-double: inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "scale-part3d/uniform-double: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "scale-part3d/uniform-double: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "scale-part3d/uniform-double: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "scale-part3d/uniform-double: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "scale-part3d/uniform-double: rejected mutation must leave the snapshot untouched");
        }
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "scale-part3d/uniform-double: produced diff differs from the committed 🔺️diff/🔣️.json");
}
