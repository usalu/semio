...
//#region 🔖️Inverse
pub fn inverse(payload: &super::ChangeEdgeKind, base: &Puzzle2dSnapshot) -> Vec<Puzzle2dMutation> {
    let Some(edge) = base.edges.iter().find(|entry| entry.id == payload.id) else {
        return Vec::new();
    };
    vec![crate::mutations::change_edge_kind::change_edge_kind(edge.id.clone(), edge.edge_kind.clone())]
}
//#endregion 🔖️Inverse
