...
//! routes through `serde_json::from_value` — `Puzzle2dSnapshot` only derives `Deserialize` under
...
//! snapshot — same shape the sibling `block2d` leaf already uses. NOT through
...
pub fn deserialize(from: &JsonSnapshot) -> Result<Puzzle2dSnapshot, store::TextError> {
    ...
    let snap: Puzzle2dSnapshot = dsl::FromValue::from_value(raw).map_err(|e| store::TextError::new(format!("puzzle2d<-json: {e}"), dsl::TextSpan::at(1, 1)))?;
    Ok(snap)
}
