//! 🧪️ `replace-edge-geometry` fixture — `🧮️reposes-a-capsule-door-edge`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧮replace-edge-geometry/🧮️reposes-a-capsule-door-edge/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧮replace-edge-geometry/🧮️reposes-a-capsule-door-edge/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧮replace-edge-geometry/🧮️reposes-a-capsule-door-edge/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧮replace-edge-geometry/🧮️reposes-a-capsule-door-edge/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🧮replace-edge-geometry/🧮️reposes-a-capsule-door-edge/🎯️outcome/🔣️.json");
...
/// ▶️ The committed `replace-edge-geometry` payload carries `before` to exactly the committed `after`.
#[test]
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "replace-edge-geometry/reposes-a-capsule-door-edge: applied state differs from committed after-snapshot");
    assert_eq!(snapshot.nodes.len(), 12, "replace-edge-geometry/reposes-a-capsule-door-edge: the applied board must hold 12 node(s)");
    assert_eq!(snapshot.edges.len(), 10, "replace-edge-geometry/reposes-a-capsule-door-edge: the applied board must hold 10 edge(s)");
}
...
fn inverse_restores_before() {
    ...
    assert_eq!(inverse.len(), 1, "replace-edge-geometry/reposes-a-capsule-door-edge: this kind undoes in exactly 1 step(s)");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-edge-geometry/reposes-a-capsule-door-edge: committed {label} JSON is not canonical");
    }
    ...
    assert_eq!(reencoded, original, "replace-edge-geometry/reposes-a-capsule-door-edge: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(serde_json::Value::as_str), Some("applied"), "replace-edge-geometry/reposes-a-capsule-door-edge declares an applied outcome");
    let produced = <Puzzle2dMutation as protocol::Mutation<Puzzle2dSnapshot>>::diff(&mutation(), &before());
    assert!(produced.messages().is_empty(), "replace-edge-geometry/reposes-a-capsule-door-edge: an applied vector carries no diagnostic, got {:?}", produced.messages());
}

/// 🔺️ The sparse delta `replace-edge-geometry` produces is exactly the committed diff — it pins WHICH
...
fn produces_committed_diff() {
    ...
    assert!(committed["edges"]["patched"][0]["patch"]["replacement"].is_object(), "replace-edge-geometry/reposes-a-capsule-door-edge: a patch entry carries the whole replacement record");
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "replace-edge-geometry/reposes-a-capsule-door-edge: committed diff JSON is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "replace-edge-geometry/reposes-a-capsule-door-edge: committed diff did not carry before to after");
}
