...
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🖇️change-edge-tips/🔀️swaps-edge-1-tips/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🖇️change-edge-tips/🔀️swaps-edge-1-tips/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🖇️change-edge-tips/🔀️swaps-edge-1-tips/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🖇️change-edge-tips/🔀️swaps-edge-1-tips/🎯️outcome/🔣️.json");
...
/// ▶️ The committed `change-edge-tips` payload carries `before` to exactly the committed `after`, and
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "change-edge-tips/swaps-edge-1-tips: inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-edge-tips/swaps-edge-1-tips: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "change-edge-tips/swaps-edge-1-tips: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "change-edge-tips/swaps-edge-1-tips: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "change-edge-tips/swaps-edge-1-tips: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "change-edge-tips/swaps-edge-1-tips: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("change-edge-tips/swaps-edge-1-tips: unknown outcome status {other:?}"),
    }
}
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "change-edge-tips/swaps-edge-1-tips: committed diff JSON is not canonical");
}
