...
use crate::editor::puzzle2d::engine::{BoardHost, HandleDescJson, NodeDescJson, SceneDescriptorJson, handle_position_on_circle};
...
fn close_fill_job(mut job: BoardFillJob) {
    ...
    for _ in 0..FILL_TEST_PUMP_LIMIT {
        if matches!(semio_framework_job::InteractiveJob::close_step(&mut job, 1, semio_framework_job::JOB_PAYLOAD_PAGE_BYTES,), semio_framework_job::InteractiveJobCloseStep::Complete) && semio_framework_job::InteractiveJob::terminal_is_empty(&job) {
            return;
        }
    }
...
fn board_fill_worker_saturation_returns_exact_session_for_close() {
    ...
    let saturated =
        matches!(session.pump_one(&pool, semio_framework_async::Lane::Background), Err(semio_framework_job::MountedWorkerJobPumpFault::Submit(semio_framework_job::WorkerJobSubmitFault::Pool(semio_framework_async::WorkerSubmitErrorKind::Saturated))));
...
fn board_host_brush_candidates_sorted_by_handle_proximity() {
    ...
    let candidates = v.as_array().and_then(|rows| rows.iter().find(|row| row.get("name").and_then(|n| n.as_str()) == Some("brushCandidates")).and_then(|row| row.get("payload")).and_then(|p| p.get("candidates")).and_then(|c| c.as_array()).cloned());
