//! 🧪️ `connect-handles` fixture — `🪢️adds-second-edge`.
//!
...
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪢️connect-handles/🪢️adds-second-edge/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪢️connect-handles/🪢️adds-second-edge/🦠️mutation/🔣️.json");
const DIFF: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪢️connect-handles/🪢️adds-second-edge/🔺️diff/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🪢️connect-handles/🪢️adds-second-edge/🎯️outcome/🔣️.json");
...
/// ▶️ The committed `connect-handles` payload carries `before` to exactly the committed `after`, and
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "connect-handles/adds-second-edge: inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "connect-handles/adds-second-edge: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "connect-handles/adds-second-edge: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "connect-handles/adds-second-edge: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "connect-handles/adds-second-edge: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "connect-handles/adds-second-edge: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("connect-handles/adds-second-edge: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta `connect-handles` produces is exactly the committed diff — the single most
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "connect-handles/adds-second-edge: produced diff differs from the committed 🔺️diff/🔣️.json");
