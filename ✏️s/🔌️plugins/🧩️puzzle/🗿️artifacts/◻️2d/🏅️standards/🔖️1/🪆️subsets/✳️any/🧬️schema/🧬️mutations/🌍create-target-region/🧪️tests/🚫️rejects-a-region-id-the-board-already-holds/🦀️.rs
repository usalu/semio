//! 🧪️ `create-target-region` fixture — `🚫️rejects-a-region-id-the-board-already-holds`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌍create-target-region/🚫️rejects-a-region-id-the-board-already-holds/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌍create-target-region/🚫️rejects-a-region-id-the-board-already-holds/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌍create-target-region/🚫️rejects-a-region-id-the-board-already-holds/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌍create-target-region/🚫️rejects-a-region-id-the-board-already-holds/🎯️outcome/🔣️.json");
const DIFF_ABSENT: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌍create-target-region/🚫️rejects-a-region-id-the-board-already-holds/🔺️diff/🚫️.absent");
...
fn rejection_leaves_the_document_at_the_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "create-target-region/rejects-a-region-id-the-board-already-holds: applied state differs from committed after-snapshot");
    assert_eq!(expected_after(), before(), "create-target-region/rejects-a-region-id-the-board-already-holds: a rejected vector's two committed snapshots must be identical");
}
...
fn the_refusal_is_the_declared_one() {
    assert!(DIFF_ABSENT.is_empty(), "create-target-region/rejects-a-region-id-the-board-already-holds: the D6 sentinel 🔺️diff/🚫️.absent must stay empty");
    let produced = <Puzzle2dMutation as protocol::Mutation<Puzzle2dSnapshot>>::diff(&mutation(), &before());
    assert_eq!(produced.diff(), &crate::standards::v1::subsets::any::schema::diff::Puzzle2dDiff::default(), "create-target-region/rejects-a-region-id-the-board-already-holds: a refusing diff builder answers the default diff, never a half-built delta");
...
fn committed_json_is_canonical() {
    ...
    assert_eq!(reencoded, original, "create-target-region/rejects-a-region-id-the-board-already-holds: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(serde_json::Value::as_str), Some("rejected"), "create-target-region/rejects-a-region-id-the-board-already-holds declares a rejected outcome");
    let produced = <Puzzle2dMutation as protocol::Mutation<Puzzle2dSnapshot>>::diff(&mutation(), &before());
    let message = produced.messages().first().expect("a rejected outcome carries a diagnostic");
    assert_eq!(outcome.get("code").and_then(serde_json::Value::as_str), Some(message.code.0.as_str()), "create-target-region/rejects-a-region-id-the-board-already-holds: the declared code must match the emitted one");
    let declared_path: Vec<String> = outcome.get("path").and_then(serde_json::Value::as_array).expect("a rejected outcome declares a path").iter().map(|entry| entry.as_str().expect("path segments are strings").to_string()).collect();
    assert_eq!(declared_path, message.target, "create-target-region/rejects-a-region-id-the-board-already-holds: the declared path must match the emitted target");
}
