//! 🧪️ `create-node` fixture — `🚫️rejects-a-capsule-id-the-tower-already-holds`.
//!
...
const BEFORE: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-node/🚫️rejects-a-capsule-id-the-tower-already-holds/📸️snapshot/⬅️before/🔣️.json");
const AFTER: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-node/🚫️rejects-a-capsule-id-the-tower-already-holds/📸️snapshot/➡️after/🔣️.json");
const MUTATION: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-node/🚫️rejects-a-capsule-id-the-tower-already-holds/🦠️mutation/🔣️.json");
const OUTCOME: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-node/🚫️rejects-a-capsule-id-the-tower-already-holds/🎯️outcome/🔣️.json");
const DIFF_ABSENT: &str = include_str!("../../../../../🧫️fixtures/🧬️mutations/🌱create-node/🚫️rejects-a-capsule-id-the-tower-already-holds/🔺️diff/🚫️.absent");
...
/// ▶️ A refused `create-node` still applies cleanly — its diff is the default one — and leaves the
...
fn rejection_leaves_the_document_at_the_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "create-node/rejects-a-capsule-id-the-tower-already-holds: applied state differs from committed after-snapshot");
    assert_eq!(expected_after(), before(), "create-node/rejects-a-capsule-id-the-tower-already-holds: a rejected vector's two committed snapshots must be identical");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "create-node/rejects-a-capsule-id-the-tower-already-holds: committed {label} JSON is not canonical");
    }
    let reencoded = serde_json::to_value(mutation()).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "create-node/rejects-a-capsule-id-the-tower-already-holds: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    assert_eq!(outcome.get("status").and_then(serde_json::Value::as_str), Some("rejected"), "create-node/rejects-a-capsule-id-the-tower-already-holds declares a rejected outcome");
    let produced = <Puzzle2dMutation as protocol::Mutation<Puzzle2dSnapshot>>::diff(&mutation(), &before());
    let message = produced.messages().first().expect("a rejected outcome carries a diagnostic");
    assert_eq!(outcome.get("code").and_then(serde_json::Value::as_str), Some(message.code.0.as_str()), "create-node/rejects-a-capsule-id-the-tower-already-holds: the declared code must match the emitted one");
    let declared_path: Vec<String> = outcome.get("path").and_then(serde_json::Value::as_array).expect("a rejected outcome declares a path").iter().map(|entry| entry.as_str().expect("path segments are strings").to_string()).collect();
    assert_eq!(declared_path, message.target, "create-node/rejects-a-capsule-id-the-tower-already-holds: the declared path must match the emitted target");
}
