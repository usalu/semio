...
//#region 🔖️Inverse
pub fn inverse(payload: &super::DeleteNode, base: &Puzzle2dSnapshot) -> Vec<Puzzle2dMutation> {
    ...
    let mut mutations = vec![crate::artifacts::puzzle2d::mutations::create_node::create_node(node.clone(), index)];
    for edge in base.edges.iter().filter(|edge| handle_ids.contains(&edge.source.as_str()) || handle_ids.contains(&edge.target.as_str())) {
        mutations.push(crate::artifacts::puzzle2d::mutations::connect_handles::connect_handles(
            edge.id.clone(),
            edge.source.clone(),
            edge.target.clone(),
