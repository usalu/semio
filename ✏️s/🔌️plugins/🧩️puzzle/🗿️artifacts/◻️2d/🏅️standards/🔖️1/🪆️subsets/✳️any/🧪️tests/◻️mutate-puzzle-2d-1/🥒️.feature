...
Feature: Apply every typed puzzle2d board mutation twice — once in Rust, once in Python — and require the same answer
  ...
  Scenario Outline: The committed <id> vector declares its own kind and moves the document
    ...
    Examples:
      | id                            | vector                                                          |
      | create-node                   | 🌱create-node/🧪️tests/🌱️appends-node-c                             |
      | delete-node                   | 🗑️delete-node/🧪️tests/🚫️removes-node-a-and-severs-edge             |
      | move-node                     | 📍move-node/🧪️tests/📍️moves-node-a                                 |
      | replace-node-geometry         | 🧊replace-node-geometry/🧪️tests/🔳️circle-to-rectangle              |
      | change-node-kind              | 🏗️change-node-kind/🧪️tests/🏷️reassigns-node-a-kind                 |
      | edit-node-text                | ✏️edit-node-text/🧪️tests/✏️retitles-node-a                        |
      | change-node-icon              | 🎨change-node-icon/🧪️tests/🎨️swaps-node-a-icon                     |
      | scale-node                    | 📏scale-node/🧪️tests/📏️doubles-node-a                              |
      | change-node-visible           | 👁️change-node-visible/🧪️tests/🙈️hides-node-a                       |
      | change-node-locked            | 🔒change-node-locked/🧪️tests/🔒️locks-node-a                        |
      | change-node-root              | 🌟change-node-root/🧪️tests/🌳️promotes-node-a-to-root               |
      | change-node-anchor            | ⚓change-node-anchor/🧪️tests/⚓️fixed-to-derived                    |
      | add-node-handle               | ➕add-node-handle/🧪️tests/➕️appends-handle-3-to-node-b             |
      | remove-node-handle            | ➖remove-node-handle/🧪️tests/🚫️removes-handle-2-and-severs-edge    |
      | replace-node-handle           | 🔌replace-node-handle/🧪️tests/⏸️rekind-handle-1-is-noop            |
      | connect-handles               | 🪢️connect-handles/🧪️tests/🪢️adds-second-edge                       |
      | disconnect-handles            | ✂️disconnect-handles/🧪️tests/🚫️removes-edge-1                     |
      | replace-edge-geometry         | 🧮replace-edge-geometry/🧪️tests/📍️repositions-edge-1               |
      | change-edge-kind              | 🏷️change-edge-kind/🧪️tests/🏷️rekinds-edge-1                        |
      | change-edge-tips              | 🖇️change-edge-tips/🧪️tests/🔀️swaps-edge-1-tips                     |
      | change-edge-visible           | 👀change-edge-visible/🧪️tests/🙈️hides-edge-1                       |
      | change-edge-locked            | 🔐change-edge-locked/🧪️tests/🔒️locks-edge-1                        |
      | change-manifest-id            | 🆔change-manifest-id/🧪️tests/📦️repoints-manifest                   |
      | connect-kind-compatibility    | 🤝connect-kind-compatibility/🧪️tests/🤝️adds-handle-kind-pair       |
      | disconnect-kind-compatibility | 💔disconnect-kind-compatibility/🧪️tests/🚫️removes-handle-kind-pair |
      | replace-kind-catalogs         | 📚replace-kind-catalogs/🧪️tests/📇️installs-handle-kind-catalog     |
