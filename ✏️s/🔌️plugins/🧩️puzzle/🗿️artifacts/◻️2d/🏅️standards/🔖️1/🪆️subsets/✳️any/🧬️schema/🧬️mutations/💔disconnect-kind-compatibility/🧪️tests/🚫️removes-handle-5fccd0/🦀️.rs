//! 🧪️ `disconnect-kind-compatibility` fixture — `🚫️removes-handle-kind-pair`.
//!
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "disconnect-kind-compatibility/removes-handle-kind-pair: applied state differs from committed after-snapshot");
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "disconnect-kind-compatibility/removes-handle-kind-pair: inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "disconnect-kind-compatibility/removes-handle-kind-pair: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "disconnect-kind-compatibility/removes-handle-kind-pair: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "disconnect-kind-compatibility/removes-handle-kind-pair: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "disconnect-kind-compatibility/removes-handle-kind-pair: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "disconnect-kind-compatibility/removes-handle-kind-pair: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("disconnect-kind-compatibility/removes-handle-kind-pair: unknown outcome status {other:?}"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "disconnect-kind-compatibility/removes-handle-kind-pair: produced diff differs from the committed 🔺️diff/🔣️.json");
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "disconnect-kind-compatibility/removes-handle-kind-pair: committed diff JSON is not canonical");
}
...
fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "disconnect-kind-compatibility/removes-handle-kind-pair: committed diff did not carry before to after");
}
