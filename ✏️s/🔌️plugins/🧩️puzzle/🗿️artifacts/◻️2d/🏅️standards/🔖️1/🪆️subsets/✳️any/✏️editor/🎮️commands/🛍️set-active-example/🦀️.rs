...
pub fn step_active_example(doc: &Puzzle2dPlaySnapshot, config: &Puzzle2dConfig, args: Option<&Value>) -> Emit<Puzzle2dMutation, Puzzle2dConfigMutation> {
    ...
    match stage {
        ...
        STAGE_CLEAR_COMPATIBILITY => {
            ...
            let mutations = rows.iter().take(MAX_MUTATIONS_PER_STEP).filter_map(|row| Puzzle2dKindCompatibility::from_value(dsl::DslValue::from(row)).ok()).map(|row| disconnect_kind_compatibility(row.source, row.target)).collect();
        ...
        STAGE_CATALOGS => {
            let current = doc.0.get("meta").and_then(|meta| meta.get("kindCatalogs")).map(dsl::DslValue::from);
            let target_value = target.meta.kind_catalogs.as_ref().map(dsl::ToValue::to_value);
            let mutation = (current != target_value).then(|| replace_kind_catalogs(target.meta.kind_catalogs.clone()));
