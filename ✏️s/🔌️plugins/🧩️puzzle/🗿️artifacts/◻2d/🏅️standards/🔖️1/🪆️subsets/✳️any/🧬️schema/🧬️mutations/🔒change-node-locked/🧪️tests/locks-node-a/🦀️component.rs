//! 🧪️ `change-node-locked` fixture — `locks-node-a`.
//!
...
/// ▶️ The committed `change-node-locked` payload carries `before` to exactly the committed `after`, and
...
async fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "change-node-locked/locks-node-a: applied state differs from committed after-snapshot");
...
async fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "change-node-locked/locks-node-a: inverse did not restore the before-snapshot");
}
...
async fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "change-node-locked/locks-node-a: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "change-node-locked/locks-node-a: committed mutation JSON is not canonical");
}
...
async fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "change-node-locked/locks-node-a: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "change-node-locked/locks-node-a: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "change-node-locked/locks-node-a: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("change-node-locked/locks-node-a: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta `change-node-locked` produces is exactly the committed diff — the single most
...
async fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "change-node-locked/locks-node-a: produced diff differs from the committed 🔺️diff/🔣️component.json");
...
async fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "change-node-locked/locks-node-a: committed diff JSON is not canonical");
}
...
async fn committed_diff_applies_to_after() {
    ...
    assert_eq!(produced, expected_after(), "change-node-locked/locks-node-a: committed diff did not carry before to after");
}
