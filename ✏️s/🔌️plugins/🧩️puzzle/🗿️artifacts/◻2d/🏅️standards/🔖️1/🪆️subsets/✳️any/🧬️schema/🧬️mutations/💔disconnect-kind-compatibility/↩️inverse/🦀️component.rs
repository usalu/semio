...
//#region 🔖️Inverse
pub fn inverse(payload: &super::mutation::DisconnectKindCompatibility, base: &Puzzle2dSnapshot) -> Vec<Puzzle2dMutation> {
    let Some(row) = base.meta.kind_compatibility.iter().find(|row| row.source == payload.source && row.target == payload.target) else {
        return Vec::new();
    };
    vec![crate::artifacts::puzzle2d::mutations::connect_kind_compatibility::mutation::connect_kind_compatibility(
