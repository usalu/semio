//! 🪆️ Subset root for `s.puzzle.puzzle2d@1/*` (ticket `26/08/17/MICROKERNEL-POOLED-ACTOR-PLUGIN-
...
//! `🧬️schema`/`🚪️io`/`👁️viewer`/`✏️editor`/`📚️examples` children — `crate::editor::puzzle2d`/
...
//! plugin-root shim: puzzle2d's `📦️glue.rs` mounts examples at the CRATE ROOT
...
//! `io() -> IoDeclaration` function their migration added to `🚪️io/🦀️component.rs`. puzzle2d's own
//! `🚪️io/🦀️component.rs` is still on the OLD `ComposerEntry`/`io_registry` channel (six composer
//! rows: native, svg, pdf, png, dwg, dxf — see the artifact root's `definition()`), and
...
async fn inference_descriptors() -> &'static [::semio_framework_schema::ArtifactInferenceDescriptor] {
    ...
    DESCRIPTORS.get_or_init(|| vec![schema::inferences::puzzle2d_artifact_inference_descriptor()]).as_slice()
}
...
async fn io_declaration() -> IoDeclaration {
    ...
    IoDeclaration {
        native: NativeCodecs {
            ...
            inferences: None,
            codec: store::ArtifactCodec::of::<Puzzle2dSnapshot, Puzzle2dMutation>(PUZZLE_2D_SCHEMA.to_string()),
        },
        entries: &[],
    }
}
...
pub async fn subset() -> SubsetDeclaration {
    SubsetDeclaration {
        ...
        io: io_declaration(),
        viewer: viewer_surface::<viewer::Puzzle2dViewer>(viewer::create_puzzle2d_viewer()),
        editor: editor_surface::<editor::Puzzle2dPlayApp>(editor::create_puzzle2d_app()),
        examples: examples(),
    }
}
