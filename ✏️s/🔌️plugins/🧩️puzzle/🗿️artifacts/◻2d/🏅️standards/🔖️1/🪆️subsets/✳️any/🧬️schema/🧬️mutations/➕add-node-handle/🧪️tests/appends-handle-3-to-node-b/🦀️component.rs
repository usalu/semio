//! 🧪️ `add-node-handle` fixture — `appends-handle-3-to-node-b`.
//!
...
/// ▶️ The committed `add-node-handle` payload carries `before` to exactly the committed `after`, and
...
fn applies_to_committed_after() {
    ...
    assert_eq!(snapshot, expected_after(), "add-node-handle/appends-handle-3-to-node-b: applied state differs from committed after-snapshot");
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "add-node-handle/appends-handle-3-to-node-b: inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "add-node-handle/appends-handle-3-to-node-b: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "add-node-handle/appends-handle-3-to-node-b: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "add-node-handle/appends-handle-3-to-node-b: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "add-node-handle/appends-handle-3-to-node-b: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "add-node-handle/appends-handle-3-to-node-b: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("add-node-handle/appends-handle-3-to-node-b: unknown outcome status {other:?}"),
    }
}

/// 🔺️ The sparse delta `add-node-handle` produces is exactly the committed diff — the single most
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "add-node-handle/appends-handle-3-to-node-b: produced diff differs from the committed 🔺️diff/🔣️component.json");
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "add-node-handle/appends-handle-3-to-node-b: committed diff JSON is not canonical");
}
