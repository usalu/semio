//! 🧪️ `replace-node-geometry` fixture — `circle-to-rectangle`.
//!
...
fn inverse_restores_before() {
    ...
    assert_eq!(snapshot, base, "replace-node-geometry/circle-to-rectangle: inverse did not restore the before-snapshot");
}
...
fn committed_json_is_canonical() {
    for (label, text) in [("before", BEFORE), ("after", AFTER)] {
        ...
        assert_eq!(reencoded, original, "replace-node-geometry/circle-to-rectangle: committed {label} JSON is not canonical");
    }
    let decoded_mutation = mutation();
    let reencoded = serde_json::to_value(&decoded_mutation).expect("mutation encodes");
    let original: serde_json::Value = serde_json::from_str(MUTATION).expect("mutation reparses");
    assert_eq!(reencoded, original, "replace-node-geometry/circle-to-rectangle: committed mutation JSON is not canonical");
}
...
fn declared_outcome_holds() {
    ...
    match status {
        "applied" => assert!(applied, "replace-node-geometry/circle-to-rectangle: declared applied but the mutation was rejected"),
        "rejected" => {
            assert!(!applied, "replace-node-geometry/circle-to-rectangle: declared rejected but the mutation applied");
            assert_eq!(snapshot, before(), "replace-node-geometry/circle-to-rectangle: rejected mutation must leave the snapshot untouched");
        }
        other => panic!("replace-node-geometry/circle-to-rectangle: unknown outcome status {other:?}"),
    }
}
...
fn produces_committed_diff() {
    ...
    assert_eq!(produced, committed, "replace-node-geometry/circle-to-rectangle: produced diff differs from the committed 🔺️diff/🔣️component.json");
...
fn committed_diff_is_canonical() {
    ...
    assert_eq!(reencoded, original, "replace-node-geometry/circle-to-rectangle: committed diff JSON is not canonical");
}
