...
impl protocol::MutationDiff<Puzzle2dPresence> for Puzzle2dPresence {
    async fn apply(&self, _base: &Puzzle2dPresence) -> protocol::MutationApplyResult<Puzzle2dPresence> {
        Ok({
            self.clone()
        })
    }
...
impl Mutation<Puzzle2dPresence> for Puzzle2dPresenceMutation {
    ...
    async fn diff(&self, _base: &Puzzle2dPresence) -> protocol::MutationOutcome<Puzzle2dPresence> {
        protocol::MutationOutcome::new(match self {
            Self::Snapshot { presence } => presence.clone(),
        })
    }

    async fn inverse(&self, base: &Puzzle2dPresence) -> Vec<Self> {
