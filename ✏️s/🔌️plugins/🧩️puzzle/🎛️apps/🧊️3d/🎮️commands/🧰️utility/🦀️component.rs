//! 🧰️ Puzzle 3d play app commands — the utility/tool switch. B1: these Commands ARE the switch now
//! (the host no longer owns an ambient `view_state.active_utility_id`/`active_tool_id` — the config
//! does), so this arm writes the new value itself, then clears any in-progress gumball scratch and
//! transient hover/suggestion state and refreshes the placement engine when the new mode needs it.
