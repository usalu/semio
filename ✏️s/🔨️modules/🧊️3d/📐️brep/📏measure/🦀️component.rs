//! 📏 Divergence-theorem mass properties, axis-aligned bounds, and solid distance queries on [`crate::brep::topo::Body`].

use crate::brep::arena::{CoedgeId, EdgeId, FaceId, SolidId, VertexId};
use crate::brep::curve::Curve3;
use crate::brep::curve_ops;
use crate::brep::error::KernelError;
use crate::brep::surface::Surface;
use crate::brep::surface_ops;
...
fn loop_area(body: &Body, face: FaceId, loop_id: crate::brep::arena::LoopId, _chord_tol: f64) -> Result<f64, KernelError> {
...
fn loop_volume_contribution(body: &Body, face: FaceId, loop_id: crate::brep::arena::LoopId, chord_tol: f64) -> Result<f64, KernelError> {
    let (sv, _, _, _) = loop_volume_moments(body, face, loop_id, chord_tol)?;
    Ok(sv / 6.0)
}

fn loop_volume_moments(body: &Body, face: FaceId, loop_id: crate::brep::arena::LoopId, chord_tol: f64) -> Result<(f64, f64, f64, f64), KernelError> {
    let surface = face_surface(body, face)?;
    let flipped = body.faces.get(face).map(|f| f.flipped).unwrap_or(false);
    match surface {
        Surface::Plane { .. } => {
...
fn loop_positions(body: &Body, loop_id: crate::brep::arena::LoopId) -> Result<Vec<Pnt3>, KernelError> {
    let mut pts = Vec::new();
...
fn outward_plane_normal(frame: &crate::brep::mat::Frame3, flipped: bool) -> Vec3 {
    let mut n = frame.z;
    if flipped {
        n = -n;
    }
...
fn loop_uv_bounds(_body: &Body, _face: FaceId, loop_id: crate::brep::arena::LoopId, surface: &Surface) -> Result<(f64, f64, f64, f64), KernelError> {
...
fn loop_uv_polygon(body: &Body, loop_id: crate::brep::arena::LoopId, surface: &Surface) -> Result<Vec<Pnt2>, KernelError> {
...
mod tests {
    ...
    fn insert_edge(body: &mut Body, curve: crate::brep::arena::Curve3Id, range: (f64, f64), v0: VertexId, v1: VertexId) -> EdgeId {
        let label = body.new_label();
        body.edges.insert(Edge { curve, range, v0, v1, tol: Tol::DEFAULT, label })
    }

    fn make_quad_loop(body: &mut Body, face: FaceId, corners: [Pnt3; 4]) -> crate::brep::arena::LoopId {
