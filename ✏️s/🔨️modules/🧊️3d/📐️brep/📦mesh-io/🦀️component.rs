...
use crate::brep::primitives::make_planar_face_from_points;
...
/// 📦 Builds a [`TriangleMesh`] from a tessellation [`MeshTransfer`].
pub fn triangle_mesh_from_transfer(transfer: &MeshTransfer) -> TriangleMesh {
    let vertex_count = transfer.position.len() / 3;
...
/// 📦 Decodes STL (binary or ASCII) and builds one solid (planar triangle faces).
pub fn import_stl_to_body(body: &mut Body, data: &[u8], tolerance: f64) -> Result<SolidId, KernelError> {
    let mesh = import_stl(data)?;
    import_triangle_mesh_to_body(body, &mesh, tolerance)
}

/// 📦 Tessellates `solid` and returns OBJ text.
...
/// 📦 Ingests OBJ text into a single solid.
pub fn import_obj_to_body(body: &mut Body, text: &str, tolerance: f64) -> Result<SolidId, KernelError> {
    let mesh = import_obj(text)?;
    import_triangle_mesh_to_body(body, &mesh, tolerance)
}

/// 📦 Tessellates `solid` and encodes GLB via [`GlbExporter`].
...
/// 📦 Decodes GLB via [`GlbImporter`] and builds a solid.
pub fn import_glb_to_body(body: &mut Body, data: &[u8], tolerance: f64) -> Result<SolidId, KernelError> {
    let mesh = import_glb(data)?;
    import_triangle_mesh_to_body(body, &mesh, tolerance)
}

/// 📦 Tessellates `solid` and encodes the hand-rolled DWG mesh bridge (matches kernel `export_dwg_sync`).
pub fn export_solid_dwg(body: &Body, solid: SolidId, deflection: f64) -> Result<Vec<u8>, KernelError> {
    let transfer = tessellate_solid(body, solid, deflection.max(1e-4))?;
    export_dwg(&triangle_mesh_from_transfer(&transfer))
}

/// 📦 Decodes DWG bytes to a triangle mesh solid (matches kernel `import_dwg_sync`).
pub fn import_dwg_to_body(body: &mut Body, data: &[u8], tolerance: f64) -> Result<SolidId, KernelError> {
    let mesh = import_dwg(data)?;
    import_triangle_mesh_to_body(body, &mesh, tolerance)
}
