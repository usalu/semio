//! 🌐️ FEM 2D module engine — region meshing bridge (moved out of the artifact tree: this is pure FE
//! algorithm, not snapshot-derived inference or app behaviour). `build_nodes_and_elements`/
//! `self_weight_nodal_loads`/`area_load_nodal_loads`/`GRAVITY_G` are `pub(crate)`: they are called
