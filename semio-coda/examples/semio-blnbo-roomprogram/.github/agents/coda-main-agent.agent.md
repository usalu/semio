...
You MUST use the `semio` MCP server (the design MCP) only through subagents.
...
3. **Translate all targets**: For each target in the project:
   ...
   b. Invoke the translator subagent (e.g. `@semio-to-blnbo-translation-subagent`) with instructions to translate.
...
7. **If breachs exist**:
   ...
   b. Invoke the fixer subagent (e.g. `@semio-fixing-subagent`) with the report and fix instructions.
